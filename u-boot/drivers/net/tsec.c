...
static int tsec_bind(struct udevice *dev)
{
	char name[16];
	...
	if (ofnode_valid(dev->node))
		sprintf(name, "eTSEC-%u", PCI_FUNC(pci_get_devfn(dev)));
	else
#endif
	sprintf(name, "eTSEC%u", eth_num_devices++);
...
#if 1/*debug by balkrow */
	debug("[%s:%d] ecntrl=%x\n", __FUNCTION__, __LINE__, ecntrl);
#endif
	ecntrl &= ~ECNTRL_R100;
...
#if 1/*debug by balkrow */
	debug("[%s:%d] ecntrl=%x,speed=%x,maccfg2=%x\n", __FUNCTION__, __LINE__, ecntrl, phydev->speed, maccfg2);
#endif
...
#endif
{
	...
	for (i = 0;
		...
		if (i >= TOUT_LOOP) {
#if 0
			printf("%s: tsec: tx error (%x)\n", dev->name, in_be16(&priv->txbd[priv->tx_idx].status));
#else
...
#endif
{
	 ...
	 */
#if 0/*debug by balkrow*/
 printf("\ntsec_init: dev[%s] %02x:%02x:%02x:%02x:%02x:%02x\n", dev->name, pdata->enetaddr[0],
                pdata->enetaddr[1],
                pdata->enetaddr[2],
                pdata->enetaddr[3],
                pdata->enetaddr[4],
