...
function initSystem(params)
    ...
    if params.forceReset ~= nil then
        ret,val = myGenWrapper("appDemoDbEntryAdd",{
            {"IN","string","namePtr","forceSoftResetInInit"},
            {"IN","GT_U32","value",1}
        })
    end
...
local initSystemParams_named_params =
{
    ...
    { format="forceReset",name="forceReset",  help="force PP reset during system initialization"},
}
...
-- enabling fastboot
    ...
    if params.insertionType == nil then
        ...
        ret,val = myGenWrapper("cpssPpInsert",{
            ...
            { "IN", "GT_U32", "busNum",         params.busNum },
...
function cpssPciRemove(params)
        ret,val = myGenWrapper("cpssPciRemove",{
            { "IN", "GT_U32",   "pciBus",   params.busNum },
...
function CLI_addAppDemoCommands()
    ...
    if ret == 0 then
        ret,val = myGenWrapper("cpssEnablerIsMixedModePlat",{})
        if ret ~= 0 then
           return
        end
    end
    ...
    CLI_addCommand("exec", "cpssPpInsert",
    {
        ...
        params={{type="named",
              {format="busType %GT_U8",              name="busType",      help="Interface type connected to CPU. PCI -> 0. SMI -> 1"},
                  {format="busNum %GT_U32",          name="busNum",       help="PCI/SMI Bus Number"},
    ...
    CLI_addCommand("exec", "change_numbers_format",
    {
        func=change_base,
        help="change numbers format",
        params={{type="named",
                  {format="base %GT_U8", name="base"},
               }}
    })
    ...
    CLI_addCommand("exec", "cpssPciRemove",
    {
        ...
        params={{type="named",
                  {format="pciBus %GT_U32", name="busNum", help="PCI Bus Number"},
