...
cmdLuaCLI_registerCfunction("wrlOsSetStdOutRedirectionFunction")
...
local help_redirect_net_stdout = "Redirects stdout back to the network console, such as telnet"
CLI_addHelp("exec", "shell-redirect-net-stdout", help_redirect_net_stdout)
CLI_addCommand("exec", "shell-redirect-net-stdout",
    { func = function(params)
            return wrlOsSetStdOutRedirectionFunction(true)
        end,
      help = help_redirect_net_stdout
    });

local help_no_redirect_net_stdout = "Stop redirecting stdout back to the network console, such as telnet"
CLI_addHelp("exec", "no shell-redirect-net-stdout", help_no_redirect_net_stdout)
CLI_addCommand("exec", "no shell-redirect-net-stdout",
    { func = function(params)
            return wrlOsSetStdOutRedirectionFunction(false)
        end,
      help = help_no_redirect_net_stdout
    });
...
-- function allow to execute any C functions with it's parameters and view output in TCP socket
local function xps_api_call(params)
    -- strip "dbg " at the begin
    local cmdline = string.gsub(params["_cmdline"],"^%s*dbg%s+","",1)
    local full_string = string.gsub(cmdline,"^%s*xps%-api call%s+","",1)

    -- print (to_string(params))
    -- call to execute the command + it's parameters
    local rc = upperLevelDump(full_string)
    if rc == LGT_OK then
        return true
    end
    if rc == LGT_FAIL then
...
-- execute any 'C' function with it's parameters
CLI_addCommand("debug", "xps-api call", {
    func = xps_api_call,
    help = "Allow any C function to execute with it's parameter ",
    first_optional = "param1", -- param1 and all other after it are 'optional'
    params = {
        { type="values",
           { format = "%s", name = "funcName", help = "function name" },
           ...
           { format = "%s", name = "param6", help = "parameter [6]" },
           { format = "%s", name = "param7", help = "parameter [7]" },
           { format = "%s", name = "param8", help = "parameter [8]" },
           { format = "%s", name = "param9", help = "parameter [9]" },
           { format = "%s", name = "param10", help = "parameter [10]" }
        },
      }
    });
