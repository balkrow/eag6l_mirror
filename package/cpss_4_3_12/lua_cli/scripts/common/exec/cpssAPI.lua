...
local function check_param_cpssAPI_params(param, name, desc, varray)
    ...
    elseif p.cls == "union" then
        ...
        return true, param, cpssAPI_next_param()
    elseif p.cls == "int" or p.cls == "float" then
        ...
        table.insert(cpssAPI_callP, { p.dir, p.type, p.name, v })
    elseif (p.type == "GT_STRING" or p.type == "string") then
        v = tostring(param)
        table.insert(cpssAPI_callP, { p.dir, p.type, p.name, v })
    else
...
--print("pname = " .. pname.. ", mname = "..mname..", mtype = "..mtype..", mclass = "..mclass..", val = "..to_string(val))
    ...
    elseif mclass == "string" then
        ...
        -- in case there is no input, use default values
        if (s=="") then
            s=val
        else
            if mclass == "bool" then
            ...
            elseif mclass == "union" and string.sub(val,-3) == "UNT" then -- in case of 'union' inside struct
                ...
                local classType = searchUnionAttrParams(val, s, "class")
                if union_mname ~= nil then
                    prompt = prompt .. "." .. union_mname
                    if (classType == "int") or (classType == "bool") then
                        return {[union_mname] = readStructParam(s,prompt,classType)}
                    else
...
readStructParam = function(ptype,pname,val)
    ...
    if string.sub(pname,-1) == "]" and (string.sub(ptype, -3) == "UNT") then
    ...
    if struct == nil then
        ...
        if val == "int" then
            local s = cmdLuaCLI_readLine(pname.."(0)>",false)
            if s=="" then
                s=0
            end
        ...
        if val == "bool" then
            local s = cmdLuaCLI_readLine(pname.."(false)>",false)
            if s=="" then
                return false
            end
...
--print("p0: special="..to_string(special))
    repeat
        ...
        if mclass == "enum" then
            local enum = cpssAPI_getenum(mtype)
            local member = nil
            local tagname
                repeat
                    member = xml_getchild(enum, member)
                    if member == nil then
                        tagname = nil
                        break
                    end
