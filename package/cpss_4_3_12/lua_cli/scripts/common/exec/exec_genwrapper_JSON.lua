...
if cli_C_functions_registered(
        ...
        "wraplZlibCompress",
        "wraplZlibDecompress",
        "wraplCompressedJsonSend",
        "wraplCompressedJsonReceive",
        "wraplMacIncrement",
        "wraplIpv4Increment",
        "wraplIpv6Increment")
then
...
-- doJsonCliCompressedFunc
-- decode JSON compressed data sent from CLI and handle errors
--
-- INPUT:
--          func - function parameters table
--
-- OUTPUT:
--          retval - decoded JSON params received or string in case of an error
local function doJsonCliCompressedFunc(func)
    local params, retval, status
    local compressed_data_output=""
    params = wraplCompressedJsonReceive()  --recieved compresses Json
    params = wraplZlibDecompress(params) --decompress it
    retval = {}
    repeat
        if type(params) ~= "string" then
            retval = { ctrl_error_ret = { retval=1, error="failed in wraplZlibDecompress(): "..to_string(params) } }
        end
        status, params = pcall(function(params) -- call actual function
            return JSON:decode(params)
        end, params)
        if status == false then
            retval = { ctrl_error_ret = { retval=1, error="JSON decode failed: "..params } }
    ...
    until true
    compressed_data=wraplZlibCompress(JSON:encode(retval))
    for i=1,string.len(compressed_data) do
        if (string.byte(compressed_data,i) == 0) then -- removed embedded zeros
            compressed_data_output=compressed_data_output.."0"
        else
            compressed_data_output=compressed_data_output..compressed_data:sub(i,i) 
        end;
    end;
    print(compressed_data_output)
end
...
-- Send return values encoded with JSON
-- This function can be called from luaCLI console
CLI_addCommand("exec", "execGenWrapperCT", {
    help = "run cpssGenWrapper with JSON interface",
    func = doJsonCliCompressedFunc,
...
-- Read JSON encoded structure { funcname=cpssAPIname, params=cpssGenWrapperParams }
-- Send return values encoded with JSON
-- This function is working with JSON interface, compressed in/out over clear text
CLI_addCommand("exec", "execJsonControlCT", {
    help = "run execJsonControlCFunc with JSON interface, compressed in/out over clear text",
    func = doJsonCliCompressedFunc,
    constFuncArgs = { execJsonControlCFunc }
})
...
CLI_addCommand("exec", "execGaltisWrapperCT", {
    help = "run cmdEventRun, JSON, compressed input:={i=\"cpssInitSystem 29,1,0\",f=nil}",
    func = doJsonCliCompressedFunc,
    constFuncArgs = { execGaltisWrapperFunc }
})
