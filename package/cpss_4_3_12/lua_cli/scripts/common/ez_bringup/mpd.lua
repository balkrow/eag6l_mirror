...
cmdLuaCLI_registerCfunction("wr_utils_mpd_set_ptp_init") 
...
cmdLuaCLI_registerCfunction("wr_utils_mpd_debug_set_ptp_tod_capture")
cmdLuaCLI_registerCfunction("wr_utils_mpd_debug_get_ptp_tod_capture_value")
...
local function mpd_set_ptp_init(params)
  local api_result=1    
	if (params["nanoSeconds"] == nil) then params["nanoSeconds"]=0 end
    if (params["lowSeconds"] == nil) then params["lowSeconds"]=0 end
    if (params["highSeconds"] == nil) then params["highSeconds"]=0 end
    if (params["fractionalNanoSeconds"] == nil) then params["fractionalNanoSeconds"]=0 end
	
    api_result = wr_utils_mpd_set_ptp_init(params["phyNumber"], params["reason"], params["nanoSeconds"], params["lowSeconds"], params["highSeconds"], params["fractionalNanoSeconds"])          
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
...
local function mpd_set_combo_media_type(params)
  local api_result=1
    api_result = wr_utils_mpd_set_combo_media_type(params["relIfindex"], params["fiberPresent"], params["speed"])
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
...
local function mpd_set_present_notification(params)
  local api_result=1
    api_result = wr_utils_mpd_set_present_notification(params["relIfindex"], params["fiberPresent"], params["opMode"])
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
...
local function mpd_set_ptp_tod_capture(params)
  local api_result=1
    api_result = wr_utils_mpd_debug_set_ptp_tod_capture(params["phyNumber"])
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
...
local function mpd_get_ptp_tod_captured_value(params)
  local api_result=1
    api_result = wr_utils_mpd_debug_get_ptp_tod_capture_value(params["phyNumber"])
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
...
CLI_type_dict["reasonPtpInit"] = {
    checker = CLI_check_param_enum,
    complete = CLI_complete_param_enum,
    help = "reasonPtpInit\n",
    enum = {
        ["MPD_PTP_LOAD_TIME_FOR_PULSE_E"] = { value="MPD_PTP_LOAD_TIME_FOR_PULSE_E", help="init in pulse mode" },        
        ["MPD_PTP_LOAD_TIME_FOR_PPS_E"] = { value="MPD_PTP_LOAD_TIME_FOR_PPS_E", help="init in PPS mode" }           
   }
}  
...
CLI_type_dict["opModeTyp"] = {
    checker = CLI_check_param_enum,
    complete = CLI_complete_param_enum,
    help = "type of media detected\n",
    enum = {
        ["fiber"] = { value="MPD_OP_MODE_FIBER_E", help="SFP and SFP+" },
        ["dac"] = { value="MPD_OP_MODE_DIRECT_ATTACH_E", help="Direct attached/ DAC cable" },
        ["copperSfp"] = { value="MPD_OP_MODE_COPPER_SFP_E", help="Copper SFP 1000Base-T/10GBase-T" },
        ["unknown"] = { value="MPD_OP_MODE_UNKNOWN_E", help="In case not preset or failed to identify" }
   }
} 
...
CLI_addCommand("mpd_test", "mpd set ptp init", {
    func   = mpd_set_ptp_init,
    help   = "mpd set ptp init <phyNumber> <reason> <nanoSeconds> <lowSeconds> <highSeconds> <fractionalNanoSeconds>",
--          Syntax: mpd set ptp init <phyNumber> <reason> <nanoSeconds> <lowSeconds> <highSeconds> <fractionalNanoSeconds>
    first_optional = "nanoSeconds", -- nanoSeconds and all other after it are 'optional'
    params = {
        { 
          type = "values",
          {format="%integer", name="phyNumber", help="phyNumber"},    
          {format="%reasonPtpInit", name="reason", help="pulse or pps"},                     
          {format="%integer", name="nanoSeconds", help="nanoSeconds"},
          {format="%integer", name="lowSeconds", help="lowSeconds"},
          {format="%integer", name="highSeconds", help="highSeconds"},
          {format="%integer", name="fractionalNanoSeconds", help="fractionalNanoSeconds"},
        },
        mandatory={ "phyNumber", "reason" }        
    }
})
...
CLI_addCommand("mpd_test", "mpd set combo media type", {
    ...
    help   = "mpd set combo media type <relIfindex> <fiberPresent> <speed>",
--          Syntax: mpd set speed <relIfindex> <fiberPresent> <speed>
    params = {
        { 
          type = "values",
          {format="%integer", name="relIfindex", help="relIfindex"},
        ...
        mandatory={ "relIfindex", "fiberPresent", "speed" }        
    }
})
...
CLI_addCommand("mpd_test", "mpd set present notification", {
    ...
    help   = "mpd set present notification <relIfindex> <fiberPresent> <opMode>",
--          Syntax: mpd set present notification <relIfindex> <fiberPresent> <opMode>
    params = {
        { 
          type = "values",
          {format="%integer", name="relIfindex", help="relIfindex"},
          {format="%fiberPresentTyp", name="fiberPresent", help="fiber present"},            
		  {format="%opModeTyp", name="opMode", help="type of media detected"},
        },
        mandatory={ "relIfindex", "fiberPresent", "opMode" }       
    }
})

CLI_addCommand("mpd_test", "mpd set ptp capture", {
    func   = mpd_set_ptp_tod_capture,
    help   = "mpd set ptp capture <phyNumber>",
--          Syntax: mpd set ptp capture <phyNumber>
    params = {
        { 
          type = "values",
          {format="%integer", name="phyNumber", help="phyNumber"},
...
CLI_addCommand("mpd_test", "mpd get ptp capture", {
    func   = mpd_get_ptp_tod_captured_value,
    help   = "mpd get ptp capture <phyNumber>",
--          Syntax: mpd get ptp capture <phyNumber>
    params = {
        { 
          type = "values",
          {format="%integer", name="phyNumber", help="phyNumber"},
