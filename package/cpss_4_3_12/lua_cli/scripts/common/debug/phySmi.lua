...
CLI_type_dict["dumpType"] = {
    checker = CLI_check_param_enum,
    complete = CLI_complete_param_enum,
    help = "type of registers dump\n",
    enum = {
        ["full"] = { value="MPD_DEBUG_REGISTERS_FULL_DUMP_E", help="full registers dump" },        
        ["strap"] = { value="MPD_DEBUG_REGISTERS_STRAP_DUMP_E", help="registers for strap info" },        
        ["linkFlap"] = { value="MPD_DEBUG_REGISTERS_LINK_FLAP_DEBUG_DUMP_E", help="registers for link flapping" }
   }
} 
--constants
...
CLI_type_dict["xsmiInterface"] = {
    ...
    enum = {
        ...
        ["2"] = { value=2, help="XSMI2 master interface number" }
   }
}
...
local function mtd_debug_reg_dump(params)
  local api_result=1
    api_result = wr_utils_mpd_mtd_debug_reg_dump(params["relIfindex"], params["dump"])
    if api_result ~= 0 then
      printMsg ("command failed rc = "..api_result)
    end
    return true
    ...
    params = {
        {
          type = "values",
          {format="%integer", name="relIfindex", help="relIfindex"},
          {format="%dumpType", name="dump", help="dump register type"},
        },
