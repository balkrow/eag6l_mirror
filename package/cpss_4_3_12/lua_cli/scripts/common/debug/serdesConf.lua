...
cmdLuaCLI_registerCfunction("wrlDevFirstNum")
...
local function traceAccessModeSet(params)
    local ret, dev, devices, i, enable, mode, tmpMode, firstDev
    ...
    firstDev = wrLogWrapper("wrlDevFirstNum")

    if (#devices == 0) then
        if (params["devID"] == "all") then
            devices = { tonumber(firstDev) }
        else
