...
local function ioam_e2e_epcl_UDBs_info_get(index,trg_eport,trg_hw_dev)
    ...
    local udbInfo = {
        ...
        ,[2] = {
               ...
               mask          = 0xff }--full byte
        -- The EPCL Metadata bit 136 called useVidx
        -- The EPCL Metadata bit  88 called Is Trg PHY Port Valid
        -- Previous versions used bit 88 and it also worked
        ,[3] = {
               udbOffsetType =  "CPSS_DXCH_PCL_OFFSET_METADATA_E",
               udbOffset     = 136 / 8,
...
--        return
    ...
    if is_sip_7(devNum) then
       ...
       --EPCL target device
        ...
        local subFieldsArr =
        {
            {"APP_REGISTRED0", 0, 10}, --EPCL target device
            {"APP_REGISTRED1", 0, 13}, --EPCL target ePort
            {"APP_REGISTRED2", 0,  1}  --EPCL useVidx
        }
        result = wrlPrvLuaTgfTcamClientsOneLookupDevInit(
            devNum, "EPCL", 10, subFieldsArr);
        if result ~= 0 then
...
local function ioam_e2e_by_epcl_set(command_data,devNum,trg_eport,trgPhyPort,unset)
    ...
    if not unset then
        ...
        if is_sip_7(devNum) then
            values.lookupCfgPtr.exactMatchLookupSubProfileId = 1
        end
    ...
    if unset then
        ...
        local action = { egressPolicy = "CPSS_PCL_DIRECTION_EGRESS_E",
        ...
        if is_sip_7(devNum) then
           local result
            ...
            {
                {"APP_REGISTRED0", 0, 10, 0x03FF,  hwDevNum}, --EPCL target device
                {"APP_REGISTRED1", 0, 13, 0x1FFF, trg_eport}, --EPCL target ePort
                {"APP_REGISTRED2", 0,  1,       1,        0}  --EPCL useVidx
            }
            -- bits[11:0] pclId, bit[13] macToMe, bits[31:24] - packet types
            local commonKeyHeaderMask    = 0
            local commonKeyHeaderPattern = 0
