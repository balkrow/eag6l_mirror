...
local function Ret_Table_Update(Ret_Table,PrbsStatus,devNum,portNum,laneNum,devPort,val,lockedPtr,errorCntrPtr,patternCntrPtr)
    ...
    if       (Ret_Table  ~= nil )
        ...
        if (PrbsStatus == "Locked" ) then
            ...
            Ret_Table[devNum][portNum][laneNum].errorCntrPtr   = tonumber(wrlCpssGtU64StrGet(val["errorCntrPtr"]))
...
local function PrbsPrintLocked(command_data,Ret_Table,devNum, portNum,laneNum,devPort,val,lockedPtr,
    ...
    if tonumber(wrlCpssGtU64StrGet(val["errorCntrPtr"]))== 0 then
        errorCntrPtrStr = string.format("0x00000000")
    else
        errorCntrPtrStr = string.format("0x%08X", wrlCpssGtU64StrGet(val["errorCntrPtr"]))
    end
    ...
    if CounterClearOnReadEnablePtr then
        ...
        .. errorCntrPtrStr .. "        " .. patternCntrPtrStr .. "                           Yes"
    else
        command_data["result"] = string.format("%-6s",devPort).."         " .. lockPtrStr .. "          "
        .. errorCntrPtrStr .. "        " .. patternCntrPtrStr .. "                           No"
    end
