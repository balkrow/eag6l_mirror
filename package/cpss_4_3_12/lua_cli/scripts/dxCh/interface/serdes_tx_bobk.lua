...
local function serdes_tx_port_mgr(params)
    ...
    if params.tx_emph~=nil and params.tx_atten~=nil and not (params.tx_atten == 0xFFFF or params.tx_emph[1] == 0xFFFF or params.tx_emph[2] == 0xFFFF or params.tx_emph[3] == 0xFFFF) then
        if is_sip_6_20() or (is_sip_6_10() and not is_sip_6_15()) then
            if math.abs(params.tx_emph[1] + params.tx_emph[2] + params.tx_emph[3] + params.tx_atten) > 63 then
               print(" Error : Tx param out of range [ABS(Post + Pre + Pre2 + main ) > 63]")
            end
        elseif is_sip_6() then
            if params.tx_emph[1] + params.tx_emph[2] + params.tx_emph[3] + params.tx_emph[4] + params.tx_atten > 26 then
               print(" Error : Tx Param out of range [Pre3 + Pre2 + Pre + Atten + Post > 26]")
            end
        end
    end
        ...
        for i=j,laneNum,1 do
            ...
            if serdesTxCfgPtr.type == "CPSS_PORT_SERDES_AVAGO_E" then
                ...
                if portmgr.portParamsStcPtr.portType == "CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E" then
                    ...
                    serdesTxCfgPtr.type = serdes.serdesType
                    serdesTxCfgPtr.txTune = {}
                    serdesTxCfgPtr.txTune = portmgr.portParamsStcPtr.portParamsType.apPort.apAttrs.overrideLaneParams[0+i].txParams.txTune
                else
                    serdesTxCfgPtr = portmgr.portParamsStcPtr.portParamsType.regPort.laneParams[i].txParams
                    serdesTxCfgPtr.type = serdes.serdesType
                    serdesTxCfgPtr.txTune = {}
                    serdesTxCfgPtr.txTune = portmgr.portParamsStcPtr.portParamsType.regPort.laneParams[i].txParams.txTune
                end

                if params.tx_atten ~= nil then
                    if serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C112G_E" then
                        serdesTxCfgPtr.txTune.comphy_C112G = {}
                        serdesTxCfgPtr.txTune.comphy_C112G.main = params.tx_atten
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C56G_E" then
                        serdesTxCfgPtr.txTune.comphy_C56G = {}
                        serdesTxCfgPtr.txTune.comphy_C56G.main = params.tx_atten
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C28G_E" then
                        serdesTxCfgPtr.txTune.comphy_C28G = {}
                        serdesTxCfgPtr.txTune.comphy_C28G.peak = params.tx_atten
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C12G_E" then
                        serdesTxCfgPtr.txTune.comphy_C12G = {}
                        serdesTxCfgPtr.txTune.comphy_C12G.peak = params.tx_atten
                    end
                end

                if params.tx_emph ~= nil then
                    if serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C112G_E" then
                        serdesTxCfgPtr.txTune.comphy_C112G.post = params.tx_emph[1]
                        serdesTxCfgPtr.txTune.comphy_C112G.pre  = params.tx_emph[2]
                        serdesTxCfgPtr.txTune.comphy_C112G.pre2 = params.tx_emph[3]
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C56G_E" then
                        serdesTxCfgPtr.txTune.comphy_C56G.post = params.tx_emph[1]
                        serdesTxCfgPtr.txTune.comphy_C56G.pre  = params.tx_emph[2]
                        serdesTxCfgPtr.txTune.comphy_C56G.pre2 = params.tx_emph[3]
                        serdesTxCfgPtr.txTune.comphy_C56G.usr  = params.tx_usr
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C28G_E" then
                        serdesTxCfgPtr.txTune.comphy_C28G.post = params.tx_emph[1]
                        serdesTxCfgPtr.txTune.comphy_C28G.pre  = params.tx_emph[2]
                    elseif serdes.serdesType == "CPSS_PORT_SERDES_COMPHY_C12G_E" then
                        serdesTxCfgPtr.txTune.comphy_C12G.post = params.tx_emph[1]
                        serdesTxCfgPtr.txTune.comphy_C12G.pre  = params.tx_emph[2]
                    end
                end
            end
            if portmgr.portParamsStcPtr.portType == "CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E" then
...
CLI_addCommand("interface", "serdes tx", {
  ...
  params = {
  {   type="named",
            { format="amplitude %type_amp",     name="tx_atten", multiplevalues=true, help="Tx Amplitude/Attenuation of the signal "},
            { format="emph %type_txpost %type_txpre %type_txpre2 %type_txpre3 ", name="tx_emph", multiplevalues=true,      help="Tx Emphasis"},
            { format="usr %bool", name="tx_usr",  help="Tx ultra short reach"},
