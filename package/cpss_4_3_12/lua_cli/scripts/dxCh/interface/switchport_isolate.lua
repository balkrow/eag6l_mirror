...
local function switchport_isolate_func(params)
    ...
    for dummy, devNum in pairs(dev_list) do
        local portBits,devBits,trunkBits
        
        if is_sip_5(devNum) and (0 == explicit_set_PortIsolationLookupBits) then
            if(is_sip_7(devNum))then
                --12 bits for {dev,port} : so give the port 7 bits to supports ports: 0..127
                -- 9 bits for trunkId
                portBits,devBits,trunkBits = 7,5,9
            else
                portBits,devBits,trunkBits = 6,5,7
            end
            ...
            myGenWrapper("cpssDxChNstPortIsolationLookupBitsSet", {
                { "IN",     "GT_U8",           "devNum",           devNum },
                { "IN",    "GT_U32",           "numberOfPortBits",   portBits },    -- default was 7
                { "IN",    "GT_U32",           "numberOfDeviceBits", devBits  },  -- as default
                { "IN",    "GT_U32",           "numberOfTrunkBits",  trunkBits}}); -- as default
            if result ~= 0 then
...
--[[    
    ...
    --NOTE : the function wrlCpssDxChNstPortIsolationTableEntrySet remove ports from the BMP that are more than the device supports
    -- so we not do here also logic for that  , and we can support all 1024 ports
    local localPortsMembers = {ports = {[0] = 
        --0..255
...
--]]
    ...
    -- Restore vlan 1 configuration (No need to restore VLAN config, Port config update will work)
