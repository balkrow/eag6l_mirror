...
function CLI_check_param_policerid(param, data)
    ...
    if (numEntries<=parmVal) or (parmVal<0) then
        return false,"Policer id is out of range, the range is: 0-".. (numEntries -1);
    end
    return true,parmVal;
end
...
local function CLI_check_param_lookupNum_field_number(param,name,desc,varray,params)
    if tonumber(param) then
        local my_dictionary = {min = 0 , max = 1}
        return CLI_check_param_number(param,name,my_dictionary,varray,params)
    end
    return false,""
end
local function CLI_complete_param_lookupNum_field_number(param,name,desc,varray,params)
    if tonumber(param) then
        local my_dictionary = {min = 0 , max = 1}
...
--[[local]] lookupNum_enum = {
    ["first"] = {value = "CPSS_PCL_LOOKUP_NUMBER_0_E"},
    ["second"] = {value = "CPSS_PCL_LOOKUP_NUMBER_1_E"},
    ["third"] = {value = "CPSS_PCL_LOOKUP_NUMBER_2_E"},
}
local function CLI_check_param_lookupNum_enum(param,name,desc,varray,params)
    local my_dictionary = {enum = lookupNum_enum}
    local my_name = "lookupNum_enum"
    return CLI_check_param_enum(param,my_name,my_dictionary,varray,params)
end
local function CLI_complete_param_lookupNum_enum(param,name,desc,varray,params)
    local my_dictionary = {enum = lookupNum_enum}
    local my_name = "lookupNum_enum"
...
CLI_type_dict["lookupNum"] = {
    checker  = {CLI_check_param_lookupNum_field_number    ,CLI_check_param_lookupNum_enum   },
    complete = {CLI_complete_param_lookupNum_field_number ,CLI_complete_param_lookupNum_enum},
    --min=0,
    --max=1,
    help="Lookup number : 0(first) or 1(last(second/third)) or enum of : first, second , third"
}
...
CLI_type_dict["ipcl-metadata"] = {
    checker = CLI_check_param_enum,
    complete = CLI_complete_param_enum,
    help = "The 'client' that IPCL need to use in metedata on udb[30]",
    enum = {
       ["tpid-index"] = { value = "tpid-index" , help = "Use udb[30] for TPID Index of tag0,tag1 : metadata Anchor offset: 29" },
       ["ptp"]        = { value = "ptp"        , help = "Use udb[30] for <isPtp> : metadata Anchor offset: 57" },
    }
}

-- see that the file 'PCLAddRule.lua' uses the UDBs 30
-- NOTE: the fucntion udb_init() also bind UDBs 0..15 to ipv6 DIP[16 bytes]
-- the 'PCLAddRule.lua' uses key of : CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_EXTENDED_UDB_STC
-- with GT_U8 udb23_30[8];
-- and for 'metadata' need to use udbIndex >= 30
-- so only single UDB can serve the metadata-- we need to allow multiple 'clients' to use it
--[[local]] used_udb_30_by_sip5_for_metadata_ingress_key = {
    ...
    ["tpid-index"] = {udbIndex = 30 , offset = 29 , offsetType = "CPSS_DXCH_PCL_OFFSET_METADATA_E"},
    -- Use UDB #30 for PTP metadata
    -- <isPtp> bit 456 in metadata
    -- Anchor offset: 57
    -- Bit offset: 0:0
    ["ptp"] = {udbIndex = 30 , offset = 57 , offsetType = "CPSS_DXCH_PCL_OFFSET_METADATA_E"},
}

-- global mode to state if IPCL set udb30 to work with tpid-index or ptp
local ipclUdb30GlobalMode = nil
function ipclUdb30GlobalModeSet(mode)
    ipclUdb30GlobalMode = mode
end
function ipclUdb30GlobalModeGet()
    return ipclUdb30GlobalMode
end
