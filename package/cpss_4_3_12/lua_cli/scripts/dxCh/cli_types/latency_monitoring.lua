...
--* For SIP7 override "latProf" and "latProfRange" with extended range
local function lm_min_max_set(param, name, desc)
    if is_sip_7() then
        desc.min = 0
        desc.max = 4095
    else
...
local function lm_check_param_number(param, name, desc)
    lm_min_max_set(param, name, desc)
    return CLI_check_param_number(param, name, desc)
end

local function lm_complete_param_number(param, name, desc)
    lm_min_max_set(param, name, desc)
    return CLI_complete_param_number(param, name, desc)
end

local function lm_check_param_number_range(param, name, desc)
    lm_min_max_set(param, name, desc)
    return CLI_check_param_number_range(param, name, desc)
end

local function lm_complete_param_number_range(param, name, desc)
    lm_min_max_set(param, name, desc)
    return CLI_complete_param_number_range(param, name, desc)
end

CLI_type_dict["latProf"] = {
    checker = lm_check_param_number,
    complete = lm_complete_param_number,
...
CLI_type_dict["latProfRange"] = {
    checker = lm_check_param_number_range,
    complete = lm_complete_param_number_range,
    help = "Latency monitoring profile range\n",
    min=0,
    max=511
}
