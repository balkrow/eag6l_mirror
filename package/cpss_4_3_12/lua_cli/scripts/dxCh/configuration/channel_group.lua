...
local function channel_group_func(params)
    ...
    if params.api == nil then 
        params.api = "cpssDxChTrunkMemberAdd"
    end
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
                ...
                trunk_member_adding_warning_string = 
                    wrLogWrapper("wrlDxChTrunkMemberAddingConditionChecking", 
                                 "(devNum, trunkId, hwDevNum, hwPortNum)", 
                                   devNum, trunkId, hwDevNum, hwPortNum)
                if        0 == result                   then
                    if(params.api == "cpssDxChTrunkMemberAdd") then
                        -- keep legacy behaviour only for 'member add' to not operate if already in the current trunk
                        trunk_member_adding_condition   = values
                    else
                        trunk_member_adding_condition = true -- let the CPSS fail the operation if not valid
                    end
                elseif 0x10 == result                   then
                    command_data:setFailPortAndLocalStatus() 
            ...
            if (true == command_data["local_status"])       and
               (true == trunk_member_adding_condition)      then
                result, values = 
                    cpssPerTrunkParamSet(params.api, devNum, 
...
CLI_addCommand("interface", "channel-group", {
  ...
  help="add the member to the channel-group",
  params={
    { type = "values",
      "%trunkID"
    }
...
-------------------------------------------------------
-- command registration: channel-group-member-add-as-disabled
-------------------------------------------------------
CLI_addCommand("interface", "channel-group-member-add-as-disabled", {
  func=function(params)
      params.api="cpssDxChTrunkMemberDisabledAdd"
      return channel_group_func(params)
  end, 
  help="add the member as disabled to the channel-group",
  params={
      { type = "values",
      "%trunkID"
    }
...
-------------------------------------------------------
-- command registration: channel-group-member-disable
-------------------------------------------------------
CLI_addCommand("interface", "channel-group-member-disable", {
  func=function(params)
      params.api="cpssDxChTrunkMemberDisable"
      return channel_group_func(params)
  end, 
  help="disable the member in the channel-group",
  params={
      { type = "values",
      "%trunkID"
    }
...
-------------------------------------------------------
-- command registration: channel-group-member-enable
-------------------------------------------------------
CLI_addCommand("interface", "channel-group-member-enable", {
  func=function(params)
      params.api="cpssDxChTrunkMemberEnable"
      return channel_group_func(params)
  end, 
  help="enable the member in the channel-group",
  params={
      { type = "values",
      "%trunkID"
    }
...
CLI_addCommand("interface", "no channel-group", {
  ...
  help="remove the member from the channel-group",
  params={
    { type = "values",
      "%trunkID"
    }
