...
-- SIP7 TCAM Clients Library Wrappers
function tcamClientsLuaWrappersRegister()
    if not luaTgfTcamClientsGlobal.tcamClientsLuaWrappersRegistered then
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsDbHandleDelete")
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsDbHandleCreate")
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsDevInit")
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsPclRuleSet")
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsPclCfgTblSet")
        cmdLuaCLI_registerCfunction("wrlPrvLuaTgfTcamClientsOneLookupDevInit")
        cmdLuaCLI_registerCfunction("wrlPrvTgfTcamClientsPclFieldRegister")
        luaTgfTcamClientsGlobal.tcamClientsLuaWrappersRegistered = true
    end
end

-- SIP7 TCAM Clients Library related Global Constants
luaTgfTcamClientsGlobal =
{
    tcamClientsLuaWrappersRegistered = false,
    packetTypesEth     = "ETH",
    packetTypesIpv4    = "IPV4",
    packetTypesIpv6    = "IPV6",
    lookupIpcl0        = "IPCL0",
    lookupIpcl1        = "IPCL1",
    lookupIpcl2        = "IPCL2",
    lookupMpcl         = "MPCL",
    lookupEpcl         = "EPCL",
    pclCfgProfileId    = 1,
    --[[] @brief EGRESS Applicable Flow Sub-template
     *  1 = IPv4_TCP
     *  2 = IPv4_UDP
     *  4 = MPLS
     *  8 = IPv4_FRAGMENT
     ...
     *  128 = UDE0-4
     --]]
    ingressCommonHeaderMaskNotIp = 0x00A40000,
    ingressCommonHeaderMaskIpv4  = 0x001B0000,
    ingressCommonHeaderMaskIpv6  = 0x00400000,
     ...
     * 0x8000   - UDE6_E
     --]]
    egressCommonHeaderMaskNotIp = 0xFE240000,
    egressCommonHeaderMaskIpv4  = 0x001B0000,
    egressCommonHeaderMaskIpv6  = 0x01C00000,
}
...
local function addPCLRule(params)
  ...
  local CPSS_DXCH_CNC_CLIENT_INGRESS_PCL_LOOKUP_0_E = 1
  local CPSS_DXCH_CNC_CLIENT_EGRESS_PCL_E = 7
  ...
  local CPSS_INTERFACE_TRUNK_E = 1
    ...
    --check that only single client used on ingress udb30
    if direction == "ingress" then
        --clients muxed on udb30
        local ingress_clients_muxed_on_udb30 = {
            [1] = {"isPtp"},
            [2] = {"tpidIndex" , "tpid1Index"},
        }

        local numUsed = 0
        local usedClients = {}

        for ii=1,#ingress_clients_muxed_on_udb30 do
            for _,client in pairs(ingress_clients_muxed_on_udb30[ii]) do
                if params[client] then
                    numUsed = numUsed + 1
                    usedClients[numUsed] = client
                    break--count only once per client
                end
            end
        end

        if (numUsed > 1) then
            local errorString = "Error: tring to use multiple fields that use udb[30] " .. to_string(usedClients)
            print(errorString)
  ...
  elseif direction == "egress" then
	...
	--supported both for IPCL0 and for EPCL
    if params["counters"] ~= nil then -- set action for CNC
		actionSTC["matchCounter"] = {}
		actionSTC["matchCounter"]["enableMatchCount"] = true
  ...
  if direction=="egress" then
    actionSTC["egressPolicy"] = "CPSS_PCL_DIRECTION_EGRESS_E"
    if params["modifyVlan0"] ~= nil then
      actionSTC["vlan"] = {}
  ...
  if direction == "ingress" then
    ...
    if (is_sip_5(nil)) then
        if direction == "ingress" then
            if (params["tpidIndex"] or params["tpid1Index"])~=nil then
                ...
                -- see the 'bind' to this UDB by used_udb_30_by_sip5_for_metadata_ingress_key in file : PCLPortBind.lua
                local udb30Mask = 0
                local udb30Value = 0
    ...
    if nil ~= params["isPtp"]--[[true/false]] then
        if(not is_sip_5(nil))then
            print("Error: <isPtp> isn't supported by this device (non-sip5)")
            return false,"Error, rule could not be added"
        end

        if direction == "ingress" then
            -- Use UDB #30 for PTP metadata
            -- bit 456 in metadata
            -- Anchor offset: 57
            -- Bit offset: 0:0
            -- see the 'bind' to this UDB by used_udb_30_by_sip5_for_metadata_ingress_key in file : PCLPortBind.lua
            local isPtp_value = params["isPtp"] and 0x1 --[[enabled]] or 0 --[[disabled]]
            local udb29Value = isPtp_value --[[bit 0]]
            local udb29Mask  = 0x1 --[[bit 0]]
            rule   ["udb23_30"][7] = udb29Value
            ruleMsk["udb23_30"][7] = udb29Mask
        else
            print("Error: <isPtp> not implemented for EPCL")
            return false,"Error, rule could not be added"
        end
     end
  ...
  if direction=="ingress" then
    ...
    local pclLookup;
    local commonKeyHeaderMask
    local commonKeyHeaderPattern
    local notIpv4 = false
    local notIpv6  = false
    local notEthOther = false
    local subFieldConditionArr = {}
    local subFieldIdx
    local udbPacketTypeGroupIndex

    if is_sip_7(nil) then
        tcamClientsLuaWrappersRegister()
        subFieldIdx = 1
        if params["isTagged"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_TAG0_EXISTS", 0, 1, 1, params["isTagged"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_TAG0_EXISTS", 0, 1, 1, params["isTagged"]}
            end
        ...
        if params["vid"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_VID0", 0, 12, 0xFFF, params["vid"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_VID0", 0, 12, 0xFFF, params["vid"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["vid1"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_VID1", 0, 12, 0xFFF, params["vid1"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_VID1", 0, 12, 0xFFF, params["vid1"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["up"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_UP0", 0, 3, 7, params["up"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_UP0", 0, 3, 7, params["up"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["qosProfile"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_QOS_PROFILE", 0, 10, 0x3FF, params["qosProfile"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_QOS_PROFILE", 0, 10, 0x3FF, params["qosProfile"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["srcPort"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_SRC_PHY_PORT", 0, 8, 0xFF, params["srcPort"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_SRC_PHY_PORT", 0, 8, 0xFF, params["srcPort"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["ip_protocol"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_IP_PROTOCOL", 0, 8, 0xFF, params["ip_protocol"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_IP_PROTOCOL", 0, 8, 0xFF, params["ip_protocol"]}
            end
            subFieldIdx = subFieldIdx + 1
            notEthOther = true
        end
        if params["macSrc"] ~= nil then
            params["macSrcMsk"] = params["macSrc"][7]
            if params["macSrcMsk"] == nil then
                params["macSrcMsk"]={"ff","ff","ff","ff","ff","ff"}
            end
            local w; msk = {}; ptn = {};
            w = prvAddAddr(params["macSrc"],16,6)
            ptn[0] = w[5] + (w[4] * 0x100) + (w[3] * 0x10000) + (w[2] * 0x1000000)
            ptn[1] = w[1] + (w[0] * 0x100)
            w = prvAddAddr(params["macSrcMsk"],16,6)
            msk[0] = w[5] + (w[4] * 0x100) + (w[3] * 0x10000) + (w[2] * 0x1000000)
            msk[1] = w[1] + (w[0] * 0x100)
            subFieldConditionArr[subFieldIdx] =
                {"L2_SRC_MAC", 0, 32, msk[0], ptn[0]}
            subFieldIdx = subFieldIdx + 1
            subFieldConditionArr[subFieldIdx] =
                {"L2_SRC_MAC", 32, 16, msk[1], ptn[1]}
            subFieldIdx = subFieldIdx + 1
            notIpv6     = true
        end
        if params["macDest"] ~= nil then
            params["macDestMsk"] = params["macDest"][7]
            if params["macDestMsk"] == nil then
                params["macDestMsk"]={"ff","ff","ff","ff","ff","ff"}
            end
            local w; msk = {}; ptn = {};
            w = prvAddAddr(params["macDest"],16,6)
            ptn[0] = w[5] + (w[4] * 0x100) + (w[3] * 0x10000) + (w[2] * 0x1000000)
            ptn[1] = w[1] + (w[0] * 0x100)
            w = prvAddAddr(params["macDestMsk"],16,6)
            msk[0] = w[5] + (w[4] * 0x100) + (w[3] * 0x10000) + (w[2] * 0x1000000)
            msk[1] = w[1] + (w[0] * 0x100)
            if direction == "ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IFIX_MAC_DA", 0, 32, msk[0], ptn[0]}
                subFieldIdx = subFieldIdx + 1
                subFieldConditionArr[subFieldIdx] =
                    {"IFIX_MAC_DA", 32, 16, msk[1], ptn[1]}
                subFieldIdx = subFieldIdx + 1
            else
                subFieldConditionArr[subFieldIdx] =
                    {"L2_DST_MAC", 0, 32, msk[0], ptn[0]}
                subFieldIdx = subFieldIdx + 1
                subFieldConditionArr[subFieldIdx] =
                    {"L2_DST_MAC", 32, 16, msk[1], ptn[1]}
                subFieldIdx = subFieldIdx + 1
            end
            notIpv6     = true
        end
        if params["ipv4-source"] ~= nil then
            params["ipv4SrcMsk"] = params["ipv4-source"][5]
            if params["ipv4SrcMsk"] == nil then
                params["ipv4SrcMsk"] = {"255","255","255","255"}
            end
            local w; msk = {}; ptn = {};
            w = prvAddAddr(params["ipv4-source"],10,4)
            ptn[0] = w[3] + (w[2] * 0x100) + (w[1] * 0x10000) + (w[0] * 0x1000000)
            w = prvAddAddr(params["ipv4SrcMsk"],10,4)
            msk[0] = w[3] + (w[2] * 0x100) + (w[1] * 0x10000) + (w[0] * 0x1000000)
            subFieldConditionArr[subFieldIdx] =
                {"FIX_IPV4_SIP", 0, 32, msk[0], ptn[0]}
            subFieldIdx = subFieldIdx + 1
            notEthOther = true
            notIpv6     = true
        end
        if params["ipv4-destination"] ~= nil then
            params["ipv4DestMsk"] = params["ipv4-destination"][5]
            if params["ipv4DestMsk"] == nil then
                params["ipv4DestMsk"] = {"255","255","255","255"}
            end
            local w; msk = {}; ptn = {};
            w = prvAddAddr(params["ipv4-destination"],10,4)
            ptn[0] = w[3] + (w[2] * 0x100) + (w[1] * 0x10000) + (w[0] * 0x1000000)
            w = prvAddAddr(params["ipv4DestMsk"],10,4)
            msk[0] = w[3] + (w[2] * 0x100) + (w[1] * 0x10000) + (w[0] * 0x1000000)
            subFieldConditionArr[subFieldIdx] =
                {"FIX_IPV4_DIP", 0, 32, msk[0], ptn[0]}
            subFieldIdx = subFieldIdx + 1
            notEthOther = true
            notIpv6     = true
        end
        if params["ipv6-source"] ~= nil then
            params["ipv6SrcMsk"]=params["ipv6-source"][1]
            if params["ipv6SrcMsk"] == nil then
                params["ipv6SrcMsk"] = {}
                params["ipv6SrcMsk"]["addr"] =
                  {65535, 65535, 65535, 65535, 65535, 65535, 65535, 65535}
            end
            local i; msk = {}; ptn = {};
            for i = 0,3 do
                ptn[i] = params["ipv6-source"]["addr"][8 - (2 * i)]
                ptn[i] = ptn[i] + (params["ipv6-source"]["addr"][7 - (2 * i)] * 0x10000)
            end
            for i = 0,3 do
                msk[i] = params["ipv6SrcMsk"]["addr"][8 - (2 * i)]
                msk[i] = msk[i] + (params["ipv6SrcMsk"]["addr"][7 - (2 * i)] * 0x10000)
            end
            for i = 0,3 do
                subFieldConditionArr[subFieldIdx] =
                    {"FIX_IPV6_SIP", (32 * i), 32, msk[i], ptn[i]}
                subFieldIdx = subFieldIdx + 1
            end
            notEthOther = true
            notIpv4     = true
        ...
        if params["ipv6-destination"] ~= nil then
            params["ipv6DestMsk"]=params["ipv6-destination"][1]
            if params["ipv6DestMsk"] == nil then
                params["ipv6DestMsk"] = {}
                params["ipv6DestMsk"]["addr"] =
                  {65535, 65535, 65535, 65535, 65535, 65535, 65535, 65535}
            end
            local i; msk = {}; ptn = {};
            for i = 0,3 do
                ptn[i] = params["ipv6-destination"]["addr"][8 - (2 * i)]
                ptn[i] = ptn[i] + (params["ipv6-destination"]["addr"][7 - (2 * i)] * 0x10000)
            end
            for i = 0,3 do
                msk[i] = params["ipv6DestMsk"]["addr"][8 - (2 * i)]
                msk[i] = msk[i] + (params["ipv6DestMsk"]["addr"][7 - (2 * i)] * 0x10000)
            end
            for i = 0,3 do
                subFieldConditionArr[subFieldIdx] =
                    {"FIX_IPV6_DIP", (32 * i), 32, msk[i], ptn[i]}
                subFieldIdx = subFieldIdx + 1
            end
            notEthOther = true
            notIpv4     = true
        end
        --must be after IP related related data to detect IPV4 or IPV6
        if params["dscp"] ~= nil then
            if notIpv4 then
                subFieldConditionArr[subFieldIdx] =
                    {"L3_IPV6_DSCP", 0, 6, 0x3F, params["dscp"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"L3_IPV4_DSCP", 0, 6, 0x3F, params["dscp"]}
            end
            subFieldIdx = subFieldIdx + 1
            notEthOther = true
        end
        if params["isPtp"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_IS_PTP", 0, 1, 1, params["isPtp"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_IS_PTP", 0, 1, 1, params["isPtp"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["tpidIndex"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_TAG0_TPID_INDEX", 0, 3, 7, params["tpidIndex"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_TAG0_TPID_INDEX", 0, 3, 7, params["tpidIndex"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        if params["tpid1Index"] ~= nil then
            if direction =="ingress" then
                subFieldConditionArr[subFieldIdx] =
                    {"IMETA_TAG1_TPID_INDEX", 0, 3, 7, params["tpid1Index"]}
            else
                subFieldConditionArr[subFieldIdx] =
                    {"EMETA_TAG1_TPID_INDEX", 0, 3, 7, params["tpid1Index"]}
            end
            subFieldIdx = subFieldIdx + 1
        end
        ...
        commonKeyHeaderPattern = getGlobal("g_pclID")
        if direction =="ingress" then
            if notEthOther then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.ingressCommonHeaderMaskNotIp
            end
            if notIpv4 then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.ingressCommonHeaderMaskIpv4
            end
            if notIpv4 then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.ingressCommonHeaderMaskIpv6
            end
        else
            if notEthOther then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.egressCommonHeaderMaskNotIp
            end
            if notIpv4 then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.egressCommonHeaderMaskIpv4
            end
            if notIpv4 then
                commonKeyHeaderMask = commonKeyHeaderMask +
                    luaTgfTcamClientsGlobal.egressCommonHeaderMaskIpv6
            end
        end

        if direction =="ingress" then
            pclLookup = luaTgfTcamClientsGlobal.lookupIpcl0
        else
            pclLookup = luaTgfTcamClientsGlobal.lookupEpcl
        end

        udbPacketTypeGroupIndex = luaTgfTcamClientsGlobal.packetTypesEth
        if notEthOther then
            udbPacketTypeGroupIndex = luaTgfTcamClientsGlobal.packetTypesIpv4
            if notIpv4 then
                udbPacketTypeGroupIndex = luaTgfTcamClientsGlobal.packetTypesIpv6
            end
        end
    ...
    for j=1,#devices do
        ...
        if is_sip_7(currDev) then
            ret = wrlPrvLuaTgfTcamClientsPclRuleSet(
                currDev,                                 --devNum,
                luaTgfTcamClientsGlobal.pclCfgProfileId, --pclCfgProfileId,
                udbPacketTypeGroupIndex               ,  --udbPacketTypeGroupIndex,
                pclLookup,                               --pclLookup,
                0,                                       --tcamSubkeyIndex,
                params["ruleID"],                        --ruleIndex
                true,                                    --writeValidRule,
                commonKeyHeaderMask,                     --commonKeyHeaderMask,
                commonKeyHeaderPattern,                  --commonKeyHeaderPattern
                subFieldConditionArr,                    --*subFieldConditionArrPtr,
                actionSTC                                --*actionPtr
                )
            if ret~=0 then
                print("wrlPrvLuaTgfTcamClientsPclRuleSet() failed: "..returnCodes[ret])
                return false,"Error, rule could not be added"
            end
        else
            ret = myGenWrapper("cpssDxChPclRuleSet",{
  ...
  if params["counters"] ~= nil then
    for j=1,#devices do
		...
		local client;
		if direction == "ingress" then
			blockNumber = getCNCBlock("IPCL_0")
			client = CPSS_DXCH_CNC_CLIENT_INGRESS_PCL_LOOKUP_0_E
		else
			blockNumber = getCNCBlock("EPCL")
			client = CPSS_DXCH_CNC_CLIENT_EGRESS_PCL_E
		end
      ...
      for i=1, #blockNumber do
        ret = myGenWrapper("cpssDxChCncBlockClientEnableSet",{
          ...
          {"IN","CPSS_DXCH_CNC_CLIENT_ENT","client",client},
	...
	if direction == "ingress" then
		...
		-- create EPCL_table if not exists
		local EPCL_table = luaGlobalGet("EPCL")
		if EPCL_table == nil then
		  EPCL_table = {}
		end

		-- store ruleId in table
		EPCL_table[params["ruleID"]] = true
		globalStore("EPCL", EPCL_table)
	end
  end
...
CLI_addCommand("config-acl", "rule-id", {
  ...
  params={
    ...
    {type="named",
      ...
      { format="isPtp %bool", name="isPtp" , help = "isPtp "},
...
local function deleteRuleID(params)
	...
	if isRuleExists == false then
		if (EPCL_table ~= nil) and (EPCL_table[params["ruleID"]]~=nil) then
		  isRuleExists = true
		  direction = "egress"
		  EPCL_table[params["ruleID"]] = nil
		  globalStore("EPCL", EPCL_table)
		end
	end
