...
local function spbm_mac_to_me(command_data, devNum, unset)
    ...
    if unset ~= true then
        ...
        macToMeMask.mac     = {[0] = 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
        macToMeValue.vlanId = 0;
        macToMeMask.vlanId  = 0;
    else
        -- never match
        macToMeValue.mac    = {[0] = 0, 0, 0, 0, 0, 0};
        macToMeMask.mac     = {[0] = 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
        macToMeValue.vlanId = 0;
...
local function spbm_pcl_default_rule_set(command_data, devNum, pcl_id, unset)
    ...
    if is_sip_7(devNum) then
        ...
        -- Fields not needed used PCL Id only via common header
        ...
        local commonKeyHeaderPattern = pcl_id
...
local function spbm_pcl_global(command_data, devNum, unset)
    ...
    if is_sip_7(devNum) then
        ...
        result = wrlPrvTgfTcamClientsPclFieldRegister(
            0, "UDB_CFG", "TUNNEL_L2", (i_sid_offset * 8), 24)
        command_data:handleCpssErrorDevice(result, "wrlPrvTgfTcamClientsPclFieldRegister() ", devNum)
        ...
        result = wrlPrvLuaTgfTcamClientsPclCfgTblSet(
            ...
            "IPCL0", true, --enable
            1,--pclCfgProfileId
            spbm_pbb_tunneled_packet_pcl_pcl_id)
...
local function spbm_pcl_rule_set(command_data, devNum, pcl_id, sid, vid, unset)
    ...
    if is_sip_7(devNum) then
        ...
        local subFieldConditionArr =
        {
            {"APP_REGISTRED0", 0, 24, 0x00FFFFFF, sid}
        }
        -- bits[11:0] pclId, bit[13] macToMe, bits[31:24] - packet types - 0xE4 - mask for all not IPV4
        local commonKeyHeaderMask    = 0x000003FF
        local commonKeyHeaderPattern = pcl_id
...
local function spbm_bridge_multicast_address_group_flood_callback(
    ...
    local maxPhyPort = system_capability_get_table_size(devNum, "PHYSICAL_PORT");
