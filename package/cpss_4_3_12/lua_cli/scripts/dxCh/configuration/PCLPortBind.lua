...
cmdLuaCLI_registerCfunction("wrlCpssDxChPclNumLookups")
...
-- IPCL set udb30 to work with tpid-index or ptp
function ipclUdb30Config(devNum)
    local command_data = Command_Data()
    local pcl_rule_format = 14
    local ipclUdb30GlobalMode = ipclUdb30GlobalModeGet()
    local service_acl_metadata = ipclUdb30GlobalMode and ipclUdb30GlobalMode or "tpid-index"
    local udbInfo = used_udb_30_by_sip5_for_metadata_ingress_key[service_acl_metadata]

    if is_sip_7(devNum) then
        -- UDBs for SIP7 configured by Global configuring function
        return
    end
    -- bind UDBs that the 'service-acl' command can use for some fields that not exists in the pre-defined fields.
    for pclPacketType = 0, CPSS_DXCH_PCL_PACKET_TYPE_UDE6_E do
        if (pclPacketType ~= CPSS_DXCH_PCL_PACKET_TYPE_IPV6_E) then
            ret,val =  myGenWrapper(
                ...
                { "IN",     "GT_U32",                               "udbIndex",   udbInfo.udbIndex },
                { "IN",     "CPSS_DXCH_PCL_OFFSET_TYPE_ENT",        "offsetType", udbInfo.offsetType },
                { "IN",     "GT_U8",                                "offset",     udbInfo.offset}
            })
            if ret~=0 then
                command_data:setFailPortAndLocalStatus()
                command_data:addError("Error setting UDB at device %d packet type %d: %s %s", devNum, pclPacketType, returnCodes[ret] , to_string(udbInfo))
            end
        end
    end
end

--translate params["lookupNum"] to enum string according to device number
local function getPclLookupNumEnumString(devNum,lookupNum,direction)
    local lookup_number

    if direction ~= "ingress" then
        return "CPSS_PCL_LOOKUP_0_E"--[[first lookup]]
    end

    if type(lookupNum) == "string"  then
        --already hold string value : like "CPSS_PCL_LOOKUP_NUMBER_1_E"
        lookup_number = lookupNum
    else
        if (lookupNum~=1) then
            lookupNum=0
        end

        local res,numLookups = wrlCpssDxChPclNumLookups(devNum)
        if (numLookups == 3) then
            -- this is strange logic but defind at start of LUA commands
            lookup_number =  lookupNum==0 and "CPSS_PCL_LOOKUP_0_E"--[[first lookup]] or "CPSS_PCL_LOOKUP_1_E"--[[last lookup(third)]]
        else
            -- Falcon/Ironman IPCL supports only 2 lookups
            if lookupNum == 0 then
                lookup_number = "CPSS_PCL_LOOKUP_NUMBER_0_E"--[[first lookup]]
            else
                lookup_number = "CPSS_PCL_LOOKUP_NUMBER_1_E"--[[last lookup(second)]]
            end
        end
    end
...
local function service_acl(params)
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getInterfacePortIterator() do
            ...
            --translate params["lookupNum"] to enum string according to device number
            lookup_number = getPclLookupNumEnumString(devNum,params["lookupNum"],direct)
            ...
            if ret~=0 then
                ...
                if ret~=0 then
                    ...
                    if is_sip_7(devNum) then
                        ...
                        ret = wrlPrvLuaTgfTcamClientsPclCfgTblSet(
                            devNum, interfaceInfo,
                            pclLookup, true, --enable
                            luaTgfTcamClientsGlobal.pclCfgProfileId,
                            params["pclID"])
                    else
                        ret,val = myGenWrapper("cpssDxChPclCfgTblSet",{
                            ...
                            {"IN","CPSS_PCL_LOOKUP_NUMBER_ENT","lookupNum",lookup_number},
...
local function no_service_acl(params)
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getInterfacePortIterator() do
            ...
            for name,entry in pairs(lookupNum_enum) do
                local lookupNum = entry.value -- enum value
...
function udb_init()
    if luaGlobalGet("udb_init")==nil then
        ...
        for j=1, #all_devices do
            if not is_sip_7(all_devices[j]) then
                -- UDBs for SIP7 configured by Global configuring function
                for i=0,15 do
