...
local function func_traffic_generator_emulate__per_port(command_data, devNum, portNum, params)
    ...
    if params.inc_start_byte then
        local operations = {[1]={}}
        operations[1].type = "increment"
        operations[1].value    = params.inc_step and params.inc_step or 1 -- step of the increment
        operations[1].offset   = params.inc_start_byte  -- byte offset from start of packet
        operations[1].numBytes = params.inc_num_bytes and params.inc_num_bytes or 1 -- num bytes that act as single field

        transmitInfo.operations = operations
    else 
        if params.inc_step or params.inc_num_bytes then
            print("NOTE:  increment parameters used only when 'increment-start-byte' is used")
        end
    end
...
CLI_addCommand("traffic", "traffic-generator emulate", {
    ...
    params={ { type="named",
        ...
        { format="increment-start-byte %number",name="inc_start_byte","(optional) allow to increment byte(s) when 'count' used" },
        { format="increment-num-bytes %number",name="inc_num_bytes","(optional)(default is 1) allow to increment several byts as 'single field'" },
        { format="increment-step %number",name="inc_step","(optional)(default is 1) allow to increment byte(s) in steps that are not 1" },
