...
local function capture_from(params)
    ...
    if params.egress_capture_type == "mirror" then
        capture_type = "mirror_egress_to_cpu";
    end
    if params.egress_capture_type == "pcl" then
        capture_type = "pcl_trap_egress_to_cpu";
    end
    ...
    setRcv(newRxPorts, true, capture_type);
...
local function capture_stop()
    ...
    for capture_type, dev_ports in pairs(ports_by_types) do
        setRcv(dev_ports, false, capture_type)
    end
    --
    if capture_pcap_file_fd ~= nil then
        -- flush data to file
...
local function capture_show(params)
    ...
    while true do
        ...
        capture_pcap_packet(rxdata)
        print(string.format("from port %d vlanId=%d tagged=%s len=%d",
                rxdata.portNum,
                rxdata.vlanId,
                tostring(rxdata.isTagged),
                rxdata.packet.len))
...
CLI_type_dict["egress_capture_type"] = {
    checker  = CLI_check_param_enum,
    complete = CLI_complete_param_enum,
    help     = "Enter egress capture type",
    enum = {
        ["mirror"]   = { value="mirror", help="Mirror to CPU as analyzer port"},
        ["pcl"]      = { value="pcl", help="Trap to CPU from PCL action"},
    }
}
...
CLI_addCommand("traffic", "capture from", {
    ...
    { type="named",
        ...
        { format="on-egress %egress_capture_type", name="egress_capture_type", help = "Egress capture type(optional)" },
        mandatory={"port"},
...
CLI_addCommand("traffic", "capture show", {
    ...
    params={
    { type="named",
        { format="timeout %timeout_sec",name="timeout", help="timeout in seconds to wait for packet" }}
    }
})
