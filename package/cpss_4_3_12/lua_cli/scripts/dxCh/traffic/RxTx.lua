...
function sendPacket(devPortTable,packetData,minLen)
    local ret = 0
    local xTaskQueue,replyQ,status,message,i

--    printCallStack(5)
--    printLog("sendPacket " , "devPortTable" , to_string(devPortTable),"packetData",to_string(packetData),"minLen",to_string(minLen))
    ...
    for dev,portTable in pairs(devPortTable) do
        for i=1,#portTable do
            ...
            --printLog("prvLuaTgfTransmitPackets : from sendPacket ")
...
local function setRcv_port_ingress_mirror_to_cpu(dev, port, enable)
    ...
    if (ret~=0) then print ("Error setting analyzer port for port number:"..port) end
end
