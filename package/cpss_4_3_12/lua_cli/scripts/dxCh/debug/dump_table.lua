...
local function generic_table_dump(params)
    ...
    for _, devNum in ipairs(devices) do
        ...
        if(params["filename"] ~= nil) then
            ...
            print(string.format("Entry Size(in 32 bit word): %d", entrySize))
            print(string.format("Table   : %s", params["tableType"]))
            print("Address :   Data")
            print("---------------------")
        end
        ...
        for i=0,(numEntries-1) do
            ...
            if(params["filename"] ~= nil) then
                ...
                tmpString = string.format("%d\t: ",i)
                for j=0,(entrySize-1) do
                    tmpString = tmpString..string.format("0x%8.8x ", values.tableBufferArr[j])
                    if((j+1)%4 == 0) then
                        tmpString = tmpString.."\n\t  "
                    end
                end
