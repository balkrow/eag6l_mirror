...
local function fecPrintPort(command_data,devNum,portNum,singlePort)
	...
	if  "CPSS_DXCH_PORT_RS_FEC_544_514_MODE_ENABLED_E"==fecType or "CPSS_DXCH_PORT_RS_FEC_MODE_ENABLED_E"==fecType then
		...
		if ret ~= 0 then
			...
			if ret == 0 then
				uncorrected = tonumber(wrlCpssGtU64StrGet(val_counter_rs.rsfecCountersPtr.uncorrectedFecCodeword))
				corrected = tonumber(wrlCpssGtU64StrGet(val_counter_rs.rsfecCountersPtr.correctedFecCodeword))
