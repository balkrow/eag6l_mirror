...
cmdLuaCLI_registerCfunction("wrlCpssDevicePhysicalPortNumberLimit")
...
---
--  is_sip_7
--        @description  Returns true if device is sip_7
--
--        @param dev     - The device ID
--
--        @return       true or false
--
function is_sip_7(devNum)
    return isSipVersion(devNum , "SIP_7")
end
-- ************************************************************************
---
...
function system_capability_get_table_size(devId, name)
    local entry_map = {
        ...
        EM = "CPSS_DXCH_CFG_TABLE_EXACT_MATCH_E",
...
--                      error code and error message
--
function vlan_info_from_c_to_lua_convert(devNum, portsMembers, portsTagging, vlanInfo, portsTaggingCmd)
    local result, values
    local portNum
    local max_port_number, taggingCmdSupported
...
function vlan_info_get(devNum, vlanId)
    ...
    result, values =
        vlan_info_from_c_to_lua_convert(devNum, portsMembers, portsTagging, vlanInfo,
                                        portsTaggingCmd)
    if 0 == result                          then
        vlanInfo    = values
    else
...
--[[ function that tests that (from config file) calling to :
callCpssAPI cpssDxChBrgFdbMacEntrySet
<<<PARAMS
{
    { "IN", "GT_U8", "devNum", ${dev} },
    { "IN","CPSS_MAC_ENTRY_EXT_STC","macEntryPtr",
        { key = { entryType = "CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E",key = {macVlan = {macAddr = "${info_802_1cb.macA_fdb}" , vlanId = ${info_802_1cb.vlanId1}}}},
          isStatic = true ,
          dstInterface = {type = "CPSS_INTERFACE_VIDX_E" , vidx=${info_802_1cb.vidx1001}},
          epgNumber = ${info_802_1cb.streamId3}
        }
    }
}
>>>

will be able to wait to the device to handle the entry , before we try to call to set new fdb-entry again
]]
function wait_till_end_of_cpssDxChBrgFdbMacEntrySet()
    local devices = wrlDevList()

    for _, devNum in ipairs(devices) do
        printLog("waitFdbAuMsgProcessed for devNum " .. to_string(devNum))
        local isOk, msg = waitFdbAuMsgProcessed(devNum)
        if not isOk then
          -- don't make function failed. Just print a warning.
          printLog("waitFdbAuMsgProcessed failed on devNum " .. to_string(devNum) .. " : ", msg)
        end
    end
    return ""
end
...
function getCNCBlock(client)
  ...
  if boardEnv.FamiliesList["CPSS_PP_FAMILY_DXCH_XCAT3_E"] or boardEnv.FamiliesList["CPSS_PP_FAMILY_DXCH_AC5_E"] then
    if (client == "IPCL_0") or (client == "EPCL") or (client == "IngressVlanPass") or
  ...
  then
    ...
    then
	  ...
	  -- for "CPSS_PP_FAMILY_DXCH_AC3X_E" not compatible with "EgressQueuePassAndDrop"
	  if (client == "EPCL") then
		 retTbl = {7}
		 return retTbl
	  end
