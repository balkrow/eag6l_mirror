...
cmdLuaCLI_registerCfunction("wrlCpssDxChEcmpEntryGet")
cmdLuaCLI_registerCfunction("wrlCpssDxChEcmpIndirectIndexGet")
...
cmdLuaCLI_registerCfunction("wrlCpssDxChIpEcmpIndirectNextHopEntryGet")
...
function Command_Data:initAllDeviceMacEntryIterator(params, filter, alternative)
    ...
    if ((self["mac_filter"]["key"]["entryType"] ~= nil) and
        ...
        if self["mac_filter"]["key"]["key"]["ipMcast"]["sip"] == "0.0.0.0" or
           self["mac_filter"]["key"]["key"]["ipMcast"]["sip"] == nil then
        ...
        if self["mac_filter"]["key"]["key"]["ipMcast"]["dip"] == "0.0.0.0" or
           self["mac_filter"]["key"]["key"]["ipMcast"]["dip"] == nil then
...
function Command_Data:configure_vlan_mru(mru_value)
    ...
    for iterator, devNum in self:getDevicesIterator() do
        ...
        if true == self["local_status"]                 then
            -- look for mru index already configured to the given MRU
            for idx = 0,7  do
                if mruIndex == -1 then
                    result, values =
                        myGenWrapper("cpssDxChBrgVlanMruProfileValueGet",
                                     {{ "IN", "GT_U8",  "devNum",   devNum},
                                      { "IN", "GT_U32", "mruIndex", idx},
                                      { "OUT","GT_U32", "mruValuePtr" }})
                    if result == 0 and values == mru_value then
                        mruIndex = idx
                    end
                end
            end
            ...
            if mruIndex == -1 then
                ...
                mruIndexes[devNum]  = mruIndex
            end
        end

        -- Setting of MRU value for a VLAN MRU profile.
        if mruIndex == -1 and true == self["local_status"]                 then
            result, values =
