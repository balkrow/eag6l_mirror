...
function show_ip_route_fdb_func(params)
    ...
    local fdb_next_hop_entry, next_hop_entry, next_hop_entry_showing, next_hop_entries
	...
	if(is_ipv4_protocol)then
		print( " vrId     prefix/mask   ecmp/nh/arp   vid     interface           mac        \n" ..
			   "                            ID                                             \n" ..
			   "-----  ----------------  --------  -----  -------------  ------------------\n")
	else
		print(" vrId                   prefix/mask                ecmp/nh/arp   vid     interface              mac          \n" ..
    ...
    if true == command_data["status"] then
        local iterator
        for iterator, devNum, entry_index, prefix_ip_address, prefix_length, vrId, fdb_next_hop_entry in
							...
							if ecmp_iter == ecmpEntryIndex then
								if(next_hop_entry_showing)then
									if(is_ipv4_protocol)then
										...
										print(string.format( "%-5s%-48s%-11s%-7s%-16s%-24s",
											alignLeftToCenterStr(vrId_string, 2),
...
--                    print("next_hop_id=",next_hop_id)
						...
						-- A regular entry can be located in the NextHop table or directly in the FDB entry
						if (num_ecmp_paths==0)then
							-- data returned from FDB entry
							next_hop_entry = fdb_next_hop_entry
							next_hop_id_string  = tostring(next_hop_id)
							vlanId=0;
						else
							-- data returned from NextHop entry
