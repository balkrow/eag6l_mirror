...
local function show_capabilities_device_func_given_device(command_data, devNum, header_str)
    ...
    local em_entries_number_string_prefix
    local em_entries_number_string_suffix
    local em_entries_number, em_entries_number_string
    ...
    em_entries_number_string_prefix         =
                                    "Number of entries in Exact Match table\t\t\t\t"
    em_entries_number_string_suffix         = ""
    ...
    -- Getting of ExactMatch table entries number.
    command_data:clearStatus()

    if true  == command_data["status"] then
        em_entries_number = system_capability_get_table_size(devNum, "EM")
        em_entries_number_string = strIfNotNil(em_entries_number)
    end

    command_data:updateStatus()

    -- ExactMatch table entries number string formatting and adding.
    command_data:setResultStr(em_entries_number_string_prefix,
                              em_entries_number_string,
                              em_entries_number_string_suffix)
    command_data:addResultToResultArray()
