...
function showCountersACL(params)
    local ret, val
    local i, j
    local header, body, block
    local ipcl0BlocksList, epclBlocksList
    ...
    ipcl0BlocksList = getCNCBlock("IPCL_0")
    if ipcl0BlocksList == -1 then
      return false, "Wrong client name IPCL_0 for selected device"
    end
    epclBlocksList = getCNCBlock("EPCL")
    if epclBlocksList == -1 then
      return false, "Wrong client name EPCL for selected device"
    end
    ...
    if ((IPCL0_table ~= nil) or (EPCL_table ~= nil)) then
        retTable["pass"] = {}
        for j = 1, #devices do -- main devices loop
            ...
            for i = 1, #ruleIdx do
			  ...
			  direction = "ingress"
			  if (EPCL_table ~= nil) and EPCL_table[index] then
				  direction = "egress"
              end

			  if direction == "ingress" then
				  block = ipcl0BlocksList[1]
			  else
				  block = epclBlocksList[1]
			  end

              if ((IPCL0_table ~= nil) and (IPCL0_table[index] ~= nil)) or
				  ((EPCL_table ~= nil) and (EPCL_table[index] ~= nil)) then
    ...
    if (luaGlobalGet("EgressQueue") ~= nil) then
        ...
        for d = 1, #devices do -- main devices loop
            ...
            if (is_sip_5_20(devNum)) then
                ret, val = myGenWrapper("cpssDxChCncEgressQueueClientModeGet", {
                    { "IN", "GT_U8", "devNum", devNum },
                    { "OUT", "CPSS_DXCH_CNC_EGRESS_QUEUE_CLIENT_MODE_ENT", "modePtr" }
                })

                if ret ~= 0 then
                    print("cpssDxChCncEgressQueueClientModeGet() failed: " .. returnCodes[ret])
                    return false, "Error in cpssDxChCncEgressQueueClientModeGet"
                end
            end

            local isReducedMode = false
            if (is_sip_5_20(devNum) and val["modePtr"] == "CPSS_DXCH_CNC_EGRESS_QUEUE_CLIENT_MODE_TAIL_DROP_REDUCED_E") then
              isReducedMode = true
            end
                ...
                for i = 1, #listOfPorts do -- loop over the ports
                    ...
                    for j = 1, #listOfTC do -- loop over the traffic class
                        for k = 1, #listOfDP do -- loop over the drop precedence
                            ...
                            local trafficClass
                            if (is_sip_5_20(devNum) and isReducedMode == true) then
                              trafficClass = numberToColor[4]
                            else
                              trafficClass = numberToColor[tonumber(currDp, 2)]
                            end
