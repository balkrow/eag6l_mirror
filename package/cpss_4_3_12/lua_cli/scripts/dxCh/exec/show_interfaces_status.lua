...
local function show_interfaces_status(params)
   ...
   CPSS_DXCH_PORT_SERDES_LOOPBACK_DIGITAL_RX2TX_E = "SD rx2tx"
    ...
    local skipRes
    local skip_entry -- indication that the info about the port should be skipped (not shown)
    ...
    if true == command_data["status"] then
        ...
        for iterator, devNum, portNum in command_data:getPortIterator() do
            ...
            command_data:setFailLocalStatusOnCondition(not isDXCHFamily(devNum))
            ...
            if true == command_data["local_status"] then
                ...
                if params.linkup_only and not port_link then
                    skip_entry = true
                end
            end
            ...
            if skip_entry then
                -- ignore the port
            elseif (port_interface_mode_string ~= "n/a" or
...
local function show_interfaces_status_linkup_only(params)
    params.linkup_only = true
    return show_interfaces_status(params)
end

-------------------------------------------------
-- command registration: show interfaces status
-------------------------------------------------
CLI_addCommand("exec", "show interfaces link-UP-status", {
    func = show_interfaces_status_linkup_only,
    help = "show ports status for those with link UP (ignore ports with link DOWN)",
    params = {
        { type = "named",
            "#all_interfaces",
          mandatory = { "all_interfaces" }
