...
local devPortPatterns =
{
        ...
        ["CPSS_PP_FAMILY_DXCH_IRONMAN_E"] = {
             {29,31,14,9,21,40}--ironMan-L
             ,{2,11,4,8,10,0}--ironMan-M
        },
        ["CPSS_PP_FAMILY_DXCH_AAS_E"] = {
              {129,40,154,89,51,109}--2 tiles
              ,{0,8,20,28,40,48}--single tile
        },
}
...
local is_IronMan_M_in_system_flag = false
function is_IronMan_M_in_system()
    return is_IronMan_M_in_system_flag
end
...
function fillDeviceEnvironment()
    ...
    -- IM-M use cpssInitSystem 41,3 or 41,4
    if (boardIdx == 41) and ((boardRevId == 3) or (boardRevId == 4)) then
        is_IronMan_M_in_system_flag = true
    end
    ...
    for i = 0,127 do
        if sysInfo[i] then
            ...
            multiDevEnv[dev_index].maxPhyPort =
                system_capability_get_table_size(dev, "PHYSICAL_PORT");
            if multiDevEnv[dev_index].maxPhyPort ~= nil then
                multiDevEnv[dev_index].ePortBase = multiDevEnv[dev_index].maxPhyPort
                if  multiDevEnv[dev_index].ePortBase < 256 then
                     multiDevEnv[dev_index].ePortBase = 256
                end
            end
            -- CPU port
    ...
    -- set c-code prvTgfDevNum variable instance for LUA task
    luaShellExecute("prvTgfSetActiveDevice " .. devEnv.dev)
...
function getPortMapInfo(devNum,portNum)
    ...
    local function mappingTypeStrGet(mappingType)
        local my_str = {    [0] = "CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E",
                            ...
                            [4] = "CPSS_DXCH_PORT_MAPPING_TYPE_SERDES_E",
                            [5] = "CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E",
                            [6] = "CPSS_DXCH_PORT_MAPPING_TYPE_MAX_E" }
...
--          "NOT_AC5P_GM"                  --> is this a not AC5P GM simulation
...
--          "VTCAM_MANAGER_HA"            --> sip6 devices , Aldrin2 and caelum : VTCAM Manager HA support
...
function is_supported_feature(devNum, singleFeature , extraParamName , extraParamValue)
    ...
    elseif singleFeature == "NOT_AC5P_GM" then
        isSupported = not (isGmUsed() and
    ...
    elseif (singleFeature == "EEE") then
        -- test supported for PM. Feature is not supported for GM or SIP_6 device.
        isSupported = dev_is_sip_5 and (not isGmUsed()) and (not dev_is_sip_6) and isPortManagerMode()
        if (is_xCat3x_in_system()) then
            xCat3x_set_ports_needed_from_aldrin(extraParamName,extraParamValue)
        end
    ...
    elseif singleFeature == "SIP6_LATENCY_MONITOR" then
        ...
        isSupported = dev_is_sip_6 and not isGmUsed()
    elseif singleFeature == "SIP6_10_LATENCY_PROFILE_MODE" then
        -- the GM not hold the LMU unit
        isSupported = dev_is_sip_6_10 and not isGmUsed()
    ...
    elseif singleFeature == "FDB_MANAGER_HA" then
        isSupported = ((devFamily == "CPSS_PP_FAMILY_DXCH_FALCON_E" or devFamily == "CPSS_PP_FAMILY_DXCH_AC5P_E" or
                        ...
                        (devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" and not isSimulationUsed()) ) and not isGmUsed())
    elseif singleFeature == "VTCAM_MANAGER_HA" then
        isSupported = ((devFamily == "CPSS_PP_FAMILY_DXCH_FALCON_E"     or devFamily == "CPSS_PP_FAMILY_DXCH_ALDRIN2_E" or
                        ...
                        devFamily == "CPSS_PP_FAMILY_DXCH_HARRIER_E"    or
                       (devFamily == "CPSS_PP_FAMILY_DXCH_BOBCAT2_E"   and subFamily == "CPSS_PP_SUB_FAMILY_BOBCAT2_BOBK_E") or
                       (devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" and not isSimulationUsed()) )  and not isGmUsed())

    elseif singleFeature == "PREEMPTION_MAC_802_3BR" then
        isSupported = (devFamily == "CPSS_PP_FAMILY_DXCH_AC5P_E" or
                       devFamily == "CPSS_PP_FAMILY_DXCH_HARRIER_E" or
                       devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" or
                       devFamily == "CPSS_PP_FAMILY_DXCH_AAS_E")
                       and not isGmUsed()

    elseif singleFeature == "QBV" then
        isSupported = (devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E")
                       and not isGmUsed()

    elseif singleFeature == "EGF_VLAN_BASED_TXQ_REMAP" then
        isSupported = dev_is_sip_6_20
    elseif singleFeature == "EPB_SPILLOVER" then
        isSupported = (devFamily == "CPSS_PP_FAMILY_DXCH_AAS_E")
    else
        -- unknown feature ?!
    end
