...
local Custom_order_List_Last = {
 ...
 "rppInit",
 "hqos",
 "shared_library_sanity"
}
-- *****************************************************************
local run_all_mode = false
local reset_cpss_if_fail_List = {
  HA_TEST_NAME,
  VTCAM_MANAGER_HA_TEST_NAME,
  VTCAM_MANAGER_PARALLELHA_TEST_NAME,
  FDB_MANAGER_HA_TEST_NAME,
...
local function check_Custom_Order_List(testName)
   if  DeviceFamily == "CPSS_PP_FAMILY_DXCH_FALCON_E" or  DeviceFamily == "CPSS_PP_FAMILY_DXCH_ALDRIN2_E" or
    DeviceFamily == "CPSS_PP_FAMILY_DXCH_ALDRIN_E" or DeviceFamily == "CPSS_PP_FAMILY_DXCH_AC5P_E" or
    DeviceFamily == "CPSS_PP_FAMILY_DXCH_HARRIER_E" or "CPSS_PP_FAMILY_DXCH_BOBCAT2_E" then
...
function checkExpectedCounters(lineNum, devPortCounters)
  ...
  for dev,portCounters in pairs(devPortCounters) do
    for port, expectedCounters in pairs(portCounters) do
     ...
     -- Begin port's counters handling
     local mapInfo = getPortMapInfo(dev,port)
     if mapInfo and 
        mapInfo.mappingType == "CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E" then
        -- Egress Loopback port has NO MAC, so skip counters handling
     else
      local counters
     ...
     end -- mapInfo.mappingType == "CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E"
     -- END  port's counters handling

    end-- for pairs(portCounters)
  end-- for pairs(devPortCounters)
...
local function internal_run_specific(name,totalResults)
    ...
    if (getTestResult() == true) then
      if (isDeviceNotSupported or isLongTest) then
        ...
        --reset cpss if error during HA tests under run all
        if run_all_mode == true then
            for ii = 1 ,#reset_cpss_if_fail_List  do
                 if  reset_cpss_if_fail_List [ii]== name then
                     executeStringCliCommands("do reload")
                 end
            end
        end
      end
...
{
    -- AAS Tests not need Jira yet because device under development
     {testName = "an_phy_mode"              , --[[isGmUsed = false ,]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = "The test fail and fails other tests"}
    ,{testName = "flow_control"             , --[[isGmUsed = false ,]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = Falcon_fail_the_test}
    ,{testName = "led_example"              , --[[isGmUsed = false ,]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = Falcon_fail_the_test}
    ,{testName = "wirespeed_with_shaper_example" , --[[isGmUsed = false ,]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = Falcon_fail_the_test}
    ,{testName = "eom"                      , --[[isGmUsed = false , ]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  ,  cause = Falcon_fail_the_test}
    ,{testName = "nat66_basic"            , isGmUsed = true ,devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = "The test fail due to reduced LPM mem size"}
    ,{testName = "nat66_tunneling"        , isGmUsed = true ,devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = "The test fail due to reduced LPM mem size"}
    ,{testName = "wirespeed_with_from_to_CPU" , isEmulatorUsed  = true ,devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" ,  cause = "Running time is 1317 second(s)"}
    ,{testName = "pip_example"                , isEmulatorUsed  = true ,devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" ,  cause = "Running time is 1519 second(s)"}
    ,{testName = "linkLocalMCFiltering"       , devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" ,  cause = "Crash on LPM address"}
    ,{testName = "bpe_802_1br",maxPortNum=128 , devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "not implemented for ports >= 128"}
    ,{testName = "falcon_50G_to_400G_port_create_port_delete_under_traffic"  , --[[isGmUsed = false ,]]devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  , cause  = "cause regression in tests after it (since  cpss_4.3_394)"}
    ,{testName = "falcon_50G_port_create_port_delete_under_traffic"         , --[[isEmulatorUsed = true ,]] devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E"  ,  cause = "cause regression in tests after it (since  cpss_4.3_394)"}
    ,{testName = "_802_1cb", devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "crash when ports 0..5 used as default "}
    ,{testName = "cnc_egress_queue", devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "CNC PREQ Pass/Drop client Flex Index not supported yet "}
    ,{testName = "policer_envelope", devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "Policer need alignment"}
    ,{testName = "policer_port"    , devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "Policer need alignment"}
    ,{testName = "pop_passenger_vlan_tag"    , devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "need to debug"}
    ,{testName = "pop_push_passenger_vlan_tag"    , devFamily = "CPSS_PP_FAMILY_DXCH_AAS_E" , cause  = "need to debug"}
    ...
    ,{testName = "prp_san"  , isGmUsed = true ,devFamily = "CPSS_PP_FAMILY_DXCH_IRONMAN_E"  , cause  = "prp not implemented in GM yet (in DP units : RX,HBU,TX)"}
    ,{testName = "cpu_rx_rate_limit" , isEmulatorUsed  = true ,devFamily = "CPSS_PP_FAMILY_DXCH_IRONMAN_E" --[[ALL]] ,  cause = "feature working but test cannot run on Emulator"}
    ,{testName = "cpss_locking_basic"     , devFamily = "CPSS_PP_FAMILY_DXCH_IRONMAN_E"   , cause = "Test sometimes crashed in LUA infra code when run on internal CPU"}
    ...
    ,{testName = "cpss_locking_basic"     , devFamily = "CPSS_PP_FAMILY_DXCH_AC5X_E"   , cause = "Test sometimes crashed in LUA infra code when run on internal CPU"}
    ...
    ,{testName = "bpe_802_1br"                    , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:8165"}
    ,{testName = "vss_lc1_vss3"                   , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:6814"}
    ,{testName = "vss_lc1_vss4"                   , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:6815"}
    ,{testName = "vss_lc5_vss3"                   , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:6816"}
    ,{testName = "fastBoot_trafficAfterEeprom"    , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:6817"}
    ,{testName = "wirespeed_with_shaper_example"  , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "JIRA:6818"}
    ,{testName = "cpss_locking_basic"             , devFamily = "CPSS_PP_FAMILY_DXCH_AC5_E"   , cause = "Test sometimes crashed in LUA infra code when run on internal CPU"}
...
local function getMaxPortNum(devEnv)
    local maxPort = 0
    for ii = 1 ,#devEnv.port do
        if maxPort < devEnv.port[ii] then
            maxPort = devEnv.port[ii]
        end
    end
...
local function check_forbidden_test_list(testName,totalResults)
    ...
    local maxPortNum = getMaxPortNum(devEnv)
    local additional_reason = ""
    local additionalCheck = {needAdditional = false, matchAdditional=false}

    for ii = 1 ,#forbidden_test_list do
        additionalCheck = {needAdditional = false, matchAdditional=false}
        additional_reason = ""
        ...
        if(forbidden_test_list[ii].maxPortNum == nil)  -- 'maxPortNum' ignored
        then
            -- ignored
        else
            if (maxPortNum < forbidden_test_list[ii].maxPortNum)
            then
                additionalCheck.needAdditional  = true
                additionalCheck.matchAdditional = false
            else
                additional_reason = " use 'max port' "..to_string(maxPortNum)..", but limited to "..to_string(forbidden_test_list[ii].maxPortNum)
                additionalCheck.needAdditional  = true
                additionalCheck.matchAdditional = true
            end
        end
        ...
        if forbidden_test_list[ii].testName == testName and isType_ok and devFamily_ok and
        (not additionalCheck.needAdditional or additionalCheck.matchAdditional) then

            printMsg("testName : " .. testName .. " skipped (is in forbidden list) , because : " .. forbidden_test_list[ii].cause .. additional_reason)
...
--]]
    ...
    --restore run all flag to default
    run_all_mode = false
    -- restore force system port to link UP
    luaTgfConfigSystemPorts(false)
