...
--default values
    ...
    if devFamily ~= "CPSS_PP_FAMILY_DXCH_HARRIER_E" and devFamily ~= "CPSS_PP_FAMILY_DXCH_IRONMAN_E" then -- harrier doesn't have mac stage
        expStage[13] ={ ["stage"] ="CPSS_DXCH_PACKET_ANALYZER_LOOKUP_STAGES_EGRESS_PRE_MAC_E"                , ["hit"]=0x0}
    end
    if devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" then -- ironman doesn't have E-POLICER stage
        expStage[12] = { ["stage"] ="CPSS_DXCH_PACKET_ANALYZER_LOOKUP_STAGES_EGRESS_PRE_MAC_E"                , ["hit"]=0x0}
    end
...
for ii, sendPort in ipairs(ports) do
    ...
    if devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" then --equeue is not defulte bound
        enterCmd = [[
            end
            debug-mode
            packet-analyzer manager 1 device 0
            no configure stage mll
    ...
    executeStringCliCommands(enterCmd)
        ...
        expStage[11]["hit"] = 0
        expStage[12]["hit"] = 0
        if devFamily ~= "CPSS_PP_FAMILY_DXCH_HARRIER_E" and devFamily ~= "CPSS_PP_FAMILY_DXCH_IRONMAN_E" then
            expStage[13]["hit"] = 0
        end
    ...
    if devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E" then
        expStage[6]["hit"] = 0
        expStage[9]["hit"] = numOfLinkUps
        expStage[12]["hit"] = numOfLinkUps  -- mac is in 12 not 13
    end
    ...
    if (devFamily == "CPSS_PP_FAMILY_DXCH_AC5X_E") or (devFamily == "CPSS_PP_FAMILY_DXCH_AC5P_E") or
       (devFamily == "CPSS_PP_FAMILY_DXCH_HARRIER_E") or (devFamily == "CPSS_PP_FAMILY_DXCH_IRONMAN_E") then
        enterCmd = [[
