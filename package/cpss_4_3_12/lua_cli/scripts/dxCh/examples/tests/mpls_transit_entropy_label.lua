...
local function set_hash_for_test(mode)
    local apiName = "cpssDxChTrunkHashGlobalModeSet"

    printLog("Set hash mode [" .. mode .."] , as the entropy lable use this value")

    local isError , result, values =
        genericCpssApiWithErrorHandler(command_data,
        apiName, {
        { "IN",     "GT_U8",     "devNum",   devNum },
        { "IN",     "CPSS_DXCH_TRUNK_LBH_GLOBAL_MODE_ENT",    "hashMode", mode}
    })
...
if is_sip_6(devNum) then 
    set_hash_for_test("CPSS_DXCH_TRUNK_LBH_PACKETS_INFO_E")
end
...
if is_sip_6(devNum) then 
    set_hash_for_test("CPSS_DXCH_TRUNK_LBH_PACKETS_INFO_CRC_E")
end
