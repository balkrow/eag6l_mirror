...
local error = false -- there is a global 'error' function , so we not want to override it

SUPPORTED_FEATURE_DECLARE(devNum, "IP_MC_ROUTING")
SUPPORTED_FEATURE_DECLARE(devNum, "NOT_AC5P_GM")
...
if tonumber(value.countersPtr.inMcNonRoutedExcpPkts) ~= 1 then
    print("ERROR in inMcNonRoutedExcpPkts")
    --error = true --> seems to get here but was not failing the test
end
