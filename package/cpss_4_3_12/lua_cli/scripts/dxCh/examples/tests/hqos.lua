...
traffic
traffic-generator emulate port ${dev}/]] ..devEnv.port[2].. [[ data 000111222333000222333444 count 1
]]
...
local function doTest()
    local value,rc
    local testedRemotePort =402
    local cascadeCpuPort =257
    local queue = 6
    local pm=0
    local tileNum=4
    local prevPortNum=0
    ...
    --use mode 13 for modeMaxMac auto-definition
    ...
    print("Read CNC counters in order to clean")
        --read counters in order to clean
res, val = myGenWrapper("prvCpssSip6TxqDebugCncCounterGet",{
    ...
    prvLuaTgfPortForceLinkUpEnableSet(devNum,devEnv.port[2],true)
