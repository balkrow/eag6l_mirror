...
local function mac_to_me(devNum, index, mac, vid, unset)
    ...
    macToMeEntryIndex = sharedResourceNamedMac2meMemoAlloc(
        "VXLAN_MAC_TO_ME", 1 --[[size--]], false --[[fromEnd--]]);

    if macToMeEntryIndex == nil then
        printLog ("macToMeEntryIndex allocation failed")
        setFailState()
        return
    end
...
local function pcl_rule(rule_index, pcl_id, service_id, e_vlan,vrfId, unset)
    ...
    if is_sip_7(devNum) then
        if unset then
            ...
            result = wrlPrvTgfTcamClientsPclFieldRegister(
                0, "UDB_CFG", "TUNNEL_L3_MINUS_2", (udb_offset * 8), 24)
            if result ~= 0 then
    ...
    if is_sip_7(devNum) then
        ...
        {
            {"APP_REGISTRED0", 0, 24, 0x00FFFFFF, service_id}
        }
...
local function configure_pcl(unset)
    ...
    if is_sip_7(devNum) then
        lookupCfgPtr = {
            ...
            groupKeyTypes = {
                nonIpKey = "CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_UDB_30_E",
                ipv4Key  = "CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_UDB_30_E",
                ipv6Key  = "CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_UDB_30_E"
            },
            exactMatchLookupSubProfileId = 1
        };
    else
        lookupCfgPtr = {
            enableLookup = enable,
