...
local managePm = false
local devFamily = wrlCpssDeviceFamilyGet(devNum)

if ("CPSS_PP_FAMILY_DXCH_AC5_E" == devFamily) then
    ...
    if val and val.valuePtr and val.valuePtr ~= 0 then
        managePm = true
        printLog("Disable PM for AC5 device.")
...
local rate_limit_in_kbps = 17500
...
local rates = {
    rate_limit_in_kbps,
    committedRateGet(2 , 100 , packet_size, true), -- the wirespeed generating port
    rate_limit_in_kbps,
    rate_limit_in_kbps,
}
...
local egressInfoTable = {
    -- expected the mac-da to be 'unregistered multicast' --> flood the vlan
    {portNum = port1 , rate = rates[1], rateDiffInPercent = rate_limit_diff_in_percent},
    {portNum = port2 , rate = rates[2], rateDiffInPercent = src_port_rate_diff_in_percent},
...
local configFileName = "storm_control_unr_mc"
...
local function postTestConfig()
    ...
    -- clean counters on source port. It's needed for BC3 CG ports after stop WS traffic.
    resetPortCounters(devNum, port2)
    if managePm == true then
        printLog("Enable PM for AC5 device.")
