...
local function switchLmuTriggering(latencyProfile1, latencyProfile2, configure, ...)
    ...
    if configure == true then
        ...
        lmu_env["latencyProfile"] = bit_and(latencyProfile1 + latencyProfile2, mask)
...
local function validateStats(mode, latencyProfile1, latencyProfile2)
    ...
    local diff, newStats
    local latencyProfile = bit_and(latencyProfile1 + latencyProfile2, mask)
...
local function main_test()
    ...
    -- sip6 has 512 profiles vs 4096 profiles on sip7
    -- For mode"port" the sum(profile1+profile2) is split by latency_monitoring.lua
    -- as (0x1F0,0xF)  vs  (0xFF0,0xF)
    if is_sip_7(devEnv.dev) then
        pext = 4000
        qext = 4000
        mask = 0xFFF
    else
    ...
    profileList = {67+qext, 23+pext} -- 67:Queue Mode Profile, 23: Port Mode Profile

    oldStats = readStats(profileList)

    for i,mode in ipairs({"port", "queue"}) do
        if mode == "port" then
            single_test(mode, 11, 12+pext, devEnv.port[1+i]) --11: srcProfile, 12:trgProfile
        else
            single_test(mode, 33, 34+qext, devEnv.port[1+i]) --33: portProfile, 34:queueProfile
        end
    end
    local latencyRange = ""
