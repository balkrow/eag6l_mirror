...
#define TM_DBG_INFO(x)              if(PRV_SHARED_GENERIC_TM_DIR_GLOBAL_VAR_GET(tmDbgLogOn)) cpssOsPrintf x

#define XEL_TO_CPSS_ERR_CODE(x)     ( (PRV_CPSS_PP_MAC(devNum)->tmInfo.tmLibIsInitilized == GT_FALSE) ? GT_NOT_INITIALIZED :  \
                                       ( (x) < 0) ? PRV_SHARED_GENERIC_TM_DIR_GLOBAL_VAR_GET(tmToCpssErrCodesNeg)[-(x)] : \
                                                    PRV_SHARED_GENERIC_TM_DIR_GLOBAL_VAR_GET(tmToCpssErrCodes)[(x)])
