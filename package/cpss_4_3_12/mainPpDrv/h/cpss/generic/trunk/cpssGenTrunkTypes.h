...
/**
* @enum CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT
 *
 * @brief enumerator that describe the load balance policy type
*/
typedef enum{

    /** @brief Load balance by rewriting the complete table
     *  More number of flow will be re-mapped. (Legacy Mode)
     */
    CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E,

    /** @brief Load balance by modifing the minimal number of flows */
    CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E
}CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT;

/**
* @struct CPSS_TRUNK_POLICY_STC
 *
 * @brief TRUNK ECMP Policy info.
 * Relevant mode : High level mode
*/
typedef struct{
    /** @brief Policy mode */
    CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT policy;

}CPSS_TRUNK_POLICY_STC;
