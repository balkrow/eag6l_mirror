...
typedef enum{
     ...
     *  cpssDxChTrunkMemberDisabledAdd(...)
...
/**
*  PRV_CPSS_TRUNK_ECMP_TABLE_SET_FUNC function
*
* DESCRIPTION:
*       Function Relevant mode : All modes
*       in sip7 the Trunk ECMP is dedicated table (not part of the L2 ECMP table).
*       in sip5 the Trunk ECMP part of L2 ECMP table.
*       Set the Trunk/L2 ECMP table specific entry.
*
...
*
*/
typedef GT_STATUS (*PRV_CPSS_TRUNK_ECMP_TABLE_SET_FUNC)
(
    IN  GT_U8                                devNum,
    IN  GT_TRUNK_ID                          trunkId,
    IN  GT_U32                               index,
    IN  CPSS_TRUNK_MEMBER_STC                *trunkMember,
...
typedef struct{
    ...
    /** @brief function that Sets Trunk/L2 ECMP entry
      * in sip7 the Trunk ECMP is dedicated table (not part of the L2 ECMP table).
      * in sip5 the Trunk ECMP part of L2 ECMP table.
     */
    PRV_CPSS_TRUNK_ECMP_TABLE_SET_FUNC ecmpTableSetFunc;

} PRV_CPSS_FAMILY_TRUNK_BIND_FUNC_STC;

/**
* @struct PRV_CPSS_TRUNK_RESILIENT_HASH_STC
 *
 * @brief Structure to hold each node of hash index list
 *
*/
typedef struct PRV_CPSS_TRUNK_RESILIENT_HASH_STC
{
    /* hash index value */
    GT_U32                            hashIndex;

    /* Point to the next hash index for the same trunk member */
    struct PRV_CPSS_TRUNK_RESILIENT_HASH_STC    *nextPtr;

    /* Point to the previous hash index for the same trunk member */
    struct PRV_CPSS_TRUNK_RESILIENT_HASH_STC    *prevPtr;
}PRV_CPSS_TRUNK_RESILIENT_HASH_STC;

/**
* @struct PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC
 *
 * @brief Trunk Hash DB to hold "Designated Hash list" per trunk and
 *        Management pointers to a list of Hash table.
 *        Memory to all the pointers, allocated at the time of cpssDxChTrunkFlexInfoSet,
 *        when the selected policy is Resilient.
 *
 *        To efficiently update "Hash index" to "trunk Member" mapping. (Resilient policy)
 *        in case of trunk member Add/Remove.
*/
typedef struct{
    /** List of ECMP Hash
      * Valid only when load balance type is FLEX, Policy is Resilient
      */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC *allResilientEcmpHashArray;

    /** List of "First ECMP Hash node" for each trunk member. (myTrunkMaxNumOfMembers)*/
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **firstEcmpResilientHashNodeArray;

    /** List of "Last ECMP Hash node" for each trunk member. (myTrunkMaxNumOfMembers) */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **lastEcmpResilientHashNodeArray;

    /** Index to "membersPtr" - Where number of hash index is less compared to the other members
     *  0 - When all members have equal number of "ECMP Hash index"
     *  n - (n-1), (n-2)...         0                 - Have X+1 "ECMP Hash Index"
     *       n   , (n+1), (n+3) ... lastTrunkMember   - Have X   "ECMP Hash Index"
     */
    GT_U32                      trunkMemberWithLessEcmpResilientHashIndex;

    /** List of all Designated Hash.(numOfDesignatedTrunkEntriesHw/2) - UC part
     *  Each Resilient trunk will have its own "designated hash list".
     */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     *allUcDesignatedResilientHashArray;

    /** List of all Designated Hash.(numOfDesignatedTrunkEntriesHw/2) - MC part
     *  Each Resilient trunk will have its own "designated hash list".
     */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     *allMcDesignatedResilientHashArray;

    /** List of "First Designated Hash node" for each member. (myTrunkMaxNumOfMembers) - UC part
      * List of "Last Designated Hash node" for each member. (myTrunkMaxNumOfMembers) - UC Part
      */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **firstUcDesignatedResilientHashNodeArray;
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **lastUcDesignatedResilientHashNodeArray;

    /** List of "First Designated Hash node" for each member. (myTrunkMaxNumOfMembers) - MC part
      * List of "Last Designated Hash node" for each member. (myTrunkMaxNumOfMembers) - MC Part
      */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **firstMcDesignatedResilientHashNodeArray;
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **lastMcDesignatedResilientHashNodeArray;

    /** Index to "membersPtr" - Where number of hash index is less compared to the other members
     *  0 - When all members have equal(X) number of "Designated Hash index"
     *  n - (n-1), (n-2)...         0                 - Have X+1 "Designated Hash Index"
     *      n    , (n-1), (n-3) ... lastTrunkMember   - Have X   "Designated Hash Index"
     */
    GT_U32                      trunkMemberWithLessUcDesignatedResilientHashIndex;
    GT_U32                      trunkMemberWithLessMcDesignatedResilientHashIndex;

    /** Sorted list of Ports for cascade Trunk
      * Based on adding sequence,
      * Gets reset by - cascadeTrunkSet
      */
    GT_PHYSICAL_PORT_NUM        *cascadeResilientPortsArray;
    GT_U32                      cascadeResilientPortsCnt;

    /** This 3 are required in case of HW update for removal of member
     *   - Indicates the last hash node from where the new hashes started appending
     */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **prevLastUcDesignatedResilientHashNodeArray;
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **prevLastMcDesignatedResilientHashNodeArray;
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC     **prevLastEcmpResilientHashNodeArray;
}PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC;
...
typedef struct{
    ...
    /** Valid for
     *      - Regular Trunk with Flex mode
     *      - Or Cascade Trunk
     */
    CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT  policy;

    /* Memory will be allocated only when the policy type is resilient */
    PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC  *resilientHashMappingDataPtr;
...
/**
* @struct PRV_CPSS_TRUNK_DB_SORT_INFO_STC
 *
 * @brief structure to allow 'qsort' function for sorting trunk members (when running in sort mode)
 *
 *
*/
typedef struct {
    CPSS_TRUNK_MEMBER_STC   member;
    GT_BOOL                 isLocalMember;
}PRV_CPSS_TRUNK_DB_SORT_INFO_STC;
...
typedef struct{
    ...
    /* @brief point to array, that used by the sorting algorithm , this is 'helper'
       memory , and not hold relevant info out of the scope of 'under sorting action'
       so used only in function trunkDbMembersFromStartIndexSort(...) */
    PRV_CPSS_TRUNK_DB_SORT_INFO_STC *tempSortingArr;
} PRV_CPSS_TRUNK_DB_INFO_STC;
...
/**
* @internal prvCpssTrunkDbCascadePolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
