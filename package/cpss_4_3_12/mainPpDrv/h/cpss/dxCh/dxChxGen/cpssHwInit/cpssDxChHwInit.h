...
/**
* @struct CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC_FDB_LEARN_PRIORITY_INFO_STC
*
* @brief     FDB global info about vPorts that needed for : MAC Spoof Protection
*            when working in 'vPort mode'
*
...
typedef struct{
    ...
    */
    ...
    /**@brief :The number of vPorts that are subject to the Learn Priority feature.
      * APPLICABLE RANGE : 0.. ('num of vPorts' - maxNumOfPhyPortsToUse - 1)
    */
    GT_U32 numberOfVPorts;

}CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC_FDB_LEARN_PRIORITY_INFO_STC;

/**
* @struct CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC
*
* @brief  info need in order to work in 'vPort mode'
*
* APPLICABLE DEVICES: AAS.
*/
typedef struct{
    ...
    */
    GT_BOOL enableVPortMode;

    /**@brief : FDB global info about vPorts that needed for : MAC Spoof Protection
     *          when working in 'vPort mode'.
     *      relevant when enableVPortMode == GT_TRUE
    */
    CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC_FDB_LEARN_PRIORITY_INFO_STC   learnPriority;

}CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC;
...
typedef struct{
    ...
    /** @brief Indicates whether serdes reference clock was found in APP database.
     *  May be used for differentiating between dedicated entry in data base
     *  and SAR register value.
     */
    GT_BOOL serRefClkOverriden;
    ...
    *         In AAS : depend on : vPortModeInfo.enableVPortMode
    *              if GT_FALSE : support any number between  0..1024.
    *              else        : support any number between 64..1024.
    *                            the 'vPort' range starts after the maxNumOfPhyPortsToUse
    *
    */
    ...
    /** @brief Info needed in order to work in 'vPort mode' to allow
     *   larger tunnels scale , with enhanced tunnel flexibilities and
     *   hierarchical ECMP.
     *
     * (APPLICABLE DEVICES: AAS)
     */
    CPSS_DXCH_PP_PHASE1_VPORT_MODE_INFO_STC vPortModeInfo;

     /** @brief config set (enable/disable) enumerator for phy ref
     *          clock Tx Pu and Oe values
     *
     * (APPLICABLE DEVICES: Ironman)
     */
    CPSS_PHY_REF_CLK_PU_OE_CFG_ENT phyRefClkPuOeCfg;
...
typedef struct{
    ...
    /** @brief FDB/CNC upload queue type
     */
...
typedef enum{
    ...
    */
    ...
    /** @brief  WA for devices: Falcon, AC5X, AC5P, Harrier, Ironman:
        For OAM UP-MEP use case, when the attached egress port is DOWN, the incoming CCM packets that are destinate
        for that egress port will be dropped by the Egress Filter (EGF) and therefore the UP-MEP instance will be
        DOWN (which is not like the standard behavior). Enable this WA to configure a new tail-drop profile that will
        be attached to such an egress port to allow packets to pass the EGF but to be dropped at the pre-Q stage.
    */
    CPSS_DXCH_IMPLEMENT_WA_OAM_UP_MEP_INIT_E,

    /** @brief  WA for devices: Falcon, AC5X, AC5P, Harrier, Ironman:
        Remove the OAM-Up-MEP WA global configurations from the device.
    */
    CPSS_DXCH_IMPLEMENT_WA_OAM_UP_MEP_REMOVE_E,

    /** @brief  WA for devices: Falcon, AC5X, AC5P, Harrier, Ironman:
        For OAM UP-MEP use case, when the attached egress port is DOWN, the incoming CCM packets that are destinate
        for that egress port will be dropped by the EGF and therefore the UP-MEP instance will be
        DOWN. This use case configures a specific port as the target egress port by making sure its EGF link status
        will be always CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E and mark it as an oam_upmep_egress_port
        in db.
    */
    CPSS_DXCH_IMPLEMENT_WA_OAM_UP_MEP_PORT_CONFIG_E,

    /** @brief  WA for devices: Falcon, AC5X, AC5P, Harrier, Ironman:
        For OAM UP-MEP use case, when the attached egress port is DOWN, the incoming CCM packets that are destinate
        for that egress port will be dropped by the EGF and therefore the UP-MEP instance will be
        DOWN. This use case de-configures the target egress port by removing the indication of this port as an
        oam_upmep_egress_port by removing it from the db
    */
    CPSS_DXCH_IMPLEMENT_WA_OAM_UP_MEP_PORT_DECONFIG_E,
...
typedef struct{
     ...
     *  (APPLICABLE RANGE: according to 'phase1'
...
/**
* @enum CPSS_DXCH_DMA_MEM_TYPE_ENT
 *
 * @brief Defines DMA memory types
*/
typedef enum{

    /** @brief Host DRAM
     */
    CPSS_DXCH_DMA_MEM_TYPE_DRAM_E,

    /** @brief Control and Management Subsystem internal SRAM
     */
    CPSS_DXCH_DMA_MEM_TYPE_CNM_SRAM_0_E

} CPSS_DXCH_DMA_MEM_TYPE_ENT;

/**
* @struct CPSS_DXCH_HW_PP_DMA_CONFIGURE_STC
 *
 * @brief Configure CNM unit DMA related registers.
 *
*/
typedef struct{
    /**
     * @brief 64-bit base address of DMA served range
     * Should be aligned to 0x100000
    */
    GT_U64   dmaBaseAdress;

    /**
     * @brief DMA served address range size in bytes
     * Should be aligned to 0x100000
     * Should not be more than 0x80000000
    */
    GT_U32   dmaRangeSize;

    /**
     * @brief Type of memory to use for DMA operations
    */
    CPSS_DXCH_DMA_MEM_TYPE_ENT  dmaMemoryType;

} CPSS_DXCH_HW_PP_DMA_CONFIGURE_STC;

/**
* @struct CPSS_DXCH_HW_PP_DMA_RESERVE_RESOURCES_STC
 *
 * @brief Reserve HW resources related to DMA configuration.
 *
*/
typedef struct{
    /**
     * @brief bitmap of Reserved MG Unit Address Windows indexes
    */
    GT_U32 dmaMG_AddressWindowsReservedBmp;

    /**
     * @brief bitmap of Reserved Output Address Translation table Indexes
    */
    GT_U32  dmaOATU_AddressWindowsReservedBmp;

} CPSS_DXCH_HW_PP_DMA_RESERVE_RESOURCES_STC;
...
* @param[in] numOfWa                  - number of WA needed
*       (number of elements in and additionalInfoBmpArr[]
*       arrays).
...
* @endinternal
*
* @brief   Configures the interrupt coalescing parameters and
...
*
* @note   APPLICABLE DEVICES:      AC5, AC5X, AC5P, Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC3X; Lion2; Bobcat2; Caelum; Aldrin; Bobcat3; Aldrin2; Falcon.
*
* @param[in] devNum                - physical device number
...
/**
* @internal cpssDxChHwPpDmaConfigSet function
* @endinternal
*
* @brief   Function configures device for DMA operations
* like TX/RX packet from/to CPU, FDB Address Update queues e.t.c.
* Each call of this function adds one Address Space Range to DMA space
* This function can be called several times for several not intersecting ranges.
*
*
...
*
* @param[in] devNum                    - device number.
* @param[in] dmaCfgPtr                 - (pointer to) DMA Configuration structure
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on failure.
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_BAD_PTR               - one of the parameters in NULL pointer
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @retval GT_NO_RESOURCE           - no free MG address windows or OATU entries to use
*
*/
GT_STATUS cpssDxChHwPpDmaConfigSet(
    IN GT_U8                             devNum,
    IN CPSS_DXCH_HW_PP_DMA_CONFIGURE_STC *dmaCfgPtr
);

/**
* @internal cpssDxChHwPpDmaReserveResourcesSet function
* @endinternal
*
* @brief   Function Reserves HW Resources for DMA address windows configuration
*     The reserved resources will not be used by cpssDxChHwPpDmaConfigSet and left to
*     other SW (as FW). This function must be called before all cpssDxChHwPpDmaConfigSet calls.
*
* @note   APPLICABLE DEVICES:      AC5;Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @param[in] devNum                    - device number.
* @param[in] dmaReserveResourcesPtr    - (pointer to) DMA
...
*
*/
GT_STATUS cpssDxChHwPpDmaReserveResourcesSet(
    IN GT_U8                                     devNum,
    IN CPSS_DXCH_HW_PP_DMA_RESERVE_RESOURCES_STC *dmaReserveResourcesPtr
);

/**
* @internal cpssDxChHwPpSoftResetSkipParamsAndTrigger function
* @endinternal
*
* @brief   This routine configures skip reset for specific functional use case 
*          and triggers the reset process.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChHwPpSoftResetSkipParamsAndTrigger
(
    IN  GT_U8                               devNum,
