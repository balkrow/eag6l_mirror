...
/* if use register description using shifts: */
   ...
   #define TXQ_SCHED_DEVDEPEND_REGISTER_VAR_UNTIL_SIP_6_30(register_name) TM_REGISTER_VAR(TM_Sched_##register_name)\
                                               TM_REGISTER_VAR(PDQ_SIP_6_30_##register_name)



    #define TXQ_SCHED_DEVDEPEND_REGISTER_VAR(register_name) TXQ_SCHED_DEVDEPEND_REGISTER_VAR_UNTIL_SIP_6_30(register_name)\
                                              TM_REGISTER_VAR(PDQ_SIP_7_##register_name)

    #define TXQ_SCHED_DEVDEPEND_REGISTER_VAR_FROM_SIP_6_30(register_name) TM_REGISTER_VAR(PDQ_SIP_6_30_##register_name)\
                                                TM_REGISTER_VAR(PDQ_SIP_7_##register_name)
	...
	{\
		...
		else	{ 	cpssOsFprintf(CPSS_OS_FILE_STDERR,"%s:%d unknown parameter %s used in assignment.\n",__FILE__,__LINE__,#param_name); cpssOsFatalError(CPSS_OS_FATAL_RESET,"aborting\n");}\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(PDQ_ENV_DEVNUM_NUM(ctl)).schedDbg.schedLogParseInRequests)\
        {\
          schedPrintf("Encode  into register %s param %s value %d (0x%x)\n",#register,#param_name,\
    ...
    {\
        ...
        else if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
            TXQ_SCHED_REGISTER_SET(PDQ_SIP_6_30_##register, param_name, param_value)\
        }\
        else\
        {\
            TXQ_SCHED_REGISTER_SET(PDQ_SIP_7_##register, param_name, param_value)\
        }\
    }\

    /*This macro is for fields started from   SIP_6_30*/
    #define  TXQ_SCHED_DEVDEPEND_FIELD_PARSE_SET_FROM_SIP_6_30(register, param_name, param_value) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
    ...
    /*This macro is for fields discontinued on SIP7*/
    #define  TXQ_SCHED_DEVDEPEND_FIELD_PARSE_SET_UNTIL_SIP_6_30(register, param_name, param_value) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
	...
	{\
		...
		else	{ 	cpssOsFprintf(CPSS_OS_FILE_STDERR,"%s:%d unknown parameter %s used in read request.\n",__FILE__,__LINE__,#param_name); cpssOsFatalError(CPSS_OS_FATAL_RESET,"aborting\n"); } \
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(PDQ_ENV_DEVNUM_NUM(ctl)).schedDbg.schedLogParseOutRequests)\
        {\
          schedPrintf("Decode  from register %s param %s value %d (0x%x) casting %s\n",#register,#param_name,\
           param_value,param_value,#casting);\
        }\
	}
...
#define  TXQ_SCHED_DEVDEPEND_FIELD_PARSE_GET(register, param_name, param_value,casting) \
    {\
        ...
        {\
            TXQ_SCHED_REGISTER_GET(PDQ_SIP_7_##register, param_name, param_value,casting)\
        }\
    }\
...
#define  TXQ_SCHED_DEVDEPEND_FIELD_PARSE_GET_UNTIL_SIP_6_30(register, param_name, param_value,casting) \
{\
    PDQ_ENV_MAGIC_CHECK(ctl)\
    if (PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
    {\
...
#define  TXQ_SCHED_DEVDEPEND_FIELD_PARSE_GET_FROM_SIP_6_30(register, param_name, param_value,casting) \
{\
    PDQ_ENV_MAGIC_CHECK(ctl)\
    if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
    {\
    ...
    {\
        ...
        {\
            TXQ_SCHED_REGISTER_RESET(PDQ_SIP_7_##register)\
        }\
        rc =0;\
    }\

    /*This macro is for fields discontinued on SIP7*/
    #define TXQ_SCHED_REGISTER_DEVDEPEND_RESET_UNTIL_SIP_6_30(register) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
    ...
    #define TXQ_SCHED_REGISTER_DEVDEPEND_RESET_FROM_SIP_6_30(register) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
