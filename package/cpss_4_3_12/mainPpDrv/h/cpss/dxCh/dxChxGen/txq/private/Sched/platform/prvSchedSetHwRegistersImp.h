...
#include <cpss/dxCh/dxChxGen/txq/private/Sched/platform/regFiles/prvShedGenRegsDescription.h>
#include <cpss/dxCh/dxChxGen/txq/private/Sched/platform/regFiles/prvShedIronmanRegsDescription.h>
#include <cpss/dxCh/dxChxGen/txq/private/Sched/platform/regFiles/prvShedAasRegsDescription.h>
...
{\
    ...
    {\
        TXQ_SCHED_WRITE_REGISTER(PDQ_ENV_DEVNUM_NUM(ctl),address, PDQ_SIP_7_##register_name)\
    }\
}\

#define TXQ_SCHED_WRITE_REGISTER_DEVDEPEND_FROM_SIP_6_30(address,register_name) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
...
#define TXQ_SCHED_READ_REGISTER(_devNum,address, register_name) \
    {\
     ...
     {\
        if (rc==0)\
        {\
        schedPrintf("RD  [addr = 0x%08x] REG TILE 0 PDQ %s 0x%08x 0x%08x\n",address.l[0],#register_name, convertReadDummy.l[1],convertReadDummy.l[0]);\
        }\
        else schedPrintf("Read  failed from [addr = 0x%08x] !\n",address.l[0]);\
     } \
    }
...
{\
    ...
    {\
        TXQ_SCHED_READ_REGISTER(PDQ_ENV_DEVNUM_NUM(ctl),address, PDQ_SIP_7_##register_name)\
    }\
}\

#define TXQ_SCHED_READ_REGISTER_DEVDEPEND_FROM_SIP_6_30(address,register_name) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
...
#define TXQ_SCHED_VALIDATE_ELIG_FUNCTION(_elig_fun,_tableSize) \
        do \
        { \
            TXQ_SCHED_NODE_ELIG_FUNC_NUM_GET(_tableSize);\
            if ((_elig_fun >= _tableSize) || (_elig_fun == TM_NODE_DISABLED_FUN))\
...
#define TXQ_SCHED_WRITE_TABLE_REGISTER(_devNum,address, index , register_name)  \
    ...
    {\
        ...
        {\
            TXQ_SCHED_WRITE_TABLE_REGISTER(PDQ_ENV_DEVNUM_NUM(ctl),address, index ,PDQ_SIP_7_##register_name)\
        }\
    }\

    /*This macro is for fields discontinued on SIP7*/

    #define TXQ_SCHED_WRITE_TABLE_REGISTER_DEVDEPEND_UNTIL_SIP_6_30(address, index , register_name) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
    ...
    #define TXQ_SCHED_WRITE_TABLE_REGISTER_DEVDEPEND_FROM_SIP_6_30(address, index , register_name) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
...
{\
    ...
    {\
        if (rc==0)\
        {\
        schedPrintf("RD  [addr = 0x%08x] MEM TILE 0 PDQ %s index %d 0x%08x 0x%08x\n",address.l[0],#register_name,index,\
          convertReadDummy.l[1],convertReadDummy.l[0]);\
        }\
        else schedPrintf("Read  failed from [addr = 0x%08x] !\n",address.l[0]);\
    } \
}\
...
{\
    ...
    {\
        TXQ_SCHED_READ_TABLE_REGISTER(PDQ_ENV_DEVNUM_NUM(ctl),address, index ,PDQ_SIP_7_##register_name)\
    }\
}\

    /*This macro is for fields discontinued on SIP7*/

#define TXQ_SCHED_READ_TABLE_REGISTER_DEVDEPEND_UNTIL_SIP_6_30(address, index , register_name) \
{\
    PDQ_ENV_MAGIC_CHECK(ctl)\
    if (PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
    {\
...
#define TXQ_SCHED_READ_TABLE_REGISTER_DEVDEPEND_FROM_SIP_6_30(address, index , register_name) \
    {\
        PDQ_ENV_MAGIC_CHECK(ctl)\
        if (PRV_CPSS_SIP_7_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_FALSE)\
        {\
