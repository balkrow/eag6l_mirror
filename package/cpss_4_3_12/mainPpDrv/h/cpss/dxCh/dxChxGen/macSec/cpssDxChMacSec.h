...
/*
 * @enum: CPSS_DXCH_MACSEC_POLICY_MODE_ENT
 *
 * @brief: Defines MACSec policy mode: channel security policy mode
 *
 *  APPLICABLE DEVICES: Ironman.
 */
typedef enum
{
    /** Must secure   - Non MACSEC frames not allowed */
    CPSS_DXCH_MACSEC_POLICY_MODE_MUST_SECURE_E,
    /** Should secure - Non MACSEC frames allowed */
    CPSS_DXCH_MACSEC_POLICY_MODE_SHOULD_SECURE_E
} CPSS_DXCH_MACSEC_POLICY_MODE_ENT;
...
typedef struct
{
    ...
    /** @brief Facilitates port reset sequence and allows in single write drop all incoming packets for a port
               true  - force dropping for all packets for a given port
               false - do not force dropping
               Valid only if exceptionCfgEn is true */
    GT_BOOL forceDrop;

    /** @brief Allows passing packets that missed the vPort matching,
    ...
    /** @brief Default vPort id value, ignored if fDefaultVPortValid is set to false
               Valid only if exceptionCfgEn is true */
    GT_U32 defaultVPort;

    /** @brief Channel policy mode configuration
     * (APPLICABLE DEVICES: Ironman)
     */
    CPSS_DXCH_MACSEC_POLICY_MODE_ENT policyMode;
...
typedef struct
{
    /** @brief MAC destination address and EtherType
    ...
    /** @brief Range of MAC destination address used with EtherType
    ...
    /** @brief Range of MAC destination address (no EtherType)
...
typedef struct
{
    ...
    /** @brief This flag controls key generation for packets with SecTAG after MAC SA.
...
typedef struct
{
    ...
    /** @brief Counter increment disable control values
        Each bit disables all the counters of one counter type.
        Counter types:
            - SA counters
            - Interface counters
...
typedef struct
{
    /** @brief Compare Ether-type in packet against EtherType value.
               false - all packets are treated as MACsec (no packets are classified as untagged).
               default is true */
    GT_BOOL compEtype;

    /** @brief true - check V bit to be 0
        default is true */
    GT_BOOL checkV;

    /** @brief true - check if this is a KaY packet (C and E bits)
        default is true */
    GT_BOOL checkKay;

    /** @brief true - check illegal C and E bits combination (C=1 and E=0)
        default is true */
    GT_BOOL checkCe;

    /** @brief true - check illegal SC/ES/SCB bits combinations
        default is true */
    GT_BOOL checkSc;

    /** @brief true - check if SL (Short Length) field value is out of range
        default is true */
    GT_BOOL checkSl;

    /** @brief true - check PN (Packet Number) is non-zero
       default is true */
    GT_BOOL checkPn;
...
typedef struct
{
    ...
    /** @brief Mode for packet sequence number threshold comparison:
...
typedef struct
{
    /** @brief true  - SA is in use, packets classified for it can be transformed
        false - SA not in use, packets classified for it can not be transformed   */
    GT_BOOL saInUse;

    /** @brief The number of bytes (in the range of 0-63) that are authenticated but not encrypted following the SecTAG in the encrypted packet. */
    GT_U8 confidentialityOffset;

    /** @brief true  - enable frame protection,
       false - bypass frame through device */
    GT_BOOL protectFrames;

    /** @brief true  - inserts explicit SCI in the packet,
       false - use implicit SCI (not transferred) */
    GT_BOOL includeSci;

    /** @brief true  - enable ES bit in the generated SecTAG
       false - disable ES bit in the generated SecTAG */
    GT_BOOL useEs;

    /** @brief true  - enable SCB bit in the generated SecTAG
       false - disable SCB bit in the generated SecTAG */
    GT_BOOL useScb;

    /** @brief true  - enable confidentiality protection
       false - disable confidentiality protection */
    GT_BOOL confProtect;

    /** @brief true  - allow data (non-control) packets.
...
typedef struct
{
    ...
    /** @brief The number of bytes (in the range of 0-127) that are authenticated but not encrypted following the SecTAG in the encrypted packet.
       Values 65-127 are reserved in hardware < 4.0 and should not be used there.     */
    GT_U8 confidentialityOffset;

    /** @brief true  - enable replay protection
    ...
    /** @brief true - allow tagged packets.
        false - drop tagged packets. */
    GT_BOOL allowTagged;

    /** @brief true - allow untagged packets.
        false - drop untagged packets. */
    GT_BOOL allowUntagged;

    /** @brief true  - enable validate untagged packets.
    ...
    /** @brief For situations when RxSC is not found or saInUse=false with validation level that allows packet to be sent
    ...
    /** @brief true - ICV is retained (allowed only when retainSecTAG is true). */
    GT_BOOL retainIcv;
...
typedef struct
{
    /** @brief true  - message has length 0
    ...
    /** @brief true  - append the calculated ICV
        false - don't append the calculated ICV */
    GT_BOOL icvAppend;

    /** @brief true  - enable ICV verification
        false - disable ICV verification */
    GT_BOOL icvVerify;

    /** @brief true  - enable confidentiality protection (AES-GCM/CTR operation)
...
typedef struct
{
    /** @brief: Priority value that is used to resolve multiple rule matches.
     ...
     *    Note - Prority based TCAM policy is not supported for Ironman and above.
     */
    GT_U8 rulePriority;

    /** @brief: GT_TRUE : Drop the packet
     *          GT_FALSE: Do not drop the packet
     */
    GT_BOOL drop;

    /** @brief: GT_TRUE : Mark packets matching the rule as control packets.
     *          GT_FALSE: Do not mark the packets
     */
...
typedef struct
{
    ...
    /** @brief Data[0] : MAC Destination Address least significant bytes (3..0)
    ...
    /** @brief Preemption class
     *  Used only for ports, with pre-emption enabled, Ignored for other ports
     *  Only valid in case of portNum!=0 in Mask
     *  CPSS_PORT_PREEMPTION_CLASS_BOTH_E is supported only in case:
     *      - Express channel and Pre-emptive channel are differs by single bit
     *      - Other cases application need to call twice with specific values. */
...
typedef struct
{
    ...
    /** @brief Extended packet numbering: if true then use 64 bits sequence number
    ...
    /** @brief Replay window size: specifies the window size for ingress sequence number checking. Value 0 is enforced for strict ordering.
