...
typedef struct{
    ...
    /** @brief The counters for passed packets.
      * The counter is Read only , and cleared after read.
    */
...
typedef struct{
    ...
    /** @brief The history length for the vector recovery algorithm
      *  NOTE: relevant only when useVectorRecoveryAlgorithm == GT_TRUE (otherwise ignores)
    ...
    /** @brief The maximum difference between <DiscardedPacketsCounter> and
      *  <PassedPacketsCounter> that is allowed. Any larger difference will
      *  trigger the detection of Latent error (LE).
      *  NOTE: used by the daemon.
      *  APPLICABLE RANGES : 1..(1M-1)
     **/
    GT_U32              leDifference;
