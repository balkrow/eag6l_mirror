...
typedef enum{
     ...
     *  a. not relevant to sip7 devices , as Trunk ECMP table is not part of L2 ECMP table.
     *      sip5 : cpssDxChBrgL2EcmpTableSet(...) :
     ...
     *  b. not relevant to sip7 devices , as Trunk ECMP table is not part of L2 ECMP table.
     ...
     *  Expectations from Application for sip7 device:
     *      1. Not to 'modify' entries in 'Trunk ECMP' table
     *         those were 'given' to the CPSS for trunk management.
     *  Expectations from Application for sip5 device:
     *      1. Not to 'modify' entries in 'L2 ECMP' table
     *         those were 'given' to the CPSS for trunk management.
...
typedef enum{
     ...
     *  for single destination packets (trunk to physical port).
     */
 ...
 *      For XOR mode not in use.
 * ipv6FlowArray[1] hold next 8 bits
 *      For XOR mode only 6 LSB in use.
 ...
 * Each MPLS Label 3 to Label 9 is of 20 bits size.
...
typedef enum{
    ...
    /** @brief the 7 MPLS lables can be used instead of all 128 bits of IP DIP and
     *  12 Least Significant bits of IP SIP.
     *  (APPLICABLE DEVICES: AAS)
     */
    CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IP_DIP_SIP_BY_7MPLS_LABELS_E
...
*                                      'Trunk/L2 ECMP' indexes from end of the table towards
...
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_BAD_PARAM             - bad device number , or
*                                    bad trunkId number , or bad member parameters :
*                                    (device & 0xE0) != 0 means that the HW can't support this value,
*                                    since HW has 5 bit for device number
*                                    (port & 0xC0) != 0 means that the HW can't support  this value ,
*                                    since HW has 6 bit for port number
* @retval GT_ALREADY_EXIST         - this member already exists in another trunk.
* @retval GT_FULL                  - trunk already contains maximum supported members
* @retval GT_BAD_STATE             - the trunk-type is not one of: CPSS_TRUNK_TYPE_FREE_E or CPSS_TRUNK_TYPE_REGULAR_E
