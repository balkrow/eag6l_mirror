...
typedef enum{
    ...
    /** @brief When parity error is detected in MLL FIFO, an interrupt
    ...
    /** @brief When parity error is detected in BMA MC data, an
     ...
     *  WA:
     *  set internal bit [10] to 1'b1 (PClk Drift Adjustment Enable).
     *  FE-3616131 (TAI-936)
     */
    PRV_CPSS_DXCH_AC5P_RM_PTP_FREQ_UPDATE_FOR_PCLK_IS_DONE_INACCURATELY_WA_E,

    /** @brief SIP6 support of lossless pools 2 and 3
     *
     *  (ERR-9117246)
     */
    PRV_CPSS_DXCH_FALCON_FOUR_TXQ_POOLS_SUPPORT_E,

     /** @brief Wrong size of the fifo may lead to FDB to be stuck.
     *  WA: Set the FDB answer fifo size to 4.
     *  RM-9972384
     */
    PRV_CPSS_DXCH_FALCON_RM_FDB_ANSWER_FIFO_SIZE_E,

     /** @brief both MAC and EXT registers related to PCH/MCH hold
     *  wrong default values. WA: Align defaults through a
     *  one-time-init. ERR-4364825, (MTIWRAPPER-467)
     */
    PRV_CPSS_DXCH_IM_PTP_PCH_MCH_RM_E,

    /** @brief Wrong action return when there is a hit.
     *  When there is a match on a key (key_x) that is bigger than 5B
     *  (meaning 19B, 33B or 47B) and there is a key (key_y) in a lower bank
     *  index that makes key_x[39:0] = key_y[39:0] than the action
     *  that is returned belongs to key_y instead of key_x.
     *  WA: when inserting new rule or relocating existing rule need to make sure:
     *      that the new or a relocated rule doesn't conflict with any other rule.
     *      conflict means rules have same size, sharing same LSB 5 bytes
     *      and both can use the hash index checked.
     *
     *  (FE-7810754)
     */
    PRV_CPSS_DXCH_FALCON_EM_RETURNS_WRONG_ACTION_E,

    /** @brief CTSU Egress TS Correction field should be enabled.
     *  The MAC can correct CF fields with a configured egress pipe delay (peer delay).
     *  Timestamping at the MAC is more accurate than at the CTSU since it accounts for
     *  delay variation between the CTSU and the MAC.
     *  However, due to this erratum, the default value of "Channel %n Egress TS Correction"
     *  bit#0 at Channel <<%n>> Timestamping Precision Control register is 0x0 = Disable.
     *  WA: Change default value to 0x1 = Enable.
     *
     *  (RM-9690890)
     */
    PRV_CPSS_DXCH_AC5P_RM_CTSU_EGRESS_TS_CORRECTION_FIELD_WA_E,

     /** @brief Aldrin3 PTP_CLK0_IN and PTP_CLK1_IN should be default as input to match the pin name.
     *  WA: Change default value to input.
     *
     *  (ERR-4898956)
     */
    PRV_CPSS_DXCH_AC5P_RM_PTP_CLK0_CLK1_IN_PIN_OUTPUT_MODE_WA_E,
