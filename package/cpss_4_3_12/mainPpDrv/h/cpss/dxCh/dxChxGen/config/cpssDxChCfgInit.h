...
typedef enum{
    ...
    */
    ...
    /** @brief : table type represent the Ip to me table
     *  (APPLICABLE DEVICES : Ironman.)
    */
    CPSS_DXCH_CFG_TABLE_IP_TO_ME_E,
...
typedef enum{
     ...
     *  AC5P-XL Profile Index 23
     * 1. L3 table size (LPM) - 28 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  4 banks
     * 3. No use of EM (Exact Match)  -  0 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 18
     * 1. L3 table size (LPM) - 20 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  4 banks
     * 3. No use of EM (Exact Match)  -  0 banks
     * 4. ARP/TS table - 8 Banks
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_MAX_L3_MIN_L2_NO_EM_E,
    /** @brief This mode :
     *  Falcon Profile Index 0
     *  AC5P-XL Profile Index 14
     * 1. L3 table size (LPM) - 0 banks (only internal dedicated memory)
     * 2. L2 table size (FDB) -  32 banks
     * 3. No use of EM (Exact Match)  - 0 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 22
     * 1. L3 table size (LPM) - 4 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  16 banks
     * 3. No use of EM (Exact Match)  -  0 banks
     * 4. ARP/TS table - 12 Banks
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MAX_L2_NO_EM_E,
    /** @brief This mode :
     *  Falcon Profile Index 2
     *  AC5P-XL Profile Index 16
     * 1. L3 table size (LPM)  - 12 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB)  - 16 banks
     * 3. EM table size (Exact Match) - 4 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 25
     * 1. L3 table size (LPM)  - 8 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB)  - 8 banks
     * 3. EM table size (Exact Match) - 4 banks
     * 4. ARP/TS table - 12 Banks
     */
     ...
     *  AC5P-XL Profile Index 26
     * 1. L3 table size (LPM)  - 12 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  4 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 10
     * 1. L3 table size (LPM)  - 8 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB)  - 4 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - 4 Banks
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_L3_MIN_L2_MAX_EM_E,
    /** @brief This mode :
     *  Falcon Profile Index 8
     *  AC5P-XL Profile Index 22
     * 1. L3 table size (LPM) -  8 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  8 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 21
     * 1. L3 table size (LPM)  - 4 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB)  - 4 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - 8 Banks
     ...
     *  AC5P-XL Profile Index 15
     * 1. L3 table size (LPM) - 16 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 16 banks
     * 3. No use of EM (Exact Match) - 0 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 24
     * 1. L3 table size (LPM) - 12 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  8 banks
     * 3. No use of EM (Exact Match)  -  0 banks
     * 4. ARP/TS table - 12 Banks
     ...
     *  AC5P-XL Profile Index 17
     * 1. L3 table size (LPM) - 8 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 16 banks
     * 3. EM table size (Exact Match) - 8 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 26
     * 1. L3 table size (LPM) - 4 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) -  8 banks
     * 3. No use of EM (Exact Match)  -  8 banks
     * 4. ARP/TS table - 12 Banks
     */
     ...
     *  AC5P-XL Profile Index 32
     * 1. L3 table size (LPM) - 12 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 8 banks
     * 3. EM table size (Exact Match) - 8 banks
     * 4. ARP/TS table - Falcon 8 Banks, AC5P-XL 12 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 29
     * 1. L3 table size (LPM) - 8 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 4 banks
     * 3. EM table size (Exact Match) - 8 banks
     * 4. ARP/TS table - 12 Banks
     ...
     *  AC5P-XL Profile Index 18
     * 1. L3 table size (LPM) - 0 banks (only internal dedicated memory)
     * 2. L2 table size (FDB) - 16 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 17
     * 1. L3 table size (LPM) - 0 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 8 banks
     * 3. EM table size (Exact Match) - 16 banks
     * 4. ARP/TS table - 8 Banks
     */
     ...
     *  AC5P-XL Profile Index 19
     * 1. L3 table size (LPM) - 24 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 8 banks
     * 3. EM table (Exact Match) - 0 banks
     * 4. ARP/TS table - Falcon 4 Banks, AC5P-XL 8 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 18
     * 1. L3 table size (LPM) - 20 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 4 banks
     * 3. EM table size (Exact Match) - 0 banks
     * 4. ARP/TS table - 8 Banks
     */
     ...
     *   AC5P-XL Profile Index 30
     * 1. L3 table size (LPM) - 20 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 8 banks
     * 3. No EM table (Exact Match) - 0 banks
     * 4. ARP/TS table - Falcon 8 Banks, AC5P-XL 12 Banks
     *
     * AC5P,Aldrin3,Aldrin3-XL,Armstrong2S Profile Index 27
     * 1. L3 table size (LPM) - 20 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 4 banks
     * 3. EM table size (Exact Match) - 0 banks
     * 4. ARP/TS table - 12 Banks
    ...
    /** @brief This mode :
     *   AC5P-XL Profile Index 13
     *   Aldrin3-XL: Same SBM configuration as AC5P-XL
     * 1. L3 table size (LPM) - 0 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 4 banks
     * 3. EM table size (Exact Match) - 32 banks
     * 4. ARP/TS table - 4 Banks
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_AC5P_32K_L2_256K_EM_64K_ARP_E,

    /** @brief This mode :
     *   Falcon Profile Index 6
     * 1. L3 table size (LPM) - 20 banks (+ internal dedicated memory)
     * 2. L2 table size (FDB) - 8 banks
     * 3. EM table size (Exact Match) - 4 banks
     * 4. ARP/TS table - 4 Banks
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_FALCON_64K_L2_32K_EM_96K_ARP_E
...
/**
 *  @brief define the max number of units that the device may support.
 *  it is needed to allow to understand the size of memory needed from the shared-tables
 *  for those units.
 */
#define CPSS_DXCH_PP_CONFIG_SHARED_MAX_NUM_EM_UNITS_E   4

/**
* @enum CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_ENT
 *
 * @brief Defines the auto learn modes of the EM unit
*/
typedef enum{
    /** @brief EM auto learn by client 0 of the unit.
     */
    CPSS_DXCH_PP_CONFIG_EM_AUTO_LEARN_MODE_CLIENT0_E,
    /** @brief EM auto learn by client 1 of the unit.
     */
    CPSS_DXCH_PP_CONFIG_EM_AUTO_LEARN_MODE_CLIENT1_E,

    /** @brief EM auto learn is not used by any client of the unit.
     */
    CPSS_DXCH_PP_CONFIG_EM_AUTO_LEARN_MODE__NOT_USED__E

}CPSS_DXCH_PP_CONFIG_EM_AUTO_LEARN_MODE_ENT;

/**
* @enum CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_ENT
 *
 * @brief Defines the client that use EM table
*/
typedef enum{
    /** @brief this EM client is not connected.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE__NOT_CONNECTED__E = 0,

    /** @brief TTI first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_TTI_FIRST_E       ,
    /** @brief TTI second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_TTI_SECOND_E      ,
    /** @brief IPCL0 first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPCL0_FIRST_E     ,
    /** @brief IPCL0 second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPCL0_SECOND_E    ,
    /** @brief IPCL1 first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPCL1_FIRST_E     ,
    /** @brief IPCL1 second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPCL1_SECOND_E    ,
    /** @brief MPCL first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_MPCL_FIRST_E      ,
    /** @brief MPCL second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_MPCL_SECOND_E     ,
    /** @brief EPCL first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_EPCL_FIRST_E      ,
    /** @brief EPCL second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_EPCL_SECOND_E     ,
    /** @brief IPE first is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPE_FIRST_E       ,
    /** @brief IPE second is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_IPE_SECOND_E      ,
    /** @brief TTI-ILM2 is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_TTI_ILM2_E        ,
    /** @brief TTI-ILM3 is the EM client.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_TTI_ILM3_E

}CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_ENT;

/**
 * @struct CPSS_DXCH_PP_CONFIG_EM_CLIENT_INFO_STC
 *
 * @brief This struct hold info about EM client : client type and table size
 *  (APPLICABLE DEVICES: AAS.)
 *
 */
typedef struct
{
    /**@brief the type of needed client.
     *  can be CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE__NOT_CONNECTED__E if the
     *  second table in the unit is connected.
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_TYPE_ENT  clientType;

    /**@brief number of entries needed by this EM client for the table.
     *  NOTE: value can't be 0 (unless clientType = 'not connected')
     */
    GT_U32      numEntriesForClient;
}CPSS_DXCH_PP_CONFIG_EM_CLIENT_INFO_STC;

/**
 * @struct CPSS_DXCH_PP_CONFIG_SHARED_EM_UNIT_STC
 *
 * @brief This struct hold the EM unit table sizes needed by the application.
 *  the CPSS will check if the table sizes can be set into the device
 *  (APPLICABLE DEVICES: AAS.)
 *
 */
typedef struct
{
    /**@brief indication if the EM client 1 need to use same table as EM client 0
     *      so only single table serves the 2 EM clients of the unit.
     *  NOTEs :
     *      1. when GT_TRUE : numEntriesForClient of the second table is ignored
     */
    GT_BOOL     singleTableUsedForThe2Clients;

    /**@brief indication if the EM 'auto learn' mode
     *  NOTEs:
     *      1. if the autoLearnMode != 'not used' the table of the relevant client
     *              will have : Table type : Non-shared-table between tiles
     *         else if singleTableUsed == GT_TRUE
     *              will have : Table type : Non-shared-table between tiles
     *         else
     *              will have : Table type : Shared-table between tiles
     */
    CPSS_DXCH_PP_CONFIG_EM_AUTO_LEARN_MODE_ENT     autoLearnMode;

    /**@brief the array with info about the 2 clients of this EM unit
     *      each client with it's own
     */
    CPSS_DXCH_PP_CONFIG_EM_CLIENT_INFO_STC   clientInfoArr[2];

}CPSS_DXCH_PP_CONFIG_SHARED_EM_UNIT_STC;

/**
 * @struct CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC
 *
 * @brief This struct hold the tables that are in the 'shared table' ,
 *  with the needed size by the application.
 *  the CPSS will check if the table sizes can be set into the device
 *  (APPLICABLE DEVICES: AAS.)
 *
 */
typedef struct
{
    /** @brief number of prefixes needed by LPM table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : shared-table between tiles
     */
    GT_U32      numPrefixesForLPM;
    /** @brief number of entries needed by FDB table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numEntriesForFDB;

    /** @brief number of EM (exact match) units.
     *  This value explain the valid indexes in emUnits[]
     * APPLICABLE VALUES: 0,1,2,3.
     */
    GT_U32      numEmUnitsNeeded;

    /** @brief info about the needed EM Units and it's Tables.
     *  NOTE: a dedicated EM API will allow bind the EM unit to it's clients (TTI,PCL,IPE...)
     */
    CPSS_DXCH_PP_CONFIG_SHARED_EM_UNIT_STC   emUnits[CPSS_DXCH_PP_CONFIG_SHARED_MAX_NUM_EM_UNITS_E];

    /** @brief number of entries needed by ARP table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     *  NOTE: separated from TS (new split in AAS)
     */
    GT_U32      numEntriesForARP;

    /** @brief number of entries needed by TS (Tunnel Start) table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  The TS table hold several types of entries , the number refers to
     *  'standard' TS (non-Ipv6) entries.
     *  Table type : shared-table between tiles
     */
    GT_U32      numEntriesForTS;

    /** @brief number of entries needed by HF0 (Header Fields) table for MPLS/SRv6.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : shared-table between tiles
     */
    GT_U32      numEntriesForHF0;
    /** @brief number of entries needed by HF1 (Header Fields) table for MPLS/SRv6.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : shared-table between tiles
     */
    GT_U32      numEntriesForHF1;

    /** @brief number of entries needed by the PLR IpFix/Billing table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numEntriesForIpfixOrBilling;

    /** @brief number of entries needed by the PLR Metering table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numEntriesForMetering;

    /** @brief number of CNC Blocks needed.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numOfCncBlocks;

    /** @brief number of entries needed by the IPE state 0 table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numEntriesForIpeState0;

    /** @brief number of entries needed by the IPE state 0 table.
     *  the CPSS will give the nearest value that is >= of the requested value.
     *  Table type : Non-shared-table between tiles
     */
    GT_U32      numEntriesForIpeState1;

}CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC;
...
typedef struct{
     ...
     * (APPLICABLE DEVICES: Falcon; AC5P.)
     * NOTE: Other Sip4,Sip5,Sip6 devices don't have 'shared tables' , so this member is ignored for them.
     *  sip7 devices that have 'shared tables' ignore this sharedTableMode member
     *  and use the new sharedTablesSizeInfo member with much more info that needed for them.
     */
    CPSS_DXCH_CFG_SHARED_TABLE_MODE_ENT sharedTableMode;

    /** @brief setting shared tables sizes, to meet system needs
     * (APPLICABLE DEVICES: AAS.)
     */
    CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC  sharedTablesSizeInfo;
