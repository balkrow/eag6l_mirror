...
#include <cpss/dxCh/dxChxGen/ptp/private/prvCpssDxChPtpCommonTypes.h>
#include <cpss/dxCh/dxChxGen/ptpManager/cpssDxChPtpManagerCommonTypes.h>


#define DUMMY_NON_SIP7_FINE_TUNE_MAC    \
        /* sip7 : Max number of entries for the EM tables */  \
        {                                                     \
            /* NOTE : !!! if any of those set as 'GT_NA' the 'emNum' should be used instead of it !!! */ \
            0, /* ttiNum;    for each of first/second profile */  \
            0,  /* ipcl0Num;  for each of first/second profile */ \
            0,  /* ipcl1Num;  for each of first/second profile */ \
            0,  /* mpclNum;   for each of first/second profile */ \
            0,  /* epclNum;   for each of first/second profile */ \
            0,  /* ipeNum;    for each of first/second profile */ \
            0   /* ttiIlmNum; for each of first/second profile */ \
        },/*sip7_em_clients;*/                                    \
                                                                  \
        /* sip7 : Max number of entries for the HF0,1 tables */   \
        0,  /* sip7_hf0;*/                                        \
        0,  /* sip7_hf1;*/                                        \
        /* sip7 : policer Max number of entries for counters/ipfix */ \
            /*NOTE: the meters are in policersNum                     \
        */                                                            \
        0, /* sip7_plr_counters;*/                                    \
        /* sip7 : Max number of entries for the IPE0,1 tables */      \
        0, /* sip7_ipe0;*/                                            \
        0  /* sip7_ipe1;*/                                            \
...
typedef struct{
    ...
    struct{
        ...
        struct{
            /* NOTE : !!! if any of those set as 'GT_NA' the 'emNum' should be used instead of it !!! */
            GT_U32  ttiNum;   /* for each of first/second profile */
            GT_U32  ipcl0Num; /* for each of first/second profile */
            GT_U32  ipcl1Num; /* for each of first/second profile */
            GT_U32  mpclNum;  /* for each of first/second profile */
            GT_U32  epclNum;  /* for each of first/second profile */
            GT_U32  ipeNum;   /* for each of first/second profile */
            GT_U32  ttiIlmNum;/* for each of first/second profile */
        }sip7_em_clients;
...
typedef enum{
    ...
    /** serdes number */
    PRV_CPSS_DXCH_PORT_TYPE_SERDES_E,
...
typedef struct{
    ...
    /** Special 'xPortDmaNum' DMA local channels */
    ...
    /** local Loopback DMA channel. For SIP7:AAS=20, for others: =0 not supported on this DP */
    GT_U32 loopbackPortDmaNum;
 ...
 *
 * @brief A Structure with info for each cpu port (mg/GDMA)
 * Notes:
...
typedef struct{
    ...
    /** info about the GDMA(s) that serve this CPU port
      * sip7 and above
      * see in : PRV_CPSS_DXCH_PP_MAC(devNum)->gdmaInfo.cpuPortsArr[]
    */
...
typedef struct{
    ...
    /** @brief : the MG unit index (the MG to serve the SDMA operation)
    */
    GT_U32 mgUnitId;
...
typedef struct{
    ...
    struct{
         ...
         *  Relevant to sip7 devices
         *  set by prvCpssDxChSharedMemoryAasConnectClientsToMemory(...)
         */
        GT_U32  sip7_cnc_2_3_usedBlocksBmp;
        /** @brief : the sip7_cnc_0_1_usedBlocksBmp,sip7_cnc_0_1_usedBlocksBmp
         *  hold the bmp of blocks in CNC0,1 and CNC2,3
         *  since in sip7 shared tables , the CNC blocks are part of shared tables in the SHM unit
         *  and more than that the IPE 0,1 and muxed on the CNC_0_1 memory
         ***********************
         *  this array will mapping between 'application CNC block' (index in the array)
         *  to the 'actual HW CNC block' (value in the array)
         *  example :  if the DB hold value of : sip7_cnc_mapToActualBlocksUsed[3] = 36
         *          it means that  'application CNC block [3]' is actually block 36 in HW
         *          (block 36 --> CNC2 , local block 4 )
         *
         *  NOTE : the parameter blockNum in the CNC APIs is the 'application CNC block'
         */
        GT_U32  sip7_cnc_mapToActualBlocksUsed[64];
    }cnc;
    ...
    struct{
        ...
        /* @brief : if != 0 , hold the number of entries in 'ip2me' table */
        GT_U32 TTI_ip2me_numEntries;
    }tti;
    ...
    struct{
        ...
        GT_U32      trunkId;
        GT_U32      ePort;
        GT_U32      vPort;  /* sip7 : vPort support */
        GT_U32      vid;
        GT_U32      vidx;
        GT_U32      eVidx;
        GT_U32      hwDevNum;
    ...
    struct{
        ...
        GT_U32      numOfTcamProfiles;
        GT_U32      numOfTcamMuxTableLines;
        GT_U32      numOfSubKeys1_3;
    }tcam;
    ...
    struct{/* next are used when not 0 corresponding to the same resource by the
        ...
        GT_U32      vPort;/* sip7 : vPort support , incase the device support less than 1<<flexFieldNumBitsSupport.vPort */
    ...
    struct{
        ...
        GT_U32 numEntriesTrunkEcmp; /* number of entries in Trunk-Ecmp , new in SIP7 */
        ...
        GT_U32 numEntriesIpclConfig;/* number of entries in the IPCL config table (IPCL)*/
        GT_U32 numEntriesEpclConfig;/* number of entries in the EPCL config table (EPCL) */
    ...
    struct{
     ...
     struct{
        GT_U32  numOfPpg;/* num of PPGs per PPA unit */
        GT_U32  numOfPpn;/* num of PPNs per PPG unit */
    }ipePpaInfo;
    ...
    /* info about the SIP7 actual size for clients in the shared memory
       how much entries per client given by
       prvCpssDxChSharedMemoryAasConnectClientsToMemory(...)
    */
    struct{
        /* the application called cpssDxChCfgPpLogicalInit(...) with info in
           CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC , asking for table sizes.
           the actual size that given by prvCpssDxChSharedMemoryAasConnectClientsToMemory()
           may be more than asked , as the function 'rounded up' to allow to support the needed value.

           NOTE: the values asked by the application need to be compared to the
            fine tuning , and not allow to be more , before 'round up' to allow to support the needed value.
            meaning that 'actual' can be more than the fine tuning value.(but not for hashed based tables : FDB/EM)

           so save the 'actual values' , if needed for later use.
        */
        CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC  actualSizes;
    }sip7_sharedMemoryClientsActualSize;
    ...
    struct{
        ...
        GT_FLOAT64  fracNanoSeconds; /* range of fractional nano seconds */
        PRV_CPSS_DXCH_PTP_PLL_CONFIG_STC  pllConfig[CPSS_DXCH_PTP_TAI_CLOCK_FREQ_LAST_E]; /* PLL configuration per frequency */
        GT_U32      *frameLatencyArr;
        GT_U32      *frameLatencyFracArr;
        CPSS_DXCH_PTP_MANAGER_DB_DELAYS_STC      *delayPortArr;
        GT_U32                                    delayPortArrNumElements;
        CPSS_DXCH_PTP_MANAGER_DB_DELAYS_STC      *delayCpuPortArr;
        GT_U32                                    delayCpuPortArrNumElements;
        CPSS_DXCH_PTP_MANAGER_DB_DELAYS_STC      *delay100GPortArr;
        GT_U32                                    delay100GPortArrNumElements;
        CPSS_DXCH_PTP_MANAGER_DB_DELAYS_STC      *delayPhyPortArr;
        GT_U32                                    delayPhyPortArrNumElements;
        CPSS_DXCH_PTP_MANAGER_PTP_PHY_DELAY_VALUES_STC  phyDelayInfo; /* Ironman-M internal PHY link delay */

    }ptpInfo;

    /* info about egf */
    struct{
        GT_U32  numBitsPerPort;/* number of registers for : PRV_DXCH_REG1_UNIT_PREQ_MAC(devNum).portMapping[] */
        GT_U32  shtRegisterOffset;/* the SHT registers starts at this offset from start of the SHT unit
                                    as the tables comes before the registers */
    }egfInfo;
...
/******************************/
/** MACROS for VPORT (start) **/
/******************************/
/* macro to check if portNum is in the vPort range or not
    1. if the device is not sip7 --> it return 'not vport'
    2. else if 'useVPortMode' is false   --> it return 'not vport'
    3. else if portNum < firstVPortToUse --> it return 'not vport'
    4. else --> it return 'YES it is vport'
            NOTE: still need to check that it is not more than 'max vPort' :
                PRV_CPSS_DXCH_VPORT_FIELD_CHECK_MAC(...) or
                PRV_CPSS_DXCH_VPORT_AS_KEY_CHECK_MAC(...)


    return value:
    0 - _portNum is not a vPort
    1 - _portNum is     a vPort (but could be 'out of range'/'bad param' like value 0xFFFFFFFF)
*/
#define PRV_CPSS_DXCH_IS_VPORT(_devNum,_portNum)                               \
    ((PRV_CPSS_DXCH_PP_MAC(_devNum)->vportInfo.useVPortMode &&                 \
     (_portNum) >= PRV_CPSS_DXCH_PP_MAC(_devNum)->vportInfo.firstVPortToUse) ? 1 : 0)

/* macro to get maxValue of vPort for fields (according to num of bits of the 'vPort feild')*/
#define PRV_CPSS_DXCH_PP_HW_MAX_VALUE_OF_VPORT_NUM_MAC(_devNum)    \
    (BIT_MASK_MAC(PRV_CPSS_DXCH_PP_HW_INFO_FLEX_FIELD_NUM_BITS_SUPPORTED_MAC(_devNum).vPort))

/* macro to get maxValue of vPort for index to 'per vport' tables
    maybe less than PRV_CPSS_DXCH_PP_HW_MAX_VALUE_OF_VPORT_NUM_MAC(...) */
#define PRV_CPSS_DXCH_MAX_VPORT_NUMBER_MAC(_devNum) \
    ____MAX_PORT_MAC(_devNum,vPort)


#define __PRV_CPSS_DXCH_VPORT_CHECK_MAC(_devNum,_portNum,_error)             \
    if ((_portNum) > PRV_CPSS_DXCH_PP_HW_MAX_VALUE_OF_VPORT_NUM_MAC(_devNum))\
    {                                                                        \
        CPSS_LOG_ERROR_AND_RETURN_MAC(_error, "[%s]=[%d] > max [%d]",        \
            #_portNum,                                                       \
            _portNum,PRV_CPSS_DXCH_MAX_PORT_NUMBER_MAC(_devNum));            \
    }

/* check that the vPort number as field is valid return GT_OUT_OF_RANGE on error */
#define PRV_CPSS_DXCH_VPORT_FIELD_CHECK_MAC(_devNum,_portNum)          \
    __PRV_CPSS_DXCH_VPORT_CHECK_MAC(_devNum,_portNum,GT_OUT_OF_RANGE)

/* check that the vPort number as key is valid return GT_BAD_PARAM on error */
#define PRV_CPSS_DXCH_VPORT_AS_KEY_CHECK_MAC(_devNum,_portNum)         \
    __PRV_CPSS_DXCH_VPORT_CHECK_MAC(_devNum,_portNum,GT_BAD_PARAM)

/******************************/
/** MACROS for VPORT  (end)  **/
/******************************/
