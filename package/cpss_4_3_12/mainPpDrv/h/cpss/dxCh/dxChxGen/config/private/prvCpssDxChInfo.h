...
#include <cpss/dxCh/dxChxGen/ptpManager/private/prvCpssDxChPtpManager.h>
...
/* CPSS definitions for TAM Info*/
#include <cpss/dxCh/dxChxGen/tam/private/prvCpssDxChTamInfo.h>

/* get CPSS internal definitions for GDMA */
...
/* maximal amount of data paths in IRONMAN devices */
/* needed for PCA arbiter shadow                   */
#define PRV_CPSS_MAX_SIP6_30_DP_CNS 1
/* maximal amount of arbiter slots in IRONMAN devices */
#define PRV_CPSS_MAX_SIP6_30_PCA_ARBITER_SLOTS_CNS 320

/* maximal amount of Datapaths for 2-tiles AAS */
#define PRV_CPSS_MAX_SIP7_DP_CNS 8
/* maximal amount of DMA Channels per Datapath for AAS */
#define PRV_CPSS_MAX_SIP7_CHANNELS_PER_DP_CNS 22
...
/* check if device is IronMan */
#define PRV_CPSS_DXCH_IRONMAN_CHECK_MAC(_devNum)                                            \
    (                                                                                       \
         (PRV_CPSS_PP_MAC(_devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)             \
    )

/* check if device is AAS */
#define PRV_CPSS_DXCH_AAS_CHECK_MAC(_devNum)                                            \
    (                                                                                   \
         (PRV_CPSS_PP_MAC(_devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)             \
    )
...
#define PRV_CPSS_DXCH_PM_IS_IN_DEBUG_STATE_CHECK_AND_RETURN_MAC(_devNum,_portNum,_portIsInDebugState)                                                                             \
         if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[_devNum].pmPortFuncPtrsStc).cpssPmIsInDebugStateFunc != NULL)                                                \
         {                                                                                                                                                                        \
             if(GT_OK != PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[_devNum].pmPortFuncPtrsStc).cpssPmIsInDebugStateFunc(_devNum, _portNum,  _portIsInDebugState))   \
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);                                                                                                                                                  \
             if (*_portIsInDebugState == GT_FALSE)                                                                                                                                \
             {                                                                                                                                                                    \
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);                                                                                                   \
             }                                                                                                                                                                    \
         }                                                                                                                                                                        \
...
typedef struct{
     ...
     *  (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.)
     *  in SIP7 devices 'trunk ECMP table' is not part of the L2 ECMP table
     */
     ...
     *  NOTE: this value is set by 'fine tunning' mechanism.
     *  (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.)
     *  NOT relevant to SIP7 devices that have dedicated 'trunk ECMP table'
     */
     ...
     *  (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.)
     *  in SIP7 devices the used index is the last index in the TRUNK ECMP table
    */
...
typedef struct{
    ...
    GT_U64 mibMtiShadow[PRV_CPSS_MTI_MIB_COUNTERS_ENTRY_SIZE_CNS];
...
/**
* @struct PRV_CPSS_DXCH_OAM_UPMEP_WA_PORT_STC
 *
 * @brief This structure is per physical port. It contains:
 *        1.    The previous tail-drop profile to restore it when the port’s tail-drop profile binding will use a deticated profile with 0 buffer assign to it.
 *        2.    If the port is configured (by the user) as an oam_upmep_egress port for the WA.
 *
 * (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman)
*/
typedef struct{
    /** The previous tail-drop profile to maintain. */
    CPSS_PORT_TX_DROP_PROFILE_SET_ENT prevTailDropProfile;

    /** Indicates if the port is configured to be an egress oam-up-mep port */
    GT_BOOL isOamUpMepEgressPort;

    /** Indicates if the port is currently under the OAM Up-MEP WA: that means the port is in a link DOWN state, packets are passing the EGF and dropped by the tail-drop profile assigned */
    GT_BOOL isWaInProgress;
}PRV_CPSS_DXCH_OAM_UPMEP_WA_PORT_STC;


/**
* @struct PRV_CPSS_DXCH_OAM_UPMEP_WA_GLOBAL_STC
 *
 * @brief This structure contains global information about:
 *        1.  Whether the OAM Up-mep WA is enabled
 *        2.  The designated and reserved tail_drop profile for the WA. When allowed, it will be allocated for WA usage only.
 *
 * (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman)
*/
typedef struct{
    /** Indicates that the OAM-Up-MEP WA was initialized. This is only used for the init stage to avoid any overwrite of the reserved tail-drop profile */
    GT_BOOL oamUpMepWaEnable;

    /** The global dedicated tail-drop profile for the OAM-Up-MEP WA. When used, it cannot be assigned to any other resources */
    CPSS_PORT_TX_DROP_PROFILE_SET_ENT oamUpMepWaReservedTailDropProfile;
}PRV_CPSS_DXCH_OAM_UPMEP_WA_GLOBAL_STC;
...
typedef struct
{
    ...
    GT_U16  serdesUsage;
    /** Preemption Mode Activated */
    GT_U8  preemptiveModeActivated;
    /** GT_TRUE - Preemptive, GT_FALSE - Regular or Express */
    GT_U8  isPreemptive;
} PRV_CPSS_DXCH_SIP6_30_DP_CHANNEL_INFO_STC;

/**
* @struct PRV_CPSS_DXCH_SIP6_30_DP_OPTIONS_ENT
 *
 * @brief A enum of option flags in bitmap per datapath for Ironman and above devices
*/
typedef enum
{
    /** 1G OUSGMII(8x1G) Preemprion channel pairs Arbiter configured */
    /** Express - 2.5G, preemptive - 1G                              */
    PRV_CPSS_DXCH_SIP6_30_DP_8X1G_LOW_LATENCY_EXP_2_5G_E = 1
} PRV_CPSS_DXCH_SIP6_30_DP_OPTIONS_ENT;

/**
* @struct PRV_CPSS_DXCH_SIP6_30_DP_INFO_STC
 *
 * @brief A structure to hold DB per datapath for Ironman and above devices
*/
typedef struct
{
    /* bitmap of flags per DP */
    GT_U32  optionFlagsBmp;
...
/**
* @struct PRV_CPSS_DXCH_SIP7_DP_INFO_STC
 *
 * @brief A structure to hold DB per datapath for AAS and above devices
*/
typedef struct
{
    /** GT_FALSE regular CPSS behavior, GT_TRUE - PUNKT algorithms */
    GT_BOOL usePunktAlgorithm;
    /** bitmap of used LL resources */
    GT_U32  sffUsedLlResourcesBitmap[2];
    /** base of LL resource range per channel */
    GT_U8   sffChannelResorcesBaseArr[PRV_CPSS_MAX_SIP7_CHANNELS_PER_DP_CNS];
    /** size of LL resource range per channel */
    GT_U8   sffChannelResorcesAmountArr[PRV_CPSS_MAX_SIP7_CHANNELS_PER_DP_CNS];
} PRV_CPSS_DXCH_SIP7_DP_INFO_STC;

typedef PRV_CPSS_DXCH_SIP7_DP_INFO_STC *PRV_CPSS_DXCH_SIP7_DP_INFO_STC_PTR;
...
typedef struct{
  ...
  *      physicalPortToAnode - mapping between physical port number  to scheduler A nodes.(APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman)
   ...
   *   (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman)
   */
    PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC pdsProfiles[CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES];
   ...
   */
    PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC lengthAdjustProfiles[CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES];

        /** @brief Shadow of txq length adjust  profile parameters
   *   (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman)
   */
    PRV_CPSS_DXCH_SIP6_TXQ_QUEUE_PROFILE_LENGTH_ADJUST_STC  lengthAdjustDb[CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES];


    PRV_CPSS_DXCH_TXQ_DEVICE_CONFIGURATION          txQDeviceConfig;
    ...
    */
    ...
    /* Info per port about the OAM UP-MEP workaround */
    PRV_CPSS_DXCH_OAM_UPMEP_WA_PORT_STC oamUpMepPortWaPtr[PRV_CPSS_MAX_PP_PORTS_NUM_CNS];

    /* Global info about the OAM UP-MEP workaround */
    PRV_CPSS_DXCH_OAM_UPMEP_WA_GLOBAL_STC oamUpMepGlobalWa;
    ...
    /** Shadow of PTP Delay Values per port speed/interface/fec.
       (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman) */
    PRV_CPSS_DXCH_PTP_MANAGER_PTP_DELAY_VALUES_STC *ptpPortSpeedDelayValArrPtr;

     /** Shadow of PTP timestamping operation mode per port.
       (APPLICABLE DEVICES: AC5P;AC5X; Harrier; Ironman) */
    PRV_CPSS_DXCH_PTP_MANAGER_PTP_TIMESTAMPING_OPERATION_MODE_STC *ptpPortTsOperationModeArrPtr;

    /* array of pointers to SIP6_30 info per datapath */
    PRV_CPSS_DXCH_SIP6_30_DP_INFO_STC_PTR sip6_30_dpInfoPtr[PRV_CPSS_MAX_SIP6_30_DP_CNS];

    /* array of pointers to SIP7 info per datapath */
    PRV_CPSS_DXCH_SIP7_DP_INFO_STC_PTR sip7_dpInfoPtr[PRV_CPSS_MAX_SIP7_DP_CNS];

    PRV_CPSS_DXCH_TIME_INFO_STC fecSerTimeSec[PRV_CPSS_MAX_MAC_PORTS_NUM_CNS];

    /* indication that the device not supports USX on any port (relevant to sip6_10 and above) */
    GT_BOOL notSupportUsxPorts;

    /* indication that MAC sec not implemented yet */
    GT_BOOL notImplementedMacSec;

    /* Bobcat3 give DP Pizza slots to 107G as for given Bandwidth */
    GT_U32 bc3PizzaSlots107gBandwidthMbPerSec;
...
#define PRV_CPSS_DXCH_PHA_THREAD_ID_CHECK_MAC(_devNum,_phaThreadId)   \
    ...
    {                                                                 \
        ...
        "wrong phaThreadId [%d], should be in range 1..%d", _phaThreadId,PRV_CPSS_DXCH_PHA_THREAD_ID_MAX_MAC); \
    }
...
/**
* @struct PRV_CPSS_DXCH_VPORT_INFO_STC
 *
 * @brief A structure to hold info needed in order to work in 'vPort mode'
 *
 *  (APPLICABLE DEVICES: AAS)
*/
typedef struct
{
    /**@brief : Enable/Disable the work in 'vPort mode' that allow larger tunnels scale ,
     *          with enhanced tunnel flexibilities and hierarchical ECMP.
     *
     *      value 0 - means that the feature not used
     *              this is the value for non-applicable devices , or for
     *              applicable device that not enabled the use of the feature.
     *      value 1 - means that the feature is  used
     *              applicable device that enabled the use of the feature.
     *
     *      NOTE: this value we get from the application during 'phase 1 init'
    */
    GT_U32 useVPortMode;

    /**@brief : The minimal 'port' number that is considered 'vPort'.
     *      port numbers that are less than this number considered 'non-vPorts'
     *      (hence 'physical ports') as in this system there are no 'ePorts'
     *
     *      NOTE: this value we get from the application during 'phase 1 init' as
     *          'maxNumOfPhyPortsToUse'
    */
    GT_U32 firstVPortToUse;

    struct{
        /**@brief : The minimal vPort number that is subject to the Learn Priority feature.
         *      APPLICABLE RANGE : maxNumOfPhyPortsToUse .. ('num of vPorts' - 1)
         *      NOTE: this value we get from the application during 'phase 1 init'
        */
        GT_U32 minimalVPortNumber;

        /**@brief :The number of vPorts that are subject to the Learn Priority feature.
          * APPLICABLE RANGE : 0.. ('num of vPort profiles' - maxNumOfPhyPortsToUse - 1)
          *      NOTE: this value we get from the application during 'phase 1 init'
          *
          *     This value used in API : cpssDxChBrgFdbLearnPrioritySet/cpssDxChBrgFdbLearnPriorityGet
          *     if useVPortMode == 0 then
          *         index the table with 'portNum' (regular logic of the API)
          *     else if 'portNum' is 0..(firstVPortToUse-1) then
          *         index the table with 'portNum' (this is non-vports range)
          *     else if 'portNum' >= minimalVPortNumber && 'portNum' < (minimalVPortNumber + numberOfVPorts) then
          *         index the table with 'portNum'-minimalVPortNumber+tableBaseIndex (this is vports range)
          *     else
          *         return GT_BAD_PARAM
          *
        */
        GT_U32 numberOfVPorts;

        /**@brief : The Learn Priority table base index for vPorts.
         *      NOTE: this value we get from the application during 'phase 1 init' as
         *          'maxNumOfPhyPortsToUse'
        */
        GT_U32 tableBaseIndex;

    }fdbLearnPriorityInfo;

}PRV_CPSS_DXCH_VPORT_INFO_STC;
...
typedef struct{
    ...
    /** @brief info needed by the TAM feature
     *  (APPLICABLE DEVICES: Falcon; Aldrin2
     */
    PRV_CPSS_DXCH_TAM_INFO_STC    tamInfo;

    /** @brief vPort info
     *  (APPLICABLE DEVICES: AAS)
     */
    PRV_CPSS_DXCH_VPORT_INFO_STC    vportInfo;

    /** @brief GDMA info
     *  (APPLICABLE DEVICES: AAS)
     */
    PRV_CPSS_DXCH_GDMA_INFO_STC    gdmaInfo;

    /** @brief ADDRESS_DECODER info
     *  (APPLICABLE DEVICES: AAS)
     */
    struct {
        /** @brief list of base addresses units for the 'ADDRESS DECODER' units
         *  (APPLICABLE DEVICES: AAS)
         *  if NULL ignored
         */
        const GT_U32        *addressDecoderArrPtr;
        /** @brief bmp of units in addressDecoderArrPtr
         *  (APPLICABLE DEVICES: AAS)
         */
        GT_U32              addressDecoderUnitsBmp;
        ...
        */
        GT_U32              fake_dmaBaseHigh;
        /* @brief : the window index within the ADDR_DECODER that need to match
                PCIe transactions that the GDMA generate.
        */
        GT_U32              CNM_WINDOW_INDEX_PCIe;

        /* @brief : the BMP of ADDR_DECODER units that need to be set with PCIe to
         *      DRAM mapping.
        */
        GT_U32              cnmAddrDecoderUnitsBmp;

        /* @brief : indication that need to set the ADDR_DECODER units.
        */
        GT_U32              addrDecoderNeeded;

    }addressDecoderInfo;

    struct{
        /* @brief : indication that need to set the oATU mapping.
        */
        GT_U32  oAtuNeeded;
        /* @brief : the available windows in the oATU for the CPSS to use.
        */
        GT_U32  oAtuWindowsBmp;
    }oAtuInfo;
...
/* macro to validate the value of RX SDMA queue parameter (DxCh format)
    queue - queue number

*/
#define PRV_CPSS_CHECK_SDMA_RX_Q_MAC(devNum,queue)                         \
if((queue) >= NUM_OF_SDMA_QUEUES || ((queue) >= PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfRxQueues)) \
{                                                                               \
    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);              \
}

/* macro to validate the value of TX SDMA queue parameter (DxCh format)
    queue - queue number

*/
#define PRV_CPSS_CHECK_SDMA_TX_Q_MAC(devNum,queue)                         \
if((queue) >= NUM_OF_SDMA_QUEUES || ((queue) >= PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfTxQueues)) \
{                                                                               \
    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);              \
}
...
/* check if the device supports GDMA */
#define PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(_devNum) PRV_CPSS_DXCH_PP_MAC(_devNum)->gdmaInfo.supported

/* check TSN support */
#define PRV_CPSS_DXCH_TSN_SUPPORTED_CHECK_MAC(_devNum)    \
    if(PRV_CPSS_DXCH_PP_MAC(_devNum)->fineTuning.featureInfo.tsnSupported == GT_FALSE)     \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, "The device not supports TSN features")
