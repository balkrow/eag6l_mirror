...
/**
* @enum HW_SIP7_FDB_UPDATE_MSG_TYPE
 *
 * @brief Enum for FDB update message type : SIP7
*/
typedef enum{

    /** @brief - New address */
    HW_SIP7_FDB_UPDATE_MSG_NA_E = 1,

    /** @brief - Moved address */
    HW_SIP7_FDB_UPDATE_MSG_MOVED_E = 2,

    /** @brief - Query Address */
    HW_SIP7_FDB_UPDATE_MSG_QA_E = 3,

    /** @brief - Query Respond */
    HW_SIP7_FDB_UPDATE_MSG_QR_E = 4,

    /** @brief - Aged Out Address */
    HW_SIP7_FDB_UPDATE_MSG_AA_E = 5,

    /** @brief - Transplanted Address */
    HW_SIP7_FDB_UPDATE_MSG_TA_E = 6,

    /** @brief - FDB Upload message */
    HW_SIP7_FDB_UPDATE_MSG_FU_E = 7,

    /** @brief - Index Query Request and Index Query Respond */
    HW_SIP7_FDB_UPDATE_MSG_QI_E = 8,

    /** @brief - FDB Hash Request */
    HW_SIP7_FDB_UPDATE_MSG_HR_E = 9,

} HW_SIP7_FDB_UPDATE_MSG_TYPE;
...
/**
* @internal internal_auDesc2UpdMsg function
* @endinternal
*
* @brief   This function is called whenever an address update message is received.
*         It translates the descriptor into CPSS_MAC_UPDATE_MSG_EXT_STC format
...
* @param[in] devNum                   - number of device whose descriptor queue is processed
* @param[in] portGroupId              - the  - for multi-port-groups support
* @param[in] auDescExtPtr             - consider the AU message as array of words
* @param[in] auDescrFromDma           - AU descriptor is from DMA queue
*                                      GT_TRUE - AU descriptor is from DMA queue
*                                      GT_FALSE - AU descriptor is from FIFO but not DMA queue
*
...
*                                       GT_OK if successful, or
*                                       GT_FAIL otherwise.
* @param[out] needToSkipCurrMsgPtr     - (pointer to) indication that the caller need to skip current msg
*                                       GT_TRUE - the caller need to skip current msg
*                                       GT_FALSE- the caller not need to skip current msg
*/
GT_STATUS internal_auDesc2UpdMsg
(
    IN  GT_U8                         devNum,
    IN  GT_U32                        portGroupId,
    IN  GT_U32                        *auMsgArrayOfWordsPtr,
    OUT CPSS_MAC_UPDATE_MSG_EXT_STC   *addrUpMsgPtr,
    OUT GT_BOOL                       *needToSkipCurrMsgPtr
);
...
*                                      for numOfAuPtr entries that was given as INPUT
*                                      parameter.
*                                        ignored when sip7 device , or when is NULL
* @param[out] sip7HwAuMessagesPtr      - sip7 HW AU/FU messages raw format
*                                        ignored when non-sip7 device , or when is NULL
