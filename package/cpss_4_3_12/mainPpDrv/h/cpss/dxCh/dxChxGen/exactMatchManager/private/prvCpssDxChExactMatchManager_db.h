...
/* aligned to 32 bits */
    ...
    GT_U32 redirectPtr                                          : 18;/* treat this field as:
                                                                        ...
                                                                        vrfId 12 bits(17-28) when Redirect_Command="Assign_VRF_ID"
                                                                        genericAction 12 bits(17-28) when Redirect_Command="Assign_GenericAction"
...
/* aligned to 32 bits */
    ...
    GT_U32 redirectPtr                      : 18;/* treat this field as:
                                                    ...
                                                    vrfId 12 bits when Redirect_Command="Assign_VRF_ID"
                                                    genericAction 12 bits when Redirect_Command="Assign_Generic_Action"
...
/* aligned to 32 bits  */
    ...
    GT_U8 pattern[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS];
...
typedef struct{
    ...
    GT_BIT                     isLock              : GT_BIT_NUM_BITS/*1*/;/* indication for cuckoo if the entry is locked; true / false */
}PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INDEX_STC;

/**
 * @struct PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC
 *
 * @brief internal Exact Match manager DB (database) hash node in the DB
 *
*/
typedef struct hash_node{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC  *entryPointer;
    struct hash_node                                       *nextNodePointer;
}PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC;

/**
 * @struct PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_LIST_STC
 *
 * @brief internal Exact Match manager DB (database) of 'hash node' lists
 *        for keys: 19B,33B,47B only.
 *
 * NOTE: the format expected to be 'as compressed as possible'
*/
typedef struct{
    /* 'semi pointer' to head of 'conflic 19 bytes rules' list of entries in 'used' list */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      headOfEntriesListPointer_19B;
    /* 'semi pointer' to head of 'conflic 33 bytes rules' list of entries in 'used' list */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      headOfEntriesListPointer_33B;
    /* 'semi pointer' to head of 'conflic 47 bytes rules' list of entries in 'used' list */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      headOfEntriesListPointer_47B;

    GT_U16                                                  entriesCounter_19B;
    GT_U16                                                  entriesCounter_33B;
    GT_U16                                                  entriesCounter_47B;

}PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_LIST_STC;
...
typedef struct{
    ...
    GT_U8     exactMatchHwReducedActionDataArr[PRV_CPSS_DXCH_EXACT_MATCH_SIP7_REDUCED_ACTION_SIZE_CNS];/* the final 8 bytes to set the HW */
...
typedef struct{
    ...
    /** number of valid elements in the crcMultiHashIsForbiddenArr */
    GT_U32                               numberOfElemInCrcMultiHashIsForbiddenArr;
    /** mark all the hash results that have conflict with other
     *  rules that exists in the database. */
    GT_U32                              crcMultiHashIsForbiddenArr[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS];
    /** number of valid elements in the crcMultiHashDeleteScanArr */
    GT_U32                              numberOfElemInCrcMultiHashDeleteScanArr;
     /** the hash results for the key of the entry before delete
      *  scan operation */
    GT_U32                              crcMultiHashDeleteScanArr[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS];
...
typedef union {
   ...
   /* invalid expanded entry */
    ...
    /* invalidate hash node entry */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC    dbNode;
    struct{
        GT_U32  entriesCounter_19B;
        GT_U32  entriesCounter_33B;
        GT_U32  entriesCounter_47B;
        GT_U32  hwIndexUsed;
    }countersInfo;
    GT_U8 pattern[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS];
...
*/
    ...
    */
    ...
    /* the 'pool of hash nodes' according to size : (capacityInfo.hwCapacity.numOfHwIndexes/bankStep)*(CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/bankStep)
       for example: maximal 128K 5 bytes entries, is translated to maximal 64K 19 bytes rules with 8 hashes each.
       dynamic allocated during 'create manager' and released during 'delete manager'

       index to this array is (0..capacityInfo.hwCapacity.numOfHwIndexes/bankStep*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/bankStep-1)
    */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC   *hashNodePoolPtr;

   /* 'semi pointer' to head of free 'list' of entries in hashNodePoolPtr */
    /* the head hold the last one that was freed */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC    headOfFreeHashNodesList;

    /* number of entries in hashNodePoolPtr (must be in range :
       0..capacityInfo.hwCapacity.numOfHwIndexes/bankStep*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/bankStep -1) */
    GT_U32 totalFreeHashNodes;
    ...
    /* the 'index array' according to size : capacityInfo.hwCapacity.numOfHwIndexes
       ...
       each entry holds a pointer to link lists, per key length, of all entries that potentially can use that index.
    */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_LIST_STC  *hashIndexArr;

    /* total potential locations for 19B rules */
    GT_U32    sumEntriesCounter_19B;
    /* total potential locations for 33B rules */
    GT_U32    sumEntriesCounter_33B;
    /* total potential locations for 47B rules */
    GT_U32    sumEntriesCounter_47B;

    /* 'dummy' entry for cuckoo usage */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC     dummyEntry;
    /* potential locations for the 'dummy' entry */
    GT_U32 potentialHashResultsArr[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS];
    /* number of potential locations for the 'dummy' entry */
    GT_U32 numPotentialHashResultsArr;
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_TYPE_COUNTERS_STC typeCountersArr[CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E][CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E];
    ...
    /* enable software workaround that fix possible incorrect action selection.
        (APPLICABLE DEVICES: FALCON) */
    GT_BOOL actionMatchWaEnable;
