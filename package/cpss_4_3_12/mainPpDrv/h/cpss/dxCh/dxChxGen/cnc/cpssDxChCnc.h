...
/**
* @enum CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_ENT
 *
 * @brief CNC index modes for client Egress Packet type Pass/Drop.
*/
typedef enum{

    /** @brief Use the full index,
     * index = {5'b0, pkt_cmd[2:0], cpu/drop code[7:0]}. This is legacy mode.
     */
    CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CMD_AND_CODE_E,

    /** @brief Mask the packet command portion of the index,
     * index = {8'b0, cpu/drop code[7:0]}.
     */
    CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CODE_ONLY_E

} CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_ENT;

/**
* @struct CPSS_DXCH_CNC_BLOCK_SHARED_CLIENTS_CONFIG_STC
 *
 * @brief The CNC block shared clients configuration.
 *
*/
typedef struct{

    /** @brief CNC block client enable
     *  GT_TRUE - shared client in the respective index (0/1/2) is enabled
     *  GT_FALSE - shared client in the respective index (0/1/2) is disabled
     */
    GT_BOOL clientEnable;

    /** @brief shared clients select for particular CNC block */
    CPSS_DXCH_CNC_CLIENT_ENT clientSelect;

    /** @brief shared clients index offset for particular CNC block
     * (APPLICABLE RANGES: 0..1023)
     */
    GT_U32 clientOffset;

    /** @brief bitmap of counter index ranges */
    GT_U64 indexRangesBmp;

} CPSS_DXCH_CNC_BLOCK_SHARED_CLIENTS_CONFIG_STC;
...
(
    IN  GT_U8                       devNum,
    OUT GT_U32                      inProcessBlocksBmpPtr[]/*maxArraySize=2*/
);
