...
/* max number of lookups */
#define CPSS_DXCH_MAX_LOOKUP_CNS    4
...
typedef enum{
    ...
    /** @brief reads/writes
     *  just the second entry of the pair
     *  (APPLICALBE_DEVICES: AAS - Hierarchical ECMP entry pair)
     */
    CPSS_DXCH_PAIR_READ_WRITE_SECOND_ONLY_E  = 1
...
/**
* @enum CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_ENT
*
* @brief Enumeration of Generic Range Profile Modes
*        (APPLICABLE DEVICES: AAS)
*/
typedef enum
{
    /** @brief entry <Range Profile> isn't used as part of the comparison logic.*/
    CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_NOT_USED_E,

    /** @brief entry <Range Profile> is compared to the <Source/Target Port Profile>*/
    CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_PORT_PROFILE_E,

    /** @brief the entry <Range Profile> is compared with selected bits from the previous
       Classification Action assignment of the <Generic Classification> field. */
    CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_GENERIC_CLASSIFICATION_E

}CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_ENT;

/**
* @struct CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC
*
* @brief Generic Range Classification parameters
*        (APPLICABLE DEVICES: AAS)
*/
typedef struct{

    /** @brief indication whether this entry is valid */
    GT_BOOL valid;

    /** @brief lookup supports 2 range checks.
        (APPLICABLE RANGES: 0..1) */
    GT_U32 lookupIndex;

    /** @brief generic range profile mode */
    CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_ENT profileMode;

    /** @brief generic range profile. (APPLICABLE RANGES: 0..1023) */
    GT_U32 profileId;

    /** @brief generic range max value. (APPLICABLE RANGES: 0..65535) */
    GT_U32 maxValue;

    /** @brief generic range min value. (APPLICABLE RANGES: 0..65535) */
    GT_U32 minValue;

    /** @brief The assigned range match id to indicate the range
     *         match in the lookup key. (APPLICABLE RANGES: 0..255) */
    GT_U32 matchId;

} CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC;

/**
 * @struct CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC
 *
 * @brief Generic Classification parameters
 *        (APPLICABLE DEVICES: AAS)
*/
typedef struct{

    /** @brief Generic Classification Field Mask per lookup.
     * Determines which bits in the range [15+lsb, lsb]
     * of the Generic Classification field are overridden by the engine
     * lookup action. Per Bit logic:
     * 0 = Not overridden by the engine lookup assignment
     * 1 = Overridden by the engine lookup assignment */
    GT_U8 mask[CPSS_DXCH_MAX_LOOKUP_CNS*2];

    /** @brief Generic Classification Field lsb location per lookup.
     *  Determines the lsb of the Generic Classification field bits that
     *  are overridden by the engine lookup action.
     *  Each lookup can set up to 16 bits in the range [lsb+15, lsb]
     *  (APPLICABLE RANGE: 0..16). */
    GT_U32 lsbLocation[CPSS_DXCH_MAX_LOOKUP_CNS];

} CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC;
