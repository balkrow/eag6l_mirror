...
typedef struct{
    ...
    /** @brief Bypass TX Queue Enable
     *
     *  @note Parameter NOT relevant to cpssDxChNetIfSdmaSyncTxPacketSend
     *
     *  GT_FALSE - Do not bypass TX queue enable when cpssDxChNetIfSdmaTxPacketSend
     *             is called
     *  GT_TRUE  - Bypass TX queue enable so that packet can be setup now but
     *             sent later. Packet can be sent later by enabling TX queue from
     *             Service CPU or by calling CPSS API cpssDxChNetIfSdmaTxQueueEnable
     *             from Host CPU. It is assumed that the caller disabled TX queue
     *             before calling CPSS API cpssDxChNetIfSdmaTxPacketSend. CPSS
     *             does not check if the TX queue has been disabled when
     *             cpssDxChNetIfSdmaTxPacketSend is called.
     *
     * @note:      bypassQueueEnable should be either set to GT_TRUE or GT_FALSE
     *             for a given TX queue when calling cpssDxChNetIfSdmaTxPacketSend
     *             multiple times.
     *
     *             For example, on TX queue 7 if cpssDxChNetIfSdmaTxPacketSend is
     *             called 3 times for 3 different packets, then all 3 times
     *             bypassQueueEnable must be either setup to GT_TRUE or GT_FALSE
     */
    GT_BOOL bypassQueueEnable;
