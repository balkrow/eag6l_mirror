...
typedef struct{
    ...
    /** At vTCAM resize function calls (both auto-resize and applicaion calls)                      */
    /** size-up value enlarged and size-down value truncated due to allocation granulatity.         */
    /** This counter accumulates reminders of size-up and size-down values for the next resizes.    */
    /** This reminder subtracted from the next size-up value or added to the next size-down value . */
    /** relevant to priority driven vTCAMs only.                                                    */
    GT_U32 priorityRulesOversize;
...
typedef GT_U32 PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_DEVS_BITMAP[(PRV_CPSS_MAX_PP_DEVICES_CNS+31)/32];
...
/* get pointer to instance of VTCAM Manager DB.
   casting added to get actual type of pointer.
   _mngId  - ID of VTCAM manager */
#define VTCAM_MNG_DB_MAC(_mngId) ((PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_PTR)(PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.dxChVTcamDir.vTcamMngDB)[_mngId]))

/* get pointer to instance of VTCAM Manager DB.
   _mngId  - ID of VTCAM manager */
#define VTCAM_MNG_DB_PTR_MAC(_mngId) (PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.dxChVTcamDir.vTcamMngDB)[_mngId])
...
GT_COMP_RES prvCpssDxChVirtualTcamDbRuleIdTableRuleIdCompare
(
    IN  GT_VOID    *data_ptr1,
    IN  GT_VOID    *data_ptr2
);

GT_COMP_RES prvCpssDxChVirtualTcamDbRuleIdTableLogicalIndexCompare
(
    IN  GT_VOID    *data_ptr1,
    IN  GT_VOID    *data_ptr2
);

GT_COMP_RES prvCpssDxChVirtualTcamDbPriorityTablePriorityCompare
(
    IN  GT_VOID    *data_ptr1,
    IN  GT_VOID    *data_ptr2
);
GT_COMP_RES prvCpssDxChVirtualTcamDbPriorityTableLogicalIndexCompare
(
    IN  GT_VOID    *data_ptr1,
    IN  GT_VOID    *data_ptr2
);
