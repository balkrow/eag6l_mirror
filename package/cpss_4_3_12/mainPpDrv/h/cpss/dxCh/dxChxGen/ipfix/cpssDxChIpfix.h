...
/* there are up to 4 valid events per pipe (core) , so in Falcon with 8 pipes may hold 32 events */
#define CPSS_DXCH_IPFIX_ALARM_EVENTS_ARRAY_LENGTH_CNS   32
...
typedef enum{
    ...
    /** @brief performing read entry operation
     *  for IPFIX engine only.
     *  NOTE: in SIP 6.10 and above - using this mode
     *  connects the device to IPFIX counting engine
     *  (APPLICABLE DEVICES: AC5P; AC5X; Harrier; Ironman)
     */
...
(
    IN GT_U8                            devNum,
    IN CPSS_DXCH_POLICER_STAGE_TYPE_ENT stage,
    OUT GT_U32                          eventsArr[CPSS_DXCH_IPFIX_ALARM_EVENTS_ARRAY_LENGTH_CNS], /*maxArraySize=32*/
