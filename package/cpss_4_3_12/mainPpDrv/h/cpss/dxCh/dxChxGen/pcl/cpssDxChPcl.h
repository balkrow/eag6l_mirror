...
typedef struct{
    ...
    /** @brief index of analyzer interface
     *  (APPLICABLE DEVICES: Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
     *  (APPLICABLE RANGES: Falcon; AC5P; AC5X; Harrier; Ironman 0..6)
     *  (APPLICABLE RANGES: AAS 0..62)
...
typedef struct{
     ...
     *  relevant for IPCL action only
     *  Dedicated for action used at IPCL lookup0_0 only
     */
    CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT pcl0_1OverrideConfigIndex;

    /** @brief algorithm of selection
     *  index of PCL Cfg Table for IPCL lookup1
     *  Dedicated for action used at IPCL lookup0_0 and lookup0_1
     *  relevant for IPCL action only
     */
    CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT pcl1OverrideConfigIndex;

    /** @brief algorithm of selection
     *  index of PCL Cfg Table for MPCL lookup
     *  relevant for IPCL action only
     *  Dedicated for action used at IPCL lookup0_0, lookup0_1 and lookup1
     *  (APPLICABLE DEVICES: AAS)
     */
    CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT mpclOverrideConfigIndex;

    /** @brief algorithm of selection
     *  index of PCL Cfg Table for EPCL lookup
     *  relevant for MPCL action only
     *  (APPLICABLE DEVICES: AAS)
     */
    CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT epclOverrideConfigIndex;
 ...
 * (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
 *      And Source vPort Profile Assignment. (in 'vPort mode')
 * (APPLICABLE DEVICES: AAS)
*/
typedef struct{

    /** @brief : Assign new source ePort/vPort from sourcePortValue (vPort in 'vPort mode')
     *  GT_FALSE - don't assign new source ePort/vPort
     *  GT_TRUE  - Assign new source ePort/vPort from sourcePortValue.
     */
    GT_BOOL assignSourcePortEnable;

    /** @brief Source ePort/vPort value. (vPort in 'vPort mode')
     */
    GT_U32 sourcePortValue;

    /** @brief : Assign new source vPort Profile from sourceVPortProfileValue
     *      NOTE: relevant only in 'vPort mode'
     *
     *  GT_FALSE - don't assign new vPort Profile ePort
     *  GT_TRUE  - Assign new source vPort Profile from sourceVPortProfileValue.
     *
     *  APPLICABLE DEVICES : AAS.
     *                      NOTE: this parameter ignored for all 'non-applicable devices'
     */
    GT_BOOL assignSourceVPortProfileEnable;

    /** @brief Source vPort Profile value.
     *      NOTE: relevant only in 'vPort mode'
     *
     *  APPLICABLE DEVICES : AAS.
     *                      NOTE: this parameter ignored for all 'non-applicable devices'
     *  (APPLICABLE RANGES: 0..16K-1)
     */
    GT_U32 sourceVPortProfileValue;
...
typedef enum{
    ...
    */
    ...
    /** @brief type for thread to handle IFA packets */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IFA_E,

    /** @brief type for thread to handle Router SA Modification */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_E,

    /** @brief type for thread to handle VXLAN DCI VNI Remapping */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_VXLAN_DCI_VNI_REMAP_E,
...
/**
 * @struct CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IFA_STC
 *
 * @brief This structure defines the PHA metadata for IFA PHA threads.
 *
*/
typedef struct{
    /** @brief IFA flow entry index to the PHA Shared Memory
     *  (APPLICABLE RANGE: 0..511)
    */
    GT_U32 flowId;
} CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IFA_STC;

/**
 * @struct CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_STC
 *
 * @brief This structure defines the PHA metadata for Router SA Modification PHA threads.
 *
*/
typedef struct{
    /** @brief member is bits [31:0] of MAC SA
     *         e.g. macSaLsb should be 0x30405060 for MAC SA 10:20:30:40:50:60
     */
    GT_U32 macSaLsb;
} CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_STC;

/**
 * @struct CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_VXLAN_DCI_VNI_REMAP_STC
 *
 * @brief This structure defines the PHA metadata for VXLAN DCI VNI REMAP PHA thread.
 *
*/
typedef struct{
    /** @brief member is bits [23:0] of Remapped VXLAN DCI VNI
     *  (APPLICABLE RANGE: 0..16777215)
     *         e.g., Remapped VNI = vxlanVni[23:0]
     */
    GT_U32 vxlanVni;
} CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_VXLAN_DCI_VNI_REMAP_STC;
...
typedef union{
    ...
    /** @brief EPCL info for thread to handle IFA packets in the system */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IFA_STC epclIfaFlow;

    /** @brief EPCL info for thread to handle Router SA modification in the system */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_STC epclRouterSaModification;

    /** @brief EPCL info for thread to handle VXLAN DCI VNI REMAP in the system */
    CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_VXLAN_DCI_VNI_REMAP_STC epclVxlanVni;
...
/**
 * @struct CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC
 * @brief Defines the MPCL action fields changing Header Alteration behavior
*/
typedef struct
{
    /** @brief Prevents the outgoing TTL from being decremented.
     *  GT_TRUE -  TTL unchanged
     *  GT_FALSE - TTL decremented
     */
    GT_BOOL  routerTtlDecrementDisable;

    /** @brief Prevents Router MAC SA assignment and clears eDSA <Routed> bit.
     *  GT_TRUE -  Disabled
     *  GT_FALSE - Uncanged
     */
    GT_BOOL  routerMacSaAssignmentDisable;

    /** @brief Prevents ARP DA and IP header modifications.
     *  GT_TRUE -  Disabled
     *  GT_FALSE - Uncanged
     */
    GT_BOOL  routerMacDaAndIpHeaderModificationsDisable;

} CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC;
...
typedef struct{
    ...
    /** @brief Cancel Drop Command of paralell sublookup
     *  GT_TRUE - cancel, GT_FALSE - leave as is
     *  (APPLICABLE DEVICES: AAS)
     *  TCAM parallel lookups 0-3 can cause resolution of 4 actions.
     *  Using this field action can cancel soft and hard drop packet
     *  commands from actions with indexes greater than index of canceling lookup.
     *  The cpssDxChPclParallelLookupDropCancelEnableSet API enables/disables
     *  this feature for different parallel lookup index pairs.
     */
    GT_BOOL cancelDrop;
    ...
    /** @brief used values:
     *  CPSS_PCL_DIRECTION_INGRESS_E == GT_FALSE - Action is used for the Ingress Policy
     *  CPSS_PCL_DIRECTION_EGRESS_E == GT_TRUE - Action is used for the Egress Policy
     *  CPSS_PCL_DIRECTION_MIDWAY_E - Action is used for the Midway Policy
     */
    CPSS_PCL_DIRECTION_ENT egressPolicy;
    ...
    /** @brief Ip Unicast Route action parameters configuration.
    ...
    /** @brief flow Id. (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
    ...
    GT_BOOL exactMatchOverPclEn;
     ...
     /** @brief GT_TRUE - Disables EM lookup on EM clients stages that
     *          configured enable for this feature.
     *          GT_FALSE - EM lookup enabled according to other configurations.
     *   (APPLICABLE DEVICES: AAS) IPCL, MPCL
     */
    GT_BOOL exactMatchDisable;
    ...
    */
    ...
    /** @brief Defines the MPCL action fields changing Header Alteration behavior.
     *   (APPLICABLE DEVICES: AAS) MPCL
     */
     CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC     headerAlterationOptions;
...
typedef enum{
     ...
     *   (APPLICABLE DEVICES: AC5P; AC5X; Harrier; Ironman)
     *   80 Bytes HW Pattern/Mask format
     *   (APPLICABLE DEVICES: AAS)
     */
    ...
    /** @brief  PCL key 80B in HW Pattern/Mask format (egress)
     *  (APPLICABLE DEVICES: AAS)
     */
    CPSS_DXCH_PCL_RULE_FORMAT_EGRESS_UDB_80_E            ,
...
/* maximal size of udb array in rule structures UDB_ONLY */
/* used for AAS 80-bytes rules                           */
#define CPSS_DXCH_PCL_UDB_ONLY_RULE_MAX_SIZE_CNS 80


/* Amount of replaceable UDBs in Ingress UDB only keys */
#define CPSS_DXCH_PCL_INGRESS_UDB_REPLACE_MAX_CNS 19

/* Amount of byte pairs selected from Action modified 40-byte data to bytes 60-79 of IPCL and MPCL superkey */
#define CPSS_DXCH_PCL_INGRESS_ACTION_MODIFIED_DATA_SELECTION_SIZE_CNS 10
...
typedef struct{
    ...
    /** @brief UDBs 23-26 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb23_26[4];
...
typedef struct{
    ...
    /** @brief UDBs 27-30 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb27_30[4];
...
typedef struct{
    ...
    /** @brief UDBs 31-34 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb31_34[4];
...
typedef struct{
    ...
    /** @brief User Defined Bytes 47-49
     *  (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  Comments:
...
typedef struct{
    ...
    /** @brief UDBs 39-46 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb39_46[8];
...
typedef struct{
    ...
    /** @brief  UDBs 47-49 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb47_49[3];

    /** @brief UDBs 0-4 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb0_4[5];
...
typedef struct{
    ...
    /** @brief UDBs 15-22 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb15_22[8];
...
typedef struct{
    ...
    /** @brief UDBs 35-38 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb35_38[4];
...
typedef struct{
    ...
    /** @brief UDBs 23-30 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb23_30[8];
...
typedef struct{
    ...
    /** @brief UDBs 0-3 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb0_3[4];
...
typedef struct{
    ...
    /** @brief UDBs 4-7 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb4_7[4];
...
typedef struct{
    ...
    /** @brief UDBs 8-11 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy standard key size 30 bytes
     */
    GT_U8 udb8_11[4];
...
typedef struct{
    ...
    /** @brief UDBs 12-19 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb12_19[8];
...
typedef struct{
    ...
    /** @brief UDBs 20-27 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb20_27[8];
...
typedef struct{
    ...
    /** @brief UDBs 28-35 (APPLICABLE DEVICES: Bobcat2, Caelum, Aldrin, AC3X, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman)
     *  The additional field of devices with legacy extended key size 60 bytes
     */
    GT_U8 udb28_35[8];
 ...
 * (APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
 * CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_UDB_60_NO_FIXED_FIELDS_E
 * (APPLICABLE DEVICES: Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
 * CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_UDB_80_E
 * (APPLICABLE DEVICES: AC5P; AC5X; Harrier; Ironman; AAS)
*/
typedef struct{

    /** @brief values of UDBs selected by application
     */
    GT_U8 udb[CPSS_DXCH_PCL_UDB_ONLY_RULE_MAX_SIZE_CNS];
 ...
 *
 *   @brief enumerator for User Defined Byte Values Bitmap Mode.
 ...
 *   Relevant for Ingress PCL only.
 *   For AAS Relevant for Ingress and Midway PCL.
*/
typedef enum{
...
/**
* @enum CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_ENT
 *
 *   @brief enumerator for User Defined Byte Values Bitmap Start byte in the key.
 *   see comment of CPSS_DXCH_PCL_UDB_VALUES_BITMAP_MODE_ENT.
*/
typedef enum
{

    /** @brief bitmap start byte is 4. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_4_E,

    /** @brief bitmap start byte is 14. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_14_E,

    /** @brief bitmap start byte is 24. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_24_E,

    /** @brief bitmap start byte is 34. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_34_E,

    /** @brief bitmap start byte is 44. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_44_E,

    /** @brief bitmap start byte is 54. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_54_E,

    /** @brief bitmap start byte is 88. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_88_E,

    /** @brief bitmap start byte is 104. */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_BYTE_104_E

} CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_ENT;

/**
 * @struct CPSS_DXCH_PCL_UDB_VALUES_BITMAP_CONFIG_STC
 *
 *  @brief User Defined Byte49 Values Bitmap Configuration.
 */
typedef struct
{

    /** @brief User Defined Byte49 Values Bitmap start byte */
    /** @brief APPLICABLE DEVICES: AAS */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_START_ENT     udb49ValsBmpStart;

    /** @brief User Defined Byte49 Values Bitmap mode */
    CPSS_DXCH_PCL_UDB_VALUES_BITMAP_MODE_ENT      udb49ValsBmpMode;

} CPSS_DXCH_PCL_UDB_VALUES_BITMAP_CONFIG_STC;
...
/**
* @struct CPSS_DXCH_PCL_LOOKUP_ACTION_MODIFIED_FIELDS_SELECT_STC
 *
*  @brief Select Action Modified Metadata Byte-Pairs for PCL Superkey bytes 60-79
*
*/
typedef struct{

    /** @brief - selects 10 byte-pairs from Action Modified Metadata (24 byte-pairs)
     *  If the configured value is offset in Action Modified Metadata x (0 =< x < 24) then:
     *  Bytes [2*x+1:2*x] of the Action Modified Metadata are selected for this Action Modified Field
     *  (APPLICABLE RANGES: 0..24)
     *  */
    GT_U8 bytePairOffsets[CPSS_DXCH_PCL_INGRESS_ACTION_MODIFIED_DATA_SELECTION_SIZE_CNS];

} CPSS_DXCH_PCL_LOOKUP_ACTION_MODIFIED_FIELDS_SELECT_STC;
...
*                                      For AAS and above devices actionPtr->egressPolicy
*                                      is input field containing direction.
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
* @retval GT_BAD_PTR               - on null pointer
...
* @note   NOT APPLICABLE DEVICES:  Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                   - device number
...
/**
* @internal cpssDxChPclMidwayPolicyEnable function
* @endinternal
*
* @brief   Enables Midway Policy.
*
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclMidwayPolicyEnable
(
    IN GT_U8   devNum,
    IN GT_BOOL enable
);
...
/**
* @internal cpssDxChPclMidwayPolicyEnableGet function
* @endinternal
*
* @brief   Get status (Enable or Disable) of Midway Policy.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclMidwayPolicyEnableGet
(
    IN  GT_U8   devNum,
    OUT GT_BOOL *enablePtr
);
...
* @param[in] direction                - policy direction
*                                       (APPLICABLE DEVICES: AAS)
*                                       (APPLICABLE VALUES: CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_DIRECTION_MIDWAY_E)
...
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X; AAS.
*
* @param[in] devNum                   - device number
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AAS.
*
* @param[in] devNum                   - device number
* @param[in] direction                - Policy Engine direction, Ingress or Egress
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; AAS.
*
* @param[in] devNum                   - device number
* @param[in] ruleFormat               - rule format
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; AAS.
*
* @param[in] devNum                   - device number
...
*
* @note   APPLICABLE DEVICES:      Lion2.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Bobcat2; Caelum; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X; AAS.
*
* @param[in] devNum                   - device number
* @param[in] portGroupsBmp            - bitmap of Port Groups.
*                                      NOTEs:
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                   - device number
...
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; AC5P; AC5X; Harrier; Ironman; xCat3; AC5; Lion2; AAS.
*
* @param[in] devNum                         - device number
* @param[in] lookupNum                      - lookup stage number for which mask to be set
...
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AAS.
*
* @param[in] devNum                         - device number
...
*          to assign 'flow-id' field descriptor and/or 'stream-id' field descriptor.
*
* @note   APPLICABLE DEVICES:      Ironman; AAS.
...
*
* @note   APPLICABLE DEVICES:      Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; AAS.
*
* @param[in] devNum                - device number
* @param[in] direction             - policy direction
*                                    (APPLICABLE DEVICES: AAS)
...
/**
* @internal cpssDxChPclLookupActionModifiedFieldsSelectSet function
* @endinternal
*
* @brief   Set Action Modified Fields Select data
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; AC5 Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                         - device number
* @param[in] direction                      - Policy direction: Ingress or Midway
* @param[in] lookupNum                      - lookup stage number for which the data need to be configured
* @param[in] actionModifiedFieldsSelectPtr  - (pointer to) Action Modified Fields Select structure
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_BAD_PARAM             - on wrong parameter value
...
*
*/
GT_STATUS cpssDxChPclLookupActionModifiedFieldsSelectSet
(
    IN GT_U8                                                   devNum,
    IN CPSS_PCL_DIRECTION_ENT                                  direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT                              lookupNum,
    IN CPSS_DXCH_PCL_LOOKUP_ACTION_MODIFIED_FIELDS_SELECT_STC  *actionModifiedFieldsSelectPtr
);

/**
* @internal cpssDxChPclLookupActionModifiedFieldsSelectGet function
* @endinternal
*
* @brief   Get Action Modified Fields Select data
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; AC5 Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @param[in] direction                      - Policy direction: Ingress or Midway
* @param[in] lookupNum                      - lookup stage number for which the data need to be configured
* @param[out] actionModifiedFieldsSelectPtr - (pointer to) Action Modified Fields Select structure
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_BAD_PARAM             - on wrong parameter value
...
*
*/
GT_STATUS cpssDxChPclLookupActionModifiedFieldsSelectGet
(
    IN  GT_U8                                                   devNum,
    IN  CPSS_PCL_DIRECTION_ENT                                  direction,
    IN  CPSS_PCL_LOOKUP_NUMBER_ENT                              lookupNum,
    OUT CPSS_DXCH_PCL_LOOKUP_ACTION_MODIFIED_FIELDS_SELECT_STC  *actionModifiedFieldsSelectPtr
);

/**
* @internal cpssDxChPclUserDefinedByteValuesBitmapExpansionConfigSet function
* @endinternal
*
* @brief   Set the User Defined Byte (UDB) Values bitmap expansion configuration.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
* @param[in] direction                - policy direction
* @param[in] lookupNum                - lookup number
* @param[in] udbValuesBmpCfgPtr       - (pointer to)UDB Values bitmap configuration
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameter
* @retval GT_BAD_PTR               - on NULL pointer
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclUserDefinedByteValuesBitmapExpansionConfigSet
(
    IN  GT_U8                                      devNum,
    IN  CPSS_PCL_DIRECTION_ENT                     direction,
    IN  CPSS_PCL_LOOKUP_NUMBER_ENT                 lookupNum,
    IN  CPSS_DXCH_PCL_UDB_VALUES_BITMAP_CONFIG_STC *udbValuesBmpCfgPtr
);

/**
* @internal cpssDxChPclUserDefinedByteValuesBitmapExpansionConfigGet function
* @endinternal
*
* @brief   Get the User Defined Byte (UDB) Values bitmap expansion configuration.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
* @param[in] direction                 - policy direction
* @param[in] lookupNum                 - lookup number
* @param[out] udbValuesBmpCfgPtr       - (pointer to)UDB Values bitmap configuration
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameter
* @retval GT_BAD_PTR               - on NULL pointer
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclUserDefinedByteValuesBitmapExpansionConfigGet
(
    IN  GT_U8                                      devNum,
    IN  CPSS_PCL_DIRECTION_ENT                     direction,
    IN  CPSS_PCL_LOOKUP_NUMBER_ENT                 lookupNum,
    OUT CPSS_DXCH_PCL_UDB_VALUES_BITMAP_CONFIG_STC *udbValuesBmpCfgPtr
);

/**
* @internal cpssDxChPclParallelLookupDropCancelEnableSet function
* @endinternal
*
* @brief   Set enable cancel hard or soft drop command from action of other paralell lookup.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @param[in] direction                    - policy direction
* @param[in] lookupNum                    - lookup number
* @param[in] cancelDropType               - cancel Hard or Soft Drop command
* @param[in] parallelLookupWithDropCancel - parallel lookup that will cancel drop command
* @param[in] parallelLookupWithDrop       - parallel lookup that its drop command will be canceled
*                                           parallelLookupWithDropCancel < parallelLookupWithDrop < 4
* @param[in] enable                       - GT_TRUE - enable, GT_FALSE - disable
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameter
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclParallelLookupDropCancelEnableSet
(
    IN  GT_U8                           devNum,
    IN  CPSS_PCL_DIRECTION_ENT          direction,
    IN  CPSS_PCL_LOOKUP_NUMBER_ENT      lookupNum,
    IN  CPSS_DROP_MODE_TYPE_ENT         cancelDropType,
    IN  GT_U32                          parallelLookupWithDropCancel,
    IN  GT_U32                          parallelLookupWithDrop,
    IN  GT_BOOL                         enable
);

/**
* @internal cpssDxChPclParallelLookupDropCancelEnableGet function
* @endinternal
*
* @brief   Get enable cancel hard or soft drop command from action of other paralell lookup.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @retval GT_HW_ERROR              - on hardware error
*/
GT_STATUS cpssDxChPclParallelLookupDropCancelEnableGet
(
    IN  GT_U8                           devNum,
    IN  CPSS_PCL_DIRECTION_ENT          direction,
    IN  CPSS_PCL_LOOKUP_NUMBER_ENT      lookupNum,
