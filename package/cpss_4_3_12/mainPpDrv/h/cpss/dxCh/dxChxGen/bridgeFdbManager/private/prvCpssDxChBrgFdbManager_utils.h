...
extern const PRV_CPSS_ENTRY_FORMAT_TABLE_STC prvCpssDxChSip7FdbFdbTableFieldsFormat[SIP6_FDB_FDB_TABLE_FIELDS___LAST_VALUE___E];
...
/* sip6 and above : set the sip for the HW of the shadow */
#define SIP_OF_HW(_fdbManagerPtr)   \
    IS_BMP_SET_MAC(_fdbManagerPtr->supportedSipArr, CPSS_GEN_SIP_7_E) ? CPSS_GEN_SIP_7_E : CPSS_GEN_SIP_6_E

/* sip6 and above : select HW format according to sipX generation */
#define SIPx_FDB_HW_FORMAT(sip_gen)                \
    ((sip_gen == CPSS_GEN_SIP_7_E) ?               \
        prvCpssDxChSip7FdbFdbTableFieldsFormat :   \
        prvCpssDxChSip6FdbFdbTableFieldsFormat)


/* sip6 and above : macro to set value to field of (FDB) FDB entry format in buffer */
/* based on : SIP6_FDB_ENTRY_FIELD_SET_MAC */
#define SIPx_FDB_HW_ENTRY_FIELD_SET_MAC(sip_gen,_hwDataArr,_fieldName,_value) \
    U32_SET_FIELD_BY_ID_MAC(_hwDataArr,                                    \
        SIPx_FDB_HW_FORMAT(sip_gen),                                          \
        _fieldName,                                                        \
        _value)


/* sip6 and above : macro to get field value of (FDB) FDB entry format in buffer */
/* based on : SIP6_FDB_ENTRY_FIELD_GET_MAC */
#define SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(sip_gen,_hwDataArr,_fieldName,_value)      \
...
/* sip6 and above : macro to set value to MAC ADDR field of (FDB) FDB entry format in buffer */
/* based on : SIP6_FDB_ENTRY_FIELD_MAC_ADDR_SET_MAC */
#define SIPx_FDB_HW_ENTRY_FIELD_MAC_ADDR_SET_MAC(sip_gen,_hwDataArr,lowWord,highWord)  \
    FDB_HW_MAC_ADDR_SET_FIELD_BY_ID_MAC(_hwDataArr,                    \
        SIPx_FDB_HW_FORMAT(sip_gen),                                          \
        SIP6_FDB_FDB_TABLE_FIELDS_MAC_ADDR_E,                               \
        lowWord,highWord)

/* sip6 and above : macro to get value to MAC ADDR field of (FDB) FDB entry format in buffer */
/* based on : SIP6_FDB_ENTRY_FIELD_MAC_ADDR_GET_MAC */
#define SIPx_FDB_HW_ENTRY_FIELD_MAC_ADDR_GET_MAC(sip_gen,_hwDataArr,_macAddrBytes)  \
...
/* sip6 and above : macro to set value to DIP field of (FDB) FDB entry format in buffer */
#define SIPx_FDB_HW_ENTRY_FIELD_DIP_ADDR_SET_MAC(sip_gen,_hwDataArr, hwValue)       \
    SIPx_FDB_HW_ENTRY_FIELD_SET_MAC(sip_gen,_hwDataArr,                             \
        SIP6_FDB_FDB_TABLE_FIELDS_DIP_E,                                    \
        hwValue)

/* sip6 and above : macro to set value to SIP field of (FDB) FDB entry format in buffer */
#define SIPx_FDB_HW_ENTRY_FIELD_SIP_ADDR_SET_MAC(sip_gen,_hwDataArr, hwValue)       \
    SIPx_FDB_HW_ENTRY_FIELD_SET_MAC(sip_gen,_hwDataArr,                             \
...
/** Macro to determine if FDB manager work in 'vPort mode' */
#define IS_FDB_MANAGER_VPORT_MODE(_fdbManagerPtr)   \
    ((_fdbManagerPtr->entryAttrInfo.vportInfo.enableVPortMode == GT_TRUE) ? 1 : 0)

/** Macro to if port is in 'vPorts range' */
#define IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(_fdbManagerPtr,_portNum)   \
    ((IS_FDB_MANAGER_VPORT_MODE(_fdbManagerPtr) &&                     \
     (_portNum) >= _fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfPhyPortsToUse)   \
        ? 1 : 0)
