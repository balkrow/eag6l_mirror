...
#define MANAGER_GET_MAC(_fdbManagerId)  ((PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC*)(PRV_SHARED_FDB_MANAGER_DB_VAR_GET(prvCpssDxChFdbManagerDbArr)[_fdbManagerId]))
...
/* Check for sip_6 or above */
#define PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(_fdbManagerPtr)                                           \
    ((_fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP6_E) ||   \
     (_fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E))
...
/* Invalid HW index */
#define PRV_CPSS_DXCH_BRG_FDB_MANAGER_INVALID_HW_INDEX_CNS               0xFFFFFFFF
...
/* aligned to 32 bits */
    ...
    GT_U32  fid             :16;/* 13 bits for sip5 , 16 bits for sip7 */
...
/* left with 'spare' of : 32-(3+3+3+1) = 22 bits */
...
/* aligned to 32 bits (22+1+1 = 24) --> spare 8 bits  */

    /** Holds the hwIndex of ipv6-key part
     *  In case of sip_5    it can be any index, in the previous banks than the data part.
     *  In case of sip_5_25 it always "nhDataHwIndex - 1"
     */
    GT_U32  keyHwIndex              : HW_INDEX_NUM_BITS/*20*/;
/* aligned to 32 bits (20)          --> spare 12 bits  */
...
/* aligned to 32 bits */
    ...
    /** Holds the hwIndex of ipv6-Data part
     *  In case of sip_5    it can be any index, in the next banks than the key part.
     *  In case of sip_5_25 it always "keyHwIndex + 1"
     */
    GT_U32  nhDataHwIndex           : HW_INDEX_NUM_BITS/*20*/;
/* aligned to 32 bits */
...
/* SIP5 : number of words in the mac entry */
#define FDB_ENTRY_WORDS_SIZE_CNS        5
/* SIP6 : number of words in the mac entry */
#define SIP6_FDB_ENTRY_WORDS_SIZE_CNS   4
/* SIP 7 : number of words in the mac entry */
#define SIP7_FDB_ENTRY_WORDS_SIZE_CNS   5
...
typedef struct{
    ...
    */
    ...
    /* @brief non-sip7 : Temp variable to store the HW AUQ messages, in case of Learning scan */
    PRV_CPSS_AU_DESC_EXT_8_STC      hwAuMessages[AU_MAX_NUM_CNS];
    /* @brief sip7 : Temp variable to store the HW AUQ messages, in case of Learning scan */
    PRV_CPSS_AU_FU_MSG_SIP7_STC     sip7_hwAuMessages[AU_MAX_NUM_CNS];
...
typedef struct{
    ...
    */
    ...
    /* The bank number of the associated IPv6-Route-Data entry
     *  in case of SIP_5_25 - Its always "selectedBankId + 1"
     */
    GT_U32                              nhDataBankNumArr[CPSS_DXCH_BRG_FDB_MANAGER_MAX_NUM_BANKS_CNS];
...
/*
 * @struct PRV_CPSS_DXCH_FDB_MANAGER_DB_REHASHING_ENTRY_HW_INFO_STC
 *
 * @brief internal structure holds info of rehashing stage HW info.
 *
*/
typedef struct{
    /** @brief HW index of the FDB entry from rehashed or selected bank
     *  Rehashed bank - 'source' bank - the entry in this bank is candidate to be moved to another bank in the same hash
     *  Selected bank - 'destination' bank the place where entry from rehashed bank will moved
     *  In terms of rehashing, the rehashing stage becomes selected after a free entry is found
     *  and the entry from the rehashed bank can potentially be moved to a new location */
    GT_U32      entryHwIndex;
    /** @brief HW index of the FDB extended data for IPv6 UC entry type from rehashed or selected bank .
     *         PRV_CPSS_DXCH_BRG_FDB_MANAGER_INVALID_HW_INDEX_CNS - the entry is not IPv6 UC */
    GT_U32      entryHwExtIndex;
}PRV_CPSS_DXCH_FDB_MANAGER_DB_REHASHING_ENTRY_HW_INFO_STC;
...
typedef struct {
    ...
    /** @brief Rehashing stage entry HW info. Used in rehashing logic to prevent cross-links */
    PRV_CPSS_DXCH_FDB_MANAGER_DB_REHASHING_ENTRY_HW_INFO_STC stageHwInfo;
...
typedef struct{
    ...
    */
    ...
    /** @brief - number of fid that dynamicUcMacLimit_fid[] need to hold .
      * sip5/6 : 8K
      * sip7   : 64K
    */
    GT_U32                                                dynamicUcMacLimit_fid_num;
    /** @brief - dynamic UC mac number of entries - per fid
      * allocated in runtime during 'FDB manager create' , with size :
      * sip5/6 : 8K
      * sip7   : 64K
    */
    PRV_CPSS_DXCH_FDB_MANAGER_DB_DYNAMIC_UC_MAC_LIMIT_STC *dynamicUcMacLimit_fid/*[array]*/;
    ...
    **/
    ...
    /** @brief - the info about vPorts limitations (index is the portNum)
    *   but this DB is NOT per device , but is global.
    *   it is relevant to 'vPort mode'
    **/
    PRV_CPSS_DXCH_FDB_MANAGER_DB_DYNAMIC_UC_MAC_LIMIT_STC   *dynamicUcMacLimit_vPortNumPtr;
...
typedef struct{
    ...
    */
     ...
     */
    GT_U32    selfTestBitmap[(CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS___LAST___E + 31)/32];
...
typedef enum{
    ...
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_E = BIT_0,

    /** @brief Indicates the HW format is for SIP6_10 devices */
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_10_E = BIT_1,

    /** @brief Indicates the HW format is for SIP5 devices */
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP5_E = BIT_2,

    /** @brief Indicates the HW format is for SIP5_20 devices */
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP5_20_E = BIT_3,

    /** @brief Indicates the HW format is for SIP4 devices */
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP4_E = BIT_4,

    /** @brief Indicates the HW format is for SIP7 devices */
    PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP7_E = BIT_5,
...
*
* @retval GT_TRUE              - the UC entry pointer is supported
* @retval GT_FALSE             - the UC entry pointer is not supported
*/
