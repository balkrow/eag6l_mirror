...
typedef enum{
    ...
    CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP4_E,
     ...
     /** @brief SIP7 type of DB,
      *  Upto 1M entries, upto 16 banks and multi hash value
      *  APPLICABLE DEVICES: AAS. */
    CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E
...
typedef struct{
     ...
     *  For AC5 devices and SIP6 shadow type working in legacy FDB hash mode - 1 Hash(4 Banks) */
    GT_U32   numOfHashes;

    /** @brief: Hash function modes
     *  APPLICABLE DEVICES: AC5; Falcon; AC5P; AC5X; Harrier; Ironman
     *  NOTE: For SIP6 shadow type the legacy mode is enabled if number of hashes is 1 Hash(4 Banks) */
...
/**
* @struct CPSS_DXCH_BRG_FDB_MANAGER_VPORT_MODE_INFO_STC
*
* @brief  info need in order for the FDB manager to work in 'vPort mode'
*
* APPLICABLE DEVICES: AAS.
*/
typedef struct{
     ...
     *  NOTE: 1. relevant only for 'sip7' type of shadow
     *        2. this value must be the same as given in : cpssDxChHwPpPhase1Init(...)
     *           as : ppPhase1ParamsPtr->vPortModeInfo.enableVPortMode
    */
    GT_BOOL enableVPortMode;

    /** @brief : Set the number of physical ports that the FDB manager should
     *   consider as 'non-vPorts' in range [0..(maxNumOfPhyPortsToUse-1)]
     *
     *  NOTEs: 1. relevant only when enableVPortMode == GT_TRUE
     *         2. This value must be the same as given in : cpssDxChHwPpPhase1Init(...)
     *            as : ppPhase1ParamsPtr->maxNumOfPhyPortsToUse
    */
    GT_U32  maxNumOfPhyPortsToUse;

    /** @brief : Set the total number of vPorts that the FDB manager should support.
     *   consider as 'vPorts' in range [maxNumOfPhyPortsToUse .. (maxNumOfVPortsToUse + maxNumOfPhyPortsToUse - 1)]
     *      (the range starts after the 'non-vPorts' range)
     *   example: maxNumOfPhyPortsToUse = 128 , maxNumOfVPortsToUse = 1000
     *           the ports   0 ..  127 are 'physical ports' that uses {portNum,hwDevNum}
     *           the ports 128 .. 1027 are 'vPorts'         that uses {portNum} (ignore hwDevNum)
     *
     *  NOTEs: 1. relevant only when enableVPortMode == GT_TRUE
     *         2. DB for vPorts is global (and not 'per device' like done for trunks)
     *
    */
    GT_U32  maxNumOfVPortsToUse;

}CPSS_DXCH_BRG_FDB_MANAGER_VPORT_MODE_INFO_STC;
...
typedef struct{
    ...
    */
     ...
     * NOTE : 1. should use the same value used for cpssDxChCfgGlobalEportSet() in globalPtr param
     *        2. in 'sip7' type of shadow :
     *           when 'vPort mode' is used , the 'Global Eports' MUST be disabled
     *           otherwise return GT_BAD_PARAM
     * APPLICABLE DEVICES: Bobcat2; Caelum; Bobcat3; Aldrin2; Aldrin; AC3X; Falcon; AC5P; AC5X; Harrier; Ironman.
     */
    CPSS_DXCH_CFG_GLOBAL_EPORT_STC  globalEportInfo;


    /** @brief: FDB Manager 'vPort mode' support.
     *      NOTE: relevant to 'sip7' type of shadow
     * APPLICABLE DEVICES: AAS.
     */
    CPSS_DXCH_BRG_FDB_MANAGER_VPORT_MODE_INFO_STC       vportInfo;
...
typedef struct{
     ...
     *  Note : in AAS the config is global and not 'per fdb entry' , see API :
     *         cpssDxChIpFdbRouteCounterSetIndexSet(...)
     */
...
typedef struct{
     ...
     *         Supported values:
     *         0        -> All trunk entries will be part of the scan
     *         non-zero -> Non of the trunk entries will  be part of aging scan.
     */
...
/**
 * @def CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_DO_ALL_MAC
 *
...
typedef enum{
    ...
    /** @brief : Indicates FDB manager "Used List check" test failed - Index is not in valid range */
    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_USED_LIST_HW_INDEX_INVALID_E,
    /** @brief : Indicates FDB manager "Used List check" test failed - entry from DB not points to the entry in index array */
    ...
    /** @brief : Indicates FDB manager "Index List check" test failed - HW index not matches index in DB */
    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E,
    /** @brief : Indicates FDB manager "Index List check" test failed - entry from indexes array not points to the current entry */
    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_INDEXES_DIFFERENT_E,
    /** @brief : Indicates FDB manager "Index List check" test failed - IPv6 Key part is not even index */
    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E,
    /** @brief : Indicates FDB manager "Index List check" test failed - IPv6 Data part is not odd index */
