...
/* macro to check that DB was allocated (initialized)
   to protect the code from access NULL pointer */
#define DB_CHECK_MAC(_ptr,_error_string)   \
    if(!_ptr) CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, _error_string)
...
#define TRUNK_VIRTUAL_FUNC_MAC(devNum) \
    (PRV_CPSS_NON_SHARED_FAMILY_INFO((PRV_CPSS_PP_MAC(devNum)->devFamily))->trunkInfo.boundFunc)
...
/* macro to allocate memory and memset to ZERO */
#define RESILIENT_MEM_CALLOC_MAC(_trunkInfoPtr,_pointer,_sizeInBytes) \
    {                                                                                \
        _pointer = cpssOsMalloc(_sizeInBytes);                                       \
        if(NULL == _pointer)                                                         \
        {                                                                            \
            prvCpssGenericTrunkResilientDbFree(_trunkInfoPtr);                       \
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM,                         \
                    "trunkManager [%s] failed to allocate memory [%d] bytes ",       \
                    #_pointer,_sizeInBytes);                                         \
        }                                                                            \
        cpssOsMemSet(_pointer,0,(_sizeInBytes));                                     \
    }

#define RESILIENT_DB(_trunkInfoPtr) (trunkInfoPtr->resilientHashMappingDataPtr)
#define RESILIENT_CASCADE_TRUNK_DESIGNATED_INDEX_GET(_dev, _index)                        \
    ((_index >=(PRV_CPSS_DEV_TRUNK_INFO_MAC(_dev)->numOfDesignatedTrunkEntriesHw/2))?     \
     (_index%(PRV_CPSS_DEV_TRUNK_INFO_MAC(_dev)->numOfDesignatedTrunkEntriesHw/2)):_index)

static GT_STATUS prvCpssGenericTrunkResilientHashDbMemberRemove
(
    IN GT_U8                                    devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC            *trunkInfoPtr,
    IN GT_U32                                   memberIndex
);
static GT_STATUS prvCpssGenericTrunkResilientHashDbMemberAdd
(
    IN GT_U8                                    devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC           *trunkInfoPtr
);
...
)
{
    ...
    if (PRV_CPSS_NON_SHARED_FAMILY_INFO(devPtr->devFamily)== NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, LOG_ERROR_NO_MSG);
    }
    ...
    virtualFunctionsPtr =
        &(PRV_CPSS_NON_SHARED_FAMILY_INFO(devPtr->devFamily)->trunkInfo.boundFunc);

    /* check that there are valid virtual functions */
    if(virtualFunctionsPtr->dbFlagsInitFunc == NULL      ||
       virtualFunctionsPtr->portTrunkIdSetFunc == NULL   ||
       virtualFunctionsPtr->membersSetFunc == NULL       ||
       virtualFunctionsPtr->nonMembersBmpSetFunc == NULL ||
       virtualFunctionsPtr->nonMembersBmpGetFunc == NULL ||
       virtualFunctionsPtr->designatedMembersBmpSetFunc == NULL ||
       virtualFunctionsPtr->designatedMembersBmpGetFunc == NULL ||
       virtualFunctionsPtr->ecmpTableSetFunc == NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, LOG_ERROR_NO_MSG);
    }
...
static int compareTrunkMembersFunc(const void *a, const void *b)
{
    CPSS_TRUNK_MEMBER_STC   *memberAPtr =  &((PRV_CPSS_TRUNK_DB_SORT_INFO_STC*)a)->member;
    CPSS_TRUNK_MEMBER_STC   *memberBPtr =  &((PRV_CPSS_TRUNK_DB_SORT_INFO_STC*)b)->member;

    if(memberAPtr->hwDevice == memberBPtr->hwDevice)
    {
        return ((int)memberAPtr->port - (int)memberBPtr->port);
    }

    return (int)memberAPtr->hwDevice - (int)memberBPtr->hwDevice;
}
...
{
    GT_U32      ii,jj;
    PRV_CPSS_TRUNK_DB_INFO_STC *genTrunkDbPtr = &(PRV_CPSS_PP_MAC(devNum)->trunkInfo);/* (pointer to) trunk db of the device */
    PRV_CPSS_TRUNK_DB_SORT_INFO_STC *sortArr = genTrunkDbPtr->tempSortingArr;
    ...
    if(numMembers > genTrunkDbPtr->maxNumMembersInDevice)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_SET_ERROR, "The tempSortingArr[] is not supporting more than [%d] members",
            genTrunkDbPtr->maxNumMembersInDevice);
    }
    ...
    {
        sortArr[ii].member        = membersArray[ii];
        sortArr[ii].isLocalMember = isLocalMembersPtr[ii];
    }

    cpssOsQsort(sortArr,numMembers,sizeof(PRV_CPSS_TRUNK_DB_SORT_INFO_STC),
        compareTrunkMembersFunc);

    /* copy back info from sorted array into the 2 arrays */
    for(ii = 0 ; ii < numMembers; ii++)
    {
        membersArray[ii]      = sortArr[ii].member;
        isLocalMembersPtr[ii] = sortArr[ii].isLocalMember;
    }
...
)
{
    ...
    rc = trunkDbMembersFromStartIndexSort(devNum,trunkInfoPtr,0,trunkInfoPtr->enabledCount);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    rc = trunkDbMembersFromStartIndexSort(devNum,trunkInfoPtr,trunkInfoPtr->enabledCount,trunkInfoPtr->disabledCount);
    return rc;
}
...
/**
* @internal resilentTrunkTableUpdateHw function
* @endinternal
*
* @brief   Update L2 ECMP table - for Resilient Trunk
*
* @param[in] devNum               - the device to update.
* @param[in] trunkId              - Trunk Id.
* @param[in] trunkInfoPtr         - (pointer to) trunk DB entry.
* @param[in] trunkAction          - Trunk Action:
*                                   Note - Below are the only Valid action
...
* @retval GT_FAIL                  - an error occurred.
*/
static GT_STATUS resilentTrunkTableUpdateHw
(
    IN GT_U8                            devNum,
    IN GT_TRUNK_ID                      trunkId,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC   *trunkInfoPtr,
    IN PRV_CPSS_TRUNK_ACTION_ENT        trunkAction
)
{
    GT_STATUS                               rc;
    GT_U32                                  index;
    GT_BOOL                                 configLtt;
    GT_BOOL                                 allow_configLtt = GT_TRUE;
    /* trunk member entry, to write to HW */
    ...
    switch(trunkAction)
    {
        case PRV_CPSS_TRUNK_ACTION_ADD_E:
            index       = trunkInfoPtr->enabledCount-1;
            allow_configLtt = GT_FALSE; /* we assume that the LTT was already set */
            GT_ATTR_FALLTHROUGH;
        case PRV_CPSS_TRUNK_ACTION_INITIALIZATION_E:
            for(; index < trunkInfoPtr->enabledCount; index++)
            {
                nodePtr = RESILIENT_DB(trunkInfoPtr)->firstEcmpResilientHashNodeArray[index];
                while(nodePtr)
                {
                    /* Hw Update */
                    trunkMember.port       = trunkInfoPtr->membersPtr[index].port;
                    trunkMember.hwDevice   = trunkInfoPtr->membersPtr[index].hwDevice;

                    if(allow_configLtt == GT_TRUE &&
                       nodePtr->nextPtr == NULL &&
                       index == (trunkInfoPtr->enabledCount - 1))
                    {
                        /* allow to set the LTT and point to the members only when update the last member entry */
                        /* Write LTT table once for the trunkID */
                        configLtt = GT_TRUE;
                    }
                    else
                    {
                    ...
                    rc = TRUNK_VIRTUAL_FUNC_MAC(devNum).ecmpTableSetFunc(devNum, trunkId,
                            nodePtr->hashIndex, &trunkMember, configLtt);
                    if(rc != GT_OK)
                    {
                        return rc;
                    }
            ...
            break;
        case PRV_CPSS_TRUNK_ACTION_REMOVE_E:
            for(index = 0; index < trunkInfoPtr->enabledCount; index++)
            {
                /** Start updating the HW from the point,
                  * where "new hash indexes" added to the member
                  */
                nodePtr = RESILIENT_DB(trunkInfoPtr)->prevLastEcmpResilientHashNodeArray[index];
                while(nodePtr)
                {
                    /* Hw Update */
                    trunkMember.port       = trunkInfoPtr->membersPtr[index].port;
                    trunkMember.hwDevice   = trunkInfoPtr->membersPtr[index].hwDevice;
                    rc = TRUNK_VIRTUAL_FUNC_MAC(devNum).ecmpTableSetFunc(devNum, trunkId,
                            nodePtr->hashIndex, &trunkMember, GT_FALSE);
                    if(rc != GT_OK)
                    {
                        return rc;
                    }
                ...
                /* Clear the memory as HW write complete for this member */
                RESILIENT_DB(trunkInfoPtr)->prevLastEcmpResilientHashNodeArray[index] = NULL;
            }
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
...
)
{
    ...
    /** Update Resilient DB to HW:
     *  If at least 1 member exist and policy is Resilient
     */
    if((trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E) &&
        (trunkInfoPtr->enabledCount))
    {
        return resilentTrunkTableUpdateHw(devNum, trunkId, trunkInfoPtr,
                PRV_CPSS_TRUNK_ACTION_INITIALIZATION_E);
    }
...
/**
* @internal prvCpssGenericTrunkCascadeDbNumberOfEnabledPortsGet function
* @endinternal
*
* @brief   Gets the enabled ports count for cascade trunk.
*
* @param[in]  devNum               - the device number .
* @param[in]  trunkInfoPtr         - Trunk Info
*
* @retval enabledCount             - Indicates number of enabled ports.
*/
static GT_U32 prvCpssGenericTrunkCascadeDbNumberOfEnabledPortsGet
(
    IN GT_U8                                    devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC            *trunkInfoPtr
)
{
    GT_U32  ii;
    GT_U32  enabledCount = 0;   /* Number of enabled members for the trunk */

    for(ii = 0; ii < MAX_MEMBER_PORT_NUM_MAC(devNum); ii++)
    {
        if(1 == CPSS_PORTS_BMP_IS_PORT_SET_MAC(&trunkInfoPtr->localPortsBmp, ii))
        {
            enabledCount++;
        }
    }
...
/**
* @internal prvCpssGenericTrunkResilientDesignatedHwUpdate function
* @endinternal
*
* @brief   rewrite the designated table port member based on resilient policy
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Aldrin2; Bobcat3; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] devNum                   - device number
* @param[in] trunkInfoPtr             - (pointer to)Trunk info
* @param[in] entryIndex               - Index to the Designated Table
* @param[in/out] designatedPortsPtr   - (pointer to) designated ports bitmap
*
* @retval GT_OK                    - successful completion
* @retval GT_FAIL                  - an error occurred.
*/
static GT_STATUS prvCpssGenericTrunkResilientDesignatedHwUpdate
(
    IN GT_U8                            devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC    *trunkInfoPtr,
    IN GT_U32                           entryIndex,
    INOUT CPSS_PORTS_BMP_STC            *designatedPortsPtr
)
{
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC   *nodePtr;
    GT_U32                              index;
    GT_U32                              enabledCount;
    GT_U32                              designatedTableSize;
    GT_PHYSICAL_PORT_NUM                portNum;

    designatedTableSize = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
    switch(trunkInfoPtr->type)
    {
        case CPSS_TRUNK_TYPE_CASCADE_E:
            enabledCount = prvCpssGenericTrunkCascadeDbNumberOfEnabledPortsGet(devNum, trunkInfoPtr);
            /** Resilient DB holds half of the Designated table(64 Entries)
             * HW update should happen to both half. */
            entryIndex = RESILIENT_CASCADE_TRUNK_DESIGNATED_INDEX_GET(devNum, entryIndex);
            break;
        case CPSS_TRUNK_TYPE_REGULAR_E:
            enabledCount = (trunkInfoPtr->enabledCount > designatedTableSize)?
                designatedTableSize:trunkInfoPtr->enabledCount;
            /** Resilient DB holds complete Designated table(128 Entries)
             * Index: 0 -63:  UC Designated table update
             * Index: 64-127: MC Designated table update
             */
             if(entryIndex >= designatedTableSize)
             {
                DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->firstMcDesignatedResilientHashNodeArray,
                    "cpssDxChTrunkDbPolicyInfoSet was not called for this regular trunk");
             }
             else
             {
                DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->firstUcDesignatedResilientHashNodeArray,
                    "cpssDxChTrunkDbPolicyInfoSet was not called for this regular trunk");
             }
            break;
        default:
    ...
    {
        nodePtr = (entryIndex >= designatedTableSize)?
            RESILIENT_DB(trunkInfoPtr)->firstMcDesignatedResilientHashNodeArray[index%designatedTableSize]:
            RESILIENT_DB(trunkInfoPtr)->firstUcDesignatedResilientHashNodeArray[index];
        /** Each trunk member will have multiple hashes,
         * Find if one of the hashes, matches with the incoming hash
         */
        while(nodePtr)
        {
            if(entryIndex == nodePtr->hashIndex)
            {
                /* Regular Trunks only local ports will be part of designated table */
                ...
                if (trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
                {
                    portNum = trunkInfoPtr->membersPtr[index].port;
                }
                else
                {
                    portNum = RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray[index];
                }

                CPSS_PORTS_BMP_PORT_SET_MAC(designatedPortsPtr, portNum);
                return GT_OK;
            }
            nodePtr = nodePtr->nextPtr;
        }
...
)
{
         ...
         entryIndex++)
    {
        ...
        if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
        {
            /* Update designatedPorts, according to resilient DB */
            rc = prvCpssGenericTrunkResilientDesignatedHwUpdate(devNum,
                    trunkInfoPtr,
                    entryIndex,
                    designatedPortsPtr);
            if (rc != GT_OK)
        ...
        else if(designatedMemberExists == GT_TRUE)
        {
...
)
{
    ...
    if(PRV_CPSS_HW_DEV_NUM_MAC(devNum) == trunkMemberPtr->hwDevice)
    {
        ...
        else if(IS_PHY_PORT_BY_MASK_MAC(devNum))
        {
            /* no special existence needed ... (all ports in mask are valid)
               following code below will check member's port by mask */
        }
        else
        {
    ...
    {
        /* member is on remote device. Allow maximal physical
           port number 1023 for SIP5 and above architecture. */
        maskPort = 1023;
    }
...
)
{
    ...
    /* Resilient Hash DB Update */
    if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
    {
        prvCpssGenericTrunkResilientHashDbMemberAdd(devNum, trunkInfoPtr);
    }
...
)
{
    ...
    */
    if(ii < trunkInfoPtr->enabledCount)
    {
        ...
        if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
        {
            /** Resilient trunk algoritham need to 'pop' removed member
             * Not swap by "last enabled" member
             */
            for(memberIndex=ii; memberIndex<(trunkInfoPtr->enabledCount-1); memberIndex++)
            {
                trunkDbMemberCopy(trunkInfoPtr, memberIndex, memberIndex+1);
            }
        }
        else
        {
    ...
    if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
    {
        prvCpssGenericTrunkResilientHashDbMemberRemove(devNum, trunkInfoPtr, ii);
    }
...
)
{
    ...
    */
    ...
    if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
    {
        return prvCpssGenericTrunkResilientHashDbMemberAdd(devNum, trunkInfoPtr);
    }
...
)
{
    ...
    if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
    {
        return prvCpssGenericTrunkResilientHashDbMemberRemove(devNum,
                trunkInfoPtr,
                ii);
    }
...
)
{
    ...
    /* check parameters validity */
    rc = trunkDbMembersValidityCheck(devNum,trunkId,0,NULL,1,memberPtr);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    if(GT_OK == trunkDbMemberIndexGet(devNum,trunkInfoPtr,memberPtr,&ii,&hwDevNumChanged))
    {
        if(hwDevNumChanged == GT_TRUE)
        {
            /* the application try to add a member ,
               but the 'similar' member was added to the trunk when an old
               hwDevNum of current local device */
        ...
        if(ii < trunkInfoPtr->enabledCount)
        {
            /* the member already enabled in the trunk , so just need to disable it , in the DB */
            return trunkDbMemberDisable(devNum,trunkId,memberPtr);
        }

        /* member already exists in this trunk as disabled */
        return GT_OK;
    }
...
)
{
    ...
    if(allowToUpdateTrunkMembers == GT_TRUE)
    {
        if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
        {
            ...
            {
                case PRV_CPSS_TRUNK_ACTION_ADD_E:
                case PRV_CPSS_TRUNK_ACTION_ENABLE_E:
                    rc = resilentTrunkTableUpdateHw(devNum, trunkId, trunkInfoPtr,
                            PRV_CPSS_TRUNK_ACTION_ADD_E);
                    break;
                case PRV_CPSS_TRUNK_ACTION_REMOVE_E:
                case PRV_CPSS_TRUNK_ACTION_DISABLE_E:
                    rc = resilentTrunkTableUpdateHw(devNum, trunkId, trunkInfoPtr,
                            PRV_CPSS_TRUNK_ACTION_REMOVE_E);
                    break;
                default:
                    /* Nothing to do for resilient DB */
                    break;
            }
        }
        else
...
)
{
    ...
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                "Trunk Policy: Resilient, Designated members can not be updated");
    }
...
/**
* @internal trunkDbCascadePortResilientDbAdd function
* @endinternal
*
* @brief  Add port to resilient DB for cascade trunk
*
...
*/
static GT_STATUS trunkDbCascadePortResilientDbAdd
(
    IN GT_U8                             devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC    *trunkInfoPtr,
...
{
    PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC    *resilientDb;
    GT_STATUS                           rc;

    DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray,
        "cpssDxChTrunkDbCascadePolicyInfoSet was not called for this cascade trunk");

    /* update dedicated for resilient algorithm cascade members DB */
    resilientDb = trunkInfoPtr->resilientHashMappingDataPtr;
    resilientDb->cascadeResilientPortsArray[resilientDb->cascadeResilientPortsCnt] =
        portNum;
    resilientDb->cascadeResilientPortsCnt += 1;

    /* Update Resilient DB */
    rc = prvCpssGenericTrunkResilientHashDbMemberAdd(devNum, trunkInfoPtr);
...
)
{
    ...
    if(portsMembersPtr)
    {
        ...
        if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
        {
            RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsCnt = 0;
        }
        /* set the ports as members of this trunk */
        for(ii = 0; ii < MAX_MEMBER_PORT_NUM_MAC(devNum); ii++)
        {
            ...
            if(trunkInfoPtr->policy == CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
            {
                rc = trunkDbCascadePortResilientDbAdd(devNum, trunkInfoPtr, ii);
                if(rc != GT_OK)
                {
                    return rc;
                }
...
*          cpssDxChTrunkMemberAdd           , cpssDxChTrunkMemberRemove, cpssDxChTrunkMemberDisabledAdd,
...
)
{
    ...
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                "Trunk Policy: Resilient, Cascade trunks weighted ports can not set");
    }
...
/**
* @internal prvCpssGenericTrunkMemberDisabledAdd function
* @endinternal
*
* @brief   Function Relevant mode : High Level mode
*         This function add member as disabled to the trunk in the device.
*         The function will disable it regardless if was already enabled/disabled in the trunk.
*         The member can be later enabled by cpssDxChTrunkMemberEnable(...)
*         Notes about designated trunk table:
*         If (no designated defined)
*           the disabled member will have 0 in it's bit in all entries of the table
*           other bits (of other ports) in the table will not be changed
*         else
*         1. If disabled member is not the designated member - set its relevant bits to 0
*         2. If disabled member is the designated member set its relevant bits
*         on all indexes to 0.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssGenericTrunkMemberDisabledAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN CPSS_TRUNK_MEMBER_STC    *memberPtr
)
{
    ...
    /* add the member to the DB */
    rc = trunkDbMemberAsDisabledAdd(devNum,trunkId,memberPtr);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    /* add the member to the HW as disabled */
    rc = trunkMemberAction(devNum,trunkId,memberPtr,
                            PRV_CPSS_TRUNK_ACTION_ADD_AS_DISABLED_E,
                            IS_LOCAL_CHECK_E,
                            GT_TRUE);/* allow update the trunk members table ,
                                as this member may have been in the trunk as enabled before
                                application tried to add it as disabled  */
...
)
{
    ...
    if(trunkId != 0)
    {
        ...
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                    "Trunk Policy: Resilient, Designated MC members can not be updated");
        }
...
/**
* @internal prvCpssGenericTrunkResilientDbFree function
* @endinternal
*
* @brief   Free the allocated memory for DB
*             "Ecmp and Designated" Resilient Hash.
*
* @param[in] trunkInfoPtr          - The trunk DB for current trunkId
*/
static GT_VOID prvCpssGenericTrunkResilientDbFree
(
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC        *trunkInfoPtr
)
{
    if(trunkInfoPtr->resilientHashMappingDataPtr == NULL)
    {
        return;
    }
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->allResilientEcmpHashArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->allUcDesignatedResilientHashArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->allMcDesignatedResilientHashArray);

    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->firstUcDesignatedResilientHashNodeArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->lastUcDesignatedResilientHashNodeArray);

    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->firstMcDesignatedResilientHashNodeArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->lastMcDesignatedResilientHashNodeArray);

    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->firstEcmpResilientHashNodeArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->lastEcmpResilientHashNodeArray);

    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray);

    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->prevLastEcmpResilientHashNodeArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->prevLastUcDesignatedResilientHashNodeArray);
    FREE_PTR_MAC(RESILIENT_DB(trunkInfoPtr)->prevLastMcDesignatedResilientHashNodeArray);
    FREE_PTR_MAC(trunkInfoPtr->resilientHashMappingDataPtr);
}
...
)
{
   ...
   /* init coreGen trunk DB */
    ...
    /* Resilient DB Clean UP - For all Trunks */
    for(ii = 0 ; ii < genTrunkDbPtr->numberOfTrunks ; ii++)
    {
        if(genTrunkDbPtr->trunksArray)
        {
            prvCpssGenericTrunkResilientDbFree(&genTrunkDbPtr->trunksArray[ii + 1]);
        }
    }
    ...
    FREE_PTR_MAC(genTrunkDbPtr->tempSortingArr);
...
/** Add a Hash Node to the "Top of the List"
 * for a particular trunk member in the "Resilient ECMP SW DB".
 *   Input: resilientDb - Pointer to the Resilient SW DB
 *          nodePtr     - pointer to the hash node, which need to be added/moved
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_ECMP_LIST_ADD_TO_FIRST(_resilientDb, _nodePtr, _destIndex)                \
    _nodePtr->prevPtr = 0;                                                                  \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->firstEcmpResilientHashNodeArray[_destIndex] =                             \
    _resilientDb->lastEcmpResilientHashNodeArray[_destIndex]  = _nodePtr;

/** Add a Hash Node to the "bottom of the List"
 * for a particular trunk member in the "Resilient ECMP SW DB".
 *   Input: resilientDb - Pointer to the Resilient SW DB
 *          nodePtr     - pointer to the hash node, which need to be added/moved
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_ECMP_LIST_ADD_TO_LAST(_resilientDb, _nodePtr, _destIndex)                 \
    _nodePtr->prevPtr = _resilientDb->lastEcmpResilientHashNodeArray[_destIndex];           \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->lastEcmpResilientHashNodeArray[_destIndex]->nextPtr = _nodePtr;           \
    _resilientDb->lastEcmpResilientHashNodeArray[_destIndex]          = _nodePtr;

/** Add a Hash Node to the "top of the List"
 * for a particular trunk member in the "Resilient Designated SW DB - UC Part".
 *   Input: resilientDb - Pointer to the Resilient SW DB
 *          nodePtr     - pointer to the hash node, which need to be added/moved
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_UC_DESIGNATED_LIST_ADD_TO_FIRST(_resilientDb, _nodePtr, _destIndex)       \
    _nodePtr->prevPtr = 0;                                                                  \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->firstUcDesignatedResilientHashNodeArray[_destIndex] =                     \
    _resilientDb->lastUcDesignatedResilientHashNodeArray[_destIndex]  = _nodePtr;
 ...
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_UC_DESIGNATED_LIST_ADD_TO_LAST(_resilientDb, _nodePtr, _destIndex)        \
    _nodePtr->prevPtr = _resilientDb->lastUcDesignatedResilientHashNodeArray[_destIndex];   \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->lastUcDesignatedResilientHashNodeArray[_destIndex]->nextPtr = _nodePtr;   \
    _resilientDb->lastUcDesignatedResilientHashNodeArray[_destIndex]          = _nodePtr;

/** Add a Hash Node to the "top of the List"
 * for a particular trunk member in the "Resilient Designated SW DB - MC Part".
 *   Input: resilientDb - Pointer to the Resilient SW DB
 *          nodePtr     - pointer to the hash node, which need to be added/moved
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_MC_DESIGNATED_LIST_ADD_TO_FIRST(_resilientDb, _nodePtr, _destIndex)       \
    _nodePtr->prevPtr = 0;                                                                  \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->firstMcDesignatedResilientHashNodeArray[_destIndex] =                     \
    _resilientDb->lastMcDesignatedResilientHashNodeArray[_destIndex]  = _nodePtr;
 ...
 *          destIndex   - Member index in the trunk
 */
#define RESILIENT_MC_DESIGNATED_LIST_ADD_TO_LAST(_resilientDb, _nodePtr, _destIndex)        \
    _nodePtr->prevPtr = _resilientDb->lastMcDesignatedResilientHashNodeArray[_destIndex];   \
    _nodePtr->nextPtr = 0;                                                                  \
    _resilientDb->lastMcDesignatedResilientHashNodeArray[_destIndex]->nextPtr = _nodePtr;   \
    _resilientDb->lastMcDesignatedResilientHashNodeArray[_destIndex]          = _nodePtr;

/** Disconnect the last node from "MC Designated list"
 *  for a particular trunk member.
 *   Input: resilientDb     - Pointer to the Resilient SW DB
 *          fromNodeIndex   - Member index in the trunk
 */
#define RESILIENT_MC_DESIGNATED_DISCONNECT_LAST_NODE(_resilientDb, _fromNodeIndex)          \
        _resilientDb->lastMcDesignatedResilientHashNodeArray[fromNodeIndex] =               \
            _resilientDb->lastMcDesignatedResilientHashNodeArray[fromNodeIndex]->prevPtr;   \
        _resilientDb->lastMcDesignatedResilientHashNodeArray[fromNodeIndex]->nextPtr = 0;   \

/** Disconnect the last node from "UC Designated list"
 *  for a particular trunk member.
 *   Input: resilientDb     - Pointer to the Resilient SW DB
 *          fromNodeIndex   - Member index in the trunk
 */
#define RESILIENT_UC_DESIGNATED_DISCONNECT_LAST_NODE(_resilientDb, _fromNodeIndex)          \
    _resilientDb->lastUcDesignatedResilientHashNodeArray[fromNodeIndex] =                   \
            _resilientDb->lastUcDesignatedResilientHashNodeArray[fromNodeIndex]->prevPtr;   \
    _resilientDb->lastUcDesignatedResilientHashNodeArray[fromNodeIndex]->nextPtr = 0;       \

/** Disconnect the last node from "ECMP list"
 *  for a particular trunk member.
 *   Input: resilientDb     - Pointer to the Resilient SW DB
 *          fromNodeIndex   - Member index in the trunk
 */
#define RESILIENT_ECMP_DISCONNECT_LAST_NODE(_resilientDb, _fromNodeIndex)           \
        _resilientDb->lastEcmpResilientHashNodeArray[fromNodeIndex] =               \
            _resilientDb->lastEcmpResilientHashNodeArray[fromNodeIndex]->prevPtr;   \
        _resilientDb->lastEcmpResilientHashNodeArray[fromNodeIndex]->nextPtr = 0;   \

/**
* @internal prvCpssGenericTrunkCascadeDbMemberIndexGet function
* @endinternal
*
* @brief   Gets the Member index for the cascade portNum.
*
* @param[in]  devNum              - the device number .
* @param[in]  trunkInfoPtr        - (Pointer to)Trunk Info.
* @param[in]  portNum             - Port Number.
* @param[out] memberIndex         - (Pointer to)Member Index.
*/
static GT_STATUS prvCpssGenericTrunkCascadeDbMemberIndexGet
(
    IN GT_U8                                     devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC            *trunkInfoPtr,
    IN GT_PHYSICAL_PORT_NUM                      portNum,
    OUT GT_U32                                  *memberIndexPtr
)
{
    GT_U32          memberIndex;
    GT_U32          maxCnt;
    ...
    maxCnt  = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
    for(memberIndex = 0; memberIndex < maxCnt; memberIndex++)
    {
        if(RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray[memberIndex] == portNum)
        {
            *memberIndexPtr = memberIndex;
            return GT_OK;
        }
    }
    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_FOUND, LOG_ERROR_NO_MSG);
...
/**
* @internal prvCpssGenericTrunkDebugResilientHashDbValidate function
* @endinternal
*
* @brief   Validate the current DB (ECMP/Designated list) list holds proper data.
*
* @param[in] devNum              - device number
* @param[in] trunkId             - Trunk ID
*
* @retval GT_OK                  - on success
* @retval GT_BAD_STATE           - bad resilient DB state
*/
GT_STATUS prvCpssGenericTrunkDebugResilientHashDbValidate
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId
)
{
    GT_U32      ii;
    GT_U32      enabledCount;   /* Number of enabled members for the trunk */
    GT_U32      maxStretch;     /* Number of hashes need to be mapped to the trunk */

    /* Temporary pointer to Hold the trunk DB and Resilient DB */
    PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC    *resilientDb;
    PRV_CPSS_TRUNK_ENTRY_INFO_STC           *trunkInfoPtr;

    /* pointer to the hash node */
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC       *nodePtr;
    GT_U32      designatedTableSize;

    /* Number of hash nodes for a particular member */
    GT_U32                                  hashCnt;
    GT_U32                                  hashCnt_max;
    GT_U32                                  hashCnt_min;
    GT_U32                                  hashCnt_min_index;

    designatedTableSize = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
    trunkInfoPtr        = TRUNK_ENTRY_GET_MAC(devNum, trunkId);
    if(!trunkInfoPtr)
    {
        cpssOsPrintf("trunk DB is not Present for dev[%d], trunkId[%d]\n", devNum, trunkId);
        return GT_OK;
    }
    ...
    if(!resilientDb)
    {
        cpssOsPrintf("Resilient DB is not Present for dev[%d], trunkId[%d]\n", devNum, trunkId);
        return GT_OK;
    }
    ...
    if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
    {
        ...
        maxStretch   = trunkInfoPtr->flexInfo.myTrunkMaxNumOfMembers;
        if(!enabledCount)
        {
            cpssOsPrintf("Resilient trunk have no Members - dev[%d], trunkId[%d]\n", devNum, trunkId);
            return GT_OK;
        }
        hashCnt_max = 0;
        /* Validate all member should have X or X+1 members only */
        for(ii=0; ii<enabledCount; ii++)
        {
            hashCnt = 0;
            nodePtr = resilientDb->firstEcmpResilientHashNodeArray[ii];
            while(nodePtr)
            {
                nodePtr = nodePtr->nextPtr;
                hashCnt +=1;
            ...
            else if((hashCnt_max != hashCnt) && (hashCnt_max != hashCnt+1))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                        "TrunkID[%d] - MemberIndex[%d] is having lesser ECMP hashIndex[%d], max is [%d]",
                        trunkId, ii, hashCnt, hashCnt_max);
            }
        }

        /* Validate the list with lowest number of HashIndex */
        /* Trunk Member Index with less(-1) number of Resilient Hash */
        if(resilientDb->trunkMemberWithLessEcmpResilientHashIndex !=
                (maxStretch % enabledCount))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                    "TrunkID[%d] - is having lesser ECMP hashIndex[%d], should be [%d]",
                    trunkId, ii, resilientDb->trunkMemberWithLessEcmpResilientHashIndex,
                    (maxStretch % enabledCount));
        }

        maxStretch = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
        enabledCount = (trunkInfoPtr->enabledCount > designatedTableSize)?
            designatedTableSize:trunkInfoPtr->enabledCount;
        /** Multicast designated table
         * Validate all member should have X or X+1 members only */
        for(ii=0; ii<enabledCount; ii++)
        {
            hashCnt = 0;
            nodePtr = resilientDb->firstMcDesignatedResilientHashNodeArray[ii];
            while(nodePtr)
            {
                nodePtr = nodePtr->nextPtr;
                hashCnt +=1;
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                        "TrunkID[%d] - MemberIndex[%d] is having lesser multicast designated hashIndex[%d], max is [%d]",
                        trunkId, ii, hashCnt, hashCnt_max);
            }
        }
        ...
        /* Validate the list with lowest number of HashIndex */
        /* Trunk Member Index with less(-1) number of Resilient Hash */
        if(resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex !=
                (maxStretch % enabledCount))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                    "TrunkID[%d] - is having lesser multicast designated hashIndex[%d], should be [%d]",
                    trunkId, ii, resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex,
                    (maxStretch % enabledCount));
        }
    }
    else
    ...
    /* Unicast designated table
     * Validate all members should have x or x+1 Members */
    if(trunkInfoPtr->type != CPSS_TRUNK_TYPE_FREE_E)
    {
        hashCnt_max       = 0;
        hashCnt_min       = 0;
        hashCnt_min_index = 0;
        ...
        {
            hashCnt = 0;
            nodePtr = resilientDb->firstUcDesignatedResilientHashNodeArray[ii];
            if(nodePtr)
            {
                while(nodePtr)
                {
                ...
                if(hashCnt_max == 0)
                {
                    hashCnt_max       = hashCnt;
                    hashCnt_min       = hashCnt;
                    hashCnt_min_index = ii;
                }
                else if((hashCnt_max != hashCnt) && (hashCnt_max != hashCnt+1))
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                            "TrunkID[%d] - MemberIndex[%d] is having lesser unicast designated hashIndex[%d], max is [%d]",
                            trunkId, ii, hashCnt, hashCnt_max);
                }
                else
                {
                    /* Get the member index holding the minimum hash count */
                    if(hashCnt_min > hashCnt)
                    {
                        hashCnt_min_index = ii;
                        hashCnt_min       = hashCnt;
                    }
        ...
        if(resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex !=
                hashCnt_min_index)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                    "TrunkID[%d] - is having lesser Unicast designated hashIndex[%d], should be [%d]",
                    trunkId, ii, resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex,
                    hashCnt_min_index);
        }
...
/**
* @internal prvDebugGenericTrunkDebugResilientHashDbDump function
* @endinternal
*
* @brief   Dump the Resilient Hash data (ECMP/Designated list).
*
* @param[in] devNum              - device number
* @param[in] trunkId             - Trunk ID
*
* @retval GT_EMPTY               - Resilient DB is empty for the trunk
* @retval GT_OK                  - on success
*/
GT_STATUS prvDebugGenericTrunkDebugResilientHashDbDump
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId
)
{
    ...
    GT_U32      enabledCount;   /* Number of enabled members for the trunk */
    GT_U32      memberIndex;    /* Index to the trunk member */
    GT_U32      designatedTableSize;
    ...
    if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
    {
        cpssOsPrintf("{ECMP Hash List}\n");
        for(memberIndex=0; memberIndex < trunkInfoPtr->enabledCount; memberIndex++)
        {
            cpssOsPrintf("Member Index [%2d] [%2d,%2d(port,dev)]:", memberIndex,
                    trunkInfoPtr->membersPtr[memberIndex].port,
                    trunkInfoPtr->membersPtr[memberIndex].hwDevice);
            nodePtr = resilientDb->firstEcmpResilientHashNodeArray[memberIndex];
            while(nodePtr)
            {
                cpssOsPrintf(" %4d,", nodePtr->hashIndex);
                nodePtr = nodePtr->nextPtr;
            }
            cpssOsPrintf("\n");
        }
        cpssOsPrintf("Ecmp Member with less Index :  [%d]\n", resilientDb->trunkMemberWithLessEcmpResilientHashIndex);
        enabledCount = (trunkInfoPtr->enabledCount > designatedTableSize)?
            designatedTableSize:trunkInfoPtr->enabledCount;
        cpssOsPrintf("\n{Multicast Designated Hash List}\n");
        for(memberIndex=0; memberIndex < enabledCount; memberIndex++)
        {
            ...
            while(nodePtr)
            {
                cpssOsPrintf(" %3d,", nodePtr->hashIndex);
                nodePtr = nodePtr->nextPtr;
            }
            cpssOsPrintf("\n");
        }
        cpssOsPrintf("\nMulticast Designated Member with less Index :  [%d]\n",
                resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex);
    }
    else
    {
        enabledCount = prvCpssGenericTrunkCascadeDbNumberOfEnabledPortsGet(devNum, trunkInfoPtr);
    }
    cpssOsPrintf("\n{Unicast Designated Hash List}\n");
    for(memberIndex=0; memberIndex < enabledCount; memberIndex++)
    {
        if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_CASCADE_E)
        {
            cpssOsPrintf("Member Index [%2d] [%2d(port)]:", memberIndex,
                RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray[memberIndex]);
        }
        else /* Regular Trunk */
        {
            cpssOsPrintf("Member Index [%2d] [%2d,%2d(port,dev)]:", memberIndex,
    ...
    cpssOsPrintf("\n Unicast Designated Member with less Index :  [%d]\n",
            resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex);
    return GT_OK;
}

/**
* @internal prvCpssGenericTrunkResilientHashDbMemberRemove function
* @endinternal
*
* @brief   Member removed from trunk, redistribute the hash among the enabled members.
*
* @param[in] devNum              - the device number.
* @param[in] trunkInfoPtr        - (pointer to) Trunk info.
* @param[in] memberIndex         - Member Index which got removed.
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameters
*/
...
)
{
    ...
    GT_U32 destNodeIndex;   /* Trunk member index */
    GT_U32 enabledCount;    /* Number of enabled members for the trunk */
    GT_U32 iiMax;
    ...
    if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
    {
        ...
        DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->firstEcmpResilientHashNodeArray,
            "cpssDxChTrunkDbPolicyInfoSet was not called for this regular trunk");

        /* Remove the last member */
        if(enabledCount == 0)
        {
            resilientDb->firstEcmpResilientHashNodeArray[0]         = NULL;
            resilientDb->lastEcmpResilientHashNodeArray[0]          = NULL;
            resilientDb->firstMcDesignatedResilientHashNodeArray[0] = NULL;
            resilientDb->firstUcDesignatedResilientHashNodeArray[0] = NULL;
            resilientDb->trunkMemberWithLessEcmpResilientHashIndex  = 0;
            resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex = 0;
        }
        else
        {
            /** Trunk DB already updated, Resilient DB is yet to update
             * enabledCount is not updated, according to Resilient DB
             */
            enabledCount  += 1;
            destNodeIndex = resilientDb->trunkMemberWithLessEcmpResilientHashIndex;
            nodePtr = resilientDb->firstEcmpResilientHashNodeArray[memberIndex];
            while(nodePtr)
            {
                /* if the member Index matches with the memberIndex of the incoming(Remove Member), go to next */
                destNodeIndex = (destNodeIndex == memberIndex)?
                    (destNodeIndex+1)%enabledCount:destNodeIndex;

                /** Store the last state, for HW update in case of Remove member
                 *  Will be cleared once the HW update complete
                 */
                if(!resilientDb->prevLastEcmpResilientHashNodeArray[destNodeIndex])
                {
                    resilientDb->prevLastEcmpResilientHashNodeArray[destNodeIndex] = nodePtr;
                }

                /* Disconnect the member from the existing list */
                resilientDb->firstEcmpResilientHashNodeArray[memberIndex] = nodePtr->nextPtr;

                /* Add to the end of the List, for a particular Trunk Member */
                RESILIENT_ECMP_LIST_ADD_TO_LAST(resilientDb, nodePtr, destNodeIndex);

                /* Get the next node to remove & destination Index, where the node need to be added */
                nodePtr = resilientDb->firstEcmpResilientHashNodeArray[memberIndex];
                destNodeIndex = (destNodeIndex+1) % enabledCount;
            }

            /* Sort the list of first, last and "list for remove" Ecmp Node array */
            for(ii = memberIndex; ii < (enabledCount - 1); ii++)
            {
                resilientDb->firstEcmpResilientHashNodeArray[ii] =
                    resilientDb->firstEcmpResilientHashNodeArray[ii+1];
                resilientDb->lastEcmpResilientHashNodeArray[ii] =
                    resilientDb->lastEcmpResilientHashNodeArray[ii+1];
                resilientDb->prevLastEcmpResilientHashNodeArray[ii] =
                    resilientDb->prevLastEcmpResilientHashNodeArray[ii+1];
            }

            destNodeIndex = (destNodeIndex == memberIndex)?
                (destNodeIndex+1)%enabledCount:destNodeIndex;
            resilientDb->trunkMemberWithLessEcmpResilientHashIndex =
                (destNodeIndex>=memberIndex)?destNodeIndex-1:destNodeIndex;

            /* Regular designated table(MC part) will be updated Both for Local and Remote ports
             * HW Update will set:
             *      Hash Index belongs to - member port is Local  - 1
             *      Hash Index belongs to - member port is remote - 0
             */
            if(memberIndex >= PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum))
            {
                /* Designated table holds first 64 members of the trunk member table */
                return GT_OK;
            }

            /* not '>=' as enabledCount is before we remove the member */
            if(enabledCount > PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum))
            {
                /* Designated table holds first 64 members of the trunk member table */
                /* so even if we remove a member from the first 64 entries , a port
                   from index 64 will take it's place */
                return GT_OK;
            }

            destNodeIndex = resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex;
            nodePtr       = resilientDb->firstMcDesignatedResilientHashNodeArray[memberIndex];
            while(nodePtr)
            {
                /* if the member Index matches with the memberIndex of the incoming(Remove Member), go to next */
                ...
                /* Disconnect the member from the existing list */
                resilientDb->firstMcDesignatedResilientHashNodeArray[memberIndex] = nodePtr->nextPtr;

                /* Add to the end of the List, for a particular Trunk Member */
                RESILIENT_MC_DESIGNATED_LIST_ADD_TO_LAST(resilientDb, nodePtr, destNodeIndex);
            ...
            /* Sort the list of first and last Designated Node array - MC Part */
            iiMax = MIN(enabledCount,PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum));
            iiMax--;/* remove 1 as we access (ii+1)*/

            for(ii = memberIndex; ii < iiMax; ii++)
            {
                resilientDb->firstMcDesignatedResilientHashNodeArray[ii] =
                    resilientDb->firstMcDesignatedResilientHashNodeArray[ii+1];
                resilientDb->lastMcDesignatedResilientHashNodeArray[ii] =
                    resilientDb->lastMcDesignatedResilientHashNodeArray[ii+1];
            }

            destNodeIndex = (destNodeIndex == memberIndex)?
                (destNodeIndex+1)%enabledCount:destNodeIndex;

            /* If the destination Index is present after the member removed,
             * Reduce it by 1, as the list adjusted
             */
            resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex =
                (destNodeIndex>=memberIndex)?destNodeIndex-1:destNodeIndex;
            enabledCount -= 1;
    ...
    /* Regular:
     *      Local  - Remove the hash List
     *      Remote - Remove the Empty Hash list
     * Cascade:
    ...
    {
        resilientDb->firstUcDesignatedResilientHashNodeArray[0] = NULL;
        resilientDb->lastUcDesignatedResilientHashNodeArray[0]  = NULL;
        resilientDb->cascadeResilientPortsCnt = 0;
        resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex = 0;
        return GT_OK;
    }

    nodePtr = resilientDb->firstUcDesignatedResilientHashNodeArray[memberIndex];
    if(resilientDb->firstUcDesignatedResilientHashNodeArray[memberIndex])
    {
        /* Find the number of enabled members with "valid list" of hashes */
        if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
        {
            for(ii=0; ii<trunkInfoPtr->enabledCount; ii++)
            {
                if(!trunkInfoPtr->isLocalMembersPtr[ii])
                {
                    remoteMemberCnt += 1;
                }
            }
        }
        /* Get the Remote Port count, and identify if this is the last member to remove */
        if((enabledCount - remoteMemberCnt) == 0)
        {
            resilientDb->firstUcDesignatedResilientHashNodeArray[memberIndex] = NULL;
            resilientDb->lastUcDesignatedResilientHashNodeArray[memberIndex]  = NULL;
            resilientDb->cascadeResilientPortsCnt = 0;
            resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex = 0;
            return GT_OK;
        }
        else
        {
             ...
             */
            enabledCount    += 1;
            destNodeIndex = resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex;
            nodePtr = resilientDb->firstUcDesignatedResilientHashNodeArray[memberIndex];
            while(nodePtr)
            {
                /* Get the next Valid List, in case of Unicast Designated table */
                for(ii=0; ii<enabledCount; ii++)
                {
                    if(resilientDb->firstUcDesignatedResilientHashNodeArray[destNodeIndex] &&
                            (destNodeIndex != memberIndex))
                    {
                        break;
                    }
                    destNodeIndex = (destNodeIndex+1) % enabledCount;
                ...
                /* Disconnect the member from the existing list */
                resilientDb->firstUcDesignatedResilientHashNodeArray[memberIndex] = nodePtr->nextPtr;

                /* Add to the end of the List, for a particular Trunk Member */
                RESILIENT_UC_DESIGNATED_LIST_ADD_TO_LAST(resilientDb, nodePtr, destNodeIndex);
    ...
    destNodeIndex = (ii == enabledCount)?0:destNodeIndex;

    iiMax = MIN(enabledCount,PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum));
    iiMax--;/* remove 1 as we access (ii+1)*/

    /* Sort the list of first and last Designated Node array */
    for(ii = memberIndex; ii < iiMax; ii++)
    {
        resilientDb->firstUcDesignatedResilientHashNodeArray[ii] =
            resilientDb->firstUcDesignatedResilientHashNodeArray[ii+1];
        resilientDb->lastUcDesignatedResilientHashNodeArray[ii] =
            resilientDb->lastUcDesignatedResilientHashNodeArray[ii+1];

        if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_CASCADE_E)
        {
            DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->cascadeResilientPortsArray,
                "cpssDxChTrunkDbCascadePolicyInfoSet was not called for this cascade trunk");
            /* Sort the member ports for the cascade trunk */
            resilientDb->cascadeResilientPortsArray[ii] =
                resilientDb->cascadeResilientPortsArray[ii+1];
        }
    }
    /* If the destination Index is present after the member removed,
     * Reduce it by 1, as the list adjusted
    ...
    if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_CASCADE_E)
    {
        /* update resilient cascade members count */
        resilientDb->cascadeResilientPortsCnt -= 1;
    }
...
/**
* @internal prvCpssGenericTrunkResilientHashDbMemberAdd function
* @endinternal
*
* @brief   New member added to trunk,
*          Redistribute the hash among the enabled members.
*          Regular Trunk:
*
*               ECMP table update
*               Unicast Designated Table update
*          cascade Trunk:
*               Unicast Designated Table update
*
* @param[in] devNum              - the device number .
...
{
    GT_U32      ii;
    GT_BOOL     updateUcDesignated = GT_TRUE;
    ...
    GT_U32 maxStretch;      /* Number of hashes need to be mapped to the trunk */
    GT_U32 fromNodeIndex;   /* Trunk member index */
    GT_U32 destMemberIndex; /* Trunk member index */
    GT_U32 enabledCount;    /* Number of enabled members for the trunk */
    GT_U32 remoteMemberCnt = 0;
    GT_U32 requiredHashForNewMember; /* Number of hashes required for the new Member */

    resilientDb     = trunkInfoPtr->resilientHashMappingDataPtr;

    if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
    {
        DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->allResilientEcmpHashArray,
            "cpssDxChTrunkDbPolicyInfoSet was not called for any regular trunk");

        maxStretch   = trunkInfoPtr->flexInfo.myTrunkMaxNumOfMembers;
        enabledCount = trunkInfoPtr->enabledCount;

        requiredHashForNewMember = maxStretch/enabledCount;
        /* New member added to the last Index */
        destMemberIndex = enabledCount-1;
        /* (memberWithLessIndex-1) holds 1 member extra, realign it with the new member */
        fromNodeIndex = resilientDb->trunkMemberWithLessEcmpResilientHashIndex;
        for(ii=0; ii<requiredHashForNewMember; ii++)
        {
            if(enabledCount == 1)
            {
                fromNodeIndex   = trunkInfoPtr->flexInfo.myTrunkStartIndexInMembersTable;
                nodePtr         = &resilientDb->allResilientEcmpHashArray[ii];
                destMemberIndex = 0;
            }
            else
            {
                fromNodeIndex = (fromNodeIndex != 0)?(fromNodeIndex-1):(destMemberIndex-1);
                nodePtr = resilientDb->lastEcmpResilientHashNodeArray[fromNodeIndex];
                /* Disconnect the node from the existing Member */
                RESILIENT_ECMP_DISCONNECT_LAST_NODE(resilientDb, fromNodeIndex);
            }

            /* Beginning of the List, for a particular Trunk Member */
            if(ii == 0)
            {
                RESILIENT_ECMP_LIST_ADD_TO_FIRST(resilientDb, nodePtr, destMemberIndex);
            }
            /* End of the List, for a particular Trunk Member */
            else
            {
                RESILIENT_ECMP_LIST_ADD_TO_LAST(resilientDb, nodePtr, destMemberIndex);
            }
        }
        resilientDb->trunkMemberWithLessEcmpResilientHashIndex = maxStretch % enabledCount;
        ...
        maxStretch = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
        requiredHashForNewMember = maxStretch/enabledCount;
        fromNodeIndex = resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex;
        for(ii=0; ii<requiredHashForNewMember; ii++)
        {
            if(enabledCount == 1)
            {
                fromNodeIndex   = 0;
                nodePtr         = &resilientDb->allMcDesignatedResilientHashArray[fromNodeIndex+ii];
                destMemberIndex = 0;
            }
            else
            {
                fromNodeIndex = (fromNodeIndex != 0)?(fromNodeIndex-1):(destMemberIndex-1);
                nodePtr = resilientDb->lastMcDesignatedResilientHashNodeArray[fromNodeIndex];
                /* Disconnect the node from the existing Member */
                RESILIENT_MC_DESIGNATED_DISCONNECT_LAST_NODE(resilientDb, fromNodeIndex);
            }

            /* Beginning of the List, for a particular Trunk Member */
            if(ii == 0)
            {
                RESILIENT_MC_DESIGNATED_LIST_ADD_TO_FIRST(resilientDb, nodePtr, destMemberIndex);
            }
            /* End of the List, for a particular Trunk Member */
            else
            {
                RESILIENT_MC_DESIGNATED_LIST_ADD_TO_LAST(resilientDb, nodePtr, destMemberIndex);
            }
        }
        resilientDb->trunkMemberWithLessMcDesignatedResilientHashIndex = maxStretch % enabledCount;

        /* Add Member to a NULL List,
         * If the port is remote
         */
        if(!trunkInfoPtr->isLocalMembersPtr[destMemberIndex])
        {
            resilientDb->lastUcDesignatedResilientHashNodeArray[destMemberIndex] = NULL;
            resilientDb->firstUcDesignatedResilientHashNodeArray[destMemberIndex] = NULL;
            updateUcDesignated = GT_FALSE;
        }
    }
    else /* CPSS_TRUNK_TYPE_CASCADE_E */
    {
    ...
    /** Update numbers for Designated table - MC Part
     *      - Regular Trunk - Remote member or
     *      - Cascade Trunk
     */
    if(updateUcDesignated)
    {
        /* Find the number of enabled members with "valid list" of hashes */
        if(trunkInfoPtr->type == CPSS_TRUNK_TYPE_REGULAR_E)
        {
        ...
        maxStretch      = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
        requiredHashForNewMember = maxStretch/(enabledCount-remoteMemberCnt);
        destMemberIndex = enabledCount-1;
        fromNodeIndex   = resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex;
        for(ii=0; ii<requiredHashForNewMember; ii++)
        {
            if((enabledCount-remoteMemberCnt) == 1)
            {
                nodePtr = &resilientDb->allUcDesignatedResilientHashArray[ii];
            }
            else
            {
                fromNodeIndex = (fromNodeIndex != 0)?(fromNodeIndex-1):(destMemberIndex-1);
                /* Get the next index, in case the current list is Invalid */
                while(!resilientDb->firstUcDesignatedResilientHashNodeArray[fromNodeIndex])
                {
                    fromNodeIndex = (fromNodeIndex != 0)?(fromNodeIndex-1):(destMemberIndex-1);
                }

                nodePtr = resilientDb->lastUcDesignatedResilientHashNodeArray[fromNodeIndex];
                /* Disconnect the node from the existing Member */
                RESILIENT_UC_DESIGNATED_DISCONNECT_LAST_NODE(resilientDb, fromNodeIndex);
            }

            /* Beginning of the List, for a particular Trunk Member */
            if(ii == 0)
            {
                RESILIENT_UC_DESIGNATED_LIST_ADD_TO_FIRST(resilientDb, nodePtr, destMemberIndex);
            }
            /* End of the List, for a particular Trunk Member */
            else
            {
                RESILIENT_UC_DESIGNATED_LIST_ADD_TO_LAST(resilientDb, nodePtr, destMemberIndex);
            }
        }
        /** The New Member is the only member having less Index,
         * OR - Its the only 1 member in the list
         */
        enabledCount -= remoteMemberCnt;
        if((maxStretch % enabledCount == enabledCount - 1) ||
                (enabledCount == 1))
        {
            resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex = destMemberIndex;
        }
        else
        {
            resilientDb->trunkMemberWithLessUcDesignatedResilientHashIndex = fromNodeIndex;
        }
    }
    return GT_OK;
}
...
/**
* @internal prvCpssGenericTrunkResilientDbListInit function
* @endinternal
*
* @brief   Initialize the Double linked list of Resilient Hash(ECMP/Designated list).
*          newly allocated memory will point to each other to form a list.
*
*
* @param[in] listPtr              - (pointer to) the ECMP/Designated List.
* @param[in] listNodeCnt          - Number of nodes in the list
* @param[in] startHashIndex       - Start Hash Index need to be added to the list.
*/
static GT_VOID prvCpssGenericTrunkResilientDbListInit
(
    IN PRV_CPSS_TRUNK_RESILIENT_HASH_STC       *listPtr,
    IN GT_U32                                   listNodeCnt,
    IN GT_U32                                   startHashIndex
)
{
    GT_U32                             ii;
    PRV_CPSS_TRUNK_RESILIENT_HASH_STC *nodePtr;

    /* init the 'linked list' of 'ECMP/Designated Resilient Hash' */
    ...
    nodePtr             = &listPtr[0];
    nodePtr->prevPtr    = 0;
    nodePtr->nextPtr    = &listPtr[1];
    nodePtr->hashIndex  = startHashIndex;
    /* Middle Nodes */
    for(ii = 1; ii < listNodeCnt-1; ii++)
    {
        nodePtr             = &listPtr[ii];

        nodePtr->prevPtr    = &listPtr[ii-1];
        nodePtr->nextPtr    = &listPtr[ii+1];
        nodePtr->hashIndex  = (startHashIndex + ii);
    }
    /* Last Node */
    nodePtr             = &listPtr[ii];
    nodePtr->prevPtr    = &listPtr[ii-1];
...
/**
* @internal prvCpssGenericTrunkResilientDbCreate function
* @endinternal
*
* @brief   Allocate the required DB for the "Ecmp and Designated" Resilient Hash.
*
* @param[in] devNum                - the device number .
* @param[in] trunkInfoPtr          - The trunk DB for current trunkId
...
*/
static GT_STATUS prvCpssGenericTrunkResilientDbCreate
(
    IN GT_U8                          devNum,
    IN PRV_CPSS_TRUNK_ENTRY_INFO_STC *trunkInfoPtr,
    IN CPSS_TRUNK_TYPE_ENT            type
)
{
    /* Temporary pointer to Hold the trunk DB and Resilient DB */
    PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC    *resilientDb;

    GT_U32 numBytesToAlloc; /* Number of bytes to allocate */
    GT_U32 memberCnt;       /* Total member count */

    /** Create DB:
     *      1. If the Current trunk is resilient(Cascade & Regular)
     *          1.1 Designated Resilient Hash List - UC Part
     *          1.2 List of First hash node - Designated Hash - UC Part
     *          1.3 List of Last hash node  - Designated Hash - UC Part
     *      2. If Trunk-Type is Regular only:
     *          2.1 ECMP Resilient Hash List
     *          2.2 Designated Resilient Hash List - MC Part
     *          2.3 List of First hash node - Designated Hash - MC Part
     *          2.4 List of Last hash node  - Designated Hash - MC Part
     *          2.5 List of First hash node - ECMP Hash
     *          2.6 List of Last hash node  - ECMP Hash
     */
    if(trunkInfoPtr->resilientHashMappingDataPtr)
    {
        /* DB Already allocated */
        return GT_OK;
    }

    RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
            resilientDb, sizeof(PRV_CPSS_RESILIENT_TRUNK_HASH_DB_STC));
    trunkInfoPtr->resilientHashMappingDataPtr           = resilientDb;

    /* 1.1 Designated Resilient Hash List - UC Part*/
    memberCnt = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
    numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC);
    RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
            resilientDb->allUcDesignatedResilientHashArray, numBytesToAlloc);

    /** 1.2 List of First hash node - Designated Hash - UC Part
     *  1.3 List of Last hash node  - Designated Hash - UC Part
     *     Designated Resilient Hash List management pointers, per Trunk
     */
    numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC*);
    RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
            resilientDb->firstUcDesignatedResilientHashNodeArray, numBytesToAlloc);
    RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
            resilientDb->lastUcDesignatedResilientHashNodeArray, numBytesToAlloc);
    RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
            resilientDb->prevLastUcDesignatedResilientHashNodeArray, numBytesToAlloc);

    /* Regular Trunk */
    if(type == CPSS_TRUNK_TYPE_REGULAR_E)
    {
        /* 2.1 ECMP Resilient Hash List */
        memberCnt = trunkInfoPtr->flexInfo.myTrunkMaxNumOfMembers;
        numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->allResilientEcmpHashArray, numBytesToAlloc);

        /* 2.5 List of first hash node - ECMP Hash
         * 2.6 List of last hash node  - ECMP Hash
         * ECMP Resilient Hash List management pointers, per Trunk
         */
        numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC*);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->firstEcmpResilientHashNodeArray, numBytesToAlloc);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->lastEcmpResilientHashNodeArray, numBytesToAlloc);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->prevLastEcmpResilientHashNodeArray, numBytesToAlloc);

        memberCnt = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
        /* 2.2 Designated Resilient Hash List - MC Part*/
        numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->allMcDesignatedResilientHashArray, numBytesToAlloc);
        ...
        numBytesToAlloc = memberCnt * sizeof(PRV_CPSS_TRUNK_RESILIENT_HASH_STC*);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->firstMcDesignatedResilientHashNodeArray, numBytesToAlloc);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->lastMcDesignatedResilientHashNodeArray, numBytesToAlloc);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->prevLastMcDesignatedResilientHashNodeArray, numBytesToAlloc);

    }
    else    /* Cascade Trunk */
    {
        /* List of sorted ports for cascade trunk */
        numBytesToAlloc = memberCnt * sizeof(GT_PHYSICAL_PORT_NUM);
        RESILIENT_MEM_CALLOC_MAC(trunkInfoPtr,
                resilientDb->cascadeResilientPortsArray, numBytesToAlloc);
    }
    return GT_OK;
}

/**
* @internal prvCpssGenericTrunkDbCascadePolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Sets policy info for the Cascade trunks. Valid only for Flex trunks.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
* @param[in] devNum                - the device number .
* @param[in] trunkId               - the trunk id.
* @param[in] policyInfoPtr         - (pointer to) Cascade trunk policy info details.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_NOT_INITIALIZED       - the trunk library was not initialized for the device
* @retval GT_BAD_PARAM             - bad device number , or bad trunkId number
* @retval GT_NOT_SUPPORTED         - changing policy in run time(Trunk type is not FREE)
* @retval GT_BAD_PTR               - one of the parameters in NULL pointer
*
*  note: This API should be called:
*        before "cpssDxChTrunkCascadePortAdd"
*
*/
GT_STATUS prvCpssGenericTrunkDbCascadePolicyInfoSet
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId,
    OUT CPSS_TRUNK_POLICY_STC       *policyInfoPtr
)
{
    GT_STATUS   rc;
    GT_U32      maxNumOfMembers;
    /* The trunk DB for current trunkId */
    PRV_CPSS_TRUNK_ENTRY_INFO_STC           *trunkInfoPtr;

    TRUNK_LIB_INIT_CHECK_MAC(devNum);
    CPSS_NULL_PTR_CHECK_MAC(policyInfoPtr);
    ...
    /* Policy update is not allowed, When trunk is already having members */
    if(trunkInfoPtr->type != CPSS_TRUNK_TYPE_FREE_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
    }
     ...
     *  Allocate the DB memory only incase RESILIENT
     */
    switch(policyInfoPtr->policy)
    {
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E:
            rc = prvCpssGenericTrunkResilientDbCreate(devNum, trunkInfoPtr,
                    CPSS_TRUNK_TYPE_CASCADE_E);
            if(rc != GT_OK)
            {
                return rc;
            ...
            /* Specific to particular Trunk - Unicast Designated Table Init */
            maxNumOfMembers = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
            prvCpssGenericTrunkResilientDbListInit(
                    RESILIENT_DB(trunkInfoPtr)->allUcDesignatedResilientHashArray,
                    maxNumOfMembers,
                    0 /*startHashIndex*/);
            trunkInfoPtr->policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E;
            break;
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E:
            trunkInfoPtr->policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E;
            /* Free Resilient DB if previously allocated */
            prvCpssGenericTrunkResilientDbFree(trunkInfoPtr);
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal prvCpssGenericTrunkDbCascadePolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Gets policy info in case of cascade trunks.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
* @param[in] devNum                - the device number .
* @param[in] trunkId               - the trunk id.
* @param[out] policyInfoPtr        - (pointer to) Cascade trunk policy info details.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_NOT_INITIALIZED       - the trunk library was not initialized for the device
...
*
*/
GT_STATUS prvCpssGenericTrunkDbCascadePolicyInfoGet
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId,
    OUT CPSS_TRUNK_POLICY_STC       *policyInfoPtr
)
{
    ...
    /* API Valid for cascade and free trunks */
    TRUNK_TYPE_SUPPORT_CASCADE_CHECK_MAC(trunkInfoPtr);

    switch(trunkInfoPtr->policy)
    {
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E:
            policyInfoPtr->policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E;
            break;
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E:
            policyInfoPtr->policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal prvCpssGenericTrunkDbPolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Gets policy info for the regular trunks. Valid only for Flex trunks.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
* @param[in] devNum                - the device number .
* @param[in] trunkId               - the trunk id.
* @param[out] policyInfoPtr        - (pointer to) policy info details.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_NOT_INITIALIZED       - the trunk library was not initialized for the device
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
*  note: This API should be called after "cpssDxChTrunkFlexInfoSet" and
*        before "cpssDxChTrunkCascadePortAdd"
*/
GT_STATUS prvCpssGenericTrunkDbPolicyInfoGet
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId,
    OUT CPSS_TRUNK_POLICY_STC       *policyInfoPtr
)
{
    ...
    /* API Valid for regular or free trunks */
    TRUNK_TYPE_SUPPORT_REGULAR_CHECK_MAC(trunkInfoPtr)
...
/**
* @internal prvCpssGenericTrunkDbPolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Sets policy info for the regular trunks. Valid only for Flex trunks.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
* @param[in] devNum                - the device number .
* @param[in] trunkId               - the trunk id.
* @param[in] policyInfoPtr         - (pointer to) policy info details.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_NOT_INITIALIZED       - the trunk library was not initialized for the device
...
* @retval GT_BAD_PARAM             - bad device number , or bad trunkId number
* @retval GT_BAD_PTR               - one of the parameters in NULL pointer
* @retval GT_BAD_STATE             - mode is not CPSS_DXCH_TRUNK_MEMBERS_MODE_FLEX_E
* @retval GT_NOT_SUPPORTED         - changing policy in run time(Trunk type is not FREE)
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
*  note: This API should be called
*        after "cpssDxChTrunkFlexInfoSet" and
*        before "cpssDxChTrunkMemberAdd/cpssDxChTrunkMemberEnable/cpssDxChTrunkMemberDisabledAdd"
*/
GT_STATUS prvCpssGenericTrunkDbPolicyInfoSet
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId,
    OUT CPSS_TRUNK_POLICY_STC       *policyInfoPtr
)
{
    ...
    if(trunkInfoPtr->type != CPSS_TRUNK_TYPE_FREE_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "Policy update is not allowed, When trunk is already having members");
    }
    ...
    {
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E:
            if(PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->loadBalanceType != PRV_CPSS_TRUNK_LOAD_BALANCE_FLEX_E)
            {
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "Resilient mode supported only with flex mode");
            }
            rc = prvCpssGenericTrunkResilientDbCreate(devNum, trunkInfoPtr,
            ...
            DB_CHECK_MAC(RESILIENT_DB(trunkInfoPtr)->allResilientEcmpHashArray,
                "probably cpssDxChTrunkDbCascadePolicyInfoSet was called for this trunk before (so DB for 'regular trunk' not allocated')");

            /* Specific to particular Trunk - ECMP Table Init */
            maxNumOfMembers = trunkInfoPtr->flexInfo.myTrunkMaxNumOfMembers;
            prvCpssGenericTrunkResilientDbListInit(
                    RESILIENT_DB(trunkInfoPtr)->allResilientEcmpHashArray,
                    maxNumOfMembers,
                    0 /*startHashIndex*/);

            /* Specific to particular Trunk - Multicast Designated Table Init */
            maxNumOfMembers = PRV_CPSS_TRUNK_DESIGNATED_TABLE_SIZE_MAC(devNum);
            prvCpssGenericTrunkResilientDbListInit(
                    RESILIENT_DB(trunkInfoPtr)->allMcDesignatedResilientHashArray,
                    maxNumOfMembers,
                    maxNumOfMembers /*startHashIndex*/);
        ...
        case CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E:
            ...
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(policyInfoPtr->policy);
    }
    return GT_OK;
}

/**
* @internal prvCpssGenericTrunkCascadePortRemove function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         This function remove port from the cascade trunk in the device.
*         Valid only if the trunk policy mode is RESILIENT.
*         If member not exists in this trunk, function do nothing and
*         return GT_OK.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
*                                    bad port number.
* @retval GT_BAD_STATE             - the trunk-type is not one of: CPSS_TRUNK_TYPE_FREE_E or CPSS_TRUNK_TYPE_REGULAR_E
*                                    or policy Mode is not RESILIENT for the trunk.
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssGenericTrunkCascadePortRemove
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    GT_STATUS       rc;
    GT_U32          memberIndex;
    /** Update Trunk DB - Cascade portMap
     * Update Resilient DB (for Resilient Trunk)
     * Update HW
     */
    ...
    /* this API supported only for cascade trunk */
    TRUNK_TYPE_SUPPORT_CASCADE_CHECK_MAC(trunkInfoPtr);

    if(trunkInfoPtr->policy != CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E)
    {
        /* API is for resilient trunk only */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "Supported only for resilient trunk");
    }

    /* clear the removed port from DB */
    CPSS_PORTS_BMP_PORT_CLEAR_MAC(&trunkInfoPtr->localPortsBmp, portNum);

    /* Update Resilient DB */
    rc = prvCpssGenericTrunkCascadeDbMemberIndexGet(devNum, trunkInfoPtr, portNum, &memberIndex);
    if(rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
    }

    rc = prvCpssGenericTrunkResilientHashDbMemberRemove(devNum,
            trunkInfoPtr,
            memberIndex);
    if(rc != GT_OK)
    {
    ...
    /* set the HW with the 'Cascade trunk' info */
    rc = trunkCascadeSet(devNum,trunkId,&trunkInfoPtr->localPortsBmp,
            0/*relevant to weighted trunk only*/,
            NULL/*relevant to weighted trunk only*/);
...
/**
* @internal prvCpssGenericTrunkCascadePortAdd function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         This function add port to the cascade trunk in the device.
*         Valid only if the trunk policy mode is RESILIENT.
*         If member is already in this trunk , function do nothing and
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssGenericTrunkCascadePortAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    /* this API supported only for cascade trunks */
    TRUNK_TYPE_SUPPORT_CASCADE_CHECK_MAC(trunkInfoPtr);
    ...
    {
        CHECK_PHY_PORT_BY_MASK_MAC(devNum, portNum);
    }
    else
    {
        /* device not support 'virtual port' only physical ports. */
    ...
    if(GT_OK != prvCpssTrunkMemberInMultipleTrunksEnableSearch(devNum, &trunkMember))
    {
        if(PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->portTrunkIdArray[portNum] != 0 &&
                PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->portTrunkIdArray[portNum] != trunkId)
        {
            /* already exists in another trunk */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_ALREADY_EXIST, LOG_ERROR_NO_MSG);
        }
    ...
    if (CPSS_PORTS_BMP_IS_PORT_SET_MAC(&trunkInfoPtr->localPortsBmp, portNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_ALREADY_EXIST, LOG_ERROR_NO_MSG);
    }

    /* set the added port in DB */
    CPSS_PORTS_BMP_PORT_SET_MAC(&trunkInfoPtr->localPortsBmp, portNum);

    rc = trunkDbCascadePortResilientDbAdd(devNum, trunkInfoPtr, portNum);
    if (rc != GT_OK)
    {
        return rc;
    }
