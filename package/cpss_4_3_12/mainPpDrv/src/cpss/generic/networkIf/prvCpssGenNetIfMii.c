...
{
    GT_STATUS rc;
    CPSS_EXTDRV_ETH_TX_MODE_ENT cpssTxMode;

    switch (txMode)
    {
        case PRV_CPSS_GEN_NETIF_MII_TX_MODE_ASYNCH_E:
            cpssTxMode = CPSS_EXTDRV_ETH_TX_MODE_ASYNC_E;
            break;
        case PRV_CPSS_GEN_NETIF_MII_TX_MODE_SYNCH_E:
            cpssTxMode = CPSS_EXTDRV_ETH_TX_MODE_SYNC_E;
            break;
    default:
    ...
    rc = cpssExtDrvEthPortTxModeSet(cpssTxMode);
    if (rc != GT_OK)
    {
        return rc;
    }
