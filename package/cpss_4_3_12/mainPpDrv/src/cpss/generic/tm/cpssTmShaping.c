...
/**
* @internal internal_cpssTmShapingProfileByIndexCreate function
* @endinternal
*
* @brief   Create a Shaping Profile at specified index
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X.
*
...
* @param[in] profileStrPtr            - Shaping profile configuration struct pointer.
*
* @param[inout] profileIndPtr            - (pointer to) The created Shaping profile index.
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on hardware error.
* @retval GT_BAD_PARAM             - on wrong device number or wrong parameter value.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device.
*/
static GT_STATUS internal_cpssTmShapingProfileByIndexCreate
(
    IN    GT_U8                                   devNum,
    IN    CPSS_TM_LEVEL_ENT                       level,
    IN    CPSS_TM_SHAPING_PROFILE_PARAMS_STC      *profileStrPtr,
...
)
{
    ...
    prof_index = *profileIndPtr;

    prf.cir_bw = profileStrPtr->cirBw;
    prf.cbs = profileStrPtr->cbs;
    prf.eir_bw = profileStrPtr->eirBw;
    prf.ebs = profileStrPtr->ebs;
    ret = tm_create_shaping_profile_at_index(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                    (enum tm_level)level,
                                    &prf,
                                    &prof_index);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
/**
* @internal cpssTmShapingProfileByIndexCreate function
* @endinternal
*
* @brief   Create a Shaping Profile at specified index
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device.
*/
GT_STATUS cpssTmShapingProfileByIndexCreate
(
    IN    GT_U8                                   devNum,
    IN    CPSS_TM_LEVEL_ENT                       level,
    IN    CPSS_TM_SHAPING_PROFILE_PARAMS_STC      *profileStrPtr,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssTmShapingProfileByIndexCreate);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, level, profileStrPtr, profileIndPtr));

    rc = internal_cpssTmShapingProfileByIndexCreate(devNum, level, profileStrPtr, profileIndPtr);
