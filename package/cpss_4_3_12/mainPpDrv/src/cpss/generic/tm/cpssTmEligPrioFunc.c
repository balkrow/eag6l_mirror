...
)
{
    ...
    ret = tm_elig_prio_func_config(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (uint16_t)eligPrioFuncPtr,(enum tm_level) level, &prms);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
/**
* @internal internal_cpssTmEligPrioFuncDump function
* @endinternal
*
* @brief   Dump Eligible priority function.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X.
*
...
* @param[in] funcIndex             - function index.
* @param[in] detailed              - detailed or short dump
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on hardware error.
* @retval GT_BAD_PARAM             - on wrong device number or wrong parameter value.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device.
*/
GT_STATUS internal_cpssTmEligPrioFuncDump
(
    IN  GT_U8               devNum,
    IN  CPSS_TM_LEVEL_ENT   level,
    IN  GT_U32              funcIndex,
...
)
{
    ...
    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        CPSS_XCAT3_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E |
        CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN_E | CPSS_AC3X_E);

    ret = tm_dump_elig_func(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (int) level,
                            (uint16_t)funcIndex, (int) detailed);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
/**
* @internal cpssTmEligPrioFuncDump function
* @endinternal
*
* @brief   Dump Eligible priority function.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device.
*/
GT_STATUS cpssTmEligPrioFuncDump
(
    IN  GT_U8               devNum,
    IN  CPSS_TM_LEVEL_ENT   level,
    IN  GT_U32              funcIndex,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssTmEligPrioFuncDump);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, level, funcIndex, detailed));

    rc = internal_cpssTmEligPrioFuncDump(devNum, level, funcIndex, detailed);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, level, funcIndex, detailed));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
