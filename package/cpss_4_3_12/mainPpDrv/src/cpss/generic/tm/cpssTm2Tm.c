...
)
{
    ...
    ret = tm2tm_port_channel_config(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                    ...
                                    (enum tm_level )srcLevel,
                                    1, /* TM2TM_BP */
                                    (enum tm_level)bpLevel,
                                    (uint16_t)bpOffset,
                                    (uint8_t)bpXon,
                                    (uint8_t)bpXoff);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
)
{
    ...
    ret = tm_set_dp_query_resp_lvl(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                   (uint8_t)dpType,
                                   (uint8_t)portDp,
                                   (enum tm_level)localLevel,
                                   (enum tm2tm_channel)remoteLevel);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
)
{
    int         ret = 0;
    GT_STATUS   rc = GT_OK;
    enum tm_dp_source src = (enum tm_dp_source)*dpSourcePtr;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);

    ret = tm2tm_set_egress_drop_aqm_mode(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                         (enum tm_level)level,
                                         (enum tm_color_num)colorNum,
                                         &src);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
)
{
    ...
    ret = tm2tm_set_ingress_drop_aqm_mode(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (enum tm2tm_channel)channel,(enum tm_color_num)colorNum);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
