...
)
{
    ...
    regData.l[0]=0;
    regData.l[1]=0;
    regAdd.l[0] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.PortRangeMap.l[0]+(portIndex*8);
    regAdd.l[1] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.PortRangeMap.l[1];
    ...
    for (ic = portFirstChild ; ic<=portLastChild ; ic++)
    {
        ...
        regData.l[0]=0;
        regData.l[1]=0;
        regAdd.l[0] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ClvltoPortAndBlvlRangeMap.l[0]+(ic*8);
        regAdd.l[1] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ClvltoPortAndBlvlRangeMap.l[1];
        ...
        for (ib = cFirstChild ; ib <= cLastChild ; ib++)
        {
            ...
            regData.l[0]=0;
            regData.l[1]=0;
            regAdd.l[0] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.BLvltoClvlAndAlvlRangeMap.l[0]+(ib*8);
            regAdd.l[1] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.BLvltoClvlAndAlvlRangeMap.l[1];
            rc = cpssTmCtlReadRegister( devNum, &regAdd, &regData);
            if (rc)
            {
                return rc;
            ...
            for (ia = bFirstChild ; ia <= bLastChild ; ia++)
            {
                ...
                regData.l[0]=0;
                regData.l[1]=0;
                regAdd.l[0] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ALvltoBlvlAndQueueRangeMap.l[0]+(ia*8);
                regAdd.l[1] =PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ALvltoBlvlAndQueueRangeMap.l[1];
                rc = cpssTmCtlReadRegister( devNum, &regAdd, &regData);
                if (rc)
                {
                    return rc;
...
)
{
    ...
    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
    CPSS_XCAT3_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
...
)
{
    ...
    *numOfLadsPtr = regData.l[0];
    ...
    /* minPkgSize */
    rc = cpssTmCtlReadRegister(devNum, &(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).QMgr.MinPkgSize), &regData);
    if (rc) {
        return rc;
    }
    ...
    /* numOfPagesPerBank */
    rc = cpssTmCtlReadRegister(devNum, &(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).QMgr.NumOfPagesPerBank), &regData);
    if (rc) {
        return rc;
    }
    ...
    /* numOfPkgesPerBank */
    rc = cpssTmCtlReadRegister(devNum, &(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).QMgr.NumOfPkgsPerBank), &regData);
    if (rc) {
        return rc;
    }
    ...
    readReg.l[0] = PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).QMgr.PortChunksEmitPerSel.l[0] + (191*8);
    readReg.l[1] = PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).QMgr.PortChunksEmitPerSel.l[1];
