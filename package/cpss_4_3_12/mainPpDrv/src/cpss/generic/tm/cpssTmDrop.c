...
/**
* @internal internal_cpssTmDropWredCurveByIndexCreate function
* @endinternal
*
* @brief   Create a WRED curve at a specific index for a TM level.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X.
*
...
* @param[in] probabilityArr[CPSS_TM_DROP_PROB_ARR_SIZE_CNS] - Array of 32 probability points in % (APPLICABLE RANGES: 0..100).
*
* @param[inout] curveIndexPtr            - (pointer to) The created WRED curve index (APPLICABLE RANGES: 0..7).
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on hardware error.
* @retval GT_BAD_PARAM             - on wrong device number or wrong parameter value.
...
*
*/
static GT_STATUS internal_cpssTmDropWredCurveByIndexCreate
(
    IN    GT_U8                   devNum,
    IN    CPSS_TM_LEVEL_ENT       level,
    IN    GT_U32                  cos,
    IN    GT_U32                  probabilityArr[CPSS_TM_DROP_PROB_ARR_SIZE_CNS],
    INOUT GT_U32                  *curveIndexPtr
)
{
    int         ret = 0;
    GT_STATUS   rc = GT_OK;
    ...
    curve_index = *curveIndexPtr;

    for(i = 0; i < 32; i++)
        prob_t[i] = (uint8_t)probabilityArr[i];
    ret = tm_create_wred_curve_at_index(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                               (enum tm_level)level,
                               (uint8_t)cos,
                               prob_t,
                               &curve_index);
...
/**
* @internal cpssTmDropWredCurveByIndexCreate function
* @endinternal
*
* @brief   Create a WRED curve at a specific index for a TM level.
*
...
* @param[in] probabilityArr[CPSS_TM_DROP_PROB_ARR_SIZE_CNS] - Array of 32 probability points in % (APPLICABLE RANGES: 0..100).
*
* @param[inout] curveIndexPtr            - (pointer to) The created WRED curve index.
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on hardware error.
* @retval GT_BAD_PARAM             - on wrong device number or wrong parameter value.
...
*
*/
GT_STATUS cpssTmDropWredCurveByIndexCreate
(
    IN    GT_U8                   devNum,
    IN    CPSS_TM_LEVEL_ENT       level,
    IN    GT_U32                  cos,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssTmDropWredCurveByIndexCreate);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, level, cos, probabilityArr, curveIndexPtr));

    rc = internal_cpssTmDropWredCurveByIndexCreate(devNum, level, cos, probabilityArr, curveIndexPtr);
...
/**
* @internal internal_cpssTmDropWredTraditionalCurveByIndexCreate function
* @endinternal
*
* @brief   Create a WRED traditional curve at a specific index for a level.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X.
*
...
*
*/
static GT_STATUS internal_cpssTmDropWredTraditionalCurveByIndexCreate
(
    IN    GT_U8                   devNum,
    IN    CPSS_TM_LEVEL_ENT       level,
    IN    GT_U32                  cos,
...
)
{
    ...
    ret = tm_create_wred_traditional_curve_at_index(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                           (enum tm_level)level,
                                           (uint8_t)cos,
                                           (float)maxProbability,
                                           &curve_index);
...
/**
* @internal cpssTmDropWredTraditionalCurveByIndexCreate function
* @endinternal
*
* @brief   Create a WRED traditional curve at a specific index for a level.
*
...
*
*/
GT_STATUS cpssTmDropWredTraditionalCurveByIndexCreate
(
    IN    GT_U8                   devNum,
    IN    CPSS_TM_LEVEL_ENT       level,
    IN    GT_U32                  cos,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssTmDropWredTraditionalCurveByIndexCreate);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, level, cos, maxProbability, curveIndexPtr));

    rc = internal_cpssTmDropWredTraditionalCurveByIndexCreate(devNum, level, cos, maxProbability, curveIndexPtr);
...
* @endinternal
*
* @brief   Create Drop profile at specified index.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Aldrin; AC3X.
*
...
* @param[in] profileStcPtr            - Drop Profile configuration structure pointer.
*
* @param[inout] profileIndexPtr          - (pointer to) The created Drop profile index
*                                      (APPLICABLE RANGES: 0..2048  for Queues,
*                                      0..255   for Anodes,
*                                      0..63    for Bnodes,Cnodes,
*                                      0..number of ports for Ports).
...
*
*/
static GT_STATUS internal_cpssTmDropProfileByIndexCreate
(
    IN    GT_U8                                   devNum,
    IN    CPSS_TM_LEVEL_ENT                       level,
    IN    GT_U32                                  cos,
    IN    CPSS_TM_DROP_PROFILE_PARAMS_STC         *profileStcPtr,
    INOUT GT_U32                                  *profileIndexPtr
)
{
    int         ret = 0;
    GT_STATUS   rc = GT_OK;
    ...
    prof_index = (uint16_t) *profileIndexPtr;
    ...
    ret = tm_create_drop_profile_at_index(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle,
                                 (enum tm_level)level,
                                 (uint8_t)cos,
                                 &prf,
                                 &prof_index);
...
*
*/
GT_STATUS cpssTmDropProfileByIndexCreate
(
    IN    GT_U8                                   devNum,
    IN    CPSS_TM_LEVEL_ENT                       level,
    IN    GT_U32                                  cos,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssTmDropProfileByIndexCreate);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, level, cos, profileStcPtr, profileIndexPtr));

    rc = internal_cpssTmDropProfileByIndexCreate(devNum, level, cos, profileStcPtr, profileIndexPtr);
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
    ret = tm_set_drop_color_num(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (enum tm_level)level,(enum tm_color_num)number);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
    ret = tm_get_drop_color_num_and_resolution(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (enum tm_level)level, &color_num , &resolution);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
    if (rc==0)
    {
...
)
{
    ...
    ret = tm_dp_source_set(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (enum tm_level)level, (uint8_t)color, (enum tm_dp_source)source);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
...
)
{
    ...
    ret = tm_dp_source_get(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (enum tm_level)level, (uint8_t)color, &dp_source);
...
)
{
    ...
    ret = tm_set_drop_query_responce(PRV_CPSS_PP_MAC(devNum)->tmInfo.tmHandle, (uint8_t)portDp, (enum tm_level)level);
    rc = XEL_TO_CPSS_ERR_CODE(ret);
