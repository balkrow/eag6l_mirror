...
)
{
	...
	*dataHigh = maskLow;
    ...
    CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, TM_GLOBAL_REG(0), TM_BASE_HIGH(bapId), MASK_ALL_BITS));

	/* Write don't care data */
    CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, TM_GLOBAL_REG(1), PARAM_NOT_CARE, MASK_ALL_BITS));

	/* Read LSB */
    CHECK_STATUS(hwsRegisterGetFuncPtr(devNum, portGroup, TM_DATA_0_REG(regAddr), dataLow, maskLow));

	/* Read MSB */
    CHECK_STATUS(hwsRegisterGetFuncPtr(devNum, portGroup, TM_DATA_1_REG(regAddr), dataHigh, maskHigh));
...
)
{
	...
	/* Write data LSB */
    CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, TM_DATA_0_REG(regAddr), dataLow, maskLow));

	/* Write data MSB */
    CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, portGroup, TM_DATA_1_REG(regAddr), dataHigh, maskHigh));
...
)
{
	...
	for(interfaceId = 0; interfaceId < MAX_INTERFACE_NUM; interfaceId++)
	{
		...
		{
			csMask[interfaceId] &= ~((GT_U32)(topologyMap->interfaceParams[interfaceId].asBusParams[busCnt].csBitmask));
		}
	}
