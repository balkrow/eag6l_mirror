...
)
{
    ...
    */
    ...
    if ((portMode == SGMII) || HWS_1000BASE_MODE_CHECK(portMode) || HWS_2500_MODE_CHECK(portMode))
    {
...
static GT_STATUS hwsAldrinSerdesIfReInit
(
    IN GT_U8 devNum
)
{
...
)
{
    ...
    CHECK_STATUS(hwsAldrinSerdesIfReInit(devNum));
...
)
{
    ...
    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*ALDRIN_SERDES_MAP_ARR_SIZE);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr,aldrinSerdesToAvagoMap,
        sizeof(GT_U32)*ALDRIN_SERDES_MAP_ARR_SIZE);

    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*ALDRIN_MAX_AVAGO_SERDES_NUMBER);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr,aldrinAvagoToSerdesMap,
        sizeof(GT_U32)*ALDRIN_MAX_AVAGO_SERDES_NUMBER);
