...
extern MV_HWS_ANP_PORT_DATA_STC mvHwsAnpPortDB;
extern GT_BOOL mvHwsAnpUsedForStaticPort;
#endif
static GT_STATUS mvHwsReplicationReset
(
    IN  GT_U8                           devNum,
    IN  GT_U32                          portGroup,
...
)
{
    ...
    if (mvHwsUsxModeCheck(devNum, portNum, portMode) == GT_TRUE)
    {
        ...
        for(i = firstIndex; i < iterNum; i++)
        {
            ...
            {
                usxExtField = IRONMAN_MTIP_MAC_PORT_EXT_PORT_MAC_CLOCK_AND_RESET_CONTROL_MAC_CLK_EN_E;
                CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, ((portNum & 0xFFFFFFF8) + i), MTI_10G_PORT_EXT_UNIT, usxExtField, &regData, NULL));
            }
            else
            {
...
{
    GT_STATUS rc;
    GT_U32 regData, localPortIndex;
    MV_HWS_HAWK_CONVERT_STC convertIdx;
#ifndef HARRIER_DEV_SUPPORT
    GT_U32 iterNum, groupStartPortNum, step, i,enaCounter = 0;
#endif
    GT_U32 usxExtField;
        ...
        for(i = 0; i < iterNum; i++)
        {
            ...
            if(regData != 0)
            {
                ...
                else
                {
                    if(HWS_DEV_SILICON_TYPE(devNum) == Ironman)
                    {
                        usxExtField = IRONMAN_MTIP_MAC_PORT_EXT_CONTROL_TX_LOC_FAULT_E;
                        CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, ((portNum & 0xFFFFFFF8) + i), MTI_10G_PORT_EXT_UNIT, usxExtField, &regData, NULL));
                    }
                    else
                    {
...
)
{
    ...
    MV_HWS_HAWK_CONVERT_STC convertIdx;
#ifndef HARRIER_DEV_SUPPORT
    GT_U32 *curMappingPtr;
    /* mac mapping jumps in OXGMII:
    Port 2(2),4(4),3(3),5(5),6(6),8(8),7(7),9(9)
    Port 10(10),12(12),11(11),13(13),14(14),16(16),15(15),17(17)
    Port 18(18),20(20),19(19),21(21),22(22),24(24),23(23),25(25)
    */
    GT_U32 octalMapping[8] = {0,2,1,3,4,6,5,7};
    /* mac mapping jumps in QXGMII      mac mapping jumps in DXGMII
    Port 2(2),4(4),3(3),5(5)            Port 2(2),4(4)
    Port 6(6),8(8),7(7),9(9)            Port 6(6),8(8)
    Port 10(10),12(12),11(11),13(13)    Port 10(10),12(12)
    Port 14(14),16(16),15(15),17(17)    Port 14(14),16(16)
    Port 18(18),20(20),19(19),21(21)    Port 18(18),20(20)
    Port 22(22),24(24),23(23),25(25)    Port 22(22),24(24)
    */
    GT_U32 quadAndDoubleMapping[8] = {0,2,1,3,0,2,1,3};
    GT_U32 firstIndex, iterNum, groupStartPortNum, i, step, enaCounter = 0;
#endif
    GT_U32 usxExtField;
    ...
    if (mvHwsUsxModeCheck(devNum, portNum, portMode) == GT_TRUE)
    {
        ...
        {
            firstIndex = 0;
            curMappingPtr = octalMapping;
        }
        else
        {
            firstIndex = convertIdx.ciderIndexInUnit % 8;
            firstIndex = firstIndex & 0x4; /* 0 or 4*/
            curMappingPtr = quadAndDoubleMapping;
        }
            ...
            */
            ...
            if(regData != 0)
            {
                ...
                else
                {
                    ...
                    {
                        usxExtField = MTIP_USX_EXT_UNITS_CONTROL_P0_TX_LOC_FAULT_E;
                        CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, (groupStartPortNum+curMappingPtr[i]), MTI_USX_EXT_UNIT, usxExtField, &regData, NULL));
                    }
                    if(regData == 0) /* if local fault is not configured - port was created */
                    {
                        enaCounter = 1;
...
)
{
    ...
    /* this patch added to prevent AAS MIF configuration   */
    /* this function called for AAS yet treated as Harrier */
...
)
{
    ...
    *fecTypePtr = FEC_OFF;
...
#endif
    {
        ...
        CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, curPortParams.portPcsNumber, MTI_CPU_RSFEC_UNIT,
                                              CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_FEC_CONTROL_FEC_ENABLE_E, &regData, NULL));
        if (regData == 0x1)
        {
            *fecTypePtr = FC_FEC;
            return GT_OK;
...
*/
#if defined (MICRO_INIT) || defined (IRONMAN_DEV_SUPPORT)
static /* needed for MI compilation ,without it we get next error:
Services/port/gop/src/port/silicon/ac5p/mvHwsAc5pPortCfgIf.c:1343:11: error: no previous prototype for 'mvHwsExtFecTypeSet' [-Werror=missing-prototypes]
*/
#endif /*MICRO_INIT*/
GT_STATUS mvHwsExtFecTypeSet
(
    IN GT_U8                       devNum,
    IN GT_U32                      portNum,
    IN MV_HWS_PORT_STANDARD        portMode,
...
*/
    ...
    for (i = 0; i < curPortParams.numOfActLanes; i++)
    {
        switch (fecType) {
            ...
            case FC_FEC:
                if ((portMode != _5GBaseR ) && (portMode != _12GBaseR) && (!HWS_10G_MODE_CHECK(portMode)) &&
...
*/
    ...
    if(HWS_1000BASE_MODE_CHECK(portMode) || HWS_2500_MODE_CHECK(portMode) || (portMode == SGMII) )
    {
        if (enable == GT_TRUE)
        {
            extField = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ?
...
* @param[in] portFullConfig  - the port place in the group
* @param[in] rxReset         - skip SD RX reset if false
*
* @retval 0                  - on success
* @retval 1                  - on error
*/
...
*/
    ...
    if (mvHwsUsxModeCheck(devNum, portNum, portMode))
    {
        ...
        /**
         * Port Shift issue ensure the SD_rx_reset
         * signal will remain in reset state till the RX training will
         * end.
         * If need to set in reset state - execute it, else do nothing.
        */
        if (regData == 0 || rxReset)
        {
            CHECK_STATUS(mvHwsPcsRxReset(devNum, 0, curPortParams.portPcsNumber, curPortParams.portPcsType, portMode, regData));
        }
    ...
    if(mvHwsMtipIsReducedPort(devNum, portNum) == GT_TRUE)
    {
        ...
        */
        if (rxReset)
        {
            CHECK_STATUS(mvHwsPcsRxReset(devNum, 0, curPortParams.portPcsNumber, curPortParams.portPcsType, portMode, 1));
        }
    ...
    * end.
    */
    if (rxReset || regData == 0)
    {
        CHECK_STATUS(mvHwsPcsRxReset(devNum, 0, curPortParams.portPcsNumber, curPortParams.portPcsType, portMode, ((regData == 0)? 0:1)));
    }
...
)
{
    ...
    if(mvHwsMtipIsReducedPort(devNum,portNum) == GT_TRUE)
    {
        if((FEC_NA == fecType) || (FEC_OFF == fecType))
        {
            ...
            cpuRsFecField = CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_FEC_CONTROL_FEC_ENABLE_E;
            CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, curPortParams.portPcsNumber, MTI_CPU_RSFEC_UNIT, cpuRsFecField, 0, NULL));
        }
        if(FC_FEC == fecType)
        {
    ...
    /*
        NOTE: FC_FEC configuration for AC5X/AC5P/Harrier for non-CPU and non-USX port modes:

            AC5P/Harrier:
                Field -         RSFEC_UNIT_FEC_CONTROL_P0_FEC_ENABLE_E
            AC5X:
                Field - PHOENIX_RSFEC_UNIT_FEC_CONTROL_P0_FEC_ENABLE_E

            Since these fields are OR with EXT configuration('gc_fec_ena') - so configuration not needed.
            Field 'gc_fec_ena' is configured in mvHwsExtFecTypeSet().
    */
...
/**
* @internal mvHwsExtForceLinkOkDisable function
* @endinternal
*
* @brief  force link_ok(link_ok_clean for Harrier) to 0x0 when link_ok_dis is set.
*
* @param[in] devNum                 - system device number
* @param[in] phyPortNum             - physical port number
* @param[in] portPcsType            - port pcs type
* @param[in] enable                 - enable/disable EXt unit force link_ok_dis bit
*
* @retval 0                         - on success
* @retval 1                         - on error
*/
static GT_STATUS mvHwsExtForceLinkOkDisable
(
    IN GT_U8                            devNum,
    IN GT_U32                           phyPortNum,
    IN MV_HWS_PORT_PCS_TYPE             portPcsType,
    IN GT_BOOL                          enable
)
{
    GT_U32                      regData;
    ...
    regData  = (enable == GT_TRUE) ? 0x1 : 0x0;

    switch(portPcsType)
    {
        case MTI_CPU_SGPCS:
            unitIdExt    = MTI_CPU_EXT_UNIT;
            extFieldName = MTIP_CPU_EXT_UNITS_CONTROL_FORCE_LINK_OK_DIS_E;
            break;
        ...
        case MTI_PCS_50:
            if(HWS_DEV_SILICON_TYPE(devNum) == AC5X)
            {
                ...
                extFieldName = PHOENIX_MTIP_EXT_UNITS_CONTROL1_P0_FORCE_LINK_OK_DIS_E;
            }
            else
            {
                unitIdExt    = MTI_EXT_UNIT;
                extFieldName = HAWK_MTIP_EXT_BR_UNITS_CONTROL_P0_FORCE_LINK_OK_DIS_E;
            }
            break;
        case MTI_USX_PCS_LOW_SPEED:
        case MTI_USX_PCS:
            unitIdExt    = MTI_USX_EXT_UNIT;
            extFieldName = MTIP_USX_EXT_UNITS_CONTROL1_P0_FORCE_LINK_OK_DIS_E;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
    }

    CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, phyPortNum, unitIdExt, extFieldName, regData, NULL));
...
)
{
    ...
    /* Un-Reset mtip */
    CHECK_STATUS(mvHwsExtReset(devNum, phyPortNum, portMode, GT_FALSE, stateInSerdesGroup, ((portInitInParamPtr->apEnabled) || (isANPStatic) || (portInitInParamPtr->lbPort))));

    if (( portInitInParamPtr->apEnabled == GT_TRUE ) || (isANPStatic))
    {
#ifndef HARRIER_DEV_SUPPORT
...
#endif
    /*  force link_ok to 0 for 1G WA implementaion */
    if ((portInitInParamPtr->autoNegEnabled == GT_TRUE) &&
        (HWS_USX_PORT_MODE_FAMILY_CHECK(portMode) || (portMode == SGMII) || (portMode == SGMII2_5)))
    {
        CHECK_STATUS(mvHwsExtForceLinkOkDisable(devNum,phyPortNum, curPortParams.portPcsType, GT_TRUE));
    }
...
)
{
    ...
    if (mvHwsUsxModeCheck(devNum, phyPortNum, portMode) || (portMode == SGMII) || (portMode == SGMII2_5) ||
        ((HWS_DEV_SILICON_TYPE(devNum) == Harrier) && (portMode == _10G_SXGMII || portMode == _5G_SXGMII || portMode == _2_5G_SXGMII)))
    {
        if((portMode != SGMII) && (portMode != QSGMII) && (portMode != SGMII2_5) &&
           (portMode != _5G_QUSGMII) && (portMode != _10G_OUSGMII))
        {
            CHECK_STATUS(mvHwsReplicationReset(devNum, 0, phyPortNum, portMode));
        }
        /*  WA: FE-4245079

            There is an option (in case of no PHY and inbandAN was enabled) to run link WA(in appDemo).
            and force link down was set. This causes the deadlock state.
            Therefore need to clean FORCE_LINK_DOWN config to '0'.
        */
        CHECK_STATUS(mvHwsExtForceLinkOkDisable(devNum,phyPortNum, curPortParams.portPcsType, GT_FALSE));
    }

    /* sd reset should be called before pcs reset (AN-154) */
    CHECK_STATUS(mvHwsExtReset(devNum, phyPortNum, portMode, GT_TRUE, stateInSerdesGroup, GT_FALSE));
...
)
{
    ...
    GT_U32                  fieldSgmiiSpeed = LPCS_UNITS_IF_MODE_P0_SGMII_SPEED_E;
    GT_U32                  fieldSgmiiPartnerAiblityRsv10 = LPCS_UNITS_PARTNER_ABILITY_P0_LP_PABILITY_RSV10_E;
    ...
    else if (HWS_DEV_SILICON_TYPE(devNum) == AC5X)
    {
        if (phyPortNum == MV_HWS_AC5X_GOP_PORT_CPU_0_CNS || phyPortNum == MV_HWS_AC5X_GOP_PORT_CPU_1_CNS)
        {
            ...
            fieldSgmiiPartnerAiblityRsv10 = CPU_SGPCS_UNIT_PORT_PARTNER_ABILITY_PORT_LP_PABILITY_RSV10_E;
        }
        else if (phyPortNum > MV_HWS_AC5X_GOP_PORT_CPU_1_CNS)
        {
            unitId = MTI_LOW_SP_PCS_UNIT;
        }
        else
    ...
    {
        if (MTI_CPU_SGPCS == curPortParams.portPcsType)
        {
            unitId = MTI_CPU_SGPCS_UNIT;
            fieldSgmiiSpeed = CPU_SGPCS_UNIT_PORT_IF_MODE_PORT_SGMII_SPEED_E;
            fieldSgmiiPartnerAiblityRsv10 = CPU_SGPCS_UNIT_PORT_PARTNER_ABILITY_PORT_LP_PABILITY_RSV10_E;
        }
        else if (MTI_PCS_LOW_SPEED == curPortParams.portPcsType)
        {
            unitId = MTI_LOW_SP_PCS_UNIT;
        }
        else
    ...
    case GT_TRUE:
        switch (portMode)
        {
            ...
            case QSGMII:
                 ...
                 *
                 */
                CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, phyPortNum, unitId, fieldSgmiiPartnerAiblityRsv10, &regData, NULL));
                data = (regData & 0x3);
                 ...
                 */
                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, phyPortNum, unitId, fieldSgmiiSpeed, data, NULL));
                return GT_OK;
        ...
        CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, phyPortNum, unitId, LPCS_UNITS_PARTNER_ABILITY_P0_LP_NP_E, &regData, NULL));

        /* return if Copper Link Status from PHY is down */
        if(regData == 0)
        {
            return GT_OK;
        }
...
{
    MV_HWS_UNITS_ID             unitId;
    MV_HWS_LPCS_UNITS_FIELDS_E  fieldNameGmii_2_5 = LPCS_UNITS_USXGMII_REP_P0_USXGMII2_5_E;
    MV_HWS_PORT_INIT_PARAMS     curPortParams;
    GT_BOOL                     isReplicationReadNeeded = GT_TRUE;
    GT_U32                      fieldSgmiiSpeed, fieldNameGmiiRep = LPCS_UNITS_USXGMII_REP_USXGMIIREP_E;
    ...
    switch(curPortParams.portPcsType)
    {
        ...
        case MTI_CPU_SGPCS:
            unitId =  MTI_CPU_SGPCS_UNIT;
            isReplicationReadNeeded = GT_FALSE;
            fieldSgmiiSpeed = CPU_SGPCS_UNIT_PORT_IF_MODE_PORT_SGMII_SPEED_E;
            break;
        case MTI_PCS_100:
        case MTI_PCS_50:
            if((HWS_DEV_SILICON_TYPE(devNum) == Harrier) &&
               (portMode == _10G_SXGMII || portMode == _5G_SXGMII || portMode == _2_5G_SXGMII ||
               (portMode == SGMII) || (portMode == QSGMII) || (portMode == _5G_QUSGMII) || (portMode == _10G_OUSGMII)))
            {
                unitId =  MTI_LOW_SP_PCS_UNIT;
                fieldSgmiiSpeed = LPCS_UNITS_IF_MODE_P0_SGMII_SPEED_E;
            }
            ...
            break;
        case MTI_PCS_10G_LPCS:
        case MTI_PCS_10G_BASE_R:
            if((portMode == _100Base_FX) || (portMode == _1000Base_X))
            {
                return GT_NOT_SUPPORTED;
            }
            if(curPortParams.portPcsType == MTI_PCS_10G_BASE_R)
            {
                switch(portMode)
                {
                    case HWS_SX_MODE_CASE:
            ...
            if((portMode == SGMII) || (portMode == QSGMII) || (portMode == _5G_QUSGMII) || (portMode == _10G_OUSGMII))
            {
                isReplicationReadNeeded = GT_FALSE;
            }
            unitId =  MTI_10G_PCS_LPCS_PORT_UNIT;
            fieldSgmiiSpeed = IRONMAN_LPCS_IF_MODE_SGMII_SPEED_E;
            fieldNameGmiiRep = IRONMAN_LPCS_USXGMII_REP_USXGMIIREP_E;
            fieldNameGmii_2_5 = (MV_HWS_LPCS_UNITS_FIELDS_E)IRONMAN_LPCS_USXGMII_REP_USXGMII2_5_E;
            break;
    ...
    *gmii_2_5Ptr = 0;
    ...
    if(isReplicationReadNeeded == GT_TRUE)
    {
...
#ifndef IRONMAN_DEV_SUPPORT
/**
* @internal mvHwsReplicationReset function
* @endinternal
*
...
#ifndef MV_HWS_FREE_RTOS
    if (hwsPpHwTraceFlag && hwsPpHwTraceEnablePtr != NULL) {
        hwsOsPrintf("****** Start of mvHwsReplicationReset ******\n");
    }
#endif
...
#ifndef MV_HWS_FREE_RTOS
    if (hwsPpHwTraceFlag && hwsPpHwTraceEnablePtr != NULL) {
        hwsOsPrintf("****** End of mvHwsReplicationReset ******\n");
    }
#endif
    return GT_OK;
}
...
)
{
    ...
    /* this patch added to prevent AAS MIF configuration   */
    /* this function called for AAS yet treated as Harrier */
    if (PRV_CPSS_SIP_7_CHECK_MAC(0/*devNum*/))
    {
        return GT_OK;
    }
...
GT_STATUS mvHwsAc5pPortDebugUnitDump
(
    GT_U32                  portNum,
    MV_HWS_PORT_STANDARD    portMode
)
{
    ...
    if (GT_OK != hwsPortModeParamsGetToBuffer(0, 0, portNum, portMode, &curPortParams))
    {
        return GT_BAD_PARAM;
    }
    ...
    {
        unitId = MTI_USX_EXT_UNIT;
        for(i = 0; i < MTIP_USX_EXT_UNITS_REGISTER_LAST_E; i++)
        {
        ...
        for(i = 0; i < MAC100_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum, unitId, portMode, i, &regData, NULL));
            if(i % 8 == 0)
            {
    ...
    else if((curPortParams.portMacType == MTI_MAC_100_BR) || (curPortParams.portMacType == MTI_MAC_100) || (curPortParams.portMacType == MTI_MAC_400))
    {
        unitId = MTI_EXT_UNIT;
        for(i = 0; i < HAWK_MTIP_EXT_BR_UNITS_REGISTER_LAST_E; i++)
        {
            MV_HWS_REG_ADDR_FIELD_STC fieldRegOut;
            GT_U32 address;
            MV_HWS_HAWK_CONVERT_STC convertIdx;

            CHECK_STATUS(mvHwsGlobalMacToLocalIndexConvert(0, portNum, portMode, &convertIdx));

            address = genUnitFindAddressPrv(0, portNum, unitId, portMode, i, &fieldRegOut);
            GT_UNUSED_PARAM(address);
            switch(fieldRegOut.regOffset)
            {
                case 0x5C:
                case 0x60:
                case 0x64:
        ...
        if((curPortParams.portMacType == MTI_MAC_400))
        {
            ...
            for(i = 0; i < MAC400_REGISTER_LAST_E; i++)
            {
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum, unitId, portMode, i, &regData, NULL));
                if(i % 8 == 0)
                {
    ...
    if((curPortParams.portPcsType == MTI_USX_PCS) || (curPortParams.portPcsType == MTI_USX_PCS_LOW_SPEED))
    {
        /*PCS_USX*/
        unitId = MTI_USX_PCS_UNIT;
        for(i = 0; i < USX_PCS_UNITS_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum, unitId, portMode, i, &regData, NULL));
            if(i % 8 == 0)
            {
        ...
        unitId = MTI_USX_LPCS_UNIT;
        for(i = 0; i < LPCS_UNITS_REGISTER_LAST_E; i++)
        {
        ...
        for(i = 0; i < USX_RSFEC_UNITS_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum, unitId, portMode, i, &regData, NULL));
            if(i % 8 == 0)
            {
        ...
        unitId = MTI_USX_MULTIPLEXER_UNIT;
        for(i = 0; i < USXM_UNITS_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum+8, unitId, portMode, i, &regData, NULL));
            if(i % 8 == 0)
            {
        ...
        /*ANP1*/
        unitId = ANP_USX_O_UNIT;
        for(i = 0; i < HAWK_ANP_UNITS_REGISTER_LAST_E; i++)
        {
        ...
        hwsOsPrintf("\n");
        /*AN1*/
        unitId = AN_USX_O_UNIT;
        for(i = 0; i < AN_UNITS_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitRegisterFieldGet(0, 0, portNum, unitId, i, &regData, NULL));
            if(i % 8 == 0)
            {
    ...
    {
        if((curPortParams.portPcsType == MTI_PCS_50))
        {
            unitId = MTI_PCS50_UNIT;
            for(i = 0; i < PCS50_REGISTER_LAST_E; i++)
            {
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum+8, unitId, portMode, i, &regData, NULL));
                if(i % 8 == 0)
                {
        ...
        else if((curPortParams.portPcsType == MTI_PCS_100))
        {
            unitId = MTI_PCS100_UNIT;
            for(i = 0; i < PCS100_REGISTER_LAST_E; i++)
            {
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum+8, unitId, portMode, i, &regData, NULL));
                if(i % 8 == 0)
                {
        ...
        else if((curPortParams.portPcsType == MTI_PCS_200))
        {
            unitId = MTI_PCS200_UNIT;
            for(i = 0; i < PCS200_UNITS_REGISTER_LAST_E; i++)
            {
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum+8, unitId, portMode, i, &regData, NULL));
                if(i % 8 == 0)
                {
        ...
        else if((curPortParams.portPcsType == MTI_PCS_400))
        {
            unitId = MTI_PCS400_UNIT;
            for(i = 0; i < PCS400_UNITS_REGISTER_LAST_E; i++)
            {
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum+8, unitId, portMode, i, &regData, NULL));
                if(i % 8 == 0)
                {
        ...
        else if((curPortParams.portPcsType == MTI_PCS_LOW_SPEED))
        {
            unitId = MTI_LOW_SP_PCS_UNIT;
            for(i = 0; i < LPCS_UNITS_REGISTER_LAST_E; i++)
            {
        ...
        for(i = 0; i < RSFEC_REGISTER_LAST_E; i++)
        {
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(0, 0, portNum, unitId, portMode, i, &regData, NULL));
            if(i % 8 == 0)
            {
