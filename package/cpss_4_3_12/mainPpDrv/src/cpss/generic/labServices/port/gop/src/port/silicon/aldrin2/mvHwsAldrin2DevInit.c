...
#ifdef SHARED_MEMORY
static GT_STATUS hwsAldrin2SerdesIfReInit
(
    IN GT_U8 devNum
)
{
...
)
{
    ...
    CHECK_STATUS(hwsAldrin2SerdesIfReInit(devNum));
...
)
{
    ...
    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*ALDRIN2_MAX_AVAGO_SERDES_NUMBER);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr,aldrin2SerdesToAvagoMap,
        sizeof(GT_U32)*ALDRIN2_MAX_AVAGO_SERDES_NUMBER);

    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*ALDRIN2_SERDES_MAP_ARR_SIZE);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr,aldrin2AvagoToSerdesMap,
        sizeof(GT_U32)*ALDRIN2_SERDES_MAP_ARR_SIZE);
