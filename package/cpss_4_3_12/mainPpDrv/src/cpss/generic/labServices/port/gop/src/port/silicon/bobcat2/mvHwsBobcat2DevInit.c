...
)
{
    ...
    {
        if ((HWS_2500_MODE_CHECK(portMode)) && (coreClk < 312))
        {
...
static GT_STATUS Getbc2BoardType(GT_U8 devNum, bc2BoardType *board)
{
    ...
    if (hwsDeviceSpecInfo[devNum].gopRev >= GOP_28NM_REV2)
    {
...
GT_STATUS hwsBobcat2IfInit(GT_U8 devNum, HWS_OS_FUNC_PTR *funcPtr)
{
    ...
    CHECK_STATUS(hwsBobcat2IfInitHwsDevFunc(devNum,funcPtr));
    hwsDeviceSpecGetFuncPtr(&hwsDevFunc);

        /* Init HWS Firmware */
    if (mvHwsServiceCpuEnableGet(devNum))
        CHECK_STATUS(mvHwsServiceCpuFwInit(devNum, SRVCPU_ID_DONT_CARE, 0, NULL));
