...
static GT_STATUS hwsBobKSerdesIfReInit
(
    IN GT_U8 devNum
)
{
  ...
  CHECK_STATUS(mvHwsAvagoCallbacksInit( hwsSerdesFuncsPtr));
...
)
{
    ...
    CHECK_STATUS(hwsBobKSerdesIfReInit(devNum));
...
GT_STATUS hwsBobKIfInit(GT_U8 devNum, HWS_OS_FUNC_PTR *funcPtr)
{
    ...
    GT_U32 bobkSerdesToAvagoMap[BOBK_MAX_AVAGO_SERDES_NUMBER] =
    {
      /*  [0] */ 15,
      /*  [1] */ 16,
      /*  [2] */ 17,
    ...
    GT_U32 bobkAvagoToSerdesMap[BOBK_SERDES_MAP_ARR_SIZE] =
    {
    /*    [0] */  0,
    /*    [1] */ 35,
    /*    [2] */ 34,
    ...
    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*BOBK_MAX_AVAGO_SERDES_NUMBER);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.serdesToAvagoMapPtr,bobkSerdesToAvagoMap,
        sizeof(GT_U32)*BOBK_MAX_AVAGO_SERDES_NUMBER);

    hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr = (GT_U32*) hwsOsMallocFuncPtr(sizeof(GT_U32)*BOBK_SERDES_MAP_ARR_SIZE);
    if(NULL==hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr)
    {
        return GT_OUT_OF_CPU_MEM;
    }

    hwsOsMemCopyFuncPtr(hwsDeviceSpecInfo[devNum].avagoSerdesInfo.avagoToSerdesMapPtr,bobkAvagoToSerdesMap,
        sizeof(GT_U32)*BOBK_SERDES_MAP_ARR_SIZE);
    ...
    if (mvHwsServiceCpuEnableGet(devNum))
    {
        ...
        /* Init HWS Firmware */
        res = mvHwsServiceCpuFwInit(devNum, SRVCPU_ID_DONT_CARE, 0, NULL /*"AP_BobK_MSYS"*/);
        if (res == GT_NO_SUCH)
        {
#ifndef ASIC_SIMULATION
            /* MSYS not available, fallback to CM3 */
            if (GT_OK != cpssSrvCpuSetOptionalFW(devNum, 0, AP_BOBK_CM3_FW_ID, "mvHwsServiceCpuCm3BobKFw") )
                return GT_FAIL;
#endif
            res = mvHwsServiceCpuFwInit(devNum, 0, 0, NULL);
        }
        if (res != GT_OK)
        {
            return res;
