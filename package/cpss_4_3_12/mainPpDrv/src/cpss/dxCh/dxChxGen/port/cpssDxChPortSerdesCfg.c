...
#define PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(hwsParam, cpssParam, defVal, cast)\
    hwsParam = (cast)((cpssParam == NA_16BIT) ?  defVal : cpssParam)

#define GET_ALL_PORTS 0xFFFF
#define SERDES_LANES_NUM 8

#define PRINT_TABLE_INFO_START(enable) \
    do { \
        if (enable == GT_TRUE) { \
            cpssOsPrintf("+------+------+--------+--------+-------+-------+\n" \
                   "| Port | Mac  | SerDes | SD Mux |Inv.Tx |Inv.Rx |\n" \
                   "+------+------+--------+--------+-------+-------+\n"); \
        } \
    } while(0)

#define PRINT_TABLE_INFO_NEW_LINE(enable) \
    do { \
        if (enable == GT_TRUE) { \
            cpssOsPrintf("+------+------+--------+--------+-------+-------+\n"); \
        } \
...
#define PRINT_TABLE_ROW_INFO(enable, rowNum, macNum, serdesNum, polarityTx, polarityRx) \
    do { \
        if (enable == GT_TRUE) { \
            cpssOsPrintf("| %4d | %4d |  %4d  |  %4d  | %5s | %5s |\n", \
                   ...
                   macNum, \
                   serdesNum, \
                   serdesNum % SERDES_LANES_NUM, \
                   (polarityTx) ? "TRUE" : "FALSE", \
                   (polarityRx) ? "TRUE" : "FALSE"); \
        } \
    } while(0)

extern GT_STATUS prvCpssDxChPortFakeSerdesEomMatrixGet
(
    IN   GT_U8                              devNum,
...
/**
* @internal internal_cpssDxChPortSerdesLaneSwappingModeSet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChPortSerdesLaneSwappingModeSet
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  GT_U8                   mode
)
{
    ...
    /* 98DXC254 and 98DX2556 don't support mode change */
    if (devType == CPSS_98DXC254_CNS ||
        devType == CPSS_98DX2556_CNS ||
        devType == CPSS_98DX2556M_CNS)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
    }
    ...
    /*Check which gop this port belongs to (valid only for QSGMII/QXGMII modes, ports 48-51 are NOT relevant )*/
    if ((portMacNum <= 7) || (portMacNum >= 24 && portMacNum <= 31))
    {
        gopNum = 0;
    }
    else if ((portMacNum <= 15) || (portMacNum >= 32 && portMacNum <= 39))
    {
        gopNum = 1;
    }
    else if ((portMacNum <= 23) || (portMacNum >= 40 && portMacNum <= 47))
    {
        gopNum = 2;
        /* 98DX2558 and CPSS_98DX2568 don't support mode change in this GoP */
        if (devType == CPSS_98DX2558_CNS  ||
            devType == CPSS_98DX2558M_CNS ||
            devType == CPSS_98DX2568_CNS  ||
            devType == CPSS_98DX2568M_CNS)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
        }
    }
    ...
    hwsDeviceSpecInfo[devNum].gopMuxMode[gopNum] = mode;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesLaneSwappingModeSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, mode));

    rc = internal_cpssDxChPortSerdesLaneSwappingModeSet(devNum, portNum, mode);
...
/**
* @internal internal_cpssDxChPortSerdesLaneSwappingModeGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChPortSerdesLaneSwappingModeGet
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    OUT  GT_U8                  *modePtr
)
{
    ...
    *modePtr  = hwsDeviceSpecInfo[devNum].gopMuxMode[gopNum];
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesLaneSwappingModeGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, modePtr));

    rc = internal_cpssDxChPortSerdesLaneSwappingModeGet(devNum, portNum, modePtr);
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_PM_NOT_APPLICABLE_DEVICE(devNum, CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                   - system device number
* @param[in] portNum                  - port  number
...
/**
* @internal internal_cpssDxChPortSerdesEomMatrixGet function
* @endinternal
*
* @brief   Returns eye values and prints the eye (optional)
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
static GT_STATUS internal_cpssDxChPortSerdesEomMatrixGet
(
    IN   GT_U8                              devNum,
    IN   GT_PHYSICAL_PORT_NUM               portNum,
    IN   GT_U32                             laneNum,
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AC5_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    CPSS_NULL_PTR_CHECK_MAC(eomInParams);
    CPSS_NULL_PTR_CHECK_MAC(eomOutParams);

    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacMap);
    rc = prvCpssCommonPortIfModeToHwsTranslate(devNum,PRV_CPSS_DXCH_PORT_IFMODE_MAC(devNum, portMacMap), PRV_CPSS_DXCH_PORT_SPEED_MAC(devNum, portMacMap), &portMode);
    ...
    /* API fails for baud rate 1.25G, delete after it fixed */
    if ( PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E )
    {
        if ( curPortParams.serdesSpeed == _1_25G )
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "1.25G SERDES speed not supported yet");
        }
    }
    ...
    cpssOsMemSet(eomOutParams, 0, sizeof(eomOutParams));

    if(CPSS_DEV_IS_WM_NATIVE_OR_ASIM_MAC(devNum) &&
       PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
    {
        /* CPSS-14649 : Aldrin3-M Serdes EOM return GT_OK in SIMICs */
        return prvCpssDxChPortFakeSerdesEomMatrixGet(devNum,portNum,laneNum,portParams.numActiveLanes,printEnable,eomInParams,eomOutParams);
    }

    CPSS_LOG_INFORMATION_MAC("Calling: mvHwsSerdesEomMatrixGet(devNum[%d], portNum[%d], laneNum[%d])", devNum, portNum, laneNum);

    rc = mvHwsSerdesEomMatrixGet(devNum, MV_HWS_SERDES_NUM(portParams.activeLaneList[laneNum]), printEnable, eomInParams, eomOutParams);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
        return rc;
...
/**
* @internal cpssDxChPortSerdesEomMatrixGet function
* @endinternal
*
* @brief   Returns eye values and prints the eye (optional)
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS cpssDxChPortSerdesEomMatrixGet
(
    IN     GT_U8                              devNum,
    IN     GT_PHYSICAL_PORT_NUM               portNum,
    IN     GT_U32                             laneNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesEomMatrixGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, laneNum, printEnable, eomInParams, eomOutParams));

    rc = internal_cpssDxChPortSerdesEomMatrixGet(devNum, portNum, laneNum, printEnable, eomInParams, eomOutParams);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, laneNum, printEnable, eomInParams, eomOutParams));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
*         Can be run only after port configured, not under traffic.
*
* @note   APPLICABLE DEVICES:      Lion2; Bobcat2; xCat3.
* @note   NOT APPLICABLE DEVICES:  Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman ; AC5.
*
* @param[in] devNum                   - system device number
* @param[in] portNum                  - number of physical port all serdeses occupied by it, in
*                                      currently configured interface mode will be configured
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_AC5_E);
...
)
{
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(devNum))
    {
        ...
        PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.lowFrequency,   serdesRxCfgPtr->rxTune.avago.LF, NA_8BIT, GT_U8);
        PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.highFrequency,  serdesRxCfgPtr->rxTune.avago.HF, NA_8BIT, GT_U8);
        PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.bandWidth,      serdesRxCfgPtr->rxTune.avago.BW, NA_8BIT, GT_U8);
        PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.dcGain,         serdesRxCfgPtr->rxTune.avago.DC, NA_8BIT, GT_U8);
        PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.squelch,        serdesRxCfgPtr->rxTune.avago.sqlch, NA_16BIT, GT_U16);

        if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
        {
            PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.gainshape1,  serdesRxCfgPtr->rxTune.avago.gainshape1, NA_8BIT, GT_U8);
            PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.gainshape2,  serdesRxCfgPtr->rxTune.avago.gainshape2, NA_8BIT, GT_U8);
            PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.dfeGAIN,     serdesRxCfgPtr->rxTune.avago.dfeGAIN,    NA_8BIT, GT_U8);
            PRV_CPSS_DXCH_PORT_SERDES_CFG_CHECK_AND_SET(configParams.dfeGAIN2,    serdesRxCfgPtr->rxTune.avago.dfeGAIN2,   NA_8BIT, GT_U8);
            configParams.shortChannelEn = (GT_BOOL)(serdesRxCfgPtr->rxTune.avago.shortChannelEn);
        }
        else
        {
...
)
{
    ...
    *signalStatePtr = GT_FALSE; /* Init the value to avoid "non initialized" problem */
    ...
    if(!PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && ((CPSS_PORT_IF_MODE_QSGMII_USX_QUSGMII_CHECK_MAC(cpssIfMode)) ||
           (cpssIfMode == CPSS_PORT_INTERFACE_MODE_MLG_40G_10G_40G_10G_E)))
    {
        phyPortNum = phyPortNum & 0xFFFFFFFC;
    }

    rc = hwsPortModeParamsGetToBuffer(devNum, portGroup, phyPortNum, hwsPortMode, &curPortParams);
    if (GT_OK != rc)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    ...
    if ((PRV_CPSS_SIP_6_20_CHECK_MAC(devNum) && cpssDeviceRunCheck_onEmulator()))
    {
...
/**
* @internal internal_cpssDxChPortSerdesGrayCodeEnableSet
*           function
* @endinternal
*
...
* @note   NOT APPLICABLE DEVICES:  Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; ; Falcon; AC5P; AC5X; Ironman;
*
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number
* @param[in] laneNum                  - lane number
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*
* @note pemitted only when port manager is in Debug state
*/
GT_STATUS internal_cpssDxChPortSerdesGrayCodeEnableSet
(
    IN GT_U8                              devNum,
    IN GT_PHYSICAL_PORT_NUM               portNum,
    IN GT_U32                             laneNum,
...
)
{
    GT_STATUS                   rc;             /* return code */
    CPSS_DXCH_ACTIVE_LANES_STC  portParams;
    GT_BOOL                     portIsInDebugState;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_HARRIER_E | CPSS_AC5P_E);
    PRV_CPSS_DXCH_PM_IS_IN_DEBUG_STATE_CHECK_AND_RETURN_MAC(devNum, portNum, &portIsInDebugState);
    ...
    rc = mvHwsSerdesGrayCodeEnableSet(devNum, MV_HWS_SERDES_NUM(portParams.activeLaneList[laneNum]), enable);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
        return rc;
...
/**
* @internal cpssDxChPortSerdesGrayCodeEnableSet function
* @endinternal
*
* @brief   enable/disable gray code
*
...
* @note pemitted only when port manager is in Debug state
*/
GT_STATUS cpssDxChPortSerdesGrayCodeEnableSet
(
    IN GT_U8                              devNum,
    IN GT_PHYSICAL_PORT_NUM               portNum,
    IN GT_U32                             laneNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesGrayCodeEnableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, laneNum, enable));

    rc = internal_cpssDxChPortSerdesGrayCodeEnableSet(devNum, portNum, laneNum, enable);
...
/**
* @internal internal_cpssDxChPortSerdesGrayCodeEnableGet
*           function
* @endinternal
*
* @brief   enable/disable gray code
...
* @param[in] laneNum                  - lane number
*
* @param[out] enable                  - (pointer to)GT_TRUE = enable, GT_FALSE = disable
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters
* @retval GT_BAD_PTR               - lockPtr == NULL
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS internal_cpssDxChPortSerdesGrayCodeEnableGet
(
    IN  GT_U8                              devNum,
    IN  GT_PHYSICAL_PORT_NUM               portNum,
    IN  GT_U32                             laneNum,
...
)
{
    ...
    rc = mvHwsSerdesGrayCodeEnableGet(devNum, MV_HWS_SERDES_NUM(portParams.activeLaneList[laneNum]), enablePtr);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
        return rc;
...
/**
* @internal cpssDxChPortSerdesGrayCodeEnableGet function
* @endinternal
*
* @brief   enable/disable gray code
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS cpssDxChPortSerdesGrayCodeEnableGet
(
    IN  GT_U8                              devNum,
    IN  GT_PHYSICAL_PORT_NUM               portNum,
    IN  GT_U32                             laneNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesGrayCodeEnableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, laneNum, enablePtr));

    rc = internal_cpssDxChPortSerdesGrayCodeEnableGet(devNum, portNum, laneNum, enablePtr);
...
/**
* @internal internal_cpssDxChPortSerdesPreCodeEnableSet
*           function
* @endinternal
*
...
* @note pemitted only when port manager is in Debug state
*/
GT_STATUS internal_cpssDxChPortSerdesPreCodeEnableSet
(
    IN GT_U8                              devNum,
    IN GT_PHYSICAL_PORT_NUM               portNum,
    IN GT_U32                             laneNum,
...
)
{
    ...
    rc = mvHwsSerdesPreCodeEnableSet(devNum, MV_HWS_SERDES_NUM(portParams.activeLaneList[laneNum]), enable);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
        return rc;
...
/**
* @internal cpssDxChPortSerdesPreCodeEnableSet function
* @endinternal
*
* @brief   enable/disable pre code
*
...
* @note pemitted only when port manager is in Debug state
*/
GT_STATUS cpssDxChPortSerdesPreCodeEnableSet
(
    IN GT_U8                              devNum,
    IN GT_PHYSICAL_PORT_NUM               portNum,
    IN GT_U32                             laneNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesPreCodeEnableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, laneNum, enable));

    rc = internal_cpssDxChPortSerdesPreCodeEnableSet(devNum, portNum, laneNum, enable);
...
/**
* @internal internal_cpssDxChPortSerdesPreCodeEnableGet function
* @endinternal
*
* @brief   enable/disable pre code
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS internal_cpssDxChPortSerdesPreCodeEnableGet
(
    IN  GT_U8                              devNum,
    IN  GT_PHYSICAL_PORT_NUM               portNum,
    IN  GT_U32                             laneNum,
...
)
{
    ...
    rc = mvHwsSerdesPreCodeEnableGet(devNum, MV_HWS_SERDES_NUM(portParams.activeLaneList[laneNum]), enablePtr);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
        return rc;
...
/**
* @internal cpssDxChPortSerdesPreCodeEnableGet function
* @endinternal
*
* @brief   enable/disable pre code
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS cpssDxChPortSerdesPreCodeEnableGet
(
    IN  GT_U8                              devNum,
    IN  GT_PHYSICAL_PORT_NUM               portNum,
    IN  GT_U32                             laneNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesPreCodeEnableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, laneNum, enablePtr));

    rc = internal_cpssDxChPortSerdesPreCodeEnableGet(devNum, portNum, laneNum, enablePtr);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacMap);
...
)
{
    ...
    MV_HWS_SERDES_AUTO_TUNE_RESULTS_UNT results;      /* current serdes tuning values in HWS
                                                         format */
    MV_HWS_SERDES_RX_CONFIG_DATA_UNT    dbRxOverrideParams;   /* rx override params saved on serdes DB */
    GT_U32                              portMacMap;   /* number of mac mapped to this physical port */
    MV_HWS_PORT_STANDARD                portMode;
    MV_HWS_PORT_INIT_PARAMS             curPortParams;
    GT_U32                              i;
...
*/
    ...
    case COM_PHY_C12GP41P2V_SIP6_30:
        ...
        if (mvHwsSerdesDbRxConfigGet(devNum, portGroup, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[laneNum]),serdesType, curPortParams.serdesSpeed, SERDES_DB_OVERRIDE_ONLY, &dbRxOverrideParams) == GT_OK)
        {
            serdesTunePtr->rxTune.comphy_C12G.minCap = dbRxOverrideParams.rxComphyC12GP41P2V.minCap;
            serdesTunePtr->rxTune.comphy_C12G.maxCap = dbRxOverrideParams.rxComphyC12GP41P2V.maxCap;
            serdesTunePtr->rxTune.comphy_C12G.minRes = dbRxOverrideParams.rxComphyC12GP41P2V.minRes;
            serdesTunePtr->rxTune.comphy_C12G.maxRes = dbRxOverrideParams.rxComphyC12GP41P2V.maxRes;
        }
        else
        {
            serdesTunePtr->rxTune.comphy_C12G.minCap = 0xffff;
            serdesTunePtr->rxTune.comphy_C12G.maxCap = 0xffff;
            serdesTunePtr->rxTune.comphy_C12G.minRes = 0xffff;
            serdesTunePtr->rxTune.comphy_C12G.maxRes = 0xffff;
        }
    ...
    case COM_PHY_C28GP4X4:
        ...
        serdesTunePtr->rxTune.comphy_C28G.inxSumfMidpointAdatptiveEnLane = results.comphyC28GP4Results.rxComphyC28GP4.inxSumfMidpointAdatptiveEnLane;
        ...
        serdesTunePtr->rxTune.comphy_C28G.minCap                         = results.comphyC28GP4Results.rxComphyC28GP4.minCap;
        serdesTunePtr->rxTune.comphy_C28G.minCapN                        = results.comphyC28GP4Results.rxComphyC28GP4.minCapN;
        serdesTunePtr->rxTune.comphy_C28G.sumfBoostTargetC0              = results.comphyC28GP4Results.rxComphyC28GP4.sumfBoostTargetC0;
        serdesTunePtr->rxTune.comphy_C28G.sumfBoostTargetC1              = results.comphyC28GP4Results.rxComphyC28GP4.sumfBoostTargetC1;
        serdesTunePtr->rxTune.comphy_C28G.sumfBoostTargetC2              = results.comphyC28GP4Results.rxComphyC28GP4.sumfBoostTargetC2;
        serdesTunePtr->rxTune.comphy_C28G.midpointPhaseOs0               = results.comphyC28GP4Results.rxComphyC28GP4.midpointPhaseOs0;
        serdesTunePtr->rxTune.comphy_C28G.midpointPhaseOs1               = results.comphyC28GP4Results.rxComphyC28GP4.midpointPhaseOs1;
        serdesTunePtr->rxTune.comphy_C28G.midpointPhaseOs2               = results.comphyC28GP4Results.rxComphyC28GP4.midpointPhaseOs2;
        ...
        if (mvHwsSerdesDbRxConfigGet(devNum, portGroup, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[laneNum]),serdesType, curPortParams.serdesSpeed, SERDES_DB_OVERRIDE_ONLY, &dbRxOverrideParams) == GT_OK)
        {
            serdesTunePtr->rxTune.comphy_C28G.minCap1 = dbRxOverrideParams.rxComphyC28GP4.minCap1;
            serdesTunePtr->rxTune.comphy_C28G.maxCap1 = dbRxOverrideParams.rxComphyC28GP4.maxCap1;
            serdesTunePtr->rxTune.comphy_C28G.minRes1 = dbRxOverrideParams.rxComphyC28GP4.minRes1;
            serdesTunePtr->rxTune.comphy_C28G.maxRes1 = dbRxOverrideParams.rxComphyC28GP4.maxRes1;
        }
        else
        {
            serdesTunePtr->rxTune.comphy_C28G.minCap1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C28G.maxCap1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C28G.minRes1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C28G.maxRes1 = 0xffff;
        }
    ...
    case COM_PHY_C112GX4:
        ...
        if (mvHwsSerdesDbRxConfigGet(devNum, portGroup, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[laneNum]),serdesType, curPortParams.serdesSpeed, SERDES_DB_OVERRIDE_ONLY, &dbRxOverrideParams) == GT_OK)
        {
            serdesTunePtr->rxTune.comphy_C112G.minCap1 = dbRxOverrideParams.rxComphyC112G.minCap1;
            serdesTunePtr->rxTune.comphy_C112G.maxCap1 = dbRxOverrideParams.rxComphyC112G.maxCap1;
            serdesTunePtr->rxTune.comphy_C112G.minRes1 = dbRxOverrideParams.rxComphyC112G.minRes1;
            serdesTunePtr->rxTune.comphy_C112G.maxRes1 = dbRxOverrideParams.rxComphyC112G.maxRes1;
        }
        else
        {
            serdesTunePtr->rxTune.comphy_C112G.minCap1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C112G.maxCap1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C112G.minRes1 = 0xffff;
            serdesTunePtr->rxTune.comphy_C112G.maxRes1 = 0xffff;
        }
    ...
    case COM_PHY_N5XC56GP5X4:
        ...
        serdesTunePtr->rxTune.comphy_C56G.align90         = (GT_U32)results.comphyC56GResults.align90;
        serdesTunePtr->rxTune.comphy_C56G.align90AnaReg   = (GT_U32)results.comphyC56GResults.align90AnaReg;
...
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number
* @param[in] min_dly                  - Minimum LF value that
*                                      can be set on Serdes
*                                      (0...15)
* @param[in] max_dly                  - Maximum LF value that
*                                      can be set on Serdes
*                                      (0...15)
*
* @retval GT_OK                    - on success.
...
* @param[out] currSerdesDelayArr      - serdeses Delay.
* @param[out] serdesList              - port's serdeses arrray
* @param[out] numOfSerdeses           - serdeses number
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
...
* @param[in] devNum                   - system device number
* @param[in] portNum                  - physical port number
* @param[in] portMode                 - Define the configured
*       port mode
* @param[in] linkUp                   - indicates if the port in
*       link up state or not
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
...
)
{
    ...
    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppClampingResetAndStatusGetFunc = prvCpssDxChPortSerdesClampingValuesResetWrapper;
    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRxInitFunc = prvCpssDxChPortSerdesRxInitWrapper;
    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesSigDetMaskSet = prvcpssDxChPortSerdesSignalDetectMaskSet;
...
)
{
    ...
    *rxTuneStatusPtr = (CPSS_PORT_SERDES_AUTO_TUNE_STATUS_ENT)rxTuneStatusTemp;
    *txTuneStatusPtr = (CPSS_PORT_SERDES_AUTO_TUNE_STATUS_ENT)txTuneStatusTemp;
...
)
{
    ...
    rc = mvHwsPortCdrLockStatusGet(devNum, portGroupId, portMacMap, portMode, cdrLockPtr);
    if (GT_OK != rc)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
    }
...
/**
* @internal prvCpssDxChPortSerdesClampingValuesResetWrapper
*           function
* @endinternal
*
* @brief   reset clamping values if clamping status is false
*
* @note   APPLICABLE DEVICES:     AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES: None.
*
*/
GT_STATUS prvCpssDxChPortSerdesClampingValuesResetWrapper
(
    IN  GT_SW_DEV_NUM                  devNum,
    IN  GT_PHYSICAL_PORT_NUM           portNum,
    OUT GT_BOOL                        *clampingStatusPtr
)
{
    GT_STATUS            rc = GT_OK;
    GT_U32               portMacMap; /* number of mac mapped to this physical port */
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    CPSS_NULL_PTR_CHECK_MAC(clampingStatusPtr);
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacMap);
    portGroupId = PRV_CPSS_PHYSICAL_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(devNum, portMacMap);
    ...
    rc = mvHwsPortSerdesClampingValuesResetAndstatusGet(devNum, portGroupId, portMacMap, portMode, clampingStatusPtr);
    if (GT_OK != rc)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
    }
...
)
{
    ...
    {
        return mvHwsIronmanReplicationSet(devNum, portGroup, phyPortNum, portMode, portInitInParamPtr, linkUp );
    }
    else
    {
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        /* run callback*/
        if (portMacObjPtr != NULL && portMacObjPtr->macDrvMacAutonegSetFunc)
        {
            rc = portMacObjPtr->macDrvMacAutonegSetFunc(devNum,portNum,autoNegotiationPtr->inbandEnable,
                                      0x00080,CPSS_MACDRV_STAGE_PRE_E);
            if(rc!=GT_OK)
            {
                return rc;
            }
...
/**
* @internal prvCpssDxChPortSerdesRxInitWrapper function
* @endinternal
*
* @brief   perform rx_init
*
* @note   APPLICABLE DEVICES:     AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES: None.
*
*/
GT_STATUS prvCpssDxChPortSerdesRxInitWrapper
(
    IN  GT_U8                                   devNum,
    IN  GT_PHYSICAL_PORT_NUM                    portNum,
    IN  CPSS_PORT_SERDES_RX_INIT_MODE_ENT       rxInitMode,
    OUT CPSS_PORT_SERDES_AUTO_TUNE_STATUS_ENT   *rxInitStatusPtr

)
{
    return cpssDxChPortSerdesRxInit(CAST_SW_DEVNUM(devNum), portNum, rxInitMode, rxInitStatusPtr);
}

/**
* @internal internal_cpssDxChPortSerdesRxInit function
* @endinternal
*
* @brief   perform rx_init
*
...
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number
* @param[in] rxInitMode               - rx_init mode
*
* @param[out] rxInitStatusPtr         - rx_init status
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
static GT_STATUS internal_cpssDxChPortSerdesRxInit
(
    IN  GT_U8                                   devNum,
    IN  GT_PHYSICAL_PORT_NUM                    portNum,
    IN  CPSS_PORT_SERDES_RX_INIT_MODE_ENT       rxInitMode,
...
)
{
    ...
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacMap);
    CPSS_NULL_PTR_CHECK_MAC(rxInitStatusPtr);
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) )
    {
        if(CPSS_DEV_IS_WM_NATIVE_OR_ASIM_MAC(devNum))
        {
            *rxInitStatusPtr = CPSS_PORT_SERDES_AUTO_TUNE_PASS_E;
            return GT_OK;
        }
    }
    portGroupId = PRV_CPSS_PHYSICAL_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(
    ...
    for(i = 0; i <  portParams.numActiveLanes; i++)
    {
        CPSS_LOG_INFORMATION_MAC("Calling: mvHwsSerdesRxInit(devNum[%d], portGroup[%d], serdesNum[%d], rxInitMode[%d])", devNum, portGroupId, portParams.activeLaneList[i], rxInitMode);
        rc = mvHwsSerdesRxInit(
                            devNum,
                            portGroupId,
                            portParams.activeLaneList[i],
                            (MV_HWS_RX_INIT_MODE)rxInitMode,
                            (MV_HWS_AUTO_TUNE_STATUS*)rxInitStatusPtr);
        if(rc != GT_OK)
        {
            CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
            break;
...
/**
* @internal cpssDxChPortSerdesRxInit function
* @endinternal
*
* @brief   perform rx_init
*
...
* @retval GT_NOT_INITIALIZED       - if serdes was not intialized
*/
GT_STATUS cpssDxChPortSerdesRxInit
(
    IN  GT_U8                                   devNum,
    IN  GT_PHYSICAL_PORT_NUM                    portNum,
    IN  CPSS_PORT_SERDES_RX_INIT_MODE_ENT       rxInitMode,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesRxInit);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, rxInitMode, rxInitStatusPtr));

    rc = internal_cpssDxChPortSerdesRxInit(devNum, portNum, rxInitMode, rxInitStatusPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, rxInitMode, rxInitStatusPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal cpssDxChPortSerdesSignalDetectMaskSet function
* @endinternal
*
* @brief   Set port serdes signal detect mask.
*
...
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number
* @param[in] maskEnable               - mask signal detect state
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on fail
* @retval GT_BAD_PARAM             - on bad parameters
...
* @retval GT_NOT_SUPPORTED         - if interface not supported
*/
GT_STATUS internal_cpssDxChPortSerdesSignalDetectMaskSet
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    ...
    rc = mvHwsPortSignalDetectMask(devNum,0,portMacNum,portMode,maskEnable);
    if(GT_OK != rc)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
...
* @retval GT_NOT_SUPPORTED         - if interface not supported
*/
GT_STATUS cpssDxChPortSerdesSignalDetectMaskSet
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  GT_BOOL                 maskEnable
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortSerdesSignalDetectMaskSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, maskEnable));

    rc = internal_cpssDxChPortSerdesSignalDetectMaskSet(devNum, portNum, maskEnable);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, maskEnable));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal prvcpssDxChPortSerdesSignalDetectMaskSet function
* @endinternal
*
* @brief  serdes detect mask set
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; Ironman.
*
*/
GT_STATUS prvcpssDxChPortSerdesSignalDetectMaskSet
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  GT_BOOL                 maskEnable
...
{
    return cpssDxChPortSerdesSignalDetectMaskSet(CAST_SW_DEVNUM(devNum), portNum, maskEnable);
}

static GT_STATUS internal_cpssDxChPortPhysicalSerdesMappingsGet
(
    IN    GT_U8                     devNum,
    IN    GT_PHYSICAL_PORT_NUM      portNum,
...
)
{
    ...
    CPSS_DXCH_PORT_SERDES_MAP_INFO_STC *portMapInfoPtr;
    GT_U32 arrayCounter = 0, startIterator = 0, stopIterator = 0;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    CPSS_NULL_PTR_CHECK_MAC(infoArraySizePtr);
    CPSS_NULL_PTR_CHECK_MAC(portMapInfoInfoArray);

    stopIterator = MIN(*infoArraySizePtr, PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(devNum));

    if (*infoArraySizePtr < 1)
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);

    if (portNum != GET_ALL_PORTS)
        startIterator = stopIterator = portNum;

    PRINT_TABLE_INFO_START(enablePrint);
    portMapInfoPtr = portMapInfoInfoArray;
    for (; startIterator <= stopIterator; startIterator++, portMapInfoPtr++, arrayCounter++)
    {
        rc = cpssDxChPortPhysicalPortMapGet(devNum, startIterator, 1, &portMap);
        if (rc != GT_OK || portMap.mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E)
            continue;

        portMapInfoPtr->portNum = startIterator;
        portMapInfoPtr->macNum = portMap.interfaceNum;
        if (hwsPortModeParamsGetToBuffer(devNum, 0, portMapInfoPtr->macNum, _10GBase_KR, &portParams) == GT_OK)
        {
            portMapInfoPtr->serdesNum = portParams.activeLanesList[0];
            rc = cpssDxChPortSerdesLanePolarityGet(
                                                    devNum,
                                                    0,
                                                    portMapInfoPtr->serdesNum,
                                                    &portMapInfoPtr->polarityTx,
                                                    &portMapInfoPtr->polarityRx);
            if (rc != GT_OK)
                continue;
        }

        PRINT_TABLE_ROW_INFO(enablePrint, startIterator,
                                    portMapInfoPtr->macNum,
                                    portMapInfoPtr->serdesNum,
                                    portMapInfoPtr->polarityTx,
                                    portMapInfoPtr->polarityRx);
        PRINT_TABLE_INFO_NEW_LINE(enablePrint);
    }

    *infoArraySizePtr = arrayCounter;
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortPhysicalSerdesMappingsGet);
    CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enablePrint, infoArraySizePtr, portMapInfoInfoArray));

    rc = internal_cpssDxChPortPhysicalSerdesMappingsGet(devNum, portNum, enablePrint, infoArraySizePtr, portMapInfoInfoArray);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, enablePrint, infoArraySizePtr, portMapInfoInfoArray));
    CPSS_API_UNLOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
