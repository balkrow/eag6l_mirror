...
/**
* @internal deviceSpecificPacketBufferClockCycleGet function
* @endinternal
*
* @brief   Get device specific packet buffer clock(in MHz).
*
...
*
*/
static GT_U32 deviceSpecificPacketBufferClockCycleGet
(
    IN  GT_U8  devNum
)
{
...
/**
* @internal cpssDxChPortPacketBufferAgingTimeoutSet function
* @endinternal
*
* @brief  Sets the packet buffer aging timeout period in milliseconds
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @param[in] devNum                   - device number
* @param[in] timeout                  - aging time (in milliseconds)
*                                       (APPLICABLE RANGES: 1000 ..5000)
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong device
* @retval GT_OUT_OF_RANGE          - aging timeout exceeds hardware limitation
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
*/
static GT_STATUS internal_cpssDxChPortPacketBufferAgingTimeoutSet
(
    IN GT_U8  devNum,
    IN GT_U32 timeout
)
{
    ...
    GT_U32 regAddr;
    GT_U32 data;
    GT_U32 clockCycle; /* PB device clock cycle in MHz */
    GT_U32 index; /* Register instances */
    ...
    clockCycle = deviceSpecificPacketBufferClockCycleGet(devNum);

    data = (timeout * 1000) * clockCycle;

    for(index = 0; index < 6; index++)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->sip6_packetBuffer.smbMc[index].cfgRefs;

        /* skip uninitialized register address */
...
*
*/
GT_STATUS cpssDxChPortPacketBufferAgingTimeoutSet
(
    IN GT_U8  devNum,
    IN GT_U32 timeout
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortPacketBufferAgingTimeoutSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, timeout));

    rc = internal_cpssDxChPortPacketBufferAgingTimeoutSet(devNum, timeout);
...
/**
* @internal internal_cpssDxChPortPacketBufferAgingTimeoutGet function
* @endinternal
*
* @brief  Get the packet buffer aging timeout period in milliseconds
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @param[in]  devNum                   - device number
* @param[out] timeout                  - (pointer to) aging time (in msec).
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong device
...
*
*/
static GT_STATUS internal_cpssDxChPortPacketBufferAgingTimeoutGet
(
    IN  GT_U8  devNum,
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->sip6_packetBuffer.smbMc[0].cfgRefs;
    ...
    *timeoutPtr = data / (clockCycle * 1000);

    return rc;
}
/**
* @internal cpssDxChPortPacketBufferAgingTimeoutGet function
* @endinternal
*
* @brief  Get the packet buffer aging timeout period in milliseconds
*
...
*
*/
GT_STATUS cpssDxChPortPacketBufferAgingTimeoutGet
(
    IN  GT_U8  devNum,
    OUT GT_U32 *timeoutPtr
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortPacketBufferAgingTimeoutGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, timeoutPtr));

    rc = internal_cpssDxChPortPacketBufferAgingTimeoutGet(devNum, timeoutPtr);
