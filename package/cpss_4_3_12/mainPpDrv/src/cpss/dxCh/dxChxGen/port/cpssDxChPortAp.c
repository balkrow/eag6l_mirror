...
{
    /* fcPause */  GT_TRUE,
    /* fcAsmDir */ CPSS_PORT_AP_FLOW_CONTROL_SYMMETRIC_E,
...
)
{
    ...
    {/*Lion2*/
        ...
        PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN2_E);
...
* @param[in] devNum                   - physical device number
* @param[in] portGroupNum             - number of port group (local core)
* @param[in] enabledPtr               - Pointer for the result
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port group or device
* @retval GT_BAD_PTR               - enabledPtr is NULL
         ...
         i++)
    {
        switch(modesAdvertiseArrPtr[i].ifMode)
        {
            ...
            case CPSS_PORT_INTERFACE_MODE_2500BASE_X_E:
                if((modesAdvertiseArrPtr[i].speed == CPSS_PORT_SPEED_2500_E) && (CPSS_PP_FAMILY_DXCH_IRONMAN_E == PRV_CPSS_PP_MAC(devNum)->devFamily))
                {
                    portMode = _2500Base_X;
                    AP_CTRL_2500Base_KX_SET(*modesVectorPtr, 1);
                }
                else
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
            ...
            case CPSS_PORT_INTERFACE_MODE_KR_E:
                ...
                else if((modesAdvertiseArrPtr[i].speed == CPSS_PORT_SPEED_12000_E) && (CPSS_PP_FAMILY_DXCH_IRONMAN_E == PRV_CPSS_PP_MAC(devNum)->devFamily))
                {
                    apCfgPtr->fecReq = fecRequired;
                    apCfgPtr->fecSup = fecSupported;
                    portMode = _12GBaseR;
                    AP_CTRL_100GBASE_CR10_SET(*modesVectorPtr, 1);
                }
            ...
            case CPSS_PORT_INTERFACE_MODE_KR4_E:
                ...
                else if (CPSS_PORT_SPEED_107G_E == modesAdvertiseArrPtr[i].speed) {
                    portMode = _100GBase_KR4;
                    apCfgPtr->specialSpeeds = 2;
                    AP_CTRL_100GBase_KR4_SET(*modesVectorPtr, 1);
                }
...
(
    ...
    IN  GT_BOOL                         skipRes,
    IN  GT_BOOL                         extSigDetCallbackEnable
)
{
    GT_STATUS                rc = GT_OK;        /* return code */
    ...
    GT_BOOL isPortManagerEnabled = GT_FALSE;

    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_PM_NOT_APPLICABLE_DEVICE(devNum, CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_AAS_E);
    ...
    apCfg.extSigDetCallbackEnable = extSigDetCallbackEnable;
    ...
    apCfg.fcAsmDir  = (GT_BOOL)localApParamsPtr->fcAsmDir;
    ...
    if ((PRV_CPSS_SIP_5_15_CHECK_MAC(devNum)) && (!PRV_CPSS_SIP_6_30_CHECK_MAC(devNum)))
    {
...
#endif
        rc = internal_cpssPortManagerEnableGet(devNum, &isPortManagerEnabled);
        if(GT_OK != rc)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "cannot evaluate if port manager is enabled for port %d of device %d", rc);
        }

        if ((PRV_CPSS_SIP_5_20_CHECK_MAC(devNum)) && !(PRV_CPSS_SIP_6_CHECK_MAC(devNum)) && (!isPortManagerEnabled))
        {
...
)
{
    ...
    CPSS_NULL_PTR_CHECK_MAC(apParamsPtr);
    return prvCpssDxChPortApPortConfigSet(devNum, portNum, apEnable, apParamsPtr, 0, GT_FALSE, GT_FALSE);
}
/**
...
refClockFreq=%d,refClockSource=%d, fecAdvanceReq=0x%x, fecAdvanceAbil=0x%x\n",
    ...
    apParamsPtr->fcAsmDir = (CPSS_DXCH_PORT_AP_FLOW_CONTROL_ENT)apCfg.fcAsmDir;
    ...
    else
    {
        if (apCfg.modesVector != 0)
        {
            ...
            if(AP_CTRL_10GBase_KX4_GET(apCfg.modesVector))
            {
                ...
                apParamsPtr->modesAdvertiseArr[i].ifMode = CPSS_PORT_INTERFACE_MODE_XGMII_E;
                apParamsPtr->modesAdvertiseArr[i++].speed = CPSS_PORT_SPEED_10000_E;
            }
            ...
            if(AP_CTRL_2500Base_KX_GET(apCfg.modesVector))
            {
                apParamsPtr->fecRequestedArr[i] = (apParamsPtr->fecRequired)? CPSS_DXCH_PORT_FEC_MODE_ENABLED_E: CPSS_DXCH_PORT_FEC_MODE_DISABLED_E;
                apParamsPtr->fecAbilityArr[i] = (apParamsPtr->fecSupported)? CPSS_DXCH_PORT_FEC_MODE_ENABLED_E: CPSS_DXCH_PORT_FEC_MODE_DISABLED_E;
                apParamsPtr->modesAdvertiseArr[i].ifMode =
                    CPSS_PORT_INTERFACE_MODE_2500BASE_X_E;
                apParamsPtr->modesAdvertiseArr[i++].speed = CPSS_PORT_SPEED_2500_E;
            }
            ...
            if(AP_CTRL_100GBASE_CR10_GET(apCfg.modesVector))
            {
                if(CPSS_PP_FAMILY_DXCH_IRONMAN_E == PRV_CPSS_PP_MAC(devNum)->devFamily)
                {
                    ...
                    apParamsPtr->modesAdvertiseArr[i++].speed = CPSS_PORT_SPEED_12000_E;
                }
            }
...
hcdFecEn=%d,hcdFcPause=%d,hcdFcAsmDir=%d,postApPortMode=%d,postApPortNum=%d,preApPortNum=%d\n",
    ...
    if(((apStatusPtr->hcdFound = apResult.hcdResult.hcdFound) != GT_TRUE) && (speed != CPSS_PORT_SPEED_106G_E) && (speed != CPSS_PORT_SPEED_102G_E)  && (speed != CPSS_PORT_SPEED_42000_E) && (speed != CPSS_PORT_SPEED_53000_E) && (speed != CPSS_PORT_SPEED_107G_E))
    ...
    switch(apResult.postApPortMode)
    {
        case _1000Base_X:
            ...
            break;
        case _2500Base_X:
            apStatusPtr->portMode.ifMode = CPSS_PORT_INTERFACE_MODE_2500BASE_X_E;
            apStatusPtr->portMode.speed = CPSS_PORT_SPEED_2500_E;
            break;
        case _10GBase_KX4:
        ...
        case _12GBaseR:
            apStatusPtr->portMode.ifMode = CPSS_PORT_INTERFACE_MODE_KR_E;
            apStatusPtr->portMode.speed = CPSS_PORT_SPEED_12000_E;
            break;
        case _40GBase_KR4:
            ...
            else if (speed == CPSS_PORT_SPEED_47200_E)
            {
                apStatusPtr->portMode.speed = CPSS_PORT_SPEED_47200_E;
                apStatusPtr->portMode.ifMode = CPSS_PORT_INTERFACE_MODE_XHGS_E;
            }
            else
            {
        ...
        case _100GBase_KR4:
            ...
            if (speed == CPSS_PORT_SPEED_107G_E)
            {
                apStatusPtr->portMode.speed = CPSS_PORT_SPEED_107G_E;
            }
            else if(speed == CPSS_PORT_SPEED_106G_E)
            {
...
)
{
    ...
    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_BOBCAT2_E
                                          | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E);
...
* @endinternal
*
* @brief   Set AP port introp information. the Interop
*          information is per AP instance and not per port. The
*          port in the API is used to calculate the AP instance
*          to configure. except for Falcon in all devices we
*          have only 1 instance, hence the configuration will be
*          applied to all ports in the device. in Falcon we have
*          instance for each Raven so the configuration will be
*          applied to all ports in the calculated Raven/Instance
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; xCat3; AC5; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2.
*
...
)
{
        ...
        PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        CPSS_LOG_INFORMATION_MAC(
                            "Calling: mvApPortCtrlDebugInfoShow(phyPortNum[%d], output[%d])",
                            phyPortNum, output);
    ...
    {
        CPSS_LOG_INFORMATION_MAC(
                            "Calling: mvHwsAnpPortDump(phyPortNum[%d], output[%d])",
                            phyPortNum, output);

        rc = mvHwsAnpPortDump(devNum, phyPortNum, output);
    }

    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("mvHw: Hws return code is %d", rc);
    }
...
)
{
    ...
    /* sanity checks */
    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
* @param[in] apParamsPtr              - (ptr to) AP parameters for port
*                                      (NULL - for CPSS defaults).
* @param[in] portOperationBitmap      - Bitmap of ports to
*       configure
* @param[in] skipRes                  - Skip the AN flow and run
*       only KR training
* @param[in] extSigDetCallbackEnable  - Enable Signal detect
...
)
{
    ...
    if ((!PRV_CPSS_SIP_6_10_CHECK_MAC(devNum)) || (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))) {
        return prvCpssDxChPortApPortConfigSet(devNum, portNum, apEnable, (CPSS_DXCH_PORT_AP_PARAMS_STC *)apParamsPtr, portOperationBitmap, skipRes, extSigDetCallbackEnable);
    }
    else
    {
        PRV_CPSS_DXCH_PORT_NUM_OR_CPU_PORT_CHECK_AND_MAC_NUM_GET_MAC(devNum,
        ...
        else
        {
            ...
            apCfg.fcAsmDir  = (GT_BOOL)apParamsPtr->fcAsmDir;
...
)
{
    ...
    /* sanity checks */
    PRV_CPSS_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
* @param[in] currSerdesDelayArr       - serdeses Delay.
* @param[in] serdesList               - port's serdeses arrray
* @param[in] numOfSerdeses            - serdeses number
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
