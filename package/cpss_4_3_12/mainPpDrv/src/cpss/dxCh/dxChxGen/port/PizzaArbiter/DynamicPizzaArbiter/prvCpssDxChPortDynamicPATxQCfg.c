...
/**
********************************************************************************
* @file prvCpssDxChPortDynamicPATxQCfg.c
*
* @brief TXQ specific pizza algorithm
*
* @version   1
********************************************************************************
*/
...
#define PRV_SHARED_PA_DB_VAR(_var) _var
...
#define PRV_TXQ_FREE_SLICE_ARR_SIZE_CNS  MAX(PRV_CPSS_DXCH_BC2_PA_MAP_TOTAL_FLDS_REG_CNS,PRV_CPSS_DXCH_Aldrin_PA_TXQ_MAP_TOTAL_FLDS_REG_CNS)
...
typedef struct
{
    ...
    GT_U32               freeSlicesArr[PRV_TXQ_FREE_SLICE_ARR_SIZE_CNS]; /* MAx TXQ currently at Aldrin */
    TxQPortConfig_STC    portsConfigArray[CPSS_MAX_PORTS_NUM_CNS];
...
)
{
    ...
    cpssOsMemSet(portList->portsConfigArray,0,sizeof(TxQPortConfig_STC)*size);
...
)
{
    ...
    {
        portList->portsConfigArray[portIdx].portIdx = portIdx;
        portList->portsConfigArray[portIdx].speed = portsConfigArrayPtr[portIdx];
        portList->portsConfigArray[portIdx].numberOfSlicesAllocated = 0;
        portList->portsConfigArray[portIdx].interSliceGap = 0;

        if(portList->portsConfigArray[portIdx].speed != INVALID_SPEED_CNS)
        {
            portList->requiredBandWidth += portList->portsConfigArray[portIdx].speed;
        }
    }
...
static void PortListSortInDecreasingOrder(TxQPortConfigList_STC * portList)
{
    ...
    portsConfigArray = portList->portsConfigArray;
    /* sort */
    ...
    for (i = 0 ; portList->portsConfigArray[i].speed > 0; i++);
...
)
{
    ...
    *numberOfSlicesPtr = numberOfSlices;
    ...
    /*--------------------------------------*/
    portList->highSpeedPortNum = 0;
    if (portList->portNum <= CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS)  /* theoretically only high speed ports */
    {
        if (portList->portsConfigArray[0].numberOfSlicesToAllocate > 1)
        {
            for (portList->highSpeedPortNum = 0 ; portList->highSpeedPortNum < portList->portNum; portList->highSpeedPortNum++)
            {
                interSliceGap = numberOfSlices / portList->portsConfigArray[portList->highSpeedPortNum].numberOfSlicesToAllocate;
    ...
    {
        if (portList->portsConfigArray[portList->highSpeedPortNum].numberOfSlicesToAllocate > 1)
        {
    ...
    for (portIdx = portList->highSpeedPortNum ; portIdx < portList->portNum ; portIdx++)
    {
        ...
        {
            interSliceGap = numberOfSlices / portList->portsConfigArray[portIdx].numberOfSlicesToAllocate;
        }
        else /* just high speed ports present */
        {
            interSliceGap = 1;
        }
        portList->portsConfigArray[portIdx].interSliceGap = interSliceGap;
    }
    ...
    for (portIdx = portList->highSpeedPortNum ; portIdx < portList->portNum ; portIdx++)
    {
        portNum2Process = portList->portsConfigArray[portIdx].portIdx;

        for (idx = 0 ; idx < portList->portsConfigArray[portIdx].numberOfSlicesToAllocate; idx++)
        {
            ...
            numberOfFreeSlices --;
            #ifdef SLICE_LIST_DEBUG
                portList->portsConfigArray[portIdx].sliceList[portList->portsConfigArray[portIdx].numberOfSlicesAllocated] = sliceIdx;
            #endif
            portList->portsConfigArray[portIdx].numberOfSlicesAllocated++;


            sliceIdx += portList->portsConfigArray[portIdx].interSliceGap;
            if (sliceIdx >= numberOfSlices)
            {
                sliceIdx -= numberOfSlices;
            }
        }
        if (portList->portsConfigArray[portIdx].numberOfSlicesAllocated != portList->portsConfigArray[portIdx].numberOfSlicesToAllocate)
        {
            return 0xFFFF;
        }
        #ifdef SLICE_LIST_DEBUG
            SliceDistributionPrintPortHdr(stdout, portList->portsConfigArray[portIdx].portIdx, portList->portsConfigArray[portIdx].speed);
    ...
    for (portIdx = 0 ; portIdx < portList->highSpeedPortNum ;  portIdx++)
    {
        ...
        portList->portsConfigArray[portIdx].interSliceGap = (numberOfFreeSlices<<10)/portList->portsConfigArray[portIdx].numberOfSlicesToAllocate;
        startSliceIdx_Q10 = 0;
        for (idx = 0 ; idx < portList->portsConfigArray[portIdx].numberOfSlicesToAllocate; idx++)
        {
            ...
            #endif
            portList->portsConfigArray[portIdx].numberOfSlicesAllocated++;
            startSliceIdx_Q10 += portList->portsConfigArray[portIdx].interSliceGap;  /* already Q10 */
        }
    }
...
)
{
    ...
    if (numberOfSlices > sizeof(portList->freeSlicesArr)/sizeof(portList->freeSlicesArr[0]))
    {
        /* check size of portList->freeSlicesArr */
        return 1;
    }
    *numberOfSlicesPtr = numberOfSlices;
    ...
    if (portList->portNum <= CPSS_DXCH_PORT_HIGH_SPEED_PORT_NUM_CNS)  /* theoretically only high speed ports */
    {
        if (portList->portsConfigArray[0].numberOfSlicesToAllocate > 1)
        {
            for (portList->highSpeedPortNum = 0 ; portList->highSpeedPortNum < portList->portNum; portList->highSpeedPortNum++)
            {
                interSliceGap_Q20 = q20_div(numberOfSlices,portList->portsConfigArray[portList->highSpeedPortNum].numberOfSlicesToAllocate);
    ...
    for (portIdx = portList->highSpeedPortNum ; portIdx < portList->portNum ; portIdx++)
    {
        ...
        if (portList->portNum > portList->highSpeedPortNum)
        {
            interSliceGap_Q20 = q20_div(numberOfSlices,portList->portsConfigArray[portIdx].numberOfSlicesToAllocate);
        }
        else /* just high speed ports present */
        {
            interSliceGap_Q20 = 1*FIXP_MUL;
        }
        portList->portsConfigArray[portIdx].interSliceGap = interSliceGap_Q20;
    }
    ...
    for (portIdx = portList->highSpeedPortNum ; portIdx < portList->portNum ; portIdx++)
    {
        ...
        portNum2Process = portList->portsConfigArray[portIdx].portIdx;
        dist_q20        = portList->portsConfigArray[portIdx].interSliceGap;
        /*-------------------------------*/
        /* find first empty slice        */
        /*-------------------------------*/
        for (sliceIdx = 0 ; pizzaArray[sliceIdx] != INVALID_PORT_CNS; sliceIdx++);
        pizzaArray[sliceIdx] = portNum2Process;
        portList->portsConfigArray[portIdx].numberOfSlicesAllocated = 1;
        ...
        for (placeIdx = 1 ; placeIdx < portList->portsConfigArray[portIdx].numberOfSlicesToAllocate; placeIdx++)
        {
            GT_U32 idx;
            GT_U32 min_sliceIdx;
            GT_U32 min_dist_q20;
    ...
    for (portIdx = 0 ; portIdx < portList->highSpeedPortNum ;  portIdx++)
    {
        ...
        portList->portsConfigArray[portIdx].interSliceGap = (numberOfFreeSlices<<10)/portList->portsConfigArray[portIdx].numberOfSlicesToAllocate;
        startSliceIdx_Q10 = 0;
        for (placeIdx = 0 ; placeIdx < portList->portsConfigArray[portIdx].numberOfSlicesToAllocate; placeIdx++)
        {
            sliceIdx = portList->freeSlicesArr[startSliceIdx_Q10>>10];
            pizzaArray[sliceIdx] = portNum2Process;
            #ifdef SLICE_LIST_DEBUG
                portList->portsConfigArray[portIdx].sliceList[placeIdx] = sliceIdx;
            #endif
            portList->portsConfigArray[portIdx].numberOfSlicesAllocated++;
            startSliceIdx_Q10 += portList->portsConfigArray[portIdx].interSliceGap;  /* already Q10 */
        }
    ...
    )
    {
        ...
        for(portIdx = 0; portIdx < portList->portNum; portIdx++)
        {
            portNum = portList->portsConfigArray[portIdx].portIdx;
            /*----------------------------------*/
            ...
            if (cnt != portList->portsConfigArray[portIdx].numberOfSlicesToAllocate)
            {
                return 1;
            }
            if (portIdx >= portList->highSpeedPortNum)
            {
                ...
                /*-----------------------------------------*/
                if (portList->portsConfigArray[portIdx].numberOfSlicesToAllocate > 1)
                {
                    ...
                    for (cnt = 0; cnt < portList->portsConfigArray[portIdx].numberOfSlicesToAllocate; cnt ++)
                    {
                        ...
                        if (cnt == portList->portsConfigArray[portIdx].numberOfSlicesToAllocate -1)
                        {
...
)
{
    ...
    prevSpeed = (GT_U32)portList->portsConfigArray[portList->portNum-1].speed;
    if (portList->portNum >= 2)
    {
        for (portIdx = (GT_32)portList->portNum-2; portIdx >= 0; portIdx --)
        {
            curSpeed = (GT_U32)portList->portsConfigArray[portIdx].speed;
...
)
{
    ...
    TxQPortConfigList_STC   *portListPtr;
    ...
    PortListInit(portListPtr, /*&portsConfigArray[0],*/CPSS_MAX_PORTS_NUM_CNS);

    if (portsConfigArrayPtr[64] != 0)
    {
        if (PRV_SHARED_PA_DB_VAR(txQPort64Bandwidth) != 0)
        {
            portsConfigArrayPtr[64] = PRV_SHARED_PA_DB_VAR(txQPort64Bandwidth);
        }
    }

    PortListSet (portListPtr, portsConfigArrayPtr,portsConfigArraySize);


    if(portListPtr->requiredBandWidth > maxPipeSizeMbps)
    {
        GT_FLOAT64  requiredBandWidth = portListPtr->requiredBandWidth;
        /* to avoid compiler warning when macro below is empty */
        requiredBandWidth = requiredBandWidth;
        cpssOsFree(portListPtr);
        CPSS_LOG_ERROR_AND_RETURN_MAC(
            GT_FAIL, "\nrequired BandWidth(%f) > available BandWidth(%d) in MBPS\n",
            requiredBandWidth, maxPipeSizeMbps);
    }

    PortListSortInDecreasingOrder(portListPtr);
...
*/
    if ( GT_OK == checkConditionForOptimalDistribution(portListPtr,maxPipeSizeMbps))
    {
        rc = configPizza_OptimalDistribution(portListPtr,maxPipeSizeMbps,minSliceResolutionMpbs, highSpeedPortThreshPtr->exactAlgoHighSpeedPortThresh, pizzaArraySize, /*OUT*/pizzaArray, /*OUT*/&numberOfSlices);
        if (rc != GT_OK)
        {
            cpssOsFree(portListPtr);
            return rc;
    ...
    {
        rc = configPizza_apprAlgo(portListPtr,maxPipeSizeMbps,minSliceResolutionMpbs, highSpeedPortThreshPtr->apprAlgoHighSpeedPortThresh_Q20,pizzaArraySize, /*OUT*/pizzaArray, /*OUT*/&numberOfSlices);
        if (rc != GT_OK)
        {
            cpssOsFree(portListPtr);
            return rc;
    ...
    *numberOfHighSpeedPortsPtr = portListPtr->highSpeedPortNum;
    for (i = 0 ; i < portListPtr->highSpeedPortNum; i++)
    {
        highSpeedPortsArr[i] = portListPtr->highSpeedPortArr[i];
    }
    #if 0  /* #ifdef STAND_ALONE */
        if (0 != CheckConfiguration(portListPtr,pizzaArray, /*OUT*/numberOfSlices))
        {
            CheckConfiguration(portListPtr,pizzaArray, /*OUT*/numberOfSlices);
            cpssOsFree(portListPtr);
            return 1;
        }
    #endif
...
GT_STATUS setTxQ64BW(GT_U32 bw)
{
    ...
    PRV_SHARED_PA_DB_VAR(txQPort64Bandwidth) = (GT_FLOAT64)bw;
    return GT_OK;
}
    ...
    int main(void)
    {
        ...
        TestConfig(maxPipeSizeMbps,portsConfigArrayPtr,CPSS_MAX_PORTS_NUM_CNS);
