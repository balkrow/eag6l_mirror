...
)
{
    ...
    GT_U32    txqPortNum;
    GT_U32    loopbackPortOffset; /* first bit of Loopback Port field */
    ...
    if (PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
        loopbackPortOffset = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 1 : 2;
        mask   =  (PRV_CPSS_SIP_6_CHECK_MAC(devNum) ?
                   ((0x3FF << loopbackPortOffset) | 1) :   /* bits 11:2, 0 OR 10:1 */
                   ((0x1FF << loopbackPortOffset) | 1));   /* bits 10:2, 0  */

        value  = loopbackPortNum << loopbackPortOffset;  /* bits 10:2 (sip5.20) or 11:2(sip6)) or 10:1(sip7) */
...
)
{
    ...
    if (PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
        ...
        *loopbackPortNumPtr = (PRV_CPSS_SIP_6_CHECK_MAC(devNum) ?
                               ((value >> loopbackPortOffset) & 0x3FF) :  /* bits 11:2 OR 10:1*/
                               ((value >> loopbackPortOffset) & 0x1FF));  /* bits 10:2 */
    }
    else
    {
        /* SIP 5.0, SIP 5.1x */
...
)
{
    ...
    if (analyzerIndex > CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
