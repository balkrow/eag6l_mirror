...
/**
* @internal prvCpssDxChPortDpDebugRxToCpCountingPortConfigSet function
* @endinternal
*
* @brief   Set port configuration for counting Rx to Cp all packets.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X Bobcat3; Aldrin2.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChPortDpDebugRxToCpCountingPortConfigSet
(
    IN GT_U8                         devNum,
    IN GT_PHYSICAL_PORT_NUM          unitRepresentingPort
)
{
    return internal_cpssDxChPortDpDebugRxToCpCountingConfigSet(
            devNum, unitRepresentingPort,
            (1 << 4), /*criteriaParamBmp*/
            GT_FALSE, /*cutThroughPacket*/
            0, /*rxToCpPriority*/
            GT_FALSE, /*latencySensitive*/
            GT_FALSE /*headOrTailDispatch*/);
}
...
/* tables field-index ==> offset and field-index ==> size */
/* size of null bits means that the field does not exist  */
/* field0 - multicastPacket       */
/* field1 - cutThroughPacket      */
/* field2 - cutThroughTerminated  */
/* field3 - trunkatedHeader       */
/* field4 - dummyDescriptor       */
/* field5 - highPriorityPacket    */
/* field6 - contextDone           */
/* field7 - descriptorValid       */
/* field8 - channel               */
/* field9 - layer2CutThrough      */
static const GT_U32 sip6_fieldCpToRxOffsetArr[]    = {0, 1, 2, 3, 4, 0, 0, 0, 5, 3};
static const GT_U32 sip6_fieldCpToRxSizeArr[]      = {1, 1, 1, 1, 1, 0, 0, 0, 6, 0};
static const GT_U32 sip6_10_fieldCpToRxOffsetArr[] = {0, 1, 2, 3, 4, 5, 6, 7, 8, 3};
static const GT_U32 sip6_10_fieldCpToRxSizeArr[]   = {1, 1, 1, 1, 1, 1, 1, 1, 6, 0};
static const GT_U32 sip6_30_fieldCpToRxOffsetArr[] = {0, 1, 2, 4, 5, 6, 7, 8, 9, 3};
static const GT_U32 sip6_30_fieldCpToRxSizeArr[]   = {1, 1, 1, 1, 1, 1, 1, 1, 6, 1};
#define CP_TO_RX_FIELDS_NUM (sizeof(sip6_fieldCpToRxOffsetArr) / sizeof(sip6_fieldCpToRxOffsetArr[0]))
...
* @param[in] layer2CutThrough      - GT_TRUE - layer2 Cut Through (default bytecount 0x3FFF).
*                                    GT_FALSE - not layer2 Cut Through (bytecount calculated by header).
*                                    Note: Parameters highPriorityPacket, contextDone, descriptorValid
*                                          Relevant for AC5P and above only.
*                                          layer2CutThrough relevant to Ironman and avove only
...
)
{
    ...
    fieldValArr[9] = BOOL2BIT_MAC(layer2CutThrough);
    hwValueRef  = 0;
    hwValueMask = 0;
    hwWriteMask = 0;
...
)
{
    ...
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, unitRepresentingPort, criteriaParamBmp,
         ...
         contextDone, descriptorValid, layer2CutThrough));
...
*                                                     GT_FALSE - descriptor invalid.
* @param[out] layer2CutThroughPtr      - (pointer to) GT_TRUE - layer2 Cut Through (default bytecount 0x3FFF).
*                                                     GT_FALSE - not layer2 Cut Through (bytecount calculated by header).
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
...
(
    ...
    OUT GT_BOOL                          *layer2CutThroughPtr
)
{
    GT_STATUS rc;               /* return code      */
    GT_U32    regAddr;          /* register address */
    ...
    GT_U32    mask;             /* work mask */
    GT_U32    i;                /* parameter index */
    GT_U32    fieldValArr[CP_TO_RX_FIELDS_NUM] = {0,0,0,0,0,0,0,0,0};   /* parameters converted to HW values */
    GT_U32    globalDmaNum;     /* (global) rxdma port number  */
    GT_U32    localDmaNum = 0;  /* local rxdma port number  */
    GT_U32    dpIndex = GT_NA;  /* indication for a specific DMA that counting for specific 'local DMA port' */
    const GT_U32 *fieldCpToRxOffsetArr; /* pointer to array of field offsets */
    ...
    CPSS_NULL_PTR_CHECK_MAC(layer2CutThroughPtr);
    ...
    *descriptorValidPtr      = BIT2BOOL_MAC(fieldValArr[7]);
    /* foundLocalDmaNum      = BIT2BOOL_MAC(fieldValArr[8]); ignored by this API */
    *layer2CutThroughPtr     = BIT2BOOL_MAC(fieldValArr[9]);
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortDpDebugCpToRxCountingConfigGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, unitRepresentingPort, criteriaParamBmpPtr,
         multicastPacketPtr, cutThroughPacketPtr, cutThroughTerminatedPtr,
         trunkatedHeaderPtr, dummyDescriptorPtr, highPriorityPacketPtr,
         contextDonePtr, descriptorValidPtr, layer2CutThroughPtr));
