...
)
{
    ...
    switch(mode)
    {
        case CPSS_PORT_PCS_RESET_MODE_RX_E:
            ...
            rc = mvHwsPcsRxReset(devNum, portGroup, pcsNum, pcsType, portMode, action);
            break;
...
/**
* @internal internal_cpssDxChPortPcsResetGet function
* @endinternal
*
* @brief   Set/unset the PCS reset for given mode on port.
*
...
*
*/
static GT_STATUS internal_cpssDxChPortPcsResetGet
(
    IN  GT_U8                          devNum,
    IN  GT_PHYSICAL_PORT_NUM           portNum,
    IN  CPSS_PORT_PCS_RESET_MODE_ENT   mode,
...
)
{
    ...
    GT_U32     portGroup;    /* local core number */
    GT_U32    localPort;    /* number of port in local core */
    MV_HWS_RESET state;    /* reset state in HWS format */
    MV_HWS_PORT_STANDARD    portMode;   /* port ifMode in HWS format */
    MV_HWS_PORT_PCS_TYPE    pcsType; /* PCS unit type used by port in HWS format */
    GT_U32                  portMacNum;      /* MAC number */
    MV_HWS_PORT_INIT_PARAMS curPortParams;

    GT_UNUSED_PARAM(pcsType);
    ...
    {
        case CPSS_PORT_PCS_RESET_MODE_RX_E:
            CPSS_LOG_INFORMATION_MAC("Calling: mvHwsPcsRxResetGet(devNum[%d], portGroup[%d], pcsNum[%d], pcsType[%d])", devNum, portGroup, pcsNum, pcsType);
            rc = mvHwsPcsRxResetGet(devNum, portGroup, pcsNum, portMode, &state);
            *statePtr = (UNRESET == state) ? GT_FALSE : GT_TRUE;
            break;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortPcsResetGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, mode, statePtr));

    rc = internal_cpssDxChPortPcsResetGet(devNum, portNum, mode, statePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, mode, statePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    rc = cpssDxChPortPcsResetSet(CAST_SW_DEVNUM(devNum),portNum,mode,state);
    if (rc!=GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"calling prvCpssDxChPortPcsResetSetWrapper from wrapper failed=%d",rc);
    }
