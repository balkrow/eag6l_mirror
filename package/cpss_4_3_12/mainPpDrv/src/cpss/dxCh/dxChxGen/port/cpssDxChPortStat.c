...
/* gtMacCounterOffset[0] - offsets for not XGMII interface */
    GT_U16 internal_gtMacCounterOffset[CPSS_LAST_MAC_COUNTER_NUM_E];

    /* for device that all counters are 64 bits : BC3 */
    GT_U16 internal_gtMacCounterOffset_all64Bits[CPSS_LAST_MAC_COUNTER_NUM_E];

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);

    cpssOsMemSet(internal_gtMacCounterOffset, 0, sizeof(internal_gtMacCounterOffset));
    cpssOsMemSet(internal_gtMacCounterOffset_all64Bits, 0, sizeof(internal_gtMacCounterOffset_all64Bits));
     ...
     if(PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset == NULL)
     {

         PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset = (GT_U16 *)cpssOsMalloc(sizeof(GT_U16)*CPSS_LAST_MAC_COUNTER_NUM_E);
         if (PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset == NULL)
         {
     ...
     {
         cpssOsMemCpy(PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset,internal_gtMacCounterOffset,sizeof(GT_U16)*CPSS_LAST_MAC_COUNTER_NUM_E);
     }
     else
     {
         /* BC3 */
         cpssOsMemCpy(PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset,internal_gtMacCounterOffset_all64Bits,sizeof(GT_U16)*CPSS_LAST_MAC_COUNTER_NUM_E);
     }
...
*
* APPLICABLE DEVICES:
*        Falcon; AC5X; AC5P; Harrier; Ironman.
*
* NOT APPLICABLE DEVICES:
*        None.
*
...
*
*******************************************************************************/
static GT_VOID prvCpssDxChPortMtiSumMibEntry
(
    IN  GT_U8  devNum,
    INOUT GT_U64 destEntryArr[/*PRV_CPSS_MTI_MIB_COUNTERS_ENTRY_SIZE_CNS*/],
    IN    GT_U64 addRxEntryArr[/*PRV_CPSS_MTI_RX_LAST_E*/],
    IN    GT_U64 addTxEntryArr[/*PRV_CPSS_MTI_TX_LAST_E*/]
)
{
    GT_U32 i;
    GT_U32 mtiRxCounterNum;
    GT_U32 mtiTxCounterNum;

    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
    {
        mtiRxCounterNum = PRV_CPSS_SIP7_MTI_RX_LAST_E;
        mtiTxCounterNum = PRV_CPSS_SIP7_MTI_TX_LAST_E;
    }
    else if(PRV_CPSS_SIP_6_CHECK_MAC(devNum) == GT_TRUE)
    {
        mtiRxCounterNum = PRV_CPSS_MTI_RX_LAST_E;
        mtiTxCounterNum = PRV_CPSS_MTI_TX_LAST_E;
    }
    else
    {
        return;
    ...
    /* Fill all Rx Counters */
    for (i = 0 ; i < mtiRxCounterNum ; i++)
    {
        destEntryArr[i] = prvCpssMathAdd64(destEntryArr[i],addRxEntryArr[i]);
    }
    /* Fill all TX counters - the offset in destination is after RX Counters */
    for (i = 0 ; i < mtiTxCounterNum ; i++)
    {
        destEntryArr[i+mtiRxCounterNum] = prvCpssMathAdd64(destEntryArr[i+mtiRxCounterNum],addTxEntryArr[i]);
    }
}
...
)
{
    ...
    if(( ((PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum)) && (portMacNum != CPSS_CPU_PORT_NUM_CNS))  ||
        PRV_CPSS_SIP_5_CHECK_MAC(devNum)) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum)))
    {
...
/**
* @internal prvCpssDxChSip6PortMacCountersMTIPortReadMem function
* @endinternal
*
* @brief   Gets all RX/TX MTI MAC MIB counters from HW.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Caelum; Aldrin; AC3X; Bobcat2; Bobcat3; Aldrin2; AAS.
*
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number associated with channel
* @param[in] newCapture               - whether or not to capture a new counters snapshot
...
*
*/
static GT_STATUS prvCpssDxChSip6PortMacCountersMTIPortReadMem
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  GT_BOOL                 newCapture,
...
)
{
    ...
    /* Arguments check */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_AAS_E);
    ...
    mtiMacMibsRegsAddrDBLength = (rxTxMode == 0) ? PRV_CPSS_MTI_RX_LAST_E : PRV_CPSS_MTI_TX_LAST_E;
    ...
    if (newCapture)
    {
        ...
        else
        {
            ...
            if((PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacNum].preemptionSupported) &&
               (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E))
            {
                if (CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E == macCountMode )
                {
                    regValue = 0x2;
                }
            }
            regMask = (GT_U32)(0x3 | (3 << 27) | (3 << 30));    /* Port 0 mask set, command TX/RX capture, clear on read */
            if(rxTxMode == 0)
            {
                regValue |=  (1 << 27) | (isReadOnClear << 30);    /* command RX capture, RX clear on read enable */
            }
    ...
    for (i=0 ; i<mtiMacMibsRegsAddrDBLength ; i++)
    {
        ...
        if(((isReducedPort == GT_TRUE) && (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)) ||
           (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E))
        {
            if((rxTxMode == 1) && (i >= PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_8_E) && (i <= PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_15_E))
            {
                continue;
            }
            if((rxTxMode == 0) && (i >= PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_8_E) && (i <= PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_15_E))
            {
                continue;
            }

            if((rxTxMode == 1) && (i > PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_15_E))
            {
                regAddr = regAddr - 0x4 * 8;
            }
            if((rxTxMode == 0) && (i > PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_15_E))
            {
                regAddr = regAddr - 0x4 * 8;
            }
        }
...
/**
* @internal prvCpssDxChSip7PortMacCountersMTIPortReadMem function
* @endinternal
*
* @brief   Gets all RX/TX MTI MAC MIB counters from HW.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Caelum; Aldrin; AC3X; Bobcat2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number associated with channel
* @param[in] newCapture               - whether or not to capture a new counters snapshot
...
* @retval GT_FAIL                  - on error
*
* @note mtiMacMibsCaptureArr should be of size PRV_CPSS_SIP7_MTI_RX_LAST_E/PRV_CPSS_SIP7_MTI_TX_LAST_E.
*
*/
static GT_STATUS prvCpssDxChSip7PortMacCountersMTIPortReadMem
(
    IN  GT_U8                   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  GT_BOOL                 newCapture,
    IN  GT_U32                  rxTxMode,
    IN  CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_ENT      macCountMode,
    OUT GT_U64                  *mtiMacMibsCaptureArr /* should be sized PRV_CPSS_SIP7_MTI_RX_LAST_E/PRV_CPSS_SIP7_MTI_TX_LAST_E */
)
{
    GT_U32 *mtiMacMibsRegsAddrDBArrPtr;      /* The MTI MIB counters addresses */
    GT_U32  mtiMacMibsRegsAddrDBLength;      /* The MTI MIB counters length */
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E |
                                                  CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AC5_E);
    CPSS_NULL_PTR_CHECK_MAC(mtiMacMibsCaptureArr);
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum,portNum,portMacNum);
    ...
    mtiMacMibsRegsAddrDBLength = (rxTxMode == 0) ? PRV_CPSS_SIP7_MTI_RX_LAST_E : PRV_CPSS_SIP7_MTI_TX_LAST_E;
    for (i=0 ; i<mtiMacMibsRegsAddrDBLength ; i++)
    {
        mtiMacMibsCaptureArr[i].l[0] = 0;
        mtiMacMibsCaptureArr[i].l[1] = 0;
    ...
    for (i=0 ; i<mtiMacMibsRegsAddrDBLength ; i++)
    {
        ...
        /* Reading LOW 32bits */
        rc = prvCpssDrvHwPpPortGroupReadRegister(devNum, portGroupId, regAddr, &(mtiMacMibsCaptureArr[i].l[0]));
        if(rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read first MTI mib counter entry from address 0x%08x", regAddr);
        }
        ...
        /* Reading HIGH 32bits */
        rc = prvCpssDrvHwPpPortGroupReadRegister(devNum, portGroupId, regAddrDataHi, &(mtiMacMibsCaptureArr[i].l[1]));
        if(rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read second MTI mib counter entry from address 0x%08x", regAddrDataHi);
        }
...
/**
* @internal prvDxChSip6PortMacBrCountersOnMtiPortGet function
* @endinternal
*
...
* @retval GT_NOT_SUPPORTED         - on not supported port
*/
GT_STATUS prvDxChSip6PortMacBrCountersOnMtiPortGet
(
    IN  GT_U8                                       devNum,
    IN  GT_PHYSICAL_PORT_NUM                        portNum,
    IN  CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_ENT      macCountMode,
...
{
    GT_STATUS     rc;
    GT_U64        rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_LAST_E];   /* Array to load to all RX MTI counters from HW */
    GT_U64        txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_LAST_E];   /* Array to load to all TX MTI counters from HW */
    PRV_CPSS_PORT_TYPE_ENT     macType;
    GT_U32                     regAddr;
    GT_U32                     value;
    GT_U32                     i;
    GT_U32  portMacNum;  /* Mac number associated with the portNum */
    PRV_CPSS_DXCH_PORT_XLG_MIB_SHADOW_STC *mibShadowPtr;

    /* Arguments check */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E |
                                      CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(mtiMibStcPtr);
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum,portNum,portMacNum);
    macType = PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum,portMacNum);
    mibShadowPtr = PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMibShadowArr[portMacNum];

    /* Read all mac mti mib counters from HW */
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 0/*RX MIB*/, macCountMode, &rxMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI mibs");
    }

    if (mibShadowPtr->clearOnReadEnable == GT_TRUE)
    {
        /* add from shadow */
        for (i = 0 ; i < PRV_CPSS_MTI_RX_LAST_E ; i++)
        {
            rxMtiMacMibsCaptureArr[i] = prvCpssMathAdd64(mibShadowPtr->mibMtiShadow[i],rxMtiMacMibsCaptureArr[i]);
        }
    }

    mtiMibStcPtr->rxCounterStc.etherStatsOctets               = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.ifInOctets                     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_OCTETS_RECEIVED_OK_E];
    mtiMibStcPtr->rxCounterStc.aAlignmentErrors               = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_ALIGNMENT_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aPAUSEMACCtrlFramesReceived    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E];
    mtiMibStcPtr->rxCounterStc.aFrameTooLongErrors            = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_FRAME_TOO_LONG_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aInRangeLengthErrors           = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_IN_RANGE_LENGTH_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aFramesReceivedOK              = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_FRAMES_RECIEVED_OK_E];
    mtiMibStcPtr->rxCounterStc.aFrameCheckSequenceErrors      = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_FRAME_CHECK_SEQUENCE_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.VLANRecievedOK                 = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_VLAN_RECIEVED_OK_E];
    mtiMibStcPtr->rxCounterStc.ifInErrors                     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.ifInUcastPkts                  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_UC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.ifInMulticastPkts              = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_MC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.ifInBroadcastPkts              = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_BC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsDropEvents           = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_DROP_EVENTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts                 = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsUndersizePkts        = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_UNDERSIZE_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts64Octets         = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_64OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts65to127Octets    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_65TO127_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts128to255Octets   = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_128TO255_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts256to511Octets   = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_256TO511_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts512to1023Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_512TO1023_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts1024to1518Octets = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts1519toMaxOctets  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsOversizePkts         = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_OVERSIZE_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsJabbers              = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_JABBERS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsFragments            = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_FRAGMENTS_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_0     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_0_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_1     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_1_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_2     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_2_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_3     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_3_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_4     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_4_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_5     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_5_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_6     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_6_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_7     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_7_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_8     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_8_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_9     = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_9_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_10    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_10_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_11    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_11_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_12    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_12_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_13    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_13_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_14    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_14_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_15    = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_15_E];
    mtiMibStcPtr->rxCounterStc.aMACControlFramesReceived      = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_MAC_CONTROL_FRAMES_RECIEVED_E];

    /* Read all mac mti mib counters from HW */
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 1/*TX MIB*/, macCountMode, &txMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
    ...
    {
        /* add from shadow */
        for (i = 0 ; i < PRV_CPSS_MTI_TX_LAST_E; i++)
        {
            txMtiMacMibsCaptureArr[i] = prvCpssMathAdd64(mibShadowPtr->mibMtiShadow[PRV_CPSS_MTI_RX_LAST_E +i],txMtiMacMibsCaptureArr[i]);
        }
        cpssOsMemSet(mibShadowPtr->mibMtiShadow, 0, sizeof(mibShadowPtr->mibMtiShadow));
    }


    mtiMibStcPtr->txCounterStc.ifOutOctets                    = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_OCTETS_E];
    mtiMibStcPtr->txCounterStc.octetsTransmittedOk            = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_OCTETS_TRANSMITTED_OK_E];
    mtiMibStcPtr->txCounterStc.aPAUSEMACCtrlFramesTransmitted = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E];
    mtiMibStcPtr->txCounterStc.aFramesTransmittedOK           = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_FRAMES_TRANSMITTED_OK_E];
    mtiMibStcPtr->txCounterStc.VLANTransmittedOK              = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_VLAN_RECIEVED_OK_E];
    mtiMibStcPtr->txCounterStc.ifOutErrors                    = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_ERRORS_E];
    mtiMibStcPtr->txCounterStc.ifOutUcastPkts                 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_UC_PKTS_E];
    mtiMibStcPtr->txCounterStc.ifOutMulticastPkts             = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_MC_PKTS_E];
    mtiMibStcPtr->txCounterStc.ifOutBroadcastPkts             = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_BC_PKTS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts64Octets         = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_64OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts65to127Octets    = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_65TO127_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts128to255Octets   = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_128TO255_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts256to511Octets   = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_256TO511_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts512to1023Octets  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_512TO1023_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts1024to1518Octets = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts1519toMaxOctets  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_0  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_0_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_1  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_1_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_2  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_2_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_3  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_3_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_4  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_4_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_5  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_5_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_6  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_6_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_7  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_7_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_8  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_8_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_9  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_9_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_10 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_10_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_11 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_11_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_12 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_12_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_13 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_13_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_14 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_14_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_15 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_15_E];
    mtiMibStcPtr->txCounterStc.aMACControlFramesTransmitted   = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_MAC_CONTROL_FRAMES_TRANSMITTED_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts                 = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PKTS_E];
    ...
    if ((PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacNum].preemptionSupported))
    {
        if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
        {
            ...
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brCntrl;
            }
            else if (macType == PRV_CPSS_PORT_MTI_CPU_E)
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brCntrl;
            }
            else
            {
        ...
        {
            /* Read all mac mti mib counters from HW */
            rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &rxMtiMacMibsCaptureArr[0]);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI mibs");
            }
            rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 1/*TX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &txMtiMacMibsCaptureArr[0]);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read TX MTI mibs");
            }
...
/**
* @internal prvDxChSip7PortMacBrCountersOnMtiPortGet function
* @endinternal
*
* @brief   AC5P - Get mac mib counters according macType
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5X; AC5P; Harrier; Ironman..
*
* @param[in] devNum                   -  physical device number
* @param[in] portNum                  -  port number
* @param[in] macCountMode             -  mac counting mode
...
* @retval GT_NOT_SUPPORTED         - on not supported port
*/
static GT_STATUS prvDxChSip7PortMacBrCountersOnMtiPortGet
(
    IN  GT_U8   devNum,
    IN  GT_PHYSICAL_PORT_NUM    portNum,
    IN  CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_ENT      macCountMode,
...
{
    GT_STATUS                               rc;
    GT_U64                                  rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_LAST_E];   /* Array to load to all RX MTI counters from HW */
    GT_U64                                  txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_LAST_E];   /* Array to load to all TX MTI counters from HW */
    GT_U32                                  portMacNum;  /* Mac number associated with the portNum */
    GT_U32                                  regAddr;
    GT_U32                                  value;
    GT_U32                                  i;
    ...
    /* Read all mti mib counters from HW */
    rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 0/*RX MIB*/, macCountMode, &rxMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI mibs");
    }
    ...
    {
        /* add from shadow */
        for (i = 0 ; i < PRV_CPSS_SIP7_MTI_RX_LAST_E ; i++)
        {
            rxMtiMacMibsCaptureArr[i] = prvCpssMathAdd64(mibShadowPtr->mibMtiShadow[i],rxMtiMacMibsCaptureArr[i]);
        }
    }

    mtiMibStcPtr->rxCounterStc.etherStatsOctets                = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.ifInOctets                      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_OCTETS_RECEIVED_OK_E];
    mtiMibStcPtr->rxCounterStc.aAlignmentErrors                = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_ALIGNMENT_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aPAUSEMACCtrlFramesReceived     = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E];
    mtiMibStcPtr->rxCounterStc.aFrameTooLongErrors             = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_FRAME_TOO_LONG_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aInRangeLengthErrors            = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_IN_RANGE_LENGTH_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.aFramesReceivedOK               = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_FRAMES_RECIEVED_OK_E];
    mtiMibStcPtr->rxCounterStc.aFrameCheckSequenceErrors       = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_FRAME_CHECK_SEQUENCE_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.VLANRecievedOK                  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_VLAN_RECIEVED_OK_E];
    mtiMibStcPtr->rxCounterStc.ifInErrors                      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_ERRORS_E];
    mtiMibStcPtr->rxCounterStc.ifInUcastPkts                   = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_UC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.ifInMulticastPkts               = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_MC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.ifInBroadcastPkts               = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_BC_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsDropEvents            = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_DROP_EVENTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts                  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsUndersizePkts         = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_UNDERSIZE_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts64Octets          = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_64OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts65to127Octets     = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_65TO127_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts128to255Octets    = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_128TO255_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts256to511Octets    = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_256TO511_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts512to1023Octets   = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_512TO1023_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts1024to1518Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts1519toMaxOctets   = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts1519to2047Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_1519TO2047_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts2048to4095Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_2048TO4095_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts4096to8191Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_4096TO8191_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts8192to9216Octets  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_8192TO9216_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPkts9217to16383Octets = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_9217TO16383_OCTETS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsPktsBiggerThan16383   = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PKTS_BIGGERTHAN16383_E];
    mtiMibStcPtr->rxCounterStc.etherStatsOversizePkts          = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_OVERSIZE_PKTS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsJabbers               = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_JABBERS_E];
    mtiMibStcPtr->rxCounterStc.etherStatsFragments             = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_FRAGMENTS_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_0      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_0_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_1      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_1_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_2      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_2_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_3      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_3_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_4      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_4_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_5      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_5_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_6      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_6_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_7      = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_CBFC_PAUSE_FRAMES_RECIEVED_7_E];
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_8      = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_9      = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_10     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_11     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_12     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_13     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_14     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aCBFCPAUSEFramesReceived_15     = zeroValue;
    mtiMibStcPtr->rxCounterStc.aMACControlFramesReceived       = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_MAC_CONTROL_FRAMES_RECIEVED_E];

    /* Read all mti mib counters from HW */
    rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 1/*TX MIB*/, macCountMode, &txMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read TX MTI mibs");
    }
    ...
    {
        /* add from shadow */
        for (i = 0 ; i < PRV_CPSS_SIP7_MTI_TX_LAST_E; i++)
        {
            txMtiMacMibsCaptureArr[i] = prvCpssMathAdd64(mibShadowPtr->mibMtiShadow[PRV_CPSS_SIP7_MTI_RX_LAST_E +i],txMtiMacMibsCaptureArr[i]);
        }
        cpssOsMemSet(mibShadowPtr->mibMtiShadow, 0, sizeof(mibShadowPtr->mibMtiShadow));
    }

    mtiMibStcPtr->txCounterStc.ifOutOctets                    = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_OCTETS_E];
    mtiMibStcPtr->txCounterStc.octetsTransmittedOk            = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_OCTETS_TRANSMITTED_OK_E];
    mtiMibStcPtr->txCounterStc.aPAUSEMACCtrlFramesTransmitted = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E];
    mtiMibStcPtr->txCounterStc.aFramesTransmittedOK           = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_FRAMES_TRANSMITTED_OK_E];
    mtiMibStcPtr->txCounterStc.VLANTransmittedOK              = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_VLAN_RECIEVED_OK_E];
    mtiMibStcPtr->txCounterStc.ifOutErrors                    = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_ERRORS_E];
    mtiMibStcPtr->txCounterStc.ifOutUcastPkts                 = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_UC_PKTS_E];
    mtiMibStcPtr->txCounterStc.ifOutMulticastPkts             = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_MC_PKTS_E];
    mtiMibStcPtr->txCounterStc.ifOutBroadcastPkts             = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_BC_PKTS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts64Octets         = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_64OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts65to127Octets    = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_65TO127_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts128to255Octets   = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_128TO255_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts256to511Octets   = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_256TO511_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts512to1023Octets  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_512TO1023_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts1024to1518Octets = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts1519toMaxOctets  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts1519to2047Octets = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_1519O2047_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts2048to4095Octets = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_2048TO4095_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts4096to8191Octets = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_4096TO8191_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts8192to9216Octets = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_8192TO9216_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts9217to16383Octets= txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_9217TO16383_OCTETS_E];
    mtiMibStcPtr->txCounterStc.etherStatsPktsBIggerThan16383  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_BIGGERTHAN16383_E];
    mtiMibStcPtr->txCounterStc.etherStatsPktsMoreThanMTU      = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_MORE_THAN_MTU_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_0  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_0_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_1  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_1_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_2  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_2_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_3  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_3_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_4  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_4_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_5  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_5_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_6  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_6_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_7  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_CBFC_PAUSE_FRAMES_TRANSMITTED_7_E];
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_8  = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_9  = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_10 = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_11 = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_12 = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_13 = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_14 = zeroValue;
    mtiMibStcPtr->txCounterStc.aCBFCPAUSEFramesTransmitted_15 = zeroValue;
    mtiMibStcPtr->txCounterStc.aMACControlFramesTransmitted   = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_MAC_CONTROL_FRAMES_TRANSMITTED_E];
    mtiMibStcPtr->txCounterStc.etherStatsPkts                 = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PKTS_E];
...
/**
* @internal internal_cpssDxChPortMacBrCountersOnMtiPortGet function
* @endinternal
*
* @brief   AC5P - Get mac mib counters according macType
*
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
    {
        return prvDxChSip7PortMacBrCountersOnMtiPortGet(devNum, portNum, macCountMode, mtiMibStcPtr);
    }
    else if(PRV_CPSS_SIP_6_CHECK_MAC(devNum) == GT_TRUE)
    {
        return prvDxChSip6PortMacBrCountersOnMtiPortGet(devNum, portNum, macCountMode, mtiMibStcPtr);
    }
    else
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
    {
        return prvDxChSip7PortMacBrCountersOnMtiPortGet(devNum, portNum, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, mtiMibStcPtr);
    }
    else if(PRV_CPSS_SIP_6_CHECK_MAC(devNum) == GT_TRUE)
    {
        return prvDxChSip6PortMacBrCountersOnMtiPortGet(devNum, portNum, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, mtiMibStcPtr);
    }
    else
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
...
/**
* @internal prvCpssDxChSip7PortMacCountersMtiMibsConvert function
* @endinternal
*
...
* @retval GT_FAIL                  - on error
*/
static GT_STATUS prvCpssDxChSip7PortMacCountersMtiMibsConvert
(
    IN  GT_U8   devNum,
    IN  GT_PHYSICAL_PORT_NUM  portNum,
    IN  GT_BOOL newCapture,
...
{
    GT_U64  rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_LAST_E];   /* Array to load to all MTI counters from HW */
    GT_U64  txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_LAST_E];   /* Array to load to all MTI counters from HW */
    ...
    rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &rxMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read MTI mibs");
    }

    /* Reading MTI TX mibs from HW */
    rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 1, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &txMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read MTI mibs");
    }

    /* Converting MTI MIBS to regular MIBS xlgMibEntry struct. Not all cg mib counters
        ...
        switch (cntrName)
        {
            case CPSS_GOOD_OCTETS_RCV_E:
                ...
                for (i=0; i<64; i++)
                {
                    ...
                    temp64 = prvCpssMathAdd64(temp64,rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E]);
                    temp64 = prvCpssMathAdd64(temp64,rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_MAC_CONTROL_FRAMES_RECIEVED_E]);
                }
                convertedMtiMacMibCounterEntry  = prvCpssMathSub64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_OCTETS_RECEIVED_OK_E], temp64);
                break;
            case CPSS_BAD_OCTETS_RCV_E:
                convertedMtiMacMibCounterEntry = prvCpssMathSub64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_OCTETS_E],
                                                                rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_OCTETS_RECEIVED_OK_E]);
                break;
            case CPSS_MAC_TRANSMIT_ERR_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_ERRORS_E];
                break;
            case CPSS_GOOD_PKTS_RCV_E:
            ...
            case CPSS_BAD_PKTS_RCV_E:
                ...
                break;
            case CPSS_BRDC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_BC_PKTS_E];
                break;
            case CPSS_MC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_MC_PKTS_E];
                break;
            case CPSS_PKTS_64_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_64OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_64OCTETS_E]);
                break;
            case CPSS_PKTS_65TO127_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_65TO127_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_65TO127_OCTETS_E]);
                break;
            case CPSS_PKTS_128TO255_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_128TO255_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_128TO255_OCTETS_E]);
                break;
            case CPSS_PKTS_256TO511_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_256TO511_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_256TO511_OCTETS_E]);
                break;
            case CPSS_PKTS_512TO1023_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_512TO1023_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_512TO1023_OCTETS_E]);
                break;
            case CPSS_PKTS_1024TOMAX_OCTETS_E:
                temp64.l[0]=0;
                temp64.l[1]=0;
                temp64 = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E],
                                          rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                temp64 = prvCpssMathAdd64(temp64,
                                          txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E]);
                convertedMtiMacMibCounterEntry = prvCpssMathAdd64(temp64,
                                          txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                break;
            case CPSS_GOOD_OCTETS_SENT_E:
                temp64.l[0]=0;
                temp64.l[1]=0;
                for (i=0; i<64; i++)
                {
                    /*  all  Flow Control packets (802.3 and PFC)are counted by CPSS_CG_A_PAUSE_MAC_CTRL_FRAMES counter
                       and per class counters shouldn't be taken into consideration in goodOctet calculation*/
                    temp64 = prvCpssMathAdd64(temp64,txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_MAC_CONTROL_FRAMES_TRANSMITTED_E]);
                    temp64 = prvCpssMathAdd64(temp64,txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E]);
                }
                convertedMtiMacMibCounterEntry  = prvCpssMathSub64(txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_OCTETS_E], temp64);
                break;
            case CPSS_GOOD_PKTS_SENT_E:
                /* MIB NOT SUPPORTED IN DEVICE */
                break;
            case CPSS_EXCESSIVE_COLLISIONS_E:
                ...
                break;
            case CPSS_MC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_MC_PKTS_E];
                break;
            case CPSS_BRDC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_BC_PKTS_E];
                break;
            case CPSS_UNRECOG_MAC_CNTR_RCV_E:
                /* MIB NOT SUPPORTED IN DEVICE */
                break;
            case CPSS_FC_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E];
                break;
            case CPSS_GOOD_FC_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E];
                break;
            case CPSS_DROP_EVENTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_DROP_EVENTS_E];
                break;
            case CPSS_UNDERSIZE_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_UNDERSIZE_PKTS_E];
                break;
            case CPSS_FRAGMENTS_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_FRAGMENTS_E];
                break;
            case CPSS_OVERSIZE_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_OVERSIZE_PKTS_E];
                break;
            case CPSS_JABBER_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_JABBERS_E];
                break;
            case CPSS_MAC_RCV_ERROR_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_ERRORS_E];
                break;
            case CPSS_BAD_CRC_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_A_FRAME_CHECK_SEQUENCE_ERRORS_E];
                break;
            case CPSS_COLLISIONS_E:
                /* COUNTER NOT EXIST IN CG UNIT */
                break;
            ...
            case CPSS_BadFC_RCV_E:
                ...
                break;
            case CPSS_GOOD_UC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_IF_IN_UC_PKTS_E];
                break;
            case CPSS_GOOD_UC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_IF_OUT_UC_PKTS_E];
                break;
            case CPSS_MULTIPLE_PKTS_SENT_E:
                /* COUNTER NOT EXIST IN CG UNIT */
                /* MIB NOT SUPPORTED IN DEVICE */
            ...
            case CPSS_DEFERRED_PKTS_SENT_E:
                ...
                break;
            case CPSS_PKTS_1024TO1518_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E ],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E]);
                break;
            case CPSS_PKTS_1519TOMAX_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_SIP7_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                break;
            default:
                continue;
    ...
    /* WA - in aggregate mode - need a dummy read of PMAC in order to reset its counter */
      if((PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacNum].preemptionSupported))
      {
           ...
           {
              /* Read all mac mti mib counters from HW */
              rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &rxMtiMacMibsCaptureArr[0]);
              if (rc != GT_OK)
              {
                  CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI mibs");
              }
              rc = prvCpssDxChSip7PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 1/*TX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &txMtiMacMibsCaptureArr[0]);
              if (rc != GT_OK)
              {
                  CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read TX MTI mibs");
              }
...
{
    GT_U64  rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_LAST_E];   /* Array to load to all MTI counters from HW */
    GT_U64  txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_LAST_E];   /* Array to load to all MTI counters from HW */
    GT_U64  convertedMtiMacMibCounterEntry;               /* Single counter entry */
    GT_U64  temp64;
    GT_U32  i;                                            /* iterator */
    GT_U32  portMacNum;                                   /* Mac number associated with the portNum */
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
    {
        return prvCpssDxChSip7PortMacCountersMtiMibsConvert(devNum,portNum,newCapture,xlgMibEntry);
    }
    ...
    /* Reading MTI RX mibs from HW */
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &rxMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read MTI mibs");
    }
    ...
    /* Reading MTI TX mibs from HW */
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 1, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &txMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read MTI mibs");
    }
        ...
        switch (cntrName)
        {
            case CPSS_GOOD_OCTETS_RCV_E:
                ...
                for (i=0; i<64; i++)
                {
                    /*  all  Flow Control packets (802.3 and PFC)are counted by CPSS_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E counter
                       and per class counters shouldn't be taken into consideration in goodOctet calculation*/
                    temp64 = prvCpssMathAdd64(temp64,rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E]);
                    temp64 = prvCpssMathAdd64(temp64,rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_MAC_CONTROL_FRAMES_RECIEVED_E]);
                }
                convertedMtiMacMibCounterEntry  = prvCpssMathSub64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_OCTETS_RECEIVED_OK_E], temp64);
                break;
            case CPSS_BAD_OCTETS_RCV_E:
                convertedMtiMacMibCounterEntry = prvCpssMathSub64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_OCTETS_E],
                                                                rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_OCTETS_RECEIVED_OK_E]);
                break;
            case CPSS_MAC_TRANSMIT_ERR_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_ERRORS_E];
                break;
            case CPSS_GOOD_PKTS_RCV_E:
                /* MIB NOT SUPPORTED IN DEVICE */
                break;
            case CPSS_BAD_PKTS_RCV_E:
                ...
                break;
            case CPSS_BRDC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_BC_PKTS_E];
                break;
            case CPSS_MC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_MC_PKTS_E];
                break;
            case CPSS_PKTS_64_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_64OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_64OCTETS_E]);
                break;
            case CPSS_PKTS_65TO127_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_65TO127_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_65TO127_OCTETS_E]);
                break;
            case CPSS_PKTS_128TO255_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_128TO255_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_128TO255_OCTETS_E]);
                break;
            case CPSS_PKTS_256TO511_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_256TO511_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_256TO511_OCTETS_E]);
                break;
            case CPSS_PKTS_512TO1023_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_512TO1023_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_512TO1023_OCTETS_E]);
                break;
            case CPSS_PKTS_1024TOMAX_OCTETS_E:
                temp64.l[0]=0;
                temp64.l[1]=0;
                temp64 = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E],
                                          rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                temp64 = prvCpssMathAdd64(temp64,
                                          txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E]);
                convertedMtiMacMibCounterEntry = prvCpssMathAdd64(temp64,
                                          txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                break;
            case CPSS_GOOD_OCTETS_SENT_E:
                temp64.l[0]=0;
                temp64.l[1]=0;
                for (i=0; i<64; i++)
                {
                    /*  all  Flow Control packets (802.3 and PFC)are counted by CPSS_CG_A_PAUSE_MAC_CTRL_FRAMES counter
                       and per class counters shouldn't be taken into consideration in goodOctet calculation*/
                    temp64 = prvCpssMathAdd64(temp64,txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_MAC_CONTROL_FRAMES_TRANSMITTED_E]);
                    temp64 = prvCpssMathAdd64(temp64,txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E]);
                }
                convertedMtiMacMibCounterEntry  = prvCpssMathSub64(txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_OCTETS_E], temp64);
                break;
            case CPSS_GOOD_PKTS_SENT_E:
                /* MIB NOT SUPPORTED IN DEVICE */
                break;
            case CPSS_EXCESSIVE_COLLISIONS_E:
                ...
                break;
            case CPSS_MC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_MC_PKTS_E];
                break;
            case CPSS_BRDC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_BC_PKTS_E];
                break;
            case CPSS_UNRECOG_MAC_CNTR_RCV_E:
                /* MIB NOT SUPPORTED IN DEVICE */
                break;
            case CPSS_FC_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_A_PAUSE_MAC_CTRL_FRAMES_TRANSMITTED_E];
                break;
            case CPSS_GOOD_FC_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_PAUSE_MAC_CTRL_FRAMES_RECIEVED_E];
                break;
            case CPSS_DROP_EVENTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_DROP_EVENTS_E];
                break;
            case CPSS_UNDERSIZE_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_UNDERSIZE_PKTS_E];
                break;
            case CPSS_FRAGMENTS_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_FRAGMENTS_E];
                break;
            case CPSS_OVERSIZE_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_OVERSIZE_PKTS_E];
                break;
            case CPSS_JABBER_PKTS_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_JABBERS_E];
                break;
            case CPSS_MAC_RCV_ERROR_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_ERRORS_E];
                break;
            case CPSS_BAD_CRC_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_A_FRAME_CHECK_SEQUENCE_ERRORS_E];
                break;
            case CPSS_COLLISIONS_E:
                /* COUNTER NOT EXIST IN CG UNIT */
                break;
            ...
            case CPSS_BadFC_RCV_E:
                ...
                break;
            case CPSS_GOOD_UC_PKTS_RCV_E:
                convertedMtiMacMibCounterEntry  = rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_IF_IN_UC_PKTS_E];
                break;
            case CPSS_GOOD_UC_PKTS_SENT_E:
                convertedMtiMacMibCounterEntry  = txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_IF_OUT_UC_PKTS_E];
                break;
            case CPSS_MULTIPLE_PKTS_SENT_E:
                /* COUNTER NOT EXIST IN CG UNIT */
                /* MIB NOT SUPPORTED IN DEVICE */
            ...
            case CPSS_DEFERRED_PKTS_SENT_E:
                ...
                break;
            case CPSS_PKTS_1024TO1518_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E ],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1024TO1518_OCTETS_E]);
                break;
            case CPSS_PKTS_1519TOMAX_OCTETS_E:
                convertedMtiMacMibCounterEntry  = prvCpssMathAdd64(rxMtiMacMibsCaptureArr[PRV_CPSS_MTI_RX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E],
                                                                txMtiMacMibsCaptureArr[PRV_CPSS_MTI_TX_ETHER_STATS_PTKS_1519TOMAX_OCTETS_E]);
                break;
            default:
                continue;
    ...
    /* WA - in aggregate mode - need a dummy read of PMAC in order to reset its counter */
        ...
        )
      {
           ...
           {
              /* Read all mac mti mib counters from HW */
              rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &rxMtiMacMibsCaptureArr[0]);
              if (rc != GT_OK)
              {
                  CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI mibs");
              }
              rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, newCapture, 1/*TX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &txMtiMacMibsCaptureArr[0]);
              if (rc != GT_OK)
              {
                  CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read TX MTI mibs");
              }
...
)
{
    ...
    GT_U32  dummyHigh32bitsCounter;/* in sip7 the EGF counters are 64 bits */
    ...
    {   CPSS_TBD_BOOKMARK /* add support for 64 bits counters */
        GT_U32  dummy;
        /* need to read also the high register */
        if (prvCpssPortGroupsBmpCounterSummary(devNum, portGroupsBmp, regAddr+0x8,0,32, &dummy,NULL) != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_HW_ERROR, LOG_ERROR_NO_MSG);
        }
    }
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* need to read also the high 32 bits of the counter , to make both 'clear on read' */
        regAddr += 0x8;
        (void)prvCpssPortGroupsBmpCounterSummary(devNum, portGroupsBmp,
            regAddr,0,32, &dummyHigh32bitsCounter,NULL);
    }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_AAS_E);
    PRV_CPSS_DXCH_MAC_IN_PHY_PORT_NUM_OR_CPU_PORT_CHECK_AND_MAC_NUM_GET_MAC(devNum,portNum,portMacNum);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
/**
* @internal internal_cpssDxChMacMtiCounterGet function
* @endinternal
*
* @brief   Gets MTI port specific counter.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5X; AC5P; Harrier; Ironman.
...
* @param[in] cntrName                 - counter name
*
* @param[out] cntrValuePtr            - (pointer to)counter value
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong port number or device
...
*
*/
static GT_STATUS internal_cpssDxChMacMtiCounterGet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum,
    IN  CPSS_PORT_MAC_MTI_COUNTER_ENT   cntrName,
    OUT GT_U64                          *cntrValuePtr
)
{
    GT_STATUS   rc;                 /* return code        */
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
    {
        CPSS_TBD_BOOKMARK_AAS /* need to add implementation for SIP7 */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_IMPLEMENTED, LOG_ERROR_NO_MSG);
    }

    macType = PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum,portMacNum);

    if(cntrName >= CPSS_MTI_LAST_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    /* MIB counters don't support single counter read, clear on read configuration
       and capture. We emulate this features using shadow MIB entries. */
    mibShadowPtr = PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMibShadowArr[portMacNum];

    /* Reading MTI RX mibs from HW */
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 0, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &rxMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read RX MTI counters");
    }
    rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 1, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_BOTH_E, &txMtiMacMibsCaptureArr[0]);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Failed to read TX MTI counters");
    }

    if(cntrName < CPSS_MTI_TX_ETHER_STATS_OCTETS_E)
    {
        offset = (GT_U32) cntrName;
        cntrValuePtr->l[0] = rxMtiMacMibsCaptureArr[offset].l[0];
        cntrValuePtr->l[1] = rxMtiMacMibsCaptureArr[offset].l[1];
    }
    else
    {
        offset = (GT_U32) (cntrName - CPSS_MTI_TX_ETHER_STATS_OCTETS_E);
        cntrValuePtr->l[0] = txMtiMacMibsCaptureArr[offset].l[0];
        cntrValuePtr->l[1] = txMtiMacMibsCaptureArr[offset].l[1];
    }
    ...
    if (mibShadowPtr->clearOnReadEnable == GT_TRUE)
    {
        ...
        */
        ...
        /* add to shadow */
        prvCpssDxChPortMtiSumMibEntry(devNum,mibShadowPtr->mibMtiShadow,rxMtiMacMibsCaptureArr,txMtiMacMibsCaptureArr);
        /* Clearing the specific entry in the shadow */
        offset = (GT_U32) cntrName;
        mibShadowPtr->mibMtiShadow[offset].l[0] = 0;
        mibShadowPtr->mibMtiShadow[offset].l[1] = 0;
    }
    ...
    if ((PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacNum].preemptionSupported))
    {
        ...
        if (U32_GET_FIELD_MAC(regValue, 24, 1) && (U32_GET_FIELD_MAC(regValue, 0, 1))) /*aggregated mode and preemption is enabled*/
        {
            /* Read all mac mti mib counters from HW */
            rc = prvCpssDxChSip6PortMacCountersMTIPortReadMem(devNum, portNum, GT_TRUE, 0/*RX MIB*/, CPSS_DXCH_PORT_MAC_BR_COUNTER_TYPE_PMAC_E, &rxMtiMacMibsCaptureArr[0]);
            if (rc != GT_OK)
...
/**
* @internal cpssDxChMacMtiCounterGet function
* @endinternal
*
* @brief   Gets MTI port specific counter.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
* Note: can't work in parallel with cpssDxChMacCounterGet()
*/
GT_STATUS cpssDxChMacMtiCounterGet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum,
    IN  CPSS_PORT_MAC_MTI_COUNTER_ENT   cntrName,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChMacMtiCounterGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, cntrName, cntrValuePtr));

    rc = internal_cpssDxChMacMtiCounterGet(devNum, portNum, cntrName, cntrValuePtr);
