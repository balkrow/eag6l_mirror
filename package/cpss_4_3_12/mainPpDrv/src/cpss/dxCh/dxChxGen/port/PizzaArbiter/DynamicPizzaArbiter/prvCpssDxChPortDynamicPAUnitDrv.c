...
)
{
    ...
    if (PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.isInitilaized) == GT_FALSE)
    {
    ...
    *unitDescrPtrPtr = &PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paUnitDrvList[unitId]);
    return GT_OK;
}
...
)
{
    ...
    {
        for (i = 0 ;
              PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].family) != CPSS_MAX_FAMILY; i++)
        {
            rc = internal_bobcat2PizzaArbiterSpecificPPUnitDescrDrvInit(
                PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].unitDefListPtr),
                CPSS_DXCH_PA_UNIT_MAX_E,
                /*OUT*/PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].unitsDescrAllPtr));
            if (rc != GT_OK)
            {
                return rc;
            }
        }
        PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.isInitilaized) = GT_TRUE;
    }
    /*-------------------------------*
     * find for specific device      *
     *-------------------------------*/
    if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paUnitDrvList)== NULL)
    {
        for (i = 0 ;
              PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].family) != CPSS_MAX_FAMILY; i++)
        {
            if (PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].family)
                    == PRV_CPSS_PP_MAC(devNum)->devFamily
                && PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].subfamily)
                    == PRV_CPSS_PP_MAC(devNum)->devSubFamily)
            {
                break;
            }
        }
        if (PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].family) == CPSS_MAX_FAMILY)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
        }
        PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paUnitDrvList)=
            PRV_NON_SHARED_PA_DB_VAR(prv_paUnitsDrv.dev_x_unitDefList[i].unitsDescrAllPtr);
    }
