...
)
{
    ...
    /* run callback*/
    if ((portMacObjPtr != NULL) && (portMacObjPtr->macDrvMacSpeedSetFunc))
    {
            ...
            if(PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum,portMacNum) == PRV_CPSS_PORT_GE_E)
            {
                ...
                if((PRV_CPSS_PP_MAC(devNum)->devFamily > CPSS_PP_FAMILY_CHEETAH3_E)
                    && ((CPSS_PORT_INTERFACE_MODE_1000BASE_X_E == PRV_CPSS_DXCH_PORT_IFMODE_MAC(devNum,portMacNum))
                        || (CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E == PRV_CPSS_DXCH_PORT_IFMODE_MAC(devNum,portMacNum))
...
)
{
    ...
    switch(ifMode)
    {
        ...
        case CPSS_PORT_INTERFACE_MODE_QSGMII_E:
            ...
            break;
        case CPSS_PORT_INTERFACE_MODE_SGMII_E:
            if (PRV_CPSS_DXCH_PORT_SPEED_MAC(devNum, portMacNum) == CPSS_PORT_SPEED_2500_E)
            {
...
)
{
    ...
    /* Port WithoutMac has MAP.interfaceNum but not portMap.macNum=-1 so
     * bypass for _MAC_IN_PHY_ checker here and in further procedures is needed.
     * Also needed bypass for ifMode/speed converters.
     * Call for special Get() - if the port is indeed PcaLoopback the GT_OK and speed
     * are returned, otherwise continue ahead for port with MAC.
     */
    rc = prvCpssDxChPortPcaLoopbackSpeedGet(devNum, portNum, speedPtr);
    if (rc == GT_OK)
    {
        return rc;
    }
    ...
    /* run MACPHY callback */
    if ((portMacObjPtr != NULL) && (portMacObjPtr->macDrvMacSpeedGetFunc))
    {
...
speedGet:
    switch(portMode)
    {
        ...
        case _800GBase_KR8:
            *speedPtr = CPSS_PORT_SPEED_800G_E;
            break;
