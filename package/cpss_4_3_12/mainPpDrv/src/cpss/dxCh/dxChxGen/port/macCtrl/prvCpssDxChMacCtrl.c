...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E
                                            ...
                                            | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    ...
    /* Restrictions for short/no preamble */
    if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
    {
        ...
        if (mvHwsUsxModeCheck(devNum,portMacNum,portMode))
        {
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "Short preamble is not allowed in USX modes");
            }
            else
            {
                return GT_OK;
    ...
    else if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
    {
        if (portMacNum <= 49)
        {
            if((length == 1) || (length == 4))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "Short preamble is not allowed for this port");
            }
            else
            {
                return GT_OK;
