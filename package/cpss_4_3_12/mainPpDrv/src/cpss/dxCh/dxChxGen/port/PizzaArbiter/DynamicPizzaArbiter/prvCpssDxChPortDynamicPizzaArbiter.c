...
GT_STATUS prvCpssDxChPortPizzaArbiterAllUnitsDrvInit
(
    IN GT_U8                 devNum,
    IN GT_BOOL               initHw
)
{
    ...
    for (unitIdx = 0 ; paWsPtr->prv_DeviceUnitListPtr[unitIdx] != CPSS_DXCH_PA_UNIT_UNDEFINED_E; unitIdx++)
    {
        ...
        rc = bobcat2PizzaArbiterUnitDrvInit(devNum,unitType,initHw);
        if (GT_OK != rc)
        {
            return rc;
        }
...
GT_STATUS prvCpssDxChPortPizzaArbiterAllUnitsDrvSwOnlyInit
(
    IN GT_U8                 devNum
)
{
    return prvCpssDxChPortPizzaArbiterAllUnitsDrvInit(devNum,GT_FALSE);
}
...
)
{
    ...
    rc = prvCpssDxChPortPizzaArbiterAllUnitsDrvInit(devNum,GT_TRUE);
    if (GT_OK != rc)
    {
        return rc;
    }
