...
*
* @note If PFC response is enabled, the shaper's baseline must
*       be at least 0x3FFFC0, see:
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
          CPSS_XCAT3_E | CPSS_AC5_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
          CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    if(portNum >=512/*size of PFCSourcePortToPFCIndexMap*/)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }
...
)
{
  ...
  CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
  CPSS_LOG_API_ENTER_MAC((funcId, devNum, dataPathBmp, availableBuffPtr));

  rc = internal_cpssDxChPortPfcDbaAvailableBuffGet(devNum,dataPathBmp, availableBuffPtr);

  CPSS_LOG_API_EXIT_MAC(funcId, rc);
  CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, dataPathBmp, availableBuffPtr));
  CPSS_API_UNLOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
  ...
  PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
  PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
{    GT_STATUS                       rc;
    ...
    CPSS_NULL_PTR_CHECK_MAC(hysteresisCfgPtr);
     ...
     if(GT_TRUE == PRV_CPSS_DXCH_ERRATA_GET_MAC(devNum,PRV_CPSS_DXCH_FALCON_FOUR_TXQ_POOLS_SUPPORT_E))
     {
        ...
        {
            if(0!=thresholdCfgPtr->guaranteedThreshold)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "guaranteedThreshold should be zero in pool mode %d",
                    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode);
            }
        }
     }
...
)
{
    ...
    rc = prvCpssFalconTxqQfcGlobalTcPfcThresholdGet(devNum,tc,0,0,enablePtr,thresholdCfgPtr,hysteresisCfgPtr);
...
)
{
    ...
    GT_U32                          tcBitVecEn=0;
    GT_BOOL                         trigger = GT_TRUE,statisticMode = GT_FALSE;
    ...
    else
    {
        ...
        rc = prvCpssFalconTxqPfcValidGet(devNum, tileNum,dpNum, localdpPortNum,&tcBitVecEn);
        if (rc != GT_OK)
        {
        ...
        /*set at leaset one tc so flow control will be triggered for global TC*/
        if(0==tcBitVecEn)
        {
            rc = prvCpssDxChPortSip6FlowControlModeGet(devNum,portNum,&fcMode);
            if(rc!=GT_OK)
            {
              CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortSip6FlowControlModeGet failed for port %d",portNum);
            }

            if (CPSS_DXCH_PORT_FC_MODE_DISABLE_E!=fcMode)
            {
               tcBitVecEn =0xFF;
               trigger = GT_TRUE;
            }
            ...
            if(0==tcBitVecEn)
            {
               ...
               if(rc!=GT_OK)
               {
                 CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip6TxqPfcStatisticModeGet failed for localdpPortNum %d",localdpPortNum);
               }
    ...
    /*Set PFCC*/
    rc = prvCpssFalconTxqPffcTableSyncSet(devNum,tileNum,dpNum,localdpPortNum,tcBitVecEn,trigger);
    if(rc!=GT_OK)
    {
...
)
{
   ...
   /*Check if the PFCC entry exist already*/
        ...
        if(entry.entryType == PRV_CPSS_PFCC_CFG_ENTRY_TYPE_PORT)
        {
            ...
            /*Entry exist .Set PFCC only if bit is not set*/
                if(((entry.tcBitVecEn)&(1<<tc))!= 0)
                {
                    ...
                    if(entry.tcBitVecEn==0)
                    {
                        /*check on which TC threshold is configured*/
                        rc = prvCpssFalconTxqPfcValidGet(devNum, tileNum,dpNum, localdpPortNum,&entry.tcBitVecEn);
                        if (rc != GT_OK)
                        {
                             CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"prvCpssFalconTxqPfcValidGet failed");
                        }
    ...
    {
        rc = prvCpssFalconTxqPffcTableSyncSet(devNum,tileNum,dpNum,localdpPortNum,entry.tcBitVecEn,GT_TRUE);
        if(rc!=GT_OK)
        {
          CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssFalconTxqPffcTableSyncSet failed for portNum %d",portNum);
        }
...
)
{
  ...
  CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
  CPSS_LOG_API_ENTER_MAC((funcId,devNum,portNum,tc,enable,thresholdCfgPtr));
...
)
{
  ...
  CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
  CPSS_LOG_API_ENTER_MAC((funcId,devNum,portNum,tc,enablePtr,thresholdCfgPtr));

  rc = internal_cpssDxChPortPfcPortTcThresholdGet(devNum,portNum,tc,enablePtr,thresholdCfgPtr);

  CPSS_LOG_API_EXIT_MAC(funcId,rc);
  CPSS_APP_SPECIFIC_CB_MAC((funcId,rc,devNum,portNum,tc,enablePtr,thresholdCfgPtr));
  CPSS_API_UNLOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
   ...
   /*TC validity is checked inside prvCpssFalconTxqSdqPfcControlEnableSet*/
    ...
    rc = prvCpssFalconTxqQfcLocalPortToSourcePortGet(devNum,tileNum,dpNum,localdpPortNum[0],NULL,&currentFcMode);
    if(rc!=GT_OK)
    {
      CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssFalconTxqQfcLocalPortToSourcePortGet failed for portNum %d",portNum);
    }

    if(CPSS_DXCH_PORT_FC_MODE_PFC_E == currentFcMode)
    {
        /*Extra messures in order to avoid corner cases.
                   Disable zero override of enabled SDQ TCs
                */
        rc = prvCpssDxChPortSip6PfcSyncMac(devNum,tileNum,dpNum,localdpPortNum[0],portNum,GT_FALSE);
        if(rc!=GT_OK)
        {
    ...
    /*check if port in preemptive mode*/
     ...
     if (GT_TRUE == portInPreemptiveMode)
     {
         ...
         /*No need to sync MAC since same MAC is used for both EXP and PRE channels*/
     }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_AC3X_E |
        CPSS_ALDRIN2_E|CPSS_IRONMAN_E | CPSS_AAS_E);
    ...
    for(ii = 0;dataPathBmp&& (ii < lDpSize); ++ii,dataPathBmp>>=1)
    {
        if(dataPathBmp&1)
        {
          ...
          {  /*Find at least one port mapped per DP*/
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        ...
        CPSS_AC3X_E | CPSS_ALDRIN2_E|CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    {
        /*Get pool id*/
        rc = prvCpssFalconTxqPfccMapTcToPoolGet(devNum,PRV_CPSS_DXCH_FALCON_TXQ_PFCC_MASTER_TILE_MAC,tc,&poolId);
        if(rc!=GT_OK)
        {
...
)
{
    ...
    if( poolId >= SIP6_SHARED_TOTAL_POOLS_NUM_CNS(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
...
)
{
    ...
    GT_U32 tcBmp,i;
    CPSS_DXCH_PORT_FC_MODE_ENT fcMode;
    GT_BOOL statisticMode,trigger = GT_TRUE;
    ...
    else
    {
        ...
        if(0==tcBmp)
        {
            ...
            {
               /*check if this is a statistic mode*/
               rc = prvCpssSip6TxqPfcStatisticModeGet(devNum,tileNum,dpNum,localPort,&statisticMode);
               if(rc!=GT_OK)
               {
                 CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip6TxqPfcStatisticModeGet failed for localdpPortNum %d",localPort);
               }
    ...
    rc = prvCpssFalconTxqPffcTableSyncSet(devNum,tileNum,dpNum,localPort,tcBmp,trigger);
    if(rc!=GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqPffcTableSyncSet  failed for portNum  %d  ",portNum);
    }
