...
)
{
    ...
    */
    ...
    /* Port in MAC loopback can not work in InBand Autoneg - if port
       connected to PHY and was configured as auto-neg=TRUE - need to disable it(inband autoneg) */
    portParamsPtr->portParamsType.regPort.portAttributes.validAttrsBitMask &= ~CPSS_PM_PORT_ATTR_AUTO_NEG_ENABLE;

    /*This code runs in UT and without cables - in BC3 and Aldrin2 Align lock will not arrive on loobpack (with regular traffic) */
    /* so for those devices need to run with RegularTraffic == FALSE */
    /*Applies to BC3 & Aldrin2 devices only*/
    if ((PRV_CPSS_SIP_5_20_CHECK_MAC(devNum)) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum)))
    {
        portParamsPtr->portParamsType.regPort.portAttributes.loopback.enableRegularTraffic = GT_FALSE;
    }
...
)
{
    ...
    portParams.portParamsType.regPort.portAttributes.loopback.enableRegularTraffic = enableRegularTraffic;
...
/**
* @internal
*           cpssDxChSamplePortManagerStaticOverANPSet function
* @endinternal
*
* @brief   set PM over FW enable in the pm database
*
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[in] enableStaticOverANP   - enable enableStaticOverANP
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChSamplePortManagerStaticOverANPSet
(
    IN  GT_U8                  devNum,
    IN  GT_PHYSICAL_PORT_NUM   portNum,
    IN  GT_BOOL                enableStaticOverANP
)
{
    CPSS_PM_PORT_PARAMS_STC portParams;
    GT_STATUS rc = GT_OK;
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && (!PRV_CPSS_SIP_6_30_CHECK_MAC(devNum)))
    {
        ...
        portParams.portParamsType.regPort.portAttributes.staticOverANP  = enableStaticOverANP;
    }
    else
    {
    ...
    if ((PRV_CPSS_SIP_6_CHECK_MAC(devNum)) || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
    {
        ...
        switch (portParamsInputStcPtr->type)
        {
            ...
            case CPSS_PORT_SERDES_COMPHY_C12G_E:
                ...
                break;
           case CPSS_PORT_SERDES_COMPHY_C112G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].txParams.txTune.comphy_C112G),
                             &(portParamsInputStcPtr->txTune.comphy_C112G),
                             sizeof(CPSS_PORT_COMPHY_C112G_TX_CONFIG_STC));
                break;
            case CPSS_PORT_SERDES_COMPHY_C28G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].txParams.txTune.comphy_C28G),
                             &(portParamsInputStcPtr->txTune.comphy_C28G),
                             sizeof(CPSS_PORT_COMPHY_C28G_TX_CONFIG_STC));
                break;
            case CPSS_PORT_SERDES_COMPHY_C56G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].txParams.txTune.comphy_C56G),
                             &(portParamsInputStcPtr->txTune.comphy_C56G),
                             sizeof(CPSS_PORT_COMPHY_C56G_TX_CONFIG_STC));
                break;
            default:
    ...
    if ((PRV_CPSS_SIP_6_CHECK_MAC(devNum)) || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
    {
        ...
        switch (portParamsInputStcPtr->type)
        {
            ...
            case CPSS_PORT_SERDES_COMPHY_C12G_E:
                ...
                break;
            case CPSS_PORT_SERDES_COMPHY_C112G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].rxParams.rxTune.comphy_C112G),
                             &(portParamsInputStcPtr->rxTune.comphy_C112G),
                             sizeof(CPSS_PORT_COMPHY_C112G_RX_CONFIG_STC));
                break;
            case CPSS_PORT_SERDES_COMPHY_C28G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].rxParams.rxTune.comphy_C28G),
                             &(portParamsInputStcPtr->rxTune.comphy_C28G),
                             sizeof(CPSS_PORT_COMPHY_C28GP4_RX_CONFIG_STC));
                break;
            case CPSS_PORT_SERDES_COMPHY_C56G_E:
                cpssOsMemCpy(&(portParams.portParamsType.apPort.apAttrs.overrideLaneParams[laneSpeed][laneNum].rxParams.rxTune.comphy_C56G),
                             &(portParamsInputStcPtr->rxTune.comphy_C56G),
                             sizeof(CPSS_PORT_COMPHY_C56G_RX_CONFIG_STC));
                break;
            default:
...
/**
* @internal
*           cpssDxChSamplePortManagerApTxPostAdaptEnable function
* @endinternal
*
* @brief   Enable Tx post-tap adaptation during Tx training
*
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[in] enable                - enable tx adapt
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChSamplePortManagerApTxPostAdaptEnable
(
    IN  GT_U8                                       devNum,
    IN  GT_PHYSICAL_PORT_NUM                        portNum,
    IN  GT_BOOL                                     enable
)
{
    ...
    /* this sample is supported only in some devices*/
    ...
    portParams.portParamsType.apPort.apAttrs.apTxPostAdapt = enable;
...
/**
* cpssDxChSamplePortManagerDuplexModeSet function
*
* @brief   defines duplex mode for the case when AN is disabled
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - port number
* @param[in] duplexMode               - duplex mode (for AN (clause37) disabled)
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
GT_STATUS cpssDxChSamplePortManagerDuplexModeSet
(
    IN  GT_U8                     devNum,
    IN  GT_PHYSICAL_PORT_NUM      portNum,
...
)
{
    ...
    if (CPSS_PORT_HALF_DUPLEX_E < duplexMode)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    /* duplex configuration is relevant only for 10M/100M + SGMII/QSGMII/OUSGMII) */
    if((portParams.portParamsType.regPort.ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E)  ||
       (portParams.portParamsType.regPort.ifMode == CPSS_PORT_INTERFACE_MODE_QSGMII_E) ||
       (portParams.portParamsType.regPort.ifMode == CPSS_PORT_INTERFACE_MODE_USX_QUSGMII_E) ||
       (portParams.portParamsType.regPort.ifMode == CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E))
    {
        if((portParams.portParamsType.regPort.speed == CPSS_PORT_SPEED_10_E) ||
           (portParams.portParamsType.regPort.speed == CPSS_PORT_SPEED_100_E))
        {
    ...
    portParams.portParamsType.regPort.portAttributes.duplexMode = duplexMode;
...
/**
* cpssDxChSamplePortManagerExternalSigDetCallbackEnableSet function
*
* @brief   defines special mode for over-FM when sigdet callback provided by customer
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - port number
* @param[in] extSigDetCallbackEnable  - ext sig det
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
GT_STATUS cpssDxChSamplePortManagerExternalSigDetCallbackEnableSet
(
    IN  GT_U8                     devNum,
    IN  GT_PHYSICAL_PORT_NUM      portNum,
...
)
{
    ...
    portParams.portParamsType.regPort.portAttributes.extSigDetCallbackEnable = extSigDetCallbackEnable;
