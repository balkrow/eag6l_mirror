...
)
{
    ...
    switch(convertedPortType)
    {
        ...
        case PRV_CPSS_DXCH_PORT_TYPE_MAC_E:
            ...
            if(PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMapInfoShadowArr[physicalPortNum].portMap.mappingType == CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E)
            {
                /* Loopback port has no MAC */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "physicalPortNum[%d] : is 'Loopback SDMA' so hold no 'MAC'",
                    physicalPortNum);
            }
            else
...
)
{
     ...
     *----------------------------------------------------------------------------------------------*/
    if (origPortType == PRV_CPSS_DXCH_PORT_TYPE_MAC_E || origPortType == PRV_CPSS_DXCH_PORT_TYPE_SERDES_E)
    {
        GT_U32 macNum = 0xFFFF;

        if (origPortType == PRV_CPSS_DXCH_PORT_TYPE_SERDES_E)
        {
            /* convert serdes to mac and then use the mac to phy port*/
            CHECK_STATUS(mvHwsSerdesGetInfoBySerdesNum(devNum,origPortNum /* serdesNum */,&serdesIndex,&serdesInfo));
            for (i = 0; i < MV_HWS_MAX_LANES_NUM_PER_PORT; i++)
            {
                if (serdesInfo->serdesToMac[i] != NA_NUM)
                {
                    macNum = serdesInfo->serdesToMac[i];
                    break;
                }
            }

            /* in case that there is no serdes to mac in any index in the array, return GT_NO_SUCH*/
            if (i >= MV_HWS_MAX_LANES_NUM_PER_PORT)
            {
                return /* not error for the CPSS LOG */ GT_NO_SUCH;
            }
        }
        else
        {
...
/**
* @internal prvCpssDxChSdmaPortNumGet function
* @endinternal
 *
* @brief    Function get the number of port that have mapping type CPSS_DXCH_PORT_MAPPING_TYPE_CPU_SDMA_E
 *
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @param[in] devNum                   - device number
* @param[out] sdmaPortNumPtr          -  number of SDMA poorts mapped
*/
GT_STATUS prvCpssDxChSdmaPortNumGet
(
    IN GT_SW_DEV_NUM                    devNum,
    OUT GT_U32                          *sdmaPortNumPtr
)
{
    CPSS_NULL_PTR_CHECK_MAC(sdmaPortNumPtr);
    *sdmaPortNumPtr = PRV_CPSS_PP_MAC(devNum)->sdmaPortNum;
    return GT_OK;

}
/**
* @internal prvCpssDxChSdmaPhysPortGet function
* @endinternal
 *
* @brief    Function get the physical port nmber of SDMA port at specific index
 *
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2;Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
 *
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @param[in] devNum                   - device number
* @param[in] index                    - index of SDMA port
* @param[out] sdmaPhysPortPtr         - (pointer to) physical port number of type SDMA
*/

GT_STATUS prvCpssDxChSdmaPhysPortGet
(
    IN GT_SW_DEV_NUM                    devNum,
    IN GT_U32                           index,
...
{
    CPSS_NULL_PTR_CHECK_MAC(sdmaPhysPortPtr);
    if(index>=CPSS_MAX_SDMA_PORT_NUM_CNS )
    {
         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "index %d is too big.Must be less then %d", index,CPSS_MAX_SDMA_PORT_NUM_CNS);
    }

     if(index>=PRV_CPSS_PP_MAC(devNum)->sdmaPortNum )
    {
         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_FOUND, "CPU port index %d not found", index);
    }

    *sdmaPhysPortPtr = PRV_CPSS_PP_MAC(devNum)-> sdmaPorts[index];
    return GT_OK;
