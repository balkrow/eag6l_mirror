...
)
{
    ...
    paWsPtr = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr);
...
)
{
    ...
    *devBwPtrPtr = (PRV_CPSS_DXCH_CC_x_DEVICE_BW_STC*)NULL;
    ...
    rev_x_devBWPtr = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr->devBwPtr);
...
)
{
    ...
    {
        *cpuSpeedEnmPtr = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr->cpuSpeedEnm);
    }
...
)
{
    ...
    if (GT_FALSE == prvCpssDxChPortPaWorSpaceInitStatusGet(
        &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet)))
    {
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC2_with_ilkn),              &prv_bc2_ws_init_params_with_ilkn);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC2_wo_ilkn),                &prv_bc2_ws_init_params_no_ilkn);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BobK_Caelum_pipe0_pipe1),    &prv_bobk_caelum_pipe_0_pipe_1);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BobK_Cetus_pipe1),           &prv_bobk_cetus_pipe_1);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_Aldrin),                     &prv_aldrin_ws_init);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_AC3X),                       &prv_alleyCat3X_ws);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_AC3plus),                    &prv_alleyCat3plus_ws);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC3),                        &prv_bc3_ws_init_params);
        prvCpssDxChPortDynamicPizzaArbiterWSByParamsInit(
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_ALDRIN2),                    &prv_aldrin2_ws_init_params);

        /* not relevant to sip6 devices */

        prvCpssDxChPortPaWorSpaceInitStatusSet(&PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet));
    }


    pDev = PRV_CPSS_DXCH_PP_MAC(devNum);

    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) = NULL; /* PA data still are not selected */
    if (PRV_CPSS_DXCH_BOBCAT2_CHECK_MAC(devNum))
    {
        if (pDev->hwInfo.gop_ilkn.supported)
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr )=
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC2_with_ilkn);
        }
        else
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC2_wo_ilkn);
        }
    }
    else if (PRV_CPSS_DXCH_CETUS_CHECK_MAC(devNum))
    {
        ...
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr)=
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BobK_Cetus_pipe1);
        }
        else
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
    ...
    else if (PRV_CPSS_DXCH_CAELUM_CHECK_MAC(devNum))
    {
        ...
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr )=
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BobK_Caelum_pipe0_pipe1);
        }
        else
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr )=
    ...
    else if(PRV_CPSS_DXCH_ALDRIN_CHECK_MAC(devNum))
    {
        ...
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr)=
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_Aldrin);
        }
        else
        if (PRV_CPSS_PP_MAC(devNum)->devType == CPSS_98DX3248_CNS)
        {
            ...
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_AC3plus);
        }
        else
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
                &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_AC3X);
        }
    }
    else if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT3_E)
    {
        PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_BC3);
    }
    else if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN2_E)
    {
        PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr) =
            &PRV_NON_SHARED_PA_DB_VAR(prv_paWsSet.paWorkSpace_ALDRIN2);
    }

    /* selection was done ? */
    if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].paWsPtr)== NULL)
    {
