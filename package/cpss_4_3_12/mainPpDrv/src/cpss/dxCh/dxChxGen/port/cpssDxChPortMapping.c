...
/**
* @internal prvCpssDxChPortMappingRxdmaToLocalDevPortNumSet function
* @endinternal
*
* @brief   sip6,7 : configure RxDMA 2 phys port mapping , in sip7 also the HBU mapping
*
...
* @param[in] localDevSrcPortNum    - the physical port
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong devNum
* @retval GT_BAD_PTR               - on bad pointer
*/
GT_STATUS prvCpssDxChPortMappingRxdmaToLocalDevSrcPortNumSet
(
    IN GT_U8  devNum,
    IN GT_U32 dpIndex,
...
{
    GT_STATUS   rc;
    GT_U32  regAddr,globalDmaNum,index,bitOffset;
    GT_U32  portGroupId = 0;

    rc = prvCpssDxChHwPpDmaLocalNumInDpToGlobalNumConvert(devNum,dpIndex,localDmaNum,&globalDmaNum);
    if (GT_OK != rc )
    {
        return rc;
    }

    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum)) /* used for traffic that using PCH */
    {
        bitOffset = 7;

        if(localDmaNum > BIT_MASK_MAC(bitOffset))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                "localDmaNum[%d] must be <= [%d] \n",
                localDmaNum , BIT_MASK_MAC(bitOffset));
        }

        index =  (localDmaNum & BIT_MASK_MAC(bitOffset)) | ((dpIndex & 0x3) << bitOffset);

        rc = prvCpssDxChWriteTableEntry(devNum,
            CPSS_DXCH_SIP7_TABLE_HBU_PORT_MAPPING_E,
            index,/*index*/
            &localDevSrcPortNum /*value*/);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_MAC(devNum).configs.channelConfig.channelToLocalDevSourcePort[globalDmaNum];
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        {
            /* move to dedicated 'per source port' table ! */
            rc = prvCpssDxChWriteTableEntryField(devNum,
                 CPSS_DXCH_SIP7_TABLE_HA_SRC_PHYSICAL_PORT_2_E,
                 physPort, /*global port*/
                 PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                 SIP7_HA_SRC_PHYSICAL_PORT_2_TABLE_FIELDS_INVALID_CRC_MODE_E, /* field name */
        ...
        /* <Recalc CRC> */
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
            !PRV_CPSS_PP_MAC(devNum)->isGmDevice)/*GM uses sip6 config*/
        {
            rc = prvCpssDrvHwPpPortGroupSetRegField(
                devNum, portGroupId, regAddr, 4, 1,
                RecalcCRC/* The field moved in AAS */);
        }
        else
        {
            rc = prvCpssDrvHwPpPortGroupSetRegField(
                devNum, portGroupId, regAddr, 0, 1,
        ...
        if(localDmaNum == PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.info[dpIdx].cpuPortDmaNum)
        {
            ...
            if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) && isSdmaCpuPort == GT_TRUE)
            {
                rc = prvCpssHwPpSetRegField(devNum,regAddr, 1, 5, localDmaNum);
            }
            else
        ...
        /* support ONLY AC5X with second CPU SDMA (in single TxFifo unit) */
        if((PRV_CPSS_SIP_6_15_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_20_CHECK_MAC(devNum)) &&
...
)
{
        ...
        !PRV_CPSS_PP_MAC(devNum)->isGmDevice)/*GM uses sip6 config*/
    {
        regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_MAC(devNum).
            configs.channelConfig.channelGeneralConfigs[rxDmaNum];
        /*<Ingress Packet Include PCH>*/
        rc = prvCpssHwPpSetRegField(devNum,regAddr, 1, 1, 0/*disable the PCH*/);
    }
    else
    {
        regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_MAC(devNum).
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        rc = prvCpssDxChWriteTableEntryField(devNum,
             ...
             SIP7_HA_SRC_PHYSICAL_PORT_2_TABLE_FIELDS_INVALID_CRC_MODE_E, /* field name */
             PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
             haRecalCrc /* like in the RxDma unit */);
    }
    else
    {
        /* NOTE: this field is per ingress port (unlike most bits in this table) */
        rc = prvCpssDxChWriteTableEntryField(devNum,
             ...
             SIP5_HA_PHYSICAL_PORT_TABLE_2_FIELDS_INVALID_CRC_MODE_E, /* field name */
             PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
             haRecalCrc /* the CPU sends with 4 bytes of CRC, For ironman - without CRC */);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
        !PRV_CPSS_PP_MAC(devNum)->isGmDevice)/*GM uses sip6 config*/
    {
        /* make sure that the value hold proper value , as default of register
           was 0x22 instead of 22
           For sdma channel RxDMA uses register value as ch_id and not the one
           driven from brg
        */

        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->sip6_rxDMA[dpIdx].
            configs.dataAccumulator.sdmaPortConfig[0];
        /*<SDMA port channel ID>*/
        rc = prvCpssHwPpSetRegField(devNum,regAddr, 0, 6, localDmaNum);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        GT_U32  BWC_Value = txqNum; /* BWC logic - till fix WM logic*/
...
)
{
    ...
    else
    {
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            ...
            (*txqNumPtr) >>= 0;/* BWC logic - till fix WM logic*/
        }
        else
        {
            rc = prvCpssDxChReadTableEntryField(
...
static PRV_DXCH_CPSS_MAPPING_SET_GET_FUN_STC prv_MappingFunList[] =
{
    ...
    },
    {
        ...
        /*  GT_U32                             applicableDevice; */ CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        srcPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_RxDMA_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        dstPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_PHYS_E,
        /*  PRV_DXCH_CPSS_MAPPING_SET_FUN_PTR  setFun;           */ prvCpssDxChPortMappingRxDMA2LocalPhysSet,
        /*  PRV_DXCH_CPSS_MAPPING_GET_FUN_PTR  getFun;           */ prvCpssDxChPortMappingRxDMA2LocalPhysGet
    ...
    /* 3 BMA map of Local Device Physical Port to RX DMA number */
    {
        ...
        /*  GT_U32                             applicableDevice; */ CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        srcPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_PHYS_E,
        /*  PRV_CPSS_DXCH_PORT_TYPE_ENT        dstPortType;      */ PRV_CPSS_DXCH_PORT_TYPE_RxDMA_E,
...
)
{
    ...
    switch (systemMappingType)
    {
        ...
        case CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E: /* LB Port's HW/DMA is same as ETH using RxDMA2LocalPhysSet */
...
)
{
    ...
    if (portMapPtr->mappingType == CPSS_DXCH_PORT_MAPPING_TYPE_CPU_SDMA_E)
    {
        ...
        for(i=0;i<PRV_CPSS_PP_MAC(devNum)->sdmaPortNum;i++)
        {
            if(PRV_CPSS_PP_MAC(devNum)->sdmaPorts[i]==portMapPtr->physicalPortNumber)
            {
                break;
            }
        }
        if(i==PRV_CPSS_PP_MAC(devNum)->sdmaPortNum)
        {
            PRV_CPSS_PP_MAC(devNum)->sdmaPorts[PRV_CPSS_PP_MAC(devNum)->sdmaPortNum] = portMapPtr->physicalPortNumber;
            PRV_CPSS_PP_MAC(devNum)->sdmaPortNum++;
        }
    }
    ...
    else
    {
        ...
        if (portMapPtr->mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E)
        {
...
*                                       None.
*/
GT_VOID prvCpssDxChPortPrintPfcRanges
(
    GT_U8   devNum
)
{
...
)
{
    ...
    {
        rc = prvDxChTxqSip7AllocManagerAddResourceRange(&PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).cncResources,
            PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).globalQueueIndexIterator,(1<<13)-1/*13 bits in PreQ*/);
        if (rc != GT_OK)
        {
...
)
{
    ...
    CPSS_SYSTEM_RECOVERY_INFO_STC           tempSystemRecovery_Info,newSystemRecoveryInfo;
    ...
    /*----------------------------------*/
        ...
        prv_mappingTypeStr[CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E     ]        = "LOOPBACK";
    #endif
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        /*First build data base*/
         for(i=0;i<numberOfTiles;i++)
         {
             if (tempSystemRecovery_Info.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E)
                {
                    /*in case of HS we dont want Txq to write to HW during traffic - CPSS-14491 */
              ...
              rc = prvCpssFalconTxqUtilsInitDb(devNum,tileConfigsPtr,i,numberOfTiles);
              if (rc != GT_OK)
              {
        ...
        for(i=0;i<numberOfTiles;i++)
        {
            ...
            tileConfigsPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->port.tileConfigsPtr[i]);
            PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode = CPSS_PORT_TX_POOL_MODE_TWO_LOSSY_E;
            PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).spPrecedence = GT_FALSE;
...
/**
* @internal internal_cpssDxChPortPhysicalPortMapWithoutMacGet function
* @endinternal
*
* @brief   Check if physical port mapped without MAC (as CPU port or Loopback port)
*
...
* @param[in] portNum                  - port number
*
* @param[out] noMacPtr                - (pointer to) is mapped without MAC (as CPU or LBP)
*                                      GT_TRUE - given port has No MAC in map (port is CPU or LBP)
*                                      GT_FALSE - there is no valid mapping for given port,
*                                      or it has MAC (and MAC-info)
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong devNum, portNum
* @retval GT_BAD_PTR               - on bad pointer
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
* @note This is a Generic AAS/SIP7 extension for the cpssDxChPortPhysicalPortMapIsCpuGet.
*       SIP7 predecessors never have PORT_MAPPING_TYPE_LOOPBACK and so the result noMac=isCpu
*/
static GT_STATUS internal_cpssDxChPortPhysicalPortMapWithoutMacGet
(
    IN GT_U8                       devNum,
    IN GT_PHYSICAL_PORT_NUM        portNum,
    OUT GT_BOOL                    *noMacPtr
)
{
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_DXCH_PHY_PORT_NUM_CHECK_MAC(devNum, portNum);
    CPSS_NULL_PTR_CHECK_MAC(noMacPtr);

    if (PRV_CPSS_DXCH_PP_HW_INFO_PORT_MAP_CHECK_SUPPORTED_MAC(devNum)) /* isUsedPortMappingCheck==true */
    {
        if ((PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMapInfoShadowArr[portNum].valid == GT_TRUE)&&
            (PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMapInfoShadowArr[portNum].portMap.mappingType
                                                        == CPSS_DXCH_PORT_MAPPING_TYPE_CPU_SDMA_E ||
             PRV_CPSS_DXCH_PP_MAC(devNum)->port.portsMapInfoShadowArr[portNum].portMap.mappingType
                                                        == CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E))
        {
            *noMacPtr = GT_TRUE;
        }
        else
...
/**
* @internal cpssDxChPortPhysicalPortMapWithoutMacGet function
* @endinternal
*
* @brief   Check if physical port mapped without MAC (as CPU port or Loopback port)
*
...
*       SIP7 predecessors never have PORT_MAPPING_TYPE_LOOPBACK and so the result noMac=isCpu
*/
GT_STATUS cpssDxChPortPhysicalPortMapWithoutMacGet
(
    IN GT_U8                       devNum,
    IN GT_PHYSICAL_PORT_NUM        portNum,
    OUT GT_BOOL                    *noMacPtr
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, noMacPtr));

    rc = internal_cpssDxChPortPhysicalPortMapWithoutMacGet(devNum, portNum, noMacPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, noMacPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    switch (origPortType)
    {
        ...
        case CPSS_DXCH_PORT_MAPPING_TYPE_SERDES_E:

            portType = PRV_CPSS_DXCH_PORT_TYPE_SERDES_E;
            break;
