...
)
{
    ...
    switch (portSpeed)
    {
        ...
        case CPSS_PORT_SPEED_107G_E:
            ...
            maxCellsCredits = 4;
            maxDescCredits = 4;
            interCellRateLimiter = 1370;
            interPacketRateLimiter = 4280;
            break;
...
)
{
    ...
    rc = prvCpssSip6TxqPfcStatisticModeGet(devNum,tileId,localDpId,channelIndex,&statisticMode);
    if(rc!=GT_OK)
    {
      CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip6TxqPfcStatisticModeGet failed for localdpPortNum %d",channelIndex);
    }
    ...
    if(0==tcBmp)
    {
        rc = prvCpssFalconTxqQfcLocalPortToSourcePortGet(devNum,tileId,localDpId,channelIndex,NULL,&fcMode);
        if(rc!=GT_OK)
        {
          CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortSip6FlowControlModeGet failed for channelIndex %d",channelIndex);
        }
     ...
     {
         rc = prvCpssFalconTxqPffcTableExplicitSet(devNum, tileId,localDpId, channelIndex,portPizzaSlices,enable?tcBmp:0,trigger);
         if (rc != GT_OK)
         {
...
)
{
    ...
    /*check if this is a statistic mode*/
    rc = prvCpssSip6TxqPfcStatisticModeGet(devNum,tileId,localDpId,channelDpIndex,&statisticMode);
    if(rc!=GT_OK)
    {
      CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip6TxqPfcStatisticModeGet failed for localdpPortNum %d",channelDpIndex);
    }
    ...
    if(0==tcBmp)
    {
        rc = prvCpssFalconTxqQfcLocalPortToSourcePortGet(devNum,tileId,localDpId,channelDpIndex,NULL,&fcMode);
        if(rc!=GT_OK)
        {
          CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortSip6FlowControlModeGet failed for channelIndex %d",channelDpIndex);
        }
        /*set at leaset one tc so flow control will be triggered for global TC*/
        if(CPSS_DXCH_PORT_FC_MODE_DISABLE_E!=fcMode)
        {
    ...
    if(tcBmp!=0)/*Mean that PFC was configured*/
    {
        rc = prvCpssFalconTxqPffcTableExplicitSet(devNum, tileId,localDpId, channelDpIndex,pizzaSliceBitMap,enable?tcBmp:0,trigger);
        if (rc != GT_OK)
        {
             CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"prvCpssFalconTxqPffcTableExplicitSet failed");
        }
