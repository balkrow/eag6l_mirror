...
)
{
    ...
    GT_U32  localDmaNum = 0xFFFFFFFF; /* local rxdma port number  */
    ...
    {
        GT_U32 neverCountMask = 0x1FFFF;
        GT_U32 fldLength = 17;
        GT_U32 prioPattern =
            counterModeInfoPtr->priorityMask & counterModeInfoPtr->priorityPattern;
        for (ii = 0; (ii < PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.maxDp); ii++)
        {
            for (channel = 0; (channel < PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.rxNumPortsPerDp); channel++)
            {
                for (prio = 0; (prio < 4); prio++)
                {
                    if ((((dpIndex == ii) && (localDmaNum == channel)) || (dpIndex == GT_NA)) &&
                        (prioPattern == (counterModeInfoPtr->priorityMask & prio)))
                    {
                        regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_INDEX_MAC(devNum,ii).
                            debug.rxChannelIngressDropCounter[channel][prio].rx_channel_ingress_drop_count_type_ref;
                        rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, fldLength, pattern);
                        if(rc != GT_OK)
                        {
                            return rc;
                        }
                        ...
                        regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_INDEX_MAC(devNum,ii).
                            debug.rxChannelIngressDropCounter[channel][prio].rx_channel_ingress_drop_count_type_mask;
                        rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, fldLength, mask);
                        if(rc != GT_OK)
                        {
                            return rc;
                        }
                    ...
                    else
                    {
                        regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_INDEX_MAC(devNum,ii).
                            debug.rxChannelIngressDropCounter[channel][prio].rx_channel_ingress_drop_count_type_ref;
                        rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, fldLength, neverCountMask);
                        if(rc != GT_OK)
                        {
                            return rc;
                        }
...
)
{
    ...
    {/*AAS*/
        GT_U32 dp, channel, prio, data;
        for (dp = 0; (dp < PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.maxDp); dp++)
        {
            if (0 == (dataPathBmp & (1 << dp))) continue;

            for (channel = 0; (channel < PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.rxNumPortsPerDp); channel++)
            {
                for (prio = 0; (prio < 4); prio++)
                {
                    regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_INDEX_MAC(devNum,dp).
                        debug.rxChannelIngressDropCounter[channel][prio].rx_channel_ingress_drop_count;
                    rc = prvCpssDrvHwPpReadRegister(devNum, regAddr, &regValue);
                    if (rc != GT_OK)
                    {
                        return rc;
                    ...
                    data = counterPtr->l[0];
                    counterPtr->l[0] += regValue;
                    if (counterPtr->l[0] < data)
                    {/*overflow*/
                        counterPtr->l[1] ++;
                    }
                    regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_INDEX_MAC(devNum, dp).
                        debug.rxChannelIngressDropCounter[channel][prio].rx_channel_ingress_drop_count_high;
                    rc = prvCpssDrvHwPpReadRegister(devNum, regAddr, &regValue);
                    if (rc != GT_OK)
                    {
                        return rc;
                    }
                    counterPtr->l[1] += regValue;
                }
            }
        }
        return GT_OK;
