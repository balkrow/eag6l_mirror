...
/**
* @internal prvCpssDxChPortPizzaArbiterBobcat3_107gBandwidthSet function
* @endinternal
*
* @brief   Set Bobcat3 Arbiter 107G bandwidth
*
* @note   APPLICABLE DEVICES:      Bobcat3.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; AC5; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                   - device number
* @param[in] bandwidthMbPerSec        - port bandwidth in Megabit per second
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong devNum, portNum
* @retval GT_NOT_APPLICABLE_DEVICE - not supported
*/
GT_STATUS prvCpssDxChPortPizzaArbiterBobcat3_107gBandwidthSet
(
    IN  GT_U8       devNum,
    IN  GT_U32      bandwidthMbPerSec
)
{
    PRV_CPSS_GEN_PP_CONFIG_STC* devPtr; /* pointer to device to be processed*/
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    ...
    devPtr = PRV_CPSS_PP_MAC(devNum);
    if (devPtr->devFamily != CPSS_PP_FAMILY_DXCH_BOBCAT3_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
    }
    PRV_CPSS_DXCH_PP_MAC(devNum)->port.bc3PizzaSlots107gBandwidthMbPerSec = bandwidthMbPerSec;
    return GT_OK;
}
...
)
{
    ...
    if (devPtr->devFamily >= CPSS_PP_FAMILY_DXCH_BOBCAT2_E) /* &&      CPSS_PP_FAMILY_DXCH_BOBCAT2_E, */
    {
        ...
        /* Give BC3 107G BW as coonfigured - the bc3PizzaSlots107gBandwidthMbPerSec can be not 0 for BC3 only */
        if ((portSpeed == CPSS_PORT_SPEED_107G_E)
            && (PRV_CPSS_DXCH_PP_MAC(devNum)->port.bc3PizzaSlots107gBandwidthMbPerSec != 0))
        {
            portSpeedInMBit = PRV_CPSS_DXCH_PP_MAC(devNum)->port.bc3PizzaSlots107gBandwidthMbPerSec;
        }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_LION2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
