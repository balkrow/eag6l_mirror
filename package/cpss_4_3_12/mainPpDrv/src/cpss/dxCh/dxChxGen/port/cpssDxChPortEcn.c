...
*                                      Drop Parameters (0..15).( not applicable for AAS device)
* @param[in] tailDropEcnProfileParamsPtr -
*                                      the Drop Profile ECN Parameters to associate
*                                      with the Traffic Class in this Profile set.(Only DP0 is applicable for SIP7 devices)
*
* @retval GT_OK                    - on success
...
)
{
    ...
    GT_U32 dpResolution = 3;/* resolution of drop precedence*/
    GT_U32      hwArray[11] = {0};   /* HW table data */
    ...
    {
        dpResolution = 1;/*No drop precedence resolution on SIP7*/
    ...
    for(i=0;i<dpResolution;i++)
    {
      ...
      else
      {
            ...
            SIP6_PREQ_PROFILE_FIELD_SET_MAC(devNum,hwArray,SIP7_PREQ_Q_PROFILES_TABLE_FIELDS_ECN_ENABLE_E,
                        BOOL2BIT_MAC(dpXEcnAttributesPtr->ecnEnable));
            SIP6_PREQ_PROFILE_FIELD_SET_MAC(devNum,hwArray,SIP7_PREQ_Q_PROFILES_TABLE_FIELDS_ECN_OFFSET_E,
                        dpXEcnAttributesPtr->ecnOffset);
...
*                                      the Drop Profile ECN Parameters to associate
*                                      with the Traffic Class in this Profile.(Only DP0 is applicable for SIP7 devices)
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number, profile set or
    ...
    for(i=0;i<dpResolution;i++)
    {
       switch (i)
        ...
        else
        {
            ...
            SIP6_PREQ_PROFILE_FIELD_GET_MAC(devNum,regData,SIP7_PREQ_Q_PROFILES_TABLE_FIELDS_ECN_ENABLE_E,
                        tmp);
            dpXEcnAttributesPtr->ecnEnable = BIT2BOOL_MAC(tmp);

            SIP6_PREQ_PROFILE_FIELD_GET_MAC(devNum,regData,SIP7_PREQ_Q_PROFILES_TABLE_FIELDS_ECN_OFFSET_E,
                        dpXEcnAttributesPtr->ecnOffset);
        }
