...
static GT_STATUS prvCpssDxChPort_SIP7_ModeSpeedSet
(
    IN  GT_U8                               devNum,
    IN  CPSS_PORTS_BMP_STC                  portsBmp,
    IN  GT_BOOL                             powerUp,
...
static GT_STATUS prvCpssDxChPort_SIP7_ModeSpeedCheck
(
    IN  GT_U8                           devNum,
    IN  CPSS_PORTS_BMP_STC              portsBmp,
    IN  GT_BOOL                         powerUp,
...
)
{
    ...
    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesFreeCheck = prvCpssDxChPortSerdesFreeCheckWrapper;
    return GT_OK;
}
    ...
    switch (ifMode)
    {
        ...
        case CPSS_PORT_INTERFACE_MODE_XHGS_SR_E:
            ...
            case CPSS_PORT_SPEED_47200_E:
                    if ( portMacNum >= 50 && portMacNum <= 53 )
                    {
                        *portTypePtr = PRV_CPSS_PORT_MTI_100_E;
                    }
                    break;
...
)
{
    ...
    switch (ifMode)
    {
        ...
        case CPSS_PORT_INTERFACE_MODE_CR_S_E:
            ...
            if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
            {
                if((speed == CPSS_PORT_SPEED_10000_E) || (speed == CPSS_PORT_SPEED_25000_E) || (speed == CPSS_PORT_SPEED_5000_E) || (speed == CPSS_PORT_SPEED_12000_E))
                {
                    if(mvHwsMtipIsReducedPort(devNum, macNum) == GT_TRUE)
                    {
                        PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[macNum].portType = PRV_CPSS_PORT_MTI_CPU_E;
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    ...
    *fullConfigPtr = GT_TRUE;
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) && !PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
        if (*fullConfigPtr == GT_TRUE)
        {
            ...
            for (i = firstInQuadruplet; i < firstInQuadruplet+4; i++)
            {
                rc = prvCpssDxChHwEgfEftFieldSet(devNum, i,
                                             PRV_CPSS_DXCH_HW_EGF_EFT_FIELD_EGR_FILTER_PHYSICAL_PORT_LINK_STATUS_MASK_E,
                                             value);
                if(rc != GT_OK)
                {
...
)
{
    ...
    if ((CPSS_PORT_SPEED_107G_E == speed) ||
        (CPSS_PORT_SPEED_106G_E == speed) ||
    ...
    if (ifMode > CPSS_PORT_INTERFACE_MODE_NA_E && speed > CPSS_PORT_SPEED_NA_E)
    {
        if ((PRV_CPSS_SIP_5_CHECK_MAC(devNum)) || (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum)))
        {
            ...
            if (apEnabled)
            {
                ...
                if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
                {
                    /* this function supports SIP6 devices only */
...
*         serdeses.
*
* @note   APPLICABLE DEVICES:      xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; AC5; Aldrin2; AAS.
...
)
{
    ...
    {
        rc = prvCpssDxChPort_SIP7_ModeSpeedSet(devNum, *portsBmpPtr, powerUp, ifMode,speed, NULL);
    }
    ...
    if(PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum))
    {
        ...
        */
        if((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E) &&
           ...
           (PRV_SHARED_PORT_DIR_PORT_CTRL_SRC_GLOBAL_VAR_GET(extendedPortDeletedFlag) == GT_FALSE))
        {
            ...
            PRV_SHARED_PORT_DIR_PORT_CTRL_SRC_GLOBAL_VAR_SET(extendedPortDeletedFlag,GT_TRUE);
        }
    ...
    else
    {
        if (PRV_CPSS_PORT_FE_E == PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum,portMacNum))
        {
            ...
            if((CPSS_PORT_INTERFACE_MODE_SGMII_E == ifMode) ||
               ...
               (CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E == ifMode) )
...
hws2cpss:
    switch(portMode)
    {
        ...
        case _1000Base_SX:
            *ifModePtr = CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E;
            break;
...
)
{
    ...
    MV_HWS_PORT_INIT_PARAMS         curPortParams;
    CPSS_PORT_FEC_MODE_ENT          commonFecMode = (CPSS_PORT_FEC_MODE_ENT)mode;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_PM_NOT_APPLICABLE_DEVICE(devNum, CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);

    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacNum);
    PRV_CPSS_CONVERT_CPSS_FEC_MODE_VAL_TO_HWS_MAC(commonFecMode,fecMode);
    if (fecMode == FEC_NA)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
...
)
{
    ...
    if(CPSS_PORT_INTERFACE_MODE_NA_E == PRV_CPSS_DXCH_PORT_IFMODE_MAC(devNum,portMacNum))
    {
        /* the function prvCpssCommonPortIfModeToHwsTranslate is going to fail.
           and we not want the 'error log' indication about it */
        *modePtr = CPSS_DXCH_PORT_FEC_MODE_DISABLED_E;
        return GT_OK;
    }
...
)
{
    ...
    {
        /* race condition in CPSS (code run in host CPU) and FW - delete port on both sides.
        the fix is to disable port delete on CPSS side.*/
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
        {
            /* check if AP is enabled */
            rc = cpssDxChPortApPortEnableGet(devNum, portNum, &apEnabled);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "cpssDxChPortModeSpeedSet - try to set AP port but can not get FW status");
            }

            if(apEnabled == GT_FALSE)
            {
                rc = prvCpssDxChPortBcat2PortReset(devNum, portNum, *portIfModePtr,
                                                   *portSpeedPtr, qsgmiiFullDelete);
                if (rc != GT_OK)
        ...
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            rc = prvCpssDxChPortDpAasPortDown(devNum, portNum);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(
                    rc,"error in prvCpssDxChPortDpAasnPortDown, portNum = %d\n",
                    portNum);
            }
        }
        else if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            ...
            rc = prvCpssSip6_30TxqPizzaPortSpeedSet(devNum, portNum,CPSS_PORT_SPEED_NA_E);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(
                    rc,"error in prvCpssSip6_30TxqPizzaPortSpeedSet, portNum = %d\n",
                    portNum);
            }
        }
        else if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E
                                            | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    if (((PRV_CPSS_SIP_5_15_CHECK_MAC(devNum)) || (CPSS_PP_FAMILY_DXCH_AC5_E == PRV_CPSS_PP_MAC(devNum)->devFamily) ) && (fecMode != CPSS_PORT_FEC_MODE_LAST_E))
    {
        for (portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
        {
            ...
            if (CPSS_PORTS_BMP_IS_PORT_SET_MAC(&portsBmp, portNum))
            {
                PRV_CPSS_CONVERT_CPSS_FEC_MODE_VAL_TO_HWS_MAC(fecMode,hwsFecMode);
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        return prvCpssDxChPort_SIP7_ModeSpeedSet(CAST_SW_DEVNUM(devNum), portsBmp, powerUp, ifMode,speed, portMgrDbPtr);
    }
    else if ( PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) )
    {
...
/**
* @internal prvCpssDxChPortSerdesFreeCheckWrapper function
* @endinternal
*
* @brief   The Wrapper is for check that the Serdes for port
*         configuration is not free and not used by another port
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChPortSerdesFreeCheckWrapper
(
    IN  GT_SW_DEV_NUM                   devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum,
    IN  CPSS_PORT_INTERFACE_MODE_ENT    ifMode,
...
)
{
    ...
    rc = prvCpssDxChPortSerdesFreeCheck(CAST_SW_DEVNUM(devNum), portNum, ifMode, speed);
    if(rc != GT_OK)
    {
         CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"prvCpssDxChPortSerdesFreeCheck : failed for portNum = %d\n", portNum);
    }
    return GT_OK;
}
...
)
{
    ...
    {
        /* mark speeds below as the fastest to cause */
        /* slow to fast cut through terminate        */
        /* all packets at egress of such port will   */
        /* not be cut through                        */
        switch (speed)
        {
            case CPSS_PORT_SPEED_11800_E: speedIndex = 31; break;
            case CPSS_PORT_SPEED_47200_E: speedIndex = 31; break;
            default: break;
        }
    }
    ...
    for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        *portSpeedPtr = CPSS_PORT_SPEED_NA_E;
        ...
        if (PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->mngInterfaceType != CPSS_CHANNEL_PEX_FALCON_Z_E)
        {
            ...
            rc = prvCpssDxChTxqBindPortQueuesToPdsProfile(devNum, portNum,  speed,NULL,NULL);
            if (rc != GT_OK)
            {
        ...
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            ...
            rc = prvCpssDxChPortDpAasPortConfigure(
                devNum, portNum, ifMode, speed, GT_FALSE/*isLoopback*/);
            if (rc != GT_OK)
            {
                prvCpssDxChPortDpAasPortDownForce(
                    devNum, portNum, ifMode, speed, GT_FALSE/*isLoopback*/);
                CPSS_LOG_ERROR_AND_RETURN_MAC(
                    rc,
                    "error in prvCpssDxChPortDpAasPortConfigure, portNum = %d\n",
                    portNum);
            }
        }
        else if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
...
GT_STATUS cpssSimDevApPortHcdFound
(
...
/**
* @internal prvCpssDxChPortPhySpeedSet function
* @endinternal
*
* @brief   Configure Interface speed , duplex on phy port
*         on port's serdeses power up sequence; or configure power down on port's
*         serdeses.
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  Falcon, AC5X, AC5P.
*
* @param[in] devNum                   - physical device number
* @param[in] portNum                  - physical port number
* @param[in] powerUp                  - serdes power:
...
*                                      GT_FALSE - down;
* @param[in] speed                    - port data  (related only for serdes power up [powerUp==GT_TRUE])
* @param[in] duplex                   - port duplex modes
* @param[in] portMacObjPtr            - port phy object pointer.
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS prvCpssDxChPortPhySpeedSet
(
    IN  GT_U8                               devNum,
    IN  GT_PHYSICAL_PORT_NUM                portNum,
    IN  GT_BOOL                             powerUp,
    IN  CPSS_PORT_SPEED_ENT                 speed,
    IN  CPSS_PORT_DUPLEX_ENT                duplexMode,
    IN  CPSS_MACDRV_OBJ_STC                *portMacObjPtr
)
{
    GT_STATUS rc = GT_OK;
    GT_BOOL doPpMacConfig = GT_TRUE;
    ...
    if(portMacObjPtr == NULL)
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "prvCpssDxChPortPhySpeedSet - portMacObjPtr NULL");

    /* run MACPHY callback */
    if (portMacObjPtr->macDrvMacPortPowerDownSetFunc != NULL)
    {
        rc = portMacObjPtr->macDrvMacPortPowerDownSetFunc(devNum,portNum,
                                                                !powerUp,
                                                                 CPSS_MACDRV_STAGE_PRE_E,
        ...
        if(rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortPhySpeedSet - error at macDrvMacPortPowerDownSetFunc");
        }
    }

    /* run callback*/
    if ( portMacObjPtr->macDrvMacSpeedSetFunc != NULL)
    {
        rc = portMacObjPtr->macDrvMacSpeedSetFunc(devNum,portNum,
                                                         speed,CPSS_MACDRV_STAGE_PRE_E,
                                                         &doPpMacConfig,&speed);
        if(rc!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortPhySpeedSet - error at macDrvMacSpeedSetFunc");
        }
    }

    if( portMacObjPtr->macDrvMacDuplexSetFunc != NULL )
    {
        rc = portMacObjPtr->macDrvMacDuplexSetFunc(devNum,portNum,
                                                          duplexMode,CPSS_MACDRV_STAGE_PRE_E,
                                                          &doPpMacConfig,&duplexMode);
        if(rc!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChPortPhySpeedSet - error at macDrvMacDuplexSetFunc");
        }
    }
    return rc;
}
...
)
{
    ...
    for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        if(portMacObjPtr != NULL)
        {
            rc = prvCpssDxChPortPhySpeedSet(devNum, portNum,
                                                    powerUp, speed,
                                                    (portMgrDbPtr!=NULL?portMgrDbPtr->duplexMode:CPSS_PORT_FULL_DUPLEX_E),
                                                    portMacObjPtr);
        }
        ...
        *portSpeedPtr = CPSS_PORT_SPEED_NA_E;
        ...
        /*Set txQ pizza  for Pnodes*/
        rc = prvCpssSip6_30TxqPizzaPortSpeedSet(devNum, portNum,speed);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"error in prvCpssSip6_30TxqPizzaPortSpeedSet, portNum = %d\n", portNum);
        }
        ...
        {
            CPSS_PORT_DUPLEX_ENT duplexMode;
            portInitInParam.autoNegEnabled = GT_FALSE;
            rc = cpssDxChPortDuplexModeGet(devNum, portNum, &duplexMode);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"cpssDxChPortDuplexModeGet: error for portNum = %d\n", portNum);
            }
            portInitInParam.duplexMode = (MV_HWS_PORT_DUPLEX_ENT)duplexMode;
        }
        else
        {
            portInitInParam.autoNegEnabled = (portMgrDbPtr->autoNegotiation.inbandEnable && !portMgrDbPtr->autoNegotiation.byPassEnable);
            portInitInParam.duplexMode = (MV_HWS_PORT_DUPLEX_ENT)portMgrDbPtr->duplexMode;
            portInitInParam.lbPort = portMgrDbPtr->lbPort;
        }

        if(PRV_CPSS_PP_MAC(devNum)->isGmDevice)
        {
            /* the GM not support MAC registers */
        }
        else
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
        {
        ...
        *portSpeedPtr = speed;
        ...
        if(apEnabled == GT_TRUE && CPSS_DEV_IS_WM_NATIVE_OR_ASIM_MAC(devNum) &&
            PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
        {
            /* the WM need to issue also link change event for the AP port */
            rc = cpssSimDevApPortHcdFound(devNum,portMacMap,GT_TRUE);
            if (rc != GT_OK)
            {
                return rc;
            }
...
/**
* @internal prvCpssDxChPort_SIP7_ModeSpeedSet function
* @endinternal
*
* @brief   Configure Interface mode and speed on a specified port and execute
*         on port's serdeses power up sequence; or configure power down on port's
...
)
{
    ...
    CPSS_PORT_INTERFACE_MODE_ENT *portIfModePtr;
    CPSS_PORT_SPEED_ENT          *portSpeedPtr;
    GT_BOOL                 fullConfig = GT_TRUE;     /* for USX modes:
                                                    one serdes serves four ports, when one port of
                                                    the quadruplete configured three others configured too,
                                                    so when application asks to configure another port from
                                                    quadruplete we don't need to execute full configuretion of new port*/
    ...
    /* Check PizzaArbiter and SFF resources are enough for new ifMode/speed setting on a Network ports.
     * The resources could be already in-use by a PCA Loopback and therefore
     * the checking should be done before Network ports' Hws and Serdes configuration
     * (prvCpssDxChPort_SIP7_ModeSpeedSet starts with Hws and Serdes).
     * Also prevents wrong try to config PCA Loopback port over prvCpssDxChPort_SIP7_ModeSpeedSet.
     * All these are even more important when multiple ports are requested in port-bitmap.
     */
    rc = prvCpssDxChPort_SIP7_ModeSpeedCheck(devNum, portsBmp, powerUp, ifMode, speed);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        /* Invalidate LED port position */
#ifndef ASIC_SIMULATION /* CPSS_TBD_BOOKMARK_AAS: kalex should be removed later */
        rc = prvCpssDxChFalconLedStreamPortPositionSet(devNum, portNum, 0x3F);
        if(rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"error in prvCpssDxChFalconLedStreamPortPositionSet, portNum = %d\n", portNum);
        ...
        *portSpeedPtr = CPSS_PORT_SPEED_NA_E;
        ...
        rc = prvCpssDxChFalconPortQueueRateSet(devNum, portNum, speed);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(
                rc,"error in prvCpssDxChFalconPortQueueRateSet, portNum %d speed %d\n",
        ...
        /* AAS
           Configuration of DP, PB, PCA and MIF units. */
        rc = prvCpssDxChPortDpAasPortConfigure(devNum, portNum, ifMode, speed, GT_FALSE/*isLoopback*/);
        if (rc != GT_OK)
        {
            prvCpssDxChPortDpAasPortDownForce(devNum, portNum, ifMode, speed, GT_FALSE/*isLoopback*/);
            CPSS_LOG_ERROR_AND_RETURN_MAC(
                rc,
                "error in prvCpssDxChPortDpAasPortConfigure, portNum = %d\n",
                portNum);
...
/**
* @internal prvCpssDxChPortPcaLoopbackInfoGet function
* @endinternal
* @brief   PCA Loopback utility to get port info
*/
static GT_STATUS prvCpssDxChPortPcaLoopbackInfoGet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum,
    IN  GT_BOOL                         checkBothMaps,
    OUT PRV_CPSS_PORT_INFO_ARRAY_STC    **portInfoPtr
)
{
    GT_STATUS                           rc;
    GT_U32                              portMacMap;
    ...
    rc = prvCpssDxChPortPhysicalPortMapCheckAndConvert(devNum, portNum,
                                        PRV_CPSS_DXCH_PORT_TYPE_RxDMA_E, &portMacMap);
    if (rc != GT_OK)
        return rc;
    ...
    portInfo = &PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacMap];

    notLoopback = (portInfo->pcaLoopbackPort == PRV_CPSS_PORT_NOT_EXISTS_E);
    if (checkBothMaps == GT_TRUE)
    {
        notLoopback &= (portMap.mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E);
    }
    if (notLoopback == GT_TRUE)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_SUCH, LOG_ERROR_NO_MSG);
    }
    *portInfoPtr = portInfo;
    return GT_OK;
}

/**
* @internal prvCpssDxChPortPcaLoopbackIsOnPort function
* @endinternal
*
* @brief   Check PCA Loopback (on dedicated Egress-loopback-channel)
*           IS configured on a given port number
*
* @note   APPLICABLE DEVICES:    xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in]  devNum               - device number
* @param[in]  portNum              - port to be checked
*
* @retval TRUE/FALSE               - port is configured as Loopback or not
*/
GT_BOOL prvCpssDxChPortPcaLoopbackIsOnPort
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            portNum
)
{
    GT_STATUS                           rc;
    GT_BOOL                             checkBothMaps = GT_FALSE; /* check only MAPPING ETH */
    PRV_CPSS_PORT_INFO_ARRAY_STC        *portInfo;

    rc = prvCpssDxChPortPcaLoopbackInfoGet(devNum, portNum, checkBothMaps, &portInfo);
    return (rc == GT_OK);
}

/**
* @internal prvCpssDxChPortPcaLoopbackSpeedGet function
* @endinternal
*
* @brief   Get speed for PCA Loopback on specified port
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*
* @retval GT_OK                    - if port is PCA Loopback and Get is succeed
* @retval GT_NO_SUCH               - no PCA Loopback on specified port or fail to Get
*/
GT_STATUS prvCpssDxChPortPcaLoopbackSpeedGet
(
    IN  GT_U8                 devNum,
    IN  GT_PHYSICAL_PORT_NUM  portNum,
...
{
    GT_STATUS                           rc;
    GT_BOOL                             checkBothMaps = GT_TRUE; /* Both - MAPPING ETH and LOOPBACK */
    PRV_CPSS_PORT_INFO_ARRAY_STC        *portInfo;
    ...
    /* Same result on both paired ports - loopback and its resource */
    *speedPtr = portInfo->portSpeed;
    return GT_OK;
}

/**
* @internal prvCpssDxChPortPcaLoopbackSpeedSet function
* @endinternal
*
* @brief   Check Network or PCA Loopback resource cross-validity and availability,
*          and for PCA Loopback port sets speed.
*
* @note   APPLICABLE DEVICES: AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in]  devNum               - device number
* @param[in]  configPortNum        - port to be configed
* @param[in]  resourcePortNum      - resource "sponsor" port
* @param[in]  powerUp              - port UP (GT_TRUE) or DOWN (GT_FALSE)
* @param[in]  ifMode               - interface mode
* @param[in]  speed                - speed
* @param[in]  loopbackSetRequest   - if GT_FALSE check Network or Loopback validity (see note below)
*                                    else - check and go to PCA Loopback setting
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @retval GT_NO_RESOURCE           - not enought resources for requested speed
* @retval GT_BAD_STATE             - requested port is not free or not enought resources
*
* @note CHECK:  Passed for both kind of ports - Network and PCA Loopback.
*               Checks PizzaArbiter and SFF resources are enough for new ifMode/speed setting on a Network ports.
*               The resources could be already in-use by a PCA Loopback and therefore
*               the checking should be done before Network ports' Hws and Serdes configuration
*               (prvCpssDxChPort_SIP7_ModeSpeedSet starts with Hws and Serdes).
*               Also prevents wrong try to config PCA Loopback port over prvCpssDxChPort_SIP7_ModeSpeedSet.
*               All these are even more important when multiple ports are requested in port-bitmap.
* @note  SET:   Relevant only for PCA Loopback ports
              1. Set speed CPSS_PORT_SPEED_NA_E brings port down and makes free for all resources.
*             2. Set new speed value which is already configured passed as OK but does not
*                change a configuration and does not abrupt a traffic on port (if any).
*                To new speed different from currently configured the port should be set down first.
*             3. Network "sponsor" is always required for speed >50G or if native Special-Channel
*                50G bandwidth resources are busy by CPU, GDMA...
*             4. Egress Loopback port and resource "sponsor" port must be on same Data-Path (DP) unit.
*/
static GT_STATUS prvCpssDxChPortPcaLoopbackSpeedSet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            configPortNum,
    IN  GT_PHYSICAL_PORT_NUM            resourcePortNum,
    IN  GT_BOOL                         powerUp,
    IN  CPSS_PORT_INTERFACE_MODE_ENT    ifMode,
    IN  CPSS_PORT_SPEED_ENT             speed,
    IN  GT_BOOL                         loopbackSetRequest
)
{
    PRV_CPSS_PORT_INFO_ARRAY_STC        *configPortInfo;
    PRV_CPSS_PORT_INFO_ARRAY_STC        *resourcePortInfo;
    GT_STATUS                           rc;
    CPSS_DXCH_SHADOW_PORT_MAP_STC       portMap;
    GT_U32                              portMacMap; /* No MAC on port, but Mac number is mapped */

    /***** Strict CHECKER, common for Network and Loopback  ports **************************
     * Refer the note about CHECK in API description. Most important chekers are inside of
     *   prvCpssDxChPortDpAasSpeedResourceCheck().
     */
    rc = prvCpssDxChPortPhysicalPortMapGet(devNum, resourcePortNum, &portMap);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }

    if ((portMap.mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_LOOPBACK_E &&
         portMap.mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E) ||
        (speed > CPSS_PORT_SPEED_NA_E || ifMode > CPSS_PORT_INTERFACE_MODE_NA_E)) /* _NA_HCD_E */
    {
        /* Port types with out-of-scope OK or Error for Loopback configuration */
        if (loopbackSetRequest == GT_FALSE)
        {
            return GT_OK;
        }
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    rc = prvCpssDxChPortPhysicalPortMapCheckAndConvert(devNum, configPortNum,
                PRV_CPSS_DXCH_PORT_TYPE_RxDMA_E, &portMacMap);
    if(GT_OK != rc)
    {
        return rc;
    }
    configPortInfo = &PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacMap];

    if (powerUp == GT_TRUE)
    {
        /* Check for PowerUP with "not already configured same" */
        if (configPortInfo->portSpeed == speed &&
            configPortInfo->portIfMode == ifMode)
        {
            if ((configPortNum != resourcePortNum) &&
                (configPortInfo->pcaLoopbackPort != configPortNum ||
                 configPortInfo->pcaLoopbackResourcePort != resourcePortNum))
            {
                /* Same speed but different wrong port */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
            }
            return GT_OK; /* already there */
        }
        /* Check config/resource ports are on same DP, resorces are free and enough */
        rc = prvCpssDxChPortDpAasSpeedResourceCheck(devNum, configPortNum, resourcePortNum,
                                                    ifMode, speed,
                                                    loopbackSetRequest);
        if (rc != GT_OK)
        {
    ...
    {
        if ((loopbackSetRequest != GT_TRUE && configPortInfo->pcaLoopbackPort != PRV_CPSS_PORT_NOT_EXISTS_E) || /* ETH(down) to LBP-used */
            (loopbackSetRequest == GT_TRUE && configPortInfo->pcaLoopbackPort == PRV_CPSS_PORT_NOT_EXISTS_E))   /* LPB(down) to ETH-used */
        {   /* Fix ret-code */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
        }
    }
    if (loopbackSetRequest == GT_FALSE) /*Check only for Network port*/
    {
        return GT_OK;
    }

    /***** LOOPBACK HANDLING **************************************************
     * Alike prvCpssDxChPort_SIP7_ModeSpeedSet, but without MAC configurations
     */
    if ((powerUp == GT_FALSE) &&
        (configPortInfo->pcaLoopbackPort != configPortNum ||
         configPortInfo->pcaLoopbackResourcePort != resourcePortNum))
    ...
    if (configPortNum != resourcePortNum)
    {
        /* Get the paired resource port info */
        rc = prvCpssDxChPortPhysicalPortMapCheckAndConvert(devNum, resourcePortNum,
                                    PRV_CPSS_DXCH_PORT_TYPE_RxDMA_E, &portMacMap);
        if(GT_OK != rc)
        {
            return rc;
        }
        resourcePortInfo = &PRV_CPSS_PP_MAC(devNum)->phyPortInfoArray[portMacMap];
    }
    else
    {
        resourcePortInfo = configPortInfo;
    }

    /* Port goes DOWN */
    rc = prvCpssDxChSip6QueueBaseIndexToValidTxCreditsMapSet(devNum, configPortNum, GT_FALSE);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    rc = cpssDxChBrgEgrFltPortLinkEnableSet(devNum, configPortNum,
    ...
    if (powerUp == GT_FALSE)
    {
        rc = prvCpssDxChPortDpAasPortDownForce(devNum, resourcePortNum, ifMode,
                                               configPortInfo->portSpeed, GT_TRUE/*loopback*/);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }
        /* Clear and invalidate port info for both ports */
        resourcePortInfo->pcaLoopbackPort = PRV_CPSS_PORT_NOT_EXISTS_E;
        configPortInfo->pcaLoopbackPort = PRV_CPSS_PORT_NOT_EXISTS_E;
        configPortInfo->portSpeed = CPSS_PORT_SPEED_NA_E;

        return GT_OK;
    }

    /* Port goes UP */
    rc = prvCpssDxChTxqSetPortSdqThresholds(devNum, resourcePortNum, speed);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    rc = prvCpssDxChTxqBindPortQueuesToPdsProfile(devNum, resourcePortNum, speed, NULL, NULL);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    rc = prvCpssDxChFalconPortQueueRateSet(devNum, resourcePortNum, speed);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    rc = prvCpssSip6_30TxqPizzaPortSpeedSet(devNum, resourcePortNum, speed);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }

    /* Configure DP, PB, PCA and MIF units: */
    rc = prvCpssDxChPortDpAasPortConfigure(devNum, resourcePortNum, ifMode, speed, GT_TRUE/*loopback*/);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }

    rc = prvCpssDxChSip6QueueBaseIndexToValidTxCreditsMapSet(devNum, configPortNum, GT_TRUE);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    rc = cpssDxChBrgEgrFltPortLinkEnableSet(devNum, configPortNum, CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }

    /* Lock/Set/Save configuration in port info */
    resourcePortInfo->pcaLoopbackPort = configPortNum;
    configPortInfo->pcaLoopbackPort = configPortNum;
    configPortInfo->pcaLoopbackResourcePort = resourcePortNum;
    configPortInfo->portSpeed = speed;

    return GT_OK;
}

/**
* @internal cpssDxChPortEgressLoopbackSpeedSet function
* @endinternal
*
* @brief   Set Egress Loopback speed and UP/Down on a specified port.
*
* @note   APPLICABLE DEVICES: AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in]  devNum               - device number
* @param[in]  configPortNum        - Egress Loopback port to be configed
* @param[in]  resourcePortNum      - resource port - same as Egress Loopback port (if
*                                    it has enough dedicated resorces) or
*                                    Network port number used as resource "sponsor"
* @param[in]  speed                - speed
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
* @retval GT_NO_RESOURCE           - no or not enought resources for requested speed
* @retval GT_BAD_STATE             - requested port is not in Down state or some
*                                    resources are already used by another port
*
* @note  1. Set speed CPSS_PORT_SPEED_NA_E brings port down and makes free for all resources.
*        2. Set new speed value which is already configured passed as OK but does not
*           change a configuration and does not abrupt a traffic on port (if any).
*           To new speed different from currently configured the port should be set down first.
*        3. Network "sponsor" is always required for speed >50G or if native Special-Channel
...
*        4. Egress Loopback port and resource "sponsor" port must be on same Data-Path (DP) unit.
*/
GT_STATUS internal_cpssDxChPortEgressLoopbackSpeedSet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            configPortNum,
    IN  GT_PHYSICAL_PORT_NUM            resourcePortNum,
...
)
{
    ...
    return prvCpssDxChPortPcaLoopbackSpeedSet(devNum,
                configPortNum, resourcePortNum,
                /* powerUp  */ (speed < CPSS_PORT_SPEED_NA_E),
                /* ifMode   */ CPSS_PORT_INTERFACE_MODE_NA_E,
                /* speed    */ speed,
                /* Loopback */ GT_TRUE);
}
...
*        4. Egress Loopback port and resource "sponsor" port must be on same Data-Path (DP) unit.
*/
GT_STATUS cpssDxChPortEgressLoopbackSpeedSet
(
    IN  GT_U8                           devNum,
    IN  GT_PHYSICAL_PORT_NUM            configPortNum,
    IN  GT_PHYSICAL_PORT_NUM            resourcePortNum,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPortEgressLoopbackSpeedSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, configPortNum, resourcePortNum, speed));

    rc = internal_cpssDxChPortEgressLoopbackSpeedSet(devNum, configPortNum, resourcePortNum, speed);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, configPortNum, resourcePortNum, speed));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal prvCpssDxChPort_SIP7_ModeSpeedCheck function
* @endinternal
*
* @brief   Check new resources are enough and not busy by another ETHERNET or LOOPBACK port
*
* @param[in]  devNum         - device number
* @param[in]  portsBmp       - pointer to ports' bitmap
* @param[in]  powerUp        - action UP (GT_TRUE) or DOWN-and-free (GT_FALSE)
* @param[in]  ifMode         - interface mode
* @param[in]  speed          - speed
*
* @retval GT_OK              - on success
* @retval GT_BAD_PARAM       - on wrong port number or device
* @retval GT_NO_RESOURCE     - no or not enought resources for requested speed
* @retval GT_BAD_STATE       - requested port is not in Down state
*                              or resource slots are overlapped
*
* @note  Check PizzaArbiter and SFF resources are enough for new ifMode/speed setting on a Network ports.
*        The resources could be already in-use by a PCA Loopback and therefore
*        the checking should be done before Network ports' Hws and Serdes configuration
*        (prvCpssDxChPort_SIP7_ModeSpeedSet starts with Hws and Serdes).
*        Also prevents wrong try to config PCA Loopback port over prvCpssDxChPort_SIP7_ModeSpeedSet.
...
)
{
    ...
    for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        if (CPSS_PORTS_BMP_IS_PORT_SET_MAC(&portsBmp, portNum))
        {
            rc = prvCpssDxChPortPcaLoopbackSpeedSet(devNum, portNum, portNum,
                                                    powerUp, ifMode, speed,
                                                    GT_FALSE/*Loopback*/);
            if (rc != GT_OK)
            {
                return rc;
