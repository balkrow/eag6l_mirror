...
/* Timeout for message reply after 500uS*/
#define MSG_REPLY_TIMEOUT_IN_US_CNS 500
#define MSG_REPLY_TIMEOUT_IN_NS_CNS (MSG_REPLY_TIMEOUT_IN_US_CNS*1000)

/* 500nS(=0.uS) delay period*/
...
#define SNAPSHOT_DB_TRACKING_MODE_MASK      0xFF000000
...
static GT_CHAR* tamSnapshotTrackingTypeStr[6]=
{
    "TRACKING_MODE_ALL",
    "TRACKING_MODE_CURRENT",
    "TRACKING_MODE_PEAK",
    "TRACKING_MODE_MINIMUM",
    "TRACKING_MODE_AVERAGE",
    "TRACKING_MODE_HISTOGRAM"
};
...
)
{
    ...
    switch ( U32_GET_FIELD_MAC(regData,0,8) )
    {
        ...
        case PRV_CPSS_DXCH_TAM_RC_CODE_INVALID_IN_PARAMS_E: rc =  GT_BAD_PARAM;
            break;

        case PRV_CPSS_DXCH_TAM_RC_CODE_INVALID_IN_SIZE_E: rc = GT_BAD_SIZE;
            break;

        case PRV_CPSS_DXCH_TAM_RC_CODE_NOT_INITIALIZED_E: rc = GT_NOT_INITIALIZED;
            break;
...
/**
* @internal internal_cpssDxChTamParamsSet function
* @endinternal
*
* @brief   Set Parameters for TAM feature.
*
* @note   APPLICABLE DEVICES:      Falcon
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; Aldrin2; AC3X; Bobcat3; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - physical device number.
* @param[in] tamParamsPtr          - pointer to CPSS_DXCH_TAM_PARAMS_STC structure which includes the
*                                    service CPU id, CNC blocks info
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
*
*/
static GT_STATUS internal_cpssDxChTamParamsSet
(
    IN  GT_U8                    devNum,
    IN  CPSS_DXCH_TAM_PARAMS_STC *tamParamsPtr
)
{
    GT_U32 i;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_ALDRIN2_E | CPSS_AC3X_E | CPSS_BOBCAT3_E);
    CPSS_NULL_PTR_CHECK_MAC(tamParamsPtr);

    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    /* Copy the TAM parameters to global structure */
    tamInfoPtr->tamParams.numOfCncBlocks = tamParamsPtr->numOfCncBlocks;
    for (i = 0; i<tamParamsPtr->numOfCncBlocks; i++)
    {
        tamInfoPtr->tamParams.cncBlockId[i] = tamParamsPtr->cncBlockId[i];
    }

    return GT_OK;
}

/* @internal cpssDxChTamParamsSet function
* @endinternal
*
* @brief   Set Parameters for TAM feature.
*
...
*
*/
GT_STATUS cpssDxChTamParamsSet
(
    IN  GT_U8                    devNum,
    IN  CPSS_DXCH_TAM_PARAMS_STC *tamParamsPtr
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamParamsSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, tamParamsPtr));

    rc = internal_cpssDxChTamParamsSet(devNum, tamParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, tamParamsPtr));
...
/**
* @internal internal_cpssDxChTamParamsGet function
* @endinternal
*
* @brief  Get Parameters for TAM feature.
*
* @note   APPLICABLE DEVICES:      Falcon
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; Aldrin2; AC3X; Bobcat3; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - physical device number.
* @param[out] tamParamsPtr         - pointer to CPSS_DXCH_TAM_PARAMS_STC structure which includes the
*                                    service CPU id, CNC blocks info
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
...
*
*/
static GT_STATUS internal_cpssDxChTamParamsGet
(
    IN   GT_U8                    devNum,
    OUT  CPSS_DXCH_TAM_PARAMS_STC *tamParamsPtr
)
{
    GT_U32 i;
    PRV_CPSS_DXCH_TAM_INFO_STC  *tamInfoPtr;
    ...
    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    /* Retrive the TAM paramters from Global Structure */
    tamParamsPtr->serviceCpuId   = tamInfoPtr->tamParams.serviceCpuId;
    tamParamsPtr->numOfCncBlocks = tamInfoPtr->tamParams.numOfCncBlocks;
    for (i = 0; i<tamInfoPtr->tamParams.numOfCncBlocks; i++)
    {
        tamParamsPtr->cncBlockId[i] = tamInfoPtr->tamParams.cncBlockId[i];
    }

    return GT_OK;
}

/* @internal cpssDxChTamParamsGet function
* @endinternal
*
* @brief  Get Parameters for TAM feature.
*
...
*
*/
GT_STATUS cpssDxChTamParamsGet
(
    IN   GT_U8                    devNum,
    OUT  CPSS_DXCH_TAM_PARAMS_STC *tamParamsPtr
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamParamsGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, tamParamsPtr));

    rc = internal_cpssDxChTamParamsGet(devNum, tamParamsPtr);
...
/**
* @internal internal_cpssDxChTamFwVersionGet function
* @endinternal
*
* @brief  Get TAM version info.
*
* @note   APPLICABLE DEVICES:      Falcon
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; Aldrin2; AC3X; Bobcat3; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - physical device number.
* @param[out] tamPidPtr            - pointer to CPSS_DXCH_TAM_PID_STC structure which includes the Product Name,
*                                    Packet processor name, release version and API version
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
*
*/
static GT_STATUS internal_cpssDxChTamFwVersionGet
(
    IN  GT_U8                  devNum,
    OUT CPSS_DXCH_TAM_PID_STC  *tamPidPtr
)
{
    GT_STATUS rc;
    GT_U32 address;
    ...
    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    address = tamInfoPtr->tamFwPidAddr;
    ...
    for (byte = 0; byte < 3; byte++)
    {
        ...
        cpssOsMemCpy((tamPidPtr->productName) + (byte * sizeof(val)), &val, sizeof(val));
        address += sizeof(val);
    }

    /* Get the packet processor Name */
    for (byte = 0; byte < 3; byte++)
    {
        val = 0;
        rc = prvCpssDxChTamSharedMemRead(devNum,
        ...
        cpssOsMemCpy((tamPidPtr->ppName) + (byte * sizeof(val)), &val, sizeof(val));
        address += sizeof(val);
    }

    /*  Get the Release Version Label */
    for (byte = 0; byte < 3; byte++)
    {
        val = 0;
        rc = prvCpssDxChTamSharedMemRead(devNum,
        ...
        cpssOsMemCpy((tamPidPtr->releaseVersionLabel) + (byte * sizeof(val)), &val, sizeof(val));
        address += sizeof(val);
    }
    ...
    for (byte = 0; byte < 3; byte++)
    {
        ...
        cpssOsMemCpy((tamPidPtr->apiVersion) + (byte * sizeof(val)), &val, sizeof(val));
        address += sizeof(val);
    }
...
/* @internal cpssDxChTamFwVersionGet function
* @endinternal
*
* @brief   Get TAM Version info
*
...
*
*/
GT_STATUS cpssDxChTamFwVersionGet
(
    IN   GT_U8                  devNum,
    OUT  CPSS_DXCH_TAM_PID_STC  *tamPidPtr
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamFwVersionGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, tamPidPtr));

    rc = internal_cpssDxChTamFwVersionGet(devNum, tamPidPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, tamPidPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
*
* @param[in] devNum                - physical device number.
* @param[in] serviceCpuId          - service CPU ID assigned for TAM feature
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E |
                                          CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E);

    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);

    /* service CPU Id check for Falcon Eagle CM3 instances */
    if (serviceCpuId < PRV_CPSS_DXCH_TAM_SERVICE_CPU_MIN_CNS || serviceCpuId > PRV_CPSS_DXCH_TAM_SERVICE_CPU_MAX_CNS)
    {
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
        rc = cpssSrvCpuSetOptionalFW(devNum, serviceCpuId, TAM_FALCON_FW_ID, "falconTamFw");
    else
        rc = cpssSrvCpuSetOptionalFW(devNum, serviceCpuId, TAM_ALDRIN2_FW_ID, "aldrin2TamFw");

    if (GT_OK != rc)
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
#endif
    /* Set the Service CPU Id */
    rc = prvCpssGenericSrvCpuInit(devNum, serviceCpuId, NULL, NULL, 0, NULL);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_MAC("prvCpssGenericSrvCpuInit failed for TAM_falcon with serviceCpuId=%u\n", serviceCpuId);
    }

    /* Update the service CPU in GlobalInitParams */
    tamInfoPtr->tamParams.serviceCpuId = serviceCpuId;

    /* Wait for 20 ms until service CPU is configured */
    ...
    tamInfoPtr->tamSnapshotDbStartAddr        = msgData[1];
    tamInfoPtr->tamCountersDbStartAddr        = msgData[2];
    tamInfoPtr->tamMicroburstDbStartAddr      = msgData[3];
    tamInfoPtr->tamHistogramDbStartAddr       = msgData[4];
    tamInfoPtr->tamThresholdDbStartAddr       = msgData[5];
    tamInfoPtr->tamThresholdNotifStartAddr    = msgData[6];
    tamInfoPtr->tamThresholdNotifBmpStartAddr = msgData[7];
    tamInfoPtr->tamFwDebugCounterAddr         = msgData[8];
    tamInfoPtr->tamFwDebugStatCounterAddr     = msgData[9];
    tamInfoPtr->tamFwPidAddr                  = msgData[10];
    tamInfoPtr->tamFwEnable                   = msgData[11];

    tamInfoPtr->freeCounterFirstAddr = tamInfoPtr->tamCountersDbStartAddr;
    tamInfoPtr->freeCountersNum = PRV_CPSS_DXCH_TAM_FW_COUNTERS;
    CPSS_LOG_INFORMATION_MAC("tamSnapshotDbStartAddr=%x,tamCountersDbStartAddr=%x,tamFwDebugCounterAddr=%x\n",
                             tamInfoPtr->tamSnapshotDbStartAddr,
                             tamInfoPtr->tamCountersDbStartAddr,
                             tamInfoPtr->tamFwDebugCounterAddr);
    CPSS_LOG_INFORMATION_MAC("tamMicroburstDbStartAddr=%x,tamHistogramDbStartAddr=%x,tamThresholdDbStartAddr=%x\n",
                             tamInfoPtr->tamMicroburstDbStartAddr,
                             tamInfoPtr->tamHistogramDbStartAddr,
                             tamInfoPtr->tamThresholdDbStartAddr);
    CPSS_LOG_INFORMATION_MAC("tamThresholdNotifStartAddr=%x,tamThresholdNotifBmpStartAddr=%x,tamFwDebugStatCounterAddr=%x\n",
                             tamInfoPtr->tamThresholdNotifStartAddr,
                             tamInfoPtr->tamThresholdNotifBmpStartAddr,
                             tamInfoPtr->tamFwDebugStatCounterAddr);
    CPSS_LOG_INFORMATION_MAC("tamFwPidAddr=%x, tamFwEnable=%x\n",
                             tamInfoPtr->tamFwPidAddr, tamInfoPtr->tamFwEnable);

    /* set for all the snapshots: firstCounterAddr=0 and statBuff=0 */
    address = tamInfoPtr->tamSnapshotDbStartAddr;
    ...
    address = tamInfoPtr->tamCountersDbStartAddr;
    ...
    /*  */
    address = tamInfoPtr->tamMicroburstDbStartAddr;
...
)
{
    ...
    rc = internal_cpssDxChTamInit(devNum, serviceCpuId);
...
/**
* @internal internal_cpssDxChTamEnableSet function
* @endinternal
*
* @brief  To enable/disable TAM feature.
*
* @note   APPLICABLE DEVICES:      Aldrin2, Falcon.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - physical device number.
* @param[in] enable                - flag to enable/disable TAM feature
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
*
*/
static GT_STATUS internal_cpssDxChTamEnableSet
(
    IN  GT_U8    devNum,
    IN  GT_BOOL  enable
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E |
                                          CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E |
                                          CPSS_AC3X_E | CPSS_BOBCAT3_E);

    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    CPSS_NULL_PTR_CHECK_MAC(tamInfoPtr);

    /* write the snapshot statBuff field & firstCounter value */
    rc = prvCpssDxChTamSharedMemWrite(devNum, tamInfoPtr->tamFwEnable, enable);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal cpssDxChTamEnableSet function
* @endinternal
*
* @brief  To enable/disable TAM feature.
*
...
*
*/
GT_STATUS cpssDxChTamEnableSet
(
    IN  GT_U8    devNum,
    IN  GT_BOOL  enable
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamEnableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enable));

    rc = internal_cpssDxChTamEnableSet(devNum, enable);
...
/**
* @internal internal_cpssDxChTamEnableGet function
* @endinternal
*
* @brief  Gets the TAM enable/disable flag.
*
* @note   APPLICABLE DEVICES:      Aldrin2, Falcon.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; AC5P; AC5X; Harrier; Ironman.
*
* @param[in]  devNum                - physical device number.
* @param[out] enable                - pointer to TAM enable/disable flag
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
*
*/
static GT_STATUS internal_cpssDxChTamEnableGet
(
    IN   GT_U8    devNum,
...
)
{
    ...
    /* write the snapshot statBuff field & firstCounter value */
    rc = prvCpssDxChTamSharedMemRead(devNum, tamInfoPtr->tamFwEnable, enablePtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal cpssDxChTamEnableGet function
* @endinternal
*
* @brief  Gets the TAM enable/disable flag.
*
...
*
* @param[in]  devNum                - physical device number.
* @param[out] enablePtr             - pointer to TAM enable/disable flag
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
*
*/
GT_STATUS cpssDxChTamEnableGet
(
    IN   GT_U8    devNum,
    OUT  GT_BOOL  *enablePtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamEnableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enablePtr));

    rc = internal_cpssDxChTamEnableGet(devNum, enablePtr);
...
/**
* @internal internal_cpssDxChTamDelete function
* @endinternal
*
* @brief Delete TAM/IFA DB and free up SCPU resources.
*
* @note   APPLICABLE DEVICES:      Aldrin2; Falcon; AC5P; AC5X; Harrier.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Ironman.
*
* @param[in] devNum            - device number
*
* @retval GT_OK                - on success
*/
static GT_STATUS internal_cpssDxChTamDelete
(
    IN  GT_U8                                   devNum
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_IRONMAN_E | CPSS_AAS_E);

    /* Get handle for TAM Db */
    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);

    rc = cpssDxChIfaEnableGet(devNum, &ifaEnable);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE, LOG_ERROR_NO_MSG);
    }
    if (ifaEnable == GT_TRUE)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                      "IFA must be disabled first");
    }

    if (tamInfoPtr->tamFwEnable == GT_TRUE)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                      "TAM must be disabled first");
    }

    /* Free service cpu */
    rc = prvCpssGenericSrvCpuRemove(devNum, tamInfoPtr->tamParams.serviceCpuId);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal cpssDxChTamDelete function
* @endinternal
*
* @brief Delete TAM/IFA DB and free up SCPU resources.
*
...
* @retval GT_OK                - on success
*/
GT_STATUS cpssDxChTamDelete
(
    IN  GT_U8                                   devNum
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTamDelete);

    CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum));

    rc = internal_cpssDxChTamDelete(devNum);
    ...
    *counterInfo = 0;
    ...
    switch (counter.type)
    {
        case CPSS_DXCH_TAM_INGRESS_GLOBAL_BUFFER_COUNTER_E:
            if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
            {
                ...
                *counterInfo = CPSS_DXCH_TAM_INGRESS_GLOBAL_BUFFER_COUNTER_E;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon: CPSS_DXCH_TAM_INGRESS_GLOBAL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
            }
            else
            {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_INGRESS_GLOBAL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
            }
            break;
        case CPSS_DXCH_TAM_INGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                 CPSS_LOG_INFORMATION_MAC("Falcon CPSS_DXCH_TAM_INGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E -- not supported\n");
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
             }
             else
             {
                 CPSS_LOG_INFORMATION_MAC("Aldrin2 CPSS_DXCH_TAM_INGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E\n");
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_INGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
        case CPSS_DXCH_TAM_INGRESS_PORT_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                 ...
                 *counterInfo = (counter.port << 8) | CPSS_DXCH_TAM_INGRESS_PORT_BUFFER_COUNTER_E;
                 if (dbgPrintEn)
                 {
                     CPSS_LOG_INFORMATION_MAC("Falcon: CPSS_DXCH_TAM_INGRESS_PORT_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                 }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2:CPSS_DXCH_TAM_INGRESS_PORT_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
        case CPSS_DXCH_TAM_INGRESS_PG_TC_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                 ...
                 *counterInfo = CPSS_DXCH_TAM_INGRESS_PG_TC_BUFFER_COUNTER_E;
                 if (dbgPrintEn)
                 {
                     CPSS_LOG_INFORMATION_MAC("Falcon: CPSS_DXCH_TAM_INGRESS_PG_TC_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                 }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_INGRESS_PG_TC_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
        case CPSS_DXCH_TAM_EGRESS_GLOBAL_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                ...
                *counterInfo = CPSS_DXCH_TAM_EGRESS_GLOBAL_BUFFER_COUNTER_E;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon:CPSS_DXCH_TAM_EGRESS_GLOBAL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_EGRESS_GLOBAL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
        case CPSS_DXCH_TAM_EGRESS_GLOBAL_MULTI_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                ...
                *counterInfo = CPSS_DXCH_TAM_EGRESS_GLOBAL_MULTI_BUFFER_COUNTER_E;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon: CPSS_DXCH_TAM_EGRESS_GLOBAL_MULTI_BUFFER_COUNTER:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_EGRESS_GLOBAL_MULTI_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             break;
        case CPSS_DXCH_TAM_EGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                 CPSS_LOG_INFORMATION_MAC("Falcon CPSS_DXCH_TAM_EGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E -- not supported\n");
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_EGRESS_GLOBAL_MIRROR_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
        case CPSS_DXCH_TAM_EGRESS_PORT_BUFFER_COUNTER_E:
             if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon:CPSS_DXCH_TAM_EGRESS_PORT_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_EGRESS_PORT_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
            break;
            ...
            if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon:CPSS_DXCH_TAM_EGRESS_QUEUE_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             else
             {
            ...
            if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
             {
                ...
                *counterInfo = CPSS_DXCH_TAM_EGRESS_SHARED_POOL_BUFFER_COUNTER_E;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Falcon:CPSS_DXCH_TAM_EGRESS_SHARED_POOL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
             }
             else
             {
                ...
                *hwRegAddr = regAddr;
                if (dbgPrintEn)
                {
                    CPSS_LOG_INFORMATION_MAC("Aldrin2: CPSS_DXCH_TAM_EGRESS_SHARED_POOL_BUFFER_COUNTER_E:hwRegAddr=0x%08x\n",*hwRegAddr);
                }
            }
            break;
...
* @param[in] trackingMode          - snapshot Tracking Mode
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
(
    ...
    IN   CPSS_DXCH_TAM_TRACKING_MODE_ENT trackingMode
)
{
    GT_STATUS   rc;
    ...
    snapshotDbAddr = tamInfoPtr->tamSnapshotDbStartAddr + (snapshotId * sizeof(PRV_CPSS_DXCH_TAM_SNAPSHOT_DB_STC));
    ...
    if (listSize == 0)
    {
        ...
        for (ii = 0; ii < PRV_CPSS_DXCH_TAM_FW_MICROBURSTS; ii++)
        {
            ...
            microburstDbAddr = tamInfoPtr->tamMicroburstDbStartAddr + (ii * sizeof(PRV_CPSS_DXCH_TAM_MICROBURST_DB_STC));
        ...
        do
        {
            ...
            counterIdx = (counterDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC);
        ...
        rc = prvCpssDxChTamSharedMemWrite(devNum,
                                          counterDbAddr + COUNTER_DB_NEXT_COUNTER_ADDR_OFFSET,
                                          tamInfoPtr->freeCounterFirstAddr);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }
        tamInfoPtr->freeCounterFirstAddr = firstCounterAddr;

        /* update the free counters number */
        tamInfoPtr->freeCountersNum = (tamInfoPtr->freeCountersNum + ii + 1);
    ...
    if (listSize > tamInfoPtr->freeCountersNum)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE, LOG_ERROR_NO_MSG);
    }

    /* save the first free counter to write it later to the snapshot table */
    firstCounterAddr = tamInfoPtr->freeCounterFirstAddr;

    /*  */
    counterDbAddr = tamInfoPtr->freeCounterFirstAddr;
    lastCounterAddr = tamInfoPtr->freeCounterFirstAddr; /*initialize it*/
    ...
    for (ii = 0; ii < listSize; ii++)
    {
        ...
        rc = getHwRegAddrForTamCounter(devNum, counters[ii], &counterInfo, &value, GT_FALSE);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    rc = internal_cpssDxChTamSnapshotSet(devNum, snapshotId, listSize,
                                         countersArray, trackingMode);
...
* @param[out] trackingMode         - snapshot Tracking Mode
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_NOT_INITIALIZED       - on un-initialized package
...
(
    ...
    OUT     CPSS_DXCH_TAM_TRACKING_MODE_ENT *trackingMode
)
{
    GT_STATUS   rc;
    ...
    /* read the snapshot tracking mode */
    *trackingMode = (value & SNAPSHOT_DB_TRACKING_MODE_MASK) >> 24;
    ...
    for (ii = 0; ii < (*listSize); ii++)
    {
        ...
        cpssOsMemCpy(&countersPtr[ii],
                     &countersShadowDb[(counterDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC)].counter,
...
)
{
    ...
    rc = internal_cpssDxChTamSnapshotGet(devNum, snapshotId, listSize,
                                         countersArray, trackingMode);
...
)
{
    ...
    CPSS_DXCH_TAM_TRACKING_MODE_ENT trackingMode;
    PRV_CPSS_DXCH_TAM_INFO_STC  *tamInfoPtr;
    ...
    if (*numOfHistograms != 0)
    {
        CPSS_NULL_PTR_CHECK_MAC(histValuePtr);
    }
    ...
    *numOfHistograms = 0;
    ...
    if (statBufNum)
    {
        value &= 0x0000FFFF; /* zero the statBufNum field (bits 16-23) */
    }
    else
    {
        value |= 0x00010000; /* write value 1 to statBufNum field (bits 16-23) */
    }
    ...
    for (ii = 0; ii < (*numOfStats); ii++)
    {
        ...
        {
            address = counterDbAddr + COUNTER_DB_STAT_1_OFFSET;
        }
        ...
        if ((trackingMode == CPSS_DXCH_TAM_TRACKING_MODE_AVERAGE_E) ||
            (trackingMode == CPSS_DXCH_TAM_TRACKING_MODE_ALL_E))
        {
        ...
        counterId = (counterDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC);
        ...
        if ((histValuePtr != NULL) &&
            ...
            ((trackingMode == CPSS_DXCH_TAM_TRACKING_MODE_HISTOGRAM_E) ||
             (trackingMode == CPSS_DXCH_TAM_TRACKING_MODE_ALL_E)))
        {
            ...
            histogramDbAddr = tamInfoPtr->tamHistogramDbStartAddr + (countersShadowDb[counterId].histogramId * sizeof(PRV_CPSS_DXCH_TAM_HISTOGRAM_DB_STC));
            ...
            if (statBufNum)
            {
                value &= 0xFF00FFFF; /* zero the statBufNum field (bits 16-23) */
            }
            else
            {
                value |= 0x00010000; /* write value 1 to statBufNum field (bits 16-23) */
            ...
            if (statBufNum == 0)
            {
                address = histogramDbAddr + HISTOGRAM_DB_BIN_VAL_STAT_0_OFFSET;
            }
            else
            {
                address = histogramDbAddr + HISTOGRAM_DB_BIN_VAL_STAT_1_OFFSET;
            }
            ...
            for (jj = 0; jj < CPSS_DXCH_TAM_HISTOGRAM_BIN_VALUES; jj++)
            {
                if ((binSum.l[0] != 0) || (binSum.l[1] != 0))
                {
                    ...
                    rc = prvCpssMathDiv64(val0, binSum, &histValuePtr[*numOfHistograms].value[jj],
                                          &histValuePtr[*numOfHistograms].reminder[jj]);
                    if (rc != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
                    }
                ...
                else
                {
                    ...
                    histValuePtr[*numOfHistograms].reminder[jj].l[0] = 0;
                    histValuePtr[*numOfHistograms].reminder[jj].l[1] = 0;
                }
            }
...
)
{
    ...
    microburstDbAddr = tamInfoPtr->tamMicroburstDbStartAddr + (microburstId * sizeof(PRV_CPSS_DXCH_TAM_MICROBURST_DB_STC));
    ...
    counterDbAddr = tamInfoPtr->tamCountersDbStartAddr + (ii * sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC));
...
* @param[inout] numOfStats         - number of microburst and histogram statistics entries to get
* @param[out] statsData[]          - microburst statistics data array.
* @param[out] histValue[]          - histogram statistics data array.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
(
    ...
    IN    GT_U32                                  microburstId,
    INOUT GT_U32                                  *numOfStats,
    OUT   CPSS_DXCH_TAM_MICROBURST_STATISTICS_STC statsData[],
    OUT   CPSS_DXCH_TAM_HISTOGRAM_VALUE_STC       histValue[]
)
{
    GT_STATUS   rc;
    ...
    /* The numOfStats is always '1' for Falcon TAM release-1,
     * In the next release the statsData and histVal array will be updated
     * based on the Snapshot entry size and numOfStats value */
    if (*numOfStats > 1)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);
    }
    ...
    {
        address = microburstDbAddr + MICROBURST_DB_STAT_1_OFFSET;
    }
    ...
    statsData[0].lastDuration = value;
    ...
    statsData[0].shortestDuration = value;
    ...
    statsData[0].longestDuration = value;
    ...
    statsData[0].numMicrobursts = value;
    ...
    /* calculate the average */
    statAverage = prvCpssMathDiv64By32(statSum, statsData[0].numMicrobursts, NULL);
    statsData[0].averageDuration = statAverage.l[0];
    ...
    if (microburstShadowDb[microburstId].histogramId == 0xFF)
    {
        ...
        histValue[0].histogramId = 0xFF;
        return GT_OK;
    }

    /* take the histogramId from shadow & update the output */
    histValue[0].histogramId = microburstShadowDb[microburstId].histogramId;

    /* calculate the address for this histogram */
    histogramDbAddr = tamInfoPtr->tamHistogramDbStartAddr + (microburstShadowDb[microburstId].histogramId * sizeof(PRV_CPSS_DXCH_TAM_HISTOGRAM_DB_STC));
    ...
    for (ii = 0; ii < CPSS_DXCH_TAM_HISTOGRAM_BIN_VALUES; ii++)
    {
        ...
        histValue[0].value[ii].l[0] = value;
        histValue[0].value[ii].l[1] = 0;
        histValue[0].reminder[ii].l[0] = 0;
        histValue[0].reminder[ii].l[1] = 0;
    }
...
* @param[in] microburstId          - snapshot id.
* @param[inout] numOfStats         - number of microburst and histogram statistics entries to get
* @param[out] statsData[]          - microburst statistics data.
* @param[out] histValue[]          - histogram statistics data.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, microburstId, statsData, histValue));

    rc = internal_cpssDxChTamMicroburstStatGet(devNum, microburstId, numOfStats, statsData, histValue);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, microburstId, statsData, histValue));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    /* calculate the address for this histogram */
    histogramDbAddr = tamInfoPtr->tamHistogramDbStartAddr + (histogramId * sizeof(PRV_CPSS_DXCH_TAM_HISTOGRAM_DB_STC));
     ...
     * OR if all the counterPtr parameters are zero then delete the Histogram */
    /* (don't zero the counter db fields and counter shadow. it will be zeroed/updated when will add a new snapshot) */
    if (counterPtr == NULL ||
        (counterPtr && (counterPtr->type == 0 && counterPtr->id == 0
           && counterPtr->port == 0 && counterPtr->queue == 0)))
    {
        ...
        {
            /* Check for valid microburst DB address */
            if (microOrCountDbAddr < tamInfoPtr->tamMicroburstDbStartAddr)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
            }

            /* calculate microburst index */
            ii = (microOrCountDbAddr - tamInfoPtr->tamMicroburstDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_MICROBURST_DB_STC);
        ...
        {
            /* Check for valid counter DB address */
            if (microOrCountDbAddr < tamInfoPtr->tamCountersDbStartAddr)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
            }

            /* calculate counter index */
            ii = (microOrCountDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC);
    ...
    if (counterPtr->type == CPSS_DXCH_TAM_MICROBURST_DURATION_COUNTER_E)
    {
        ...
        /* calculate the address for this microburst */
        microOrCountDbAddr = tamInfoPtr->tamMicroburstDbStartAddr + (counterPtr->id * sizeof(PRV_CPSS_DXCH_TAM_MICROBURST_DB_STC));
    ...
    else
    {
        ...
        /* calculate the counterDb address to write it later to the microburstDb */
        microOrCountDbAddr = tamInfoPtr->tamCountersDbStartAddr + (ii * sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC));
        snapshotDbAddr = tamInfoPtr->tamSnapshotDbStartAddr + (countersShadowDb[ii].snapshotId * sizeof(PRV_CPSS_DXCH_TAM_SNAPSHOT_DB_STC));
...
)
{
    ...
    if (histogramType == PRV_CPSS_DXCH_TAM_HISTOGRAM_TYPE_MICROBURST_E)
    {
        ...
        counterPtr->id = (microOrCountDbAddr - tamInfoPtr->tamMicroburstDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_MICROBURST_DB_STC);
    }
    else /* histogram defined on counter */
    {
        /* calculate the counterId */
        counterId = (microOrCountDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC);
...
*                                    if null - delete the histogram
* @param[in] thresholdLevelPtr     - breach watermark level for this statistic
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - on wrong device number
...
)
{
    ...
    thresholdDbAddr = tamInfoPtr->tamThresholdDbStartAddr + (thresholdId * sizeof(PRV_CPSS_DXCH_TAM_THRESHOLD_DB_STC));
    ...
    if (statisticPtr == NULL)
    {
        ...
        rc = prvCpssDxChTamSharedMemWrite(devNum,
                                          tamInfoPtr->tamThresholdNotifStartAddr + (sizeof(GT_U32) * thresholdId),
                                          0);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, thresholdId, statisticPtr, thresholdLevelPtr));

    rc = internal_cpssDxChTamThresholdSet(devNum, thresholdId, statisticPtr, thresholdLevelPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, thresholdId, statisticPtr, thresholdLevelPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
{
    GT_STATUS   rc;
    GT_U32      bmp, value, thresholdId, inNumOfThresholds;
    PRV_CPSS_DXCH_TAM_INFO_STC  *tamInfoPtr;
    GT_BOOL     thresholdBreach = GT_FALSE;
    GT_U32      thresholdDbAddr, counterDbAddr;
    ...
    *numOfThresholds = 0;
    ...
    for (thresholdId = 0; thresholdId < PRV_CPSS_DXCH_TAM_FW_THRESHOLDS; thresholdId++)
    {
        /* check if this thresholdId enabled in bitmap (i.e. this threshold was crossed) */
        if ((bmp >> thresholdId) & 0x1)
        {
            thresholdBreach = GT_TRUE;
            ...
            thresholdsArray[*numOfThresholds].thresholdId = thresholdId;
            ...
            /* Check if the counterDb address is valid */
            if (counterDbAddr < tamInfoPtr->tamCountersDbStartAddr)
            {
                /* The counterDb address is invalid */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);
            }

            /* copy the counter from the counters-shadow to the output */
            cpssOsMemCpy(&thresholdsArray->counter,
                         &countersShadowDb[(counterDbAddr - tamInfoPtr->tamCountersDbStartAddr) / sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC)].counter,
                         sizeof(CPSS_DXCH_TAM_COUNTER_ID_STC));
    ...
    /* If threshold is crossed for any thresholdId then report the event */
    if (thresholdBreach && tamInfoPtr->eventNotifyFuncPtr)
    {
        tamInfoPtr->eventNotifyFuncPtr(devNum, *numOfThresholds, thresholdsArray);
    }
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, numOfThresholds, thresholdsArray));

    rc = internal_cpssDxChTamThresholdNotificationsGet(devNum, numOfThresholds, thresholdsArray);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, numOfThresholds, thresholdsArray));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    CPSS_NULL_PTR_CHECK_MAC(notifyFunc);

    tamInfoPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->tamInfo);
    CPSS_NULL_PTR_CHECK_MAC(tamInfoPtr);

    /* update the function pointer */
    tamInfoPtr->eventNotifyFuncPtr = notifyFunc;
...
GT_STATUS prvCpssDxChTamInit(GT_U8 devNum, GT_U8 serviceCpuId)
{
    GT_STATUS rc;

    rc = cpssDxChTamInit(devNum, serviceCpuId);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
...
GT_STATUS prvCpssDxChTamEnableSet(GT_U8 devNum, GT_U8 enable)
{
    GT_STATUS rc;

    rc = cpssDxChTamEnableSet(devNum, enable);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
...
GT_STATUS prvCpssDxChTamEnableGet(GT_U8 devNum)
{
    GT_STATUS rc;
    GT_BOOL   enable;

    rc = cpssDxChTamEnableGet(devNum, &enable);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
    cpssOsPrintf("TAM enable = %u\n", enable);

    return GT_OK;
}

GT_STATUS prvCpssDxChTamParamsSet(GT_U8 devNum)
{
    GT_STATUS rc;
    CPSS_DXCH_TAM_PARAMS_STC tamParams;
    GT_U32 i;

    tamParams.serviceCpuId   = 16;
    tamParams.numOfCncBlocks = 2;

    for (i=0; i<tamParams.numOfCncBlocks; i++)
    {
        tamParams.cncBlockId[i]  = i;
    }

    rc = cpssDxChTamParamsSet(devNum, &tamParams);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
...
GT_STATUS prvCpssDxChTamParamsGet(GT_U8 devNum)
{
    GT_STATUS rc;
    CPSS_DXCH_TAM_PARAMS_STC tamParams;
    GT_U32 i;

    cpssOsMemSet(&tamParams, '\0', sizeof(CPSS_DXCH_TAM_PARAMS_STC));

    rc = cpssDxChTamParamsGet(devNum, &tamParams);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }

    cpssOsPrintf("serviceCpuId=%u\nnumOfCncBlocks=%u\n",
                 tamParams.serviceCpuId,
                 tamParams.numOfCncBlocks);

    for(i=0; i<tamParams.numOfCncBlocks; i++)
    {
        cpssOsPrintf("tamParams.cncBlockId[%u]=%u\n", i, tamParams.cncBlockId[i]);
    }

    return GT_OK;
}

GT_STATUS prvCpssDxChTamFwVersionGet(GT_U8 devNum)
{
    GT_STATUS rc;
    CPSS_DXCH_TAM_PID_STC tamPid;
    cpssOsMemSet(&tamPid, '\0', sizeof(CPSS_DXCH_TAM_PID_STC));

    rc = cpssDxChTamFwVersionGet(devNum, &tamPid);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }

    cpssOsPrintf("devNum=%u,\ntamPidProductName=%s,\ntamPidPpName=%s,\ntamPidRelVerLabel=%s,\ntamPidApiVer=%s\n",
                 devNum, tamPid.productName, tamPid.ppName,
                 tamPid.releaseVersionLabel, tamPid.apiVersion);
...
GT_STATUS prvDebugSnapshotSet(GT_U8 devNum, GT_U32 snapshotId, CPSS_DXCH_TAM_COUNTER_TYPE_ENT counterType, GT_U32 id, GT_U32 port, GT_U32 queue)
{
    ...
    rc = cpssDxChTamSnapshotSet(devNum, snapshotId, 1,
                                counters, CPSS_DXCH_TAM_TRACKING_MODE_ALL_E);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
...
GT_STATUS prvDebugSnapshotSetListSize(GT_U8 devNum, GT_U32 snapshotId, GT_U32 listSize)
{
    ...
    rc = cpssDxChTamSnapshotSet(devNum, snapshotId, listSize,
                                counters, CPSS_DXCH_TAM_TRACKING_MODE_ALL_E);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
...
GT_STATUS prvDebugSnapshotGet(GT_U8 devNum, GT_U32 snapshotId, GT_U32 size)
{
    ...
    GT_U32                       listSize, ii;
    CPSS_DXCH_TAM_COUNTER_ID_STC counters[PRV_CPSS_DXCH_TAM_FW_COUNTERS];
    CPSS_DXCH_TAM_TRACKING_MODE_ENT trackingMode=0;

    cpssOsPrintf("snapshotId=%d inputSize=%d\n", snapshotId, size);

    cpssOsMemSet(counters, 0, size * sizeof(CPSS_DXCH_TAM_COUNTER_ID_STC));
    listSize = size;
    rc = cpssDxChTamSnapshotGet(devNum, snapshotId, &listSize, counters, &trackingMode);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
        return rc;
...
GT_STATUS prvDebugPrintCountersDb(GT_U8 devNum, GT_U16 firstIdx, GT_U16 entries)
{
    ...
    for (ii = firstIdx; ii < (firstIdx + entries); ii++)
    {
        ...
        cpssOsPrintf("%5d ", (counterDbAddr - tamInfoPtr->tamCountersDbStartAddr)/sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC));

        rc = prvCpssDxChTamSharedMemRead(devNum, counterDbAddr, &value);
        if (rc != GT_OK)
        {
            cpssOsPrintf("0xxxxxxxxx    ");
        }
        else
        {
            cpssOsPrintf("0x%08x    ", value);
        }
...
GT_STATUS prvDebugPrintSnapshotsDb(GT_U8 devNum)
{
    ...
    {
        snapshotDbAddr = tamInfoPtr->tamSnapshotDbStartAddr + (ii * 4);
        cpssOsPrintf("0x%08x %10d ",snapshotDbAddr, ii);

        rc = prvCpssDxChTamSharedMemRead(devNum, snapshotDbAddr, &value);
        if (rc != GT_OK)
        {
            cpssOsPrintf("xxxxxxxxxx ");
        }
        else
        {
...
GT_STATUS prvDebugSnapshotSetTrackingMode(GT_U8 devNum, GT_U32 snapshotId,
                                          CPSS_DXCH_TAM_COUNTER_TYPE_ENT counterType, GT_U32 id,
                                          GT_U32 port, GT_U32 queue,
                                          CPSS_DXCH_TAM_TRACKING_MODE_ENT trackingMode)
{
    GT_STATUS                      rc;
    CPSS_DXCH_TAM_COUNTER_ID_STC   counters[1];
    ...
    cpssOsPrintf("counterType=%d=%s\n", counterType, tamCounterTypeStr[counterType]);
    cpssOsPrintf("id=%d port=%d queue=%d\n", id, port, queue);
    cpssOsPrintf("trackingMode=%u=%s\n", trackingMode, tamSnapshotTrackingTypeStr[trackingMode]);
    ...
    rc = cpssDxChTamSnapshotSet(devNum, snapshotId, 1/*listSize*/,
                                counters, trackingMode);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    }
...
GT_STATUS prvDebugPrintSnapshotCounters(GT_U8 devNum, GT_U32 snapshotId)
{
    ...
    snapshotDbAddr = tamInfoPtr->tamSnapshotDbStartAddr + (snapshotId * 4);
...
GT_STATUS prvDebugPrintSnapshotCountersShadow(GT_U8 devNum, GT_U32 snapshotId)
{
    ...
    while (counterDbAddr != 0)
    {
        ii = (counterDbAddr - tamInfoPtr->tamCountersDbStartAddr)/sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC);
        cpssOsPrintf("(0x%08x): ", counterDbAddr);
        cpssOsPrintf("%5d ", ii);
...
{
    GT_STATUS                               rc;
    CPSS_DXCH_TAM_MICROBURST_STATISTICS_STC statsData[1];
    CPSS_DXCH_TAM_HISTOGRAM_VALUE_STC       histValue[1];
    GT_U32                                  numOfStats = 1;

    cpssOsPrintf("microburstId=%d\n", microburstId);

    cpssOsMemSet(statsData, 0, sizeof(CPSS_DXCH_TAM_MICROBURST_STATISTICS_STC));
    cpssOsMemSet(histValue, 0, sizeof(CPSS_DXCH_TAM_HISTOGRAM_VALUE_STC));

    rc = cpssDxChTamMicroburstStatGet(devNum, microburstId, &numOfStats,
                                      statsData, histValue);
    if (rc != GT_OK)
    {
        cpssOsPrintf("rc=%d\n", rc);
    ...
    cpssOsPrintf("%10u %10u %10u %10u\n",
                 statsData[0].lastDuration,
                 statsData[0].shortestDuration,
                 statsData[0].longestDuration,
                 statsData[0].averageDuration);

    cpssOsPrintf("\n\nhistogramId     value0     value1     value2     value3\n");
    cpssOsPrintf("%11u %10u %10u %10u %10u\n",
                 histValue[0].histogramId,
                 histValue[0].value[0].l[0],
                 histValue[0].value[1].l[0],
                 histValue[0].value[2].l[0],
                 histValue[0].value[3].l[0]);
...
GT_STATUS prvDebugPrintHistogramDb(GT_U8 devNum)
{
    ...
    {
        histogramDbAddr = tamInfoPtr->tamHistogramDbStartAddr + (ii * sizeof(PRV_CPSS_DXCH_TAM_HISTOGRAM_DB_STC));
...
GT_STATUS prvDebugPrintFreeCountersList(GT_U8 devNum, GT_U16 entries)
{
    ...
    {
        cpssOsPrintf("0x%08x: ", counterDbAddr);
        cpssOsPrintf("%10d ", (counterDbAddr - tamInfoPtr->tamCountersDbStartAddr)/sizeof(PRV_CPSS_DXCH_TAM_COUNTER_DB_STC));
...
GT_STATUS prvDebugPrintFwMemory(GT_U8 devNum, GT_U32 addr, GT_U16 words, GT_U8 numWordsInLine)
{
    ...
    for (ii = 0; ii < words; ii++)
    {
        ...
        if (rc != GT_OK)
        {
            cpssOsPrintf("xxxxxxxx ");
        }
        else
        {
            cpssOsPrintf("%08x ", value);
        }
