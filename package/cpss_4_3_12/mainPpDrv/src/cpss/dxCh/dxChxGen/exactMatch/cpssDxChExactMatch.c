...
#define EM_LOCK(_devNum, semId)   ipfixEmSemaLock(_devNum, semId)

#define EM_UNLOCK(_devNum, semId) ipfixEmSemaUnlock(_devNum, semId)
...
/* sip7 : the size of expanded action in bytes */
#define PRV_CPSS_SIP7_DXCH_EXACT_MATCH_EXPANDED_ACTION_SIZE_CNS      45

/* the MAX size of expanded action in bytes */
#define PRV_CPSS_DXCH_MAX_EXACT_MATCH_EXPANDED_ACTION_SIZE_CNS      48
...
#define PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_HW_TO_CLIENT_TYPE_VAL_MAC(_devNum, _clientType, _hwVal)\
    switch (_hwVal)                                                                     \
        ...
        case 2:                                                                         \
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                           \
            _clientType = CPSS_DXCH_EXACT_MATCH_CLIENT_IPCL_2_E;                        \
        else                                                                            \
            _clientType = CPSS_DXCH_EXACT_MATCH_CLIENT_IPCL_1_E;                        \
        break;                                                                          \
        case 3:                                                                         \
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                           \
            _clientType = CPSS_DXCH_EXACT_MATCH_CLIENT_MPCL_E;                          \
        else                                                                            \
        ...
        case 5:                                                                         \
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                       \
                _clientType = CPSS_DXCH_EXACT_MATCH_CLIENT_IPE_E;                       \
            else                                                                        \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);          \
            break;                                                                      \
        case 6:                                                                         \
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                       \
                _clientType = CPSS_DXCH_EXACT_MATCH_CLIENT_TTI_ILM_E;                   \
            else                                                                        \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);          \
            break;                                                                      \
        default:                                                                        \
...
#define PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_CLIENT_TYPE_TO_HW_VAL_MAC(_devNum, _hwVal, _clientType)\
    switch (_clientType)                                                                \
        ...
        case CPSS_DXCH_EXACT_MATCH_CLIENT_EPCL_E:                                       \
            ...
            break;                                                                      \
        case CPSS_DXCH_EXACT_MATCH_CLIENT_MPCL_E:                                       \
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                       \
                _hwVal = 3;                                                             \
            else                                                                        \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);          \
            break;                                                                      \
        case CPSS_DXCH_EXACT_MATCH_CLIENT_IPE_E:                                        \
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                       \
                _hwVal = 5;                                                             \
            else                                                                        \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);          \
            break;                                                                      \
        case CPSS_DXCH_EXACT_MATCH_CLIENT_TTI_ILM_E:                                    \
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))                                       \
                _hwVal = 6;                                                             \
            else                                                                        \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);          \
...
)
{
    ...
    GT_U32    tempHwActionArray[PRV_CPSS_DXCH_EXACT_MATCH_ACTION_MAX_SIZE_CNS];/* 12 words - 354 bits of data */
    GT_U32    tempHwReducedActionMaskWordsArray[PRV_CPSS_DXCH_EXACT_MATCH_ACTION_MAX_SIZE_CNS];/* 12 words - 354 bits of data */
    GT_U8     tempHwReducedActionMaskBytesArray[PRV_CPSS_DXCH_MAX_EXACT_MATCH_EXPANDED_ACTION_SIZE_CNS+1];/* 12 words - 354 bits of data */
    ...
    GT_U32    wordsInAction;
    GT_U32    maxReducedBytes;

    wordsInAction = PRV_CPSS_SIP_7_CHECK_MAC(devNum)
        ? PRV_CPSS_DXCH_EXACT_MATCH_ACTION_SIP7_SIZE_CNS
        : PRV_CPSS_DXCH_EXACT_MATCH_ACTION_SIZE_CNS;
    ...
    /* sip7 : exactMatchActionAssignment[16][45] Packet Type/Byte*/
    /* sip6 : exactMatchActionAssignment[16][32] Packet Type/Byte*/
    /* sip5 : exactMatchActionAssignment[16][31] Packet Type/Byte*/
    expandedActioSize = PRV_CPSS_DXCH_EXACT_MATCH_EXPANDED_ACTION_MAX_SIZE_CNS(devNum);
    maxReducedBytes = (PRV_CPSS_SIP_7_CHECK_MAC(devNum))?
                       PRV_CPSS_DXCH_EXACT_MATCH_SIP7_REDUCED_ACTION_SIZE_CNS:
                       PRV_CPSS_DXCH_EXACT_MATCH_REDUCED_ACTION_SIZE_CNS;
    for (i=0;i<expandedActioSize;i++)
    {
        if (tempHwReducedActionMaskBytesArray[i]!=0)
        {
            numberOfReducedBytesUsed++;
            /* SIP6: max 8 bytes can be used
               SIP7: max 10 bytes can be used */
            if(numberOfReducedBytesUsed > maxReducedBytes)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                                              "Error: Can not configure more than %u bytes in reduce entry\n",
                                              maxReducedBytes);
            }
        }
    }
...
)
{
    ...
    rc = prvCpssHwPpSetRegField(devNum,
           PRV_DXCH_REG1_UNIT_EM_MAC(devNum,emId).EMGlobalConfiguration.EMGlobalConfiguration1,
           fieldOffset,
           fieldLength,
           hwValue);
    if (rc != GT_OK)
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        regAddr = PRV_DXCH_REG1_UNIT_EM_MAC(devNum,emId).EMGlobalConfiguration.EMGlobalConfiguration1;
        fieldOffset = 3;
    }
    else if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        regAddr = PRV_DXCH_REG1_UNIT_EM_MAC(devNum,CPSS_DXCH_EXACT_MATCH_UNIT_0_E).EMGlobalConfiguration.EMGlobalConfiguration1;
        fieldOffset = 3;
    }
    else
    {
        regAddr = PRV_DXCH_REG1_UNIT_EM_MAC(devNum,CPSS_DXCH_EXACT_MATCH_UNIT_0_E).EMGlobalConfiguration.EMGlobalConfiguration2;
        fieldOffset = 2;
    }
    value = mht == 4 ? 0 :
            mht == 8 ? 1 :
...
)
{
    ...
    /* Validate EM unit
    * SIP7: Not applicable on Reduced EM unit */
    PRV_CPSS_DXCH_EXACT_MATCH_NON_REDUCED_EM_UNIT_NUM_CHECK_MAC(devNum, emUnitNum);
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* EM unit 0 - Channel 0 and 1
         * EM unit 1 - Channel 2 and 3
         * EM unit 2 - Channel 4 and 5
         * EM unit 3 - Channel 6 and 7
         * Channel calculation, e.g
         * EM unit 1, lookup 1 - 1*2 + 1 = 3 */
        emChannel = (emUnitNum * 2) + ttiHitNum;
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.emChannel[emChannel];
        rc =  prvCpssHwPpGetRegField(devNum, regAddr, 0, 4, &value);
        if (rc != GT_OK)
        {
            return rc;
        }
        value = ((value-ttiHitNum)/2) & 0xF;
    }
    else
    {
...
)
{
    ...
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, keyType, ttiLookupNum ,enableExactMatchLookup, profileId));

    rc =  internal_cpssDxChExactMatchTtiProfileIdModePacketTypeSet(devNum, emUnitNum, keyType, ttiLookupNum,
                                                                   enableExactMatchLookup, profileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, keyType, ttiLookupNum ,enableExactMatchLookup, profileId));
...
)
{
    ...
    * SIP7: Not applicable on Reduced EM unit */
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,tcamPortGroupsBmp,portGroupId)
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            rc =  prvCpssHwPpPortGroupGetRegField(devNum, portGroupId, regAddr, 0, 4, &value);
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portNum ,ttiLookupNum, enableExactMatchLookup, profileId));

    rc = internal_cpssDxChExactMatchTtiPortProfileIdModePortSet(devNum, emUnitNum, portNum, ttiLookupNum,
                                                                enableExactMatchLookup, profileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, portNum ,ttiLookupNum, enableExactMatchLookup, profileId));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portNum ,keyType,ttiLookupNum, enableExactMatchLookup, profileId));

    rc = internal_cpssDxChExactMatchTtiPortProfileIdModePortPacketTypeSet(devNum, emUnitNum, portNum ,keyType,
                                                                          ttiLookupNum, enableExactMatchLookup, profileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, portNum ,keyType, ttiLookupNum, enableExactMatchLookup, profileId));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    * SIP7: Not applicable on Reduced EM unit */
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
         ...
         * Channel calculation, e.g
         * EM unit 1, lookup 1 - 1*2 + 1 = 3 */
        emChannel = (emUnitNum * 2) + pclHitNum;
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.emChannel[emChannel];
        rc =  prvCpssHwPpGetRegField(devNum, regAddr, 0, 4, &value);
        if (rc != GT_OK)
        {
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, direction ,packetType,  subProfileId, pclLookupNum, enableExactMatchLookup, profileId));

    rc = internal_cpssDxChExactMatchPclProfileIdSet(devNum, emUnitNum, direction ,packetType, subProfileId,
                                                    pclLookupNum, enableExactMatchLookup, profileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, direction ,packetType, subProfileId, pclLookupNum, enableExactMatchLookup, profileId));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    * SIP7: Not applicable on Reduced EM unit */
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
         ...
         * Channel calculation, e.g
         * EM unit 1, lookup 1 - 1*2 + 1 = 3 */
        emChannel = (emUnitNum * 2) + clientIndex;
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.emChannel[emChannel];
        value = ((value*2) + clientIndex) & 0xF;
        rc =  prvCpssHwPpSetRegField(devNum, regAddr, 0, 4, value);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchLookupNum, clientType));

    rc = internal_cpssDxChExactMatchClientLookupSet(devNum, emUnitNum, exactMatchLookupNum, clientType);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, exactMatchLookupNum, clientType));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchLookupNum,clientTypePtr));

    rc = internal_cpssDxChExactMatchClientLookupGet(devNum,emUnitNum,exactMatchLookupNum,clientTypePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, exactMatchLookupNum,clientTypePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CHECK_IF_EM_UNIT_SUPPORTED_MAC(devNum);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    /* check validity of EM unit */
    PRV_CPSS_DXCH_EXACT_MATCH_EM_UNIT_NUM_CHECK_MAC(devNum, emUnitNum);
    /* check validity of EM lookupNum */
    PRV_CPSS_DXCH_EXACT_MATCH_EM_LOOKUP_NUM_CHECK_MAC(lookupNum);

    if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[(2*emUnitNum)+lookupNum].EMGlobalConfiguration.EMGlobalConfiguration1;
    }
    else
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->REDUCED_EM.EMGlobalConfiguration.EMGlobalConfiguration1;
    }
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, enable));

    rc = internal_cpssDxChExactMatchActivityBitEnableSet(devNum, emUnitNum, lookupNum, enable);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, enable));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    GT_U32    fieldOffset;
    GT_U32    portGroupId;
    PRV_CPSS_DXCH_UNIT_ENT    emUnitId;
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_EM_LOOKUP_NUM_CHECK_MAC(lookupNum);
    /* Get the emUnit ID */
    PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_EM_UNIT_NUM_TO_EM_UNIT_ID_MAC(emUnitNum, lookupNum, emUnitId);
    PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_UNIT_MAC(devNum, portGroupsBmp, emUnitId);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, enablePtr));

    rc = internal_cpssDxChExactMatchActivityBitEnableGet(devNum, emUnitNum, lookupNum, enablePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, enablePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    /* Get the emUnit Table ID */
    PRV_CPSS_DXCH_EXACT_MATCH_GET_EM_TABLE_ID_FOR_EM_UNIT_NUM_MAC(emUnitNum, lookupNum, emTableId);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp,exactMatchEntryIndex,
                            exactMatchClearActivity,exactMatchActivityStatusPtr));

    rc = internal_cpssDxChExactMatchActivityStatusGet(devNum, emUnitNum, lookupNum, portGroupsBmp,
                                                      exactMatchEntryIndex, exactMatchClearActivity,
                                                      exactMatchActivityStatusPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp,exactMatchEntryIndex,
                              exactMatchClearActivity,exactMatchActivityStatusPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* Key Size configuration is applicable only for EM units 0, 1, 2 & 3 */
        /* For reduced EM unit, keysize is 5B by default */
        if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].
                                                               exactMatchProfileTableConfig[exactMatchProfileIndex];
            /* KeySize */
            rc = prvCpssHwPpSetRegField(devNum,regAddr,1,2,value);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    else
    {
        ...
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.exactMatchProfilesTable.
                                                           exactMatchProfileTableControl[exactMatchProfileIndex];
        /* KeySize */
    ...
    maxKeySize = PRV_CPSS_DXCH_EXACT_MATCH_MAX_KEY_SIZE_CNS(devNum, emUnitNum);

    /* loop on PRV_CPSS_DXCH_EXACT_MATCH_MAX_KEY_SIZE_CNS bytes of data */
    for (i=0; i < maxKeySize; i = i+4)
    {
        ...
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            ...
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].
                                                    exactMatchProfilesTableMaskData[exactMatchProfileIndex][wordNum];
            }
            else
            {
                if (wordNum < 2)
                {
                    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.ilmExactMatchProfileTable.
                                                        ilmExactMatchProfileTableMaskData[exactMatchProfileIndex][wordNum];
                }
                else
                {
                    /* for Reduced EM the keysize is 5 Bytes and it should not
                     * exceed 2 words */
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
                }
            }
        ...
        {
            /* Check if less than 12 words */
            if (wordNum < (((CPSS_DXCH_EXACT_MATCH_MAX_KEY_SIZE_CNS-6)/4)+1))
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.exactMatchProfilesTable.
         ...
         * For SIP7 devices the maximum is 53 bytes (13 words) */
        value = ((i+3) < maxKeySize)?(keyParamsPtr->mask[i + 3]):0;
        value = ((i+2) < maxKeySize)?((value<<8)|(keyParamsPtr->mask[i+2])):0;
        value = ((i+1) < maxKeySize)?((value<<8)|(keyParamsPtr->mask[i+1])):0;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex,keyParamsPtr));

    rc = internal_cpssDxChExactMatchProfileKeyParamsSet(devNum, emUnitNum, exactMatchProfileIndex,keyParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum,emUnitNum,exactMatchProfileIndex,keyParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
        {
            regAddr1 = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].
                                                                    exactMatchProfileTableConfig[exactMatchProfileIndex];
        }
    }
    else
    {
        /* set ketStart and KeySize fields from control table */
        regAddr1 = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.exactMatchProfilesTable.
                                                                 exactMatchProfileTableControl[exactMatchProfileIndex];
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].exactMatchProfileTableConfig[exactMatchProfileIndex];
            /* KeySize */
        ...
        {
            /* For reduced EM unit, keysize is 5B by default */
            value = CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E;
        }

        PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_HW_TO_KEY_SIZE_VAL_MAC(devNum, (keyParamsPtr->keySize), value);
        /* Key Start field is not relevant for AAS */
        keyParamsPtr->keyStart = 0;
    }
    else
    {
...
)
{
    ...
    rc = internal_cpssDxChExactMatchProfileKeyParamsGet(devNum, emUnitNum, exactMatchProfileIndex,keyParamsPtr);
...
)
{
    GT_STATUS rc=GT_OK;
    GT_U32    regAddr = 0;
    GT_U32    hwDefaultActionArray[PRV_CPSS_DXCH_EXACT_MATCH_DEFAULT_ACTION_MAX_SIZE_CNS];
    GT_U32    i=0;/* for loop */
    GT_U32    wordsInAction;
    ...
    wordsInAction = PRV_CPSS_SIP_7_CHECK_MAC(devNum)
        ? PRV_CPSS_DXCH_EXACT_MATCH_DEFAULT_ACTION_SIP7_SIZE_CNS
        : PRV_CPSS_DXCH_EXACT_MATCH_DEFAULT_ACTION_SIZE_CNS;
    ...
    if (defActionEn==GT_TRUE)
    {
        ...
        for (i = 0; (i < wordsInAction); i++)
        {
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                ...
                {
                    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.ilmExactMatchProfileTable.
                                                    ilmExactMatchProfileTableDefaultActionData[exactMatchProfileIndex][i];
                }
            }
            else
            {
                if (i < PRV_CPSS_DXCH_EXACT_MATCH_DEFAULT_ACTION_SIZE_CNS)
                {
                    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.exactMatchProfilesTable.
                                                          exactMatchProfileTableDefaultActionData[exactMatchProfileIndex][i];
                }
    ...
    /* set enable default field from control table */
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
        {
        ...
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.ilmExactMatchProfileTable.ilmExactMatchProfileTableConfig[exactMatchProfileIndex];
        }
        rc = prvCpssHwPpSetRegField(devNum,regAddr,0,1,BOOL2BIT_MAC(defActionEn));
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex,actionType,
                            actionPtr,defActionEn));

    rc = internal_cpssDxChExactMatchProfileDefaultActionSet(devNum, emUnitNum, exactMatchProfileIndex,
                                                            actionType,actionPtr,defActionEn);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum,emUnitNum,exactMatchProfileIndex,actionType,
                            actionPtr,defActionEn));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.ilmExactMatchProfileTable.
                                                                   ilmExactMatchProfileTableConfig[exactMatchProfileIndex];
        }
        /* loop on all active port groups in the bmp */
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
        {
            rc = prvCpssHwPpPortGroupSetRegField(devNum,portGroupId,regAddr,0,1,BOOL2BIT_MAC(defActionEn));
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    rc = internal_cpssDxChExactMatchProfileDefaultActionGet(devNum, emUnitNum, exactMatchProfileIndex,
...
)
{
    ...
    {
        /* get enable default field from control table */
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.exactMatchProfilesTable.
                                                                exactMatchProfileTableControl[exactMatchProfileIndex];
...
)
{
    ...
    /* sip6 : exactMatchActionAssignment[16][32] Packet Type/Byte*/
    /* sip5 : exactMatchActionAssignment[16][31] Packet Type/Byte*/
    expandedActionSize = PRV_CPSS_DXCH_EXACT_MATCH_EXPANDED_ACTION_MAX_SIZE_CNS(devNum);

    for (i = 0; i < expandedActionSize; i++)
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].
                                                                   exactMatchProfileTableReducedActionData[expandedActionIndex][i];
            }
            else
            {
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.ilmExactMatchProfileTable.
                                                                   ilmExactMatchProfileTableReducedActionData[expandedActionIndex][i];
            }
        }
        else
        {
            if (i < PRV_CPSS_SIP6_10_DXCH_EXACT_MATCH_EXPANDED_ACTION_SIZE_CNS)
            {
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, expandedActionIndex,actionType,
                            actionPtr,expandedActionOriginPtr));

    rc = internal_cpssDxChExactMatchExpandedActionSet(devNum, emUnitNum,
                                                      expandedActionIndex, actionType,
                                                      actionPtr,expandedActionOriginPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum,emUnitNum,expandedActionIndex,actionType,
                            actionPtr,expandedActionOriginPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    rc = internal_cpssDxChExactMatchExpandedActionGet(devNum, emUnitNum,
                                                      expandedActionIndex,actionType,
                                                      actionPtr,expandedActionOriginPtr);
...
)
{
    ...
    else
    {
        if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
        {
            if(logicFormatPtr->redirectCommand==CPSS_DXCH_TTI_ASSIGN_GENERIC_ACTION_E)
            {
                if (ttiExpandedActionOriginPtr->overwriteExpandedActionGenericAction==GT_FALSE)
                {
                    /* check that the fields are equal */
                    if (logicFormatPtr->genericAction != dbLogicFormatPtr->genericAction)
                    {
                         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "mismatch in genericAction field \n");
                    }
                }
            }
        }
...
)
{
   GT_STATUS   rc=GT_OK;
    ...
    if((PRV_CPSS_SIP_6_30_CHECK_MAC(devNum)) &&
       (logicFormatPtr->redirect.redirectCmd == CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E))
    {
        if (pclExpandedActionOriginPtr->overwriteExpandedActionGenericAction==GT_FALSE)
        {
            /* check that the fields are equal */
            if (logicFormatPtr->redirect.data.genericActionInfo.genericAction != dbLogicFormatPtr->redirect.data.genericActionInfo.genericAction)
            {
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "mismatch in redirect.data.genericActionInfo.genericAction field \n");
            }
        }
    }
...
)
{
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        if (emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
        {
            regAddr = PRV_DXCH_REG1_UNIT_EM_MAC(devNum,(2*emUnitNum) + lookupNum).EMGlobalConfiguration.EMGlobalConfiguration1;
        }
        else
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->REDUCED_EM.EMGlobalConfiguration.EMGlobalConfiguration1;
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMGlobalConfiguration.EMGlobalConfiguration2;
        fieldOffset = 2;
    }
    ...
    if (((((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E) || ((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_53B_E)) &&(bankNum%4!=0))||
        ((((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E) || ((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E))&&(bankNum%3!=0))||
        ((((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E) || ((keySize)==CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E))&&(bankNum%2!=0)))
    {
...
)
{
    ...
    for (i=0;
          (i<expandedActioSize &&
           numberOfReducedBytesUsed < maxReducedBytes);
          i++)
    {
        /* reset values for new loop */
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E)
        {
            ...
            /*  bit 0
                0x0 = invalid
                0x1 = valid */
            SIP6_EXACT_MATCH_ENTRY_FIELD_SET_MAC(devNum,&hwDataArr[5],
                                            SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                            1);/* set valid bit to 1 */
            /*  bit 1
                Defines the Exact Match entry type
        ...
        if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E)
        {
            ...
            /*  bit 0
                0x0 = invalid
                0x1 = valid */
            SIP6_EXACT_MATCH_ENTRY_FIELD_SET_MAC(devNum,&hwDataArr[10],
                                            SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                            1);/* set valid bit to 1 */
            /*  bit 1
                Defines the Exact Match entry type
            ...
            SIP6_EXACT_MATCH_ENTRY_FIELD_KEY_ONLY_SET_MAC(devNum,
                                                          (&hwDataArr[10]),
                                                          (&entryPtr->key.pattern[21]));
        }

        if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E)
        {
            ...
            /*  bit 0
                0x0 = invalid
                0x1 = valid */
            SIP6_EXACT_MATCH_ENTRY_FIELD_SET_MAC(devNum,&hwDataArr[15],
                                            SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                            1);/* set valid bit to 1 */
            /*  bit 1
                Defines the Exact Match entry type
...
/*GT_U32 i;*/
    ...
    *validPtr=GT_TRUE;
    ...
    SIP6_EXACT_MATCH_ENTRY_FIELD_GET_MAC(devNum,hwDataArr,
                                         SIP6_EXACT_MATCH_TABLE_FIELDS_KEY_SIZE_E,
                                         value);
    PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_HW_TO_KEY_SIZE_VAL_MAC(devNum, (entryPtr->key.keySize), value);
    ...
    if(fullEntryEnable==GT_TRUE)
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E)
            {
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E */
                /*  bit 0
                    0x0 = invalid
                    0x1 = valid */
                SIP6_EXACT_MATCH_ENTRY_FIELD_GET_MAC(devNum,(&hwDataArr[5]),
                                                    SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                                    value);/* get valid bit to 1*/
                ...
                Bits from the Concatenated Key.
                The location of the bits depend on the location of the entry in the Concatenated Chain, as follows:
                First Entry after the action: Bits [167:40] of the Concatenated Key
                Second Entry after the action: Bits [295:168] of the Concatenated Key
                Third Entry after the action: Bits [423:296] of the Concatenated Key*/
            ...
            if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E)
            {
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E */
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E */
                /*  bit 0
                    0x0 = invalid
                    0x1 = valid */
                SIP6_EXACT_MATCH_ENTRY_FIELD_GET_MAC(devNum,(&hwDataArr[10]),
                                                    SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                                    value);/* get valid bit to 1*/
            ...
            if (entryPtr->key.keySize>CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E)
            {
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_53B_E */
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E */
                /* build next bank for size CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E */
                /*  bit 0
                    0x0 = invalid
                    0x1 = valid */
                SIP6_EXACT_MATCH_ENTRY_FIELD_GET_MAC(devNum,(&hwDataArr[15]),
                                                    SIP6_EXACT_MATCH_TABLE_FIELDS_VALID_E,
                                                    value);/* get valid bit to 1*/
...
)
{
    ...
    GT_U32    portGroupId;/*the port group Id - support multi-port-groups device */
    /* SIP6: the final 8 bytes to set the HW
       SIP7: the final 10 bytes to set the HW*/
    GT_U8     reducedActionHwDataArr[PRV_CPSS_DXCH_EXACT_MATCH_SIP7_REDUCED_ACTION_SIZE_CNS];
    /* SIP6: 8 words - 256 bits of data
       SIP7: 12 words - 354 bits of data*/
    GT_U32    exactMatchReducedHwArray[PRV_CPSS_DXCH_EXACT_MATCH_ACTION_MAX_SIZE_CNS];
    ...
    /* SIP6: data for one bank - 115 bits
       SIP7: data for one bank - 130 bits */
    GT_U32    singleHwDataArr[PRV_CPSS_DXCH_EXACT_MATCH_ENTRY_BANK_SIZE_IN_WORDS_CNS];
    GT_U32    startWord, entryIndexPerBank;
    GT_U32    bankNum=0,numOfBanks=0;
    GT_U32    regAddr=0,value,line,bank,hwIndex,valid;
    GT_U32    exactMatchHwActionArray[PRV_CPSS_DXCH_EXACT_MATCH_ACTION_MAX_SIZE_CNS];/* the full Hw representation for actionPtr */
    PRV_CPSS_DXCH_UNIT_ENT    emUnitId;
    CPSS_DXCH_TABLE_ENT       emTableId;
    GT_U32                    emEntryWidth;
    /*GT_U32 i;*/
    ...
    rc = prvCpssDxChExactMatchIndexValidity(devNum,emUnitNum, lookupNum, index,entryPtr->key.keySize);
    if (rc  != GT_OK)
    {
    ...
    EM_LOCK(devNum, HW_SEMA_IPFIX_EM);
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !(PRV_CPSS_SIP_7_CHECK_MAC(devNum)))
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMSemaphore;
        value = 0;
       ...
       /* loop on all active port groups in the bmp:
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum, portGroupsBmp, portGroupId)
        {
            ...
            if (rc != GT_OK)
            {
                EM_UNLOCK(devNum, HW_SEMA_IPFIX_EM);
                rc1 = rc;
                goto exit_cleanly_lbl;
            }

            emEntryWidth = (PRV_CPSS_SIP_7_CHECK_MAC(devNum))?
                            PRV_CPSS_DXCH_EXACT_MATCH_ENTRY_BANK_SIZE_IN_WORDS_CNS:
                            (PRV_CPSS_DXCH_EXACT_MATCH_ENTRY_BANK_SIZE_IN_WORDS_CNS-1);
            /* Read all banks of specific index to check that the entry is still empty */
            for (bankNum=0; bankNum<numOfBanks; bankNum++)
            {
                startWord = bankNum*emEntryWidth;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index, expandedActionIndex,
                            entryPtr,actionType,actionPtr));

    rc = internal_cpssDxChExactMatchPortGroupEntrySet(devNum, emUnitNum, lookupNum, portGroupsBmp, index,
                                                      expandedActionIndex,entryPtr,actionType,
                                                      actionPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index, expandedActionIndex,
                            entryPtr,actionType,actionPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index,
                            validPtr,actionTypePtr,actionPtr,entryPtr,expandedActionIndexPtr));

    rc = internal_cpssDxChExactMatchPortGroupEntryGet(devNum, emUnitNum, lookupNum, portGroupsBmp, index,
                                                      validPtr,actionTypePtr,actionPtr,entryPtr,
                                                      expandedActionIndexPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index,
                            validPtr,actionTypePtr,actionPtr,entryPtr,expandedActionIndexPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        PRV_CPSS_DXCH_EXACT_MATCH_CONVERT_HW_TO_KEY_SIZE_VAL_MAC(devNum, keySize, ((value>>3)&0x3));

        /* check validity of Exact Match Entry index */
        rc = prvCpssDxChExactMatchIndexValidity(devNum,emUnitNum,lookupNum,index,keySize);
        if (rc  != GT_OK)
        {
             EM_UNLOCK(devNum, HW_SEMA_IPFIX_EM);
...
)
{
    ...
    rc = internal_cpssDxChExactMatchPortGroupEntryInvalidate(devNum, emUnitNum, lookupNum, portGroupsBmp, index);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index, validPtr));

    rc = internal_cpssDxChExactMatchPortGroupEntryStatusGet(devNum, emUnitNum, lookupNum, portGroupsBmp, index, validPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, index, validPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    /* SIP6: max key size 47B,  numOfBitsInData is 376 bits
     * SIP7: max key size 53B,  numOfBitsInData is 424 bits*/
    numOfBitsInData = (PRV_CPSS_SIP_7_CHECK_MAC(devNum))?
                       ((emUnitNum < CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)?
                       PRV_CPSS_DXCH_SIP7_EXACT_MATCH_HASH_DATA_SIZE_VALUE_CNS:
                       PRV_CPSS_DXCH_SIP7_EXACT_MATCH_REDUCED_HASH_DATA_SIZE_VALUE_CNS):
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, entryKeyPtr,numberOfElemInCrcMultiHashArrPtr,exactMatchCrcMultiHashArr));

    rc = internal_cpssDxChExactMatchHashCrcMultiResultsByParamsCalc(devNum, emUnitNum, entryKeyPtr,
                                                                    numberOfElemInCrcMultiHashArrPtr,
                                                                    exactMatchCrcMultiHashArr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum,emUnitNum,entryKeyPtr,numberOfElemInCrcMultiHashArrPtr,exactMatchCrcMultiHashArr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E:
        bytesNum=21;
        cpssOsPrintf(rule_ind_Hdr, ruleIndex,lookupNum, "21_BYTES");
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E:
        bytesNum=37;
        cpssOsPrintf(rule_ind_Hdr, ruleIndex,lookupNum, "37_BYTES");
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_53B_E:
        bytesNum=53;
        cpssOsPrintf(rule_ind_Hdr, ruleIndex,lookupNum, "53_BYTES");
        break;
    default:
...
)
{
    ...
    GT_U32                                          actionArr[PRV_CPSS_DXCH_EXACT_MATCH_ACTION_MAX_SIZE_CNS];/* the full Hw representation for actionPtr */
    /* SIP6: the final 8 bytes to set the HW
       SIP7: the final 10 bytes to set the HW */
    GT_U8                                           reducedActionArr[PRV_CPSS_DXCH_EXACT_MATCH_SIP7_REDUCED_ACTION_SIZE_CNS];
    ...
    for (exactMatchIndex = startIndex; exactMatchIndex <= (startIndex + numOfRules); exactMatchIndex++)
    {
        ...
        rc = prvUtilCpssDxChExactMatchIsValid(devNum,emUnitNum,lookupNum,portGroupsBmp,exactMatchIndex,&isValid);
        ...
        rc = prvUtilCpssDxChExactMatchRuleGet(devNum,emUnitNum,lookupNum,portGroupsBmp,exactMatchIndex,&isValid,
        ...
        prvUtilCpssDxChExactMatchRuleDump(
            devNum,
            exactMatchIndex,
    ...
    {
        rc = cpssDxChExactMatchProfileKeyParamsGet(devNum, emUnitNum, exactMachProfileIndex, &keyParams);
        if (rc != GT_OK)
        {
        ...
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_21B_E:
            maskBytesNum=21;
            cpssOsPrintf(key_params_Hdr,exactMachProfileIndex, "21_BYTES", keyParams.keyStart);
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_37B_E:
            maskBytesNum=37;
            cpssOsPrintf(key_params_Hdr,exactMachProfileIndex, "37_BYTES", keyParams.keyStart);
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_53B_E:
            maskBytesNum=53;
            cpssOsPrintf(key_params_Hdr,exactMachProfileIndex, "53_BYTES", keyParams.keyStart);
            break;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, lookupNum, portGroupsBmp, startIndex, numOfRules));

    rc = internal_cpssDxChExactMatchRulesDump(devNum, emUnitNum, lookupNum, portGroupsBmp, startIndex, numOfRules);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, lookupNum, portGroupsBmp, startIndex, numOfRules));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_AC3X_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AAS_E);
    CHECK_IF_EM_UNIT_SUPPORTED_MAC(devNum);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    /* check validity of EM unit */
    PRV_CPSS_DXCH_EXACT_MATCH_EM_UNIT_NUM_CHECK_MAC(devNum, emUnitNum);
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMGlobalConfiguration.EMGlobalConfiguration1;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchAutoLearnLookupNum));

    rc = internal_cpssDxChExactMatchAutoLearnLookupSet(devNum, emUnitNum, exactMatchAutoLearnLookupNum);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchAutoLearnLookupNum));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchAutoLearnLookupNumPtr));

    rc = internal_cpssDxChExactMatchAutoLearnLookupGet(devNum, emUnitNum, exactMatchAutoLearnLookupNumPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchAutoLearnLookupNumPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMProfileConfiguration2[exactMatchProfileIndex];
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMProfileConfiguration3[exactMatchProfileIndex];
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMProfileConfiguration4[exactMatchProfileIndex];
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMProfileConfiguration1[exactMatchProfileIndex];
    value = 0;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndex, actionType, actionPtr, actionEn));

    rc = internal_cpssDxChExactMatchAutoLearnProfileDefaultActionSet(devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndex, actionType, actionPtr, actionEn);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndex, actionType, actionPtr, actionEn));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndexPtr, actionTypePtr, actionPtr, actionEnPtr));

    rc = internal_cpssDxChExactMatchAutoLearnProfileDefaultActionGet(devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndexPtr, actionTypePtr, actionPtr, actionEnPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchProfileIndex, expandedActionIndexPtr, actionTypePtr, actionPtr, actionEnPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex, actionEn));

    rc = internal_cpssDxChExactMatchAutoLearnProfileDefaultActionEnableSet(devNum, emUnitNum, exactMatchProfileIndex, actionEn);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchProfileIndex, actionEn));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, exactMatchProfileIndex, actionEnPtr));

    rc = internal_cpssDxChExactMatchAutoLearnProfileDefaultActionEnableGet(devNum, emUnitNum, exactMatchProfileIndex, actionEnPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, exactMatchProfileIndex, actionEnPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMProfileConfiguration1[i];
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdAllocationConfiguration1;
        ...
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdAllocationConfiguration2;
    ...
    /* enable counting */
    rc = prvCpssHwPpSetRegField(devNum,
           PRV_DXCH_REG1_UNIT_EM_MAC(devNum,CPSS_DXCH_EXACT_MATCH_UNIT_0_E).EMGlobalConfiguration.EMGlobalConfiguration1,
           7,
           1,
           1);
    if (rc != GT_OK)
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portGroupsBmp, confPtr));

    rc = internal_cpssDxChExactMatchPortGroupAutoLearnFlowIdParamSet(devNum, emUnitNum, portGroupsBmp, confPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, emUnitNum, portGroupsBmp, confPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    rc = internal_cpssDxChExactMatchPortGroupAutoLearnFlowIdParamGet(devNum, emUnitNum, portGroupsBmp, confPtr);
...
)
{
    ...
    * SIP7: Not applicable on Reduced EM unit */
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdAllocationStatus1;
...
)
{
    ...
    rc = internal_cpssDxChExactMatchPortGroupAutoLearnFlowIdAllocStatusGet(devNum, emUnitNum, portGroupsBmp, statusPtr);
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdAllocationStatus2;
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdAllocationStatus3;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portGroupsBmp, flowIdPtr, exactMatchIndexPtr));

    rc = internal_cpssDxChExactMatchPortGroupAutoLearnEntryGetNext(devNum, emUnitNum, portGroupsBmp,
                                                                   flowIdPtr, exactMatchIndexPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, portGroupsBmp, flowIdPtr, exactMatchIndexPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMAutoLearningCollisionStatus1;
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMAutoLearningCollisionStatus_n[i];
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portGroupsBmp, keyParamsPtr));

    rc = internal_cpssDxChExactMatchPortGroupAutoLearnCollisionKeyParamsGet(devNum, emUnitNum, portGroupsBmp, keyParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, portGroupsBmp, keyParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    rc = internal_cpssDxChExactMatchPortGroupAutoLearnEntryRecycleGet(devNum, emUnitNum, portGroupsBmp,
                                                                      flowIdPtr, exactMatchIndexPtr);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, portGroupsBmp));

    rc = internal_cpssDxChExactMatchPortGroupAutoLearnEntryRecycleTrigger(devNum, emUnitNum, portGroupsBmp);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum,  emUnitNum, portGroupsBmp));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMFlowIdFailCounter;
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EM[emUnitNum].EMAutoLearning.EMIndexFailCounter;
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum,  emUnitNum, portGroupsBmp, failCounterPtr));

    rc = internal_cpssDxChExactMatchPortGroupAutoLearnFailCountersGet(devNum, emUnitNum, portGroupsBmp, failCounterPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, portGroupsBmp, failCounterPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChExactMatchSerialEmPortProfileIdSet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchSerialEmPortProfileIdSet
(
    IN GT_U8                          devNum,
    IN GT_PORT_NUM                    portNum,
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E |
                                          ...
                                          CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E);

    CHECK_IF_EM_UNIT_SUPPORTED_MAC(devNum);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    /* check validity of portProfileId */
    PRV_CPSS_DXCH_EXACT_MATCH_PORT_PROFILE_ID_VALUE_CHECK_MAC(portProfileId);
    ...
    return prvCpssDxChWriteTableEntryField(devNum,
                                           ...
                                           portNum,
                                           PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                                           SIP7_TTI_DEFAULT_EPORT_TABLE_FIELDS_SERIAL_EM_PORT_PROFILE_ID_E, /* field name */
                                           PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
                                           portProfileId);
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchSerialEmPortProfileIdSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, portProfileId));

    rc = internal_cpssDxChExactMatchSerialEmPortProfileIdSet(devNum, portNum, portProfileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, portNum, portProfileId));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChExactMatchSerialEmPortProfileIdGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchSerialEmPortProfileIdGet
(
    IN  GT_U8                          devNum,
    IN  GT_PORT_NUM                    portNum,
...
)
{
    ...
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    CPSS_NULL_PTR_CHECK_MAC(portProfileIdPtr);
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchSerialEmPortProfileIdGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, portProfileIdPtr));

    rc = internal_cpssDxChExactMatchSerialEmPortProfileIdGet(devNum, portNum, portProfileIdPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, portNum, portProfileIdPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChExactMatchIlmProfileIdMappingTableEntrySet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchSerialEmProfileIdMappingTableEntrySet
(
    IN GT_U8                                            devNum,
...
)
{
    ...
    CHECK_IF_EM_UNIT_SUPPORTED_MAC(devNum);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    /* check validity of packetType */
    PRV_CPSS_DXCH_EXACT_MATCH_SERIAL_EM_PACKET_TYPE_CHECK_MAC(packetType);
    /* check validity of portProfileId */
    PRV_CPSS_DXCH_EXACT_MATCH_PORT_PROFILE_ID_VALUE_CHECK_MAC(portProfileId);
    /* check validity of serial EM type */
    PRV_CPSS_DXCH_EXACT_MATCH_SERIAL_EM_TYPE_CHECK_MAC(serialEmType);
    /* check validity of serialEmProfileParamsPtr */
    CPSS_NULL_PTR_CHECK_MAC(serialEmProfileParamsPtr);
    /* check validity of emProfileId */
    PRV_CPSS_DXCH_EXACT_MATCH_CHECK_PROFILE_ID_VALUE_MAC(serialEmProfileParamsPtr->serialEmProfileId);
    /* check validity of serial EM KeyTpe */
    PRV_CPSS_DXCH_EXACT_MATCH_SERIAL_EM_KEY_TYPE_CHECK_MAC(serialEmProfileParamsPtr->serialEmKeyType);
    /* check validity of PCL ID */
    PRV_CPSS_DXCH_EXACT_MATCH_SERIAL_EM_PCL_ID_CHECK_MAC(serialEmProfileParamsPtr->serialEmPclId);

    /* Concatenate packetType(4b) and portProfileId(3b) as the 7b key */
    serialEmKey = (((packetType & 0xF) << 3) | (portProfileId & 0x7));
    if (serialEmKey >= PRV_CPSS_DXCH_EXACT_MATCH_SERIAL_EM_PROFILE_ID_TABLE_MAX_ENTRIES_VAL_CNS)
    {
        /* Serial EM key is greater than 127, hence return error */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);
    }

    regVal = ((serialEmProfileParamsPtr->serialEmProfileId & 0xF)                   |
             ((serialEmProfileParamsPtr->serialEmKeyType & 0x3) << 4)               |
             ((serialEmProfileParamsPtr->serialEmEnableUDB29VlanTagKey & 0x1) << 6) |
             ((serialEmProfileParamsPtr->serialEmEnableUDB28VlanTagKey & 0x1) << 7) |
             ((serialEmProfileParamsPtr->serialEmPclId & 0x3) << 8));

    /* set Serial EM params in the ILM LU configuration table */
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ilmLuConfig[serialEmType].ilmLuSerialEmProfile[serialEmKey];
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 32, regVal);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal cpssDxChExactMatchIlmProfileIdMappingTableEntrySet function
* @endinternal
*
* @brief   Sets the Serial Exact match Profile Id mapping Entry which
*          maps the Serial EM Type for the given source port and packetType
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchSerialEmProfileIdMappingTableEntrySet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetType, portProfileId, serialEmType, serialEmProfileParamsPtr));

    rc = internal_cpssDxChExactMatchSerialEmProfileIdMappingTableEntrySet(devNum, packetType,
                                                                          portProfileId, serialEmType,
                                                                          serialEmProfileParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, packetType, portProfileId, serialEmType, serialEmProfileParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChExactMatchSerialEmProfileIdMappingTableEntryGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchSerialEmProfileIdMappingTableEntryGet
(
    IN  GT_U8                                            devNum,
    IN  CPSS_DXCH_EXACT_MATCH_SERIAL_EM_PACKET_TYPE_ENT  packetType,
    IN  GT_U32                                           portProfileId,
...
)
{
    ...
    /* Get the Serial EM params in the ILM LU configuration table */
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ilmLuConfig[serialEmType].ilmLuSerialEmProfile[serialEmKey];
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 32, &regVal);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    serialEmProfileParamsPtr->serialEmEnableUDB29VlanTagKey  = BIT2BOOL_MAC((regVal >> 6) & 0x1);
    serialEmProfileParamsPtr->serialEmEnableUDB28VlanTagKey  = BIT2BOOL_MAC((regVal >> 7) & 0x1);
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchSerialEmProfileIdMappingTableEntryGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetType, portProfileId, serialEmType, serialEmProfileParamsPtr));

    rc = internal_cpssDxChExactMatchSerialEmProfileIdMappingTableEntryGet(devNum, packetType,
                                                                          portProfileId, serialEmType,
                                                                          serialEmProfileParamsPtr);
...
/**
* @internal internal_cpssDxChExactMatchProfileMuxTableLineSet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchProfileMuxTableLineSet
(
    IN GT_U8                                         devNum,
    IN CPSS_DXCH_EXACT_MATCH_UNIT_NUM_ENT            emUnitNum,
...
{
    GT_STATUS rc = GT_OK;
    GT_U32    word, select, i, regAddr, value;
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_EM_UNIT_NUM_CHECK_MAC(devNum, emUnitNum);
    /* check validity of emProfileId */
    PRV_CPSS_DXCH_EXACT_MATCH_CHECK_PROFILE_ID_VALUE_MAC(emProfileId);
    /* check validity of emMuxTableLineCfgPtr */
    CPSS_NULL_PTR_CHECK_MAC(emMuxTableLineCfgPtr);
    /* Mux Table is not supported for Reduced EM unit */
    if (emUnitNum == CPSS_DXCH_EXACT_MATCH_UNIT_REDUCED_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "Mux key is not supported for Reduced EM unit\n");
    }
    /* check validity of superkey Two byte offsets */
    for (i = 0; (i < PRV_CPSS_DXCH_EXACT_MATCH_MUX_MAX_KEY_BYTE_PAIRS_OFFSET_VAL_CNS); i++)
    {
        if (emMuxTableLineCfgPtr->superKeyTwoByteUnitsOffsets[i] >= PRV_CPSS_DXCH_EXACT_MATCH_MUX_MAX_KEY_BYTE_PAIRS_VAL_CNS)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "superKeyTwoByteUnitsOffsets out of range in byte %u\n", i);
        }
    }
    for (word = 0; (word < PRV_CPSS_DXCH_EXACT_MATCH_MUX_MAX_10_BYTE_SELECT_WORDS_VAL_CNS); word++)
    {
        value = 0;
        for (select = 0; (select < 5); select++)
        {
            value |= (emMuxTableLineCfgPtr->superKeyTwoByteUnitsOffsets[(word * 5) + select] << (6 * select));
        }
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].exactMatchProfileTable10BSelect[emProfileId][word];
        rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 30, value);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchProfileMuxTableLineSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, emProfileId, emMuxTableLineCfgPtr));

    rc = internal_cpssDxChExactMatchProfileMuxTableLineSet(devNum, emUnitNum, emProfileId,
                                                           emMuxTableLineCfgPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, emProfileId, emMuxTableLineCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChExactMatchProfileMuxTableLineGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchProfileMuxTableLineGet
(
    IN  GT_U8                                         devNum,
    IN CPSS_DXCH_EXACT_MATCH_UNIT_NUM_ENT             emUnitNum,
    IN  GT_U32                                        emProfileId,
...
)
{
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EMX.exactMatchProfilesTable[emUnitNum].exactMatchProfileTable10BSelect[emProfileId][word];
        rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 30, &value);
        if(rc != GT_OK)
        {
            return rc;
        }
        for (select = 0; (select < 5); select++)
        {
            emMuxTableLineCfgPtr->superKeyTwoByteUnitsOffsets[(word * 5) + select] = ((value >> (6 * select)) & 0x3F);
        }
    }
    for (i = 0; (i < PRV_CPSS_DXCH_EXACT_MATCH_MUX_MAX_KEY_BYTE_PAIRS_OFFSET_VAL_CNS); i++)
    {
        if (emMuxTableLineCfgPtr->superKeyTwoByteUnitsOffsets[i] >= PRV_CPSS_DXCH_EXACT_MATCH_MUX_MAX_KEY_BYTE_PAIRS_VAL_CNS)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "superKeyTwoByteUnitsOffsets out of range at byte %u\n", i);
        }
    }
    return rc;
}
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchProfileMuxTableLineGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, emUnitNum, emProfileId, emMuxTableLineCfgPtr));

    rc = internal_cpssDxChExactMatchProfileMuxTableLineGet(devNum, emUnitNum, emProfileId,
                                                           emMuxTableLineCfgPtr);
    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, devNum, emUnitNum, emProfileId, emMuxTableLineCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
