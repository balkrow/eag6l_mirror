...
prvCpssMacSecExtByPassEnable(GT_U8 devNum, GT_U32 dpId, GT_U32 unitType, GT_U32 unitDir, GT_BOOL macSecBypass)
{
    ...
    if ((unitType == 1) && (unitDir == 1) &&
        (CPSS_PP_FAMILY_DXCH_IRONMAN_E == PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        /* Update TX_DMA profiles values depending from MACSEC */
        GT_BOOL macSecEnable = (macSecBypass == GT_FALSE) ? GT_TRUE : GT_FALSE;
