...
#define EIP164_SECY_ALL_CLOCKS (EIP164_SECY_DEVICE_PE_CLOCK | \
                                ...
                                EIP164_SECY_DEVICE_PE_SM4_KS_CLOCK | \
                                EIP164_SECY_DEVICE_PE_SM4_CLOCK)
        ...
        const SecY_Settings_t * const Settings_p)
{
    ...
    Settings.VPInvDropType       = EIP164_SECY_SA_DROP_INTERNAL;
    Settings.FLInvDropType       = EIP164_SECY_SA_DROP_INTERNAL;
    Settings.fBypassControlPkts  = !Settings_p->fDropControlPkts;

    Settings.SARetireTimerPrescale = Settings_p->SARetireTimerPrescale;
    Settings.SARetireTimeout     = Settings_p->SARetireTimeout;
        ...
        const SecY_Device_Params_t * const Device_p)
{
    ...
    if (Device_p->SARetireConf_p)
    {
        Rc = EIP164_Device_SA_Retire_Write(&PRV_SHARED_MACSEC_SECY_GLOBAL_VAR_GET(SecY_Device)[DeviceId]->IOArea,
                                           Device_p->SARetireConf_p->TimerPrescale,
                                           Device_p->SARetireConf_p->SATimeout);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to configure SA Retire EIP-164 device for device %d "
                 "(%s), error %d\n",
                 __func__,
                 DeviceId,
                     ADAPTER_EIP164_DEVICE_NAME(DeviceId),
        ...
        SecY_Device_Params_t * const Control_p)
{
    ...
    if (Control_p->SARetireConf_p)
    {
        Rc = EIP164_Device_SA_Retire_Read(&PRV_SHARED_MACSEC_SECY_GLOBAL_VAR_GET(SecY_Device)[DeviceId]->IOArea,
                                           &Control_p->SARetireConf_p->TimerPrescale,
                                           &Control_p->SARetireConf_p->SATimeout);
        if (Rc != EIP164_NO_ERROR)
        {
            LOG_CRIT("%s: Failed to read SA Retire EIP-164 device for device %d "
                     "(%s), error %d\n",
                     __func__,
                     DeviceId,
                     ADAPTER_EIP164_DEVICE_NAME(DeviceId),
...
SecY_Channel_PacketsInflight_Get(
        ...
        SecY_Ch_Mask_t * const Mask_p)
{
    EIP164_Error_t Rc;
    EIP164_Ch_Mask_t Inflight;
    ...
    {
        *fInflight_p =
            (Inflight.ch_bitmask[ChannelId/32] & (1<<(ChannelId%32))) != 0;
    }
    ...
    if (Mask_p != NULL)
    {
        ...
        for (i = 0 ; i < SECY_CHANNEL_WORDS; i++)
            Mask_p->ch_bitmask[i] = Inflight.ch_bitmask[i];
    }
...
/*-----------------------------------------------------------------------------
 * SecY_Channel_Config_Update
 */
SecY_Status_t
SecY_Channel_Config_Update(
        const unsigned int DeviceId,
        const unsigned int ChannelId,
        const SecY_Channel_t * const ChannelConfig_p)
{
    EIP164_Error_t Rc;
    EIP164_Channel_Rule_SecTAG_t Dev_SecTag_Rules;
    EIP164_PN_Threshold_t PN_Threshold_Cfg;
    ...
    /* Skip MACsec-specific configuration for non-MACsec bypass channel */
    if (!ChannelConfig_p->fLowLatencyBypass)
    {
       ZEROINIT(PN_Threshold_Cfg);

       PN_Threshold_Cfg.SeqNrThreshold       = ChannelConfig_p->StatCtrl.SeqNrThreshold;
       PN_Threshold_Cfg.SeqNrThreshold64.low =
           ChannelConfig_p->StatCtrl.SeqNrThreshold64.Lo;
       PN_Threshold_Cfg.SeqNrThreshold64.hi  =
           ChannelConfig_p->StatCtrl.SeqNrThreshold64.Hi;
      ...
      Dev_SecTag_Rules.EtherType       = ChannelConfig_p->RuleSecTAG.EtherType;
      Dev_SecTag_Rules.fCheckCE        = ChannelConfig_p->RuleSecTAG.fCheckCE;
      Dev_SecTag_Rules.fCheckKay       = ChannelConfig_p->RuleSecTAG.fCheckKay;
      Dev_SecTag_Rules.fCheckPN        = ChannelConfig_p->RuleSecTAG.fCheckPN;
      Dev_SecTag_Rules.fCheckSC        = ChannelConfig_p->RuleSecTAG.fCheckSC;
      Dev_SecTag_Rules.fCheckSL        = ChannelConfig_p->RuleSecTAG.fCheckSL;
      Dev_SecTag_Rules.fCheckV         = ChannelConfig_p->RuleSecTAG.fCheckV;
      Dev_SecTag_Rules.fCheckSLExt     = false;
      Dev_SecTag_Rules.fCompEType      = ChannelConfig_p->RuleSecTAG.fCompEType;
