        ...
        const bool fSARetire)
{
    ...
    if (TrueIOArea_p->fRetireEnabled && fSARetire)
    {
        rc = EIP164_RETIRE_CTRL_WR(Device,
                                   EIP164_RETIRE_CTRL_OP_WRITEDEF,
                                   1,
                                   SAIndex);
        if (rc) return rc;
    }
        ...
        const unsigned int SecY_Index)
{
    ...
    rc = EIP164_COUNTER_64_DEFAULT_WR(Device,
                                 EIP164_REG_SECY_STATISTICS(SecY_Index),
                                 EIP164_SECY_STAT_E_OUT_PKTS_SA_NOT_IN_USE);
    if (rc) return rc;
        ...
        const unsigned int SecY_Index)
{
    ...
    rc = EIP164_COUNTER_64_DEFAULT_WR(Device,
                                 EIP164_REG_SECY_STATISTICS(SecY_Index),
                                 EIP164_SECY_STAT_I_IN_PKTS_SA_NOT_IN_USE);
    if (rc) return rc;
        ...
        const bool fClear)
{
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SECY_STATISTICS(SecY_Index),
                              EIP164_SECY_STAT_E_OUT_PKTS_SA_NOT_IN_USE,
                              &Stats_p->OutPktsSANotInUse.low,
                              &Stats_p->OutPktsSANotInUse.hi,
                              fClear);
    if (rc) return rc;
        ...
        const bool fClear)
{
    ...
    rc = EIP164Lib_Counter_Read(Device,
                              EIP164_REG_SECY_STATISTICS(SecY_Index),
                              EIP164_SECY_STAT_I_IN_PKTS_SA_NOT_IN_USE,
                              &Stats_p->InPktsSANotInUse.low,
                              &Stats_p->InPktsSANotInUse.hi,
                              fClear);
    if (rc) return rc;
