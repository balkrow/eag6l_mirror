...
#include <cpss/dxCh/dxChxGen/macSec/private/prvCpssDxChMacSecDriver.h>
...
/**
* @internal prvCpssMacSecPreemptiveChannelGet function
* @endinternal
*
* @brief   Get the preemptive status and preemptive channel number
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
*
...
* @param[in] expChannelNum             - MACSec express channel number
* @param[out] portInPreemptiveModePtr  - (pointer to) port preemption status.
* @param[out] prechannelNumPtr         - (pointer to) MACSec preemption channel number.
*
* @retval GT_OK                        - on success
* @retval GT_FAIL                      - operation failed
*/
static GT_STATUS prvCpssMacSecPreemptiveChannelGet
(
    IN  GT_U8                                   devNum,
    IN  GT_PHYSICAL_PORT_NUM                    portNum,
    IN  GT_U32                                  expChannelNum,
    OUT GT_BOOL                                *portInPreemptiveModePtr,
    OUT GT_U32                                 *preChannelNumPtr
)
{
    GT_STATUS    rc;

    /* Check preemption enable status on the physical port */
    rc = prvCpssDxChTxqSip6_10PreemptionEnableGet(devNum, portNum, 0/*speedInMb*/, portInPreemptiveModePtr);
    if(rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Fail in prvCpssDxChTxqSip6_10PreemptionEnableGet for port %d", portNum);
    }

    if(*portInPreemptiveModePtr != GT_TRUE)
    {
        return GT_OK;
    }
    return prvCpssDxChTxqSip6_10PreChannelGet(devNum,0, expChannelNum, preChannelNumPtr);
}
...
)
{
    ...
    GT_U32                   macSecPreemptiveChannelNum = 0;
    GT_BOOL                  portInPreemptiveMode;
    ...
    if(portCfgPtr->exceptionCfgEnable)
    {
        ...
        unitExceptionsParams.ECCDropAction      = CFYE_DROP_INTERNAL;
        ...
        {
            unitExceptionsParams.fShouldSecure      = (portCfgPtr->policyMode == CPSS_DXCH_MACSEC_POLICY_MODE_MUST_SECURE_E)?0:1;
        }
        else
        {
            unitExceptionsParams.fShouldSecure = 0;
        }
    ...
    rc = prvCpssMacSecPreemptiveChannelGet(devNum, portNum, macSecChannelNum,
            &portInPreemptiveMode, &macSecPreemptiveChannelNum);
    if(rc != GT_OK)
    {
        return rc;
    }

    if(portInPreemptiveMode == GT_TRUE)
    {
        /* Call Classifier update API to apply these parameters */
        rc = CfyE_Device_Update(macSecUnitId, macSecPreemptiveChannelNum, &unitTopParams);
    }
    return rc;
}
...
*
*          note:
*          exceptionCfgEnable flag is not valid. It is ignored by
*          function and all exception parameters are read in any case
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
*
...
)
{
    ...
    if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        portCfgPtr->policyMode        = (unitTopParams.Control_p->Exceptions_p->fShouldSecure == 0)?
                                        CPSS_DXCH_MACSEC_POLICY_MODE_MUST_SECURE_E :
                                        CPSS_DXCH_MACSEC_POLICY_MODE_SHOULD_SECURE_E;
    }
...
)
{
    ...
    rc = CfyE_vPort_Add(macSecUnitId, &cfyeVportHandle, &vPortParams, CYFE_MODE_MACSEC);
...
)
{
    ...
    GT_U32                channelNum;
    GT_U32                macSecPreemptiveChannelNum;
    GT_U32                dpId;
    GT_U8                 byteArr[4];
    GT_U32                i;
    GT_BOOL               portInPreemptiveMode;
    ...
    if(ruleParamsPtr->mask.portNum !=0 )
    {
        ...
        rc = prvCpssMacSecPreemptiveChannelGet(devNum, ruleParamsPtr->key.portNum,
                channelNum,
                &portInPreemptiveMode,
                &macSecPreemptiveChannelNum);
        if(rc != GT_OK)
        {
            return rc;
        }
        ...
        {
            /* Set channel ID */
            switch(ruleParamsPtr->preemptionClass)
            {
                case CPSS_PORT_PREEMPTION_CLASS_PREEMPTIVE_E:
                    ruleParams.Key.ChannelID  = macSecPreemptiveChannelNum;
                    break;
                case CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E:
                    ruleParams.Key.ChannelID  = channelNum;
                    break;
                case CPSS_PORT_PREEMPTION_CLASS_BOTH_E:
                    /* In case of CPSS_PORT_PREEMPTION_CLASS_BOTH_E - Need to verify preemptive channel number variess by 1 bit */
                    tempData = (channelNum ^ macSecPreemptiveChannelNum);
                    if(1 == prvCpssPpConfigBitmapNumBitsGet(tempData))
                    {
                        /* Reset Common bit from mask and pattern */
                        ruleParams.Mask.ChannelID  &= ~tempData;
                        ruleParams.Key.ChannelID   &= ruleParams.Mask.ChannelID;
                    }
                    else
                    {
        ...
        /* In case preemption disabled, set the express channel data */
        else
        {
            ruleParams.Key.ChannelID  = channelNum;
        }
...
/**
* @internal internal_cpssDxChMacSecClassifyRuleAddIndex function
* @endinternal
*
* @brief   Adds a new rule for matching a packet to a vPort identified by vPortHandle
*          for one MACSec classifier using the given TCAM index.
*
*          note: The API cpssDxChMacSecClassifyRuleAdd() allocates TCAM entries by itself
*                and when this API is called, the given index may not be available.
*                Either allocate all rules with cpssDxChMacSecClassifyRuleAdd() or allocate
*                all rules with cpssDxChMacSecClassifyRuleAddIndex()
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
*
...
* @param[in]  vPortHandle         - vPort handle for the vPort where the packet matching rule must be added.
* @param[in]  ruleParamsPtr       - (pointer to) data for the rule.
* @param[in]  ruleIndex           - TCAM index where the rule must be added. This entry must be free when API is called.
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
static GT_STATUS internal_cpssDxChMacSecClassifyRuleAddIndex
(
    IN  GT_U8                                   devNum,
    IN  GT_MACSEC_UNIT_BMP                      unitBmp,
    IN  CPSS_DXCH_MACSEC_DIRECTION_ENT          direction,
...
)
{
    ...
    /* Call Classifier API to add vPort policy */
    rc = CfyE_Rule_Add_Index(macSecUnitId, (CfyE_vPortHandle_t)vPortHandle, &cfyeRuleHandle, &ruleParams, ruleIndex);
...
/**
* @internal cpssDxChMacSecClassifyRuleAddIndex function
* @endinternal
*
* @brief   Adds a new rule for matching a packet to a vPort identified by vPortHandle
*          for one MACSec classifier using the given TCAM index.
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
GT_STATUS cpssDxChMacSecClassifyRuleAddIndex
(
    IN  GT_U8                                   devNum,
    IN  GT_MACSEC_UNIT_BMP                      unitBmp,
    IN  CPSS_DXCH_MACSEC_DIRECTION_ENT          direction,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChMacSecClassifyRuleAddIndex);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, unitBmp, direction, vPortHandle, ruleParamsPtr, ruleIndex, ruleHandlePtr));

    rc = internal_cpssDxChMacSecClassifyRuleAddIndex(devNum, unitBmp, direction, vPortHandle, ruleParamsPtr,ruleIndex, ruleHandlePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, unitBmp, direction, vPortHandle, ruleParamsPtr,ruleIndex, ruleHandlePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
         ...
         (saParamsPtr->actionType != CPSS_DXCH_MACSEC_SECY_SA_ACTION_DROP_E) )
    {
        ...
        PRV_DXCH_MACSEC_CNV_CPSS_TO_SAB_DIRECTION_MAC(direction, trRecParams.direction);
    ...
    PRV_DXCH_MACSEC_CNV_CPSS_TO_SECY_SA_ACTION_MAC(saParamsPtr->actionType, saParams.ActionType);
    PRV_DXCH_MACSEC_CNV_CPSS_TO_SECY_PORT_MAC(saParamsPtr->destPort, saParams.DestPort);
    ...
    switch(saParamsPtr->actionType)
    {
        ...
        case CPSS_DXCH_MACSEC_SECY_SA_ACTION_INGRESS_E:
            ...
            PRV_DXCH_MACSEC_CNV_CPSS_TO_SECY_FRAME_VALID_MAC(
                saParamsPtr->params.ingress.validateFramesTagged,
...
* @param[in]  syncEnable          - If true, ensure that all packets available in this MACSec unit at the time
*                                   of the API call are processed before the statistics are read
* @param[in]  preemptionClass     - preemption class for the port
*                                    NOTE - Valid only if premption is enabled on the port.
...
(
    ...
    IN   CPSS_DXCH_MACSEC_DIRECTION_ENT                 direction,
    IN   GT_BOOL                                        syncEnable,
    IN   CPSS_PORT_PREEMPTION_CLASS_ENT                 preemptionClass,
...
)
{
    ...
    GT_U32                    macSecChannelNum[2]; /* 0-Express, 1-Preemptive */
    GT_BOOL                   portInPreemptiveMode;
    GT_U32                    channelLoop = 0;     /* Array offset for macSecChannelNum */
    GT_U32                    channelCount;
    ...
    rc = prvCpssMacSecPreemptiveChannelGet(devNum, portNum, macSecChannelNum[0],
            &portInPreemptiveMode, &macSecChannelNum[1]);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    {
        /* Else - preemptive is disabled - Only express channel data will be copied */
        channelCount = 1;
        channelLoop  = 0;
    }

    /* Clear structure before filling it */
    cpssOsMemSet(portStatPtr, 0, sizeof(CPSS_DXCH_MACSEC_CLASSIFY_STATISTICS_PORT_STC));

    for(; channelLoop<channelCount; channelLoop++)
    {
        /* Clear structure before filling it */
        cpssOsMemSet(&channelCounter, 0, sizeof(channelCounter));

        /* Call Classifier API to get TCAM hits statistics */
        rc = CfyE_Statistics_Channel_Get(macSecUnitId, macSecChannelNum[channelLoop], &channelCounter, syncEnable);

        /* Update output: set channel counters */
        /* TODO - Is there an util API for 64 bit addition to handle overflow ?? */
...
*                                    NOTE - Valid only if premption is enabled on the port.
* @param[out] portStatPtr         - (pointer to) Classifier statistics counter identified by portNum.
*                                    NOTE: Gets statistics of both(Express & premptive) channel,
*                                          if preemption is enabled and preemptionClass is CPSS_DXCH_PORT_PREEMPTION_BOTH_E
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PTR               - one of the parameters is NULL pointer
* @retval GT_BAD_PARAM             - on wrong parameter
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, direction, syncEnable, preemptionClass, portStatPtr));

    rc = internal_cpssDxChMacSecPortClassifyStatisticsGet(devNum, portNum, direction, syncEnable, preemptionClass, portStatPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, direction, syncEnable, preemptionClass, portStatPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if (direction == CPSS_DXCH_MACSEC_DIRECTION_EGRESS_E)
    {
        ...
        statSecyPtr->egress.outPktsSANotInUse.l[0] = secyEgressCounters.OutPktsSANotInUse.Lo;
        statSecyPtr->egress.outPktsSANotInUse.l[1] = secyEgressCounters.OutPktsSANotInUse.Hi;
    }
    else
    {
        /* Clear structure before filling it */
        ...
        statSecyPtr->ingress.inPktsSANotInUse.l[0] = secyIngressCounters.InPktsSANotInUse.Lo;
        statSecyPtr->ingress.inPktsSANotInUse.l[1] = secyIngressCounters.InPktsSANotInUse.Hi;
    }
...
)
{
    ...
    /* Set transform record parameters */
    trRecParams.direction     = SAB_DIRECTION_EGRESS;
    ...
    saParams.TransformRecord_p = trRecBufferPtr;
    saParams.SA_WordCount      = saWordCount;
    saParams.ActionType        = SECY_SA_ACTION_EGRESS;
...
/**
* @internal internal_cpssDxChMacSecSecySaSwitch function
* @endinternal
*
* @brief   Manually switch to the new SA from the current active SA
*
*   NOTEs: - This API should be used on the Egress direction only
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
...
*                                   - for non multi data paths device this parameter is IGNORED.
* @param[in] activeSaHandle       - SA handle of the current active SA
* @param[in] newSaHandle          - SA handle of the SA to be switched to
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameter
* @retval GT_FAIL                  - operation failed
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
static GT_STATUS internal_cpssDxChMacSecSecySaSwitch
(
    IN  GT_U8                               devNum,
    IN  GT_MACSEC_UNIT_BMP                  unitBmp,
    IN  CPSS_DXCH_MACSEC_SECY_SA_HANDLE     activeSaHandle,
    IN  CPSS_DXCH_MACSEC_SECY_SA_HANDLE     newSaHandle
)
{
    GT_STATUS          rc;                 /* return code status */
    GT_U32             macSecUnitId;       /* MACSec unit ID     */
    SecY_SAHandle_t    newSecySaHandle;    /* new SA handle      */
    SecY_SAHandle_t    activeSecySaHandle; /* active SA handle   */
    SecY_SA_t          saParams;           /* new SA parameters  */
    ...
    /* Set active & new SA handles */
    activeSecySaHandle.p = (void*)activeSaHandle;
    newSecySaHandle.p = (void*)newSaHandle;

    /* Switch to the second SA */
    rc = SecY_SA_Switch(macSecUnitId, activeSecySaHandle, newSecySaHandle, &saParams);

    return rc;
}

/**
* @internal cpssDxChMacSecSecySaSwitch function
* @endinternal
*
* @brief   Manually switch to the new SA from the current active SA
*
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
GT_STATUS cpssDxChMacSecSecySaSwitch
(
    IN  GT_U8                               devNum,
    IN  GT_MACSEC_UNIT_BMP                  unitBmp,
    IN  CPSS_DXCH_MACSEC_SECY_SA_HANDLE     activeSaHandle,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChMacSecSecySaSwitch);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, unitBmp, activeSaHandle, newSaHandle));

    rc = internal_cpssDxChMacSecSecySaSwitch(devNum, unitBmp, activeSaHandle, newSaHandle);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, unitBmp, activeSaHandle, newSaHandle));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        channelParams.fShouldSecure            = (portCfgPtr->policyMode == CPSS_DXCH_MACSEC_POLICY_MODE_MUST_SECURE_E)?0:1;
    }
    else
    {
        channelParams.fShouldSecure = 0;
    }
    ...
    if(portInPreemptiveMode == GT_TRUE)
    {
        channelParams.ChannelId = macSecPreemptiveChannelNum;
        /* Call Transformer update API to apply these parameters */
        rc = SecY_Device_Update(macSecUnitId, &unitTopParams);
    }
    return rc;
...
)
{
    ...
    if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        portCfgPtr->policyMode                   = (unitTopParams.ChConf_p->Params.Channel_p[macSecChannelNum].fShouldSecure == 0)?
                                                    CPSS_DXCH_MACSEC_POLICY_MODE_MUST_SECURE_E:CPSS_DXCH_MACSEC_POLICY_MODE_SHOULD_SECURE_E;
    }
...
)
{
    ...
    for (i=0;i<CPSS_DXCH_MACSEC_CLASSIFY_MAC_DA_ET_MATCH_RULES_COUNT_CNS;i++)
    {
        ...
        unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaEtRules[i].macDaMask.secondaryMask)<<(i);
        ...
        unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaEtRules[i].etherTypeMask.secondaryMask)<<(i+maskEnableEtStartLocation);
        ...
        unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaEtRules[i].etherTypeMatchMode.secondaryMatchMode)<<(maskModeOffset);
    ...
    for (i=0;i<CPSS_DXCH_MACSEC_CLASSIFY_MAC_DA_ET_RANGE_MATCH_RULES_COUNT_CNS;i++)
    {
        ...
        unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaEtRange[i].macDaRangeEtherTypeMask.secondaryMask)<<(maskEnableOffset);
        ...
        unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaEtRange[i].etherTypeMatchMode.secondaryMatchMode)<<(maskModeOffset);
    ...
    unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDaRange.macRangeMask.secondaryMask)<<(maskEnableOffset);
    ...
    unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDa44BitsConstMask.secondaryMask)<<(maskEnableOffset);
    ...
    unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macDa48BitsConstMask.secondaryMask)<<(maskEnableOffset);
    ...
    unitControlPacketDetectionParams.CPMatchSubMask    |= (ctrlPktDetectPtr->macSecKayPktMask.secondaryKay)<<(maskEnableOffset);
...
)
{
    ...
    for (i=0;i<CPSS_DXCH_MACSEC_CLASSIFY_MAC_DA_ET_MATCH_RULES_COUNT_CNS;i++)
    {
        ...
        ctrlPktDetectPtr->macDaEtRules[i].macDaMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(i)) & BIT_0;
        ...
        ctrlPktDetectPtr->macDaEtRules[i].etherTypeMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(i+maskEnableEtStartLocation)) & BIT_0;
        ...
        ctrlPktDetectPtr->macDaEtRules[i].etherTypeMatchMode.secondaryMatchMode = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskModeOffset)) & BIT_0;
    ...
    for (i=0;i<CPSS_DXCH_MACSEC_CLASSIFY_MAC_DA_ET_RANGE_MATCH_RULES_COUNT_CNS;i++)
    {
        ...
        ctrlPktDetectPtr->macDaEtRange[i].macDaRangeEtherTypeMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskEnableOffset)) & BIT_0;
        ...
        ctrlPktDetectPtr->macDaEtRange[i].etherTypeMatchMode.secondaryMatchMode = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskModeOffset)) & BIT_0;
    ...
    ctrlPktDetectPtr->macDaRange.macRangeMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskEnableOffset)) & BIT_0;
    ...
    ctrlPktDetectPtr->macDa44BitsConstMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskEnableOffset)) & BIT_0;
    ...
    ctrlPktDetectPtr->macDa48BitsConstMask.secondaryMask = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskEnableOffset)) & BIT_0;
    ...
    ctrlPktDetectPtr->macSecKayPktMask.secondaryKay = ((unitTopParams.CP_p->CPMatchSubMask)>>(maskEnableOffset)) & BIT_0;
...
)
{
    ...
    /* Update SA structure with SA parameters */
    PRV_DXCH_MACSEC_CNV_SECY_SA_ACTION_TO_CPSS_MAC(saParams.ActionType, saParamsPtr->actionType);
    PRV_DXCH_MACSEC_CNV_SECY_PORT_TO_CPSS_MAC(saParams.DestPort, saParamsPtr->destPort);
    ...
    switch(saParamsPtr->actionType)
    {
        ...
        case CPSS_DXCH_MACSEC_SECY_SA_ACTION_INGRESS_E:
            ...
            PRV_DXCH_MACSEC_CNV_SECY_FRAME_VALID_TO_CPSS_MAC(
                saParams.Params.Ingress.ValidateFramesTagged,
                saParamsPtr->params.ingress.validateFramesTagged);
            /* Get SCI */
...
)
{
    ...
    {
        /* Provide a diagnostics dump of all global device configuration and status */
        return SecY_Diag_Channel_Dump(macSecUnitId, macSecChannelNum, allPorts);
    }
    ...
    if(portInPreemptiveMode == GT_TRUE)
    {
        ...
        {
            case CPSS_PORT_PREEMPTION_CLASS_PREEMPTIVE_E:
                return SecY_Diag_Channel_Dump(macSecUnitId, macSecPreemptiveChannelNum, allPorts);
            case CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E:
                return SecY_Diag_Channel_Dump(macSecUnitId, macSecChannelNum, allPorts);
            case CPSS_PORT_PREEMPTION_CLASS_BOTH_E:
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, unitBmp, direction, portNum, preemptionClass, allPorts));

    rc = internal_cpssDxChMacSecPortSecyDump(devNum, unitBmp, direction, portNum, preemptionClass, allPorts);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, unitBmp, direction, portNum, preemptionClass, allPorts));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    {
        /* Provide a diagnostics dump of all global device configuration and status */
        return CfyE_Diag_Channel_Dump(macSecUnitId, macSecChannelNum, allPorts);
    }
    ...
    if(portInPreemptiveMode == GT_TRUE)
    {
        ...
        {
            case CPSS_PORT_PREEMPTION_CLASS_PREEMPTIVE_E:
                return CfyE_Diag_Channel_Dump(macSecUnitId, macSecPreemptiveChannelNum, allPorts);
            case CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E:
                return CfyE_Diag_Channel_Dump(macSecUnitId, macSecChannelNum, allPorts);
            case CPSS_PORT_PREEMPTION_CLASS_BOTH_E:
...
)
{
    ...
    rc = internal_cpssDxChMacSecPortClassifyDump(devNum, unitBmp, direction, portNum, preemptionClass, allPorts);
...
/**
* @internal internal_cpssDxChMacSecSecyStatisticsSaClear function
* @endinternal
*
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
static GT_STATUS internal_cpssDxChMacSecSecyStatisticsSaClear
(
    IN   GT_U8                              devNum,
    IN   GT_MACSEC_UNIT_BMP                 unitBmp,
    IN   CPSS_DXCH_MACSEC_DIRECTION_ENT     direction,
...
)
{
    ...
    return prvCpssDxChMacSecStatisticsSaClear(devNum, unitBmp, direction, saHandle);
}

/**
* @internal cpssDxChMacSecSecyStatisticsSaClear function
* @endinternal
*
* @brief   Reset SA statistics.
*
...
* @retval GT_NOT_INITIALIZED       - device was not initialized
*/
GT_STATUS cpssDxChMacSecSecyStatisticsSaClear
(
    IN   GT_U8                              devNum,
    IN   GT_MACSEC_UNIT_BMP                 unitBmp,
    IN   CPSS_DXCH_MACSEC_DIRECTION_ENT     direction,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChMacSecSecyStatisticsSaClear);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, unitBmp, direction, saHandle));

    rc = internal_cpssDxChMacSecSecyStatisticsSaClear(devNum, unitBmp, direction, saHandle);
