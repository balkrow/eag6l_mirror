        ...
        bool * const fEnabled_p)
{
    ...
    EIP163_ChannelMode_t ChannelMode = EIP163_CHANNEL_MODE_MACSEC;
...

    {
        ...
        Rule_p->Key.PacketType = (GT_U32)DeviceRule.Key.PacketType;
        ...
        Rule_p->Mask.PacketType = (GT_U32)DeviceRule.Mask.PacketType;
        ...
        Rule_p->Policy.fControlPacketSub = DeviceRule.Policy.fControlPacketSub;
        ...
        const bool fAllChannels)
{
    ...
    else
    {
        ...
        Log_FormattedMessage(
            ...
            "DefaultVPort=%u fShouldSecure=%d\n"
            "fForceDrop=%d  Dropaction=%s ECCDropAction=%s\n",
            DevCtrl.fLowLatencyBypass,
            DevExceptions.fDefaultVPortValid,
            DevExceptions.DefaultVPort,
            DevExceptions.fShouldSecure,
            ...
            DevExceptions.ECCDropAction==CFYE_DROP_CRC_ERROR?"CRC ERR":
            (DevExceptions.ECCDropAction==CFYE_DROP_PKT_ERROR?"PKT ERR":
             (DevExceptions.ECCDropAction==CFYE_DROP_INTERNAL?"INTERNAL":"NONE")));
        Log_FormattedMessage(
            "CtrlPkt  CPMatchModeMask=0x%x  CPMatchEnableMask 0x%x CPMatchSubMask=0x%x\n",
            CP.CPMatchModeMask, CP.CPMatchEnableMask, CP.CPMatchSubMask);
        for (i=0; i<16; i++)
        {
...
CfyELib_PktType_To_Name(
        CfyE_Rule_PacketType_t t)
{
    switch(t)
    {
    case CFYE_RULE_PKT_TYPE_OTHER:
        return "OTHER";
    default:
        return "Unknown";
    }
        ...
        const bool fAllRules)
{
    ...
    else
    {
        ...
        Log_FormattedMessage("Rule enabled=%d Policy vPortId=%d"
                             ...
                             Rule.Policy.fControlPacketSub);
        Log_FormattedMessage(
            "Packet type=%s (mask=0x%x)"
            " NumTags=0x%02x (nask=0x%02x ChannelID=%d (mask=0x%02x\n",
            CfyELib_PktType_To_Name(Rule.Key.PacketType),
