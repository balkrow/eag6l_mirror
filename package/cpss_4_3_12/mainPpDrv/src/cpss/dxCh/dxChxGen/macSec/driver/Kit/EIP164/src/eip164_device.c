        ...
        const EIP164_Settings_t * const Settings_p)
{
    ...
    rc = EIP164_DBG_DROP_CTRL_WR(Device,
                                 Settings_p->VPInvDropType,
                                 Settings_p->FLInvDropType,
                                 Settings_p->fBypassControlPkts);
    if (rc) return rc;

    rc = EIP164_TIME_PRESCALE_VAL_WR(Device, Settings_p->SARetireTimerPrescale);
    if (rc) return rc;

    rc = EIP164_RETIRE_ENABLE_WR(Device, Settings_p->SARetireTimeout,
                                     Settings_p->SARetireTimerPrescale != 0);
    if (rc) return rc;

    TrueIOArea_p->fRetireEnabled = Settings_p->SARetireTimerPrescale != 0;
        ...
        const EIP164_Device_Mode_t Mode)
{
    ...
    if (TrueIOArea_p->HWConf == EIP164_HW_CONF_E)
    {
        TrueIOArea_p->fRetireEnabled = false;
    }
    else
    {
        unsigned int Prescale = 0;
        rc = EIP164_TIME_PRESCALE_VAL_RD(Device, &Prescale);
        if (rc) return rc;
        TrueIOArea_p->fRetireEnabled = Prescale != 0;
    }
    ...
    const unsigned int Timeout)
{
    ...
    Device = TrueIOArea_p->Device;
    rc = EIP164_TIME_PRESCALE_VAL_WR(Device, Prescale);
    if (rc) return rc;

    rc = EIP164_RETIRE_ENABLE_WR(Device, Timeout, Prescale != 0);
    if (rc) return rc;
...
/*----------------------------------------------------------------------------
 * EIP164_Device_SA_Retire_Read
 */
EIP164_Error_t
EIP164_Device_SA_Retire_Read(
    const EIP164_IOArea_t * const IOArea_p,
    unsigned int * const Prescale_p,
    unsigned int * const Timeout_p)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
    int rc = 0;
    ...
    EIP164_CHECK_POINTER(IOArea_p);
    EIP164_CHECK_POINTER(Prescale_p);
    EIP164_CHECK_POINTER(Timeout_p);

    Device = TrueIOArea_p->Device;

    rc = EIP164_TIME_PRESCALE_VAL_RD(Device, Prescale_p);
    if (rc) return rc;

    rc = EIP164_RETIRE_ENABLE_RD(Device, Timeout_p, &fEnable);
    if (rc) return rc;
...
EIP164_Device_Channel_PacketsInFlight_Read(
        const EIP164_IOArea_t * const IOArea_p,
        EIP164_Ch_Mask_t * const InFlight_p)
{
    Device_Handle_t Device;
    const volatile EIP164_True_IOArea_t * const TrueIOArea_p = IOAREA_IN(IOArea_p);
    int rc = 0;
    ...
    /* Read channel activity from all in flight registers available in
       hardware*/
    for (i = 0; i < (TrueIOArea_p->MaxChCount+31)/32; i++)
    {
        rc = EIP164_CHANNEL_IN_FLIGHT_RD(Device, i*32,
                                         &InFlight_p->ch_bitmask[i]);
        if (rc) return rc;
    }
    /* Fill the remaining ones with zero */
    for (; i < EIP164_CHANNEL_WORDS; i++)
    {
        InFlight_p->ch_bitmask[i] = 0;
    }
        ...
        const bool fAlwaysOn)
{
    ...
    rc = EIP164_FORCE_CLOCK_ON_WR(Device,
                                  fAlwaysOn ? (ClockMask & MASK_16_BITS) : 0);
    if (rc) return rc;

    rc = EIP66_FORCE_CLOCK_ON_WR(Device,
                            fAlwaysOn ? ((ClockMask >> 16) & MASK_16_BITS) : 0);
    if (rc) return rc;
        ...
        const uint32_t ClockMask)
{
    ...
    rc = EIP164_FORCE_CLOCK_OFF_WR(Device, ClockMask & MASK_16_BITS);
    if (rc) return rc;

    rc = EIP66_FORCE_CLOCK_OFF_WR(Device, (ClockMask >> 16) & MASK_16_BITS);
    if (rc) return rc;
