...
static const uint8_t SABuilder_DefaultSM4SysParam[] =
{
    0xA3, 0xB1, 0xBA, 0xC6, 0x56, 0xAA, 0x33, 0x50,
    0x67, 0x7D, 0x91, 0x97, 0xB2, 0x70, 0x22, 0xDC,
};
        ...
        SABuilder_AESCallback_t AES_CB)
{
    ...
    case SAB_OP_MACSEC:
        ...
        else /* SAB_DIRECTION_INGRESS */
        {
            ...
            if ((SAParams_p->flags & SAB_MACSEC_DISABLE_ICV_CHECK) != 0)
                SABuffer_p[0] |= SAB_CW0_NO_ICV_CHECK;
        }
    ...
    if (SAParams_p->flags & SAB_MACSEC_FLAG_SM4)
    {
        SABuffer_p[0] |= SAB_CW0_SM4;
    }
    else
    {
    ...
    SABuilderLib_CopyKeyMat(SABuffer_p, SAOffsets.KeyOffs, SAParams_p->Key_p,
                            SAParams_p->KeyByteCount);
    if (SAParams_p->flags & SAB_MACSEC_FLAG_SM4 && SAParams_p->KeyByteCount == 16)
    {
        /* Write the default SM4 system parameter to the SA */
        SABuilderLib_CopyKeyMat(SABuffer_p, SAOffsets.KeyOffs + 4,
                                SABuilder_DefaultSM4SysParam, 16);
