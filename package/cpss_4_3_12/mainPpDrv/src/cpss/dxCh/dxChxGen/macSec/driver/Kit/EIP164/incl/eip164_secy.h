...
#define EIP164_SECY_DEVICE_PE_SM4_KS_CLOCK  BIT_21  /* SM4 Key schedule */
#define EIP164_SECY_DEVICE_PE_SM4_CLOCK     BIT_22  /* SM4 core */
...
typedef struct
{
    ...
    EIP164_UI64_t InPktsSANotInUse;
...
typedef struct
{
    ...
    /* true - Enables basic crypt-authenticate mode */
    bool fFlowCryptAuth;
} EIP164_SecY_SA_t;
 ...
 *
 * fLowLatencyBypass_p (output)
 *      Indication if the low latency bypass must be enabled (true)
 *      or the MACsec mode (false).
 *
 * fShouldSecure_p (output)
 *      Indication if should-secure  must be enabled by default (true)
 *      or must-secure (false)
 *
 ...
 *
 * This function writes the SA retire settings of an EIP-164 device.
 *
 * IOArea_p (input/output)
 *      Pointer to the Device instance I/O area in memory that identifies the
 *      EIP-164 device to be used.
 ...
 *
 * Prescale (input)
 *      Prescale value for SA retire timer. Set to 0 to disable the feature.
 *
 * Timeout (input)
 *      Number of timer ticks without packet traffic before SA is retired.
 *
 * Return value
 *      EIP164_NO_ERROR : Operation is completed.
 *      EIP164_ARGUMENT_ERROR : Passed wrong argument.
 ...
 *
 * This function reads the SA retire settings of an EIP-164 device.
 *
 * IOArea_p (input)
 *      Pointer to the Device instance I/O area in memory that identifies the
 *      EIP-164 device to be used.
 ...
 *
 * fSARetire (input)
 *      Enable the retire timeout fir this SA.
 *
 * This function is NOT re-entrant for the same Device.
 * This function is re-entrant for different Devices.
 *
