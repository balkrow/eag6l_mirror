...
#define PRV_CPSS_UNUSED_MAC(x) (void)x
...
static inline int
EIP164_DBG_DROP_CTRL_WR(
        Device_Handle_t Device,
        uint8_t const VPInvDropAction,
        uint8_t const FLInvDropAction,
        bool const fBypassControlPkt)
{
    uint32_t RegVal = 0;

    RegVal |= (VPInvDropAction & MASK_2_BITS);
    RegVal |= (FLInvDropAction & MASK_2_BITS) << 2;

    if (fBypassControlPkt)
        RegVal |= BIT_4;

    return EIP164_Write32(Device, EIP164_REG_DBG_DROP_CTRL, RegVal);
}


static inline int
EIP164_DBG_DROP_CTRL_RD(
        Device_Handle_t Device,
        uint8_t * const VPInvDropAction,
        uint8_t * const FLInvDropAction,
        bool * const fBypassControlPkt)
{
    uint32_t RegVal;
    int rc = EIP164_Read32(Device, EIP164_REG_DBG_DROP_CTRL, &RegVal);

    if (!rc)
    {
        *VPInvDropAction = RegVal & MASK_2_BITS;
        *FLInvDropAction = (RegVal >> 2) & MASK_2_BITS;
        *fBypassControlPkt = (RegVal & BIT_4) != 0;
    }
...
static inline int
EIP164_TIME_PRESCALE_VAL_WR(
        Device_Handle_t Device,
        const unsigned int Prescale)
{
    PRV_CPSS_UNUSED_MAC(Device);
    PRV_CPSS_UNUSED_MAC(Prescale);
#if 0 /* change done by CPSS. Register does not exist in HW. */
    return EIP164_Write32(Device, EIP164_REG_TIME_PRESCALE_VAL, Prescale);
#endif
    return 0;
}
...
static inline int
EIP164_TIME_PRESCALE_VAL_RD(
        Device_Handle_t Device,
        unsigned int * const Prescale)
{
    PRV_CPSS_UNUSED_MAC(Device);
    PRV_CPSS_UNUSED_MAC(Prescale);
#if 0 /* change done by CPSS. Register does not exist in HW. */
    uint32_t RegVal = 0;
    int rc = EIP164_Read32(Device, EIP164_REG_TIME_PRESCALE_VAL, &RegVal);
    if (!rc)
    {
        *Prescale = RegVal;
    }
...
static inline int
EIP164_RETIRE_ENABLE_WR(
        Device_Handle_t Device,
        const unsigned int Timeout,
        const bool fEnable)
{
    PRV_CPSS_UNUSED_MAC(Device);
    PRV_CPSS_UNUSED_MAC(Timeout);
    PRV_CPSS_UNUSED_MAC(fEnable);
#if 0 /* change done by CPSS. Register does not exist in HW. */
    uint32_t RegVal = Timeout & MASK_12_BITS;
    if (fEnable)
        RegVal |= BIT_31;

    return EIP164_Write32(Device, EIP164_REG_RETIRE_ENABLE, RegVal);
#endif
    return 0;
}

static inline int
EIP164_RETIRE_ENABLE_RD(
        Device_Handle_t Device,
...
#if 0 /* change done by CPSS. Register does not exist in HW. */
    uint32_t RegVal = 0;
    int rc = EIP164_Read32(Device, EIP164_REG_RETIRE_ENABLE, &RegVal);
    if (!rc)
    {
        *Timeout = RegVal & MASK_12_BITS;
        *fEnable = (RegVal & BIT_31) != 0;
    }
    return rc;
#endif
    return 0;
...
{
    PRV_CPSS_UNUSED_MAC(Device);
    PRV_CPSS_UNUSED_MAC(Opcode);
    PRV_CPSS_UNUSED_MAC(Value);
    PRV_CPSS_UNUSED_MAC(Index);
#if 0 /* change done by CPSS. Register does not exist in HW. */
    uint32_t RegVal = BIT_31;
    RegVal |= Index & MASK_12_BITS;
    RegVal |= (Value & MASK_11_BITS) << 16;
    RegVal |= (Opcode & MASK_2_BITS) << 28;

    return EIP164_Write32(Device, EIP164_REG_RETIRE_CTRL, RegVal);
#endif
    return 0;
}
        ...
        bool * const fShouldSecure_p)
{
    ...
    if (!rc)
    {
        ...
        *BurstLimit_p     = 0;
        *fShouldSecure_p  = ((RegVal & BIT_2) != 0);
