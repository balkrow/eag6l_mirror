...
)
{
    ...
    /* initialize cuckoo DB callbacks */
    ...
    {
        /* delete cuckoo DB */
        (void)deleteFunc(exactMatchManagerId);
    }
    return rc;
}
...
)
{
    ...
    for(ii = 0 ; ii < numOfPairs ; ii++)
    {
        ...
        if (GT_TRUE == PRV_CPSS_DXCH_ERRATA_GET_MAC(devNum, PRV_CPSS_DXCH_FALCON_EM_RETURNS_WRONG_ACTION_E))
        {
            if (exactMatchManagerPtr->actionMatchWaEnable == GT_FALSE)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                                              "FALCON device requires actionMatchWaEnable == GT_TRUE\n");
            }
        }
        else
        {
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                                              "device requires actionMatchWaEnable == GT_FALSE\n");
            }
        }
...
)
{
    ...
    {
        if(logicFormatPtr->redirect.redirectCmd == CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E)
        {
...
)
{
    ...
    if(((entryPtr->exactMatchActionType==CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E)&&
       (entryPtr->exactMatchAction.pclAction.egressPolicy != CPSS_PCL_DIRECTION_EGRESS_E))||
       ...
       (entryPtr->exactMatchAction.pclAction.egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)))
    {
         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of EPCL/PCL Action. "
                                                    "entryPtr->exactMatchActionType[%d] do not fit with, "
                                                    "entryPtr->exactMatchAction.pclAction.egressPolicy[%d] ",
                                       entryPtr->exactMatchActionType,
                                       entryPtr->exactMatchAction.pclAction.egressPolicy);
    }
...
)
{
    ...
    PRV_CPSS_DXCH_CUCKOO_DB_ENTRY_ADD_FUN entryAddFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbEntryAddFunc,PRV_CPSS_DXCH_CUCKOO_DB_ENTRY_ADD_FUN);
    PRV_CPSS_DXCH_CUCKOO_DB_SYNC_STATISTICS_FUN syncStatisticsFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbSyncStatisticsFunc,PRV_CPSS_DXCH_CUCKOO_DB_SYNC_STATISTICS_FUN);
    ...
    PRV_CPSS_DXCH_CUCKOO_DB_LOG_PTR_GET_FUN logPtrGetFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbLogPtrGetFunc,PRV_CPSS_DXCH_CUCKOO_DB_LOG_PTR_GET_FUN);
    PRV_CPSS_DXCH_CUCKOO_DB_LOG_UPDATE_STAGES_FUN logUpdateStagesFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbLogUpdateStagesFunc,PRV_CPSS_DXCH_CUCKOO_DB_LOG_UPDATE_STAGES_FUN);
    PRV_CPSS_DXCH_CUCKOO_DB_LOG_ARRAY_DELETE_FUN logArrayDeleteFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbLogArrayDeleteFunc,PRV_CPSS_DXCH_CUCKOO_DB_LOG_ARRAY_DELETE_FUN);
    ...
    entryInfo.line = 0;
    entryInfo.lookupNumber = entryPtr->exactMatchEntry.lookupNum;

    /* add 'dummy' entry to rule's conflicts DB in order to consider also new rule potential valid indexes when cuckoo logic relocate existing rules */
    rc = prvCpssDxChExactMatchManagerDbAddDummyEntryBeforeCuckoo(exactMatchManagerPtr,entryPtr);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* if no potentail valid indexes skip cuckoo */
    if (exactMatchManagerPtr->numPotentialHashResultsArr == 0)
    {
        if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
        {
            cpssOsPrintf("skipping cuckoo. number of potential hash results are '0'\n");
            EM_PRINT_KEY(entryPtr,sizeInBytes);
        }
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FULL,
                                      "the entry not exists and not able to add it (skipping 'Cuckoo') ");
    }
    ...
    if (rc == GT_OK)
    {
        /* delete 'dummy' entry from conflicts DB */
        rc = prvCpssDxChExactMatchManagerDbDeleteDummyEntryAfterCuckoo(exactMatchManagerPtr);
        if (rc != GT_OK)
        {
        ...
        rc = logUpdateStagesFunc(exactMatchManagerId);
        if (rc != GT_OK)
        {
            goto exit_cleanly_lbl;
        ...
        */
        for (i=0,leftOperations=numOfOperations; i<numOfOperations; i+=bankStep*2,leftOperations-=bankStep*2)
        {
            ...
            if (leftOperations == bankStep)
            {
                ...
                rc = entryAddFunc(exactMatchManagerId,cuckooDbLogPtr[i].bank,cuckooDbLogPtr[i].line,
                                                                      cuckooDbLogPtr[i].sizeInBytes,cuckooDbLogPtr[i].data,
                                                                      entryPtr->exactMatchEntry.lookupNum,GT_FALSE);
                if (rc != GT_OK)
                {
                    goto exit_cleanly_lbl;
                }
            ...
            else
            {
                ...
                rc = entryAddFunc(exactMatchManagerId,cuckooDbLogPtr[i].bank,cuckooDbLogPtr[i].line,
                                                                      cuckooDbLogPtr[i].sizeInBytes,cuckooDbLogPtr[i].data,
                                                                      currentEntryPtr->exactMatchEntry.lookupNum,GT_FALSE);
                if (rc != GT_OK)
                {
                    goto exit_cleanly_lbl;
                }
    ...
    /* delete 'dummy' entry from conflicts DB */
    prvCpssDxChExactMatchManagerDbDeleteDummyEntryAfterCuckoo(exactMatchManagerPtr);
...
exit_cleanly_lbl:
    ...
    /* delete cuckoo operation log */
    logArrayDeleteFunc(exactMatchManagerId);

    /* update cuckoo statistics counters according to exact match DB */
    syncStatisticsFunc(exactMatchManagerId);
...
)
{
    ...
    *addApiStatisticsPtr = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_ADD_API_STATISTICS___LAST___E;
    ...
    if(rc != GT_OK)
    {
        ...
        {
            /* the entry can't be added to EMM: entry was blocked in all hash indexes due to conflicts. */
            if (rc==GT_FULL)
            {
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("the entry not exists and not able to add it due to conflicts in all hash indexes (skipping 'Cuckoo') ");
                    EM_PRINT_KEY(entryPtr,sizeInBytes);
                }
                *addApiStatisticsPtr = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_ADD_API_STATISTICS_ERROR_TABLE_FULL_E;
            ...
            if (paramsPtr->rehashEnable)
            {
                ...
                if(rc != GT_OK)
                {
                    if (rc == GT_FULL)
                    {
                        if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                        {
                            cpssOsPrintf("the entry not exists and not able to add it (after 'Cuckoo')\n");
                            EM_PRINT_KEY(entryPtr,sizeInBytes);
                        }
                        *addApiStatisticsPtr = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_ADD_API_STATISTICS_ERROR_TABLE_FULL_E;
                    }
    ...
    *addApiStatisticsPtr = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_ADD_API_STATISTICS_ERROR_BAD_STATE_E;
    ...
    rc = entryAddFunc(exactMatchManagerId, calcInfo.selectedBankId,line,sizeInBytes,entryPtr->exactMatchEntry.key.pattern,entryPtr->exactMatchEntry.lookupNum,GT_TRUE);
    if (rc!=GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    switch(entryPtr->redirectCommand)
    {
        ...
        case CPSS_DXCH_TTI_ASSIGN_GENERIC_ACTION_E:
            entryPtr->genericAction=dbEntryPtr->redirectPtr;
            break;
        default:
            break;
    }
...
)
{
    ...
    switch(entryPtr->redirect.redirectCmd)
    {
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ROUTER_E:
            entryPtr->redirect.data.routerLttIndex = dbEntryPtr->redirectPtr;
            break;
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ROUTER_AND_ASSIGN_VRF_ID_E:
            entryPtr->redirect.data.routeAndVrfId.routerLttIndex = dbEntryPtr->redirectPtr;
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
            entryPtr->redirect.data.genericActionInfo.genericAction= dbEntryPtr->redirectPtr;
        default:
            break;
    }
    return GT_OK;
...
)
{
    ...
    entryPtr->egressPolicy = CPSS_PCL_DIRECTION_INGRESS_E;
...
)
{
    ...
    entryPtr->egressPolicy = CPSS_PCL_DIRECTION_EGRESS_E;
...
)
{
    ...
    PRV_CPSS_DXCH_CUCKOO_FUNC_PTR_STC *cuckooDbFuncPtr =&(PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.cuckooDir.cuckooSrc.cuckooDbFuncPtr[PRV_CPSS_DXCH_CUCKOO_CLIENTS_EXACT_MATCH_E]));
    PRV_CPSS_DXCH_CUCKOO_DB_ENTRY_DELETE_FUN entryDeleteFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbEntryDeleteFunc,PRV_CPSS_DXCH_CUCKOO_DB_ENTRY_DELETE_FUN);
    ...
    *deleteApiStatisticsPtr = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_DELETE_API_STATISTICS_ERROR_BAD_STATE_E;
    ...
    rc = entryDeleteFunc(exactMatchManagerId, calcInfo.selectedBankId,line,sizeInBytes,updateStatistics);
...
)
{
    GT_STATUS                                                           rc;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_DELETE_API_STATISTICS_ENT   deleteApiStatistics;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_CALC_ENTRY_STC            calcInfo,calcInfoFullHash;
    GT_U32                                                              bankId;
    GT_U32                                                              i=0;
    GT_U32                                                              sizeInBytes,line,exactMatchManagerId,ii;
    GT_BOOL                                                             updateStatistics = GT_TRUE;
       ...
       dbEntryPtr->hwExactMatchKeySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
        ...
        cpssOsMemCpy(&entry.exactMatchEntry.key.pattern,dbEntryPtr->pattern,sizeof(dbEntryPtr->pattern));
        entry.exactMatchEntry.key.keySize = dbEntryPtr->hwExactMatchKeySize;
        ...
        rc = prvCpssDxChExactMatchHashCrcMultiResultsByParamsCalcExt(exactMatchSize,
                                                                     exactMatchBanksNum,
                                                                     &entry.exactMatchEntry.key,
                                                                     PRV_CPSS_DXCH_EXACT_MATCH_HASH_DATA_SIZE_VALUE_CNS,
                                                                     &calcInfoFullHash.numberOfElemInCrcMultiHashArr,
                                                                     &calcInfoFullHash.crcMultiHashArr[0]);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    if(dbEntryPtr->hwExactMatchKeySize >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
       dbEntryPtr->hwExactMatchKeySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
        /* we need to delete entry from all potantial locations; not just selected one */
        calcInfo.numberOfElemInCrcMultiHashDeleteScanArr = calcInfoFullHash.numberOfElemInCrcMultiHashArr;
        for (i=0;i<calcInfoFullHash.numberOfElemInCrcMultiHashArr;i++)
        {
            calcInfo.crcMultiHashDeleteScanArr[i] = calcInfoFullHash.crcMultiHashArr[i];
        }
    }
...
)
{
   ...
   /* in HA we should not write to HW */
    rc = cpssSystemRecoveryStateGet(&tempSystemRecovery_Info);
    if (rc != GT_OK)
    {
        return rc;
       ...
       (managerHwWriteBlock==GT_TRUE))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED,
                    "cpssDxChExactMatchManagerEntryRewrite is not supported during HA, "
                    "since the function rewrites Exact Match Manager HW entries according to SW DB, "
                    "and during HA we should not do write to HW\n");
    }
...
/**
* @internal internal_cpssDxChExactMatchManagerProfileEntryUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager profile entry.
*        This API overrite the configuration done in the
*        Exact Match Manager Create.
*
*        NOTE:
*        Before using this API it is the user responsibility to make sure there
*        are no exact match rules configured on the specific profile that will
*        be effected from the change.
*        for example changing rule size setting is problematic.
*        it’s expected that user either flush the rules before or verify that none such exist
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
* @param[in] lookupNum             - the lookup num for which we
*                                    are setting the profileId
...
* @retval GT_NOT_INITIALIZED       - if the Exact Match Manager is not initialized
*/
static GT_STATUS internal_cpssDxChExactMatchManagerProfileEntryUpdate
(
    IN  GT_U32                                                  exactMatchManagerId,
...
)
{
    ...
    switch (lookupNum)
    {
        case CPSS_DXCH_EXACT_MATCH_LOOKUP_FIRST_E:
        case CPSS_DXCH_EXACT_MATCH_LOOKUP_SECOND_E:
            /* the profile is validated according to the client defined on the lookup number.
               need to verify we are trying to configure a lookup that is enabled for exact match */
            if(exactMatchManagerPtr->lookupInfo.lookupsArray[lookupNum].lookupEnable==GT_TRUE)
            {
                /* update the DB */
                rc = prvCpssDxChExactMatchManagerDbProfileEntryUpdate(exactMatchManagerPtr,
                                                                   exactMatchManagerPtr->lookupInfo.lookupsArray[lookupNum].lookupClient,
                                                                   profileId,
                ...
                /* update the HW
                   the DB is already updated in the correct profileId place in the DB
                   so no need to pass the profileEntryParamsPtr to the function */
                rc = prvCpssDxChExactMatchManagerHwProfileEntryUpdate(exactMatchManagerPtr,profileId);
                if (rc != GT_OK)
                {
                    return rc;
                }
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"the lookup [%d] is not enabled - can not configure a profile",lookupNum);
            }
...
/**
* @internal cpssDxChExactMatchManagerProfileEntryUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager profile entry.
*        This API overrite the configuration done in the
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchManagerProfileEntryUpdate);

    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, exactMatchManagerId,lookupNum,profileId,profileEntryParamsPtr));

    rc = internal_cpssDxChExactMatchManagerProfileEntryUpdate(exactMatchManagerId,lookupNum,profileId,profileEntryParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, exactMatchManagerId,lookupNum, profileId,profileEntryParamsPtr));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
...
/**
* @internal internal_cpssDxChExactMatchManagerProfileEntryGet function
* @endinternal
*
* @brief API for fetching current Exact Match Manager profile entry setting as they
*        were defined in Exact Match Manager Create or changed by cpssDxChExactMatchManagerProfileEntryUpdate.
*
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
...
* @retval GT_NOT_INITIALIZED       - if the Exact Match Manager is not initialized
*/
static GT_STATUS internal_cpssDxChExactMatchManagerProfileEntryGet
(
    IN  GT_U32                                                  exactMatchManagerId,
    IN  GT_U32                                                  profileId,
...
)
{
    ...
    return prvCpssDxChExactMatchManagerDbProfileEntryGet(exactMatchManagerId,profileId, profileEntryParamsPtr);
}

/**
* @internal cpssDxChExactMatchManagerProfileEntryGet function
* @endinternal
*
* @brief API for fetching current Exact Match Manager profile entry setting as they
*        were defined in Exact Match Manager Create or changed by cpssDxChExactMatchManagerProfileEntryUpdate.
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchManagerProfileEntryGet);

    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, exactMatchManagerId,profileId,profileEntryParamsPtr));

    rc = internal_cpssDxChExactMatchManagerProfileEntryGet(exactMatchManagerId,profileId,profileEntryParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, exactMatchManagerId,profileId,profileEntryParamsPtr));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
...
/**
* @internal internal_cpssDxChExactMatchManagerLookupUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager Lookup configuration.
*        This API overrite the configuration done in the
*        Exact Match Manager Create.
*
*        NOTE:
*        Before using this API it is the user responsibility to make sure there
*        are no exact match rules configured on the specific lookup that will
*        be effected from the change. It is expected that user
*        either flush the rules before or verify that none such
*        exist. Profiles configured on the changed lookup should
*        be updated to fit the new lookup configuration
*        Expanded actions configured on the updated lookup should be updated
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
*
*/
static GT_STATUS internal_cpssDxChExactMatchManagerLookupUpdate
(
    IN  GT_U32                                              exactMatchManagerId,
    IN  CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                    lookupNum,
...
)
{
    ...
    switch (lookupNum)
    {
        ...
        case CPSS_DXCH_EXACT_MATCH_LOOKUP_SECOND_E:
            /* update the DB */
            rc = prvCpssDxChExactMatchManagerDbLookupUpdate(exactMatchManagerPtr,
                                                            lookupNum,
            ...
            /* update the HW
               the DB is already updated in the correct lookup place in the DB
               so no need to pass the lookupEntryParamsPtr to the function */
            rc = prvCpssDxChExactMatchManagerHwLookupUpdate(exactMatchManagerPtr,lookupNum);
            if (rc != GT_OK)
            {
                return rc;
            }
            ...
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"the lookup [%d] is not valid",lookupNum);
    }
...
/**
* @internal cpssDxChExactMatchManagerLookupUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager Lookup configuration.
*        This API overrite the configuration done in the
...
* @param[in] lookupEntryParamsPtr  -(pointer to) parameters of
*       lookup entry
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchManagerLookupUpdate);

    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, exactMatchManagerId,lookupNum,lookupEntryParamsPtr));

    rc = internal_cpssDxChExactMatchManagerLookupUpdate(exactMatchManagerId,lookupNum,lookupEntryParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, exactMatchManagerId,lookupNum, lookupEntryParamsPtr));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
...
/**
* @internal internal_cpssDxChExactMatchManagerLookupGet function
* @endinternal
*
...
* @retval GT_NOT_INITIALIZED       - if the Exact Match Manager is not initialized
*/
static GT_STATUS internal_cpssDxChExactMatchManagerLookupGet
(
    IN  GT_U32                                              exactMatchManagerId,
    IN  CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                    lookupNum,
...
)
{
    ...
    return prvCpssDxChExactMatchManagerDbLookupGet(exactMatchManagerId,lookupNum, lookupEntryParamsPtr);
}

/**
* @internal cpssDxChExactMatchManagerLookupGet function
* @endinternal
*
* @brief API for fetching current Exact Match Manager lookup
*        configuration setting as they were defined in Exact Match
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChExactMatchManagerLookupGet);

    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_EXACT_MATCH_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, exactMatchManagerId,lookupNum,lookupEntryParamsPtr));

    rc = internal_cpssDxChExactMatchManagerLookupGet(exactMatchManagerId,lookupNum,lookupEntryParamsPtr);
