...
/* debug function to print the counters for the length of conflict rules per hwIndex
   no HW access

   exactMatchanagerId - Exact Match Manager instance ID
   hwIndex              - hw index
*/
GT_STATUS prvCpssDxChExactMatchManagerConflictRulesCountersPrint
(
...
)
{
    ...
    cpssOsPrintf("\n");
    cpssOsPrintf("hwIndex[%d]\n", hwIndex);
    cpssOsPrintf("19B Conflict Rules = [%d]\n", exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_19B);
    cpssOsPrintf("33B Conflict Rules = [%d]\n", exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_33B);
    cpssOsPrintf("47B Conflict Rules = [%d]\n", exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_47B);

    counter = exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_19B;
    nodePtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_19B;

    if (counter > 0)
    {
        printf("19B rules list:\n");
        while (counter > 0 && nodePtr->nextNodePointer != NULL)
        {
            ...
            for (n=0; n<19; n++)
            {
                cpssOsPrintf("[%x]",nodePtr->entryPointer->pattern[n]);
            }
            cpssOsPrintf("\n");
            cpssOsPrintf("hwIndex: [%d] lookup number: [%d] [%s]\n",nodePtr->entryPointer->hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum,entryInPool.isUsedEntry ? "V" : "P");
            counter--;
        }
    }

    counter = exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_33B;
    nodePtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_33B;

    if (counter > 0)
    {
        printf("33B rules list:\n");
        while (counter > 0 && nodePtr->nextNodePointer != NULL)
        {
            dbIndex  = exactMatchManagerPtr->indexArr[hwIndex].entryPointer;
            entryInPool = exactMatchManagerPtr->entryPoolPtr[dbIndex];
    ...
    counter = exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_47B;
    nodePtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_47B;

    if (counter > 0)
    {
        printf("47B rules list:\n");
        while (counter > 0 && nodePtr->nextNodePointer != NULL)
        {
            dbIndex  = exactMatchManagerPtr->indexArr[hwIndex].entryPointer;
            entryInPool = exactMatchManagerPtr->entryPoolPtr[dbIndex];
...
/* debug function to print the total counters for the length of conflict rules
   no HW access

   exactMatchanagerId - Exact Match Manager instance ID
*/
GT_STATUS prvCpssDxChExactMatchManagerConflictRulesTotalCountersPrint
(
    IN  GT_U32          exactMatchManagerId
)
{
    ...
    cpssOsPrintf("\n");
    cpssOsPrintf("19B Total Conflict Indexes [%d] = numOfRules*numOfHash [%d]*[%d]]\n", exactMatchManagerPtr->sumEntriesCounter_19B,
                  exactMatchManagerPtr->sumEntriesCounter_19B/(exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/2),
                  exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/2);
    cpssOsPrintf("33B Total Conflict Indexes [%d] = numOfRules*numOfHash [%d]*[%d]]\n", exactMatchManagerPtr->sumEntriesCounter_33B,
                  exactMatchManagerPtr->sumEntriesCounter_33B/(exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/3),
                  exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/3);
    cpssOsPrintf("47B Total Conflict Indexes [%d] = numOfRules*numOfHash [%d]*[%d]]\n", exactMatchManagerPtr->sumEntriesCounter_47B,
                  exactMatchManagerPtr->sumEntriesCounter_47B/(exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/4),
                  exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/4);
    return GT_OK;
}
