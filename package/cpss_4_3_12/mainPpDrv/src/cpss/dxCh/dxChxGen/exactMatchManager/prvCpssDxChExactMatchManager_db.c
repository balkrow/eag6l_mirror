...
#define EM_PRINT_PATTERN(pattern,keySize)                               \
{                                                                       \
    GT_U32 n;                                                           \
...
#define EM_SELF_TEST_BMP_SET_MAC(_exactMatchManagerId, _bitPos)   EM_BMP_SET_MAC(&_exactMatchManagerId->selfTestBitmap[_bitPos>>5], (_bitPos & 0x1F))
#define EM_SELF_TEST_BMP_CLR_MAC(_exactMatchManagerId, _bitPos)   EM_BMP_CLEAR_MAC(&_exactMatchManagerId->selfTestBitmap[_bitPos>>5], (_bitPos & 0x1F))
...
)
{
    ...
    switch(entryPtr->redirectCommand)
    {
        ...
        case CPSS_DXCH_TTI_ASSIGN_GENERIC_ACTION_E:
            dbEntryPtr->redirectPtr=entryPtr->genericAction;
            break;
        default:
            break;
    }
...
)
{
    ...
    switch(entryPtr->redirect.redirectCmd)
    {
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ROUTER_E:
            dbEntryPtr->redirectPtr = entryPtr->redirect.data.routerLttIndex;
            break;
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ROUTER_AND_ASSIGN_VRF_ID_E:
            dbEntryPtr->redirectPtr = entryPtr->redirect.data.routeAndVrfId.routerLttIndex;
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
            dbEntryPtr->redirectPtr = entryPtr->redirect.data.genericActionInfo.genericAction;
        default:
            break;
    }
...
)
{
    ...
    case CPSS_DXCH_TTI_REDIRECT_TO_ROUTER_LOOKUP_E:
        ...
        logicFormatPtr->vrfId=0;
        if(logicFormatPtr->routerLttPtr >= BIT_18)
        {
    ...
    case CPSS_DXCH_TTI_VRF_ID_ASSIGN_E:
        ...
        break;
    case CPSS_DXCH_TTI_ASSIGN_GENERIC_ACTION_E:
        if(logicFormatPtr->genericAction >= BIT_12)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM," illegal logicFormatPtr->genericAction[%d]", logicFormatPtr->genericAction);
        }
        break;
    default:
    ...
    /* the real value supported (12 bits or 16 bits) will be checked when device is added */
...
)
{
    ...
    if(logicFormatPtr->egressPolicy != CPSS_PCL_DIRECTION_EGRESS_E)
    {
        ...
        switch(logicFormatPtr->redirect.redirectCmd)
        {
            ...
            case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
                useIpclProfileIndexAndEnablers = GT_TRUE;
                if (logicFormatPtr->redirect.data.genericActionInfo.genericAction >= BIT_12)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"logicFormatPtr->redirect.data.genericActionInfo.genericAction[%d]",
                                              logicFormatPtr->redirect.data.genericActionInfo.genericAction);
                }
                break;
            default:
    ...
    if (expandedActionParamsPtr->exactMatchExpandedEntryValid==GT_TRUE)
    {
        ...
        switch(expandedActionParamsPtr->expandedActionType)
        {
            ...
            case CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E:
                /* check that exactMatchActionType fit to the egressPolicy configured in the action */
                if(((expandedActionParamsPtr->expandedActionType==CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E)&&
                    (expandedActionParamsPtr->expandedAction.pclAction.egressPolicy!=CPSS_PCL_DIRECTION_EGRESS_E))||
                   ((expandedActionParamsPtr->expandedActionType==CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E)&&
                    (expandedActionParamsPtr->expandedAction.pclAction.egressPolicy==CPSS_PCL_DIRECTION_EGRESS_E)))
                {
                     CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of EPCL/PCL Action. "
                                                                "expandedActionParamsPtr->expandedActionType[%d] do not fit with, "
                                                                "expandedActionParamsPtr->expandedAction.pclAction.egressPolicy[%d] ",
                                                   expandedActionParamsPtr->expandedActionType,
                                                   expandedActionParamsPtr->expandedAction.pclAction.egressPolicy);
                }
                    ...
                    if (lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionEn == GT_TRUE)
                    {
                        ...
                        switch(lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType)
                        {
                            ...
                            case CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E:
                                ...
                                {
                                    /* check that exactMatchActionType fit to the egressPolicy configured in the action */
                                    if((lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType==CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E)&&
                                       (lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy==CPSS_PCL_DIRECTION_EGRESS_E))
                                    {
                                         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of PCL Action. "
                                                                                    "lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType[%d] do not fit with, "
                                                                                    "lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy[%d] ",
                                                                       lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType,
                                                                       lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy);
                                    }
                            ...
                            case CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E:
                                ...
                                {
                                    /* check that exactMatchActionType fit to the egressPolicy configured in the action */
                                    if((lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType==CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E)&&
                                       (lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy!=CPSS_PCL_DIRECTION_EGRESS_E))
                                    {
                                         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of EPCL Action. "
                                                                                    "lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType[%d] do not fit with, "
                                                                                    "lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy[%d] ",
                                                                       lookupPtr->profileEntryParamsArray[profileIdVal].defaultActionType,
                                                                       lookupPtr->profileEntryParamsArray[profileIdVal].defaultAction.pclAction.egressPolicy);
...
* @param[in] exactMatchManagerId   - the Exact Match Manager id to associate with the newly created manager.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] actionMatchWaEnable   - whether action match WA is enable.
...
(
    IN GT_U32   exactMatchManagerId,
    IN GT_BOOL  actionMatchWaEnable
)
{
    GT_U32                                                  numBytesToAlloc,numOfHashNodes;
    ...
    GT_U32                                                  ii,iiMax;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *nodePtr;
    ...
    /* maximal 128K 5 bytes entries, is translated to maximal 64K 19 bytes rules with 8 hashes each */
    numOfHashNodes = (exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2)*(CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/2);
    numBytesToAlloc = numOfHashNodes * sizeof(PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC);
    EM_MEM_CALLOC_MAC(exactMatchManagerId,exactMatchManagerPtr->hashNodePoolPtr,numBytesToAlloc);

    numBytesToAlloc = exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes * sizeof(PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_LIST_STC);
    EM_MEM_CALLOC_MAC(exactMatchManagerId,exactMatchManagerPtr->hashIndexArr,numBytesToAlloc);

    for(ii = 0 ; ii < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; ii++)
    {
        exactMatchManagerPtr->hashIndexArr[ii].headOfEntriesListPointer_19B.nextNodePointer = 0; /* no entries that use this index at this point */
        exactMatchManagerPtr->hashIndexArr[ii].entriesCounter_19B = 0;

        exactMatchManagerPtr->hashIndexArr[ii].headOfEntriesListPointer_33B.nextNodePointer = 0; /* no entries that use this index at this point */
        exactMatchManagerPtr->hashIndexArr[ii].entriesCounter_33B = 0;

        exactMatchManagerPtr->hashIndexArr[ii].headOfEntriesListPointer_47B.nextNodePointer = 0; /* no entries that use this index at this point */
        exactMatchManagerPtr->hashIndexArr[ii].entriesCounter_47B = 0;
    }

    /* first node in the 'linked list' of 'free nodes list' */
    nodePtr = &exactMatchManagerPtr->hashNodePoolPtr[0];
    nodePtr->entryPointer = 0;
    nodePtr->nextNodePointer = 0;
    exactMatchManagerPtr->headOfFreeHashNodesList.entryPointer = 0;
    exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = nodePtr;

    /* init the 'linked list' of 'free nodes list'; Add to the top */
    for(ii = 1 ; ii < numOfHashNodes; ii++)
    {
        nodePtr = &exactMatchManagerPtr->hashNodePoolPtr[ii];
        nodePtr->entryPointer = 0;
        nodePtr->nextNodePointer = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
        exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = nodePtr;
    }

    exactMatchManagerPtr->totalFreeHashNodes = numOfHashNodes;
    ...
    exactMatchManagerPtr->actionMatchWaEnable = actionMatchWaEnable;
...
)
{
    ...
    *managerAllocatedPtr = GT_FALSE;
    ...
    /*************************/
    rc = initNewManager(exactMatchManagerId,capacityPtr->actionMatchWaEnable);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    EM_MEM_FREE_MAC(exactMatchManagerId,exactMatchManagerPtr->hashIndexArr);
    EM_MEM_FREE_MAC(exactMatchManagerId,exactMatchManagerPtr->hashNodePoolPtr);
...
GT_STATUS prvCpssDxChExactMatchManagerAllDbDelete()
{
    ...
    PRV_CPSS_DXCH_CUCKOO_DB_DELETE_FUN deleteFunc;
    ...
    for(exactMatchManagerId = 0; exactMatchManagerId < CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_INSTANCE_NUM_CNS; exactMatchManagerId++)
    {
        ...
        {
            deleteFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbDeleteFunc,PRV_CPSS_DXCH_CUCKOO_DB_DELETE_FUN);
            if(deleteFunc)
            {
                /* delete cuckoo DB */
                rc = deleteFunc(exactMatchManagerId);
                if(rc != GT_OK)
                {
                    return rc;
                }
...
)
{
    ...
    for(bankId = 0 ; bankId <= calcInfoPtr->lastBankIdChecked ; (bankId=bankId+calcInfoPtr->bankStep))
    {
        ...
        {
        case 1:
            if ((calcInfoPtr->usedBanksArr[bankId])||(calcInfoPtr->crcMultiHashIsForbiddenArr[bankId]))
            {
                 /* if one of the banks are used or forbidden due to potential conflicts , it can't be used for new entry */
                continue;
            }
            break;
        case 2:
            if ((calcInfoPtr->usedBanksArr[bankId])||(calcInfoPtr->usedBanksArr[bankId+1])||
                (calcInfoPtr->crcMultiHashIsForbiddenArr[bankId])||(calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+1]))
            {
                 /* if one of the banks are used or forbidden due to potential conflicts , it can't be used for new entry */
                continue;
            }
        ...
        case 3:
            if ((calcInfoPtr->usedBanksArr[bankId])||(calcInfoPtr->usedBanksArr[bankId+1])||
                ...
                (calcInfoPtr->crcMultiHashIsForbiddenArr[bankId])||(calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+1])||
                (calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+2]))
            {
                 /* if one of the banks are used or forbidden due to potential conflicts , it can't be used for new entry */
                continue;
            }
        ...
        case 4:
            if ((calcInfoPtr->usedBanksArr[bankId])||(calcInfoPtr->usedBanksArr[bankId+1])||
                ...
                (calcInfoPtr->crcMultiHashIsForbiddenArr[bankId])||(calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+1])||
                (calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+2])||(calcInfoPtr->crcMultiHashIsForbiddenArr[bankId+3]))
            {
                /* if one of the banks are used or forbidden due to potential conflicts , it can't be used for new entry */
                continue;
            }
    ...
    if(bestBankId == GT_NA)
    {
        ...
        {
            cpssOsPrintf("GT_BAD_STATE: bestBankId must be found; already checked that valid potential hash index exists\n");
            cpssOsPrintf("number of forbidden indexes %d\n",calcInfoPtr->numberOfElemInCrcMultiHashIsForbiddenArr);
        }
...
)
{
    ...
    GT_U32          maxNumOfValidBitsInBitmap=0;/* in the last bitmap not all bits must be valid */
    ...
    while (GT_TRUE)
    {
        ...
        /* if we are in the last bitmap element the it is possible that the last bit map will not be full
           for instance if we have 48 age bin elements in the age bin array
           we need 2 bitmaps to specify if the age bin is used.
           the first bitmap will use all 32 bits but the second bitmap will use only 16 bits
           so the pos should be from 0 to 15 */
        if (exactMatchManagerPtr->agingBinInfo.totalAgeBinAllocated%32!=0)
        {
            if (level2Index == (exactMatchManagerPtr->agingBinInfo.totalAgeBinAllocated / 32))
            {
                maxNumOfValidBitsInBitmap = exactMatchManagerPtr->agingBinInfo.totalAgeBinAllocated%32;
            }
            else
            {
                maxNumOfValidBitsInBitmap=32;
            }
        }
        else
        {
        ...
        for (pos = (*lastLocationLevel1HitPtr); pos < maxNumOfValidBitsInBitmap; pos++)
        {
            if((exactMatchManagerPtr->agingBinInfo.level1UsageBitmapPtr[level2Index] & (1 << pos)) == 0)
            {
                ...
                *ageBinIndex = (level2Index * 32) + level1Index;
                ...
                if((exactMatchManagerPtr->agingBinInfo.ageBinListPtr[*ageBinIndex].totalUsedEntriesCnt+1)<=
...
)
{
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *headOfEntriesListPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *nodePtr;
    GT_U16                                                  *entriesCounterPtr;
    GT_U32                                                  *sumEntriesCounterPtr;
    GT_U32                                                  dbIndex=0;
    ...
    {
        EM_CHECK_X_NOT_MORE_THAN_Y_RETURN_GT_FULL_MAC(
    ...
    for (step=0; step<calcInfoPtr->bankStep; step++)
    {
       ...
       /* totalPopulation is updated according to keySize
        ...
        if (step==0)
        {
            /* save only the index of the LSB rule */
            dbIndex = currFree_dbIndex;
        }
        ...
        /************************************/
        /* Get the Free age-bin Index - only one bin per entry */
        if (step==0)
        {
            rc = prvCpssDxChExactMatchManagerDbFreeAgeBinIndexGet(exactMatchManagerPtr, &lastLocationLevel1Hit, &lastLocationLevel2Hit, &ageBinIndex);
        ...
        if (step==0)
        {
            ...
            EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
                ...
                CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E);

            /* coherency check */
            EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
                calcInfoPtr->counterKeyType,
                CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E);
    ...
    if(calcInfoPtr->counterKeyType >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
       calcInfoPtr->counterKeyType <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
     ...
     for( bankId = 0 ;
        ...
        {
        /* DB elements should be added to all its possible hashes - regardless which one it uses currently
           (not only to selectedBankId) */
        ...
        switch (calcInfoPtr->counterKeyType)
        {
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
            headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_19B;
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_19B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_19B;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
            headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_33B;
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_33B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_33B;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
            headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_47B;
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_47B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_47B;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                          "keySize [%d] is not supported in DB exactMatchManagerPtr>hashIndexArr",dbEntryPtr->hwExactMatchKeySize);
        }

        /* coherency check -- can't be ZERO because we are going to decrement it */
        EM_CHECK_X_NOT_ZERO_MAC(exactMatchManagerPtr->totalFreeHashNodes);
        exactMatchManagerPtr->totalFreeHashNodes--;

        /* get node from head of 'free hash nodes list' */
        nodePtr = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;

        /**************************************/
        /* update the headOfFreeHashNodesList */
        /**************************************/

        exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = nodePtr->nextNodePointer;

        /* Add to top of the existing list */
        if(*entriesCounterPtr == 0)
        {
            nodePtr->nextNodePointer = 0;
        }
        else
        {
            nodePtr->nextNodePointer = headOfEntriesListPtr->nextNodePointer;
        }
        nodePtr->entryPointer = dbEntryPtr;
        headOfEntriesListPtr->nextNodePointer = nodePtr;
        ...
        /* coherency check */
        EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
            *sumEntriesCounterPtr,
            /* maximal 64K 19 bytes rules with 8 hashes each */
            exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/2);
...
/**
* @internal prvCpssDxChExactMatchManagerDbAddDummyEntryBeforeCuckoo function
* @endinternal
*
* @brief   This function addes 'dummy' entry to rule's conflicts DB in order to consider
*          also new rule potential valid indexes when cuckoo logic relocate existing rules.
*
*        NOTE: the function doing only DB operations (no HW operations)
*
* @param[in] exactMatchManagerPtr    - (pointer to) the Exact Match manager.
* @param[in] entryPtr                - (pointer to) the entry (application format)
*
* @retval GT_OK                      - on success: entry was added to all possible hash locations.
* @retval GT_BAD_STATE /GT_BAD_PARAM - non-consistency between different parts of
*                                      the manager.
*
*/
GT_STATUS   prvCpssDxChExactMatchManagerDbAddDummyEntryBeforeCuckoo
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC           *exactMatchManagerPtr,
...
)
{
    ...
    GT_U32                                                  ii,sizeInBytes,bankId,hwIndex;
    GT_U32                                                  exactMatchManagerId,counter=0;

    cpssOsMemSet(&calcInfo,0,sizeof(PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_CALC_ENTRY_STC));

    exactMatchManagerPtr->numPotentialHashResultsArr = 0xFFFF; /* indication as not relevant for non-errata */

    exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize = entryPtr->exactMatchEntry.key.keySize;

    /* consider also new entry potential indexes when cuckoo logic relocate existing entries */
    if(entryPtr->exactMatchEntry.key.keySize >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
       entryPtr->exactMatchEntry.key.keySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
        ...
        cpssOsMemCpy(&exactMatchManagerPtr->dummyEntry.pattern[0],&entryPtr->exactMatchEntry.key.pattern[0],sizeof(GT_U8)*sizeInBytes);
        exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize = entryPtr->exactMatchEntry.key.keySize;
        exactMatchManagerPtr->dummyEntry.hwExactMatchLookupNum = entryPtr->exactMatchEntry.lookupNum;

        /* calculate hash for Exact Match entry */
        rc = prvCpssDxChExactMatchManagerDbHashCalculate(exactMatchManagerPtr, entryPtr, &calcInfo);
        if(rc != GT_OK)
        {
            return rc;
        }

        /* save hash results in DB; avoid conflict hash indexes. */
        for( counter = 0, bankId = 0 ;
             (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<calcInfo.numberOfElemInCrcMultiHashArr);
             bankId += calcInfo.bankStep)
        {
            ...
            /* scan the relevant hwIndex list in DB and check if one of the rules has same size, lookup number and same LSB as the new entry. */
            rc = prvCpssDxChExactMatchManagerDbRuleConflictCheck(exactMatchManagerId,entryPtr->exactMatchEntry.key.pattern,entryPtr->exactMatchEntry.key.keySize,entryPtr->exactMatchEntry.lookupNum,hwIndex);
            if (rc == GT_FULL)
            {
                /* ACTUAL entry with conflict exists in this location, all the other potential locations must be blocked; skip cuckoo.*/
                counter = 0;
                break;
            }
            ...
            if (rc == GT_OK)
            {
                switch (calcInfo.bankStep)
                {
                case 2:
                    exactMatchManagerPtr->potentialHashResultsArr[bankId] = calcInfo.crcMultiHashArr[bankId];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+1] = calcInfo.crcMultiHashArr[bankId+1];
                    counter+=2;
                    break;
                case 3:
                    exactMatchManagerPtr->potentialHashResultsArr[bankId] = calcInfo.crcMultiHashArr[bankId];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+1] = calcInfo.crcMultiHashArr[bankId+1];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+2] = calcInfo.crcMultiHashArr[bankId+2];
                    counter+=3;
                    break;
                case 4:
                    exactMatchManagerPtr->potentialHashResultsArr[bankId] = calcInfo.crcMultiHashArr[bankId];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+1] = calcInfo.crcMultiHashArr[bankId+1];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+2] = calcInfo.crcMultiHashArr[bankId+2];
                    exactMatchManagerPtr->potentialHashResultsArr[bankId+3] = calcInfo.crcMultiHashArr[bankId+3];
                    counter+=4;
                    break;
                default:
                    CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(calcInfo.bankStep);
                }
            }
        }

        /* update number of potential hash indexes */
        exactMatchManagerPtr->numPotentialHashResultsArr = counter;

        /* if no potential hash locations for 'dummy' entry; the entry can't be added skip cuckoo. */
        if (counter == 0)
            return GT_OK;

        /* add entry as 'dummy' entry to hashIndexArr to all its possible hashes */
        dbEntryPtr  = &exactMatchManagerPtr->dummyEntry;

        for( bankId = 0 ;
             (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<exactMatchManagerPtr->numPotentialHashResultsArr);
             bankId += calcInfo.bankStep)
        {
            /* DB elements should be added to all its possible hashes */
            hwIndex = exactMatchManagerPtr->potentialHashResultsArr[bankId];

            /* protect access to array out of range */
            EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(hwIndex,exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes);

            switch (exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize)
            {
            case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
                headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_19B;
                entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_19B;
            ...
            default:
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                              "keySize [%d] is not supported in DB exactMatchManagerPtr>hashIndexArr",exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize);
            }
...
/**
* @internal prvCpssDxChExactMatchManagerDbDeleteDummyEntryAfterCuckoo function
* @endinternal
*
* @brief   This function deletes 'dummy' entry from rule's conflicts DB.
*
*        NOTE: the function doing only DB operations (no HW operations)
*
* @param[in] exactMatchManagerPtr    - (pointer to) the Exact Match manager.
*
* @retval GT_OK                      - on success: entry was deleted from all possible hash locations.
* @retval GT_BAD_STATE /GT_BAD_PARAM - non-consistency between different parts of
*                                      the manager.
*
*/
GT_STATUS   prvCpssDxChExactMatchManagerDbDeleteDummyEntryAfterCuckoo
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC           *exactMatchManagerPtr
)
{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *headOfEntriesListPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *prvNodePtr,*nextNodePtr;
    GT_U16                                                  *entriesCounterPtr;
    GT_U32                                                  *sumEntriesCounterPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC   *dbEntryPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_CALC_ENTRY_STC calcInfo;
    GT_U32                                                  bankId,hwIndex,bankStep;
    GT_U16                                                  entriesCounter=0;
    GT_BOOL                                                 isEqual=GT_TRUE;

    cpssOsMemSet(&calcInfo,0,sizeof(PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_CALC_ENTRY_STC));

    if (exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize == CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E)
        return GT_OK;

    /* delete 'dummy' entry potential indexes after cuckoo logic relocate existing entries */
    if(exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
       exactMatchManagerPtr->dummyEntry.hwExactMatchKeySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
        /* delete 'dummy' entry from hashIndexArr from all its possible hashes */
        dbEntryPtr  = &exactMatchManagerPtr->dummyEntry;
        ...
        for( bankId = 0 ;
             ...
             bankId += bankStep)
        {
            /* DB elements should be removed from all its possible hashes */
            hwIndex = exactMatchManagerPtr->potentialHashResultsArr[bankId];
            ...
            entriesCounter = *entriesCounterPtr;
            if (entriesCounter == 0)
            {
                /* no entries in hash DB with the same hash index, key size and same 5 Bytes --> continue to next hah index.*/
                continue;
            }

            prvNodePtr = headOfEntriesListPtr->nextNodePointer;
            /* check if the pointers are the same */
            isEqual = (&(dbEntryPtr->pattern[0]) == &(prvNodePtr->entryPointer->pattern[0])) ? GT_TRUE : GT_FALSE;
            if (isEqual==GT_TRUE)
            {
                /* found entry in head of hash list with the same hash index, key size and same pattern --> delete the entry from head.*/
                headOfEntriesListPtr->nextNodePointer = prvNodePtr->nextNodePointer;

                EM_CHECK_X_NOT_ZERO_MAC(*entriesCounterPtr);
                (*entriesCounterPtr)--;

                EM_CHECK_X_NOT_ZERO_MAC(*sumEntriesCounterPtr);
                (*sumEntriesCounterPtr)--;


                /* return node to head of 'free hash nodes list' */
                prvNodePtr->nextNodePointer = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
                exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = prvNodePtr;
                prvNodePtr->entryPointer = 0;

                /* coherency check */
                EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(exactMatchManagerPtr->totalFreeHashNodes,
                                                        (exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2)*(CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/2));
            ...
            while(entriesCounter >= 2)
            {
                nextNodePtr = prvNodePtr->nextNodePointer;
                /* check if the pointers are the same */
                isEqual = (&(dbEntryPtr->pattern[0]) == &(nextNodePtr->entryPointer->pattern[0])) ? GT_TRUE : GT_FALSE;
                if (isEqual==GT_TRUE)
                {
                    /* found entry in the middle hash list with the same hash index, key size and same pattern --> delete the entry from list.*/
                    prvNodePtr->nextNodePointer = nextNodePtr->nextNodePointer;
                    (*entriesCounterPtr)--;
                    ...
                    /* return node to head of 'free hash nodes list' */
                    nextNodePtr->nextNodePointer = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
                    exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = nextNodePtr;
                    nextNodePtr->entryPointer = 0;

                    /* coherency check */
                    EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(exactMatchManagerPtr->totalFreeHashNodes+1,
                                                            (exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2)*(CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/2));
                ...
                entriesCounter--;
                if (nextNodePtr->nextNodePointer == NULL)
                {
                    /* no more entries in the list; didn't delete the entry */
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
                }
                prvNodePtr = nextNodePtr;
            }
        }
    }
...
)
{
    ...
    GT_U16                                                  *entriesCounterPtr;
    GT_U16                                                  entriesCounter=0;
    GT_U32                                                  *sumEntriesCounterPtr,fullNum;
    GT_BOOL                                                 isEqual=GT_TRUE;
    ...
    if(dbEntryPtr->hwExactMatchKeySize >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
       dbEntryPtr->hwExactMatchKeySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {

     /* we are in delete scan; need to delete rule from all potential locations */
     fullNum = calcInfoPtr->numberOfElemInCrcMultiHashArr == 0 ? calcInfoPtr->numberOfElemInCrcMultiHashDeleteScanArr : calcInfoPtr->numberOfElemInCrcMultiHashArr;

     for( bankId = 0 ;
         (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<fullNum);
         bankId += calcInfoPtr->bankStep)
        {
            /* DB elements should be removed from all its possible hashes - regardless which one it uses currently
               (not only from selectedBankId) */

            hwIndex = calcInfoPtr->numberOfElemInCrcMultiHashArr == 0 ? calcInfoPtr->crcMultiHashDeleteScanArr[bankId] : calcInfoPtr->crcMultiHashArr[bankId];
    ...
    for (step=0; step<calcInfoPtr->bankStep; step++)
    {
        ...
        if (step==0)/* one age bin for an entry */
        {
            /**********************************/
...
)
{
    ...
    rc = prvCpssDxChExactMatchHashCrcMultiResultsByParamsCalcExt(exactMatchSize,
                                                                 ...
                                                                 PRV_CPSS_DXCH_EXACT_MATCH_HASH_DATA_SIZE_VALUE_CNS,
...
)
{
    ...
    *isMatchPtr = GT_FALSE;
    ...
    isEqual = (cpssOsMemCmp(&(entryPtr->exactMatchEntry.key.pattern[0]),&(dbEntryPtr->pattern[0]), sizeof(GT_U8)*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS)==0) ? GT_TRUE : GT_FALSE;
    if (isEqual==GT_FALSE)
    {
         /* the entry type in ExactMatch manager is not the same ... so check next index */
        return GT_OK;
...
/**
* @internal prvCpssDxChExactMatchManagerDbRuleConflictCheck function
* @endinternal
*
* @brief The function scan the relevant hwIndex list in DB and check if one of the
*        rules has same size, LSB and lookup number as the new/relocated entry.
*
* @param[in] exactMatchManagerId   - the Exact Match Manager id.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] patternPtr            - (pointer to) 5 LSB of entry's pattern
* @param[in] keySize               - Exact match key size
* @param[in] lookupNumber          - Exact match lookup number
* @param[in] hwIndex               - index into Exact match 'index array' DB.
*
* @retval GT_OK                    - on success; index can be used.
* @retval GT_FAIL                  - on error: index is blocked and cannot be used.
* @retval GT_FULL                  - the table is FULL : entry exists in potential index.
*                                                        all indexes are blocked.
*/
GT_STATUS prvCpssDxChExactMatchManagerDbRuleConflictCheck
(
    IN GT_U32                                                       exactMatchManagerId,
    IN GT_U8                                                        *patternPtr,
    IN CPSS_DXCH_EXACT_MATCH_KEY_SIZE_ENT                           keySize,
    IN CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                             lookupNumber,
    IN GT_U32                                                       hwIndex
)
{
    GT_U16 counter;
    ...
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                      "keySize [%d] is not supported in DB exactMatchManagerPtr>hashIndexArr",keySize);
    }

    counter = *entriesCounterPtr;
    nodePtr = headOfEntriesListPtr->nextNodePointer;
    if (counter == 0 || nodePtr == NULL)
    {
        /* no entries in hash DB with the same hash index, key size and same 5 Bytes --> entry can be added.*/
        return GT_OK;
    }

    /* check first node */
    isEqual = (cpssOsMemCmp(&(patternPtr[0]),&(nodePtr->entryPointer->pattern[0]), sizeof(GT_U8)*CPSS_DXCH_EXACT_MATCH_ACTION_MATCH_CMP_SIZE_CNS/*5B*/)==0) ? GT_TRUE : GT_FALSE;
    if (isEqual==GT_TRUE)
    {
        if (lookupNumber == nodePtr->entryPointer->hwExactMatchLookupNum)
        {
            /* found entry in hash DB with the same hash index, key size, lookup number and same 5 Bytes:
               if its me --> it is ok. (allowing the cuckoo to move this entry).
               otherwise --> entry is blocked. need to check other hash indexes. */
            isEqual = (cpssOsMemCmp(&(patternPtr[0]),&(nodePtr->entryPointer->pattern[0]), sizeof(GT_U8)*numOfBytes)==0) ? GT_TRUE : GT_FALSE;
            if (isEqual == GT_TRUE)
            {
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("OK, this is me. hwIndex %d lookupNumber %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                    EM_PRINT_PATTERN(nodePtr->entryPointer->pattern,numOfBytes);
                }
            }
            else
            {
                /* if entry with conflict exists in this location, all the other potential locations must be blocked. */
                if (exactMatchManagerPtr->indexArr[hwIndex].isValid_entryPointer == GT_TRUE)
                {
                    if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                    {
                        cpssOsPrintf("GT_FULL, found valid rule in hwIndex %d lookupNumber %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                        EM_PRINT_PATTERN(nodePtr->entryPointer->pattern,numOfBytes);
                    }
                ...
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("GT_FAIL, found conflict in hwIndex %d lookupNumber %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                    EM_PRINT_PATTERN(nodePtr->entryPointer->pattern,numOfBytes);
                }
    ...
    while (nodePtr != NULL && counter > 0)
    {
        ...
        if (isEqual==GT_TRUE)
        {
            ...
            {
                /* found entry in hash DB with the same hash index, key size, lookup number and same 5 Bytes:
                   if its me --> return ok. (allowing the cuckoo to move this entry).
                   otherwise --> entry is blocked. need to check other hash indexes. */
                isEqual = (cpssOsMemCmp(&(patternPtr[0]),&(nodePtr->entryPointer->pattern[0]), sizeof(GT_U8)*numOfBytes)==0) ? GT_TRUE : GT_FALSE;
                if (isEqual == GT_TRUE)
                {
...
/**
* @internal prvCpssDxChExactMatchManagerDbRuleConflictValidityCheck function
* @endinternal
*
* @brief The function scan the relevant hwIndex list in DB and check if one of the
*        rules has conflicts.
...
*                                                        all indexes are blocked.
*/
GT_STATUS prvCpssDxChExactMatchManagerDbRuleConflictValidityCheck
(
    IN GT_U32                                                       exactMatchManagerId,
    IN GT_U8                                                        *patternPtr,
    IN CPSS_DXCH_EXACT_MATCH_KEY_SIZE_ENT                           keySize,
...
)
{
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC           *nodePtr;
    GT_U16                                                       *entriesCounterPtr;
    GT_U32                                                       numOfBytes,dbIndex;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC        entryInPool;
    ...
    if (isEqual==GT_TRUE)
    {
        if (lookupNumber == nodePtr->entryPointer->hwExactMatchLookupNum)
        {
            /* found entry in hash DB with the same hash index, lookup number, key size and same 5 Bytes:
               if its me --> its ok, continue to next.
               otherwise --> check if ACTUAL entry conflicts. */
            dbIndex  = exactMatchManagerPtr->indexArr[hwIndex].entryPointer;
            entryInPool = exactMatchManagerPtr->entryPoolPtr[dbIndex];
            isEqual = (cpssOsMemCmp(&(patternPtr[0]),&(nodePtr->entryPointer->pattern[0]), sizeof(GT_U8)*numOfBytes)==0) ? GT_TRUE : GT_FALSE;
            if (isEqual == GT_TRUE && entryInPool.hwIndex == nodePtr->entryPointer->hwIndex)
            {
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("OK, this is me. hwIndex %d lookup number %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                    EM_PRINT_PATTERN(nodePtr->entryPointer->pattern,numOfBytes);
                }
            }
            else
            {
                /* if entry with conflict exists in this location, return GT_FULL */
                if (exactMatchManagerPtr->indexArr[hwIndex].isValid_entryPointer == GT_TRUE)
                {
                    /* found ACTUAL entry in hash DB with the same hash index, key size, lookup number and same 5 Bytes in common */
                    isEqual = (cpssOsMemCmp(&(patternPtr[0]),&(entryInPool.pattern[0]), sizeof(GT_U8)*CPSS_DXCH_EXACT_MATCH_ACTION_MATCH_CMP_SIZE_CNS /*5B*/)==0) ? GT_TRUE : GT_FALSE;
                    if (isEqual == GT_TRUE && entryInPool.hwIndex != nodePtr->entryPointer->hwIndex
                                           && entryInPool.hwExactMatchLookupNum == nodePtr->entryPointer->hwExactMatchLookupNum
                                           && entryInPool.hwExactMatchKeySize == nodePtr->entryPointer->hwExactMatchKeySize)
                    {
                        if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                        {
                            cpssOsPrintf("GT_FULL, found valid rule in hwIndex %d lookup number %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                            EM_PRINT_PATTERN(entryInPool.pattern,numOfBytes);
                        }
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FULL, LOG_ERROR_NO_MSG);
                    }
                }
                ...
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("GT_FAIL, found conflict in hwIndex %d lookup number %d\n",hwIndex,nodePtr->entryPointer->hwExactMatchLookupNum);
                    EM_PRINT_PATTERN(nodePtr->entryPointer->pattern,numOfBytes);
                }
            }
        }
...
/**
* @internal prvCpssDxChExactMatchManagerDbCalcForbiddenHashArray function
* @endinternal
*
* @brief For every index in crcMultiHashArr, this function scan the relevant hwIndex list in DB and
*        check for conflicts: one of the rules has same size and same LSB as the new/relocated entry.
*
* @param[in] exactMatchManagerId   - the Exact Match Manager id.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] verifyValidConflictEntry - check also if ACTUAL conflicting rule exists
* @param[in] patternPtr            - (pointer to) 5 LSB of entry's pattern
* @param[in] keySize               - Exact match key size
* @param[in] lookupNumber          - Exact match lookup number
* @param[in] bankStep              - the step between banks depending on the key size
* @param[in] numberOfElemInCrcMultiHashArr - number of valid elements in the crcMultiHashArr
* @param[in] crcMultiHashArr       - the hash results for the key of the entry
*
* @param[out] numberOfElemInCrcMultiHashIsForbiddenArrPtr
*                                  - (pointer to) number of valid elements in the crcMultiHashIsForbiddenArr
* @param[out] crcMultiHashIsForbiddenArr - the hash results that have conflict with other
*                                    rules that exists in the database
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
* @retval GT_BAD_PTR               - on NULL pointer.
*/
GT_STATUS prvCpssDxChExactMatchManagerDbCalcForbiddenHashArray
(
    IN GT_U32                                                   exactMatchManagerId,
    IN GT_BOOL                                                  verifyValidConflictEntry,
    IN GT_U8                                                    *patternPtr,
    IN CPSS_DXCH_EXACT_MATCH_KEY_SIZE_ENT                       keySize,
    IN CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                         lookupNumber,
    IN GT_U32                                                   bankStep,
    IN GT_U32                                                   numberOfElemInCrcMultiHashArr,
    IN  GT_U32                                                  crcMultiHashArr[],
    OUT GT_U32                                                  *numberOfElemInCrcMultiHashIsForbiddenArrPtr,
    OUT GT_U32                                                  crcMultiHashIsForbiddenArr[]
)
{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC       *exactMatchManagerPtr;
    GT_STATUS rc;
    GT_U32    bankId,hwIndex,forbiddenIndexCounter=0;

    /* get pointer to the Exact Match manager */
    exactMatchManagerPtr = EM_MANAGER_GET_MAC(exactMatchManagerId);

    if (keySize >= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E &&
        keySize <= CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E && exactMatchManagerPtr->actionMatchWaEnable)
    {
        for( bankId = 0 ;
             (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<numberOfElemInCrcMultiHashArr);
             bankId += bankStep )
        {
            hwIndex = crcMultiHashArr[bankId];

            /* protect access to array out of range */
            EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(hwIndex,exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes);

            /* scan the relevant hwIndex list in DB and check if one of the rules has same size and same LSB as the new entry. */
            rc = prvCpssDxChExactMatchManagerDbRuleConflictCheck(exactMatchManagerId,patternPtr,keySize,lookupNumber,hwIndex);
            if (rc == GT_FULL && verifyValidConflictEntry == GT_TRUE)
            {
                /* GT_FULL indicates that we found  ACTUAL conflicting rule in hwIndex.
                   if verifyValidConflictEntry == TRUE:  can't add this enrty; skip the cuckoo.
                   if verifyValidConflictEntry == FALSE: no need to check if the entry actually exists in this location.
                                                         So, if rc != GT_OK add only this index to forbidden array. */
            ...
            if (rc != GT_OK)
            {
                /*  the new entry conflict with other rule that exists in the database. this index is blocked and cannot be used.
                    EM Manager need to check the entry alternative locations. */
                switch (bankStep)
                {
                case 1:
                    /* mark this index as forbidden so it will not be choosen for the new entry in mostPopulatedBank. */
                    crcMultiHashIsForbiddenArr[bankId] = GT_TRUE;
                    break;
                case 2:
                    /* mark this index as forbidden so it will not be choosen for the new entry in mostPopulatedBank. */
                    crcMultiHashIsForbiddenArr[bankId] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+1] = GT_TRUE;
                    break;
                case 3:
                     /* mark this index as forbidden so it will not be choosen for the new entry in mostPopulatedBank. */
                    crcMultiHashIsForbiddenArr[bankId] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+1] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+2] = GT_TRUE;
                    break;
                case 4:
                    /* mark this index as used so it will not be choosen for the new entry in mostPopulatedBank. */
                    crcMultiHashIsForbiddenArr[bankId] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+1] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+2] = GT_TRUE;
                    crcMultiHashIsForbiddenArr[bankId+3] = GT_TRUE;
                    break;
                default:
                    CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(bankStep);
    ...
    *numberOfElemInCrcMultiHashIsForbiddenArrPtr = forbiddenIndexCounter;
...
)
{
    ...
    else
    {
        ...
        /* We found a potential location. For every index that is considered as potential location for the new entry
           or as alternative location for move entry (as part of the cuckoo) that index in the database is examined.
           scan the relevant list and check if one of them has same size, same LSB as the new entry */
        if (calcInfoPtr->calcEntryType == PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_FREE_E)
        {
            /* get Exact Match manager Id from Excat Match pointer */
            EM_MANAGER_ID_GET_MAC(exactMatchManagerPtr,exactMatchManagerId);
            ...
            cpssOsMemSet(calcInfoPtr->crcMultiHashIsForbiddenArr,0,sizeof(calcInfoPtr->crcMultiHashIsForbiddenArr));
            calcInfoPtr->numberOfElemInCrcMultiHashIsForbiddenArr = 0;

            /* for every index in crcMultiHashArr, scan the relevant hwIndex list in DB and check
               for conflicts: one of the rules has same size and same LSB as the new entry. */
            rc = prvCpssDxChExactMatchManagerDbCalcForbiddenHashArray(exactMatchManagerId,GT_TRUE,entryPtr->exactMatchEntry.key.pattern,entryPtr->exactMatchEntry.key.keySize,
                                                                      entryPtr->exactMatchEntry.lookupNum,calcInfoPtr->bankStep,calcInfoPtr->numberOfElemInCrcMultiHashArr,
                                                                      calcInfoPtr->crcMultiHashArr,&calcInfoPtr->numberOfElemInCrcMultiHashIsForbiddenArr,calcInfoPtr->crcMultiHashIsForbiddenArr);
            if (rc == GT_FULL)
            {
                calcInfoPtr->calcEntryType = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CALC_ENTRY_TYPE_NOT_FOUND_AND_NO_FREE_E;
                calcInfoPtr->dbEntryPtr    = NULL;
                /* if during entryAdd we want to return GT_FULL.
                   if during entryUpdate/entryDelete/EntryGet we want to return GT_BAD_STATE. */
                if (addParamsPtr == NULL)
                {
                    return GT_OK;
                }
                ...
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("GT_FULL, rule was blocked in all indexes, skip cuckoo.\n");
                }
                return rc;
            }

            /* verify that the free index is not forbidden or used. then need to use cuckoo */
            calcInfoPtr->lastBankIdChecked = bankId - calcInfoPtr->bankStep;
            rc = prvCpssDxChExactMatchManagerDbGetMostPopulatedBankId(exactMatchManagerPtr,calcInfoPtr);
            if (rc != GT_OK)
            {
                calcInfoPtr->calcEntryType = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CALC_ENTRY_TYPE_NOT_FOUND_AND_NO_FREE_E;
                calcInfoPtr->dbEntryPtr    = NULL;
                ...
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("rule was blocked, forbidden indexes + used indexes == all indexes. need to use cuckoo.\n");
                }
                return GT_OK;
            }

            /* the new entry conflict with all other rules that exists in the database. all indexes are blocked and cannot be used;
               no alternative locations, try to use cuckoo to move other entries. */
            if (calcInfoPtr->numberOfElemInCrcMultiHashIsForbiddenArr == calcInfoPtr->numberOfElemInCrcMultiHashArr/calcInfoPtr->bankStep)
            {
                calcInfoPtr->calcEntryType = PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CALC_ENTRY_TYPE_NOT_FOUND_AND_NO_FREE_E;
                calcInfoPtr->dbEntryPtr    = NULL;
                ...
                if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
                {
                    cpssOsPrintf("rule was blocked in all indexes. need to use cuckoo.\n");
                }
                return GT_OK;
            }
...
)
{
    ...
    while (counterClientType < CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E)
    {
        counterKeySize = CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E;
        while (counterKeySize < CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E)
        {
...
(
    IN  PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC      *exactMatchManagerPtr,
    OUT PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_TYPE_COUNTERS_STC   typeCountersArr[CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E][CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E],
...
)
{
    ...
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_ENTRY_TYPE_COUNTERS_STC   typeCountersArray[CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E][CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E];
    ...
    PRV_CPSS_DXCH_CUCKOO_FUNC_PTR_STC *cuckooDbFuncPtr =&(PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.cuckooDir.cuckooSrc.cuckooDbFuncPtr[PRV_CPSS_DXCH_CUCKOO_CLIENTS_EXACT_MATCH_E]));
    PRV_CPSS_DXCH_CUCKOO_DB_STATISTICS_PTR_GET_FUN statisticsPtrGetFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbStatisticsPtrGetFunc,PRV_CPSS_DXCH_CUCKOO_DB_STATISTICS_PTR_GET_FUN);
    ...
    counterClientType=CPSS_DXCH_EXACT_MATCH_CLIENT_TTI_E;
    for(;counterClientType < CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E; counterClientType++)
    {
        counterKeySize=CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E;
        for (;counterKeySize < CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E; counterKeySize++)
        {
            if (typeCountersArray[counterClientType][counterKeySize].entryTypePopulation !=
                exactMatchManagerPtr->typeCountersArr[counterClientType][counterKeySize].entryTypePopulation && !errorDetected)
            {
                PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
    ...
    rc = statisticsPtrGetFunc(exactMatchManagerId,&cuckooDbBankStPtr,&cuckooDbSizeStPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal prvCpssDxChExactMatchManagerDbCheck_dbHashFreeList function
* @endinternal
*
* @brief The function verifies the Exact Match manager hash free list in the DB.
*
* @param[in] exactMatchManagerPtr  - (pointer to) the Exact Match manager
* @param[out] resultArray          - (pointer to) Test result array.
* @param[out] errorNumPtr          - (pointer to) Number of error types filled in resultArray
...
*
*/
static GT_STATUS prvCpssDxChExactMatchManagerDbCheck_dbHashFreeList
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC   *exactMatchManagerPtr,
    OUT CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_RESULT_ENT      resultArray[],
    OUT GT_U32                                                  *errorNumPtr
...
{
    GT_U32  totalFreeHashNodes,freeCnt = 0;
    GT_BOOL getNextErrorDetected = GT_FALSE;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC  *nodePtr;

    /* Check head of hash free list, Head is expect to be invalid in case of the pool of hash nodes in empty */
    if((exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer == NULL) &&
            (exactMatchManagerPtr->totalFreeHashNodes != 0))
    {
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                            CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_1ST_PTR_INVALID_E,
                                                            errorNumPtr);
    }

    if((exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer == NULL) &&
       (exactMatchManagerPtr->totalFreeHashNodes == exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS/2))
    {
        /* Head of hash free list not valid, skip the hash list check
         * Scenario - Many rules were added with conflicts and their hash locations were taken from hash free list untill filled with max entries. */
        return GT_OK;
    }
    ...
    /* Traverse till end of free list (Head is verified, start with next); count number of nodes in the 'hash free list' */
    totalFreeHashNodes = exactMatchManagerPtr->totalFreeHashNodes;
    while(totalFreeHashNodes > 1)
    {
        if ((nodePtr == NULL || nodePtr->nextNodePointer == NULL) && !getNextErrorDetected)
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_GETNEXT_PTR_INVALID_E,
                                                                 errorNumPtr);
            getNextErrorDetected = GT_TRUE;
            break;
        }
    ...
    /* Match SW counters with Traverse counter */
    if(freeCnt != exactMatchManagerPtr->totalFreeHashNodes)
    {
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                            CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_ENTRIES_NUM_MISMATCH_E,
                                                            errorNumPtr);
    }
...
static GT_STATUS prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perKey
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC   *exactMatchManagerPtr,
    IN CPSS_DXCH_EXACT_MATCH_KEY_SIZE_ENT                       keySize,
    INOUT GT_BOOL                                               *indexCounterInvalidEnPtr,
    INOUT GT_BOOL                                               *getNextErrorDetectedPtr,
    INOUT GT_BOOL                                               *keySizeInvalidEnPtr,
    INOUT GT_BOOL                                               *firstPtrInvalidEnPtr,
    INOUT CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_RESULT_ENT    resultArray[],
    INOUT GT_U32                                                *errorNumPtr
)
{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *headOfEntriesListPtr=NULL;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *nodePtr=NULL;
    GT_U16                                                  *entriesCounterPtr=NULL;
    GT_U32                                                  *sumEntriesCounterPtr=NULL;
    GT_U32                                                  entriesCounter=0,sumEntriesCounter=0;
    GT_U32                                                  hwIndex,listCnt=0;
    GT_U32                                                  maxEntriesLength_19B=0,maxEntriesLength_33B=0,maxEntriesLength_47B=0;
    GT_U32                                                  *maxEntriesLengthPtr=NULL,hwIndexSave=0xFFFFFFFF;
    GT_U32                                                  totalEntriesLength=0,totalIndexes=0,freeIndexes=0;
    GT_U32                                                  totalIndex_listZero=0,totalIndex_listOne=0,totalIndex_listBiggerOne=0;
    GT_U32                                                  countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS___LAST___E];

    /* in this function we want to count each error only once since the
       hash arrays can lead us to the same array for different hwIndexes */
    cpssOsMemSet(&countedErrors,0,sizeof(countedErrors));
    ...
    for(hwIndex = 0 ; hwIndex < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; hwIndex++)
    {
        ...
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
            ...
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_19B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_19B;
            maxEntriesLengthPtr = &maxEntriesLength_19B;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
            headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_33B;
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_33B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_33B;
            maxEntriesLengthPtr = &maxEntriesLength_33B;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
            headOfEntriesListPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].headOfEntriesListPointer_47B;
            entriesCounterPtr = &exactMatchManagerPtr->hashIndexArr[hwIndex].entriesCounter_47B;
            sumEntriesCounterPtr = &exactMatchManagerPtr->sumEntriesCounter_47B;
            maxEntriesLengthPtr = &maxEntriesLength_47B;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                          "keySize [%d] is not supported in DB exactMatchManagerPtr>hashIndexArr",keySize);
        ...
        /* Check head of entries list, Head is expexcted to always be valid */
        if ((headOfEntriesListPtr == NULL)&&
            (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E]==0))
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E,
                                                                 errorNumPtr);
            *firstPtrInvalidEnPtr = GT_TRUE;
            countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E]++;
           /* do not return GT_OK;
              we want to continue with the checks to see all errors we have */
           continue;
        }

        /* Check head of entries list, Head->next is expected to be invalid in case of the counter of the entries is '0'. */
        if ((headOfEntriesListPtr != NULL) &&
            (headOfEntriesListPtr->nextNodePointer != NULL && (*entriesCounterPtr == 0) && !*firstPtrInvalidEnPtr)&&
            (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E]==0))
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E,
        ...
        /* Check counter of entries list, Counter is expect to be invalid in case of no entries in the list. */
        if ((headOfEntriesListPtr != NULL) &&
            (headOfEntriesListPtr->nextNodePointer == NULL && (*entriesCounterPtr != 0) && !*indexCounterInvalidEnPtr)&&
            (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E]==0))
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
                                                                 errorNumPtr);
            *indexCounterInvalidEnPtr = GT_TRUE;
            countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E]++;
            /* do not return GT_OK;
               we want to continue with the checks to see all errors we have */
            continue;
        }
        ...
        if (*entriesCounterPtr == 0)
        {
            /* the entries list is empty nothing to check. */
            totalIndex_listZero++;
            continue;
        }

        /* update the max length of conflict rules in a list */
        if ((*maxEntriesLengthPtr < *entriesCounterPtr) && (*entriesCounterPtr > 1))
        {
            *maxEntriesLengthPtr = *entriesCounterPtr;
            hwIndexSave = hwIndex;
        }

        /* sum all lists for average calculation  */
        if (*entriesCounterPtr > 1)
        {
            totalEntriesLength += *entriesCounterPtr;
            totalIndexes++;
            totalIndex_listBiggerOne++;
        }
        else /* *entriesCounterPtr == 1 */
        {
            totalIndex_listOne++;
        }

        /* Initialize the iterator with HEAD node */
        nodePtr     = headOfEntriesListPtr;
        entriesCounter = *entriesCounterPtr;
        listCnt = 0;

        if ((nodePtr != NULL && nodePtr->nextNodePointer == NULL && !*getNextErrorDetectedPtr)&&
            (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E]==0))
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E,
                                                                 errorNumPtr);
            *getNextErrorDetectedPtr = GT_TRUE;
            countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E]++;
            /* do not return GT_OK;
               we want to continue with the checks to see all errors we have */
            continue;
        }

        /* Traverse till end of free list (Head is verified, start with next); count number of nodes in the 'entries list' */
        while(entriesCounter > 0)
        {
            if (nodePtr != NULL && nodePtr->nextNodePointer == NULL)
            {
                if ((!*getNextErrorDetectedPtr)&&
                    (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E]==0))
                {
                    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                         CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E,
                ...
                /* do not return GT_OK;
                   we want to continue with the checks to see all errors we have
                   break the loop to continue to the next hwIndex checks */
                break;
            }
            ...
            /* Valid entry but keySize is not matching. */
            if((nodePtr != NULL && nodePtr->entryPointer != NULL && nodePtr->entryPointer->hwExactMatchKeySize != keySize)&&
               (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E]==0))
            {
                PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                     CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E,
                                                                     errorNumPtr);
                *keySizeInvalidEnPtr = GT_TRUE;
                countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E]++;
                /* do not return GT_OK;
                   we want to continue with the checks to see all errors we have */
                continue;
            }
        ...
        /* Match SW counters with Traverse counter */
        if((listCnt != *entriesCounterPtr && !*indexCounterInvalidEnPtr)&&
           (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E]==0))
        {
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
        ...
        sumEntriesCounter += listCnt;
    }

    /* Match SW counters with Traverse counter */
    if((sumEntriesCounterPtr != NULL && sumEntriesCounter != *sumEntriesCounterPtr && !*indexCounterInvalidEnPtr)&&
       (countedErrors[CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E]==0))
    {
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                            CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
    ...
    if (hwIndexSave != 0xFFFFFFFF)
    {
        switch (keySize)
        {
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
            /*cpssOsPrintf("\nkey size 19B: ");*/
            freeIndexes = totalIndex_listZero-exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/2*1 /* count only heads */;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
            /*cpssOsPrintf("\nkey size 33B: ");*/
            freeIndexes = totalIndex_listZero-exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/3*2 /* count only heads */;
            break;
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
            /*cpssOsPrintf("\nkey size 47B: ");*/
            freeIndexes = totalIndex_listZero-exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes/4*3 /* count only heads */;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,
                                          "keySize [%d] is not supported in DB exactMatchManagerPtr>hashIndexArr",keySize);
       }
        freeIndexes = freeIndexes; /* skip warnings */
#if 0
        cpssOsPrintf("max conflict rules in list [%d], for hwIndex [%d], average conflict rules (>1) in list [%d], total conflicts rules (>1) [%d]\n",
                      *maxEntriesLengthPtr,hwIndexSave,(totalIndexes == 0 ? 0xFFFFFFFF : totalEntriesLength/totalIndexes),totalEntriesLength);
        cpssOsPrintf("\nindexes not used [%d]\n",freeIndexes);
        cpssOsPrintf("indexes with single rule [%d]\n",totalIndex_listOne);
        cpssOsPrintf("indexes with conflicts [%d]\n\n",totalIndex_listBiggerOne);
#endif
    }
...
static GT_STATUS prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perUsedEntry
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC   *exactMatchManagerPtr,
    IN GT_U32                                                   exactMatchBanksNum,
    IN GT_U32                                                   exactMatchSize,
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC    *dbEntryPtr,
    INOUT GT_U32                                                *sumEntriesCounter_19BPtr,
    INOUT GT_U32                                                *sumEntriesCounter_33BPtr,
    INOUT GT_U32                                                *sumEntriesCounter_47BPtr,
    OUT GT_BOOL                                                 *counterInvalidEnPtr,
    OUT GT_BOOL                                                 *foundActualConflictEnPtr,
    INOUT CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_RESULT_ENT    resultArray[],
    INOUT GT_U32                                                *errorNumPtr
)
{
    GT_U32                                                  numOfElemInCrcMultiHashArr;
    GT_U32                                                  crcMultiHashArr[CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_NUM_BANKS_CNS];
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *headOfEntriesListPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC      *nodePtr;
    GT_U16                                                  *entriesCounterPtr;
    GT_U32                                                  *sumEntriesCounterPtr;
    GT_U32                                                  entriesCounter=0,foundHashLocations;
    GT_BOOL                                                 isEqual=GT_TRUE;
    GT_U32                                                  bankId,hwIndex,bankStep;
    GT_STATUS                                               rc = GT_OK;
    CPSS_DXCH_EXACT_MATCH_KEY_STC                           entryKey;
    GT_U32                                                  exactMatchManagerId,ii,sizeInBytes;
    ...
    entryKey.keySize = dbEntryPtr->hwExactMatchKeySize;
    cpssOsMemCpy(&entryKey.pattern[0],&dbEntryPtr->pattern[0],sizeof(GT_U8)*CPSS_DXCH_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS);
    ...
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
        bankStep = 2;
        sumEntriesCounterPtr = sumEntriesCounter_19BPtr;
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
        bankStep = 3;
        sumEntriesCounterPtr = sumEntriesCounter_33BPtr;
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
        bankStep = 4;
        sumEntriesCounterPtr = sumEntriesCounter_47BPtr;
        break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(dbEntryPtr->hwExactMatchKeySize);
    }

    /* check that entry exists in 'hash index array' for all hash indexes locations */
    for (bankId = 0, foundHashLocations = 0;
         (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<numOfElemInCrcMultiHashArr);
         bankId += bankStep)
    {
        /* DB elements should be added to all its possible hashes - regardless which one it uses currently
           (not only to selectedBankId) */
        ...
        /* search dbEntry in 'entries list' */
        while (nodePtr->nextNodePointer != NULL && entriesCounter > 0)
        {
            nodePtr = nodePtr->nextNodePointer;
            /* check if the pointers are the same */
            isEqual = (&(dbEntryPtr->pattern[0]) == &(nodePtr->entryPointer->pattern[0])) ? GT_TRUE : GT_FALSE;
            if (isEqual==GT_TRUE)
            {
                /* found the entry in the 'entry list'. check the next hash location */
                foundHashLocations++;
                (*sumEntriesCounterPtr)++;
                break;
            }
    ...
    for (ii=0; ii<*errorNumPtr; ii++)
    {
        /* when HW_ENTRY_INVALID == GT_TRUE it influence on HASH_ENTRIES_LIST; in this case we don't want the error */
        if (resultArray[ii] == CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HW_ENTRY_INVALID_E)
            *counterInvalidEnPtr = GT_TRUE;
    }
    if (foundHashLocations != exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes/bankStep && !*counterInvalidEnPtr)
    {
        /* didn't find the entry in all 'entries lists' */
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                             CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
                                                             errorNumPtr);
        *counterInvalidEnPtr = GT_TRUE;
    ...
    /* check that there are ACTUAL conflicts for entry 'hash index array' */
    for (bankId = 0;
         (bankId < exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)&&(bankId<numOfElemInCrcMultiHashArr);
         bankId += bankStep)
    {
        ...
        /* scan the relevant hwIndex list in DB and check if one of the rules has same size and same LSB as the entry */
        rc = prvCpssDxChExactMatchManagerDbRuleConflictValidityCheck(exactMatchManagerId,entryKey.pattern,dbEntryPtr->hwExactMatchKeySize,dbEntryPtr->hwExactMatchLookupNum,hwIndex);
        /* found ACTUAL conflict rule */
        if (rc == GT_FULL && !*foundActualConflictEnPtr)
        {
            if (PRV_SHARED_EMM_DIR_TTI_SRC_GLOBAL_VAR_GET(debugPrint))
            {
                cpssOsPrintf("found ACTUAL conflict rule hwIndex %d lookup number %d\n",hwIndex,dbEntryPtr->hwExactMatchLookupNum);
                EM_PRINT_PATTERN(entryKey.pattern,sizeInBytes);
            }

            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_FOUND_ACTUAL_CONFLICT_E,
                                                                 errorNumPtr);
            *foundActualConflictEnPtr = GT_TRUE;
        }
    }
...
/**
* @internal prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray function
* @endinternal
*
* @brief The function verifies the Exact Match manager hash index array data & entries list.
*
* @param[in] exactMatchManagerPtr  - (pointer to) the Exact Match manager
* @param[out] resultArray          - (pointer to) Test result array.
* @param[out] errorNumPtr          - (pointer to) Number of error types filled in resultArray
...
*
*/
static GT_STATUS prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray
(
    IN PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC   *exactMatchManagerPtr,
    OUT CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_RESULT_ENT      resultArray[],
    OUT GT_U32                                                  *errorNumPtr
...
{
    GT_STATUS                                               rc = GT_OK;
    GT_BOOL                                                 counterInvalidEn = GT_FALSE, getNextErrorDetected = GT_FALSE;
    GT_BOOL                                                 keySizeInvalidEn = GT_FALSE,firstPtrInvalidEn = GT_FALSE,foundActualConflictEn = GT_FALSE;
    GT_U32                                                  sumEntriesCounter_19B=0,sumEntriesCounter_33B=0,sumEntriesCounter_47B=0;
    CPSS_EXACT_MATCH_MHT_ENT                                exactMatchBanksNum;
    CPSS_EXACT_MATCH_TABLE_SIZE_ENT                         exactMatchSize;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC   *dbEntryPtr;
    GT_BOOL                                                 getFirst = GT_TRUE;

    if(exactMatchManagerPtr->actionMatchWaEnable)
    {
        /* Index array DB direction checks */
        prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perKey(exactMatchManagerPtr,CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E,
                                                                    &counterInvalidEn,&getNextErrorDetected,&keySizeInvalidEn,
                                                                    &firstPtrInvalidEn,resultArray,errorNumPtr);

        prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perKey(exactMatchManagerPtr,CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E,
                                                                    &counterInvalidEn,&getNextErrorDetected,&keySizeInvalidEn,
                                                                    &firstPtrInvalidEn,resultArray,errorNumPtr);

        prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perKey(exactMatchManagerPtr,CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E,
                                                                    &counterInvalidEn,&getNextErrorDetected,&keySizeInvalidEn,
                                                                    &firstPtrInvalidEn,resultArray,errorNumPtr);
        /* Regular entries DB direction */
        if(!exactMatchManagerPtr->headOfUsedList.isValid_entryPointer)
        {
            /* Head of list not valid, skip the list check
             * Scenario - Exact Match Manager created with at least one device but no entry populated */
        ...
        if (counterInvalidEn)
        {
            /* this error is already set in prev checks. */
            return GT_OK;
        }
              ...
              prvCpssDxChExactMatchManagerDbEntryIteratorGetNext(exactMatchManagerPtr,getFirst,&dbEntryPtr))
        {
            ...
            if (dbEntryPtr->hwExactMatchKeySize == CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E)
                continue;

            rc = prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray_perUsedEntry(exactMatchManagerPtr,exactMatchBanksNum,exactMatchSize
                                                                                   ,dbEntryPtr,&sumEntriesCounter_19B,&sumEntriesCounter_33B,
                                                                                   &sumEntriesCounter_47B,&counterInvalidEn,&foundActualConflictEn,
                                                                                   resultArray,errorNumPtr);
            if (rc != GT_OK)
            {
                return rc;
        ...
        if (sumEntriesCounter_19B != exactMatchManagerPtr->sumEntriesCounter_19B && !counterInvalidEn)
        {
            /* Match SW counters with Traverse counter */
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
            ...
            counterInvalidEn = GT_TRUE;
        }

        if (sumEntriesCounter_33B != exactMatchManagerPtr->sumEntriesCounter_33B && !counterInvalidEn)
        {
            /* Match SW counters with Traverse counter */
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
        ...
        if (sumEntriesCounter_47B != exactMatchManagerPtr->sumEntriesCounter_47B && !counterInvalidEn)
        {
            /* Match SW counters with Traverse counter */
            PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                                 CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E,
...
)
{
    ...
    GT_U32                                                  totalEntriesPopulation=0;/* population according to entries and not indexes */
    GT_U32                                                  totalIndexPopulation=0;/* population according to entries and not indexes */
    GT_U32                                                  i,j;
    ...
    /* find the total population entries.
       we can not use exactMatchManagerPtr->totalPopulation since it
       is the population in indexes and the totalAgeBinCnt in per entry */
    for (i=0;i<CPSS_DXCH_EXACT_MATCH_MANAGER_CLIENT_LAST_E;i++)
    {
        for (j=0;j<CPSS_DXCH_EXACT_MATCH_MANAGER_KEY_SIZE_LAST_E;j++)
        {
            totalEntriesPopulation += exactMatchManagerPtr->typeCountersArr[i][j].entryTypePopulation;
            totalIndexPopulation += (exactMatchManagerPtr->typeCountersArr[i][j].entryTypePopulation)*(j+1);
        }
    }
    if (totalAgeBinCnt != totalEntriesPopulation)
    {
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
    ...
    if (totalIndexPopulation != exactMatchManagerPtr->totalPopulation)
    {
        PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                                            CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_TOTAL_NUM_MISMATCH_E,
                                                            errorNumPtr);
        CPSS_LOG_ERROR_MAC("mismatch for totalIndexPopulation[%d] != totalPopulation[%d]",
            totalIndexPopulation,
            exactMatchManagerPtr->totalPopulation);
    }
...
)
{
    ...
    *errorNumPtr = 0;
    ...
    if(checksPtr->dbHashFreeListCheckEnable) /* dbHashFreeListCheckEnable */
    {
        rc = prvCpssDxChExactMatchManagerDbCheck_dbHashFreeList(exactMatchManagerPtr, resultArray, errorNumPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
    }
    if(checksPtr->dbHashIndexArrayCheckEnable) /* dbHashIndexArrayCheckEnable */
    {
        rc = prvCpssDxChExactMatchManagerDbCheck_dbHashIndexArray(exactMatchManagerPtr, resultArray, errorNumPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    GT_U32                                                      tempDbIndex;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC      *dbEntryPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC      *tempDbEntryPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_ENTRY_FORMAT_STC      *tempOverlapDbEntryPtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_NODE_PTR_STC          *dbNodePtr;  /* For delete scan */
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC         *dbHashNodePtr;
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_HASH_NODE_STC         *dbEntryNodePtr=NULL;
    GT_U32                                                      elementNum=0;
    GT_U32                                                      step;
    ...
    for(ii=0; ii<expErrorNum; ii++)
    {
        ...
        if((errorEnable && EM_IS_BMP_SET_MAC(&exactMatchManagerPtr->selfTestBitmap[expResultArray[ii] >> 5], (expResultArray[ii] & 0x1F))) ||
           (!errorEnable && !EM_IS_BMP_SET_MAC(&exactMatchManagerPtr->selfTestBitmap[expResultArray[ii] >> 5], (expResultArray[ii] & 0x1F))))
        {
            continue;
        }
        ...
        switch(expResultArray[ii])
        {
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_GLOBAL_PARAM_HW_INVALID_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->capacityInfo.maxTotalEntries;
                ...
                {/* Correct last injected error*/
                    exactMatchManagerPtr->capacityInfo.maxTotalEntries = restoreInfoPtr->counterAndHwInfo.counter;
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_TYPE_COUNTERS_INVALID_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->typeCountersArr[counterClientType][counterKeySize].entryTypePopulation;
                ...
                {/* Correct last injected error*/
                    exactMatchManagerPtr->typeCountersArr[counterClientType][counterKeySize].entryTypePopulation = restoreInfoPtr->counterAndHwInfo.counter;
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_CUCKOO_BANK_COUNTERS_MISMATCH_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->bankInfoArr[bankId].bankPopulation;
                ...
                {/* Correct last injected error*/
                    exactMatchManagerPtr->bankInfoArr[bankId].bankPopulation = restoreInfoPtr->counterAndHwInfo.counter;
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_TOTAL_NUM_MISMATCH_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->totalPopulation;
                ...
                {/* Correct last injected error*/
                    exactMatchManagerPtr->totalPopulation = restoreInfoPtr->counterAndHwInfo.counter;
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_SCAN_PTR_INVALID_E:
                ...
                dbIndex     = exactMatchManagerPtr->headOfUsedList.entryPointer;
                dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                {
                    dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                    if(dbEntryPtr->isFirst==GT_TRUE)
                    {
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_USAGE_MATRIX_L1_INVALID_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->agingBinInfo.ageBinListPtr[0].totalUsedEntriesCnt;
                    ...
                    /* we need also to inject error for MATRIX_L2_INVALID
                       if we decrease the totalUsedEntriesCnt by 1 we have 2 options
                       if the l1 bins bitmap if full we will expect l2 bit set to 1
                       inspire the reduction and this will pop the fail we expect
                       but if we have a case that the l1 bins bitmap is not full then
                       the l2 bit is 0 and even after we decrease the totalUsedEntriesCnt
                       the status of l2 is legal and we will not get a fail
                       we want to prevent having same test will different behavior */

                    /* Inject Error */
                    /* if the l2 bit is not set to 1 (meaning full) - we want to set it*/
                    tmpCnt = 0;
                    if ((exactMatchManagerPtr->agingBinInfo.level1UsageBitmapPtr[tmpCnt]!=0xFFFFFFFF)&&
                        (!EM_IS_BMP_SET_MAC(&exactMatchManagerPtr->agingBinInfo.level2UsageBitmap, tmpCnt)))
                    {
                        EM_BMP_SET_MAC(&exactMatchManagerPtr->agingBinInfo.level2UsageBitmap, tmpCnt);
                        restoreInfoPtr->counterAndHwInfo.l2BitStatus = GT_FALSE;
                        EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_USAGE_MATRIX_L2_INVALID_E);
                    }
                }
                else
                {/* Correct last injected error*/
                    exactMatchManagerPtr->agingBinInfo.ageBinListPtr[0].totalUsedEntriesCnt = restoreInfoPtr->counterAndHwInfo.counter;
                    ...
                    /* Correct l2 matrix last injected error*/
                    tmpCnt = 0;
                    if ((exactMatchManagerPtr->agingBinInfo.level1UsageBitmapPtr[tmpCnt]!=0xFFFFFFFF)&&
                        (restoreInfoPtr->counterAndHwInfo.l2BitStatus == GT_FALSE))
                    {
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_ENTRY_BIN_INDEX_INVALID_E:
                ...
                for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                {
                    ...
                    /* age bit is kept per entry and not per index
                       for example for 3 key entry we have 3 indexes used
                       only the first index hold valid data for the age bin
                       if we want to get to the third age bin we need to jump
                       according to elements and not the indexes */
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_USAGE_MATRIX_L2_INVALID_E:
                if (expErrorNum==CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS___LAST___E)
                {
                    /* in case of CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_ALL_E we need to skip
                       CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_AGING_USAGE_MATRIX_L2_INVALID_E
                       since it will return GT_BAD_PARAM and it will break
            ...
            case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HW_LOOKUP_CLIENT_INVALID_E:
                ...
                {
                    /* in case of CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_ALL_E we need to skip
                       LOOKUP_CLIENT_INVALID since it will return GT_BAD_PARAM and it will break
                       the loop on the rest of the expected fails*/
                    break;
                }
                if (errorEnable)
        ...
        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_1ST_PTR_INVALID_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->dbNode.nextNodePointer = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->totalFreeHashNodes;
                    exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = NULL;
                    exactMatchManagerPtr->totalFreeHashNodes++;
                    EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_1ST_PTR_INVALID_E);
                }
                else
                {/* Correct last injected error*/
                    exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer = restoreInfoPtr->dbNode.nextNodePointer;
                    exactMatchManagerPtr->totalFreeHashNodes = restoreInfoPtr->counterAndHwInfo.counter;
                    EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_1ST_PTR_INVALID_E);
                }
                break;

        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_GETNEXT_PTR_INVALID_E:
                if(errorEnable)
                {/* Inject Error */
                    if (exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer != NULL)
                    {
                        dbHashNodePtr = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
                        dbHashNodePtr = dbHashNodePtr->nextNodePointer;
                        restoreInfoPtr->dbNode.nextNodePointer = dbHashNodePtr->nextNodePointer;
                        dbHashNodePtr->nextNodePointer = NULL;
                        EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_GETNEXT_PTR_INVALID_E);
                    }
                }
                else
                {/* Correct last injected error*/
                    if (exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer != NULL)
                    {
                        dbHashNodePtr = exactMatchManagerPtr->headOfFreeHashNodesList.nextNodePointer;
                        dbHashNodePtr = dbHashNodePtr->nextNodePointer;
                        dbHashNodePtr->nextNodePointer = restoreInfoPtr->dbNode.nextNodePointer;
                        EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_GETNEXT_PTR_INVALID_E);
                    }
                }
                break;

        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_ENTRIES_NUM_MISMATCH_E:
                if(errorEnable)
                {/* Inject Error */
                    restoreInfoPtr->counterAndHwInfo.counter = exactMatchManagerPtr->totalFreeHashNodes;
                    exactMatchManagerPtr->totalFreeHashNodes++;
                    EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_ENTRIES_NUM_MISMATCH_E);
                }
                else
                {/* Correct last injected error*/
                    exactMatchManagerPtr->totalFreeHashNodes = restoreInfoPtr->counterAndHwInfo.counter;
                    EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_ENTRIES_NUM_MISMATCH_E);
                }
                break;

        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E:
            if(exactMatchManagerPtr->actionMatchWaEnable == GT_TRUE)
            {
                if(errorEnable)
                {
                    dbIndex     = exactMatchManagerPtr->headOfUsedList.entryPointer;
                    ...
                    for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                    {
                           ...
                           (dbEntryPtr->hwExactMatchKeySize!=0))
                        {
                            ...
                            {
                            case 1:
                                dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_19B;
                                break;
                            case 2:
                                dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_33B;
                                break;
                            case 3:
                                dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_47B;
                                break;
                            default:
                                /* do nothing */
                                break;
                            }
                            if (dbEntryNodePtr != NULL && dbEntryNodePtr->nextNodePointer != NULL)
                            {
                                tempDbIndex = dbIndex;
                                tempDbEntryPtr = dbEntryNodePtr->nextNodePointer->entryPointer;
                                step = tempDbEntryPtr->hwExactMatchKeySize+1;
                                /* we want to check that after the increment in the keySize we will
                                   get to a valid first entry - otherwhise it will cause a fail in other checks */
                                if (tempDbEntryPtr->isValid_prevEntryPointer==0)
                                {
                                    break;
                                }
                                tempDbIndex = tempDbEntryPtr->prevEntryPointer;
                                tempDbIndex++;/* get to the place we need */
                                /* protect access to array out of range */
                                EM_CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(tempDbIndex,exactMatchManagerPtr->capacityInfo.maxTotalEntries);
                                tempDbEntryPtr =  &exactMatchManagerPtr->entryPoolPtr[tempDbIndex];

                                tempOverlapDbEntryPtr = &exactMatchManagerPtr->entryPoolPtr[tempDbIndex+step];
                                /* we need the counters to fail in the bankPopulation check so we want to chose
                                   an hwIndex that its last index do not fall in the same bank as the original
                                   hwIndex we overlaped due to increase of keySize*/
                                if (((tempDbEntryPtr->hwIndex + step) % exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes)!=
                                    ((tempOverlapDbEntryPtr->hwIndex) % exactMatchManagerPtr->capacityInfo.hwCapacity.numOfHashes))
                                {
                                   /* Inject Error */
                                   dbEntryNodePtr = dbEntryNodePtr->nextNodePointer;
                                   restoreInfoPtr->counterAndHwInfo.counter = dbEntryNodePtr->entryPointer->hwExactMatchKeySize;
                                   restoreInfoPtr->counterAndHwInfo.hwIndexUsed = tempDbEntryPtr->hwIndex;
                                   dbEntryNodePtr->entryPointer->hwExactMatchKeySize++; /* Invalidate the entry next to head */
                                   EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E);
                                   break;
                                }
                            }
                        }
                ...
                {
                    /* Correct last injected error*/
                    switch (restoreInfoPtr->counterAndHwInfo.counter)
                    {
                    case 1:
                        dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[restoreInfoPtr->counterAndHwInfo.hwIndexUsed].headOfEntriesListPointer_19B;
                        break;
                    case 2:
                        dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[restoreInfoPtr->counterAndHwInfo.hwIndexUsed].headOfEntriesListPointer_33B;
                        break;
                    case 3:
                        dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[restoreInfoPtr->counterAndHwInfo.hwIndexUsed].headOfEntriesListPointer_47B;
                        break;
                    default:
                        /* do nothing */
                        break;
                    ...
                    {
                       dbEntryNodePtr = dbEntryNodePtr->nextNodePointer;
                       dbEntryNodePtr->entryPointer->hwExactMatchKeySize = restoreInfoPtr->counterAndHwInfo.counter;
                       EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_KEY_SIZE_INVALID_E);
                       break;
                    }
                }
            }
        ...
        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E:
                if(exactMatchManagerPtr->actionMatchWaEnable == GT_TRUE)
                {
                    dbIndex     = exactMatchManagerPtr->headOfUsedList.entryPointer;
                    dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                    for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                    {
                        ...
                        if(errorEnable)
                        {
                            if(exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B > 0)
                            {
                                /* Inject Error */
                                dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_19B;
                                if (dbEntryNodePtr->nextNodePointer != NULL)
                                {
                                    restoreInfoPtr->countersInfo.entriesCounter_19B = exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B;
                                    restoreInfoPtr->countersInfo.hwIndexUsed = dbEntryPtr->hwIndex;

                                    exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B = 0;
                                    EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E);
                                    break;
                                }
                            }
                        }
                        ...
                        {
                            /* if we are in correcting last injected error, then entriesCounter_19B is already 0
                               for the hwIndex we are looking */
                            if(dbEntryPtr->hwIndex==restoreInfoPtr->countersInfo.hwIndexUsed)
                            {
                                /* Correct last injected error*/
                                exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B = restoreInfoPtr->countersInfo.entriesCounter_19B;
                                EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_1ST_PTR_INVALID_E);
        ...
        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E:
                if(exactMatchManagerPtr->actionMatchWaEnable == GT_TRUE)
                {
                    dbIndex     = exactMatchManagerPtr->headOfUsedList.entryPointer;
                    dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                    for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                    {
                        dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                        EM_GET_nextEntryPointer_MAC(dbEntryPtr, dbIndex);
                        if (exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B > 3)
                        {
                            break;
                        }
                    }
                    ...
                    {/* Inject Error */
                        dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_19B;
                        restoreInfoPtr->dbNode.nextNodePointer = dbEntryNodePtr->nextNodePointer->nextNodePointer->nextNodePointer;
                        dbEntryNodePtr->nextNodePointer->nextNodePointer->nextNodePointer = NULL;
                        EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E);
                    }
                    else
                    {/* Correct last injected error*/
                        dbEntryNodePtr = &exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_19B;
                        dbEntryNodePtr->nextNodePointer->nextNodePointer->nextNodePointer = restoreInfoPtr->dbNode.nextNodePointer;
                        EM_SELF_TEST_BMP_CLR_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_GETNEXT_PTR_INVALID_E);
                    }
                }
                break;

        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_ENTRIES_NUM_MISMATCH_E:
                if(exactMatchManagerPtr->actionMatchWaEnable == GT_TRUE)
                {
                    dbIndex     = exactMatchManagerPtr->headOfUsedList.entryPointer;
                    dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                    for(tmpCnt = 0; tmpCnt<exactMatchManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
                    {
                        dbEntryPtr  = &exactMatchManagerPtr->entryPoolPtr[dbIndex];
                        EM_GET_nextEntryPointer_MAC(dbEntryPtr, dbIndex);
                        if (exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].headOfEntriesListPointer_19B.nextNodePointer == NULL)
                        {
                            break;
                        }
                    }
                    ...
                    {/* Inject Error */
                        restoreInfoPtr->countersInfo.entriesCounter_19B = exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B;
                        exactMatchManagerPtr->hashIndexArr[dbEntryPtr->hwIndex].entriesCounter_19B++;
                        EM_SELF_TEST_BMP_SET_MAC(exactMatchManagerPtr, CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_FREE_LIST_ENTRIES_NUM_MISMATCH_E);
                    }
                    else
                    {/* Correct last injected error*/
                ...
                break;
        case CPSS_DXCH_EXACT_MATCH_MANAGER_DB_CHECKS_HASH_ENTRY_LIST_FOUND_ACTUAL_CONFLICT_E:
            /* skip: this error can't be reconstructed as it causes additional errors that ares caught by earlier errors and exist */
            break;
        default:
                break;
        }
...
)
{
    ...
    GT_U32                                                      i=0;
    PRV_CPSS_DXCH_CUCKOO_FUNC_PTR_STC *cuckooDbFuncPtr =&(PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.cuckooDir.cuckooSrc.cuckooDbFuncPtr[PRV_CPSS_DXCH_CUCKOO_CLIENTS_EXACT_MATCH_E]));
    PRV_CPSS_DXCH_CUCKOO_DB_DELETE_FUN deleteFunc = PRV_CALLBACK_GET(cuckooDbFuncPtr->prvCpssDxChCuckooDbDeleteFunc,PRV_CPSS_DXCH_CUCKOO_DB_DELETE_FUN);
...
/**
* @internal prvCpssDxChExactMatchManagerDbProfileEntryUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager profile entry to the DB.
*        This API overrite the configuration done in the
*        Exact Match Manager Create.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @param[in] exactMatchManagerPtr   - (pointer to) the Exact Match Manager.
* @param[in] lookupClient           - the lookupClient for wich we are setting the profileId
* @param[in] profileId              - Exact Match profile identifier
*                                    (APPLICABLE RANGES: 1..15)
* @param[in] profileEntryParamsPtr-(pointer to) parameters of profile entry

*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
*/
GT_STATUS prvCpssDxChExactMatchManagerDbProfileEntryUpdate
(
    IN  PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC  *exactMatchManagerPtr,
    IN  CPSS_DXCH_EXACT_MATCH_CLIENT_ENT                        lookupClient,
    IN  GT_U32                                                  profileId,
    IN  CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC  *profileEntryParamsPtr
)
{
    GT_STATUS rc = GT_OK;
    if((profileId > CPSS_DXCH_EXACT_MATCH_MAX_PROFILE_ID_CNS)||(profileId<CPSS_DXCH_EXACT_MATCH_MIN_PROFILE_ID_CNS))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "profileId is not valid in call to prvCpssDxChExactMatchManagerDbProfileEntryGet");
    }

    /**********************************/
    /* check the profileId parameters */
    /**********************************/
    EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->keyParams.keySize, cpssDxChExactMatchProfileKeyParamsSet);

    switch(profileEntryParamsPtr->keyParams.keySize)
    {
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_5B_E:
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
        case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(profileEntryParamsPtr->keyParams.keySize);
    }

    EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->keyParams.keyStart, cpssDxChExactMatchProfileKeyParamsSet);
    /* check validity of keyStart (6 bits)*/
    if(profileEntryParamsPtr->keyParams.keyStart >= BIT_6)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
        "profileEntryParamsPtr->keyParams.keyStart[%d] is more than the 'manager' supports[%d]",
        profileEntryParamsPtr->keyParams.keyStart,BIT_6);
    }

    EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->keyParams.mask, cpssDxChExactMatchProfileKeyParamsSet);

    if (profileEntryParamsPtr->defaultActionEn == GT_TRUE)
    {
        EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->defaultActionType, cpssDxChExactMatchProfileDefaultActionSet);

        switch(profileEntryParamsPtr->defaultActionType)
        {
            case CPSS_DXCH_EXACT_MATCH_ACTION_TTI_E:
            case CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E:
            case CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E:
                break;
            default:
                CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(profileEntryParamsPtr->defaultActionType);
        }
        EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->defaultAction, cpssDxChExactMatchProfileDefaultActionSet);
        ...
        switch(profileEntryParamsPtr->defaultActionType)
        {
            ...
            case CPSS_DXCH_EXACT_MATCH_ACTION_TTI_E:
                ...
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                    "profileEntryParamsPtr->defaultActionType[%d] must be the same as lookupClient[%d]",
                    profileEntryParamsPtr->defaultActionType,lookupClient);
                }
                else
                {
                    /* check validity of default TTI entry */
                    rc = prvCpssDxChExactMatchManagerDbValidateTtiAction(&(profileEntryParamsPtr->defaultAction.ttiAction));
                    if (rc != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"profileEntryParamsPtr->defaultAction.ttiAction");
                    }
                }
                break;
            case CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E:
                ...
                {
                    /* check that exactMatchActionType fit to the egressPolicy configured in the action */
                    if((profileEntryParamsPtr->defaultActionType==CPSS_DXCH_EXACT_MATCH_ACTION_PCL_E)&&
                       (profileEntryParamsPtr->defaultAction.pclAction.egressPolicy==CPSS_PCL_DIRECTION_EGRESS_E))
                    {
                         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of PCL Action. "
                                                                    "profileEntryParamsPtr->defaultActionType[%d] do not fit with, "
                                                                    "profileEntryParamsPtr->defaultAction.pclAction.egressPolicy[%d] ",
                                                       profileEntryParamsPtr->defaultActionType,
                                                       profileEntryParamsPtr->defaultAction.pclAction.egressPolicy);
                    }

                    /* check validity of default PCL entry */
                    rc = prvCpssDxChExactMatchManagerDbValidatePclAction(&(profileEntryParamsPtr->defaultAction.pclAction));
                    if (rc != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"Error in validation of PCL/EPCL Action "
                                                      "profileEntryParamsPtr->defaultAction.pclAction, profileId[%d] ",
                                                      profileId);
                    }
                }
                break;
            case CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E:
                ...
                {
                    /* check that exactMatchActionType fit to the egressPolicy configured in the action */
                    if((profileEntryParamsPtr->defaultActionType==CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E)&&
                       (profileEntryParamsPtr->defaultAction.pclAction.egressPolicy!=CPSS_PCL_DIRECTION_EGRESS_E))
                    {
                         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error in validation of EPCL Action. "
                                                                    "profileEntryParamsPtr->defaultActionType[%d] do not fit with, "
   ...
   EM_BIND_PARAM_TO_LOW_LEVEL_CPSS_API_MAC(profileEntryParamsPtr->defaultActionEn,
                                         cpssDxChExactMatchProfileDefaultActionSet);

    /********************/
    /* set the info     */
    /********************/
    cpssOsMemCpy(&exactMatchManagerPtr->lookupInfo.profileEntryParamsArray[profileId],
                 profileEntryParamsPtr,
                 sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC));
    return GT_OK;
}

/**
* @internal prvCpssDxChExactMatchManagerDbProfileEntryGet function
* @endinternal
*
* @brief The function fetches current Exact Match Manager profile entry setting as they were
*        defined in Exact Match Manager Create or in prvCpssDxChExactMatchManagerDbProfileEntryUpdate.
*
* @param[in] exactMatchManagerId   - the Exact Match Manager id.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] profileId             - Exact Match profile identifier
*                                   (APPLICABLE RANGES: 1..15)
* @param[out] profileEntryParamsPtr-(pointer to) parameters of profile entry
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
* @retval GT_BAD_PTR               - on NULL pointer.
*/
GT_STATUS prvCpssDxChExactMatchManagerDbProfileEntryGet
(
    IN  GT_U32                                                  exactMatchManagerId,
    IN  GT_U32                                                  profileId,
    OUT CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC  *profileEntryParamsPtr
)
{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC  *exactMatchManagerPtr;
    ...
    /* we have valid new manager pointer */
    exactMatchManagerPtr = EM_MANAGER_GET_MAC(exactMatchManagerId);
    CPSS_NULL_PTR_CHECK_MAC(profileEntryParamsPtr);

    if((profileId > CPSS_DXCH_EXACT_MATCH_MAX_PROFILE_ID_CNS)||(profileId<CPSS_DXCH_EXACT_MATCH_MIN_PROFILE_ID_CNS))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "profileId is not valid in call to "
                                                       "prvCpssDxChExactMatchManagerDbProfileEntryUpdate");
    }

    /********************/
    /* extract the info */
    /********************/
    cpssOsMemCpy(profileEntryParamsPtr,
                 &exactMatchManagerPtr->lookupInfo.profileEntryParamsArray[profileId],
                 sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC));
    return GT_OK;
}

/**
* @internal prvCpssDxChExactMatchManagerDbLookupUpdate function
* @endinternal
*
* @brief API for updating Exact Match Manager Lookup configuration to the DB.
*        This API overrite the configuration done in the
*        Exact Match Manager Create.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in] lookupEntryParamsPtr  -(pointer to) parameters of lookup entry
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
*
*/
GT_STATUS prvCpssDxChExactMatchManagerDbLookupUpdate
(
    IN  PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC  *exactMatchManagerPtr,
    IN  CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                        lookupNum,
    IN  CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC         *lookupEntryParamsPtr
)
{
    GT_STATUS rc = GT_OK;
    GT_U32                                      i=0,entryIndex;
    GT_BOOL                                     lookupEnable;
    CPSS_DXCH_EXACT_MATCH_CLIENT_ENT            lookupClient;
    GT_U32                                      lookupClientMappingsNum;
    CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ID_MAPPING_UNT *lookupClientMappingsArray;
    ...
    /**********************************/
    /* check the lookup parameters */
    /**********************************/

    cpssOsMemSet(clientMappingProfileBmpArr,0,sizeof(clientMappingProfileBmpArr));

    lookupEnable                =   lookupEntryParamsPtr->lookupEnable;
    lookupClient                =   lookupEntryParamsPtr->lookupClient;
    lookupClientMappingsNum     =   lookupEntryParamsPtr->lookupClientMappingsNum;
    lookupClientMappingsArray   =   lookupEntryParamsPtr->lookupClientMappingsArray;
    ...
    /* set the info     */
    /********************/
   cpssOsMemCpy(&exactMatchManagerPtr->lookupInfo.lookupsArray[lookupNum],
                 lookupEntryParamsPtr,
                 sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC));
    return GT_OK;
}

/**
* @internal prvCpssDxChExactMatchManagerDbLookupGet function
* @endinternal
*
* @brief API for fetching current Exact Match Manager lookup
*        configuration setting as they were defined in Exact Match
*        Manager Create or changed by cpssDxChExactMatchManagerLookupUpdate.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] lookupNum             - Exact Match lookup number
* @param[out] lookupEntryParamsPtr -(pointer to) parameters of lookup entry
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong parameter
* @retval GT_BAD_PTR               - on NULL pointer.
*/
GT_STATUS prvCpssDxChExactMatchManagerDbLookupGet
(
    IN  GT_U32                                              exactMatchManagerId,
    IN  CPSS_DXCH_EXACT_MATCH_LOOKUP_ENT                    lookupNum,
    OUT CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC    *lookupEntryParamsPtr
)
{
    PRV_CPSS_DXCH_EXACT_MATCH_MANAGER_DB_INSTANCE_INFO_STC  *exactMatchManagerPtr;
    ...
    /* we have valid new manager pointer */
    exactMatchManagerPtr = EM_MANAGER_GET_MAC(exactMatchManagerId);
    CPSS_NULL_PTR_CHECK_MAC(lookupEntryParamsPtr);

    if(lookupNum >= CPSS_DXCH_EXACT_MATCH_LOOKUP_LAST_E)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "lookupNum is not valid in call to "
                                                       "prvCpssDxChExactMatchManagerLookupGet");
    }

    /********************/
    /* extract the info */
    /********************/
    cpssOsMemCpy(lookupEntryParamsPtr,
                 &exactMatchManagerPtr->lookupInfo.lookupsArray[lookupNum],
                 sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC));
    return GT_OK;
}
