...
********************************************************************************
* prvCpssDxChSip7LpmRamMngDefrag.c
*
* DESCRIPTION:
...
/**
* @internal prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockSip7
*           function
* @endinternal
*
...
*/
GT_STATUS prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockSip7
(
...
)
{
    ...
    bucketBlock = (GT_DMM_BLOCK_SIP7 *)oldHwBucketOffsetHandle;

    if( (oldHwBucketOffsetHandle == 0)||
        (DMM_BLOCK_STATUS_SIP7(bucketBlock)!= DMM_BLOCK_ALLOCATED_SIP7)||
        (DMM_BLOCK_STAMP_SIP7(bucketBlock) != DMM_STAMP_USED_SIP7) )
    {
        cpssOsPrintf("oldHwBucketOffsetHandle was not allocated\n");
        *freeBlockCanBeFoundPtr=GT_FALSE;
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockSip7 - GT_FAIL \n");
    }

    newBucketSizeInWords = newBucketSize * PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS;

    sizeOfOptionalUsedAndFreeBlock = prvCpssDmmCheckResizeAvailableWithSameMemorySip7(oldHwBucketOffsetHandle,newBucketSizeInWords);
...
/**
* @internal
*           prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockForRegToCompConversionSip7
*           function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockForRegToCompConversionSip7
(
    ...
    for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
        if (bucketPtr->hwGroupOffsetHandle[j]!=0)
        {
             /* for a given hwGroupOffsetHandle[j] we check if there is free memory near it
                ...
                memory space to create a bigger block */
            retVal = prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockSip7(bucketPtr->hwGroupOffsetHandle[j],
                                                                                      newBucketSize,
                                                                                      freeBlockCanBeFoundPtr);
            if (retVal!=GT_OK)
            {
                *freeBlockCanBeFoundPtr=GT_FALSE;
                CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "error in prvCpssDxChLpmRamMngFindIfSwapAreaCanBeUsefulToFindFreeBlockSip7\n");
            }
...
/**
* @internal prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7
(
...
)
{
    ...
    GT_U32              oldSpaceUsedFromBlockIndexArray[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS]={0}; /* in words not in lines */
    ...
    for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
        if (((neededMemoryBlocksPtr->neededMemoryBlocks[j] == DMM_BLOCK_NOT_FOUND_SIP7)||
             (neededMemoryBlocksPtr->neededMemoryBlocks[j] == DMM_MALLOC_FAIL_SIP7))&&
             (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j]!=0))
        {
            ...
            {
                usedBlock = (GT_DMM_BLOCK_SIP7 *)neededMemoryBlocksPtr->neededMemoryBlocksOldHandleAddr[j];
                oldBlockSize = SIZE_IN_WORDS_SIP7(usedBlock);
    ...
    if (totalNewBucketSize != 0)
    {
        ...
        totalNewBucketSize = totalNewBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS;
        ...
        else
        {
            ...
            {
                partition = (GT_DMM_PARTITION_SIP7 *)tempMemInfoPtr->structsMemPool;
                if( (partition == 0) ||
                    (partition == (GT_DMM_PARTITION_SIP7*)((GT_UINTPTR)0xFFFFFFFE)))
                {
                    /* should never happen */
                    *freeBlockCanBeFoundPtr=GT_FALSE;
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7, "
                                                          "wrong partition bind to the octet - GT_FAIL \n");
                }
                sumOfFreeBlockAfterShrink = prvCpssDmmGetEmptyMemoryLeftSip7(partition);
            ...
            /*  summing all new / extend buckets sizes, deducting their current sizes &
                ...
                prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7ForRegToCompConversion*/
            ...
            else
            {
                ...
                for (i = 0; i < shadowPtr->numOfLpmMemories; i++)
                {
                    ...
                    {
                        partition = (GT_DMM_PARTITION_SIP7 *)shadowPtr->lpmRamStructsMemPoolPtr[i];
                        if( (partition == 0) ||
                            (partition == (GT_DMM_PARTITION_SIP7*)((GT_UINTPTR)0xFFFFFFFE)))
                        {
                            /* should never happen */
                ...
                if ((totalNewBucketSize - totalOldBlockSize) > sumOfTotalFreeBlockInAllBanksAfterShrink)
                {
                    for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
                    {
                        if ((neededMemoryBlocksPtr->neededMemoryBlocksSizes[j] > 0) &&
                            (neededMemoryBlocksPtr->neededMemoryBlocks[j] == DMM_BLOCK_NOT_FOUND_SIP7))
                        {
    ...
    else
    {
        ...
        *freeBlockCanBeFoundPtr=GT_FALSE;
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7 - GT_FAIL \n");
    }
    ...
    for (j= 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
        ...
        *freeBlockCanBeFoundPtr = GT_FALSE;
        ...
        if (neededMemoryBlocksPtr->neededMemoryBlocksOldHandleAddr[j] != 0)
        {
            oldBlockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksPtr->neededMemoryBlocksOldHandleAddr[j]) /
                                                                                    (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
            retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&oldBlockIndex);
            if (retVal != GT_OK)
            {
            ...
            sumOfFreeBlockAfterShrink = prvCpssDmmCheckIfShrinkOperationUsefulForDefragSip7(shadowPtr->lpmRamStructsMemPoolPtr[oldBlockIndex],
                                                                                        newBucketSize * PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,
                                                                                        oldBlockSize,
                                                                                        &useSwapArea);
            if(sumOfFreeBlockAfterShrink!=0)
            {
                     ...
                     (newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS))
                {
                    ...
                    freeSpaceTakenFromBlockIndexArray[oldBlockIndex]+= (newBucketSize * PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS);
        ...
        while((tempMemInfoPtr!= NULL)&&(tempMemInfoPtr->structsMemPool!=0))
        {
            ...
            newBucketSize = neededMemoryBlocksPtr->neededMemoryBlocksSizes[j];
            sumOfFreeBlockAfterShrink = prvCpssDmmCheckIfShrinkOperationUsefulForDefragSip7(tempMemInfoPtr->structsMemPool,
                                                                                        newBucketSize * PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,
                                                                                        oldBlockSize,
                                                                                        &useSwapArea);
            ...
            if(sumOfFreeBlockAfterShrink!=0)
            {
                     ...
                     (newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS))
                {
                    ...
                    /* keep what was already taken */
                    freeSpaceTakenFromBlockIndexArray[tempMemInfoPtr->ramIndex]+= (newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS);
        ...
        for (i = 0; i < shadowPtr->numOfLpmMemories; i++)
        {
            ...
            if (PRV_CPSS_DXCH_LPM_RAM_IS_BLOCK_USED_BY_PROTOCOL_GET_MAC(shadowPtr,protocol,i)==GT_FALSE)
            {
                ...
                oldBlockSize = 0; /* using the old block in previous stage did not help */
                newBucketSize = neededMemoryBlocksPtr->neededMemoryBlocksSizes[j];
                sumOfFreeBlockAfterShrink = prvCpssDmmCheckIfShrinkOperationUsefulForDefragSip7(shadowPtr->lpmRamStructsMemPoolPtr[i],
                                                                                           newBucketSize * PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,
                                                                                           oldBlockSize,
                                                                                           &useSwapArea);
                      ...
                      (newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS)))
                {
                    ...
                    /* keep what was already taken */
                    freeSpaceTakenFromBlockIndexArray[i]+=(newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS);
...
/**
* @internal
*           prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragForRegToCompConversionSip7
*           function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragForRegToCompConversionSip7
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC            *shadowPtr,
    IN  PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC          *lpmMemInfoPtr,
    IN  PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT        protocol,
...
)
{
    ...
    /* all calculations in partition are in words */
    totalNewBucketSize = newBucketSize*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS;
    ...
    for (j= 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
        ...
        {
            usedBlock = (GT_DMM_BLOCK_SIP7 *)(hwGroupOffsetHandle[j]);
            oldBlockSize = SIZE_IN_WORDS_SIP7(usedBlock);
            totalOldBlockSize += oldBlockSize;
        }
    }
    ...
    for (j= 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
        ...
        oldBlockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(hwGroupOffsetHandle[j])/
                                                                        (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
        retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&oldBlockIndex);
        if (retVal != GT_OK)
        {
...
/*******************************************************************************
* handleMcSrcRootNodeForShrinkSip7
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_STATUS handleMcSrcRootNodeForShrinkSip7
(
...
/**
* @internal prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7 function
* @endinternal
*
...
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
...
)
{
    ...
    if (phaseFlag == PRV_CPSS_DXCH_LPM_RAM_HW_AND_POINTERS_UPDATE_PHASE1_E)
    {
        ...
        else
        {
            ...
            {
                /* build needed gon*/
                retVal = getMirrorGroupOfNodesDataAndUpdateRangesAddressSip7(oldBucketShadowPtr->bucketType,
        ...
        if (retVal != GT_OK)
        {
            ...
            if(needToFreeAllocationInCaseOfFail == GT_TRUE)
            {
                ...
                blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempHwAddrHandleToBeFreed)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                ...
                shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates +=
                    PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(tempHwAddrHandleToBeFreed);
                prvCpssDmmFreeSip7(tempHwAddrHandleToBeFreed);
            }

            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7 - GT_FAIL \n");
        }
        ...
        for (i = 0; i < shareDevListLen; i++)
        {
            ...
            if (retVal != GT_OK)
            {
                ...
                if((needToFreeAllocationInCaseOfFail == GT_TRUE)&&
                   ...
                   (tempHwAddrHandleToBeFreed != DMM_BLOCK_NOT_FOUND_SIP7) &&
                   (tempHwAddrHandleToBeFreed != DMM_MALLOC_FAIL_SIP7))
                {
                    blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempHwAddrHandleToBeFreed)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                    retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
                    if (retVal != GT_OK)
    ...
    else /* PRV_CPSS_DXCH_LPM_RAM_HW_AND_POINTERS_UPDATE_PHASE2_E */
    {
        ...
        case 0:/* this is the root */
            ...
            if(ucMcType == PRV_CPSS_DXCH_LPM_ALLOC_MC_SRC_TYPE_E)
            {
               ...
               retVal = handleMcSrcRootNodeForShrinkSip7(parentUpdateParams,
                                                     shareDevsList,
                                                     shareDevListLen,
                                                     bucketBaseAddress);
            ...
            else
            {
                ...
                retVal = prvCpssDxChLpmRamMngVrfEntryUpdateSip7(parentUpdateParams->vrId,
        ...
        case 1:
            ...
            retVal = updateHwRangeDataAndGonPtrSip7(parentUpdateParams);
            break;
        default:
...
/**
* @internal prvCpssDxChLpmRamMngShrinkPointersUpdateSip7 function
* @endinternal
*
...
* @retval GT_FAIL                  - on error.
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkPointersUpdateSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN GT_U32                                       bucketBaseAddress,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC      *oldBucketShadowPtr,
...
)
{
    ...
    if (gonIndex==0xFFFF)
    {
        ...
        protocol   = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwBucketOffsetHandle);
    }
    else
    {
        if (oldBucketShadowPtr->hwBucketOffsetHandle!=0)
        {
            ...
            parentNodeAdress = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldBucketShadowPtr->hwBucketOffsetHandle) + gonIndex;
        }
        else
        {
            parentNodeAdress = oldBucketShadowPtr->nodeMemAddr + gonIndex;
        }
        protocol    = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex]);
        offset      = 0;
    }
    ...
    if (isRoot==GT_TRUE)
    {
         ...
         retVal = prvCpssDxChLpmRamMngVrfEntryUpdateSip7(vrId,
                                                     protocol,
                                                     shadowPtr);
    }
    else
    {
       /* in all other blocks move we need to use the parentNodeAdress we got from the shadow data */
        for (i = 0; i < shareDevsListLen; i++)
        {
            ...
            retVal = prvCpssDxChLpmRamIndirectReadTableMultiEntrySip7(shareDevsList[i],
                                                 CPSS_DXCH_SIP5_TABLE_LPM_MEM_E,
                                                 parentNodeAdress,
                                                 1,
                                                 gonDataArray);
            ...
            retVal = prvCpssDxChLpmRamIndirectWriteTableMultiEntrySip7(shareDevsList[i],
                                             CPSS_DXCH_SIP5_TABLE_LPM_MEM_E,
                                             parentNodeAdress,
                                             1,
                                             gonDataArray);
    ...
    if (retVal != GT_OK)
    {
        ...
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "error in prvCpssDxChLpmRamMngShrinkPointersUpdateSip7 - GT_FAIL \n");
    }
...
/**
* @internal prvCpssDxChLpmRamMngShrinkFreeOldBucketSip7 function
* @endinternal
*
...
* @retval GT_FAIL                  - on error.
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkFreeOldBucketSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC        *shadowPtr,
    IN GT_U32                                  bankIndex,
...
)
{
        ...
        (oldBucketShadowPtr->bucketType != CPSS_DXCH_LPM_MULTIPATH_ENTRY_PTR_TYPE_E))
    {
        ...
        if (blockIndex!=bankIndex)
        {
            ...
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkFreeOldBucketSip7 - GT_FAIL \n");
        }
        if(oldHwAddrHandle==0)
        {
             CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "Error: unexpected oldHwAddrHandle=0\n");
...
/**
* @internal prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_U32                                                       bankIndex,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC                      *oldBucketShadowPtr,
...
)
{
    ...
    if(partition->allocatedBytes == 0)
    {
        ...
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 - GT_FAIL \n");
    }
    ...
    if (gonIndex==0xFFFF)
    {
        ...
        oldHwProtocol       = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwBucketOffsetHandle);
    ...
    else
    {
        ...
        if(gonIndex==0xFFFE)
        {
            ...
            oldHwProtocol       = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[0]);
        }
        else
        {
            oldHwAddrHandle     = oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex];
            oldHwProtocol       = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex]);
        }
    }
    ...
    if(copyToFromSwapArea==PRV_CPSS_DXCH_LPM_RAM_COPY_TO_FIRST_SWAP_AREA_E)
    {
        ...
        case 1:
            ...
            tempAddr = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(shadowPtr->swapMemoryAddr);
            break;
        case 2:
            ...
            tempAddr = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(shadowPtr->secondSwapMemoryAddr);
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 wronf swapAreaIndex- GT_FAIL \n");
        }
    ...
    else
    {
        ...
        tempHwAddrHandle = prvCpssDmmAllocateSip7(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                              DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * oldMemSize,
        ...
        if ((tempHwAddrHandle == DMM_BLOCK_NOT_FOUND_SIP7)||(tempHwAddrHandle == DMM_MALLOC_FAIL_SIP7))
        {
            /* should never happen since we just freed this mem when calling
               prvCpssDxChLpmRamMngShrinkBankCopyToSwapArea1AndUpdatePointersSip7*/
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 - GT_FAIL \n");
        }
        else
        {
           ...
           /*  set pending flag for future need */
            blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempHwAddrHandle)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
            retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
            if (retVal != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
        ...
        tempAddr = (PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempHwAddrHandle));
    ...
    retVal = prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7(shadowPtr,
    ...
    if (retVal!=GT_OK)
    {
       /* in case of a fail any allocations done in previous stages was already
          free in prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7*/
       CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 - GT_FAIL \n");
    }
    if(copyToFromSwapArea==PRV_CPSS_DXCH_LPM_RAM_COPY_TO_FIRST_SWAP_AREA_E)
    {
        ...
        if (gonIndex==0xFFFF)
        {
            ...
            SET_DMM_BLOCK_WAS_MOVED_SIP7(oldBucketShadowPtr->hwBucketOffsetHandle, 1);
            ...
            SET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwBucketOffsetHandle, oldHwProtocol);
            SET_DMM_BUCKET_SW_ADDRESS_SIP7(oldBucketShadowPtr->hwBucketOffsetHandle, oldBucketShadowPtr);
        }
        else
        {
            /* set in lpmFalconCalcNeededMemory to specify we are moving from compress to regular,
            ...
            {
               /* first we need to set the old block as a blocked that was moved to a new location */
                SET_DMM_BLOCK_WAS_MOVED_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[0], 1);
                ...
                SET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[0], oldHwProtocol);
                SET_DMM_BUCKET_SW_ADDRESS_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[0], oldBucketShadowPtr);
            }
            else
            {
                /* first we need to set the old block as a blocked that was moved to a new location */
                SET_DMM_BLOCK_WAS_MOVED_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex], 1);
                ...
                SET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex], oldHwProtocol);
                SET_DMM_BUCKET_SW_ADDRESS_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[gonIndex], oldBucketShadowPtr);
            }
        }
    }
    else
    ...
    if(copyToFromSwapArea==PRV_CPSS_DXCH_LPM_RAM_COPY_TO_FIRST_SWAP_AREA_E)
    {
       /* now it's ok to free this bucket's old memory */
        retVal = prvCpssDxChLpmRamMngShrinkFreeOldBucketSip7(shadowPtr,
                                                         bankIndex,
                                                         oldHwAddrHandle,
                                                         oldBucketShadowPtr);
        if (retVal!=GT_OK)
...
/*******************************************************************************
* updateParentSwapGonsAdressesSip7
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_STATUS updateParentSwapGonsAdressesSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_UPDATE_NODE_STC     *parentUpdateParams,
    IN GT_U32                                    address,
    IN GT_U32                                    gonIndex
...
/*
* @internal prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_U32                                                       bankIndex,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC                      *oldBucketShadowPtr,
...
)
{
    ...
    if(partition->allocatedBytes == 0)
    {
        /* nothing to shrink
           should never happen since this function should be called after
           checking shrink operation may be useful for defrag */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 - GT_FAIL \n");
    }
    ...
    while (GT_TRUE)
    {
        if (copyToSwapArea3 == GT_TRUE)
        {
            ...
            tempAddr = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(shadowPtr->thirdSwapMemoryAddr);

            retVal = updateParentSwapGonsAdressesSip7(parentUpdateParams,tempAddr,gonIndex);/* pointers will be updated to point to swap area */
            if (retVal!=GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(retVal,"error in prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 - updateSwapGonsAdresses - GT_FAIL \n");
            }
        }
        else
        {
            ...
            {
                /* should never happen since we just freed this mem when calling
                   prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 - GT_FAIL \n");
            }
            else
            {
            ...
            retVal = updateParentSwapGonsAdressesSip7(parentUpdateParams,tempAddr,gonIndex);/* pointers will be updated to point to new allocated memory area */
            if (retVal!=GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(retVal,"error in prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 - updateSwapGonsAdresses - GT_FAIL \n");
            }
       ...
       retVal = prvCpssDxChLpmRamMngShrinkHwWriteAndPointersUpdatePhase1Phase2Sip7(shadowPtr,
        ...
        retVal = prvCpssDxChLpmRamMngShrinkPointersUpdateSip7(shadowPtr,
                                                              tempAddr,/* third swap area or an allocated one */
                                                              oldBucketShadowPtr,
                                                              gonIndex,
                                                              needToFreeAllocationInCaseOfFail,
...
/**
* @internal prvCpssDxChLpmRamMngShrinkOneBlockSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkOneBlockSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_U32                                                       bankIndex,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC                      *oldBucketShadowPtr,
...
)
{
    ...
    if(partition->allocatedBytes == 0)
    {
        /* nothing to shrink
           should never happen since this function should be called after
           checking shrink operation may be useful for defrag */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkOneBlockSip7 - GT_FAIL \n");
    }
    ...
    else
    {
        ...
        oldHwProtocol       = GET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[oldMemGonIndex]);
    }

    /* double check that the element we want to move have a free space it will fit
       before its current location */
    if (((GT_DMM_BLOCK_SIP7 *)oldHwAddrHandle)->prevByAddr != NULL)
    {
        newLocationBlockSize =  SIZE_IN_WORDS_SIP7(((GT_DMM_BLOCK_SIP7 *)oldHwAddrHandle)->prevByAddr);
    }
    else
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkOneBlockSip7 - GT_FAIL \n");
    ...
    if((DMM_BLOCK_STATUS_SIP7(((GT_DMM_BLOCK_SIP7 *)oldHwAddrHandle)->prevByAddr) == DMM_BLOCK_FREE_SIP7) &&
       ((OFFSET_IN_WORDS_SIP7(((GT_DMM_BLOCK_SIP7 *)oldHwAddrHandle)->prevByAddr) + newLocationBlockSize) <=
         OFFSET_IN_WORDS_SIP7(oldHwAddrHandle)) &&
       (oldMemSize<=newLocationBlockSize))
    {

        retVal = prvCpssDmmAllocateByPtrSip7(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                 DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS*(OFFSET_IN_WORDS_SIP7(((GT_DMM_BLOCK_SIP7 *)oldHwAddrHandle)->prevByAddr)),
                                 DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS* oldMemSize,
                                 DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS,
                                 &tempHwAddrHandle);
        ...
        {
            /* should never happen since we just freed this mem when calling
               prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7 */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkOneBlockSip7 - GT_FAIL \n");
        }
        else
        {
        ...
        retVal = updateParentSwapGonsAdressesSip7(parentUpdateParams,tempAddr,oldMemGonIndex);/* pointers will be updated to point to new allocated memory area */
        if (retVal!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(retVal,"error in prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7 - updateSwapGonsAdresses - GT_FAIL \n");
        }
        ...
        {
            /* first we need to set the old block as a blocked that was moved to a new location */
            SET_DMM_BLOCK_WAS_MOVED_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[oldMemGonIndex], 1);

            oldBucketShadowPtr->hwGroupOffsetHandle[oldMemGonIndex] = tempHwAddrHandle;

            SET_DMM_BLOCK_PROTOCOL_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[oldMemGonIndex], oldHwProtocol);
            SET_DMM_BUCKET_SW_ADDRESS_SIP7(oldBucketShadowPtr->hwGroupOffsetHandle[oldMemGonIndex], oldBucketShadowPtr);
        }
...
/*******************************************************************************
* findBlockSizeAndIndexToMoveSip7
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_STATUS findBlockSizeAndIndexToMoveSip7
(
    IN  GT_DMM_BLOCK_SIP7                             *blockToMove,
...
)
{
    ...
    oldBucketShadowPtr = (PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC *)GET_DMM_BUCKET_SW_ADDRESS_SIP7(blockToMove); /* old bucket representation*/
    ...
    if (oldBucketShadowPtr!=0)
    {
        ...
        {
            if (PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldBucketShadowPtr->hwBucketOffsetHandle)==
                PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(blockToMove))
            {
                /* we are moving the root */
                *oldMemSizeToMovePtr         = SIZE_IN_WORDS_SIP7(blockToMove);
        ...
        if (*oldMemSizeToMovePtr==0)/* continue looking */
        {
            ...
            for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
            {
                ...
                if (PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldBucketShadowPtr->hwGroupOffsetHandle[j])==
                    PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(blockToMove))
                {
                    *oldMemSizeToMovePtr         = SIZE_IN_WORDS_SIP7(blockToMove);
        ...
        if (*oldMemSizeToMovePtr==0)
        {
            ...
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in findBlockSizeAndIndexToMoveSip7 oldMemSizeToMove==0 - GT_FAIL \n");
        }
    }
    else
    {
        /* this case can happen in case we are working on group prefix
           ...
           CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_PP_MEM,"error in findBlockSizeAndIndexToMoveSip7 - GT_OUT_OF_PP_MEM \n");
    }
...
/*******************************************************************************
* moveDataFromSwapAreaBackToTheOriginalBlockSip7
*
* DESCRIPTION:
...
* @param[in] shadowPtr                - (pointer to) the shadow information
* @param[in] bankIndex                - the index of the bank to shrink
* @param[in] neededMemoryBlocksSizes  - needed memory size that should be allocated
* @param[in] oldBucketShadowPtr       - the old SW bucket handle
...
*                                      prefix
* @param[in] parentUpdateParams       - parameters needed for parent update
* @param[in] isFatalError             - valid if neededMemoryBlocksSizes is Null- means we are in error handling.
*                                       GT_TRUE: fatal error occured
*                                       GT_FALSE: non fatal error, need to continue recover data to original state
*
*
* OUTPUTS:
...
*
*******************************************************************************/
static GT_STATUS moveDataFromSwapAreaBackToTheOriginalBlockSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_U32                                                       bankIndex,
    ...
    IN PRV_CPSS_DXCH_LPM_RAM_UPDATE_NODE_STC                        *parentUpdateParams,
    IN GT_BOOL                                                      isFatalError
)
{
    GT_STATUS retVal= GT_OK;
    GT_U32 j,i=0;
    GT_U32 oldGonSize = 0;
    GT_U32 oldGonOffset = 0;
    GT_U32 gonSizeDiff = 0;
    GT_UINTPTR      tempHwAddrHandle[2] = {0,0};
    GT_BOOL swapAreaUsedForReconstructOriginalData = GT_FALSE;
    ...
    for (i=0;i<2;i++)
    {
        ...
        {
            swapAreaUsedForReconstructOriginalData = GT_TRUE;
            retVal = prvCpssDxChLpmRamMngShrinkCopyToOrFromSwapArea1and2Sip7(shadowPtr,
                                                                    bankIndex,
            ...
            if (retVal!=GT_OK)
            {
                for (j=0;j<i;j++)
                {
                    if (tempHwAddrHandle[j] != 0)
                    {
                        prvCpssDmmFreeSip7(tempHwAddrHandle[j]);
                    }
                }
                /* should never happen since swap area is a free memory that can be used */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in moveDataFromSwapAreaBackToTheOriginalBlockSip7 - GT_FAIL \n");
            }
            if (neededMemoryBlocksSizes == NULL)
            {
                continue;
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    /* we supposing that old allocation is done . It is a case with swap usage, so new size is bigger*/
                    /* the new allocation for next swap usage should be done on the border of new size */
                    oldGonSize = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(oldHwHandleAddrArr[i]);
                    oldGonOffset = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldHwHandleAddrArr[i]);
                    if (neededMemoryBlocksSizes[oldHwHandleGonIndexArr[i]] > oldGonSize)
                    {
                        gonSizeDiff = neededMemoryBlocksSizes[oldHwHandleGonIndexArr[i]] - oldGonSize;
                    }
                    else
                    {
                        for (j=0;j<i;j++)
                        ...
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in gon sizes: new less than old - GT_FAIL \n");
                    }
                    retVal = prvCpssDmmAllocateByPtrSip7(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS* (oldGonOffset+oldGonSize),
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS* gonSizeDiff,
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS,
                                                     &tempHwAddrHandle[i]);
                    if ((tempHwAddrHandle[i] == DMM_BLOCK_NOT_FOUND_SIP7)||(tempHwAddrHandle[0] == DMM_MALLOC_FAIL_SIP7))
                    {
                        for (j=0;j<i;j++)
                        {
                            if (tempHwAddrHandle[j] != 0)
                        ...
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDmmAllocateByPtrSip7 - GT_FAIL \n");
                    }
                }
            }
        }
    }
    if ((neededMemoryBlocksSizes == NULL) && (isFatalError == GT_TRUE) )
    {
       CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"fatal error shrink failed \n");
    }
    else
    if ((neededMemoryBlocksSizes == NULL) && (isFatalError == GT_FALSE) && (swapAreaUsedForReconstructOriginalData == GT_TRUE) )
    {
        return /* do not log this error */ GT_OUT_OF_PP_MEM;
    }
    /* free fake allocations */
    for (i=0;i<2;i++)
    {
        if (tempHwAddrHandle[i] != 0)
        {
            prvCpssDmmFreeSip7(tempHwAddrHandle[i]);
        }
    }
    return retVal;
}
...
/**
* @internal prvCpssDxChLpmRamMngShrinkBankSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkBankSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_U32                                                       bankIndex,
    IN GT_U32                                                       neededMemoryBlocksSizes[],
...
)
{
    ...
    GT_DMM_BLOCK_SIP7        *firstFreeBlock;
    GT_DMM_BLOCK_SIP7        *firstOccupiedBlockAfterFirstFreeBlock;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        {
            oldBucketShadowPtr[i] = (PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC *)GET_DMM_BUCKET_SW_ADDRESS_SIP7(oldHwHandleAddrArr[i]);/* old bucket representation*/
            if(oldHwHandleGonIndexArr[i]==0xFFFF)/* set in lpmFalconCalcNeededMemory to specify the old  block is of a SRC root*/
            {
                oldMemSizeArr[i]         = SIZE_IN_WORDS_SIP7(oldBucketShadowPtr[i]->hwBucketOffsetHandle)
                                                                    /DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS;
            }
            else
            {
                ...
                if(oldHwHandleGonIndexArr[i]==0xFFFE)
                {
                    oldMemSizeArr[i]         = SIZE_IN_WORDS_SIP7(oldBucketShadowPtr[i]->hwGroupOffsetHandle[0])
                                                                    /DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS;
                }
                else
                {
                    oldMemSizeArr[i]         = SIZE_IN_WORDS_SIP7(oldBucketShadowPtr[i]->hwGroupOffsetHandle[oldHwHandleGonIndexArr[i]])
                                                                    /DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS;
                }
            }
    ...
    if(partition->allocatedBytes == 0)
    {
        /* nothing to shrink
           should never happen since this function should be called after
           checking shrink operation may be useful for defrag */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkBankSip7 - GT_FAIL \n");
    }
    ...
    if((firstFreeBlock == NULL)||(DMM_BLOCK_STATUS_SIP7(firstFreeBlock)!=DMM_BLOCK_FREE_SIP7))
    {
         /* nothing to shrink
            should never happen since this function should be called after
            checking shrink operation may be useful for defrag */
       ...
       retVal = moveDataFromSwapAreaBackToTheOriginalBlockSip7(shadowPtr,
                                                       bankIndex,
                                                       NULL,
                                                       oldBucketShadowPtr,
                                                       oldMemSizeArr,
        ...
        if (retVal!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(retVal,"error in moveDataFromSwapAreaBackToTheOriginalBlockSip7 - GT_FAIL \n");
        }
        else
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkBankSip7 - GT_FAIL \n");
    ...
    while(GT_TRUE)
    {
        ...
        if (newBucketSize<=SIZE_IN_WORDS_SIP7(firstFreeBlock))
        {
        ...
        if (SIZE_IN_WORDS_SIP7(firstOccupiedBlockAfterFirstFreeBlock)<=SIZE_IN_WORDS_SIP7(firstFreeBlock))
        {
            retVal = findBlockSizeAndIndexToMoveSip7(firstOccupiedBlockAfterFirstFreeBlock,&oldMemSizeToMove,&oldMemGonIndexToMove);
            if (retVal!=GT_OK)
            {
                retVal2 = retVal;
                /* this case can happen in case we are working on group prefix
                ...
                if (retVal2==GT_OUT_OF_PP_MEM)
                {
                    ...
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(retVal2,"error in prvCpssDxChLpmRamMngShrinkBankSip7 call to findGonSizeAndIndexToMove - GT_FAIL \n");
                    }
                }
                else
                {
            ...
            oldBucketShadowToMovePtr = (PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC *)GET_DMM_BUCKET_SW_ADDRESS_SIP7(firstOccupiedBlockAfterFirstFreeBlock);
            retVal = prvCpssDxChLpmRamMngShrinkOneBlockSip7(shadowPtr,
                                                          bankIndex,
        ...
        else
        {
            ...
            oldBucketShadowToMovePtr = (PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC *)GET_DMM_BUCKET_SW_ADDRESS_SIP7(firstOccupiedBlockAfterFirstFreeBlock);
            retVal = prvCpssDxChLpmRamMngShrinkOneBlockUsingSwapArea3Sip7(shadowPtr,
                                                                          bankIndex,
                                                                          oldBucketShadowToMovePtr,
                                                                          oldMemSizeToMove,
                                                                          oldMemGonIndexToMove,
...
/**
* @internal prvCpssDxChLpmRamMngShrinkAllGonBanksSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkAllGonBanksSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                     *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_NEEDED_MEM_DATA_STC            *neededMemoryBlocksPtr,
                   ...
                   if (tempNewTotalSizeNeeded<=newTotalSizeNeeded)
                   {
                           ...
                           (neededMemoryBlocksPtr->neededMemoryBlocksOldHandleAddr[k]!=0))
                       {
                           ...
                           else
                           {
                               ...
                               CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngShrinkAllGonBanksSip7 "
                                                                     "error can not have more than 2 old hanle using swap area in shrink operation \n");
                           }
                       }
                   }
           ...
           /* now go and find if we have a gonIndex that is marked to use swapArea in add operation,
            ...
            for (k= 0; k < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; k++)
            {
                   ...
                   (neededMemoryBlocksPtr->neededMemoryBlocksOldHandleGonIndex[k]==0xFFFE))
                {
                    ...
                    oldBlockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksPtr->neededMemoryBlocksOldHandleAddr[k]) /
                                                                                    (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                    rc = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&oldBlockIndex);
                    if (rc != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
                    }
                    if (oldBlockIndex==neededMemoryBlocksPtr->bankIndexForShrinkArr[j])
                    {
                        if(tempIndex<2)
                        {
                           ...
                           /* no need to keep value in oldHandleAddrUsedForAddArr, since it will not be used in
                              prvCpssDxChLpmRamMngShrinkBankSip7 if swapUsefullUsedForShrinkArr=false
                              we will use the value of
                              neededMemoryBlocksPtr->neededMemoryBlocksOldShadowBucket[k]->hwGroupOffsetHandle[0];
                              to update the pointer, since this is update inside the prvCpssDxChLpmRamMngShrinkBankSip7
            ...
            rc = prvCpssDxChLpmRamMngShrinkBankSip7(shadowPtr,
                                                    neededMemoryBlocksPtr->bankIndexForShrinkArr[j],
                                                    neededMemoryBlocksPtr->neededMemoryBlocksSizes, /* array of 6 elements*/
                                                    ...
                                                    newTotalSizeNeeded*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,/* the total size needed from a specific bank for all GONs in this bank */
                                                    swapUsefullUsedForShrinkArr, /* array of 2 elements, can hold up to 2 flags for use of swap area */
                                                    parentUpdateParams);

            if (rc!=GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"error in prvCpssDxChLpmRamMngShrinkAllGonBanksSip7 \n");
            }
            else
            {
...
/**
* @internal prvCpssDxChLpmRamMngShrinkAllGonsRegToCompressBankSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkAllGonsRegToCompressBankSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC                             *shadowPtr,
    IN GT_UINTPTR                                                   *hwGroupOffsetHandle,
    IN GT_U32                                                       bankIndex,
...
)
{
    ...
    GT_UINTPTR      oldHwHandleAddrArr[2]={0};    /* empty arrays to be passed to prvCpssDxChLpmRamMngShrinkBankSip7 - no use of swap area in this case */
    GT_U32          oldHwHandleGonIndexArr[2]={0};/* empty arrays to be passed to prvCpssDxChLpmRamMngShrinkBankSip7 - no use of swap area in this case */
    GT_BOOL         swapUsedForShrinkArr[2]={0};  /* empty arrays to be passed to prvCpssDxChLpmRamMngShrinkBankSip7 - no use of swap area in this case */
    ...
    GT_U32          memoryNeededFromEachBlock[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS]={0};
    GT_U32          swapOffset;/* offset of swap area1 to be used */
    GT_U32          hwGonDataArr[PRV_CPSS_DXCH_LPM_RAM_SIP7_MAX_GROUP_OF_NODES_SIZE_IN_WORDS_CNS] = {0};
    ...
    if (useSwapArea==GT_TRUE)
    {
        ...
        retVal = updateSwapAreaSip7(bvLineIndex,hwGroupOffsetHandle,0xffffffff,PRV_CPSS_DXCH_LPM_RAM_ALL_GONS_SWAP_INDEX_CNS,
        ...
        for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
        {
            if ((tempOldHwAddrHandle[j] != DMM_BLOCK_NOT_FOUND_SIP7) && (tempOldHwAddrHandle[j] != DMM_MALLOC_FAIL_SIP7))
            {
                /* save old block index to be used latter */
                tempOldHwAddrMemPool[j] = DMM_GET_PARTITION_SIP7(tempOldHwAddrHandle[j]);
                tempOldHwAddrBlockSize[j]=PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(tempOldHwAddrHandle[j]);

                /*  set pending flag for future need */
                blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempOldHwAddrHandle[j])/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                ...
                shadowPtr->pendingBlockToUpdateArr[blockIndex].updateDec=GT_TRUE;
                shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates +=
                PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(tempOldHwAddrHandle[j]);

                memoryNeededFromEachBlock[blockIndex]+=tempOldHwAddrBlockSize[j];
                prvCpssDmmFreeSip7(tempOldHwAddrHandle[j]);
            }
            else
            {
        ...
        if (newBucketSize>(memoryNeededFromEachBlock[bankIndex]*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS))
        {
            /* shrink the bank - no use of swap area1 or 2 */
            retVal = prvCpssDxChLpmRamMngShrinkBankSip7(shadowPtr,
                                                bankIndex,/* bank index */
                                                NULL,     /* no need to use this param since no use of swap for shrink */
        ...
        else
        {
            ...
            retVal = prvCpssDxChLpmRamMngShrinkBankSip7(shadowPtr,
                                                    ...
                                                    oldHwHandleAddrArr,/* no use of old mem */
                                                    oldHwHandleGonIndexArr,/* no use of gon index */
                                                    memoryNeededFromEachBlock[bankIndex]*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,/* the total size needed from a specific bank for all GONs in this bank */
                                                    swapUsedForShrinkArr, /* no use of swap */
                                                    parentUpdateParams);
        }
        ...
        for (k=0;k<shadowPtr->numOfLpmMemories;k++)
        {
            if (memoryNeededFromEachBlock[k]!=0)
            {
                /* if the size needed for the new compress is bigger then the size used currently by
                   the regular gon in the specific bank, we will shrink according to the bigger value */
                if ((k==bankIndex)&&
                    (newBucketSize>(memoryNeededFromEachBlock[k]*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS)))
                {
                    /* shrink the bank - no use of swap area1 or 2 */
                    retVal = prvCpssDxChLpmRamMngShrinkBankSip7(shadowPtr,
                                                        k,/* bank index */
                ...
                else
                {
                    ...
                    retVal = prvCpssDxChLpmRamMngShrinkBankSip7(shadowPtr,
                                                            ...
                                                            oldHwHandleAddrArr,/* no use of old mem */
                                                            oldHwHandleGonIndexArr,/* no use of gon index */
                                                            memoryNeededFromEachBlock[k]*PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_WORDS_CNS,/* the total size needed from a specific bank for all GONs in this bank */
                                                            swapUsedForShrinkArr, /* no use of swap */
                                                            parentUpdateParams);
                }
    ...
    if (useSwapArea==GT_TRUE)
    {
         ...
         for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
         {
            ...
            if ((tempOldHwAddrHandle[j] != DMM_BLOCK_NOT_FOUND_SIP7) && (tempOldHwAddrHandle[j] != DMM_MALLOC_FAIL_SIP7))
            {
                tempHwAddrHandle[j] = prvCpssDmmAllocateSip7(tempOldHwAddrMemPool[j],
                                                         DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * tempOldHwAddrBlockSize[j],
                                                         DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS);
                ...
                else
                {
                    ...
                    parentUpdateParams->swapGonsAdresses[j] = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(tempHwAddrHandle[j]);
                }
            }
            else
            {
...
/**
* @internal prvCpssDxChLpmRamMngShrinkCheckIfSwapAreaIsUsefulSip7 function
* @endinternal
*
...
*
*/
GT_VOID prvCpssDxChLpmRamMngShrinkCheckIfSwapAreaIsUsefulSip7
(
...
)
{
    ...
    oldBlockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksSizesPtr->neededMemoryBlocksOldHandleAddr[bvLineIndex])/
                                                                        (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
    retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&oldBlockIndex);
    if (retVal != GT_OK)
    {
    ...
    while((tempNextMemInfoForSwapUsePtr!= NULL)&&(neededMemoryBlocksPtr[levelIndex].neededMemoryBlocks[bvLineIndex]==DMM_BLOCK_NOT_FOUND_SIP7))
    {
        ...
        if (tempNextMemInfoForSwapUsePtr->ramIndex==oldBlockIndex)
        {
                ...
                (neededMemoryBlocksSizesPtr->regularNode==GT_TRUE)))
            {
                 sizeOfOptionalUsedAndFreeBlock =
                   prvCpssDmmCheckResizeAvailableWithSameMemorySip7(neededMemoryBlocksSizesPtr->neededMemoryBlocksOldHandleAddr[bvLineIndex],
...
/**
* @internal prvCpssDxChLpmRamMngShrinkAllocRegularGonCheckSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngShrinkAllocRegularGonCheckSip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmEngineMemPtrPtr,
    IN    PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
...
)
{
    ...
    sizeOfOptionalUsedAndFreeBlock =
      prvCpssDmmCheckResizeAvailableWithSameMemorySip7(neededMemoryBlocksPtr[levelIndex].neededMemoryBlocksOldHandleAddr[bvLineIndex],
   ...
   else
   {
       ...
       {
           neededMemoryBlocksPtr[levelIndex].neededMemoryBlocks[bvLineIndex] = DMM_BLOCK_NOT_FOUND_SIP7;
       ...
       *thirdSwapUsedPtr  = thirdSwapUsed;
       ...
       if ((neededMemoryBlocksPtr[levelIndex].neededMemoryBlocks[bvLineIndex]==DMM_BLOCK_NOT_FOUND_SIP7)&&(tempNewStructsMemPool!=0))
       {
          /* in case we have 2 gons that wanted to use the swap areas but then they couldn't
           ...
           neededMemoryBlocksPtr[levelIndex].neededMemoryBlocks[bvLineIndex] =
                                       prvCpssDmmAllocateSip7(tempNewStructsMemPool,
                                                          DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * (neededMemoryBlocksPtr[levelIndex].neededMemoryBlocksSizes[bvLineIndex]),
                                                          DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS);
       }
       ...
       if (neededMemoryBlocksPtr[levelIndex].neededMemoryBlocks[bvLineIndex]==DMM_BLOCK_NOT_FOUND_SIP7)
       {
...
/**
* @internal prvCpssDxChLpmRamRetryAllocNeededMemoryAfterShrinkSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
static GT_STATUS prvCpssDxChLpmRamMngRetryAllocNeededMemoryAfterShrinkSip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmEngineMemPtrPtr,
    IN    PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
...
)
{
    ...
    GT_DMM_BLOCK_SIP7    *nextByAddr,*prevByAddr;
    ...
    for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
            ...
            (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j] != 0))
        {
            ...
            if ((neededMemoryBlocksPtr->swapUsedForShrinkArr[j] == GT_TRUE)||
                (neededMemoryBlocksPtr->neededMemoryBlocksSwapUsedForAdd[j] == GT_TRUE))
            {
                ...
                if (firstSwapUsed == GT_FALSE)
                {
                    /* in case swap is used for add and not for shrink nothing to do since the data in
                       neededMemoryBlocksPtr is updated correctly with the swap parameters */
                    if(neededMemoryBlocksPtr->swapUsedForShrinkArr[j] == GT_TRUE)
                    {
                    ...
                    if (newSize[0]>=PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(oldHandleToCheck[0]))
                    {
                        numberOfLinesToMark = newSize[0] - PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(oldHandleToCheck[0]);

                        nextByAddr = ((GT_DMM_BLOCK_SIP7 *)oldHandleToCheck[0])->nextByAddr;
                        prevByAddr = ((GT_DMM_BLOCK_SIP7 *)oldHandleToCheck[0])->prevByAddr;
                        if (nextByAddr != NULL)
                        {
                            if (DMM_BLOCK_STATUS_SIP7(nextByAddr) == DMM_BLOCK_FREE_SIP7)
                            {
                                sizeOfMarkedArea=PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(nextByAddr);
                        ...
                        if (prevByAddr != NULL)
                        {
                            if (DMM_BLOCK_STATUS_SIP7(prevByAddr)==DMM_BLOCK_FREE_SIP7)
                            {
                                sizeOfMarkedArea=PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(prevByAddr);
                        ...
                        else if (sizeOfMarkedArea0 == 0)
                        {
                            ...
                            lpmGroupLineOffset = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(prevByAddr);
                        }
                        else if (sizeOfMarkedArea1 == 0)
                        {
                            /* alocation would be done by nextByAddr */
                            lpmGroupLineOffset = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(nextByAddr);
                        }
                        else
                        {
                        ...
                        /* do fake allocation */
                        bankIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldHandleToCheck[0]) /
                                                                                    (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                        retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&bankIndex);
                        if (retVal != GT_OK)
                        {
                            /* free fake allocations */
                            for (i=0;i<2;i++)
                        ...
                        retVal = prvCpssDmmAllocateByPtrSip7(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                                         ...
                                                         DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS* numberOfLinesToMark,
                                                         DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS,
                                                         &tempHwAddrHandle[0]);
                        if ((tempHwAddrHandle[0] == DMM_BLOCK_NOT_FOUND_SIP7)||(tempHwAddrHandle[0] == DMM_MALLOC_FAIL_SIP7))
                        {
                            /* free fake allocations */
                            for (i=0;i<2;i++)
                            {
                ...
                else
                {
                    if (secondSwapUsed == GT_FALSE)
                    {
                        ...
                        if (newSize[1]>=PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(oldHandleToCheck[1]))
                        {
                            numberOfLinesToMark = newSize[1] - PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(oldHandleToCheck[1]);

                            nextByAddr = ((GT_DMM_BLOCK_SIP7 *)oldHandleToCheck[1])->nextByAddr;
                            prevByAddr = ((GT_DMM_BLOCK_SIP7 *)oldHandleToCheck[1])->prevByAddr;
                            if (nextByAddr != NULL)
                            {
                                if (DMM_BLOCK_STATUS_SIP7(nextByAddr)==DMM_BLOCK_FREE_SIP7)
                                {
                            ...
                            else
                            {
                               ...
                               /* do fake allocation */
                            bankIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(oldHandleToCheck[1]) /
                                                                                           (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                            retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&bankIndex);
                            if (retVal != GT_OK)
                            {
                            ...
                            retVal = prvCpssDmmAllocateByPtrSip7(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                                             ...
                                                             DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS,
                                                             &tempHwAddrHandle[1]);
                            if ((tempHwAddrHandle[1] == DMM_BLOCK_NOT_FOUND_SIP7)||(tempHwAddrHandle[1] == DMM_MALLOC_FAIL_SIP7))
                            {
                                /* free fake allocations */
                                for (i=0;i<2;i++)
                                {
                    ...
                    else
                    {
                        ...
                        if(neededMemoryBlocksPtr->neededMemoryBlocks[j] == DMM_MALLOC_FAIL_SIP7)
                ...
                (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j] != 0))
            {
                ...
                else
                {
                    ...
                    neededMemoryBlocksPtr->neededMemoryBlocks[j] =
                                prvCpssDmmAllocateSip7(shadowPtr->lpmRamStructsMemPoolPtr[neededMemoryBlocksPtr->bankIndexForShrinkArr[j]],
                                    DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j]),
                                    DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS);
                }
                ...
                if (neededMemoryBlocksPtr->swapUsedForShrinkArr[j] == GT_FALSE)
                {
                    ...
                    /* set pending flag for future need */
                    blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksPtr->neededMemoryBlocks[j]) /
                                        (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                    retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
                    if (retVal != GT_OK)
                    {
                    ...
                    shadowPtr->pendingBlockToUpdateArr[blockIndex].updateInc = GT_TRUE;
                    shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfIncUpdates +=
                        PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(neededMemoryBlocksPtr->neededMemoryBlocks[j]);
                }
            }
        }
    }
...
/**
* @internal prvCpssDxChLpmRamMngRetryAllocNeededMemoryAfterMergeSip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
static GT_STATUS prvCpssDxChLpmRamMngRetryAllocNeededMemoryAfterMergeSip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmEngineMemPtrPtr,
    IN    PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
    ...
    for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
    {
            ...
            (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j] != 0))
        {
            if (neededMemoryBlocksPtr->mergeOperationUsefulForDefragArr[j] == GT_TRUE)
            {
                /* try to allocate again the memory needed - operation should pass */
                neededMemoryBlocksPtr->neededMemoryBlocks[j] =
                            prvCpssDmmAllocateSip7(shadowPtr->lpmRamStructsMemPoolPtr[neededMemoryBlocksPtr->bankIndexForMergeArr[j]],
                                DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * (neededMemoryBlocksPtr->neededMemoryBlocksSizes[j]),
                                DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS);
            }
            ...
            /* set pending flag for future need */
            blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksPtr->neededMemoryBlocks[j])/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
            retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
            if (retVal != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
...
/**
* @internal prvCpssDxChLpmRamMngDefragCheckAndApplySip7 function
* @endinternal
*
...
* @retval GT_NOT_FOUND             - an available block was not found
*/
GT_STATUS prvCpssDxChLpmRamMngDefragCheckAndApplySip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmEngineMemPtrPtr,
    IN    PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
...
)
{
    ...
    GT_U32      freeSpaceTakenFromBlockIndexArray[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS]={0}; /* in words not in lines */
    ...
    PRV_CPSS_SIP6_LPM_MERGE_RESERVATIONS_STC   mergeReservationsArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS];
    GT_BOOL                                    isMarkedForShrinkArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS];
    ...
    for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
    {
        ...
        if(freeBlockCanBeFoundForAllNeededBlocks==GT_TRUE)
        {
            ...
            for (j= 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
            {
                if(((neededMemoryBlocksInfo[i].neededMemoryBlocks[j] == DMM_BLOCK_NOT_FOUND_SIP7)||
                    (neededMemoryBlocksInfo[i].neededMemoryBlocks[j] == DMM_MALLOC_FAIL_SIP7)||
            ...
            if (checkShrink == GT_TRUE)
            {
                ...
                retVal = prvCpssDxChLpmRamMngCheckIfShrinkOperationUsefulForDefragSip7(shadowPtr,
                ...
                if ((freeBlockCanBeFound == GT_FALSE) && (neededMemoryBlocksInfo[i].mergeOperationUsefulForDefragGlobalFlag == GT_FALSE))
                {
                    if (shadowPtr->defragSip6MergeEnable == GT_TRUE)
                    {
    ...
    for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
    {
        for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
        {
            if (shadowPtr->neededMemoryBlocksInfo[i].neededMemoryBlocksSwapUsedForAdd[j] == GT_TRUE)
            {
                ...
                oldBlockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(shadowPtr->neededMemoryBlocksInfo[i].neededMemoryBlocksOldHandleAddr[j])/
                                                                    (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&oldBlockIndex);
                if (retVal != GT_OK)
                {
                ...
                if (oldBlockIndex >= PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS)
                {
    ...
    for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
    {
            ...
            (neededMemoryBlocksInfo[i].mergeOperationUsefulForDefragGlobalFlag == GT_TRUE))
        {
            ...
            retVal = prvCpssDxChLpmRamMngCheckIfMergeBankOperationUsefulForDefragSip7(shadowPtr,
                                                                                      lpmEngineMemPtrPtr,
                                                                                      protocol,
                                                                                      i,
            ...
            /* Recheck for merge considering unassigned banks */
            if (mergeBankCanBeFound == GT_FALSE)
            {
                mergeWasCheckedSuccessfully = GT_FALSE;
                neededMemoryBlocksInfo[i].mergeOperationUsefulForDefragGlobalFlag = GT_FALSE;

                retVal = prvCpssDxChLpmRamMngCheckIfUnassignedBankUsefulForMergeSip7(shadowPtr,
                                                                                     lpmEngineMemPtrPtr,
                                                                                     protocol,
                                                                                     i,
                                                                                     isMarkedForShrinkArr,
       ...
       (shrinkWasCheckedSuccessfully==GT_TRUE))/* if we get here it means all needed memory can be found using shrink */
    {
        ...
        for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
        {
            if (neededMemoryBlocksInfo[i].shrinkOperationUsefulForDefragGlobalFlag==GT_TRUE)
            {
                ...
                for (j = 0; j < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; j++)
                {
                        ...
                        (neededMemoryBlocksInfo[i].neededMemoryBlocksSizes[j]!=0))
                    {
                        ...
                        if (neededMemoryBlocksInfo[i].neededMemoryBlocksSwapUsedForAdd[j] == GT_FALSE)
                        {
                            ...
                            blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(neededMemoryBlocksInfo[i].neededMemoryBlocks[j])/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                            retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
                            if (retVal != GT_OK)
                            {
                                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
                            ...
                            if (freeSpaceTakenFromBlockIndexArray[blockIndex]!=0)
                            {
                                ...
                                shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates +=
                                        PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(neededMemoryBlocksInfo[i].neededMemoryBlocks[j]);

                                prvCpssDmmFreeSip7(neededMemoryBlocksInfo[i].neededMemoryBlocks[j]);

                                /* reset value */
                                neededMemoryBlocksInfo[i].neededMemoryBlocks[j]=DMM_BLOCK_NOT_FOUND_SIP7;
                            }
                        }
                    }
                }
                ...
                retVal = prvCpssDxChLpmRamMngShrinkAllGonBanksSip7(shadowPtr,
                                                            &neededMemoryBlocksInfo[i],
                                                            freeSpaceTakenFromBlockIndexArray,
                                                            parentUpdateParams);
                if (retVal != GT_OK)
                ...
                retVal = prvCpssDxChLpmRamMngRetryAllocNeededMemoryAfterShrinkSip7(shadowPtr,
                                                            lpmEngineMemPtrPtr,
                                                            protocol,
                                                            &neededMemoryBlocksInfo[i],
                                                            firstSwapUsedPtr,
        ...
        (mergeWasCheckedSuccessfully == GT_TRUE))
    {
        for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
        {
            ...
            /* Merge banks */
            retVal = prvCpssDxChLpmRamMngMergeAllGonBanksSip7(shadowPtr,
                                                              lpmEngineMemPtrPtr,
                                                              protocol,
                                                              &neededMemoryBlocksInfo[i],
                                                              isMarkedForShrinkArr,
            ...
            /* banks were shrinked try to allocate again */
            retVal = prvCpssDxChLpmRamMngRetryAllocNeededMemoryAfterMergeSip7(shadowPtr,
                                                                              lpmEngineMemPtrPtr,
                                                                              protocol,
                                                                              &neededMemoryBlocksInfo[i]);
            if (retVal != GT_OK)
...
/**
* @internal prvCpssDxChLpmRamMngCheckIfUnassignedBankUsefulForMergeSip7 function
* @endinternal
*
* @brief   Check if merge bank operation may help for defrag by using unassigned banks
*
* @param[in] shadowPtr                - (pointer to) the shadow information
...
*/
GT_STATUS prvCpssDxChLpmRamMngCheckIfUnassignedBankUsefulForMergeSip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC            *shadowPtr,
...
)
{
    ...
    GT_U32  octetInd = 0;                    /* index to run over all octets */
    GT_U32  sumOfSourceLines = 0;            /* calculation of total lines that will have relocated -
    ...
    GT_U32      unassignedBanksIndexArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS] = {0};
    GT_U32      unassignedBanksFreeLinesArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS] = {0};
    GT_U32      unassignedBanksNum = 0;
    GT_U32      freeLinesInBank = 0;
    GT_BOOL     foundBankToBeFreed = GT_FALSE;
    GT_U32      bankToBeAssigned = 0;
    GT_U32      potentialBankToMove = 0;
    GT_U32      potentialBankFreeLines = 0;
    GT_U32      octetsToBlockBitmap = 0;
    GT_U32      potentialOctetToMove = 0;
    ...
    /* Calculate number of unassigned banks and create a linked list */
    unassignedBanksNum = 0;
    for (i=0; i<shadowPtr->numOfLpmMemories; i++)
    {
        if (PRV_CPSS_DXCH_LPM_RAM_IS_BLOCK_USED_BY_PROTOCOL_GET_MAC(shadowPtr,protocol,i) == GT_FALSE)
        {
            freeLinesInBank = shadowPtr->lpmRamBlocksSizeArrayPtr[i] -
                              (shadowPtr->protocolCountersPerBlockArr[i].sumOfIpv4Counters +
                               shadowPtr->protocolCountersPerBlockArr[i].sumOfIpv6Counters +
                               shadowPtr->protocolCountersPerBlockArr[i].sumOfFcoeCounters);

            if (freeLinesInBank > 0) {
                unassignedBanksIndexArr[unassignedBanksNum] = i;
                unassignedBanksFreeLinesArr[unassignedBanksNum] = freeLinesInBank;
                unassignedBanksNum++;
            }
        }
    }
    if (unassignedBanksNum == 0)
    {
        /* Can't find any unassigned banks */
        *mergeBankCanBeFoundPtr = GT_FALSE;
        return GT_OK;
    }

    for (i=0; i<unassignedBanksNum; i++)
    {
        foundBankToBeFreed = GT_FALSE;
        potentialBankFreeLines = 0;
        potentialOctetToMove = maxNumOfOctets + 1;

        /* start with 1st bank. Do not move bank 0 */
        for (j=1; j<shadowPtr->numOfLpmMemories; j++)
        {
            /* Do not check unassigned octets*/
            if (unassignedBanksIndexArr[i] == j)
            {
                continue;
            }
            ...
            {
            case PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E:
                sumOfSourceLines = (shadowPtr->protocolCountersPerBlockArr[j].sumOfIpv4Counters+
                                    shadowPtr->pendingBlockToUpdateArr[j].numOfIncUpdates-
                                    shadowPtr->pendingBlockToUpdateArr[j].numOfDecUpdates);
                break;
            case PRV_CPSS_DXCH_LPM_PROTOCOL_IPV6_E:
                sumOfSourceLines = (shadowPtr->protocolCountersPerBlockArr[j].sumOfIpv6Counters+
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfIncUpdates-
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfDecUpdates);
                break;
            case PRV_CPSS_DXCH_LPM_PROTOCOL_FCOE_E:
                sumOfSourceLines = (shadowPtr->protocolCountersPerBlockArr[j].sumOfFcoeCounters+
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfIncUpdates-
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfDecUpdates);
                break;
            default:
            ...
            if (sumOfSourceLines <= unassignedBanksFreeLinesArr[i])
            {
                freeLinesInBank = sumOfSourceLines +
                                shadowPtr->lpmRamBlocksSizeArrayPtr[j]-
                                (shadowPtr->protocolCountersPerBlockArr[j].sumOfIpv4Counters+
                                 shadowPtr->protocolCountersPerBlockArr[j].sumOfIpv6Counters+
                                 shadowPtr->protocolCountersPerBlockArr[j].sumOfFcoeCounters+
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfIncUpdates-
                                 shadowPtr->pendingBlockToUpdateArr[j].numOfDecUpdates);

                /* Get octet assigned to the potential bank to be freed */
                octetsToBlockBitmap = shadowPtr->lpmRamOctetsToBlockMappingPtr[j].octetsToBlockMappingBitmap[protocol];
                octetInd = 0;
                while (!(octetsToBlockBitmap & 0x1))
                {
                    octetsToBlockBitmap >>= 1;
                    octetInd++;
                    if (octetInd > maxNumOfOctets)
                    {
                        break;
                    }
                }

                /* If there are more than two banks that can be moved, Preference order is
                   1. Freeing the bank should provide greater number of free lines
                   2. If both banks provide same number of free lines, free bank with lesser octet number */
                if ((freeLinesInBank > potentialBankFreeLines) ||
                    ((freeLinesInBank == potentialBankFreeLines) &&
                     (octetInd > 0) && (octetInd <= maxNumOfOctets) && (octetInd < potentialOctetToMove)))
                {
                    potentialOctetToMove = octetInd;
                    potentialBankToMove = j;
                    foundBankToBeFreed = GT_TRUE;
                    potentialBankFreeLines = freeLinesInBank;
                    bankToBeAssigned = unassignedBanksIndexArr[i];
                }
            }
        }
        if(foundBankToBeFreed == GT_TRUE)
        {
            break;
        }
    }

    if (foundBankToBeFreed == GT_FALSE)
    {
        *mergeBankCanBeFoundPtr = GT_FALSE;
        return GT_OK;
    }
    ...
    /* Get octet assigned to the potential bank to be freed */
    octetsToBlockBitmap = shadowPtr->lpmRamOctetsToBlockMappingPtr[potentialBankToMove].octetsToBlockMappingBitmap[protocol];
    octetInd = 0;
    while (!(octetsToBlockBitmap & 0x1))
    {
        octetsToBlockBitmap >>= 1;
    ...
    /* Bind the unassigned Bank to the octet */
    tempLpmMemInfoPtr = lpmMemInfoPtrPtr[octetInd];

    /* if this is the first element in the list, just need to bind */
    if(tempLpmMemInfoPtr->structsMemPool==0)
    {
        tempLpmMemInfoPtr->ramIndex = bankToBeAssigned;
        tempLpmMemInfoPtr->structsBase=0;
        /* bind the new block */
        tempLpmMemInfoPtr->structsMemPool=shadowPtr->lpmRamStructsMemPoolPtr[bankToBeAssigned];
        tempLpmMemInfoPtr->nextMemInfoPtr=NULL;
        /* first element in linked list of the blocks that
           need to be freed in case of an error is the root */
        firstMemInfoInListToFreePtr = tempLpmMemInfoPtr;
    }
    else
    {
         /* get to the last pointer in the list */
        while(tempLpmMemInfoPtr->nextMemInfoPtr!= NULL)
        {
        ...
        potentialNewBlockPtr->ramIndex = bankToBeAssigned;
        potentialNewBlockPtr->structsBase = 0;
        potentialNewBlockPtr->structsMemPool = 0;
        potentialNewBlockPtr->nextMemInfoPtr = NULL;

        /* bind the new block */
        potentialNewBlockPtr->structsMemPool=shadowPtr->lpmRamStructsMemPoolPtr[bankToBeAssigned];
        tempLpmMemInfoPtr->nextMemInfoPtr = potentialNewBlockPtr;

        /* first element in linked list of the blocks that
           need to be freed in case of an error is the father
           of the new block added to the list
          (father of potentialNewBlockPtr is tempLpmMemInfoPtr) */
        firstMemInfoInListToFreePtr = tempLpmMemInfoPtr;
    }

    /* mark the block as used */
    shadowPtr->lpmRamOctetsToBlockMappingPtr[bankToBeAssigned].isBlockUsed=GT_TRUE;
    /*  set the block to be used by the specific octet and protocol*/
    PRV_CPSS_DXCH_LPM_RAM_OCTET_TO_BLOCK_MAPPING_SET_MAC(shadowPtr,protocol,
                                                         octetInd,
                                                         bankToBeAssigned);
    ...
    /* We only save the first element allocated or bound per octet */
    if ((allNewNextMemInfoAllocatedPerOctetArrayPtr[octetInd]==NULL)||
        (allNewNextMemInfoAllocatedPerOctetArrayPtr[octetInd]->structsMemPool==0))
    {
        /* keep the head of the list we need to free in case of an error -
        first element is the father of the first element that should be freed */
        allNewNextMemInfoAllocatedPerOctetArrayPtr[octetInd] = firstMemInfoInListToFreePtr;
    }
...
* @retval GT_NOT_FOUND             - a potential bank for merge was not found
*/
GT_STATUS prvCpssDxChLpmRamMngCheckIfMergeBankOperationUsefulForDefragSip7
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC            *shadowPtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC          **lpmMemInfoPtrPtr,
    IN    PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT        protocol,
    ...
    for (i = 0; i < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; i++)
    {
        if ((neededMemoryBlocksPtr->neededMemoryBlocksSizes[i] == 0) ||
            ((neededMemoryBlocksPtr->neededMemoryBlocksSizes[i] != 0) &&
             (neededMemoryBlocksPtr->neededMemoryBlocks[i] != DMM_BLOCK_NOT_FOUND_SIP7) &&
             (neededMemoryBlocksPtr->neededMemoryBlocks[i] != DMM_MALLOC_FAIL_SIP7) &&
        ...
        for (octetInd = 0; octetInd < maxNumOfOctets; octetInd++)
        {
            ...
            while (tempLpmMemInfoPtr != NULL)
            {
                ...
                retVal = prvCpssDmmAllPartitionIsUpdatedInHwSip7(tempLpmMemInfoPtr->structsMemPool,&allUpdatedInHwFlag);
                if (retVal!=GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(retVal,"error in prvCpssDmmAllPartitionIsUpdatedInHwSip7 - GT_FAIL \n");
                }
                ...
                /* total lines that will have relocated - only specific protocol buckets
                   ...
                   at this point the counters we have in protocolCountersPerBlockArr do
                   not show the correct picture.
                   In some cases prvCpssDxChLpmRamUpdateBlockUsageCountersSip7 was not
                   called yet so we need to take into consideration also the pending counters */
                switch (protocol)
                {
                case PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E:
                ...
                numFreeLines = shadowPtr->lpmRamBlocksSizeArrayPtr[tempLpmMemInfoPtr->ramIndex]-
                                (shadowPtr->protocolCountersPerBlockArr[tempLpmMemInfoPtr->ramIndex].sumOfIpv4Counters+
                                 shadowPtr->protocolCountersPerBlockArr[tempLpmMemInfoPtr->ramIndex].sumOfIpv6Counters+
                                 shadowPtr->protocolCountersPerBlockArr[tempLpmMemInfoPtr->ramIndex].sumOfFcoeCounters+
                                 shadowPtr->pendingBlockToUpdateArr[tempLpmMemInfoPtr->ramIndex].numOfIncUpdates-
                                 shadowPtr->pendingBlockToUpdateArr[tempLpmMemInfoPtr->ramIndex].numOfDecUpdates);
                ...
                if (sumOfSourceLines + numFreeLines < newBucketSize)
                {
                    tempLpmMemInfoPtr = tempLpmMemInfoPtr->nextMemInfoPtr;
                    continue;
                }
                ...
                while (secondTempLpmMemInfoPtr != NULL)
                {
                    ...
                    numFreeLines = shadowPtr->lpmRamBlocksSizeArrayPtr[secondTempLpmMemInfoPtr->ramIndex]-
                    ...
                    sumOfFreeLinesAtTargetBanks += numFreeLines;

                    mergeReservationsArr[secondTempLpmMemInfoPtr->ramIndex].freeLines            = numFreeLines;
...
/**
* @internal prvCpssDxChLpmRamMngMoveBlockToNewLocationSip7 function
* @endinternal
*
...
*/
GT_STATUS prvCpssDxChLpmRamMngMoveBlockToNewLocationSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC        *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC      **lpmMemInfoPtrPtr,
...
)
{
    ...
    bucketShadowToMovePtr = (PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC *)GET_DMM_BUCKET_SW_ADDRESS_SIP7(blockHandleAddrForMerge);
    memSizeToMove         = SIZE_IN_WORDS_SIP7(blockHandleAddrForMerge);
    bucketShadowToMoveProtocol   = GET_DMM_BLOCK_PROTOCOL_SIP7(blockHandleAddrForMerge);
    ...
    *bucketRelocatedPtr = GT_FALSE;
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        while ((tempLpmMemInfoPtr!=NULL) && (newHwBucketOffsetHandle==0))
        {
            ...
            if (useShrink == GT_TRUE)
            {
                ...
                sumOfFreeBlockAfterShrink = prvCpssDmmCheckIfShrinkOperationUsefulForDefragSip7(tempLpmMemInfoPtr->structsMemPool,
                                                                                            memSizeToMove,
            ...
            /* look for the smallest (which is big enough) free space
             ...
             newHwBucketOffsetHandle = prvCpssDmmAllocateSip7(tempLpmMemInfoPtr->structsMemPool,
                                                    DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS * memSizeToMove,
                                                    DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS);
             if ((newHwBucketOffsetHandle == DMM_BLOCK_NOT_FOUND_SIP7)||(newHwBucketOffsetHandle == DMM_MALLOC_FAIL_SIP7))
             {
                 if (useShrink == GT_TRUE)
                 {
                     /* This case should never happen */
                     CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "error in prvCpssDxChLpmRamMngMoveBlockToNewLocationSip7 (useShrink = GT_TRUE)\n");
                 }
    ...
    if(newHwBucketOffsetHandle != 0)
    {
        ...
        /*  set pending flag for future need */
        blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(newHwBucketOffsetHandle)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
        retVal = prvCpssDxChLpmRamSip7CalcBankNumberIndex(shadowPtr,&blockIndex);
        if (retVal != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
        ...
        {
            /* we should be working on the bankIndex we got as parameter to the function */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDxChLpmRamMngMoveBlockToNewLocationSip7 - GT_FAIL \n");
        }

        shadowPtr->pendingBlockToUpdateArr[blockIndex].updateInc = GT_TRUE;
        shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfIncUpdates +=
                PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_SIZE_FROM_DMM_MAC(newHwBucketOffsetHandle);

        tempAddr = (PRV_CPSS_DXCH_LPM_RAM_GET_SIP7_LPM_OFFSET_FROM_DMM_MAC(newHwBucketOffsetHandle)) + memBlockBase;

        /* reset OUT parameters */
        oldMemSizeToMove = 0;
        oldMemGonIndexToMove=0;

        retVal = findBlockSizeAndIndexToMoveSip7((GT_DMM_BLOCK_SIP7 *)blockHandleAddrForMerge, &oldMemSizeToMove, &oldMemGonIndexToMove);
        if (retVal!=GT_OK)
        {
            /* This case should never happen */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in findBlockSizeAndIndexToMoveSip7 - GT_FAIL \n");
        }
        ...
        retVal = updateParentSwapGonsAdressesSip7(parentUpdateParams, tempAddr, oldMemGonIndexToMove);/* pointers will be updated to point to new allocated memory area */
        if (retVal!=GT_OK)
        {
        ...
        if (oldMemGonIndexToMove < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS)
        {
            ...
            SET_DMM_BLOCK_WAS_MOVED_SIP7(bucketShadowToMovePtr->hwGroupOffsetHandle[oldMemGonIndexToMove], 1);
            bucketShadowToMovePtr->hwGroupOffsetHandle[oldMemGonIndexToMove] = newHwBucketOffsetHandle;

           SET_DMM_BLOCK_PROTOCOL_SIP7(bucketShadowToMovePtr->hwGroupOffsetHandle[oldMemGonIndexToMove], bucketShadowToMoveProtocol);
           SET_DMM_BUCKET_SW_ADDRESS_SIP7(bucketShadowToMovePtr->hwGroupOffsetHandle[oldMemGonIndexToMove], bucketShadowToMovePtr);
        }
        else if (oldMemGonIndexToMove == 0xFFFF)
        {
            ...
            SET_DMM_BLOCK_WAS_MOVED_SIP7(bucketShadowToMovePtr->hwBucketOffsetHandle, 1);
            bucketShadowToMovePtr->hwBucketOffsetHandle = newHwBucketOffsetHandle;

            SET_DMM_BLOCK_PROTOCOL_SIP7(bucketShadowToMovePtr->hwBucketOffsetHandle, bucketShadowToMoveProtocol);
            SET_DMM_BUCKET_SW_ADDRESS_SIP7(bucketShadowToMovePtr->hwBucketOffsetHandle, bucketShadowToMovePtr);
        }
        else
        {
            /* This shouldn't happen */
...
/**
* @internal prvCpssDxChLpmRamMngMergeBankSip7 function
* @endinternal
*
...
*/
GT_STATUS prvCpssDxChLpmRamMngMergeBankSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmMemInfoPtrPtr,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
...
)
{
    ...
    GT_DMM_SORTED_PARTITION_ELEMENT_SIP7    *tempElemList;          /* temporary list to be used when running over
   ...
   /* Create a sorted list of all buckets at the bank we need to release (only of relevant protocol),
    ...
    partition = (GT_DMM_PARTITION_SIP7 *)shadowPtr->lpmRamStructsMemPoolPtr[bankIndexForMerge];

    retVal = prvCpssDmmSortPartitionSip7(partitionId,(GT_U32)protocol);
    if (retVal!=GT_OK)
    {
    ...
    for (relocatedBucketSize = DMM_MAXIMAL_BLOCK_SIZE_ALLOWED_SIP7; relocatedBucketSize > 0; relocatedBucketSize--)
    {
        ...
        while (tempElemList != NULL)
        {
            ...
            retVal = prvCpssDxChLpmRamMngMoveBlockToNewLocationSip7(shadowPtr,
                                                                    lpmMemInfoPtrPtr,
                                                                    octetIndexForMerge,
                                                                    bankIndexForMerge,
                                                                    isMarkedForShrinkArr,
            ...
            if (retVal!=GT_OK)
            {
                ...
                prvCpssDxChLpmRamUpdateBlockUsageCountersSip7(shadowPtr->lpmRamBlocksSizeArrayPtr,
            ...
            if (bucketRelocated==GT_TRUE)
            {
                ...
                if (retVal1!=GT_OK)
                {
                    /* put back the values kept in pendingBlockToUpdateArr before the merge */
                    cpssOsMemCpy(shadowPtr->pendingBlockToUpdateArr,tempPendingBlockToUpdateArr,sizeof(tempPendingBlockToUpdateArr));
                    CPSS_LOG_ERROR_AND_RETURN_MAC(retVal1,"error in prvCpssDxChLpmRamUpdateBlockUsageCountersSip7 - GT_FAIL \n");
                }
...
/**
* @internal prvCpssDxChLpmRamMngMergeAllGonBanksSip7 function
* @endinternal
*
* @brief   Merge one bank to the rest of the banks mapped to a given octet
*
...
* @retval GT_FAIL                  - merge operation fail
*/
GT_STATUS prvCpssDxChLpmRamMngMergeAllGonBanksSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC          *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC        **lpmMemInfoPtrPtr,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT      protocol,
...
)
{
    GT_STATUS rc = GT_OK;
    GT_U32    i;
    GT_BOOL   isBlockMerged[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS] = {GT_FALSE};
    ...
    for (i = 0; i < PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS; i++)
    {
        if ((neededMemoryBlocksPtr->octetIndexForMergeArr[i] >= PRV_CPSS_DXCH_LPM_NUM_OF_OCTETS_MAX_CNS) ||
            (neededMemoryBlocksPtr->bankIndexForMergeArr[i] >= PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_SIP7_CNS))
        {
        ...
        {
            rc = prvCpssDxChLpmRamMngMergeBankSip7(shadowPtr, lpmMemInfoPtrPtr, protocol,
            ...
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "error in prvCpssDxChLpmRamMngMergeBankSip7 - GT_FAIL \n");
            }
