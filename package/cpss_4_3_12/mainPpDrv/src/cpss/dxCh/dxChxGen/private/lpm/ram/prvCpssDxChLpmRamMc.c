...
)
{
    ...
    if (rootBucketPtr->hwBucketOffsetHandle != 0)
    {
        ...
        origRootHwBucketSize   = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(rootBucketPtr->hwBucketOffsetHandle);
    }
    lpmEngineMemPtr        = shadowPtr->mcSearchMemArrayPtr[protocol];
        ...
        if ((insertMode != PRV_CPSS_DXCH_LPM_RAM_TRIE_INSERT_SDW_ONLY_MODE_E) &&
            ...
            (((shadowPtr->vrRootBucketArray[vrId].rootBucket[protocol]->hwBucketOffsetHandle!=0)&&
             ...
             ((PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(
               shadowPtr->vrRootBucketArray[vrId].rootBucket[protocol]->hwBucketOffsetHandle) != origRootHwBucketSize))))
        {
            retVal = prvCpssDxChLpmRamMngVrfEntryUpdate(vrId ,protocol, shadowPtr);
        }
    }
...
)
{
    ...
    if (rootBucketPtr->hwBucketOffsetHandle != 0)
    {
        ...
        origRootHwBucketSize   = PRV_CPSS_DXCH_LPM_RAM_GET_LPM_SIZE_FROM_DMM_MAC(rootBucketPtr->hwBucketOffsetHandle);
    }
...
)
{
    ...
    {
        retVal =
            prvCpssDxChLpmSip6RamMngBucketTreeFlush(shadowPtr->mcSearchMemArrayPtr[protocolStack],
                                                    shadowPtr,
                                                    protocolStack,
                                                    vrId,
                                                    GT_FALSE,
