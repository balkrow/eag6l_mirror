...
/**
* @internal getMirrorGroupOfNodesDataAndUpdateRangesAddressForBulk function
*
* @brief This function gets a the bucket's shadow data and formats accordingly
*        the bitvector,compressed and the next pointer array.
*
...
* @retval   GT_FAIL - otherwise
*/
GT_STATUS getMirrorGroupOfNodesDataAndUpdateRangesAddressForBulk
(
    IN  CPSS_DXCH_LPM_NEXT_POINTER_TYPE_ENT             bucketType,
    IN  GT_U8                                           startAddress,
    IN  GT_U8                                           endAddress,
...
/**
* @internal lpmFalconBulkGonHwUpdate function
* @endinternal
*
...
*/
static GT_STATUS lpmFalconBulkGonHwLeafUpdate
(
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC         *bucketPtr,
...
)
{
    ...
    cpssOsMemSet(groupOfNodes,0,PRV_CPSS_DXCH_LPM_RAM_BIT_VECTOR_LINES_NUMBER_CNS*sizeof(PRV_CPSS_DXCH_LPM_GROUP_OF_NODES_RANGE_STC));
     ...
     for (i = 0; i < numberOfUpdatedGons; i++)
     {
         ...
         tempAddr[i] = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(bucketPtr->hwGroupOffsetHandle[bvlStartNumber+i]);

         retVal = getMirrorGroupOfNodesDataAndUpdateRangesAddressForBulk(bucketPtr->bucketType,
         ...
         {
             cpssOsFree(groupOfNodes);
             CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "Error:getMirrorGroupOfNodesDataAndUpdateRangesAddress failed\n");
         }
     }
...
* @retval   GT_OUT_OF_PP_MEM - if failed to allocate PP memory.
*/
static GT_STATUS lpmFalconBulkGonHwUpdate
(
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC         *bucketPtr,
    ...
    while (pRange != NULL)
    {
        ...
        if (pRange->updateRangeInHw == GT_TRUE)
        {
                ...
                (pRange->pointerType == CPSS_DXCH_LPM_ROUTE_ENTRY_NEXT_PTR_TYPE_E) )
            {
                ...
                retVal = lpmFalconBulkGonHwLeafUpdate(bucketPtr,pRange,shadowPtr,&lastUpdatedBvl);
                if (retVal != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "Error: lpmFalconBulkGonHwLeafUpdate is failed\n");
                }
            }
            else
            {
                ...
                case CPSS_DXCH_LPM_COMPRESSED_3_RANGES_3_LEAVES_NODE_PTR_TYPE_E:
                    ...
                    if (retVal != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "Error: lpmFalconFillCompressedData is failed\n");
                    }
                    break;
                case CPSS_DXCH_LPM_REGULAR_NODE_PTR_TYPE_E:
                    ...
                    if (retVal != GT_OK)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "Error: lpmFalconFillRegularData is failed\n");
                    }
                    break;
                default:
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_VALUE, LOG_ERROR_NO_MSG);
          ...
          (rootBucketState != LPM_ROOT_BUCKET_UPDATE_NONE_E)))
    {
        ...
        if (bucketUpdateMode != LPM_BUCKET_UPDATE_LAST_LEVEL_SHADOW_ONLY_E)
        {
            ...
            else
            {
                ...
                {
                    retVal = lpmFalconBulkGonHwUpdate(bucketPtr,pRange,shadowPtr);
                    if (retVal != GT_OK)
                    {
                        return retVal;
                    }
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation
(
...
* @retval   GT_FAIL             - Otherwise.
*/
GT_STATUS lpmFalconGetStaticGonsSizes
(
    IN    PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC       *bucketPtr,
    OUT   GT_U32                                        *groupOfNodesSizes
)
...
)
{
    ...
    if(parentNodeDataPtr->addOperation==GT_TRUE)
    {
        if(parentNodeDataPtr->swapAreaAlsoUsedForAddOnParentLevel==GT_FALSE)
        {
            ...
            blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(oldHwBucketOffsetHandle)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
            rc = prvCpssDxChLpmRamSip6CalcBankNumberIndex(shadowPtr,&blockIndex);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
            ...
            shadowPtr->pendingBlockToUpdateArr[blockIndex].updateDec=GT_TRUE;
            shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates +=
            PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(oldHwBucketOffsetHandle);

            /* free src root node */
            prvCpssDmmFree(oldHwBucketOffsetHandle);
        }
...
/**
* @internal handleDestRootNode function
* @endinternal
*
* @brief This function updates root node under swap operation mode
*
...
* @retval   GT_OUT_OF_PP_MEM - if failed to allocate PP memory.
*/
static GT_STATUS handleDestRootNode
(
    IN PRV_CPSS_DXCH_LPM_RAM_UPDATE_NODE_STC    *parentNodeDataPtr,
    IN GT_U8                                    *shareDevsList,
    IN GT_U32                                    shareDevsListLen,
...
)
{
         ...
         (parentNodeDataPtr->addOperation==GT_FALSE))
    {
        ...
        rc = prvCpssDxChLpmRamMngVrfEntryUpdate(parentNodeDataPtr->vrId, parentNodeDataPtr->protocol, shadowPtr);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmRamMngVrfEntryUpdate - failed\n");
        }

        /* free dest root node */
        prvCpssDmmFree(origHwBucketOffsetHandle);
...
)
{
         ...
         (parentNodeDataPtr->funcCallCounter == 2)  )
    {
        ...
        if (parentNodeDataPtr->isRootNode == GT_TRUE)
        {
            ...
            {
                return handleDestRootNode(parentNodeDataPtr,shareDevsList,shareDevListLen,swap_addr);
            }
            else
            {
...
*/
GT_STATUS prvCpssDxChLpmSip6RamMngRootBucketDelete
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN GT_U32                                       vrId,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol
...
/**
* @internal prvCpssDxChLpmTrieFlushDel function
* @endinternal
*
...
*
*/
static GT_STATUS prvCpssDxChLpmTrieFlushDel
(
...
/**
* @internal flushDelFromTrie function
* @endinternal
*
...
*/
static GT_STATUS flushDelFromTrie
(
    IN  PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC     *bucketPtr,
    IN  GT_U8                                       startAddr,
    IN  GT_U32                                      prefix,
...
)
{
    ...
    rc = prvCpssDxChLpmTrieFlushDel(&(bucketPtr->trieRoot),addr,prefix,trieDepth,
                                    &nextPointer);
    if (rc != GT_OK)
    {
        if (rc != GT_NOT_FOUND)
...
/**
* @internal prvCpssDxChLpmSip6RamMngBucketRangeGet function
* @endinternal
*
...
* @retval GT_OK           - if OK
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngBucketRangeGet
(
    IN PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC       **pRange,
...
* @retval GT_OK           - if OK
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngRootBucketRangeGet
(
...
/**
* @internal mergeFlushRange function
* @endinternal
*
...
*/
static GT_STATUS mergeFlushRange
(
    INOUT PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC    **rangePtr,
    IN    PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC    *prevRangePtr,
    IN    GT_U8                                     levelPrefix,
...
/**
* @internal prvCpssDxChLpmSip6RamMngLpmNodeRelease function
* @endinternal
*
* @brief free lpm node
*
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngLpmNodeRelease
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
...
)
{
    GT_STATUS                                   rc = GT_OK;
    GT_U32                                      blockIndex=0;
    blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC( *hwBucketOffsetHandlePtr) / (shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
    rc = prvCpssDxChLpmRamSip6CalcBankNumberIndex(shadowPtr,&blockIndex);
    if (rc != GT_OK)
    {
    ...
    shadowPtr->pendingBlockToUpdateArr[blockIndex].updateDec=GT_TRUE;
    shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates +=
        PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(*hwBucketOffsetHandlePtr);
    prvCpssDmmFree(*hwBucketOffsetHandlePtr);
    return rc;
}

/**
* @internal
*           prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops
*           function
* @endinternal
*
* @brief free lpm node
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops
(
    IN PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC       *rangePtr,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC      *bucketPtr,
    ...
    while ((rangePtr != NULL) && (rangePtr->startAddr <= flushEndAddr))
    {
        ...
        if ( (isRootNode == GT_FALSE) || ( (isRootNode == GT_TRUE) && (isUnicast == GT_TRUE) ) )
        {
            while (rangePtr->mask != 0)
            {
                ...
                rangePrefixLength = mask2PrefixLength(rangePtr->mask, PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS);
                ...
                rc = flushDelFromTrie(bucketPtr, rangePtr->startAddr, rangePrefixLength,
                                      PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS,&delNhEntry);
                if (rc != GT_OK)
                {
                    if (rc != GT_NOT_FOUND)
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "delFromTrie failed \n");
                    }
                    else
                    {
                        delNhEntry = NULL;
...
/**
* @internal
*           prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree function
* @endinternal
*
* @brief free lpm node
*
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree
(
    IN PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC       *rangePtr,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC      *bucketPtr,
    IN GT_U32                                       flushEndAddr
)
{
    ...
    while ((rangePtr != NULL) && (rangePtr->startAddr <= flushEndAddr))
    {
        rc = mergeFlushRange(&rangePtr,prevRangePtr,PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS,&numOfNewRanges);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "mergeFlushRange failed \n");
        }
...
/**
* @internal
*           prvCpssDxChLpmSip6RamMngGetDefaultNhs function
* @endinternal
*
* @brief free lpm node
*
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngGetDefaultNhs
(
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC      *bucketPtr,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocol,
    IN GT_BOOL                                      isUnicast,
...
)
{
    ...
    defNextPtr = getFromTrie(bucketPtr,startAddr,rangePrefixLength,PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS);
    if (defNextPtr==NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_FOUND, "\n ERROR got nextPtr==NULL from getFromTrie \n");
    }
    ...
    if (isUnicast == GT_TRUE)
    {
        *ucNextHopPtr = defNextPtr;
        if ( protocol == PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E )
        {
            ...
            {
                defNextPtr = getFromTrie(bucketPtr,startAddrEclass,rangePrefixLengthEclass,PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS);
                if (defNextPtr==NULL)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_FOUND, "\n ERROR got nextPtr==NULL from getFromTrie \n");
                }
...
/**
* @internal
*           prvCpssDxChLpmSip6RamMngFlushAssignDefault function
* @endinternal
*
* @brief free lpm node
*
...
* @retval   GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS prvCpssDxChLpmSip6RamMngFlushAssignDefault
(
    IN PRV_CPSS_DXCH_LPM_RAM_RANGE_SHADOW_STC       *rangePtr,
...
/**
 * @internal prvCpssDxChLpmSip6RamMngRootNodeSwap function
 * @endinternal
 *
 ...
 * @retval GT_OK - on success
 */
static GT_STATUS prvCpssDxChLpmSip6RamMngRootNodeSwap
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC            *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC     *bucketPtr,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT        protocol,
...
)
{
    ...
    bankIndex = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(hwBucketOffsetHandle)/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
    rc = prvCpssDxChLpmRamSip6CalcBankNumberIndex(shadowPtr,&bankIndex);
    if (rc != GT_OK)
    {
    ...
    /* need to temporarly update bucketPtr->bucketType so we will not fail on validity check comparing
       bucketType to rangePtr->pointerType in the call to lpmFalconFillCompressedData */
    ...
    {
        lpmFalconGetCompressedBucketType(bucketPtr, &newRootBucketType);
    }
    ...
    rc = prvCpssDxChLpmSip6RamMngShrinkCopyToOrFromSwapArea1and2(shadowPtr,
                                                                 bankIndex,
                                                                 bucketPtr,
                                                                 oldMemSize,
                                                                 0xFFFF,
    ...
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc,"prvCpssDxChLpmSip6RamMngShrinkCopyToOrFromSwapArea1and2 failed \n");
    }
    ...
    rc = prvCpssDxChLpmRamMngVrfEntryUpdate(vrId, protocol, shadowPtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "error in prvCpssDxChLpmRamMngVrfEntryUpdate \n");
    }
...
/**
* @internal prvCpssDxChLpmSip6RamMngBucketFlush function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmSip6RamMngBucketFlush
(
    IN PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC     *bucketPtr,
    IN GT_BOOL                                      isRootNode,
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
...
)
{
    ...
    if (isRootNode == GT_FALSE)
    {
        ...
        if (bucketPtr->hwBucketOffsetHandle != 0)
        {
            rc = prvCpssDxChLpmSip6RamMngLpmNodeRelease(shadowPtr,
                                                        &bucketPtr->hwBucketOffsetHandle);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "ERROR: prvCpssDxChLpmSip6RamMngLpmNodeRelease failed \n");
            }
        }
        bucketPtr->hwBucketOffsetHandle = 0;
        ...
        /* free nexthop memory*/
        rc = prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops(rangePtr,bucketPtr,255,GT_FALSE,isUnicast,GT_FALSE);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops failed \n");
        }
        /* free cpu memory */

        rangePtr = bucketPtr->rangeList;
        /* merge ranges and free its memory */
        rc = prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree(rangePtr,bucketPtr,255);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree failed \n");
        }
        ...
        {
            rc = prvCpssDxChLpmRamMngBucketDelete(bucketPtr, PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS, NULL);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmRamMngBucketDelete failed \n");
            }
        }
        else
        {
            rc = prvCpssDxChLpmRamMngBucketDelete(bucketPtr, PRV_CPSS_DXCH_LPM_RAM_MAX_LEVEL_LENGTH_CNS, (GT_PTR*)(GT_UINTPTR)&delEntry);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmRamMngBucketDelete failed \n");
            }
    ...
    else
    {
        ...
        /* get default */
        rc = prvCpssDxChLpmSip6RamMngGetDefaultNhs(bucketPtr,protocol,isUnicast,eclassSeparateTreatment,
                                                   &ucNextHopPtr,&ucEclassNextHopPtr,&mcNextHopPtr);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngRestoreDefault failed \n");
        }

        /* free nexthop memory */

        rc = prvCpssDxChLpmSip6RamMngBucketRangeGet(&rangePtr,flushStartAddr);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngBucketRangeGet  failed");
        }
        flushStartRangePtr = rangePtr;
        rc = prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops(rangePtr,bucketPtr,flushEndAddr,GT_TRUE,isUnicast,GT_FALSE);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops failed \n");
        ...
        /* merge ranges and free its memory */
        rc = prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree(rangePtr,bucketPtr,flushEndAddr);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngMergeNodeRangesAndFree failed \n");
        ...
        case PRV_CPSS_DXCH_LPM_PROTOCOL_IPV6_E:
            if (isUnicast == GT_TRUE)
            {
                rc = prvCpssDxChLpmSip6RamMngFlushAssignDefault(rangePtr, ucNextHopPtr);
                if (rc != GT_OK)
                {
                    cpssOsFree(groupOfNodes);
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngFlushAssignDefault failed \n");
                }
            }
            else
            {
                rc = prvCpssDxChLpmSip6RamMngFlushAssignDefault(rangePtr, mcNextHopPtr);
                if (rc != GT_OK)
                {
                    cpssOsFree(groupOfNodes);
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngFlushAssignDefault failed \n");
        ...
        if ( (isUnicast == GT_TRUE) && (protocol == PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E) )
        {
            ...
            rc = prvCpssDxChLpmSip6RamMngBucketRangeGet(&rangePtr,PRV_CPSS_DXCH_LPM_RAM_START_OF_IPV4_RESERVED_SPACE_ADDRESS_SPACE_CNS);
            if (rc != GT_OK)
            {
                cpssOsFree(groupOfNodes);
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngBucketRangeGet  failed");
            ...
            flushStartRangePtr = rangePtr;
            rc = prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops(rangePtr,bucketPtr,flushEndAddr,GT_TRUE,isUnicast,eclassSeparateTreatment);
            if (rc != GT_OK)
            {
                cpssOsFree(groupOfNodes);
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngHandleRangeMaskAndFreeNextHops failed \n");
            ...
            rc = prvCpssDxChLpmSip6RamMngFlushAssignDefault(rangePtr, ucEclassNextHopPtr);
            if (rc != GT_OK)
            {
                cpssOsFree(groupOfNodes);
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngFlushAssignDefault failed \n");
        ...
        oldHwBucketOffsetHandle = bucketPtr->hwBucketOffsetHandle;
        cpssOsMemCpy(oldHwGroupOffsetHandle,bucketPtr->hwGroupOffsetHandle,sizeof(oldHwGroupOffsetHandle));
        rc = prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_TRUE);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngRootNodeCopyToSwap failed \n");
        }
        ...
        if ((isUnicast == GT_TRUE) && (bucketPtr->bucketType == CPSS_DXCH_LPM_REGULAR_NODE_PTR_TYPE_E))
        {
            ...
            for (j = 0; j < 5; j++)
            {
                if (oldHwGroupOffsetHandle[j] != 0)
                {
                    ...
                    {
                        cpssOsFree(groupOfNodes);
                        prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_FALSE);
                        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngLpmNodeRelease failed");
                    }
        ...
        rc = lpmFalconGetGroupOfNodesSizes(bucketPtr,
                                           newRootBucketType,
        ...
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_FALSE);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "lpmFalconGetGroupOfNodesSizes failed");
        }
        ...
        shareDevListLen = (GT_U8)shadowPtr->workDevListPtr->shareDevNum;
        /* allocate memory for root bucket */
        rc = prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation(lpmEngineMemPtrPtr[0], memSize,
                                                              DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS,protocol,0,shadowPtr,
                                                              &bucketPtr->hwBucketOffsetHandle);
        if (rc != GT_OK)
        {
            cpssOsFree(groupOfNodes);
            prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_FALSE);
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation failed");
        }
        if (bucketPtr->hwBucketOffsetHandle !=0)
        {
        ...
        for (j = 0; j < gonNumberPointedByRoot; j++)
        {
            ...
            {
                rc = prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation(lpmEngineMemPtrPtr[0], grOfNodesSizes[j],
                                                                      DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS,protocol,0,shadowPtr,
                                                                      &bucketPtr->hwGroupOffsetHandle[j]);
                if (rc != GT_OK)
                {
                ...
                SET_DMM_BLOCK_PROTOCOL(bucketPtr->hwGroupOffsetHandle[j], protocol);
                SET_DMM_BUCKET_SW_ADDRESS(bucketPtr->hwGroupOffsetHandle[j], bucketPtr);
            }
        }
        ...
        for (j = 0; j < gonNumberPointedByRoot; j++)
        {
            ...
            {
                tempAddr[j] = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(bucketPtr->hwGroupOffsetHandle[j]);
                rc = getMirrorGroupOfNodesDataAndUpdateRangesAddress(newRootBucketType,
                                                                     startSubNodeAddress[j],
                                                                     endSubNodeAddress[j],
                                                                     bucketPtr->rangeList,
                                                                     shadowPtr->lpmRamTotalBlocksSizeIncludingGap,
                ...
                if (rc != GT_OK)
                {
                    cpssOsFree(groupOfNodes);
                    prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_FALSE);
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "getMirrorGroupOfNodesDataAndUpdateRangesAddress failed");
                }
            }
        }
        ...
        for (j = 0; j < gonNumberPointedByRoot; j++)
        {
            if (bucketPtr->hwGroupOffsetHandle[j] != 0)
            {
                for (devNum = 0; devNum < shareDevListLen; devNum++)
                {
                    rc =  prvCpssDxChLpmGroupOfNodesWrite(shareDevsList[devNum],
                                                          tempAddr[j],
                                                          &groupOfNodes[j],
                                                          &groupOfNodesLines);
                    if (rc != GT_OK)
        ...
        /* write updated root node */
         ...
         {
             rc =  prvCpssDxChLpmGroupOfNodesWrite(shareDevsList[devNum],
                                                   PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(bucketPtr->hwBucketOffsetHandle),
                                                   &groupOfNodes[0],
                                                   &groupOfNodesLines);
             if (rc != GT_OK)
             {
        ...
        rc = prvCpssDxChLpmRamUpdateBlockUsageCounters(shadowPtr->lpmRamBlocksSizeArrayPtr,
                                                       shadowPtr->pendingBlockToUpdateArr,
                                                       shadowPtr->protocolCountersPerBlockArr,
                                                       shadowPtr->pendingBlockToUpdateArr,
                                                       protocol,
       ...
       if (rc!=GT_OK)
       {
           ...
           cpssOsFree(groupOfNodes);
           prvCpssDxChLpmSip6RamMngRootNodeSwap(shadowPtr, bucketPtr, protocol, vrId, oldHwBucketOffsetHandle, GT_FALSE);
           CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "error in prvCpssDxChLpmRamUpdateBlockUsageCounters \n");
       }
...
/**
* @internal prvCpssDxChLpmSip6RamMngBucketTreeFlash
*           function
* @endinternal
*
* @brief   write an lpm bucket tree to the HW, and if neccessary allocate memory
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmSip6RamMngBucketTreeFlush
(
    IN PRV_CPSS_DXCH_LPM_RAM_MEM_INFO_STC           **lpmEngineMemPtrPtr,
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocolStack,
   ...
   /* lpmEngineMemPtrPtr++;*/
    ...
    case PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E:
        ...
        else
        {
            ...
            {
                retVal = prvCpssDxChLpmSip6RamMngRootBucketRangeGet((*currRange)->lowerLpmPtr.nextBucket,&tempRootRange,flushStartAddr);
                if (retVal != GT_OK)
                {
    ...
    while (currRange >= rangeMem)
    {
        if ((*currRange) == NULL)
        {
            ...
            if (currRange >= rangeMem)
            {
                ...
                retVal =  prvCpssDxChLpmSip6RamMngBucketFlush((*currRange)->lowerLpmPtr.nextBucket,
                                                              rootNode,
                                                              shadowPtr,
                ...
                if ((isDestTreeRootBucket == GT_TRUE) && ((*currRange)->startAddr > flushEndAddr))
                {
                    if ( (isUnicast == GT_TRUE) && (protocolStack == PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E) )                    {
                        ...
                        retVal = prvCpssDxChLpmSip6RamMngBucketRangeGet(currRange,PRV_CPSS_DXCH_LPM_RAM_START_OF_IPV4_RESERVED_SPACE_ADDRESS_SPACE_CNS);
                        if (retVal != GT_OK)
                        {
                            CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "prvCpssDxChLpmSip6RamMngRootBucketRangeGet failed \n");
                        }
...
/**
* @internal prvCpssDxChLpmDefaultHeadOfTrieLineWrite function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmDefaultHeadOfTrieLineWrite
(
    IN  GT_U8   devNum,
    IN  GT_U32  lpmRamTotalBlocksSizeIncludingGap,
...
)
{
    ...
    rc = lpmFalconFillCompressedData(&lpmRamBucket,
                                     lpmRamTotalBlocksSizeIncludingGap,
                                     &groupOfNodes.compressedNodesArray[0],
                                     &compressedNodesData);
    if (rc != GT_OK)
