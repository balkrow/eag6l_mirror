...
)
{
    ...
    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        GT_U32              leafIndexInLine = 0;
        GT_U32              bankNumber = 0;
        GT_STATUS rc = GT_OK;

        leafIndexInLine = leafIndex % 5;                            /* Leaf Index in LPM Line (0,1,2,3,4) */

        bankNumber = leafIndex / (PRV_CPSS_DXCH_PP_MAC(devNum)->moduleCfg.ip.pbrBankSize * 5);     /* Bank Index */
        ...
        switch (pbrBankSize)
        {
            case 0:
                /* Bank: bits 17:15. Line: bits 14:3. Leaf Index in Line: bits 2:0 */
          		*lpmIndexPtr = ((bankNumber << 15) & 0x3FFFF) + (((leafIndex / 5) - (bankNumber * PRV_CPSS_DXCH_PP_MAC(devNum)->moduleCfg.ip.pbrBankSize)) << 3) + leafIndexInLine;
        		break;

            case 1:
                /* Bank: bits 17:16. Line: bits 15:3. Leaf Index in Line: bits 2:0 */
                *lpmIndexPtr = ((bankNumber << 16) & 0x3FFFF) + (((leafIndex / 5) & 0x1FFF) << 3) + leafIndexInLine;
        		break;

            case 2:
                /* Bank: bit 17. Line: bits 16:3. Leaf Index in Line: bits 2:0 */
          		*lpmIndexPtr = ((bankNumber << 17) & 0x3FFFF) + (((leafIndex / 5) & 0x3FFF) << 3) + leafIndexInLine;
        		break;

            case 3:
                /* Line: bits 17:3. Leaf Index in Line: bits 2:0 */
                *lpmIndexPtr = (((leafIndex / 5) & 0x7FFF) << 3) + leafIndexInLine;
        		break;
...
* @retval GT_OK                    - on success
*/
GT_STATUS prvCpssDxChLpmLeafEntryCalcLpmSwIndexFromHwIndex
(
    IN GT_U8            devNum,
...
)
{
    ...
    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        leafSwIndex = 5 * (((leafHwIndex & mask) >> 3 )  +
                       (leafHwIndex >> shift) * (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.totalNumOfLinesInBlockIncludingGap)) +
                       (leafHwIndex & 0x7);

        *leafSwIndexPtr = leafSwIndex;
    ...
    switch (nextPointerData.pointerType)
    {
        ...
        case CPSS_DXCH_LPM_COMPRESSED_2_NODE_PTR_TYPE_E:
            ...
            maxMemSize =
               ...
               (PRV_CPSS_ACCESS_TABLE_INFO_PTR(dev)[CPSS_DXCH_SIP5_TABLE_LPM_MEM_E].maxNumOfEntries);
...
)
{
    ...
    pbrBankSize = (moduleCfgPtr->ip.pbrBankSize);
