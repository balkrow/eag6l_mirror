...
)
{
    ...
    GT_U32  lpmRamTotalBlocksSizeIncludingGap = PRV_CPSS_DXCH_LPM_RAM_FALCON_MAX_PBR_SIZE_IN_LPM_LINES_CNS;
...
)
{
    ...
    GT_BOOL                 octet0ToSmallBankSupported = GT_TRUE;
    ...
    for (k=0;k<memoryCfgPtr->lpmRamConfigInfoNumOfElements;k++)
    {
        switch (memoryCfgPtr->lpmRamConfigInfo[k].devType)
        {
            ...
            case CPSS_IRONMAN_ALL_DEVICES_CASES_MAC:
                ...
                octet0ToSmallBankSupported = GT_FALSE;
                break;
        ...
        else
        {
            ...
            else
            {
                ...
                case CPSS_DXCH_CFG_SHARED_TABLE_MODE_AC5P_32K_L2_256K_EM_64K_ARP_E:
                    ...
                    ramMemoryPtr->lpmRamConfigInfoNumOfElements=memoryCfgPtr->lpmRamConfigInfoNumOfElements;
                    ramMemoryPtr->lpmRamConfigInfo[k].devType = memoryCfgPtr->lpmRamConfigInfo[k].devType;
                    ramMemoryPtr->lpmRamConfigInfo[k].sharedMemCnfg = PRV_CPSS_DXCH_CFG_SHARED_TABLE_MODE_AC5P_32K_L2_256K_EM_64K_ARP_E;
        ...
        if((octet0ToSmallBankSupported) &&                                  /* assign small bank to octet0 supported by the device */
           (ramMemoryArray[k].octetsGettingSmallBanksPriorityBitMap > 0) && /* There are small banks */
           (ramMemoryArray[k].octetsGettingBigBanksPriorityBitMap > 0) &&   /* There are big banks */
           (memoryCfgPtr->assignSmallBankToOctet0))                         /* There is a request to assign small bank to octet0 */
        {
            ramMemoryArray[k].octetsGettingSmallBanksPriorityBitMap |= 0x1;        /* Assign Octet0 to small bank  */
            ramMemoryArray[k].octetsGettingBigBanksPriorityBitMap  &= 0xFFFFFFFE;  /* De-Assign Octet0 to big bank */
        }
    }
...
)
{
    ...
    GT_DMM_BLOCK  *dmmBlockPtr = NULL;
    GT_DMM_BLOCK  *secondDmmBlockPtr = NULL;
    GT_DMM_BLOCK  *thirdDmmBlockPtr = NULL;
    GT_DMM_BLOCK  *defaultAddrForHeadOfTriePtr = NULL;
    ...
    for (i = 0; i < shadowPtr->numOfLpmMemories + offsetCompensation; i++)
    {
        ...
        if (k == 0)
        {
            ...
            defaultAddrForHeadOfTriePtr = (GT_DMM_BLOCK*)cpssOsLpmMalloc(sizeof(GT_DMM_BLOCK));
            ...
            /* memSize is in lines, each line hold 4 words */
            SET_OFFSET_IN_WORDS(defaultAddrForHeadOfTriePtr, (swapFirstBlockOffset*shadowPtr->lpmRamTotalBlocksSizeIncludingGap)*4);
            startAddressOffset = numDefaultAddrLines * DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS;
    ...
    for (i = 0; i < PRV_CPSS_DXCH_LPM_PROTOCOL_LAST_E; i++)
    {
        ...
        if ((shadowPtr->bigBanksNumber==0)||
            ((shadowPtr->smallBanksNumber!=0)&&(((shadowPtr->octetsGettingSmallBanksPriorityBitMap) & 0x1) != 0)))/* we want to set octet0 to a small bank */
        {
            ...
            {
                if (((shadowPtr->smallBanksIndexesBitMap.l[0]) & (1<<k))!=0)
                {
        ...
        else
        {
            ...
            for (k=0;k<shadowPtr->numOfLpmMemories;k++)
            {
                ...
                if (((shadowPtr->bigBanksIndexesBitMap.l[0])&(1<<k))!=0)
                {
...
/*GM do not support new memory banks logic,
  ...
  bank0 to octet0 and bank1 to octet1 */
    ...
    if (PRV_CPSS_DXCH_LPM_RAM_OCTET_TO_BLOCK_MAPPING_3_OCT_PER_BANK_MAC(shadowPtr) == GT_FALSE)
    {
...
)
{
    ...
    for (protStk = PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E; protStk < PRV_CPSS_DXCH_LPM_PROTOCOL_LAST_E; protStk++)
    {
        ...
        if (shadowPtr->shadowType == PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E)
        {
            retVal = prvCpssDxChLpmSip6RamMngRootBucketDelete(shadowPtr, vrId, protStk);
            if (retVal != GT_OK)
            {
...
)
{
         ...
         protStk++)
    {
        ...
        if (ucSupportArr[protStk] == GT_TRUE)
        {
            ...
            if(protStk == PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E)
            {
                if ((vrEclassNonRegularUc == GT_TRUE) || (shadowPtr->shadowType != PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E))
                {
...
)
{
    ...
    if (shadowPtr->shadowType == PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E)
    {
        ...
        headOfTrie = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(shadowPtr->defaultAddrForHeadOfTrie);
    }
          ...
          protocol++)
    {
        ...
        {
            retVal = prvCpssDxChLpmHwVrfEntryWrite(shadowPtr->workDevListPtr->shareDevs[dev],
...
)
{
    ...
    if (shadowPtr->vrRootBucketArray[vrId].isUnicastSupported[PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E] == GT_TRUE)
    {
        ...
        cpssOsMemSet(&ipv4Addr, 0, sizeof(ipv4Addr));
        ipv4Addr.arIP[0] = 240;
        retVal = prvCpssDxChLpmRamIpv4UcPrefixSearch(lpmDbPtr,vrId,ipv4Addr,4,&routeEntry);
        if ((retVal != GT_OK) && (retVal != GT_NOT_FOUND) && (retVal != GT_NOT_INITIALIZED))
        {
            return retVal;
        }
...
)
{
    ...
    {
        if (blockIndex >= PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_FALCON_CNS)
        {
...
/**
* @internal prvCpssDxChLpmRamIpv4UcPrefixBulkAddForHa function
* @endinternal
*
* @brief  Function Relevant mode : High Level API modes
...
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; AAS.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmRamIpv4UcPrefixBulkAddForHa
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr,
...
)
{
    ...
    for (shadowIdx = 0 ; shadowIdx < lpmDbPtr->numOfShadowCfg; shadowIdx++)
    {
            ...
            (tempSystemRecovery_Info.systemRecoveryState == CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E)))
        {
            ...
            return prvCpssDxChLpmRamIpv4UcPrefixBulkAddForHa(shadowPtr,
                                                             ipv4PrefixArrayLen,
                                                             ipv4PrefixArrayPtr,
                                                             defragmentationEnable);
        }
...
/**
* @internal prvCpssDxChLpmRamIpv6UcPrefixBulkAddForHa function
* @endinternal
*
* @brief  Function Relevant mode : High Level API modes
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChLpmRamIpv6UcPrefixBulkAddForHa
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr,
...
)
{
    ...
    for (i = 0 ; i < ipv6PrefixArrayLen ; i++)
    {
        ...
        if (shadowPtr->shadowType == PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E)
        {
            ipv6PrefixArrayPtr[i].returnStatus = prvCpssDxChLpmSip6RamUcEntryAdd(ipv6PrefixArrayPtr[i].vrId,
        ...
        {
            ipv6PrefixArrayPtr[i].returnStatus = prvCpssDxChLpmRamUcEntryAdd(ipv6PrefixArrayPtr[i].vrId,
                                                ipv6PrefixArrayPtr[i].ipAddr.arIP,
                                                ipv6PrefixArrayPtr[i].prefixLen,
                                                &lpmRouteEntry.routeEntry,
                                                PRV_CPSS_DXCH_LPM_RAM_TRIE_INSERT_SDW_ONLY_MODE_E,
...
/**
* @internal prvCpssDxChLpmRamIpv6UcPrefixBulkAdd
* BulkAdd function
* @endinternal
*
* @brief   Function Relevant mode : High Level API modes
...
)
{
    ...
    for (shadowIdx = 0 ; shadowIdx < lpmDbPtr->numOfShadowCfg; shadowIdx++)
    {
        ...
        {
            /* in HA process we want to add the prefixes to the shadow one by one and not using bulk */
            return prvCpssDxChLpmRamIpv6UcPrefixBulkAddForHa(shadowPtr,
                                                             ipv6PrefixArrayLen,
                                                             ipv6PrefixArrayPtr,
                                                             defragmentationEnable);
        }
    ...
    while (rangePtr)
    {
        ...
        if ((rangePtr->pointerType != CPSS_DXCH_LPM_ROUTE_ENTRY_NEXT_PTR_TYPE_E) &&
            ...
            (!(((GT_U32)rangePtr->pointerType==(GT_U32)PRV_CPSS_DXCH_LPM_RAM_TRIE_PTR_TYPE_E) &&
               ((rangePtr->lowerLpmPtr.nextBucket->isImplicitGroupDefault==GT_FALSE) &&
                (rangePtr->lowerLpmPtr.nextBucket->numOfRanges == 1)))))/* when isImplicitGroupDefault==GT_FALSE we are dealing with (G,*) */
        {
...
)
{
    ...
    for (i=0; i<hwNumOfRanges; i++)
    {
        ...
        if ((swTotalRangesIsNonRealArr[i]==GT_FALSE)&&
            ...
            (tempRangePtr->lowerLpmPtr.nextBucket->isImplicitGroupDefault==GT_FALSE) &&
            (tempRangePtr->lowerLpmPtr.nextBucket->numOfRanges == 1)) /* when isImplicitGroupDefault==GT_FALSE we are dealing with (G,*) */
            {
...
)
{
    ...
    GT_U8                                           hwBucketFifthStartAddr = 0;
...
)
{
    ...
    while (1)
    {
        ...
        {
            return prvCpssDxChLpmRamSyncSwHwForHaSip7();
        }
                        ...
                        (ipShadowPtr->vrRootBucketArray[i].rootBucket[currProtocolStack] != NULL))
                    {
                        ...
                        if (rc != GT_OK)
                        {
                            CPSS_LOG_ERROR_MAC("Error: got a fail in SW/HW sync in HA  - prvCpssDxChLpmRamSyncSwHwForHa \n");
                             ...
                             /* for sip6 we support logic to reconstruct HW according to existing SW */
                             if (shadowType == CPSS_DXCH_IP_LPM_RAM_SIP6_SHADOW_E)
                             {
                                ...
                                cpssOsMemSet(ipShadowPtr->allNewNextMemInfoAllocatedPerOctetArrayPtr,0,sizeof(ipShadowPtr->allNewNextMemInfoAllocatedPerOctetArrayPtr));
                                cpssOsMemSet(ipShadowPtr->tempLpmRamOctetsToBlockMappingUsedForReconstractPtr,0,sizeof(ipShadowPtr->tempLpmRamOctetsToBlockMappingUsedForReconstractPtr));
                                rc = prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed(ipShadowPtr);
                                ...
                                if (rc!=GT_OK)
                                {
                                    ...
                                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "error in prvCpssDxChLpmRamSyncSwHwForHa-prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed");
                                }
...
/**
* @internal prvCpssDxChLpmCleanHwAfterHaSyncFailed function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmCleanHwAfterHaSyncFailed
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr
)
{
...
/**
* @internal cleanSwTreeRangesFromDmmAllocDoneInHaSync function
* @endinternal
*
...
* @retval GT_OK                    - on success
*/
GT_STATUS cleanSwTreeRangesFromDmmAllocDoneInHaSync
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC          *shadowPtr,
    IN  GT_U32                                    vrId,
    IN  PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC   *bucketPtr,
...
)
{
    ...
    while (rangePtr)
    {
            ...
            (rangePtr->pointerType != CPSS_DXCH_LPM_MULTIPATH_ENTRY_PTR_TYPE_E))
        {
            ...
            status = cleanSwTreeFromDmmAllocDoneInHaSync(shadowPtr,
                                                           vrId,
                                                           rangePtr->lowerLpmPtr.nextBucket,
                                                           (GT_U8)(level + 1),
                                                           numOfMaxAllowedLevels,
...
GT_STATUS cleanSwTreeFromDmmAllocDoneInHaSync
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC          *shadowPtr,
    IN  GT_U32                                    vrId,
    IN  PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC   *bucketPtr,
...
)
{
    ...
    if(bucketPtr->hwBucketOffsetHandle!=0)
    {
        ...
        status = prvCpssDxChLpmRamSip6CalcBankNumberIndex(shadowPtr,&blockIndex);
        if (status != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
        }
        ...
        shadowPtr->pendingBlockToUpdateArr[blockIndex].updateDec=GT_TRUE;
        shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates += PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(bucketPtr->hwBucketOffsetHandle);
       ...
       for (j=0;j<PRV_CPSS_DXCH_LPM_RAM_FALCON_MAX_SIZE_OF_BUCKET_IN_LPM_LINES_CNS;j++)
        {
            ...
            if (bucketPtr->hwGroupOffsetHandle[j]!=0)
            {
                ...
                /* free DMM that was set during HA sync operation that failed */
                blockIndex = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(bucketPtr->hwGroupOffsetHandle[j])/(shadowPtr->lpmRamTotalBlocksSizeIncludingGap);
                status = prvCpssDxChLpmRamSip6CalcBankNumberIndex(shadowPtr,&blockIndex);
                if (status != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "ERROR:illegal blockIndex - fall in holes \n");
                ...
                shadowPtr->pendingBlockToUpdateArr[blockIndex].updateDec=GT_TRUE;
                shadowPtr->pendingBlockToUpdateArr[blockIndex].numOfDecUpdates += PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(bucketPtr->hwGroupOffsetHandle[j]);

                prvCpssDmmFree(bucketPtr->hwGroupOffsetHandle[j]);
    ...
    {
        /* Ranges clean */
        status = cleanSwTreeRangesFromDmmAllocDoneInHaSync(shadowPtr,
                                                           vrId,
                                                           bucketPtr,
                                                           level,
                                                           numOfMaxAllowedLevels,
...
/**
* @internal prvCpssDxChIpLpmCleanSwAllocationsPerProtocolAndPrefixType function
* @endinternal
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS prvCpssDxChIpLpmCleanSwAllocationsPerProtocolAndPrefixType
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN GT_U32                                       vrId,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocolStack,
...
)
{
   ...
   /* go over valid VR , if it's initilized for this protocol */
    ...
    if (shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack])
    {
        ...
        retVal = cleanSwTreeFromDmmAllocDoneInHaSync(shadowPtr,
                                                       vrId,
                                                       bucketPtr,
                                                       0,
                                                       numOfMaxAllowedLevels,
...
/**
* @internal prvCpssDxChIpLpmUpdateAndCleanCountersAfterHaSyncFail function
* @endinternal
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS prvCpssDxChIpLpmUpdateAndCleanCountersAfterHaSyncFail
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr
)
{
         ...
         protocolStack++)
    {
        ...
        for (bankIndex=0; bankIndex < shadowPtr->numOfLpmMemories; bankIndex++)
        {
            if (bankIndex==firstBlockIndex)
            {
                ...
                if (PRV_CPSS_DXCH_LPM_RAM_OCTET_TO_BLOCK_MAPPING_3_OCT_PER_BANK_MAC(shadowPtr))
                {
                    ...
                    rc = prvCpssDxChLpmRamMngUnbindFirstBlock(shadowPtr, protocolStack, bankIndex);
                    if (rc!=GT_OK)
                    {
                        return rc;
                    }
            ...
            else if (PRV_CPSS_DXCH_LPM_RAM_IS_BLOCK_USED_BY_PROTOCOL_GET_MAC(shadowPtr,protocolStack,bankIndex)==GT_TRUE)
            {
                ...
                rc = prvCpssDxChLpmRamMngUnbindBlock(shadowPtr, bankIndex);
                if (rc!=GT_OK)
                {
                    return rc;
                }
        ...
        /* free the allocated/bound RAM memory */
        rc = prvCpssDxChLpmRamMngAllocatedAndBoundMemFree(shadowPtr,protocolStack);
        if (rc!=GT_OK)
        {
...
/**
* @internal prvCpssDxChLpmCleanSwAfterHaSyncFailed function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmCleanSwAfterHaSyncFailed
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr
)
{
         ...
         protocolStack++)
    {
        ...
        for (vrfId = 0 ; vrfId < BIT_12 ; vrfId++)
        {
            if (shadowPtr->vrRootBucketArray[vrfId].valid == GT_TRUE)
            {
                ...
                {
                    rc = prvCpssDxChLpmRamMemFreeListMng(0,PRV_CPSS_DXCH_LPM_RAM_MEM_LIST_RESET_OP_E,
                                              &shadowPtr->freeMemListDuringUpdate,shadowPtr);
                    if (rc != GT_OK)
                    {
                    ...
                    for (prefixType=CPSS_UNICAST_E;prefixType<CPSS_UNICAST_MULTICAST_E;prefixType++)
                    {
                            ...
                            (shadowPtr->vrRootBucketArray[vrfId].isMulticastSupported[protocolStack] == GT_TRUE)))
                        {
                            ...
                            rc = prvCpssDxChIpLpmCleanSwAllocationsPerProtocolAndPrefixType(shadowPtr,
                                                                                            vrfId,
                                                                                            protocolStack,
                                                                                            prefixType);
                            if (rc != GT_OK)
                ...
                /* now it's possible to free all the "end of update to be freed
                   memory" memory , now that the HW is updated (and the "during
                   update to be freed" memory if needed ) */
                rc = prvCpssDxChLpmRamMemFreeListMng(0,PRV_CPSS_DXCH_LPM_RAM_MEM_LIST_FREE_MEM_OP_E,
                                                         &shadowPtr->freeMemListDuringUpdate,shadowPtr);
    ...
    rc = prvCpssDxChIpLpmUpdateAndCleanCountersAfterHaSyncFail(shadowPtr);
    if (rc != GT_OK)
    {
...
/**
* @internal allocateSwTreeRangesFromDmmAllocDoneInHaSync function
* @endinternal
*
...
* @retval GT_OK                    - on success
*/
GT_STATUS allocateSwTreeRangesFromDmmAllocDoneInHaSync
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC          *shadowPtr,
    IN  GT_U32                                    vrId,
    IN  PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC   *bucketPtr,
...
)
{
    ...
    while (rangePtr)
    {
            ...
            (rangePtr->pointerType != CPSS_DXCH_LPM_MULTIPATH_ENTRY_PTR_TYPE_E))
        {
            ...
            status = allocateSwTreeFromDmmAllocDoneInHaSync(shadowPtr,
                                                           vrId,
                                                           rangePtr->lowerLpmPtr.nextBucket,
                                                           (GT_U8)(level + 1),
                                                           numOfMaxAllowedLevels,
...
GT_STATUS allocateSwTreeFromDmmAllocDoneInHaSync
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC          *shadowPtr,
    IN  GT_U32                                    vrId,
    IN  PRV_CPSS_DXCH_LPM_RAM_BUCKET_SHADOW_STC   *bucketPtr,
...
)
{
    ...
    if (bucketPtr->bucketType != CPSS_DXCH_LPM_ROUTE_ENTRY_NEXT_PTR_TYPE_E)
    {
        ...
        if((isSrcRoot == GT_TRUE) || (isRoot == GT_TRUE))
        {
            if(bucketPtr->hwBucketOffsetHandle==0)
            {
                ...
                status = prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation(lpmEngineMemPtrPtr[octetNum],
                                                                      memSize,
                ...
                else
                {
                    ...
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "after prvCpssDxChLpmSip6RamMngHandleRootNodeAllocation "
        ...
        status = lpmFalconGetStaticGonsSizes(bucketPtr,
                                         &subnodesSizes[0]);
        if (status != GT_OK)
        {
            return status;
            ...
            if (bucketPtr->hwGroupOffsetHandle[j] == 0)
            {
                ...
                if (lpmEngineMemPtrPtr[gonOctetNum]->structsMemPool != 0)
                {
                   bucketPtr->hwGroupOffsetHandle[j] =
                        prvCpssDmmAllocate(lpmEngineMemPtrPtr[gonOctetNum]->structsMemPool,
                                           DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS *subnodesSizes[j],
                                           DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS);
                }
                else
                {
                ...
                while((tempNextMemInfoPtr->nextMemInfoPtr!= NULL)&&
                      ((bucketPtr->hwGroupOffsetHandle[j] == DMM_BLOCK_NOT_FOUND)||
                       (bucketPtr->hwGroupOffsetHandle[j] == 0)))
                {
                    bucketPtr->hwGroupOffsetHandle[j] =
                ...
                if ((tempNextMemInfoPtr->nextMemInfoPtr==NULL)&&
                    ((bucketPtr->hwGroupOffsetHandle[j]==DMM_BLOCK_NOT_FOUND)||
                     (bucketPtr->hwGroupOffsetHandle[j]==0)))
                {
                    status = findAndBindValidMemoryBlock(shadowPtr, protocol, (gonOctetNum),
                                                         &bucketPtr->hwGroupOffsetHandle[j],
                                                         subnodesSizes[j],
                                                         tempNextMemInfoPtr,
                ...
                else
                {
                    ...
                    if(bucketPtr->hwGroupOffsetHandle[j] == DMM_MALLOC_FAIL)
                    {
                        status = GT_OUT_OF_PP_MEM;
                        break;
                    }
    ...
    {
        /* Ranges allocate */
        status = allocateSwTreeRangesFromDmmAllocDoneInHaSync(shadowPtr,
                                                           vrId,
                                                           bucketPtr,
                                                           level,
                                                           numOfMaxAllowedLevels,
...
/**
* @internal prvCpssDxChIpLpmAllocateSwAllocationsPerProtocolAndPrefixType function
* @endinternal
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS prvCpssDxChIpLpmAllocateSwAllocationsPerProtocolAndPrefixType
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC             *shadowPtr,
    IN GT_U32                                       vrId,
    IN PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT         protocolStack,
...
)
{
   ...
   /* go over valid VR , if it's initilized for this protocol */
    ...
    if (shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack])
    {
        ...
        retVal = allocateSwTreeFromDmmAllocDoneInHaSync(shadowPtr,
                                                       vrId,
                                                       bucketPtr,
                                                       0,
                                                       numOfMaxAllowedLevels,
...
/**
* @internal prvCpssDxChLpmAllocateSwAfterHaSyncFailed function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmAllocateSwAfterHaSyncFailed
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr
)
{
                ...
                if (shadowPtr->vrRootBucketArray[vrfId].rootBucket[protocolStack] != NULL)
                {
                    ...
                    for (prefixType=CPSS_UNICAST_E;prefixType<CPSS_UNICAST_MULTICAST_E;prefixType++)
                    {
                            ...
                            (shadowPtr->vrRootBucketArray[vrfId].isMulticastSupported[protocolStack] == GT_TRUE)))
                        {
                            ...
                            rc = prvCpssDxChIpLpmAllocateSwAllocationsPerProtocolAndPrefixType(shadowPtr,
                                                                                        vrfId,
                                                                                        protocolStack,
                                                                                        prefixType);
                            if (rc != GT_OK)
...
/**
* @internal prvCpssDxChLpmUpdateHwAccordingToSwAfterHaSyncFailed function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmUpdateHwAccordingToSwAfterHaSyncFailed
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr
)
{
...
/**
* @internal prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed function
* @endinternal
*
...
*
*/
GT_STATUS prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed
(
    IN PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC     *shadowPtr
)
{
    ...
    rc = prvCpssDxChLpmCleanHwAfterHaSyncFailed(shadowPtr);
    if (rc !=0 )
    {
    ...
    /* Clean all allocations done in the sync process, those allocations were kept in the shadow,
       including: dmm allocations, Octet to bank mapping allocations, counters updates */
    rc = prvCpssDxChLpmCleanSwAfterHaSyncFailed(shadowPtr);
    if (rc !=0 )
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "ERROR: fail in prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed, "
    ...
    /* Allocate new DMM memory in the shadow, according to the current buckets */
    rc = prvCpssDxChLpmAllocateSwAfterHaSyncFailed(shadowPtr);
    if (rc !=0 )
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "ERROR: fail in prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed, "
    ...
    rc = prvCpssDxChLpmUpdateHwAccordingToSwAfterHaSyncFailed(shadowPtr);
    if (rc !=0 )
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "ERROR: fail in prvCpssDxChLpmRamCleanAndUpdateSwAndHwAfterHaSyncFailed, "
