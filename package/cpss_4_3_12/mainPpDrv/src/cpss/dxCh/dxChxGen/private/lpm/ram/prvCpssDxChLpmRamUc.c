...
)
{
    ...
    GT_U32                                            origRootHwBucketSize = 0xffffffff;
    ...
    if (pBucket->hwBucketOffsetHandle != 0)
    {
        ...
        origRootHwBucketSize = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(pBucket->hwBucketOffsetHandle);
    }
    ...
    if (retVal == GT_OK)
    {
        ...
        if ((insertMode == PRV_CPSS_DXCH_LPM_RAM_TRIE_INSERT_SDW_MEM_HW_MODE_E) &&
            (((shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack]->hwBucketOffsetHandle!=0)&&
             ...
             ((PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(
                 shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack]->hwBucketOffsetHandle) != origRootHwBucketSize))))
        {
            ...
            else
            {
                ...
                if (insertMode == PRV_CPSS_DXCH_LPM_RAM_TRIE_INSERT_SDW_MEM_MODE_E)
                {
                    ...
                    cpssOsMemCpy(tempPendingBlockToUpdateArr,shadowPtr->pendingBlockToUpdateArr,
                                 (PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_FALCON_CNS*sizeof(PRV_CPSS_DXCH_LPM_RAM_PENDING_BLOCK_TO_UPDATE_STC)));
                }
            }
        }
        cpssOsFree(tempPendingBlockToUpdateArr);
...
)
{
    ...
    if (pBucket->hwBucketOffsetHandle != 0)
    {
        ...
        origRootHwBucketSize = PRV_CPSS_DXCH_LPM_RAM_GET_LPM_SIZE_FROM_DMM_MAC(pBucket->hwBucketOffsetHandle);
    }
    ...
    if (retVal == GT_OK)
    {
        ...
        if ((insertMode == PRV_CPSS_DXCH_LPM_RAM_TRIE_INSERT_SDW_MEM_HW_MODE_E) &&
            ...
            ((PRV_CPSS_DXCH_LPM_RAM_GET_LPM_OFFSET_FROM_DMM_MAC(
                 shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack]->hwBucketOffsetHandle) != origRootHwBucketOffset) ||
             (origRootBucketType != rootBucketType)||
             ((PRV_CPSS_DXCH_LPM_RAM_GET_LPM_SIZE_FROM_DMM_MAC(
                 shadowPtr->vrRootBucketArray[vrId].rootBucket[protocolStack]->hwBucketOffsetHandle) != origRootHwBucketSize))))
        {
            retVal = prvCpssDxChLpmRamMngVrfEntryUpdate(vrId, protocolStack, shadowPtr);
            if (retVal != GT_OK)
            ...
            for (bankIndex=0; bankIndex < shadowPtr->numOfLpmMemories; bankIndex++)
            {
                if (bankIndex==firstBlockIndex)
                {
                    ...
                    if (PRV_CPSS_DXCH_LPM_RAM_OCTET_TO_BLOCK_MAPPING_3_OCT_PER_BANK_MAC(shadowPtr))
                    {
                        ...
                        retVal = prvCpssDxChLpmRamMngUnbindFirstBlock(shadowPtr, protocolStack, bankIndex);
                        if (retVal!=GT_OK)
                        {
                            return retVal;
                        }
...
)
{
    ...
    if ( (shadowPtr->shadowType == PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E) && (protocolStack != PRV_CPSS_DXCH_LPM_PROTOCOL_FCOE_E) )
    {
        retVal =
            prvCpssDxChLpmSip6RamMngBucketTreeFlush(shadowPtr->ucSearchMemArrayPtr[protocolStack],
                                                    shadowPtr,
                                                    protocolStack,
                                                    vrId,
                                                    GT_TRUE,
        ...
        (*addrPtr <= PRV_CPSS_DXCH_LPM_RAM_END_OF_IPV4_MC_ADDRESS_SPACE_CNS))
    {
        ...
        *prefixPtr = PRV_CPSS_DXCH_LPM_RAM_IPV4_RESERVED_ADDRESS_SPACE_PREFIX_CNS;
        ...
        if (shadowPtr->shadowType == PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E)
        {
            ...
            retVal = prvCpssDxChLpmRamMngSearch(bucketPtr,addrPtr, *prefixPtr, (GT_PTR*)&dummy);
            if (retVal != GT_OK)
