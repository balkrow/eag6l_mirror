...
)
{
    ...
    if (prefix > 128)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "Shouldn't happen: max prefix lenght is 128");
    }
...
*
*******************************************************************************/
static PRV_CPSS_DXCH_LPM_RAM_TRIE_NODE_STC* findNextValidNodeSip7
(
    IN PRV_CPSS_DXCH_LPM_RAM_TRIE_NODE_STC *root,
...
/**
* @internal buildAddressFromTrieNodeSip7 function
* @endinternal
*
* @brief   This function gets a pointer to a node in the trie and returns the
...
*                                   Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] trieNodePtr              - A pointer to the tree node to calculate the address for
* @param[in] trieDepth                - The maximum depth of the trie.
*
* @param[out] addrPtr                  - Holds the calculated addr.
* @param[out] prefixPtr                - The prefix of AddrPtr.
*                                       None.
*/
static void buildAddressFromTrieNodeSip7
(
...
* @param[in,out] prefixPtr                - the prefix of the address.
* @param[in,out] addrPtr                  - The address associated with the next node.
* @param[in,out] prefixPtr                - The prefix associated with the address.
*
...
)
{
    ...
    trieNode = prvCpssDxChLpmTrieSearchSip7(rootPtr,addrPtr,*prefixPtr,trieDepth,NULL);
    ...
    retTrieNode = findNextValidNodeSip7(rootPtr,trieNode);

    if(retTrieNode != NULL)
    {
        buildAddressFromTrieNodeSip7(retTrieNode,trieDepth,tmpAddr,&tmpPrefix);
