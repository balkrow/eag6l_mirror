        ...
        if ((useSwapAreaArr[i]==GT_TRUE)&&(oldBucketShouldBeMovedArr[i]==GT_TRUE))
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    oldGonSize = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(oldHwHandleAddrArr[i]);
                    oldGonOffset = PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_OFFSET_FROM_DMM_MAC(oldHwHandleAddrArr[i]);
                    ...
                    retVal = prvCpssDmmAllocateByPtr(shadowPtr->lpmRamStructsMemPoolPtr[bankIndex],
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS* (oldGonOffset+oldGonSize),
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS* gonSizeDiff,
                                                     DMM_MIN_ALLOCATE_SIZE_IN_BYTE_FALCON_CNS,
                                                     &tempHwAddrHandle[i]);
                    if ((tempHwAddrHandle[i] == DMM_BLOCK_NOT_FOUND)||(tempHwAddrHandle[0] == DMM_MALLOC_FAIL))
                    {
                        for (j=0;j<i;j++)
                        {
                            if (tempHwAddrHandle[j] != 0)
                        ...
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,"error in prvCpssDmmAllocateByPtr - GT_FAIL \n");
                    }
                }
            }
        }
                ...
                (neededMemoryBlocksPtr->neededMemoryBlocksSwapUsedForAdd[j] == GT_TRUE))
            {
                ...
                else
                {
                    if (secondSwapUsed == GT_FALSE)
                    {
                        ...
                        if (newSize[1]>=PRV_CPSS_DXCH_LPM_RAM_GET_FALCON_LPM_SIZE_FROM_DMM_MAC(oldHandleToCheck[1]))
                        {
    ...
    for(i = startIdx; i < shadowPtr->neededMemoryListLen; i++)
    {
            ...
            (neededMemoryBlocksInfo[i].mergeOperationUsefulForDefragGlobalFlag == GT_TRUE))
        {
            ...
            if (mergeBankCanBeFound == GT_FALSE)
            {
                ...
                retVal = prvCpssDxChLpmSip6RamMngCheckIfUnassignedBankUsefulForMerge(shadowPtr,
                                                                                     lpmEngineMemPtrPtr,
                                                                                     protocol,
                                                                                     i,
                                                                                     isMarkedForShrinkArr,
...
/**
* @internal prvCpssDxChLpmSip6RamMngCheckIfUnassignedBankUsefulForMerge function
* @endinternal
*
...
*/
GT_STATUS prvCpssDxChLpmSip6RamMngCheckIfUnassignedBankUsefulForMerge
(
    IN    PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC            *shadowPtr,
...
)
{
    ...
    GT_U32      unassignedBanksIndexArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_FALCON_CNS] = {0};
    GT_U32      unassignedBanksFreeLinesArr[PRV_CPSS_DXCH_LPM_RAM_NUM_OF_MEMORIES_FALCON_CNS] = {0};
