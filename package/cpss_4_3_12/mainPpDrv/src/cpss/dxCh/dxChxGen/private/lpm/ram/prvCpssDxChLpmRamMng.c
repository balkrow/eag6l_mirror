...
/**
* @internal prvCpssDxChLpmRamMngUnbindFirstBlock function
* @endinternal
*
...
* @retval GT_OK                    - on success
*/
GT_STATUS prvCpssDxChLpmRamMngUnbindFirstBlock
(
    IN  PRV_CPSS_DXCH_LPM_RAM_SHADOW_STC        *shadowPtr,
    IN  PRV_CPSS_DXCH_LPM_PROTOCOL_STACK_ENT    protocol,
    IN  GT_U32                                  blockIndex
)
{
    ...
    if (PRV_CPSS_DXCH_LPM_RAM_IS_BLOCK_USED_BY_PROTOCOL_GET_MAC(shadowPtr, protocol, blockIndex) == GT_TRUE)
    {
        ...
        for (octetIndex=1; octetIndex < maxNumOfOctets; octetIndex++)
        {
            ...
            {
                if(((GT_DMM_PARTITION *)shadowPtr->lpmMemInfoArray[protocol][octetIndex].structsMemPool != NULL ) &&
                   (((GT_DMM_PARTITION *)shadowPtr->lpmMemInfoArray[protocol][octetIndex].structsMemPool)->allocatedBytes != 0))
                {
...
)
{
    ...
    {
        firstPriorityBanksIndexesBitMap = shadowPtr->bigBanksIndexesBitMap.l[0];
        secondPriorityBanksIndexesBitMap = shadowPtr->smallBanksIndexesBitMap.l[0];
    }
    else/* it mean smallBanksPriority==GT_TRUE */
    {
        firstPriorityBanksIndexesBitMap = shadowPtr->smallBanksIndexesBitMap.l[0];
        secondPriorityBanksIndexesBitMap = shadowPtr->bigBanksIndexesBitMap.l[0];
    }
