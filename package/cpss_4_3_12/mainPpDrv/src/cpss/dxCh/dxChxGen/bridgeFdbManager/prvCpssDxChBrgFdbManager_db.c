...
#if CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS != PRV_CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS
    #error "CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS and PRV_CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS must be equal"
#endif
...
#define MEM_CALLOC_MAC(_fdbManagerId,_pointer,_sizeInBytes)       \
    {                                                             \
        ...
        cpssOsMemSet(_pointer,0,(_sizeInBytes));\
                                                \
        MANAGER_GET_MAC(_fdbManagerId)->numOfAllocatedPointers++;        \
...
#define SELF_TEST_BMP_SET_MAC(_fdbManagerPtr, _bitPos)   BMP_SET_MAC(&_fdbManagerPtr->selfTestBitmap[0], _bitPos)
#define SELF_TEST_BMP_CLR_MAC(_fdbManagerPtr, _bitPos)   BMP_CLEAR_MAC(&_fdbManagerPtr->selfTestBitmap[0], _bitPos)
...
)
{
    ...
    GT_U32      isVPort;
    GT_U32      multiple = 0;
    CPSS_GEN_SIP_ENT    genSip = SIP_OF_HW(fdbManagerPtr);

    if (isMacEntry)
    {
        /* multiple */
        SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
        ...
        {/* bit40MacAddr=0 and entry is PORT or TRUNK */
            ...
            {
                if(IS_FDB_MANAGER_VPORT_MODE(fdbManagerPtr))
                {
                    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                            SIP7_FDB_FDB_TABLE_FIELDS_USE_VPORT_E,
                            hwValue);
                    isVPort = hwValue;
                }
                else
                ...
                {
                    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                            SIP7_FDB_FDB_TABLE_FIELDS_TRG_VPORT_0_13_E,
                            hwValue);
                    dstInterfacePtr->interfaceInfo.devPort.portNum = hwValue;
                    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                            SIP7_FDB_FDB_TABLE_FIELDS_TRG_VPORT_14_17_E,
                            hwValue);
                    dstInterfacePtr->interfaceInfo.devPort.portNum |= hwValue << 14;
                }
                else
                {
                    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
    ...
    if (multiple == 1)
    {
        ...
        SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
            ...
            hwValue);
        dstInterfacePtr->interfaceInfo.vidx = hwValue;

        if(CPSS_GEN_SIP_7_E == genSip)
        {
            SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                SIP7_FDB_FDB_TABLE_FIELDS_VIDX_16_E,
                hwValue);

            dstInterfacePtr->interfaceInfo.vidx |= hwValue << 16;/* bit 16 */
        }


        dstInterfacePtr->type = (dstInterfacePtr->interfaceInfo.vidx == 0xFFF)?CPSS_INTERFACE_VID_E:CPSS_INTERFACE_VIDX_E;
    }
    return GT_OK;
}
...
)
{
    ...
    CPSS_GEN_SIP_ENT    genSip = SIP_OF_HW(fdbManagerPtr);
    GT_U32    supportFidBit15 = 0;/*indication that the device support bit 15 in the entry */

    if(genSip == CPSS_GEN_SIP_7_E)
    {
        if(fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes == 1 &&
           fdbManagerPtr->capacityInfo.hwCapacity.mode == CPSS_MAC_HASH_FUNC_XOR_E)
        {
            /* skip bit used */
        }
        else
    ...
    {
        SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
            SIP7_FDB_FDB_TABLE_FIELDS_FID_15_E,
            hwValue);

        entryPtr->fid |= hwValue << 15;/* bit 15 */
    }


    /* MAC address */
    SIPx_FDB_HW_ENTRY_FIELD_MAC_ADDR_GET_MAC(genSip,hwDataArr, entryPtr->macAddr.arEther);
    ...
    return buildDbEntry_dstInterface_macOrIpmc(fdbManagerPtr,GT_TRUE /* MAC entry */,
            hwDataArr,
...
)
{
    ...
    if (multiple == 0)
    {
        ...
        else
        {
            ...
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                        SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_USE_VPORT_E,
                        hwValue);
                isVPort = hwValue;
            }
            else
            ...
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                        SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_TRG_VPORT_E,
                        hwValue);
            }
            else
            {
...
)
{
    ...
    if(hwValue == 0)
    {
        ...
        if(genSip < CPSS_GEN_SIP_7_E)/* not valid field in sip7 */
        {
            /* Counter set */
            SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
        ...
        {
            entryPtr->ucRouteInfo.fullFdbInfo.countSet = CPSS_IP_CNT_NO_SET_E;
        }
        ...
        {
            SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr,
                SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15_E,
                hwValue);

            entryPtr->ucRouteInfo.fullFdbInfo.nextHopVlanId |= hwValue << 15;/* bit 15 */
        }
        ...
        rc = buildDbEntry_dstInterface_IpvxUc(fdbManagerPtr,hwDataArr,
...
)
{
    ...
    for(wordNum = 0; wordNum < 4; wordNum++)
    {
         ...
         */
        SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwKeyArr,
...
)
{
    ...
    return buildDbEntry_dstInterface_macOrIpmc(fdbManagerPtr,GT_FALSE /* IP MC entry */,
            hwDataArr,
            &entryPtr->dstInterface);
}
...
)
{
    ...
    if (multiple == 1)
    {
        ...
        /* there is no space for User Defined field in this case */
        if(userDefinedPtr)
        {
            /* assign Zero to avoid trash values */
            *userDefinedPtr = 0;
        }
    }
...
)
{
    ...
    if (hwValue == 1)
    {
        ...
        nextHopInfoPtr->fullFdbInfo.dstInterface.interfaceInfo.vidx = hwValue;
    }
    else
    {
...
)
{
    ...
    switch(hwFormatType)
    {
        ...
        case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_10_E:
        case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP7_E:
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(hwFormatType);
    }

    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,&fdbManagerPtr->tempInfo.sip6_hwFormatWords[0],
...
)
{
    ...
    if((dbExist == GT_FALSE) && MANAGER_GET_MAC(fdbManagerId))
    {
    ...
    switch(entryAttrPtr->shadowType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E:
    ...
    if(entryAttrPtr->shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E &&
       entryAttrPtr->vportInfo.enableVPortMode == GT_TRUE)
    {
        /* check the 'vPort mode' parameters */
        CHECK_X_NOT_MORE_THAN_Y_MAC(entryAttrPtr->vportInfo.maxNumOfPhyPortsToUse,  (_1K-1));
        /* the 2 ranges together can't be over the 256K range */
        CHECK_X_NOT_MORE_THAN_Y_MAC((entryAttrPtr->vportInfo.maxNumOfVPortsToUse +
                                     entryAttrPtr->vportInfo.maxNumOfPhyPortsToUse),(_256K-1));

        /* we don't allow 'global eport' ranges in this shadow as we not work with eports */
        if(entryAttrPtr->globalEportInfo.enable != CPSS_DXCH_CFG_GLOBAL_EPORT_CONFG_DISABLE_E)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                "entryAttrPtr->globalEportInfo.enable[%d] must be value 0 ('disabled') when entryAttrPtr->vportInfo.enableVPortMode == GT_TRUE",
                entryAttrPtr->globalEportInfo.enable);
        }
    }
...
)
{
    ...
    switch(fdbManagerPtr->entryAttrInfo.shadowType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP6_E:
            ...
            break;
        case CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E:
            /* Only SIP7 devices can be added to this FDB manager ! */
            BMP_SET_MAC(fdbManagerPtr->supportedSipArr, CPSS_GEN_SIP_7_E);
            fdbManagerPtr->cpssHashParams.fid16BitHashEn       = GT_TRUE;
            fdbManagerPtr->cpssHashParams.crcHashUpperBitsMode = (GT_U32)fdbManagerPtr->lookupInfo.crcHashUpperBitsMode;
            break;
        default:
    ...
    fdbManagerPtr->cpssHashParams.numOfMultipleHashes = fdbManagerPtr->numOfBanks;
    ...
    if(fdbManagerPtr->capacityInfo.enableLearningLimits == GT_TRUE)
    {
        ...
        if(IS_FDB_MANAGER_VPORT_MODE(fdbManagerPtr))
        {
            /* when 'vPort mode' is used , the 'Global Eports' not used ! */
            numGlobalEports = 0;
        }
        else
        ...
        if(fdbManagerPtr->entryAttrInfo.globalEportInfo.enable ==
            ...
            fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E )
        {
        ...
        fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid_num =
            fdbManagerPtr->entryAttrInfo.shadowType != CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E ?
                _8K : _64K;
        numBytesToAlloc = fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid_num *
            sizeof(PRV_CPSS_DXCH_FDB_MANAGER_DB_DYNAMIC_UC_MAC_LIMIT_STC);
        MEM_CALLOC_MAC(fdbManagerId, fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid, numBytesToAlloc);

        for(ii = 0 ; ii < fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid_num ; ii++)
        {
        ...
        if(IS_FDB_MANAGER_VPORT_MODE(fdbManagerPtr))
        {
            numBytesToAlloc = fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse * sizeof(PRV_CPSS_DXCH_FDB_MANAGER_DB_DYNAMIC_UC_MAC_LIMIT_STC);
            MEM_CALLOC_MAC(fdbManagerId, fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr, numBytesToAlloc);
        }
    }
...
)
{
    ...
    *managerAllocatedPtr = GT_TRUE;
    ...
    if(fdbManagerPtr->entryAttrInfo.shadowType != CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E)
    {
        /* 'vPort mode' info not relevant to non-sip7 shadow , so state it is not enabled */
        /* allow the generic code can check the 'enableVPortMode' for all shadows */
        fdbManagerPtr->entryAttrInfo.vportInfo.enableVPortMode = GT_FALSE;
    }
    else
    if(fdbManagerPtr->entryAttrInfo.vportInfo.enableVPortMode == GT_TRUE)
    {
        /* sip 7 that allow 'vPort mode' if application enable it */

        /* the vPorts starts after the 'physical ports' , so to allow access into
           array of vPorts , we need to add the physicals , BUT we should not
           access to the lower part of the vPorts (vPort < maxNumOfPhyPortsToUse) */
        fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse +=
            fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfPhyPortsToUse;

        fdbManagerPtr->entryAttrInfo.globalEportInfo.enable =
...
)
{
    ...
    if(fdbManagerPtr->dynamicUcMacLimitPtr)
    {
        ...
        MEM_FREE_MAC(fdbManagerId,fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid);
        MEM_FREE_MAC(fdbManagerId,fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr);
...
*
* @retval GT_OK                     - on success
* @retval GT_BAD_STATE              - wrong state of DB or inputs
*/
...
)
{
    ...
    prvCpssDxChBrgFdbManagerDbEntryGet(fdbManagerPtr, bankId, calcInfoPtr, &dbEntryPtr);

    /* IPv6 entry */
    if(bankStep > 1)
    {
        /* Get data base entry - Second bank */
        prvCpssDxChBrgFdbManagerDbEntryGet(fdbManagerPtr, bankId+1, calcInfoPtr, &dbEntryExtPtr);

        if(calcInfoPtr->usedBanksArr[bankId])
        {
            if (dbEntryPtr == NULL)
            {
                /* should not happen - DB is corrupted or bug in algorithm */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
            }
    ...
    else
    {
        if(calcInfoPtr->usedBanksArr[bankId])
        {
            ...
            else
            {
                ...
                {
                    /* Get data base entry - Second bank */
                    prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet(fdbManagerPtr, hwIndex + 1, &dbEntryExtPtr);
                }
                else
                {
                    dbEntryExtPtr = dbEntryPtr;
                    /* Get data base entry - First bank */
                    prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet(fdbManagerPtr, hwIndex - 1, &dbEntryPtr);
                }

                banksState = PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_NON_IPV6_TO_IPV6_E; /* Non-IPv6->IPv6 - Lookup/Move Bank[0,1] */
            }
        }
        else
        {
...
/**
* @internal prvCpssDxChBrgFdbManagerBanksStateGet_sip5 function
* @endinternal
*
* @brief    The function checks state of calculated banks according to entry format.
*           Its aligned for SIP_5 devices.
*           Hashing for IPv6 UC entries differs between SIP_5 and SIP_5_25 and above.
*
*
* @param[in] fdbManagerPtr          - (pointer to) the FDB manager
* @param[in] bankId                 - bank ID to get info from calc
...
* @retval GT_BAD_STATE              - wrong state of DB or inputs
*/
static GT_STATUS  prvCpssDxChBrgFdbManagerBanksStateGet_sip5
(
    IN    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN    GT_U32                                            bankId,
    IN    GT_U32                                            bankStep,
...
)
{
    ...
    PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC    *dbEntryPtr;
    PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC    *dbEntryExtPtr = NULL;
    GT_U32                                            nhDataBankNum;
    GT_U32                                            hwIndex;

    /* Get data base entry - First bank */
    prvCpssDxChBrgFdbManagerDbEntryGet(fdbManagerPtr, bankId, calcInfoPtr, &dbEntryPtr);

    /** In case of non sip_6, IPv6 Data entry can reach here.
     *  Returning as non-matching as no movement can happen
     */
    if(dbEntryPtr && (dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E))
    {
        calcInfoPtr->lastBankIdChecked  = bankId;
        calcInfoPtr->selectedBankId     = bankId;
        *banksStatePtr                  = banksState;
    ...
    if(bankStep > 1)
    {
        if(calcInfoPtr->usedBanksArr[bankId])
        {
            ...
            {
                /* Get data base entry - Second bank */
                hwIndex         = dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
                nhDataBankNum   = hwIndex % fdbManagerPtr->numOfBanks;
                /* IPV6_TO_IPV6 is valid, only when the stored hwIndex is present in the crcHashList */
                if(hwIndex == calcInfoPtr->crcMultiHashArr[nhDataBankNum])
                {
                    prvCpssDxChBrgFdbManagerDbEntryGet(fdbManagerPtr, nhDataBankNum, calcInfoPtr, &dbEntryExtPtr);
                    if(calcInfoPtr->usedBanksArr[nhDataBankNum])
                    {
                        if (dbEntryExtPtr == NULL)
                        {
                            /* should not happen - DB is corrupted or bug in algorithm */
                        ...
                        if(dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E &&
                                dbEntryExtPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E)
                        {
                            calcInfoPtr->nhDataBankNumArr[bankId] = nhDataBankNum;
                            /* IPv6->IPv6 - Lookup/Move Bank[0,1] */
                            banksState = PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_IPV6_TO_IPV6_E;
                        }
                    }
                }
            }
        ...
        {
            calcInfoPtr->nhDataBankNumArr[bankId] = 0;
            /* Find the next free bank in case first free bank found */
            for(nhDataBankNum = bankId+1; nhDataBankNum < fdbManagerPtr->numOfBanks; nhDataBankNum++)
            {
                /* Get data base entry - Second bank */
                prvCpssDxChBrgFdbManagerDbEntryGet(fdbManagerPtr, nhDataBankNum, calcInfoPtr, &dbEntryExtPtr);
                if(!calcInfoPtr->usedBanksArr[nhDataBankNum])
                {
                    ...
                    banksState = PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_IPV6_TWO_BANKS_FREE_E;
                    calcInfoPtr->nhDataBankNumArr[bankId] = nhDataBankNum;
                    break;
                }
            }
    ...
    else
    {
        if(calcInfoPtr->usedBanksArr[bankId])
        {
               ...
               dbEntryPtr->hwFdbEntryType != PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E)
            {
                ...
                banksState = PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_NON_IPV6_TO_NON_IPV6_E;
            }
            else
            {
                if(dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E)
                {
                    ...
                    if(hwIndex == calcInfoPtr->crcMultiHashArr[nhDataBankNum])
                    {
                        ...
                        if(calcInfoPtr->usedBanksArr[nhDataBankNum])
                        {
                            /* Non-IPv6->IPv6 - Lookup/Move Bank[0,1] */
                            banksState = PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_NON_IPV6_TO_IPV6_E;
                            calcInfoPtr->nhDataBankNumArr[bankId] = nhDataBankNum;
                        }
                    }
                }
...
/**
* @internal prvCpssDxChBrgFdbManagerBankCalcInfoGet_sip5 function
* @endinternal
*
...
* @retval GT_BAD_PARAM             - wrong parameter
*/
GT_STATUS  prvCpssDxChBrgFdbManagerBankCalcInfoGet_sip5
(
    IN    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN    GT_U32                                            bankId,
    IN    GT_U32                                            bankStep,
...
)
{
    ...
    rc = prvCpssDxChBrgFdbManagerBanksStateGet_sip5(fdbManagerPtr, bankId, bankStep, calcInfoPtr, &banksState);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    switch(banksState)
    {
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_NON_IPV6_TO_NON_IPV6_E:
            /* The entry in the bank need to be compared */
            if(entryPtr == NULL)
            {
                calcInfoPtr->bankStepArr[bankId] = 1;
            }
...
)
{
    ...
    if(!PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
    {
        return prvCpssDxChBrgFdbManagerBankCalcInfoGet_sip5(fdbManagerPtr, bankId, bankStep, NULL, addParamsPtr, calcInfoPtr);
    }
    ...
    switch(banksState)
    {
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_CALC_ENTRY_IPV6_TWO_BANKS_FREE_E:
            /* There are free banks to add IPv6 entry */
            calcInfoPtr->calcEntryType            = PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_FREE_E;
            calcInfoPtr->nhDataBankNumArr[bankId] = bankId + 1;
            if(entryPtr == NULL)
            {
                calcInfoPtr->bankStepArr[bankId] = 2;
            }
...
*                                                 NULL if entry not valid
*
* @retval none
*/
GT_VOID prvCpssDxChBrgFdbManagerDbEntryGet
(
    IN    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN    GT_U32                                            bankId,
    INOUT PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *calcInfoPtr,
...
/**
* @internal prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet function
* @endinternal
*
* @brief   Returns DB entry by HW index
*
*
* @param[in] fdbManagerPtr          - (pointer to) the FDB manager
...
* @retval none
*/
GT_VOID prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet
(
    IN    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN    GT_U32                                            hwIndex,
    OUT   PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC    **dbEntryPtrPtr
...
/**
* @internal prvCpssDxChFdbManagerDbCalcExistingEntryFind_sip5 function
* @endinternal
*
* @brief   This function finds existing entry in DB, from the hashArray. (Only ipv6)
*          Its aligned for SIP_5 devices.
*          Hashing for IPv6 UC entries differs between SIP_5 and SIP_5_25 and above.
*            NOTE: the function doing only DB operations (no HW operations)
*
...
*
*/
GT_STATUS prvCpssDxChFdbManagerDbCalcExistingEntryFind_sip5
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC       *fdbManagerPtr,
    IN CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_STC                  *entryPtr,
...
)
{
    ...
    calcInfoPtr->calcEntryType = PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_NOT_FOUND_AND_NO_FREE_E;
    for(bankId = 0 ; bankId < fdbManagerPtr->numOfBanks; bankId += 1)
    {
        hwIndex = calcInfoPtr->crcMultiHashArr[bankId];
        prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet(fdbManagerPtr,
                hwIndex,
                &dbEntryPtr);
        if(dbEntryPtr == NULL)
        {
        ...
        if(dbEntryExtPtr->hwFdbEntryType != PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
        }
        ...
        calcInfoPtr->dbEntryExtPtr              = dbEntryExtPtr;
        calcInfoPtr->selectedBankId             = bankId;
        calcInfoPtr->nhDataBankNumArr[bankId]   = hwIndex % fdbManagerPtr->numOfBanks;
        calcInfoPtr->bankStepArr[bankId]        = 2;
        calcInfoPtr->calcEntryType              = PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_EXISTS_E;
        break;
    }
...
/**
* @internal prvCpssDxChFdbManagerDbCalcFreeEntryFind_sip5 function
* @endinternal
*
...
*
*/
static GT_STATUS prvCpssDxChFdbManagerDbCalcFreeEntryFind_sip5
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC       *fdbManagerPtr,
    INOUT PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC  *calcInfoPtr
)
{
    ...
    for(bankId = 0 ; bankId < fdbManagerPtr->numOfBanks; bankId += 1)
    {
        ...
        rc = prvCpssDxChBrgFdbManagerBankCalcInfoGet_sip5(fdbManagerPtr,
                bankId,
                calcInfoPtr->bankStep,
                NULL,
                NULL,
                calcInfoPtr);
        if(rc != GT_OK)
        ...
        {
            if (fdbManagerPtr->bankInfoArr[bankId].bankPopulation == fdbManagerPtr->maxPopulationPerBank)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
            }
            break;
...
* @param[in] fdbManagerPtr         - (pointer to) the FDB manager
* @param[inout] calcInfoPtr        - (pointer to) the calculated hash and lookup results
* @param[in]    originalBankStep   - the bank step of the entry being rehashed
*
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
...
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    INOUT PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *calcInfoPtr,
    IN GT_U32                                            originalBankStep
)
{
    GT_STATUS                                       rc;
    GT_U32                                          bankId;
    GT_U32                                          ipv6BankId = calcInfoPtr->rehashStageBank/2;
    ...
    for(bankId = 0 ; bankId < fdbManagerPtr->numOfBanks; bankId += calcInfoPtr->bankStep)
    {
        ...
        /* Rehashing is for IPV6 entry but lookup entry is not IPV6 */
        if ((originalBankStep == 2) && (calcInfoPtr->bankStep == 1))
        {
            /* Two banks: bankId and bankId+1 might be used for IPV6 entry */
            if(ipv6BankId == bankId/2)
            {
                /* skip current rehashing bank */
                continue;
            }
...
/**
* @internal prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck_sip5 function
* @endinternal
*
...
* @param[in] previousBankStep       - previous rehashing entry bank step
* @param[in] calcInfoPtr            - (pointer to) the calculated hash and lookup results
*
* @retval GT_OK                    - on success
* @retval GT_BAD_VALUE             - HW index already in use
*/
static GT_STATUS  prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck_sip5
(
    IN      PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN      GT_U32                                            rehashStageId,
    IN      PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *prevCalcInfoPtr,
    IN      PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *calcInfoPtr,
    IN      GT_U32                                            reservedHwIdx
)
{
    GT_U32      hwIndex;           /* HW index of entry that will be moved or empty place for last stage */
    GT_U32      nhDataIndex=GT_NA; /* HW index of IPv6 Data entry that will be moved or empty place for last stage */
    GT_U32      ii;                /* iterator */
    GT_U32      bankId;            /* hash bank id that is used for index calculation */
    GT_BOOL     useNhDataIndex = GT_FALSE; /* need to check two indexes or single */

    /* Initial HW indexes per current stage.
       The real HW indexes will be checked in comparison logic of the next stage */
    fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwIndex = PRV_CPSS_DXCH_BRG_FDB_MANAGER_INVALID_HW_INDEX_CNS;
    fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwExtIndex = PRV_CPSS_DXCH_BRG_FDB_MANAGER_INVALID_HW_INDEX_CNS;

    if(calcInfoPtr->calcEntryType != PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_FREE_E)
    {
        /* Rehashing bank that need to be tested during rehashing for move possibility */
        bankId = calcInfoPtr->rehashStageBank;
        if(reservedHwIdx != GT_NA)
        {
            /* Non IPv6_UC is candidate for moving to free place for IPv6_UC.
               Reserved free index is used for this case also */
            nhDataIndex = reservedHwIdx;
            useNhDataIndex = GT_TRUE;
        }
        else if (prevCalcInfoPtr->bankStepArr[bankId] > 1)
        {
            /* IPv6_UC is candidate for moving, take Data part index from Key part DB entry because
               hash array for such entry may differ from one in prevCalcInfoPtr */
            nhDataIndex = prevCalcInfoPtr->dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
            useNhDataIndex = GT_TRUE;
        }
    }
    else
    {
        /* Selected bank of the last rehashing stage when the free place is found */
        ...
        else if (calcInfoPtr->bankStep > 1)
        {
            /* free place is for IPv6_UC, need to check second index also */
            nhDataIndex = calcInfoPtr->crcMultiHashArr[calcInfoPtr->nhDataBankNumArr[bankId]];
            useNhDataIndex = GT_TRUE;
        }
    }
    ...
    {
        /* Compare rehashing bank index with rehashed indexes from DB */
        if (hwIndex == fdbManagerPtr->rehashingTraceArray[ii].stageHwInfo.entryHwIndex ||
            hwIndex == fdbManagerPtr->rehashingTraceArray[ii].stageHwInfo.entryHwExtIndex)
        {
            /* The HW index was used during rehashing */
            return /* it's not error */ GT_BAD_VALUE;
        }

        if(useNhDataIndex)
        {
            /* Compare rehashing bank index for IPv6 Data part with rehashed indexes from DB */
            if (nhDataIndex == fdbManagerPtr->rehashingTraceArray[ii].stageHwInfo.entryHwIndex ||
                nhDataIndex == fdbManagerPtr->rehashingTraceArray[ii].stageHwInfo.entryHwExtIndex)
            {
                /* The HW index was used during rehashing */
                return /* it's not error */ GT_BAD_VALUE;
            }
            ...
            /* Non IPv6_UC is candidate for moving to free place for IPv6_UC.
               Reserved free index is used for this case. Reserved index
               may be found as free one for Non IPv6_UC. Need to check this
               case also */
            if (hwIndex == nhDataIndex)
            {
                return /* it's not error */ GT_BAD_VALUE;
            }
        }
    ...
    /* HW index has not been used during rehashing */
    fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwIndex = hwIndex;

    /* Add second entry when it's applicable */
    if(useNhDataIndex)
    {
        fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwExtIndex = nhDataIndex;
    }
...
/**
* @internal prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck function
* @endinternal
*
* @brief   This function validates HW index usage for rehashed FDB entry from calculated HASH.
*
...
* @retval GT_BAD_VALUE             - HW index already in use
*/
static GT_STATUS  prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck
(
    IN      PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN      GT_U32                                           rehashStageId,
    IN      GT_U32                                           previousBankStep,
    IN      PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *calcInfoPtr
)
{
    GT_U32      hwIndex;
    ...
    {
        /* Selected bank of the last rehashing stage when the free bank is found */
        bankId = calcInfoPtr->selectedBankId;
    }
    ...
    /* HW index has not been used during rehashing */
    fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwIndex = hwIndex;
    /* For IPv6 UC entry update HW index of the entry from adjacent bank
       E.g. IPv6 UC (Stage 0) -> MAC (Stage 1)
            1. Stage 1 Bank 0 HW index = X   ,Bank 1 - N/A    --> update Bank 1 HW index = X+1
            2. Stage 1 Bank 0 HW index - N/A ,Bank 1 = Y      --> update Bank 0 HW index = Y-1
    */
    if(calcInfoPtr->bankStep > 1 || previousBankStep > 1)
    {
        if (bankId % 2 == 0)
        {
            /* Update HW index of the next entry */
            fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwExtIndex = hwIndex+1;
        }
        else
        {
            /* Update HW index of the previous entry */
            fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].stageHwInfo.entryHwExtIndex = hwIndex-1;
        }
    }
...
/**
* @internal prvCpssDxChFdbManagerDbRehashEntry_sip5 function
* @endinternal
*
...
*
*/
GT_STATUS   prvCpssDxChFdbManagerDbRehashEntry_sip5
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC      *fdbManagerPtr,
    IN GT_U32                                              rehashStageId,
    INOUT PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC *calcInfoPtr
...
{
    GT_STATUS                                           rc;
    PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC    calcInfoCurrent; /* hashing info for entry candidate to be moved */
    GT_U32                                              bankId;          /* iterator for rehashed bank */
    CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_STC                 currentEntry;    /* entry candidate to be moved */
    CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_STC                *entryPtr = &currentEntry; /* pointer to entry */
    GT_U32                                              emptyBankId = GT_NA;     /* empty bank in hash array from previose stage */
    GT_U32                                              reservedHwIdx; /* reserved index in hash array from previose stage for IPv6_UC
                                                                          that may be moved to place of non IPV6_UC */

    /* Copy current calc info for the next recursion step */
    calcInfoCurrent = *calcInfoPtr;

    /* Check if current calcInfoPtr holds any empty bank */
    for (bankId = 0; bankId < fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes; bankId += 1)
    {
        if(!fdbManagerPtr->indexArr[calcInfoPtr->crcMultiHashArr[bankId]].isValid_entryPointer)
        {
            emptyBankId = bankId;
            break;
        }
    ...
    for (bankId = 0; bankId < fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes; bankId += 1)
    {
        ...
        calcInfoCurrent.rehashStageBank = bankId;
        ...
        /* init values for each iteration */
        reservedHwIdx = GT_NA;

        /* Get FDB entry per rehash stage bank */
        rc = prvCpssDxChBrgFdbManagerRehashBankFdbEntryGet(fdbManagerPtr,
                calcInfoCurrent.rehashStageBank,
                calcInfoPtr->bankStep,
                calcInfoPtr,
                entryPtr);
        if(rc != GT_OK)
        ...
        /* Cases are eligible for rehash:
         *   1. same size entry found.
         *   2. ipv6 entry found for non-ipv6.
         *   3. Non-ipv6 entry found for ipv6 + Another empty Bank.(emptyBankId != GT_NA)
         */
        if (calcInfoPtr->calcEntryType == PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_EXISTS_E)
        {
            /* check in case of ipv6->non-ipv6_bank_0
             *    Another empty bank availability */
            if((calcInfoPtr->bankStep > 1) && (calcInfoPtr->bankStepArr[bankId] == 1))
            {
                if((emptyBankId == GT_NA))
                {
                    continue;
                }
                calcInfoPtr->nhDataBankNumArr[bankId] = emptyBankId;
                reservedHwIdx = calcInfoPtr->crcMultiHashArr[emptyBankId];
            }
        }
        else if (calcInfoPtr->calcEntryType == PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_NOT_FOUND_AND_NO_FREE_E)
        {
        ...
        rc = prvCpssDxChFdbManagerDbCalcFreeEntryFind_sip5(fdbManagerPtr, &calcInfoCurrent);
        if(rc != GT_OK)
        {
            return rc;
        }

        /* Check HW index is not cross-linked */
        rc = prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck_sip5(fdbManagerPtr, rehashStageId, calcInfoPtr, &calcInfoCurrent,reservedHwIdx);
        if(rc != GT_OK)
        {
        ...
        if (calcInfoCurrent.calcEntryType != PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_FREE_E)
        {
            ...
            rc = prvCpssDxChFdbManagerDbRehashEntry_sip5(fdbManagerPtr, rehashStageId + 1, &calcInfoCurrent);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    {
        /* Calculate the current bank that need to be rehashed */
        calcInfoCurrent.rehashStageBank = bankId % fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes;
        ...
        /* Find free banks in the new HASH for the current FDB entry */
        rc = prvCpssDxChFdbManagerDbCalcFreeEntryFind(fdbManagerPtr, &calcInfoCurrent, calcInfoPtr->bankStep);
        if(rc != GT_OK)
        {
            return rc;
        }
        ...
        /* Check HW index is not cross-linked */
        rc = prvCpssDxChBrgFdbManagerRehashStageFdbEntryHwIndexCheck(fdbManagerPtr, rehashStageId, calcInfoPtr->bankStep, &calcInfoCurrent);
        if(rc != GT_OK)
        {
            /* HW index was used in previous rehashing stages */
            continue;
        ...
        if (calcInfoCurrent.calcEntryType != PRV_CPSS_DXCH_FDB_MANAGER_DB_CALC_ENTRY_TYPE_FOUND_FREE_E)
        {
            ...
            if (rehashStageId < calcInfoCurrent.rehashStageId)
            {
                ...
                /* Save current entry and it's calcInfo in data base */
                calcInfoCurrent.lastBankIdChecked = fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes - 1;
                /* For IPv6 UC data - align rehashing stage bank to IPv6 UC key */
                if (calcInfoCurrent.bankStep == 2 &&
                    calcInfoCurrent.rehashStageBank % 2)
                {
                    calcInfoCurrent.rehashStageBank = calcInfoCurrent.rehashStageBank - 1;
                }
                /* Add current entry and it's hash info to the data base per stage */
                fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].entry    = currentEntry;
                fdbManagerPtr->rehashingTraceArray[rehashStageId - 1].calcInfo = calcInfoCurrent;
...
)
{
    ...
    /* SIP_5-IPv6 can take any 2 banks available, so need to loop for all banks
     * while calculating info */
    if(PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
    {
        for(bankId = 0; bankId < fdbManagerPtr->numOfBanks; bankId += calcInfoPtr->bankStep)
        {
    ...
    else
    {
        for(bankId = 0; bankId < fdbManagerPtr->numOfBanks; bankId += 1)
        {
            rc = prvCpssDxChBrgFdbManagerBankCalcInfoGet_sip5(fdbManagerPtr, bankId, calcInfoPtr->bankStep, entryPtr, addParamsPtr, calcInfoPtr);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    if(calcInfoPtr->bankStep > 1)
    {
        ...
        else
        {
            ...
            {
                nhDataBankNum = calcInfoPtr->nhDataBankNumArr[bankId];
                if(!nhDataBankNum)
                {
                    /* No Valid next hop */
                    continue;
                ...
                /* Check if the bank index already in use
                 * Or the bank is already FULL */
                if((calcInfoPtr->usedBanksArr[bankId] || calcInfoPtr->usedBanksArr[nhDataBankNum]) ||
                        (fdbManagerPtr->bankInfoArr[bankId].bankPopulation == fdbManagerPtr->maxPopulationPerBank) ||
                        (fdbManagerPtr->bankInfoArr[nhDataBankNum].bankPopulation == fdbManagerPtr->maxPopulationPerBank))
                {
                    continue;
                }
...
)
{
    ...
    {
        bankId = calcInfoPtr->nhDataBankNumArr[calcInfoPtr->selectedBankId];
    }
    ...
    /* update indexArr[] */
    /*********************/
    if(fdbManagerPtr->indexArr[hwIndex].isValid_entryPointer == 1)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "the incoming DB entry is not FREE");
    }
...
)
{
    ...
    dbEntryPtr = (primaryBank)?calcInfoPtr->dbEntryPtr:calcInfoPtr->dbEntryExtPtr;
    hwIndex    = dbEntryPtr->hwIndex;
    bankId     = hwIndex % fdbManagerPtr->numOfBanks;
...
)
{
    ...
    for(ii = 0 ; ii < numOfDevs ; ii++)
    {
        ...
        if(fdbManagerPtr->dynamicUcMacLimitPtr)
        {
            ...
            MANAGER_GET_MAC(fdbManagerId)->numOfAllocatedPointers++;
...
)
{
    ...
    /* check all staying after removal devices */
    for(ii = 0; ii < PRV_CPSS_MAX_PP_DEVICES_CNS; ii++)
    {
        if(IS_DEV_BMP_SET_MAC(fdbManagerPtr->devsBmpArr, ii))
        {
            rc = prvCpssDxChFdbManagerDeviceHwFormatGet(fdbManagerPtr, (GT_U8)ii, &currentDeviceType);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    GT_BOOL                                             hwIndexInvalidErrorDetected = GT_FALSE, hwIndexMismatchErrorDetected = GT_FALSE;
    ...
    while(dbEntryPtr->isValid_nextEntryPointer)
    {
        ...
        /* Verify entry from DB points to the entry in index array */
        if((dbIndexPtr->entryPointer != dbIndex) && !hwIndexMismatchErrorDetected)
        {
            PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_USED_LIST_HW_INDEX_MISMATCH_E,
                    errorNumPtr);

            hwIndexMismatchErrorDetected = GT_TRUE;
        }

        /* Verify index is in valid range */
        if ((dbEntryPtr->hwIndex > fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes - 1) && !hwIndexInvalidErrorDetected)
        {
            PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_USED_LIST_HW_INDEX_INVALID_E,
                errorNumPtr);

            hwIndexInvalidErrorDetected = GT_TRUE;
        }
...
)
{
    ...
    for(hwIndex = 0 ; hwIndex < fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; hwIndex++)
    {
        ...
        if(fdbManagerPtr->indexArr[hwIndex].isValid_entryPointer)
        {
            ...
            {
                PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                        CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E,
                        errorNumPtr);
            ...
            if(!indexInvalidEn)
            {
                ...
                if(PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
                {
                    /* IPv6 entry DB index check - two indexes point to same entry */
                    if(dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E)
                    {
                        /* Check that IPv6 Key part is indeed even index */
                        if(dbEntryPtr->hwIndex % 2)
                        {
                            PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                    CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E,
                                    errorNumPtr);
                        ...
                        {
                            if(dbIndex != dbIndexPtr->entryPointer)
                            {
                                PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                        CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_INDEXES_DIFFERENT_E,
                                        errorNumPtr);
                            ...
                            else
                            {
                                ...
                                dbIndex = fdbManagerPtr->indexArr[hwIndex + 1].entryPointer;
                                /* Skip to the next index in the index array */
                                dbEntryPtr = &fdbManagerPtr->entryPoolPtr[dbIndex];
                                dbIndexPtr = &fdbManagerPtr->indexArr[dbEntryPtr->hwIndex];
                    ...
                    else
                    {
                        ...
                        {
                            /* Check that IPv6 Data part is indeed odd index */
                            if(dbEntryPtr->hwIndex % 2 == 0)
                            {
                                PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_ERROR_ASSIGN(resultArray,
                                        CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_DATA_NOT_ODD_INDEX_E,
                                        errorNumPtr);
                            ...
                            {
                                dbIndex = fdbManagerPtr->indexArr[hwIndex - 1].entryPointer;
                                /* Skip to the previous index in the index array */
                                dbEntryPtr = &fdbManagerPtr->entryPoolPtr[dbIndex];
                                dbIndexPtr = &fdbManagerPtr->indexArr[dbEntryPtr->hwIndex];
...
)
{
    ...
    if(checksPtr->learnLimitFidCheckEnable == GT_TRUE)
    {
        ...
        for(ii = 0 ; ii < fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_fid_num; ii++,currLimitPtr++)
        {
       ...
       checksPtr->learnLimitSrcInterfacesCheckEnable == GT_TRUE)
    {
        ...
        if(IS_FDB_MANAGER_VPORT_MODE(fdbManagerPtr))
        {
            /* add to count the entries on the vPort range */
            currLimitPtr = &fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr[0];
            for(ii = 0 ; ii < fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse; ii++,currLimitPtr++)
            {
                portTotalCounter += currLimitPtr->actualCounter;
            }
        }
...
/*
* @internal prvCpssDxChFdbManagerDebugSelfTest_IPv6_UC_Entry function
* @endinternal
*
* @brief   The function Injects/corrects error related to IPv6 UC entry.
*
* @param[in] fdbManagerPtr         - (pointer to) the FDB manager
* @param[in] errorType             - IPv6 UC related error types
* @param[in] errorEnable           - status value indicates error/correct state
*                                       GT_TRUE  - error need to be injected
*                                       GT_FALSE - error need to be corrected
*
* @param[inout] restoreInfoPtr     - (pointer to) FDB manager restore info
* @retval GT_OK                    - on success.
...
*   NONE
*/
static GT_STATUS prvCpssDxChFdbManagerDebugSelfTest_IPv6_UC_Entry
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC      *fdbManagerPtr,
    IN CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_RESULT_ENT       errorType,
    IN GT_BOOL                                              errorEnable,
    INOUT PRV_CPSS_DXCH_FDB_MANAGER_DEBUG_RESTORE_INFO_UNT *restoreInfoPtr
)
{
    ...
    if (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP4_E)
    {
        /* AC5 not supports IPv6 UC entry type */
        return GT_OK;
    }

    dbIndex     = fdbManagerPtr->headOfUsedList.entryPointer;
    dbEntryPtr  = &fdbManagerPtr->entryPoolPtr[dbIndex];
    for(tmpCnt = 0; tmpCnt < fdbManagerPtr->capacityInfo.maxTotalEntries; tmpCnt++)
    {
        dbEntryPtr  = &fdbManagerPtr->entryPoolPtr[dbIndex];
        /* break on IPv6 UC entry */
        if(dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E)
        {
            entryFound = GT_TRUE;
            break;
    ...
    switch(errorType)
    {
        case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_INDEXES_DIFFERENT_E:
            if(errorEnable)
            {
                /* Store IPv6 data HW index */
                restoreInfoPtr->hwIndex = fdbManagerPtr->entryPoolPtr[dbIndex+1].hwIndex;
                /* Invalidate IPv6 data HW index */
                fdbManagerPtr->entryPoolPtr[dbIndex+1].hwIndex +=1;
                SELF_TEST_BMP_SET_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_INDEXES_DIFFERENT_E);
            }
            else
            {
                /* Restore IPv6 data HW index */
                fdbManagerPtr->entryPoolPtr[dbIndex+1].hwIndex = restoreInfoPtr->hwIndex;
                SELF_TEST_BMP_CLR_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_INDEXES_DIFFERENT_E);
            }
            break;
        case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E:
            if(errorEnable)
            {
                /* Store IPv6 DB index */
                restoreInfoPtr->dbIndex = dbIndex;
                /* Invalidate IPv6 DB index data */
                fdbManagerPtr->entryPoolPtr[dbIndex+1].hwFdbEntryType = PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E;
                SELF_TEST_BMP_SET_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E);
            }
            else
            {
                /* Restore IPv6 DB index data */
                dbIndex = restoreInfoPtr->dbIndex;
                fdbManagerPtr->entryPoolPtr[dbIndex+1].hwFdbEntryType = PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E;
                SELF_TEST_BMP_CLR_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E);
            }
            break;
        case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_DATA_NOT_ODD_INDEX_E:
            if(errorEnable)
            {
                /* Store IPv6 DB index */
                restoreInfoPtr->dbIndex = dbIndex;
                /* Invalidate IPv6 DB index data */
                fdbManagerPtr->entryPoolPtr[dbIndex].hwFdbEntryType = PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E;
                SELF_TEST_BMP_SET_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_DATA_NOT_ODD_INDEX_E);
            }
            else
            {
                /* Restore IPv6 DB index data */
                dbIndex = restoreInfoPtr->dbIndex;
                fdbManagerPtr->entryPoolPtr[dbIndex].hwFdbEntryType = PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E;
                SELF_TEST_BMP_CLR_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_DATA_NOT_ODD_INDEX_E);
            }
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
...
)
{
    ...
    for(ii=0; ii<expErrorNum; ii++)
    {
        ...
        if((errorEnable && IS_BMP_SET_MAC(&fdbManagerPtr->selfTestBitmap[0], expResultArray[ii])) ||
           (!errorEnable && !IS_BMP_SET_MAC(&fdbManagerPtr->selfTestBitmap[0], expResultArray[ii])))
        {
            continue;
        }
        ...
        switch(expResultArray[ii])
        {
            ...
            case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E:
            case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_USED_LIST_1ST_PTR_INVALID_E:
                ...
                {/* Inject Error */
                    ...
                    if(expResultArray[ii] == CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E)
                    {
                        restoreInfoPtr->hwIndex = fdbManagerPtr->entryPoolPtr[dbIndex].hwIndex;
                        fdbManagerPtr->entryPoolPtr[dbIndex].hwIndex +=1;
                        SELF_TEST_BMP_SET_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E);
                    }
                    else
                    {
                ...
                {/* Correct last injected error*/
                    if(expResultArray[ii] == CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E)
                    {
                        ...
                        SELF_TEST_BMP_CLR_MAC(fdbManagerPtr, CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_ENTRY_HW_INDEX_MISMATCH_E);
                    }
                    else
                    {
            ...
            case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_KEY_NOT_EVEN_INDEX_E:
            case CPSS_DXCH_BRG_FDB_MANAGER_DB_CHECKS_IPV6_HOST_DATA_NOT_ODD_INDEX_E:
                if(expResultArray[0] == expResultArray[ii])
                {
                    rc = prvCpssDxChFdbManagerDebugSelfTest_IPv6_UC_Entry(fdbManagerPtr, expResultArray[ii], errorEnable, restoreInfoPtr);
                    if(rc != GT_OK)
                    {
                        return rc;
                    }
...
/**
* @internal isOverLimitCheck_vPortNum function
* @endinternal
*
* @brief  function to check if the entry violate the dynamic UC MAC limit : vPortNum
*
* @param[in] fdbManagerPtr         - (pointer to) the FDB Manager.
* @param[in] oper                  - operation type
* @param[in] vPortNum              - the vPortNum of the entry
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Falcon; AC5P; AC5X; Harrier; Ironman; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @retval GT_TRUE              - the entry     violate one of the limits
* @retval GT_FALSE             - the entry not violate any of the limits
*/
static GT_BOOL isOverLimitCheck_vPortNum(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC          *fdbManagerPtr,
    IN LIMIT_OPER_ENT         oper,
    IN GT_PORT_NUM            vPortNum
)
{
    GT_U32  operDiff = OPER_DIFF_MAC(oper);

    if(vPortNum >= fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse)
    {
        /* the vPortNum is too high , we can't check it in the DB */
        return GT_FALSE;
    }

    if((fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr[vPortNum].actualCounter + operDiff) >
        fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr[vPortNum].limitThreshold)
    {
        return GT_TRUE;
    }
    ...
    switch(dbEntryMacPtr->dstInterface_type)
    {
        ...
        case CPSS_INTERFACE_PORT_E:
            ...
            else
            {
                ...
                if(IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,
                    dbEntryMacPtr->dstInterface.devPort.portNum))
                {
                    limitInfoPtr->devPort.hwDevNum = GT_NA;/* indication of 'vPort' , as vPorts are not per device */
                    if(GT_TRUE == isOverLimitCheck_vPortNum(fdbManagerPtr,oper,
                            dbEntryMacPtr->dstInterface.devPort.portNum))
                    {
...
)
{
    ...
    if(devPort.portNum != GT_NA && IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,devPort.portNum))
    {
        if(devPort.portNum >= fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse)
        {
    ...
    if(oper == LIMIT_OPER_ADD_NEW_E || oper == LIMIT_OPER_INTERFACE_UPDATE_E)
    {
        ...
        if(devPort.portNum != GT_NA && IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,devPort.portNum))
        {
            fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr[devPort.portNum].actualCounter ++;
        }
        else
        if(devPort.hwDevNum != GT_NA && devPort.portNum != GT_NA)
        {
    ...
    if(oper == LIMIT_OPER_DELETE_OLD_E || oper == LIMIT_OPER_INTERFACE_UPDATE_E)
    {
        ...
        if(devPort.portNum != GT_NA && IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,devPort.portNum))
        {
            DECREMENT_PARAM_MAC(fdbManagerPtr->dynamicUcMacLimitPtr->dynamicUcMacLimit_vPortNumPtr[devPort.portNum].actualCounter);
        }
        else
        if(devPort.hwDevNum != GT_NA && devPort.portNum != GT_NA)
        {
...
)
{
    ...
    if(IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,portNum))
    {
        /* vPorts are counted 'device less' and not per 'hwDevNum' for this limit */
        return GT_OK;
    }
...
)
{
    ...
    switch(new_interfacePtr->type)
    {
        ...
        case CPSS_INTERFACE_PORT_E:
            ...
            {
                if(IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,
                    old_dbEntryMacPtr->dstInterface.devPort.portNum) &&
                   new_interfacePtr->interfaceInfo.devPort.portNum  ==
                    old_dbEntryMacPtr->dstInterface.devPort.portNum)
                {
                    /* the vPort not changed so not look for vPort violation */
                }
                limitInfoPtr->devPort.hwDevNum = GT_NA;
...
/**
* @internal prvCpssDxChFdbManagerUcRoutEntryPointerCheck function
* @endinternal
*
* @brief  Check if the UC entry pointer type is supported for current device.
*
* @param[in] fdbManagerPtr      - (pointer to) the FDB Manager
* @param[in] entryType          - FDB UC entry type
*
* @retval GT_TRUE               - the UC entry type pointer is supported
* @retval GT_FALSE              - the UC entry type pointer is not supported
*/
GT_BOOL prvCpssDxChFdbManagerUcRoutEntryPointerCheck
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC   *fdbManagerPtr,
    IN CPSS_DXCH_BRG_FDB_MANAGER_UC_ROUTING_TYPE_ENT     entryType
)
{
    if(fdbManagerPtr->hwDeviceTypesBitmap & PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_E)
    {
        if((fdbManagerPtr->hwDeviceTypesBitmap & PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_10_E) == 0)
        {
            if (entryType == CPSS_DXCH_BRG_FDB_MANAGER_UC_ROUTING_TYPE_NEXT_HOP_POINTER_TO_ROUTER_E)
            {
                /* NHE pointer not supported in Falcon devices */
                return GT_FALSE;
            }
        }
    }
