...
/* debug function to print the FDB manager current entries (from the DB) ...
   no HW access

   fdbManagerId - the manager Id
   dbEntryPtr   - (pointer to) DB entry to be printed
   withMetadata - indication to print per entry the 'metadata' in addition to 'entry format'
                  0 - print only 'entry format' without 'metadata'
                  1 - print      'entry format' and the 'metadata'
*/
static GT_STATUS internal_prvCpssDxChFdbManagerDebugPrintDbEntry
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC    *fdbManagerPtr,
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC   *dbEntryPtr,
    IN GT_U32   withMetadata
)
{
    ...
    /* print header */
    prvCpssDxChFdbManagerDebugPrintValidEntries_iterator(fdbManagerPtr,0,dbEntryPtr,
        withMetadata,
        GT_TRUE/* print header */);
    ...
    {
        dbEntryDataPtr = dbEntryPtr;
        rc = prvCpssDxChFdbManagerDbFdbEntryIteratorGetNext(fdbManagerPtr, GT_FALSE, &dbEntryPtr);
        if (rc != GT_OK)
        {
            return rc;
        }
        dbEntryKeyPtr = dbEntryPtr;
        /* print info on this iteration*/
        prvCpssDxChFdbManagerDebugPrintValidIpV6UcEntries_iterator(fdbManagerPtr, 0, dbEntryDataPtr, dbEntryKeyPtr, withMetadata);
    }
    else
    {
        /* print info on this iteration*/
...
/* debug function to print the FDB manager entry (from the DB) ...
   no HW access
   ...
   withMetadata - indication to print per entry the 'metadata' in addition to 'entry format'
                  ...
                  1 - print      'entry format' and the 'metadata'
*/
GT_STATUS prvCpssDxChFdbManagerDebugPrintDbEntry
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC    *fdbManagerPtr,
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC   *dbEntryPtr,
    IN GT_U32   withMetadata
)
{
    ...
    rc = internal_prvCpssDxChFdbManagerDebugPrintDbEntry(fdbManagerPtr,dbEntryPtr,withMetadata);
...
)
{
    ...
    GT_U32                      bankStep = 1;
    GT_U32                      numOfHashes;
    PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC    *dbEntryPtr, *dbEntryExtPtr=NULL;

    numOfHashes = fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes;

    /* fix Klocwork warning */
    if (numOfHashes > CPSS_DXCH_BRG_FDB_MANAGER_MAX_NUM_BANKS_CNS)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }

    for(bankId = 0; bankId < numOfHashes; bankId += bankStep)
    {
...
)
{
    ...
    CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_ENT            fdbEntryType = CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE__LAST__E;
    GT_U32                                              hwIndex = 0, hwIndex1 = 0;

    calcInfo = *calcInfoPtr;
    calcInfo.dbEntryPtr = NULL;
    ...
    for (bankId = 0; bankId < fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes; bankId += calcInfo.bankStepArr[bankId])
    {
        ...
        if(selectedBank == bankId)
        {
            if (calcInfo.bankStepArr[bankId] == 1)
            {
                cpssOsPrintf("bankId[%d], bankHwIndex[0x%x], bankPopulation[%d]*\n", bankId, hwIndex, fdbManagerPtr->bankInfoArr[bankId].bankPopulation);
            }
            else
            {
                hwIndex1 = calcInfo.crcMultiHashArr[bankId+1];
                cpssOsPrintf("bankId[%d],   bankHwIndex[0x%x],  bankPopulation[%d]*\n"
                             "bankId+1[%d], bank1HwIndex[0x%x], bank1Population[%d]\n",
                             bankId,  hwIndex, fdbManagerPtr->bankInfoArr[bankId].bankPopulation,
                             bankId+1,  hwIndex1, fdbManagerPtr->bankInfoArr[bankId+1].bankPopulation);
            }
        }
        else
        {
...
)
{
    ...
    /* copy pointer from global DB */
    globalFdbManagerPtr = (PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC *)PRV_SHARED_FDB_MANAGER_DB_VAR_GET(globalFdbManagerPtr);

    /* Clean last backUp */
    if(globalFdbManagerPtr)
    {
        MEM_FREE_MAC(globalFdbManagerPtr->agingBinInfo.ageBinListPtr);
        MEM_FREE_MAC(globalFdbManagerPtr->agingBinInfo.level1UsageBitmapPtr);
        MEM_FREE_MAC(globalFdbManagerPtr->indexArr);
        MEM_FREE_MAC(globalFdbManagerPtr->entryPoolPtr);
        MEM_FREE_MAC(globalFdbManagerPtr);
    }

    /* alloc memory, assign value to pointer in global DB and BackUp to global memory */
    ...
    /* copy pointer from global DB after allocation above */
    globalFdbManagerPtr = (PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC *)PRV_SHARED_FDB_MANAGER_DB_VAR_GET(globalFdbManagerPtr);
    ...
    MEM_CALLOC_AND_CPY_MAC(globalFdbManagerPtr->agingBinInfo.ageBinListPtr,
    ...
    MEM_CALLOC_AND_CPY_MAC(globalFdbManagerPtr->agingBinInfo.level1UsageBitmapPtr,
    ...
    MEM_CALLOC_AND_CPY_MAC(globalFdbManagerPtr->indexArr, fdbManagerPtr->indexArr, sizeInBytes);

    /* BackUp entryPoolPtr */
    sizeInBytes = fdbManagerPtr->capacityInfo.maxTotalEntries*sizeof(PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC);
    MEM_CALLOC_AND_CPY_MAC(globalFdbManagerPtr->entryPoolPtr, fdbManagerPtr->entryPoolPtr, sizeInBytes);
    return GT_OK;
}
...
)
{
    ...
    *isExpected = GT_FALSE;
    ...
    oldDbIndex      = globalFdbManagerPtr->indexArr[hwIndex].entryPointer;
    ...
    switch(fdbManagerPtr->entryPoolPtr[dbIndex].hwFdbEntryType)
    {
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_MAC_ADDR_E:
            ...
            {
                globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvMacEntryFormat.dstInterface_type =
                    fdbManagerPtr->entryPoolPtr[dbIndex].specificFormat.prvMacEntryFormat.dstInterface_type;
            }
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvMacEntryFormat.age =
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV4_MCAST_E:
            ...
            {
                globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv4McEntryFormat.dstInterface_type =
                    fdbManagerPtr->entryPoolPtr[dbIndex].specificFormat.prvIpv4McEntryFormat.dstInterface_type;
            }
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv4McEntryFormat.age =
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_MCAST_E:
            ...
            {
                globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv6McEntryFormat.dstInterface_type =
                    fdbManagerPtr->entryPoolPtr[dbIndex].specificFormat.prvIpv6McEntryFormat.dstInterface_type;
            }
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv6McEntryFormat.age =
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV4_UC_E:
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv4UcEntryFormat.ipUcCommonInfo.age =
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv6UcKeyEntryFormat.age =
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E:
            globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat.prvIpv6UcDataEntryFormat.ipUcCommonInfo.age =
    ...
    if(cpssOsMemCmp(&fdbManagerPtr->entryPoolPtr[dbIndex].specificFormat,
                &globalFdbManagerPtr->entryPoolPtr[oldDbIndex].specificFormat,
...
)
{
    ...
    if (globalFdbManagerPtr == NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    }

    /* Aging info(Usage bitmap and age-bin ID) verification */
    if(globalFdbManagerPtr->agingBinInfo.level2UsageBitmap != fdbManagerPtr->agingBinInfo.level2UsageBitmap)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "FDB Manager HA: Level2 Usage Bitmap recovered wrongly\n");
    }

    sizeInBytes = sizeof(GT_U32) * (1 + (fdbManagerPtr->agingBinInfo.totalAgeBinAllocated >> 5));
    if(cpssOsMemCmp(globalFdbManagerPtr->agingBinInfo.level1UsageBitmapPtr,
                fdbManagerPtr->agingBinInfo.level1UsageBitmapPtr,
                sizeInBytes))
    {
    ...
    for(hwIndex = 0; hwIndex < fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; hwIndex++)
    {
        ...
        else if(globalFdbManagerPtr->indexArr[hwIndex].isValid_entryPointer == GT_TRUE)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "FDB Manager HA not recovered for hwIndex[%d]\n", hwIndex);
        }
    }
    ...
    for(countertype = 0; countertype < PRV_CPSS_DXCH_FDB_MANAGER_COUNTERS_TYPE__LAST__E; countertype++)
    {
        if(fdbManagerPtr->typeCountersArr[countertype].entryTypePopulation !=
                globalFdbManagerPtr->typeCountersArr[countertype].entryTypePopulation)
        {
    ...
    for(bankId = 0; bankId < CPSS_DXCH_BRG_FDB_MANAGER_MAX_NUM_BANKS_CNS; bankId++)
    {
        if(fdbManagerPtr->bankInfoArr[bankId].bankPopulation !=
                globalFdbManagerPtr->bankInfoArr[bankId].bankPopulation)
        {
    ...
    MEM_FREE_MAC(globalFdbManagerPtr->indexArr);
    MEM_FREE_MAC(globalFdbManagerPtr->entryPoolPtr);
    /* use global variable to free allocated memory and assign NULL to pointer */
...
)
{
    ...
    if(selectedBank < CPSS_DXCH_BRG_FDB_MANAGER_MAX_NUM_BANKS_CNS)
    {
        entryHwIndex = calcInfoPtr->crcMultiHashArr[selectedBank];
    }
    ...
    switch(currentEntryPtr->fdbEntryType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV4_UC_E:
            ...
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "FDB Manager - wrong fdbEntryType\n");
    }

    PRINT_BANKS_ADDRESSES(currentCalcInfoPtr, selectedBank);

    return prvCpssDxChBrgFdbManagerBanksPopulationCheck(fdbManagerPtr, &totalPopulation);
}
...
(
    IN PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC       *fdbManagerPtr,
    OUT GT_U32                                              *totalPopulationPtr
)
{
    GT_U32  bankId;
    GT_U32  totalPopulation = 0;
    GT_U32  totalHwUsedIndexes = 0;
    GT_U32  hwIndex;
    PRV_CPSS_DXCH_FDB_MANAGER_DB_INDEX_STC  *dbIndexPtr = NULL;
    ...
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "FDB Manager inconsistent: total banks population [%d], current banks population[%d]\n",
                                      fdbManagerPtr->totalPopulation, totalPopulation);
    }

    /* Loop over all HW indexes and check all used */
    for (hwIndex = 0; hwIndex < fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; hwIndex++)
    {
        dbIndexPtr = &fdbManagerPtr->indexArr[hwIndex];
        /* Used HW index */
    ...
    if(totalHwUsedIndexes != totalPopulation)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "FDB Manager inconsistent: total banks population [%d], total used HW indexes[%d]\n",
                                      totalPopulation, totalHwUsedIndexes);
    }

    *totalPopulationPtr = totalPopulation;
