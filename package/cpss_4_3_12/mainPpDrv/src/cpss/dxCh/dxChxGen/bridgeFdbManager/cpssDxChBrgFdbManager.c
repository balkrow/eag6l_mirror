...
)
{
    ...
    switch(dstInterfacePtr->type)
    {
        case CPSS_INTERFACE_PORT_E:
            ...
            {
                maxVal = fdbManagerPtr->entryAttrInfo.vportInfo.maxNumOfVPortsToUse;
            }
            else
            {
    ...
    switch(dstInterfacePtr->type)
    {
        case CPSS_INTERFACE_PORT_E:
            ...
            CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
                dstInterfacePtr->interfaceInfo.devPort.portNum,
                maxVal);
            /* hwDevNum not relevant for vPort entry */
            if(!IS_FDB_MANAGER_PORT_IN_VPORT_RANGE(fdbManagerPtr,dstInterfacePtr->interfaceInfo.devPort.portNum))
            {
                CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
                    dstInterfacePtr->interfaceInfo.devPort.hwDevNum,
                    BIT_10);
        ...
        case CPSS_INTERFACE_TRUNK_E:
            ...
            break;
        case CPSS_INTERFACE_VIDX_E:
            maxVal = (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E) ? BIT_17:
                     (fdbManagerPtr->entryAttrInfo.shadowType != CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP4_E) ? BIT_16:
                     BIT_14;
            CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(
                    dstInterfacePtr->interfaceInfo.vidx,
                    maxVal);
...
)
{
    ...
    switch(ucRouteType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_UC_ROUTING_TYPE_NEXT_HOP_POINTER_TO_ROUTER_E:
            ...
            if(GT_FALSE == prvCpssDxChFdbManagerUcRoutEntryPointerCheck(fdbManagerPtr, ucRouteType))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "FDB UC entry NHE pointer is not supported in Falcon");
            }
    ...
    switch(fdbManagerPtr->entryAttrInfo.shadowType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E:
            switch(fdbManagerPtr->learningInfo.macVlanLookupMode)
            {
                case CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_ONLY_E:
                case CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_AND_FID_E:
                    CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(entryPtr->fid      ,BIT_16);
                    break;
                case CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_AND_FID_AND_VID1_E:
                    CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(entryPtr->fid      ,BIT_16);
                    CHECK_X_NOT_EQUAL_OR_MORE_THAN_Y_MAC(entryPtr->vid1     ,BIT_12);
...
)
{
    ...
    maxVal = (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E)?BIT_16:
             (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP4_E)?BIT_12:BIT_13;
...
)
{
    ...
    if (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP6_E ||
        ...
        fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E)
    {
        if (fdbManagerPtr->lookupInfo.ipv4PrefixLength < 32)
        {
            ipv4LookupMask = ((1 << (fdbManagerPtr->lookupInfo.ipv4PrefixLength)) - 1) << (32 - fdbManagerPtr->lookupInfo.ipv4PrefixLength);
        }
        else
        {
            ipv4LookupMask = 0xFFFFFFFF;
        }
        ...
        for (ii = 0; ii < 4; ii++)
        {
            ipv4LookupPattern += entryPtr->ipv4Addr.arIP[3-ii] << (ii*8);
        }
        /* Check if IPv4 address is correct for FDB IPv4 lookup mask */
        if (~ipv4LookupMask & ipv4LookupPattern)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                "invalid entryPtr->ipv4Addr value for lookup mask [%d.%d.%d.%d]",
...
)
{
    ...
    if (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP6_E ||
        ...
        fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP7_E)
    {
        numPrefixWords = fdbManagerPtr->lookupInfo.ipv6PrefixLength/32;
        wordPrefix = fdbManagerPtr->lookupInfo.ipv6PrefixLength % 32;
        if (wordPrefix)
        {
            numPrefixWords++;
        }
        /* Create four words mask value */
        for (ii = 0; ii < 4; ii++)
        {
            if (numPrefixWords == (ii + 1))
            {
                if (wordPrefix != 0)
                {
                    ipv6LookupMask[ii] = ((1 << (wordPrefix)) - 1) << (32 - wordPrefix);
                }
                else
                {
                    ipv6LookupMask[ii] = 0xFFFFFFFF;
                }
            }
            else if (numPrefixWords > ii)
            {
                ipv6LookupMask[ii] = 0xFFFFFFFF;
            }
            else
        ...
        for (ii = 0; ii < 4; ii++)
        {
            ipv6LookupPattern[ii] = entryPtr->ipv6Addr.arIP[(3-ii)*4] << 24 |
                                    entryPtr->ipv6Addr.arIP[(3-ii)*4 + 1] << 16 |
                                    entryPtr->ipv6Addr.arIP[(3-ii)*4 + 2] << 8  |
                                    entryPtr->ipv6Addr.arIP[(3-ii)*4 + 3] ;
        }
        /* Check if IPv6 address is correct for IPv6 lookup mask */
        for (ii = 0; ii < 4; ii++)
        {
            if (~ipv6LookupMask[ii] & ipv6LookupPattern[ii])
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                    "invalid entryPtr->ipv6Addr.arIP value for FDB IPv6 lookup mask [0x %8.8x.%8.8x.%8.8x.%8.8x]",
                    ipv6LookupMask[0],
                    ipv6LookupMask[1],
                    ipv6LookupMask[2],
                    ipv6LookupMask[3]);
...
)
{
    switch(entryPtr->fdbEntryType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV4_UC_E:
            return validateEntryKeyFormatParams_fdbEntryIpV4UcAddrFormat(fdbManagerPtr,
                                                                &entryPtr->format.fdbEntryIpv4UcFormat);
        case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV6_UC_E:
            return validateEntryKeyFormatParams_fdbEntryIpV6UcAddrFormat(fdbManagerPtr,
        ...
        (shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP5_E))
    {
        ...
        switch(entryPtr->mirrorToAnalyzerPort)
        {
            ...
            case CPSS_DXCH_BRG_FDB_MANAGER_MIRROR_TO_ANALYZER_DISABLED_E:
                ...
                dbEntryPtr->daLookupIngressMirrorToAnalyzerPort = 0;
                break;
            case CPSS_DXCH_BRG_FDB_MANAGER_MIRROR_TO_ANALYZER_ON_SA_OR_DA_E:
...
)
{
    ...
    {
        dbEntryPtr->countSet                        = CPSS_IP_CNT_NO_SET_E;
    }
...
)
{
    ...
    {
        case CPSS_DXCH_BRG_FDB_MANAGER_UC_ROUTING_TYPE_NEXT_HOP_INFO_FULL_IN_FDB_TUNNEL_START_PTR_E:
            prvCpssDxChFdbManagerNextHopCommonInfoSet(fdbManagerPtr,&ucRouteInfoPtr->fullFdbInfo, ipUcCommonInfoPtr);
...
)
{
    ...
    {
        entryPtr->countSet = CPSS_IP_CNT_NO_SET_E;
    }
...
)
{
    ...
    /* save to DB format from application format  */
    prvCpssDxChFdbManagerIpUcCommonInfoSet(fdbManagerPtr,entryPtr->ucRouteType,
...
{
    return saveEntryFormatParamsToDb_fdbEntryIpv4UcFormat(fdbManagerPtr,entryPtr, shadowType, dbEntryPtr);
}
...
)
{
    ...
    dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex  = dbEntryExtPtr->hwIndex;
    dbEntryExtPtr->specificFormat.prvIpv6UcDataEntryFormat.keyHwIndex = dbEntryPtr->hwIndex;
    return saveEntryFormatParamsToDb_fdbEntryIpv6UcFormat(fdbManagerPtr,&entryPtr->format.fdbEntryIpv6UcFormat,
                                                           shadowType,
...
)
{
    switch (entryPtr->fdbEntryType)
    {
        ...
        case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV4_UC_E:
            dbEntryPtr->hwFdbEntryType = PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV4_UC_E;
            return updateEntryFormatParamsToDb_fdbEntryIpV4UcRoutingFormat(fdbManagerPtr,
                &entryPtr->format.fdbEntryIpv4UcFormat,
                shadowType,
                &dbEntryPtr->specificFormat.prvIpv4UcEntryFormat);
        default:
...
)
{
    ...
    if (calcInfoPtr->bankStep > 1)
    {
        rc = saveEntryIpV6UcFormatParamsToDb(fdbManagerPtr,entryPtr,
    ...
    {
        rc = saveEntryFormatParamsToDb(fdbManagerPtr,entryPtr,
                fdbManagerPtr->entryAttrInfo.shadowType,
                calcInfoPtr->dbEntryPtr);
        if(rc != GT_OK)
        {
...
)
{
    ...
    **************************************/
    ...
    {
        rc = updateEntryIpV6UcFormatParamsToDb(fdbManagerPtr,entryPtr,
                fdbManagerPtr->entryAttrInfo.shadowType,
                calcInfoPtr->dbEntryPtr,
                calcInfoPtr->dbEntryExtPtr);
        if(rc != GT_OK)
    ...
    {
        rc = updateEntryFormatParamsToDb(fdbManagerPtr,entryPtr,
                fdbManagerPtr->entryAttrInfo.shadowType,
                paramsPtr,
                calcInfoPtr->dbEntryPtr);
        if(rc != GT_OK)
...
* @param[in] previousCalcInfoPtr    - (pointer to) the calculated hash and lookup results for the FDB entry being moved
* @param[out] addApiStatisticsPtr   - (pointer to) the type of API ok/error statistics
*
* @retval GT_OK                     - on success
...
(
    ...
    IN  PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC        *previousCalcInfoPtr,
    OUT PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_ADD_API_STATISTICS_ENT  *addApiStatisticsPtr
)
{
    GT_STATUS                                                   rc;
    ...
    if(previousCalcInfoPtr->bankStep != currentCalcInfoPtr->bankStep)
    {
        rc = prvCpssDxChFdbManagerEntryDelete(fdbManagerPtr, previousCalcInfoPtr, &deleteApiStatistics);
        ...
        if(currentCalcInfoPtr->bankStep > 1)
        {
            if(PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
            {
                /* For IPv6 entry align bank to start from the even index */
                /** Reset nhDataBankNumArr for the old selected bank
                 *      (Reset is at first as the 'old' and 'new' selected bank may be same)
                 *  Update selected bank
                 *  Update nhDataBankNumArr for the new selected bank
                 */
                bankId = currentCalcInfoPtr->selectedBankId;
                currentCalcInfoPtr->nhDataBankNumArr[bankId] = 0;
                currentCalcInfoPtr->selectedBankId = (bankId % 2) ? bankId-1 : bankId;
                currentCalcInfoPtr->nhDataBankNumArr[currentCalcInfoPtr->selectedBankId] =
                    currentCalcInfoPtr->selectedBankId + 1;
            }
        }
...
)
{
    ...
    PRV_CPSS_DXCH_FDB_MANAGER_DB_HASH_CALC_ENTRY_STC            previousCalcInfo;
    ...
    {
        cpssOsPrintf("-------------------------Before Relocation (all stages) ---------------------------------\n");
        for (ii = 0; ii < totalStages; ii++)
        {
            /* Get the entry to be relocated */
            currentEntryPtr     = &fdbManagerPtr->rehashingTraceArray[ii].entry;
            /* Get relocation info relevant to the entry */
            currentCalcInfoPtr  = &fdbManagerPtr->rehashingTraceArray[ii].calcInfo;

            rc = prvCpssDxChBrgFdbManagerRehashInfoPrint(fdbManagerPtr, currentEntryPtr, currentCalcInfoPtr, ii, currentCalcInfoPtr->rehashStageBank);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    for (ii = 0; ii < totalStages; ii++)
    {
        ...
        {
            rc = prvCpssDxChFdbManagerUpdateRelocatedFdbEntry(fdbManagerPtr, currentEntryPtr, currentCalcInfoPtr, &previousCalcInfo, addApiStatisticsPtr);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        previousCalcInfo = *currentCalcInfoPtr;

        /* Get additional HASH information for the entry being moved - used for mixed entries formats logic */
        rc = prvCpssDxChBrgFdbManagerBankCalcInfoGet(fdbManagerPtr, currentCalcInfoPtr->rehashStageBank, currentCalcInfoPtr->bankStep, currentEntryPtr, NULL, &previousCalcInfo);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    rc = prvCpssDxChFdbManagerUpdateRelocatedFdbEntry(fdbManagerPtr, entryPtr, calcInfoPtr, &previousCalcInfo, addApiStatisticsPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    if (PRV_SHARED_FDB_MANAGER_DB_VAR_GET(reHashDebugPrint))
    {
        rc = prvCpssDxChBrgFdbManagerRehashInfoPrint(fdbManagerPtr, entryPtr, calcInfoPtr, 0, calcInfoPtr->selectedBankId);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    {
        /* Rehash FDB entry in the current bank starting from the first stage */
        rc = prvCpssDxChFdbManagerDbRehashEntry_sip5(fdbManagerPtr, 1, calcInfoPtr);
    }
    if (rc != GT_OK)
    {
        return rc;
    ...
    CPSS_LOG_INFORMATION_MAC("The entry not exists and not able to add it (trying 'Cuckoo') ");

    return /* not error */ GT_FULL;
}
...
* @param[in] inputHwIndexExt       - Indicates exact hwIndex of ipv6 Data part.
...
)
{
    ...
    *addApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_ADD_API_STATISTICS_ERROR_INPUT_INVALID_E;
    ...
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "IPv6 not supported in case of Hybrid shadow");
    }
    ...
    *addApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_ADD_API_STATISTICS___LAST___E;
    ...
    if(isInputHwIndexValid)
    {
        ...
        {
            calcInfo.bankStep   = 2;
            /* bank ID is in LSBs of HW Index */
            nhDataBankId        = nhDataHwIndex % fdbManagerPtr->numOfBanks;
            calcInfoPtr->crcMultiHashArr[nhDataBankId]                 = nhDataHwIndex;
            calcInfoPtr->nhDataBankNumArr[calcInfoPtr->selectedBankId] = nhDataBankId;
        }
    }
    else
    {
...
)
{
    ...
    /* call the main logic */
    rc = mainLogicEntryAdd(fdbManagerPtr,entryPtr,paramsPtr,0,0,GT_FALSE,isAddFromLearningscan,&addApiStatistics);
...
)
{
    ...
    *deleteApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_DELETE_API_STATISTICS_ERROR_INPUT_INVALID_E;
    ...
    {
        calcInfoPtr->counterType = PRV_CPSS_DXCH_FDB_MANAGER_COUNTERS_TYPE_IPV6_UNICAST_ROUTE_DATA_E;
        ipv6DataHwIndex = calcInfoPtr->dbEntryExtPtr->hwIndex;
    ...
    {
        rc = prvCpssDxChFdbManagerHwFlushByHwIndex(fdbManagerPtr,
            ipv6DataHwIndex,                                /*hwIndex*/
            ipv6DataHwIndex % fdbManagerPtr->numOfBanks,    /* bankId */
            GT_TRUE);/* we update the bank counters in HW */
        if(rc != GT_OK)
        {
            *deleteApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_DELETE_API_STATISTICS_ERROR_HW_UPDATE_E;
...
)
{
    ...
    *deleteApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRY_DELETE_API_STATISTICS_ERROR_BAD_STATE_E;
    ...
    if(!PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr) &&
            (entryPtr->fdbEntryType == CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV6_UC_E))
    {
        rc = prvCpssDxChFdbManagerDbCalcExistingEntryFind_sip5(fdbManagerPtr, entryPtr, &calcInfo);
    }
    else
    {
...
*          "temp" entry is Storm Protected one. It may be only MAC type.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2.
*
    ...
    *isMatchedPtr = GT_FALSE;
    ...
    switch(hwFormatType)
    {
        ...
        case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP7_E:
            if(hwFormatType!=PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP7_E)/* not valid field in sip7 */
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_SKIP_E, hwValue);
            ...
            SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_VALID_E, hwValue);
            isValid = BIT2BOOL_MAC(hwValue);
            SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_SP_UNKNOWN_E, hwValue);
            ...
            /* MAC address */
            SIPx_FDB_HW_ENTRY_FIELD_MAC_ADDR_GET_MAC(genSip,hwDataArr, macAddr.arEther);
            if(cpssOsMemCmp(&entryPtr->macAddr, &macAddr, sizeof(macAddr)) != 0)
            {
                return GT_OK;
            }
            ...
            if(fdbManagerPtr->learningInfo.macVlanLookupMode !=
                    CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_ONLY_E)
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_FID_E, hwValue);

                if(supportFidBit15)
                {
                    SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP7_FDB_FDB_TABLE_FIELDS_FID_15_E, hwValue1);
                    hwValue |= hwValue1 << 15;/* bit 15 */
                }
            ...
            if(fdbManagerPtr->learningInfo.macVlanLookupMode ==
                    CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_AND_FID_AND_VID1_E)
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_ORIG_VID1_0_E, hwValue);
                U32_SET_FIELD_MAC(vid1, 0, 1, hwValue);

                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_ORIG_VID1_6_1_E, hwValue);
                U32_SET_FIELD_MAC(vid1, 1, 6, hwValue);

                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_ORIG_VID1_11_7_E, hwValue);
...
)
{
    ...
    for(ii = 0 ; ii < numOfDevs ; ii++)
    {
        ...
        /* Update the device types in case of shadow type is hybrid */
        if((fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP5_SIP6_HYBRID_E) ||
            (fdbManagerPtr->entryAttrInfo.shadowType == CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP5_E) ||
...
/*
* @internal prvCpssDxChFdbManagerHwAgeBitStatusGet function
* @endinternal
*
* @brief    The function gets the HW age bit status and
*           update the entryPtr corresponding field according to the type of the entry.
*
* @param[in] fdbManagerPtr         - (pointer to) the FDB manager
* @param[in] dbEntryPtr            - (pointer to) the DB entry
* @param[inout] entryPtr           - (pointer to) FDB entry format with full info.
*
* @retval GT_OK                    - on success.
* @retval GT_FAIL                  - on failure.
* @retval GT_BAD_PARAM             - on wrong parameters
*/
static GT_STATUS prvCpssDxChFdbManagerHwAgeBitStatusGet
(
    IN  PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC          *fdbManagerPtr,
    IN  PRV_CPSS_DXCH_FDB_MANAGER_DB_ENTRY_FORMAT_STC           *dbEntryPtr,
...
)
{
    ...
    rc = prvCpssDxChFdbManagerHwAgedOutVerify(fdbManagerPtr, dbEntryPtr, GT_FALSE, &agedOut);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    {
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_MAC_ADDR_E:
            entryPtr->format.fdbEntryMacAddrFormat.age = (agedOut == GT_FALSE)?GT_TRUE:GT_FALSE;
            break;
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV4_UC_E:
            entryPtr->format.fdbEntryIpv4UcFormat.age = (agedOut == GT_FALSE)?GT_TRUE:GT_FALSE;
            break;
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV4_MCAST_E:
            entryPtr->format.fdbEntryIpv4McFormat.age = (agedOut == GT_FALSE)?GT_TRUE:GT_FALSE;
            break;
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_MCAST_E:
            entryPtr->format.fdbEntryIpv6McFormat.age = (agedOut == GT_FALSE)?GT_TRUE:GT_FALSE;
            break;
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
            entryPtr->format.fdbEntryIpv6UcFormat.age = (agedOut == GT_FALSE)?GT_TRUE:GT_FALSE;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(dbEntryPtr->hwFdbEntryType);
    }
...
* @param[in] fdbManagerId          - the FDB Manager id.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] getAgeStatusFromHw    - get the age bit value from hardware.
*                                      GT_TRUE  - get the age bit value from hardware
*                                      GT_FALSE - get the age bit value from the DB.
...
)
{
    ...
    if(getAgeStatusFromHw)
    {
        rc = prvCpssDxChFdbManagerHwAgeBitStatusGet(fdbManagerPtr, calcInfo.dbEntryPtr, entryPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, fdbManagerId, getAgeStatusFromHw, entryPtr));

    rc = internal_cpssDxChBrgFdbManagerEntryGet(fdbManagerId, getAgeStatusFromHw, entryPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, fdbManagerId, getAgeStatusFromHw, entryPtr));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
...
* @param[in] fdbManagerId          - the FDB Manager id.
*                                    (APPLICABLE RANGES : 0..31)
* @param[in] getFirst              - get the first entry.
*                                      GT_TRUE  - get the first entry
*                                      GT_FALSE - get the next entry from the last fetched position.
* @param[in] getAgeStatusFromHw    - get the age bit value from hardware.
*                                      GT_TRUE  - get the age bit value from hardware
*                                      GT_FALSE - get the age bit value from the DB.
* @param[out] entryPtr             - (pointer to) FDB entry format with full info.
...
)
{
    ...
    /** In case of SIP_5, multiple consecutive ipv6 Data entry can be present,
     * Need to ignore */
    if(!PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
    {
        while(dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E)
        {
    ...
    if (dbEntryPtr->hwFdbEntryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E)
    {
        if(PRV_FDB_MANAGER_IS_SHADOW_SIP6_MAC(fdbManagerPtr))
        {
             ...
             */
            rc = prvCpssDxChFdbManagerDbApplicationIteratorEntryGetNext(fdbManagerPtr,GT_FALSE,&fdbManagerPtr->lastGetNextInfo,&dbEntryDataPtr);
        }
        else
        {
            /* SIP_5 ipv6 entry is not linear */
            prvCpssDxChBrgFdbManagerDbEntryByHwIndexGet(fdbManagerPtr,
                    dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex,
                    &dbEntryDataPtr);
        }
        if(rc != GT_OK)
        {
        ...
        /* convert DB format to application format - IPv6 UC */
        rc = prvCpssDxChBrgFdbManagerRestoreEntryIpV6UcFormatParamsFromDb(fdbManagerPtr, dbEntryPtr, dbEntryDataPtr, entryPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    if(getAgeStatusFromHw)
    {
        rc = prvCpssDxChFdbManagerHwAgeBitStatusGet(fdbManagerPtr, dbEntryPtr, entryPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, fdbManagerId, getFirst, getAgeStatusFromHw, entryPtr));

    rc = internal_cpssDxChBrgFdbManagerEntryGetNext(fdbManagerId, getFirst, getAgeStatusFromHw, entryPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, fdbManagerId, getFirst, getAgeStatusFromHw, entryPtr));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
            SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15_E,
            hwValue);

        associatedVlanId |= 1 << hwValue;/* bit 15 */
    }
    ...
    if(hwValue == 1)
    {
        ...
        SIP5_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                                      ...
                                      hwValue);
        ucRouteInfoPtr->fullFdbInfo.dstInterface.interfaceInfo.vidx = hwValue;
    ...
    else /* not a vidx */
    {
         ...
         else/* port interface */
         {
            ...
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                     SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_USE_VPORT_E,
                     hwValue);
                isVPort = hwValue;
            }
            else
            ...
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_VPORT_E,
                    hwValue);
            }
            else
            {
    ...
    if(!PRV_CPSS_SIP_7_CHECK_MAC(devNum))/* not valid field in sip7 */
    {
        /* countSet */
        SIP5_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
...
)
{
    ...
    PRV_CPSS_CONVERT_HW_VAL_TO_PACKET_CMD_MAC(
        ...
        (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 21, 3));
    PRV_CPSS_CONVERT_HW_VAL_TO_PACKET_CMD_MAC(
        fdbEntryMacPtr->saCommand,
        (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 24, 3));
...
)
{
    ...
    {
        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                                  SIP7_FDB_AU_MSG_TABLE_FIELDS_FID_15_E,
                                  hwValue);
        fdbEntryMacPtr->fid |= hwValue << 15;
    }
    ...
    if(isSip6)
    {
        ...
        else
        {
            ...
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                     SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_USE_VPORT_E,
                     hwValue);
                isVPort = hwValue;
            }
            else
            ...
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_VPORT_0_13_E,
                    hwValue);
                fdbEntryMacPtr->dstInterface.interfaceInfo.devPort.portNum = hwValue;

                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_VPORT_14_17_E,
                    hwValue);
                fdbEntryMacPtr->dstInterface.interfaceInfo.devPort.portNum |= hwValue << 14;
            }
            else
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
    ...
    if(isMacMove)
    {
        ...
        fdbEntryMacPtr->saCommand       = CPSS_PACKET_CMD_FORWARD_E;
        fdbEntryMacPtr->daCommand       = CPSS_PACKET_CMD_FORWARD_E;
    ...
    else
    {
        ...
        if(isSip6)
        {
            ...
            SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    ...
                    hwValue);
            SIP6_CONVERT_HW_DA_PACKET_CMD_TO_SW_VAL_MAC(
                devNum, fdbEntryMacPtr->daCommand, hwValue);

            /* saCommand */
            SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP6_FDB_AU_MSG_TABLE_FIELDS_SA_CMD_E,
                    hwValue);
            SIP6_CONVERT_HW_SA_PACKET_CMD_TO_SW_VAL_MAC(
                devNum, fdbEntryMacPtr->saCommand, hwValue);
...
)
{
    ...
    ipMulticastPtr->sipAddr[3] = (GT_U8)(sipAddr >> 0);
    ipMulticastPtr->sipAddr[2] = (GT_U8)(sipAddr >> 8);
    ipMulticastPtr->sipAddr[1] = (GT_U8)(sipAddr >> 16);
    ipMulticastPtr->sipAddr[0] = (GT_U8)(sipAddr >> 24);
    ...
    if(isSip6)
    {
        ...
        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                ...
                hwValue);
        SIP6_CONVERT_HW_DA_PACKET_CMD_TO_SW_VAL_MAC(
            devNum, ipMulticastPtr->daCommand, hwValue);
    }
    else
    {
        /* daCommand */
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        switch(hwValue)
        {
            case HW_SIP7_FDB_UPDATE_MSG_NA_E:
                hwValue = CPSS_NA_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_MOVED_E:
                hwValue = CPSS_NA_E;
                sip7IsMoved = 1;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_QA_E:
                hwValue = CPSS_QA_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_QR_E:
                hwValue = CPSS_QR_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_AA_E:
                hwValue = CPSS_AA_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_TA_E:
                hwValue = CPSS_TA_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_FU_E:
                hwValue = CPSS_FU_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_QI_E:
                hwValue = CPSS_QI_E;
                break;
            case HW_SIP7_FDB_UPDATE_MSG_HR_E:
                hwValue = CPSS_HR_E;
                break;
            default:
                CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(hwValue);
    ...
    else
    {
        ...
        {
            SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                 SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_OLD_USE_VPORT_E,
                 hwValue);
            isVPort = hwValue;
        }
        else
        ...
        {
            SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_OLD_VPORT_0_13_E,
                hwValue);
            eventPtr->updateInfo.movedEntryFormat.oldInterface.interfaceInfo.devPort.portNum = hwValue;

            SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                SIP7_FDB_AU_MSG_TABLE_FIELDS_NA_MOVED_OLD_VPORT_14_17_E,
                hwValue);
            eventPtr->updateInfo.movedEntryFormat.oldInterface.interfaceInfo.devPort.portNum |= hwValue << 14;
        }
        else
        {
            SIP5_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
...
*
* @param[in]  devNum                  - number of device whose descriptor queue is processed
* @param[in]  auMsgArrayOfWordsPtr    - (pointer to) the AU raw words in HW format filled by PP.
...
)
{
    ...
    auMsgArrayOfWordsPtr[0] &= macAddrOffsetMask;

    /* Check if entry exist */
    for(i=0; i<cache->usedEntriesCnt; i++)
    {
        if(cpssOsMemCmp(&cache->noSpaceKeyCache[i], auMsgArrayOfWordsPtr, PRV_CPSS_DXCH_FDB_AU_NA_TO_CPU_MSG_DATA_BYTES_SIZE_CNS) == 0)
        {
    ...
    cpssOsMemCpy(&cache->noSpaceKeyCache[key], auMsgArrayOfWordsPtr, PRV_CPSS_DXCH_FDB_AU_NA_TO_CPU_MSG_DATA_BYTES_SIZE_CNS);
...
*                                        ignored when sip7 device
* @param[out] sip7HwAuMessagesPtr      - sip7 HW AU/FU messages raw format
*                                        ignored when non-sip7 device
...
(
    ...
    IN  PRV_CPSS_AU_FU_MSG_SIP7_STC                        *sip7HwAuMessagesPtr,
...
{
    GT_U32                                            eventArrIndex;/* Index to the OUT event Array - only updates successful entries */
    ...
    GT_BOOL                                           auMsgProcessed;     /* Indicates if AU message is processed */
    ...
    for (auIndex = fdbManagerPtr->tempInfo.lastHwAuMessageIndex; auIndex < numOfAu; auIndex++)
    {
        ...
        {
            auMsgArrayOfWordsPtr = &sip7HwAuMessagesPtr[auIndex].fdbMsgWords[0];
        }
        else
        {
            /* consider the AU message as array of words .
               by this we can treat it as 'fields format in table entry' */
            auMsgArrayOfWordsPtr = (GT_U32*)(&hwAuMessagesPtr[auIndex].elem0);
        }

        /* parse the AU message from HW format to event format */
        rc = au2EventMsg(devNum, auMsgArrayOfWordsPtr, eventPtr);
        if (rc != GT_OK)
        {
            continue;
        }
        ...
        switch (eventPtr->updateType)
        {
            ...
            case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_UPDATE_EVENT_TYPE_NEW_E:
                ...
                if (scanParamsPtr->addNewMacUcEntries)
                {
                    ...
                    {
                        /* Ignore statistics update, which are already counted as a part of Add API logic */
                        case GT_OK:
                            if(eventPtr->updateType == CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_UPDATE_EVENT_TYPE_NO_SPACE_E)
                            {
                                /* New address added. Update message type from NO_SPACE to NEW_ADDR */
                                eventPtr->updateType = CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_UPDATE_EVENT_TYPE_NEW_E;
                            }
                            break;
                        case GT_ALREADY_EXIST:
                        case GT_FULL:
                            /* No need to pass event message to application (just move to next message) */
                            auMsgProcessed = GT_FALSE;
                            break;
                        case GT_LEARN_LIMIT_PORT_ERROR           :
                ...
                /* The entry was updated in FDB manager and HW */
                if (auMsgProcessed == GT_TRUE)
                {
                    /* increment the number of events */
                    eventArrIndex++;
                }
    ...
    while(nextValidDeviceRc == GT_OK)
    {
        ...
        else
        {
            ...
            rc = prvDxChBrgFdbAuFuMsgBlockGet(devNum, MESSAGE_QUEUE_PRIMARY_AUQ_E,
                    ...
                    fdbManagerPtr->tempInfo.sip7_hwAuMessages,
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
* @note If application works in application learning control (paramsPtr->addNewMacUcEntries == GT_TRUE && paramsPtr->updateMovedMacUcEntries == GT_TRUE):
*  - For every New Entry update application receives it must call Add-Entry or Delete temp entry (to flush temporal entry HW prepared)
*  - For every Moved entry update application received it must call Update entry (with updateOnlySrcInterface = True) or Delete entry (if it doesn't accept this update)
*  - For every Entry No-space event application may call Add-Entry API (to try Cuckoo) or simply ignore it.
*/
...
)
{
    ...
    GT_U8                                               devNum;
    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC      *fdbManagerPtr;
    GT_U32                                              auMsgHwWords[16];/*16 words to support sip7*/
    GT_U32                                              index;
    ...
    for(index = 0; index<fdbManagerPtr->noSpaceCacheEntry.usedEntriesCnt; index++)
    {
        ...
        cpssOsMemCpy(&auMsgHwWords[0], &(fdbManagerPtr->noSpaceCacheEntry.noSpaceKeyCache[index]), sizeof(auMsgHwWords));

        /* Parse the HW format to DB format */
        rc = au2EventMsg(devNum, &auMsgHwWords[0], &updateEvent);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    {
        /* IPv6 Data entry */
        hwIndexExt    = dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
        bankId        = hwIndexExt % fdbManagerPtr->numOfBanks;

        calcInfo.bankStep                 = 2;
        calcInfo.dbEntryExtPtr            = &fdbManagerPtr->entryPoolPtr[fdbManagerPtr->indexArr[hwIndexExt].entryPointer];
        calcInfo.crcMultiHashArr[bankId]  = calcInfo.dbEntryExtPtr->hwIndex;
    }
    else
    {
        calcInfo.bankStep                 = 1;
...
)
{
    ...
    switch(dbEntryIpUcPtr->dstInterface_type)
    {
        ...
        case CPSS_INTERFACE_PORT_E:
            ...
            {
                /* not check hwDevNum that is not part of the entry */
                /* not check hwDevNum as the FDB manager treat it as device less */
            }
            else
...
)
{
    ...
    switch(dbEntryMacPtr->dstInterface_type)
    {
        ...
        case CPSS_INTERFACE_TRUNK_E:
            ...
            FDB_SCAN_VALIDATE_FILTER_MAC_AND_RETURN_MAC(paramsPtr->userDefinedMask, dbEntryMacPtr->userDefined, paramsPtr->userDefined);
            /* Device specific trunk aging is not supported */
            FDB_SCAN_VALIDATE_EXACT_MATCH_AND_RETURN_MAC(paramsPtr->hwDevNumMask_trunk, 0);
    ...
    *deleteEnPtr = GT_FALSE;
    switch(dbEntryPtr->hwFdbEntryType)
    {
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
            ...
            nhHwIndex     = dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
            dbEntryExtPtr   = &fdbManagerPtr->entryPoolPtr[fdbManagerPtr->indexArr[nhHwIndex].entryPointer];
            return validateAgingParamsEntryTypeAndFilters_ip_unicast(fdbManagerPtr,paramsPtr,
    ...
    *agingScanStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_ERROR_FAILED_TABLE_UPDATE_E;
    ...
    while(rc != GT_NO_MORE)
    {
        ...
        if(isValid)
        {
            ...
            rc = prvCpssDxChFdbManagerHwAgedOutVerify(fdbManagerPtr, dbEntryPtr, GT_TRUE, &agedOut);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    switch(dbEntryPtr->hwFdbEntryType)
    {
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
            ...
            hwIndexExt                            = dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
            bankId                                = hwIndexExt % fdbManagerPtr->numOfBanks;
            calcInfo.crcMultiHashArr[bankId]      = hwIndexExt;
            calcInfo.dbEntryExtPtr                = &fdbManagerPtr->entryPoolPtr[fdbManagerPtr->indexArr[hwIndexExt].entryPointer];
    ...
    *isValidPtr  = GT_FALSE;
    switch(dbEntryPtr->hwFdbEntryType)
    {
        ...
        case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
            ...
            nhHwIndex      = dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
            dbIndexExtPtr  = fdbManagerPtr->indexArr[nhHwIndex].entryPointer;
...
/*
* @internal prvCpssDxChFdbManagerHaIpv6DataGet_sip5 function
* @endinternal
*
* @brief   The function gets ipv6 data part, in case of HA,
*          In case of SIP_5 ipv6 data part can reside in any bank next to the key bank.
*          HA finds the Data entry as below:
*               - Read key entry, find the nextHopDataBankId.
*               - Read all the entry which belongs to that bankId
*                   - If IPv6 Data, Gets the keyIndex stored in ipv6-data
*                       - if invalid or skip - continue;
*                       - if valid check the "keyIndex" with the incoming key hwIndex
*                           - Return if match found
*
* @param[in] devNum                - device number.
* @param[in] fdbManagerPtr         - (pointer to) the FDB manager
* @param[in] keyHwIndex            - hwIndex of ipv6 key entry
* @param[out] nhDataHwIndexPtr     - exact HW index for ipv6 data part
* @param[out] isValidPtr           - valid value for ipv6 data part.
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_STATE             - Ipv6 Data entry not found
*/
static GT_STATUS prvCpssDxChFdbManagerHaIpv6DataGet_sip5
(
    IN  GT_U8                                              devNum,
    IN  PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC    *fdbManagerPtr,
    IN  GT_U32                                             keyHwIndex,
    OUT GT_U32                                            *nhDataHwIndexPtr,
    OUT GT_U32                                            *isValidPtr
)
{
    GT_STATUS                                           rc;
    GT_U32                                              dataHwIndex = 0;
    GT_U32                                              hwValue;
    GT_U32 *hwDataArr    = &fdbManagerPtr->tempInfo.sip6_hwFormatWords[0];
    GT_U32 *hwDataExtArr = &fdbManagerPtr->tempInfo.sip6_ipv6Ext_hwFormatWords[0];

    /* Non SIP_6 - In case of DB does not exist (HA) */
    /* Get the Data bank ID from the HW Data */
    SIP5_FDB_HW_ENTRY_FIELD_GET_MAC(hwDataArr, SIP5_FDB_FDB_TABLE_FIELDS_NH_DATA_BANK_NUM_E, hwValue);

    /* Scans specific bank, where the Data resides */
    dataHwIndex = hwValue;
    for(;dataHwIndex<fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; dataHwIndex+=fdbManagerPtr->numOfBanks)
    {
        rc = prvCpssDxChPortGroupReadTableEntry(devNum,
                0,
                CPSS_DXCH_TABLE_FDB_E,
                dataHwIndex,                 /* IPv6 Data Part, next HwIndex */
                hwDataExtArr);
        if(rc != GT_OK)
        {
            return rc;
        ...
        /* check for non-ipv6-data entry */
        SIP5_FDB_HW_ENTRY_FIELD_GET_MAC(hwDataExtArr, SIP5_FDB_FDB_TABLE_FIELDS_FDB_ENTRY_TYPE_E, hwValue);
        if(hwValue != PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E)
        {
            continue;
        }
        ...
        *isValidPtr = hwValue;
        ...
        /* check for skip entry */
        SIP5_FDB_HW_ENTRY_FIELD_GET_MAC(hwDataExtArr, SIP5_FDB_FDB_TABLE_FIELDS_SKIP_E, hwValue);
        if(hwValue == 1)
        {
            continue;
        }

        /* Check in the Data entry, if stored hwIndex of the key matches */
        SIP5_FDB_HW_ENTRY_FIELD_GET_MAC(hwDataExtArr, SIP5_FDB_FDB_TABLE_FIELDS_NH_KEY_HW_INDEX_E, hwValue);
        if(keyHwIndex == hwValue)
        {
            *nhDataHwIndexPtr = dataHwIndex;
            return GT_OK;
        }
    }
    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "not found ipv6 entry's Data part");
}
...
* @param[in] fdbManagerPtr        - (pointer to) the FDB Manager.
* @param[out] nhDataHwIndexPtr    - (pointer to) the exact hwIndex value of ipv6 data part.
...
)
{
    ...
    *isValidPtr = GT_FALSE;
    ...
    if(hwValue == 1 && (isSkip == GT_FALSE))
    {
        ...
        {
            isIpv6 = GT_TRUE;
            rc = prvCpssDxChFdbManagerHaIpv6DataGet_sip5(devNum, fdbManagerPtr, hwIndex, nhDataHwIndexPtr, &hwValue/*isValid*/);
            if(rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    *isValidPtr = GT_FALSE;
    ...
    {
        SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_VALID_E, isValid);
        ...
        else    /* sip7*/
        {
            ...
            if(fdbManagerPtr->capacityInfo.hwCapacity.numOfHashes == 1 &&
               fdbManagerPtr->capacityInfo.hwCapacity.mode == CPSS_MAC_HASH_FUNC_XOR_E &&
               entryType <= PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_MCAST_E)
            {
                /* skip */
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataArr, SIP6_FDB_FDB_TABLE_FIELDS_SKIP_E, hwValue);
                isSkip = BIT2BOOL_MAC(hwValue);
    ...
    if(isValid == 1 && (isSkip == GT_FALSE))
    {
        ...
        if(entryType == PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E)
        {
            ...
            {
                SIPx_FDB_HW_ENTRY_FIELD_GET_MAC(genSip,hwDataExtArr, SIP6_FDB_FDB_TABLE_FIELDS_VALID_E, hwValue);
            }
            if(hwValue == 1)
            {
                *isValidPtr = GT_TRUE;
...
)
{
    ...
    for(hwIndex = 0; hwIndex<fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes; hwIndex++)
    {
        ...
        switch(hwFormatType)
        {
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP5_E:
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP5_20_E:
                rc = prvCpssDxChFdbManagerHaReadEntry_sip5(devNum, hwIndex, fdbManagerPtr, &nhDataHwIndex, &addParams, &isValid);
                break;
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_E:
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP6_10_E:
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP4_E:
            case PRV_CPSS_DXCH_BRG_FDB_MANAGER_HW_FORMAT_TYPE_SIP7_E:
                ...
                /* will be used only in case of entry type is IPv6 */
                nhDataHwIndex = hwIndex+1;
                break;
            default:
                UNLOCK_DEV_NUM(devNum);
        ...
        if(isValid)
        {
            ...
            /* call the main logic */
            rc = mainLogicEntryAdd(fdbManagerPtr, &entry, &addParams, hwIndex, nhDataHwIndex,
                    GT_TRUE, GT_FALSE, &addApiStatistics);
...
/**
* @internal falconShmSmbMemoryEntryRewrite function
* @endinternal
*
* @brief   The function rewrites damaged SBM entries in memories 4 and 5 out of FDB bank.
*
* @note   APPLICABLE DEVICES:      Falcon.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; AC5P; AC5X; Harrier; Ironman; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
* @param[in] fdbManagerPtr         - (pointer to) the FDB Manager.
* @param[in] hwEntryIndex          - The HW Entry index.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS falconShmSmbMemoryEntryRewrite
(
    IN  PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC     *fdbManagerPtr,
    IN  GT_U32                                             hwEntryIndex
)
{
    GT_STATUS rc;
    GT_U8  devNum;
    ...
    firstDev = GT_TRUE;
    for (firstDev = GT_TRUE;
          (GT_OK == prvCpssDxChFdbManagerDbDeviceIteratorGetNext(fdbManagerPtr, firstDev, &devNum));
          firstDev = GT_FALSE)
    {
        LOCK_DEV_NUM(devNum);
        isFalcon =
            PRV_CPSS_IS_DEV_EXISTS_MAC(devNum)
            && (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E);
        UNLOCK_DEV_NUM(devNum);
        if (isFalcon == 0) continue;

        location.info.hwEntryInfo.hwTableEntryIndex = hwEntryIndex;
        rc = cpssDxChDiagDataIntegrityTableEntryFix(
            devNum, &location);
        if (rc != GT_OK)
        {
            return rc;
...
)
{
    ...
    *rewriteApiStatisticsPtr = PRV_CPSS_DXCH_FDB_MANAGER_ENTRIES_REWRITE_API_STATISTICS_ERROR_INPUT_INVALID_E;
    ...
    for(indexNum=0; indexNum<entriesIndexesNum; indexNum++)
    {
        ...
        {
            /* Falcon SBM memories 4 and 5 out of FDB bank */
            rc = falconShmSmbMemoryEntryRewrite(fdbManagerPtr, hwIndex);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        if(fdbManagerPtr->indexArr[hwIndex].isValid_entryPointer)
        {
            ...
            switch(calcInfo.dbEntryPtr->hwFdbEntryType)
            {
                ...
                case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_KEY_ENTRY_E:
                    ...
                    nhDataHwIndex           = calcInfo.dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.nhDataHwIndex;
                    dbIndexPtr              = &fdbManagerPtr->indexArr[nhDataHwIndex];
                ...
                case PRV_CPSS_DXCH_FDB_MANAGER_HW_FDB_ENTRY_TYPE_IPV6_UC_DATA_ENTRY_E:
                    ...
                    /* Key Hw Index */
                    nhDataHwIndex           = calcInfo.dbEntryExtPtr->specificFormat.prvIpv6UcDataEntryFormat.keyHwIndex;
                    dbIndexPtr              = &fdbManagerPtr->indexArr[nhDataHwIndex];
                    calcInfo.dbEntryPtr     = &fdbManagerPtr->entryPoolPtr[dbIndexPtr->entryPointer];
                    calcInfo.dbEntryPtr->specificFormat.prvIpv6UcKeyEntryFormat.age                     = 1;
                    calcInfo.dbEntryExtPtr->specificFormat.prvIpv6UcDataEntryFormat.ipUcCommonInfo.age  = 1;
...
/**
* @internal internal_cpssDxChBrgFdbManagerEntryTempGetNext function
* @endinternal
*
* @brief  The API scans the HW indexes, starting from the first one,
*         reads FDB entries from all HW tiles and looks up for temp (SP) entries in any one of them.
*         If  entry is found, the API fetches temp entry data - either first one or next
*         relative to the last retrieved entry.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2.
*
* @param[in] fdbManagerId           - the FDB Manager id
*                                       (APPLICABLE RANGES : 0..31)
* @param[in] getFirst               - read from the first entry
*                                       GT_TRUE  - start reading entry from the first index
*                                       GT_FALSE - start reading entry at the index following the last fetched one
* @param[inout] entryIndexPtr       - (pointer to) HW index of the entry to start reading from ('input parameter')
*                                                  HW index of the fetched entry, in case an SP entry is found ('output parameter')
*                                       If input entryIndex = 0, start reading entry from the first index
* @param[in] indexesNumToCheck      - number of HW indexes to be checked per one cycle starting from the input entryIndex
*                                       If indexesNumToCheck == 0xFFFFFFFF, read all possible indexes
*
* @param[out] entryPtr              - (pointer to) SP entry in format of the FDB manager
* @param[in]  deleteEntry           - specifies whether to flush entry at HW index from all tiles if SP entry is found
*                                       GT_TRUE  - If SP entry detected, flush HW index from all tiles.
*                                                  If there are tiles with valid entries and other one(s) with SP entries,
*                                                  the lowest valid entry should be written to all
*                                       GT_FALSE - don't flush/update HW index with SP entry
*
* @retval GT_OK                     - on success
* @retval GT_FAIL                   - on error
* @retval GT_HW_ERROR               - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChBrgFdbManagerEntryTempGetNext
(
    IN GT_U32 fdbManagerId,
    IN GT_BOOL getFirst,
...
)
{
    ...
    PRV_CPSS_DXCH_FDB_MANAGER_DB_INSTANCE_INFO_STC *fdbManagerPtr;  /* pointer to FDB manager */
    GT_BOOL                                         tempEntryFound; /* SP entry found */
    GT_U32                                          hwIndex;        /* HW index used in iterator */
    GT_U32                                          currentIndex;   /* HW index of the entry to start reading from */
    GT_U32                                          numOfReadEntries = 0; /* Number of HW entries used in iterator  */
    ...
    currentIndex = *entryIndexPtr;

    if(getFirst == GT_FALSE && currentIndex == 0)
    {
    ...
    {
        hwIndex = currentIndex + 1;
    }

    if (indexesNumToCheck == 0xFFFFFFFF)
    {
        numOfReadEntries = fdbManagerPtr->capacityInfo.hwCapacity.numOfHwIndexes;
    }
    else
    {
        numOfReadEntries = hwIndex + indexesNumToCheck;
    }

    for(; hwIndex < numOfReadEntries; hwIndex++)
    {
        rc = prvCpssDxChBrgFdbManagerHwTempEntryReadByIndex(fdbManagerPtr, hwIndex, entryPtr, &tempEntryFound, deleteEntry);
        if(rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal cpssDxChBrgFdbManagerEntryTempGetNext function
* @endinternal
*
* @brief  The API scans the HW indexes, starting from the first one,
*         reads FDB entries from all HW tiles and looks up for temp (SP) entries in any one of them.
...
*
*/
GT_STATUS cpssDxChBrgFdbManagerEntryTempGetNext
(
    IN GT_U32 fdbManagerId,
    IN GT_BOOL getFirst,
    INOUT GT_U32 *entryIndexPtr,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgFdbManagerEntryTempGetNext);

    CPSS_API_LOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, fdbManagerId, getFirst, entryIndexPtr, indexesNumToCheck, entryPtr, deleteEntry));

    rc = internal_cpssDxChBrgFdbManagerEntryTempGetNext(fdbManagerId, getFirst, entryIndexPtr, indexesNumToCheck, entryPtr, deleteEntry);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, fdbManagerId, getFirst, entryIndexPtr, indexesNumToCheck, entryPtr, deleteEntry));
    CPSS_API_UNLOCK_DEVICELESS_MAC(PRV_CPSS_FUNCTIONALITY_FDB_MANAGER_CNS);
