...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, ttiRuleIndex, profileNum));
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, ttiRuleIndex, profileNumPtr));
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, srcPortNum, profileNum));
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, srcPortNum, profileNumPtr));
...
)
{
    ...
    for (i=0; i<CPSS_DXCH_PPU_KSTG_KEY_GEN_PROFILE_BYTE_SELECT_MAX_CNS; i++)
    {
        if (keygenProfilePtr->byteSelOffset[i] > CPSS_DXCH_PPU_KSTG_KEY_GEN_PROFILE_BYTE_SELECT_MAX_VALUE_CNS)
        {
...
)
{
    ...
    CPSS_DATA_CHECK_MAX_MAC(index, CPSS_DXCH_PPU_KSTG_TCAM_ENTRIES_MAX_CNS);

    CPSS_NULL_PTR_CHECK_MAC(tcamEntryPtr);

    rc = prvCpssDxChPpuKstgTcamEntrySet(devNum, kstgNum, index, tcamEntryPtr);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, kstgNum, index, tcamEntryPtr));

    rc = internal_cpssDxChPpuKstgTcamEntrySet(devNum, kstgNum, index, tcamEntryPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, kstgNum, index, tcamEntryPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal  internal_cpssDxChPpuDauProtectedWindowErrorLatchDataGet function
* @endinternal
*
* @brief  This function gets DAU protected window error latched data entry.
*
* @note   APPLICABLE DEVICES:  AC5P; AC5X; Harrier; Ironman
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; BobCat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                 - device number
* @param[out] protWinIdxPtr         - pointer to PPU DAU protection error latched window index.
* @param[out] errorLatchDataPtr     - pointer to PPU DAU protection error latched data.
* @param[out] spByteIndexPtr        - pointer to PPU DAU protection error latch SP byte index.
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
* @retval GT_OUT_OF_RANGE          - one of the parameters is out of range
...
*
*/
GT_STATUS internal_cpssDxChPpuDauProtectedWindowErrorLatchDataGet
(
    IN   GT_U8                                       devNum,
    OUT  GT_U32                                      *protWinIdxPtr,
    OUT  GT_U32                                      *errorLatchDataPtr,
    OUT  GT_U32                                      *spByteIndexPtr
)
{
    GT_STATUS   rc;
    ...
    CPSS_NULL_PTR_CHECK_MAC(protWinIdxPtr);
    CPSS_NULL_PTR_CHECK_MAC(errorLatchDataPtr);
    CPSS_NULL_PTR_CHECK_MAC(spByteIndexPtr);

    rc = prvCpssDxChPpuDauProtectedWindowErrorLatchDataGet(devNum,
                                                           protWinIdxPtr,
                                                           errorLatchDataPtr,
                                                           spByteIndexPtr);
...
/**
* @internal  cpssDxChPpuDauProtectedWindowErrorLatchDataGet function
* @endinternal
*
* @brief  This function gets DAU protected window error latched data entry.
*
...
*
*/
GT_STATUS cpssDxChPpuDauProtectedWindowErrorLatchDataGet
(
    IN   GT_U8                                       devNum,
    OUT  GT_U32                                      *protWinIdxPtr,
    OUT  GT_U32                                      *errorLatchDataPtr,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId,
                                       cpssDxChPpuDauProtectedWindowErrorLatchDataGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, protWinIdxPtr,
                            errorLatchDataPtr, spByteIndexPtr));

    rc = internal_cpssDxChPpuDauProtectedWindowErrorLatchDataGet(devNum,
                                                                 protWinIdxPtr,
                                                                 errorLatchDataPtr,
                                                                 spByteIndexPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, protWinIdxPtr,
                              errorLatchDataPtr, spByteIndexPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
* @endinternal
*
* @brief    Set error profile for a given profile number.
*
* @note   APPLICABLE DEVICES:  AC5P; AC5X; Harrier; Ironman
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; BobCat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @endinternal
*
* @brief    Get error profile for a given profile number.
*
* @note   APPLICABLE DEVICES:  AC5P; AC5X; Harrier; Ironman
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; BobCat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
