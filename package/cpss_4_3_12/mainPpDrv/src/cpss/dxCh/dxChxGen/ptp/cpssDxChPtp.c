...
#define PRV_CPSS_DXCH_TAI_CHECK_MAC(_dev, _taiNumber)                                           \
    switch (PRV_CPSS_PP_MAC(_dev)->devType)                                                     \
    {                                                                                           \
        case CPSS_AC5Y_DEVICES_CASES_MAC:                                                       \
            if ((_taiNumber != CPSS_DXCH_PTP_TAI_NUMBER_0_E) && (_taiNumber != CPSS_DXCH_PTP_TAI_NUMBER_4_E))\
...
#define PRV_CPSS_DXCH_PTP_TAI_SELECTION_BMP_CHECK_MAC(_dev,_bmp)                                \
    ...
    {                                                                                           \
        case CPSS_AC5Y_DEVICES_CASES_MAC:                                                       \
            if ((_bmp != 1 /*TAI0*/) && (_bmp != 0x10 /*TAI4*/) && (_bmp != 0x11 /*TAI0&4*/))   \
...
/* CP clock TAI frequency */
#define HARRIER_PTP_CP_800_CLK_CNS                  800000000 /* 800 MHz */
#define HARRIER_PTP_CP_600_CLK_CNS                  600000000 /* 600 MHz */
#define HARRIER_PTP_CP_200_CLK_CNS                  200000000 /* 200 MHz */
        ...
        case PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STATE_START_E:
            ...
            switch (taiIdPtr->taiNumber)
            {
                ...
                case CPSS_DXCH_PTP_TAI_NUMBER_1_E:
                    if (isAc5y == GT_TRUE || PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
                    {
                        iteratorPtr->state = PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STATE_NO_MORE_E;
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
                    }
                ...
                case CPSS_DXCH_PTP_TAI_NUMBER_2_E:
                    if (PRV_CPSS_PP_MAC(devNum)->devFamily != CPSS_PP_FAMILY_DXCH_IRONMAN_E || isAc5y == GT_TRUE)
                    {
                        iteratorPtr->state = PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STATE_NO_MORE_E;
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
                    }
...
* @retval GT_BAD_PARAM             - on wrong parameter
*/
GT_STATUS prvCpssDxChPtpPLLMsi5nmConfig
(
    IN GT_U8                                    devNum,
...
)
{
    ...
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_HARRIER_E);
    ...
    /*Toggle PTP clock from PTP PLL to MISC0 PLL*/
    regAddr = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.deviceCtrl5;
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 17, 1, 0);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* configure PLL MSI PTP ref clock select */
    ...
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 29, 3, hwValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* PLL MSI PLL_LOCK mode select */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 1, 2, 3);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* PLL MSI Bypass Frequency Control Register Selection */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 3, 1, 1);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* PLL MSI PLL Clock Mode CH1 Clock Output Gating Control */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 5, 1, 1);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* PLL MSI Phase Interpolator Bias Current Select */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 7, 4, 0xA);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* PLL MSI VCO Capacitor Selection */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 11, 4, 0xA);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    ...
    {
        return prvCpssDxChPtpPLLMsi5nmConfig(devNum,CPSS_DXCH_PTP_REF_CLOCK_SELECT_MAIN_25_MHZ_E,CPSS_DXCH_PTP_25_FREQ_E);
    }
    ...
    case CPSS_DXCH_PTP_10_FREQ_E:
    case CPSS_DXCH_PTP_20_FREQ_E:
        K = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].K;
        M = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].M;
        N = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].N;
        vcoBand = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].vcoBand;
        lpfCtrl = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].lpfCtrl;
        break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal prvCpssDxChPtpSip6PLLConfig function
* @endinternal
*
* @brief   config the PTP PLL parameters to support different frequencies for SIP6 devices.
*
...
* @retval GT_BAD_PARAM             - on wrong parameter
*/
GT_STATUS prvCpssDxChPtpSip6PLLConfig
(
    IN GT_U8                                   devNum,
    IN CPSS_DXCH_PTP_REF_CLOCK_SELECT_ENT      refClockSource,
    IN CPSS_DXCH_PTP_TAI_CLOCK_FREQ_ENT        clockFrequency,
...
)
{
    GT_STATUS   rc;
    GT_U32 pllConfigRegAddr;
    GT_U32 regAddr,regAddr1,hwValue;
    GT_U32 K;
    GT_U32 M;
    GT_U32 N;
    GT_U32 vcoBand;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_IRONMAN_E | CPSS_AAS_E);

    /* configure Harrier PTP PLL MSI 5nm; PTP PLL is powered down by default */
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
    {
        return prvCpssDxChPtpPLLMsi5nmConfig(devNum,refClockSource,clockFrequency);
    }

    if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E) && (clockFrequency == CPSS_DXCH_PTP_156_25_FREQ_E))
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);

    PRV_CPSS_DXCH_CHECK_AC5Y_DEVICE_MAC(devNum,isAc5y);
    if ((isAc5y == GT_TRUE) && (taiNumber != CPSS_DXCH_PTP_TAI_NUMBER_0_E) && (taiNumber != CPSS_DXCH_PTP_TAI_NUMBER_4_E))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    case CPSS_DXCH_PTP_20_FREQ_E:
    case CPSS_DXCH_PTP_25_FREQ_E:
    case CPSS_DXCH_PTP_156_25_FREQ_E:
        K = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].K;
        M = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].M;
        N = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].N;
        vcoBand = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.ptpInfo.pllConfig[clockFrequency].vcoBand;
    ...
    /* 1. Toggle PTP clock from PTP PLL to MISC PLL */
    if (PRV_CPSS_SIP_6_15_CHECK_MAC(devNum))
    {
        regAddr1 = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.deviceCtrl19;
        taiOffset = (taiNumber == CPSS_DXCH_PTP_TAI_NUMBER_0_E)?14:17;
    }
    else if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        regAddr1 = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.deviceCtrl13;
        taiOffset = (taiNumber == CPSS_DXCH_PTP_TAI_NUMBER_0_E)?16:19;
    }
    else
    {
        regAddr1 = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.deviceCtrl15;
        taiOffset = (taiNumber == CPSS_DXCH_PTP_TAI_NUMBER_0_E)?4:7;
    }
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr1, taiOffset, 1, 0);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* 2. PTP PLL bypass; power down mode */
    pllConfigRegAddr = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.pllPTPConfig;
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 0, 1, 1);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* 3. configure PTP ref clock select */
    if (PRV_CPSS_SIP_6_15_CHECK_MAC(devNum))
    {
        ...
        case CPSS_DXCH_PTP_REF_CLOCK_SELECT_SD_DIFFRENTIAL_REF_CLK_E:
            hwValue = 1;
            break;
        default: /* PTP PLL has no option of global ref clock */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
        rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 8, 1, hwValue);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    else if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ...
        rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 13, 3, hwValue);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    {
        /* nothing to do PTP ref Clock is always global ref clock  */
    }

    /* 4. set PTP PLL configurations values */
    ...
    /* Set PTP PLL Reconfiguration Enable */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 9, 1, 1);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* 5. clear PTP PLL bypass; power up mode */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, pllConfigRegAddr, 0, 1, 0);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* 6. Toggle PTP clock from MISC PLL to PTP PLL */
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr1, taiOffset, 1, 1);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_AAS_E);
...
/**
* @internal prvCpssDxChPtpTaiDriftCompensationControlConfig function
* @endinternal
*
* @brief   config the TAI Drift Compensation Control parameters.
*
...
* @retval GT_BAD_PARAM             - on wrong parameter
*/
GT_STATUS prvCpssDxChPtpTaiDriftCompensationControlConfig
(
    IN GT_U8                                    devNum
)
{
    ...
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, &taiId, taiIterator)
    {
        ...
        regAddr = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).TAIDriftCompensationControl;
        /* total cycles: 3
           number of cycles: 2
           patch logic integer: 1
           patch logic fraction: 8
           tod step patch enable: 1 */
        regData = 0x18010203;
        rc = prvCpssDrvHwPpPortGroupSetRegField(devNum, taiIterator.portGroupId, regAddr, 0, 32, regData);
        if (rc != GT_OK)
        {
            return rc;
        }
    }
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_END_MAC(devNum, &taiId, taiIterator)

    return GT_OK;
}

/**
* @internal prvCpssDxChPtpTaiGroupSelect function
* @endinternal
*
* @brief   Selecte all  TAIs units for triggering.
*
* @note   APPLICABLE DEVICES:      Ironman.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChPtpTaiGroupSelect
(
    IN GT_U8                                    devNum,
...
)
{
    ...
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, taiIdPtr, taiIterator)
    {
        ...
        /* In IM there is a different mechanism for the triggering: the user need to choose the tais for triggering and then activate the trigger.
           It is a 2 stages operation and the user doesn't need to clean the tai selection after.
           This check is to insure that the tai selection is cleared in other devices.
        */
        if ((status == GT_TRUE) && (PRV_CPSS_PP_MAC(devNum)->devFamily != CPSS_PP_FAMILY_DXCH_IRONMAN_E))
        {
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        ...
        PRV_CPSS_DXCH_TAI_CHECK_MAC(devNum,taiIdPtr->taiNumber);
        ...
        {
            PRV_CPSS_DXCH_CHECK_AC5Y_DEVICE_MAC(devNum,isAc5y);
            *finishPtr = (isAc5y == GT_FALSE) ? ((hwValue == 0x3FF) ? GT_TRUE : GT_FALSE) :/* TAI0 .. TAI4 */
                                                ((hwValue == 0x303) ? GT_TRUE : GT_FALSE); /* TAI0 & TAI4 */
        }
        else
        {
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    PRV_CPSS_DXCH_PTP_NOT_SINGLE_TAI_PTR_CHECK_MAC(devNum, taiIdPtr);
    ...
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, taiIdPtr, taiIterator)
    {
        ...
        else if (PRV_CPSS_SIP_6_20_CHECK_MAC(devNum))
        {
            ...
            for (harrierTaiSubUnitType = 1; (GT_U32)harrierTaiSubUnitType < totalSubUnitType; harrierTaiSubUnitType++)
            {
                ...
                case PRV_CPSS_DXCH_PTP_TAI_INSTANCE_SUB_TYPE_HARRIER_EPCL_HA_TAI0_E:
                    /* read cp frequency; different TAI CP clients in order to save power */
                    regAddr[0] = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum)->DFXServerUnitsDeviceSpecificRegs.deviceSAR2;
                    rc = prvCpssDrvHwPpResetAndInitControllerGetRegField(devNum,regAddr[0], 18, 3,&regData);
                    if(rc != GT_OK)
                    {
                        return rc;
                    }
                    ...
                    {
                    case 3: /* 600 MHz */
                        ptpClkInKhz = HARRIER_PTP_CP_600_CLK_CNS;
                        break;
                    case 5: /* 200 MHz */
                        ptpClkInKhz = HARRIER_PTP_CP_200_CLK_CNS;
                        break;
                    default: /* 800 MHz */
                        ptpClkInKhz = HARRIER_PTP_CP_800_CLK_CNS;
                        break;
                    }
                    break;
        ...
        else if (PRV_CPSS_SIP_6_15_CHECK_MAC(devNum))
        {
            ...
            for (phoenixTaiSubUnitType = 1; (GT_U32)phoenixTaiSubUnitType < totalSubUnitType; phoenixTaiSubUnitType++)
            {
                ...
                {
                    /* NOTE: the casting to GT_U32 i so.k. when casting a positive float */
                    todStepSlave.fracNanoSeconds = (GT_U32)(0xFFFFFFFF *
                            (1000000000.0/ptpClkInKhz - todStepSlave.nanoSeconds) +
                            (1000000000.0/ptpClkInKhz - todStepSlave.nanoSeconds));
                }
        ...
        if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
        {
           /* get Egress Pipe Delay Corrections */
            ...
            if ((PRV_CPSS_DXCH_PP_MAC(devNum)->port.ptpPortTsOperationModeArrPtr[portNum].tsMode == CPSS_DXCH_PTP_ENHANCED_TIMESTAMPING_MODE_MACSEC_E) &&
                (PRV_CPSS_DXCH_PP_MAC(devNum)->port.ptpPortTsOperationModeArrPtr[portNum].operationMode == CPSS_DXCH_PTP_MANAGER_OPERATION_MODE_ONE_OR_TWO_STEP_IN_SWITCH_E))
            {
                regAddr[0] = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_CTSU[tsuIndex].channelTsPrecision[tsuLocalChannel];
                rc = prvCpssHwPpGetRegField(devNum,regAddr[0],1,1,&hwData[0]);
                if (rc != GT_OK)
                {
                    return rc;
                }
                /* TSTF supported (Harrier); TSTF with errata (Hawk) */
                if (hwData[0] == 1) /* TSTF */
                {
                    /* nanosecond timestamp in the range -2^30 to 2^30 */
                    nanoSeconds = (entryPtr->timestamp && 0x3FFFFFFF) + egrDelayVal.egressPipeDelayEncrypted;
                    seconds = (entryPtr->timestamp >> 30);
                    if (nanoSeconds >= 1000000000)
                    {
                        nanoSeconds -= 1000000000;
                        seconds += 1;
                        entryPtr->timestamp = ((seconds & 0x3) << 30) | (nanoSeconds & 0x3FFFFFFF);
                    }
                    else
                    {
                        entryPtr->timestamp += egrDelayVal.egressPipeDelayEncrypted;
                    }
                }
                else
                {
                    /* nanosecond timestamp in the range -2^30 to 2^30 */
                    nanoSeconds = (entryPtr->timestamp && 0x3FFFFFFF) + egressPipeDelayCtsu;
                    seconds = (entryPtr->timestamp >> 30);
                    if (nanoSeconds >= 1000000000)
                    {
                        nanoSeconds -= 1000000000;
                    ...
                    {
                        entryPtr->timestamp += egressPipeDelayCtsu;
                    }
                }
            }
        }
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
    {
        if (egrDelayValPtr->egressPipeDelayFsuOffset != 0)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
    }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
{
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    PRV_CPSS_DXCH_PHY_PORT_NUM_CHECK_MAC(devNum, portNum);
...
)
{
    ...
    tsQueueEntryPtr->isPtpException = BIT2BOOL_MAC(hwValue);
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        tsQueueEntryPtr->taiSelect = (((hwEntry[2] >> 13) & 3) << 1) | ((hwEntry[0] >> 5) & 1);
    }
    else
    {
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        tsQueueEntryPtr->sequenceId =
            ((hwEntry[0] >> 19) & 0x1FFF) | ((hwEntry[1] & 0x3F) << 13);

        tsQueueEntryPtr->timestamp =
            ((hwEntry[1] >> 6) & 0x3FFFFFF) | ((hwEntry[2] & 0x3F) << 29);

        tsQueueEntryPtr->portNum = ((hwEntry[2] >> 6) & 0xFF);
    }
    else
    {
        tsQueueEntryPtr->sequenceId =
...
)
{
    ...
    {
        tsQueueEntryPtr->sequenceId =
            ((hwEntry[0] >> 17) & 0x7FFF) | ((hwEntry[1] & 0xF) << 15);

        tsQueueEntryPtr->queueEntryId = ((hwEntry[1] >> 0xF) & 0x3FF);

        tsQueueEntryPtr->portNum = ((hwEntry[1] >> 14) & portMask);
    }
    else
    {
        tsQueueEntryPtr->sequenceId =
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                                  CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_CAELUM_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    /* Bobcat2; Caelum; Bobcat3 */
    if(PRV_CPSS_DEV_DFX_SERVER_SUPPORTED_MAC(devNum) == GT_FALSE)
    {
        /* current GM doesn't support DFX unit so far */
    ...
    rc =  prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, bitOffset, 1, fieldValue);
    if (rc != GT_OK)
    {
        return rc;
    }

    if (clockSelect == CPSS_DXCH_PTP_TAI_CLOCK_SELECT_PTP_PLL_E)
    {
        if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
        {
            return prvCpssDxChPtpSip6PLLConfig(devNum,CPSS_DXCH_PTP_REF_CLOCK_SELECT_DEDICATED_PTP_REF_CLK_E,CPSS_DXCH_PTP_25_FREQ_E,taiNumber);
        }
        else if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
        {
            return prvCpssDxChPtpSip6PLLConfig(devNum,CPSS_DXCH_PTP_REF_CLOCK_SELECT_MAIN_25_MHZ_E,CPSS_DXCH_PTP_25_FREQ_E,taiNumber);
        }
        else if(PRV_CPSS_SIP_5_16_CHECK_MAC(devNum))
        {
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        ...
        /* select all tais for triggering */
        PRV_CPSS_DXCH_CHECK_AC5Y_DEVICE_MAC(devNum,isAc5y);
        rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 0, 10, (isAc5y == GT_FALSE) ? 0x3FF : 303);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_IRONMAN_E | CPSS_AAS_E);
    PRV_CPSS_DXCH_TAI_CHECK_MAC(devNum,taiSelect);
...
)
{
    ...
    /* save tai triggering selection */
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 10, &valueSave);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* restore tai triggering selection */
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 10, valueSave);
    return rc;
...
/**
* @internal internal_cpssDxChPtpPortPhyTsEnableSet function
* @endinternal
*
* @brief   Enable/Disable Reliance on Timestamping in the PHY per port Configuration.
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - port number.
* @param[in] enable                   - enable/disbale timestamping in the PHY.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChPtpPortPhyTsEnableSet
(
    IN  GT_U8                               devNum,
    IN  GT_PHYSICAL_PORT_NUM                portNum,
    IN  GT_BOOL                             enable
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        rc = prvCpssDxChTxqSip6_10PreemptionEnableGet(devNum,portNum,0 /* ignore */,&preemptionEnable);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_USX_E)
        {
            ...
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 5, 1, ((enable == GT_FALSE) ? 1 : 0));
            if (rc != GT_OK)
            {
                return rc;
            }

            /* enable/disable PCH encode/decode functions */
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.usxPchControl;
            ...
            /* enable/disable check for SFD (0xd5) and accept frame with any character */
            ...
            /* Receive GMII stream from the 1000Base-X decoder is assumed to be synchronized to 10B idle code (disable SFD search). */
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_LPCS.portIfMode;
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 7, 1, data);
            if (rc != GT_OK)
            {
                return rc;
            }

            /* enable/disable for the PCH TX unit */
            ...
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portPmacUsxPchTxControl1;
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 21, 1, data);
            if (rc != GT_OK)
            {
                return rc;
            ...
            if (preemptionEnable)
            {
                /* When set to 1, the preamble is checked so all bytes except the SFD are 0x55 */
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brCntrl;
                rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 23, 1, data);
                if (rc != GT_OK)
                {
                    return rc;
                }

                /* When set to '1', PCH for frames with CTSU trigger use Extension Field Type of "PTP_BR".
                   Else, they use Extension Field Type of "PTP" P must be set to '1'. */
                regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portEmacUsxPchTxControl1;
                rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 1, 1, data);
                if (rc != GT_OK)
                {
                    return rc;
                }
                ...
                /* When set to '1', PCH with Extension Field Type of "PTP_BR" which is not express is errored.
                   Must be set to '0' for P. */
        ...
        else if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_CPU_E)
        {
            ...
            /* enable/disable PCH encode/decode functions */
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.usxPchControl;
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 1, data);
            if (rc != GT_OK)
            {
                return rc;
            ...
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portPmacUsxPchTxControl1;
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 21, 1, data);
            if (rc != GT_OK)
            {
                return rc;
    ...
    if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) != PRV_CPSS_PORT_MTI_USX_E)
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, "support only port type USX");

    /* enable/disable ingress PCH unit */
    regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portUsxPchControl;
    rc = prvCpssDrvHwPpResetAndInitControllerSetRegField(devNum, regAddr, 0, 1, data);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal cpssDxChPtpPortPhyTsEnableSet function
* @endinternal
*
* @brief   Enable/Disable Reliance on Timestamping in the PHY per port Configuration.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChPtpPortPhyTsEnableSet
(
    IN  GT_U8                               devNum,
    IN  GT_PHYSICAL_PORT_NUM                portNum,
    IN  GT_BOOL                             enable
)
{
    ...
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpPortPhyTsEnableSet);

    CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enable));

    rc = internal_cpssDxChPtpPortPhyTsEnableSet(devNum, portNum, enable);
...
/**
* @internal internal_cpssDxChPtpPortPhyTsEnableGet function
* @endinternal
*
* @brief   Enable/Disable Reliance on Timestamping in the PHY per port Configuration.
*
...
* @param[in] devNum                   - device number
* @param[in] portNum                  - port number.
* @param[out] enablePtr               - (pointer to) enable/disbale timestamping in the PHY.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChPtpPortPhyTsEnableGet
(
    IN  GT_U8                               devNum,
    IN  GT_PHYSICAL_PORT_NUM                portNum,
    OUT GT_BOOL                             *enablePtr
)
{
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        ...
        {
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portEmacUsxPchTxControl1;
            rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 21, 1, &data);
            if (rc != GT_OK)
            {
                return rc;
            }
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portEmacUsxPchRxControl;
            rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 21, 1,&data1);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        else if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_CPU_E)
        {
            ...
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portEmacUsxPchRxControl;
            rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 1, &data1);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        *enablePtr = (data == 1 && data1 == 1) ? GT_TRUE : GT_FALSE;
        return GT_OK;
    }
    ...
    /* enable/disable ingress PCH unit */
    regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portUsxPchControl;
    rc = prvCpssDrvHwPpResetAndInitControllerGetRegField(devNum, regAddr, 0, 1, &data);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal cpssDxChPtpPortPhyTsEnableGet function
* @endinternal
*
* @brief   Enable/Disable Reliance on Timestamping in the PHY per port Configuration.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChPtpPortPhyTsEnableGet
(
    IN  GT_U8                               devNum,
    IN  GT_PHYSICAL_PORT_NUM                portNum,
    OUT GT_BOOL                             *enablePtr
)
{
    ...
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpPortPhyTsEnableGet);

    CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enablePtr));

    rc = internal_cpssDxChPtpPortPhyTsEnableGet(devNum, portNum, enablePtr);
...
/**
* @internal internal_cpssDxChPtpTaiSerialGenerationModeSet function
* @endinternal
*
* @brief  Set the Serial Generation Mode.
*
...
* @param[in] devNum                 - device number.
* @param[in] taiIdPtr               - (pointer to) TAI Units identification.
* @param[in] serGenerationMode     - Serial Genartion Mode.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChPtpTaiSerialGenerationModeSet
(
    IN GT_U8                                  devNum,
    IN CPSS_DXCH_PTP_TAI_ID_STC               *taiIdPtr,
    IN CPSS_DXCH_PTP_TAI_SER_GEN_MODE_ENT     serGenerationMode
)
{
    GT_STATUS                          rc;          /* return code      */
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STC taiIterator; /* TAI iterator     */
    ...
    switch (serGenerationMode)
    {
        case CPSS_DXCH_PTP_TAI_SER_GEN_MODE_DISABLED_E:
            hwValue = 0;
            break;
        case CPSS_DXCH_PTP_TAI_SER_GEN_MODE_SERIAL_GEN_E:
            hwValue = 1;
            break;
        default: CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, taiIdPtr, taiIterator)
    {
        /* enable/disable SER TX outpot; the TAI SER-TX periodically sends serial frames. */
        regAddr = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).serTxControl;
        rc = prvCpssDrvHwPpPortGroupSetRegField(devNum, taiIterator.portGroupId, regAddr, 0, 1, hwValue);
        if (rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal cpssDxChPtpTaiSerialGenerationModeSet function
* @endinternal
*
* @brief  Set the Serial Generation Mode.
*
...
*
*/
GT_STATUS cpssDxChPtpTaiSerialGenerationModeSet
(
    IN GT_U8                                   devNum,
    IN CPSS_DXCH_PTP_TAI_ID_STC               *taiIdPtr,
    IN CPSS_DXCH_PTP_TAI_SER_GEN_MODE_ENT     serGenerationMode
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpTaiSerialGenerationModeSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, taiIdPtr, serGenerationMode));

    rc = internal_cpssDxChPtpTaiSerialGenerationModeSet(devNum, taiIdPtr, serGenerationMode);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, taiIdPtr, serGenerationMode));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChPtpTaiSerialGenerationModeGet function
* @endinternal
*
* @brief  Get the Serial Generation Mode.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                 - device number.
* @param[in] taiIdPtr               - (pointer to) TAI Units identification.
* @param[OUT] serGenerationModePtr  - (pointer to) Serial Genartion Mode.
*
* @retval GT_OK                     - on success
* @retval GT_FAIL                   - on error
* @retval GT_HW_ERROR               - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChPtpTaiSerialGenerationModeGet
(

    IN  GT_U8                                   devNum,
    IN  CPSS_DXCH_PTP_TAI_ID_STC               *taiIdPtr,
    OUT CPSS_DXCH_PTP_TAI_SER_GEN_MODE_ENT     *serGenerationModePtr
)
{
    GT_STATUS                          rc;          /* return code      */
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STC taiIterator; /* TAI iterator     */
    ...
    PRV_CPSS_DXCH_PTP_NOT_SINGLE_TAI_PTR_CHECK_MAC(devNum, taiIdPtr);
    CPSS_NULL_PTR_CHECK_MAC(serGenerationModePtr);
    PRV_CPSS_DXCH_TAI_CHECK_MAC(devNum,taiIdPtr->taiNumber);
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_GET_SINGLE_MAC(devNum, taiIdPtr, taiIterator);

    regAddr = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).serTxControl;
    rc = prvCpssDrvHwPpPortGroupGetRegField(devNum, taiIterator.portGroupId, regAddr, 0, 1, &hwValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    {
        case 0:
            *serGenerationModePtr = CPSS_DXCH_PTP_TAI_SER_GEN_MODE_DISABLED_E;
            break;
        case 1:
            *serGenerationModePtr = CPSS_DXCH_PTP_TAI_SER_GEN_MODE_SERIAL_GEN_E;
            break;
        default: CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG); /*never occurs*/
    }
...
/**
* @internal cpssDxChPtpTaiSerialGenerationModeGet function
* @endinternal
*
* @brief  Get the Serial Generation Mode.
*
...
*
*/
GT_STATUS cpssDxChPtpTaiSerialGenerationModeGet
(
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpTaiSerialGenerationModeGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, taiIdPtr, serGenerationModePtr));

    rc = internal_cpssDxChPtpTaiSerialGenerationModeGet(devNum, taiIdPtr, serGenerationModePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, taiIdPtr, serGenerationModePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChPtpTaiSerialReceptionModeSet function
* @endinternal
*
* @brief  Set the Serial Reception Mode.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number.
* @param[in] taiIdPtr              - (pointer to) TAI Units identification.
* @param[in] serReceptionMode      - Serial Reception Mode.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChPtpTaiSerialReceptionModeSet
(
    IN GT_U8                                         devNum,
    IN CPSS_DXCH_PTP_TAI_ID_STC                     *taiIdPtr,
    IN CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_ENT     serReceptionMode
)
{
    GT_STATUS                          rc;          /* return code      */
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STC taiIterator; /* TAI iterator     */
    ...
    switch (serReceptionMode)
    {
        case CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_DISABLED_E:
            hwValue = 0;
            break;
        case CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_SERIAL_RECEPTION_E:
            hwValue = 1;
            break;
        default: CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, taiIdPtr, taiIterator)
    {
        /* enable/disable SER RX input; TAI SER-RX periodically receives, decodes and processes serial frames, for updating the TOD. */
        regAddr = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).serRxControl;
        rc = prvCpssDrvHwPpPortGroupSetRegField(devNum, taiIterator.portGroupId, regAddr, 0, 1, hwValue);
        if (rc != GT_OK)
        {
            return rc;
...
/**
* @internal cpssDxChPtpTaiSerialReceptionModeSet function
* @endinternal
*
* @brief  Set the Serial Reception Mode.
*
...
*
*/
GT_STATUS cpssDxChPtpTaiSerialReceptionModeSet
(
    IN GT_U8                                         devNum,
    IN CPSS_DXCH_PTP_TAI_ID_STC                     *taiIdPtr,
    IN CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_ENT     serReceptionMode
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpTaiSerialReceptionModeSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, taiIdPtr, serReceptionMode));

    rc = internal_cpssDxChPtpTaiSerialReceptionModeSet(devNum, taiIdPtr, serReceptionMode);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, taiIdPtr, serReceptionMode));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChPtpTaiSerialReceptionModeGet function
* @endinternal
*
* @brief  Get the Serial Reception Mode.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number.
* @param[in] taiIdPtr              - (pointer to) TAI Units identification.
* @param[OUT] serReceptionModePtr  - (pointer to) Serial Reception Mode.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChPtpTaiSerialReceptionModeGet
(
    IN  GT_U8                                         devNum,
    IN  CPSS_DXCH_PTP_TAI_ID_STC                     *taiIdPtr,
    OUT CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_ENT     *serReceptionModePtr
)
{
    GT_STATUS                          rc;          /* return code      */
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_STC taiIterator; /* TAI iterator     */
    ...
    PRV_CPSS_DXCH_PTP_NOT_SINGLE_TAI_PTR_CHECK_MAC(devNum, taiIdPtr);
    CPSS_NULL_PTR_CHECK_MAC(serReceptionModePtr);
    PRV_CPSS_DXCH_TAI_CHECK_MAC(devNum,taiIdPtr->taiNumber);
    PRV_CPSS_DXCH_PTP_TAI_ITERATOR_GET_SINGLE_MAC(devNum, taiIdPtr, taiIterator);
    ...
    {
        case 0:
            *serReceptionModePtr = CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_DISABLED_E;
            break;
        case 1:
            *serReceptionModePtr = CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_SERIAL_RECEPTION_E;
            break;
        default: CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG); /*never occurs*/
    }
...
/**
* @internal cpssDxChPtpTaiSerialReceptionModeGet function
* @endinternal
*
* @brief  Get the Serial Reception Mode.
*
...
*
*/
GT_STATUS cpssDxChPtpTaiSerialReceptionModeGet
(
    IN  GT_U8                                         devNum,
    IN  CPSS_DXCH_PTP_TAI_ID_STC                     *taiIdPtr,
    OUT CPSS_DXCH_PTP_TAI_SER_RECEPTION_MODE_ENT     *serReceptionModePtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpTaiSerialReceptionModeGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, taiIdPtr, serReceptionModePtr));

    rc = internal_cpssDxChPtpTaiSerialReceptionModeGet(devNum, taiIdPtr, serReceptionModePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, taiIdPtr, serReceptionModePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    /* In Ironman there are no TAI slaves; only need to disable the TSX */
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        for (i=0; i<PRV_CPSS_MAX_MAC_PORTS_NUM_CNS; i++)
        {
            PRV_CPSS_DXCH_PORT_SKIP_CHECK(devNum, i);
            ...
            PRV_CPSS_DXCH_PORT_NUM_OR_CPU_PORT_CHECK_AND_MAC_NUM_GET_MAC(devNum,i,portMacNum);

            if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_USX_E)
            {
                mtipRegAddr[0] = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portTsxControl2;
                mtipRegAddr[1] = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portTsxControl3;
                mtipRegAddr[2] = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portTsxControl4;
            }
            else /* PRV_CPSS_PORT_MTI_CPU_E */
            {
                mtipRegAddr[0] = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portTsxControl2;
                mtipRegAddr[1] = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portTsxControl3;
                mtipRegAddr[2] = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portTsxControl4;
            }
        ...
        PRV_CPSS_DXCH_PTP_TAI_ITERATOR_LOOP_START_MAC(devNum, &taiId, taiIterator)
        {
            /* configure TAI Drift compensation patch. */
            regAddr[0] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).TAIDriftCompensationControl;

            rc = prvCpssHwPpSetRegField(devNum, regAddr[0], 0, 32, 0);
            if (rc != GT_OK)
            {
                return rc;
            }

            /* configure fractional nano-second drift value. */
            regAddr[0] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiIterator.gopNumber, taiIterator.taiNumber).fractionalNanosecondDriftAdjustment;
    ...
    {
        totalSubUnitType = FALCON_PER_TILE_TAI_SUB_UNITS_NUMBER_CNS;
        numOfTiles = PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfTiles ? PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfTiles : 1;

        /* freeze slave clocks */
        for (ii=1; ii<totalSubUnitType; ii++)
        {
            for(tileId = 0; tileId < numOfTiles; tileId++)
            {
                taiGop = (tileId*FALCON_PER_TILE_TAI_SUB_UNITS_NUMBER_CNS) + ii;
                regAddr[0] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiGop, taiNumber).TODStepNanoConfigLow;
                regAddr[1] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiGop, taiNumber).TODStepFracConfigHigh;
                regAddr[2] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, taiGop, taiNumber).TODStepFracConfigLow;
    ...
    for (ii=1; ii<totalSubUnitType; ii++)
    {
        regAddr[0] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, ii, taiNumber).TODStepNanoConfigLow;
        regAddr[1] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, ii, taiNumber).TODStepFracConfigHigh;
        regAddr[2] = PRV_DXCH_REG1_UNIT_GOP_TAI_MAC(devNum, ii, taiNumber).TODStepFracConfigLow;
    ...
    for (i=0; i<PRV_CPSS_MAX_MAC_PORTS_NUM_CNS; i++)
    {
        ...
        {
            mtipRegAddr[0] = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).tsuControl0;
            mtipRegAddr[1] = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).tsuControl2;
        ...
        {
            mtipRegAddr[0] = PRV_DXCH_REG1_UNIT_GOP_MTI_EXT_MAC(devNum, portMacNum).tsuControl0;
            mtipRegAddr[1] = PRV_DXCH_REG1_UNIT_GOP_MTI_EXT_MAC(devNum, portMacNum).tsuControl2;
...
)
{
    ...
    rc = internal_cpssDxChPtpTaiTodStepDbgSet(devNum, taiNumber, todStepPtr);
...
)
{
    ...
    case CPSS_DXCH_PTP_TAI_SELECT_UNIT_CTSU_E:
        taiNumMax = (isAc5y == GT_FALSE) ? 3 : 0;
        if (taiSelect > taiNumMax)
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
...
/**
* @internal internal_cpssDxChPtpPortMacTsCrcErrorCounterDbgGet function
* @endinternal
*
* @brief   Get the dropped frames due to CRC errors per port.
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon.
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - egress port number.
* @param[out] crcErrorCounterPtr      - (pointer to) crc errors counter.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChPtpPortMacTsCrcErrorCounterDbgGet
(
    IN  GT_U8                                                       devNum,
    IN  GT_PHYSICAL_PORT_NUM                                        portNum,
    OUT GT_U32                                                      *crcErrorCounterPtr
)
{
    GT_STATUS rc;
    GT_U32 regAddr,value=0;
    GT_U32 portMacNum;

    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(crcErrorCounterPtr);
    ...
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_USX_E)
        {
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portEmacUsxPchBadCrcCounter;
            rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 8, &value);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        {
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_CPU_EXT_MAC(devNum, portMacNum).portEmacUsxPchBadCrcCounter;
    ...
    else
    {
        if (PRV_CPSS_DXCH_PORT_TYPE_MAC(devNum, portMacNum) == PRV_CPSS_PORT_MTI_USX_E)
        {
            regAddr = PRV_DXCH_REG1_UNIT_GOP_MTI_USX_EXT_MAC(devNum, portMacNum).portUsxPchCrcErrorCounter;
   ...
   *crcErrorCounterPtr = value;

    return GT_OK;
}

/**
* @internal cpssDxChPtpPortMacTsCrcErrorCounterDbgGet function
* @endinternal
*
* @brief   Get the dropped frames due to CRC errors per port.
*
...
*
*/
GT_STATUS cpssDxChPtpPortMacTsCrcErrorCounterDbgGet
(
    IN  GT_U8                                                       devNum,
    IN  GT_PHYSICAL_PORT_NUM                                        portNum,
    OUT GT_U32                                                      *crcErrorCounterPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChPtpPortMacTsCrcErrorCounterDbgGet);

    CPSS_API_LOCK_MAC(devNum, PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, crcErrorCounterPtr));

    rc = internal_cpssDxChPtpPortMacTsCrcErrorCounterDbgGet(devNum, portNum, crcErrorCounterPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, crcErrorCounterPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
