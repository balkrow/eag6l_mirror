...
)
{
    ...
    if(action == PRV_ACTION_CREATE_E)
    {
        ...
        prvInfoPtr->buffList[0] = cpssOsCacheDmaMallocByDevice(devNum, prvInfoPtr->buffLenList[0]);
#else
        ...
        if(prvInfoPtr->buffList[0] == NULL)
        {
            ...
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_CREATE_ERROR,
                "failed to allocate 'cpssOsCacheDmaMallocByDevice' for [%d] device [%d] bytes",
                devNum, prvInfoPtr->buffLenList[0]);
#else
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_CREATE_ERROR,
        ...
        /* allocate memory in the SDMA for this packet */
#if defined(CONFIG_V2_DMA)
        prvInfoPtr->longBuffList[0] = cpssOsCacheDmaMallocByDevice(devNum, prvInfoPtr->longBuffLenList[0]);
#else
        ...
        if(prvInfoPtr->longBuffList[0] == NULL)
        {
            ...
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_CREATE_ERROR,
                "failed to allocate 'cpssOsCacheDmaMallocByDevice' for [%d] device [%d] bytes",
                devNum, prvInfoPtr->longBuffLenList[0]);
#else
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_CREATE_ERROR,
                "failed to allocate 'cpssOsCacheDmaMalloc' for [%d] bytes",
...
)
{
    ...
    *doTimeMeasurePtr = GT_FALSE;
    ...
    if(cpssSpeed != CPSS_PORT_SPEED_100G_E &&
       ...
       cpssSpeed != CPSS_PORT_SPEED_107G_E)
    {
        /* the WA not needed */
        return GT_OK;
    }
    ...
    *doTimeMeasurePtr = GT_TRUE;
    ...
    OS_PRINTF(("Started the WA on port [%d] down from speed[%s] \n",portNum,
        (cpssSpeed == CPSS_PORT_SPEED_100G_E ? "100G":
         ...
         cpssSpeed == CPSS_PORT_SPEED_107G_E ? "107G":
...
static const GPC_VALUES_STC gpcValues_25G = {
    /* fields in 'reorder 0' */
...
)
{
    ...
    if(infoPtr->reservedCpuSdmaGlobalQueue[WA_PACKET_TYPE_SHORT_E] != 0xFFFFFFFF)
    {
        ...
        for(ii = 0 ; ii < 2 ; ii++)
        {
            /* make sure that the CPU TX SDMA Queue is valid (that application mapped !) */
            PRV_CPSS_CHECK_SDMA_TX_Q_MAC(devNum, infoPtr->reservedCpuSdmaGlobalQueue[ii]);
