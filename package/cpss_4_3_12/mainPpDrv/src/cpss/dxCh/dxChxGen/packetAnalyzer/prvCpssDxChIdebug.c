...
(
    ...
    IN GT_U32                                           segmentId,
    IN GT_CHAR_PTR                                      origInterfaceName,
    IN GT_U32                                           muxId,
    OUT GT_U32                                         *sizePtr
);
    ...
    *sizePtr = cpssOsStrTo32(attr);
    ...
    if(GT_TRUE==PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
       ...
       if(segmentId>0)
       {
           ...
           rc =mxmlInstanceInfoSegmentSizeGet(devNum,node,segmentId-1,interfaceName,muxId,&prevSize);
           if (rc != GT_OK)
            return NULL;
           *sizePtr+=prevSize;
...
)
{
    switch(PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
            *xmlFileName = PRV_CPSS_IRONMAN_IDEBUG_XML_FILE;
            break;
        default:
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_XCAT3_E | CPSS_AC5_E |
                                          CPSS_BOBCAT3_E | CPSS_CAELUM_E | CPSS_AC3X_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    PRV_CPSS_PACKET_ANALYZER_DFX_SERVER_SUPPORTED_MAC(devNum);
    PRV_CPSS_PACKET_ANALYZER_INTERFACE_ID_CHECK_MAC(devNum,interfaceName);
    CPSS_NULL_PTR_CHECK_MAC(numOfMultiplicationsPtr);
...
)
{
    ...
    if(GT_FALSE==PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN2_E)
        {
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error: interfaceName [%s] is not multiple", interfaceName);
            }
        }
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN_E)
        {
     ...
     {
           CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"Error: prvCpssDxChIdebugMultipleInterfaceByIndexGet is not applicable for SIP6");
     }
     ...
     node = node->child;
      ...
      while(node != NULL)
      {
          ...
          if(currentMuxId == muxId)
          {
             ...
             {
                 if (cpssOsStrCmp(origInterfaceName,interfaceName)==0 )
                 {
                     /*idebug doesn't support multiplay interfaces with same internal name in the same instance */
                     CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED,"Error : more then 1 interface with same internal name in instance ");
                 }
