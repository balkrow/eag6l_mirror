...
)
{
    ...
    GT_U32                              leafLine=0;/* each line hold 5 entries */
    PRV_CPSS_DXCH_MODULE_CONFIG_STC     *moduleCfgPtr; /* pointer to the module configure of the PP's database*/
    ...
    case CPSS_DXCH_LPM_ROUTE_ENTRY_NEXT_PTR_TYPE_E:
        if((PRV_CPSS_SIP_7_CHECK_MAC(devNum))|| (PRV_CPSS_SIP_6_CHECK_MAC(devNum)))
        {
        ...
        case CPSS_DXCH_LPM_MULTIPATH_ENTRY_PTR_TYPE_E:
            if(!PRV_CPSS_SIP_6_CHECK_MAC(devNum) && !PRV_CPSS_SIP_7_CHECK_MAC(devNum))
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        prvCpssLpmFillPrvLeafValueSip7(leafPtr, &leafPrvData);
        /* Calculate LPM leaf value */
        rc = prvCpssLpmBuildLeafValueSip7(devNum, &leafPrvData, hwData);/* support for 36 bits of data */
        if (rc != GT_OK)
        {
            return rc;
        }

        /* extract the bankNumber from the leaf index */
        leafLine = leafIndex/MAX_NUMBER_OF_LEAVES_IN_LPM_LINE_CNS;

        /* if we have big PBR banks then the PBR first bank is 0  - pbr starts from the first bank
           if we do not have big PBR banks but we have small PBR banks then the PBR starts
           from the fisrt small bank*/
        if(moduleCfgPtr->ip.numOfBigPbrBanks>0)
        {
            /* consecutive banks for PBR starting from 0 */
            bankNumber = leafLine/moduleCfgPtr->ip.pbrBankSize;
        }
        else
        {
            if(moduleCfgPtr->ip.numOfSmallPbrBanks>0)
            {
                 /* consecutive banks for PBR starting from first small bank index */
                bankNumber = (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared/*48*/ -
                              moduleCfgPtr->ip.numOfSmallPbrBanks) +
                              (leafLine/moduleCfgPtr->ip.pbrBankSize);
            }
            else
            {
                /* no PBR support */
                 CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "CPSS_DXCH_SIP6_TABLE_PBR_E FAIL, no support for PBR banks \n");
            }
        }

        if (bankNumber>=PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "FAIL, bank number bigger then max allowed banks \n");
        }
        ...
        prvCpssLpmPbrIndexConvertSip7(devNum,&leafIndex);

        /* extract the bankNumber and offset from the line index */
        offsetInBank = leafIndex;
    ...
    else
    {
        if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
        {
            ...
            /* Calculate LPM leaf value */
            rc = prvCpssLpmFalconBuildLeafValue(devNum, &leafPrvData, &hwData[0]);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        else
        {
            ...
            rc = prvCpssDxChLpmNextPointerDataBuild(devNum, nextPointerData, &hwData[0]);
            if (rc != GT_OK)
            {
                return rc;
            }
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        /* extract the offset from the line index */
        offsetInBank = leafIndex;

        /* read the hwData from the correct offset of the bank */
        rc = prvCpssDxChReadTableEntry(devNum,
                                       CPSS_DXCH_SIP6_TABLE_PBR_E,
                                       offsetInBank,/* the offset in the specific PBR bank 0-256K */
                                       hwData);

        /* Convert HW value to LPM leaf SW format */
        rc =  prvCpssLpmDecodeLeafValueSip7(devNum, hwData, leafPtr);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    else
    {
        if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
        {
            ...
            /* Convert HW value to LPM leaf SW format */
            rc =  prvCpssLpmFalconDecodeLeafValue(devNum, hwData[0], leafPtr);
            if (rc != GT_OK)
            {
                return rc;
            }
...
*
* @note   APPLICABLE DEVICES:      Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
...
)
{
    ...
    {
        if ((PRV_CPSS_SIP_6_CHECK_MAC(devNum) == GT_FALSE) && (PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_FALSE))
        {
...
{
    /* The 'PBR bank size' configuration is used for an efficient addressing of PBR entries
       Based on this configuration the LPM leaf address is calculated differently
       - PBR pointer bits 0..2 always represent the PBR entry within the LPM line
       - PBR pointer bits 3..N represents the line address within the LPM memory bank
       - PBR pointer bits N+1..MAX represents the bank number (banks associated with PBR must be consecutive)
       The value of N is defined with this configuration

    0x0 = 4K; N=14 offset addr = {iu_lpm_ipvx2lpm_req_next_bucket[19:15],5'b0,iu_lpm_ipvx2lpm_req_next_bucket[14:3]}
    0x1 = 8K; N=15 offset addr = {iu_lpm_ipvx2lpm_req_next_bucket[19:16],4'b0,iu_lpm_ipvx2lpm_req_next_bucket[15:3]}
    0x2 = 16K; N=16 offset addr ={iu_lpm_ipvx2lpm_req_next_bucket[19:17],3'b0,iu_lpm_ipvx2lpm_req_next_bucket[16:3]}
    0x3 = 32K; N=17 offset addr =offset addr ={iu_lpm_ipvx2lpm_req_next_bucket[19:18],2'b0,iu_lpm_ipvx2lpm_req_next_bucket[17:3]}
    0x4 = 64K; N=18 offset addr ={iu_lpm_ipvx2lpm_req_next_bucket[19],1'b0,iu_lpm_ipvx2lpm_req_next_bucket[18:3]}
    0x5 = 128K; N=19 offset addr =iu_lpm_ipvx2lpm_req_next_bucket[19:3] */
