...
#include    <cpss/dxCh/dxChxGen/phy/private/mpdDebug.h>

#ifdef MTD_DEBUG

#include    <cpss/dxCh/dxChxGen/phy/private/mpdVersion.h>
...
static const char prvMpdDebugMTDFlagName[] = "mtd";
static const char prvMpdDebugMTDFlagHelp[] = "Enable MTD debug";
#endif
...
#if defined (MTD_PHY_INCLUDE) && defined(MTD_DEBUG)
...
static void prvMpdMtdDebugPrintFunction(
    MTD_U16 dbg_level,
    char* format,
    ...)
{
...
#define MAX_DEBUG_SIZE 1024
   char debugText[MAX_DEBUG_SIZE];
   if (PRV_MPD_DEBUG_IS_ACTIVE_MAC(PRV_MPD_NONSHARED_GLBVAR_MPD_DBG_MTD_MAC())) {
       if (dbg_level && dbg_level <= mtd_debug_level) {
           va_start(argP, format);
           vsnprintf(debugText,MAX_DEBUG_SIZE, format, argP);
           PRV_MPD_DEBUG_LOG_MAC(PRV_MPD_NONSHARED_GLBVAR_MPD_DBG_MTD_MAC())("MTD","%s",debugText);
           va_end(argP);
       }
   }
}
...
)
{
    ...
    PRV_MPD_DEBUG_BIND_MAC(prvMpdDebugComponentName,
                           prvMpdDebugPckName,
                           prvMpdDebugMTDFlagName,
                           prvMpdDebugMTDFlagHelp,
                           &(PRV_MPD_NONSHARED_GLBVAR_MPD_DBG_MTD_MAC()));

    mtdSetDbgLogFunc((MTD_DBG_API)prvMpdMtdDebugPrintFunction);
#endif
