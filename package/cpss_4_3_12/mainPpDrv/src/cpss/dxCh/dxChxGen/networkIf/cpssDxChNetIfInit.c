...
)
{
    ...
    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE,
            "The device used 'GDMA' and not supports 'MG' units for networkIf ");
    }
...
)
{
        ...
        */
        for (ii = 0; ii < CPSS_MAX_SDMA_CPU_PORTS_CNS; ii++)
        {
            ...
            if(rc == GT_OK && portNum == CPSS_CPU_PORT_NUM_CNS/* 63 */)
            {
                ...
                {
                    rc = prvCpssDxChNetIfGdmaInitAfterPortMapping(CAST_SW_DEVNUM(devNum),
                         portNum , globalDma , netIfNum);
                    if(rc != GT_OK)
                    {
                        return rc;
                    }
    ...
    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        /* GDMA has own logic for queues */
        rc = prvCpssDxChGdmaNetIfNumQueuesGet(devNum,
                        &PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfRxQueues,
                        &PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfTxQueues);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    {
        /* devices without GDMA have equal number of TX and RX queues.
           Each SDMA CPU Port (network interface) has 8 TX and 8 RX queues.
           The following function calculates amount of CPU ports and has
           additional logic that checks available MG units for Aldrin2 and Bobcat3. */
        rc = prvCpssDxChNetIfMultiNetIfNumberGet(devNum,&netIfNum);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfRxQueues =
        PRV_CPSS_DXCH_PP_MAC(devNum)->netIf.numOfTxQueues =  (netIfNum * 8);
    }
...
)
{
    ...
    {
        /********* TX Small Packets Reject ************/
        rc = prvCpssHwPpMgWriteReg(devNum,  mgUnitId, PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->sdmaRegs.txSdmaSmallPacketReject, 0x78);
        if(rc != GT_OK)
            return rc;
    }
    ...
    for(netIfCounter = 0; netIfCounter < numOfNetIfs; netIfCounter++)
    {
        ...
        for(txQueue = 0; txQueue < NUM_OF_TX_QUEUES; txQueue++,globalTxQueue++)
        {
            ...
            txDescList[globalTxQueue].revPacketIdDb =
                (GT_U32*)cpssOsMalloc(sizeof(GT_U32)*txDescList[globalTxQueue].maxDescNum);
            ...
            /* reset memory */
            cpssOsMemSet(txDescList[globalTxQueue].revPacketIdDb,0,(sizeof(GT_U32)*txDescList[globalTxQueue].maxDescNum));
...
)
{
    ...
    GT_UINTPTR  phyNext2Feed;       /* The physical address of the next2Feed*/
                                /* field.                               */
    GT_UINTPTR nextDescPointer;
    GT_U8    *descBlockPtr = NULL, *origDescBlockPtr = NULL, *tmpSramDescBlockPtr = NULL;
    GT_U32   descBlockSize;
    GT_U32      mgUnitId;/*the MG to serve the SDMA operation*/
    CPSS_TX_BUFF_MALLOC_FUNC mallocFunc = NULL;/* Function for allocating the */
                                             /* descriptors memory. */
    GT_U32 sramUsableSpace = 0;     /* Space available in CnM SRAM to use */
    GT_U32 sramStartOffset = 0;     /* Start of CnM SRAM address to be used for desc */
    GT_U32 wordCount;               /* Num of words to read or write */

    if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
    {
        PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, (CPSS_CH1_E | CPSS_CH1_DIAMOND_E | CPSS_CH2_E |
            CPSS_CH3_E | CPSS_XCAT_E | CPSS_LION_E | CPSS_XCAT2_E | CPSS_LION2_E | CPSS_BOBCAT2_E |
            CPSS_XCAT3_E | CPSS_BOBCAT3_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_ALDRIN2_E |
            CPSS_PIPE_E | CPSS_AC5P_E | CPSS_AC5_E | CPSS_HARRIER_E | CPSS_AAS_E));
    }
    ...
    if( CPSS_TX_BUFF_STATIC_ALLOC_E == sdmaParamsPtr->buffAndDescAllocMethod )
    {
        ...
        {
            /*
             * TODO: Replace with "Shared Memory Manager API" once ready
             */
            if ((PRV_CPSS_DXCH_PP_MAC(devNum)->genInfo.devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E) ||
                (PRV_CPSS_DXCH_PP_MAC(devNum)->genInfo.devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E))
            {
                /*
                 * 0x7fe00000 to 0x7fe0c000 (48KB) is left for FW to use
                 * 0x7fe0c000 to 0x7fe20000 (80KB) is used by ATF BL31
                 * 0x7fe20000 to 0x7fe30000 (64KB) is unused. During boot up this
                 * region is used by ATF BL1 but is no more in use once we reach
                 * Linux Kernel
                 */
                sramUsableSpace = (0x7fe30000 - 0x7fe20000);
                sramStartOffset = PRV_CPSS_PP_MAC(devNum)->hwInfo.cnmRam[0].cnmRamBase.l[0] + (0x7fe20000 - 0x7fe00000);
            }
            else if ((PRV_CPSS_DXCH_PP_MAC(devNum)->genInfo.devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E))
            {
                /*
                 * Using second half of the CnM SRAM for setting up descriptors
                 * and buffers.
                 */
                sramUsableSpace = (PRV_CPSS_PP_MAC(devNum)->hwInfo.cnmRam[0].cnmRamSize.l[0] / 2);
                sramStartOffset = PRV_CPSS_PP_MAC(devNum)->hwInfo.cnmRam[0].cnmRamBase.l[0] +
                    (PRV_CPSS_PP_MAC(devNum)->hwInfo.cnmRam[0].cnmRamSize.l[0] / 2);
            }

            if(sdmaParamsPtr->memData.staticAlloc.buffAndDescMemSize > sramUsableSpace)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE,
                    "No space left in CnM SRAM. Available space [0x%X]\n", sramUsableSpace);
            }

            descBlockPtr = (GT_U8*)(GT_UINTPTR)sramStartOffset;
    ...
    if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
    {
        /* Store SRAM address at which TX desc starts */
        txDescList[(netIfNum*8)+queueNum].sramTxDescStart = (GT_U32)((GT_UINTPTR)descBlockPtr);

        txDescList[(netIfNum*8)+queueNum].sramTxBuffStart =
        txDescList[(netIfNum*8)+queueNum].sramTxBuffOffset =
            (GT_U32)((GT_UINTPTR)origDescBlockPtr +
            sdmaParamsPtr->memData.staticAlloc.buffAndDescMemSize);

        txDescList[(netIfNum*8)+queueNum].sramTxBuffEnd =
            txDescList[(netIfNum*8)+queueNum].sramTxDescStart + sramUsableSpace;
    }
    ...
    {
        /*
         * Allocate memory in HOST DRAM to create TX descriptor chain to
         * copy later to SRAM
         */
        tmpSramDescBlockPtr = (GT_U8 *)
                    cpssOsMalloc(sizeof(PRV_CPSS_TX_DESC_STC )*numOfTxDesc);

        if(tmpSramDescBlockPtr == NULL)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
        }

        cpssOsMemSet(tmpSramDescBlockPtr, 0, sizeof(PRV_CPSS_TX_DESC_STC )*numOfTxDesc);
    }
    ...
    for(i = 0; i < numOfTxDesc; i++)
    {
        ...
        if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
        {
            swTxDesc->txDesc = (PRV_CPSS_TX_DESC_STC*)tmpSramDescBlockPtr;
            swTxDesc->sramTxDescOffset = (GT_U32)((GT_UINTPTR)descBlockPtr);
        }
        else
        {
            swTxDesc->txDesc = (PRV_CPSS_TX_DESC_STC*)descBlockPtr;
        ...
        if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
        {
            tmpSramDescBlockPtr = (GT_U8*)((GT_UINTPTR)tmpSramDescBlockPtr + sizeOfDesc);
        }
        ...
        if((numOfTxDesc - 1) != i)
        {
            ...
            if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
            {
                nextDescPointer = (GT_UINTPTR)descBlockPtr;
            }
            else
            {
                rc = cpssOsVirt2Phy((GT_UINTPTR)descBlockPtr, /*OUT*/&nextDescPointer);
    ...
    if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
    {
        nextDescPointer = (GT_UINTPTR)firstTxDesc;
    }
    else
    {
        rc = cpssOsVirt2Phy((GT_UINTPTR)firstTxDesc, /*OUT*/&nextDescPointer);
    ...
    if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
    {
        phyNext2Feed = (GT_UINTPTR)txDescList[(netIfNum*8)+queueNum].sramTxDescStart;
    }
    else
    {
    ...
    if(sdmaParamsPtr->queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E)
    {
        swTxDesc = firstSwTxDesc;

        for(i = 0; i < numOfTxDesc; i++)
        {
            wordCount = 4;  /* Size of word1, word2, buffPointer and nextDescPointer */
            rc = prvCpssHwPpWriteRam(devNum, (GT_U32)swTxDesc->sramTxDescOffset, wordCount, (GT_U32 *)swTxDesc->txDesc);
            if(rc != GT_OK) {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "TX SDMA Descriptor copy to CnM SRAM at 0x%X failed\n", swTxDesc->sramTxDescOffset);
            }
...
GT_STATUS printDescList (GT_U8 devNum, GT_U32 queueNum)
{
    ...
    PRV_CPSS_CHECK_SDMA_RX_Q_MAC(devNum, queueNum);

    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_IMPLEMENTED,
            "GDMA device : not implemented");
    }

    rxDescList  = &PRV_CPSS_PP_MAC(devNum)->intCtrl.rxDescList[queueNum];

    cpssOsPrintf("freeDescNum %u\n",rxDescList->freeDescNum);
    cpssOsPrintf("maxDescNum %u\n",rxDescList->maxDescNum);
    cpssOsPrintf("next2Receive->rxDesc->nextDescPointer 0x%08x\n",CPSS_32BIT_LE(rxDescList->next2Receive->rxDesc->nextDescPointer));
    cpssOsPrintf("next2Return->rxDesc->nextDescPointer 0x%08x\n",CPSS_32BIT_LE(rxDescList->next2Return->rxDesc->nextDescPointer));
    cpssOsPrintf("RX Descriptors list for queue %d:\n",queueNum);
    cpssOsPrintf("-----------------------------------\n");
    descCount =0;
    tmpRxDescPtr = rxDescList->next2Receive;
    rc = cpssOsVirt2Phy((GT_UINTPTR)(tmpRxDescPtr->rxDesc),  /*OUT*/&phyAddr);
    if (rc != GT_OK)
    {
        cpssOsPrintf("-------------------------Cant get phy address (%d) -----------\n", rc);
        phyAddr = 0;
    }

    if (queueNum < NUM_OF_RX_QUEUES)
    {
        PRV_CPSS_DXCH_PP_REGS_ADDR_STC *regsAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum);
        GT_U32                          regValue;

        /* This part is of the debug is only easily obtained for the lower 8 queues */
        rc = prvCpssHwPpReadRegister(devNum,regsAddr->sdmaRegs.rxDmaCdp[queueNum],&regValue);
        if(rc != GT_OK)
        {
            cpssOsPrintf("-------------------------Cant get rxDmaCdp (%d) -----------\n", rc);
            return rc;
        }
        cpssOsPrintf("next2Receive phyAddr SW:0x%08x, HW:0x%08x\n",phyAddr, regValue);

        rc = prvCpssHwPpReadRegister(devNum,regsAddr->sdmaRegs.rxQCmdReg,&regValue);
        if(rc != GT_OK)
        {
            cpssOsPrintf("-------------------------Cant get rxQCmdReg (%d) -----------\n", rc);
            return rc;
        }
        cpssOsPrintf("rxQCmdReg:0x%08x\n", regValue);
    }

    while(CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->nextDescPointer) != phyAddr)
    {
        cpssOsPrintf("Desc number %d, 0x%08x:%08x, nextDescPointer 0x%08x buffPointer 0x%08x \n",
                        descCount,
                        CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->word1),
                        CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->word2),
                        CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->nextDescPointer),
                        CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->buffPointer) );
        tmpRxDescPtr = tmpRxDescPtr->swNextDesc;
        descCount++;
    }
    cpssOsPrintf("Last Desc number %d, 0x%08x:%08x, nextDescPointer 0x%08x buffPointer 0x%08x \n",
                    descCount,
                    CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->word1),
                    CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->word2),
                    CPSS_32BIT_LE(tmpRxDescPtr->rxDesc->nextDescPointer),
...
/**
* @internal printTxDescList function
* @endinternal
*
* @brief   Debug function to print TX descriptor list of an SDMA queue.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
GT_STATUS printTxDescList (GT_U8 devNum, GT_U32 queueNum)
{
    /* print the descriptors linked list*/
    PRV_CPSS_TX_DESC_LIST_STC *txDescList;   /* Points to the relevant Tx desc. list */
    GT_UINTPTR phyAddr;         /* Physical Tx descriptor's address.    */
    PRV_CPSS_SW_TX_DESC_STC  *tmpTxDescPtr;
    GT_U32 descCount;
    GT_STATUS rc;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_DXCH_NETIF_INITIALIZED_CHECK_MAC(devNum);
    PRV_CPSS_CHECK_SDMA_TX_Q_MAC(devNum,queueNum);
    ...
    if ((txDescList->next2Feed == NULL) ||
        (txDescList->next2Free == NULL))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE, "printTxDescList: Queue #%d is not used for normal SDMA (maybe used for traffic generator?)", queueNum);
    }

    cpssOsPrintf("freeDescNum %u\n",txDescList->freeDescNum);
    cpssOsPrintf("maxDescNum %u\n",txDescList->maxDescNum);
    cpssOsPrintf("next2Feed->txDesc->nextDescPointer 0x%08x\n",CPSS_32BIT_LE(txDescList->next2Feed->txDesc->nextDescPointer));
    cpssOsPrintf("next2Free->txDesc->nextDescPointer 0x%08x\n",CPSS_32BIT_LE(txDescList->next2Free->txDesc->nextDescPointer));
    cpssOsPrintf("TX Descriptors list for queue %d:\n",queueNum);
    cpssOsPrintf("-----------------------------------\n");
    descCount =0;
    tmpTxDescPtr = txDescList->next2Feed;
    rc = cpssOsVirt2Phy((GT_UINTPTR)(tmpTxDescPtr->txDesc),  /*OUT*/&phyAddr);
    if (rc != GT_OK)
    {
        cpssOsPrintf("-------------------------Cant get phy address (%d) -----------\n", rc);
        phyAddr = 0;
    ...
    if (queueNum < NUM_OF_TX_QUEUES)
    {
        PRV_CPSS_DXCH_PP_REGS_ADDR_STC *regsAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum);
        GT_U32                          regValue;

        /* This part is of the debug is only easily obtained for the lower 8 queues */
        rc = prvCpssHwPpReadRegister(devNum,regsAddr->sdmaRegs.txDmaCdp[queueNum],&regValue);
        if(rc != GT_OK)
        {
            cpssOsPrintf("-------------------------Cant get txDmaCdp (%d) -----------\n", rc);
            return rc;
        }
        cpssOsPrintf("next2Feed phyAddr SW:0x%08x, HW:0x%08x\n",phyAddr, regValue);

        rc = prvCpssHwPpReadRegister(devNum,regsAddr->sdmaRegs.txQCmdReg,&regValue);
        if(rc != GT_OK)
        {
            cpssOsPrintf("-------------------------Cant get txQCmdReg (%d) -----------\n", rc);
            return rc;
        }
        cpssOsPrintf("txQCmdReg:0x%08x\n", regValue);
    }

    while(CPSS_32BIT_LE(tmpTxDescPtr->txDesc->nextDescPointer) != phyAddr)
    {
        cpssOsPrintf("Desc number %d, 0x%08x:%08x, nextDescPointer 0x%08x buffPointer 0x%08x \n",
                        descCount,
                        CPSS_32BIT_LE(tmpTxDescPtr->txDesc->word1),
                        CPSS_32BIT_LE(tmpTxDescPtr->txDesc->word2),
                        CPSS_32BIT_LE(tmpTxDescPtr->txDesc->nextDescPointer),
                        CPSS_32BIT_LE(tmpTxDescPtr->txDesc->buffPointer) );
        tmpTxDescPtr = tmpTxDescPtr->swNextDesc;
        descCount++;
    }
    cpssOsPrintf("Last Desc number %d, 0x%08x:%08x, nextDescPointer 0x%08x buffPointer 0x%08x \n",
                    descCount,
    ...
    if (!PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
             ...
             (oldSystemRecoveryInfo.systemRecoveryMode.continuousRx == GT_FALSE)) )
        {
            ...
            if(moduleCfgPtr->useMultiNetIfSdma == GT_FALSE)
            {
                if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "GDMA not supports useMultiNetIfSdma == GT_FALSE");
                }
            ...
            else /*useMultiNetIfSdma is TRUE*/
            {
                if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
                {
                    rc = prvCpssDxChGdmaNetIfRxInit(devNum);
                    if (GT_OK != rc)
                    {
                        prvCpssSystemRecoveryStateUpdate(&oldSystemRecoveryInfo);
                        return rc;
             ...
             (oldSystemRecoveryInfo.systemRecoveryMode.continuousTx == GT_FALSE)) )
        {
            ...
            if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
            {
                rc = prvCpssDxChGdmaNetIfTxInit(devNum); /*TxGeneratorInit is inside */
                if (GT_OK != rc)
                {
                    prvCpssSystemRecoveryStateUpdate(&oldSystemRecoveryInfo);
                    return rc;
                ...
                for(netIfCounter = 0; netIfCounter < numOfNetIfs; netIfCounter++)
                {
                    for(queueCounter =0; queueCounter < CPSS_MAX_TX_QUEUE_CNS; queueCounter++ )
                    {
                        if(moduleCfgPtr->multiNetIfCfg.txSdmaQueuesConfig[netIfCounter][queueCounter].numOfTxDesc != 0 &&
                            ...
                            moduleCfgPtr->multiNetIfCfg.txSdmaQueuesConfig[netIfCounter][queueCounter].queueMode == CPSS_TX_SDMA_QUEUE_MODE_SRAM_NORMAL_E))
                        {
...
)
{
    ...
    if(!PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
        if(PRV_CPSS_HW_IF_PCI_COMPATIBLE_MAC(devNum))
        {
            if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
            {
                rc = prvCpssDxChGdmaNetIfNumQueuesGet(devNum,&toCpu_numOfQueues,NULL/*fromCpu_numOfQueues*/);
                if(rc != GT_OK)
                {
                    return rc;
                }
            ...
            else
            {
                ...
                toCpu_numOfQueues = CPSS_MAX_RX_QUEUE_CNS * numOfNetIfs;
            }


            for(queueCounter = 0; queueCounter < toCpu_numOfQueues; queueCounter++ )
            {
                rc = cpssDxChNetIfSdmaRxQueueEnable(devNum, queueCounter, GT_FALSE);
                if(rc != GT_OK)
                {
                    return rc;
                }
...
)
{
    ...
    if(!PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
        if(PRV_CPSS_HW_IF_PCI_COMPATIBLE_MAC(devNum))
        {
            ...
            for(queueCounter = 0; queueCounter < toCpu_numOfQueues; queueCounter++ )
            {
                if(GT_FALSE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
                {
                    rc = cpssDxChNetIfSdmaRxResourceErrorModeSet(devNum, queueCounter,
                ...
                rc = cpssDxChNetIfSdmaRxQueueEnable(devNum, queueCounter, GT_TRUE);
                if(rc != GT_OK)
                {
