...
)
{
    ...
    switch(dsaInfoPtr->dsaType)
    {
        ...
        case CPSS_DXCH_NET_DSA_CMD_FORWARD_E:
            forwardPtr->srcIsTagged =
                (U32_GET_FIELD_MAC(dsaWord, 29, 1) == 0)
        ...
        case CPSS_DXCH_NET_DSA_CMD_FROM_CPU_E:
            if(U32_GET_FIELD_MAC(dsaWord,18,1))/* use vidx */
            {
                ...
                vidx = U32_GET_FIELD_MAC(dsaWord,
                                20,
                                9 );
...
)
{
    ...
    switch(dsaInfoPtr->dsaType)
    {
        ...
        case CPSS_DXCH_NET_DSA_CMD_FORWARD_E:
            ...
            if(useVidx)
            {
                vidx = U32_GET_FIELD_MAC(wordsPtr[1],0,12);
...
)
{
    ...
    if(PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.networkIfDir.debug_dumpFullPacket))
    {
        GT_U32 iter;
        GT_U8*  bufferPtr = &dsaBytesPtr[0];
        GT_U32  length = (dsaInfoPtr->commonParams.dsaTagType + 1) * 4;

        cpssOsPrintf("FROM_CPU device[%d] DSA tag : [%d] bytes :",
            devNum,length);
...
)
{
    ...
    numBitsPerPort = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.egfInfo.numBitsPerPort ?
                         PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.egfInfo.numBitsPerPort :
...
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_STATUS internal_cpssDxChNetIfGdmaCpuPortsInfoGet
(
    IN GT_U8                        devNum,
...
)
{
    ...
    *infoPtr = PRV_CPSS_DXCH_PP_MAC(devNum)->gdmaInfo.allCpuPortsInfo;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChNetIfGdmaCpuPortsInfoGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_RXTX_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, infoPtr));

    rc = internal_cpssDxChNetIfGdmaCpuPortsInfoGet(devNum,infoPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, infoPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_RXTX_CNS);
