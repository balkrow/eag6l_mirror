...
)
{
    ...
    {
        /* handle GDMA device */
        return prvCpssDxChGdmaNetIfRxBufFreeWithSize(devNum,rxQueue,rxBuffList,rxBuffSizeList,buffListLen);
    }
...
)
{
    ...
    PRV_CPSS_CHECK_SDMA_RX_Q_MAC(devNum, rxQueue);
    if(buffListLen == 0)
    {
...
/**
* @internal dxChNetIfRxPacketDsaInfoMoveFromBufferToStruct function
* @endinternal
*
* @brief   This function get the buffers of the packet , and remove the DSA from
*           the first buffers , and parse it into rxParamsPtr .
*          it also move the MAC_DA,MAC_SA to close the gap on the removed DSA
*           and update the pointer of the first buffer to be on the moved MAC_DA.
*
*
* @note   APPLICABLE DEVICES:     xCat3; AC5; Bobcat2; Caelum; Bobcat3; Aldrin; AC3X; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES: None
*
...
*
*/
static GT_STATUS dxChNetIfRxPacketDsaInfoMoveFromBufferToStruct
(
    IN GT_U8                                devNum,
    IN GT_U8                                queueIdx,
...
)
{
    ...
    switch(rxParamsPtr->dsaParam.dsaType)
    {
        ...
        case CPSS_DXCH_NET_DSA_CMD_FORWARD_E:
            /* srcOrDstIsTagged is GT_TRUE only for TAG0_CMD */
            srcOrDstIsTagged =
                (rxParamsPtr->dsaParam.dsaInfo.forward.srcIsTagged
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_CHECK_SDMA_RX_Q_MAC(devNum, queueIdx);
        ...
        && !PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
        if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
        {
            /* allow the GDMA to handle the buffers */
            rc = prvCpssDxChGdmaNetIfRxPacketGet(devNum, queueIdx,
                numOfBuffPtr, packetBuffsArrPtr,
                buffLenArr);
            if(rc != GT_OK)
            {
                return rc;
            ...
            /* parse the DSA and move the pointers in the first buffer  */
            rc = dxChNetIfRxPacketDsaInfoMoveFromBufferToStruct(
                devNum, queueIdx, numOfBuffPtr, packetBuffsArrPtr,
                buffLenArr, rxParamsPtr,tempBufferSize);
        ...
        if (descNum > *numOfBuffPtr)
        {
            ...
            for(i = 0; i < descNum ; i++)
            {
                ...
                rc = perDev_cpssOsPhy2Virt(devNum,(GT_UINTPTR)(CPSS_32BIT_LE(temp)), &virtAddr);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "Physical address %p to virtual conversion failed\n",
                        (GT_UINTPTR)(CPSS_32BIT_LE(temp)));
                }
...
)
{
    ...
    if (0 == sdmaByMiiNeeded)
    {
        ...
        if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
        {
            ...
            if(rc == GT_OK && GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
            {
                rc = prvCpssDxChGdmaNetIfPrePendTwoBytesHeaderSet(devNum,enable);
            }
...
)
{
        ...
        && !PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
        if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
        {
            return  prvCpssDxChGdmaNetIfRxCountersGet(devNum,queueIdx,rxCountersPtr);
        }
...
)
{
        ...
        && !PRV_CPSS_DXCH_CHECK_SDMA_PACKETS_FROM_CPU_CORRUPT_WA_MAC(devNum))
    {
        if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
        {
            return  prvCpssDxChGdmaNetIfRxErrorCountersGet(devNum,rxErrCountPtr);
        }
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_CHECK_SDMA_RX_Q_MAC(devNum, queue);

    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        return  prvCpssDxChGdmaNetIfRxQueueEnableSet(devNum,queue,enable);
    }
...
)
{
    ...
    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        return  prvCpssDxChGdmaNetIfRxQueueEnableGet(devNum,queue,enablePtr);
    }
