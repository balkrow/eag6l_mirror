...
)
{
    ...
    const GT_U32 (*meterRateBurstRange)[CPSS_DXCH_METER_RATE_BURST_GRANULARITY_CNS]; /* pointer to array of meter rate and burst granularity elements */
    ...
    for (rateType = 0; rateType < PRV_CPSS_DXCH3_POLICER_RATE_TYPE_MAX_CNS; rateType++)
    {
        ...
        if ((hwRate > actualMaxBucketSize) && (actualMaxBucketSize != 0))
        {
            continue;
        }
...
)
{
    ...
    GT_U32  mru;           /* Policer MRU */
    GT_U32  actualMaxBucketSize; /* actual maximal bucket size. MRU is taken into account */
    GT_U32  actualMaxBucketSizeByRate; /* actual maximal bucket size enlarged to HW Rate value */
    GT_U32  hwBurstSize; /* burst size in rate type caused granularity units */
    GT_STATUS rc;                /* return status */
    const GT_U32 (*meterRateBurstRange)[CPSS_DXCH_METER_RATE_BURST_GRANULARITY_CNS]; /* pointer to array of meter rate and burst granularity elements */
    ...
    else if( PRV_CPSS_DXCH3_POLICER_RATE_TYPE_MAX_CNS == rateType )
    {
            ...
            *hwRatePtr = swRate / meterRateBurstRange[*rateTypePtr][0];
            ...
            if( (actualMaxBucketSize/rateTypeBurstGranularity) <= (*hwRatePtr/rateTypeBurstGranularity) )
            {
                ...
                if (actualMaxBucketSize != swBurstSize0)
                {
                    ...
                    actualMaxBucketSizeByRate = *hwRatePtr + mru;
                }
                else
                {
                    /* burst size is less than MRU. Ignore MRU for calculation */
                    actualMaxBucketSizeByRate = *hwRatePtr;
                }

                *hwBurstSize0Ptr = actualMaxBucketSizeByRate/rateTypeBurstGranularity +
                                    (actualMaxBucketSizeByRate%rateTypeBurstGranularity == 0 ? 0: 1 );
            }
            else
            {
                hwBurstSize = swBurstSize0/rateTypeBurstGranularity +
                                    (swBurstSize0%rateTypeBurstGranularity == 0 ? 0: 1 );
                /* 16 bits in Bucket entry for all devices */
                if (hwBurstSize > 0xFFFF)
                {
                    hwBurstSize = 0xFFFF;
                }
                *hwBurstSize0Ptr = hwBurstSize;
            }
        }
        ...
        if (hwBurstSize1Ptr != NULL)
        {
            ...
            if( (actualMaxBucketSize/rateTypeBurstGranularity) <= (*hwRatePtr/rateTypeBurstGranularity) )
            {
                ...
                *hwBurstSize1Ptr = actualMaxBucketSizeByRate/rateTypeBurstGranularity +
                                    (actualMaxBucketSizeByRate%rateTypeBurstGranularity == 0 ? 0: 1 );
            }
            else
            {
                hwBurstSize = swBurstSize1/rateTypeBurstGranularity +
                                    (swBurstSize1%rateTypeBurstGranularity == 0 ? 0: 1 );
                /* 16 bits in Bucket entry for all devices */
                if (hwBurstSize > 0xFFFF)
                {
                    hwBurstSize = 0xFFFF;
                }
                *hwBurstSize1Ptr = hwBurstSize;
            }
            return GT_OK;
        }
    }
...
)
{
    ...
    {
        rc = prvCpssDxCh3PolicerHwTbCirCalculate(devNum,
                                                   stage,
                                                   swRate,
                                                   swBurstSize0,
                                                   swBurstSize1,
    ...
    {
        rc = prvCpssDxCh3PolicerHwTbCbsCalculate(devNum,
                                                   stage,
                                                   swRate,
                                                   swBurstSize0,
                                                   swBurstSize1,
    ...
    /* add option to set zero burst sizes */
    if ((swBurstSize0 == 0) && hwBurstSize0Ptr)
    {
        *hwBurstSize0Ptr = 0;
    }
    if ((swBurstSize1 == 0) && hwBurstSize1Ptr)
    {
        *hwBurstSize1Ptr = 0;
    }
    return rc;
...
)
{
    ...
    if(PRV_CPSS_PP_MAC(devNum)->isGmDevice)
    {
        ...
        {
            /* GM not do 'self clear' to the trigger bit when operating
        ...
        {
            /* the GM not do 'self clear' to the trigger bit when operating
               this function ... so to not damage other functionality ...
            */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_IMPLEMENTED, LOG_ERROR_NO_MSG);
        }
    ...
    if(((triggerCmd == PRV_CPSS_DXCH3_POLICER_CNTR_READ_AND_RESET_E) ||
        ...
        (triggerCmd == PRV_CPSS_SIP_6_10_IPFIX_ENGINE_CNTR_READ_ONLY_E)) &&
    ...
    switch(triggerCmd)
    {
        ...
        case PRV_CPSS_SIP_6_10_IPFIX_ENGINE_CNTR_READ_ONLY_E:
        ...
        case PRV_CPSS_SIP_5_20_POLICER_MANAGEMENT_COUNTERS_READ_AND_RESET_E:
            ...
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "The device not supports the 'Management Counters'\n");
            }
            break;
    ...
    if ((PRV_CPSS_SIP_6_10_IPFIX_ENGINE_READ_AND_RESET_E == triggerCmd) ||
        (PRV_CPSS_SIP_6_10_IPFIX_ENGINE_CNTR_READ_ONLY_E == triggerCmd) ||
...
)
{
    ...
    rc = prvCpssHwPpSetRegField(
        devNum, regAddr, 1, PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 1 : 2, fieldValue);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    /* Get Policer Control register, <counting_Mode> field */
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 7 : 16;
    ...
    return prvCpssHwPpSetRegField(devNum, regAddr, startBit, length, regValue);
}
...
)
{
    ...
    retStatus = prvCpssHwPpGetRegField(devNum, regAddr, startBit, 2, &regValue);
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 3 : 5;
...
)
{
    ...
    retStatus = prvCpssHwPpGetRegField(devNum, regAddr, startBit, 1, &regValue);
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 4 : 7;
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    PRV_CPSS_DXCH_PLR_STAGE_CHECK_MAC(devNum, stage);
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 12 : 24;
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 9 : 18;
...
)
{
    ...
    retStatus = prvCpssHwPpGetRegField(devNum, regAddr, startBit, 1, &hwData);
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 13 : 31;
...
)
{
    ...
    GT_U32      maxMeteringSize;   /* max size for metering memory */
    CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC  *actualSizesPtr; /* pointer on actual sizes data */
    ...
    {
        /* Metering and Counting memories are in Shared Memories in SIP7 devices.
           cpssDxChCfgPpLogicalInit divides Shared Memories between clients
           including Policer. */
        actualSizesPtr = &PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.sip7_sharedMemoryClientsActualSize.actualSizes;

        maxMeteringSize = actualSizesPtr->numEntriesForMetering;
        maxCountingSize = actualSizesPtr->numEntriesForIpfixOrBilling;
    }
    else
    {
        maxMeteringSize = PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.policersNum;
    ...
    if((iplr0MeteringSize + iplr1MeteringSize + eplrMeteringSize) > maxMeteringSize)
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /** In SIP7, counting enable field is removed from PLR_E_ATTRIBUTES table and
         *  the policerIndex field size is scaled from 16b to 17b */
        U32_SET_FIELD_IN_ENTRY_MAC(hwDataPtr,1,17,entryPtr->policerIndex);
    }
    else
    {
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /** In SIP7, counting enable field is removed from PLR_E_ATTRIBUTES table and
         *  the policerIndex field size is scaled from 16b to 17b */
        U32_GET_FIELD_IN_ENTRY_MAC(hwDataPtr,1,17,tmpValue)
        dec = 0;
    }
    else
    {
...
*
* @note   APPLICABLE DEVICES:      Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; AAS.
*
* @param[in] devNum                   - physical device number.
* @param[in] portGroupsBmp            - bitmap of Port Groups.
*                                      NOTEs:
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 2 : 4;
    regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum,stage).policerCtrl0;
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    rc = prvCpssHwPpSetRegField(devNum, regAddr, startBit, fieldLength, hwValue);
...
)
{
    ...
    rc = prvCpssHwPpGetRegField(devNum, regAddr, startBit, fieldLength, &hwValue);
...
/**
* @internal prvCpssDxChPolicerInit function
* @endinternal
*
* @brief   Initialization of Policer related DBs and default settings.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*
*/
GT_STATUS prvCpssDxChPolicerInit
(
    IN  GT_U8   devNum
)
{
    GT_STATUS   rc;
    CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC  *actualSizesPtr; /* pointer on actual sizes data */
    CPSS_DXCH_POLICER_MEMORY_STC memoryCfg; /* policer memory configuration */
    GT_U32      metersTableSize;   /* number of entries in Meters table */
    GT_U32      countingTableSize; /* number of entries in Counting/IPFIX table */
    GT_U32      regAddr;    /* register address */

    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* set different defaults according to the specific PLR unit (IPLR0/IPLR1/EPLR) */
        /* as the HW not have the needed difference */

        /*********/
        /* IPLR0 */
        /*********/
        /* bit 15 : <Policer index based metering enable> = 1 */
        /* bit 16 : <LM/IPFIX Counting Enable> = 0            */
        regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, 0).policerCtrl0;
        rc = prvCpssHwPpSetRegField(devNum, regAddr, 15, 2, 1);
        if(rc != GT_OK)
        {
            return rc;
        }
        ...
        /* IPLR1 */
        /*********/
        /* bit 15 : <Policer index based metering enable> = 0 */
        /* bit 16 : <LM/IPFIX Counting Enable> = 1            */
        regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, 1).policerCtrl0;
        rc = prvCpssHwPpSetRegField(devNum, regAddr, 15, 2, 2);
        if(rc != GT_OK)
        {
            return rc;
        }
        ...
        /* bit 16 : <LM/IPFIX Counting Enable> = 1            */
        regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum, 2).policerCtrl0;
        rc = prvCpssHwPpSetRegField(devNum, regAddr, 15, 2, 3);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    /*Configure Policer Metering Configuration Table flags*/
    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        /* IPLR0 */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[0] = GT_FALSE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[0] = GT_TRUE;
        /* IPLR1 */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[1] = GT_FALSE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[1] = GT_TRUE;
        /* EPLR */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[2] = GT_FALSE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[2] = GT_TRUE;
    }
    else if (PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
       /* In sip20 the Metering Configuration table is shared between IPLR0/1 and EPLR.
        ...
        /* IPLR0 */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[0] = GT_TRUE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[0] = GT_TRUE;
        /* IPLR1 */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[1] = GT_TRUE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[1] = GT_TRUE;
        /* EPLR */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[2] = GT_TRUE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[2] = GT_TRUE;
    }
    else if(PRV_CPSS_SIP_5_15_CHECK_MAC(devNum))
    {
       /* In sip 5_15 the Metering Configuration table is shared between IPLR0/1
        ...
        /* EPLR */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.isMeterConfigTableShared[2] = GT_FALSE;
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.meterConfigTableAddBaseAddr[2] = GT_FALSE;
     }
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* Metering and Counting memories are in Shared Memories in SIP7 devices.
           cpssDxChCfgPpLogicalInit divides Shared Memories between clients
           including Policer. Therefore call of this function is done inside
           cpssDxChCfgPpLogicalInit for SIP7 devices. */
        actualSizesPtr = &PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.sip7_sharedMemoryClientsActualSize.actualSizes;

        metersTableSize = actualSizesPtr->numEntriesForMetering;
        countingTableSize = actualSizesPtr->numEntriesForIpfixOrBilling;
    }
    else
    {
        metersTableSize = PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.policersNum;
        countingTableSize = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.plrInfo.countingEntriesNum;
    }
    ...
    if (PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.featureInfo.iplrSecondStageSupported)
    {
        /* There are two Policer Ingress stages, 256 to lower and the rest to
           upper memory */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[0] = (metersTableSize - 256);
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[1] = 256;

        /* Update the ingress policer counting memory DB with the default sizes
         */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.countingMemSize[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] = PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[0];
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.countingMemSize[CPSS_DXCH_POLICER_STAGE_INGRESS_1_E] = PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[1];
        ...
        {
            rc = cpssDxChPolicerMemorySizeModeSet(devNum, CPSS_DXCH_POLICER_MEMORY_CTRL_MODE_0_E,0,0);
            if (rc != GT_OK)
            {
                CPSS_LOG_INFORMATION_MAC("cpssDxChPolicerMemorySizeModeSet(..) : return code is %d", rc);
                return rc;
            }
        }
    }
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        /* AC5P and above sets policer metering and counting memory differently as sizes
        ...
        if(PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesEplrMetering)
        {
            ...
            memoryCfg.numMeteringEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] =
                metersTableSize - (128/*IPLR1*/ + _1K/*EPLR*/);
            ...
            memoryCfg.numCountingEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] =
                countingTableSize - (128/*IPLR1*/ + _1K/*EPLR*/);
        ...
        {
            if(PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.featureInfo.iplrSecondStageSupported == GT_FALSE)
            {
                /*Ironman_S*/
                /* all goes to IPLR0 (No IPLR1 and no EPLR) */
                memoryCfg.numMeteringEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] = metersTableSize;

                memoryCfg.numCountingEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] = countingTableSize;
            }
            else /* no such device at the moment */
            {
                /* give 128 to PLR1 */
                memoryCfg.numMeteringEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] = metersTableSize - 128/*IPLR1*/;
                memoryCfg.numMeteringEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_1_E] = 128;

                memoryCfg.numCountingEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_0_E] = countingTableSize - 128/*IPLR1*/;
                memoryCfg.numCountingEntries[CPSS_DXCH_POLICER_STAGE_INGRESS_1_E] = 128;
            }
        }
        ...
        if (rc != GT_OK)
        {
            CPSS_LOG_INFORMATION_MAC("cpssDxChPolicerMemorySizeSet(..) : return code is %d", rc);
            return rc;
        }
    }
    else if(PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
        /* set PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[2] by
    ...
    else
    {
        ...
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[2] =
            PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.egressPolicersNum;
        /* Update the egress policer counting memory DB with the default sizes
         */
        PRV_CPSS_DXCH_PP_MAC(devNum)->policer.countingMemSize[CPSS_DXCH_POLICER_STAGE_EGRESS_E] = PRV_CPSS_DXCH_PP_MAC(devNum)->policer.memSize[2];
    }
    ...
    rc = cpssDxChPolicerMeteringCalcMethodSet(devNum, CPSS_DXCH_POLICER_METERING_CALC_METHOD_CIR_AND_CBS_E /*meteringCalcMethod*/,
                                              10 /*cirPirAllowedDeviation*/, GT_TRUE /*cbsPbsCalcOnFail*/);
    if (rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("cpssDxChPolicerMeteringCalcMethodSet(..) : return code is %d", rc);
        return rc;
    }
