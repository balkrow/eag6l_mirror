...
/* amount of words in bitmap of TCAM blocks*/
/* AAS has 48 floors, i.e. 96 blocks       */
#define PRV_WORDS_IN_BLOCK_BITMAP_CNS 3
...
/**
* @internal prvCpssDxChVirtualTcamDbSegSpaceHorzBlockRowsTreePrint function
* @endinternal
*
* @brief   Print Horizontal blocks rows tree.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] treeId                   - thee to print
*
* @retval GT_OK                    - success.
* @retval GT_NO_RESOURCE           - on memory allocation fail.
*/
void prvCpssDxChVirtualTcamDbSegSpaceHorzBlockRowsTreePrint
(
...
)
{
    ...
    GT_U32 priority, horzBlockIndex, rowsBase, rowsAmount;
            ...
            treeId, seekType, path, &dbEntryPtr))
    {
        ...
        PRV_HORZ_BLOCK_ROWS_RANGE4_FROM_VOIDPTR_MAC(
            dbEntryPtr, priority, horzBlockIndex, rowsBase, rowsAmount);
        cpssOsPrintf(
            "priority %d horzBlockIndex %d rowsBase %d rowsAmount %d \n",
            priority, horzBlockIndex, rowsBase, rowsAmount);
    }
}
...
)
{
    ...
    *ruleUseActualPtr = maxRuleUseAmount;
    ...
    for (i = 0; (i < rulePtr->changeArrSize); i++)
    {
        srcIdx = (GT_U32)rulePtr->changeArr[i].source;
        if (srcIdx >= (GT_U32)PRV_SEG_DRAG_HORZ_BLOCK_ROW_STATUS_INDEX_LAST_E)
        {/*Klockwork*/
            prvCpssDxChVirtualTcamDbSegmentTableException();
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }
        dstIdx = (GT_U32)rulePtr->changeArr[i].target;
        if (dstIdx >= (GT_U32)PRV_SEG_DRAG_HORZ_BLOCK_ROW_STATUS_INDEX_LAST_E)
        {/*Klockwork*/
            prvCpssDxChVirtualTcamDbSegmentTableException();
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    work_rules_arr = (PRV_SEG_DRAG_HORZ_BLOCK_ROW_STATUS_CHANGE_RULE_PTR*)&(VTCAM_GLOVAR(work_rules_arr[0]));
    work_rules_arr_size =
    ...
    while (1) /* break conditions at the end */
    {
        ...
        for (ruleSizeIdx = 0; (ruleSizeIdx < ruleSizesArrSize); ruleSizeIdx++)
        {
            ...
            for (vTcamId = 0; (vTcamId < pVTcamMngPtr->vTcamCfgPtrArrSize); vTcamId++)
            {
                ...
                if (vTcamInfoPtr->tcamInfo.tcamSpaceUnmovable != CPSS_DXCH_VIRTUAL_TCAM_MOVABLE_E) continue;
...
)
{
    ...
    GT_U32    unmovableBlocksBmpArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
...
)
{
    ...
    {
        cpssOsPrintf("prvCpssDxChVirtualTcamDbSegSpaceDragVTcamsToOtherSpaceTree\n");
        cpssOsPrintf("source space tree: \n");
        prvCpssDxChVirtualTcamDbSegSpaceHorzBlockRowsTreePrint(sourceSpaceTree);
        cpssOsPrintf("target space tree: \n");
        prvCpssDxChVirtualTcamDbSegSpaceHorzBlockRowsTreePrint(targetSpaceTree);
        cpssOsPrintf("\n");
    }
            ...
            for (horzBlockIdx = 0; (horzBlockIdx < blocksInRow); horzBlockIdx++)
            {
                        ...
                        rangePath, &rangeFoundDataPtr))
                {
                    ...
                    if (remainderRowsBase < commonRowsBase)
                    {
                        ...
                        segCopyPtr->rowsAmount = (commonRowsBase - remainderRowsBase);
                        rc = prvCpssAvlItemInsert(segmentsTree, segCopyPtr);
                        if (rc != GT_OK)
                        {
                            cpssBmPoolBufFree(pVTcamMngPtr->segmentsPool, segCopyPtr);
...
)
{
    ...
    PRV_CPSS_AVL_TREE_PATH                              path;
    GT_VOID                                             *dbDataPtr;
    GT_U32                                              base, num, mask, idx, w0, w1;
...
)
{
    GT_STATUS rc;
    GT_U32    actualCapacity;
    GT_U32    freeBlocksBmpArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
    GT_U32    lookupBlocksBmpArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
    GT_U32    freeBlocksBmpCopyArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
    GT_U32    lookupBlocksBmpCopyArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
    ...
    GT_U32    usedBlocksBmpArr[PRV_WORDS_IN_BLOCK_BITMAP_CNS] = {0};
