...
/**
* @internal printSegment function
* @endinternal
*
* @brief   print single segment..
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] segmentsNodePtr                - a pointer to the segments structure
*                                       None
*/
static GT_VOID printSegment
(
    IN PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_ENTRY_STC  *segmentsNodePtr
)
{
    cpssOsPrintf(
        "rowsBase %d rowsAmount %d segmentColumnsMap \n",
        segmentsNodePtr->rowsBase, segmentsNodePtr->rowsAmount);
    cpssOsPrintf("segmentColumnsMap ");
    print_segmentColumnsMap(segmentsNodePtr->segmentColumnsMap);
    cpssOsPrintf("\n");
    cpssOsPrintf(
        "vTcamId %d lookupId %d rulesAmount %d ruleSize %d baseLogicalIndex %d \n",
        segmentsNodePtr->vTcamId, segmentsNodePtr->lookupId,
        segmentsNodePtr->rulesAmount, segmentsNodePtr->ruleSize,
        segmentsNodePtr->baseLogicalIndex);
}
...
*
*/
GT_VOID prvCpssDxChVirtualTcamDbVTcamDumpFullTcamPriorityModeDataParam
(
    IN  GT_U32  vTcamMngId,
    IN  GT_U32  vTcamId
)
{
    ...
    if (vTcamMngPtr->vTcamCfgPtrArr == NULL)
    {
        cpssOsPrintf("\nvTcamMngPtr->vTcamCfgPtrArr == NULL\n");
        return;
    }
    for (vTcamId = 0; (vTcamId < vTcamMngPtr->vTcamCfgPtrArrSize); vTcamId++)
    {
...
GT_VOID prvCpssDxChVirtualTcamDbVTcamDumpFullTcamPriorityModeData()
{
    ...
    prvCpssDxChVirtualTcamDbVTcamDumpFullTcamPriorityModeDataParam(0, 0);
}
...
)
{
    ...
    if (vTcamMngPtr == NULL)
    {
        cpssOsPrintf("tcamSegCfgPtr->vTcamMngPtr == NULL\n");
        return;
    }
    if (vTcamMngPtr->vTcamCfgPtrArr == NULL)
    {
        cpssOsPrintf("vTcamMngPtr->vTcamCfgPtrArr == NULL\n");
        return;
    }
    ...
    for (vTcamId = 0; (vTcamId < vTcamMngPtr->vTcamCfgPtrArrSize); vTcamId++)
    {
        ...
        while (seeked != GT_FALSE)
        {
            ...
            if (rc != GT_OK)
            {
                ...
                printSegment((PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_ENTRY_STC*)dbEntryPtr);
            }
            seeked = prvCpssAvlPathSeek(
