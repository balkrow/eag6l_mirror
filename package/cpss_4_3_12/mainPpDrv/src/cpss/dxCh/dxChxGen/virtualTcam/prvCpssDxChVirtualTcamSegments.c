...
#ifdef SHARED_MEMORY
extern GT_STATUS prvCpssDxChVirtualTcamManagerBlockFromLookupUnmap
(
...
/**
* @internal prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsBitmapToSegColMap function
* @endinternal
*
* @brief   Convert bitmap of minimal rule places to segment columns map.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] tcamWidth                - number of columns in TCAM
* @param[in] horzBlockWidth           - amount of columns in horizontal block
* @param[in] minRulePlacesBmp         - bitmap of minimal rule places
* @param[in] segmentColumnsMap        - segment columns map
*
* @retval GT_OK                    - success.
* @retval other                    - fail.
*/
GT_STATUS prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsBitmapToSegColMap
(
    IN  GT_U32                                              tcamWidth,
    IN  GT_U32                                              horzBlockWidth,
    IN  GT_U32                                              minRulePlacesBmp,
    OUT PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP *segmentColumnsMapPtr
)
{
    static const GT_U32 bitmapArr[] =
        PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_BITMAPS_MAC;
    static const GT_U32 bitmapArrSize = (sizeof(bitmapArr) / sizeof(bitmapArr[0]));
    GT_U32 mask;
    GT_U32 bmp;
    GT_U32 horzBlock;
    GT_U32 horzBlocksNum;
    GT_U32 i;
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP segmentColumnsMap;

    mask = (1 << horzBlockWidth) - 1;
    horzBlocksNum = tcamWidth / horzBlockWidth;
    segmentColumnsMap = PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_EMPTY_CNS;
    for (horzBlock = 0; (horzBlock < horzBlocksNum); horzBlock++)
    {
        bmp = (minRulePlacesBmp >> (horzBlockWidth * horzBlock)) & mask;
        for (i = 0; (i < bitmapArrSize); i++)
        {
            if (bmp == bitmapArr[i])
            {
                PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_SET_MAC(
                    segmentColumnsMap, horzBlock, i);
                break;
            }
        }
        if (i >= bitmapArrSize)
    ...
    *segmentColumnsMapPtr = segmentColumnsMap;
    return GT_OK;
}

/**
* @internal prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsSpaceToSegTree function
* @endinternal
*
* @brief   Convert bitmap of minimal rule places space to segment tree and remove it from free space,
*           map Blocks to client group/hit number.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] vTcamId                 - vTcam Id
* @param[in] tcamCfgPtr              - pointer to internal DM vTCAM structure
...
* @retval other                    - fail.
*/
GT_STATUS prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsSpaceToSegTree
(
    IN   GT_U32                                      vTcamId,
...
)
{
    ...
    GT_U32    rowsAmount;
    GT_U32    rowsHigh;
    GT_U32    segRowsAmount;

    lookupId = PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_LOOKUP_ID_MAC(
        tcamCfgPtr->tcamInfo.clientGroup,
        tcamCfgPtr->tcamInfo.hitNumber);

    ruleSize =
        prvCpssDxChVirtualTcamSegmentTableRuleSizeToUnits(
            tcamCfgPtr->tcamSegCfgPtr, tcamCfgPtr->tcamInfo.ruleSize);

    rc = prvCpssAvlTreeCreate(
            (GT_INTFUNCPTR)prvCpssDxChVirtualTcamDbSegmentTableLookupOrderCompareFunc,
            tcamCfgPtr->tcamSegCfgPtr->pVTcamMngPtr->segNodesPool, &(tcamCfgPtr->segmentsTree));
    if (rc != GT_OK)
    {
    ...
    for (i = 0; (i < allocationSpaceArrSize); i++)
    {
        rc = prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsBitmapToSegColMap(
            tcamCfgPtr->tcamSegCfgPtr->tcamColumnsAmount,
            tcamCfgPtr->tcamSegCfgPtr->lookupColumnAlignment,
            allocationSpaceArr[i].columnsBitmap,
            &segmentColumnsMap);
        if (rc != GT_OK)
        {
            prvCpssAvlTreeDelete(
                ...
                (GT_VOID*)tcamCfgPtr->tcamSegCfgPtr->pVTcamMngPtr->segNodesPool);
            tcamCfgPtr->segmentsTree = NULL;
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "wrong columns bitmap");
        }
        rowsBase    = allocationSpaceArr[i].startRowIndex;
        rowsAmount  = allocationSpaceArr[i].numOfRows;

        while (rowsAmount > 0)
        {
            rowsHigh = rowsBase + tcamCfgPtr->tcamSegCfgPtr->lookupRowAlignment;
            rowsHigh -= (rowsHigh % tcamCfgPtr->tcamSegCfgPtr->lookupRowAlignment);
            segRowsAmount = rowsHigh - rowsBase;
            if (segRowsAmount > rowsAmount)
            {
                segRowsAmount = rowsAmount;
            }
            segPtr    = cpssBmPoolBufGet(tcamCfgPtr->tcamSegCfgPtr->pVTcamMngPtr->segmentsPool);
            if (segPtr == NULL)
            {
                prvCpssAvlTreeDelete(
                    tcamCfgPtr->segmentsTree,
            ...
            rowsBase    += segRowsAmount;
            rowsAmount  -= segRowsAmount;
        }
    }
    ...
    /* remove allocated space from free space tree */
    rc =  prvCpssDxChVirtualTcamDbSegmentTableTreeRemoveFromFreeSpaceAndMap(
        tcamCfgPtr->tcamSegCfgPtr, lookupId, tcamCfgPtr->segmentsTree);
    if (rc != GT_OK)
    {
        prvCpssAvlTreeDelete(
            tcamCfgPtr->segmentsTree,
            ...
            (GT_VOID*)tcamCfgPtr->tcamSegCfgPtr->pVTcamMngPtr->segNodesPool);
        tcamCfgPtr->segmentsTree = NULL;
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChVirtualTcamDbSegmentTablePrimaryTreeAllocAndMap");
    }
...
GT_COMP_RES prvCpssDxChVirtualTcamDbSegmentTableFreeCompare
(
    IN  GT_VOID    *data_ptr1,
    IN  GT_VOID    *data_ptr2
)
...
#else
            rc = prvCpssDxChVirtualTcamManagerBlockFromLookupUnmap(
#endif
...
)
{
    ...
    lookupIdArr[0] = tcamSegCfgPtr->blockInfArr[blockIndex].lookupId;
    lookupIdArr[1] =
        PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_LOOKUP_FREE_BLOCK_CNS;

    for (lookupIdNdx = 0; (lookupIdNdx < 2); lookupIdNdx++)
    {
        cpssOsMemSet(&segPattern, 0, sizeof(segPattern));
        segPattern.lookupId          = lookupIdArr[lookupIdNdx];
        ...
        do
        {
            ...
            if ((segPtr->lookupId != lookupIdArr[lookupIdNdx])) break;
...
/**
* @internal prvCpssDxChVirtualTcamDbSegmentTableCheckThatMemoryIsFree function
* @endinternal
*
* @brief   Check that the given memory contained in free space DB.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*
* @retval GT_OK                    - on success
* @retval GT_NO_SUCH               - memory is not free
* @retval GT_BAD_PARAM             - wrong input parameters
* @retval other                    - on fail (something in DB wrong)
*/
GT_STATUS prvCpssDxChVirtualTcamDbSegmentTableCheckThatMemoryIsFree
(
    IN     PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_CFG_STC             *tcamSegCfgPtr,
    IN     GT_U32                                                     lookupId,
    IN     GT_U32                                                     rowsBase,
...
)
{
    ...
    GT_U32                                                     bitmap;
    GT_U32                                                     blockHorzIdx;
    GT_U32                                                     blockHorzRowsBase;
    GT_U32                                                     blockHorzRowsAmount;
    GT_U32                                                     checkedSegColBmp;
    GT_U32                                                     foundSegColumns;
    GT_U32                                                     foundSegColBmp;
    GT_U32                                                     rowBound;
    GT_U32                                                     rowsCovered;
    ...
    for (blockHorzIdx = 0; (blockHorzIdx < blocksInRow); blockHorzIdx++)
    {
        ...
        PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_BITMAP_OF_COLUMNS_MAC(
            segmentColumns, checkedSegColBmp);
    ...
    for (blockHorzIdx = 0; (blockHorzIdx < blocksInRow); blockHorzIdx++)
    {
        ...
        blockHorzRowsBase   = rowsBase;
        blockHorzRowsAmount = rowsAmount;
        segmentColumns =
            PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_GET_MAC(
                segmentColumnsMap, blockHorzIdx);
        PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_BITMAP_OF_COLUMNS_MAC(
        ...
        while (1)
        {
            rc = prvCpssDxChVirtualTcamDbSegmentTableFindFirstCoveringSegment(
                tcamSegCfgPtr, tcamSegCfgPtr->segFreeLuTree,
                blockHorzRowsBase, blockHorzRowsAmount, blockHorzIdx, segmentColumns,
                path, &segPtr);
            if (rc != GT_OK)
            {
            ...
            if ((segPtr->lookupId != lookupId) &&
                (segPtr->lookupId != PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_LOOKUP_FREE_BLOCK_CNS))
            {
                /* found free subspace mapped to another lookup */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_SUCH, LOG_ERROR_NO_MSG);
            }
            if (segPtr->rowsBase > blockHorzRowsBase)
            {
                /* rowsBase-th row not covered */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_SUCH, LOG_ERROR_NO_MSG);
            }
            foundSegColumns =
                PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_GET_MAC(
                segPtr->segmentColumnsMap, blockHorzIdx);
            PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP_BITMAP_OF_COLUMNS_MAC(
                foundSegColumns, foundSegColBmp);
            if (checkedSegColBmp & (~ foundSegColBmp))
            {
                /* intersects by columns, but not covers */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_SUCH, LOG_ERROR_NO_MSG);
            }
            rowBound = segPtr->rowsBase + segPtr->rowsAmount;
            if (rowBound <= blockHorzRowsBase)
            {
                /* should not occur */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
            }
            rowsCovered = rowBound - blockHorzRowsBase;
            if (rowsCovered >= blockHorzRowsAmount)
            {
                /* all horizontal blocks column covered */
                break;
            }
            blockHorzRowsBase   += rowsCovered;
            blockHorzRowsAmount -= rowsCovered;
        }
    }
    ...
    */
    ...
    for (i = 0; i < tcamSegCfgPtr->blocksAmount - 1; i++)
    {
        ...
        for (vTcamId = 0; vTcamId < pVTcamMngPtr->vTcamCfgPtrArrSize; vTcamId++)
        {
            ...
            if (vTcamCfgPtr->tcamInfo.tcamSpaceUnmovable != CPSS_DXCH_VIRTUAL_TCAM_MOVABLE_E) continue;
...
*
* @param[in] tcamSegCfgPtr            - (pointer to) segment configuration information.
* @param[in] lookupId                 - lookupId (combined client group and hit number)
* @param[in] treeId                   - primary tree for allocation.
*
* @retval GT_OK                    - on success
* @retval other                    - on fail
*/
GT_STATUS prvCpssDxChVirtualTcamDbSegmentTablePrimaryTreeAllocAndMap
(
    IN     PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_CFG_STC              *tcamSegCfgPtr,
    IN     GT_U32                                                     lookupId,
    IN     PRV_CPSS_AVL_TREE_ID                                       treeId
)
{
    ...
    if (rc == GT_OK)
    {
        ...
        {
            savedBlockPtr[savedBlockNum]->lookupId = lookupId;
        }
    }
...
/**
* @internal prvCpssDxChVirtualTcamDbSegmentTableTreeRemoveFromFreeSpaceAndMap function
* @endinternal
*
* @brief   Remove tree from free space and map free blocks to lookup.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
* @retval other                    - on fail
*/
GT_STATUS prvCpssDxChVirtualTcamDbSegmentTableTreeRemoveFromFreeSpaceAndMap
(
    IN     PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_CFG_STC              *tcamSegCfgPtr,
    IN     GT_U32                                                     lookupId,
    IN     PRV_CPSS_AVL_TREE_ID                                       treeId
...
{
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_ENTRY_STC *segPtr; /* found segment  */
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_ENTRY_STC *freeSegPtr; /* found free segment  */
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_ENTRY_STC segPattern; /* pattern segment  */
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_BLOCK_INFO_STC* blockPtr; /* pointer to block info */
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ENT segmentColumns; /* columns inside block */
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP          segmentColumnsMap; /* columns map */
    GT_STATUS               rc = GT_OK;  /* return code                        */
    GT_U32                  blocksInRow; /* number of blocks In Row            */
    GT_U32                  blockHorzIdx;/* block In Row index                 */
    GT_BOOL                 seeked;      /* node seeked                        */
    ...
    /* fix lookup ids in free space DB */
    seeked = prvCpssAvlPathSeek(
            treeId, PRV_CPSS_AVL_TREE_SEEK_FIRST_E,
            path, &dbEntryPtr /* use dedicated var to avoid warnings */);
    segPtr = dbEntryPtr;
    while (seeked != GT_FALSE)
    {
        for (blockHorzIdx = 0; (blockHorzIdx < blocksInRow); blockHorzIdx++)
        {
            ...
            segPattern.lookupId =
                PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_LOOKUP_FREE_BLOCK_CNS;
            segPattern.rowsBase = blockPtr->rowsBase;
            segPattern.segmentColumnsMap = segmentColumnsMap;

            freeSegPtr = prvCpssAvlSearch(
                tcamSegCfgPtr->segFreeLuTree, &segPattern);
            if ((freeSegPtr == NULL) || (freeSegPtr->lookupId
                != PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_LOOKUP_FREE_BLOCK_CNS))
            {
                continue;
            }
            ...
            /* update segment lookup Id */
            prvCpssDxChVirtualTcamSegmentTableAvlItemRemove(
                tcamSegCfgPtr->segFreeLuTree, (GT_VOID*)freeSegPtr);
            prvCpssDxChVirtualTcamSegmentTableAvlItemRemove(
                tcamSegCfgPtr->segFreePrefTree, (GT_VOID*)freeSegPtr);
            freeSegPtr->lookupId = lookupId;
            rc = prvCpssDxChVirtualTcamSegmentTableAvlItemInsert(
                tcamSegCfgPtr->segFreeLuTree, freeSegPtr);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "failed on prvCpssAvlItemInsert");
            }
            rc = prvCpssDxChVirtualTcamSegmentTableAvlItemInsert(
                tcamSegCfgPtr->segFreePrefTree, freeSegPtr);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "failed on prvCpssAvlItemInsert");
            }
...
)
{
    ...
    rc = prvCpssDxChVirtualTcamDbSegmentTablePrimaryTreeAllocAndMap(
        tcamSegCfgPtr, allocRequestPtr->lookupId, treeId);
    if (rc != GT_OK)
    {
