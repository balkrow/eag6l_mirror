...
#if CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS != PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS
    #error "CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS and PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS must be equal"
#endif

/*due to const will be located in read only section*/
static GT_COMP_RES (* const compareFunctionDb[])(GT_VOID *,GT_VOID *) =
{
   prvCpssDxChVirtualTcamDbSegmentTableLookupOrderCompareFunc,
   prvCpssDxChVirtualTcamDbRuleIdTableRuleIdCompare,
   prvCpssDxChVirtualTcamDbRuleIdTableLogicalIndexCompare,
   prvCpssDxChVirtualTcamDbPriorityTablePriorityCompare,
   prvCpssDxChVirtualTcamDbPriorityTableLogicalIndexCompare,
   prvCpssDxChVirtualTcamDbSegmentTableFreeCompare,
   prvCpssDxChVirtualTcamDbSegmentTableLookupOrderCompareFreeSegFunc,
   prvCpssDxChVirtualTcamDbSegSpaceWorkDbCompareFunc,
   prvCpssDxChVirtualTcamDbSegSpaceWorkDbSourceCompareFunc,
   prvCpssDxChVirtualTcamDbSegSpaceWorkDbTargetCompareFunc,
   NULL
};
...
tcamRuleSizeMap[CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_80_B_E + 1] =
{
    ...
    (CPSS_DXCH_TCAM_RULE_SIZE_ENT)0,
...
)
{
    ...
    vTcamMngPtr = VTCAM_MNG_DB_MAC(vTcamMngId);
    if(vTcamMngPtr == NULL)
    {
...
)
{
    ...
    if (vTcamInfoPtr->tcamInfo.tcamSpaceUnmovable
        == CPSS_DXCH_VIRTUAL_TCAM_UNMOVABLE_APP_ALLOCATED_E)
    {
        /* not relevant to Application allocated vTCAM */
        return GT_OK;
    }
...
)
{
    ...
    if (vTcamInfoPtr->tcamInfo.tcamSpaceUnmovable
        == CPSS_DXCH_VIRTUAL_TCAM_UNMOVABLE_APP_ALLOCATED_E)
    {
        /* all DB contents created below already done at HA Replay stage */
        return GT_OK;
    }
...
)
{
    ...
    for (i=0; i<totalEntries; ++i)
    {
        ...
        /* read action */
        cpssOsMemSet(action, 0, (CPSS_DXCH_PCL_ACTION_SIZE_IN_WORDS_CNS * 4));
...
)
{
    ...
    CPSS_DXCH_VIRTUAL_TCAM_HA_UNMAPPED_RULE_ENTRY_STC
                                                 *replayRulePtr = NULL;
    GT_U32                                        unmovableSegmentsTotal;
    GT_U32                                        segmentsTotal;
    GT_U32                                        numOfSegments;
    GT_U32                                        vTcamId;
    CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC      *segmentArrayPtr;
    CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC      *segmentPtr;
    GT_U32                                        segIdx;
    GT_U32                                        segFound;
    ...
    /* unmovable Application allocation vTCAMs support */

    segmentArrayPtr = NULL;
    unmovableSegmentsTotal = 0;
    for (vTcamId = 0; (vTcamId < vTcamMngPtr->vTcamCfgPtrArrSize); vTcamId++)
    {
        if (vTcamMngPtr->vTcamCfgPtrArr[vTcamId] == NULL) continue;
        if (vTcamMngPtr->vTcamCfgPtrArr[vTcamId]->tcamInfo.tcamSpaceUnmovable
            != CPSS_DXCH_VIRTUAL_TCAM_UNMOVABLE_APP_ALLOCATED_E) continue;

        /* get amount of segments only */
        rc = cpssDxChVirtualTcamSpaceLayoutGet(
            vTcamMngId, vTcamId, 0/*maxNumOfSegments*/,
            &numOfSegments /*numOfSegmentsPtr*/,
            NULL /*segmentArrayPtr*/);
        if (rc != GT_OK)
        {
            return rc;
        }
        unmovableSegmentsTotal += numOfSegments;
    }

    if (unmovableSegmentsTotal != 0)
    {
        segmentArrayPtr = (CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC*)cpssOsMalloc(
            (sizeof(CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC) * unmovableSegmentsTotal));
        ...
        for (vTcamId = 0; (vTcamId < vTcamMngPtr->vTcamCfgPtrArrSize); vTcamId++)
        {
            ...
            rc = cpssDxChVirtualTcamSpaceLayoutGet(
                vTcamMngId, vTcamId,
                (unmovableSegmentsTotal - segmentsTotal)/*maxNumOfSegments*/,
                &numOfSegments /*numOfSegmentsPtr*/,
                &(segmentArrayPtr[segmentsTotal]) /*segmentArrayPtr*/);
            if (rc != GT_OK)
            {
                return rc;
            }
            segmentsTotal += numOfSegments;
        }
    }
            ...
            for(i=0; i< vTcamMngPtr->vTcamHaDb[tcamRuleSize][lkp].numRules; i++)
            {
                if( vTcamMngPtr->vTcamHaDb[tcamRuleSize][lkp].rulesArr[i].found == 0 )
                {
                    ...
                    if (vTcamMngPtr->haFeaturesSupportedBmp & CPSS_DXCH_VIRTUAL_TCAM_HA_FEATURE_SAVE_ALL_RULES_E)
                    {
                        ...
                        {
                            ((CPSS_DXCH_VIRTUAL_TCAM_HA_UNMAPPED_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayUnmappedRuleDbTail))->next = replayRulePtr;
                        }
                    ...
                    segFound = 0;
                    if (unmovableSegmentsTotal)
                    {
                        /* bypass rules in unmovable segments */
                        /* for legacy (not SIP5) devices PCL TCAM only supported */
                        physicalIndex = vTcamMngPtr->vTcamHaDb[tcamRuleSize][lkp].rulesArr[i].physicalIndex;
                        row    = physicalIndex / vTcamMngPtr->tcamSegCfg.tcamColumnsAmount;
                        column = physicalIndex % vTcamMngPtr->tcamSegCfg.tcamColumnsAmount;
                        for (segIdx = 0; (segIdx < unmovableSegmentsTotal); segIdx++)
                        {
                            segmentPtr = &(segmentArrayPtr[segIdx]);

                            if (segmentPtr->startRowIndex > row) continue;
                            if ((segmentPtr->startRowIndex
                                 + segmentPtr->numOfRows) <= row) continue;
                            if ((segmentPtr->columnsBitmap & (1 << column)) == 0) continue;
...
)
{
    ...
    GT_U32                                           hwActionSizeBytes;

    hwActionSizeBytes = CPSS_DXCH_PCL_ACTION_SIZE_IN_WORDS_CNS * 4;
        ...
        for (i = 0; (i < vTcamMngPtr->vTcamHaDb[tcamRuleSize][l].numRules); i++)
        {
            ...
            if ((rulePtr->vTcamId == vTcamId) && (rulePtr->ruleId == ruleId))
            {
                if ((prvCpssPatternMaskCompare(
                    ...
                    (cpssOsMemCmp(hwAction,  rulePtr->action, hwActionSizeBytes) == 0))
                {
                    rulePtr->found = 1;
...
)
{
    ...
    if(! PRV_CPSS_SIP_5_15_CHECK_MAC(devNum)) /* BobK and above */
    {
    ...
    if (VTCAM_GLOVAR(replayRuleDb))
    {
        ...
        for (vTcamMngId = 0;vTcamMngId <= CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS; ++vTcamMngId)
        {
                 ...
                 || vTcamMngPtr->haFeaturesSupportedBmp & CPSS_DXCH_VIRTUAL_TCAM_HA_FEATURE_SAVE_ALL_RULES_E )
            {
                ...
                replayRulePtr = (CPSS_DXCH_VIRTUAL_TCAM_HA_DB_REPLAY_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayRuleDb);
...
)
{
    ...
    if (vTcamMngPtr->vTcamCfgPtrArr[vTcamId]->tcamInfo.tcamSpaceUnmovable
        == CPSS_DXCH_VIRTUAL_TCAM_UNMOVABLE_APP_ALLOCATED_E)
    {
        /* all segment DB created at HA Replay stage */
        return GT_OK;
    }
        ...
        switch(vtcamRuleSize)
        {
            ...
            case CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_30_B_E :
                ...
                for(j=0; j<usedRowsArrSize; j++)
                {
                    ...
                    {
                        colInBlockBmp |= (GT_U32)usedRowsArr[j].colInBlockBmp;
...
)
{
    ...
    for(i=0; i < totalEntries; ++i)
    {
        rc = perDevice_ruleClear(devNum, portGroupsBmp, i, devClass, CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_10_B_E);
        if (GT_OK != rc)
        {
...
)
{
    ...
    physicalIndex   = rowsBase * 12 + columnsBase;
     ...
     if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
     {
         ...
         ruleSize = CPSS_DXCH_TCAM_RULE_SIZE_20_B_E;
     }
     else
     {
...
)
{
    ...
    vTcamMngDBPtr = VTCAM_MNG_DB_MAC(vTcamMngId);
...
)
{
    ...
    if (VTCAM_MNG_DB_MAC(vTcamMngId) == NULL)
    {
...
)
{
    switch (ruleFormat)
    {
        ...
        case CPSS_DXCH_PCL_RULE_FORMAT_EGRESS_UDB_80_E                                  :
 ...
 *
 */
GT_STATUS prvCpssDxChVirtualTcamManagerBlockFromLookupUnmap
(
    IN  GT_VOID     *cookiePtr,
    IN  GT_U32      baseRow,
    IN  GT_U32      baseColumn
...
)
{
    ...
    pVTcamMngPtr = VTCAM_MNG_DB_MAC(vTcamMngId);
...
)
{
    ...
    VTCAM_MNG_DB_PTR_MAC(vTcamMngId) = NULL;
...
)
{
    ...
    VTCAM_MNG_DB_PTR_MAC(vTcamMngId) = pVTcamMngPtr;
...
)
{
    ...
    tcamMngDbPtr = VTCAM_MNG_DB_MAC(vTcamMngId);
...
)
{
    ...
    return VTCAM_MNG_DB_MAC(vTcamMngId);
}
...
)
{
    ...
    if (vTcamInfoPtr->tcamInfo.tcamSpaceUnmovable != CPSS_DXCH_VIRTUAL_TCAM_MOVABLE_E)
    {
...
)
{
    ...
    /* adjust oversize - priorityRulesOversize updated by resize functions only */
    /* write and delete functions can cause it being out of free space.         */
    if (vTcamInfoPtr->priorityRulesOversize >
        (vTcamInfoPtr->rulesAmount - vTcamInfoPtr->usedRulesAmount))
    {
        vTcamInfoPtr->priorityRulesOversize =
            (vTcamInfoPtr->rulesAmount - vTcamInfoPtr->usedRulesAmount);
    }

    /* add accumulated reminders of previous resize values          */
    /* recalculate downsize and reminder values due to  granularity */
    downSizeValue += vTcamInfoPtr->priorityRulesOversize;
    vTcamInfoPtr->priorityRulesOversize = (downSizeValue % rulesAmountGranularity);
    downSizeValue -= vTcamInfoPtr->priorityRulesOversize;
    if (downSizeValue == 0)
    {
        return GT_OK;
...
)
{
        ...
        CPSS_DXCH_VIRTUAL_TCAM_RULE_ADDITION_METHOD_PRIORITY_E)
    {
        ...
        if (vTcamInfoPtr->priorityRulesOversize >= resizeValue)
        {
            /* resize not needed - the reminders of previous allocations space (more than was required) */
            /* and reminders of previous freings space (less than was required) are enough              */
            vTcamInfoPtr->priorityRulesOversize -= resizeValue;
            return GT_OK;
        }
        else
        {
            /* Subtract from added space value the reminders of previous allocations space (more   */
            /* than was required) and reminders of previous freings space (less than was required) */
            resizeValue -= vTcamInfoPtr->priorityRulesOversize;
            vTcamInfoPtr->priorityRulesOversize = 0;
        }
    }
    ...
    */
    ...
    /* recalculate priorityRulesOversize after adding space to vTCAM */
    if (vTcamInfoPtr->tcamInfo.ruleAdditionMethod ==
        CPSS_DXCH_VIRTUAL_TCAM_RULE_ADDITION_METHOD_PRIORITY_E)
    {
        GT_U32 diff;

        if (actualResizeValue >= resizeValue)
        {
            /* added more than required - accumulate the reminder */
            diff = (actualResizeValue - resizeValue);
            vTcamInfoPtr->priorityRulesOversize += diff;
        }
        else
        {
            /* added less than required - recalculate the reminder  */
            diff = (resizeValue - actualResizeValue);
            if (vTcamInfoPtr->priorityRulesOversize >= diff)
            {
                vTcamInfoPtr->priorityRulesOversize -= diff;
            }
            else
    ...
    switch(actionTypePtr->ruleType)
    {
        case CPSS_DXCH_VIRTUAL_TCAM_RULE_TYPE_PCL_E:
            ...
            if(actionTypePtr->action.pcl.direction != CPSS_PCL_DIRECTION_INGRESS_E &&
               ...
               actionTypePtr->action.pcl.direction != CPSS_PCL_DIRECTION_MIDWAY_E)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
            }

            if(actionTypePtr->action.pcl.direction !=
               actionDataPtr->action.pcl.actionPtr->egressPolicy)
            {
            ...
            CPSS_DXCH_VIRTUAL_TCAM_RULE_ADDITION_METHOD_PRIORITY_E)
        {
            if(calledFromResize == GT_FALSE)
            {
                rc = priorityDbRemoveRuleId(
                    VTCAM_MNG_DB_MAC(vTcamMngId), vtcamInfoPtr, srcLogicalIndex);
                if(rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc,LOG_ERROR_NO_MSG);
                }
...
)
{
    ...
    GT_U32  logicalIndex;
    GT_U32  hwIndex;
    CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_ENT ruleSize = vtcamInfoPtr->tcamInfo.ruleSize;
...
)
{
       ...
       )
    {
        ...
        cpssOsMemSet(hwAction, 0, (CPSS_DXCH_PCL_ACTION_SIZE_IN_WORDS_CNS * 4));
        ...
        if (rc != GT_OK)
        {
            ...
            {
                ((CPSS_DXCH_VIRTUAL_TCAM_HA_DB_REPLAY_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayRuleDbTail))->next = replayRulePtr;
            }
...
/**
 * @internal  prvCpssDxChVirtualTcamAppAllocationBlocksMappingCheck
 * @endinternal
 * @brief  Check Mapping of TCAM blocks for Application allocated vTCAM
 *
 * @param[in] vTcamMngDBPtr              - pointer to Virtual TCAM Manager structure
 * @param[in] allocationSpaceArrSize     - amount of allocated space segmewnts
 * @param[in] allocationSpaceArr         - pointer to array of allocated space segmewnts
 * @param[in] clientGroup                - Client Group
 * @param[in] hitNumber                  - Hit Number
 * @param[in] isUnmappedBlockValid       - if block containing allocated segment yet unmapped
 *                                         to deside that mapping is valid
 * @retval GT_OK                         - on success
 * @retval GT_BAD_PARAM                  - on wrong parameters
 */
static GT_STATUS prvCpssDxChVirtualTcamAppAllocationBlocksMappingCheck
(
    IN   PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_STC       *vTcamMngDBPtr,
    IN   GT_U32                                   allocationSpaceArrSize,
    IN   CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC *allocationSpaceArr,
    IN   GT_U32                                   clientGroup,
    IN   GT_U32                                   hitNumber,
    IN   GT_BOOL                                  isUnmappedBlockValid
)
{
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_BLOCK_INFO_STC *blockInfArr;
    GT_U32    blocksInRow, blockWidth, blockMask, segmentMask;
    GT_U32    i, baseRow, rowsAmount, floorIndex, floorsAmount;
    GT_U32    blockInFloor, lookupId;
    GT_U32    lastFloorIndex;

    lookupId = PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_LOOKUP_ID_MAC(clientGroup, hitNumber);
    blockWidth = vTcamMngDBPtr->tcamSegCfg.lookupColumnAlignment;
    blockMask = (1 << blockWidth) - 1;
    blocksInRow = (vTcamMngDBPtr->tcamSegCfg.tcamColumnsAmount / blockWidth);

    for (i = 0; (i < allocationSpaceArrSize); i++)
    {
        baseRow      = allocationSpaceArr[i].startRowIndex;
        rowsAmount   = allocationSpaceArr[i].numOfRows;
        if (rowsAmount == 0) continue;
        segmentMask  = allocationSpaceArr[i].columnsBitmap;
        floorIndex   = baseRow / vTcamMngDBPtr->tcamSegCfg.lookupRowAlignment;
        lastFloorIndex = (baseRow + rowsAmount - 1) / vTcamMngDBPtr->tcamSegCfg.lookupRowAlignment;
        floorsAmount = lastFloorIndex - floorIndex + 1;
        for (/*no init*/; (floorsAmount > 0); floorsAmount --, floorIndex ++)
        {
            for (blockInFloor = 0; (blockInFloor < blocksInRow); blockInFloor++)
            {
                /* block not intersects the segment */
                if (((blockMask << (blockWidth * blockInFloor)) & segmentMask) == 0)
                    { continue; }
                blockInfArr  = &(vTcamMngDBPtr->tcamSegCfg.blockInfArr[
                    (floorIndex * blocksInRow) + blockInFloor]);
                /* already mapped to the needed lookup */
                if (blockInfArr->lookupId == lookupId){ continue; }
                /* free bock can be mapped to the needed lookup */
                if ((blockInfArr->lookupId == PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_TAB_LOOKUP_FREE_BLOCK_CNS)
                    && (isUnmappedBlockValid != GT_FALSE))
                    { continue; }
                /* another block mapping */
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
            }
        }
    }
...
/**
 * @internal  prvCpssDxChVirtualTcamAppAllocationBlocksMappingReadAndCheck
 * @endinternal
 * @brief  Read from HW and Check Mapping of TCAM blocks for Application allocated vTCAM
 *
 * @param[in] vTcamMngDBPtr              - pointer to Virtual TCAM Manager structure
 * @param[in] allocationSpaceArrSize     - amount of allocated space segmewnts
 * @param[in] allocationSpaceArr         - pointer to array of allocated space segmewnts
 ...
 * @retval GT_BAD_PARAM                  - on wrong parameters
 */
static GT_STATUS prvCpssDxChVirtualTcamAppAllocationBlocksMappingReadAndCheck
(
    IN   PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_STC       *vTcamMngDBPtr,
    IN   GT_U32                                   allocationSpaceArrSize,
    IN   CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC *allocationSpaceArr,
...
)
{
    ...
    for (i = 0; (i < allocationSpaceArrSize); i++)
    {
        ...
        segmentMask  = allocationSpaceArr[i].columnsBitmap;
        floorIndex   = baseRow / vTcamMngDBPtr->tcamSegCfg.lookupRowAlignment;
        floorsAmount = (rowsAmount / vTcamMngDBPtr->tcamSegCfg.lookupRowAlignment) + 1;
        for (/*no init*/; (floorsAmount > 0); floorsAmount --, floorIndex ++)
        {
            rc = cpssDxChTcamPortGroupIndexRangeHitNumAndGroupGet(
                devNum, vTcamMngDBPtr->portGroupBmpArr[devNum], floorIndex, floorInfoArr);
            ...
            for (blockInFloor = 0; (blockInFloor < blocksInRow); blockInFloor++)
            {
                ...
                if (((blockMask << (blockWidth * blockInFloor)) & segmentMask) == 0)
                    { continue; }
                if ((floorInfoArr[blockInFloor].group != clientGroup)
                    || (floorInfoArr[blockInFloor].hitNum != hitNumber))
                {
                    /* not matches mapping found in HW */
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
                }
            }
        }
    ...
    /* Mark checked blocks by found lookup */
    for (i = 0; (i < allocationSpaceArrSize); i++)
    {
        baseRow      = allocationSpaceArr[i].startRowIndex;
        rowsAmount   = allocationSpaceArr[i].numOfRows;
...
/**
 * @internal  prvCpssDxChVirtualTcamAppAllocationValidityCheck
 * @endinternal
 * @brief  Checks validity for Application allocated vTCAM
 *
 * @param[in] vTcamMngDBPtr              - pointer to Virtual TCAM Manager structure
 * @param[in] allocationSpaceArrSize     - amount of allocated space segmewnts
 * @param[in] allocationSpaceArr         - pointer to array of allocated space segmewnts
 ...
 * @retval GT_BAD_PARAM                  - on wrong parameters
 */
static GT_STATUS prvCpssDxChVirtualTcamAppAllocationValidityCheck
(
    IN   PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_STC       *vTcamMngDBPtr,
    IN   GT_U32                                   allocationSpaceArrSize,
    IN   CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC *allocationSpaceArr,
...
)
{
    ...
    GT_U32    segIdx;
    GT_U32    row;
    GT_U32    numOfRowsInTcam;
    GT_U32    tcamWidth;
    GT_U32    horzBlockWidth;
    GT_U32    columnMaxMask;
    GT_U32    lookupId;
    PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_SEG_COLUMNS_ROW_MAP segmentColumnsMap;
    CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC allocationSpace;

    numOfRowsInTcam = vTcamMngDBPtr->tcamSegCfg.tcamRowsAmount;
    tcamWidth       = vTcamMngDBPtr->tcamSegCfg.tcamColumnsAmount;
    horzBlockWidth  = vTcamMngDBPtr->tcamSegCfg.lookupColumnAlignment;
    columnMaxMask   = (1 << tcamWidth) - 1;
    lookupId        = PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_LOOKUP_ID_MAC(clientGroup, hitNumber);

    /* ascending order of base rows and checkin overlapping of segments rows */
    for (segIdx = 0; (segIdx < allocationSpaceArrSize); segIdx++)
    {
        row = 0;
        allocationSpace = allocationSpaceArr[segIdx];
        if (allocationSpace.startRowIndex < row)
        {
            /* not ascending order or overlapping */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
        row = allocationSpace.startRowIndex + allocationSpace.numOfRows;
        if (row > numOfRowsInTcam)
        {
            /* out of TCAM range */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
        if (allocationSpace.columnsBitmap & (~ columnMaxMask))
        {
            /* out of TCAM columns */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
        rc = prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsBitmapToSegColMap(
            tcamWidth, horzBlockWidth, allocationSpace.columnsBitmap,
            &segmentColumnsMap);
        if (rc != GT_OK)
        {
            /* out of supported column bitmaps */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
        /* search for free space */
        rc = prvCpssDxChVirtualTcamDbSegmentTableCheckThatMemoryIsFree(
            &(vTcamMngDBPtr->tcamSegCfg), lookupId,
            allocationSpace.startRowIndex, allocationSpace.numOfRows,
            segmentColumnsMap);
        if (rc != GT_OK)
        {
            /* not all the mewmory is free */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
    }
    return GT_OK;
...
)
{
    ...
    if (vTcamInfoPtr->tcamSpaceUnmovable != CPSS_DXCH_VIRTUAL_TCAM_MOVABLE_E)
    {
    ...
    /* don'save application allocation data */
    tcamCfgPtr->tcamInfo.allocationSpaceArr     = NULL;
    tcamCfgPtr->tcamInfo.allocationSpaceArrSize = 0;
    ...
    /* special treatment of unmovable vTCAM with application specified allocation */
    if (vTcamInfoPtr->tcamSpaceUnmovable == CPSS_DXCH_VIRTUAL_TCAM_UNMOVABLE_APP_ALLOCATED_E)
    {
        if ((vTcamInfoPtr->allocationSpaceArrSize == 0)
            || (vTcamInfoPtr->allocationSpaceArr == NULL))
        {
            prvCpssDxChVirtualTcamDbPriorityTableVTcamTreesDelete(
                vTcamMngDBPtr,  tcamCfgPtr);
            prvCpssDxChVirtualTcamDbRuleIdTableVTcamTreesDelete(
            ...
            cpssOsFree(tcamCfgPtr);
            vTcamMngDBPtr->vTcamCfgPtrArr[vTcamId] = NULL;
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "no allocationSpaceArr");
        }

        /* relevant only to devices having block mapping */
        if ((vTcamMngDBPtr->tcamSegCfg.lookupColumnAlignment != vTcamMngDBPtr->tcamSegCfg.tcamColumnsAmount)
            || (vTcamMngDBPtr->tcamSegCfg.lookupRowAlignment != vTcamMngDBPtr->tcamSegCfg.tcamRowsAmount))
        {
            if (vTcamMngDBPtr->haSupported
                ...
                (managerHwWriteBlock != GT_FALSE)))
            {
                /* HA Replay recovery state */
                rc = prvCpssDxChVirtualTcamAppAllocationBlocksMappingReadAndCheck(
                    vTcamMngDBPtr,
                    vTcamInfoPtr->allocationSpaceArrSize, vTcamInfoPtr->allocationSpaceArr,
                    vTcamInfoPtr->clientGroup, vTcamInfoPtr->hitNumber);
            }
            else
            {
                /* regular recovery state */
                /* check TCAM block mapping - at regular stage blocks can be free */
                rc = prvCpssDxChVirtualTcamAppAllocationBlocksMappingCheck(
                    vTcamMngDBPtr,
                    vTcamInfoPtr->allocationSpaceArrSize, vTcamInfoPtr->allocationSpaceArr,
                    vTcamInfoPtr->clientGroup, vTcamInfoPtr->hitNumber,
                    GT_TRUE/*isUnmappedBlockValid*/);
            }
            if (rc != GT_OK)
            {
                prvCpssDxChVirtualTcamDbPriorityTableVTcamTreesDelete(
                ...
                cpssOsFree(tcamCfgPtr);
                vTcamMngDBPtr->vTcamCfgPtrArr[vTcamId] = NULL;
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "not matches TCAM blocks Mapping");
            }

            rc = prvCpssDxChVirtualTcamAppAllocationValidityCheck(
                vTcamMngDBPtr,
                vTcamInfoPtr->allocationSpaceArrSize, vTcamInfoPtr->allocationSpaceArr,
                vTcamInfoPtr->clientGroup, vTcamInfoPtr->hitNumber);
            if (rc != GT_OK)
        ...
        /* VTCAM Segments DB allocated both on regular state and on HA Replay state */
        rc = prvCpssDxChVirtualTcamDbSegmentTableMinRuleColumnsSpaceToSegTree(
            vTcamId, tcamCfgPtr,
            vTcamInfoPtr->allocationSpaceArrSize, vTcamInfoPtr->allocationSpaceArr);
        if (rc != GT_OK)
        {
            ...
            cpssOsFree(tcamCfgPtr);
            vTcamMngDBPtr->vTcamCfgPtrArr[vTcamId] = NULL;
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "Application space allocation failed");
        }
        ...
        prvCpssDxChVirtualTcamDbSegmentTableLogToPhyIndexTabGenerate(
                tcamCfgPtr->tcamSegCfgPtr,
                tcamCfgPtr->segmentsTree,
                tcamCfgPtr->rulePhysicalIndexArr);

        bmpSize = (((tcamCfgPtr->rulesAmount + 31) / 32) * sizeof(GT_U32));
        ...
        managerHwWriteBlock == GT_TRUE))
    {
        rc = prvCpssAvlTreeCreate(
                ...
                &vTcamMngDBPtr->vTcamCfgPtrArr[vTcamId]->segmentsTree);
        if (rc != GT_OK)
        {
...
)
{
    ...
    for (vTcamMngId = 0; (vTcamMngId < CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS); vTcamMngId++)
    {
        while ((vTcamMng = VTCAM_MNG_DB_MAC(vTcamMngId)) != NULL)
        {
            ...
            if (rc1 == GT_NO_MORE)
            {
                ...
                if (rc1 == GT_OK)
                {
                    ...
                    if ((vTcamMng = VTCAM_MNG_DB_MAC(vTcamMngId)) != NULL)
                    {
                        cpssOsFree(vTcamMng);
                        VTCAM_MNG_DB_PTR_MAC(vTcamMngId) = NULL;
                    }
...
)
{
    ...
    return
        ((vTcamMngDBPtr->vTcamCfgPtrArr[vTcamId]->tcamInfo.tcamSpaceUnmovable
          == CPSS_DXCH_VIRTUAL_TCAM_MOVABLE_E) ? GT_FALSE : GT_TRUE);
}
...
)
{
    ...
    while (seeked != GT_FALSE)
    {
        ...
        {
            /* accumulated horizontal segments merging */
...
)
{
    ...
    *gtNokRulesListPtr = (CPSS_DXCH_VIRTUAL_TCAM_HA_DB_REPLAY_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayRuleDb);
    *unmappedRulesListPtr = (CPSS_DXCH_VIRTUAL_TCAM_HA_UNMAPPED_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayUnmappedRuleDb);
...
)
{
    ...
    replayRulePtr         = (CPSS_DXCH_VIRTUAL_TCAM_HA_DB_REPLAY_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayRuleDb);
    replayUnmappedRulePtr = (CPSS_DXCH_VIRTUAL_TCAM_HA_UNMAPPED_RULE_ENTRY_STC*)VTCAM_GLOVAR(replayUnmappedRuleDb);
...
/**
 * @internal prvCpssDxChVirtualTcamCallbacksBind function
 * @endinternal
 *
 ...
 *
 */
GT_VOID * prvCpssDxChVirtualTcamCallbacksBind
(
    GT_VOID
)
{
    return (GT_VOID *)compareFunctionDb;
}
