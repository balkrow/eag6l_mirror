...
/* bookmark to state that we need last index in the Trunk/L2 ECMP table to hold the NULL port */
...
#define NUMBITS_L2_ECMP_START_INDEX_MAC(devNum) \
    (PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : PRV_CPSS_SIP_5_20_CHECK_MAC(devNum) ? 14 : 13)

#define NUMBITS_TARGET_EPORT_PHY_PORT_MAC(devNum)   \
    (PRV_CPSS_SIP_7_CHECK_MAC(devNum)    ? 15 : PRV_CPSS_SIP_6_CHECK_MAC(devNum)    ? 13 : \
        PRV_CPSS_SIP_5_20_CHECK_MAC(devNum) ? 14 : 13)

/* sip7 : check that the index not over it's boundaries */
#define SIP7_TRUNK_ECMP_MEMBER_INDEX_CHECK_MAC(_devNum,_index)    \
    if((_index) >= PRV_CPSS_DXCH_PP_MAC(_devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp)   \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "trunk ECMP index[%d] >= max [%d]",          \
            _index,PRV_CPSS_DXCH_PP_MAC(_devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp)
...
#define HASH_MASK_CRC_INDEX_CHECK_MAC(_devNum,_index)   \
    if((_index) >=                                      \
       PRV_CPSS_ACCESS_TABLE_INFO_PTR(_devNum)[CPSS_DXCH_LION_TABLE_TRUNK_HASH_MASK_CRC_E].maxNumOfEntries)\
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG)
...
)
{
    ...
    {
        devTrunkInfoPtr->numOfDesignatedTrunkEntriesHw = 128;
    }
    else
    ...
    */
    if (PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        ...
        /* SIP_6 devices support 64 physical ports mode */
        devTrunkInfoPtr->validityMask.port   = MAX(0x3F , (PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(devNum)-1));
    }
    else
    {
    ...
    {   /* native mode or soho emulation mode */
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            /* sip7 : the Trunk ECMP is not part of the L2 ECMP that is in sip5 */
    ...
    else
    {
        ...
        */
        ...
        devTrunkInfoPtr->maxNumMembersInDevice = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ?
            PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp :
    ...
    if (PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        ...
        {
            /*sip7 : last index in TRUNK ECMP table */
            PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.l2EcmpIndex_NULL_port =
                 PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp - 1;
        }
...
)
{
    ...
    if(PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->tempSortingArr == NULL &&
       enable == GT_TRUE)
    {
        PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->tempSortingArr =
            cpssOsMalloc(sizeof(PRV_CPSS_TRUNK_DB_SORT_INFO_STC)*(PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->maxNumMembersInDevice));
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            /* set also the dedicated 'per source port' table ! */
            rc = prvCpssDxChWriteTableEntryField(devNum,
                     CPSS_DXCH_SIP7_TABLE_HA_SRC_PHYSICAL_PORT_2_E,
                     portNum, /*global port*/
                     PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                     SIP7_HA_SRC_PHYSICAL_PORT_2_TABLE_FIELDS_TRUNK_ID_E, /* field name */
                     PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
                     value);
            if(rc != GT_OK)
            {
...
/**
* @internal prvCpssDxChTrunkL2EcmpPortSet function
* @endinternal
*
* @brief   Function Relevant mode : All modes
*         Set entry in Trunk/L2 ECMP table with the valid port.
*
...
* @param[in] devNum                - device number
* @param[in] trunkId               - Trunk ID
* @param[in] index                 - Index of the member in a trunk
* @param[in] trunkMember           - (pointer to) Trunk Member Entry
* @param[in] configLtt             - GT_TRUE - Configure LTT table
*                                    GT_FALSE- Do not configure LTT table
*
*
* @retval GT_OK                    - on success
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChTrunkL2EcmpPortSet
(
    IN  GT_U8                                devNum,
    IN  GT_TRUNK_ID                          trunkId,
    IN  GT_U32                               index,
    IN  CPSS_TRUNK_MEMBER_STC                *trunkMemberPtr,
    IN  GT_BOOL                              configLtt
)
{
    ...
    CPSS_DXCH_TABLE_ENT hwTable =
        ...
        CPSS_DXCH_SIP7_TABLE_EQ_TRUNK_ECMP_E :
        CPSS_DXCH_SIP5_TABLE_EQ_L2_ECMP_E;

    trunkEntryPtr = &PRV_CPSS_DEV_TRUNK_INFO_MAC(devNum)->trunksArray[trunkId];
    data          = trunkMemberPtr->hwDevice | (trunkMemberPtr->port << 10);

    if(index >= trunkEntryPtr->flexInfo.myTrunkMaxNumOfMembers)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    rc =  prvCpssDxChWriteTableEntry(devNum,
        hwTable,
        trunkEntryPtr->flexInfo.myTrunkStartIndexInMembersTable + index,
        &data);
    if(rc != GT_OK)
    {
        return rc;
    ...
    if(configLtt)
    {
        /* update the LTT only after the members already set in 'Trunk/L2 ECMP'*/

        cpssOsMemSet(&ecmpLttInfo, 0, sizeof(ecmpLttInfo));
        ecmpLttInfo.ecmpStartIndex = trunkEntryPtr->flexInfo.myTrunkStartIndexInMembersTable;
        ecmpLttInfo.ecmpNumOfPaths = trunkEntryPtr->flexInfo.myTrunkMaxNumOfMembers;
...
)
{
    ...
    if(devTrunkInfoPtr->shadowValid == GT_FALSE)
    {
        ...
        */
        ...
        offset = U32_GET_FIELD_MAC(dataArr[0],2,numBits_L2_ECMP_Start_Index);
    }
    else /* we use DB and we need to get info from it for our calculations */
    {
        if(devTrunkInfoPtr->maxNumMembersInTrunk_isPerTrunk == GT_FALSE)
        {
            ...
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                ...
                offset = (trunkId-1) * devTrunkInfoPtr->maxNumMembersInTrunk;
            }
            else
            {
    ...
    for (ii = 0;ii < maxStretch; ii ++)
    {
       ...
       /* check phyPort and hwDev.
          allow up to maximum value 1023 (10 bits) for physical port number */
        if((membersArray[memberIndex].port      >= BIT_10) ||
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        if(0 == U32_GET_FIELD_MAC(dataArr[0],0,1))/*<ECMP Enable>*/
        {
            *numMembersPtr = 0;
            return GT_OK;
        }
        ...
        numBits_L2_ECMP_Start_Index = NUMBITS_L2_ECMP_START_INDEX_MAC(devNum);
        numBits_target_eport_phy_port = NUMBITS_TARGET_EPORT_PHY_PORT_MAC(devNum);

        *numMembersPtr = 1 + (U32_GET_FIELD_MAC(dataArr[0],2+numBits_L2_ECMP_Start_Index,12));
        offset = U32_GET_FIELD_MAC(dataArr[0],2,numBits_L2_ECMP_Start_Index);
...
*                                            in AAS TRUNK_DESIGNATED_TABLE (APPLICABLE RANGES: 1..16)
...
*                                           startBit + numOfBits) must not exceed value 12.
*
* @retval GT_OK                    - on success.
...
{
    GT_U32    regAddr;         /* register address                         */
    GT_U32    value,hw_numOfBits;
    GT_U32    startBitOffset;
    GT_U32    lastBitOffset;
    GT_U32    numBitsToCheck;/* num of bits supported by the client */
    ...
    if((startBit + numOfBits) > 32) /* no client can exceed 32 bits */
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
    ...
    switch(hashClient)
    {
        case CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_TRUNK_DESIGNATED_TABLE_E:
            numBitsToCheck = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : 12;
            break;
        ...
        case CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E:
            /* in sip7 the egress pipe get full 32 bits of ingress pipe , and not only 12 bits
               so limit with 32 bits already done at start of function */
            if((!PRV_CPSS_SIP_7_CHECK_MAC(devNum)) &&
               ((startBit + numOfBits) > 12))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
                    "((startBit[%d] + numOfBits[%d]) = [%d] must not exceed 12 (that is the max bits that the egress pipe may get out of the 32 bits of hash in ingress pipe)",
                    startBit, numOfBits, (startBit  + numOfBits));
            }
            GT_ATTR_FALLTHROUGH;
    ...
    if(numOfBits > numBitsToCheck)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
            "numOfBits [%d] must not exceed [%d]", startBit,numBitsToCheck);
    }
    ...
    switch(hashClient)
    {
        ...
        case CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_TRUNK_DESIGNATED_TABLE_E:
            if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                /* actual config in the EFT unit , as the EQ unit send full 32 bits to egress pipe */
                regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EGF_eft.egrFilterConfigs.globalHashSelection;
            }
            else
            {
                /* actual config in the EQ unit */
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
       hashClient == CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_TRUNK_DESIGNATED_TABLE_E)
    {
        /*  NOTE : next logic remove the need to call to
            sip5HashNumRightShiftBitsSet() , as the 'LFSR' here is <Designated Trunk Num Hash Bits>
            that is in the same register as the 'start' and 'last' bits */

        /* set also the <Designated Trunk Num Hash Bits>*/
        value |= numOfBits << hw_numOfBits;
        hw_numOfBits += 5;

        /* set also the designated_trunk_block_size */
        if(numOfBits < 7)
        {
            /* the '+1' comes from the fact that the HW designed to use the value 'as is'
               and not adding the '+1' , so we add it here */
            value |= (1 + BIT_MASK_0_31_MAC(numOfBits)) << hw_numOfBits;
        }
        else
        {
            value |= 0x7F << hw_numOfBits;
        }
        hw_numOfBits += 7;
    }

    return prvCpssHwPpSetRegField(devNum, regAddr ,startBitOffset ,hw_numOfBits ,value);
}
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* need to give proper init for the EFT unit for accessing the designated table */
        rc = cpssDxChTrunkHashNumBitsSet(devNum,CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_TRUNK_DESIGNATED_TABLE_E,startBit,numOfBits);
        if(rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal internal_cpssDxChTrunkMemberDisabledAdd function
* @endinternal
*
* @brief   Function Relevant mode : High Level mode
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTrunkMemberDisabledAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN CPSS_TRUNK_MEMBER_STC    *memberPtr
...
{
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    return prvCpssGenericTrunkMemberDisabledAdd(devNum,trunkId,memberPtr);
}

/**
* @internal cpssDxChTrunkMemberDisabledAdd function
* @endinternal
*
* @brief   Function Relevant mode : High Level mode
*         This function add member as disabled to the trunk in the device.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTrunkMemberDisabledAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN CPSS_TRUNK_MEMBER_STC    *memberPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkMemberDisabledAdd);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRUNK_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, memberPtr));

    rc = internal_cpssDxChTrunkMemberDisabledAdd(devNum, trunkId, memberPtr);
...
/**
* @internal sip5TrunkEcmpReservedNullPortSet function
* @endinternal
*
* @brief   Function Relevant mode : All modes
*         Set entry in Trunk/L2 ECMP table with the 'NULL port' for the use of all
*         'empty trunks'.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS sip5TrunkEcmpReservedNullPortSet
(
    IN  GT_U8                                devNum,
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        */
        ...
        L2_ECMP_NULL_PORT_BOOKMARK
        /* we need extra index to be untouched (last index) to hold the NULL port */
        rc = sip5TrunkEcmpReservedNullPortSet(devNum,
...
)
{
    ...
    if(PRV_CPSS_NON_SHARED_FAMILY_INFO(devFamily) == NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, LOG_ERROR_NO_MSG);
    }

    bindFuncPtr = &(PRV_CPSS_NON_SHARED_FAMILY_INFO(devFamily)->trunkInfo.boundFunc);
    ...
    bindFuncPtr->ecmpTableSetFunc            = prvCpssDxChTrunkL2EcmpPortSet;
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        if(PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.l2EcmpIndex_NULL_port)
        {
            L2_ECMP_NULL_PORT_BOOKMARK
            /* we need extra index to be untouched (last index) to hold the NULL port */
            rc = sip5TrunkEcmpReservedNullPortSet(devNum,hwDevNum);
            if(rc != GT_OK)
            {
                return rc;
            }
...
/**
* @internal addL4Hash function
* @endinternal
*
* @brief Add L4 hash value to existing simple hash mode calculation.
* @endinternal
*/
static GT_VOID addL4Hash
(
    IN CPSS_DXCH_PCL_PACKET_TYPE_ENT                packetType,
    IN CPSS_DXCH_TRUNK_L4_LBH_MODE_ENT              l4HashMode,
    IN CPSS_DXCH_TRUNK_LBH_INPUT_DATA_STC           *hashInputDataPtr,
    IN GT_U32                                       l4SrcPortMask,
    IN GT_U32                                       l4DstPortMask,
    INOUT GT_U32                                    *hashValPtr
)
{
    GT_U32 hashVal;         /* hash trunk value */
    GT_U32 hSrcTcpPort;     /* hash part of source TCP/UDP port */
    GT_U32 hDstTcpPort;     /* hash part of destination TCP/UDP port */

    /* addL4Hash */
    if (l4HashMode == CPSS_DXCH_TRUNK_L4_LBH_DISABLED_E)
    {
        return;
    }
        ...
        packetType == CPSS_DXCH_PCL_PACKET_TYPE_IPV6_TCP_E)
    {
        ...
        /* Bytes are in 'network order' - byte0 holds bits 15:8, byte1 - bits 7:0 */
        hSrcTcpPort = hashInputDataPtr->l4SrcPortArray[1] & l4SrcPortMask;
        hDstTcpPort = hashInputDataPtr->l4DstPortArray[1] & l4DstPortMask;
        hashVal = hashVal ^ hSrcTcpPort ^ hDstTcpPort;

        if (l4HashMode == CPSS_DXCH_TRUNK_L4_LBH_LONG_E)
        {
            /* L4LongTrunk LBH */
            hSrcTcpPort = hashInputDataPtr->l4SrcPortArray[0] & l4SrcPortMask;
            hDstTcpPort = hashInputDataPtr->l4DstPortArray[0] & l4DstPortMask;
            hashVal = hashVal ^ hSrcTcpPort ^ hDstTcpPort;
        }
...
/**
* @internal addMACHash function
* @endinternal
*
* @brief Add MAC hash value to existing simple hash mode calculation.
* @endinternal
*/
static GT_VOID addMACHash
(
    IN GT_BOOL                                      addMacHash,
    IN GT_U32                                       hDstMac,
    IN GT_U32                                       hSrcMac,
    INOUT GT_U32                                    *hashValPtr
)
{
    ...
    if (addMacHash)
    {
        hashVal = *hashValPtr;
        /* Add MAC Hash */
        *hashValPtr = hashVal ^ hSrcMac ^ hDstMac;
    }
...
/**
* @internal simpleTrunkHashIndexCalculate function
* @endinternal
*
* @brief  Index of load-balancing group member calculation for given hash client based on a simple hash mode.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
...
* @param[in] packetType             - packet type
* @param[in] hashInputDataPtr       - (pointer to) hash input data
* @param[in] hashEntityType         - client that uses HASH
* @param[in] numberOfMembers        - number of members in load-balancing group
* @param[in] hashMode               - LBH hash mode
* @param[out] hashIndexPtr          - (pointer to) calculated hash index of load-balancing group member
*
* @retval GT_OK                     - on success
* @retval GT_FAIL                   - on error
* @retval GT_HW_ERROR               - on hardware error
...
*
*/
static GT_STATUS simpleTrunkHashIndexCalculate
(
    IN GT_U8                                        devNum,
    IN CPSS_DXCH_PCL_PACKET_TYPE_ENT                packetType,
    IN CPSS_DXCH_TRUNK_LBH_INPUT_DATA_STC           *hashInputDataPtr,
...
)
{
    ...
    GT_U8* hSipPtr;         /* pointer to source IP address             */
    GT_U8* hDipPtr;         /* pointer to destination IP address        */
    ...
    CPSS_DXCH_TRUNK_L4_LBH_MODE_ENT     l4HashMode;   /* L4 Hash mode   */
    CPSS_DXCH_TRUNK_IPV6_HASH_MODE_ENT  ipv6HashMode; /* IPV6 Hash mode */
    GT_BOOL addIpHash;      /* add IP Hash                              */
    GT_BOOL addMplsHash;    /* add MPLS Hash                            */
    GT_BOOL addMacHash;     /* add MAC Hash                             */
    GT_U8  ipv6SipMask;     /* ipv6 Sip Mask                            */
    GT_U8  ipv6DipMask;     /* ipv6 Dip Mask                            */
    ...
    GT_U8  macSaMask;       /* MAC SA Mask                              */
    GT_U8  macDaMask;       /* MAC DA Mask                              */
    GT_U8  ipv6FlowMask;    /* ipv6 Flow Mask                           */
    GT_U8  ipv4SipMask;     /* ipv4 Sip Mask                            */
    GT_U8  ipv4DipMask;     /* ipv4 Dip Mask                            */
    GT_U8  mplsLab0Mask;    /* MPLS Label0 Mask                         */
    GT_U8  mplsLab1Mask;    /* MPLS Label1 Mask                         */
    GT_U8  mplsLab2Mask;    /* MPLS Label2 Mask                         */
    GT_U8  l4SrcPortMask;   /* L4 Source port Mask                      */
    GT_U8  l4DstPortMask;   /* L4 Destination port Mask                 */
    GT_U32 startBit;        /* start hash bit                           */
    GT_U32 numOfBits;       /* number of bits in hash                   */
    GT_U32 hashBitsValue;   /* hash bits value                          */
    ...
    rc = prvCpssDxChTrunkHashBitsSelectionGet(devNum, hashEntityType, &startBit, &numOfBits);
    if (rc != GT_OK)
    {
        return rc;
    }

    if (hashMode == CPSS_DXCH_TRUNK_LBH_PACKETS_INFO_E)
    {
        ...
        rc = cpssDxChTrunkHashIpModeGet(devNum, &addIpHash);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMplsModeEnableGet(devNum, &addMplsHash);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc= cpssDxChTrunkHashIpAddMacModeGet(devNum, &addMacHash);
        if (rc != GT_OK)
        {
            return rc;
        }

        rc = cpssDxChTrunkHashIpShiftGet(devNum, CPSS_IP_PROTOCOL_IPV4_E, GT_FALSE, &ipv4DipShift);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashIpShiftGet(devNum, CPSS_IP_PROTOCOL_IPV4_E, GT_TRUE, &ipv4SipShift);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashIpShiftGet(devNum, CPSS_IP_PROTOCOL_IPV6_E, GT_FALSE, &ipv6DipShift);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashIpShiftGet(devNum, CPSS_IP_PROTOCOL_IPV6_E, GT_TRUE, &ipv6SipShift);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_MAC_SA_E, &macSaMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_MAC_DA_E, &macDaMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_IPV4_SIP_E, &ipv4SipMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_IPV4_DIP_E, &ipv4DipMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_IPV6_FLOW_E, &ipv6FlowMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_MPLS_LABEL0_E, &mplsLab0Mask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_MPLS_LABEL1_E, &mplsLab1Mask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_MPLS_LABEL2_E, &mplsLab2Mask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_L4_SRC_PORT_E, &l4SrcPortMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_L4_DST_PORT_E, &l4DstPortMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_IPV6_SIP_E, &ipv6SipMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        rc = cpssDxChTrunkHashMaskGet(devNum, CPSS_DXCH_TRUNK_LBH_MASK_IPV6_DIP_E, &ipv6DipMask);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        hashVal = 0;
        hDstMac = hashInputDataPtr->macDaArray[5] & macDaMask;
        hSrcMac = hashInputDataPtr->macSaArray[5] & macSaMask;

        /* Calculate hash for given packet type */
        switch(packetType)
        {
            ...
            case CPSS_DXCH_PCL_PACKET_TYPE_IPV4_OTHER_E:
                if (addIpHash)
                {
                    hSipPtr = &hashInputDataPtr->ipSipArray[12];
                    hDipPtr = &hashInputDataPtr->ipDipArray[12];

                    /* SIP[5:0] ^ SIP[21:16] ^ DIP[5:0] ^ DIP[21:16] */
                    hashVal = (hSipPtr[IPV4_SHIFT_INDEX_MAC(3,ipv4SipShift)] & ipv4SipMask) ^
                              (hSipPtr[IPV4_SHIFT_INDEX_MAC(1,ipv4SipShift)] & ipv4SipMask) ^
                              (hDipPtr[IPV4_SHIFT_INDEX_MAC(3,ipv4DipShift)] & ipv4DipMask) ^
                              (hDipPtr[IPV4_SHIFT_INDEX_MAC(1,ipv4DipShift)] & ipv4DipMask);

                    /* addL4Hash */
                    addL4Hash(packetType, l4HashMode, hashInputDataPtr,
                              l4SrcPortMask, l4DstPortMask, &hashVal);

                    /* AddMACHash */
                    addMACHash(addMacHash, hDstMac, hSrcMac, &hashVal);
                }
                else
                {
                    /* AddMACHash */
                    addMACHash(GT_TRUE, hDstMac, hSrcMac, &hashVal);
                }
                break;
            case CPSS_DXCH_PCL_PACKET_TYPE_MPLS_E:
                ...
                {
                    /* MPLS hash function */
                    hMplsLabel0 = hashInputDataPtr->mplsLabel0Array[2] & mplsLab0Mask;
                    hMplsLabel1 = hashInputDataPtr->mplsLabel1Array[2] & mplsLab1Mask;
                    hMplsLabel2 = hashInputDataPtr->mplsLabel2Array[2] & mplsLab2Mask;
            ...
            case CPSS_DXCH_PCL_PACKET_TYPE_IPV6_UDP_E:
                if (addIpHash)
                {
                    hSipPtr = hashInputDataPtr->ipSipArray;
                    hDipPtr = hashInputDataPtr->ipDipArray;

                    switch (ipv6HashMode)
                    {
                        case CPSS_DXCH_TRUNK_IPV6_HASH_LSB_SIP_DIP_FLOW_E:
                            /* SIP[5:0] ^ SIP[21:16] ^ DIP[5:0] ^ DIP[21:16] ^ flow[5:0] */
                            hFlowLabel = hashInputDataPtr->ipv6FlowArray[2] & ipv6FlowMask;
                            hashVal = (hSipPtr[IPV6_SHIFT_INDEX_MAC(15,ipv6SipShift)] & ipv6SipMask) ^
                                      (hSipPtr[IPV6_SHIFT_INDEX_MAC(13,ipv6SipShift)] & ipv6SipMask) ^
                                      (hDipPtr[IPV6_SHIFT_INDEX_MAC(15,ipv6DipShift)] & ipv6DipMask) ^
                                      (hDipPtr[IPV6_SHIFT_INDEX_MAC(13,ipv6DipShift)] & ipv6DipMask) ^
                                      hFlowLabel;
                            break;
                        case CPSS_DXCH_TRUNK_IPV6_HASH_MSB_SIP_DIP_FLOW_E:
                            /* SIP[69:64] ^ SIP[125:120] ^ DIP[69:64] ^ DIP[125:120 ^ flow[13:8] */
                            hFlowLabel = hashInputDataPtr->ipv6FlowArray[1] & ipv6FlowMask;
                            hashVal = (hSipPtr[IPV6_SHIFT_INDEX_MAC(7,ipv6SipShift)] & ipv6SipMask) ^
                                      (hSipPtr[IPV6_SHIFT_INDEX_MAC(0,ipv6SipShift)] & ipv6SipMask) ^
                                      (hDipPtr[IPV6_SHIFT_INDEX_MAC(7,ipv6DipShift)] & ipv6DipMask) ^
                                      (hDipPtr[IPV6_SHIFT_INDEX_MAC(0,ipv6DipShift)] & ipv6DipMask) ^
                                      hFlowLabel;
                            break;
                        case CPSS_DXCH_TRUNK_IPV6_HASH_MSB_LSB_SIP_DIP_FLOW_E:
                            /* SIP[5:0] ^ SIP[21:16] ^ DIP[5:0] ^ DIP[21:16] ^ flow[5:0] */
                        ...
                        case CPSS_DXCH_TRUNK_IPV6_HASH_LSB_SIP_DIP_E:
                            ...
                            break;
                        default:
                            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "IPv6 Hash Mode[%d]", ipv6HashMode);
                    }
                    /* addL4Hash */
                    addL4Hash(packetType, l4HashMode, hashInputDataPtr,
                              l4SrcPortMask, l4DstPortMask, &hashVal);
    ...
    {
        /* CPSS_DXCH_TRUNK_LBH_INGRESS_PORT_E */
        hashVal = hashInputDataPtr->localSrcPortArray[0] & 0x3f;
    }

    hashBitsValue =  U32_GET_FIELD_MAC(hashVal, startBit, numOfBits);
    ...
    switch(hashEntityType)
    {
        ...
        case CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_L3_ECMP_E:
            /* now calculate hash index of load-balancing group member*/
            *hashIndexPtr = (numberOfMembers * hashBitsValue) >> numOfBits;
            *hashIndexPtr = *hashIndexPtr & 0xfff;
            break;
        default:
            *hashIndexPtr = hashBitsValue;
    }
...
)
{
    ...
    rc = cpssDxChTrunkHashGlobalModeGet(devNum, &hashMode);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* CRC Hash based on the packet's data */
    ...
    {
        rc = simpleTrunkHashIndexCalculate(devNum,
                                           packetType,
                                           hashInputDataPtr,
                                           hashEntityType,
                                           numberOfMembers,
...
)
{
    ...
    switch(fieldType)
    {
        ...
        case CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IPV6_ADDR_24MSB_BY_24UDBS_E:
            ...
            break;
        case CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IP_DIP_SIP_BY_7MPLS_LABELS_E:
...
*         the 'start index' in the 'Trunk/L2 ECMP' table , and the number of consecutive
*         entries that will be reserved for this trunk(in 'Trunk/L2 ECMP').
*         NOTE:
*         1. See more details about this mode in description of 'CPSS_DXCH_TRUNK_MEMBERS_MODE_FLEX_E'
*         2. This API is NOT intend to 'move' trunks from one range in the 'Trunk/L2 ECMP'
...
* @param[in] devNum                   - the device number .
* @param[in] trunkId                  - the trunk id.
* @param[in] l2EcmpStartIndex         - The 'start index' in the 'Trunk/L2 ECMP' table.
*                                      (APPLICABLE RANGES: 0..('Trunk/L2 ECMP' table size - 1))
* @param[in] maxNumOfMembers          - 1. The number of consecutive entries that will be reserved
*                                      for this trunk(in 'Trunk/L2 ECMP')
...
)
{
    ...
    if(trunkEntryPtr->flexInfo.myTrunkMaxNumOfMembers)
    {
        ...
        if(trunkEntryPtr->flexInfo.myTrunkMaxNumOfMembers > maxNumOfMembers)
        {
            ...
            if(dataArr[0] & 1)  /*<ECMP Enable>*/
            {
                /*<Number of L2 ECMP Paths>*/
                numL2EcmpPaths = dataArr[0] >> (2+numBits_L2_ECMP_Start_Index);
    ...
    if(relativeStartMembersIndex >= devTrunkInfoPtr->maxNumMembersInDevice)
    {
        /* protect access violation to : devTrunkInfoPtr->allMembersArray[] */
        /* and devTrunkInfoPtr->allMembersIsLocalArray[] */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
            "relativeStartMembersIndex [%d] must not be less than maxNumMembersInDevice [%d] in trunkId[%d]",
            relativeStartMembersIndex,
            devTrunkInfoPtr->maxNumMembersInDevice,
            trunkId);
    }
...
)
{
    ...
    GT_U32  trunkIdIndex;
    GT_U32  ecmpNumMembers;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E);

    ecmpNumMembers = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ?
            PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp :
            PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.l2EcmpNumMembers;
    ...
    if(trunkId == TRUNK_ID_ALL_CNS)
    {
        ...
        */
        ...
        {
            /* sip7 : the Trunk ECMP is not part of the L2 ECMP that is in sip5 */
            /* use the table from it's base */
            ecmpNumMembers = maxNumOfMembers * devTrunkInfoPtr->numberOfTrunks;
            L2_ECMP_NULL_PORT_BOOKMARK
            /* we use last index to hold the NULL port */
            ecmpNumMembers ++;
        }
        ...
        l2EcmpStartIndex = ecmpNumMembers;
    ...
    else
    {
        ...
        if(l2EcmpStartIndex > ecmpNumMembers ||
            maxNumOfMembers > ecmpNumMembers)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
...
* @endinternal
*
* @brief   Get per trunk the 'start index' in the 'Trunk/L2 ECMP' table , and the number of consecutive
*         entries reserved for this trunk(in 'Trunk/L2 ECMP').
*         function uses the DB (no HW operations)
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
...
* @param[in] trunkId                  - the trunk id.
*
* @param[out] l2EcmpStartIndexPtr      - (pointer to) The 'start index' in the 'Trunk/L2 ECMP' table.
...
)
{
    ...
    /* sip7 : the Trunk ECMP is not part of the L2 ECMP that is in sip5 */
    maxMembers = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ?
        PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp :
        PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.l2EcmpNumMembers;
    ...
    dataArr[0] = BOOL2BIT_MAC(ecmpLttInfoPtr->ecmpEnable) << 0 |
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        U32_SET_FIELD_IN_ENTRY_MAC(dataArr,30,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
    else
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        U32_SET_FIELD_MAC(dataArr[0],28,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
...
)
{
    ...
    ecmpLttInfoPtr->ecmpEnable = BIT2BOOL_MAC(U32_GET_FIELD_MAC(dataArr[0],0,1));
    ecmpLttInfoPtr->ecmpRandomPathEnable = BIT2BOOL_MAC(U32_GET_FIELD_MAC(dataArr[0],1,1)); /*<Random ECMP Path Enable>*/
    ecmpLttInfoPtr->ecmpNumOfPaths = 1 + U32_GET_FIELD_MAC(dataArr[0],(2+numBits_L2_ECMP_Start_Index), 12);
    ecmpLttInfoPtr->ecmpStartIndex = U32_GET_FIELD_MAC(dataArr[0],2,numBits_L2_ECMP_Start_Index);

    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        U32_GET_FIELD_IN_ENTRY_MAC(dataArr,30,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
    else
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ecmpLttInfoPtr->hashBitSelectionProfile = U32_GET_FIELD_MAC(dataArr[0],28,4);
    }
...
/**
* @internal internal_cpssDxChTrunkEcmpTableSet function
* @endinternal
*
* @brief   Function Relevant mode : Low level mode
*          Set Trunk ECMP entry info .
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] index                 - the Trunk ECMP entry index
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*
* @note Index according to index from Trunk ECMP LTT
*
*/
static GT_STATUS internal_cpssDxChTrunkEcmpTableSet
(
    IN GT_U8                                devNum,
    IN GT_U32                               index,
    IN CPSS_TRUNK_MEMBER_STC               *memberPtr
)
{
    ...
    /*sip4*/ CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E |
    /*sip5*/ CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E |
    /*sip6*/ CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    CPSS_NULL_PTR_CHECK_MAC(memberPtr);

    /* check that index not exceed the table */
    SIP7_TRUNK_ECMP_MEMBER_INDEX_CHECK_MAC(devNum,index);

    if(PRV_CPSS_HW_DEV_NUM_MAC(devNum) == memberPtr->hwDevice)
    {
        PRV_CPSS_PHY_PORT_CHECK_MAC(devNum,memberPtr->port);
    }
    else
    {
        /* member is on remote device. Allow maximal physical
           port number 1023 for SIP5 and above architecture. */
        if(memberPtr->port >= BIT_10)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "memberPtr->port[%d] >= 'max' [%d]",
                memberPtr->port , BIT_10);
        }
        if(memberPtr->hwDevice >= BIT_10)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "memberPtr->hwDevice[%d] >= 'max' [%d]",
                memberPtr->hwDevice , BIT_10);
        }
    }

    hwFormat = 0;
    U32_SET_FIELD_MAC(hwFormat,0 ,10,memberPtr->hwDevice);
    U32_SET_FIELD_MAC(hwFormat,10,15,memberPtr->port);
...
/**
* @internal cpssDxChTrunkEcmpTableSet function
* @endinternal
*
* @brief   Function Relevant mode : Low level mode
*          Set Trunk ECMP entry info .
...
*
*/
GT_STATUS cpssDxChTrunkEcmpTableSet
(
    IN GT_U8                                devNum,
    IN GT_U32                               index,
    IN CPSS_TRUNK_MEMBER_STC               *memberPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkEcmpTableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, index, memberPtr));

    rc = internal_cpssDxChTrunkEcmpTableSet(devNum, index, memberPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, index, memberPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTrunkEcmpTableGet function
* @endinternal
*
* @brief   Function Relevant mode : ALL modes
*          Get Trunk ECMP entry info.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
...
* @param[in] index                 - the Trunk ECMP entry index
*
* @param[out] memberPtr            - (pointer to) trunk member.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChTrunkEcmpTableGet
(
    IN  GT_U8                                devNum,
    IN  GT_U32                               index,
    OUT CPSS_TRUNK_MEMBER_STC               *memberPtr
)
{
    ...
    memberPtr->hwDevice = U32_GET_FIELD_MAC(hwFormat,0 ,10);
    memberPtr->port     = U32_GET_FIELD_MAC(hwFormat,10,15);

    if(memberPtr->port >= BIT_10)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "memberPtr->port[%d] >= 'max' [%d]",
            memberPtr->port , BIT_10);
    }
...
/**
* @internal cpssDxChTrunkEcmpTableGet function
* @endinternal
*
* @brief   Function Relevant mode : ALL modes
*          Get Trunk ECMP entry info.
...
*
*/
GT_STATUS cpssDxChTrunkEcmpTableGet
(
    IN  GT_U8                                devNum,
    IN  GT_U32                               index,
    OUT CPSS_TRUNK_MEMBER_STC               *memberPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkEcmpTableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, index, memberPtr));

    rc = internal_cpssDxChTrunkEcmpTableGet(devNum, index, memberPtr);
...
/**
* @internal internal_cpssDxChTrunkDbCascadePolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Sets policy info for the Cascade trunks.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
...
*
*/
static GT_STATUS internal_cpssDxChTrunkDbCascadePolicyInfoSet
(
    IN GT_U8                             devNum,
    IN GT_TRUNK_ID                       trunkId,
    IN CPSS_TRUNK_POLICY_STC             *policyInfoPtr
)
{
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E);
    ...
    CASCADE_TRUNK_ID_FOR_SEPERATION_FROM_REGULARE_TRUNK_TABLE_CHECK(devNum,trunkId);
    return prvCpssGenericTrunkDbCascadePolicyInfoSet(devNum, trunkId, policyInfoPtr);
}

/**
* @internal cpssDxChTrunkDbCascadePolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Sets policy info for the Cascade trunks.
...
*
*/
GT_STATUS cpssDxChTrunkDbCascadePolicyInfoSet
(
    IN GT_U8                             devNum,
    IN GT_TRUNK_ID                       trunkId,
    IN CPSS_TRUNK_POLICY_STC             *policyInfoPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkDbCascadePolicyInfoSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, policyInfoPtr));

    rc = internal_cpssDxChTrunkDbCascadePolicyInfoSet(devNum, trunkId, policyInfoPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, trunkId, policyInfoPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTrunkDbCascadePolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
...
*
*/
static GT_STATUS internal_cpssDxChTrunkDbCascadePolicyInfoGet
(
    IN GT_U8                                 devNum,
    IN GT_TRUNK_ID                           trunkId,
...
)
{
    ...
    return prvCpssGenericTrunkDbCascadePolicyInfoGet(devNum, trunkId, policyInfoPtr);
}

/**
* @internal cpssDxChTrunkDbCascadePolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Gets policy info in case of cascade trunks.
...
*
*/
GT_STATUS cpssDxChTrunkDbCascadePolicyInfoGet
(
    IN GT_U8                                 devNum,
    IN GT_TRUNK_ID                           trunkId,
    OUT CPSS_TRUNK_POLICY_STC                *policyInfoPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkDbCascadePolicyInfoGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, policyInfoPtr));

    rc = internal_cpssDxChTrunkDbCascadePolicyInfoGet(devNum, trunkId, policyInfoPtr);
...
/**
* @internal internal_cpssDxChTrunkDbPolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
...
*
*/
static GT_STATUS internal_cpssDxChTrunkDbPolicyInfoSet
(
    IN GT_U8                             devNum,
    IN GT_TRUNK_ID                       trunkId,
    IN CPSS_TRUNK_POLICY_STC             *policyInfoPtr
)
{
    ...
    return prvCpssGenericTrunkDbPolicyInfoSet(devNum, trunkId, policyInfoPtr);
}

/**
* @internal cpssDxChTrunkDbPolicyInfoSet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
...
*
*/
GT_STATUS cpssDxChTrunkDbPolicyInfoSet
(
    IN GT_U8                             devNum,
    IN GT_TRUNK_ID                       trunkId,
    IN CPSS_TRUNK_POLICY_STC             *policyInfoPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkDbPolicyInfoSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, policyInfoPtr));

    rc = internal_cpssDxChTrunkDbPolicyInfoSet(devNum, trunkId, policyInfoPtr);
...
/**
* @internal internal_cpssDxChTrunkDbPolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Gets policy info for the regular trunks. Valid only for Flex trunks.
...
*
*/
static GT_STATUS internal_cpssDxChTrunkDbPolicyInfoGet
(
    IN GT_U8                                 devNum,
    IN GT_TRUNK_ID                           trunkId,
    OUT CPSS_TRUNK_POLICY_STC               *policyInfoPtr
)
{
    ...
    return prvCpssGenericTrunkDbPolicyInfoGet(devNum, trunkId, policyInfoPtr);
}

/**
* @internal cpssDxChTrunkDbPolicyInfoGet function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         Gets policy info for the regular trunks. Valid only for Flex trunks.
...
*
*/
GT_STATUS cpssDxChTrunkDbPolicyInfoGet
(
    IN GT_U8                                devNum,
    IN GT_TRUNK_ID                          trunkId,
    OUT CPSS_TRUNK_POLICY_STC               *policyInfoPtr
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkDbPolicyInfoGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, policyInfoPtr));

    rc = internal_cpssDxChTrunkDbPolicyInfoGet(devNum, trunkId, policyInfoPtr);
...
/**
* @internal internal_cpssDxChTrunkCascadePortAdd function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS internal_cpssDxChTrunkCascadePortAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    return prvCpssGenericTrunkCascadePortAdd(devNum, trunkId, portNum);
}

/**
* @internal cpssDxChTrunkCascadePortAdd function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         This function add port to the cascade trunk in the device.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTrunkCascadePortAdd
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkCascadePortAdd);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, portNum));

    rc = internal_cpssDxChTrunkCascadePortAdd(devNum, trunkId, portNum);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, trunkId, portNum));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTrunkCascadePortRemove function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS internal_cpssDxChTrunkCascadePortRemove
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    return prvCpssGenericTrunkCascadePortRemove(devNum, trunkId, portNum);
}

/**
* @internal cpssDxChTrunkCascadePortRemove function
* @endinternal
*
* @brief  Function Relevant mode : High Level mode
*         This function remove port from the cascade trunk in the device.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTrunkCascadePortRemove
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_PHYSICAL_PORT_NUM     portNum
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTrunkCascadePortRemove);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, trunkId, portNum));

    rc = internal_cpssDxChTrunkCascadePortRemove(devNum, trunkId, portNum);
