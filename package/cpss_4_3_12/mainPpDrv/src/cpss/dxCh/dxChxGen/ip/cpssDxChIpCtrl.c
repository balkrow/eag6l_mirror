...
#define PRV_CPSS_GLOBAL_MAC_SA_INDEX_CHECK_MAC(devNum,index)    \
...
static const CPSS_DXCH_IP_EXCPTION_CMD_REG_INFO_STC eArchExceptions[] =
{
    ...
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV4_E,       CPSS_PACKET_CMD_BRIDGE_E,           0,0,3,6},
    /* IPv6 Unicast MTU Exceeded command */
    {CPSS_DXCH_IP_EXCP_UC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_ROUTE_E,            1,3,3,0},
    {CPSS_DXCH_IP_EXCP_UC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_TRAP_TO_CPU_E,      1,3,3,2},
    {CPSS_DXCH_IP_EXCP_UC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_DROP_HARD_E,        1,3,3,3},
    {CPSS_DXCH_IP_EXCP_UC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_DROP_SOFT_E,        1,3,3,4},
    /* IPv6 Multicast MTU Exceeded command */
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_ROUTE_E,            1,0,3,0},
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_TRAP_TO_CPU_E,      1,0,3,2},
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_DROP_HARD_E,        1,0,3,3},
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_DROP_SOFT_E,        1,0,3,4},
    {CPSS_DXCH_IP_EXCP_MC_MTU_EXCEEDED_E,CPSS_IP_PROTOCOL_IPV6_E,       CPSS_PACKET_CMD_BRIDGE_AND_MIRROR_E,1,0,3,5},
...
)
{
    ...
    rc = prvCpssHwPpSetRegField(devNum,regAddr, fieldOffset,4,ctrlMultiTargetTCQueue | (failRpfMultiTargetTCQueue << 2));
    return rc;
}
...
)
{
    ...
    rc = prvCpssHwPpGetRegField(devNum,regAddr,fieldOffset+2,2,failRpfMultiTargetTCQueuePtr);
    if(GT_OK != rc)
        return rc;

    rc = prvCpssHwPpGetRegField(devNum,regAddr, fieldOffset,2,ctrlMultiTargetTCQueuePtr);
...
*                                      1. for non multi-port groups device this parameter is IGNORED.
*                                      2. for multi-port groups device :
*                                      (APPLICABLE DEVICES Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
*                                      bitmap must be set with at least one bit representing
*                                      valid port group(s). If a bit of non valid port group
*                                      is set then function returns GT_BAD_PARAM.
*                                      value CPSS_PORT_GROUP_UNAWARE_MODE_CNS is supported.
...
*                                      1. for non multi-port groups device this parameter is IGNORED.
*                                      2. for multi-port groups device :
*                                      (APPLICABLE DEVICES Bobcat3; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
*                                      bitmap must be set with at least one bit representing
*                                      valid port group(s). If a bit of non valid port group
*                                      is set then function returns GT_BAD_PARAM.
*                                      value CPSS_PORT_GROUP_UNAWARE_MODE_CNS is supported.
...
*          For other devices (Falcon; AC5P; AC5X; Harrier; Ironman; AAS) it sets the 36 most significant bits
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*          For other devices (Falcon; AC5P; AC5X; Harrier; Ironman; AAS) it gets the 36 most significant bits
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*                                      (APPLICABLE RANGES: Falcon; AC5P; AC5X; Harrier; Ironman; AAS 0..8191).
...
*                                      (APPLICABLE RANGES: Falcon; AC5P; AC5X; Harrier; Ironman; AAS 0..4095).
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error.
* @retval GT_HW_ERROR              - on hardware error
...
)
{
    ...
    PRV_CPSS_GLOBAL_MAC_SA_INDEX_CHECK_MAC(devNum,routerMacSaIndex);
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {CPSS_TBD_BOOKMARK_AAS /* do we need dedicated API for this ? */
        /* set also into src eport - used for MIM tunnel start */
        /* in sip5 the CPSS_DXCH_SIP5_TABLE_HA_EGRESS_EPORT_1_E was accessed with src eport */
        rc = prvCpssDxChWriteTableEntryField(devNum,
               CPSS_DXCH_SIP7_TABLE_HA_SRC_EPORT_1_E,
               portNum,
               PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
               SIP7_HA_SRC_EPORT_1_TABLE_FIELDS_ROUTER_MAC_SA_INDEX_E, /* field name */
               PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
               routerMacSaIndex);
        if(rc != GT_OK)
        {
...
/**
* @internal internal_cpssDxChIpEcmpUcRpfAccessLevelSet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChIpEcmpUcRpfAccessLevelSet
(
    IN GT_U8                            devNum,
    IN CPSS_IP_PROTOCOL_STACK_ENT       protocolStack,
...
)
{
    ...
    {
         CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "accessLevel range is up to 6 bits");
    }

    if ((protocolStack==CPSS_IP_PROTOCOL_IPV4_E)||(protocolStack==CPSS_IP_PROTOCOL_IPV4V6_E))
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.IPv4GlobalCtrl.IPv4CtrlReg1;
        rc = prvCpssHwPpSetRegField(devNum,regAddr,22,6,accessLevel);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.IPv6GlobalCtrl.IPv6CtrlReg1;
        rc = prvCpssHwPpSetRegField(devNum,regAddr,23,6,accessLevel);
        if (rc != GT_OK)
        {
            return rc;
        }
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpEcmpUcRpfAccessLevelSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, protocolStack, accessLevel));

    rc = internal_cpssDxChIpEcmpUcRpfAccessLevelSet(devNum, protocolStack, accessLevel);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, protocolStack, accessLevel));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);

    return rc;
}
/**
* @internal internal_cpssDxChIpEcmpUcRpfAccessLevelGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChIpEcmpUcRpfAccessLevelGet
(
    IN  GT_U8                           devNum,
    IN CPSS_IP_PROTOCOL_STACK_ENT       protocolStack,
...
)
{
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.IPv4GlobalCtrl.IPv4CtrlReg1;
        rc = prvCpssHwPpGetRegField(devNum,regAddr,22,6,&hwData_ipv4);
        if (rc != GT_OK)
        {
            return rc;
        }
        *accessLevelPtr = hwData_ipv4;
    }
    if ((protocolStack==CPSS_IP_PROTOCOL_IPV6_E)||(protocolStack==CPSS_IP_PROTOCOL_IPV4V6_E))
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.IPv6GlobalCtrl.IPv6CtrlReg1;
        rc = prvCpssHwPpGetRegField(devNum,regAddr,23,6,&hwData_ipv6);
        if (rc != GT_OK)
        {
            return rc;
        }
        *accessLevelPtr = hwData_ipv6;
    }

    if ((protocolStack==CPSS_IP_PROTOCOL_IPV4V6_E)&&(hwData_ipv4!=hwData_ipv6))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "Error:IPv4 accessLevel is not equal to IPv6 accessLevel."
                                               "Can not use CPSS_IP_PROTOCOL_IPV4V6_E as input ");
    }
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpEcmpUcRpfAccessLevelGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, protocolStack, accessLevelPtr));

    rc = internal_cpssDxChIpEcmpUcRpfAccessLevelGet(devNum, protocolStack, accessLevelPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, protocolStack, accessLevelPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
)
{
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            /* CPU write is not supported
               the register is cleared after read */
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
        }
        else
        {
    ...
    /* set the counter */
    rc = prvCpssPortGroupsBmpCounterSet(devNum, portGroupsBmp, regAddr, 0, 32, dropPkts);
    return rc;
}
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* drop counter is GT_U64 and not GT_U32 as in sip6 */
        regAddr1 = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.routerDropCntr.routerDropCntr1;
        rc = prvCpssPortGroupsBmpCounterSummary(devNum, portGroupsBmp, regAddr1,
                                                0, 32, &(dropPktsPtr->l[1]), NULL);
    }
    return rc;
}
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2;
*          Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                   - device number
* @param[in] enable                   - enable/disable FDB
*                                       routing for PBR packets
...
)
{
    ...
    return prvCpssHwPpSetRegField(devNum, regAddr, offset, bitLen, value);
}
/**
...
)
{
    ...
    rc = prvCpssHwPpGetRegField(devNum, regAddr, offset, bitLen, &value);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal internal_cpssDxChIpFdbRouteCounterSetIndexSet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChIpFdbRouteCounterSetIndexSet
(
    IN GT_U8                        devNum,
...
)
{
    ...
    switch(countSetIndex)
    {
        case CPSS_IP_CNT_SET0_E:
        case CPSS_IP_CNT_SET1_E:
        case CPSS_IP_CNT_SET2_E:
        case CPSS_IP_CNT_SET3_E:
            hwValue = countSetIndex;/*0..3*/
            break;
        case CPSS_IP_CNT_NO_SET_E:
            hwValue = 7;/*0x7 = NoSetAssociated*/
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_ON_ENUM_MAC(countSetIndex);
    }

    return prvCpssHwPpSetRegField(devNum,
        PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->IPvX.routerGlobalCtrl.routerGlobalCtrl3,
        3, 3, hwValue);
}
/**
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpFdbRouteCounterSetIndexSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, countSetIndex));

    rc = internal_cpssDxChIpFdbRouteCounterSetIndexSet(devNum, countSetIndex);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, countSetIndex));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChIpFdbRouteCounterSetIndexGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChIpFdbRouteCounterSetIndexGet
(
    IN  GT_U8                        devNum,
...
)
{
    GT_STATUS   rc;
    GT_U32  hwValue;
    CPSS_IP_CNT_SET_ENT countSetIndex;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        ...
        CPSS_BOBCAT2_E | CPSS_CAELUM_E  | CPSS_ALDRIN_E | CPSS_AC3X_E    | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E |
        CPSS_FALCON_E  | CPSS_AC5P_E    | CPSS_AC5X_E   | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    CPSS_NULL_PTR_CHECK_MAC(countSetIndexPtr);
    ...
    {
        case 0:
            countSetIndex = CPSS_IP_CNT_SET0_E;
            break;
        case 1:
            countSetIndex = CPSS_IP_CNT_SET1_E;
            break;
        case 2:
            countSetIndex = CPSS_IP_CNT_SET2_E;
            break;
        case 3:
            countSetIndex = CPSS_IP_CNT_SET3_E;
            break;
        case 7:
            countSetIndex = CPSS_IP_CNT_NO_SET_E;
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE,"unexpected value[%d]",hwValue);
    }

    *countSetIndexPtr = countSetIndex;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpFdbRouteCounterSetIndexGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, countSetIndexPtr));

    rc = internal_cpssDxChIpFdbRouteCounterSetIndexGet(devNum, countSetIndexPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, countSetIndexPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet
(
    IN  GT_U8    devNum,
    IN  GT_BOOL  enable
)
{
    ...
    /* update enable bypass value */
    value = (enable == GT_TRUE) ? 1:0;
    rc = prvCpssHwPpSetRegField(devNum,regAddr,4,1,value);
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enable));

    rc = internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet(devNum, enable);
...
/**
* @internal internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet function
* @endinternal
*
...
* @retval GT_BAD_PTR               - on NULL ptr
*/
static GT_STATUS internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet
(
    IN  GT_U8    devNum,
    OUT GT_BOOL  *enablePtr
)
{
    ...
    /* Get status of bypassing router for IGMP/MLD packets */
    ...
    *enablePtr = (0 == hwValue) ? GT_FALSE : GT_TRUE;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enablePtr));

    rc = internal_cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet(devNum, enablePtr);
