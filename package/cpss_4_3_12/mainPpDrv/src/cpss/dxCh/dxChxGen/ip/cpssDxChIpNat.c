...
/* sip5 need 6 words for 192 bit */
/* sip7 need 7 words for 198 bit */
#define NAT_HW_SIZE_CNS       7
...
)
{
    ...
    switch (natType)
    {
        case CPSS_IP_NAT_TYPE_NAT44_E:
            ...
            /* Set SIP and DIP Prefix Lengths only for Ironman*/
            if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                /* check sipPrefixLen is legal (1..32)*/
                if (logicFormatPtr->nat44Entry.sipPrefixLen > BIT_5 || logicFormatPtr->nat44Entry.sipPrefixLen < BIT_0)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
                }
                value = logicFormatPtr->nat44Entry.sipPrefixLen - 1;
                SIP5_HA_NAT44_FIELD_VALUE_SET_MAC(devNum,SIP6_30_HA_NAT44_TABLE_FIELDS_NAT_SIP_PREFIX_LENGTH_E);

                /* check dipPrefixLen is legal (1..32)*/
                if (logicFormatPtr->nat44Entry.dipPrefixLen > BIT_5 || logicFormatPtr->nat44Entry.dipPrefixLen < BIT_0)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
                }
                value = logicFormatPtr->nat44Entry.dipPrefixLen - 1;
    ...
    switch (value)
    {
        case 0:
            *natTypePtr = CPSS_IP_NAT_TYPE_NAT44_E;
            ...
            /* Get SIP and DIP Prefix Lengths only for Ironman*/
            if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                SIP5_HA_NAT44_FIELD_VALUE_GET_MAC(devNum,SIP6_30_HA_NAT44_TABLE_FIELDS_NAT_SIP_PREFIX_LENGTH_E);
                logicFormatPtr->nat44Entry.sipPrefixLen = value + 1;
                SIP5_HA_NAT44_FIELD_VALUE_GET_MAC(devNum,SIP6_30_HA_NAT44_TABLE_FIELDS_NAT_DIP_PREFIX_LENGTH_E);
                logicFormatPtr->nat44Entry.dipPrefixLen = value + 1;
            }
...
/**
* @internal internal_cpssDxChIpNatL2GlobalConfigSet function
* @endinternal
*
* @brief  This function is used to set the L2NAT SrcID size and
*         SrcID LSB in the global configuration register.
*
...
*
*/
static GT_STATUS internal_cpssDxChIpNatL2GlobalConfigSet
(
    IN   GT_U8                                 devNum,
    IN   CPSS_IP_NAT_TYPE_ENT                  natType,
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
                                          CPSS_BOBCAT2_E | CPSS_BOBCAT3_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_LION2_E |
                                          CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_HARRIER_E);

    if(PRV_CPSS_PP_MAC(devNum)->l2NatSupported == GT_FALSE)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
    }

    CPSS_NULL_PTR_CHECK_MAC(l2NatGlobalCfgPtr);

    /* SST-ID is 12 bits */
    l2NatMax = l2NatGlobalCfgPtr->l2NatSrcIdSize + l2NatGlobalCfgPtr->l2NatSrcIdLsb;
    if(l2NatMax > PRV_CPSS_DXCH_PP_HW_INFO_FLEX_FIELD_NUM_BITS_SUPPORTED_MAC(devNum).sstId)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }

    /* L2NAT configurations are supported only on NAT44 table type */
    ...
    /*<L2NAT Src ID - Size> 14:17*/
    regAddr = PRV_DXCH_REG1_UNIT_HA_MAC(devNum).haGlobalConfig2;
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 14, 4, l2NatGlobalCfgPtr->l2NatSrcIdSize);
    if (rc != GT_OK)
    {
        return rc;
    }

    /*<L2NAT Src ID - LSB> 18:21*/
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 18, 4, l2NatGlobalCfgPtr->l2NatSrcIdLsb);
    if (rc != GT_OK)
    {
        return rc;
    }
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpNatL2GlobalConfigSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, natType, l2NatGlobalCfgPtr));

    rc = internal_cpssDxChIpNatL2GlobalConfigSet(devNum, natType,
                                                 l2NatGlobalCfgPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, natType, l2NatGlobalCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChIpNatL2GlobalConfigGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChIpNatL2GlobalConfigGet
(
    IN   GT_U8                                 devNum,
    IN   CPSS_IP_NAT_TYPE_ENT                  natType,
...
)
{
    ...
    /* L2NAT configurations are supported only on Ironman device */
    if (!PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_APPLICABLE_DEVICE, LOG_ERROR_NO_MSG);
    }
    ...
    /*<L2NAT Src ID - Size> 14:17*/
    regAddr = PRV_DXCH_REG1_UNIT_HA_MAC(devNum).haGlobalConfig2;
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 14, 4, &hwValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    l2NatGlobalCfgPtr->l2NatSrcIdSize = hwValue;

    /*<L2NAT Src ID - LSB> 18:21*/
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 18, 4, &hwValue);
    if (rc != GT_OK)
    {
        return rc;
    }
    l2NatGlobalCfgPtr->l2NatSrcIdLsb = hwValue;
...
* @endinternal
*
* @brief  This function is used to get the L2NAT SrcID size and SrcID LSB from the
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpNatL2GlobalConfigGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, natType, l2NatGlobalCfgPtr));

    rc = internal_cpssDxChIpNatL2GlobalConfigGet(devNum, natType,
                                                 l2NatGlobalCfgPtr);
