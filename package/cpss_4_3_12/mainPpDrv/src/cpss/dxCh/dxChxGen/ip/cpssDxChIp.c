...
#define CPSS_DXCH_MAX_SIZE_OF_IPVX_NEXTHOP_IN_WORD 5 /* sip 6 was 4 , sip7 is 5 */
...
* @endinternal
*
* @brief   Writes a block of consecutive UC route entries to HW.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2;
*                                  Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] devNum                - the device number
* @param[in] baseRouteEntryIndex   - The index of the line from which to start writing the block.
...
)
{
    GT_STATUS retVal = GT_OK;
    GT_U32 i;
    GT_U32 hwData[CPSS_DXCH_MAX_SIZE_OF_IPVX_NEXTHOP_IN_WORD];
    PRV_CPSS_DXCH_PP_CONFIG_FINE_TUNING_STC *fineTuningPtr;
...
* @endinternal
*
* @brief   Read a block of consecutive UC route entries from HW.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2;
*                                  Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] devNum                - the device number
* @param[in] baseRouteEntryIndex   - The index of the line from which to start reading the block.
* @param[in] routeEntriesArray     - the uc route entries array, for each element
*                                    set the uc route entry type to determine how
*                                    entry data will be interpreted
* @param[in] numOfRouteEntries     - the number route entries in the array (= the
*                                    number of route entries to read)
* @param[out] routeEntriesArray    - the uc route entries array read
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on devNum not active.
* @retval GT_BAD_PTR               - one of the parameters is NULL pointer
...
*
*       For Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS devices, Each line can hold:
*       - 2 NAT entries
...
)
{
    ...
    GT_U32        isVPort;/* support 'vPort mode'*/
    GT_STATUS     rc;

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);

    /* clear hw data */
    cpssOsMemSet(hwDataPtr, 0, sizeof(GT_U32) * CPSS_DXCH_MAX_SIZE_OF_IPVX_NEXTHOP_IN_WORD);
    ...
    case CPSS_DXCH_IP_UC_ROUTE_ENTRY_E:
        ...
        if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
        {
            ...
            switch (muxMode)
            {
                ...
                case CPSS_DXCH_IP_NEXT_HOP_MUX_MODE_TRG_EPG_FOR_UC_MC_E:
                {
                    ...
                    if((!(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))) && (value > 0x1ff))
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
                    }
        ...
        if (routeEntry->nextHopInterface.type == CPSS_INTERFACE_PORT_E)
        {
            ...
            if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
            {
                ...
                isVPort = PRV_CPSS_DXCH_IS_VPORT(devNum,routeEntry->nextHopInterface.devPort.portNum);
                if(isVPort)
                {
                    PRV_CPSS_DXCH_VPORT_FIELD_CHECK_MAC(devNum,routeEntry->nextHopInterface.devPort.portNum);

                    value = 1;  /* target is vPort */
                    SIP5_IPVX_FIELD_VALUE_SET_MAC(devNum,SIP7_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_TARGET_IS_VPORT_E);

                    value = routeEntry->nextHopInterface.devPort.portNum;
                    SIP5_IPVX_FIELD_VALUE_SET_MAC(devNum,SIP7_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_TARGET_VPORT_E);
                }
                else
                {
                    value = targHwDev;
    ...
    case CPSS_DXCH_IP_UC_ECMP_RPF_E:
        ...
        if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
        {
            ...
            {
                /* in sip7 vlan is 16 bits */
                hwDataPtr[0] |= (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[0]) |
                            (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[1] << 16);

                hwDataPtr[1] |= (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[2]) |
                            (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[3] << 16);

                hwDataPtr[2] |= (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[4]) |
                            (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[5] << 16);

                hwDataPtr[3] |= (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[6]) |
                            (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[7] << 16);
            }
            else
            {
                /* in sip6 vlan is 13 bits */
                hwDataPtr[0] |= (ipUcRouteEntryPtr->entry.ecmpRpfCheck.vlanArray[0]) |
        ...
        if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
        {
            ...
            if (value == 1)
            {
                ...
                else
                {
                    ...
                    regularEntry->nextHopInterface.vidx = value;
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        SIP5_IPVX_FIELD_VALUE_GET_MAC(devNum,SIP7_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_TARGET_IS_VPORT_E);
                        isVPort = value;
                    }
                    else
                    {
                    ...
                    if (isVPort)/*vPort*/
                    {
                        SIP5_IPVX_FIELD_VALUE_GET_MAC(devNum,SIP7_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_TARGET_VPORT_E);
                        regularEntry->nextHopInterface.devPort.portNum  = value;
                        regularEntry->nextHopInterface.devPort.hwDevNum = CPSS_PARAM_NOT_USED_CNS;
                    }
                    else
                    {
                        /* PORT */
...
)
{
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        ...
        if(!((muxMode == CPSS_DXCH_IP_NEXT_HOP_MUX_MODE_TRG_EPG_FOR_UC_MC_E) && (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))))
        {
            value = modifyDscp;
            SIP5_IPVX_FIELD_VALUE_SET_MAC(devNum,SIP5_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_MODIFY_DSCP_E);
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
        {
            /* hw support 18 bits but only values with 17 bits are legal */
            value = (ipMcRouteEntryPtr->externalMLLPointer & 0x1FFFF);
            SIP5_IPVX_FIELD_VALUE_SET_MAC(devNum,SIP5_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_EXTERNAL_MLL_PTR_E);

            value = (ipMcRouteEntryPtr->internalMLLPointer & 0x1FFFF);
            SIP5_IPVX_FIELD_VALUE_SET_MAC(devNum,SIP5_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_INTERNAL_MLL_PTR_E);

        }
        else
        {
            /* 16 bits spread over 2 nonconsecutive location */
...
{
    GT_U32 value, cmd, cpuCodeIdx, qosPrecedence=0, modifyDscp=0, modifyUp=0,
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
        {
            ...
            ipMcRouteEntryPtr->externalMLLPointer = (value & 0x1FFFF);

            SIP5_IPVX_FIELD_VALUE_GET_MAC(devNum,SIP5_IPVX_ROUTER_NEXTHOP_TABLE_FIELDS_INTERNAL_MLL_PTR_E);
            ipMcRouteEntryPtr->internalMLLPointer = (value & 0x1FFFF);
        }
        else
        {
             /* 16 bits spread over 2 nonconsecutive location */
...
)
{
    ...
    for (i = 0; i<numOfRouteEntries; i++)
    {
        ...
        if ( ( (routeEntry->cmd == CPSS_PACKET_CMD_ROUTE_E) || (routeEntry->cmd == CPSS_PACKET_CMD_ROUTE_AND_MIRROR_E) )
            ...
            if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
            {
                if ((routeEntry->ingressMirror == GT_TRUE) &&
                    (routeEntry->ingressMirrorToAnalyzerIndex > PRV_CPSS_DXCH_IP_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum)))
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
                }
            }
        ...
        case CPSS_DXCH_IP_UC_ECMP_RPF_E:
            ...
            for (j = 0 ; j < 8 ; j++)
            {
                /* in sip7 VID is 16 bits, in sip6 it is 13 bits */
...
)
{
    ...
    if (PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_ENABLED_MAC(devNum) == GT_TRUE)
    {
        if ((routeEntryPtr->ingressMirror == GT_TRUE) &&
            (routeEntryPtr->ingressMirrorToAnalyzerIndex > PRV_CPSS_DXCH_IP_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum)))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    if(useVidx0 == 1)
    {
        ...
        mllEntryPtr->nextHopInterface.vidx =
            U32_GET_FIELD_MAC(hwDataPtr[relativeLocation],16,12);
...
)
{
    ...
    else
    {
        ...
        {
            isVPort = PRV_CPSS_DXCH_IS_VPORT(devNum,currSwPtr->nextHopInterface.devPort.portNum);
...
)
{
    ...
    if(value)
    {
        ...
        if(value != 0xFFF)
        {
            ...
            currSwPtr->nextHopInterface.vidx = value;
        }
        else
        {
    ...
    else
    {
        ...
        else
        {
            ...
            if(isVPort)
            {
                ...
                currSwPtr->nextHopInterface.devPort.hwDevNum = CPSS_PARAM_NOT_USED_CNS;
            }
            else
            {
                SIP5_IP_MLL_FIELD_GET_MAC(devNum,hwDataPtr,
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
        return prvCpssDxChLpmHwEcmpEntryWriteSip7(devNum, ecmpEntryIndex, &lpmEcmpEntry);
    }
    else
    {
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        rc = prvCpssDxChLpmHwEcmpEntryReadSip7(devNum, ecmpEntryIndex, &lpmEcmpEntry);
    }
    else
    {
...
*
* @note   APPLICABLE DEVICES:      Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
        maxNumOfNextHopEntries = MIN(BIT_18, PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.routerNextHop);
    }
    else
    {
