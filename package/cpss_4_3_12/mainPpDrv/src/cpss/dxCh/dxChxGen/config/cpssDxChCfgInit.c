...
#include <cpss/common/diag/sse/cpssDxChSseIf.h>
...
#define TABLE_NUM_ENTRIES_GET_MAC(_devNum, _tableType) \
    ((_tableType < PRV_CPSS_DXCH_PP_MAC(devNum)->accessTableInfoSize) ?  \
        ...
        PRV_CPSS_ACCESS_TABLE_INFO_PTR(_devNum)[_tableType].maxNumOfEntries : \
...
#define PRV_CPSS_SIP6_MAX_PBR_ENTRIES_CNS 163840 /* (32k)*(5 entries in a line) */

/* max number of PBR entries for SIP7 */
#define PRV_CPSS_SIP7_MAX_PBR_ENTRIES_CNS 655360 /* (128K)*(5 entries in a line) */
...
/* max number of PBR lines for SIP6 32K */
...
#define PRV_CPSS_SIP7_MAX_PBR_LINES_CNS PRV_CPSS_SIP7_MAX_PBR_ENTRIES_CNS/PRV_CPSS_DXCH_CFG_SIP6_NUM_PBR_ENTRIES_IN_LPM_LINE_CNS
...
#define SIP7_LPM_MAX_NUM_OF_SMALL_BLOCKS 8
#define SIP7_LPM_MAX_NUM_OF_BIG_BLOCKS 48
...
)
{
    ...
    *pbrBankSizePtr = bankSize;
    ...
    if(bankSize >= _32KB)
    {
        ...
        *pbrBankSizePtr = _32KB;
    }
    else if(bankSize >= _16KB)
    {
        *pbrBankInitSizePtr = 2;
        *pbrBankSizePtr = _16KB;
    }
    else if(bankSize >= _8KB)
    {
    ...
    pbrLinesPerBank = ((bankSize < *pbrBankSizePtr) ? bankSize : *pbrBankSizePtr);
    pbrMaxBankNumber = (numberOfPbrLines / pbrLinesPerBank);
    if(numberOfPbrLines > pbrMaxBankNumber*pbrLinesPerBank)
    {
        pbrMaxBankNumber++;
    }

    *numberOfPbrBanksPtr = pbrMaxBankNumber;

    if(*pbrBankSizePtr > bankSize)
    {
        *pbrBankSizePtr = bankSize;
    }
...
/**
* @internal prvCpssDxChCfgCalculatePbrBanksNumberAndHwSizeCnfgSip7 function
* @endinternal
*
* @brief   This function calculates pbr banks number and pbr bank size
*
...
* @retval GT_OK                    - on success
*/
GT_STATUS prvCpssDxChCfgCalculatePbrBanksNumberAndHwSizeCnfgSip7
(
    IN GT_U32 bankSize,
    IN GT_U32 numberOfPbrLines,
    OUT GT_U32 *numberOfPbrBanksPtr,
...
{
    GT_U32 maxAddressablePbrBanks = 32;
    GT_U32 loopCount=0;
    GT_U32 pbrMaxBankNumber = 0;
    *pbrBankSizePtr = bankSize;
    ...
    while (GT_TRUE)
    {
        if (maxAddressablePbrBanks==1)
        {
            if (numberOfPbrLines <= bankSize)
            {
                /* max addressable pbr banks is 1 */
                /* each bank can address 256K */
                *pbrBankInitSizePtr = loopCount;
                break;
            }
            else
            {
        ...
        {
            if ((numberOfPbrLines > (maxAddressablePbrBanks / 2) * bankSize) && (numberOfPbrLines <= maxAddressablePbrBanks * bankSize))
            {
                *pbrBankInitSizePtr = loopCount;
                break;
            }
            ...
            {
                maxAddressablePbrBanks=maxAddressablePbrBanks/2;
                loopCount++;
            }
        }
    }

    /* max numbers of pbr bank is set according to the value of pbrBankInitSizePtr
       if pbrBankInitSizePtr = 0 ==>  max addressable pbr banks is 32 and each bank is 8K
       if pbrBankInitSizePtr = 1 ==>  max addressable pbr banks is 16 and each bank is 16K
       if pbrBankInitSizePtr = 2 ==>  max addressable pbr banks is 8 and each bank is 32K
       if pbrBankInitSizePtr = 3 ==>  max addressable pbr banks is 4 and each bank is 64K
       if pbrBankInitSizePtr = 4 ==>  max addressable pbr banks is 2 and each bank is 128K
       if pbrBankInitSizePtr = 5 ==>  max addressable pbr bank is 1 with size 256K */
    pbrMaxBankNumber = 32>>(*pbrBankInitSizePtr);

    /* if the bank size is bigger then the max addressable pbr bank
       then set the output pbrBankSizePtr to be the max size allowed */
    if (bankSize > (PRV_CPSS_SIP7_MAX_PBR_LINES_CNS/pbrMaxBankNumber))
    {
        *pbrBankSizePtr = (PRV_CPSS_SIP7_MAX_PBR_LINES_CNS/pbrMaxBankNumber);
    }
...
)
{
    ...
    else
    {
        ...
        if ((smallBanksNumber > 0) && (numberOfPbrLines <= smallBankSize))
        {
            numOfBigPbrBlocks = 0;
    ...
    if (numOfSmallPbrBlocks > 0)
    {
        ...
        /* 32K lines in a block, each line is 16 bytes ==> 32*1024*16 = 0x80000 number of bytes in each block */
        *pbrStartIndexPtr = (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared/*30*/ - numOfSmallPbrBlocks)*0x80000;
    }
...
/**
* @internal prvCpssDxChCfgCalculatePbrIndexAndBankSizeSip7 function
* @endinternal
*
* @brief   This function calculates pbr start index and pbr bank size
*
...
* @retval GT_OK                    - on success
*/
static GT_STATUS prvCpssDxChCfgCalculatePbrIndexAndBankSizeSip7
(
    IN GT_U8     devNum,
    OUT GT_U32   *pbrStartIndexPtr,
    OUT GT_U32   *pbrInitBankSizePtr
)
{
    ...
    if (moduleCfgPtr->ip.maxNumOfPbrEntries == 0)
    {
        /*big banks up to 48, small banks up to 8 (but then big is only 40) */
        moduleCfgPtr->ip.numOfBigBanks = moduleCfgPtr->ip.lpmSharedMemoryBankNumber;
        if (moduleCfgPtr->ip.numOfBigBanks >= (SIP7_LPM_MAX_NUM_OF_BIG_BLOCKS-SIP7_LPM_MAX_NUM_OF_SMALL_BLOCKS))
        {
            moduleCfgPtr->ip.numOfSmallBanks = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared/*48*/
                                                - moduleCfgPtr->ip.numOfBigBanks;
        }
        else
        {
            moduleCfgPtr->ip.numOfSmallBanks = SIP7_LPM_MAX_NUM_OF_SMALL_BLOCKS;/* 8 small banks */
        }
        return GT_OK;
    }
    numberOfPbrLines = moduleCfgPtr->ip.maxNumOfPbrEntries / PRV_CPSS_DXCH_CFG_SIP6_NUM_PBR_ENTRIES_IN_LPM_LINE_CNS;
    ...
    moduleCfgPtr->ip.numberOfPbrLines = numberOfPbrLines;
     /* there is no case when big blocks and small blocks are used together for PBR:
        or small blocks, or big blocks */
    /* big banks for PBR is taken from the beginning of bank addresses, small banks from the end */
    if (moduleCfgPtr->ip.lpmSharedMemoryBankNumber == 0)
    {
        /* only small banks */
        bigBanksNumber = 0;
        bigBanksSize = 0;
        smallBanksNumber = SIP7_LPM_MAX_NUM_OF_SMALL_BLOCKS;/* 8 small banks */
        *pbrInitBankSizePtr = 0; /* 8K */
        rc = prvCpssDxChCfgCalculatePbrBanksNumberAndHwSizeCnfgSip7(smallBankSize,numberOfPbrLines,
                                                                    &numOfSmallPbrBlocks,pbrInitBankSizePtr,&pbrBankSize);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "bad lpm-pbr configuration");
    ...
    else
    {
        ...
        bigBanksNumber = moduleCfgPtr->ip.lpmSharedMemoryBankNumber;
        bigBanksSize = moduleCfgPtr->ip.lpmBankSize;
        smallBanksNumber = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared/*48*/ - bigBanksNumber;
        if(smallBanksNumber > PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksNonShared)
        {
            /* limited to 8 small banks */
            smallBanksNumber = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksNonShared;
        }
        ...
        else
        {
            ...
            rc = prvCpssDxChCfgCalculatePbrBanksNumberAndHwSizeCnfgSip7(bigBanksSize,numberOfPbrLines,
                                                                    &numOfBigPbrBlocks,pbrInitBankSizePtr,&pbrBankSize);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "bad lpm-pbr configuration");
    ...
    if (numOfSmallPbrBlocks > 0)
    {
        /* small banks is starting from position 40 and ending on position 47 */
        /* We have 48 big banks on address lane and taking for PBR from the end  */
        /* 256K lines in a block, each line is 32 bytes ==> 256*1024*32 = 0x800000 number of bytes in each block */
        *pbrStartIndexPtr = (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.sip6_maxBlocksWithShared/*48*/ - numOfSmallPbrBlocks)*0x800000;
    }
    return GT_OK;
}
...
*                                      Valid if useFuQueue is GT_TRUE.
* @param[in] useFuQueue               - FDB/CNC upload queue type.
...
(
    ...
    IN CPSS_PP_PHASE2_UPLOAD_QUEUE_TYPE_ENT  useFuQueue,
...
)
{
    ...
    if(doSdma == GT_TRUE)
    {
        ...
        {
            if (useFuQueue != CPSS_PP_PHASE2_UPLOAD_QUEUE_SHARED_WITH_AUQ_E)
            {
                /* FUQ registers */
                addrUpdateQueueCtrl[numOfIter] =
            ...
            if (oldSystemRecoveryInfo.systemRecoveryProcess != CPSS_SYSTEM_RECOVERY_PROCESS_HA_E)
            {
                for(ii = 0; ii < numOfIter; ii++)
                {
                    ...
                    retVal = prvCpssHwPpMgSetRegField(devNum,
                           ...
                           PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->globalRegs.fuQControl,
                           31,1,
                           (useFuQueue != CPSS_PP_PHASE2_UPLOAD_QUEUE_SHARED_WITH_AUQ_E) ? 1 : 0);
                    if (retVal != GT_OK)
                    {
                         prvCpssSystemRecoveryStateUpdate(&oldSystemRecoveryInfo);
                         return retVal;
        ...
        for(ii = 0; ii < numOfIter; ii++)
        {
            ...
            if (retVal != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(retVal, "cpssOsVirt2Phy failed for address %p\n", descCtrl[ii]->blockAddr);
            }
...
)
{
    ...
    rc = cpssDxChPortManagerEnableGet(devNum,&pmEnable);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    if(GT_TRUE==pmEnable)
    {
        ...
        tmpPortManagerDbPtr = (PRV_CPSS_PORT_MNG_DB_STC* )(&(PRV_CPSS_DXCH_PP_MAC(devNum)->port.portManagerDb));
        if(tmpPortManagerDbPtr!=NULL)
        {
            if(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr!=NULL)
            {
                for(ii=0;ii<PRV_CPSS_MAX_PP_PORTS_NUM_CNS;ii++)
                {
                    if(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[ii]!=NULL)
                    {
                        FREE_PTR_MAC(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[ii]);
                    }
                }
                FREE_PTR_MAC(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr);
            }
        }
     }
    ...
    */
    ...
    FREE_PTR_MAC(PRV_CPSS_DXCH_PP_MAC(devNum)->port.falconPortDeleteInfoPtr);
    {
        ...
        for (dp = 0; (dp < PRV_CPSS_MAX_SIP7_DP_CNS); dp++)
        {
            FREE_PTR_MAC(PRV_CPSS_DXCH_PP_MAC(devNum)->port.sip7_dpInfoPtr[dp]);
        }
    }
    ...
    if(NULL!=PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset)
    {
        FREE_PTR_MAC(PRV_CPSS_DXCH_PP_MAC(devNum)->port.gtMacCounterOffset);
    }


    for(ii = 0;ii <3;ii++)
    {
        if(NULL!=PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.info[ii].localDmaMapArr)
        {
            FREE_PTR_MAC(PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.info[ii].localDmaMapArr);
        }
    }

    if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
    {
        rc = prvCpssDxChGdmaFreeDb(devNum);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    if((CPSS_PP_FAMILY_DXCH_FALCON_E==PRV_CPSS_PP_MAC(devNum)->devFamily)||
       ...
       (CPSS_PP_FAMILY_DXCH_ALDRIN_E==PRV_CPSS_PP_MAC(devNum)->devFamily)||
       ((CPSS_PP_FAMILY_DXCH_ALDRIN_E==PRV_CPSS_PP_MAC(devNum)->devFamily)&&(PRV_CPSS_PP_MAC(devNum)->devSubFamily == CPSS_PP_SUB_FAMILY_BOBCAT2_BOBK_E))||
       (CPSS_PP_FAMILY_DXCH_BOBCAT3_E==PRV_CPSS_PP_MAC(devNum)->devFamily)
       )
    {
       hwsAvagoSerdesFreeInfoMapDb(devNum);
    }

    prvCpssGenericSrvCpuRemove(devNum, 0xFF);

    prvCpssDxChPhyMpdDbClear(devNum);
...
)
{
    ...
    if ((useFuQueue == CPSS_PP_PHASE2_UPLOAD_QUEUE_SEPARATE_E)
         && (PRV_CPSS_DXCH_PP_HW_INFO_CNC_MAC(devNum).cncUnits > 2))
    {
        /* but not CPSS_PP_PHASE2_UPLOAD_QUEUE_SEPARATE_AND_LIMITED_CNC_E */
        useCnc23Fuq = GT_TRUE;
    }
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        ...
        (useFuQueue!= CPSS_PP_PHASE2_UPLOAD_QUEUE_SHARED_WITH_AUQ_E)
...
)
{
    ...
    if((lpm_numOfSbm == 4) && (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E))
    {
...
/* function for WM/GM - to allow optimize the memory allocations */
...
)
{
    ...
    GT_U32  wmTableType =
            tableType == GT_NA                                  ? WM_TABLE_STATRTED_SESSION_E :
            tableType == GT_NA-1                                ? WM_TABLE_ENDED_SESSION_E    :

            tableType == CPSS_DXCH_TABLE_FDB_E                  ? WM_TABLE_FDB_E :
            tableType == CPSS_DXCH_SIP6_TABLE_EXACT_MATCH_E     ? WM_TABLE_EM_E :
            tableType == CPSS_DXCH_SIP5_TABLE_LPM_MEM_E         ? WM_TABLE_LPM_E :
            tableType == CPSS_DXCH2_TABLE_TUNNEL_START_CONFIG_E ? WM_TABLE_TS_E :
            0;/* unknown */
    GT_U32  wmSize = tableSize / _1K;
    GT_U32  regAddr,value;

    if(wmTableType == 0)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,"unknown table type [%d]",tableType);
    }

    value = wmTableType     | /* 8 bits  : up to 256 different table types  */
            wmSize    <<  8 | /* 17 bits : for table size (times 1K)        */
            numOfSbms << 25;  /* 7 bits  : for up to 128 SBMs               */

    regAddr = SHM_ADDR_REG_FOR_WM_TABLE_SIZE +
        (PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->SHM.shm_engines_size_mode & ~ 0xFFFF);
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesEm)
    {
        ...
        rc = prvCpssDxChCfgEmSizeSet(devNum, CPSS_DXCH_EXACT_MATCH_UNIT_0_E, emSize);
        if(rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    else
    {
        ...
        if(CPSS_DEV_IS_WM_NATIVE_OR_ASIM_MAC(devNum))
        {
            ...
            wmOnly_sharedTablesSizeSet(devNum,CPSS_DXCH_TABLE_FDB_E             ,fdbSize  ,fdbNumSmb);
            wmOnly_sharedTablesSizeSet(devNum,CPSS_DXCH_SIP6_TABLE_EXACT_MATCH_E,emSize   ,emNumSmb);
            wmOnly_sharedTablesSizeSet(devNum,CPSS_DXCH_SIP5_TABLE_LPM_MEM_E    ,lpmSize  ,lpmNumSmb);
            wmOnly_sharedTablesSizeSet(devNum,CPSS_DXCH2_TABLE_TUNNEL_START_CONFIG_E,arpSize/8,arpNumSmb);/*8 ARPs in TS (ipv6) entry*/
...

        {
            for(n = 0 ; n < maxSbmNum ; n++) {
                if(getRepresentativeClient(clientTypeArr[n][0]) == SBM_CLIENT_TYPE__LAST__E)
                {
                    ...
                    CPSS_LOG_INFORMATION_MAC("SMB[%d] : is not used",
                        n);
                    didError++;
                }
            }
            ...
            if(didError)
            {
                CPSS_LOG_INFORMATION_MAC("got [%d] SMBs (out of[%d]) that are not bound to any client \n",
                    didError,maxSbmNum);
            }
        }
    }
    ...
    else
    {
        ...
        rc = prvCpssDxChCfgEmMhtSet(devNum,CPSS_DXCH_EXACT_MATCH_UNIT_0_E,value);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum)) /* AAS */
    {
        /* dummy */
        fdbNumSmb = 16; /*128K*/
        emNumSmb  = 4; /* 32K*/
    ...
    if(!PRV_CPSS_SIP_6_10_CHECK_MAC(devNum)) /* Falcon */
    {
        switch(sharedTableMode)
        {
            ...
            case CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_HIGH_L3_MID_LOW_L2_NO_EM_MAX_ARP_E:
                    ...
                    arpNumSmb = 8;
                    break;
            case CPSS_DXCH_CFG_SHARED_TABLE_MODE_FALCON_64K_L2_32K_EM_96K_ARP_E:
                    fdbNumSmb = 8;
                    emNumSmb  = 4;
                    lpmNumSmb = 20;
                    arpNumSmb = 4;
        ...
        else
        {
            switch(sharedTableMode)
            {
                ...
                case CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MAX_L2_NO_EM_E:
                    if(!isAC5PsmallScale)
                    {
                        if(PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.fdb == _128K)
                        {
                            fdbNumSmb = 16;
                        }
                        else
                ...
                case CPSS_DXCH_CFG_SHARED_TABLE_MODE_AC5P_32K_L2_256K_EM_64K_ARP_E:
                    switch(devType)
                    {
                        case CPSS_AC5P_XL_DEVICES_CASES_MAC:
                            fdbNumSmb = 4;
    ...
    for (smbBlock = 0; (smbBlock < numOfSmbBlocks); smbBlock++)
    {
        ...
        for (smbBlockPort = 0; (smbBlockPort < 2); smbBlockPort++)
        {
            ...
            case CPSS_DXCH2_TABLE_TUNNEL_START_CONFIG_E:
                ...
                {
                    cpssOsPrintf("TUNNEL_START");/*the TS is not with the ARP */
                }
                else
                {
                    cpssOsPrintf("ARP/TUNNEL_START");
                }
            ...
            case CPSS_DXCH_SIP7_TABLE_HA_ARP_E:
                cpssOsPrintf("ARP");/*Sip7 only*/
                break;
...
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 18 */
/* test case 97  */ ,{4      ,  0     ,8         , 20    /    1     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MAX_L2_NO_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 22 */
/* test case 98  */ ,{16     ,  0     ,12         , 8    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_L3_MID_L2_MIN_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 25 */
/* test case 99  */ ,{8     ,  4     ,12         , 16    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_L3_MIN_L2_MAX_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 10 */
/* test case 100 */ ,{4     ,  16     ,4         , 16    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_LOW_L3_MID_LOW_L2_MAX_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 21 */
/* test case 101 */ ,{4     ,  16     ,8         , 8    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_L3_MID_L2_NO_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 24 */
/* test case 102 */ ,{8     ,  0     ,12         , 24    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_L3_MID_LOW_L2_MID_EM_MAX_ARP_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 29 */
/* test case 103 */ ,{4     ,  8     ,12         , 16    /    2     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MID_L2_MAX_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 17 */
/* test case 104 */ ,{8     ,  16     ,8         , 0    /    1     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_LOW_MAX_L3_MID_LOW_L2_NO_EM_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 18 */
/* test case 105 */ ,{4     ,  0     ,8         , 20    /    1     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MID_HIGH_L3_MID_LOW_L2_NO_EM_MAX_ARP_E */
/* AC5P,Aldrin3,Aldrin3-XL,Armstro2S - Profile Index 27 */
/* test case 106 */ ,{4     ,  0     ,12         , 16    /    1     /*   32    */  }

/* CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MAX_L2_NO_EM_E with 128K FDB */
/* AC5P XL - Profile Index 14. Device ID = CPSS_98DX4570_CNS */
/* test case 107 */ ,{16      ,  0     ,8         , 0    /    1     /*   24    */  }
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.parametericTables.numEntriesEm)
    {
        /* update the <emSize> (involve updating other SW parameters and update the HW)*/
        /* will update : PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.emNum      */
        rc = prvCpssDxChCfgEmSizeSet(devNum, CPSS_DXCH_EXACT_MATCH_UNIT_0_E,
                                     PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.emNum);
        if (rc != GT_OK)
        {
            return rc;
    ...
    {
        /* will update HW and DB : PRV_CPSS_DXCH_PP_MAC(devNum)->exactMatchInfo.exactMatchNumOfBanks  */
        rc = prvCpssDxChCfgEmMhtSet(devNum ,CPSS_DXCH_EXACT_MATCH_UNIT_0_E/* EM unit */,16/* max MHT */);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            /* disable the auto-aging before the FDB-table going to be connected
               to actual memory in the SHT , so no FDB transactions during the connection.
               application can set auto aging at any stage later
            */
            ...
            /*AAS*/
            rc = prvCpssDxChSharedMemoryAasConnectClientsToMemory(devNum,
                &ppConfigPtr->sharedTablesSizeInfo);
            if (rc != GT_OK)
            {
                return rc;
            }

            /* the IP still not aligned to the LPM options of AAS , so let it use sharedTableMode */
            moduleCfgPtr->ip.sharedTableMode = ppConfigPtr->sharedTableMode;

            /* init Policer's DB and default settings */
            rc = prvCpssDxChPolicerInit(devNum);
            if (rc != GT_OK)
            {
                CPSS_LOG_INFORMATION_MAC("prvCpssDxChPolicerInit(..) : return code is %d", rc);
                return rc;
            }
        }
        else  /*Falcon/Hawk*/
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->gdmaInfo.supportGdmaFor_AUQ)
    {
        /* init the AUQ , FUQ */
        rc = prvCpssDxChGdmaFdbMsgInit(devNum,
            256/*maxNum64BMsgPerAuq*/,
            256/*maxNum32BMsgPerFuq*/);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    else
    {
        /* as the function internalCfgAddrUpInit(...) may do alignment on some of
           ...
           'auMessageNumBytes'
        */
        alignPointersAndNumberOfElements(
            devNum,
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        rc = prvCpssDxChCfgCalculatePbrIndexAndBankSizeSip7(devNum, &indexForPbr, &pbrInitBankSize);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }
        if (ppConfigPtr->maxNumOfPbrEntries > 0)
        {
            ...
            rc = prvCpssHwPpSetRegField(devNum,
                                        PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->LPM.directAccessMode,
                                        0, 24,indexForPbr/PRV_CPSS_DXCH_LPM_RAM_SIP7_SIZE_OF_LPM_ENTRY_IN_BYTES_CNS);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
            }
            /* set pbr bank size */
            rc = prvCpssDxChLpmPbrBankSizeSetSip7(devNum,pbrInitBankSize);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
            }
        ...
        /* Set head of the trie as the first line of the LPM bank */
        if (moduleCfgPtr->ip.sharedTableMode == CPSS_DXCH_CFG_SHARED_TABLE_MODE_MIN_L3_MAX_L2_NO_EM_E)
        {
            /* All small banks. No big banks present */
            bankOffset = smallBankFirstPossiblePosition + moduleCfgPtr->ip.numOfSmallPbrBanks;
        }
        else
        {
            /* First big bank */
            bankOffset = moduleCfgPtr->ip.numOfBigPbrBanks;
        }


        headOfTrie = bankOffset * PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.totalNumOfLinesInBlockIncludingGap;/*PRV_CPSS_DXCH_LPM_RAM_TOTAL_BLOCKS_SIZE_INCLUDING_GAP_FALCON_CNS*/

        rc = prvCpssDxChLpmDefaultHeadOfTrieLineWriteSip7(devNum,
                                              PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.totalNumOfLinesInBlockIncludingGap,
                                              headOfTrie);
        if (rc != GT_OK)
        {
        ...
        {
            /* Initialize VRF Entry tables for IPV4 and IPV6 protocols */
            for (index = 0; index < PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.vrfIdNum; index++)
            {
    ...
    else
    {
        if (PRV_CPSS_SIP_6_CHECK_MAC(devNum) == GT_TRUE)
        {
            ...
            if (ppConfigPtr->maxNumOfPbrEntries > 0)
            {
                ...
                /* set pbr bank size */
                rc = prvCpssDxChLpmPbrBankSizeSet(devNum,pbrInitBankSize);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
                }
            ...
            rc = prvCpssDxChLpmDefaultHeadOfTrieLineWrite(devNum,
                                                          PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.totalNumOfLinesInBlockIncludingGap,
                                                          headOfTrie);
            if (rc != GT_OK)
            {
            ...
            for (index = 0; index < PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.vrfIdNum; index++)
            {
                rc = prvCpssDxChLpmHwVrfEntryWrite(devNum,
                                                       PRV_CPSS_DXCH_LPM_RAM_SIP6_SHADOW_E,
                                                       index,
                                                       PRV_CPSS_DXCH_LPM_PROTOCOL_IPV4_E,
                                                       CPSS_DXCH_LPM_COMPRESSED_NODE_PTR_TYPE_E,
                                                       headOfTrie);
...
)
{
            ...
            case CPSS_DXCH_CFG_TABLE_FDB_E:
                if(PRV_CPSS_DXCH_PP_MAC(devNum)->moduleCfg.wasDone_cpssDxChCfgPpLogicalInit == GT_FALSE)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED,
                        "SIP7: the Shared Tables [FDB,EM,ARP,TS,LPM,PLR-METER,PLR-COUNTER,HF0,HF1,CNC,IPE0,IPE1] sizes must be first defined by cpssDxChCfgPpLogicalInit (see info in sharedTablesSizeInfo)");
                }
                break;
            default:
                break;
    ...
    switch(table)
    {
        ...
        case CPSS_DXCH_CFG_TABLE_VIDX_E:
            ...
            if(PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum))
            {
                ...
                if(isPE == GT_TRUE)
                {
                    ...
                    numEntries =
                        PRV_CPSS_ACCESS_TABLE_INFO_PTR(devNum)[CPSS_DXCH_TABLE_MULTICAST_E].maxNumOfEntries;
                }
            }
            break;
        case CPSS_DXCH_CFG_TABLE_ARP_E:
        ...
        case CPSS_DXCH_CFG_TABLE_MAC_TO_ME_E:
            ...
            break;
        case CPSS_DXCH_CFG_TABLE_IP_TO_ME_E:
            numEntries = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tti.TTI_ip2me_numEntries;
            break;
        case CPSS_DXCH_CFG_TABLE_CNC_E:
...
)
{
    ...
    rc = prvCpssSip6PdqRenumber(oldDevNum,newDevNum);
    if(rc!=GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc,LOG_ERROR_NO_MSG);
    }
    ...
    cpssOsMemCpy(&PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[newDevNum]),
        &PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[oldDevNum]),sizeof(PRV_CPSS_NON_SHARED_DEVICE_SPECIFIC_DATA));

    cpssOsMemSet(&PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[oldDevNum]),0,sizeof(PRV_CPSS_NON_SHARED_DEVICE_SPECIFIC_DATA));
        ...
        || (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(newDevNum)))
    {
        ...
        /* update port manager with new devNum */
        prvCpssPortManagerRenumber(oldDevNum,newDevNum);    
    }
...
)
{
    ...
    GT_U32      i;
    GT_U32      numWords;
    PRV_CPSS_RESET_AND_INIT_CTRL_REGS_ADDR_STC *ctrlRegsAddrPtr;
    ...
    ctrlRegsAddrPtr = PRV_CPSS_DEV_RESET_AND_INIT_CONTROLLER_REGS_MAC(devNum);
    ...
    cpssOsMemSet(
        &(devInfoPtr->genDevInfo.unitLevelTraceabilityArr[0]),
        0, sizeof(devInfoPtr->genDevInfo.unitLevelTraceabilityArr));
    ...
    for (i = 0; (i < numWords); i++)
    {
        ...
        {
            regAddr = ctrlRegsAddrPtr->DFXServerUnits.DFXServerRegs.id0eFuseSlaveChipIdArr[i];
        }
        else
        {
            regAddr = ctrlRegsAddrPtr->sip6_tile_DFXServerUnits[0].DFXServerRegs.id0eFuseSlaveChipIdArr[i];
        }
        rc = prvCpssDrvHwPpResetAndInitControllerReadReg(
            devNum, regAddr, &(devInfoPtr->genDevInfo.unitLevelTraceabilityArr[i]));
        if (rc != GT_OK)
        {
            return rc;
        }
...
{
    if(bind)
        cpssOsMemCpy( &PRV_NON_SHARED_REG_GLOBAL_TABLE_ACCESS_OBJECT, cfgAccessObjPtr, sizeof(CPSS_DXCH_CFG_HW_ACCESS_OBJ_STC));
    else
        cpssOsMemSet( &PRV_NON_SHARED_REG_GLOBAL_TABLE_ACCESS_OBJECT, 0, sizeof(CPSS_DXCH_CFG_HW_ACCESS_OBJ_STC));
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E | CPSS_CAELUM_E);
...
/**
* @internal internal_cpssDxChCfgAuqFuqDisable function
* @endinternal
*
* @brief  Disables AUQ/FUQ and CNC upload messages.
*
* @note   APPLICABLE DEVICES: Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
...
*
**/
static GT_STATUS internal_cpssDxChCfgAuqFuqDisable
(
    IN GT_U8   devNum
)
{
    ...
    {
        /* Stop the AUQ - allow it to be set with new AUQ/FUQ parameters */
        rc = prvCpssDxChCfgInitAuqEnableDisable(devNum, portGroupId,
                                                GT_FALSE /*disable*/, &isAuqDisabled);
        if (rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal cpssDxChCfgAuqFuqDisable function
* @endinternal
*
* @brief  Disables AUQ/FUQ and CNC upload messages.
*
...
*
**/
GT_STATUS cpssDxChCfgAuqFuqDisable
(
    IN GT_U8   devNum
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChCfgAuqFuqDisable);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum));

    rc = internal_cpssDxChCfgAuqFuqDisable(devNum);
...
/**
* @internal internal_cpssDxChCfgServiceCpuDisable function
* @endinternal
*
* @brief  Disables service CPU processor
*
...
*
**/
static GT_STATUS internal_cpssDxChCfgServiceCpuDisable
(
    IN GT_U8 devNum
)
{
    ...
    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        numMgs = PRV_CPSS_DXCH_PP_HW_INFO_MG_MAC(devNum).sip6MgNumOfUnits;
        if(numMgs == 0)/* sip7 have no MG(s) */
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_IMPLEMENTED,
                "not implemented for SIP7 device(s)");
        }

        /* SIP6 devices */
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->MG.confiProcessor.serviceCpuGlobalConfiguration;
    }
    else
    {
        /* SIP5 devices */
        if (PRV_CPSS_SIP_5_CHECK_MAC(devNum))
        {
            return prvCpssHwPpMgSetRegField(devNum, 0 /* single MG[0] */, 0x500, 28, 2, 0);
        }
        else
        {
            /* AC5 */
            regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->globalRegs.coprocessor.serviceCpuGlobalConfiguration;
        }
    }

    for(mgIndex = 0; mgIndex < numMgs; mgIndex++ /* CM3 is in each forth MG */)
    {
        mgOffset = (mgIndex == 0) ? 0 : prvCpssSip6OffsetFromFirstInstanceGet(devNum, mgIndex, PRV_CPSS_DXCH_UNIT_MG_E);

        rc = prvCpssHwPpSetRegField(devNum, regAddr + mgOffset, 28, 2, 0);
        if (rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal cpssDxChCfgServiceCpuDisable function
* @endinternal
*
* @brief  Disables service CPU processor
*
...
*
**/
GT_STATUS cpssDxChCfgServiceCpuDisable
(
    IN GT_U8 devNum
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChCfgServiceCpuDisable);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum));

    rc = internal_cpssDxChCfgServiceCpuDisable(devNum);
