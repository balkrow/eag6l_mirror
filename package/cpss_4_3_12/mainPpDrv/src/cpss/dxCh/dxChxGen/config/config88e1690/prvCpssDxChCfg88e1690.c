...
)
{
    ...
    {
        cfg.BSPFunctions.readMii   = PRV_CALLBACK_ID_GET(gtBspReadMii);
        cfg.BSPFunctions.writeMii  = PRV_CALLBACK_ID_GET(gtBspWriteMii);
    ...
    /* we need to generate single protection for synchronization of
       ...
       since the ISR can't use semaphores , it is protected by :
        ...
        NOTE: all the 88e1690 devices that are on current 'DX device' must use
              the same 'lock'.
    */
    devDrvInstancePtr->globalProtection = PRV_CALLBACK_ID_GET(drv88e1690Protection);
...
GT_VOID * prvCpssDxChBspCallbacksBind
(
    GT_VOID
)
{
   ...
   static GT_STATUS (* const gtBspFunctionDb[])(GT_VOID *) =
   {
      (FUNCP_FGT_CPSS_GENERAL_ACTION_MII)gtBspReadMii,
      (FUNCP_FGT_CPSS_GENERAL_ACTION_MII)gtBspWriteMii,
      (FUNCP_FGT_CPSS_GENERAL_ACTION_MII)drv88e1690Protection,
      NULL
   };

  return (GT_VOID *)gtBspFunctionDb;
}
