...
{
    FUNCP_FGT_CPSS_GLOBAL_PROTECTION   globalProtection;

    if(dev->globalProtection)
    {
        globalProtection = PRV_CALLBACK_GET(dev->globalProtection,FUNCP_FGT_CPSS_GLOBAL_PROTECTION);
        globalProtection(dev,startProtection,cookiePtr);
    }
}
...
)
{
    GT_U16 smiReg;
    unsigned int timeOut;
    FUNCP_FGT_CPSS_READ_MII fgtReadMii;
    FUNCP_FGT_CPSS_WRITE_MII fgtWriteMii;
    ...
    timeOut = PRV_CPSS_QD_SMI_ACCESS_LOOP; /* initialize the loop count */
    fgtReadMii = PRV_CALLBACK_GET(dev->fgtReadMii,FUNCP_FGT_CPSS_READ_MII);
    fgtWriteMii  = PRV_CALLBACK_GET(dev->fgtWriteMii,FUNCP_FGT_CPSS_WRITE_MII);
    do
    {
        if((fgtReadMii(dev,dev->phyAddr,PRV_CPSS_QD_REG_SMI_COMMAND, &smiReg,context)) != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }
        if(timeOut-- < 1 )
    ...
    if((fgtWriteMii(dev,dev->phyAddr,PRV_CPSS_QD_REG_SMI_COMMAND, smiReg,context)) != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    }
    ...
    if((fgtReadMii(dev,dev->phyAddr,PRV_CPSS_QD_REG_SMI_DATA, &smiReg,context)) != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    }
    *value = smiReg;
...
)
{
    ...
    if((fgtWriteMii(dev,dev->phyAddr,PRV_CPSS_QD_REG_SMI_DATA, value,context)) != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    }
    smiReg = QD_SMI_BUSY | (phyAddr << QD_SMI_DEV_ADDR_BIT) | (QD_SMI_WRITE << QD_SMI_OP_BIT) |
