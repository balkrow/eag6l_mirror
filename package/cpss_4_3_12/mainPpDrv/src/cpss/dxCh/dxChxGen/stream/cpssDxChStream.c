...
)
{
    ...
    numOfBits = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 24 : 23;

    /* 23 bits in HW for <Restart Period> */
    if( ((BIT_MASK(numOfBits)) < infoPtr->restartPeriodTimeInMicroSec) ||
        (((BIT_MASK(numOfBits)) / PRV_CPSS_PP_MAC(devNum)->coreClock) < infoPtr->restartPeriodTimeInMicroSec))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
            ...
            ((BIT_MASK(numOfBits)) / PRV_CPSS_PP_MAC(devNum)->coreClock));
    }

    numOfBits = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 27 : 26;
    /* 26 bits in HW for <Latent Error Period> */
    if( ((BIT_MASK(numOfBits)) < infoPtr->latentErrorDetectionPeriodTimeInMicroSec) ||
        (((BIT_MASK(numOfBits)) / PRV_CPSS_PP_MAC(devNum)->coreClock) < infoPtr->latentErrorDetectionPeriodTimeInMicroSec))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
    ...
    */
    ...
    /* <Restart Period> */
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 32, hwValue);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    regAddr = PRV_DXCH_REG1_UNIT_PREQ_MAC(devNum).FRE.RestartPeriod;
    /* <Restart Period> */
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, numOfBits, &hwValue);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    if(sngIrfInfoPtr)
    {
        ...
        SIP6_30_SMU_IRF_SNG_FIELD_GET_MAC(devNum,hwValue,
            SIP6_30_SMU_IRF_SNG_TABLE_FIELDS_IRF_TAKE_ANY_E,
            fieldValue);
        sngIrfInfoPtr->takeAnySeqNum = BIT2BOOL_MAC(fieldValue);

        SIP6_30_SMU_IRF_SNG_FIELD_GET_MAC(devNum,hwValue,
            SIP6_30_SMU_IRF_SNG_TABLE_FIELDS_IRF_INDIVIDUAL_RECOVERY_ENABLED_E,
            fieldValue);
        sngIrfInfoPtr->individualRecoveryEnable = BIT2BOOL_MAC(fieldValue);

        SIP6_30_SMU_IRF_SNG_FIELD_GET_MAC(devNum,hwValue,
            SIP6_30_SMU_IRF_SNG_TABLE_FIELDS_IRF_RECOVERY_SEQUENCE_NUM_E,
            fieldValue);
        sngIrfInfoPtr->recoverySeqNum = fieldValue;
    ...
    if(irfCountersPtr)
    {
        ...
        U_ANY_GET_FIELD_BY_ID_MAC(hwValue,
            ...
            &irfCountersPtr->byteCounter.l[0]);
    }
...
)
{
    ...
    if(srfConfigInfoPtr->leDifference >= BIT_20)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
            "The leDifference[%d] is more than MAX[%d]",
            srfConfigInfoPtr->leDifference,
            BIT_20-1);
    }
    ...
    if(srfConfigInfoPtr->useVectorRecoveryAlgorithm == GT_FALSE)
    {
        ...
        /* ignore non-history fields :
           1. vectorSequenceHistoryLength
           2. firstHistoryBuffer
           3. numOfBuffers

           --> on the 'Get' that API return the 'default' values of those 3 fields
        */
    }
    else
    {
...
)
{
    ...
    GT_U32      fieldValue; /* value read from HW */
    GT_U32      fieldValue1;/* value read from HW (another value)*/
    GT_U32      restartPeriodTimeInMicroSec;/*The restart daemon starts scanning time (in micro seconds)*/
    ...
    if(srfConfigInfoPtr)
    {
        ...
        SIP6_30_PREQ_SRF_DAEMON_FIELD_GET_MAC(devNum,hwValue,
            SIP6_30_PREQ_SRF_DAEMON_TABLE_FIELDS_LE_DIFFERENCE_E,
            fieldValue);
        srfConfigInfoPtr->leDifference = fieldValue;
    ...
    if(srfStatusInfoPtr)
    {
        ...
        /* <Restart Period> */
        rc = prvCpssHwPpGetRegField(devNum,
            PRV_DXCH_REG1_UNIT_PREQ_MAC(devNum).FRE.RestartPeriod,
            0, 23, &fieldValue1);
        if(rc != GT_OK)
        {
            return rc;
        }
        /* like calculated in cpssDxChStreamSrfDaemonInfoGet(...) */
        restartPeriodTimeInMicroSec = fieldValue1 / PRV_CPSS_PP_MAC(devNum)->coreClock;

        if(restartPeriodTimeInMicroSec == 0)
        {
            /* we can't divide by ZERO ... */
            restartPeriodTimeInMicroSec = 1;
        }

        if((0xFFFFFFFF / restartPeriodTimeInMicroSec) > fieldValue)
        {
            srfStatusInfoPtr->untillRestartTimeInMicroSec = fieldValue * restartPeriodTimeInMicroSec;
        }
        else
        {
            /* the multiple value will lost the MSbits , so state the max GT_U32 value */
            srfStatusInfoPtr->untillRestartTimeInMicroSec = 0xFFFFFFFF;
        }
    }
