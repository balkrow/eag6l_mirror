...
const PRV_CPSS_LOG_FUNC_PARAM_STC DX_OUT_GT_U32_ARRAY_maskArr = {
     "maskArr", PRV_CPSS_LOG_PARAM_OUT_E,  PRV_CPSS_LOG_FUNC_TYPE_ARRAY_MAC(GT_U32)
};
const PRV_CPSS_LOG_FUNC_PARAM_STC DX_OUT_GT_U32_ARRAY_patternArr = {
     "patternArr", PRV_CPSS_LOG_PARAM_OUT_E,  PRV_CPSS_LOG_FUNC_TYPE_ARRAY_MAC(GT_U32)
};
const PRV_CPSS_LOG_FUNC_PARAM_STC DX_OUT_GT_U32_ARRAY_actionArr = {
     "actionArr", PRV_CPSS_LOG_PARAM_OUT_E,  prvCpssLogParamFunc_GT_U32_ARRAY_sizeSet1
};

const PRV_CPSS_LOG_FUNC_PARAM_STC * const cpssDxChPclRuleGet_PARAMS[] =  {
    &DX_IN_GT_U8_devNum,
    &DX_IN_GT_U32_tcamIndex,
    &DX_IN_CPSS_PCL_RULE_SIZE_ENT_ruleSize,
    &DX_IN_GT_U32_ruleIndex,
    &DX_OUT_GT_U32_ARRAY_maskArr,
    &DX_OUT_GT_U32_ARRAY_patternArr,
    &DX_OUT_GT_U32_ARRAY_actionArr
};
const PRV_CPSS_LOG_FUNC_PARAM_STC * const cpssDxChPclPortGroupRuleGet_PARAMS[] =  {
    &DX_IN_GT_U8_devNum,
...
)
{
    ...
    switch (valPtr->redirectCmd)
    {
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
            PRV_CPSS_LOG_STC_NUMBER_MAC((&(&valPtr->data)->genericActionInfo), genericAction);
            break;
        default:
            break;
    }
...
)
{
    ...
    if (inOutParamInfoPtr->paramKey.paramKeyArr[4] == GT_FALSE)
    {
        ...
        PRV_CPSS_LOG_STC_ENUM_MAC((&valPtr->ingress), cfi1, CPSS_DXCH_PCL_ACTION_CFI_UPDATE_ENT);
        PRV_CPSS_LOG_STC_ENUM_MAC((&valPtr->ingress), cfi0, CPSS_DXCH_PCL_ACTION_CFI_UPDATE_ENT);
        PRV_CPSS_LOG_STC_BOOL_MAC((&valPtr->ingress), updateTag1FromUdb);
        PRV_CPSS_LOG_STC_BOOL_MAC((&valPtr->ingress), copyUdb);
        prvCpssLogStcLogEnd(contextLib, logType);
    }
    else
    {
...
)
{
    ...
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, cancelDrop);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, bypassBridge);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, bypassIngressPipe);
    PRV_CPSS_LOG_STC_ENUM_MAC(valPtr, egressPolicy, CPSS_PCL_DIRECTION_ENT);
    inOutParamInfoPtr->paramKey.paramKeyArr[4] = (CPSS_PCL_DIRECTION_ENT)valPtr->egressPolicy;
    PRV_CPSS_LOG_STC_STC_MAC(valPtr, lookupConfig, CPSS_DXCH_PCL_ACTION_LOOKUP_CONFIG_STC);
    PRV_CPSS_LOG_STC_STC_MAC(valPtr, headerAlterationOptions, CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC);
    ...
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, terminateCutThroughMode);
    PRV_CPSS_LOG_STC_STC_MAC(valPtr, latencyMonitor, CPSS_DXCH_PCL_ACTION_LATENCY_MONITORING_STC);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, skipFdbSaLookup);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, triggerInterrupt);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, exactMatchOverPclEn);
    PRV_CPSS_LOG_STC_BOOL_MAC(valPtr, exactMatchDisable);
    PRV_CPSS_LOG_STC_STC_MAC(valPtr, epclPhaInfo, CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_INFO_STC);
    PRV_CPSS_LOG_STC_STC_MAC(valPtr, copyReserved, CPSS_DXCH_PCL_ACTION_COPY_RESERVED_STC);
