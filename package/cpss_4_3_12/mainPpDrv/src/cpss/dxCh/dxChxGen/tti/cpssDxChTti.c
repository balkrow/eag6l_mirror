...
#define TTI_ACTION_TYPE_2_SIZE_CNS      12
...
#define NUM_OF_META_DATA_BYTES_MAC(_devNum) \
    (PRV_CPSS_SIP_7_CHECK_MAC(_devNum))?                                                                                                                    \
        (((PRV_SHARED_TTI_DIR_TTI_SRC_GLOBAL_VAR_GET(ttiMetaDataFieldsFormat)[TTI_META_DATA_FIELDS___LAST_VALUE___E - 3].startBit +                         \
           PRV_SHARED_TTI_DIR_TTI_SRC_GLOBAL_VAR_GET(ttiMetaDataFieldsFormat)[TTI_META_DATA_FIELDS___LAST_VALUE___E - 3].numOfBits) + 7) / 8)               \
       : (PRV_CPSS_SIP_6_30_CHECK_MAC(_devNum))?                                                                                                            \
            (((PRV_SHARED_TTI_DIR_TTI_SRC_GLOBAL_VAR_GET(ttiMetaDataFieldsFormat)[TTI_META_DATA_FIELDS_SIP6_30_LAST_VALUE_E - 2].startBit +                 \
               PRV_SHARED_TTI_DIR_TTI_SRC_GLOBAL_VAR_GET(ttiMetaDataFieldsFormat)[TTI_META_DATA_FIELDS_SIP6_30_LAST_VALUE_E - 2].numOfBits) + 7) / 8)       \
...
#define NUM_OF_MAX_META_DATA_BYTES_MAC(_devNum) \
    PRV_CPSS_SIP_7_CHECK_MAC(_devNum) ? 40 : (PRV_CPSS_SIP_6_30_CHECK_MAC(_devNum) ? 38:((PRV_CPSS_SIP_6_CHECK_MAC(_devNum)) ? 37:32))
...
static const TTI_META_DATA_FIELDS_ENT  legacyKey_common_metadata22_29_fieldsArr[]={
    ...
    SIP7_TTI_META_DATA_FIELDS_EVLAN_15_13_E,
...
)
{
    ...
    if(commonPtr)/*legacy keys only */
    {
        ...
        hwValue = tmpValue & 0xff;/* 8 bits*/
        SET_FIELD_TO_LEGACY_KEY_MAC(hwValue,TTI_LEGACY_KEY_FIELDS_EVLAN_7_0_E);

        hwValue = (tmpValue >> 8)&0x1F;
        SET_FIELD_TO_LEGACY_KEY_MAC(hwValue,TTI_LEGACY_KEY_FIELDS_EVLAN_12_8_E);

         /* sip 7 support up to 64K evlans
           since we do not have a devNum to know is this is a sip7 we set those bits
           for all devices, non sip7 devices will not use those bits (they are defined as reserved)*/
        /* 3 bits */
        hwValue = (tmpValue >> 13)&0x7 ;
        SET_FIELD_TO_LEGACY_KEY_MAC(hwValue,SIP7_TTI_LEGACY_KEY_FIELDS_EVLAN_15_13_E);
...
)
{
    ...
    if(commonPtr)/*legacy keys only */
    {
        ...
        /* sip 7 support up to 64K evlans
           ...
           for all devices, non sip7 devices will not use those bits (they are defined as reserved)*/
        /* 3 bits */
        GET_FIELD_FROM_LEGACY_KEY_MAC(hwValue,SIP7_TTI_LEGACY_KEY_FIELDS_EVLAN_15_13_E);
        tmpValue += hwValue << 13;
...
)
{
    ...
    U32_SET_FIELD_MASKED_MAC(hwFormatArray[3],31,1,logicFormatPtr->trustUp);
...
)
{
    ...
    GT_U32  isVPort;/* sip7 : support 'vPort mode' */
    GT_U32      fieldLength;/* Table field length */
    ...
    if(logicFormatPtr->mirrorToIngressAnalyzerEnable == GT_TRUE)
    {
        ...
        fieldLength = PRV_CPSS_SIP_7_CHECK_MAC(devNum)?  6 : 3;
        if(0 == CHECK_BITS_DATA_RANGE_MAC((GT_32)logicFormatPtr->mirrorToIngressAnalyzerIndex + 1, fieldLength))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
    }
    ...
    case CPSS_DXCH_TTI_REDIRECT_TO_EGRESS_E:
        ...
        case CPSS_INTERFACE_PORT_E:
            ...
            isVPort = PRV_CPSS_DXCH_IS_VPORT(devNum,hwValuesInfoPtr->hwPort);
            if(isVPort)
            {
                /* target is vPort */
                value = 1;
    ...
    case CPSS_DXCH_TTI_REDIRECT_TO_ROUTER_LOOKUP_E:
        ...
        {
            value -= PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.lpm.indexForPbr;
        }
    ...
    if(logicFormatPtr->redirectCommand != CPSS_DXCH_TTI_REDIRECT_TO_EGRESS_E)
    {
        ...
        if(!PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            ...
            if(logicFormatPtr->copyReserved >= copyReservedMask)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE,
                    "copyReserved must be limited to [0..0x%x] but got value[0x%x]",
                    copyReservedMask,logicFormatPtr->copyReserved);
            }
        ...
        {
            /* in sip7 there is single bit that we not modify */
        }
    }
    ...
    if(!PRV_CPSS_SIP_7_CHECK_MAC(devNum)/*sip7 : field not valid*/)
    {
    ...
    isVPort = PRV_CPSS_DXCH_IS_VPORT(devNum,logicFormatPtr->sourceEPort);
    if (isVPort)
    {
        value = BOOL2BIT_MAC(logicFormatPtr->sourceEPortAssignmentEnable) ? 2/*ASSIGN_SRC_VPORT*/ : 0;
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
    ...
    {
        if(PRV_CPSS_DXCH_PP_MAC(devNum)->vportInfo.useVPortMode == GT_TRUE &&
           logicFormatPtr->sourceEPortAssignmentEnable == GT_TRUE)
        {
            /* the limit needed to not collide with : assignSourceVPortProfileEnable == GT_TRUE
                that need to use the same field of SIP5_TTI_ACTION_TABLE_FIELDS_SOURCE_EPORT_E
                this system not need to work with ePorts so , not expected to reassign to new srcePort
            */

            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                "'vPort mode' : not allow sourceEPortAssignmentEnable == GT_TRUE with sourceEPort[%d] that is not in 'vPort range' ",
                    logicFormatPtr->sourceEPort);
        }
        ...
        {
            /* Source interface Assignment Enable */
            value = BOOL2BIT_MAC(logicFormatPtr->sourceEPortAssignmentEnable) ? 1/*ASSIGN_SRC_EPORT*/ : 0;
            TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
                SIP7_TTI_ACTION_TABLE_FIELDS_SOURCE_INTERFACE_ASSIGNMENT_EN_E,
                value);
        }
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->vportInfo.useVPortMode == GT_TRUE &&
        logicFormatPtr->assignSourceVPortProfileEnable == GT_TRUE)
    {
        /* the 'src eport' field hold the 'vPort profile' !!! */
        value = logicFormatPtr->sourceVPortProfileValue;
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
            SIP5_TTI_ACTION_TABLE_FIELDS_SOURCE_EPORT_E,
            value);
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        value = logicFormatPtr->qosModel;
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MODEL_E,
            value);

        value = logicFormatPtr->ttlModel;
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_TTL_MODEL_E,
            value);

        value = logicFormatPtr->qosMappingMode;
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MAPPING_MODE_E,
            value);

        value = BOOL2BIT_MAC(logicFormatPtr->stopMpls);
        TTI_ACTION_FIELD_SET_MAC(devNum,hwFormatArray,
...
)
{
    ...
    if(logicFormatPtr->redirectCommand == CPSS_DXCH_TTI_REDIRECT_TO_EGRESS_E)
    {
        ...
        {
        case CPSS_INTERFACE_PORT_E:
            isVPort = PRV_CPSS_DXCH_IS_VPORT(devNum,logicFormatPtr->egressInterface.devPort.portNum);
            if(isVPort)
            {
                PRV_CPSS_DXCH_VPORT_FIELD_CHECK_MAC(devNum,logicFormatPtr->egressInterface.devPort.portNum);

                hwValues.hwPort = logicFormatPtr->egressInterface.devPort.portNum;
            }
            else
            {
                PRV_CPSS_DXCH_DUAL_HW_DEVICE_AND_PORT_CHECK_MAC(
        ...
        logicFormatPtr->bindToPolicer == GT_TRUE)
    {
        ...
        {
            if (logicFormatPtr->policerIndex >= BIT_17)
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    case 1:
        ...
        {
            logicFormatPtr->egressInterface.vidx = U32_GET_FIELD_MAC(hwFormatArray[0],15,12);
    ...
    logicFormatPtr->trustUp   = U32_GET_FIELD_MAC(hwFormatArray[3],31,1);
...
)
{
    ...
    GT_BOOL   enablePtr;
    GT_U32    leafSwIndex;
    GT_U32  isVPort;/*sip7 support for vPort*/
...
*/
    ...
    case 1:
            ...
            if (value == 0) /* target is not trunk */
            {
                ...
                if(isVPort)
                {
                    TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
                     ...
                     logicFormatPtr->egressInterface.devPort.hwDevNum = CPSS_PARAM_NOT_USED_CNS;
                }
                else
                {
                    TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
        ...
        else /* target is VIDX */
        {
            ...
            logicFormatPtr->egressInterface.vidx = value;
    ...
    case 2:
        ...
        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP5_TTI_ACTION_TABLE_FIELDS_REDIRECT_TO_ROUTER_ROUTER_LTT_INDEX_E,
            value);
        /* translate HW index to SW index */
        rc = prvCpssDxChLpmLeafEntryCalcLpmSwIndexFromHwIndex(devNum,value,&leafSwIndex);
        if(rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "failed to convert to leafIndex");
        }
    ...
    case 5:
        if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            ...
            TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
                SIP6_30_TTI_ACTION_TABLE_FIELDS_GENERIC_ACTION_E,
                value);
            logicFormatPtr->genericAction   = value;
        }
        break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
    ...
    else
    {
        ...
        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_SOURCE_INTERFACE_ASSIGNMENT_EN_E,
            value);
        logicFormatPtr->sourceEPortAssignmentEnable = value != 0 ? GT_TRUE : GT_FALSE;

        isVPort = (value == 2)/*ASSIGN_SRC_VPORT*/ ? 1 : 0;

        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_SOURCE_VPORT_PROFILE_ASSIGNMENT_EN_E,
            value);

        logicFormatPtr->assignSourceVPortProfileEnable = BIT2BOOL_MAC(value);

        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP5_TTI_ACTION_TABLE_FIELDS_SOURCE_EPORT_E,
            value);
        logicFormatPtr->sourceVPortProfileValue = value;
    }
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MODEL_E,
            value);
        logicFormatPtr->qosModel = value;

        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_TTL_MODEL_E,
            value);
        logicFormatPtr->ttlModel = value;

        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MAPPING_MODE_E,
            value);
        logicFormatPtr->qosMappingMode = value;

        TTI_ACTION_FIELD_GET_MAC(devNum,hwFormatArray,
            SIP7_TTI_ACTION_TABLE_FIELDS_STOP_MPLS_E,
            value);
        logicFormatPtr->stopMpls = BIT2BOOL_MAC(value);
    }
...
)
{
    ...
    if (ttiExpandedActionOriginPtr->overwriteExpandedActionEgressInterface==GT_TRUE)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    SIP6_TTI_ACTION_FIELD_SPECIFIC_MAX_VAL_GET_MAC(devNum,
                                                                   SIP5_TTI_ACTION_TABLE_FIELDS_REDIRECT_TO_EGRESS_INTERFACE_EPORT_NUMBER_E,
                                                                   ((ttiExpandedActionOriginPtr->egressInterfaceMask) & 0xFFFF),
                                                                   value);
                    TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                 SIP5_TTI_ACTION_TABLE_FIELDS_REDIRECT_TO_EGRESS_INTERFACE_EPORT_NUMBER_E,
                                 value);
                    ...
                    /* Mask must include the first byte since the isTrunk bit  and the useVidx bit are located
                       ...
                       is no issue, all bytes are taken */
                    if (((ttiExpandedActionOriginPtr->egressInterfaceMask & 0xFFFF) != 0) &&
                        ((ttiExpandedActionOriginPtr->egressInterfaceMask & 0xFFFF) <= value) &&
                        ((ttiExpandedActionOriginPtr->egressInterfaceMask & 0x3F) == 0))
                    {
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
                    }
                    ...
                    if((ttiExpandedActionOriginPtr->egressInterfaceMask == 0) ||
                       (((ttiExpandedActionOriginPtr->egressInterfaceMask) >> 16) & 0xFFFF))
                    {
                        SIP6_TTI_ACTION_FIELD_SPECIFIC_MAX_VAL_GET_MAC(devNum,
                                                                       SIP5_TTI_ACTION_TABLE_FIELDS_REDIRECT_TO_EGRESS_INTERFACE_DEVICE_NUMBER_E,
                                                                       (((ttiExpandedActionOriginPtr->egressInterfaceMask) >> 16) & 0xFFFF),
                    ...
                    {
                        /* Target Device is taken from the Expander action */
                        /* Target port is taken from the Expander action */
                        /* if Redirect_Command="Egress_Interface" need to make sure the redirectCmd */
                        if (ttiExpandedActionOriginPtr->overwriteExpandedActionRedirectCommand==GT_TRUE)
                        {
                            if(logicFormatPtr->redirectCommand != CPSS_DXCH_TTI_REDIRECT_TO_EGRESS_E)
                            {
    ...
    if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        if (ttiExpandedActionOriginPtr->overwriteExpandedActionGenericAction==GT_TRUE)
        {
             SIP6_TTI_ACTION_FIELD_SPECIFIC_MAX_VAL_GET_MAC(devNum, SIP6_30_TTI_ACTION_TABLE_FIELDS_GENERIC_ACTION_E,(ttiExpandedActionOriginPtr->genericActionMask), value);
             TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                         SIP6_30_TTI_ACTION_TABLE_FIELDS_GENERIC_ACTION_E,
                                         value);
        }
    ...
    if (ttiExpandedActionOriginPtr->overwriteExpandedActionSourceEPortAssignmentEnable==GT_TRUE)
    {
        ...
        {
            SIP6_TTI_ACTION_FIELD_MAX_VAL_GET_MAC(devNum, SIP7_TTI_ACTION_TABLE_FIELDS_SOURCE_INTERFACE_ASSIGNMENT_EN_E, value);
            TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                  SIP7_TTI_ACTION_TABLE_FIELDS_SOURCE_INTERFACE_ASSIGNMENT_EN_E,
                                  value);
        }
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        if (ttiExpandedActionOriginPtr->overwriteQosModel==GT_TRUE)
        {
            SIP6_TTI_ACTION_FIELD_MAX_VAL_GET_MAC(devNum, SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MODEL_E, value);
            TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                      SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MODEL_E,
                                      value);
        }
        if (ttiExpandedActionOriginPtr->overwriteTtlModel==GT_TRUE)
        {
            SIP6_TTI_ACTION_FIELD_MAX_VAL_GET_MAC(devNum, SIP7_TTI_ACTION_TABLE_FIELDS_TTL_MODEL_E, value);
            TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                      SIP7_TTI_ACTION_TABLE_FIELDS_TTL_MODEL_E,
                                      value);
        }
        if (ttiExpandedActionOriginPtr->overwriteQosMappingMode==GT_TRUE)
        {
            SIP6_TTI_ACTION_FIELD_MAX_VAL_GET_MAC(devNum, SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MAPPING_MODE_E, value);
            TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                      SIP7_TTI_ACTION_TABLE_FIELDS_QOS_MAPPING_MODE_E,
                                      value);
        }
        if (ttiExpandedActionOriginPtr->overwriteStopMpls==GT_TRUE)
        {
            SIP6_TTI_ACTION_FIELD_MAX_VAL_GET_MAC(devNum, SIP7_TTI_ACTION_TABLE_FIELDS_STOP_MPLS_E, value);
            TTI_ACTION_FIELD_SET_MAC(devNum,exactMatchReducedHwMaskArray,
                                      SIP7_TTI_ACTION_TABLE_FIELDS_STOP_MPLS_E,
                                      value);
        }
    ...
    if (PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        case CPSS_DXCH_TTI_EXCEPTION_MPLS_ILLEGAL_TTL_ERROR_E:
            *offsetPtr = 0;
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                *regAddressPtr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuMplsExceptionCmds;
            }
            else
            {
        ...
        case CPSS_DXCH_TTI_EXCEPTION_ILLEGAL_TUNNEL_LENGTH_ERROR_E:
            if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                *regAddressPtr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).generalRegs.ttiLuGlobalConfig;
                *offsetPtr = 21;
            }
...
)
{
    ...
    case CPSS_TUNNEL_ETHERTYPE_TYPE_MPLS_MULTICAST_E:
        ...
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            regAddr = PRV_DXCH_REG1_UNIT_HA_MAC(devNum).MPLSEtherType[0];
        }
        else
        {
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        if (valuePtr->vlanId >= (PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? BIT_16 : BIT_13))
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        hwMacToMeArray[0] = (GT_HW_MAC_LOW16(&(valuePtr->mac)) << 16) | (valuePtr->vlanId & (PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 0xffff : 0x1fff));
        hwMacToMeArray[1] =  GT_HW_MAC_HIGH32(&(valuePtr->mac));
        hwMacToMeArray[2] = (GT_HW_MAC_LOW16(&(maskPtr->mac)) << 16)  | (maskPtr->vlanId & (PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 0xffff : 0x1fff));
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        valuePtr->vlanId = (GT_U16)U32_GET_FIELD_MAC(hwMacToMeArray[0], 0, PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : 13);
        maskPtr->vlanId  = (GT_U16)U32_GET_FIELD_MAC(hwMacToMeArray[2], 0, PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : 13);
...
)
{
    ...
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        regAddr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuTtSrcIdAssignment[lookup];
        return prvCpssHwPpSetRegField(devNum, regAddr, 0, 12, overrideBitmap);
    }
...
)
{
    ...
    {
        regAddr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuTtSrcIdAssignment[lookup];
        return prvCpssHwPpGetRegField(devNum, regAddr,0 , 12, overrideBitmapPtr);
    }
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        regAddr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuTunnelHeaderLenProfile[profileIdx];
    }
    else
    {
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, profileIdx, tunnelHeaderLengthProfilePtr));

    rc = internal_cpssDxChTtiTunnelHeaderLengthProfileSet(devNum, profileIdx, tunnelHeaderLengthProfilePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, profileIdx, tunnelHeaderLengthProfilePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
*                                      (APPLICABLE RANGES: Ironman: 0..7, AAS: 0..255)
...
)
{
    ...
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_IRONMAN_E | CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(valuePtr);
    CPSS_NULL_PTR_CHECK_MAC(interfaceInfoPtr);
    if (entryIndex > PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tti.TTI_ip2me_numEntries)
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    if ((valuePtr->includeVlanId == GT_TRUE) && (valuePtr->vlanId >= (PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? BIT_16 : BIT_13)))
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    ...
    switch (interfaceInfoPtr->includeSrcInterface)
    {
    case CPSS_DXCH_TTI_IP_TO_ME_DONT_USE_SRC_INTERFACE_FIELDS_E:
        U32_SET_FIELD_MASKED_MAC(data,26,2,0);
        break;
    case CPSS_DXCH_TTI_IP_TO_ME_USE_SRC_INTERFACE_FIELDS_E:
        ...
        break;
    case CPSS_DXCH_TTI_IP_TO_ME_USE_SRC_INTERFACE_FIELDS_EXCLUDE_SRC_DEVICE_E:
    ...
    U32_SET_FIELD_MASKED_MAC(data1,8,(PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : 13),valuePtr->vlanId);
    U32_SET_FIELD_MASKED_MAC(data1,(PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 24 : 21),1,BOOL2BIT_MAC(valuePtr->includeVlanId));
...
)
{
    ...
    valuePtr->vlanId = (GT_U16)U32_GET_FIELD_MAC(data,8,(PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 16 : 13));
    valuePtr->includeVlanId = BIT2BOOL_MAC(U32_GET_FIELD_MAC(data,(PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 24 : 21),1));
    ...
    {
    case 0:
        interfaceInfoPtr->includeSrcInterface = CPSS_DXCH_TTI_IP_TO_ME_DONT_USE_SRC_INTERFACE_FIELDS_E;
        break;
    case 1:
        interfaceInfoPtr->includeSrcInterface = CPSS_DXCH_TTI_IP_TO_ME_USE_SRC_INTERFACE_FIELDS_E;
        ...
        break;
    case 2:
        interfaceInfoPtr->includeSrcInterface = CPSS_DXCH_TTI_IP_TO_ME_USE_SRC_INTERFACE_FIELDS_EXCLUDE_SRC_DEVICE_E;
        interfaceInfoPtr->srcIsTrunk = BIT2BOOL_MAC(U32_GET_FIELD_MAC(data,10,1));
        interfaceInfoPtr->srcPortTrunk = U32_GET_FIELD_MAC(data,11,15);
        break;
    default:
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        regAddr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuGenericActionConfig;
    }
    else
    {
...
/**
* @internal internal_cpssDxChTtiGenericRangeSrcPortProfileIdSet function
* @endinternal
*
* @brief   Sets the Generic Range Source Port Profile ID for specific port
*
...
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[in] srcPortProfileId      - src port profile identifier
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeSrcPortProfileIdSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  GT_U32                              srcPortProfileId
)
{
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    ...
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    CPSS_DATA_CHECK_MAX_MAC(srcPortProfileId, BIT_10);
    ...
    return prvCpssDxChWriteTableEntryField(devNum,
                                            ...
                                            portNum,
                                            PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                                            SIP7_TTI_DEFAULT_EPORT_TABLE_FIELDS_GENERIC_RANGE_SOURCE_PORT_PROFILE_E, /* field name */
                                            PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
                                            srcPortProfileId);
}

/**
* @internal cpssDxChTtiGenericRangeSrcPortProfileIdSet function
* @endinternal
*
* @brief   Sets the Generic Range Source Port Profile ID for specific port
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeSrcPortProfileIdSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  GT_U32                              srcPortProfileId
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeSrcPortProfileIdSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, srcPortProfileId));

    rc = internal_cpssDxChTtiGenericRangeSrcPortProfileIdSet(devNum, portNum, srcPortProfileId);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, srcPortProfileId));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericRangeSrcPortProfileIdGet function
* @endinternal
*
* @brief   Gets the Generic Range Source Port Profile ID for specific port
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[out] srcPortProfileIdPtr  - (pointer to) src port profile identifier
*
* @retval GT_OK                      - on success
* @retval GT_BAD_PARAM               - wrong value in any of the parameters
* @retval GT_BAD_PTR                 - NULL pointer parameter
...
* @retval GT_NOT_APPLICABLE_DEVICE   - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeSrcPortProfileIdGet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    OUT GT_U32                              *srcPortProfileIdPtr
)
{
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    CPSS_NULL_PTR_CHECK_MAC(srcPortProfileIdPtr);
...
/**
* @internal cpssDxChTtiGenericRangeSrcPortProfileIdGet function
* @endinternal
*
* @brief   Gets the Generic Range Source Port Profile ID for specific port
*
...
* @retval GT_NOT_APPLICABLE_DEVICE   - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeSrcPortProfileIdGet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    OUT GT_U32                              *srcPortProfileIdPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeSrcPortProfileIdGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, srcPortProfileIdPtr));

    rc = internal_cpssDxChTtiGenericRangeSrcPortProfileIdGet(devNum, portNum, srcPortProfileIdPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, srcPortProfileIdPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericRangeBytePairCfgSet function
* @endinternal
*
* @brief   Set Generic Range Byte Pair Configuration.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum              - device number
* @param[in] bytePairIndex       - byte pair index.(APPLICABLE RANGES: 0..1)
* @param[in] bytePairCfgPtr      - (pointer to) generic range byte pair configuration
*
* @retval GT_OK                    - on success
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeBytePairCfgSet
(
    IN GT_U8                                        devNum,
    IN GT_U32                                       bytePairIndex,
    IN CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC  *bytePairCfgPtr
)
{
    GT_STATUS   rc;
    GT_U32      regAddr;
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    CPSS_PARAM_CHECK_MAX_MAC(bytePairIndex,BIT_1);
    CPSS_NULL_PTR_CHECK_MAC(bytePairCfgPtr);
    CPSS_DATA_CHECK_MAX_MAC(bytePairCfgPtr->byteOffset,BIT_8);

    U32_SET_FIELD_MAC(data, 0, 1, BOOL2BIT_MAC(bytePairCfgPtr->enable));
    switch (bytePairCfgPtr->anchorType)
    {
    case CPSS_DXCH_TTI_OFFSET_L2_E:
        U32_SET_FIELD_MAC(data, 1, 3, UDB_HW_L2_CNS);
        break;
    case CPSS_DXCH_TTI_OFFSET_L3_MINUS_2_E:
        U32_SET_FIELD_MAC(data, 1, 3, UDB_HW_L3_CNS);
        break;
    case CPSS_DXCH_TTI_OFFSET_L4_E:
        U32_SET_FIELD_MAC(data, 1, 3, UDB_HW_L4_CNS);
        break;
    case CPSS_DXCH_TTI_OFFSET_MPLS_MINUS_2_E:
        U32_SET_FIELD_MAC(data, 1, 3, UDB_HW_MPLS_CNS);
        break;
    case CPSS_DXCH_TTI_OFFSET_METADATA_E:
        U32_SET_FIELD_MAC(data, 1, 3, UDB_HW_METADATA_CNS);
        break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    U32_SET_FIELD_MAC(data, 4, 8, bytePairCfgPtr->byteOffset);
    U32_SET_FIELD_MAC(data, 12, 8, bytePairCfgPtr->mask[0]);
    U32_SET_FIELD_MAC(data, 20, 8, bytePairCfgPtr->mask[1]);

    regAddr = (bytePairIndex == 0 ? PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI.ttiRangeCheckGlobalConfig0 : PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI.ttiRangeCheckGlobalConfig1);

    rc = prvCpssHwPpSetRegField(devNum,regAddr,0,28,data);
    return rc;
}

/**
* @internal cpssDxChTtiGenericRangeBytePairCfgSet function
* @endinternal
*
* @brief   Set Generic Range Byte Pair Configuration.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeBytePairCfgSet
(
    IN GT_U8                                        devNum,
    IN GT_U32                                       bytePairIndex,
    IN CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC  *bytePairCfgPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeBytePairCfgSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, bytePairIndex, bytePairCfgPtr));

    rc = internal_cpssDxChTtiGenericRangeBytePairCfgSet(devNum, bytePairIndex, bytePairCfgPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, bytePairIndex, bytePairCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericRangeBytePairCfgGet function
* @endinternal
*
* @brief   Get Generic Range Byte Pair Configuration.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                  - device number
* @param[in] bytePairIndex           - byte pair index.(APPLICABLE RANGES: 0..1)
* @param[out] bytePairCfgPtr         - (pointer to) generic range byte pair configuration
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeBytePairCfgGet
(
    IN  GT_U8                                        devNum,
    IN  GT_U32                                       bytePairIndex,
    OUT CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC  *bytePairCfgPtr
)
{
    GT_STATUS   rc;
    GT_U32      regAddr;
    ...
    bytePairCfgPtr->enable = BIT2BOOL_MAC(U32_GET_FIELD_MAC(data,0,1));

    switch (U32_GET_FIELD_MAC(data,1,3))
    {
     case UDB_HW_L2_CNS:
         bytePairCfgPtr->anchorType = CPSS_DXCH_TTI_OFFSET_L2_E;
         break;
     case UDB_HW_L3_CNS:
         bytePairCfgPtr->anchorType = CPSS_DXCH_TTI_OFFSET_L3_MINUS_2_E;
         break;
     case UDB_HW_L4_CNS:
         bytePairCfgPtr->anchorType = CPSS_DXCH_TTI_OFFSET_L4_E;
         break;
     case UDB_HW_MPLS_CNS:
         bytePairCfgPtr->anchorType = CPSS_DXCH_TTI_OFFSET_MPLS_MINUS_2_E;
         break;
     case UDB_HW_METADATA_CNS:
         bytePairCfgPtr->anchorType = CPSS_DXCH_TTI_OFFSET_METADATA_E;
         break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
    }

    bytePairCfgPtr->byteOffset = U32_GET_FIELD_MAC(data,4,8);
    bytePairCfgPtr->mask[0] = U32_GET_FIELD_MAC(data,12,8);
    bytePairCfgPtr->mask[1] = U32_GET_FIELD_MAC(data,20,8);

    return GT_OK;
}

/**
* @internal cpssDxChTtiGenericRangeBytePairCfgGet function
* @endinternal
*
* @brief   Get Generic Range Byte Pair Configuration.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeBytePairCfgGet
(
    IN  GT_U8                                        devNum,
    IN  GT_U32                                       bytePairIndex,
    OUT CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC  *bytePairCfgPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeBytePairCfgGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, bytePairIndex, bytePairCfgPtr));

    rc = internal_cpssDxChTtiGenericRangeBytePairCfgGet(devNum, bytePairIndex, bytePairCfgPtr);
...
/**
* @internal internal_cpssDxChTtiGenericRangeParamsSet function
* @endinternal
*
* @brief   Sets the Generic Range Classifiction parameters
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                       - device number
* @param[in] rangeClassifictionIndex      - generic range classifiction index.  (APPLICABLE RANGES: 1..127)
* @param[in] rangeClassifictionParamsPtr  - (pointer to) generic range classifiction parameters
*                                           rangeClassifictionParamsPtr.profileMode field; valid values:
*                                           CPSS_DXCH_RANGE_PROFILE_MODE_NOT_USED_E
*                                           CPSS_DXCH_RANGE_PROFILE_MODE_SRC_PORT_PROFILE_E
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeParamsSet
(
    IN GT_U8                                                devNum,
    IN GT_U32                                               rangeClassifictionIndex,
    IN CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC    *rangeClassifictionParamsPtr
)
{
    GT_STATUS   rc;
    GT_U32      regAddr;
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    if ((rangeClassifictionIndex > 127) || (rangeClassifictionIndex < 1))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    CPSS_NULL_PTR_CHECK_MAC(rangeClassifictionParamsPtr);
    CPSS_PARAM_CHECK_MAX_MAC(rangeClassifictionParamsPtr->profileId,BIT_10);
    CPSS_PARAM_CHECK_MAX_MAC(rangeClassifictionParamsPtr->lookupIndex,BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(rangeClassifictionParamsPtr->matchId,BIT_8);
    CPSS_DATA_CHECK_MAX_MAC(rangeClassifictionParamsPtr->maxValue,BIT_16);
    CPSS_DATA_CHECK_MAX_MAC(rangeClassifictionParamsPtr->minValue,BIT_16);

    U32_SET_FIELD_MAC(data, 0, 1, BOOL2BIT_MAC(rangeClassifictionParamsPtr->valid));
    U32_SET_FIELD_MAC(data, 1, 10, rangeClassifictionParamsPtr->profileId);
    switch (rangeClassifictionParamsPtr->profileMode)
    {
    case CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_NOT_USED_E:
        U32_SET_FIELD_MAC(data, 11, 1, 0);
        break;
    case CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_PORT_PROFILE_E:
        U32_SET_FIELD_MAC(data, 11, 1, 1);
        break;
    default:
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    U32_SET_FIELD_MAC(data, 12, 8, rangeClassifictionParamsPtr->matchId);
    U32_SET_FIELD_MAC(data, 20, 1, rangeClassifictionParamsPtr->lookupIndex);

    data |= ((rangeClassifictionParamsPtr->matchId & 0xFF) << 12);
    data |= ((rangeClassifictionParamsPtr->lookupIndex & 1) << 20);

    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuGenericRangesConfig.ttiLuRangeClassification0[rangeClassifictionIndex];
    rc = prvCpssHwPpSetRegField(devNum,regAddr,0,21,data);
    if (rc != GT_OK)
    {
        return rc;
    }

    U32_SET_FIELD_MAC(data, 0, 16, rangeClassifictionParamsPtr->minValue);
    U32_SET_FIELD_MAC(data, 16, 16, rangeClassifictionParamsPtr->maxValue);

    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuGenericRangesConfig.ttiLuRangeClassification1[rangeClassifictionIndex];
    rc = prvCpssHwPpSetRegField(devNum,regAddr,0,32,data);
    return rc;
}

/**
* @internal cpssDxChTtiGenericRangeParamsSet function
* @endinternal
*
* @brief   Sets the Generic Range Classifiction parameters
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeParamsSet
(
    IN GT_U8                                                devNum,
    IN GT_U32                                               rangeClassifictionIndex,
    IN CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC    *rangeClassifictionParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeParamsSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, rangeClassifictionIndex, rangeClassifictionParamsPtr));

    rc = internal_cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, rangeClassifictionParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, rangeClassifictionIndex, rangeClassifictionParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericRangeParamsGet  function
* @endinternal
*
* @brief   Gets the Generic Range Classifiction parameters
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                        - device number
* @param[in] rangeClassifictionIndex       - generic range classifiction index.  (APPLICABLE RANGES: 1..127)
* @param[out] rangeClassifictionParamsPtr  - (pointer to) generic range classifiction parameters
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericRangeParamsGet
(
    IN  GT_U8                                               devNum,
    IN  GT_U32                                              rangeClassifictionIndex,
    OUT CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC   *rangeClassifictionParamsPtr
)
{
    ...
    rangeClassifictionParamsPtr->valid = BIT2BOOL_MAC(U32_GET_FIELD_MAC(data, 0, 1));
    /* if the entry is not valid no need to get all other parameters*/
    if (rangeClassifictionParamsPtr->valid == GT_FALSE)
        return GT_OK;

    rangeClassifictionParamsPtr->profileId = U32_GET_FIELD_MAC(data, 1, 10);
    rangeClassifictionParamsPtr->profileMode = (CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_ENT)U32_GET_FIELD_MAC(data, 11, 1);
    rangeClassifictionParamsPtr->matchId = U32_GET_FIELD_MAC(data, 12, 8);
    rangeClassifictionParamsPtr->lookupIndex = U32_GET_FIELD_MAC(data, 20, 1);
    ...
    rangeClassifictionParamsPtr->minValue = U32_GET_FIELD_MAC(data, 0, 16);
    rangeClassifictionParamsPtr->maxValue = U32_GET_FIELD_MAC(data, 16, 16);

    return GT_OK;
}

/**
* @internal cpssDxChTtiGenericRangeParamsGet  function
* @endinternal
*
* @brief   Gets the Generic Range Classifiction parameters
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericRangeParamsGet
(
    IN  GT_U8                                               devNum,
    IN  GT_U32                                              rangeClassifictionIndex,
    OUT CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC   *rangeClassifictionParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericRangeParamsGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, rangeClassifictionIndex, rangeClassifictionParamsPtr));

    rc = internal_cpssDxChTtiGenericRangeParamsGet(devNum, rangeClassifictionIndex, rangeClassifictionParamsPtr);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationLsbLocationSet function
* @endinternal
*
* @brief   Sets the lsb location in the Generic Classification field
*          of the 10 bits default value.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] lsbLocation           - generic classification field default
*                                    value lsb location
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationLsbLocationSet
(
    IN  GT_U8                               devNum,
    IN  GT_U32                              lsbLocation
)
{
    GT_STATUS rc;
    GT_U32  regAddr;
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    CPSS_DATA_CHECK_MAX_MAC(lsbLocation,BIT_7);

    regAddr = PRV_DXCH_REG1_UNIT_TTI_MAC(devNum).TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt3;
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 3, 7, lsbLocation);
    return rc;
}

/**
* @internal cpssDxChTtiGenericClassificationLsbLocationSet function
* @endinternal
*
* @brief   Sets the lsb location in the Generic Classification field
*          of the 10 bits default value.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationLsbLocationSet
(
    IN  GT_U8                               devNum,
    IN  GT_U32                              lsbLocation
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationLsbLocationSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lsbLocation));

    rc = internal_cpssDxChTtiGenericClassificationLsbLocationSet(devNum, lsbLocation);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, lsbLocation));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationLsbLocationGet function
* @endinternal
*
* @brief   Gets the lsb location in the Generic Classification field
*          of the 10 bits default value.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[out] lsbLocationPtr       -(pointer to) generic classification
*                                    field default value lsb location
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PTR               - NULL pointer parameter
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationLsbLocationGet
(
    IN  GT_U8                               devNum,
    OUT GT_U32                              *lsbLocationPtr
)
{
    GT_U32  regAddr;
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(lsbLocationPtr);

    regAddr = PRV_DXCH_REG1_UNIT_TTI_MAC(devNum).TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt3;
    return prvCpssDrvHwPpGetRegField(devNum, regAddr, 3, 7, lsbLocationPtr);
}

/**
* @internal cpssDxChTtiGenericClassificationLsbLocationGet function
* @endinternal
*
* @brief   Gets the lsb location in the Generic Classification field
*          of the 10 bits default value.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationLsbLocationGet
(
    IN  GT_U8                               devNum,
    OUT GT_U32                              *lsbLocationPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationLsbLocationGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lsbLocationPtr));

    rc = internal_cpssDxChTtiGenericClassificationLsbLocationGet(devNum, lsbLocationPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, lsbLocationPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationDefaultValueSet function
* @endinternal
*
* @brief   Sets the default value assigned into 10 bits of the
*          Generic Classification field for specific port.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[in] defaultValue          - generic classification field default value
*                                    (APPLICABLE RANGES: 0 ... 1023)
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong value in any of the parameters
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationDefaultValueSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  GT_U32                              defaultValue
)
{
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    CPSS_DATA_CHECK_MAX_MAC(defaultValue, BIT_10);
    ...
    return prvCpssDxChWriteTableEntryField(devNum,
                                            ...
                                            portNum,
                                            PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                                            SIP7_TTI_DEFAULT_EPORT_TABLE_FIELDS_GENERIC_CLASSIFICATION_FIELD_DEFAULT_VALUE_E, /* field name */
                                            PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
                                            defaultValue);
}

/**
* @internal cpssDxChTtiGenericClassificationDefaultValueSet function
* @endinternal
*
* @brief   Sets the default value assigned into 10 bits of the
*          Generic Classification field for specific port.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationDefaultValueSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  GT_U32                              defaultValue
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationDefaultValueSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, defaultValue));

    rc = internal_cpssDxChTtiGenericClassificationDefaultValueSet(devNum, portNum, defaultValue);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, defaultValue));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationDefaultValueGet function
* @endinternal
*
* @brief   Gets the default value assigned into 10 bits of the
*          Generic Classification field for specific port.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in] devNum                - device number
* @param[in] portNum               - port number
* @param[out] defaultValuePtr      - (pointer to) generic classification field default value
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - wrong value in any of the parameters
* @retval GT_BAD_PTR               - NULL pointer parameter
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationDefaultValueGet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    OUT GT_U32                              *defaultValuePtr
)
{
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    PRV_CPSS_DXCH_DEFAULT_EPORT_CHECK_MAC(devNum,portNum);
    CPSS_NULL_PTR_CHECK_MAC(defaultValuePtr);
...
/**
* @internal cpssDxChTtiGenericClassificationDefaultValueGet function
* @endinternal
*
* @brief   Gets the default value assigned into 10 bits of the
*          Generic Classification field for specific port.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationDefaultValueGet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    OUT GT_U32                              *defaultValuePtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationDefaultValueGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, defaultValuePtr));

    rc = internal_cpssDxChTtiGenericClassificationDefaultValueGet(devNum, portNum, defaultValuePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portNum, defaultValuePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationParamsSet function
* @endinternal
*
* @brief   Sets the Generic Classifiction parameters
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                    - device number
* @param[in] genClassifictionParamsPtr - (pointer to) generic classifiction parameters
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationParamsSet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC     *genClassifictionParamsPtr
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(genClassifictionParamsPtr);
    CPSS_DATA_CHECK_MAX_MAC(genClassifictionParamsPtr->lsbLocation[0],BIT_5);
    CPSS_DATA_CHECK_MAX_MAC(genClassifictionParamsPtr->lsbLocation[1],BIT_5);
    CPSS_DATA_CHECK_MAX_MAC(genClassifictionParamsPtr->lsbLocation[2],BIT_5);
    CPSS_DATA_CHECK_MAX_MAC(genClassifictionParamsPtr->lsbLocation[3],BIT_5);

    for (i=0; i<MAX_TTI_LU_PARALLEL_LOOKUP_NUM_CNS; i++)
    {
        regAddr = PRV_DXCH_REG1_UNIT_TTI_LU_MAC(devNum).ttiLuActionConfig.ttiLuGenericClassifParallelLu[i];
        U32_SET_FIELD_MAC(data, 0, 8, genClassifictionParamsPtr->mask[i*2]); /* Mask lookup i network order*/
        U32_SET_FIELD_MAC(data, 8, 8, genClassifictionParamsPtr->mask[i*2+1]);
        U32_SET_FIELD_MAC(data, 16, 5, genClassifictionParamsPtr->lsbLocation[i]);
...
/**
* @internal cpssDxChTtiGenericClassificationParamsSet function
* @endinternal
*
* @brief   Sets the Generic Classifiction parameters
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationParamsSet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC     *genClassifictionParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationParamsSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, genClassifictionParamsPtr));

    rc = internal_cpssDxChTtiGenericClassificationParamsSet(devNum, genClassifictionParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, genClassifictionParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiGenericClassificationParamsGet function
* @endinternal
*
* @brief   Gets the Generic Classifiction parameters
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                     - device number
* @param[out] genClassifictionParamsPtr - (pointer to) generic classifiction parameters
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiGenericClassificationParamsGet
(
    IN  GT_U8                                           devNum,
    OUT CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC     *genClassifictionParamsPtr
)
{
    CPSS_TBD_BOOKMARK_AAS
    GT_STATUS rc;
    ...
    for (i=0; i<MAX_TTI_LU_PARALLEL_LOOKUP_NUM_CNS; i++)
    {
        ...
        rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 21, &data);
        if (rc != GT_OK)
        {
            return rc;
        }

        genClassifictionParamsPtr->mask[i*2] = U32_GET_FIELD_MAC(data, 0, 8); /* Mask lookup i network order*/
        genClassifictionParamsPtr->mask[i*2+1] = U32_GET_FIELD_MAC(data, 8, 8);
        genClassifictionParamsPtr->lsbLocation[i] = U32_GET_FIELD_MAC(data, 16, 5);
    }
...
/**
* @internal cpssDxChTtiGenericClassificationParamsGet function
* @endinternal
*
* @brief   Gets the Generic Classifiction parameters
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiGenericClassificationParamsGet
(
    IN  GT_U8                                           devNum,
    OUT CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC     *genClassifictionParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiGenericClassificationParamsGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, genClassifictionParamsPtr));

    rc = internal_cpssDxChTtiGenericClassificationParamsGet(devNum, genClassifictionParamsPtr);
...
/**
* @internal internal_cpssDxChTtiEliExceptionCmdSet function
* @endinternal
*
* @brief   Sets Exception command and CPU/Drop Code for Special Labels(ELI/XL)
*
* @note   APPLICABLE DEVICES:      AAS.
...
*
* @param[in] devNum                - device number
* @param[in] boSExcpCmd            - BoS Exception Command
*                                    (Applicable ranges: 0-4)
* @param[in] boSExcpDropCode       - BoS Exception CPU/Drop Code
*                                    (Applicable ranges: 0-255)
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiEliExceptionCmdSet
(
    IN GT_U8                          devNum,
    IN CPSS_PACKET_CMD_ENT            boSExcpCmd,
    IN CPSS_NET_RX_CPU_CODE_ENT       boSExcpDropCode
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          CPSS_ALDRIN_E | CPSS_BOBCAT3_E | CPSS_AC3X_E | CPSS_ALDRIN2_E | CPSS_FALCON_E |
                                          CPSS_PIPE_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    CPSS_DATA_CHECK_MAX_MAC(boSExcpCmd, CPSS_PACKET_CMD_ROUTE_E);
    CPSS_DATA_CHECK_MAX_MAC(boSExcpDropCode, CPSS_NET_EGRESS_MIRROR_TO_CPU_E+1);

    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI.TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt3;

    value = 0;
    /* BoS Exception Config
     * Exception Command(3b)      bits 21 to 23
     * CPU/Drop Code(8b)          bits 24 to 31
     */
    value |= (boSExcpCmd&0x7);
    value |= (boSExcpDropCode&0xFF)<<3;

    /* Set the updated value to TTI Global Config Ext 3 register */
    rc = prvCpssHwPpSetRegField(devNum,regAddr,21,11,value);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal cpssDxChTtiEliExceptionCmdSet function
* @endinternal
*
* @brief   Sets Exception command and CPU/Drop Code for Special Labels(ELI/XL)
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiEliExceptionCmdSet
(
    IN GT_U8                          devNum,
    IN CPSS_PACKET_CMD_ENT            boSExcpCmd,
    IN CPSS_NET_RX_CPU_CODE_ENT       boSExcpDropCode
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiEliExceptionCmdSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, boSExcpCmd, boSExcpDropCode));

    rc = internal_cpssDxChTtiEliExceptionCmdSet(devNum, boSExcpCmd, boSExcpDropCode);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, boSExcpCmd, boSExcpDropCode));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiEliExceptionCmdGet function
* @endinternal
*
* @brief   Gets Exception command and CPU/Drop Code for Special Labels(ELI/XL)
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in]  devNum                - device number
* @param[out] boSExcpCmdPtr         - (pointer to)BoS Exception Command
*                                     (Applicable ranges: 0-4)
* @param[out] boSExcpDropCodePtr    - (pointer to)BoS Exception CPU/Drop Code
*                                     (Applicable ranges: 0-255)
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiEliExceptionCmdGet
(
    IN  GT_U8                          devNum,
    OUT CPSS_PACKET_CMD_ENT            *boSExcpCmdPtr,
    OUT CPSS_NET_RX_CPU_CODE_ENT       *boSExcpDropCodePtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          ...
                                          CPSS_PIPE_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    CPSS_NULL_PTR_CHECK_MAC(boSExcpCmdPtr);
    CPSS_NULL_PTR_CHECK_MAC(boSExcpDropCodePtr);
    ...
    * CPU/Drop Code(8b)          bits 24 to 31
    */
    /* Get the value from TTI Global Config Ext 3 register */
    rc = prvCpssHwPpGetRegField(devNum,regAddr,21,11,&value);
    if (rc != GT_OK)
    {
        return rc;
    }

    *boSExcpCmdPtr      = (value&0x7);
    *boSExcpDropCodePtr = (value>>3)&0xFF;

    CPSS_DATA_CHECK_MAX_MAC(*boSExcpCmdPtr, CPSS_PACKET_CMD_ROUTE_E);
    CPSS_DATA_CHECK_MAX_MAC(*boSExcpDropCodePtr, CPSS_NET_EGRESS_MIRROR_TO_CPU_E+1);

    return rc;
}

/**
* @internal cpssDxChTtiEliExceptionCmdGet function
* @endinternal
*
* @brief   Gets Exception command and CPU/Drop Code for Special Labels(ELI/XL)
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiEliExceptionCmdGet
(
    IN  GT_U8                          devNum,
    OUT CPSS_PACKET_CMD_ENT            *boSExcpCmdPtr,
    OUT CPSS_NET_RX_CPU_CODE_ENT       *boSExcpDropCodePtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiEliExceptionCmdGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, boSExcpCmdPtr, boSExcpDropCodePtr));

    rc = internal_cpssDxChTtiEliExceptionCmdGet(devNum, boSExcpCmdPtr, boSExcpDropCodePtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, boSExcpCmdPtr, boSExcpDropCodePtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiNonForwardingLabelQosAttributesSet function
* @endinternal
*
* @brief   Sets the QoS Parameters for Non-Forwarding labels
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] labelType             - Non-forwarding Label type
* @param[in] qosParamsPtr          - (pointer to) Qos Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiNonForwardingLabelQosAttributesSet
(
    IN  GT_U8                                              devNum,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_TYPE_ENT        labelType,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC  *qosParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          ...
                                          CPSS_PIPE_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    CPSS_DATA_CHECK_MAX_MAC(labelType, CPSS_DXCH_TTI_EXPLICIT_NULL_LABEL_E+1);
    CPSS_NULL_PTR_CHECK_MAC(qosParamsPtr);
    CPSS_DATA_CHECK_MAX_MAC(qosParamsPtr->qosModel, BIT_2-1);
    CPSS_DATA_CHECK_MAX_MAC(qosParamsPtr->ttlModel, BIT_2-1);
    CPSS_DATA_CHECK_MAX_MAC(qosParamsPtr->qosMappingTableIndex, BIT_7);
    CPSS_DATA_CHECK_MAX_MAC(qosParamsPtr->trustExpFlag, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(qosParamsPtr->trustUpFlag, BIT_2-1);

    if (labelType == CPSS_DXCH_TTI_ELI_LABEL_E)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI.TTIUnitGlobalConfigs.TTIEliQosAttributes;
    }
    else
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI.TTIUnitGlobalConfigs.TTIExplicitNullQosAttributes;
    }

    value = 0;
    /* QoS Action Resolution Fields
     * ELI/Explicit Null QoS Model(3b)      bits 0 to 2
     * ELI/Explicit Null TTL Model(2b)      bits 3 to 4
     * ELI/Explicit mapping Index(7b)       bits 5 to 11
     * ELI/Explicit Trust EXP flag(1b)      bits 12 to 12
     * ELI/Explicit Trust UP flag(2b)       bits 13 to 14
     */
    value |= qosParamsPtr->qosModel;
    value |= ((qosParamsPtr->ttlModel)<<3);
    value |= ((qosParamsPtr->qosMappingTableIndex)<<5);
    value |= ((qosParamsPtr->trustExpFlag)<<12);
    value |= ((qosParamsPtr->trustUpFlag)<<13);
...
/**
* @internal cpssDxChTtiNonForwardingLabelQosAttributesSet function
* @endinternal
*
* @brief   Sets the QoS Parameters for Non-Forwarding labels
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiNonForwardingLabelQosAttributesSet
(
    IN  GT_U8                                              devNum,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_TYPE_ENT        labelType,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC  *qosParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiNonForwardingLabelQosAttributesSet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, labelType, qosParamsPtr));

    rc = internal_cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, qosParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, labelType, qosParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiNonForwardingLabelQosAttributesGet function
* @endinternal
*
* @brief   Gets the QoS Parameters for Non-Forwarding labels
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in]  devNum                - device number
* @param[in]  labelType             - Non-forwarding Label type
* @param[out] qosParamsPtr          - (pointer to) Qos Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiNonForwardingLabelQosAttributesGet
(
    IN  GT_U8                                              devNum,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_TYPE_ENT        labelType,
    OUT  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC *qosParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
     ...
     * ELI/Explicit Trust UP flag(2b)       bits 13 to 14
     */
    qosParamsPtr->qosModel             = value&0x7;
    qosParamsPtr->ttlModel             = (value>>3)&0x3;
    qosParamsPtr->qosMappingTableIndex = (value>>5)&0x7F;
    qosParamsPtr->trustExpFlag         = (value>>12)&0x1;
    qosParamsPtr->trustUpFlag          = (value>>13)&0x3;
...
/**
* @internal cpssDxChTtiNonForwardingLabelQosAttributesGet function
* @endinternal
*
* @brief   Gets the QoS Parameters for Non-Forwarding labels
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiNonForwardingLabelQosAttributesGet
(
    IN  GT_U8                                              devNum,
    IN  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_TYPE_ENT        labelType,
    OUT  CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC *qosParamsPtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiNonForwardingLabelQosAttributesGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, labelType, qosParamsPtr));

    rc = internal_cpssDxChTtiNonForwardingLabelQosAttributesGet(devNum, labelType, qosParamsPtr);
...
/**
* @internal internal_cpssDxChTtiQosActionResolutionTableEntrySet function
* @endinternal
*
* @brief   Sets the QoS Action resolution Parameters for TTI Serial and Parallel lookups
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                        - device number
* @param[in] lookupType                    - TTI lookup type ( Parallel/Serial)
* @param[in] lookupNum                     - TTI lookup number
* @param[in] entryIndex                    - Index to QoS entry in the table
*                                             (Applicable Range : 0-63)
* @param[in] qosActionResolutionParamsPtr  - (pointer to) Qos Action Resolution Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiQosActionResolutionTableEntrySet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
    IN  GT_U32                                          entryIndex,
    IN  CPSS_DXCH_TTI_QOS_ACTION_RESOLUTION_PARAMS_STC  *qosActionResolutionParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          ...
                                          CPSS_PIPE_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    CPSS_DATA_CHECK_MAX_MAC(lookupType, CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E+1);
    CPSS_DATA_CHECK_MAX_MAC(entryIndex, BIT_6);
    CPSS_NULL_PTR_CHECK_MAC(qosActionResolutionParamsPtr);
    CPSS_DATA_CHECK_MAX_MAC(qosActionResolutionParamsPtr->nextQosModelSelectionMode, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(qosActionResolutionParamsPtr->nextQosMappingTableSelectionMode, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(qosActionResolutionParamsPtr->nextQosTrustFieldsSelectionMode, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(qosActionResolutionParamsPtr->nextExp4QosSelectionMode, BIT_2-1);
    CPSS_DATA_CHECK_MAX_MAC(qosActionResolutionParamsPtr->nextExp4EgressSelectionMode, BIT_2-1);

    if (lookupType == CPSS_DXCH_TTI_PARALLEL_LOOKUP_E)
    {
        /* Parallel lookup case */
        /* Check Lookup value between 0..3*/
        CPSS_DATA_CHECK_MAX_MAC(lookupNum, CPSS_DXCH_TTI_LOOKUP_NUM_3_E+1);
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuActionConfig.
                    ttiLuParallelLookupIdxQosActionResolution[lookupNum].qosEntry[entryIndex];
    }
    else
    {
        /* Serial EM lookup case */
        /* Check Lookup value between 0..2*/
        CPSS_DATA_CHECK_MAX_MAC(lookupNum, CPSS_DXCH_TTI_LOOKUP_NUM_3_E);
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuActionConfig.
                    ttiLuIlmLookupIdxQosActionResolution[lookupNum].qosEntry[entryIndex];
    }

    value = 0;
    /* QoS Action Resolution Fields
     * nextQosModelSelectionMode(1b)        bits 0 to 0
     * nextQosMappingTableSelectionMode(1b) bits 1 to 1
     * nextQosTrustFieldsSelectionMode(1b)  bits 2 to 2
     * nextExp4QosSelectionMode(2b)         bits 3 to 4
     * nextExp4EgressSelectionMode(2b)      bits 5 to 6
     */
    value |= qosActionResolutionParamsPtr->nextQosModelSelectionMode;
    value |= ((qosActionResolutionParamsPtr->nextQosMappingTableSelectionMode)<<1);
    value |= ((qosActionResolutionParamsPtr->nextQosTrustFieldsSelectionMode)<<2);
    value |= ((qosActionResolutionParamsPtr->nextExp4QosSelectionMode)<<3);
    value |= ((qosActionResolutionParamsPtr->nextExp4EgressSelectionMode)<<5);
    rc = prvCpssHwPpSetRegField(devNum,regAddr,0,32,value);
    if (rc != GT_OK)
    {
        return rc;
...
/**
* @internal cpssDxChTtiQosActionResolutionTableEntrySet function
* @endinternal
*
* @brief   Sets the QoS Action resolution Parameters for TTI Serial and Parallel lookups
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiQosActionResolutionTableEntrySet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiQosActionResolutionTableEntrySet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lookupType, lookupNum, entryIndex, qosActionResolutionParamsPtr));

    rc = internal_cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                              entryIndex, qosActionResolutionParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, lookupType, lookupNum, entryIndex, qosActionResolutionParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiQosActionResolutionTableEntryGet function
* @endinternal
*
* @brief   Gets the QoS Action resolution Parameters for TTI Serial and Parallel lookups
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in]  entryIndex                    - Index to QoS entry in the table
*                                             (Applicable Range : 0-63)
* @param[out] qosActionResolutionParamsPtr  - (pointer to) Qos Action Resolution Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiQosActionResolutionTableEntryGet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
    IN  GT_U32                                          entryIndex,
    OUT CPSS_DXCH_TTI_QOS_ACTION_RESOLUTION_PARAMS_STC  *qosActionResolutionParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
     ...
     * nextExp4EgressSelectionMode(2b)      bits 5 to 6
     */
    qosActionResolutionParamsPtr->nextQosModelSelectionMode        = (value&0x1);
    qosActionResolutionParamsPtr->nextQosMappingTableSelectionMode = (value>>1)&0x1;
    qosActionResolutionParamsPtr->nextQosTrustFieldsSelectionMode  = (value>>2)&0x1;
    qosActionResolutionParamsPtr->nextExp4QosSelectionMode         = (value>>3)&0x3;
    qosActionResolutionParamsPtr->nextExp4EgressSelectionMode      = (value>>5)&0x3;
...
/**
* @internal cpssDxChTtiQosActionResolutionTableEntryGet function
* @endinternal
*
* @brief   Gets the QoS Action resolution Parameters for TTI Serial and Parallel lookups
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiQosActionResolutionTableEntryGet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiQosActionResolutionTableEntryGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lookupType, lookupNum, entryIndex, qosActionResolutionParamsPtr));

    rc = internal_cpssDxChTtiQosActionResolutionTableEntryGet(devNum, lookupType, lookupNum,
                                                              entryIndex, qosActionResolutionParamsPtr);
...
/**
* @internal internal_cpssDxChTtiTtlActionResolutionTableEntrySet function
* @endinternal
*
* @brief   Sets the TTL Action resolution Parameters for TTI Serial and Parallel lookups
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in] lookupType                    - TTI lookup type ( Parallel/Serial)
* @param[in] lookupNum                     - TTI lookup number
* @param[in] entryIndex                    - Index to TTL entry in the table
*                                            (Applicable Range : 0-15)
* @param[in] ttlActionResolutionParamsPtr  - (pointer to) TTL Action Resolution Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiTtlActionResolutionTableEntrySet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
    IN  GT_U32                                          entryIndex,
    IN  CPSS_DXCH_TTI_TTL_ACTION_RESOLUTION_PARAMS_STC  *ttlActionResolutionParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
    ...
    PRV_CPSS_DXCH_PP_HW_INFO_E_ARCH_SUPPORTED_CHECK_MAC(devNum);
    CPSS_DATA_CHECK_MAX_MAC(lookupType, CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E+1);
    CPSS_DATA_CHECK_MAX_MAC(entryIndex, BIT_4);
    CPSS_NULL_PTR_CHECK_MAC(ttlActionResolutionParamsPtr);
    CPSS_DATA_CHECK_MAX_MAC(ttlActionResolutionParamsPtr->nextTTLModelSelectionMode, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(ttlActionResolutionParamsPtr->nextTTLSelectionMode, BIT_2-1);
    ...
    if (lookupType == CPSS_DXCH_TTI_PARALLEL_LOOKUP_E)
    {
        ...
        CPSS_DATA_CHECK_MAX_MAC(lookupNum, CPSS_DXCH_TTI_LOOKUP_NUM_3_E+1);
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuActionConfig.
                    ttiLuParallelLookupIdxTtlActionResolution[lookupNum].ttlEntry[entryIndex];
    }
    else
    {
        /* Serial EM lookup case */
        ...
        CPSS_DATA_CHECK_MAX_MAC(lookupNum, CPSS_DXCH_TTI_LOOKUP_NUM_3_E);
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TTI_LU.ttiLuActionConfig.
                    ttiLuIlmLookupIdxTtlActionResolution[lookupNum].ttlEntry[entryIndex];
    }

    value = 0;
    /* QoS Action Resolution Fields
     * nextTTLModelSelectionMode(1b)        bits 0 to 0
     * nextTTLSelectionMode(2b)             bits 1 to 2
     */
    value |= ttlActionResolutionParamsPtr->nextTTLModelSelectionMode;
    value |= ((ttlActionResolutionParamsPtr->nextTTLSelectionMode)<<1);
    rc = prvCpssHwPpSetRegField(devNum,regAddr,0,32,value);
    if (rc != GT_OK)
    {
        return rc;
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiTtlActionResolutionTableEntrySet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiTtlActionResolutionTableEntrySet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lookupType, lookupNum, entryIndex, ttlActionResolutionParamsPtr));

    rc = internal_cpssDxChTtiTtlActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                              entryIndex, ttlActionResolutionParamsPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, lookupType, lookupNum, entryIndex, ttlActionResolutionParamsPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTtiTtlActionResolutionTableEntryGet function
* @endinternal
*
* @brief   Gets the TTL Action resolution Parameters for TTI Serial and Parallel lookups
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;
*                                  AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
...
* @param[in]  entryIndex                    - Index to TTL entry in the table
*                                             (Applicable Range : 0-15)
* @param[out] ttlActionResolutionParamsPtr  - (pointer to) TTL Action Resolution Params
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChTtiTtlActionResolutionTableEntryGet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
    IN  GT_U32                                          entryIndex,
    OUT CPSS_DXCH_TTI_TTL_ACTION_RESOLUTION_PARAMS_STC  *ttlActionResolutionParamsPtr
)
{
    GT_U32    regAddr,value;
    GT_STATUS rc;
     ...
     * nextTTLSelectionMode(2b)             bits 1 to 2
     */
    ttlActionResolutionParamsPtr->nextTTLModelSelectionMode = value&0x1;
    ttlActionResolutionParamsPtr->nextTTLSelectionMode      = (value>>1)&0x3;
    CPSS_DATA_CHECK_MAX_MAC(ttlActionResolutionParamsPtr->nextTTLModelSelectionMode, BIT_1);
    CPSS_DATA_CHECK_MAX_MAC(ttlActionResolutionParamsPtr->nextTTLSelectionMode, BIT_2-1);
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChTtiTtlActionResolutionTableEntryGet
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                   lookupType,
    IN  CPSS_DXCH_TTI_LOOKUP_NUM_ENT                    lookupNum,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTtiTtlActionResolutionTableEntryGet);
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lookupType, lookupNum, entryIndex, ttlActionResolutionParamsPtr));

    rc = internal_cpssDxChTtiTtlActionResolutionTableEntryGet(devNum, lookupType, lookupNum,
                                                              entryIndex, ttlActionResolutionParamsPtr);
