...
{
    int rc,data;
    GT_U32 maxPnodes,maxQueues,maxAnodes,maxBnodes,maxCnodes,i;
    GT_U8 devNum;
   ...
   /* Get general parameters : LAD frequency,  dwrr params  - they are readed from H/W resources*/
    ...
    if (rc) goto out;
     ...
     rc = prvSchedLowLevelDisableAlevelEligPrioFunc((PRV_CPSS_SCHED_HANDLE) ctl, 0,maxAnodes);
     if (rc) goto out;
     ...
     rc = prvSchedLowLevelSetDefaultAnodeMapping((PRV_CPSS_SCHED_HANDLE) ctl,0, maxAnodes);
     if (rc) goto out;

     for(i=0;i<maxQueues;i++)
     {
         rc = prvSchedLowLevelSetDefaultQnodeMapping((PRV_CPSS_SCHED_HANDLE) ctl, i);
         if (rc) goto out;
     }
