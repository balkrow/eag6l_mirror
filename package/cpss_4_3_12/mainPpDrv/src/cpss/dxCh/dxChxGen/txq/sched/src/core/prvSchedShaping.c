                                ...
                                uint32_t node_index)
{
  ...
  (
    ...
    {
        int rc;
        uint16_t    node_shaping_profile=0;
        ...
        rc = prvSchedShapingUtilsCalculateNodeShaping(
                                    ...
                                    &cir_token,
                                    &eir_token,hndl,
                                    ctl->level_data[level].divider_res_exp,
                                    &cir_accuracy,&eir_accuracy);
        if (rc)
            goto out;
        ...
        if((PRV_CPSS_SIP_6_30_CHECK_MAC(PDQ_ENV_DEVNUM_NUM(ctl)) == GT_TRUE)&&(SCHED_Q_LEVEL==level))
        {   /*required for credit_accumulation_disable*/
            rc = prvSchedLowLevelGetNodeShapingParametersFromHw(hndl,
                                     SCHED_Q_LEVEL, node_index, &tmp_shp_profile);
            if (rc) goto out;
        }
