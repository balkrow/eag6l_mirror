 ...
 * @param[in] pfcRespBitMapPtr                   -(pointer to)current PFC responce mode
 * @param[in] pfcGenerationModePtr               - (pointer to)current PFC generation mode
 * @param[in] pauseIgnorePtr                     - (pointer to)current pause ignore state at the MAC
 *
...
)
{
    ...
    rc = prvCpssDxChPortSip6PfcPauseIgnoreEnableSet(devNum, portNum, *pauseIgnorePtr);
    if (rc != GT_OK)
    {
 ...
 * @param[out] pauseIgnorePtr                     - (pointer to)current pause ignore state at the MAC
 *
 * @retval GT_OK                    - on success
 * @retval GT_BAD_PARAM             - on wrong port number or device
 * @retval GT_HW_ERROR              - on hardware error
...
(
    IN GT_U8                                  devNum,
    IN GT_PHYSICAL_PORT_NUM                   portNum,
    OUT GT_U32                               *pfcRespBitMapPtr,
    OUT CPSS_DXCH_PORT_FC_MODE_ENT           *pfcGenerationModePtr,
    OUT GT_BOOL                              *pauseIgnorePtr
)
{
    GT_STATUS           rc;
    ...
    CPSS_NULL_PTR_CHECK_MAC(pfcRespBitMapPtr);
    CPSS_NULL_PTR_CHECK_MAC(pfcGenerationModePtr);
    CPSS_NULL_PTR_CHECK_MAC(pauseIgnorePtr);
    ...
    rc = prvCpssDxChPortSip6PfcPauseIgnoreEnableGet(devNum, portNum, pauseIgnorePtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssDxChPortSip6PfcPauseIgnoreEnableSet failed for portNum  %d  ", portNum);
    }
...
* @endinternal
*
* @brief   This function check if PFC is configured to statistic mode.
*          Meaning that trigger bit is off at PFCC and QFC is at FC disabled mode.
*          The function only check that corresponding PFCC entry is "port" and trigger bit is off.
*          QFC mode is not checked here.
*
* @note   APPLICABLE DEVICES:           Falcon; AC5P; AC5X; Harrier; Ironman
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2.
*
...
* @param[in] dpNum         -                 data path[0..7]
* @param[in] localPort                       local port number[0..8]
* @param[out] statisticModePtr               (pointer to) GT_TRUE in case port is in the statistic mode,GT_FALSE otherwise
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - wrong pdx number.
* @retval GT_HW_ERROR              - on writing to HW error.
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssSip6TxqPfcStatisticModeGet
(
    IN GT_U8            devNum,
    IN GT_U32           tileNum,
    IN GT_U32           dpNum,
    IN GT_U32           localPort,
    OUT GT_BOOL         *statisticModePtr
)
{

    GT_STATUS rc;
    CPSS_DXCH_PORT_FC_MODE_ENT qfcMode;
    GT_U32 index;
    PRV_CPSS_PFCC_CFG_ENTRY_STC entry;

    /*Get current configured mode*/
    rc = prvCpssFalconTxqQfcLocalPortToSourcePortGet(devNum,tileNum,dpNum,localPort,NULL,&qfcMode);
    if(rc!=GT_OK)
    {
      CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssFalconTxqQfcLocalPortToSourcePortGet failed for local port %d",localPort);
    }

    index= (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.maxDp)*localPort+dpNum+tileNum*(MAX_DP_IN_TILE(devNum));
    ...
    if((qfcMode== CPSS_DXCH_PORT_FC_MODE_DISABLE_E)&&
        (entry.entryType ==PRV_CPSS_PFCC_CFG_ENTRY_TYPE_PORT )&&
        (entry.pfcMessageTrigger == GT_FALSE))
    {
        *statisticModePtr = GT_TRUE;
    }
