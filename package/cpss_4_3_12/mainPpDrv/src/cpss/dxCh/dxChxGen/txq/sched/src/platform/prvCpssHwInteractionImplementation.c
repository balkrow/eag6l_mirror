...
#define PRV_SIP_6_15_PDQ_FREQ_MB 600
#define PRV_SIP_6_30_PDQ_FREQ_MB 600
#define PRV_SIP_7_PDQ_FREQ_MB 1400
...
int prvSchedGetGenParams(PRV_CPSS_SCHED_HANDLE hndl)
{
    ...
    {
        ctl->freq = PRV_SIP_7_PDQ_FREQ_MB;
    }
    else if(GT_TRUE == PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        ctl->freq = PRV_SIP_6_30_PDQ_FREQ_MB;
    }
    else if(GT_TRUE==isSip6_15)
    {
...
int prvSchedDevNumSet
(
    PRV_CPSS_SCHED_HANDLE hndl,
    GT_U32 newDevNum
)
{
    struct prvCpssDxChTxqSchedShadow * ctl= (struct prvCpssDxChTxqSchedShadow *)hndl;
    struct cpssSchedEnv * env =((struct cpssSchedEnv *)ctl->hEnv);

    CHECK_TM_CTL_PTR(ctl)
    CHECK_HENV_PTR(env,  -EINVAL, -EBADF)

    env->devNumber = newDevNum;
    return 0;
}
