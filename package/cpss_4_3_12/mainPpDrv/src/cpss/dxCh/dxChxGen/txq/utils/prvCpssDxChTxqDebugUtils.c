...
#include <cpss/dxCh/dxChxGen/txq/private/regFiles/seahawk/central/prvCpssAasTxqPdxRegFile.h>
       ...
       switch(_preemptionType)\
        {\
            ...
            case PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_PRE_E:\
                ...
                break;\
            case PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_NONE_E:\
...
#define PRV_TXQ_SIP_7_Q2Q_TABLE_MAX_SIZE_CNS 131072
...
#define PRV_TXQ_FORCE_HQOS_FILE_NAME "/tmp/hqos.dbg"




#define PRV_TXQ_QUEUE_STAT_CNC_BLOCK_FIRST(_devNum)  ((PRV_CPSS_SIP_6_30_CHECK_MAC(_devNum))?4:8)
...
extern GT_STATUS prvCpssDxChTxqDumpNodeShapingParameters
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 nodeStart,
...
)
{
       ...
       rc = prvCpssSip6TxqPdqPerLevelShapingParametersGet(devNum,tileNum,PRV_CPSS_PDQ_LEVEL_Q_E,&shaperDec,&perInter,&perEn);
        ...
        for(i=queueStart; (i<=queueEnd) && (i<CPSS_DXCH_SIP_6_MAX_PDQ_QUEUE_NUM);i++)
        {
            rc = prvCpssFalconTxqPdqMinBwEnableGet(devNum,tileNum,i,&shapingEnabled,NULL);
...
)
{
    ...
    for(i=0;i<8;i++)
    {
     ...
     {   /*only one TC supported at Ironman */
         break;
     }
    }
    cpssOsPrintf("|\n+------+------+------+------+------+------+------+------+\n");
    ...
    if(rc!=GT_OK)
     {
         CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssDxChPortSip6PfcRxPauseStatusGet  failed for portNum  %d  ",portNum);
     }
...
)
{
    ...
    GT_BOOL headRoom;
    GT_U32 expectedEgfPoolId =0;
    GT_U32  tableSize,fieldSize ;
    ...
    {
        tableSize = PRV_SIP_6_EGF_QAG_TC_DP_MAPPER_SIZE;
        fieldSize = 1;
    }
    else
    {
        tableSize = PRV_SIP_7_EGF_QAG_TC_DP_MAPPER_SIZE;
        fieldSize = 2;
    }
    ...
    for(i=0;i<tableSize;i++)
    {
        ...
        poolId = U32_GET_FIELD_MAC(value,9,fieldSize);
        ...
        {
           switch(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode)
           {
        ...
        {
            cpssOsPrintf("FAIL for index %d for Vector 0x%x tc4Pfc %d poolId %d [EGF expected %d]\n",i,pfccTcMapping,tc4Pfc,poolId,expectedEgfPoolId);
...
)
{
    ...
    if(debugMode&PRV_CPSS_TXQ_DEBUG_PORT_SHAPING_CNS)
    {
        ...
        if(GT_FALSE == isCascadePort)
        {
            ...
            /*Shaping*/
            rc = prvCpssDxChTxqDumpNodeShapingParameters(devNum,tileNum,aNodePtr->aNodeIndex,aNodePtr->aNodeIndex,PRV_CPSS_PDQ_LEVEL_A_E);
            if(rc!=GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
...
)
{
    ...
    GT_U32 max;
    PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC *profilePtr;
    GT_CHAR * string[] = {"NA","LP","HP"};

    profilePtr = &PRV_CPSS_DXCH_PP_MAC(devNum)->port.pdsProfiles[first];
    ...
    if(lengthAdjust)
    {
        ...
        {
            profilePtr = &PRV_CPSS_DXCH_PP_MAC(devNum)->port.lengthAdjustProfiles[first];
        }
    }

    max = (last>=PDS_PROFILE_MAX_MAC(devNum))?PDS_PROFILE_MAX_MAC(devNum):(last+1);


    for(i = first;i<max ;i++,profilePtr++)
    {
            cpssOsPrintf(" \n%s Profile %d binded ports (num %d , speed in G : %d , EPB %s): \n",lengthAdjust?"Length adjust":"PDS",i,
                profilePtr->numberOfBindedPorts,
                 profilePtr->speed_in_G,string[profilePtr->spillOver]);
...
)
{
    ...
    for(tileNum=0;tileNum<numberOfTiles;tileNum++)
    {
        for(dpNum=0;dpNum<MAX_DP_IN_TILE(devNum);dpNum++)
        {
            ...
            regAddr = PRV_CPSS_DXCH_PP_MAC(devNum)->regsAddrVer1.TXQ.sdq[dpNum].SDQ_Metal_Fix;
            tileOffset = prvCpssSip6TileOffsetGet(devNum,tileNum/*tileId*/);
            regAddr+=tileOffset;
            rc =prvCpssHwPpReadRegister(devNum,regAddr,&regData);
            if(rc!=GT_OK)
            ...
            cpssOsPrintf("tile %d dp %d SDQ_Metal_Fix RAW 0x%08x \n",tileNum,dpNum,regData);
...
)
{
    ...
    {
        rc = prvCpssDxChNetIfCpuToDsaCode(PRV_TXQ_DBG_DEFAULT_TD_CODE_CNS, &dsaCpuCode);
        if (rc != GT_OK)
        {
            return rc;
        }
        counterBase = (GT_U32)dsaCpuCode;
...
)
{
    ...
    /*check that no one took this block before*/
        for(i=0;i<CPSS_DXCH_CNC_CLIENT_LAST_E;i++)
        {
            ...
            {
                if(GT_TRUE == forceInit)
                {
                    status = cpssDxChCncBlockClientEnableSet(devNum, blockNum,(CPSS_DXCH_CNC_CLIENT_ENT)i,GT_FALSE);
                    if (status != GT_OK)
                    {
                        return status;
                    }
...
)
{
    ...
    status = cpssDxChCncCounterGet(devNum,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_FIRST(devNum)+blockInternalIndex,((queueBase&0x1FF)+queue)<<1,
    ...
    status = cpssDxChCncCounterGet(devNum,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_FIRST(devNum)+blockInternalIndex,(((queueBase&0x1FF)+queue)<<1)+1,
                PRV_TXQ_QUEUE_STAT_CNC_BLOCK_DEFAULT_FORMAT,&localCnt);
    if (GT_OK != status)
    {
        return status;
...
)
{
    ...
    for(i=0;i<numberOfBlocks;i++)
    {
        rc = prvCpssFalconTxqDebugBlockClientEnableAndBindSet(devNum,firstBlock+i,
            CPSS_DXCH_CNC_CLIENT_EGRESS_QUEUE_PASS_DROP_E,GT_FALSE,i,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_DEFAULT_FORMAT,GT_FALSE,GT_FALSE);
        if(rc != GT_OK)
        {
          return rc;
        }
...
)
{
    ...
    {
        rc = prvCpssFalconTxqDebugBlockClientEnableAndBindSet(devNum,firstBlock+i,
            CPSS_DXCH_CNC_CLIENT_EGRESS_QUEUE_PASS_DROP_E,GT_TRUE,i,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_DEFAULT_FORMAT,validateNotTaken,forceInit);
        if(rc != GT_OK)
        {
          return rc;
        }
...
{
    return prvCpssFalconQueueStatisticInitSpecificBlock(devNum,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_FIRST(devNum),
        PRV_TXQ_QUEUE_STAT_BLOCKS_NUM_MAX_REQUIRED,GT_TRUE,forceInit);
}
...
{
    return prvCpssFalconQueueStatisticDeInitSpecificBlock(devNum,PRV_TXQ_QUEUE_STAT_CNC_BLOCK_FIRST(devNum),
...
)
{
    ...
    rc = cpssDxChPortTxPoolBuffNumberGet(devNum,poolId,&cnt);
...
)
{
    ...
    PRV_CPSS_TXQ_CIDER_INFO_STC initInfo [] =
        {       /*AC5P*/
                ...
                /*Ironman*/
                {
                    CPSS_PP_FAMILY_DXCH_IRONMAN_E,4,5,5
                },
                /*AAS*/
                {
                    CPSS_PP_FAMILY_DXCH_AAS_E,6,3,3
                }
        };
...
)
{
    ...
    PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE *    pnodePtr;
    GT_U32                              queueToSet;
    GT_U32 regValue[2],value,size,tmpSize,offset;
    ...
    if (mappingType != CPSS_DXCH_PORT_MAPPING_TYPE_REMOTE_PHYSICAL_PORT_E)
    {
        ...
        if((negativeCreditThreshold>>PRV_TH_GRANULARITY_SHIFT)>= (GT_U32)(1<<size))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
       ...
       if((lowCreditTheshold>>PRV_TH_GRANULARITY_SHIFT)>= (GT_U32)(1<<size))
       {
           CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
       }


        value = U32_GET_FIELD_MAC(lowCreditTheshold>>PRV_TH_GRANULARITY_SHIFT,0,32 - offset);
        tmpSize = 32 - offset;
       ...
       if((highCreditThreshold>>PRV_TH_GRANULARITY_SHIFT)>= (GT_U32)(1<<size))
       {
           CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
       }
...
{
    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).dataPathEventLog = enable;
    return GT_OK;
}
...
{
    return PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).dataPathEventLog;
}
...
)
{
    ...
    for(i=0;i<CPSS_DXCH_CNC_CLIENT_LAST_E;i++)
    {
        ...
        if((GT_OK==rc)&&(GT_TRUE==updateEnable))
        {
            ...
            {
               /*Disable the client*/
               rc = cpssDxChCncBlockClientEnableSet(devNum, PRV_TXQ_DROP_REASON_CNC_BLOCK,(CPSS_DXCH_CNC_CLIENT_ENT)i,GT_FALSE);
               if(rc!=GT_OK)
               {
...
)
{
    ...
    rc = prvCpssDxChNetIfCpuToDsaCode(tailDropCode, &dsaCpuCode);
    if (rc != GT_OK)
    {
        return rc;
    }
...
GT_STATUS prvCpssDxChTxqDumpNodeShapingParameters
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 nodeStart,
...
)
{
    ...
    /*move to KHz*/
    shaperFreq  /= 1000;
    rc      = prvCpssSip6TxqPdqPerLevelShapingParametersGet(devNum, tileNum, level, &shaperDec, &perInter, &perEn);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6TxqPdqPerLevelShapingParametersGet  failed  ");
    }
    cpssOsPrintf("\nLevel %d parameters:\n\nPeriodic interval %d\n\nshaperDec %d\n\nperEn %d\n", level,perInter, shaperDec, perEn);
    cpssOsPrintf("\n+----+---+-------+-------+-----+-------+-----------+");
    cpssOsPrintf("\n| #  |shp| divExp|token  |res  | burst |BW in Kbit |");
    cpssOsPrintf("\n+----+---+-------+-------+-----+-------+-----------+");
    for (i = nodeStart; (i <= nodeEnd); i++)
    {
        rc = prvCpssFalconTxqPdqShapingEnableGet(devNum, tileNum, i, level, &shapingEnabled);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqPdqShapingEnableGet  failed for q  %d  ", i);
        }
        ...
        rc = prvCpssSip6TxqPdqNodeShapingParametersGet(devNum, tileNum, i, level,&divExp, &token, &res, &burst);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqPdqGetQueueShapingParameters  failed for q  %d  ", i);
        }
...
)
{
    ...
    cpssOsFprintf(file->fd, "\n+-------------+--+--------+--------+--------+--------+----+--+");
    cpssOsFprintf(file->fd, "\n| queueNumber |tc|  high  |  low   |  neg   |balance |elig|HP|");
    cpssOsFprintf(file->fd, "\n+-------------+--+--------+--------+--------+--------+----+--+");
    for (i = startQ; i < (startQ + size); i++)
    {
        ...
        cpssOsFprintf(file->fd, "\n|%13d|%2d|%8d|%8d|%8d|%8d|0x%02x|%2d|", i,  queueAttributes.tc,
...
/**
 * @internal prvCpssFalconTxqUtilsPdqQueueProfilesBindDump function
 * @endinternal
 *
...
/**
 * @internal prvCpssFalconTxqUtilsPdqQueueProfilesAttributesDump function
 * @endinternal
 *
...
)
{
    ...
    PRV_CPSS_DXCH_SIP7_TXQ_EPB_QUEUE_PROFILE_STC            epbProfile = {0,0,{0,0}};
    GT_CHAR * types []= {"N"/*npds*/,"R"/*dpds rx*/,"T"/*dpds tx*/};
    profile.longQueueParametersPtr      = &longQ;
    profile.lengthAdjustParametersPtr   = &lengthAdjust;
    cpssOsPrintf("\n PDS queue profile tile %d  pds %d\n", tileNumber, pdsNum);
    cpssOsPrintf("\n+----------+-----------------+-----------------+-----------------+-----------------+-----------------+-----------------+-+---+---+---+");
    cpssOsPrintf("\n| profile  | longQueueEnable |   headEmptyLimit| longQueueLimit  |lengthAdjEnable  |lengthAdjSubstrct|lengthAdjByteCnt |T|EPB|MB0|MB1|");
    cpssOsPrintf("\n+----------+-----------------+-----------------+-----------------+-----------------+-----------------+-----------------+-+---+---+---+");

    if(last>=PDS_PROFILE_MAX_MAC(devNum))
    {
        last = PDS_PROFILE_MAX_MAC(devNum)-1;
    }
    ...
    for (i = first; i<=last; i++)
    {
        ...
        {
            rc = prvCpssSip7TxqPdsEpbProfileGet(devNum, tileNumber, pdsNum, i,&epbProfile);
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqPdsEpbProfileGet  failed for tile  %d  ", i);
            }
        }
        cpssOsPrintf("\n|%10d|%17d|%17d|%17d|%17d|%17d|%17d|%s|%3d|%3d|%3d|", i,
                 ...
                 profile.lengthAdjustParametersPtr->lengthAdjustByteCount,types[epbProfile.type],
                 epbProfile.epbSpilloverEnable,epbProfile.mailBoxId[0],epbProfile.mailBoxId[1]);
    }
    cpssOsPrintf("\n+----------+-----------------+-----------------+-----------------+-----------------+-----------------+-----------------+-----+---+---+");
    cpssOsPrintf("\n");
    return GT_OK;
...
)
{
    ...
    cpssOsPrintf("Number of tiles         : %6d\n",platform.numberOfTiles);
    cpssOsPrintf("Number of DPs           : %6d\n",platform.numberOfDp);
    cpssOsPrintf("Number of local ports   : %6d\n",platform.numberOfLocalPorts);
    cpssOsPrintf("Number of queues per DP : %6d\n",platform.numberOfDpQueues);
    cpssOsPrintf("PB size (all tiles) :   : %6d\n",platform.pbSize);
    cpssOsPrintf("SDQ2PDQ ports           : %6d\n",platform.pdqNumPorts);
    cpssOsPrintf("Shaper frequency(Mhz)   : %6d\n",platform.freq/1000000);
    cpssOsPrintf("cncBlocks               : %6d\n",PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.cncBlocks);
...
)
{
    ...
    if(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).pdsProfilesCombined == GT_FALSE)
    {
...
)
{
    ...
    *txFifoEmptyPtr = ((rawData>>5)&0x1);
    ...
    if (GT_TRUE==prvCpssTxqSip6MacTypeSupportPreemptionGet(devNum,macType))
    {
        rc = prvCpssFalconTxqUtilsPortPreemptionAllowedGet(devNum,portNum,&preeemptionCapable);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqUtilsPortPreemptionAllowedGet  failed for portNum  %d  ", portNum);
        }

        if(GT_TRUE == preeemptionCapable)
        {
...
#endif
    {
        if(bufferPtr&&GT_TRUE==PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).logEnable)
        {
...
{
    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).logEnable = enable?GT_TRUE:GT_FALSE;
    return GT_OK;
}


GT_VOID prvCpssTxqSip6DebugHqosForceSet
(
    IN  GT_U8          devNum
)
{
    ...
    file.type = CPSS_OS_FILE_REGULAR;
    file.fd = cpssOsFopen(PRV_TXQ_FORCE_HQOS_FILE_NAME, "r",&file);

    if (file.fd!=CPSS_OS_FILE_INVALID)
    {
        cpssOsPrintf(">>>>> File:%s found <<<<<\n",PRV_TXQ_FORCE_HQOS_FILE_NAME);
        PRV_CPSS_DXCH_PP_MAC(devNum)->port.txQDeviceConfig.treeMode = PRV_CPSS_DXCH_TXQ_SCHEDULING_TREE_MODE_FULL_E;
        cpssOsFclose(&file);
    }
}
...
)
{
    ...
    GT_STATUS rc;
    GT_U32 val;
    CPSS_DXCH_PORT_PREEMPTION_STATUS_STC brStatus;
    ...
    rc =cpssDxChPortTxPreemptionStatusGet(devNum,portNum,&brStatus);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxPreemptionStatusGet  failed for portNum  %d  ", portNum);
    }
    cpssOsPrintf("enabled = %d\n",brStatus.enabled);
    cpssOsPrintf("asmErrDetected = %d\n",brStatus.asmErrDetected);
    cpssOsPrintf("smdErrDetected = %d\n",brStatus.smdErrDetected);
    cpssOsPrintf("verifyFrameRx = %d\n",brStatus.verifyFrameRx);
    cpssOsPrintf("responceFrameRx = %d\n",brStatus.responceFrameRx);
    cpssOsPrintf("txVerify = %d\n",brStatus.txVerify);

    return GT_OK;
}

GT_STATUS  prvCpssTxqSip6PdqSelfTest
(
    IN GT_U8                devNum
)
{
    ...
    rc = prvCpssFalconTxqPdqGlobalTxEnableSet(devNum, 0, GT_FALSE);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqGlobalTxEnableSet failed for tile 0");
    }

    rc = prvCpssFalconTxqPdqGlobalTxEnableGet(devNum, 0, &enable);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqPdqGlobalTxEnableGet failed for tile - 0");
    }

    if(GT_TRUE == enable)
    {
        cpssOsPrintf("PDQ self test failed.Check point 1\n");
        return GT_OK;
    }

    rc = prvCpssFalconTxqPdqGlobalTxEnableSet(devNum, 0, GT_TRUE);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqGlobalTxEnableSet failed for tile - 0");
    }
    ...
    if(GT_FALSE== enable)
    {
        cpssOsPrintf("PDQ self test failed.Check point 2\n");
        return GT_OK;
    }

    cpssOsPrintf("PDQ self test success.\n");
...
GT_STATUS prvCpssSip6TxqUtilsQbvPortDump
(
    IN  GT_U8  devNum,
    IN  GT_U32 tileNumber,
    IN  GT_U32 sdqNum,
...
)
{
    ...
    TXQ_SIP_6_CHECK_TILE_NUM_MAC(tileNumber);

    TXQ_SIP_6_CHECK_LOCAL_SDQ_NUM_MAC(sdqNum);


    cpssOsFprintf(file->fd,"\nQvb config tile %d SDQ %d\n",tileNumber,sdqNum);
    cpssOsFprintf(file->fd,"\n+-----------------+-----+--------+");
    cpssOsFprintf(file->fd,"\n| local port ind  | PRE | QVB EN |");
    cpssOsFprintf(file->fd,"\n+-----------------+-----+--------+");


    for(i =0;i< CPSS_DXCH_SIP_6_MAX_LOCAL_PORT_NUM_MAC(devNum);i++)
    {
        rc = prvCpssSip6_30TxqSdqLocalPortQbvEnableGet(devNum,tileNumber,sdqNum,i,&qvb);

        if(rc!=GT_OK)
        {
            cpssOsPrintf("%d = prvCpssSip6_30TxqSdqLocalPortQbvEnableGet\n",rc );
            return rc;
        }

        rc = prvCpssSip6_30TxqSdqLocalPortPreemptionTypeGet(devNum,tileNumber,sdqNum,i,&pre);
        if(rc!=GT_OK)
        {
            cpssOsPrintf("%d = prvCpssSip6_30TxqSdqLocalPortPreemptionTypeGet\n",rc );
            return rc;
        }
        cpssOsFprintf(file->fd,"\n|%-17d|0x%03x|0x%06x|",i,pre,qvb);
        cpssOsFprintf(file->fd,"\n+-----------------+-----+--------+");
    }
    cpssOsFprintf(file->fd,"\n");
...
GT_STATUS prvCpssSip6TxqUtilsQbvQueueDump
(
    IN  GT_U8  devNum,
    IN  GT_U32 tileNumber,
    IN  GT_U32 sdqNum,
...
)
{
    ...
    for(i = qStart;(i<qEnd)&&(i< CPSS_DXCH_SIP_6_MAX_DATA_PATH_QUEUE_NUM_MAC);i++)
    {
        rc = prvCpssSip6_30TxqSdqQueueGateGet(devNum,tileNumber,sdqNum,i,&gate);
        if (rc != GT_OK)
        {
...
GT_STATUS prvCpssSip6TxqUtilsQbvCalendarDumpRaw
(
    IN  GT_U8  devNum,
    IN GT_U32  tableSet,
    IN GT_U32  sizeToDump
)
{

    GT_U32   i,data[2],timeToAdvance[2],size,gate[2],hold[2];
    GT_STATUS rc;
    ...
    {
        rc = prvCpssSip6_30TxqSdqQbvCalendarRawDataGet(devNum,0,0,tableSet,i,data);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqQbvCalendarRawDataGet failed  ");
        }

        timeToAdvance[0] = U32_GET_FIELD_MAC(data[0],
               TXQ_IRONMAN_SDQ_QBV_CFG_TIME_TO_ADVANCE_FIELD_OFFSET,
              size);

        timeToAdvance[1] = U32_GET_FIELD_MAC(data[1],
               0,
              TXQ_IRONMAN_SDQ_QBV_CFG_TIME_TO_ADVANCE_FIELD_SIZE-size);

        timeToAdvance[0]|=(timeToAdvance[1]<<size);

        gate[0] = U32_GET_FIELD_MAC(data[0],
               TXQ_IRONMAN_SDQ_QBV_CFG_GATE_STATE_CURRENT_FIELD_OFFSET,
              TXQ_IRONMAN_SDQ_QBV_CFG_GATE_STATE_CURRENT_FIELD_SIZE);

        gate[1] = U32_GET_FIELD_MAC(data[0],
               TXQ_IRONMAN_SDQ_QBV_CFG_GATE_STATE_NEXT_FIELD_OFFSET,
              TXQ_IRONMAN_SDQ_QBV_CFG_GATE_STATE_NEXT_FIELD_SIZE);

        hold[0] = U32_GET_FIELD_MAC(data[0],
               TXQ_IRONMAN_SDQ_QBV_CFG_HOLD_RELEASE_CURRENT_FIELD_OFFSET,
              TXQ_IRONMAN_SDQ_QBV_CFG_HOLD_RELEASE_CURRENT_FIELD_SIZE);

        hold[1] = U32_GET_FIELD_MAC(data[0],
               TXQ_IRONMAN_SDQ_QBV_CFG_HOLD_RELEASE_NEXT_FIELD_OFFSET,
              TXQ_IRONMAN_SDQ_QBV_CFG_HOLD_RELEASE_NEXT_FIELD_SIZE);

        cpssOsPrintf("\n\t[%3d]         RAW msb 0x%08x lsb 0x%08x\n",i,data[1],data[0]);
        cpssOsPrintf("\n\t[%3d] timeToAdvance 0x%08x gate current 0x%02x gate next 0x%02x hc 0x%x hn 0x%x\n",i,timeToAdvance[0],gate[0],gate[1],hold[0],hold[1]);
    }
...
GT_STATUS prvCpssSip6TxqUtilsQbvCalendarDump
(
    IN  GT_U8  devNum,
    IN GT_U32  tableSet,
    IN GT_U32  sizeToDump
)
{
    ...
    GT_U32   i;
    GT_STATUS rc;
    CPSS_DXCH_STREAM_EGRESS_TIME_SLOT_INFO_STC entry;

    for(i=0;i<sizeToDump;i++)
    {
        rc = prvCpssSip6_30TxqSdqQbvCalendarGet(devNum,0,0,tableSet,i,&entry);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqQbvCalendarGet failed  ");
        }
        cpssOsPrintf("\n\t[%3d] timeToAdvance 0x%08x gate  0x%02x  hold 0x%x\n",
            i,entry.timeToAdvance,entry.gateStateBmp,entry.hold);
    }
...
GT_STATUS prvCpssSip6TxqUtilsQbvSelfTest
(
    IN GT_U8   devNum,
    IN GT_U32  portNum,
    IN GT_U32  tableSet,
...
{
    CPSS_DXCH_STREAM_EGRESS_TIME_SLOT_INFO_STC table[5];
    GT_U32   i;
    GT_STATUS rc;
    CPSS_DXCH_STREAM_EGREESS_PORT_BIND_ENTRY_STC bindInfo;

    GT_U32  portSpeedInGb =10;

    for(i=0;i<5;i++)
    {
        table[i].timeToAdvance = (1<<i)|(1<<(i+24));
        table[i].gateStateBmp= i;
        table[i].hold = i%2?GT_TRUE:GT_FALSE;
        cpssOsPrintf("Input Line %d timeToAdvance 0x%08x\n",i,table[i].timeToAdvance);
    }

    rc = prvCpssSip6_30TxqQbvConfigureTableSet(devNum,0,5,table);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqQbvConfigureTableSet failed  ");
    }

    rc =prvCpssSip6TxqUtilsQbvCalendarDump(devNum,0,10);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6TxqUtilsQbvCalendarDump failed  ");
    }

    bindInfo.gateReconfigRequestParam.cycleTime = table[4].timeToAdvance;
    bindInfo.gateReconfigRequestParam.egressTodOffset = 3;
    bindInfo.gateReconfigRequestParam.lastEntry = lastEntry;
    bindInfo.gateReconfigRequestParam.lengthAware = GT_TRUE;
    bindInfo.gateReconfigRequestParam.bitsFactorResolution =3;
    bindInfo.gateReconfigRequestParam.maxAlwdBcOffset =0;

    bindInfo.gateReconfigTimeParam.configChangeTimeExtension=2;
    bindInfo.gateReconfigTimeParam.todMsb.l[0]=100;
    bindInfo.gateReconfigTimeParam.todMsb.l[1]=20;
    bindInfo.gateReconfigTimeParam.todLsb= 12345;

    /*1 Gbit/sec =1  bit/nanosec =16 bit/16nanosec*/
    bindInfo.gateReconfigRequestParam.remainingBitsFactor = portSpeedInGb*16;

    rc = prvCpssSip6_30TxqQbvBindPortToTableSet(devNum,portNum,tableSet,&bindInfo);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqQbvBindPortToTableSet failed  ");
    }
    ...
    /*check if done*/
    rc =prvCpssSip6_30TxqQbvChangeReqCompleteGet(devNum);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqQbvChangeReqCompleteGet failed  ");
    }
...
GT_STATUS  prvCpssTxqSip6SchedEligFuncSelfTest
(
    IN GT_U8                devNum,
    IN GT_U32               portNum
)
{
    GT_STATUS rc;
    GT_U32 i,pdqNum,nodeInd;
    CPSS_DXCH_DETAILED_PORT_MAP_STC map;
    struct sched_queue_params   queueParams;
    int             ret;

    for(i=0;i<8;i++)
    {
        rc = cpssDxChPortTxQArbGroupSet(devNum,i,CPSS_PORT_TX_SP_ARB_GROUP_E,CPSS_PORT_TX_SCHEDULER_PROFILE_16_E);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxQArbGroupSet failed  ");
        }
    }

    rc = cpssDxChPortTxBindPortToSchedulerProfileSet(devNum,portNum,CPSS_PORT_TX_SCHEDULER_PROFILE_16_E);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxBindPortToSchedulerProfileSet failed  ");
    }

    rc = cpssDxChPortPhysicalPortDetailedMapGet(devNum,portNum,&map);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortPhysicalPortDetailedMapGet failed  ");
    }

    pdqNum = map.extPortMap.tileId;
    nodeInd = map.extPortMap.txqInfo.primarySchFirstQueueIndex;

    for(i=0;i<8;i++)
    {

        ret = prvCpssSchedNodesReadQueueConfiguration(PRV_CPSS_DXCH_PP_MAC(devNum)->pdqInfo.schedHandle[pdqNum],
                                      nodeInd+i, &queueParams);
        if (ret)
        {
            rc = FALCON_XEL_TO_CPSS_ERR_CODE(ret, pdqNum);
            return rc;
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q_PRIO0+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_NORMAL_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q_PRIO0+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_NORMAL_E success.\n");



    for(i=0;i<8;i++)
    {
        rc = cpssDxChPortTxQShaperEnableSet(devNum,portNum,i,GT_TRUE);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxShaperEnableSet failed  ");
        }
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q_MIN_SHP_PRIO0+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_SHAPING_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q_MIN_SHP_PRIO0+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_SHAPING_E success.\n");


    for(i=0;i<8;i++)
    {

        rc = cpssDxChPortTxQMinimalRateEnableSet(devNum,portNum,i,GT_TRUE,CPSS_DXCH_PORT_TX_GUARANTEED_BANDWITH_REACHED_ACTION_SET_PRIORITY_TO_ZERO_E);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxShaperEnableSet failed  ");
        }
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q0_MIN_BW_TYPE0_WITH_SHAPING+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE0_WITH_SHAPING_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q0_MIN_BW_TYPE0_WITH_SHAPING+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE0_WITH_SHAPING_E success.\n");

    for(i=0;i<8;i++)
    {

        rc = cpssDxChPortTxQMinimalRateEnableSet(devNum,portNum,i,GT_TRUE,CPSS_DXCH_PORT_TX_GUARANTEED_BANDWITH_REACHED_ACTION_SET_PRIORITY_TO_ORIGINAL_E);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxQMinimalRateEnableSet failed  ");
        }
        ret = prvCpssSchedNodesReadQueueConfiguration(PRV_CPSS_DXCH_PP_MAC(devNum)->pdqInfo.schedHandle[pdqNum],
                                      nodeInd+i, &queueParams);
        if (ret)
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q0_MIN_BW_TYPE1_WITH_SHAPING+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE1_WITH_SHAPING_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q0_MIN_BW_TYPE1_WITH_SHAPING+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE1_WITH_SHAPING_E success.\n");

    for(i=0;i<8;i++)
    {
        rc = cpssDxChPortTxQShaperEnableSet(devNum,portNum,i,GT_FALSE);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " cpssDxChPortTxShaperEnableSet failed  ");
        }
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q0_MIN_BW_TYPE1+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE1_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q0_MIN_BW_TYPE1+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE1_E success.\n");
    ...
    for(i=0;i<8;i++)
    {
        ...
        if(queueParams.elig_prio_func_ptr !=PRV_SCHED_ELIG_Q0_MIN_BW_TYPE0+i)
        {
            cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE0_E fail.\n");
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, " Expected %d got %d for queue %d ",PRV_SCHED_ELIG_Q0_MIN_BW_TYPE1+i,queueParams.elig_prio_func_ptr,i);
        }
    }

    cpssOsPrintf("PRV_SCHED_ELIG_FUNC_GROUP_MIN_BW_TYPE0_E success.\n");

    return GT_OK;
}

GT_STATUS  prvCpssTxqSip6_30PdqPizzaDumpRawFormat
(
    IN GT_U8                 devNum,
    IN GT_U32               *rawDataArr,
    IN GT_U32                size
)
{
    GT_U32 i,j;
    GT_U32 pdqPortsNum;
    GT_U32 *portSliceNumPtr;
    GT_U32  pNodeInd;
    GT_U32  mask,tmp;
    GT_U32  ignoredSlices=0;
    CPSS_NULL_PTR_CHECK_MAC(rawDataArr);
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);

    pdqPortsNum = CPSS_DXCH_SIP_6_MAX_PDQ_PORT_NUM(devNum);
    cpssOsPrintf("pdqPortsNum %d \n",pdqPortsNum);

    portSliceNumPtr = (GT_U32 *)cpssOsMalloc(sizeof(GT_U32)*pdqPortsNum);
    if (NULL == portSliceNumPtr)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
    }

    cpssOsMemSet(portSliceNumPtr,0,sizeof(GT_U32)*pdqPortsNum);

    mask = (1<<PRV_TXQ_PIZZA_SLICE_WIDTH_IN_BITS(devNum))-1;

    for(i=0;i<size;i+=PRV_TXQ_SIP6_30_PIZZA_SLICE_PER_LINE)
    {
        if(0==(i%PRV_TXQ_SIP6_30_PIZZA_SLICE_PER_LINE))
        {
            tmp = rawDataArr[i/PRV_TXQ_SIP6_30_PIZZA_SLICE_PER_LINE];
        }
        for(j=0;j<PRV_TXQ_SIP6_30_PIZZA_SLICE_PER_LINE;j++)
        {
            pNodeInd = tmp&mask;
            tmp>>=PRV_TXQ_PIZZA_SLICE_WIDTH_IN_BITS(devNum);
            if(i+j<size)
            {
                if(pNodeInd<pdqPortsNum)
                {
                    cpssOsPrintf("Slice [%d] line %d contain value %d \n",i+j,i>>2,pNodeInd);
                    portSliceNumPtr[pNodeInd]++;
                }
                else
                {
                    cpssOsPrintf("Slice [%d]contain value %d is ignored \n",i+j,pNodeInd);
                    ignoredSlices++;
                }
            }
            else
    ...
    for(i=0;i<pdqPortsNum;i++)
    {
        if(0!=portSliceNumPtr[i])
        {
            cpssOsPrintf("P [%d] => Num of slices [%d]\n",i,portSliceNumPtr[i]);
        }
    }

    cpssOsFree(portSliceNumPtr);

    return GT_OK;

}

GT_STATUS  prvCpssTxqSip6_30PdqPizzaDump
(
    IN GT_U8                 devNum,
    IN GT_U32                tileNum
)
{   GT_STATUS rc;
    GT_U32 lastSlice,i,numOfLines;
    GT_U32 *pizzaRawFormat;
    ...
    cpssOsPrintf("LastSlice %d (numOfLines = %d)\n",lastSlice,numOfLines);

    pizzaRawFormat = (GT_U32 *)cpssOsMalloc(sizeof(GT_U32)*numOfLines);
    if (NULL == pizzaRawFormat)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
    }

    cpssOsMemSet(pizzaRawFormat,0,sizeof(GT_U32)*numOfLines);

    for(i=0;i<numOfLines;i++)
    {
        rc = prvCpssSip6_30TxqPdqPizzaSliceLineGet(devNum,tileNum,i,pizzaRawFormat+i);
        if (rc != GT_OK)
        {
    ...
    rc = prvCpssTxqSip6_30PdqPizzaDumpRawFormat(devNum,pizzaRawFormat,
        lastSlice+1);

    cpssOsFree(pizzaRawFormat);

    return rc;

}

GT_STATUS  prvCpssTxqSip6_30PdqPizzaDefaultSet
(
    IN GT_U8                 devNum,
    IN GT_U32                tileNum,
    IN GT_U32                size
)
{
    ...
    rc = prvCpssSip6_30TxqPdqPortPizzaLastSliceSet(devNum,tileNum,size-1);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
    ...
    for(i=0;i<size;i++)
    {
        rc =prvCpssSip6_30TxqPdqPizzaSliceSet(devNum,tileNum,i,i);
        if (rc != GT_OK)
        {
...
GT_STATUS  prvCpssTxqSip7DumpPnodeMapping
(
    IN GT_U8                 devNum,
    IN GT_U32                tileNum
)
{
    ...
    mappingPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->port.tileConfigsPtr[tileNum].mapping);

    for(i=0;i<mappingPtr->size;i++)
    {
        if(NULL!= mappingPtr->pNodeMappingConfiguration[i])
        {
            cpssOsPrintf("Pnode index [%d]\n",mappingPtr->pNodeMappingConfiguration[i]->pNodeIndex);
            cpssOsPrintf(" cNodeListSize [%d]\n",mappingPtr->pNodeMappingConfiguration[i]->cNodeListSize);
            for(j=0;j<mappingPtr->pNodeMappingConfiguration[i]->cNodeListSize;j++)
            {
                cpssOsPrintf("  cNodeIndex [%d] service [%d]  bNodeListSize [%d]\n",
                    mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].cNodeIndex,
                    mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].serviceGroup,
                    mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeListSize);
                for(k=0;k<mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeListSize;k++)
                {
                    cpssOsPrintf("    bNodeIndex [%d] subService [%d] aNodeListSize [%d]\n",
                      mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeList[k].bNodeIndex,
                      mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeList[k].subServiceGroup,
                      mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeList[k].aNodeListSize);

                     for(l=0;l<mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeList[k].aNodeListSize;l++)
                     {
                        cpssOsPrintf("      aNodeIndex [%d] \n",
                          mappingPtr->pNodeMappingConfiguration[i]->cNodeList[j].bNodeList[k].aNodePtrList[l]->aNodeIndex);
                     }
                }
            }
        }
...
GT_STATUS  prvCpssTxqSip7DumpServiceDump
(
    IN GT_U8 devNum,
...
)
{
    GT_STATUS rc;
    GT_BOOL   enable;
    GT_U32    priorityGroup,weight;

    cpssOsPrintf("ServiceId [%d] \n",serviceId);
    cpssOsPrintf("================\n",serviceId);
    rc = prvCpssSip7TxqUtilsServiceShapingEnableGet(devNum,physicalPortNum,serviceId,&enable);

    if(GT_OK==rc)
    {
        cpssOsPrintf("shaping enable %d\n",enable);
        rc = prvCpssDxChTxqServicePriorityGet(devNum,physicalPortNum,serviceId,&priorityGroup);
        if(GT_OK==rc)
        {
            cpssOsPrintf("priority %d\n",priorityGroup);
            rc = prvCpssDxChTxqServiceDwrrWeightGet(devNum,physicalPortNum,serviceId,&weight);
            if(GT_OK==rc)
            {
                cpssOsPrintf("weight %d\n",weight);
            }
        }
    }
    cpssOsPrintf("================================\n");
...
GT_STATUS  prvCpssTxqSip7DumpSubServiceDump
(
    IN GT_U8 devNum,
    IN GT_U32 physicalPortNum,
    IN GT_U32 serviceId,
...
)
{
    ...
    cpssOsPrintf("ServiceId [%d] subServiceId [%d]\n",serviceId,subServiceId);
    cpssOsPrintf("================================\n",serviceId,subServiceId);
    rc = prvCpssSip7TxqUtilsSubServiceShapingEnableGet(devNum,physicalPortNum,serviceId,subServiceId,&enable);

    if(GT_OK==rc)
    {
        cpssOsPrintf("shaping enable %d\n",enable);
        rc = prvCpssDxChTxqSubServicePriorityGet(devNum,physicalPortNum,serviceId,subServiceId,&priorityGroup);
        if(GT_OK==rc)
        {
            cpssOsPrintf("priority %d\n",priorityGroup);

            rc = prvCpssDxChTxqSubServiceDwrrWeightGet(devNum,physicalPortNum,serviceId,
                subServiceId,&weight);
            if(GT_OK==rc)
            {
                cpssOsPrintf("weight %d\n",weight);
...
GT_STATUS prvCpssSip7TxqDebugUtilsServiceShapingParametersSet
(
    IN GT_U8        devNum,
    IN GT_U32       physicalPortNum,
    IN GT_U32       serviceId,
...
)
{
    ...
    rc = prvCpssSip7TxqUtilsServiceShapingParametersSet(devNum,physicalPortNum,serviceId,burstSize,&maxRate);
    if(GT_OK!=rc)
    {
        return rc;
    }
    cpssOsPrintf("  IN     serviceId [%d] maxRate [%d]\n",serviceId,maxRate);

    maxRate =0;
    burstSize =0;

    rc = prvCpssSip7TxqUtilsServiceShapingParametersGet(devNum,physicalPortNum,serviceId,&burstSize,&maxRate);

    if(GT_OK==rc)
    {
        cpssOsPrintf("  OUT  maxRate [%d] burstSize [%d]\n",maxRate,burstSize);
    }
...
GT_STATUS prvCpssSip7TxqDebugUtilsSubServiceShapingParametersSet
(
    IN GT_U8        devNum,
    IN GT_U32       physicalPortNum,
    IN GT_U32       serviceId,
...
)
{
    ...
    rc = prvCpssSip7TxqUtilsSubServiceShapingParametersSet(devNum,physicalPortNum,serviceId,subServiceId,burstSize,&maxRate);
    if(GT_OK!=rc)
    {
        return rc;
    }
    cpssOsPrintf("  IN     serviceId [%d] subServiceId [%d]  maxRate [%d]\n",serviceId,subServiceId,maxRate);

    maxRate =0;
    burstSize =0;

    rc = prvCpssSip7TxqUtilsSubServiceShapingParametersGet(devNum,physicalPortNum,serviceId,subServiceId,&burstSize,&maxRate);
...
GT_STATUS prvCpssDxChTxqDumpLKN
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum
)
{
    ...
    for(i=PRV_CPSS_PDQ_LEVEL_Q_E;i<=PRV_CPSS_PDQ_LEVEL_P_E;i++)
    {
        rc = prvCpssSip6LKNGet(devNum,tileNum,i,&L,&K,&N);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6LKNGet failed  ");
        }

        cpssOsPrintf(" Level %d => L %d K %d N %d\n",i,L,K,N);
    }
...
static GT_STATUS prvCpssDxChTxqRangeDumpQnodeToAnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 start,
...
)
{
    ...
    PRV_CPSS_SCHED_HW_PARAMS_STC hwParams;
    ...
    rc = prvCpssSip6TxqPdqSchedulerHwParamsGet(devNum,&hwParams);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6TxqPdqSchedulerHwParamsGet failed  ");
    }

    cpssOsPrintf("Total available Q %d \n",hwParams.maxQueues);

    for(i=start;i<=end;i++)
    {
        cpssOsPrintf("qNodeIndexToAnodeIndex [%d] %d\n",i,mappingPtr->searchTable.qNodeIndexToAnodeIndex[i]);
    }
...
GT_STATUS prvCpssDxChTxqDumpQnodeToAnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum
)
{
    ...
    return prvCpssDxChTxqRangeDumpQnodeToAnode(devNum,tileNum,0,mappingPtr->searchTable.qNodeIndexToAnodeIndexSize);

}

GT_STATUS prvCpssDxChTxqRangeDumpAnodeToPnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 start,
...
)
{
    ...
    cpssOsPrintf("Total available A %d \n",hwParams.maxAnodes);

    for(i=start;i<=end;i++)
    {
        cpssOsPrintf("aNodeIndexToPnodeIndex [%d] = %d\n",i,mappingPtr->searchTable.aNodeIndexToPnodeIndex[i]);
    }
...
GT_STATUS prvCpssDxChTxqDumpAnodeToPnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum
)
{
    ...
    return prvCpssDxChTxqRangeDumpAnodeToPnode(devNum,tileNum,0,mappingPtr->searchTable.aNodeIndexToPnodeIndexSize-1);
}

GT_STATUS prvCpssDxChTxqDumpPdqResources
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 level
)
{
    PRV_CPSS_DXCH_TXQ_SIP_6_MAPPING_CONFIGURATION * mappingPtr;
    PRV_CPSS_DXCH_TXQ_SIP_6_CONFIGURATION * tileConfigsPtr;
    PRV_CPSS_TXQ_RESOURCE_RANGE_STC * head;
    GT_U32                                index =0;
    ...
    if(level>=PRV_CPSS_MAX_LEVELS_NUM_CNS)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, " Level %d out of range",level);
    }
    head = mappingPtr->pdqResources[level];

    while(head)
    {
        cpssOsPrintf("[%d] first %d last %d\n",index,head->first,head->last);
        head =head->next;
        index++;
    }
...
GT_STATUS prvCpssDxChTxqDumpSdqResources
(
    IN GT_U8 devNum,
    IN GT_U32 tileNum,
    IN GT_U32 dp
)
{
    ...
    if(dp>=MAX_DP_IN_TILE(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, " DP %d out of range",dp);
    }

    head = mappingPtr->sdqResources[dp];
...
GT_STATUS prvCpssTxqSip6DebugGlobalDpNumGet
(
    IN   GT_U8                   devNum,
    OUT  GT_U32                  *dpNumPtr
)
{
    ...
    *dpNumPtr = platform.numberOfTiles *platform.numberOfDp;

    return GT_OK ;
}

GT_STATUS prvCpssTxqSip6DebugLocalDpNumGet
(
    IN   GT_U8                   devNum,
    OUT  GT_U32                  *dpNumPtr
)
{
    ...
    *dpNumPtr = platform.numberOfDp;

    return GT_OK ;
}


GT_STATUS prvCpssSip7TxqDumpAlevelRange
(
    IN GT_U8 devNum,
    IN GT_U32 tileIndex,
    IN GT_U32 aLowIndex,
    IN GT_U32 aHighIndex
)
{
    GT_U32    ret;
    GT_U32    i,parent,status;
    GT_U32    qLowIndex,qHighIndex;
    GT_STATUS rc = GT_OK;
    GT_U32    ef;

    for(i=aLowIndex;i<=aHighIndex;i++)
    {
        ret = prvCpssSchedNodesReadAnodeMappingSw(PRV_CPSS_DXCH_PP_MAC(devNum)->pdqInfo.schedHandle[tileIndex],
                              i,&parent,&qLowIndex,&qHighIndex,&ef,&status);
        if (ret)
        {
            rc = FALCON_XEL_TO_CPSS_ERR_CODE(ret, tileIndex);
        }
        cpssOsPrintf("SW A %d Q %d->%d st %d parent %d ef %d\n",i, qLowIndex,qHighIndex,status,parent,ef);

        ret = prvCpssSchedNodesReadAnodeMappingHw(PRV_CPSS_DXCH_PP_MAC(devNum)->pdqInfo.schedHandle[tileIndex],
                              i,&parent,&qLowIndex,&qHighIndex,&ef);
        if (ret)
        {
            rc = FALCON_XEL_TO_CPSS_ERR_CODE(ret, tileIndex);
        }
        cpssOsPrintf("HW A %d Q %d->%d parent %d ef %d\n",i, qLowIndex,qHighIndex,parent,ef);
    }

    return rc;
}

GT_STATUS prvCpssSip7TxqDumpSwDbPnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileIndex,
    IN GT_U32 pNodeInd
)
{
    GT_U32 ii;
    PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE * pNodePtr;
    PRV_TXQ_SIP_6_PNODE_INSTANCE_GET(pNodePtr,devNum,tileIndex,pNodeInd);

    for(ii=0;ii<pNodePtr->aNodeListSize;ii++)
    {
       cpssOsPrintf("ind %d A %d \n",ii, pNodePtr->aNodelist[ii].aNodeIndex);
    }
...
GT_STATUS prvCpssSip7TxqDumpSwDbBnode
(
    IN GT_U8 devNum,
    IN GT_U32 tileIndex,
    IN GT_U32 pNodeInd,
    ...
    for(ii=0;ii<pNodePtr->cNodeListSize;ii++)
    {
        if(pNodePtr->cNodeList[ii].cNodeIndex==cNodeInd)
        {
          for(jj=0;jj<pNodePtr->cNodeList[ii].bNodeListSize;jj++)
          {
             ...
             {
                for(kk=0;kk<pNodePtr->cNodeList[ii].bNodeList[jj].aNodeListSize;kk++)
                {
                    cpssOsPrintf("prvCpssSip7TxqDumpSwDbBnode ind %d A %d \n",kk, pNodePtr->cNodeList[ii].bNodeList[jj].aNodePtrList[kk]->aNodeIndex);
                }
                break;
             }
          }
...
GT_STATUS prvCpssSip7TxqDynamicRemotePortAddTest
(
    IN GT_U8 devNum
)
{
    PRV_CPSS_DXCH_TXQ_SIP_7_ADD_REMOTE_PORT_IN_PARAMS_STC params;
    GT_U32    bNodeIndex,tileIndex,ret;
    GT_U32    cNodeIndex;
    GT_U32    aLowIndex,aHighIndex;
    PRV_CPSS_DXCH_TXQ_SIP_6_CONFIGURATION *tilePtr;
    GT_STATUS rc = GT_OK;
    GT_U32    pdqBase;
    PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE * pNodePtr;
    GT_BOOL   isCascade;
    PRV_CPSS_DXCH_TXQ_SIP_7_ADD_REMOTE_PORT_OUT_PARAMS_STC outParams;


    params.cscdPort =0;
    params.numberOfQueues=8;
    params.portNum  = 80;
    params.hqosId[0]=1;
    params.hqosId[1]=1;

    rc = prvCpssFalconTxqUtilsIsCascadePort(devNum, params.cscdPort, &isCascade, &pNodePtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssFalconTxqUtilsIsCascadePort  failed for portNum  %d  ", params.cscdPort);
    }


    rc = prvCpssSip7TxqUtilsSubServiceIdToBnodeIndexGet(devNum,params.cscdPort,
        params.hqosId[0], params.hqosId[1],&bNodeIndex,&tileIndex);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqUtilsSubServiceIdToBnodeIndexGet failed ");
    }

    cpssOsPrintf("Bnode %d Tile %d\n", bNodeIndex,tileIndex);

    ret = prvCpssSchedNodesReadBnodeMappingHw(PRV_CPSS_DXCH_PP_MAC(devNum)->pdqInfo.schedHandle[tileIndex],
                              bNodeIndex,&cNodeIndex,&aLowIndex,&aHighIndex,NULL);
    if (ret)
    {
        rc = FALCON_XEL_TO_CPSS_ERR_CODE(ret, tileIndex);
    }
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSchedNodesReadBnodeMappingHw failed ");
    }


    tilePtr = PRV_CPSS_TILE_HANDLE_GET_MAC(devNum,tileIndex);

    rc = prvCpssSip7TxqDumpSwDbPnode(devNum,tileIndex,pNodePtr->pNodeIndex);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChTxqRangeDumpQnodeToAnode failed ");
    }

    cpssOsPrintf("Before A %d->%d ,aNodeIndexToPnodeIndexSize %d \n", aLowIndex,aHighIndex,
        tilePtr->mapping.searchTable.aNodeIndexToPnodeIndexSize);
    rc = prvCpssSip7TxqDumpAlevelRange(devNum,tileIndex,aLowIndex,aHighIndex);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqDumpAlevelRange failed ");
    }

    rc = prvCpssSip7TxqPortAddToExistingBranch(devNum,&params,&outParams);
    if (rc != GT_OK)
    {
        cpssOsPrintf("%d = prvCpssSip7TxqPortAddToExistingBranch\n", rc );
        return rc;
    }

    cNodeIndex = outParams.cNodeInd;
    aLowIndex = outParams.newAnodeStart;
    aHighIndex = outParams.newAnodeEnd;
    pdqBase = outParams.newQbase;

    cpssOsPrintf("\n\n\nAfter B node  %d Cnode  %d Pnode %d\n", outParams.bNodeInd,outParams.cNodeInd,outParams.pNodeInd);
    cpssOsPrintf("Old A %d->%d\n", outParams.oldAnodeStart,outParams.oldAnodeEnd);
    cpssOsPrintf("New A %d->%d aNodeIndexToPnodeIndexSize %d\n", aLowIndex,aHighIndex,
        tilePtr->mapping.searchTable.aNodeIndexToPnodeIndexSize);
    ...
    rc = prvCpssDxChTxqRangeDumpAnodeToPnode(devNum,tileIndex,outParams.oldAnodeStart,outParams.oldAnodeEnd);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChTxqDumpAnodeToPnode failed ");
    }

    rc = prvCpssDxChTxqRangeDumpAnodeToPnode(devNum,tileIndex,outParams.newAnodeStart,outParams.newAnodeEnd);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChTxqDumpAnodeToPnode failed ");
    }

    rc = prvCpssDxChTxqRangeDumpQnodeToAnode(devNum,tileIndex,pdqBase,pdqBase+params.numberOfQueues-1);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChTxqRangeDumpQnodeToAnode failed ");
    }
    ...
    rc = prvCpssSip7TxqDumpSwDbBnode(devNum,tileIndex,pNodePtr->pNodeIndex,outParams.cNodeInd,outParams.bNodeInd);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqDumpSwDbBnode failed ");
    }
...
GT_STATUS prvCpssSip6TxqPfcMacRxResponceReceiveDump
(
    IN GT_U8   devNum,
    IN  GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    rc = prvCpssDxChPortSip6PauseOverrideGet(devNum,portNum,&value,&regAddr);
    if(rc!=GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }

    cpssOsPrintf("ADDR 0x%08x  CTRL 0x%04x  VAL 0x%04x \n", regAddr,value&0xFF,value>>16);

    return GT_OK;
}

GT_STATUS prvCpssSip6TxqErrataEnable
(
    IN GT_U8   devNum,
    IN GT_BOOL enable
)
{
    if(GT_TRUE == enable)
    {
        PRV_CPSS_DXCH_ERRATA_SET_MAC(devNum, PRV_CPSS_DXCH_FALCON_TAIL_DROP_ON_VIRTUAL_BUFFERS_E);
    }
    else
    {
        PRV_CPSS_DXCH_ERRATA_CLEAR_MAC(devNum, PRV_CPSS_DXCH_FALCON_TAIL_DROP_ON_VIRTUAL_BUFFERS_E);
    }
    return GT_OK;
}

GT_STATUS prvCpssSip6TxqDumpAllErrata
(
    IN GT_U8   devNum
)
{
    GT_BOOL enabled = PRV_CPSS_DXCH_ERRATA_GET_MAC(
            devNum,PRV_CPSS_DXCH_FALCON_TAIL_DROP_ON_VIRTUAL_BUFFERS_E);

    cpssOsPrintf("PRV_CPSS_DXCH_FALCON_TAIL_DROP_ON_VIRTUAL_BUFFERS_E is %s enabled\n", enabled?"":"NOT");
    return GT_OK;
}

GT_STATUS  prvCpssSip6TxqCapabilityDump
(
    IN GT_U8 devNum
)
{
    GT_BOOL isCapable;
    cpssOsPrintf("Raw bmp is 0x%x \n\n", PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).capabilitiesBmp);
    (GT_VOID)prvCpssSip6TxqCapabilitySupportedGet(devNum,PRV_CPSS_DXCH_TXQ_SIP_6_FEATURE_QBV_E,&isCapable);
    cpssOsPrintf("PRV_CPSS_DXCH_TXQ_SIP_6_FEATURE_QBV_E        [%s] \n", isCapable?"V":"X");
    (GT_VOID)prvCpssSip6TxqCapabilitySupportedGet(devNum,PRV_CPSS_DXCH_TXQ_SIP_6_FEATURE_PREEMPTION_E,&isCapable);
    cpssOsPrintf("PRV_CPSS_DXCH_TXQ_SIP_6_FEATURE_PREEMPTION_E [%s] \n", isCapable?"V":"X");
    isCapable = TXQ_IS_DEEP_BUFFER_CAPABLE_DEVICE(devNum);
    cpssOsPrintf("PRV_CPSS_DXCH_TXQ_SIP_7_FEATURE_DEEP_BUFFER_E [%s] \n", isCapable?"V":"X");

    return GT_OK;
}

GT_STATUS prvCpssSip6TxqShaperCalculator
(
    IN GT_U8  devNum,
    IN GT_U32 maxRate
)
{
    ...
    CPSS_DXCH_TXQ_SHAPER_OUTPUT_STC   output;
    rc = prvCpssSip6TxqPdqShapingParametersCalculate(devNum,CPSS_DXCH_TXQ_SHAPER_TYPE_CIR_E,
        CPSS_DXCH_TXQ_SHAPER_LEVEL_QUEUE_E,maxRate,&output);
    cpssOsPrintf("rc %d\n",rc);
    cpssOsPrintf("outRate %d\n",output.outRate);
    cpssOsPrintf("inaccuracy %s%d.%d %\n",output.roundDown?"-":"",output.inAccuracyPercent,output.inAccuracyFracPercent);
    cpssOsPrintf("effectiveBurst %d \n",output.effectiveBurst);
    cpssOsPrintf("DIV EXP %d \n",output.dividerExp);
    cpssOsPrintf("TOKEN RES %d \n",output.tokenResolution);
    cpssOsPrintf("TOKEN     %d \n",output.token);
    return rc;
}


GT_STATUS prvCpssSip7TxqQ2QTableDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   sdqNum,
...
)
{
    ...
    if(end>CPSS_DXCH_SIP_6_MAX_DATA_PATH_QUEUE_NUM_MAC-1)
    {
       end =  CPSS_DXCH_SIP_6_MAX_DATA_PATH_QUEUE_NUM_MAC-1;
    }

    for(i=start;i<=end;i++)
    {
        rc = prvCpssSip7QueueToQueueTableGet(devNum,tileNum,sdqNum,i,&valid,&pdqQ,&index);
        if(rc!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }

        cpssOsPrintf("[index %d (0x%08x)]  [SDQ %d  Q %d ]  Valid %d PDQ Q %d \n",index,index,sdqNum,i,valid,pdqQ);
    }

    return GT_OK;
}

GT_STATUS prvCpssSip7TxqPsiBpPdqToSdqDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdqPortNum
)
{
    GT_STATUS rc;
    GT_U32    i,bmp;

    cpssOsPrintf("PDQ port %d \n",pdqPortNum);
    cpssOsPrintf("============\n");


    for(i=0;i< MAX_DP_IN_TILE(devNum);i++)
    {
        rc = prvCpssSip7PsiBpPdqToSdqGet(devNum,tileNum,i,pdqPortNum,&bmp);
        if(rc!=GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
        }

        cpssOsPrintf("[SDQ %d ]  BMP 0x%08x \n",i,bmp);
    }
...
GT_STATUS prvCpssSip7TxqSdqIdDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum
)
{
    ...
    for(i=0;i< MAX_DP_IN_TILE(devNum);i++)
    {
        rc = prvCpssSip7TxqPdsIdGet(devNum,tileNum,i,&id);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsIdGet fail for %d\n", i);
        }

        cpssOsPrintf("[PDS %d ]  ID %d \n",i,id);
    }

    return GT_OK;
}

GT_STATUS prvCpssSip7TxqMailBoxDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
...
{
    GT_STATUS rc;
    PRV_CPSS_TXQ_NPDS2DPDS_CONNECTION_STC connection;
    GT_U32    i,dramQ;


    for(i=0;i< mailBoxNum;i++)
    {
        connection.tile[0] = tileNum;
        connection.pds[0] = pdsNum;
        connection.mailBoxId[0] = i;

        rc = prvCpssSip7TxqPdsMailBoxGet(devNum,tileNum,pdsNum,i,&connection.pds[1],
            &connection.mailBoxId[1],&connection.hiPriority);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsIdGet fail for %d\n", i);
        }

        rc = prvCpssSip7TxqDpdsMailBoxDramQueueGet(devNum,tileNum,pdsNum,i,&dramQ);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqDpdsMailBoxDramQueueGet fail for %d\n", i);
        }

        cpssOsPrintf("[MailBox %d ]  Target Mailbox ID %d Target PDS ID %d HP %d DRAM %d\n",i,connection.mailBoxId[1],
        connection.pds[1],connection.hiPriority,dramQ);
    }

    return GT_OK;
}

GT_STATUS prvCpssSip7TxqEpbNpdsProfileSet
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
    IN  GT_U32   profileInd,
    IN  GT_BOOL  spillOverEnable,
    IN  GT_U32   mb0Id,
    IN  GT_U32   mb1Id
)
{
    ...
    epbProfile.epbSpilloverEnable = spillOverEnable;
    epbProfile.mailBoxId[0] = mb0Id;
    epbProfile.mailBoxId[1] = mb1Id;

    return prvCpssSip7TxqPdsEpbProfileSet(devNum,tileNum,pdsNum,profileInd,&epbProfile);
}

GT_STATUS prvCpssSip7TxqEpbDpdsProfileSet
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
...
)
{
    ...
    epbProfile.type = (GT_TRUE==rx)?PRV_CPSS_DXCH_SIP7_TXQ_EPB_QUEUE_PROFILE_TYPE_DPDS_RX_E:
        PRV_CPSS_DXCH_SIP7_TXQ_EPB_QUEUE_PROFILE_TYPE_DPDS_TX_E;
...
GT_STATUS prvCpssSip7TxqEpbProfilesDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
    IN  GT_U32   firstProfile,
    IN  GT_U32   lastProfile

)
{   GT_STATUS rc;
    PRV_CPSS_DXCH_SIP7_TXQ_EPB_QUEUE_PROFILE_STC  epbProfile;
    GT_CHAR * type2String[]={"NPDS","RX","TX"};
    GT_U32 i;

    if(lastProfile>PDS_PROFILE_MAX_MAC(devNum)-1)
    {
       lastProfile =  PDS_PROFILE_MAX_MAC(devNum)-1;
    }

    for(i=firstProfile;i<=lastProfile;i++)
    {
        rc = prvCpssSip7TxqPdsEpbProfileGet(devNum,tileNum,pdsNum,i,&epbProfile);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsEpbProfileGet fail for profile %d\n", i);
        }

        cpssOsPrintf("[Profile %d ]  Type %s epbSpilloverEnable %d MB ID 0 %d MB ID 1 %d\n",i,type2String[epbProfile.type],
            epbProfile.epbSpilloverEnable,epbProfile.mailBoxId[0],epbProfile.mailBoxId[1]);
    }
...
GT_STATUS prvCpssSip7TxqDebugDeepBufferingModeDump
(
    IN GT_U8 devNum,
    IN GT_PHYSICAL_PORT_NUM portNum
)
{
    GT_STATUS rc;
    GT_CHAR * type2String[]={"DISABLE","ENABLE_LP","ENABLE_HP"};
    PRV_CPSS_DXCH_SIP7_TXQ_EPB_TYPE_ENT mode;

    rc = prvCpssSip7TxqPdsDeepBufferingModeGet(devNum,portNum,&mode);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsDeepBufferingModeGet fail for port %d\n",portNum);
    }

    cpssOsPrintf("[Port %d ]  Type %d (%s)\n",portNum,mode,type2String[mode] );

    return GT_OK;
}

GT_STATUS prvCpssSip7TxqDebugQueueRenamingEnableDump
(
    IN GT_U8 devNum
)
{
    ...
    for(i=0;i<numberOfTiles;i++)
    {
        rc = prvCpssDxChTxqSip7PdxQueueRenamingEnableGet(devNum,i,&enable);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssDxChTxqSip7PdxQueueRenamingEnableSet failed\n");
        }

        cpssOsPrintf("[Tile %d ]  Enable %d \n",i,enable);
    }
...
GT_STATUS prvCpssTxqSip7DebugEpbProfileGet
(
    IN  GT_U8                       devNum,
    IN  GT_PHYSICAL_PORT_NUM        portNum,
    OUT GT_U32                      *pdsProfileIndexPtr,
...
)
{
    ...
    CPSS_NULL_PTR_CHECK_MAC(tilePtr);
    CPSS_NULL_PTR_CHECK_MAC(pdsPtr);
    ...
    firstQueue = aNodePtr->queuesData.queueBase;
    *tilePtr = aNodePtr->queuesData.tileNum;
    *pdsPtr = aNodePtr->queuesData.dp;

    rc = prvCpssDxChTxqFalconPdsQueueProfileMapGet(devNum,*tilePtr,*pdsPtr,firstQueue,pdsProfileIndexPtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssDxChTxqFalconPdsQueueProfileMapGet  failed for portNum  %d  ", portNum);
    }

    rc = prvCpssSip7TxqPdsEpbProfileRawGet(devNum,*tilePtr,*pdsPtr,*pdsProfileIndexPtr,epbSpilloverEnablePtr,
        mailBox0IdPtr,mailBox1IdPtr,typePtr);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqPdsEpbProfileRawGet  failed for portNum  %d  ", portNum);
    }
...
GT_STATUS prvCpssSip7TxqDebugUtilsPnodeShapingParametersSet
(
    IN GT_U8        devNum,
    IN GT_U32       tileIndex,
...
)
{
    ...
    rc = prvCpssSip7TxqUtilsPnodeShapingParametersSet(devNum,tileIndex,pNodeIndex,burstSize,&maxRate);
    if(GT_OK!=rc)
    {
        return rc;
    }
    cpssOsPrintf("  IN     pNodeIndex [%d] maxRate [%d]\n",pNodeIndex,maxRate);


    maxRate =0;
    burstSize =0;

    rc = prvCpssSip7TxqUtilsPnodeShapingParametersGet(devNum,tileIndex,pNodeIndex,&burstSize,&maxRate);
...
GT_STATUS prvCpssSip7DebugEgfQagQueueMapTableDump
(
    IN GT_U8    devNum,
    IN GT_U32   start,
    IN GT_U32   stop
)
{
    ...
    {
        rc = prvCpssSip7EgfQagQueueMapTableGet(devNum,i,&localDpQnum);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7EgfQagQueueMapTableGet  failed for index  %d ", i);
        }
        cpssOsPrintf("\n|%6d|%7d|\n",i,localDpQnum);
        cpssOsPrintf("+------+-------+");
...
GT_STATUS prvCpssSip7DebugEgfQagTargetPortGlobalDpGet
(
    IN GT_U8    devNum,
...
GT_STATUS prvCpssSip7DebugEgfQagPortMapDump
(
    IN GT_U8    devNum,
    IN GT_U32   start,
    IN GT_U32   stop
...
{   GT_STATUS rc=GT_OK;
    ...
    for(i=start;i<=stop;i++)
    {
        rc = prvCpssSip7DebugEgfQagTargetPortGlobalDpGet(devNum,i,&globalDp);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7DebugEgfQagTargetPortGlobalDpGet  failed for index  %d ", i);
        }
        cpssOsPrintf("\n|%6d|%7d|\n",i,globalDp);
        cpssOsPrintf("+------+-------+");
    }
...
GT_STATUS prvCpssDxChTxqSip7DebugDpMappingGet
(
    IN GT_U8     devNum,
    IN GT_U32    tileToRead,
    IN GT_U32    globalDp,
    OUT GT_U32   *tilePtr,
...
{
    GT_STATUS rc;
    GT_U32      regAddr,offset  ,size,regValue = 0;

    CPSS_NULL_PTR_CHECK_MAC(tilePtr);
    CPSS_NULL_PTR_CHECK_MAC(localDpPtr);

    regAddr =PRV_CPSS_DXCH_PP_MAC(devNum)->regsAddrVer1.TXQ.pdx.pds_msg_mapping_register_pds_id;

    size   = TXQ_AAS_PDX_PDS_MSG_MAP_PDS_ID_PDS_MSG_MAP_PDS_ID_0_FIELD_SIZE;

    rc = prvCpssSip6TxqRegisterFieldRead(devNum,tileToRead,regAddr,0,32,&regValue);
    if(rc!=GT_OK)
    {
      CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6TxqRegisterFieldRead  failed \n");
    }

    offset = size*globalDp;

    *localDpPtr = U32_GET_FIELD_MAC(regValue, offset,size);


    regAddr =PRV_CPSS_DXCH_PP_MAC(devNum)->regsAddrVer1.TXQ.pdx.pds_msg_mapping_register_pdx_id;
    size   = TXQ_AAS_PDX_PDS_MSG_MAP_PDX_ID_PDS_MSG_MAP_PDX_ID_0_FIELD_SIZE;
    ...
    *tilePtr = U32_GET_FIELD_MAC(regValue, offset,size);

    return rc;
}


GT_STATUS prvCpssSip7DebugQfcSnapshotPeriodTimerDump
(
    IN GT_U8    devNum,
    IN GT_U32   tileNum,
    IN GT_U32   qfcNum
...
{   GT_STATUS rc=GT_OK;
    ...
    for(i=0;i<=PRV_MAX_QFC_SNAPSHOT_TIMERS_CNS;i++)
    {
        rc = prvCpssSip7TxqQfcPeriodTimerConfigGet(devNum,tileNum,qfcNum,i,&value);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqQfcPeriodTimerConfigGet  failed for index  %d ", i);
        }
        cpssOsPrintf("\n|%6d|%7d|\n",i,value);
        cpssOsPrintf("+------+-------+");
    }
...
GT_STATUS prvCpssSip7DebugQfcProfileAverageCoefficientAndTimerIdDump
(
    IN GT_U8    devNum,
    IN GT_U32   tileNum,
    IN GT_U32   qfcNum
...
{   GT_STATUS rc=GT_OK;
    ...
    cpssOsPrintf("\n+----- +-------+-------+-------+");
    cpssOsPrintf("\n|Prof #| Coeff |TimerId| Shift |");
    cpssOsPrintf("\n+------+-------+-------+-------+");

    for(i=0;i<CPSS_DXCH_SIP_7_MAX_UBURST_PROFILE_NUM_MAC;i++)
    {
        rc = prvCpssSip7TxqQfcProfileAverageCoefficientGet(devNum,tileNum,qfcNum,i,&value[0]);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqQfcPeriodTimerConfigGet  failed for index  %d ", i);
        }

        rc = prvCpssSip7TxqQfcProfileTimerConfigureGet(devNum,tileNum,qfcNum,i,&value[1]);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqQfcProfileTimerConfigureGet  failed for index  %d ", i);
        }

        rc = prvCpssSip7TxqQfcProfileRateCounterShiftGet(devNum,tileNum,qfcNum,i,&value[2]);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip7TxqQfcProfileRateCounterShiftGet  failed for index  %d ", i);
        }

        cpssOsPrintf("\n|%6d|%7d|%7d|%7d|",i,value[0],value[1],value[2]);
        cpssOsPrintf("\n+------+-------+-------+-------+");
    }
...
GT_STATUS prvCpssSip7TxqPdsDynamicProfilesDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
...
{   GT_STATUS rc;
    PRV_CPSS_DXCH_SIP7_TXQ_PDS_DYN_THR_ENABLE_STC  enable;
    GT_U32 i;
    ...
    for(i=firstProfile;i<=lastProfile;i++)
    {
        rc = prvCpssSip7TxqPdsProfileDynamicThresholdEnableGet(devNum,tileNum,pdsNum,i,&enable);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsEpbProfileGet fail for profile %d\n", i);
        }

        cpssOsPrintf("[Profile %d ] Dynamic enable :  CopyB %d SpillOver  %d OpenL %d\n",i,enable.copybackEnable,
            enable.spilloverEnable,enable.openLongEnable);
    }

    return GT_OK;
}

GT_STATUS prvCpssSip7TxqPdsDbaDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum
...
{   GT_STATUS rc;
    GT_U32 i,pbSize,bcTh,bcAlfa,fragSize;
    GT_U32 pdsSize,descTh,descAlfa,selectFactor;

    cpssOsPrintf("NOTE : congestion_facor = (Alpha == 0) ? 0, else 2^(-8+Alpha).\n");
    cpssOsPrintf(" Select factor :\n");
    cpssOsPrintf(" 0x0 = x1; x1; Queue selection Factor by 1\n");
    cpssOsPrintf(" 0x1 = x1p25; x1p25; Queue selection Factor by 1.25\n");
    cpssOsPrintf(" 0x2 = x1p5; x1p5; Queue selection Factor by 1.5\n");
    cpssOsPrintf(" 0x3 = x2; x2; Queue selection Factor by 2\n");


    cpssOsPrintf("\n+----- +--------+---------+------------+-------+-------------+--------+---------+-------------+");
    cpssOsPrintf("\n|Port  |PB Avail|PDS Avail| BC HW Alfa | BC Th |Desc HW Alfa |Desc  Th|Frag Size|Select Factor|");
    cpssOsPrintf("\n+----- +--------+---------+------------+-------+-------------+--------+---------+-------------+\n");

    for(i=0;i<23;i++)
    {
        rc = prvCpssSip7TxqPdsPortPbAvailableSizeGet(devNum,tileNum,pdsNum,i,&pbSize);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortPbAvailableSizeGet fail for port %d\n", i);
        }

        rc = prvCpssSip7TxqPdsPortPdsAvailableSizeGet(devNum,tileNum,pdsNum,i,&pdsSize);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortPdsAvailableSizeGet fail for port %d\n", i);
        }


        rc = prvCpssSip7TxqPdsPortPbDbaParamsGet(devNum,tileNum,pdsNum,i,&bcAlfa,&bcTh);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortPbDbaParamsGet fail for port %d\n", i);
        }

        rc = prvCpssSip7TxqPdsPortPdsDbaParamsGet(devNum,tileNum,pdsNum,i,&descAlfa,&descTh);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortPdsDbaParamsGet fail for port %d\n", i);
        }

        rc = prvCpssSip7TxqPdsPortFragSizeGet(devNum,tileNum,pdsNum,i,&fragSize);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortFragSizeGet fail for port %d\n", i);
        }

        rc = prvCpssSip7TxqPdsPortSelectFactorGet(devNum,tileNum,pdsNum,i,&selectFactor);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsPortSelectFactorGet fail for port %d\n", i);
        }

        cpssOsPrintf("|%6d|0x%-6x|0x%-7x|%12d|%7d|%13d|%8d|%9d|%13d|",i,pbSize,pdsSize,bcAlfa,bcTh,descAlfa,descTh,fragSize,selectFactor);
        cpssOsPrintf("\n+----- +--------+---------+------------+-------+-------------+--------+---------+-------------+\n");
    }
...
GT_STATUS prvCpssSip7TxqPdsProfileDump
(
    IN  GT_U8    devNum,
    IN  GT_U32   tileNum,
    IN  GT_U32   pdsNum,
...
)
{
    ...
    rc = prvCpssDxChTxqFalconPdsProfileGet(devNum, tileNum, pdsNum, profileIndex, GT_FALSE, &profile);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssFalconTxqUtilsPdqQueueProfilesAttributesDump fail\n");
    }

    cpssOsPrintf("\nStatic Thresholds\n");
    cpssOsPrintf("=================\n");

    cpssOsPrintf("longQueueEnable %d headEmptyLimit %d longQueueLimit %d \n\n",longQ.longQueueEnable,
            longQ.headEmptyLimit,longQ.longQueueLimit);

    cpssOsPrintf("Length Adjust\n");
    cpssOsPrintf("=================\n");

    cpssOsPrintf("lengthAdjustEnable %d lengthAdjustSubstruct %d longQueueLimit %d \n\n",lengthAdjust.lengthAdjustEnable,
            lengthAdjust.lengthAdjustSubstruct,lengthAdjust.lengthAdjustByteCount);
    ...
    cpssOsPrintf("\nDynamic Thresholds\n");
    cpssOsPrintf("=================\n");
    rc = prvCpssSip7TxqPdsDynamicProfilesDump(devNum,tileNum,pdsNum,profileIndex,profileIndex);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsDynamicProfilesDump fail\n");
    }

    cpssOsPrintf("\nSpill over\n");
    cpssOsPrintf("=================\n");
    rc = prvCpssSip7TxqEpbProfilesDump(devNum,tileNum,pdsNum,profileIndex,profileIndex);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsDynamicProfilesDump fail\n");
    }
...
GT_STATUS prvCpssSip7TxqSdqProfileDump
(
    IN  GT_U8  devNum,
    IN  GT_U32 tileNumber,
    IN  GT_U32 sdqNum,
...
)
{
    ...
    for(i=0;i<PRV_CPSS_DXCH_TXQ_SIP_7_SDQ_PROFILE_ATTRIBUTE_LAST_E;i++)
    {
        rc = prvCpssSip7TxqSdqProfileGet(devNum,tileNumber,sdqNum,i,profile,&value);
        if (rc != GT_OK)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "prvCpssSip7TxqPdsDynamicProfilesDump fail\n");
        }
...
GT_STATUS prvCpssSip7TxqUtilsSdqQueueProfilesBindDump
(
    IN GT_U8 devNum,
    IN GT_U32 tileNumber,
    IN GT_U32 pdsNum,
...
)
{
    ...
    {
        rc = prvCpssSip7TxqSdqMapQueueToProfileGet(devNum, tileNumber, pdsNum, i, &profile);
        if (rc != GT_OK)
        {
            cpssOsPrintf("%d = prvCpssSip7TxqSdqMapQueueToProfileGet\n", rc );
            return rc;
        }
