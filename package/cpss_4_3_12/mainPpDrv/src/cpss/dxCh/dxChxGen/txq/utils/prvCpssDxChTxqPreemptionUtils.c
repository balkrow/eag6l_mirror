    ...
    *actAsPreemptiveChannelPtr = GT_FALSE;
    ...
    if(GT_FALSE == isCascade)
    {
        ...
        if((GT_TRUE==aNodePtr->preemptionActivated)&&
            (PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_PRE_E==aNodePtr->preemptionType))
         {
...
)
{
    ...
    {
      *enablePtr  = prvCpssSip6TxqPreemptionCapableDeviceGet(devNum);
    }
    else
    {
      *enablePtr  = GT_FALSE;
...
)
{
    ...
    for(numberOfHandledQueues =0,i=aNodePtr[1]->queuesData.pdqQueueFirst;i<=aNodePtr[1]->queuesData.pdqQueueLast;numberOfHandledQueues++,i++)
    {
      ...
      rc      = prvCpssFalconTxqPdqMinBwEnableSet(devNum, aNodePtr[1]->queuesData.tileNum,i,
          ...
          (aNodePtr[1]->queuesData.restoreData[i-aNodePtr[1]->queuesData.pdqQueueFirst].minBwType==0)?GT_FALSE:GT_TRUE);
      if (rc != GT_OK)
      {
          return rc;
      }

      rc = prvCpssSip6TxqPdqNodeShapingCreditAccumulationEnableSet(devNum, aNodePtr[1]->queuesData.tileNum,i,
          aNodePtr[1]->queuesData.restoreData[i-aNodePtr[1]->queuesData.pdqQueueFirst].creditAccumulation);
      if (rc != GT_OK)
      {
          return rc;
      }
    ...
    /*restore QBV*/
    if(GT_TRUE == prvCpssSip6TxqQbvCapableDeviceGet(devNum))
    {
      rc = prvCpssSip6_30TxqSdqLocalPortQbvEnableSet(devNum,
                   aNodePtr[1]->queuesData.tileNum,
                   aNodePtr[1]->queuesData.dp,
                   aNodePtr[1]->queuesData.localPort,
                   aNodePtr[1]->queuesData.restoreQbvEnable);
      ...
      for(numberOfHandledQueues =0,i=0;i<numberOfMappedQueues;numberOfHandledQueues++,i++)
      {
        if(numberOfHandledQueues>=PRV_CPSS_DXCH_SIP_6_MAX_TC_QUEUE_TO_RESTORE_NUM_MAC)
        {
            break;
       ...
       rc = prvCpssSip6_30TxqSdqQueueGateSet(devNum,aNodePtr[1]->queuesData.tileNum,aNodePtr[1]->queuesData.dp,
        aNodePtr[1]->queuesData.queueBase +i,aNodePtr[1]->queuesData.restoreData[i].qbvGate);
       if (rc != GT_OK)
       {
...
)
{
    ...
    GT_U32  numberOfHandledQueues = 0,numberOfMappedQueues;
    ...
    numberOfMappedQueues = aNodePtr[0]->queuesData.pdqQueueLast - aNodePtr[0]->queuesData.pdqQueueFirst+1;
    ...
    for(j=0;j<2;j++)
    {
      for(numberOfHandledQueues =0,i=aNodePtr[j]->queuesData.pdqQueueFirst;i<=aNodePtr[j]->queuesData.pdqQueueLast;i++,numberOfHandledQueues++)
      {
          ...
          rc      = prvCpssFalconTxqPdqMinBwEnableGet(devNum, aNodePtr[j]->queuesData.tileNum,i,
              ...
              &(aNodePtr[j]->queuesData.restoreData[i-aNodePtr[j]->queuesData.pdqQueueFirst].minBwType));
          if (rc != GT_OK)
          {
              return rc;
          }
          ...
          rc = prvCpssSip6TxqPdqNodeShapingCreditAccumulationEnableGet(devNum, aNodePtr[j]->queuesData.tileNum,i,
            &(aNodePtr[j]->queuesData.restoreData[i-aNodePtr[j]->queuesData.pdqQueueFirst].creditAccumulation));
          if (rc != GT_OK)
          {
              return rc;
          }
    ...
    for(numberOfHandledQueues =0,i=aNodePtr[1]->queuesData.pdqQueueFirst;i<=aNodePtr[1]->queuesData.pdqQueueLast;i++,numberOfHandledQueues++)
    {
        ...
        rc      = prvCpssFalconTxqPdqMinBwEnableSet(devNum, aNodePtr[1]->queuesData.tileNum,i,
            ...
            aNodePtr[0]->queuesData.restoreData[i-aNodePtr[1]->queuesData.pdqQueueFirst].minBwType==0?GT_FALSE:GT_TRUE);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        rc = prvCpssSip6TxqPdqNodeShapingCreditAccumulationEnableSet(devNum, aNodePtr[1]->queuesData.tileNum,i,
        aNodePtr[0]->queuesData.restoreData[i-aNodePtr[1]->queuesData.pdqQueueFirst].creditAccumulation);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    if(GT_TRUE==txLibInitDone)
    {
       ...
       for(numberOfHandledQueues =0,i=aNodePtr[1]->queuesData.pdqQueueFirst;i<=aNodePtr[1]->queuesData.pdqQueueLast;i++,numberOfHandledQueues++)
       {
        ...
        {
            /*save QBV enable*/
            rc = prvCpssSip6_30TxqSdqLocalPortQbvEnableGet(devNum,
                aNodePtr[1]->queuesData.tileNum,
                aNodePtr[1]->queuesData.dp,
                aNodePtr[1]->queuesData.localPort,
                &aNodePtr[1]->queuesData.restoreQbvEnable);
            ...
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqLocalPortQbvEnableGet failed  ");
            }

             /*copy  QBV enable*/
             rc = prvCpssSip6_30TxqSdqLocalPortQbvEnableGet(devNum,
                aNodePtr[0]->queuesData.tileNum,
                aNodePtr[0]->queuesData.dp,
                aNodePtr[0]->queuesData.localPort,
                &aNodePtr[0]->queuesData.restoreQbvEnable);
             ...
             for(numberOfHandledQueues = 0,i=0;i<numberOfMappedQueues;numberOfHandledQueues++,i++)
             {
               if(numberOfHandledQueues>=PRV_CPSS_DXCH_SIP_6_MAX_TC_QUEUE_TO_RESTORE_NUM_MAC)
               {
                ...
                /*save QBV map*/
                rc = prvCpssSip6_30TxqSdqQueueGateGet(devNum,aNodePtr[1]->queuesData.tileNum,aNodePtr[1]->queuesData.dp,
                 aNodePtr[1]->queuesData.queueBase +i,&aNodePtr[1]->queuesData.restoreData[i].qbvGate);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqQueueGateSet failed  ");
                ...
                rc = prvCpssSip6_30TxqSdqQueueGateGet(devNum,aNodePtr[0]->queuesData.tileNum,aNodePtr[0]->queuesData.dp,
                 aNodePtr[0]->queuesData.queueBase +i,&aNodePtr[0]->queuesData.restoreData[i].qbvGate);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqQueueGateSet failed  ");
                }
                ...
                rc = prvCpssSip6_30TxqSdqQueueGateSet(devNum,aNodePtr[1]->queuesData.tileNum,aNodePtr[1]->queuesData.dp,
                 aNodePtr[1]->queuesData.queueBase +i,aNodePtr[0]->queuesData.restoreData[i].qbvGate);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, " prvCpssSip6_30TxqSdqQueueGateSet failed  ");
                }
...
static GT_VOID prvCpssDxChTxqSip7PreemptionEnableSet
(
    IN GT_U8   devNum,
    IN GT_PHYSICAL_PORT_NUM physicalPortNum,
    IN GT_BOOL enable
)
{
    ...
    index = physicalPortNum>>5;
    bit   = physicalPortNum&0x1F;

    U32_SET_FIELD_MAC(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).preemptionEnableShadow[index],bit,1,((enable == GT_TRUE) ? 1 : 0));
...
)
{
    ...
    if(currentState!=enable)
    {
        ...
        {
            prvCpssDxChTxqSip7PreemptionEnableSet(devNum, physicalPortNum,enable);
        }
...
static GT_STATUS prvCpssDxChTxqSip7PreemptionEnableGet
(
    IN GT_U8   devNum,
    IN GT_PHYSICAL_PORT_NUM physicalPortNum,
    OUT GT_BOOL *enablePtr
)
{
    ...
    data = U32_GET_FIELD_MAC(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).preemptionEnableShadow[index],
        bit,1);
...
)
{
    ...
    *enablePtr = GT_FALSE;
    ...
    if(GT_TRUE == preemptionFeatureAllowed)
    {
        ...
        {
            return prvCpssDxChTxqSip7PreemptionEnableGet(devNum, physicalPortNum, enablePtr);
        }
    }

    return GT_OK;
}
GT_STATUS  prvCpssDxChTxqSip6_10PrCouplesStartOffsetAndSizeGet
(
    IN GT_U8                devNum,
    OUT GT_U32              *startIndexPtr,
    OUT GT_U32              *sizePtr
)
{
    CPSS_NULL_PTR_CHECK_MAC(startIndexPtr);
    ...
    if (TXQ_IS_PREEMPTIVE_DEVICE(devNum))
    {
        ...
        {
         *startIndexPtr = TXQ_AAS_DP_MAX_PREEMPTIVE_COUPLES_FIRST_MAC;
         *sizePtr = TXQ_AAS_DP_MAX_PREEMPTIVE_COUPLES_NUM_MAC;
        }
        else if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum) == GT_TRUE)
        {
         *startIndexPtr = TXQ_IRONMAN_DP_MAX_PREEMPTIVE_COUPLES_FIRST_MAC;
         *sizePtr = TXQ_IRONMAN_DP_MAX_PREEMPTIVE_COUPLES_NUM_MAC;
        }
        else if(PRV_CPSS_SIP_6_20_CHECK_MAC(devNum) == GT_TRUE)
        {
         *startIndexPtr = TXQ_HARRRIER_DP_MAX_PREEMPTIVE_COUPLES_FIRST_MAC;
         *sizePtr = TXQ_HARRRIER_DP_MAX_PREEMPTIVE_COUPLES_NUM_MAC;
        }
        else
        {
         *startIndexPtr = TXQ_AC5P_DP_MAX_PREEMPTIVE_COUPLES_FIRST_MAC;
         *sizePtr = TXQ_AC5P_DP_MAX_PREEMPTIVE_COUPLES_NUM_MAC;
        }
    }
    else
    {
...
)
{
    ...
    *prePortPtr = CPSS_SIP6_TXQ_INVAL_DATA_CNS;
    ...
    rc = prvCpssDxChTxqSip6_10PrCouplesStartOffsetAndSizeGet(devNum,&startIndex,&size);
    if(rc!=GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED,"Device %d does not support preemption couples",devNum);
    }
...
)
{
    ...
    *preANodePtr = NULL;
    ...
    /*check  validity*/
     if(expANodePtr->preemptionType!=PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_EXP_E)
     {
     ...
     /*sanity check*/
     if((*preANodePtr)->preemptionType!=PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_PRE_E)
     {
...
)
{
    ...
    const GT_U32    maxSpeed[] =
    {
        ...
        TXQ_AAS_DP_PREEMPTIVE_COUPLES_MAX_SPEED_MAC,
    };
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
     startIndex = TXQ_AAS_DP_MAX_PREEMPTIVE_COUPLES_FIRST_MAC;
     size = TXQ_AAS_DP_MAX_PREEMPTIVE_COUPLES_NUM_MAC;
    }
    else if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum) == GT_TRUE)
    {
...
)
{
    ...
    if((GT_FALSE == PRV_CPSS_PP_MAC(devNum)->isGmDevice)&&(prvCpssSip6TxqPreemptionCapableDeviceGet(devNum)))
    {
                ...
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                     ...
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brTxFragCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brTxFragCount;
                        break;
                    default:
            ...
            case CPSS_MTI_BR_RX_FRAG_COUNT_RX_FRAG_COUNT_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxFragCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxFragCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_TX_HOLD_COUNT_TX_HOLD_COUNT_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brTxHoldCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brTxHoldCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_RX_SMD_ERR_COUNT_RX_SMD_ERR_COUNT_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxSmdErrCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxSmdErrCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_RX_ASSY_ERR_COUNT_RX_ASSY_ERR_COUNT_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxAssyErrCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxAssyErrCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_RX_ASSY_OK_COUNT_RX_ASSY_OK_COUNT_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxAssyOkCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxAssyOkCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_RX_VERIFY_COUNT_RX_VERIFY_COUNT_BAD_E:
                ...
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxVerifyCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxVerifyCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_RX_RESPONSE_COUNT_RX_RESP_COUNT_BAD_E:
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brRxResponseCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brRxResponseCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
            ...
            case CPSS_MTI_BR_TX_VERIF_COUNT_TX_RESP_COUNT_E:
                ...
                switch(macType)
                {
                    case PRV_CPSS_PORT_MTI_100_E:
                        ...
                        break;
                     case PRV_CPSS_PORT_MTI_USX_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_USX_MAC.BR.brTxVerifCount;
                        break;
                     case PRV_CPSS_PORT_MTI_CPU_E:
                        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->GOP.MTI[portMacNum].MTI_CPU_MAC.BR.brTxVerifCount;
                        break;
                    default:
                        regAddr = PRV_CPSS_SW_PTR_ENTRY_UNUSED;
                        break;
...
GT_BOOL prvCpssTxqSip6MacTypeSupportPreemptionGet
(
    IN GT_U8                                devNum,
...
)
{
    ...
    GT_BOOL macTypeSupportPreemption;
    ...
    if (TXQ_IS_PREEMPTIVE_DEVICE(devNum))
    {
        if(GT_FALSE == PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            ...
            {
                case PRV_CPSS_PORT_MTI_100_E:
                    macTypeSupportPreemption = GT_TRUE;
                    break;
                default :
                    macTypeSupportPreemption = GT_FALSE;
                    break;
            }
        }
        else
    ...
    return macTypeSupportPreemption;
}


GT_STATUS prvCpssTxqSip6_30_IsDeviceIsIronmanMGet
(
    IN GT_U8                                devNum,
    IN GT_BOOL                              *isMType
)
{
    CPSS_NULL_PTR_CHECK_MAC(isMType);
