...
)
{
    ...
    *pvar = (*pvar & mask) | uvalue;
    ...
    if(valueWidth>width)
    {
        CPSS_LOG_ERROR_MAC("Error in setting field %s .Expected witdth %d < received width %d (value %d).\n",fieldName,width,valueWidth,originalValue);
