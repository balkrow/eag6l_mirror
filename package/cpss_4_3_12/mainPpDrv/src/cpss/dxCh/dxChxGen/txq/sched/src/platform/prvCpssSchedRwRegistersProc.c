...
int prvSchedPrintRxRegistersLogStatus(GT_U8 devNum)
{
    schedPrintf(" --------------------\n");
    schedPrintf("\n  print write calls : ");
    schedPrintf(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogWriteRequests? "yes" : "no");
    schedPrintf("\n  print read  calls : ");
    schedPrintf(PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogReadRequests? "yes" : "no");
    schedPrintf("\n");
    return 0;
}
...
int prvSchedSetRegistersLogStatus(GT_U8 devNum,int write_status, int read_status)
{
    ...
    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogWriteRequests = write_status?GT_TRUE:GT_FALSE;
    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogReadRequests = read_status?GT_TRUE:GT_FALSE;
    return 0;
}
...
GT_BOOL  prvSchedPrintRegistersLogEnableGet(GT_U8 devNum,GT_BOOL read)
{
    if(GT_TRUE==read)
    {
        return PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogReadRequests;
    }

    return PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).schedDbg.schedLogWriteRequests;
}
