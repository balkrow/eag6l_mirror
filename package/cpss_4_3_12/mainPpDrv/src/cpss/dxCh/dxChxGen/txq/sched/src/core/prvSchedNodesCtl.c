...
int     prvSchedNodesCtlFreeQueue(struct prvCpssDxChTxqSchedShadow *ctl, uint32_t index, tm_node_status_t node_status,GT_BOOL force)
{
    struct tm_queue     *queue=&(ctl->tm_queue_array[index]);
...
/**
 */
int prvSchedDbDeleteANodeProc(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index)
{
    ...
    a_node->mapping.nodeStatus = TM_NODE_FREE;

    return 0;
}


int prvSchedANodeUpdateChildren(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index,GT_U32 firstChild,GT_U32 lastChild)
{
    ...
    a_node->mapping.childLo = firstChild;
    a_node->mapping.childHi= lastChild;
    a_node->mapping.nodeStatus = TM_NODE_USED;

    if (prvSchedLowLevelSetHwNodeMapping(hndl,SCHED_A_LEVEL, index))return TM_HW_A_NODE_CONFIG_FAIL;

    return 0;
}


int prvSchedBNodeUpdateChildren(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index,GT_U32 firstChild,GT_U32 lastChild)
{
    ...
    b_node->mapping.childLo = firstChild;
    b_node->mapping.childHi= lastChild;

    if (prvSchedLowLevelSetHwNodeMapping(hndl,SCHED_B_LEVEL, index))return TM_HW_A_NODE_CONFIG_FAIL;

    return 0;
}


int prvSchedQNodeUpdateParent(PRV_CPSS_SCHED_HANDLE hndl, GT_U32 firstChild,GT_U32 lastChild,GT_U32 parent)
{

    struct tm_queue *q_node = NULL;
    GT_U32          i;
    ...
    {
        q_node = &(ctl->tm_queue_array[i]);

        q_node->mapping.nodeParent=parent;

        q_node->elig_prio_func =PRV_SCHED_ELIG_Q_PRIO0;

        if (prvSchedLowLevelSetHwNodeMapping(hndl,SCHED_Q_LEVEL, i))return TM_HW_A_NODE_CONFIG_FAIL;

        if (prvSchedLowLevelSetNodeEligPriorityFunc(hndl, SCHED_Q_LEVEL, i))return TM_HW_A_NODE_CONFIG_FAIL;
    }
...
int prvSchedANodeUpdateParent(PRV_CPSS_SCHED_HANDLE hndl, GT_U32 firstChild,GT_U32 lastChild,GT_U32 parent)
{
    ...
    for(i=firstChild;i<=lastChild;i++)
    {
        a_node = &(ctl->tm_a_node_array[i]);

        a_node->mapping.nodeParent=parent;

        a_node->elig_prio_func =PRV_SCHED_ELIG_N_FP0;

        if (prvSchedLowLevelSetHwNodeMapping(hndl,SCHED_A_LEVEL, i))return TM_HW_A_NODE_CONFIG_FAIL;

        if (prvSchedLowLevelSetNodeEligPriorityFunc(hndl, SCHED_A_LEVEL, i))return TM_HW_A_NODE_CONFIG_FAIL;
    }
