...
)
{
    ...
    GT_U32                          speed_in_G;
    GT_U32                         lengthAdjustResored [CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES]={0};
         ...
         for(dpNum=0;dpNum<MAX_DP_IN_TILE(devNum);dpNum++)
         {
            for(localPortNum=0;localPortNum<CPSS_DXCH_SIP_6_MAX_LOCAL_PORT_NUM_MAC(devNum);localPortNum++)
            {
                ...
                if(CPSS_SIP6_TXQ_INVAL_DATA_CNS!=physicalPortNum)
                {
                    ...
                    firstQInd = pNodePtr->aNodelist[0].queuesData.queueBase;
                    ...
                    {
                      rc = prvCpssDxChTxqFalconPdsQueueLengthAdjustProfileMapGet(devNum,tileNum,dpNum,firstQInd,&lengthAdjustIndex);
                      if(rc !=GT_OK)
                      {
                         return rc;
                      }
                    ...
                    /*ironMan doesnt support speed profiles, no need to recover */
                    ...
                    rc = prvCpssDxChTxqFalconPdsQueueProfileMapGet(devNum,tileNum,dpNum,firstQInd,&profileIndex);
                    if(rc !=GT_OK)
                    {
                       return rc;
                    }
...
)
{
   ...
   PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC lengthAdjust[CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES];
   PRV_CPSS_DXCH_SIP6_TXQ_QUEUE_PROFILE_LENGTH_ADJUST_STC * lengthAdjustParametersPtr;
   PRV_CPSS_DXCH_SIP6_TXQ_QUEUE_PROFILE_LENGTH_ADJUST_STC  lengthAdjustParametersArr[CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES];

   cpssOsMemCpy(pdsProfiles,PRV_CPSS_DXCH_PP_MAC(devNum)->port.pdsProfiles,sizeof(PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC)*CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES);
   cpssOsMemCpy(lengthAdjust,PRV_CPSS_DXCH_PP_MAC(devNum)->port.lengthAdjustProfiles,sizeof(PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC)*CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES);

   for(i=0;i<CPSS_DXCH_SIP_6_MAX_PDS_PROFILE_NUM_ALL_DEVICES;i++)
   {
   ...
   rc = prvCpssFalconTxqUtilsInitPdsProfilesDb(devNum,PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).numberOfMappedPorts);
   if(rc !=GT_OK)
   {
      return rc;
   }
...
)
{
    ...
    GT_STATUS rc;
    PRV_CPSS_DXCH_SIP6_TXQ_PDS_QUEUE_PROFILE_STC profileFromHw;
    PRV_CPSS_DXCH_SIP6_TXQ_QUEUE_PROFILE_LENGTH_ADJUST_STC lengthAdjustParameters;
    PRV_CPSS_DXCH_SIP6_TXQ_QUEUE_PROFILE_LONG_Q_STC        longQueueParameters;
