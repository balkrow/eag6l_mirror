...
int prvCpssSchedUpdateNodeTypeA(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index,
                     struct sched_a_node_params *params, PRV_CPSS_NODE_UPDATE_TYPE_ENT updateType)
{
    int rc;
    int i;
    uint8_t fl_changed = 0;
    ...
    if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_ALL_E)
     {
      ...
      else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_QUANTUM_E)
      {
         if (params->quantum != TM_INVAL_16_BIT)
         {
             PRV_SCHED_VALIDATE_NODE_QUANTUM_VALUE(params->quantum, TM_CONF_A_QUANTUM_OOR,rc)
      ...
      else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_ELIG_FUNC_E)
      {
            UPDATE_NODE_ELIGIBLE_FUNCTION(SCHED_A_LEVEL,ctl->tm_elig_prio_a_lvl_tbl, fl_changed = 1)
...
int prvCpssSchedUpdateNodeTypeB(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index,
                     struct sched_b_node_params *params,PRV_CPSS_NODE_UPDATE_TYPE_ENT updateType)
{
    int rc;
    int i;
    uint8_t fl_changed = 0;
    ...
    else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_QUANTUM_E)
    {
       if (params->quantum != TM_INVAL_16_BIT)
       {
           ...
           /* Update SW DB */
           node->dwrr_quantum = params->quantum;
          if (prvSchedLowLevelSetBNodeQuantum(hndl, index))
          {
              rc = TM_HW_A_NODE_CONFIG_FAIL;
              goto out;
          }
    ...
    else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_ELIG_FUNC_E)
    {
          ...
          if (fl_changed)
          {
              if(prvSchedLowLevelSetNodeEligPriorityFunc(hndl, SCHED_B_LEVEL, index))
              {
                  rc = TM_HW_B_NODE_CONFIG_FAIL;
                  goto out;
              }
...
int prvCpssSchedUpdateNodeTypeC(PRV_CPSS_SCHED_HANDLE hndl, uint32_t index,
                     struct sched_c_node_params *params,PRV_CPSS_NODE_UPDATE_TYPE_ENT updateType)
{
   ...
   else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_QUANTUM_E)
   {
      if (params->quantum != TM_INVAL_16_BIT)
      {
          ...
          /* Update SW DB */
         node->dwrr_quantum = params->quantum;
         if (prvSchedLowLevelSetCNodeQuantum(hndl, index))
         {
             rc = TM_HW_C_NODE_CONFIG_FAIL;
             goto out;
         }
   ...
   else if(updateType == PRV_CPSS_NODE_UPDATE_TYPE_ELIG_FUNC_E)
   {
         ...
         if (fl_changed)
         {
             if(prvSchedLowLevelSetNodeEligPriorityFunc(hndl, SCHED_C_LEVEL, index))
             {
                 rc = TM_HW_A_NODE_CONFIG_FAIL;
                 goto out;
             }
                           ...
                           uint32_t ebs)
{
    ...
    rc = prvSchedShapingUtilsCalculatePortShaping(PRV_SCHED_DEV_NUM_GET(ctl),1000.0*port_cir_bw/ctl->level_data[SCHED_P_LEVEL].unit,
