...
#define PRV_PORT_MAX_BURST_SIZE(_devNum) (PRV_CPSS_SIP_7_CHECK_MAC(_devNum))?0xfff:0x1ffff
                                ...
                                uint8_t port_index)
{
    ...
    {
        /* no shaping for port */
        port->cir_burst_size = PRV_PORT_MAX_BURST_SIZE(PDQ_ENV_DEVNUM_NUM(ctl));
        port->eir_burst_size = PRV_PORT_MAX_BURST_SIZE(PDQ_ENV_DEVNUM_NUM(ctl));
    }
...
{
    return  tm_create_asym_port_ex(hndl, port_index,  params,firstChildInRange);
}
...
out:
    if (rc)
    {
        if (rc == TM_HW_QUEUE_CONFIG_FAIL)
        {
            ...
            prvSchedNodesCtlFreeQueue(ctl,*queue_index_ptr,old_node_status,GT_FALSE);       /* restore queue mapping */
        }
    }
    return rc;
}
...
)
{
    ...
    if(optPtr->addToExistingBnode==GT_FALSE)
    {
        ...
        if (rc)
        {
            CPSS_TXQ_DEBUG_LOG("Error %d for tile %d , port index %d addToExistingCnode %d",rc,tileNum,port_index,optPtr->addToExistingCnode);
            return rc;
        }
    }
...
)
{
    ...
    if(optPtr->addToExistingCnode==GT_FALSE)
    {
        rc = prvCpssSchedNodesCreateCNodeToPort(hndl, port_index,optPtr->explicitBnodeInd,optPtr->bNodeInd, c_params, c_node_index_ptr);
        if (rc) return rc;
    }
...
{
    return tm_create_c_node_to_port_ex(hndl, parent_port_index, c_params, explicitBnodeInd?bNodeInd:(uint32_t)NO_PREDEFINED_RANGE,c_node_index_ptr);
}
