...
#include <cpss/dxCh/dxChxGen/txq/private/regFiles/seahawk/central/prvCpssAasTxqPfccRegFile.h>
...
#define PRV_PFCC_SIP_6_30_DEV_DEP_FIELD_GET_MAC(_dev,_fieldName) \
    (PRV_CPSS_SIP_7_CHECK_MAC(_dev)?TXQ_AAS_##_fieldName:TXQ_IRONMAN_##_fieldName)


#define PRV_PFCC_SIP_6_20_DEV_DEP_FIELD_GET_MAC(_dev,_fieldName) \
    (PRV_CPSS_SIP_6_30_CHECK_MAC(_dev)?PRV_PFCC_SIP_6_30_DEV_DEP_FIELD_GET_MAC(_dev,_fieldName):TXQ_HARRIER_##_fieldName)
...
#define SIP6_SHARED_EXT_POOLS_NUM_CNS 2
...
GT_BOOL prvCpssFalconTxqPfccPoolIdValid
(
    IN  GT_U8                                devNum,
    IN  GT_U32                               poolId
)
...
#endif
    GT_U32     availeblePools ;

    if( poolId >= SIP6_TXQ_PFCC_SHARED_MAX_POOLS_NUM_CNS)
    {
        CPSS_LOG_ERROR_MAC("Pool id is oout of range %d \n",poolId);
        return GT_FALSE;
    }

    if((1== poolId)&&
        (NULL != PRV_CPSS_DXCH_PP_MAC(devNum)->port.tileConfigsPtr)&&
               (CPSS_PORT_TX_POOL_MODE_ONE_LOSSY_TWO_LOSLESS_E ==
                    PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode))
    {
       CPSS_LOG_ERROR_MAC("Pool %d is reserved in %s",poolId,
           mode2str[PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode]);
       return GT_FALSE;
    }

    if(GT_FALSE==PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        if(poolId>=SIP6_SHARED_TAIL_DROP_POOLS_NUM_CNS)
        {
            /*init systen was not done yet ,we are at the default mode*/
            if(NULL == PRV_CPSS_DXCH_PP_MAC(devNum)->port.tileConfigsPtr)
            {
                CPSS_LOG_ERROR_MAC( "Pool %d can not be configured in \
                    CPSS_PORT_TX_POOL_MODE_TWO_LOSSY_E (default) mode.",poolId);
                return GT_FALSE;
            }
            else
            {
                 availeblePools = SIP6_SHARED_TOTAL_POOLS_NUM_CNS(devNum);

                 if( poolId >= availeblePools)
                 {
                    CPSS_LOG_ERROR_MAC( "Pool %d can not be configured in \
                     %s mode [availeble pools %d].",poolId,
                     mode2str[PRV_CPSS_TXQ_PER_DEVICE_DB_MAC(devNum).poolMode],availeblePools);
                    return GT_FALSE;
                 }
            }
        }
...
)
{
    ...
    GT_U32  chunkNum,valueToWrite,fieldSize ;
    PRV_CPSS_DXCH_TXQ_SIP_6_REG_LOG_STC log;
    ...
    if( GT_FALSE==prvCpssFalconTxqPfccPoolIdValid(devNum,poolId))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    for(tileNum=0;tileNum<numberOfTiles;tileNum++)
    {
        ...
        if(oldPoolId!=SIP6_TXQ_PFCC_SHARED_MAX_POOLS_NUM_CNS)
        {
            offset = trafClass;
    ...
    /*Now sync all the EGF entries that are mapped to this tc4Pfc.
        ...
        |Pool that TC is mapped at CPSS   INPUT         |                                      |                                                         |
        |(cpssDxChPortTxTcMapToSharedPoolSet)   | Pool mapped at txQ  |          Pool mapped at EGF table   |
        ======================================================================
       | 0                                                                            |      0                             |                     0                                   |
       | 1                                                                            |      1                             |                     1                                   |
       | 2                                                                            |      BAD PARAM          |                     BAD PARAM                |
       | 3                                                                            |      BAD PARAM          |                     BAD PARAM                |



        CPSS_PORT_TX_POOL_MODE_ONE_LOSSY_TWO_LOSLESS_E

        Limitation : Pool1 set to maximal threshold at TD and reserved.

         |Pool that TC is mapped at CPSS   INPUT        |                                      |                                                         |
        |(cpssDxChPortTxTcMapToSharedPoolSet)   | Pool mapped at txQ  |          Pool mapped at EGF table   |
        ======================================================================
       | 0                                                                            |      0                             |                     0                                   |
       | 1                                                                            |      BAD PARAM          |                     BAD PARAM                |
       | 2                                                                            |      2                             |                     1                                   |
       | 3                                                                            |      3                             |                     1                                   |
        ...
        Limitation : Pool0 and Pool1 set to maximal threshold at TD
    ...
    if(oldPoolId==!poolId)
    {
        ...
        if(GT_FALSE==PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            chunkNum = PRV_SIP_6_EGF_QAG_TC_DP_MAPPER_SIZE>>5;
            valueToWrite = (poolId==0)?0:1;
            fieldSize = 1;
        }
        else
        {
            chunkNum = PRV_SIP_7_EGF_QAG_TC_DP_MAPPER_SIZE>>5;
            valueToWrite = poolId;
            fieldSize = 2;
        }
           ...
           for(j=0;chunkValue&&(j<32);j++,chunkValue>>=1)
           {
               if(chunkValue&0x1)
               {
                   ...
                   rc = prvCpssDxChWriteTableEntryField(devNum,
                       ...
                       (i<<5)+j,
                       PRV_CPSS_DXCH_TABLE_WORD_INDICATE_GLOBAL_BIT_CNS,9,fieldSize,
                       valueToWrite);
...
)
{
    ...
    *poolIdPtr=SIP6_TXQ_PFCC_SHARED_MAX_POOLS_NUM_CNS;/*unmapped by default*/
    ...
    else if((regValue>>TXQ_PFCC_FIELD_NAME_GET(devNum,PFCC_POOL_CFG_POOL_2_TC_EN_VEC_FIELD_OFFSET))&(1<<trafClass))
    {
        *poolIdPtr=2;
    }
...
)
{
    ...
    extPoolId+= SIP6_SHARED_REGULAR_TD_POOLS_NUM_CNS;
