...
int prvCpssTxqSchedulerGetOptimalResolutionExponent(enum schedLevel level,uint8_t * exponent)
{
    ...
    case SCHED_C_LEVEL:
        *exponent = FALCON_C_NODE_OPTIMAL_RES_EXP;
        break;
    case SCHED_P_LEVEL:
        *exponent = FALCON_P_NODE_OPTIMAL_RES_EXP;
        break;
...
int prvCpssTxqSchedulerGetMaximalToken(GT_U8 devNum,unsigned int * maximal_token)
{
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
        *maximal_token = (1<<SIP7_TOKEN_WIDTH_BITS)-1;
    }
    else
    {
        *maximal_token = (1<<SIP6_TOKEN_WIDTH_BITS)-1;
    }
    return 0;
}
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) == GT_TRUE)
    {
        *optimal_shaper_update_frequency = SEAHAWK_OPTIMAL_NODE_FREQUENCY;
    }
    else if(PRV_CPSS_SIP_6_30_CHECK_MAC(devNum) == GT_TRUE)
    {
            *optimal_shaper_update_frequency = IRONMAN_OPTIMAL_NODE_FREQUENCY;
    }
    else
    {
...
int prvCpssTxqSchedulerGetDividerExp
(
    GT_U8 devNum,
...
)
{
    ...
    {
        *dividerExp = MAX_DIVIDER_EXP;
    }
    else
    {
        if(SCHED_P_LEVEL==level)
        {
            *dividerExp = SIP_7_P_LEVEL_MAX_DIVIDER_EXP;
        }
        else
        {
            *dividerExp = SIP_7_MAX_DIVIDER_EXP;
        }
    }
...
)
{
    ...
    case SCHED_Q_LEVEL:
        ...
        else if(GT_TRUE == PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            *unit = IRONMAN_Q_UNIT;
        }
        else
    ...
    case SCHED_C_LEVEL:
        ...
        {
            *unit = isSip6_10?(isSip6_15?PHOENIX_C_UNIT:HAWK_C_UNIT):FALCON_C_UNIT;
        }
        break;
    case SCHED_P_LEVEL:
        if(GT_TRUE == PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        ...
        {
            *unit = isSip6_10?(isSip6_15?PHOENIX_P_UNIT:HAWK_P_UNIT):FALCON_P_UNIT;
        }
        break;
    default:
        return -EINVAL;
...
int prvCpssTxqSchedulerConfigurePeriodicScheme(PRV_CPSS_SCHED_HANDLE hndl, struct schedPerLevelPeriodicParams *lvl_params)
{
    ...
    TM_CTL(ctl, hndl)
        ...
        * shaper update for Rev B */
        ...
        if (!prvSchedShapingUtilsFindLKN(&L, &K , &N))  return TM_CONF_PER_RATE_L_K_N_NOT_FOUND;
        ...
        rc = prvCpssTxqSchedulerGetDividerExp(PRV_SCHED_DEV_NUM_GET(ctl),SCHED_Q_LEVEL,&(ctl->level_data[SCHED_Q_LEVEL].divider_res_exp));
        if(rc)return rc;

        ctl->level_data[SCHED_Q_LEVEL].per_interval = node_shaper_update_frequency*8/(1<<(ctl->level_data[SCHED_Q_LEVEL].divider_res_exp))/ctl->level_data[SCHED_Q_LEVEL].unit;
        ...
        rc = prvCpssTxqSchedulerGetDividerExp(PRV_SCHED_DEV_NUM_GET(ctl),SCHED_A_LEVEL,&(ctl->level_data[SCHED_A_LEVEL].divider_res_exp));
        if(rc)return rc;

        ctl->level_data[SCHED_A_LEVEL].per_interval = node_shaper_update_frequency*8/(1<<(ctl->level_data[SCHED_A_LEVEL].divider_res_exp))/ctl->level_data[SCHED_A_LEVEL].unit;
        ...
        rc = prvCpssTxqSchedulerGetDividerExp(PRV_SCHED_DEV_NUM_GET(ctl),SCHED_B_LEVEL,&(ctl->level_data[SCHED_B_LEVEL].divider_res_exp));
        if(rc)return rc;

        ctl->level_data[SCHED_B_LEVEL].per_interval = node_shaper_update_frequency*8/(1<<(ctl->level_data[SCHED_B_LEVEL].divider_res_exp))/ctl->level_data[SCHED_B_LEVEL].unit;
        ...
        rc = prvCpssTxqSchedulerGetDividerExp(PRV_SCHED_DEV_NUM_GET(ctl),SCHED_C_LEVEL,&(ctl->level_data[SCHED_C_LEVEL].divider_res_exp));
        if(rc)return rc;

        ctl->level_data[SCHED_C_LEVEL].per_interval = node_shaper_update_frequency*8/(1<<(ctl->level_data[SCHED_C_LEVEL].divider_res_exp))/ctl->level_data[SCHED_C_LEVEL].unit;
        ...
        */
        ...
        if (base_update_frequency > optimal_node_shaper_update_frequency)
            ...
            port_shaper_update_frequency = (base_update_frequency/1000000)*1000000; /* rounding in order to get more accuracy */

        if (prvSchedShapingUtilsFindLKN(&L, &K , &N ))
        {
            ...
            rc = prvCpssTxqSchedulerGetDividerExp(PRV_SCHED_DEV_NUM_GET(ctl),SCHED_P_LEVEL,&(ctl->level_data[SCHED_P_LEVEL].divider_res_exp));
            if(rc)return rc;

            port_max_periods = 1<<ctl->level_data[SCHED_P_LEVEL].divider_res_exp;
            ...
            rc = prvCpssTxqSchedulerGetOptimalResolutionExponent(SCHED_P_LEVEL,&(ctl->level_data[SCHED_P_LEVEL].token_res_exp));
            if(rc)return rc;
        }
