...
/**
 * @internal prvCpssDxChTxqSip7InitCnode function
 * @endinternal
 *       NONE
 *
 * @brief   Initialize C  Node structure
 *
 ...
 *
 * @param[in] cNodePtr           - pointer to C node structure
 *
 * @retval GT_OK                    - on success.
 * @retval GT_BAD_PTR          - NULL pointer used
 */
static GT_STATUS  prvCpssDxChTxqSip7InitCnode
(
    PRV_CPSS_DXCH_TXQ_SIP_7_C_NODE * cNodePtr
)
{

    CPSS_NULL_PTR_CHECK_MAC(cNodePtr);

    cNodePtr->bNodeList = NULL;
    cNodePtr->bNodeListSize = CPSS_SIP6_TXQ_INVAL_DATA_CNS;
    cNodePtr->cNodeIndex = CPSS_SIP6_TXQ_INVAL_DATA_CNS;
    cNodePtr->serviceGroup = CPSS_SIP6_TXQ_INVAL_DATA_CNS;
...
)
{
    ...
    aNodePtr->preemptionType = PRV_DXCH_TXQ_SIP6_PREEMTION_PORT_TYPE_NONE_E;
...
* @param[in] numberOfAnodes           - number of A nodes connected to P node (SIP 6)
* @param[in] numberOfCnodes           - number of A nodes connected to P node (SIP 7).For SIP6 is 1.
 *
 * @retval GT_OK                    - on success.
 * @retval GT_BAD_PTR          - NULL pointer used
 */
...
(
    ...
    IN GT_U32                           numberOfAnodes,
    IN GT_U32                           numberOfCnodes
)
{
    GT_U32      i;
    GT_STATUS   rc;
    ...
    {
        pNodePtr->cNodeList = cpssOsMalloc(sizeof(PRV_CPSS_DXCH_TXQ_SIP_7_C_NODE)*numberOfCnodes);
        if(NULL == pNodePtr->cNodeList)
        {
             CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
        }

        pNodePtr->cNodeListSize = numberOfCnodes;

        for (i = 0; i < numberOfCnodes; i++)
        {
            rc = prvCpssDxChTxqSip7InitCnode(&(pNodePtr->cNodeList[i]));
            if (rc != GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
            }
...
(
    IN    GT_U8 devNum ,
    INOUT PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE ** pNodePtrPtr,
    IN    GT_U32                           numberOfAnodes,
    IN    GT_U32                           numberOfCnodes
)
{
    ...
    *pNodePtrPtr = (PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE *)cpssOsMalloc(sizeof(PRV_CPSS_DXCH_TXQ_SIP_6_P_NODE));
    ...
    rc = prvCpssDxChTxqSip6InitPnode(devNum,*pNodePtrPtr,numberOfAnodes,numberOfCnodes);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
 ...
 *       NONE
 *
 * @brief   ReAllocate  search array
 *
 * @note   APPLICABLE DEVICES: AAS
 * @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin;  Bobcat3; Aldrin2;Falcon; AC5P; AC5X; Harrier; Ironman;AC3X;.
 *
 * @param[in] pNodePtrPtrPtr                   - pointer to pointer to pointer to P node structure
 * @param[in] numberOfAnodes           -number of A nodes to initilize
 *
...
GT_STATUS  prvCpssDxChTxqSip7SearchArrayReAllocate
(
    GT_U32                             ** searchArrayPtrPtr,
    GT_U32                             oldArraySize,
    GT_U32                             newArraySize
)
{
     ...
     {
         newArray= (GT_U32 *)cpssOsMalloc(newArraySize*sizeof(GT_U32));
         if( NULL==newArray)
         {
             CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
         }
         ...
         {
            sizeToCopy = newArraySize;
         }
         else
         {
            sizeToCopy = oldArraySize;
         }
         cpssOsMemCpy(newArray,*searchArrayPtrPtr,(sizeToCopy*sizeof(GT_U32)));

         FREE_PTR_MAC(*searchArrayPtrPtr);

         *searchArrayPtrPtr = newArray;
     }
