...
/* sip5 need for ipv6 12 words for 384 bits */
/* sip7 need for 'extra long' TS of 1584 bits --> 50 words (1600 bits) */
#define TUNNEL_START_HW_SIZE_CNS       50

/* the size of HFT entry in words */
#define HFT_HW_SIZE_CNS       25 /* according to AAS_DIRECT_MAC(131072,       25,  CPSS_DXCH_SIP7_TABLE_HA_HEADER_FIELDS_0_E), */
/* the size of Tunnel Start/HFT MPLS entry in words */
#define TUNNEL_START_MPLS_LABEL_HW_SIZE_CNS       16
...
#define HW_TO_SW_B_DA_ASSIGN_MODE_MAC(swValue,hwValue)                       \
    if (hwValue)                                                             \
        swValue = CPSS_DXCH_TUNNEL_START_MIM_B_DA_ASSIGN_FROM_TS_ENTRY_AND_VLAN_ENTRY_E; \
    else                                                                     \
...
#define PRV_TUNNEL_START_VLAN_QOS_MARK_MODE_CHECK_MAC(mode)             \
    if(mode > CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_CIRCUIT_BASED_E)     \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);

#define PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_HW_FORMAT_MAC(mode,value) \
    switch ((mode))                                                              \
    {                                                                            \
    case CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_INGRESS_ASSIGNMENT_E:              \
        (value) = 0;                                                             \
        break;                                                                   \
    case CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E:                    \
        (value) = 1;                                                             \
        break;                                                                   \
    case CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_CIRCUIT_BASED_E:                   \
        (value) = 2;                                                             \
        break;                                                                   \
    default:  /* default set some value as the field must not be valid */        \
        (value) = 0;                                                             \
...
#define PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_SW_FORMAT_MAC(mode,value) \
    switch ((mode))                                                              \
    {                                                                            \
    case 0:                                                                      \
        (value) = CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_INGRESS_ASSIGNMENT_E;     \
        break;                                                                   \
    case 1:                                                                      \
        (value) = CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E;           \
        break;                                                                   \
    case 2:                                                                      \
        (value) = CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_CIRCUIT_BASED_E;          \
        break;                                                                   \
    default:  /* default set some value as the field must not be valid */        \
        (value) = 0;                                                             \
        break;                                                                   \
...
#define PRV_HFT_TYPE_CHECK_MAC(hftType)             \
    if(hftType > CPSS_DXCH_HFT_MPLS_EXT_E)          \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);

/* macro assume : value hold the 'value' and set it into hwFormatArray */
#define HA_HFT_FIELD_VALUE_SET_MAC(_dev, _fieldName) \
    SIP7_HA_HFT_FIELD_SET_MAC(_dev, hwFormatArray, _fieldName, value)

#define HA_HFT_FIELD_VALUE_GET_MAC(_dev, _fieldName) \
    SIP7_HA_HFT_FIELD_GET_MAC(_dev, hwFormatArray, _fieldName, value)
...
typedef enum{
     ...
     * SIP7: Adding the SRH header after the passenger header */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_IPV4_OR_INSERT_SRH_E = 0x01

    /** 0x2 = MacInMac */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_MIM_E = 0x02

    /** 0x3 = TRILL. Applicable for sip7 devices */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_TRILL_E = 0x03

    /** 0x4 = GENERIC; Applicable for sip6 only
     * SIP7: PHA Generic TS */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_PHA_GENERIC_E = 0x04

    /** 0x5 = IPv6; IPv6; This Entry is an X-over-IPv6 or X-Over-IPv6/GRE Tunnel Start.;
     * Not Applicable for SIP7 */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_IPV6_E = 0x05

    /** 0x6 = Generic IPv4; GenericIPv4; Flexible X-over-IPv4/GRE/UDP;
     * SIP7: Generic or vPort IPv4 Tunnel-start entry type.
     *       vPort entry is used if vPort building is triggered; */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV4_E = 0x06

    /** 0x7 = GenericIPv6; GenericIPv6; Flexible X-over-IPv6/GRE/UDP;
     * SIP7: Generic or vPort IPv6 Tunnel-start entry type.
     *       vPort entry is used if vPort building is triggered; */
    ,TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV6_E = 0x07
...
typedef enum{
    ...
    /** 0x0 = Generic */
    TUNNEL_START_HW_HFT_ENTRY_TYPE_GENERIC_E = 0x00

    /** 0x1 = Generic Extended */
    ,TUNNEL_START_HW_HFT_ENTRY_TYPE_GENERIC_EXT_E = 0x01

    /** 0x2 = Mpls Label */
    ,TUNNEL_START_HW_HFT_ENTRY_TYPE_MPLS_LABEL_E = 0x02

    /** 0x3 = Mpls Label Extended */
    ,TUNNEL_START_HW_HFT_ENTRY_TYPE_MPLS_LABEL_EXT_E = 0x03

} TUNNEL_START_HW_HFT_ENTRY_TYPE_ENT;


#define PRV_HFT_VLAN_QOS_MARK_MODE_CHECK_MAC(mode)                      \
    if(mode > CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_CIRCUIT_BASED_E)     \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
...
)
{
    ...
    switch (tunnelType)
    {
        ...
        /* RETAIN_INNER_CRC field was removed in SIP7 */
        ...
        case CPSS_TUNNEL_GENERIC_IPV4_E:
            hwTunnelType = TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV4_E;
        ...
        case CPSS_TUNNEL_GENERIC_IPV6_E:
            hwTunnelType = TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV6_E;
...
)
{
    ...
    switch (tunnelType)
    {
        case CPSS_TUNNEL_GENERIC_E:
            value = TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_PHA_GENERIC_E;
...
/**
* @internal tunnelStartConfigHw2LogicFormat_sip7 function
* @endinternal
*
* @brief   Converts a given tunnel start configuration from hardware format
*         to logic format.
...
* @retval GT_BAD_STATE             - on invalid tunnel type
*/
static GT_STATUS tunnelStartConfigHw2LogicFormat_sip7
(
    IN  GT_U8                               devNum,
    IN  GT_U32                              *hwFormatArray,
...
{
    CPSS_TUNNEL_TYPE_ENT                   swTunnelType;
    GT_U32                                 value;
    TUNNEL_START_HW_FIELD_TUNNEL_TYPE_ENT  hwTunnelType;
    GT_U32                                 idx, startBit;
    GT_U32                                 tempHwFormatArray[16];
    GT_U32                                 word[2];
    /* zero out temp hw format */
    cpssOsMemSet(tempHwFormatArray,0,sizeof(GT_U32)*16);
    ...
    switch(hwTunnelType)
    {
        case TUNNEL_START_HW_FIELD_TUNNEL_TYPE_MPLS_E:
            ...
            /* TAG0 QoS Marking Mode */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG0_QOS_MARKING_MODE_E);
            PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_SW_FORMAT_MAC(value, logicFormatPtr->mplsVportCfg.tag0QosMarkMode);

            /* TAG1 QoS Marking Mode */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG1_QOS_MARKING_MODE_E);
            PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_SW_FORMAT_MAC(value, logicFormatPtr->mplsVportCfg.tag1QosMarkMode);

            /* UP0 */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP0_E);
            logicFormatPtr->mplsVportCfg.up0 = value;

            /* DEI0 */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI0_E);
            logicFormatPtr->mplsVportCfg.dei0 = value;

            /* UP1 */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP1_E);
            logicFormatPtr->mplsVportCfg.up1 = value;

            /* DEI1 */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI1_E);
            logicFormatPtr->mplsVportCfg.dei1 = value;

            /* Thread ID */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_THREAD_ID_E);
            logicFormatPtr->mplsVportCfg.threadId = value;

            /* Analyzer Index */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_ANALYZER_INDEX_E);
            logicFormatPtr->mplsVportCfg.analyzerIndex = value;

            /* Egress Mirror Mode */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_EGRESS_MIRROR_MODE_E);
            PRV_CPSS_DXCH_MIRROR_CONVERT_EGRESS_MIRROR_MODE_TO_SW_VAL_MAC(value, logicFormatPtr->mplsVportCfg.egressMirrorMode);

            /* Raw Labels */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_RAW_LABELS_E);
            logicFormatPtr->mplsVportCfg.rawLabels = BIT2BOOL_MAC(value);

            /* MPLS EtherType Select */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_MPLS_ETHERTYPE_SELECT_E);
            logicFormatPtr->mplsVportCfg.mplsEthertypeSelect = value;

            /* CNC Pointer */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_CNC_PTR_E);
            logicFormatPtr->mplsVportCfg.cncPtr = value;

            /* Flow ID/GCF Profile ID */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_OR_GCF_PROFILE_ID_E);
            logicFormatPtr->mplsVportCfg.flowOrGcfProfileId = value;

            /* Flow ID/GCF Bits */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_OR_GCF_BITS_E);
            logicFormatPtr->mplsVportCfg.flowOrGcfBits = value;

            /* Number of Labels */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_NUMBER_OF_LABELS_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels = value;

            /* Last Qos Uniform Label */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_QOS_UNIFORM_LABEL_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastQosUniformLabel = value;

            /* Global EXP Mapping Table Index */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.globaExplMappingTableIndex = value;

            /* Last TTL Uniform Label */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_TTL_UNIFORM_LABEL_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastTtlUniformLabel = value;

            /* CW Index */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_CW_INDEX_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.cwIndex = value;

            /* Flow Label Insertion */
            HA_TUNNEL_START_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_LABEL_INSERTION_E);
            logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.flowLabelInsertion = BIT2BOOL_MAC(value);

            if (logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels > 0)
            {
                U_ANY_GET_FIELD_BY_ID_MAC(hwFormatArray,
                                          PRV_TABLE_FORMAT_INFO(devNum)[PRV_CPSS_DXCH_TABLE_FORMAT_TUNNEL_START_E].fieldsInfoPtr,
                                          SIP7_HA_TUNNEL_START_TABLE_FIELDS_MPLS_VPORT_LABELS_E,
                                          &tempHwFormatArray[0]);
                for (idx = 0; idx < logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels; idx++)
                {
                    /* zero out the word */
                    cpssOsMemSet(word, 0, sizeof(GT_U32)*2);
                    startBit = idx*2;
                    word[0]  = U32_GET_FIELD_MAC(tempHwFormatArray[idx], startBit, 32-startBit);
                    word[0] |= (U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], 0, startBit)<<(32-startBit));
                    word[1]  = U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], startBit, 2);
                    logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode     = word[0] & 0x3;
                    logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].label              = (word[0]>>2) & 0xFFFFF;
                    logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].exp                = (word[0]>>22) & 0x7;
                    logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].ttl                = (((word[0]>>25) & 0x7F)|((word[1] & 0x1)<<7));
                    logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = BIT2BOOL_MAC((word[1]>>1) & 0x1);
                }
            }
            break;
        default:
...
)
{
    ...
    switch(hwTunnelType)
    {
        case TUNNEL_START_HW_FIELD_TUNNEL_TYPE_MPLS_E:
            ...
            break;
        case TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV4_E:
            ...
            break;
        case TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_VPORT_IPV6_E:
            ...
            {
                GT_U32 wordNum;
                GT_U8 ipv6HwArray[20];

                cpssOsMemSet(ipv6HwArray,0,sizeof(ipv6HwArray));
...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
       PRV_CPSS_DXCH_PP_MAC(devNum)->vportInfo.useVPortMode)
    {
        return tunnelStartConfigHw2LogicFormat_sip7(devNum, hwFormatArray, tunnelTypePtr, logicFormatPtr);
    }

    switch(hwTunnelType)
    {
        case TUNNEL_START_HW_FIELD_TUNNEL_TYPE_GENERIC_OR_PHA_GENERIC_E:
...
/**
* @internal mplsLabelStackConfigConvertHwFormat function
* @endinternal
*
* @brief  Converts a given MPLS Label Stack configuration from logic format
*         to hardware format.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] numOfLabels              - number of MPLS Labels
* @param[in] mplsLabelStackCfgPtr     - pointer to MPLS Label Stack configuration
* @param[out] tempHwFormatArrayPtr    - pointer to HW format array (16 words)
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters.
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_VOID mplsLabelStackConfigConvertHwFormat
(
    IN  GT_U32                                            numOfLabels,
    IN  CPSS_DXCH_TUNNEL_START_MPLS_LABEL_STACK_STC       *mplsLabelStackCfgPtr,
    OUT GT_U32                                            *tempHwFormatArrayPtr
)
{
    GT_U32  idx;
    GT_U32  word[2];
    GT_U32  startBit;
    for (idx = 0; idx < numOfLabels; idx++)
    {
        /* zero out the word */
        cpssOsMemSet(word, 0, sizeof(GT_U32)*2);
        word[0] = (mplsLabelStackCfgPtr->mplsLabels[idx].expMappingMode |
                  (mplsLabelStackCfgPtr->mplsLabels[idx].label << 2)    |
                  (mplsLabelStackCfgPtr->mplsLabels[idx].exp << 22)     |
                  (mplsLabelStackCfgPtr->mplsLabels[idx].ttl << 25));
        word[1] = ((mplsLabelStackCfgPtr->mplsLabels[idx].ttl >> 7) |
                  (BOOL2BIT_MAC(mplsLabelStackCfgPtr->mplsLabels[idx].isElOrEliInsertion)) << 1);
        startBit = idx*2;
        U32_SET_FIELD_MAC(tempHwFormatArrayPtr[idx],   startBit, 32-startBit,word[0]);
        U32_SET_FIELD_MAC(tempHwFormatArrayPtr[idx+1], 0,        startBit,   word[0] >> (32-startBit));
        U32_SET_FIELD_MAC(tempHwFormatArrayPtr[idx+1], startBit, 2,          word[1]);
    }
}

/**
* @internal tunnelStartConfigLogic2HwFormat_sip7 function
* @endinternal
*
* @brief   Converts a given tunnel start configuration from logic format
*         to hardware format.
...
*                                   Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; xCat3; AC5; Lion2.
*
* @param[in] devNum                   - physical device number
* @param[in] tunnelType               - type of the tunnel
* @param[in] logicFormatPtr           - points to tunnel start configuration
...
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_STATUS tunnelStartConfigLogic2HwFormat_sip7
(
    IN  GT_U8                               devNum,
    IN  CPSS_TUNNEL_TYPE_ENT                tunnelType,
    IN  CPSS_DXCH_TUNNEL_START_CONFIG_UNT   *logicFormatPtr,
...
)
{
    GT_U32  value = 0;
    GT_U32  hwTunnelType;
    GT_U32  tempHwFormatArray[TUNNEL_START_MPLS_LABEL_HW_SIZE_CNS];
    /* zero out hw format */
    cpssOsMemSet(hwFormatArray, 0, sizeof(GT_U32)*TUNNEL_START_HW_SIZE_CNS);
    /* zero out temp hw format */
    cpssOsMemSet(tempHwFormatArray, 0, sizeof(GT_U32)*TUNNEL_START_MPLS_LABEL_HW_SIZE_CNS);
    switch (tunnelType)
    {
        case CPSS_TUNNEL_X_OVER_MPLS_E:
            hwTunnelType = TUNNEL_START_HW_FIELD_TUNNEL_TYPE_MPLS_E;
            ...
            /* TAG0 QoS Marking Mode */
            PRV_TUNNEL_START_VLAN_QOS_MARK_MODE_CHECK_MAC(logicFormatPtr->mplsVportCfg.tag0QosMarkMode);
            PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_HW_FORMAT_MAC(logicFormatPtr->mplsVportCfg.tag0QosMarkMode, value);
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG0_QOS_MARKING_MODE_E);

            /* TAG1 QoS Marking Mode */
            PRV_TUNNEL_START_VLAN_QOS_MARK_MODE_CHECK_MAC(logicFormatPtr->mplsVportCfg.tag1QosMarkMode);
            PRV_TUNNEL_START_CONVERT_VLAN_QOS_MARK_MODE_TO_HW_FORMAT_MAC(logicFormatPtr->mplsVportCfg.tag1QosMarkMode, value);
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG1_QOS_MARKING_MODE_E);

            if (logicFormatPtr->mplsVportCfg.tag0QosMarkMode == CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E)
            {
                /* UP0 */
                CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.up0, BIT_3-1)
                value = logicFormatPtr->mplsVportCfg.up0;
                HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP0_E);

                /* DEI0 */
                value = BOOL2BIT_MAC(logicFormatPtr->mplsVportCfg.dei0);
                HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI0_E);
            }

            if (logicFormatPtr->mplsVportCfg.tag1QosMarkMode == CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E)
            {
                /* UP1 */
                CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.up1, BIT_3-1)
                value = logicFormatPtr->mplsVportCfg.up1;
                HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP1_E);

                /* DEI1 */
                value = BOOL2BIT_MAC(logicFormatPtr->mplsVportCfg.dei1);
                HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI1_E);
            }

            /* Thread ID */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.threadId, BIT_8);
            value = logicFormatPtr->mplsVportCfg.threadId;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_THREAD_ID_E);

            /* Analyzer Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.analyzerIndex, BIT_6);
            value = logicFormatPtr->mplsVportCfg.analyzerIndex;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_ANALYZER_INDEX_E);

            /* Egress Mirror Mode */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.egressMirrorMode, BIT_2);
            PRV_CPSS_DXCH_MIRROR_CONVERT_EGRESS_MIRROR_MODE_TO_HW_VAL_MAC(value, logicFormatPtr->mplsVportCfg.egressMirrorMode);
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_EGRESS_MIRROR_MODE_E);

            /* Raw Labels */
            value = BOOL2BIT_MAC(logicFormatPtr->mplsVportCfg.rawLabels);
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_RAW_LABELS_E);

            /* MPLS EtherType Select */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsEthertypeSelect, BIT_3);
            value = logicFormatPtr->mplsVportCfg.mplsEthertypeSelect;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_MPLS_ETHERTYPE_SELECT_E);

            /* CNC Pointer */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.cncPtr, BIT_19);
            value = logicFormatPtr->mplsVportCfg.cncPtr;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_CNC_PTR_E);

            /* Flow ID/GCF Profile ID */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.flowOrGcfProfileId, BIT_4);
            value = logicFormatPtr->mplsVportCfg.flowOrGcfProfileId;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_OR_GCF_PROFILE_ID_E);

            /* Flow ID/GCF Bits */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.flowOrGcfBits, BIT_31);
            value = logicFormatPtr->mplsVportCfg.flowOrGcfBits;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_OR_GCF_BITS_E);

            /* Number of Labels */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels, BIT_4);
            value = logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_NUMBER_OF_LABELS_E);

            /* Last Qos Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastQosUniformLabel, BIT_4);
            value = logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastQosUniformLabel;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_QOS_UNIFORM_LABEL_E);

            /* Global EXP Mapping Table Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.globaExplMappingTableIndex, BIT_3);
            value = logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.globaExplMappingTableIndex;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);

            /* Last TTL Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastTtlUniformLabel, BIT_4);
            value = logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.lastTtlUniformLabel;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_TTL_UNIFORM_LABEL_E);

            /* CW Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.cwIndex, BIT_4);
            value = logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.cwIndex;
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_CW_INDEX_E);

            /* Flow Label Insertion */
            value = BOOL2BIT_MAC(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.flowLabelInsertion);
            HA_TUNNEL_START_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_LABEL_INSERTION_E);

            if (logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels > 0)
            {
                mplsLabelStackConfigConvertHwFormat(logicFormatPtr->mplsVportCfg.mplsLabelStackCfg.numOfLabels,
                                                    &logicFormatPtr->mplsVportCfg.mplsLabelStackCfg,
                                                    &tempHwFormatArray[0]);
...
)
{
    ...
    /* Check for SIP7 device and if vPort Mode is Enabled */
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
        PRV_CPSS_DXCH_PP_MAC(devNum)->vportInfo.useVPortMode)
    {
        return tunnelStartConfigLogic2HwFormat_sip7(devNum, tunnelType, logicFormatPtr, hwFormatArray);
    }
...
)
{
    ...
    /* check parameters */
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
/**
* @internal hftConfigLogic2HwFormat function
* @endinternal
*
* @brief   Converts a given HFT configuration from logic format
*         to hardware format.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Aldrin; AC3X; Bobcat3;
...
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_STATUS hftConfigLogic2HwFormat
(
    IN  GT_U8                          devNum,
    IN  CPSS_DXCH_HFT_ENTRY_TYPE_ENT   hftEntryType,
    IN  CPSS_DXCH_HFT_CONFIG_UNT       *logicFormatPtr,
    OUT GT_U32                         *hwFormatArray
)
{
    GT_U32  value = 0;
    GT_U32  hwHftEntryType;
    GT_U32  gcfBits[2];
    GT_U32  tempHwFormatArray[TUNNEL_START_MPLS_LABEL_HW_SIZE_CNS];

    /* zero out hw format */
    cpssOsMemSet(hwFormatArray, 0, sizeof(GT_U32)*HFT_HW_SIZE_CNS);
    /* zero out gcf Bits */
    cpssOsMemSet(gcfBits, 0, sizeof(GT_U32)*2);

    switch (hftEntryType)
    {
        case CPSS_DXCH_HFT_MPLS_E:
            hwHftEntryType = TUNNEL_START_HW_HFT_ENTRY_TYPE_MPLS_LABEL_E;

            /* Sub Type */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.subType, BIT_3);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.subType;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_TYPE_E);

            /* Analyzer Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.analyzerIndex, BIT_6);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.analyzerIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ANALYZER_INDEX_E);

            /* Egress Mirror Mode */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.egressMirrorMode, BIT_2);
            PRV_CPSS_DXCH_MIRROR_CONVERT_EGRESS_MIRROR_MODE_TO_HW_VAL_MAC(value, logicFormatPtr->mplsLabelCfg.hftCommonCfg.egressMirrorMode);
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_EGRESS_MIRROR_MODE_E);

            /* Sub Thread ID */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.subThreadId, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.subThreadId;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_THREAD_ID_E);

            /* CNC Pointer */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.cncPtr, BIT_19);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.cncPtr;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CNC_PTR_E);

            /* GCF Profile Id */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfProfileId, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfProfileId;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_PROFILE_ID_E);

            /* GCF Bits [0:4] */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfBits0To4, BIT_5);
            value = logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfBits0To4;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_BITS_0_4_E);

            /* Number of Labels */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_NUMBER_OF_LABELS_E);

            /* Last Qos Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastQosUniformLabel, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastQosUniformLabel;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_QOS_UNIFORM_LABEL_E);

            /* Global EXP Mapping Table Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.globaExplMappingTableIndex, BIT_3);
            value = logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.globaExplMappingTableIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);

            /* Last TTL Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastTtlUniformLabel, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastTtlUniformLabel;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_TTL_UNIFORM_LABEL_E);

            /* CW Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.cwIndex, BIT_4);
            value = logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.cwIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CW_INDEX_E);

            /* Flow Label Insertion */
            value = BOOL2BIT_MAC(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.flowLabelInsertion);
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_LABEL_INSERTION_E);

            /* MPLS Label Stack Configuration */
            if (logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels > 0)
            {

                mplsLabelStackConfigConvertHwFormat(logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels,
                                                    &logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg,
                                                    &tempHwFormatArray[0]);
                U_ANY_SET_FIELD_BY_ID_MAC(hwFormatArray,
                                          PRV_TABLE_FORMAT_INFO(devNum)[PRV_CPSS_DXCH_TABLE_FORMAT_HA_HEADER_FIELDS_E].fieldsInfoPtr,
                                          SIP7_HA_HEADER_FIELDS_MPLS_LABELS_E,
                                          &tempHwFormatArray[0]);
            }
            break;
        case CPSS_DXCH_HFT_MPLS_EXT_E:
            hwHftEntryType = TUNNEL_START_HW_HFT_ENTRY_TYPE_MPLS_LABEL_EXT_E;

            /* Sub Type */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subType, BIT_3);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subType;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_TYPE_E);

            /* Analyzer Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.analyzerIndex, BIT_6);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.analyzerIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ANALYZER_INDEX_E);

            /* Egress Mirror Mode */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.egressMirrorMode, BIT_2);
            PRV_CPSS_DXCH_MIRROR_CONVERT_EGRESS_MIRROR_MODE_TO_HW_VAL_MAC(value, logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.egressMirrorMode);
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_EGRESS_MIRROR_MODE_E);

            /* Sub Thread ID */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subThreadId, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subThreadId;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_THREAD_ID_E);

            /* CNC Pointer */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.cncPtr, BIT_19);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.cncPtr;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CNC_PTR_E);

            /* GCF Profile Id */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfProfileId, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfProfileId;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_PROFILE_ID_E);

            /* GCF Bits [0:4] */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfBits0To4, BIT_5);
            value = logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfBits0To4;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_BITS_0_4_E);

            /* Number of Labels */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_NUMBER_OF_LABELS_E);

            /* Last Qos Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastQosUniformLabel, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastQosUniformLabel;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_QOS_UNIFORM_LABEL_E);

            /* Global EXP Mapping Table Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.globaExplMappingTableIndex, BIT_3);
            value = logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.globaExplMappingTableIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);

            /* Last TTL Uniform Label */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastTtlUniformLabel, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastTtlUniformLabel;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_TTL_UNIFORM_LABEL_E);

            /* CW Index */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.cwIndex, BIT_4);
            value = logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.cwIndex;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CW_INDEX_E);

            /* Flow Label Insertion */
            value = BOOL2BIT_MAC(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.flowLabelInsertion);
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_LABEL_INSERTION_E);

            /* GCF Bits [5:47] */
            gcfBits[0] = logicFormatPtr->mplsLabelExtCfg.gcfBits5To47[0];
            gcfBits[1] = logicFormatPtr->mplsLabelExtCfg.gcfBits5To47[1] & 0x7FF;
            U_ANY_SET_FIELD_BY_ID_MAC(hwFormatArray,
                                      PRV_TABLE_FORMAT_INFO(devNum)[PRV_CPSS_DXCH_TABLE_FORMAT_HA_HEADER_FIELDS_E].fieldsInfoPtr,
                                      SIP7_HA_HEADER_FIELDS_GCF_BITS_5_47_E,
                                      &gcfBits[0]);
            /* Flow ID */
            CPSS_DATA_CHECK_MAX_MAC(logicFormatPtr->mplsLabelExtCfg.flowId, BIT_20);
            value = logicFormatPtr->mplsLabelExtCfg.flowId;
            HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_ID_E);

            /* MPLS Label Stack Configuration */
            if (logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels > 0)
            {

                mplsLabelStackConfigConvertHwFormat(logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels,
                                                    &logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg,
                                                    &tempHwFormatArray[0]);
                U_ANY_SET_FIELD_BY_ID_MAC(hwFormatArray,
                                          PRV_TABLE_FORMAT_INFO(devNum)[PRV_CPSS_DXCH_TABLE_FORMAT_HA_HEADER_FIELDS_E].fieldsInfoPtr,
                                          SIP7_HA_HEADER_FIELDS_MPLS_LABELS_E,
    ...
    value = hwHftEntryType;
    HA_HFT_FIELD_VALUE_SET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ENTRY_TYPE_E);

    return GT_OK;
}

/**
* @internal internal_cpssDxChTunnelStartHftPortGroupEntrySet function
* @endinternal
*
* @brief   Set Header Fields Table Entry.
*
* @note   APPLICABLE DEVICES:      AAS
* @param[in] devNum                - physical device number
* @param[in] portGroupsBmp         - bitmap of Port Groups.
* @param[in] entryIndex            - index for the Header Fields Table entry
* @param[in] hftTableId            - Header Fields Table ID
* @param[in] hftEntryType          - Header Fields Table Entry Type
* @param[in] configPtr             - points to HFT configuration
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameter.
* @retval GT_BAD_PTR               - on NULL pointer
...
*
*/
static GT_STATUS internal_cpssDxChTunnelStartHftPortGroupEntrySet
(
    IN  GT_U8                          devNum,
    IN  GT_PORT_GROUPS_BMP             portGroupsBmp,
    IN  GT_U32                         entryIndex,
    IN  CPSS_DXCH_HFT_TABLE_ENT        hftTableId,
    IN  CPSS_DXCH_HFT_ENTRY_TYPE_ENT   hftEntryType,
    IN  CPSS_DXCH_HFT_CONFIG_UNT       *configPtr
)
{
    GT_U32      hwConfigArray[HFT_HW_SIZE_CNS]; /* tunnel start config in hardware format */
    GT_STATUS   rc;         /* function return code */
    GT_U32      portGroupId;/*the port group Id - support multi-port-groups device */
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_XCAT3_E | CPSS_BOBCAT3_E |
                                          CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_ALDRIN2_E | CPSS_FALCON_E |
                                          CPSS_AC5P_E | CPSS_AC5X_E | CPSS_AC5_E | CPSS_HARRIER_E | CPSS_IRONMAN_E);

    tableType = (hftTableId == CPSS_DXCH_HFT_TABLE_0_E)?
                CPSS_DXCH_SIP7_TABLE_HA_HEADER_FIELDS_0_E:
                CPSS_DXCH_SIP7_TABLE_HA_HEADER_FIELDS_1_E;
    PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_TABLE_MAC(devNum, portGroupsBmp, tableType);

    PRV_HFT_TYPE_CHECK_MAC(hftEntryType);
    CPSS_NULL_PTR_CHECK_MAC(configPtr);

    /* convert tunnel start entry to hw format while checking parameters */
    rc = hftConfigLogic2HwFormat(devNum, hftEntryType, configPtr, hwConfigArray);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum, portGroupsBmp, portGroupId)
    {
        rc = prvCpssDxChPortGroupWriteTableEntry(devNum, portGroupId, tableType, entryIndex, hwConfigArray);
        if(rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal cpssDxChTunnelStartHftPortGroupEntrySet function
* @endinternal
*
* @brief   Set Header Fields Table Entry.
*
...
*
*/
GT_STATUS cpssDxChTunnelStartHftPortGroupEntrySet
(
    IN  GT_U8                          devNum,
    IN  GT_PORT_GROUPS_BMP             portGroupsBmp,
    IN  GT_U32                         entryIndex,
...
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTunnelStartHftPortGroupEntrySet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portGroupsBmp, entryIndex, hftTableId, hftEntryType, configPtr));

    rc = internal_cpssDxChTunnelStartHftPortGroupEntrySet(devNum, portGroupsBmp, entryIndex,
                                                          hftTableId, hftEntryType, configPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portGroupsBmp, entryIndex, hftTableId, hftEntryType, configPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal hftConfigHw2LogicFormat function
* @endinternal
*
* @brief   Converts a given HFT configuration from HW format
*          to Logic format.
*
* @note   APPLICABLE DEVICES:      AAS
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Aldrin; AC3X; Bobcat3;
...
* @param[in]  devNum                   - physical device number
* @param[in]  hwFormatArray            - The configuration in HW format (6 words).
* @param[out] hftEntryTypePtr          - HFT Entry type pointer
* @param[out] logicFormatPtr           - points to tunnel start configuration
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters.
* @retval GT_BAD_PTR               - on NULL pointer
*/
static GT_STATUS hftConfigHw2LogicFormat
(
    IN  GT_U8                          devNum,
    IN  GT_U32                         *hwFormatArray,
    OUT CPSS_DXCH_HFT_ENTRY_TYPE_ENT   *hftEntryTypePtr,
    OUT CPSS_DXCH_HFT_CONFIG_UNT       *logicFormatPtr
)
{
    GT_U32  value = 0;
    GT_U32  gcfBits[2];
    ...
    HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ENTRY_TYPE_E);
    *hftEntryTypePtr = (CPSS_DXCH_HFT_ENTRY_TYPE_ENT)value;

    switch (*hftEntryTypePtr)
    {
        case CPSS_DXCH_HFT_MPLS_E:
            /* Sub Type */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_TYPE_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.subType = value;

            /* Analyzer Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ANALYZER_INDEX_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.analyzerIndex = value;

            /* Egress Mirror Mode */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_EGRESS_MIRROR_MODE_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.egressMirrorMode = value;

            /* Sub Thread ID */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_THREAD_ID_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.subThreadId = value;

            /* CNC Pointer */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CNC_PTR_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.cncPtr = value;

            /* GCF Profile Id */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_PROFILE_ID_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfProfileId = value;

            /* GCF Bits [0:4] */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_BITS_0_4_E);
            logicFormatPtr->mplsLabelCfg.hftCommonCfg.gcfBits0To4 = value;

            /* Number of Labels */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_NUMBER_OF_LABELS_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels = value;

            /* Last Qos Uniform Label */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_QOS_UNIFORM_LABEL_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastQosUniformLabel = value;

            /* Global EXP Mapping Table Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.globaExplMappingTableIndex = value;

            /* Last TTL Uniform Label */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_TTL_UNIFORM_LABEL_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.lastTtlUniformLabel = value;

            /* CW Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CW_INDEX_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.cwIndex = value;

            /* Flow Label Insertion */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_LABEL_INSERTION_E);
            logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.flowLabelInsertion = BIT2BOOL_MAC(value);
            ...
            if (logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels > 0)
            {
                U_ANY_GET_FIELD_BY_ID_MAC(hwFormatArray,
                                          ...
                                          SIP7_HA_HEADER_FIELDS_MPLS_LABELS_E,
                                          &tempHwFormatArray[0]);
                for (idx = 0; idx < logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.numOfLabels; idx++)
                {
                    /* zero out words */
                    cpssOsMemSet(word, 0, sizeof(GT_U32)*2);
                    startBit = idx*2;
                    ...
                    word[0] |= (U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], 0, startBit)<<(32-startBit));
                    word[1]  = U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], startBit, 2);
                    logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode     = word[0] & 0x3;
                    logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].label              = (word[0]>>2) & 0xFFFFF;
                    logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].exp                = (word[0]>>22) & 0x7;
                    logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].ttl                = (((word[0]>>25) & 0x7F)|((word[1] & 0x1)<<7));
                    logicFormatPtr->mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = BIT2BOOL_MAC((word[1]>>1) & 0x1);
                }
            }
            break;
        case CPSS_DXCH_HFT_MPLS_EXT_E:
            /* Sub Type */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_TYPE_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subType = value;

            /* Analyzer Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_ANALYZER_INDEX_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.analyzerIndex = value;

            /* Egress Mirror Mode */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_EGRESS_MIRROR_MODE_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.egressMirrorMode = value;

            /* Sub Thread ID */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_SUB_THREAD_ID_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.subThreadId = value;

            /* CNC Pointer */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CNC_PTR_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.cncPtr = value;

            /* GCF Profile Id */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_PROFILE_ID_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfProfileId = value;

            /* GCF Bits [0:4] */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GCF_BITS_0_4_E);
            logicFormatPtr->mplsLabelExtCfg.hftCommonCfg.gcfBits0To4 = value;

            /* Number of Labels */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_NUMBER_OF_LABELS_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels = value;

            /* Last Qos Uniform Label */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_QOS_UNIFORM_LABEL_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastQosUniformLabel = value;

            /* Global EXP Mapping Table Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.globaExplMappingTableIndex = value;

            /* Last TTL Uniform Label */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_LAST_TTL_UNIFORM_LABEL_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.lastTtlUniformLabel = value;

            /* CW Index */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_CW_INDEX_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.cwIndex = value;

            /* Flow Label Insertion */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_LABEL_INSERTION_E);
            logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.flowLabelInsertion = BIT2BOOL_MAC(value);
            ...
            U_ANY_GET_FIELD_BY_ID_MAC(hwFormatArray,
                                      ...
                                      SIP7_HA_HEADER_FIELDS_GCF_BITS_5_47_E,
                                      &gcfBits[0]);
            logicFormatPtr->mplsLabelExtCfg.gcfBits5To47[0] = gcfBits[0];
            logicFormatPtr->mplsLabelExtCfg.gcfBits5To47[1] = gcfBits[1] & 0x7FF;

            /* Flow ID */
            HA_HFT_FIELD_VALUE_GET_MAC(devNum, SIP7_HA_HEADER_FIELDS_FLOW_ID_E);
            logicFormatPtr->mplsLabelExtCfg.flowId = value;
            ...
            if (logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels > 0)
            {
                U_ANY_GET_FIELD_BY_ID_MAC(hwFormatArray,
                                          ...
                                          SIP7_HA_HEADER_FIELDS_MPLS_LABELS_E,
                                          &tempHwFormatArray[0]);
                for (idx = 0; idx < logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels; idx++)
                {
                    /* zero out words */
                    cpssOsMemSet(word, 0, sizeof(GT_U32)*2);
                    startBit = idx*2;
                    ...
                    word[0] |= (U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], 0, startBit)<<(32-startBit));
                    word[1]  = U32_GET_FIELD_MAC(tempHwFormatArray[idx+1], startBit, 2);
                    logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode     = word[0] & 0x3;
                    logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].label              = (word[0]>>2) & 0xFFFFF;
                    logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].exp                = (word[0]>>22) & 0x7;
                    logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].ttl                = (((word[0]>>25) & 0x7F)|((word[1] & 0x1)<<7));
                    logicFormatPtr->mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = BIT2BOOL_MAC((word[1]>>1) & 0x1);
                }
            }
            break;
        default:
...
/**
* @internal internal_cpssDxChTunnelStartHftPortGroupEntryGet function
* @endinternal
*
* @brief   Set Header Fields Table Entry.
*
...
* @param[in]  devNum                 - physical device number
* @param[in]  portGroupsBmp          - bitmap of Port Groups.
* @param[in]  entryIndex             - index for the HFT table entry
* @param[in]  hftTableId             - Header Fields Table ID
* @param[out] hftEntryTypePtr        - points to the type of HFT Table Entry
* @param[out] configPtr              - points to HFT Table configuration
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameter.
* @retval GT_BAD_PTR               - on NULL pointer
...
*
*/
static GT_STATUS internal_cpssDxChTunnelStartHftPortGroupEntryGet
(
    IN  GT_U8                          devNum,
    IN  GT_PORT_GROUPS_BMP             portGroupsBmp,
    IN  GT_U32                         entryIndex,
    IN  CPSS_DXCH_HFT_TABLE_ENT        hftTableId,
    IN  CPSS_DXCH_HFT_ENTRY_TYPE_ENT   *hftEntryTypePtr,
    IN  CPSS_DXCH_HFT_CONFIG_UNT       *configPtr
)
{
    GT_U32      hwConfigArray[HFT_HW_SIZE_CNS]; /* tunnel start config in hardware format */
    ...
    /* zero out hw format */
    cpssOsMemSet(hwConfigArray, 0, sizeof(GT_U32)*HFT_HW_SIZE_CNS);
    ...
    CPSS_NULL_PTR_CHECK_MAC(hftEntryTypePtr);
    CPSS_NULL_PTR_CHECK_MAC(configPtr);

    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum, portGroupsBmp, portGroupId)
    {
        /* read the entry */
        rc = prvCpssDxChPortGroupReadTableEntry(devNum, portGroupId, tableType, entryIndex, hwConfigArray);
        if(rc != GT_OK)
        {
            return rc;
        }
    ...
    /* convert HFT entry to Logic format from HW Format */
    rc = hftConfigHw2LogicFormat(devNum, hwConfigArray, hftEntryTypePtr, configPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal cpssDxChTunnelStartHftPortGroupEntryGet function
* @endinternal
*
* @brief   Get Header Fields Table Entry.
*
* @note   APPLICABLE DEVICES:      AAS
* @param[in]  devNum                 - physical device number
* @param[in]  portGroupsBmp          - bitmap of Port Groups.
...
*
*/
GT_STATUS cpssDxChTunnelStartHftPortGroupEntryGet
(
    IN  GT_U8                          devNum,
    IN  GT_PORT_GROUPS_BMP             portGroupsBmp,
    IN  GT_U32                         entryIndex,
    IN  CPSS_DXCH_HFT_TABLE_ENT        hftTableId,
    OUT CPSS_DXCH_HFT_ENTRY_TYPE_ENT   *hftEntryTypePtr,
    OUT CPSS_DXCH_HFT_CONFIG_UNT       *configPtr
)
{
    GT_STATUS rc;

    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTunnelStartHftPortGroupEntryGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portGroupsBmp, entryIndex, hftTableId, hftEntryTypePtr, configPtr));

    rc = internal_cpssDxChTunnelStartHftPortGroupEntryGet(devNum, portGroupsBmp, entryIndex,
                                                          hftTableId, hftEntryTypePtr, configPtr);
    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, portGroupsBmp, entryIndex, hftTableId, hftEntryTypePtr, configPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
