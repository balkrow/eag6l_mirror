...
#define IPFIX_LOCK(_devNum, semId)   ipfixEmSemaLock(_devNum, semId)

#define IPFIX_UNLOCK(_devNum, semId) ipfixEmSemaUnlock(_devNum, semId)
...
)
{
    ...
    startBit = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 10 : 22;
    ...
    return prvCpssHwPpSetRegField(devNum, regAddr, startBit, 1, fieldValue);
}
...
)
{
    ...
    rc = prvCpssHwPpGetRegField(devNum, regAddr, startBit, 1, &fieldValue);
    if (GT_OK != rc)
    {
        return rc;
    }
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum,stage).policerIPFIXPktCountWAThresholdLSB;
    }
    else
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->
...
)
{
    ...
    portGroupIndex = 0;
    IPFIX_LOCK(devNum, HW_SEMA_IPFIX_EM);
        ...
        devNum, portGroupsBmp, portGroupId)
    {
        ...
        if (rc != GT_OK)
        {
           IPFIX_UNLOCK(devNum, HW_SEMA_IPFIX_EM);
...
)
{
    ...
    switch (reset)
    {
        ...
        case CPSS_DXCH_IPFIX_RESET_MODE_IPFIX_E:
            ...
            break;
        case CPSS_DXCH_IPFIX_RESET_MODE_IPFIX_DISABLED_E:
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
            {
                accessAction = PRV_CPSS_SIP_6_10_IPFIX_ENGINE_CNTR_READ_ONLY_E;
            }
            else
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
...
)
{
    ...
    *eventsNumPtr = 0;
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum,stage).ipfixSampleEntriesLog_n[1];
        }
        else
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->
        ...
        if(*eventsNumPtr >= CPSS_DXCH_IPFIX_ALARM_EVENTS_ARRAY_LENGTH_CNS)
        {
            /* the caller not hold more place in the array */
            break;
        }
        ...
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            regAddr = PRV_DXCH_REG1_UNIT_PLR_MAC(devNum,stage).ipfixSampleEntriesLog_n[0];
        }
        else
        {
            regAddr = PRV_CPSS_DXCH_DEV_REGS_MAC(devNum)->
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AAS_E);
...
)
{
    ...
    GT_STATUS   rc;             /* return code  */
    GT_U32      fieldValue;     /* field value for the register field */
    GT_U32      dsaCpuCode;     /* DSA code (from the register) */
