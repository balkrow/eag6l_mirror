...
static GT_STATUS prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskGetIronMan
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_ENT      memType,
    OUT CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_ENT   *errorTypePtr,
...
static GT_STATUS prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskIronMan
(
    IN  GT_U8                                           devNum,
    IN  CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_ENT      memType,
    IN  CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_ENT   errorType,
...
static GT_STATUS prvCpssDxChDiagDataIntegrityNonDfxEventsGetIronMan
(
    IN  GT_U8                                       devNum,
    IN  GT_U32                                      intNum,
    INOUT GT_U32                                    *eventsNumPtr,
...
static const GT_U32 eagleDfxPipeClientsBmpArr[] = {
...
static const GT_U32 ravenDfxPipeClientsBmpArr[] = {
...
)
{
    switch(PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
            *dbArrayPtrPtr = ironmanDataIntegrityDbArray;
            *dbArrayEntryNumPtr = ironmanDataIntegrityDbArrayEntryNum;
            break;
        default:
...
)
{
    ...
    switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
            rc = prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskIronMan(
                    devNum, memType, errorType, operation);
            break;
...
/**
* @internal prvCpssDxChDiagHarrierPramAccessEnableSet function
* @endinternal
*
* @brief   Function enables/disables access to PRAM DFX Memory wrapper registers of Harrier
*
* @note   APPLICABLE DEVICES:      Harrier.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Bobcat3; Aldrin2; Aldrin; AC3X; Falcon; AC5P; AC5X; Ironman.
*
* @param[in] devNum                   - PP device number
* @param[in] enable                   - enable or disable access
* @param[out] isConfigNeededPtr       - is configuration needed to enable access
*                                       GT_FALSE - not needed
*                                       GT_TRUE - needed including restore logic after access.
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on hardware error
*/
static GT_STATUS prvCpssDxChDiagHarrierPramAccessEnableSet
(
    IN GT_U8         devNum,
    IN GT_BOOL       enable,
    OUT GT_BOOL     *isConfigNeededPtr
)
{
    GT_STATUS rc;       /* return code          */
    ...
    regAddr = PRV_CPSS_HARRIER_PCI_E_SW_CONF_CONTROL_REG_ADDR_CNC;
    ...
    if (enable)
    {
        /* check bit#0 - Skip_Enable.
           value 0 - means that access is already enabled it's need not enable it again
           value 1 - means that access is disabled and need to reset bit */
        *isConfigNeededPtr = (regData & 1) ? GT_TRUE : GT_FALSE;
...
)
{
    ...
    GT_U32 regAddr;     /* address of register  */
    GT_U32 regData;     /* data of register     */
    GT_BOOL pramAccessEnable; /* enable write access to PRAM DFX Memory wrapper */
        ...
        (CPSS_PP_FAMILY_DXCH_AC5_E == PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        ...
        if ((CPSS_PP_FAMILY_DXCH_HARRIER_E == PRV_CPSS_PP_MAC(devNum)->devFamily) &&
            (memInfoPtr->memType == CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_PCIE_GEN3_EP_X1_U_SRAM_40960X32_E))
        {
            /* CPSS configures PCI_e related registers to perform fast Soft Reset procedure.
               These configurations disable write access to memory wrapper registers of this RAM.
               See more details in CPSS-14522 and driverHwCntlInitAndDeviceIdGet.
               Need to enable write access to this RAM before mask configuration. */
            pramAccessEnable = GT_TRUE;
        }
    }
    else
    {
    ...
    if (pramAccessEnable)
    {
        /* enable access to related registers */
        rc = prvCpssDxChDiagHarrierPramAccessEnableSet(devNum, GT_TRUE, &pramAccessEnable);
        if(GT_OK != rc)
        {
            return rc;
        }
    ...
    if (pramAccessEnable)
    {
        /* restore access to related registers */
        rc = prvCpssDxChDiagHarrierPramAccessEnableSet(devNum, GT_FALSE, &pramAccessEnable);
        if(GT_OK != rc)
        {
            return rc;
        }
...
)
{
    ...
    switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
            rc = prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskGetIronMan(
                devNum, memType, errorTypePtr, operationPtr);
            break;
...
/**
* @internal prvCpssDxChDiagDataIntegrityEventsGetIronMan function
* @endinternal
*
* @brief   Function performs interrupt tree scan for DataIntegrity event details
*         for IronMan devices.
...
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5X; AC5P; Harrier.
*
* @param[in] devNum                   - PP device number
* @param[in] evExtData                - event external data
* @param[in,out] eventsNumPtr        - in : max num of enabled events to
...
* @retval GT_BAD_STATE             - illegal HW state
*/
static GT_STATUS prvCpssDxChDiagDataIntegrityEventsGetIronMan
(
    IN  GT_U8                                       devNum,
    IN  GT_U32                                      evExtData,
    INOUT GT_U32                                    *eventsNumPtr,
...
)
{
    ...
    if ((intNum >= PRV_CPSS_IRONMAN_DFX_SERVER_INTERRUPT_SUMMARY_PIPE_0_INTERRUPT_SUM_E) &&
        (intNum <= PRV_CPSS_IRONMAN_DFX_SERVER_INTERRUPT_SUMMARY_PIPE_2_INTERRUPT_SUM_E))
    {
        /* handle DFX events for specific DFX Pipe */
        pipeNum = intNum - PRV_CPSS_IRONMAN_DFX_SERVER_INTERRUPT_SUMMARY_PIPE_0_INTERRUPT_SUM_E;
        rc = prvCpssDxChDiagDataIntegrityDfxEventsViaDfxServerGetSip6_10(
                devNum, pipeNum, eventsNumPtr, eventsArr, isNoMoreEventsPtr);
    ...
    {
        /* handle non-DFX interrupts */
        rc = prvCpssDxChDiagDataIntegrityNonDfxEventsGetIronMan(
                                            devNum, intNum,
                                            eventsNumPtr, eventsArr,
                                            isNoMoreEventsPtr);
    }
...
)
{
    ...
    GT_U32 i;
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AAS_E);
    ...
    switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
    {
        ...
        default:
            ...
            {
                /* IronMan */
                rc = prvCpssDxChDiagDataIntegrityEventsGetIronMan(devNum, evExtData,
                                                               eventsNumPtr, eventsArr,
                                                               isNoMoreEventsPtr);
            }
            else
            ...
            {
                /* not yet implemented (make the 'cpss_log' test pass) */
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    CPSS_NULL_PTR_CHECK_MAC(memEntryPtr);
...
)
{
    ...
    {
        return /* it's legal for regular usage */ GT_NOT_FOUND;
    }
    else
    {
        return GT_OK;
...
)
{
    ...
    GT_BOOL  enableInHw; /* state of the daemon in HW */
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        ...
        else
        {
            ...
            if (activeTcamBmp == 0)
            {
                /* All TCAMs are powered down. Daemon should be disabled in HW. */
                enableInHw = GT_FALSE;
            }
        }

        rc = prvCpssDxChDiagDataIntegrityPortGroupTcamParityDaemonEnableSet(
                             devNum, activeTcamBmp, enableInHw);
        if (rc != GT_OK)
        {
            return rc;
        }
...
* @retval GT_OK                    - on success
*/
GT_STATUS prvCpssDxChDfxPipesBmpGet
(
    IN  GT_U8  devNum,
...
)
{
    ...
    {
        /* use the global info */
        pipesBmp = PRV_CPSS_PP_MAC(devNum)->diagInfo.dfxPipesBmp;
    }
...
)
{
    ...
    */
    ...
    /* Falcon only                                                          */
    /* special case: Integrity error in SHM blocks mapped to FDB            */
    /* but not containg real data and not written at writing FDB entry      */
    /* HM_TYPE2_SBM_DATA memories 4 and 5                                   */
    /* the calculated hwTableEntryIndex are the same as in memories 0 and 1 */
    if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
    {
        if ((hwTableType == CPSS_DXCH_TABLE_FDB_E)
            && (baseTableId == PRV_CPSS_DXCH_DIAG_DATA_INTEGRITY_SPECIAL_TABLE_SHM_TYPE2_SBM_0_E)
            && (dbItemPtr->firstTableLine >= 8192))
        {
            /* mark entry index for special treatment setting special bit to 1 */
            hwErrorInfoPtr->hwTableEntryIndex |=
...
/**
* @internal prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskIronMan function
* @endinternal
*
* @brief   Function configures mask/unmask for non DFX protected memories.
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5X; AC5P; Harrier.
*
* @param[in] devNum                   - device number
* @param[in] memType                  - memory type
* @param[in] errorType                - error type
...
)
{
    ...
    {
        case CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_BMA_MC_COUNTERS_FLOOR_BANK_E:
            intIndex = PRV_CPSS_IRONMAN_BMA_MC_CNT_PARITY_ERROR_E;
            rc = prvCpssDrvEventsMask(devNum, intIndex, intMaskEn);
            break;

        case CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_HBU_HEADER_BUFFER_E:
            intIndex = PRV_CPSS_IRONMAN_HA_HA_INTERRUPT_CAUSE_ECC_SINGLE_ERROR_E;
            intIndex2 = PRV_CPSS_IRONMAN_HA_HA_INTERRUPT_CAUSE_ECC_DOUBLE_ERROR_E;
            rc = prvCpssDxChDiagDataIntegrityEccInterruptsMask(devNum, errorType, intMaskEn,
                                                               intIndex, intIndex2);
            break;
...
/**
* @internal prvCpssDxChDiagDataIntegrityExtMemoryProtectionEventMaskGetIronMan function
* @endinternal
*
* @brief   Function gets mask state for non DFX protected memories.
*
...
)
{
    ...
    GT_U32 regData; /* data of register    */
    GT_U32 regAddr; /* address of register */
    GT_U32 maskRegData; /* data of interrupt mask register    */
    GT_U32 maskRegAddr; /* address of of interrupt mask register */
        ...
        (CPSS_PP_FAMILY_DXCH_AC5_E == PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        ...
        maskRegAddr = PRV_CPSS_PP_MAC(devNum)->resetAndInitControllerRegsAddr.
                                    sip6_tile_DFXRam[0].memoryInterruptMask;
    }
    else
    {
        regAddr = PRV_CPSS_DXCH_DIAG_DATA_INTEGRITY_ADDR_TYPE_MEMORY_INT_CAUSE_REG_CNS;
        maskRegAddr = PRV_CPSS_DXCH_DIAG_DATA_INTEGRITY_ADDR_TYPE_MEMORY_INT_MASK_REG_CNS;
    }
    ...
    {
        /* Read Memory Interrupt Mask Register */
        rc = prvCpssDfxMemoryRegRead(devNum, pipeId, clientId, memId, maskRegAddr, &maskRegData);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* apply mask to filter out masked cause bits and use only ECC related bits 3:2 */
        regData &= (maskRegData & 0xC);
        ...
        switch (regData)
        {
            ...
            case 0x8:
                ...
                break;
            case 0xC:
                /* both bits 2 and 3 set */
                eventPtr->eventsType = CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_SINGLE_AND_MULTIPLE_ECC_E;
                break;
            default:
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
...
)
{
    ...
    GT_U32 regData;     /* data of interrupt cause register    */
    GT_U32 rc;          /* return code                         */
    GT_U32 regAddr;     /* address of of interrupt cause register */
    GT_U32 maskRegData; /* data of interrupt mask register    */
    GT_U32 maskRegAddr; /* address of of interrupt mask register */
    const PRV_CPSS_DFX_DIAG_DATA_INTEGRITY_MAP_DB_STC    *dbItemPtr; /* pointer to map DB item */
    GT_U32  dfxInstanceIndex; /* tile or chiplet instance index */
    GT_U32  portGroup;        /* port group                     */
    ...
    if (dfxInstanceType == CPSS_DIAG_DATA_INTEGRITY_DFX_INSTANCE_TILE_E)
    {
        ...
        maskRegAddr = PRV_CPSS_PP_MAC(devNum)->resetAndInitControllerRegsAddr.
                  sip6_tile_DFXRam[dfxInstanceIndex].memoryInterruptMask;
    }
    else if (dfxInstanceType == CPSS_DIAG_DATA_INTEGRITY_DFX_INSTANCE_CHIPLET_E)
    {
        ...
        maskRegAddr = PRV_CPSS_PP_MAC(devNum)->resetAndInitControllerRegsAddr.
                  sip6_chiplet_DFXRam[dfxInstanceIndex].memoryInterruptMask;
    }
    else
    {
        /* dfx instance type is invalid */
    ...
    else
    {
        ...
        rc = prvCpssDfxMemoryRegReadFalcon(
            devNum, pipeId, clientId, memId,
            maskRegAddr, dfxInstanceType, tileIndex, chipletIndex,
            &maskRegData);
...
/**
* @internal prvCpssDxChDiagDataIntegrityNonDfxEventsGetIronMan function
* @endinternal
*
* @brief   Function scans interrupt tree for non-DFX DataIntegrity event details.
*
...
)
{
    ...
    *eventsNumPtr = 1;          /* default value */
    ...
    switch(intNum)
    {
        case PRV_CPSS_IRONMAN_TCAM_TCAM_INTERRUPT_CAUSE_TCAM_ARRAY_PARITY_ERROR_E:
            isTcam = GT_TRUE;
            break;

        case PRV_CPSS_IRONMAN_BMA_MC_CNT_PARITY_ERROR_E:
            eventsArr[0].location.ramEntryInfo.memType          = CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_BMA_MC_COUNTERS_FLOOR_BANK_E;
            eventsArr[0].eventsType = CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_PARITY_E;
            break;

        case PRV_CPSS_IRONMAN_HA_HA_INTERRUPT_CAUSE_ECC_SINGLE_ERROR_E:
            eventsArr[0].location.ramEntryInfo.memType          = CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_HBU_HEADER_BUFFER_E;
            eventsArr[0].eventsType = CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_SINGLE_ECC_E;
            break;

        case PRV_CPSS_IRONMAN_HA_HA_INTERRUPT_CAUSE_ECC_DOUBLE_ERROR_E:
            eventsArr[0].location.ramEntryInfo.memType          = CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_HBU_HEADER_BUFFER_E;
            eventsArr[0].eventsType = CPSS_DIAG_DATA_INTEGRITY_ERROR_CAUSE_TYPE_MULTIPLE_ECC_E;
            break;
...
)
{
    ...
    PRV_CPSS_PP_MAC(devNum)->diagInfo.dfxPipesBmpGetFunc = PRV_CALLBACK_ID_GET(prvCpssDxChDfxPipesBmpGet);
...
)
{
    ...
    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, cpuCodePtr, pktCmdPtr));
...
GT_VOID * prvCpssDxChDiagCallbacksBind
(
    GT_VOID
)
{
    /*due to const will be located in read only section*/
     static GT_STATUS (* const prvCpssDiagFunctionDb[])(GT_U8 ,GT_U32 *) =
    {
       prvCpssDxChDfxPipesBmpGet,
       NULL
    };

    return (GT_VOID *)prvCpssDiagFunctionDb;
}
