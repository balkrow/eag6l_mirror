...
#define IS_NOT_IMPLEMENTED_DEV_CHECK_MAC(_devNum) \
    if (GT_TRUE == PRV_CPSS_PP_MAC(_devNum)->diagInfo.notSupportPacketGenerator)\
    {                                                                           \
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_IMPLEMENTED, "Packet Generator APIs not implemented for the device"); \
    }
...
/* Repeat count value for continuous mode */
#define PG_CONTINUOUSE_COUNT_CNS 255

/* The channel is disconnected from PG */
#define PG_NOT_VALID_CHANNEL_CNS                 0x3f
/*
...
} PRV_PG_CHANNEL_INFO_STC;

static GT_STATUS prvCpssDxChDiagPacketGeneratoPortlnfoGet
(
    IN    GT_U8                                devNum,
    IN    GT_PHYSICAL_PORT_NUM                 portNum,
    OUT   PRV_PG_CHANNEL_INFO_STC             *expChannelInfoPtr,
    OUT   PRV_PG_CHANNEL_INFO_STC             *preChannelInfoPtr
);

static GT_STATUS prvCpssDxChDiagPacketGeneratorStatusGet
(
    IN    GT_U8                             devNum,
...
)
{
    ...
    if (configPtr->transmitMode == CPSS_DIAG_PG_TRANSMIT_SINGLE_BURST_E)
    {
        ...
        if ((GT_U32)configPtr->packetCountMultiplier > (GT_U32)CPSS_DIAG_PG_PACKET_COUNT_MULTIPLIER_16M_E)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
    }
...
/**
* @internal prvCpssDxChDiagPacketGeneratorPortConnectionStatus function
* @endinternal
*
* @brief   Get packet generator connection status on specified port.
*
...
* @param[out] connectedPtr          - (pointer to) port connection status
*                                       GT_TRUE - port connected to packet generator
*                                       GT_FALSE - port is not connected to any packet generator
* @param[out] packGenIndexPtr       - (pointer to) packet generator index
*                                       if connected == true, returns packet generator index of connected port,
*                                       if connected == false, returns packet generator index of not connected packet generator
*
* @retval GT_OK                     - on success
* @retval GT_NO_MORE                - all packet generators connected to other ports
* @retval GT_BAD_PTR                - one of the parameters is NULL pointer
*
*
*/
static GT_STATUS prvCpssDxChDiagPacketGeneratorPortConnectionStatus
(
    IN  GT_U8                devNum,
    IN  GT_PHYSICAL_PORT_NUM portNum,
    OUT GT_BOOL             *connectedPtr,
    OUT GT_U32              *packGenIndexPtr
)
{
    GT_STATUS   rc;           /* function return value */
    ...
    GT_U32      regAddr;      /* address of register */
    GT_U32      data;         /* data of register or field */
    GT_BOOL     isConnected;  /* is packet generator already connected or not */
    GT_U32      ii;           /* iteration index */
    GT_U32      packGenIndex; /* packet generator index */

    PRV_PG_CHANNEL_INFO_STC expressChannelInfo;     /* express channel info */
    PRV_PG_CHANNEL_INFO_STC preemptiveChannelInfo;  /* preeemtive channel info */
    PRV_PG_CHANNEL_INFO_STC *expressChannelInfoPtr = &expressChannelInfo;
    PRV_PG_CHANNEL_INFO_STC *preemptiveChannelInfoPtr = &preemptiveChannelInfo;

    CPSS_NULL_PTR_CHECK_MAC(connectedPtr);
    CPSS_NULL_PTR_CHECK_MAC(packGenIndexPtr);

    cpssOsMemSet(expressChannelInfoPtr,    0, sizeof(PRV_PG_CHANNEL_INFO_STC));
    cpssOsMemSet(preemptiveChannelInfoPtr, 0, sizeof(PRV_PG_CHANNEL_INFO_STC));

    rc = prvCpssDxChDiagPacketGeneratoPortlnfoGet(devNum, portNum,
                                               expressChannelInfoPtr, preemptiveChannelInfoPtr);
    if (rc != GT_OK)
    {
        return rc;
    }

    *connectedPtr = GT_FALSE;
    packGenIndex = CPSS_DXCH_DIAG_PG_INSTANCES_CNS;

    /* Check status of PG instances */
    for (ii = 0; ii < CPSS_DXCH_DIAG_PG_INSTANCES_CNS; ii++)
    {
        /* Tested Channel Control <<%n>> */
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelControl[ii];
        rc = prvCpssDrvHwPpReadRegister(devNum, regAddr, &data);
        if (rc != GT_OK)
        {
            return rc;
        ...
        rc = prvCpssDxChDiagPacketGeneratorStatusGet(devNum, ii, &isConnected);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Current PG connected to port */
        if (isConnected == GT_TRUE)
        {
            /* The PG is connected to the specific port */
            if (expressChannelInfoPtr->channelNum == chId)
            {
                /* Get PG index for connected port */
                *connectedPtr = GT_TRUE;
                packGenIndex = ii;
                break;
            }
        }
        ...
        {
            /* No port connected to current PG */
            packGenIndex = ii;
        }
    }

    if (packGenIndex == CPSS_DXCH_DIAG_PG_INSTANCES_CNS)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NO_MORE, LOG_ERROR_NO_MSG);
    }

    /* Return valid PG index */
    *packGenIndexPtr = packGenIndex;

    return GT_OK;
}

/**
* @internal prvCpssDxChDiagPacketGeneratorTransmitEnable function
* @endinternal
*
...
*
*/
static GT_STATUS prvCpssDxChDiagPacketGeneratorTransmitEnable
(
    IN  GT_U8                devNum,
    IN  GT_PHYSICAL_PORT_NUM portNum,
    IN  GT_BOOL              enable
)
{
    ...
    /* Check port connection to packet generator */
    rc = prvCpssDxChDiagPacketGeneratorPortConnectionStatus(devNum, portNum, &isConnected, &packGenIndex);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    return cpssDxChDiagPacketGeneratorTransmitStart(devNum, packGenIndex, enable);
}
...
)
{
    ...
    GT_BOOL     isConnected;  /* is PG already connected or not */
    GT_U32      pgChIdx;      /* channel id of packet generator */
    GT_BOOL     restoreCount; /* need to restore value of count */
    ...
    restoreCount = GT_FALSE;

    if (enable == GT_FALSE)
    {
        /* read Repeat Count to check mode */
        rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 16, 8, &data);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* gracefully disable PG in continuous mode */
        if (data == PG_CONTINUOUSE_COUNT_CNS)
        {
            /* change repeat count to be 'one repeat' */
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 16, 8, 0);
            if (rc != GT_OK)
            {
                return rc;
            }

            restoreCount = GT_TRUE;

            /* make sure that last frame was transmitted.
               wait for TX of 256 packets of maximal size 2000 byte for 1Gbps mode */
    ...
    {
        /* restore value back */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 16, 8, PG_CONTINUOUSE_COUNT_CNS);
        if (rc != GT_OK)
        {
            return rc;
        }
...
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5X; Ironman.
*
* @param[in] devNum                - device number
* @param[in] dpIdx                 - data path index
...
)
{
    ...
    GT_U32      frameRepeatCount; /* frame repeat count of PG */
    ...
    if (connect)
    {
        ...
        *headerPtr++ = (GT_U8)(configPtr->etherType & 0xFF);
        ...
        if (configPtr->packetLengthType == CPSS_DIAG_PG_PACKET_LENGTH_CONSTANT_E)
        {
            ...
            /* get value Idle Words from DB based on packet size and speed */
            idleWord = GT_NA;
        ...
        /* configure payload */
        data =  configPtr->cyclicPatternArr[3]        | (configPtr->cyclicPatternArr[2] << 8) |
               (configPtr->cyclicPatternArr[1] << 16) | (configPtr->cyclicPatternArr[0] << 24);
        ...
        if (configPtr->transmitMode == CPSS_DIAG_PG_TRANSMIT_CONTINUES_E)
        {
            ...
            /* continuous*/
            repeatCount = PG_CONTINUOUSE_COUNT_CNS;
        }
        else
        {
            frameRepeatCount = (configPtr->packetCount - 1) & 0xFF;
            repeatCount = 0;
        }

        /* configure number of packets and idle word */
        data =  (frameRepeatCount << 16) | ((idleWord & 0xFF) << 8);
...
/**
* @internal prvCpssDxChDiagPacketGeneratorConnectSet function
* @endinternal
*
* @brief  Connect/Disconnect port to packet generator.
*         Set packet generator's configurations.
...
* @param[in] portNum                - physical port number to connect
*                                     its MAC to packet generator.
* @param[in] connect                - GT_TRUE:  connect the given port's MAC to packet generator.
*                                     GT_FALSE: disconnect the given port's MAC from packet generator.
* @param[in] configPtr              - (pointer to) packet generator configurations.
*                                     Relevant only if enable==GT_TRUE
*
* @retval GT_OK                     - on success
* @retval GT_BAD_PTR                - the parameter is NULL pointer
*
*/
static GT_STATUS prvCpssDxChDiagPacketGeneratorConnectSet
(
    IN GT_U8                                 devNum,
    IN GT_PHYSICAL_PORT_NUM                  portNum,
    IN GT_BOOL                               connect,
...
)
{
    ...
    GT_U32      packGenIndex; /* packet generator index */
    GT_BOOL     isConnected;  /* is packet generator already connected or not */
    CPSS_DXCH_DIAG_PG_PROFILE_CONFIG_STC profileConfig; /* profile configuration info */
    CPSS_DXCH_DIAG_PG_CONFIG_STC         pgConfig;      /* packet generation info */

    /* Check port connection to PG */
    rc = prvCpssDxChDiagPacketGeneratorPortConnectionStatus(devNum, portNum, &isConnected, &packGenIndex);
    if(rc != GT_OK)
    {
        return rc;
    ...
    if (isConnected == GT_TRUE)
    {
        if (connect == GT_FALSE)
        {
            /* Disconnect current port from PG */
            return cpssDxChDiagPacketGeneratorConfigSet(devNum, packGenIndex, GT_FALSE, NULL);
        }
    }
    else
    {
    ...
    cpssOsMemSet(&profileConfig, 0, sizeof(CPSS_DXCH_DIAG_PG_PROFILE_CONFIG_STC));
    cpssOsMemSet(&pgConfig, 0, sizeof(CPSS_DXCH_DIAG_PG_CONFIG_STC));

    /* Configure profile */
    /* MAC DA */
    cpssOsMemCpy(profileConfig.macDa.arEther, configPtr->macDa.arEther, 6);
    /* MAC SA */
    cpssOsMemCpy(profileConfig.macSa.arEther, configPtr->macSa.arEther, 6);
    /* VLAN tag */
    if (configPtr->vlanTagEnable)
    {
        profileConfig.vlanTagEnable = configPtr->vlanTagEnable;
        profileConfig.vpt = configPtr->vpt;
        profileConfig.cfi = configPtr->cfi;
        profileConfig.vid = configPtr->vid;
    }
    /* Ethernet type */
    profileConfig.etherType = configPtr->etherType;
    /* Payload type */
    profileConfig.payloadType = configPtr->payloadType;
    /* Payload pattern arrray */
    profileConfig.payloadPatternArr[0] = configPtr->cyclicPatternArr[0];
    profileConfig.payloadPatternArr[1] = configPtr->cyclicPatternArr[1];

    /* Packet length */
    profileConfig.packetLength = configPtr->packetLength;

    /* Call the API for profile configuration */
    rc = cpssDxChDiagPacketGeneratorProfileConfigSet(devNum, packGenIndex, &profileConfig);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* Init PG configurations */

    /* Physical port */
    pgConfig.portNum = portNum;
    /* Profile indexes */
    pgConfig.profileIndexes[0] =
    pgConfig.profileIndexes[1] =
    pgConfig.profileIndexes[2] =
    pgConfig.profileIndexes[3] = packGenIndex;

    /* Transmit mode */
    pgConfig.transmitMode = configPtr->transmitMode;

    /* Packets number */
    pgConfig.packetsNumber = configPtr->packetCount;

    /* Call API for PG configuration */
    return cpssDxChDiagPacketGeneratorConfigSet(devNum, packGenIndex, GT_TRUE, &pgConfig);
}
...
*         Set packet generator's configurations.
*
* @note   APPLICABLE DEVICES:      Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; AC5P; AC5X; Harrier; Ironman.
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_FALCON_E | CPSS_AAS_E);
    IS_NOT_IMPLEMENTED_DEV_CHECK_MAC(devNum);
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ...
        {
            return prvCpssDxChDiagPacketGeneratorConnectSet(devNum, portNum, connect, configPtr);
        }
        else
        {
...
)
{
    ...
    /* restore payload */
    configPtr->cyclicPatternArr[3] = (GT_U8)(data);
    configPtr->cyclicPatternArr[2] = (GT_U8)(data >> 8);
    configPtr->cyclicPatternArr[1] = (GT_U8)(data >> 16);
    configPtr->cyclicPatternArr[0] = (GT_U8)(data >> 24);
    ...
    configPtr->transmitMode = (repeatCount == PG_CONTINUOUSE_COUNT_CNS) ? CPSS_DIAG_PG_TRANSMIT_CONTINUES_E : CPSS_DIAG_PG_TRANSMIT_SINGLE_BURST_E;
...
/**
* @internal prvCpssDxChDiagPacketGeneratorConnectGet function
* @endinternal
*
* @brief  Get connection status of port to packet generator.
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier.
*
...
* @retval GT_BAD_PTR                - one of the parameters is NULL pointer
*/
static GT_STATUS prvCpssDxChDiagPacketGeneratorConnectGet
(
    IN GT_U8                                    devNum,
    IN GT_PHYSICAL_PORT_NUM                     portNum,
    OUT GT_BOOL                                *connectPtr,
...
)
{
    ...
    /* Get packet generator configuration */
    rc = cpssDxChDiagPacketGeneratorConfigGet(devNum, packGenIndex, connectPtr, &pgConfig);
    if (rc != GT_OK)
    {
        return rc;
    }

    /* Get packet generator's profile configuration */
    rc = cpssDxChDiagPacketGeneratorProfileConfigGet(devNum, pgConfig.profileIndexes[0], &profileConfig);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* MAC DA */
    cpssOsMemCpy(configPtr->macDa.arEther, profileConfig.macDa.arEther, 6);
    /* MAC SA */
    cpssOsMemCpy(configPtr->macSa.arEther, profileConfig.macSa.arEther, 6);
    /* VLAN tag */
    if (profileConfig.vlanTagEnable)
    {
        configPtr->vlanTagEnable = profileConfig.vlanTagEnable;
        configPtr->vpt = profileConfig.vpt;
        configPtr->cfi = profileConfig.cfi;
        configPtr->vid = profileConfig.vid;
    }
    /* Ethernet type */
    configPtr->etherType = profileConfig.etherType;
    /* Payload type */
    configPtr->payloadType = profileConfig.payloadType;
    /* Payload pattern arrray */
    configPtr->cyclicPatternArr[0] = profileConfig.payloadPatternArr[0];
    configPtr->cyclicPatternArr[1] = profileConfig.payloadPatternArr[1];
    /* Packet length */
    configPtr->packetLength = profileConfig.packetLength;
    /* Transmit mode */
    configPtr->transmitMode = pgConfig.transmitMode;
    /* Packets number */
    configPtr->packetCount = pgConfig.packetsNumber;
...
)
{
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            return prvCpssDxChDiagPacketGeneratorConnectGet(devNum, portNum, connectPtr, configPtr);
        }
        else
        {
...
)
{
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            return prvCpssDxChDiagPacketGeneratorTransmitEnable(devNum, portNum, enable);
        }
        else
        {
...
/**
* @internal prvCpssDxChDiagIronmanPacketGeneratorProfileHeaderCheck function
* @endinternal
*
* @brief   Check parameters of PG profile configuration
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier.
*
...
* @retval GT_OUT_OF_RANGE          - on param is out of range
*/
static GT_STATUS prvCpssDxChDiagIronmanPacketGeneratorProfileHeaderCheck
(
...
)
{
    ...
    if (configPtr->packetLength < 60 || configPtr->packetLength > 10240)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }

    if (configPtr->rateControlGap > 15)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }
...
/**
* @internal internal_cpssDxChDiagPacketGeneratorProfileConfigSet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorProfileConfigSet
(
    IN GT_U8                                  devNum,
...
{
    GT_STATUS rc;
    GT_U8     header[IRONMAN_MAX_PCA_PG_HEADER_BYTES_CNS];    /* data of packet's header in Network order */
    GT_U8    *headerPtr;    /* pointer to byte in header */
    GT_U32    regAddr;      /* address of register */
    GT_U32    data;         /* data of register or field */
    GT_U32    payloadType;  /* filling payload type */
    ...
    rc = prvCpssDxChDiagIronmanPacketGeneratorProfileHeaderCheck(configPtr);
    if( rc != GT_OK )
    {
        return rc;
    }
    ...
    *headerPtr += 2;
    ...
    cpssOsMemCpy(headerPtr, configPtr->headerByteArr, configPtr->vlanTagEnable ? 13 : 17);

    /* write header to registers */
    for(ii = 0; ii < IRONMAN_MAX_PCA_PG_HEADER_REGS_CNS; ii++)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgProfileHeaderCfg[profileIndex][ii];
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgProfileGapsCfg[profileIndex];
    rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 4, configPtr->rateControlGap);
    if (rc != GT_OK)
    {
        return rc;
    }

    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgProfilePayloadCfg[profileIndex];
    switch (configPtr->payloadType)
    {
        case CPSS_DIAG_PG_PACKET_PAYLOAD_CONSTANT_E:
            payloadType = 0;
    ...
    data = (configPtr->payloadPatternArr[0] |
            configPtr->payloadPatternArr[1] << 8 |
            configPtr->packetLength << 16 |
            payloadType << 30);
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorProfileConfigSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, profileIndex, configPtr));

    rc = internal_cpssDxChDiagPacketGeneratorProfileConfigSet(devNum, profileIndex, configPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, profileIndex, configPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorProfileConfigGet
(
    IN  GT_U8                                  devNum,
    IN  GT_U32                                 profileIndex,
...
)
{
    ...
    cpssOsMemSet(header, 0, sizeof(header));
    cpssOsMemSet(configPtr, 0, sizeof(CPSS_DXCH_DIAG_PG_PROFILE_CONFIG_STC));
    ...
    headerPtr += 2;
    /* copy the rest of the header data */
    cpssOsMemCpy(configPtr->headerByteArr, headerPtr, configPtr->vlanTagEnable ? 13 : 17);


    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgProfileGapsCfg[profileIndex];
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 0, 4, &configPtr->rateControlGap);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    configPtr->payloadPatternArr[0] = data & 0xFF;
    configPtr->payloadPatternArr[1] = (data >> 8) & 0xFF;
    configPtr->packetLength = (data >> 16) & 0x3FFF;
    payloadType = (data >> 30) & 0x3;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorProfileConfigGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, profileIndex, configPtr));

    rc = internal_cpssDxChDiagPacketGeneratorProfileConfigGet(devNum, profileIndex, configPtr);
...
/**
* @internal prvCpssDxChDiagIronmanPacketGeneratorConfigParamCheck function
* @endinternal
*
* @brief   Check parameters of PG connect configuration
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  Lion2; xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier.
*
...
* @retval GT_OUT_OF_RANGE          - on param is out of range
*/
static GT_STATUS prvCpssDxChDiagIronmanPacketGeneratorConfigParamCheck
(
    IN GT_U8                                 devNum,
...
)
{
    ...
    PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, configPtr->portNum, portMacNum);

    switch (configPtr->preemptionClass)
    {
        case CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E:
        case CPSS_PORT_PREEMPTION_CLASS_PREEMPTIVE_E:
            break;
        default:
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ERROR_NO_MSG);
    }

    for (ii = 0; ii < CPSS_DXCH_DIAG_PG_MAX_PROFILES_CNS; ii++)
    {
        if (configPtr->profileIndexes[ii] > 3)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
    }
    ...
    if (configPtr->transmitMode == CPSS_DIAG_PG_TRANSMIT_SINGLE_BURST_E)
    {
        if (configPtr->packetsNumber > 0xFFFF)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
    }
...
/**
* @internal prvCpssDxChDiagPacketGeneratoPortlnfoGet function
* @endinternal
*
* @brief    Get channels information per physical port.
*
* @note   APPLICABLE DEVICES:     Ironman.
...
* @param[in]  devNum                - device number
* @param[in]  portNum               - physical port number
* @param[out] expChannelInfoPtr     - (pointer to) express channel info structure
* @param[out] preChannelInfoPtr     - (pointer to) preeemptive channel info structure
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PTR               - one of the parameters is NULL pointer
*/
...
)
{
    ...
    CPSS_NULL_PTR_CHECK_MAC(expChannelInfoPtr);
    CPSS_NULL_PTR_CHECK_MAC(preChannelInfoPtr);

    PRV_CPSS_DXCH_PORT_NUM_OR_CPU_PORT_CHECK_AND_TXDMA_NUM_GET_MAC(
        devNum, portNum, globalTxDma);

    /* Ironman has one DP only */
    expChannelInfoPtr->channelNum = globalTxDma;

    rc = prvCpssDxChTxqSip6_10PreemptionEnableGet(devNum, portNum,
                                                  0 /*not used */,
                                                  &expChannelInfoPtr->preemptiveModeActivated);
    if (rc != GT_OK)
    {
        return rc;
    }

    if (expChannelInfoPtr->preemptiveModeActivated == GT_TRUE)
    {
        /*get additional channel*/
        rc = prvCpssDxChTxqSip6_10PreChannelGet(devNum,
                                                0 /* not used */,
                                                expChannelInfoPtr->channelNum,
                                                &preChannelInfoPtr->channelNum);
        if (rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal prvCpssDxChDiagPacketGeneratorStatusGet function
* @endinternal
*
* @brief    Get packet generator status.
*
* @note   APPLICABLE DEVICES:     Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon;  AC5P; AC5X; Harrier.
*
...
* @param[out] enablePtr            - (pointer to) PG status
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PTR               - the parameter is NULL pointer
*/
...
)
{
    ...
    /* Tested Channel Control <<%n>> */
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelControl[packetGeneratorIndex];
    rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 2, 6, &data);
    if (rc != GT_OK)
    {
        return rc;
    }
    /* Ch %n Tested CHID */
    if (data != PG_NOT_VALID_CHANNEL_CNS)
    {
        *enablePtr = GT_TRUE;
    }
    else
...
/**
* @internal prvCpssDxChDaigPacketGeneratorGlobalStatusGet function
* @endinternal
*
* @brief    Get packet generator global status.
*
* @note   APPLICABLE DEVICES:     Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon;  AC5P; AC5X; Harrier.
*
* @param[in]  devNum               - device number
* @param[out] enablePtr            - (pointer to) PGC global status
*                                       GT_TRUE     - at list one PG instance is enabled
*                                       GT_FALSE    - all PG instances are disabled
*
* @retval GT_OK                    - on success
* @retval GT_HW_ERROR              - on hardware error
*/
static GT_STATUS prvCpssDxChDaigPacketGeneratorGlobalStatusGet
(
    IN    GT_U8                             devNum,
    OUT   GT_BOOL                          *enablePtr
)
{
    ...
    for (ii = 0; ii < CPSS_DXCH_DIAG_PG_INSTANCES_CNS; ii++)
    {
        rc = prvCpssDxChDiagPacketGeneratorStatusGet(devNum, ii, enablePtr);
        if (rc != GT_OK)
        {
            return rc;
        }
...
/**
* @internal internal_cpssDxChDiagPacketGeneratorConfigSet function
* @endinternal
*
...
* @retval GT_NOT_SUPPORTED         - on not supported port
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorConfigSet
(
    IN GT_U8                                 devNum,
    IN GT_U32                                packetGeneratorIndex,
    IN GT_BOOL                               enable,
...
)
{
    ...
    GT_U32                      regAddr;        /* address of register */
    GT_U32                      data;           /* data of register or field */
    GT_BOOL                     pgcEnable;      /* PGC global status */
    GT_U32                      numOfCredits;   /* Number of credits allocated per port */
    GT_U32                      portMac;        /* MAC number */
    CPSS_PORT_SPEED_ENT         speed;          /* port speed */
    GT_U32                      speedInMbps;    /* speed in MB per second */
    PRV_PG_CHANNEL_INFO_STC     expressChannelInfo;       /* express chanel info */
    PRV_PG_CHANNEL_INFO_STC     preemptiveChannelInfo;    /* preemptive chanel info */
    PRV_PG_CHANNEL_INFO_STC     *expressChannelInfoPtr = &expressChannelInfo;
    PRV_PG_CHANNEL_INFO_STC     *preemptiveChannelInfoPtr = &preemptiveChannelInfo;
    ...
    if (packetGeneratorIndex >= CPSS_DXCH_DIAG_PG_INSTANCES_CNS)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    ...
    if (enable == GT_TRUE)
    {
        ...
        /* Get global status before PG configuration and if not active - enable it at the end of the sequence */
        rc = prvCpssDxChDaigPacketGeneratorGlobalStatusGet(devNum, &pgcEnable);
        if (rc != GT_OK)
        {
            return rc;
        }

        rc = prvCpssDxChDiagIronmanPacketGeneratorConfigParamCheck(devNum, configPtr);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Get port's chanel info */
        rc = prvCpssDxChDiagPacketGeneratoPortlnfoGet(devNum, configPtr->portNum,
                                                   expressChannelInfoPtr, preemptiveChannelInfoPtr);
        if (rc != GT_OK)
        {
            return rc;
        ...
        if (expressChannelInfoPtr->preemptiveModeActivated)
        {
            /* Configure Ch %n Tested CHID */
            if (configPtr->preemptionClass == CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E)
            {
                data = expressChannelInfoPtr->channelNum << 2;
            }
            else
            {
                data = preemptiveChannelInfoPtr->channelNum << 2;
            }
        }
        else
        {
        ...
        PRV_CPSS_DXCH_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, configPtr->portNum, portMac);
        speed = PRV_CPSS_DXCH_PORT_SPEED_MAC(devNum, portMac);
        speedInMbps = prvCpssCommonPortSpeedEnumToMbPerSecConvert(speed);

        numOfCredits = (speedInMbps < 5000)  ? 10 :
                       (speedInMbps < 12000) ? 20 : 22;

        /* Configure Ch %n Profile Indexes */
        data |= (configPtr->profileIndexes[0] << 8)  | (configPtr->profileIndexes[1] << 10)  |
                (configPtr->profileIndexes[2] << 12) | (configPtr->profileIndexes[3] << 14)  | numOfCredits << 16;

        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 2, 19, (data >> 2));
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Configure Ch_%n_2Credit_eop - use two credits */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 24, 1, 1);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Configure Ch %n finish test by packet number */
        data = (configPtr->transmitMode == CPSS_DIAG_PG_TRANSMIT_CONTINUES_E) ? 0 : 1;

        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 27, 1, data);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Ch %n stop test - do not stop test in loop mode */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 28, 1, 0);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelPacketNumber[packetGeneratorIndex];

        data = configPtr->packetsNumber;
    ...
    else
    {
        /* Tested Channel Control <<%n>> */
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelControl[packetGeneratorIndex];
        data = PG_NOT_VALID_CHANNEL_CNS << 2;
        /* Ch %n Tested Enable + Ch %n Tested CHID */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 8, data);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Get global status after PG configuration and if not active - disable it at the end of the sequence */
        rc = prvCpssDxChDaigPacketGeneratorGlobalStatusGet(devNum, &pgcEnable);
        if (rc != GT_OK)
        {
            return rc;
    ...
    if (pgcEnable == GT_FALSE)
    {
        /* Global Configurations */
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgGlobalCfg;
        if (enable == GT_TRUE)
        {
            /* Global Enable - Enable PGC */
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorConfigSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetGeneratorIndex, enable, configPtr));

    rc = internal_cpssDxChDiagPacketGeneratorConfigSet(devNum, packetGeneratorIndex, enable, configPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, packetGeneratorIndex, enable, configPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChDiagPacketGeneratorConfigGet function
* @endinternal
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorConfigGet
(
    IN  GT_U8                               devNum,
    IN  GT_U32                              packetGeneratorIndex,
    OUT GT_BOOL                            *enablePtr,
...
)
{
    ...
    PRV_PG_CHANNEL_INFO_STC expressChannelInfo;
    PRV_PG_CHANNEL_INFO_STC preemptiveChannelInfo;
    PRV_PG_CHANNEL_INFO_STC *expressChannelInfoPtr = &expressChannelInfo;
    PRV_PG_CHANNEL_INFO_STC *preemptiveChannelInfoPtr = &preemptiveChannelInfo;
    ...
    cpssOsMemSet(configPtr, 0, sizeof(CPSS_DXCH_DIAG_PG_CONFIG_STC));
    cpssOsMemSet(expressChannelInfoPtr,    0, sizeof(PRV_PG_CHANNEL_INFO_STC));
    cpssOsMemSet(preemptiveChannelInfoPtr, 0, sizeof(PRV_PG_CHANNEL_INFO_STC));
    ...
    /* Ch %n Tested CHID */
    localPort = (data >> 2) & 0x3f;
    /* Ch %n Profile Indexes */
    configPtr->profileIndexes[0] = (data >>  8) & 0x3;
    configPtr->profileIndexes[1] = (data >> 10) & 0x3;
    configPtr->profileIndexes[2] = (data >> 12) & 0x3;
    configPtr->profileIndexes[3] = (data >> 14) & 0x3;

    /* Ch %n finish test by packet number */
    configPtr->transmitMode = ((data >> 27) == 0) ? CPSS_DIAG_PG_TRANSMIT_CONTINUES_E : CPSS_DIAG_PG_TRANSMIT_SINGLE_BURST_E;
    ...
    /* Ch %n Num Pkt Sent */
    configPtr->packetsNumber = data;

    if (localPort != PG_NOT_VALID_CHANNEL_CNS)
    {
        /* Get physical port */
        rc = prvCpssDxChPortPhysicalPortMapReverseMappingGet(devNum, PRV_CPSS_DXCH_PORT_TYPE_TxDMA_E, localPort, &portNum);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* Get port's chanels info */
        rc = prvCpssDxChDiagPacketGeneratoPortlnfoGet(devNum, portNum,
                                                   expressChannelInfoPtr, preemptiveChannelInfoPtr);
        if (rc != GT_OK)
        {
        ...
        if (expressChannelInfoPtr->preemptiveModeActivated == GT_FALSE)
        {
            configPtr->preemptionClass = CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E;
        }
        else
        {
            if (expressChannelInfoPtr->channelNum == localPort)
            {
                configPtr->preemptionClass = CPSS_PORT_PREEMPTION_CLASS_EXPRESS_E;
            }
            else
            {
                configPtr->preemptionClass = CPSS_PORT_PREEMPTION_CLASS_PREEMPTIVE_E;
            }
        }

        configPtr->portNum = portNum;

        *enablePtr = GT_TRUE;
    }
    else
    {
        configPtr->portNum = PG_NOT_VALID_CHANNEL_CNS;
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorConfigGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetGeneratorIndex, enablePtr, configPtr));

    rc = internal_cpssDxChDiagPacketGeneratorConfigGet(devNum, packetGeneratorIndex, enablePtr, configPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, packetGeneratorIndex, enablePtr, configPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal prvCpssDxChDaigPacketGeneratorCreditCounterBusyWait function
* @endinternal
*
* @brief   Query the PG channel credit conter status in busy wait.
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5; AC5P; AC5X; Harrier.
*
* @param[in] devNum                - device number
* @param[in] packetGeneratorIndex  - packet generator index
*                                    (APPLICABLE RANGES: 0..1)
...
*/
static GT_STATUS prvCpssDxChDaigPacketGeneratorCreditCounterBusyWait
(
    IN  GT_U8   devNum,
    IN  GT_U32  packetGeneratorIndex
)
{
    ...
    GT_U32      expCreditsNumber;   /* Tested channel maximal number of credits */
    GT_U32      hwValue;            /* HW register value */

    busyIterator = 0;

    /* Tested Channel Control <<%n>> */
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelControl[packetGeneratorIndex];
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 16, 5, &expCreditsNumber);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    /* Tested Channel Control <<%n>> */
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelRateControl[packetGeneratorIndex];

    /* use "busy" wait */
    do
    {
        /* Ch-%n Rate Control Credit Counter Status */
        rc = prvCpssHwPpGetRegField(devNum, regAddr, 0, 5, &hwValue);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    while ((hwValue & expCreditsNumber) != expCreditsNumber);
...
*
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorTransmitStart
(
    IN  GT_U8                devNum,
    IN  GT_U32               packetGeneratorIndex,
    IN  GT_BOOL              start
)
{
    ...
    /* Packet generator connection status */
    rc = prvCpssDxChDiagPacketGeneratorStatusGet(devNum, packetGeneratorIndex, &enable);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    if (start == GT_TRUE)
    {
        /* Ch %n Tested Enable - reset PGC channel before start */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 1, 0);
        if (rc != GT_OK)
        {
            return rc;
        }
        /* Ch %n Tested Enable - start PGC traffic */
        rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 0, 1, 1);
        if (rc != GT_OK)
        {
            return rc;
    ...
    {
        /* Ch %n finish test by packet number/loop mode */
        rc = prvCpssDrvHwPpGetRegField(devNum, regAddr, 27, 1, &data);
        if (rc != GT_OK)
        {
            return rc;
        }

        /* The PGC is running in loop mode */
        if (data == 0)
        {
            /* Ch %n stop test */
            rc = prvCpssDrvHwPpSetRegField(devNum, regAddr, 28, 1, 1);
            if (rc != GT_OK)
            {
                return rc;
            }
        ...
        /* Check value of Credit Counter to ensure that all credits have returned to PGC */
        rc = prvCpssDxChDaigPacketGeneratorCreditCounterBusyWait(devNum, packetGeneratorIndex);
        if (rc != GT_OK)
        {
            return rc;
        }
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorTransmitStart);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetGeneratorIndex, start));

    rc = internal_cpssDxChDiagPacketGeneratorTransmitStart(devNum, packetGeneratorIndex, start);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, packetGeneratorIndex, start));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChDiagPacketGeneratorStatisticGet function
* @endinternal
*
...
*
*/
static GT_STATUS internal_cpssDxChDiagPacketGeneratorStatisticGet
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  packetGeneratorIndex,
...
)
{
    ...
    regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->PCA_PGC[TX_PG_CNS].pgChannelCounter[packetGeneratorIndex];

    /* Ch-%n Packet Counter Status */
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChDiagPacketGeneratorStatisticGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, packetGeneratorIndex, counterPtr));

    rc = internal_cpssDxChDiagPacketGeneratorStatisticGet(devNum, packetGeneratorIndex, counterPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, packetGeneratorIndex, counterPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
