...
*
* @note   APPLICABLE DEVICES:      Lion2; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
...
*       For port having express and preemtiion channels
*       this function configures the express channel only
*       The preemptive channel CT disabled at Mode/Speed configuration.
*
*/
...
)
{
    ...
    GT_U32    globalMac;         /* Global MAC */
    CPSS_PORT_SPEED_ENT           portSpeed; /* speed of the port */
    ...
    PRV_CPSS_DXCH_PORT_NUM_OR_CPU_PORT_CHECK_AND_RXDMA_NUM_GET_MAC(devNum, portNum, globalMac);

    /* convert the 'Physical port' to portGroupId,local port -- supporting multi port groups device */
    portGroupId = PRV_CPSS_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(devNum, globalMac);
    localPort = PRV_CPSS_GLOBAL_PORT_TO_LOCAL_PORT_CONVERT_MAC(devNum, globalMac);
        ...
        && (enable != GT_FALSE))
    {
        ...
        if ((portSpeed == CPSS_PORT_SPEED_11800_E)
            || (portSpeed == CPSS_PORT_SPEED_42000_E))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
        }
    }
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3X; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                   - device number.
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
*                                      (APPLICABLE VALUES: Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS 128 bytes)
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        value = BOOL2BIT_MAC(bypassModePtr->bypassFdbLookup);
        ...
        value = BOOL2BIT_MAC(bypassModePtr->bypassMidwayPcl);
        ...
        value = BOOL2BIT_MAC(bypassModePtr->bypassIpe);
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->EQ.preEgrEngineGlobalConfig.preEgrEngineGlobalConfig2;
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        bypassModePtr->bypassFdbLookup = BIT2BOOL_MAC(value);
        ...
        bypassModePtr->bypassMidwayPcl = BIT2BOOL_MAC(value);
        ...
        bypassModePtr->bypassIpe = BIT2BOOL_MAC(value);
    }
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                   - device number.
    ...
    */
    ...
    if (udeByteCount != 0x3FFF)
    {
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            if (udeByteCount >= 4)
            {
                /* HBU of Ironman does not subtract 8 of PCH, but adds 4 of CRC  */
                udeByteCount -= 4;
            }
        }
        else
        {
            /* HBU subtracts 8 of PCH, but adds 4 of CRC  */
            udeByteCount += 4;
        }
    }
...
/**
* @internal prvCpssDxChCutThroughSip6RxChannelEnableSet function
* @endinternal
*
* @brief   Enable/Disable Cut Through forwarding for packets received on the Rx Channel.
*
* @note   APPLICABLE DEVICES:      Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Bobcat2; Caelum; Aldrin; AC3XLion2; Bobcat3; Aldrin2.
*
* @param[in] devNum                   - device number
* @param[in] portGlobalRxNum          - global Rx DMA port number including CPU port.
* @param[in] enable                   - GT_TRUE:  Enable the port for Cut Through.
...
*
*/
GT_STATUS prvCpssDxChCutThroughSip6RxChannelEnableSet
(
    IN GT_U8    devNum,
    IN GT_U32   portGlobalRxNum,
    IN GT_BOOL  enable
)
{
    ...
    regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_MAC(devNum).
        configs.cutThrough.channelCTConfig[portGlobalRxNum];
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(
        devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E |
        CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E |
        CPSS_FALCON_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_AC5P_E);
    PRV_CPSS_DXCH_PHY_PORT_NUM_CHECK_MAC(devNum, portNum);
