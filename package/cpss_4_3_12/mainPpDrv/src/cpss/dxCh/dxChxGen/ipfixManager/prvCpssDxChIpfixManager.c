...
)
{
    ...
    case CPSS_DXCH_IPFIX_MANAGER_IPC_MSG_OPCODE_ENTRY_INVALIDATE_E:
        ...
        ipcMsgPtr->entryInvalidate.endReason =
            (CPSS_DXCH_IPFIX_MANAGER_FLOW_END_REASON_ENT)prvCpssDxChIpfixManagerBitsGet(ipcDataPtr, 8, 1);
...
)
{
    ...
    GT_U32 maxFlowId = PRV_CPSS_DXCH_IPFIX_MANAGER_MAX_FLOWS_PER_PORT_GROUP_CNS;
    ...
    flowId = 0;
    maxFlowId = ipfixMgrDbPtr->portGroupConfig[portGroupId].maxIpfixIndex;

    for (i = 0; i < maxFlowId/32 + 1; i++)
    {
        ...
        for (j = 0; j < 32; j++)
        {
            if ((i == maxFlowId/32) && (j >= maxFlowId%32))
            {
