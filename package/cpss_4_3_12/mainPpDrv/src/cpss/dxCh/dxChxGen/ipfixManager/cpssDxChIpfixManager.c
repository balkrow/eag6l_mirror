...
#ifndef PRV_CPSS_DXCH_IPFIX_MANAGER_DB_PTR_GET
...
)
{
    ...
    cpssOsMemSet(ipfixMgrDbPtr->globalConfig.txDsaTag, 0, CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS);
    for(i=0; i<CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS; i++)
    {
        ipfixMgrDbPtr->globalConfig.txDsaTag[i/4] |= (dsaTag[i] << (8*(i%4)));
    }
...
)
{
    ...
    if ((globalCfgPtr->ipfixEntriesPerFlow !=
         CPSS_DXCH_IPFIX_MANAGER_ENTRIES_PER_FLOW_IS_TWO_E))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED,
                                      "2 IPFIX entries per flow is only supported");
    }
 ...
 * @retval GT_OK       - on success
 */
static GT_STATUS internal_cpssDxChIpfixManagerDataGet
(
    IN  GT_U8                                  devNum,
    IN  GT_PORT_GROUPS_BMP                     portGroupsBmp,
    IN  GT_U32                                 flowId
 ...
 * @retval GT_OK       - on success
 */
GT_STATUS cpssDxChIpfixManagerDataGet
(
    IN  GT_U8                                  devNum,
    IN  GT_PORT_GROUPS_BMP                     portGroupsBmp,
    IN  GT_U32                                 flowId
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpfixManagerDataGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_IPFIX_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portGroupsBmp, flowId));

    rc = internal_cpssDxChIpfixManagerDataGet(devNum, portGroupsBmp, flowId);
 ...
 * @retval GT_OK       - on success
 */
static GT_STATUS internal_cpssDxChIpfixManagerDataGetAll
(
    IN  GT_U8                                  devNum
)
{
 ...
 * @retval GT_OK       - on success
 */
GT_STATUS cpssDxChIpfixManagerDataGetAll
(
    IN  GT_U8                                  devNum
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChIpfixManagerDataGetAll);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_PER_DEVICE_IPFIX_MANAGER_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum));

    rc = internal_cpssDxChIpfixManagerDataGetAll(devNum);
