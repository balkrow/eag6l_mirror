...
#define PRV_GLOBAL_PPDRV_HWINIT(_var)\
    PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.hwInitDir._var)
...
#define NUM_BITS_FOR_LINK_NUM_MAC(_devNum)       \
   (PRV_CPSS_SIP_7_CHECK_MAC(_devNum)    ? 10 :/* 8 for trunkId , 10 for portNum */ \
...

{
    ...
    if(GT_FALSE == prvCpssSip6TxqPreemptionCapableDeviceGet(devNum))
    {
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
            ...
            !PRV_CPSS_PP_MAC(devNum)->isGmDevice)/*GM uses sip6 config*/
        {
            ...
            regAddr = PRV_DXCH_REG1_UNIT_SIP6_RXDMA_MAC(devNum).
                configs.channelConfig.channelGeneralConfigs[localPort];
        }
        else
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        */
        ...
        if(myPhysicalPortNum == CPSS_CPU_PORT_NUM_CNS/*63*/&&
...
/**
* @internal internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet function
* @endinternal
*
* @brief   Enables/disables the adding a DSA tag in addition to the tag0,tag1 in
*          FORWARD and FROM_CPU packets (instead of the DSA to hold tag0 info).
*          If enabled and the port is cascade and Outgoing DSA command is FORWARD/FROM_CPU
*          then new FORWARD/FROM_CPU DSA tag is added to the packet, ignoring whether the
*            packet's egress tagged state is tagged or not.
*            This is useful in CC type (with remote physical ports) systems where
*            the egress tag state is determined on
*            this device, and the line card device just strips the DSA tag and sends the packet as is.
*            In this case, the DSA<tag0_src_tagged/trg_tagged> (word0 bit 29) is
*            cleared to 0, so the egress device will strip the DSA without replacing it with a VLAN tag.
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - port number
* @param[in] enable                   - GT_TRUE  - enable forced adding DSA FORWARD/FROM_CPU
*                                       GT_FALSE - disable the force.
*
* @retval GT_OK                       - on success
* @retval GT_BAD_PARAM                - wrong value in any of the parameter
* @retval GT_HW_ERROR                 - on hardware error
...
*
*/
static GT_STATUS internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet
(
    IN    GT_U8                       devNum,
    IN    GT_PHYSICAL_PORT_NUM        portNum,
    IN    GT_BOOL                     enable
...
/**
* @internal cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet function
* @endinternal
*
* @brief   Enables/disables the adding a DSA tag in addition to the tag0,tag1 in
*          FORWARD and FROM_CPU packets (instead of the DSA to hold tag0 info).
...
*
*/
GT_STATUS cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet
(
    IN    GT_U8                       devNum,
    IN    GT_PHYSICAL_PORT_NUM        portNum,
    IN    GT_BOOL                     enable
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enable));

    rc = internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableSet(devNum, portNum, enable);
...
/**
* @internal internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet function
* @endinternal
*
* @brief   Get the state of adding a DSA tag in addition to the tag0,tag1 in
*          FORWARD and FROM_CPU packets (instead of the DSA to hold tag0 info).
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2.
...
*                                       GT_TRUE  - enable forced adding DSA FORWARD/FROM_CPU
*                                       GT_FALSE - disable the force.
*
* @retval GT_OK                       - on success
* @retval GT_BAD_PARAM                - wrong value in any of the parameter
...
*
*/
static GT_STATUS internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet
(
    IN    GT_U8                       devNum,
    IN    GT_PHYSICAL_PORT_NUM        portNum,
    OUT   GT_BOOL                    *enablePtr
)
{
    ...
    rc = prvCpssDxChReadTableEntryField(devNum,
                                     ...
                                     portNum,/*global port*/
                                     PRV_CPSS_DXCH_TABLE_WORD_INDICATE_FIELD_NAME_CNS,
                                     SIP5_HA_PHYSICAL_PORT_TABLE_2_FIELDS_FORCE_NEW_DSA_FORWARD_OR_FROM_CPU_E, /* field name */
                                     PRV_CPSS_DXCH_TABLES_WORD_INDICATE_AUTO_CALC_LENGTH_CNS,
                                     &value);
    *enablePtr = BIT2BOOL_MAC(value);
...
/**
* @internal cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet function
* @endinternal
*
* @brief   Get the state of adding a DSA tag in addition to the tag0,tag1 in
*          FORWARD and FROM_CPU packets (instead of the DSA to hold tag0 info).
...
*
*/
GT_STATUS cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet
(
    IN    GT_U8                       devNum,
    IN    GT_PHYSICAL_PORT_NUM        portNum,
    OUT   GT_BOOL                    *enablePtr
)
{
    ...
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, portNum, enablePtr));

    rc = internal_cpssDxChCscdPortForceNewForwardOrFromCpuDsaEnableGet(devNum, portNum, enablePtr);
