...
*/
#define PHY_LMU_MAX_UNITS(_dev)     PRV_CPSS_DXCH_PP_MAC(_dev)->hwInfo.lmuInfo.numLmuUnits
#define USR_LMU_MAX_UNITS(_dev)     (PRV_CPSS_SIP_7_CHECK_MAC(_dev) ? \
                PRV_CPSS_DXCH_PP_MAC(_dev)->hwInfo.multiDataPath.maxDp : PHY_LMU_MAX_UNITS(_dev))
#define PHY_LMU_MAX_PROFILES(_dev)  512
#define USR_LMU_MAX_PROFILES(_dev)  (PRV_CPSS_SIP_7_CHECK_MAC(_dev) ? 4096 : PHY_LMU_MAX_PROFILES(_dev))
...
#define LMU_CONVERT_NEEDED(_dev)     PRV_CPSS_SIP_7_CHECK_MAC(_dev)
...
* @param[in] profile               - latency profile
*                                   (APPLICABLE RANGES: Falcon, AC5P, AC5X, Harrier, Ironman 0..511)
*                                   (APPLICABLE RANGES: AAS 0..4095)
*
...
)
{
    ...
    if(profile >= PCL_MAX_PROFILES(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }

    profileBitSize = PCL_PROFILE_BIT_SIZE(devNum);
    ...
    if((CPSS_DIRECTION_INGRESS_E == portType) || (CPSS_DIRECTION_BOTH_E == portType))
    {
        rc = prvCpssDxChWriteTableEntryField(
            ...
            0, (8 + profileBitSize), (index | (profile << 8))
        );
        if(GT_OK != rc)
        {
            return rc;
...
)
{
    ...
    *profilePtr = U32_GET_FIELD_MAC(value, 8, profileBitSize);
...
*
* @param[in] devNum                - device number
* @param[in] latencyProfile        - latency profile
*                                   (APPLICABLE RANGES: Falcon, AC5P, AC5X, Harrier, Ironman 0..511)
*                                   (APPLICABLE RANGES: AAS 0..4095)
...
)
{
    ...
    if(latencyProfile >= PCL_MAX_PROFILES(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
...
/* check that the BMP is valid for the device - LMU registers/tables (but not EPCL) */
static GT_STATUS lmuBmpProfileCheckAndConvert
(
    IN  GT_U8                                  devNum,
    INOUT  GT_U32                              *latencyProfile,
    INOUT  GT_LATENCY_MONITORING_UNIT_BMP      *lmuBmpPtr,
    OUT  GT_U32                                *numLmuUnitsPtr
)
{
    GT_U32  lmuMaxUnits = PHY_LMU_MAX_UNITS(devNum);
    ...
    GT_U32                          profile, gopLmuBmp, gopLmuBitSize, usrLmuNum;
    ...
    if(*latencyProfile >= USR_LMU_MAX_PROFILES(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "latencyProfile must be less than [%d] but got [%d]",
            USR_LMU_MAX_PROFILES(devNum),
            *latencyProfile);
    }

    if (numLmuUnitsPtr != NULL)
    {
        *numLmuUnitsPtr = lmuMaxUnits;
    }

    if (LMU_CONVERT_NEEDED(devNum))
    {
        /* Check USER given parameters (not physical) */
        if (lmuBmp == CPSS_LMU_UNAWARE_MODE_CNS)
        { /* all LMUs are selected */
            activeLmuBmp = (1 << USR_LMU_MAX_UNITS(devNum)) - 1;
        }
        else
        {
            /* Check USER given parameters (not physical) */
            ...
            if (lmuBmp > activeLmuBmp)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM,
                    "Device[%u] supports %d LMUs but got BMP of[0x%08x]",
                    devNum, USR_LMU_MAX_UNITS(devNum), activeLmuBmp);
            }
        }

        /* Convert USER given into physical */
        lmuBmp = 0;
        gopLmuBmp = 1 << (*latencyProfile / PHY_LMU_MAX_PROFILES(devNum)); /* bmp[0..7] */
        gopLmuBitSize = 8;
        profile = *latencyProfile % PHY_LMU_MAX_PROFILES(devNum);          /* 0..511 */

        for (usrLmuNum = 0; usrLmuNum < USR_LMU_MAX_UNITS(devNum); usrLmuNum++)
        {
            if (((1 << usrLmuNum) & activeLmuBmp) != 0)
            {
                lmuBmp |= gopLmuBmp << (gopLmuBitSize * usrLmuNum);
            }
        }
...
)
{
    ...
    rc = lmuBmpProfileCheckAndConvert(devNum, &latencyProfile, &lmuBmp, &lmuMaxUnits);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    for(ii = 0; ii < lmuMaxUnits; ii++)
    {
        ...
        {
            regAddr = PRV_DXCH_REG1_UNIT_GLOBAL_LMU_MAC(devNum, db_globalLmuIndex).statClear;
            rc = prvCpssHwPpSetRegField(devNum, regAddr, 0, 12, latencyProfile);
        }
        else
        {
...
)
{
    ...
    rc = lmuBmpProfileCheckAndConvert(devNum, &latencyProfile, &lmuBmp, NULL);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    if (PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfTiles && LMU_GAP_CHECK_NEEDED(devNum))
    {
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; AC5P; AC5X; Harrier; Ironman; AAS.
*
* @param[in] devNum                - device number
* @param[in] portNum               - port number
...
)
{
    ...
    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
        CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | CPSS_AC3X_E |CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
    LMU_UNIT_EXISTS_CHECK(devNum);
...
GT_STATUS prvCpssDxChLatencyMonitoringRegDbPrint(IN GT_U8 devNum)
{
    ...
    maxLmu = PHY_LMU_MAX_UNITS(devNum);
...
/**
* @internal internal_cpssDxChLatencyMonitoringProfileModeGet function
* @endinternal
*
...
)
{
    ...
    GT_STATUS   ii;                 /* Queue offset in group */
    GT_U32      entryData[4] = { 0 };

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E |
                                          CPSS_ALDRIN_E | CPSS_AC3X_E |CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E);
    PRV_CPSS_DXCH_PHY_PORT_NUM_CHECK_MAC(devNum, portNum);
    if(portProfile >= PCL_MAX_PROFILES(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }
    ...
    /* SIP7 has 16K lines with (8_profile * 12b + GAP_32b)=16Bytes,
     * so totally supports (16K * 8)=128K profiles = 17bits
     * Access for Queue: groupIndex=line=[16..3] + inGroupOffset=LMU=[2..0]
     * Config same profile for all 8 queues in 8 fields of an entry.
     */
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        for (ii = 0; ii < 8; ii++)
        {
            U32_SET_FIELD_IN_ENTRY_MAC(entryData, ii * 12, 12, portProfile);
        }
        rc = prvCpssDxChWriteTableEntry(devNum,
                CPSS_DXCH_SIP6_10_TABLE_EGRESS_PCL_QUEUE_GROUP_LATENCY_PROFILE_CONFIG_E,
                groupOfQueuesIndex >> 3,
                entryData);
    }
    else
    {
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        GT_U32 portQueue = 0; /* get profile for port's queue0 is enought */

        rc = prvCpssDxChReadTableEntryField(devNum,
                CPSS_DXCH_SIP6_10_TABLE_EGRESS_PCL_QUEUE_GROUP_LATENCY_PROFILE_CONFIG_E,
                groupOfQueuesIndex >> 3,
                PRV_CPSS_DXCH_TABLE_WORD_INDICATE_GLOBAL_BIT_CNS,
                portQueue * 12, PCL_PROFILE_BIT_SIZE(devNum), portProfilePtr);
    }
    else
    {
        rc = prvCpssDxChReadTableEntryField(devNum,
...
)
{
    ...
    if(queueProfile >= PCL_MAX_PROFILES(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }
    /* Priority Queue Latency Monitoring Profile Tables: entryIndex per even/odd queue number */
    ...
    return prvCpssHwPpSetRegField(devNum, regAddr,
                                 (tcQueue % 2) * profileBitSize/*offset*/,
                                 profileBitSize/*Length*/,
                                 queueProfile);
}
...
)
{
    ...
    return prvCpssHwPpGetRegField(devNum, regAddr,
                                 (tcQueue % 2) * profileBitSize/*offset*/,
                                 profileBitSize/*length*/,
                                 queueProfilePtr);
}
