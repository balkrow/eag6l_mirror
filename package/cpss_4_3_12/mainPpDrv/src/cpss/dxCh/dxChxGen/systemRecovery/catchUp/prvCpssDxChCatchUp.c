...
)
{
    ...
    for (ii = 0; ii < PRV_CPSS_MAX_PP_DEVICES_CNS; ii++)
    {
        ...
        if (hwInfo != NULL)
        {
            ...
            {
                /*same restriction as in drvInterruptsInit*/
                if (ii >= MAX_INTERRUPT)
                {
                    continue;
                }
                /* enable interrupts in accordance with interrupt mask */
                PRV_SHARED_COMMON_INT_DIR_DXEXMX_GLOBAL_VAR_SET(intArr[ii].enabled, GT_TRUE);
            }
            else
            {
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        rc = cpssDxChBrgFdb16BitFidHashEnableGet(devNum,&fid16BitHashEn);
        if (rc != GT_OK)
        {
            return rc;
        }

        rc = cpssDxChBrgFdbCrcHashUpperBitsModeGet(devNum,&crcHashUpperBitsMode);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        /* Update FDB hash parameters */
        PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.fdbHashParams.fid16BitHashEn = fid16BitHashEn;
        PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.fdbHashParams.crcHashUpperBitsMode = crcHashUpperBitsMode;
    }
    return rc;
}
...
)
{
    ...
    for (port = 0; port < maxNumberOfPorts; port++)
    {
        ...
        if( (CPSS_PORT_SPEED_2500_E == speed) &&
            ((CPSS_PORT_INTERFACE_MODE_1000BASE_X_E == ifMode) || (CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E == ifMode)) )
        {
        ...
        if (PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
                ...
                ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC3X_E)))
            {
                ...
                else if ( (ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E) || (ifMode == CPSS_PORT_INTERFACE_MODE_1000BASE_X_E) || (ifMode == CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E) )
                {
...
)
{
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(devNum,portGroupId)
    {
        ...
        if (moduleCfgPtr->fuqUseSeparate != CPSS_PP_PHASE2_UPLOAD_QUEUE_SHARED_WITH_AUQ_E)
        {
...
)
{
    ...
    moduleCfgPtr = &(PRV_CPSS_DXCH_PP_MAC(devNum)->moduleCfg);
    if ((moduleCfgPtr->fuqUseSeparate == CPSS_PP_PHASE2_UPLOAD_QUEUE_SHARED_WITH_AUQ_E)
        || (moduleCfgPtr->fuCfg.fuDescBlock == NULL))
    {
        return GT_OK;
    }
...
)
{
    ...
    if (tempSystemRecovery_Info.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E)
    {
        ...
        for (devNum = 0; devNum < PRV_CPSS_MAX_PP_DEVICES_CNS; devNum++)
        {
            ...
            rc = prvCpssDxChHitlessStartupEventClean(devNum);
            if (rc !=GT_OK)
            {
                return rc;
            }
...
/**
* @internal prvCpssDxChSystemRecoveryCompletionHandleSip7 function
* @endinternal
*
* @brief   Perform synchronization of hardware data and software DB after special init sequence.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssDxChSystemRecoveryCompletionHandleSip7
(
   GT_VOID
)
{
    ...
    {
        rc = prvCpssDxChLpmRamSyncSwHwForHaSip7();
        if(rc != GT_OK)
        {
            return rc;
        }
     ...
     for (devNum = 0; devNum < PRV_CPSS_MAX_PP_DEVICES_CNS; devNum++)
     {
               ...
               ( tempSystemRecovery_Info.systemRecoveryMode.haReadWriteState != CPSS_SYSTEM_RECOVERY_HA_STATE_READ_ENABLE_WRITE_ENABLE_E))
         {
             if (mvHwsServiceCpuEnableGet(devNum))
             {
                 ...
                 rc = mvHwsServiceCpuFwInit(devNum, 0, 0, NULL);
                 if (rc != GT_OK)
                 {
                     CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
                     return rc;
