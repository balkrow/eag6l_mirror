    ...
    case PRV_CPSS_DXCH_HSU_STAGE_ARRAY_DATA_TYPE_ALL_E:
        {
            ...
            stageArrayPtr[PRV_CPSS_DXCH_HSU_GEN_TRUNK_ALL_MEMBERS_ARRAY_STAGE_E].stageSize =
                sizeof(CPSS_TRUNK_MEMBER_STC)*genTrunkDbPtr->maxNumMembersInDevice;
            ...
            stageArrayPtr[PRV_CPSS_DXCH_HSU_GEN_TRUNK_ALL_MEMBERS_IS_LOCAL_ARRAY_STAGE_E].stageSize =
                 sizeof(GT_BOOL)*genTrunkDbPtr->maxNumMembersInDevice;
            ...
            {
                maxSerdesLanesNum = prvCpssDxChHwInitNumOfSerdesGet(devNum);
                ...
                if(PRV_CPSS_DXCH_PP_MAC(devNum)->serdesCfgDbArrPtr!=NULL)
                {
                          ...
                          stageIndex <= PRV_CPSS_DXCH_HSU_SERDES_LANE35_STAGE_E; stageIndex++)
                    {
                        ...
                        {
                            stageArrayPtr[stageIndex].stageSize = sizeof(CPSS_PORT_SERDES_TUNE_STC) * CPSS_DXCH_PORT_SERDES_SPEED_NA_E;
        ...
        case PRV_CPSS_DXCH_HSU_STAGE_ARRAY_DATA_TYPE_RXTX_E:
        {
                  ...
                  stageIndex <= PRV_CPSS_DXCH_HSU_GEN_RX_QUEUE7_STAGE_E; stageIndex++ )
            {
                ...
                if (tempSystemRecovery_Info.systemRecoveryMode.continuousRx == GT_TRUE)
                {
                  ...
                  stageIndex <= PRV_CPSS_DXCH_HSU_GEN_TX_QUEUE7_STAGE_E; stageIndex++ )
            {
                ...
                if (tempSystemRecovery_Info.systemRecoveryMode.continuousTx == GT_TRUE)
                {
                    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
                    {
    ...
    *sizePtr = 0;
    ...
    for (devNum = 0; devNum < PRV_CPSS_MAX_PP_DEVICES_CNS; devNum++)
    {
        ...
        *sizePtr +=( sizeof(PRV_CPSS_GEN_PP_CONFIG_STC)+
        ...
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
        {
            maxSerdesLanesNum = prvCpssDxChHwInitNumOfSerdesGet((GT_U8)devNum);
            ...
            *sizePtr += sizeof(GT_32)*2;
            ...
            if(PRV_CPSS_DXCH_PP_MAC(devNum)->serdesCfgDbArrPtr!=NULL)
            {
                ...
                *sizePtr += (sizeof(CPSS_PORT_SERDES_TUNE_STC) * CPSS_DXCH_PORT_SERDES_SPEED_NA_E * numOfAllocatedSerdes);
            }
...
/**
* @internal internal_cpssDxChHsuBlockSizeGet function
* @endinternal
*
...
/**
* @internal internal_cpssDxChHsuExport function
* @endinternal
*
...
/**
* @internal internal_cpssDxChHsuImport function
* @endinternal
*
                   ...
                   (currentIterPtr->devNum == PRV_CPSS_MAX_PP_DEVICES_CNS /* new device */)) )
            {
                if (currentIterPtr->genIterator.currStageMemPtr == NULL)
                {
                    ...
                    if(currentStage == PRV_CPSS_DXCH_HSU_GEN_PP_CONFIG_STAGE_E)
                    {
                        ...
                        if ((tempSystemRecovery_Info.systemRecoveryMode.continuousRx == GT_FALSE)         ||
                             (tempSystemRecovery_Info.systemRecoveryMode.continuousAuMessages == GT_FALSE)||
                             (tempSystemRecovery_Info.systemRecoveryMode.continuousTx == GT_FALSE)        ||
                             (tempSystemRecovery_Info.systemRecoveryMode.continuousFuMessages == GT_FALSE))
                        {
                ...
                if (enoughMemoryForStage == GT_TRUE )
                {
                    ...
                    if(currentStage == PRV_CPSS_DXCH_HSU_GEN_PP_CONFIG_STAGE_E)
                    {
                        ...
                        if (tempSystemRecovery_Info.systemRecoveryMode.continuousAuMessages == GT_FALSE)
                        {
                        ...
                        if (tempSystemRecovery_Info.systemRecoveryMode.continuousFuMessages == GT_FALSE)
                        {
                        ...
                        if (tempSystemRecovery_Info.systemRecoveryMode.continuousTx == GT_FALSE)
                        {
                        ...
                        if ((tempSystemRecovery_Info.systemRecoveryMode.continuousRx == GT_TRUE) ||
                            (tempSystemRecovery_Info.systemRecoveryMode.continuousTx == GT_TRUE))
                        {
                        ...
                        if(genTrunkDbPtr->maxNumMembersInTrunk_isPerTrunk == GT_FALSE)
                        {
                            for(i = 0 ; i < genTrunkDbPtr->numberOfTrunks ; i++)
                            {
                                genTrunkDbPtr->trunksArray[i + 1].membersPtr =
                                        &(genTrunkDbPtr->allMembersArray[i * genTrunkDbPtr->maxNumMembersInTrunk]);
                                genTrunkDbPtr->trunksArray[i + 1].isLocalMembersPtr =
                                    &(genTrunkDbPtr->allMembersIsLocalArray[i * genTrunkDbPtr->maxNumMembersInTrunk]);
                            }
                        }
                        else
                        {
                            PRV_CPSS_TRUNK_ENTRY_INFO_STC *trunkEntryPtr;/*The trunk DB for current trunkId*/
                            GT_U32  index;
                            trunkEntryPtr = &genTrunkDbPtr->trunksArray[0];
                            for(i = 0 ; i < genTrunkDbPtr->numberOfTrunks ; i++,trunkEntryPtr++)
                            {
                                index = trunkEntryPtr->flexInfo.myTrunkStartIndexInMembersTable;

                                genTrunkDbPtr->trunksArray[i + 1].membersPtr =
                                        &(genTrunkDbPtr->allMembersArray[index]);
                                genTrunkDbPtr->trunksArray[i + 1].isLocalMembersPtr =
                                    &(genTrunkDbPtr->allMembersIsLocalArray[index]);
                            }
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
    {
        if((serdesLanesBmp[0] != 0)||(serdesLanesBmp[1] != 0))
        {
            ...
            PRV_CPSS_DXCH_PP_MAC(devNum)->serdesCfgDbArrPtr = (CPSS_PORT_SERDES_TUNE_STC_PTR*)
                cpssOsMalloc(sizeof(CPSS_PORT_SERDES_TUNE_STC_PTR)*maxSerdesLanesNum);
            ...
            cpssOsMemSet(PRV_CPSS_DXCH_PP_MAC(devNum)->serdesCfgDbArrPtr, 0,
                         sizeof(CPSS_PORT_SERDES_TUNE_STC_PTR)*maxSerdesLanesNum);
...
)
{
    ...
    GT_U32                           temp_portGroupsBmp;/* port group bitmap for FDB unit */
    CPSS_SYSTEM_RECOVERY_INFO_STC    tempSystemRecovery_Info;
                        ...
                        for (i = 0; i < uploadNumber; i++)
                        {
                            ...
                            while (actFinished == GT_FALSE)
                            {
                                ...
                                if (trigCounter > 500)
                                {
                                    ...
                                    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,temp_portGroupsBmp,portGroupIndex)
                                    {
                                    ...
                                    PRV_CPSS_GEN_PP_END_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,temp_portGroupsBmp,portGroupIndex)
                                }
                            }
                        }
