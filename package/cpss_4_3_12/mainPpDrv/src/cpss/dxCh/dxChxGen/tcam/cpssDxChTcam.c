...
* @param[in] index                    - global line  in TCAM.
*
...
* @param[in] index                    - global line  in TCAM to read.
...
)
{
    ...
    GT_U32    tcamRepresentativePortGroupsBmp = CPSS_PORT_GROUP_UNAWARE_MODE_CNS;

    /* get the port groups that hold TCAM */
    /* NOTE: in Falcon it is only on event port groups (0,2,4,6) without odd ones(1,3,5,7)*/
    /* NOTE: in Seahawk with 2 tiles it is for all port groups (0,1) */
    /* therefore the check of representativePortGroupId , should be according to :
       tcamRepresentativePortGroupsBmp and not '&1'*/
    /* the MACRO : PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_UNIT_MAC
        updates the tcamRepresentativePortGroupsBmp for multi-pipe device according to
        actual pipes that hold the TCAM unit */
    PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_UNIT_MAC(devNum,tcamRepresentativePortGroupsBmp,PRV_CPSS_DXCH_UNIT_TCAM_E);

    if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        numOfClientGroups = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId;
    }
    else
    {
    ...
    if(0 == ((1 << representativePortGroupId) & tcamRepresentativePortGroupsBmp))
    {
        /* only representatives port groups supported */
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "the tcam not support port group[%d] (non-representative)\n",
            representativePortGroupId);
    }

    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        numClientIds = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.pcl.iPcl0Bypass ? 6 : 7;
    }
    else
    {
    ...
    if (!PRV_CPSS_IS_MULTI_PORT_GROUPS_DEVICE_MAC(devNum) ||
        PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfPipesPerTile < 2)
    {
        /* Bobcat2, Aldrin, Caelum, Aldrin2, AC5P ,AAS (also with 2 tiles) */
        numOfPipeOnTcam = 1;
        valueMask = BIT_0;
    }
    else
...
*
* @note   APPLICABLE DEVICES:      Bobcat2; Caelum; Aldrin; AC3X; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
...
)
{
    ...
    if (CPSS_PORT_GROUP_UNAWARE_MODE_CNS != portGroupsBmp)
    {
        ...
        client_portGroupsBmp = PRV_CPSS_PP_MAC(devNum)->portGroupsInfo.activePortGroupsBmp;
    }
...
*                                       APPLICABLE DEVICES: Bobcat2, Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS.
        ...
        devNum, portGroupsBmp, portGroupId)
    {
        ...
        else
        {
            for(ii = 0 ; ii < numOfBlocks; ii++)
            {
                ...
                /* this condition bypasses writing fields absenting on Ironman */
                if ((PRV_CPSS_SIP_6_CHECK_MAC(devNum) == 0) ||
                    (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId > 1))
                {
                    rc = prvCpssHwPpPortGroupSetRegField(
...
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AAS.
*
* @param[in] devNum                   - the device number
...
)
{
    ...
    GT_U32    hwKeySize;                   /* size of key in HW Enum format */
    GT_U32    hwValue;                     /* HW value */
    GT_U32    numOfTcamProfiles;           /* number of profiles supported by device */
    GT_U32    numOfTcamMuxTableLines;      /* number of MUX Table lines  supported by device */
    GT_U32    numOfSubKeys1_3;             /* number of keys of 1-3-keys supported by device */

    PRV_CPSS_DXCH_DEV_CHECK_MAC(devNum);
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(
        devNum, CPSS_XCAT3_E | CPSS_AC5_E | CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E
        | CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_BOBCAT3_E | CPSS_ALDRIN2_E | CPSS_FALCON_E | CPSS_AAS_E);
    PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_UNIT_MAC(devNum, portGroupsBmp, PRV_CPSS_DXCH_UNIT_TCAM_E);

    numOfTcamProfiles      = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamProfiles ?
                             PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamProfiles :
                             64;
    numOfTcamMuxTableLines = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamMuxTableLines ?
                             PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamMuxTableLines :
                             48;
    numOfSubKeys1_3        = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfSubKeys1_3 ?
                             PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfSubKeys1_3 :
                             3;

    if ((tcamProfileId == 0) || (tcamProfileId >= numOfTcamProfiles))
    {
    ...
    for (i = 0; (i < numOfSubKeys1_3); i++)
    {
        if (tcamLookupProfileCfgPtr->subKey1_3_MuxTableLineIndexes[i] >= numOfTcamMuxTableLines)
        {
        ...
        devNum, portGroupsBmp, portGroupId)
    {
        ...
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.subKey0Mapping[0][tcamProfileId - 1];
        ...
        for (i = 0; (i < numOfSubKeys1_3); i++)
        {
            ...
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.subKey1_3Mapping[0][tcamProfileId - 1][i];
...
/**
* @internal prvCpssDxChTcamLookupMuxTableLineSet function
* @endinternal
*
...
*                                      The parameter reserved for future big devices.
*                                      For all devices having one TCAM unit the parameter ignored.
* @param[in] tcamClientGroup         - TCAM client group index
*                                      (APPLICABLE RANGES: AAS 0..5).
...
* @retval GT_OUT_OF_RANGE          - one of parameters value out-of-range
*/
static GT_STATUS prvCpssDxChTcamLookupMuxTableLineSet
(
    IN   GT_U8                                         devNum,
    IN   GT_PORT_GROUPS_BMP                            portGroupsBmp,
    IN   GT_U32                                        tcamClientGroup,
...
)
{
    ...
    GT_U32    hwValue;                     /* HW value */
    GT_U32    numOfTcamMuxTableLines;      /* number of MUX Table lines  supported by device */
    GT_U32    maxNumOfKeyBytePairs;        /* max number of Key byte pairs  */

    PRV_CPSS_MULTI_PORT_GROUPS_BMP_CHECK_SPECIFIC_UNIT_MAC(devNum, portGroupsBmp, PRV_CPSS_DXCH_UNIT_TCAM_E);

    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        if (tcamClientGroup >= PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "tcamClientGroup out of range\n");
        }
        maxNumOfKeyBytePairs = 60;
    }
    else
    {
        /* fix value of not applicable parameter */
        tcamClientGroup = 0;
        maxNumOfKeyBytePairs = 40;
    }

    numOfTcamMuxTableLines = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamMuxTableLines;
    if (numOfTcamMuxTableLines == 0)
    {
        numOfTcamMuxTableLines = 48;
    }

    if ((entryIndex == 0) || (entryIndex >= numOfTcamMuxTableLines))
    {
    ...
    for (i = 0; (i < 30); i++)
    {
        if (tcamLookupProfileCfgPtr->superKeyTwoByteUnitsOffsets[i] >= maxNumOfKeyBytePairs)
        {
        ...
        devNum, portGroupsBmp, portGroupId)
    {
        for (n = 0; (n < 6); n++)
        {
            ...
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.keyMuxCfgTable[tcamClientGroup][entryIndex - 1][n];
...
)
{
    ...
    return prvCpssDxChTcamLookupMuxTableLineSet(
        devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
        0 /*tcamClientGroup*/, entryIndex, tcamLookupMuxTableLineCfgPtr);
}
...
)
{
    ...
    rc = internal_cpssDxChTcamLookupMuxTableLineSet(
        devNum, entryIndex, tcamLookupMuxTableLineCfgPtr);
...
/**
* @internal internal_cpssDxChTcamClientGroupLookupMuxTableLineSet function
* @endinternal
*
* @brief   Sets configuration of TCAM sub-key Mux Table Entry.
*
...
* @retval GT_OUT_OF_RANGE          - one of parameters value out-of-range
*/
static GT_STATUS internal_cpssDxChTcamClientGroupLookupMuxTableLineSet
(
    IN   GT_U8                                         devNum,
    IN   GT_U32                                        tcamClientGroup,
    IN   GT_U32                                        entryIndex,
...
)
{
    ...
    return prvCpssDxChTcamLookupMuxTableLineSet(
        devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
        tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr);
}

/**
* @internal cpssDxChTcamClientGroupLookupMuxTableLineSet function
* @endinternal
*
* @brief   Sets configuration of TCAM sub-key Mux Table Entry.
*
...
* @retval GT_OUT_OF_RANGE          - one of parameters value out-of-range
*/
GT_STATUS cpssDxChTcamClientGroupLookupMuxTableLineSet
(
    IN   GT_U8                                         devNum,
    IN   GT_U32                                        tcamClientGroup,
    IN   GT_U32                                        entryIndex,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTcamClientGroupLookupMuxTableLineSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr));

    rc = internal_cpssDxChTcamClientGroupLookupMuxTableLineSet(
        devNum, tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC(
        (funcId, rc, devNum, tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal prvCpssDxChTcamLookupMuxTableLineGet function
* @endinternal
*
...
* @retval GT_BAD_PARAM             - one of parameters value invalid
*/
static GT_STATUS prvCpssDxChTcamLookupMuxTableLineGet
(
    IN   GT_U8                                         devNum,
    IN   GT_PORT_GROUPS_BMP                            portGroupsBmp,
    IN   GT_U32                                        tcamClientGroup,
...
)
{
    ...
    GT_U32    hwValue;                     /* HW value */
    GT_U32    numOfTcamMuxTableLines;      /* number of MUX Table lines  supported by device */
    GT_U32    maxOffsetInSperkey;          /* maxOffsetInSperkey */
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        maxOffsetInSperkey = 60; /* superkey size 120 bytes */
    }
    else
    {
        /* the parameters not relevant - value fixed */
        tcamClientGroup = 0;
        maxOffsetInSperkey = 40; /* superkey size 80 bytes */
    }
    ...
    for (i = 0; (i < 30); i++)
    {
        /* SIP6_10 max - 80 bytes - 40 2-bytes units */
        /* SIP7        - 120 bytes - 60 2-bytes units */
        if (tcamLookupMuxTableLineCfgPtr->superKeyTwoByteUnitsOffsets[i] >= maxOffsetInSperkey)
        {
...
)
{
    ...
    return prvCpssDxChTcamLookupMuxTableLineGet(
        devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
        0/*tcamClientGroup*/, entryIndex, tcamLookupMuxTableLineCfgPtr);
}
...
/**
* @internal internal_cpssDxChTcamClientGroupLookupMuxTableLineGet function
* @endinternal
*
* @brief   Gets configuration of TCAM sub-key Mux Table Entry.
*
...
* @retval GT_BAD_PARAM             - one of parameters value invalid
*/
static GT_STATUS internal_cpssDxChTcamClientGroupLookupMuxTableLineGet
(
    IN   GT_U8                                         devNum,
    IN   GT_U32                                        tcamClientGroup,
    IN   GT_U32                                        entryIndex,
...
/**
* @internal cpssDxChTcamClientGroupLookupMuxTableLineGet function
* @endinternal
*
* @brief   Gets configuration of TCAM sub-key Mux Table Entry.
*
...
* @retval GT_BAD_PARAM             - one of parameters value invalid
*/
GT_STATUS cpssDxChTcamClientGroupLookupMuxTableLineGet
(
    IN   GT_U8                                         devNum,
    IN   GT_U32                                        tcamClientGroup,
    IN   GT_U32                                        entryIndex,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTcamClientGroupLookupMuxTableLineGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr));

    rc = internal_cpssDxChTcamClientGroupLookupMuxTableLineGet(
        devNum, tcamClientGroup, entryIndex, tcamLookupMuxTableLineCfgPtr);
...
/**
* @internal internal_cpssDxChTcamClientGroupLookupProfileCfgSet function
* @endinternal
*
* @brief   Sets Parallel lookups per Profile configuration.
*
...
*                                       For all devices having one TCAM unit the parameter ignored.
* @param[in] tcamClientGroup          - TCAM client group index
*                                       The last client group supports one subkey only,
*                                       no shared configuration for key bytes 60-79
*                                       (APPLICABLE RANGES: AAS 0..5).
*                                       (APPLICABLE DEVICES: AAS).
* @param[in] tcamProfileId            - TCAM Profile Id (APPLICABLE RANGES: 1..63).
*                                       TCAM Profile 0 has no configuration and implemented by passing super-key to all lookups.
...
* @retval GT_OUT_OF_RANGE          - one of parameters value out-of-range
*/
static GT_STATUS internal_cpssDxChTcamClientGroupLookupProfileCfgSet
(
    IN   GT_U8                                               devNum,
    IN   GT_PORT_GROUPS_BMP                                  portGroupsBmp,
    IN   GT_U32                                              tcamClientGroup,
    IN   GT_U32                                              tcamProfileId,
    IN   CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC  *tcamLookupProfileCfgPtr
)
{
    GT_STATUS rc;                          /* return code */
    GT_U32    regAddr;                     /* register address */
    ...
    GT_U32    hwKeySize;                   /* size of key in HW Enum format */
    GT_U32    hwValue;                     /* HW value */
    GT_U32    hwValueBits;                 /* HW value bit size */
    GT_U32    numOfTcamProfiles;           /* number of profiles supported by device */
    GT_U32    numOfTcamMuxTableLines;      /* number of MUX Table lines  supported by device */
    GT_U32    numOfSubKeys;                /* number of sub-keys supported by device */
    GT_U32    lastClientGroup;             /* Last Client Group */
    ...
    lastClientGroup = 0xFFFFFFFF; /* not-exist mark */
    if (PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        /* the last Client Group supports one subkey only - no shared config for key bytes 60-79  */
        if (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId > 1)
        {
            lastClientGroup = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId - 1;
        }
        if (tcamClientGroup >= PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "tcamClientGroup out of range\n");
    ...
    {
        /* fix not applicable parameter value */
        tcamClientGroup = 0;
    }
    ...
    numOfTcamMuxTableLines = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamMuxTableLines ?
                             PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfTcamMuxTableLines :
                             48;
    numOfSubKeys           = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfSubKeys1_3 ?
                             (PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.numOfSubKeys1_3 + 1) :
                             4;
    if (tcamClientGroup == lastClientGroup)
    {
        numOfSubKeys = 1;
    }
    ...
    for (i = 0; (i < numOfSubKeys); i++)
    {
        ...
        PRV_CPSS_DXCH_TCAM_CONVERT_RULE_SIZE_TO_KEY_SIZE_VAL_MAC(
            hwKeySize, tcamLookupProfileCfgPtr->subKeySizes[i]);

        if ((PRV_CPSS_SIP_7_CHECK_MAC(devNum)) || (i > 0))
        {
            /* 0-th subkey muxing applicable to SIP7 only */
            if (tcamLookupProfileCfgPtr->subKeyMuxTableLineIndexes[i] >= numOfTcamMuxTableLines)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "subKeyMuxTableLineOffsets out of range\n");
            }
            if (tcamLookupProfileCfgPtr->subKeyMuxTableLineOffsets[i] > 5)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "subKey1MuxTableLineOffsets out of range\n");
            }
        }
    }
    if ((PRV_CPSS_SIP_7_CHECK_MAC(devNum)) && (tcamClientGroup != lastClientGroup))
    {
        if (tcamLookupProfileCfgPtr->sharedMuxTableLineIndex >= numOfTcamMuxTableLines)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "sharedMuxTableLineIndex out of range\n");
        }
        if (tcamLookupProfileCfgPtr->sharedMuxTableLineOffset > 5)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "sharedMuxTableLineOffset out of range\n");
        }
    }
        ...
        devNum, portGroupsBmp, portGroupId)
    {
        for (i = 0; (i < numOfSubKeys); i++)
        {
            PRV_CPSS_DXCH_TCAM_CONVERT_RULE_SIZE_TO_KEY_SIZE_VAL_MAC(hwKeySize, tcamLookupProfileCfgPtr->subKeySizes[i]);
            hwValue     = hwKeySize;
            hwValueBits = 3;
            if ((PRV_CPSS_SIP_7_CHECK_MAC(devNum)) || (i > 0))
            {
                /* 0-th subkey muxing applicable to SIP7 only */
                hwValue = (hwKeySize
                    | (tcamLookupProfileCfgPtr->subKeyMuxTableLineIndexes[i] << 3)
                    | (tcamLookupProfileCfgPtr->subKeyMuxTableLineOffsets[i] << 9));
                hwValueBits = 12;
            }
            regAddr = (i == 0)
                ? PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.subKey0Mapping[tcamClientGroup][tcamProfileId - 1]
                : PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.subKey1_3Mapping[tcamClientGroup][tcamProfileId - 1][i - 1];
            rc = prvCpssHwPpPortGroupSetRegField(devNum, portGroupId, regAddr, 0, hwValueBits, hwValue);
            if(rc != GT_OK)
            {
                return rc;
            }
        ...
        if ((PRV_CPSS_SIP_7_CHECK_MAC(devNum)) && (tcamClientGroup != lastClientGroup))
        {
            hwValue = (tcamLookupProfileCfgPtr->sharedMuxTableLineIndex
                | (tcamLookupProfileCfgPtr->sharedMuxTableLineOffset << 6));
            regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.sharedMapping[tcamClientGroup][tcamProfileId - 1];
            rc = prvCpssHwPpPortGroupSetRegField(devNum, portGroupId, regAddr, 0, 9, hwValue);
            if(rc != GT_OK)
            {
                return rc;
            }
...
/**
* @internal cpssDxChTcamClientGroupLookupProfileCfgSet function
* @endinternal
*
* @brief   Sets Parallel lookups per Profile configuration.
*
...
* @retval GT_OUT_OF_RANGE          - one of parameters value out-of-range
*/
GT_STATUS cpssDxChTcamClientGroupLookupProfileCfgSet
(
    IN   GT_U8                                               devNum,
    IN   GT_U32                                              tcamClientGroup,
    IN   GT_U32                                              tcamProfileId,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTcamClientGroupLookupProfileCfgSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, tcamClientGroup, tcamProfileId, tcamLookupProfileCfgPtr));

    rc = internal_cpssDxChTcamClientGroupLookupProfileCfgSet(
        devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
        tcamClientGroup, tcamProfileId, tcamLookupProfileCfgPtr);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC(
        (funcId, rc, devNum, tcamClientGroup, tcamProfileId, tcamLookupProfileCfgPtr));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
...
/**
* @internal internal_cpssDxChTcamClientGroupLookupProfileCfgGet function
* @endinternal
*
* @brief   Gets Parallel lookups per Profile configuration.
*
...
* @retval GT_BAD_STATE             - one HW fields out of range
*/
static GT_STATUS internal_cpssDxChTcamClientGroupLookupProfileCfgGet
(
    IN   GT_U8                                               devNum,
    IN   GT_PORT_GROUPS_BMP                                  portGroupsBmp,
    IN   GT_U32                                              tcamClientGroup,
    IN   GT_U32                                              tcamProfileId,
    OUT  CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC  *tcamLookupProfileCfgPtr
)
{
    GT_STATUS rc;                          /* return code */
    GT_U32    regAddr;                     /* register address */
    ...
    for (i = 0; (i < numOfSubKeys); i++)
    {
        ...
        hwKeySize = (hwValue & 0x7);
        PRV_CPSS_DXCH_TCAM_CONVERT_KEY_SIZE_TO_RULE_SIZE_VAL_MAC_EX(
            tcamLookupProfileCfgPtr->subKeySizes[i], hwKeySize, GT_BAD_STATE);
        if ((PRV_CPSS_SIP_7_CHECK_MAC(devNum)) || (i > 0))
        {
            tcamLookupProfileCfgPtr->subKeyMuxTableLineIndexes[i] = ((hwValue >> 3) & 0x3F);
            if (tcamLookupProfileCfgPtr->subKeyMuxTableLineIndexes[i] >= numOfTcamMuxTableLines)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "subKeyMuxTableLineOffsets out of range\n");
            }
            tcamLookupProfileCfgPtr->subKeyMuxTableLineOffsets[i] = ((hwValue >> 9) & 0x7);
            if (tcamLookupProfileCfgPtr->subKeyMuxTableLineOffsets[i] > 5)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "subKeyMuxTableLineOffsets out of range\n");
            }
    ...
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->TCAM.sharedMapping[tcamClientGroup][tcamProfileId - 1];
        rc = prvCpssHwPpPortGroupGetRegField(devNum, portGroupId, regAddr, 0, 9, &hwValue);
        if(rc != GT_OK)
        {
            return rc;
        }
        tcamLookupProfileCfgPtr->sharedMuxTableLineIndex = (hwValue & 0x3F);
        if (tcamLookupProfileCfgPtr->sharedMuxTableLineIndex >= numOfTcamMuxTableLines)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "sharedMuxTableLineIndex out of range\n");
        }
        tcamLookupProfileCfgPtr->sharedMuxTableLineOffset = ((hwValue >> 6) & 0x7);
        if (tcamLookupProfileCfgPtr->sharedMuxTableLineOffset > 5)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "sharedMuxTableLineOffset out of range\n");
        }
    }
...
/**
* @internal cpssDxChTcamClientGroupLookupProfileCfgGet function
* @endinternal
*
* @brief   Gets Parallel lookups per Profile configuration.
*
...
* @retval GT_BAD_STATE             - one HW fields out of range
*/
GT_STATUS cpssDxChTcamClientGroupLookupProfileCfgGet
(
    IN   GT_U8                                               devNum,
    IN   GT_U32                                              tcamClientGroup,
    IN   GT_U32                                              tcamProfileId,
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChTcamClientGroupLookupProfileCfgGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC(
        (funcId, devNum, tcamClientGroup, tcamProfileId, tcamLookupProfileCfgPtr));

    rc = internal_cpssDxChTcamClientGroupLookupProfileCfgGet(
        devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS,
        tcamClientGroup, tcamProfileId, tcamLookupProfileCfgPtr);
