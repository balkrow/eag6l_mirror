...
)
{
    ...
    for(i = 0 ; i < numOfPaths/4 ; i++)
    {
        ...
        rc = prvCpssDxChWriteTableEntry(devNum,
             pathUtilizationTable[tableId + i],
             index,
             hwData);
...
)
{
    ...
    PRV_CPSS_NOT_APPLICABLE_DEV_CHECK_MAC(devNum,
          CPSS_LION2_E | CPSS_BOBCAT2_E | CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT3_E | CPSS_CAELUM_E |
          CPSS_ALDRIN_E | CPSS_AC3X_E | CPSS_ALDRIN2_E | CPSS_AC5P_E | CPSS_AC5X_E | CPSS_HARRIER_E | CPSS_IRONMAN_E | CPSS_AAS_E);
...
/**
* @internal internal_cpssDxChBrgL2DlbWaLttInfoDelete function
* @endinternal
*
* @brief  Remove LTT Info from DB and free related memory.
*
...
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on wrong input parameters
* @retval GT_NOT_FOUND             - Ltt Info not found
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChBrgL2DlbWaLttInfoDelete
(
    IN GT_U8                                devNum,
    IN GT_U32                               lttIndex
)
{
    ...
    PRV_CPSS_DXCH_BRG_L2_DLB_WA_LTT_USAGE_STC   *dlbWaUsageNodeNextPtr;
    PRV_CPSS_DXCH_BRG_L2_DLB_WA_LTT_USAGE_STC   *dlbWaUsageNodePrevPtr  = NULL;
    ...
    if(lttIndex > PRV_CPSS_DXCH_PP_HW_INFO_LAST_INDEX_IN_L2_LTT_ECMP_MAC(devNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
    }
    ...
    while(dlbWaUsageNodePtr)
    {
        ...
        if(dlbWaUsageNodePtr->lttIndex == lttIndex)
        {
            ...
            if(dlbWaUsageNodePrevPtr == NULL)
            {
                PRV_DLB_LTT_WA_ERRTA_MAC(devNum).headOfDlbWaUsageList = dlbWaUsageNodeNextPtr;
            }
            /* 'Middle or End' of the List */
            else
            {
                dlbWaUsageNodePrevPtr->nextPointer = dlbWaUsageNodeNextPtr;
            }

            /* Remove the Node */
            cpssOsFree(dlbWaUsageNodePtr);
            return GT_OK;
        }
        else
        {
            /* Update node pointers for next run - in case lttIndex did not match */
            dlbWaUsageNodePrevPtr   = dlbWaUsageNodePtr;
...
/**
* @internal cpssDxChBrgL2DlbWaLttInfoDelete function
* @endinternal
*
* @brief  Remove LTT Info from DB and free related memory.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS cpssDxChBrgL2DlbWaLttInfoDelete
(
    IN GT_U8                                devNum,
    IN GT_U32                               lttIndex
)
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgL2DlbWaLttInfoDelete);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, lttIndex));

    rc = internal_cpssDxChBrgL2DlbWaLttInfoDelete(devNum, lttIndex);

    CPSS_LOG_API_EXIT_MAC(funcId, rc);
    CPSS_APP_SPECIFIC_CB_MAC((funcId, rc, devNum, lttIndex));
    CPSS_API_UNLOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    return rc;
}
...
)
{
    ...
    /* secondaryDlbBaseIndex is an index to L2 ECMP table. */
    ECMP_MEMBER_INDEX_CHECK_MAC(devNum, secondaryDlbBaseIndex);
    ...
    while(dlbWaUsageNodePtr)
    {
        ...
        for(i = 0; i < ecmpLttInfo_primary.ecmpNumOfPaths; i++)
        {
            ...
            PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(devNum, portGroupId)
            {
                ...
                U32_SET_FIELD_MAC(hwFormat,0 ,19, 0 /*lastSeenTimeStamp*/);
                rc = prvCpssDxChPortGroupWriteTableEntry(devNum,
                        portGroupId,
                        CPSS_DXCH_SIP5_TABLE_EQ_L2_ECMP_E,
