...
)
{
    ...
    *isValidPtr = BIT2BOOL_MAC(hwDataArr[0] & 0x1);
    ...
    vlanInfoPtr->unkSrcAddrSecBreach = BIT2BOOL_MAC(1 - value);
...
)
{
    ...
    tableInfoPtr = &(PRV_CPSS_ACCESS_TABLE_INFO_PTR(devNum)[CPSS_DXCH_TABLE_VLAN_E]);
...
)
{
    ...
    if (isCpu == GT_FALSE ||
        PRV_CPSS_SIP_5_CHECK_MAC(devNum)/* support CPU SDMA ports */)
    {
    ...
    else
    {
        ...
        {
            /* not defined vlan entry format that supports 128 ports*/
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
...
* @param[in] index                    - Mirror to Analyzer Index
...
*                                      to this Vlan to the analyzer port
* @param[in] index                    - Mirror to Analyzer Index
*                                      (APPLICABLE DEVICES: xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; AAS)
...
)
{
    ...
    switch (direction)
    {
        ...
        case CPSS_DIRECTION_EGRESS_E:
            ...
            {
                /* the field moved from HA to EGF-EFT , and impact the EGF decisions
                   for the 'bmp of physical ports'  that 'attached' to eVlan */
                tableType = CPSS_DXCH_SIP7_EGF_EFT_EVLAN_E;
            }
            break;
...
)
{
    ...
    {
        if (txAnalyzerIndex > CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    {
        if(vlanInfoPtr->mirrToRxAnalyzerIndex >  CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
    ...
    {
        if(vlanInfoPtr->ipv4McBcMirrToAnalyzerIndex >  CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
    ...
    {
        if(vlanInfoPtr->ipv6McMirrToAnalyzerIndex >  CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
        }
...
)
{
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        {
            if(vlanInfoPtr->mirrToTxAnalyzerIndex >  CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(devNum))
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);
            }
...
/**************** EGF_SHT section *********************************************/
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) && vlanId >= PRV_CPSS_MAX_NUM_VLANS_CNS/*4K*/)
    {
        /* only the 'standard' 4K Vlans hold members (and tagging state) */
    }
    else
    {
        /* port members */
...
/**************** EGF_QAG section *********************************************/
    ...
    else
    {
        ...
        {
            /* allow to get vlan tagging on all ports in the physical ports range */
...
*                                      GT_FALSE - IPv6 MC packets in this eVLAN are NOT mirrored to the
*                                      analyzer specified in this field
* @param[out] indexPtr                 - (pointer to) Mirror to Analyzer Index
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on wrong input parameters
* @retval GT_HW_ERROR              - failed to write to hardware
...
/**
* @internal internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet function
* @endinternal
*
* @brief   Enable/Disable Tag0 to be classified if it is found as the outer tag.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
* @param[in] enable                - GT_TRUE  : Enable  : Tag0 may be classified as either the outer tag or inner tag
*                                    GT_FALSE : Disable : Tag0 may be classified as outer tag but not as inner tag
*
* @retval GT_OK                    - on success.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet
(
    IN GT_U8            devNum,
    IN GT_BOOL          enable
)
{
    GT_STATUS   rc;      /* return value */
    GT_U32      regAddr; /* register to use */
    ...
    regAddr = PRV_DXCH_REG1_UNIT_TTI_MAC(devNum).TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt5;
    /* <Enable classifying inner tag as Tag0> */
    rc = prvCpssHwPpSetRegField(devNum, regAddr, 28, 1, BOOL2BIT_MAC(enable));
    if(GT_OK != rc)
    {
        return rc;
    }
...
/**
* @internal cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet function
* @endinternal
*
* @brief   Enable/Disable Tag0 to be classified if it is found as the outer tag.
*
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enable));

    rc = internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet(devNum, enable);
...
/**
* @internal internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableGet function
* @endinternal
*
* @brief   Get Enable/Disable Tag0 to be classified if it is found as the outer tag.
*
* @note   APPLICABLE DEVICES:      AAS.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
*
* @param[in] devNum                - device number
*
* @param[out] enablePtr            - (pointer to) Enabled/Disabled.
*                                    GT_TRUE  : Enable  : Tag0 may be classified as either the outer tag or inner tag
*                                    GT_FALSE : Disable : Tag0 may be classified as outer tag but not as inner tag
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - wrong device.
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
static GT_STATUS internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableGet
(
    IN  GT_U8            devNum,
    OUT GT_BOOL          *enablePtr
)
{
    ...
    regAddr = PRV_DXCH_REG1_UNIT_TTI_MAC(devNum).TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt5;
    /* <Enable classifying inner tag as Tag0> */
    rc = prvCpssHwPpGetRegField(devNum, regAddr, 28, 1, &hwValue);
    if(GT_OK != rc)
    {
        return rc;
    }
...
/**
* @internal cpssDxChBrgVlanClassifyInnerTagAsTag0EnableGet function
* @endinternal
*
* @brief   Get Enable/Disable Tag0 to be classified if it is found as the outer tag.
*
...
{
    GT_STATUS rc;
    CPSS_LOG_FUNC_VARIABLE_DECLARE_MAC(funcId, cpssDxChBrgVlanClassifyInnerTagAsTag0EnableGet);

    CPSS_API_LOCK_MAC(devNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    CPSS_LOG_API_ENTER_MAC((funcId, devNum, enablePtr));

    rc = internal_cpssDxChBrgVlanClassifyInnerTagAsTag0EnableGet(devNum, enablePtr);
