...
/**
********************************************************************************
* @file cpssDxChBrgFdbAu.c
*
* @brief This file provides the CPSS DxCh FDB Address Update (AU) functionality
*
*
* @version   59
********************************************************************************
...
const GT_U32  prvCpssDxChPerPortGroupNumOfAu  = 1;/* <-- allow to modify from test in order to check performances*/
#define PRV_DX_CH_FDB_MANAGER_PER_PORT_GROUP_NUM_OF_AU      4
...
/**
* @internal auDescMultiPortGroupNonSourceFilterCheck function
* @endinternal
*
* @brief   This function check if need to filter message from the AUQ/FUQ due to
...
* @param[in] swapedAuDescExtPtr       - (pointer) to the AU descriptor filled by PP.
*                                      this is pointer to 'swapped' auDesc
*
* @retval GT_TRUE                  - need to filter the message
* @retval GT_FALSE                 - NO need to filter the message
*/
...
/**
* @internal prvAuDesc2UpdMsgUcRoute function
* @endinternal
*
...
* @param[in] auMsgArrayOfWordsPtr     - (pointer to) consider the AU message as array of words
*
* @param[out] addrUpMsgPtr             - (pointer to) MAC format of entry
...
)
{
    ...
    GT_U32                  hwValue1;            /* hardware value 1 of the field      */
    GT_U32                  wordNum;/* used for ipv6 address */
    GT_U32                  isVPort;/* sip7 : interface is vPort ? */
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        switch(hwValue)
        {
        default: /* will not get here on 2 bits field */
        case 0:
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        addrUpMsgPtr->macEntry.fdbRoutingInfo.nextHopVlanId |= (GT_U16)(1 << hwValue);/* bit 15 */
    }
    ...
    if(hwValue==1)/* use vidx*/
    {
        addrUpMsgPtr->macEntry.dstInterface.type = CPSS_INTERFACE_VIDX_E;
         ...
         addrUpMsgPtr->macEntry.dstInterface.vidx = hwValue;
...
)
{
    ...
    *needToSkipCurrMsgPtr = GT_FALSE;
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        ...
        addrUpMsgPtr->isMoved = 0;/* for sip5 will be set later , for sip7 set below */
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            switch(hwValue)
            {
                ...
                case HW_SIP7_FDB_UPDATE_MSG_AA_E:
                    addrUpMsgPtr->updType = CPSS_AA_E;
                    break;
                ...
                case HW_SIP7_FDB_UPDATE_MSG_QI_E:
                    addrUpMsgPtr->updType = CPSS_QI_E;
                    break;
                case HW_SIP7_FDB_UPDATE_MSG_HR_E:
                    addrUpMsgPtr->updType = CPSS_HR_E;
                    break;
                default:
    ...
    {
        addrUpMsgPtr->updType = (CPSS_UPD_MSG_TYPE_ENT)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 4, 3);
        addrUpMsgPtr->associatedHwDevNum = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 7, 5);
    }

    /* The FDB message type 5 (CPSS_SA_E) is used only for Cheetah 2 devices
     for FDB Upload messages.
     not for sip7 as we already given 'addrUpMsgPtr->updType' special treatment above
     */
    if(!PRV_CPSS_SIP_7_CHECK_MAC(devNum) && addrUpMsgPtr->updType == 5)
    {
        addrUpMsgPtr->updType = CPSS_FU_E;
    }
    ...
    if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
    {
        (void) prvCpssDxChBrgFdbAuMsgSpecialMuxedFieldsGet(devNum,auMsgArrayOfWordsPtr,macEntryType, naFormatVersion, &specialFields);
        ...
        SIP5_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                SIP5_FDB_AU_MSG_TABLE_FIELDS_FID_E, hwValue);
    ...
    else
    {
        ...
        {
            macEntryExtKeyPtr->entryType = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 19, 2);
        }
        else
        {
        ...
        if(macEntryExtKeyPtr->entryType == CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E)
        {
            macEntryExtKeyPtr->key.macVlan.vlanId =
                                         (GT_U16)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 0, 12);
            vlanId = macEntryExtKeyPtr->key.macVlan.vlanId;

            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[5] =
                                          (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 16, 8);
            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[4] =
                                          (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 24, 8);
            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[3] =
                                           (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[1], 0, 8);
            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[2] =
                                           (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[1], 8, 8);
            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[1] =
                                          (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[1], 16, 8);
            macEntryExtKeyPtr->key.macVlan.macAddr.arEther[0] =
                                          (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[1], 24, 8);
        ...
        else /* The entry is IPv4 or IPv6 Multicast */
        {
            ...
            macEntryExtKeyPtr->key.ipMcast.sip[1] =
                (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 0, 8);
            macEntryExtKeyPtr->key.ipMcast.sip[0] =
                 (GT_U8)(U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 8, 4) |
        ...
        if (naFormatVersion == 0)
        {
            addrUpMsgPtr->macEntry.daSecurityLevel =
                (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 1, 3);
            addrUpMsgPtr->macEntry.saSecurityLevel =
                (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 12, 3);
        }
        else
        {
            /* set default values */
        ...
        addrUpMsgPtr->macEntry.appSpecificCpuCode =
                                    (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 29, 1);
    }
    ...
    if (CPSS_QR_E == addrUpMsgPtr->updType)
    {
        ...
        {
            addrUpMsgPtr->entryWasFound = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 15, 1);
        }
    }
    else
    {
    ...
    else
    {
        if (naFormatVersion == 0)
        {
            addrUpMsgPtr->skip =  (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 12, 1);
            addrUpMsgPtr->aging = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 13, 1);
        }
        else
        {
            /* set default values */
    ...
    else
    {
        addrUpMsgPtr->macEntry.spUnknown =
            BIT2BOOL_MAC(((auMsgArrayOfWordsPtr[2] >> 14) & 1));
    }
    ...
    if (naFormatVersion == 0)
    {
        ...
        {
            multiple = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2],15, 1);
        }
    }
    else
    {
    ...
    if(multiple || useVidx)
    {
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            ...
            if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                ...
                addrUpMsgPtr->macEntry.dstInterface.vidx |= hwValue << 16;/* bit 16 */
            }
        }
        else
        {
            addrUpMsgPtr->macEntry.dstInterface.vidx =
                U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 17, 12);
        }
    ...
    else
    {
        if(PRV_CPSS_SIP_6_CHECK_MAC(devNum) &&
           (macEntryExtKeyPtr->entryType == CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E||
            macEntryExtKeyPtr->entryType == CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_FID_VID1_E))
        {
            /* MAC table FDB Upload(FU) message type uses different format than
             * the New Address (NA) message type. Need to use the FU specific
             * format to derive the correct interface details.
             */
            if (naFormatVersion == 0)
            {
                /* is trunk */
                ...
                else
                {
                    ...
                    if(isVPort)
                    {
                        ...
                        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                            ...
                            hwValue);
                        addrUpMsgPtr->macEntry.dstInterface.devPort.portNum |= hwValue << 14;
                    }
                    else
                    {
                        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
            ...
            else /* (naFormatVersion == 1) --> 'NA moved format' */
            {
                /* is trunk */
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
        ...
        {
            addrUpMsgPtr->macEntry.userDefined = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 25, 4);

            if((GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 17, 1) == 1)
            {/* the message is associated with a trunk */
                addrUpMsgPtr->macEntry.dstInterface.trunkId =
                    (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 18, 7);
    ...
    if (naFormatVersion == 0)
    {
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            ...
            if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
            {
                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    ...
                    hwValue);
                SIP6_CONVERT_HW_DA_CMD_TO_SW_VAL_MAC(devNum, addrUpMsgPtr->macEntry.daCommand, hwValue);

                SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                    SIP6_FDB_AU_MSG_TABLE_FIELDS_SA_CMD_E,
                    hwValue);
                SIP6_CONVERT_HW_SA_CMD_TO_SW_VAL_MAC(devNum, addrUpMsgPtr->macEntry.saCommand, hwValue);
        ...
        {
            addrUpMsgPtr->macEntry.isStatic = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 18, 1);

            addrUpMsgPtr->macEntry.daQosIndex = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 15, 3);
            addrUpMsgPtr->macEntry.saQosIndex = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 12, 3);
            ...
            addrUpMsgPtr->macEntry.mirrorToRxAnalyzerPortEn =
                                            (GT_BOOL)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 31, 1);

            addrUpMsgPtr->macEntry.daRoute = (GT_BOOL)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 30, 1);
        }
    }
    else
    {
    ...
    {
        addrUpMsgPtr->macEntry.sourceID = (GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 2, 5);

        addrUpMsgPtr->naChainIsTooLong = (GT_BOOL)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 31, 1);
    }
       ...
       (addrUpMsgPtr->updType == CPSS_QA_E))
    {
        ...
        {
            addrUpMsgPtr->entryOffset = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 7, 5);
        }
    }
        ...
        (addrUpMsgPtr->updType == CPSS_TA_E))
    {
        ...
        {
            addrUpMsgPtr->macEntryIndex = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 7, 8);
            if(macEntryExtKeyPtr->entryType == CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E)
            {
                addrUpMsgPtr->macEntryIndex |=
                    ((U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 27, 4) << 8) |
                     (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 0, 2) << 12));
            }
        }
    }
    ...
    if (naFormatVersion == 1)
    {
        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum) == GT_TRUE)
        {
            ...
            {
                /*sip7 : we already set addrUpMsgPtr->isMoved according to MSG_TYPE
                    as there is no 'is moved' bit any more */
            }
                ...
                else
                {
                    ...
                    if(isVPort)
                    {
                        ...
                        SIP6_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
                            ...
                            hwValue);
                        addrUpMsgPtr->oldDstInterface.devPort.portNum |= hwValue << 14;
                    }
                    else
                    {
                        SIP5_FDB_AU_MSG_FIELD_GET_MAC(devNum,auMsgArrayOfWordsPtr,
        ...
        else
        {
            /* vid1 */
            addrUpMsgPtr->vid1 =
                (GT_U16)(U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 1, 2) |            /* bits 1..2 */
                         (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 12, 3) << 2) |    /* bits 12..14 */
                         (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 27, 4) << 5) |    /* bits 123..126 */
                         (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 0, 2) << 9) |     /* bits 96..97 */
                         (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 15, 1) << 11));   /* bit 15 */

            /* up0 */
            addrUpMsgPtr->up0 = U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 12, 3);

            /* check if the station was moved */
            addrUpMsgPtr->isMoved = (GT_BOOL)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 16, 1);

            if (addrUpMsgPtr->isMoved == GT_TRUE)
            {
                addrUpMsgPtr->oldSrcId =
                    (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 31, 1) |           /* bit 127 */
                     (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 21, 3) << 1) |    /* bits 117..119 */
                     (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 30, 1) << 4));    /* bit 94   */

                addrUpMsgPtr->oldAssociatedHwDevNum =
                    (GT_U8)(U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 19, 2) |           /* bit 115..116 */
                            (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 15, 3) << 2));    /* bit 111..113 */

                oldPortTrunk = (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[0], 3, 1) |         /* bit 3 */
                                (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 15, 1) << 1) | /* bit 79 */
                                (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 13, 1) << 2) | /* bit 77 */
                                (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[2], 12, 1) << 3) | /* bit 76 */
                                (U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 24, 3) << 4)); /* bits 120..122 */

                /* check if old interface type is port or trunk */
                if((GT_U8)U32_GET_FIELD_MAC(auMsgArrayOfWordsPtr[3], 18, 1) == 1)
                {
                    /* the old interface is associated with a trunk */
...
)
{
    ...
    PRV_CPSS_AU_DESC_EXT_8_STC  auDescExt;
    ...
    if (auDescrFromDma == GT_TRUE)
    {
        ...
        if(PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.auMessageNumOfWords == 8)
        {
    ...
    rc = internal_auDesc2UpdMsg(devNum,portGroupId,
                auMsgArrayOfWordsPtr,
                addrUpMsgPtr,
                &needToSkipCurrMsg);

    if(needToSkipCurrMsg == GT_TRUE)
    {
        AU_DESC_RESET_MAC(auDescPtr);
    }
...
/**
* @internal auMsgFifoBlockGet function
* @endinternal
*
* @brief   Device specific get function for AU messages from FIFO -
...
/**
* @internal prvDxChBrgFdbAuMsgFifoBlockDevGet function
* @endinternal
*
* @brief   Device specific get function for AU messages from FIFO
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        portGroupsBmp = PRV_CPSS_DXCH_PP_HW_INFO_MG_MAC(devNum).sip6AuqPortGroupBmp;
    }
    else
    {
...
/**
* @internal auFuMsgBlockGet function
* @endinternal
*
...
)
{
    ...
    {
        /* for GDMA device see prvCpssDxChGdmaFdbAuFuMsgBlockGet */
       ...
       (PRV_CPSS_PP_MAC(devNum)->intCtrl.auqDeadLockWa[portGroupId].primaryState == PRV_CPSS_AUQ_STATE_ALL_EMPTY_E))
    {
        ...
        rc = auFuMsgBlockGet(devNum,portGroupId,MESSAGE_QUEUE_SECONDARY_AUQ_E,
                                  &numOfWaAu,auFuMessagesPtr, hwAuMessagesPtr);
    ...
    *numOfAuFuPtr = ii;
    ...
    for (/*ii was already initialized*/; ii < auMaxCnt; ii++)
    {
        ...
        if(hwAuMessagesPtr != NULL)
        {
            ...
            AU_DESC_SWAP_MAC((volatile PRV_CPSS_AU_DESC_STC *)(&descExtPtr->elem0), hwAuMessagesPtr[ii].elem0);
            AU_DESC_SWAP_MAC((volatile PRV_CPSS_AU_DESC_STC *)(&descExtPtr->elem1), hwAuMessagesPtr[ii].elem1);
        }
        else
        {
        ...
        if(descCtrlPtr->currDescIdx == 0)
        {
               ...
               (PRV_CPSS_PP_MAC(devNum)->intCtrl.auqDeadLockWa[portGroupId].primaryState == PRV_CPSS_AUQ_STATE_ALL_EMPTY_E))
            {
                ...
                for ( /*continue ii */  ; ii < auMaxCnt; ii++)
                {
                    ...
                    numOfWaAu = 1;
                    rc = auFuMsgBlockGet(devNum,portGroupId,MESSAGE_QUEUE_SECONDARY_AUQ_E,
                                    &numOfWaAu,&auFuMessagesPtr[ii], hwAuMessagesPtr);
            ...
            if (enableRewind == GT_TRUE)
            {
                rc = prvCpssDxChFullQueueRewind(
                    devNum,
                        isFuQueue == GT_FALSE ?
                        MESSAGE_QUEUE_PRIMARY_AUQ_E:/*AUQ*/
                        MESSAGE_QUEUE_PRIMARY_FUQ_E,/*FUQ*/
...
)
{
    ...
    /* number of AU to serve from each port group , before query other port groups */
    GT_U32  perPortGroupNumOfAu = (hwAuMessagesPtr == NULL)? prvCpssDxChPerPortGroupNumOfAu :PRV_DX_CH_FDB_MANAGER_PER_PORT_GROUP_NUM_OF_AU;
    ...
    if ((tempSystemRecovery_Info.systemRecoveryState != CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E) &&
    ...
    if(singleFdbInstance == GT_TRUE)
    {
        if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
        {
            return prvCpssDxChGdmaFdbAuFuMsgBlockGet(devNum,
                CPSS_NON_MULTI_PORT_GROUP_DEVICE_INDEX_CNS,
                queueType,
                numOfAuFuPtr,
                auFuMessagesPtr,
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        /* according to AUQ also for FUQ ... because :
           the FUQ get FDB messages from the single FDB unit (just like AUQ)

           NOTE: there is FUQ per pipe but this is for CNC upload ... and not for
           FDB dump that need FUQ only 'per tile'
    ...
    for (auCnt = 0; (auCnt < auMaxCnt); auCnt += perPortGroupNumOfAu)
    {
        ...
        /* For FDB manager prvCpssDxChPerPortGroupNumOfAu should be 4 */
        perPortGroupNumOfAu = (hwAuMessagesPtr == NULL)?prvCpssDxChPerPortGroupNumOfAu:PRV_DX_CH_FDB_MANAGER_PER_PORT_GROUP_NUM_OF_AU;

        /* Check current AUQ index for acceptable range */
        if (auCnt + perPortGroupNumOfAu > auMaxCnt)
        {
            /* Limit number of AUQ messages to minimal acceptable value */
            perPortGroupNumOfAu = auMaxCnt - auCnt;
        }
        ...
        if (hwAuMessagesPtr == NULL)
        {
            if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
            {
                rc =  prvCpssDxChGdmaFdbAuFuMsgBlockGet(devNum,
                    portGroupId,
                    queueType,
                    &perPortGroupNumOfAu,
                    &auFuMessagesPtr[auCnt],
                    NULL);
            }
            else
            {
                rc = auFuMsgBlockGet(devNum ,portGroupId, queueType, &perPortGroupNumOfAu , &auFuMessagesPtr[auCnt], NULL);
            }
        }
        else
        {
            if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
            {
                rc =  prvCpssDxChGdmaFdbAuFuMsgBlockGet(devNum,
                    ...
                    &perPortGroupNumOfAu,
                    NULL,
                    &sip7HwAuMessagesPtr[auCnt]);
            }
            else
            {
                rc = auFuMsgBlockGet(devNum ,portGroupId, queueType, &perPortGroupNumOfAu , NULL, &hwAuMessagesPtr[auCnt]);
            }
            for(ii=0; ii<perPortGroupNumOfAu; ii++)
            {
                portGroupIdPtr[auCnt+ii] = portGroupId;
            }
        }
        if(rc == GT_OK)
        {
...
/**
* @internal internal_cpssDxChBrgFdbFuMsgBlockGet function
* @endinternal
*
...
)
{
                        ...
                        enabled == GT_FALSE))
    {
       ...
       {
            return prvDxChBrgFdbAuFuMsgBlockGet(devNum,MESSAGE_QUEUE_PRIMARY_FUQ_E,numOfFuPtr,fuMessagesPtr, NULL, NULL,NULL);
       }
       else
       {
            return prvDxChBrgFdbAuFuMsgBlockGet(devNum,MESSAGE_QUEUE_PRIMARY_AUQ_E,numOfFuPtr,fuMessagesPtr, NULL, NULL,NULL);
       }
    }
    else
    {
...
/**
* @internal internal_cpssDxChBrgFdbAuMsgBlockGet function
* @endinternal
*
* @brief   The function return a block (array) of FDB Address Update (AU) messages,
*         the max number of elements defined by the caller
...
)
{
    ...
    {
        return
          prvDxChBrgFdbAuFuMsgBlockGet(devNum,MESSAGE_QUEUE_PRIMARY_AUQ_E,numOfAuPtr,auMessagesPtr, NULL, NULL,NULL);
    }
    else
    {
        return
...
/**
* @internal internal_cpssDxChBrgFdbAuqFuqMessagesNumberGet function
* @endinternal
*
* @brief   The function scan the AU/FU queues and returns the number of
*         AU/FU messages in the queue.
...
)
{
    ...
    GT_U32      portGroupsBmp;/* port group bitmap for FDB unit */
    ...
    *endOfQueueReachedPtr = GT_FALSE;
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(devNum,portGroupsBmp,portGroupId)
    {
        if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(devNum))
        {
            rc =  prvCpssDxChGdmaFdbAuqFuqMessagesNumberGet(devNum ,portGroupId,
...
/**
* @internal internal_cpssDxChBrgFdbSecondaryAuMsgBlockGet function
* @endinternal
*
