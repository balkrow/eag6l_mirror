...
)
{
    ...
    GT_U32    hwFormatArr[2] = {0,0}; /* data of LTT entry in HW format */
    ...
    U32_SET_FIELD_MAC(hwFormatArr[0],2,numBits_L2_ECMP_Start_Index,ecmpLttInfoPtr->ecmpStartIndex);
    /* The number of paths in the ECMP block.
       0x0 = 1 path.
       0x1 = 2 paths.*/
    U32_SET_FIELD_MAC(hwFormatArr[0],2+numBits_L2_ECMP_Start_Index,12,(ecmpLttInfoPtr->ecmpNumOfPaths - 1));
    U32_SET_FIELD_MAC(hwFormatArr[0],0,1,ecmpLttInfoPtr->ecmpEnable);
    U32_SET_FIELD_MAC(hwFormatArr[0],1,1,BOOL2BIT_MAC(ecmpLttInfoPtr->ecmpRandomPathEnable));

    /* index to hash bit selection profile */
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        U32_SET_FIELD_IN_ENTRY_MAC(hwFormatArr,30,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
    else
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        U32_SET_FIELD_MAC(hwFormatArr[0],28,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
...
)
{
    ...
    ecmpLttInfoPtr->ecmpStartIndex = U32_GET_FIELD_MAC(hwFormatArr[0],2,numBits_L2_ECMP_Start_Index);
     /* The number of paths in the ECMP block.
       0x0 = 1 path.
       0x1 = 2 paths.*/
    ecmpLttInfoPtr->ecmpNumOfPaths = U32_GET_FIELD_MAC(hwFormatArr[0],2+numBits_L2_ECMP_Start_Index,12) + 1;
    ecmpLttInfoPtr->ecmpEnable     = BIT2BOOL_MAC(U32_GET_FIELD_MAC(hwFormatArr[0],0,1));
    ecmpLttInfoPtr->ecmpRandomPathEnable     = BIT2BOOL_MAC(U32_GET_FIELD_MAC(hwFormatArr[0],1,1));

    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        U32_GET_FIELD_IN_ENTRY_MAC(hwFormatArr,30,4,ecmpLttInfoPtr->hashBitSelectionProfile);
    }
    else
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ecmpLttInfoPtr->hashBitSelectionProfile = U32_GET_FIELD_MAC(hwFormatArr[0],28,4);
    }
...
*       2. a. in SIP7 the L2 ECMP is not used by the Trunk , as there is dedicated Trunk ECMP table.
*            see APIs : cpssDxChTrunkEcmpTableSet/cpssDxChTrunkEcmpTableGet
*          b. in SIP5 the L2 ECMP is used also by the Trunk so when trunk mode
*           is 'CPSS_DXCH_TRUNK_MEMBERS_MODE_FLEX_E' the CPSS allows access to all table entries.
*           Otherwise the CPSS allows access to ONLY to lower half of table entries.
*           so Index to this table is also according to index from Trunk ECMP LTT
*
*/
