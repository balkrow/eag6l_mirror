...
* @param[in] dataArr[]               - array of 32bit words, contain data.
...
)
{
    ...
    ipv6IndexMask = 0xFFFFFFFF - (hashParams.numOfMultipleHashes - 1);
        ...
        PRV_CPSS_SIP_5_25_CHECK_MAC(devNum))
    {
            ...
            ii += 2)
        {
            crcMultiHashArr[(ii + 1) - multiHashStartBankIndex] =
                ...
                (crcMultiHashArr[ii - multiHashStartBankIndex] & ipv6IndexMask) + (ii + 1);
        }
    }
...
*/
GT_STATUS prvCpssDxChBrgFdbHashCrcMultiResultsByParamsCalc_maxFidValue_Set(IN GT_U32    maxFidValue)
{
    PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.dxChConfigDir.debug_maxFidValue) = maxFidValue;
...
)
{
    ...
    CPSS_TBD_BOOKMARK_AAS  /* need to add 'sip7' option to CPSS_DXCH_BRG_FDB_HASH_PARAMS_STC */
    maxFidValue = PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.dxChConfigDir.debug_maxFidValue) ?
                  PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.dxChConfigDir.debug_maxFidValue) : /* option to state for sip7 */
                  _8K-1;/*default for sip5*/
