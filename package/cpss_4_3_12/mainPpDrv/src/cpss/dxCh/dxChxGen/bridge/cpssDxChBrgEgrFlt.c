...
)
{
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        ...
        queue_base_index >>= 0;/* BWC logic - till fix WM logic*/
    }
    else
    {
        /* get the <queue_base_index> of the portNum */
...
)
{
    ...
    if(PRV_CPSS_DXCH_PP_MAC(devNum)->port.oamUpMepPortWaPtr[portNum].isOamUpMepEgressPort == GT_TRUE)
    {
        /* OAM WA: In case the port is configured to be an target egress port on an UP-MEP OAM instance, return TRUE because
           we ensure that the packet will be dropped after the EGF - by the preq */
        return GT_TRUE;
    }
...
)
{
    ...
    if(PRV_CPSS_SIP_5_20_CHECK_MAC(devNum))
    {
        ...
        if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
        {
            /* This conditions applies only after init stage */
            if ( PRV_CPSS_DXCH_PP_MAC(devNum)->port.oamUpMepPortWaPtr[portNum].isOamUpMepEgressPort == GT_TRUE)
            {
                rc = prvCpssDxChPortLinkStatusGet(devNum, portNum, &isLinkUp);
                if (rc != GT_OK)
                {
                    return rc;
                }
                if (value == 1 && isLinkUp == GT_FALSE)
                {
                    /* This Attachment to the tail-drop profile must happen BEFORE setting the port's EGF status */
                    CPSS_LOG_INFORMATION_MAC("OAM Up-MEP WA: devNum[%d] portNum[%d] link state is DOWN but EGR filter status is FORCE_UP to allow" \
                        "packets to pass the EGF and get  dropped by the WA tail-drop profile.", devNum,portNum);

                    rc = prvCpssDxChPortOamUpMepWaAttachTailDropProfileToPort(devNum, portNum);
                    if (rc != GT_OK)
                    {
                        return rc;
                    }
