...
/* Located in the SCPU SRAM reserved area */
...
int ipcInit(void* ipcBase)
{
    shmIpcInit( &shm, ipcBase, IPC_SIZE, 0 /* Client */, wr_Ipc_sync, NULL);
...
int send_ipc_message(char* ipcBase, char* str)
{
...
void receive_ipc_message(char* ipcBase, char* str)
{
    struct IPC_MSG msg = {0};
    int size, i;
