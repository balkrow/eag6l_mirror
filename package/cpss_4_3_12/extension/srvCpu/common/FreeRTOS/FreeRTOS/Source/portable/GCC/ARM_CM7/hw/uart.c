...
MV_VOID mvUartInit( MV_U32 port, MV_U32 baudDivisor, MV_UART_PORT *base )
{
    ...
    unsigned long i;
    volatile unsigned *pRegPtr;

    uartBase[ port ] = pUartPort = ( volatile MV_UART_PORT * )base;

    mvSemaLock( MV_SEMA_UART );

#if (defined CONFIG_AAS && !defined AAS_EMULATOR)
    pRegPtr = (unsigned *)0x5c4e0104;
    *pRegPtr &= 0xffffff00; /* Disconnect UART from MPP */
#endif

#ifdef AAS_EMULATOR
    baudDivisor = 1; /* emulation baud divisor */
#endif

    pUartPort->ier = 0x00;
    pRegPtr = (unsigned *)(((unsigned long)base) + 0x7C); /* USR - UART Status */

    i = 100000;
    while ( (*pRegPtr & 0x1) && (i--) ) {} /* wait for UART to be IDLE before accessing LCR register divisor bit */
    pUartPort->lcr = LCR_DIVL_EN | LCR_8N1; /* Access baud rate */
    if ( (pUartPort->dll == 0) || (baudDivisor == 1 /* emulator */) ) {
    ...
    for ( i = 0; i < MV_UART_FIFO_SIZE; i++ )
        pUartPort->rbr;
    pUartPort->fcr = FCR_RXSR | FCR_TXSR; /* reset and disable FIFO */

    /* Enable Rx/Tx interrupts */
    pUartPort->ier = IER_RX_INT_EN;

#if (defined CONFIG_AAS && !defined AAS_EMULATOR)
    *pRegPtr |= 0x11; /* Restore MPPs and reconnect UART to MPP */
#endif
