...
/* CONFIG_A8K is not in use for now, if will be,
   these definitions will overwrite definitions above */
#ifdef CONFIG_A8K
#define MV_UART_REGS_OFFSET( port ) ( 0x40512000 | 0x100 * ( port ))
#endif

#define mvUartBase( port )      \
        (( MV_UART_PORT * )( MV_UART_REGS_OFFSET( port )))
