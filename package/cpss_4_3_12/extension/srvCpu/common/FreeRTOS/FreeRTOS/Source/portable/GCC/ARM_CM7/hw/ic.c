...
extern unsigned long exception_table[];
...
{
    /* set excption table - start at address VTOR */
    unsigned long int *VTOR = (unsigned long *)0xE000ED08;

    ( *(( volatile unsigned long * ) ( (interrupt * 4) + (*VTOR) ))) = ( unsigned long )func;
}
...
void vICInit( void )
{
    ...
    if (!(*VTOR))
        *VTOR = (unsigned long)exception_table; /* Assumed aligned to 128 bytes boundary */
    __sync_synchronize();
}
