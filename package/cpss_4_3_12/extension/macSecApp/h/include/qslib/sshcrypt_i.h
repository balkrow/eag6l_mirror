...
/* Indicates to the crypto library that a crypto object has been
   allocated and (about to be) returned to a caller for general
   use. This must be called by all gen* or allocate routines at
   appropriate points. This routine will call ssh_fatal if it is
   called at inappropriate state. */
Boolean
ssh_crypto_library_object_use(void);
