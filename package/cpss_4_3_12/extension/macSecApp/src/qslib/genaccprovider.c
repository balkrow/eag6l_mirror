...
/* Read specific info about a SshPrivateKey into the SshECDSAPrivateKeyInfo
   object. */
static Boolean
get_ecdsa_prvkey_info(SshPrivateKey key, SshECDSAPrivateKeyInfo ecdsa_info)
{
  char *key_type = NULL;
  unsigned int entropy = 0;
  SshECDSAPrivateKeyInfo ecdsa = ecdsa_info;
  char *fixed_curve = NULL;

  SSH_ASSERT(ecdsa != NULL);
  ...
  if (strcmp(key_type, "ec-modp"))
    return FALSE;

  ssh_mprz_init(&ecdsa->px);
  ssh_mprz_init(&ecdsa->py);
  ssh_mprz_init(&ecdsa->x);

  if ((ssh_private_key_get_info(key,
                                SSH_PKF_PUBLIC_Y, &ecdsa->px, &ecdsa->py,
                                SSH_PKF_SECRET_X, &ecdsa->x,
                                SSH_PKF_PREDEFINED_GROUP, &fixed_curve,
                                SSH_PKF_RANDOMIZER_ENTROPY, &entropy,
                                SSH_PKF_END))
      != SSH_CRYPTO_OK)
    {
  ...
  ecdsa->exponent_entropy = entropy;
  ecdsa->fixed_curve = fixed_curve;
                                        ...
                                        void *context)
{
  ...
  if (is_ecdsa_key)
    {
      if (ssh_private_key_get_info(source,
                                   ...
                                   SSH_PKF_END) != SSH_CRYPTO_OK)
        {
          SSH_DEBUG(SSH_D_FAIL, ("Could not read the ECDSA prime"));
          goto fail;
        }
  ...
  else if (is_ecdsa_key)
    {
      if (get_ecdsa_prvkey_info(source, &acc_key->u.ecdsa_prv)
          == FALSE)
        {
          SSH_DEBUG(SSH_D_FAIL, 
                    ("Could not get the ECDSA private key info"));
          goto fail;
        }
      ...
      target =
          ssh_private_key_create_proxy(SSH_PROXY_ECDSA,
                                       (SshUInt32)key_size,
                                       ssh_genacc_modexp_key_op,
                                       ssh_ecdsa_prvkey_free_op,
                                       acc_key);
    }
  else
    SSH_NOTREACHED;
                                  ...
                                  void *context)
{
  ...
  (*get_pk_group_cb)(target ? SSH_EK_OK : SSH_EK_FAILED, target, context);
  return NULL;
