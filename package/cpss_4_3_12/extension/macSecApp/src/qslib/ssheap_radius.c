...
ssh_eap_radius_reset(SshEap eap)
{
  ...
  if (eap->key_name != NULL)
    {
      ssh_free(eap->key_name);
      eap->key_name    = NULL;
      eap->key_namelen = 0;
    }
                         ...
                         SshRadiusOperationCode reply_code)
{
  ...
  SSH_DEBUG(SSH_D_NICETOKNOW,("RADIUS code: %d: %s", reply_code, code_name));
    ...
    if (ret == SSH_RADIUS_AVP_STATUS_SUCCESS)
      {
        ...
        SSH_DEBUG(SSH_D_NICETOKNOW,
                ("RADIUS AVP type %d: name: %s: length %d",
                type, avp_name, value_len));
      }
    ...
    if (ret == SSH_RADIUS_AVP_STATUS_SUCCESS)
      {
	    ...
	    break;
          ...
          case SSH_RADIUS_AVP_EAP_KEY_NAME:
	    
	    SSH_DEBUG_HEXDUMP(5, ("Key name %d", reply_code), ucp, value_len);

	    /* EAP-Key-Name is only sent in an Access-Accept message. */
	    if (reply_code == SSH_RADIUS_ACCESS_ACCEPT)
	      {
		eap->key_name = ssh_memdup(ucp, value_len);
		eap->key_namelen = value_len;
	      }
