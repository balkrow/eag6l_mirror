...
static const int ssh_encoded_sha512_oid_len =
sizeof(ssh_encoded_sha512_oid) / sizeof(unsigned char);

static const int ssh_encoded_sha384_oid_len =
sizeof(ssh_encoded_sha384_oid) / sizeof(unsigned char);

/* Compares the given oid with max size of max_len to the oid defined
   for the hash. If they match, then return the number of bytes
   actually used by the oid. If they do not match, return 0. */
size_t ssh_sha512_asn1_compare(const unsigned char *oid, size_t max_len)
{
  if (max_len >= ssh_encoded_sha512_oid_len &&
      (memcmp(oid,
	      ssh_encoded_sha512_oid,
	      ssh_encoded_sha512_oid_len) == 0))
    return ssh_encoded_sha512_oid_len;
  return 0;
}
...
/* Generate encoded asn1 oid. Returns the pointer to the staticly
   ...
   of the oid. */
const unsigned char *ssh_sha512_asn1_generate(size_t *len)
{
  if (len) *len = ssh_encoded_sha512_oid_len;
  return ssh_encoded_sha512_oid;
}

/* Compares the given oid with max size of max_len to the oid defined
   for the hash. If they match, then return the number of bytes
   actually used by the oid. If they do not match, return 0. */
size_t ssh_sha384_asn1_compare(const unsigned char *oid, size_t max_len)
{
  if (max_len >= ssh_encoded_sha384_oid_len &&
      (memcmp(oid,
	      ssh_encoded_sha384_oid,
	      ssh_encoded_sha384_oid_len) == 0))
    return ssh_encoded_sha384_oid_len;
  return 0;
}
...
/* Generate encoded asn1 oid. Returns the pointer to the staticly
   allocated buffer of the oid. Sets the len to be the length
   of the oid. */
const unsigned char *ssh_sha384_asn1_generate(size_t *len)
{
  if (len) *len = ssh_encoded_sha384_oid_len;
  return ssh_encoded_sha384_oid;
}
...
const SshHashDefStruct ssh_hash_sha512_def =
{
  ...
  ssh_sha512_final,
  /* ASN1. */
  ssh_sha512_asn1_compare,
  ssh_sha512_asn1_generate
};
...
const SshHashDefStruct ssh_hash_sha384_def =
{
  ...
  ssh_sha384_final,
  /* No ASN1. */
  ssh_sha384_asn1_compare,
  ssh_sha384_asn1_generate
};
