                ...
                SshCMOcspKeyID key_id)
{
  ...
  SSH_DEBUG(6, ("Add to nega cache: URL: %s, serial: %s, hash: %s.",
          ...
          if (bsr->status.status == SSH_OCSP_CERT_STATUS_REVOKED)
            {
              ...
              SSH_DEBUG(5,
                      ("OCSP Result: Certificate is %s.",
                      ((reason == SSH_X509_CRLF_CERTIFICATE_HOLD) ?
                      "on hold" : "revoked")));
  ...
  if (index < ncerts)
    {
	  ...
	  if (resp_cert) ssh_cm_cert_free(resp_cert);
        ...
        {
          SshCMStatus l_status = SSH_CM_STATUS_OK;

          SSH_DEBUG(SSH_D_HIGHOK,
                    ("OCSP Responder certificate found from the response."));
          l_status = ssh_cm_add(resp_cert);
          if (l_status != SSH_CM_STATUS_OK &&
              l_status != SSH_CM_STATUS_ALREADY_EXISTS)
            {
	      ssh_cm_cert_free(resp_cert);
          ...
          else
            {
              ...
              if (l_status == SSH_CM_STATUS_ALREADY_EXISTS)
                {
                  ssh_cm_cert_free(resp_cert);
                }
            }
