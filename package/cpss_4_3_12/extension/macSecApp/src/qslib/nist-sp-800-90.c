/**
  ...
  @copyright
  Copyright (c) 2010 - 2013, Rambus Inc. All rights reserved.
                               ...
                               SshDrbgState *initial_state)
{
  ...
  *initial_state = ssh_calloc(1, sizeof(**initial_state));
  ...
  aes_context = ssh_malloc(ssh_rijndael_ctxsize());
...
ssh_drbg_uninstantiate(SshDrbgState state_handle)
{
  ...
  if (state_handle->aes_context != NULL)
    ssh_free(state_handle->aes_context);
...
/* Use QuickSec IKEv2 vendor id as personalization string */
const unsigned char personalization_string[] =
  {0x2c, 0x18, 0x29, 0x2c, 0x67, 0x5f, 0xbf, 0x1c, 
   0x9c, 0xad, 0x0e, 0x9f, 0xea, 0x7c, 0x05, 0x10};
