                      ...
                      char *cause_str)
{
  ...
  SSH_DEBUG(SSH_D_NETGARB,("discarding token of type %d%s%s",
                           ssh_eap_get_token_type_from_buf(buf),
                           (cause_str != NULL ? ": " : ""),
                       ...
                       char *cause_str)
{
  ...
  SSH_DEBUG(SSH_D_NETGARB,
            ("discarding EAP packet with code %d type %d%s%s",
             (ssh_eap_packet_isvalid(buf) ? ssh_eap_packet_get_code(buf) : 0),
             eap_type,
             (cause_str != NULL ? ": " : ""),
             (cause_str != NULL ? cause_str : "")));
...
ssh_eap_input_succfail(SshEap eap, SshBuffer buf)
{
  ...
  switch (code)
    ...
    case SSH_EAP_CODE_SUCCESS:
      SSH_DEBUG(SSH_D_HIGHOK,("received success message"));
    ...
    case SSH_EAP_CODE_FAILURE:
      SSH_DEBUG(SSH_D_HIGHOK,("received failure message"));
