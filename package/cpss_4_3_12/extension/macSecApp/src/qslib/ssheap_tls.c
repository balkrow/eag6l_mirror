                         ...
                         SshUInt32 total_length)
{
  ...
  if (flags & SSH_EAP_TLS_FLAGS_LENGTH_BIT)
    {
      SshUInt32 c_len = ((total_length & 0xff) << 24) + 
        ((total_length & 0xff00) << 8) +
        ((total_length & 0xff0000) >> 8) + 
        ((total_length & 0xff000000) >> 24);

      SSH_ASSERT(total_length != 0);

      SSH_DEBUG(SSH_D_MY, ("Including length %X %X", total_length, c_len));

      if (ssh_buffer_append(pkt, (unsigned char *)&c_len, 
                            sizeof(c_len)) != SSH_BUFFER_OK)
        {
          ssh_buffer_free(pkt);
          pkt = NULL;
          ssh_eap_fatal(eap, protocol, "Out of memory, cannot send "
                     ...
                     SshEap eap, SshBuffer buf)
{
  ...
  msg_len = (ssh_buffer_ptr(buf)[2] << 8) | ssh_buffer_ptr(buf)[3];
                                           ...
                                           unsigned char **cas)
{
  ...
  for (i = 0; i < cnt; i++)
    {
      new_cas[i] = ssh_memdup(cas[i], strlen((const char *)cas[i]));
      if (new_cas[i] == NULL)
        {
                ...
                SshEap eap, SshUInt8 type)
{
  ...
  if ((eap->msk = ssh_malloc(64)) == NULL)
    return SSH_EAP_OPSTATUS_FAILURE;
  if (eap->key_namelen > 0 &&
      (eap->session_id = ssh_malloc(eap->key_namelen)) == NULL)
    {
      ssh_free(eap->msk);
      eap->msk = NULL;
      return SSH_EAP_OPSTATUS_FAILURE;
  ...
  if (eap->key_namelen > 0)
    {
      memcpy(eap->session_id, eap->key_name, eap->key_namelen);
      eap->session_id_len = eap->key_namelen;
    }
