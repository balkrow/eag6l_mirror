...
extern GT_STATUS luaCLIEngine_initSharedLib
(
    void
);
...
CPSS_OS_IO_PRINTF_FUNC         cmdOsPrintf = printf;
...
extern GT_STATUS cpssLogDelete
(
    GT_VOID
);

extern CPSS_OS_MUTEX       storageMtxId;
...
void mainTask(void)
{
    ...
    printf("Exit Lua standalone engine.\n");
    osStopEngine();
    cpssOsMutexDelete(storageMtxId);
    exit(0);
...
#ifdef SHARED_MEMORY
#ifdef ASIC_SIMULATION
    GT_BOOL aslr = OS_ASLR_SUPPORT_DEFAULT;
