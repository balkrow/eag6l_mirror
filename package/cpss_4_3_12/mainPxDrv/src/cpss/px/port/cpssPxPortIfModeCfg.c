...
)
{
    ...
    if (fecAbility != CPSS_PORT_FEC_MODE_LAST_E)
    {
        for (portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
        {
            ...
            {
                PRV_CPSS_CONVERT_CPSS_FEC_MODE_VAL_TO_HWS_MAC(fecAbility, fecMode);

                PRV_CPSS_PX_PORT_NUM_CHECK_AND_MAC_NUM_GET_MAC(devNum, portNum, portMacNum);

                if ((CPSS_PORT_INTERFACE_MODE_NA_E == ifMode) || (CPSS_PORT_SPEED_NA_E == speed) || (FEC_NA == fecMode))
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
                }
...
)
{
    ...
    PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesFreeCheck = prvCpssPxPortSerdesFreeCheck;
