...
/* tgfTrafficGeneratorPortTxEthTransmit checks if packets were transmitted.
   and function prints error message if packets were not transmitted by port.
   It's may be OK for some case where traffic from CPU is dropped.
   test may set tgfPrvIntendedError to be GT_TRUE to avoid redundant messages.
   test must restore tgfPrvIntendedError to be GT_FALSE also.
   */
