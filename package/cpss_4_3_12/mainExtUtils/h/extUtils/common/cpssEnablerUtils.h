...
#ifdef CHX_FAMILY
/* check that port has valid mapping or not CPU and not Loopback port */
#define CPSS_ENABLER_PORT_SKIP_CHECK(dev,port)                                                   \
    if(PRV_CPSS_DXCH_PP_HW_INFO_PORT_MAP_CHECK_SUPPORTED_MAC(dev) == GT_TRUE)                    \
    {                                                                                            \
        GT_BOOL   _isValid, _noMac/*isCpu or Loopback ...*/;                                     \
        GT_STATUS _rc, _rc1;                                                                     \
        if ((port) >= PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(dev))                                \
            break; /* no more */                                                                 \
        _rc = cpssDxChPortPhysicalPortMapWithoutMacGet(dev, port, &_noMac);                      \
        _rc1 = cpssDxChPortPhysicalPortMapIsValidGet(dev, port, &_isValid);                      \
        if((_rc != GT_OK) || (_noMac == GT_TRUE) ||(_rc1 != GT_OK) || (_isValid != GT_TRUE ) )   \
            continue;                                                                            \
    }                                                                                            \
    else                                                                                         \
    {                                                                                            \
