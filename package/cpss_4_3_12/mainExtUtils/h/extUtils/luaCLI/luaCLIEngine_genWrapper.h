...
#define F_NUMBER_LONG_N(structPtr, Lindex, field, fieldname, fieldtype) \
    lua_getfield(L, Lindex, #fieldname); \
    ...
    { \
        if (strncmp("0x",lua_tostring(L, -1),2) == 0) \
            ...
            (structPtr)->field = (fieldtype)(strtoul(lua_tostring(L, -1),NULL,10)); \
    } \
    else \
    { \
...
#define F_NUMBER_LONG(structPtr, Lindex, field, fieldtype) \
    F_NUMBER_LONG_N(structPtr, Lindex, field, field, fieldtype)
...
#define F_ARRAY_CHAR(structPtr, field, index, fieldtype) \
    lua_rawgeti(L, -1, index); \
    if (lua_type(L, -1) == LUA_TSTRING) \
    { \
        (structPtr)->field[index] = (fieldtype)((int)(*lua_tostring(L, -1))); \
    } \
    else \
    { \
...
#define FO_NUMBER_LONG_N(structPtr, Lindex, field, fieldname, fieldtype) \
    char buffer[21]; \
    sprintf (buffer, "%llu", (structPtr)->field); \
    lua_pushstring(L, buffer); \
    lua_setfield(L, Lindex, #fieldname);

#define FO_NUMBER_LONG(structPtr, Lindex, field, fieldtype) \
    FO_NUMBER_LONG_N(structPtr, Lindex, field, field, fieldtype)
...
#define FO_ARRAY_CHAR(structPtr, field, index, fieldtype) \
    lua_pushlstring(L, (const char*)(&(structPtr)->field[index]),1); \
    lua_rawseti(L, -2, index);
...
/* UNION MEMBER ARRAY NUMBER*/
#define UNION_MEMBER_ARRAY_NUMBER(typename,member,membertype,params) \
PRV_UNION_LUA_TO_C_START(typename, member) \
...
/* UNION MEMBER ARRAY CHAR*/
#define UNION_MEMBER_ARRAY_CHAR(typename,member,membertype,params) \
PRV_UNION_LUA_TO_C_START(typename, member) \
    F_ARRAY_START(val, -1, member); \
    { \
        int idx; \
        for (idx = 0; idx < params; idx++) { \
            F_ARRAY_CHAR(val, member, idx, membertype); \
        } \
    } \
    F_ARRAY_END(val, -1, temp); \
PRV_UNION_LUA_TO_C_END() \
...
PRV_UNION_C_TO_LUA_START(typename,member) \
    ...
    { \
        int idx; \
        for (idx = 0; idx < params; idx++) { \
            FO_ARRAY_CHAR(val, member, idx, membertype); \
        } \
    } \
    FO_ARRAY_END(val, t, temp); \
	PRV_UNION_C_TO_LUA_END() \
