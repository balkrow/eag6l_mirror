...
#ifdef CMD_LUA_CLI
extern GT_U32   example_run_in_limited_physical_ports;
#else
static GT_U32   example_run_in_limited_physical_ports = 0;
#endif
    ...
    if (CPSS_IS_DXCH_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily))
    {
        if (PRV_CPSS_SIP_5_CHECK_MAC(devNum))
        {
            if(example_run_in_limited_physical_ports &&
                example_run_in_limited_physical_ports < PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(devNum) &&
                portNum >= example_run_in_limited_physical_ports)
            {
                /* not checked for physical , will be checked only as eport range */
            }
            else
