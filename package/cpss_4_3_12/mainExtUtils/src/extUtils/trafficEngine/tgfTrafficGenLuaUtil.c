...
#define PRINT_VLAN_TAG(col1, col2, tpid, tci)       \
    cpssOsPrintf("%10s : %-15s: 0x%04x:0x%04x\n", col1, col2, tpid, tci)

#define PRINT_ARR_MAC(col1, col2, arr, maxIndex)    \
{                                                   \
    GT_U32 n;                                       \
    cpssOsPrintf("%10s : %-15s: ", col1, col2);           \
    for (n=0; n<maxIndex-1; n++)                    \
    {                                               \
        cpssOsPrintf("0x%02x:",arr[n]);                   \
    }                                               \
    cpssOsPrintf("%02x\n",arr[n]);                        \
}

#define PRINT_PORT_MAC(col1, col2, port)            \
    cpssOsPrintf("%10s : %-15s: 0x%04x\n", col1, col2, port)
...
)
{
    ...
    if (GT_OK != rc)
    {
        PRV_TGF_LOG1_MAC("[TGF]: tgfTrafficGeneratorCpuDeviceSet FAILED, rc = [%d]", rc);
...
)
{
    ...
    if (GT_OK != rc)
    {
        PRV_TGF_LOG2_MAC("[TGF]: tgfTrafficGeneratorPortLoopbackModeEnableSet FAILED, rc = [%d] , port[%d]", rc , portNum);
    ...
    if (GT_OK != rc)
    {
        PRV_TGF_LOG1_MAC("[TGF]: tgfTrafficGeneratorPortTxEthTransmit FAILED, rc = [%d]", rc);
    }
...
extern void debugForcePrint(IN GT_U32 forcePrint);
/**
...
)
{
    ...
    if((pktCnt <= 1) || (streamType == TGF_TRAFFIC_GEN_STREAM_TYPE_INPUT_PKT_E))
    {
        ...
        rc = prvTgfSetTxSetupEth(devNum, packetInfoPtr, pktCnt, 0, NULL);
        TGF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "ERROR of SetTxSetup:");

        /* Send Packet */
        rc = prvTgfStartTransmitingEth(devNum, portId);
        TGF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "ERROR of StartTransmitting: %d, %d",
                devNum, portId);
    }
    ...
    else
    {
         ...
         */
#if !defined(SHARED_MEMORY) && !defined(PX_FAMILY)
...
)
{
    ...
    prvTgfTrafficGenSendPkt(params->pktCnt, params->streamType, params->devNum, params->port, &packet);
