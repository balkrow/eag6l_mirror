...
typedef enum {
    ...
    PARAM_TYPE_STRING_PTR_E, /* OUT GT_STRING* */
...
)
{
    ...
    if (p->typeIO == PARAM_DIRECTION_OUT_E &&
            (string_equal_to(L, "GT_STRING")))
    {
        /* special case: 'OUT GT_STRING *' will be passed as 'pointer to char*' */
        p->pType = PARAM_TYPE_STRING_PTR_E;
    ...
    if (string_equal_to(L, "GT_U16") || string_equal_to(L, "GT_U8") || string_equal_to(L, "GT_U64_BIT") )
    {
        ...
        if (p->typeIO != PARAM_DIRECTION_OUT_E)
        {
                ...
                ((lua_type(L, -1) == LUA_TSTRING) && (p->pType == PARAM_TYPE_VAL_U64_E)))
            {
                ...
                {
                    /* due to LUA 5.1 limitation, number is represented as double, so we can't store the whole 64 bit,
                    our WA is to save the number as a string */
                    if (strncmp("0x",lua_tostring(L, -1),2) == 0)
                        p->value = (GT_U64_BIT)strtoul(lua_tostring(L, -1),NULL,16);
                    else
                        p->value = (GT_U64_BIT)strtoul(lua_tostring(L, -1),NULL,10);
                }
    ...
    for (k = 0; k < nparams; k++)
    {
        ...
        switch (p->pType)
        {
            ...
            case PARAM_TYPE_ARR_PTR_E: /* Array of pointers */
                p->param = p->value;
                break;
            case PARAM_TYPE_STRING_PTR_E: /* OUT GT_STRING* */
                /* we just need the parameter to be 'pointer' ,
                   so it is pointing to p->value
                */
                p->param = (GT_UINTPTR)(&(p->value));
                break;
    ...
    for (k = 0; k < nparams; k++)
    {
        ...
        switch (p->pType)
        {
            ...
            case PARAM_TYPE_PTR_U8_E:  /* OUT GT_U8* */
                ...
                break;
            case PARAM_TYPE_PTR_U64_E:  /* OUT GT_U64 */
                sprintf (buffer, "%llu", *((GT_U64_BIT*)valPtr));
            ...
            case PARAM_TYPE_STRING_PTR_E: /* OUT GT_STRING* */
                /* valPtr is the pointer to the value p->value ,
                which is the same as p->param
                meaning p->param == valPtr
                cast one of them to 'GT_STRING*' and push the '*' (content) to the
                LUA stack
                */
                lua_pushstring(L, *((GT_STRING*)valPtr));
                break;
            default:
                lua_pushnil(L);
...
char* itoa_func(int inNumbr, char* outString, int base)
{
    ...
    {
        mod = inNumbr % base;
        outString[i++] = (char)((mod >= 10) ? (mod - 10) + 'a' : mod + '0');
    }
...
/*************************************************************************
* mgmType_to_lua_GT_STRING
*
* Description:
*       push GT_STRING to Lua
*
* Parameters:
*  L
*
...
*
*************************************************************************/
static int mgmType_to_lua_GT_STRING(
    lua_State *L
)
{
    if (lua_isnil(L, 1))
...
*
*
* @retval 0                        - no returned parameters added no lua stack
*/
int luaEngineGenWrapper_init(lua_State *L)
{
    ...
    lua_register(L, "mgmType_to_c_GT_FLOAT64", mgmType_to_c_int);
    ...
    lua_register(L, "mgmType_to_lua_GT_FLOAT64", mgmType_to_lua_int);
    ...
    /*  NOTE: the convert for mgmType_to_c_GT_STRING is not needed (not called from LUA) */
    lua_register(L, "mgmType_to_lua_GT_STRING", mgmType_to_lua_GT_STRING);
