...
static int luaCLI_readkey(IOStreamPTR IOStream, int timeout)
{
    ...
    int ret, k, matches;
    GT_U32 self_tid = 0;
    GT_BOOL waitForEver = 0;


    osTaskGetSelf(&self_tid);
    if (timeout == -1)
    {
        waitForEver = 1;
        timeout = MV_DEFAULT_TIMEOUT;
    }
    ...
    while (IOStream->connected(IOStream))
    {
        ...
        if (ret < 0 || osTaskGracefulGetTerminationReq(self_tid))
            return -1;

        if (ret == 0 && waitForEver)
            continue;
...
)
{
    ...
    if (memMgr == NULL)
    {
        memMgr = (LUA_CLI_MALLOC_STC*) LUACLI_MALLOC(sizeof(*memMgr));
        if(NULL == memMgr)
        {
          return GT_FAIL;
        }
...
)
{
    ...
    {
        while(GT_TRUE) {
            if (osTaskGracefulGetTerminationReq(self_tid) == GT_TRUE)
...
static GT_VOID luaTracePrintHwData
(
    IN GT_U32                   addr,
    IN GT_U32                   length,
    IN GT_U32                   *dataPtr,
...
)
{
    ...
    osPrintf("ADDR  0x%08X", addr);
    osPrintf("  DATA  ");

    for (ii = 0; ii < length; ii++)
    {
        osPrintf("0x%08X", dataPtr[ii]);
        osPrintf(" ");
    ...
    {
        osPrintf(" MASK  0x%08X", mask);
        osPrintf("  ");
    }

}

static GT_VOID luaTraceHwAccessPrint
(
    IN GT_U8       devNum,
...
static GT_STATUS luaTraceReadHwAccess
(
    IN GT_U8       devNum,
    IN GT_U32      portGroupId,
    IN GT_BOOL     isrContext,
...
)
{
    ...
    osPrintf("RD");
    luaTraceHwAccessPrint(devNum,portGroupId,isrContext,addrSpace);
    luaTracePrintHwData(addr,length,dataPtr,0xFFFFFFFF);
    osPrintf("\n");
...
static GT_STATUS luaTraceDelayHwAccess
(
       IN GT_U8       devNum,
       IN GT_U32      portGroupId,
       IN GT_U32      millisec
)
{
    ...
    osPrintf("%d", portGroupId);
    osPrintf(" ");
    osPrintf("\t\t\t\t%8d",millisec);

    return GT_OK;
}


static GT_STATUS luaTraceWriteHwAccess
(
    IN GT_U8       devNum,
    IN GT_U32      portGroupId,
    IN GT_BOOL     isrContext,
...
)
{
    ...
    osPrintf("WR");
    luaTraceHwAccessPrint(devNum,portGroupId,isrContext,addrSpace);
    luaTracePrintHwData(addr,length,dataPtr,mask);
    osPrintf("\n");

    return GT_OK;
}

static GT_STATUS luaGetDefaultTraceFuncs(
...
)
{
    ...
    traceFuncBindInfoPtr->traceHwBindInfo.traceHwAccessReadFunc  = luaTraceReadHwAccess;
    traceFuncBindInfoPtr->traceHwBindInfo.traceHwAccessWriteFunc = luaTraceWriteHwAccess;
    traceFuncBindInfoPtr->traceHwBindInfo.traceHwAccessDelayFunc = luaTraceDelayHwAccess;
...
/**
* @internal luaStandalone_configure_dma_per_devNum function
* @endinternal
*
...
GT_STATUS   luaStandalone_configure_dma_per_devNum
(
...
)
{
    ...
    for(i=0;i<numberOfDevices;i++)
    {
      ...
      rc = prvCpssDeviceNumberByHwInfoGet(hwInfoPtr+i,&devNum);
      if(rc != GT_OK)
      {
...
)
{
    ...
    if((GT_TRUE == cpssAslrSupport)&&(GT_FALSE == prvCpssGlobalDbExistGet()))
    {
        ...
        osFuncBind.osLogBindInfo.osLogFunc = luaStandaloneOsLog;
        ...
        rc = luaGetDefaultTraceFuncs(&traceFuncBindInfo);
        if(rc != GT_OK)
        {
            return rc;
        }
        ...
        /*loop over all existing devices and get hw info*/
        rc = cpssGlobalDbInit(GT_TRUE,GT_FALSE,/*insert here pointer to hwInfo*/
            osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList,
        ...
        /*DMA map*/
#if defined(CONFIG_V2_DMA)&&!defined ASIC_SIMULATION
        rc = luaStandalone_configure_dma_per_devNum(
            osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList,
            osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum);
...
#ifdef INCLUDE_MPD
/*link stub*/
void cpss_prvMpdMtdDebugRegDump()
{
}
#endif
#endif
