...
CPSS_PORTS_BMP_STC appRefTodoTuneBmp[CPSS_APP_PLATFORM_MAX_PP_CNS]; /* bitmap of ports to run tuning*/
...
static GT_BOOL  changeToDownHappend[CPSS_APP_PLATFORM_MAX_PP_CNS][CPSS_MAX_PORTS_NUM_CNS] = {{ 0 }};
static GT_BOOL  changeToUpHappend[CPSS_APP_PLATFORM_MAX_PP_CNS][CPSS_MAX_PORTS_NUM_CNS] = {{ 0 }};
...
)
{
    ...
    while(1)
    {
        ...
        for(portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
        {
            ...
            if(0 == CPSS_PORTS_BMP_IS_PORT_SET_MAC(&appRefTodoTuneBmp[devNum], portNum))
            {
                continue;
            }
            ...
            {
                CPSS_PORTS_BMP_PORT_CLEAR_MAC(&appRefTodoTuneBmp[devNum], portNum);
                rc = cpssDxChPortEnableSet(devNum, portNum, GT_TRUE);
                if(rc != GT_OK)
                {
...
)
{
    ...
    if(trxTraining || rxTraining)
    {
        if(serdesTuneSmId[devNum] == 0)
        {
            ...
            CPSS_PORTS_BMP_PORT_CLEAR_ALL_MAC(&appRefTodoTuneBmp[devNum]);
