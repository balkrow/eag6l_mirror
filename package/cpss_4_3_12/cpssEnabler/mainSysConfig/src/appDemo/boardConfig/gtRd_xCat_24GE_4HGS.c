...
)
{
    ...
    if ( (appDemoPpConfigList[devIdx].pciInfo.pciBusNum == 0xFF) && /* MBUS driver identified internal CPU */
         (appDemoPpConfigList[devIdx].devFamily == CPSS_PP_FAMILY_DXCH_AC5_E) )
    {
        cpssOsPrintf("AC5 internal CPU - found by MBUS driver\n");
...
)
{
    ...
    for(ii = SYSTEM_DEV_NUM_MAC(0); ii < SYSTEM_DEV_NUM_MAC(ppCounter); ++ii,tmpIndex++)
    {
        if(appDemoPpConfigList[ii].devFamily != CPSS_PP_FAMILY_DXCH_AC5_E)
        {
            ...
            rc = appDemo_configure_dma_per_devNum(CAST_SW_DEVNUM(dev));
        ...
        {
            rc = cpssDxChHwMppSelectSet(dev, mppListArr[jj][0], mppListArr[jj][1]);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    else
    {
        ...
        if((xcat3BoardType != APP_DEMO_AC3_BOARD_INVALID_CNS) && (!xcat3BoardResetDone))
        {
            ...
            */
            rc = prvCpssHwPpGetRegField(dev, 0x7C, 0, 8, &regValue);
...
/**
* @internal getCascadeDeviceIndex function
* @endinternal
*
* @brief   get device index to use in xcat3CscdPortDb
*
* @param[in] devNum                   - Device number.
*
* @retval Cascade device index.
*/
static GT_U8 getCascadeDeviceIndex
(
    IN  GT_U8                    devNum
)
{
...
)
{
    ...
    devIdx = getCascadeDeviceIndex(devNum);

    /* Klocwork fix */
    if (devIdx > 1)
    {
        DBG_TRACE(("xcat3CascadePortConfig devNum %d devIdx %d\n", devNum, devIdx));
        return GT_BAD_PARAM;
    }

    if (xcat3CscdPortDb[devIdx][0].portNum == portNum)
    {
        portIndex = 0;
    }
    else if (xcat3CscdPortDb[devIdx][1].portNum == portNum)
    {
        portIndex = 1;
    }
    else
    ...
    {
        rc = cpssDxChPortExtendedModeEnableSet(devNum, portNum, xcat3CscdPortDb[devIdx][portIndex].isExtendedMac);
    ...
    rc = cpssDxChPortModeSpeedSet(
        devNum, &initPortsBmp, GT_TRUE, xcat3CscdPortDb[devIdx][portIndex].ifMode, xcat3CscdPortDb[devIdx][portIndex].speed);
    ...
    rc = prvCpssDxChPortIfModeSerdesNumGet(
        devNum, portNum, xcat3CscdPortDb[devIdx][portIndex].ifMode, &startSerdes, &serdesNum);
    if(GT_OK != rc)
    {
        return rc;
    }
    ...
    {
        if((xcat3CscdPortDb[devIdx][portIndex].rxLanesPolarity[i] != GT_FALSE) ||
           (xcat3CscdPortDb[devIdx][portIndex].txLanesPolarity[i] != GT_FALSE))
        {
            rc = cpssDxChPortSerdesPolaritySet(devNum,
                                               portNum,
                                               (1 << i),
                                               xcat3CscdPortDb[devIdx][portIndex].txLanesPolarity[i],
                                               xcat3CscdPortDb[devIdx][portIndex].rxLanesPolarity[i]);
            if(GT_OK != rc)
            {
                return rc;
            }
...
/**
* @internal cascadePortRxTrainConfig function
* @endinternal
*
* @brief   Additional configuration for cascading ports
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS cascadePortRxTrainConfig
(
    IN GT_U8                    devNum,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    /* Need to run RX train on BaseR port mode */
    rc = cpssDxChPortSerdesAutoTune(devNum, portNum, CPSS_DXCH_PORT_SERDES_AUTO_TUNE_MODE_RX_TRAINING_E);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortSerdesAutoTune", rc);
    if(rc != GT_OK)
    {
        return rc;
    }
    ...
    for(i = 0; i < 100; i++)
    {
        ...
        CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortSerdesAutoTuneStatusGet", rc);
        if(rc != GT_OK)
        {
            return rc;
        }

        if(rxTuneStatus == CPSS_DXCH_PORT_SERDES_AUTO_TUNE_PASS_E)
        {
    ...
    rc = cpssDxChPortSerdesAutoTune(devNum, portNum, CPSS_DXCH_PORT_SERDES_AUTO_TUNE_MODE_RX_TRAINING_STOP_E);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortSerdesAutoTune", rc);
    if(rc != GT_OK)
    {
        return rc;
    ...
    if(i == 100)
    {
        cpssOsPrintf("cascadePortRxTrainConfig: TIMEOUT - RX Training dev[%d],port[%d],status[%d] \n", devNum, portNum, rxTuneStatus);
        return GT_TIMEOUT;
    }
    else if(rxTuneStatus != CPSS_DXCH_PORT_SERDES_AUTO_TUNE_PASS_E)
    {
        cpssOsPrintf("cascadePortRxTrainConfig: RX Training Failed, dev[%d],port[%d],status[%d] \n", devNum, portNum, rxTuneStatus);
        return GT_BAD_STATE;
    }
    else
    {
...
)
{
    ...
    GT_U8       devIndex;   /* device index */

    devIndex = getCascadeDeviceIndex(devNum);
    ...
    for (port = 0; port < 24;port++)
    {
        ...
        retVal = appDemoSetPortPhyAddr(devNum, port, phyAddr1680_binding[devIndex][port]);
...
)
{
    ...
    GT_U32                          cscdDbIdx;           /* device index in DB xcat3CscdPortDb */
    ...
    for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
    {
        ...
        if(xcat3BoardType == APP_DEMO_AC5_BOARD_RD_B2B_CNS)
        {
            appDemoPpConfigList[i].internalCscdPortRxTrainConfigFuncPtr = cascadePortRxTrainConfig;
        }
    ...
    {/* ppCounter == 2 */
        for(i = SYSTEM_DEV_NUM_MAC(0); i < SYSTEM_DEV_NUM_MAC(ppCounter); ++i)
        {
            /* Cascading */
            devNum = appDemoPpConfigList[i].devNum;
            /* index of device in xcat3CscdPortDb */
            cscdDbIdx = getCascadeDeviceIndex(devNum);
            ...
            */
            ...
            /* 0 - CPU enabled */
            /* 1 - CPU disabled */
            cpuDisabled = i - SYSTEM_DEV_NUM_MAC(0);

            if (cpuDisabled == 1)
            {
                /* device CPSS_98DX3336_CNS with CPU disabled */
                appDemoPpConfigList[i].cscdPortsArr[0].portNum = 27;
                xcat3CscdPortDb[cscdDbIdx][0].portNum = appDemoPpConfigList[i].cscdPortsArr[0].portNum;
                xcat3CscdPortDb[cscdDbIdx][0].ifMode = CPSS_PORT_INTERFACE_MODE_KR2_E;
                xcat3CscdPortDb[cscdDbIdx][0].isExtendedMac = GT_TRUE;
                xcat3CscdPortDb[cscdDbIdx][0].lanesNum = 2;
                xcat3CscdPortDb[cscdDbIdx][0].speed = CPSS_PORT_SPEED_20000_E;
                xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[1] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[1] = GT_FALSE;

                if(xcat3BoardType == APP_DEMO_AC5_BOARD_RD_B2B_CNS)
                {/*PP[1]*/
                    /* port 27 */
                    appDemoPpConfigList[i].cscdPortsArr[0].portNum = 27;
                    xcat3CscdPortDb[cscdDbIdx][0].ifMode = CPSS_PORT_INTERFACE_MODE_XHGS_SR_E;
                    xcat3CscdPortDb[cscdDbIdx][0].isExtendedMac = GT_TRUE;
                    xcat3CscdPortDb[cscdDbIdx][0].speed = CPSS_PORT_SPEED_22000_E;
                    /* lave 10 */
                    xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[0] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[0] = GT_FALSE;
                    /* lave 11 */
                    ...
                    xcat3CscdPortDb[cscdDbIdx][1].lanesNum = 2;
                    xcat3CscdPortDb[cscdDbIdx][1].ifMode = CPSS_PORT_INTERFACE_MODE_XHGS_SR_E;
                    xcat3CscdPortDb[cscdDbIdx][1].isExtendedMac = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][1].speed = CPSS_PORT_SPEED_22000_E;
                    /* lave 8 */
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[0] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[0] = GT_FALSE;
                    /* lave 9 */
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[1] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[1] = GT_FALSE;
                ...
                if(xcat3BoardType == APP_DEMO_AC3_BOARD_RD_MTL_TYPE1_CNS)
                {
                    ...
                    xcat3CscdPortDb[cscdDbIdx][1].lanesNum = 2;
                    xcat3CscdPortDb[cscdDbIdx][1].speed = CPSS_PORT_SPEED_20000_E;
                    xcat3CscdPortDb[cscdDbIdx][1].ifMode = CPSS_PORT_INTERFACE_MODE_KR2_E;
                ...
                else if (xcat3BoardType == APP_DEMO_AC3_BOARD_RD_MTL_TYPE2_CNS)
                {
                    ...
                    xcat3CscdPortDb[cscdDbIdx][1].lanesNum = 1;
                    xcat3CscdPortDb[cscdDbIdx][1].speed = CPSS_PORT_SPEED_10000_E;
                    xcat3CscdPortDb[cscdDbIdx][1].ifMode = CPSS_PORT_INTERFACE_MODE_KR_E;
                ...
                xcat3CscdPortDb[cscdDbIdx][1].portNum = appDemoPpConfigList[i].cscdPortsArr[1].portNum;
                xcat3CscdPortDb[cscdDbIdx][1].isExtendedMac = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[1] = GT_FALSE;
            ...
            else
            {
               /* device CPSS_98DX3336_CNS with CPU enabled */
                ...
                xcat3CscdPortDb[cscdDbIdx][0].portNum = appDemoPpConfigList[i].cscdPortsArr[0].portNum;
                xcat3CscdPortDb[cscdDbIdx][0].ifMode = CPSS_PORT_INTERFACE_MODE_KR2_E;
                xcat3CscdPortDb[cscdDbIdx][0].isExtendedMac = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].lanesNum = 2;
                xcat3CscdPortDb[cscdDbIdx][0].speed = CPSS_PORT_SPEED_20000_E;
                xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[0] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[1] = GT_FALSE;
                xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[1] = GT_TRUE;
                ...
                if(xcat3BoardType == APP_DEMO_AC5_BOARD_RD_B2B_CNS)
                {
                    ...
                    xcat3CscdPortDb[cscdDbIdx][0].speed = CPSS_PORT_SPEED_22000_E;
                    /*lane 6*/
                    xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[0] = GT_TRUE;
                    xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[0] = GT_FALSE;
                    /*lane 7*/
                    xcat3CscdPortDb[cscdDbIdx][0].rxLanesPolarity[1] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][0].txLanesPolarity[1] = GT_FALSE;
                    ...
                    xcat3CscdPortDb[cscdDbIdx][1].speed = CPSS_PORT_SPEED_22000_E;
                    /* lane 8 */
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[0] = GT_TRUE;
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[0] = GT_TRUE;
                    /* lane 9 */
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[1] = GT_TRUE;
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[1] = GT_FALSE;
                }
                else
                if(xcat3BoardType == APP_DEMO_AC3_BOARD_RD_MTL_TYPE1_CNS)
                {
                    ...
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[0] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[1] = GT_FALSE;
                    xcat3CscdPortDb[cscdDbIdx][1].txLanesPolarity[1] = GT_TRUE;
                }
                else if (xcat3BoardType == APP_DEMO_AC3_BOARD_RD_MTL_TYPE2_CNS)
                {
                    /* device CPSS_98DX3336_CNS with CPU enabled */
                    appDemoPpConfigList[i].cscdPortsArr[1].portNum = 27;
                    xcat3CscdPortDb[cscdDbIdx][1].isExtendedMac = GT_TRUE;
                    xcat3CscdPortDb[cscdDbIdx][1].lanesNum = 1;
                    xcat3CscdPortDb[cscdDbIdx][1].speed = CPSS_PORT_SPEED_10000_E;
                    xcat3CscdPortDb[cscdDbIdx][1].ifMode = CPSS_PORT_INTERFACE_MODE_KR_E;
                    xcat3CscdPortDb[cscdDbIdx][1].rxLanesPolarity[0] = GT_TRUE;
            ...
            appDemoPpConfigList[i].cscdPortsArr[0].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_2_WORDS_E;
            appDemoPpConfigList[i].cscdPortsArr[0].trunkId = (GT_TRUNK_ID)trunkNum;

            appDemoPpConfigList[i].cscdPortsArr[1].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_2_WORDS_E;
...
)
{
    ...
    if ((boardRevId == 4) || (boardRevId == 6))
    {
        ...
        localPpCfgParams.routingMode = APP_DEMO_CPSS_DXCH_POLICY_BASED_ROUTING_ONLY_E;
...
)
{
            ...
            xcat3BoardType == APP_DEMO_AC5_BOARD_RD_B2B_CNS)
    {
        ...
        /*device with CPU*/
        if (dev == appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(0)].devNum)
        {
    ...
    else
    {
        ...
        for (portNum = 0 ; portNum < 28;)
        {
            ...
            {
                if (appDemoPpConfigList[dev].devFamily != CPSS_PP_FAMILY_DXCH_AC5_E)
                {
