...
#ifdef DXCH_CODE
extern GT_STATUS appDemoSystemRenumber
(
    IN GT_U8 oldDevNum,
    IN GT_U8 newDevNum
);

extern GT_STATUS extDrvMapDmaRenumber
(
    IN GT_U8 oldDevNum,
    IN GT_U8 newDevNum
);
...
GT_STATUS appDemoPortMgrfuncInit(GT_U8  devNum)
{
...
/* the devices that use polling and therefore NOT use the signal */
#define  PORT_MANAGER_DEVICE_NEED_POLLING(_devNum) \
...
/* the devices that uses the signal */
#define PORT_MANAGER_DEVICE_USE_SIGNAL_INTERRUPT(_devNum)                \
    ((PORT_MANAGER_DEVICE_NEED_POLLING(_devNum)) ? 0 : 1)

static CPSS_OS_SIG_SEM     portInitSeqSignalDetSem[PRV_CPSS_MAX_PP_DEVICES_CNS] = {0};


/*****************************************************************
                         Forward declarations
*****************************************************************/
static unsigned __TASKCONV appDemoPortInitSeqSignalDetectedStage
(
        GT_VOID * param
);
...
)
{
    ...
    {
        GT_STATUS rc;
        if (devNum >= PRV_CPSS_MAX_PP_DEVICES_CNS || devNum < SYSTEM_DEV_NUM_MAC(0))
        {
            return GT_BAD_PARAM;
        }
        if(PORT_MANAGER_DEVICE_USE_SIGNAL_INTERRUPT(devNum))
        {
        ...
        cpssOsSnprintf(name, sizeof(name), "portMngrTask_%d", devNum);
        rc = cpssOsTaskCreate(name,                                                                /* Task Name      */
                              ...
                              &appDemoPpConfigList[SYSTEM_DEV_IDX_MAC(devNum)].portManagerTaskId); /* task ID        */
        if (rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling cpssOsTaskCreate returned=%d", rc);
            return rc;
        }
        rc = osTaskEnableJoin(appDemoPpConfigList[SYSTEM_DEV_IDX_MAC(devNum)].portManagerTaskId);
        if(rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling osTaskEnableJoin returned %d", GT_FAIL);
            return rc;
        }
    }
...
)
{
    ...
    {
        GT_STATUS rc;
        GT_U32    isPmEnabled = appDemoPortManagerTaskEnable;

        appDemoPortManagerTaskEnableSet(0);

        /* Delete the old task */
        rc = cpssOsTaskDelete(appDemoPpConfigList[SYSTEM_DEV_IDX_MAC(oldDevNum)].portManagerTaskId);
        if (rc != GT_OK)
        {
        ...
        if(PORT_MANAGER_DEVICE_USE_SIGNAL_INTERRUPT(newDevNum))
        {
            /* If handler was not made yet and we are in multi-threaded mode */
        ...
        rc = appDemoSystemRenumber(oldDevNum, newDevNum);
        if (rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling appDemoSystemRenumber returned=%d\n", rc);
            return rc;
        }

        rc = extDrvMapDmaRenumber(oldDevNum, newDevNum);
        if (rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling extDrvMapDmaRenumber returned=%d\n", rc);
            return rc;
        }

        /* create new task with new devNum */
        cpssOsSnprintf(name, sizeof(name), "portMngrTask_%d", newDevNum);
        rc = cpssOsTaskCreate(name,                                                                   /* Task Name      */
                              500,                                                                    /* Task Priority  */
                              _64K,                                                                   /* Stack Size     */
                              appDemoPortInitSeqSignalDetectedStage,                                  /* Starting Point */
                              (GT_VOID*)((GT_UINTPTR)newDevNum),                                      /* Arguments list */
                              &appDemoPpConfigList[SYSTEM_DEV_IDX_MAC(newDevNum)].portManagerTaskId); /* task ID        */
        if (rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling cpssOsTaskCreate returned=%d", rc);
            return rc;
        }
        rc = osTaskEnableJoin(appDemoPpConfigList[SYSTEM_DEV_IDX_MAC(newDevNum)].portManagerTaskId);
        if(rc != GT_OK)
        {
            APP_DEMO_LOG_ERROR("calling osTaskEnableJoin returned %d", GT_FAIL);
            return rc;
        ...
        appDemoPortManagerTaskEnableSet(isPmEnabled);
    }
#else
...
)
{
    ...
    ***********************************************************/
    ...
    {
        APP_DEMO_LOG("port is in HW failure state so abort");
        return GT_OK;
    }
        ...
        (CPSS_PORT_MANAGER_EVENT_PORT_AP_PARALLEL_DETECT_E != portEventStc.portEvent))
    {
        ...
        if (appDemoPortManagerMultiThreadedUsage)
        {
            ...
            if(PORT_MANAGER_DEVICE_USE_SIGNAL_INTERRUPT(devNum))
            {
                ...
                if(rc != GT_OK)
                {
                    APP_DEMO_LOG_ERROR("calling cpssOsSigSemSignal returned=%d, portNum=%d/%d", rc, devNum, portNum);
                    return rc;
                }
            }
        }
        else /* currently not supported */
        {
            ...
            if(rc != GT_OK)
            {
                APP_DEMO_LOG_ERROR("calling appDemoPortInitSequentialInit returned=%d, portNum=%d/%d", rc, devNum, portNum);
                return rc;
            }
        }
    }
...
/**
* @internal appDemoPortInitSeqSignalDetectedStage_GracefulExitLastOperation_CB_FUNC function
* @endinternal
*
* @brief   This routine clean info for the soon to be killed port manager task
*           task 'appDemoPortInitSeqSignalDetectedStage'.
*
*/
static void appDemoPortInitSeqSignalDetectedStage_GracefulExitLastOperation_CB_FUNC
(
    IN void*   param
)
{
    ...
    if(portInitSeqSignalDetSem[devNum])
    {
        cpssOsSigSemDelete(portInitSeqSignalDetSem[devNum]);
    }

    portInitSeqSignalDetSem[devNum] = 0;
...
/*    GT_U32                  remainder = 0;*/
    ...
    /* if you want to debug port manager callback function
       call function appDemoPortMgrfuncInit(devNum); from XXX_g_SimpleInit and not here */
    appDemoPortMgrfuncInit(devNum);

    isSip6 = PRV_CPSS_SIP_6_CHECK_MAC(devNum);

    pollingDevice = PORT_MANAGER_DEVICE_USE_SIGNAL_INTERRUPT(devNum) ? GT_FALSE : GT_TRUE;
    ...
    appDemoDeviceTaskSupportGracefulExit(appDemoPortInitSeqSignalDetectedStage_GracefulExitLastOperation_CB_FUNC,
        param,
        devNum);
    if(isSip6)
    {
    ...
    while(1)
    {
        if(pollingDevice == GT_FALSE)
        {
            while(1)
            {
                if(GT_OK != cpssOsSigSemWait(portInitSeqSignalDetSem[devNum],
                    appDemoPortManagerTaskSleepTime))
                {
                    /* wake every 50 mili to check if the task need to be killed */
                    /* check if task need termination */
                    appDemoTaskCheckIfNeedTermination();
                    /* as we not yet got signal ... continue to wait on the semaphore */
                }
                else
                {
                    /* we got signal , act on it */
                    break;
                }
            }
        }
        ...
        while (portsExistFlag != GT_FALSE)
        {
            ...
            if(pollingDevice == GT_FALSE)
            {
                /* need to go to wait on semaphore if no other port still need treatment */
            ...
            **********************************************************************/
                ...
                if((GT_OK != rc) || (portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_RESET_E))
                {
                   continue;
                }
                ...
                if(pollingDevice == GT_TRUE)
                {
                         ...
                         ((portParams.portParamsType.regPort.portAttributes.validAttrsBitMask & CPSS_PM_PORT_ATTR_PM_OVER_FW_E) && (portParams.portParamsType.regPort.portAttributes.pmOverFw)) )
                    {
                        ...
                        if ((portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_LINK_DOWN_E) &&
                            ((portParams.portParamsType.regPort.portAttributes.validAttrsBitMask & CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E) && (portParams.portParamsType.regPort.portAttributes.extSigDetCallbackEnable)) &&
                            ((portParams.portParamsType.regPort.portAttributes.validAttrsBitMask & CPSS_PM_PORT_ATTR_PM_OVER_FW_E) && (portParams.portParamsType.regPort.portAttributes.pmOverFw)))
                        {
                            if (appDemoPortMgrFuncPtrs.appDemoPrtMgrLowLevelFunc != NULL)
                            {
                    ...
                    if(!loopback)
                    {
                            ...
                            if (appDemoPortMgrFuncPtrs.appDemoPrtMgrLowLevelFunc != NULL)
                            {
                                ...
                                if(rc != GT_OK)
                                {
                                   ...
                                   This is not error but in order to ingore flase return errors, check the status again and if in reset, stop the loop for this port until next create */
                                    ...
                                    if((portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_RESET_E)
                                        || (portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_FORCE_LINK_DOWN_E))
                                    {
                                        continue;
                                    }
                                    APP_DEMO_LOG_ERROR("calling cpssDxChPortSerdesSignalDetectGet returned=%d, portNum=%d/%d", rc, devNum, portNum);
                                }
                                if(signalDetect)
                                {
                                    portEventStc.portEvent = CPSS_PORT_MANAGER_EVENT_LOW_LEVEL_STATUS_CHANGED_E;
                        ...
                        if((!CPSS_PORT_IF_MODE_QSGMII_USX_QUSGMII_CHECK_MAC(portConfigOutParams.ifMode)) &&
                           ((portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_LINK_UP_E) || (portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E)) &&
                           !(APP_DEMO_PORT_MANAGER_ANALOG_TX2RX_LOOPBACK_CHECK(portparams)))
                        {
                            if (isSip6)
                            {
                                ...
                                if(rc != GT_OK)
                                {
                                    ...
                                    {
                                        /* In cases  that the port move to reset state (due to the user delete/force link down) but the application still didn't update with the port change status,
                                           it's will return GT_NOT_INITIALIZED, since prvCpssPortPcsAlignLockStatusGet is not a PM functions it sees this as an error, but it's not for the port manager,
                                           and the state will be corrected in the next application cycle */
                                        continue;
                                    }
                                    else
                                    {
            ...
            {
                /* the device supports polling is about to go to sleep on the semaphore */
                /* so not expected to utilize all the CPU time , if we not sleep here   */
            }
...
)
{
    ...
    *******************************/
    ...
    {
        APP_DEMO_LOG("port is in HW failure state so abort mac_level_change notification");
        return GT_OK;
    }
...
)
{
    ...
    *******************************************/
    ...
    osTaskGracefulCallerRegister(eagleTsenTemperatureGetStartTaskId, 1, NULL, NULL);
    return GT_OK;
}
