...
/**
* @internal kernelSliceTimeSet function
* @endinternal
*
* @brief   This routine enables round-robin selection among tasks of same priority
...
*         scheduling.
* @param[in] ticks                    - time-slice in  or 0 to disable round-robin
*                                       None
*
* @note Round-robin scheduling allows the processor to be shared fairly by all
...
/**
* @internal appDemoLion2TrunkExtServicesBind function
* @endinternal
*
* @brief   Lion2 application (that implements this function) will bind the
...
)
{
    GT_U8       i;                      /* Loop index.                  */
    GT_STATUS   rc;
    GT_U32      usedDevices[APP_DEMO_CPSS_MAX_NUM_PORT_GROUPS_CNS]={0}; /* In Lion2 each portGroup is a device */
    ...
    if(0 == managementBus)
    {
        ...
        devVendorId = appDemoPpConfigList[devIdx].pciInfo.pciDevVendorId;
    }
    else
    {
        /* Scanning devices */
        ...
        osMemCpy(&devVendorId, &appDemoPpConfigList[devIdx].smiInfo.smiDevVendorId,
                 sizeof(devVendorId));
...
/* SMI supports traffic to CPU only by RGMII interface */
    ...
    firstDev = 0;
    /* Updating global 'appDemoPpConfigList' according to data got until here,
       and initlizing local 'usedDevices' with only valid devices indexes. */
    for(i = 0; i < PRV_CPSS_MAX_PP_DEVICES_CNS; i++)
    {
        if(appDemoPpConfigList[i].valid == GT_TRUE)
        {
            ...
            /* As each portGroup in Lion2 is a device, each device not in portGroup is invalidated */
            ...
            /* 'usedDevices' contains valid devices numbers, starting from its first index */
            usedDevices[index++] = i;
    ...
    for(i = 0, index = 0, activeGroupsNum=0; i < appDemoPpConfigList[devIndex0].numOfPortGroups; i++, index++)
    {
        ...
        /* Invalidating inactive port groups */
...
/**
* @internal lion2AppDemoPortsConfig function
* @endinternal
*
* @brief   Init required ports of Lion2 to specified ifMode and speed
*
...
)
{
    ...
    if(rc != GT_OK)
    {
        cpssOsPrintSync("cpssDxChPortSpeedGet(portNum=%d,ifMode=%d):rc=%d\n",
                        port, ifMode, rc);
        return rc;
    }
...
/**
* @internal lionPortInterfaceInitBoardConfigPortsDefaultByProfile function
* @endinternal
*
* @brief   Configs all ports from 0 to max in profile to default value
*
* @param[in] dev                      - device number
...
/**
* @internal lionPortInterfaceInitBoard360MHz_4x10G_3x40G function
* @endinternal
*
* @brief   Init ports for board with clock 360 MHz 7 ports 4 x 10G and 3 x 40G
*
* @param[in] dev                      - device number
*
* @retval GT_OK                    - on success,
...
)
{
    ...
    static appDemoPortConfigSpec_STC localPortList[] =
    {
        ...
        ,{ 4, CPSS_PORT_INTERFACE_MODE_SR_LR4_E, CPSS_PORT_SPEED_40000_E }
        ,{ 8, CPSS_PORT_INTERFACE_MODE_SR_LR4_E, CPSS_PORT_SPEED_40000_E }
        ,{ 9, CPSS_PORT_INTERFACE_MODE_SR_LR4_E, CPSS_PORT_SPEED_40000_E }
    };
...
/**
* @internal lionPortInterfaceInitJustSuperGop0 function
* @endinternal
*
* @brief   by default configure just ports of Super GOP 0
*
* @param[in] dev                      - device number
* @param[in] ifMode                   - interface mode
* @param[in] speed                    - port speed
...
/**
* @internal appDemoHooperRdInit function
* @endinternal
*
* @brief   run specific Hooper RD ports configuration
*
* @param[in] dev                      - device number
*
* @retval GT_OK                    - on success,
...
/**
* @internal lion2AppDemoPortInterfaceInit function
* @endinternal
*
* @brief   Init ports. Should be done after board initialization !
*
* @param[in] dev                      - device number
    ...
    for(ii = 0 ; ii < appDemoPpConfigList[devIdx].numOfPortGroups ;ii++)
    {
        if (CPSS_CHANNEL_PCI_E == appDemoPpConfigList[devIdx].channel)
        {
            rc = extDrvPexConfigure(
                    ...
                    (localPpPh1Config.mngInterfaceType == CPSS_CHANNEL_PEX_KERNEL_E)
                        ? MV_EXT_DRV_CFG_FLAG_DONT_MAP_E
                        : 0,
                    &(localPpPh1Config.hwInfo[ii]));
            if (rc != GT_OK)
                return GT_FAIL;
#ifdef USE_KERN_PP_DRV
            if (localPpPh1Config.mngInterfaceType == CPSS_CHANNEL_PEX_KERNEL_E)
            {
                localPpPh1Config.hwInfo[ii].driver = cpssExtDrvPresteraKernDrvCreate(
                        &(localPpPh1Config.hwInfo[ii]), 0xf);
            }
#endif
        }
        else
        {
            localPpPh1Config.hwInfo[ii].busType = CPSS_HW_INFO_BUS_TYPE_SMI_E;
            localPpPh1Config.hwInfo[ii].hwAddr.devSel =
                appDemoPpConfigList[devIdx].portGroupsInfo[ii].portGroupSmiInfo.smiIdSel;
            /* get interrupt */
        }
    }

    appDemoPpConfigList[devIdx].hwInfo = localPpPh1Config.hwInfo[localPpPh1Config.numOfPortGroups-1];
...
)
{
    ...
    rc = appDemo_configure_dma_per_devNum(devIdx);
...
/**
* @internal appDemoCt10And40ParamsSet function
* @endinternal
*
* @brief   Set cascaded trunk ID and
*         default source ID for network relay and uplink ports.
* @param[in] networkPortSrcId         -  default source ID for network ports.
* @param[in] relayPortSrcId           - default source ID for relay ports.
* @param[in] uplinkPortSrcId          -  default source ID for uplink ports.
* @param[in] trunkId                  - cascaded trunk ID.
*
* @retval GT_OK                    - on success,
* @retval GT_FAIL                  - otherwise.
*/
...
/**
* @internal appDemoLion2Ct10And40TrunkSet function
* @endinternal
*
* @brief   Configure uplink and relay trunks for mixed Cut-Through 40G and 10G
*         configuration.
* @param[in] devNum                   - device number
* @param[in] numOfUplinkTrunks        - number of uplink trunks.
* @param[in] firstUplinkTrunkId       - trunk ID of first uplink trunk.
*
* @retval GT_OK                    - on success,
* @retval GT_FAIL                  - otherwise.
*/
...
/**
* @internal appDemoLion2Ct10And40CscdUplinkTrunkSet function
* @endinternal
*
* @brief   Configure cascade uplink trunk and relay trunks for mixed Cut-Through 40G and 10G
*         configuration.
* @param[in] devNum                   - device number
...
/**
* @internal prvLoopBackCt10And40Config function
* @endinternal
*
* @brief   loopBack configurations.
*
* @param[in] devNum                   -  Device number.
*
* @retval GT_OK                    - on success,
...
)
{
    ...
    for(i = 0; i < PRV_UPLINK_PORTS_NUMBER; i++)
    {
     ...
     rc = cpssDxChTrunkHashCrcParametersSet(devNum, CPSS_DXCH_TRUNK_LBH_CRC_6_MODE_E, 0x15, 0/*crcSeedHash1*/);
     if(rc != GT_OK)
     {
         return rc;
     }
...
/*   maskEntry.localSrcPortMaskBmp = 1; */ /* one byte */
     ...
     for (i = 16; i < 28; i++)
     {
         rc = cpssDxChTrunkHashMaskCrcEntrySet(devNum, 0/*hashIndex*/, i, &maskEntry);
         if(rc != GT_OK)
         {
             return rc;
         }
...
/**
* @internal lion2AppDemoCpuPortPhyConfig function
* @endinternal
*
* @brief   Initiate the Phy (88E1512). It prepares the Phy device to automatically
...
* @retval GT_FAIL                  - otherwise.
*
* @note gigPhyRegWrite 0 0 0 0x16 0 0xff
...
/**
* @internal apRestartOnPort function
* @endinternal
*
* @brief   Cause restart AP on port
*
* @param[in] devNum                   - device number
...
/**
* @internal lion2AppDemoApResolvedPortsConfig function
* @endinternal
*
* @brief   Configure ports of core that get AP resolution and get bitmap of ports
*         where need to run TRX training
* @param[in] devNum                   - device number
* @param[in] portGroupId              - core number
* @param[in,out] trxTrainBmpPtr           - (pointer to) bitmap of ports where need to run TRX
*                                      training
*                                       None.
*/
        ...
        if((apResolvedPortsBmp1>>i) & 1)
        {
            ...
            portNum =
                PRV_CPSS_LOCAL_PORT_TO_GLOBAL_PORT_CONVERT_MAC(devNum,
                                                       portGroupId, i);
#if ((defined _VXWORKS) && (defined CPU_78200)) || ((defined LINUX)&& (!defined ASIC_SIMULATION))
...
/**
* @internal lion2AppDemoApTrxTrainingStart function
* @endinternal
*
* @brief   Start TRX training on defined ports
*
* @param[in] devNum                   - device number
* @param[in] portNum                  - global number of first port in core where AP resolved on any port
* @param[in] trxTrainBmpPtr           - (pointer to) bitmap of ports where need to run TRX
*                                      training
*                                       None.
*/
...
/**
* @internal lion2AppDemoApAlign90StatusCheck function
* @endinternal
*
* @brief   Check if there is still on core port with not complited TRX training
*
* @param[in] devNum                   - device number
*                                      portNum - global number of first port in core where AP resolved on any port
*
* @retval GT_TRUE                  - TRX training not complited of some port of core
* @retval GT_FALSE                 - no more ports running TRX training
*/
...
/**
* @internal intCounterPrint function
* @endinternal
*
* @brief   This routine is the event handler for 802.3ap (Z80) event
*
* @retval GT_OK                    - on success,
* @retval GT_FAIL                  - otherwise.
*
* @note This function is part of the AP appDemo code that can be used as reference code.
...
/**
* @internal lion2AppDemoApInit function
* @endinternal
*
* @brief   Example of init for 802.3ap support.
*
* @param[in] devNum                   - physical device number
*
* @retval GT_OK                    - on success,
...
)
{
    ...
    if (portNum%4 == 0)
    {
        ...
        apParams.modesAdvertiseArr[3].ifMode = CPSS_PORT_INTERFACE_MODE_KR4_E;
    ...
    rc = cpssDxChPortApPortEnableGet(devNum, portNum, &portApEnable);
    if(rc != GT_OK)
    {
...
/**
* @internal gtDbDxLion2BoardCleanDbDuringSystemReset function
* @endinternal
*
...
/**
* @internal cpssDxChPortPizzaArbiterIfTablesDump function
* @endinternal
*
* @brief   Print Pizza Arbiter tables for given device portGroup
*         1. Pizza profile
...
/**
* @internal gtLion2PortPizzaArbiterIfStateDump function
* @endinternal
*
* @brief   Dump Lion 2,3 Pizza Arbiter Registers
*
* @note   APPLICABLE DEVICES:      Lion2.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5.
*
...
/**
* @internal gtLion2PortPizzaArbiterIfRegistersDump function
* @endinternal
*
* @brief   Dump Lion 2,3 Pizza Arbiter Registers
*
...
GT_STATUS appDemoDataIntegrityXsbRegRead
(
    IN GT_U8 devNum,
    IN GT_U32 pipeId,
    IN GT_U32 clientId,
...
)
{
    ...
    rc = prvCpssDfxXsbRegRead(devNum, pipeId, clientId, xsbId, xsbReg, &regData);
    if(GT_OK != rc)
    {
        return rc;
    }
