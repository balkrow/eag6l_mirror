...
)
{
    ...
    /*
     * cpssInitSystem 33,3 - SMI driver
     */
    if(isUsingSmiDriver != GT_TRUE) {
        appDemoPpConfigList[devNum].hwInfo = hwInfoPtr[0];
        appDemoPpConfigList[devNum].pciInfo.pciBusNum = hwInfoPtr[0].hwAddr.busNo;
        appDemoPpConfigList[devNum].pciInfo.pciIdSel = hwInfoPtr[0].hwAddr.devSel;
        appDemoPpConfigList[devNum].pciInfo.funcNo = hwInfoPtr[0].hwAddr.funcNo;

        rc = appDemo_configure_dma_per_devNum(devNum);
...
static PortInitList_STC px_force_Ports_0_3_10G_12_13_25G_PortsInitList[] =
{
     /* GearBox : 4*10G KR */
...
/* allow to test specific ports speed */
GT_STATUS   px_force_Ports_0_3_10G_12_13_25G(void)
{
    px_force_PortsInitListPtr = px_force_Ports_0_3_10G_12_13_25G_PortsInitList;
...
#if defined(CONFIG_V2_DMA)
        osCacheDmaMallocByDevice(devNum, netIfCfgPtr->txDescBlockSize);
#else
...
#if defined(CONFIG_V2_DMA)
        osCacheDmaMallocByDevice(devNum, netIfCfgPtr->rxDescBlockSize);
#else
...
)
{
    ...
    rc = px_initNetIfParamsGet(devNum, &netIfCfg);
...
#if defined(CONFIG_V2_DMA)
    mem.dmaDescBlock = osCacheDmaMallocByDevice(devNum, mem.dmaDescBlockSize);
#else
