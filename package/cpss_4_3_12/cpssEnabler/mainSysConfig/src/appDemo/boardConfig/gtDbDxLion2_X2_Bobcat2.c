...
/**
********************************************************************************
* @file gtDbDxLion2_X2_Bobcat2.c
*
* @brief Initialization functions for the two Lion2 and one Bobcat2 board.
...
#if PRV_CPSS_MAX_PP_DEVICES_CNS >= 128
/* SW devic IDs*/
...
#else /* system not supported with such settings */
#define LION2_0_SW_DEV_NUM 0
#define LION2_1_SW_DEV_NUM 0
#define BOBCAT2_SW_DEV_NUM 1

#endif /* PRV_CPSS_MAX_PP_DEVICES_CNS  >= 128 */
...
)
{
    ...
    if ( (boardRevId == 3) || (boardRevId == 4) )
    {
        ...
        {
            cpssOsPrintf("Found %d PCI devices instead of 3\n", numOfPciDevs);
            return GT_FAIL;
        }
        if ((boardRevId == 4) && (numOfPciDevs != 2))
        {
            cpssOsPrintf("Found %d PCI devices instead of 2\n", numOfPciDevs);
            return GT_FAIL;
        }

        if (numOfPciDevs > 3)
        {
            cpssOsPrintf("Found more than 3 supported PCI devices %d\n", numOfPciDevs);
            return GT_FAIL;
        }
...
)
{
    ...
    if (BOBCAT2_DEV_IND != devIdx)
    {
        /* LION2_0_DEV_IND or LION2_1_DEV_IND */
    ...
    /* Interrupt line overriden by predefinitions */
    {
        ...
        {
            localPpPh1Config.hwInfo[0].irq.switching = intVec;
            intMask = intVec;
            cpssOsSprintf(entryName, "intMask_devIdx%d", devIdx);
            appDemoDbEntryGet(entryName, &intMask);
            localPpPh1Config.hwInfo[0].intMask.switching = intMask;
        }
        for(ii = 0 ; ii < appDemoPpConfigList[devIdx].numOfPortGroups ;ii++)
        {
            ...
            if(GT_OK == appDemoDbEntryGet(entryName, &intVec))
            {
                localPpPh1Config.hwInfo[ii].irq.switching = intVec;
                intMask = intVec;
                cpssOsSprintf(entryName, "intMask_devIdx%d_portGroup%d", devIdx, ii);
                appDemoDbEntryGet(entryName, &intMask);
                localPpPh1Config.hwInfo[ii].intMask.switching = intMask;
            }
        }
    }
    ...
    if ((appDemoDbEntryGet("intVecPrint", &tmpData) == GT_OK) && (tmpData != 0))
    {
        ...
        cpssOsPrintf(
            ...
            localPpPh1Config.hwInfo[0].irq.switching, localPpPh1Config.hwInfo[0].intMask.switching);
        for(ii = 0 ; ii < localPpPh1Config.numOfPortGroups ;ii++)
        {
            cpssOsPrintf(
                "Port Grop %d, vector 0x%08X, mask 0x%08X\n",
                ii, localPpPh1Config.hwInfo[ii].irq.switching,
                localPpPh1Config.hwInfo[ii].intMask.switching);
        }
        osTimerWkAfter(300); /* time to finish previous print */
    }}
...
/**
* @internal lion2WaInit function
* @endinternal
*
* @brief   init the Lion2 WA needed after phase1.
*
* @param[in] boardRevId               - The board revision Id.
*
* @retval GT_OK                    - on success,
...
/**
* @internal bc2WaInit function
* @endinternal
*
* @brief   init the Bobcat2 WA needed after phase1.
*
* @param[in] boardRevId               - The board revision Id.
*
* @retval GT_OK                    - on success,
...
)
{
    ...
    /* This will allow to reset device again*/
    appDemoDbEntryAdd("useMultiNetIfSdma", GT_FALSE);
    ...
    if (appDemoPpConfigList[devIdx].numOfPortGroups <= 1)
    {
        ...
        auDescNum = AU_DESC_NUM_DEF/2;

        fuDescNum = AU_DESC_NUM_DEF/2;
    ...
    else /* LION2_0_SW_DEV_NUM or LION2_1_SW_DEV_NUM */
    {
        ...
        auDescNum = (AU_DESC_NUM_DEF/2) * appDemoPpConfigList[devIdx].numOfPortGroups;
    }
...
)
{
    ...
    if (boardRevId == 2)
    {
        ...
        appDemoPpConfigList[dev].cscdPortsArr[2].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_2_WORDS_E;
        appDemoPpConfigList[dev].cscdPortsArr[2].trunkId = (GT_TRUNK_ID)(trunkNum - 1);

        appDemoPpConfigList[dev].cscdPortsArr[3].portNum = 20;
        appDemoPpConfigList[dev].cscdPortsArr[3].cscdPortType = CPSS_CSCD_PORT_DSA_MODE_2_WORDS_E;
...
/**
* @internal gtDbDxLion2_X2_Bobcat2BoardCleanDbDuringSystemReset function
* @endinternal
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS gtDbDxLion2_X2_Bobcat2BoardCleanDbDuringSystemReset
(
    IN  GT_U8   boardRevId
)
{
    ...
    ppCounter = 0;      /* Number of all Packet processors. */
    ppLion2Counter = 0; /* Number of Lion2 Packet processors. */
    BOBCAT2_DEV_IND = 2;
...
)
{
    ...
    boardCfgFuncs->boardCleanDbDuringSystemReset= gtDbDxLion2_X2_Bobcat2BoardCleanDbDuringSystemReset;
