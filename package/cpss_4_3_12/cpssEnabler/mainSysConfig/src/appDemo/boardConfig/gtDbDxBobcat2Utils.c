...
)
{
    ...
    {
        unitDefPtr = (PRV_CPSS_DXCH_BC2_PA_UNIT_STC*)&(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[dev].paUnitDrvList[i]));
...
)
{
    ...
    GT_U32 TO_CPU_numOfValidPorts, FROM_CPU_numOfValidPorts;
    CPSS_DXCH_NETIF_GDMA_TO_CPU_PORT_INFO_STC   *toCpuPortInfoPtr;
    ...
    GT_U32  devIndex;
    APP_DEMO_STR_STC *portsNotesArray = NULL;
    ...
    {
        if(appDemoPpConfigList[devIndex].devNum == dev)
        {
            portsNotesArray = appDemoPpConfigList[devIndex].portsNotesArray;
            break;
        }
    }
    ...
    for (portIdx = 0 ; portIdx < maxPortIdx; portIdx++)
    {
        ...
        if (system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
        {
            ...
            else
            {
                ...
                {
                    GT_PHYSICAL_PORT_NUM maxGigPhyPort;

                    /* RD board has 24 Gig ports, DB has 16 such ports */
                    maxGigPhyPort = (currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID) ? 15 : 23;

                    if(((portList[portIdx].interfaceMode == CPSS_PORT_INTERFACE_MODE_USX_QUSGMII_E) ||
                        (portList[portIdx].interfaceMode == CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E)) &&
                        (portList[portIdx].speed == CPSS_PORT_SPEED_1000_E) &&
                        (GT_32)(portList[portIdx].portNum) >= 0 &&
                        (portList[portIdx].portNum) <= maxGigPhyPort
                       )
                    {
                        /**
                         *  Override defaults to enable InBand AN for USGMII modes,
                         *  which is necessary to get Copper Link Status for ports
                         *  connected to the PHY 1781
                         */
                        rc = cpssDxChSamplePortManagerAutoNegotiationSet(dev,portNum,GT_TRUE,GT_FALSE,GT_FALSE,GT_FALSE,GT_TRUE,GT_TRUE,GT_FALSE);
                        if(GT_OK != rc)
                        {
                ...
                else if ((currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_M_ID) || (currBoardType == APP_DEMO_IRONMAN_A0_RD_IM_DX1508M_8G4XG_ID))
                {
                    if(((portList[portIdx].interfaceMode == CPSS_PORT_INTERFACE_MODE_USX_QUSGMII_E) ||
                        (portList[portIdx].interfaceMode == CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E)) &&
                        (portList[portIdx].speed == CPSS_PORT_SPEED_1000_E) &&
                        (GT_32)(portList[portIdx].portNum) >= 0 &&
                        (GT_32)(portList[portIdx].portNum) <= 7
                       )
                    {
                        /**
                         *  Override defaults to enable InBand AN for USGMII modes,
        ...
        if(portsNotesArray)
        {
            cpssOsPrintf("%s",portsNotesArray[portIdx]);
        }
        else
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
    ...
    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev))
    {
        rc = cpssDxChNetIfGdmaCpuPortsInfoGet(dev,&allCpuPortsInfo);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        TO_CPU_numOfValidPorts   = allCpuPortsInfo.toCpuInfo.numOfValidPorts;
        FROM_CPU_numOfValidPorts = allCpuPortsInfo.fromCpuInfo.numOfValidPorts;

        if(TO_CPU_numOfValidPorts)
        {
            toCpuPortInfoPtr = & allCpuPortsInfo.toCpuInfo.portsInfo[0];
            /* start with 'TO_CPU' */
            for(/*portIdx*/;portIdx < (maxPortIdx+TO_CPU_numOfValidPorts);
                portIdx++,toCpuPortInfoPtr++)
            {
                GT_U32  firstGlobalQueue = toCpuPortInfoPtr->firstQueue;
                GT_U32  ii;
                for(ii = 0 ; ii < toCpuPortInfoPtr->numOfGdmaUnits ; ii++)
                {
                    cpssOsPrintf("\n| %2d | %4d | TO_CPU   queues [%3.1d..%3.1d]  GDMA[%2.1d] first local ring[%2.1d]",
                        portIdx,
                        toCpuPortInfoPtr->cpuPortNumber,
                        firstGlobalQueue,
                        firstGlobalQueue + toCpuPortInfoPtr->gdmaUnitInfoArr[ii].gdmaNumOfQueues -1,
                        toCpuPortInfoPtr->gdmaUnitInfoArr[ii].gdmaUnitId,
                        toCpuPortInfoPtr->gdmaUnitInfoArr[ii].gdmaFirstQueue);
                    firstGlobalQueue += toCpuPortInfoPtr->gdmaUnitInfoArr[ii].gdmaNumOfQueues;
                }
            }
        }
        else
        {
            cpssOsPrintf("\n NO 'TO_CPU' queues initialized ");
        }
        cpssOsPrintf(SIP6_ROW_DELEMITER);

        if(FROM_CPU_numOfValidPorts)
        {
            fromCpuPortInfoPtr = & allCpuPortsInfo.fromCpuInfo.portsInfo[0];
            /* start with 'TO_CPU' */
            for(/*portIdx*/;portIdx < (maxPortIdx+FROM_CPU_numOfValidPorts+TO_CPU_numOfValidPorts);
                portIdx++,fromCpuPortInfoPtr++)
            {
                cpssOsPrintf("\n| %2d | %4d | FROM_CPU queues [%3.1d..%3.1d]  GDMA[%2.1d] first local ring[%2.1d]",
                    portIdx,
                    fromCpuPortInfoPtr->cpuPortNumber,
                    fromCpuPortInfoPtr->firstQueue,
                    fromCpuPortInfoPtr->firstQueue + fromCpuPortInfoPtr->numOfQueues -1,
                    fromCpuPortInfoPtr->gdmaUnitId,
                    fromCpuPortInfoPtr->gdmaFirstQueue);
            }
        }
        else
        {
            cpssOsPrintf("\n NO 'FROM_CPU' queues initialized ");
        }
        cpssOsPrintf(SIP6_ROW_DELEMITER);
