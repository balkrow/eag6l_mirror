...
)
{
    ...
    while(1)
    {
        ...
        {
            rc = cpssDxChPortApDebugInfoGet(devNum, portNum, MV_HWS_AP_DEBUG_LOG_OUTPUT_CPSS_LOG_E);
            if(rc != GT_OK)
            {
...
)
{
     ...
     cpssOsTimerWkAfter(1000*enDelay);
                   ...
                   if ((GT_FALSE == isLinkUp_1) || (GT_FALSE == isLinkUp_0)) {
                             rc = cpssDxChPortApDebugInfoGet(devNum, 255, MV_HWS_AP_DEBUG_LOG_OUTPUT_TERMINAL_E);
                             if (rc != GT_OK) {
