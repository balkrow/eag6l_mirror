...
{ /* Port,            mappingType                portGroupm, intefaceNum, txQPort, trafficManagerEnable, tmPortInd*/
    ...
    {  80, CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E,   0,      60,      60,      GT_FALSE,          GT_NA,GT_FALSE,HQOS_UNAWARE}, /* use Physical port 80 because 60 is reserved one */
...
)
{
    ...
    for(dev = SYSTEM_DEV_NUM_MAC(0); dev < SYSTEM_DEV_NUM_MAC(ppCounter); dev++)
    {
        ...
        {
            /* PTP (and TAIs) configurations */
            rc = appDemoB2PtpConfig(appDemoPpConfigList[dev].devNum);
