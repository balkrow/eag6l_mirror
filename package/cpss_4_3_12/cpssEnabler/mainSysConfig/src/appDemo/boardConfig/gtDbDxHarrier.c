...
static GT_U32   is_106xxs_pcie_to_DB_ALDRIN_3M = 0;
...
/* need to state before init : appDemoDbEntryAdd("106xxs_pcie_to_DB_ALDRIN_3M",1) */
#define HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M    4 /* real HW connecting 2 boards with PCIe cable: (EBB 106xxs) + (DB of Aldrin3M) */
...
*/
    ...
    */
    ...
    switch(harrierBoardType)
    {
        ...
        case HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M:
            if(0x9000 == ((PRV_CPSS_PP_MAC(devNum)->devType >> 16) & 0xFF00))/*Aldrin3M*/
            {
                /* this is DB board of Aldrin3M that connected to the 106xxs with PCIe cable */
                goto DB_ALDRIN_3M_lbl;
            }
            else
            {
                /* this is 106xxs board with the Harrier (connected to the DB Aldrin3M board with PCIe cable) */
                goto DB_106xxS_lbl;
            }
...
static GT_STATUS fillDbForCpssPortMappingInfo(IN GT_U32 devNum)
{
    ...
    if(harrierBoardType == HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M)
    {
...
)
{
    ...
    if(appDemoPpConfigDevAmount == 2) /* hold priority over 'harrier_force_PortsInitListPtr' */
    {
        if(harrierBoardType == HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M)
        {
            ...
            {
                portInitListPtr = harrier_portInitlist;/*harrier_port_mode;*/
            }
            else
            {
                /* use speed table with the needed speeds on the connection ports */
                portInitListPtr = aldrin3M_portInitlist;/*aldrin3M_port_mode;*/
            }
        }
        else
        {
...
/**
* @internal harrier_MPP_set function
* @endinternal
*
...
* @retval GT_FAIL                  - otherwise.
*/
static GT_STATUS harrier_MPP_set
(
    IN GT_SW_DEV_NUM      devNum
)
{
...
)
{
    ...
    *devTypePtr = devType;
    ...
    /* configure MPPs */
    rc = harrier_MPP_set(devNum);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("harrier_MPP_set", rc);
    if (rc != GT_OK)
        return rc;
...
#ifdef CONFIG_V2_DMA
        /* used as input param 'devNum'*/
        appDemoPpConfigList[devIndex].dmaWindow = devNum;
#endif  /*CONFIG_V2_DMA*/
...
#ifndef CONFIG_V2_DMA /*not used in this mode*/
...
GT_STATUS harrier_b2b_cascadeInit(GT_VOID)
{
    ...
    */
    ...
    GT_U32  index;
    GT_HW_DEV_NUM   hwDevNum[2] = {0,0}; /* init to avoid Klocwork warning */
...
)
{
    ...
    if(appDemoDbEntryGet("106xxs_pcie_to_DB_ALDRIN_3M", &tmpValue) == GT_OK)
    {
        is_106xxs_pcie_to_DB_ALDRIN_3M = tmpValue;
    }
    ...
    if(appDemoPpConfigDevAmount == 2)
    {
        ...
        harrierBoardType = is_106xxs_pcie_to_DB_ALDRIN_3M ?
                           HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M :
                           HARRIER_BOARD_TYPE_B2B_ALDRIN_106xxS;
          ...
          (devIndex < SYSTEM_DEV_NUM_MAC(appDemoPpConfigDevAmount)); devIndex++)
    {
        ...
        if(harrier_force_single_device == 2)
        {
            ...
            if(harrierBoardType == HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M)
            {
                harrier_forcePortMapPtr = aldrin3M_port_mode;
                force_actualNum_harrier_port_modePtr = &actualNum_aldrin3M_port_mode;
            }
        ...
        if(harrierBoardType == HARRIER_BOARD_TYPE_106XXS_AND_DB_ALDRIN_3M)
        {
            /* the second device need to use the Aldrin3M in harrier_appDemoInitSequence_part2 */
            harrier_forcePortMapPtr = aldrin3M_port_mode;
            force_actualNum_harrier_port_modePtr = &actualNum_aldrin3M_port_mode;
        }
    }
    ...
    START_LOOP_ALL_DEVICES(devIndex,devNum)
    {
        ...
        rc = harrier_appDemoInitSequence_part2(boardRevId , devNum);
        if (rc != GT_OK)
        {
            RESILIENT_INIT_SET_OR_RETURN(rc,resilientInit,devIndex);
        }
...
GT_STATUS harrier_appDemoPreFirstDevice(GT_VOID)
{
    ...
    /* must set also appDemoSysConfig.appDemoActiveDeviceBmp */
    appDemoSysConfig.appDemoActiveDeviceBmp = 0x1;
