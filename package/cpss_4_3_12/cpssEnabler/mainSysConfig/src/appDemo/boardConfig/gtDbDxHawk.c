...
)
{
    ...
    appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(devIndex)].pciInfo = *pciInfo;
    ...
    appDemoPpConfigList[SYSTEM_DEV_NUM_MAC(devIndex)].hwInfo = *hwInfoPtr;
    return rc;
}
...
)
{
    ...
    rc = hawk_getBoardInfo(GT_TRUE/*firstDev*/, devIndex, hwInfoPtr++, pciInfo);
    if (rc != GT_OK) {return rc;}

    devIndex = numOfDevices = 1;

    /* In our case we want to find just one prestera device on PCI bus*/
    while (GT_OK == hawk_getBoardInfo(GT_FALSE/*firstDev*/, devIndex, hwInfoPtr++, pciInfo))
    {
        numOfDevices++;
        devIndex = numOfDevices;
...
)
{
    ...
    if (appDemoDbEntryGet("fuqUseSeparate", &tmpData) == GT_NO_SUCH)
    {
...
/**
* @internal hawk_MPP_set function
* @endinternal
*
...
)
{
    ...
    *devTypePtr = devType;
    ...
    /* configure MPPs */
    rc = hawk_MPP_set(devNum);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("hawk_MPP_set", rc);
    if (rc != GT_OK)
        return rc;
...
)
{
    ...
    appDemoPpConfigList[devIndex].ppPhase1Done = GT_TRUE;
    appDemoPpConfigList[devIndex].ppPhase2Done = GT_TRUE;
    ...
    if(rc != GT_OK)
        return rc;
    appDemoPpConfigList[devIndex].ppLogicalInitDone = GT_TRUE;
    ...
    if(rc != GT_OK)
        return rc;
    appDemoPpConfigList[devIndex].ppGeneralInitDone = GT_TRUE;
...
/**
* @internal gtDbHawkBoardReg_AllDeviceSimpleInit function
* @endinternal
*
* @brief    This is the 'simple' board initialization function for Hawk
*           This API is used for Hot Insertion Flow.
*
...
*
*/
static GT_STATUS gtDbHawkBoardReg_AllDeviceSimpleInit
(
    IN  GT_U8  devIndex,
    IN  GT_U8  boardRevId
)
{
    ...
    rc = extDrvPexConfigure(
            appDemoPpConfigList[devIndex].pciInfo.pciBusNum,
    ...
    rc = hawk_appDemoInitSequence(boardRevId, devIndex, devNum, &hwInfo);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    cpssOsPrintf("Time processing the gtDbHawkBoardReg_DeviceSimpleInit (from 'phase1 init') is [%d] seconds + [%d] nanoseconds \n",
            end_sec  - start_sec,
            end_nsec - start_nsec);
...
/**
* @internal gtDbHawkBoardReg_DeviceSimpleInit function
* @endinternal
*
...
*
*/
static GT_STATUS gtDbHawkBoardReg_DeviceSimpleInit
(
    IN  GT_U8  devIndex,
    IN  GT_U8  boardRevId
)
{
    ...
    rc = gtDbHawkBoardReg_AllDeviceSimpleInit(devIndex, boardRevId);
...
)
{
    ...
    if (RESILIENT_INIT_IS_DEV_SET(SYSTEM_DEV_NUM_MAC(0)) == 0)
    {
...
)
{
    ...
    boardCfgFuncs->deviceSimpleInit              = gtDbHawkBoardReg_DeviceSimpleInit;
