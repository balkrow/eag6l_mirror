...
extern GT_STATUS cpssEnGenAuMsgHandle(IN GT_UINTPTR auEvHandler,
                                      IN GT_U8  devNum,
                                      IN GT_U32 evExtData);
extern GT_STATUS cpssEnGenFuMsgHandle(IN GT_UINTPTR auEvHandler,
                                      IN GT_U8  devNum,
                                      IN GT_U32 evExtData);
#else /*CHX_FAMILY*/
/* STUBS */
...
static GT_STATUS cpssEnGenFuMsgHandle(IN GT_UINTPTR auEvHandler,
                                      IN GT_U8  devNum,
                                      IN GT_U32 evExtData)
{
    GT_UNUSED_PARAM(devNum);
...
#if (defined(DXCH_CODE)) && (defined(STACK_EXTENSION))
extern void stackAppPmLinkUpCallback(
    IN GT_U32       devNum,
    IN GT_U32       portNum,
    IN GT_BOOL      linkUp
);
#endif /* #if (defined(DXCH_CODE)) && (defined(STACK_EXTENSION)) */
/**
...
/* AC5X AN WA for port 48 & 49 */
CPSS_OS_SIG_SEM waAnSemId = 0;
CPSS_PORTS_BMP_STC waAnPortBmp;
GT_BOOL   anDonePrev[2] = {0, 0};
GT_BOOL   anDone[2] = {0, 0};

#define PRV_AC5X_AN_WA_CLEAR_PORT_FROM_POLL(port)                 \
    CPSS_PORTS_BMP_PORT_CLEAR_MAC(&waAnPortBmp,port); \
    anDonePrev[port-48] = anDone[port-48] = 0;
...
static GT_U32   portManager_printLinkChangeEnabled = 0;
static GT_U32   appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E = 0;
...
/* flag that state the tasks may process events that relate to FUQ messages.
    this flag allow us to start processing those messages , by that to check that
    the FUQ messages removed from the FUQ in the DMA.

    use function appDemoAllowProcessingOfFuqMessages(...) to set this flag
*/
static GT_BOOL  allowProcessingOfFuqMessages = GT_FALSE;
...
* @endinternal
*
* @brief   function to allow set the flag of : portManager_printLinkChangeEnabled
*
* @param[in] enable        - enable/disable the printings of 'link change' from the port manager
*                               that relate to event CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E
*
* @retval GT_OK                    - on success
*/
GT_STATUS appDemoPortManagerPrintLinkChangeFlagSet
(
    IN GT_U32   enable
)
{
    portManager_printLinkChangeEnabled = enable;
    return GT_OK;
}

/**
* @internal appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E_Set function
* @endinternal
*
* @brief   function to allow set the flag of : appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E
*
* @param[in] not_bind    - not_bind/bind the bind to CPSS_PP_PORT_PM_LINK_STATUS_CHANGED
*
* @retval GT_OK          - on success
*/
GT_STATUS appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E_Set
(
    IN GT_U32   not_bind
)
{
    appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E = not_bind;
    return GT_OK;
}
...
)
{
    ...
    if(appDemoFdbManagerLearningTaskNeedAuqMessagesFromCpss == GT_TRUE)
    {
        ...
        numEntries = MANAGER_GET_MAC(fdbManagerId)->capacityInfo.maxEntriesPerLearningScan;
...
/**
* @internal appDemoAllowProcessingOfFuqMessages function
* @endinternal
*
* @brief   function to allow set the flag of : allowProcessingOfFuqMessages
*
* @param[in] enable                   - enable/disable the processing of the FUQ messages
*
* @retval GT_OK                    - on success
*
* @note :
*
*/
GT_STATUS   appDemoAllowProcessingOfFuqMessages
(
    GT_BOOL     enable
)
{
    ...
    if(enable == GT_TRUE && allowProcessingOfFuqMessages == GT_FALSE)
    {
        allowProcessingOfFuqMessages = GT_TRUE;/*allow appDemoEnPpEvTreat to work on waiting messages !!! */
        /* start processing the messages stuck in the queue */
        ...
        for(dev = SYSTEM_DEV_NUM_MAC(0); dev < SYSTEM_DEV_NUM_MAC(maxDevNum); dev++)
        {
            ...
            /* call appDemoEnPpEvTreat instead of cpssEnGenFuMsgHandle */
            appDemoEnPpEvTreat(dev,CPSS_PP_EB_FUQ_PENDING_E,0);/*unused param*/
        }
    }

    allowProcessingOfFuqMessages = enable;
...
)
{
    ...
    {
        DBG_LOG(("NOTE appDemoDeviceEventRequestDrvnModeInit skiped due to system recovery in HA/HS process"));
        return GT_OK;
    }

    for (i = 0; i < APP_DEMO_PROCESS_NUMBER; i++)
    {
        if(0 != taskParamArr[i].evHndl)
        {
            initWasDone = GT_TRUE;
            break;
        }
...

    {
        ...
        {
            for(i = 0 ; i < evHndlrCauseAllArrSize[9];i++)
            {
                if(appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)
                {
                    /* not need the CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */
                    /* it allow to test application that not bind to this event */
                    evHndlr9CauseDefaultArr[i] = evHndlr9CauseDefaultArr[evHndlrCauseAllArrSize[9] - 1];
                    evHndlrCauseAllArrSize[9]--; /* not need the CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */

                    /* allow to print in case we do get event due to some bug*/
                    portManager_printLinkChangeEnabled = 1;
                    break;
                }
            }
        }
   ...
   if (initWasDone == GT_FALSE)
   {
       ...
       for (i = 0; i < APP_DEMO_PROCESS_NUMBER; i++)
       {
           ...
           if (rc != GT_OK)
           {
               cmdOsPrintf("failed to spawn evHndl_%d\n", i);
               return GT_FAIL;
           }
           osTaskGracefulCallerRegister(eventHandlerTidArr[i], 1, NULL , NULL);
...
/**
* @internal prvAppDemoEventRequestDrvnModeInit function
* @endinternal
*
* @brief   This routine spawns the App Demo event handlers.
*
...
* @retval GT_FAIL                  - otherwise.
*/
GT_STATUS prvAppDemoEventRequestDrvnModeInit
(
    IN GT_VOID
)
{
    ...
    if (system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HA_E ||
        system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E)
    {
        DBG_LOG(("NOTE appDemoEventRequestDrvnModeInit  skiped due to system recovery in HA/HS process"));
        return GT_OK;
    }

    if(appDemoSysConfig.appDemoActiveDeviceBmp)
    {
        GT_U8   devIndex,_dummy;
        START_LOOP_ALL_DEVICES(devIndex,_dummy)
        {
...

    {
        ...
        if(appDemo_not_bind_CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)
        {
            for(i = 0 ; i < evHndlrCauseAllArrSize[9];i++)
            {
                if(evHndlr9CauseDefaultArr[i] == CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)
                {
                    /* not need the CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */
                    /* it allow to test application that not bind to this event */
                    evHndlr9CauseDefaultArr[i] = evHndlr9CauseDefaultArr[evHndlrCauseAllArrSize[9] - 1];
...
#ifdef DXCH_CODE
            if (PRV_CPSS_PP_MAC(devNum) &&
                (CPSS_IS_DXCH_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily)))
            {
                if((cpssUniEventArr[i] == CPSS_PP_GPP_E)&&
                    (GT_TRUE == PRV_CPSS_DXCH_ERRATA_GET_MAC(devNum,PRV_CPSS_DXCH_BOBCAT2_GPP_INTERRUPT_WA_E)))
                {
...
{
    GT_U32 i,arraySize;
    GT_PHYSICAL_PORT_NUM * remotePhysicalPorts;
    ...
    remotePhysicalPorts = (GT_PHYSICAL_PORT_NUM *)cpssOsMalloc(sizeof(GT_PHYSICAL_PORT_NUM)*arraySize);

    if (remotePhysicalPorts == NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM,"remotePhysicalPorts allocation failed");
    }
    ...
    cpssOsFree(remotePhysicalPorts);
...
)
{
    ...
    */
    ...
    if(portMap.portMap.isExtendedCascadePort &&
        !appDemoCallBackOnLinkStatusChaneFunc)/*appDemoCallBackOnLinkStatusChaneFunc responsible for this logic when exists */
    {
...
)
{
    ...
    queue = (GT_U8)evExtData; /* the global queueId (0..127) */
...
static GT_U32                                  appDemoAutoAgingIsIdle = 1;
...
)
{
    ...
    while (1)
    {
        ...
        {
            appDemoFdbManagerAutoAging_cleanup(NULL);
...
)
{
    ...
    osTaskGracefulCallerRegister(autoAgeTid,1, NULL, NULL);
...
GT_STATUS prvAppDemoFdbManagerControlSet
(
    IN APP_DEMO_AU_MESSAGE_HANDLER_METHORD_ENT  auMsgHandler,
...
)
{
    ...
    switch(auMsgHandler)
    {
        ...
        case APP_DEMO_AU_MESSAGE_HANDLER_FDB_MANAGER_WITH_CUCKOO_E:
            ...
            if(autoAgingEn == GT_FALSE)
            {
                /* we wait for the task to be idle --> not doing aging any more */
                /* this wait as test prvTgfBrgFdbManagerAgingScan_performance_traffic_and_verify
                   checks that aging stops , but it not know how much time to wait
                   and the 200 milliseconds may not be enough */
                while(!appDemoAutoAgingIsIdle)
                {
...
*
* DESCRIPTION:
*      WA for missing pcs_link_status interrupt when AN restart on CPU_MAC ports.
*      Applicable to ports 48 & 49 of AC5X.
* INPUTS:
*       param   - device number
*
* OUTPUTS:
...
*
*******************************************************************************/
static unsigned __TASKCONV ac5xAnWaTask
(
    GT_VOID * param
)
{
    GT_STATUS rc = GT_OK;
    GT_U32    *params = (GT_U32 *)param;
    GT_U32    devNum;
    GT_U32    i;
    GT_U32 port[2];
    ...
    while(1)
    {
        if(!(CPSS_PORTS_BMP_IS_PORT_SET_MAC(&waAnPortBmp,48) || CPSS_PORTS_BMP_IS_PORT_SET_MAC(&waAnPortBmp,49)))
        {
            cpssOsSigSemWait(waAnSemId,0);
        }

        for(i = 0; i < 2; i++)
        {
            if(CPSS_PORTS_BMP_IS_PORT_SET_MAC(&waAnPortBmp,i+48))
            {
                if(anDonePrev[i] != anDone[i])
                {
                    rc = prvCpssPortManagerLinkChangeWA(devNum, port[i]);
                    if(rc != GT_OK)
                    {
                        PRV_AC5X_AN_WA_CLEAR_PORT_FROM_POLL(i+48);
                    }
                }
            }
        }

        cpssOsTimerWkAfter(4);

        for(i = 0; i < 2; i++)
        {
            if(CPSS_PORTS_BMP_IS_PORT_SET_MAC(&waAnPortBmp,i+48))
            {
                anDonePrev[i] = anDone[i];
                rc = prvCpssPortPcsAnDoneGet(devNum, i+48, &anDone[i]);
                if(rc != GT_OK)
                {
                    PRV_AC5X_AN_WA_CLEAR_PORT_FROM_POLL(i+48);
                }
...
)
{
    ...
    switch (uniEv)
    {
         case CPSS_SRVCPU_SDK_WATCHDOG_E:
            cpssOsPrintf("\nSCPU Watchdog interrupt asserted from MG%d!\n", evExtData);
            break;
        case CPSS_SRVCPU_SDK_DOORBELL_E:
            cpssOsPrintf("\nSCPU Doorbell interrupt asserted from MG%d!\n", evExtData);
            break;
        ...
        case CPSS_PP_EB_FUQ_PENDING_E:
            if(allowProcessingOfFuqMessages == GT_TRUE)
            {
                rc = cpssEnGenFuMsgHandle(auEventHandlerHnd,devNum, evExtData);
                if (GT_OK != rc)
                {
                    DBG_LOG(("cpssEnAuMsgGet: error, devNum=%d, uniEv=%d, rc=%d\n", devNum, uniEv, rc, 4, 5, 6));
                }
 ...
 /* because now we miss sometimes this interrupt from unknown reason treat of AP link change done upon
    ...
    case CPSS_SRVCPU_PORT_REMOTE_FAULT_TX_CHANGE_E:
        ...
        case CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E:
            /* this is SW event and appears only in port manager mode , and gives
               the final indication that a link changed in the port manager POV */
            #if (defined PX_CODE) || (defined DXCH_CODE)
            portNum = evExtData;

            if(portManager_printLinkChangeEnabled)
            {
                ...
                if(gotValue)
                {
                    ...
                    osPrintf("PM link status changed on : device [%d] portNum:[%d] LINK[%s] \n",
                        devNum , portNum,
                        (portStage.portState == CPSS_PORT_MANAGER_STATE_LINK_UP_E)? "Up" : "Down");
                }
            }

            #endif /*(defined PX_CODE) || (defined DXCH_CODE)*/
            ...
            if (CPSS_IS_DXCH_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily))
            {
                ...
                if (rc == GT_OK)
                {
                    stackAppPmLinkUpCallback(devNum, portNum, (portStage.portState == CPSS_PORT_MANAGER_STATE_LINK_UP_E) ? GT_TRUE : GT_FALSE);
                }
            }
            #endif /* #if (defined(DXCH_CODE)) && (defined(STACK_EXTENSION)) */
        ...
        if (linkChngFuncPtr)
        {
            ...
            if (CPSS_IS_DXCH_FAMILY_MAC(devFamily))
            {
                ...
                if (portMgr && !disableAutoPortMgr)
                {
                    ...
                    {
                        /* we need to do extra logic before finish handling the port */
                        goto    dxch_appDemoCallBackOnLinkStatusChaneFunc_lbl;
                    }
                    break;
                }
...
#endif
dxch_appDemoCallBackOnLinkStatusChaneFunc_lbl:
                if(appDemoCallBackOnLinkStatusChaneFunc)
                {
        ...
        /* we need also in WM to handle this event */
        case CPSS_PP_PORT_MMPCS_SIGNAL_DETECT_CHANGE_E:
                ...
                (PRV_CPSS_SIP_5_10_CHECK_MAC(devNum)))  /* Bobcat2_B0, Bobk, Aldrin, AC3X, Bobcat3 */
           {
                ...
                portNum = evExtData & (CPSS_MAX_PORTS_NUM_CNS-1);
                ...
                else
                {
                    ...
                    {
                         /* must use '& (CPSS_MAX_PORTS_NUM_CNS-1))' otherwise compiler issus an error */
                        locksTimeoutArray[portNum] = 0;
        ...
        case CPSS_PP_DATA_INTEGRITY_ERROR_E:
            ...
            if( PRV_CPSS_DXCH_LION2_FAMILY_CHECK_MAC(devNum) ||
                ...
                PRV_CPSS_DXCH_IRONMAN_CHECK_MAC(devNum)
              )
        ...
        case CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E:
            ...
            if(devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
            {
                ...
                if(rc != GT_OK)
                {
                    DBG_LOG(("CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E - cpssDxChPortManagerPortParamsGet:rc=%d,portNum=%d\n",rc, portNum,3,4,5,6));
                    return rc;
                }

                if (((portMap.interfaceNum == 48) || (portMap.interfaceNum == 49)) && portMgr)
                {
                    rc = cpssDxChPortManagerPortParamsGet(devNum, portNum, &portParams);
                    if ((rc != GT_OK) && (rc != GT_NOT_INITIALIZED))
                    {
                        DBG_LOG(("CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E - cpssDxChPortManagerPortParamsGet:rc=%d,portNum=%d\n",rc, portNum,3,4,5,6));
                        PRV_AC5X_AN_WA_CLEAR_PORT_FROM_POLL(portMap.interfaceNum);
                        return rc;
                    }

                    /****************************************************************************************************************************
                      * HW bug and WA: missing link_status change interrupt upon AN restart on CPU_MAC ports
                      *
                      *   For the CPU port there is no lpcs_link_status, only link_status which MUX between the BASE-R and BASE-X.
                      *   While for other ports the lpcs_link_status look at an_enable and decides rather to take rx_sync or (rx_sync && an_done),
                      *   In the CPU port when the link_status is driven by BASE-X, it takes directly the rx_sync without carrying for an_enable or an_done.
                      *   Due to this reason AN has no effect at all on the link_status nor link_ok at all.
                      *
                      * WA steps:
                      *   upon link_status_change interrupt and link_status == 0 --> move to [a] from any step*
                      *
                      *   a. Set force_link_dis to 0x1.
                      *   b. Upon link_status_change interrupt, check link_status. If it is 0x0 stay in this step, else move to [c].
                      *   c. Poll for sg_an_done till becomes 0x1. (fastest it can become 0x1 is 4.832ms, if we wait more than that, we delay the actual link_ok indication).
                      *   d. Read LP_Ability[15]. If 0x1 go to [e], else skip to [f].
                      *   e. Set force_link_dis to 0x0.
                      *   f. Poll for sg_an_done with polling interval < 4.832ms. if becomes 0x0, move to next step.
                      *   g. Set force_link_dis to 0x1.
                      *   h. Go back to [c].
                      *
                      *   polling interval calculation : 3xlink_timer + 2x(16 bit time) = 3x1.6ms + 2x16ns = 4.8ms + 32ns = 4.832ms
                    */
                    if((portParams.portParamsType.regPort.ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E) &&
                       (portParams.portParamsType.regPort.portAttributes.autoNegotiation.inbandEnable == GT_TRUE))
                    {
                        rc = prvCpssPortPcsAlignLockStatusGet(devNum, portNum, portMap.interfaceNum, &alignLock);
                        if(rc != GT_OK)
                        {
                            DBG_LOG(("prvCpssPortPcsAlignLockStatusGet: error, devNum=%d, port=%d, rc=%d\n",
                                   devNum, portNum, rc, 4, 5, 6));
                        ...
                        if(alignLock == GT_TRUE )
                        {
                            if(waAnSemId == 0)
                            {
                                GT_U32 waTaskAnId = 0, i;
                                GT_U32 params[3] ; /*devNum , p48, p49 */
                                params[0] = devNum;
                                for(i = 0; i < 2; i++)
                                {
                                    rc = cpssDxChPortPhysicalPortMapReverseMappingGet(devNum,CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E,i+48, &params[1 + i]);
                                    if(rc != GT_OK)
                                    {
                                        osPrintf("ac5xAnWaTask - cpssDxChPortPhysicalPortMapReverseMappingGet:rc=%d,portNum=%d\n",rc, portNum);
                                        return rc;
                                    }
                                }

                                CPSS_PORTS_BMP_PORT_SET_MAC(&waAnPortBmp, portMap.interfaceNum);
                                if (cpssOsSigSemBinCreate("waAnSem", CPSS_OS_SEMB_EMPTY_E, &waAnSemId) != GT_OK)
                                {
                                    return GT_NO_RESOURCE;
                                }
                                rc = cpssOsTaskCreate("waTaskAn",    /* Task Name      */
                                                       500,              /* Task Priority  */
                                                       _64K,             /* Stack Size     */
                                                       ac5xAnWaTask,      /* Starting Point */
                                                       (GT_VOID*)((GT_UINTPTR)&params[0]), /* Arguments list */
                                                       &waTaskAnId);       /* task ID        */
                                if(rc != GT_OK)
                                {
                                    DBG_LOG(("CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E - failed to create links WA task :rc=%d,portNum=%d\n",rc, portNum,3,4,5,6));
                                    return rc;
                                }
                            }
                            else
                            {
                               ...
                               {
                                    CPSS_PORTS_BMP_PORT_SET_MAC(&waAnPortBmp, portMap.interfaceNum);
                                    rc = cpssOsSigSemSignal(waAnSemId);
                                    if(rc != GT_OK)
                                    {
                                        DBG_LOG(("CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E - failed to create links WA task :rc=%d,portNum=%d\n",rc, portNum,3,4,5,6));
                                        return rc;
       ...
       case CPSS_PP_MACSEC_TRANSFORM_ENGINE_ERR_E:
        ...
        case CPSS_PP_PORT_LANE_FEC_ERROR_E:
         ...
         case CPSS_PP_PORT_LANE_SQ_EVENT_E:
            ...
            {
                osPrintf("CPSS_PP_PORT_LANE_SQ_EVENT_E - cpssDxChPortPhysicalPortMapReverseMappingGet:rc=%d,portNum=%d\n",rc, evExtData);
                return rc;
            }
            ...
            if (portMgr && !disableAutoPortMgr)
            {
                appDemoPortInitSeqPortStatusChangeSignal(devNum, physicalPortNum, CPSS_PP_PORT_LANE_SQ_EVENT_E);
                break;
            }
#endif
           DBG_LOG(("Received SQUELCH DETECT event: %d %d\n", uniEv,evExtData));
           break;
#endif
        default:
            break;
...
)
{
    ...
    GT_U32              timeOut = MV_DEFAULT_TIMEOUT;          /* default T.O         */
    GT_U32              self_tid = 0;
    ...
    while (1)
    {
        rc = cpssEventSelect(evHndl, &timeOut, evBitmapArr, (GT_U32)CPSS_UNI_EV_BITMAP_SIZE_CNS);
        if(( eventRequestDrvnModeReset) || (osTaskGracefulGetTerminationReq(self_tid)))
        {
            break;
        }
        if (rc == GT_TIMEOUT)/*timeOut*/
        {
            continue;
        }
...
)
{
    ...
    GT_U32 counter =0;
    GT_U32 fullQueues =0;
    GT_U32 timeOut = MV_DEFAULT_TIMEOUT;
    GT_U32 self_tid = 0;
    hndlrParamPtr = (EV_HNDLR_PARAM*)param;
    evHndl        = hndlrParamPtr->evHndl;
...
)
{
    ...
    for (i = 0; i < APP_DEMO_PROCESS_NUMBER; i++)
    {
        ...
        /* call CPSS to unbind from events */
        rc = cpssEventUnBind(evHndlrCauseAllArr[i],
                             evHndlrCauseAllArrSize[i]);
        if (GT_OK != rc)
        {
            osPrintSync("cpssEventUnBind: failed on [i=%d] \n",i);
            return GT_FAIL;
        }
    }
