...
)
{
    ...
    {
        CPSS_MAC_ENTRY_EXT_STC temp_macEntry;
        if(appDemoSysConfig.mixedSip5AndSip6System && PRV_CPSS_SIP_6_CHECK_MAC(devNum) &&
           macEntryPtr->userDefined != 0 &&
           macEntryPtr->userDefined == macEntryPtr->dstInterface.devPort.portNum /*learned from sip 5 device */)
        {
            /* System has devices of SIP5 and SIP6 generations.
               Such devices has different muxing of UDB, SourceID and VID1 fields
               in FDB entry.
               By default SIP5 supports 8 bits of UDB - bits 0..7,
               but SIP6 supports 8 bits also with hole in middle:
               - bits 0..3 - any values
               - bits 4..5 - must be zero
               - bits 6..9 - any values.
               userDefined in NA message holds value of Source Port. NA from SIP5 device
               may hold value of userDefined with bits 4..5 are not zero. And addition
               of MAC entry with such userDefined will fail on SIP_6 devices.
               Use userDefined = zero to avoid such problems. */
            temp_macEntry = *macEntryPtr;
            temp_macEntry.userDefined = 0;
            macEntryPtr = &temp_macEntry;
        }
...
/**
* @internal cpssEnGenFuMsgHandle function
* @endinternal
*
* @brief   This routine get FUQ messages in the DxCh devices.
*
* @param[in] devNum                   - the device number.
* @param[in] evExtData                - Unified event additional information
*
...
* @retval GT_FAIL                  - otherwise.
*/
GT_STATUS cpssEnGenFuMsgHandle
(
    IN GT_UINTPTR           auEvHandler,
    IN GT_U8                devNum,
    GT_U32                  evExtData
)
{
    ...
    if(!IS_API_CH_DEV_MAC(devNum))/* is device support dxch API ? */
    {
        /* who called this for non-DXCH device ? */
    ...
    while(rc != GT_NO_MORE)
    {
        ...
        AU_FDB_UPDATE_LOCK();
#if defined DXCH_CODE
        rc = cpssDxChBrgFdbFuMsgBlockGet(devNum, &numOfAu, bufferArr);
#else /*DXCH_CODE*/
        rc = GT_NOT_IMPLEMENTED;
#endif /*DXCH_CODE*/
        AU_FDB_UPDATE_UNLOCK();
...
static void fill_txPcktParams(IN CPSS_DXCH_NET_TX_PARAMS_STC *txPcktParamsPtr)
{
    ...
    */
    ...
    txPcktParamsPtr->dsaParam.commonParams.dsaTagType = CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT;
