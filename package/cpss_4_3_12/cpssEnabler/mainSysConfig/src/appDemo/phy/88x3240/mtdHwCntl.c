...
)
{
    ...
    if( 0 != cpssXsmiPortGroupRegisterWrite(switchInfo->devNum, ( 1 << switchInfo->portGroup), switchInfo->xsmiInterface ,port, reg, dev, value) )
        {
    result = MTD_FAIL;
...
)
{
    ...
    *data = (MTD_U16)MdioRead((unsigned long)port,
    ...
    if( 0 != cpssXsmiPortGroupRegisterRead(switchInfo->devNum, ( 1 << switchInfo->portGroup), switchInfo->xsmiInterface, port, reg, dev, data) )
        {
    result = MTD_FAIL;
