...
Fault Signal, CRC Error, x FIFO Interrupt, etc.

The interrupt polarity at the PIN can be configured. You can also force
the PIN active by mydSetGlobalInterruptCntl() function. If you don't plan to use
the PIN to implement an interrupt handler, you may poll the status bits to implement
...
(
 ...
 Outputs:
    portsIntrStatusArray - returns a structure containing all level interrupt status. Caller
 ...
 Description:
    ...
    If portsIntrStatusArray[portIndex].gpioIntr returns a 1, users should check Pin mode and consider calling
    mydGetGPIOInterruptStatus() to poll which Pin is generating the Level 3 GPIO Interrupt.
 ...
 Notes/Warnings:
    ...
    Calling this function will clean all latched PCS units interrupt status.
...
(
 ...
 Description:
    ...
    indicating a interrupt. If INTn Pin is configured to MYD_PIN_MODE_RCLK, the output of this
    API is invalid.

    For PCS unit interrupts, ONLY enabled interrupt sources will be returned.

    As descriped in the "Interrupt overview", all interrupts generated on the device
    will be aggregated to the INTn PIN. Users may call mydGetGlobalInterruptCntl()
    first to understand the polarity of the INTn PIN. Then call this function to read
    enabled interrupt sorces if INTn PIN generates interrupt.

    Please check the interrupt definition of the corresponding unit to identify
    the Unit-level(Level 3) PCS interrupt source.

    If portsIntrStatusArray[portIndex].gpioIntr returns a 1, users should check Pin mode and consider
    calling mydGetGPIOInterruptConfig() and mydGetGPIOInterruptStatus() to check which Pin is generating
...
*/
#define MYD_INTR_TRISTATE_BIT       (1<<1)  /* Active - drive INTn high, Inactive - drive tristate INTn (first port only) */
#define MYD_INTR_POLARITY_BIT       (1<<0)  /* Active - drive INTn high, Inactive - drive INTn low (first port only)  */
...
(
 ...
 Inputs:
    ...
    interruptPinControl - on first port this controls the INTn pin.
        Pass the logical OR of the following: MYD_INTR_TRISTATE_BIT
 ...
 Description:
    This function sets global interrupt control bits. On the first port of a multi-port
    PHY it controls the pin polarity and what happens when the interrup is inactive
...
(
 ...
 Description:
    This function reads global interrupt control bits. On the first port of a multi-port
    PHY it controls the pin polarity and what happens when the interrup is inactive
...
/******************************************************************************
MYD_STATUS mydSetGPIOInterruptConfig
(
    IN MYD_DEV_PTR pDev,
 ...
 Inputs:
    ...
    - X7120 Pins available for MYD_PIN_MODE_GPIO mode:
        GPIO1, GPIO2, GPIO3, GPIO4, CLK_OUT_SE1, CLK_OUT_SE2
    - X6141 Pins available for MYD_PIN_MODE_GPIO mode:
        GPIO0, GPIO1, GPIO2, GPIO3, CLK_OUT_SE1, CLK_OUT_SE2
 ...
 Description:
    This function enable or disable the GPIO interrupt on the given GPIO Pin. User also
    need to select gpioIntrType when enabling interrupt. The default value of gpioIntrType
    is MYD_GPIO_INTR_NONE, which means no interrupt is expected.
...
/******************************************************************************
MYD_STATUS mydGetGPIOInterruptConfig
(
    IN MYD_DEV_PTR pDev,
    IN MYD_PIN_ID gpioPinId,
 ...
 Inputs:
    pDev - pointer to MYD_DEV initialized by mydInitDriver() call
    gpioPinId - The Pin to read, the Pin MUST be set to MYD_PIN_MODE_GPIO mode
                See the bit definitions and header of mydSetPinMode() for details.
    - X7120 Pins available for MYD_PIN_MODE_GPIO mode:
        GPIO1, GPIO2, GPIO3, GPIO4, CLK_OUT_SE1, CLK_OUT_SE2
    - X6141 Pins available for MYD_PIN_MODE_GPIO mode:
 ...
 Description:
    This function reads back the GPIO interrupt enable setting along with the interrupt type.
    Noticed that the Level 3 GPIO interrupt ONLY works as an input.
...
(
 ...
 Inputs:
    pDev - pointer to MYD_DEV initialized by mydInitDriver() call
    gpioPinId - The Pin to read status, the Pin MUST be set to MYD_PIN_MODE_GPIO mode
                See the bit definitions and header of mydSetPinMode() for details.
    - X7120 Pins available for MYD_PIN_MODE_GPIO mode:
        GPIO1, GPIO2, GPIO3, GPIO4, CLK_OUT_SE1, CLK_OUT_SE2
    - X6141 Pins available for MYD_PIN_MODE_GPIO mode:
...
#define MYD_200G_LPI_TX_RT_BIT       (1<<9)  /* 200G PCS Tx Receving LPI Words */
#define MYD_200G_LPI_RX_RT_BIT       (1<<8)  /* 200G PCS Rx Receving LPI Words */
...
(
 ...
 Description:
    ...
    If you only want to enable certain interrupts, please read the interrupt enable
    setting back first by mydGetPCSInterruptEnable(), then set corresponding bits and call
...
(
 ...
 Description:
    ...
    To determin if the interrupt is still asserted at the INTn pin, the
...
(
 ...
 Description:
    This function configure the mode of given multi-purpose pins. Here the Pins
    only refers to the one defined in the _MYD_PIN_ID struct

    - Modes Available on X7120 -
    MYD_PIN_MODE_BACKPRESSURE: 40G-25G Gearbox Backpressure Signaling Mode:
    ...
    MYD_PIN_MODE_INTR: Device Interrupt Pin INTn (Not GPIO Interrupt)
    INTn
 ...
 Notes/Warnings:
    User SHALL NOT change the Pin Mode once it is configured, since each the mode of
    Pin should match the board schematic. User may only call this function on the SAME Pin
    again for developing/debug/diagnostic purpose.

    DO NOT unmasked/enable any interrupt if INTn pin is set to MYD_PIN_MODE_RCLK mode,
    otherwise the recovered clock will be interfered. DO NOT configure INTn to RCLK mode
    if you are not sure whether there is any interrupt enabled or not.

    Recovered Clock Limitations:
    1. On the X7120, GPIO2 is only capable to output Recovered Clock from Port0 and Port1,
    2. CLK_OUT_SE1 is only capable to output Recovered Clock from Port2 and Port3.
    3. CLK_OUT_DIFF_N and CLK_OUT_DIFF_P are only capable to output Receovered Clock
       from Port 3.
...
(
 ...
 Description:
    ...
    the output register, bit but will have no effect on given GPIO Pin.
    ...
    The given GPIO Pin interrupt should not be enabled.
...
(
 ...
 Inputs:
    ...
    ledCtrl - Configure LED sub-lane select, interface slect, solid and blink activity,
 ...
 Description:
    ...
    select the lane and interface sorce for the mapped LED. The API provides several activity choices
 ...
 Side effects:
    ...
    two LEDs.
    On the X6141, all LED share same ledTimerConfig. Changing ledTimerConfig on any of the LED
    will affect all LEDs.
...
(
 ...
 Inputs:
    pDev - pointer to MYD_DEV initialized by mydInitDriver() call
    portSelect - 0-31, select the port of clock for the Recovered Clock Pin
    interfaceSelect - select the interface of clock for the Recovered Clock Pin
        MYD_LINE_SIDE
        MYD_HOST_SIDE
 ...
 Description:
    Configure Recovered Pin and select the Recovered Clock source.
    ...
    divide ratio(8 bits) and divider enable. Each (port, interface) have TWO MUX Options.
 ...
 Note/Warning:
    ...
    User MUST call mydConfigRclkMux() to set the MUX options before using the RCLK Pin.

    On the X7120, GPIO2 is only capable to output Recovered Clock from Port0 and Port1,
    CLK_OUT_SE1 is only capable to output Recovered Clock from Port2 and Port3.
    CLK_OUT_DIFF_N and CLK_OUT_DIFF_P are only capable to output Receovered Clock
    from Port 3.
******************************************************************************/
MYD_STATUS mydConfigRClkPin
(
...
typedef struct _MYD_RCLK_MUX_OPTION MYD_RCLK_MUX_OPTION;
    ...
    laneSelect - select the recovered clock source
...
(
    IN MYD_DEV_PTR pDev,
    IN MYD_RCLK_MUX rClkMux
);

 Inputs:
    pDev - pointer to MYD_DEV initialized by mydInitDriver() call
    rClkMux - Configure the Recovered Clock MUX on all ports and interfaces
               User MUST pass-in correct union type for the rClkMux
 ...
 Description:
    Configure all Recovered MUXs on the device. This functions should be used with
    mydConfigRClkPin() when configure recovered clock Pin.

    This funtion allow the user to configure the Recovered Clock MUX options for
 ...
 Note/Warning:
    ...
    the pass-in array will be used to set MUX options even the field comes with
    default value 0.
******************************************************************************/
MYD_STATUS mydConfigRclkMux
(
