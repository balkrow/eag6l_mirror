...
)
{
    MYD_SPEED speed;
    MYD_U16 mdioPortIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    MYD_U16 sAddr, regCntlPCS = 0, regStatusPCS = 0, controlANEGReg;     
    MYD_BOOL speedSet = MYD_TRUE;

    GT_UNUSED_PARAM(stateDumpOptions);
...
)
{
    ...
    {
        case MYD_NIMB_EYE_DEFAULT:
        {
            /* Trigger iCal if min eye value setting */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F6, 0x10));
            /* Trigger iCal if max eye deviation to 6-eye average */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F7, 0xF));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F8, 0x38));
            break;
        }

        case MYD_NIMB_EYE_UPDATE:
        {
...
/*******************************************************************
 MYD_STATUS mydNIMBSetICal
 Access NIMB register to start/stop the iCal polling and calibration
*******************************************************************/
MYD_STATUS mydNIMBSetICal
(
    IN MYD_DEV_PTR pDev,
    IN MYD_U16 mdioPort,
    IN MYD_U16 laneOffset, /* unused. reserve for future */
    IN MYD_U16 iCalNIMBOption /* 0 to stop AutoCal; non-0 to start */
)
{
    MYD_U16 major, minor;
    ...
    if (major == 0)
    {
        MYD_DBG_ERROR("mydNIMBSetICal: unsupported firmware version %u.%u\n", major, minor);
        return MYD_FAIL;
    }

    if (iCalNIMBOption)
    {
        /* start iCal auto calibration */
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0x1));
    }
    else
    {
        /* start iCal auto calibration */
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0x0));
    }
...
/*******************************************************************
 MYD_STATUS mydNIMBGetICalStatus
 Access NIMB register for iCal status
*******************************************************************/
MYD_STATUS mydNIMBGetICalStatus
(
    IN MYD_DEV_PTR pDev,
    IN MYD_U16 mdioPort,
    IN MYD_U16 laneOffset, /* unused. reserve for future */
    OUT MYD_U16 *iCalNIMBStatus /* 1=auto iCal running, 0 = stop */
)
{
    MYD_U16 major, minor;
    ...
    *iCalNIMBStatus = 0;
    ...
    /* Okay to query the status and return MYD_OK for unsupported firmware */
    if (major == 0)
    {
        MYD_DBG_INFO("mydNIMBSetICal: unsupported firmware version %u.%u\n", major, minor);
        return MYD_OK;
    }
    
    ATTEMPT(mydHwXmdioRead(pDev, mdioPort, MYD_PMA_DEV, 0xC8F5, iCalNIMBStatus));
...
/* The mydNIMBxxx API requires the supported NIMB firmware */
/*******************************************************************
 MYD_STATUS mydNIMBSetEyeCoeffForLoopback
 Access NIMB register to set the min/max to trigger the iCal calibration
*******************************************************************/
MYD_STATUS mydNIMBSetEyeCoeffForLoopback
(
...
)
{
    ...
    switch (eyeCoeffOptions)
    {
        ...
        case MYD_NIMB_EYE_UPDATE:
        {
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F7, 0x0038));
            break;
        }
