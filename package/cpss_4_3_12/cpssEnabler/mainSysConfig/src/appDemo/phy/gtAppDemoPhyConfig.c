...
GT_APPDEMO_XPHY_STATIC_INFO_STC portToPhyArrayAc5xRd2540_SR1[] = {
...
)
{
    ...
    else
    {
        ...
        {
            cpssDrvHwPpResetAndInitControllerWriteReg(devNum, 0xf8d24, 0x18200ce);
...
)
{
    ...
    *phyInfoArrayLenPtr = 0;
    ...
    switch(currBoardType)
    {
        ...
        case APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID:
            smiMaxNum = BIT_4;
            smiInterfaceEnum = CPSS_PHY_SMI_INTERFACE_0_E;
            break;

        case APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID:
            smiMaxNum = 24;
            smiInterfaceEnum = CPSS_PHY_SMI_INTERFACE_0_E;
            break;
    ...
    PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(devNum, portGroupId)
    {
        ...
        {
            if ((currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID) ||
                (currBoardType == APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID) )
            {
                data = (0x2D | (0x9 << 16));
            }
            ...
            {
                rc = gtAppDemoPhyIdGet(devNum, portGroupsBmp, smiInterfaceEnum, smiAddr, &data1);
                if(rc != GT_OK)
                {
            ...
            phyInfoArray[*phyInfoArrayLenPtr].xsmiInterface = (CPSS_PHY_XSMI_INTERFACE_ENT)smiInterfaceEnum;
    ...
    else
    {
        switch(currBoardType){
            ...
            case APP_DEMO_ALDRIN2_BOARD_RD_CNS:
                ...
                maxXSMIinterface = CPSS_PHY_XSMI_INTERFACE_3_E;
                break;
            case APP_DEMO_IRONMAN_A0_DB_IM_L_SFP_ID:
            case APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID:
            case APP_DEMO_IRONMAN_A0_DB_IM_M_ID:
            case APP_DEMO_IRONMAN_A0_RD_IM_DX1508M_8G4XG_ID:
            case APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID:
                return;
            default:
                maxXSMIinterface = CPSS_PHY_XSMI_INTERFACE_1_E;
            ...
            for(xsmiAddr = 0; xsmiAddr < BIT_5; xsmiAddr++)
            {
                ...
                if ((data != MRVL_PHY_UID_CNS) && (data != MRVL_PHY_REV_A_UID_CNS))
                {
                    continue;
                        ...
                        appDemoCheckEnableGet() == GT_TRUE)
                {
                    vendorId = data;
                    appDemoCheckEnableSet(GT_FALSE);
                }
                rc = gtAppDemoXPhyIdGet(devNum, portGroupsBmp, xsmiInterface, xsmiAddr, &data);
                if(rc != GT_OK)
                {
    ...
    if (GT_APPDEMO_PHY_FLASH_E == mode)
    {
        ...
        for(i=0;i < numPort;i++)
        {
                ...
                */
                ...
                /* LED control */
                        ret = cpssXsmiPortGroupRegisterWrite(devNum, 0, xsmiInterface,  phyAddr[i],  0xf022, 31,  0x29);
                if (ret != GT_OK)
                {
                        cpssOsPrintf(
                            "cpssXsmiPortGroupRegisterWrite(%d, 0, %u,  %u,  0xf022, 31,  0x29):ret=%d\n",
                            devNum, xsmiInterface, phyAddr[i], ret);
                        return ret;
                }
                        ret = cpssXsmiPortGroupRegisterWrite(devNum, 0, xsmiInterface,  phyAddr[i],  0xf023, 31,  0x31);
                if (ret != GT_OK)
                {
                        cpssOsPrintf(
                            "cpssXsmiPortGroupRegisterWrite(%d, 0, %u,  %u,  0xf023, 31,  0x31):ret=%d\n",
                            devNum, xsmiInterface, phyAddr[i], ret);
                        return ret;
                }
                ret = cpssXsmiPortGroupRegisterWrite(devNum, 0, xsmiInterface,  phyAddr[i],  0xf015, 31,  0x8800);
                if (ret != GT_OK)
                {
                        cpssOsPrintf(
                            "cpssXsmiPortGroupRegisterWrite(%d, 0, %u,  %u,  0xf015, 31,  0x31):ret=%d\n",
                            devNum, xsmiInterface, phyAddr[i], ret);
                        return ret;
                }
                /* power up phy */
...
)
{
    ...
    if ((port_entry_PTR->initData_PTR->phyType != MPD_TYPE_88E1780_E) &&
        (port_entry_PTR->initData_PTR->phyType != MPD_TYPE_88E1781_E))
    {
...
static GT_STATUS gtAppDemoPhyStaticPhyInfoGet
(
    IN  GT_U32                      currBoardType,
    IN  GT_U8                       boardRevId,
    IN  GT_APPDEMO_XPHY_INFO_STC    phyInfo,
    OUT GT_APPDEMO_XPHY_STATIC_INFO_STC *staticPhyInfo_PTR
)
{
    GT_U32  i;
    GT_U32  portStart = 0;
    ...
    case APP_DEMO_XCAT5X_A0_BOARD_RD_CNS:
        ...
        if(phyInfo.phyType == MRVL_PHY_2540_MODEL_NUM_CNS)
        {
            ...
            {
                staticPhyInfoArray = portToPhyArrayAc5xRd2540_SR1;
                portStart = 32;
                portEnd   = 47;
            }
        }
    ...
    case APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID:
        if(phyInfo.phyType == MRVL_PHY_1781_MODEL_NUM_CNS)
        {
            staticPhyInfoArray = portToPhyArrayIronmanRd24g6xg1781;
            portStart = 0;
            portEnd   = 23;
        }
        break;
    ...
    for(i = 0; staticPhyInfoArray[i].portNum != 0xff ; i++)
    {
        ...
        {
            osMemCpy(staticPhyInfo_PTR, &(staticPhyInfoArray[i]), sizeof(GT_APPDEMO_XPHY_STATIC_INFO_STC));
        }
    }
    return GT_OK;
}
...
GT_U32 gtAppDemoPhyMpdPhyNumGet
    (
    IN  GT_U8                       devNum,
    IN  GT_U32                      portNum
)
{
    GT_U32 rel_ifIndex, phyNumber;
    PRV_MPD_PORT_HASH_ENTRY_STC *port_entry_PTR = NULL;

    rel_ifIndex = gtAppDemoPhyMpdIndexGet(devNum, portNum);
    port_entry_PTR = prvMpdGetPortEntry(rel_ifIndex);
    phyNumber = port_entry_PTR->initData_PTR->phyNumber;
...
)
{
    switch(phyType)
    {
        ...
        case MRVL_PHY_1781_MODEL_NUM_CNS:
            return MPD_TYPE_88E1781_E;
...
)
{
    ...
    GT_U32                                      phyNumber = 0;
    GT_APPDEMO_XPHY_STATIC_INFO_STC             staticPhyInfo;
    ...
    if((currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_M_ID) ||
       (currBoardType == APP_DEMO_IRONMAN_A0_RD_IM_DX1508M_8G4XG_ID))
        return GT_OK;
    ...
    else if ((currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID) ||
             (currBoardType == APP_DEMO_IRONMAN_A0_DB_IM_M_ID) ||
             (currBoardType == APP_DEMO_IRONMAN_A0_RD_IM_DX1508M_8G4XG_ID) ||
             (currBoardType == APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID))
    {
        /* configure MPP Control 34-35 to 0x2 - M0_MDC (out) and M0_MDIO (inout) */
        rc = genRegisterSet(devNum, 0, 0x80020110, 0x00002200, 0x0000FF00);
        if (rc != GT_OK)
        {
            cpssOsPrintf("genRegisterSet(0, 0, 0x80020110, 0x00002200, 0x0000FF00): rc = %d\n", rc);
            return rc;
        }
    }
    ...
    for(i = 0; i < xsmiPhyInfoArrayLen ; i++)
    {
        switch (currBoardType){
            ...
            case APP_DEMO_CETUS_BOARD_LEWIS_RD_CNS:
            case APP_DEMO_CETUS_BOARD_DB_CNS:
                rc = gtAppDemoPhyPortNumGet(currBoardType, xsmiPhyInfoArray[i], &portNum);
                if(rc != GT_OK)
                {
            ...
            case APP_DEMO_XCAT5P_A0_BOARD_RD_CNS:
            case APP_DEMO_XCAT5X_A0_BOARD_RD_CNS:
                rc = gtAppDemoPhyStaticPhyInfoGet(currBoardType, boardRevId, xsmiPhyInfoArray[i], &staticPhyInfo);
                if(rc != GT_OK)
                {
                    cpssOsPrintf("gtAppDemoPhyMpdInit failed \n");
                    return GT_FAIL;
                ...
                rel_ifIndex = gtAppDemoPhyMpdIndexGet(devNum, staticPhyInfo.portNum);
                break;
        }
    ...
    for(i = 0; i < smiPhyInfoArrayLen; i++)
    {
        switch (currBoardType){
            ...
            case APP_DEMO_CETUS_BOARD_LEWIS_RD_CNS:
            case APP_DEMO_CETUS_BOARD_DB_CNS:
                rc = gtAppDemoPhyPortNumGet(currBoardType, smiPhyInfoArray[i], &portNum);
                if(rc != GT_OK)
                {
                    cpssOsPrintf("gtAppDemoPhyMpdInit failed \n");
                    return GT_FAIL;
            ...
            case APP_DEMO_IRONMAN_A0_DB_IM_L_PHY_ID:
            case APP_DEMO_IRONMAN_A0_RD_IM_DX1518M_24G6XG_ID:
                rc = gtAppDemoPhyStaticPhyInfoGet(currBoardType, boardRevId, smiPhyInfoArray[i], &staticPhyInfo);
                if(rc != GT_OK)
                {
                    cpssOsPrintf("gtAppDemoPhyMpdInit failed \n");
                    return GT_FAIL;
...
)
{
    ...
    params.phySpeedExt.hostSide.speed   = (MPD_SPEED_ENT)speedParam.hostSide.speed;
    params.phySpeedExt.hostSide.ifMode  = (MPD_INTERFACE_MODE_ENT)speedParam.hostSide.ifMode;
    params.phySpeedExt.hostSide.fecMode = (MPD_FEC_MODE_SUPPORTED_ENT)speedParam.hostSide.fecMode;
    params.phySpeedExt.hostSide.apEnable = apEnableHost;

    params.phySpeedExt.lineSide.speed   = (MPD_SPEED_ENT)speedParam.lineSide.speed;
    params.phySpeedExt.lineSide.ifMode  = (MPD_INTERFACE_MODE_ENT)speedParam.lineSide.ifMode;
    params.phySpeedExt.lineSide.fecMode = (MPD_FEC_MODE_SUPPORTED_ENT)speedParam.lineSide.fecMode;
...
)
{
    ...
    if(ret == MPD_OK_E)
    {
        *opMode       = (GT_APPDEMO_XPHY_OP_MODE)params.phySpeedExt.hostSide.speed;
        *fecModeHost  = (GT_APPDEMO_XPHY_FEC_MODE)params.phySpeedExt.hostSide.fecMode;
        *apEnableHost = params.phySpeedExt.hostSide.apEnable;
        *fecModeLine = (GT_APPDEMO_XPHY_FEC_MODE)params.phySpeedExt.lineSide.fecMode;
...
)
{
    ...
    params.phyTune.txConfig = MPD_SERDES_CONFIG_PERMANENT_E;
    params.phyTune.tuneParams.txTune.avago.atten =  tuneParamsPtr->txTune.avago.atten;
    params.phyTune.tuneParams.txTune.avago.post  =  tuneParamsPtr->txTune.avago.post;
    params.phyTune.tuneParams.txTune.avago.pre   =  tuneParamsPtr->txTune.avago.pre;
    params.phyTune.tuneParams.txTune.avago.pre2  =  tuneParamsPtr->txTune.avago.pre2;
    params.phyTune.tuneParams.txTune.avago.pre3  =  tuneParamsPtr->txTune.avago.pre3;

    params.phyTune.hostOrLineSide          = (MPD_PHY_HOST_LINE_ENT)hostOrLineSide;
...
)
{
    ...
    for (laneIndex = 0; laneIndex < polaritySwapParamsArrSize ; laneIndex++)
    {
        if(lanesBmp & (1<<laneIndex))
        {
            ...
            params.phySerdesPolarity.hostOrLineSide = (MPD_PHY_HOST_LINE_ENT)hostOrLineSide;
...
/**
* @internal gtAppDemoPhyMacLoopbackSet function
* @endinternal
*
* @brief   Function enables/disables MAC interface loopback in PHY.
*
*
* @param[in] devNum     - PP's device number.
* @param[in] portNum    - port number
* @param[in] enable     - GT_TRUE - enable MAC interface loopback.
*                                 All recieved from PP frames are looped back to PP
*                         GT_FALSE - disable MAC interface loopback
*
* @retval GT_OK         - on success.
* @retval GT_FAIL       - on error.
*
*/
GT_STATUS gtAppDemoPhyMacLoopbackSet
(
    IN GT_U8                    devNum,
...
)
{
    ...
    PRV_MPD_OP_CODE_ENT op = PRV_MPD_OP_CODE_SET_LOOP_BACK_E;

    cpssOsMemSet(&phyParams,0,sizeof(phyParams));
    phyParams.internal.phyLoopback.enable = enable;

    rel_ifIndex = gtAppDemoPhyMpdIndexGet(devNum, portNum);

    result = mpdPerformPhyOperation(rel_ifIndex, (MPD_OP_CODE_ENT)op, &phyParams);
...
/**
* @internal gtAppDemoPhyMacLoopbackGet function
* @endinternal
*
* @brief   Function get status (enabled/disabled) MAC interface loopback in PHY.
*
*
* @param[in] devNum     - PP's device number.
* @param[in] portNum    - port number
* @param[out] enablePtr  - GT_TRUE - MAC interface loopback enabled.
*                                 All recieved from PP frames are looped back to PP
*                         GT_FALSE - MAC interface loopback disabled
*
* @retval GT_OK         - on success.
* @retval GT_FAIL       - on error.
*
*/
GT_STATUS gtAppDemoPhyMacLoopbackGet
(
    IN GT_U8                    devNum,
    IN GT_PHYSICAL_PORT_NUM     portNum,
...
)
{
    ...
    result = prvMpdMdioReadRegister("gtAppDemoPhyMacLoopbackGet",
                                    port_entry_PTR,
                                    0,
                                    0,
                                    &value);
    ...
    *enablePtr = (value & BIT_14) ? GT_TRUE : GT_FALSE;
    return GT_OK;
}

/**
* @internal appDemoPhyLoopBackStateDump function
* @endinternal
*
* @brief   Function prints status (enabled/disabled) of MAC interface loopback in PHY.
*
*
* @param[in] devNum     - PP's device number.
* @param[in] portNum    - port number
...
*
*/
GT_STATUS appDemoPhyLoopBackStateDump
(
    IN GT_U8                    devNum,
    IN GT_PHYSICAL_PORT_NUM     portNum
)
{
    ...
    rc = gtAppDemoPhyMacLoopbackGet(devNum,portNum,&enable);
    if (rc != GT_OK)
    {
        cpssOsPrintf("appDemoPhyLoopBackStateDump: failed rc %d\n", rc);
    }

    cpssOsPrintf("appDemoPhyLoopBackStateDump: enable %d\n", enable);
