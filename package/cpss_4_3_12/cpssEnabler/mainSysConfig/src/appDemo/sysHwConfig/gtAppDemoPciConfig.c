...
static  GT_U32  debug_unknown_pcieDevId = 0x0;
/* example pcieDevId = 0x2101 that is 'wrong' Harrier-MCM .
to allow the appDemo 'pci scan' to recognize the pcieDevId , and the cpss will also be 'faked' with :

do shell-execute     appDemoPciDebugDeviceIdSet   0x2101
do shell-execute     appDemoDebugDeviceIdSet 0, 0x214111AB
*/
GT_STATUS appDemoPciDebugDeviceIdSet(IN GT_U32   pcieDevId)
{
    if(pcieDevId != 0 && debug_unknown_pcieDevId != 0x0)
    {
        /* supports only single 'debug_unknown_pcieDevId' */
        return GT_FULL;
    }
    ...
    debug_unknown_pcieDevId = pcieDevId;
...
)
{
    ...
    /* optimize the number of times calling the device to get PCI/PEX info */
    {
        ...
        for(/*continue*/;instance < 256; instance++)
        {
            ...
            for(deviceIdx = 0; currentDevListPtr[deviceIdx].vendorId; deviceIdx++)
            {
                ...
                if(debug_unknown_pcieDevId && /* no match so far and we need to allow recognize the fake pci devId */
                   currentDevListPtr[deviceIdx+1].vendorId &&/* next is valid */
                   currentDevListPtr[deviceIdx+2].vendorId == 0)/* but no more after that */
                {
                    /* so the 'deviceIdx' is one for the last one in the array */
                    /* lets assume it is not important for our system          */

                    /* set it with the debug value */
                    currentDevListPtr[deviceIdx+1].vendorId = 0x11ab;/* Marvell vendor ID */
                    currentDevListPtr[deviceIdx+1].devId    = (GT_U16)debug_unknown_pcieDevId;
                }
            }
        }
    ...
    for(;currentDevListPtr[deviceIdx].vendorId != 0;)
    {
                            ...
                            instance++, &busNo, &deviceNo, &funcNo) != GT_OK)
        {
            ...
            if(debug_unknown_pcieDevId && /* no match so far and we need to allow recognize the fake pci devId */
               currentDevListPtr[deviceIdx].vendorId &&/* next is valid */
               currentDevListPtr[deviceIdx+1].vendorId == 0)/* but no more after that */
            {
                /* so the 'deviceIdx' is one for the last one in the array */
                /* lets assume it is not important for our system          */

                /* set it with the debug value */
                currentDevListPtr[deviceIdx].vendorId = 0x11ab;/* Marvell vendor ID */
                currentDevListPtr[deviceIdx].devId    = (GT_U16)debug_unknown_pcieDevId;
            }
