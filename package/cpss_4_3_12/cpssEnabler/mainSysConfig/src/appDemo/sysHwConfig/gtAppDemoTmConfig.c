...
extern GT_STATUS appDemoTmQCountersGet(GT_U8 dev, GT_BOOL includeBytes);
...
static  GT_U32 overrideRoot = 0;

/* used for treeGrow, to override the root of TM tree to a values different from 0 or the smalles TM enabled port index value
  it is a responsibility of a user to make sure that the overrideRoot port is TM-enabled*/
GT_VOID appDemoTmOverrideRootSet(GT_U32 overrideRootValue)
{
    overrideRoot = overrideRootValue;
}

GT_STATUS appDemoTmQNodeTGet(GT_U32 qNodeIndex)
{
    if ((qNodeIndex >= QUEUE_POOL_SIZE_CNS) ||
        (qNodeTopology[qNodeIndex].exists != GT_TRUE))
    {
...
shell-execute appDemoTmTMMemUseGet 0
*/
GT_STATUS appDemoTmTMMemUseGet
(
    IN  GT_U8   devNum
)
{
...
/* get TM non empty Qs */
GT_STATUS appDemoTmNonEmptyQueuesGet
(
    IN  GT_U8   devNum
)
{
...
GT_STATUS appDemoTmPortsIdsGet
(
...
(
    ...
    IN    GT_U32 *tmPortsArr,           /* tmPortsArr retrived from appDemoTmPortsIdsGet */
...
* @endinternal
*
* @brief   Implements general UpTo 48 Ports initialization for specific device.
*
...
)
{
    ...
    rc = appDemoTmPortsIdsGet(dev, &tmPortsArrSize, tmPortsArr);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    for (i = 0; i < portsNum; i++)
    {
        ...
        if (dumpTmPorts)
        {
            cpssOsPrintf("cpssTmPortCreate: port: %d, tmPort: %d\n",
                         i,
                         tmPort);
        }

        rc = cpssTmPortCreate(dev, tmPort, &portParams, 1, 1, 1, queuesNum);
...
/**
* @internal tmTreeGrow function
* @endinternal
*
* @brief   Builds a basic symmetric TM tree with default attributes.
*
* @param[in] dev                      - device number
* @param[in] portsNum                 - number of consequitive TM ports
* @param[in] cNodesNum                - per port node
* @param[in] bNodesNum                - per C node
* @param[in] aNodesNum                - per B node
* @param[in] queuesNum                - per A node
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - otherwise
*
* @note Should be moved to TM utils. Restriction: TM ports are numbered from 0.
*
*/
...
)
{
    ...
    for (tmLevel = CPSS_TM_LEVEL_Q_E; tmLevel < CPSS_TM_LEVEL_P_E; tmLevel++)
    {
        ...
        osPrintf("%d level shaping profile indexes: ",tmLevel);
        for (i = 0; i < 10; i++)
        {
...
/**
* @internal appDemoTmScenarioBISTInit function
* @endinternal
*
* @brief   Implements BIST scenario 18 initialization for specific device.
*         4 1G ports, 1 c node per port, 2 b node per c node, 2 a node per b node, 8 queues per a node
*         shaping on all levels (a,b,c,p), propagated priority.
*         Tail Drop on port wred drop all queues
* @param[in] dev                      -   device number
...
* @retval GT_FAIL                  - otherwise
*/
GT_STATUS appDemoTmScenarioBISTInit
(
    IN  GT_U8   dev
)
{
...
* @endinternal
*
* @brief   Implements 8 ports with 10G BW. each
...
)
{
    ...
    rc = appDemoTmPortsIdsGet(dev, &allTmPortsArrSize, allTmPortsArr);
    if (rc != GT_OK)
    {
        return rc;
    }
 ...
 /*   queueParams.dropProfileInd = queueDropProfileId; */
    ...
    /* validate (and update if necessary)  cbs & ebs values                   */
    /**************************************************************************/
    rc = cpssTmPortShapingValidate(dev, portParams.cirBw, portParams.eirBw , &(portParams.cbs), &(portParams.ebs));
    /*
    ...
    for (i = 0; i < tmPortsArrSize; i++)
    {
        tmPort = tmPortsArr[i];
        /*create port*/
        rc = cpssTmAsymPortCreate(dev, tmPort, &portParams);
...
* @retval GT_FAIL                  - otherwise
*
* @note For Cos Port mode the next APi should be run!!!! AA TBD
*
*/
...
/**
* @internal appDemoTmScenarioSimpleShaping function
* @endinternal
*
* @brief   Implements scenario for testing shaping.
*         1 10M port, 1 c node per port, 1 b node per c node, 1 a node per b node, 8 queues per a node
*         shaping on all levels (a,b,c,p), propagated priority.
*         Tail Drop on port and queue level and color aware on Q1 in each A node and wred mode on queue0 in each A node
* @param[in] dev                      -   device number
* @param[in] param                    -   parameter for customization configuration
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - otherwise
*/
...
/**
* @internal appDemoTmTxFifoScenarioInit function
* @endinternal
*
* @brief   Implements 8 ports with 10G BW. each
*         8 ports, 512 C nodes, 2K B nodes, 4K A nodes, 16K queues
...
* @endinternal
*
* @brief   Implements only cpssTmInit() - default init configuration for TM
*
* @param[in] dev                      -   device number
*
* @retval GT_OK                    - on success
