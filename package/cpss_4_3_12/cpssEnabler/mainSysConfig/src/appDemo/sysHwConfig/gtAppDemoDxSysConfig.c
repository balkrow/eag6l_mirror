...
#include <cpss/common/init/cpssInit.h>/*for cpssPpWmDeviceInAsimEnvGet()*/
...
(
    ...
    IN  CPSS_PORT_INTERFACE_MODE_ENT    iface
);
...
extern GT_STATUS appDemoPortManagerTaskEnableSet(IN GT_U32 enable);
...
)
{
    ...
    {
        osPrintf(
            "Device[%d] Chip ID bits[31:0] 0x%08X bits[63:32] 0x%08X bits[95:64] 0x%08X \n",
            devNum, devInfo.genDevInfo.unitLevelTraceabilityArr[0],
            devInfo.genDevInfo.unitLevelTraceabilityArr[1],
            devInfo.genDevInfo.unitLevelTraceabilityArr[2]);
    }
    else if (PRV_CPSS_SIP_6_CHECK_MAC(devNum) ||
        (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
    {
        osPrintf(
            "Device[%d] Chip ID bits[31:0] 0x%08X bits[63:32] 0x%08X \n",
            devNum, devInfo.genDevInfo.unitLevelTraceabilityArr[0],
            devInfo.genDevInfo.unitLevelTraceabilityArr[1]);
    }
...
)
{
    ...
    if(GT_TRUE == appDemoPpConfigList[devIdx].fdbNotUsed)
    {
        /* this device stated on it self to not be part of NA learning */
        /* so it not need to set NA to CPU , and SP (Storm Prevention) */
        auMsgEn = GT_FALSE;
    }
    ...
    else
    {
        ...
        if(PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev))
        {
            /* there is no such indication in the GDMA */
        }
        else
        {
            rc = cpssDxChBrgFdbQueueFullGet(dev,CPSS_DXCH_FDB_QUEUE_TYPE_AU_E,&isFull);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChBrgFdbQueueFullGet", rc);
            if(rc != GT_OK)
            {
                return rc;
            }
            ...
            if(isFull == GT_TRUE)
            {
                /*  HS will not trigger FDB action that may send AA to AUQ that is FULL
                    (as no 'event handler' for it ready at this stage) , as it will cause action to not end
                */
                cpssOsPrintf("Note : AUQ FULL \n");
            }
        }

        /* The old code was calling cpssDxChBrgFdbTrigActionStart(dev, actionMode) ,
           that could cause AA to AUQ , as AA/TA are enabled to CPU at this stage.
           but if the AUQ is already full , than this trigger will not get 'finished' indication,
           because there is no event handler yet (not created at this stage yet)
           to empty the AUQ , to receive the new messages
        */

        /* only set the action , do not trigger */
    ...
    if((appDemoDbEntryGet("forceSkipFdbFlush", &value) == GT_OK) && (value == 1))
    {
        /* force to skip FDB Flush , due to issue that need to be ignored during
           init , so the init can finish without stuck on wait for it to finish */
    }
    else
...
)
{
    ...
    GT_U32  ii;
    GT_U32  apEnable = 0;
    GT_BOOL forceNoDataIntegrity = GT_FALSE;
    ...
    cpssPpPhase1ParamsPtr->serRefClkOverriden = 0;
    if(appDemoDbEntryGet("serdesRefClock", &value) == GT_OK)
    {
        if(value <= CPSS_DXCH_PP_SERDES_REF_CLOCK_INTERNAL_125_E)
        {
            ...
            /* entry found, update serRefClkOverriden for future use */
            cpssPpPhase1ParamsPtr->serRefClkOverriden = 1;
        }
    }


    /* enable/disable status for phy ref clk Pu and Oe */
    if ( appDemoDbEntryGet("phyRefClkPuOeCfg", &value) == GT_OK )
    {
        cpssPpPhase1ParamsPtr->phyRefClkPuOeCfg = (CPSS_PHY_REF_CLK_PU_OE_CFG_ENT)value;
    }
    else
    {
       cpssPpPhase1ParamsPtr->phyRefClkPuOeCfg = 0x0;
    }
...
/*#endif*/
    ...
    switch(deviceId)
    {
...
#ifdef GM_USED
            /* don't enable shadow in GM because of low perfomance */
            break;
#endif  /* GM_USED */
        case CPSS_BOBCAT2_ALL_DEVICES_CASES_MAC:
        case CPSS_ALDRIN_DEVICES_CASES_MAC:
        ...
        case CPSS_HARRIER_ALL_DEVICES_CASES_MAC:
            ...
            else if(GT_TRUE==forceNoDataIntegrity)
            {
                cpssPpPhase1ParamsPtr->numOfDataIntegrityElements = 0;
            }
            else
    ...
    if(cpssPpPhase1ParamsPtr->numOfDataIntegrityElements != 0 &&
       cpssPpWmDeviceInAsimEnvGet())
    {
        /* ASIM only (not WM native) */
        /* on Harrier the cpssInitSystem takes 157 seconds without shadow.
           but takes 450 seconds with the shadow.
           so the default for ASIM is 'NO shadow'
        */
        cpssPpPhase1ParamsPtr->numOfDataIntegrityElements = 0;
        if((appDemoDbEntryGet("numOfDataIntegrityElements", &value) == GT_OK) && (value != 0))
        {
            /* allow to 'force' in ASIM the 0xFFFFFFFF to enable the shadow */
            cpssPpPhase1ParamsPtr->numOfDataIntegrityElements = value;
        }
    }
...
)
{
    ...
    /* needed to allow sequence Phase1Init, Reset, Remove device from DB, Phase1Init again */
    /* appDemoDxHwPpPhase1Init related to not SIP6 devices                                 */
    rc = appDemoDxChHwPpPhase1ResetAndInit(&cpssPpPhase1Params, ppDevTypePtr);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("appDemoDxChHwPpPhase1ResetAndInit", rc);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    rc = cpssDxChHwPpStartInit(
        dev, initDataListPtr, initDataListLen);
...
)
{
    ...
    /* check if pre-cpssInitSystem we forced different enabling on the libraries */
    /* this to support the 'simple-init-system' (of sip6 devices) that not call
       appDemoUpdateLibInitParams() from appDemoBoardGeneralInit*/
...
)
{
    ...
    /*skip during System Recovery*/
    if (CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E != system_recovery.systemRecoveryProcess)
    {
        return GT_OK;
    }

    if (appDemoDbEntryGet("appDemoEventRequestDrvnModeInit = disable", &value) == GT_OK && value != 0)
    {
        /* the appDemo not allow (at this stage) to enable the events */
        /* maybe later will be called again after the logic will set value = 0 to this string */
        return GT_OK;
    }
...
)
{
    ...
    if (appDemoDbEntryGet("appDemoDxTrafficEnable = disable", &value) == GT_OK && value != 0)
    {
        /* the appDemo not allow (at this stage) to enable the traffic ! */
        /* maybe later will be called again after the logic will set value = 0 to this string */
        return GT_OK;
    }
    ...
    for (port = 0; port < (appDemoPpConfigList[devIdx].maxPortNumber); port++)
    {
        ...
        if(appDemoDxChIsLocalPort_or_RemotePort_with_MAC_PHY_OBJ(dev,port))
        {
            ...
            if (portMgr)
            {
                ...
                if(rc != GT_OK)
                {
                    /* allow a port to be asymetric RX/TX directions */
                    /* if this port is 'on board' cascade port - the specific init
                       will set this port as 'enable'
                    */
                    continue;
                }
            }
            ...
            if ((!portMgr) ||
                /* skip all types of AC5X ports in PM mode */
                ((portType != CPSS_CSCD_PORT_NETWORK_E) && ((PRV_CPSS_PP_MAC(dev)->devFamily != CPSS_PP_FAMILY_DXCH_AC5X_E))))
            {
...
)
{
    ...
    for (port = 0; port < (appDemoPpConfigList[devIdx].maxPortNumber); port++)
    {
            ...
            (system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E))
        {
            ...
            {
                if(GT_FALSE ==PRV_CPSS_SIP_6_CHECK_MAC(dev))
                {
                    rc = cpssDxChPortFlowControlEnableSet(
                        dev, port,
                        ((flowControlEnable == GT_FALSE)
                    ...
                    rc = cpssDxChPortPeriodicFcEnableSet(
                        dev, port, CPSS_PORT_PERIODIC_FLOW_CONTROL_DISABLE_E);
...
)
{
    ...
    for(ii = 0; ii < appDemoPpConfigList[devIdx].numberOfCscdPorts; ++ii)
    {
        ...
        if((CPSS_CSCD_PORT_DSA_MODE_1_WORD_E == cscdPortType)
           ...
           || (CPSS_CSCD_PORT_DSA_MODE_4_WORDS_E == cscdPortType))
        {
...
)
{
    ...
    switch (appCpssPpPhase1ParamsPtr->serdesRefClock)
    {
        default: return GT_BAD_PARAM;
        case APPDEMO_SERDES_REF_CLOCK_EXTERNAL_25_SINGLE_ENDED_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_EXTERNAL_25_SINGLE_ENDED_E;
            break;
        case APPDEMO_SERDES_REF_CLOCK_EXTERNAL_125_SINGLE_ENDED_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_EXTERNAL_125_SINGLE_ENDED_E;
            break;
        case APPDEMO_SERDES_REF_CLOCK_EXTERNAL_125_DIFF_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_EXTERNAL_125_DIFF_E;
            break;
        case APPDEMO_SERDES_REF_CLOCK_EXTERNAL_156_25_SINGLE_ENDED_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_EXTERNAL_156_25_SINGLE_ENDED_E;
            break;
        case APPDEMO_SERDES_REF_CLOCK_EXTERNAL_156_25_DIFF_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_EXTERNAL_156_25_DIFF_E;
            break;
        case APPDEMO_SERDES_REF_CLOCK_INTERNAL_125_E:
            cpssPpPhase1InfoPtr->serdesRefClock = CPSS_DXCH_PP_SERDES_REF_CLOCK_INTERNAL_125_E;
            break;
    }
...
)
{
    ...
    /* ip routing mode*/
    ppLogicalInfoPtr->routingMode =
        (ppConfigPtr->routingMode == APP_DEMO_CPSS_DXCH_POLICY_BASED_ROUTING_ONLY_E)
...
)
{
    ...
    for(portNum=0; portNum < numOfEports; portNum++)
    {
        ...
        if(rc != GT_OK)
        {
            /* trace only on error , as we get 8K times on Harrier */
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChNstPortIsolationModeSet", rc);
            return rc;
        }
    }

    /* Enable Egress VLAN Filter for BC2 A0.
       HW default was fixed starting from BC2 B0 */
    if (!PRV_CPSS_SIP_5_10_CHECK_MAC(devNum))
    {
        for(portNum=0; portNum < numOfEports; portNum++)
        {
...
)
{
    ...
    GT_U32  devIndex;
    GT_U32   bc3HardWireTcamGroupsArr[5];
    static const CPSS_DXCH_TCAM_CLIENT_ENT tcamClientsArr[] =
    {
    ...
    static const GT_U32 tcamClientsArrSize =
        (sizeof(tcamClientsArr) / sizeof(tcamClientsArr[0]));
    GT_U32 clientIndex;

    GT_UNUSED_PARAM(sysConfigParamsPtr);

    bc3HardWireTcamGroupsArr[CPSS_DXCH_TCAM_TTI_E   ] = GROUP_0;
    bc3HardWireTcamGroupsArr[CPSS_DXCH_TCAM_IPCL_0_E] = GROUP_1;
    bc3HardWireTcamGroupsArr[CPSS_DXCH_TCAM_IPCL_1_E] = GROUP_2;
    bc3HardWireTcamGroupsArr[CPSS_DXCH_TCAM_IPCL_2_E] = GROUP_3;
    bc3HardWireTcamGroupsArr[CPSS_DXCH_TCAM_EPCL_E  ] = GROUP_4;
    ...
    for (devIndex = SYSTEM_DEV_NUM_MAC(0);
          (devIndex < SYSTEM_DEV_NUM_MAC(maxDevNum)); devIndex++)
    {
    ...
    if(hardWireTcamGroupsPtr)
    {
        ...
        for (clientIndex = 0; (clientIndex < tcamClientsArrSize); clientIndex++)
        {
            client = tcamClientsArr[clientIndex];
            switch(client)
            {
    ...
    /* 3-floor TCAMs of SIP5 units excludind SIP6 units wits one only TCAM-Client group */
    if ((tcamFloorsNum == 3) && PRV_CPSS_SIP_5_15_CHECK_MAC(dev) &&
        ((PRV_CPSS_SIP_6_CHECK_MAC(dev) == 0) ||
        (PRV_CPSS_DXCH_PP_MAC(dev)->hwInfo.tcam.sip6maxTcamGroupId >= 2)))
    {
...
)
{
    ...
    if (PRV_CPSS_SIP_5_CHECK_MAC(dev))
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            /*sip7: unlike sip5 the trunk ECMP is dedicated and not need to use for trunks 1/2 the L2 ecmp table */
            maxTrunksNeeded = PRV_CPSS_DXCH_PP_MAC(dev)->hwInfo.parametericTables.numEntriesTrunkEcmp;
...
)
{
    ...
    CPSS_DXCH_IP_TCAM_ROUTE_ENTRY_INFO_UNT          defEclassLttEntry;
    CPSS_DXCH_IP_LPM_VR_CONFIG_STC                  vrConfigInfo;
    GT_U32                                          lpmDbId = 0;
    GT_U32                                          devIdx; /* index to appDemoPpConfigList */
    GT_U32                                          tmpValue;
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(appDemoPpConfigList[devIdx].devNum))
    {
    ...
    cpssOsMemSet(&defEclassLttEntry,0,sizeof(CPSS_DXCH_IP_TCAM_ROUTE_ENTRY_INFO_UNT));
    ...
    if((shadowType == CPSS_DXCH_IP_LPM_RAM_SIP6_SHADOW_E)||
       (shadowType == CPSS_DXCH_IP_LPM_RAM_SIP7_SHADOW_E))
    {
        defEclassLttEntry.ipLttEntry.ipv6MCGroupScopeLevel    = CPSS_IPV6_PREFIX_SCOPE_GLOBAL_E;
        defEclassLttEntry.ipLttEntry.numOfPaths               = 0;
        defEclassLttEntry.ipLttEntry.routeEntryBaseIndex      = 2;
        if ((shadowType == CPSS_DXCH_IP_LPM_RAM_SIP5_SHADOW_E)||
            (shadowType == CPSS_DXCH_IP_LPM_RAM_SIP6_SHADOW_E)||
            (shadowType == CPSS_DXCH_IP_LPM_RAM_SIP7_SHADOW_E))
        {
            defEclassLttEntry.ipLttEntry.routeType            = CPSS_DXCH_IP_ROUTE_ENTRY_METHOD_REGULAR_E;
        }
        else
        {
            defEclassLttEntry.ipLttEntry.routeType            = CPSS_DXCH_IP_ROUTE_ENTRY_METHOD_ECMP_E;
        }

        defEclassLttEntry.ipLttEntry.sipSaCheckMismatchEnable = GT_FALSE;
        defEclassLttEntry.ipLttEntry.ucRPFCheckEnable         = GT_FALSE;
    }
    ...
    case CPSS_IP_PROTOCOL_IPV4_E:
        ...
        if((shadowType == CPSS_DXCH_IP_LPM_RAM_SIP6_SHADOW_E)||
           (shadowType == CPSS_DXCH_IP_LPM_RAM_SIP7_SHADOW_E))
        {
            vrConfigInfo.treatIpv4ClassEasNonRegularUc = sysConfigParamsPtr->treatIpv4ClassEasNonRegularUc;
            cpssOsMemCpy(&vrConfigInfo.defIpv4ClassENextHopInfo.ipLttEntry,
                         &defEclassLttEntry.ipLttEntry,
                         sizeof(CPSS_DXCH_IP_LTT_ENTRY_STC));
        }
        else
        {
    ...
    if( vrConfigInfo.treatIpv4ClassEasNonRegularUc == GT_TRUE )
    {
        /*******************************************************/
        /* in this case e class  has it's own default route    */
        /* set route entry 2 as default e class route          */
        /*******************************************************/
        cpssOsMemSet(&ucRouteEntry,0,sizeof(ucRouteEntry));
        ucRouteEntry.type = CPSS_DXCH_IP_UC_ROUTE_ENTRY_E;
    ...
    {
        /*skip device add*/
        if((appDemoDbEntryGet("appDemoSkipLpmDeviceAdd", &tmpValue) == GT_OK) && (tmpValue != 0))
        {
            return GT_OK;
        }
...
)
{
    ...
    /* PHA is not supported for GM */
    if (PRV_CPSS_PP_MAC(dev)->isGmDevice)
    {
        return GT_OK;
    }
...
extern GT_VOID* appDemoMemCacheDmaMalloc
(
    IN GT_U8  devNum,
    IN GT_U32 size
);

/* NOTE: relevant to SIP7 that have CNC DMA per unit , that is not shared with the FUQ */
static GT_STATUS prvCncLibInit
(
    IN  GT_U8                       devNum,
    IN  APP_DEMO_LIB_INIT_PARAMS    *libInitParamsPtr,
    IN  CPSS_PP_CONFIG_INIT_STC     *sysConfigParamsPtr
...
{
    GT_STATUS   rc;
    GT_U32      cncUnitId,value;
    GT_U8       dmaWindow;
    GT_U32      cncDumpSizeInBytes;
    CPSS_DMA_QUEUE_CFG_STC  cncCfg;

    sysConfigParamsPtr = sysConfigParamsPtr;
    ...
    if (libInitParamsPtr->initCncDump == GT_FALSE)
    {
        return GT_OK;
    }

    if(appDemoDbEntryGet("skipCncDumpInit", &value) == GT_OK)
    {
        return GT_OK;
    }

    if(appDemoDbEntryGet("cncDumpSizeInBytes", &value) == GT_OK)
    {
        cncDumpSizeInBytes = value;
    }
    else
    {
        GT_U32  singleDumpSize  = _8K/*counters*/ * 16 /*bytes : counters of 128bits*/;
        GT_U32  maxSingleSgd = _64K;
        GT_U32  numChunksByCpss;
        GT_U32  overheadByCpss;

        cncDumpSizeInBytes = PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfPipes *
            singleDumpSize * 4;/* size for 4 dumps */

        /* calc the num of chunks that CPSS will split */
        /* NOTE : the 1K comes from the way CPSS internally calls to :
           prvCpssDxChGdmaCncMsgInit(...) */
        if(maxSingleSgd > singleDumpSize)
        {
            numChunksByCpss = cncDumpSizeInBytes / singleDumpSize;
        }
        else
        {
            numChunksByCpss = cncDumpSizeInBytes / maxSingleSgd;
        }

        overheadByCpss = numChunksByCpss * 16/*bytes for SGD for the chunk*/;

        /* add the overhead by the CPSS */
        cncDumpSizeInBytes += overheadByCpss;
    }
    ...
    */
    ...
    dmaWindow = 0;
    {
        ...
        {
            if(appDemoPpConfigList[ii].valid && appDemoPpConfigList[ii].devNum == devNum)
            {
                dmaWindow = (GT_U8)appDemoPpConfigList[ii].dmaWindow;
                break;
            }
        }
    }
    ...
    for(cncUnitId = 0 ;
        cncUnitId < PRV_CPSS_DXCH_PP_HW_INFO_CNC_MAC(devNum).cncUnits ;
        cncUnitId++)
    {
        cncCfg.dmaDescBlock     = appDemoMemCacheDmaMalloc(dmaWindow,cncDumpSizeInBytes);
        cncCfg.dmaDescBlockSize = cncDumpSizeInBytes;

        if(cncCfg.dmaDescBlock == NULL)
        {
            rc = GT_OUT_OF_CPU_MEM;
            CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCncLibInit : appDemoMemCacheDmaMalloc", rc);
            return rc;
        }

        rc = cpssDxChCncUploadInit(CAST_SW_DEVNUM(devNum),cncUnitId,
            &cncCfg);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCncLibInit : cpssDxChCncUploadInit", rc);
        if (GT_OK != rc)
        {
            return rc;
        }
...
)
{
    ...
    rc = waTriggerInterruptsMask(devNum, portNum, iface, CPSS_EVENT_MASK_E);
    if(rc != GT_OK)
    {
        return rc;
    }
...
/* #ifndef ASIC_SIMULATION */
    ...
    if(CPSS_PORT_INTERFACE_MODE_RXAUI_E == ifMode)
    {
        ...
        cpssOsTimeRT(&secondsStart, &nanoSecondsStart);
        rc = cpssDxChHwPpImplementWaInit(
            devNum,1,&waArr[0],  &additionalInfoBmpArr[0]);
        if (prvAppDemoFirstInitTimes.initSystemTimePpImplementWaInit == 0)
        {
            prvAppDemoFirstInitTimes.initSystemTimePpImplementWaInit =
                prvAppDemoGetDurationTimeMilliSec(secondsStart, nanoSecondsStart);
...
)
{
    ...
    for(portNum = 0; portNum < (appDemoPpConfigList[devIdx].maxPortNumber); portNum++)
    {
        ...
        if(GT_FALSE ==PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            ...
            rc = cpssDxChPortPeriodicFcEnableSet(dev, portNum, CPSS_PORT_PERIODIC_FLOW_CONTROL_XON_XOFF_E);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortPeriodicFcEnableSet", rc);
            if(rc != GT_OK)
                return rc;
        }
    ...
    rc = cpssDxChPortTxTailDropProfileSet(
        dev, tailDropForPfcProfileSet, CPSS_PORT_TX_TAIL_DROP_DBA_ALPHA_0_E,
        dpMaxBuffNum[5], dpMaxDescrNum[5]);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChPortTxTailDropProfileSet", rc);
    if(rc != GT_OK)
        return rc;
...
/**
* @internal appDemoDxChHwPpPhase1ResetAndInit function
* @endinternal
*
* @brief   This function performs basic hardware configurations on the given PP, in
*         Hw registration phase - supported reset and recall cpssDxChHwPpPhase1Init
*         According to mode and HW status.
*
...
*/
GT_STATUS appDemoDxChHwPpPhase1ResetAndInit
(
...
{
    GT_STATUS rc;
    GT_U32    forceSoftResetInInit;
    CPSS_SYSTEM_RECOVERY_INFO_STC systemRecoverysInfo;
    GT_U32    secondsStart;
    GT_U32    nanoSecondsStart;

    forceSoftResetInInit = 0;

    /* save phase 1 parameters to local DB */
    ppPhase1ParamsDb = *ppPhase1ParamsPtr;

    rc = appDemoDbEntryGet("forceSoftResetInInit", &forceSoftResetInInit);
    if (rc != GT_OK)
    {
        forceSoftResetInInit = 0;
    }

    rc = cpssSystemRecoveryStateGet(&systemRecoverysInfo);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    cpssOsTimeRT(&secondsStart, &nanoSecondsStart);
    rc = cpssDxChHwPpPhase1Init(ppPhase1ParamsPtr, deviceTypePtr);
    ...
    if (systemRecoverysInfo.systemRecoveryProcess
        != CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
    {
        return rc;
    }
    if ((rc != GT_HW_ERROR_NEED_RESET) && (forceSoftResetInInit == 0))
    {
        /* Phase1 did not require the second call */
        return rc;
    }

    if ((PRV_CPSS_PP_MAC(ppPhase1ParamsPtr->devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E) &&
        (ppPhase1ParamsPtr->hwInfo[0].busType == CPSS_HW_INFO_BUS_TYPE_MBUS_E))
    {
        /* IronMan with internal CPU does not support soft reset of Switch
           because internal CPU get reset also.*/
        osPrintf("IronMan device already was initialized. Need to make full Soft Reset or HW Reset.\n");
        return rc;
    }

    osPrintSync("RESET and Remove started\n");
    doAtomicDeviceResetAndRemove(ppPhase1ParamsPtr->devNum);
    osPrintSync("RESET and Remove ended\n");
    osTimerWkAfter(100);
    osPrintSync("PHASE1 started\n");
    cpssOsTimeRT(&secondsStart, &nanoSecondsStart);
    rc = cpssDxChHwPpPhase1Init(ppPhase1ParamsPtr, deviceTypePtr);
    if (prvAppDemoFirstInitTimes.initSystemTimePpPhase1AfterReset == 0)
    {
        prvAppDemoFirstInitTimes.initSystemTimePpPhase1AfterReset =
            prvAppDemoGetDurationTimeMilliSec(secondsStart, nanoSecondsStart);
    }
    osPrintSync("PHASE1 ended\n");
...
)
{
    ...
    {
        rc = prvCpssDxChGdmaNetIfNumQueuesGet(devNum,&toCpuNumQueues,NULL/*fromCpu*/);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDxChGdmaNetIfNumQueuesGet", rc);
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    else
    {
        ...
        toCpuNumQueues = 8*numOfNetIfs;

        if(PRV_CPSS_SIP_5_CHECK_MAC(devNum))
        {
            /* Disable AUQ/FUQ and CNC upload messages */
            rc = cpssDxChCfgAuqFuqDisable(devNum);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChCfgAuqFuqDisable", rc);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    for(queueIdx = 0; queueIdx < toCpuNumQueues ; queueIdx++)
    {
...
/* mechaninism for manual disabling access to unit for debugging */

/* bitmap: bit per unit 0 - enable, 1 - disable */
/* size for one tile                            */
static GT_U32 appDemoDebug_unitAccessDisableBitmapArr[(PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E + 31) / 32] = {0};

/* callback to bind */
static GT_U32 appDemoDebug_unitAccessEnableCheckCallbackFunc(GT_U8 devNum, GT_U32 unitId)
{
    /* reduce unitId to tile0 - the same units enabled/disabled in all tiles */
    unitId %= PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E;
    devNum = devNum; /* not used patameter */
    if (appDemoDebug_unitAccessDisableBitmapArr[unitId / 32] & (1 << (unitId % 32)))
    {
        return 0; /* disable */
    }
    return 1; /* enable */
...
/* function to call from command line, bind == 1 - to bind, bind == 0 - to unbind */
GT_VOID appDemoDebug_unitAccessEnableCheckCallbackSet(GT_U32 bind)
{
    GT_STATUS rc;
    GT_BOOL cpssAslrSupport ;

    /* cpssGlobalDbInit does nothing for already initialysed DB (see implementation) */
    /* The DB usually initialized in cpssPpInit by calling cpssGlobalDbInit.         */
    /* This code copied from it to enable callback binding before init-system.       */
    /* The DB will be initialized before init-system exactly as inside of it.        */
    /* So the second cpssGlobalDbInit call from cpssPpInit will change nothing.      */
    ...
    rc = cpssGlobalDbInit(cpssAslrSupport, GT_FALSE,0,0);
    if(rc != GT_OK)
    {
        cpssOsPrintf(
            "appDemoDebug_unitAccessEnableCheckCallbackSet: cpssGlobalDbInit failed\n");
        return;
    }

    if (bind)
    {
        debug_unitAccessEnableCheckCallbackSet(
            &appDemoDebug_unitAccessEnableCheckCallbackFunc);
    }
    else
    {
        debug_unitAccessEnableCheckCallbackSet(NULL);
    }
}

#define STR_TO_UNIT(_name) {#_name, PRV_CPSS_DXCH_UNIT_ ## _name ## _E}
static const struct
{
    const char* name;
    const PRV_CPSS_DXCH_UNIT_ENT unitId;
} unitNameToIdArr[] =
{
    STR_TO_UNIT(TTI),
    STR_TO_UNIT(PCL),
    ...
    STR_TO_UNIT(ERMRK),
    STR_TO_UNIT(BM),
    STR_TO_UNIT(TM_INGRESS_GLUE),
    STR_TO_UNIT(TM_EGRESS_GLUE),
    STR_TO_UNIT(ETH_TXFIFO),
    STR_TO_UNIT(ILKN_TXFIFO),
    STR_TO_UNIT(ILKN),
    STR_TO_UNIT(LMS0_0),
    STR_TO_UNIT(LMS0_1),
    STR_TO_UNIT(LMS0_2),
    ...
    STR_TO_UNIT(CNC_0),
    STR_TO_UNIT(CNC_1),
    STR_TO_UNIT(TXQ_QUEUE),
    STR_TO_UNIT(TXQ_LL),
    STR_TO_UNIT(TXQ_PFC),
    STR_TO_UNIT(TXQ_QCN),
    STR_TO_UNIT(TXQ_DQ),
    STR_TO_UNIT(DFX_SERVER),
    STR_TO_UNIT(MPPM),
    STR_TO_UNIT(LMS1_0),
    STR_TO_UNIT(LMS1_1),
    STR_TO_UNIT(LMS1_2),
    ...
    STR_TO_UNIT(CTU_1),
    STR_TO_UNIT(TXQ_SHT),
    STR_TO_UNIT(TXQ_EGR0),
    STR_TO_UNIT(TXQ_EGR1),
    STR_TO_UNIT(TXQ_DIST),
    STR_TO_UNIT(IPLR_1),
    STR_TO_UNIT(TXDMA),
    STR_TO_UNIT(MG),
    STR_TO_UNIT(TCC_IPCL),
    STR_TO_UNIT(TCC_IPVX),
    STR_TO_UNIT(TCC_IPCL_1),
    STR_TO_UNIT(CNM_RFU),
    STR_TO_UNIT(CNM_MPP_RFU),
    STR_TO_UNIT(CNM_AAC),
    STR_TO_UNIT(CNM_RUNIT),
    STR_TO_UNIT(CNM_ADDR_DECODER),
    STR_TO_UNIT(SMI_0),
    STR_TO_UNIT(SMI_1),
    STR_TO_UNIT(SMI_2),
    STR_TO_UNIT(SMI_3),
    ...
    STR_TO_UNIT(RXDMA1),
    STR_TO_UNIT(TXDMA1),
    STR_TO_UNIT(TX_FIFO1),
    STR_TO_UNIT(ETH_TXFIFO1),
    STR_TO_UNIT(RXDMA_GLUE),
    STR_TO_UNIT(TXDMA_GLUE),
    STR_TO_UNIT(RXDMA2),
    STR_TO_UNIT(RXDMA3),
    STR_TO_UNIT(RXDMA4),
    STR_TO_UNIT(RXDMA5),
    ...
    STR_TO_UNIT(TXDMA6),
    STR_TO_UNIT(TXDMA7),
    STR_TO_UNIT(TX_FIFO2),
    STR_TO_UNIT(TX_FIFO3),
    STR_TO_UNIT(TX_FIFO4),
    STR_TO_UNIT(TX_FIFO5),
    STR_TO_UNIT(TX_FIFO6),
    STR_TO_UNIT(TX_FIFO7),
    STR_TO_UNIT(TXQ_DQ1),
    STR_TO_UNIT(TXQ_DQ2),
    STR_TO_UNIT(TXQ_DQ3),
    STR_TO_UNIT(TXQ_DQ4),
    ...
    STR_TO_UNIT(TAI3),
    STR_TO_UNIT(TAI4),
    STR_TO_UNIT(EGF_SHT_1),
    STR_TO_UNIT(TTI_1),
    STR_TO_UNIT(IPCL_1),
    STR_TO_UNIT(L2I_1),
    STR_TO_UNIT(IPVX_1),
    STR_TO_UNIT(IPLR_0_1),
    STR_TO_UNIT(IPLR_1_1),
    STR_TO_UNIT(IOAM_1),
    STR_TO_UNIT(MLL_1),
    STR_TO_UNIT(EQ_1),
    STR_TO_UNIT(EGF_EFT_1),
    STR_TO_UNIT(CNC_0_1),
    STR_TO_UNIT(CNC_1_1),
    STR_TO_UNIT(SERDES_1),
    STR_TO_UNIT(HA_1),
    STR_TO_UNIT(ERMRK_1),
    STR_TO_UNIT(EPCL_1),
    STR_TO_UNIT(EPLR_1),
    STR_TO_UNIT(EOAM_1),
    STR_TO_UNIT(RX_DMA_GLUE_1),
    STR_TO_UNIT(POE),
    STR_TO_UNIT(TXQ_BMX),
    STR_TO_UNIT(LPM_1),
    STR_TO_UNIT(IA),
    ...
    STR_TO_UNIT(MG_1_3),
    STR_TO_UNIT(BMA_1),
    STR_TO_UNIT(EGF_QAG_1),
    STR_TO_UNIT(TXQ_PSI),
    STR_TO_UNIT(TXQ_PDX),
    STR_TO_UNIT(TXQ_PDX_PAC_0),
    STR_TO_UNIT(TXQ_PDX_PAC_1),
    STR_TO_UNIT(TXQ_PFCC),
    STR_TO_UNIT(EM),
    STR_TO_UNIT(TXQ_PIPE0_PDS0),
    STR_TO_UNIT(TXQ_PIPE0_PDS1),
    STR_TO_UNIT(TXQ_PIPE0_PDS2),
    STR_TO_UNIT(TXQ_PIPE0_PDS3),
    STR_TO_UNIT(TXQ_PIPE1_PDS0),
    STR_TO_UNIT(TXQ_PIPE1_PDS1),
    STR_TO_UNIT(TXQ_PIPE1_PDS2),
    STR_TO_UNIT(TXQ_PIPE1_PDS3),
    STR_TO_UNIT(TXQ_PIPE0_SDQ0),
    STR_TO_UNIT(TXQ_PIPE0_SDQ1),
    STR_TO_UNIT(TXQ_PIPE0_SDQ2),
    STR_TO_UNIT(TXQ_PIPE0_SDQ3),
    STR_TO_UNIT(TXQ_PIPE1_SDQ0),
    STR_TO_UNIT(TXQ_PIPE1_SDQ1),
    STR_TO_UNIT(TXQ_PIPE1_SDQ2),
    STR_TO_UNIT(TXQ_PIPE1_SDQ3),
    STR_TO_UNIT(TXQ_PIPE0_QFC0),
    STR_TO_UNIT(TXQ_PIPE0_QFC1),
    STR_TO_UNIT(TXQ_PIPE0_QFC2),
    STR_TO_UNIT(TXQ_PIPE0_QFC3),
    STR_TO_UNIT(TXQ_PIPE1_QFC0),
    STR_TO_UNIT(TXQ_PIPE1_QFC1),
    STR_TO_UNIT(TXQ_PIPE1_QFC2),
    STR_TO_UNIT(TXQ_PIPE1_QFC3),
    STR_TO_UNIT(TAI_SLAVE_PIPE0_TAI0),
    STR_TO_UNIT(TAI_SLAVE_PIPE0_TAI1),
    STR_TO_UNIT(TAI_SLAVE_PIPE1_TAI0),
    STR_TO_UNIT(TAI_SLAVE_PIPE1_TAI1),
    STR_TO_UNIT(TXQ_TAI_SLAVE_PIPE0_TAI0),
    STR_TO_UNIT(TXQ_TAI_SLAVE_PIPE0_TAI1),
    STR_TO_UNIT(TXQ_TAI_SLAVE_PIPE1_TAI0),
    STR_TO_UNIT(TXQ_TAI_SLAVE_PIPE1_TAI1),
    STR_TO_UNIT(PB_CENTER_BLK),
    STR_TO_UNIT(PB_COUNTER),
    STR_TO_UNIT(PB_SMB_WRITE_ARBITER),
    STR_TO_UNIT(PB_SMB_WRITE_ARBITER_1),
    STR_TO_UNIT(PB_SMB_WRITE_ARBITER_2),
    STR_TO_UNIT(PB_SMB_WRITE_ARBITER_3),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_WRITE_0),
    STR_TO_UNIT(PB_GPC_GRP_CELL_READ_0),
    STR_TO_UNIT(PB_GPC_GRP_CELL_READ_1),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_READ_0),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_READ_1),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_WRITE_1),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_WRITE_2),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_WRITE_3),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_WRITE_4),
    STR_TO_UNIT(PB_GPC_GRP_CELL_READ_2),
    STR_TO_UNIT(PB_GPC_GRP_CELL_READ_3),
    STR_TO_UNIT(PB_GPC_GRP_CELL_READ_4),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_READ_2),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_READ_3),
    STR_TO_UNIT(PB_GPC_GRP_PACKET_READ_4),
    STR_TO_UNIT(PB_NEXT_POINTER_MEMO_0),
    STR_TO_UNIT(PB_NEXT_POINTER_MEMO_1),
    STR_TO_UNIT(PB_NEXT_POINTER_MEMO_2),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_0_0),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_0_1),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_1_0),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_1_1),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_2_0),
    STR_TO_UNIT(PB_SHARED_MEMO_BUF_2_1),
    STR_TO_UNIT(PCA_BRG_0),
    STR_TO_UNIT(PCA_PZ_ARBITER_I_0),
    STR_TO_UNIT(PCA_PZ_ARBITER_E_0),
    STR_TO_UNIT(PCA_SFF_0),
    STR_TO_UNIT(PCA_LMU_0),
    STR_TO_UNIT(PCA_CTSU_0),
    STR_TO_UNIT(PCA_BRG_1),
    STR_TO_UNIT(PCA_PZ_ARBITER_I_1),
    STR_TO_UNIT(PCA_PZ_ARBITER_E_1),
    STR_TO_UNIT(PCA_SFF_1),
    STR_TO_UNIT(PCA_LMU_1),
    STR_TO_UNIT(PCA_CTSU_1),
    STR_TO_UNIT(PCA_BRG_2),
    STR_TO_UNIT(PCA_PZ_ARBITER_I_2),
    STR_TO_UNIT(PCA_PZ_ARBITER_E_2),
    STR_TO_UNIT(PCA_SFF_2),
    STR_TO_UNIT(PCA_LMU_2),
    STR_TO_UNIT(PCA_CTSU_2),
    STR_TO_UNIT(PCA_BRG_3),
    STR_TO_UNIT(PCA_PZ_ARBITER_I_3),
    STR_TO_UNIT(PCA_PZ_ARBITER_E_3),
    STR_TO_UNIT(PCA_SFF_3),
    STR_TO_UNIT(PCA_LMU_3),
    STR_TO_UNIT(PCA_CTSU_3),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_163_0),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_164_0),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_163_0),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_164_0),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_163_1),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_164_1),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_163_1),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_164_1),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_163_2),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_164_2),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_163_2),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_164_2),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_163_3),
    STR_TO_UNIT(PCA_MACSEC_EXT_I_164_3),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_163_3),
    STR_TO_UNIT(PCA_MACSEC_EXT_E_164_3),
    STR_TO_UNIT(PCA_MACSEC_EIP_163_I_0),
    STR_TO_UNIT(PCA_MACSEC_EIP_163_E_0),
    STR_TO_UNIT(PCA_MACSEC_EIP_164_I_0),
    STR_TO_UNIT(PCA_MACSEC_EIP_164_E_0),
    STR_TO_UNIT(PCA_MACSEC_EIP_163_I_1),
    STR_TO_UNIT(PCA_MACSEC_EIP_163_E_1),
    STR_TO_UNIT(PCA_MACSEC_EIP_164_I_1),
    STR_TO_UNIT(PCA_MACSEC_EIP_164_E_1),
    STR_TO_UNIT(PCA_BRG_EDGE_RX_0),
    STR_TO_UNIT(PCA_BRG_EDGE_RX_1),
    STR_TO_UNIT(PCA_BRG_EDGE_RX_2),
    STR_TO_UNIT(PCA_BRG_EDGE_RX_3),
    STR_TO_UNIT(PCA_BRG_EDGE_TX_0),
    STR_TO_UNIT(PCA_BRG_EDGE_TX_1),
    STR_TO_UNIT(PCA_BRG_EDGE_TX_2),
    STR_TO_UNIT(PCA_BRG_EDGE_TX_3),
    STR_TO_UNIT(PCA_BRG_PACK_RX_0),
    STR_TO_UNIT(PCA_BRG_PACK_RX_1),
    STR_TO_UNIT(PCA_BRG_PACK_RX_2),
    STR_TO_UNIT(PCA_BRG_PACK_RX_3),
    STR_TO_UNIT(PCA_BRG_UNPACK_TX_0),
    STR_TO_UNIT(PCA_BRG_UNPACK_TX_1),
    STR_TO_UNIT(PCA_BRG_UNPACK_TX_2),
    STR_TO_UNIT(PCA_BRG_UNPACK_TX_3),
    STR_TO_UNIT(PCA_BRG_SHM_RX_0),
    STR_TO_UNIT(PCA_BRG_SHM_RX_1),
    STR_TO_UNIT(PCA_BRG_SHM_RX_2),
    STR_TO_UNIT(PCA_BRG_SHM_RX_3),
    STR_TO_UNIT(PCA_BRG_SHM_TX_0),
    STR_TO_UNIT(PCA_BRG_SHM_TX_1),
    STR_TO_UNIT(PCA_BRG_SHM_TX_2),
    STR_TO_UNIT(PCA_BRG_SHM_TX_3),
    STR_TO_UNIT(BASE_RAVEN_0),
    STR_TO_UNIT(BASE_RAVEN_1),
    STR_TO_UNIT(BASE_RAVEN_2),
    STR_TO_UNIT(BASE_RAVEN_3),
    STR_TO_UNIT(RAVEN_0_TAI0),
    STR_TO_UNIT(RAVEN_0_TAI1),
    STR_TO_UNIT(RAVEN_1_TAI0),
    STR_TO_UNIT(RAVEN_1_TAI1),
    STR_TO_UNIT(RAVEN_2_TAI0),
    STR_TO_UNIT(RAVEN_2_TAI1),
    STR_TO_UNIT(RAVEN_3_TAI0),
    STR_TO_UNIT(RAVEN_3_TAI1),
    STR_TO_UNIT(EAGLE_D2D_0),
    STR_TO_UNIT(EAGLE_D2D_1),
    STR_TO_UNIT(EAGLE_D2D_2),
    STR_TO_UNIT(EAGLE_D2D_3),
    STR_TO_UNIT(EAGLE_D2D_4),
    STR_TO_UNIT(EAGLE_D2D_5),
    STR_TO_UNIT(EAGLE_D2D_6),
    STR_TO_UNIT(EAGLE_D2D_7),
    STR_TO_UNIT(EAGLE_D2D_CP_IN_RAVEN),
    STR_TO_UNIT(MAC_400G_0),
    STR_TO_UNIT(MAC_400G_1),
    STR_TO_UNIT(MAC_400G_2),
    STR_TO_UNIT(MAC_400G_3),
    STR_TO_UNIT(MAC_400G_4),
    STR_TO_UNIT(MAC_400G_5),
    STR_TO_UNIT(MAC_400G_6),
    STR_TO_UNIT(MAC_400G_7),
    STR_TO_UNIT(PCS_400G_0),
    STR_TO_UNIT(PCS_400G_1),
    STR_TO_UNIT(PCS_400G_2),
    STR_TO_UNIT(PCS_400G_3),
    STR_TO_UNIT(MAC_CPU_0),
    STR_TO_UNIT(PCS_CPU_0),
    STR_TO_UNIT(LED_CPU_0),
    STR_TO_UNIT(SDW_CPU_0),
    STR_TO_UNIT(CNC_2),
    STR_TO_UNIT(CNC_3),
    STR_TO_UNIT(PPU),
    STR_TO_UNIT(PPU_FOR_IPE),
    STR_TO_UNIT(MIF_800G_0),
    STR_TO_UNIT(MIF_800G_1),
    STR_TO_UNIT(MIF_800G_2),
    STR_TO_UNIT(MIF_800G_3),
    STR_TO_UNIT(MIF_400G_0),
    STR_TO_UNIT(MIF_400G_1),
    STR_TO_UNIT(MIF_400G_2),
    STR_TO_UNIT(MIF_400G_3),
    STR_TO_UNIT(MIF_4P_0),
    STR_TO_UNIT(MIF_4P_1),
    STR_TO_UNIT(MIF_4P_2),
    STR_TO_UNIT(MIF_4P_3),
    STR_TO_UNIT(MIF_USX_0),
    STR_TO_UNIT(MIF_USX_1),
    STR_TO_UNIT(MIF_USX_2),
    STR_TO_UNIT(MIF_CPU_0),
    STR_TO_UNIT(MIF_RP_0),
    STR_TO_UNIT(MIF_RP_1),
    STR_TO_UNIT(MIF_RP_2),
    STR_TO_UNIT(MIF_RP_3),
    STR_TO_UNIT(ANP_400G_0),
    STR_TO_UNIT(ANP_400G_1),
    STR_TO_UNIT(ANP_400G_2),
    STR_TO_UNIT(ANP_400G_3),
    STR_TO_UNIT(ANP_400G_4),
    STR_TO_UNIT(ANP_400G_5),
    STR_TO_UNIT(ANP_400G_6),
    STR_TO_UNIT(ANP_400G_7),
    STR_TO_UNIT(ANP_USX_0),
    STR_TO_UNIT(ANP_USX_1),
    STR_TO_UNIT(ANP_USX_2),
    STR_TO_UNIT(ANP_USX_3),
    STR_TO_UNIT(ANP_USX_4),
    STR_TO_UNIT(ANP_USX_5),
    STR_TO_UNIT(ANP_USX_6),
    STR_TO_UNIT(ANP_USX_7),
    STR_TO_UNIT(ANP_USX_8),
    STR_TO_UNIT(ANP_USX_9),
    STR_TO_UNIT(ANP_USX_10),
    STR_TO_UNIT(ANP_USX_11),
    STR_TO_UNIT(ANP_CPU_0),
    STR_TO_UNIT(ANP_CPU_1),
    STR_TO_UNIT(ANP_CPU_2),
    STR_TO_UNIT(ANP_CPU_3),
    STR_TO_UNIT(ANP_CPU_4),
    STR_TO_UNIT(ANP_CPU_5),
    STR_TO_UNIT(ANP_CPU_6),
    STR_TO_UNIT(MTI_USX_0_MAC_0),
    STR_TO_UNIT(MTI_USX_1_MAC_0),
    STR_TO_UNIT(MTI_USX_2_MAC_0),
    STR_TO_UNIT(MTI_USX_0_MAC_1),
    STR_TO_UNIT(MTI_USX_1_MAC_1),
    STR_TO_UNIT(MTI_USX_2_MAC_1),
    STR_TO_UNIT(MTI_USX_0_PCS_0),
    STR_TO_UNIT(MTI_USX_1_PCS_0),
    STR_TO_UNIT(MTI_USX_2_PCS_0),
    STR_TO_UNIT(MTI_USX_0_PCS_1),
    STR_TO_UNIT(MTI_USX_1_PCS_1),
    STR_TO_UNIT(MTI_USX_2_PCS_1),
    STR_TO_UNIT(L2I_TAI0),
    STR_TO_UNIT(L2I_TAI1),
    STR_TO_UNIT(TTI_TAI0),
    STR_TO_UNIT(TTI_TAI1),
    STR_TO_UNIT(TXQS_TAI0),
    STR_TO_UNIT(TXQS_TAI1),
    STR_TO_UNIT(TXQS1_TAI0),
    STR_TO_UNIT(TXQS1_TAI1),
    STR_TO_UNIT(PHA_TAI0),
    STR_TO_UNIT(PHA_TAI1),
    STR_TO_UNIT(EPCL_HA_TAI0),
    STR_TO_UNIT(EPCL_HA_TAI1),
    STR_TO_UNIT(CPU_PORT_TAI0),
    STR_TO_UNIT(CPU_PORT_TAI1),
    STR_TO_UNIT(400G0_TAI0),
    STR_TO_UNIT(400G0_TAI1),
    STR_TO_UNIT(400G1_TAI0),
    STR_TO_UNIT(400G1_TAI1),
    STR_TO_UNIT(400G2_TAI0),
    STR_TO_UNIT(400G2_TAI1),
    STR_TO_UNIT(400G3_TAI0),
    STR_TO_UNIT(400G3_TAI1),
    STR_TO_UNIT(USX0_TAI0),
    STR_TO_UNIT(USX0_TAI1),
    STR_TO_UNIT(USX1_TAI0),
    STR_TO_UNIT(USX1_TAI1),
    STR_TO_UNIT(PCA2_TAI0),
    STR_TO_UNIT(PCA2_TAI1),
    STR_TO_UNIT(PCA3_TAI0),
    STR_TO_UNIT(PCA3_TAI1),
    STR_TO_UNIT(PCA0_TAI0),
    STR_TO_UNIT(PCA0_TAI1),
    STR_TO_UNIT(PCA1_TAI0),
    STR_TO_UNIT(PCA1_TAI1),
    STR_TO_UNIT(MAC_CPUM_TAI0),
    STR_TO_UNIT(MAC_CPUM_TAI1),
    STR_TO_UNIT(MAC_CPUC_TAI0),
    STR_TO_UNIT(MAC_CPUC_TAI1),
    STR_TO_UNIT(100G_TAI0),
    STR_TO_UNIT(100G_TAI1),
    STR_TO_UNIT(USX2_0_TAI0),
    STR_TO_UNIT(USX2_0_TAI1),
    STR_TO_UNIT(USX2_1_TAI0),
    STR_TO_UNIT(USX2_1_TAI1),
    STR_TO_UNIT(MAC_CPU_1),
    STR_TO_UNIT(PCS_CPU_1),
    STR_TO_UNIT(SDW0_0),
    STR_TO_UNIT(SDW1_0),
    STR_TO_UNIT(SDW0_1),
    STR_TO_UNIT(SDW1_1),
    ...
    STR_TO_UNIT(HBU_1),
    STR_TO_UNIT(SMU),
    STR_TO_UNIT(MTI_10G_MAC_BR_0),
    STR_TO_UNIT(MTI_10G_MAC_BR_1),
    STR_TO_UNIT(MTI_10G_MAC_BR_2),
    STR_TO_UNIT(MTI_10G_MAC_BR_3),
    STR_TO_UNIT(MTI_10G_MAC_BR_4),
    STR_TO_UNIT(MTI_10G_MAC_BR_5),
    STR_TO_UNIT(MTI_10G_MAC_BR_6),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_0),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_1),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_2),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_3),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_4),
    STR_TO_UNIT(MTI_10G_MAC_NON_BR_5),
    STR_TO_UNIT(MTI_10G_PCS_x1_0),
    STR_TO_UNIT(MTI_10G_PCS_x1_1),
    STR_TO_UNIT(MTI_10G_PCS_x1_2),
    STR_TO_UNIT(MTI_10G_PCS_x1_3),
    STR_TO_UNIT(MTI_10G_PCS_x1_4),
    STR_TO_UNIT(MTI_10G_PCS_x1_5),
    STR_TO_UNIT(MTI_10G_PCS_x1_6),
    STR_TO_UNIT(MTI_10G_PCS_x8_0),
    STR_TO_UNIT(MTI_10G_PCS_x8_1),
    STR_TO_UNIT(MTI_10G_PCS_x8_2),
    STR_TO_UNIT(MTI_10G_PCS_x8_3),
    STR_TO_UNIT(MTI_10G_PCS_x8_4),
    STR_TO_UNIT(MTI_10G_PCS_x8_5),
    STR_TO_UNIT(PCA_PGC_TX),
    STR_TO_UNIT(PCA_PGC_RX),
    STR_TO_UNIT(EMX),
    STR_TO_UNIT(TTI_LU),
    STR_TO_UNIT(MPCL),
    STR_TO_UNIT(EM_1),
    ...
    STR_TO_UNIT(EM_6),
    STR_TO_UNIT(EM_7),
    STR_TO_UNIT(REDUCED_EM),
    STR_TO_UNIT(IPE),
    STR_TO_UNIT(CNM_GDMA_0),
    STR_TO_UNIT(CNM_GDMA_1),
    STR_TO_UNIT(CNM_GDMA_2),
    STR_TO_UNIT(CNM_GDMA_3),
    STR_TO_UNIT(CNM_GDMA_DISPATCHER),
    STR_TO_UNIT(CNM_INTERRUPTS),
    STR_TO_UNIT(CNM_PERIPHERALS),
    STR_TO_UNIT(MAC_CPU_100G_0),
    STR_TO_UNIT(MAC_CPU_100G_1),
    STR_TO_UNIT(MAC_CPU_100G_2),
    STR_TO_UNIT(MAC_CPU_100G_3),
    STR_TO_UNIT(MAC_CPU_100G_4),
    STR_TO_UNIT(MAC_CPU_100G_5),
    STR_TO_UNIT(MAC_CPU_100G_6),
    STR_TO_UNIT(MAC_CPU_100G_7),
    STR_TO_UNIT(MAC_CPU_100G_8),
    STR_TO_UNIT(MAC_CPU_100G_9),
    STR_TO_UNIT(MAC_CPU_100G_10),
    STR_TO_UNIT(MAC_CPU_100G_11),
    STR_TO_UNIT(MAC_CPU_100G_12),
    STR_TO_UNIT(MAC_CPU_100G_13),
    STR_TO_UNIT(MAC_CPU_100G_14),
    STR_TO_UNIT(MAC_CPU_100G_15),
    STR_TO_UNIT(PCS_CPU_100G_0),
    STR_TO_UNIT(PCS_CPU_100G_1),
    STR_TO_UNIT(PCS_CPU_100G_2),
    STR_TO_UNIT(PCS_CPU_100G_3),
    STR_TO_UNIT(PCS_CPU_100G_4),
    STR_TO_UNIT(PCS_CPU_100G_5),
    STR_TO_UNIT(PCS_CPU_100G_6),
    STR_TO_UNIT(PCS_CPU_100G_7),
    STR_TO_UNIT(PCS_CPU_100G_8),
    STR_TO_UNIT(PCS_CPU_100G_9),
    STR_TO_UNIT(PCS_CPU_100G_10),
    STR_TO_UNIT(PCS_CPU_100G_11),
    STR_TO_UNIT(PCS_CPU_100G_12),
    STR_TO_UNIT(PCS_CPU_100G_13),
    STR_TO_UNIT(PCS_CPU_100G_14),
    STR_TO_UNIT(PCS_CPU_100G_15),
    STR_TO_UNIT(ANP1_CPU_100G_0),
    STR_TO_UNIT(ANP1_CPU_100G_1),
    STR_TO_UNIT(ANP1_CPU_100G_2),
    STR_TO_UNIT(ANP1_CPU_100G_3),
    STR_TO_UNIT(ANP1_CPU_100G_4),
    STR_TO_UNIT(ANP1_CPU_100G_5),
    STR_TO_UNIT(ANP1_CPU_100G_6),
    STR_TO_UNIT(ANP1_CPU_100G_7),
    STR_TO_UNIT(ANP1_CPU_100G_8),
    STR_TO_UNIT(ANP1_CPU_100G_9),
    STR_TO_UNIT(ANP1_CPU_100G_10),
    STR_TO_UNIT(ANP1_CPU_100G_11),
    STR_TO_UNIT(ANP1_CPU_100G_12),
    STR_TO_UNIT(ANP1_CPU_100G_13),
    STR_TO_UNIT(ANP1_CPU_100G_14),
    STR_TO_UNIT(ANP1_CPU_100G_15),
    STR_TO_UNIT(SDW56G_CPU_GOP_0),
    STR_TO_UNIT(SDW56G_CPU_GOP_1),
    STR_TO_UNIT(SDW56G_CPU_GOP_2),
    STR_TO_UNIT(SDW56G_CPU_GOP_3)
};

/**
* @internal appDemoDebugUnitNameToUnitId function
* @endinternal
*
* @brief   Convert unit name string to unit id.
*
* @param[in]  name           - unit name or name-prefix string
* @param[in]  name_is_prefix - 0 - full name, other - name-prefix
* @param[in]  numOfSkip      - amount of found matching names to skip
*
* @retval Id of found unit, 0 if not found.
*/
GT_U32 appDemoDebugUnitNameToUnitId
(
    IN const char* name,
    IN GT_U32 name_is_prefix,
    IN GT_U32 numOfSkip
)
{
    GT_U32 arrSize = sizeof(unitNameToIdArr) / sizeof(unitNameToIdArr[0]);
    GT_U32 i;
    GT_U32 len;
    GT_U32 numOfFound;

    len = cpssOsStrlen(name);
    numOfFound = 0;
    for (i = 0; (i < arrSize); i++)
    {
        if (name_is_prefix)
        {
            /* prefix match */
            if (cpssOsStrNCmp(name, unitNameToIdArr[i].name, len) != 0) continue;
        }
        else
        {
            /* exact match */
            if (cpssOsStrCmp(name, unitNameToIdArr[i].name) != 0) continue;
        }
        numOfFound ++;
        if (numOfFound < numOfSkip) continue;

        /* found after given amount of skips */
        return unitNameToIdArr[i].unitId;
    }
    return PRV_CPSS_DXCH_UNIT___NOT_VALID___E;
}
...
* @endinternal
*
* @brief   Print all unit names matching prefix string.
*
* @param[in]  prefix         - unit name-prefix string
*
* @retval none.
*/
GT_VOID appDemoDebugUnitNamesByPrefixPrint
(
...
)
{
    ...
    len = cpssOsStrlen(prefix);
    numOfFound = 0;
    for (i = 0; (i < arrSize); i++)
    {
        if (cpssOsStrNCmp(prefix, unitNameToIdArr[i].name, len) != 0) continue;
        cpssOsPrintf("%s ", unitNameToIdArr[i].name);
        numOfFound ++;
        if ((numOfFound % 8) == 0)
        {
            cpssOsPrintf("\n");
...
/**
* @internal appDemoDebugUnitRegsSortedPrint function
* @endinternal
*
* @brief    Print the device unit registers sorted by address.
*
* @param[in] devNum                   - The device number
...
* @param[in] unitName                 - unit name string
* @param[in] sizeOfBufferInKilobytes  - size Of Buffer In Kilobytes
...
*
*/
GT_STATUS appDemoDebugUnitRegsSortedPrint
(
    IN    GT_U8                devNum,
    IN GT_PORT_GROUPS_BMP      portGroupsBmp,
    IN const char*             unitName,
    IN GT_U32                  sizeOfBufferInKilobytes,
    IN GT_U32                  tileIndex
)
{
    PRV_CPSS_DXCH_UNIT_ENT  prvUnitId;

    prvUnitId = (PRV_CPSS_DXCH_UNIT_ENT)appDemoDebugUnitNameToUnitId(
        unitName, 0/*name_is_prefix*/,0/*numOfSkip*/);
    if (PRV_CPSS_DXCH_UNIT___NOT_VALID___E)
    {
        cpssOsPrintf("Unit name [%s] not found in DB\n", unitName);
        return GT_BAD_PARAM;
    }

    prvUnitId += (PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E * tileIndex);

    return prvCpssDxChDiagPrvUnitRegsSortedPrint(
        devNum, portGroupsBmp, prvUnitId, sizeOfBufferInKilobytes);
}

 /* function to call from command line */
GT_VOID appDemoDebug_unitAccessDisableSet(const char* name, GT_U32 name_is_prefix, GT_U32 disable)
{
    GT_U32 arrSize = sizeof(unitNameToIdArr) / sizeof(unitNameToIdArr[0]);
    GT_U32 i;
    GT_U32 len;
    ...
    for (i = 0; (i < arrSize); i++)
    {
        ...
        unitId = unitNameToIdArr[i].unitId;
        if (unitId >= PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E)
        {
            continue;
        }
        ...
        if (disable)
        {
            appDemoDebug_unitAccessDisableBitmapArr[unitId / 32] |= (1 << (unitId % 32));
        }
        else
        {
            appDemoDebug_unitAccessDisableBitmapArr[unitId / 32] &= (~ (1 << (unitId % 32)));
...
/* function to call from command line */
GT_VOID appDemoDebug_unitAccessDisablePrint()
{
    GT_U32 arrSize = sizeof(unitNameToIdArr) / sizeof(unitNameToIdArr[0]);
    GT_U32 i;
    GT_U32 unitId;
    ...
    for (i = 0; (i < arrSize); i++)
    {
        ...
        if (appDemoDebug_unitAccessDisableBitmapArr[unitId / 32] & (1 << (unitId % 32)))
        {
            cpssOsPrintf("%s\n", unitNameToIdArr[i].name);
        }
    }
}

/* function to call from command line */
GT_VOID appDemoDebug_unitAccessNameByAddrPrint(GT_U8 devNum, GT_U32 regAddr)
{
    ...
    globalUnitId = prvCpssDxChHwRegAddrToUnitIdConvert(
        devNum, regAddr);
    if (globalUnitId >= PRV_CPSS_DXCH_UNIT_LAST_E)
    {
        cpssOsPrintf("there is no unit for the given address\n");
        return;
    }

    tile   = globalUnitId / PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E;
    unitId = globalUnitId % PRV_CPSS_DXCH_UNIT_LAST____IN_TILE_0____E;

    for (i = 0; (i < arrSize); i++)
    {
        if (unitId == unitNameToIdArr[i].unitId)
        {
            name = unitNameToIdArr[i].name;
            break;
        }
    }

    cpssOsPrintf("tile %d name %s\n", tile, name);
}

/**
* @internal appDemoDxChCnmResetPreparation function
* @endinternal
*
* @brief Halts all related activities before CPU Reset:
*              - Traffic to CPU
*              - Traffic from CPU and traffic generator
*              - AUQ, FUQ, CNC Upload
*              - Service CPU transactions (only in case of Ac5 and AC5X internal cpu)
*              - Remove the device from the CPSS - depends on the input parameter
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  None.
*
* @param[in] devNum                - device number
* @param[in] removeDevice          - the flag indicates whether the device should be removed from CPSS after preparation
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameter
* @retval GT_HW_ERROR              - on hardware error
*
**/
GT_STATUS appDemoDxChCnmResetPreparation
(
    IN GT_U8   devNum,
    IN GT_BOOL removeDevice
)
{
    GT_STATUS       rc;             /* funcion return code */
    GT_U8           queueIdx;       /* traffic class queue */
    GT_U32          numOfNetIfs;    /* number of CPU ports */

    PRV_CPSS_DXCH_MODULE_CONFIG_STC *moduleCfgPtr;
    /* pointer to port group modules configuraion */
    GT_U32          portGroupId;    /* port group */
    GT_U32          toCpuNumQueues,fromCpuNumQueues;
    CPSS_HW_INFO_STC* ppHwInfoStcPtr;
    ...
    /* Disable port manager task */
    rc = appDemoPortManagerTaskEnableSet(0);
    CPSS_ENABLER_DBG_TRACE_RC_MAC("appDemoPortManagerTaskEnableSet", rc);
    if (rc != GT_OK)
    {
        return rc;
    }
    ...
    {
        rc = prvCpssDxChNetIfMultiNetIfNumberGet(devNum, &numOfNetIfs);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("prvCpssDxChNetIfMultiNetIfNumberGet", rc);
        if (rc != GT_OK)
        {
            return rc;
        }
        toCpuNumQueues   = 8*numOfNetIfs;
        fromCpuNumQueues = toCpuNumQueues;
    }
    ...
    for(queueIdx = 0; queueIdx < fromCpuNumQueues; queueIdx++)
    {
        if(CPSS_TX_SDMA_QUEUE_MODE_PACKET_GENERATOR_E ==
            moduleCfgPtr->
               multiNetIfCfg.txSdmaQueuesConfig[portGroupId][queueIdx].queueMode)
        {
        ...
        {
            /* Disable traffic from CPU */
            rc = cpssDxChNetIfSdmaTxQueueEnable(devNum, queueIdx, GT_FALSE);
            CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChNetIfSdmaTxQueueEnable", rc);
            if (rc != GT_OK)
            {
                return rc;
            }
    ...
    ppHwInfoStcPtr = cpssDrvHwPpHwInfoStcPtrGet(devNum, CPSS_PORT_GROUP_UNAWARE_MODE_CNS);
    if (ppHwInfoStcPtr == NULL)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(
            GT_FAIL, "Fail to obtain device bus type");
    }

    if((PRV_CPSS_DXCH_AC5X_CHECK_MAC(devNum) && ppHwInfoStcPtr->busType == CPSS_HW_INFO_BUS_TYPE_MBUS_E)||
        PRV_CPSS_DXCH_AC5_CHECK_MAC(devNum))
    {
        /* Disable service CPU processor */
        rc = cpssDxChCfgServiceCpuDisable(devNum);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChCfgServiceCpuDisable", rc);
        if (rc != GT_OK)
        {
            return rc;
        }
    }
    /*skip dev remove in case of shared lib*/
#ifndef SHARED_MEMORY
    if (removeDevice == GT_TRUE)
    {
        /* Remove device from CPSS */
        rc = cpssDxChCfgDevRemove(devNum);
        CPSS_ENABLER_DBG_TRACE_RC_MAC("cpssDxChCfgDevRemove", rc);
    }
#else
    GT_UNUSED_PARAM(removeDevice);
#endif
    return rc;
}

/**
* @internal appDemoResilientInitFailedDevicesGet function
* @endinternal
*
* @brief  return array bmp of devices failed during init
*
* @param[in] clearOnRead         - in case of true  - clear device bmp
*                                             false - nothing
* @param[in] numOfBmpArr         - number of bmp in devBmpArr
* @param[out] devBmpArr[]        - bmp of devices failed during init
*
* @retval GT_OK                    - on success
*
*/
GT_STATUS appDemoResilientInitFailedDevicesGet
(
    IN  GT_BOOL clearOnRead,
    IN GT_U32   numOfBmpArr,
    OUT GT_U32  devBmpArr[]
)
{
    GT_U32 ii ;

    if (numOfBmpArr > (sizeof(appDemoSysConfig.resilientInitDevErrorArr)/sizeof(appDemoSysConfig.resilientInitDevErrorArr[0])))
    {
        numOfBmpArr = (sizeof(appDemoSysConfig.resilientInitDevErrorArr)/sizeof(appDemoSysConfig.resilientInitDevErrorArr[0]));
    }
    for (ii = 0; ii < numOfBmpArr; ii++)
    {
        devBmpArr[ii] = appDemoSysConfig.resilientInitDevErrorArr[ii];
    }
