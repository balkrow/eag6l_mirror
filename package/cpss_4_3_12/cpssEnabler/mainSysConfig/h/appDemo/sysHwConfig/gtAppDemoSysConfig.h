...
/* max number of device bmp that needed  */
#define APP_DEMO_CPSS_MAX_NUM_OF_DEV_BMP_BANKS     (PRV_CPSS_MAX_PP_DEVICES_CNS/32)+1
...
typedef struct{
    ...
    /** @brief Typef of FUQ - member of enum passed from PHASE2_PARAM to MODULE structure
     */
...
typedef struct{
    ...
    /** whether support E class as non regular unicast  */
...
typedef struct
{
    ...
    GT_BOOL                 initCncDump;/* relevant to SIP7 that have CNC DMA per unit , that is not shared with the FUQ */
...
typedef GT_STATUS (*FUNCP_PORT_RX_TRAIN_CONFIG)
(
    IN  GT_U8     devNum,
    IN  GT_PHYSICAL_PORT_NUM     portNum
);
...
typedef struct
{
    ...
    FUNCP_PORT_RX_TRAIN_CONFIG      internalCscdPortRxTrainConfigFuncPtr;
    ...
    /** array of strings that each describe a port in the 'port mapping' */
    APP_DEMO_STR_STC                *portsNotesArray;
    /** the device can stated on it self to not be part of NA learning */
    GT_BOOL                         fdbNotUsed;
    /** port manager task id **/
    GT_TASK                         portManagerTaskId;
}APP_DEMO_PP_CONFIG;
...
typedef struct{
    ...
    /** @brief Is system has devices from SIP5 generation and SIP6 generation:
     *  GT_TRUE - devices of both generations are in system. E.g. AC5X and Caelum
     *  GT_FALSE - otherwise */
    GT_BOOL mixedSip5AndSip6System;

    /** bmp of devices that had error during init.
     * bmp update when appDemoDbEntryAdd "resilientInit" == 1
     * the BMP is ignored if 0x0 */

    GT_U32  resilientInitDevErrorArr[APP_DEMO_CPSS_MAX_NUM_OF_DEV_BMP_BANKS];
...
/* macro to start loop on devices , and allow to skip 'non-active' device */
#define START_LOOP_ALL_DEVICES(_devIndex,_devNum) \
    for (_devIndex = SYSTEM_DEV_NUM_MAC(0); \
          (_devIndex < SYSTEM_DEV_NUM_MAC(appDemoPpConfigDevAmount)); _devIndex++) \
    {                                                                            \
        _devNum =   appDemoPpConfigList[_devIndex].devNum;                       \
        _devNum = _devNum;/* avoid gcc warning (when _devNum not used by the caller) */ \
        SYSTEM_SKIP_NON_ACTIVE_DEV((GT_U8)_devIndex)

/* macro to start loop on devices , and allow to skip 'non-active' device.
   _devAmount limits number of iterations */
#define START_LOOP_ALL_DEVICES_WITH_AMOUNT(_devIndex,_devNum,_devAmount) \
    for (_devIndex = SYSTEM_DEV_NUM_MAC(0); \
          (_devIndex < SYSTEM_DEV_NUM_MAC(_devAmount)); _devIndex++) \
    {                                                                            \
        _devNum =   appDemoPpConfigList[_devIndex].devNum;                       \
        _devNum = _devNum;/* avoid gcc warning (when _devNum not used by the caller) */ \
        SYSTEM_SKIP_NON_ACTIVE_DEV((GT_U8)_devIndex)
...
/* macro to end loop on devices , that started by START_LOOP_ALL_DEVICES(...) */
#define END_LOOP_ALL_DEVICES \
    }

/* macro to get the first active device (knows to skip 'non-active' devices) */
#define GET_FIRST_VALID_DEV_INDEX(_devIndex)     \
    {                                            \
        GT_U8   _dummy;                          \
        START_LOOP_ALL_DEVICES(_devIndex,_dummy) \
        {                                        \
            break;                               \
        }                                        \
...
#define RESILIENT_INIT_IS_DEV_SET(_devIndex) (appDemoSysConfig.resilientInitDevErrorArr[(_devIndex /32)] & (1 << (_devIndex % 32)))

/* set device index in init failed devices bmp array */
#define RESILIENT_INIT_DEV_INDEX_ERROR_SET(_devIndex) \
        appDemoSysConfig.resilientInitDevErrorArr[(_devIndex/32)] |= 1<<(_devIndex % 32);

#define RESILIENT_INIT_SET_OR_RETURN(rc,resilientInit,_devIndex)     \
            if (resilientInit){                                      \
                RESILIENT_INIT_DEV_INDEX_ERROR_SET(_devIndex);       \
                continue;                                            \
            }                                                        \
            else{                                                    \
                return rc;                                           \
...
#define RESILIENT_INIT_CHECK_AND_SKIP(_devIndex)       \
        if (RESILIENT_INIT_IS_DEV_SET(_devIndex)){     \
            continue;                                  \
        }

/**
* @struct PRV_APPDEMO_FIRST_INIT_TIMES_STC
 *
 * @brief Times of first Init System (total and parts) in milliseconds
*/
typedef struct{

    /** InitSystem Duration Total MiliSeconds */
    GT_U32 initSystemTimeTotal;

    /** InitSystem Duration PpPhase1BeforeReset MiliSeconds */
    GT_U32 initSystemTimePpPhase1BeforeReset;

    /** InitSystem Duration PpPhase1AftereReset MiliSeconds */
    GT_U32 initSystemTimePpPhase1AfterReset;

    /** InitSystem Duration PpPhase2Init MiliSeconds */
    GT_U32 initSystemTimePpPhase2Init;

    /** InitSystem Duration PpLogicalInit MiliSeconds */
    GT_U32 initSystemTimePpLogicalInit;

    /** InitSystem Duration PpImplementWaInit MiliSeconds */
    GT_U32 initSystemTimePpImplementWaInit;

    /** InitSystem Duration CpssLibInit MiliSeconds */
    GT_U32 initSystemTimeCpssLibInit;

} PRV_APPDEMO_FIRST_INIT_TIMES_STC;

extern PRV_APPDEMO_FIRST_INIT_TIMES_STC prvAppDemoFirstInitTimes;

/**
* @internal prvAppDemoGetDurationTimeMilliSec function
* @endinternal
*
* @brief   This function gets current time and subtracts from it the given start time.
*          The result converted to miliseconds.
*
* @param[in] secondsStart        - start time seconds.
* @param[in] nanoSecondsStart    - start time nano seconds.
*
* @retval duration time in milli-seconds
*/
GT_U32 prvAppDemoGetDurationTimeMilliSec
(
    IN  GT_U32  secondsStart,
    IN  GT_U32  nanoSecondsStart
);
...
* @param[in] devNum                   - device number
* @param[in] portNum                  - global port number
* @param[in] iface                - current  of port
...
/**
* @internal appDemoDxChSoftResetSkipParamsAndTrigger function
* @endinternal
*
* @brief 
*       1. Halts all relevant activities before a CPU Reset
*          using the appDemoDxChCnmResetPreparation function
*          with the flag removeDevice = False (no device removal).
*  
*       2. Invoke CPSS API cpssDxChHwPpSoftResetSkipParamsAndTrigger
*          to perform skip configuration sequence
*          based on the specific functional use-case and trigger a reset.
*
...
*
**/
GT_STATUS appDemoDxChSoftResetPrepareAndTrigger
(
    IN GT_U8   devNum,
