...
)
{
    ...
    *counterPtr = 0;
    ...
    for(; uniEvCurrentCounter <= CPSS_SRVCPU_MAX_E; uniEvCurrentCounter++)
    {
...
)
{
    ...
    if(devNum >= APP_UTILS_MAX_INSTANCES_CNS || appUtilsEventCounters[devNum] == NULL)
    {
        /* the device is out of range or DB is not allocated */
        return;
    }

    osMutexLock(appUtilsEventCounterUpdateLockMtx);

    for(uniEvCounter = CPSS_PP_UNI_EV_MIN_E; uniEvCounter <= CPSS_SRVCPU_MAX_E; uniEvCounter++)
    {
