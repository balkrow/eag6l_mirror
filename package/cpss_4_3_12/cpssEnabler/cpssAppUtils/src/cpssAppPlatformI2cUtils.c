...
)
{
    ...
    if (GT_OK != rc && printErrorEnable) {
        cpssOsPrintf("prvUtilsHostI2cRead: bus_id 0x%x, slave_address 0x%x, offset_type 0x%x, offset 0x%x, size 0x%x\n",
...
)
{
    ...
    if (GT_OK != rc  && printErrorEnable) {
        cpssOsPrintf("prvUtilsHostI2cWrite: bus_id 0x%x, slave_address 0x%x, offset_type 0x%x, offset 0x%x\nnof_args %d, arg1 0x%x,arg3 0x%x,arg3 0x%x,arg4 0x%x\n",
