...
#define LIB_CPSS_NAME "libcpss.so"
#define LIB_CPSS_NAME_NO_ASLR  "libcpss_noaslr.so"
#define LIB_OS_NAME "libhelper.so"
#define LIB_OS_NAME_NO_ASLR  "libhelper_noaslr.so"


#define AT_EXIT_DEPTH_MAX 32
...
typedef struct{
 ...
 /** @brief Bmp that mark the ownership of semaphores.
  ...
  /** @brief : Current device selected for DMA memory allocation
   */
  GT_U32               dmaConfigCurrentDevice;
  GT_BOOL              dmaConfigCurrentDeviceConfigured;
#endif
  ...
  GT_U32           noKmMappingsNum ;
  GT_U32           mallocPoolSize;

  /* DMA driver file handle for the device */
  GT_VOID*         dmaDrvHandle[GT_MEMORY_DMA_DEVICE_INFO_CNS];

  GT_VOID * dbInitSem;
  GT_VOID * dbDiscoverySem;

  GT_VOID (*exitFuncs[AT_EXIT_DEPTH_MAX])(GT_VOID);
  GT_U32   exitFuncDepth;
