 ...
 *
 * Non-supported architectures for BSS remap:  ARMv7 (both le and be)
 */
...
#else
#  if defined(CPU_AARCH64v8) || (defined(INTEL64_DRV) && defined(CPU_i386))|| defined(AARCH64v8)
#   define SHARED_DATA_ADDR_CNS	0x13000000
#   define SHARED_MAINOS_DATA_ADDR_CNS	0x14680000
#   define SHARED_MEMORY_MALLOC_VIRT_ADDR_MAC 0x50000000
#  endif
...
/*
    Default mapping .
    Need to keep default configuration since we still support noASLR build for ARMv8.
    This defenition is needed in order to ARMv7 ASLR mode to compile.So keep it.
    */
