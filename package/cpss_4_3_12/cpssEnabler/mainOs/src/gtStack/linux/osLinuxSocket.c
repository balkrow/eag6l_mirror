...
#define PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_SET(_var,_value)\
    PRV_HELPER_SHARED_GLOBAL_VAR_SET(cpssEnablerMod.mainOsDir.osLinuxSocketSrc._var,_value)

#define PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_GET(_var)\
    PRV_HELPER_SHARED_GLOBAL_VAR_GET(cpssEnablerMod.mainOsDir.osLinuxSocketSrc._var)
...
)
{
    ...
    if(0 == PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_GET(socketAddrPool))
    {
        if(gtPoolCreatePool(sizeof(struct sockaddr_in),
                            GT_4_BYTE_ALIGNMENT,
                            SOCKET_ADDR_POOL_SIZE,
                            GT_TRUE, &PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_GET(socketAddrPool)) != GT_OK)
        {
            return GT_FAIL;
        }
    }

    *sockAddr = (struct sockaddr_in*) gtPoolGetBuf(PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_GET(socketAddrPool));
    ...
    *sockAddrLen = sizeof(struct sockaddr_in) ;
    ...
    {
        gtPoolFreeBuf(PRV_SHARED_MAIN_OS_DIR_OS_LINUX_SOCKET_SRC_GLOBAL_VAR_GET(socketAddrPool),*sockAddr);
        return GT_FAIL ;
    }
...
/**
* @internal osSocketExtractPortFromSocketAddr function
* @endinternal
*
* @brief   Extracts the port from sockaddr_in structure.
*
* @param[in] sockAddr                 - pointer to sockaddr_in.
* @param[in] port                     - pointer to port in GT_U16 format to be filled by the
*                                      routine.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on fail
*/
GT_STATUS osSocketExtractPortFromSocketAddr
(
    IN  GT_VOID*   sockAddr,
    OUT GT_U16*    port
)
{
    /* extract the port from the socket address struct */
