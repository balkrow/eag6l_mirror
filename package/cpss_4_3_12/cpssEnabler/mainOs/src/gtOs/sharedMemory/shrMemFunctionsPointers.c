...
)
{
    ...
    char * libHelperName = LIB_OS_NAME;
 #ifdef PIC_BUILD
    if(GT_FALSE==osNonSharedGlobalVars.osNonVolatileDb.aslrSupport)
    {
      libHelperName = LIB_OS_NAME_NO_ASLR;
    }
#endif
    handle = getCpssSharedLibraryHandle(libHelperName);
...
#define ASSIGN_FUNCPTR(_member,_funcname) if((osFuncBindPtr->_member=getFunctionThruDynamicLoader(handle, _STR(_funcname))) == NULL) retcode = GT_FAIL
    ...
    ASSIGN_FUNCPTR(osMemBindInfo.osMemCacheDmaMallocByDeviceFunc,osCacheDmaMallocByDevice);
#endif
...
)
{
    ...
    handle=getCpssSharedLibraryHandle(cpssLibname);
    if(handle==NULL) return GT_FAIL;
