...
#define PRV_SHARED_MAIN_OS_DIR_SHMEM_IPC_SRC_GLOBAL_VAR_SET(_var,_value)\
    PRV_HELPER_SHARED_GLOBAL_VAR_SET(cpssEnablerMod.mainOsDir.gtOsShmemIpcSrc._var,_value)

#define PRV_SHARED_MAIN_OS_DIR_SHMEM_IPC_SRC_GLOBAL_VAR_GET(_var)\
    PRV_HELPER_SHARED_GLOBAL_VAR_GET(cpssEnablerMod.mainOsDir.gtOsShmemIpcSrc._var)
...
)
{
    ...
    mp = &(PRV_SHARED_MAIN_OS_DIR_SHMEM_IPC_SRC_GLOBAL_VAR_GET(remoteArray[paramPtr->remote]));
...
)
{
    ...
    mp = &(PRV_SHARED_MAIN_OS_DIR_SHMEM_IPC_SRC_GLOBAL_VAR_GET(remoteArray[program]));
    if (!mp->semMutex) {
        /* not initialized yet */
        char name[64];
