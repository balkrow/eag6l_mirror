...
/**
* @internal osPrintToSocket function
* @endinternal
*
* @brief   Write a string to the socket output stream.
*/
static int osPrintToSocket(char *buff, int len)
{
    /*socket*/
	int retVal;

    if (osTaskIsSocketConnection(NULL /* Use self ID */) == GT_TRUE)
    {
        retVal = send(osTaskFdGet(NULL /* Use self ID */), buff, len, 0);
		send(osTaskFdGet(NULL /* Use self ID */), "\r", 1, 0);
		return retVal;
    }
    else
        return -1000;
