...
GT_VOID osTaskFdSet(IN GT_VOID *pthrid, GT_FD fd)
{
    ...
    if (NULL == pthrid)
        ...
        threadId = (DWORD)pthrid;

    for(i = 1; i < OS_MAX_TASKS; i++)
    {
        if(threadId == OS_TaskArray[i].threadId)
        {
            OS_TaskArray[i].redirStdoutFd = fd;
        }
    }
}
...
GT_FD osTaskFdGet(IN GT_VOID *pthrid)
{
    ...
    for(i = 1; i < OS_MAX_TASKS; i++)
    {
        if(threadId == OS_TaskArray[i].threadId)
        {
            return OS_TaskArray[i].redirStdoutFd;
        }
    }
...
GT_BOOL osTaskIsSocketConnection(IN GT_VOID *pthrid)
{
    ...
    for(i = 1; i < OS_MAX_TASKS; i++)
    {
        if(threadId == OS_TaskArray[i].threadId)
        {
            if (!strncmp(OS_TaskArray[i].threadName,"CPSSGenServer",13))
                return GT_TRUE;
            else
                return GT_FALSE;
        }
...
* @endinternal
*
* @brief   declare that thread support graceful termination
*
...
* @retval GT_FALSE
*/
GT_BOOL osTaskGracefulGetTerminationReq(GT_U32 tid)
{
    return GT_FALSE;
}
...
* @retval GT_FALSE
*/
GT_BOOL osTaskGracefulGetTerminationAck(GT_32 tid)
{
    return GT_FALSE;
}
...
*
* @note Not implemented for windows, as it is only needed for BM
*
*/
