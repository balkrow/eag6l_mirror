...
int osVprintf(const char* format, va_list args)
{
    ...
    char *p, *newFmt, *buff, *dst;
    unsigned lenToAlloc = 256; /* Reserved size for base allocation, in case we use %s */
    ...
    while (*p)
    {
        lenToAlloc++;
        if (*p == '%')
            lenToAlloc += 15; /* Assume we format numbers, so this should be maximum allocation per item. */
        p++;
    }
    buff = alloca(lenToAlloc); /* allocate dynamically on stack. Always returns a pointer to stack area. Automatically released on exit. */

    /*socket*/
    if ( (osTaskIsSocketConnection((GT_VOID *)&rootTask) == GT_TRUE) &&
         (osNonSharedGlobalVarsPtr) &&
         (PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_IO_SRC_GLOBAL_VAR_GET(redirectNetStdio) == GT_TRUE) )
    {
        newFmt = alloca(strlen(format)+32); /* allocate dynamically on stack. Always returns a pointer to stack area. Automatically released on exit. */

        /* Add carriage return after each new-line - needed for proper telnet formatting */
        p = (char *)format;
        dst = newFmt;
        while (*p)
        {
        ...
        *dst = '\0';
        ...
        ret = vsnprintf(buff, lenToAlloc, newFmt, args);
        if (ret <= 0)
            return ret;
        if (write(osTaskFdGet((GT_VOID *)&rootTask), buff, ret) < 0) {
    ...
    {
        ret = vsnprintf(buff, lenToAlloc, format, args);
        if (ret <= 0)
            return ret;
