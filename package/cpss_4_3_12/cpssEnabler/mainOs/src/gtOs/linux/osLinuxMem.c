...
extern GT_STATUS prvNoKmDrv_get_pci_info_by_devNum(
    IN  GT_U8    devNum,
...
/*******************************************************************************
* internal_osCacheDmaMallocByDevice
*
* DESCRIPTION:
...
*
*******************************************************************************/
static GT_VOID *internal_osCacheDmaMallocByDevice
(
    IN GT_U8  devNum,
    IN GT_U32 size
)
{
    ...
    GT_U8       pciDomainNum = 0;
    GT_U8       pciBusNum = 0;
    GT_U8       pciDevNum = 0;
    GT_U8       pciFunNum = 0;
    GT_STATUS   rc;

    GT_VOID*     prvExtDrvDmaVirtPtr;
    unsigned long long  prvExtDrvDmaPhysPtr;

    rc = prvNoKmDrv_get_pci_info_by_devNum(devNum, &pciDomainNum, &pciBusNum, &pciDevNum, &pciFunNum);
    if(rc != GT_OK) {
        osPrintf("prvNoKmDrv_get_pci_info_by_devNum failed, rc = %d\n", rc);
        return NULL;
    }

    if (PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum]) != NULL)
    {
        prvExtDrvDmaVirtPtr = extDrvDmaAlloc(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum]), size, &prvExtDrvDmaPhysPtr);
        if (!prvExtDrvDmaVirtPtr)
        {
            perror("extDrvDmaAlloc");
            PRV_NON_SHARED_DMA_GLOBAL_VAR_SET(prvExtDrvDmaPtr,NULL);
            return NULL;
        }

        return prvExtDrvDmaVirtPtr;
    }
...
{
    return internal_osCacheDmaMallocByDevice(0,size);
}
/*******************************************************************************
* osCacheDmaMallocByDevice
*
* DESCRIPTION:
*       Allocate a cache-safe buffer of specified size for DMA devices
*       and drivers
...
*
*******************************************************************************/
GT_VOID *osCacheDmaMallocByDevice
(
    IN GT_U8  devNum,
    IN GT_U32 size
)
{
    IS_WRAPPER_OPEN_PTR;
#if defined(CONFIG_V2_DMA)
    return internal_osCacheDmaMallocByDevice(devNum,size);
#else
    GT_U32 windowId = devNum;
    ...
    return internal_osCacheDmaMallocByDevice(windowId,size);
#endif
}
/**
...
GT_STATUS osCacheDmaFreeAll(GT_VOID)
{
    ...
    for(devNum = 0; devNum < GT_MEMORY_DMA_DEVICE_INFO_CNS; devNum++) {
        if(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum])) {
            extDrvDmaClose(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum]));
            PRV_NON_SHARED_DMA_GLOBAL_VAR_GET(prvExtDrvDmaPtr) = NULL;
            PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum]) = NULL;
            rc = prvNoKmDrv_clear_dma_info_by_devNum(devNum);
            if(rc != GT_OK) {
                osPrintf("prvNoKmDrv_clear_dma_info_by_devNum failed, rc = %d\n", rc);
                return rc;
            }
        }
    }
...
#if !defined(ASIC_SIMULATION) && !defined(CONFIG_V2_DMA)
...
#ifdef CONFIG_V2_DMA
GT_STATUS   osVirt2Phy_trace_enable_set(IN GT_U32   enable)
{
    PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_SET(osVirt2Phy_trace_enable,enable);
...
*
*  @param[in] devNum   - (for compilation of CONFIG_V2_DMA) the device that the DMA relate to.
...
{
#if defined(CONFIG_V2_DMA)
    GT_U8 iter_devNum,devNumStart,devNumLast;

    if(devNum == CPSS_OS_DEV_NUM_UNKNOWN_CNS)
    {
        devNumStart = 0;
        devNumLast  = GT_MEMORY_DMA_DEVICE_INFO_CNS;
    }
    else if(devNum >= GT_MEMORY_DMA_DEVICE_INFO_CNS)
    {
        if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
        {
            osPrintf("osPhy2Virt: devNum[%d] >= [%d]\n",
                devNum,GT_MEMORY_DMA_DEVICE_INFO_CNS);
        }
        return GT_BAD_PARAM;
    }
    else
    ...
    for(iter_devNum = devNumStart; iter_devNum < devNumLast; iter_devNum++) {
        if(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[iter_devNum])) {
            *virtAddr = (GT_UINTPTR)extDrvDmaToVirt(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[iter_devNum]), phyAddr);
            if(*virtAddr)
            {
                if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
                {
                    osPrintf("osPhy2Virt : on device[%d] converted phyAddr[%p] to virtAddr[%p] \n",
                        iter_devNum,phyAddr,*virtAddr);
                }
                return GT_OK;
            }
            if(devNum != CPSS_OS_DEV_NUM_UNKNOWN_CNS)
            {
                if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
                {
                    osPrintf("osPhy2Virt: devNum[%d] failed to convert phyAddr[0x %p] to virtual , no such phyAddr for the device \n",
                        devNum,phyAddr);
                }
                /* the dma address not belong to the only device that need to be checked */
                return GT_FAIL;
            }
        }
        else
        if(devNum != CPSS_OS_DEV_NUM_UNKNOWN_CNS)
        {
            if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
            {
                osPrintf("osPhy2Virt: devNum[%d] failed to convert phyAddr[0x %p] to virtual ,  'not initialized device' \n",
                    devNum,phyAddr);
            }
            /* the handle to the only device that need to be checked was not initialized */
    ...
    if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
    {
        osPrintf("osPhy2Virt: failed to convert phyAddr[0x %p] to virtual , 'not part of any device' \n",
            phyAddr);
    }

    /* should get here only for 'devNum == CPSS_OS_DEV_NUM_UNKNOWN_CNS' */
    return GT_FAIL;
#else
#ifndef ASIC_SIMULATION
    GT_U32  windowId = 0;
...
)
{
    ...
    for(devNum = 0; devNum < GT_MEMORY_DMA_DEVICE_INFO_CNS; devNum++) {
        if(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum])) {
            rc = extDrvVirtToDma(PRV_NON_SHARED_DMA_DEVICE_INFO_GLOBAL_VAR_GET(dmaDrvHandle[devNum]), (void *)virtAddr, (unsigned long *)phyAddr);
            if(rc == GT_OK)
            {
                if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
                {
                    osPrintf("osVirt2Phy : converted virtAddr[%p] to phyAddr[%p] \n",
                        virtAddr,
                        *phyAddr);
                }
                return rc;
    ...
    if(PRV_NON_SHARED_MAIN_OS_DIR_OS_LINUX_MEM_SRC_GLOBAL_VAR_GET(osVirt2Phy_trace_enable))
    {
        osPrintf("extDrvVirtToDma: failed to convert virtAddr[%p] \n",
            virtAddr);
    }
    return GT_FAIL;
#else
...
)
{
    ...
    if (initialize) {
        ...
        *sharedGlobalVarsPtrPtr = sharedMem;
        ...
        ((PRV_OS_SHARED_GLOBAL_DB *)sharedMem)->initializerPid = (GT_U32)getpid();
        ((PRV_OS_SHARED_GLOBAL_DB *)sharedMem)->dbInitialized = GT_FALSE;
    }
...
)
{
    ...
    shrAddNameSuffix(name,buff,sizeof(buff));
#endif
    SHM_PRINT("Shared memory [%s]  ", buff);

    if(unlink(buff) < 0)
    {
        if(errno == EBUSY)
        {
           SHM_PRINT("cannot be unlinked because it is being used by the system or another proces\n");
        }
        else if(errno == ENOENT)
        {
           SHM_PRINT("No such file or directory\n");
        }
        else if(0 != errno)
        {
            SHM_PRINT ("unmap error  %s\n", strerror(errno));
            rc = GT_FAIL;
        }
    }
    else
...
extern GT_STATUS shrMemSharedDumpBufferListUsage
(
    GT_VOID
);

extern GT_U32  shrMemSharedFreeBufferListSizeGet
(
    GT_VOID
);

GT_STATUS shrMemSharedHugeChunksMergeTest
(
    GT_VOID
)
{
    GT_VOID * chunks[3];
    GT_U32    sizes[3]= {2400000 ,2300000,2200000},i;
    GT_U32    listSize,totalSize=0;

    osPrintf ("Check merge\n");

    for(i=0;i<3;i++)
    {
        chunks[i] = internalOsMalloc(sizes[i]);
        if(chunks[i]==NULL)
        {
            osPrintf ("Allocation fail at line %d size %d \n",__LINE__,sizes[i]);
            return GT_FAIL;
        }
    ...
    for(i=0;i<3;i++)
    {
        internalOsFree(chunks[i]);
    }

    shrMemSharedDumpBufferListUsage();
    listSize = shrMemSharedFreeBufferListSizeGet();

    osPrintf ("listSize %d\n",listSize);
    osPrintf ("totalSize %d\n",totalSize);
    osPrintf ("sizeof(OSMEM_NOPOOLS_HDR_STC) %d\n",sizeof(OSMEM_NOPOOLS_HDR_STC));

    if(listSize==totalSize+3*sizeof(OSMEM_NOPOOLS_HDR_STC))
    {
        osPrintf ("Pass\n");
    }
    else
...
GT_STATUS shrMemSharedHugeChunksReuseTest
(
    GT_VOID
)
{
    ...
    if(listSize==totalSize+ sizeof(OSMEM_NOPOOLS_HDR_STC))
    {
        osPrintf ("Pass\n");
    }
    else
...
GT_STATUS shrMemSharedHugeChunksSortTest
(
    GT_VOID
)
{
    GT_VOID * chunks[5];
    GT_U32    sizes[5]= {2400000 ,2300000,2200000,2100000,2150000};
    GT_32 i;
