...
)
{
    ...
    if ( services[srvIndex].onHold == GT_FALSE ) {
        ...
        if ( rc != GT_OK && luaRestartRequest == 1){
            ...
            if ( rc != GT_OK )
            {
                if (services[srvIndex].stream != NULL)
                {
...
)
{
    ...
    GT_32 retVal;
    GT_U32 self_tid = 0;
    GT_BOOL telnet_socket;

    if (osTaskGetSelf(&self_tid) != GT_OK)
    {
        cmdOsPrintf("osTaskGetSelf(tid:%u) in cmdStreamSocketServiceListenerTask has failed.\n", self_tid);
        return;
    }
    ...
    if(rdSet!=NULL)
    {
        while (GT_TRUE)
        {
            ...
            retVal = cmdOsSelect(max_fd + 1, rdSet, NULL, NULL, MV_DEFAULT_TIMEOUT);
            if ( retVal < 0)
                continue;
            if (osTaskGracefulGetTerminationReq(self_tid) == GT_TRUE)
            {
                ...
                if (numServices == 0)/*In case services[] is empty*/
                {
            ...
            for ( srvIndex = 0 ; srvIndex < SERVICES_MAX ; srvIndex++)
            {
                ...
                if ( cmdOsSelectIsFdSet(rdSet, fd) == GT_TRUE ){
                    osTaskFdSet(NULL /* Use self ID */, fd);
                    ...
                    if ((cmdOsStrCmp(services[srvIndex].serviceName, MASTER_SOCKET_SERVICE_NAME) == 0) ||
                        ...
                        if( rc != GT_OK ){
                            ...
                            /* Relevant for unix domain socket only - In case the connection is closed but session needs to be saved,
                                keep session alive on hold */
                            if  ((cmdOsStrNCmp(services[srvIndex].serviceName, "telnet",6) == 0))
                                telnet_socket = GT_TRUE;
                            else
                                telnet_socket = GT_FALSE; /*unix domain socket*/

                            /* unix domain socket - save the session */
                            if  (telnet_socket == GT_FALSE)
                            {
                            ...
                            /* In case of telnet - always terminate the session */
                            if (rc != GT_NO_RESOURCE || (telnet_socket == GT_TRUE) ){
                                /* rc==GT_NO_RESOURCE means we closed the session not using "CLIexit" command, and stream is lost */
