...
static int serialStreamRead(IOStreamPTR streamP, void* bufferPtr, int bufferLength)
{
    ...
    while (1)
    {
        ...
        cmdOsSelectAddFdToSet(rset, stream->fd);
        retval = cmdOsSelect(stream->fd + 1, rset, NULL, NULL, MV_DEFAULT_TIMEOUT);

        if (retval == -1 || osTaskGracefulGetTerminationReq(self_tid))
            break;

        if (cmdOsSelectIsFdSet(rset, stream->fd))
        {
