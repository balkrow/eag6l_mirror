...
)
{
...
#if defined(CONFIG_RMON) && !defined(ASIC_SIMULATION) && !defined(WIN32)
    extern const char *VERSION_DATE;
    extern const char *VERSION_FLAGS;

    if(osTaskRmonStart(VERSION_DATE, VERSION_FLAGS) != GT_OK)
    {
        cmdOsPrintf("osTaskRmonStart initialization failure!\n");
        return GT_FAIL;
    }
#endif
#ifndef PSS_PRODUCT
    ...
    osTaskGracefulCallerRegister(taskId, 1, NULL, NULL);
    ...
    if((rc = cmdStreamStartEventLoopHandler(
            "SERIAL",
            cmdEventLoop,
            defaultStdio,
            GT_TRUE/*isConsole*/,
    ...
    osTaskGracefulCallerRegister(taskSerial, 1, NULL, NULL);
#endif /* CMD_INCLUDE_SERIAL */
