...
)
{
    ...
    PDL_PP_B2B_ATTRIBUTES_STC          b2bLinkPortAttr;
    ...
    xmlPortInfoPtr->transceiverType = (GT_U32)portAttr.transceiverType;
    ...
    if(portAttr.isPhyExists)
    {
        ...
        xmlPortInfoPtr->phyInfo.smiXmsiInterface.interfaceType  = (GT_U32)portAttr.phyData.smiXmsiInterface.interfaceType;
        xmlPortInfoPtr->phyInfo.smiXmsiInterface.devNum         = portAttr.phyData.smiXmsiInterface.phyInfo.readWriteAddress.dev;
        xmlPortInfoPtr->phyInfo.smiXmsiInterface.interfaceId    = portAttr.phyData.smiXmsiInterface.phyInfo.readWriteAddress.interfaceId;
        xmlPortInfoPtr->phyInfo.smiXmsiInterface.address        = portAttr.phyData.smiXmsiInterface.phyInfo.readWriteAddress.address;
...
)
{
    ...
    callbacks.pdLibCallbacks.printStringPtr        = cpssOsPrintf;
    callbacks.pdLibCallbacks.mallocPtr             = util_malloc;
    callbacks.pdLibCallbacks.freePtr               = cpssOsFree;
    ...
    callbacks.pdLibCallbacks.arXmlUncompressClbk   = util_archive_uncompress;
    callbacks.pdLibCallbacks.getXmlSignatureClbk       = NULL;
...
GT_STATUS  ezbXmlDestroy(GT_VOID)
{
    ...
    pdl = pdlDestroy(PDL_OS_INIT_TYPE_FULL_E);
    if(pdl != PDL_OK)
    {
        return GT_FAIL;
    }
