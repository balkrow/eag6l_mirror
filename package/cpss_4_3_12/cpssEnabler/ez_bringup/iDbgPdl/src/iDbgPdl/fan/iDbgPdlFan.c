...
)
{
    ...
    for (i = firstLogicalFanId; i <lastFanLogicalId; i++) {
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &typePtr);
        }
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &initPtr);
        }
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &rolePtr);
        }
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    if (pdlStatus == PDL_OK) {
                        ...
                        if (pdlStatus == PDL_OK) {
                            ...
                            pdlibStatusToString(pdlStatus, &rotationPtr);
                        }
                    }
                    else {
                        pdlibStatusToString(pdlStatus, &rotationPtr);
        ...
        if (controllerCapabilities.dutyCycleMethodControlIsSupported == TRUE) {
            ...
            if (pdlStatus == PDL_OK) {
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    pdlibStatusToString(pdlStatus, &dutyCyclePtr);
                }
            }
            else {
                pdlibStatusToString(pdlStatus, &dutyCyclePtr);
            ...
            dutyCycleMethodPtr = "Not Supported";
        }

        if (controllerCapabilities.rpmInfoSupported == TRUE) {
            pdlStatus = pdlFanHwRpmGet(tempFanControllerId, tempFanNumber, &rpm);
            if (pdlStatus == PDL_OK) {
                rpmPtr = "Pass";
            }
            else {
                pdlibStatusToString(pdlStatus, &rpmPtr);
            }
        }
        else {
            rpmPtr = "Not Supported";
        }
        ...
        if (controllerCapabilities.faultControlIsSupported == TRUE) {
            ...
            if (pdlStatus == PDL_OK) {
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    pdlibStatusToString(pdlStatus, &faultPtr);
                }
            }
            else {
                pdlibStatusToString(pdlStatus, &faultPtr);
        ...
        if (controllerCapabilities.stateInfoIsSupported == TRUE) {
            ...
            if (pdlStatus == PDL_OK) {
                ...
                pdlibStatusToString(pdlStatus, &statePtr);
            }
        }
        else {
            statePtr = "Not Supported";
        }
            ...
            if (pdlStatus == PDL_OK) {
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    if (pdlStatus == PDL_OK) {
                        ...
                        pdlibStatusToString(pdlStatus, &stateConPtr);
                    }
                }
                else {
                    pdlibStatusToString(pdlStatus, &stateConPtr);
...
)
{
    ...
    for (i = firstLogicalFanId; i <lastFanLogicalId; i++) {
        ...
        if (fanCapabilities.dutyCycleInfoIsSupported == TRUE) {
            ...
            switch (fanState) {
                case PDL_FAN_STATE_INFO_OK_E:
                    prvPdlibOsSnprintf(fanStateStr, 20, "OK");
                    break;
                case PDL_FAN_STATE_INFO_OVERFLOW_E:
                    prvPdlibOsSnprintf(fanStateStr, 20, "Overflow");
                    break;
                case PDL_FAN_STATE_INFO_FAULT_E:
                    prvPdlibOsSnprintf(fanStateStr, 20, "Fault");
                    break;
                case PDL_FAN_STATE_INFO_READY_E:
                    prvPdlibOsSnprintf(fanStateStr, 20, "Ready");
                    break;
                default:
                    prvPdlibOsSnprintf(fanStateStr, 20, "Unknown");
                    break;
            }
        }
        if (fanCapabilities.dutyCycleInfoIsSupported == TRUE) {
        ...
        if (fanCapabilities.rpmInfoSupported == TRUE) {
            IDBG_PDL_LOGGER_PDL_EXECUTE(pdlStatus, pdlFanHwRpmGet, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, tempFanControllerId), IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, tempFanId), IDBG_PDL_LOGGER_ADD_OUT_PARAM_MAC(PTR, &fanRpm));
            PDL_CHECK_STATUS(pdlStatus);
        }
        prvPdlibOsSnprintf(fanDcPercentStr, 20, "%d%%", fanDcPercent);
        ...
        switch (cmdType) {
            case IDBG_PDL_FAN_SET_CMD_STATE_E:
                if (controllerCapabilities.stateControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("State Not Supported");
                }
                break;
            ...
            case IDBG_PDL_FAN_SET_CMD_ROTATION_DIRECTION_E:
                if (controllerCapabilities.rotationDirectionControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("Rotation-Direction Not Supported");
                }
                break;
            case IDBG_PDL_FAN_SET_CMD_DUTY_CYCLE_METHOD_E:
                if (controllerCapabilities.dutyCycleMethodControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("Duty-Cycle-Method Not Supported");
                }
                break;
            case IDBG_PDL_FAN_SET_CMD_FAULT_E:
                if (controllerCapabilities.faultControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("Fault Method Not Supported");
                }
                break;
            ...
            case IDBG_PDL_FAN_SET_CMD_PULSE_PER_ROTATION_E:
                if (controllerCapabilities.pulsesPerRotationControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("Pulse Per Rotation Not Supported");
                }
                break;
            case IDBG_PDL_FAN_SET_CMD_THRESHOLD_E:
                if (controllerCapabilities.thresholdControlIsSupported == TRUE) {
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("Threshold Not Supported");
                }
                break;
            default:
    ...
    for (i = firstfanControllerId; i <lastfanControllerId; i++) {
        ...
        for (j = firstLogicalFanId; j <lastFanLogicalId; j++) {
            ...
            if (tempFanControllerId == i) {
                ...
                if (strlen(fans) < sizeof(fans) - 5) {
                    strncat(fans, tempfan, 5);
                }
            }
        ...
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_FAN_TYPE_E, fanType, &fanTypeString);
        PDL_CHECK_STATUS(pdlStatus);
        ...
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_I2C_TRANSACTION_TYPE_E, fanAttributes.transactionType, &fanTransactionTypeString);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, fanInterface.interfaceType, &fanInterfaceTypeString);
        PDL_CHECK_STATUS(pdlStatus);
        prvPdlibOsSnprintf(i2cInterfaceId, 25, "%d",  fanInterface.interfaceId);
...
)
{
    ...
    PDL_FAN_DIRECTION_ENT           direction = PDL_FAN_DIRECTION_LAST_E;
    PDL_STATUS                      pdlStatus;
    PDL_FAN_CONTROLLER_XML_CAPABILITIES_STC    controllerCapabilities;
    BOOLEAN                         find = FALSE;
    /* ***************************************************************************/
    ...
    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-20s | %-20s |\n", "       Ctrl ID      ", " Rotation Direction ");
    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-20s | %-20s |\n", "--------------------", "--------------------");
    ...
    for (i =firstfanControllerId; i<lastfanControllerId; i++) {
        ...
        if (controllerCapabilities.rotationDirectionControlIsSupported == TRUE) {
            pdlStatus =  pdlFanControllerHwRotationDirectionGet(i, &direction);
            PDL_CHECK_STATUS(pdlStatus);
            PRV_IDBG_PDL_LOGGER_SNPRINTF_MAC(directionStr, sizeof(directionStr)-1, "%s", (direction == PDL_FAN_DIRECTION_LEFT_E) ? "left" : "right");
        }
        else {
            prvPdlibOsSnprintf(directionStr, 20, "Not Supported");
        }

        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-20d | %-20s |\n",
                                       i,
                                       directionStr);
    }
