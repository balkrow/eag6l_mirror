...
)
{
    ...
    pdlStatus = iDbgPdlConvertStringToEnum(IDBG_PDL_UTILS_INTERFACE_EXTENDED_TYPE, interfaceTypeStr, (UINT_32*) &interfaceType);
    ...
    switch (interfaceType) {
        case PDL_INTERFACE_EXTENDED_TYPE_GPIO_E:
            ...
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-12s | %-11s | %-10s | %-26s | %-10s |\n", "------------", "-----------", "----------", "--------------------------", "----------");
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_MPP_PIN_MODE_TYPE_E, gpioInfo.type, &gpioType);
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(gpioInitValueStr, 10, "%d", gpioInfo.initialValue);
        ...
        case PDL_INTERFACE_EXTENDED_TYPE_I2C_E:
            ...
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-12s | %-10s | %-7s | %-11s | %-8s | %-10s | %-9s |\n", "------------", "----------", "-------", "-----------", "--------", "----------", "---------");
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_I2C_ACCESS_TYPE_E, i2cInfo.accessSize, &i2cAcessSize);
            PDL_CHECK_STATUS(pdlStatus);
            ...
            if (i2cInfo.numberOfMuxes > 1) {
                ...
                for (i = 1; i < i2cInfo.numberOfMuxes; i++) {
                    ...
                    PDL_CHECK_STATUS(pdlStatus);
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("#%-6d | %-4s | %-20s |\n", i, muxInfo.interfaceType==PDL_INTERFACE_EXTENDED_TYPE_GPIO_E ? "GPIO":"I2C", "");
        ...
        case PDL_INTERFACE_EXTENDED_TYPE_MPD_E:
            ...
            PDL_CHECK_STATUS(pdlStatus);
            if (smiInfo.interfaceType == PDL_INTERFACE_EXTENDED_TYPE_SMI_E ||  smiInfo.interfaceType == PDL_INTERFACE_EXTENDED_TYPE_XSMI_E) {
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-6s | %-15s | %-8s |\n", "Interface Type", "Device", "Phy InterfaceId", "Address ");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-6s | %-15s | %-8s |\n", "--------------", "------", "---------------", "--------");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-6d | %-15d | %-8d |\n", smiInfo.interfaceType==PDL_INTERFACE_EXTENDED_TYPE_SMI_E ?"SMI":"XSMI",
                                               smiInfo.phyInfo.readWriteAddress.dev, smiInfo.phyInfo.readWriteAddress.interfaceId, smiInfo.phyInfo.readWriteAddress.address);
            }
            else if (smiInfo.interfaceType == PDL_INTERFACE_EXTENDED_TYPE_MPD_E) {
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-14s |\n", "Interface Type", "mpdLogicalPort");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-14s |\n", "--------------", "--------------");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-14s | %-14d |\n", "MPD_PHY", smiInfo.phyInfo.mpdLogicalPort);
            }
            else {
                return PDL_BAD_VALUE;
            }
            break;
        case PDL_INTERFACE_EXTENDED_TYPE_PPREG_E:
            pdlStatus = PdlPpRegDebugDbGetAttributes(interfaceId, &ppRegInfo);
            PDL_CHECK_STATUS(pdlStatus);
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-12s | %-10s | %-10s | %-10s |\n", "Interface-ID", "  Device  ", " Address  ", "   mask   ");
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-12s | %-10s | %-10s | %-10s |\n", "------------", "----------", "----------", "----------");
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-12d | %-10d | 0x%08x | 0x%08x |\n", interfaceId, ppRegInfo.dev, ppRegInfo.regAddr, ppRegInfo.mask);
            break;
        default:
...
)
{
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_INTERFACE_TYPE_E, interfaceTypeStr, (UINT_32*) &interfaceType);
    PDL_CHECK_STATUS(pdlStatus);

    pdlStatus = prvPdlInterfaceHwGetValue(interfaceType, interfaceId, &value);
    PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    /* ***************************************************************************/
    PDL_UNUSED_PARAM(resultPtr);
    PDL_UNUSED_PARAM(interfaceId);
    PDL_UNUSED_PARAM(value);
...
)
{
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_MPP_PIN_MODE_TYPE_E, typeStr, (UINT_32*) &typeId);
    if (pdlStatus != PDL_OK) {
        typeId = 0;
    }
...
)
{
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_I2C_ACCESS_TYPE_E, accessSizeStr, (UINT_32*) &accessSize);
    if (pdlStatus != PDL_OK) {
        accessSize = 0;
    }
...
)
{
    ...
    smiInfo.phyInfo.readWriteAddress.dev = (UINT_8) smiDev;
    smiInfo.phyInfo.readWriteAddress.interfaceId = (UINT_8) phyInterfaceId;
    smiInfo.phyInfo.readWriteAddress.address = (UINT_8) smiAddress;
