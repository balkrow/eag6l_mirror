...
)
{
    ...
    for (i = firstPhyId; i <lastPhyId; i++) {
        ...
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PHY_TYPE_TYPE_E, phyInfo.phyType, &phyTypeString);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PHY_FIRMWARE_DOWNLOAD_METHOD_TYPE_E, phyInfo.phyDownloadType, &phyDownloadTypeString);
        PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    for (i = firstPhyId; i <lastPhyId; i++) {
        ...
        if (pdlStatus == PDL_OK) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PHY_TYPE_TYPE_E, phyInfo.phyType, &phyTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            PhyInfoSt = "Pass";
        }
        else {
            pdlibStatusToString(pdlStatus, &PhyInfoSt);
            phyTypeSt = PhyInfoSt;
        }
