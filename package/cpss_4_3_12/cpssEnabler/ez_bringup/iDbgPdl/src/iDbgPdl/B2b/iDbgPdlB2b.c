...
)
{
    ...
    for (b2bLink = 0; b2bLink < ppAttributes.numOfBackToBackLinksPerPp; b2bLink ++) {
        ...
        while (pdlStatus == PDL_OK) {
            isSerdes = TRUE;
            prvPdlibOsSnprintf(tempSerde, 4, "%d, ",  serdesInfo.absSerdesNum);
            strncat(leftSerdesList, tempSerde, 5);
            pdlStatus = pdlSerdesPortDbSerdesInfoGetNext(b2bAttributes.firstDev, b2bAttributes.firstPort, &serdesInfo, &serdesInfo);
        }
        if (!isSerdes) {
            strcpy(leftSerdesList, "Not Found");
        }
        isSerdes = FALSE;
        ...
        while (pdlStatus == PDL_OK) {
            isSerdes = TRUE;
            prvPdlibOsSnprintf(tempSerde, 4, "%d, ",  serdesInfo.absSerdesNum);
            strncat(rightSerdesList, tempSerde, 5);
            pdlStatus = pdlSerdesPortDbSerdesInfoGetNext(b2bAttributes.secondDev, b2bAttributes.secondPort, &serdesInfo, &serdesInfo);
        }
        if (!isSerdes) {
            strcpy(rightSerdesList, "Not Found");
        }
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, b2bAttributes.interfaceMode, &modePtr);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, b2bAttributes.maxSpeed, &speedPtr);
        PDL_CHECK_STATUS(pdlStatus);
        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-11d | %-7d | %-8d | %-12d | %-13d | %-18d | %-18d | %-5s | %-10s | %-12s | %-12s |\n", b2bLink, b2bAttributes.firstDev, b2bAttributes.firstPort, b2bAttributes.secondDev, b2bAttributes.secondPort, b2bAttributes.firstMacPort, b2bAttributes.secondMacPort, speedPtr, modePtr, leftSerdesList, rightSerdesList);
    }
...
)
{
    ...
    for (b2bLink = 0; b2bLink < ppAttributes.numOfBackToBackLinksPerPp; b2bLink ++) {
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &InfoSt);
        }
