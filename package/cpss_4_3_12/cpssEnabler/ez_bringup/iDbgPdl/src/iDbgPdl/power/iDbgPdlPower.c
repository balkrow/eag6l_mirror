...
)
{
    ...
    /*!****************************************************************************/
    PDL_POWER_CAPABILITIES_STC              capabilities;
    ...
    if (flagAll == FALSE) {
        if ((strcmp(powerId, "rps") == 0) && (capabilities.RpsIsSupported == FALSE)) {
            ...
            PDL_CHECK_STATUS(pdlStatus);
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, interfacePs.interfaceType, &powerInterfaceTypeStr);
            PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    if (flagAll == FALSE) {
        if ((strcmp(powerId, "rps") == 0) && (capabilities.RpsIsSupported == FALSE)) {
            ...
            PDL_CHECK_STATUS(pdlStatus);
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_POWER_STATUS_TYPE_E, rpsStatus, &rpsStatusStr);
            PDL_CHECK_STATUS(pdlStatus);
    ...
    if (flagAll == FALSE) {
        if ((strcmp(powerId, "rps") == 0) && (capabilities.RpsIsSupported == FALSE)) {
            ...
            if (pdlStatus == PDL_OK) {
                ...
                pdlibStatusToString(pdlStatus, &statusStr);
            }
            pdlStatus = pdlPowerDbCapabilitiesGet(&capabilities);
            if (pdlStatus == PDL_OK) {
                pdlStatus = pdlPowerDebugStatusAccessInfoGet(typePs, &interfacePs);
