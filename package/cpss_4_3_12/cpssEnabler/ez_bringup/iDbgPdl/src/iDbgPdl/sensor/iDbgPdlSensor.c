...
)
{
    ...
    for (i = firstSensorId; i <lastSensorId; i++) {
        ...
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SENSOR_HW_TYPE_E, sensorInfo.sensorType, &sensorTypeString);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, sensorInfo.sensorTempratureInterface.interfaceType, &interfaceTypeString);

        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-9d | %-8s | %-20s | %-15d | %-15s |\n",
                                       i,
                                       (sensorTypeString == NULL) ? "" : sensorTypeString,
                                       sensorInfo.sensorNamePtr,
...
)
{
    ...
    for (i = firstSensorId; i <lastSensorId; i++) {
        ...
        if (sensorInfo.sensorType == PDL_SENSOR_TYPE_BOARD_E ||
            ...
            IDBG_PDL_LOGGER_PDL_EXECUTE(pdlStatus, pdlSensorHwTemperatureGet, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, i), IDBG_PDL_LOGGER_ADD_OUT_PARAM_MAC(INT_32, &sensorTemprature));
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(sensorTemperatureString, 70, "%d", sensorTemprature);
        }
        else {
            prvPdlibOsSnprintf(sensorTemperatureString, 70, "Sensor Temperature is unreadable (not a BOARD type sensor)");
        }
...
)
{
    ...
    if (pdlStatus == PDL_OK) {
        ...
        pdlibStatusToString(pdlStatus, &sensorCountSt);
    }
    ...
    for (i = firstSensorId; i <lastSensorId; i++) {
        ...
        if (pdlStatus == PDL_OK) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SENSOR_HW_TYPE_E, sensorInfo.sensorType, &sensorTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            sensorInfoSt = "Pass";
        }
        else {
            pdlibStatusToString(pdlStatus, &sensorInfoSt);
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &sensorTemperatureSt);
        }
