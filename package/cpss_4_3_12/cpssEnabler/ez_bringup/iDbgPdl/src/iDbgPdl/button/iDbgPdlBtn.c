...
)
{
    ...
    for (i = firstBtnId; i <lastBtnId; i++) {
        ...
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_BUTTON_TYPE_E, i, &btnNamePtr);
        PDL_CHECK_STATUS(pdlStatus);

        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-10s | %-9s | %-14s | %-8d |\n",
                                       btnNamePtr,
                                       btnCapabilities.isSupported ==  PDL_IS_SUPPORTED_YES_E ? "+":"-",
                                       btnCapabilities.isSupported ? btnInterface.interfaceType == PDL_INTERFACE_TYPE_INTERFACE_GPIO_E ? "GPIO":PDL_INTERFACE_TYPE_INTERFACE_I2C_E ? "I2C":"Unknown" : "Not relevant",
                                       btnInterface.values.pushValue);
    }
    pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, btnInterface.interfaceType, &btnInterfaceTypeStr);
    if ((single) && (pdlStatus == PDL_OK)) {
        IDBG_PDL_LOGGER_API_OUTPUT_LOG("\n");
        pdlStatus = iDbgPdlInterfaceGetInfo(resultPtr, btnInterfaceTypeStr, 0);
        PDL_CHECK_STATUS(pdlStatus);
    }
...
)
{
    ...
    for (i = firstBtnId; i <lastBtnId; i++) {
        ...
        pdlibStatusToString(pdlStatusCapabilities, &pdlStatusStr);
        if (pdlStatusCapabilities == PDL_OK && btnCapabilities.isSupported == PDL_IS_SUPPORTED_YES_E) {
            ...
            pdlibStatusToString(pdlStatusHw, &pdlStatusStr);
        }
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_BUTTON_TYPE_E, i, &btnNamePtr);
        PDL_CHECK_STATUS(pdlStatus);
        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-10s | %-16s | %-14s |\n", btnNamePtr,
