...
static PDL_STATUS prvIDbgPdlInbandInfoGetWrapper(
    ...
    /*!     OUTPUTS:            */
    OUT PDL_PP_NETWORK_PORT_ATTRIBUTES_STC     *portDataPtr
)
{
    /*!****************************************************************************/
...
)
{
    ...
    PDL_STATUS                              pdlStatus;
    PDL_PP_NETWORK_PORT_ATTRIBUTES_STC      portInfo;
    ...
    for (; pdlStatus == PDL_OK; pdlStatus = pdlPpDbPortAttributesGetNext(i_dev, i_port, &i_dev, &i_port)) {
        ...
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, portInfo.transceiverType, &transceiverTypeStr);
        ...
        for (index = 0; index < portInfo.numOfCopperModes || index < portInfo.numOfFiberModes; index++) {
            ...
            if (index < portInfo.numOfCopperModes) {
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, portInfo.copperModesArr[index].speed, &speedStr);
                PDL_CHECK_STATUS(pdlStatus);
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, portInfo.copperModesArr[index].mode, &modeStr);
                PDL_CHECK_STATUS(pdlStatus);
            }
            else {
                speedStr = "";
            ...
            if (index < portInfo.numOfFiberModes) {
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, portInfo.fiberModesArr[index].speed, &speedStr);
                PDL_CHECK_STATUS(pdlStatus);
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, portInfo.fiberModesArr[index].mode, &modeStr);
                PDL_CHECK_STATUS(pdlStatus);
            }
            else {
                speedStr = "";
...
)
{
    ...
    /* L O C A L   D E C L A R A T I O N S   A N D   I N I T I A L I Z A T I O N */
    /* ***************************************************************************/
    UINT_32                                 i_port, firstPortId, lastPortId, portCount, numOfPhys;
    PDL_STATUS                              pdlStatus;
    PDL_PP_NETWORK_PORT_ATTRIBUTES_STC      portInfo;
    char                                    phyNumStr[20], phyPosStr[20], *ifTypePtr, ifAddressStr[20], ifDevStr[20], ifIdStr[20], mpdLogicalPort[20];
    /****************************************************************************/
    /*                     F U N C T I O N   L O G I C                          */
    /****************************************************************************/
    PDL_UNUSED_PARAM(resultPtr);
    ...
    pdlStatus = pdlPhyDbNumberOfPhysGet(&numOfPhys);
    PDL_CHECK_STATUS(pdlStatus);
    ...
    if (numOfPhys > 0) {
        ...
        for (i_port = firstPortId; i_port < lastPortId; i_port++) {
            ...
            if (portInfo.isPhyExists) {
                prvPdlibOsSnprintf(phyNumStr, 20,  "%d", portInfo.phyData.phyNumber);
                prvPdlibOsSnprintf(phyPosStr, 20,  "%d", portInfo.phyData.phyPosition);
                prvPdlibOsSnprintf(ifAddressStr, 20, "%d", portInfo.phyData.smiXmsiInterface.phyInfo.readWriteAddress.address);
                prvPdlibOsSnprintf(ifDevStr, 20,   "%d", portInfo.phyData.smiXmsiInterface.phyInfo.readWriteAddress.dev);
                prvPdlibOsSnprintf(ifIdStr, 20,    "%d", portInfo.phyData.smiXmsiInterface.phyInfo.readWriteAddress.interfaceId);
                pdlStatus = iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_INTERFACE_EXTENDED_TYPE, portInfo.phyData.smiXmsiInterface.interfaceType, &ifTypePtr);
                PDL_CHECK_STATUS(pdlStatus);
            }
            else {
                prvPdlibOsSnprintf(phyNumStr, 20, "NA");
                prvPdlibOsSnprintf(phyPosStr, 20, "NA");
                prvPdlibOsSnprintf(ifAddressStr, 20, "NA");
                prvPdlibOsSnprintf(ifDevStr, 20, "NA");
                prvPdlibOsSnprintf(ifIdStr, 20, "NA");
                ifTypePtr = "NA";
            }
        ...
        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-3s | %-4s | %-9s | %-7s |\n", "", "", "",          "  PHY  ");
        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-3s | %-4s | %-9s | %-7s |\n", "DEV", "PORT", "INTERFACE", "LOGICAL");
        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-3s | %-4s | %-9s | %-7s |\n", "", "", "   TYPE  ", "  PORT ");
        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-3s | %-4s | %-9s | %-7s |\n", "---", "----", "---------", "-------");
        ...
        for (i_port = firstPortId; i_port < lastPortId; i_port++) {
            ...
            if (portInfo.isPhyExists) {
                prvPdlibOsSnprintf(mpdLogicalPort, 20,  "%d", portInfo.phyData.smiXmsiInterface.phyInfo.mpdLogicalPort);
                pdlStatus = iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_INTERFACE_EXTENDED_TYPE, portInfo.phyData.smiXmsiInterface.interfaceType, &ifTypePtr);
                PDL_CHECK_STATUS(pdlStatus);
            }
            else {
                prvPdlibOsSnprintf(mpdLogicalPort, 20, "NA");
                ifTypePtr = "NA";
            }

            IDBG_PDL_LOGGER_API_OUTPUT_LOG(" %-3d | %-4d | %-9s | %-7s |\n",
                                           dev, i_port, ifTypePtr, mpdLogicalPort);
...
)
{
    ...
    PDL_STATUS                                  pdlStatus;
    PDL_PP_NETWORK_PORT_PHY_INIT_INFO_STC       phyInitValueInfo;
    PDL_PP_NETWORK_PORT_ATTRIBUTES_STC          portInfo;
    ...
    for (i_port = firstPortId; i_port < lastPortId; i_port++) {
        ...
        for (transceiverType = PDL_TRANSCEIVER_TYPE_FIBER_E; transceiverType <= PDL_TRANSCEIVER_TYPE_COPPER_E; transceiverType ++) {
            if (transceiverType == portInfo.transceiverType || portInfo.transceiverType == PDL_TRANSCEIVER_TYPE_COMBO_E) {
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, transceiverType, &transceiverTypeStr);
                PDL_CHECK_STATUS(pdlStatus);
                ...
                for (index = 0; index < numOfModes; index++) {
                    ...
                    printSpeed = TRUE;
                    pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, speed, &speedStr);
                    PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    PDL_STATUS                                  pdlStatus;
    PDL_PP_NETWORK_PORT_PHY_INIT_INFO_STC       phyPostInitValueInfo;
    PDL_PP_NETWORK_PORT_ATTRIBUTES_STC          portInfo;
...
)
{
    ...
    /* ***************************************************************************/
    UINT_32                                 tempDev, tempPort, firstDev, firstPort, tempGroupNumber, tempNumberInGroup, tempFrontPanel;
    PDL_PP_FRONT_PANEL_ATTRIBUTES_STC       frontPanelAttributes;
    ...
    if (groupNumber == IDBG_PDL_ID_ALL) {
        ...
        for (; pdlStatus == PDL_OK; pdlStatus = pdlPpDbPortAttributesGetNext(tempDev, tempPort, &tempDev, &tempPort)) {
            ...
            PDL_CHECK_STATUS(pdlStatus);
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PANEL_GROUP_ORDERING_TYPE_E, frontPanelAttributes.ordering, &orderPtr);
            PDL_CHECK_STATUS(pdlStatus);
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, frontPanelAttributes.portMaximumSpeed, &speedPtr);
            PDL_CHECK_STATUS(pdlStatus);
...
PDL_STATUS printSpeedMode(PDL_PP_NETWORK_PORT_ATTRIBUTES_STC portAttributes)
{
    ...
    switch (portAttributes.transceiverType) {
        case PDL_TRANSCEIVER_TYPE_COPPER_E:
            for (i=0; i<portAttributes.numOfCopperModes; i++) {
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, portAttributes.copperModesArr[i].mode, &modePtr);
                PDL_CHECK_STATUS(pdlStatus);
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, portAttributes.copperModesArr[i].speed, &speedPtr);
                PDL_CHECK_STATUS(pdlStatus);
                prvPdlibOsSnprintf(speedModeSt, 15, "%s, %s ",  modePtr, speedPtr);
        ...
        case PDL_TRANSCEIVER_TYPE_FIBER_E:
            for (i=0; i<portAttributes.numOfFiberModes; i++) {
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, portAttributes.fiberModesArr[i].mode, &modePtr);
                PDL_CHECK_STATUS(pdlStatus);
                pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_SPEED_TYPE_E, portAttributes.fiberModesArr[i].speed, &speedPtr);
                PDL_CHECK_STATUS(pdlStatus);
                prvPdlibOsSnprintf(speedModeSt, 15, "%s, %s ",  modePtr, speedPtr);
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-16s | %-15s |%-12s |\n", "", "", "", speedModeSt, "");
            }
...
)
{
    ...
    char                                    serdesList[12], tempSerde[8] = {0};
    ...
    if (devNumber == IDBG_PDL_ID_ALL) {
        ...
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, portAttributes.transceiverType, &transceiverTypePtr);
        PDL_CHECK_STATUS(pdlStatus);
        ...
        while (pdlStatus == PDL_OK) {
            prvPdlibOsSnprintf(tempSerde, 8, "%d, ",  serdesInfo.absSerdesNum);
            strncat(serdesList, tempSerde, 8);
...
)
{
    ...
    /* L O C A L   D E C L A R A T I O N S   A N D   I N I T I A L I Z A T I O N */
    /* ***************************************************************************/
    UINT_32                                 tempDev, tempPort, firstDev, firstPort, tempGroupNumber, tempNumberInGroup, numOfPhys;
    char                                  * phyTypePtr = NULL;
    char                                  * phyInterfaceTypePtr = NULL;
    BOOLEAN                                 single = FALSE, flagAll = FALSE;
    PDL_PP_NETWORK_PORT_ATTRIBUTES_STC      portAttributes;
    ...
    if (devNumber == IDBG_PDL_ID_ALL) {
        ...
        pdlStatus = pdlPpDbPortAttributesGet(devNumber, portNumberInDev, &portAttributes);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus =  iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_INTERFACE_EXTENDED_TYPE, portAttributes.phyData.smiXmsiInterface.interfaceType, &phyInterfaceTypePtr);
        PDL_CHECK_STATUS(pdlStatus);
        switch (portAttributes.phyData.smiXmsiInterface.interfaceType) {
            ...
            case PDL_INTERFACE_EXTENDED_TYPE_XSMI_E:
                if (portAttributes.isPhyExists == TRUE) {
                    ...
                    PDL_CHECK_STATUS(pdlStatus);
                    pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PHY_TYPE_TYPE_E, phyConfig.phyType, &phyTypePtr);
                    PDL_CHECK_STATUS(pdlStatus);
                    ...
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-18s | %-10s | %-12s | %-18s | %-11s |\n", "-------------", "--------------", "------------------", "----------", "------------", "------------------", "-----------");
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13d | %-14d | %-18s | %-10d | %-12d | %-18s | %-11d |\n", devNumber, portNumberInDev, phyTypePtr, portAttributes.phyData.phyNumber, portAttributes.phyData.phyPosition, phyInterfaceTypePtr, portAttributes.phyData.smiXmsiInterface.phyInfo.readWriteAddress.address);
                }
                else {
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("phy info doesn't exist");
                }
                break;
            case PDL_INTERFACE_EXTENDED_TYPE_MPD_E:
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-18s | %-14s |\n", "pp-dev-number", "pp-port-number",  "phy interface type", "mpdLogicalPort");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-18s | %-14s |\n", "-------------", "--------------",  "------------------", "--------------");
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13d | %-14d | %-18s | %-14d |\n", devNumber, portNumberInDev, phyInterfaceTypePtr, portAttributes.phyData.smiXmsiInterface.phyInfo.mpdLogicalPort);
                break;
            default:
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("phy info doesn't exist");
                break;
    ...
    if (flagAll == TRUE) {
        pdlStatus = pdlPhyDbNumberOfPhysGet(&numOfPhys);
        PDL_CHECK_STATUS(pdlStatus);
        /* phy configuration is external (MPD XML) */
        if (numOfPhys == 0) {
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-18s | %-14s |\n", "pp-dev-number", "pp-port-number",  "phy interface type", "mpdLogicalPort");
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-18s | %-14s |\n", "-------------", "--------------",  "------------------", "--------------");
        }
                ...
                switch (portAttributes.phyData.smiXmsiInterface.interfaceType) {
                    ...
                    case PDL_INTERFACE_EXTENDED_TYPE_XSMI_E:
                        if (portAttributes.isPhyExists == TRUE) {
                            ...
                            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_PHY_TYPE_TYPE_E, phyConfig.phyType, &phyTypePtr);
                            PDL_CHECK_STATUS(pdlStatus);
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13d | %-14d | %-18s | %-10d | %-12d | %-18s | %-11d |\n", tempDev, tempPort, phyTypePtr, portAttributes.phyData.phyNumber, portAttributes.phyData.phyPosition, phyInterfaceTypePtr, portAttributes.phyData.smiXmsiInterface.phyInfo.readWriteAddress.address);
                        }
                        else {
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("phy info doesn't exist");
                        }
                        break;
                    case PDL_INTERFACE_EXTENDED_TYPE_MPD_E:
                        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13d | %-14d | %-18s | %-14d |\n", tempDev, tempPort, phyInterfaceTypePtr, portAttributes.phyData.smiXmsiInterface.phyInfo.mpdLogicalPort);
                        break;
                    default:
                        IDBG_PDL_LOGGER_API_OUTPUT_LOG("phy info doesn't exist");
                        break;
...
)
{
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, transceiverStrPtr, &value);
    if (pdlStatus != PDL_OK) {
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_SPEED_TYPE_E, speedStrPtr, &value);
    if (pdlStatus != PDL_OK) {
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, modeStrPtr, &value);
    if (pdlStatus != PDL_OK) {
...
)
{
    ...
    PDL_PP_NETWORK_PORT_PHY_INIT_INFO_STC       initValues;
    ...
    if (postInitOrAllInitListPtr == NULL) {
        ...
        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-13s | %-14s | %-13s |\n", "-------------", "--------------", "-------------");
        pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_SPEED_TYPE_E, speedPtr, &speedValue);
        PDL_CHECK_STATUS(pdlStatus);
        pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, transceiverPtr, &transceiverValue);
        PDL_CHECK_STATUS(pdlStatus);
        ...
        for (transceiver=PDL_TRANSCEIVER_TYPE_FIBER_E; transceiver<PDL_TRANSCEIVER_TYPE_LAST_E; transceiver++) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_TRANSCEIVER_TYPE_E, transceiver, &transceiverStr);
            PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    if (postInitPtr == NULL) {
        ...
        if (strcmp(postInitPtr, "post-init") == 0) {
            ...
            IDBG_PDL_LOGGER_API_OUTPUT_LOG("Speed parameter is missing");
        }
    }
...
PDL_STATUS printSfpInfo(UINT_32 dev, UINT_32 logicalPort, PDL_SFP_INTERFACE_STC* sfpInterface)
{
    ...
    if (sfpInterface != NULL) {
        if (sfpInterface->presentInfo.isSupported) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, sfpInterface->presentInfo.interfaceType, &interfaceTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(sfpPresentSt, 30, "%s id:%-4d", interfaceTypeSt, sfpInterface->presentInfo.interfaceId);
        }
        else {
            prvPdlibOsSnprintf(sfpPresentSt, 30, "%s", "Not Supported");
        }
        if (sfpInterface->txDisableInfo.isSupported) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, sfpInterface->txDisableInfo.interfaceType, &interfaceTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(sfpTxDisableSt, 30, "%s id:%-4d", interfaceTypeSt, sfpInterface->txDisableInfo.interfaceId);
        }
        else {
            prvPdlibOsSnprintf(sfpTxDisableSt, 30, "%s", "Not Supported");
        }
        if (sfpInterface->txEnableInfo.isSupported) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, sfpInterface->txEnableInfo.interfaceType, &interfaceTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(sfpTxEnableSt, 30, "%s id:%-4d", interfaceTypeSt, sfpInterface->txEnableInfo.interfaceId);
        }
        else {
            prvPdlibOsSnprintf(sfpTxEnableSt, 30, "%s", "Not Supported");
        }
        if (sfpInterface->lossInfo.isSupported) {
            pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_INTERFACE_TYPE_E, sfpInterface->lossInfo.interfaceType, &interfaceTypeSt);
            PDL_CHECK_STATUS(pdlStatus);
            prvPdlibOsSnprintf(sfpLossSt, 30, "%s id:%-4d", interfaceTypeSt, sfpInterface->lossInfo.interfaceId);
        }
        else {
            prvPdlibOsSnprintf(sfpLossSt, 30, "%s", "Not Supported");
        }
    }
    else {
        prvPdlibOsSnprintf(sfpPresentSt, 30, "%s", "Not Supported");
...
PDL_STATUS printFrontPanelValidation(UINT_32 tempDev, UINT_32 tempPort, UINT_32 convertDev, UINT_32 convertPort, UINT_32 tempGroupNumber, UINT_32 tempNumberInGroup, PDL_STATUS pdlStatus)
{
    ...
    if ((tempDev == convertDev) && (tempPort == convertPort)) {
        ...
        pdlibStatusToString(pdlStatus, &portInfoPtr);
    }
    pdlStatus = pdlPpDbFrontPanelAttributesGet(tempGroupNumber, &frontPanelAttributes);
    if (pdlStatus == PDL_OK) {
        groupInfoPtr = "Pass";
        ...
        pdlibStatusToString(pdlStatus, &groupInfoPtr);
    }
...
)
{
    ...
    if (devNumber == IDBG_PDL_ID_ALL) {
        ...
        if (pdlStatus == PDL_OK) {
            ...
            pdlibStatusToString(pdlStatus, &sfpLossSt);
        }
        pdlStatus = pdlSfpHwPresentGet(devNumber, portNumberInDev, &sfpPresent);
        if (pdlStatus == PDL_OK) {
            sfpPresentSt = "Pass";
            ...
            pdlibStatusToString(pdlStatus, &sfpPresentSt);
        }
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    if (pdlStatus == PDL_OK) {
                        ...
                        if (pdlStatus == PDL_OK) {
                            ...
                            pdlibStatusToString(pdlStatus, &sfpTxSt);
                        }
                    }
                    else {
                        pdlibStatusToString(pdlStatus, &sfpTxSt);
