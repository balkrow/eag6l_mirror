...
#define PDL_ASSERT_TRUE(_condition) PDL_ASSERT_TRUE_GEN(_condition, prvPdlLibDebugFlag)
...
#define PRV_IDBG_PDL_LOGGER_TAG_DB_ARRAY_SIZE (sizeof(prvIDbgPdlLoggerTagDbArr)/sizeof (PRV_IDBG_PDL_LOGGER_FILE_TAG_DB_STC))
...
)
{
    ...
    if (pathPtr) {
        prvPdlibOsSnprintf(pathPtr, path_len, "%s/%s", rootPathPtr, dirPathPtr);
...
)
{
    ...
    while ((de = readdir(mainDirPtr)) != NULL) {
        sscanf(de->d_name, "run_%u", &dirIndex);
        if (sscanf(de->d_name, "run_%u", &dirIndex) && dirIndex >= maxDirIndex) {
...
#else
    prvPdlibOsSnprintf(pathPtr, path_len, "%s/*", rootPathPtr);
    for (moreFiles = ((hFile = FindFirstFile((LPCSTR)pathPtr, &FindFileData)) != INVALID_HANDLE_VALUE) ? TRUE : FALSE;
        ...
        if (sscanf((char *)FindFileData.cFileName, "run_%u", &dirIndex) && dirIndex >= maxDirIndex) {
            maxDirIndex = dirIndex;
    ...
    prvPdlibOsSnprintf(pathPtr, path_len, "%s/run_%05d", rootPathPtr, maxDirIndex);
#ifndef _WIN32
...
#endif
    {
        ...
        prvPdlibOsSnprintf(prvIDbgPdlLoggerRootPathhNamePtr, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s", rootPathPtr);
        prvPdlibOsSnprintf(prvIDbgPdlLoggerCurrentPathNamePtr, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s", pathPtr);
        pdlStatus = PDL_OK;
    }
...
)
{
    ...
    if ((ch = strtok(fullFileNamePtr, "_")) != NULL) {
        if (sscanf(ch, "%05u", commandIndexPtr) == 0) {
            return FALSE;
        }
    }
    else {
    ...
    if ((ch = strtok(NULL, "_")) != NULL) {
        if (sscanf(ch, "%80s", categoryNamePtr) == 0) {
            return FALSE;
        }
...
)
{
    ...
    /*!                      F U N C T I O N   L O G I C                          */
    /*!****************************************************************************/
    prvPdlibOsSnprintf(dirName, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s/%s", rootPathPtr, currDirNamePtr);
    ...
    while ((de = readdir(dirHandlePtr)) != NULL) {
        ...
        prvPdlibOsSnprintf(fileName, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s/%s", dirName, de->d_name);
...
#else
    prvPdlibOsSnprintf(dirName, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "/*");
    for (moreFiles = ((hFile = FindFirstFile((LPCSTR)dirName, &FindFileData)) != INVALID_HANDLE_VALUE) ? TRUE : FALSE;
        ...
        prvPdlibOsSnprintf(fileName, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s/%s/%s", rootPathPtr, currDirNamePtr, (char *)FindFileData.cFileName);
...
)
{
    ...
    prvPdlibOsSnprintf(prvIDbgPdlLoggerCurrentFileNamePtr, PRV_IDBG_PDL_LOGGER_MAX_PATH_CNS, "%s/%05d_%s_%s.log", prvIDbgPdlLoggerCurrentPathNamePtr, prvIDbgPdlLoggerCurrentRunIndex ++, ptr->categoryNamePtr, cliFuncNamePtr);
...
)
{
        ...
        for (type = va_arg(varList, UINT_32); type != IDBG_PDL_LOGGER_TYPEOF_LAST_CNS; type = va_arg(varList, UINT_32)) {
            ...
            switch (typeWOFlags) {
                case IDBG_PDL_LOGGER_TYPEOF_UINT_8_CNS  :
                    ...
                    prvPdlibOsSnprintf(numberStr, 11, "%d", *val8Ptr);
                    break;
                case IDBG_PDL_LOGGER_TYPEOF_UINT_16_CNS :
                    val16Ptr = (UINT_16 *)va_arg(varList, UINT_16 *);
                    prvPdlibOsSnprintf(numberStr, 11, "%d", *val16Ptr);
                    break;
                case IDBG_PDL_LOGGER_TYPEOF_UINT_32_CNS :
                    val32Ptr = (UINT_32 *)va_arg(varList, UINT_32 *);
                    prvPdlibOsSnprintf(numberStr, 11, "%lu", *val32Ptr);
                    break;
                case IDBG_PDL_LOGGER_TYPEOF_BOOLEAN_CNS :
                    valBPtr = (BOOLEAN *)va_arg(varList, BOOLEAN *);
                    prvPdlibOsSnprintf(numberStr, 11, "%s", *valBPtr ? "true" : "false");
                    break;
                case IDBG_PDL_LOGGER_TYPEOF_CHAR_PTR_CNS:
                    ptr = (char *)va_arg(varList, char *);
                    break;
    ...
    if (FALSE == pdlibStatusToString(resultDataPtr->result, &pdlStatusStr)) {
...
)
{
    ...
    switch (paramTypeWOFlags) {
        case IDBG_PDL_LOGGER_TYPEOF_UINT_8_CNS  :
            ...
            prvPdlibOsSnprintf(numberStr, 11, formatPtr, *val8Ptr);
            break;
        case IDBG_PDL_LOGGER_TYPEOF_UINT_16_CNS :
            val16Ptr = (UINT_16 *)paramValuePtr;
            prvPdlibOsSnprintf(numberStr, 11, formatPtr, *val16Ptr);
            break;
        case IDBG_PDL_LOGGER_TYPEOF_UINT_32_CNS :
            val32Ptr = (UINT_32 *)paramValuePtr;
            prvPdlibOsSnprintf(numberStr, 11, formatPtr, *val32Ptr);
            break;
        case IDBG_PDL_LOGGER_TYPEOF_BOOLEAN_CNS :
    ...
    PDL_ASSERT_TRUE((paramTagId+2) < PRV_IDBG_PDL_LOGGER_TAG_DB_ARRAY_SIZE);
    fprintf(prvIDbgPdlLoggerOpenedFilePtr,
...
)
{
    ...
    if (resultValueIntPtr) {
        prvPdlibOsSnprintf(numberStr, 11, "%lu", *resultValueIntPtr);
    }
...
)
{
    ...
    prvPdlibOsSnprintf(pathPtr, path_len, "%s/*", prvIDbgPdlLoggerRootPathhNamePtr);
    ...
    for (moreFiles = ((hFile = FindFirstFile((LPCSTR)pathPtr, &FindFileData)) != INVALID_HANDLE_VALUE) ? TRUE : FALSE;
        ...
        if (sscanf(namePtr, "run_%u", &dirIndex)) {
...
)
{
    ...
    /*                      F U N C T I O N   L O G I C                          */
    /* ***************************************************************************/
    if (callbacksPtr == NULL || callbacksPtr->pdLibCallbacks.mallocPtr == NULL || callbacksPtr->pdLibCallbacks.freePtr == NULL) {
        return PDL_BAD_PARAM;
    }

    PRV_IDBGPDL_FLOW_LOCK_MAC();

    PRV_IDBG_PDL_LOGGER_MALLOC_MAC = callbacksPtr->pdLibCallbacks.mallocPtr;
    PRV_IDBG_PDL_LOGGER_FREE_MAC   = callbacksPtr->pdLibCallbacks.freePtr;
    prvIDbgPdlPrintCallbackPtr     = callbacksPtr->pdLibCallbacks.printStringPtr;
