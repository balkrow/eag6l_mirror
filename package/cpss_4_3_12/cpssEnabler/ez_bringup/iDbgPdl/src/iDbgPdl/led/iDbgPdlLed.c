...
#include <pdl/interface/pdlPpRegDebug.h>
...
)
{
    ...
    /* ***************************************************************************/
    UINT_32                         i, j, key = 0, functionNum, paramNum, paramValue;
    PDL_STATUS                      pdlStatus;
    PDL_LED_CAPABILITIES_STC        ledCapabilities;
    ...
    PDL_INTERFACE_PP_REG_STC        ppRegInfo;
    char                          * ledTypePtr, *ledColorPtr, *ledStatePtr;
    char                            ledStreamFunctionName[100], ledStreamParamName[100], ledStreamReadableParamName[100], tempStr[120], ledInterfaceTypeStr[20];
    /* ***************************************************************************/
    ...
    if (ledCapabilities.isSupported) {
        ...
        if (ledCapabilities.ledType == PDL_LED_TYPE_GPIO_E) {
            ...
            for (color = PDL_LED_COLOR_GREEN_E; color < PDL_LED_COLOR_LAST_E; color++) {
                IDBG_PDL_LOGGER_PDL_EXECUTE(pdlStatus, pdlLedDebugInterfaceGetFirst, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, ledId),
                                            ...
                                            IDBG_PDL_LOGGER_ADD_OUT_PARAM_MAC(UINT_32, &key),
                ...
                if (pdlStatus == PDL_OK && ledInterface.values.value.gpioValue != PDL_LED_UNSUPPORTED_VALUE) {
                    ...
                    PDL_CHECK_STATUS(pdlStatus);
                    pdlStatus = PdlGpioDebugDbGetAttributes(ledInterface.ledInterfaceId, &gpioInfo);
                    PDL_CHECK_STATUS(pdlStatus);
                    prvPdlibOsSnprintf(tempStr, 120, "dev=%d pin=%d", gpioInfo.dev, gpioInfo.pinNumber);
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12d | %-30s | %-18d |\n",
                                                   ledTypePtr,
                                                   "GPIO",
                                                   "Not Relevant",
                                                   ledColorPtr,
                 ...
                 ledCapabilities.ledType == PDL_LED_TYPE_MPD_E) {
            for (state = PDL_LED_STATE_OFF_E; state < PDL_LED_STATE_LAST_E; state ++) {
                for (color = PDL_LED_COLOR_NONE_E; color < PDL_LED_COLOR_LAST_E; color++) {
                    pdlStatus = pdlLedDebugInterfaceGetFirst(ledId, dev, logicalPort, state, color, &key, &ledInterface);
                    if (pdlStatus == PDL_OK) {
                        pdlStatus = iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_LED_COLOR, color, &ledColorPtr);
                        PDL_CHECK_STATUS(pdlStatus);
                        ...
                        if (ledCapabilities.ledType == PDL_LED_TYPE_I2C_E && ledInterface.values.value.i2cValue != PDL_LED_UNSUPPORTED_VALUE) {
                            pdlStatus = pdlI2CDebugDbGetAttributes(ledInterface.ledInterfaceId, &i2cInfo);
                            PDL_CHECK_STATUS(pdlStatus);
                            prvPdlibOsSnprintf(tempStr, 120, "add=0x%x offs=0x%x msk=0x%x", i2cInfo.i2cAddress, i2cInfo.offset, i2cInfo.mask);
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12d | %-30s | %-18d |\n",
                                                           ledTypePtr,
                                                           "I2C",
                                                           ledStatePtr,
                        ...
                        else if ((ledCapabilities.ledType == PDL_LED_TYPE_SMI_E || ledCapabilities.ledType == PDL_LED_TYPE_XSMI_E || ledCapabilities.ledType == PDL_LED_TYPE_MPD_E) &&
                                 ledInterface.values.value.smiValue.value != PDL_LED_UNSUPPORTED_VALUE) {
                            switch (ledCapabilities.ledType) {
                                case PDL_LED_TYPE_SMI_E:
                                    prvPdlibOsSnprintf(ledInterfaceTypeStr, 20, "SMI");
                                    break;
                                case PDL_LED_TYPE_XSMI_E:
                                    prvPdlibOsSnprintf(ledInterfaceTypeStr, 20, "XSMI");
                                    break;
                                case PDL_LED_TYPE_MPD_E:
                                    prvPdlibOsSnprintf(ledInterfaceTypeStr, 20, "MPD_PHY");
                                    break;
                                default:
                                    prvPdlibOsSnprintf(ledInterfaceTypeStr, 20, "NA");
                                    break;
                            }
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12d | %-30s | %-10s: %6d |\n",
                                                           ledTypePtr,
                                                           ledInterfaceTypeStr,
                                                           ledStatePtr,
                                                           ledColorPtr,
                            ...
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-10s: 0x%04x |\n", "", "", "", "", "", "", "regAddress", ledInterface.values.value.smiValue.registerAddress);
                            ...
                            while (pdlStatus == PDL_OK) {
                                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-10s: %6d |\n", "", "", "", "", "", "", "devOrPage", ledInterface.values.value.smiValue.devOrPage);
                                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-10s: 0x%04x |\n", "", "", "", "", "", "", "regAddress", ledInterface.values.value.smiValue.registerAddress);
                                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-10s: 0x%04x |\n", "", "", "", "", "", "", "value", ledInterface.values.value.smiValue.value);
                                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-10s: 0x%04x |\n", "", "", "", "", "", "", "mask", ledInterface.values.value.smiValue.mask);
                                pdlStatus = pdlLedDebugInterfaceGetNext(ledId, dev, logicalPort, state, color, &key, &ledInterface);
            ...
            for (state = PDL_LED_STATE_OFF_E; state < PDL_LED_STATE_LAST_E; state ++) {
                ...
                if (pdlStatus == PDL_OK) {
                    pdlStatus = iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_LED_STATE, state, &ledStatePtr);
                    PDL_CHECK_STATUS(pdlStatus);
                    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-79s | %-18s |\n", ledTypePtr, "LedStream", ledStatePtr, "");
                    for (i = 0; i < functionNum; i++) {
                        ...
                        prvPdlibOsSnprintf(tempStr, 120, "func #%d: %s", i, ledStreamFunctionName);
                        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-100s |\n", "", "", tempStr);
                        ...
                        for (j = 0; j < paramNum; j++) {
                            ...
                            PDL_CHECK_STATUS(pdlStatus);
                            prvPdlibOsSnprintf(tempStr, 120, "param #%d: %s", j, ledStreamReadableParamName);
                            IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-79s | %-18d |\n", "", "", tempStr, paramValue);
                        }
                    }
            ...
            IDBG_PDL_LOGGER_PDL_EXECUTE(pdlStatus, pdlLedDebugInterfaceGetFirst, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, ledId),
                                        ...
                                        IDBG_PDL_LOGGER_ADD_OUT_PARAM_MAC(PTR, &ledInterface));
            if (pdlStatus == PDL_OK) {
                IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12d | %-30s | %-18s |\n",
                                               ledTypePtr,
                                               "EXTERNAL",
                                               "----",
                                               "----",
        ...
        else if (ledCapabilities.ledType == PDL_LED_TYPE_PP_REG_E) {
            for (state = PDL_LED_STATE_OFF_E; state < PDL_LED_STATE_LAST_E; state ++) {
                for (color = PDL_LED_COLOR_NONE_E; color < PDL_LED_COLOR_LAST_E; color++) {
                    pdlStatus = pdlLedDebugInterfaceGetFirst(ledId, dev, logicalPort, state, color, &key, &ledInterface);
                    if (pdlStatus == PDL_OK) {
                        ...
                        pdlStatus = iDbgPdlConvertEnumToString(IDBG_PDL_UTILS_LED_STATE, state, &ledStatePtr);
                        PDL_CHECK_STATUS(pdlStatus);
                        pdlStatus = PdlPpRegDebugDbGetAttributes(ledInterface.ledInterfaceId, &ppRegInfo);
                        PDL_CHECK_STATUS(pdlStatus);
                        prvPdlibOsSnprintf(tempStr, 120, "dev=%d reg=x%x", ppRegInfo.dev, ppRegInfo.regAddr);
                        IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12d | %-30s | 0x%08x%-8s |\n",
                                                       ledTypePtr,
                                                       "PP Register",
                                                       ledStatePtr,
                                                       ledColorPtr,
                                                       ledInterface.ledInterfaceId,
                                                       tempStr,
                                                       ledInterface.values.value.ppRegValue, "");
                        pdlStatus = pdlLedDebugInterfaceGetNext(ledId, dev, logicalPort, state, color, &key, &ledInterface);
                        while (pdlStatus == PDL_OK) {
                            pdlStatus = PdlPpRegDebugDbGetAttributes(ledInterface.ledInterfaceId, &ppRegInfo);
                            PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-18s |\n", "      Led ID      ", "Interface Type", "       State        ", "Color", "Interface ID","        Interface Info        ", "       Value      ");
    IDBG_PDL_LOGGER_API_OUTPUT_LOG("%-18s | %-15s | %-20s | %-8s | %-12s | %-30s | %-18s |\n", "------------------", "--------------", "--------------------", "-----", "------------","------------------------------",  "------------------");
...
)
{
    ...
    if (state == PDL_LED_STATE_OFF_E || state == PDL_LED_STATE_INIT_E || state == PDL_LED_STATE_ALTERNATE_E) {
...
)
{
    ...
    if (pdlStatus != PDL_OK) {
        pdlibStatusToString(pdlStatus, &pdlStatusStr);
...
)
{
    ...
    PDL_LED_CAPABILITIES_STC        capabilitiesPtr;
    PDL_STATUS                      pdlStatus;
    /* ***************************************************************************/
    /*                      F U N C T I O N   L O G I C                          */
    /* ***************************************************************************/
        ...
        if (pdlStatus == PDL_OK) {
            if (capabilitiesPtr.isSupported == TRUE) {
                ...
                if (pdlStatus == PDL_OK) {
                    ...
                    pdlibStatusToString(pdlStatus, &infoStr);
                }
                printf("%-15s | %-15s | %-15s | %-20s | %-20s |\n", ledTypeStr, infoStr, "", "", "");
                for (ledColor = PDL_LED_COLOR_NONE_E; ledColor< PDL_LED_COLOR_LAST_E; ledColor++) {
                    for (ledState = PDL_LED_STATE_OFF_E; ledState<PDL_LED_STATE_LAST_E; ledState++) {
                        ...
                        if (pdlStatus == PDL_OK) {
                            ...
                            pdlibStatusToString(pdlStatus, &hwStr);
                        }
