...
)
{
    ...
    pdlStatus = pdlLibConvertEnumValueToString(PDL_ENUM_ID_CPU_TYPE_TYPE_E, cpuType, &cpuTypeStr);
    PDL_CHECK_STATUS(pdlStatus);
...
)
{
    ...
    pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_CPU_TYPE_TYPE_E, cpuTypeStrPtr, &value);
    if (pdlStatus != PDL_OK) {
        IDBG_PDL_LOGGER_API_OUTPUT_LOG("Unknown cpu type. Possibly your xml file typedef section is out-of-date.\n");
        return pdlStatus;
    }

    IDBG_PDL_LOGGER_PDL_EXECUTE(pdlStatus, pdlCpuTypeSet, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, value));
    PDL_CHECK_STATUS(pdlStatus);
