...
typedef struct {
    ...
    UINT_32                 blinkSelect[PDL_LED_COLOR_LAST_E][PDL_LED_STATE_LAST_E];        /* value that will be written to blink offset to set this blinking mode (two modes are currently supported) */
    BOOLEAN                 blinkRateControlIsSupported[PDL_LED_COLOR_LAST_E];
...
typedef struct {
    /** @brief   The value[ pdl LED color last e][pdl LED state last e] */
    PRV_PDLIB_DB_TYP            value[PDL_LED_COLOR_LAST_E][PDL_LED_STATE_LAST_E];
...
typedef struct {
    PRV_PDLIB_DB_TYP          functionCallDb[PDL_LED_STATE_LAST_E];
...
typedef struct {
    UINT_32                             index;
} PDL_LED_PP_REG_KEY_STC;

typedef struct {
    UINT_32                             value;
} PDL_LED_PP_REG_VALUE_ENTRY_STC;

typedef struct {
    PRV_PDLIB_DB_TYP                    valueDb[PDL_LED_COLOR_LAST_E][PDL_LED_STATE_LAST_E];
} PDL_LED_PP_REG_VALUES_STC;

typedef struct {
    PDL_INTERFACE_TYP                   interfaceId;
} PDL_LED_PP_REG_INTERFACE_ENTRY_STC;

typedef struct {
    PRV_PDLIB_DB_TYP                    colorStateInterfaceDb[PDL_LED_COLOR_LAST_E][PDL_LED_STATE_LAST_E];
} PDL_LED_PP_REG_INTERFACES_STC;
...
typedef struct {
    union {
        ...
        PDL_LED_PP_REG_INTERFACES_STC           ppRegInterface;
    } interface;
    ...
    union {
        ...
        PDL_LED_PP_REG_VALUES_STC           ppRegValues;
    } values;
    /** @brief   The capabilities */
    PDL_LED_CAPABILITIES_STC                capabilities;
} PDL_LED_PRV_DB_STC;
...
typedef struct {
    ...
    PRV_PDLIB_DB_TYP                      manipulationList[PDL_LED_COLOR_LAST_E][PDL_LED_STATE_LAST_E];
    PRV_PDLIB_DB_TYP                      leftPortModeManipulationDb;
    PRV_PDLIB_DB_TYP                      rightPortModeManipulationDb;
...
typedef struct {
    ...
    PRV_PDLIB_DB_TYP                      manipulationList;
} PDL_LED_PRV_LEDSTREAM_PORT_MODE_INFO_STC;

/**
 * @struct  PDL_LED_PRV_GPIO_BLINK_MAPPING_KEY_STC
 *
 * @brief   defines structure for gpio blink0/blink1 mapping
 */
typedef struct {
    UINT_32                             dev;
    PDL_LED_STATE_ENT                   ledState;
} PDL_LED_PRV_GPIO_BLINK_MAPPING_KEY_STC;

/**
 * @struct  PDL_LED_PRV_GPIO_BLINK_MAPPING_DATA_STC
 *
 * @brief   defines dev& state gpio blink value (0/1)
 */

typedef struct {
    UINT_32                               blinkSelect; /* value that will be written to blink offset to set this blinking mode (two modes are currently supported) */
    UINT_32                               onDurationRegisterOffset;
    UINT_32                               onDurationValue;
    UINT_32                               offDurationRegisterOffset;
    UINT_32                               offDurationValue;
} PDL_LED_PRV_GPIO_BLINK_MAPPING_DATA_STC;
