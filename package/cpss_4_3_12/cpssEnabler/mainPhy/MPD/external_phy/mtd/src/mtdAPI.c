...
status of the Marvell 88X32X0, 88X33X0, 88X35X0, CUX36X0, CUE26X0, 88E20X0 
 ...
 ****************************************************************** */
MTD_STATIC MTD_STATUS mtdAutonegIsSpeedDuplexResolutionDone
(
...
MTD_STATIC MTD_STATUS mtdAutonegIsCompleted
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
MTD_STATIC MTD_STATUS mtdReadMasterOrSlaveBit
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
MTD_STATIC MTD_STATUS mtdGetMasterSlaveConfigFault
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,15,1,1)); /* bit self clears when done */

    if (timeoutMs)
    {
        counter=0;
        MTD_ATTEMPT(mtdIsPhyReadyAfterReset(devPtr,port,&phyReady));
...
{    
    ...
    *downloadMode = MTD_FALSE;
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,MTD_BOOT_STATUS_REG,&val));
...
)
{
    ...
    *tunitReady = MTD_TRUE;
    ...
    MTD_ATTEMPT(mtdIsPhyReadyAfterReset(devPtr,port,tunitReady));
    ...
    MTD_ATTEMPT(mtdIsTunitInLowPowerMode(devPtr,port,&lowpowermode));
    ...
    /* Check if unreleased code has had a BIST or fatal error of some kind, which will cause code
       to hang instead of restarting. Also check on a released code or unreleased code for fatal BIST errors or
       BIST information that indicates the T unit is not ready yet. */
    MTD_ATTEMPT(mtdGetFirmwareVersion(devPtr,port,&major,&minor,&inc,&test));
    MTD_ATTEMPT(mtdGetTunitBISTStatus(devPtr,port,&bistStatus));

    if (((test != 0) && 
         (bistStatus & (MTD_BIST_ABNORMAL_RESTART | MTD_BIST_ECC_CORRECT))) ||
         (bistStatus & (MTD_BIST_T_UNIT_NOT_READY)))
    {
        *tunitReady = MTD_FALSE;
        return MTD_OK;        
    }
    ...
    MTD_ATTEMPT(mtdGetTunitFirmwareMode(devPtr,port,&tunitMode,&tunitState));
...
)              
{
    ...
    *phyReady = MTD_FALSE;
    ...
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,15,1,&val));
...
)
{
    ...
    for (horxunit=MTD_X_UNIT; horxunit<=MTD_H_UNIT; horxunit++)
    {
        if ((horxunit == MTD_X_UNIT && MTD_HAS_X_UNIT(devPtr)) ||
            (horxunit == MTD_H_UNIT))
        {
            MTD_ATTEMPT(mtdGet1000BXSGMIIControl(devPtr,port,horxunit,&sgmiiloopback,&sgmiispeed,&sgmiian_enable,&dummy));
            MTD_ATTEMPT(mtdGet10GBRControl(devPtr,port,horxunit,&xfiloopback,&dummy));

            MTD_ATTEMPT(mtdSet1000BXSGMIIControl(devPtr,port,horxunit,sgmiiloopback,sgmiispeed,sgmiian_enable,MTD_TRUE,MTD_FALSE,MTD_FALSE));
            MTD_ATTEMPT(mtdSet10GBRControl(devPtr,port,horxunit,xfiloopback,MTD_TRUE,MTD_FALSE));
        }
    }

    MTD_ATTEMPT(mtdPutTunitInLowPowerMode(devPtr,port));
    ...
    else
    {
        ...
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,3,0x8FC6,&tmp1));
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,3,0x8FC3,&tmp2));

        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,3,0x8FC6,15,1,1));
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,3,0x8FC6,12,2,1));

        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,3,0x8FC3,15,1,1));
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,3,0x8FC3,12,1,1));
        
        /* delay only needs to be 1 us, guaranteed by serial interface speed and rmw access */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,3,0x8FC6,tmp1));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,3,0x8FC3,tmp2));
    }    
...
)
{
    ...
    for (horxunit=MTD_X_UNIT; horxunit<=MTD_H_UNIT; horxunit++)
    {
        ...
        {        
            ...
            MTD_ATTEMPT(mtdSet1000BXSGMIIControl(devPtr,port,horxunit,sgmiiloopback,sgmiispeed,sgmiian_enable,MTD_FALSE,MTD_FALSE,MTD_FALSE));
            MTD_ATTEMPT(mtdSet10GBRControl(devPtr,port,horxunit,xfiloopback,MTD_FALSE,MTD_FALSE));
        }
    }

    MTD_ATTEMPT(mtdRemoveTunitLowPowerMode(devPtr,port));
...
{
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,31,0xF008,5,1,1));
    MTD_ATTEMPT(mtdTunitHwReset(devPtr,port));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,31,0xF008,5,1,0));
    MTD_ATTEMPT(mtdTunitHwReset(devPtr,port));
    ...
    OUT MTD_U8 *test)
{
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,49169,&reg_49169));
    
    *major = (reg_49169 & 0xFF00) >> 8;
    *minor = (reg_49169 & 0x00FF);

    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,49170,&reg_49170));
...
{
	MPD_UNUSED_PARAM(port);
    *speed_bits = MTD_ADV_NONE;

    if (MTD_IS_E2XX0_DEVICE(devPtr->deviceId))
    {
       *speed_bits = MTD_SPEED_ALL_2XX0; /* this needs to be first, because it's based on 33X0 and 35X0 device type */
    }    
    ...
    {
        *speed_bits = MTD_SPEED_ALL_33X0_35X0;
    }
    else if (MTD_IS_E21X0_BASE(devPtr->deviceId))
    {
        if (MTD_IS_E21X1_DEVICE(devPtr->deviceId))
        {
            *speed_bits = MTD_SPEED_ALL_21X1;
        }
        else
        {
            *speed_bits = MTD_SPEED_ALL_2XX0;
        }
    }
    else if (MTD_IS_X35X0_BASE(devPtr->deviceId))
    {
        *speed_bits = MTD_SPEED_ALL_33X0_35X0;
    }
    else if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        *speed_bits = MTD_SPEED_ALL_36X0;    
    }
    else
    {
...
)
{
    ...
    MTD_ATTEMPT(mtdGetForcedSpeed(devPtr,port,&speedForced,&dummy));
    if (speedForced)
    {
        MTD_ATTEMPT(mtdUndoForcedSpeed(devPtr,port,MTD_FALSE));
    }
    ...
    else
    {
        ...
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,7,0x0020,&tempRegValue));
        MTD_ATTEMPT(mtdHwSetRegFieldToWord(tempRegValue,MTD_GET_10GBT_BIT(speed_bits),\
        ...
        {            
            ...
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(tempRegValue,MTD_GET_2P5GBT_BIT(speed_bits),\
                    7, 1, \
                    &tempRegValue));

            /* Now take care of 5G bit in 7.0020 */
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(tempRegValue,MTD_GET_5GBT_BIT(speed_bits),\
                    8, 1, \
                    &tempRegValue));                 
        }
    
        /* Now write result back to 7.0020 */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,7,0x0020,tempRegValue));
        ...
        if (MTD_GET_10GBT_BIT(speed_bits) || MTD_GET_2P5GBT_BIT(speed_bits) || MTD_GET_5GBT_BIT(speed_bits))
        {
            ...
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,7,0,13,1,1));
        }
    }   
...
)
{
    ...
    *speed_bits |= (xgig_speed <<= MTD_XGIG_SPEED_POS);
    ...
    if (MTD_IS_X33X0_BASE(devPtr->deviceId) || MTD_IS_E21X0_BASE(devPtr->deviceId) || MTD_IS_X35X0_BASE(devPtr->deviceId) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
...
)
{
    ...
    switch (forcedSpeedBit)
    {
        ...
        case MTD_SPEED_10M_FD_AN_DIS:
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,13,1,0));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,6,1,0));
            ...
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,7,0x8000,4,1,duplexBit));
            break;

        case MTD_SPEED_100M_HD_AN_DIS:
        case MTD_SPEED_100M_FD_AN_DIS:
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,13,1,1));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,6,1,0));
    ...
    /* 1G or 10G in 1.0 */
    MTD_ATTEMPT(mtdAutonegDisable(devPtr,port));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,13,1,1));
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,6,1,1));
...
)
{
    ...
    *forcedSpeed = MTD_ADV_NONE;
    ...
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,7,0,12,1,&val));
    ...
    if (anDisabled)
    {
        ...
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,6, 1,&bit0));
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,MTD_TUNIT_IEEE_PMA_CTRL1,13, 1,&bit1));

        /* now read the duplex bit setting */
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,7,0x8000,4, 1,&duplexBit));
...
)
{
    ...
    /* reads bits 11, 10 (A6,A5 in the tech bit field of 7.16) */
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port, 7, 0x0010, \
                        10, 2, &temp));

   *pauseType = (MTD_U8)temp;
...
)
{
    ...
    *nbasetSpeedBits = MTD_ADV_NONE;
    ...
    MTD_ATTEMPT(mtdAutonegIsCompleted(devPtr,port,&anStatusReady));
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,7,0x0013,&val));
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,7,0x8001,&val));
    ...
    *ieeeSpeedBits = (lower_speeds | (gig_speeds <<= MTD_GIG_SPEED_POS));
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,7,0x0021,&val));
    ...
    /* Check if LP advertised 2.5G/5G via NBASE-T message page */
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,0xC035,&val));    
...
)
{
    ...
    MTD_ATTEMPT(mtdAutonegIsSpeedDuplexResolutionDone(devPtr,port,pauseResolved));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port, 7, 32, \
                            14, 2, 3));
...
)
{
    ...
    *anMsResolution = MTD_AN_RESOLUTION_UNKNOWN;
    ...
    MTD_ATTEMPT(mtdAutonegIsSpeedDuplexResolutionDone(devPtr,port, &anSpeedDuplexDone));
    ...
    if (anSpeedDuplexDone == MTD_TRUE)
    {
        ...
        {                    
            MTD_ATTEMPT(mtdReadMasterOrSlaveBit(devPtr,port, &msBit));
    ...
    else
    {
        ...
        MTD_ATTEMPT(mtdGetMasterSlaveConfigFault(devPtr,port,&cfgFault));
...
)
{
    ...
    *speedResolution = MTD_ADV_NONE;
    ...
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port, \
            7, 0, 12, 1, &val));
    
    if (val)
    {
        /* an is enabled, check if speed is resolved */
        MTD_ATTEMPT(mtdAutonegIsSpeedDuplexResolutionDone(devPtr,port, &resDone));
...
)
{
    ...
    *speed = MTD_ADV_NONE;
    ...
    MTD_ATTEMPT(mtdGetForcedSpeed(devPtr,port,&speedIsForced, &forcedSpeed));
    
    if (speedIsForced)
    {
        /* check if the link is up at the speed it's forced to */
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,3,0x8008,14,2,&cuSpeed));
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,3,0x8008,10,1,&cuLinkStatus));
    ...
    else
    {
        ...
        MTD_ATTEMPT(mtdGetAutonegSpeedDuplexResolution(devPtr,port,speed));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,3,\
                            0x8000,5,2,MDIXOptions));
   
    if(swReset)
    {
        MTD_ATTEMPT(mtdSoftwareReset(devPtr,port,1000)); /* this needs to be polled for 'done' before
...
{
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,3,0x8000,5,2,MDIXOptions));
...
)
{
    ...
    MTD_ATTEMPT(mtdAutonegIsSpeedDuplexResolutionDone(devPtr,port,&anDone));
    ...
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,3,0x8008,6,1,&bitVal));
...
)
{
    ...
    {
        MTD_DBG_ERROR("mtdAutonegSetNbaseTAdvertisement: NbaseT Advertisement is not supported on this device.\n");
        return MTD_FAIL;
    }

    if (enableNbaseTMP)
    {
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port, \
...
)
{
    ...
    if ((availableSpeeds & (MTD_SPEED_2P5GIG_FD | MTD_SPEED_5GIG_FD)) == 0)
    {
        MTD_DBG_ERROR("mtdAutonegSetNbaseTDownshiftControl: NbaseT Downshift is not supported on this device.\n");
        return MTD_FAIL;
    }
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,0xC034,&temp));
    MTD_ATTEMPT(mtdHwSetRegFieldToWord(temp,MTD_GET_BOOL_AS_BIT(downshiftEnable),4,1,&temp));
    MTD_ATTEMPT(mtdHwSetRegFieldToWord(temp,MTD_GET_BOOL_AS_BIT(noEnergyReset),5,1,&temp));
    MTD_ATTEMPT(mtdHwSetRegFieldToWord(temp,downshiftThreshold,0,4,&temp));
    MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,1,0xC034,temp));
...
)
{
    ...
    if ((availableSpeeds & (MTD_SPEED_2P5GIG_FD | MTD_SPEED_5GIG_FD)) == 0)
    {
        MTD_DBG_ERROR("mtdAutonegGetNbaseTDownshiftControl: NbaseT Downshift is not supported on this device.\n");
        return MTD_FAIL;
    }
    
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,0xC034,&temp));
    
    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,4,1,&temp2));
    ...
    *noEnergyReset= MTD_GET_BIT_AS_BOOL(temp2);        
    ...
    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,0,4,downshiftThreshold));    
...
)
{
    ...
    if ((availableSpeeds & (MTD_SPEED_2P5GIG_FD | MTD_SPEED_5GIG_FD)) == 0)
    {
        MTD_DBG_ERROR("mtdAutonegGetNbaseTDownshiftStatus: NbaseT Downshift is not supported on this device.\n");
        return MTD_FAIL;
    }

    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,1,0xC035,&temp));
    
    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,0,1,&temp2));
    *downshiftFrom1G = MTD_GET_BIT_AS_BOOL(temp2);    

    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,1,1,&temp2));
    *downshiftFrom2P5G = MTD_GET_BIT_AS_BOOL(temp2); 

    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,2,1,&temp2));
    *downshiftFrom5G = MTD_GET_BIT_AS_BOOL(temp2); 
    
    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,3,1,&temp2));
    *downshiftFrom10G = MTD_GET_BIT_AS_BOOL(temp2); 
     
    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(temp,9,4,downshiftCount));        
...
/*********************************************************************/
MTD_STATUS mtdSetFEXT
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
    IN MTD_BOOL enableFEXT,
    IN MTD_BOOL anRestart
)
{
    if (MTD_IS_X33X0_BASE(devPtr->deviceId) || MTD_IS_E21X0_BASE(devPtr->deviceId) || MTD_IS_X35X0_BASE(devPtr->deviceId) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        /* Write 1.C000.2 */
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,1,0xC000,2,1,MTD_GET_BOOL_AS_BIT(enableFEXT)));
    ...
    {
        MTD_DBG_ERROR("mtdSetFEXT: FEXT is not supported on this device.\n");
        return MTD_FAIL;
    }
...
/*********************************************************************/
MTD_STATUS mtdSetAEC
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
    IN MTD_U16 enableAEC,
    IN MTD_BOOL anRestart
)
{
    if (MTD_IS_X33X0_BASE(devPtr->deviceId) || MTD_IS_X35X0_BASE(devPtr->deviceId) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        /* Write 1.C000.5:4 */
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,1,0xC000,4,2,enableAEC));
    ...
    {
        MTD_DBG_ERROR("mtdSetAEC: AEC is not supported on this device.\n");
        return MTD_FAIL;
    }
...
/*********************************************************************/
MTD_STATUS mtdSetCMN
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
    IN MTD_BOOL enableCMN,
    IN MTD_BOOL anRestart
)
{
    if (MTD_IS_X33X0_BASE(devPtr->deviceId) || MTD_IS_E21X0_BASE(devPtr->deviceId) || MTD_IS_X35X0_BASE(devPtr->deviceId) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        /* Write 1.C000.7 - 1 is disable / 0 is enable */
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,1,0xC000,7,1,!MTD_GET_BOOL_AS_BIT(enableCMN)));
    ...
    {
        MTD_DBG_ERROR("mtdSetCMN: CMN is not supported on this device.\n");
        return MTD_FAIL;
    }
...
/******************************************************************************
 ...
 Description:
    ...
    called if autonegotiation is enabled and speed is not forced.
