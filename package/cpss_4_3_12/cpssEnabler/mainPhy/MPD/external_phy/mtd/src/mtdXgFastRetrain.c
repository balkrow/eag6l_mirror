...
)
{
    ...
    switch ((fr_mode & 0x0F))
    {
        ...
        case MTD_ENABLE_NFR_ONLY:
            MTD_ATTEMPT(mtdDoNotAdvert10GBTFR(devPtr,port,MTD_FALSE));
            MTD_ATTEMPT(mtdAdvertNFR(devPtr,port,frSignalType,MTD_NFR_TIMER_UNCHANGED,MTD_FALSE));
            break;
...
{    
    ...
    /* set extended NFR timer */
    if (extendedNFR != MTD_NFR_TIMER_UNCHANGED)
    {
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,7,0x800C,8,1,extendedNFR));
    }
