...
********************************************************************/
/*! \defgroup mtdDiagnostics mtdDiagnostics.h
@{*/
#ifndef MTDDIAG_H
#define MTDDIAG_H
...
Outputs:
   buildInfo1 - detailed build information part 1
   buildInfo2 - detailed build information part 2
   buildInfo3 - detailed build information part 3

Returns:
   MTD_OK if query was successful, MTD_FAIL if not.

Description:
   Reads additional detailed build information about the currently running 
   firmware if available. If no firmware is running, or if it is not supported
   on this PHY or this version of the firmware it will return all 0's.

Side effects:
   None.

Notes/Warnings:
   Only available starting with E21X0 and PHYs after this. This data can
   be passed to the support team for additional information on the currently
   running code to help in debug efforts. May not be available on earlier
   firmware versions.

   This is a supplement to the information returned by mtdGetFirmwareVersion().
...
#define MTD_BIST_COMPHY_LOAD_FAILED ((MTD_U16)(1 << 11))/* Fatal BIST, Serdes firmware load failed, T unit firmware will not load */
#define MTD_BIST_COMPHY_LOAD_SKIP   ((MTD_U16)(1 << 12))/* Info-only BIST, last reset was caused by a T unit processor reset, Serdes firmware load skipped*/
#define MTD_BIST_COMPHY_LOADING     ((MTD_U16)(1 << 13))/* Info-only BIST,  Serdes load in process, T unit firmware not loaded yet */
...
/******************************************************************************
 ...
 Inputs:
    ...
    speed - Speed selection for the test modes. The following are valid options    
        ...
        MTD_SPEED_5GIG_FD
        MTD_SPEED_2P5GIG_FD
        MTD_SPEED_1GIG_FD or MTD_SPEED_1GIG_HD
...
#define MTD_PACKETGEN_DISABLE     0x0000
 ...
 Inputs:
    ...
    generatorControl - Packet generator control with the following: 
                       ...
                       MTD_PACKETGEN_TO_BOTH - Inject packet to both LINE and 
                                               MAC(XGMII)
                       MTD_PACKETGEN_DISABLE - Inject to neither side
...
/******************************************************************************
 ...
 Notes/Warnings:
    ...
    For x36x0 the T Unit temperature sensor is enabled and controlled by the firmware,
    this function checks if the sensor has been enabled by the firmware to return MTD_OK otherwise MTD_FAIL
