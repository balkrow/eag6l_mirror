...
)
{
    ...
    MCESD_U32 i, j, adjustedVoltageCenter, findCount, segment0Min, segment0Max, segment1Min, segment1Max;
    segment1Min = 0;
...
)
{
    ...
    if (sampleCount <= 0)
    {
        MCESD_DBG_ERROR("Invalid sample count!\n");
        return MCESD_FAIL;
    }
    ...
    */
    for (i = arrayCenter; (i < (MCESD_32)arrayCount) && (i >= 0); i += direction)
    {
        /* buffer[e][i] */
    ...
    * buffer[e] = [error rate]          buffer[y] = [unused]
    */
    for (i = innerMin + 2 * direction; (i < (MCESD_32)arrayCount) && (i >= 0); i += direction)
    {
        /* Inner eye end = 0.01 */
        /* buffer[e][i] */
    ...
    if (innerCount < 3)
        return MCESD_OK;    /* Not enough error data */
    ...
    *c = 0;
    ...
    for (i = 0; i < degree; i++)                /* Diagonal index */
    {
        ...
        for (j = i + 1; j < degree + 1; j++)    /* Row dimension to zero below the diagonal */
        {
            if (reduction[i][i] == 0)
            {
                MCESD_DBG_ERROR("Divide by zero error!\n");
                return MCESD_FAIL;
            }
