...
Marvell 88X32X0, 88X33X0, 88X35X0, CUX36X0, CUE26X0, 88E20X0 
...
)
{
    ...
    else if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        ...
        {
            MCESD_U8 lane;
            E_N5C32GP5X4_POLARITY txPolarity;
            E_N5C32GP5X4_POLARITY rxPolarity;
            MCESD_DEV_PTR pSerdesDev;
            MTD_U16 numLanesPerSerdes = (MTD_IS_X3610_DEVICE(devPtr)?1:4);
            
            MTD_ATTEMPT(mtdSetSerdesPortGetDevPtr(devPtr,port,&pSerdesDev));
            
            lane = port % numLanesPerSerdes;
            txPolarity = (invert_output_pol==MTD_TRUE)?N5C32GP5X4_POLARITY_INVERTED:N5C32GP5X4_POLARITY_NORMAL;
            rxPolarity = (invert_input_pol==MTD_TRUE)?N5C32GP5X4_POLARITY_INVERTED:N5C32GP5X4_POLARITY_NORMAL;

            MTD_ATTEMPT((MTD_STATUS)API_N5C32GP5X4_SetTxRxPolarity(pSerdesDev,lane,txPolarity,rxPolarity));
        }
#else
        MTD_DBG_ERROR("mtdSetSerdesLanePolarity: Serdes 32G X1/X4 code not selected.\n");
        return MTD_FAIL;
#endif  /* MTD_CE_SERDES32X1X4 */       
    }
    else
...
)
{
    ...
    else if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
...
#if MTD_CE_SERDES32X1X4
        {
            ...
            MTD_ATTEMPT((MTD_STATUS)API_N5C32GP5X4_GetTxRxPolarity(pSerdesDev,lane,&txPolarity,&rxPolarity));

            *invert_input_pol = (rxPolarity==N5C32GP5X4_POLARITY_INVERTED)?MTD_TRUE:MTD_FALSE;
            *invert_output_pol = (txPolarity==N5C32GP5X4_POLARITY_INVERTED)?MTD_TRUE:MTD_FALSE;
        }
#else
        MTD_DBG_ERROR("mtdGetSerdesLanePolarity: COMPHY Serdes(28G X2) code not selected.\n");
        return MTD_FAIL;
...
MTD_STATUS mtdGetPktGenRegOffset
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
    IN MTD_U16 HorXunit,
    OUT MTD_U16 *regOffset
)
{
    *regOffset = 0;
...
{
    MTD_U16 regOffset;
    MTD_ATTEMPT(mtdGetPktGenRegOffset(devPtr, port, HorXunit, &regOffset));
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL1 + regOffset,15,1,MTD_GET_BOOL_AS_BIT(readToClear)));
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL1 + regOffset,2,1,MTD_GET_BOOL_AS_BIT(dontuseSFDinChecker)));

    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL2 + regOffset,4,4,pktPatternControl));
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL2 + regOffset,3,1,MTD_GET_BOOL_AS_BIT(generateCRCoff)));

    /* load up initial payload */
    MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_PAYLOAD1 + regOffset,(MTD_U16)initialPayload));
    MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_PAYLOAD2 + regOffset,(MTD_U16)(initialPayload>>16)));

    MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_LENGTH + regOffset,frameLengthControl));

    MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_BURSTSEQ + regOffset,numPktsToSend));

    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_IPG + regOffset,15,1,MTD_GET_BOOL_AS_BIT(randomIPG)));
    
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_IPG + regOffset,0,15,ipgDuration));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_CONTROL1 + regOffset,15,1,&temp));
    *readToClear = MTD_GET_BIT_AS_BOOL(temp);
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_CONTROL1 + regOffset,2,1,&temp));
    *dontuseSFDinChecker = MTD_GET_BIT_AS_BOOL(temp);

    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_CONTROL2 + regOffset,4,4,pktPatternControl));
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_CONTROL2 + regOffset,3,1,&temp));
    *generateCRCoff = MTD_GET_BIT_AS_BOOL(temp);

    /* lower byte */
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXUnit,MTD_PKT_GEN_PAYLOAD1 + regOffset,&temp));
    *initialPayload = (MTD_U16)temp;
    /* upper byte */
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXUnit,MTD_PKT_GEN_PAYLOAD2 + regOffset,&temp));
    *initialPayload |= (MTD_U32)temp<<16;

    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXUnit,MTD_PKT_GEN_LENGTH + regOffset,frameLengthControl));

    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXUnit,MTD_PKT_GEN_BURSTSEQ + regOffset,numPktsToSend));

    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_IPG + regOffset,15,1,&temp));
    *randomIPG = MTD_GET_BIT_AS_BOOL(temp);
    
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,HorXUnit,MTD_PKT_GEN_IPG + regOffset,0,15,ipgDuration));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, HorXunit, MTD_PKT_GEN_CONTROL1 + regOffset, &temp));
    ...
    {
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_BURSTSEQ + regOffset,&numPacket));
        /* need to write X.F017 = 0 to insure no fragments are sent when enabling/disabling the generator */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_BURSTSEQ + regOffset,0x0000));
    }

    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL1 + regOffset,0,2,temp));

    if (currGeneratorStatus != enableGenerator)
    {
        /* restore X.F017 to the configured value */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,HorXunit,MTD_PKT_GEN_BURSTSEQ + regOffset,numPacket));
    }
...
)
{
    ...
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL1 + regOffset,6,1,1));   
    ...
    /* speeds */
    MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,port,HorXunit,MTD_PKT_GEN_CONTROL1 + regOffset,6,1,0));    
...
)
{
    ...
    *packetCount = *byteCount = 0;
    ...
    {
        case MTD_PKT_GET_TX:
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXPKTCTR1 + regOffset,&temp));
            *packetCount = temp;
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXPKTCTR2 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<16);
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXPKTCTR3 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<32);

            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXBYTCTR1 + regOffset,&temp));
            *byteCount = temp;
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXBYTCTR2 + regOffset,&temp));
            *byteCount |= (((MTD_U64)(temp))<<16);
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_TXBYTCTR3 + regOffset,&temp));
            *byteCount |= (((MTD_U64)(temp))<<32);            
            break;

        case MTD_PKT_GET_RX:
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXPKTCTR1 + regOffset,&temp));
            *packetCount = temp;
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXPKTCTR2 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<16);
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXPKTCTR3 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<32);

            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXBYTCTR1 + regOffset,&temp));
            *byteCount = temp;
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXBYTCTR2 + regOffset,&temp));
            *byteCount |= (((MTD_U64)(temp))<<16);
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_RXBYTCTR3 + regOffset,&temp));
            *byteCount |= (((MTD_U64)(temp))<<32);            
            break;

        case MTD_PKT_GET_ERR:
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_ERRPKTCTR1 + regOffset,&temp));
            *packetCount = temp;
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_ERRPKTCTR2 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<16);
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_PKT_GEN_ERRPKTCTR3 + regOffset,&temp));
            *packetCount |= (((MTD_U64)(temp))<<32);            
            break;
...
)
{
    ...
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,HorXunit,MTD_LINK_DROP_COUNTER + regOffset,linkDropCounter));
...
)
{
    ...
    if (MTD_IS_X35X0_E2540_DEVICE(devPtr) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {        
...
)
{
    ...
    if (MTD_IS_X35X0_E2540_DEVICE(devPtr) || MTD_IS_X36X0_DEVICE(devPtr))
    {
        MTD_ATTEMPT(mtdGetMacInterfaceCopperOnlyPhy(devPtr, port, &macType, &macIfPowerDown, &macLinkDownSpeed, &macMaxIfSpeed));

        if ((macType == MTD_MAC_TYPE_10G_DXGMII) || (macType == MTD_MAC_TYPE_10G_QXGMII) || (macType == MTD_MAC_TYPE_20G_DXGMII) ||
            (macType == MTD_MAC_TYPE_20G_QXGMII) || (macType == MTD_MAC_TYPE_OXGMII) || (macType == MTD_MAC_TYPE_OXGMII_PARTNER))
        {
            MTD_DBG_ERROR("mtdSetShallowMacLoopbackLinkDown: not supported for E25X0/X35X0/X36X0 in MPUXSGMII mode!\n");
            return MTD_FAIL;
        }
        else
        {
    ...
    {
        /* max speed only applicable when mac type  is MTD_MAC_TYPE_SXGMII*/
