...
{
    MTD_U32 tempu32, baseType;
    MTD_U16 temp, tryCounter, temp2, reportedHwRev, tunitReportedPhyRev;
    ...
    MTD_BOOL registerExists, regReady, hasMacsec = MTD_FALSE, hasCopper = MTD_FALSE, is5GDevice = MTD_FALSE, is2P5GDevice = MTD_FALSE,
             isTDevice = MTD_FALSE, isOctalDevice = MTD_FALSE; 
    ...
    *thisPort = 0;        
    ...
    baseType = (MTD_U32)((temp & 0xFC00)>>6);
    reportedHwRev = (temp & 0x000F);
    numPortsRead = ((temp & 0x0380)>>7)+1;
    thisport = ((temp & 0x0070)>>4);

    /* check for extended base type, i.e. something after X35X0 devices starting with X36X0 */
    if (baseType == (MTD_EXTENDED_BASE))
    {
        mtdGetExtendedBase(devPtr, port, &baseType); /* get base from other place for these phys */
    }
    ...
    MTD_ATTEMPT(mtdCheckDeviceCapabilities(devPtr,port,baseType,ignoreFwReportedVersion,&hasMacsec,&hasCopper,&is5GDevice,&is2P5GDevice,&isTDevice,&isOctalDevice));  
    ...
    if ((hasMacsec == MTD_FALSE) && isTDevice)
    {
        /* This is an error. T devices must have macsec. */ 
        MTD_DBG_ERROR("mtdGetPhyRevision: unexpected error processing phy revision (T device no macsec). \n");
        return MTD_FAIL;    
    }
    ...
    if ((major == 0 && minor == 0 && inc == 0 && test == 0) || ignoreFwReportedVersion)
    {
        ...
        else if (MTD_IS_X33X0_BASE(baseType) || MTD_IS_E21X0_BASE(baseType) || MTD_IS_X35X0_BASE(baseType) || MTD_IS_X36X0_BASE(baseType))
        {        
            /* all 33X0, 35X0, 21X0, and 36X0 devices report correct revision */
            revision = reportedHwRev;            
        }
        else
        {
        ...
        *thisPort = (MTD_U8)thisport;
        ...
        if ((MTD_IS_X35X0_BASE(baseType) && reportedHwRev == MTD_X35X0BASE_OCTAL_PORTA0) ||
             (MTD_IS_X35X0_BASE(baseType) && reportedHwRev == MTD_X35X0BASE_QUADOCTAL_PORTB3 && isOctalDevice))
        {
    ...
    else
    {
        ...
        if (MTD_IS_X36X0_BASE(baseType) && (fwNumports == 0))
        {
            fwNumports = 8;
        }
        ...
        if (MTD_IS_X35X0_BASE(baseType) && (fwNumports == 0) && 
                                           ((revision == MTD_X35X0BASE_OCTAL_PORTA0) ||
                                            (revision == MTD_X35X0BASE_QUADOCTAL_PORTB3 && isOctalDevice)))
        {
        ...
        else if (fwNumports == numPortsRead && fwThisport == thisport)
        {
            ...
            /* At this point device reported num ports and this port number should match */
            *numPortsPerDevice = (MTD_U8)numPortsRead;
    ...
    if (hasMacsec)
    {
        tempu32 |= MTD_MACSEC_CAPABLE;        
    }

    if (isTDevice)
    {
        tempu32 |= MTD_X36X0_TDEVICE;
    }

    if (isOctalDevice)
    {
        tempu32 |= MTD_X35X0_OCTALDEV;
    }

    if (hasCopper)
    {
        tempu32 |= MTD_COPPER_CAPABLE;
    }

    if ((MTD_IS_X33X0_BASE(baseType) || MTD_IS_X35X0_BASE(baseType) || MTD_IS_X36X0_BASE(baseType)) && is5GDevice)
    {
        tempu32 |= MTD_E2XX0_DEVICE;
    }

    if (MTD_IS_E21X0_BASE(baseType) && is2P5GDevice)
    {
        tempu32 |= MTD_E21X1_DEVICE;
    }

    tempu32 |= (MTD_U32)(revision & 0xF);

    *phyRev = (MTD_DEVICE_ID)tempu32;
...
MTD_STATUS mtdGetFwDetailedBuildInfo
(
   IN MTD_DEV_PTR devPtr,
   IN MTD_U16 port,
   OUT MTD_U16 *buildInfo1,
   OUT MTD_U16 *buildInfo2,
   OUT MTD_U16 *buildInfo3
)
{
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_FWBUILD_INFO1, buildInfo1));
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_FWBUILD_INFO2, buildInfo2));
    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_FWBUILD_INFO3, buildInfo3));
...
)
{
    ...
    if ((speed == MTD_SPEED_1GIG_HD) || (speed == MTD_SPEED_1GIG_FD))
    {
...
)
{
    ...
    {
        if ((MTD_IS_X36X0_DEVICE(devPtr) || MTD_IS_X35X0_E2540_DEVICE(devPtr)) && (speed == MTD_SPEED_10M_FD || speed == MTD_SPEED_10M_HD))
        {
...
)
{
    ...
    else if (MTD_IS_X36X0_BASE(deviceId))
    {
        if (numPortsPerDevice == 8)
        {
            *sensorPort = basePort;
        }
        else
...
{
    MTD_U16 sensorPort;
    MTD_U16 e21x0SensorSetting, x36x0SensorSetting;
    ...
    else if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        ...
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, sensorPort, MTD_TUNIT_TESNE_CTL0, 0, 2, &x36x0SensorSetting));

        if (x36x0SensorSetting == 3) 
        {
            return MTD_OK;
        }
        else 
...
)
{
    ...
    {
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr, sensorPort, MTD_TUNIT_TESNE_DATA, &temp));
        
        /* convert 12 bit signed integer to 32 bit*/
        temp &= 0xFFF;
        temp32 = (temp & 0x800) ? (0xFFFFF000 | (MTD_U32)temp) : (MTD_U32)temp;
        stemp32 = (MTD_32)temp32;

        /* convert to Celsius, ref to TESNE doc but scale up 10000 to avoid float type */
        tesneCelsius = (stemp32 * 905 + 1122000) / 10000;
    }
    else
    {
    ...
    {
        *temperature = (MTD_S16)tesneCelsius;
    }
    else
    {
