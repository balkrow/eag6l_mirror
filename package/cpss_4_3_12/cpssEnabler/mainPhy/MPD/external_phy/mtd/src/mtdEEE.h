...
#define MTD_EEE_ALL_36X0       MTD_EEE_ALL_33X0_35X0 /* same as 35x0 */
...
/******************************************************************************
 ...
 Inputs: 
     ...
     XGMII_Exit_Timeout - Sets the delay in microseconds from the first frame 
       ...
       on X33x0/E20x0/E21x0/X35X0 PHYs. XGMII_Exit_Timeout is ignored on these devices 
       and the exit timer is set to 8, 15 and 30 microseconds for 10G, 5G and 2.5GBASE-T by firmware.
     ...
     anRestart - This takes the value of MTD_TRUE or MTD_FALSE and indicates 
                 if auto-negotiation should be restarted following the EEE
                 changes. If this is MTD_FALSE, the change will not
 ...
 Notes/Warnings:
    ...
    The calls:
    mtdEeeBufferConfig(devPtr,port,MTD_EEE_MODE_DISABLE,0,0,0,0,0,0,0,MTD_TRUE)
    will disable the EEE and set the timeouts/IPG to the suggested values.

    mtdEeeBufferConfig(devPtr,port,MTD_EEE_MODE_ENABLE_NO_LEGACY,0,0,0,0,0,0,0,MTD_TRUE)
    ...
    mtdEeeBufferConfig(devPtr,port,MTD_EEE_MODE_ENABLE_WITH_LEGACY,0,0,0,0,0,0,0,MTD_TRUE)
    will enable the EEE and set the timeouts/IPG to the suggested values. This
