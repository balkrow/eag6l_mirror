...
MTD_STATIC MTD_STATUS Cl45UsingCl22ReadWrite
(
...
{
    if (devPtr->fmtdWaitFunc != NULL)
    {
        if (devPtr->fmtdWaitFunc(devPtr, x) == MTD_FAIL)
        {
            MTD_DBG_INFO("fmtdWaitFunc failed\n");
            return MTD_FAIL;
        }
    }
    else
    {
        ...
        Sleep(x);
#else
        /* Not implemented. This is required to be implemented here or provided to the mtdLoadDriver */ 
        return MTD_FAIL;
#endif  /* DB_TEST_ENVIRONMENT */
    }
...
)
{
    ...
    waitCounter = 0;
    MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,dev,regAddr,fieldOffset,fieldLength,&valueRead));
