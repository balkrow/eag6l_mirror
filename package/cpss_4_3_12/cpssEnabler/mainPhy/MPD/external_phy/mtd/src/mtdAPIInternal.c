...
MTD_STATIC MTD_STATUS mtdWaitForRegFieldAltValues(IN MTD_DEV_PTR devPtr, IN MTD_U16 port, IN MTD_U16 dev, IN MTD_U16 regAddr,IN MTD_U8 fieldOffset, IN MTD_U8 fieldLength,IN MTD_U16 expectedValue0, IN MTD_U16 expectedValue1, IN MTD_U16 timeoutMs);
                              ...
                              MTD_U16 *errCode) 
{
    ...
    *errCode = 0;
    ...
    if (MTD_IS_X35X0_BASE(devPtr->deviceId) || MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        ...
        if (!(fHeader.port_skip & (1<<relativePort)))
        {
            ...
            if (!fHeader.data_only)
            {
                ...
                if ((fHeader.nextHeaderOffset != 0 && fHeader.nextHeaderOffset != 0xFFFFFFFF) && ((sectionCount+1) < maxSections))
                {
                    ...
                    /* This code assumes the first executable is the serdes loader which sets this status when done or skipped. */
                    if (mtdWaitForRegFieldAltValues(devPtr, port, MTD_TUNIT_XG_EXT_STATUS, 0, 12, MTD_SERDES_LOADER_DONE, MTD_SERDES_LOADER_SKIPPED, MTD_EXECUTABLE_TO) == MTD_FAIL)
                    {
...
/* note: for X3680 parts it always skips the odd ports regardless the portsSkip bitmask */
/* since the odd ports share the code RAM with the even ports, and since the even ports only */
/* only upload data from the flash in the boot code. */
                                          ...
                                          MTD_U16 *errCode)
{
    ...
    /* and on X3680 skip odd ports always quietly */
    localPorts[0] = 0;
    for (i=0, localNumPorts=0; i < numPorts; i++)
    {
        ...
        {
#if DB_TEST_FLASHCODE
            if (!(MTD_IS_X3680_DEVICE(devPtr) && (relativePort%2)))
            {
                /* Print this on all PHYS, but on X3680 print it only on the even ports that were skipped */
                MTD_DBG_INFO("Section %d skipped for port %d for downloading to ram\n", (int)sectionCount, (int)ports[i]);
            }
            else
            {
                /* Quietly skip all the odd ports on X3680 devices. It's normal behavior. */
            }
#endif
                                      ...
                                      MTD_U16 *errCode) 
{
    ...
    *errCode = 0;
    ...
    do
    {
        ...
        {
            /* Download header */
            MTD_U32 addressOffset; 

            if (MTD_IS_X35X0_E2540_DEVICE(devPtr))
            {
                addressOffset = 0x28000000 + MTD_HEADER_SIZE*sectionCount;
            }
            else if (MTD_IS_X3680_DEVICE(devPtr))
            {
                addressOffset = 0x0013FEE0 + MTD_HEADER_SIZE*sectionCount;
            }
...
MEM_SIZE_BYTES mtdGetDevMaxImageSize(MTD_DEV_PTR devPtr)
{
    ...
    if (devPtr->devEnabled)
    {
        ...
        {
            maxImageSize = MTD_MAX_X36X0_FLASH_IMAGE_SIZE;
        }
    }
...
)
{
    ...
    MTD_U16 abilities, revision, hwRevisionTemp;
    ...
    *isOctalDevice = MTD_FALSE;
    ...
    if (MTD_IS_X36X0_BASE(baseType))
    {
        ...
        if ((temp & 0x8) == 0)
        {
            /* It's X3610/E2610 (single port), need to check for T device */
            /* which fw does not report */
            ignoreFwReportedVals = MTD_TRUE;
        }
    }
    ...
    if ((major == 0 && minor == 0 && inc == 0 && test == 0) || ignoreFwReportedVals)
    {
        ...
        else if (MTD_IS_X35X0_BASE(baseType) || MTD_IS_X36X0_BASE(baseType))
        {
            MTD_U16 bitVal;
            MTD_U16 counter = 0; 
            ...
            *phyHasMacsec = MTD_FALSE;
            ...
            if (counter < 100)
            {
                ...
                if (MTD_IS_X35X0_BASE(baseType))
                {
                    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],0,1,&bitVal));
                    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, 3, 0xD00D, &hwRevisionTemp));
                    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(hwRevisionTemp, 0, 4, &revision));
                    ...
                    {
                        MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],1,1,&bitVal));
                        if(bitVal && (revision == MTD_X35X0BASE_QUADOCTAL_PORTB3))
                        {
                            *isOctalDevice = MTD_TRUE;
                        }
                        else
                        {
                            *isOctalDevice = MTD_FALSE;
                ...
                else if (MTD_IS_X36X0_BASE(baseType))
                {
                    MTD_U16 temp;
                    ...
                    if ((temp & 0x8) == 0)
                    {
                        ...
                        *isTDevice = MTD_TRUE;
                        ...
                        if (bitVal)
                        {
                            MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],4,1,&bitVal));
                            ...
                            MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],9,1,&bitVal));
                    ...
                    /* Common between octal and single port */
                    MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],0,1,&bitVal));
                    ...
                    if (bitVal)
                    {
                        ...
                        MTD_ATTEMPT(mtdHwGetRegFieldFromWord(values16[0],6,1,&bitVal));
    ...
    else
    {
        ...
        if (MTD_IS_X35X0_BASE(baseType))
        {
            MTD_U16 tunitReportedPhyRev, fwNumports;
            ...
            if ((fwNumports == 0) && (revision == MTD_X35X0BASE_QUADOCTAL_PORTB3)) /* Firmware uses 0->8 ports */
            {   
                *isOctalDevice = MTD_TRUE; 
            }        
        }
...
)
{
    switch(phyRev)
    {
        ...
        case MTD_REV_X3540_B3:    
        case MTD_REV_E2540_A0:
        case MTD_REV_E2540_B0:
        case MTD_REV_E2540_B3:
        case MTD_REV_X3580_A0:
        case MTD_REV_X3580_B3:
        case MTD_REV_E2580_A0:
        case MTD_REV_E2580_B3:
            return MTD_OK;
            break;

        case MTD_REV_X3610_A0:
        case MTD_REV_X3610_A1:
        case MTD_REV_X3610_A2:
        case MTD_REV_X3610_B0:
        case MTD_REV_X3610M_A0:
        case MTD_REV_X3610M_A1:
        case MTD_REV_X3610M_A2:    
        case MTD_REV_X3610M_B0:            
        case MTD_REV_X3610T_A0:
        case MTD_REV_X3610T_A1:
        case MTD_REV_X3610T_A2:    
        case MTD_REV_X3610T_B0:            
        case MTD_REV_X3680_A0:
        case MTD_REV_X3680_A1:
        case MTD_REV_X3680_B0:

        case MTD_REV_E2610_A0:
        case MTD_REV_E2610_A1:
        case MTD_REV_E2610_A2:
        case MTD_REV_E2610_B0:
        case MTD_REV_E2610M_A0:
        case MTD_REV_E2610M_A1:
        case MTD_REV_E2610M_A2:    
        case MTD_REV_E2610M_B0:            
        case MTD_REV_E2610T_A0:
        case MTD_REV_E2610T_A1:
        case MTD_REV_E2610T_A2:    
        case MTD_REV_E2610T_B0:            
        case MTD_REV_E2680_A0:
        case MTD_REV_E2680_A1:
        case MTD_REV_E2680_B0:        
            return MTD_OK;
            break;
...
{
    if (MTD_IS_X32X0_BASE(phyRev) ||  MTD_IS_X33X0_BASE(phyRev) || MTD_IS_E21X0_BASE(phyRev) || MTD_IS_X35X0_BASE(phyRev) || MTD_IS_X36X0_BASE(phyRev) )
    {
        return MTD_OK;
    }
    else
...
/* Gets the extended model number on X36X0 devices and later */
/* On X3680/E2680 parts before B0, must be read on port 0 of device */
MTD_STATUS mtdGetExtendedBase(MTD_DEV_PTR devPtr, MTD_U16  port, MTD_U32 *baseType)
{
    ...
    if (temp & 0x8)
    {
        /* It's multi-port, need to go to first port before B0 */
        port = (port/8) * 8;
    }

    MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, 3,0xDB01, &temp));
    temp >>= 8;
    temp &= 0xFF; /* Bits 15:8 */
    *baseType |= (*baseType) | (((MTD_U32)(temp))<<16); /* (extended base @ bit 16) | 0x3F */
...
MTD_STATIC MTD_STATUS mtdStartMultipleAppCode(MTD_DEV_PTR devPtr, MTD_U16 ports[], MTD_U16 numPorts, MTD_U16 portsSkip, MTD_BOOL waitDone, MTD_U16 sectionCount, MTD_U16 erroredPorts[], MTD_U16 *errCode)
{
    ...
    MTD_U16 i, basePort, relativePort, tmp;
	/*KW*/
    MPD_UNUSED_PARAM(sectionCount);
    if (numPorts > MTD_MAX_PORTS_TO_DOWNLOAD) {
    	return MTD_FAIL;
    ...
    if (waitDone)
    {
        ...
        while(!timedOut && i < localNumPorts)
        {
            ...
            if (erroredPorts[portsIndexes[i]] == 0)
            {
                if (mtdWaitForRegFieldAltValues(devPtr, localPorts[i], MTD_TUNIT_XG_EXT_STATUS, 0, 12, MTD_SERDES_LOADER_DONE, MTD_SERDES_LOADER_SKIPPED, MTD_EXECUTABLE_TO) == MTD_FAIL)
                {
                    timedOut = MTD_TRUE;
        ...
        if (MTD_IS_X3580_DEVICE(devPtr))
        {
            ...
            for (i = 0; i < localNumPorts; i++)
            {
                MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, localPorts[i], MTD_BOOT_STATUS_REG, 15, 1, &tmp));
                if (tmp)
                {
                    /* Now restart to download the next section */
                    TRY_IO(mtdHwSetPhyRegField(devPtr, localPorts[i], MTD_BOOT_STATUS_REG, MTD_CODE_DOWNLOADED_POS, 1, 0));
                }
            }
        }
...
/* On X3680 devices, always puts even/odd ports in download mode in pairs. */
MTD_STATIC MTD_STATUS mtdPutMultipleDownloadMode(MTD_DEV_PTR devPtr, MTD_U16 ports[], MTD_U16 numPorts, MTD_U16 erroredPorts[], MTD_U16 *errCode)
{
    MTD_U32 i;
    MTD_BOOL downloadMode;
	...
	/*End KW*/
/*	check if already in the MDIO download mode*/
    for (i = 0; i < numPorts; i++)
    {
    ...
    {
        /* X3680 device */
        /* If either P0/P1 in download, put them both in download mode */
        for(i = 0; i < numPorts; i++)
        {
            if (ports[i]%2)
            {
                /* It's an odd port, so skip it (images should never be downloaded only to odd ports anyways) */
                /* Should always be the even port and skip the odd, or skip both. */
            }
            else
            {   /* It's an even port, put both in download mode */
                TRY_IO(mtdHwSetPhyRegField(devPtr,ports[i]+1,31,0xF008,5,1,1)); /* Set strap on next port */
                TRY_IO(mtdPutPhyInMdioDownloadMode(devPtr,ports[i])); /* Reset on even port will reset both */
            }
        }
...
/******************************************
 wait for alternative values, either expectedValue0 or expectedValue1
******************************************/
MTD_STATIC MTD_STATUS mtdWaitForRegFieldAltValues(IN MTD_DEV_PTR devPtr, IN MTD_U16 port, IN MTD_U16 dev, IN MTD_U16 regAddr,IN MTD_U8 fieldOffset, IN MTD_U8 fieldLength,IN MTD_U16 expectedValue0, IN MTD_U16 expectedValue1, IN MTD_U16 timeoutMs)
{
    MTD_U16 timeOutCounter = 0;
    ...
    while (timeOutCounter++ < timeoutMs)
    {
        if ((mtdHwWaitForRegFieldValue(devPtr, port, dev, regAddr, fieldOffset, fieldLength, expectedValue0, 1) == MTD_OK) ||
            (mtdHwWaitForRegFieldValue(devPtr, port, dev, regAddr, fieldOffset, fieldLength, expectedValue1,1) == MTD_OK))
        {
            status = MTD_OK;
            break;
        }
