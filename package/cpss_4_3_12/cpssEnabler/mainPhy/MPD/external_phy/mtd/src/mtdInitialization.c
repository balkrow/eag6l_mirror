...
/*    MTD_DBG_INFO("mtdLoadDriver Called.\n"); */
    ...
    if((mtdHwXmdioRead(dev, anyPort,3,0 ,&data))!= MTD_OK) /* try to read 1.0 */
    {
        MTD_DBG_ERROR("Reading to reg 3.0000 failed.\n");
        mtdUnloadDriver(dev);
    ...
    {
        MTD_DBG_ERROR("Reading to reg 3.0000 has bad value.\n");
        mtdUnloadDriver(dev);
        *errCode = MTD_ERR_READ_REG_FAIL;
        return MTD_FAIL;
    }
    ...
    if (MTD_IS_X35X0_BASE(dev->deviceId)|| MTD_IS_X36X0_BASE(dev->deviceId))
    {
...
#if MTD_RS_MACSEC
    if (MTD_IS_X3610_DEVICE(dev))
    {
        /* Initialize RS MACSec device structure for X36X0 */
        if(mtdRSMACSecInit(dev) != MTD_OK)
        {
            MTD_DBG_ERROR("mtdLoadDriver: RS MACSec initialization failed.\n");
            return MTD_FAIL;
        }
    }
#endif  /* MTD_RS_MACSEC */
