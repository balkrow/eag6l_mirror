...
Precision Time Protocol(PTP) functions for the 88X35X0 and
...
)
{
    ...
    if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        ...
        {
            /* enable ptp clock */
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_CLK_ENABLE, &data));
            MTD_ATTEMPT(mtdHwGetRegFieldFromWord(data, 8, 1, &bitVal));
            if (bitVal == 0)
            {
                MTD_ATTEMPT(mtdHwXmdioWrite(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_CLK_ENABLE, (data|0x0100))); 
            }
    
            /* toggle mpm resetn to reset SPF */
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_BLK_RESET, &data));
            MTD_ATTEMPT(mtdHwXmdioWrite(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_BLK_RESET, (data & 0xFFBF))); 
            MTD_ATTEMPT(mtdHwXmdioWrite(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_BLK_RESET, (data | 0x0040))); 

            /* enable PTP */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_MPM_CONFIG, 0, 1, ptpEnable));

            /* set disable_ptp_bypass low/high speed */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, 0x8FF3, 11, 1, ptpEnable));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGM_PCS156_TST_CTRL, 12, 1, ptpEnable));

            /* disable PTP bypass */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 0, 1, (ptpEnable ? 0:1)));
            ...
            /* disable bypass MPM block */
            MTD_ATTEMPT(mtdHwXmdioRead(devPtr, port, MTD_TU_PTP_DEV, 0xD81D, &data));
            MTD_ATTEMPT(mtdHwGetRegFieldFromWord(data, 6, 1, &bitVal));
            if (bitVal)
            {
                MTD_ATTEMPT(mtdHwXmdioWrite(devPtr, port, MTD_TU_PTP_DEV, 0xD81D, (data & 0xFFBF)));
            }
    
            /* PTP is enabled by default at 0 */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_PORT_REG_0, 0, 1, 0x0));
        }
        else
        {
            /* disable ptp clock and toggle mpm resetn. */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGTOP_XG_CLK_ENABLE, 8, 1, 0));

            /* disable PTP */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_MPM_CONFIG, 0, 1, ptpEnable));

            /* do not disable_ptp_bypass low/high speed */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, 0x8FF3, 11, 1, ptpEnable));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGM_PCS156_TST_CTRL, 12, 1, ptpEnable));
            ...
            /* bypass MPM block */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, 0xD81D, 6, 1, 1));

            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 12, 1, 0));
        }
    }
    else
    {
        if (ptpEnable == MTD_ENABLE)
        {
            /* disable PTP bypass */
...
)
{
    ...
    if (MTD_IS_X3610_DEVICE(devPtr))
    {
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 1, 1, 0x1));
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 1, 1, 0x0));
    }
    else
    {
...
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
    IN MTD_U16 ptpBypass /* 1:PTP bypass */
)
{
    ...
    {
        /* set disable_ptp_bypass low/high speed */
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, 0x8FF3, 11, 1, (ptpBypass) ? 1 : 0));
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_XGM_PCS156_TST_CTRL, 12, 1, (ptpBypass) ? 1 : 0));

        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 0, 1, (ptpBypass) ? 1 : 0));
    }
    else
    {
...
)
{
    ...
    if (MTD_IS_X3610_DEVICE(devPtr))
    {
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 0, 1, 
                                        &data));
    }
    else
    {
...
)
{
    ...
    if (MTD_IS_X3610_DEVICE(devPtr))
    {
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG0, 0, 1, todCtrlOption));
    }
    else
    {
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_CNTL_REG0, 0, 1, todCtrlOption));
    }
...
)
{
    ...
    if (MTD_IS_X3610_DEVICE(devPtr))
    {
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG0, 0, 1, 
                                        &data));
    }
    else
    {
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_CNTL_REG0, 0, 1, &data));
    }
...
)
{
    ...
    if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        if (oneStep == MTD_PTP_ONE_STEP)
        {
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 12, 1, 1));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_PORT_REG_0, 6, 2, 0x3));
        }
        else if (oneStep == MTD_PTP_TWO_STEP)
        {
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_36X0_CNTL_REG1, 12, 1, 0));
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_PORT_REG_0, 6, 2, 0x0));
        }
        else
        {
            MTD_DBG_ERROR("mtdTuPTPSetOneStep: incorrect oneStep input %d.\n", oneStep);
            return MTD_FAIL;
        }
    }
    else
...
)
{
    ...
    if (MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr, port, MTD_TU_PTP_DEV, MTD_TU_PTP_PORT_REG_0, 6, 2, &data));

        if (data == 0)
        {
            *oneStep = MTD_PTP_TWO_STEP;
        }
        else
        {
            *oneStep = MTD_PTP_ONE_STEP;
        }
    }
    else
    {
...
/*******************************************************************************
 mtdTuPTPSetOneStepDomain - Set OneStep Domain
*******************************************************************************/
MTD_STATUS mtdTuPTPSetOneStepDomain
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
{
    MTD_U16 data, writeData;
    MTD_U16 featureIndex = 2;

    if (!MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        MTD_DBG_ERROR("mtdTuPTPSetOneStepDomain: Only for X36X0 devices only\n");
        return MTD_FAIL;
    }
    ...
    writeData = ((data & 0xFFF9) | (oneStepDomainMode << 1));
...
/*******************************************************************************
 mtdTuPTPGetOneStepDomain - Get OneStep Domain
*******************************************************************************/
MTD_STATUS mtdTuPTPGetOneStepDomain
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
)
{
    ...
    if (!MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        MTD_DBG_ERROR("mtdTuPTPGetOneStepDomain: Only for X36X0 devices only\n");
        return MTD_FAIL;
    }
    ...
    mtdHwGetRegFieldFromWord(data, 1, 2, &bitVal);

    *oneStepDomainMode = bitVal;

    return MTD_OK;
}

/*******************************************************************************
 mtdTuPTPSetInternalTime - Set PTP internal time
*******************************************************************************/
MTD_STATUS mtdTuPTPSetInternalTime
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
{
    MTD_U16 data, writeData, bitVal;
    MTD_U16 featureIndex = 1;

    if (!MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        MTD_DBG_ERROR("mtdTuPTPSetInternalTime: Only for X36X0 devices only\n");
        return MTD_FAIL;
    }

    bitVal = PTPInternalTimeOption ? 1 : 0;

    MTD_ATTEMPT(mtdTuPTPIsDevValid(devPtr));

    MTD_ATTEMPT(mtdTuPTPGlobalConfRead(devPtr, port, featureIndex, &data));

    writeData = ((data & 0xFFBF) | (bitVal << 6));
...
/*******************************************************************************
 mtdTuPTPGetInternalTime - Get PTP internal time
*******************************************************************************/
MTD_STATUS mtdTuPTPGetInternalTime
(
    IN MTD_DEV_PTR devPtr,
    IN MTD_U16 port,
...
)
{
    ...
    mtdHwGetRegFieldFromWord(data, 6, 1, &bitVal);

    *PTPInternalTimeOption = bitVal;
...
)
{
    ...
    ptpStatus->timeStamped = (MTD_U32)((valueHi << 16) | valueLo);
...
)
{
    ...
    MTD_ATTEMPT(mtdTuPTPWaitGlobalBitSC(devPtr, port, MTD_TUPTP_WAITTYPE_TOD)); /* Wait for ToDBusy cleared */
...
)
{
    ...
    timeArray->clkActive = ((data64Bits >> 8) & 1) == 1 ? MTD_TRUE : MTD_FALSE;
    timeArray->domainNumber = (MTD_U8)(data64Bits & 0xFF);

    data = (MTD_U16)data64Bits & 0x89FF;    /* clear ToDOp bits and timeArrayIndex bits */
...
)
{
    ...
    MTD_ATTEMPT(mtdTuPTPGlobalConfCompRead(devPtr, port, MTD_TU_PTP_TRIG_CLK_CNTL, &data));

    MTD_ATTEMPT(mtdHwSetRegFieldToWord(data, ((enableDriftComp) ? 1 : 0), 7, 1, &data));

    MTD_ATTEMPT(mtdTuPTPGlobalConfCompWrite(devPtr, port, MTD_TU_PTP_TRIG_CLK_CNTL, data));
...
)
{
    ...
    MTD_ATTEMPT(mtdTuPTPGlobalConfCompRead(devPtr, port, MTD_TU_PTP_TAI_MIN_DRIFT_TH, &data));

    MTD_ATTEMPT(mtdHwSetRegFieldToWord(data, minDriftThreshold, 0, 4, &data));

    MTD_ATTEMPT(mtdTuPTPGlobalConfCompWrite(devPtr, port, MTD_TU_PTP_TAI_MIN_DRIFT_TH, data));
...
)
{
    ...
    MTD_ATTEMPT(mtdTuPTPGlobalConfCompRead(devPtr, port, MTD_TU_PTP_TAI_MAX_DRIFT_TH1, &data));
    MTD_ATTEMPT(mtdHwSetRegFieldToWord(data, dataL, 0, 8, &data));
    MTD_ATTEMPT(mtdTuPTPGlobalConfCompWrite(devPtr, port, MTD_TU_PTP_TAI_MAX_DRIFT_TH1, data));

    MTD_ATTEMPT(mtdTuPTPGlobalConfCompRead(devPtr, port, MTD_TU_PTP_TAI_MAX_DRIFT_TH2, &data));
    MTD_ATTEMPT(mtdHwSetRegFieldToWord(data, dataH, 0, 2, &data));
    MTD_ATTEMPT(mtdTuPTPGlobalConfCompWrite(devPtr, port, MTD_TU_PTP_TAI_MAX_DRIFT_TH2, data));
