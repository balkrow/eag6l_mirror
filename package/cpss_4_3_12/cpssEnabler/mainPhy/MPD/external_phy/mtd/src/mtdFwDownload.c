...
)
{
    ...
    *errCode = 0;
    ...
    {
        MTD_DBG_ERROR("X3680/E2680 devices must call mtdParallelUpdateRamImage() to download firmware on port quads.\n");
        *errCode = MTD_NON_GROUPED_RAM_DOWNLOAD;
        return MTD_FAIL;
    }
...
)
{
    ...
    *errCode = 0;
    ...
    if (MTD_IS_X3680_DEVICE(devPtr))
    {
        ...
        if (numPorts % 8)
        {
            MTD_DBG_ERROR("X3580/E2680 device must download firmware on all ports.\n). numPorts must be groups of 8!\n");
            *errCode = MTD_GROUPED_PORTS_NOT_FOUND;
            return MTD_FAIL;        
        }
        ...
        /* Make sure if any device has a port download, all the ports on that device must be downloaded */
        for (i = 0; i < MTD_MAX_PORTS_TO_DOWNLOAD; )
        {
            if (visitedPorts[i])
            {
                if ((i % 8) != 0)
                {
                    /* Found a port specified in the range 1...7, without the first port specified */
                    MTD_DBG_ERROR("X3680/E2680 device must download firmware on all 8 ports. First port is missing in ports[]\n");
                    *errCode = MTD_GROUPED_PORTS_NOT_FOUND;
                    return MTD_FAIL;                
                }
                else
                {
                    /* Found a first port of the device, make sure all 7 others are in the list */
                    for (firstPort = i; firstPort < (i+8); firstPort++)
                    {
                        if (visitedPorts[firstPort] == MTD_FALSE)
                        {
                            MTD_DBG_ERROR("X3680/E2680 device must download firmware on all 8 ports. Some port is missing in ports[]\n");
                            *errCode = MTD_GROUPED_PORTS_NOT_FOUND;
                            return MTD_FAIL;                        
                        }
                    }
                    i += 8; /* All 8 ports were specified, start searching on next group of 8 boundary */
                }
            }
            else
            {
...
{
    const MTD_U32 deviceBase = MTD_DEVID_GETBASE(devPtr->deviceId);
    const float flashSpeeds[4] = {1220700.0, 2441400.0, 4882812.0, 9765625.0}; /* Flash selection interface speeds in Mhz */    
                                                                           /* For X36X0 parts speed was increased x 4 */
    MTD_U16 flashSpeedSelection;
    float timeInSecs, timeInMs, spiDivisor;
    ...
    if (deviceBase == MTD_X36X0_BASE)
    {
        spiDivisor = 4.0f; /* For X36X0 SPI speed was increased x 4 */
    }
    else
    {
        spiDivisor = 1.0f;
    ...
    switch (deviceBase)
    {
        ...
        case MTD_X35X0_BASE:
        case MTD_X36X0_BASE:
            timeInSecs = (1.0f/flashSpeeds[flashSpeedSelection] * 8.0f * ((float)maxImageSize))/spiDivisor;
