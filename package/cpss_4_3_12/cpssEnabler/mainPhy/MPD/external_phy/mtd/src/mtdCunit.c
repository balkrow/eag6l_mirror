...
the Control Unit (C Unit) of the Marvell 88X32X0, 88X33X0, 88X35X0, 
...
{
    /* x3680 shares code ram every two ports, must apply reset to the primary port only */
    if (MTD_IS_X3680_DEVICE(devPtr) && (port % 2 != 0))
    {
        MTD_DBG_ERROR("mtdTunitHwReset: Must pass in the Master(even) port\n");
        return MTD_FAIL;
    }
...
)
{
    ...
    {
        MTD_DBG_ERROR("mtdSetPortPowerDown is not supported on X35X0 or X36X0 devices.\n");
        return MTD_FAIL;
    }
...
)
{
    ...
    if (MTD_IS_X35X0_BASE(devPtr->deviceId)|| MTD_IS_X36X0_BASE(devPtr->deviceId))
    {
        MTD_DBG_ERROR("mtdGetPortPowerDown is not supported on X35X0 or X36X0 devices.\n");
        return MTD_FAIL;
    }
...
)
{
    ...
    {
        MTD_DBG_ERROR("This function does not support MTD_E21X0_BASE, MTD_X35X0_BASE or MTD_X36X0_BASE devices\n");
        return MTD_FAIL;
    }
