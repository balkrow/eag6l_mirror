...
the Host Unit (H Unit) of the Marvell 88X32X0, 88X33X0, 88X35X0, 
CUX36X0, CUE26X0,88E20X0 and 88E21X0 ethernet PHYs.
******************************************************************* */
...
)
{
    ...
    {
        MTD_U16 mtd_cunit_port_ctrl2_rd_val; /* temp variable to hold the changes to 31.F007 */

        /* readback is incorrect until after sw reset */       
        /* need to accumulate values until sw reset is issued */
        /* NOTE: after this do not read 31.F007, do RMW on this temp */
        /* then do write directly to 31.F007, and after that can read it back */

        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,MTD_CUNIT_PORT_CTRL2,&mtd_cunit_port_ctrl2_rd_val)); 
        ...
        if (prevMacType == MTD_MAC_TYPE_10GR_1000BX_AP_AN && macType != MTD_MAC_TYPE_10GR_1000BX_AP_AN)
        {
            ...
            /* Disable KR Training */
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,0x0,11,1,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */                                                                                                                   
        }
        ...
        if (macType == MTD_MAC_TYPE_10GR_1000BX_AP_AN)
        {
            ...
            /* Enable KR Training */ 
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,0x1,11,1,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
        ...
        {
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,0x0,7,1,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
        }

        /* Turn AMD On */
        if (macType == MTD_MAC_TYPE_10GR_RM_AMD_ON)
        {
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,0x1,7,1,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
        }
        ...
        do
        {
            ...
            /* Write accumulate value, then write entire accumulated value to the register without reading it */
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,macMaxIfSpeed,2,2,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
            MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,portIndex,MTD_CUNIT_PORT_CTRL2,mtd_cunit_port_ctrl2_rd_val));
            ...
            /* Now 31.F007 readback should have all updated values and can be read back */

            /* Let the T unit firmware pick up any changes to mactypes */
            /* The firmware will do a C unit software reset if this bit is set when it configures the MP-USXGMII modes */
            /* host will see all links go down when this happens in MP-USXGMII mode changes to/from these modes */
            MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr,portIndex,7,0x800F,14,1,1)); /* Bit self clears by the firmware */
            MTD_ATTEMPT(mtdSoftwareReset(devPtr, portIndex, 1000)); 
        }
        while (portIndex > port);
                
    }
    else if(MTD_IS_X3610_DEVICE(devPtr))
    {
        MTD_U16 mtd_cunit_port_ctrl2_rd_val; /* temp variable to hold the changes to 31.F007 */
        ...
        if (macType == MTD_MAC_LEAVE_UNCHANGED)
        {
            macType = prevMacType;
                 ...
                 !(macType == MTD_MAC_TYPE_SXGMII || macType == MTD_MAC_TYPE_10GR_SGMII_AN_EN ||
                   macType == MTD_MAC_TYPE_10GR_1000BX_AP_AN || macType == MTD_MAC_TYPE_10GR_RM_AMD_OFF)))
        {
            /* Range check for X3610 device failed */
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: Mac Type out of range.\n");
            return MTD_FAIL;
        }
        ...
        if (macType != MTD_MAC_TYPE_SXGMII &&
            macMaxIfSpeed != MTD_MAX_MAC_SPEED_NOT_APPLICABLE)
        {
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: macMaxIfSpeed cannot be set on this mac type.\n");
            return MTD_FAIL;
        }    

        if (macType == MTD_MAC_TYPE_SXGMII &&
            !(macMaxIfSpeed == MTD_MAX_MAC_SPEED_10G ||
              macMaxIfSpeed == MTD_MAX_MAC_SPEED_SXGMII_5G ||
              macMaxIfSpeed == MTD_MAX_MAC_SPEED_2P5G))
        {
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: macMaxIfSpeed invalid option for macType SXGMII.\n");
            return MTD_FAIL;
        }

        if (macMaxIfSpeed == MTD_MAX_MAC_SPEED_NOT_APPLICABLE)
        {
            macMaxIfSpeed = MTD_MAX_MAC_SPEED_10G; /* All other mactypes need to set speed to 10G */
        }
        ...
        /* Write accumulate value, then write entire accumulated value to the register without reading it */
        MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val, macMaxIfSpeed, 2, 2, &mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr, port, MTD_CUNIT_PORT_CTRL2, mtd_cunit_port_ctrl2_rd_val));
        ...
        /* The firmware will do a C unit software reset if this bit is set when it configures the MP-USXGMII modes */
        /* host will see all links go down when this happens in MP-USXGMII mode changes to/from these modes */
        MTD_ATTEMPT(mtdHwSetPhyRegField(devPtr, port, 7, 0x800F, 14, 1, 1)); /* Bit self clears by the firmware */
        MTD_ATTEMPT(mtdSoftwareReset(devPtr, port, 1000)); 
    }
    else if (MTD_IS_X3680_DEVICE(devPtr))
    {
        MTD_U16 mtd_cunit_port_ctrl2_rd_val;
        ...
        if (macType == MTD_MAC_LEAVE_UNCHANGED)
        {
            macType = prevMacType;
               ...
               !(macType == MTD_MAC_TYPE_20G_DXGMII || macType == MTD_MAC_TYPE_SXGMII_AMD_ON ||
                 macType == MTD_MAC_TYPE_20G_QXGMII || macType == MTD_MAC_TYPE_10GR_RM_AMD_ON))
        {
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: Mac Type out of range.\n");
            return MTD_FAIL;
        }
        ...
        /* Only MTD_MAC_TYPE_SXGMII_AMD_OFF allow to set speed*/
        if ((macType != MTD_MAC_TYPE_SXGMII_AMD_OFF) && (macMaxIfSpeed != MTD_MAX_MAC_SPEED_NOT_APPLICABLE))
        {
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: macMaxIfSpeed cannot be set on this mac type.\n");
            return MTD_FAIL;
        }

        /* For MTD_MAC_TYPE_SXGMII_AMD_OFF only 10G, 5G and 2.5G allowed to be set*/
        if ((macType == MTD_MAC_TYPE_SXGMII_AMD_OFF) &&
           !(macMaxIfSpeed == MTD_MAX_MAC_SPEED_10G ||
             macMaxIfSpeed == MTD_MAX_MAC_SPEED_SXGMII_5G ||
             macMaxIfSpeed == MTD_MAX_MAC_SPEED_2P5G))
        {
            MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: macMaxIfSpeed invalid option for SXGMII AMD Off.\n");
            return MTD_FAIL;
        }

        if (macType == MTD_MAC_TYPE_SXGMII || macType == MTD_MAC_TYPE_OXGMII ||
            macType == MTD_MAC_TYPE_10G_DXGMII || macType == MTD_MAC_TYPE_20G_DXGMII ||
            macType == MTD_MAC_TYPE_10G_QXGMII || macType == MTD_MAC_TYPE_20G_QXGMII)
        {
            if (macIfPowerDown == MTD_FALSE)
            ...
            if ((macType == MTD_MAC_TYPE_OXGMII) && (port != 0))
            {
                MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: Must pass in the port 0 for OXGMII\n");
                return MTD_FAIL;
            }
            ...
            if ((macType == MTD_MAC_TYPE_10G_QXGMII || macType == MTD_MAC_TYPE_20G_QXGMII) && (port%4 != 0))
            {
                MTD_DBG_ERROR("mtdSetMacInterfaceCopperOnlyPhy: Must pass in the first port of the port group (P0 for P0-3) for QXGMII/OXGMII\n");
                return MTD_FAIL;
            }
        ...
        /* Turn AMD On */
        if (macType == MTD_MAC_TYPE_10GR_RM_AMD_ON || macType == MTD_MAC_TYPE_SXGMII_AMD_ON)
        {
            MTD_ATTEMPT(mtdHwSetRegFieldToWord(mtd_cunit_port_ctrl2_rd_val,0x1,7,1,&mtd_cunit_port_ctrl2_rd_val)); /* MTD_CUNIT_PORT_CTRL2 accumulate */
        }
        else
        ...
        else if (macType == MTD_MAC_TYPE_10G_QXGMII || macType == MTD_MAC_TYPE_20G_QXGMII)
        {
            groupedPortNum = 4; /* set 4 of grouped port */
        }
        else if (macType == MTD_MAC_TYPE_OXGMII)
        {
            groupedPortNum = 8; /* set 8 of grouped port */
        }
        else    /* All other modes */
        {
            groupedPortNum = 1;  /* Set each port individually */
        ...
        do
        {
            ...
            /* Master and Slave ports all need to be set to 10G/20G. For MTD_MAC_TYPE_SXGMII_AMD_OFF use the allowed
               speed 10G, 5G, 2.5G passed from the API */
            if (macType == MTD_MAC_TYPE_20G_DXGMII || macType == MTD_MAC_TYPE_20G_QXGMII || macType == MTD_MAC_TYPE_OXGMII)
            {
                macMaxIfSpeed = MTD_MAX_MAC_SPEED_20G;
            }
            else if (macType != MTD_MAC_TYPE_SXGMII_AMD_OFF)
            {
                macMaxIfSpeed = MTD_MAX_MAC_SPEED_10G;
            }
            ...
            if (macMaxIfSpeed == MTD_MAX_MAC_SPEED_NOT_APPLICABLE)
            {
                ...
                macMaxIfSpeed = MTD_MAX_MAC_SPEED_10G; /* Make sure never write the invalid value to the register */
            }
...
)
{
    ...
    else if (MTD_IS_X35X0_E2540_DEVICE(devPtr))
    {
        ...
        *macIfPowerDown = MTD_GET_BIT_AS_BOOL(temp);
        ...
        /* 31.F000.7:6 no longer indicate link downs speed, ignore it, return MTD_MAC_SPEED_NOT_APPLICABLE*/
        *macLinkDownSpeed = MTD_MAC_SPEED_NOT_APPLICABLE;
    }
    else if (MTD_IS_X3610_DEVICE(devPtr))
    {
        /* Get Host Speed, only valid when in USXGMII mode for X3610/E2610 */
        MTD_ATTEMPT(mtdHwGetPhyRegField(devPtr,port,MTD_CUNIT_PORT_CTRL2,2,2,macMaxIfSpeed));
        ...
        *macIfPowerDown = MTD_GET_BIT_AS_BOOL(temp);
        ...
        /* for x36x0, 31.31.F000.7:6 no longer indicate link downs speed, ignore it, return MTD_MAC_SPEED_NOT_APPLICABLE*/
        *macLinkDownSpeed = MTD_MAC_SPEED_NOT_APPLICABLE;
    ...
    else if (MTD_IS_X3680_DEVICE(devPtr))
    {
        ...
        /* Only mac type 0 (SXGMII) with AMD off allows to set speed 10G, 5G or 2.5G. Other mac types either not
           allow to set speed or the API will set the speed according to the mac type automatically, therefore should
           pass MTD_MAX_MAC_SPEED_NOT_APPLICABLE to the API
         */
        if (!((*macType == MTD_MAC_TYPE_SXGMII_AMD_OFF) && (temp == 0)))
        {
            *macMaxIfSpeed = MTD_MAX_MAC_SPEED_NOT_APPLICABLE;
        }
