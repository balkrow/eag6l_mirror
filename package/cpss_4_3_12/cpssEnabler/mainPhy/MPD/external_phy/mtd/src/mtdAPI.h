...
status of the Marvell 88X32X0, 88X33X0, 88X35X0, CUX36X0, CUE26X0, 88E20X0 and 
88E21X0 ethernet PHYs.
********************************************************************/
/*! \defgroup mtdAPI mtdAPI.h
@{*/
#ifndef MTD_API_H
#define MTD_API_H
...
#define MTD_API_MAJOR_VERSION 4
#define MTD_API_MINOR_VERSION 1
#define MTD_API_BUILD_ID      3
...
(
 ...
 Notes/Warnings:
    ...
    On X3680 devices, port pairs share the same code RAM. On these devices
    it's not expected to download to one port and not the other. This
    function, however, controls them individually. To put both ports into
    download mode, set the strap override on the next (odd) port and
    call this function to finish putting both ports into the download mode.
******************************************************************************/
MTD_STATUS mtdPutPhyInMdioDownloadMode
(
    IN MTD_DEV_PTR devPtr,
...
(
 ...
 Notes/Warnings:
    ...
    On X3680 devices, port pairs share the same code RAM. On these devices
    it's not expected to download to one port and not the other. This
    function, however, controls them individually. To remove both ports from
    download mode, set the strap override on the next (odd) port and
    call this function to finish removing both ports from the download mode.    
******************************************************************************/
MTD_STATUS mtdRemovePhyMdioDownloadMode
(
    IN MTD_DEV_PTR devPtr,
...
(
 ...
 Notes/Warnings:
    ...
    May also want to consider calling mtdGetFwDetailedBuildInfo() for
    more detailed information about the firmware to pass to the support team
    for PHYs E21X0 and after. See that function for more information.
...
/* All speeds available on X36X0 devices */
#define MTD_SPEED_ALL_36X0 (MTD_SPEED_ALL_33X0_35X0)
...
/******************************************************************************
 ...
 Inputs:
    devPtr - pointer to MTD_DEV initialized by mtdLoadDriver() call
    port - MDIO port address, 0-31
    enableFEXT - MTD_ENABLE to always enable FEXT or MTD_DISABLE to only
                 enable when required
    anRestart - this takes the value of MTD_TRUE or MTD_FALSE and indicates 
                if auto-negotiation should be restarted following the
                FEXT setting change. If this is MTD_FALSE, the change will not
                take effect until AN is restarted in some other way (link
                drop, toggle low power, toggle AN enable, toggle soft reset).
 ...
 Description:
    This function will either always enable the FEXT canceller or only enable it
    when required.
 ...
 Notes/Warnings:
    FEXT is only available for 33X0 and later devices.
******************************************************************************/
MTD_STATUS mtdSetFEXT
(
    IN MTD_DEV_PTR devPtr,
...
/******************************************************************************
 ...
 Inputs:
    devPtr - pointer to MTD_DEV initialized by mtdLoadDriver() call
    port - MDIO port address, 0-31
    enableAEC - MTD_ENABLE to force enable AEC or MTD_DISABLE to force disable 
                it. Set to MTD_ENABLE_AS_NEEDED to only enable AEC when required.
    anRestart - this takes the value of MTD_TRUE or MTD_FALSE and indicates 
                if auto-negotiation should be restarted following the 
                AEC setting change. If this is MTD_FALSE, the change will not
                take effect until AN is restarted in some other way (link
                drop, toggle low power, toggle AN enable, toggle soft reset).
 ...
 Description:
    This function will enable, only enable when required, or force disable the 
    Analog Echo Canceller (AEC).

 Side effects:
    None.

 Notes/Warnings:
    AEC is only available for 33X0 and later devices that support 10G.
******************************************************************************/
MTD_STATUS mtdSetAEC
(
    IN MTD_DEV_PTR devPtr,
...
/******************************************************************************
 ...
 Inputs:
    devPtr - pointer to MTD_DEV initialized by mtdLoadDriver() call
    port - MDIO port address, 0-31
    enableCMN - MTD_ENABLE to enable CMN canceller or MTD_DISABLE to disable it
    anRestart - this takes the value of MTD_TRUE or MTD_FALSE and indicates 
                if auto-negotiation should be restarted following the
                CMN setting change. If this is MTD_FALSE, the change will not
                take effect until AN is restarted in some other way (link
                drop, toggle low power, toggle AN enable, toggle soft reset).
 ...
 Description:
    This function will either enable or disable Common Mode Noise (CMN) 
    Cancellation.
 ...
 Notes/Warnings:
    CMN is only available for 33X0 and later devices.
******************************************************************************/
MTD_STATUS mtdSetCMN
(
    IN MTD_DEV_PTR devPtr,
