...
#define MYD_ENABLE_SERDES_API 0
#else
#define MYD_ENABLE_SERDES_API 1
#endif                             /* Set to 1 to include the SERDES API.
                                   All SERDES source files in the serdes folder
                                   are needed to be in the makefile. Otherwise,
                                   it will generate build errors.
                                   Default = 1  */

#define MYD_FW_WRITE_VERIFIED 0 /* Faster firmware download without write verify.
...
#define MYD_UNUSED_PARAM(x) x = x

#if defined(__GNUC__) && __GNUC__ >= 7
#define MYD_ATTR_FALLTHROUGH  __attribute__ ((fallthrough))
#else
#define MYD_ATTR_FALLTHROUGH  ((MYD_VOID)0)
#endif
