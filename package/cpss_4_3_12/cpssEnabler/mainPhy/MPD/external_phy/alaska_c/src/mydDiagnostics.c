...
functions of the Marvell X7120/X6181/X6141/X6142 PHY.
********************************************************************/
...
)
{
    ...
    switch((chipData2 & 0x000C)>>2)
    {
        ...
        case MYD_DEV_X7120:
        {
            MYD_U16 deviceSubType;

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_CHIP_REG, 0xF411, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_CHIP_REG, 0xF410, 0x70));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_CHIP_REG, 0xF410, 0x30));
            ATTEMPT(mydWait(pDev, 10));
            ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, MYD_CHIP_REG, 0xF412, 7, 1, &deviceSubType));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_CHIP_REG, 0xF410, 0x90));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_CHIP_REG, 0xF410, 0x91));

            if (deviceSubType)
            {
                pDev->deviceId = MYD_DEV_X6142;
                *deviceId = MYD_DEV_X6142;
            }
            else
            {
...
)
{
    switch(pDev->deviceId)
    {
        ...
        case MYD_DEV_X6142:
            *portCount = MYD_X6142_PORT_COUNT;
            break;
...
)
{
    ...
    else
    {
        ...
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 
                                ((host_or_line == MYD_HOST_SIDE)? 0x15E8:0x05E8)+(laneOffset*0x2000), 0));

        /* Clear all others PRBS patterns setting */
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, prbsControlRegAddr, 0));

        switch(pattSel)
        {
            case MYD_PAM4_PRBS31Q:
                ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, prbsControlRegAddr, 0x2000));
                break;

            case MYD_PAM4_JP03A:
                ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, prbsControlRegAddr, 0x100));
                break;

            case MYD_PAM4_JP03B:
                ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, prbsControlRegAddr, 0x200));
                break;

            case MYD_PAM4_SSPRQ:
                ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, prbsControlRegAddr, 0x4000));
                break;
            ...
            case MYD_PAM4_PRBS13Q:
            {
                ...
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, MYD_PMA_DEV, prbs13QReg+(laneOffset*0x2000), 0, 4, 0xF));
                break;
            }
...
)
{
    MYD_SPEED speed;
    MYD_U16 mdioPortIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    MYD_U16 regCntlPCS = 0, regStatusPCS = 0, controlANEGReg;     
...
#endif
    MYD_UNUSED_PARAM(stateDumpOptions);
    return MYD_OK;
}
...
{
    MYD_U16 speed, autoNeg, mdioIndex;
    MYD_OP_MODE opMode = MYD_OP_MODE_UNKNOWN;

    /* this is a diagnostic function. limited error checking is done. */
    MYD_CHECK_DEV_SIDE(pDev, mdioPort, laneOffset, host_or_line);

    /* sanity check for inputs within ranges */
    if ((txFFEConfig.preCursor < -7) || (txFFEConfig.preCursor > 15) ||
        (txFFEConfig.postCursor < -31) || (txFFEConfig.postCursor > 31) ||
        (txFFEConfig.attenuation < 0) || (txFFEConfig.attenuation > 31))
    {
        MYD_DBG_ERROR("mydSetTxFFE: invalid FFE values - preCursor:%d; attenuation:%d; postCursor:%d\n",
                      txFFEConfig.preCursor, txFFEConfig.attenuation, txFFEConfig.postCursor);
        return MYD_FAIL;
    }
    mdioIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    if (mdioIndex >= MYD_MAX_PORTS) {
        MYD_DBG_ERROR("mydSetTxFFE: mdioIndex out of bounds %d\n",mdioIndex);
        return MYD_FAIL;

    }
    if (host_or_line == MYD_HOST_SIDE)
    {
        speed = pDev->hostConfig[mdioIndex][laneOffset].speed;
        opMode = pDev->hostConfig[mdioIndex][laneOffset].opMode;
        autoNeg = pDev->hostConfig[mdioIndex][laneOffset].autoNeg;
    }
    else
    {
        speed = pDev->lineConfig[mdioIndex][laneOffset].speed;
        opMode = pDev->lineConfig[mdioIndex][laneOffset].opMode;
        autoNeg = pDev->lineConfig[mdioIndex][laneOffset].autoNeg;
    }

    if (opModeTable[opMode][MYD_OP_TABLE_SIGNAL] == MYD_SIGNAL_PAM4)
    {
        /* PAM4 uses different registers and not based on speed */
        /* set preCursor */
        ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_17+(0x80*laneOffset), 
                                    0, 8, txFFEConfig.preCursor));

        /* set attenuation */
        ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_18+(0x80*laneOffset), 
                                    0, 8, txFFEConfig.attenuation));

        /* set postCursor */
        ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_19+(0x80*laneOffset), 
                                    0, 8, txFFEConfig.postCursor));
    }
    else
    {
        switch (speed)
        {
            ...
            case MYD_5GB:
                /* set preCursor */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_14+(0x80*laneOffset), 
                                            0, 8, txFFEConfig.preCursor));

                /* set attenuation */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_15+(0x80*laneOffset), 
                                            0, 8, txFFEConfig.attenuation));

                /* set postCursor */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_16+(0x80*laneOffset), 
                                            0, 8, txFFEConfig.postCursor));
                break;
            ...
            case MYD_40GB:
                /* set preCursor */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_14+(0x80*laneOffset), 
                                            8, 8, txFFEConfig.preCursor));

                /* set attenuation */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_15+(0x80*laneOffset), 
                                            8, 8, txFFEConfig.attenuation));

                /* set postCursor */
                ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_16+(0x80*laneOffset), 
                                            8, 8, txFFEConfig.postCursor));
                break;
            ...
            default:
                MYD_DBG_ERROR("mydSetTxFFE: speed not supported\n");
                return MYD_FAIL;
                break;
        }
    ...
    {
        /* enable/disable Tx initial taps command to serdes for AN mode */
        ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, 0xF27F+(0x80*laneOffset), 0, 1, enableInitialTaps));
    }
    else
    {
        /* enable/disable Tx initial taps command to serdes for non-AN mode */
        ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, 0xF26F+(0x80*laneOffset), 0, 1, enableInitialTaps));
    }
...
)
{
    ...
    mdioIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
	if (mdioIndex >= MYD_MAX_PORTS) {
		MYD_DBG_ERROR("mydGetTxFFE: mdioIndex out of bounds %d\n",mdioIndex);
		return MYD_FAIL;

	}
    mydMemSet(txFFEConfig, 0, sizeof(MYD_TX_FFE_CONFIG));
    ...
    {
        /* set preCursor */
        ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_17+(0x80*laneOffset), 
                                    0, 8, (MYD_U16*)&txFFEConfig->preCursor));

        /* set attenuation */
        ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_18+(0x80*laneOffset), 
                                    0, 8, (MYD_U16*)&txFFEConfig->attenuation));

        /* set postCursor */
        ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_19+(0x80*laneOffset), 
                                    0, 8, (MYD_U16*)&txFFEConfig->postCursor));
    }
    else
    {
        switch (speed)
        {
            ...
            case MYD_5GB:
                /* set preCursor */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_14+(0x80*laneOffset), 
                                            0, 8, (MYD_U16*)&txFFEConfig->preCursor));
    
                /* set attenuation */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_15+(0x80*laneOffset), 
                                            0, 8, (MYD_U16*)&txFFEConfig->attenuation));

                /* set postCursor */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_16+(0x80*laneOffset), 
                                            0, 8, (MYD_U16*)&txFFEConfig->postCursor));
                break;
            ...
            case MYD_40GB:
                /* set preCursor */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_14+(0x80*laneOffset), 
                                            8, 8, (MYD_U16*)&txFFEConfig->preCursor));

                /* set attenuation */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_15+(0x80*laneOffset), 
                                            8, 8, (MYD_U16*)&txFFEConfig->attenuation));

                /* set postCursor */
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort, host_or_line, MYD_SERDES_LANE0_REG_16+(0x80*laneOffset), 
                                            8, 8, (MYD_U16*)&txFFEConfig->postCursor));
                break;
            ...
            default:
                MYD_DBG_ERROR("mydGetTxFFE: speed not supported\n");
                return MYD_FAIL;
                break;
        }
    }

    if (txFFEConfig->preCursor & 0x80)
    {
        txFFEConfig->preCursor |= 0xFF00;
    }

    if (txFFEConfig->postCursor & 0x80)
    {
        txFFEConfig->postCursor |= 0xFF00;
    }

    MYD_DBG_INFO("mydGetTxFFE: preCursor:%d, attenuation:%d, postCursor:%d\n", txFFEConfig->preCursor, 
                 txFFEConfig->attenuation, txFFEConfig->postCursor);
