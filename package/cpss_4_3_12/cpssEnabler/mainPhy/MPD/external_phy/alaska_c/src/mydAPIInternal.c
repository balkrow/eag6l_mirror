...
)
{
    ...
    hostLaneCount = opModeTable[hostOpMode][MYD_OP_TABLE_LANE_COUNT];
    ...
    if (pDev->deviceId == MYD_DEV_X6142)
    {
        if (hostLaneCount > MYD_2_LANE_MODE)
        {
            MYD_DBG_ERROR("mydIsOpModeValid: 4 or more host lane mode is not supported on X6142.\n");
            return MYD_FAIL;
        }
    }
...
)
{
    ...
    switch(laneCount)
    {
        ...
        case MYD_2_LANE_MODE:
            ...
            {
            	if (mdioPortIndex >= MYD_MAX_PORTS) {
            		MYD_DBG_ERROR("mydCheckNeedReConfig: mdioPortIndex %d out of bounds\n", mdioPortIndex);
            	    return MYD_FAIL;
            	}
            	if (laneOffset+i >= MYD_NUM_LANES) {
            		MYD_DBG_ERROR("mydCheckNeedReConfig: laneOffset %d + i %d out of bounds\n", laneOffset,i);
            		return MYD_FAIL;
            	}
        ...
        case MYD_8_LANE_MODE:
            ...
            for (j=0; j<2; j++) /* 8-lane mode uses 2 lanes */
            {
                for (i=0; i<MYD_NUM_LANES; i++)
                {
                	if (mdioPortIndex+j >= MYD_MAX_PORTS) {
                		MYD_DBG_ERROR("mydCheckNeedReConfig: mdioPortIndex %d + j %d out of bounds\n", mdioPortIndex,j);
                		return MYD_FAIL;
                	}
                	if (laneOffset+i >= MYD_NUM_LANES) {
                		MYD_DBG_ERROR("mydCheckNeedReConfig: laneOffset %d + i %d out of bounds\n", laneOffset,i);
...
)
{
    ...
    if (mdioIndex >= MYD_MAX_PORTS) {
        MYD_DBG_ERROR("mydCheckMixedOpModeSupport: mdioIndex out of bounds %d\n",mdioIndex);
        return MYD_FAIL;
    ...
    {
        if (pDev->hostConfig[mdioIndex][laneIndex].opMode != MYD_OP_MODE_UNKNOWN)
        {
            if (hostModeType != pDev->hostConfig[mdioIndex][laneIndex].opModeType)
            {
...
)
{
    ...
    mdioIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    if (mdioIndex >= MYD_MAX_PORTS) {
    	MYD_DBG_ERROR("mydSetConfigInfo: mdioIndex out of bounds %d\n",mdioIndex);
        return MYD_FAIL;
    }
    for (laneIndex=laneOffset; laneIndex<(laneOffset+laneCount); laneIndex++)
    {
        if (laneIndex >= MYD_NUM_LANES) {
        	MYD_DBG_ERROR("mydSetConfigInfo: laneIndex out of bounds %d\n",laneIndex);
            return MYD_FAIL;
        }
        if (host_or_line == MYD_HOST_SIDE)
        {
            pModeConfig = &pDev->hostConfig[mdioIndex][laneIndex];
        }
        else
        {
            pModeConfig = &pDev->lineConfig[mdioIndex][laneIndex];
        }
...
{
    MYD_U16 modeType, laneCount, speed, mdioIndex;
    MYD_OP_MODE opMode;

    mdioIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    if (mdioIndex >= MYD_MAX_PORTS) {
    	MYD_DBG_ERROR("mydGetDetailedLinkStatus: mdioIndex out of bounds %d\n",mdioIndex);
        return MYD_FAIL;
    }
    if (host_or_line == MYD_HOST_SIDE)
    {
        opMode = pDev->hostConfig[mdioIndex][laneOffset].opMode;
    }
    else
    {
        opMode = pDev->lineConfig[mdioIndex][laneOffset].opMode;
    }
...
/*******************************************************************
 MYD_STATUS mydPowerDownSharedLane
   Used internally for mode configuration. 
   Power down shared lane on both the host and line side
*******************************************************************/
static MYD_STATUS mydPowerDownSharedLane
(
    IN MYD_DEV_PTR pDev,
    IN MYD_U16 mdioPort,
    IN MYD_U16 host_or_line,
...
)
{
    ...
    if (laneCount == MYD_4_LANE_MODE)
    {
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF001, 0x2000));
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF002, 0x2000));
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF003, 0x2000));
    }
    else if (laneCount == MYD_2_LANE_MODE)
    {
        ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF000+(laneOffset+1), 0x2000));
    }
        ...
        case MYD_25GB: 
            ...
            else if (laneCount == MYD_2_LANE_MODE)
            {
                if (laneOffset == 0)
                {
                    ...
                    ATTEMPT(mydPowerDownSharedLane(pDev, mdioPort, host_or_line, laneOffset, opMode));
...
)
{
    ...
    switch(laneCount)
    {
        ...
        case MYD_8_LANE_MODE:
            ...
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF258, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF259, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25E, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25F, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25A, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25B, 0x0));
            ...
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0211, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0411, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0611, 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0212, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0412, 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, 0x0612, 0x0));
            break;

        case MYD_2_LANE_MODE:
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF378, 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF258+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF258+(0x0080*(laneOffset+1)), 0x0));
            
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF259+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF259+(0x0080*(laneOffset+1)), 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, (0xF25E)+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, (0xF25E)+(0x0080*(laneOffset+1)), 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25F+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25F+(0x0080*(laneOffset+1)), 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25A+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25A+(0x0080*(laneOffset+1)), 0x0));

            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25B+(0x0080*laneOffset), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, host_or_line, 0xF25B+(0x0080*(laneOffset+1)), 0x0));
            ...
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, (0x0011 + (0x0200*(laneOffset+1))), 0x0));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_AUTONEG, (0x0012 + (0x0200*(laneOffset+1))), 0x0));
            break;
...
{
    MYD_U16 signaling, autoNeg;
    MYD_U16 temp, baseReg, stageValidBits=0;
    MYD_U16 speedCheckNRZ=0, isAneg=0;
    MYD_U16 maxHF, minHF, maxLF, minLF, channelVal, channelLen, tempRampVal;
    /* MYD_U16 speed = opModeTable[opMode][MYD_OP_TABLE_SPEED]; */
    /* MYD_U16 modeType = opModeTable[opMode][MYD_OP_TABLE_MODE_TYPE]; */

    maxHF = MYD_MODE_OPTION_IGNORE;
    minHF = MYD_MODE_OPTION_IGNORE;
    maxLF = MYD_MODE_OPTION_IGNORE;
    minLF = MYD_MODE_OPTION_IGNORE;
    channelVal  = MYD_MODE_OPTION_IGNORE;
    tempRampVal = MYD_MODE_OPTION_IGNORE;
    channelLen = MYD_CHANNEL_IGNORE;

    signaling  = opModeTable[opMode][MYD_OP_TABLE_SIGNAL];
    autoNeg  = opModeTable[opMode][MYD_OP_TABLE_AN_TRAIN];

    /* Only need to proceed if any of these are set */
    if (!((modeOption & MYD_MODE_CTLE) || (modeOption & MYD_MODE_CHANNEL_LEN) ||
        (modeOption & MYD_MODE_TEMPERATURE_RAMP)))
    {
        return MYD_OK;
    }

    if (modeOption & MYD_MODE_CTLE)
    {
        if (host_or_line == MYD_HOST_SIDE)
        {
            maxHF = (pDev->modeParams.hostMaxHF == MYD_MODE_OPTION_DEFAULT)? 
                    MYD_MODE_DEF_HF_MAX:pDev->modeParams.hostMaxHF;
            minHF = (pDev->modeParams.hostMinHF == MYD_MODE_OPTION_DEFAULT)? 
                    MYD_MODE_DEF_HF_MIN:pDev->modeParams.hostMinHF;
            maxLF = (pDev->modeParams.hostMaxLF == MYD_MODE_OPTION_DEFAULT)? 
                    MYD_MODE_DEF_LF_MAX:pDev->modeParams.hostMaxLF;
            minLF = (pDev->modeParams.hostMinLF == MYD_MODE_OPTION_DEFAULT)? 
                    MYD_MODE_DEF_LF_MIN:pDev->modeParams.hostMinLF;
        }
        else
        {
            maxHF = (pDev->modeParams.lineMaxHF == MYD_MODE_OPTION_DEFAULT)?
                    MYD_MODE_DEF_HF_MAX:pDev->modeParams.lineMaxHF;
            minHF = (pDev->modeParams.lineMinHF == MYD_MODE_OPTION_DEFAULT)?
                    MYD_MODE_DEF_HF_MIN:pDev->modeParams.lineMinHF;
            maxLF = (pDev->modeParams.lineMaxLF == MYD_MODE_OPTION_DEFAULT)?
                    MYD_MODE_DEF_LF_MAX:pDev->modeParams.lineMaxLF;
            minLF = (pDev->modeParams.lineMinLF == MYD_MODE_OPTION_DEFAULT)?
                    MYD_MODE_DEF_LF_MIN:pDev->modeParams.lineMinLF;
        }
    }

    if (modeOption & MYD_MODE_CHANNEL_LEN)
    {
        if (host_or_line == MYD_HOST_SIDE)
        {
            if (pDev->modeParams.hostChannelLenOption == MYD_CHANNEL_LONG)
            { 
                channelLen = MYD_CHANNEL_LONG;
                channelVal = (pDev->modeParams.hostChannelLenVal == MYD_MODE_OPTION_DEFAULT)? 
                             MYD_MODE_DEF_CHANNEL_LONG:pDev->modeParams.hostChannelLenVal;

            }

            if (pDev->modeParams.hostChannelLenOption == MYD_CHANNEL_SHORT)
            {
                channelLen = MYD_CHANNEL_SHORT;
                channelVal = (pDev->modeParams.hostChannelLenVal == MYD_MODE_OPTION_DEFAULT)? 
                             MYD_MODE_DEF_CHANNEL_SHORT:pDev->modeParams.hostChannelLenVal;
            }
        }
        else
        {
            if (pDev->modeParams.lineChannelLenOption == MYD_CHANNEL_LONG)
            { 
                channelLen = MYD_CHANNEL_LONG;
                channelVal = (pDev->modeParams.lineChannelLenVal == MYD_MODE_OPTION_DEFAULT)? 
                             MYD_MODE_DEF_CHANNEL_LONG:pDev->modeParams.lineChannelLenVal;
            }

            if (pDev->modeParams.lineChannelLenOption == MYD_CHANNEL_SHORT)
            {
                channelLen = MYD_CHANNEL_SHORT;
                channelVal = (pDev->modeParams.lineChannelLenVal == MYD_MODE_OPTION_DEFAULT)? 
                             MYD_MODE_DEF_CHANNEL_SHORT:pDev->modeParams.lineChannelLenVal;
            }
        }
    }

    if (modeOption & MYD_MODE_TEMPERATURE_RAMP)
    {
        if (host_or_line == MYD_HOST_SIDE)
        {
            tempRampVal = (pDev->modeParams.hostTemperatureRampVal == MYD_MODE_OPTION_DEFAULT)? 
                          MYD_MODE_DEF_TEMP_RAMP:pDev->modeParams.hostTemperatureRampVal;
        }
        else
        {
            tempRampVal = (pDev->modeParams.lineTemperatureRampVal == MYD_MODE_OPTION_DEFAULT)? 
                          MYD_MODE_DEF_TEMP_RAMP:pDev->modeParams.lineTemperatureRampVal;
        }
    }
    ...
    if (maxHF != MYD_MODE_OPTION_IGNORE)
    {
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 0, 0x2C, 0x908, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 1, 0x6C, maxHF, 1, speedCheckNRZ));
    }

    if (minHF != MYD_MODE_OPTION_IGNORE)
    {
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 2, 0x2C, 0x907, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 3, 0x6C, minHF, 1, speedCheckNRZ));
    }

    if (maxLF != MYD_MODE_OPTION_IGNORE)
    {
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 4, 0x2C, 0x90A, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 5, 0x6C, maxLF, 1, speedCheckNRZ));
    }

    if (minLF != MYD_MODE_OPTION_IGNORE)
    {
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 6, 0x2C, 0x909, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 7, 0x6C, minLF, 1, speedCheckNRZ));
    }

    if (tempRampVal != MYD_MODE_OPTION_IGNORE)
    {
        /* enabled minLF with Temp Ramp option. */
        if (minLF == MYD_MODE_OPTION_IGNORE)
        {
            ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 6, 0x2C, 0x909, 1, speedCheckNRZ));
            ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 7, 0x6C, MYD_MODE_DEF_LF_MIN, 1, speedCheckNRZ));
        }

        /* RX FFE post max */
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 8, 0x2C, 0xD0D, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 9, 0x6C, 0x8, 1, speedCheckNRZ));

        /* RX FFE pre2 min */
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 10, 0x2C, 0xD09, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 11, 0x6C, 0xFFFD, 1, speedCheckNRZ));

        /* HF post gradient threshold */
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 12, 0x2C, 0x117, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 13, 0x6C, 0x295C, 1, speedCheckNRZ));

        /* common mode adjustment */
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 14, 0x2C, 0x809E, 1, speedCheckNRZ));
        ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 15, 0x6C, tempRampVal, 1, speedCheckNRZ));
    }

    if (channelVal != MYD_MODE_OPTION_IGNORE)
    {
        if (channelLen == MYD_CHANNEL_LONG)
        {
            if (signaling == MYD_SIGNAL_PAM4)
            {
                /* pCal effort = 1 */
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 17, 0x2C, 0x10D, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 18, 0x6C, 0x1, 1, speedCheckNRZ));

                /* BFLF */
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 19, 0x2C, 0xE03, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 20, 0x6C, channelVal, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 21, 0xEC, 0x12, 1, speedCheckNRZ));
            }
        }

        if (channelLen == MYD_CHANNEL_SHORT)
        {
            if (autoNeg == MYD_AN_TRAIN_ON)
            {
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 16, 0x2C, 0x4301, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 17, 0x6C, 0x0078, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 18, 0x2C, 0x4302, 1, speedCheckNRZ));

                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 19, 0x6C, 0x0044, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 20, 0x2C, 0x2800, 1, speedCheckNRZ));
                ATTEMPT(mydGenerateInterruptNoStageValid(pDev, mdioPort, host_or_line, laneOffset, isAneg, 21, 0x6C, channelVal, 1, speedCheckNRZ));
            }
        }
    }
...
)
{
    ...
    if (modeOption & MYD_MODE_INIT_CTLE)
    {
        if (host_or_line == MYD_HOST_SIDE)
        {
            if (!(pDev->modeParams.hostInitHF == MYD_MODE_OPTION_DEFAULT ||
                  pDev->modeParams.hostInitHF == MYD_MODE_OPTION_IGNORE))
            {
                HF = pDev->modeParams.hostInitHF;
            }
            if (!(pDev->modeParams.hostInitLF == MYD_MODE_OPTION_DEFAULT ||
                  pDev->modeParams.hostInitLF == MYD_MODE_OPTION_IGNORE))
            {
                LF = pDev->modeParams.hostInitLF;
            }
        }
        else
        {
            if (!(pDev->modeParams.lineInitHF == MYD_MODE_OPTION_DEFAULT ||
                  pDev->modeParams.lineInitHF == MYD_MODE_OPTION_IGNORE))
            {
                HF = pDev->modeParams.lineInitHF;
            }
            if (!(pDev->modeParams.lineInitLF == MYD_MODE_OPTION_DEFAULT ||
                  pDev->modeParams.lineInitLF == MYD_MODE_OPTION_IGNORE))
            {
                LF = pDev->modeParams.lineInitLF;
            }
        }
    }
...
)
{
    ...
    MYD_BOOL longChannel = MYD_FALSE;
...
)
{
    ...
    for (portIndex=0; portIndex<pDev->portCount; portIndex++)
    {
        for(interfaceIndex=0; interfaceIndex<MYD_NUM_INTERFACE; interfaceIndex++)
        {
            ...
            {
                ATTEMPT(mydHwGetPhyRegField(pDev, mdioPort+portIndex, host_or_line, 0xF0EF, (2*laneOffset), 1, &lanePowerDownVal));
                if (lanePowerDownVal == 1)
                {
                    continue;    /* Skip the lane that not used */
                }
                ...
                for (opModeIndex=0; opModeIndex<MYD_OP_MODE_COUNT; opModeIndex++)
                {
                    if ((modeReg & 0x0FFF) == (opModeTable[opModeIndex][MYD_OP_TABLE_REG] & 0x0FFF))
                    {
                        ...
                        laneCount = opModeTable[opModeIndex][MYD_OP_TABLE_LANE_COUNT];
                        if (laneCount == MYD_1_LANE_MODE)
                        {
                ...
                if (opModeIndex >= MYD_OP_MODE_COUNT)
                {
                    MYD_DBG_ERROR("mydReloadConfigInfo: Error reloading. No match for mode register reading 0x%04X\n", modeReg);
                    return MYD_FAIL;
                }
            }
        }
...
)
{
    ...
    if (pDev->deviceId == MYD_DEV_X7120 || pDev->deviceId == MYD_DEV_X6142)
    {
        switch(gpioPinId)
        {
            case MYD_PIN_GPIO1:
...
)
{
    ...
    if (!(ledCtrl.blinkActivity == MYD_LED_ACT_BLINK_OFF ||
          ...
          ledCtrl.blinkActivity == MYD_LED_ACT_FORCE_BLINK ||
...
)
{
    ...
    MYD_UNUSED_PARAM(portIndex);
    MYD_UNUSED_PARAM(interfaceIndex);
    MYD_UNUSED_PARAM(muxNum);
    return MYD_OK;
}
