...
#define PRV_MPD_MYD_OBJECT_MAC(_rel_ifindex)        PRV_MPD_SHARED_GLBVAR_MYD_PORT_DB_ACCESS_MAC()[_rel_ifindex]->mydObject_PTR
#define PRV_MPD_MYD_CONTEXT_MAC(_rel_ifindex)        PRV_MPD_SHARED_GLBVAR_MYD_PORT_DB_ACCESS_MAC()[_rel_ifindex]->mydObject_PTR->hostContext
...
#ifdef MTD_PHY_INCLUDE
    /** @brief Port DB Array for MTD based ports */
    PRV_MPD_MTD_OBJECT_STC              * prvMpdSharedMtdPortDb_ARR[MPD_MAX_PORT_NUMBER_CNS + 1];
#endif

#ifdef MYD_PHY_INCLUDE
    /** @brief Port DB Array for MYD based ports */
    PRV_MPD_MYD_OBJECT_STC              * prvMpdSharedMydPortDb_ARR[MPD_MAX_PORT_NUMBER_CNS + 1];
#endif
...
#ifdef MTD_PHY_INCLUDE
typedef struct {
    /** @brief Debug Flag: debug FW load sequence */
    UINT_32                                 prvMpdMtdFwFlagId;

    /** @brief Debug Flag: debug SF config sequence */
    UINT_32                                 prvMpdMtdConfigFlagId;

    /** @brief Debug Flag: debug VCT sequence */
    UINT_32                                 prvMpdMtdVctFlagId;

    /** @brief Debug Flag: debug SF EEE */
    UINT_32                                 prvMpdMtdEeeFlagId;

    /** @brief Debug Flag: MTD Error */
    UINT_32                                 prvMpdMtdErrorFlagId;

    /** @brief Debug Flag: debug link change sequence */
    UINT_32                                 prvMpdMtdLinkFlagId;

    /** @brief Debug Flag: MTD informational */
    UINT_32                                 prvMpdMtdInfoFlagId;

    /** @brief Debug Flag: MTD critical info */
    UINT_32                                 prvMpdMtdCriticalFlagId;

    /** @brief Number of ports per FW download instance (In devices where FW download is needed only once per device) */
    UINT_16                                 prvMpdRepresentativeModulo_ARR[MPD_TYPE_NUM_OF_TYPES_E+1];

    /** @brief String buffer to hold Firmware Version info for Printing */
    BOOLEAN                                 prvMpdMtdPrintPhyFw_ARR[MPD_TYPE_NUM_OF_TYPES_E];
} PRV_MPD_NON_SHARED_GLBVAR_MPDMTD_DB_STC;
#endif

#ifdef MYD_PHY_INCLUDE
typedef struct {
    /** @brief PSET to indicate which Ports need Firmware Download */
    PRV_MPD_PORT_LIST_TYP prvMpdMydDownloadModeEnableSet;
} PRV_MPD_NON_SHARED_GLBVAR_MPDMYD_DB_STC;
#endif
...
#ifdef MTD_PHY_INCLUDE
    /** @brief MTD Non-shared DB */
    PRV_MPD_NON_SHARED_GLBVAR_MPDMTD_DB_STC         prvMpdNonSharedMpdMtdDb;
#endif

#ifdef MYD_PHY_INCLUDE
    /** @brief MYD Non-shared DB */
    PRV_MPD_NON_SHARED_GLBVAR_MPDMYD_DB_STC         prvMpdNonSharedMpdMydDb;
#endif
