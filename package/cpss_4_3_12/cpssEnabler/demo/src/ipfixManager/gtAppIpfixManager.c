...
)
{
    ...
    GT_U32                               i;
    GT_U8                                dsaTag[CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS];
    ...
    appIpfixManagerDbPtr->testParams.emm.emKeySize  = emKeySize;
    ...
    cpssOsMemSet(appIpfixManagerDbPtr->globalCfg.txDsaTag, 0, CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS);
    for(i=0; i<CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS; i++)
    {
        appIpfixManagerDbPtr->globalCfg.txDsaTag[i/4] |= (dsaTag[i] << (8*(i%4)));
    }
...
GT_STATUS appDemoIpfixManagerGlobalConfigSet
(
    IN  GT_U8                                     devNum,
...
)
{
    ...
    rc = cpssDxChIpfixManagerGlobalConfigSet(devNum, globalCfgPtr);
    if (rc != GT_OK)
    {
    ...
    cpssOsMemCpy(&appIpfixManagerDbPtr->globalCfg, globalCfgPtr, sizeof(CPSS_DXCH_IPFIX_MANAGER_GLOBAL_CONFIG_STC));

    /* we only support CPSS_DXCH_IPFIX_MANAGER_ENTRIES_PER_FLOW_IS_TWO_E in this phase */
...
/**
 * @internal  appDemoIpfixManagerPortGroupConfigSet function
 * @endinternal
 *
 * @brief configure maxIpfixIndex for all port groups
 *
 * @param[in] devNum   - device number
 * @param[in] maxIpfixIndex - max IPFIX entries per tile
 *
 * @retval GT_OK    - on success
 */
GT_STATUS appDemoIpfixManagerPortGroupConfigSet
(
    IN  GT_U8         devNum,
    IN  GT_U32        maxIpfixIndex
)
{
    GT_STATUS rc = GT_OK;
    ...
    portGroupCfg.maxIpfixIndex = maxIpfixIndex;
    portGroupsBmp    =  0xFF;
...
)
{
    ...
    case CPSS_DXCH_EXACT_MATCH_ACTION_EPCL_E:
        ...
        actionPtr->pclAction.bypassBridge = GT_FALSE;
...
)
{
    ...
    CPSS_DXCH_EXACT_MATCH_KEY_SIZE_ENT                    emKeySize;
    ...
    emKeySize           = appIpfixManagerDbPtr->testParams.emm.emKeySize;
    ...
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E:
        emmCapacity.maxTotalEntries           = PRV_APP_IPFIX_MGR_EMM_ENTRIES_MAX_CNS * 2;
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_33B_E:
        emmCapacity.maxTotalEntries           = PRV_APP_IPFIX_MGR_EMM_ENTRIES_MAX_CNS * 3;
        break;
    case CPSS_DXCH_EXACT_MATCH_KEY_SIZE_47B_E:
        emmCapacity.maxTotalEntries           = PRV_APP_IPFIX_MGR_EMM_ENTRIES_MAX_CNS * 4;
        break;
    default:
        break;
    }
    ...
    emmLookup.profileEntryParamsArray[emmProfileEntryIndex].keyParams.keySize  = emKeySize;
...
)
{
    ...
    GT_U32                                  numPolicerEntries0 = 0;
    GT_U32                                  numPolicerEntries1 = 0;
    GT_U32                                  numPolicerEntries2 = 0;
    ...
    rc = cpssDxChPolicerMemorySizeModeGet(devNum, &mode, &numPolicerEntries0,
                                          &numPolicerEntries1, &numPolicerEntries2);
    if(rc != GT_OK)
    {
    ...
    rc = cpssDxChPolicerMemorySizeModeSet(devNum, mode, numPolicerEntries0, numPolicerEntries1);
    if(rc != GT_OK)
    {
    ...
    if (policerStage == CPSS_DXCH_POLICER_STAGE_INGRESS_0_E)
    {
        numPolicerEntries = numPolicerEntries0;
    }
    else if (policerStage == CPSS_DXCH_POLICER_STAGE_INGRESS_1_E)
    {
        numPolicerEntries = numPolicerEntries1;
    }
    else
    {
        numPolicerEntries = numPolicerEntries2;
    }
...
)
{
    ...
    GT_U32                          pclId;
    GT_U32                          maxPorts = PRV_APP_IPFIX_MGR_MAX_PORT_NUM_CNS;
...
)
{
    ...
    flowPtr->key.keySize = appIpfixManagerDbPtr->testParams.emm.emKeySize;
...
static GT_STATUS prvAppDemoIpfixManagerSdmaRxBufFree
(
    IN GT_U8      devNum,
    IN GT_U8      sdmaQNum,
...
)
{
    ...
    GT_U8                                       devNum = 0;
    GT_U32                                      numOfBuffs = PRV_APP_IPFIX_MGR_PKT_BUFF_LEN;
           ...
           (appIpfixManagerDbPtr->newFlows.taskTerminate == GT_FALSE))
    {
        ...
        if(PRV_CPSS_PP_MAC(devNum)->cpuPortMode == CPSS_NET_CPU_PORT_MODE_SDMA_E)
        {
            numOfBuffs = PRV_APP_IPFIX_MGR_PKT_BUFF_LEN;
        ...
        if (isFound == GT_FALSE)
        {
            ...
            if (rc != GT_OK)
            {
                ...
                rc2 = prvAppDemoIpfixManagerSdmaRxBufFree(devNum, sdmaQNum, numOfBuffs, (GT_U8 **)packetBuffs, buffLenArr);
                if (rc2 != GT_OK)
                {
        ...
        rc = prvAppDemoIpfixManagerSdmaRxBufFree(devNum, sdmaQNum, numOfBuffs, (GT_U8 **)packetBuffs, buffLenArr);
        if (rc != GT_OK)
        {
            __IPFIX_MANAGER_LOG("RX Buff Free failed: rc=%d\n",rc);
        }
...
)
{
    ...
    for (i=0; i<dataPktPtr->header.numExported; i++)
    {
        ...
        __IPFIX_MANAGER_VAL_COMPARE(flowId, dbDataRecordPtr->flowId, rc, GT_FALSE);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
           ...
           (appIpfixManagerDbPtr->dataPkts.taskTerminate == GT_FALSE))
    {
        ...
        rc = cpssDxChIpfixManagerDataGetAll(devNum);
        if (rc != GT_OK)
        {
            __IPFIX_MANAGER_LOG("cpssDxChIpfixManagerDataGetAll failed: rc = %d\n", rc);
            continue;
        }
...
/**
 * @internal   appDemoIpfixManagerFlowDeleteAllOneByOne function
 * @endinternal
 *
 * @brief Delete all active flows one after another
 *
 * @param[in]  devNum        - device number
 *
 * @return GT_OK    - on success
 */
GT_STATUS appDemoIpfixManagerFlowDeleteAllOneByOne
(
    IN  GT_U8     devNum
)
{
    ...
    for (i=0; i<CPSS_DXCH_MAX_PORT_GROUPS_CNS; i++)
    {
        for (j = 0; j < PRV_APP_IPFIX_MGR_FLOWS_PER_PORT_GROUP_MAX_CNS; j++)
        {
            ...
            rc = appDemoIpfixManagerFlowDelete(devNum, i, j);
            if (rc != GT_OK)
            {
                __IPFIX_MANAGER_LOG("appDemoIpfixManagerFlowDelete failed: portGroupId: %d, flowId: %d, rc=%d\n", i,j, rc);
                return rc;
            }
            numDeleted++;
        }
    }
    __IPFIX_MANAGER_DBG_LOG("Deleted %d Entries %d\n", numDeleted);
 ...
 *
 * @param[out] handledIpcMsgPtr - (pointer to) IPC message that we handle.
 *
 * @return GT_OK    -  on success
 */
...
/**
 * @internal appDemoIpfixManagerEventNotifyBind function
 * @endinternal
 *
 * @brief  Bind event notification function
 *
 * @param[in] devNum - device number
 *
 * @retval GT_OK - on success
 */
GT_STATUS appDemoIpfixManagerEventNotifyBind
(
    GT_U8           devNum
)
{
...
/**
 * @internal   appDemoIpfixManagerThreadsStatusDump function
 * @endinternal
 *
 ...
 * @return GT_OK    - on success
 */
GT_STATUS appDemoIpfixManagerThreadsStatusDump
(
    GT_VOID
)
{
    if (appIpfixManagerDbPtr->newFlows.isTaskActive == GT_TRUE)
    {
        __IPFIX_MANAGER_DBG_LOG("Learning new flows task   : active\n");
    }
    else
    {
        __IPFIX_MANAGER_DBG_LOG("Learning new flows task   : inactive\n");
    }

    if (appIpfixManagerDbPtr->dataPkts.isTaskActive == GT_TRUE)
    {
        __IPFIX_MANAGER_DBG_LOG("DataGetAll requests task  : active\n");
    }
    else
    {
        __IPFIX_MANAGER_DBG_LOG("DataGetAll requests task  : inactive\n");
    }

    if (appIpfixManagerDbPtr->ipcFetch.isTaskActive == GT_TRUE)
    {
        __IPFIX_MANAGER_DBG_LOG("IPC message handling task : active\n");
    }
    else
    {
        __IPFIX_MANAGER_DBG_LOG("IPC message handling task : inactive\n");
    }
...
)
{
    ...
    appIpfixManagerDbPtr->allTasksTerminate = GT_FALSE;
...
)
{
    ...
    rc = appDemoIpfixManagerDbCreate(devNum, emKeySize);
    if (rc != GT_OK)
    {
    ...
    if (rc != GT_OK)
    {
        __IPFIX_MANAGER_LOG("cpssDxChIpfixAgingEnableSet failed: rc=%d\n", rc);
        return rc;
    }
    ...
    /* Bind event notfication function */
    rc = appDemoIpfixManagerEventNotifyBind(devNum);
    if (rc != GT_OK)
    {
        __IPFIX_MANAGER_LOG("appDemoIpfixManagerEventNotifyBind failed: rc=%d\n", rc);
        return rc;
    }
    ...
    cpssOsMemSet(globalCfg.txDsaTag,0,CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS);
    for(i=0; i<CPSS_IPFIX_MANAGER_TX_DSA_TAG_LEN_CNS; i++)
    {
        globalCfg.txDsaTag[i/4] |= (dsaTag[i] << (8*(i%4)));
    }

    rc = appDemoIpfixManagerGlobalConfigSet(devNum, &globalCfg);
    if (rc != GT_OK)
    {
        __IPFIX_MANAGER_LOG("appDemoIpfixManagerGlobalConfigSet failed: rc=%d\n", rc);
        return rc;
    }

    rc = appDemoIpfixManagerPortGroupConfigSet(devNum, 4 * _1K);
    if (rc != GT_OK)
    {
        __IPFIX_MANAGER_LOG("appDemoIpfixManagerPortGroupConfigSet failed: rc=%d\n", rc);
        return rc;
    }
    ...
    appIpfixManagerDbPtr->ipfixEnable = GT_FALSE;

#ifndef ASIC_SIMULATION
    rc = appDemoIpfixManagerConfigCheck(devNum, GT_FALSE);
    if (rc != GT_OK)
    {
    ...
    appIpfixManagerDbPtr->ipfixEnable = GT_TRUE;
...
)
{
    ...
    /* Disabled the activation of aging for IPFIX */
