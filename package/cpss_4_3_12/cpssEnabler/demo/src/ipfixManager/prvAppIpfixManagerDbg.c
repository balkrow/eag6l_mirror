...
)
{
    ...
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.ipfixEnable,
                                appIpfixManagerDbPtr->ipfixEnable, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.policerStage,
                                appIpfixManagerDbPtr->globalCfg.policerStage, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.agingOffload,
                                appIpfixManagerDbPtr->globalCfg.agingOffload, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.monitoringOffload,
                                appIpfixManagerDbPtr->globalCfg.monitoringOffload, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.deltaMode,
                                appIpfixManagerDbPtr->globalCfg.deltaMode, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.idleTimeout,
                                appIpfixManagerDbPtr->globalCfg.idleTimeout, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.activeTimeout,
                                appIpfixManagerDbPtr->globalCfg.activeTimeout, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.dataPktMtu,
                                appIpfixManagerDbPtr->globalCfg.dataPktMtu, rc2, printEnable);
    rc |= rc2;
    __IPFIX_MANAGER_VAL_COMPARE(configReturnIpcMsg.configReturn.localQueueNum,
                                (appIpfixManagerDbPtr->globalCfg.ipfixDataQueueNum & 0x7), rc2, printEnable);
    rc |= rc2;

    rc2 = prvCpssDxChNetifCheckNetifNumAndConvertToMgUnitId(devNum,
...
)
{
    ...
    if ((portGroupId > 7) || (flowId > 4*_1K))
    {
        return GT_BAD_PARAM;
    }

    cpssOsMemCpy(flowInfoPtr, &appIpfixManagerDbPtr->flowsDb[portGroupId][flowId],
                 sizeof(PRV_APP_IPFIX_MGR_FLOWS_STC));
...
)
{
    ...
    cpssOsMemCpy(&appIpfixManagerDbPtr->flowsDb[portGroupId][flowId], flowInfoPtr,
                 sizeof(PRV_APP_IPFIX_MGR_FLOWS_STC));
