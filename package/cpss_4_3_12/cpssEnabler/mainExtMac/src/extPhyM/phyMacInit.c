...
{
    GT_STATUS rc = GT_OK;
    GT_BOOL ssState, psState;

    psState = ssState = (dMode == CPSS_PORT_HALF_DUPLEX_E) ? GT_TRUE : GT_FALSE;

    if (stage == CPSS_MACDRV_STAGE_PRE_E) {
        /* set both MACs */
        rc = phyMacMSecSetDuplexMode(devNum, portNum, ssState, psState, PHYMAC_BOTH_SIDE_SET_E);
        if (rc != GT_OK) {
