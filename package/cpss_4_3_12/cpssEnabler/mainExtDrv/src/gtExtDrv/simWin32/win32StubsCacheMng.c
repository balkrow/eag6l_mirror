...
#include <gtOs/globalDb/gtOsGlobalDb.h>/*for osNonSharedGlobalVars , used by PRV_NON_SHARED_DMA_GLOBAL_VAR_SET */
#include <gtOs/globalDb/gtOsGlobalDbInterface.h>/*for : PRV_NON_SHARED_DMA_GLOBAL_VAR_SET */
...
#define ALIGNMENT_MASK          (dmaLen - 1)
...
)
{
    ...
    if(actualMallocOfDma == NULL)
    {
        ...
        /* save the value to CPSS global DB : for proper support of '-usePexLogic' */
        PRV_NON_SHARED_DMA_GLOBAL_VAR_SET(prvExtDrvDmaPhys64/*name in DB*/,uint64_to_GT_U64(prvExtDrvDmaPhys64)/*value*/);
        PRV_NON_SHARED_DMA_GLOBAL_VAR_SET(prvExtDrvDmaLen/*name in DB*/,prvExtDrvDmaLen/*value*/);
    }
