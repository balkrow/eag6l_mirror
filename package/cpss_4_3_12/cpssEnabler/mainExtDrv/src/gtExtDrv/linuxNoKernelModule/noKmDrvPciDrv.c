...
#define R hwInfoPtr->resource
    ...
    {
        if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.busNo != busNo)
            continue;
        if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.devSel != devSel)
            continue;
        if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.funcNo != funcNo)
            continue;
        *hwInfoPtr = osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i];
        if (!(flags & MV_EXT_DRV_CFG_FLAG_ALLOW_NO_PP_E))
        {
            if (R.switching.start == 0 || R.cnm.start == 0)
                return GT_FAIL;
        }
        if (hwInfoPtr->busType != CPSS_HW_INFO_BUS_TYPE_PEX_E)
            return GT_OK; /* MBUS is not removable, once map then forget */

        break; /* If we got here, we matched a previously mapped PCIe device. PCIe might have experienced removal and will need to be remmaped */
    }


    if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum >= NOKM_DEVICES_MAX)
...
CPSS_HW_INFO_STC* prvExtDrvNokmGetMappingByIntNum(GT_U32 intNum)
{
    ...
    for (i = 0; i < osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum; i++)
    {
        if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].irq.switching == intNum)
            return &(osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i]);
    }
    return NULL;
}
...
{
    return prvExtDrvSysfsGetDev(domain, busNo, devSel, funcNo, vendorId, devId);
}
...
)
{
    ...
    {
        if ((osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.busNo == busNo) &&
            (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.devSel == devSel) &&
            (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].hwAddr.funcNo == funcNo))
                break;
    }
...
#define R osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].resource
...
#undef R
    ...
    for (; i < osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum-1; i++)
        osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i] = osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i+1];

    memset(&osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i], 0, sizeof(CPSS_HW_INFO_STC));
