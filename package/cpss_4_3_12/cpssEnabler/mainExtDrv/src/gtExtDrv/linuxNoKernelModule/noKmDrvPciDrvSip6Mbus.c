...
)
{
    ...
    if(mvMbusDrvFd > 0) {
        vaddr = mmap(
#ifdef SHARED_MEMORY
                osNonSharedGlobalVars.osNonVolatileDb.aslrSupport?NULL:( (void*)((GT_UINTPTR)(offset))),
#else
                NULL,
#endif
...
)
{
    ...
    osGlobalDbNonSharedDbPpMappingStageDone();
    osGlobalDbNonSharedDbMbusPpMappingSet(offset>>SYSTEM_PAGE_SHIFT,vaddr);
