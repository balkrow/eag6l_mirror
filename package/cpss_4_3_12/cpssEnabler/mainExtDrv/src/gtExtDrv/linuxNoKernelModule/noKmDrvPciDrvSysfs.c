...
/*
 * List of devices that do not support MSI, names must be from the above
 * prestera_sysmap.name
 */
static const char *no_msi_devices[] = {
    "falcon",
    "hawk",
    "phoenix",
    "ac5",
...
static GT_BOOL sysfs_pci_is_valid_dbdf(
    DBDF_DECL
)
{
...
{
    char fname[128];
    if (sysfs_pci_is_valid_dbdf(DBDF) != GT_OK)
        return GT_BAD_PARAM;
...
/* Add device to mvIntDrv's supported device list */
static GT_STATUS sysfs_pci_add_dev_to_drv(
    DBDF_DECL
)
{
    ...
    fd = open("/sys/bus/pci/drivers/mvpci/new_id", O_WRONLY);
    if (fd < 0) {
        /* Fallback to legacy */
        fd = open("/sys/bus/pci/drivers/mvIntDrv/new_id", O_WRONLY);
        if (fd < 0) {
            return GT_FAIL;
        }
    }

    sprintf(buf, "%x %x", pciVendorId, pciDeviceId);
...
#ifdef SHARED_MEMORY
    if ((fdPtr == NULL)&&(GT_FALSE==osNonSharedGlobalVars.osNonVolatileDb.aslrSupport))
    {
...
)
{
    ...
    if (sysfs_pci_is_valid_dbdf(DBDF) != GT_OK)
    {
        fprintf(stderr, "**** Invalid PCI device %04x:%02x:%02x.%x\n",
                pciDomain, pciBus, pciDev, pciFunc);
        return;
    }
    ...
    /*
     * Try to enable PCI device. For backward compatibility reason do not get
     * upset if this fails because it can happen when we still run an old
     * version of mvIntDrv which does not support adding devices on the fly
     */
    sysfs_pci_add_dev_to_drv(DBDF);
    ...
    if (!sysmap) /* no matching entries, nothing to do */
    {
        ...
        if ((PRV_NON_SHARED_DMA_GLOBAL_VAR_GET(prvExtDrvDmaPhys64).l[1]))
        {
...
static GT_BOOL prvExtDrvPciSupportMsi
(
    const GT_CHAR *name
)
{
    ...
    for (i = 0; no_msi_devices[i]; i++)
        if (!osStrCmp(name, no_msi_devices[i]))
            return GT_FALSE;
...
)
{
    ...
    GT_BOOL     isSip5;
    unsigned    pciInt;
    GT_U32      family, mapIdx, i;
    CPSS_SYSTEM_RECOVERY_INFO_STC system_recovery;
    int prestera_sysmap_idx;
    ...
    /* See if this PCIe bus/device/func already exists in mapping list,
       and if so use it (unmapping it before): */
    for (i = 0; i < osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum; i++)
    {
        ...
        if (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[i].busType == CPSS_HW_INFO_BUS_TYPE_PEX_E)
            break;
    }

    if (i == osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum)
    { /* not found in list, append new entry: */
        mapIdx = osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum;
        osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum++;
    }
    else
    {
        mapIdx = i; /* use found entry, unmap valid current mapping: */
	if ( (hwInfoPtr->resource.cnm.size) && (hwInfoPtr->resource.cnm.start) )
            munmap((void *)hwInfoPtr->resource.cnm.start, hwInfoPtr->resource.cnm.size);

        if ( (hwInfoPtr->resource.switching.size) && (hwInfoPtr->resource.switching.start) )
            munmap((void *)hwInfoPtr->resource.switching.start, hwInfoPtr->resource.switching.size);

        if ( (hwInfoPtr->resource.resetAndInitController.size) &&
             (hwInfoPtr->resource.resetAndInitController.start) )
            munmap((void *)hwInfoPtr->resource.resetAndInitController.start, hwInfoPtr->resource.resetAndInitController.size);
    }
    ...
    prestera_sysmap_idx = sysfs_pci_detect_dev(DBDF, 0);
    if (prestera_sysmap_idx < 0)
    {
        return GT_FAIL;
    }
...
#warning "The option USE_PP_SRAM not supported now"
        ...
        if (prvExtDrvMvIntDrvConnected() == GT_TRUE)
        {
            ...
            rc = sysfs_pci_readNum(DBDF, "device", &pciDeviceId);
            family = pciDeviceId & 0xff00;
            if ( (rc == GT_OK) && ((family == 0xbc00) || (family == 0xbe00) || (family == 0xf400) || (family == 0xf500) || (family == 0xfc00)) )
            {
    ...
    /* Enable MSI on devices that supports it, fallback to legacy on error */
    if (prvExtDrvPciSupportMsi(prestera_sysmap[prestera_sysmap_idx].name))
    {
        if (prvExtDrvMvIntDrvConnected() == GT_TRUE)
        {
            if (prvExtDrvMvIntDrvEnableMsi(DBDF) != GT_OK)
            {
                fprintf(stderr, "Fail to enable MSI, defaulting to legacy\n");
            }
            else
            {
                if ((isSip5 == GT_TRUE) &&
                    (system_recovery.systemRecoveryProcess !=
                    CPSS_SYSTEM_RECOVERY_PROCESS_HA_E))
                        prvNoKm_reg_write_field(R.cnm.start, 0x40050,
                                                0x00010000, 0x00010000);

                NOKMDRV_IPRINTF(("  MSI enabled on %s...\n",
                                prestera_sysmap[i].name));
            }
        }
    }
    ...
    if (rc != GT_OK)
    {
        pciInt = 0x11ab0000 | (osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsNum & 0xffff);
    }
    ...
    osNonSharedGlobalVars.osNonVolatileDb.noKmMappingsList[mapIdx] = *hwInfoPtr;
...
)
{
    ...
    vaddrm = mmap(osNonSharedGlobalVars.osNonVolatileDb.aslrSupport?NULL:vaddr,
   ...
   osGlobalDbNonSharedDbPciePpMappingSet(fname,vaddrm);
