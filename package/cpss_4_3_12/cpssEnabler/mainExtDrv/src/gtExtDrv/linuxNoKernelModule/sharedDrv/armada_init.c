...
static void soc_init_armada38x(void)
{
    ...
    CPSS_HW_DRIVER_SOC_STC drvSoC = DRV_SOC_INIT;
    ...
    /*note that devNum and adressSpaceType are don't care since they will be overriden later.
        This code is used only by primary process.*/
    internalRegs = cpssHwDriverGenMmapCreateDrv(PRV_DEVNUM_DONT_CARE_VALUE,drvSoC.res[0].start, drvSoC.res[0].size,PRV_ADRESS_SPACE_DONT_CARE_VALUE);
...
void soc_init_armada39x(void)
{
    ...
    cpssHwDriverRegister(&drvSoC.common, "SoC");
    pss = cpssHwDriverGenMmapAc8CreateDrv(drvSoC.res[0].start, drvSoC.res[0].size, 0x80,CPSS_HW_DRIVER_AS_LAST_E,0);
    if (pss == NULL)
        return;
