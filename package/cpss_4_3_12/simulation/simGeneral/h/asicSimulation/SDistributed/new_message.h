...
typedef enum{
    ...
    /** @brief : get pid of the WM simulation main process (app ==> Device)
     */
    MSG_TYPE_GET_WM_SIM_PID,

    /** @brief : get pid of the WM simulation main process response message (Device==>App)
     */
    MSG_TYPE_GET_WM_SIM_PID_RESPONSE,
...
typedef enum{
    ...
    DFX_REGISTERS_ACCESS,

    /** BAR0 configuration registers */
    BAR0_REGISTERS_ACCESS
...
typedef struct{
     ...
     *  @brief tree id. (of the interrupt tree in the simulation)
     *  Comments:
     *  Sent from Device Side to Application side
     */
    VARIABLE_TYPE deviceId;
...
/**
* @struct GET_WM_SIM_PID_MSG
 *
 * @brief Describe the "get WM simulation PID" message
*/
typedef struct{
...
/**
* @struct WM_SIM_PID_RESPONSE_MSG
 *
 * @brief Describe the "reply to read register" message
*/
typedef struct{
    ...
    /** @brief WM simulation's process Id
     *  Comments:
     *  sent from Device side to Application side.(as response to message
     *  GET_WM_SIM_PID_MSG)
     */
...
/* value to handle all interrupt trees regardless of their Id value */
#define ALL_INTERRUPT_TREES_ID      0xffeeddcc
