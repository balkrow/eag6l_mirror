...
)
{
    ...
    scibInit0(MAX_DEV_NUMBER); /* memory needs to be allocated here since it is used before scibInit() */
#ifndef RTOS_ON_SIM
...
* @endinternal
*
* @brief   function to replace printf
*   the function used to do print under SCIB lock to prevent deadlocks when
*   interrupt handling of 'vxworks' style in SHOST in windows did preemption to threads.
*   but
*   1. since the cpss not works in this 'vxworks' style mode any more
*   2. in linux cs_ver the soft-reset tests may got to deadlock on this SCIB lock issue
*
* @param[in] format                   -  for printing.
...
{
    sasicgSimulationRole = SASICG_SIMULATION_ROLE_DISTRIBUTED_ASIC_SIDE_E;
    sinitNumOfApplications = clientNum;
    tcpPorts_2[0]= tcpPortSync;
    tcpPorts_2[1]= tcpPortAsync;
}
...
)
{
    ...
    if(tcpPortAsync>=(1<<16))
    {
        return GT_OUT_OF_RANGE;
    }
    sasicgSimulationRole = SASICG_SIMULATION_ROLE_DISTRIBUTED_APPLICATION_SIDE_E;
    client_index =index;
    tcpPorts_2[0]= (GT_U16)tcpPortSync;
    tcpPorts_2[1]= (GT_U16)tcpPortAsync;
...
{
   return sasicgSimulationRole == SASICG_SIMULATION_ROLE_DISTRIBUTED_ASIC_SIDE_E;
}

GT_BOOL  simulationDistributedActAsApp(void)
{
   return sasicgSimulationRole == SASICG_SIMULATION_ROLE_DISTRIBUTED_APPLICATION_SIDE_E;
}
