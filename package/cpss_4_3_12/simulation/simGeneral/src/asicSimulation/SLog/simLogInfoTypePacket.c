...
#define SIM_LOG_PACKET_COMPARE_MAC____SKERNEL_E_ARCH_EXT_INFO_STC(x)            \
    ...
    {                                                                           \
        SIM_LOG_PACKET_COMPARE_MAC(x.srcTrgIsVPort);                            \
        SIM_LOG_PACKET_COMPARE_MAC(x.srcTrgVPort);                              \
    }                                                                           \
    ...
    SIM_LOG_PACKET_COMPARE_MAC(x.haEgressPhyPort1Table_index);                  \
    SIM_LOG_PACKET_COMPARE_MAC(x.haEgressPhyPort2Table_index);                  \
                                                                                \
...
)
{
    ...
    {
        simLogMessage(functionNameString, fileNameString, 0, devObjPtr, SIM_LOG_INFO_TYPE_PACKET_E,
            "empty (in range[%d..%d]) \n",startPortNum,startPortNum+numOfBits-1);
    }
    else
    {
        simLogMessage(functionNameString, fileNameString, 0, devObjPtr, SIM_LOG_INFO_TYPE_PACKET_E,
            "([%d]total ports)(in range[%d..%d]) \n", numPorts,startPortNum,startPortNum+numOfBits-1);
    }
...
)
{
    ...
    if(didDiff_added || didDiff_removed)
    {
        ...
        log_printBitmapArr(devObjPtr ,fileNameString , functionNameString, "",variableNamePtr ,
            newArr,
            SKERNEL_DEV_EGRESS_MAX_PORT_CNS(devObjPtr),0);
...
/*    SIM_LOG_PACKET_COMPARE_MAC(srcTagged);  replaced with tagSrcTagged[SNET_CHT_TAG_0_INDEX_CNS] */
    ...
    SIM_LOG_PACKET_COMPARE_MAC(ipOptionFragError);
    ...
    SIM_LOG_PACKET_COMPARE_MAC(ttlArr[0]);
    SIM_LOG_PACKET_COMPARE_MAC(ttlArr[1]);
    SIM_LOG_PACKET_COMPARE_MAC(ttlArr[2]);
    SIM_LOG_PACKET_COMPARE_MAC(ttlArr[3]);
    ...
    SIM_LOG_PACKET_COMPARE_MAC(label[0]);
    SIM_LOG_PACKET_COMPARE_MAC(exp[0]);
    SIM_LOG_PACKET_COMPARE_MAC(label[1]);
    SIM_LOG_PACKET_COMPARE_MAC(exp[1]);
    SIM_LOG_PACKET_COMPARE_MAC(label[2]);
    SIM_LOG_PACKET_COMPARE_MAC(exp[2]);
    SIM_LOG_PACKET_COMPARE_MAC(label[3]);
    SIM_LOG_PACKET_COMPARE_MAC(exp[3]);
    ...
    SIM_LOG_PACKET_COMPARE_MAC(macTimestampingEnable);

    /* timestamp */
/*    SIM_LOG_PACKET_COMPARE_MAC____SNET_TOD_TIMER_STC(timestamp[0]);*/
/*    SIM_LOG_PACKET_COMPARE_MAC____SNET_TOD_TIMER_STC(timestamp[1]);*/

    SIM_LOG_PACKET_COMPARE_MAC(ingressPchInfoValid);
    SIM_LOG_PACKET_COMPARE_MAC(egressPchInfoValid);
    ...
    SIM_LOG_PACKET_COMPARE_MAC(ptpDomain_8bits);
    ...
    SIM_LOG_PACKET_COMPARE_MAC(packetCmdAssignedByEgress);
    SIM_LOG_PACKET_COMPARE_MAC(maxAllowedVlanTagOffset);

    SIM_LOG_PACKET_COMPARE_MAC(isPktMirrorInLocalDev);

    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        SIM_LOG_PACKET_COMPARE_MAC(genericClassification[0]);
        SIM_LOG_PACKET_COMPARE_MAC(genericClassification[1]);
        SIM_LOG_PACKET_COMPARE_MAC(genericClassification[2]);
        SIM_LOG_PACKET_COMPARE_MAC(genericClassification[3]);

        SIM_LOG_PACKET_COMPARE_MAC(trgVPort);
        SIM_LOG_PACKET_COMPARE_MAC(trgIsVPort);
        SIM_LOG_PACKET_COMPARE_MAC(srcVPort);
        SIM_LOG_PACKET_COMPARE_MAC(srcIsVPort);
        SIM_LOG_PACKET_COMPARE_MAC(trgCircuit);
        SIM_LOG_PACKET_COMPARE_MAC(hft0Triggered);
        SIM_LOG_PACKET_COMPARE_MAC(hft0Ptr);
        SIM_LOG_PACKET_COMPARE_MAC(hft1Triggered);
        SIM_LOG_PACKET_COMPARE_MAC(hft1Ptr);

        SIM_LOG_PACKET_COMPARE_MAC(multiTargetVPort.isUsed);
        SIM_LOG_PACKET_COMPARE_MAC(multiTargetVPort.origTrgVPort);

        SIM_LOG_PACKET_COMPARE_MAC(ipclActionStop);
        SIM_LOG_PACKET_COMPARE_MAC(mpclActionStop);

        SIM_LOG_PACKET_COMPARE_MAC(totalPoppedLabels);
        SIM_LOG_PACKET_COMPARE_MAC(isLabelSwapped   );
        SIM_LOG_PACKET_COMPARE_MAC(fwdLabelsPopSwap );
        SIM_LOG_PACKET_COMPARE_MAC(currFwdLabelIndex);
        SIM_LOG_PACKET_COMPARE_MAC(exp4Egress   );
        SIM_LOG_PACKET_COMPARE_MAC(exp4Qos      );
        SIM_LOG_PACKET_COMPARE_MAC(outerEthDeiUp);
        SIM_LOG_PACKET_COMPARE_MAC(qosModel     );
        SIM_LOG_PACKET_COMPARE_MAC(ttlModel     );
        SIM_LOG_PACKET_COMPARE_MAC(trustExpFlag );
        SIM_LOG_PACKET_COMPARE_MAC(trustDscpFlag);
        SIM_LOG_PACKET_COMPARE_MAC(trustUpFlag  );

        SIM_LOG_PACKET_COMPARE_MAC(sip7TrgPreempt  );
        SIM_LOG_PACKET_COMPARE_MAC(trgPortTcProfile);

        SIM_LOG_PACKET_COMPARE_MAC(tag0Vid);
    }
...
/**
* @internal log__memory_printBitmapArr function
* @endinternal
*
* @brief   print a bitmap of ports from the memory of vlan/vidx/stp...
*
* @param[in] devObjPtr                - device object
* @param[in] fileNameString           - the name (string) of the calling file
* @param[in] functionNameString       - the name (string) of the calling function
* @param[in] variableNamePtr          - the name (string) of the variable
* @param[in] startPort                - the first port printed
* @param[in] endPort                  - the last port printed
* @param[in] bitmapArr                - pointer to bitmap array (size in bits is according to endPort-startPort+1)
*
*/
void log__memory_printBitmapArr
(
    IN SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN GT_CHAR                         *fileNameString,
    IN GT_CHAR                         *functionNameString,
    ...
    IN GT_U32                           startPort,
    IN GT_U32                           endPort,
    IN GT_U32                          *bitmapArr /*memory to start bit (always start at bit 0 regardless to startPort)*/
)
{
    if(!simLogIsOpenFlag)
    {
    ...
    if(NULL == bitmapArr)
    {
        /* support BMP that may be NULL (depend on configuration or state machine) */
        /* save info for the use of simLogInternalLog */
        simLogInfoSave(functionNameString,fileNameString , 0/*line*/,
            devObjPtr,SIM_LOG_INFO_TYPE_PACKET_E);

        simLogInternalLog("[%s] is NULL pointer (not used) \n",variableNamePtr);
    }
    else
    {
        log_printBitmapArr(devObjPtr ,fileNameString , functionNameString, "",variableNamePtr ,
            bitmapArr,
            endPort - startPort+1,startPort);
    }
