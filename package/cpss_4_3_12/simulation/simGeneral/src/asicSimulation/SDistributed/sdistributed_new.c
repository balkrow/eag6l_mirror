...
#define WAIT_CNT_FOR_PRINT_MAX_CNS             500000
...
typedef struct{
    ...
    /** : The remote socket TCP protocol port number */
    GT_U16 remoteProtocolPort;
...
typedef enum{
    ...
    DISTRIBUTED_MESSAGE_TYPE_GET_WM_SIM_PID_E    = MSG_TYPE_GET_WM_SIM_PID,
...
#define MSG_MEMORY_WRITE_DFX_E        DFX_REGISTERS_ACCESS
#define MSG_MEMORY_READ_DFX_E         DFX_REGISTERS_ACCESS
#define MSG_MEMORY_WRITE_BAR0_E       BAR0_REGISTERS_ACCESS
#define MSG_MEMORY_READ_BAR0_E        BAR0_REGISTERS_ACCESS
...
#define OPCODE_TO_STR(read,accessType)                                     \
    (((read) &&(accessType == REGISTER_MEMORY_ACCESS))   ? "read" :        \
     ...
     ((read) &&(accessType == DFX_REGISTERS_ACCESS))     ? "read_DFX" :    \
     ((!read)&&(accessType == DFX_REGISTERS_ACCESS))     ? "write_DFX" :   \
     ((read) &&(accessType == BAR0_REGISTERS_ACCESS))    ? "read_BAR0" :   \
     ((!read)&&(accessType == BAR0_REGISTERS_ACCESS))    ? "write_BAR0" :  \
                                                           "???")
#define DMA_READ    0
#define DMA_WRITE   1
...
/* Number of connected distributed application sockets */
static GT_U32   g_numConnections = 0;
...
)
{
    ...
    if(_bus)
    {
        switch(msgType)
        {
            ...
            case DISTRIBUTED_MESSAGE_TYPE_GET_WM_SIM_PID_E:
                /* the bus convert only when send to the devices */
                convert = 1;
                break;
...
)
{
    ...
    switch(sasicgSimulationRole)
    {
        ...
        case SASICG_SIMULATION_ROLE_DISTRIBUTED_APPLICATION_SIDE_VIA_BROKER_E:
            ...
            if(0xFFFF==client_index)
            {
                skernelFatalError(" simDistributedInit : client_index == 0xFFFF.Please add \"-index <some number> \" to command line \n");
            }
        ...
        case SASICG_SIMULATION_ROLE_DISTRIBUTED_ASIC_SIDE_E:
            ...
            if(sinitNumOfApplications==0)
            {
                skernelFatalError(" simDistributedInit : sinitNumOfApplications == 0.Please add \"-clients <some number> \" to command line \n");
            }
        ...
        case SASICG_SIMULATION_ROLE_DISTRIBUTED_APPLICATION_SIDE_VIA_BROKER_E:
            ...
            for(ii = 0 ; ii < CONNECTION_TYPE_LAST_E; ii ++)
            {
                                         ...
                                         SIM_OS_CNF_FILE_MAX_LINE_LENGTH_CNS, getStr))
                {
                    ...
                    {
                        skernelFatalError(" simDistributedInit %d : tcp port[%d] out of range \n",__LINE__,tmpValue);
                    }
                }
                else
                {
    ...
    */
    ...
    for(ii = 0 ; ii < CONNECTION_TYPE_LAST_E; ii ++)
    {
        ...
        if(tcpPorts_2[ii]!=0)
        {
            ...
            if((tcpPorts_2[ii] +((readSuccess!=0)?(GT_U16)envOffset:0))>=(1<<16))
            {
                skernelFatalError(" simDistributedInit %d : tcp port[%d] out of range. \n",__LINE__,tcpPorts_2[ii] +((readSuccess!=0)?(GT_U16)envOffset:0));
            }

            tcpPorts_2[ii] += ((readSuccess!=0)?(GT_U16)envOffset:0);

        }
        else
        {
            if(ii==CONNECTION_TYPE_SYNCH_E)
            {
                skernelFatalError(" simDistributedInit : missing  sync port configuration.Please use Please add \"-p0  <some port number> \" to command line  \n");
            }
            else
            {
                skernelFatalError(" simDistributedInit : missing  async port configuration.Please use Please add \"-p1  <some port number> \" to command line  \n");
            }
    ...
    g_numConnections = numConnections;
...
* @param[in] treeId                 - the interrupt tree id that set the interrupt
*/
...
)
{
    ...
    message.treeId = (VARIABLE_TYPE)treeId;
    /****************************/
    ...
    simDistributedMessageSend(clientId,connectionPtr,messageHeaderPtr,1,&dataLen, (GT_U32**)(&dataPtr),
        GT_FALSE,GT_FALSE,GT_FALSE /* non blocking */);
...
*         function should be called only on the asic side
* @param[in] deviceId                 - the device id that set the interrupt
* @param[in] treeId                   - the interrupt tree ID number
*/
...
)
{
    ...
    for (i = 0; i < g_numConnections; i++)
    { /* send interrupts to all connections (multiple CPU processes) */
        connectionPtr = convertMessageTypeToConnectionIndex(msgType, i);
        if (connectionPtr != NULL)
            simDistributedInterruptSetExt(deviceId, connectionPtr, treeId);
    }
}
...
/*    GT_U32  disableInterruptValue=0;*/
    ...
    connectionPtr = convertMessageTypeToConnectionIndex(msgType, 0);
...
/*    disableInterruptValue = SHOSTG_interrupt_disable();*/
    ...
    switch(accessType)
    {
        ...
        case SCIB_MEMORY_WRITE_E:
            message.accessType = (VARIABLE_TYPE)REGISTER_MEMORY_ACCESS;
         break;
        case SCIB_MEMORY_READ_PCI_E:
        case SCIB_MEMORY_WRITE_PCI_E:
            message.accessType = (VARIABLE_TYPE)PCI_REGISTERS_ACCESS;
         break;

        case SCIB_MEMORY_READ_BAR0_E:
        case SCIB_MEMORY_WRITE_BAR0_E:
            message.accessType = (VARIABLE_TYPE)BAR0_REGISTERS_ACCESS;
         break;
        case SCIB_MEMORY_WRITE_DFX_E:
        case SCIB_MEMORY_READ_DFX_E:
        default:
           message.accessType = (VARIABLE_TYPE)DFX_REGISTERS_ACCESS;
           break;
    }
        ...
        switch(readRegisterRequest.accessType)
        {
            ...
            case MSG_MEMORY_READ_PCI_E:
                scibPciRegRead(readRegisterRequest.deviceId,
                               ...
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
            case MSG_MEMORY_READ_DFX_E:
                scibMemoryClientRegRead(readRegisterRequest.deviceId,SCIB_MEM_ACCESS_DFX_E,
                               readRegisterRequest.address,
                               readRegisterRequest.readLen,
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
            case MSG_MEMORY_READ_BAR0_E:
                scibMemoryClientRegRead(readRegisterRequest.deviceId,SCIB_MEM_ACCESS_BAR0_E,
                               readRegisterRequest.address,
                               readRegisterRequest.readLen,
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
...
/**
* @internal messagePidReadAndReply function
* @endinternal
*
* @brief   function Received request for read current process pid value ,
*          will get current process pid
*          and will send reply with the data
* @param[in] connectionPtr            - pointer to connection info
* @param[in] offsetInBuf              - the offset in the read buffer
* @param[in,out] messageHeaderPtr         - pointer to the parsed header info
* @param[in] clientIndex              - client index , to send reply to .
...
*                                       none
*/
static void messagePidReadAndReply(
    IN CONNECTION_INFO_STC *connectionPtr,
    IN GT_U32               offsetInBuf,
    INOUT MSG_HDR           *messageHeaderPtr,
    IN GT_U32               clientIndex
...
{
    WM_SIM_PID_RESPONSE_MSG   message;
    GT_U32  dataLenWords = LEN_NO_HEADER_WORDS(WM_SIM_PID_RESPONSE_MSG);
    GT_U32  *dataPtr = FIRST_FIELD_AFTER_HEADER_PTR(&message);
    GT_STATUS status;
    GT_PROCESS_ID pid = 0;
    ...
    if((1 * sizeof(GT_U32)) > connectionPtr->replyBufSize)
    {
        skernelFatalError(" messagePidReadAndReply : too many word to read[%d] , buffer not large enough[%d]",
                            1,
                            connectionPtr->replyBufSize / sizeof(GT_U32));
    }
    ...
    if (status)
    {
        skernelFatalError(" messagePidReadAndReply : pid get err[%d]",
                          status);
    }
    ...
    messageHeaderPtr = &message.hdr;
    /* update the message type */
    messageHeaderPtr->type = (VARIABLE_TYPE)MSG_TYPE_GET_WM_SIM_PID_RESPONSE;
    /* modify the total length field */
    messageHeaderPtr->msgLen = (VARIABLE_TYPE)(dataLenWords*4);/* in bytes */
    ...
    simDistributedMessageSend(clientIndex,connectionPtr,messageHeaderPtr,
                1,&dataLenWords,(GT_U32**)(&dataPtr),
                GT_FALSE,GT_FALSE,GT_TRUE);

    LOG_INFO_READ_DATA_REPLY(1,
                                      ((GT_U32*)connectionPtr->replyBufPtr));
        ...
        switch(writeRegisterRequest.accessType)
        {
            ...
            case MSG_MEMORY_WRITE_PCI_E:
                scibPciRegWrite(writeRegisterRequest.deviceId,
                               ...
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
            case MSG_MEMORY_WRITE_DFX_E:
                scibMemoryClientRegWrite(writeRegisterRequest.deviceId,SCIB_MEM_ACCESS_DFX_E,
                               writeRegisterRequest.address,
                               writeRegisterRequest.writeLen,
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
            case MSG_MEMORY_WRITE_BAR0_E:
                scibMemoryClientRegWrite(writeRegisterRequest.deviceId,SCIB_MEM_ACCESS_BAR0_E,
                               writeRegisterRequest.address,
                               writeRegisterRequest.writeLen,
                               (GT_U32*)connectionPtr->replyBufPtr);
                break;
...
)
{
    ...
    treeId = (VARIABLE_TYPE)(SIM_HTONL(connectionPtr->readBufPtr + offsetInBuf + sizeof(VARIABLE_TYPE)));

    if(msgInterruptSet)
    {
        msgInterruptSet(deviceId, treeId);
    }
    else
    {
...
static unsigned __TASKCONV distributedDispatcherMainTask(IN void* connPtr)
{
    ...
    while(1)
    {
        ...
        if(waitForConnect == 0)
        {
            waitForConnect = 1;
            waitCounter = 0;
            printf("Still waiting for a client %d[%s]\n ",
...
receive_data_from_socket_lbl:
        ...
        else if(connectionPtr->firstValidMessageReceived==GT_FALSE)
        {
            if(bufSize>sizeof(messageHeader))
            {
                ...
                printf("connptr %p firstValidMessageReceived\n",
                       connectionPtr);
            }
            else
            {
        ...
        while (1)
        {
            ...
            switch(messageHeader.type)
            {
                ...
                case DISTRIBUTED_MESSAGE_TYPE_GET_WM_SIM_PID_E:
                    /* get (standalone) distributed simulation process pid */
                    /* asic side --> got the request from the application side */
                    /* reply to message with own pid */
                    /* convert socketId to socket index */
                    /* because the reply will be done on the same socket !!! */
                    clientIndex = convertSocketIdToClientIndex(connectionPtr,currSockId);
                    messagePidReadAndReply(connectionPtr,/* pointer to connection info */
                                                offsetInBuf,/* the offset in the read buffer */
                                                &messageHeader,/* pointer to the parsed header info */
                                                clientIndex);/* socketId to reply to */
                    break;
...
static unsigned __TASKCONV distributedDispatcherServerAccept(IN void* connPtr)
{
    ...
    while(1)
    {
        ...
        /*************************/
        newConnection =
            SIM_OS_MAC(osSocketAccept) (connectionPtr->serverSocket.sockId,sa,&scklen);
        ...
        socketPtr->ready = 1;
        SIM_OS_MAC(osSocketExtractPortFromSocketAddr)(sa, &socketPtr->remoteProtocolPort);
...
*         interrupt .(will not send reply)
* @param[in] deviceId                 - the device Id that set the interrupt
* @param[in] treeId                 - the interrupt tree number that set the interrupt
*                                       none
*/
        ...
        for(jj = 0; jj < MAX_INTERRUPT_LINE_CNS; jj ++)
        {
                ...
                (brokerClientCookiesArray[ii]->interruptInfoArray[jj].interruptLine == ALL_INTERRUPT_LINES_ID)))
            {
                ...
                if(brokerInterruptMode == BROKER_USE_SOCKET)
                {
                    ...
                    simDistributedInterruptSetExt(deviceId,brokerClientCookiesArray[ii]->connectionPtr, treeId);
                }
                else
                {
...
)
{
    ...
    if(connectionPtr->socketServerSide == GT_TRUE)
    {
        ...
        for(ii = 0 ; ii < MAX_INTERRUPT_LINE_CNS ; ii++)
        {
            ...
            cookiePtr->interruptInfoArray[ii].treeId = 0;
        }
...
)
{
    ...
    cookiePtr->interruptInfoArray[ii].masked = GT_FALSE;
    cookiePtr->interruptInfoArray[ii].treeId = msgRegisterInterrupt.treeId;
...
* @param[in] treeId                   - the  interrupt tree Id
*                                       none
*/
...
{
    /* ask to send it to the next station */
    simDistributedInterruptSet(deviceId, treeId);
}
...
* @param[in] treeId                 - the tree ID to match and send on 'Interrupt set'
*/
...
/**
* @internal processAppViaBrokerInterruptHandler function
* @endinternal
*
...
* @param[in] signalId                 - the signal ID to send on 'Interrupt set'
*/
static void processAppViaBrokerInterruptHandler(
    IN GT_SIGNAL_ID signalId
)
{
    /* signal Id used as device ID */
    scibSetInterrupt((GT_U32)signalId, 0);
}
...
* @param[in] signalId                 - the signal ID to send on 'Interrupt set'
*                                      mode          - broker interrupt mode , one of BROKER_INTERRUPT_MASK_MODE
* @param[in] treeId                 - the tree ID to send on 'Interrupt set'
*/
    ...
    if(toRegister == GT_TRUE)
    {
        ...
        if(brokerInterruptMode != BROKER_USE_SOCKET)
        {
            ...
            rc = SIM_OS_MAC(simOsProcessHandler)(signalId,
                                    &processAppViaBrokerInterruptHandler);
            if(rc != GT_OK)
            {
        ...
        appViaBrokerRegisterInterruptMsgSend(interruptLineId, signalId, maskMode, treeId);
    }
}
    ...
    switch(sasicgSimulationRole)
    {
        ...
        case SASICG_SIMULATION_ROLE_DISTRIBUTED_ASIC_SIDE_VIA_INTERFACE_BUS_BRIDGE_E:
            switch(msgType)
            {
                ...
                case DISTRIBUTED_MESSAGE_TYPE_INTERRUPT_SET_E:
                case DISTRIBUTED_MESSAGE_TYPE_GET_WM_SIM_PID_E:
                    if (connIdx >= g_numConnections)
                        return NULL;
                    if (!connectionsArray[CONNECTION_TYPE_ASYNCH_E][connIdx].valid)
                        return NULL;
                    return &connectionsArray[CONNECTION_TYPE_ASYNCH_E][connIdx];
