...
int simFSsave(INOUT char *dirName, IN const char *fname, IN char files[20][100])
{
    ...
    if(dirName[0] == 0)
    {
        ...
        sprintf(dirName, EMBEDDED_FS_TEMP_DIR, getenv(USER_TEMP_DIR), "embeddedFs",
                "CHIP_SIMULATION");
        #else
        /* set a 'per process' folder name so access to it will be only from th single process */
        /* the cs_ver ,if run without '-I' will delete the folders under /tmp/embeddedFs/CHIP_SIMULATION/ */
        sprintf(dirName,"%s%ld%s","/tmp/embeddedFs/CHIP_SIMULATION/",getpid(),"/");
        #endif
