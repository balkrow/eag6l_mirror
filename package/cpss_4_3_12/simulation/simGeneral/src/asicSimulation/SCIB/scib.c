...
*          As well as database allocation
*
...
)
{
    ...
    if (maxDevNumber == 0) {
        skernelFatalError("scibInit0: illegal maxDevNumber %lu\n", maxDevNumber);
    }
    ...
    if ( scibDbPtr == NULL ) {
        skernelFatalError("scibInit0: scibDbPtr allocation error\n");
    }
    ...
    if ( scibFaDbPtr == NULL )
    {
        skernelFatalError("scibInit0: scibFaDbPtr - allocation error\n");
    }
...
* @param[in] treeId                   - ID of interrupt tree
*
...
{
    void* devObjPtr = scibDbPtr[deviceId].deviceObj;/*needed by MACRO:__LOG_NO_LOCATION_META_DATA__WITH_SCIB_LOCK*/

    __LOG_NO_LOCATION_META_DATA__WITH_SCIB_LOCK(("%s: treeId %d\n", "scibSetInterrupt", treeId));
    if(_non_dest == 1 )
    {
    ...
    {
        scibDbPtr[deviceId].interruptMppTriggerFun(deviceId, treeId);
    }
    else
    {
        globalObjectPtr->interruptSetFun(deviceId, treeId);
    }
}
/**
...
)
{
    ...
    if(_app)
    {
        ...
        /* only in the application side of  distributed architecture we need
           function to send the info to the other side (Asic side)*/
        registerReadWrite_distributedApplication(accessType,
                                                 deviceId,
                                                 memAddr,
                                                 length,
                                                 dataPtr);
...
/* struct : PRE_INIT_SPECIFIC_DEVICE_INFO_STC
    hold info about the device before the device is created , to be used during
    it's initialization.
    NOTE: this info is not reset during soft/hard reset , and is used again after
        soft/hard reset.

    the entry state if it hold valid info and to which device
*/
typedef struct{
    GT_U32                          isValid;
    GT_U32                          wmDeviceId;
    WM_SPECIFIC_DEVICE_INFO_STC     devInfo;
}PRE_INIT_SPECIFIC_DEVICE_INFO_STC;

/* no need to make array for too many devices */
#define PRE_INIT_MAX_DEVICES_CNS    8
static PRE_INIT_SPECIFIC_DEVICE_INFO_STC    preInitDeviceInfoArr[PRE_INIT_MAX_DEVICES_CNS];

/**
* @internal wmForceSpecificDeviceInfo function
* @endinternal
*
* @brief   WM function to set information to the device , and ignoring values
*          that may come from the 'registers file' of the device.
*          like forcing specific <pci device id> 0x9061 instead of 0x9041.
*
* @param[in] wmDeviceId    - the WM device ID. (as appears in the INI file)
* @param[in] devInfoPtr    - (pointer to) device info to override 'registers file' info
*
*/
WM_STATUS wmForceSpecificDeviceInfo(
    IN GT_U32   wmDeviceId,
    IN WM_SPECIFIC_DEVICE_INFO_STC   *devInfoPtr
)
{
    GT_U32  ii;
    PRE_INIT_SPECIFIC_DEVICE_INFO_STC   *currEntryPtr;

    currEntryPtr = &preInitDeviceInfoArr[0];
    for(ii = 0 ; ii < PRE_INIT_MAX_DEVICES_CNS; ii++)
    {
        if(!currEntryPtr->isValid)
        {
            /* add new */
            currEntryPtr->isValid    = 1;
            currEntryPtr->wmDeviceId = wmDeviceId;
            currEntryPtr->devInfo    = *devInfoPtr;
            return GT_OK;
        }

        if(currEntryPtr->isValid && wmDeviceId == currEntryPtr->wmDeviceId)
        {
            /* already exists : so update it's data */
            currEntryPtr->devInfo = *devInfoPtr;
            return WM_OK;
        }
    }
...
/* make function to be able to call from terminal and then do soft reset and check it */
WM_STATUS debug_wmForceSpecificDeviceInfo(
    IN GT_U32   wmDeviceId,
    IN GT_U16   pciDeviceId,
    IN GT_U32   revision
)
{
    WM_SPECIFIC_DEVICE_INFO_STC  devInfo;
    ...
    devInfo.pciDeviceId = pciDeviceId;
    devInfo.revision    = revision;

    return wmForceSpecificDeviceInfo(wmDeviceId,&devInfo);
}

/**
* @internal scibSpecificDeviceInfoGet function
* @endinternal
*
* @brief   Get information to the device , and ignoring values
*          that may come from the 'registers file' of the device.
*          like forcing specific <pci device id> 0x9061 instead of 0x9041.
*          relevant when the function return GT_OK
*
* @param[in] wmDeviceId    - the WM device ID. (as appears in the INI file)
* @param[out] devInfoPtr    - (pointer to) device info to override 'registers file' info
*
*   returns :
*       GT_OK           - if the device     hold such info
*       GT_NOT_FOUND    - if the device NOT hold such info
*/
GT_STATUS scibSpecificDeviceInfoGet(
    IN GT_U32   wmDeviceId,
    OUT WM_SPECIFIC_DEVICE_INFO_STC   *devInfoPtr
)
{
    GT_U32  ii;
    PRE_INIT_SPECIFIC_DEVICE_INFO_STC   *currEntryPtr;
    ...
    for(ii = 0 ; ii < PRE_INIT_MAX_DEVICES_CNS; ii++)
    {
        ...
        {
            /* found */
            *devInfoPtr = currEntryPtr->devInfo;
            return WM_OK;
        }
    }
    /* not found */
