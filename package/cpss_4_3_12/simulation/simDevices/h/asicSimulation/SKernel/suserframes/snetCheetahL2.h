...
typedef struct {
    ...
    union {
        ...
        struct {
            ...
            GT_BIT                      isVPort ;  /* sip7 : 1 - DA learned on vPort (when isTrunk == 0) */
            GT_U32                      vportNum; /* sip7 : vport number on which MAC was learned */
        }ucast;
    }targed;
...
typedef struct {
    ...
    GT_BIT                          allPktToPveUplinkEn ; /* Relevant when pvlanEn is 1.
                                                             Enable forwading of all packets, including control packets.
                                                             0x0 = Disable, control traffic is not forwarded to PVLAN uplink
                                                             0x1 = Enable, control traffic is forwarded to PVLAN uplink  */
...
typedef struct{
    ...
    /* needed for SIP7 for NA message to CPU */
...
typedef enum
{
    ...
    */
    ...
    /* IPVX not found cpu codes */
    /* These cpu codes overlaps with user defined cpu codes */
    SNET_CHT_PACKET_IPV4_MC_DIP_NOT_FOUND = 200,
    SNET_CHT_PACKET_IPV4_UC_DIP_NOT_FOUND = 201,
    SNET_CHT_PACKET_IPV6_MC_DIP_NOT_FOUND = 202,
    SNET_CHT_PACKET_IPV6_UC_DIP_NOT_FOUND = 203,
    SNET_CHT_PACKET_IPV4_UC_SIP_NOT_FOUND = 204,
    SNET_CHT_PACKET_IPV6_UC_SIP_NOT_FOUND = 205,
...
typedef enum {
    ...
    /* SIP7 Midway PCL */
    SNET_CHEETAH_ENGINE_UNIT_MPCL_E
