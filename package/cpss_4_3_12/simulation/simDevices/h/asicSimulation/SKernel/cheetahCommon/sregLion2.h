...
/* EQ - EQ To EGF Hash Bit Selection Configuration*/     /* obsolete in sip7 */
...
#define SMEM_LION2_HA_DSA_ERSPAN_REG(dev, index) \
    ...
    (SMEM_CHT_MAC_REG_DB_GET(dev))->HA.dsaErspan[index])
...
#define SMEM_LION2_DFX_SERVER_RESET_CONTROL_REG(dev)        \
    ...
    (SMEM_CHT_MAC_REG_DB_GET(dev))->DFX_SERVER.serverResetControl )
...
#define SMEM_LION2_OAM_EXCESS_KEEPALIVE_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].excessKeepaliveGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* Invalid Keepalive group status */
#define SMEM_LION2_OAM_INVALID_KEEPALIVE_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].invalidKeepaliveGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* MEG Level group status */
#define SMEM_LION2_OAM_MEG_LEVEL_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].MEGLevelGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* Source Interface group status */
#define SMEM_LION2_OAM_SOURCE_INTERFACE_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].sourceInterfaceGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* RDI Status Change Exception group status */
#define SMEM_LION2_OAM_RDI_STATUS_CHANGE_EXCEPTION_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].RDIStatusChangeExceptionGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* Tx Period Exception group status */
#define SMEM_LION2_OAM_TX_PERIOD_EXCEPTION_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].txPeriodExceptionGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* Aging Exception group status */
#define SMEM_LION2_OAM_AGING_EXCEPTION_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].agingEntryGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)

/* Summary Exception group status */
#define SMEM_LION2_OAM_SUMMARY_EXCEPTION_GROUP_STATUS_REG(dev, stage, index)       \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].summaryExceptionGroupStatus[index] :  \
        SMAIN_NOT_VALID_CNS)
...
#define SMEM_LION2_OAM_SUMMARY_EXCEPTION_COUNTER_REG(dev, stage)         \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].summaryExceptionCntr : \
        SMAIN_NOT_VALID_CNS)
...
/* OAM Packet command exception recovery configuration1 */
#define SMEM_LION2_OAM_RECOVERY_PACKET_COMMAND_CONF1_REG(dev, stage)      \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].recoveryPacketCommandConfig1 :  \
        SMAIN_NOT_VALID_CNS)

/* OAM Packet command exception recovery configuration2 */
#define SMEM_LION2_OAM_RECOVERY_PACKET_COMMAND_CONF2_REG(dev, stage)      \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].recoveryPacketCommandConfig2 :  \
        SMAIN_NOT_VALID_CNS)

/* OAM Packet command exception recovery configuration3 */
#define SMEM_LION2_OAM_RECOVERY_PACKET_COMMAND_CONF3_REG(dev, stage)      \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].recoveryPacketCommandConfig3 :  \
        SMAIN_NOT_VALID_CNS)

/* OAM Packet command exception suppress configuration1 */
#define SMEM_LION2_OAM_SUPPRESS_PACKET_COMMAND_CONF1_REG(dev, stage)      \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].exceptionSupressionConfig1 :  \
        SMAIN_NOT_VALID_CNS)

/* OAM Packet command exception suppress configuration2 */
#define SMEM_LION2_OAM_SUPPRESS_PACKET_COMMAND_CONF2_REG(dev, stage)      \
        (SMEM_CHT_IS_SIP5_GET(dev) ?               \
            SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->OAMUnit[stage].exceptionSupressionConfig2 :  \
        SMAIN_NOT_VALID_CNS)
...
#define SMEM_LION2_TXDMA_ENGINE_CONFIG_EXT_PORT_CONFIG_REG(dev)      \
    (SMEM_CHT_MAC_REG_DB_GET(dev))->TXDMA.TXDMAEngineConfig.extPortsConfigReg
...
#define SMEM_LION2_RXDMA_HIGH_DMA_IF_CONFIG_REG(dev)        \
    SMEM_CHT_MAC_REG_DB_GET(dev)->RXDMA.WRDMABAConfigs.highDMAIFConfig
...
#define SMEM_LION3_TTI_LOOKUP_MPLS_EXCEPTION_COMMANDS_REG(dev)     \
        (SMEM_CHT_IS_SIP7_GET(dev) ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->TTI_LU.ttiLuActionConfig.ttiLuMplsExceptionCmds : \
...
/* tti - My physical port attributes, index range 0..255; SIP6 0..287; SIP7 0..303 */
...
/*MLL - Multi-target TC Queues Global Configuration Register */
#define SMEM_LION3_MLL_MULTI_TARGET_TC_GLOBAL_CONF_REG(dev) \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesGlobalConfig : \
    SMAIN_NOT_VALID_CNS)

/*MLL - multi target TC Queues Priority Configuration Register */
#define SMEM_LION3_MLL_MULTI_TARGET_TC_QUEUES_PRIORITY_CONF_REG(dev) \
    (SMEM_CHT_IS_SIP6_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesPriorityConfig : \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesStrictPriorityEnableConfig : \
    SMAIN_NOT_VALID_CNS))

/*MLL - multi target TC Queues Weight Configuration Register */
#define SMEM_LION3_MLL_MULTI_TARGET_TC_QUEUES_WEIGHT_CONF_REG(dev) \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesWeightConfig : \
    SMAIN_NOT_VALID_CNS)

/*MLL - MC Queues drop counter Register */
#define SMEM_LION3_MLL_MC_QUEUE_DROP_CNTR_REG(dev,queue) \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.MLLOutInterfaceCntrs.MLLMCQueueDropCnter[queue] : \
    SMAIN_NOT_VALID_CNS)

/*MLL - qoS Profile To Multi Target TC Queues Register */
#define SMEM_LION3_MLL_QOS_PROFILE_TO_MULTI_TARGET_TC_QUEUES_REG(dev,profile) \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.mcLinkedListMLLTables.qoSProfileToMultiTargetTCQueuesReg[profile/16] : \
    SMAIN_NOT_VALID_CNS)

/*MLL - extended qoS Profile To Multi Target TC Queues Register */
#define SMEM_LION3_MLL_EXT_QOS_PROFILE_TO_MULTI_TARGET_TC_QUEUES_REG(dev,profile) \
    (SMEM_CHT_IS_SIP5_GET(dev)  ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->MLL.mcLinkedListMLLTables.extQoSModeMCQueueSelectionReg[profile/16] : \
    SMAIN_NOT_VALID_CNS)
...
#define SMEM_LION3_PCL_LOOKUP_SOURCE_ID_MASK_REG(dev,index) \
    (SMEM_CHT_IS_SIP6_30_GET(dev) ? SMEM_SIP6_30_IPCL_POLICY_GLOBAL_CONFIG_REG(dev,index) : \
...
#define SMEM_LION3_GOP_TAI_LOAD_VALUE_FRAC_LOW_REG(dev,group,inst)     \
    (SMEM_CHT_IS_SIP5_GET(dev) ? SMEM_CHT_MAC_REG_DB_SIP5_GET(dev)->TAI.TAI[group][inst].timeLoadValueFracLow: \
                                 SMAIN_NOT_VALID_CNS)
...
#define SMEM_LION2_GOP_MPCS_PCS40G_COMMON_CONTROL_REG(dev, port) \
    (SMEM_CHT_MAC_REG_DB_GET(dev))->GOP.ports.MPCSIP[port].PCS40GCommonControl
...
#define SMEM_LION3_MG_GENERAL_CONFIG_REG(dev)                                       \
    (SMEM_CHT_MAC_REG_DB_GET(dev))->MG.globalConfig.generalConfig

#define SMEM_LION3_MG_AUQ_GENERAL_CONFIG_REG(dev)                                       \
    (SMEM_CHT_MAC_REG_DB_GET(dev))->MG.addrUpdateQueueConfig.GeneralControl
