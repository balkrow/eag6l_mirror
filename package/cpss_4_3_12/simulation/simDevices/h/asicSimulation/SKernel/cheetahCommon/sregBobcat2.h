...
/* TM_DROP unit : global configuration */
#define SMEM_BOBCAT2_TM_DROP_GLOBAL_CFG_REG(dev) \
    (SMEM_CHT_IS_SIP5_GET(dev) ? SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TMDROP.globalConfigs : \
        SMAIN_NOT_VALID_CNS)
...
/* TM_DROP unit : Queue Mapping Target Device To Selector Index Table */
#define SMEM_BOBCAT2_TM_QMAP_TRG_DEV_TO_INDEX_TBL_MEM(dev, index) \
    SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, tmQueueMapTargetDeviceToIndex, index)

/* TM_DROP unit : Queue Mapping Queue Selector Table */
#define SMEM_BOBCAT2_TM_QMAP_QUEUE_SELECTOR_TBL_MEM(dev, index) \
    SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, tmQueueMapQueueSelector, index)

/* TM_DROP unit : Queue Mapping Target Device To Selector Index Table */
#define SMEM_BOBCAT2_TM_QMAP_CPU_CODE_TO_TC_TBL_MEM(dev, index) \
    SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, tmQueueMapCpuCodeToTc, index)
