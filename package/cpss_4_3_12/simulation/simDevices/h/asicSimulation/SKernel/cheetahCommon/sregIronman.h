...
#define SMEM_SIP6_30_FDB_UNIT_HSR_PRP_OVERRIDEN_NON_EXPIRED_COUNTER_REG(dev)   \
        SIP_6_REG(dev, FDB.FDBCore.hsrPrp.hsrPrpOverridenNonExpiredCounter)
...
/* Stream Gate Control (SGC) registers macros */
#define SMEM_SIP6_30_SMU_SGC_GLOBAL_CONFIG_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcGlobalConfig)

#define SMEM_SIP6_30_SMU_SGC_GLOBAL_SCAN_CONFIG_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcGlobalScanConfig)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_TOD_MSB_HIGH_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigTodMsbHigh)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_TOD_MSB_LOW_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigTodMsbLow)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_TOD_LSB_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigTodLsb)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_TABLE_SET_CONFIG_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigTableSetConfig)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_PENDING_STATUS_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigPendingStatus)

#define SMEM_SIP6_30_SMU_SGC_RECONFIG_VALID_STATUS_REG(dev)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcReconfigValidStatus)

#define SMEM_SIP6_30_SMU_SGC_TABLE_SET_IS_ACTIVE_REG(dev, _index)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcTableSetIsActive[_index])

#define SMEM_SIP6_30_SMU_SGC_GATE_ID_2_TABLE_SET_CONFIG_REG(dev, _index)      \
        SIP_6_REG(dev, SMU.streamGateControl.sgcGateId2TableSetConfig[_index])

/* Stream Gate Control (SGC) tables macros */
#define SMEM_SIP6_30_SMU_SGC_INTERVAL_MAX_TBL_MEM(dev,index)              \
        SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, smuSgcIntervalMax, index)

#define SMEM_SIP6_30_SMU_SGC_OCTET_COUNTERS_TBL_MEM(dev,index)              \
        SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, smuSgcOctetCounters, index)

#define SMEM_SIP6_30_SMU_SGC_TABLE_SET_TIME_CONFIGURATIONS_TBL_MEM(dev,index)              \
        SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, smuSgcTableSetTimeConfigurations, index)

#define SMEM_SIP6_30_SMU_SGC_TIME_SLOT_ATTRIBUTES_TBL_MEM(dev,index)              \
        SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, smuSgcTimeSlotAttributes, index)

#define SMEM_SIP6_30_SMU_SGC_TIME_TO_ADVANCE_TBL_MEM(dev,index)              \
        SMEM_TABLE_ENTRY_INDEX_GET_MAC(dev, smuSgcTimeToAdvance, index)
