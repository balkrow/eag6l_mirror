...
/* TXQ-SDQ -    Queue_to_Port_Map stable */
#define SMEM_SIP7_TXQ_SDQ_QUEUE_TO_PORT_MAP_TBL_MEM(dev, index, instance,dpIndex) \
    SMEM_TABLE_ENTRY_3_PARAMS_INDEX_GET_MAC(dev, txqSdqSip7QueueToPortMap, index, instance,dpIndex)
...
/* SIP7 RXDMA - rx ingress drop counter */
#define SMEM_SIP7_RXDMA_RX_INGRESS_DROP_COUNTER_MAC(dev, unitIndex, channel, prio) \
    (SIP6_RXDMA_REG_MAC(dev, unitIndex, debug.rxIngressDropCounter.rx_ingress_drop_count) \
        + ( 64 * channel) + (16 * prio))

/* SIP7 RXDMA - rx ingress drop counter config 'pattern' */
#define SMEM_SIP7_RXDMA_RX_INGRESS_DROP_COUNT_TYPE_REF_REG_MAC(dev, unitIndex, channel, prio) \
    (SIP6_RXDMA_REG_MAC(dev, unitIndex, debug.rxIngressDropCounter.rx_ingress_drop_count_type_ref) \
        + ( 64 * channel) + (16 * prio))

/* SIP7 RXDMA - rx ingress drop counter config 'mask' */
#define SMEM_SIP7_RXDMA_RX_INGRESS_DROP_COUNT_TYPE_MASK_REG_MAC(dev, unitIndex, channel, prio) \
    (SIP6_RXDMA_REG_MAC(dev, unitIndex, debug.rxIngressDropCounter.rx_ingress_drop_count_type_mask) \
        + ( 64 * channel) + (16 * prio))
...
/* FDB : get pointer to the memory of the FDB entry .
   ...
   USED only when : devObjPtr->fdbNoDirectMemory != 0
*/
...
*/
#define SMEM_FALCON_EXACT_MATCH_GLOBAL_CONFIGURATION_REG(dev,emUnitNum,regName) \
    SIP_6_REG(dev,EXACT_MATCH[emUnitNum].emGlobalConfiguration.regName)
