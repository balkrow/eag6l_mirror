...
#define SMEM_MAC_TABLE_SIZE_1MB            (0x100000)
...
/*set SMEM_CHUNK_BASIC_STC entry , by the number of bytes, with Linux shared memory support */
#define SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_SHARED_MEM_MAC(startAddr,numBytes, filename)\
    (startAddr),   CONVERT_BYTES_TO_WORDS_MAC(numBytes), 0, 0, 0, 0, 0, 0, GT_TRUE, filename
...
#define SMEM_BIND_TABLE_MAC(tableName)  \
    ...
    GT_FALSE/*isAllocationOfMemoryForbidden*/
...
/*set next 2 fields in SMEM_CHUNK_BASIC_STC entry : tableOffsetValid, tableOffsetInBytes */
#define SMEM_BIND_SHARED_TABLE_MAC(tableName)  \
    1/*tableOffsetValid*/,              \
    FIELD_OFFSET_IN_STC_MAC(tableName,SKERNEL_TABLES_INFO_STC),/*tableOffsetInBytes*/ \
    GT_FALSE,/*not forbidDirectWriteToChunk*/ \
    GT_TRUE/*isAllocationOfMemoryForbidden*/
...
typedef struct SMEM_CHUNK_STCT{
    ...
    GT_BOOL forbidDirectWriteToChunk;/*Indication that must not write to memory , but allow only write to 'lastEntry'
             ...
             */
    GT_BOOL isAllocationOfMemoryForbidden;/* when set the allocation of the chunk is done later.
            and the 'numOfRegisters' is ignored at this state , and the 'last address' is kept
            as 'memFirstAddr' .
            this is needed for shared tables , that allocation done later after the SHM unit is set.
            */
    ...
    GT_BOOL isAllocationofMemoryShared; /* Shared Linux memory for SRAM simulation */
    char *sharedMemFilename; /* filename pointing to the Linux shared memory file */
    int fdSharedMem; /* File descriptor of Linux shared memory */
}SMEM_CHUNK_STC;
...
typedef struct SMEM_ACTIVE_MEM_ENTRY_T {
    ...
    GT_UINTPTR                readFunParam;
    SMEM_ACTIVE_MEM_WRITE_FUN writeFun;
    GT_UINTPTR                writeFunParam;
...
*/
typedef enum{
     SMEM_GENERIC_HIDDEN_MEM_FDB_E          /* sip6 only (not sip5 or sip7) see : devObjPtr->fdbNoDirectMemory*/
    ,SMEM_GENERIC_HIDDEN_MEM_FDB_TILE1_E    /* sip6 only (not sip5 or sip7) see : devObjPtr->fdbNoDirectMemory*/
    ,SMEM_GENERIC_HIDDEN_MEM_FDB_TILE2_E    /* sip6 only (not sip5 or sip7) see : devObjPtr->fdbNoDirectMemory*/
    ,SMEM_GENERIC_HIDDEN_MEM_FDB_TILE3_E    /* sip6 only (not sip5 or sip7) see : devObjPtr->fdbNoDirectMemory*/
    ...
    /* in AAS the tile hold 4 units of EM , that each hold 2 tables (in the SHM unit) */
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_0_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_1_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_2_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_3_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_4_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_5_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_6_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_7_E

    /* the EM tables of AAS in the second tile (each should 'share' with it's 'sister' in tile 0)
       but not for TTI-ILM2,3 client tables
       on real HW this is sharing config in SHM */
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_0_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_1_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_2_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_3_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_4_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_5_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_6_E
    ,SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_1_TABLE_7_E


    /* SIP7 LPM indirect table */
    ,SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_0_BANK_0_E
    ,SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_0_BANK_47_E = SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_0_BANK_0_E + 47
    ,SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_1_BANK_0_E
    ,SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_1_BANK_47_E = SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_1_BANK_0_E + 47
...
* @param[out] fd                      - file descriptor allocated for shared memory
*/
