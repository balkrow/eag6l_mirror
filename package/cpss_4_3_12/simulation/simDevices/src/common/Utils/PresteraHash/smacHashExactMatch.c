...
)
{
    ...
    for (byte = 0; byte < SIP7_EXACT_MATCH_MAX_KEY_SIZE_WORD_CNS; byte++)
    {
        word=dataIn[byte];
        if (byte!= (SIP7_EXACT_MATCH_MAX_KEY_SIZE_WORD_CNS-1) )
        {
            /* bytes 0 to 51 */
            dataOut[(byte*4)]   = (word&0xff);
            dataOut[(byte*4)+1] = (word&0xff00)>>8;
...
/**
* @internal aasHashConfig function
* @endinternal
*
* @brief   This function get the key size and array of indexes and
*          Replicates the index accordingly .
...
* @retval GT_FAIL                  - on error
*/
static GT_STATUS aasHashConfig
(
    IN SIP7_EXACT_MATCH_KEY_SIZE_ENT       keySize,
    IN GT_U32                     mask,
    IN GT_U32                     banks,
    OUT GT_U32                    HashArr[]
)
{
    ...
    switch (keySize)
    {
        case SIP7_EXACT_MATCH_KEY_SIZE_5B_E :
            break;
        case SIP7_EXACT_MATCH_KEY_SIZE_21B_E : /*bank n+1 should have the same index as n*/
            for (ii=0; ii<(banks/2) ;ii++)
            {
                HashArr[banks - 2*ii - 1] = HashArr[(banks/2 - ii - 1)];
                HashArr[banks - 2*ii - 2] = HashArr[(banks/2 - ii - 1)];
            ...
            break;
        case SIP7_EXACT_MATCH_KEY_SIZE_37B_E : /*banks n+1 n+2 should have the same index as n*/
            HashArr[14] = HashArr[13] = HashArr[12] = HashArr[4];
            HashArr[11] = HashArr[10] = HashArr[9]  = HashArr[3];
            HashArr[8]  = HashArr[7]  = HashArr[6]  = HashArr[2];
            HashArr[5]  = HashArr[4]  = HashArr[3]  = HashArr[1];
            HashArr[2]  = HashArr[1]                = HashArr[0];
            break;
        case SIP7_EXACT_MATCH_KEY_SIZE_53B_E : /*banks n+1 n+2 n+3 should have the same index as n*/
            for (ii=0; ii<(banks/4) ;ii++)
            {
                HashArr[banks - 4*ii - 1] = HashArr[(banks/4 - ii - 1)];
                HashArr[banks - 4*ii - 2] = HashArr[(banks/4 - ii - 1)];
...
)
{
    ...
    emNumEntries = devObjPtr->emNumEntries[instance] ;
    emNumOfBanks = devObjPtr->emNumOfBanks[instance] ;
...
)
{
    ...
    __LOG(("CRC mask is [0x%X] for mth[%u] and exactMatchSize[%u]\n", mask, mth, exactMatchSize));
...
/**
* @internal smacHashAasExactMatchMultiHashResultsCalc function
* @endinternal
*
* @brief   This function calculates the CRC multiple hash results
*          (indexes into the Exact Match table).
...
* @retval GT_FAIL                  - on error
*/
GT_STATUS smacHashAasExactMatchMultiHashResultsCalc
(
    IN  SKERNEL_DEVICE_OBJECT               *devObjPtr,
...
)
{
    ...
    GT_STATUS rc = GT_OK;
    GT_U8     cKey[SIP7_EXACT_MATCH_MAX_KEY_SIZE_BYTE_CNS]={0};
    GT_U32    numBitsInData = (emUnitNum == SIP7_EXACT_MATCH_UNIT_REDUCED_E)?
                              SIP7_EXACT_MATCH_REDUCED_MAX_KEY_SIZE_BIT_CNS:
                              SIP7_EXACT_MATCH_MAX_KEY_SIZE_BIT_CNS;
    GT_U32    numOfBanks,mask;
    GT_U32 initCrc = SIP6_EXACT_MATCH_CRC_INIT_VALUE_CNS ;
    ...
    DECLARE_FUNC_NAME(smacHashAasExactMatchMultiHashResultsCalc);
    __LOG(("start smacHashAasExactMatchMultiHashResultsCalc  \n"));

    /*convert 32bit array into 8 bit array */
    convertSip7Key(entryKeyPtr, &cKey[0]);

    ExactMatchGlobalConfigurationGet(devObjPtr,&exactMatchSize,&mth);

    if (emUnitNum == SIP7_EXACT_MATCH_UNIT_REDUCED_E)
    {
        exactMatchSize = SIP6_EXACT_MATCH_TABLE_SIZE_8KB;
        mth            = SIP6_EXACT_MATCH_MHT_4_E;
    }
    crcEm16(cKey, numBitsInData, 0x18005,  (GT_U16) initCrc  , &crcMultiHashArr[0])  ;
    crcEm16(cKey, numBitsInData, 0x11021,  (GT_U16) initCrc  , &crcMultiHashArr[1])  ;
    crcEm16(cKey, numBitsInData, 0x18bb7,  (GT_U16) initCrc  , &crcMultiHashArr[2])  ;
    ...
    __LOG(("smacHashAasExactMatchMultiHashResultsCalc: hash index befor Config : \n"
           "B0 0x%X B1 0x%X B2 0x%X B3 0x%X B4 0x%X B5 0x%X B6 0x%X B7 0x%X "
           "B8 0x%X B9 0x%X B10 0x%X B11 0x%X B12 0x%X B13 0x%X B14 0x%X B15 0x%X \n",
           crcMultiHashArr[0],crcMultiHashArr[1],crcMultiHashArr[2],crcMultiHashArr[3],crcMultiHashArr[4],
           crcMultiHashArr[5],crcMultiHashArr[6],crcMultiHashArr[7],crcMultiHashArr[8],crcMultiHashArr[9],
    ...
    rc = aasHashConfig(keySize, mask, numOfBanks, &crcMultiHashArr[0]);
    if (rc != 0)
    {
        return GT_FAIL;
    }
    ...
    *numOfValidBank = numOfBanks;
    __LOG(("smacHashAasExactMatchMultiHashResultsCalc: hash index after Config : \n"
       "B0 0x%X B1 0x%X B2 0x%X B3 0x%X B4 0x%X B5 0x%X B6 0x%X B7 0x%X "
       "B8 0x%X B9 0x%X B10 0x%X B11 0x%X B12 0x%X B13 0x%X B14 0x%X B15 0x%X \n",
       crcMultiHashArr[0],crcMultiHashArr[1],crcMultiHashArr[2],crcMultiHashArr[3],crcMultiHashArr[4],
       crcMultiHashArr[5],crcMultiHashArr[6],crcMultiHashArr[7],crcMultiHashArr[8],crcMultiHashArr[9],
