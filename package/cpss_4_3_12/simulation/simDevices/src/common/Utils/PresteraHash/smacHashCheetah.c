...
)
{
    ...
    /* set vrfId
       for SIP7 vrfId is 15 bits
       for all the rest vrfId is 12 bits - 3 bits will be zero */
    snetFieldValueSet(dataPtr, 128, 15, vrfId);
}
...
)
{
    ...
    /* set vrfId
       for SIP7 vrfId is 15 bits
       for all the rest vrfId is 12 bits - 3 bits will be zero */
    snetFieldValueSet(dataPtr, 32, 15, vrfId);
}
...
)
{
    ...
    case 0x0103: /* CRC + IPV4 UC */
    case 0x0003: /* XOR + IPV4 UC */
        /* Only CRC algorithm is used in case of ipv4 unicast entries */
        hashCalcFillDipV4VfrIdDataCrc(fdbEntryHashInfoPtr->info.ucRouting.dip[0],
                fdbEntryHashInfoPtr->info.ucRouting.vrfId,
                data);
        dataLen = 44;
        hashCalcDataCrc(
            &(data[0]), crc_numOfDataBits, numBitsToUse, &hash, fdbEntryHashInfoPtr->crcInitValueMode) ;
