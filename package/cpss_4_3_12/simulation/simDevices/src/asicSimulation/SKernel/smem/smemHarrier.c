...
#define HARRIER_NUM_TSU_UNITS  3
...
#define SERDES_0_BASE_ADDR 0x0D100000
#define SERDES_1_BASE_ADDR 0x0D180000
#define SERDES_2_BASE_ADDR 0x0F100000
#define SERDES_3_BASE_ADDR 0x0F180000
#define SERDES_4_BASE_ADDR 0x00B00000
...
/*  ,UNIT_INFO_MAC(0x--------,UNIT_SHM               ,64  *     _1K) no such unit */
    ...
    ,UNIT_INFO_MAC(SERDES_0_BASE_ADDR,      UNIT_SERDES_0_0         ,512  *     _1K)     /* 56G Quad SD Wrapper */
    ,UNIT_INFO_MAC(SERDES_1_BASE_ADDR,      UNIT_SERDES_0_1         ,512  *     _1K)     /* 56G Quad SD Wrapper */
    ,UNIT_INFO_MAC(SERDES_2_BASE_ADDR,      UNIT_SERDES_0_2         ,512  *     _1K)     /* 56G Quad SD Wrapper */
    ,UNIT_INFO_MAC(SERDES_3_BASE_ADDR,      UNIT_SERDES_0_3         ,512  *     _1K)     /* 56G Quad SD Wrapper */
    ,UNIT_INFO_MAC(SERDES_4_BASE_ADDR,      UNIT_SERDES_0_4         ,512  *     _1K)     /* 56G Quad SD Wrapper */
...
static MAC_NUM_INFO_EXT_STC harrier100GPortsArr[] = {
    ...
    ,{{16 ,           {1, 0} ,    {1,0 }       ,1    }   ,    {3,0/*SWAP*/}   ,   8}
    ,{{18 ,           {1, 1} ,    {1,2 }       ,1    }   ,    {3,1/*SWAP*/}   ,   9}
    ,{{20 ,           {1, 2} ,    {1,4 }       ,1    }   ,    {3,2/*SWAP*/}   ,  10}
    ,{{22 ,           {1, 3} ,    {1,6 }       ,1    }   ,    {3,3/*SWAP*/}   ,  11}
    ,{{24 ,           {1, 4} ,    {1,8 }       ,1    }   ,    {2,0/*SWAP*/}   ,  12}
    ,{{26 ,           {1, 5} ,    {1,10}       ,1    }   ,    {2,1/*SWAP*/}   ,  13}
    ,{{28 ,           {1, 6} ,    {1,12}       ,1    }   ,    {2,2/*SWAP*/}   ,  14}
    ,{{30 ,           {1, 7} ,    {1,14}       ,1    }   ,    {2,3/*SWAP*/}   ,  15}
    ...
    ,{{SMAIN_NOT_VALID_CNS,{0,0},{0,0},0},{0,0},SMAIN_NOT_VALID_CNS}
};
...
/**
* @internal smemHarrierGopPortByAddrGet_SERDES function
* @endinternal
*
* @brief   get SERDES info according to associated address (in SERDES unit).
*
* @param[in] devObjPtr                - pointer to device object.
* @param[in] address                  - the  in the  space of the port
*
* @param[out] unitIdPtr               - pointer to the unit index
* @param[out] localSerdesPtr          - pointer to the local lane index
*/
static void smemHarrierGopPortByAddrGet_SERDES
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_U32                   address,
    OUT GT_U32                  *unitIdPtr,
    OUT GT_U32                  *localSerdesPtr
)
{
    GT_U32  ii;
    GT_U32  unitSize = (512*_1K);
    GT_U32  offsetFromUnit;
    GT_U32  laneNumber;

    for(ii = 0 ; ii < devObjPtr->numOfSip6_10_SERDES_units ; ii++)
    {
        if(address < devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[ii] ||
           address >= devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[ii] + unitSize)
        {
            continue;
        }
        offsetFromUnit = address - devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[ii];
        if(offsetFromUnit >= 0x20 && offsetFromUnit < 0x80)
        {
            /*/Cider/EBU/Harrier/Harrier {Current}/Harrier/Core/EPI/<SDW>SDW %j/<SDW> SDW IP TLU/SDW IP Units/Configurations/Per-Lane Configurations*/
            laneNumber = (offsetFromUnit >> 2) & 0x3;/*0..3*/
        }
        else
        if(offsetFromUnit >= 0x220 && offsetFromUnit < 0x300)
        {
            /*/Cider/EBU/Harrier/Harrier {Current}/Harrier/Core/EPI/<SDW>SDW %j/<SDW> SDW IP TLU/SDW IP Units/Status/Per-Lane Status*/
            laneNumber = (offsetFromUnit >> 2) & 0x3;/*0..3*/
        }
        else
        {
            /* not per lane , but representative to the unit */
            laneNumber = 0;
        }

        *unitIdPtr = ii;
        *localSerdesPtr = laneNumber;

        /* make the lane number to be global MAC number */
        return ;
    }

    skernelFatalError("smemHarrierGopPortByAddrGet_SERDES : address [0x%8.8x] not part of the SERDES unit(s) \n",
        address);
    return;
}
...
)
{
    ...
    for (ii=0; ii<HARRIER_NUM_TSU_UNITS; ii++)
    {
            ...
            address <  (devObjPtr->memUnitBaseAddrInfo.tsu[ii] + (64 *_1K)))
        {
            ...
            {
                skernelFatalError("smemHarrierGopPortByAddrGet : unknown port for address [0x%8.8x] as 'TSU unit' (case 1)\n",address);
                return 0;
            }

            unitIndex = ii;

            for(ii = 0 ; harrier100GPortsArr[ii].genInfo.globalMacNum != SMAIN_NOT_VALID_CNS; ii++)
            {
                if(harrier100GPortsArr[ii].genInfo.channelInfo.unitIndex   == unitIndex &&
                   harrier100GPortsArr[ii].genInfo.channelInfo.indexInUnit == localPortInUnit)
                {
                    return harrier100GPortsArr[ii].genInfo.globalMacNum;
                }
            }
            return SMAIN_NOT_VALID_CNS;
        }
    ...
    for(ii = 0 ; harrier100GPortsArr[ii].genInfo.globalMacNum != SMAIN_NOT_VALID_CNS; ii++)
    {
        if(harrier100GPortsArr[ii].genInfo.macInfo.unitIndex   == unitIndex &&
           harrier100GPortsArr[ii].genInfo.macInfo.indexInUnit == localPortInUnit)
        {
            return harrier100GPortsArr[ii].genInfo.globalMacNum;
        }
    }
            ...
            ii++,tmpMacInfoPtr++)
        {
            ...
            switch(unitType)
            {
                ...
                case SMEM_UNIT_TYPE_EPI_MIF_CHANNEL_E:
                    {
                        /*100G mac */
                        portInfoPtr->simplePortInfo.unitIndex   = tmpMacInfoPtr->genInfo.macInfo.unitIndex;/*0..3*/ /*400G_MAC_INST*/
                        portInfoPtr->simplePortInfo.indexInUnit = tmpMacInfoPtr->genInfo.macInfo.indexInUnit;/*0..7*/
...
*
*/
void smemHarrierActiveRead_pm_ctrl_tx_lane_reg1_lane (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
/**
* @internal smemHarrierActiveRead_reset_core_ack_rx_tx function
* @endinternal
*
* @brief   Read reset_core_ack_rx/tx values.
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemHarrierActiveRead_reset_core_ack_rx_tx (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    /* output GT_TRUE value */
    *outMemPtr = (*memPtr) | (3<<14)/*[15:14]reset_core_ack_rx, reset_core_ack_tx*/;
...
*
*/
void smemHarrierActiveRead_pm_ctrl_rx_lane_reg1_lane (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
*
*/
void smemHarrierActiveRead_tx_train_if_reg3 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
/*******************************************************************************
*   smemHarrierLinkStateNotify_bySerdesInfo
*
* DESCRIPTION:
* @brief   Notify device that link state changed on SERDES with SERDES info
*
* INPUTS:
*       devObjPtr   - pointer to device object.
*       unitIndex   - the unit index of the SERDES
*       indexInUnit - the local lane index of the SERDES
...
*
*******************************************************************************/
GT_VOID smemHarrierLinkStateNotify_bySerdesInfo
(
    IN SKERNEL_DEVICE_OBJECT_T_PTR devObjPtr,
    IN GT_U32                  unitIndex,
    IN GT_U32                  indexInUnit,
    IN GT_U32                  linkState
)
{
    /* generate a serdes interrupt , in addition to the interrupt from the MAC */
    snetChetahDoInterrupt(devObjPtr,
      SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip_6_10_SERDES[unitIndex].sip_6_10_SERDES.SDWInterruptCause,
      SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip_6_10_SERDES[unitIndex].sip_6_10_SERDES.SDWInterruptMask,
      1 << (12 + indexInUnit),/* <sq_detect_0/1/2/3> */
      0/*not used*/);
}

/*******************************************************************************
*   smemHarrierLinkStateNotify
*
* DESCRIPTION:
* @brief   Notify device that link state changed
*
* INPUTS:
*       devObjPtr   - pointer to device object.
*       portNum     - the port num
...
*
*******************************************************************************/
GT_VOID smemHarrierLinkStateNotify
(
    IN SKERNEL_DEVICE_OBJECT_T_PTR devObjPtr,
    IN GT_U32                  portNum,
    IN GT_U32                  linkState
)
{
    ...
    /* in addition to what snetFalconLinkStateNotify doing */

    /* calling smemHarrierPortInfoGet */
    if(GT_OK != devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_EPI_MAC_TO_SERDES_E,portNum,&portInfo))
    {
        return;
    }
    unitIndex   = portInfo.simplePortInfo.unitIndex;
    indexInUnit = portInfo.simplePortInfo.indexInUnit;

    smemHarrierLinkStateNotify_bySerdesInfo(devObjPtr,unitIndex,indexInUnit,linkState);
}

/**
* @internal smemHarrierActiveWriteSerdesInterruptsMaskReg function
* @endinternal
*
* @brief   The application changed the value of the interrupts mask register.
*/
void smemHarrierActiveWriteSerdesInterruptsMaskReg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    smemHarrierGopPortByAddrGet_SERDES(devObjPtr,address,&unitIndex,&indexInUnit);

    /* Address of Port<n> interrupt cause register */
    dataArray[0] = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip_6_10_SERDES[unitIndex].sip_6_10_SERDES.SDWInterruptCause;
    /* Address of Port<n> table interrupt mask register */
    dataArray[1] = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip_6_10_SERDES[unitIndex].sip_6_10_SERDES.SDWInterruptMask;
    /* <sq_detect_0/1/2/3> */
    dataArray[2] = 1 << (12 + indexInUnit);
...
#if 0
/**
* @internal smemHarrierActiveWrite_ana_sq_reg0 function
* @endinternal
*
* @brief   The application write to the ana_sq_reg0 register
*/
void smemHarrierActiveWrite_ana_sq_reg0
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    GT_U32  orig_sq_cal_en_lane,new_sq_cal_en_lane;
    GT_U32  regAddr_CommonSystemControls,LANE_SEL;
    GT_U32 unitIndex,dummy;

    *memPtr = newValue;/* save value that written by the CPU*/

    orig_sq_cal_en_lane = (origValue & (1<<2)) ? 1 : 0;
    new_sq_cal_en_lane  = (newValue  & (1<<2)) ? 1 : 0;

    if(orig_sq_cal_en_lane == 1 && new_sq_cal_en_lane == 0)
    {
        /* CPU 'power down' the 'UNIT' SERDES , so need to re-enable the link status */
        /* as the SERDES was considered down , and it's interrupts cleared by the CPU
           without checking them */

        /*/Cider/EBU/Harrier/Harrier {Current}/Harrier/Core/EPI/<SDW>SDW %j/<COMPHY_56G_PIPE5_X4_4PLL_1_0> COMPHY_56G_PIPE5_X4_4PLL_1_2/units/Memory Map/Address Block/Common System Controls*/
        regAddr_CommonSystemControls = (address & 0xFFF00000) | 0x0002a318;
        smemRegFldGet(devObjPtr,regAddr_CommonSystemControls,28,4,&LANE_SEL);

        /* get unitIndex */
        smemHarrierGopPortByAddrGet_SERDES(devObjPtr,address,&unitIndex,&dummy/*dummy not needed*/);

        smemHarrierLinkStateNotify_bySerdesInfo(devObjPtr,unitIndex,LANE_SEL,0/*linkState*/);
    }
...
/**
* @internal smemHarrierActiveWrite_CDR_Lock function
* @endinternal
*
* @brief   The application write to the CDR Lock register
*/
void smemHarrierActiveWrite_CDR_Lock
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    /* Serdes driver performs the following seq:
        write 0x0 to 0x6430 (field [15..8])
        sleep
        write 0xFF to 0x6430 (field [15..8])
        sleep
        read 0x6430
        expected result 0x1 in field [15..8]
    */
    if(newValue == 0xFF00)
    {
        newValue = 1<<8;
...
/**
* @internal smemHarrierActiveWrite_Lane_Control_0 function
* @endinternal
*
* @brief   The application write to the Lane_Control_0 register
*/
void smemHarrierActiveWrite_Lane_Control_0
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    GT_U32  origValue = *memPtr;
    GT_U32  newValue  = *inMemPtr;
    GT_U32  orig_pu_pll,new_pu_pll;
    GT_U32 unitIndex,indexInUnit;

    *memPtr = newValue;/* save value that written by the CPU*/

    orig_pu_pll = (origValue & (1<<0)) ? 1 : 0;
    new_pu_pll  = (newValue  & (1<<0)) ? 1 : 0;

    *memPtr = newValue;/* save value that written by the CPU*/

    if(orig_pu_pll == 0 && new_pu_pll)
    {
        smemHarrierGopPortByAddrGet_SERDES(devObjPtr,address,&unitIndex,&indexInUnit);

        smemHarrierLinkStateNotify_bySerdesInfo(devObjPtr,unitIndex,indexInUnit,1/*linkState*/);
    }
}
...
#define SMEM_PER_LANE_MASK_CNS (0xFFFFFFF3)/* support 4 lanes */
    ...
    {0x00000304, SMEM_FULL_MASK_CNS, NULL, 0, smemHarrierActiveWriteSerdesInterruptsMaskReg, 0},
    /* DSW COMPHY 56 */
#if 0
    {0x000210F4, SMEM_FULL_MASK_CNS, NULL, 0 ,smemHarrierActiveWrite_ana_sq_reg0 , 0},
#endif /*0*/
    {0x00026430, SMEM_PER_LANE_MASK_CNS, NULL, 0 , smemHarrierActiveWrite_CDR_Lock, 0},

    /* bit 0 pu_pll for the logic in API_N5XC56GP5X4_SetPowerPLL(...) */
    {0x00000020, SMEM_PER_LANE_MASK_CNS, NULL, 0 , smemHarrierActiveWrite_Lane_Control_0, 0},

    /* bits 14, 15 reset_core_ack_tx/rx for the logic in mvHwsComphyN5XC56GP5X4SerdesArrayPowerCtrl(...) */
    {0x000002A0, SMEM_PER_LANE_MASK_CNS,smemHarrierActiveRead_reset_core_ack_rx_tx, 0 ,NULL,0},
...

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00026430, 0x00026434)}
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x0002E668, 0x0002E750)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x0002E774, 0x0002E774)}
...
)
{
    ...
    unitDbPtr->Global_SD_Mux_Control[0] = unitOffset + 0x08;/*0x00000008*/
    unitDbPtr->Global_SD_Mux_Control[1] = unitOffset + 0x0c;/*0x0000000c*/
}
...
)
{
    ...
    if(indexInUnit == 0)
    {
        unitDbPtr->SDWInterruptCause = unitOffset  + 0x300; /*0x00000300*/
        unitDbPtr->SDWInterruptMask  = unitOffset  + 0x304; /*0x00000304*/
    }
    else
    {
        /* the DB not hold any per port (lane) registers */
    }
}
...
)
{
    ...
    for(ii = 0 ; harrier100GPortsArr[ii].genInfo.globalMacNum != SMAIN_NOT_VALID_CNS; ii++)
    {
        regAddr_serdes(devObjPtr,
            harrier100GPortsArr[ii].serdesInfo.unitIndex,
            harrier100GPortsArr[ii].serdesInfo.indexInUnit);
    }
...
)
{
    ...
    if (isHarrier == GT_TRUE)
    {
        ...
        devObjPtr->devMemSerdesLinkChangedPtr = smemHarrierLinkStateNotify;
        ...
        devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[0] = SERDES_0_BASE_ADDR;
        devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[1] = SERDES_1_BASE_ADDR;
        devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[2] = SERDES_2_BASE_ADDR;
        devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[3] = SERDES_3_BASE_ADDR;
        devObjPtr->memUnitBaseAddrInfo.sip_6_10_SERDES[4] = SERDES_4_BASE_ADDR;
        devObjPtr->numOfSip6_10_SERDES_units = 5;
...
{
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00096000, 512), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(17, 4),SMEM_BIND_TABLE_MAC(egfQagPortTargetAttribute)}
