...
/* NOTE : the SFDB_MAC_TBL_ACT_WORDS need to cover the mac table action
   ...
   SIP7 need 9 words from the memory:
   address 0x00000020 .. 0x00000040

*/
#define SFDB_MAC_TBL_ACT_WORDS  9
...
)
{
    ...
    WM_SPECIFIC_DEVICE_INFO_STC devInfo;
    GT_U32  deviceId_regAddr,deviceId;


    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        deviceId_regAddr = SMEM_SIP7_DEVICE_ID_REG(devObjPtr);
    }
    else
    {
        deviceId_regAddr = SMEM_CHT_DEVICE_ID_REG(devObjPtr);
    }

    /* check if need to override values from the registers file */
    if(GT_OK == scibSpecificDeviceInfoGet(devObjPtr->deviceId,&devInfo))
    {
        smemRegGet(devObjPtr, deviceId_regAddr, &data);
        if(!SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            devRev = data & 0xF;
            deviceId = (data >> 4) & 0xFFFF;

            if(devInfo.revision <= 0xF)
            {
                /* override value from the registers file */
                devRev = devInfo.revision;
            }
            if(devInfo.pciDeviceId != 0xFFFF)
            {
                /* override value from the registers file */
                deviceId = devInfo.pciDeviceId;
            }

            data &= 0xFFF00000;/* remove deviceId , revision */
            data |= (deviceId << 4) | devRev;
        }
        else /*SMEM_CHT_IS_SIP7_GET(devObjPtr)*/
        {
            smemRegGet(devObjPtr, deviceId_regAddr, &data);

            deviceId = data & 0xFFFF;
            ...
            data &= 0xFFFF0000;/* remove deviceId , revision */
            data |= deviceId;
        }
        /* write the register without active memories */
        smemRegSet(devObjPtr, deviceId_regAddr, data);
        /* so now when we read SMEM_CHT_DEVICE_ID_REG(devObjPtr) , we will get
           updated  deviceId , revision
        */
    }

    /* Get the 'devId' and 'revision'  */
    scibReadMemory(devObjPtr->deviceId, deviceId_regAddr, 1, &data);
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        devRev = 0;/*TBD*/
    }
    ...
    if((GT_U32)devObjPtr->deviceType > SMEM_CHT_MARVELL_VENDOR_ID_CNS)
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            SMEM_U32_SET_FIELD(data, 0, 16, pciDeviceAndVendorIdRegisterVal);
        }
        else
        {
    ...
    else /* support 'wildcard' devId */
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            pciDeviceAndVendorIdRegisterVal = SMEM_U32_GET_FIELD(data, 0, 16);
        }
        else
        {
        ...
        if(pciDeviceAndVendorIdRegisterVal == 0)
        {
            ...
            {
                /* Device ID Register */
                SMEM_U32_SET_FIELD(data, 0, 16, pciDeviceAndVendorIdRegisterVal >> 16);
            }
            else
            {
                /* Device ID Register */
    ...
    scibWriteMemory(devObjPtr->deviceId, deviceId_regAddr, 1, &data);
...
)
{
    ...
    GT_U32  index,ii,maxIndex;/*iterator*/
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        smemSip6DfxServerOwnDevNum(devObjPtr,GT_FALSE,&ownDev);
    }
    else
    {
    ...
    for(physicalPort = 0 ; physicalPort < maxPhysicalPort ; physicalPort++)
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            memPtr = smemMemGet(devObjPtr,SMEM_SIP7_HA_INGRESS_PHYSICAL_PORT_2_ATTRIBUTES_TBL_MEM(devObjPtr, physicalPort));
            /* init the HA source physical port 2 table */
            snetFieldFromEntry_GT_U32_Set(devObjPtr,
                memPtr,
                devObjPtr->tableFormatInfo[SKERNEL_TABLE_FORMAT_HA_SRC_PHYSICAL_PORT_2_E].formatNamePtr,
                physicalPort,/* the physical port - as src port */
                devObjPtr->tableFormatInfo[SKERNEL_TABLE_FORMAT_HA_SRC_PHYSICAL_PORT_2_E].fieldsInfoPtr,
                devObjPtr->tableFormatInfo[SKERNEL_TABLE_FORMAT_HA_SRC_PHYSICAL_PORT_2_E].fieldsNamePtr,
        ...
        else
        {
            ...
            snetFieldFromEntry_GT_U32_Set(devObjPtr,
                ...
                physicalPort,/* the physical port - as trg port */
    ...
    for(port = 0 ; port < maxPort ; port++)
    {
        ...
        /* Default changed from 6.30 */
        if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
        {
            lion3IpvxLocalDevSrcEportBitsSet(devObjPtr, port, IPVX_PER_SRC_PORT_FIELD_DISABLE_SIP_LOOKUP_E, 0x1);
        }
        ...
        if(0 ==(port % 4))
        {
            ...
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                snetFieldValueSet(memPtr,0,32,0xC0F03C0F);
                snetFieldValueSet(memPtr,32,8,0x3);
            }
            else if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
            {
    ...
    maxIndex = devObjPtr->limitedResources.l2Ecmp ?
               devObjPtr->limitedResources.l2Ecmp :
               devObjPtr->limitedResources.ePort;
    /* loop on l2ECMP : set 'NULL port' on all members */
    for(index = 0 ; index < maxIndex; index++)
    {
        memPtr = smemMemGet(devObjPtr,SMEM_LION2_EQ_L2_ECMP_TBL_MEM(devObjPtr, index));
        *memPtr = 0x0000f800;
    }

    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        maxIndex = devObjPtr->limitedResources.trunkEcmp;
        /* loop on TRUNK_ECMP : set 'NULL port' on all members */
        for(index = 0 ; index < maxIndex; index++)
        {
            memPtr = smemMemGet(devObjPtr,SMEM_SIP7_EQ_TRUNK_ECMP_TBL_MEM(devObjPtr, index));
            *memPtr = 0x0000f800;
        }
    }
    ...
    */
        ...
        smemIsDeviceMemoryOwner(devObjPtr,SMEM_XCAT_L2_PORT_ISOLATION_REG(devObjPtr, 0)))
    {
        GT_U32  portIsolationNum =
                devObjPtr->limitedResources.portIsolationNum ? devObjPtr->limitedResources.portIsolationNum :
                (2*_1K + 128) ;/*default for devices before sip6_10*/
    ...
    else
    {
        ...
        for (index=0;index < devObjPtr->ipvxEcmpIndirectMaxNumEntries;index++)
        {
            ...
            for( ii = 0 ; ii < 4 ; ii++)
            {
                /* access IPvX ECMP indirect table : default 1:1 ... */
                if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
                    snetFieldValueSet(&memPtr[0],ii*18, 18 , index*4 + ii);
                else
    ...
    if(devObjPtr->limitedResources.smuSgcTimeSlotAttributesNum)/* SMU SGC Time Slote Attributes table */
    {
        for(index = 0 ; index < devObjPtr->limitedResources.smuSgcTimeSlotAttributesNum ; index++)
        {
            memPtr = smemMemGet(devObjPtr,
                SMEM_SIP6_30_SMU_SGC_TIME_SLOT_ATTRIBUTES_TBL_MEM(devObjPtr,index));

            SNET_TABLE_ENTRY_FIELD_SET(devObjPtr,memPtr,index,
                        SMEM_SIP6_30_SMU_SGC_TIME_SLOT_ATTRIBUTES_TABLE_FIELDS_IMX_PROFILE_0_E,
                        0xFF,
                        SKERNEL_TABLE_FORMAT_SMU_SGC_TIME_SLOT_ATTRIBUTES_E);
...
)
{
    ...
    if(devObjPtr->devMemUnitNameAndIndexPtr == NULL || /* support xcat3/ac5/lion2*/
       SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_MG))
    {
...
)
{
    ...
    *internalMemPtr = 1;
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        /* init default for statistical ratio for SIP_6 devices to be 1 */
        internalMemPtr = CHT_INTERNAL_MEM_PTR(devObjPtr,
            CHT_INTERNAL_SIMULATION_USE_MEM_RX_STATISTICAL_SNIFF_RATIO_ANALYZER_1_E);
        for(ii = 0 ; ii < 7 ; ii++)
        {
            *internalMemPtr = 1;
            internalMemPtr++;
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        numOfWordsToLoop = SIP5_MAC_UPD_FIFO_REGS_NUM;
    }
    else
    {
        numOfWordsToLoop = SIP4_MAC_UPD_FIFO_REGS_NUM;
    }
    ...
    for(ii = 0 ; ii < numOfTiles ; ii++)
    {
        memset(commonDevMemInfoPtr->auqFifoMem[ii].macUpdFifoRegs, 0xFF,
               numOfWordsToLoop * sizeof(SMEM_REGISTER));
    }
...
/**
* @internal smemChtActiveReadCntrs64Bit_low32 function
* @endinternal
*
* @brief   Read from low 32 bits of 64 bits counter - the active 'ROC' happens on both registers (high,low)
*         NOTE: the function support 64 bits counter that is NOT in address x , x+4.
*               it supports general x , x+offset (x+param)
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
* @param[in] param                    - Registers' specific parameter -
*                                       offset in bytes to the 'high register'
*
* @param[out] outMemPtr                - Pointer to the memory to copy register's content.
*/
void smemChtActiveReadCntrs64Bit_high32 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    GT_U32  byteOffset = (GT_U32)param;

    if((byteOffset & 0x3) || (0 == byteOffset))
    {
        skernelFatalError("smemChtActiveReadCntrs64Bit_high32: param must be in steps of 0x4 bytes (0x4,0x8,0xc,0x10...) not [0x%x] (and NON-zero) \n",
            byteOffset);
    }
    ...
    * outMemPtr = * memPtr;
    ...
    /* reset the 'high' register */
    memPtr[0] = 0;
    /* reset the 'low' register */
    smemRegSet(devObjPtr,address - byteOffset,0);
}
...
/**
* @internal smemChtActiveReadEntryWithSnapShot function
* @endinternal
*
* @brief   Read table entry , that need to generate 'snap shot' of the entry ,
*          so all it's words are coherent to the moment of read of the first word in line.
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
* @param[out] outMemPtr                - Pointer to the memory to copy register's content.
*
* @note if 'clear' after this read needed then see the use of function :
*       smemChtPostStartSnapshotWriteClearEntry
*
*/
void smemChtActiveReadEntryWithSnapShot (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    SMEM_CHT_READ_ENTRY_SNAP_SHOT_INFO_STC *snapShotInfoPtr = (SMEM_CHT_READ_ENTRY_SNAP_SHOT_INFO_STC *)param;
    GT_U32  ii;
    GT_U32  entryAlignmentInWords;
    GT_U32 *internalDataPtr = smemTableInternalDataPtrGet(devObjPtr,address,&entryAlignmentInWords);
    GT_U32  wordIndex = (address % (entryAlignmentInWords * 4)) / 4;

    if(wordIndex == 0)
    {
        /* output actual value of word 0 */
        * outMemPtr = * memPtr;

        /* on the first word we trigger the read of the 'rest of the' entry , and save it */
        for(ii = 1 ; ii < entryAlignmentInWords; ii++)
        {
            internalDataPtr[ii] = memPtr[ii];
        }

        /* check if we have call back to the 'post' start of triggering the snapshot
           this function in prototype of 'active write' */
        if(snapShotInfoPtr && snapShotInfoPtr->postStartSnapshotWriteFunc)
        {
            SMEM_ACTIVE_MEM_WRITE_FUN func = (SMEM_ACTIVE_MEM_WRITE_FUN)snapShotInfoPtr->postStartSnapshotWriteFunc;
            func(devObjPtr,address,memSize,memPtr,param,outMemPtr);
        }
    }
    else
    {
        /* on the other words we read the data from the 'saved' buffer */
        * outMemPtr = internalDataPtr[wordIndex];
    }
}

/**
* @internal smemChtPostStartSnapshotWriteClearEntry function
* @endinternal
*
* @brief   The read of entry (probably with counters) was triggered , and this function
*          need to do the clear after the read.
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
* @param[in] inMemPtr                 - Pointer to the memory to get register's content.
*/
void smemChtPostStartSnapshotWriteClearEntry(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
)
{
    ...
    (void)smemTableInternalDataPtrGet(devObjPtr,address,&entryAlignmentInWords);
    wordIndex = (address % (entryAlignmentInWords * 4)) / 4;

    if(wordIndex == 0)
    {
        for(ii = 0 ; ii < entryAlignmentInWords; ii++)
        {
            /* clear after the read that was done in smemChtActiveReadEntryWithSnapShot */
            memPtr[ii] = 0;
        }
    }
}
...
)
{
    ...
    regValPtr = 0;
    devMemInfoPtr = (SMEM_CHT_GENERIC_DEV_MEM_INFO  *)devObjPtr->deviceMemory;
...
{
    CHT_AUQ_MEM  * auqMemPtr;     /* pointer to AUQ state DB */
    GT_U32         auqSize;       /* AUQ size */
    GT_U32       * fifoBufferPrt; /* pointer to FDB AU messages FIFO buffer */
    GT_U32         i, k;          /* iterators */
    GT_U32         numOfWordsToMove;
    GT_U32         numOfWordsToLoop;
    GT_BOOL        status;
    ...
    *memPtr = *inMemPtr;
    ...
    else
    {
        ...
        else
        {
            ...
            /* check internal FIFO and perform DMA for stored messages */

            /* get pointer to the start of FIFO for MAC update messages */
            fifoBufferPrt = SMEM_CHT_MAC_AUQ_FIFO_MEM_GET(devObjPtr)->macUpdFifoRegs;
            ...
            for (i = 0; i < numOfWordsToLoop; i+= devObjPtr->numOfWordsInAuMsg)
            {
                if (*fifoBufferPrt == SMEM_CHEETAH_INVALID_MAC_MSG_CNS)
                {
                    break;
                ...
                /* send message from FIFO to DMA */
                status = snetChtL2iPciAuMsg(devObjPtr, fifoBufferPrt, GT_TRUE , devObjPtr->numOfWordsInAuMsg);
                if (status == GT_FALSE)
                {
                    break;
                }

                /* move forward rest of messages */
                numOfWordsToMove = numOfWordsToLoop - devObjPtr->numOfWordsInAuMsg;
                for (k = 0; k < numOfWordsToMove; k++)
                {
                    fifoBufferPrt[k] = fifoBufferPrt[k + devObjPtr->numOfWordsInAuMsg];
                }

                /* invalidate last message */
                for (/*continue*/; k < (numOfWordsToMove + devObjPtr->numOfWordsInAuMsg); k++)
                {
                    fifoBufferPrt[k] = SMEM_CHEETAH_INVALID_MAC_MSG_CNS;
                }
            }
        }
    }
...
/**
* @internal smemChtActiveAuqControlWrite function
* @endinternal
*
* @brief   Address Update Queue Control active write.
*           active memory for FUQ too !
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
* @param[in] inMemPtr                 - Pointer to the memory to get register's content.
*/
void smemChtActiveAuqControlWrite
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
{
    /* bit 30 is read only  */
    GT_U32  readOnly_mask = (1<<30);/* <GeneralAUQFull/FUAUQFull> : SMEM_CHT_AUQ_CTRL_REG(devObjPtr) */
    GT_U32  readOnly_value;

    readOnly_value = (*memPtr) & readOnly_mask;
    *memPtr = ((*inMemPtr) & ~readOnly_mask) | readOnly_value;
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    if (bufferId == NULL) /* code copied from smemChtActiveWriteInterruptsMaskReg */
    {
        ...
        do
        {
            ...
            {
                simGeneralPrintf(" smemChtActiveWriteFdbActionTrigger: managed to trigger aging after [%d] retries \n",
                    1+(20-retry_allowed));
            }
        }
    ...
    if (bufferId == NULL)
    {
        simWarningPrintf(" smemChtActiveWriteFdbActionTrigger: no buffers for trigger aging\n");
        return;
    }
    /* Get actual data pointer */
    sbufDataGet(bufferId, &dataPtr, &dataSize);
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    supportSoftReset = SMEM_CHT_IS_SIP5_GET(devObjPtr) ? 0 :/* sip5 --> DFX server */
                       SKERNEL_IS_LION2_DEV(devObjPtr) ? 0 :/* lion2--> DFX server */
                       1;
...
{
    GT_U32 dataArray[3];                     /* Data array to put into buffer */
    GT_U32 treeId = snetChtInterruptGetTreeId(devPtr,address);

    /* Address of SDMA Interrupt Registers */
    dataArray[0] = SMEM_CHT_SDMA_INT_CAUSE_REG(devPtr,treeId);
    /* Address of SDMA Interrupt Mask Register */
    dataArray[1] = SMEM_CHT_SDMA_INT_MASK_REG(devPtr,treeId);
...
)
{
    ...
    dataArray[0] = SMEM_CHT_TX_SDMA_INT_CAUSE_REG(devPtr,treeId);
    /* Address of transmit SDMA interrupt mask register */
    dataArray[1] = SMEM_CHT_TX_SDMA_INT_MASK_REG(devPtr,treeId);
    /* Receive SDMA Interrupt Cause Register Summery Bit */
...
{
    GT_U32 dataArray[3];                     /* Data array to put into buffer */
    GT_U32 treeId = snetChtInterruptGetTreeId(devObjPtr,address);

    /* Address of miscellaneous interrupt cause register */
    dataArray[0] = SMEM_CHT_MISC_INTR_CAUSE_REG(devObjPtr,treeId);
    /* Address of miscellaneous  interrupt mask register */
    dataArray[1] = SMEM_CHT_MISC_INTR_MASK_REG(devObjPtr,treeId);
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    /* Get buffer      */
    bufferId = sbufAlloc(deviceObjPtr->bufPool, 50/*SMI_TRANSACTION_SIZE*/);
    if (bufferId == NULL)
    {
...
)
{
    ...
    devObjPtr->tablesInfo.fdbTable.commonInfo.baseAddress = 0x0D800000;
    devObjPtr->tablesInfo.fdbTable.paramInfo[0].step = 0x10;
    ...
    devObjPtr->tablesInfo.arpTable.commonInfo.baseAddress = 0x0F800000;
    devObjPtr->tablesInfo.arpTable.paramInfo[0].step = 0x8;
...
)
{
    ...
    */
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(tunnelStartTable);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(mllTable);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqTrunkEcmp);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(egfQueueMapTable);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(lpmCommonVrfId);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(tmQueueMapQueueSelector);
    TABLES_NAME_TO_STRING_CONVERT_MAC(tmQueueMapTargetDeviceToIndex);
    TABLES_NAME_TO_STRING_CONVERT_MAC(tmQueueMapCpuCodeToTc);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(smuSgcIntervalMax);
    TABLES_NAME_TO_STRING_CONVERT_MAC(smuSgcOctetCounters);
    TABLES_NAME_TO_STRING_CONVERT_MAC(smuSgcTableSetTimeConfigurations);
    TABLES_NAME_TO_STRING_CONVERT_MAC(smuSgcTimeSlotAttributes);
    TABLES_NAME_TO_STRING_CONVERT_MAC(smuSgcTimeToAdvance);
    ...
    /*SIP7 MPCL*/
    TABLES_NAME_TO_STRING_CONVERT_MAC(mpclExactMatchProfileIdMapping);
    TABLES_NAME_TO_STRING_CONVERT_MAC(mpclConfigTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(mpclSourcePortCfgTable);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqQueueGroupProfile);
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqVPortMapping);
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqHEcmpStatusVector);
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqHEcmpLttTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(eqHEcmpEcmpTable);

    TABLES_NAME_TO_STRING_CONVERT_MAC(gdmaDispatcherCpuCodeToGdmaQ);

    /* sip7 HA */
    TABLES_NAME_TO_STRING_CONVERT_MAC(haIngressEPortAttr1);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haIngressEPortAttr2);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haTargetCircuits);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haQueueMapping);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haIngressPhyPort2);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haAnalyzersTargetCircuits);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haQosMapping);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haHeaderFieldsTable0);
    TABLES_NAME_TO_STRING_CONVERT_MAC(haHeaderFieldsTable1);

    /* sip7 IPVX */
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipvxL3NheProfile);

    /* sip7 PPU for IPE */
    TABLES_NAME_TO_STRING_CONVERT_MAC(ppuForIpeActionTable0);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ppuForIpeActionTable1);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ppuForIpeActionTable2);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ppuForIpeActionTable3);

    /* sip7 IPE */
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeStateAddrProfile);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeIctTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeStateDataCmdProfile);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeDpoTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeDpiTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeStateT0Table);
    TABLES_NAME_TO_STRING_CONVERT_MAC(ipeStateT1Table);

    TABLES_NAME_TO_STRING_CONVERT_MAC(hbuPortMappingTable);

    /* Sip7 Reduced EM table */
    TABLES_NAME_TO_STRING_CONVERT_MAC(reducedEmTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(egfEftEVlanAttributesTable);
    TABLES_NAME_TO_STRING_CONVERT_MAC(egfEftTrgDevToHashBlockSizeMappingTable);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(lpmMemorySingleBank);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(tcamGroup5ProfileSubkeySizeAndMux);
    TABLES_NAME_TO_STRING_CONVERT_MAC(tcamGroups0_5SubkeySharedMux);
    ...
    TABLES_NAME_TO_STRING_CONVERT_MAC(tcamGroups0_4ProfileSubkeySizeAndMux);
    TABLES_NAME_TO_STRING_CONVERT_MAC(tcamGroups0_5SubkeyMux2byteUnits);
    TABLES_NAME_TO_STRING_CONVERT_MAC(txqSdqSip7QueueToPortMap);
...
*           the function is only for NON AC3/5 and non sip5/6
*/
...
static SUPPORTED_CORE_CLOCKS_INFO_STC supportedCoreClocksDb[]=
{
    ...
    {SKERNEL_HARRIER_FAMILY,  0,  SCIB_MEM_ACCESS_DFX_E ,
        SMAIN_NOT_VALID_CNS,18,3 , 800 , {800, 825, 750, 600, 300, 200, 100, 12} },

    /*SKERNEL_IRONMAN_FAMILY*/ /*currently values from Hawk */
    {SKERNEL_IRONMAN_FAMILY,  0,  SCIB_MEM_ACCESS_DFX_E ,
        SMAIN_NOT_VALID_CNS,16,2 , 600 , {600, 600, 600, 25} },

    /*SKERNEL_AAS_FAMILY*/
    {SKERNEL_AAS_FAMILY,  0,  SCIB_MEM_ACCESS_DFX_E ,
        SMAIN_NOT_VALID_CNS,16,2 , 1300 , {1300, 1300, 1300, 25} },
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        {
            if(devObjPtr->fdbNoDirectMemory)
            {
                /* Get FDB entry */
            ...
            {
                value2 = 1; /* <skip> relevant to the entry type */
                if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
                {
                    switch(SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,memPtr,
                        ii,
                        SMEM_SIP6_FDB_FDB_TABLE_FIELDS_FDB_ENTRY_TYPE))
                    {
                        ...
                        case SNET_CHEETAH_FDB_ENTRY_IPV6_UC_ROUTING_DATA_E:/*Unicast routing IPv6 address data entry.*/
                        case SNET_CHEETAH_FDB_ENTRY_FCOE_UC_ROUTING_E     :/*Unicast routing FCOE address entry.*/
                            value2 = 0;/* <skip> not relevant to the entry type */
                            break;
                        default:
                            break;
                    }
                ...
                {
                    skip = 0;/* not relevant to the entry type */
                }
            ...
            if(SNET_CHEETAH_FDB_ENTRY_IPV6_IPMC_BRIDGING_E >=
                SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,memPtr,
                ii,
                SMEM_SIP6_FDB_FDB_TABLE_FIELDS_FDB_ENTRY_TYPE))
            {
...
* @param[in] dupUnitName              - name of the duplicated unit
*
    ...
    *dupUnitChunkPtr = *origUnitChunkPtr;
    ...
    for(ii = 0 ; ii < dupUnitChunkPtr->numOfChunks ; ii++,dupChunkPtr++,origChunkPtr++)
    {
        ...
        *dupChunkPtr = *origChunkPtr;
        ...
        /* handle dynamic allocations */
        if (!dupChunkPtr->isAllocationofMemoryShared)
        {
            dupChunkPtr->memPtr =
        ...
        {
            char *cptr = dupChunkPtr->sharedMemFilename;

            if (cptr != NULL)
            {
                /* Make a unique, Process ID based name out of
                   the duplicated unit name, and create a shared
                   memory file name out of it: */
                if (smemInitSharedFileUniqueName(devObjPtr,
                                                 dupUnitName,
                                                 &dupChunkPtr->sharedMemFilename) != GT_OK)
                    skernelFatalError("smemCheetahDuplicateSingleUnit: cannot make unique filename out of %s!\n", dupUnitName);
                dupChunkPtr->memPtr = smemDeviceObjMemoryAlloc__shared(dupChunkPtr->memSize , sizeof(SMEM_REGISTER), dupChunkPtr->sharedMemFilename, &dupChunkPtr->fdSharedMem);
            }
        }
...
)
{
    ...
    while(currentPtr->unitNameStr != NULL)
    {
        ...
        for(ii = 0 ; ii < numOfUnits ; ii++ , currentPtr++)
        {
            ...
            smemCheetahDuplicateSingleUnit(devObjPtr,origUnitChunkPtr,currUnitChunkPtr,currentUnitBaseAddr, name_b);
...
)
{
    ...
    GT_U32 txMaxNum_aCBFCPAUSEFramesTransmitted = 16;
    GT_U32 rxMaxNum_aCBFCPAUSEFramesTransmitted = 16;
    GT_U32 offset;
    ...
    if(devObjPtr->numOfRavens == 0) /* Hawk */
    {
        ...
        {
            rx_portFormula = (35 * portIndex * 0x4);
            tx_portFormula = ((26+35) * portIndex * 0x4);
            txMaxNum_aCBFCPAUSEFramesTransmitted = 8;
            rxMaxNum_aCBFCPAUSEFramesTransmitted = 8;
            if(portNum < 24)
            {
                tx_portFormula_additional = 0x9c0;
            }
            else if(portNum < 48)
            {
                tx_portFormula_additional = 0x560;
            }
            else if(portNum < 52)
            {
                tx_portFormula_additional = 0x218;
            }
            else
            {
                tx_portFormula_additional = 0x18c;
            }

        }
        else
        {
            rx_portFormula = (CHT_INTERNAL_SIMULATION_RX_STATISTIC_PAGE * portIndex * 0x4);
            tx_portFormula = (CHT_INTERNAL_SIMULATION_TX_STATISTIC_PAGE * portIndex * 0x4);
        }
...
doPreemptionInit_lbl:
    ...
    for (ii = 0; ii < rxMaxNum_aCBFCPAUSEFramesTransmitted; ii++)
    {
        portStatsPtr->RX_aCBFCPAUSEFramesReceived[ii]          = 0x00000168 + 0x4 * ii + portFormula; /* 0x00450168 */
    }
    portStatsPtr->RX_aMACControlFramesReceived                 = portStatsPtr->RX_aCBFCPAUSEFramesReceived[ii-1] + 4; /* 0x004501A8 */
    ...
    portStatsPtr->TX_etherStatsOctets                          = offset + 0x00000000 + portFormula; /* 0x00450660 */
    portStatsPtr->TX_OctetsTransmittedOK                       = offset + 0x00000004 + portFormula; /* 0x00450664 */
    portStatsPtr->TX_aPauseMacCtrlFramesTransmitted            = offset + 0x00000008 + portFormula; /* 0x00450668 */
    portStatsPtr->TX_aFramesTransmittedOK                      = offset + 0x0000000C + portFormula; /* 0x0045066C */
    portStatsPtr->TX_VLANTransmittedOK                         = offset + 0x00000010 + portFormula; /* 0x00450670 */
    portStatsPtr->TX_ifOutErrors                               = offset + 0x00000014 + portFormula; /* 0x00450674 */
    portStatsPtr->TX_ifOutUcastPkts                            = offset + 0x00000018 + portFormula; /* 0x00450678 */
    portStatsPtr->TX_ifOutMulticastPkts                        = offset + 0x0000001C + portFormula; /* 0x0045067C */
    portStatsPtr->TX_ifOutBroadcastPkts                        = offset + 0x00000020 + portFormula; /* 0x00450680 */
    portStatsPtr->TX_etherStatsPkts64Octets                    = offset + 0x00000024 + portFormula; /* 0x00450684 */
    portStatsPtr->TX_etherStatsPkts65to127Octets               = offset + 0x00000028 + portFormula; /* 0x00450688 */
    portStatsPtr->TX_etherStatsPkts128to255Octets              = offset + 0x0000002C + portFormula; /* 0x0045068C */
    portStatsPtr->TX_etherStatsPkts256to511Octets              = offset + 0x00000030 + portFormula; /* 0x00450690 */
    portStatsPtr->TX_etherStatsPkts512to1023Octets             = offset + 0x00000034 + portFormula; /* 0x00450694 */
    portStatsPtr->TX_etherStatsPkts1024to1518Octets            = offset + 0x00000038 + portFormula; /* 0x00450698 */
    portStatsPtr->TX_etherStatsPkts1519toMaxOctets             = offset + 0x0000003C + portFormula; /* 0x0045069C */
    for (ii = 0; ii < txMaxNum_aCBFCPAUSEFramesTransmitted; ii++)
    {
        portStatsPtr->TX_aCBFCPAUSEFramesTransmitted[ii]       = offset + 0x00000040 + 0x4 * ii + portFormula; /*0x004506A0*/
    }

    portStatsPtr->TX_aMACControlFramesTransmitted              = portStatsPtr->TX_aCBFCPAUSEFramesTransmitted[ii-1] + 4;
    portStatsPtr->TX_etherStatsPkts                            = portStatsPtr->TX_aCBFCPAUSEFramesTransmitted[ii-1] + 8;
    ...
    if(portSupportDualMacMode == 1)
    {
        ...
        if(devObjPtr->deviceFamily == SKERNEL_IRONMAN_FAMILY)
        {
            rx_portFormula = portBaseAddr + (35 * portIndex * 0x4);
            tx_portFormula = portBaseAddr + ((26+35) * portIndex * 0x4);
        }
        else
        {
            rx_portFormula = portBaseAddr + (CHT_INTERNAL_SIMULATION_RX_STATISTIC_PAGE * portIndex * 0x4);
            tx_portFormula = portBaseAddr + (CHT_INTERNAL_SIMULATION_TX_STATISTIC_PAGE * portIndex * 0x4);
        }
...
#define PORT_FORMULA(offset)    ((offset) + CHANNEL_FORMULA)
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].MTI_EXT_PORT.portControl = 0x00478084  + portFormula;
...
)
{
    ...
    GT_U32  isSip7    = SMEM_CHT_IS_SIP7_GET(devObjPtr);
    ...
    GT_U32  mllRegBaseOffset;

    if (isSip7)
    {
        mllRegBaseOffset = 0x00460000;
    }
    else
    {
        mllRegBaseOffset = 0;
    ...
    {/*start of unit TTI */     /* manually added : TTI_Internal */
        ...
        {/*start of unit TTIUnitGlobalConfigs */
            ...
            if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt4       = 0x000001F0;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.TTIUnitGlobalConfigs.TTIUnitGlobalConfigExt5       = 0x000001F4;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.TTIUnitGlobalConfigs.TTIEliQosAttributes           = 0x000000B0;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.TTIUnitGlobalConfigs.TTIExplicitNullQosAttributes  = 0x000000B4;
            }
        ...
        if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
        {/*start of unit IP2ME */
            GT_U32      nMax = devObjPtr->limitedResources.ipToMeNum - 1;
            {/*4000+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEDip0 = 0x4000 + n*0x18;
                }/* end of loop n */
            }/*4000+n*0x18*/
            {/*4004+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEDip1 = 0x4004 + n*0x18;
                }/* end of loop n */
            }/*4004+n*0x18*/
            {/*4008+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEDip2 = 0x4008 + n*0x18;
                }/* end of loop n */
            }/*4008+n*0x18*/
            {/*400c+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEDip3 = 0x400c + n*0x18;
                }/* end of loop n */
            }/*400c+n*0x18*/
            {/*4010+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEConf0 = 0x4010 + n*0x18;
                }/* end of loop n */
            }/*4010+n*0x18*/
            {/*4014+n*0x18*/
                GT_U32    n;
                for(n = 0 ; n <= nMax ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.IP2ME[n].IP2MEConf1 = 0x4014 + n*0x18;
                }/* end of loop n */
            }/*4014+n*0x18*/
        }/*end of unit IP2ME */
        ...
        if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.exp4qosDebugCounter        = 0x0000344;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.exp4egressDebugCounter     = 0x0000348;

            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.ttiRangeCheckGlobalConfig0 = 0x0000400;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.ttiRangeCheckGlobalConfig1 = 0x0000404;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.pclRangeCheckGlobalConfig0 = 0x0000408;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TTI.pclRangeCheckGlobalConfig1 = 0x000040C;
        }
    }/*end of unit TTI */
    ...
    {/*start of unit L2I */
        ...
        {/*start of unit securityBreachStatus */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.securityBreachStatus.securityBreachStatus0 = 0x00001400 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.securityBreachStatus.securityBreachStatus1 = 0x00001404 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.securityBreachStatus.securityBreachStatus2 = 0x00001408 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.securityBreachStatus.securityBreachStatus3 = 0x0000140c + (isSip7 ? 0x200 : 0);
            ...
            {/*1544+n*0x20*/
                ...
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.setBridgeFilteredPktCount[n] =
                        0x1544+n*0x20 + (isSip7 ? 0x200 : 0);
                }/* end of loop n */
            }/*1544+n*0x20*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.MACAddrCount0 = 0x00001500 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.MACAddrCount1 = 0x00001504 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.MACAddrCount2 = 0x00001508 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.hostOutgoingMcPktCount = 0x00001520 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.hostOutgoingPktsCount = 0x00001514 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.hostIncomingPktsCount = 0x00001510 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.hostOutgoingBcPktCount = 0x00001524 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.layer2BridgeMIBCntrs.matrixSourceDestinationPktCount = 0x00001528 + (isSip7 ? 0x200 : 0);
        ...
        {/*start of unit bridgeEngineInterrupts */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineInterrupts.bridgeInterruptCause = 0x00002100 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineInterrupts.bridgeInterruptMask = 0x00002104 + (isSip7 ? 0x200 : 0);
        ...
        {/*start of unit bridgeEngineConfig */
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeCommandConfig0 = 0x00000020 + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeCommandConfig1 = 0x00000024 + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeCommandConfig2 = 0x00000028 + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeCommandConfig3 = 0x0000002c + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeSecurityBreachDropCntrCfg0 = 0x00000040 + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeSecurityBreachDropCntrCfg1 = 0x00000044 + (isSip7 ? 0x10 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgecpuPortConfig = 0x00000048 + (isSip7 ? 0x10 : 0);

            if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeGlobalConfig4 = 0x0000000C;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeGlobalConfig5 = 0x00000010;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeMacSpoofProtectionConfig0 = 0x00000014;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeEngineConfig.bridgeMacSpoofProtectionConfig1 = 0x00000018;
            }
        ...
        {/*start of unit bridgeDropCntrAndSecurityBreachDropCntrs */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeDropCntrAndSecurityBreachDropCntrs.globalSecurityBreachFilterCntr = 0x00001300 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeDropCntrAndSecurityBreachDropCntrs.portVLANSecurityBreachDropCntr = 0x00001304 + (isSip7 ? 0x200 : 0);
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.bridgeDropCntrAndSecurityBreachDropCntrs.bridgeFilterCntr = 0x00001308 + (isSip7 ? 0x200 : 0);
    ...
    {/*start of unit FDB */
        {/*start of unit FDBCore */
            {/*start of unit sourceAddrNotLearntCntr */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.sourceAddrNotLearntCntr.learnedEntryDiscardsCount = devObjPtr->fdbRegistersRelativeBase + 0x00000060;

            }/*end of unit sourceAddrNotLearntCntr */


            {/*start of unit FDB */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDB.MTCPipeDispatcher = devObjPtr->fdbRegistersRelativeBase + 0x00000400;
                ...
                {/*420+4*n*/
                    ...
                    for(n = 0 ; n <= 4 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSERProtection.FDBFaultAddr[n] =
                            devObjPtr->fdbRegistersRelativeBase + 0x420+4*n;
                    }/* end of loop n */
                }/*420+4*n*/
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSERProtection.FDBSerConfig = devObjPtr->fdbRegistersRelativeBase + 0x00000410;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSERProtection.FDBSerErrData = devObjPtr->fdbRegistersRelativeBase + 0x00000414;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSERProtection.FDBSerErrAddr = devObjPtr->fdbRegistersRelativeBase + 0x00000418;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSERProtection.FDBSerErrCntr = devObjPtr->fdbRegistersRelativeBase + 0x0000041c;

            }/*end of unit FDBSERProtection */


            {/*start of unit FDBSchedulerConfig */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSchedulerConfig.FDBSchedulerSWRRCtrl = devObjPtr->fdbRegistersRelativeBase + 0x00000380;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBSchedulerConfig.FDBSchedulerSWRRConfig = devObjPtr->fdbRegistersRelativeBase + 0x00000384;

            }/*end of unit FDBSchedulerConfig */

            {/*start of unit FDBInterrupt */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBInterrupt.FDBInterruptCauseReg = devObjPtr->fdbRegistersRelativeBase + 0x000001b0;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBInterrupt.FDBInterruptMaskReg = devObjPtr->fdbRegistersRelativeBase + 0x000001b4;

            }/*end of unit FDBInterrupt */


            {/*start of unit FDBInternal */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBInternal.metalFix = devObjPtr->fdbRegistersRelativeBase + 0x00000170;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBInternal.FDBIdleState = devObjPtr->fdbRegistersRelativeBase + 0x00000174;
                ...
                {/*134+n*4*/
                    ...
                    for(n = 0 ; n <= 4 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBIndirectAccess.FDBIndirectAccessData[n] =
                            devObjPtr->fdbRegistersRelativeBase + 0x134+n*4;
                    }/* end of loop n */
                }/*134+n*4*/
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBIndirectAccess.FDBIndirectAccessCtrl = devObjPtr->fdbRegistersRelativeBase + 0x00000130;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBIndirectAccess.FDBIndirectAccessAccess = devObjPtr->fdbRegistersRelativeBase + 0x00000150;
                ...
                {/*300+0x4*n*/
                    ...
                    for(n = 0 ; n <= 7 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBHashResults.FDBMultiHashCRCResultReg[n] =
                            devObjPtr->fdbRegistersRelativeBase + 0x300+0x4*n;
                    }/* end of loop n */
                }/*300+0x4*n*/
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBHashResults.FDBNonMulti_hash_crc_result = devObjPtr->fdbRegistersRelativeBase + 0x00000320;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBHashResults.FDBXorHash = devObjPtr->fdbRegistersRelativeBase + 0x00000324;

            }/*end of unit FDBHashResults */


            {/*start of unit FDBGlobalConfig */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.FDBGlobalConfig = devObjPtr->fdbRegistersRelativeBase + 0x00000000;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.FDBGlobalConfig1 = devObjPtr->fdbRegistersRelativeBase + 0x00000004;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.FDBCRCHashInitConfig = devObjPtr->fdbRegistersRelativeBase + 0x0000000c;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.bridgeAccessLevelConfig = devObjPtr->fdbRegistersRelativeBase + 0x00000010;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.FDBAgingWindowSize = devObjPtr->fdbRegistersRelativeBase + 0x00000014;

                if(isSip6)
                {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBGlobalConfig.FDBGlobalConfig2 = devObjPtr->fdbRegistersRelativeBase + 0x00000008;
                }
            ...
            {/*start of unit FDBCntrsUpdate */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBCntrsUpdate.FDBCntrsUpdateCtrl = devObjPtr->fdbRegistersRelativeBase + 0x00000340;
                ...
                {/*200+n * 0x4*/
                    ...
                    for(n = 0 ; n <= 15 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBBankCntrs.FDBBankCntr[n] =
                            devObjPtr->fdbRegistersRelativeBase + 0x200+n * 0x4;
                    }/* end of loop n */
                }/*200+n * 0x4*/
                {/*240+n * 0x4*/                 /*manually replaced 16 registers into array*/
                    ...
                    for(n = 0 ; n <= 15 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBBankCntrs.FDBBankRank[n] =
                            devObjPtr->fdbRegistersRelativeBase + 0x240+n * 0x4;
                    }/* end of loop n */
                }/*240+n * 0x4*/
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBBankCntrs.FDBBankCntrsCtrl = devObjPtr->fdbRegistersRelativeBase + 0x00000280;
            ...
            {/*start of unit FDBAddrUpdateMsgs */
                {/*start of unit AUMsgToCPU */
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAddrUpdateMsgs.AUMsgToCPU.AUMsgToCPU = devObjPtr->fdbRegistersRelativeBase + 0x00000090;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAddrUpdateMsgs.AUMsgToCPU.AUFIFOToCPUConfig = devObjPtr->fdbRegistersRelativeBase + 0x00000094;
                    ...
                    {/*d0+n*4*/
                        ...
                        for(n = 0 ; n <= 5 ; n++) {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAddrUpdateMsgs.AUMsgFromCPU.AUMsgFromCPUWord[n] =
                                devObjPtr->fdbRegistersRelativeBase + 0xd0+n*4;
                        }/* end of loop n */
                    }/*d0+n*4*/
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAddrUpdateMsgs.AUMsgFromCPU.AUMsgFromCPUCtrl = devObjPtr->fdbRegistersRelativeBase + 0x000000c0;
            ...
            {/*start of unit FDBAction */
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBActionGeneral = devObjPtr->fdbRegistersRelativeBase + 0x00000020;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBAction0 = devObjPtr->fdbRegistersRelativeBase + 0x00000024;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBAction1 = devObjPtr->fdbRegistersRelativeBase + 0x00000028;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBAction2 = devObjPtr->fdbRegistersRelativeBase + 0x0000002c;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBAction3 = devObjPtr->fdbRegistersRelativeBase + 0x00000030;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBAction4 = devObjPtr->fdbRegistersRelativeBase + 0x00000034;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBActionOldParameters = devObjPtr->fdbRegistersRelativeBase + 0x00000038;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.FDBAction.FDBActionNewParameters = devObjPtr->fdbRegistersRelativeBase + 0x0000003c;
            ...
            {
                {/*hsrPrp*/
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpGlobalConfig       = devObjPtr->fdbRegistersRelativeBase + 0x00000500;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.ddeExpirationTimeConfig  = devObjPtr->fdbRegistersRelativeBase + 0x00000504;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpTimer              = devObjPtr->fdbRegistersRelativeBase + 0x00000508;
                    {
                        GT_U32    n;
                        for(n = 0 ; n < devObjPtr->limitedResources.l2iHportsNum ; n++) {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpCountersFirst [n] = devObjPtr->fdbRegistersRelativeBase + 0x00000510 + 0xc*n;
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpCountersSecond[n] = devObjPtr->fdbRegistersRelativeBase + 0x00000514 + 0xc*n;
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpCountersThird [n] = devObjPtr->fdbRegistersRelativeBase + 0x00000518 + 0xc*n;
                        }
                    }
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpOverridenNonExpiredCounter = devObjPtr->fdbRegistersRelativeBase + 0x00000590;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->FDB.FDBCore.hsrPrp.hsrPrpAging     = devObjPtr->fdbRegistersRelativeBase + 0x00000594;
                }/*hsrPrp;*/
            }
        }/*end of unit FDBCore */
    ...
    {/*start of unit EQ */
        ...
        {/*start of unit preEgrEngineGlobalConfig */
            {/*a100+r*0x4*/
                GT_U32    r;
                GT_U32    rMax =  (SNET_SIP5_EGR_MAX_PHY_PORTS_MAC(devObjPtr)/32)-1;
                for(r = 0 ; r <= rMax ; r++) {
            ...
            if(!SMEM_CHT_IS_SIP7_GET(devObjPtr))/* obsolete in sip7 */
            {
        ...
        {/*start of unit mirrToAnalyzerPortConfigs */
            GT_U32  numAnalyzers = 63;

            {/*b310+i*4*/
                GT_U32    i;
                for(i = 0 ; i <= (numAnalyzers-1) ; i++) {
        ...
        {/*start of unit L2ECMP */
            ...
            if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr))/* added manually */
            {
                ...
                for(n = 0 ; n <= 15 ; n ++)
                {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EQ.L2ECMP.L2ECMP_Hash_Bit_Selection[n] =
                        (SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 0x00005100 : 0x00005060) + n*4;
                }
    ...
    {/*start of unit LPM */
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            /* already done as part of unit allocation */
        }
        else
        if(!SMEM_CHT_IS_SIP6_GET(devObjPtr))
        {
    ...
    if(isDxControlPipe)
    {/*start of unit TCAM */
        /* devObjPtr->numofTcamClients is the Amount of  TCAM Client Groups */
        /* Falcon - 4, AC5P - 5, AC5X - 3, Harrier - 3, Ironman - 1, AAS - 5 */
        GT_U32  maxGroups = (devObjPtr->numofTcamClients - 1);
        GT_U32  maxKeySizeGroupIdx = isSip6_10 ? 0 : (devObjPtr->numofTcamClients - 1);
        GT_U32  maxHit = 3;
        GT_U32  tcamCtrlRegOffeset;
        GT_U32  mgGroupHitStep;

        tcamCtrlRegOffeset = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 0x00700000 : 0x00500000;
        mgGroupHitStep     = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 0x10 : 0x14;

        if (devObjPtr->limitedNumOfParrallelLookups)
        {
            /* Ironman has Hit0 and Hit1 only */
            maxHit = devObjPtr->limitedNumOfParrallelLookups-1;
        }

        {/*start of unit tcamInterrupts */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamInterrupts.TCAMInterruptsSummaryMask = tcamCtrlRegOffeset + 0x00001000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamInterrupts.TCAMInterruptsSummaryCause = tcamCtrlRegOffeset + 0x00001004;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamInterrupts.TCAMInterruptMask = tcamCtrlRegOffeset + 0x00001008;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamInterrupts.TCAMInterruptCause = tcamCtrlRegOffeset + 0x0000100c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamInterrupts.tcamParityErrorAddr = tcamCtrlRegOffeset + 0x00001010;

        }/*end of unit tcamInterrupts */


        {/*start of unit tcamArrayConf */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamArrayConf.tcamActiveFloors = tcamCtrlRegOffeset + 0x00004010;

        }/*end of unit tcamArrayConf */


        {/*start of unit parityDaemonRegs */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonCtrl = tcamCtrlRegOffeset + 0x00003000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonWriteAdressStart = tcamCtrlRegOffeset + 0x00003004;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonWriteAdressEnd = tcamCtrlRegOffeset + 0x00003008;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonWriteAdressJump = tcamCtrlRegOffeset + 0x0000300c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonReadAdressStart = tcamCtrlRegOffeset + 0x00003010;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonReadAdressEnd = tcamCtrlRegOffeset + 0x00003014;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonReadAdressJump = tcamCtrlRegOffeset + 0x00003018;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataX0 = tcamCtrlRegOffeset + 0x0000301c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataX1 = tcamCtrlRegOffeset + 0x00003020;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataY0 = tcamCtrlRegOffeset + 0x00003028;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataX2 = tcamCtrlRegOffeset + 0x00003024;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataY1 = tcamCtrlRegOffeset + 0x0000302c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonDefaultWriteDataY2 = tcamCtrlRegOffeset + 0x00003030;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.parityDaemonRegs.parityDaemonStatus = tcamCtrlRegOffeset + 0x0000303c;
                ...
                for(n = 0 ; n <= maxGroups ; n++) {
                    for(i = 0 ; i <= maxHit ; i++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.mgAccess.mgHitGroupHitNum[n][i] =
                            tcamCtrlRegOffeset + 0x000021bc + (i * 0x4) + (n * mgGroupHitStep);
                    }/* end of loop i */
                }/* end of loop n */
            }/*5021bc+i*0x4+n*0x14*/
            {/*502004+i*0x4+n*0x54*/
                GT_U32    n,i,numOfChunks;
                numOfChunks = isSip6_10 ? 19 : 20;
                if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
                {
                    numOfChunks = 30;
                }
                for (n = 0; n <= maxKeySizeGroupIdx; n++)
                {
                    for(i = 0 ; (i <= numOfChunks); i++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.mgAccess.mgKeyGroupChunk[n][i] =
                            tcamCtrlRegOffeset + 0x00002004 + i*0x4 + n*0x54;
                    }/* end of loop i */
                }/* end of loop n */
            }/*502004+i*0x4+n*0x54*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.mgAccess.mgCompRequest = tcamCtrlRegOffeset + 0x00002000;
            if(!isSip6_10)
            {
        ...
        {/*500000+n*0x4*/
            ...
            for(n = 0 ; n <= maxGroups ; n++) {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.groupClientEnable[n] =
                    tcamCtrlRegOffeset + 0x00000000 + n*0x4;
            }/* end of loop n */
        }/*500000+n*0x4*/
        {/*500100+n*0x8*/
            GT_U32    n;
            for(n = 0 ; n < devObjPtr->tcamNumOfFloors ; n++) {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamHitNumAndGroupSelFloor[n] =
                    tcamCtrlRegOffeset + 0x00000100 + n*0x8;
            }/* end of loop n */
        }/*500100+n*0x8*/

        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TCAM.tcamGlobalConfig = tcamCtrlRegOffeset + 0x00007008;

        /* Ironman has no EM support */
        /* EM related registers of SIP7 movrd from TCAM unit to client units */
        if ((! SMEM_CHT_IS_SIP7_GET(devObjPtr)) && isSip6 && devObjPtr->emMaxNumEntries)
        {/*start of unit exactMatchProfilesTable and exactMatchActionAssignment */
            if (isSip6_10)
            {
                {/*507080+n*0x4*/
                    GT_U32    n;
        ...
        }/*end of unit exactMatchProfilesTable and exactMatchActionAssignment*/
    ...
    {/*start of unit HA */
        ...
        {
            {
                GT_U32    i;
                for(i = 0; i < 4; i++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.MPLSEtherType[i] = 0x00002CA4 + (i*4);
                }/* end of loop i */
            }
        }
        else
        {
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.MPLSEtherType[0] = 0x00000550;
        }
        ...
        if (isSip7)
        {
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.numberOfCircuitsInFixedTable = 0x00000A04;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.circuitsLocationInTheSbms = 0x00000A08;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.ipv6TsFlowLabelEntropyOnly = 0x00000A0C;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.ipv6TsFlowLabelEntropyWithMetadata0 = 0x00000A10;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.ipv6TsFlowLabelEntropyWithMetadata1 = 0x00000A14;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.meshIdConfiguration = 0x00000A18;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.srv6Configuration = 0x00000A2C;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.epclRangeCheck0Configuration = 0x00000A34;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.epclRangeCheck1Configuration = 0x00000A38;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.edsa20bConfiguration = 0x00000A3C;
            {/*0x00A40+4*r*/
                GT_U32 r;
                for(r=0; r<=15; r=r+1) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.gcfFlowId_r[r] = 0x00A40+4*r;
                }/* end of loop r */
            }/*0x00A40+4*r*/
            {/*0x00AC0+(r*4+i)*4*/
                GT_U32 r;
                ...
                for(r=0; r<=7; r=r+1) {
                    for(i=0; i<=3; i=i+1) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.globalExpMappingRow_r_Reg_i[r][i] = 0x00AC0+(r*4+i)*4;
                    }/* end of loop i */
                }/* end of loop r */
            }/*0x00AC0+(r*4+i)*4*/
            {/*0x00C00+(p*0x20+n)*4*/
                ...
                for(p=0; p<=15; p=p+1) {
                    for(n=0; n<=31; n=n+1) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.gcfAssignmentProfileTable_p_Nibble_n_Orig[p][n] = 0x00C00+(p*0x20+n)*4;
                    }/* end of loop n */
                }/* end of loop p */
            }/*0x00C00+(p*0x20+n)*4*/
            {/*0x01C00+(p*0x20+n)*4*/
                ...
                for(p=0; p<=15; p=p+1) {
                    for(n=0; n<=31; n=n+1) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.gcfAssignmentProfileTable_p_Nibble_n_Mirrored[p][n] = 0x01C00+(p*0x20+n)*4;
                    }/* end of loop n */
                }/* end of loop p */
            }/*0x01C00+(p*0x20+n)*4*/
            {/*0x02C00+r*4*/
                GT_U32 r;
                for(r=0; r<=1; r=r+1) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.ingressMplsReservedTsPtr_r[r] = 0x02C00+r*4;
                }/* end of loop r */
            }/*0x02C00+r*4*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.cncBcMode = 0x00002CB4;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->HA.ingressMplsReservedCircuitEntry = 0x00002C08;
        }
    }/*end of unit HA */
    ...
    {/*start of unit MLL */
        {/*start of unit multiTargetRateShape */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetRateShape.multiTargetRateShapeConfig = mllRegBaseOffset+0x00000210;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetRateShape.multiTargetRateShapeConfig2 = mllRegBaseOffset+0x00000218;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetRateShape.multiTargetMTUReg = mllRegBaseOffset+0x0000021c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetRateShape.tokenBucketBaseLine = mllRegBaseOffset+0x00000220;
        ...
        {/*start of unit multiTargetVsUcSDWRRAndStrictPriorityScheduler */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetVsUcSDWRRAndStrictPriorityScheduler.mcUcSDWRRAndStrictPriorityConfig = mllRegBaseOffset+0x00000214;
            if (isSip7)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetVsUcSDWRRAndStrictPriorityScheduler.mcPriorityTypeConfiguration = mllRegBaseOffset+0x00000224;
            }
        }/*end of unit multiTargetVsUcSDWRRAndStrictPriorityScheduler */


        {/*start of unit multiTargetTCQueuesAndArbitrationConfig */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesGlobalConfig = mllRegBaseOffset+0x00000004;
            if(isSip6)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesPriorityConfig = mllRegBaseOffset+0x00000208;
            }
            else
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesStrictPriorityEnableConfig = mllRegBaseOffset+0x00000200;
            }
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetTCQueuesAndArbitrationConfig.multitargetTCQueuesWeightConfig = mllRegBaseOffset+0x00000204;
        ...
        {/*start of unit multiTargetEPortMap */
            if(isSip6)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.multiTargetGlobalEPortMinValue = mllRegBaseOffset+0x00000310;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.multiTargetGlobalEPortMaxValue = mllRegBaseOffset+0x00000314;
            }
            else
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.multiTargetEPortValue = mllRegBaseOffset+0x00000300;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.multiTargetEPortMask = mllRegBaseOffset+0x00000304;
            }
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.multiTargetEPortBase = mllRegBaseOffset+0x00000308;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.multiTargetEPortMap.ePortToEVIDXBase = mllRegBaseOffset+0x0000030c;
            ...
            {/*c84+n*0x100*/
                ...
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLOutInterfaceCntrConfig1[n] =
                        (mllRegBaseOffset+0xc84)+n*0x100;
                }/* end of loop n */
            }/*c84+n*0x100*/
            {/*c80+n*0x100*/
                GT_U32    n;
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLOutInterfaceCntrConfig[n] =
                        (mllRegBaseOffset+0xc80)+n*0x100;
                }/* end of loop n */
            }/*c80+n*0x100*/
            {/*e20+(n-1)*0x4*/
                GT_U32    n;
                for(n = 1 ; n <= 3 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLValidProcessedEntriesCntr[n-1] =
                        (mllRegBaseOffset+0xe20)+(n-1)*0x4;
                }/* end of loop n */
            }/*e20+(n-1)*0x4*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLTTLExceptionCntr = mllRegBaseOffset+0x00000e08;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLSkippedEntriesCntr = mllRegBaseOffset+0x00000e04;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLSilentDropCntr = mllRegBaseOffset+0x00000e00;
            {/*c00+n*0x100*/
                GT_U32    n;
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.L2MLLOutMcPktsCntr[n] =
                        (mllRegBaseOffset+0xc00)+n*0x100;
                }/* end of loop n */
            }/*c00+n*0x100*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.MLLSilentDropCntr = mllRegBaseOffset+0x00000800;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.MLLMCFIFODropCntr = mllRegBaseOffset+0x00000804;
            if(isSip6)
            {
                {/*808+n*0x4*/
                    GT_U32    n;
                    for(n = 0 ; n <= 3 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.MLLMCQueueDropCnter[n] =
                            (mllRegBaseOffset+0x808)+n*0x4;
                    }/* end of loop n */
                }/*808+n*0x4*/
            }
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.IPMLLSkippedEntriesCntr = mllRegBaseOffset+0x00000b04;
            {/*900+n*0x100*/
                GT_U32    n;
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.IPMLLOutMcPktsCntr[n] =
                        ( mllRegBaseOffset+0x900)+n*0x100;
                }/* end of loop n */
            }/*900+n*0x100*/
            {/*984+n*0x100*/
                GT_U32    n;
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.IPMLLOutInterfaceCntrConfig1[n] =
                        (mllRegBaseOffset+0x984)+n*0x100;
                }/* end of loop n */
            }/*984+n*0x100*/
            {/*980+n*0x100*/
                GT_U32    n;
                for(n = 0 ; n <= 1 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLOutInterfaceCntrs.IPMLLOutInterfaceCntrConfig[n] =
                        (mllRegBaseOffset+0x980)+n*0x100;
                }/* end of loop n */
            }/*980+n*0x100*/
        ...
        {/*start of unit MLLGlobalCtrl */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLGlobalCtrl =  mllRegBaseOffset+0x00000000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLLookupTriggerConfig =  mllRegBaseOffset+0x0000000c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.globalTTLExceptionConfig =  mllRegBaseOffset+0x00000010;

            if(isSip6)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.L2mllGlobalEPortMinValue = mllRegBaseOffset+0x00000068;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.L2mllGlobalEPortMaxValue = mllRegBaseOffset+0x0000006c;
            }
            else
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.globalEPortRangeConfig = mllRegBaseOffset+0x00000014;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.globalEPortRangeMask = mllRegBaseOffset+0x00000018;
            }

            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.IPMLLTableResourceSharingConfig = mllRegBaseOffset+0x00000020;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.L2MLLTableResourceSharingConfig = mllRegBaseOffset+0x00000024;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLInterruptCauseReg = mllRegBaseOffset+0x00000030;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLInterruptMaskReg = mllRegBaseOffset+0x00000034;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLReplicatedTrafficCtrl = mllRegBaseOffset+0x00000040;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.sourceBasedL2MLLFiltering = mllRegBaseOffset+0x00000054;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLEccStatus = mllRegBaseOffset+0x00000058;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.MLLMCSourceID = mllRegBaseOffset+0x00000060;
            if (isSip7)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.etreeConfiguration = mllRegBaseOffset+0x00000600;

                {/*0x00604+x*0x4*/
                    GT_U32 x;
                    for(x=0; x<=3; x=x+1) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.ipMllGlobalGcfConfigValue[x] = (mllRegBaseOffset+0x00604)+x*0x4;
                    }/* end of loop x */
                }/*0x00604+x*0x4*/
                {/*0x00614+x*4*/
                    GT_U32 x;
                    for(x=0; x<=3; x=x+1) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.ipMllGlobalGcfConfigMask[x] = (mllRegBaseOffset+0x00614)+x*4;
                    }/* end of loop x */
                }/*0x00614+x*4*/

                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MLL.MLLGlobalCtrl.bierConfiguration = mllRegBaseOffset+0x00000628;
            }
    ...
    {/*start of unit PCL */
        ...
        {/*start of unit HASH */
            GT_U32 hashBase = isSip7 ? 0x6000 : 0;
            {/*5c0+4*i*/
                GT_U32    i;
                for(i = 0 ; i <= 15 ; i++) {
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.HASH.CRCHashConfig = hashBase + 0x00000020;
            {/*804+0x4*n*/
                GT_U32    n;
                for(n = 0 ; n <= 19 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.HASH.CRC32Salt[n] =
                        hashBase + 0x804+0x4*n;
                }/* end of loop n */
            }/*804+0x4*n*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.HASH.CRC32Seed = hashBase + 0x00000800;
            if(isSip6_10)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.HASH.CRC32SeedHash1 = hashBase + 0x00000860;
                {/*864+0x4*n*/
                    GT_U32    n;
                    for(n = 0 ; n <= 19 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.HASH.CRC32SaltHash1[n] =
                            hashBase + 0x864+0x4*n;
                    }/* end of loop n */
                }/*864+0x4*n*/
            }
        }/*end of unit HASH */
        ...
        if(!isSip6_30)/* lookupSourceIDMask renamed to : policyGlobalConfig */
        {/*34+4*n*/
            GT_U32    n;
            GT_U32    nMin = devObjPtr->isIpcl0NotValid ? 1 : 0;
            for(n = nMin ; n <= 2 ; n++) {
        ...
        if(isSip6_30)
        {
            ...
            {/*0x150+4*n*/
                ...
                for(n = nMin ; n <= 2 ; n++) {
                    ...
                    if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
                    {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.policyGlobalConfig_ext1[n] =
                            0x140+4*n;
                        for (ndx = 0; (ndx < 2); ndx++)
                        {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PCL.actionModifiedFieldsSelect[n][ndx] =
                                0x7A0 + (8 * n) + (4 * ndx);
                        }
                    }
                    else
    ...
    if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        {/*start of unit OAMConfigs */
            {/*740+4*n*/
                GT_U32    n;
                for(n = 1 ; n <= 15 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.OAMConfigs.channelTypeOAMOpcode[n-1] =
                        0x740+4*n;
                }/* end of loop n */
            }/*740+4*n*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.OAMConfigs.OAMRDIConfigs = 0x00000730;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.OAMConfigs.OAMMEGLevelModeConfigs = 0x00000734;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.OAMConfigs.OAMRFlagConfigs = 0x00000738;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.OAMConfigs.RBitAssignmentEnable = 0x0000073c;

        }/*end of unit OAMConfigs */

        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.ingrPolicyGlobalConfig  = 0x00000000;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.PCLUnitInterruptCause   = 0x00000004;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.PCLUnitInterruptMask    = 0x00000008;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.policyEngineConfig      = 0x0000000C;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.pclBadAddressLatchReg   = 0x00000010;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.policyGlobalConfig      = 0x00000034;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.policyGlobalConfig_ext1 = 0x00000060;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.countingModeConfig      = 0x00000070;
        {/*0x110+4*n*/
            GT_U32    n;
            for(n = 0; (n <= 3); n++)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.maxSduSizeProfile[n] =
                    0x110 + (4 * n);
            }/* end of loop n */
        }/*0x110+4*n*/
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.maxSduException = 0x00000130;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.policyGenericActionConfig = 0x00000140;
        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.flowIdActionAssignmentModeConfig = 0x00000150;
        {/*0x800+4*n*/
            GT_U32    n;
            for(n = 0; (n <= 1); n++)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.actionModifiedFieldsSelect[n] =
                    0x800 + (4 * n);
            }/* end of loop n */
        }/*0x800+4*n*/
        {
            ...
            for (i = 0; (i <= 255); i++)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.exactMatchProfileIdMapEntry[i] =
                    0x900 + (4 * i);
            }
        }
    ...
    {/*start of unit EPCL */
        ...
        if(isSip6)
        {
            ...
            /* NextAddressed queueOffsetLatencyProfileConfig[] is set by appropriated smemXxUnitEpcl() */
        ...
        if (isSip7)
        {
            /* TBD - not found in cider, copied from MPCL */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EPCL.epclPolicyLookupGlobalConfig = 0x00000070;
        }
    }/*end of unit EPCL */
    ...
    if(isDxControlPipe)
    {
        ...
        GT_U32  groupStatusRegsNum;

        groupStatusRegsNum = isSip7 ? OAM_MAX_GROUP_STATUS_CNS : 2;
        ...
        {/*start of unit OAMUnit */
            ...
            {/*e0+4*n*/
                GT_U32    n;
                for(n = 0 ; n < groupStatusRegsNum ; n++) {
            ...
            if(isSip5_15)
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->OAMUnit[index].exceptionSupressionConfig1 = 0x00000A40;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->OAMUnit[index].exceptionSupressionConfig2 = 0x00000A44;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->OAMUnit[index].recoveryPacketCommandConfig1 = 0x00000A50;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->OAMUnit[index].recoveryPacketCommandConfig2 = 0x00000A54;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->OAMUnit[index].recoveryPacketCommandConfig3 = 0x00000A58;
            }
    ...
    {/*start of unit IPvX */
        ...
        {/*start of unit routerGlobalCtrl */
            ...
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.routerGlobalCtrl1 = 0x00000004;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.routerGlobalCtrl2 = 0x00000008;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.routerGlobalCtrl3 = 0x0000000c;
                { /*0x00000010+n*0x4*/
                    GT_U32    n;
                    for(n = 0 ; n <= 3 ; n++) {
                ...
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.genericClassificationFieldDipLookupNheFetched = 0x000000020;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.genericClassificationFieldDipLookupPrefixMatch= 0x000000024;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.genericClassificationFieldSipLookupPrefixMatch= 0x000000028;
                { /*0x00000030+n*0x4*/
                    GT_U32    n;
                    for(n = 0 ; n <= 3 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.routerUcGlobalCfgValue[n] =
                            0x00000030+n*0x4;
                    }/* end of loop n */
                }/*0x00000030+n*0x4*/
                ...
                { /*0x00000040+n*0x4*/
                    GT_U32    n;
                    for(n = 0 ; n <= 3 ; n++) {
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerGlobalCtrl.routerUcGlobalCfgMask[n] =
                            0x00000040+n*0x4;
                    }/* end of loop n */
                }/*0x00000040+n*0x4*/
            }
        ...
        {/*start of unit routerDropCntr */
            ...
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerDropCntr.routerDropCntr1 = 0x00000954;
                SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->IPvX.routerDropCntr.routerDropCntrConfig = 0x00000958;
            }
            else
            {
        ...
        {/*start of unit TAI[36][5] */
            ...
            if(SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_TAI))
            {
                ...
                if((devObjPtr->supportSingleTai) && isSip6_30 &&
                    SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_TAI_2))
                {
                    /* define additional MASTER TAI 2,3,4 */
                    GT_U32        unitArrayIndex;
                    GT_U32        *unitArray_0_ptr,*currU32Ptr;
                    GT_U32        lastTaiIndex = 2;
                    ...
                    if(SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_TAI_3))
                    {
                        lastTaiIndex = 3;
                    ...
                    if(SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_TAI_4))
                    {
                        lastTaiIndex = 4;
                    ...
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[0][lastTaiIndex].phySignature        = 0x00000500;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[0][lastTaiIndex].phyTimestampSecLow  = 0x00000504;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[0][lastTaiIndex].phyTimestampSecHigh = 0x00000508;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[0][lastTaiIndex].phyTimestampNanoSec = 0x0000050c;
    ...
    if(isDxControlPipe && !SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        {/*start of unit IPLR */
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PLR[plrUnit].policerIPFIXPktCountWAThresholdLSB = 0x00000034;
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PLR[plrUnit].ipfixSampleEntriesLog_n[1] = 0x0000004c;
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PLR[plrUnit].policerMeteringRefreshScanAddrRange0 = 0x000000b4;
            ...
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->PLR[plrUnit].ipfixSampleEntriesLog_n[0] = 0x00000048;
    ...
    {/*start of unit CNC */
        {/*start of unit perBlockRegs */
            ...
            /* start of unit clientInterrupts */
            if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
            {
                GT_U32    n;
                GT_U32    m,m_max=2;
                ...
                for(m = 0; m <= m_max; m++)
                {
                    for(n = 0; n <= (devObjPtr->cncBlocksNum-1); n++)
                    {
                        /*CNCBlockClientInterruptCauseRegister: 00001598+n*0x20+m*0x4 : where n (0-7) represents Block, where m (0-2) represents Client*/
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->CNC[0].perBlockRegs.
                            clientInterrupts.CNCBlockClientInterruptCauseRegister[n][m] =
                            0x00001598 + n*0x20+m*0x4;

                        /*CNCBlockClientInterruptMaskRegister: 00001798+n*0x20+m*0x4 : where n (0-7) represents Block, where m (0-2) represents Client*/
                        SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->CNC[0].perBlockRegs.
                            clientInterrupts.CNCBlockClientInterruptMaskRegister[n][m] =
                            0x00001798 + n*0x20+m*0x4;
                    }/* end of loop n */
                }/* end of loop m */
            }
    ...
    if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
    {
        ...
        {/* start of unit L2i */
            ...
            {
                {/*hsrPrp*/
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpGlobalConfig0  = 0x00002200 + (isSip7 ? 0x200 : 0);
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpGlobalConfig1  = 0x00002204 + (isSip7 ? 0x200 : 0);
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpGlobalConfig2  = 0x00002208 + (isSip7 ? 0x200 : 0);
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpGlobalConfig3  = 0x0000220C + (isSip7 ? 0x200 : 0);
                    {
                        GT_U32    n;
                        for(n = 0 ; n <= devObjPtr->limitedResources.l2iHportsNum-1 ; n++) {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpSourcePortConfig0[n] = 0x00002210 + n * 0x8 + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpSourcePortConfig1[n] = 0x00002214 + n * 0x8 + (isSip7 ? 0x200 : 0);
                        }
                    }
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntCounter          = 0x000022A0 + (isSip7 ? 0x200 : 0);
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntCounterThreshold = 0x000022A4 + (isSip7 ? 0x200 : 0);

                    {
                        GT_U32    n;
                        for(n = 0 ; n <= 3 ; n++) {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntDiscardPacketsLreACounter[n] = 0x000022C0 + n*0x4 + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntDiscardPacketsLreBCounter[n] = 0x000022D0 + n*0x4 + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpLreInstanceTable[n]             = 0x00002300 + n*0x4 + (isSip7 ? 0x200 : 0);
                        }
                    }

                    {
                        GT_U32    n;
                        for(n = 0 ; n <= 127 ; n++) {
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntEntryPart0 [n] = 0x00002400 + n * 0xC + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntEntryPart1 [n] = 0x00002404 + n * 0xC + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntEntryPart2 [n] = 0x00002408 + n * 0xC + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntReady      [n] = 0x00002A00 + n * 0x4 + (isSip7 ? 0x200 : 0);
                            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->L2I.hsrPrp.hsrPrpPntValid      [n] = 0x00002C00 + n * 0x4 + (isSip7 ? 0x200 : 0);
                        }
                    }
                }/*hsrPrp;*/
            }
    ...
    {/*start of unit resetAndInitCtrller */
        ...
        {/*start of unit DFXServerUnitsBC2SpecificRegs */
            ...
            if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
            {
                ...
                SMEM_CHT_MAC_REG_DB_DFX_SERVER_GET(devObjPtr)->resetAndInitCtrller.DFXServerUnitsBC2SpecificRegs.deviceCtrl26 = 0x000f8D08;
            }

            if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
            {
                SMEM_CHT_MAC_REG_DB_DFX_SERVER_GET(devObjPtr)->resetAndInitCtrller.DFXServerUnitsBC2SpecificRegs.deviceCtrl7 = 0x000F826C;
            }
