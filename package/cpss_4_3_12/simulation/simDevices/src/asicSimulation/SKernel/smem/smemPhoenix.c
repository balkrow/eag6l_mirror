...
#define PHOENIX_NUM_TSU_UNITS  1
...
static SMEM_GEN_UNIT_INFO_STC phoenix_units[] =
{
    ...
    ,UNIT_INFO_MAC(0x7FE00000,UNIT_CNM_SRAM   ,192   *     _1K)/*in CnM */
...
)
{
    ...
    for (ii=0; ii<PHOENIX_NUM_TSU_UNITS; ii++)
    {
            ...
            address <  (devObjPtr->memUnitBaseAddrInfo.tsu[ii] + (64 *_1K)))
        {
            ...
            {
                skernelFatalError("smemPhoenixGopPortByAddrGet : unknown port for address [0x%8.8x] as 'TSU unit' (case 1)\n",address);
                return 0;
            }
            ...
            {
                if(phoenix100GPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   phoenix100GPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
            ...
            {
                if(phoenixUsxPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   phoenixUsxPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
            ...
            {
                if(phoenixCpuPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   phoenixCpuPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
...
/**
* @internal smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg function
* @endinternal
*
...
*/
static void smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    DECLARE_FUNC_NAME(smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg);
    ...
    *outMemPtr = *memPtr;
    ...
    if (egressPort == SMAIN_NOT_VALID_CNS)
    {
     ...
     __LOG(("smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg , address 0x%8.8x, egress port: %d",address, egressPort));
    queueNum = 0xFFFFFFFF;
    if( ((address & 0xFF00) == 0x4000) || ((address & 0xFF00) == 0x4100) )
    {
        queueNum = 0;
    }
    else if( ((address & 0xFF00) == 0x4800) || ((address & 0xFF00) == 0x4900) )
    {
        queueNum = 1;
    }
    else
    {
        skernelFatalError("smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg: illegal queue<n> address\n");
    }
...
)
{
    ...
    /* start of unit MTI_EXT - per channel/port*/
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].MTI_EXT_PORT.portControl         = 0x0000002C + portFormula + unitOffset;
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].MTI_EXT_PORT.portPeerDelay       = 0x00000040 + portFormula + unitOffset;
...
)
{
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI_cpu_port[unitIndex].MTI_EXT_PORT.portPeerDelay       = 0x00000030 + portFormula;
...
)
{
...

    {
        SMEM_CHUNK_EXTENDED_STC  chunksMem[]=
        {
            ...
            ,{{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (PCS_RS_FEC_OFFSET + 0x00000300, PCS_RS_FEC_OFFSET + 0x0000031c)} , FORMULA_SINGLE_PARAMETER(NUM_PORTS_PER_100G_UNIT, 0x20)}
             /* PCS-RSFEC - end */
             /* PCS-LPCS - start */
...
*
*/
void smemPhoenixActiveRead_tx_train_if_reg3_28G_x4 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    /* output value */
    * outMemPtr = (* memPtr) | (1<<0)/*RX_TRAIN_COMPLETE_LANE*/ | (1<<2)/*TX_TRAIN_COMPLETE_LANE*/;
...
#define SMEM_PER_LANE_MASK_CNS (0xFFFFFFF3)/* support 4 lanes */
    ...
    /* bit 4 RX_TRAIN_COMPLETE_LANE for the logic in mvHwsComphyC28GP4X4SerdesAutoTuneStatusShort */
    {0x00000240, SMEM_PER_LANE_MASK_CNS, smemPhoenixActiveRead_tx_train_if_reg3_28G_x4         , 0 ,NULL,0},
...
*
*/
void smemPhoenixActiveRead_tx_train_if_reg3_28G_x1 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
)
{
    ...
    {0x00022600, SMEM_PER_LANE_MASK_CNS, smemPhoenixActiveRead_tx_train_if_reg3_28G_x1         , 0 ,NULL,0},
...
)
{
    START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
        ...
        {0x00003000 , SMEM_FULL_MASK_CNS - (0x3f << 2), smemChtActiveReadIntrCauseReg, 0, smemChtActiveWriteIntrCauseReg, 0},
        {0x00003400 , SMEM_FULL_MASK_CNS - (0x3f << 2), NULL, 0, smemChtActiveGenericWriteInterruptsMaskReg, 0},
        /* Channel Egress Timestamp Queue 0 Entry High */
        {0x00004004 , SMEM_FULL_MASK_CNS - (0x3f << 3) , smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg, 0, NULL, 0},
        /* Channel Egress Timestamp Queue 1 Entry High */
        {0x00004804 , SMEM_FULL_MASK_CNS - (0x3f << 3) , smemPhoenixActiveReadGopPtpTxTsQueueReg2Reg, 0, NULL, 0},
    END_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
    ...
    }
    {
        static SMEM_REGISTER_DEFAULT_VALUE_STC myUnit_registersDefaultValueArr[] =
        {
            ...
            ,{DUMMY_NAME_PTR_CNS,            0x00001400,         0x00000020,      64,    0x4 }
            ,{NULL,                          0,                  0x00000000,      0,    0x0 }
        };
        static SMEM_LINK_LIST_REGISTER_DEFAULT_VALUE_STC list = {myUnit_registersDefaultValueArr,NULL};
        unitPtr->unitDefaultRegistersPtr = &list;
...
)
{
...

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x000F8340, 0x000F8370)}
    ...
    if (devObjPtr->deviceFamily == SKERNEL_IRONMAN_FAMILY)
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x000F8054, 0x000F805C)}
           ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x000F8D6C, 0x000F8D74)}
...
static void smemPhoenixUnitCnmSramUnit
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    INOUT SMEM_UNIT_CHUNKS_STC  * unitPtr
)
{
    ...
    if (devObjPtr->deviceFamily == SKERNEL_IRONMAN_FAMILY)
    {
        /* Address space */
        {
            SMEM_CHUNK_BASIC_STC  chunksMem[]=
            {
               {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_SHARED_MEM_MAC (0x00000000, (512*_1K), "CnmSram")}
            };
            GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
    ...
    else
    {
        /* Address space */
        {
            SMEM_CHUNK_BASIC_STC  chunksMem[]=
            {
               {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_SHARED_MEM_MAC (0x00000000, (192*_1K), "CnmSram")}
            };
            GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
)
{
    SMEM_DEVICE_UNIT_ALLOCATION_STC allocUnitsArr[] = {
        ...
        ,{STR(UNIT_CNM_SRAM)         ,smemPhoenixUnitCnmSramUnit}
...
*/
void smemPhoenixPrepareMultiMgRecognition
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr
)
{
