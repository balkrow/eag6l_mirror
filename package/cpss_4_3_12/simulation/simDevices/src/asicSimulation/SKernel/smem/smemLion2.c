...
#define  a0_fdbSizeNumBytes  ((SMEM_MAC_TABLE_SIZE_64KB) * a0_fdbEntryNumBytes_align)
...

        {
            ...
            {
                /*FDB table*/
                 {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC(0x06400000 ,a0_fdbSizeNumBytes), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(a0_fdbEntryNumBits, a0_fdbEntryNumBytes_align),SMEM_BIND_TABLE_MAC(fdbTable)}
            };
            GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
            SMEM_UNIT_CHUNKS_STC    tmpUnitChunk;
...
{
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC(0x0E840000 , 64*1024), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(192, 32) , SMEM_BIND_TABLE_MAC(arpTable)/*tunnelStartTable*/}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
{
        {
            SMEM_CHUNK_BASIC_STC  chunksMem[]=
            {
                ...
                /* base of 0x0c880000 */
                ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC(0x0c880000 ,65536), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(128, 16) , SMEM_BIND_TABLE_MAC(mllTable)}      /* MLL Entry */
            };
            GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
)
{
    ...
    {
        devObjPtr->fdbMaxNumEntries = SKERNEL_TABLE_NUM_OF_ENTRIES_GET_MAC(devObjPtr,fdbTable);
    }
...
)
{
    ...
    SKERNEL_DEVICE_OBJECT *coreDevicePtr;
    ...
    *memPtr = *inMemPtr;
    ...
    if (fieldVal == 0)
    {
        ...
        if(devObjPtr->portGroupSharedDevObjPtr)
        {
            ...
            {
                coreDevicePtr = shellDevicePtr->coreDevInfoPtr[ii].devObjPtr;
                scibUnBindDevice(coreDevicePtr->deviceId);

                if(devObjPtr->deviceId == coreDevicePtr->deviceId)
                {
                    continue;
                }
                sbufPoolSuspend(coreDevicePtr->bufPool);
                squeSuspend(coreDevicePtr->queueId);
            }
        }
        else
        {
...
)
{
    ...
    {/*start of unit MG - MG  */
        ...
        {/*start of unit MGRegsMGMiscellaneous */
            ...
            /* for AC3/5 see : smemXCat3RegsInfoSet(...) */
            if(SMEM_CHT_IS_SIP5_GET(devObjPtr))/* not support Lion2 */
            {
                ...
                /* MG1 Internal registers introduced in BC2 B0 */
                if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
                {
                    regAddrDbPtr->MG.globalInterrupt[0].mg1InternalInterruptsSummaryCause   = 0x0000009C;
                    regAddrDbPtr->MG.globalInterrupt[0].mg1InternalInterruptsSummaryMask    = 0x000000A0;
                }
            }
            ...
            if(SMEM_CHT_IS_SIP5_GET(devObjPtr))/* not support Lion2 */
            {
                ...
                if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
                {
                    regAddrDbPtr->MG.globalInterrupt[1].mg1InternalInterruptsSummaryCause   = 0x00000144;
                    regAddrDbPtr->MG.globalInterrupt[1].mg1InternalInterruptsSummaryMask    = 0x00000148;
                }
                ...
                if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
                {
                    regAddrDbPtr->MG.globalInterrupt[2].mg1InternalInterruptsSummaryCause   = 0x0000010C;
                    regAddrDbPtr->MG.globalInterrupt[2].mg1InternalInterruptsSummaryMask    = 0x00000114;
                }
            }
...
/*        SMEM_CHT_PP_REGS_UNIT_START_INFO_SET_MAC(devObjPtr,LMS                ,0); not use unit*/
    ...
    {/*start of unit HA - HA  */
        ...
        {/*start of unit HA_IP */
            ...
            regAddrDbPtr->HA.MPLSEthertype[0] = 0x0e800550;
