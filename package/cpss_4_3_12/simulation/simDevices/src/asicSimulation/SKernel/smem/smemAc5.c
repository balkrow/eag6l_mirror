...
#define BASE_MPP_AC5_UNIT            0x80000000   /*contains ADDR_DECODE0-8 from 0x80400000*/
...
/*SMEM_AC5_UNIT_MPP_E*/             BASE_MPP_AC5_UNIT             >> 23
};
...
/**
* @internal smemXCat5ActiveWriteToSd1RxInit function
* @endinternal
*
* @brief   Writing to PU PLL and PU_TX/PU_RX sets PLL ready indication
*
...
* @param[in] inMemPtr                 - Pointer to the memory to get register's content.
*/
void smemXCat5ActiveWriteToSd1RxInit
(
    IN  SKERNEL_DEVICE_OBJECT   *devObjPtr,
    IN  GT_U32                  address,
    IN  GT_U32                  memSize,
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    /* calculate address */
    regAddr = address + 0x14; /* SERDES External Status 0 */

    smemRegFldSet(devObjPtr, regAddr, 4/*rx_init_done*/ ,1, ((*inMemPtr & (1<<4)/*SDRxInit*/) != 0));
...
/**
* @internal smemPhoenixActiveRead_tx_train_if_reg3_28G_x4 function
* @endinternal
*
...
*
*/
void smemAc5ActiveRead_tx_train_if_reg3_12G (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    /* output value */
    * outMemPtr = (* memPtr) | (1<<4)/*RX_TRAIN_COMPLETE_LANE*/ | (1<<6)/*TX_TRAIN_COMPLETE_LANE*/;
...
)
{
    ...
    {0x13000004, 0xFFF00FFF, NULL, 0 ,smemXCat5ActiveWriteToSd1RxInit,0},
    {0x1300000C, 0xFFF00FFF, NULL, 0 ,smemXCat5ActiveWriteToSd1TxRxTraining,0},

    /* bit 4 RX_TRAIN_COMPLETE_LANE for the logic in mvHwsComphyC12GP41P2VSerdesAutoTuneStatusShort */
    {0x1300d944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
    {0x1300f944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
    {0x13011944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
    {0x13013944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
    {0x13015944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
    {0x13017944, SMEM_FULL_MASK_CNS, smemAc5ActiveRead_tx_train_if_reg3_12G         , 0 ,NULL,0},
...
#define CNM_MG_REGISTERS(unitBase)  \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000030, unitBase + 0x00000044)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x0000004C, unitBase + 0x00000084)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000090, unitBase + 0x00000100)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000150, unitBase + 0x0000017C)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x000003F0, unitBase + 0x000003FC)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000500, unitBase + 0x0000051C)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000610 ,unitBase + 0x00000614)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000638 ,unitBase + 0x0000063c)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000670 ,unitBase + 0x00000674)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000698 ,unitBase + 0x0000069C)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00000800, unitBase + 0x000008F4)}, \
    {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (unitBase + 0x00030000 ,unitBase + 0x00030014)}  /* XSMI */

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
             CNM_MG_REGISTERS(BASE_MG0_AC5_UNIT)
            ,CNM_MG_REGISTERS(BASE_MG1_AC5_UNIT)
            ,CNM_MG_REGISTERS(BASE_MG2_AC5_UNIT)
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
/**
* @internal smemXCat5UnitMppAndAdrDecode function
* @endinternal
*
* @brief   Allocate address type specific memories -- for the MPP unit and Addr Decode0-8 units
*/
static void smemXCat5UnitMppAndAdrDecode
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr
)
{
...

    {
        ...
        {
            /* CNM RFU registers */
             {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00010000, BASE_MPP_AC5_UNIT + 0x00010044)}
            /* MPP space */
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00020100, BASE_MPP_AC5_UNIT + 0x0002011C)}
            /* CNM ADDR_DECODE0_8 window0-15 control registers*/
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00400100, BASE_MPP_AC5_UNIT + 0x0040027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00410100, BASE_MPP_AC5_UNIT + 0x0041027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00420100, BASE_MPP_AC5_UNIT + 0x0042027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00430100, BASE_MPP_AC5_UNIT + 0x0043027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00440100, BASE_MPP_AC5_UNIT + 0x0044027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00450100, BASE_MPP_AC5_UNIT + 0x0045027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00460100, BASE_MPP_AC5_UNIT + 0x0046027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00470100, BASE_MPP_AC5_UNIT + 0x0047027C)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (BASE_MPP_AC5_UNIT + 0x00480100, BASE_MPP_AC5_UNIT + 0x0048027C)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
)
{
    ...
    /* MPP and Address Decode units registers */
    smemXCat5UnitMppAndAdrDecode(devObjPtr);
...
#define ac5_mg0_globalInterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC  ac5_mg0_globalInterruptsSummary_##treeId = \
{                                                                                 \
    /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalAc5CnmInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_GLB_INT_CAUSE_REG*/\
    /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalAc5CnmInterruptMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                               \
    /*myFatherInfo*/{                                                                                                                         \
        /*myBitIndex*/0 ,                                                                                                                     \
...
ac5_mg0_globalInterruptsSummary_tree(0);
ac5_mg0_globalInterruptsSummary_tree(1);
ac5_mg0_globalInterruptsSummary_tree(2);


#define ac5_mg0_dfx1InterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC  ac5_mg0_dfx1InterruptsSummary_##treeId = \
{                                                                               \
    /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfx1Ac5CnmInterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_GLB_INT_CAUSE_REG*/\
    /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfx1Ac5CnmInterruptsSummaryMask) ,0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                               \
    /*myFatherInfo*/{                                                                                                                               \
        /*myBitIndex*/dfx1IntSum ,                                                                                                                  \
        /*interruptPtr*/&ac5_mg0_globalInterruptsSummary_##treeId                                                                                   \
        },                                                                                                                                          \
    /*isTriggeredByWrite*/ 1                                                                                                                        \
}

ac5_mg0_dfx1InterruptsSummary_tree(0);
ac5_mg0_dfx1InterruptsSummary_tree(1);
ac5_mg0_dfx1InterruptsSummary_tree(2);


#define ac5_GlobalInterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC ac5_GlobalInterruptsSummary_##treeId = \
{                                                                     \
    /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_GLB_INT_CAUSE_REG*/\
    /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalInterruptMask) ,0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                               \
    /*myFatherInfo*/{                                                                                                                   \
        /*myBitIndex*/ext_mg_int_out0,                                                                                                  \
        /*interruptPtr*/&ac5_mg0_dfx1InterruptsSummary_##treeId                                                                                  \
        },                                                                                                                              \
    /*isTriggeredByWrite*/ 1                                                                                                            \
}

ac5_GlobalInterruptsSummary_tree(0);
ac5_GlobalInterruptsSummary_tree(1);
ac5_GlobalInterruptsSummary_tree(2);

#define ac5_FunctionalInterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC ac5_FunctionalInterruptsSummary_##treeId = \
{                                                                                \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].functionalInterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].functionalInterruptsSummaryMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS}, \
        /*myFatherInfo*/{                                                                                                 \
            /*myBitIndex*/FuncUnitsIntsSum,                                                                               \
            /*interruptPtr*/&ac5_GlobalInterruptsSummary_##treeId                                                                  \
            },                                                                                                            \
        /*isTriggeredByWrite*/ 1                                                                                          \
}

ac5_FunctionalInterruptsSummary_tree(0);
/*ac5_FunctionalInterruptsSummary_tree(1);
ac5_FunctionalInterruptsSummary_tree(2);*/

/* node in MG that aggregates ports 0..30 */
#define ac5_portsInterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC  ac5_portsInterruptsSummary_##treeId = \
{                                                                            \
    /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].stackPortsInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
    /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].stackPortsInterruptMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
    /*myFatherInfo*/{                                                                                                                                   \
...
ac5_portsInterruptsSummary_tree(0);
/*ac5_portsInterruptsSummary_tree(1);
ac5_portsInterruptsSummary_tree(2);*/
...
static SKERNEL_INTERRUPT_REG_INFO_STC  eqInterruptSummary =
{
    ...
    /*myFatherInfo*/{
        ...
        /*interruptPtr*/&ac5_FunctionalInterruptsSummary_0
        }
};
...
#define dataPathInterruptsSummary_tree(treeId)    \
static SKERNEL_INTERRUPT_REG_INFO_STC  dataPathInterruptsSummary_##treeId = \
{                                                                           \
    /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SERInterruptsSummary),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS}, \
    /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SERInterruptsMask)   ,0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},    \
    /*myFatherInfo*/{                                                                                      \
        /*myBitIndex*/DataPathIntSum ,                                                                     \
        /*interruptPtr*/&ac5_GlobalInterruptsSummary_0                                                     \
        }                                                                                                  \
}

dataPathInterruptsSummary_tree(0);
/*dataPathInterruptsSummary_tree(1);
dataPathInterruptsSummary_tree(2);*/
...
static SKERNEL_INTERRUPT_REG_INFO_STC  mibCountersInterruptsSummary =
{
    ...
    /*myFatherInfo*/{
        /*myBitIndex*/1 ,
        /*interruptPtr*/&dataPathInterruptsSummary_0
        }
};
...
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(24,25,ac5_portsInterruptsSummary_0);
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(25,26,ac5_portsInterruptsSummary_0);
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(26,27,ac5_portsInterruptsSummary_0);
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(27,28,ac5_portsInterruptsSummary_0);
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(28,26,ac5_portsInterruptsSummary_0);
XLG_SUMMARY_PORT_IN_MG_INTERRUPT_SUMMARY_MAC(29,28,ac5_portsInterruptsSummary_0);
...
/* connecting the MG3 (Ac3 legacy MG) to MG0 in CnM that goes to CPU (via dfx1InterruptsSummary) */
#define MG_HEAD_PER_TREE(treeId)  \
    {                                                                                                                                                                                      \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_GLB_INT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].globalInterruptMask) ,0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                              \
        /*myFatherInfo*/{                                                                                                                   \
            /*myBitIndex*/ext_mg_int_out0 + treeId,                                                                                         \
            /*interruptPtr*/&ac5_mg0_dfx1InterruptsSummary_##treeId                                                                         \
            },                                                                                                                              \
        /*isTriggeredByWrite*/ 1                                                                                                            \
    }
...
#define MG_NODES_PER_TREE(treeId,globalInterruptsSummary)  \
    MG_HEAD_PER_TREE(treeId)                               \
    ,{                                                                            \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].ports1InterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].ports1InterruptsSummaryMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
        /*myFatherInfo*/{                                                                                                                                   \
            /*myBitIndex*/ports_1_int_sum ,                                                                                                                     \
            /*interruptPtr*/&globalInterruptsSummary##_##treeId                                                                                           \
            },                                                                                                                                              \
        /*isTriggeredByWrite*/ 1                                                                                                                            \
    }                                                                                                                                                       \
    ,{                                                                            \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].ports2InterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].ports2InterruptsSummaryMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
        /*myFatherInfo*/{                                                                                                                                   \
    ...
    ,{                                                                            \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].functionalInterruptsSummary1Cause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].functionalInterruptsSummary1Mask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
        /*myFatherInfo*/{                                                                                                                                   \
    ...
    ,{                                                                            \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfxInterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfxInterruptsSummaryMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
        /*myFatherInfo*/{                                                                                                                                   \
    ...
    ,{                                                                            \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfx1InterruptsSummaryCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].dfx1InterruptsSummaryMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT3_XG_PORTS_INTERRUPT_MASK_REG*/  \
        /*myFatherInfo*/{                                                                                                                                   \
            /*myBitIndex*/dfx1IntSum ,                                                                                                                     \
            /*interruptPtr*/&globalInterruptsSummary##_##treeId                                                                                           \
            },                                                                                                                                              \
    ...
    ,{                                                                                                                    \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SDMARegs.receiveSDMAInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS}, /*SMEM_CHT_SDMA_INT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SDMARegs.receiveSDMAInterruptMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                                 \
        /*myFatherInfo*/{                                                                                                                                   \
    ...
    ,{                                                                                                                                                      \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SDMARegs.transmitSDMAInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_TX_SDMA_INT_CAUSE_REG*/\
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].SDMARegs.transmitSDMAInterruptMask ),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                                   \
        /*myFatherInfo*/{                                                                                                                                      \
    ...
    ,{                                                                                                                                                         \
        /*causeReg*/ {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].miscellaneousInterruptCause),0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},/*SMEM_CHT_MISC_INTR_CAUSE_REG*/          \
        /*maskReg*/  {SKERNEL_INTERRUPT_BIND_REG_LEGACY_MAC(MG.globalInterrupt[treeId].miscellaneousInterruptMask) ,0, MG0 | MG_TREE_ID(treeId) | MG_UNIT_USED_CNS},                                           \
        /*myFatherInfo*/{                                                                                                                                      \
...
static SKERNEL_INTERRUPT_REG_INFO_STC ac5InterruptsTreeDb[] = {
    MG_NODES_PER_TREE(0,ac5_GlobalInterruptsSummary)
    ,MG_NODES_PER_TREE(1,ac5_GlobalInterruptsSummary)
    ,MG_NODES_PER_TREE(2,ac5_GlobalInterruptsSummary)
    ,{
...
{
    SMEM_CHT_PP_REGS_ADDR_STC        * regAddrDbPtr;
    GT_U32      cnm_mgOffset = BASE_MG0_AC5_UNIT/*0x7F900000*/;
    ...
    regAddrDbPtr->MG.globalInterrupt[0].globalAc5CnmInterruptCause = 0x00000030 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[0].globalAc5CnmInterruptMask = 0x00000034 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[0].dfx1Ac5CnmInterruptsSummaryCause = 0x000000B8 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[0].dfx1Ac5CnmInterruptsSummaryMask = 0x000000BC + cnm_mgOffset;

    regAddrDbPtr->MG.globalInterrupt[1].globalAc5CnmInterruptCause = 0x00000610 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[1].globalAc5CnmInterruptMask = 0x00000614 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[1].dfx1Ac5CnmInterruptsSummaryCause = 0x0000638 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[1].dfx1Ac5CnmInterruptsSummaryMask = 0x000063c + cnm_mgOffset;

    regAddrDbPtr->MG.globalInterrupt[2].globalAc5CnmInterruptCause = 0x00000670 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[2].globalAc5CnmInterruptMask = 0x00000674 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[2].dfx1Ac5CnmInterruptsSummaryCause = 0x00000698 + cnm_mgOffset;
    regAddrDbPtr->MG.globalInterrupt[2].dfx1Ac5CnmInterruptsSummaryMask = 0x0000069C + cnm_mgOffset;
