...
static SMEM_GEN_UNIT_INFO_STC hawk_units[] =
{
    ...
    ,UNIT_INFO_MAC(0x3D010000 ,UNIT_CNM_RUNIT    ,64   *     _1K) /*in CnM */
...
)
{
    ...
    for (ii=0; ii<HAWK_NUM_TSU_UNITS; ii++)
    {
            ...
            address <  (devObjPtr->memUnitBaseAddrInfo.tsu[ii] + (64 *_1K)))
        {
            ...
            {
                if(hawk100GPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   hawk100GPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
            ...
            {
                if(hawkUsxPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   hawkUsxPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
            ...
            {
                if(hawkCpuPortsArr[ii].channelInfo.unitIndex   == unitIndex &&
                   hawkCpuPortsArr[ii].channelInfo.indexInUnit == localPortInUnit)
                {
...
/**
* @internal smemHawkActiveReadGopPtpTxTsQueueReg2Reg function
* @endinternal
*
...
* @param[out] outMemPtr                - Pointer to the memory to copy register's content.
*/
static void smemHawkActiveReadGopPtpTxTsQueueReg2Reg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    DECLARE_FUNC_NAME(smemHawkActiveReadGopPtpTxTsQueueReg2Reg);
    ...
    *outMemPtr = *memPtr;
    ...
    {
        __LOG(("smemGopPortByAddrGet : unknown port for address [0x%8.8x] as 'TSU unit' (case 2)\n",address));
        return;
    }

     __LOG(("smemHawkActiveReadGopPtpTxTsQueueReg2Reg , address 0x%8.8x, egress port: %d",address, egressPort));
    queueNum = 0xFFFFFFFF;
    if( (address & 0xFF00) == 0x4000 )
    {
        queueNum = 0;
    }
    else if( (address & 0xFF00) == 0x4800 )
    {
        queueNum = 1;
    }
    else
    {
        skernelFatalError("smemHawkActiveReadGopPtpTxTsQueueReg2Reg: illegal queue<n> address\n");
    }
...
#define HAWK_TSU_QUEUE_OFFSET 0x8
...
)
{
...

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00400000, 2097152),SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(384,64), SMEM_BIND_SHARED_TABLE_MAC(arpTable)/*tunnelStartTable*/}
        };
...
)
{
    ...
    {/*start of unit Exact Match */
        ...
        {/*start of emGlobalConfiguration*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emGlobalConfiguration.emGlobalConfiguration1 = 0x0000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emGlobalConfiguration.emCRCHashInitConfiguration = 0x0004;
        }/*end of emGlobalConfiguration*/

        {/*start of emIndirectAccess*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emIndirectAccess.emIndirectAccessControl = 0x00002000;
            {/* 0x00002004 + n*3 */
                GT_U32    n;
                for (n = 0 ; n <= 3 ; n++) {
        ...
        {/*start of emInterrupt*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emInterrupt.emInterruptCauseReg = 0x00004000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emInterrupt.emInterruptMaskReg = 0x00004004;
        }/*end of emInterrupt*/

        {/*start of emSchedulerConfiguration*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emSchedulerConfiguration.emSchedulerSWRRArbiterWeights = 0x00005000;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emSchedulerConfiguration.emSchedulerSWRRArbiterPriority = 0x00005004;
        ...
        {/*start of emAutoLearning*/ /* sip6_10 only*/

            {
                GT_U32    n;
                for (n = 0 ; n <= 15 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emProfileConfiguration1[n] =
                        0x00001000+0x10*n;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emProfileConfiguration2[n] =
                        0x00001004+0x10*n;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emProfileConfiguration3[n] =
                        0x00001008+0x10*n;
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emProfileConfiguration4[n] =
                        0x0000100c+0x10*n;
                }/* end of loop n */
            }
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdAllocationConfiguration1=0x00001100;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdAllocationConfiguration2=0x00001104;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdAllocationStatus1=0x00001108;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdAllocationStatus2=0x0000110c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdAllocationStatus3=0x00001160;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emAutoLearningCollisionStatus1=0x00001110;
            {/* 0x00001114 + n*4 */
                GT_U32    n;
                for (n = 2 ; n <= 13 ; n++) {
                     SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emAutoLearningCollisionStatus_n[n-2] =
                        0x00001114+(n-2)*4 ;
                }/* end of loop n */
            }/* 0x00001114 + (n-2)*4: where n (2-13) represents chunk*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emSemaphore=0x00001144;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emFlowIdFailCounter=0x00001148;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emIndexFailCounter=0x0000114c;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emRateLimiterFailCounter=0x00001150;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emRateLimitConfiguration=0x00001154;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emRateLimitBurstCounter=0x00001158;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emAutoLearning.emRateLimitPacketsCounter=0x0000115c;
    ...
    /* support phoenix */
    {
        ...
        GT_U32  index = 0;
        {
            SMEM_CHUNK_BASIC_STC  chunksMem[]=
            {
                ...
                /*Duplication of Packets to CPU Configuration*/
                ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x0000A100, 0x00000010)}
            };
            ...
            index++;
            /*Duplication of Packets to CPU Configuration*/
            /* Ironman need 2 registers but hold 4 that have default value of 0xFFFFFFFF */
            chunksMem[index].numOfRegisters = MAX(4,(numPhyPorts/32));/* no alignment as it is not 'memory' (rather it is registers) */
            index++;
...

        {
            ...
            {
                /* allow align of width of LTT to sip7 */
                /* before need to align the whole EQ unit */

                /*eqL2EcmpLtt*/
                chunksMem[1].memFirstAddr            = 0x00140000;
                chunksMem[1].enrtyNumBytesAlignement = 8;

                /*eqL2EcmpLtt*/
                chunksMem[2].enrtyNumBytesAlignement = 8;
            }
...
)
{
    ...
    /* SIP7 address is 0x00C00000 + g*0x1000000+i*0x20000
     * where g (0-3) represents GOP, where i (0-7) represents Instance
     */
    addrMask = (SMEM_CHT_IS_SIP7_GET(devObjPtr)) ? 0xFFFE0000 : 0xFFF80000;

    /* Calculate LMU and index */
    for(lmuNum = 0;lmuNum < devObjPtr->numOfLmus;lmuNum++)
    {
        if((address & addrMask) == devObjPtr->memUnitBaseAddrInfo.lmu[lmuNum])
        {
            break;
        }
    }
...
)
{
    ...
    START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
        ...
        {0x00008100, 0xFFFFFFC0,         smemChtActiveReadIntrCauseReg, 0, smemFalconActiveWriteLatencyOverThreshIntrCauseReg, 0},
        {0x00008140, SMEM_FULL_MASK_CNS, smemChtActiveReadIntrCauseReg, 0, smemFalconActiveWriteLatencyOverThreshIntrCauseReg, 0},
        {0x00008180, 0xFFFFFFC0,         NULL, 0, smemChtActiveGenericWriteInterruptsMaskReg,         0},
        {0x000081c0, SMEM_FULL_MASK_CNS, NULL, 0, smemChtActiveGenericWriteInterruptsMaskReg,         0},
        ...
        {0x00008220, SMEM_FULL_MASK_CNS, smemChtActiveReadIntrCauseReg, 0, smemChtActiveWriteIntrCauseReg, 0},
        {0x00008224, SMEM_FULL_MASK_CNS, NULL, 0, smemChtActiveGenericWriteInterruptsMaskReg,         0},

        /* LMU lmu_cause_summary/lmu_mask_summary*/
        {0x00008230, SMEM_FULL_MASK_CNS, NULL, 0, smemChtActiveWriteIntrCauseReg, 0},
...
)
{
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].MTI_EXT_PORT.portControl         = 0x00000084 + portFormula + unitOffset;
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].MTI_EXT_PORT.portPeerDelay       = 0x0000014c + portFormula + unitOffset;
...
)
{
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].USX_MAC_PORT.portControl         = 0x0000002C + portFormula + unitOffset;
    ...
    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->sip6_MTI[portNum].USX_MAC_PORT.portPeerDelay       = 0x00000040 + portFormula + unitOffset;
...
)
{
    ...
    /* MAC 400G EXT - start */
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (MAC_EXT_BASE_OFFSET + 0x00000380, MAC_EXT_BASE_OFFSET + 0x000003C0)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
        SMEM_UNIT_CHUNKS_STC    tmpUnitChunk;
...
/**
* @internal smemHawkActiveWriteRxDmaDebugClearAllCounters function
* @endinternal
*
...
*/
void smemHawkActiveWriteRxDmaDebugClearAllCounters (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
)
{
    ...
    } counterOffsetArr[] =
    {
        ...
        ,{0x2000, 0x2004, 16, 5 }/*RX Ingress Drop Counters*/
        ,{0x2080, 0x2084,  0, 1 }/*RX to CP Counters*/
        ,{0x2100, 0x2104, 16, 2 }/*CP to RX Counters*/
        ,{0x2180, 0x2184, 16, 2 }/*RX Egress Drop Counters*/
        ,{0x2200, 0x2204,  0, 1 }/*RX to PB Counters*/
        ,{0x2280, 0x2284,  0, 1 }/*PB to RX Counters*/
        ,{0x2300, 0x2304,  0, 1 }/*RX to PDX Counters*/
        ,{0x2380, 0x2384,  0, 1 }/*PDX to RX Counters*/
    };
    ...
    {
        for (j = 0; (j < counterOffsetArr[1].numOf); j++)
        {
            smemRegSet(
                devObjPtr,
...
/**
* @internal smemHawkActiveRead_tx_train_if_reg3 function
* @endinternal
*
...
*
*/
void smemHawkActiveRead_tx_train_if_reg3 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
/**
* @internal smemHawkActiveRead_Tx_Reserved_2 function
* @endinternal
*
* @brief   Read Tx_Reserved_2 value.
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemHawkActiveRead_Tx_Reserved_2 (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    /* output value */
    * outMemPtr = (* memPtr) | (1<<0);

    /* support: mvHwsComphyC112GX4SerdesCtleLimitsWrite */
...
#define SMEM_PER_LANE_MASK_CNS (0xFFFFFFF3)/* support 4 lanes */
    ...
    /* bit 4 RX_TRAIN_COMPLETE_LANE for the logic in mvHwsComphyC112GX4SerdesAutoTuneStatusShort */
    {0x00022c20, SMEM_PER_LANE_MASK_CNS, smemHawkActiveRead_tx_train_if_reg3         , 0 ,NULL,0},
    {0x00022060, SMEM_PER_LANE_MASK_CNS, smemHawkActiveRead_Tx_Reserved_2            , 0 ,NULL,0},
...
/**
* @internal smemHawkActiveRead_tx_train_if_reg3_28G function
* @endinternal
*
* @brief   Read tx_train_if_reg3 value.
*
...
*
*/
void smemHawkActiveRead_tx_train_if_reg3_28G (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
{
    /* output value */
    * outMemPtr = (* memPtr) | (1<<24)/*RX_TRAIN_COMPLETE_LANE*/ | (1<<11) /*TX_TRAIN_COMPLETE_LANE*/;
...
)
{
    ...
    /* bit 4 RX_TRAIN_COMPLETE_LANE for the logic in mvHwsComphyC28GP4X1SerdesAutoTuneStatusShort */
    {0x00022600, SMEM_PER_LANE_MASK_CNS, smemHawkActiveRead_tx_train_if_reg3_28G         , 0 ,NULL,0},
    {0x00022060, SMEM_PER_LANE_MASK_CNS, smemHawkActiveRead_Tx_Reserved_2            , 0 ,NULL,0},
...
)
{
    START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
        ...
        {0x00003000 , SMEM_FULL_MASK_CNS - (0x1f << 2), smemChtActiveReadIntrCauseReg, 0, smemChtActiveWriteIntrCauseReg, 0},
        {0x00003400 , SMEM_FULL_MASK_CNS - (0x1f << 2), NULL, 0, smemChtActiveGenericWriteInterruptsMaskReg, 0},
        /* Channel Egress Timestamp Queue 0 Entry High */
        {0x00004004 , SMEM_FULL_MASK_CNS - (0x1f << 3) , smemHawkActiveReadGopPtpTxTsQueueReg2Reg, 0, NULL, 0},
        /* Channel Egress Timestamp Queue 1 Entry High */
        {0x00004804 , SMEM_FULL_MASK_CNS - (0x1f << 3) , smemHawkActiveReadGopPtpTxTsQueueReg2Reg, 0, NULL, 0},
    END_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
    ...
    }
    {
        static SMEM_REGISTER_DEFAULT_VALUE_STC myUnit_registersDefaultValueArr[] =
        {
             {DUMMY_NAME_PTR_CNS,            0x00000004,         0x83aa7e5d,      1,    0x4 }
            ,{DUMMY_NAME_PTR_CNS,            0x00000008,         0x00002020,      8,    0x4 }
            ,{DUMMY_NAME_PTR_CNS,            0x0000003c,         0xffff0000,      1,    0x4 }
            ,{DUMMY_NAME_PTR_CNS,            0x00001400,         0x00000020,      32,    0x4 }
            ,{NULL,                          0,                  0x00000000,      0,    0x0 }
        };
        static SMEM_LINK_LIST_REGISTER_DEFAULT_VALUE_STC list = {myUnit_registersDefaultValueArr,NULL};
        unitPtr->unitDefaultRegistersPtr = &list;
...
/**
* @internal smemHawkUnitCnmRunit function
* @endinternal
*
...
* @param[in,out] unitPtr              - pointer to the unit chunk
*/
static void smemHawkUnitCnmRunit
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    INOUT SMEM_UNIT_CHUNKS_STC  * unitPtr
)
...
)
{
    ...
    SMEM_DEVICE_UNIT_ALLOCATION_STC allocUnitsArr[] = {
        ...
        ,{STR(UNIT_CNM_RUNIT)     ,smemHawkUnitCnmRunit }
...
)
{
    ...
    dpIndex %= devObjPtr->multiDataPath.maxDp;/* the dpIndex needed as local DP in the pipe */
    ...
    *isPreemptiveChannelPtr = isPreemptiveChannel;
    ...
    if(fieldType == SNET_CHT_mif_Tx_channel_id_number_E || SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        ...
        {
            smemRegFldGet(devObjPtr, regAddr ,6 ,1, &txEnabled);
            *valuePtr = txEnabled;
        }
    }
...
)
{
    SMEM_CHT_PORT_MTI_MIF_TYPE_ENT mifType = param;
    GT_U32   dpIndex;
    GT_U32   ii,kk,mifUnitId,baseAddr,mifUnitIdMax=devObjPtr->numOfMifs?devObjPtr->numOfMifs:SIM_MAX_MIF_UNITS;
    GT_U32   local_mif_channel_number;/* local mif channel number */
    GT_U32   local_mac_number;   /*local mac port */
    GT_U32   globalMacPort;      /* global MAC number */
    ...
    *memPtr = *inMemPtr;
    ...
    for(kk = 0 ; kk < 32; kk++)
    {
        ...
        /* this condition must allow to configure ethernet CPU port */
        if ((local_dma_number >= devObjPtr->multiDataPath.info[dpIndex].dataPathNumOfPorts)
            && (local_dma_number != devObjPtr->multiDataPath.info[dpIndex].cpuPortDmaNum))
        {
            /*FE-3290083 - Incorrect default values for assigned MIF channel ID*/
            return;
        }
...
)
{
    ...
    if (isHawk == GT_TRUE)
    {
        ...
        devObjPtr->limitedResources.ipclConfigNum = 6*_1K;/*For Hawk , but not for Phoenix/Harrier/Ironman */
    }
    SET_IF_ZERO_MAC(devObjPtr->limitedResources.portIsolationNum,(4*_1K + 128));
    ...
    devObjPtr->errata.ipMllWrongSetMllRepInBridgedCopy = 1;

    /* This ERRATA exists for HAWK(POLICER-1896) and IRONMAN(POLICER-2027) only */
    if((devObjPtr->deviceFamily == SKERNEL_HAWK_FAMILY) ||
       (devObjPtr->deviceFamily == SKERNEL_IRONMAN_FAMILY))
    {
        devObjPtr->errata.ipfixAlarmIndex = 1;
    }
...
)
{
    ...
    if(virtualPortNum >= (GT_U32)(1 << devObjPtr->flexFieldNumBitsSupport.phyPort))
    {
...
)
{
    ...
    {
        /*Rx enable*/
        SMEM_U32_SET_FIELD(regValue, 14,1,1); /*Rx*/
        /*Tx enable*/
        SMEM_U32_SET_FIELD(regValue, 6,1,1); /*Tx*/
    }
