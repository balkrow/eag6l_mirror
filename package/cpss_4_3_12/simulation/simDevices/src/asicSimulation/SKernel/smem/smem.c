...
)
{
    ...
    switch(deviceObjPtr->deviceFamily)
    {
        ...
        case SKERNEL_IRONMAN_FAMILY:
            ...
            break;
        case SKERNEL_AAS_FAMILY:
            smemAasInit(deviceObjPtr);
            break;
         default:
              skernelFatalError(" smemInit: not valid deviceFamily[%d]",
...
{
    GT_BOOL skipInitStateConfig = GT_FALSE;
...
)
{
    ...
    {
        if(SCIB_MEMORY_WRITE_BAR0_E == accessType ||
            SCIB_MEMORY_READ_BAR0_E == accessType)
        {
            /* support device that not support this memory */
            static GT_U32   dummyBar0 = 0;
            returnMemory = &dummyBar0;
            return  returnMemory;
        }
...
/**
* @internal smemInitSharedFileUniqueName function
* @endinternal
*
* @brief   init unit memory chunk shared file name unique filename
*
*
* @param[in] devObjPtr            - pointer to device Object
* @param[in] srcFileNamePtr       - pointer to source file name (unique inside the specific device type, not unique per multiple WM instances of the same device)
* @param[out] srcFileNamePtr      - pointer to target file name (unique)
*/
GT_STATUS smemInitSharedFileUniqueName
(
    IN SKERNEL_DEVICE_OBJECT  * devObjPtr,
    IN const char*     srcFileNamePtr,
    OUT char**     trgFileNamePtr
)
{
    GT_STATUS status;
    GT_PROCESS_ID pid = 0;

    status = SIM_OS_MAC(simOsProcessIdGet(&pid));

    /* don't check status, pid was pre-initialized to zero,
       if the call fails we will not be able to make a unique
       shared file name */
    *trgFileNamePtr = malloc(strlen(srcFileNamePtr) + 1 +
                      + sizeof(pid)*2 /* char per nibble */ + 1
                      + sizeof(devObjPtr->deviceId)*2 + 1 + 1);

    if (!*trgFileNamePtr)
        return GT_FAIL;

    sprintf(*trgFileNamePtr, "%s_%u_%u",
            srcFileNamePtr,
            pid,
            devObjPtr->deviceId);
...
)
{
    ...
    currChunkPtr->isAllocationOfMemoryForbidden = basicMemChunkPtr->isAllocationOfMemoryForbidden;

    if(!devObjPtr->sharedTablesInfo.isSharedTablesSupported)
    {
        /* ignore the caller , allow device that not supports shared tables , to
           use the max size of table without the SHM unit */
        currChunkPtr->isAllocationOfMemoryForbidden = GT_FALSE;
    }

    if(currChunkPtr->isAllocationOfMemoryForbidden == GT_TRUE)
    {
        /* we allocate minimal value , to bypass 'NULL' checks */
        /* we delay with the allocation of the 'real' size of table */
        basicMemChunkPtr->numOfRegisters = currChunkPtr->enrtyNumWordsAlignement ?
            currChunkPtr->enrtyNumWordsAlignement : 1;/* ignore value from the caller */
    }
    ...
    /* If allocation is of shared memory file, use unique file names
       Based on process ID: */
    currChunkPtr->isAllocationofMemoryShared = basicMemChunkPtr->isAllocationOfMemoryShared;
    if (!basicMemChunkPtr->isAllocationOfMemoryShared)
    ...
    else
    {
        if (smemInitSharedFileUniqueName(devObjPtr,
                                         basicMemChunkPtr->sharedMemFilename,
                                         &currChunkPtr->sharedMemFilename) != GT_OK)
            skernelFatalError("smemInitBasicMemChunk: cannot make unique filename out of %s!\n",
                              basicMemChunkPtr->sharedMemFilename);
        currChunkPtr->memPtr = smemDeviceObjMemoryAlloc__shared(totalMemWords , sizeof(SMEM_REGISTER),
            currChunkPtr->sharedMemFilename, &currChunkPtr->fdSharedMem);
    }
...
)
{
    ...
    for(ii = 0 ; ii < numOfChunks; ii++, currChunkPtr++ )
    {
           ...
           chunksMemArrExt[ii].formulaCellArr[0].step)
        {
            ...
            currChunkPtr->isAllocationOfMemoryForbidden = chunksMemArrExt[ii].memChunkBasic.isAllocationOfMemoryForbidden;
            ...
            if(currChunkPtr->isAllocationOfMemoryForbidden == GT_TRUE)
            {
                /* we allocate minimal value , to bypass 'NULL' checks */
                /* we delay with the allocation of the 'real' size of table */
                memoryWords = currChunkPtr->enrtyNumWordsAlignement ?
                    currChunkPtr->enrtyNumWordsAlignement : 1;/* ignore value from the caller */
            }
            ...
            currChunkPtr->isAllocationofMemoryShared = chunksMemArrExt[ii].memChunkBasic.isAllocationOfMemoryShared;
            if (!chunksMemArrExt[ii].memChunkBasic.isAllocationOfMemoryShared)
                currChunkPtr->memPtr = smemDeviceObjMemoryAlloc__internal(devObjPtr,totalMemWords, sizeof(SMEM_REGISTER),fileNamePtr,line);
            else
            {
                if (smemInitSharedFileUniqueName(devObjPtr,
                                                 chunksMemArrExt[ii].memChunkBasic.sharedMemFilename,
                                                 &currChunkPtr->sharedMemFilename) != GT_OK)
                    skernelFatalError("smemInitBasicMemChunk: cannot make unique filename out of %s!\n",
                                      chunksMemArrExt[ii].memChunkBasic.sharedMemFilename);

                currChunkPtr->memPtr = smemDeviceObjMemoryAlloc__shared(totalMemWords, sizeof(SMEM_REGISTER),
                    currChunkPtr->sharedMemFilename,
                    &currChunkPtr->fdSharedMem);
            }
...
/**
* @internal smemTableEntryFormulaAddressGet function
* @endinternal
*
* @brief   function return the address of the entry in the table.
* @param[in] tablePtr                 - pointer to table info
...
*                                      ignored when == SMAIN_NOT_VALID_CNS
*                                       the address of the specified entry in the specified table
* @param[in] index0                   -  of the entry in the table
* @param[in] index1                   -  of the entry in the table
* @param[in] index2                   -  of the entry in the table
* @param[in] index3                   -  of the entry in the table
* @param[in] index4                   -  of the entry in the table
* @param[in] index5                   -  of the entry in the table
*
* @retval  required register address.
*/
GT_U32  smemTableEntryFormulaAddressGet(
    IN  SKERNEL_TABLE_INFO_STC    *tablePtr,
    IN  GT_U32                    instanceIndex,
    IN  GT_U32                    index0,
    IN  GT_U32                    index1,
...
)
{
    ...
    GT_U32  indexArr[6];
    GT_U32  address;
    SMEM_FORMULA_DIM_STC *formulaPtr;

    baseAddress = tablePtr->commonInfo.baseAddress;
    if ((instanceIndex != SMAIN_NOT_VALID_CNS) &&
        tablePtr->commonInfo.multiInstanceInfo.numBaseAddresses)
    {
        if (instanceIndex >= tablePtr->commonInfo.multiInstanceInfo.numBaseAddresses)
        {
            skernelFatalError("smemTableEntryFormulaAddressGet: instanceIndex[%d] out of range[%d]\n",
                instanceIndex,tablePtr->commonInfo.multiInstanceInfo.numBaseAddresses);
        }
    ...
    formulaPtr = &(tablePtr->commonInfo.memChunkPtr->formulaData[0]);
    address = baseAddress;
    for (ii = 0; (ii < 6); ii++)
    {
        if (indexArr[ii] == 0) continue;
        if (indexArr[ii] >= formulaPtr[ii].size)
        {
            skernelFatalError(
                "smemTableEntryFormulaAddressGet: index numer %d value %d out of range %d\n",
                ii, indexArr[ii], formulaPtr[ii].size);
        }
        address += (indexArr[ii] * formulaPtr[ii].step);
    }
    return address;
}
...
*
* OUTPUTS:
*       enrtyNumWordsAlignementPtr - (pointer to) the number of words of entry alignment
*                                    ignored if NULL
*
* RETURNS:
...
(
    IN SKERNEL_DEVICE_OBJECT *devObjPtr,
    IN GT_U32 address,
    OUT GT_U32 *enrtyNumWordsAlignementPtr
)
{
    GT_U32                  chunkIndex;/*index of the chunk that hold the memory */
    GT_U32                  memIndex;  /* index in the memory that represent the address*/
    ...
    if(enrtyNumWordsAlignementPtr)
    {
        *enrtyNumWordsAlignementPtr = 1;
    }
    ...
    if(GT_FALSE == smemFindMemChunk(devObjPtr, SCIB_MEMORY_READ_E, unitChunksPtr, address,
                                    &chunkIndex, &memIndex))
    {
        /* memory was not found */
        return NULL;
    ...
    if(enrtyNumWordsAlignementPtr)
    {
        *enrtyNumWordsAlignementPtr = chunkPtr->enrtyNumWordsAlignement;
    }
...
/*******************************************************************************
*   smemDevUpdateUnitChunkSize
*
* DESCRIPTION:
*       find the memory in the specified unit chunk , and update it's size
*
* INPUTS:
*       deviceObj   - pointer to device object.
*       address     - address of memory(register or table).
*       newMemNumOfBytes  - new size in num of bytes
*
* OUTPUTS:
*       None.
*
...
*
*******************************************************************************/
void  smemDevUpdateUnitChunkSize
(
    IN SKERNEL_DEVICE_OBJECT *devObjPtr,
    IN GT_U32 address,
    IN GT_U32  newMemNumOfBytes
)
{
    GT_U32                  chunkIndex;/*index of the chunk that hold the memory */
    GT_U32                  memIndex;  /* index in the memory that represent the address*/
    ...
    SMEM_CHT_GENERIC_DEV_MEM_INFO *devMemInfoPtr = devObjPtr->deviceMemory;
    SMEM_CHUNK_STC           *chunkPtr;
    GT_U32  totalMemWords = newMemNumOfBytes / 4;
    SCIB_MEMORY_ACCESS_TYPE accessType = SKERNEL_MEMORY_READ_E;

    /* support multi-tile/pipes , the address is given in tile 0/pipe 0
       and need to be converted according to 'current' tile,pipe */
    ...
    {
        /* memory was not found */
        skernelFatalError("smemDevUpdateUnitChunkSize: memory not found \n");
    }
    chunkPtr = &unitChunksPtr->chunksArray[chunkIndex];

    chunkPtr->isAllocationOfMemoryForbidden = 0;

    if(chunkPtr->calcIndexType != SMEM_INDEX_IN_CHUNK_TYPE_FORMULA_FUNCTION_E)
    {
        /* update the last address */
        chunkPtr->memLastAddr = chunkPtr->memFirstAddr + newMemNumOfBytes - 4;
    }
    ...
    if(chunkPtr->lastEntryWordIndex != 0)
    {
        /* needed support for 'write table on last word' only for memories that
           are not 1 word entry */
        totalMemWords += chunkPtr->enrtyNumWordsAlignement;
    }

    chunkPtr->memSize = totalMemWords;
    chunkPtr->memPtr = smemDeviceObjMemoryRealloc(devObjPtr,chunkPtr->memPtr , totalMemWords * sizeof(SMEM_REGISTER) );
    if(chunkPtr->memPtr == NULL)
    {
        skernelFatalError("smemDevUpdateUnitChunkSize: allocation failed for [%d] bytes \n" ,
                            totalMemWords * sizeof(SMEM_REGISTER));
    }
...
)
{
    ...
    for(pipeId = 0 ; pipeId < numOfPipes ; pipeId++)
    {
        if(pipeId > 0)
        {
            ...
            if(deviceObjPtr->numOfPipesPerTile)
            {
                tileId  = pipeId / deviceObjPtr->numOfPipesPerTile;
                ...
                pipeOffset += tileId * deviceObjPtr->tileOffset;
            }
...
/**
* @internal smemDeviceObjMemoryAlloc__shared function
* @endinternal
*
* @brief   allocate memory using Linux shared memory.
*          this allows external processes to directly access this memory
*
* @param[in] nitems                   - alloc for number of elements (calloc style)
* @param[in] size                     -  of each element         (calloc style)
* @param[in] fileNamePtr              -  file name of shared memory to open
* @param[out] fd                      - file descriptor allocated for shared mem
* ory
*/
GT_PTR smemDeviceObjMemoryAlloc__shared
(
    IN size_t   nitems,
    IN size_t   size,
    IN const char*     fileNamePtr,
...
)
{
    ...
    status = SIM_OS_MAC(simOsSharedMemOpenAndMap)(fileNamePtr, nitems * size, fd, &ptr);
    if (status != GT_OK)
    {
        skernelFatalError("smemDeviceObjMemoryAlloc__shared: cannot mmap shared memory file %s to size %u!\n", fileNamePtr, nitems * size);
        return NULL;
    }
...
)
{
    ...
    GT_U8* reallocPtr;/*reallocate memory needed by the caller*/
    ...
    if(size /*new size*/ > elementPtr->myMemoryNumOfBytes/* old size */)
    {
        /* reset the memory of the 'extra' size,
           as in WM we expect 'calloc' like memory */
        memset(reallocPtr+(elementPtr->myMemoryNumOfBytes/sizeof(*reallocPtr)),
            0,
            size-elementPtr->myMemoryNumOfBytes);
    }

    devObjPtr->totalNumberOfBytesAllocated += size /*new size*/ -
                                              elementPtr->myMemoryNumOfBytes/*old size*/;
...
)
{
    ...
    devObjPtr->totalNumberOfBytesAllocated -= elementPtr->myMemoryNumOfBytes/*old size*/;
...
)
{
    ...
    if(devObjPtr->numOfPipes < 2 && devObjPtr->numOfTiles < 2)
    {
        /***********************************/
...
/**
* @internal smemIsPortEgressLoopbackGet function
* @endinternal
*
* @brief   check if given global port is Egress Loopback port
*          (is in dma_specialPortMappingArr but not any kind of CPU port)
*
* @param[in] devObjPtr                - pointer to device object.
* @param[in] globalDmaNum             - the global port number
*
* return GT_TRUE  - the global port is special Loopback (but not 'CPU SDMA or GOP')
*        GT_FALSE - the global port is NOT Loopback
*/
GT_BOOL smemIsPortEgressLoopbackGet
(
    IN SKERNEL_DEVICE_OBJECT    *devObjPtr,
    IN GT_U32                   globalDmaNum
)
{
    ...
    if ((SMEM_CHT_IS_SIP7_GET(devObjPtr) == 0) ||
        (devObjPtr->dma_specialPortMappingArr == NULL) ||
        (isCpuSdmaPortGet(devObjPtr,globalDmaNum,GT_FALSE, NULL) == GT_TRUE))
    {
        return GT_FALSE;
    }
    ...
    for (index = 0; devObjPtr->dma_specialPortMappingArr[index].globalPortNumber != SMAIN_NOT_VALID_CNS; index++)
    {
        if(devObjPtr->dma_specialPortMappingArr[index].globalPortNumber == globalDmaNum)
        {
            return GT_TRUE;
        }
    }
...
/* check that base address of unit , aligned on it's size */
static void checkUnitAlignmentToSize(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_CHAR* unitName,
...
{
    GT_U32  currAlignment = 1,ii;

    for(ii = 0 ; ii < 32 ; ii++)
    {
        currAlignment = 1 << ii;
        if(unitSize <= currAlignment)
        {
            break;
        }
    }

    if(unitBaseAddr & (currAlignment-1))
    {
        simForcePrintf("Unit [%s] base addr [0x%8.8x] not fit it's size [0x%x] as alignment should be [0x%x] for this size \n",
            unitName , unitBaseAddr , unitSize , currAlignment);
    }
}
...
)
{
    ...
    for(ii = 0 ; unitInfoPtr->size != SMAIN_NOT_VALID_CNS; ii++,unitInfoPtr++)
    {
        ...
        /* check that base address of unit , aligned on it's size */
        checkUnitAlignmentToSize(devObjPtr,
            unitInfoPtr->nameStr,
            unitInfoPtr->base_addr,
            unitInfoPtr->size);
...
static GT_CHAR* hiddenMemNames[SMEM_GENERIC_HIDDEN_MEM___LAST___E+1] = {
    ...
    ,STR(AAC_MEM_CHANNEL0  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL0_E*/
    ,STR(AAC_MEM_CHANNEL1  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL1_E*/
    ,STR(AAC_MEM_CHANNEL2  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL2_E*/
    ,STR(AAC_MEM_CHANNEL3  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL3_E*/
    ,STR(AAC_MEM_CHANNEL4  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL4_E*/
    ,STR(AAC_MEM_CHANNEL5  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL5_E*/
    ,STR(AAC_MEM_CHANNEL6  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL6_E*/
    ,STR(AAC_MEM_CHANNEL7  )/*SMEM_GENERIC_HIDDEN_MEM_AAC_MEM_CHANNEL7_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_0_E  )/*EXACT_MATCH_TILE_0_TABLE_0_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_1_E  )/*EXACT_MATCH_TILE_0_TABLE_1_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_2_E  )/*EXACT_MATCH_TILE_0_TABLE_2_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_3_E  )/*EXACT_MATCH_TILE_0_TABLE_3_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_4_E  )/*EXACT_MATCH_TILE_0_TABLE_4_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_5_E  )/*EXACT_MATCH_TILE_0_TABLE_5_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_6_E  )/*EXACT_MATCH_TILE_0_TABLE_6_E*/
    ,STR(EXACT_MATCH_TILE_0_TABLE_7_E  )/*EXACT_MATCH_TILE_0_TABLE_7_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_0_E  )/*EXACT_MATCH_TILE_1_TABLE_0_E*/
    ,STR(EXACT_MATCH_TILE_2_TABLE_1_E  )/*EXACT_MATCH_TILE_1_TABLE_1_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_2_E  )/*EXACT_MATCH_TILE_1_TABLE_2_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_3_E  )/*EXACT_MATCH_TILE_1_TABLE_3_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_4_E  )/*EXACT_MATCH_TILE_1_TABLE_4_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_5_E  )/*EXACT_MATCH_TILE_1_TABLE_5_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_6_E  )/*EXACT_MATCH_TILE_1_TABLE_6_E*/
    ,STR(EXACT_MATCH_TILE_1_TABLE_7_E  )/*EXACT_MATCH_TILE_1_TABLE_7_E*/
    ,STR(LPM_TILE_0_BANK_0_E)
    ,STR(LPM_TILE_0_BANK_1_E)
    ,STR(LPM_TILE_0_BANK_2_E)
    ,STR(LPM_TILE_0_BANK_3_E)
    ,STR(LPM_TILE_0_BANK_4_E)
    ,STR(LPM_TILE_0_BANK_5_E)
    ,STR(LPM_TILE_0_BANK_6_E)
    ,STR(LPM_TILE_0_BANK_7_E)
    ,STR(LPM_TILE_0_BANK_8_E)
    ,STR(LPM_TILE_0_BANK_9_E)
    ,STR(LPM_TILE_0_BANK_10_E)
    ,STR(LPM_TILE_0_BANK_11_E)
    ,STR(LPM_TILE_0_BANK_12_E)
    ,STR(LPM_TILE_0_BANK_13_E)
    ,STR(LPM_TILE_0_BANK_14_E)
    ,STR(LPM_TILE_0_BANK_15_E)
    ,STR(LPM_TILE_0_BANK_16_E)
    ,STR(LPM_TILE_0_BANK_17_E)
    ,STR(LPM_TILE_0_BANK_18_E)
    ,STR(LPM_TILE_0_BANK_19_E)
    ,STR(LPM_TILE_0_BANK_20_E)
    ,STR(LPM_TILE_0_BANK_21_E)
    ,STR(LPM_TILE_0_BANK_22_E)
    ,STR(LPM_TILE_0_BANK_23_E)
    ,STR(LPM_TILE_0_BANK_24_E)
    ,STR(LPM_TILE_0_BANK_25_E)
    ,STR(LPM_TILE_0_BANK_26_E)
    ,STR(LPM_TILE_0_BANK_27_E)
    ,STR(LPM_TILE_0_BANK_28_E)
    ,STR(LPM_TILE_0_BANK_29_E)
    ,STR(LPM_TILE_0_BANK_30_E)
    ,STR(LPM_TILE_0_BANK_31_E)
    ,STR(LPM_TILE_0_BANK_32_E)
    ,STR(LPM_TILE_0_BANK_33_E)
    ,STR(LPM_TILE_0_BANK_34_E)
    ,STR(LPM_TILE_0_BANK_35_E)
    ,STR(LPM_TILE_0_BANK_36_E)
    ,STR(LPM_TILE_0_BANK_37_E)
    ,STR(LPM_TILE_0_BANK_38_E)
    ,STR(LPM_TILE_0_BANK_39_E)
    ,STR(LPM_TILE_0_BANK_40_E)
    ,STR(LPM_TILE_0_BANK_41_E)
    ,STR(LPM_TILE_0_BANK_42_E)
    ,STR(LPM_TILE_0_BANK_43_E)
    ,STR(LPM_TILE_0_BANK_44_E)
    ,STR(LPM_TILE_0_BANK_45_E)
    ,STR(LPM_TILE_0_BANK_46_E)
    ,STR(LPM_TILE_0_BANK_47_E)
    ,STR(LPM_TILE_1_BANK_0_E)
    ,STR(LPM_TILE_1_BANK_1_E)
    ,STR(LPM_TILE_1_BANK_2_E)
    ,STR(LPM_TILE_1_BANK_3_E)
    ,STR(LPM_TILE_1_BANK_4_E)
    ,STR(LPM_TILE_1_BANK_5_E)
    ,STR(LPM_TILE_1_BANK_6_E)
    ,STR(LPM_TILE_1_BANK_7_E)
    ,STR(LPM_TILE_1_BANK_8_E)
    ,STR(LPM_TILE_1_BANK_9_E)
    ,STR(LPM_TILE_1_BANK_10_E)
    ,STR(LPM_TILE_1_BANK_11_E)
    ,STR(LPM_TILE_1_BANK_12_E)
    ,STR(LPM_TILE_1_BANK_13_E)
    ,STR(LPM_TILE_1_BANK_14_E)
    ,STR(LPM_TILE_1_BANK_15_E)
    ,STR(LPM_TILE_1_BANK_16_E)
    ,STR(LPM_TILE_1_BANK_17_E)
    ,STR(LPM_TILE_1_BANK_18_E)
    ,STR(LPM_TILE_1_BANK_19_E)
    ,STR(LPM_TILE_1_BANK_20_E)
    ,STR(LPM_TILE_1_BANK_21_E)
    ,STR(LPM_TILE_1_BANK_22_E)
    ,STR(LPM_TILE_1_BANK_23_E)
    ,STR(LPM_TILE_1_BANK_24_E)
    ,STR(LPM_TILE_1_BANK_25_E)
    ,STR(LPM_TILE_1_BANK_26_E)
    ,STR(LPM_TILE_1_BANK_27_E)
    ,STR(LPM_TILE_1_BANK_28_E)
    ,STR(LPM_TILE_1_BANK_29_E)
    ,STR(LPM_TILE_1_BANK_30_E)
    ,STR(LPM_TILE_1_BANK_31_E)
    ,STR(LPM_TILE_1_BANK_32_E)
    ,STR(LPM_TILE_1_BANK_33_E)
    ,STR(LPM_TILE_1_BANK_34_E)
    ,STR(LPM_TILE_1_BANK_35_E)
    ,STR(LPM_TILE_1_BANK_36_E)
    ,STR(LPM_TILE_1_BANK_37_E)
    ,STR(LPM_TILE_1_BANK_38_E)
    ,STR(LPM_TILE_1_BANK_39_E)
    ,STR(LPM_TILE_1_BANK_40_E)
    ,STR(LPM_TILE_1_BANK_41_E)
    ,STR(LPM_TILE_1_BANK_42_E)
    ,STR(LPM_TILE_1_BANK_43_E)
    ,STR(LPM_TILE_1_BANK_44_E)
    ,STR(LPM_TILE_1_BANK_45_E)
    ,STR(LPM_TILE_1_BANK_46_E)
    ,STR(LPM_TILE_1_BANK_47_E)
...
)
{
    ...
    if(!devObjPtr->fdbNoDirectMemory && memType == SMEM_GENERIC_HIDDEN_MEM_FDB_E)
    {
        skernelFatalError("smemGenericHiddenFindMem: FDB table need to be accessed directly (not as 'hidden' memory) \n");
    }


    isFdbShared = (memType == SMEM_GENERIC_HIDDEN_MEM_FDB_E) &&
                    devObjPtr->sharedTablesInfo.isFdbShared;

    if(((memType >= SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_0_BANK_0_E)&&
       (memType <= SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_0_BANK_0_E+47))||
       ((memType >= SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_1_BANK_0_E)&&
       (memType <= SMEM_GENERIC_HIDDEN_MEM_LPM_TILE_1_BANK_0_E+47)))
    {
        instanceId = 0;
    }
    else
    {
        switch(memType)
        {
            ...
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_0_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_1_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_2_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_3_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_4_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_5_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_6_E:
            case SMEM_GENERIC_HIDDEN_MEM_EXACT_MATCH_TILE_0_TABLE_7_E:
    ...
    {
        if(isFdbShared)/* allow the auto-aging to scan the table even though there is no table yet */
        {
            static GT_U32   dummyMem[4] = {0,0,0,0};
            /* return pointer to dummy zeroed memory */
            return &dummyMem[0];
        }
    ...
    if(entryIndex >= infoPtr->numOfEntries)
    {
        skernelFatalError("smemGenericHiddenFindMem: type to access memType[%s] at index[0x%8.8x] but max is[0x%8.8x] \n",
            hiddenMemNames[memType],
            entryIndex,
...
static GT_STATUS skernelPortApPortHcdFound
(
    IN  GT_U32   deviceNumber,
    IN  GT_U32   port,
    IN  GT_BOOL  found
)
{
    ...
    if(!CHT_IS_MTI_MAC_USED_MAC(devObjPtr,port))
    {
        /* not supported */
        return GT_OK;
    }
    ...
    if(found == GT_TRUE &&
       GT_TRUE == snetChtPortMacFieldGet(devObjPtr, port,
        SNET_CHT_PORT_MAC_FIELDS_LinkState_E))
    {
        ...
        {
            devObjPtr->devMemSerdesLinkChangedPtr(devObjPtr,port,GT_TRUE);
        }
    }
...
/*  enum to state the 'IPC of CPSS to WM' message types.
    ...
    SIM_IPC_TYPE_AP_PORT_HCD_FOUND_E - message to notify WM that AP port got HCD found on the port.
                                The WM need to issue link change interrupt about the port.
                                otherwise the port manager will not have it's 'low level' mac link change.
...
enum {
    ...
    SIM_IPC_TYPE_AP_PORT_HCD_FOUND_E,
...
*/
            ...
            case SIM_IPC_TYPE_AP_PORT_HCD_FOUND_E:
                skernelPortApPortHcdFound(params[0],params[1],params[2]);
                break;
...
)
{
    SKERNEL_DEVICE_OBJECT* devObjPtr = smemTestDeviceIdToDevPtrConvert(wmDevNum);
    GT_U32  baseAddr;
    GT_U32  replyOffset = 0x000FFFF0;
    GT_U32  *startOfMemoryPtr;

    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        /* no MG unit , so use dedicated UNIT_WM_IPC unit */
        baseAddr = smemUnitBaseAddrByNameGet(devObjPtr,STR(UNIT_WM_IPC),0);
        replyOffset = 0xf0;
    }
    else
    ...
    startOfMemoryPtr = smemMemGet(devObjPtr,baseAddr + replyOffset);
...
/**
* @internal smemRegGet_noAddrConvert function
* @endinternal
*
* @brief   Read data from the register.
*
* @param[out] data                     - pointer to register's value.
*
* @note the function like smemRegGet , but not do smemConvertDevAndAddrToNewDevAndAddr
*       this is needed for interrupts mechanize that already hold final MG register addresses
*
*/
...
/**
* @internal smemRegSet_noAddrConvert function
* @endinternal
*
* @brief   Write data to the register.
*
* @note the function like smemRegSet , but not do smemConvertDevAndAddrToNewDevAndAddr
*       this is needed for interrupts mechanize that already hold final MG register addresses
*
*/
...
)
{
    ...
    for(ii = 0 ; ii < numOfFull32Bits; ii++)
    {
        ...
        value = snetFieldValueGet(sourceMemPtr,sourceStartBit + (ii*32), 32);

        /* set 32 bits into target */
        snetFieldValueSet(targetMemPtr,targetStartBit + (ii*32), 32,value);
    }

    if(numBitsLastWord)
    {
        /* get last less than 32 bits from source */
        value = snetFieldValueGet(sourceMemPtr,sourceStartBit + (ii*32), numBitsLastWord);

        /* set last less than 32 bits into target */
        snetFieldValueSet(targetMemPtr,targetStartBit + (ii*32), numBitsLastWord,value);
    }
