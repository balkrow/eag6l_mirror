...
{ /*tableOffsetValid,tableOffsetInBytes*/                           /*startAddr*/ /*lastAddr*/    /*table_unitType_deviceAccess*/  /*table_unitType_cpu_access*/
    /* HA : shared tables between 2 pipes */
     {TABLE_SHARED_2_PIPES_MAC(arpTable)}
...
typedef enum {
    ...
    ,MTI_ACTIVE_MEM_REG_TSU_E                               /* formula for TSU memory */
...
)
{
    ...
    if(regAddress < TILE_OFFSET)
    {
        ...
        else if (regAddress >= 0x1BF00000 && regAddress < 0x1C000000)
        {
            /* TAI 0 master or TAI 1 master support all tails from single unit */
            return 0;
        }
        else
        {
...
{
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00096000, 4096), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(20, 4),SMEM_BIND_TABLE_MAC(egfQagPortTargetAttribute)}
...

    {
        static SMEM_REGISTER_DEFAULT_VALUE_STC myUnit_registersDefaultValueArr[] =
        {
            ...
            /* egfQagPortTargetAttribute */
           ,{DUMMY_NAME_PTR_CNS,    0x00096000,         0x00000040,   1024,    0x4}
           ,{NULL,                  0,                  0x00000000,      0,    0x0}
        };
        static SMEM_LINK_LIST_REGISTER_DEFAULT_VALUE_STC list = {myUnit_registersDefaultValueArr,NULL};
        unitPtr->unitDefaultRegistersPtr = &list;
...
)
{
...

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00200000, 1572864 ), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(384,64) , SMEM_BIND_SHARED_TABLE_MAC(arpTable)/*tunnelStart*/}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
{
    GT_U32   maxAnalyzer = SMEM_CHT_IS_SIP7_GET(devObjPtr)? 62 : 6;/* RegOffset - Maximum analyzer index */
    GT_U32   PRPGlobalConfigurationAddr = SMEM_CHT_IS_SIP7_GET(devObjPtr)? 0x00001150 : 0x00001050;
    {
        START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00002044, 0x00002140)}
    ...
    if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (PRPGlobalConfigurationAddr, PRPGlobalConfigurationAddr)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
        SMEM_UNIT_CHUNKS_STC    tmpUnitChunk;
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            static SMEM_REGISTER_DEFAULT_VALUE_STC myUnit_registersDefaultValueArr[] =
            {
                { DUMMY_NAME_PTR_CNS,   0x00001150,     0x000088FB ,     1,      0x0 }
...

    {
        ...
        if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
        {
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EREP.configurations.PRP_Global_Configuration = PRPGlobalConfigurationAddr;
        }
    }
}
...
#define  fdbEntryNumBytes_align 16 /* entry is 115 bits */

    {
        ...
        {
            infoPtr->startOfMemPtr = smemDeviceObjMemoryAlloc__internal(devObjPtr,numEntries,fdbEntryNumBytes_align,__FILE__,__LINE__);
            if(infoPtr->startOfMemPtr == NULL)
            {
...
)
{
    ...
    *memPtr = *inMemPtr;/* save value that written by the CPU*/
    ...
    {
        GT_U32  currentPipeId,currentTiledId;

        currentPipeId = smemGetCurrentPipeId(devObjPtr);
        currentTiledId = currentPipeId / devObjPtr->numOfPipesPerTile;
...
)
{
    ...
    if ((devObjPtr->deviceFamily == SKERNEL_PHOENIX_FAMILY)
        || (devObjPtr->deviceFamily == SKERNEL_IRONMAN_FAMILY))
    {
        START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
    ...
    else
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00001530, 0x00001534)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00001540, 0x00001544)}
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00002000, 0x00002024)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
/**
* @internal smemFalconSharedTablesForPipe1 function
* @endinternal
*
* @brief   update ranges of tables that are shared between pipe 0 and pipe 1
*
* @param[in] devObjPtr                - pointer to device object of pipe 0
*/
static void smemFalconSharedTablesForPipe1
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr
)
{
...
/**
* @internal smemFalconUnitLpmTable function
* @endinternal
*
...
*/
static void smemFalconUnitLpmTable
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_U32                  tileId,
    IN GT_U32                  numEntries
)
{
    ...
    {{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC(0x00000000, ((14*1024)*16)),
         SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(115, 16),SMEM_BIND_SHARED_TABLE_MAC(lpmMemory)},
         FORMULA_SINGLE_PARAMETER(30, (32*1024)*16)},
#endif /*0*/

    /* not know how to split the banks to large and small banks !!!
       so we allocated the size of 30 LPM banks */
    smemDevUpdateUnitChunkSize(devObjPtr,
        devObjPtr->tablesInfo.lpmMemory.commonInfo.baseAddress,
        devObjPtr->lpmRam.numOfLpmRams *
        devObjPtr->lpmRam.perRamNumEntries * 16/* num of bytes */ );

    if(tileId == 0)/* assuming all tiles with the same tables */
    {
        /* called after smemDevUpdateUnitChunkSize that updated chunk->lastAddr */
        smemFalconSharedTablesForPipe1(devObjPtr);
    }
}

/**
* @internal smemFalconUnitHaTsTable function
* @endinternal
*
* @brief   Allocate address type specific memories -- for the TS/ARP/NAT TABLE
*
* @param[in] devObjPtr            - pointer to device memory.
* @param[in] tileId               - the tileId of the TS/ARP/NAT TABLE
*/
static void smemFalconUnitHaTsTable
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_U32                  tileId,
    IN GT_U32                  numEntries
)
{
    ...
    devObjPtr->tablesInfo.tunnelStartTable.paramInfo[0].outOfRangeIndex =
        numEntries * devObjPtr->tablesInfo.tunnelStartTable.paramInfo[0].divider;/* support 1 arps in single line */
...
/**
* @internal smemFalconActiveWriteShmSizeForWm function
* @endinternal
*
...
*/
void smemFalconActiveWriteShmSizeForWm (
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
    IN         GT_U32 * memPtr,
...
)
{
    ...
    GT_U32  wmSize;     /* bits 8..24 (in 1K values)*/
    ...
    *memPtr = *inMemPtr;
    ...
    wmTableType = value & 0xFF;/* 8 bits */
    wmSize      = (value >> 8) & 0x1FFFF;  /* 17 bits is 128K (times 1K)*/
    ...
    if(wmTableType == WM_TABLE_FDB_E && devObjPtr->sharedTablesInfo.isFdbShared)
    {
        ...
        smemFalconUnitFdbTable(devObjPtr,tileId,wmSize*1024);
    }
    else
    if(wmTableType == WM_TABLE_EM_E && devObjPtr->sharedTablesInfo.isEmShared)
    {
        /* allocate the Exact Match hidden memory */
        /* (according to it's size in the SHM unit) */
        smemFalconUnitExactMatchTable(devObjPtr,tileId,wmSize*1024);
    }
    else
    if(wmTableType == WM_TABLE_LPM_E && devObjPtr->sharedTablesInfo.isLpmShared)
    {
        ...
        /* (according to it's size in the SHM unit) */
        smemFalconUnitLpmTable(devObjPtr,tileId,wmSize*1024);
    }
    else
    if(wmTableType == WM_TABLE_TS_E && devObjPtr->sharedTablesInfo.isTsShared)
    {
        /* allocate the TS/ARP/NAT memory */
        /* (according to it's size in the SHM unit) */
        smemFalconUnitHaTsTable(devObjPtr,tileId,wmSize*1024);
    }
    else
...
)
{
    START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
        ...
        {SHM_ADDR_REG_FOR_WM_TABLE_SIZE , SMEM_FULL_MASK_CNS, NULL, 0, smemFalconActiveWriteShmSizeForWm, 0},
    END_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            /* for WM purpose only : see smemFalconActiveWriteEm_sizeForWm */
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (SHM_ADDR_REG_FOR_WM_TABLE_SIZE, SHM_ADDR_REG_FOR_WM_TABLE_SIZE)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
    ...
    */
...

    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            /* patch for sip7 */
            /*,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_END_ADDR_MAC (0x00000020, 0x0000003C)}*/
            chunksMem[1].numOfRegisters = 9;/*to support 0x00000040*/
        }
...
/*EM*/
    ...
    {/*start of unit Exact Match */
        ...
        {/*start of emGlobalConfiguration*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emGlobalConfiguration.emGlobalConfiguration1 = 0x0004;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emGlobalConfiguration.emGlobalConfiguration2 = 0x0008;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emGlobalConfiguration.emCRCHashInitConfiguration = 0x000c;
        }/*end of emGlobalConfiguration*/

        {/*start of emIndirectAccess*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emIndirectAccess.emIndirectAccessControl = 0x0130;
            {/* 0x134 + n*4 */
                GT_U32    n;
                for (n = 0 ; n <= 4 ; n++) {
                    SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emIndirectAccess.emIndirectAccessData[n] =
                        0x134+4*n;
                }/* end of loop n */
            }/* 0x134 + n*4 */
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emIndirectAccess.emIndirectAccessAccess = 0x0150;
        }/*end of emIndirectAccess*/

        {/*start of emInterrupt*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emInterrupt.emInterruptCauseReg = 0x01b0;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emInterrupt.emInterruptMaskReg = 0x01b4;
        }/*end of emInterrupt*/
        ...
        {/*start of emHashResults*/
            {/* 0x300 + n*4 */
                GT_U32    n;
                for (n = 0 ; n <= 7 ; n++) {
                     SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emHashResults.emMultiHashCRCResultReg[n] =
                        0x300+n*4 ;
                }/* end of loop n */
            }/* 0x300 + n*4 */
        ...
        {/*start of emSchedulerConfiguration*/
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emSchedulerConfiguration.emSchedulerSWRRControl = 0x0380;
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT].emSchedulerConfiguration.emSchedulerSWRRConfig = 0x0384;
...
/*MLL*/
...

    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
            ...
            ,{SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00100000, 524288), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(162, 32) , SMEM_BIND_TABLE_MAC(mllTable)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
        SMEM_UNIT_CHUNKS_STC    tmpUnitChunk;
...
/*
globalInterruptCause             = 0x00300030 ;
globalInterruptMask              = 0x00300034 ;
mgInternalInterruptCause         = 0x00300038 ;
mgInternalInterruptMask          = 0x0030003c ;
portsInterruptCause              = 0x00300080 ;
portsInterruptMask               = 0x00300084 ;
mg1InternalInterruptCause        = 0x0030009c ;
mg1InternalInterruptMask         = 0x003000A0 ;
dfxInterruptCause                = 0x003000AC ;
dfxInterruptMask                 = 0x003000B0 ;
ports1InterruptsSummaryCause     = 0x00300150 ;
ports1InterruptsSummaryMask      = 0x00300154 ;
functionalInterruptsSummaryCause = 0x003003F8 ;
functionalInterruptsSummaryMask  = 0x003003FC ;
functional1InterruptsSummaryCause= 0x003003F0 ;
functional1InterruptsSummaryMask = 0x003003F4 ;
*/
...
)
{
    ...
    GT_U32                             mgUnitOffset = 0x00300000;


    {/* Raven : start of MG unit interrupts */

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].functionalInterruptsSummaryCause = 0x000003F8 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].functionalInterruptsSummaryMask = 0x000003FC + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].functionalInterruptsSummary1Cause = 0x000003F4 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].functionalInterruptsSummary1Mask = 0x000003F0 + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].ports1InterruptsSummaryCause = 0x00000150 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].ports1InterruptsSummaryMask  = 0x00000154 + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].ports2InterruptsSummaryCause = 0x00000158 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].ports2InterruptsSummaryMask  = 0x0000015c + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].dfx1InterruptsSummaryCause   = 0x000000B8 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].dfx1InterruptsSummaryMask    = 0x000000BC + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].globalInterruptCause = 0x00000030 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].globalInterruptMask = 0x00000034 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].miscellaneousInterruptCause = 0x00000038 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].miscellaneousInterruptMask = 0x0000003c + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].stackPortsInterruptCause = 0x00000080 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].stackPortsInterruptMask = 0x00000084 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].SERInterruptsSummary = 0x000000a4 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].SERInterruptsMask = 0x000000a8 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].dfxInterruptsSummaryCause = 0x000000AC + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].dfxInterruptsSummaryMask  = 0x000000B0 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].mg1InternalInterruptsSummaryCause   = 0x0000009C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[0].mg1InternalInterruptsSummaryMask    = 0x000000A0 + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].globalInterruptCause = 0x00000610 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].globalInterruptMask = 0x00000614 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].miscellaneousInterruptCause = 0x00000618 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].miscellaneousInterruptMask = 0x0000061C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].stackPortsInterruptCause = 0x00000620 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].stackPortsInterruptMask = 0x00000624 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].SERInterruptsSummary = 0x00000628 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].SERInterruptsMask = 0x0000062c + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].functionalInterruptsSummaryCause = 0x0000064c + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].functionalInterruptsSummaryMask = 0x00000650 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].functionalInterruptsSummary1Cause = 0x00000648 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].functionalInterruptsSummary1Mask = 0x00000644 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].ports1InterruptsSummaryCause = 0x00000160 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].ports1InterruptsSummaryMask = 0x00000164 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].ports2InterruptsSummaryCause = 0x00000168 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].ports2InterruptsSummaryMask = 0x0000016c + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].dfxInterruptsSummaryCause = 0x00000630 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].dfxInterruptsSummaryMask =0x00000634 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].dfx1InterruptsSummaryCause = 0x0000638 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].dfx1InterruptsSummaryMask = 0x0000063c + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].mg1InternalInterruptsSummaryCause   = 0x00000144 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[1].mg1InternalInterruptsSummaryMask    = 0x00000148 + mgUnitOffset;

        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].globalInterruptCause = 0x00000670 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].globalInterruptMask = 0x00000674 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].miscellaneousInterruptCause = 0x00000678 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].miscellaneousInterruptMask = 0x0000067C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].stackPortsInterruptCause = 0x00000680 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].stackPortsInterruptMask = 0x00000684 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].SERInterruptsSummary = 0x00000688 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].SERInterruptsMask = 0x0000068C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].functionalInterruptsSummaryCause = 0x000006AC + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].functionalInterruptsSummaryMask = 0x000006B0 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].functionalInterruptsSummary1Cause = 0x000006A8 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].functionalInterruptsSummary1Mask = 0x000006A4 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].ports1InterruptsSummaryCause = 0x00000170 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].ports1InterruptsSummaryMask = 0x00000174 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].ports2InterruptsSummaryCause = 0x00000178 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].ports2InterruptsSummaryMask = 0x0000017C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].dfxInterruptsSummaryCause = 0x00000690 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].dfxInterruptsSummaryMask =0x00000694 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].dfx1InterruptsSummaryCause = 0x00000698 + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].dfx1InterruptsSummaryMask = 0x0000069C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].mg1InternalInterruptsSummaryCause   = 0x0000010C + mgUnitOffset;
        regAddrDbSip5Ptr->MG_RAVEN[0].globalInterrupt[2].mg1InternalInterruptsSummaryMask    = 0x00000114 + mgUnitOffset;

        /* NO SDMA in MG of Raven , so no interrupts for it */
    }
...
/**
* @internal smemFalconActiveReadGopPtpTxTsQueueReg2Reg function
* @endinternal
*
...
* @param[out] outMemPtr                - Pointer to the memory to copy register's content.
*/
static void smemFalconActiveReadGopPtpTxTsQueueReg2Reg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    DECLARE_FUNC_NAME(smemFalconActiveReadGopPtpTxTsQueueReg2Reg);

    GT_U32 egressPort;
    GT_U32 queueNum;
    *outMemPtr = *memPtr;

    egressPort = smemFalconMtiPortByAddrGet(devObjPtr, address, (MTI_ACTIVE_MEM_REG_ENT)param);

     __LOG(("smemFalconActiveReadGopPtpTxTsQueueReg2Reg , address 0x%8.8x, egress port: %d",address, egressPort));
    queueNum = 0xFFFFFFFF;
    ...
    {
        skernelFatalError("smemFalconActiveReadGopPtpTxTsQueueReg2Reg: illegal queue<n> address\n");
    }
...
)
{
    ...
    START_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
        ...
        /*/Cider/EBU/Falcon/Falcon {Current}/Raven_Full/<Raven>Raven_A0/<Raven> Device Units/<GOP TAP 0>GOP TAP 0/<GOP TAP 0> <MPF>MPF/<TSU> TSU/TSU Units %a %b*/
        /*SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->GOP_PTP.PTP[portNum].PTPTXTimestampQueue0Reg1*/
        /*0x00405000 + 0x80000*a + 0x4000*b: where a (0-1) represents mpf num, where b (0-7) represents mpfs num*/
        {0x00405028, SMEM_FULL_MASK_CNS-GOP_CHANNEL_OFFSET-0x1C000, smemFalconActiveReadGopPtpTxTsQueueReg2Reg, MTI_ACTIVE_MEM_REG_TSU_E, NULL, 0},
        /*0x00405000 + 0x80000*a + 0x4000*b: where a (0-1) represents mpf num, where b (8-9) represents mpfs num*/
        {0x00405028+(8*0x4000), SMEM_FULL_MASK_CNS-GOP_CHANNEL_OFFSET-0x4000, smemFalconActiveReadGopPtpTxTsQueueReg2Reg, MTI_ACTIVE_MEM_REG_TSU_E, NULL, 0},

        /*/Cider/EBU/Falcon/Falcon {Current}/Raven_Full/<Raven>Raven_A0/<Raven> Device Units/<GOP TAP 0>GOP TAP 0/<GOP TAP 0> <MPF>MPF/<TSU> TSU/TSU Units %a %b*/
        /*SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->GOP_PTP.PTP[portNum].PTPTXTimestampQueue1Reg1*/
        /*0x00405000 + 0x80000*a + 0x4000*b: where a (0-1) represents mpf num, where b (0-7) represents mpfs num*/
        {0x00405030, SMEM_FULL_MASK_CNS-GOP_CHANNEL_OFFSET-0x1C000, smemFalconActiveReadGopPtpTxTsQueueReg2Reg, MTI_ACTIVE_MEM_REG_TSU_E, NULL, 0},
        /*0x00405000 + 0x80000*a + 0x4000*b: where a (0-1) represents mpf num, where b (8-9) represents mpfs num*/
        {0x00405030+(8*0x4000), SMEM_FULL_MASK_CNS-GOP_CHANNEL_OFFSET-0x4000, smemFalconActiveReadGopPtpTxTsQueueReg2Reg, MTI_ACTIVE_MEM_REG_TSU_E, NULL, 0},
...
#if 0 /* those not exists in the 'interrupt tree' of the simulation */
...
/**
* @internal smemFalconAllocTablesInSharedTables function
* @endinternal
*
* @brief   allocate memories for tables that are in the shared-tables
* @param[in] devObjPtr                - pointer to device object.
*                                       Pointer to object for specific subunit
*/
static void smemFalconAllocTablesInSharedTables
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr
)
{
    ...
    {
        if(!devObjPtr->sharedTablesInfo.isFdbShared)
        {
            /* allocate the FDB hidden memory */
            smemFalconUnitFdbTable(devObjPtr,tileId,devObjPtr->fdbMaxNumEntries);
        }
        else
        {
            /* the FDB table allocated only after CPSS sets proper 'allocation' in the SHM unit */
        }

        if(!devObjPtr->sharedTablesInfo.isEmShared)
        {
            /* allocate the Exact Match hidden memory */
            smemFalconUnitExactMatchTable(devObjPtr,tileId,devObjPtr->emMaxNumEntries);
        }
        else
        {
            /* the EM table allocated only after CPSS sets proper 'allocation' in the SHM unit */
        }
    }

    /* see tables indicated with : SMEM_BIND_SHARED_TABLE_MAC */
}
...
)
{
    ...
    /* allocate memories for tables that are in the shared-tables */
    smemFalconAllocTablesInSharedTables(devObjPtr);
...
{
    {
        ...
        /* all Debug Counters are read only, but not clear on read */
        END_BIND_UNIT_ACTIVE_MEM_MAC(devObjPtr,unitPtr)
    }
...
static char* addrDecodeUnitNames[11] = {
    ...
    ,STR(UNIT_CNM_ADDR_DECODER_9)
    ,STR(UNIT_CNM_ADDR_DECODER_10)
};
...
)
{
    ...
    {
        /*per MG base address*/
        if(devObjPtr->mgUnitsBaseAddrArr)
        {
            baseAddr = devObjPtr->mgUnitsBaseAddrArr[mgId];
        }
        else
        {
...
)
{
    ...
    /* Address space */
    {
        SMEM_CHUNK_BASIC_STC  chunksMem[]=
        {
           {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_SHARED_MEM_MAC (0x00000000, 65536, "CnmSram")}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
...
)
{
    ...
    ALIGN_REG_DB_TO_UNIT_MAC(UNIT_EM,EXACT_MATCH[SIP6_EXACT_MATCH_UNIT_NUM_DEFAULT]);
...
)
{
    ...
    devObjPtr->errata.upChangesInEgressMirroredPackets = 1;/*EREP-43 : UP changes in Egress Mirrored packets*/
...
)
{
    ...
    if (SMEM_CHT_IS_SIP7_GET(devObjPtr)) /*AAS*/
    {
    ...
    if (SMEM_CHT_IS_SIP6_30_GET(devObjPtr)) /*Ironman*/
    {
        regAddr = SMEM_CHT_MAC_REG_DB_DFX_SERVER_GET(devObjPtr)->resetAndInitCtrller.
                         DFXServerUnitsBC2SpecificRegs.deviceCtrl26;
        startBit = 22;
    }
    else
    ...
    if(GT_TRUE == setOrGet)
    {
        smemRegFldSet(devObjPtr,regAddr, startBit, 10, *valuePtr);
    }
    else
    {
        smemRegFldGet(devObjPtr,regAddr, startBit, 10, valuePtr);
    }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr) &&
           !SMEM_CHT_IS_SIP7_GET(devObjPtr))/*TBD ... need to see the kind on MAC that will be in sip7 */
        {
            bitIndex++;/* the bits moved by 1 bit */
        }

        /* dont care in devices with 'tree of interrupts' */
        portInGlobBmp = 0;
    }
    else
    {
        /* Aldrin2, BC3, PIPE CG ports */
        bitIndex = 3;
    ...
    /* check if the device hold also interrupt support for the SERDES */
    if(devObjPtr->devMemSerdesLinkChangedPtr)
    {
        devObjPtr->devMemSerdesLinkChangedPtr(devObjPtr,port,linkState);
    }
}
...
)
{
    ...
    switch (mtiRegister)
    {
        ...
        case MTI_ACTIVE_MEM_REG_STAT_CAPTURE_E:
            ...
            break;
        case MTI_ACTIVE_MEM_REG_TSU_E:
            portIndex = ((address - 0x5000) >> 14) & 0xF; /* >> 14: to just 0x4000; 0xF: valid ports 0..9 */
...
)
{
    ...
    counterNumber = (devObjPtr->portsArr[macPort].mtiTxCountersCapture == 0) ?
        devObjPtr->mtiMibCounter.rxCntNum :
        devObjPtr->mtiMibCounter.txCntNum;
    ...
    *outMemPtr = regPtr[counterIndex];
    ...
    {
        internalMemPtr += devObjPtr->mtiMibCounter.rxCntNum;
    }
    ...
    *memPtr = *inMemPtr;
    ...
    if(!usePmacCounters)
    {
        ...
        memcpy(CHT_INTERNAL_MEM_PTR(devObjPtr, CHT_INTERNAL_SIMULATION_USE_MEM_MTI_STATISTICS_COUNTERS_CAPTURE_HI_WORD_START_E
                    + (CHT_INTERNAL_SIMULATION_STATISTIC_NUM_REGISTERS_CNS * macPort)) + devObjPtr->mtiMibCounter.rxCntNum * txBit,
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    numOfBanks = devObjPtr->emNumOfBanks[0] ;
    ...
    *memPtr = *inMemPtr;
    ...
    switch(emSize)
    {
        ...
        default:/*on 3 bits value .. can not get here*/
            emNumEntries = devObjPtr->emNumEntries[0];/*unchanged*/
            break;
    }

    devObjPtr->emNumEntries[0] = emNumEntries;

    if(devObjPtr->emMaxNumEntries < devObjPtr->emNumEntries[0])
    {
        skernelFatalError("smemFalconActiveWriteExactMatchGlobalCfg1Reg: EM num entries in HW [0x%4.4x] , but set to use[0x%4.4x] \n",
            devObjPtr->emMaxNumEntries , devObjPtr->emNumEntries[0]);
    }
    ...
    *memPtr = *inMemPtr;
    ...
    switch(banksNumber)
    {
        ...
        case 2:
            ...
            break;
        default:/*on 3 bits value .. can not get here*/
            emNumOfBanks =  devObjPtr->emNumOfBanks[0];/*unchanged*/
            break;
    }

    devObjPtr->emNumOfBanks[0] = emNumOfBanks;
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    if(devObjPtr->numOfPipes > 1)
    {
        if(devObjPtr->numOfRavens)/*Falcon*/
        {
            /* strip the pipe and tile from the address */
        ...
        else /*Seahawk(AAS) with 2 tiles*/
        {
            /* strip the tile from the address */
            tmpRegAddr = regAddr % devObjPtr->tileOffset;
        }
    }
    else
    {
...
/**
* @internal smemFalconActiveWriteHWSemaphore function
* @endinternal
*
* @brief   Implement simulation of HW Semaphores
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
* @param[in] param                    - Registers' specific parameter - global interrupt bit number.
*
* @param[in] inMemPtr                - Pointer to the memory to copy register's content from.
*
*
*/
void smemFalconActiveWriteHWSemaphore
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
{
    GT_U32 oldVal, newVal = *inMemPtr;

    /* Only firs 8 bits of semaphore register are defined: */
    smemRegFldGet(devObjPtr, address, 0, 8, &oldVal);

    if ( (oldVal == 0xff) || (newVal == 0xff) )
    { /* if unlocked or unlocking, then accept the new value. Else block write to enforce the semaphore protection: */
        *memPtr = newVal;
        smemRegFldSet(devObjPtr, address, 0, 8, newVal);
    }
}

/**
* @internal smemFalconActiveWriteDoorbellHostToCm function
* @endinternal
*
* @brief   Implement simulation of Host To CM Doorbell Register
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemFalconActiveWriteDoorbellHostToCm
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    *memPtr = *inMemPtr;
    ...
    snetChtDoInterruptInMgInAllTrees(devObjPtr,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].mg1InternalInterruptsSummaryCause,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].mg1InternalInterruptsSummaryMask,
        BIT_1, /* host_to_cm3_doorbell_int */
        0/*dont care*/);

}

/**
* @internal smemFalconActiveWriteDoorbellCmToHost function
* @endinternal
*
* @brief   Implement simulation of CM to Host Doorbell Register
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemFalconActiveWriteDoorbellCmToHost
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
{
    *memPtr = *inMemPtr;
/*    smemRegSet(devObjPtr, address, *inMemPtr);*/

    snetChtDoInterruptInMgInAllTrees(devObjPtr,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].miscellaneousInterruptCause,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].miscellaneousInterruptMask,
        BIT_14, /*CM to Host Doorbell*/
        0/*dont care*/);

}

/**
* @internal memFalconActiveWriteDoorbellCPToCP function
* @endinternal
*
* @brief   Implement simulation of CPU to CPU Doorbell Register
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemFalconActiveWriteDoorbellCPToCP
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
/*    smemRegSet(devObjPtr, address, *inMemPtr);*/
    ...
    snetChtDoInterruptInMgInAllTrees(devObjPtr,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].mg1InternalInterruptsSummaryCause,
       SMEM_CHT_MAC_REG_DB_GET(devObjPtr)->MG.globalInterrupt[0].mg1InternalInterruptsSummaryMask,
        BIT_7 << ((address & 0xF) >> 2), /* cp_to_cp_doorbell0_int / cp_to_cp_doorbell1_int / cp_to_cp_doorbell2_int / cp_to_cp_doorbell3_int */
        0/*dont care*/);

}

/**
* @internal smemFalconActiveWriteTimer1StartReg function
* @endinternal
*
* @brief   Implement simulation of Timer1 Start Register
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemFalconActiveWriteTimer1StartReg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    ...
    *memPtr = *inMemPtr;
    smemRegSet(devObjPtr, address, *inMemPtr);
    /* clear timer control reset/stop/restart conditions: */
    smemRegFldSet(devObjPtr, address + 4, 0, 1, 0);
    smemRegFldSet(devObjPtr, address + 4, 2, 2, 0);

    for (i = 0; i < SIM_MAX_NUM_OF_MG; i++)
    { /* update address in timer table, if not already there: */
        if (devObjPtr->timer1StartAddrs[i] == address)
            break;

        if (!devObjPtr->timer1StartAddrs[i])
        {
            devObjPtr->timer1StartAddrs[i] = address;
            break;
        }
    }

    skernelCreateTimer1Task(devObjPtr);
}

/**
* @internal memFalconActiveWriteTimer1ControlReg function
* @endinternal
*
* @brief   Implement simulation of Timer1 Control Register
*
* @param[in] devObjPtr                - device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
...
*
*/
void smemFalconActiveWriteTimer1ControlReg
(
    IN         SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN         GT_U32   address,
    IN         GT_U32   memSize,
...
)
{
    *memPtr = *inMemPtr;
    ...
    if (*inMemPtr & 0xd) /* timer reset/stop/restart - stop timer */
        smemRegSet(devObjPtr, address - 4, 0); /* stop the timer (restart seem to stop the timer from running ??? */

    if (*inMemPtr & 8) /* timer reset */
    { /* reset timer current value, first MSB then LSB: */
        smemRegSet(devObjPtr, address - 8, 0);
        smemRegSet(devObjPtr, address - 0xc, 0);
    }
}
