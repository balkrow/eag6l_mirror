...
/*extern*/ SMEM_REGISTER_DEFAULT_VALUE_STC sip5_registersDefaultValueArr[] =
{
    ...
    /* the CNC register with offset 0 - CNC Global Configuration Register */
    /* the bits 31:11 not used up to SIP 5_20  old default value - 0x07df */
    /* bits 9:2 and 20:11 ignored by simulation                           */
    /* the default value for SIP5_20 and upper coded here                 */
    ,DEFAULT_REG_FOR_CNC_UNIT_MAC(0x00000000,         0x03E647CB,      1,    0x0      )
...
/*extern*/ SMEM_REGISTER_DEFAULT_VALUE_STC sip5_10_registersDefaultValueArr[] =
{
    ...
    ,{&STRING_FOR_UNIT_NAME(UNIT_EGF_SHT_12),     0x000200b0,         0xffffffff,      8,    0x4      } /* ePort eVLAN Filtering Enable <%n>*/
...
#define INIT_INI_RUNTIME    0
...

    {
        ...
        {
            /*FDB table*/
             {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC(0 ,INIT_INI_RUNTIME), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(fdbEntryNumBits, fdbEntryNumBytes_align),SMEM_BIND_TABLE_MAC(fdbTable)}
        };
        GT_U32  numOfChunks = sizeof(chunksMem)/sizeof(SMEM_CHUNK_BASIC_STC);
       ...
       of physical ports */
    {
        ...
        if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr) == 0)
        {
            SMEM_CHUNK_BASIC_STC  chunksMemBc2A0[]=
            {
                ...
                {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00200000, 1048576), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(384,64) , SMEM_BIND_TABLE_MAC(arpTable)/*tunnelStartTable*/}
            };
            index = 0;
        ...
        else
        {
            ...
            {
                /*Router ARP DA and Tunnel Start Table*/
                {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00400000, 1048576), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(384,64) , SMEM_BIND_TABLE_MAC(arpTable)/*tunnelStartTable*/}
            };
            index = 0;
            /* number of entries : (2*numTs) . keep alignment and use for memory size */
...
)
{
       ...
       of physical ports */
    {
        ...
        }
        {
            SMEM_CHUNK_BASIC_STC  chunksMem[]=
            {
                {SET_SMEM_CHUNK_BASIC_STC_ENTRY_BY_NUM_BYTES_MAC (0x00080000, 524288), SET_SMEM_ENTRY_SIZE_AND_ALIGN_MAC(158, 32) , SMEM_BIND_TABLE_MAC(mllTable)}
            };
...
)
{
    ...
    if(SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_MG))
    {
    ...
    {
        /* in sip7 the CNC_1 must be part of the duplicatedUnits of the device */
        /* as a duplication of CNC                                             */
        /* because the smemLion3UnitCnc() not fit for sip7 devices             */
    }
    else
...
)
{
    ...
    {
        /*TBD multi tile*/
        devObjPtr->tablesInfo.txqSdqSip7QueueToPortMap.paramInfo[2].step = 0x01000000;
    }

     if (SMEM_CHT_IS_SIP7_GET(devObjPtr)){
        devObjPtr->tablesInfo.lpmMemory.paramInfo[0].step = 32;
        devObjPtr->tablesInfo.lpmMemory.paramInfo[0].outOfRangeIndex = devObjPtr->lpmRam.perRamNumEntries;
        devObjPtr->tablesInfo.lpmMemory.paramInfo[1].step = devObjPtr->lpmRam.numOfEntriesBetweenRams * /* 256k entries between 2 rams */
            devObjPtr->tablesInfo.lpmMemory.paramInfo[0].step;

        /* the caller of this table uses the index of tcam rule , but the index to the action is '/2' */
    ...
    devObjPtr->tablesInfo.macToMe.commonInfo.baseAddress = baseAddress + (SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 0x00008000 : 0x00001600);
    ...
    if(SMAIN_NOT_VALID_CNS != UNIT_BASE_ADDR_GET_ALLOW_NON_EXIST_UNIT_MAC(devObjPtr,UNIT_FDB_TABLE_0))
    {
        ...
        {
            devObjPtr->tablesInfo.fdbTable.commonInfo.baseAddress    = GET_FDB_DEVICE_FROM_MY_DEVICE_MAC(devObjPtr)->tablesInfo.fdbTable.commonInfo.baseAddress;
            devObjPtr->tablesInfo.fdbTable.paramInfo[0].step         = GET_FDB_DEVICE_FROM_MY_DEVICE_MAC(devObjPtr)->tablesInfo.fdbTable.paramInfo[0].step;
            devObjPtr->tablesInfo.fdbTable.paramInfo[0].divider      = GET_FDB_DEVICE_FROM_MY_DEVICE_MAC(devObjPtr)->tablesInfo.fdbTable.paramInfo[0].divider;
        }
        else
        {
            baseAddress = UNIT_BASE_ADDR_GET_MAC(devObjPtr,UNIT_FDB_TABLE_0);
            devObjPtr->tablesInfo.fdbTable.commonInfo.baseAddress = baseAddress;
        }
    }
    ...
    devObjPtr->tablesInfo.arpTable.paramInfo[0].step                 = 0x40;
    devObjPtr->tablesInfo.arpTable.paramInfo[0].divider              = 8;/* 8 sub entries in each line */
    ...
    if(!SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        ...
        devObjPtr->tablesInfo.tunnelStartTable.paramInfo[0].divider      = 2;/* 2 sub entries in each line */
    }
    else
    {
        /* sip7 : the arp table is split from the TS table */
        /* 1. the base address was already set by the unit , and also the step */
        /* 2. so we only need to set the divider           */
        devObjPtr->tablesInfo.tunnelStartTable.paramInfo[0].divider      = 8;/* 8 sub entries in each line */
    }
    ...
    devObjPtr->memUnitBaseAddrInfo.mllRegBase = SMAIN_NOT_VALID_CNS;
    ...
    devObjPtr->tablesInfo.cncMemory.paramInfo[0].step = 8*devObjPtr->cncBlockMaxRangeIndex;/* block --> x entries of 2 words (8 bytes)*/
...
)
{
    ...
    devObjPtr->devMemUnitMemoryGetPtr = NULL;/* no multi-core devices like Lion2 */
...
*/
    ...
    SET_IF_ZERO_MAC(devObjPtr->cncBlockMaxRangeIndex , _1K);
...
)
{
    ...
    else
    {
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            /* sip7 : cause to check the FDB size mode
                the <fdb_size> moved from 'config 1' to 'config 2'  */
            regAddr = SMEM_SIP6_FDB_GLOBAL_CONFIG_2_REG(devObjPtr);
        }
        else
        {
            /* cause to check the FDB size mode */
...
/**
* @internal smemLion3OamExceptionGroupStatus function
* @endinternal
*
...
*/
static void smemLion3OamExceptionGroupStatus
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_U32                  address,
    IN GT_U32                  stage
)
{
    ...
    /* Tabel ID [18:15] */
    tableId     = (address >> 15) & 0xF;
    entryIndex  = (address & (SMEM_CHT_IS_SIP7_GET(devObjPtr)?0x1FFF:0xFF))/4;
    switch(tableId)
    {
        /* AGING_TABLE */
        case 0:
            regAddr = SMEM_LION2_OAM_AGING_EXCEPTION_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* MEG_EXCEPTION_TABLE */
        case 2:
            regAddr = SMEM_LION2_OAM_MEG_LEVEL_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* SOURCE_INTERFACE_EXCEPTION_TABLE */
        case 3:
            regAddr = SMEM_LION2_OAM_SOURCE_INTERFACE_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* INVALID_KEEP_ALIVE_HASH_TABLE */
        case 4:
            regAddr = SMEM_LION2_OAM_INVALID_KEEPALIVE_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* EXCESS_KEEP_ALIVE_TABLE */
        case 5:
            regAddr = SMEM_LION2_OAM_EXCESS_KEEPALIVE_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* EXCEPTION_SUMMARY_TABLE */
        case 6:
            regAddr = SMEM_LION2_OAM_SUMMARY_EXCEPTION_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* RDI_STATUS_CHANGE_TABLE */
        case 7:
            regAddr = SMEM_LION2_OAM_RDI_STATUS_CHANGE_EXCEPTION_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;

        /* TX_PERIOD_EXCEPTION_TABLE */
        case 8:
            regAddr = SMEM_LION2_OAM_TX_PERIOD_EXCEPTION_GROUP_STATUS_REG(devObjPtr, stage, (entryIndex/32));
            break;
        default: /* Not expected */
            skernelFatalError("smemLion3OamExceptionGroupStatus: Invalid tableId[%d]\n", tableId);
            break;
    }
    grpMemPtr = smemMemGet(devObjPtr, regAddr);
    /** 0-1023 index -> summary register 1
     *      0-31 Index ->offset 0
     */
    SMEM_U32_SET_FIELD(*grpMemPtr, (entryIndex % 32), 1, 0);
}
...
)
{
    ...
    /* Clear exception group status based on exception read */
    smemLion3OamExceptionGroupStatus(devObjPtr, address, param);
}
    ...
    *memPtr = *inMemPtr;
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        if (rdWr)/* write the data from the msg registers to the FDB table  */
        {
            ...
            smemGenericHiddenMemSet(devObjPtr, SMEM_GENERIC_HIDDEN_MEM_FDB_E, line , regPtr, SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 5 : 4);
        }
        else/* read the data from the FDB table into the msg registers */
        {
            regPtr = SMEM_SIP6_HIDDEN_FDB_PTR(devObjPtr, line);
            /* Mac Table Access Data */
            smemMemSet(devObjPtr, SMEM_CHT_MAC_TBL_ACC_DATA0_REG(devObjPtr), regPtr, SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 5 : 4);
        }
...
* @param[in] inMemPtr                 - Pointer to the memory to get register's content.
*/
void smemLion3ActiveWriteTodStepConfReg (
    IN  SKERNEL_DEVICE_OBJECT *devObjPtr,
    IN  GT_U32                 address,
    IN  GT_U32                 memSize,
    IN  GT_U32                *memPtr,
...
)
{
    ...
    *memPtr = *inMemPtr;
    ...
    fieldVal = SMEM_U32_GET_FIELD(*inMemPtr, 0, 16);
    if(fieldVal == 0) /* TOD Step frac configuration low == 0 */
    {
        regPtr = smemMemGet(devObjPtr, address - 4);
        fieldVal = SMEM_U32_GET_FIELD(regPtr[0], 0, 16);
        if(fieldVal == 0) /* TOD Step frac configuration high == 0 */
        {
            regPtr = smemMemGet(devObjPtr, address - 8);
            fieldVal = SMEM_U32_GET_FIELD(regPtr[0], 0, 16);
            if(fieldVal == 0) /* TOD Step nano configuration == 0 */
            {
                /* the time has frozen , and we want to freeze current clock */
                if(devObjPtr->supportSingleTai)
                {
                    unitBaseAddr = UNIT_BASE_ADDR_FROM_STRING_GET_MAC(devObjPtr,STR(UNIT_TAI));
                    if (SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
                    {
                        taiInst = ((address >> 20) & 0xF) - ((unitBaseAddr >> 20) & 0xF);
                    }
                ...
                devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer += nanoSeconds;
                ...
                devObjPtr->eTod[taiGroup][taiInst].secondTimer =
                    prvSimMathAdd64(devObjPtr->eTod[taiGroup][taiInst].secondTimer, seconds64);
                return;
            }
        }
    }
...
/**
* @internal smemLion3ActiveWriteProtectReadOnly function
* @endinternal
*
* @brief   Protect read only registers
*
* @param[in] devObjPtr                - Device object PTR.
* @param[in] address                  - Address for ASIC memory.
* @param[in] memPtr                   - Pointer to the register's memory in the simulation.
* @param[in] param                    - Registers range protection parameters
*                                       base - bits27:16, step - bits15:8, num - bits7:0
* @param[in] inMemPtr                 - Pointer to the memory to get register's content.
*/
void smemLion3ActiveWriteProtectReadOnly (
    IN  SKERNEL_DEVICE_OBJECT *devObjPtr,
    IN  GT_U32                 address,
    IN  GT_U32                 memSize,
    IN  GT_U32                *memPtr,
...
)
{
    ...
    if ((void*)param == (void*)0)
    {
        /* all addresses protected */
        return;
    }

    /* protected address sequence specified by base, step, num */
    w = (GT_U32)param;
    base = (w >> 16) & 0xFFF;
    step = (w >> 8) & 0xFF;
    num  = w & 0xFF;

    addrLow = (address & 0xFFF);
    if (addrLow < base)
    {
        /* not protected */
        *memPtr = *inMemPtr;
