...
exit_function_lbl:
    ...
    if (pclExtraDataPtr->isL4Valid == GT_FALSE)
    {
        descrPtr->l4Valid = 0;
    }
...
)
{
          ...
          ++ii, ++layer4CompareValPtr)
    {
        ...
        {/* source IP port*/
            __LOG(("index [%2.2x] source IP port[%4.4x]", ii, srcPktIPPort));
            pktIPPort = srcPktIPPort ;
        }
        else
        {/* destination IP port*/
            __LOG(("index [%2.2x] destination IP port[%4.4x]", ii, dstPktIPPort));
        ...
        switch (l4Operator)
        {
            ...
            case CHT_PCL_IP_PORT_OPERATOR_GREATER_OR_EQUAL_E:
                ...
                __LOG(("l4Operator = GREATER_OR_EQUAL, compIPPort [%4.4x] res [%d]", compIPPort, res1));
                break;

            case CHT_PCL_IP_PORT_OPERATOR_LESS_OR_EQUAL_E:
                res1 = (pktIPPort <= compIPPort )? 1 : 0;
                __LOG(("l4Operator = LESS_OR_EQUAL, compIPPort [%4.4x] res [%d]", compIPPort, res1));
                break;

            case CHT_PCL_IP_PORT_OPERATOR_NOT_EQUAL_E:
                res1 = (pktIPPort != compIPPort )? 1 : 0;
                __LOG(("l4Operator = NOT_EQUAL, compIPPort [%4.4x] res [%d]", compIPPort, res1));
                break;
        }
    ...
    __LOG(("L4 Port Comparator result 0x%08X 0x%08X", bytePortCompareVal.l[0], bytePortCompareVal.l[1]));
...
/**
* @internal snetCht2EPclTcpUdpPortExtendedRangeCompareGet function
* @endinternal
*
* @brief   Build TCP/UDP comparator value for EPCL search key - EPCL - from ch2
*
...
*                                      COMMENTS:
*/
extern GT_STATUS snetCht2EPclTcpUdpPortExtendedRangeCompareGet
(
    IN SKERNEL_DEVICE_OBJECT            * devObjPtr,
    IN SKERNEL_FRAME_CHEETAH_DESCR_STC  * descrPtr,
    OUT GT_U64                          * byteCompareVal
