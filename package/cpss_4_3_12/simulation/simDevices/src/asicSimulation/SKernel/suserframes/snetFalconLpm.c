        ...
        if (sipAllZero == GT_FALSE)
        {
            if (isAgingEnabled)
            {
                     ...
                     ( (descrPtr->ipm) && (isSipLookup) ) )
                {
                    ...
                    bankSize = devObjPtr->lpmRam.perRamNumEntries;
...
*
*******************************************************************************/
static GT_VOID processSip6UcSipLpmStages
(
    IN    SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN    SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
    IN    GT_U8                            addressDataLen,
...
{
    DECLARE_FUNC_NAME(processSip6UcSipLpmStages);
...
)
{
    ...
    GT_U32               totalNumOfLinesInBankIncludingGap = 0x8000;
    GT_U32               pbrBankSize;
    GT_U32               lineNumber;
    ...
    smemRegFldGet(devObjPtr, SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->LPM.directAccessMode,24, 2, &pbrBankSize);
    switch(pbrBankSize)
    {
        case 0:
            /* Bank: bits 17:15. Line: bits 14:3. Leaf Index in Line: bits 2:0 */
            mask = 0x07FF8;
            shift = 15;
            break;
        case 1:
            /* Bank: bits 17:16. Line: bits 15:3. Leaf Index in Line: bits 2:0 */
            mask = 0x1FFF;
            shift = 16;
            break;
        case 2:
            /* Bank: bit 17. Line: bits 16:3. Leaf Index in Line: bits 2:0 */
            mask = 0x3FFF;
            shift = 17;
            break;
        case 3:
            /* Line: bits 14:3. Leaf Index in Line: bits 2:0 */
            mask = 0x3FFF;
            shift = 18;
            break;
        default:
            __LOG(("illegat pbr Bank Size\n"));
            return;
            break;
    }

    pbrBankSize = (0x1000) << pbrBankSize;

    if((SMEM_CHT_IS_SIP6_15_GET(devObjPtr)) && (!SMEM_CHT_IS_SIP6_30_GET(devObjPtr)))
    {
        pbrBankSize = 1024; /* AC5X and Harrier */
    }
    else if((SMEM_CHT_IS_SIP6_30_GET(devObjPtr)) && (!SMEM_CHT_IS_SIP7_GET(devObjPtr)))
    {
        pbrBankSize = 1536; /* IM */
    }
        ...
        descrPtr->pclRedirectCmd == PCL_TTI_ACTION_REDIRECT_CMD_LTT_ROUTER_AND_ASSIGN_VRF_ID_E))
    {
        ...
        lineNumber = descrPtr->pceRoutLttIdx;
    ...
    else /*descrPtr->ttiRedirectCmd == PCL_TTI_ACTION_REDIRECT_CMD_LTT_ROUTER_E*/
    {
        ...
        lineNumber = descrPtr->ttRouterLTT;
        leafNumber = descrPtr->ttRouterLTT & 0x7;
    }

    lineNumber = ((lineNumber & mask) >> 3 )  + ( lineNumber >> shift) * pbrBankSize;

    addrSpaceLineNumber = (lineNumber/(pbrBankSize))*totalNumOfLinesInBankIncludingGap;
    addrSpaceLineNumber = addrSpaceLineNumber + lineNumber%pbrBankSize;
    pbrLpmEntryAddr += addrSpaceLineNumber;
...
{
    DECLARE_FUNC_NAME(snetFalconLpmUcSipLookup);
    ...
    if(continueSipStages)
    {
        ...
        processSip6UcSipLpmStages(devObjPtr, descrPtr, addressDataLen, &sipLpmData, sipLpmLeafPtr);
