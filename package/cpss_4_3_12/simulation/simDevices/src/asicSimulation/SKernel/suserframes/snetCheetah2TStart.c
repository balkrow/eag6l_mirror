...
#define SIP7_HFT_FIELD_GET_MAC(field)\
    SMEM_SIP7_HA_HEADER_FIELDS_ENTRY_FIELD_GET_MAC(devObjPtr, hftActionPtr, index, field)
...
)
{
    ...
    */
    ...
    /* EXP */
    fieldValue = 0;
    serviceId = snetFieldValueGet(descrPtr->eArchExtInfo.haEgressVlanTablePtr,startBit,24);
...
)
{
    ...
    if(ttl == 0)
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            Pop_or_Swap = (descrPtr->totalPoppedLabels > 0)? 1 :0;
        }
        else
        {
        ...
        if(Pop_or_Swap)
        {
            switch(swapTtlMode)
            {
                ...
                default:
                    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
                    {
                        if((descrPtr->totalPoppedLabels > 1) &&
                            devObjPtr->errata.haMplsUniformTtlPop2Swap)
                        {
                            __LOG(("WARNING : FE-8602419 (HA-3727) use TTL[%d] from second outer label instead of most outer label TTL[%d] \n",
                                descrPtr->ttlArr[1],
                                descrPtr->ttlArr[0]));
                            ttl = descrPtr->ttlArr[1];  /* the second outer label of the incoming packet */
                        }
                        else
                        {
                            ttl = descrPtr->ttlArr[0];  /*the Outer label of the incoming packet*/
                        }
                    }
                    else
                    {
    ...
    __LOG_PARAM(autoSetSBitEnable_global);
    ...
    for(currentLabelIndex = numOfTsLabels ; currentLabelIndex > 0 ; currentLabelIndex--)
    {
        ...
        if( (currentLabelIndex == 1) && (numOfTsLabels == numOfLabels))/* most inner label */
        {
            ...
            __LOG(("For Last label, setting the SBit as [%u]\n", sBit));
        }
        else
        {
            /* not bottom of stack */
...
)
{
    ...
    if(fieldValue)
    {
        ...
        fieldValue =
            snetFieldValueGet(descrPtr->eArchExtInfo.haEgressVlanTablePtr,startBit,24);
        ...
        if(fieldValue)
        {
            ...
            if(descrPtr->origIsTrunk == GT_FALSE)
            {
                ...
                else
                {
                    ...
                    {
                        index =
                            SMEM_SIP7_HA_SRC_EPORT_1_ENTRY_FIELD_GET_MAC(devObjPtr,
                                haEgressSourceEPortAtt1TablePtr,
...
)
{
    ...
    /* eVlan <Service ID> */
    tsInfo.eVlanServiceId = snetFieldValueGet(descrPtr->eArchExtInfo.haEgressVlanTablePtr,startBit,24);
    ...
    if(tsInfo.udp)
    {
        ...
        /* SIP7 supports 32 bit packet hash, use 14 LSbits of hash descriptor for src UDP port */
        GT_U32 packetHash = SMEM_CHT_IS_SIP7_GET(devObjPtr)? (descrPtr->pktHash & 0x3FFF) : descrPtr->pktHash;

        GT_U32 udpSrcPort = udpSrcMode ? packetHash :
        ...
        if(udpSrcMode)
        {
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
            {
                __LOG(("NOTE: 'Src UDP port' set by descrPtr->pktHash[0x%3.3x] 14 LSbits , and 2 MSbits are HardCoded 0x3 (as VXLAN RFC 7348 recommends the hash value assigned be in the dynamic/private port range 49152-65535.!!!\n",
                descrPtr->pktHash));
            }
            else
            {
    ...
    if((SKERNEL_FRAME_TUNNEL_START_TYPE_GENERIC_IPV4_E == descrPtr->tunnelStartType) ||
       (SKERNEL_FRAME_TUNNEL_START_TYPE_GENERIC_IPV6_E == descrPtr->tunnelStartType))
    {
...
*
* DESCRIPTION:
*         Builds the MPLS labels
* INPUTS:
*        devObjPtr     - pointer to device object.
*        descrPtr      - pointer to the frame's descriptor.
*        egrBufPtr     - pointer to the descriptor buffer to store the packet
*        mplsFlowLabel - to indicate if there is MPLS Flow label
*        numOfLabels   - number of MPLS Labels to be added
*        rawLabels     - to indicate if the labels needs to be added
*                        with/without Qos paramters
*        qosMappingTableIndex  - Index to the Qos mapping table
*        lastQosUniformLabel   -  Last label that is subject to the QoS uniform model.
*        lastTtlUniformLabel   -  Last label that is subject to the TTL uniform model.
*        mplsLabelStackPtr     - pointer to the entry in the TS table
* OUTPUTS:
*
* RETURNS: GT_U8* - pointer to the end of the tunnel start
*
...
*
*******************************************************************************/
static GT_U8 *sip7BuildMplsLabels
(
  IN SKERNEL_DEVICE_OBJECT           *devObjPtr,
  IN SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
  IN GT_U8                           *egrBufPtr,
  IN GT_U32                          mplsFlowLabel,
  IN GT_U32                          numOfLabels,
  IN GT_U32                          rawLabels,
  IN GT_U32                          qosMappingTableIndex,
  IN GT_U32                          lastQosUniformLabel,
  IN GT_U32                          lastTtlUniformLabel,
  IN GT_U32                          *mplsLabelStackPtr
)
{
    DECLARE_FUNC_NAME(sip7BuildMplsLabels);
    ...
    GT_U32  startBit;
    GT_U32  word[2];
    GT_U32  expMappingMode[16];
    GT_U32  label[16];
    GT_U32  exp[16];
    GT_U32  ttl[16];
    GT_U32  isElOrEliInsertion[16];
    GT_U32  sBit[16];
    GT_U32  prevTtl, prevExp;

    prevTtl = prevExp = 0;

    memset(expMappingMode, 0, sizeof(GT_U32)*16);
    memset(label, 0, sizeof(GT_U32)*16);
    memset(exp, 0, sizeof(GT_U32)*16);
    memset(ttl, 0, sizeof(GT_U32)*16);
    memset(isElOrEliInsertion, 0, sizeof(GT_U32)*16);
    memset(sBit, 0, sizeof(GT_U32)*16);

    __LOG(("sip7BuildMplsLabels:  mplsFlowLabel[%u], numOfLabels[%u], rawLabels[%u], \
           qosMappingTableIndex[%u], lastQosUniformLabel[%u], lastTtlUniformLabel[%u]\n",
           mplsFlowLabel, numOfLabels, rawLabels, qosMappingTableIndex,
           lastQosUniformLabel, lastTtlUniformLabel));

    for (idx = 0; idx < numOfLabels; idx++)
    {
        /* zero out words */
        memset(word, 0, sizeof(GT_U32)*2);
        startBit = idx*2;
        word[0]  = SMEM_U32_GET_FIELD(mplsLabelStackPtr[idx], startBit, 32-startBit);
        word[0] |= (SMEM_U32_GET_FIELD(mplsLabelStackPtr[idx+1], 0, startBit)<<(32-startBit));
        word[1]  = SMEM_U32_GET_FIELD(mplsLabelStackPtr[idx+1], startBit, 2);

        expMappingMode[idx]     = word[0] & 0x3;
        label[idx]              = (word[0]>>2) & 0xFFFFF;
        exp[idx]                = (word[0]>>22) & 0x7;
        ttl[idx]                = (((word[0]>>25) & 0x7F)|((word[1] & 0x1)<<7));
        isElOrEliInsertion[idx] = (word[1]>>1) & 0x1;

        __LOG(("Before EXP/TTL Calculation: for Label Index[%u], expMappingMode[%u], label[%u], \
                exp[%u], ttl[%u], isElOrEliInsertion[%u]\n",
                expMappingMode[idx], label[idx], exp[idx], ttl[idx], isElOrEliInsertion[idx]));

        /* EXP Calculation */
        if (rawLabels == 0)
        {
            __LOG(("Raw labels NOT enabled. Apply EXP models"));
            if (idx == 0) /* First Label */
            {
                if ((idx <= lastQosUniformLabel) || (descrPtr->isLabelSwapped))
                {
                    __LOG(("First Label. Applying QoS Uniform Model"));
                    exp[idx] = descrPtr->exp4Egress;
                }
            }
            else
            {
                if (idx <= lastTtlUniformLabel)
                {
                    __LOG(("Label[%u]. Applying QoS Uniform Model", idx));
                    exp[idx] = prevExp;
                }
            }
            prevExp = exp[idx];
        ...
        if (rawLabels == 0)
        {
            __LOG(("Raw labels NOT enabled. Apply TTL models"));
            if (idx == 0) /* First Label */
            {
                if ((idx <= lastTtlUniformLabel) || (descrPtr->isLabelSwapped))
                {
                    __LOG(("First Label. Applying TTL Uniform Model"));
                    ttl[idx] = descrPtr->ttl;
                }
            }
            else
            {
                if (idx <= lastTtlUniformLabel)
                {
                    __LOG(("Label[%u]. Applying QoS TTL Model", idx));
                    ttl[idx] = prevTtl;
                }
            }
            prevTtl = ttl[idx];
    ...
    /* check idx to avoid false warnings in some GCC versions */
    if (idx)
    {
        sBit[idx-1] = (mplsFlowLabel==1)?0:1;
    }

    for (idx = numOfLabels-1; idx >= 0; idx--)
    {
        switch(expMappingMode[idx])
        {
            case 0: /* Global */
                __LOG(("Label[%u]. Applying EXP Global Mapping Mode", idx));
                smemRegFldGet(devObjPtr,
                              SMEM_LION2_HA_QOS_PROFILE_TO_EXP_TBL_MEM(devObjPtr, qosMappingTableIndex),
                              0,3,
                              &exp[idx]);
                break;
            case 1: /* Interface */
                __LOG(("Label[%u]. Applying EXP Interface Mapping Mode", idx));
                /* TBD : Take from Circuit Table*/
                break;
            case 2: /* Custom */
                __LOG(("Label[%u], Custom EXP Mapping Mode, the exp value [%u] from configuration is used\n", idx, exp[idx]));
                break;
            default:
                __LOG(("Invalid EXP Mapping Mode [%u]\n", expMappingMode));
        }

        mplsEncap = 0;
        /* TTL */
        SMEM_U32_SET_FIELD(mplsEncap,0,8,ttl[idx]);
        /* S-Bit */
        SMEM_U32_SET_FIELD(mplsEncap,8,1,(isElOrEliInsertion[idx] == 1)?0:sBit[idx]);
        /* EXP */
        SMEM_U32_SET_FIELD(mplsEncap,9,3,exp[idx]);
        /* label */
        SMEM_U32_SET_FIELD(mplsEncap,12,20,label[idx]);
        ...
        /* push entropy label */
        if (isElOrEliInsertion[idx] == 1)
        {
            __LOG(("Adding ELI and EL Label\n"));
            ...
            SMEM_U32_SET_FIELD(mplsEncap,0,8,0);
            /* EL S-Bit */
            SMEM_U32_SET_FIELD(mplsEncap,8,1,sBit[idx]);
            /* EL EXP */
            SMEM_U32_SET_FIELD(mplsEncap,9,3,0);
            /* EL label */
            SMEM_U32_SET_FIELD(mplsEncap,12,20,hash);
...
/*******************************************************************************
*   sip7HaTunnelStartL3_vPortMPLS
*
* DESCRIPTION:
*        Create buffer for tunnel start - MPLS vPort info
* INPUTS:
*        devObjPtr            - pointer to device object.
*        descrPtr             - pointer to the frame's descriptor.
*        haInfoPtr            - ha internal info
...
*
*******************************************************************************/
static GT_U8* sip7HaTunnelStartL3_vPortMPLS
(
  IN SKERNEL_DEVICE_OBJECT           *devObjPtr,
  IN SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
  IN HA_INTERNAL_INFO_STC*           haInfoPtr,
...
{
    DECLARE_FUNC_NAME(sip7HaTunnelStartL3_vPortMPLS);

    GT_U8  tmpBuffer[4];
    GT_U8  *egrBufPtr = descrPtr->tunnelStartRestOfHeaderInfoPtr;/*buffer to put the TS L3 header + ethertype */
    GT_U32 fieldValue;
    GT_U32 numOfTsLabels;
    GT_U32 regAddress;
    GT_BIT mplsFlowLabel;
    GT_U32 qosMappingTableIndex;
    GT_U32 startBit, regIdx;
    GT_U32 lastQosUniformLabel, lastTtlUniformLabel;
    GT_U32 rawLabels = 0;
    GT_U32 mplsLabelStack[16];

    /* TAG0 Qos Marking mode */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG0_QOS_MARKING_MODE_E);
    __LOG(("TAG0 QoS Marking Mode [%u]\n", fieldValue));
    if (TAG0_EXIST_MAC(descrPtr))
    {
        /* TBD the logic */
        switch (fieldValue)
        {
            case 0: /* Ingress Assignment */
                break;
            case 1: /* Tunnel Based */
                /* UP0 */
                fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP0_E);
                /* DEI0 */
                fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI0_E);
                break;
            case 2: /* Circuit Based */
                break;
            default:
                __LOG(("Invalid TAG0 marking mode [%u]\n", fieldValue));
        }
    }

    /* TAG1 Qos Marking mode  */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_TAG1_QOS_MARKING_MODE_E);
    __LOG(("TAG1 QoS Marking Mode [%u]\n", fieldValue));
    if (TAG1_EXIST_MAC(descrPtr))
    {
        ...
        switch (fieldValue)
        {
            ...
            case 1: /* Tunnel Based */
                /* UP1 */
                fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_UP1_E);
                /* DEI1 */
                fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_DEI1_E);
                break;
            case 2: /* Circuit Based */
                break;
            default:
                __LOG(("Invalid TAG1 marking mode [%u]\n", fieldValue));
        }
    }

    /* Thread ID */
    descrPtr->pha.pha_threadId = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_THREAD_ID_E);
    __LOG(("ThreadId [%u]\n", descrPtr->pha.pha_threadId ));

    /* Analyzer Index */
    descrPtr->analyzerIndex = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_ANALYZER_INDEX_E);
    __LOG(("Analyzer Index[%u]\n", descrPtr->analyzerIndex));

    /* Egress Mirror mode */
    descrPtr->mirroringMode = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_EGRESS_MIRRORING_MODE_E);
    __LOG(("Egress Mirror Mode [%u]\n", descrPtr->mirroringMode));

    /* Raw Labels */
    rawLabels = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_RAW_LABELS_E);
    __LOG(("Raw Labels [%u]\n", rawLabels));

    /* MPLS Ethertype Select */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_MPLS_ETHERTYPE_SELECT_E);
    __LOG(("Ethertype Select Val [%u]\n", fieldValue));
    regIdx   = fieldValue/2;
    startBit = (fieldValue % 2) * 16;
    smemRegFldGet(devObjPtr, SMEM_SIP7_HA_MPLS_ETHERTYPES_REG(devObjPtr, regIdx),
                  startBit, 16, &fieldValue);
    tmpBuffer[0] = (GT_U8)(fieldValue >> 8);
    tmpBuffer[1] = (GT_U8)(fieldValue);
    MEM_APPEND(egrBufPtr, tmpBuffer, 2);
    ...
    /* CNC Ptr */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_CNC_PTR_E);
    __LOG(("CNC Pointer [%u]\n", fieldValue));
    /* TBD: to save in desc */

    /* Flow ID / GCF profile ID */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_GCF_PROFILE_ID_E);
    __LOG(("Flow Id/ GCF Profile ID [%u]\n", fieldValue));
    descrPtr->flowId = fieldValue;

    /* Flow ID / GCF bits */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_ID_GCF_BITS_E);
    __LOG(("Flow Id/ GCF Bits [%u]\n", fieldValue));
    /* TBD: to save in desc */

    /* Get number of Labels to be added */
    numOfTsLabels = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_NUMBER_OF_LABELS_E);
    __LOG(("Number of TS Labels [%u]\n", numOfTsLabels));

    /* Last Qos Uniform label */
    lastQosUniformLabel = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_QOS_UNIFORM_LABEL_E);
    __LOG(("Last Qos Uniform Label [%u]\n", lastQosUniformLabel));

    /* Global EXP mapping index */
    qosMappingTableIndex = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_QOS_MAPPING_TABLE_INDEX_E);
    __LOG(("QoS Mapping Table Index [%u]\n", qosMappingTableIndex));

    /* Last TTL Uniform Label */
    lastTtlUniformLabel = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_LAST_TTL_UNIFORM_LABEL_E);
    __LOG(("Last TTL Uniform Label [%u]\n", lastTtlUniformLabel));

    /* Control Word Index */
    fieldValue = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_CW_INDEX_E);
    __LOG(("Control Word Index [%u]\n", fieldValue));

    /* insert MPLS control word if the CW_INDEX is non-zero */
    if (fieldValue)
    {
        /* Set HA descriptor fields relevant to MPLS tunnel start */
        descrPtr->tsEgressMplsControlWordExist = 1;
        ...
        /*Insert PW Control word (indexed by this field) to the MPLS tunnel*/
        regAddress = SMEM_XCAT_HA_TS_MPLS_CONTROL_WORD_REG(devObjPtr, fieldValue);
    ...
    /* MPLS Flow label push Insertion */
    mplsFlowLabel = SIP5_TSE_FIELD_GET_MAC(SMEM_SIP7_HA_TUNNEL_START_TABLE_FIELDS_FLOW_LABEL_INSERTION_E);
    __LOG(("MPLS Flow Label [%u]\n", mplsFlowLabel));
    ...
    memset(mplsLabelStack, 0, sizeof(GT_U32)*16);

    SMEM_SIP7_HA_TUNNEL_START_MPLS_LABEL_STACK_GET(devObjPtr, tunnelStartActionPtr,
                                                   descrPtr->tunnelPtr, mplsLabelStack);

    /* Build the Label Stack */
    egrBufPtr = sip7BuildMplsLabels(devObjPtr, descrPtr, egrBufPtr, mplsFlowLabel,
                                    numOfTsLabels, rawLabels, qosMappingTableIndex,
                                    lastQosUniformLabel, lastTtlUniformLabel,
                                    mplsLabelStack);

    if(mplsFlowLabel)
    {
        __LOG(("Adding MPLS Flow Label"));
        /* TBD: indication that even that we are 'most inner'
         * there are additional labels that we are not aware of
         * Modify sBit accordingly */

        /* Add 'Flow label' */
        egrBufPtr = lion3BuildMplsFlowLabel(devObjPtr, descrPtr, egrBufPtr, 1);
    }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        GT_U32  indexMask = devObjPtr->tablesInfo.tunnelStartTable.paramInfo[0].divider - 1;
        GT_U32  numBits = 198;
        GT_U32  relativeIndex = tsIndex & indexMask;
        GT_U32  potentialEntrySize;
        if(relativeIndex)
        {
            switch(relativeIndex)
            {
                ...
                case 5 :
                case 7 :
                    potentialEntrySize = numBits;
                    break;
                case 2 :
                case 6 :
                    potentialEntrySize = numBits * 2;
                    break;
                case 4 :
                    potentialEntrySize = numBits * 4;
                    break;
                default :/* should not get here */
                    skernelFatalError("unknown relativeIndex [%d] \n", relativeIndex);
                    return;
            }

            wm_copyBits(&devObjPtr->sip7_ha.tunnelStartTempSubEntry[0]/*target ptr*/,0/*target bit*/,
               tunnelStartActionPtr/*source ptr*/ , numBits * (tsIndex & indexMask) /*sourceStartBit*/,
               potentialEntrySize);

            tunnelStartActionPtr = &devObjPtr->sip7_ha.tunnelStartTempSubEntry[0];
        }
    }
    else
    {
    ...
    else
    {
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
             __LOG(("WARNING SIP7: the device not supports 'retain inner CRC' !!! (although configured to do so) \n"));
    ...
    switch(descrPtr->tunnelStartType)
    {
        case SKERNEL_FRAME_TUNNEL_START_TYPE_MPLS_E    :
            /* TBD: Enable after the Enhanced UT is completed */
            if (descrPtr->trgIsVPort && 0)
            {
                nextBufferPtr = sip7HaTunnelStartL3_vPortMPLS(devObjPtr, descrPtr,
                                                              haInfoPtr, tunnelStartActionPtr);
            }
            else
            {
        ...
        {/* MPLS Tunneling */
            ...
            if (ttl == 0)
            {
                if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
                {
                    ...
                    if (swapTtlMode == 0 && (descrPtr->totalPoppedLabels == 0))
                    {
            ...
            tsInfo.exp[0] = (exp_mode1 == 1) ? exp1 : descrPtr->exp[0];
            ...
            if (numOfLabels >= 1)
            {
                 ...
                 tsInfo.exp[1] = (exp_mode2 == 1) ? exp2 : descrPtr->exp[1];
                 tsInfo.sBit[1] = 0;
            }
            ...
            if (numOfLabels >= 2)
            {
                ...
                tsInfo.exp[2] = (exp_mode3 == 1) ? exp3 : descrPtr->exp[2];
                tsInfo.sBit[2] = 0;
            }
        }
        break;
...
static GT_VOID hftCommonConfig
(
  IN SKERNEL_DEVICE_OBJECT           *devObjPtr,
  IN SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
  IN GT_U32                          index,
  IN GT_U32                          *hftActionPtr
)
{
    DECLARE_FUNC_NAME(hftCommonConfig);

    GT_U32 fieldValue;

    /* Sub Type */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_SUB_TYPE_E);
    __LOG(("Sub Type [%u]\n", fieldValue));

    /* Sub-Thread ID */
    descrPtr->pha.pha_threadId = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_SUB_THREAD_ID_E);
    __LOG(("ThreadId [%u]\n", fieldValue));

    /* Analyzer Index */
    descrPtr->analyzerIndex = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_ANALYZER_INDEX_E);
    __LOG(("Analyzer Index[%u]\n", descrPtr->analyzerIndex));

    /* Egress Mirror mode */
    descrPtr->mirroringMode = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_EGRESS_MIRROR_MODE_E);
    __LOG(("Egress Mirror Mode [%u]\n", descrPtr->mirroringMode));

    /* CNC Ptr */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_CNC_PTR_E);
    __LOG(("CNC Pointer [%u]\n", fieldValue));
    /* TBD: to save in desc */

    /* GCF profile ID */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_GCF_PROFILE_ID_E);
    __LOG(("GCF Profile ID [%u]\n", fieldValue));
    /* TBD: to save in desc */

    /* GCF bits[0..4] */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_GCF_BITS_0_4_E);
    __LOG(("GCF Bits [%u]\n", fieldValue));
    /* TBD: to save in desc */

}

static GT_VOID hftLabelStackConfig
(
  IN    SKERNEL_DEVICE_OBJECT           *devObjPtr,
  IN    SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
  IN    GT_U32                          index,
...
{
    DECLARE_FUNC_NAME(hftLabelStackConfig);
    ...
    /* Get number of Labels to be added */
    numOfLabels = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_NUMBER_OF_LABELS_E);
    __LOG(("Number of HFT Labels [%u]\n", numOfLabels));

    /* Last Qos Uniform label */
    lastQosUniformLabel = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_LAST_QOS_UNIFORM_LABEL_E);
    __LOG(("Last Qos Uniform Label [%u]\n", lastQosUniformLabel));

    /* Global EXP mapping index */
    qosMappingTableIndex = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_GLOBAL_EXP_MAPPING_TBL_INDEX_E);
    __LOG(("QoS Mapping Table Index [%u]\n", qosMappingTableIndex));

    /* Last TTL Uniform Label */
    lastTtlUniformLabel = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_LAST_TTL_UNIFORM_LABEL_E);
    __LOG(("Last TTL Uniform Label [%u]\n", lastTtlUniformLabel));

    /* Control Word Index */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_CW_INDEX_E);
    __LOG(("Control Word Index [%u]\n", fieldValue));
    ...
    /* MPLS Flow label push Insertion */
    mplsFlowLabel = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_FLOW_LABEL_INSERTION_E);
    __LOG(("MPLS Flow Label [%u]\n", mplsFlowLabel));
    ...
    SMEM_SIP7_HA_HFT_MPLS_LABEL_STACK_GET(devObjPtr, hftActionPtr,
                                          index, mplsLabelStack);

    /* Build the Label Stack */
    egrBufPtr = sip7BuildMplsLabels(devObjPtr, descrPtr, egrBufPtr, mplsFlowLabel,
                                    numOfLabels, 0/* don't care for HFT */,
                                    qosMappingTableIndex, lastQosUniformLabel,
                                    lastTtlUniformLabel, mplsLabelStack);
...
{
    DECLARE_FUNC_NAME(hftMplsLabel);

    GT_U8  *egrBufPtr = descrPtr->tunnelStartRestOfHeaderInfoPtr;/*buffer to put the TS L3 header + ethertype */

    __LOG(("Configuring HFT Common Parameters"));
    /* HFT Common Configuration */
    hftCommonConfig(devObjPtr, descrPtr, index, hftActionPtr);

    __LOG(("Configuring HFT Label Stack"));
    /* HFT MPLS Label Stack Configuration */
    hftLabelStackConfig(devObjPtr, descrPtr, index, hftActionPtr, egrBufPtr);
...
{
    DECLARE_FUNC_NAME(hftMplsLabelExt);
    ...
    /* GCF_Bits [47:5] */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_GCF_BITS_5_47_E);

    memset(gcfBits, 0, sizeof(GT_U32)*2);

    SMEM_SIP7_HA_HFT_GCF_BITS_GET(devObjPtr, hftActionPtr, index, gcfBits);
    __LOG(("GCF Bits[47:5] [%u] [%u] \n", gcfBits[0], gcfBits[1]));

    /* GCF_Bits [47:5] */
    fieldValue = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_FLOW_ID_E);
    __LOG(("Flow ID [%u]\n", fieldValue));
...
/**
* @internal sip7HaHft function
* @endinternal
*
* @brief   HFT Handling for SIP7
*
* @param[in]     devObjPtr                - pointer to device object.
* @param[in]     descrPtr                 - pointer to the frame's descriptor.
* @param[in,out] haInfoPtr                - ha internal info
...
(
    IN    SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN    SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
    IN    SMEM_SIP7_HA_HFT_TABLE_TYPE_ENT hftTableType,
    INOUT HA_INTERNAL_INFO_STC            *haInfoPtr,
    IN    GT_U32                          egressPort
)
{
    DECLARE_FUNC_NAME(sip7HaHft);
    ...
    /* pointer to the 'memory line' of the HFT entry */
    if (hftTableType == SMEM_SIP7_HA_HFT_TABLE_0_E)
    {
        index        = descrPtr->hft0Ptr;
        hftActionPtr = smemMemGet(devObjPtr,
            SMEM_TABLE_ENTRY_INDEX_GET_MAC(devObjPtr, haHeaderFieldsTable0, index));
    }
    else
    {
        index        = descrPtr->hft1Ptr;
        hftActionPtr = smemMemGet(devObjPtr,
            SMEM_TABLE_ENTRY_INDEX_GET_MAC(devObjPtr, haHeaderFieldsTable1, index));
    }

    hftType = SIP7_HFT_FIELD_GET_MAC(SMEM_SIP7_HA_HEADER_FIELDS_ENTRY_TYPE_E);
    __LOG_PARAM(hftType);

    switch(hftType)
    {
        case SKERNEL_FRAME_HFT_TYPE_GENERIC_E:
            /* TBD */
            break;
        case SKERNEL_FRAME_HFT_TYPE_GENERIC_EXT_E:
            /* TBD */
            break;
        case SKERNEL_FRAME_HFT_TYPE_MPLS_LABEL_E:
            nextBufferPtr = hftMplsLabel(devObjPtr, descrPtr, haInfoPtr, index, hftActionPtr);
            break;
        case SKERNEL_FRAME_HFT_TYPE_MPLS_LABEL_EXT_E:
            nextBufferPtr = hftMplsLabelExt(devObjPtr, descrPtr, haInfoPtr, index, hftActionPtr);
            break;
        default:
            skernelFatalError("Not supported HFT type: %d\n", hftType);
            break;
    }
    ...
    __LOG(("end of build Header Fields Table\n"));
