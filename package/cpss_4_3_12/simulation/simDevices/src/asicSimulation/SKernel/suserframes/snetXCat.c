...
)
{
    ...
    GT_BOOL tagFound;
    GT_U32  tagSize;
    GT_U32  maxAllowedVlanTagOffset = descrPtr->maxAllowedVlanTagOffset;

    tagFound = GT_TRUE; /* default value */
    ...
    for (i = 0; i < SNET_XCAT_GLOBAL_TPID_NUM_CNS; i++)
    {
        ...
        if((ethTypeOffset + tagSize) > maxAllowedVlanTagOffset)
        {
            __LOG(("WARNING : for etherType[0x%4.4x] matched TPID[%d], but (ethTypeOffset[%d] + tagSize [%d]) > maxAllowedVlanTagOffset[%d] (so not parsed as vlan TAG) \n"
                   "may also impact the L3 offset recognition \n",
                etherType,i,ethTypeOffset,tagSize,maxAllowedVlanTagOffset));
            break;
        }
        ...
        if ((didTag1Parse == GT_FALSE) && (tagFound != GT_FALSE)
           && (tagIndex == 1) && (modifyTag0Tag1Indication == GT_TRUE))
        {
            descrPtr->vid1 =
...
)
{
    ...
    GT_U32  enable_classifying_inner_tag_as_tag0;/*sip7 : when Enabled. Tag0 may be classified as either the outer tag or inner tag. */
    ...
    if(descrPtr->eArchExtInfo.ttiPreTtiLookupIngressEPortTablePtr)
    {
        ...
        {
            /* assign default values of UP1 and CFIDEI1 */
            descrPtr->up1 =
                SMEM_LION3_TTI_DEFAULT_EPORT_ENTRY_FIELD_GET(devObjPtr, descrPtr,
    ...
    else /* Check Non DSA Outer Tag1 match */
    {
        ...
        if(retVal)
        {
            ...
            *inVlanEtherType1Ptr = etherType;
            ...
            {
                /* enable_classifying_inner_tag_as_tag0 */
                smemRegFldGet(devObjPtr, SMEM_SIP7_TTI_UNIT_GLOBAL_CONFIG_EXT5_REG(devObjPtr) ,
                    28 , 1 ,&enable_classifying_inner_tag_as_tag0);
                __LOG_PARAM(enable_classifying_inner_tag_as_tag0);
            }
            else    /* allow common code for sip7 devices and legacy devices to sip7  */
            {
                enable_classifying_inner_tag_as_tag0 = 1;
            }

            if(enable_classifying_inner_tag_as_tag0)
            {
                /* Tag0 ethernet type */
                etherType  = (descrPtr->startFramePtr[ethTypeOffset] << 8) |
                              descrPtr->startFramePtr[ethTypeOffset + 1];
            ...
            else    /* relevant only to sip7 */
            {
                __LOG(("sip7 : The inner tag can't be classified as 'TAG0' (according to <enable_classifying_inner_tag_as_tag0>)\n"
                       "       so it is considered 'TAG0 not found' \n" ));
                descrPtr->ingressVlanTag0Type =
                    SKERNEL_FRAME_TR101_VLAN_TAG_NOT_FOUND_E;
            }
        }
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        if(2 == port_isolation_indexing_mode)/* new mode in sip6.10 */
        {
            ...
            else
            {
                ...
                {
                    __LOG(("VLAN based Port isolation is NOT performed. Feature is disabled because SrcID isolation bit[%d] is off \n",
                        fieldVal));
                    return;
                }
        ...
        if(1 == port_isolation_indexing_mode)
        {
            ...
            if(origIsTrunk == 0)
            {
                ...
                {
                    __LOG(("Port isolation is NOT performed. because origIsTrunk == 0 and srcIsVPort == 1 and indexing mode is 'eport'  \n"));
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        trgSniffPort    = descrPtr->trgEPort;/* use eport and not physical (JIRA : CPSS-15563 : [WM][DSA][TO_ANALYZER][TO_CPU] Remote CPU Analyzer Port)*/
    }
    else
    {
        /* Source-based forwarding mode. Port+Device of the analyzer from the DSA tag */
