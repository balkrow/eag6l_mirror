...
)
{
    ...
    SNET_SIP6_10_DESC_SW2HW_FIELD_SET( hwDescBusPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL2_FOR_HASH_E,              descrPtr->label[2]);
    SNET_SIP6_10_DESC_SW2HW_FIELD_SET( hwDescBusPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL1_FOR_HASH_E,              descrPtr->label[1]);
    SNET_SIP6_10_DESC_SW2HW_FIELD_SET( hwDescBusPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL0_FOR_HASH_E,              descrPtr->label[0]);
...
)
{
    ...
    SNET_SIP6_10_DESC_HW2SW_FIELD_SET( hwDescBusPtr, hwDescMaskPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL2_FOR_HASH_E,              descrPtr->label[2]);
    SNET_SIP6_10_DESC_HW2SW_FIELD_SET( hwDescBusPtr, hwDescMaskPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL1_FOR_HASH_E,              descrPtr->label[1]);
    SNET_SIP6_10_DESC_HW2SW_FIELD_SET( hwDescBusPtr, hwDescMaskPtr, SNET_SIP6_10_PPU_DAU_DESC_FIELD_MPLS_LABEL0_FOR_HASH_E,              descrPtr->label[0]);
...
)
{
    ...
    if(kstgHdShift > descrPtr->frameBuf->actualDataSize ||
       kstgHdShift > SNET_SIP6_10_PPU_TOTAL_HD_LEN_MAX_CNS)
    {
    ...
    for(ii = 0; ii < SNET_SIP6_10_PPU_KSTG_ROTS_MAX_CNS; ii++)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                __LOG_PARAM(ppuActionEntryPtr->rotActionEntry[ii].setBitsOffset    );
...
)
{
    ...
    {
        __LOG(("K_stg tcam match not found.\nHence the 64th action(index=%u)"
               " is considered as default action.\n", *matchIndex));
    }
    else
    {
...
)
{
    ...
    hdShift = ppuActionEntry.setConstNextShift;
...
static GT_VOID snetHawkPpuDauProtectedWindowLatchDataWrite
(
        IN  SKERNEL_DEVICE_OBJECT           *devObjPtr,
        IN  GT_U32                          spByteIndex,
        IN  GT_U32                          value,
        IN  GT_U32                          protWinIdx
)
{
    DECLARE_FUNC_NAME(snetHawkPpuDauProtectedWindowLatchDataWrite);

    GT_U32 regValue;
    __LOG(("DAU Protected Window Error Latch Data : spByteIndex=%u, value=0x%X, protWinIdx=%u\n",
           spByteIndex, value, protWinIdx));
    regValue = ((protWinIdx & 0x7) |
                ((value & 0xFF) << 3) |
                ((spByteIndex & 0x1F) << 11));
    smemRegSet(devObjPtr, SMEM_SIP6_10_PPU_DAU_PROTECTED_WINDOW_ERROR_LATCH_REG(devObjPtr), regValue);
}
...
)
{
    ...
    GT_U32   matchIndex = SNET_SIP6_10_PPU_KSTG_TCAM_ENTRIES_MAX_CNS;
...
)
{
    ...
    if(isError == GT_FALSE)
    {
        ...
        for(ii=0; ii<SNET_SIP6_10_PPU_DAU_DESC_BYTE_SET_MAX_CNS; ii++)
        {
            ...
            rc = snetHawkPpuDauProtectedWindowCheck(devObjPtr, spByteTargetOffset, spByteNumBits, &protWinIdx);
            if(rc != GT_OK)
            {
                __LOG(("Descriptor alteration bits inside protected window. offset: %d, numBits: %d\n",
                        spByteTargetOffset, spByteNumBits));

                snetHawkPpuDauProtectedWindowLatchDataWrite(devObjPtr, ii, val, protWinIdx);
    ...
    else if(errProfIdx > 0)
    {
        ...
        for(ii = 0; ii < SNET_SIP6_10_PPU_ERROR_PROFILE_FIELDS_MAX_CNS; ii++)
        {
            ...
            rc = snetHawkPpuDauProtectedWindowCheck(devObjPtr, errProfOffset, errProfNumBits, &protWinIdx);
            if(rc != GT_OK)
            {
                __LOG(("Descriptor alteration bits inside protected window. offset: %d, numBits: %d\n",
                        errProfOffset, errProfNumBits));

                /* SP bus data is unavailable hence Protection Window error latch data is not updated */
