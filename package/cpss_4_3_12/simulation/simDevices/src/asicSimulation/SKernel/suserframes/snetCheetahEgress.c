...
static GT_BOOL wmIsPhyIn2StepMode = GT_FALSE;
GT_STATUS wmSetPhyIn2StepMode(IN GT_BOOL enable)
{
    wmIsPhyIn2StepMode = enable;

    return GT_OK;
}
GT_STATUS wmGetPhyIn2StepMode(OUT GT_BOOL *enablePtr)
{
    *enablePtr = wmIsPhyIn2StepMode;

    return GT_OK;
}

/* cumulativeScaledRateOffset location in the TLV header */
#define ERMERK_FOLOWUP_TLV_CUM_SCALED_RATE_OFFSET   54

/* cumulativeScaledRateOffset length in octets */
#define ERMERK_FOLOWUP_TLV_CUM_SCALED_RATE_WIDTH    4
...
)
{
    ...
    */
    ...
    {
        outPorts = 1;
        if(descrPtr->trgEPort < SKERNEL_CHEETAH_EGRESS_MAX_PORT_CNS)
        {
...
)
{
    ...
    /* mirror copy should not derive Latency monitoring parameters */
    txSniffDescPtr->lmuEn = 0;
...
#define __LOG_SIP7_EGF_EFT_MIB_COUNTER(counterName,setId,oldValue) \
    __LOG(("EGF_EFT mib counter [%s high 32 bits] for set[%d], incremented by 1 from value of [0x%8.8x] \n", \
        #counterName , setId , oldValue))
...
)
{
    ...
    if (outCount0 || outCount1)
    {
        ...
        if(outCount0)
        {
            ...
            /* support 64bit counter in sip7 */
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr) && Counter < outCount0)
            {
                /* update the high 32 bits of the counter */
                smemRegGet(devObjPtr,counterRegAddr0+0x8, &Counter);
                __LOG_SIP7_EGF_EFT_MIB_COUNTER(egrMIBBridgeEgrFilteredPktCntr,0,Counter);
                Counter ++;
                smemRegSet(devObjPtr,counterRegAddr0+0x8, Counter);
            }
        }
        ...
        if(outCount1)
        {
            ...
            /* support 64bit counter in sip7 */
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr) && Counter < outCount1)
            {
                /* update the high 32 bits of the counter */
                smemRegGet(devObjPtr,counterRegAddr1+0x8, &Counter);
                __LOG_SIP7_EGF_EFT_MIB_COUNTER(egrMIBBridgeEgrFilteredPktCntr,1,Counter);
                Counter ++;
                smemRegSet(devObjPtr,counterRegAddr1+0x8, Counter);
            }
        }
    }
    ...
    if (mcFltr0 || mcFltr1)
    {
        ...
        if(mcFltr0)
        {
            ...
            /* support 64bit counter in sip7 */
            if(SMEM_CHT_IS_SIP7_GET(devObjPtr) && Counter == 0)
            {
                /* update the high 32 bits of the counter */
                smemRegGet(devObjPtr,counterRegAddr0+0x8, &Counter);
                __LOG_SIP7_EGF_EFT_MIB_COUNTER(egrMIBMcFilteredPktCntr,1,Counter);
                Counter ++;
                smemRegSet(devObjPtr,counterRegAddr0+0x8, Counter);
            }
        }
...
)
{
    ...
    numBitsPerPort     = devObjPtr->egfNumBitsPerPort ?  devObjPtr->egfNumBitsPerPort :
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        GT_U32  numBitsPort  = SMEM_CHT_IS_SIP7_GET(devObjPtr)    ? 10 :
                               SMEM_CHT_IS_SIP6_10_GET(devObjPtr) ?  8 :
                               SMEM_CHT_IS_SIP6_GET(devObjPtr)    ? 10 :
                              (SMEM_CHT_IS_SIP5_20_GET(devObjPtr) ? 9 : 8);
    ...
    else
    {
        ...
        if(SMEM_U32_GET_FIELD(regData, 8, 1) == 0)
        {
            cscdTrunkMode = TRUNK_HASH_MODE_USE_PACKET_HASH_E;
    ...
    switch(tableType)
    {
        case SNET_CHT_EGF_SHT_TABLE_NON_TRUNK_MEMBERS_2_TYPE_E:
            if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr))
            {
                ...
                regPtr = smemMemGet(devObjPtr, regAddr);
                /* fill the info */
                SKERNEL_FILL_PORTS_BITMAP_MAC(devObjPtr, egressPortBmpPtr, regPtr);
            }
            else
            if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
            {
...
)
{
    ...
    {                                                            /*called from loop in snetChtTxQPortFrwrdRestriction*/
        ...
        if(descrPtr->eArchExtInfo.egfShtEgressEPortTablePtr)
        {
            ...
            {
                /* Only outgoing FORWARD packets are checked by the filter */
                if(descrPtr->outGoingMtagCmd == SKERNEL_MTAG_CMD_FORWARD_E)
                {
                    if (descrPtr->doRouterHa == 0)
                    {
...
/**
* @internal snetPcaLmuProcessing function
* @endinternal
*
* @brief   (MAC) PCA level latency monitoring processing
*
* @param[in] devObjPtr                -  pointer to device object.
...
*
*/
static GT_VOID snetPcaLmuProcessing
(
    IN SKERNEL_DEVICE_OBJECT              * devObjPtr,
...
{
    DECLARE_FUNC_NAME(snetPcaLmuProcessing);
    ...
    lmuProfile = descrPtr->lmuProfile;

    if(devObjPtr->numOfRavens == 0)/*Hawk or SIP7 */
    {
        ...
        {
            gopIndex = portInfo.simplePortInfo.unitIndex;
            lmuNum = gopIndex * 8 + lmuProfile / 512; /* profile:[11..9][8..0]bit */
            lmuProfile %= 512;
            __LOG(("Latency monitoring (gop=%u + lmuProfile=%u -> lmuNum=%d) for macPort [%d] \n",
                   gopIndex,lmuProfile , lmuNum, macPort));
        }
        else
        {
    ...
    __LOG(("Latency profile configuration for profile# %d\n"
           ...
           lmuProfile, cfgRangeMin, cfgRangeMax,
...
)
{
    ...
    if (descrPtr->ptpPrecisionMode != SKERNEL_PTP_PRECISION_MODE_USX_E)
    {
    ...
    {
        regAddr = MTI_PORT_EXT(devObjPtr, macPort).portEmacUsxPchTxControl1;
    }
    else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
    {
        ...
        regAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex). portEmacUsxPchTxControl1;
    }
    else
    {
    ...
    else /* CONFIG_CTR */
    {
        ...
        {
            /* counter WA is modified to 14b upon increment */
            counter = SMEM_U32_GET_FIELD(regPtr[0], 2, 14) + 1;
            if(counter == 0x4000)
            {
                counter = 0;
            }
            SMEM_U32_SET_FIELD(regPtr[0], 2, 14, counter);

        }
        pchInfoPtr->signature.mchSignature = counter;
    }
...
)
{
    ...
    else /* SNET_PTP_PHY_STEP_TYPE_ONE_STEP_E */
    {
       ...
       /* save values before the update for checksum calculations */
       descrPtr->ptpTsOffset = (GT_U32)(ptpStart - frameDataPtr);
       descrPtr->ptpTsOffset1 = 0;
       descrPtr->ptpTsByteLength = 6;
       memset(&descrPtr->ptpTimestamp[0],0,10);
       memcpy(&descrPtr->ptpTimestamp[0],ptpStart,6);
        ...
        +--------+--------+--------+--------+
        |                          | frac   |
        | NanoSec(48bit)           | NanoSec|
        +--------+--------+--------+--------+*/

        /* handle the 48 bits (6 bytes) of NanoSec                     */
        oldCorrectionField.l[1] = ptpStart[0] << 8  |
                                  ptpStart[1] << 0;
        oldCorrectionField.l[0] = ptpStart[2] << 24 |
                                  ptpStart[3] << 16 |
                                  ptpStart[4] << 8  |
                                  ptpStart[5] << 0;
        ...
        {
            deltaTimer.nanoSecondTimer = (1000000000) -
        ...
        if (correctionField.l[1] > 0xFFFF)
        {
            ...
            correctionField.l[1] = 0x007FFFFF;
...
/**
* @internal snetCtsuProcessing function
* @endinternal
*
* @brief   CTSU processing
*
* @param[in] devObjPtr                -  pointer to device object.
* @param[in,out] descrPtr             -  pointer to the frame's descriptor.
* @param[in] egressPort               -  number of egress port.
...
*
*/
static GT_VOID snetCtsuProcessing
(
    IN SKERNEL_DEVICE_OBJECT              * devObjPtr,
    INOUT SKERNEL_FRAME_CHEETAH_DESCR_STC * descrPtr,
    GT_U32                                  egressPort,
...
{
    DECLARE_FUNC_NAME(snetCtsuProcessing);

    GT_U32 regAddr;  /* Register address */
    GT_U32 *regPtr;  /* Register entry pointer */

    /* Do timestamp action for CTSU unit */
    snetlion3PtpTimestampingAction(devObjPtr, descrPtr, egressPort, GT_TRUE);
    ...
    if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
    {
        ...
        /* bit 1: Channel %n Egress TS Encrypted: When this bit is enabled, channel %n egress timestamp is corrected by adding egress pipe delay
           stored in register <Channel %n Egress Pipe Delay Encrypted>, thus channel %n encrypted frames are time-stamped at high precision. */
        if (SMEM_U32_GET_FIELD(regPtr[0], 1, 1) == 1)
        {
            descrPtr->ptpPrecisionMode = SKERNEL_PTP_PRECISION_MODE_TSTF_E;
            __LOG(("Precision Mode TSTF for Channel [%d].\n",macPort));
        }
        else if (descrPtr->mtiCfUpdate)
        {
            if (descrPtr->pktMchTwoStepEn)
            {
                ...
                __LOG(("Precision Mode USX for Channel [%d].\n",macPort));
            }
            else
            {
                ...
                __LOG(("Precision Mode CF for Channel [%d].\n",macPort));
            }
        }
        else
        {
            ...
            else
            {
                ...
                __LOG(("Precision Mode DISABLED for Channel [%d].\n",macPort));
            }
        }
    }

    /* if targetPort is connected to PHY && port is USXGMII && CF was not done in the MAC */
    if(connectedPortToPhyEgress[egressPort] && devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E && descrPtr->ptpPrecisionMode == SKERNEL_PTP_PRECISION_MODE_USX_E)
    {
        /* MAC creates USXGMII PCH for the PHY */
        snetSip6_30PtpUsxgmiiPchTxProcessing(devObjPtr,descrPtr,macPort,pchInfoPtr);
    }
    ...
    /* if targetPort is connected to PHY && port is USXGMII && CF was not done in the MAC;
       or when capturing in the TSQ and identifying as 'USX' so port is not really connected to PHY  */
    if(descrPtr->ptpPrecisionMode == SKERNEL_PTP_PRECISION_MODE_USX_E)
    {
        /* Capture to timestamp port MAC egress queue */
        snetSip6_30TimestampPortMacEgressQueueCapture(devObjPtr,descrPtr,macPort);
    }
...
)
{
    ...
    isEgressLoopback = smemIsPortEgressLoopbackGet(devObjPtr, egressPort);
    if (isEgressLoopback)
    {
        __LOG(("packet Tx on Egress Loopback port %d\n", egressPort));
        portInLoopBack = 1; /* HW permanent Egress Loopback on dedicated channel */
        goto afterMac;
    }
    ...
    if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr) && !debug_performance_bypass_tx_mif_mapping)
    {
        ...
        simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_MIF_E);
        ...
        */
        ...
        /*restore to SIM_LOG_FRAME_UNIT_PORT_MAC_EGRESS_E*/
        simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT___RESTORE_PREVIOUS_UNIT___E);
    }
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_CTSU_E);

        SIM_LOG_PACKET_DESCR_SAVE
        snetCtsuProcessing(devObjPtr,descrPtr,egressPort, macPort,&pchInfo);
        SIM_LOG_PACKET_DESCR_COMPARE("snetCtsuProcessing");
        ...
        if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
        {
            simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_LMU_E);
            snetPcaLmuProcessing(devObjPtr, descrPtr, macPort);
            /*restore to SIM_LOG_FRAME_UNIT_PORT_MAC_EGRESS_E*/
            simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT___RESTORE_PREVIOUS_UNIT___E);
        }
    }
...
)
{
    ...
    /* fields after bit 3 moved by 3 in sip7 */
    startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 11 : 8;

    /*eVLAN Command*/
    fldVal = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr,
        startBit , 1);
    if(fldVal)
    {
        /* fields after bit 3 moved by 3 in sip7 */
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 15 : 12;

        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            /* sip7 : Assign new tag0Vid (not changing eVid) to the egress packet */
            descrPtr->tag0Vid =
                snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , evidNumBits);
            __LOG(("Sip7 : Assign new tag0Vid [%d] to the egress packet (by eport[%d]) (eVid keep[%d])\n",
                descrPtr->tag0Vid,
                descrPtr->eArchExtInfo.egfQagEgressEPortTable_index,
                descrPtr->eVid));
        }
        else
        {
            ...
            descrPtr->eVid =
                snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , evidNumBits);
            __LOG(("Assign new eVLAN [%d] to the egress packet (by eport[%d]) \n",
                descrPtr->eVid,
                descrPtr->eArchExtInfo.egfQagEgressEPortTable_index));
        }
    }
    ...
    /* fields after bit 3 moved by 3 in sip7 */
    startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 12 : 9;
    /*VID1 Command*/
    fldVal = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 1);
    if(fldVal)
    {
        /* fields after bit 3 moved by 3 in sip7 */
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 27 : startBitAfterEvid;

        descrPtr->vid1 =
            snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 12);
        __LOG(("Assign new vid1 [%d] to the egress packet (by eport[%d]) \n",
            descrPtr->vid1,
            descrPtr->eArchExtInfo.egfQagEgressEPortTable_index));
    }

    /* fields after bit 3 moved by 3 in sip7 */
    startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 13 : 10;
    /*UP0 Command*/
    fldVal = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 1);
    if(fldVal)
    {
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 39 : (startBitAfterEvid+12);
        descrPtr->up =
            snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 3);
        __LOG(("Assign new up0 [%d] to the egress packet (by eport[%d]) \n",
            descrPtr->up,
            descrPtr->eArchExtInfo.egfQagEgressEPortTable_index));
    }

    /* fields after bit 3 moved by 3 in sip7 */
    startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 14 : 11;
    /*UP1 Command*/
    fldVal = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 1);
    if(fldVal)
    {
        /* fields after bit 3 moved by 3 in sip7 */
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 42 : (startBitAfterEvid+12+3);
        descrPtr->up1 =
            snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 3);
        __LOG(("Assign new up1 [%d] to the egress packet (by eport[%d]) \n",
            descrPtr->up1,
            descrPtr->eArchExtInfo.egfQagEgressEPortTable_index));
    }
...
)
{
    ...
    snetChtDoInterruptInMgInAllTrees(devObjPtr,
                           SMEM_CHT_SDMA_INT_CAUSE_REG(devObjPtr,0),
                           SMEM_CHT_SDMA_INT_MASK_REG(devObjPtr,0),
                           (1 << firstBit),
...
)
{
    ...
    if (devObjPtr->supportGdma)
    {
        snetSip7GdmaSendPacketToCpu(devObjPtr,
            descrPtr->egressPhysicalPortInfo.txDmaMacPort,/* DMA# of the target port (for __LOG info ) */
            descrPtr->egressPhysicalPortInfo.sip6_sdqLocalQueue & 0xF ,/* instead of descrPtr->tc */
            frameData, frameSize);
        return;
    }

    simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_MG_TO_CPU_E);
...
*                                      None
*/
static GT_VOID snetlion3PtpUdpChecksumTrailerCalc
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN SKERNEL_FRAME_CHEETAH_DESCR_STC * descrPtr
)
{
    DECLARE_FUNC_NAME(snetlion3PtpUdpChecksumTrailerCalc);
    GT_U8 *udpHeaderPtr;
    GT_U8  udpTrailerPseudoHeader[42];
    GT_U32 regAddr;             /* Register address */
    GT_U32 *regPtr;             /* Register pointer */
    GT_U32 headerSize,diff;
    GT_U16 udpTrailerChecksum;
    GT_U32 i,j;
    ...
    if(descrPtr->ptpTsOffset == 0 || descrPtr->ptpTsByteLength == 0)
    {
        __LOG(("Timestamp Header Not Valid"));
        return;
    }

    /* descrPtr->ptpUdpChecksumMode == SNET_PTP_UDP_CHECKSUM_MODE_UPDATE_E */
    __LOG(("Update UDP checksum trailer field for PTPoUDP"));

    memset(&udpTrailerPseudoHeader[0],0,42);

    /* copy ~bytes orig timestamp */
    for (i=0; i<descrPtr->ptpTsByteLength;i++)
    {
        udpTrailerPseudoHeader[i] = ~descrPtr->ptpTimestamp[i];
    }
    if (descrPtr->ptpTsOffset1 != 0)
    {
        /* copy ~bytes orig timestamp1 */
        for (;i<descrPtr->ptpTsByteLength; i++)
        {
            udpTrailerPseudoHeader[i] = ~descrPtr->ptpTimestamp1[i];
        }
    }
    /* copy bytes new timestamp */
    for (j=0; j<descrPtr->ptpTsByteLength;j++)
    {
        udpTrailerPseudoHeader[i++] = devObjPtr->egressBuffer[descrPtr->ptpTsOffset+j];
    }
    if (descrPtr->ptpTsOffset1 != 0)
    {
        /* copy bytes new timestamp1 */
        for (j=0; j<descrPtr->ptpTsByteLength;j++)
        {
            udpTrailerPseudoHeader[i++] = devObjPtr->egressBuffer[descrPtr->ptpTsOffset1+j];
        }
    }
    ...
    if (descrPtr->ptpUdpChecksumUpdateEnable && SMEM_U32_GET_FIELD(regPtr[0], 12, 1) == 0)
    {
        __LOG(("NO UDP checksum trailer field update for PTPoUDP"));
        return;
    }

    headerSize = MIN((SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 256 : 128),descrPtr->byteCount - 4 /* 4 bytes of CRC */);
    if(descrPtr->egressByteCount > descrPtr->byteCount)
    {
        diff = (descrPtr->egressByteCount - descrPtr->byteCount) + headerSize;
    }
    else
    {
        diff = 0;
    ...
    descrPtr->ptpUdpChecksumOffset = headerSize + diff;
    descrPtr->ptpUdpChecksumOffset = descrPtr->ptpUdpChecksumOffset - 2 /* 2 bytes of checksum */;
    if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
        descrPtr->ptpUdpChecksumOffset &= 0xFF;/*limited to 8 bits in HW in 2 bits resolution ---> so 0xFF for WM (this is 0xFF  = (0xff<<1) , 8 bits in 2 bits resolution) */
    else
        descrPtr->ptpUdpChecksumOffset &= 0xFE;/*limited to 7 bits in HW in 2 bits resolution ---> so 0xFE for WM (this is 0xFE  = (0x7f<<1) , 7 bits in 2 bits resolution) */

    /* copy ~2 last bytes for compensation (trailer offset) */
    udpTrailerPseudoHeader[i]   = ~devObjPtr->egressBuffer[descrPtr->ptpUdpChecksumOffset];
    udpTrailerPseudoHeader[i+1] = ~devObjPtr->egressBuffer[descrPtr->ptpUdpChecksumOffset+1];

    /* clac incremental checksum of timestamp bytes + 2 trailer bytes */
    udpTrailerChecksum = ~checkSumCalc(udpTrailerPseudoHeader, i+2);
    devObjPtr->egressBuffer[descrPtr->ptpUdpChecksumOffset] = (udpTrailerChecksum & 0xFF00)>>8;
    devObjPtr->egressBuffer[descrPtr->ptpUdpChecksumOffset+1] = (udpTrailerChecksum & 0xFF);
}
...
)
{
    ...
    snetlion3PtpUdpChecksumTrailerCalc(devObjPtr, descrPtr);
}
...
)
{
    ...
    if((egressPort == SNET_CHT_CPU_PORT_CNS) && (descrPtr->cpuCode == SNET_CHT_INGRESS_MIRRORED_TO_ANALYZER))
    {
        __LOG(("ingress Timestamp not relevant to CPU port (only to physical ports)\n"));
        return;
    }
    ...
    {
        __LOG(("Ingress Timestamp nanoSec was piggybacked onto the 1588v2 packet, using the 4B <reserved>. field in the PTP header.\n"));
        /* PTP timestamping based on Local Action Table */
        ...
        nanoSecondTimer = descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer;
...
)
{
    ...
    ptpStart = descrPtr->haToEpclInfo.l3StartOffsetPtr +
               (descrPtr->ptpGtsInfo.ptpMessageHeaderPtr - descrPtr->payloadPtr);
    ptpStart += 8; /* correction field offset; nanoSec part */

    /* save values before the update for checksum calculations */
    descrPtr->ptpTsOffset = (GT_U32)(ptpStart - devObjPtr->egressBuffer);
    descrPtr->ptpTsOffset1 = 0;
    descrPtr->ptpTsByteLength = 6;
    memset(&descrPtr->ptpTimestamp[0],0,10);
    memcpy(&descrPtr->ptpTimestamp[0],ptpStart,6);
    ...
    __LOG(("before change : correction field: [0x%2.2x%2.2x%2.2x%2.2x%2.2x%2.2x] \n",
        ptpStart[0],
        ptpStart[1],
        ptpStart[2],
        ptpStart[3],
    ...
    ptpStart[0] = (correctionField.l[1] >> 8) & 0xFF;
    ptpStart[1] = (correctionField.l[1] >> 0) & 0xFF;
    ptpStart[2] = (correctionField.l[0] >> 24) & 0xFF;
    ptpStart[3] = (correctionField.l[0] >> 16) & 0xFF;
    ptpStart[4] = (correctionField.l[0] >> 8) & 0xFF;
    ptpStart[5] = (correctionField.l[0] >> 0) & 0xFF;
    __LOG(("after change : correction field: [0x%2.2x%2.2x%2.2x%2.2x%2.2x%2.2x] \n",
        ptpStart[0],
        ptpStart[1],
        ptpStart[2],
        ptpStart[3],
...
)
{
    ...
    if(followUpTlvUpdateEn || syncTlvUpdateEn)
    {
        ...
        {
            __LOG(("cumulative scaled rate offset was NOT updated in the 1588v2 packet"));
            return;
        }

        if(descrPtr->haToEpclInfo.tunnelStartL2StartOffsetPtr != NULL)
        {
            ptpOffset = ptpStart - descrPtr->haToEpclInfo.tunnelStartL2StartOffsetPtr;
        }
        else
        {
            ptpOffset = ptpStart - descrPtr->haToEpclInfo.macDaSaPtr;
        }

        if((GT_U32)(ptpOffset + ERMERK_FOLOWUP_TLV_CUM_SCALED_RATE_OFFSET + ERMERK_FOLOWUP_TLV_CUM_SCALED_RATE_WIDTH) >= descrPtr->byteCount)
        {
            /* PTP Exception: PTPV2 Follow-Up TLV Cumulative Scaled Rate Offset field can't be reached */
            __LOG(("PTP Exception due to PTPV2 Follow-Up TLV Cumulative Scaled Rate Offset field can't be reached"));
            snetChetahDoInterrupt(devObjPtr,
        ...
        ptpStart += ERMERK_FOLOWUP_TLV_CUM_SCALED_RATE_OFFSET; /* cumulative scaled rate offset */

        regAddr = SMEM_LION3_ERMRK_CUMULATIVE_SCALE_RATE_CONFIG_REG(devObjPtr,descrPtr->ptpTaiSelect,1);
        regPtr = smemMemGet(devObjPtr, regAddr);

        *(ptpStart++) = SMEM_U32_GET_FIELD(regPtr[0],24, 8);
        *(ptpStart++) = SMEM_U32_GET_FIELD(regPtr[0],16, 8);
        *(ptpStart++) = SMEM_U32_GET_FIELD(regPtr[0],8, 8);
        *(ptpStart) = SMEM_U32_GET_FIELD(regPtr[0],0, 8);
...
)
{
    ...
    if ( ((descrPtr->timestampTagged[SMAIN_DIRECTION_INGRESS_E] == SKERNEL_TIMESTAMP_TAG_TYPE_NON_EXTENDED_E) ||
         ...
         (descrPtr->timestampTagged[SMAIN_DIRECTION_INGRESS_E] == SKERNEL_TIMESTAMP_TAG_TYPE_HYBRID_E)) && (egressPort != SNET_CHT_CPU_PORT_CNS))
    {
         /* take ingress time from timestamp tag */
         descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E] = descrPtr->timestampTagInfo[SMAIN_DIRECTION_INGRESS_E].timestamp;
         __LOG(("ingress time was update according to timestamp tag: tai [%d] nanoSec[0x%x] TST mode [%d]",descrPtr->ptpTaiSelect,descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer,descrPtr->timestampTagged[SMAIN_DIRECTION_INGRESS_E]));
    }

    if(descrPtr->timestampTagged[SMAIN_DIRECTION_EGRESS_E] == SKERNEL_TIMESTAMP_TAG_TYPE_PIGGY_BACKED_E)
    {
        /* The piggyback timestamp is represented as a 32-bit signed nanosecond value, that is computed as follows:
        ...
        descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer += (descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].secondTimer.l[0] & 1)*1000000000;
        if(descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].secondTimer.l[0] & 1)
        {
            __LOG(("piggyBack mode : Nanosecond field conversion from the ingress timestamp; adding the lsbit of the Second field: tai [%d] nanoSec[0x%x]",descrPtr->ptpTaiSelect,descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer));
        }
    }

    *ingressTimestampDataPtr = descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E];
    *egressTimestampDataPtr = descrPtr->timestamp[descrPtr->ptpEgressTaiSel][SMAIN_DIRECTION_EGRESS_E];
    ...
    if (descrPtr->ptpActionIsLocal)
    {
        ...
        __LOG(("ingressTimestamp.nanoSecondTimer was update [%ld]\n",ingressTimestampDataPtr->nanoSecondTimer));
    }
}
...
* @param[in] descrPtr                 - pointer to the frame's descriptor.
* @param[in] egressPort               - target port.
* @param[in] isCtsu                   - action is updated from CTSU unit or ERMRK unit.
*                                      OUTPUT:
*                                      None
*/
static GT_VOID snetlion3PtpTimestampingAction
...
)
{
    ...
    GT_U32 *regPtr;             /* Register pointer */
    GT_U64 correctionField,oldCorrectionField;
    GT_U64 deltaTimer64,reservedField;
    GT_BOOL isPhyIn2StepMode = GT_FALSE;
    GT_U8  *tmpPtpChangedBytes;/* pointer to start of timestamp changes */
    GT_U32 macPort,receptionEnable;
    GT_CHAR*    tsByteNamePtr;

    memset(&deltaTimer64,0,sizeof(deltaTimer64));
    memset(&correctionField,0,sizeof(correctionField));

    ingressTimestampData = descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E];
    egressTimestampData = descrPtr->timestamp[descrPtr->ptpEgressTaiSel][SMAIN_DIRECTION_EGRESS_E];
    ...
    /* pointer to CF; nanoSec part to be used when port is connected to PHY */
    if(descrPtr->haToEpclInfo.tunnelStartL2StartOffsetPtr != NULL)
    {
        descrPtr->ptpCfOffset = ptpStart + 8 - descrPtr->haToEpclInfo.tunnelStartL2StartOffsetPtr;
    }
    else
    {
        descrPtr->ptpCfOffset = ptpStart + 8 - descrPtr->haToEpclInfo.macDaSaPtr;
    }

    if (SMEM_CHT_IS_SIP6_30_GET(devObjPtr) && (isCtsu == GT_TRUE))
    {
        ...
        if ((SMEM_U32_GET_FIELD(regPtr[0], 3, 1) == 1) /* port enabled for USXGMII */ &&
            ...
            (descrPtr->timestampActionInfo.packetFormat == SKERNEL_TIMESTAMP_PACKET_FORMAT_PTPV2_E))
        {
            __LOG(("macPort [%d] is enabled for USXGMII PTP processing.\n",macPort));
            pktUsxEn = 1;
        }

        if ((SMEM_U32_GET_FIELD(regPtr[0], 6, 1) == 1) /* port enabled for MCH */ && pktUsxEn)
        {
            __LOG(("macPort [%d] is enabled for MCH PTP processing.\n",macPort));
            pktMchEn = 1;
        }
        ...
        {
            pktMchTwoStepEn = 1;
            __LOG(("macPort [%d] is set to two step MCH for a capture action.\n",macPort));
        }

        if (SMEM_U32_GET_FIELD(regPtr[0], 0, 1) == 1 /* mac_cf_update_en */ &&
            (descrPtr->timestampActionInfo.packetFormat == SKERNEL_TIMESTAMP_PACKET_FORMAT_PTPV2_E &&
        ...
        {
            __LOG(("Channel [%d] is enabled for additional Egress TS Correction in the MAC.\n",macPort));
            descrPtr->mtiCfUpdate = 1;
        }
        else
        {
            __LOG(("Channel [%d] is not enabled for additional Egress TS Correction in the MAC.\n",macPort));
        ...
        {
            pktMchOneStepEn = 1;
            __LOG(("macPort [%d] is set to one step MCH for a capture action.\n",macPort));
        }

        /* build PTP PHY signature */
        wmGetPhyIn2StepMode(&isPhyIn2StepMode);

        if(isPhyIn2StepMode) /* PHY to capture egress time */
        {
        ...
        else /* CF to be updated by USX PHY; if (PTP ACTION is ADD_CORRECTED_TIME or CAPTURE_ADD_CORRECTED_TIME) CTSU CF update */
        {
            ...
            descrPtr->ptpPhyInfo.cfOffset = ((descrPtr->ptpCfOffset >> 1) & 0x7F);
        }
    }

    descrPtr->pktMchTwoStepEn = pktMchTwoStepEn;
    ...
    switch(descrPtr->timestampActionInfo.action)
    {
        ...
        case SKERNEL_TIMESTAMP_ACTION_CAPTURE_ADD_CORRECTED_TIME_E:
            ...
            {
                timestampData.nanoSecondTimer = (1000000000) -
                    ( egressTimestampData.nanoSecondTimer -
                      ingressTimestampData.nanoSecondTimer );
    ...
    else if(tsMode == TIMESTAMP_TIME_ADD_MODE_CORRECTED_E)
    {
        ...
        {
            case SKERNEL_TIMESTAMP_PACKET_FORMAT_PTPV2_E:
                ptpStart += 8; /* pointer to CF; nanoSec part */
                tsByteLength = 6;

                tsByteNamePtr = "correction field";
                ...
                if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
                {
                    receptionEnable = SMEM_SIP7_HA_SRC_PHYSICAL_PORT_2_ENTRY_FIELD_GET(devObjPtr,
                ...
                {
                    receptionEnable = SMEM_LION3_HA_PHYSICAL_PORT_2_ENTRY_FIELD_BY_SRC_PORT_GET(devObjPtr,
                         descrPtr->localDevSrcPort,/* the src physical port */
                         SMEM_LION3_HA_PHYSICAL_PORT_TABLE_2_FIELDS_TIMESTAMP_RECEPTION_EN);
                }

                /* add nanoSec from reserved field to CF nanoSec */
                if (receptionEnable)
                {
                    __LOG(("add reserved field from timestampTagInfo [0x%x]\n",descrPtr->timestampTagInfo[SMAIN_DIRECTION_INGRESS_E].timestamp.nanoSecondTimer));

                    reservedField.l[0] = descrPtr->timestampTagInfo[SMAIN_DIRECTION_INGRESS_E].timestamp.nanoSecondTimer;
                    reservedField.l[1] = 0;

                    correctionField = prvSimMathAdd64(correctionField,reservedField);
                }
    ...
    if (isCtsu == GT_FALSE)
    {
        ...
        descrPtr->ptpTsOffset = (GT_U32)(ptpStart - devObjPtr->egressBuffer);
        descrPtr->ptpTsOffset1 = 0;
        descrPtr->ptpTsByteLength = tsByteLength;
        memset(&descrPtr->ptpTimestamp[0],0,10);
        memcpy(&descrPtr->ptpTimestamp[0],ptpStart,tsByteLength);
        if (ptpStart1 != NULL)
        {
            descrPtr->ptpTsOffset1 = (GT_U32)(ptpStart1 - devObjPtr->egressBuffer);
            memset(&descrPtr->ptpTimestamp1[0],0,10);
            memcpy(&descrPtr->ptpTimestamp1[0],ptpStart1,tsByteLength);
        }
    }

    tmpPtpChangedBytes = ptpStart;

    if(tsByteLength)
    {
        __LOG(("PTP : Update of %s %d bytes\n",tsByteNamePtr,tsByteLength));

        __LOG(("Dump PTP BEFORE the change of [%d] bytes \n",
            tsByteLength));
        /* dump the start of L3 */
        simLogPacketDump(devObjPtr,
                        GT_FALSE,/*egress*/
                        egressPort,/* egress port */
                        tmpPtpChangedBytes,/*start of the changed bytes */
                        tsByteLength);/* dump the changed bytes */
    }
    ...
    if(tsByteLength)
    {
        __LOG(("Dump PTP AFTER the change of [%d] bytes \n", tsByteLength));
        /* dump the start of L3 */
        simLogPacketDump(devObjPtr,
                        GT_FALSE,/*egress*/
                        egressPort,/* egress port */
...
)
{
        ...
        (packetFormat == SKERNEL_TIMESTAMP_PACKET_FORMAT_PTPV2_E) )
    {
        ...
        queueDomainNumber = descrPtr->ptpDomain_8bits;
    ...
    memset(timestampQueueEntry, 0x0, sizeof(timestampQueueEntry));
                    ...
                    SKERNEL_TIMESTAMP_ACTION_CAPTURE_ADD_CORRECTED_TIME_E) )
    {
        ...
        {
            if (descrPtr->ptpEgressTaiSel != 0 && !SMEM_CHT_IS_SIP6_GET(devObjPtr))
            {
        ...
        {
            /* Timestamp (full single word) */
            timestampQueueEntry[1] =  (egressTimestampData.secondTimer.l[0] & 0x3) << 30 | (egressTimestampData.nanoSecondTimer & 0x3FFFFFFF);
        }
                    ...
                    SKERNEL_TIMESTAMP_ACTION_CAPTURE_INGRESS_TIME_E) )
    {
        ...
        snetFieldValueSet(timestampQueueEntry, 35, 32,
                          ingressTimestampData.nanoSecondTimer);
        /* Physical Port*/
        snetFieldValueSet(timestampQueueEntry, 67, 8, descrPtr->localDevSrcPort);
...
/**
* @internal snetSip6_30TimestampPortMacEgressQueueInterruptEvent function
* @endinternal
*
* @brief   Setting interrupt for timestamp port egress queue event - full or new entry.
*
...
*                                      None
*/
static GT_VOID snetSip6_30TimestampPortMacEgressQueueInterruptEvent
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN GT_U32 macPort,
    IN GT_U32 interruptBit
)
{
    ...
    DECLARE_FUNC_NAME(snetSip6_30TimestampPortMacEgressQueueInterruptEvent);

    if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E)
    {
        causeRegAddr = MTI_PORT_EXT(devObjPtr, macPort).portInterruptCause;
        maskRegAddr = MTI_PORT_EXT(devObjPtr, macPort).portInterruptMask;
    }
    else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
    {
        devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,macPort,&portInfo);
        causeRegAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).portInterruptCause;
        maskRegAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).portInterruptMask;
    }
    else
    {
        causeRegAddr = SMAIN_NOT_VALID_CNS;
        maskRegAddr = SMAIN_NOT_VALID_CNS;
    }
...
)
{
    ...
    {
        /* Queue Full interrupt */
        __LOG(("MAC TSQ FIFO overflow indication interrupt MAC[%d]",macPort));
        snetSip6_30TimestampPortMacEgressQueueInterruptEvent(devObjPtr, macPort, 22);
    ...
    snetSip6_30TimestampPortMacEgressQueueInterruptEvent(devObjPtr, macPort, 21);
    ...
    if( queuePtr->isEmpty == GT_TRUE )
    {
        if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E)
        {
            regAddr = MTI_PORT_EXT(devObjPtr, macPort).tsqTimestamp;
        }
        else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
        {
            devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,macPort,&portInfo);
            regAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).tsqTimestamp;
        }
        else
        {
            regAddr = SMAIN_NOT_VALID_CNS;
        ...
        *regPtr = queueEntryPtr[0];
        ...
        if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E)
        {
            regAddr = MTI_PORT_EXT(devObjPtr, macPort).tsqSignature;
        }
        else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
        {
            devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,macPort,&portInfo);
            regAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).tsqSignature;
        }
        else
        {
            regAddr = SMAIN_NOT_VALID_CNS;
...
)
{
    ...
    GT_U32 mchSignature = 0;
    GT_U32 portPeerDelay = 0;
    GT_U32 JIRA_MTIWRAPPER_390 = 1;
    ENHANCED_PORT_INFO_STC portInfo;
    ...
    if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E)
    {
        regAddr = MTI_PORT_EXT(devObjPtr, macPort).portControl;
    }
    else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
    {
        devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,macPort,&portInfo);
        regAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).portControl;
    }
    else
    {
        regAddr = SMAIN_NOT_VALID_CNS;
    ...
    if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_USX_E)
    {
        regAddr = MTI_PORT_EXT(devObjPtr, macPort).portPeerDelay;
    }
    else if (devObjPtr->portsArr[macPort].state == SKERNEL_PORT_STATE_MTI_CPU_E)
    {
        devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,macPort,&portInfo);
        regAddr = MTI_CPU_PORT_EXT(devObjPtr, portInfo.simplePortInfo.unitIndex).portPeerDelay;
    }
    else
    {
        regAddr = SMAIN_NOT_VALID_CNS;
    ...
    regPtr = smemMemGet(devObjPtr, regAddr);
    /* port peer delay, represented as an unsigned 30 bit nanosecond value; range 0 to 10^9-1. */
    portPeerDelay = SMEM_U32_GET_FIELD(regPtr[0], 0, 30);
    ...
    if(JIRA_MTIWRAPPER_390 && portPeerDelay)
    {
        /* JIRA : MTIWRAPPER-390 : peer_delay update is used only for 1step, not for 2step.*/
        __LOG(("WARNING : (JIRA : MTIWRAPPER-390) : port peer delay [%d] (nano) ,per mac port [%d] but ignored on capture (2 steps) \n",portPeerDelay,macPort));
    }
    else
    /* Timestamp; Entry Timestamp. {2b Sec, 30b ns} per the MAC TAI (TAI4). The timestamp shall also include the <peer delay> as configured in the MAC */
    if (portPeerDelay) {
        tai4TimeCounter.nanoSecondTimer += portPeerDelay;
        __LOG(("port peer delay [%d] (nano) per mac port [%d]",portPeerDelay,macPort));
        __LOG(("egressTimestamp.nanoSecondTimer was update [%d]\n",tai4TimeCounter.nanoSecondTimer));
    }

    /* support wraparound of the nano seconds */
    while(tai4TimeCounter.nanoSecondTimer >= 1000000000)
    {
        tai4TimeCounter.nanoSecondTimer -= 1000000000;
        tai4TimeCounter.secondTimer.l[0] ++;
    }

    snetFieldValueSet(timestampQueueEntry, 0, 30, tai4TimeCounter.nanoSecondTimer);
    snetFieldValueSet(timestampQueueEntry, 30, 2, tai4TimeCounter.secondTimer.l[0]);

    /* {3'b0,Marvell_PCH<QueueSelect>,Marvell_PCH<QueueEntryId<[9:0]>} */
    mchSignature = (descrPtr->timestampActionInfo.egressInfo.egressQueueNum & 1) << 10 | (descrPtr->timestampActionInfo.egressInfo.egressQueueEntryId & 0x3FF);

    /* TSQ Signature Value */
    snetFieldValueSet(timestampQueueEntry, 32, 14, SMEM_U32_GET_FIELD(mchSignature, 0, 14));

    /* TSQ Valid */
    snetFieldValueSet(timestampQueueEntry, 48, 1, 1);

    __LOG(("Port Mac Egress timestamp queue"));
    snetSip6_30TimestampPortMacEgressQueueInsertEntry(devObjPtr,
                                                             macPort,
                                                             timestampQueueEntry);
}
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
    {
        ...
        {
            __LOG(("SIP6: Egress TAI number uses ingress one : ptpEgressTaiSel [%d] (on configuration)\n",
                descrPtr->ptpTaiSelect));
        ...
        else
        {
            ...
            __LOG(("Egress TAI number selected by the egress port[%d] to : ptpEgressTaiSel [%d] \n",
                egressPort,descrPtr->ptpEgressTaiSel));
        }
    }
    else
    {
        ...
        {
            __LOG(("Egress TAI number uses ingress one : ptpEgressTaiSel [%d] \n",
                descrPtr->ptpTaiSelect));
            descrPtr->ptpEgressTaiSel = descrPtr->ptpTaiSelect;
        }
    }
    ...
    snetLion3PtpTodGetTimeCounter(descrPtr->egressPhysicalPortInfo.txDmaDevObjPtr,
                                  ...
                                  &descrPtr->timestamp[descrPtr->ptpEgressTaiSel][SMAIN_DIRECTION_EGRESS_E]);
...
)
{
    ...
    {
        __LOG(("No Timestamp Action performed \n"));
        descrPtr->timestampActionInfo.action = SKERNEL_TIMESTAMP_ACTION_NONE_E;
    }

    __LOG_PARAM(descrPtr->timestampActionInfo.action);
...
)
{
    ...
    /* Do timestamp action */
    snetlion3PtpTimestampingAction(devObjPtr, descrPtr, egressPort, GT_FALSE);
...
)
{
    ...
    GT_U32 regAddr;
    GT_U32 regValue;
    GT_U32 selectorEntryIndex;
    GT_U32 *selectorEntyPtr;
    GT_U32 queueIdBase;
    GT_U32 queueIdAdd;
    GT_U32 cmdIndex;
    GT_U32 tc2tcIndex;
    ...
    regAddr = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TMQMAP.TMQMAPGlobalConfig;
    smemRegGet(devObjPtr, regAddr, &regValue);
    if (regValue & (1 << 2))/*QueueSelectorAccessMode*/
    {
        /*TrgDeviceMapIndex[7:0]*/
        regAddr = SMEM_BOBCAT2_TM_QMAP_TRG_DEV_TO_INDEX_TBL_MEM(
            devObjPtr, (descrPtr->trgDev & 0x3FF));
        smemRegGet(devObjPtr, regAddr, &selectorEntryIndex);
        selectorEntryIndex &= 0xFF;
    }
    else
    {
        /* LocalDevTrgPhyPort[7:0] */
        selectorEntryIndex = (egressPort & 0xFF);
    }
    ...
    {
        regAddr =
            SMEM_BOBCAT2_TM_QMAP_CPU_CODE_TO_TC_TBL_MEM(devObjPtr, descrPtr->cpuCode);
        smemRegGet(devObjPtr, regAddr, &regValue);
        descrPtr->trafficManagerTc = regValue & 0x0F;
    }
    else
    {
        tc2tcIndex =
            (cmdIndex << 1) |
            ((descrPtr->macDaType == SKERNEL_UNICAST_MAC_E) ? 1 : 0);
        regAddr =
            SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TMQMAP.TC2TCMap8To16[tc2tcIndex];
        smemRegGet(devObjPtr, regAddr, &regValue);
        descrPtr->trafficManagerTc =
            SMEM_U32_GET_FIELD(regValue, (4 * descrPtr->tc), 4);
    }

    regAddr = SMEM_BOBCAT2_TM_QMAP_QUEUE_SELECTOR_TBL_MEM(
        devObjPtr, selectorEntryIndex);
    selectorEntyPtr = smemMemGet(devObjPtr, regAddr);

    queueIdBase = SMEM_U32_GET_FIELD(selectorEntyPtr[0], 0, 14);
    queueIdAdd  = 0;
    for (ii = 0; (ii < 14); ii++)
    {
        selType = snetFieldValueGet(
            selectorEntyPtr, (14 + (4 * ii))/*startBit*/, 4/*numBits*/);
        bitSel = snetFieldValueGet(
            selectorEntyPtr, (70 + (4 * ii))/*startBit*/, 4/*numBits*/);
        switch (selType)
        {
            /*0x0 = Zero; Set to 0 */
            case 0: bitSource = 0; break;
            /*0x1 = EPCL-Q-ID  */
            case 1: bitSource = 0; break; /*yet not supported*/
            /*0x2 = Flow-ID */
            case 2: bitSource = descrPtr->flowId; break;
            /*0x3 = LocalDevTrgPhyPort */
            case 3: bitSource = egressPort; break;
            /*0x4 = TM-TC */
            case 4: bitSource = descrPtr->trafficManagerTc; break;
            /*0x5 = Packet-Hash    */
            case 5: bitSource = descrPtr->pktHash; break;
            /*0x6 = TargetDevice;  */
            case 6: bitSource = descrPtr->trgDev; break;
            /*0x7 = TrgPhyPort;    */
            case 7: bitSource = descrPtr->eArchExtInfo.trgPhyPort; break;
            /*0x8 = Trg-ePort   */
            case 8: bitSource = descrPtr->trgEPort; break;
            /*0x9 = One; Set to 1; ;                    */
            case 9: bitSource = 0xFFFFFFFF; break;
            default: bitSource = 0; break;
        }
        queueIdAdd |= (((bitSource >> bitSel) & 1) << ii);
    }

    descrPtr->tmQueueId = queueIdBase + queueIdAdd;
...
)
{
    ...
    /* 3 bits cos per tmTc */
    smemRegFldGet(devObjPtr, regAddr,(descrPtr->trafficManagerTc % 8)*3,3,&cos);

    /* save the egress port for later use (after packet returns from TM unit) */
    descrPtr->trafficManagerCos   = cos;
    descrPtr->trafficManagerColor = descrPtr->dp;

    __LOG_PARAM(dropCommand);

    __LOG(("Warning: Egress port is [%d] as WM not supports TM logic \n",
        egressPort));
    ...
    snetCht3CncCount(
        ...
        (GT_UINTPTR)((dropCommand == GT_FALSE) ? 0 : 1) /*userDefined*/);

    /* descrPtr->trafficManagerEnabled = 0; We have no remote TM simulation */
...
)
{
    ...
    if(modifyUp && devObjPtr->errata.upChangesInEgressMirroredPackets)
    {
        /* the 'bug' in HW that we simulate here :
           is that the original egress descriptor modified the up0 (from up0 to up0') and then this
            up0' goes also to the tx mirrored copy , that is also doing 'remap' from up0' to up0''
            so the egress from the analyzer holding up0'' instead of up0'
        */

        /* EREP-43 : UP changes in Egress Mirrored packets */
        __LOG(("update the UP in the descriptor from [%d] to [%d] \n",
            descrPtr->up,up));
...
)
{
    ...
    for(portInx = 0 ; portInx < SKERNEL_DEV_EGRESS_MAX_PORT_CNS(devObjPtr); portInx++)
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            descrPtr->txqDestPorts[physicalPort] = SMEM_SIP6_EGRESS_EGF_QAG_TARGET_PORT_MAPPER_FIELD_GET(
                devObjPtr, regPtr, physicalPort,
...
)
{
    ...
    if(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr != NULL)
    {
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 45 : (startBitAfterEvid+12+3+3);

        /*Remove Vlan Tag 1 If Rx Without Tag 1*/
        value = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 1);
    ...
    if(descrPtr->vid1 != 0 && value <= 2)
    {
        __LOG(("not remove tag 1 when descrPtr->vid1 != 0 \n"));
        return tagState;
    }
    ...
    if(value == 3 && SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
    {
        ...
        {
            __LOG(("global : remove tag1 because <Src ID Bit Add Tag1> = [%d] >= 12 (value of the bit considered ZERO) \n",
                value));
        }
    }
...
)
{
    ...
    startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 48 : 46;
...
)
{
    ...
    */
    ...
    if(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr != NULL)
    {
        startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 7 : 4;

        /*Eport Tag State Mode*/
        if(snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 1))
        {
            startBit = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 8 : 5;
            tagState = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, startBit , 3);
...
)
{
    ...
    if(GT_FALSE == SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        ...
        __LOG_PARAM_WITH_NAME("Source_Physical_Port_TC_Profile :",uSrcPortTcProfile);
...
*/
    ...
    {
        uTrgPortTcProfile = descrPtr->trgPortTcProfile;/* coming from the EQ */

        /* sip7 : tc_dp_tbl_addr[9:0] = {TC[2:0],cast[0],tc_profile[5:0]} */
        tcDpIndex =  uNwTc << 7 |
                mCast <<6|
                (uTrgPortTcProfile&0x3F);
    }
    ...
    {   /*From IAS :
        ...
        Indicates that the target queue is a preemption queue.
        Preemptive queues are assigned a dedicated channel (port).
        In such a case the SW should assign an even local_dev_trg_phy_port (bit 0 is 0) and the QAG sets it per this bit
        (i.e. INC +1 if preemption)
        */
        descrPtr->sip7TrgPreempt = snetFieldValueGet(memPtr, 14, 1);
         __LOG_PARAM(descrPtr->sip7TrgPreempt);
    }
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        /*
                From IAS
                The following tables are removed from QAG as they’re no longer needed
/Cider/EBU-IP/SIP/EGF_IP/SIP 7.0 (Seahawk1)/EGF_IP {Current}/EGF_IP_qag/Tables/EGF qag
Common/VLAN Q Offset Mapping Table
        */
        ...
        if(SMEM_CHT_IS_SIP6_20_GET(devObjPtr))
        {
         snetSip6QueueRangeCheck(devObjPtr,descrPtr,egressPort,descrPtr->egressPhysicalPortInfo.sip6_queue_group_offset);
        }
    }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        /* check for QAG evlan/vid1 reassignment , to apply only before going to the
           HA unit , and not apply on the logic in the QAG/SHT tables */
    ...
    while (portInx < maxPortNum)
    {
        if ((destPorts == NULL) || (destPorts && destPorts[portInx]) )
        {
            if(isFirst == 0)
            {
                ...
                newDescPtr->rxDmaDebugCpTpRxCntDone = descrPtr->rxDmaDebugCpTpRxCntDone;
                *descrPtr = *newDescPtr;
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr) && prevCmd >= SKERNEL_EXT_PKT_CMD_TO_TRG_SNIFFER_E/*5*/ &&
       currCmd <= SKERNEL_EXT_PKT_CMD_SOFT_DROP_E/*5*/)
    {
        /* keep the previous command unless 'HARD DROP' */
        descrPtr->packetCmd = (currCmd == SKERNEL_EXT_PKT_CMD_TRAP_TO_CPU_E) ? SKERNEL_EXT_PKT_CMD_TRAP_TO_CPU_E :
                              (currCmd == SKERNEL_EXT_PKT_CMD_HARD_DROP_E) ? SKERNEL_EXT_PKT_CMD_HARD_DROP_E :
                               prevCmd;
    }
    else
    {
    ...
    if(prevCmd != descrPtr->packetCmd)
    {
        /* the egress pipe actually changed the command */
        descrPtr->packetCmdAssignedByEgress = 1;
    }
