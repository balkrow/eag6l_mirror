...
)
{
    ...
    /* number of TCAM Client Groups                             */
    /* Falcon - 4, AC5P - 5, AC5X - 3, Harrier - 3, Ironman - 1 */
    numOfTcamClientGroups = devObjPtr->numofTcamClients;
    ...
    /* get groupId */
    for(group = 0; group < numOfTcamClientGroups; group++)
    {
        ...
        else
        {
            ...
            smemRegFldGet(devObjPtr, regAddr,
                          portGroupId*numOfTcamClientGroups + hwClientId,
                          1, &isClientEnabled);
        }
...
/**
* @internal snetLion3TcamPrintToLogKeyChunksArray function
* @endinternal
*
* @brief   sip5 function that prints to LOG key chunks (84 bits each chunk)
*
* @param[in] devObjPtr                - (pointer to) the device object
* @param[in] keyArrayPtr              - array of keys bytes
* @param[in] keyChunksNum             - number of 10-byt chunks in the key
*
* @param[out] chunksArrayPtr           - array of key chunks
*                                      COMMENTS:
*/
static GT_VOID snetLion3TcamPrintToLogKeyChunksArray
(
    IN  SKERNEL_DEVICE_OBJECT    *devObjPtr,
    IN  GT_U32                   *keyArrayPtr,
    IN  GT_U32                   keyChunksNum
)
{
    GT_U32  chunkNum;
    GT_U32  keyWordsNum;
    ...
    __LOG_NO_LOCATION_META_DATA__WITH_SCIB_LOCK(
        ("dump key size [%d] bytes from client: \n",
        (keyChunksNum * 10)));

    /* loop of the number of chunks that need to build*/
    for(chunkNum = 0; chunkNum < keyChunksNum; chunkNum++)
    {
        /* first 64 bits */
        for(keyWordsNum = 0;
            keyWordsNum < SIP5_TCAM_NUM_OF_WORDS_IN_KEY_CHUNK_CNS - 1;/*words0,1*/
...
)
{
    switch(keyFormat)
    {
        ...
        case CHT_PCL_KEY_100B_E: /* SIP7 EPCL superkey */
            *sip5KeySizePtr = SIP5_TCAM_KEY_SIZE_100B_E;
            break;
        case CHT_PCL_KEY_120B_E: /* SIP7 IPCL and MPCL superkey */
            *sip5KeySizePtr = SIP5_TCAM_KEY_SIZE_120B_E;
            break;
...
* @param[in] keySize                  - size of the key (superkey) - for SIP7 up to 120 bytes (i.e. 12 80-bit chunks)
* @retval                             - number of hits found
*/
...
)
{
    ...
    GT_U32      regVal;
    GT_U32      subKeySize;
    GT_U32      subKey0_3Size;
    GT_U32      subKeySharedSize;
    GT_U32      muxTableLine;
    ...
    /* init variables for not-found case as default values */
    numOfHits = 0;
    ...
    if (SMEM_CHT_IS_SIP7_GET(devObjPtr) && (tcamProfileId == 0))
    {
        switch (keySize)
        {
            case SIP5_TCAM_KEY_SIZE_100B_E:
            case SIP5_TCAM_KEY_SIZE_120B_E:
                __LOG(("SIP7 TCAM tcamProfileId == 0, keySize > 80 nothing to search\n"));
                return 0;
            default: break;
        }
    }
    ...
    else if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        snetLion3TcamPrintToLogKeyChunksArray(devObjPtr, keyArrayPtr, keySize);
        if (groupId > 4)
        {
            __LOG(("groupId [%d]: more than 4 not supported by simulation \n", groupId));
            goto exitCleanly_lbl;
        }
        for (ii = 0; (ii < SIP5_TCAM_MAX_NUM_OF_HITS_CNS); ii++)
        {
            ...
            /* byte pairs 0-29 per parallel sublookup */
            regAddr = smemTableEntryFormulaAddressGet(
                &(devObjPtr->tablesInfo.tcamGroups0_4ProfileSubkeySizeAndMux),
                SMAIN_NOT_VALID_CNS /*instanceIndex*/,
                groupId, (tcamProfileId - 1), ii, 0, 0, 0);
            smemRegGet(devObjPtr, regAddr, &regVal);
            subKeySize       = ((regVal & 0x7) + SIP5_TCAM_KEY_SIZE_10B_E);
            subKey0_3Size    = (subKeySize > 6) ? 6 : subKeySize;
            subKeySharedSize = (subKeySize > 6) ? (subKeySize - 6) : 0;
            ...
            if (muxTableLine == 0)
            {
                ...
                numOfWords = ((subKey0_3Size / 2) * 5) + ((subKey0_3Size % 2) * 3);
                for (kk = 0; (kk < numOfWords); kk++)
                {
            ...
            else
            {
                ...
                for (jj = 0; (jj < subKey0_3Size); jj++)
                {
                    regAddr = smemTableEntryFormulaAddressGet(
                        &(devObjPtr->tablesInfo.tcamGroups0_5SubkeyMux2byteUnits),
                        SMAIN_NOT_VALID_CNS /*instanceIndex*/,
                        groupId, (muxTableLine - 1), (muxTableOffset + jj), 0, 0, 0);
                    smemRegGet(devObjPtr, regAddr, &regVal);
                    for (kk = 0; (kk < 5); kk++)
                    {
            ...
            if (subKeySharedSize > 0)
            {
                /* shared bytes 60-79 of all 4 subkeys */
                regAddr = smemTableEntryFormulaAddressGet(
                    &(devObjPtr->tablesInfo.tcamGroups0_5SubkeySharedMux),
                    SMAIN_NOT_VALID_CNS /*instanceIndex*/,
                    groupId, (tcamProfileId - 1), 0, 0, 0, 0);
                smemRegGet(devObjPtr, regAddr, &regVal);
                muxTableLine    = regVal & 0x3F; /* index of mux table line plus one */
                muxTableOffset  = (regVal >> 6) & 0x7;
                if (muxTableLine == 0)
                {
                    /* if muxTableLine is 0 no muxing        */
                    numOfWords = ((subKeySharedSize / 2) * 5) + ((subKeySharedSize % 2) * 3);
                    for (kk = 0; (kk < numOfWords); kk++)
                    {
                        workU32keyArray[15 + kk] = keyArrayPtr[15 + kk];
                    }
                }
                else
                {
                    for (jj = 0; (jj < subKeySharedSize); jj++)
                    {
                        regAddr = smemTableEntryFormulaAddressGet(
                            &(devObjPtr->tablesInfo.tcamGroups0_5SubkeyMux2byteUnits),
                            SMAIN_NOT_VALID_CNS /*instanceIndex*/,
    ...
    {
        /* convert master key 84-bit chunks to be printed in log only */
        __LOG(("Master Key in chunks:\n"));
        snetLion3TcamPrepareKeyChunksArray(devObjPtr, keyArrayPtr, keySize, keyChunksArr[0]);
