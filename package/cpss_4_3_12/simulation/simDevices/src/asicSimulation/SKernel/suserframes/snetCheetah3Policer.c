...
static GT_VOID snetCht3PolicerCountersUpdate(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN SKERNEL_FRAME_CHEETAH_DESCR_STC * descrPtr,
    IN SKERNEL_EXT_PACKET_CMD_ENT        inPacketCmd,
...
)
{
    ...
    SKERNEL_EXT_PACKET_CMD_ENT           inPacketCmd; /* packet command that PLR unit get from previous unit */
         ...
         descrPtr->policerCycle++)
    {
        ...
        /* store input packet command */
        inPacketCmd = descrPtr->packetCmd;
        ...
        snetCht3PolicerCountersUpdate(devObjPtr, descrPtr, inPacketCmd, policerMeterEntryPtr,
...
)
{
    ...
    *bytesCountPtr = bytesCount = 0;
    ...
    fldValue = SMEM_U32_GET_FIELD(*policerCtrlRegPtr, SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 7 : 16, 1);
    ...
    /*if (fldValue == SNET_CHT3_LAYER2_PCKT_SIZE_E)*/
    {
        ...
        fldValue = SMEM_U32_GET_FIELD(*policerCtrlRegPtr, SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 8 : 17, 1);
        if (fldValue)
        {
        ...
        fldValue = SMEM_U32_GET_FIELD(*policerCtrlRegPtr, SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 11 : 23, 1);
...
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN SKERNEL_FRAME_CHEETAH_DESCR_STC * descrPtr,
    IN SKERNEL_EXT_PACKET_CMD_ENT        inPacketCmd, /* packet command that PLR unit get from previous unit */
    IN GT_U32 * policerMeterEntryPtr,
    IN SKERNEL_CONFORMANCE_LEVEL_ENT cl,
    OUT SNET_CHT3_POLICER_MNG_COUNT_STC * mngCountDataPtr
)
{
    ...
    {
        /* NO counter set */
        __LOG(("sip7 : no support for legacy counter SET \n"));
        return;
    }
...
)
{
    ...
    if (fldValue == SNET_CHT_POLICER_CMD_DROP_E)
    {
        ...
        smemRegFldGet(devObjPtr,
                      SMEM_CHT_POLICER_GLB_CONF_REG(devObjPtr, descrPtr->policerCycle),
                      SMEM_CHT_IS_SIP7_GET(devObjPtr)? 3 : 5, 1, &fldValue);
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        __LOG(("Sip7 : no support for management counters any more \n"));
        return;
    }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        __LOG(("sip7 : no PLR HQOS table (no hierarchical QoS) \n"));
        return;
    }
