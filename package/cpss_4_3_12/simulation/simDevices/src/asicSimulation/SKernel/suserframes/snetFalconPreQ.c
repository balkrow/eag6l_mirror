...
)
{
    ...
    {
        if (descrPtr->egressPacketType == SKERNEL_EGR_PACKET_BRG_UCAST_E ||
            descrPtr->egressPacketType == SKERNEL_EGR_PACKET_BRG_UNK_UCAST_E ||
            descrPtr->egressPacketType == SKERNEL_EGR_PACKET_ROUT_UCAST_E)
        {
            ...
            {
                /* update broadcast packet counter */
        ...
        else if (descrPtr->egressPacketType == SKERNEL_EGR_PACKET_BRG_REG_MCAST_BCAST_E ||
                 descrPtr->egressPacketType == SKERNEL_EGR_PACKET_BRG_UNREG_MCAST_E ||
                 descrPtr->egressPacketType == SKERNEL_EGR_PACKET_ROUT_MCAST_E)
        {
            if (descrPtr->macDaType == SKERNEL_MULTICAST_MAC_E || descrPtr->macDaType == SKERNEL_UNICAST_MAC_E)
            {
                /* update multicast packet counter */
                __LOG(("update multicast packet counter"));
                counterRegAddr0 =
...
)
{
    ...
    /* Get analyzer index  */
    txSniffEn = snetFieldValueGet(memPtr, 3, fieldLength);
...
)
{
    ...
    bitmap = SMEM_U32_GET_FIELD(bitmap,startBit,numOfBits);

    /* loop on (up to) 8 4-bit in 32-bit bitmap        */
    for (i = 0; (i < 8) && bitmap; i++, bitmap >>= 4)
    {
        numOfBitsSetTo_1 += numBits[bitmap & 0x0F];
    }

    return numOfBitsSetTo_1;
}
    ...
    *srfNumberPtr = 0;/* dummy */
    ...
    if(preqSrf_IAS_1_30 && tempValue == 1/*multicast*/)
    {
        ...
        */
        smemRegGet(devObjPtr,
            ...
            &tempValue);
        fieldOffset = SMEM_CHT_IS_SIP7_GET(devObjPtr)? 9 : 6;
        portIndex = SMEM_U32_GET_FIELD(tempValue,fieldOffset,
        ...
        SMEM_SIP6_30_PREQ_SRF_MAPPING_TABLE_FIELDS_STREAM_TYPE_E))
    {
        ...
        for(ii = 0 ; ii < (portIndex >> 5); ii++)
        {
            ...
            totalBits += bitcount(tempValue,0,32);
        ...
        totalBits += bitcount(tempValue,0,portIndex);
    }
    else
    {
...
* @param[in] daemonMemPtr             - pointer to the daemon entry
*
...
)
{
       ...
       (Src_ID_Indicates_Tag1 == 0 || (0 == SMEM_U32_GET_FIELD(descrPtr->sstId,Src_ID_Add_Tag1_location,1))))
    {
        ...
        return SRF_STATE_DONE_TAG_LESS_E;
    }
    ...
    if(TakeAny)
    {
        ...
        /*<Enable Latent Error Detection Reset>*/
        smemRegFldGet(devObjPtr,
        ...
        /*If Global<Enable Latent Error Detection Reset>==1 &&<LE ResetTime> > 0*/
        if(regValue && tempValue > 0)
        {
            ...
            {
                /*If diff >= 0 then <LE BaseDifference>= diff;*/
                diff = PassedPacketsCounter - DiscardedPacketsCounter;
            }
            else
            {
        ...
        return SRF_STATE_DONE_TAKE_ANY_E;
    }

    return SRF_STATE_DONE_NONE_E;
}
...
)
{
    ...
    GT_32  currBit_32 = (GT_32)currBit;
    ...
    */
    BitNum = (currBit_32 + delta) % 128;
    if((currBit_32 + delta) < 0)  /*If a < 0 then (a div 128) <= a/128*/
    {
        y      = ((currBit_32 + delta) / 128) - 1;
    }
    else
    {
        y      = (currBit_32 + delta) / 128;
    }
...
)
{
    ...
    /* HistoryBuffers[BufferNum,*] should be read as 0 */
    {
        ...
        bitCount = bitcount(~histMemPtr[0],0,32) +
                   bitcount(~histMemPtr[1],0,32) +
                   bitcount(~histMemPtr[2],0,32) +
                   bitcount(~histMemPtr[3],0,32) ;
    ...
    /* HistoryBuffers[BufferNum,0:<CurBit>-1] should be read as 0 */
    {
        ...
        for(ii = 0 ; ii < 3 ; ii++)
        {
            ...
            bitCount += bitcount(~currHistMemPtr[ii],0,32);
        }

        bitCount += bitcount(~currHistMemPtr[ii],0,currBit%32);
    ...
    */
    ...
    for(ii = 0 ; ii < 3 ; ii++)
    {
        ...
        if((currBit / 32) == ii && (0 != (currBit % 32)))
        {
            ...
            bitCount += bitcount(~currHistMemPtr[ii],currBit%32,31);
        }
        else
        {
            bitCount += bitcount(~currHistMemPtr[ii],0,32);
    ...
    if((currBit / 32) != (x / 32))
    {
        ...
        bitCount += bitcount(~currHistMemPtr[ii],0,x%32);
    }
    else
    {
        /* in the same word */
        bitCount += bitcount(~currHistMemPtr[ii],currBit%32,x-currBit);
    }
    /*<ClearedZeroHistoryBitsCounter>+= y;*/
    incrementSrfCounter(devObjPtr,srfNumber,countersMemPtr,
        SMEM_SIP6_30_PREQ_SRF_COUNTERS_TABLE_FIELDS_CLEARED_ZERO_HIST_CNT_E,
...
)
{
    ...
    countersMemPtr = smemMemGet(devObjPtr,
        SMEM_SIP6_30_PREQ_SRC_COUNTERS_TBL_MEM(devObjPtr,srfNumber));

    __LOG(("EREP : Access to SRF daemon table at index  = srfNumber[0x%8.8x] \n",
        srfNumber));

    daemonMemPtr = smemMemGet(devObjPtr,
        SMEM_SIP6_30_PREQ_DAEMONS_TBL_MEM(devObjPtr,srfNumber));
    ...
    if(tempValue == 0)
    {
        ...
        state = snet6_30PreqSrfCommonAlgorithm(devObjPtr,descrPtr,srfNumber,SeqNr,memPtr,countersMemPtr,daemonMemPtr);
    ...
    ActualNumberOfBuffers = 1 + SMEM_SIP6_30_PREQ_SRF_CONFIG_FIELD_GET(devObjPtr,memPtr,
        srfNumber,
