...
/**
* @internal erepPrepareCopyDescToEq function
* @endinternal
*
* @brief   the function used as preperations for mirror to tx analyzer from PREQ and
*          for mirror/trap to CPU from PREQ
*
* @param[in] devObjPtr                - pointer to device object.
* @param[in] orig_descrPtr            - pointer to the original frame's descriptor.
* @param[in] dup_descrPtr             - pointer to the duplicated frame's descriptor.
* @param[in] orig_egressPort          - orig target port.
*
* COMMENTS:
*
*/
static GT_VOID  erepPrepareCopyDescToEq
(
    IN SKERNEL_DEVICE_OBJECT * devObjPtr,
    IN SKERNEL_FRAME_CHEETAH_DESCR_STC * orig_descrPtr,
    INOUT SKERNEL_FRAME_CHEETAH_DESCR_STC * dup_descrPtr,
    IN GT_U32               orig_egressPort
)
{
    /* descrPtr : needed only for the SIM_LOG_PACKET_DESCR_SAVE , SIM_LOG_PACKET_DESCR_COMPARE */
    SKERNEL_FRAME_CHEETAH_DESCR_STC * descrPtr = dup_descrPtr;

    SIM_LOG_PACKET_DESCR_SAVE
    /* mirror copy should not derive Latency monitoring parameters */
    dup_descrPtr->lmuEn = 0;

    dup_descrPtr->trgTagged = orig_descrPtr->egressPhysicalPortInfo.egressVlanTagMode ;

    dup_descrPtr->egressTrgPort = orig_egressPort;/* this is global port */

    if(dup_descrPtr->useVidx ||
       dup_descrPtr->trgDev != orig_descrPtr->ownDev)
    {
        /* save the target phy port of the orig packet used by HA for access tables 'phy port 1' ' eport 1' */
        dup_descrPtr->eArchExtInfo.srcTrgEPort = orig_egressPort;
    }
    else
    {
        /* save the target eport of the orig packet used by HA for access tables 'phy port 1' ' eport 1' */
        dup_descrPtr->eArchExtInfo.srcTrgEPort = orig_descrPtr->trgEPort;
    }

    /* save the target eport of the orig packet before modification in EQ unit */
    dup_descrPtr->eArchExtInfo.eqInTrgEPort = orig_descrPtr->trgEPort;

    dup_descrPtr->srcTrgDev = dup_descrPtr->useVidx ? orig_descrPtr->ownDev : dup_descrPtr->trgDev;
    dup_descrPtr->srcTrgPhysicalPort       = dup_descrPtr->egressTrgPort;

    dup_descrPtr->srcTrg = 1;
    dup_descrPtr->srcTaggedTrgTagged = dup_descrPtr->trgTagged;
    dup_descrPtr->packetCmdAssignedByEgress = 0;

    dup_descrPtr->txqToEq = 1;
    dup_descrPtr->origDescrPtr = orig_descrPtr;

    /* since TXQ send back to the EQ we need to set descriptor 'local/global src ports' accordingly */
    {
        GT_U32  localSrcPort = dup_descrPtr->localPortGroupPortAsGlobalDevicePort;/*value of 'local port' (at this stage in TXQ) */
        GT_U32  globalSrcPort = dup_descrPtr->localDevSrcPort;/* the global port num (at this stage in TXQ) */

        /* convert ingress local src port to src egress global port */
        dup_descrPtr->localDevSrcPort   = localSrcPort;

        if(dup_descrPtr->origIsTrunk == 0 && dup_descrPtr->marvellTagged == 0)
        {
            dup_descrPtr->origSrcEPortOrTrnk                = localSrcPort;
            dup_descrPtr->localPortTrunkAsGlobalPortTrunk   =  globalSrcPort;
        }

        dup_descrPtr->localPortGroupPortAsGlobalDevicePort = globalSrcPort;/* global port(not local) */
    }

    dup_descrPtr->origIsTrunk = 0;/* the target port is never 'trunk' */


    /* l3StartOffsetPtr will be updated in chtHaUnit */
    dup_descrPtr->haToEpclInfo.l3StartOffsetPtr = NULL;

    if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr))
    {
        /* Copy Original packet queue info to the duplicated descriptor */
        dup_descrPtr->origQueueOffset       = orig_descrPtr->egressPhysicalPortInfo.sip6_queue_group_offset;
        dup_descrPtr->origQueueGroupIndex   = orig_descrPtr->egressPhysicalPortInfo.sip6_queue_group_index;
        dup_descrPtr->epclMetadataReady     = 0;  /* Need to rebuild the epcl metadata */
        dup_descrPtr->localDevPacketSource  = SKERNEL_SRC_TX_REPLICATED_TYPE_E;
    }

    /* used for EPCL in : snetChtHaEgressEPortEntryGet :
        index2 = descrPtr->eArchExtInfo.toTargetSniffInfo.sniffTrgEPort;
    */
    dup_descrPtr->eArchExtInfo.toTargetSniffInfo.sniffTrgEPort = dup_descrPtr->eArchExtInfo.srcTrgEPort;
    dup_descrPtr->egressTrgPort = orig_egressPort;

    dup_descrPtr->forceNewDsaToCpu = 0; /* restore default value */

    /* CPSS-17127: CNC should use ingress byteCount for CNC counting instead of egressByteCount.
     * To retain this, setting egressByteCount to 0.
     */
    dup_descrPtr->egressByteCount = 0;

    SIM_LOG_PACKET_DESCR_COMPARE("erepPrepareCopyDescToEq");
}
...
)
{
    ...
    /* prepare the txSniffDescPtr with info from descrPtr,egressPort */
    erepPrepareCopyDescToEq(descrPtr->ingressDevObjPtr,descrPtr,txSniffDescPtr,egressPort);

    __LOG(("The EREP unit send duplicated descriptor to the EQ (mirror to TX analyzer) \n"));
    /* call the EQ */
...
)
{
        ...
        (descrPtr->eArchExtInfo.egfShtEgressClass != SHT_PACKET_CLASS_3_E) )
    {
        ...
        fldVal = snetFieldValueGet(descrPtr->eArchExtInfo.egfQagEgressEPortTablePtr, 1 , fieldLength);
        if(fldVal)
        {
...
)
{
    ...
    profileBitSize = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 12 : 9;

    memAddr = SMEM_SIP6_EPCL_SRC_PHYSICAL_PORT_MAP_TBL_MEM(devObjPtr, localDevSrcPort);
    smemRegGet(devObjPtr, memAddr, &memValue);
    srcIdx = SMEM_U32_GET_FIELD(memValue, 0, 8);
    srcProfile = SMEM_U32_GET_FIELD(memValue, 8, profileBitSize);

    memAddr = SMEM_SIP6_EPCL_TRG_PHYSICAL_PORT_MAP_TBL_MEM(devObjPtr, localDevTrgPort);
    smemRegGet(devObjPtr, memAddr, &memValue);
    dstIdx = SMEM_U32_GET_FIELD(memValue, 0, 8);
    dstProfile = SMEM_U32_GET_FIELD(memValue, 8, profileBitSize);
    ...
    if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr))
    {
        ...
        if(profileMode == 1)
        {
            /* Priority Queue Latency Monitoring Profile. Entry depends upon qNum even/odd */
            entryIdx = descrPtr->egressPhysicalPortInfo.sip6_queue_group_offset;
            memAddr = SMEM_SIP6_10_EPCL_QUEUE_OFFSET_LATENCY_PROFILE_CONFIG_REG(devObjPtr, entryIdx >> 1);
            smemRegFldGet(devObjPtr, memAddr,
                          (entryIdx % 2) * profileBitSize /* bitOffset*/,
                          profileBitSize,
                          &queueProfile);
            ...
            {
                GT_U32 *memPtr;
                GT_U32  queueNum = descrPtr->egressPhysicalPortInfo.sip6_queue_group_offset & 7;

                entryIdx = descrPtr->egressPhysicalPortInfo.sip6_queue_group_index >> 3;
                memAddr = SMEM_SIP6_10_EPCL_QUEUE_GROUP_LATENCY_PROFILE_CONFIG_TBL_MEM(devObjPtr, entryIdx);
                memPtr = smemMemGet(devObjPtr, memAddr);
                portProfile = snetFieldValueGet(memPtr, queueNum * 12, 12);

            }
            else
            {
                entryIdx = descrPtr->egressPhysicalPortInfo.sip6_queue_group_index;
                memAddr = SMEM_SIP6_10_EPCL_QUEUE_GROUP_LATENCY_PROFILE_CONFIG_TBL_MEM(devObjPtr, entryIdx);
    ...
    if(SMEM_U32_GET_FIELD(memValue, entryIndex & 0x1F, 1) == 1)
    {
        ...
        {
            descrPtr->lmuProfile = (portProfile + queueProfile) & ((1 << profileBitSize) - 1);
            __LOG(("LMU-Trig mode=P profile=%u=p+q=%u+%u\n",
                   descrPtr->lmuProfile, portProfile, queueProfile));
        }
        else
        {
            descrPtr->lmuProfile = (srcProfile + dstProfile) & ((1 << profileBitSize) - 1);
            __LOG(("LMU-Trig mode=Q profile=%u=src+dst=%u+%u\n",
                   descrPtr->lmuProfile, srcProfile, dstProfile));
        }
    }
}
...
)
{
    ...
    /* profile2sampling reg entry is upon lmuProfile
     *   SIP7 12b = [11..3]b + [2..0]b = 512 * 8 = 4096
     *   else  9b =  [8..3]b + [2..0]b =  64 * 8 = 512
     *         regAddr_/          \_fieldOffset=(3b*N) in reg
     */
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        smemRegFldGet(devObjPtr,
            SMEM_SIP6_RXDMA_CHANNEL_GENERAL_CONFIG_REG(devObjPtr,rxPort),
            1,1,&pchEnabled);
    }
    else
    {
...
)
{
    ...
    GT_U32  saveByteCount;

    simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_TXDMA_E);

    saveByteCount = frameDataSize;
    if (frameDataSize > 16000) /* drop cut-through packets with not calculated size */
    {
        /* the primary packet size was 0x3FFF              */
        /* it was increased or/and decreased by TTI and HA */
        frameDataSize -= (0x3FFF - descrPtr->frameBuf->actualDataSize);

        __LOG(("WARNING : cut-through packet (egress port[%d]) not calculated size [%d] fixed to [%d]\n",
            txFifoPort, saveByteCount, frameDataSize));
    }
...
)
{
    ...
    SKERNEL_FRAME_CHEETAH_DESCR_STC * txToCpuDescPtr;/* trap/mirror descriptor */
    GT_U32  origPipeId;/* support multi-pipe device */

    simLogPacketFrameUnitSet(SIM_LOG_FRAME_UNIT_EREP_E);

    /* save the pipeId ... as the EQ may send the packet to other egress pipe ! */
        ...
        else
        {
            ...
            else
            {
                ...
                if (didReplication)
                {
                    ...
                    /* duplicate descriptor from the ingress core */
                    txToCpuDescPtr = snetChtEqDuplicateDescr(descrPtr->ingressDevObjPtr,descrPtr);

                    /* prepare the txToCpuDescPtr with info from descrPtr,egressPort */
                    erepPrepareCopyDescToEq(descrPtr->ingressDevObjPtr,descrPtr,txToCpuDescPtr,egressPort);

                    __LOG(("The EREP unit send duplicated descriptor to the EQ (TRAP/Mirror to CPU) \n"));
                    /* call the EQ (for the duplicated descriptor) */
                    snetChtEqToCpuFromEgress(devObjPtr, txToCpuDescPtr);
...
)
{
    ...
    if(orig_packetCmd != descrPtr->packetCmd)
    {
        switch(descrPtr->packetCmd)
        {
            ...
            case SKERNEL_EXT_PKT_CMD_TRAP_TO_CPU_E:/* was trapped just now */
                if(descrPtr->outGoingMtagCmd == SKERNEL_MTAG_CMD_TO_CPU_E)
                {
                    __LOG(("packet's re-trap does not work and original packet will go to egress port [%d] \n",
                        egressPort));
                }
                else
                {
    ...
    else
    {
        ...
        SIM_LOG_PACKET_DESCR_SAVE
        snetFalconTxqProcess(devObjPtr,descrPtr,egressPort);
