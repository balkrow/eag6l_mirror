...
static CHT_PCL_KEY_FIELDS_INFO_STC lionCrcHashKeyFields[SNET_LION_CRC_HASH_LAST_E]=
{
        ...
        /*same as IP fields (SNET_LION_CRC_HASH_IP_DIP_3_E to SNET_LION_CRC_HASH_IP_DIP_0_E)*/
        {56  ,87 , GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_3_TO_4_E "    }, /* MPLS Label4 31..20(12 bits) and MPLS Label3 19..0(20 bits) */
        {88  ,119, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_4_TO_6_E "    }, /* MPLS Label6 31..28(4 bits), MPLS Label5 27..8(20 bits) and MPLS Label4 7..0(8 bits) */
        {120 ,151, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_6_TO_7_E "    }, /* MPLS Label7 31..16(16 bits) and MPLS Label6 15..0(16) */
        {152 ,183, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_7_TO_9_E "    }, /* MPLS Label9 31..24(8 bits) MPLS Label8 23..4(20 bits) and MPLS Label7 3..0(4 bits) */

        {184 ,215, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_9_E "         }, /* Dummy bits 31..12(20 bits) MPLS Label9 11..0(12 bits) */
        {216 ,247, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_DUMMY_0_E "   }, /* Dummy bits 31..0(32 bits) */
        {248 ,279, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_DUMMY_1_E "   }, /* Dummy bits 31..0(32 bits) */
        {278 ,311, GT_FALSE, " SNET_LION3_CRC_HASH_MPLS_LABEL_DUMMY_2_E "   }  /* Dummy bits 31..0(32 bits) */
...
)
{
    ...
    GT_U32 hashIPv6UseFullAddrEnable,hashEVlanEnable,hashKeyMplsModeEnable = 0;/*global hash modes*/
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        /*Determines whether the 70B hash key includes the 10 MPLS labels instead of IP fields
          (IP DIP all 128 bits and IP SIP 12 Least Significant bits).*/
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            hashKeyMplsModeEnable = SMEM_U32_GET_FIELD(regValue, 15, 1);
        }
        ...
        __LOG_PARAM(hashKeyMplsModeEnable);
    ...
    /* When 10 MPLS label mode is enabled:
     * For MPLS label 3,
     * IP DIP[15] holds 8 LS bits
     * IP DIP[14] holds next 8 bits and
     * IP DIP[13] holds MS 4 bits. (as bits 0..3)
     * For MPLS Label 4,
     * IP DIP[13] holds 4 LS bits (as bits 4..7)
     * IP DIP[12] holds next 8 bits and
     * IP DIP[11] holds MS 8 bits.
     * For MPLS label 5,
     * IP DIP[10] holds 8 LS bits
     * IP DIP[9] holds next 8 bits and
     * IP DIP[8] holds MS 4 bits. (as bits 0..3)
     * For MPLS Label 6,
     * IP DIP[8] holds 4 LS bits(as bits 4..7)
     * IP DIP[7] holds next 8 bits and
     * IP DIP[6] holds MS 8 bits.
     * For MPLS label 7,
     * IP DIP[5] holds 8 LS bits
     * IP DIP[4] holds next 8 bits and
     * IP DIP[3] holds MS 4 bits. (as bits 0..3)
     * For MPLS Label 8,
     * IP DIP[3] holds 4 LS bits(as bits 4..7)
     * IP DIP[2] holds next 8 bits and
     * IP DIP[1] holds MS 8 bits.
     * For MPLS label 9,
     * IP DIP[0] holds 8 LS bits
     * IP SIP[15] holds next 8 bits and
     * IP SIP[14] holds MS 4 bits. (as bits 0..3)
     * IP SIP[14](half byte) and IP SIP[13]  .. IP SIP[0] will be set to 0.*/
    if((descrPtr->mpls) && (hashKeyMplsModeEnable == 1))
    {
        __LOG(("use IP DIP all 128 bits and IP SIP 12 LS bits to get 7 MPLS Labels (3 to 9) \n"));
        ...
        {
            /* IPv6 DIP */
            __LOG(("Hash Key MPLS Mode: DIP[%d] (ipv6) \n",
                i));
            snetLionCrcHashKeyFieldBuildByValueWithMask(devObjPtr, applyCRC32Mask, crcHashKeyMaskPtr,
                                                        crcHashKeyPtr,
                                                        descrPtr->dip[3-i],
                                                        SNET_LION3_CRC_HASH_MPLS_LABEL_3_TO_4_E + i, offset);
            /* IPv6 SIP */
            __LOG(("Hash Key MPLS Mode: SIP[%d] (ipv6) \n",
                i));
            snetLionCrcHashKeyFieldBuildByValueWithMask(devObjPtr, applyCRC32Mask, crcHashKeyMaskPtr,
                                                        crcHashKeyPtr,
                                                        descrPtr->sip[3-i],
                                                        SNET_LION3_CRC_HASH_MPLS_LABEL_9_E + i, offset);
        }
    }
    else if(descrPtr->isIp || descrPtr->isFcoe)
    {
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        {
            /* When Hash Key MPLS Mode is enabled, UDB0-11 are not used */
            if(hashKeyMplsModeEnable == 0)
            {
            ...
            if(symmetricIpv6)
            {
                ...
                for(i = 0; i < 12; i++)
                {
                    tmpFieldVal[i]   = (hashKeyMplsModeEnable == 0) ? tmpFieldVal[i] : 0;
    ...
    {
        __LOG(("use descrPtr->eVid instead of descrPtr->label[0] \n"));
        /* <eVLAN> */
        snetLionCrcHashKeyFieldBuildByValueWithMask(devObjPtr, applyCRC32Mask, crcHashKeyMaskPtr, crcHashKeyPtr,
                                                descrPtr->eVid,
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        indexingVid = SMEM_CHT_IS_SIP7_GET(devObjPtr) && (!descrPtr->basicMode) ? descrPtr->tag0Vid/* 12 bits value */ : bridgeVid;
        ...
        indexingVid = bridgeVid;/*no change in sip7 for 'STP index'*/
        if( 0 == descrPtr->basicMode )
        {
            /* <UseVid1ForSSGIndex> */
...
)
{
    ...
    GT_U32  fieldLength; /* Analyzer index field length */
    ...
    __LOG_PARAM(endPort);
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr) && descrPtr->basicMode == 0)
    {
        regAddress = SMEM_SIP7_EGF_EFT_EVLAN_TBL_MEM(devObjPtr, descrPtr->eVid);
        smemRegGet(devObjPtr, regAddress, &fldValue);
        descrPtr->tag0Vid = fldValue & 0xFFF;/* 12 bits */
        __LOG(("Sip7: EGF-EFT : map eVid[%d] to tag0 vid [%d] to be used for physical members of the vlan",
            descrPtr->eVid,descrPtr->tag0Vid));

        /* support translation in EFT that impact the SHT */
        vid = descrPtr->eVid;
        indexingVidForVlanMembers = descrPtr->tag0Vid;
    }
    else
    {
    ...
    if(SMEM_CHT_IS_SIP5_20_GET(devObjPtr))
    {
        /* Vlan entry is always valid . (valid bit was removed) */
    }
    else
    {
        /* VLAN validation */
    ...
    if(useFloodVidxEntry)
    {
        ...
        if(vidx != floodVidx)
        {
            ...
            __LOG(("NOTE: Flood in vlan[%d], with vidx [%d] was recognized , will use 'Flood eVIDX' [%d] in the VIDX table \n",
                indexingVidForVlanMembers,vidx,floodVidx));
            vidx = floodVidx;
        }
        else
        {
            __LOG(("Flood in vlan[%d], with vidx [%d] was recognized \n",
                indexingVidForVlanMembers,vidx));
        }
    }
    ...
    if(SMEM_CHT_IS_SIP6_10_GET(devObjPtr))
    {
        /* no modes support ! single port mode */
        /* endPort ... should come from SNET_CHT_EGR_TXQ_END_PORT_MAC(...) */
        vlanNumPortsToJump = SMAIN_NOT_VALID_CNS;/* will cause never to be matched*/
        stgNumPortsToJump  = SMAIN_NOT_VALID_CNS;/* will cause never to be matched*/
    }
    else
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
    ...
    if(simLogIsOpenFlag)
    {
        GT_U32  firstPort= 0 , lastPort = endPort < vlanNumPortsToJump ? endPort :vlanNumPortsToJump;

        lastPort--;

        log__memory_printBitmapArr(devObjPtr ,__FILE__ , functionNameString,"vlanEntryPtr" ,
            firstPort,lastPort,vlanEntryPtr);
        log__memory_printBitmapArr(devObjPtr ,__FILE__ , functionNameString,"mcastEntryPtr" ,
            firstPort,lastPort,mcastEntryPtr);
        log__memory_printBitmapArr(devObjPtr ,__FILE__ , functionNameString,"stgEntryPtr" ,
            firstPort,lastPort,stgEntryPtr);
    }
    ...
    for (port = startPort,localPort = 0; port < endPort; port++ , localPort++,nonVlanPortIndex++,vlanPortIndex++,stgPortIndex++)
    {
        if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
        {
            if(vlanPortIndex == vlanNumPortsToJump)
            {
                ...
                if(simLogIsOpenFlag)
                {
                    GT_U32  firstPort= port , lastPort = port+vlanNumPortsToJump;
            ...
            if(stgPortIndex == stgNumPortsToJump)
            {
                ...
                if(checkVidxGroup)
                {
                    ...
                    if(simLogIsOpenFlag)
                    {
                        GT_U32  firstPort= port , lastPort = port+nonVlanChunkSize;
        ...
        if(descrPtr->tableAccessMode.EGF_SHT_TableMode && (port == (endPort/2)))
        {
            ...
            if(simLogIsOpenFlag)
            {
                GT_U32  firstPort= port , lastPort = endPort;
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        if( SKERNEL_MTAG_CMD_TO_TRG_SNIFFER_E != descrPtr->outGoingMtagCmd)
        {
            ...
            fldValue = snetFieldValueGet(regValuePtr, 3 , fieldLength);
...
)
{
    ...
    GT_U32  designatedTableIndexMask = 0x3f;
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        ...
        if(multicastTrunkHashMode == 0)
        {
            GT_U32 designated_trunk_hash_first_bit,designated_trunk_hash_last_bit,designated_trunk_num_hash_bits,designated_trunk_block_size;


            __LOG(("sip7 : do linear scaled index into the MC Designated Table \n"));

            regAddress = SMEM_SIP7_EFT_GLOBAL_HASH_SELECTION_REG(devObjPtr);
            smemRegGet(devObjPtr, regAddress,&regValue);

            designated_trunk_block_size    = SMEM_U32_GET_FIELD(regValue, 15, 7);
            designated_trunk_num_hash_bits = SMEM_U32_GET_FIELD(regValue, 10, 5);
            designated_trunk_hash_first_bit= SMEM_U32_GET_FIELD(regValue,  0, 5);
            designated_trunk_hash_last_bit = SMEM_U32_GET_FIELD(regValue,  5, 5);

            __LOG_PARAM(designated_trunk_hash_first_bit);
            __LOG_PARAM(designated_trunk_hash_last_bit);
            __LOG_PARAM(designated_trunk_num_hash_bits);
            __LOG_PARAM(designated_trunk_block_size);

            if(designated_trunk_hash_last_bit >= designated_trunk_hash_first_bit)
            {
                GT_U64_BIT  tempHash;

                tempHash = SMEM_U32_GET_FIELD(descrPtr->pktHash,
                    designated_trunk_hash_first_bit ,
                    (designated_trunk_hash_last_bit - designated_trunk_hash_first_bit) + 1);
                tempHash = (GT_U64_BIT)(tempHash * (GT_U64_BIT)designated_trunk_block_size) >> designated_trunk_num_hash_bits;

                pktHash = tempHash;
                __LOG(("Use bits [%d]:[%d] from the packet hash [0x%x] to calculate index [0x%x] into the designated table \n",
                    designated_trunk_hash_last_bit,
                    designated_trunk_hash_first_bit,
                    descrPtr->pktHash,
                    pktHash));

                designatedTableIndexMask = 0x7F;/* table size is 128 */
            }
            else
            {
                __LOG(("WARNING: potential configuration ERROR : designated_trunk_hash_last_bit[%d] < designated_trunk_hash_first_bit[%d] (cause hash = [0]) \n",
                    designated_trunk_hash_last_bit,
                    designated_trunk_hash_first_bit));
                pktHash = 0;
            }
...
useIndex_lbl:
    entry &= designatedTableIndexMask;

    if (devObjPtr->supportSplitDesignatedTrunkTable)
    {
        designatedTableSize = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? 128 : 64;
        __LOG_PARAM(designatedTableSize);
        ...
        {
            __LOG(("Use 'dedicated' MC designated table (high [%d] entries) \n",designatedTableSize));
            entry += designatedTableSize;
        }
        else
        if(splitDesignatedTableEnable)
        {
            __LOG(("Use 'dedicated' UC designated table (low [%d] entries) \n",designatedTableSize));
        }
        else
        {
            __LOG(("Use 'shared' UC,MC designated table (legacy mode - [%d] entries) \n",designatedTableSize));
        }
    }
    else
    {
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        /* trim the result into 13 bits */
        entryIndex &= 0x1FFF;/*13 bits*/
    }
    else
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        /* In SIP7 devices, flowId is scaled from 16b to 19b */
        gtsEntryPtr->seqID = (gtsEntryPtr->seqID << 3) | (ptpMessageHeaderPtr[32] & 0x7);
    }
...
)
{
    ...
    else
    {
        ...
        else
        {
            /* PTP over IEEE 802.3 / Ethernet */
            __LOG(("PTP over IEEE 802.3 / Ethernet"));
            ptpMessageHeaderPtr = &descrPtr->l3StartOffsetPtr[0];
    ...
    {
        __LOG(("looking for V1Subdomain[0x%8.8x][0x%8.8x][0x%8.8x][0x%8.8x] to convert to 'ptp domain' (and TAI select) \n",
            gtsEntryPtr->V1Subdomain[3],
            gtsEntryPtr->V1Subdomain[2],
            gtsEntryPtr->V1Subdomain[1],
            gtsEntryPtr->V1Subdomain[0]
            ));
        descrPtr->ptpDomain_8bits = gtsEntryPtr->V1Subdomain[0] & 0xFF;
    }
    else /* gtsEntryPtr->ptpVersion == 2 */
    {
        __LOG(("looking for V2DomainNumber[0x%2.2x] to convert to 'ptp domain' (and TAI select) \n",
            gtsEntryPtr->V2DomainNumber));
        descrPtr->ptpDomain_8bits = gtsEntryPtr->V2DomainNumber;
    }
        ...
        if(gtsEntryPtr->ptpVersion == 1)
        {
            if( SMEM_U32_GET_FIELD(regData, ((domainN+1)*2),  2) == 1 )
            {
                ...
                if(V1SubdomainMatch == GT_TRUE)
                {
                    ...
                    __LOG(("PTP Domain Resolution found match for PTPv1 as domain [%d]",domainN));
                    pktDomain = domainN;
                    break;
                }
            }
        ...
        else /* gtsEntryPtr->ptpVersion == 2 */
        {
            if( SMEM_U32_GET_FIELD(regData, ((domainN+1)*2),  2) == 2 )
            {
                ...
                if(SMEM_U32_GET_FIELD(*regPtr, (domainN*8) ,  8) == gtsEntryPtr->V2DomainNumber)
                {
                    ...
                    __LOG(("PTP Domain Resolution found match for PTPv2 as domain [%d]",domainN));
                    pktDomain = domainN;
                    break;
                }
                else
                {
                    __LOG(("From the register PTP Domain [%d] with value [0x%2.2x]  not match V2DomainNumber \n",
                        domainN,gtsEntryPtr->V2DomainNumber));
                }
            }
        }
    }
...
)
{
    ...
    GT_U32 osClock = SIM_OS_MAC(simOsTickGet)();
    GT_U32 regAddr;                     /* Register's address */
    GT_U32 *regPtr;                     /* Register entry pointer */
    GT_U32 fieldVal;                    /* Register's field value */
    static GT_U32 nanoStep = 333; /* random val to ensure progress of the nanoSec part in TS */

    if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr) && taiInst > devObjPtr->freeRunningTaiInst)
    {
        taiInst = devObjPtr->freeRunningTaiInst;
    }

    /* Current TOD value */
    clocks = osClock - devObjPtr->eTodTimeStampClockDiff[taiGroup][taiInst];
    ...
    *timeCounterPtr = devObjPtr->eTod[taiGroup][taiInst];
    ...
    {
         /* the time has frozen , and we not use the 'diff' from the time clock was set till now */
        regAddr = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[taiGroup][taiInst].TODStepNanoConfigLow;
        regPtr = smemMemGet(devObjPtr, regAddr);
        fieldVal = SMEM_U32_GET_FIELD(regPtr[0], 0, 16);
        if (fieldVal == 0)
        {
            regAddr = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[taiGroup][taiInst].TODStepFracConfigHigh;
            regPtr = smemMemGet(devObjPtr, regAddr);
            fieldVal = SMEM_U32_GET_FIELD(regPtr[0], 0, 16);
            if (fieldVal == 0)
            {
                regAddr = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->TAI.TAI[taiGroup][taiInst].TODStepFracConfigLow;
                regPtr = smemMemGet(devObjPtr, regAddr);
                fieldVal = SMEM_U32_GET_FIELD(regPtr[0], 0, 16);
            }
            if (fieldVal == 0)
    ...
    /* simulation doesn't support increment of milisec so we need to avoid a situation that the ingress packet TS & egress packet TS are the same */
    nanoSeconds += nanoStep;
    nanoStep +=333;
    timeCounterPtr->nanoSecondTimer += nanoSeconds;

    if(timeCounterPtr->nanoSecondTimer > 1000000000)
    {
        /* wraparound in the nano-seconds */
        timeCounterPtr->nanoSecondTimer -= 1000000000;
        seconds += 1;
    }
...
)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (devObjPtr->portsArr[descrPtr->ingressGopPortNumber].state == SKERNEL_PORT_STATE_MTI_USX_E)
            {
                if (devObjPtr->deviceFamily == SKERNEL_HAWK_FAMILY)
                {
                    regAddr = USX_PORT_EXT(devObjPtr, descrPtr->ingressGopPortNumber).portControl;
                }
                else
                {
                    regAddr = MTI_PORT_EXT(devObjPtr, descrPtr->ingressGopPortNumber).portControl;
                }
                offset = 3;
            }
            else if (devObjPtr->portsArr[descrPtr->ingressGopPortNumber].state == SKERNEL_PORT_STATE_MTI_CPU_E)
            {
                devObjPtr->devMemPortInfoGetPtr(devObjPtr,SMEM_UNIT_TYPE_MTI_MAC_CPU_E,descrPtr->ingressGopPortNumber,&portInfo);
                cpuPortIndex = portInfo.simplePortInfo.unitIndex;
                regAddr = MTI_CPU_PORT_GLOBAL(devObjPtr,cpuPortIndex).control;
            ...
            regPtr = smemMemGet(devObjPtr, regAddr);
            descrPtr->ptpTaiSelect = SMEM_U32_GET_FIELD(*regPtr, offset, 1);
        }
    }
    ...
    snetLion3PtpTodGetTimeCounter(devObjPtr,
                                  ...
                                  &descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E]);
}
...
)
{
    ...
    else
    {
        ...
        {
            /* In SIP7 devices, flowId is scaled from 16b to 19b */
            SMEM_U32_SET_FIELD(freeFifoEntryPtr[0],10, 19, gtsEntryPtr->seqID);
        }
        else
        {
...
)
{
   ...
   if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
   {
       ...
       if(descrPtr->ptpTaiSelect < 4)
       {
           ...
           descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].secondTimer = prvSimMathSub64(descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].secondTimer,
                                                                                        deltaTimer.secondTimer);

           /* check for a WA in the nanoSec */
           if(descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer >= deltaTimer.nanoSecondTimer)
           {
               descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer -= deltaTimer.nanoSecondTimer;
           }
           else
           {
               descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E].nanoSecondTimer = (1000000000) -
       ...
       else
       {
           ...
           memcpy(&descrPtr->timestamp[descrPtr->ptpTaiSelect][SMAIN_DIRECTION_INGRESS_E], &descrPtr->tai4Timestamp[SMAIN_DIRECTION_INGRESS_E], sizeof(SNET_TOD_TIMER_STC));
       }
   }
    ...
    switch(todEvent)
    {
        ...
        case SNET_LION3_PTP_TOD_GRACEFUL_DEC_E:
                ...
                (todEvent == SNET_LION3_PTP_TOD_GRACEFUL_INC_E) )
            {
                ...
                if(devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer > 1000000000)
                {
                    /* wraparound in the nano-seconds */
                    devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer -= 1000000000;
                    timeLoadValue.secondTimer.l[0] += 1;
                }
            }
            else
            {
                /* decrement function */
                if(devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer >= timeLoadValue.nanoSecondTimer)
                {
                    devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer -=
                                                timeLoadValue.nanoSecondTimer;
                }
                else
                {
                    /* wraparound in the nano-seconds */
                    devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer =
                        (1000000000 - timeLoadValue.nanoSecondTimer) +
                        devObjPtr->eTod[taiGroup][taiInst].nanoSecondTimer;
                    timeLoadValue.secondTimer.l[0] -= 1;
                }
            }
