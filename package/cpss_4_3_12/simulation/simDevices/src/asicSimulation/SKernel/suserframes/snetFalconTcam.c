...
*                                       hits
* @param[out] tcamOverExactMatchPriorityArr - denotes priority is TCAM or EM
*/
...
(
    ...
    OUT GT_BOOL                   tcamOverExactMatchPriorityArr[SIP5_TCAM_MAX_NUM_OF_HITS_CNS]
)
{
    ...
    /* SIP7 : EM Lookup not part of TCAM.
     * Hence return the TCAM hits */
...
)
{
    ...
    if (SMEM_CHT_IS_SIP6_10_GET(devObjPtr))
    {
        ...
        else if (iPclTcamClient == SIP5_TCAM_CLIENT_MPCL_E)
        {
            /* MPCL */
            regAddr = SMEM_CHT_MAC_REG_DB_SIP5_GET(devObjPtr)->MPCL.exactMatchProfileIdMapEntry[keyIndex];
            smemRegGet(devObjPtr, regAddr, &regVal);
            tcamProfileIndex = (regVal >> 8) & 0x3F;
        }
        else
    ...
    if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        GT_U32 exactMatchNumOfHits;
        exactMatchNumOfHits = snetAasExactMatchLookupInEmx(devObjPtr, NULL/*descrPtr*/, iPclTcamClient,
                                                           keyIndex, u32keyArrayPtr, sip5KeySize,
                                                           resultArr, resulDoneInTcamArr,
                                                           tcamOverExactMatchPriorityArr);
        /* need to return the bigger num of hits */
        numOfHits = ((numOfHits > exactMatchNumOfHits) ? numOfHits : exactMatchNumOfHits);
    }
