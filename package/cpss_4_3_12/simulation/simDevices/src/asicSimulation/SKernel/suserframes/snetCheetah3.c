...
)
{
    ...
    else
    {
        ...
        {
            singleMllPtr->lll.dstInterface = SNET_DST_INTERFACE_VPORT_E;
            singleMllPtr->lll.interfaceInfo.vport =
                SMEM_LION3_IP_MLL_ENTRY_FIELD_GET(devObjPtr,mllEntryPtr,mllIndex,
                    selectorIndex ?
...
)
{
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        if(!SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            memPtr = smemMemGet(devObjPtr,SMEM_CHT3_ROUTER_MULTICAST_LIST_TBL_MEM(devObjPtr,mllIndex));
        }
        else
        {
            mllPairsLineIndex = mllIndex / devObjPtr->sip7_mllInfo.numMllPairsEntriesInLine;
            mllPairsStartBit  = (mllIndex % devObjPtr->sip7_mllInfo.numMllPairsEntriesInLine) * devObjPtr->sip7_mllInfo.numMllPairsBitsPerEntry;
        ...
        lion3ReadMllEntrySection(devObjPtr,mllIndex,0,memPtr,mllPtr);
        /* read second half of the entry .
           this also read the 'next pointer' */
        lion3ReadMllEntrySection(devObjPtr,mllIndex,1,memPtr,mllPtr);
...
)
{
    ...
    switch(singleMllPtr->lll.dstInterface)
    {
        case SNET_DST_INTERFACE_PORT_E:
            ...
            SNET_E_ARCH_CLEAR_IS_TRG_PHY_PORT_VALID_MAC(devObjPtr,descrPtr,mll);
        ...
        case SNET_DST_INTERFACE_VIDX_E:
            ...
            break;
        case SNET_DST_INTERFACE_VPORT_E:/* sip7 */
            descrPtr->trgIsVPort = 1;
            descrPtr->trgVPort   = singleMllPtr->lll.interfaceInfo.vport;
            /* call after setting trgVPort */
            SIP7_INVALIDATE_TRG_EPORT_TRG_DEV_MAC(devObjPtr,descrPtr,mll);
            break;
        default:
...
* @param[out] wasReplicatedPtr        - pointer to GT_TRUE - packet was replicated
*                                                  GT_FALSE - packet was not replicated
    ...
    while(1)
    {
        if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
        {
                ...
                snetLion3IngressMllAccessCheck(devObjPtr,descrPtr,GT_TRUE,mllIndex))
            {
                ...
                __LOG(("The MLL at index [%d] is out of range (ERROR : stopping MLL processing) \n",mllIndex));
        ...
        for(ii = 0 ; ii < 2 ; ii++)
        {
            ...
            if(ii == 0)
            {
                __LOG(("check the MLL at index[%d] , 'First' section \n",
                        mllIndex));
                singleMllPtr = &mll.first_mll;
            }
            else
            {
                __LOG(("check the MLL at index[%d] , 'Second' section \n",
                        mllIndex));
                singleMllPtr = &mll.second_mll;
            }

            if (singleMllPtr->excludeSrcVlan &&
                descrPtr->eVid == singleMllPtr->vid)
            {
                __LOG(("Replication filtered (ignored) because 'excludeSrcVlan = 1' and matched the ingress vlan[%d] \n",
                    descrPtr->eVid));
            }
            else
            {
        ...
        __LOG(("Jump to next MLL at index [%d] \n",mllIndex));
    }

    exit_cleanly_lbl:

    __LOG(("Ended the MLL replications \n"));
...
)
{
    ...
    GT_BOOL wasReplicated; /* GT_TRUE - at least one replica was done */
    ...
    snetCht3IngressMllReplication(devObjPtr,descrPtr,mllIndex, &wasReplicated);
    ...
    if (devObjPtr->errata.ipMllWrongSetMllRepInBridgedCopy && wasReplicated)
    {
         ...
         * Device erroneously sets this bit for original bridged descriptor.
         */
        ...
        __LOG(("MLL-518 erratum: repMll [0x%x] copyReserved [0x%x]\n", descrPtr->repMll, descrPtr->copyReserved));
    }
