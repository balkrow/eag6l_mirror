...
)
{
    ...
    if(descrPtr->tunnelTerminated == 0 || irf_sng_counting_mode == 1/*L3*/)
    {
        /* count L3 of TT need to count only L3 of passenger */
        if(irf_sng_counting_mode == 1 && descrPtr->tunnelTerminated) /* TT - L3 mode*/
        {   /* the behavior of Emulator */
            __LOG(("Tunnel Termination : count the L3 of the passenger \n"));
        }
        else if(irf_sng_counting_mode == 1) /* no TT - L3 mode */
        {
            __LOG(("count the L3 of the packet \n"));
        }
        else /* non TT - L2 mode */
        {
            __LOG(("count the L2 of the packet \n"));
        }
        /* count L2 of TT need to count FULL L2 just like non-TT , so it will go to the other part of the 'if' */

        byteCount = descrPtr->byteCount;
    }
    else /* TT -  L2 mode */
    {
        /* count L2 of TT need to count FULL L2 just like non-TT */
        /* count of L3 of TT is not done here , but in the other part of the 'if' */
        __LOG(("Tunnel Termination : count the L2 of the 'original' packet (not only the L2 of the passenger) \n"));
...
)
{
    ...
    {
        __LOG(("SMU : do not access to IRF table ,because streamId = 0 (not assigned) \n"));
        return;
    }
    ...
    else  /*SeqNr != recovery_sequence_num*/
    {
        ...
        __LOG_PARAM((recovery_sequence_num+1));

        if(SeqNr != ((recovery_sequence_num+1) & 0xFFFF))
        {
            incrementIrfCounter(devObjPtr,descrPtr->streamId , countersMemPtr ,
