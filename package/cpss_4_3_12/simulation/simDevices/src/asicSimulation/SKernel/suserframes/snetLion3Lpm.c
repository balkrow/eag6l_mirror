...
/**
* @internal processLpmUcSipStages function
* @endinternal
*
* @brief   search lpm, 16-31, for SIP lookup
*
* @param[in] devObjPtr                - pointer to device object.
* @param[in] descrPtr                 - pointer to the frame's descriptor.
* @param[in] addressDataLen           - address data length
...
* @retval nextHopFound                  - indicates that lookup success or not
*/
static GT_VOID processLpmUcSipStages
(
    IN    SKERNEL_DEVICE_OBJECT           *devObjPtr,
    IN    SKERNEL_FRAME_CHEETAH_DESCR_STC *descrPtr,
    IN    GT_U8                            addressDataLen,
...
{
    DECLARE_FUNC_NAME(processLpmUcSipStages);

    GT_U8   stage;
    GT_U8   startStage = SIP5_LPM_NUM_OF_DIP_SIP_STAGES;
    GT_U8   endStage   = startStage  + SIP5_LPM_NUM_OF_DIP_SIP_STAGES;
    GT_U8   dataByte   = 0;
    GT_U32  lpmEntryAddr = 0;
    ...
    for(stage = startStage; stage < endStage; stage++)
    {
        ...
        /* prepare data byte based on current stage */
        convertStageNumToDataByte(devObjPtr, descrPtr, 1, stage, &dataByte);
        ...
        {
            /* got leaf (and not bucket), done */
            __LOG(("got leaf: stage [%d], isSip [%d]", stage, 1));
...
/**
* @internal snetLion3LpmUcSipLookup function
* @endinternal
*
* @brief   sip5 LPM UC SIP lookup function
*
* @param[in] devObjPtr              - pointer to device object.
* @param[in] descrPtr               - pointer to the frame's descriptor.
*
* @param[out] matchIndexPtr         - array to indicate sip match
* @param[out] sipLpmDataPtr         - sip lpm entry data whether sip lookup was performed
* @param[out] ipSecurChecksInfoPtr  - routing security checks information
*/
GT_U32 snetLion3LpmUcSipLookup
(
    IN  SKERNEL_DEVICE_OBJECT               *devObjPtr,
    IN  SKERNEL_FRAME_CHEETAH_DESCR_STC     *descrPtr,
...
{
    DECLARE_FUNC_NAME(snetLion3LpmUcSipLookup);
    ...
    /* clear sip lpm entry data first */
    memset(sipLpmDataPtr, 0, sizeof(SNET_LPM_MEM_ENTRY_STC));

    /* init sip lpm entry with head of the trie */
    sipLpmDataPtr->nextPtr = headOfTheTrie;
    sipLpmDataPtr->data.bucket.bucketType = headBucketType;

    __LOG_PARAM(sipLpmDataPtr->nextPtr);
    __LOG_PARAM(sipLpmDataPtr->data.bucket.bucketType);
    ...
    if(continueSipStages)
    {
        ...
        processLpmUcSipStages(devObjPtr, descrPtr, addressDataLen, bypassBitmask,sipLpmDataPtr);

        /* get security checks info */
        if(LPM_POINTER_TYPE_BUCKET_E != sipLpmDataPtr->pointerType)
        {
            matchIndexPtr[1] = sipLpmDataPtr->nextPtr;

            ipSecurChecksInfoPtr->rpfCheckMode = (sipLpmDataPtr->data.leaf.lttData.lttUnicastRpfCheckEnable) ?
                                                   SNET_RPF_VLAN_BASED_E : SNET_RPF_DISABLED_E;
            ipSecurChecksInfoPtr->unicastSipSaCheck =
                            sipLpmDataPtr->data.leaf.lttData.lttUnicastSrcAddrCheckMismatchEnable;

            ipSecurChecksInfoPtr->ipv6MulticastGroupScopeLevel =
                            sipLpmDataPtr->data.leaf.lttData.lttIpv6MulticastGroupScopeLevel;
        }
        if((LPM_POINTER_TYPE_ECMP_LEAF_E == sipLpmDataPtr->pointerType) ||
           (LPM_POINTER_TYPE_QOS_LEAF_E  == sipLpmDataPtr->pointerType))
        {
            ipSecurChecksInfoPtr->sipFromEcmpOrQosBlock  = 1;

            ipSecurChecksInfoPtr->sipNumberOfPaths       = sipLpmDataPtr->data.leaf.ecmpOrQosData.numOfPaths;
            ipSecurChecksInfoPtr->sipBaseRouteEntryIndex = sipLpmDataPtr->data.leaf.ecmpOrQosData.nextHopBaseAddr;
