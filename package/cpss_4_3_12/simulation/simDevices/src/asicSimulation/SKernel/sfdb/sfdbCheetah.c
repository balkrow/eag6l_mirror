...
typedef struct{
    /* transplant info : 'old interface' : port/trunk/vPort */
    GT_U32  oldPortOrTrunkOrVPort;
    GT_U32  oldDev;
    GT_U32  oldIsTrunk;
    GT_U32  oldIsVPort;             /* sip7 : the old interface of type vPort */
    /* transplant info : 'new interface' : port/trunk/vPort */
    GT_U32  newPortOrTrunkOrVPort;
    GT_U32  newDev;
    GT_U32  newIsTrunk;
    GT_U32  newIsVPort;             /* sip7 : the new interface of type vPort */

    /* all actions (not transplant) info: value */
    GT_U32  actVlan;                /*also relevant for transplant action */
    GT_U32  actIsTrunk;
    GT_U32  actPortTrunkVPort;      /*the action port/trunk/vport value */
    GT_U32  actDevice;
    GT_U32  actUserDefined;
    GT_U32  actVID1;
    GT_U32  actIsVPort;             /* sip7 : is action on vPorts (not support transplant of vPort entries) */
    GT_U32  actEpg;                 /* sip7 : EPG for the action */

    /* all actions (not transplant) info: mask */
    GT_U32  actVlanMask;            /*also relevant for transplant action */
    GT_U32  actIsTrunkMask;
    GT_U32  actPortTrunkVPortMask;  /*the action port/trunk/vport Mask */
    GT_U32  actDeviceMask;
    GT_U32  actUserDefinedMask;
    GT_U32  actVID1Mask;           /*also relevant for transplant action */
    GT_U32  actIsVPortMask;        /* sip7 : the mask for action on vPorts  (not support transplant of vPort entries) */
    GT_U32  actEpgMask;            /* sip7 : EPG mask for the action */
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            entryInfoPtr->fid |=
                SMEM_SIP6_FDB_AU_MSG_ENTRY_FIELD_GET(devObjPtr,msgWordPtr,
                    SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_FID_15) << 15;
        }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        ...
        switch(fldValue)
        {
            case SIP7_SFDB_UPDATE_MSG_NA_E:
            case SIP7_SFDB_UPDATE_MSG_MOVED_E:
                msgType = SFDB_UPDATE_MSG_NA_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_QA_E:
                msgType = SFDB_UPDATE_MSG_QA_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_QR_E:
                msgType = SFDB_UPDATE_MSG_QR_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_AA_E:
                msgType = SFDB_UPDATE_MSG_AA_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_TA_E:
                msgType = SFDB_UPDATE_MSG_TA_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_FU_E:
                msgType = SFDB_UPDATE_MSG_FU_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_QI_E:
                msgType = SFDB_UPDATE_MSG_QI_E;
                break;
            case SIP7_SFDB_UPDATE_MSG_HR_E:
                msgType = SFDB_UPDATE_MSG_HR_E;
                break;
            default:
                msgType = SFDB_UPDATE_MSG_HR_E + 1;
                break;
        }
    }
    else
...
#define SIP6_GET_VALUE(structFieldName, tableFieldName)\
    ...
    memset(fdbEntryInfoPtr,0,sizeof(*fdbEntryInfoPtr));
    fdbEntryInfoPtr->valid = 1;
       ...
       fdb_entry_type == SNET_CHEETAH_FDB_ENTRY_IPV6_IPMC_BRIDGING_E)
    {
        ...
        {
            fldValue =
                SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,fdbEntryPtr, entryIndex,
                    SMEM_SIP7_FDB_FDB_TABLE_FIELDS_FID_15);
            fdbEntryInfoPtr->fid |= fldValue << 15;


            if(fdbEntryInfoPtr->multiple)
            {
                fldValue =
                    SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,fdbEntryPtr, entryIndex,
                        SMEM_SIP7_FDB_FDB_TABLE_FIELDS_VIDX_16);
                fdbEntryInfoPtr->vidx |= fldValue << 16;
            }
            else
            if( fdbEntryInfoPtr->is_trunk == 0 )
            {
                SIP6_GET_VALUE(isVPort       , SMEM_SIP7_FDB_FDB_TABLE_FIELDS_USE_VPORT           );

                if(fdbEntryInfoPtr->isVPort)
                {
                    GT_U32  vportNum ;
                    ...
                    vportNum =
                        ...
                        (SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,fdbEntryPtr, entryIndex,
                            SMEM_SIP7_FDB_FDB_TABLE_FIELDS_TRG_VPORT_14_17) << 14);

                    fdbEntryInfoPtr->vportNum = vportNum;
                }
            }
        }
       ...
       fdb_entry_type == SNET_CHEETAH_FDB_ENTRY_FCOE_UC_ROUTING_E)
    {
        ...
        if(SMEM_CHT_IS_SIP6_30_GET(devObjPtr))
        {
            SIP6_GET_VALUE(epg_streamId                               , SMEM_SIP6_30_FDB_FDB_TABLE_FIELDS_UC_ROUTE_EPG_STREAM_ID                  );
        }

        if(SMEM_CHT_IS_SIP7_GET(devObjPtr) &&
            fdbEntryInfoPtr->ucRoute.use_vidx     == 0 &&
            fdbEntryInfoPtr->ucRoute.trg_is_trunk == 0)
        {
            SIP6_GET_VALUE(ucRoute.trg_isVPort       , SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_USE_VPORT           );
            if(fdbEntryInfoPtr->ucRoute.trg_isVPort)
            {
                SIP6_GET_VALUE(ucRoute.trg_vportNum      , SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_TRG_VPORT           );
            }
        }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        {
            /* skip is not part of the entry (sip7) */
            skip_old = 0;
        }
        else
        {
        ...
        if(valid_old && skip_old == 0 && macEntryType == SNET_CHEETAH_FDB_ENTRY_MAC_E)
        {
            spUnknown_old =
                SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,oldEntryPtr,
                oldIndex,
        ...
        if(valid && skip == 0 && macEntryType == SNET_CHEETAH_FDB_ENTRY_MAC_E)
        {
            spUnknown =
                SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,newEntryPtr,
                newIndex,
...
)
{
    ...
    if (TriggerMode)
    {
        /* allow trigger aging/transplant to send LOG info , like 'processing' daemon */
        SIM_OS_MAC(simOsTaskOwnTaskPurposeSet)(SIM_OS_TASK_PURPOSE_TYPE_PP_AGING_DAEMON_E, NULL);
...
)
{
    ...
    GT_U32    vlanSize; /* size of portTrunk field in SIP7 and above registers */
    GT_U32    udbSize;  /* size of UDB field in SIP5 and above registers */
    ...
    {
        vlanSize = 13;
        udbSize = SMEM_CHT_IS_SIP6_GET(devObjPtr) ? 11 : 8;
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            portTrunkSize = 18;/* larger field to support vport */
            vlanSize = 16;
        }
        else
        if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr))
        ...
        actionInfoPtr->actVlan =   /* Action Active VLAN */
            SMEM_U32_GET_FIELD(tblActPtr[1], 0, vlanSize);

        actionInfoPtr->actVlanMask = /* Action Active VLAN Mask */
            SMEM_U32_GET_FIELD(tblActPtr[1], 16, vlanSize);

        actionInfoPtr->actPortTrunkVPort = /* Action Active Port/Trunk */
        ...
        actionInfoPtr->actPortTrunkVPortMask = /* Action Active Port/Trunk Mask */
        ...
        actionInfoPtr->actUserDefined =
            SMEM_U32_GET_FIELD(tblActPtr[4],  0, udbSize);

        actionInfoPtr->actUserDefinedMask =
            SMEM_U32_GET_FIELD(tblActPtr[4], udbSize, udbSize);
        ...
        actionInfoPtr->oldPortOrTrunkOrVPort = /* Old Port/Old Trunk */
        ...
        actionInfoPtr->newPortOrTrunkOrVPort = /* new Port/Old Trunk */
        ...
        if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
        {
            actionInfoPtr->actIsVPort     =
                SMEM_U32_GET_FIELD(tblActPtr[2], 29, 1);
            actionInfoPtr->actIsVPortMask =
                SMEM_U32_GET_FIELD(tblActPtr[3], 29, 1);

            actionInfoPtr->actEpg     =
                SMEM_U32_GET_FIELD(tblActPtr[8], 0, 12);
            actionInfoPtr->actEpgMask =
                SMEM_U32_GET_FIELD(tblActPtr[8], 12, 12);

            SIM_TBD_BOOKMARK /* still not in Cider */
            actionInfoPtr->oldIsVPort =                   /* 18+1+10 = bit 29*/
                SMEM_U32_GET_FIELD(tblActPtr[6], portTrunkSize + 1/*oldIsTrunk*/ +10/*oldDev*/, 1);
            actionInfoPtr->newIsVPort =                   /* 18+1+10 = bit 29*/
                SMEM_U32_GET_FIELD(tblActPtr[7], portTrunkSize + 1/*newIsTrunk*/ +10/*newDev*/, 1);

            if(actionInfoPtr->newIsTrunk)
            {
                actionInfoPtr->newIsVPort = 0;/* 'trunk' is checked before 'vPort'*/
            }

            if(actionInfoPtr->oldIsTrunk)
            {
                actionInfoPtr->oldIsVPort = 0;/* 'trunk' is checked before 'vPort'*/
            }
    ...
    else
    {
        ...
        */
        ...
        actionInfoPtr->newPortOrTrunkOrVPort = /* New Port/New Trunk */
    ...
    if(SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        smemSip6DfxServerOwnDevNum(devObjPtr,GT_FALSE,&actionInfoPtr->ownDevNum);
    }
    else
    {
    ...
    else
    {
        ...
        actionInfoPtr->IPMCAddrDelMode = 0;
    }
...
{
    GT_U32  useSkipBit = actionInfoPtr->multiHashEnable ? 0 : 1;

    if(useSkipBit  && SMEM_CHT_IS_SIP7_GET(devObjPtr))
    {
        GT_U32  fdb_entry_type =
            SMEM_SIP6_FDB_FDB_ENTRY_FIELD_GET(devObjPtr,entryPtr,
                index,
                SMEM_SIP6_FDB_FDB_TABLE_FIELDS_FDB_ENTRY_TYPE);
        if(fdb_entry_type >= SNET_CHEETAH_FDB_ENTRY_IPV4_UC_ROUTING_E &&
           fdb_entry_type <= SNET_CHEETAH_FDB_ENTRY_FCOE_UC_ROUTING_E)
    ...
    */
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        if(useSkipBit == 0)/*relevant to SIP5*/
        {
...
)
{
    ...
    {
        fldValue = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? SIP7_SFDB_UPDATE_MSG_TA_E : SFDB_UPDATE_MSG_TA_E;
    }
    else
    if(action == SFDB_CHEETAH_TRIGGER_UPLOAD_E)
    {
        fldValue = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? SIP7_SFDB_UPDATE_MSG_FU_E : SFDB_UPDATE_MSG_FU_E;
    }
    else
    {
       fldValue = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? SIP7_SFDB_UPDATE_MSG_AA_E : SFDB_UPDATE_MSG_AA_E;
    }
    ...
    while(GT_FALSE == status)
    {
        ...
        {
            status = snetChtL2iSendUpdMsg2Cpu(devObjPtr, macUpdMsg,devObjPtr->numOfWordsInAuMsg);
        }
...
)
{
    ...
    GT_U32  entry_isVPort;/* the field use_vport in the entry */
    GT_U32  entry_portTrunkNum_vPort; /* the field portTrunkNum in the entry */
    ...
    GT_U32  bypassCompareDevIdInEntry;/*when 1 : indication that action would not use 'devId' from the entry ,
                                        and not use actDevice,actDeviceMask on the entry */
    ...
    *needBreakPtr = 0;
    ...
    if(sip6FdbEntryInfoPtr)
    {
        ...
        if(macEntryType == SNET_CHEETAH_FDB_ENTRY_HSU_PRP_DDE_E) /* sip6.30 */
        {
            /* such entry should not be in the FDB partition that is scanned */
            /* why it exists here ???                                        */
            return;
        }
    }
    ...
    if(SMEM_CHT_IS_SIP5_GET(devObjPtr))
    {
        ...
        if(macEntryType > SNET_CHEETAH_FDB_ENTRY_FCOE_UC_ROUTING_E)
        {
            /* unknown entry type : should not exist here */
            return;
        }
    }
    else
    {
        ...
        if(macEntryType > SNET_CHEETAH_FDB_ENTRY_IPV6_IPMC_BRIDGING_E)
        {
            /* unknown entry type : should not exist here */
            return;
        }
        ...
        SNET_CHEETAH_FDB_ENTRY_IPV6_IPMC_BRIDGING_E == macEntryType)
    {
        ...
        if(actionInfoPtr->IPMCAddrDelMode == 0)
        {
    ...
    if(sip6FdbEntryInfoPtr)
    {
        ...
        {
            /* trunk num */
            entry_portTrunkNum_vPort = isUcRoutingEntry ?
        ...
        else
        {
            ...
            entry_isVPort = sip6FdbEntryInfoPtr->isVPort;
            /* the DevID is not part of the IP UC entry (muxed with vPort) ,
               so don't compare it */
            bypassCompareDevIdInEntry = entry_isVPort && isUcRoutingEntry ? 1 : 0;

            if((isUcRoutingEntry && sip6FdbEntryInfoPtr->ucRoute.trg_isVPort) ||
                (isUcRoutingEntry == 0 && sip6FdbEntryInfoPtr->isVPort)) /* sip7 only */
            {
                /* vport num */
                entry_portTrunkNum_vPort =  isUcRoutingEntry ?
                    sip6FdbEntryInfoPtr->ucRoute.trg_vportNum :
                    sip6FdbEntryInfoPtr->vportNum;
            }
            else
            {
                /* eport num */
    ...
    else
    {
        ...
        /* Port Num/Trunk Num */
        entry_portTrunkNum_vPort = SMEM_U32_GET_FIELD(entryPtr[2], 14, 7);
    }
    ...
    /* Sip7: check EPG filter , for MAC entry only (MAC,IPV4_IPMC,IPV6_IPMC)*/
    /* NOTE: the actEpg,actEpgMask not exists in sip6.10 although supporting EPG ! */
    if(sip6FdbEntryInfoPtr && !isUcRoutingEntry)
    {
        if ((sip6FdbEntryInfoPtr->epg_streamId  & actionInfoPtr->actEpgMask) !=
             actionInfoPtr->actEpg)
        {
            /* don't act on entries that are not match on 'EPG' */
            return;
        }
    }
    ...
    if(action == SFDB_CHEETAH_TRIGGER_TRANS_E)
    {
        ...
        if(bypassCompareDevIdInEntry)
        {
            /* ignore 'devNumInEntry' */
        }
        else
        /* DevID */
        ...
        if(entry_isVPort != actionInfoPtr->oldIsVPort)
        {
            return;
        }

        /* Port Num/Trunk Num/vPort */
        fldValue = entry_portTrunkNum_vPort;

        if (fldValue != actionInfoPtr->oldPortOrTrunkOrVPort)
        {
            return;
        }
        ...
        if(sip6FdbEntryInfoPtr)
        {
            ...
            if(actionInfoPtr->newIsTrunk)
            {
                ...
                index,
                entryField,
                actionInfoPtr->newPortOrTrunkOrVPort);
            }
            else
            if(actionInfoPtr->newIsVPort)
            {
                /* New vPort */
                entryField = isUcRoutingEntry ? SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_TRG_VPORT :
                                                SMEM_SIP7_FDB_FDB_TABLE_FIELDS_TRG_VPORT_0_13 ;
                ...
                if(!isUcRoutingEntry)
                {
                    entryField = SMEM_SIP7_FDB_FDB_TABLE_FIELDS_TRG_VPORT_14_17;
                    /* additional 4 bits after first 14 */
                    SMEM_SIP6_FDB_FDB_ENTRY_FIELD_SET(devObjPtr,entryPtr,
                    index,
                    entryField,
                    actionInfoPtr->newPortOrTrunkOrVPort >> 14);
                }
            }
            else
            {
            ...
            if(actionInfoPtr->newIsVPort != actionInfoPtr->oldIsVPort) /* SIP7 */
            {
                entryField = isUcRoutingEntry ? SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_USE_VPORT :
                                                SMEM_SIP7_FDB_FDB_TABLE_FIELDS_USE_VPORT;
                SMEM_SIP6_FDB_FDB_ENTRY_FIELD_SET(devObjPtr,entryPtr,
                index,
                entryField,
                actionInfoPtr->newIsVPort);
            }
        ...
        else
        {
            ...
            /* New port */
            SMEM_U32_SET_FIELD(entryPtr[2], 14, 7, actionInfoPtr->newPortOrTrunkOrVPort);
        }
    }
    else
    {
        ...
        /* for BobCat2 B0 and above
           ...
           actPortTrunkVPortMask, actIsTrunkMask, actDeviceMask.  */
        if(SMEM_CHT_IS_SIP5_10_GET(devObjPtr)&&
            ...
            (SNET_CHEETAH_FDB_ENTRY_IPV6_UC_ROUTING_DATA_E == macEntryType)))
        {
            if ((actionInfoPtr->actDeviceMask!=0)||
                ...
                (actionInfoPtr->actPortTrunkVPortMask!=0))
            {
       ...
       if(isMultiCastEntry == GT_FALSE)
       {
           ...
           isLocalDevPort = 1 - fldValue;
            ...
            /* Port Number/ Trunk Number / vPort */
            fldValue = entry_portTrunkNum_vPort;
            ...
            if ((isUcIpv6KeyRoutingEntry==0) &&
                ((fldValue & actionInfoPtr->actPortTrunkVPortMask) != actionInfoPtr->actPortTrunkVPort))
            {
                return;
            }
            else
            if ((isUcIpv6KeyRoutingEntry==0) &&   /* sip7 only */
                ((fldValue & actionInfoPtr->actIsVPortMask) != actionInfoPtr->actIsVPort))
            {
                return;
            }
        }
    ...
    */
    ...
    {
        portInEntry = entry_portTrunkNum_vPort;
...
)
{
    ...
    if(devObjPtr->sharedTablesInfo.isFdbShared &&
       (!devObjPtr->fdbNoDirectMemory) &&
       (0 == devObjPtr->tablesInfo.fdbTable.paramInfo[0].outOfRangeIndex))
    {
        return;
    }
...
)
{
    ...
    if(SMEM_CHT_IS_SIP6_GET(devObjPtr))
    {
        ...
        /* Message Type */
        fldValue = SMEM_CHT_IS_SIP7_GET(devObjPtr) ? SIP7_SFDB_UPDATE_MSG_QR_E : SFDB_UPDATE_MSG_QR_E;
        SMEM_SIP6_FDB_AU_MSG_ENTRY_FIELD_SET(devObjPtr,macUpdMsg,
            SMEM_SIP6_FDB_AU_MSG_TABLE_FIELDS_MSG_TYPE,
            fldValue);
    ...
    /* Send MAC update message to CPU */
    while(GT_FALSE == snetChtL2iSendUpdMsg2Cpu(devObjPtr, &macUpdMsg[0],devObjPtr->numOfWordsInAuMsg))
    {
        if(oldWaitDuringSkernelTaskForAuqOrFua)
        {
            /* wait for SW to free buffers */
...
static SIP6_FDB_AU_MSG_CONVERT_INFO sip6FdbAuMsgConvertInfoArr[] = {
    ...
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_FID_15                   ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_FID_15               }
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_VIDX_16                  ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_VIDX_16              }
...
static SIP6_FDB_AU_MSG_CONVERT_INFO sip6FdbAuMsgUcRoutePortConvertInfoArr[] = {
    ...
    ,{SMEM_SIP6_30_FDB_FDB_TABLE_FIELDS_UC_ROUTE_EPG_STREAM_ID                        ,SMEM_SIP6_30_FDB_AU_MSG_TABLE_FIELDS_UC_EPG_STREAM_ID                          }
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15                       ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15                   }
...
/* array to convert fields from the FDB format to AU_MSG format and vice versa - when interface is vPort */
static SIP6_FDB_AU_MSG_CONVERT_INFO sip7FdbAuMsgUcRoutePortConvertInfoArr[] = {
    /* common fields */
     {SMEM_SIP6_FDB_FDB_TABLE_FIELDS_VALID                    ,SMEM_SIP6_FDB_AU_MSG_TABLE_FIELDS_VALID                }
    ,{SMEM_SIP6_FDB_FDB_TABLE_FIELDS_SKIP                     ,SMEM_SIP6_FDB_AU_MSG_TABLE_FIELDS_SKIP                 }
    ,{SMEM_SIP6_FDB_FDB_TABLE_FIELDS_AGE                      ,SMEM_SIP6_FDB_AU_MSG_TABLE_FIELDS_AGE                  }
    ...
    ,{SMEM_SIP6_30_FDB_FDB_TABLE_FIELDS_UC_ROUTE_EPG_STREAM_ID                        ,SMEM_SIP6_30_FDB_AU_MSG_TABLE_FIELDS_UC_EPG_STREAM_ID                          }
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15                       ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_NEXT_HOP_EVLAN_15                   }
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_USE_VPORT                               ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_USE_VPORT                           }
    ,{SMEM_SIP7_FDB_FDB_TABLE_FIELDS_UC_ROUTE_TRG_VPORT                               ,SMEM_SIP7_FDB_AU_MSG_TABLE_FIELDS_UC_ROUTE_VPORT                               }
...
)
{
    ...
    for(/*already init*/; convertPtr->fdbField != SMEM_SIP6_FDB_FDB_TABLE_FIELDS___LAST_VALUE___E; convertPtr++)
    {
        ...
        if(convertPtr->fdbField == SMEM_SIP6_FDB_FDB_TABLE_FIELDS_MAC_ADDR &&
            macEntryType <= SNET_CHEETAH_FDB_ENTRY_IPV6_IPMC_BRIDGING_E)/*IPV4_MC/IPV6_MC*/
        {
            /* field not relevant for IPMC format. need to skip it. */
            continue;
        }
        else
        if(macEntryType == SNET_CHEETAH_FDB_ENTRY_MAC_E &&
           (convertPtr->fdbField == SMEM_SIP6_FDB_FDB_TABLE_FIELDS_SIP_26_0 ||
            convertPtr->fdbField == SMEM_SIP6_FDB_FDB_TABLE_FIELDS_SIP_30_27 ||
            convertPtr->fdbField == SMEM_SIP6_FDB_FDB_TABLE_FIELDS_SIP_31 ||
            convertPtr->fdbField == SMEM_SIP6_FDB_FDB_TABLE_FIELDS_DIP ))
        {
            /* field not relevant for MAC format. need to skip it. */
            continue;
        }
        ...
        case SNET_CHEETAH_FDB_ENTRY_FCOE_UC_ROUTING_E:
            ...
            else
            {
                ...
                else
                {
                    if (SMEM_CHT_IS_SIP7_GET(devObjPtr))
                    {
                        isVport = SMEM_SIP6_FDB_AU_MSG_ENTRY_FIELD_GET(devObjPtr, msgWordPtr,
                    ...
                    if (isVport)
                    {
                        convertPtr = &sip7FdbAuMsgUcRoutePortConvertInfoArr[0];
                    }
                    else
                    {
                        convertPtr = &sip6FdbAuMsgUcRoutePortConvertInfoArr[0];
...
#ifdef _WIN32
        /* the test cpssDxChBrgFdbAgingTimeoutSet__auto_aging_functionality fail on
           windows with numOfEntries_debug = 256 , but pass with 1024 and 512
           when numOfEntries_debug = 256 it takes ~30 seconds to do full full cycle ,
           instead of the 20 seconds that the test sets .
        */

        if(deviceObjPtr->deviceFamily == SKERNEL_AAS_FAMILY)
        {
            numOfEntries = 2 * numOfEntries_debug;
        }
#endif /*_WIN32*/
