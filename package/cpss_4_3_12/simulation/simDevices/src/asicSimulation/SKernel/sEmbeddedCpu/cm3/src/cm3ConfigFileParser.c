...
GT_STATUS cm3ParseConfigCommand( void )
{
    ...
    if ( stricmp( type, "rerun" ) == 0)
    {
        if(0 == fileFound)
        {
            if(strlen(configNameOrId)==1 && ((GT_U32)(configNameOrId[0]) >= (GT_U32)('0')))
            {
                fprintf( stderr,"Micro Init : WARNING : can't find profile [%d] in the supper image \n",
                    configNameOrId[0]-'0');
            }
            else
            {
        ...
        else
        {
            if(strlen(configNameOrId)==1 && ((GT_U32)(configNameOrId[0]) >= (GT_U32)('0')))
            {
                fprintf( stderr,"Micro Init : processed profile [%d] \n",
                    configNameOrId[0]-'0');
            }
            else
            {
                fprintf( stderr,"Micro Init : processed file/profile [%s] \n",
                    configNameOrId);
            }
        }
    }
