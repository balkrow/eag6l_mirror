...
void falcon_init_shared_data_memory(uint32_t ppn_num)
{
    ...
    #if (defined(ENABLE_THR81_Erspan_TypeIII_SameDevMirroring_IPv4) || defined(ENABLE_THR84_Erspan_TypeIII_OrigDevMirroring) || defined(ENABLE_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged) || defined(ENABLE_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged))
        /* set default value in shared memory template:  | MAC DA SA = 0x123456789ab, 0x123456789ac | TPID = 0x5555 | UP = 0x3 | CFI = 0 | VID = 666 |  */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE + 4, (uint32_t)0x12345678 );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE + 8, (uint32_t)0x9abc1234 );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE + 12,(uint32_t)0x56789abd );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE + 16,(uint32_t)0x55556666 );
        /* PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE + 16,(uint32_t)0x00000000 ); */

        /* set default ipv4 values in shared memory template: */
        /* ipv4 Version[4bit] - 0x4 | IHL[4bit] - 0x5 | DSCP[6bit] - API | ECN[2b] - 0x0 | Total length[2B] - Thread */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  0, 0x45FC0000  );
        /* ipv4 Identification[2B] - 0x0 | Flags[3bit] - API | Fragment offset[13bit] - 0x0 )  */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  4, 0x0000E000  );
        /* ipv4 TTL[1B] - API | Protocol[1B] - 0x1f (47) | Header CS[2B] - Thread */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  8, 0x201f0000 );
        /* ipv4.SIP - API */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR + 12,0xcccccccc );
        /* ipv4.DIP - API*/
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR + 16,0xdddddddd );

        /* reserved = 0x0 HW_ID = 0x3F ingress=0xf8 egress=0x1f*/
        PPA_FW_SP_WRITE(DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR +  IPV4_BASIC_HEADER_SIZE, 0x03F3e01F );
        /* | reserved (6b)  | Switch_ID (10b) | VoQ (8b)  | reserved (8b) | */
        PPA_FW_SP_WRITE(DRAM_CC_ERSPAN_GLOBAL_ADDR, (uint32_t)0x03FF0000);


    #endif
    #if (defined(ENABLE_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass) ||  defined(ENABLE_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass) || defined(ENABLE_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged) || defined(ENABLE_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged))

        /* set default value in shared memory template:  | MAC DA SA = 0x123456789ab, 0x123456789ac | TPID = 0x5555 | UP = 3 | CFI = 0 | VID = 666 |  */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE + 4, (uint32_t)0x12345678 );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE + 8, (uint32_t)0x9abc1234 );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE + 12,(uint32_t)0x56789abd );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE + 16,(uint32_t)0x55556666 );
        /* PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE + 16,(uint32_t)0x00000000 ); */


        /* set ipv6 header values in shared memory template:    */
        /* ipv6 Version[4bit] - 0x6 | TC[1B] - API(AB) | Flow_label[20its] - API(12345) | */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  0, 0x6AB12345  );
        /* Payload length[2B] - THREAD | Next_Header[1B] - 0x47 | Hop_Limit[1B] - API(0xBA) )  */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  4, 0x000047BA  );
        /* ipv6.SIP - API */
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  8, 0xdddddddd );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 12, 0xdddddddd );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 16, 0xdddddddd );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 20, 0xdddddddd );
        /* ipv6.DIP - API*/
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 24, 0xcccccccc );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 28, 0xcccccccc );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 32, 0xcccccccc );
        PPA_FW_SP_WRITE( DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR + 36, 0xcccccccc );

        /* reserved(6b) = 0x0 HW_ID(6b) = 0x3F ingress=0xf8(10b) egress=0x1f(10b)*/
        PPA_FW_SP_WRITE(DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR +  IPV6_BASIC_HEADER_SIZE, 0x03F3e01F );
        /* | reserved (6b)  | Switch_ID (10b) = 0x3FF | VoQ (8b)  | reserved (8b) | */
        PPA_FW_SP_WRITE(DRAM_CC_ERSPAN_GLOBAL_ADDR, (uint32_t)0x03FF0000);


        #if 0
        /* Should be executed in thread83/85  context for debug */
        /* GRE header */
        /* Flags[2B] - 0x1000 | Protocol[2B] - 0x22eb | Sequence number[4B] - 0x88888888*/
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET,      0x100022eb);
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 4,  0x88888888);
        /* ERSPAN header */
        /* Ver[4b] - 0x2 | VLAN[12b] - 0x0 | COS[3b] - 0x2 | BSO[2b] - 0x0 | T[1b] - 0| c(sessionId)[10b] - 0xf8  */
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 8,  0x200040f8);
        /* TimeStamp[4B] - 0xb6b6b6b6 */
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 12, 0xb6b6b6b6);
        /* SGT[2B] - 0x0 | P[1b] - 0x1 | FT[5b] - 0x0 | HW ID[6b] - 0x3f | D[1b] - 0 | GRA[2b] - 0x2 | O[1b] - 0x1 */
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 16, 0x000083f5);
        /* Platform specific sub header */
        /* Platf_ID[6b] - 0x5 | Switch_ID[10b] - 0x3ff | Port_ID[2B] - 0xcafe */
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 20, 0x17ffcafe);
        /* TimeStamp[4B] - 0xa1a1a1a1 */
        PPA_FW_SP_WRITE(FALCON_PKT_REGs_lo + THR83_IN_HDR_GRE__C_OFFSET + 24, 0xa1a1a1a1);
        #endif

        #if 0
        /* Should be executed in thread82 context for debug */
        /* write edsa tags*/
...
void falcon_sFlowTablesInit ()
{
    ...
    for (i=0;i<128;i++)
    {
        ...
        /* Clear Drops table. 128 entries each of 4B */
        PPA_FW_SP_WRITE( DRAM_SFLOW_SOURCE_PORT_DROPS_TABLE_ADDR + i*4 , 0x0 );
    }

    /* Run over all table entries as the number of target collector IDs */
    for (i=0;i<16;i++)
    {
        /* Clear Datagram Header Sequence Number table. 16 entries each of 4B */
        PPA_FW_SP_WRITE( DRAM_SFLOW_DATAGRAM_HEADER_SEQ_NUM_TABLE_ADDR + i*4 , 0x0 );
    }
