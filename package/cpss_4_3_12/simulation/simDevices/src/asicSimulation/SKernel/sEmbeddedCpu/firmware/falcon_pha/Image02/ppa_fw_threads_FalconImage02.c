...
void PPA_FW(THR56_Clear_Outgoing_Mtag_Cmd) () {
    ...
    uint32_t cfg_reserved_0 = PPA_FW_SP_READ(FALCON_CFG_REGs_lo);
...
void PPA_FW(THR76_Copy_Flow_ID_to_Trg_Phy_Port)() {
    ...
    struct thr76_Copy_Flow_ID_to_Trg_Phy_Port_in_hdr* thr76_in_hdr_ptr = FALCON_MEM_CAST(FALCON_PKT_REGs_lo,thr76_Copy_Flow_ID_to_Trg_Phy_Port_in_hdr);
    uint32_t cfg_reserved_0 = PPA_FW_SP_READ(FALCON_CFG_REGs_lo);
    ...
    else
    {
        ...
        /* Get the local source port from the descriptor */
        localDevSrcPort = desc_ptr->phal2ppa.local_dev_src_port;
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    falcon_stack_overflow_checker(76);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR80_PHA_ECN_counting
 * inparam      None
 * return       None
 * description  Thread maintain counter table and increase the specific counter
 *              according to the target port and descriptor fields mark_ecn bit and tc.
 *
 *              cfg template: Not in use (16B)
 *              src & trg entries:  (4B)
 *                  src entry not in use.
 *                  trg entry is not in use.
 *
 *              hdr in:    | Extension space (32B) | MAC Header(12B) | Payload(16B)|
 *              hdr out:   | Extension space (32B) | MAC Header(12B) | Payload(16B)|
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *              - increase counter by one depending on mark_ecn and tc fields.
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR80_PHA_ECN_counting)()
{
    /* xt_iss_client_command("profile","enable");             */
    /* xt_iss_client_command("profile","disable"); _exit(0);  */
    struct ppa_in_desc* thr80_desc_ptr = FALCON_MEM_CAST(FALCON_DESC_REGs_lo,ppa_in_desc);

    __LOG_FW(("thr80_desc_ptr->phal2ppa.mark_ecn"));
    __LOG_PARAM_FW(thr80_desc_ptr->phal2ppa.mark_ecn);
    __LOG_FW(("thr80_desc_ptr->phal2ppa.outgoing_mtag_cmd"));
    __LOG_PARAM_FW(thr80_desc_ptr->phal2ppa.outgoing_mtag_cmd);

    if ( (thr80_desc_ptr->phal2ppa.mark_ecn == 1) && (thr80_desc_ptr->phal2ppa.outgoing_mtag_cmd != 2) /*TO_ANALYZER*/)
    {
        __PRAGMA_FREQUENCY_HINT_FREQUENT

        uint32_t queue_counter;
        uint32_t base_address = DRAM_ECN_COUNTER_TABLE_ADDR; /* 0x1061c */
        uint32_t local_dev_trg_phy_port = thr80_desc_ptr->phal2ppa.local_dev_trg_phy_port;
        uint32_t tc = thr80_desc_ptr->phal2ppa.tc;

        /* local_dev_trg_phy_port [table index] * 32  */
        __LOG_FW(("Get index to shared memory ECN counters table. Get it from desc_ptr->phal2ppa.local_dev_trg_phy_port"));
        __LOG_PARAM_FW(thr80_desc_ptr->phal2ppa.local_dev_trg_phy_port);
        base_address += local_dev_trg_phy_port * DRAM_ECN_COUNTER_TABLE_ENTRY_SIZE;
        /* Each counter(queue) size is 4B */
        base_address += tc * 4;
        __LOG_FW(("tc (queue number):"));
        __LOG_PARAM_FW(thr80_desc_ptr->phal2ppa.tc);

        queue_counter = PPA_FW_SP_READ(base_address);
        queue_counter++;
        __LOG_FW(("Updated counter value:"));
        __LOG_PARAM_FW(queue_counter);

        PPA_FW_SP_WRITE(base_address, queue_counter);
    }

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    falcon_stack_overflow_checker(80);
#endif
...
THREAD_TYPE phaThreadsTypeFalconImage02[] = {
    ...
    /*80*/ ,PPA_FW(THR80_PHA_ECN_counting)
