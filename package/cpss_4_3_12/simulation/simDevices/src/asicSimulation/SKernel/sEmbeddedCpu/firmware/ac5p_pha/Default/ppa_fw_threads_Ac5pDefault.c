...
/*************************************************************************************************************************************//**
 * funcname     THR58_IPSEC_PTP
 * inparam      None
 * return       None
 * description  Part of sequence flow to add timestamp to packet with PTP encrypted by IPSEC 
 *              Thread is triggered by trg_ePort or EPCL to modify descriptor PTP PACKET FORMAT to PTPv2,
 *              otherwise the current format of Y1731 which was set in timestamp configuration table by application,
 *              zeros the QueueEntryId in the TX PCH (validity logic).
 * 
 *              cfg template: Not in use (16B) 
 *              src & trg entries: Not in use (4B)
 *              hdr in:    | Extension space (32B) | IPv4 | UDP | PTP | } encrypted
 *              hdr out:   | Extension space (32B) | IPv4 | UDP | PTP | } encrypted
 *              Header Window anchor= Outer Layer 3
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *              - Set Desc<PTP Packet Format> to PTPv2 (0x0)  
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR58_IPSEC_PTP)() {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc*  thr58_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    /* Desc<PTP Packet Format>=PTPv2 (0x0) */
    __LOG_FW(("Set Desc<PTP Packet Format> to PTPv2 (0x0)"));
    thr58_desc_ptr->phal2ppa.ptp_packet_format = PTP_PKT_FORMAT_PTP_V2_E;

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(58);
#endif
...
void PPA_FW(THR59_SLS_Test)() {
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(59);
#endif
...
void PPA_FW(THR68_Erspan_TypeI_Ipv4)() {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr68_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6. */
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr68_desc_ptr->phal2ppa.pha_metadata0 & 0x7)*64;
    __LOG_PARAM_FW(thr68_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + 40) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + 44) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + 48) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + 52) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + 56) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16)  );
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 0));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 4));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 8));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(68);
#endif
...
void PPA_FW(THR69_Erspan_TypeI_Ipv6)() {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr69_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6. */
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr69_desc_ptr->phal2ppa.pha_metadata0 & 0x7)*64;
    __LOG_PARAM_FW(thr69_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(69);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR70_radio_header
 * inparam      None
 * return       None
 * description  Proprietary Radio Header insertion. Radio header is a proprietary 4-byte header that needs to be inserted between the
 *              preamble and the first byte of the MAC DA of a standard Ethernet frame.
 *              Radio Header format (Network Order)
 ...
 *               	The frame length from the MAC DA to the FCS, including FCS
 *              Priority - 3 bits
 *              	CoS calculated by the SoC
 *              PTP - 1 bit
 *              	PTP - event packets, any supported encapsulation
 *              Reserved
 ...
 *              src & trg entries:  Not in use (4B)
 *
 *              hdr in:  | Extension space (64B) | MAC addresses(12B) | payload |
 *              hdr out: | Extension space (60B) | Radio Header (4B) | MAC addresses(12B) | payload |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmware adds the Radio Header and sets the values according to architect's definitions.
 *              - Packet length is copied from Desc<egress_byte_count> + 4 bytes for FCS
 *              - PTP is copied from Desc<is_ptp>
 *              - Priority is copied from Desc<tc>
 *              - Update Desc<timestamp_offset> +=4
 *              - Update Desc<udp_checksum_offset> +=4
 *              - Update Desc<fw_bc_modification> +=4
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR70_radio_header)() {
	...
	uint32_t  radio_header; /* data of radio header */
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr70_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    __LOG_FW(("Build and write radio header."));
     /* build radio header and write it */
     radio_header = ((thr70_desc_ptr->phal2ppa.egress_byte_count + 4) << 16) |
    		        (thr70_desc_ptr->phal2ppa.is_ptp << 11) |
    		        (thr70_desc_ptr->phal2ppa.tc << 8);
     PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 64 - 4, radio_header);
     __LOG_PARAM_FW(PPA_FW_SP_READ(AC5P_PKT_REGs_lo + 64 - 4));

     /* Update Byte Count and offsets for timestamping */
     __LOG_FW(("Update Byte Count and offsets for timestamping."));
     thr70_desc_ptr->phal2ppa.timestamp_offset += 4;
     thr70_desc_ptr->phal2ppa.udp_checksum_offset += 4;
     thr70_desc_ptr->ppa_internal_desc.fw_bc_modification = 4;
     __LOG_PARAM_FW(thr70_desc_ptr->phal2ppa.timestamp_offset);
     __LOG_PARAM_FW(thr70_desc_ptr->phal2ppa.udp_checksum_offset);
     __LOG_PARAM_FW(thr70_desc_ptr->ppa_internal_desc.fw_bc_modification);

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(70);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR81_Erspan_TypeIII_SameDevMirroring_IPv4
 * inparam      None
 * return       None
 * description  ERSPAN over IPv4 tunnel. Encapsulated Remote Switch Port Analyzer (ERSPAN)
 *              allows packets to be mirrored to a remote analyzer over a tunnel encapsulation.
 *              - ERSPAN Type III is used
 *              - ERSPAN is sent over an IPv4 GRE tunnel
 *              - Support handling of Ingress and Egress mirroring
 *              - Support handling vlan tagged or untagged
 *
 ...
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:            | Extension space (64B) | MAC Header(12B) | VLAN(4B) |
 *              hdr out with vlan: | Extension space (2B) | MAC Header(12B) | VLAN(4B) | Ethertype(2B) | IPv4(20B) | GRE Header(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | MAC Header(12B) |
 *              hdr out no vlan:   | Extension space (6B) | MAC Header(12B) | Ethertype(2B) | IPv4(20B) | GRE Header(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | MAC Header(12B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmwares actions:
 *                     - Move original MAC header to new offset
 *                     - Add 64 or 62 Bytes
 *                     - Copy L2 and ipv4 headers from shared memory calculate checksum and length for ipv1
 *                     - Set GRE, ERSPAN and Platform Specific sub header headers
 *              Note: Debug is available when ERSPAN_III_DEBUG is defined.
 *                    Shared memory Debug template values are available for DRAM_CC_ERSPAN_ANALYZER_EPORT_1_TEMPLATE_ADDR
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR81_Erspan_TypeIII_SameDevMirroring_IPv4)() {
    ...
    uint32_t    erspan_header_VER_to_C = 0x20000000;
    uint32_t    erspan_header_SGT_to_O = 0x00008005;
    uint32_t    erspan_HW_ID;
    uint32_t    pssh_header_platId_index = 0x14000000;
    uint32_t    ts_sec, ts_sec_new;
    uint32_t    ts_nano_sec;

    /* Get pointers to descriptor,cfg & packet */
    struct ppa_in_desc* thr81_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr81_desc_ptr->phal2ppa.rx_sniff;
    erspan_header_SGT_to_O |= (!descRxSniff) << 3;
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr81_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr81_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr81_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_FW(("copy pkt MAC addresses to new offset (same offset for both out_hdr types) "));
    ACCEL_CMD_TRIG(COPY_BYTES_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_PKT_mac_header__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR81_OUT_HDR_IPV4__VER_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR81_OUT_HDR_IPV4__VER_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR81_OUT_HDR_IPV4__VER_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR81_OUT_HDR_IPV4__VER_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR81_OUT_HDR_IPV4__VER_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 0));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 4));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 8));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    ...
    /* Get session ID from shared memory. This address contains  | reserved(6b) | HW_ID(6b, for type-3) | Ingress session ID(10b) | Egress session ID(10b) | */
    __LOG_FW(("Get session ID from shared memory. This address contains  | reserved(6b) | HW_ID(6b, for type-3) | Ingress session ID(10b) | Egress session ID(10b) |"));
    ...
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x08_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER
    ...
    /* Set IPv4 Header<Total Length>= Desc<Egress Byte Count>+44 */
    __LOG_FW(("Set IPv4 Header<Total Length>= Desc<Egress Byte Count> + 44"));

    /* First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b).
       Assumption 2 MS bits were already cleared by application when setting IPv4<Total length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv4_Header__total_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* IPv4 total Length +=44 */
    __LOG_FW(("IPv4 total Length += 44"));
    ACCEL_CMD_TRIG(ADD_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x2C_LEN16_TO_PKT_no_vlan__IPv4_Header__total_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Sum first 8 bytes of IPv4 */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_LEN8_FROM_PKT_IPv4_Header__version_OFFSET, 0);

    /* Skip cs field and add to sum 2B of ttl and protocol */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_LEN2_FROM_PKT_IPv4_Header__ttl_OFFSET, 0);

    /* Sum the last 8 bytes of IPv4 (src and dest IP addresses fields) */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_LEN8_FROM_PKT_IPv4_Header__sip_high_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* Store the new IPv4 CS */
    __LOG_FW(("Store the new IPv4 CS"));
    ACCEL_CMD_TRIG(CSUM_STORE_IP_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_TO_PKT_IPv4_Header__header_checksum_OFFSET, 0);

    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Set packet GRE header (8B) This address contains  | flags[2B] - 0x1000 | Protocol[2B] - (0x22EB) | Sequence number[4B] - Desc <LM_counter> |
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set packet GRE header (8B)"));

    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x10_LEN8_TO_PKT_no_vlan__GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x22_LEN8_TO_PKT_no_vlan__GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0xEB_LEN8_TO_PKT_no_vlan__GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number to Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_no_vlan__GRE_header__Seq_num_OFFSET, 0);

    /*-----------------------------------------------------------------------------------------------------------------------------------------------
     - Set packet ERSPAN header (12B)
     ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set packet ERSPAN header (12B)"));
    __LOG_FW(("Set ERSPAN.Ver to 0x2 and ERSPAN.C set rest fields to zero"));
    __LOG_FW(("Extract 10 bits of Ingress or Egress session ID according to Desc<rx_sniff> field"));
    erspan_header_VER_to_C |= (sessionId >> (10*descRxSniff)) & 0x3FF; /* set proper seesionId */
    __LOG_FW(("sessionId"));
    __LOG_PARAM_FW((sessionId >> (10*descRxSniff)) & 0x3FF);
    __LOG_FW(("erspan_header_VER_to_C"));
    __LOG_PARAM_FW(erspan_header_VER_to_C);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR81_OUT_HDR_ERSPAN__VER_OFFSET , erspan_header_VER_to_C);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Set ERSPAN.COS to DESC.qos_mapped_up "));
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_DESC_phal2ppa__qos_mapped_up_LEN3_TO_PKT_no_vlan__ERSPAN_type_III_header__COS_OFFSET, 0);

    __LOG_FW(("Set ERSPAN.COS to DESC.truncated"));
    ACCEL_CMD_TRIG(COPY_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_DESC_phal2ppa__truncated_LEN1_TO_PKT_no_vlan__ERSPAN_type_III_header__T_OFFSET, 0);

     /*------------------------------------------------------------------------------------------------------------------------------------------------
          - Handle Timestamp for ERSPAN and platform sub header
    ------------------------------------------------------------------------------------------------------------------------------------------------*/

    /* Read TOD word 1 which represents seconds field [31:0] */
    __LOG_FW(("Read TOD word 1 which represents seconds field [31:0]"));
    ts_sec = READ_TOD_IF_REG(TOD_WORD_1, thr81_desc_ptr->phal2ppa.ptp_tai_select);
    ...
    ts_nano_sec = READ_TOD_IF_REG(TOD_WORD_0, thr81_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_nano_sec);

    /* Check for wraparound */
    __LOG_FW(("Read again seconds (TOD word 1) to check for wraparound"));
    ts_sec_new = READ_TOD_IF_REG(TOD_WORD_1, thr81_desc_ptr->phal2ppa.ptp_tai_select);
    ...
    {
        /* Update ts_sec with new value since wraparound occurred */
        __LOG_FW(("Update ts_sec with new value since wraparound occurred"));
        ts_sec = ts_sec_new ;
        __LOG_PARAM_FW(ts_sec);
    }

    __LOG_FW(("Set ERSPAN.T to TOD nano seconds"));
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR81_OUT_HDR_ERSPAN__VER_OFFSET + 4 , ts_nano_sec);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Get HW_ID"));
    erspan_HW_ID = (sessionId >> 20) & 0x3F;
    __LOG_FW(("HW ID"));
    __LOG_PARAM_FW(erspan_HW_ID);
    __LOG_FW(("erspan_header_SGT_to_O"));
    erspan_header_SGT_to_O |= (erspan_HW_ID << 4);  /* set HW ID */
    __LOG_PARAM_FW(erspan_header_SGT_to_O);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR81_OUT_HDR_ERSPAN__VER_OFFSET + 8 , erspan_header_SGT_to_O);

    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Set packet Platform-specific_sub_header header (8B)
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set packet Platform-specific_sub_header (8B)"));
    /* Get Global parameters from shared memory. Should includes | reserved (6b)  | Switch_ID (10b) | VoQ (8b)  | reserved (8b) | */
    __LOG_FW(("Get Global parameters from shared memory. Type-III: | reserved (6b)  | Switch_ID (10b) | VoQ (8b)  | reserved (8b) |"));
    smemGlobalVal = PPA_FW_SP_READ(DRAM_CC_ERSPAN_GLOBAL_ADDR);
    __LOG_PARAM_FW(smemGlobalVal);
    pssh_header_platId_index |= smemGlobalVal & 0x03FF0000;
    __LOG_FW(("pssh_header_platId_index:"));
    __LOG_PARAM_FW(pssh_header_platId_index);
    __PRAGMA_NO_REORDER
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR81_OUT_HDR_ERSPAN__VER_OFFSET + 12 , pssh_header_platId_index);
    /* Set port Index according to RxSniff */
    __LOG_FW(("Set port Index according to RxSniff"
              "if descRxSniff = 0 trigger target port accelerator if 1 trigger source port accelerator (next command 4B offset)"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CFG_srcTrgPortEntry__targetPortEntry_LEN2_TO_PKT_no_vlan__ERSPAN_type_III_Platf_sub_hdr__Platf_id_PLUS2_OFFSET + 4*descRxSniff, 0);

    __LOG_FW(("pssh_Timestamp:"));
    __LOG_PARAM_FW(ts_sec);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR81_OUT_HDR_ERSPAN__VER_OFFSET + 16 , ts_sec);
    ...
    { /* no vlan */
        ...
        /* Desc<Egress Byte Count> +=58 */
        __LOG_FW(("Desc<Egress Byte Count> +=58"));
        ACCEL_CMD_TRIG(ADD_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x3A_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        /* Indicates HW about the new starting point of the packet header (+58 bytes). */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+58 bytes)."));
        thr81_desc_ptr->ppa_internal_desc.fw_bc_modification = 58;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header_outer__mac_da_47_32_OFFSET, 0);
    }
    else
    { /* with vlan */
        __LOG_FW(("with vlan"));
        /* Desc<Egress Byte Count> +=62 */
        __LOG_FW(("Desc<Egress Byte Count> +=62"));
        ACCEL_CMD_TRIG(ADD_BITS_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CONST_0x3E_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        /* Indicates HW about the new starting point of the packet header (+62 bytes). */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+62 bytes)."));
        thr81_desc_ptr->ppa_internal_desc.fw_bc_modification = 62;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR81_Erspan_TypeIII_SameDevMirroring_IPv4_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header_outer__mac_da_47_32_OFFSET, 0);
    }

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(81);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass
 * inparam      None
 * return       None
 * description  ERSPAN over IPv6 tunnel. Encapsulated Remote Switch Port Analyzer (ERSPAN)
 *              allows packets to be mirrored to a remote analyzer over a tunnel encapsulation.
 ...
 *              - ERSPAN is sent over an IPv4 GRE tunnel
 *              - Support handling of Ingress and Egress mirroring
 *              NOTE: since in case of IPv6 tunneling need to add more than 64 bytes and PHA fw thread can only add maximum 64 bytes.
 *                    Firstpass adds ERSPAN (20B) and GRE(8B) while the secondpass will add remainning fields like ethertype, ipv4, vlan and another L2 header.
 *
 *              cfg template:    Not in use (16B). It is used by fw to hold L2 parameters copied from shared memory.
 *              src & trg entries: PHA Target port table holds original packet target physical port for Egress mirroring (2B)
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:    | Extension space (32B) | MAC Header(12B) | TO_CPU eDSA(16B) |
 *              hdr out:   | Extension space (4B) | MAC Header(12B) | TO_CPU eDSA(16B) | GRE Header(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *                  - Move mac header and edsa tags to new offset
 *                  - Add and set ERSPAN, Platform_specific_sub_header and GRE headers
 *              Note: Debug is available when ERSPAN_III_DEBUG is defined.
 *                    Shared memory Debug template values are available for DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass)() {
    ...
    struct ppa_in_desc* thr82_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    #ifdef ERSPAN_III_DEBUG
        /* write edsa tags*/
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 44, 0x01234567 );
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 48, 0x89abcef0);
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 52, 0x01234567 );
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 56, 0x89abcef0);
    #endif

    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr82_desc_ptr->phal2ppa.rx_sniff;
    erspan_header_SGT_to_O |= (!descRxSniff << 3);
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr82_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr82_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr82_desc_ptr->phal2ppa.pha_metadata0 & 0x7);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_FW(("save MAC addresses[12B] + edsa tags[16B] to new offsets"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_PKT_mac_header__mac_da_47_32_LEN28_TO_PKT_mac_header__mac_da_47_32_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_CONST_0x10_LEN8_TO_PKT_GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_CONST_0x22_LEN8_TO_PKT_GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_CONST_0xEB_LEN8_TO_PKT_GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number to Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_GRE_header__Seq_num_OFFSET, 0);
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
     - Set packet ERSPAN header (12B)
     ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set packet ERSPAN header (12B)"));
    ...
    __LOG_FW(("erspan_header_VER_to_C"))
    __LOG_PARAM_FW(erspan_header_VER_to_C);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR82_OUT_HDR_ERSPAN__VER_OFFSET , erspan_header_VER_to_C);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Set ERSPAN.COS to DESC.qos_mapped_up "));
    ACCEL_CMD_TRIG(COPY_BITS_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_DESC_phal2ppa__qos_mapped_up_LEN3_TO_PKT_ERSPAN_type_III_header__COS_OFFSET, 0);

    __LOG_FW(("Set ERSPAN.COS to DESC.truncated"));
    ACCEL_CMD_TRIG(COPY_BITS_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_DESC_phal2ppa__truncated_LEN1_TO_PKT_ERSPAN_type_III_header__T_OFFSET, 0);
    ...
    /* Read TOD word 1 which represents seconds field [31:0] */
    __LOG_FW(("Read TOD word 1 which represents seconds field [31:0]"));
    ts_sec = READ_TOD_IF_REG(TOD_WORD_1, thr82_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_sec);
    __PRAGMA_NO_REORDER

    /* Read TOD word 0 which represents nano seconds field */
    __LOG_FW(("Read TOD word 0 which represents nano seconds field"));
    ts_nano_sec = READ_TOD_IF_REG(TOD_WORD_0, thr82_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_nano_sec);
    __PRAGMA_NO_REORDER

    /* Check for wraparound */
    __LOG_FW(("Read again seconds (TOD word 1) to check for wraparound"));
    ts_sec_new = READ_TOD_IF_REG(TOD_WORD_1, thr82_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_sec_new);
    /* < 500000000 (half a second) indicates that the nanosecond value is small, and there was a recent wraparound */
    __LOG_FW(("< 500000000 (half a second) indicates that the nanosecond value is small, and there was a recent wraparound"));
    if ( (ts_sec_new != ts_sec) && (ts_nano_sec < 500000000) )
    ...
    __LOG_FW(("Set ERSPAN.T to TOD nano seconds"));
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR82_OUT_HDR_ERSPAN__VER_OFFSET + 4 , ts_nano_sec);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Get HW_ID from shared memory"));
    erspan_HW_ID = (sessionId >> 20) & 0x3F;
    __LOG_FW(("erspan_HW_ID"));
    __LOG_PARAM_FW(erspan_HW_ID);
    erspan_header_SGT_to_O |= (erspan_HW_ID << 4);
    __LOG_FW(("erspan_header_SGT_to_O"));
    __LOG_PARAM_FW(erspan_header_SGT_to_O);
    __LOG_FW(("Set ERSPAN fiels: SGT:0x0 P:0x1 FT:0x0 hw_id:erspan_HW_ID D:descRxSniff Gra:0x2 O:0x1"));
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR82_OUT_HDR_ERSPAN__VER_OFFSET + 8 , erspan_header_SGT_to_O);
    ...
    __LOG_PARAM_FW(pssh_header_platId_index);
    __PRAGMA_NO_REORDER
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR82_OUT_HDR_ERSPAN__VER_OFFSET + 12 , pssh_header_platId_index);
    /* Set port Index according to RxSniff */
    __LOG_FW(("Set port Index according to RxSniff"
              "if descRxSniff = 0 trigger target port accelerator if 1 trigger source port accelerator (next command 4B offset)"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass_CFG_srcTrgPortEntry__targetPortEntry_LEN2_TO_PKT_ERSPAN_type_III_Platf_sub_hdr__Platf_id_PLUS2_OFFSET + 4*descRxSniff, 0);

    __LOG_FW(("pssh_Timestamp:"));
    __LOG_PARAM_FW(ts_sec);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR82_OUT_HDR_ERSPAN__VER_OFFSET + 16 , ts_sec);
    ...
    thr82_desc_ptr->ppa_internal_desc.fw_bc_modification = 28;

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(82);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass
 * inparam      None
 * return       None
 * description  ERSPAN over IPv6 tunnel. Encapsulated Remote Switch Port Analyzer (ERSPAN)
 *              allows packets to be mirrored to a remote analyzer over a tunnel encapsulation.
 ...
 *              cfg template: Not in use (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in:    | Extension space (64B) | MAC Header(12B) | VLAN Header(4B) | vlan_header(4B) | GRE Header(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out:   | Extension space (10 / 6B) | MAC Header(12B) | VLAN Header(4B)(optional) | Ether Type (2B) | IPv6 (40B) | GRE Header(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | Mac Header (12B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmwares actions:
 *                  - Move MAC header to new offset
 *                  - Add 58 or 54 bytes (mac + ipv6 + ethertype and an optional vlan)
 *                  - Set newly added values from shared memory reading or thread calculations.
 *                  - Add VLAN according to shared memory VLAN header
 *
 *              Note: Debug is available when ERSPAN_III_DEBUG is defined.
 *                    Debug is available for IPv6 in DRAM_CC_ERSPAN_ANALYZER_EPORT_3_TEMPLATE_ADDR
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass)() {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr83_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr83_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr83_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr83_desc_ptr->phal2ppa.pha_metadata0 & 0x7);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __PRAGMA_NO_REORDER
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
     - Move origin MAC addresses To Expension space, move ERSPAN and GRE to new position, move original mac from epension space to new out header offset.
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    /* Copy origin MAC addresses to temp location (same offset for both out headers)*/
    __LOG_FW(("Copy origin MAC addresses by 12 bytes to reserved_0"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_PKT_mac_header__mac_da_47_32_LEN12_TO_PKT_no_vlan__expansion_space__reserved_0_OFFSET, 0);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Copy GRE and ERSPAN to new location"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_PKT_GRE_header__C_LEN28_TO_PKT_no_vlan__GRE_header__C_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* Copy origin MAC addresses to final offset */
    __LOG_FW(("Copy origin MAC addresses from reserved_0 to 12 bytes to origin mac"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_PKT_expansion_space__reserved_0_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Get 40B of IPv6 header from shared memory and place them in output packet
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("IPv6 header in shared memory"));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 0));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 4));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 8));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    ...
    __LOG_FW(("Get 40B of IPv6 header from shared memory and place them in output packet"));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 20) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 20)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 24) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 24)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 28) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 28)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 32) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 32)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR83_OUT_HDR_NO_VLAN_IPV6__VERSION_OFFSET + 36) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 36)  );

    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Set Ether Type to 0x86dd (IPv6)
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set Ether Type to 0x86dd (IPv6)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CONST_0x86_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CONST_0xDD_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_PLUS8_OFFSET, 0);
    __PRAGMA_NO_REORDER
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Update packet IPv6 payload length field
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    /* First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b).
        Assumption 2 MS bits were already cleared by application when setting IPv6<Payload length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv6_Header__payload_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* IPv6 payload Length -8 */
    __LOG_FW(("IPv6 payload Length -=8"));
    ACCEL_CMD_TRIG(SUB_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CONST_0x8_LEN16_TO_PKT_no_vlan__IPv6_Header__payload_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    { /* No vlan */
        ...
        /* Indicates HW about the new starting point of the packet header (+46 bytes) */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+46 bytes)"));
        thr83_desc_ptr->ppa_internal_desc.fw_bc_modification = 46;

        /* No vlan, just copy MAC 12 bytes from thread's template */
        __LOG_FW(("No vlan, just copy MAC 12 bytes from thread's template"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header_outer__mac_da_47_32_OFFSET, 0);

        /* Desc<Egress Byte Count> +=46 */
        __LOG_FW(("Desc<Egress Byte Count> +=46"));
        ACCEL_CMD_TRIG(ADD_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CONST_0x2E_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);
    }
    else
    {/* With vlan */
        __LOG_FW(("With Vlan"));
        ...
        /* Indicates HW about the new starting point of the packet header (+50 bytes) */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+50 bytes)"));
        thr83_desc_ptr->ppa_internal_desc.fw_bc_modification = 50;

        /* With vlan, copy MAC(12B) + vlan tag(4B) from thread's template */
        __LOG_FW(("With vlan, copy MAC(12B) + vlan tag(4B) from thread's template"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header_outer__mac_da_47_32_OFFSET, 0);

        /* Desc<Egress Byte Count> +=50 */
        __LOG_FW(("Desc<Egress Byte Count> +=50"));
        ACCEL_CMD_TRIG(ADD_BITS_THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass_CONST_0x32_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(83);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR84_Erspan_TypeIII_OrigDevMirroring
 * inparam      None
 * return       None
 * description  Encapsulated Remote Switch Port Analyzer (ERSPAN)
 *              allows packets to be mirrored to a remote analyzer.
 *              - ERSPAN Type III is used
 *              - Support handling of Ingress mirroring
 *
 *              cfg template:      Not in use (16B). It is used by fw to hold L2 parameters copied from shared memory.
 *              src & trg entries: PHA Target port table holds original packet target physical port for Egress mirroring (2B)
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:  | Extension space (32B) | MAC Header(12B) | TO_CPU eDSA(16B) |
 *              hdr out: | Extension space (12B) | MAC Header(12B) | TO_CPU eDSA(16B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *                  - Add and set ERSPAN and Platform specific sub header headers.
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR84_Erspan_TypeIII_OrigDevMirroring)() {
    ...
    struct ppa_in_desc* thr84_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    #ifdef ERSPAN_III_DEBUG
        /* write edsa tags*/
        /* TagCommand:2: 0x0, SrcTrg_Tagged:1: 0x0, SrcTrg_Dev_4_0:5: 0x1, SrcTrg_Phy_Port_4_0:1: 0x4, RxSniff:1: 0x0, Reserved0:1: 0x1, CFI: 0x1, UP:3: 0x2, Extend0:1: 0x0, eVLAN_11_0:12: 0x567 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 44, 0x01234567 );
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 48, 0x89abcef0);
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 52, 0x01234567 );
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + 56, 0x89abcef0);
    ...
    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr84_desc_ptr->phal2ppa.rx_sniff;
    erspan_header_SGT_to_O |= (!descRxSniff << 3);
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr84_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr84_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr84_desc_ptr->phal2ppa.pha_metadata0 & 0x7);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_FW(("save MAC addresses[12B] + edsa tags[16B] to new offsets"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR84_Erspan_TypeIII_OrigDevMirroring_PKT_mac_header__mac_da_47_32_LEN28_TO_PKT_mac_header__mac_da_47_32_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* Get session ID from shared memory. This address contains  | reserved(6b) | HW_ID(6b, for type-3) | Ingress session ID(10b) | Egress session ID(10b) | */
    __LOG_FW(("Get session ID from shared memory. This address contains  | reserved(6b) | HW_ID(6b, for type-3) | Ingress session ID(10b) | Egress session ID(10b) |"));

    /* TODO: Verify which IP offset should be used */
    /* sessionId = PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  IPV6_BASIC_HEADER_SIZE); */
    sessionId = PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  IPV4_BASIC_HEADER_SIZE);
    __PRAGMA_NO_REORDER
    ...
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
     ...
     ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set packet ERSPAN header (12B)"));
    __LOG_FW(("Set ERSPAN.COS same as PKT.ToAnalyzer__UP "));
    erspan_header_VER_to_C |= (PPA_FW_SP_READ( AC5P_PKT_REGs_lo + THR84_OUT_HDR_EDSA_W0_TO_ANALYZER__VER_OFFSET) & 0xE000);
    __PRAGMA_NO_REORDER
    __LOG_FW(("Set ERSPAN.Ver = 0x2 and ERSPAN.C = session id, set other fields to zero"));
    __LOG_FW(("Extract 10 bits of Ingress or Egress session ID according to Desc<rx_sniff> field"));
    __LOG_PARAM_FW((sessionId >> (10*descRxSniff)) & 0x3FF);
    erspan_header_VER_to_C |= (sessionId >> (10*descRxSniff)) & 0x3FF; /* set proper seesionId */
    __LOG_FW(("erspan_header_VER_to_C"));
    __LOG_PARAM_FW(erspan_header_VER_to_C);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR84_OUT_HDR_ERSPAN__VER_OFFSET , erspan_header_VER_to_C);
    __PRAGMA_NO_REORDER

    __LOG_FW(("Set DESC.T = <DESC>truncated "));
    ACCEL_CMD_TRIG(COPY_BITS_THR84_Erspan_TypeIII_OrigDevMirroring_DESC_phal2ppa__truncated_LEN1_TO_PKT_ERSPAN_type_III_header__T_OFFSET, 0);
    ...
    /* Read TOD word 1 which represents seconds field [31:0] */
    __LOG_FW(("Read TOD word 1 which represents seconds field [31:0]"));
    ts_sec = READ_TOD_IF_REG(TOD_WORD_1, thr84_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_sec);
    __PRAGMA_NO_REORDER

    /* Read TOD word 0 which represents nano seconds field */
    __LOG_FW(("Read TOD word 0 which represents nano seconds field"));
    ts_nano_sec = READ_TOD_IF_REG(TOD_WORD_0, thr84_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_nano_sec);
    __PRAGMA_NO_REORDER

    /* Check for wraparound */
    __LOG_FW(("Read again seconds (TOD word 1) to check for wraparound"));
    ts_sec_new = READ_TOD_IF_REG(TOD_WORD_1, thr84_desc_ptr->phal2ppa.ptp_tai_select);
    __LOG_PARAM_FW(ts_sec_new);
    /* < 500000000 (half a second) indicates that the nanosecond value is small, and there was a recent wraparound */
    __LOG_FW(("< 500000000 (half a second) indicates that the nanosecond value is small, and there was a recent wraparound"));
    if ( (ts_sec_new != ts_sec) && (ts_nano_sec < 500000000) )
    ...
    __LOG_FW(("Set ERSPAN.T to TOD nano seconds"));
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR84_OUT_HDR_ERSPAN__VER_OFFSET + 4 , ts_nano_sec);
    __PRAGMA_NO_REORDER
    ...
    __LOG_PARAM_FW(erspan_header_SGT_to_O);
    __LOG_FW(("Set ERSPAN fiels: SGT:0x0 P:0x1 FT:0x0 hw_id:erspan_HW_ID D:descRxSniff Gra:0x2 O:0x1"));
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR84_OUT_HDR_ERSPAN__VER_OFFSET + 8 , erspan_header_SGT_to_O);
    ...
    __LOG_PARAM_FW(pssh_header_platId_index);
    __PRAGMA_NO_REORDER
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR84_OUT_HDR_ERSPAN__VER_OFFSET + 12 , pssh_header_platId_index);
    /* Set port Index according to RxSniff */
    __LOG_FW(("Set port Index according to RxSniff"
              "if descRxSniff = 0 trigger target port accelerator if 1 trigger source port accelerator (next command 4B offset)"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR84_Erspan_TypeIII_OrigDevMirroring_CFG_srcTrgPortEntry__targetPortEntry_LEN2_TO_PKT_ERSPAN_type_III_Platf_sub_hdr__Platf_id_PLUS2_OFFSET + 4*descRxSniff, 0);

    __LOG_FW(("pssh_Timestamp:"));
    __LOG_PARAM_FW(ts_sec);
    PPA_FW_SP_WRITE( AC5P_PKT_REGs_lo + THR84_OUT_HDR_ERSPAN__VER_OFFSET + 16 , ts_sec);

    /* Indicates HW about the new starting point of the packet header (+20 bytes). */
    __LOG_FW(("Indicates HW about the new starting point of the packet header (+20 bytes)."));
    thr84_desc_ptr->ppa_internal_desc.fw_bc_modification = 20;


#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(84);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged
 * inparam      None
 * return       None
 * description  ERSPAN Type III for CC (Centralized Chassis) solution
 *              - ERSPAN is sent over an IPv4 GRE tunnel
 *              - Support handling of Ingress and Egress mirroring
 *              - Support handling vlan tagged
 *
 *              cfg template:    Not in use (16B). It is used by fw to hold L2 parameters copied from shared memory.
 *              src & trg entries: PHA Target port table holds original packet target physical port for Egress mirroring (2B)
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:            | Extension space (32B)| MAC Header(12B) | VLAN(4B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out with vlan: | Extension space (2B) | MAC Header(12B) | VLAN(4B) | Ethertype (2B) | IPv4(20B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out no vlan:   | Extension space (6B) | MAC Header(12B) | Ethertype (2B) | IPv4(20B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *                - Add and set Ethertype, VLAN(optional), IPv4 and GRE.
 *                - Set IPv4 Header<Total Length>= Desc<Egress Byte Count>+36
 *	              - Use the Checksum accelerator to calculate IPv4 Header<Checksum>
 *	              - If Thread Template<VLAN>=0xFFFF then shift by 4B the MAC DA and SA of the header
 *	              - Update Desc<Egress Byte Count> // The updated value depends on if VLAN was added
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged)() {
    ...
    /* Get pointers to descriptor,cfg & packet */
    struct ppa_in_desc* thr85_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr85_desc_ptr->phal2ppa.rx_sniff;
    __LOG_PARAM_FW(descRxSniff);

    #ifdef ERSPAN_III_DEBUG
        /* ERSPAN header */
        /* Ver[4b] - 0x2 | VLAN[12b] - 0x0 | COS[3b] - 0x2 | BSO[2b] - 0x0 | T[1b] - 0| c(sessionId)[10b] - 0xf8  */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR85_IN_HDR_ERSPAN__VER_OFFSET ,  (descRxSniff == 1) ? 0x200040f8: 0x2000401f);
        /* TimeStamp[4B] - 0xb6b6b6b6 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR85_IN_HDR_ERSPAN__VER_OFFSET + 4, 0xb6b6b6b6);
        /* SGT[2B] - 0x0 | P[1b] - 0x1 | FT[5b] - 0x0 | HW ID[6b] - 0x3f | D[1b] - 0 | GRA[2b] - 0x2 | O[1b] - 0x1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR85_IN_HDR_ERSPAN__VER_OFFSET + 8, (descRxSniff == 1) ? 0x000083f5 : 0x000083fa);
        /* Platform specific sub header */
        /* Platf_ID[6b] - 0x5 | Switch_ID[10b] - 0x3ff | Port_ID[2B] - 0xcafe */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR85_IN_HDR_ERSPAN__VER_OFFSET + 12, (descRxSniff == 1) ? 0x17ffcafe : 0x17ffbabe);
        /* TimeStamp[4B] - 0xa1a1a1a1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR85_IN_HDR_ERSPAN__VER_OFFSET + 16, 0xa1a1a1a1);
    #endif
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr85_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr85_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr85_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
      ...
      ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Copy 20B of IPv4 header from shared memory into packet output"));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR85_OUT_HDR_IPV4__VER_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR85_OUT_HDR_IPV4__VER_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR85_OUT_HDR_IPV4__VER_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR85_OUT_HDR_IPV4__VER_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR85_OUT_HDR_IPV4__VER_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 0));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 4));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 8));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    ...
    /* EthType 0x0800. Set 0x08, LSB 0x00 is cleared by SET_REMAIN_FIELDS_TO_ZERO data */
    __LOG_FW(("EthType 0x0800. Set 0x08, LSB 0x00 is cleared by SET_REMAIN_FIELDS_TO_ZERO data "));
    ACCEL_CMD_TRIG(COPY_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x08_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER
    ...
    /* First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b).
       Assumption 2 MS bits were already cleared by application when setting IPv4<Total length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv4_Header__total_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    __LOG_FW(("IPv4 total Length += 36"));
    ACCEL_CMD_TRIG(ADD_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x24_LEN16_TO_PKT_no_vlan__IPv4_Header__total_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Sum first 8 bytes of IPv4 */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_LEN8_FROM_PKT_IPv4_Header__version_OFFSET, 0);

    /* Skip cs field and add to sum 2B of ttl and protocol */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_LEN2_FROM_PKT_IPv4_Header__ttl_OFFSET, 0);

    /* Sum the last 8 bytes of IPv4 (src and dest IP addresses fields) */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_LEN8_FROM_PKT_IPv4_Header__sip_high_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* Store the new IPv4 CS */
    __LOG_FW(("Store the new IPv4 CS"));
    ACCEL_CMD_TRIG(CSUM_STORE_IP_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_TO_PKT_IPv4_Header__header_checksum_OFFSET, 0);
    ...
    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x10_LEN8_TO_PKT_no_vlan__GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x22_LEN8_TO_PKT_no_vlan__GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0xEB_LEN8_TO_PKT_no_vlan__GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number to Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_no_vlan__GRE_header__Seq_num_OFFSET, 0);
    ...
    { /* no vlan */
        ...
        /* Desc<Egress Byte Count> +=26 */
        __LOG_FW(("Desc<Egress Byte Count> +=26"));
        ACCEL_CMD_TRIG(ADD_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x1A_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        /* Indicates HW about the new starting point of the packet header (+26 bytes). */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+26 bytes)."));
        thr85_desc_ptr->ppa_internal_desc.fw_bc_modification = 26;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    else
    { /* with vlan */
        __LOG_FW(("with vlan"));
        /* Desc<Egress Byte Count> +=30 */
        __LOG_FW(("Desc<Egress Byte Count> +=30"));
        ACCEL_CMD_TRIG(ADD_BITS_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CONST_0x1E_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        /* Indicates HW about the new starting point of the packet header (+30 bytes). */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+30 bytes)."));
        thr85_desc_ptr->ppa_internal_desc.fw_bc_modification = 30;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(85);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged
 * inparam      None
 * return       None
 * description  ERSPAN Type III for CC (Centralized Chassis) solution
 *              - ERSPAN is sent over an IPv4 GRE tunnel
 *              - Support handling of Ingress and Egress mirroring
 *              - Support handling vlan untagged
 *
 *              cfg template: Not in use (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in:             | Extension space (48B) | MAC Header(12B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | Payload(16B) |
 *              hdr out with vlan:  | Extension space (14B) | MAC Header(12B) | VLAN(4B) | Ethertype (2B) | IPv4(20B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | Payload(16B) |
 *              hdr out no vlan:    | Extension space (18B) | MAC Header(12B) | Ethertype (2B) | IPv4(20B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) | Payload(16B) | 
 *              Header Window anchor= Outer Layer 2
...
void PPA_FW(THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged)() {
    ...
    struct ppa_in_desc* thr86_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr86_desc_ptr->phal2ppa.rx_sniff;
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr86_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr86_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr86_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_PARAM_FW(smemAnalyzerTemplateBaseAddr);
        ...
        /* ERSPAN header */
        /* Ver[4b] - 0x2 | VLAN[12b] - 0x0 | COS[3b] - 0x2 | BSO[2b] - 0x0 | T[1b] - 0| c(sessionId)[10b] - 0xf8  */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR86_IN_HDR_ERSPAN__VER_OFFSET ,  (descRxSniff == 1) ? 0x200040f8: 0x2000401f);
        /* TimeStamp[4B] - 0xb6b6b6b6 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR86_IN_HDR_ERSPAN__VER_OFFSET + 4, 0xb6b6b6b6);
        /* SGT[2B] - 0x0 | P[1b] - 0x1 | FT[5b] - 0x0 | HW ID[6b] - 0x3f | D[1b] - 0 | GRA[2b] - 0x2 | O[1b] - 0x1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR86_IN_HDR_ERSPAN__VER_OFFSET + 8, (descRxSniff == 1) ? 0x000083f5 : 0x000083fa);
        /* Platform specific sub header */
        /* Platf_ID[6b] - 0x5 | Switch_ID[10b] - 0x3ff | Port_ID[2B] - 0xcafe */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR86_IN_HDR_ERSPAN__VER_OFFSET + 12, (descRxSniff == 1) ? 0x17ffcafe : 0x17ffbabe);
        /* TimeStamp[4B] - 0xa1a1a1a1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR86_IN_HDR_ERSPAN__VER_OFFSET + 16, 0xa1a1a1a1);
    #endif
    ...
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
      ...
      ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Copy 20B of IPv4 header from shared memory into packet output"));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR86_OUT_HDR_IPV4__VER_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR86_OUT_HDR_IPV4__VER_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR86_OUT_HDR_IPV4__VER_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR86_OUT_HDR_IPV4__VER_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR86_OUT_HDR_IPV4__VER_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 0));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 4));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 8));
    __LOG_PARAM_FW(PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12));
    ...
    /* EthType 0x0800. Set 0x08, LSB 0x00 is cleared by SET_REMAIN_FIELDS_TO_ZERO data */
    __LOG_FW(("EthType 0x0800. Set 0x08, LSB 0x00 is cleared by SET_REMAIN_FIELDS_TO_ZERO data "));
    ACCEL_CMD_TRIG(COPY_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x08_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER
    ...
    /* First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b).
       Assumption 2 MS bits were already cleared by application when setting IPv4<Total length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv4.Total Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv4_Header__total_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* IPv4 total Length +=36 */
    __LOG_FW(("IPv4 total Length += 36"));
    ACCEL_CMD_TRIG(ADD_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x24_LEN16_TO_PKT_no_vlan__IPv4_Header__total_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Sum first 8 bytes of IPv4 */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_LEN8_FROM_PKT_IPv4_Header__version_OFFSET, 0);

    /* Skip cs field and add to sum 2B of ttl and protocol */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_LEN2_FROM_PKT_IPv4_Header__ttl_OFFSET, 0);

    /* Sum the last 8 bytes of IPv4 (src and dest IP addresses fields) */
    __LOG_FW(("Sum new Total Length"));
    ACCEL_CMD_TRIG(CSUM_LOAD_NEW_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_LEN8_FROM_PKT_IPv4_Header__sip_high_OFFSET, 0);
    __PRAGMA_NO_REORDER

    /* Store the new IPv4 CS */
    __LOG_FW(("Store the new IPv4 CS"));
    ACCEL_CMD_TRIG(CSUM_STORE_IP_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_TO_PKT_IPv4_Header__header_checksum_OFFSET, 0);
    ...
    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x10_LEN8_TO_PKT_no_vlan__GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x22_LEN8_TO_PKT_no_vlan__GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0xEB_LEN8_TO_PKT_no_vlan__GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number as Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_no_vlan__GRE_header__Seq_num_OFFSET, 0);
    ...
    { /* no vlan */
        ...
        /* Desc<Egress Byte Count> +=30 */
        __LOG_FW(("Desc<Egress Byte Count> +=30"));
        ACCEL_CMD_TRIG(ADD_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x1E_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        /* Indicates HW about the new starting point of the packet header (+30 bytes). */
        __LOG_FW(("Indicates HW about the new starting point of the packet header (+30 bytes)."));
        thr86_desc_ptr->ppa_internal_desc.fw_bc_modification = 30;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    else
    { /* with vlan */
        __LOG_FW(("with vlan"));
        ...
        ACCEL_CMD_TRIG(ADD_BITS_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CONST_0x22_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);
        ...
        thr86_desc_ptr->ppa_internal_desc.fw_bc_modification = 34;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(86);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged
 * inparam      None
 * return       None
 * description  ERSPAN Type III for CC (Centralized Chassis) solution
 *              - ERSPAN is sent over an IPv6 GRE tunnel
 ...
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:            | Extension space (64B)| MAC Header(12B) | VLAN(4B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out with vlan: | Extension space (14B) | MAC Header(12B) | VLAN(4B) | Ethertype (2B) | IPv6(40B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out no vlan:   | Extension space (18B) | MAC Header(12B) | Ethertype (2B) | IPv6(40B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmwares actions:
 *                - Add and set Ethertype, VLAN(optional), IPv6 and GRE.
 *                - Set IPv6 Header<Total Length>= Desc<Egress Byte Count>+16
 *                - If Thread Template<VLAN>=0xFFFF then shift by 4B the MAC DA and SA of the header
 *                - Update Desc<Egress Byte Count> // The updated value depends on if VLAN was added
 *
 *
...
void PPA_FW(THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged)() {
    ...
    /* Get pointers to descriptor,cfg & packet */
    struct ppa_in_desc* thr87_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);

    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr87_desc_ptr->phal2ppa.rx_sniff;
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr87_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr87_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr87_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_PARAM_FW(smemAnalyzerTemplateBaseAddr);
        ...
        /* ERSPAN header */
        /* Ver[4b] - 0x2 | VLAN[12b] - 0x0 | COS[3b] - 0x2 | BSO[2b] - 0x0 | T[1b] - 0| c(sessionId)[10b] - 0xf8  */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR87_IN_HDR_ERSPAN__VER_OFFSET ,  (descRxSniff == 1) ? 0x200040f8: 0x2000401f);
        /* TimeStamp[4B] - 0xb6b6b6b6 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR87_IN_HDR_ERSPAN__VER_OFFSET + 4, 0xb6b6b6b6);
        /* SGT[2B] - 0x0 | P[1b] - 0x1 | FT[5b] - 0x0 | HW ID[6b] - 0x3f | D[1b] - 0 | GRA[2b] - 0x2 | O[1b] - 0x1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR87_IN_HDR_ERSPAN__VER_OFFSET + 8, (descRxSniff == 1) ? 0x000083f5 : 0x000083fa);
        /* Platform specific sub header */
        /* Platf_ID[6b] - 0x5 | Switch_ID[10b] - 0x3ff | Port_ID[2B] - 0xcafe */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR87_IN_HDR_ERSPAN__VER_OFFSET + 12, (descRxSniff == 1) ? 0x17ffcafe : 0x17ffbabe);
        /* TimeStamp[4B] - 0xa1a1a1a1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR87_IN_HDR_ERSPAN__VER_OFFSET + 16, 0xa1a1a1a1);
    #endif
    ...
    __LOG_FW(("Get 40B of IPv6 header from shared memory and place them in output packet"));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 20) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 20)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 24) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 24)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 28) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 28)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 32) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 32)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR87_OUT_HDR_IPV6__VER_OFFSET + 36) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 36)  );
    ...
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set Ether Type to 0x86dd (IPv6)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x86_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0xDD_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_PLUS8_OFFSET, 0);
    __PRAGMA_NO_REORDER
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Update packet IPv6 payload length field
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    /* First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b).
        Assumption 2 MS bits were already cleared by application when setting IPv6<Payload length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv6_Header__payload_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    __LOG_FW(("IPv6 payload Length +=16"));
    ACCEL_CMD_TRIG(ADD_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x10_LEN16_TO_PKT_no_vlan__IPv6_Header__payload_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x10_LEN8_TO_PKT_no_vlan__GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x22_LEN8_TO_PKT_no_vlan__GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0xEB_LEN8_TO_PKT_no_vlan__GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number to Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_no_vlan__GRE_header__Seq_num_OFFSET, 0);

    /* TODO: verify that vlanVid should be compared to 0 same as pizza box solution or as defined to 0xFFFF */
     if(vlanVid == 0)
    { /* no vlan */
        __LOG_FW(("no vlan"));

        __LOG_FW(("Desc<Egress Byte Count> +=46"));
        ACCEL_CMD_TRIG(ADD_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x2E_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        __LOG_FW(("Indicates HW about the new starting point of the packet header (+46 bytes)."));
        thr87_desc_ptr->ppa_internal_desc.fw_bc_modification = 46;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    else
    { /* with vlan */
        __LOG_FW(("with vlan"));
        __LOG_FW(("Desc<Egress Byte Count> +=50"));
        ACCEL_CMD_TRIG(ADD_BITS_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CONST_0x32_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        __LOG_FW(("Indicates HW about the new starting point of the packet header (+50 bytes)."));
        thr87_desc_ptr->ppa_internal_desc.fw_bc_modification = 50;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(87);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged
 * inparam      None
 * return       None
 * description  ERSPAN Type III for CC (Centralized Chassis) solution
 *              - ERSPAN is sent over an IPv6 GRE tunnel
 ...
 *                                 PHA Source port table holds original packet source physical port for Ingress mirroring (2B)
 *
 *              hdr in:            | Extension space (64B)| MAC Header(12B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out with vlan: | Extension space (10B) | MAC Header(12B) | VLAN(4B) | Ethertype (2B) | IPv6(40B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              hdr out no vlan:   | Extension space (14B) | MAC Header(12B) | Ethertype (2B) | IPv6(40B) | GRE(8B) | ERSPAN Type III Header(12B) | Platform Specific Sub Header (8B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmwares actions:
...
void PPA_FW(THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged)() {
    ...
    /* Get pointers to descriptor,cfg & packet */
    struct ppa_in_desc* thr88_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    /* Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring */
    __LOG_FW(("Get Rx Sniff value from descriptor to tell us if Ingress mirroring or Egress mirroring"));
    descRxSniff = thr88_desc_ptr->phal2ppa.rx_sniff;
    __LOG_PARAM_FW(descRxSniff);
    ...
    /* Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6.*/
    __LOG_FW(("Desc.pha_metadata[0:2] contains shared memory index. Applicable values are 0:6."));
    smemAnalayzerTemplateOfst = (thr88_desc_ptr->phal2ppa.pha_metadata0 & 0x7) * 64;
    __LOG_FW(("thr88_desc_ptr->phal2ppa.pha_metadata0"));
    __LOG_PARAM_FW(thr88_desc_ptr->phal2ppa.pha_metadata0);
    __LOG_FW(("smemAnalayzerTemplateOfst"));
    __LOG_PARAM_FW(smemAnalayzerTemplateOfst);
    ...
    __LOG_PARAM_FW(smemAnalyzerTemplateBaseAddr);
        ...
        /* ERSPAN header */
        /* Ver[4b] - 0x2 | VLAN[12b] - 0x0 | COS[3b] - 0x2 | BSO[2b] - 0x0 | T[1b] - 0| c(sessionId)[10b] - 0xf8  */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR88_IN_HDR_ERSPAN__VER_OFFSET ,  (descRxSniff == 1) ? 0x200040f8: 0x2000401f);
        /* TimeStamp[4B] - 0xb6b6b6b6 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR88_IN_HDR_ERSPAN__VER_OFFSET + 4, 0xb6b6b6b6);
        /* SGT[2B] - 0x0 | P[1b] - 0x1 | FT[5b] - 0x0 | HW ID[6b] - 0x3f | D[1b] - 0 | GRA[2b] - 0x2 | O[1b] - 0x1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR88_IN_HDR_ERSPAN__VER_OFFSET + 8, (descRxSniff == 1) ? 0x000083f5 : 0x000083fa);
        /* Platform specific sub header */
        /* Platf_ID[6b] - 0x5 | Switch_ID[10b] - 0x3ff | Port_ID[2B] - 0xcafe */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR88_IN_HDR_ERSPAN__VER_OFFSET + 12, (descRxSniff == 1) ? 0x17ffcafe : 0x17ffbabe);
        /* TimeStamp[4B] - 0xa1a1a1a1 */
        PPA_FW_SP_WRITE(AC5P_PKT_REGs_lo + THR88_IN_HDR_ERSPAN__VER_OFFSET + 16, 0xa1a1a1a1);
    #endif
    ...
    __LOG_FW(("Get 40B of IPv6 header from shared memory and place them in output packet"));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  0));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 4) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  4));
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 8) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr +  8)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 12) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 12)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 16) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 16)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 20) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 20)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 24) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 24)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 28) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 28)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 32) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 32)  );
    PPA_FW_SP_WRITE( (AC5P_PKT_REGs_lo + THR88_OUT_HDR_IPV6__VER_OFFSET + 36) , PPA_FW_SP_READ(smemAnalyzerTemplateBaseAddr + 36)  );
    ...
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    __LOG_FW(("Set Ether Type to 0x86dd (IPv6)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x86_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0xDD_LEN8_TO_PKT_no_vlan__ethertype_header__ethertype_PLUS8_OFFSET, 0);
    __PRAGMA_NO_REORDER
    /*-----------------------------------------------------------------------------------------------------------------------------------------------
    - Update packet IPv6 payload length field
    ------------------------------------------------------------------------------------------------------------------------------------------------*/
    ...
    /* First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b).
        Assumption 2 MS bits were already cleared by application when setting IPv6<Payload length> field in shared memory */
    __LOG_FW(("First copy Desc.Egress Byte Count(14b) to IPv6.Payload Length(16b)"));
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_DESC_phal2ppa__egress_byte_count_LEN14_TO_PKT_no_vlan__IPv6_Header__payload_length_PLUS2_OFFSET, 0);
    __PRAGMA_NO_REORDER

    __LOG_FW(("IPv6 payload Length +=16"));
    ACCEL_CMD_TRIG(ADD_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x10_LEN16_TO_PKT_no_vlan__IPv6_Header__payload_length_OFFSET, 0);
    __PRAGMA_NO_REORDER
    ...
    /* Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value */
    __LOG_FW(("Set GRE.flags to 0x1000. Clear all other bits in range of 32 bits to zero to get 0x1000 value"));
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x10_LEN8_TO_PKT_no_vlan__GRE_header__C_OFFSET, SET_REMAIN_FIELDS_TO_ZERO);
    __PRAGMA_NO_REORDER

    /* Set GRE.Protocol to 0x22BE */
    __LOG_FW(("Set GRE.Protocol to 0x22EB"));
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x22_LEN8_TO_PKT_no_vlan__GRE_header__NP_OFFSET, 0);
    ACCEL_CMD_TRIG(COPY_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0xEB_LEN8_TO_PKT_no_vlan__GRE_header__NP_PLUS8_OFFSET, 0);

    /* Set GRE.Sequence Number to Desc<LM Counter> */
    __LOG_FW(("Set GRE.Sequence Number to Desc<LM Counter>"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_DESC_phal2ppa__lm_counter_LEN4_TO_PKT_no_vlan__GRE_header__Seq_num_OFFSET, 0);
    ...
    { /* no vlan */
        __LOG_FW(("no vlan"));
        __LOG_FW(("Desc<Egress Byte Count> +=50"));
        ACCEL_CMD_TRIG(ADD_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x32_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        __LOG_FW(("Indicates HW about the new starting point of the packet header (+50 bytes)."));
        thr88_desc_ptr->ppa_internal_desc.fw_bc_modification = 50;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CFG_erspan_template__mac_da_47_32_LEN12_TO_PKT_no_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    else
    { /* with vlan */
        __LOG_FW(("with vlan"));
        __LOG_FW(("Desc<Egress Byte Count> +=54"));
        ACCEL_CMD_TRIG(ADD_BITS_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CONST_0x36_LEN14_TO_DESC_phal2ppa__egress_byte_count_OFFSET, 0);

        __LOG_FW(("Indicates HW about the new starting point of the packet header (+54 bytes)."));
        thr88_desc_ptr->ppa_internal_desc.fw_bc_modification = 54;

        __LOG_FW(("Set tunnel MAC addresses + vlan tag from template (copied earlier from shared memory to template)"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged_CFG_erspan_template__mac_da_47_32_LEN16_TO_PKT_with_vlan__mac_header__mac_da_47_32_OFFSET, 0);
    }
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(88);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     ENABLE_THR93_MPLS_LSR_In_Stacking_System_Thread
 * inparam      None
 * return       None
 * description  overwrite the eDSA<Trg ePort> with the desc<trg_phy_port>, so this prevents the Trg ePort
 *              attributes from being applied again on the egress device.
 *
 *              cfg template:   HA_Table_reserved_space.reserved_0[9:0]: own hardware device number.
 *                              HA_Table_reserved_space.reserved_0[10]: odd - normal forwarding, even - MPLS LSR.
 *              src & trg entries: Not in use (2B)
 *                                 Not in use (2B)
 *
 *              hdr in:     | Extension space (32B) | MAC Header(12B) | eDSA_fwd_w0(4B) | eDSA_fwd_w1(4B) | eDSA_fwd_w2(4B) | eDSA_fwd_w3(4B) |
 *              hdr out:    | Extension space (32B) | MAC Header(12B) | eDSA_fwd_w0(4B) | eDSA_fwd_w1(4B) | eDSA_fwd_w2(4B) | eDSA_fwd_w3(4B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *               eDSA<Trg_ePort> = desc<trg_phy_port>
 *               eDSA<TrgDev> = desc<trg_dev> ^ template0[10]
 *
 *
 *
 *****************************************************************************************************************************************/
...
void PPA_FW(THR93_MPLS_LSR_In_Stacking_System_Thread)() {
    ...
    /* Get pointer to descriptor */
    struct thr93_MPLS_LSR_In_Stacking_System_Thread_in_hdr* thr93_in_hdr_ptr = AC5P_MEM_CAST(AC5P_PKT_REGs_lo,thr93_MPLS_LSR_In_Stacking_System_Thread_in_hdr);
    struct ppa_in_desc* desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    struct thr93_MPLS_LSR_In_Stacking_System_Thread_cfg* thr93_cfg_ptr = AC5P_MEM_CAST(AC5P_CFG_REGs_lo,thr93_MPLS_LSR_In_Stacking_System_Thread_cfg);
    uint32_t self_hw_dev_id = thr93_cfg_ptr->mpls_lsr_stacking_system_template.self_hw_dev_id;
    uint32_t dev_map_table_select_bit = thr93_cfg_ptr->mpls_lsr_stacking_system_template.dev_map_table_select_bit;

    __LOG_FW(("Read mpls_lsr_stacking_system_template.self_hw_dev_id"));
    __LOG_PARAM_FW(self_hw_dev_id);

    if ( (desc_ptr->phal2ppa.outgoing_mtag_cmd == OUTGOING_MTAG_FORWARD_CMD_CNS) && (desc_ptr->phal2ppa.trg_dev != (self_hw_dev_id & 0x3ff))
        && thr93_in_hdr_ptr->eDSA_fwd_w1.Routed && (desc_ptr->phal2ppa.use_vidx == 0)
        && desc_ptr->phal2ppa.is_trg_phy_port_valid)
        {
           __LOG_FW(("thr93_in_hdr_ptr->eDSA_fwd_w3.Trg_ePort_15_0 [16-bits]= desc_ptr->phal2ppa.trg_phy_port [10-bits]"));
           thr93_in_hdr_ptr->eDSA_fwd_w3.Trg_ePort_15_0 = desc_ptr->phal2ppa.trg_phy_port;
           __LOG_FW(("thr93_in_hdr_ptr->eDSA_fwd_w1.TrgDev_4_0 = (desc_ptr->phal2ppa.trg_dev & 0x1F) ^ dev_map_table_select_bit"));
           thr93_in_hdr_ptr->eDSA_fwd_w1.TrgDev_4_0 = (desc_ptr->phal2ppa.trg_dev & 0x1F) ^ dev_map_table_select_bit;
           __LOG_FW(("thr93_in_hdr_ptr->eDSA_fwd_w3.TrgDev_9_5 = ((desc_ptr->phal2ppa.trg_dev >> 5) & 0x1f)"));
           thr93_in_hdr_ptr->eDSA_fwd_w3.TrgDev_9_5 = ((desc_ptr->phal2ppa.trg_dev >> 5) & 0x1f);
        }

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(93);
#endif
...
THREAD_TYPE phaThreadsTypeAc5pDefault[] = {
    ...
    /*81*/ ,PPA_FW(THR81_Erspan_TypeIII_SameDevMirroring_IPv4)
    /*82*/ ,PPA_FW(THR82_Erspan_TypeIII_SameDevMirroring_IPv6_FirstPass)
    /*83*/ ,PPA_FW(THR83_Erspan_TypeIII_SameDevMirroring_IPv6_SecondPass)
    /*84*/ ,PPA_FW(THR84_Erspan_TypeIII_OrigDevMirroring)
    /*85*/ ,PPA_FW(THR85_Erspan_TypeIII_TrgDevMirroring_IPv4_Tagged)
    /*86*/ ,PPA_FW(THR86_Erspan_TypeIII_TrgDevMirroring_IPv4_Untagged)
    /*87*/ ,PPA_FW(THR87_Erspan_TypeIII_TrgDevMirroring_IPv6_Tagged)
    /*88*/ ,PPA_FW(THR88_Erspan_TypeIII_TrgDevMirroring_IPv6_Untagged)
    /*89*/ ,invalidFirmwareThread
    /*90*/ ,invalidFirmwareThread
    /*91*/ ,invalidFirmwareThread
    /*92*/ ,invalidFirmwareThread
    /*93*/ ,PPA_FW(THR93_MPLS_LSR_In_Stacking_System_Thread)
