...
#include "asicSimulation/SKernel/sEmbeddedCpu/firmware/ac5p_pha/Image02/ppa_fw_threads_defs.h"
...
void PPA_FW(THR54_IPv4_TTL_Increment) () {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr54_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(54);
#endif
...
void PPA_FW(THR55_IPv6_HopLimit_Increment) () {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr55_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(55);
#endif
 ...
 *
 *              cfg template: HA_Table_reserved_space.reserved_0 determines old or new behavior. (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in:    | Extension space (32B) | MAC Header(12B) | TO_CPU eDSA(16B) | VLAN_Header (4B) |
 *              hdr out:   | Extension space (32B) | MAC Header(12B) | TO_CPU eDSA(16B) | VLAN_Header (4B) |
 *
 *              Header Window anchor= Outer Layer 2
 ...
 *              - IF config.reserved_0 is 0 then, Old Behavior is selected. Else ,New.
 *                  Old:
 *                  - Set Desc<Flow-ID> = InDesc<orig src eport>
 *                  New:
 *                  - IF InDesc<force_new_dsa_tag>==0 THEN
 *                      OutDesc<copy_reserved[19:8]> = eDSA<eVLAN[11:0]>
 *                  - ELSE IF InDesc<egress_outer_is_tagged>
 *                      OutDesc<copy_reserved[19:8]> = PKT<VID[11:0]>
 *
 ************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR56_Clear_Outgoing_Mtag_Cmd) () {
    ...
    /* Get pointer to descriptor */
    struct ppa_in_desc* thr56_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo,ppa_in_desc);
    uint32_t cfg_reserved_0 = PPA_FW_SP_READ(AC5P_CFG_REGs_lo);
    ...
    if( cfg_reserved_0 == 0 )
    {
      ...
      - Since Desc<Flow-ID> is 16 bits and Desc<orig Src ePort> is 13 bits only, need to to it in 2 commands
      - First to copy 13 bits of Desc<Local Dev Src Port> to the 13 LS bits of Desc<Flow-ID>
      - Second to clear 3 MS bits of Desc<Flow-ID> with zeros
      ------------------------------------------------------------------------------------------------------------------------------------------------*/
        /* Copy 13 bits of Desc<orig_src_eport> to the 13 LS bits of Desc<Flow-ID> */
        __LOG_FW(("Copy 13 bits of Desc<orig_src_eport> to the 13 LS bits of Desc<Flow-ID>"));
        ACCEL_CMD_TRIG(COPY_BITS_THR56_Clear_Outgoing_Mtag_Cmd_DESC_phal2ppa__orig_src_eport_LEN13_TO_DESC_phal2ppa__flow_id_PLUS3_OFFSET, 0);
    ...
    else /* Save the original VID for second pass THR76_Copy_Flow_ID_to_Trg_Phy_Port */
    {
        if (thr56_desc_ptr->phal2ppa.force_new_dsa_tag == 0)
        {
            /* Copy original eDSA<EVLAN>[11:0] to OutDesc<copy_reserved[19:8]> */
            __LOG_FW(("Copy original eDSA<EVLAN>[11:0] to OutDesc<copy_reserved[19:8]>"));
            ACCEL_CMD_TRIG(COPY_BITS_THR56_Clear_Outgoing_Mtag_Cmd_PKT_eDSA_w0_ToCpu__OuterTagVid_eVLAN_11_0_LEN12_TO_DESC_phal2ppa__copy_reserved_OFFSET, 0);
        }
        else if (thr56_desc_ptr->phal2ppa.egress_outer_is_tagged)
        {
            /* Copy original PKT<VLAN>[11:0] to OutDesc<copy_reserved[19:8]> */
            __LOG_FW(("Copy original PKT<VLAN>[11:0] to OutDesc<copy_reserved[19:8]>"));
            ACCEL_CMD_TRIG(COPY_BITS_THR56_Clear_Outgoing_Mtag_Cmd_PKT_vlan__vid_LEN12_TO_DESC_phal2ppa__copy_reserved_OFFSET, 0);
        }
    }
#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(56);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR76_Copy_Flow_ID_to_Trg_Phy_Port
 * inparam      None
 * return       None
 * description  Thread copies value of eDSA Flow-Id field to srcTrg_phy_port.
 *              It's applied to TO_CPU eDSA tagged packets where Flow-Id holds
 *              value of Local Dev Source Port. This thread is applied for descriptor
 *              that was re-trapped and handled by THR56_Clear_Outgoing_Mtag_Cmd.
 *
 *              cfg template: HA_Table_reserved_space.reserved_0 determines old or new behavior. (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in:    | Extension space (64B) | MAC Header(12B) | TO_CPU eDSA(16B) |
 *              hdr out:   | Extension space (64B) | MAC Header(12B) | TO_CPU eDSA(16B) |
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 96 bytes + 64 bytes Header increment space
 *
 *              Firmwares actions:
 *              - IF config.reserved_0 is 0 then, Old Behavior is selected. Else ,New.
 *              OLD:
 *                  - Get value of FlowId_TT_Offset from eDSA tag
 *                  This field hold 9 bits of <local_dev_src_port> of original packet.
 *                  See THR56_Clear_Outgoing_Mtag_Cmd for reference.
 *                  - write value back to eDSA fields srcTrg_phy_port
 *              New:
 *                  - Set eDSA<Src Phy Port> = InDesc<Local Dev Src Port>
 *                  - Set eDSA<Src Trg> = 0 (Src)
 *                  - Set eDSA<Orig Is Trunk> = InDesc<Orig Is Trunk>
 *                  - Set eDSA< Src ePort/Src Trunk> = InDesc<orig_src_eport/trunk>
 *                  - IF InDesc<force_new_dsa_tag>==0 THEN
 *                      Set eDSA<eVLAN[11:0]> = InDesc<copy_reserved[19:8]>
 *                  - ELSE IF InDesc<egress_outer_is_tagged>
 *                      Set PKT<VID[11:0]> = InDesc<copy_reserved[19:8]>
 *                  - write value back to eDSA fields srcTrg_phy_port
 *
 *
 *****************************************************************************************************************************************/
__attribute__ ((aligned (16)))
void PPA_FW(THR76_Copy_Flow_ID_to_Trg_Phy_Port)() {
    /* xt_iss_client_command("profile","enable");            */
    /* xt_iss_client_command("profile","disable"); _exit(0); */

    uint16_t localDevSrcPort;
    struct thr76_Copy_Flow_ID_to_Trg_Phy_Port_in_hdr* thr76_in_hdr_ptr = AC5P_MEM_CAST(AC5P_PKT_REGs_lo,thr76_Copy_Flow_ID_to_Trg_Phy_Port_in_hdr);
    uint32_t cfg_reserved_0 = PPA_FW_SP_READ(AC5P_CFG_REGs_lo);

    if( cfg_reserved_0 == 0 )
    {
        /* FlowId_TT_Offset has 20 bits but SrcTrg_Phy_Port has only 9 bits.
        * 16 bits of localDevSrcPort is enough for the purpose */
        localDevSrcPort = thr76_in_hdr_ptr->eDSA_w3_ToCpu.FlowId_TT_Offset;
        __LOG_FW(("Get Local Dev Source Port from eDSA Flow-Id field"));
        __LOG_PARAM_FW(localDevSrcPort);
    }
    else
    {
        /* Get pointer to descriptor */
        ...
        /* Get the local source port from the descriptor. Masking is done at setting.*/
        localDevSrcPort = desc_ptr->phal2ppa.orig_src_eport;
        __LOG_FW(("Get Local Dev Source Port from Desc local_dev_src_port field"));
        __LOG_PARAM_FW(localDevSrcPort);

        /* Write the source info */
        __LOG_FW(("thr76_in_hdr_ptr->eDSA_w1_ToCpu.OrigIsTrunk = desc_ptr->phal2ppa.orig_is_trunk"));
        thr76_in_hdr_ptr->eDSA_w1_ToCpu.OrigIsTrunk = desc_ptr->phal2ppa.orig_is_trunk;
        __LOG_FW(("thr76_in_hdr_ptr->eDSA_w1_ToCpu.SrcTrg = 0"));
        thr76_in_hdr_ptr->eDSA_w1_ToCpu.SrcTrg = 0;
        __LOG_FW(("thr76_in_hdr_ptr->eDSA_w2_ToCpu.SrcTrg_ePort_SrcTrunk_11_0 = desc_ptr->phal2ppa.orig_src_eport"));
        thr76_in_hdr_ptr->eDSA_w2_ToCpu.SrcTrg_ePort_SrcTrunk_11_0 = desc_ptr->phal2ppa.orig_src_eport;

        if (desc_ptr->phal2ppa.force_new_dsa_tag == 0)
        {
            /* Copy OutDesc<copy_reserved> (the original outer VID) to eDSA<EVLAN>[11:0] */
            __LOG_FW(("thr76_in_hdr_ptr->eDSA_w0_ToCpu.OuterTagVid_eVLAN_11_0 = desc_ptr->phal2ppa.copy_reserved >> 8"));
            thr76_in_hdr_ptr->eDSA_w0_ToCpu.OuterTagVid_eVLAN_11_0 = desc_ptr->phal2ppa.copy_reserved >> 8;
        }
        else if (desc_ptr->phal2ppa.egress_outer_is_tagged)
        {
            /* Copy OutDesc<copy_reserved> (the original outer VID) to PKT<VLAN>[11:0] */
            __LOG_FW(("thr76_in_hdr_ptr->vlan.vid = desc_ptr->phal2ppa.copy_reserved >> 8"));
            thr76_in_hdr_ptr->vlan.vid = desc_ptr->phal2ppa.copy_reserved >> 8;
        }
    }
    /* write Local Dev Source Port to srcTrg_phy_port parts */
    __LOG_FW(("Write Local Dev Source Port to srcTrg_phy_port parts"));
    thr76_in_hdr_ptr->eDSA_w0_ToCpu.SrcTrg_Phy_Port_4_0 = localDevSrcPort & 0x1f;
    thr76_in_hdr_ptr->eDSA_w1_ToCpu.SrcTrg_Phy_Port_6_5 = (localDevSrcPort >> 5) & 0x3;
    thr76_in_hdr_ptr->eDSA_w2_ToCpu.SrcTrg_Phy_Port_8_7 = (localDevSrcPort >> 7) & 0x3;

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(76);
#endif
...
/*************************************************************************************************************************************//**
 * funcname     THR77_PHA_THREAD_TYPE_RESERVED_1_0
 * inparam      None
 * return       None
 * description
 *
 *              cfg template: Not in use (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in (RXG): | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | RXG_Ingress_0(5B)
 *              hdr in (JTM): | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | JTM_Ingress_2(10B)
 *              hdr out (RXG):| Extension space (56B) | L2_Header(14B) | TXP_Egress_Type_4(7B)
 *              hdr out (JTM):| Extension space (61B) | L2_Header(14B) | TXP_Egress_Type_4(7B)
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 *              - Remove generic tunnel start header
 *              - Remove ingress transport header
 *              - Set propper egress transport packet header

 ****************************************************************************************************************************************/
__attribute__((aligned (16)))
void PPA_FW(THR77_PHA_THREAD_TYPE_RESERVED_1_0)()
{
    /*
    xt_iss_client_command("profile","enable");
    xt_iss_client_command("profile","disable"); _exit(0);
    */
    uint8_t pType = (PPA_FW_SP_BYTE_READ(AC5P_PKT_REGs_lo + RESERVED_PTYPE_OFFSET) >> 2) & 0x1f;

    if(pType == 0) /* Flow 1 */
    {
        __LOG_FW(("Copy M,X and Y to outgoing packet"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR77_PHA_THREAD_TYPE_RESERVED_1_0_PKT_RXG_Type_0_I__RXG_Ingress_Type_0__M_LEN2_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__M_OFFSET, 0x0);
        __PRAGMA_NO_REORDER
        __LOG_FW(("Copy L2 DMAC/SMAC and EthType from TS to outgoing packet"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR77_PHA_THREAD_TYPE_RESERVED_1_0_PKT_RXG_Type_0_I__Generic_TS_Untagged_Data_medium__mac_da_47_32_LEN14_TO_PKT_RXG_Type_0_O__mac_header__mac_da_47_32_OFFSET, 0x0);
        __PRAGMA_NO_REORDER
        __LOG_FW(("Update PType to 0x04"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_CONST_0x04_LEN5_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__PType_OFFSET, 0x0);
        __LOG_FW(("Copy Flow_id[4:0] to A"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS11_LEN5_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__valueA_OFFSET, 0x0);
        __LOG_FW(("Copy Flow_id[14:5] to B[9:0]"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS1_LEN2_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__valueB_PLUS14_OFFSET, 0x0);
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS3_LEN8_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__valueB_PLUS16_OFFSET, 0x0);
        __LOG_FW(("Copy copy_reserved[17:4] to B[23:10]"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__copy_reserved_PLUS2_LEN14_TO_PKT_RXG_Type_0_O__TXP_Egress_Type_4__valueB_OFFSET, 0x0);
        __LOG_FW(("thr77_desc_ptr->ppa_internal_desc.fw_bc_modification = -24"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_CONST_0xE8_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);
    }

    else /* Flow 4 */
    {
        __LOG_FW(("Copy M and X"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR77_PHA_THREAD_TYPE_RESERVED_1_0_PKT_JTM_Type_2_I__JTM_Type_2__M_LEN2_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__M_OFFSET, 0x0);
        __PRAGMA_NO_REORDER
        __LOG_FW(("Copy Y"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR77_PHA_THREAD_TYPE_RESERVED_1_0_PKT_JTM_Type_2_I__JTM_Type_2__Y_LEN2_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__Y_OFFSET, 0x0);
        __PRAGMA_NO_REORDER
        __LOG_FW(("Copy L2 DMAC/SMAC and EthType from TS to outgoing packet"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR77_PHA_THREAD_TYPE_RESERVED_1_0_PKT_JTM_Type_2_I__Generic_TS_Untagged_Data_medium__mac_da_47_32_LEN14_TO_PKT_JTM_Type_2_O__mac_header__mac_da_47_32_OFFSET, 0x0);
        __LOG_FW(("Update PType to 0x04"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_CONST_0x04_LEN5_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__PType_OFFSET, 0x0);
        __LOG_FW(("Copy Flow_id[4:0] to A"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS11_LEN5_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__valueA_OFFSET, 0x0);

        __LOG_FW(("Copy Flow_id[14:5] to B[9:0]"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS1_LEN2_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__valueB_PLUS14_OFFSET, 0x0);
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__flow_id_PLUS3_LEN8_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__valueB_PLUS16_OFFSET, 0x0);
        __LOG_FW(("Copy copy_reserved[17:4] to B[23:10]"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_DESC_phal2ppa__copy_reserved_PLUS2_LEN14_TO_PKT_JTM_Type_2_O__TXP_Egress_Type_4__valueB_OFFSET, 0x0);
        __LOG_FW(("thr77_desc_ptr->ppa_internal_desc.fw_bc_modification = -29"));
        ACCEL_CMD_TRIG(COPY_BITS_THR77_PHA_THREAD_TYPE_RESERVED_1_0_CONST_0xE3_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);
    }

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(77);
#endif
    /* Execute the packet swap sequence to jump to the next Thread */
    __LOG_FW(("Execute the packet swap sequence to jump to the next Thread"));
    packet_swap_sequence();
...
/*************************************************************************************************************************************//**
 * funcname     THR78_PHA_THREAD_TYPE_RESERVED_1_1
 * inparam      None
 * return       None
 * description
 *
 *              cfg template: Not in use (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in (RXP): | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | RXG_Ingress_0(8B)
 *              hdr in (JTM): | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | JTM_Ingress_2(11B)
 *              hdr out (RXP):| Extension space (56B) | L2_Header(14B) | TXG_Egress_Type_6(8B)
 *              hdr out (JTM):| Extension space (61B) | L2_Header(14B) | TXG_Egress_Type_6(8B)
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 ...
 ****************************************************************************************************************************************/
__attribute__((aligned (16)))
void PPA_FW(THR78_PHA_THREAD_TYPE_RESERVED_1_1)()
{
    /*
    xt_iss_client_command("profile","enable");
    xt_iss_client_command("profile","disable"); _exit(0);
    */
    ...
    __LOG_FW(("Copy L2 DMAC/SMAC and EthType from TS to outgoing packet"));
    ACCEL_CMD_TRIG(COPY_BYTES_THR78_PHA_THREAD_TYPE_RESERVED_1_1_PKT_RXP_Type_6_I__Generic_TS_Untagged_Data_medium__mac_da_47_32_LEN14_TO_PKT_RXP_Type_6_O__mac_header__mac_da_47_32_OFFSET, 0x0);
    __LOG_FW(("Update PType to 0x06"));
    ACCEL_CMD_TRIG(COPY_BITS_THR78_PHA_THREAD_TYPE_RESERVED_1_1_CONST_0x06_LEN5_TO_PKT_RXP_Type_6_O__TXG_Egress_Type_6__PType_OFFSET, 0x0);
    __PRAGMA_NO_REORDER

    if(pType == 0x07) /* Flow 8 */
    {
        __LOG_FW(("Adjust L2 DMAC/SMAC, EthType, M, PType and X locations to fit JTM->TXG format"));
        ACCEL_CMD_TRIG(COPY_BYTES_THR78_PHA_THREAD_TYPE_RESERVED_1_1_PKT_JTM_Type_7_I__mac_header__mac_da_47_32_LEN15_TO_PKT_JTM_Type_7_O__mac_header__mac_da_47_32_OFFSET, 0x0);
        /* reduce outgoing packet length by 0x1D */
        __LOG_FW(("thr78_desc_ptr->ppa_internal_desc.fw_bc_modification = -29"));
        ACCEL_CMD_TRIG(COPY_BITS_THR78_PHA_THREAD_TYPE_RESERVED_1_1_CONST_0xE3_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);

    }
    else /* Flow 5 PType == 0x06 */
    {
        /* reduce outgoing packet length by 0x1A */
        __LOG_FW(("thr78_desc_ptr->ppa_internal_desc.fw_bc_modification = -26"));
        ACCEL_CMD_TRIG(COPY_BITS_THR78_PHA_THREAD_TYPE_RESERVED_1_1_CONST_0xE6_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);
    }

#if (CHECK_STACK)
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(78);
#endif
    /* Execute the packet swap sequence to jump to the next Thread */
    __LOG_FW(("Execute the packet swap sequence to jump to the next Thread"));
    packet_swap_sequence();
...
/*************************************************************************************************************************************//**
 * funcname     THR79_PHA_THREAD_TYPE_RESERVED_1_2
 * inparam      None
 * return       None
 * description
 *
 *              cfg template: Not in use (16B)
 *              src & trg entries: Not in use (4B)
 *              hdr in (RXP_Type_6):       | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | RXP_Ingress_Type_6(8B)
 *              hdr in (RXG_JTM_Type_2):   | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | JTM_Type_2(10B)
 *              hdr in (JTM_Type_7):       | Extension space (32B) | TS_Medium(26B) | L2_Header(14B) | JTM_Type_7(11B)
 *              hdr out (RXP_Type_6):      | Extension space (55B) | L2_Header(14B) | JTM_Type_7(11B)
 *              hdr out (RXG_JTM_Type_2):  | Extension space (58B) | L2_Header(14B) | JTM_Type_2(10B)
 *              hdr out (JTM_Type_7):      | Extension space (58B) | L2_Header(14B) | JTM_Type_7(11B)
 *              Header Window anchor= Outer Layer 2
 *              Header Window size= 128 bytes + 32 bytes Header increment space
 *
 *              Firmwares actions:
 ...
 ****************************************************************************************************************************************/
__attribute__((aligned (16)))
void PPA_FW(THR79_PHA_THREAD_TYPE_RESERVED_1_2)()
{
    /*
    xt_iss_client_command("profile","disable"); _exit(0);
    xt_iss_client_command("profile","enable");
     */
    struct ppa_in_desc* thr79_desc_ptr = AC5P_MEM_CAST(AC5P_DESC_REGs_lo, ppa_in_desc);
    if(thr79_desc_ptr->phal2ppa.outgoing_mtag_cmd == 3)
    {
        __PRAGMA_FREQUENCY_HINT_FREQUENT
        ...
        if(pType == 0x06)
        {
            __LOG_FW(("Save TS.data_0_3[15:0]"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_RXP_Type_6_I__Generic_TS_Untagged_Data_medium__data_3_0_PLUS2_LEN2_TO_PKT_RXP_Type_6_O__expansion_space__reserved_0_OFFSET, 0x0);
            __PRAGMA_NO_REORDER
            __LOG_FW(("Copy L2 DMAC/SMAC and EthType from TS to outgoing packet"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_RXP_Type_6_I__Generic_TS_Untagged_Data_medium__mac_da_47_32_LEN14_TO_PKT_RXP_Type_6_O__mac_header__mac_da_47_32_OFFSET, 0x0);
            __PRAGMA_NO_REORDER
            __LOG_FW(("Copy M and X field"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_RXP_Type_6_I__RXP_Ingress_Type_6__M_LEN2_TO_PKT_RXP_Type_6_O__JTM_Type_7__M_OFFSET, 0x0);
            __PRAGMA_NO_REORDER
            __LOG_FW(("Header<Fwding label> = TS<Data1[15:0]>"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_RXP_Type_6_I__expansion_space__reserved_0_LEN2_TO_PKT_RXP_Type_6_O__JTM_Type_7__Forwarding_Label_OFFSET, 0x0);
            __LOG_FW(("Header<Hop Limit> = CopyReserved[15:8]"));
            ACCEL_CMD_TRIG(COPY_BITS_THR79_PHA_THREAD_TYPE_RESERVED_1_2_DESC_phal2ppa__copy_reserved_PLUS4_LEN8_TO_PKT_RXP_Type_6_O__JTM_Type_7__Hop_Limit_OFFSET, 0x0);
            __PRAGMA_NO_REORDER
            __LOG_FW(("Update PType to 0x07"));
            ACCEL_CMD_TRIG(COPY_BITS_THR79_PHA_THREAD_TYPE_RESERVED_1_2_CONST_0x07_LEN5_TO_PKT_RXP_Type_6_O__JTM_Type_7__PType_OFFSET, 0x0);
            /* reduce outgoing packet length by 0x1D */
            __LOG_FW(("thr79_desc_ptr->ppa_internal_desc.fw_bc_modification = -23"));
            ACCEL_CMD_TRIG(COPY_BITS_THR79_PHA_THREAD_TYPE_RESERVED_1_2_CONST_0xE9_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);
        }
        /* Flows 2,3,7 */
        else
        {
            uint32_t ts_data_0_3 = PPA_FW_SP_SHORT_READ(AC5P_PKT_REGs_lo + RESERVED_TS_DATA_0_3_OFFSET + 2) | (PPA_FW_SP_SHORT_READ(AC5P_PKT_REGs_lo + RESERVED_TS_DATA_0_3_OFFSET) << 16);
            uint8_t hopLimit = PPA_FW_SP_BYTE_READ(AC5P_PKT_REGs_lo + RESERVED_HOP_LIMIT_I_OFFSET);
            uint8_t is_dec_Hop_Limit = (ts_data_0_3 >> 16) & 0x1;


            __LOG_FW(("Header<Fwding label> = Data1[15:0]"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_JTM_Type_7_I__Generic_TS_Untagged_Data_medium__data_3_0_PLUS2_LEN2_TO_PKT_JTM_Type_7_O__JTM_Type_7__Forwarding_Label_OFFSET, 0x0);

            if (is_dec_Hop_Limit)
            {
                if(hopLimit <= 1)
                {
                    uint8_t is_trap_cmd = (ts_data_0_3 >> 17) & 0x1;
                    uint8_t is_cnc_en = (ts_data_0_3 >> 18) & 0x1;
                    uint16_t cnc_index_0_1 = ((ts_data_0_3 >> 19) & 0x1fff);

                    __LOG_FW(("Set packet command "));
                    thr79_desc_ptr->ppa_internal_desc.fw_packet_command = is_trap_cmd ? TRAP_TO_CPU : HARD_DROP;
                    __LOG_FW(("Set thr79_desc_ptr->ppa_internal_desc.fw_drop_code to 202 "));
                    thr79_desc_ptr->ppa_internal_desc.fw_drop_code = 202;
                    __LOG_FW(("Set CNC "));
                    thr79_desc_ptr->ppa_internal_desc.pha_cnc_client_trigger = is_cnc_en;
                    thr79_desc_ptr->ppa_internal_desc.pha_cnc_client_index_override = is_cnc_en;
                    PPA_FW_SP_SHORT_WRITE( RESERVED_DESC_PHA_METADATA_0_OFFSET - 0x1, cnc_index_0_1);
                }
                else
                {
                    __LOG_FW(("Header<Hop Limit>-- "));
                    ACCEL_CMD_TRIG(SUB_BITS_THR79_PHA_THREAD_TYPE_RESERVED_1_2_CONST_0x1_LEN8_TO_PKT_JTM_Type_7_O__JTM_Type_7__Hop_Limit_OFFSET, 0x0);
                }
            }

            __LOG_FW(("Copy L2 DMAC/SMAC and EthType"));
            ACCEL_CMD_TRIG(COPY_BYTES_THR79_PHA_THREAD_TYPE_RESERVED_1_2_PKT_JTM_Type_7_I__Generic_TS_Untagged_Data_medium__mac_da_47_32_LEN14_TO_PKT_JTM_Type_7_O__mac_header__mac_da_47_32_OFFSET, 0x0);

            __LOG_FW(("thr79_desc_ptr->ppa_internal_desc.fw_bc_modification = -26"));
            ACCEL_CMD_TRIG(COPY_BITS_THR79_PHA_THREAD_TYPE_RESERVED_1_2_CONST_0xE6_LEN8_TO_DESC_ppa_internal_desc__fw_bc_modification_OFFSET, 0x0);
    ...
    /* Check if stack was overflowed */
    __LOG_FW(("Check if stack was overflowed"));
    ac5p_stack_overflow_checker(79);
    #endif
    /* Execute the packet swap sequence to jump to the next Thread */
    __LOG_FW(("Execute the packet swap sequence to jump to the next Thread"));
    packet_swap_sequence();
...
THREAD_TYPE phaThreadsTypeAc5pImage02[] = {
    ...
    /*76*/ ,PPA_FW(THR76_Copy_Flow_ID_to_Trg_Phy_Port)
    /*77*/ ,PPA_FW(THR77_PHA_THREAD_TYPE_RESERVED_1_0)
    /*78*/ ,PPA_FW(THR78_PHA_THREAD_TYPE_RESERVED_1_1)
    /*79*/ ,PPA_FW(THR79_PHA_THREAD_TYPE_RESERVED_1_2)
