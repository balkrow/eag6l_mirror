...
*/
    ...
    if(UTF_CPSS_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(prvTgfDevNum))
    {
        ...
        /* Set TTI configuration */
        prvTgfTunnelTermEtherOverMplsRedirectToEgressConfiguredUdbConfigSet(0);
...
/* AUTODOC: Test SIP6 TTI metadata fields:
    ...
    verify CPU code                                             ;
    modify first TTI rule to match IPv6 packet with extension Segment Routing, and 'not known' <next header>;                                                            ;
    verify CPU code                                                                                                                                                                     ;
    restore configuration;
*/
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test SIP6 TTI metadata Tag1 exists field:
    configure VLAN, configure 8-byte Tag parsing - parsing enable and value/mask ;
    configure 1 TTI rule with trap to cpu action;
    send traffic with matched packet;
    verify that packet reached to CPU;
    send traffic with unmatched packet;
    verify that packet not reached to CPU;
    restore configuration;
*/
UTF_TEST_CASE_MAC(tgfTunnelTermSip6_10Tag1Exist)
{
    /* Supported by SIP6_10 devices only */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
    ...
    PRV_TGF_SKIP_PASS_OK_LONG_TEST_GM_MAC(UTF_ALL_FAMILY_E);

    tgfTunnelTermSip6_10Tag1Exist();
}
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------*/
/* Test Tunnel Termination for MPLS packets with 6 labels and more than 6 labels

    > Test1:
        > Ether packet is encapsulated with 6 MPLS labels and send to Ingress Port#0
        > The packet is sent towards target port#1 and undergoes the following actions:
            - Tunnel Termination is set and all the 6 labels are removed by TTI module
            - The passenger L2 packet is not redirected towards the Egress Port#1
        > The passenger L2 packet exits the device from target port#1
    > Test2:
        > Ether packet is encapsulated with 7 MPLS labels and send to Ingress Port#0
        > The packet is sent towards target port#1 and undergoes the following actions:
            - Since the packet has more than 6 labels, it will be Mirrored to CPU
              with CPU code (CPSS_NET_TT_MPLS_HEADER_CHECK_E)
*/
UTF_TEST_CASE_MAC(tgfTunnelTermMplsMultiLabel)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E | UTF_BOBCAT2_E );

    /* Set the Bridge configs */
    prvTgfTunnelTermMplsMultiLabelBridgeConfigSet();

    /* Set the TTI configs */
    prvTgfTunnelTermMplsMultiLabelTtiConfigSet();

    /* Generate the traffic */
    prvTgfTunnelTermMplsMultiLabelTrafficGenerate();

    /* Restore the old configs */
    prvTgfTunnelTermMplsMultiLabelConfigRestore();
}

/*----------------------------------------------------------------------------------*/
/* Test Tunnel Termination for MPLS packets with 1 label and POP_LABEL1 action

    > Ether packet is encapsulated with 1 MPLS label and send to Ingress Port#0
    > The packet is sent towards target port#1 and undergoes the following actions:
        - POP label1 is configured in the TTI action table which strips the label and
          performs tunnel termination implicitly
        - The passenger L2 packet is not redirected towards the Egress Port#1
    > The passenger L2 packet exits the device from target port#1
*/
UTF_TEST_CASE_MAC(tgfTunnelTermImplicitMplsPop1Label)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E | UTF_BOBCAT2_E );
    ...
    /* Set the TTI configs */
    prvTgfTunnelTermImplicitMplsPop1LabelTtiConfigSet();

    /* Generate the traffic */
    prvTgfTunnelTermImplicitMplsPop1LabelTrafficGenerate();
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test Gneric Classification TTI part:
       - configure TTI UDB key Mpls;
       - set TTI Generic Range classification - according to port profile -->
         UDB28 & UDB29 are changed according to matchId in Generic Classification table.
       - send tunneled traffic to match TTI rule;
       - verify passenger traffic on nextHop port.

       - set TTI Generic Range classification - according to generic classification metadata.
       - send tunneled traffic to match TTI rule;
       - verify passenger traffic on nextHop port.
*/
UTF_TEST_CASE_MAC(tgfTunnelGenericRangeClassification)
{
/*

    1. Build Packet
    2. Set Base configuration
    3. Set TTI configuration including Generic Range classification - according to port profile
    4. Generate traffic
    5. Set TTI configuration including Generic Range classification - according to generic classification default eport value
    6. Generate traffic
    7. Restore configuration
*/
    ...
    {
        /* No support for AAS in APIs. */ CPSS_TBD_BOOKMARK_AAS
        prvUtfSkipTestsSet();
        return;
    }
    ...
    /* Set TTI configuration including Generic Range classification - according to port profile */
    prvTgfTunnelTermEtherOverMplsRedirectToEgressConfiguredUdbConfigSet(1);

    /* Generate traffic */
    prvTgfTunnelTermEtherOverMplsRedirectToEgressTrafficGenerate(GT_TRUE);

    /* Set TTI configuration including Generic Range classification - according to generic classification default eport value */
    prvTgfTunnelTermEtherOverMplsRedirectToEgressConfiguredUdbConfigSet(2);

    /* Restore configuration */
    prvTgfTunnelTermEtherOverMplsRedirectToEgressConfiguredUdbConfigurationRestore();

    /* Restore Generic Classification configuration */
    prvTgfTunnelTermEtherOverMplsRedirectToEgressGenericClassificationConfigRestore();

    prvTgfPclRestore();

}

UTF_TEST_CASE_MAC(tgfTunnelMplsCutThrough)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E |
         UTF_BOBCAT2_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_AC3X_E |
         UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_FALCON_E | UTF_IRONMAN_L_E);

    /* Set the Bridge configs */
    prvTgfTunnelTermMplsMultiLabelBridgeConfigSet();

    /* Set the TTI configs */
    prvTgfTunnelMplsCutThroughConfigSet();

    /* Generate the traffic */
    prvTgfTunnelMplsCutThroughTrafficGenerate();

    /* Restore the old configs */
    prvTgfTunnelMplsCutThroughConfigRestore();
}
...
/*    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelTermTrustDscpQosField_qosMappingTableIterator)*/
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelTermSip6_10Tag1Exist)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelTermMplsMultiLabel)
    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelTermImplicitMplsPop1Label)

    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelGenericRangeClassification)
    UTF_SUIT_DECLARE_TEST_MAC(tgfTunnelMplsCutThrough)
