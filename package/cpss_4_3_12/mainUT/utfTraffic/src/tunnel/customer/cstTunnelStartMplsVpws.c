...
)
{
   ...
   /* AUTODOC: Configure TPID for Tunnel Start header */
    ...
    rc = cpssDxChBrgVlanTpidEntryGet(devNum,CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E,tpidEntryIndex,&defaultEtherTypeGet);
    CHECK_RC_MAC(rc);

    /* AUTODOC: Sets TPID (Tag Protocol ID) table entry  */
    rc = cpssDxChBrgVlanTpidEntrySet(devNum,CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E,tpidEntryIndex,0x9200);
    CHECK_RC_MAC(rc);
...
)
{
    ...
    rc = cpssDxChBrgVlanTpidEntrySet(devNum,CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E,tpidEntryIndex,defaultEtherTypeGet);
    CHECK_RC_MAC(rc);
