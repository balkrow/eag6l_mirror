...
)
{
    ...
    CHECK_RC_MAC(
        cpssDxChTtiMacModeGet(prvTgfDevNum,CPSS_DXCH_TTI_KEY_MPLS_E,
                              &prvCstRestoreCfg.macMode));

    /* AUTODOC: sets the lookup Mac mode */
    CHECK_RC_MAC(
        cpssDxChTtiMacModeSet(prvTgfDevNum,CPSS_DXCH_TTI_KEY_MPLS_E,
...
)
{
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
    {
        ...
        CHECK_RC_MAC(prvTgfTrunkHashGlobalModeSet(PRV_TGF_TRUNK_LBH_PACKETS_INFO_E));
    }
