...
/* IPFIX index used for MPLS */
#define PRV_TGF_MPLS_IPFIX_INDEX_CNS     0xA
...
static struct
{
    ...
    /* Policer config params */
    GT_BOOL                                       meteringEnable;
    PRV_TGF_POLICER_STAGE_METER_MODE_ENT          stageMeterMode;
    PRV_TGF_POLICER_COUNTING_MODE_ENT             countingMode;
    CPSS_DXCH3_POLICER_TT_PACKET_SIZE_MODE_ENT    packetSizeModeForTT;
...
)
{
    ...
    default:
        ...
        /* set action - to trigger IPFIX */
        ttiAction.bindToPolicer = GT_TRUE;
        ttiAction.counterEnable = GT_TRUE;
        ttiAction.policerIndex = PRV_TGF_MPLS_IPFIX_INDEX_CNS;
        break;
    }
    ...
    rc = prvTgfTtiRuleGet(prvTgfDevNum, prvTgfTtiRuleIndex, PRV_TGF_TTI_RULE_MPLS_E,
                          &ttiPattern, &ttiMask, &ttiAction);
...
/**
* @internal prvTgfDefIpfixConfSet function
* @endinternal
*
* @brief   Config Ipfix
*
* @retval GT_OK                    - on success
*/
static GT_STATUS prvTgfDefIpfixConfSet
(
    GT_VOID
)
{
    ...
    PRV_TGF_IPFIX_WRAPAROUND_CFG_STC    wraparoundConf;

    rc = prvTgfPolicerMeteringEnableGet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                        &prvTgfRestoreCfg.meteringEnable);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerMeteringEnableGet");

    rc = prvTgfPolicerMeteringEnableSet(PRV_TGF_POLICER_STAGE_INGRESS_0_E, GT_FALSE);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerMeteringEnableGet");

    /* IPFIX works in Flow mode */
    rc = prvTgfPolicerStageMeterModeGet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                        &prvTgfRestoreCfg.stageMeterMode);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerStageMeterModeGet");

    rc = prvTgfPolicerStageMeterModeSet(PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                        PRV_TGF_POLICER_STAGE_METER_MODE_FLOW_E);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerStageMeterModeSet");

    rc = prvTgfPolicerCountingModeGet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                      &prvTgfRestoreCfg.countingMode);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerCountingModeGet");

    rc = prvTgfPolicerCountingModeSet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                      PRV_TGF_POLICER_COUNTING_BILLING_IPFIX_E);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerCountingModeSet");

    rc = cpssDxCh3PolicerPacketSizeModeForTunnelTermGet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, &prvTgfRestoreCfg.packetSizeModeForTT);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeForTunnelTermGet");

    rc = cpssDxCh3PolicerPacketSizeModeForTunnelTermSet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, CPSS_DXCH3_POLICER_TT_PACKET_SIZE_PASSENGER_E);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeForTunnelTermSet");

    rc = cpssDxCh3PolicerPacketSizeModeGet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, &prvTgfRestoreCfg.packetSizeMode);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeGet");

    rc = cpssDxCh3PolicerPacketSizeModeSet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, CPSS_POLICER_PACKET_SIZE_L2_INCLUDE_E);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeSet");

    wraparoundConf.action = PRV_TGF_IPFIX_WRAPAROUND_ACTION_NONE_E;
    wraparoundConf.dropThreshold = 0x3FFFFFFF;
    wraparoundConf.packetThreshold = 0x3FFFFFFF;
    wraparoundConf.byteThreshold.l[0] = 0xFFFFFFFF;
    wraparoundConf.byteThreshold.l[1] = 0xF;

    /* set wraparound configuration */
    rc = prvTgfIpfixWraparoundConfSet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E, &wraparoundConf);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfIpfixWraparoundConfSet");

    cpssOsMemSet(&ipfixEntry, 0, sizeof(ipfixEntry));

    ipfixEntry.samplingMode = PRV_TGF_IPFIX_SAMPLING_MODE_BYTE_E;
    ipfixEntry.lastPacketCommand = CPSS_PACKET_CMD_FORWARD_E;
    ipfixEntry.lastCpuOrDropCode = CPSS_NET_BRIDGED_PACKET_FORWARD_E;

    rc = prvTgfIpfixEntrySet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                             PRV_TGF_MPLS_IPFIX_INDEX_CNS, &ipfixEntry);
...
/**
* @internal prvTgfDefIpfixConfReset function
* @endinternal
*
* @brief   Reset config Ipfix
*
* @retval GT_OK                    - on success
*/
static GT_STATUS prvTgfDefIpfixConfReset
(
    GT_VOID
)
{
    ...
    rc = prvTgfPolicerMeteringEnableSet(PRV_TGF_POLICER_STAGE_INGRESS_0_E, prvTgfRestoreCfg.meteringEnable);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerMeteringEnableSet");

    rc = prvTgfPolicerStageMeterModeSet(PRV_TGF_POLICER_STAGE_INGRESS_0_E, prvTgfRestoreCfg.stageMeterMode);
    PRV_UTF_VERIFY_GT_OK(rc, "prvTgfPolicerStageMeterModeSet");
    ...
    rc = cpssDxCh3PolicerPacketSizeModeForTunnelTermSet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, prvTgfRestoreCfg.packetSizeModeForTT);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeForTunnelTermSet");

    rc = cpssDxCh3PolicerPacketSizeModeSet(prvTgfDevNum, CPSS_DXCH_POLICER_STAGE_INGRESS_0_E, prvTgfRestoreCfg.packetSizeMode);
    PRV_UTF_VERIFY_GT_OK(rc, "cpssDxCh3PolicerPacketSizeModeSet");
...
)
{
    ...
    GT_U32    expectedByteCount = expectedByteCount;
    ...
    {
        if(pushType == PRV_TGF_TUNNEL_MPLS_LSR_PUSH_1_LABEL_E)
        {
            rc = prvTgfDefIpfixConfSet();
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfDefIpfixConfSet: %d", prvTgfDevNum);
        }
    }
    ...
    if (GT_OK == rc)
    {
        ...
        cpssOsMemSet(&ipfixEntry, 0, sizeof(ipfixEntry));
        /* This test scenario is skipped for non SIP5 devices */
        if(GT_FALSE == prvUtfDeviceTestNotSupport(prvTgfDevNum, UTF_CPSS_PP_E_ARCH_CNS))
        {
            if(pushType == PRV_TGF_TUNNEL_MPLS_LSR_PUSH_1_LABEL_E)
            {
                rc = prvTgfIpfixEntryGet(prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E,
                                         PRV_TGF_MPLS_IPFIX_INDEX_CNS, GT_TRUE, &ipfixEntry);
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfIpfixEntryGet: %d", prvTgfDevNum);

                expectedByteCount = packetSize;
                if(GT_FALSE != prvUtfDeviceTestNotSupport(prvTgfDevNum, UTF_CPSS_PP_ALL_SIP6_CNS))
                {
                    expectedByteCount -= TGF_L2_HEADER_SIZE_CNS;
                }
                else
                {
                    expectedByteCount -= (TGF_L2_HEADER_SIZE_CNS + TGF_VLAN_TAG_SIZE_CNS + TGF_ETHERTYPE_SIZE_CNS + TGF_MPLS_HEADER_SIZE_CNS);
                }
                UTF_VERIFY_EQUAL1_STRING_MAC(expectedByteCount, ipfixEntry.byteCount.l[0], "prvTgfIpfixEntryGet: %d", prvTgfDevNum);
            }
        }
    }
    ...
    /* Restore IPFIX config */
    if(GT_FALSE == prvUtfDeviceTestNotSupport(prvTgfDevNum, UTF_CPSS_PP_E_ARCH_CNS))
    {
        if(pushType == PRV_TGF_TUNNEL_MPLS_LSR_PUSH_1_LABEL_E)
        {
            rc = prvTgfDefIpfixConfReset();
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfDefIpfixConfReset: %d", prvTgfDevNum);
        }
    }
}
