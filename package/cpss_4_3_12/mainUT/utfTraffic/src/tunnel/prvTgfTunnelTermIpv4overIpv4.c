...
)
{
    ...
    rc = prvTgfTtiRuleSet(prvTgfTtiIllegalRuleIndex, PRV_TGF_TTI_RULE_IPV4_E,
                          &ttiPattern, &ttiMask, &ttiAction);
    ...
    {
        /* AUTODOC: add TTI rule with illegal index */
        rc = prvTgfTtiRuleSet(prvTgfTtiRuleIndex + 1, PRV_TGF_TTI_RULE_IPV4_E,
                              &ttiPattern, &ttiMask, &ttiAction);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PARAM, rc, "prvTgfTtiPortGroupRuleSet: %d", prvTgfDevNum);
        checkOriginalRule();
...
)
{
    ...
    switch (prvTgfTestConfigNum)
    {
        ...
        default:
            ruleType = PRV_TGF_TTI_RULE_IPV4_E;
            break;
    }
