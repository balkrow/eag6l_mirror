...
/* AAS VXLAN-GBP default UDP source port number  */
#define PRV_TGF_VXLAN_GBP_AAS_RFC_UDP_SRC_PORT_CNS  60716
...
)
{
    ...
    /* AUTODOC: Set the CRC hashing mode assuming default values of CRC Mask, all bits masked
       - generate hash = 0 */
...
)
{
    ...
    cpssOsMemSet(&profileData,0,sizeof(profileData));
    profileData.udpSrcPortMode          = (udpSrcPortGeneric == 1) ? PRV_TGF_TUNNEL_START_UDP_SRC_PORT_ASSIGN_FROM_IP_GEN_TS_ENTRY_E:
                                                                     PRV_TGF_TUNNEL_START_UDP_SRC_PORT_ASSIGN_FROM_PACKET_HASH_VALUE_E;
    profileData.templateDataSize        = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_SIZE_8B_E; /* 8 bytes */
    profileData.templateDataBitsCfg[7]  = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E; /* G Bit */
    profileData.templateDataBitsCfg[3]  = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E; /* I bit */
    profileData.templateDataBitsCfg[14] = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E; /* D Bit */
    profileData.templateDataBitsCfg[11] = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E; /* A Bit */
    profileData.templateDataBitsCfg[51] = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E; /* VNI, 0x18 in our case*/
    profileData.templateDataBitsCfg[52] = PRV_TGF_TUNNEL_START_TEMPLATE_DATA_CFG_CONST_1_E;
...
)
{
    ...
    /* AUTODOC: Restore global hashing mode */
...
)
{
    ...
    GT_U16              udpSrcPortReceived, udpSrcPortExpected, udpDstPortReceived, udpDstPortExpected, rfcUdpSrcPort;
    ...
    For AAS, 'Src UDP port' set by descrPtr->pktHash[0x2D2C] 14 LSbits , and 2 MSbits are HardCoded 0x3 (as VXLAN RFC 7348 recommends the hash value assigned be in the dynamic/private port range 49152-65535.!!!*/
    rfcUdpSrcPort = PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum)? PRV_TGF_VXLAN_GBP_AAS_RFC_UDP_SRC_PORT_CNS : PRV_TGF_VXLAN_GBP_RFC_UDP_SRC_PORT_CNS;

    udpSrcPortExpected = (udpSrcPortMode == 1) ? PRV_TGF_VXLAN_GBP_UDP_SRC_PORT_CNS : rfcUdpSrcPort;
