...
static GT_HW_DEV_NUM   prvTgfDsaMetaDevNum = PRV_TGF_DSA_DEV_CNS;
...
)
{
    ...
    prvTgfPacketDsaTagPart_forward.srcHwDev       = prvTgfDsaMetaDevNum;
...
)
{
    ...
    if ((prvTgfDsaMetaDevNum == prvTgfDevNum) ||
        (prvTgfDsaMetaDevNum == hwDevNum))
    {
        prvTgfDsaMetaDevNum++;
    }
    ...
    prvTgfTunnelTermTtiDsaMetadataTtiPatternSet(prvTgfDsaMetaDevNum, deviceAnchorOffset);
    prvTgfTunnelTermTtiDsaMetadataTrafficGenerate(GT_FALSE, GT_FALSE);
