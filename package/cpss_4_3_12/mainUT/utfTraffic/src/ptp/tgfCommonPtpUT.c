...
/* AUTODOC: Test use (if system has two TODs)  TAI_0 for ingress port and use TAI_1 for egress port.
            Device calculates "latency" by  TOD of Egress port - time_stamp_from_ingress one.
            Step 1. Configure both TODs to be equal ingress_TOD_nano.
                    Get correction field value 1
            Step 2. Configure egress_TOD_nano = 200000  + ingress_TOD_nano
                    Get correction field value 2.
            Expect correction field value 2 should be in range +-latency + (egress_TOD_nano - ingress_TOD_nano)
*/
UTF_TEST_CASE_MAC(tgfPtpAddCorrectionTimeTest)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_AC5Y_CHECK_MAC(prvTgfDevNum);

    prvTgfPtpGenVidAndFdbDefCfgSet();

    prvTgfPtpGenAddCorrectedTimeCfgSet(GT_TRUE);
...
/* AUTODOC: Should not be part of the regression as it changes PTP device default settings !!!!!!
            run this test only manualy.                                                                                             .
                                                                                                   .
            Test use (if system has two TODs)  TAI_0 for ingress port and use TAI_1 for egress port.
            Device calculates "latency" by  TOD of Egress port - time_stamp_from_ingress one.
            Step 1. Configure TOD Step to be 0 for all TAIs.
            Step 2. Configure both TODs to be equal to 0.
                    Get correction field value.
            Expect correction field value to be 0.
*/
GT_VOID tgfPtpAddCorrectionTimeTodStepZeroTest()
{
    ...
    prvTgfPtpGenAddCorrectedTimeCfgSet(GT_FALSE);

    prvTgfPtpGenAddCorrectionTimeTodStepZeroTrafficCheck();
...
UTF_TEST_CASE_MAC(tgfPtpEgressTimestampQueueReading)
{
  ...
  prvTgfPtpGenEgressCaptureQueue1Check(GT_TRUE,EGRESS_PORT_INDEX_PIPE_1);
  prvTgfPtpGenEgressCaptureQueue1Check(GT_TRUE,EGRESS_PORT_INDEX_PIPE_0);
  prvTgfPtpGenEgressCaptureQueue1Check(GT_TRUE,EGRESS_PORT_INDEX_PIPE_1);
  prvTgfPtpGenEgressCaptureQueue1Check(GT_TRUE,EGRESS_PORT_INDEX_PIPE_0);
  /* NOTE : the Golden model not supporting dequeueing */
  if (GT_FALSE == prvUtfIsGmCompilation())
  {
      prvTgfPtpGenEgressCaptureQueue1Check(GT_FALSE,EGRESS_PORT_INDEX_PIPE_1);
      prvTgfPtpGenEgressCaptureQueue1Check(GT_FALSE,EGRESS_PORT_INDEX_PIPE_0);
  }
  prvTgfPtpGenCfgRestore();
...
UTF_TEST_CASE_MAC(tgfPtpEgressTimestampQueueinterrupt)
{
  ...
  PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E);
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
