...
#include <extUtils/trafficEngine/tgfTrafficGenerator_forXml.h>   /* needed for tgfTrafficGeneratorXmlPacketPartsDelete */
...
/* Run SIP6 devices in legacy FDB hash mode */
static GT_BOOL  legacyFdbHashModeForSip6Devices = GT_FALSE;

GT_VOID prvFdbManagerLegacyHashModeForSip6DevicesEnable(GT_U32 enable)
{
    legacyFdbHashModeForSip6Devices = (enable) ? GT_TRUE : GT_FALSE;
    cpssOsPrintf("Legacy mode for SIP6 device - [%s]\n", legacyFdbHashModeForSip6Devices ? "GT_TRUE" : "GT_FALSE");
    return;
}

GT_BOOL prvFdbManagerLegacyHashModeForSip6DevicesGet()
{
    return legacyFdbHashModeForSip6Devices;
}
...
/**
* @internal prvTgfBrgFdbManager_restoreFdbConfig function
* @endinternal
*
* @brief  restore FDB Configurations
*/
GT_VOID prvTgfBrgFdbManager_restoreFdbConfig()
{
    GT_STATUS               rc;
    GT_U32                  numOfEports;
    ...
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvCpssDxChBrgFdbSizeSet: expected to GT_OK");
    #endif

    if(PRV_CPSS_SIP_5_CHECK_MAC(prvTgfDevNum) == GT_TRUE)
    {
        numOfEports = PRV_CPSS_DXCH_MAX_PORT_NUMBER_MAC(prvTgfDevNum);

        /* appDemo sets such number of ports to work in control learning */
        numOfPhysicalPorts = MAX(256,PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(prvTgfDevNum));

        /* FDB manager disables default HW state (auto learning) for all ePorts.
           restore HW settings for upper ePorts with numbres >= numOfPhysicalPorts */
        for(portNum = numOfPhysicalPorts ; portNum < numOfEports ; portNum++)
        {
            /* enable the auto learning */
            rc = cpssDxChBrgFdbPortLearnStatusSet(prvTgfDevNum, portNum, GT_TRUE, CPSS_LOCK_FRWRD_E);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "cpssDxChBrgFdbPortLearnStatusSet: expected to GT_OK on port [%d]",portNum);

            /* Disable new address message Storm Prevention */
            rc =  cpssDxChBrgFdbNaStormPreventSet(prvTgfDevNum,portNum,GT_FALSE);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "cpssDxChBrgFdbNaStormPreventSet: expected to GT_OK on port [%d]",portNum);
        }
    }
}
...
/* add fdb entry to the manager : type : ipv6 addr */
static void fdbManagerEntryAdd_ipv6Addr(
    IN GT_U32   fdbManagerId,
...
)
{
    ...
    CPSS_DXCH_BRG_FDB_MANAGER_IPV6_UC_ROUTING_INFO_STC     *fdbEntryIpv6FormatPtr;
    GT_U32                                                  offset;
    ...
    *entryAddedPtr = GT_FALSE;
    ...
    fdbEntryIpv6FormatPtr           = &entry.format.fdbEntryIpv6UcFormat;
    fdbEntryIpv6FormatPtr->age      = GT_TRUE;
    fdbEntryIpv6FormatPtr->vrfId    = prvUtfVrfId;

    for (offset = 0; offset < 8; offset++)
    {
        fdbEntryIpv6FormatPtr->ipv6Addr.arIP[offset << 1] =
            (GT_U8)(prvTgfPacketIpv6Part.dstAddr[offset] >> 8);
        fdbEntryIpv6FormatPtr->ipv6Addr.arIP[(offset << 1) + 1] =
            (GT_U8) prvTgfPacketIpv6Part.dstAddr[offset];
    }
    fdbEntryIpv6FormatPtr->ucRouteType  =
        CPSS_DXCH_BRG_FDB_MANAGER_UC_ROUTING_TYPE_NEXT_HOP_INFO_FULL_IN_FDB_ARP_PTR_E;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.nextHopVlanId        = PRV_TGF_NEXTHOPE_VLANID_CNS;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.ttlHopLimitDecEnable = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.ttlHopLimDecOptionsExtChkByPass = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.countSet             = CPSS_IP_CNT_SET0_E;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.ICMPRedirectEnable   = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.mtuProfileIndex      = 0;

    /* Interface Info */
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.dstInterface.type    =
        CPSS_INTERFACE_PORT_E;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.dstInterface.interfaceInfo.devPort.hwDevNum =
        PRV_CPSS_HW_DEV_NUM_MAC(prvTgfDevNum);
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.dstInterface.interfaceInfo.devPort.portNum =
        prvTgfPortsArray[PRV_TGF_NEXTHOPE_2_PORT_IDX_CNS];

    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.pointer.arpPointer   = prvTgfRouterArpIndex;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.ipv6ExtInfo.siteId   = CPSS_IP_SITE_ID_INTERNAL_E;
    fdbEntryIpv6FormatPtr->ucRouteInfo.fullFdbInfo.ipv6ExtInfo.scopeCheckingEnable = GT_FALSE;

    /* Unicast common info */
    fdbEntryIpv6FormatPtr->ucCommonInfo.qosProfileMarkingEnable         = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucCommonInfo.qosProfileIndex                 = 0;
    fdbEntryIpv6FormatPtr->ucCommonInfo.qosProfilePrecedence            =
        CPSS_PACKET_ATTRIBUTE_ASSIGN_PRECEDENCE_SOFT_E;
    fdbEntryIpv6FormatPtr->ucCommonInfo.modifyUp                        =
        CPSS_PACKET_ATTRIBUTE_MODIFY_KEEP_PREVIOUS_E;
    fdbEntryIpv6FormatPtr->ucCommonInfo.modifyDscp                      =
        CPSS_PACKET_ATTRIBUTE_MODIFY_KEEP_PREVIOUS_E;
    fdbEntryIpv6FormatPtr->ucCommonInfo.arpBcTrapMirrorEnable           = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucCommonInfo.dipAccessLevel                  = 0;
    fdbEntryIpv6FormatPtr->ucCommonInfo.ingressMirrorToAnalyzerEnable   = GT_FALSE;
    fdbEntryIpv6FormatPtr->ucCommonInfo.ingressMirrorToAnalyzerIndex    = 0;
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK,st,
            "fdbManagerEntryAdd_ipv6Addr: unexpected error in manager [%d]",
            fdbManagerId);

    if((*entryAddedPtr) == GT_TRUE)
    {
        /* start -- the parts of the key */
        cpssOsMemCpy(&entryGet.format.fdbEntryIpv6UcFormat.ipv6Addr,
                &fdbEntryIpv6FormatPtr->ipv6Addr,
                sizeof(entry.format.fdbEntryIpv6UcFormat.ipv6Addr));

        entryGet.format.fdbEntryIpv6UcFormat.vrfId = fdbEntryIpv6FormatPtr->vrfId;
        ...
        UTF_VERIFY_EQUAL1_STRING_MAC(0,cpssOsMemCmp(
                    &entryGet.format.fdbEntryIpv6UcFormat, fdbEntryIpv6FormatPtr,
...
static GT_STATUS createFdbManager(
    IN GT_U32                                       fdbManagerId,
...
/* create fdb manager , with adding the device - minimal parameters for tests */
...
{
    return   createFdbManager(fdbManagerId,maxTotalEntries,inputCapacityInfo,macNoSpaceUpdatesEnable,
        GT_TRUE/*with device*/);
}
/* create fdb manager , without adding the device  - minimal parameters for tests */
static GT_STATUS createFdbManagerWithoutDevice(
    IN GT_U32                                       fdbManagerId,
    IN GT_U32                                       maxTotalEntries,/* up to 128K (limited by numOfHwIndexes) */
    IN CPSS_DXCH_BRG_FDB_MANAGER_CAPACITY_STC      *inputCapacityInfo,
    IN GT_BOOL                                      macNoSpaceUpdatesEnable
...
)
{
    ...
    rc = cpssDxChBrgFdbManagerEntryGetNext(fdbManagerId, GT_TRUE, GT_FALSE, &entry);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
...
/**
* @internal prvTgfBrgFdbManagerLearningScan_no_space_traffic_and_verify function
* @endinternal
*
* @brief  configures the learning scan related parameters to verify
*         learning scan events NO_SPACE and entry ALREADY_EXIST
*/
...
)
{
    ...
    GT_U32                                                  usedEntriesCnt = 0;
    GT_BOOL                                                 prevState;
    TGF_PACKET_L2_STC                                       prvTgfPacketL2PartSaved;

    fdbManagerPtr = MANAGER_GET_MAC(fdbManagerId);

    /* Save L2 part of packet */
    prvTgfPacketL2PartSaved = prvTgfPacketL2Part;
    ...
    for(packetCnt = 0; packetCnt < PRV_TGF_MAX_TOTAL_ENTRIES; packetCnt++)
    {
        ...
        /* In order to analyze specific AU events, need to leave space for adding entries to the FDB by sending traffic */
        if(numEntriesAdded == fdbManagerPtr->thresholdB)
        {
            PRV_UTF_LOG2_MAC("ThresholdB reached, numEntriesAdded = %d, packetCnt = %d\n", numEntriesAdded, packetCnt);
            break;
    ...
    paramsPtr.addWithRehashEnable       = GT_TRUE;
    ...
    for(packetCnt = 0; packetCnt < (throttlingData.thresholdC - throttlingData.thresholdB); packetCnt++)
    {
        ...
        if(fdbManagerPtr->noSpaceCacheEntry.usedEntriesCnt)
        {
            if(entriesLearningArray[0].updateType == CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_UPDATE_EVENT_TYPE_NEW_E)
            {
                UTF_VERIFY_EQUAL0_STRING_MAC(1, entriesLearningNum,
                             "New address event is not as expected");
            }
            else
            {
                if(usedEntriesCnt != fdbManagerPtr->noSpaceCacheEntry.usedEntriesCnt)
                {
                    UTF_VERIFY_EQUAL0_STRING_MAC(0, entriesLearningNum,
                                 "No space event is not as expected");

                    usedEntriesCnt = fdbManagerPtr->noSpaceCacheEntry.usedEntriesCnt;
                }
            }
        ...
        {
            /* There are still free space in FDB */
            if(packetCnt % 2)
            {
                /* Trying to add the same entry  */
                UTF_VERIFY_EQUAL0_STRING_MAC(0, entriesLearningNum,
                             "Entry already exist is not as expected");
            }
            else
            {
                /* The new entry is added */
                UTF_VERIFY_EQUAL0_STRING_MAC(1, entriesLearningNum,
                             "New AU message is not as expected");
            }

            if(packetCnt % 2)
            {
                /* Change the src MAC every even packet */
                prvTgfPacketL2Part.saMac[5]+=1;
                if(prvTgfPacketL2Part.saMac[5] == 1)    /* 1 Byte overflowed */
                {
                    prvTgfPacketL2Part.saMac[4]+=1;
    ...
    /* Restore L2 part of packet */
    prvTgfPacketL2Part = prvTgfPacketL2PartSaved;


    UTF_VERIFY_EQUAL0_STRING_MAC(4,
                 fdbManagerPtr->apiLearningScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_LEARNING_API_STATISTICS_TOTAL_NO_SPACE_MSG_E],
                 "No space message counter is not as expected");
...
)
{
    ...
    GT_U32                                                  ii;
    CPSS_DXCH_BRG_FDB_MANAGER_STATISTICS_STC                statistics;
    CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_ENT      maxMacVlanLookUpMode;
    ...
    rc = cpssDxChBrgFdbManagerEntryGet(fdbManagerId, GT_FALSE, &updatedEntry);
    ...
    if(is_sip6_xor)
    {
        maxMacVlanLookUpMode = CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_AND_FID_E;
    }
    else
    {
        maxMacVlanLookUpMode = CPSS_DXCH_BRG_FDB_MANAGER_MAC_VLAN_LOOKUP_MODE_MAC_AND_FID_AND_VID1_E;
    }

    for(;vlanLookUpMode<=maxMacVlanLookUpMode; vlanLookUpMode++)
    {
...
)
{
    ...
    GT_STATUS                                               rc;
    rc = cpssDxChBrgFdbManagerStatisticsGet(fdbManagerId, &statistics);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "ERROR: cpssDxChBrgFdbManagerStatisticsGet on manager[%d]", fdbManagerId);
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(agingScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_OK_E],
                                 ...
                                 "ERROR: cpssDxChBrgFdbManagerStatisticsGet on manager[%d]", fdbManagerId);
...
)
{
    ...
    for(i = 0; i < fdbManagerPtr->agingBinInfo.totalAgeBinAllocated; i++)
    {
        ...
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "ERROR: cpssDxChBrgFdbManagerAgingScan on manager[%d]", fdbManagerId);
    ...
    for(i = 0; i < fdbManagerPtr->agingBinInfo.totalAgeBinAllocated; i++)
    {
        ...
        if((fdbManagerPtr->agingBinInfo.level1UsageBitmapPtr[i >> 5] & (1 << (i & 0x1F))) != 0)
        {
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_FALSE, "ERROR: level1 usage matrix check failed on manager[%d]", fdbManagerId);
        }
        if( (i != 0) && ((i % 32) == 0) &&
            ((fdbManagerPtr->agingBinInfo.level2UsageBitmap & (1 << (i >>  5))) != 0) )
        {
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_FALSE, "ERROR: level2 usage matrix check failed on manager[%d]", fdbManagerId);
        }
    }
    ...
    rc = cpssDxChBrgFdbManagerStatisticsGet(fdbManagerId, &statistics);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgFdbManagerStatisticsGet: expected to GT_OK on manager[%d]", fdbManagerId);
    UTF_VERIFY_EQUAL0_STRING_MAC(statistics.entryAddOkRehashingStage0, 4,
            "cpssDxChBrgFdbManagerStatisticsGet: Added entry is not as expected");
    ...
    for(packetCnt = 0; packetCnt < 4; packetCnt++)
    {
        rc = cpssDxChBrgFdbManagerEntryGetNext(fdbManagerId, getFirst, GT_TRUE, &entry);
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN_MAC(GT_OK, rc,
                "cpssDxChBrgFdbManagerEntryGetNext: expected to GT_OK");
        switch(entry.fdbEntryType)
        {
            case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_MAC_ADDR_E:
                UTF_VERIFY_EQUAL0_STRING_NO_RETURN_MAC(1, entry.format.fdbEntryMacAddrFormat.age, "Expected Age bit 1");
                break;
            default:
                PRV_UTF_LOG0_MAC("Unexpected entry type\n");
                return;
        }
        getFirst = GT_FALSE;
    }
    ...
    {
        rc = cpssDxChBrgFdbManagerEntryGetNext(fdbManagerId, getFirst, GT_TRUE, &entry);
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN_MAC(GT_OK, rc, "cpssDxChBrgFdbManagerEntryGetNext: expected to GT_OK");
        switch(entry.fdbEntryType)
        {
            case CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_MAC_ADDR_E:
                UTF_VERIFY_EQUAL0_STRING_NO_RETURN_MAC(0, entry.format.fdbEntryMacAddrFormat.age, "Expected Age bit 1");
                break;
            default:
                PRV_UTF_LOG0_MAC("Unexpected entry type\n");
                return;
    ...
    UTF_VERIFY_EQUAL0_STRING_MAC(statistics.scanAgingTotalAgedOutEntries, 4,
            "cpssDxChBrgFdbManagerStatisticsGet: Aged out is not as expected");
    ...
    for(packetCnt = 0; packetCnt < 4; packetCnt++)
    {
        ...
        cpssOsMemCpy(macEntryKey.key.macVlan.macAddr.arEther, &prvTgfPacketL2Part.saMac, sizeof(TGF_MAC_ADDR));
        /* Calculate the index of the inserted mac address for later use */
        rc = prvTgfBrgFdbMacEntryIndexFind(&macEntryKey, &hwIndexArr[packetCnt]);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, rc);
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgFdbManagerStatisticsGet: expected to GT_OK on manager[%d]", fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(statistics.entryUpdateOk, 4,
            "cpssDxChBrgFdbManagerStatisticsGet: Aged out deleted is not as expected on manager[%d]", fdbManagerId);
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgFdbManagerStatisticsGet: expected to GT_OK on manager[%d]", fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(statistics.scanAgingTotalAgedOutEntries, 4,
            "cpssDxChBrgFdbManagerStatisticsGet: Aged out is not as expected on manager[%d]", fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(statistics.scanAgingTotalAgedOutDeleteEntries, 0,
            "cpssDxChBrgFdbManagerStatisticsGet: Aged out deleted is not as expected on manager[%d]", fdbManagerId);



    /*** CASE_5: Set age-bit for entries per tile - entries in tiles with the age bit set should not be aged-out  ***/

    /* Clear Statistics */
    rc = cpssDxChBrgFdbManagerStatisticsClear(fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgFdbManagerStatisticsClear: expected to GT_OK on manager[%d]", fdbManagerId);

    cpssOsMemSet(&agingScanStatisticsArr, 0, sizeof(agingScanStatisticsArr));
    /* The total number of entries available for aging */
    agingScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_E] = 4;

    if(PRV_CPSS_PP_MAC(prvTgfDevNum)->multiPipe.numOfTiles > 1)
    {
        usePortGroupsBmp = GT_TRUE;
        /* loop on all the tiles in the device */
        for(tileId = 0; tileId < PRV_CPSS_PP_MAC(prvTgfDevNum)->multiPipe.numOfTiles; tileId++)
        {
            portGroupId = tileId * PRV_CPSS_PP_MAC(prvTgfDevNum)->multiPipe.numOfPipesPerTile;

            currPortGroupsBmp = BIT_0 << portGroupId;
            ...
            {
                age = GT_FALSE;
                /* Decrease the total number of entries that should be aged out */
                agingScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_E]--;
            }

            /* Set age bit in specific FDB entry per tile */
            rc = prvTgfBrgFdbMacEntryAgeBitSet(hwIndexArr[tileId % 4], age);
            UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, rc);
        }
    }
    else
    {
        /* Set age bit in specific FDB entry for all tiles */
        rc = prvTgfBrgFdbMacEntryAgeBitSet(hwIndexArr[0], GT_TRUE);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, rc);
        /* Set the total number of entries that should be aged out   */
        agingScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_E] = 3;
    }

    /* Call second iteration for aging scan - only entries without age bit set should be aged-out */
    for(i = 0; i < fdbManagerPtr->agingBinInfo.totalAgeBinAllocated; i++)
    {
        rc = cpssDxChBrgFdbManagerAgingScan(fdbManagerId, &ageParams, entriesAgingArray, &entriesAgingNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "ERROR: cpssDxChBrgFdbManagerAgingScan on manager[%d]", fdbManagerId);
    ...
    /* Verify that only entries without age bit set should be aged-out */
    rc = cpssDxChBrgFdbManagerStatisticsGet(fdbManagerId, &statistics);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgFdbManagerStatisticsGet: expected to GT_OK on manager[%d]", fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(agingScanStatisticsArr[PRV_CPSS_DXCH_FDB_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_E],
...
)
{
    ...
    if (is_sip6_xor == GT_FALSE)
    {
        ...
        if(SHADOW_TYPE_PER_DEVICE() != CPSS_DXCH_BRG_FDB_MANAGER_SHADOW_TYPE_SIP5_SIP6_HYBRID_E)
        {
            /* Add IPV6 Entry */
            cpssOsMemSet(&entry ,0,sizeof(CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_STC));
            entry.fdbEntryType                                          = CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_IPV6_UC_E;
...
static GT_VOID prvTgfBrgFdbManagerHashTest1_non_stream_verify(GT_VOID)
{
    ...
    for(entiesCnt = 0; entriesToAdd > numEntriesAdded; entiesCnt++)
    {
        entryAdded = GT_FALSE;
        fdbManagerEntryAdd_macAddr(hash_fdbManagerId, entiesCnt, &entryAdded);
        if(entryAdded == GT_TRUE)
    ...
    entriesToAdd = 1024;
    prvSpecificFidFilterValue = PRV_TGF_FDB_MANAGER_SHARED_FID_CNS;
    for(numEntriesAdded = 0, entiesCnt = 0; entriesToAdd > numEntriesAdded; entiesCnt++)
    {
        entryAdded = GT_FALSE;
        /* 128 entries already added,
         * Continue the iterationNum for fdbManagerEntryAdd_macAddr */
        fdbManagerEntryAdd_macAddr(hash_fdbManagerId, entiesCnt+128, &entryAdded);
        if(entryAdded == GT_TRUE)
        {
            numEntriesAdded+=1;
        }
...
GT_VOID prvTgfBrgFdbManager_clean()
{
    ...
    FDB_SIZE_FULL_INIT();
    prvTgfBrgFdbManager_restoreFdbConfig();
...
/**
* @internal prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationRestore function
* @endinternal
*
* @brief   Set LTT Route Configuration
*/
static GT_VOID prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationRestore
(
    GT_VOID
)
{
...
/**
* @internal prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationSet function
* @endinternal
*
* @brief   Set LTT Route Configuration
*/
static GT_VOID prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationSet
(
    GT_U32                                  fdbManagerId
)
{
    ...
    fdbManagerEntryAdd_ipv6Addr(fdbManagerId, &entryAdded);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_TRUE, entryAdded, "fdbManagerEntryAdd_ipv6Addr: %d",
                             prvTgfDevNum);
}
...
GT_VOID prvTgfBrgFdbManagerBasicRoutingTest()
{
    ...
    prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationSet(fdbManagerId);
    ...
    prvTgfFdbManagerBasicIpv6UcRoutingRouteConfigurationRestore();

    CLEANUP_MANAGER(fdbManagerId);
}

static GT_ETHERADDR     cv_mac_addr_1 = {{0xcc,0x66,0x7a,0xf1,0xca,0x1a}};
static GT_ETHERADDR     cv_mac_addr_2 = {{0xf2,0xc0,0x67,0xfd,0x66,0x2c}};
static GT_ETHERADDR     cv_mac_addr_3 = {{0x6e,0xbc,0x93,0x5d,0x3d,0xf0}};
static GT_ETHERADDR     cv_mac_addr_4 = {{0x68,0x94,0xd4,0x55,0xe9,0x09}};
static GT_ETHERADDR     cv_mac_addr_5 = {{0x00,0x11,0x22,0x33,0x44,0x55}};
static GT_ETHERADDR     cv_mac_addr_6 = {{0x00,0x11,0x22,0x33,0x44,0x66}};
static GT_ETHERADDR     cv_mac_addr_7 = {{0x00,0x11,0x22,0x33,0x44,0x77}};
static GT_ETHERADDR     cv_mac_addr_8 = {{0x00,0x11,0x22,0x33,0x44,0x88}};
static GT_ETHERADDR     cv_mac_addr_9 = {{0x00,0x11,0x22,0x33,0x44,0x99}};
static GT_ETHERADDR     cv_mac_addr_10 = {{0x00,0x11,0x22,0x33,0x44,0xaa}};
static GT_ETHERADDR     cv_mac_addr_11 = {{0x00,0x11,0x22,0x33,0x44,0xbb}};

static CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_INTERFACE_INFO_STC cv_dstInterface_trg;
static CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_INTERFACE_INFO_STC cv_srcInterface;
static GT_U32 cv_dstInterface_Index = 0; /* index in prvTgfPortsArray[] */
static GT_U32 cv_srcInterface_Index = 2; /* index in prvTgfPortsArray[] */
static GT_U32 cv_otherInterface_Index = 3; /* index in prvTgfPortsArray[] */
static GT_U32 cv_streamId = 1;
static GT_U32 cv_packet_size = 64;
static GT_U32 cv_burstCount = 2;
static GT_U32 timerSeconds, timerNsec;

#define PRINT_TIME_STAMP(fmt, ...)                                        \
    {                                                           \
        cpssOsTimeRT(&timerSeconds, &timerNsec);                \
        cpssOsPrintf("[UTF]: [%s] line[%d] timestamp [%d.%09d] " fmt "\n",\
            globalDbFuncName,__LINE__,timerSeconds,timerNsec,   \
            ## __VA_ARGS__);  \
    }

static GT_VOID  CV_FdbManagerEntryAdd_MAC(
    IN GT_U32           fdbManagerId,
    IN GT_U32           fid,
...
)
{
    ...
    CLEAR_VAR_MAC(entry);
    CLEAR_VAR_MAC(fdbEntryAddInfo);
    entry.fdbEntryType = CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_MAC_ADDR_E;
    cpssOsMemCpy(entry.format.fdbEntryMacAddrFormat.macAddr.arEther,
                 macAddr->arEther,
                 sizeof(macAddr->arEther));
    entry.format.fdbEntryMacAddrFormat.fid = fid;
    entry.format.fdbEntryMacAddrFormat.dstInterface = *dstInterfacePtr;
    entry.format.fdbEntryMacAddrFormat.isStatic = isStatic;
...
static GT_VOID  CV_Create_FDB_manager_Add_device_Add_entries(
    IN GT_U32 fdbManagerId,
    IN GT_U32 fdbCapacity
)
{
    GT_STATUS   st,rc;
    GT_U8  devListArr[1];
    ...
    PRINT_TIME_STAMP("start");
/*
    Create_FDB_manager_Add_device_Add_entries(self):
        1. Create FDB manager
        2. Add entry to FDB manager (before device is using FDB manager)
        3. Add device to FDB manager
        4. Add another entry to FDB manager (after device is using FDB manager)
        both entries added (before and after) expected to be learn on device FDB
*/
    /* create fdb manager , without device */
    rc = createFdbManagerWithoutDevice(fdbManagerId, fdbCapacity, NULL, GT_FALSE);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "createFdbManager: expected to GT_OK on manager [%d]",
            fdbManagerId);
    ...
    CV_FdbManagerEntryAdd_MAC(fdbManagerId,
        PRV_TGF_VLANID_CNS/*fid=vid*/,
        &cv_mac_addr_1,
        &cv_dstInterface_trg,
        GT_TRUE/*isStatic*/);

    /* add device to the manager (after we added first fdb entry) */
    devListArr[0] = prvTgfDevNum;
    st = cpssDxChBrgFdbManagerDevListAdd(fdbManagerId,devListArr,numOfDevs);
    UTF_VERIFY_EQUAL1_STRING_NO_RETURN_MAC(GT_OK, st,
            "cpssDxChBrgFdbManagerDevListAdd: expected to GT_OK on manager [%d]",
...
static GT_VOID CV_FdbManagerCountersGet
(
    IN GT_U32 fdbManagerId,
    IN GT_U32 expected_usedEntries,
    IN GT_U32 expected_freeEntries,
    IN GT_U32 expected_macUnicastStatic,
    IN GT_U32 expected_macUnicastDynamic
)
{
    GT_STATUS   rc;
    CPSS_DXCH_BRG_FDB_MANAGER_COUNTERS_STC  getCounters;
    ...
    /* Verify Counter */
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_usedEntries,      getCounters.usedEntries,      "getCounters: usedEntries");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_freeEntries,      getCounters.freeEntries,      "getCounters: freeEntries");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_macUnicastStatic, getCounters.macUnicastStatic, "getCounters: macUnicastStatic");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_macUnicastDynamic,getCounters.macUnicastDynamic, "getCounters: macUnicastDynamic");

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_FdbManagerStatisticsGet(
    IN GT_U32 fdbManagerId,
    IN GT_U32 expected_entryAddOkRehashingStage0,
    IN GT_U32 expected_scanLearningOkNoMore,
    IN GT_U32 expected_scanLearningTotalHwEntryNewMessages,
    IN GT_U32 expected_entryDeleteOk
)
{
    CPSS_DXCH_BRG_FDB_MANAGER_STATISTICS_STC                statistics;
    GT_STATUS                                               rc;
    ...
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_entryAddOkRehashingStage0, statistics.entryAddOkRehashingStage0,  "statistics.entryAddOkRehashingStage0");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_scanLearningOkNoMore, statistics.scanLearningOkNoMore,  "statistics.scanLearningOkNoMore");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_scanLearningTotalHwEntryNewMessages, statistics.scanLearningTotalHwEntryNewMessages,  "statistics.scanLearningTotalHwEntryNewMessages");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_entryDeleteOk, statistics.entryDeleteOk,  "statistics.entryDeleteOk");
...
/*  Read FDB Manager Entries)*/
static GT_STATUS cpssDxChEmbComFdbManagerEntryRefresh
(
    IN GT_U32 fdbManagerId,
...
)
{
    ...
    while (rc == GT_OK)
    {
        rc = cpssDxChBrgFdbManagerEntryGetNext(fdbManagerId, getFirst, GT_FALSE, &entry);
        if (rc == GT_OK)
        {
            numOfEntries++;
        }
    ...
    *numOfEntriesPtr = numOfEntries;

    PRINT_TIME_STAMP("end");
    return GT_OK;
}

static GT_STATUS cpssDxChEmbComGetGroupFDBTableEntriesNumb
(
    IN GT_U8                   devNum,
...
)
{
    ...
    *numOfEntriesPtr = (numOfValid - numOfSkip);
...
static GT_U32   CV_FdbManagerEntryRefresh(
    IN GT_U32   fdbManagerId
)
{
    GT_STATUS        rc;
    ...
    rc = cpssDxChEmbComFdbManagerEntryRefresh(fdbManagerId,&numOfEntries);
...
static GT_U32   CV_FdbNumOfEntries(
    IN GT_U8   devNum
)
{
    GT_STATUS        rc;
    ...
    rc = cpssDxChEmbComGetGroupFDBTableEntriesNumb(devNum,0/*portGroupId*/,&numOfEntries);
    UTF_VERIFY_EQUAL0_NO_RETURN_MAC(GT_OK, rc);
...
static GT_VOID CV_FDB_MANAGER_and_HW_FDB_entries_amount_check(
    IN GT_U32   fdbManagerId,
    IN  GT_U32  expectedAmount
)
{
    GT_U32  numOfSwEntries,numOfHwEntries;

    PRINT_TIME_STAMP("start");

    cpssOsTimerWkAfter(10);/*time.sleep(0.01)*/

    numOfSwEntries = CV_FdbManagerEntryRefresh(fdbManagerId);
    cpssOsPrintf("FDB manager (SW) number of entries [%d] \n",numOfSwEntries);
    numOfHwEntries = CV_FdbNumOfEntries(prvTgfDevNum);
    cpssOsPrintf("FDB (HW) number of entries [%d] \n",numOfHwEntries);

    if((expectedAmount == numOfHwEntries) && (expectedAmount==numOfSwEntries))
    {
        cpssOsPrintf("Good. There are [%d] entries on both FDB manager (SW) and FDB (HW) \n",expectedAmount);
    }
    else
    {
        cpssOsPrintf("Fail!!! Expected [%d] entries. got FDB manager (SW) [%d] entries. got FDB (HW) [%d] entries. \n",
            expectedAmount,
            numOfSwEntries,
            numOfHwEntries);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_BAD_STATE,
                "CV_FDB_MANAGER_and_HW_FDB_entries_amount_check: num entries not as expected [%d]",
                fdbManagerId);
    }
...
static GT_VOID CV_Read_FDB_manager(
    IN GT_U32           fdbManagerId,
    IN GT_U32           key_fid,
    IN GT_ETHERADDR     *key_macAddr,
    IN CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_INTERFACE_INFO_STC *expected_expectedInterfacePtr,
    IN GT_BOOL          expected_isStatic,
    IN GT_U32           checkTotalNumOfEntries/* ignored if 0*/
)
{
    GT_STATUS   st;
    CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_STC           entryGet;
    ...
    if(checkTotalNumOfEntries)
    {
        CV_FDB_MANAGER_and_HW_FDB_entries_amount_check(fdbManagerId , checkTotalNumOfEntries);
    }

    CLEAR_VAR_MAC(entryGet);
    entryGet.fdbEntryType = CPSS_DXCH_BRG_FDB_MANAGER_ENTRY_TYPE_MAC_ADDR_E;
    cpssOsMemCpy(entryGet.format.fdbEntryMacAddrFormat.macAddr.arEther,
                 key_macAddr->arEther,
                 sizeof(key_macAddr->arEther));
    entryGet.format.fdbEntryMacAddrFormat.fid = key_fid;

    st = cpssDxChBrgFdbManagerEntryGet(fdbManagerId, GT_FALSE/*getAgeStatusFromHw*/, &entryGet);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
            "cpssDxChBrgFdbManagerEntryGet: expected GT_OK in manager [%d]",
            fdbManagerId);

    UTF_VERIFY_EQUAL0_STRING_MAC(expected_isStatic,      entryGet.format.fdbEntryMacAddrFormat.isStatic,      "entryGet: isStatic");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_expectedInterfacePtr->type,entryGet.format.fdbEntryMacAddrFormat.dstInterface.type,      "entryGet: dstInterface.type");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_expectedInterfacePtr->interfaceInfo.devPort.hwDevNum,entryGet.format.fdbEntryMacAddrFormat.dstInterface.interfaceInfo.devPort.hwDevNum,      "entryGet: dstInterface.interfaceInfo.devPort.hwDevNum");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_expectedInterfacePtr->interfaceInfo.devPort.portNum,entryGet.format.fdbEntryMacAddrFormat.dstInterface.interfaceInfo.devPort.portNum,      "entryGet: dstInterface.interfaceInfo.devPort.portNum");

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_clear_TG_counters(GT_VOID)
{
    GT_STATUS               rc;
    GT_U8                   portIter;
    CPSS_INTERFACE_INFO_STC portInterface;
    ...
    for (portIter = 0; portIter < prvTgfPortsNum; portIter++)
    {
        ...
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorPortForceLinkUpEnableSet: %d, %d",
                prvTgfDevNum, prvTgfPortsArray[portIter]);

        rc = tgfTrafficGeneratorPortCountersEthReset(&portInterface);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorPortCountersEthReset: %d, %d",
                prvTgfDevNum, prvTgfPortsArray[portIter]);

        rc = tgfTrafficGeneratorXmlPacketPartsDelete(prvTgfDevNum,prvTgfPortsArray[portIter],GT_NA);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorXmlPacketPartsDelete: %d, %d",
                prvTgfDevNum, prvTgfPortsArray[portIter]);
    }
...
static GT_VOID CV_Tx_setup_L2_BURST
(
    IN GT_U32           senderPortNum,
    IN GT_ETHERADDR     *macAddr_da,
    IN GT_ETHERADDR     *macAddr_sa,
    IN GT_U32           vlanId,
...
)
{
    ...
    CLEAR_VAR_MAC(partInfo);

    cpssOsMemCpy(partInfo.l2Info.daMac.arEther,
                 macAddr_da->arEther,
                 sizeof(macAddr_da->arEther));
    cpssOsMemCpy(partInfo.l2Info.saMac.arEther,
                 macAddr_sa->arEther,
                 sizeof(macAddr_sa->arEther));
    partIndex = 1;
    rc = tgfTrafficGeneratorXmlPacketPartSet(prvTgfDevNum,
        senderPortNum,
        cv_streamId/*packetIndex*/,
        partIndex/*partIndex*/,
        TGF_PACKET_PART_L2_E/*partType*/,
        &partInfo);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorXmlPacketPartSet: %d, %d",
            prvTgfDevNum, senderPortNum);

    partIndex++;

    CLEAR_VAR_MAC(partInfo);
    partInfo.vlanTagInfo.etherType = TGF_ETHERTYPE_8100_VLAN_TAG_CNS;
    partInfo.vlanTagInfo.pri = 0;
    partInfo.vlanTagInfo.cfi = 0;
    partInfo.vlanTagInfo.vid = (TGF_VLAN_ID)vlanId;

    rc = tgfTrafficGeneratorXmlPacketPartSet(prvTgfDevNum,
        senderPortNum,
        cv_streamId/*packetIndex*/,
        partIndex,
        TGF_PACKET_PART_VLAN_TAG_E/*partType*/,
        &partInfo);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorXmlPacketPartSet: %d, %d",
            prvTgfDevNum, senderPortNum);

    rc = tgfTrafficGeneratorXmlPacketSizeSet(prvTgfDevNum,
        senderPortNum,
        cv_streamId/*packetIndex*/,
        packetSize);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorXmlPacketSizeSet: %d, %d",
            prvTgfDevNum, senderPortNum);

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_start_transmit
(
    IN GT_U32           senderPortNum,
...
)
{
    ...
    rc = tgfTrafficGeneratorXmlPacketTransmit(prvTgfDevNum,
        senderPortNum,
        cv_streamId/*packetIndex*/,
        burstCount,
        0/*sleepAfterXCount*/,
    ...
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorXmlPacketTransmit: %d, %d",
            prvTgfDevNum, senderPortNum);

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_Check_TG_counters(
    IN GT_U32           portNum,
    IN GT_U32           expected_txFrames,
    IN GT_U32           expected_rxFrames
)
{
    GT_STATUS            rc;
    CPSS_INTERFACE_INFO_STC         portInterface;
    ...
    rc = tgfTrafficGeneratorPortCountersEthRead(&portInterface,&counters);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorPortCountersEthRead: %d, %d",
            prvTgfDevNum, portNum);

    UTF_VERIFY_EQUAL0_STRING_MAC(expected_txFrames, counters.goodPktsRcv .l[0], "entryGet: expected_txFrames");
    UTF_VERIFY_EQUAL0_STRING_MAC(expected_rxFrames, counters.goodPktsSent.l[0], "entryGet: expected_rxFrames");

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_Send_UC_and_check_it_was_received
(
    IN GT_U32           senderPortNum,
    IN GT_U32           expectedTrgPortNum,
    IN GT_ETHERADDR     *macAddr_da,
    IN GT_ETHERADDR     *macAddr_sa
)
{
    ...
    CV_clear_TG_counters();

    CV_Tx_setup_L2_BURST(senderPortNum,
        macAddr_da,
        macAddr_sa,
        PRV_TGF_VLANID_CNS ,
        cv_packet_size);

    CV_start_transmit(senderPortNum,cv_burstCount);

    /*check TG counters*/
    for (portIter = 0; portIter < prvTgfPortsNum; portIter++)
    {
        if(senderPortNum == prvTgfPortsArray[portIter])
        {
            CV_Check_TG_counters(senderPortNum, cv_burstCount/*txFrames*/, 0 /*rxFrames*/);
        }
        else
        if(expectedTrgPortNum == prvTgfPortsArray[portIter])
        {
            CV_Check_TG_counters(expectedTrgPortNum, 0/*txFrames*/, cv_burstCount /*rxFrames*/);
        }
        else
        {
            CV_Check_TG_counters(prvTgfPortsArray[portIter], 0/*txFrames*/, 0 /*rxFrames*/);
        }
    }
...
static GT_VOID CV_Delete_FDB_manager_entry(
    IN GT_U32           fdbManagerId,
    IN GT_U32           key_fid,
    IN GT_ETHERADDR     *key_macAddr
)
{
    ...
    cpssOsMemCpy(entry.format.fdbEntryMacAddrFormat.macAddr.arEther,
                 key_macAddr->arEther,
                 sizeof(key_macAddr->arEther));
    entry.format.fdbEntryMacAddrFormat.fid = key_fid;

    st = cpssDxChBrgFdbManagerEntryDelete(fdbManagerId, &entry);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
            "cpssDxChBrgFdbManagerEntryDelete: expected GT_OK in manager [%d]",
            fdbManagerId);

    PRINT_TIME_STAMP("end");
}

static GT_VOID CV_Send_unknown_UC_and_check_flooding(
    IN GT_U32           senderPortNum,
    IN GT_ETHERADDR     *macAddr_da,
    IN GT_ETHERADDR     *macAddr_sa
)
{
    ...
    for (portIter = 0; portIter < prvTgfPortsNum; portIter++)
    {
        ...
        {
            CV_Check_TG_counters(prvTgfPortsArray[portIter], 0/*txFrames*/, cv_burstCount /*rxFrames*/);
        }
    }
...
static GT_VOID CV_VlanInvalidate(
    IN GT_U8   devNum,
    IN GT_U16  vlanId
)
{
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgVlanEntryInvalidate: vlanId [%d]",
            vlanId);
...
static GT_VOID CV_FdbManagerDelete(
    IN GT_U32   fdbManagerId
)
{
    GT_STATUS   rc;
    ...
    rc = cpssDxChBrgFdbManagerDelete(fdbManagerId);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "cpssDxChBrgVlanEntryInvalidate: fdbManagerId [%d]",
            fdbManagerId);
...
static GT_VOID CV_FDB_Manager_Negate(
    IN GT_U32   fdbManagerId,
    IN GT_U16   vlanId
)
{
    ...
    CV_VlanInvalidate(prvTgfDevNum,vlanId);

    /*'shell-execute appDemoFdbManagerControlSet 0, 0, 0'*/
    rc = prvWrAppDemoFdbManagerControlSet(APP_DEMO_AU_MESSAGE_HANDLER_LOW_LEVEL_E, GT_FALSE, 0/*agingPeriod*/);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
            "prvWrAppDemoFdbManagerControlSet: expected to GT_OK on manager [%d]",
            fdbManagerId);

    CV_FdbManagerDelete(fdbManagerId);

    PRINT_TIME_STAMP("end");
}

static GT_VOID free_allocated_memory(void) /*memLeakageSkippedTestArray*/
{
    PRINT_TIME_STAMP("start");

    tgfTrafficGeneratorXmlPacketPartsDelete(prvTgfDevNum,
        GT_NA,
        GT_NA);

    tgfTrafficGeneratorXmlPacketVfdsDelete(prvTgfDevNum,GT_NA);
...
)
{
    ...
    FdbMaxSize = FDB_SIZE_FULL;/* need to come after FDB_SIZE_FULL_INIT() */

    /* prepare init values */
    cv_dstInterface_trg.type = CPSS_INTERFACE_PORT_E;
    cv_dstInterface_trg.interfaceInfo.devPort.hwDevNum = PRV_CPSS_HW_DEV_NUM_MAC(prvTgfDevNum);
    cv_dstInterface_trg.interfaceInfo.devPort.portNum  = prvTgfPortsArray[cv_dstInterface_Index];

    cv_srcInterface.type = CPSS_INTERFACE_PORT_E;
    cv_srcInterface.interfaceInfo.devPort.hwDevNum = PRV_CPSS_HW_DEV_NUM_MAC(prvTgfDevNum);
    cv_srcInterface.interfaceInfo.devPort.portNum  = prvTgfPortsArray[cv_srcInterface_Index];

    PRV_UTF_LOG0_MAC("======= test started =======\n");
    ...
    /* create the manager and add 2 mac addresses (one before and one after adding the device to the manager)*/
    CV_Create_FDB_manager_Add_device_Add_entries(fdbManagerId,FdbMaxSize);

    CV_FdbManagerCountersGet(fdbManagerId,
        2/*usedEntries*/,
        FdbMaxSize - 2/*freeEntries*/,
        2/*macUnicastStatic*/,
        0/*macUnicastDynamic*/);
    CV_FdbManagerStatisticsGet(fdbManagerId,
        2/*entryAddOkRehashingStage0*/,
        0/*scanLearningOkNoMore*/,
        0/*scanLearningTotalHwEntryNewMessages*/,
        0/*expected_entryDeleteOk*/);
    /* check that the 2 entries exists in the manager */
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_1,&cv_dstInterface_trg,GT_TRUE/*isStatic*/,2/*checkTotalNumOfEntries*/);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_2,&cv_dstInterface_trg,GT_TRUE/*isStatic*/,0);

    CV_STEP(4);
    CV_Send_UC_and_check_it_was_received(
        prvTgfPortsArray[cv_srcInterface_Index],
        prvTgfPortsArray[cv_dstInterface_Index],
        &cv_mac_addr_1,
        &cv_mac_addr_3/*mac SA to be learned*/); /*cv_mac_addr_1 was added to FDB manager before device was added to manager */
    ...
    CV_Send_UC_and_check_it_was_received(
        ...
        prvTgfPortsArray[cv_dstInterface_Index],
        &cv_mac_addr_2,
        &cv_mac_addr_4/*mac SA to be learned*/); /*cv_mac_addr_2 was added to FDB manager before device was added to manager */

    CV_STEP(6);
    CV_FdbManagerCountersGet(fdbManagerId,
        4/*usedEntries*/,
        FdbMaxSize - 4/*freeEntries*/,
        2/*macUnicastStatic*/,
        2/*macUnicastDynamic*/);
    CV_FdbManagerStatisticsGet(fdbManagerId,
        2/*entryAddOkRehashingStage0*/,
        2/*scanLearningOkNoMore*/,
        2/*scanLearningTotalHwEntryNewMessages*/,
        0/*expected_entryDeleteOk*/);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_1,&cv_dstInterface_trg,GT_TRUE/*isStatic*/,4/*checkTotalNumOfEntries*/);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_2,&cv_dstInterface_trg,GT_TRUE/*isStatic*/,0);
    /* check the additional macs (as macSA) leared by traffic */
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_3,&cv_srcInterface,GT_FALSE/*not isStatic*/,0);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_4,&cv_srcInterface,GT_FALSE/*not isStatic*/,0);

    CV_STEP(7);
    CV_Delete_FDB_manager_entry(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_1);

    CV_FdbManagerCountersGet(fdbManagerId,
        3/*usedEntries*/,
        FdbMaxSize - 3/*freeEntries*/,
        1/*macUnicastStatic*/,
        2/*macUnicastDynamic*/);
    CV_FdbManagerStatisticsGet(fdbManagerId,
        0/*entryAddOkRehashingStage0*/,
        0/*scanLearningOkNoMore*/,
        0/*scanLearningTotalHwEntryNewMessages*/,
        1/*entryDeleteOk*/);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_2,&cv_dstInterface_trg,GT_TRUE/*isStatic*/,3/*checkTotalNumOfEntries*/);
    /* check the additional macs (as macSA) leared by traffic */
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_3,&cv_srcInterface,GT_FALSE/*not isStatic*/,0);
    CV_Read_FDB_manager(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_4,&cv_srcInterface,GT_FALSE/*not isStatic*/,0);

    CV_STEP(8);
    CV_Send_unknown_UC_and_check_flooding(
        prvTgfPortsArray[cv_srcInterface_Index],
        &cv_mac_addr_1,
        &cv_mac_addr_5);
    CV_FdbManagerCountersGet(fdbManagerId,
    ...
    CV_Send_UC_and_check_it_was_received(
        prvTgfPortsArray[cv_otherInterface_Index],
        prvTgfPortsArray[cv_dstInterface_Index],
        &cv_mac_addr_2,
        &cv_mac_addr_6/*mac SA to be learned*/);

    CV_Send_UC_and_check_it_was_received(
        prvTgfPortsArray[cv_otherInterface_Index],
        prvTgfPortsArray[cv_srcInterface_Index],
        &cv_mac_addr_3,
        &cv_mac_addr_7/*mac SA to be learned*/);

    CV_Send_UC_and_check_it_was_received(
        prvTgfPortsArray[cv_otherInterface_Index],
        prvTgfPortsArray[cv_srcInterface_Index],
        &cv_mac_addr_4,
        &cv_mac_addr_8/*mac SA to be learned*/);

    CV_FdbManagerCountersGet(fdbManagerId,
        7/*usedEntries*/,
        FdbMaxSize - 7/*freeEntries*/,
        1/*macUnicastStatic*/,
        6/*macUnicastDynamic*/);

    /*Delete all old entries (another 3)*/
    CV_Delete_FDB_manager_entry(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_2);
    CV_Delete_FDB_manager_entry(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_3);
    CV_Delete_FDB_manager_entry(fdbManagerId,PRV_TGF_VLANID_CNS,&cv_mac_addr_4);
    ...
    /*check all random macs were learned - 7 packets were sent since phase 8*/
    CV_FDB_MANAGER_and_HW_FDB_entries_amount_check(fdbManagerId , 7);
    CV_FdbManagerCountersGet(fdbManagerId,
        7/*usedEntries*/,
        FdbMaxSize - 7/*freeEntries*/,
        0/*macUnicastStatic*/,
    ...
    CV_STEP(9);
    /*Negate*/
    CV_FDB_Manager_Negate(fdbManagerId, PRV_TGF_VLANID_CNS);
