...
{
    /* test not ready for multi core devices */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_BOBCAT3_E | UTF_FALCON_E );
...
{
    /* test not ready for multi core devices */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_BOBCAT3_E | UTF_FALCON_E );
...
/*
    AUTODOC: tgfCncEgrPacketTypePassDropIndexMode
    Test on EREP unit for CNC client Egress packet Type Pass/drop client for index mode.
    Apllicable device : Ironman
    This test perfoms
        1. Configures Cnc Client egress packet type pass/drop client with index mode.
        2. Test different cnc index by setting
           egress PCL rule to Forward, mirror, trap, soft drop and hard drop packet command
           and configure cpu code for drop and mirror/trap case.
        3. Verify the counter entries.
...
*/
UTF_TEST_CASE_MAC(tgfCncEgrPacketTypePassDropIndexMode)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~UTF_CPSS_PP_ALL_SIP6_30_CNS));

    /* skip this test for all families under Golden Model */
    PRV_TGF_SKIP_LONG_TEST_GM_MAC(UTF_ALL_FAMILY_E);

    prvTgfCncTestEgrPacketTypeIndexModeTrafficGenerateAndCheck();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test CNC Shared client:
    Client 0: EGRESS_PACKET_TYPE_PASS_DROP with offset 0
    Client 1: INGRESS_PCL_LOOKUP_0_PARALLEL_0 with offset 128
    Client 2: EGRESS_VLAN_PASS_DROP with offset 256
    configure PCL rule and set action as cnc counting index;
    configure Egress Packet Type Pass Drop with packet command Forward;
    configure Vlan index 5
    configure other cnc general configurations and shared clients configurations
    send traffic;
    verify CNC counters increment respectively to byteCount mode
*/
UTF_TEST_CASE_MAC(prvTgfCncSharedClients)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, (~UTF_CPSS_PP_ALL_SIP6_30_CNS));

    /* Test */
    prvTgfCncSharingTrafficGenerateAndCheck();
    prvTgfCncSharingTestRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test CNC Ingress Packet Type PASS/DROP client:
    CNC Client : PACKET_TYPE_PASS_DROP client with 'FROM_CPU' as packet type
    create VLAN 5 with untagged ports [0,1,2,3]
    add FDB entry with MAC 00:11:22:33:44:00, VLAN 5, port 2
    configure cnc general configurations and client configuration
    send traffic(packet size = 64B);
    verify for CNC counter packet count and byte count value with expected value
*/
UTF_TEST_CASE_MAC(prvTgfCncIngPacketTypeClientTest)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, (~UTF_CPSS_PP_ALL_SIP6_CNS));

    /* Test Configuration, traffic transmission and CNC counter parameter(packet and byte count) values check  */
    prvTgfCncIngPacketTypeClientConfigTest();

    /* Test Restore configurations */
    prvTgfCncIngPacketTypeClientRestoreTest();
}
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfCnc)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfCncEgrPacketTypePassDropIndexMode)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfCncSharedClients)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfCncIngPacketTypeClientTest)
