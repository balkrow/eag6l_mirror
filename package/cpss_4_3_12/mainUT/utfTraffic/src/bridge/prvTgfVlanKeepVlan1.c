...
/* port number[IDX] to send traffic from CPU/TrafficGenerator */
#define PRV_TGF_SEND_PORT_IDX_CNS     0

/* egress port number[IDX] to analyze packet or to be mirrored */
#define PRV_TGF_RECEIVE_PORT_IDX_CNS  2

/* Analyzer port number[IDX] for the PRV_TGF_RECEIVE_PORT_IDX_CNS */
#define PRV_TGF_ANALYZER_PORT_IDX_CNS 3

/* Mirror analyzer-interface index */
#define PRV_TGF_ANALYZER_INDEX_CNS    5
...
static GT_BOOL prvTgfVlanKeepMirrorUsed;
static GT_BOOL prvTgfVlanKeepForceOneStep = GT_FALSE;

/******************************************************************************\
 *                            Private test functions                          *
\******************************************************************************/
/**
* @internal prvTgfVlanKeepMirrorUseCfg function
* @endinternal
*
* @brief   Modify-set test mode without/with mirroring
*
* @param[in] mirrorUsed            - to be configured
*
* @retval GT_OK                    - on success
*  Note: retval is needed only for CLI
*/
GT_STATUS prvTgfVlanKeepMirrorUseCfg
(
    IN GT_BOOL mirrorUsed
)
{
    prvTgfVlanKeepMirrorUsed = mirrorUsed;
    return GT_OK;
}
...
* @param[in] srcPort       - egress port to be mirrored from
...
)
{
    ...
    PRV_TGF_MIRROR_ANALYZER_INTERFACE_STC mirrorInterface;
    GT_U32          index;
    GT_STATUS       rc;

    if (prvTgfVlanKeepMirrorUsed != GT_TRUE)
    {
        return;
    }
    ...
    index = (set == GT_TRUE) ? PRV_TGF_ANALYZER_INDEX_CNS : 0;

    /* Configure mirroring-analyzer port */
    mirrorInterface.interface.type = CPSS_INTERFACE_PORT_E;
    mirrorInterface.interface.devPort.hwDevNum = prvTgfDevNum;
    mirrorInterface.interface.devPort.portNum = analyzerPort;
    rc = prvTgfMirrorAnalyzerInterfaceSet(index, &mirrorInterface);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfTxMirrorSet(%d) analyzer", set);

    /* Configure source/mirrored port */
    rc = prvTgfMirrorTxPortSet(srcPort, set, index);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfTxMirrorSet(%d) srcPort", set);

    if (set == GT_TRUE)
    {
        PRV_UTF_LOG2_MAC("-- Port[%u] is mirrored to analyzing port [%u]\n",
                         srcPort, analyzerPort);
    }
}
...
)
{
    ...
    {
        if ((prvTgfVlanKeepMirrorUsed == GT_TRUE) &&
            (portIter == PRV_TGF_ANALYZER_PORT_IDX_CNS))
        {
            continue; /* exclude analyzer port from vlan */
        }
...
)
{
    ...
    analyzerPort = prvTgfPortsArray[PRV_TGF_ANALYZER_PORT_IDX_CNS];
    ...
    portInterface.devPort.portNum = (prvTgfVlanKeepMirrorUsed == GT_TRUE) ?
                                    analyzerPort : destPort;
    ...
    prvTgfTxMirrorSet(GT_TRUE, destPort, analyzerPort);
...
/**
* @internal prvTgfVlanKeepForceOneStepSet function
* @endinternal
*
* @brief   Set to run every test twice (in 2 different modes) or once only
*
* @param[in] set  - GT_FALSE run twice(default), GT_TRUE run once
* @retval GT_OK   - just for CLI
*/
GT_STATUS prvTgfVlanKeepForceOneStepSet
(
    IN GT_BOOL set
)
{
    prvTgfVlanKeepForceOneStep = set;
    return GT_OK; /* this ret-OK is just for CLI */
}

/**
* @internal prvTgfVlanKeepCaller function
* @endinternal
*
* @brief   2-step caller for 1-step callbacks
*       prvTgfVlanKeepVlan1EnTagStateVlan1Vlan0DoubleTagTrafficGenerate1
*       prvTgfVlanKeepVlan1EnTagStateVlan0DoubleTagTrafficGenerate1
*       prvTgfVlanKeepVlan1EnTagStateUntaggedDoubleTagTrafficGenerate1
*       prvTgfVlanKeepVlan1EnTagStateVlan0SingleTagTrafficGenerate1
*       prvTgfVlanKeepVlan1DisTagStateVlan0DoubleTagTrafficGenerate1
*       prvTgfVlanKeepVlan1DisTagStateUntaggedDoubleTagTrafficGenerate1
*
* @param[in] pFunc  - callback prvTgfVlanKeep*TrafficGenerate
*
* By default every xxTagTrafficGenerate has 2 steps: without and with Mirror.
* This default could be changed by prvTgfVlanKeepForceOneStepSet(true).
*/
GT_VOID prvTgfVlanKeepCaller
(
    GT_VOID pFunc(GT_VOID)
)
{
    /* Mirror-mode step requires CPSS_DXCH_MIRROR_TO_ANALYZER_FORWARDING_END_TO_END_E
     * but is not supported by SIP-LEGACY devices.
     * Check all devices are SIP5 or later family.
     */
    if(!PRV_CPSS_SIP_5_CHECK_MAC(prvTgfDevNum))
    {
        PRV_UTF_LOG0_MAC("---- Step-1/1 (no MIRROR mode supported) ---\n");
        prvTgfVlanKeepForceOneStep = GT_TRUE;
        prvTgfVlanKeepMirrorUsed = GT_FALSE;
    }

    if (prvTgfVlanKeepForceOneStep == GT_TRUE)
    {
        /* run 1 step only with current prvTgfVlanKeepMirrorUsed TRUE/FALSE mode */
        pFunc();
        /* prvTgfVlanKeepVlan1ConfigurationRestore() called from UTF_TEST_CASE_MAC */
        return;
    }
    PRV_UTF_LOG0_MAC("-------- Step-1/2 (no MIRROR mode) --------------\n");
    prvTgfVlanKeepMirrorUseCfg(GT_FALSE);
    pFunc();
    prvTgfVlanKeepVlan1ConfigurationRestore();
    PRV_UTF_LOG0_MAC("-------- Step-2/2 (MIRROR mode) -----------------\n");
    prvTgfVlanKeepMirrorUseCfg(GT_TRUE);
    pFunc();
    /* prvTgfVlanKeepVlan1ConfigurationRestore() called from UTF_TEST_CASE_MAC */
}
...
*/
static GT_VOID prvTgfVlanKeepVlan1EnTagStateVlan1Vlan0DoubleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1EnTagStateVlan1Vlan0DoubleTagTrafficGenerate1);
}
...
*         Tag1 is not removed and it's value 7
*/
static GT_VOID prvTgfVlanKeepVlan1EnTagStateVlan0DoubleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1EnTagStateVlan0DoubleTagTrafficGenerate1);
}
...
*         Tag1 is not removed and it's value 7
*/
static GT_VOID prvTgfVlanKeepVlan1EnTagStateUntaggedDoubleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1EnTagStateUntaggedDoubleTagTrafficGenerate1);
}
...
*         Tag1 is removed
*/
static GT_VOID prvTgfVlanKeepVlan1EnTagStateVlan0SingleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1EnTagStateVlan0SingleTagTrafficGenerate1);
}
...
*         Tag1 is removed
*/
static GT_VOID prvTgfVlanKeepVlan1DisTagStateVlan0DoubleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1DisTagStateVlan0DoubleTagTrafficGenerate1);
}
...
*         Tag1 is removed
*/
static GT_VOID prvTgfVlanKeepVlan1DisTagStateUntaggedDoubleTagTrafficGenerate1
(
    void
)
{
...
{
    prvTgfVlanKeepCaller(prvTgfVlanKeepVlan1DisTagStateUntaggedDoubleTagTrafficGenerate1);
}
...
)
{
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "Error in prvTgfBrgVlanKeepVlan1EnableSet %d", GT_FALSE);
    ...
    prvTgfTxMirrorSet(GT_FALSE, prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS], prvTgfPortsArray[PRV_TGF_ANALYZER_PORT_IDX_CNS]);
}
