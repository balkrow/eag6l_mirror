...
#include <bridge/prvTgfBrgNestedDsaToAccess.h>
...
UTF_TEST_CASE_MAC(tgfBasicDynamicLearning_fromCascadePortDsaTagForward)
{
    ...
    */
    ...
    {   /* skip multi device boards for cascading related tests */
        ...
        else if ((boardIdx == 35) && (boardRevId == 1))
        {
...
*/
    ...
    /* Restore configuration */
    prvTgfBrgVlanManipulationUntaggedCmdDoubleTagConfigurationRestore(isWithErrorInjection);
}
...
UTF_TEST_CASE_MAC(tgfVlanManipulationUntaggedCmdDoubleTag_withErrorInjection)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum,
            UTF_XCAT3_E | UTF_LION2_E | UTF_AAS_E);
    ...
    /* asic simulation not support error injection */
...
/* AUTODOC:
   run test tgfVlanManipulationOuterTag0InnerTag1CmdDoubleTag and check when
   ingress traffic should be matched as outer tag1 , inner tag0 and will
   egress  traffic should be            outer tag0 , inner tag1

   NOTE: in sip7 need to enable cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet to allow
        ingress traffic to be matched as outer tag1 , inner tag0
*/
UTF_TEST_CASE_MAC(tgfVlanManipulationOuterTag0InnerTag1CmdDoubleTag_ingressOuterTag1InnerTag0)
{
    if (UTF_CPSS_DXCH_XCAT3X_CHECK_MAC(prvTgfDevNum))
    {
        /* test is not suitable for AC3X-SOHO systems */
        prvUtfSkipTestsSet();
        return ;
    }
    ...
    {
        /* set sip7 to run in legacy mode */
        prvTgfVlanManipulationTestAs_sip7_allow_innerTag0(GT_TRUE);
    }

    prvTgfVlanManipulationTestAs_ingressOuterTag1InnerTag0(GT_TRUE);
    UTF_TEST_CALL_MAC(tgfVlanManipulationOuterTag0InnerTag1CmdDoubleTag);

    if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        /* set sip7 to run in NON legacy mode */
        prvTgfVlanManipulationTestAs_sip7_allow_innerTag0(GT_FALSE);
        UTF_TEST_CALL_MAC(tgfVlanManipulationOuterTag0InnerTag1CmdDoubleTag);
    }

    prvTgfVlanManipulationTestAs_ingressOuterTag1InnerTag0(GT_FALSE);
...
/* AUTODOC:
    ...
    check IP counters values.
*/
UTF_TEST_CASE_MAC(prvTgfFdbBasicIpv4UcRoutingByIndex_TtiRedirectToVrfId)
{
/*
    1. Set configuration
    2. Generate traffic
...
*/
    ...
    /* Set Route configuration */
    prvTgfFdbBasicIpv4UcRoutingRouteConfigurationWithVrfIdSet(GT_TRUE,vrfId);
    ...
    /* Restore configuration */
    prvTgfFdbBasicIpv4UcRoutingConfigurationWithVrfIdRestore();
}
...
UTF_TEST_CASE_MAC(prvTgfBrgVlanDsaTagCheckOnRouting)
{
    ...
    prvTgfBrgVlanDsaTagCheckOnRoutingAllSteps();
}
...
UTF_TEST_CASE_MAC(tgfBasicDlbWaTest)
{
    ...
    /* the test is not stable on WM - skip it for cs_verify and baseline runs.
       it will be tested on board */
    PRV_TGF_SKIP_LONG_TEST_MAC(UTF_DXCH_E);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum,  UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_FALCON_E | UTF_IRONMAN_L_E | UTF_AAS_E );
...
{   /* skip multi device boards for cascading related tests */
    ...
    prvTgfBrgNestedDsaToAccessConfig();
    prvTgfBrgNestedDsaToAccessTrafficSend();
    prvTgfBrgNestedDsaToAccessConfigRestore();
}
/**
* @internal prvTgfBrgPVEMirroring UT
* @endinternal
*
* @brief   This UT configures the PCL to capture ingress control packets,
*          send traffic and test PVE Control Packets To Up link.
*/
UTF_TEST_CASE_MAC(prvTgfBrgPVEMirroring)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum,  UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E );
    prvTgfBrgPVEMirroringConfigSet();
    prvTgfBrgPVEMirroringTrafficGenerate();
    prvTgfBrgPVEMirroringConfigRestore();
}
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfBridge)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfVlanManipulationOuterTag0InnerTag1CmdDoubleTag_ingressOuterTag1InnerTag0)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfFdbBasicIpv4UcRoutingByIndex_TtiRedirectToVrfId)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfBrgNestedDsaToAccessConfig)

        UTF_SUIT_DECLARE_TEST_MAC(prvTgfBrgPVEMirroring)
