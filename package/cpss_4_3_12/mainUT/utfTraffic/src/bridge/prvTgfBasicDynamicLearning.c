...
)
{
    ...
    if(prvTgfResetModeGet() == GT_TRUE)
    {
        ...
        if(testingDsaTag == GT_TRUE)
        {
            /* provide time to create Event Handlers tasks and take care of all simulated events during init */
...
static GT_VOID prvTgfIpv4AddrPrint(IN GT_U8 *ipv4Addr)
{
    /* Print IPV4 address (DEC) - totally 17 chars with head-spaces for
     * first DOT alignment and tail/trailing spaces:
     *   >   1.23.234.234  <
     *   > 123.123.123.123 <
     */
    GT_U8 dip, i, head, tail;
...
static GT_VOID prvTgfFdbIpvXMcEntryDump(IN GT_U32  index, IN GT_U32 associatedHwDevNum,
                                        IN CPSS_MAC_ENTRY_EXT_STC * entryPtr,
                                        IN GT_BOOL isNew,
                                        ...
                                        IN GT_BOOL isIpv4)
{
    ...
    {
        cpssOsPrintf(
        "index |McIpV%d sip | dip       | vlan|intface|A   R  MS  |dc|sc|ud|dQi|sQi|dSl|sSl \n"
         "             (Hex) (Hex)        (Hex)         S   M   MD                    v1|sId\n"
         "                                               SP  C                       (Hex)  \n",
        (isIpv4 == GT_TRUE) ? 4 : 6);
    }
...
static GT_VOID prvTgfFdbIpvXUcDataEntryDump(IN GT_U32  index, IN GT_U32 associatedHwDevNum,
                                     IN CPSS_MAC_ENTRY_EXT_STC * entryPtr,
                                     IN GT_BOOL isNew,
                                     IN GT_BOOL printHeader,
                                     IN GT_BOOL isIpv4,
                                     IN GT_BOOL isSip6)
{
    if (printHeader == GT_TRUE)
    {
        ...
        */
        ...
        if(isFirst == GT_TRUE)
        {
            cpssOsPrintf("ipv6 UC data entry format \n"
            "A    = aging \n"
            "vlan = nextHopVlanId \n"
            "sid  = siteID \n"
            "type = routingType \n"
        ...
        {
            cpssOsPrintf(
                    "index | dip ipv4        | vrfId|intface|A|NH vlan|sId| type |td |bp| ir|TS | PTR \n"
                    "        (Dec)             (Hex)           (Hex)                cs  mpi      (Hex)\n"
                    );
        }
        else
        {
            ...
            {
                cpssOsPrintf(
                        "index | vrfId|intface|A|NH vlan|sId| type |td |bp| ir|TS | PTR \n"
                        "        (Hex)           (Hex)                cs  mpi      (Hex)\n");
            }
        }
    }
    ...
    if (isIpv4 == GT_TRUE)
    {
        prvTgfIpv4AddrPrint(entryPtr->key.key.ipv4Unicast.dip);
        cpssOsPrintf("|");
        cpssOsPrintf(" %4.4x ", entryPtr->key.key.ipv4Unicast.vrfId);
    }
    else
    {
        if(isSip6)
    ...
    do
    {
        ...
        switch (entry.key.entryType)
        {
            ...
            case CPSS_MAC_ENTRY_EXT_TYPE_IPV4_UC_E:
                prvTgfFdbIpvXUcDataEntryDump(index,associatedHwDevNum, &entry, isNew,
                                                (entryTypePrev != entry.key.entryType) ? GT_TRUE : GT_FALSE, GT_TRUE/*ipv4*/, isSip6);
                break;
            case CPSS_MAC_ENTRY_EXT_TYPE_IPV4_MCAST_E:
                prvTgfFdbIpvXMcEntryDump(index,associatedHwDevNum, &entry, isNew,
                                                (entryTypePrev != entry.key.entryType) ? GT_TRUE : GT_FALSE, GT_TRUE/*ipv4*/);
                break;
            case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_MCAST_E:
                prvTgfFdbIpvXMcEntryDump(index,associatedHwDevNum, &entry, isNew,
                                                (entryTypePrev != entry.key.entryType) ? GT_TRUE : GT_FALSE, GT_FALSE);
                break;
            ...
            case CPSS_MAC_ENTRY_EXT_TYPE_IPV6_UC_DATA_ENTRY_E:
                ...
                {
                    isSip6 = PRV_CPSS_SIP_6_CHECK_MAC(devNum)?GT_TRUE:GT_FALSE;
                    prvTgfFdbIpvXUcDataEntryDump(index,associatedHwDevNum, &entry, isNew,
                                                (entryTypePrev != entry.key.entryType) ? GT_TRUE : GT_FALSE, GT_FALSE, isSip6);
                }
                else
                {
...
/**
* @internal cpssDxChUpperLevelFdbDump function
* @endinternal
*
* @brief   test function --- print FDB entries in TCP socket(test LUA_CLI cmd xps-api call)
*
* @param[in] para                   - (pointer to) I/O stream
* @param[in] printFunc              - (pointer to) LUA CLI print function to print dump in socket
* @param[in] argc                   - number of arguments
* @param[in] argv                   - (pointer to) vector of arguments
* @retval GT_OK                   - on success,
* @retval GT_FAIL                 - otherwise.
* @retval GT_BAD_PARAM            - on bad parameters,
*/
GT_STATUS cpssDxChUpperLevelFdbDump(void *para, void *(printFunc)(void *para, const char *fmt, ...), int32_t argc, char  *argv[] )
{
    GT_STATUS               rc = GT_OK;
    ...
    if(argc == 2)
    {
        rc = cpssDxChBrgFdbMacEntryRead((GT_UINTPTR)argv[0], (GT_UINTPTR)argv[1], &valid, &skip, &aged, &hwDevNum, &macEntry);
        if(rc == GT_OK)
        {
            printFunc(para, " FDB entry type %d \n", macEntry.key.entryType);
            printFunc(para, " FDB VID %d \n", macEntry.key.vid1);
            printFunc(para, " FDB key %d \n", macEntry.key.key);
            printFunc(para, " FDB dest type %d \n", macEntry.dstInterface.type);
            printFunc(para, " FDB dest devNum %d \n", macEntry.dstInterface.devPort.hwDevNum);
            printFunc(para, " FDB dest portNum %d \n", macEntry.dstInterface.devPort.portNum);
            printFunc(para, " FDB dest trunkId %d \n", macEntry.dstInterface.trunkId);
            printFunc(para, " FDB dest vidx %d \n", macEntry.dstInterface.vidx);
            printFunc(para, " FDB dest vlanID %d \n", macEntry.dstInterface.vlanId);
            printFunc(para, " FDB destInterface devNum %d \n", macEntry.dstInterface.hwDevNum);
            printFunc(para, " FDB destination interface fabricVidx %d \n", macEntry.dstInterface.fabricVidx);
            printFunc(para, " FDB destination interface index %d \n", macEntry.dstInterface.index);
            printFunc(para, " FDB aged %d \n", macEntry.age);
            printFunc(para, " FDB is static %d \n", macEntry.isStatic);
            printFunc(para, " FDB DA command %d \n", macEntry.daCommand);
            printFunc(para, " FDB SA command %d \n", macEntry.saCommand);
