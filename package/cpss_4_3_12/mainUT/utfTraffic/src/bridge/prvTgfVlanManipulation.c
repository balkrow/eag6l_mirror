...
#include <cpss/dxCh/dxChxGen/bridge/cpssDxChBrgVlan.h> /* for cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet */
...
/* PARTS of double tagged packet : outer tag0 , inner tag1 */
...
/* PARTS of double tagged packet : outer tag1 , inner tag0 */
static TGF_PACKET_PART_STC prvTgfPacketDoubleTagPartArray_swappedTags[] =
{
    {TGF_PACKET_PART_L2_E,       &prvTgfPacketL2Part},  /* type, partPtr */
    {TGF_PACKET_PART_VLAN_TAG_E, &prvTgfPacketVlanTag1Part},
    {TGF_PACKET_PART_VLAN_TAG_E, &prvTgfPacketVlanTag0Part},
...
/* Double tagged PACKET to send , with swapped taggs than prvTgfPacketInfoDoubleTag*/
static TGF_PACKET_STC prvTgfPacketInfoDoubleTag_swappedTags =
{
    PRV_TGF_PACKET_DOUBLE_TAG_LEN_CNS,                                    /* totalLen */
    sizeof(prvTgfPacketDoubleTagPartArray_swappedTags) / sizeof(TGF_PACKET_PART_STC), /* numOfParts */
    prvTgfPacketDoubleTagPartArray_swappedTags                                        /* partsArray */
};
...
#define DI_ERROR_EVENT_DB_SIZE_CNS 128
static DATA_INTEGRITY_STAGE_ENT errorInjectionStage = DATA_INTEGRITY_STAGE_NONE_E;
static CPSS_DXCH_DIAG_DATA_INTEGRITY_EVENT_STC  errorInterrupts[DI_ERROR_EVENT_DB_SIZE_CNS];
...
/* indication that the ingress traffic need to be recognized as outer Tag1 inner Tag0. */
static GT_BOOL  ingressOuterTag1InnerTag0 = GT_FALSE;
/* indication that for sip7 the ingress traffic need to be recognized as inner Tag0.*/
static GT_BOOL  sip7_allow_innerTag0 = GT_FALSE;
...
)
{
    ...
    if (numOfErrorInterrupts < DI_ERROR_EVENT_DB_SIZE_CNS)
    {
        errorInterrupts[numOfErrorInterrupts] = *eventPtr;
    }
    else
    {
        PRV_UTF_LOG0_MAC(" Events DB is Full!!!\n");
    }
...
static void dataIntegrityInterruptsCheckAndFix(void)
{
    ...
    if(errorInjectionStage & DATA_INTEGRITY_STAGE_INGRESS_PIPE_E)
    {
        ...
        else
        {
            ...
            if (PRV_CPSS_DXCH_BOBCAT2_CHECK_MAC(prvTgfDevNum))
            {
                expectedNumInterrupts = 4;
            }
            else if (PRV_CPSS_DXCH_BOBCAT3_CHECK_MAC(prvTgfDevNum))
            {
                expectedNumInterrupts = 6;
            }
            else if (PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
            {
                if (PRV_CPSS_DXCH_AC5P_CHECK_MAC(prvTgfDevNum))
                {
                    expectedNumInterrupts = 8;
                }
                else if (PRV_CPSS_DXCH_IRONMAN_CHECK_MAC(prvTgfDevNum))
                {
                    expectedNumInterrupts = 4;
                }
                else
                {
                    expectedNumInterrupts = 7;
                }
            }
            else
            {
                expectedNumInterrupts = 3; /* bobk, aldrin */
            }
        }
    }
    else
    if(errorInjectionStage & DATA_INTEGRITY_STAGE_EGRESS_PIPE_PART_1_E)
    {
        /* port bitmap related number of RAMs depends on type of device.
           Align expected events number accordingly. */
        if (PRV_CPSS_DXCH_HARRIER_CHECK_MAC(prvTgfDevNum) ||
            PRV_CPSS_DXCH_IRONMAN_CHECK_MAC(prvTgfDevNum))
        {
...
)
{
    ...
    if(ingressOuterTag1InnerTag0 == GT_TRUE)
    {
        if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) &&
           sip7_allow_innerTag0 == GT_FALSE)
        {
            /* in this 'default' mode of sip7 , the device will not recognize tag0 as inner tag */
            /* therefore it will be considered as 'tag1' only */
            /* so PVID '1' will be selected ... therefore the 'egress tag command' will be 'untagged' */
            /* so the egress packet will come out without tag1 , but with the  8100005 that considered as 'payload' */
            /* set VFD array of expected fields value in received packet*/
            /* 4 bytes of the 8100005 that considered as 'payload' */
            vfdArray[0].cycleCount = 4;
            vfdArray[0].mode = TGF_VFD_MODE_STATIC_E;
            vfdArray[0].modeExtraInfo = 0;
            vfdArray[0].offset = 12;
            ...
            vfdArray[1].modeExtraInfo = 0;
            vfdArray[1].offset = 16;
            vfdArray[1].patternPtr[0] = 0x55;
            vfdArray[1].patternPtr[1] = 0x55;
            vfdArray[1].patternPtr[2] = 0x00;
            vfdArray[1].patternPtr[3] = 0x00;
        }

        /* send packet */
        prvTgfBrgVlanManipulationTestPacketSend(&portInterface, &prvTgfPacketInfoDoubleTag_swappedTags, 2, vfdArray, 0);
    }
    else
    {
        /* send packet */
...
/**
* @internal prvTgfVlanManipulationTestAs_ingressOuterTag1InnerTag0 function
* @endinternal
*
* @brief   set/unset the ingress traffic need to be recognized as outer Tag1 inner Tag0.
*
* @param[in] enable      - enable/disable this mode.
* @return               None
*/
GT_VOID prvTgfVlanManipulationTestAs_ingressOuterTag1InnerTag0(
    IN GT_BOOL                  enable
)
{
    ingressOuterTag1InnerTag0 = enable;
}

/**
* @internal prvTgfVlanManipulationTestAs_sip7_allow_innerTag0 function
* @endinternal
*
* @brief   set/unset for sip7 the ingress traffic need to be recognized as inner Tag0.
*
* @param[in] enable      - enable/disable this mode.
* @return               None
*/
GT_VOID prvTgfVlanManipulationTestAs_sip7_allow_innerTag0(
    IN GT_BOOL                  enable
)
{
    GT_STATUS       rc;

    sip7_allow_innerTag0 = enable;

    rc = cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet(prvTgfDevNum,enable);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChBrgVlanClassifyInnerTagAsTag0EnableSet");
