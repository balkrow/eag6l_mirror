...
)
{
    ...
    cpssOsMemSet(&cpssBypassMode, 0, sizeof(cpssBypassMode));
    ...
    cpssBypassMode.bypassFdbLookup = bypassModePtr->bypassFdbLookup;
    cpssBypassMode.bypassMidwayPcl = bypassModePtr->bypassMidwayPcl;
    cpssBypassMode.bypassIpe       = bypassModePtr->bypassIpe;
...
)
{
    ...
    cpssOsMemSet(bypassModePtr, 0, sizeof(PRV_TGF_CUT_THROUGH_BYPASS_STC));
    ...
    bypassModePtr->bypassFdbLookup = cpssBypassMode.bypassFdbLookup;
    bypassModePtr->bypassMidwayPcl = cpssBypassMode.bypassMidwayPcl;
    bypassModePtr->bypassIpe       = cpssBypassMode.bypassIpe;
...
/**
* @internal prvTgfCutThroughSdmaPortsStatusGet function
* @endinternal
*
* @brief   Get Cut Through status of all SDMA ports.
*
* @param[in] devNum                   - device number
* @param[out] sdmaPortsStatusPtr      - (ponter to)SDMA Ports Status
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
GT_STATUS prvTgfCutThroughSdmaPortsStatusGet
(
    IN  GT_U8                                      devNum,
    OUT PRV_TGF_CUT_THROUGH_SDMA_PORTS_STATUS_STC  *sdmaPortsStatusPtr
)
{
#ifdef CHX_FAMILY
    GT_STATUS rc;
    ...
    sdmaPortsStatusPtr->numOfPorts = 0;
    ...
    for (physicalPortNum = 0; (physicalPortNum < PRV_CPSS_MAX_MAC_PORTS_NUM_CNS); physicalPortNum++)
    {
        if (sdmaPortsStatusPtr->numOfPorts >= CPSS_MAX_SDMA_CPU_PORTS_CNS) break;

        rc = cpssDxChPortPhysicalPortMapIsCpuGet(
            devNum, physicalPortNum, &isSdmaPort);
        if (rc != GT_OK) break;

        if (isSdmaPort != GT_FALSE)
        {
            sdmaPortsStatusPtr->portNumArr[sdmaPortsStatusPtr->numOfPorts] = physicalPortNum;
            sdmaPortsStatusPtr->numOfPorts ++;
        }
    }

    for (idx = 0; (idx < sdmaPortsStatusPtr->numOfPorts); idx++)
    {
        rc = cpssDxChCutThroughPortEnableGet(
            devNum, sdmaPortsStatusPtr->portNumArr[idx],
            &(sdmaPortsStatusPtr->ctEnableArr[idx]),
            &(sdmaPortsStatusPtr->untaggedCtEnableArr[idx]));
        if (rc != GT_OK)
        {
            return rc;
        }
    ...
    /* avoid warnings */
    TGF_PARAM_NOT_USED(devNum);
    TGF_PARAM_NOT_USED(sdmaPortsStatusPtr);
    return GT_NOT_IMPLEMENTED;
#endif /* CHX_FAMILY */
}

/**
* @internal prvTgfCutThroughSdmaPortsStatusRestore function
* @endinternal
*
* @brief   Restore Cut Through status of all SDMA ports.
*
* @param[in] devNum                 - device number
* @param[n] sdmaPortsStatusPtr      - (ponter to)SDMA Ports Status
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_HW_ERROR              - on hardware error
...
*
*/
GT_STATUS prvTgfCutThroughSdmaPortsStatusRestore
(
    IN  GT_U8                                      devNum,
    IN PRV_TGF_CUT_THROUGH_SDMA_PORTS_STATUS_STC  *sdmaPortsStatusPtr
)
{
#ifdef CHX_FAMILY
    GT_STATUS rc;
...
*
*/
GT_STATUS prvTgfCutThroughSdmaPortsStatusSet
(
    IN  GT_U8                                      devNum,
    IN PRV_TGF_CUT_THROUGH_SDMA_PORTS_STATUS_STC  *sdmaPortsStatusPtr,
    IN GT_BOOL                                     enable,
