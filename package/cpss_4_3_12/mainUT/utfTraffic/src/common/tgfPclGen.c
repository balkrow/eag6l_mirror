...
)
{
    ...
    switch (dxChRedirectPtr->redirectCmd)
    {
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_LOGICAL_PORT_ASSIGN_E:
            redirectPtr->redirectCmd = PRV_TGF_PCL_ACTION_REDIRECT_CMD_LOGICAL_PORT_ASSIGN_E;
            /* convert into generic format */
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
           redirectPtr->redirectCmd = PRV_TGF_PCL_ACTION_REDIRECT_CMD_GENERIC_ACTION_E;

           /* convert into device specific format */
           PRV_TGF_STC_D2S_FIELD_COPY_MAC(&(dxChRedirectPtr->data.genericActionInfo),
                                          &(redirectPtr->data.genericActionInfo), genericAction);
...
)
{
    ...
    switch (epclPhaPtr->phaThreadType)
    {
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_TYPE_NOT_USED_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_TYPE_NOT_USED_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_IOAM_INGRESS_SWITCH_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IOAM_INGRESS_SWITCH_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_INT_IOAM_EGRESS_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_INT_IOAM_MIRROR_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_CLASSIFIER_NSH_OVER_VXLAN_GPE_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_CLASSIFIER_NSH_OVER_VXLAN_GPE_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ERSPAN_TYPE_II_MIRROR_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ERSPAN_TYPE_II_MIRROR_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_SFLOW_V5_MIRROR_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_SFLOW_V5_MIRROR_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ENHANCED_SFLOW_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ENHANCED_SFLOW_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_IFA_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_IFA_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_E:
            dxchEpclPhaPtr->phaThreadType =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ROUTER_SA_MODIFICATION_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_VXLAN_DCI_VNI_REMAP_E:
            dxchEpclPhaPtr->phaThreadType =
    ...
    switch (epclPhaPtr->phaThreadIdAssignmentMode)
    {
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_DISABLED_E:
            dxchEpclPhaPtr->phaThreadIdAssignmentMode =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_DISABLED_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_ENABLED_E:
            dxchEpclPhaPtr->phaThreadIdAssignmentMode =
                CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_ENABLED_E;
            break;
        case PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_BYPASS_PHA_E:
            dxchEpclPhaPtr->phaThreadIdAssignmentMode =
...
)
{
    ...
    dxChRuleActionPtr->egressPolicy =
        (ruleActionPtr->egressPolicy == GT_TRUE)
    ...
    {
        rc = prvUtfHwDeviceNumberGet(ruleActionPtr->redirect.data.outIf.outInterface.devPort.hwDevNum,
    ...
    if (dxChRuleActionPtr->egressPolicy != CPSS_PCL_DIRECTION_EGRESS_E)
    {
        ...
        dxChRuleActionPtr->vlan.ingress.copyUdb =
            ruleActionPtr->vlan.copyUdb;
    }
    else
    {
    ...
    PRV_TGF_STC_S2D_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, egressCncIndexMode);
    PRV_TGF_STC_S2D_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, enableEgressMaxSduSizeCheck);
    PRV_TGF_STC_S2D_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, egressMaxSduSizeProfile);
...
)
{
    ...
    ruleActionPtr->egressPolicy =
        (dxChRuleActionPtr->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
            ? GT_TRUE : GT_FALSE;
    ...
    prvTgfConvertDxChToGenericQos(
        &(dxChRuleActionPtr->qos),
        ((dxChRuleActionPtr->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E) ? GT_TRUE : GT_FALSE),
        &(ruleActionPtr->qos));
    ...
    PRV_TGF_STC_D2S_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, egressCncIndexMode);
    PRV_TGF_STC_D2S_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, enableEgressMaxSduSizeCheck);
    PRV_TGF_STC_D2S_FIELD_COPY_MAC(dxChRuleActionPtr, ruleActionPtr, egressMaxSduSizeProfile);
...
/**
* @internal prvTgfPclMidwayPolicyEnable function
* @endinternal
*
* @brief   Enables Ingress Policy
*
...
* @retval GT_FAIL                  - otherwise
*/
GT_STATUS prvTgfPclMidwayPolicyEnable
(
    IN GT_BOOL                        enable
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChPclMidwayPolicyEnable(devNum, enable);
        if (GT_OK != rc)
        {
            PRV_UTF_LOG1_MAC("[TGF]: cpssDxChPclIngressPolicyEnable FAILED, rc = [%d]", rc);
...
)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        {
            rc = prvUtfHwDeviceNumberGet(interfaceInfo.devPort.hwDevNum,
...
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChPclInvalidUdbCmdSet(
            devNum, CPSS_PCL_DIRECTION_INGRESS_E, dxChUdbErrorCmd);
        if (GT_OK != rc)
        {
...
)
{
    ...
    if (direction == CPSS_PCL_DIRECTION_INGRESS_E)
    {
        ...
        rc = prvTgfPclPortIngressPolicyEnable(portNum, lookupCfgPtr->enableLookup);
        if (GT_OK != rc)
        {
    ...
    else if (direction == CPSS_PCL_DIRECTION_MIDWAY_E)
    {
        /* enables ingress policy per devices */
        rc = prvTgfPclMidwayPolicyEnable(GT_TRUE);
        if (GT_OK != rc)
        {
            PRV_UTF_LOG1_MAC("[TGF]: prvTgfPclIngressPolicyEnable FAILED, rc = [%d]", rc);
    ...
    else
    {
        ...
        rc = prvTgfPclEgressPclPacketTypesSet(
            ...
            PRV_TGF_PCL_EGRESS_PKT_NON_TS_E, lookupCfgPtr->enableLookup);
        if (GT_OK != rc)
        {
    ...
    if (accessMode != PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_DONT_CARE_E)
    {
...
/**
* @internal prvTgfPclDefPortInitSip7 function
* @endinternal
*
* @brief   Initialize PCL Engine
*
...
* @param[in] direction                - PCL_DIRECTION
* @param[in] lookupNum                - PCL_LOOKUP_NUMBER
* @param[in] enableLookup             - enable Lookup
* @param[in] accessMode               - configuration table access mode
* @param[in] cfgIndex                 - configuration table index - used for any access mode beside "port"
* @param[in] pclId                    -  pcl Id
* @param[in] emSubProfileId           -  Exact Match SubProfile Id
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
* @retval GT_FAIL                  - on general failure error
*/
GT_STATUS prvTgfPclDefPortInitSip7
(
    IN GT_U32                                       portNum,
    IN CPSS_PCL_DIRECTION_ENT                       direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT                   lookupNum,
    IN PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_ENT accessMode,
    IN GT_U32                                       cfgIndex,
    IN GT_BOOL                                      enableLookup,
    IN GT_U32                                       pclId,
    IN GT_U32                                       emSubProfileId
)
{
    PRV_TGF_PCL_LOOKUP_CFG_STC       lookupCfg;

    cpssOsMemSet(&lookupCfg, 0, sizeof(lookupCfg));
    /* not relevant fields, but should pass checking */
    switch (direction)
    {
        default:
        case CPSS_PCL_DIRECTION_INGRESS_E:
        case CPSS_PCL_DIRECTION_MIDWAY_E:
            lookupCfg.groupKeyTypes.nonIpKey = PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_60_E;
            lookupCfg.groupKeyTypes.ipv4Key = PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_60_E;
            lookupCfg.groupKeyTypes.ipv6Key = PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_60_E;
            break;
        case CPSS_PCL_DIRECTION_EGRESS_E:
            lookupCfg.groupKeyTypes.nonIpKey = PRV_TGF_PCL_RULE_FORMAT_EGRESS_UDB_60_E;
            lookupCfg.groupKeyTypes.ipv4Key = PRV_TGF_PCL_RULE_FORMAT_EGRESS_UDB_60_E;
            lookupCfg.groupKeyTypes.ipv6Key = PRV_TGF_PCL_RULE_FORMAT_EGRESS_UDB_60_E;
            break;
    }
    lookupCfg.lookupType = PRV_TGF_PCL_LOOKUP_TYPE_SINGLE_E;
    /* all 4 parallel sub-lookups */
    lookupCfg.tcamSegmentMode = PRV_TGF_PCL_TCAM_SEGMENT_MODE_4_TCAMS_E;
    /* parameter values */
    lookupCfg.enableLookup = enableLookup;
    lookupCfg.exactMatchLookupSubProfileId = emSubProfileId;
    lookupCfg.pclId = pclId;

    return prvTgfPclDefPortInitExtGeneric(
        portNum, direction, lookupNum,
        accessMode, cfgIndex, &lookupCfg);
}
...
)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        rc = cpssDxChPclSourceIdMaskSet(
            devNum, CPSS_PCL_DIRECTION_INGRESS_E, lookupNum, mask);
        if (GT_OK != rc)
        {
...
)
{
    ...
    rc = cpssDxChPclSourceIdMaskGet(
        devNum, CPSS_PCL_DIRECTION_INGRESS_E, lookupNum, maskPtr);
    if (GT_OK != rc)
    {
