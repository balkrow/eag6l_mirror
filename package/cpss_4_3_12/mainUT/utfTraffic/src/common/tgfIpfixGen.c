...
)
{
    ...
    CPSS_DXCH_IPFIX_ENTRY_STC           dxChEntry;
    CPSS_DXCH_POLICER_STAGE_TYPE_ENT    dxChStage;
    CPSS_DXCH_IPFIX_RESET_MODE_ENT      cpssReset;

    cpssReset = (reset == GT_FALSE)
    ...
    if(usePortGroupsBmp == GT_FALSE)
    {
        rc = cpssDxChIpfixEntryGet(
            devNum, dxChStage, entryIndex, cpssReset, &dxChEntry);
    }
    else
    {
        rc = cpssDxChIpfixPortGroupEntryGet(
            devNum, currPortGroupsBmp, dxChStage, entryIndex, cpssReset, &dxChEntry);
    }
...
)
{
    ...
    /* call device specific API */
    rc = cpssDxChIpfixPhaMetadataCounterMaskGet(devNum, dxChStage, phaCounterMaskPtr);
    if (GT_OK != rc)
    {
