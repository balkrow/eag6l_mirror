...
)
{
    ...
    switch (dxChVlanInfo.floodVidxMode)
    {
...
)
{
    ...
    for( entryIndex = 0 ; entryIndex < fdbSize; entryIndex++)
    {
        ...
        {
            /* valid bit */
            SIP6_FDB_ENTRY_FIELD_GET_MAC(devNum,hwData,
            ...
            else
            {
                ...
                if(entryType >= HW_FDB_ENTRY_TYPE_IPV4_UC_E) /*UC rout*/
                {
                    /* no skip bit*/
                }
                else /* MAC/IPMC-ipv4/IPMC-ipv6 */
                if(PRV_CPSS_DXCH_PP_MAC(devNum)->bridge.fdbHashParams.hashMode == CPSS_MAC_HASH_FUNC_XOR_E)
                {
                    /* skip bit */
                    SIP6_FDB_ENTRY_FIELD_GET_MAC(devNum,hwData,
                        SIP6_FDB_FDB_TABLE_FIELDS_SKIP_E,
...
)
{
    ...
    CPSS_DXCH_ETHERTYPE_TABLE_ENT ethTypeDirection;
        ...
        if(direction == CPSS_DIRECTION_EGRESS_E)
        {
            ...
            else
            {
                ...
                rc = cpssDxChBrgVlanTpidEntrySet(
                    devNum, CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E, largestIndex, etherType);
                if (GT_OK != rc)
                {
    ...
    {
        /* call device specific API */
        rc = cpssDxChBrgVlanTpidEntrySet(devNum, ethTypeDirection, entryIndex, etherType);
        if (GT_OK != rc)
        {
            PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgVlanTpidEntrySet FAILED, rc = [%d]", rc);
...
)
{
    ...
    ethTypeDirection =
        (direction == CPSS_DIRECTION_EGRESS_E)
            ...
            : CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E;
    /* call device specific API */
    return cpssDxChBrgVlanTpidEntryGet(
        prvTgfDevNum, ethTypeDirection, entryIndex, etherTypePtr);
#endif /* CHX_FAMILY */
...
{
#ifdef CHX_FAMILY
    CPSS_DXCH_BRG_VLAN_ETHERTYPE_TABLE_ENTRY_SELECT_MODE_ENT cpssMode;
    ...
    switch (ethMode)
    {
        case CPSS_VLAN_ETHERTYPE0_E:
            cpssMode = CPSS_DXCH_BRG_VLAN_ETHERTYPE_TABLE_ENTRY_SELECT_MODE_ETHERTYPE0_E;
            break;
        case CPSS_VLAN_ETHERTYPE1_E:
            cpssMode = CPSS_DXCH_BRG_VLAN_ETHERTYPE_TABLE_ENTRY_SELECT_MODE_ETHERTYPE1_E;
            break;
        default: return GT_BAD_PARAM;
    }

    /* call device specific API */
    return cpssDxChBrgVlanPortEgressTpidSet(devNum, portNum, cpssMode, tpidEntryIndex);
#endif /* CHX_FAMILY */
...
)
{
    ...
    /* call device specific API */
    return cpssDxChBrgVlanPortEgressTpidGet(
        devNum, portNum, cpssMode, tpidEntryIndexPtr);
...
)
{
        ...
        mode == PRV_TGF_BRG_VLAN_REMOVE_TAG1_IF_ZERO_NOT_DOUBLE_TAG_E)
    {
        ...
        mode = PRV_TGF_BRG_VLAN_REMOVE_TAG1_IF_ZERO_AND_INGRESS_WITHOUT_TAG1_E;
    }
...
)
{
    ...
    {
        rc = prvUtfHwDeviceNumberGet(physicalInfoPtr->devPort.hwDevNum,
...
)
{
    ...
    cpssOsMemSet(&dxChEgressInfo, 0, sizeof(CPSS_DXCH_BRG_EGRESS_PORT_INFO_STC));
...
)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        /* call device specific API */
        rc = cpssDxChBrgVlanTpidEntrySet(
            devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E, entryIndex, etherType);
        if (GT_OK != rc)
        {
            PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgVlanTpidEntrySet FAILED, rc = [%d]", rc);
...
)
{
    ...
    /* call device specific API */
    return cpssDxChBrgVlanTpidEntryGet(
        devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E, entryIndex, etherTypePtr);
...
)
{
    ...
    /* call device specific API */
    return cpssDxChBrgVlanPortEgressTpidSet(devNum, portNum, ethMode, entryIndex);
#endif /* CHX_FAMILY */
...
)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        rc = cpssDxChBrgVlanTpidEntrySet(
            devNum, CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E, entryIndex, etherType);
        if (rc != GT_OK)
        {
            PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgVlanTpidEntrySet FAILED, rc = [%d]", rc);
...
)
{
    ...
    return (GT_OK != rc) ? rc :
        cpssDxChBrgVlanTpidEntryGet(
            prvTgfDevNum, CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E, entryIndex, etherTypePtr);
...
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChBrgVlanMembersTableIndexingModeSet(
            devNum, direction, dxChMode);
        if (GT_OK != rc)
        {
...
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChBrgVlanStgIndexingModeSet(
            devNum, direction, dxChMode);
        if (GT_OK != rc)
        {
...
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChBrgVlanTagStateIndexingModeSet(
            devNum, portNum, dxChMode);
        if (GT_OK != rc)
        {
...
)
{
    ...
    {
        /* call device specific API */
        rc = cpssDxChBrgGenBypassModeSet(devNum, dxChMode);
        if (GT_OK != rc)
        {
...
)
{
    ...
    /* call device specific API */
    rc = cpssDxChBrgGenPortBypassModeSet(devNum, portNum, dxChMode);
    if (GT_OK != rc)
    {
