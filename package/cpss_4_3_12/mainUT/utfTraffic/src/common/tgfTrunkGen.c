...
GT_STATUS prvTgfTrunkUseResilientModeSet(IN GT_BOOL enable)
{
    use_resilient_mode = enable;
    return GT_OK;
}

extern GT_STATUS prvDebugGenericTrunkDebugResilientHashDbDump
(
    IN GT_U8                        devNum,
...
\******************************************************************************/
static void regularTrunkResilientModeEnable(
    IN GT_U8        devNum,
    IN GT_TRUNK_ID  trunkId
)
{
    ...
    rc = cpssDxChTrunkDbPolicyInfoSet(devNum, trunkId ,
        &policyInfo);
    if(rc == GT_NOT_SUPPORTED)
    {
        /* the trunk already hold members */
    }
    else if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: regularTrunkResilientModeEnable FAILED, rc = [%d]", rc);
    }
...
static GT_STATUS wr_cpssDxChTrunkMemberRemove_1
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN CPSS_TRUNK_MEMBER_STC    *memberPtr
...
)
{
    ...
    {
        regularTrunkResilientModeEnable(devNum, trunkId);
    }
    rc = wr_cpssDxChTrunkMemberRemove_1(devNum,trunkId,memberPtr);
    if(use_resilient_mode)
    {
        prvDebugGenericTrunkDebugResilientHashDbDump(devNum, trunkId);
    }
    return rc;
}
static GT_STATUS wr_cpssDxChTrunkMemberAdd_1
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN CPSS_TRUNK_MEMBER_STC    *memberPtr
...
)
{
    ...
    rc = wr_cpssDxChTrunkMemberAdd_1(devNum,trunkId,memberPtr);
    if(use_resilient_mode)
    {
        prvDebugGenericTrunkDebugResilientHashDbDump(devNum, trunkId);
    }
...
static GT_STATUS wr_cpssDxChTrunkMembersSet_1
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_U32                   numOfEnabledMembers,
...
#endif/*CHX_FAMILY*/
GT_STATUS wr_cpssDxChTrunkMembersSet
(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
    IN GT_U32                   numOfEnabledMembers,
    ...
    rc = wr_cpssDxChTrunkMembersSet_1(devNum,trunkId,
        numOfEnabledMembers ,enabledMembersArray,
        numOfDisabledMembers,disabledMembersArray);
    if(use_resilient_mode)
    {
...
)
{
   ...
   while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
   {
       ...
       {
           rc = prvUtfHwDeviceNumberGet(enabledMembersArray[i].hwDevice,
       ...
       {
           rc = prvUtfHwDeviceNumberGet(disabledMembersArray[i].hwDevice,
...
)
{
   ...
   if( NULL != memberPtr)
   {
       ...
       rc = prvUtfHwDeviceNumberGet((memberPtr->hwDevice), &localMember.hwDevice);
       if (GT_OK != rc)
       {
           PRV_UTF_LOG1_MAC(
               "[TGF]: prvUtfHwDeviceNumberGet FAILED, rc = [%d]", rc);
...
)
{
    ...
    /* call device specific API */
    rc = cpssDxChTrunkHashIpv6ModeGet(devNum, &hashMode);
    if (GT_OK != rc)
    {
...
)
{
   ...
   CPSS_DXCH_TRUNK_L4_LBH_MODE_ENT hashMode;
    ...
    /* call device specific API */
    rc = cpssDxChTrunkHashL4ModeGet(devNum, &hashMode);
    if (GT_OK != rc)
    {
...
)
{
    ...
    {
        rc = prvUtfHwDeviceNumberGet(weightedMembersArray[i].member.hwDevice,
...
/**
* @internal prvTgfTrunkHashMaskCrcParamOverrideGet function
* @endinternal
*
* @brief   Function Relevant mode : ALL modes
...
*
*/
GT_STATUS prvTgfTrunkHashMaskCrcParamOverrideGet
(
    IN GT_U8                                            devNum,
    IN PRV_TGF_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_ENT   fieldType,
    IN GT_BOOL                                          *overridePtr
)
{
    PRV_UTF_LOG2_MAC("prvTgfTrunkHashMaskCrcParamOverrideGet(%d,%d) \n",
                     devNum, fieldType);
...
#if (defined CHX_FAMILY)
    {
        ...
        switch(fieldType)
        {
            ...
            case  PRV_TGF_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IPV6_ADDR_24MSB_BY_24UDBS_E:
                ...
                break;
            case PRV_TGF_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IP_DIP_SIP_BY_7MPLS_LABELS_E:
                dxFieldType = CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IP_DIP_SIP_BY_7MPLS_LABELS_E;
                break;
            default:
                return GT_BAD_PARAM;
        }
        ...
        return cpssDxChTrunkHashMaskCrcParamOverrideGet(devNum, dxFieldType, overridePtr);
    }
#else
    return GT_NOT_IMPLEMENTED;
#endif
...
/**
* @internal prvTgfTrunkHashIpModeGet function
* @endinternal
*
* @brief   Function Relevant mode : ALL modes
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvTgfTrunkHashIpModeGet
(
    IN  GT_U8    devNum,
    OUT GT_BOOL *enablePtr
)
{
   ...
   /* call device specific API */
   rc = cpssDxChTrunkHashIpModeGet(devNum, enablePtr);
   if (GT_OK != rc)
   {
       PRV_UTF_LOG1_MAC(
           "[TGF]: cpssDxChTrunkHashIpModeGet FAILED, rc = [%d]", rc);
       return rc;
   }
...
/**
* @internal prvTgfTrunkHashMplsModeEnableGet function
* @endinternal
*
* @brief   Function Relevant mode : ALL modes
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvTgfTrunkHashMplsModeEnableGet
(
    IN  GT_U8     devNum,
    OUT GT_BOOL  *enablePtr
)
{
   ...
   /* call device specific API */
   rc = cpssDxChTrunkHashMplsModeEnableGet(devNum, enablePtr);
   if (GT_OK != rc)
   {
       PRV_UTF_LOG1_MAC(
           "[TGF]: cpssDxChTrunkHashMplsModeEnableGet FAILED, rc = [%d]", rc);
       return rc;
   }
