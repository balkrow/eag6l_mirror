...
)
{
    ...
    {
        rc = prvUtfHwDeviceNumberGet(ipUcRouteEntryPtr->nextHopInterface.devPort.hwDevNum,
...
)
{
    ...
    case PRV_TGF_IP_ROUTING_MODE_PCL_ACTION_E:
        ...
        PRV_TGF_STC_S2D_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction,
                                       &defUcNextHopInfoPtr->pclIpUcAction,
                                       bypassIngressPipe);
        dxChDefUcNextHopInfoPtr->pclIpUcAction.egressPolicy =
            (defUcNextHopInfoPtr->pclIpUcAction.egressPolicy != GT_FALSE)
        ...
        PRV_TGF_STC_S2D_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig,
                                       ...
                                       ipclConfigIndex);
        switch (defUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl0_1OverrideConfigIndex)
        {
            case PRV_TGF_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E:
                dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl0_1OverrideConfigIndex =
       ...
       switch (defUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex)
       {
           case PRV_TGF_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E:
               dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex =
                   CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E;
               break;
           case PRV_TGF_PCL_LOOKUP_CONFIG_INDEX_RETAIN_E:
               dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex =
        ...
        PRV_TGF_STC_S2D_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.qos.ingress,
                                       &defUcNextHopInfoPtr->pclIpUcAction.qos,
                                       profilePrecedence);
        switch (defUcNextHopInfoPtr->pclIpUcAction.redirect.redirectCmd)
        {
            case PRV_TGF_PCL_ACTION_REDIRECT_CMD_NONE_E:
                dxChDefUcNextHopInfoPtr->pclIpUcAction.redirect.redirectCmd =
        ...
        PRV_TGF_STC_S2D_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.policer,
                                       ...
                                       policerId);
        switch (defUcNextHopInfoPtr->pclIpUcAction.policer.policerEnable)
        {
            case PRV_TGF_PCL_POLICER_DISABLE_ALL_E:
                dxChDefUcNextHopInfoPtr->pclIpUcAction.policer.policerEnable =
...
)
{
    ...
    PRV_TGF_STC_S2D_FIELD_COPY_MAC(dxChDefMcRouteLttEntryPtr, defMcRouteLttEntryPtr, epgAssignedToLeafNode);
    return rc;
}
    ...
    switch (routingMode)
    {
        case PRV_TGF_IP_ROUTING_MODE_PCL_ACTION_E:
            ...
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction,
                                           &defUcNextHopInfoPtr->pclIpUcAction,
                                           bypassIngressPipe);
            defUcNextHopInfoPtr->pclIpUcAction.egressPolicy =
                (dxChDefUcNextHopInfoPtr->pclIpUcAction.egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
                    ? GT_TRUE : GT_FALSE;
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig,
                                           &defUcNextHopInfoPtr->pclIpUcAction.lookupConfig,
                                           ipclConfigIndex);
            switch (dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl0_1OverrideConfigIndex)
            {
                case CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E:
                    defUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl0_1OverrideConfigIndex =
            ...
            switch (dxChDefUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex)
            {
                case CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E:
                    defUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex =
                        PRV_TGF_PCL_LOOKUP_CONFIG_INDEX_OVERRIDE_E;
                    break;
                case CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_RETAIN_E:
                    defUcNextHopInfoPtr->pclIpUcAction.lookupConfig.pcl1OverrideConfigIndex =
            ...
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.qos.ingress,
                                           &defUcNextHopInfoPtr->pclIpUcAction.qos,
                                           profilePrecedence);
            switch (dxChDefUcNextHopInfoPtr->pclIpUcAction.redirect.redirectCmd)
            {
                case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_NONE_E:
                    defUcNextHopInfoPtr->pclIpUcAction.redirect.redirectCmd =
            ...
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&dxChDefUcNextHopInfoPtr->pclIpUcAction.policer,
                                           &defUcNextHopInfoPtr->pclIpUcAction.policer,
                                           policerId);
            switch (dxChDefUcNextHopInfoPtr->pclIpUcAction.policer.policerEnable)
            {
                case CPSS_DXCH_PCL_POLICER_DISABLE_ALL_E:
                    defUcNextHopInfoPtr->pclIpUcAction.policer.policerEnable =
                        PRV_TGF_PCL_POLICER_DISABLE_ALL_E;
                    break;
...
)
{
    ...
    switch(routingMode)
    {
        ...
        case PRV_TGF_IP_ROUTING_MODE_IP_LTT_ENTRY_E:
            ...
            PRV_TGF_STC_S2D_FIELD_COPY_MAC(&(dxChNextHopInfo.ipLttEntry), ipLttEntryPtr, epgAssignedToLeafNode );
...
)
{
        ...
        mllPairWriteForm == PRV_TGF_PAIR_READ_WRITE_FIRST_ONLY_E)
    {
        ...
        {
            rc = prvUtfHwDeviceNumberGet(mllPairEntryPtr->firstMllNode.nextHopInterface.devPort.hwDevNum,
        ...
        PRV_TGF_PAIR_READ_WRITE_SECOND_NEXT_POINTER_ONLY_E)
    {
        ...
        {
            rc = prvUtfHwDeviceNumberGet(mllPairEntryPtr->secondMllNode.nextHopInterface.devPort.hwDevNum,
...
)
{
    ...
    {
        dxChShadowType = CPSS_DXCH_IP_LPM_RAM_SIP7_SHADOW_E;
...
)
{
    ...
    leafEntry.isIpv6Mc = leafPtr->isIpv6Mc;
...
)
{
    switch(natType)
    {
        case PRV_TGF_IP_NAT_TYPE_NAT44_E:
            ...
            PRV_TGF_STC_S2D_FIELD_COPY_MAC(&(dxChIpNatRouteEntryPtr->nat44Entry), &(prvTgfNatEntryPtr->nat44Entry), sipPrefixLen);
            PRV_TGF_STC_S2D_FIELD_COPY_MAC(&(dxChIpNatRouteEntryPtr->nat44Entry), &(prvTgfNatEntryPtr->nat44Entry), dipPrefixLen);
            break;
...
)
{
    switch(natType)
    {
        case PRV_TGF_IP_NAT_TYPE_NAT44_E:
            ...
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&(dxChIpNatRouteEntryPtr->nat44Entry), &(prvTgfNatEntryPtr->nat44Entry), sipPrefixLen);
            PRV_TGF_STC_D2S_FIELD_COPY_MAC(&(dxChIpNatRouteEntryPtr->nat44Entry), &(prvTgfNatEntryPtr->nat44Entry), dipPrefixLen);
            break;
        case PRV_TGF_IP_NAT_TYPE_NAT66_E:
...
/**
* @internal prvTgfIpNatL2ConfigSet function
* @endinternal
*
* @brief  Sets the L2NAT Enable field and NAT pointer in egress EPORT table and
*         L2NAT SrcId Size and LSB in Global configuration register.
*
* @note   APPLICABLE DEVICES:      Ironman.
* @note   NOT APPLICABLE DEVICES:  Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P;
*                                  AC5X; Harrier; DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3;
*                                  AC5; Lion; xCat2; Lion2.
*
* @param[in] devNum                - physical device number
* @param[in] portNum               - the port number
* @param[in] natType               - type of the NAT
* @param[in] l2NatPortCfgPtr       - pointer to structure PRV_TGF_IP_NAT_L2_PORT_CONFIG_STC
* @param[in] l2NatGlobalCfgPtr     - pointer to structure PRV_TGF_IP_NAT_L2_GLOBAL_CONFIG_STC
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameter.
* @retval GT_BAD_PTR               - on NULL pointer
...
GT_STATUS prvTgfIpNatL2ConfigSet
(
    IN  GT_U8                               devNum,
    IN  GT_PORT_NUM                         portNum,
    IN  PRV_TGF_IP_NAT_TYPE_ENT             natType,
    IN  PRV_TGF_IP_NAT_L2_PORT_CONFIG_STC   *l2NatPortCfgPtr,
    IN  PRV_TGF_IP_NAT_L2_GLOBAL_CONFIG_STC *l2NatGlobalCfgPtr
)
{
#ifdef CHX_FAMILY
    GT_STATUS   rc = GT_OK;
    ...
    CPSS_IP_NAT_TYPE_ENT                   natTypeCpss;
    CPSS_DXCH_IP_NAT_L2_GLOBAL_CONFIG_STC  l2NatGlobalCfgCpss;
    ...
    egressEportInfo.l2NatEnable = l2NatPortCfgPtr->l2NatEnable;
    egressEportInfo.natPtr      = l2NatPortCfgPtr->natPtr;

    rc = cpssDxChBrgEportToPhysicalPortEgressPortInfoSet(prvTgfDevNum, portNum, &egressEportInfo);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgEportToPhysicalPortEgressPortInfoSet FAILED, rc = [%d]", rc);
    }

    l2NatGlobalCfgCpss.l2NatSrcIdSize = l2NatGlobalCfgPtr->l2NatSrcIdSize;
    l2NatGlobalCfgCpss.l2NatSrcIdLsb  = l2NatGlobalCfgPtr->l2NatSrcIdLsb;
    rc = cpssDxChIpNatL2GlobalConfigSet(devNum, natTypeCpss, &l2NatGlobalCfgCpss);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: cpssDxChIpNatL2GlobalConfigSet FAILED, rc = [%d]", rc);
    }
    ...
    TGF_PARAM_NOT_USED(portNum);
    TGF_PARAM_NOT_USED(natType);
    TGF_PARAM_NOT_USED(l2NatPortCfgPtr);
    TGF_PARAM_NOT_USED(l2NatGlobalCfgPtr);
    return GT_BAD_STATE;
#endif
}
