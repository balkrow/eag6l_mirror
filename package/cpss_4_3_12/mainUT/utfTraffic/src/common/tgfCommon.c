...
/**
* @internal prvTgfPrintPortCountersEthClearOnReadSet function
* @endinternal
*
* @brief   Enable/Disable clear-on-read for all TGF ports and
*          print/clear counters on every set command
*
* @param[in] clearEnable    - GT_FALSE disable clear-on-read for counters-eth
*                             GT_TRUE  enable clear-on-read and read without print
*
* @retval GT_OK                    - on success.
...
*/
static GT_STATUS prvTgfPrintPortCountersEthClearOnReadSetUtil
(
    IN  GT_BOOL     clearEnable
)
{
    GT_U32                              portIndex;
    GT_U8                               devNum;
    ...
    for (portIndex = 0; (portIndex < prvTgfPortsNum); portIndex++)
    {
        devNum = prvTgfDevsArray[portIndex];
        portNum = prvTgfPortsArray[portIndex];

        rc = prvTgfDevPortMacCountersClearOnReadSet(devNum, portNum, clearEnable);
        if (rc != GT_OK)
        {
            PRV_UTF_LOG3_MAC(
                "cpssXXPortMacCountersClearOnReadSet(%d) failed, dev: %d, port: %d\n",
                clearEnable, devNum, portNum);
            return rc;
        }
    }
    return GT_OK;
...
GT_STATUS prvTgfPrintPortCountersEthClearOnReadSet
(
    IN  GT_BOOL     clearEnable
)
{
    ...
    rc = prvTgfPrintPortCountersEthClearOnReadSetUtil(GT_TRUE);
    prvTgfPrintPortCountersEth();
    if (clearEnable != GT_TRUE)
    {
        rc = prvTgfPrintPortCountersEthClearOnReadSetUtil(GT_FALSE);
    }
    return rc;
}
    ...
    for (packetIter = 0; packetIter < partsCount; packetIter++)
    {
        switch (partsArray[packetIter].type)
        {
            ...
            case TGF_PACKET_PART_PTP_V2_E:
                packetSize += TGF_PTP_V2_HDR_SIZE_CNS;
                break;
...
/**
* @internal prvTgfEthCounterLoopbackEgrPortCmdLine function
* @endinternal
*
* @brief   Enable/disable loopback on port to count ingress packets
*         instead of egress counters.
...
*                                      GT_FALSE - disable loopback.
*                                      pVid and pVidForce restored.
* @param[in] vid                     - Port VID, Applicable only when loopback == GT_FALSE.
* @param[in] vidForce                - Port VID Force, Applicable only when loopback == GT_FALSE.
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - wrong interface
*/
GT_STATUS prvTgfEthCounterLoopbackEgrPortCmdLine
(
    IN    GT_U8                          devNum,
    IN    GT_PHYSICAL_PORT_NUM           portNum,
    IN    GT_BOOL                        loopback,
...
{
    return prvTgfEthCounterLoopbackEgrPort(
        devNum, portNum, loopback,
        &vid, &vidForce);
}
...
)
{
    ...
    if(PRV_CPSS_SIP_5_20_CHECK_MAC(prvTgfDevNum))/*BC3 and above */
    {/* see also function utfIsMustUseOrigPorts(IN GT_U8  devNum)
        ...
        if(PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
        {
            ...
            {
                static GT_U32   aas_1TilePorts[12] = {
                   28, 40, 48, 70,
                   20, 60,  0, 8,
                   0,  0,  0, 0
                };
                for(jj = 0 ; jj < 12; jj++)
                {
                    prvTgfPortsArray[jj] = aas_1TilePorts[jj];
                }
            }
            else
            {
                static GT_U32   aas_2TilesPorts[12] = {
                   112, 98, 107, 95,
                    64, 71,  80, 58,
                   155,161, 143,134
                };
                for(jj = 0 ; jj < 12; jj++)
                {
                    prvTgfPortsArray[jj] = aas_2TilesPorts[jj];
                }
            }
        }
        else
        if(PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E)
        {
            ...
            /* don't use port 48 because it's not used on IM-L-RD board */
            ...
            static GT_U32   ironmanMPorts[12] = {
                 0,  4, 8, 11,
                 2,  1, 7, 10,
                 9 , 3, 6,  5
            };
            ...
            switch(PRV_CPSS_PP_MAC(prvTgfDevNum)->devType)
            {
                case CPSS_IRONMAN_M_DEVICES_CASES_MAC:
                    portListPtr = ironmanMPorts;
                    break;
                default:
                    portListPtr = ironmanLPorts;
                    break;
            }
        ...
        else if(PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN2_E)
        {
            /* Aldrin2's BW is 403.2 Gpbs for 600 Mhz Core Clock,
             ...
             /* exclude 100G ports 4, 8, 16 and other ports those fails in 100G lanes */
             prvTgfPortFWSSkipPort(1,GT_TRUE);
             prvTgfPortFWSSkipPort(2,GT_TRUE);
             prvTgfPortFWSSkipPort(3,GT_TRUE);

             /* allocated by 100G port 4 */
             prvTgfPortFWSSkipPort(4,GT_TRUE);
             prvTgfPortFWSSkipPort(5,GT_TRUE);
             prvTgfPortFWSSkipPort(6,GT_TRUE);
             prvTgfPortFWSSkipPort(7,GT_TRUE);
...
/**
* @internal prvTgfAreRemotePortsSet function
* @endinternal
*
* @brief   Check if remote physical ports are configured in
*          device
*
*
* @retval GT_TRUE                  - remote ports are in
*                                    physical port map.
* @retval GT_FALSE                 - otherwise.
*/
GT_BOOL prvTgfAreRemotePortsSet
(
    GT_VOID
)
{
...
static TGF_PACKET_PART_STC prvTgfDummyPacketPartsArray[] = {
...
static TGF_PACKET_STC prvTgfDummyPacketInfo = {
    TGF_L2_HEADER_SIZE_CNS+TGF_ETHERTYPE_SIZE_CNS+sizeof(prvTgfPayloadDataArr), /* totalLen */
    sizeof(prvTgfDummyPacketPartsArray) / sizeof(TGF_PACKET_PART_STC),/* numOfParts */
    prvTgfDummyPacketPartsArray                                       /* partsArray */
};

/* indexes of Drop All and Do Nothing thread IDs */
#define DROP_ALL_THR_ID_CNS 1
#define DO_NOTHING_THR_ID_CNS 2

/**
* @internal prvTgfPhaCacheFlushByTraffic function
* @endinternal
*
* @brief  Flush PPN IMEM cache by traffic.
*         PHA unit logic use 48 Bytes prefetch buffer as cache
*         for IMEM read requests. This buffer should be "flushed"
*         by execute special DROP_ALL or DO_NOTHING threads before
*         change PHA FW. These threads are in same address for all
*         types of PHA FW and therefore buffer will have same data
*         for all types of FW.
*/
void prvTgfPhaCacheFlushByTraffic(GT_VOID)
{

    GT_STATUS             rc;
    GT_U32                burstForFlush = 40; /* need to send number of packets > number of PPNs
                                                 AC5P has maximal number and it's 40 PPNs */
    GT_U32                portIter;
    CPSS_DXCH_PHA_THREAD_INFO_UNT            extInfo;
    CPSS_DXCH_PHA_COMMON_THREAD_INFO_STC     commonInfo;
    CPSS_DXCH_PHA_THREAD_TYPE_ENT            extType; /* type of thread */
    GT_U32                                   portsNum; /* number of ports to send traffic */
    ...
    /* Falcon has multiple Control Pipes. Control pipe is defined by ingress port.
       Usage of DROP_ALL_TRAFFIC is not working for Falcon because only single control
       pipe is used in this case. Need to use send packets to all UT ports and need use
       DO_NOTHING thread in this case.
       Send packets to all ports to ensure that all CPs will be handled.
       Use flooding to default VLAN#1  */
    portsNum = prvTgfPortsNum;

    extType = CPSS_DXCH_PHA_THREAD_TYPE_DROP_ALL_TRAFFIC_E;

    /*config the Drop All thread */
    rc = cpssDxChPhaThreadIdEntrySet(prvTgfDevNum,
                DROP_ALL_THR_ID_CNS, &commonInfo, extType, &extInfo);
    ...
    /*config the Do Nothing thread */
    rc = cpssDxChPhaThreadIdEntrySet(prvTgfDevNum,
                DO_NOTHING_THR_ID_CNS, &commonInfo, extType, &extInfo);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaThreadIdEntrySet enhanced sflow save_src_mod_id");

    rc = prvTgfSetTxSetupEth(prvTgfDevNum, &prvTgfDummyPacketInfo, burstForFlush, 0, NULL);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "ERROR of prvTgfSetTxSetupEth");

    /* disable "packet is dropped" warnings */
    tgfPrvIntendedError = GT_TRUE;

    /* configure all ports with Drop All thread */
    for (portIter = 0; portIter < portsNum ; portIter++)
    {
    ...
    /* Falcon has multiple Control Pipes.
       Send packets to all ports to ensure that all CPs will be handled */
    for (portIter = 0; portIter < portsNum ; portIter++)
    {
        /* AUTODOC: Assign Do Nothing thread to the inject traffic ePort.
           Packet FROM_CPU is handled by inject egress ePort first.
           And this packet should pass to MAC for loopback operation.
           Therefore DO Nothing is used for this port. */
        rc = cpssDxChPhaPortThreadIdSet(prvTgfDevNum,
                                        prvTgfPortsArray[portIter],
                                        GT_TRUE,
                                        DO_NOTHING_THR_ID_CNS);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaPortThreadIdSet");
        ...
        /* AUTODOC: restore Drop All thread to the inject traffic ePort */
        rc = cpssDxChPhaPortThreadIdSet(prvTgfDevNum,
                                        prvTgfPortsArray[portIter],
                                        GT_TRUE,
                                        DROP_ALL_THR_ID_CNS);
    ...
    /* enable back "packet is dropped" warnings */
    tgfPrvIntendedError = GT_FALSE;

    for (portIter = 0; portIter < portsNum ; portIter++)
    {
        /* AUTODOC: disable PHA for the ePort */
        rc = cpssDxChPhaPortThreadIdSet(prvTgfDevNum,
                                        prvTgfPortsArray[portIter],
                                        GT_FALSE,
                                        0 /* don't care when disabled */);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaPortThreadIdSet");
    }
...
/* variable to store previouse image ID */
static CPSS_DXCH_PHA_FW_IMAGE_ID_ENT    store_phaFwImageId = CPSS_DXCH_PHA_FW_IMAGE_ID_LAST_E;

/**
* @internal prvTgfPhaFWLoad function
* @endinternal
*
* @brief  Load dedicated image of PHA FW.
*         Previose image ID is storred in DB.
*
* @param[in] phaFwImageId - Image ID to load.
*
*/
void prvTgfPhaFWLoad
(
...
)
{
    GT_STATUS             rc;
     ...
     /* get current FW ID */
    rc = cpssDxChPhaFwImageIdGet(prvTgfDevNum, &store_phaFwImageId);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaFwImageIdGet");

    if (phaFwImageId == store_phaFwImageId)
    {
        /* right image already loaded */
        store_phaFwImageId = CPSS_DXCH_PHA_FW_IMAGE_ID_LAST_E;
        return;
    }

#ifndef ASIC_SIMULATION
    /* clean IMEM cache */
    prvTgfPhaCacheFlushByTraffic();
#endif /* ASIC_SIMULATION */

    /* load new image */
    rc = cpssDxChPhaInit(prvTgfDevNum, GT_TRUE, phaFwImageId);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaInit");

    PRV_UTF_LOG1_MAC("Load new PHA FW [%d] done\n", phaFwImageId);
}

/**
* @internal prvTgfPhaFWRestore function
* @endinternal
*
* @brief  Load PHA FW back to device that was changed by prvTgfPhaFWLoad.
*         Previose image ID is taken from DB.
*
*
*/
void prvTgfPhaFWRestore
(
    GT_VOID
)
{
    ...
    if (CPSS_DXCH_PHA_FW_IMAGE_ID_LAST_E == store_phaFwImageId)
    {
        /* right image already loaded */
        return;
    }
    ...
    /* load new image */
    rc = cpssDxChPhaInit(prvTgfDevNum, GT_TRUE, store_phaFwImageId);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaInit");

    PRV_UTF_LOG1_MAC("Restore PHA FW [%d] done\n", store_phaFwImageId);
}
