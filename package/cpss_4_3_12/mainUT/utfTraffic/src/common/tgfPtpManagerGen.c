...
static GT_STATUS tgf2dxchPTP_TAI_CLOCK_FREQ_ENT
(
    IN  PRV_TGF_PTP_TAI_CLOCK_FREQ_ENT    tgfTaiClockFreq,
    OUT CPSS_DXCH_PTP_TAI_CLOCK_FREQ_ENT  *dxchTaiClockFreqPtr
)
{
    switch (tgfTaiClockFreq)
    {
        case PRV_TGF_PTP_10_FREQ_E:
            *dxchTaiClockFreqPtr = CPSS_DXCH_PTP_10_FREQ_E;
            break;
        case PRV_TGF_PTP_20_FREQ_E:
            *dxchTaiClockFreqPtr = CPSS_DXCH_PTP_20_FREQ_E;
            break;
        case PRV_TGF_PTP_25_FREQ_E:
            *dxchTaiClockFreqPtr = CPSS_DXCH_PTP_25_FREQ_E;
            break;
        case PRV_TGF_PTP_156_25_FREQ_E:
            *dxchTaiClockFreqPtr = CPSS_DXCH_PTP_156_25_FREQ_E;
            break;
...
static GT_STATUS tgf2dxchPTP_REF_CLOCK_SELECT_ENT
(
    IN  PRV_TGF_PTP_REF_CLOCK_SELECT_ENT    tgfRefClockSelect,
    OUT CPSS_DXCH_PTP_REF_CLOCK_SELECT_ENT  *dxchRefClockSelectPtr
)
{
    switch (tgfRefClockSelect)
    {
        case PRV_TGF_PTP_REF_CLOCK_SELECT_MAIN_25_MHZ_E:
            *dxchRefClockSelectPtr = CPSS_DXCH_PTP_REF_CLOCK_SELECT_MAIN_25_MHZ_E;
            break;
        case PRV_TGF_PTP_REF_CLOCK_SELECT_DEDICATED_PTP_REF_CLK_E:
            *dxchRefClockSelectPtr = CPSS_DXCH_PTP_REF_CLOCK_SELECT_DEDICATED_PTP_REF_CLK_E;
            break;
        case PRV_TGF_PTP_REF_CLOCK_SELECT_SD_DIFFRENTIAL_REF_CLK_E:
            *dxchRefClockSelectPtr = CPSS_DXCH_PTP_REF_CLOCK_SELECT_SD_DIFFRENTIAL_REF_CLK_E;
            break;
        default: return GT_BAD_PARAM;
    }
...
/**
* @internal prvTgfConvertGenericToDxChPtpManagerPtpRefClockConfStc
*           function
* @endinternal
*
* @brief   Convert generic Ptp Manager Ptp reference clock stc into
*          device specific Ptp Manager Ptp reference clock stc
*
* @param[in] ptpRefClockConfPtr       - (pointer to) Ptp Manager Ptp reference clock structure
*
* @param[out] dxChPtpRefClockConfPtr  - (pointer to) DxCh Ptp Manager Ptp reference clock structure
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
*/
static GT_STATUS prvTgfConvertGenericToDxChPtpManagerPtpRefClockConfStc
(
    IN  PRV_TGF_PTP_MANAGER_REF_CLOCK_CFG_STC   *ptpRefClockConfPtr,
    OUT CPSS_DXCH_PTP_MANAGER_REF_CLOCK_CFG_STC *dxChPtpRefClockConfPtr
)
{
    GT_STATUS rc;

    /* reset variables */
    cpssOsMemSet((GT_VOID*) dxChPtpRefClockConfPtr, 0, sizeof(CPSS_DXCH_PTP_MANAGER_REF_CLOCK_CFG_STC));

    rc = tgf2dxchPTP_REF_CLOCK_SELECT_ENT(ptpRefClockConfPtr->refClockSource, &(dxChPtpRefClockConfPtr->refClockSource));
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: tgf2dxchPTP_REF_CLOCK_SELECT_ENT FAILED, rc = [%d]", rc);
    }
    rc = tgf2dxchPTP_TAI_CLOCK_FREQ_ENT(ptpRefClockConfPtr->refClockFrequency, &(dxChPtpRefClockConfPtr->refClockFrequency));
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: tgf2dxchPTP_TAI_CLOCK_FREQ_ENT FAILED, rc = [%d]", rc);
    }
...
* @retval GT_BAD_PARAM             - on wrong parameters
*/
static GT_STATUS prvTgfConvertGenericToDxChPtpManagerPtpManagerTodCountStc
(
    IN  PRV_TGF_PTP_MANAGER_TOD_COUNT_STC   *ptpTodCountPtr,
    OUT CPSS_DXCH_PTP_MANAGER_TOD_COUNT_STC *dxChPtpTodCountPtr
)
{
    /* reset variables */
    cpssOsMemSet((GT_VOID*) dxChPtpTodCountPtr, 0, sizeof(CPSS_DXCH_PTP_TOD_COUNT_STC));
...
)
{
    ...
    CPSS_DXCH_PTP_MANAGER_REF_CLOCK_CFG_STC             dxChPtpRefClockConf;
    ...
    rc = prvTgfConvertGenericToDxChPtpManagerPtpRefClockConfStc(ptpRefClockConfPtr,&dxChPtpRefClockConf);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: prvTgfConvertGenericToDxChPtpManagerPtpRefClockConfStc FAILED, rc = [%d]", rc);
    }
    ...
    rc = cpssDxChPtpManagerPtpInit(devNum,&dxChOutputInterfaceConf,&dxChPtpOverEthernetConf,&dxChPtpOverIpUdpConf,&dxChPtpTsTagGlobalConf,&dxChPtpRefClockConf,cpuCode);
    if (GT_OK != rc)
    {
    ...
    TGF_PARAM_NOT_USED(ptpRefClockConfPtr);
...
(
    IN  GT_U8                                 devNum,
    IN  PRV_TGF_PTP_TAI_NUMBER_ENT            taiNumber,
    IN  PRV_TGF_PTP_MANAGER_TOD_COUNT_STC     *updateValuePtr,
...
)
{
    ...
    GT_STATUS rc;
    CPSS_DXCH_PTP_TAI_NUMBER_ENT            dxChTaiNumber = CPSS_DXCH_PTP_TAI_NUMBER_0_E;
    CPSS_DXCH_PTP_MANAGER_TOD_COUNT_STC     dxChUpdateValue;
    ...
    rc = prvTgfConvertGenericToDxChPtpManagerPtpManagerTodCountStc(updateValuePtr,&dxChUpdateValue);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: prvTgfConvertGenericToDxChPtpManagerPtpTodCountStc FAILED, rc = [%d]", rc);
    }
...
)
{
    ...
    rc = cpssDxChPtpManagerTriggerGenerate(devNum,dxChTaiNumber,extPulseWidth, &dxChTriggerTime, &dxChMaskValue);
    if (GT_OK != rc)
    {
