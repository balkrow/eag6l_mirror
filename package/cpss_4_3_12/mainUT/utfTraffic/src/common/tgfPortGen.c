...
)
{
    ...
    /* call device specific API */
    rc = cpssDxChPortTxSharedPoolLimitsSet(devNum, poolNum, maxBufNum, maxDescNum,GT_FALSE);
    if (GT_OK != rc)
    {
...
)
{
    ...
    if (GT_OK != rc)
    {
        PRV_UTF_LOG2_MAC("[TGF]: prvTgfPortSpeedGet FAILED, port [%d] rc = [%d]",portNum, rc);
