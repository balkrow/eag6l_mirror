...
/**
********************************************************************************
* @file prvTgfCscdDsaTxMirrAnInfo.c
*
...
* Test check next cases in two modes HOP_BY_HOP and END_TO_END
...
static GT_U32      prvTgfAnPortIx;       /* an analyzer port's index */
static GT_U32      prvTgfCscdPortIx;     /* an index of port mapped to remoted device */
...
/**
* @internal prvTgfCscdTxAnInitArrayByRandom function
* @endinternal
*
...
*/
static GT_STATUS prvTgfCscdTxAnInitArrayByRandom
(
    INOUT GT_U32 arr[],
    IN    GT_U32 arrLen,
    IN    GT_U32 min,
...
/**
* @internal prvTgfCscdTxAnInitRandomValues function
* @endinternal
*
* @brief   Initialize some random values
*/
static GT_VOID prvTgfCscdTxAnInitRandomValues
(
    GT_VOID
)
{
    ...
    GT_STATUS rc;
    GT_U32  maxEports = UTF_CPSS_PP_MAX_PORT_NUM_CNS(prvTgfDevNum);
    ...
    */
    ...
    rc = prvTgfCscdTxAnInitArrayByRandom(randArr, PRV_TGF_PORTS_COUNT_CNS,
                                         0, 3);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
                                 "prvTgfCscdTxAnInitArrayByRandom");

    prvTgfRxPortIx   = randArr[0];
    prvTgfTxPortIx   = randArr[1];
    prvTgfAnPortIx   = randArr[2];
    prvTgfCscdPortIx = randArr[3];

    /* random ePorts > 512 to be different than any physical port number */
    rc = prvTgfCscdTxAnInitArrayByRandom(randArr, 2, maxEports/2, maxEports-1);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
                                "prvTgfCscdTxAnInitArrayByRandom");
    /* get random eport in higher 1/2 range of eports */
    prvTgfEport   = (maxEports/2) + cpssOsRand() % (maxEports/2);

    /* random vlan id */
    prvTgfVlanId = (2 + cpssOsRand() % (_4K - 2)) % UTF_CPSS_PP_MAX_VLAN_INDEX_CNS(prvTgfDevNum);

    /* random trunkId */
    if (PRV_TGF_MAX_TRUNK_ID <= (UTF_CPSS_PP_MAX_TRUNK_ECMP_INDEX_CNS(prvTgfDevNum) / 16))
    {
        prvTgfTrunkId = 1 + cpssOsRand() % (PRV_TGF_MAX_TRUNK_ID - 1);
    }
    else
    {
        prvTgfTrunkId = 1 + cpssOsRand() % ((GT_TRUNK_ID)(UTF_CPSS_PP_MAX_TRUNK_ECMP_INDEX_CNS(prvTgfDevNum) / 16) - 1);
    }
...
/**
* @internal prvTgfCscdTxAnMirroringSet function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfCscdTxAnMirroringSet
(
...
/**
* @internal prvTgfCscdTxAnConfigSave function
* @endinternal
*
* @brief   Save configuration to be restored at the end of test
*/
static GT_VOID prvTgfCscdTxAnConfigSave
(
    GT_VOID
)
{
...
/**
* @internal prvTgfCscdTxAnConfigSet function
* @endinternal
*
* @brief   Setup configuration
*/
static GT_VOID prvTgfCscdTxAnConfigSet
(
    GT_VOID
)
{
    ...
    /* portsArr couldn't be initialized in place of declaration because
       prvTgfRxPortIx, prvTgfTxPortIx was not initialized */
    ...
    /* AUTODOC: map the cascade port to a remote analyzer device */
...
/**
* @internal prvTgfCscdTxAnConfigRestore function
* @endinternal
*
* @brief   Restore configuration
*/
static GT_VOID prvTgfCscdTxAnConfigRestore
(
    GT_VOID
)
{
...
/**
* @internal prvTgfCscdTxAnSendPacket function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfCscdTxAnSendPacket
(
...
/**
* @internal prvTgfCscdTxAnCheckOnPorts function
* @endinternal
*
...
*
*/
static GT_VOID prvTgfCscdTxAnCheckOnPorts
(
...
/**
* @internal prvTgfCscdTxAnDsaDestSrcFieldsSet function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfCscdTxAnDsaDestSrcFieldsSet
 (
 ...
 )
 {
         ...
         && GT_TRUE == isEgressed && GT_TRUE == isRxCascade)
     {
         ...
         tagTrgPort        = prvTgfPortsArray[prvTgfAnPortIx];
     }
     else
     {
         switch (analyzerType)
 ...
 /*******************************************************************************
 * prvTgfCscdTxAnPrepareSendAndCheck
 *
 * DESCRIPTION:
 ...
 *
 *******************************************************************************/
 static GT_VOID prvTgfCscdTxAnPrepareSendAndCheck
 (
     IN PRV_TGF_MIRROR_TO_ANALYZER_FORWARDING_MODE_ENT frwdMode,
 ...
 )
{
            ...
            case PRV_TGF_AN_IFACE_TRUNK_E:
                ...
                iface.trunkId = prvTgfTrunkId;
                 ...
                 if(prvUtfIsDoublePhysicalPortsModeUsed() &&
                    ...
                    prvTgfPortsArray[prvTgfAnPortIx] >= 256)/* need more than 8 bits in eDSA */
                 {
    ...
    /* AUTODOC: enable mirroring to {targetDevNum, ePort} */
    prvTgfCscdTxAnMirroringSet(frwdMode, prvTgfSavedAnalyzerIndex,
                               targetDevNum, prvTgfEport);

    /* AUTODOC: send the packet and check the result */
    iface.type              = CPSS_INTERFACE_PORT_E;
    iface.devPort.hwDevNum  = prvTgfDevNum;
    iface.devPort.portNum   = prvTgfPortsArray[captPortIx];
    ...
    prvTgfCscdTxAnDsaDestSrcFieldsSet(&prvTgfDsaTag, frwdMode, isRxCascade,
                                      isEgrCascade, anType, GT_FALSE);

    prvTgfCscdTxAnSendPacket( GT_TRUE == isRxCascade ? &prvTgfCscdPacketInfo
    ...
    expTxCntArr[prvTgfTxPortIx] = GT_TRUE == isRxCascade ? 0 : prvTgfBurstCount;

    if(expectFail == GT_TRUE)
    {
        expRxCntArr[captPortIx] = 0;
        expTxCntArr[captPortIx] = 0;

        expPktArr[captPortIx] = NULL;
    }
    else
    {
        expRxCntArr[captPortIx]     = prvTgfBurstCount;
        expTxCntArr[captPortIx]     = prvTgfBurstCount;

        expPktArr[captPortIx] = (GT_TRUE == isEgrCascade) ? &prvTgfCscdPacketInfo
                                                          : &prvTgfNetwPacketInfo;
    }
    ...
    prvTgfCscdTxAnCheckOnPorts(expRxCntArr, expTxCntArr, expPktArr);
...
/**
* @internal prvTgfCscdTxAnTrafficGenerate function
* @endinternal
*
...
*                                       None
*/
static GT_VOID prvTgfCscdTxAnTrafficGenerate
...
)
{
    ...
    prvTgfCscdTxAnPrepareSendAndCheck(mode, GT_FALSE, GT_TRUE,
                                      PRV_TGF_AN_IFACE_LOCAL_EPORT_E);
    ...
    prvTgfCscdTxAnPrepareSendAndCheck(mode, GT_TRUE, GT_TRUE,
                                      PRV_TGF_AN_IFACE_REMOTE_EPORT_E);
    ...
    prvTgfCscdTxAnPrepareSendAndCheck(mode, GT_TRUE, GT_FALSE,
                                      PRV_TGF_AN_IFACE_REMOTE_EPORT_E);
...
{
    prvTgfCscdTxAnInitRandomValues();

    prvTgfCscdTxAnConfigSave();
    prvTgfCscdTxAnConfigSet();
