...
/**
* @internal
*           prvTgfExactMatchManagerBasicDefaultActionTtiLookupUpdate
*           function
* @endinternal
*
* @brief   update the lookup configured in the manager create
*/
GT_VOID prvTgfExactMatchManagerBasicDefaultActionTtiLookupUpdate
(
    IN GT_U32                               exactMatchManagerId,
    IN GT_U32                               isTrap
)
{
    GT_STATUS                                               rc;
    CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC         lookupConfig;
    CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC  profileEntry;

    cpssOsMemSet(&lookupConfig,0,sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_LOOKUP_CONFIG_STC));
    cpssOsMemSet(&profileEntry,0,sizeof(CPSS_DXCH_EXACT_MATCH_MANAGER_PROFILE_ENTRY_PARAMS_STC));

    /* get the configuration from propfile 5 and change only the trap command */
    rc =  cpssDxChExactMatchManagerProfileEntryGet(exactMatchManagerId,
                                                   TGF_EXACT_MATCH_PROFILE_ID_CNS,
                                                   &profileEntry);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChExactMatchManagerProfileEntryUpdate");

    /* configure profileId=6 for TTI client */
    if(isTrap==GT_TRUE)
    {
       profileEntry.defaultAction.ttiAction.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
       profileEntry.defaultAction.ttiAction.userDefinedCpuCode = PRV_TGF_EXACT_MATCH_CPU_CODE_CNS+2;/* 504 */
    }
    else
    {
       profileEntry.defaultAction.ttiAction.command = CPSS_PACKET_CMD_DROP_HARD_E;
       profileEntry.defaultAction.ttiAction.userDefinedCpuCode = CPSS_NET_FIRST_USER_DEFINED_E+1;/* 501 */
    }

    rc =  cpssDxChExactMatchManagerProfileEntryUpdate(exactMatchManagerId,
                                                      CPSS_DXCH_EXACT_MATCH_LOOKUP_FIRST_E,
                                                      TGF_EXACT_MATCH_PROFILE_ID_CNS+1,
                                                      &profileEntry);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChExactMatchManagerProfileEntryUpdate");


    /* get the current lookup configuration and change the value of the profileId*/
    rc =  cpssDxChExactMatchManagerLookupGet(exactMatchManagerId,CPSS_DXCH_EXACT_MATCH_LOOKUP_FIRST_E,&lookupConfig);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChExactMatchManagerLookupUpdate");

    lookupConfig.lookupClientMappingsArray[0].ttiMappingElem.profileId=TGF_EXACT_MATCH_PROFILE_ID_CNS+1;

    rc =  cpssDxChExactMatchManagerLookupUpdate(exactMatchManagerId,CPSS_DXCH_EXACT_MATCH_LOOKUP_FIRST_E,&lookupConfig);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChExactMatchManagerLookupUpdate");
}