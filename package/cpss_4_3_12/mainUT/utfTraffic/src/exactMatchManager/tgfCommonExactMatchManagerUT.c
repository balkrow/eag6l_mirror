...
/*----------------------------------------------------------------------------*/
/*
   tgfExactMatchManagerBasicDefaultActionTti_UpdateLookupAfterManagerCreate :
   Test functionality of Exact Match default action ,
   base on tgfTunnelTermLlcNonSnapType
   Exact Match default action gets active incase no hit in Tcam or Exact Match lookup
   update lookup configuration and check profileId configured in first_lookup was
   changed and the CPU code was changed according to the new profileId
*/
UTF_TEST_CASE_MAC(tgfExactMatchManagerBasicDefaultActionTti_UpdateLookupAfterManagerCreate)
{
    /*
    1. Set Exact match profileId and Default Action
    2. Set TTI Ether Type Key Base configuration
    ...
    6. change Default Action for Trap
    7. Generate traffic -- check traffic block and cpuCode
    8. Update lookup config and profile entry
    7. Generate traffic -- check traffic block and new cpuCode
    */
    ...
    /* update Lookup configuration - trap to cpu with code 504 */
    prvTgfExactMatchManagerBasicDefaultActionTtiLookupUpdate(exactMatchManagerId,GT_TRUE);

    /* Generate traffic -- check no traffic and cpuCode 504*/
    prvTgfExactMatchBasicDefaultActionTtiTrafficGenerate(GT_TRUE,GT_FALSE,PRV_TGF_EXACT_MATCH_CPU_CODE_CNS+2);
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfExactMatchManager)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfExactMatchManagerBasicDefaultActionTti_UpdateLookupAfterManagerCreate)
