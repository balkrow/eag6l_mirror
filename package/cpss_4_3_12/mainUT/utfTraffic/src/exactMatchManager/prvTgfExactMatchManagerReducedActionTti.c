...
)
{
    ...
    lookupInfo.profileEntryParamsArray[TGF_EXACT_MATCH_PROFILE_ID_CNS].keyParams.keySize = PRV_TGF_EXACT_MATCH_KEY_SIZE_33B_E;
    lookupInfo.profileEntryParamsArray[TGF_EXACT_MATCH_PROFILE_ID_CNS].keyParams.keyStart = 0;
    cpssOsMemSet((GT_VOID*) &(lookupInfo.profileEntryParamsArray[TGF_EXACT_MATCH_PROFILE_ID_CNS].keyParams.mask[0]), 0, sizeof(GT_U8)*PRV_TGF_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS);
    ...
    {
        entryAttrInfo.expandedArray[TGF_EXACT_MATCH_PROFILE_ID_CNS].expandedAction.ttiAction.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
...
)
{
    ...
    entry.exactMatchAction.ttiAction.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
    if(isReduced==GT_TRUE)
    {
