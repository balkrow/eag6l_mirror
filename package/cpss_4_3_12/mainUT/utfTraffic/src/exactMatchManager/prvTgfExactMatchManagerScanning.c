...
)
{
    ...
    cpssOsMemCpy((GT_VOID*) &(lookupInfoPtr->profileEntryParamsArray[TGF_EXACT_MATCH_PROFILE_ID_CNS].keyParams.mask[0]), mask.ruleIngrUdbOnly.udb, sizeof(GT_U8)*PRV_TGF_EXACT_MATCH_MANAGER_MAX_KEY_SIZE_CNS);
...
)
{
    ...
    for(ii = 0 ; ii < maxTotalEntries; /*update according to entry size*/)
    {
        ...
        if(st==GT_OK)
        {
            ...
            if((ii % 100) == 0)
            {
                prvTgfExactMatchManagerAgeBinUsage_Verify(exactMatchManagerId, (numEntriesAdded+1));/* +1 since we added another single entry before the loop */
                PRV_UTF_LOG2_MAC("numIndexesAdded = %d, packetCnt = %d\n", numEntriesAdded+1, ii+1);
            }
...
)
{
    ...
    /* maxTotalEntriesAdded+1 is the number of entries added in the test
       each ageBin Id will hold 256 entries in case of 19b entry size
       the lats bun will hold the remaining indexes.
       we need to find what is the last ageBin and what is the remaining value */
    lastFullAgeBinId = (maxTotalEntriesAdded+1)/256;
    numOfEntriesInNonullAgeBinId=(maxTotalEntriesAdded+1)%256;
    ...
    for(i = 0; i < exactMatchManagerPtr->agingBinInfo.totalAgeBinAllocated; i++)
    {
        ...
        {
            /* 256 entries with size of 19B fill a single ageBin with 256 indexes */
            agingScanStatisticsArr[PRV_TGF_EXACT_MATCH_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_E] += 256;
        }
        else
        {
            if(i==lastFullAgeBinId)
    ...
    for(i = 0; i < exactMatchManagerPtr->agingBinInfo.totalAgeBinAllocated; i++)
    {
        ...
        if (i<lastFullAgeBinId)
        {
            /* 256 entries with size of 19B fill a single ageBin */
            agingScanStatisticsArr[PRV_TGF_EXACT_MATCH_MANAGER_SCAN_AGE_API_STATISTICS_TOTAL_AGEDOUT_DELETED_E] += 256;
        }
        else
        {
            if(i==lastFullAgeBinId)
