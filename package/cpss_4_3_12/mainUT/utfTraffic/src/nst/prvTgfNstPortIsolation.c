...
#define PRV_TGF_NST_DEFAULT_TC_IDX_CNS    0

/* Bridge Access Matrix number of supported commands */
#define PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_MAX_CMD_CNS     5

/* Bridge Access Matrix SA/DA Access Level */
#define PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_DA_LEVEL_CNS    0
#define PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_SA_LEVEL_CNS    0
...
/* Bridge Access Matrix command Array */
static CPSS_PACKET_CMD_ENT prvTgfNstCommandArray[PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_MAX_CMD_CNS] =
...
)
{
    ...
    /* this part relevant only if the device supports more than 4096 VLANs
       where each one has own port bitmap. SIP7 has port bitmap for first 4K. */
    if ((numOfVlans>=PRV_TGF_E_VLANID_BASE_CNS) && !(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum)))
    {
...
/**
* @internal prvTgfNstBridgeAccessMatrixConfigurationSet function
* @endinternal
*
* @brief Set test configuration
*        Configure VLAN 5
*        Add FDB entry: MAC DA = prvTgfPacketL2Part.daMac, VLAN 5 and SA/DA Access level = 0
*/
GT_VOID prvTgfNstBridgeAccessMatrixConfigurationSet
(
    GT_VOID
)
{
    ...
    /* Set PVID 5 for port 3 */
    rc = prvTgfBrgVlanPortVidSet(prvTgfDevNum,
        prvTgfPortsArray[PRV_TGF_EGRESS_PORT_IDX_CNS], PRV_TGF_VLANID_CNS);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanPortVidSet");

    /* Save DA MAC */
    cpssOsMemCpy(prvTgfRestoreCfg.daMac, prvTgfPacketL2Part.daMac, sizeof(TGF_MAC_ADDR));
    ...
    prvTgfPacketL2Part.daMac[5] = 0x01;

    /* Add FDB entry with MAC prvTgfPacketL2Part.daMac,
     * VLAN PRV_TGF_NST_VLANID_CNS,
     * port prvTgfPortsArray[PRV_TGF_EGRESS_PORT_IDX_CNS] */
    rc = prvTgfBrgDefFdbMacEntryOnPortSet(
...
/**
* @internal prvTgfNstBridgeAccessMatrixTrafficGenerate function
* @endinternal
*
* @brief Generate traffic and verify expected packet count on target and CPU port
*/
GT_VOID prvTgfNstBridgeAccessMatrixTrafficGenerate
(
    GT_VOID
)
{
    ...
    GT_U32                                      portIter;
    CPSS_DXCH_NET_SDMA_RX_COUNTERS_STC          rxCounters;
    GT_BOOL                                     expectedPktToCpu = GT_TRUE;
    GT_BOOL                                     receivedPktToCpu = GT_TRUE;
    GT_U32                                      txPacketsCount = 0;
    GT_U32                                      numOfPacketsToSend = 0;
    GT_U32                                      packetIter = 0;
    CPSS_PORT_MAC_COUNTER_SET_STC               portCntrs;
    ...
    numOfPacketsToSend = PRV_CPSS_SIP_5_CHECK_MAC(prvTgfDevNum) ?
                         PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_MAX_CMD_CNS :
                         /* MIRROR and TRAP to CPU not supported in non SIP5 devices */
                        (PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_MAX_CMD_CNS - 2);

    /* save bridge access matrix command for SA/DA Access level 0 */
    rc = cpssDxChNstBridgeAccessMatrixCmdGet(prvTgfDevNum,
                                             PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_SA_LEVEL_CNS,
                                             PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_DA_LEVEL_CNS,
                                             &prvTgfRestoreCfg.packetCmd);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChNstBridgeAccessMatrixCmdGet");

    for(packetIter =0; packetIter < numOfPacketsToSend; packetIter++)
    {
        /* clear the queue counters */
        rc = cpssDxChNetIfSdmaRxCountersGet(prvTgfDevNum, PRV_TGF_NST_DEFAULT_TC_IDX_CNS, &rxCounters);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "cpssDxChNetIfSdmaRxCountersGet failed, device %d tcQueue %d\n",
                                     prvTgfDevNum, PRV_TGF_NST_DEFAULT_TC_IDX_CNS);
        ...
        targetPortInterface.devPort.portNum   = prvTgfPortsArray[PRV_TGF_EGRESS_PORT_IDX_CNS];
        ...
        /* set bridge access matrix command for SA/DA Access level 0 */
        rc = cpssDxChNstBridgeAccessMatrixCmdSet(prvTgfDevNum,
                                                 PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_SA_LEVEL_CNS,
                                                 PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_DA_LEVEL_CNS,
                                                 prvTgfNstCommandArray[packetIter]);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChNstBridgeAccessMatrixCmdSet");
        ...
        switch(prvTgfNstCommandArray[packetIter])
        {
        ...
        UTF_VERIFY_EQUAL2_STRING_MAC(txPacketsCount, portCntrs.goodPktsSent.l[0],
                                     "get another Tx packets than expected on port[%d] = %d",
                                     portIter, prvTgfPortsArray[PRV_TGF_EGRESS_PORT_IDX_CNS]);
        ...
        if(packetIter > 2)
        {
            ...
            receivedPktToCpu = (rxCounters.rxInPkts > 0) ? GT_TRUE : GT_FALSE;
            UTF_VERIFY_EQUAL0_STRING_MAC(expectedPktToCpu, receivedPktToCpu, "Packet count to CPU Mismatch");
        }
    }
}

/**
* @internal prvTgfNstBridgeAccessMatrixConfigurationRestore function
* @endinternal
*
* @brief Restore configuration
*/
GT_VOID prvTgfNstBridgeAccessMatrixConfigurationRestore
(
    GT_VOID
)
{
    ...
    /* Vlan restore */
    /* Restore default PVID on port 3 */
    rc = prvTgfBrgVlanPortVidSet(prvTgfDevNum,
        prvTgfPortsArray[PRV_TGF_EGRESS_PORT_IDX_CNS], prvTgfDefVlanId);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanPortVidSet");
    ...
    /* restore bridge access matrix command for SA/DA Access level 0 */
    rc = cpssDxChNstBridgeAccessMatrixCmdSet(prvTgfDevNum,
                                             PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_SA_LEVEL_CNS,
                                             PRV_TGF_NST_BRIDGE_ACCESS_MATRIX_DA_LEVEL_CNS,
                                             prvTgfRestoreCfg.packetCmd);
    ...
    /* Restore DA MAC */
    cpssOsMemCpy(prvTgfPacketL2Part.daMac, prvTgfRestoreCfg.daMac, sizeof(TGF_MAC_ADDR));
}
