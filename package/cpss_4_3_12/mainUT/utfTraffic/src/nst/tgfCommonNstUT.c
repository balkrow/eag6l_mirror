...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test Bridge Access Matrix
    configure VLAN, FDB
    configure Bridge Access Matrix to all commands
    SIP_4 supports only SOFT_DROP, HARD_DROP and FORWARD
    and SIP_5 supports additional MIRROR_TO_CPU and TRAP_TO_CPU
    send traffic
    verify that traffic based on the command
*/
UTF_TEST_CASE_MAC(tgfNstBridgeAccessMatrix)
{

    /* Set configuration */
    prvTgfNstBridgeAccessMatrixConfigurationSet();

    /* Generate traffic */
    prvTgfNstBridgeAccessMatrixTrafficGenerate();

    /* Restore configuration */
    prvTgfNstBridgeAccessMatrixConfigurationRestore();
}
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfNst)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfNstBridgeAccessMatrix)
