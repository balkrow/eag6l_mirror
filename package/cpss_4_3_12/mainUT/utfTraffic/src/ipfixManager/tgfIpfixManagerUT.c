...
UTF_TEST_CASE_MAC(tgfIpfixManagerFirstPacketsCheck)
{
    ...
    /* 1.1: Ipfix Manager config and initialization */
    rc = appDemoIpfixManagerMain(prvTgfDevNum,CPSS_DXCH_EXACT_MATCH_KEY_SIZE_19B_E);
 ...
 * @brief Test Learning of new flows
 *          1.1: Ipfix Manager config and initialization
 *          1.2: Send 4K flows and check they are learned
 *          1.3: Send same 4K flows again and check they are not learned again
 *          1.4: Delete Ipfix manager and restore configurations
 */
UTF_TEST_CASE_MAC(tgfIpfixManagerLearn4KFlows)
{
    GT_STATUS   rc = GT_OK;
    GT_U32      burstCount = PRV_TGF_FULL_CAPACITY_FLOWS_NUM_CNS;
    PRV_APP_IPFIX_MGR_DBG_COUNTERS_STC dbgCounters;
    ...
    PRV_UTF_LOG1_MAC("Learning %d flows ...\n", burstCount);

    /* 1.2: Send 4 flows and check they are learned */
    prvTgfIpfixManager4KFlowsTrafficGenerate();
    ...
    PRV_UTF_LOG0_MAC("Deleting all flows ... \n");
    rc = appDemoIpfixManagerFlowDeleteAllOneByOne(prvTgfDevNum);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "appDemoIpfixManagerFlowDeleteAll failed");
    appDemoIpfixManagerDebugCountersDump();

    PRV_UTF_LOG1_MAC("Re learing %d flows ...\n", burstCount);
    /* 1.2: Send 4 flows and check they are learned */
    prvTgfIpfixManager4KFlowsTrafficGenerate();
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfIpfixManager)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpfixManagerLearn4KFlows)
