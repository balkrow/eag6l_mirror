...
/* updated VLAN0 Id */
#define PRV_TGF_UPDATED_VLANID_CNS        6
...
/* EPCL CNC counter index */
#define PRV_TGF_EPCL_CNC_COUNTER_INDEX_CNS         0

/* LLC packets byte count range check fail Macros */
#define PRV_TGF_CUT_THROUGH_MIN_BYTE_COUNT_BOUNDARY    600
#define PRV_TGF_CUT_THROUGH_MAX_BYTE_COUNT_BOUNDARY    700
...
/* PARTS of the ETH_OTHER packet with L2 in passenger */

/* packet's ethertype */
static TGF_PACKET_ETHERTYPE_STC prvTgfEthOtherPacketEtherTypePart = {0x3333};

static TGF_PACKET_PART_STC prvTgfEthOtherPacketPartArray[] =
{
    {TGF_PACKET_PART_L2_E,        &prvTgfPacketL2Part},
    {TGF_PACKET_PART_ETHERTYPE_E, &prvTgfEthOtherPacketEtherTypePart},
    {TGF_PACKET_PART_PAYLOAD_E,   &prvTgfPacketPayloadPart}
};
...
static TGF_PACKET_STC prvTgfEthOtherPacketInfo =
{
    ...
    sizeof(prvTgfEthOtherPacketPartArray)
        / sizeof(prvTgfEthOtherPacketPartArray[0]),        /* numOfParts */
    prvTgfEthOtherPacketPartArray                          /* partsArray */
};

/* DATA of packet */
static GT_U8 prvTgfPayload64DataArr[] =
{
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
    0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
    0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
...
/* PAYLOAD part */
static TGF_PACKET_PAYLOAD_STC prvTgfPacketPayload64Part =
{
    sizeof(prvTgfPayload64DataArr),                       /* dataLength */
    prvTgfPayload64DataArr                                /* dataPtr */
};

/* PARTS of 64-byte payload ETH_OTHER packet */
static TGF_PACKET_PART_STC prvTgfEthOther64PacketPartArray[] =
{
    {TGF_PACKET_PART_L2_E,        &prvTgfPacketL2Part},  /* type, partPtr */
    {TGF_PACKET_PART_VLAN_TAG_E,  &prvTgfPacketVlanTagPart}, /*PRV_TGF_VLANID_CNS*/
    {TGF_PACKET_PART_ETHERTYPE_E, &prvTgfEthOtherPacketEtherTypePart}, /*3333*/
    {TGF_PACKET_PART_PAYLOAD_E,   &prvTgfPacketPayload64Part}
};

/* LENGTH of 64-byte payload ETH_OTHER packet */
#define PRV_TGF_ETH_OTHER_64_PACKET_LEN_CNS \
    TGF_L2_HEADER_SIZE_CNS + TGF_VLAN_TAG_SIZE_CNS + TGF_ETHERTYPE_SIZE_CNS +  \
    sizeof(prvTgfPayload64DataArr)

/* 64-byte payload ETH_OTHER PACKET to send */
static TGF_PACKET_STC prvTgfEthOther64PacketInfo =
{
    PRV_TGF_ETH_OTHER_64_PACKET_LEN_CNS,                                    /* totalLen */
    sizeof(prvTgfEthOther64PacketPartArray) / sizeof(TGF_PACKET_PART_STC), /* numOfParts */
    prvTgfEthOther64PacketPartArray                                        /* partsArray */
};
...
static struct
{
    ...
    GT_BOOL    layer2CutThrough;
    CPSS_DXCH_EARLY_PROCESSING_MODE_ENT    mode;
...
/**
* @internal prvTgfCutThroughBypassPclSip7CaseConfigure function
* @endinternal
*
* @brief   Configure Vlans, Lookup Bypass, PCL
*/
static GT_VOID prvTgfCutThroughBypassPclSip7CaseConfigure
(
    IN CPSS_PCL_DIRECTION_ENT                       direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT                   lookupNum,
    IN GT_BOOL                                      bypassLookup,  /* EPCL, MPCL */
    IN CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_ENT ipclBypassMode /* IPCL */
)
{
    GT_STATUS rc;
    TGF_TCAM_CLIENTS_PCL_LOOKUP_ENT tcamClientLookup;
    CPSS_DXCH_CUT_THROUGH_BYPASS_STC cpssPclBypass =
    {
        GT_FALSE,/* Router */
        GT_FALSE,/* Ingress Policer 0 */
        GT_FALSE,/* Ingress Policer 1 */
        GT_FALSE,/* Egress Policer*/
        GT_FALSE,/* EgressPCL */
        CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E, /* IPCL mode */
        GT_FALSE,/* IOAM */
        GT_FALSE,/* EOAM */
        GT_FALSE,/* MLL */
        GT_FALSE,/* FDB */
    ...
    static TGF_TCAM_CLIENTS_PCL_RULE_SUB_FIELD_STC emptySubFieldArr[] =
    {
        {TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E, 0,  0}
    };
    static TGF_TCAM_CLIENTS_PCL_SUB_FIELD_CONDITION_STC    emptySubFieldConditionArr[] =
    {
        {{TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E, 0,  0}, 0, 0}
    };
    CPSS_DXCH_PCL_ACTION_STC         cpssAction;
    ...
    rc = prvTgfBrgDefVlanEntryWithPortsSet(
        PRV_TGF_VLANID_CNS,
        NULL /*portsArray*/, NULL /*devArray*/, NULL /*tagArray*/,
        0 /*numPorts*/);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgDefVlanEntryWithPortsSet");

    rc = prvTgfBrgVlanTagMemberAdd(
        PRV_TGF_VLANID_CNS, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        PRV_TGF_BRG_VLAN_PORT_OUTER_TAG0_INNER_TAG1_CMD_E);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanTagMemberAdd");

    rc = prvTgfBrgVlanTagMemberAdd(
        PRV_TGF_VLANID_CNS, prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS],
        PRV_TGF_BRG_VLAN_PORT_OUTER_TAG0_INNER_TAG1_CMD_E);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanTagMemberAdd");

    rc = prvTgfBrgDefVlanEntryWithPortsSet(
        PRV_TGF_UPDATED_VLANID_CNS,
        NULL /*portsArray*/, NULL /*devArray*/, NULL /*tagArray*/,
        0 /*numPorts*/);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgDefVlanEntryWithPortsSet");

    rc = prvTgfBrgVlanTagMemberAdd(
        PRV_TGF_UPDATED_VLANID_CNS, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        PRV_TGF_BRG_VLAN_PORT_OUTER_TAG0_INNER_TAG1_CMD_E);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanTagMemberAdd");

    rc = prvTgfBrgVlanTagMemberAdd(
        PRV_TGF_UPDATED_VLANID_CNS, prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS],
        PRV_TGF_BRG_VLAN_PORT_OUTER_TAG0_INNER_TAG1_CMD_E);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanTagMemberAdd");

    portNum = (direction == CPSS_PCL_DIRECTION_EGRESS_E)
        ? prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS]
        : prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS];
    ...
    switch (direction)
    {
        ...
        case CPSS_PCL_DIRECTION_INGRESS_E:
            cpssPclBypass.bypassIngressPcl = ipclBypassMode;
            break;
        case CPSS_PCL_DIRECTION_MIDWAY_E:
            cpssPclBypass.bypassMidwayPcl = bypassLookup;
            break;
        case CPSS_PCL_DIRECTION_EGRESS_E:
            cpssPclBypass.bypassEgressPcl = bypassLookup;
            break;
    }

    rc = cpssDxChCutThroughBypassModeSet(prvTgfDevNum, &cpssPclBypass);
    if (GT_OK != rc)
    {
    ...
    tcamClientLookup =
        tgfTcamClientsPclDirLookupToClientLookup(direction, lookupNum);


    prvTgfSip7TcamClientsDbHandleDelete();
    rc = prvTgfSip7TcamClientsOneLookupDevInit(
        prvTgfDevNum, tcamClientLookup,
        CPSS_DXCH_TCAM_RULE_SIZE_30_B_E, emptySubFieldArr);

    /* Ingress */
    rc = prvTgfPclDefPortInitSip7(
        portNum, direction, lookupNum,
        PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_BY_PORT_E,
        0 /*cfgIndex*/, GT_TRUE /*enableLookup*/,
        pclId, 1/*emSubProfileId*/);
    UTF_VERIFY_EQUAL2_STRING_MAC(
        GT_OK, rc, "prvTgfPclDefPortInitSip7: %d, %d",
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS]);

    cpssOsMemSet(&cpssAction, 0, sizeof(cpssAction));
    cpssAction.egressPolicy         = direction;
    cpssAction.pktCmd               = CPSS_PACKET_CMD_FORWARD_E;
    if (direction == CPSS_PCL_DIRECTION_EGRESS_E)
    {
        cpssAction.vlan.egress.vlanId = PRV_TGF_UPDATED_VLANID_CNS;
        cpssAction.vlan.egress.vlanCmd =
            CPSS_DXCH_PCL_ACTION_EGRESS_TAG0_CMD_MODIFY_OUTER_TAG_E;
    }
    else
    {
        cpssAction.vlan.ingress.vlanId = PRV_TGF_UPDATED_VLANID_CNS;
        cpssAction.vlan.ingress.modifyVlan =
            CPSS_PACKET_ATTRIBUTE_ASSIGN_FOR_ALL_E;
    }
    commonKeyHeaderMask    = 0x3FF;
    commonKeyHeaderPattern = 0x3FF & pclId;
    rc = tgfTcamClientsPclRuleSet(
        prvTgfDevNum,
        prvTgfTcamClientCommomHandlersHandleGet(0),
        1 /*pclCfgProfileId*/,
        1 /*udbPacketTypeGroupIndex*/,
        tcamClientLookup,
        0 /*tcamSubkeyIndex*/,
        0/*ruleIndex*/,
        GT_TRUE /*writeValidRule*/,
        &commonKeyHeaderMask, &commonKeyHeaderPattern,
        emptySubFieldConditionArr, &cpssAction);
    UTF_VERIFY_EQUAL0_STRING_MAC(
        GT_OK, rc, "tgfTcamClientsPclRuleSet");
}

/**
* @internal prvTgfCutThroughBypassPclSip7CaseRestore function
* @endinternal
*
* @brief   Restore configuration
*/
static GT_VOID prvTgfCutThroughBypassPclSip7CaseRestore
(
    IN CPSS_PCL_DIRECTION_ENT                       direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT                   lookupNum
)
{
    ...
    rc = prvTgfPclDefPortInitSip7(
        portNum, direction, lookupNum,
        PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_BY_PORT_E,
        0 /*cfgIndex*/, GT_FALSE /*enableLookup*/,
        0 /*pclId*/, 1/*emSubProfileId*/);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: prvTgfPclDefPortInitSip7 FAILED, rc = [%d]", rc);
    }

    /* AUTODOC: invalidate PCL rules */
    rc = prvTgfPclRuleValidStatusSet(
        CPSS_PCL_RULE_SIZE_STD_E, 0/*ruleIndex*/, GT_FALSE);
    if (GT_OK != rc)
    {
        PRV_UTF_LOG1_MAC("[TGF]: prvTgfPclRuleValidStatusSet FAILED, rc = [%d]", rc);
    }

    prvTgfBrgDefVlanEntryInvalidate(PRV_TGF_VLANID_CNS);

    prvTgfBrgDefVlanEntryInvalidate(PRV_TGF_UPDATED_VLANID_CNS);
}

/**
* @internal prvTgfCutThroughBypassPclSip7TrafficGenerate function
* @endinternal
*
* @brief   Generate and check testing traffic
*/
static GT_VOID prvTgfCutThroughBypassPclSip7TrafficGenerate
(
    IN GT_BOOL vlanUpdated
)
{
    GT_STATUS rc;
    GT_U32 expectedVlan;

    /* AUTODOC: reset counters, force Link UP, clear capturing RxPcktTable */
    rc = prvTgfEthCountersReset(prvTgfDevNum);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfEthCountersReset");

    expectedVlan =
        (vlanUpdated == GT_FALSE) ? PRV_TGF_VLANID_CNS : PRV_TGF_UPDATED_VLANID_CNS;

    rc = prvTgfTransmitPacketsWithCapture(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        &prvTgfEthOther64PacketInfo,
        1      /*burstCount*/,
        0      /*numVfd*/,
        NULL   /*vfdArray*/,
    ...
    prvTgfPclTrafficEgressVidCheck(
        prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS],
        (GT_U16)expectedVlan, GT_FALSE /*checkMsb*/);
}

/**
* @internal prvTgfCutThroughBypassPclSip7Test function
* @endinternal
*
* @brief   SIP7 PCL Bypass test
*/
GT_VOID prvTgfCutThroughBypassPclSip7Test
(
    GT_VOID
)
{
    ...
    /* IPCL Lookup0 */
    prvTgfCutThroughBypassPclSip7CaseConfigure(
        CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_NUMBER_0_E,
        GT_FALSE /*bypassLookup*/,
        CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E);
    prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_TRUE);
    prvTgfCutThroughBypassPclSip7CaseRestore(
        CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_NUMBER_0_E);
    ...
    prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_FALSE);
    prvTgfCutThroughBypassPclSip7CaseRestore(
        CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_NUMBER_0_E);
    ...
    /* IPCL Lookup1 */
    prvTgfCutThroughBypassPclSip7CaseConfigure(
        CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_NUMBER_1_E,
        GT_FALSE /*bypassLookup*/,
        CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E);
    prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_TRUE);
    prvTgfCutThroughBypassPclSip7CaseRestore(
    ...
    if (PRV_CPSS_DXCH_PP_MAC(prvTgfDevNum)->hwInfo.pcl.iPcl0Bypass == 0)
    {
        /* IPCL Lookup2 */
        prvTgfCutThroughBypassPclSip7CaseConfigure(
            CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_NUMBER_2_E,
            GT_FALSE /*bypassLookup*/,
            CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E);
        prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_TRUE);
        prvTgfCutThroughBypassPclSip7CaseRestore(
    ...
    /* MPCL */
    prvTgfCutThroughBypassPclSip7CaseConfigure(
        CPSS_PCL_DIRECTION_MIDWAY_E, CPSS_PCL_LOOKUP_NUMBER_0_E,
        GT_FALSE /*bypassLookup*/,
        CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E);
    prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_TRUE);
    prvTgfCutThroughBypassPclSip7CaseRestore(
    ...
    /* EPCL */
    prvTgfCutThroughBypassPclSip7CaseConfigure(
        CPSS_PCL_DIRECTION_EGRESS_E, CPSS_PCL_LOOKUP_NUMBER_0_E,
        GT_FALSE /*bypassLookup*/,
        CPSS_DXCH_CUT_THROUGH_BYPASS_INGRESS_PCL_NO_BYPASS_E);
    prvTgfCutThroughBypassPclSip7TrafficGenerate(GT_TRUE);
    prvTgfCutThroughBypassPclSip7CaseRestore(
...
)
{
         ...
         ii++)
    {
        ...
        policerEntry.modifyDscp = PRV_TGF_POLICER_MODIFY_DSCP_DISABLE_E;
        ...
        policerEntry.modifyUp   = PRV_TGF_POLICER_MODIFY_UP_DISABLE_E;
...
)
{
    ...
    GT_U32 commonKeyHeaderMask;
    GT_U32 commonKeyHeaderPattern;
    GT_U32 pclIdIngress = 0x33;
    GT_U32 pclIdEgress  = 0x44;
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        rc = prvTgfPclDefPortInitSip7(
            ...
            PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_BY_PORT_E,
            0 /*cfgIndex*/, GT_TRUE /*enableLookup*/,
            pclIdIngress, 1/*emSubProfileId*/);
        UTF_VERIFY_EQUAL2_STRING_MAC(
            GT_OK, rc, "prvTgfPclDefPortInitSip7: %d, %d",
            prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS]);

            cpssOsMemSet(&cpssAction, 0, sizeof(cpssAction));
            cpssAction.egressPolicy         = CPSS_PCL_DIRECTION_INGRESS_E;
            cpssAction.pktCmd               = CPSS_PACKET_CMD_FORWARD_E;
            cpssAction.oam.oamProcessEnable = GT_TRUE;
            cpssAction.oam.oamProfile       = 1;
            cpssAction.flowId               = PRV_TGF_IPCL_ACTION_FLOW_ID_CNS;
            commonKeyHeaderMask    = 0x3FF;
            commonKeyHeaderPattern = 0x3FF & pclIdIngress;
        rc = tgfTcamClientsPclRuleSet(
            prvTgfDevNum,
            prvTgfTcamClientCommomHandlersHandleGet(0),
            1 /*pclCfgProfileId*/,
            ...
            0 /*tcamSubkeyIndex*/,
            (6 * prvTgfIpclTcamAbsoluteIndexWithinTheLookupGet(0, PRV_TGF_OAM_IPCL_RULE_IDX_CNS)),
            GT_TRUE /*writeValidRule*/,
            &commonKeyHeaderMask, &commonKeyHeaderPattern,
            emptySubFieldConditionArr, &cpssAction);
        UTF_VERIFY_EQUAL0_STRING_MAC(
        ...
        rc = prvTgfPclDefPortInitSip7(
            ...
            PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_BY_PORT_E,
            0 /*cfgIndex*/, GT_TRUE /*enableLookup*/,
            pclIdEgress, 1/*emSubProfileId*/);
        UTF_VERIFY_EQUAL2_STRING_MAC(
            GT_OK, rc, "prvTgfPclDefPortInitSip7: %d, %d",
            prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_ANOTHER_CNS]);

            cpssOsMemSet(&cpssAction, 0, sizeof(cpssAction));
            cpssAction.egressPolicy         = CPSS_PCL_DIRECTION_EGRESS_E;
            cpssAction.pktCmd               = CPSS_PACKET_CMD_FORWARD_E;
            cpssAction.oam.oamProcessEnable = GT_TRUE;
            cpssAction.oam.oamProfile       = 1;
            cpssAction.flowId               = PRV_TGF_EPCL_ACTION_FLOW_ID_CNS;
            commonKeyHeaderMask    = 0x3FF;
            commonKeyHeaderPattern = 0x3FF & pclIdEgress;
        rc = tgfTcamClientsPclRuleSet(
            prvTgfDevNum,
            prvTgfTcamClientCommomHandlersHandleGet(0),
            1 /*pclCfgProfileId*/,
            ...
            0 /*tcamSubkeyIndex*/,
            (6 * prvTgfIpclTcamAbsoluteIndexWithinTheLookupGet(0, PRV_TGF_OAM_EPCL_RULE_IDX_CNS)),
            GT_TRUE /*writeValidRule*/,
            &commonKeyHeaderMask, &commonKeyHeaderPattern,
            emptySubFieldConditionArr, &cpssAction);
        UTF_VERIFY_EQUAL0_STRING_MAC(
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        rc = prvTgfPclRuleValidStatusSet(CPSS_PCL_RULE_SIZE_60_BYTES_E,
            (6 * prvTgfEpclTcamAbsoluteIndexWithinTheLookupGet(PRV_TGF_OAM_EPCL_RULE_IDX_CNS)),
                                         GT_FALSE);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfPclRuleValidStatusSet: "
            "Rule size = [%d], rule index = [%d]",
            CPSS_PCL_RULE_SIZE_60_BYTES_E, PRV_TGF_OAM_EPCL_RULE_IDX_CNS);
        ...
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfPclRuleValidStatusSet: "
            "Rule size = [%d], rule index = [%d]",
            CPSS_PCL_RULE_SIZE_60_BYTES_E, PRV_TGF_OAM_IPCL_RULE_IDX_CNS);
    }
    else
    {
        rc = prvTgfPclRuleValidStatusSet(CPSS_PCL_RULE_SIZE_STD_E,
...
(
    IN  GT_U32                          counterIndex/*sendPortNum*/,
    IN  GT_U32                          cncBlock,
    IN  PRV_TGF_CNC_CLIENT_ENT          cncClient
)
{
    GT_STATUS                       rc;
    ...
    rc = prvTgfCncCounterSet(
        cncBlock, counterIndex /*sendPortNum*/,
...
)
{
    ...
    rc = prvTgfCncBlockClientEnableSet(
        cncBlock, cncClient, GT_FALSE/*enable*/);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfCncBlockClientEnableSet");
}
...
static GT_VOID prvTgfSendPacketWithCapture
(
    IN  GT_U32          sendPortNum,
...
)
{
    prvTgfCutThroughTestCncConfigure(
        sendPortNum, cncBlock, PRV_TGF_CNC_CLIENT_INGRESS_SRC_EPORT_E);

    prvTgfSendPacketWithCapture(
        sendPortNum, recievePortNum, packetInfoPtr,
        skippedPacketPartsBitmap,
        patternPacketLen, comparePacketLen);
...
)
{
    ...
    GT_U32                          nonComparedBytesArr[1];
    GT_U32                          actualCapturedNumOfPackets;
    GT_U32                          rxToCpCountZero;
    GT_U32                          cpToRxCountZero;
    ...
    rc = prvTgfPortDpDebugCpToRxCountingConfigSet(
        ...
        GT_FALSE/*highPriorityPacket*/, GT_FALSE/*contextDone*/,
        GT_FALSE/*descriptorValid*/, GT_FALSE /*layer2CutThrough*/);
    ...
    {
        /* Check clear on read counters behavior */
        rc = prvTgfPortDpDebugRxToCpCountValueGet(
            prvTgfDevNum, sendPortNum, &rxToCpCountZero);
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfPortDpDebugRxToCpCountValueGet");
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN(0, rxToCpCountZero, "wrong clear on read");

        rc = prvTgfPortDpDebugCpToRxCountValueGet(
            prvTgfDevNum, sendPortNum, &cpToRxCountZero);
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfPortDpDebugCpToRxCountValueGet");
        UTF_VERIFY_EQUAL0_STRING_NO_RETURN(0, cpToRxCountZero, "wrong clear on read");
    }
}
...
)
{
    ...
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(
        0, extractBcFailsCount, "extractBcFailsCount must be zero");
...
)
{
    ...
    {
        rc = prvTgfSip7TcamClientsOneLookupDevInit(
            prvTgfDevNum, TGF_TCAM_CLIENTS_PCL_LOOKUP_EPCL_E,
            CPSS_DXCH_TCAM_RULE_SIZE_30_B_E, emptySubFieldArr);
        ...
        UTF_VERIFY_EQUAL2_STRING_MAC(
            ...
            cpssAction.egressPolicy            = CPSS_PCL_DIRECTION_EGRESS_E;
            cpssAction.pktCmd                  = CPSS_PACKET_CMD_FORWARD_E;
            cpssAction.terminateCutThroughMode = GT_TRUE;
            cpssAction.matchCounter.enableMatchCount = GT_TRUE;
            cpssAction.matchCounter.matchCounterIndex= PRV_TGF_EPCL_CNC_COUNTER_INDEX_CNS;
        rc = tgfTcamClientsPclRuleSet(
            prvTgfDevNum,
            prvTgfTcamClientCommomHandlersHandleGet(0),
            1 /*pclCfgProfileId*/,
            0 /*udbPacketTypeGroupIndex*/,
            TGF_TCAM_CLIENTS_PCL_LOOKUP_EPCL_E,
            0 /*tcamSubkeyIndex*/,
            ruleIndex,
            GT_TRUE /*writeValidRule*/,
    ...
    else
    {
        ...
        action.matchCounter.enableMatchCount = GT_TRUE;
        action.matchCounter.matchCounterIndex= PRV_TGF_EPCL_CNC_COUNTER_INDEX_CNS;
...
/**
* @internal prvTgfCutThroughIpv6CtTerminationTest_EPCL_CncZero function
* @endinternal
*
* @brief   Test EPCL caused Cut Through termination Zero CNC Counters
*/
GT_VOID prvTgfCutThroughIpv6CtTerminationTest_EPCL_CncZero
(
    GT_VOID
)
{
    ...
    prvTgfCutThroughTestCncConfigure(
        PRV_TGF_EPCL_CNC_COUNTER_INDEX_CNS, 0 /*cncBlock*/,
        PRV_TGF_CNC_CLIENT_EGRESS_PCL_E);

    /* do not compare payload - expected to be corrupted                          */
    /* byteCount calculated by header is less than ingress packet size            */
    /* cut through terminated and egress packet padded by ingress packet reminder */
    expectedByteCount = prvTgfIpv6PacketInfo.totalLen;
    egressByteCount = (prvTgfIpv6PacketInfo.totalLen - prvTgfPacketPayloadPart.dataLength);
    prvTgfSendPacketWithCapture(
        prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS],
        &prvTgfIpv6PacketInfo,
        0 /*skippedPacketPartsBitmap*/,
        egressByteCount /*patternPacketLen*/,
        (egressByteCount) /*comparePacketLen*/);

    rc = prvTgfCncCounterGet(
        0/*cncBlock*/, PRV_TGF_EPCL_CNC_COUNTER_INDEX_CNS,
        PRV_TGF_CNC_COUNTER_FORMAT_MODE_0_E, &counter);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfCncCounterGet");
    PRV_UTF_LOG2_MAC(
        "IPV6 packetCount 0x%08X, byteCount: 0x%08X\n",
        counter.packetCount.l[0], counter.byteCount.l[0]);
    ...
    rc = prvTgfCutThroughAllPacketTypesEnableSet(GT_TRUE);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(
        GT_OK, rc, "prvTgfCutThroughAllPacketTypesEnableSet");

    expectedByteCount = prvTgfEthOtherPacketInfo.totalLen;
    egressByteCount = expectedByteCount;
    prvTgfSendPacketWithCapture(
        prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS],
    ...
    rc = prvTgfCutThroughAllPacketTypesEnableSet(GT_FALSE);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(
        GT_OK, rc, "prvTgfCutThroughAllPacketTypesEnableSet");
    ...
    PRV_UTF_LOG2_MAC(
        ...
        counter.packetCount.l[0], counter.byteCount.l[0]);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(
        0, counter.byteCount.l[0], "0 != CNC byte count");
...
/**
* @internal prvTgfCutThroughByteCountRangeCheckFailConfigSet function
* @endinternal
*
* @brief   Config to test CT byte count range check fail
*          1. configure bridge.
*          2. configure debug Cp to Rx packets to count cutThroughterminated packect only.
*          3. configure CT header integrity check to fail byte count range check.
*          4. configure early processing mode according to the passed mode.
*/
static GT_VOID prvTgfCutThroughByteCountRangeCheckFailConfigSet
(
    CPSS_DXCH_EARLY_PROCESSING_MODE_ENT    mode
)
{
    ...
    GT_STATUS                                                   rc;
    GT_U32                                                      portsArr[2];
    GT_U32                                                      criteriaParamBmp        = 68;
    GT_BOOL                                                     multicastPacket         = GT_FALSE;
    GT_BOOL                                                     cutThroughPacket        = GT_FALSE;
    GT_BOOL                                                     cutThroughTerminated    = GT_FALSE;
    GT_BOOL                                                     trunkatedHeader         = GT_FALSE;
    GT_BOOL                                                     dummyDescriptor         = GT_FALSE;
    GT_BOOL                                                     highPriorityPacket      = GT_FALSE;
    GT_BOOL                                                     contextDone             = GT_FALSE;
    GT_BOOL                                                     descriptorValid         = GT_FALSE;
    GT_BOOL                                                     layer2CutThrough        = GT_FALSE;
    ...
    /* AUTODOC: Set port vid 5 of ingress port */
    rc = prvTgfBrgVlanPortVidSet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS], PRV_TGF_VLANID_CNS);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanPortVidSet");

    rc = prvTgfPortDpDebugCountersResetAndEnableSet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS], GT_TRUE/*enable*/);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfPortDpDebugCountersResetAndEnableSet");

    /* AUTODOC: save configuration for counting Cp to Rx packets */
    rc = cpssDxChPortDpDebugCpToRxCountingConfigGet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        &prvTgfRestoreCfg.criteriaParamBmp, &prvTgfRestoreCfg.multicastPacket, &prvTgfRestoreCfg.cutThroughPacket,
        &prvTgfRestoreCfg.cutThroughTerminated, &prvTgfRestoreCfg.trunkatedHeader, &prvTgfRestoreCfg.dummyDescriptor,
        &prvTgfRestoreCfg.highPriorityPacket, &prvTgfRestoreCfg.contextDone, &prvTgfRestoreCfg.descriptorValid,
        &prvTgfRestoreCfg.layer2CutThrough);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChPortDpDebugCpToRxCountingConfigGet");

    /* AUTODOC: configure to count only cutThroughTerminated Cp to Rx packets */
    cutThroughTerminated = GT_TRUE;
    rc = cpssDxChPortDpDebugCpToRxCountingConfigSet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        criteriaParamBmp, multicastPacket, cutThroughPacket, cutThroughTerminated, trunkatedHeader,
        dummyDescriptor, highPriorityPacket, contextDone, descriptorValid, layer2CutThrough);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChPortDpDebugCpToRxCountingConfigSet");

    /* AUTODOC: save CT header integrity check config */
    rc = cpssDxChCutThroughPacketHdrIntegrityCheckCfgGet(
        prvTgfDevNum, &prvTgfRestoreCfg.cfg);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughPacketHdrIntegrityCheckCfgGet");

    /* AUTODOC: configure CT header integrity check */
    cfg.minByteCountBoundary = PRV_TGF_CUT_THROUGH_MIN_BYTE_COUNT_BOUNDARY;
    cfg.maxByteCountBoundary = PRV_TGF_CUT_THROUGH_MAX_BYTE_COUNT_BOUNDARY;
    cfg.enableByteCountBoundariesCheck = GT_TRUE;
    cfg.enableIpv4HdrCheckByChecksum   = GT_FALSE;
    cfg.hdrIntergrityExceptionPktCmd   = CPSS_PACKET_CMD_FORWARD_E;
    cfg.hdrIntergrityExceptionCpuCode  = CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E;
    rc = cpssDxChCutThroughPacketHdrIntegrityCheckCfgSet(
        prvTgfDevNum, &cfg);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughPacketHdrIntegrityCheckCfgSet");

    /* AUTODOC: save CT early processing mode */
    rc = cpssDxChCutThroughEarlyProcessingModeGet(
        prvTgfDevNum, &prvTgfRestoreCfg.mode);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughEarlyProcessingModeGet");

    /* AUTODOC: configure CT early processing mode */
    rc = cpssDxChCutThroughEarlyProcessingModeSet(
        prvTgfDevNum, mode);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughEarlyProcessingModeSet");
}

/**
* @internal prvTgfCutThroughByteCountRangeCheckFailConfigTrafficSendAndValidate function
* @endinternal
*
* @brief  Traffic test for LLC packet for CT byte count range check fail
*/
static GT_VOID prvTgfCutThroughByteCountRangeCheckFailConfigTrafficSendAndValidate
(
    GT_VOID
)
{
    ...
    GT_STATUS                rc                 = GT_OK;
    GT_U32                   countValue         = 0;
    GT_BOOL                  isCountExpected    = GT_FALSE;

    /* AUTODOC: Clear debug Cp to Rx counters */
    rc = cpssDxChPortDpDebugCpToRxCountValueGet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS], &countValue);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChPortDpDebugCpToRxCountValueGet");
    ...
    targetPortInterface.devPort.portNum   = prvTgfPortsArray[PRV_TGF_RECEIVE_PORT_IDX_CNS];
    rc = tgfTrafficGeneratorPortTxEthCaptureSet(
        &targetPortInterface, TGF_CAPTURE_MODE_MIRRORING_E, GT_TRUE);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "tgfTrafficGeneratorPortTxEthCaptureSet");

    rc = prvTgfSetTxSetupEth(
        prvTgfDevNum, &prvTgfLlcPacketInfo, 1, 0, NULL);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfSetTxSetupEth");
    ...
    rc = tgfTrafficGeneratorRxInCpuFromExpectedInterfaceGet(
        &targetPortInterface, TGF_PACKET_TYPE_CAPTURE_E, getFirst, GT_TRUE, packetBuf,
        &packetLen, &packetActualLength, &dev, &queue, &rxParam);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
            "Error: failed capture on inPortId %d outPortId %d \n" );

    /* AUTODOC: Get debug Cp to Rx counters */
    rc = cpssDxChPortDpDebugCpToRxCountValueGet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS], &countValue);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChPortDpDebugCpToRxCountValueGet");

    /* AUTODOC: Validate debug Cp to Rx counters */
    isCountExpected = (countValue > 0) ? GT_TRUE : GT_FALSE;
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_TRUE, isCountExpected, "expected count mismatch");

    /* AUTODOC: Clear RX packet table */
    tgfTrafficTableRxPcktTblClear();
}

/**
* @internal prvTgfCutThroughByteCountRangeCheckFailConfigReset function
* @endinternal
*
* @brief   Restore configuration for  CT byte count range check fail
*          1. Restore bridge.
*          2. Restore debug Cp to Rx packets configuration.
*          3. Restore CT header integrity check configuration.
*          4. Restore early processing mode.
*/
static GT_VOID prvTgfCutThroughByteCountRangeCheckFailConfigReset
(
    GT_VOID
)
{
    ...
    /* AUTODOC: Restore port vid of ingress port */
    rc = prvTgfBrgVlanPortVidSet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS], 1 /*vlanId*/);
    UTF_VERIFY_EQUAL0_STRING_NO_RETURN(GT_OK, rc, "prvTgfBrgVlanPortVidSet");
    ...
    /* AUTODOC: Restore configuration for counting Cp to Rx packets */
    rc = cpssDxChPortDpDebugCpToRxCountingConfigSet(
        prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],
        prvTgfRestoreCfg.criteriaParamBmp, prvTgfRestoreCfg.multicastPacket, prvTgfRestoreCfg.cutThroughPacket,
        prvTgfRestoreCfg.cutThroughTerminated, prvTgfRestoreCfg.trunkatedHeader, prvTgfRestoreCfg.dummyDescriptor,
    ...
    /* AUTODOC: Restore configuration for CT header integrity check */
    rc = cpssDxChCutThroughPacketHdrIntegrityCheckCfgSet(
        prvTgfDevNum, &prvTgfRestoreCfg.cfg);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughPacketHdrIntegrityCheckCfgSet");

    /* AUTODOC: Restore CT early processing mode */
    rc = cpssDxChCutThroughEarlyProcessingModeSet(
        prvTgfDevNum, prvTgfRestoreCfg.mode);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChCutThroughEarlyProcessingModeSet");
}
...
/**
* @internal prvTgfCutThroughByteCountRangeCheckFail function
* @endinternal
*
* @brief   Test CT byte count range check fail
*/
GT_VOID prvTgfCutThroughByteCountRangeCheckFail
(
    GT_VOID
)
{
    /* AUTODOC: Test 1: In case of cutThrough2StoreAndForward is disabled and
     * byte count range fails the CT was not disbaled.
     * This test ensures that when cutThrough2StoreAndForward is disabled and
     * byte count range fails the CT is disbaled.
     */
    /* AUTODOC: Configure CT byte count range check fail */
    prvTgfCutThroughByteCountRangeCheckFailConfigSet(
        CPSS_DXCH_EARLY_PROCESSING_CUT_THROUGH_E);
    /* AUTODOC: Send Traffic and validate the CT is disabled */
    prvTgfCutThroughByteCountRangeCheckFailConfigTrafficSendAndValidate();
    /* AUTODOC: Configure CT byte count range check fail */
    prvTgfCutThroughByteCountRangeCheckFailConfigReset();
}
