...
)
{
    ...
    else if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN2_E)
    {
        ...
        /* need to configure ports with forced link UP in Egress Link Filter for Aldrin2 with Port manager */
        if (portMgr)
        {
            egfConfig = GT_TRUE;
        }
    ...
    for (portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
    {
        ...
        if (egfConfig)
        {
            /* need to manage EGF filter for special cases of PM implementation */
...
)
{
    ...
    if(GT_TRUE == prvUtfIsPortManagerUsed())
    {
        /* disable PM task to improve accuracy of rate calculation */
...
)
{
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ...
        {
            /* there is no oversubscription in IronMan */
            return GT_OK;
        }
        ...
        {
            /* Harrier A1 has 600Mhz core clock. This may take care of 403.2 Gbps of 64 byte packets.
               Each port is 50G. Only 8 ports may be used to avoid oversubscription. */
            maxPortPerPipe = 8;
        }
        else
        {
