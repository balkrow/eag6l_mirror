...
#include <port/prvTgfPortArbitrationGroups.h>
...
UTF_TEST_CASE_MAC(tgfPortTxTailDropDbaQueueResources)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E |
        ...
        UTF_ALDRIN_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Check SP% to SIP6 backword compatability logic
Configuration.
1. Configure scheduling profile using SIP5 API
2. Compare to expected SIP6 eligeble function.
3. Restore
*/

UTF_TEST_CASE_MAC(prvTgfPortArbitrationGroupsManipulations)
{
    GT_U32 notAppFamilyBmp;
    GT_U32 maxIter,i;
    ...
    prvTgfPortArbitrationGroupConfigurationsSizeGet(&maxIter);

    prvTgfPortArbitrationGroupSave();

    for(i=0;i<maxIter;i++)
    {
        prvTgfPortArbitrationGroupConfigure(i,GT_FALSE);
        prvTgfPortArbitrationGroupCheckResult(i);
    }


    for(i=0;i<maxIter;i++)
    {
        prvTgfPortArbitrationGroupConfigure(i,GT_TRUE);
        prvTgfPortArbitrationGroupCheckResult(i);
    }

    prvTgfPortArbitrationGroupRestore();

}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test checks addition of Radio Header before MAC header
   - Use PVE for forwarding to egress port
   - enable recognition of PTP packets and enabled timestamp addition for PTP packets
   - Send several packets
      -- configure Traffic Class for packet
      -- capture packet on egress port
      -- validate all Radio Header fields of captured packet
      -- validate position of timestamp in captured PTP packet
*/
UTF_TEST_CASE_MAC(tgfPortRadioHeader)
{
    ...
    tgfPortRadioHeaderConfigSet();
    tgfPortRadioHeaderTest();
    tgfPortRadioHeaderConfigRestore();
}
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfPort)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPortArbitrationGroupsManipulations)
    UTF_SUIT_DECLARE_TEST_MAC(tgfPortRadioHeader)
