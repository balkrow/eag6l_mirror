...
#include <appDemo/boardConfig/gtDbDxBobcat2Mappings.h> /*For HQOS_UNAWARE*/
...
GT_U32 prvTgfPortTxSpIngrPortsNumSet(GT_U32 newPortNum)
{
    GT_U32 oldPortNum = prvTgfPortTxSpIngrPortsNum;
    prvTgfPortTxSpIngrPortsNum = newPortNum;
    return oldPortNum;
}
...
)
{
    ...
    else
    {
        ...
        {
            /* test may support only 3 ingress ports */
            prvTgfPortTxSpIngrPortsNumSet(3);
        }
    }
    ...
    {
        /* disable PM task to avoid problems on XG ports */
...
)
{
    ...
    if (PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
    {
        /* restore number of ingress ports */
        prvTgfPortTxSpIngrPortsNumSet(4);
    }
    ...
    if((prvWrAppDbEntryGet("portMgr", &portMgrValue) == GT_OK) && (portMgrValue != 0))
    {
        ...
        /* provide time for PM task to restart */
...
(
    ...
    OUT GT_BOOL *wirespeedReachedPtr
)
{
    GT_STATUS       rc;                          /* return code*/
    ...
    GT_U32          i,j=0;                           /* iterator */
    GT_U32           portNum;                     /* port number */
    GT_U32          tmp;                         /* variable */

    *wirespeedReachedPtr = GT_TRUE;
        ...
        for(j=0;j<MAX_WS_RETRY_CNS;j++)
        {
            ...
            if (tmp)
            {
                /* the rate is not good .
                   start again */
                PRV_UTF_LOG3_MAC("Iteration %d [TxSpScheduler]: wrong ingress port %d rate %d\n",j, prvTgfPortsArray[inPortsIdxArr[i]], rateArr[i]);
            ...
            {
                PRV_UTF_LOG3_MAC("Iteration %d [TxSpScheduler]: correct ingress port %d rate %d\n",j, prvTgfPortsArray[inPortsIdxArr[i]], rateArr[i]);
                /*correct rate is reached.Lets sleep so it will become more stable*/
                cpssOsTimerWkAfter(1000);
                break;
            }
          }
          PRV_UTF_LOG1_MAC("Last iteration index -> %d\n",j);

          if(j == MAX_WS_RETRY_CNS)
          {
              *wirespeedReachedPtr = GT_FALSE;
          }
...
)
{
    ...
    else if(GT_FALSE == prvUtfDeviceTestNotSupport(prvTgfDevNum, UTF_IRONMAN_L_E))
    {
        /* IronMan M board has 8 Gig and 4 XG ports.
           One of RX ports is 10G but other RX and TX are 1 Gig ports
           Drop rate for case 10G RX is diff between rates */
        if ((txRate < 1600000) && (rxRate > 1600000))
        {
...
)
{
    ...
    GT_BOOL      wirespeedReached = GT_FALSE;
    ...
    /* send packets */
    prvTgfPortTxSpSchedulerPacketSend(rateArr,&wirespeedReached);
    PRV_UTF_LOG1_MAC("wirespeedReached %d\n", wirespeedReached);

    if(GT_FALSE ==wirespeedReached )
    {
        PRV_UTF_LOG0_MAC("Wirespeed was not reached so aborting further validation.\n");
        return;
    }
...
)
{
    static CPSS_DXCH_PORT_MAP_STC  portMapArray[] =
    {
       ...
       ,{12, CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E,   0,       12,    1,   GT_FALSE,   GT_NA,GT_FALSE,HQOS_UNAWARE}
       ,{63, CPSS_DXCH_PORT_MAPPING_TYPE_CPU_SDMA_E,       0,    GT_NA,   95,   GT_FALSE,   GT_NA,GT_FALSE,HQOS_UNAWARE}
    };
