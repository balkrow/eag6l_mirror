...
#define PRV_TGF_PHA_THREAD_ECN_COUNTING_CNS    80

/* ECN Counting QoS profile id */
#define PRV_TGF_ECN_COUNTING_QOS_PROFILE0      0
...
/* save pha FW image id */
static CPSS_DXCH_PHA_FW_IMAGE_ID_ENT        savePhaFwImageId;

/* save QoS profile */
static PRV_TGF_COS_PROFILE_STC              saveQosProfile;
...
GT_VOID prvTgfPolicerECNMarkingGenerateTraffic()
{
    ...
    /* Restore expected packet IP header */
    prvTgfExpectedPacketInfo.partsArray[4].partPtr = &prvTgfExpectedPacketIpv4Part;
...
)
{
    ...
    /* Load FW img 02 */
    rc = cpssDxChPhaFwImageIdGet(prvTgfDevNum, &savePhaFwImageId);
    ...
    commonInfo.statisticalProcessingFactor = statisticalProcessingFactor;
    ...
    /* AUTODOC: Assign ECN Counting fw thread to target port */
    rc = cpssDxChPhaPortThreadIdSet(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS],
                                    GT_TRUE, PRV_TGF_PHA_THREAD_ECN_COUNTING_CNS);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaPortThreadIdSet");

    /* AUTODOC: Config the CPSS_DXCH_PHA_THREAD_TYPE_ECN_COUNTING_E */
    rc = cpssDxChPhaThreadIdEntrySet(prvTgfDevNum,
                PRV_TGF_PHA_THREAD_ECN_COUNTING_CNS,
                &commonInfo,
                CPSS_DXCH_PHA_THREAD_TYPE_ECN_COUNTING_E,
                &extInfo);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in cpssDxChPhaThreadIdEntrySet for ECN Counting");
}
...
GT_VOID prvTgfPolicerECNCountingPhaConfigurationRestore()
{
    ...
    rc = cpssDxChPhaInit(prvTgfDevNum, GT_TRUE, savePhaFwImageId);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "[TGF]: cpssDxChPhaInit FAILED for phaFwImageId = [%d], rc = [%d]", savePhaFwImageId, rc);

    rc = cpssDxChPhaPortThreadIdSet(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS],
                                    GT_FALSE, PRV_TGF_PHA_THREAD_ECN_COUNTING_CNS);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "[TGF]: cpssDxChPhaPortThreadIdSet FAILED for ECN Counting, rc = [%d]", rc);
}
...
)
{
    ...
    rc = prvTgfCosProfileEntryGet(
        prvTgfDevNum, PRV_TGF_ECN_COUNTING_QOS_PROFILE0, &saveQosProfile);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfCosProfileEntryGet");

    /* Set packets DP to YEELOW, TC to queueNum */
    qosProfile.dropPrecedence = CPSS_DP_YELLOW_E;
    qosProfile.trafficClass   = queueNum;
    qosProfile.dscp           = 0;
    qosProfile.exp            = 0;
    qosProfile.userPriority   = 0;

    /* AUTODOC: config cos profile entry 0 with: */
    /* AUTODOC:   DP=YELLOW, UP=DSCP=EXP=0 TC=queueNum */
    rc = prvTgfCosProfileEntrySet(
        PRV_TGF_ECN_COUNTING_QOS_PROFILE0, &qosProfile);
...
)
{
    ...
    /* AUTODOC: Get ECN counter entry */
    rc = cpssDxChPhaSharedMemoryEcnCounterGet(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS], queueNum, &ecnCount);
    UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc, "[TGF]: cpssDxChPhaSharedMemoryEcnCounterGet FAILED for port = [%d], queue = [%d], rc = [%d]",
                                 prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS], queueNum, rc);

    /* AUTODOC: Verify count value is equal to 1 */
    /* When statistical processing factor is set to 32 or 64,
     * in the first run only significant count value is seen.
     * Except first run count value was always 0.
     * 128 is lab data provided by APPs team.
     */
    PRV_UTF_LOG1_MAC("PHA shared memory ECN count is [%d] \n", ecnCount.l[0]);
    isCountExpected = (ecnCount.l[0] > 0) ? GT_TRUE : GT_FALSE;
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_TRUE, isCountExpected, "expected ECN count mismatch");
}
...
)
{
    GT_U64       ecnCount;
    GT_STATUS    rc = GT_OK;
    GT_U32       expectedCount = 0;

    /* AUTODOC: Get ECN counter entry */
    rc = cpssDxChPhaSharedMemoryEcnCounterReset(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS], queueNum);
    UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc, "[TGF]: cpssDxChPhaSharedMemoryEcnCounterReset FAILED for port = [%d], queue = [%d], rc = [%d]",
                                 prvTgfPortsArray[PRV_TGF_PORT_IDX_2_CNS], queueNum, rc);
    ...
    /* AUTODOC: Verify count value is equal to 0 */
    UTF_VERIFY_EQUAL0_STRING_MAC(expectedCount, ecnCount.l[0], "[TGF]: ECN count mismatch");
    UTF_VERIFY_EQUAL0_STRING_MAC(expectedCount, ecnCount.l[1], "[TGF]: ECN count mismatch");
}
...
GT_VOID prvTgfPolicerECNCountingQosConfigReset()
{
    ...
    rc = prvTgfCosProfileEntrySet(
        PRV_TGF_ECN_COUNTING_QOS_PROFILE0, &saveQosProfile);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfCosProfileEntrySet");
}
