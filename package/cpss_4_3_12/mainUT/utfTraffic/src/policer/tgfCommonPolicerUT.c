...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - Explicit Congestion Notification (ECN) Counting:
 * Test case 1:
 *     1. Enable policer port metering on target port to enable ECN marking.
 *     2. Enable ECN extension in Ipv4 header.
 *     3. Config PHA for ECN Counting thread with statistical processing factor = 0.
 *     4. Config QoS to send traffic to all queues (one queue at a time)
 *     5. Send traffic.
 *     6. Get PHA shared ECN Counting for target port and all queues (one queue at a time) and
 *        verify counter to be incremented.
 *     7. Reset PHA shared ECN Counting for target port and all queues (one queue at a time) and
 *        verify counter to be 0.
 *     8. Restore Config.
 * Test case 2 (For BM only):
 *     1. Enable policer port metering on target port to enable ECN marking.
 *     2. Enable ECN extension in Ipv4 header.
 *     3. Config PHA for ECN Counting thread with statistical processing factor = 128.
 *     4. Config QoS to send traffic to  queue 4.
 *     5. Send traffic.
 *     6. Get PHA shared ECN Counting for target port and queue 4 and
 *        verify counter to be incremented.
 *     7. Reset PHA shared ECN Counting for target port and queue 4 and
 *        verify counter to be 0.
 *     8. Restore Config.
*/
UTF_TEST_CASE_MAC(prvTgfPolicerECNCounting)
{
#ifndef ASIC_SIMULATION
    GT_U32    spf = 0;
#endif
    GT_U32    queueNum = 0;
    GT_U32    statisticalProcessingFactor = 0;
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, ~UTF_FALCON_E) ;

    /* Test 1: Configure statisticalProcessingFactor = 0.
     * statisticalProcessingFactor = 0 indicates PHA will porecess every packet.
     */
    /* AUTODOC: Test configurations */
    prvTgfPolicerECNMarkingConfigurationSet();
    prvTgfPolicerECNCountingPhaConfigurationSet(statisticalProcessingFactor);

    /* AUTODOC: Test traffic and checks */
    for(queueNum = 0; queueNum < 8; queueNum++)
    {
        prvTgfPolicerECNCountingQosConfigSet(queueNum);
        prvTgfPolicerECNMarkingGenerateTraffic();
        prvTgfPolicerECNCountingPhaCounterGetAndCheck(queueNum);

        /* Clear ECN Counter for queueNum */
        prvTgfPolicerECNCountingPhaCounterReset(queueNum);
        /* Reset QoS config */
        prvTgfPolicerECNCountingQosConfigReset();
    }

    /* AUTODOC: Restore configurations */
    prvTgfPolicerECNCountingPhaConfigurationRestore();
    prvTgfPolicerECNMarkingRestore();

#ifndef ASIC_SIMULATION
    /* Test 2: Configure statisticalProcessingFactor = 128.
     * statisticalProcessingFactor = 128 indicates there is 1 in 128 chance of PHA will process packet.
     * Note: statistical Processing Factor supports fixed value 128.
     */
    queueNum = 4;
    statisticalProcessingFactor = 128;
    ...
    /* AUTODOC: Test traffic and checks */
    prvTgfPolicerECNCountingQosConfigSet(queueNum);
    for(spf = 0; spf < statisticalProcessingFactor; spf++)
    {
        prvTgfPolicerECNMarkingGenerateTraffic();
    }
    prvTgfPolicerECNCountingPhaCounterGetAndCheck(queueNum);
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfPolicer)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPolicerECNCounting)
