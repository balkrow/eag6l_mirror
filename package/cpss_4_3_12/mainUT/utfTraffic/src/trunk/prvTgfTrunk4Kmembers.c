...
/* array to set members : may copy from 'orig' and do modifications */
static CPSS_TRUNK_MEMBER_STC   resilient_membersArray_set_4K_tests[CPSS_TRUNK_4K_MAX_NUM_OF_MEMBERS_CNS];
#define DESIGNATD_TABLE_SIZE    64
...
/* indication to run the test in resilient mode */
static GT_BOOL  use_resilient_mode = GT_FALSE;
...
static GT_PHYSICAL_PORT_NUM findDesignatedPort
(
...
)
{
    ...
    for(ii = 0,portNum = 0; ii < CPSS_MAX_PORTS_BMP_NUM_CNS; ii++, portNum += 32)
    {
        overlap = (~nonTrunkPortsPtr->ports[ii]) & designatedPortsPtr->ports[ii];
        if(overlap == 0)
        {
            /* no overlap */
            continue;
        ...
        for(jj = 0 ; jj < 32; jj++)
        {
            ...
            /* match , should be single port ! */
            return portNum + jj;
        }
    }

    return GT_NA; /* no designated port on this device (maybe on other device there is)*/
}


GT_STATUS   tgfTrunkPrintDesignatedPortInTrunk(
    IN GT_U8            devNum,
    IN GT_TRUNK_ID      trunkId,
    IN GT_BOOL          isMcPart /*GT_FALSE = 'UC' , GT_TRUE = 'MC' */
)
{
    GT_STATUS             st;
    GT_U32                index,indexLast;
    GT_U32                ii;
    CPSS_PORTS_BMP_STC    designatedPorts;
    CPSS_PORTS_BMP_STC    nonTrunkPorts;
    static  GT_PHYSICAL_PORT_NUM  designated_ports_arr[DESIGNATD_TABLE_SIZE];

    st = cpssDxChTrunkNonTrunkPortsEntryGet(devNum,trunkId,&nonTrunkPorts);
    if(st != GT_OK)
    {
        return st;
    }

    indexLast = isMcPart ? (2*DESIGNATD_TABLE_SIZE) : DESIGNATD_TABLE_SIZE;
    index     = isMcPart ?     DESIGNATD_TABLE_SIZE : 0;

    /* index  0.. 63 are for the 'UC designated table' */
    /* index 64..127 are for the 'MC designated table' */
    for(/*index already init*/ii = 0; index < indexLast; index++,ii++)
    {
        st = cpssDxChTrunkDesignatedPortsEntryGet(devNum,index,&designatedPorts);
        if(st != GT_OK)
        {
            return st;
        }

        designated_ports_arr[ii] = findDesignatedPort(&nonTrunkPorts,&designatedPorts);
    }

    cpssOsPrintf("Print %s part of the designated table for trunk[%d] \n",
        (isMcPart?"MC":"UC"),trunkId);

    cpssOsPrintf("%s    |   %s  \n","index","portNum");
    for(ii = 0; ii < DESIGNATD_TABLE_SIZE; ii++)
    {
        if(designated_ports_arr[ii] == GT_NA)
        {
            /* no designated port on this device (maybe on other device there is)*/
            cpssOsPrintf("%3.3d    |   %s  \n",ii,"---");
        }
        else
        {
            cpssOsPrintf("%3.3d    |   %3.3d  \n",ii,designated_ports_arr[ii]);
        }
    }
...
)
{
    ...
    ecmpStartIndex = l2EcmpNumMembers - maxNumOfMembers;
    ...
    st = cpssDxChTrunkFlexInfoSet(dev,trunkId_for_4K_members,ecmpStartIndex,maxNumOfMembers);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
...
membersArray_ready_lbl:
        ...
        for(ii = 0 ; ii < 2 ; ii++)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    st = cpssDxChTrunkMemberDisabledAdd(dev, trunkId_for_4K_members, &tempMember);
                    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberDisabledAdd: %d", prvTgfDevNum);
                    /* then enable the disabled member */
                    st = cpssDxChTrunkMemberEnable(dev, trunkId_for_4K_members, &tempMember);
                    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberEnable: %d", prvTgfDevNum);
                }
            }
        }
    }
    ...
    if(use_resilient_mode)
    {
        ...
        /* we need to know the actual HW order of members , and it is
            NOT like the 'DB' that returned by cpssDxChTrunkDbEnabledMembersGet */
        st = cpssDxChTrunkTableEntryGet(dev,trunkId_for_4K_members,&numMembers,
            &resilient_membersArray_set_4K_tests[0]);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChTrunkDbEnabledMembersGet: %d", prvTgfDevNum);
    }
...
)
{
        ...
        targetHwDevNum++)
    {
        ...
        */
        ...
        rc = cpssDxChCscdDevMapTableSet(
            dev, targetHwDevNum, 0/*sourceHwDevNum -- dont care*/,
            0/* targetPortNum -- dont care */, 0/* hash -- dont care */, &cascadeLink,
            CPSS_DXCH_CSCD_TRUNK_LINK_HASH_IS_INGR_PIPE_HASH_E, GT_FALSE);
...
static GT_U32   findIndexOfResilientMember(IN CPSS_TRUNK_MEMBER_STC *memberPtr)
{
    GT_U32 ii;
    for(ii = 0 ; ii < actualNumOfMembersInTrunk ; ii++)
    {
        if(resilient_membersArray_set_4K_tests[ii].hwDevice == memberPtr->hwDevice &&
           resilient_membersArray_set_4K_tests[ii].port     == memberPtr->port)
        {
            return ii;
        }
    }
...
)
{
    ...
    GT_U32      alreadyHaveValue;

    /* provide time for simulation */
    ...
    for(ii = 0 ; ii < maxTrunkMembers ; ii++)
    {
        ...
        if(use_resilient_mode && floodTesting == GT_FALSE)
        {
            index = findIndexOfResilientMember(&membersArray_set_4K_tests[ii]);
            index = CONVERT_TRUNK_MEMBER_TO_CNC_INDEX_MAC(index);
        }
        ...
        if(floodTesting == GT_FALSE)
        {
            ...
            if(expectedCounter)
            {
                ...
                while(counter.packetCount.l[0] == 0 && timeout)
                {
                    ...
                    {
                        PRV_UTF_LOG1_MAC("waiting for CNC counters to see the traffic - iteration [%d] \n",
                            10-timeout);

                        /* wait till the WM get it's packets counted */
                        cpssOsTimerWkAfter(100);
                        timeout--;
                    }
                }
                if(timeout == 0)
                {
                    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_TIMEOUT,
                        "waited too long for CNC counters to 'see' the traffic at index[%d]",
                        index);
                }
                alreadyHaveValue = 1;
            }
...
/**
* @internal tgfTrunkCrcHash_4K_members_known_UC_EPCL_resilient_mode function
* @endinternal
*
* @brief   running tgfTrunkCrcHash_4K_members_known_UC_EPCL in resilient mode.
*/
void tgfTrunkCrcHash_4K_members_known_UC_EPCL_resilient_mode
(
    void
)
{
    PRV_UTF_LOG0_MAC("======= Start test in resilient mode =======\n");
    use_resilient_mode = GT_TRUE;

    tgfTrunkCrcHash_4K_members_known_UC_EPCL();

    use_resilient_mode = GT_FALSE;
    PRV_UTF_LOG0_MAC("======= Ended running in resilient mode =======\n");
}

/**
* @internal tgfTrunk_80_members_flood_EPCL_resilient_mode function
* @endinternal
*
* @brief   run tgfTrunk_80_members_flood_EPCL in resilient mode
*/
void tgfTrunk_80_members_flood_EPCL_resilient_mode
(
    void
)
{
