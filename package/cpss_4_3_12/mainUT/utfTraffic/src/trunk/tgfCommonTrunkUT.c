...
static void prvTgfTrunkReInitTrunkLib(IN GT_U8 devNum)
{
    GT_STATUS   rc;
    GT_U32    orig_maxNumberOfTrunks;
    CPSS_DXCH_TRUNK_MEMBERS_MODE_ENT orig_trunkMembersMode;

    rc = cpssDxChTrunkDbInitInfoGet(devNum,&orig_maxNumberOfTrunks,&orig_trunkMembersMode);
    ...
    rc = cpssDxChTrunkDestroy(devNum);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, rc, devNum);
    /* re-init the lib */
    rc = cpssDxChTrunkInit(devNum , orig_maxNumberOfTrunks,orig_trunkMembersMode);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, rc, devNum);
...
#define REDO_TEST_WITH_SORTING_MAC                                                          \
    ...
    /* re-init the trunk from 'sort-mode'*/                                                 \
    prvTgfTrunkReInitTrunkLib(prvTgfDevNum)
...
static GT_VOID internal_tgfTrunkSimpleHashMacSaDa(GT_VOID)
{
    /* must be first line */
    DECLARE_START_OF_TEST_MAC;
...
UTF_TEST_CASE_MAC(tgfTrunkSimpleHashMacSaDa)
{
    ...
    /* test frees and allocates all trunk DB memories.
       algorithm of osMemPool does not guaranty that sequence free->allocate
       (for same size and > 64K) allocates chunk in pool with exactly same size that was free.
       e.g. Trunk Array consumes 221184 bytes those allocated during init.
       Trunk test free this chunk and ask to allocate 221184 again. But memory
       pools manager provides bigger chunk with size 221192. And test detects memory leakage.
       This specific test may detect 8 bytes memory leakage because of mentioned above.
       Tolerance restored to 0 automatically after this test. */
    utfMemoryHeapLeakageToleranceSet(8);

    /* call test's logic */
    internal_tgfTrunkSimpleHashMacSaDa();
}
...
/*----------------------------------------------------------------------------*/
/*
    tgfTrunkAddDisabledMember
    the test will start with 2 ports in the trunk , and will add third port as disabled.
    NOTE: the first 2 ports are in the vlan , but the third port is not.
    the test will check that there is no packet loss when doing LBH as flood ,
    and no packet loss when doing LBH as known UC.
    meaning that the third port that added as disabled should not be part of the LBH (known UC/flood)
*/
UTF_TEST_CASE_MAC(tgfTrunkAddDisabledMember)
{
    tgfTrunkAddDisabledMember();
}
...
UTF_TEST_CASE_MAC(tgfTrunk_80_members_flood_EPCL)
{
    ...
    PRV_TGF_SKIP_LONG_TEST_MAC(UTF_CPSS_PP_ALL_SIP6_CNS);
...
UTF_TEST_CASE_MAC(tgfTrunkCrcHash_4K_members_known_UC_EPCL_resilient_mode)
{
    GT_U32 notAppFamilyBmp;
...
UTF_TEST_CASE_MAC(tgfTrunk_80_members_flood_EPCL_resilient_mode)
{
    GT_U32 notAppFamilyBmp;
...
/* AUTODOC: Test Hash Value Extraction from Packet Header and Load balancing
*  1.        Initialize the trunk for test type TRUNK_TEST_TYPE_SIMPLE_HASH_E
*  2.        Set the general hashing mode to be based on the packets data.
*  3.        Set the packet type hash mode to extract from hash Key.
*  4.        Test for different trafficTypes.
*  5.        Check for load balancing based to hash value.
*/
UTF_TEST_CASE_MAC(tgfTrunkPacketTypeXorHashModeTest)
{
...
{   /* skip multi device boards for cascading related tests */
    ...
    prvTgfTrunkCrcHash_cpssPredictEgressPortEnableSet(GT_TRUE);
    prfTgfTrunkPacketTypeXorHashModeBasicConfig();
    prfTgfTrunkPacketTypeXorHashModeSendTrafficAndCheck();
    prfTgfTrunkPacketTypeXorHashModeRestore();
}
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - tgfTrunkPrngSeedTest:
*  To test PRNG seed test"
*  Configure Trunk with 2 ports
*  Send traffic with random mode disabled and expect all packets egres through same port
*  Configure PRNG seed for the trunk
*  Send traffic with random mode enabled and expect packets to egress though both the ports
*/
UTF_TEST_CASE_MAC(tgfTrunkPrngSeedTest)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E);
    ...
    /* Set configuration */
    prvTgfHLTrunkPrngSeedTestConfig();

    /* Generate traffic */
    prvTgfHLTrunkPrngSeedTestConfigTrafficGenerate();

    /* Restore configuration */
    prvTgfHLTrunkPrngSeedTestConfigRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Hash Key with 10 MPLS Mode Test trunk hash mask:
    configure TRUNK with 2 ports;
    configure general trunk hashing mode;
    mask\unmask IP and MPLS fields for trunk hash calc and
    send MPLS traffic and verify expected traffic on trunk ports.
*/
UTF_TEST_CASE_MAC(tgfTrunk10MplsLabelHashMaskSet)
{
    ...
    /* Generate traffic */
    prvTgfTrunkHashTestTrafficMaskSet(GT_TRUE);
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfTrunk)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunkAddDisabledMember)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunkCrcHash_4K_members_known_UC_EPCL_resilient_mode)
    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunk_80_members_flood_EPCL_resilient_mode)

    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunkIpclLoadBalancingHash)

    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunkPacketTypeXorHashModeTest)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunkPrngSeedTest)
    UTF_SUIT_DECLARE_TEST_MAC(tgfTrunk10MplsLabelHashMaskSet)
