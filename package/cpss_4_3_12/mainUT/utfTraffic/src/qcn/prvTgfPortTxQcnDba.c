...
)
{
    ...
    if(configSet)
    {
        ...
        rc = prvTgfPortCnModeEnableSet(prvTgfDevNum, CPSS_DXCH_PORT_CN_MODE_ENABLE_E);
...
)
{
    ...
    static GT_U32                                              poolBuffersRestore[2];
    GT_U32                                                     temp;
    GT_U32                                                     trigIdx;
    ...
    if(configSet)
    {
        ...
        else
        {
           ...
           if (poolNum == 1)
           {
               ...
               rc = prvTgfPortTxSharedPoolLimitsGet(prvTgfDevNum, poolNum, &poolBuffersRestore[1], &temp);
               UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxSharedPoolLimitsGet");

                  /* AUTODOC: Save number of pool#1 available buffers */
               rc = prvTgfPortTxSharedPoolLimitsGet(prvTgfDevNum, 0, &poolBuffersRestore[0], &temp);
               UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxSharedPoolLimitsGet");

               /* AUTODOC: Set number of pool #1 available buffers */
               rc = prvTgfPortTxSharedPoolLimitsSet(prvTgfDevNum, 0, poolBuffersRestore[0]-(2*globalBuffForDba), temp);
    ...
    else
    {
        ...
        else
        {
           ...
           if (poolNum == 1)
           {
               ...
               rc = prvTgfPortTxSharedPoolLimitsSet(prvTgfDevNum, poolNum, poolBuffersRestore[1], poolBuffersRestore[1] /* not used */);
               UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxSharedPoolLimitsSet");

               /* AUTODOC: restore number of pool #1 available buffers */
               rc = prvTgfPortTxSharedPoolLimitsSet(prvTgfDevNum, 0, poolBuffersRestore[0], poolBuffersRestore[0] /* not used */);
               UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPortTxSharedPoolLimitsSet");
           }
        }
