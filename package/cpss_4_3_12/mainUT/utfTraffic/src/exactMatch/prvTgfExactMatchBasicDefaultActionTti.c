...
)
{
    ...
    action.ttiAction.command = CPSS_PACKET_CMD_DROP_HARD_E ;
    ...
    rc = prvTgfExactMatchProfileDefaultActionSet(prvTgfDevNum, PRV_TGF_EXACT_MATCH_UNIT_0_E,
                                                 PRV_TGF_EXACT_MATCH_PROFILE_ID_CNS, actionType, &action, enableDefaultAction);
...
* @param[in] expectCpuCode        - Expect Cpu Code in case expectTrapTraffic is GT_TRUE
*
*/
    ...
    else
    {
        ...
        if (GT_TRUE == prvTgfActionTrapToCpuCheck[PRV_TGF_CPU_PORT_IDX_CNS])
        {
            ...
            if(expectTrapTraffic==GT_FALSE)
            {
                ...
                UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(expectCpuCode,
                                                rxParams.cpuCode);
