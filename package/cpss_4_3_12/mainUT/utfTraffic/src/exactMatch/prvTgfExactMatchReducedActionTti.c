...
*
*/
static GT_VOID prvTgfExactMatchReducedActionTtiGenericConfigInit
(
...
)
{
    ...
    /* init configuration */
    prvTgfExactMatchReducedActionTtiGenericConfigInit(prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS],GT_TRUE, 0);
}
...
)
{
    ...
    keyParams.keySize           = (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))?
    ...
    exactMatchEntry.key.keySize = (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))?
                                   PRV_TGF_EXACT_MATCH_KEY_SIZE_37B_E:
                                   PRV_TGF_EXACT_MATCH_KEY_SIZE_33B_E;
    ...
    if (flowIdMaskEnable==GT_TRUE)
    {
        ...
        /* flowId attribute in TTI action is located in bits [29:17]
            ...
            0 - use all the bits from the reduced entry (will occupy 2 bytes in the reduced entry)
            1-0x7F - use 7 lsb from the reduced entry (will occupy 1 byte in the reduced entry)
            0x80-0x1F80 - use 6 msb from the reduced entry (will occupy 1 byte in the reduced entry)
            0x2000-0xFFFFE000- Same as value 0*/
        expandedActionOrigin.ttiExpandedActionOrigin.flowIdMask = 1; /* first byte in the reduced, second in the expander */
        actionData.ttiAction.flowId = 0xFF; /* bit 7 is up and should be taken from the expander the rest will be take from the reduced */
    }
    ...
    if (flowIdMaskEnable==GT_TRUE)
    {
        /* bit 0-6 should be taken from the reduced the rest will be take from the expander
           the final flowId we expect to see in the packet is 0x8F :
           bits 0-6 from the reduced (0xF) bits 7-12 from the expander (0x80)*/
        actionData.ttiAction.flowId = 0x1F8F;
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        rc =  prvTgfExactMatchProfileMuxTableLineSet(prvTgfDevNum, PRV_TGF_EXACT_MATCH_UNIT_0_E, profileId, exactMatchEntry.key.keySize);
...
)
{
    ...
    zeroAction.ttiAction.command = CPSS_PACKET_CMD_FORWARD_E ;
    /*same as set on init*/
    actionData.ttiAction.command  = CPSS_PACKET_CMD_TRAP_TO_CPU_E ;
    ...
    if (flowIdMaskEnable==GT_TRUE)
    {
        ...
        /* flowId attribute in TTI action is located in bits [29:17]
            ...
            0x2000-0xFFFFE000- Same as value 0*/
        expandedActionOrigin.ttiExpandedActionOrigin.flowIdMask = 1; /* first byte in the reduced, second in the expander */
        zeroAction.ttiAction.flowId = 0xFF; /* bit 7 is up and should be taken from the expander the rest will be take from the reduced */
    }

    rc = prvTgfExactMatchExpandedActionSet(prvTgfDevNum, PRV_TGF_EXACT_MATCH_UNIT_0_E, expandedActionIndex,
    ...
    else
    {
        ...
        if (GT_TRUE == prvTgfActionTrapToCpuCheck[PRV_TGF_CPU_PORT_IDX_CNS])
        {
            ...
            else
            {
                 ...
                 if (flowIdMaskEnable==GT_TRUE)
                 {
                    UTF_VERIFY_EQUAL0_PARAM_MAC(0x8F, rxParams.flowIdTtOffset.flowId);
                    PRV_UTF_LOG1_MAC("Packet is Trapped to  CPU with flowId  :%d\n\n",rxParams.flowIdTtOffset.flowId);
                }
            }
        }
    }
