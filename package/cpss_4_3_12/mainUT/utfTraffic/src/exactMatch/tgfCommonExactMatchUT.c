...
UTF_TEST_CASE_MAC(tgfExactMatchBasicDefaultActionTti)
{
    ...
    */
    ...
    prvTgfExactMatchBasicDefaultActionTtiTrafficGenerate(GT_FALSE,GT_TRUE,PRV_TGF_EXACT_MATCH_CPU_CODE_CNS);

    /* disable TTI rule */
    prvTgfExactMatchBasicDefaultActionTtiRuleValidStatusSet(GT_FALSE);

    /* Generate traffic -- check no traffic and no cpuCode */
    prvTgfExactMatchBasicDefaultActionTtiTrafficGenerate(GT_FALSE,GT_FALSE,PRV_TGF_EXACT_MATCH_CPU_CODE_CNS);
    ...
    prvTgfExactMatchBasicDefaultActionTtiTrafficGenerate(GT_TRUE,GT_FALSE,PRV_TGF_EXACT_MATCH_CPU_CODE_CNS);
...
UTF_TEST_CASE_MAC(tgfExactMatchAutoLearnTti)
{
    ...
    */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP6_10_CNS | UTF_CPSS_PP_ALL_SIP7_CNS))
...
/*----------------------------------------------------------------------------*/
/*
   tgfSerialExactMatchBasicDefaultActionTti :
   Test functionality of Serial Exact Match default action
*/
UTF_TEST_CASE_MAC(tgfSerialExactMatchBasicDefaultActionTti)
{
   /*
      1. Set TTI configuration
      2. Set Exact Match Default Action for Serial EM1 - Forward
      3. Generate traffic - Expect Default Action in Serial EM1 - traffic forwarded
      4. Change the Serial EM1 default action to Trap to CPU with code 502
      5. Generate traffic - Expect Default Action in Serial EM1 - traffic trapped to CPU
      6. Restore TTI configuration
      7. Restore Exact Match configuration
    */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
    ...
    /* Set TTI Generic configuration not related to Exact Match */
    prvTgfSerialExactMatchTtiGenericConfigSet();

    /* Set Serial Exact Match Default Action TTI basic configuration */
    prvTgfSerialExactMatchBasicDefaultActionTtiConfigSet();

    /* Generate traffic - expect to be forwarded */
    prvTgfSerialExactMatchTrafficGenerate(PRV_TGF_SINGLE_TAGGED_E, GT_FALSE, GT_TRUE);

    /* change Default Action for Trap */
    prvTgfSerialExactMatchBasicDefaultActionTtiCommandTrap();

    /* Generate traffic -- check no traffic and cpuCode 502*/
    prvTgfSerialExactMatchTrafficGenerate(PRV_TGF_SINGLE_TAGGED_E, GT_TRUE, GT_FALSE);

    /* Restore TTI generic configuration */
    prvTgfSerialExactMatchTtiGenericConfigRestore();

    /* Restore Exact match Default Action TTI Configuration */
    prvTgfSerialExactMatchBasicDefaultActionTtiConfigRestore();
}

/*----------------------------------------------------------------------------*/
/*
   tgfSerialExactMatchBasicExpandedActionTti
   Test functionality of Serial Exact Match Expanded Action ,
*/
UTF_TEST_CASE_MAC(tgfSerialExactMatchBasicExpandedActionTti)
{
   /*
      1. Set TTI configuration
      2. Set Exact Match Expanded Action for Serial EM1 - Forward
      3. Generate traffic - expect match in Serial EM1 - traffic forwarded
      4. Invalidate Exact Match Entry
      5. Generate traffic - expect no hit in Serial EM1 and no Default Action - traffic forwarded
      6. Restore TTI configuration
      7. Restore Exact Match configuration
    */
    ...
    /* Set Exact Match Expanded Action TTI configuration */
    prvTgfSerialExactMatchBasicExpandedActionTtiConfigSet();

    /* Generate traffic - expect to be forwarded */
    prvTgfSerialExactMatchTrafficGenerate(PRV_TGF_SINGLE_TAGGED_E, GT_FALSE, GT_TRUE);

    /* invalidate Exact Match Entry */
    prvTgfSerialExactMatchInvalidateEmEntry();
    ...
    /* Restore Exact Match Expanded Action TTI configuration */
    prvTgfSerialExactMatchBasicExpandedActionTtiConfigRestore();
}

/*----------------------------------------------------------------------------*/
/*
   tgfSerialExactMatchUntaggedPacket
   Test functionality of Serial EM0 with Untagged packet
*/
UTF_TEST_CASE_MAC(tgfSerialExactMatchUntaggedPacket)
{
   /*
      1. Set TTI configuration
      2. Set Exact Match Expanded Action for Serial EM1 - Forward
    ...
    */
    ...
    prvTgfSerialExactMatchKeyAndActionConfigSet();

    /* Generate traffic - expect to be forwarded */
    prvTgfSerialExactMatchTrafficGenerate(PRV_TGF_UNTAGGED_E, GT_FALSE, GT_TRUE);

    /* Restore TTI generic configuration */
    prvTgfSerialExactMatchTtiGenericConfigRestore();

    /* Restore Serial EM configuration */
    prvTgfSerialExactMatchKeyAndActionConfigRestore();
}

/*----------------------------------------------------------------------------*/
/*
   tgfSerialExactMatchSingleTaggedPacket
   Test functionality of Serial EM0,EM1 & EM2 with
   Single Tagged packet
*/
UTF_TEST_CASE_MAC(tgfSerialExactMatchSingleTaggedPacket)
{
   /*
      1. Set TTI configuration
      2. Set Exact Match Expanded Action for Serial EM1 - Forward
...
/*----------------------------------------------------------------------------*/
/*
   tgfSerialExactMatchDoubleTaggedPacket
   Test functionality of Serial EM0,EM1 & EM2 with
   Double Tagged packet
*/
UTF_TEST_CASE_MAC(tgfSerialExactMatchDoubleTaggedPacket)
{
   /*
      1. Set TTI configuration
      2. Set Exact Match Expanded Action for Serial EM1 - Forward
    ...
    */
    ...
    /* Set Serial EM configuration */
    prvTgfSerialExactMatchKeyAndActionConfigSet();

    /* Generate traffic - expect to be forwarded */
    prvTgfSerialExactMatchTrafficGenerate(PRV_TGF_DOUBLE_TAGGED_E, GT_FALSE, GT_TRUE);
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfExactMatch)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfSerialExactMatchBasicDefaultActionTti)
    UTF_SUIT_DECLARE_TEST_MAC(tgfSerialExactMatchBasicExpandedActionTti)
    UTF_SUIT_DECLARE_TEST_MAC(tgfSerialExactMatchUntaggedPacket)
    UTF_SUIT_DECLARE_TEST_MAC(tgfSerialExactMatchSingleTaggedPacket)
    UTF_SUIT_DECLARE_TEST_MAC(tgfSerialExactMatchDoubleTaggedPacket)
