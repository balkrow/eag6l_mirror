...
static CPSS_DXCH_VIRTUAL_TCAM_MNG_CONFIG_PARAM_STC haAppVtcamMngCfgParam = {GT_TRUE, CPSS_DXCH_VIRTUAL_TCAM_HA_FEATURE_REPLAY_FAILED_RULES_ADD_E};
...
(
    ...
    IN  GT_U8                           vtcamMode,
    IN  GT_U32                          priority,
    IN  CPSS_DXCH_VIRTUAL_TCAM_MNG_CONFIG_PARAM_STC *haAppVtcamMngCfgParamPtr
);
...
)
{
    ...
    for (i = 0; (i < udbAmount); i++)
    {
        ...
        {
            if (i <= 1) udbSelect.egrUdb01Replace      = GT_TRUE;
            if (i == 1) udbSelect.egrUdbBit15Replace   = GT_TRUE;
            if (i < 19) udbSelect.ingrUdbReplaceArr[i] = GT_TRUE;
        }
        udbSelect.udbSelectArr[i] = udbIndexBase + (udbIndexIncrement * i);
    }
...
)
{
    ...
    /* this test uses only replaced fields                                  */
    /* Structure filled by udb0-69 to match lookups having no select tables */
    rc = prvTgfPclUdb10KeyCustomerUdbOnlyRangeSelectSet(
        ...
        70/*udbAmount*/,
        0 /*udbIndexBase*/, /* was 30, but this test uses only replaced fields */
        1  /*udbIndexIncrement*/);
...
)
{
    ...
    while(GT_OK == prvUtfNextMacPortGet(&portNum, GT_TRUE))
    {
        ...
        {
            CPSS_PCL_LOOKUP_NUMBER_ENT lookup1 = CPSS_PCL_LOOKUP_1_E;
            if (PRV_CPSS_DXCH_PP_MAC(prvTgfDevNum)->hwInfo.pcl.iPcl0Bypass)
            {
                lookup1 = CPSS_PCL_LOOKUP_0_1_E;
            }
            vTcamId = 11;
...
)
{
    ...
    for (ii = 0; ii < cncBlocks; ii++)
    {
        ...
        {
            /* limit Falcon and Ironman only by two PCL lookups */
...
)
{
    ...
    /* (for WM) wait for processing of all packets after the WS breaks. */
...
)
{
    ...
    /* fill to be correct for lookups having no UDB select table */
    cpssOsMemSet(&udbSelect, 0, sizeof(udbSelect));
    for (ii = 0; (ii < 70); ii++)
    {
    ...
    for (ii = 0; ii < cncBlocks; ii++)
    {
        ...
        for (portNum = 0; portNum < CPSS_MAX_PORTS_NUM_CNS; portNum++)
        {
            ...
            if (check)
            {
                ...
                {
                    PRV_UTF_LOG4_MAC(
                        "prvTgfFWSPclCncCheck failure block %d port %d cnc %d port %d\n",
                        ii, portNum, cncCounter.packetCount.l[0], portCntrArr[portNum]);
                    rc1 = GT_FAIL;
                }
            }
...
)
{
    ...
    haAppVtcamMngCfgParamPtr = &haAppVtcamMngCfgParam;
...
)
{
    ...
    rc = ruleWriteHa2 (1, 1, 5, 6, (0x01020300 | 1), (GT_U16)0x0C06, 1);
    ...
    rc = ruleWriteHa2 (0, 2, 1, 5, (0x01020300 | 2), (GT_U16)0x0C06, 2);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Rule write (vtcam=2, rule-id=0) (config) failed.\n");
...
)
{
    ...
    /* AUTODOC: set MAC_MODE_DA for TTI_KEY_ETH */
    rc = cpssDxChTtiMacModeSet(prvTgfDevNum, CPSS_DXCH_TTI_KEY_ETH_E, CPSS_DXCH_TTI_MAC_MODE_DA_E);
