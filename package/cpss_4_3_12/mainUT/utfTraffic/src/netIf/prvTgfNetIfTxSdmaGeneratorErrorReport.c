...
/* SIP7/GDMA has timer clock 12.5MHz vs SDMA 25MHz with same 30bit value, and therefore use 160s vs 80s
 * Big cycle configuration, based on at least 2 packets in chain 160s = 160,000,000,000ns = 0x25 40BE 4000
 */
#define PRV_TGF_TX_GDMA_GENERATOR_QUEUE_BIG_CYCLE_NS_HIGH_WORD_CNS 0x25
#define PRV_TGF_TX_GDMA_GENERATOR_QUEUE_BIG_CYCLE_NS_LOW_WORD_CNS  0x40BE4000
...
)
{
    ...
    packetSize = PRV_TGF_GEN_BUFFER_SIZE_CNS + 1 - TGF_eDSA_TAG_SIZE_CNS;

    /* Starting from Ironman packet are sent by TX SDMA without CRC.
       For Ironman GM the packet sent with CRC as for older devices.   */
    if ((PRV_CPSS_PP_MAC(prvTgfDevNum)->isGmDevice == 0) &&
         PRV_CPSS_SIP_6_30_CHECK_MAC(prvTgfDevNum))
    {
        packetSize += 4;
    }
...
)
{
    ...
    if (PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(prvTgfDevNum))
    {
        rateValue.l[0] = PRV_TGF_TX_GDMA_GENERATOR_QUEUE_BIG_CYCLE_NS_LOW_WORD_CNS;
        rateValue.l[1] = PRV_TGF_TX_GDMA_GENERATOR_QUEUE_BIG_CYCLE_NS_HIGH_WORD_CNS;
    }
    else
    {
