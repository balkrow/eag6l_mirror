...
#include <netIf/prvTgfNetIfLinuxKernelEthernetDriver.h>
...
UTF_TEST_CASE_MAC(prvTgfNetIfTxSdmaGeneratorLinkList)
{
    ...
    /* Test is time sensitive. Disable PM task to avoid failures. */
    prvUtfPmTaskEnableSet(GT_FALSE);

    prvTgfNetIfTxSdmaGeneratorLinkListTest();

    /* Enable PM task back. */
    prvUtfPmTaskEnableSet(GT_TRUE);
}
...
UTF_TEST_CASE_MAC(prvTgfNetIfRxToTxSdmaPerformance)
{
    ...
    /* IM-M board use mixture of 8 Gig and 4 10G ports
       test choose two types and cannot work */
    if(prvUtfIsIronMan_M_System())
    {
        SKIP_TEST_MAC;
    }
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Linux Kernel SDMA performance test:
   1. Configure linux kernel interface on port #0,
   - Configure port #0 for loopback mode
   - Configure mirror to CPU port of port #0
   - Transmit 1000 DHCP broadcast packets using busybox udhcpc
   - Check that we received at least 1000 packets.
*/
UTF_TEST_CASE_MAC(prvTgfNetIfLinuxKernelSdma)
{
    /* only supported on Aldrin2, Falcon, AC5P and AC5X: */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_PIPE_E | UTF_BOBCAT2_E | UTF_BOBCAT3_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_AC3X_E | UTF_IRONMAN_L_E | UTF_HARRIER_E | UTF_AAS_E);

    /* Simulation does not support the test */
    ASIC_SIMULATION_NOT_SUPPORT_THIS_TEST_MAC;
#ifdef CPU_BE
    /* Kernel driver is currently broken for big endian until CPSS-19149 is resolved */
    SKIP_TEST_MAC;
#endif
    ...
    prvTgfNetIfRxToTxLinuxKernelEthernetDriverTestInit();
    prvTgfNetIfRxToTxLinuxKernelEthernetDriverTestRun();
    prvTgfNetIfRxToTxLinuxKernelEthernetDriverTestRestore();
#endif
}
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfNetIf)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfNetIfLinuxKernelSdma)
