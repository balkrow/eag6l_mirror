...
/**
* @internal prvTgfPclUdbOnlyKeysUdbOnlyGenericTestNotSip7 function
* @endinternal
*
* @brief   Generic Test on UDB Only Key for Not Sip7 devices
*
* @param[in] direction                - PCL direction
...
*                                       None
*/
static GT_VOID prvTgfPclUdbOnlyKeysUdbOnlyGenericTestNotSip7
(
    IN CPSS_PCL_DIRECTION_ENT           direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT       lookupNum,
    IN PRV_TGF_PCL_PACKET_TYPE_ENT      packetType,
...
static GT_STATUS prvTgfPclUdbAnchorCnv
(
    IN  PRV_TGF_PCL_OFFSET_TYPE_ENT srcType,
    OUT CPSS_DXCH_PCL_OFFSET_TYPE_ENT *dstTypePtr
)
{
    PRV_TGF_S2D_PCL_UDB_OFFSET_TYPE_CONVERT_MAC(*dstTypePtr, srcType);
    return GT_OK;
}

/**
* @internal prvTgfPclUdbOnlyKeysUdbOnlyGenericTestSip7 function
* @endinternal
*
* @brief   Generic Test on UDB Only Key for Sip7 devices
*
* @param[in] direction                - PCL direction
* @param[in] lookupNum                - lookup Number
* @param[in] ruleSize                 - rule Size
...
*                                       None
*/
static GT_VOID prvTgfPclUdbOnlyKeysUdbOnlyGenericTestSip7
(
    IN CPSS_PCL_DIRECTION_ENT           direction,
    IN CPSS_PCL_LOOKUP_NUMBER_ENT       lookupNum,
    IN CPSS_PCL_RULE_SIZE_ENT           ruleSize,
...
)
{
    ...
    TGF_AUTO_CHECKSUM_FULL_INFO_STC  checksumInfoArr[TGF_AUTO_CHECKSUM_NUM_OF_HEADERS_CNS];/* auto checksum info */
    TGF_AUTO_CHECKSUM_EXTRA_INFO_STC extraChecksumInfoArr[TGF_AUTO_CHECKSUM_NUM_OF_HEADERS_CNS];/* extra checksum info */
    TGF_TCAM_CLIENTS_PCL_RULE_FIELD_ENT      fieldIdBase;
    GT_U32                                   fieldsAmount;
    TGF_TCAM_CLIENTS_PCL_RULE_FIELD_INFO_STC fieldInfo;
    /* max - 60 UDBs - 15 fields * 4 UDBs + end mark - 16 records */
    static TGF_TCAM_CLIENTS_PCL_RULE_SUB_FIELD_STC pclSubFieldArr[16];
    static TGF_TCAM_CLIENTS_PCL_SUB_FIELD_CONDITION_STC    pclSubFieldConditionArr[16];
    static TGF_TCAM_CLIENTS_PCL_TCAM_PROFILE_SUBKEY_COMMON_HEADER_STC emptyCommonSubkeyHeader;
    static TGF_TCAM_CLIENTS_PCL_TCAM_MUXING_TABLE_SUBFIELDS_CFG_STC appMuxingTableSubfieldsCfgArr[2];
    ...
    /* register fields covering UDBs */
    fieldIdBase  = TGF_TCAM_CLIENTS_PCL_RULE_FIELD_APP_REGISTRED_BASE_E;
    fieldsAmount = (udbAmount + 3) / 4; /* 4 UDBs per field */
    fieldInfo.fieldType = TGF_TCAM_CLIENTS_PCL_RULE_FIELD_TYPE_UDB_CFG_E;
    rc = prvTgfPclUdbAnchorCnv(offsetType, &(fieldInfo.udbAnchor));
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPclUdbAnchorCnv");
    for (i = 0; (i < fieldsAmount); i++)
    {
        fieldInfo.bitOffset = (8 * (udbOffsetLow + (i * 4)));
        fieldInfo.bitLength =
            ((i + 1) < fieldsAmount) ? 32 : ((udbAmount * 8) - ((fieldsAmount - 1) * 32));
        rc = tgfTcamClientsPclFieldRegister((fieldIdBase + i), &fieldInfo);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "tgfTcamClientsPclFieldRegister");
    }
    /* build field list for superkey */
    for (i = 0; (i < fieldsAmount); i++)
    {
        pclSubFieldArr[i].fieldId = (fieldIdBase + i);
        pclSubFieldArr[i].bitOffset = 0;
        pclSubFieldArr[i].bitLength =
            ((i + 1) < fieldsAmount) ? 32 : ((udbAmount * 8) - ((fieldsAmount - 1) * 32));
    }
    pclSubFieldArr[fieldsAmount].fieldId =
        TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E;
    /* build field list for PCL rule condition */
    for (i = 0; (i < fieldsAmount); i++)
    {
        pclSubFieldConditionArr[i].subField.fieldId = (fieldIdBase + i);
        pclSubFieldConditionArr[i].subField.bitOffset = 0;
        pclSubFieldConditionArr[i].subField.bitLength =
            ((i + 1) < fieldsAmount) ? 32 : ((udbAmount * 8) - ((fieldsAmount - 1) * 32));
        pclSubFieldConditionArr[i].mask =
            (pclSubFieldConditionArr[i].subField.bitLength >= 32)
                ? 0xFFFFFFFF : (GT_U32)((1 << pclSubFieldConditionArr[i].subField.bitLength) - 1);
        w = 0;
        j2 = (pclSubFieldConditionArr[i].subField.bitLength / 8);
        for (j1 = 0; (j1 < j2); j1++)
        {
            w |= ((GT_U32)(patternPtr[(i * 4) + j1]) << ((j2 - j1 - 1) * 8));
        }
        pclSubFieldConditionArr[i].pattern = w;
    }
    pclSubFieldConditionArr[fieldsAmount].subField.fieldId =
        TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E;
    ...
    cpssOsMemSet(&emptyCommonSubkeyHeader, 0, sizeof(emptyCommonSubkeyHeader));
    appCfg.subkeyCommonHeaderPtr = &emptyCommonSubkeyHeader;
    cpssOsMemCpy(
        &appMuxingTableSubfieldsCfgArr,
        appCfg0Ptr->muxingTableSubfieldsCfgArrPtr,
        sizeof(appMuxingTableSubfieldsCfgArr));
    appMuxingTableSubfieldsCfgArr[0].muxWordWithCommonHeaderIndexesBmp = 0;
    appCfg.muxingTableSubfieldsCfgArrPtr = appMuxingTableSubfieldsCfgArr;
    for (i = 0; (i < TGF_TCAM_CLIENTS_PCL_LOOKUP_INGRESS_NUM_OF_E); i++)
    {
        /* 60 UDBs IPCL */
        appCfg.ingressSuperKeyFieldsSet.udbCfgAllFields.udbIndexesBmpArr[i][0] = 0xFFFFFFFF;
        appCfg.ingressSuperKeyFieldsSet.udbCfgAllFields.udbIndexesBmpArr[i][1] = 0x0FFFFFFF;
    }
    /* 60 UDBs EPCL */
    appCfg.egressSuperKeyFieldsSet.udbIndexesBmp[0] = 0xFFFFFFFF;
    appCfg.egressSuperKeyFieldsSet.udbIndexesBmp[1] = 0x0FFFFFFF;
    ...
    if (direction == CPSS_PCL_DIRECTION_EGRESS_E)
    {
        ...
        cpssAction.vlan.egress.vlanId = PRV_TGF_PCL_NEW_VLAN_ID_CNS;
    }
    else
    {
        cpssAction.vlan.ingress.modifyVlan =
            CPSS_PACKET_ATTRIBUTE_ASSIGN_FOR_ALL_E;
        cpssAction.vlan.ingress.vlanId = PRV_TGF_PCL_NEW_VLAN_ID_CNS;
    }
    /* set currentRuleIndex variable */
    setRuleIndex(direction, lookupNum);

    devRuleIndex = prvWrAppDxChTcamPclConvertedIndexGet_fromUT(
        prvTgfDevNum, currentRuleIndex, tcamRuleSize);
    commonKeyHeaderMask = 0;
    commonKeyHeaderPattern = 0;
...
)
{
    ...
    {
        prvTgfPclUdbOnlyKeysUdbOnlyGenericTestSip7(
            direction, lookupNum, ruleSize,
            offsetType, udbAmount, udbOffsetLow,
            packetPtr, offsetInPacket);
    }
    else
    {
        prvTgfPclUdbOnlyKeysUdbOnlyGenericTestNotSip7(
            direction, lookupNum, packetType, packetTypeIndex,
            ruleFormat, ruleSize,
            offsetType, udbAmount, udbIndexLow, udbOffsetLow, udbOrderAscend,
            packetPtr, offsetInPacket);
    }
}
...
)
{
    ...
    rc = cpssDxChPclSourceIdMaskSet(
        prvTgfDevNum, CPSS_PCL_DIRECTION_INGRESS_E, lookupNum, 0xFFF);
...
)
{
    ...
    {
        rc = cpssDxChPclEpgConfigGet(
            prvTgfDevNum, CPSS_PCL_DIRECTION_INGRESS_E, lookupNum, &epgConfigRestore);
        ...
        rc = cpssDxChPclEpgConfigSet(
            prvTgfDevNum, CPSS_PCL_DIRECTION_INGRESS_E, lookupNum, &epgConfig);
