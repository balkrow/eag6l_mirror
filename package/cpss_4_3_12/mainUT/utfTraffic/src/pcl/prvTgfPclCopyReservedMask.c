...
*
* @brief Test includes :
*                 CopyReserved feature support test - TTI/IPCL/EPCL
*                 PCL action <triggerInterupt> validation test for SIP6 above devices
*                 Flow_Id extension (16 bits) Test for SIP6_10 and above devices
*                 IPv6 EH detection metadata  validation test for SIP6_10 and above devices
*
* @version   1
********************************************************************************
*/
...
/* packet's IPv6 header - EH*/
static TGF_PACKET_IPV6_STC prvTgfPacketIpv6Part = {
    6,                  /* version */
    0,                  /* trafficClass */
    0,                  /* flowLabel */
    0x00,               /* payloadLen */
    0x2B,               /* nextHeader - Routing header */
    0x40,               /* hopLimit */
...
#define PRV_TGF_IPV6_PACKET_LEN_CNS \
    ...
    TGF_IPV6_HEADER_SIZE_CNS + sizeof(prvTgfPacketPayloadDataArr)
...
/* packet to send */
static TGF_PACKET_STC prvTgfPacketInfo1 =
{
    TGF_PACKET_AUTO_CALC_LENGTH_CNS,              /* totalLen */
    (sizeof(prvTgfPacketPartArray1)
        / sizeof(prvTgfPacketPartArray1[0])), /* numOfParts */
    prvTgfPacketPartArray1                    /* partsArray */
};
...
)
{
    ...
    GT_U32                              udbIndexArr[7] = {40,41,42,43,44,45,46};
    ...
    /* Mapping UDB_0/1/2/3/4/5/6 to UDB_40/41/42/43/44/45/46 */
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) && (ttiBasedCopyReservedAssignEn != GT_TRUE))
    {
        ...
        udbSelect.udbSelectArr[5] = udbIndexArr[5];
        udbSelect.udbSelectArr[6] = udbIndexArr[6];
    }
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) && (ttiBasedCopyReservedAssignEn != GT_FALSE) && PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily != CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        /* AUTODOC: Metadata UDBs configuration for IPv6 EH detection */
        /* IPCL UDBs Metadata:(1322:1312) -  (11 bits) */
        ...
        rc = prvTgfPclUserDefinedByteSet(PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_10_E,
                                     ...
                                     udbIndexArr[6],
                                     PRV_TGF_PCL_OFFSET_METADATA_E,
                                     165);  /* Anchor offset: 165 Bit offset: 2:0 */
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, rc,
                                    "prvTgfPclUserDefinedByteSet: packetType %d, udbIndex %d,"
                                    "OffsetType %d, Offset %d\n",
                                     PRV_TGF_PCL_PACKET_TYPE_IPV6_OTHER_E,
                                     udbIndexArr[6], PRV_TGF_PCL_OFFSET_METADATA_E, 165);

        rc = prvTgfPclUserDefinedBytesSelectSet(PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_10_E,
                                                PRV_TGF_PCL_PACKET_TYPE_IPV6_OTHER_E, lookupNum,
                                                &udbSelect);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfPclUserDefinedBytesSelectSet: %d, %d",
                                     PRV_TGF_PCL_PACKET_TYPE_IPV6_OTHER_E, lookupNum);

        /* AUTODOC: mask and pattern for IPv6 EH detection */
        mask.ruleIngrUdbOnly.udb[5] = 0xFF;
        mask.ruleIngrUdbOnly.udb[6] = 0x7;

        pattern.ruleIngrUdbOnly.udb[5] =  2;
...
)
{
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) && ttiBasedCopyReservedAssignEn != GT_FALSE && PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily != CPSS_PP_FAMILY_DXCH_IRONMAN_E)
    {
        rc = prvTgfPclDefPortInit(
                ...
                CPSS_PCL_DIRECTION_INGRESS_E,
                lookupNum,
                PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_10_E /*nonIpKey*/,
                PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_10_E /*ipv4Key*/,
                PRV_TGF_PCL_RULE_FORMAT_INGRESS_UDB_10_E /*ipv6Key*/);
    ...
    if(ttiBasedCopyReservedAssignEn)
    {
        prvTgfPclCopyReservedTestPclRulesAndActionSet(PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) ? TTI_COPY_RESERVED_SIP_6_10_ACTION_CNS :
                                                      TTI_COPY_RESERVED_ACTION_CNS, ttiBasedCopyReservedAssignEn, lookupNum);
    }
    else
    {
        prvTgfPclCopyReservedTestPclRulesAndActionSet(PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) ? COPY_RESERVED_SIP_6_10_ACTION_CNS :
                                                      COPY_RESERVED_ACTION_CNS, ttiBasedCopyReservedAssignEn, lookupNum);
    }
}
/**
...
)
{
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(prvTgfDevNum) && ttiBasedCopyReservedAssignEn != GT_FALSE &&
