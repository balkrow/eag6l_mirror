...
typedef struct
{
    ...
    PRV_TGF_PCL_PACKET_TYPE_ENT       tgfPclPacketType;
    ...
    /* use 80-byte IPCL UDB70+10fixed instead of UDB60 */
    GT_BOOL useIpclUdb70fixed10;

    /* low 16 bits of MAC_SA for each sublookup      */
    /* used only if useIpclUdb70fixed10 == GT_TRUE   */
    /* dedicated to use MAC_SA in fixed10 of UDB80   */
    GT_U16         udb80MacSaBits_15_0_arr[4];

    /** tcam Profile0 configuration - Sip7 Mux Table Line Index (TTI only) */
    GT_U32         tcamMuxTableLineIndexSip7Subkey0;

    /** tcam Profile0 configuration - Sip7 Mux Table Line Offset (TTI only) */
    GT_U32         tcamMuxTableLineOffsetSip7Subkey0;
...
(
    ...
    IN  GT_U16           newVid,
    IN  GT_BOOL          useMacSaBits_15_0,
    IN  GT_U16           macSaBits_15_0
)
{
    ...
    if (useMacSaBits_15_0 != GT_FALSE)
    {
        prvTgfPacketL2PartCopy.saMac[4] = (GT_U8)((macSaBits_15_0 >> 8) & 0xFF);
        prvTgfPacketL2PartCopy.saMac[5] = (GT_U8)(macSaBits_15_0 & 0xFF);
    }
...
)
{
    ...
    for (subLookup = 0; (subLookup < 4); subLookup++)
    {
        ...
        prvTgfPclQuadDiffKeySendAndCheckPacket(
            ...
            testCgrPtr->useIpclUdb70fixed10 /*useMacSaBits_15_0 - for UDB80 inly*/,
            testCgrPtr->udb80MacSaBits_15_0_arr[subLookup]/*macSaBits_15_0*/);
...
*/
static GT_VOID prvTgfPclQuadDiffKeyLookupTestPclConfigureSip6_10
(
   IN PRV_TGF_TCAM_DIFF_KEYS_TEST_PARAM_STC *testCgrPtr
)
{
    ...
    keyType =
        (testCgrPtr->pclDirection == CPSS_PCL_DIRECTION_INGRESS_E)
            ? ((testCgrPtr->useIpclUdb70fixed10 != GT_FALSE)
    ...
    rc = prvTgfPclUserDefinedBytesSelectSet(
        keyType, testCgrPtr->tgfPclPacketType, testCgrPtr->pclLookupNum,
        &udbSelect);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPclUserDefinedBytesSelectSet");

    rc = prvTgfPclUserDefinedByteSet(
        keyType, testCgrPtr->tgfPclPacketType, testCgrPtr->pclDirection,
    ...
    for (subLookup = 0; (subLookup < 4); subLookup++)
    {
        ...
        if (testCgrPtr->pclDirection == CPSS_PCL_DIRECTION_INGRESS_E)
        {
            ...
            if ((testCgrPtr->useIpclUdb70fixed10 != GT_FALSE)
                && (subLookup != 0))
            {
                mask.ruleIngrUdbOnly.udb[fieldOffset + 2] = 0xFF;
                mask.ruleIngrUdbOnly.udb[fieldOffset + 3] = 0xFF;
                pattern.ruleIngrUdbOnly.udb[fieldOffset + 2] =
                    (GT_U8)(testCgrPtr->udb80MacSaBits_15_0_arr[subLookup] & 0xFF);
                pattern.ruleIngrUdbOnly.udb[fieldOffset + 3] =
                    (GT_U8)((testCgrPtr->udb80MacSaBits_15_0_arr[subLookup] >> 8) & 0xFF);
            }
...
* @brief   IPCL Quad lookup of parallel different keys test - PCL configuration
*/
static GT_VOID prvTgfPclQuadDiffKeyLookupTestPclConfigureSip7
(
   IN PRV_TGF_TCAM_DIFF_KEYS_TEST_PARAM_STC *testCgrPtr
)
{
    ...
    static TGF_TCAM_CLIENTS_PCL_RULE_SUB_FIELD_STC pclSubFieldArr[] =
    {
        /* 16 LSBs of DST and SRC MACs */
        {TGF_TCAM_CLIENTS_PCL_RULE_FIELD_L2_DST_MAC_E, 0, 16},
        {TGF_TCAM_CLIENTS_PCL_RULE_FIELD_L2_SRC_MAC_E, 0, 16},
        {TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E, 0, 0}
    };
    static TGF_TCAM_CLIENTS_PCL_SUB_FIELD_CONDITION_STC    pclSubFieldConditionArr[] =
    {
        {{TGF_TCAM_CLIENTS_PCL_RULE_FIELD_L2_DST_MAC_E, 0, 16}, 0, 0/*update both */},
        {{TGF_TCAM_CLIENTS_PCL_RULE_FIELD_L2_SRC_MAC_E, 0, 16}, 0, 0/*update both */},
        {{TGF_TCAM_CLIENTS_PCL_RULE_FIELD_STAMP_LIST_END_E, 0, 0}, 0, 0}
    };
    const TGF_TCAM_CLIENTS_PCL_FULL_CFG_SET_STC*  appCfg0Ptr;
    TGF_TCAM_CLIENTS_PCL_FULL_CFG_SET_STC         appCfg;
    TGF_TCAM_CLIENTS_PCL_TCAM_PROFILE_CFG_STC     profileSubkeyCfgArr[2];
    ...
    pclLookup = tgfTcamClientsPclDirLookupToClientLookup(
        testCgrPtr->pclDirection, testCgrPtr->pclLookupNum);
    /* just as one lookup, but different sublookup key sizes with the same MUX line */
    /* key size will be updated - presents in TCAM profile only */
    appCfg0Ptr = tgfTcamClientsOneLookupPclConfigurationBuild(
        pclLookup, CPSS_DXCH_TCAM_RULE_SIZE_60_B_E, pclSubFieldArr);
    if (appCfg0Ptr == NULL)
    {
        UTF_VERIFY_EQUAL0_STRING_MAC(
            GT_OK, GT_FAIL, "tgfTcamClientsOneLookupPclConfigurationBuild");
        return;
    }
    cpssOsMemCpy(&appCfg, appCfg0Ptr, sizeof(appCfg));
    cpssOsMemCpy(profileSubkeyCfgArr, appCfg0Ptr->profileSubkeyCfgArrPtr, sizeof(profileSubkeyCfgArr));
    appCfg.profileSubkeyCfgArrPtr = profileSubkeyCfgArr;
    for (subLookup = 0; (subLookup < 4); subLookup++)
    {
        profileSubkeyCfgArr[0].tcamProfileCfg.subKeySizes[subLookup] =
            (testCgrPtr->ruleSizesArr[subLookup] <= 6)
                ? (CPSS_DXCH_TCAM_RULE_SIZE_ENT)(testCgrPtr->ruleSizesArr[subLookup] - 1)
                : CPSS_DXCH_TCAM_RULE_SIZE_80_B_E;
    }
    rc = prvTgfSip7TcamClientsGenericDevInit(
        testCgrPtr->devNum, &appCfg);
    UTF_VERIFY_EQUAL0_STRING_MAC(
        GT_OK, rc, "prvTgfSip7TcamClientsGenericDevInit");
    ...
    rc = prvTgfPclDefPortInitSip7(
        portNum,
        testCgrPtr->pclDirection,
        testCgrPtr->pclLookupNum,
        PRV_TGF_PCL_PORT_LOOKUP_CFG_TAB_ACC_MODE_BY_PORT_E,
        0 /*cfgIndex*/, GT_TRUE /*enableLookup*/,
        testCgrPtr->pclCfgPclId, 1/*emSubProfileId*/);
    UTF_VERIFY_EQUAL0_STRING_MAC(
        GT_OK, rc, "prvTgfPclDefPortInitSip7");

    for (subLookup = 0; (subLookup < 4); subLookup++)
    {
        pclSubFieldConditionArr[0].mask    = 0xFFFF;
        pclSubFieldConditionArr[0].pattern =
            testCgrPtr->macDaBits_15_0_arr[subLookup];
        pclSubFieldConditionArr[1].mask    = 0;
        pclSubFieldConditionArr[1].pattern = 0;
        if (testCgrPtr->useIpclUdb70fixed10 != GT_FALSE)
        {
            pclSubFieldConditionArr[1].mask    = 0xFFFF;
            pclSubFieldConditionArr[1].pattern =
                testCgrPtr->udb80MacSaBits_15_0_arr[subLookup];
        }

        cpssOsMemSet(&cpssAction, 0, sizeof(cpssAction));
        cpssAction.egressPolicy   = testCgrPtr->pclDirection;
        cpssAction.pktCmd         = CPSS_PACKET_CMD_FORWARD_E;
        if (testCgrPtr->pclDirection == CPSS_PCL_DIRECTION_EGRESS_E)
        {
            cpssAction.vlan.egress.vlanCmd =
                CPSS_DXCH_PCL_ACTION_EGRESS_TAG0_CMD_MODIFY_OUTER_TAG_E;
            cpssAction.vlan.egress.vlanId = testCgrPtr->newVidArr[subLookup];
        }
        else
        {
            cpssAction.vlan.ingress.modifyVlan = CPSS_PACKET_ATTRIBUTE_ASSIGN_FOR_ALL_E;
            cpssAction.vlan.ingress.vlanId = testCgrPtr->newVidArr[subLookup];
        }

        commonKeyHeaderMask = 0x3FF;
        commonKeyHeaderPattern = testCgrPtr->pclCfgPclId;
        rc = tgfTcamClientsPclRuleSet(
            prvTgfDevNum, prvTgfTcamClientCommomHandlersHandleGet(0),
            1 /*pclCfgProfileId*/,
            0/*udbPacketTypeGroupIndex*/,
            pclLookup,
            subLookup /*tcamSubkeyIndex*/,
            testCgrPtr->ruleIndexArr[subLookup],
            GT_TRUE /*writeValidRule*/,
            &commonKeyHeaderMask, &commonKeyHeaderPattern,
            pclSubFieldConditionArr, &cpssAction);
...
{
    if (PRV_CPSS_SIP_7_CHECK_MAC(testCgrPtr->devNum))
    {
        prvTgfPclQuadDiffKeyLookupTestPclConfigureSip7(testCgrPtr);
    }
    else
    {
        prvTgfPclQuadDiffKeyLookupTestPclConfigureSip6_10(testCgrPtr);
    }
}
...
)
{
    ...
    GT_U32  subLookup;
    GT_U32  index;
    CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC tcamClGrLookupProfileCfg;
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(testCgrPtr->devNum)
        && (testCgrPtr->tcamClient != CPSS_DXCH_TCAM_TTI_E))
    {
        /* done in PCL configuring code */
        return;
    }

    if (PRV_CPSS_SIP_7_CHECK_MAC(testCgrPtr->devNum))
    {
        cpssOsMemSet(&tcamClGrLookupProfileCfg, 0, sizeof(tcamClGrLookupProfileCfg));
        tcamClGrLookupProfileCfg.subKeySizes[0] = CPSS_DXCH_TCAM_RULE_SIZE_10_B_E + (testCgrPtr->ruleSizesArr[0] - 1);
        tcamClGrLookupProfileCfg.subKeySizes[1] = CPSS_DXCH_TCAM_RULE_SIZE_10_B_E + (testCgrPtr->ruleSizesArr[1] - 1);
        tcamClGrLookupProfileCfg.subKeySizes[2] = CPSS_DXCH_TCAM_RULE_SIZE_10_B_E + (testCgrPtr->ruleSizesArr[2] - 1);
        tcamClGrLookupProfileCfg.subKeySizes[3] = CPSS_DXCH_TCAM_RULE_SIZE_10_B_E + (testCgrPtr->ruleSizesArr[3] - 1);
        tcamClGrLookupProfileCfg.subKeyMuxTableLineIndexes[0] = testCgrPtr->tcamMuxTableLineIndexSip7Subkey0;
        tcamClGrLookupProfileCfg.subKeyMuxTableLineIndexes[1] = testCgrPtr->tcamMuxTableLineIndexesArr[0];
        tcamClGrLookupProfileCfg.subKeyMuxTableLineIndexes[2] = testCgrPtr->tcamMuxTableLineIndexesArr[1];
        tcamClGrLookupProfileCfg.subKeyMuxTableLineIndexes[3] = testCgrPtr->tcamMuxTableLineIndexesArr[2];
        tcamClGrLookupProfileCfg.subKeyMuxTableLineOffsets[0] = testCgrPtr->tcamMuxTableLineOffsetSip7Subkey0;
        tcamClGrLookupProfileCfg.subKeyMuxTableLineOffsets[1] = testCgrPtr->tcamMuxTableLine10BOffsetsArr[0];
        tcamClGrLookupProfileCfg.subKeyMuxTableLineOffsets[2] = testCgrPtr->tcamMuxTableLine10BOffsetsArr[1];
        tcamClGrLookupProfileCfg.subKeyMuxTableLineOffsets[3] = testCgrPtr->tcamMuxTableLine10BOffsetsArr[2];

        rc = cpssDxChTcamClientGroupLookupProfileCfgSet(
            testCgrPtr->devNum, clientGroup, testCgrPtr->tcamProfileId, &tcamClGrLookupProfileCfg);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "cpssDxChTcamClientGroupLookupProfileCfgSet");

        /* Mux table for sublookup0 */
        cpssOsMemSet(&tcamLookupMuxTableLineCfg, 0, sizeof(tcamLookupMuxTableLineCfg));
        for (i = 0; (i < 30); i++)
        {
            tcamLookupMuxTableLineCfg.superKeyTwoByteUnitsOffsets[i] = (GT_U8)i;
        }
        rc = cpssDxChTcamClientGroupLookupMuxTableLineSet(
            testCgrPtr->devNum, clientGroup,
            testCgrPtr->tcamMuxTableLineIndexSip7Subkey0,
            &tcamLookupMuxTableLineCfg);
        /* Mux table for sublookup1-3 */
        ...
        for (subLookup = 1; (subLookup < 4); subLookup++)
        {
            ...
            if (testCgrPtr->useIpclUdb70fixed10 != GT_FALSE)
            {
                tcamLookupMuxTableLineCfg.superKeyTwoByteUnitsOffsets[index + 1]
                    = (70 / 2); /* bytes 0-1 of MAC_SA from 80-byte-key byte 70 */
            }
...
)
{
    ...
    testCfg.macDaBits_15_0_arr[2] = 0x9ABC;
    testCfg.macDaBits_15_0_arr[3] = 0xDEF1;
    testCfg.prvTgfPacketInfoPtr = &prvTgfPacketInfo;
    testCfg.pclPacketType = CPSS_DXCH_PCL_PACKET_TYPE_ETHERNET_OTHER_E;
    testCfg.tgfPclPacketType = PRV_TGF_PCL_PACKET_TYPE_ETHERNET_OTHER_E;
...
/**
* @internal prvTgfPclQuadDiffKeyLookupIPCL0Test80Bit function
* @endinternal
*
* @brief   IPCL Quad lookup of parallel different keys test
*/
GT_VOID prvTgfPclQuadDiffKeyLookupIPCL0Test80Bit
(
    GT_VOID
)
{
    ...
    /* UDB80 related */
    testCfg.useIpclUdb70fixed10 = GT_TRUE;
    testCfg.udb80MacSaBits_15_0_arr[0] = 0xFEDC;
    testCfg.udb80MacSaBits_15_0_arr[1] = 0xBA98;
    testCfg.udb80MacSaBits_15_0_arr[2] = 0x7654;
    testCfg.udb80MacSaBits_15_0_arr[3] = 0x321F;
...
)
{
    ...
    testCfg.tcamMuxTableLine10BOffsetsArr[1] = 2;
    testCfg.tcamMuxTableLine10BOffsetsArr[2] = 3;
    testCfg.tcamMuxTableLineIndexSip7Subkey0 = 7;
    testCfg.tcamMuxTableLineOffsetSip7Subkey0 = 0;
