...
*/
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_AAS_E);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_AAS_E);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_CPSS_PP_E_ARCH_CNS);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION_E | UTF_AAS_E);
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, UTF_XCAT3_E | UTF_LION_E | UTF_AAS_E);
...
*/
    ...
    UTF_E_ARCH_NOT_SUPPORTED_FAMILY_GET_MAC(&notAppFamilyBmp);
    notAppFamilyBmp |= UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E;
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, notAppFamilyBmp) ;
...
UTF_TEST_CASE_MAC(prvTgfPclEgrUdb20Ipv6UdpReplacedPclId)
{
    ...
    UTF_E_ARCH_NOT_SUPPORTED_FAMILY_GET_MAC(&notAppFamilyBmp);
    notAppFamilyBmp |= UTF_IRONMAN_L_E | UTF_AAS_E;
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, notAppFamilyBmp) ;
...
UTF_TEST_CASE_MAC(prvTgfPclIngrUdb20Ipv6UdpReplacedPclIdSrcTrgMode)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, (~ notAppFamilyBmp) | UTF_AAS_E) ;
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP6_CNS) | UTF_AAS_E);
...
UTF_TEST_CASE_MAC(prvTgfPclUdbOnlyKeysUdbOnlyIngressPortPclId2)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~  UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_AAS_E);
...
/*  AUTODOC: prvTgfPclMetadataIpv4FragTest
    Test IPCL Metadata fields Ingress UDB.
    Classify IPV4 Fragments First, Middle and Last */
UTF_TEST_CASE_MAC(prvTgfPclMetadataIpv4Frag)
{
    GT_U32 notAppFamilyBmp;
    ...
    prvTgfPclMetadataPacketIpv4FragTest();
    prvTgfPclMetadataPacketIpv4FragTestRestore();
}
...
UTF_TEST_CASE_MAC(tgfPclEgressMirroring)
{
    ...
    */
    ...
    prvTgfPclEgressMirroringCfgSet(
        GT_FALSE /*actionLatencyMonitor*/,
        GT_TRUE /*actionTxMirror*/,
        GT_FALSE /*notActionTxMirror*/);
...
/*----------------------------------------------------------------------------*/
/*
    AUTODOC: Test tgfPclEgressMirroringWithLM_ActionMirror
    configure VLAN, FDB entries;
    configure PCL rules - to forward, configure Egress mirroring,
        Enable Latency monitoring;
    send traffic ;
    verify traffic is sent from analyzer.
    verify Latency monitoring counters.
*/
UTF_TEST_CASE_MAC(tgfPclEgressMirroringWithLM_ActionMirror)
{
    /*
        1. Set configuration
        2. Set PCL configuration
    ...
    */
    ...
    /* Set PCl configuration */
    prvTgfPclEgressMirroringCfgSet(
        GT_TRUE /*actionLatencyMonitor*/,
        GT_TRUE /*actionTxMirror*/,
        GT_FALSE /*notActionTxMirror*/);

    /* Clear Latency Monitoring Unit results on egress and analizer ports */
    prvTgfPclEgressMirroringLMClear();

    /* Generate traffic */
    prvTgfPclEgressMirroringTrafficGenerate();

    /* Check Latency Monitoring Unit results on egress and analizer ports */
    prvTgfPclEgressMirroringLMCheck(
        GT_TRUE, GT_FALSE);
...
/*----------------------------------------------------------------------------*/
/*
    AUTODOC: Test tgfPclEgressMirroringWithLM_NotActionMirror
    configure VLAN, FDB entries;
    configure PCL rules - to forward, Enable Latency monitoring;
    Configure Egress mirroring by mirror API;
    send traffic ;
    verify traffic is sent from analyzer.
    verify Latency monitoring counters.
*/
UTF_TEST_CASE_MAC(tgfPclEgressMirroringWithLM_NotActionMirror)
{
    /*
        1. Set configuration
        2. Set PCL configuration
    ...
    */
    ...
    prvTgfPclEgressMirroringCfgSet(
        GT_TRUE /*actionLatencyMonitor*/,
        GT_FALSE /*actionTxMirror*/,
        GT_TRUE /*notActionTxMirror*/);
...
/*----------------------------------------------------------------------------*/
/*
    AUTODOC: Test tgfPclEgressMirroringWithLM_NoPclRule
    configure VLAN, FDB entries;
    Configure Egress mirroring by mirror API;
    Enable Latency monitoring by Latency monitoring API;
    send traffic ;
    verify traffic is sent from analyzer.
    verify Latency monitoring counters.
*/
UTF_TEST_CASE_MAC(tgfPclEgressMirroringWithLM_NoPclRule)
{
    /*
        1. Set configuration
        2. Set PCL configuration
    ...
    */
    ...
    prvTgfPclEgressMirroringNotPclLmCfgSet();
    ...
    /* Restore configuration */
    prvTgfPclEgressMirroringNotPclLmCfgRestore();
}
...
UTF_TEST_CASE_MAC(tgfPclEgreesUDB60PclIdDrop)
{
    ...
    */
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ((~ UTF_CPSS_PP_E_ARCH_CNS) | UTF_CPSS_PP_ALL_SIP6_10_CNS ));
...
UTF_TEST_CASE_MAC(tgfPclIngressReflectAction)
{
    ...
    */
    ...
    /* Set PCl rule */
    prvTgfPclIngressReflectActionConfigurationSet();

    /* Generate traffic */
    prvTgfPclIngressReflectActionTrafficGenerateAndCheck();
...
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ((~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_IRONMAN_L_E))
...
UTF_TEST_CASE_MAC(prvTgfPclQuadDiffKeyLookupIPCL2)
{
    /* AAS has no IPCL2 */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ((~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_IRONMAN_L_E | UTF_AAS_E))
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - Ingress PCL Quad Different Keys parallel lookup0 70UDBs + 10 fixed Key:
*/
UTF_TEST_CASE_MAC(prvTgfPclQuadDiffKeyLookupIPCL0_80Bit)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ((~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_IRONMAN_L_E))

    prvTgfPclQuadDiffKeyLookupIPCL0Test80Bit();
}
...
/*----------------------------------------------------------------------------------------------------------------------------------*/
/* AUTODOC: Test New CPU code assigment mapped to different TC than original one for Ingress TRAP packet using EPCL and PHA Threads :
    ...
    Match second pass Egress TRAP packet in EPCL using SrcTrg/Rx Sniff and egress Mtag related attributes;
    Assign Copy Flow-Id to Target Port PHA thread;
    Verify:
    ...
    match src/target_pfysical_port with source physical port of original packet;
    In addition to this, if mode is use copy reserved for vid, verify original VLAN and Flow ID is preserved.
...
UTF_TEST_CASE_MAC(prvTgfEpclTrapIngressTrapPacket)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ~ (UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E ));

    /* Test uses PHA feature but GM does not support PHA, therefore skip test */
    GM_NOT_SUPPORT_THIS_TEST_MAC

    prvTgfEpclReTrapPacketConfig(CPSS_DXCH_PHA_RE_TRAP_MODE_USE_FLOW_ID_FOR_PORT_E);

    prvTgfEpclReTrapPacketTrafficGenerate(CPSS_DXCH_PHA_RE_TRAP_MODE_USE_FLOW_ID_FOR_PORT_E);

    prvTgfEpclReTrapPacketRestore();

    prvTgfEpclReTrapPacketConfig(CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E);

    prvTgfEpclReTrapPacketTrafficGenerate(CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E);
...
UTF_TEST_CASE_MAC(prvTgfEpclDropPacketToErepTrap)
{
    /*
        ...
        3. Restore configuration
    */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, (~UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_AAS_E);
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - EPCL TCP/UDP Port Comparator extension metadata test:
   Checks matching EPCL L4 Port Comparator (indexes more than 8) and SRC_IP and DST_IP from metadata.
*/
UTF_TEST_CASE_MAC(prvTgfPclTcpUdpPortEgressMetadata)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, ((~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_IRONMAN_L_E | UTF_AAS_E))
    prvTgfPclTcpUdpPortMetadataTestEgressConfigurationSet();
    prvTgfPclTcpUdpPortMetadataTestEgressConfigurationRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - IPV6 header extensions bitmap IPCL metadata test.
   Sent packet with two IPV6 extensions: Hop to Hop (type==0)
   and Encapsulate Secure Payload (type==50)
   Matched IPCL Metadata field IPV6 Extension Bitmap value 0x09
*/
UTF_TEST_CASE_MAC(prvTgfPclIpv6ExtBmpMetadata)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_AAS_E);
    GM_NOT_SUPPORT_THIS_TEST_MAC;

    prvTgfPclIpv6ExtBmpMetadataTestConfigurationSet();
    prvTgfPclIpv6ExtBmpMetadataTestConfigurationRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - IPV6 Min and Max L4 Port data in IPCL metedata.
   Sent IPV6 Udppacket
   Matched IPCL Metadata fields IPV6 Min and Max L4 Port
   Used 80-byte key, UDB0-11 for MACs UDB30-65 for metadata
*/
UTF_TEST_CASE_MAC(prvTgfPclIpv6MinMaxL4PortMetadata)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_AAS_E);
    prvTgfPclIpv6MinMaxL4PortMetadataTestConfigurationSet();
    prvTgfPclTcpUdpPortMetadataTestConfigurationRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - IPV6 L4 Valid in IPCL metadata for IPV6 with ESP extension.
   Sent IPV6 with ESP extension packet
   Matched IPCL Metadata fields IPx protocol, IPV6 extension bitmask, L4 offset invalid, L4 valid.
   Used 60-byte key, UDB0-11 for MACs UDB30-34 for metadata
*/
UTF_TEST_CASE_MAC(prvTgfPclIpv6EspExtNoL4Metadata)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_AAS_E);
    prvTgfPclIpv6EspExtNoL4TestConfigurationSet();
    prvTgfPclTcpUdpPortMetadataTestConfigurationRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test - SIP7 MPCL MAC_DA and MAC_SA match
*/
UTF_TEST_CASE_MAC(prvTgfSip7PclMpclMacDaSa)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP7_CNS));
    prvTgfSip7PclMpclMacDaSaTest();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: IPCL,MPCL,EPCL Tests matching MAC_DA and MAC_SA and UDB58-59
*/
UTF_TEST_CASE_MAC(prvTgfSip7PclUdbAppField)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP7_CNS));
    prvTgfSip7PclUdbAppFieldTest();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: IPCL0 -> IPCL1, IPCL1 ->MPCL, MPCL->EPCL
   set Cfg table entry index for the next lookup
   Rules match PCL ID only (took from Cfg table entry).
   EPCL updates VID, test checks it.
*/
UTF_TEST_CASE_MAC(prvTgfSip7PclCfgEntryIndexSet)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP7_CNS));
    prvTgfSip7PclCfgEntryIndexSetTest();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: IPCL0->IPCL1, IPCL0->MPCL, IPCL1->MPCL, MPCL->EPCL
   cases of actionStop checked.
   For each lookup pair - 2 rules updating VLAN id. Two cases below:
   When first lookup has actionStop=1 egress packet has VLAN Id of the first Action.
   When first lookup has actionStop=0 egress packet has VLAN Id of the second Action.
*/
static GT_U32 prvTgfSip7PclActionStopSkipTestBmp = 0;
void prvTgfSip7PclActionStopSkipTestBmpSet(GT_U32 bmp)
{
    prvTgfSip7PclActionStopSkipTestBmp = bmp;
}

UTF_TEST_CASE_MAC(prvTgfSip7PclActionStop)
{
    GT_U32 stepIdx = 0;
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP7_CNS));

    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
        prvTgfSip7PclActionStopTestStep(
             CPSS_PCL_DIRECTION_INGRESS_E /*direction0*/,
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum0*/,
             GT_FALSE /*actionStop*/,
             CPSS_PCL_DIRECTION_INGRESS_E /*direction1*/,
             CPSS_PCL_LOOKUP_0_1_E /*lookupNum1*/);
    }
    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
        prvTgfSip7PclActionStopTestStep(
    ...
    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
        prvTgfSip7PclActionStopTestStep(
             ...
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum0*/,
             GT_FALSE /*actionStop*/,
             CPSS_PCL_DIRECTION_MIDWAY_E /*direction1*/,
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum1*/);
    }
    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
        prvTgfSip7PclActionStopTestStep(
    ...
    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
        prvTgfSip7PclActionStopTestStep(
             CPSS_PCL_DIRECTION_INGRESS_E /*direction0*/,
             CPSS_PCL_LOOKUP_0_1_E /*lookupNum0*/,
             GT_FALSE /*actionStop*/,
             CPSS_PCL_DIRECTION_MIDWAY_E /*direction1*/,
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum1*/);
    }
    ...
    {
        prvTgfSip7PclActionStopTestStep(
             CPSS_PCL_DIRECTION_MIDWAY_E /*direction0*/,
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum0*/,
             GT_FALSE /*actionStop*/,
             CPSS_PCL_DIRECTION_EGRESS_E /*direction1*/,
             CPSS_PCL_LOOKUP_0_0_E /*lookupNum1*/);
    }
    if ((prvTgfSip7PclActionStopSkipTestBmp & (1 << stepIdx++)) == 0)
    {
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: lookus IPCL0, IPCL1, MPCL, EPCL
   parallel sublookups {0,1}, {0,2}, {0,3}, {1,2}, {1,3}, {2,3}
   types Hard Drop and Soft Drop
   cases of cancelDrop enabled and cancelDrop disabled checked.
   Action of first sublookup contains cancelDrop = GT_TRUE,
   Action of second sublookup contains Hard or Soft Drop command.
   cpssDxChPclParallelLookupDropCancelEnableSet enables/disables
   cancelDrop for each case.
   When cancelDrop enabled - the sent packet must pass (and be captured)
   When cancelDrop disabled - the sent packet must be dropped.
*/
static GT_U32 prvTgfSip7PclActionCancelDropSkipTestBmp[4] = {0,0,0,0};
void prvTgfSip7PclActionCancelDropSkipTestBmpSet(
    GT_U32 bmp0, GT_U32 bmp1, GT_U32 bmp2, GT_U32 bmp3)
{
    prvTgfSip7PclActionCancelDropSkipTestBmp[0] = bmp0;
    prvTgfSip7PclActionCancelDropSkipTestBmp[1] = bmp1;
    prvTgfSip7PclActionCancelDropSkipTestBmp[2] = bmp2;
    prvTgfSip7PclActionCancelDropSkipTestBmp[3] = bmp3;
}

UTF_TEST_CASE_MAC(prvTgfSip7PclActionCancelDrop)
{
    static const struct
    {
    ...
    } lookupArr[] =
    {
        ...
        {CPSS_PCL_DIRECTION_INGRESS_E, CPSS_PCL_LOOKUP_0_1_E},
        {CPSS_PCL_DIRECTION_MIDWAY_E, CPSS_PCL_LOOKUP_0_0_E},
        {CPSS_PCL_DIRECTION_EGRESS_E, CPSS_PCL_LOOKUP_0_0_E},
    };
    static const GT_U32 lookupArrSize = sizeof(lookupArr) / sizeof(lookupArr[0]);
    static const struct
    {
        GT_U32 subLookup0;
        GT_U32 subLookup1;
    ...
    static const GT_U32 sublookupPairArrSize =
        sizeof(sublookupPairArr) / sizeof(sublookupPairArr[0]);
    GT_U32 lookupIdx;
    GT_U32 sublookupPairIdx;
    GT_U32 hardOrSoftDropIdx;
    GT_U32 enableCancelDropIdx;
    GT_U32 stepIdx;

    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(
        prvTgfDevNum, (~ UTF_CPSS_PP_ALL_SIP7_CNS));

    for (lookupIdx = 0; (lookupIdx < lookupArrSize); lookupIdx++)
    {
        for (sublookupPairIdx = 0; (sublookupPairIdx < sublookupPairArrSize); sublookupPairIdx++)
        {
            for (hardOrSoftDropIdx = 0; (hardOrSoftDropIdx < 2); hardOrSoftDropIdx++)
            {
                for (enableCancelDropIdx = 0; (enableCancelDropIdx < 2); enableCancelDropIdx++)
                {
                    stepIdx = ((sublookupPairIdx * 4) + (hardOrSoftDropIdx * 2) + enableCancelDropIdx);
                    if ((prvTgfSip7PclActionCancelDropSkipTestBmp[lookupIdx] & (1 << stepIdx)) == 0)
                    {
                        prvTgfSip7PclActionCancelDropStep(
                            lookupArr[lookupIdx].direction,
                            lookupArr[lookupIdx].lookupNum,
                            (hardOrSoftDropIdx ? GT_TRUE : GT_FALSE),
                            (enableCancelDropIdx ? GT_TRUE : GT_FALSE),
                            sublookupPairArr[sublookupPairIdx].subLookup0, /*hitWithCancelDrop*/
                            sublookupPairArr[sublookupPairIdx].subLookup1 /*hitWithDrop*/
                        );
                    }
                }
            }
...
UTF_SUIT_BEGIN_TESTS_MAC(tgfPcl)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclMetadataIpv4Frag)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfPclEgressMirroringWithLM_ActionMirror)
    UTF_SUIT_DECLARE_TEST_MAC(tgfPclEgressMirroringWithLM_NotActionMirror)
    UTF_SUIT_DECLARE_TEST_MAC(tgfPclEgressMirroringWithLM_NoPclRule)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclQuadDiffKeyLookupIPCL0_80Bit)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclTcpUdpPortEgressMetadata)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclIpv6ExtBmpMetadata)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclIpv6MinMaxL4PortMetadata)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfPclIpv6EspExtNoL4Metadata)

    UTF_SUIT_DECLARE_TEST_MAC(prvTgfSip7PclMpclMacDaSa)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfSip7PclUdbAppField)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfSip7PclCfgEntryIndexSet)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfSip7PclActionStop)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfSip7PclActionCancelDrop)
