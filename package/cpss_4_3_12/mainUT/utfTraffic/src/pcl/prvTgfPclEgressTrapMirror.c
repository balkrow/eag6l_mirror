...
#define PRV_TGF_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PORT_CNS     76
...
#define EPCL_MATCH_INDEX_2_CNS                                  prvTgfEpclTcamAbsoluteIndexWithinTheLookupGet(12)

/* Max flow ID */
#define PRV_TGF_FLOW_ID_MAX_CNS                                 0xFFF
...
/* flowId used for validation */
static GT_U32        flowId                     =   0;
...
static prvTgfPclUdb prvTgfEpclMetadataUdbInfo[] = {
     ...
     {0 , PRV_TGF_PCL_OFFSET_METADATA_E, 53, 0x01},

     /* Egress Mtag Cmd [0x0 = TO_CPU],Bit[1:0]; Egress Marvell Tagged [0x1],Bit[2]; Egress Marvell Tagged Extended [0x3 = 16B eDSA], Bit[4:3] */
     {1 , PRV_TGF_PCL_OFFSET_METADATA_E, 64, 0x1F},

     /* Flow Id [7:0] */
     {2 , PRV_TGF_PCL_OFFSET_METADATA_E, 82, 0xFF},

     /* Flow Id [3:0] */
     {3 , PRV_TGF_PCL_OFFSET_METADATA_E, 83, 0x0F},

     /* eVid[7:0] */
     {4, PRV_TGF_PCL_OFFSET_METADATA_E , 26, 0xFF},

     /* eVid[11:8] */
     {5, PRV_TGF_PCL_OFFSET_METADATA_E , 27, 0x0F},

     /* must be last */
     {6, PRV_TGF_PCL_OFFSET_INVALID_E , 0, 0}
};

static prvTgfPclUdb prvTgfEpclTrapDropMetadataUdbInfo[] = {

     /* Egress Mtag Command 0x3 = FORWARD Bit[1:0] */
     {0 , PRV_TGF_PCL_OFFSET_METADATA_E, 64, 0x03},

     /* must be last */
     {1, PRV_TGF_PCL_OFFSET_INVALID_E , 0, 0}
};
...
)
{
    ...
    while(1)
    {
        if(prvTgfEpclTrapDropMetadataUdbInfo[udbSelectidx].offsetType == PRV_TGF_PCL_OFFSET_INVALID_E)
            break;
...
)
{
    ...
    if(reTrapMode >= CPSS_DXCH_PHA_RE_TRAP_MODE_LAST_E)
    {
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_BAD_PARAM, "Invalid reTrapMode:%d", reTrapMode);
    }
    ...
    if(reTrapMode == CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E)
    {
         flowId           = cpssOsRand() % (PRV_TGF_FLOW_ID_MAX_CNS + 1);
    ...
    while(1)
    {
        if(prvTgfEpclMetadataUdbInfo[udbSelectidx].offsetType == PRV_TGF_PCL_OFFSET_INVALID_E)
            break;
    ...
    /* bit 0 Anchor Offset 53 */
    pattern.ruleEgrUdbOnly.udb[0] = 0x1 /* Src Trg/Rx Sniff [0x1 = Egress Trapped] */;

    /* AUTODOC: matching with flow id and vlan id will ensure that
     *          the packet's incoming flow id and original vlan id is preserved
     */
    /* bit [7:0] Anchor Offset 82 */
    pattern.ruleEgrUdbOnly.udb[2] = flowId & 0xFF/* Flow ID[7:0] */;
    mask.ruleEgrUdbOnly.udb[2]    = (reTrapMode == CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E) ?
                                    prvTgfEpclMetadataUdbInfo[2].byteMask : 0;

    /* bit [3:0] Anchor Offset 83 */
    pattern.ruleEgrUdbOnly.udb[3] = (flowId >> 8) & 0xF/* Flow ID[11:8] */;
    mask.ruleEgrUdbOnly.udb[3]    = (reTrapMode == CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E) ?
                                    prvTgfEpclMetadataUdbInfo[3].byteMask : 0;

    if (reTrapMode == CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E)
    {
        /* get default vlanId */
        rc = prvTgfBrgVlanPortVidGet(
            prvTgfDevNum, prvTgfPortsArray[PRV_TGF_INGRESS_PORT_IDX_CNS], &ingrPortVid);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanPortVidGet");

        /* bit [7:4] Anchor Offset 26 */
        pattern.ruleEgrUdbOnly.udb[4] = ingrPortVid & 0x0F/* eVlan ID[3:0] */;
        mask.ruleEgrUdbOnly.udb[4]    = prvTgfEpclMetadataUdbInfo[4].byteMask;

        /* bit [3:0] Anchor Offset 27 */
        pattern.ruleEgrUdbOnly.udb[5] = (ingrPortVid >> 4) & 0xFF/* eVlan ID[11:4] */;
        mask.ruleEgrUdbOnly.udb[5]    = prvTgfEpclMetadataUdbInfo[5].byteMask;
    }

    /* AUTODOC: EPCL action - enable PHA FW to fix trg_phy_port the packet */
    action.egressPolicy                          = GT_TRUE;
    action.pktCmd                                = CPSS_PACKET_CMD_FORWARD_E;
    action.epclPhaInfo.phaThreadIdAssignmentMode = PRV_TGF_PCL_ACTION_EGRESS_PHA_THREAD_ID_ASSIGNMENT_MODE_ENABLED_E;
    action.epclPhaInfo.phaThreadId               = PRV_TGF_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PORT_CNS;

    rc = prvTgfPclRuleSet(
            PRV_TGF_PCL_RULE_FORMAT_EGRESS_UDB_10_E,
            EPCL_MATCH_INDEX_2_CNS, &mask, &pattern, &action);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfPclRuleSet: %d, %d",
                                 PRV_TGF_PCL_RULE_FORMAT_EGRESS_UDB_10_E,
                                 EPCL_MATCH_INDEX_CNS);
...
)
{
    ...
    extInfo.clearOutgoingMtagCmd.reTrapMode = reTrapMode;
    ...
    extInfo.copyFlowIdToTrgPhyPort.reTrapMode = reTrapMode;
    /* AUTODOC: Set the thread entry */
    rc = cpssDxChPhaThreadIdEntrySet(prvTgfDevNum, PRV_TGF_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PORT_CNS, &commonInfo,
                                     CPSS_DXCH_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PHY_PORT_E, &extInfo);
    UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc,"[TGF]: cpssDxChPhaThreadIdEntrySet FAILED, phaThreadId = [%d], extType = [%d], rc = [%d]",
    PRV_TGF_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PORT_CNS, CPSS_DXCH_PHA_THREAD_TYPE_COPY_FLOW_ID_TO_TRG_PHY_PORT_E, rc);
...
)
{
    ...
    prvTgfIEpclConfig(reTrapMode);

    /* PHA - Attach PHA Thread as per EPCL action */
    prvTgfPhaConfig(reTrapMode);
...
)
{
    ...
    UTF_VERIFY_EQUAL0_PARAM_MAC(CPSS_NET_USER_DEFINED_0_E,
                                rxParams.cpuCode);
    UTF_VERIFY_EQUAL0_PARAM_MAC(prvTgfPortsArray[PRV_TGF_INGRESS_PORT_IDX_CNS],
                                rxParams.portNum);
    if(reTrapMode == CPSS_DXCH_PHA_RE_TRAP_MODE_USE_COPY_RESERVED_FOR_VID_E)
    {
        UTF_VERIFY_EQUAL0_PARAM_MAC(flowId, rxParams.flowIdTtOffset.flowId);
    }
...
)
{
    ...
    /* AUTODOC: invalidate I/EPCL rule */
    rc = prvTgfPclRuleValidStatusSet(
            CPSS_PCL_RULE_SIZE_10_BYTES_E, EPCL_MATCH_INDEX_2_CNS, GT_FALSE);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPclRuleValidStatusSet");
