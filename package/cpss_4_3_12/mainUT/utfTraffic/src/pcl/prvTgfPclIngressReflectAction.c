...
/* Ports array index */
#define PRV_TGF_PORTS_ARRAY_SEND_IDX_CNS     0

/* Ports array index */
#define PRV_TGF_PORTS_ARRAY_REDIRECT_IDX_CNS  1

/* Ports array index */
#define PRV_TGF_PORTS_ARRAY_RECEIVE_IDX_CNS  2
...
)
{
    ...
    rc = prvTgfPclDefPortInit(
            prvTgfPortsArray[PRV_TGF_PORTS_ARRAY_SEND_IDX_CNS],
    ...
    prvTgfPclIngressReflectActionRulesAndActionSet(
        prvTgfPortsArray[PRV_TGF_PORTS_ARRAY_REDIRECT_IDX_CNS]);
}
...
)
{
    ...
    rc = prvTgfBrgDefFdbMacEntryOnPortSet(
            ...
            1/*VLANID*/, prvTgfDevNum,
            prvTgfPortsArray[PRV_TGF_PORTS_ARRAY_RECEIVE_IDX_CNS], GT_TRUE);
    ...
    /* Send packet */
    rc = prvTgfStartTransmitingEth(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_PORTS_ARRAY_SEND_IDX_CNS]);
    ...
    for (portIter = 0; portIter < PRV_TGF_PORTS_NUM_CNS; portIter++)
    {
        ...
        if (portIter == PRV_TGF_PORTS_ARRAY_SEND_IDX_CNS)
        {
            continue;
        }

        /* check ETH counters */
        if (portIter == PRV_TGF_PORTS_ARRAY_REDIRECT_IDX_CNS)
        {
            rc = (portCntrs.goodPktsSent.l[0] >= prvTgfBurstCount) ? GT_OK : GT_ERROR;
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "got another goodPktsSent counter than expected");

            rc = (portCntrs.goodOctetsSent.l[0] >=
                      ((PRV_TGF_IPV4_PACKET_LEN_CNS + TGF_CRC_LEN_CNS) * prvTgfBurstCount)) ? GT_OK : GT_ERROR;
