...
/* Mirror analyzer ePort Interface index */
#define PRV_TGF_DEFAULT_EPORT_CNS      1024

/* Mirror analyzer index */
#define PRV_TGF_MIRROR_INDEX_CNS            3
#define PRV_TGF_MIRROR_TX_INDEX_CNS         5

#define PRV_TGF_PACKET_SIZE_CNS             200
#define PRV_TGF_TUNNEL_START_IPV4_INDEX_CNS 1
...
static GT_HW_DEV_NUM               dummyRemoteHwDevNum  = DUMMY_REMOTE_DEV_NUM_CNS;
static GT_HW_DEV_NUM               dummyRemoteHwDevNum2 = DUMMY_REMOTE_DEV_NUM_CNS - 2;
...
static struct
{
    ...
    GT_BOOL                                        ttiLookup;
    GT_BOOL                                        enable;
    GT_U32                                         rxMirrorIndex;
    GT_U32                                         txMirrorIndex;
    GT_U16                                         pvid;
    PRV_TGF_MIRROR_ANALYZER_INTERFACE_STC          rxAnalyzerInterface;
    PRV_TGF_MIRROR_ANALYZER_INTERFACE_STC          txAnalyzerInterface;
    PRV_TGF_MIRROR_TO_ANALYZER_FORWARDING_MODE_ENT forwardingModeGet;
...
)
{
    ...
    if(testingDsaTag == GT_TRUE)
    {
        ...
        prvTgfPacketDsaTagPart_toAnalyzer.devPort.hwDevNum = dummyRemoteHwDevNum;/*dummy device*/
        ...
        if(prvTgfPacketDsaTagPart.dsaType == TGF_DSA_4_WORD_TYPE_E)
        {
            ...
            prvTgfPacketDsaTagPart_toAnalyzer.extDestInfo.devPort.analyzerHwTrgDev = dummyRemoteHwDevNum2;/*dummy device*/
    ...
    if(testingDsaTag == GT_TRUE)
    {
        ...
        if(prvTgfPacketDsaTagPart.dsaType == TGF_DSA_4_WORD_TYPE_E)
        {
            ...
            else /*testingMirrorMode = PRV_TGF_MIRROR_TO_ANALYZER_FORWARDING_SOURCE_BASED_E*/
            {
                ...
                prvTgfPacketDsaTagPart_toAnalyzer.analyzerTrgType = CPSS_INTERFACE_PORT_E;
                prvTgfPacketDsaTagPart_toAnalyzer.extDestInfo.devPort.analyzerIsTrgPortValid = GT_FALSE;/* the physical not valid in legacy mode of 'Source Based Forwarding with Overriding of Src/Trg' */
                prvTgfPacketDsaTagPart_toAnalyzer.extDestInfo.devPort.analyzerHwTrgDev = dummyRemoteHwDevNum;
                prvTgfPacketDsaTagPart_toAnalyzer.extDestInfo.devPort.analyzerTrgPort = CPSS_NULL_PORT_NUM_CNS;
                prvTgfPacketDsaTagPart_toAnalyzer.extDestInfo.devPort.analyzerTrgEport = DUMMY_TARGET_EPORT_NUM_CNS;
            }
...
)
{
    ...
    /* make sure correct values for Remote HW Device Number */
    if ((dummyRemoteHwDevNum == prvTgfDevNum) ||
        (dummyRemoteHwDevNum == hwDevNum))
    {
        dummyRemoteHwDevNum--;
    }

    if ((dummyRemoteHwDevNum2 == prvTgfDevNum) ||
        (dummyRemoteHwDevNum2 == hwDevNum))
    {
        dummyRemoteHwDevNum2--;
    }
    ...
    if(enable == GT_TRUE)
    {
           ...
           HWINIT_GLOVAR(sysGenGlobalInfo.supportDualDeviceId) == GT_TRUE)
        {
            ...
            rc = prvTgfCscdDbRemoteHwDevNumModeSet(dummyRemoteHwDevNum,CPSS_GEN_CFG_HW_DEV_NUM_MODE_DUAL_E);
    ...
    else /*when restore values make sure to do all restore without rc checking !*/
    {
        ...
        prvTgfCscdDbRemoteHwDevNumModeSet(dummyRemoteHwDevNum,CPSS_GEN_CFG_HW_DEV_NUM_MODE_SINGLE_E);
...
)
{
    ...
    for (ii = 0 ; ii < prvTgfPortsNum; ii ++)
    {
        ...
        {
            if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) && portsMirrInfoPtr->physicalPortInfo.mirrorEn)
            {
                PRV_UTF_LOG1_MAC("In SIP7 devices expect port[%d] to recieve packets as Physical port based analyzer index wont compete with incoming analyzerIndex and ePort based analyzerIndex \n",prvTgfPortsArray[ii]);
            }
            else
            {
...
)
{
    ...
    /* check that egress packets as expected */
    UTF_VERIFY_EQUAL1_STRING_MAC((prvTgfBurstCount * numOfPackets) , portCntrs.goodPktsSent.l[0], "get another egress packets value at port %d.", prvTgfPortsArray[egressPortIndex]);
}
/**
...
)
{
    ...
    /* AUTODOC : Restore Tx Analyzer port index to given VLAN */
    prvTgfBrgVlanMirrorToTxAnalyzerSet(vlanId,GT_FALSE,VLAN_EGRESS_ANALYZER_INDEX_CNS);
...
)
{
    ...
    /* AUTODOC : set all 5 ports as 'enable' the vlan egress mirroringi, as it's enabled by default */
    for(ii = 0 ; ii < prvTgfPortsNum ; ii++)
    {
        prvTgfEgressVlanMirroringTest_vlanEgressMirrorEnableOnPort(ii, GT_TRUE);
    }
}
...
static void sendTraffic(void)
{
    ...
    {
        prvTgfPacketDsaTagPart_toAnalyzer_vidx.devPort.hwDevNum = dummyRemoteHwDevNum;
...
)
{
   ...
   /* AUTODOC: Set analyzer port for analyzer */
    ...
    /** AUTODOC : set all 5 ports as 'disable' the vlan egress mirroring, this will reset the default value,
     * VLAN egress mirror enable/disable for test is done in prvTgfBrgEnhancedMirroringPrioritySingleTest,
     * Test initially checks for only ingress mirror, to avoid Tx mirror to analyzer the
     * VLAN egress mirroring is disabled for all 5 ports
     */
    for(ii = 0 ; ii < prvTgfPortsNum ; ii++)
    {
        prvTgfEgressVlanMirroringTest_vlanEgressMirrorEnableOnPort(ii, GT_FALSE);
...
)
{
    ...
    /* AUTODOC : set all 5 ports as 'enable' the vlan egress mirroring, as it's enabled by default */
    for(ii = 0 ; ii < prvTgfPortsNum ; ii++)
    {
        prvTgfEgressVlanMirroringTest_vlanEgressMirrorEnableOnPort(ii, GT_TRUE);
    }
...
#define COMPARE_RX_PARAM(_expected_RxParams,_ectual_RxParams ,_field)   \
        UTF_VERIFY_EQUAL0_STRING_MAC(_expected_RxParams._field, _ectual_RxParams._field, #_field)
GT_STATUS  allowPrint_not_expected_interface_checkPacketFromExpectedInterface(
    IN GT_U32   allowPrint
);
/**
* @internal copyToCpuCheck function
* @endinternal
*
* @brief   Check that the CPU got the expected number of packets from the selected port.
*         NOTE: for egress mirror-to-cpu/TRAP.
...
)
{
    ...
    CPSS_INTERFACE_INFO_STC portInterface;
    TGF_NET_DSA_STC expected_rxParams;
    ...
    cpssOsMemSet(&expected_rxParams, 0, sizeof(expected_rxParams));
    expected_rxParams.dsaCmdIsToCpu = GT_TRUE;
    expected_rxParams.cpuCode = (1063-1024)+CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E;
    expected_rxParams.hwDevNum = hwDevNum;
    expected_rxParams.portNum = portNum;
    expected_rxParams.originByteCount = 64;
    expected_rxParams.ePortNum = portNum;

    /* Go through entries from rxNetworkIf table */
    while (GT_OK == rc)
    {
        packetLen     = 0x600;
        allowPrint_not_expected_interface_checkPacketFromExpectedInterface(GT_TRUE);
        ...
        allowPrint_not_expected_interface_checkPacketFromExpectedInterface(GT_FALSE);
        ...
        COMPARE_RX_PARAM(expected_rxParams,rxParam,dsaCmdIsToCpu);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,cpuCode);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,srcIsTrunk);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,hwDevNum);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,portNum);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,originByteCount);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,trunkId);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,ePortNum);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,packetIsTT);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,flowIdTtOffset.flowId);
        COMPARE_RX_PARAM(expected_rxParams,rxParam,flowIdTtOffset.ttOffset);
...
)
{
    ...
    rc = prvUtfHwDeviceNumberGet(prvTgfDevNum, &ownHwDevNum);
    ...
    /* check the DSA of packets that got to the CPU */
    copyToCpuCheck(prvTgfDevNum/*0*/,62/*portNum(NULL port)*/,2/*num packets*/);
    ...
    /* AUTODOC: configure analyzer on remote device. To analyzer packet will be dropped by EGF.
                But replication counter will be incremented */
    analyzerDev = (prvTgfDevNum + 1) % 128;
    if (analyzerDev == ownHwDevNum)
    {
        analyzerDev = (analyzerDev + 1) % 128;
    }

    prvMirrorAnalyzerInterfaceSet(dropConfig.analyzerIndex, analyzerDev, 0);
...
/**
* @internal prvTgfMirrorAndTunnelStartTestBridgeConfigSet function
* @endinternal
*
* @brief  Bridge config for vlan 5 traffic
*/
static void prvTgfMirrorAndTunnelStartTestBridgeConfigSet()
{
    GT_STATUS rc;
    ...
    /* Create VLAN 5 with untagged ports [0,1,2,3] */
    rc = prvTgfBrgDefVlanEntryWrite(PRV_TGF_VLANID_ANOTHER_CNS);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgDefVlanEntryWrite: %d", PRV_TGF_VLANID_ANOTHER_CNS);

    /*remove all, except ingress, remote ports from vlan 5*/
    rc = prvTgfBrgVlanMemberRemove(prvTgfDevNum, PRV_TGF_VLANID_ANOTHER_CNS, prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS]);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanMemberRemove: port %d", prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS]);
    rc = prvTgfBrgVlanMemberRemove(prvTgfDevNum, PRV_TGF_VLANID_ANOTHER_CNS, prvTgfPortsArray[PRV_TGF_RX_PORT2_IDX_CNS]);
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanMemberRemove: port %d", prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS]);
    rc = prvTgfBrgVlanPortVidSet(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], PRV_TGF_VLANID_ANOTHER_CNS);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanMemberRemove: port %d", prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS]);
}

/**
* @internal prvTgfMirrorAndTunnelStartTestMirrorPortConfigSet function
* @endinternal
*
* @brief  Rx Port Mirror Config
*/
static void prvTgfMirrorAndTunnelStartTestMirrorPortConfigSet
(
    GT_U32 testRun
)
{
    ...
    if(testRun == PRV_TGF_RX_MIRROR_AND_NO_TS)
    {
        /* AUTODOC: enable Rx mirroring on port=0, index=6 */
        rc = prvTgfMirrorRxPortGet(prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], &prvTgfRestoreCfg.rxMirrorEnable, &prvTgfRestoreCfg.rxMirrorIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfMirrorRxPortGet");
        rc = prvTgfMirrorRxPortSet(prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], GT_TRUE, PRV_TGF_MIRROR_INDEX_CNS);
        UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc, "prvTgfMirrorRxPortSet: %d %d %d",
                                     prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], GT_TRUE, PRV_TGF_MIRROR_INDEX_CNS);

        /* AUTODOC: set analyzer interface index=6: */
        /* AUTODOC:   analyzer devNum=0, eport = 1024 , phy port=2 */
        interface.interface.type = CPSS_INTERFACE_PORT_E;
        interface.interface.devPort.hwDevNum  = prvTgfDevNum;
        interface.interface.devPort.portNum   = prvTgfPortsArray[PRV_TGF_RX_PORT2_IDX_CNS];
        rc = prvTgfMirrorAnalyzerInterfaceGet(prvTgfDevNum, PRV_TGF_MIRROR_INDEX_CNS, &prvTgfRestoreCfg.rxAnalyzerInterface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceGet: %d", PRV_TGF_MIRROR_INDEX_CNS);
        rc = prvTgfMirrorAnalyzerInterfaceSet(PRV_TGF_MIRROR_INDEX_CNS, &interface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceSet: %d", PRV_TGF_MIRROR_INDEX_CNS);
    }
    else
    {
        /* AUTODOC: enable Tx mirroring on port=3, index=5 */
        rc = prvTgfMirrorTxPortGet(prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS], &prvTgfRestoreCfg.txMirrorEnable, &prvTgfRestoreCfg.txMirrorIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfMirrorRxPortGet");
        rc = prvTgfMirrorTxPortSet(prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS], GT_TRUE, PRV_TGF_MIRROR_TX_INDEX_CNS);
        UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc, "prvTgfMirrorRxPortSet: %d %d %d",
                                     prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS], GT_TRUE, PRV_TGF_MIRROR_TX_INDEX_CNS);

        /* AUTODOC: set analyzer interface index=6: */
        /* AUTODOC:   analyzer devNum=0, eport = 1024 , phy port=2 */
        interface.interface.type = CPSS_INTERFACE_PORT_E;
        interface.interface.devPort.hwDevNum  = prvTgfDevNum;
        interface.interface.devPort.portNum   = prvTgfPortsArray[PRV_TGF_RX_PORT_IDX_CNS];
        rc = prvTgfMirrorAnalyzerInterfaceGet(prvTgfDevNum, PRV_TGF_MIRROR_TX_INDEX_CNS, &prvTgfRestoreCfg.txAnalyzerInterface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceGet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
        rc = prvTgfMirrorAnalyzerInterfaceSet(PRV_TGF_MIRROR_TX_INDEX_CNS, &interface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceSet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
    }

    cpssOsMemSet(&tunnelEntry, 0, sizeof(tunnelEntry));
    cpssOsMemCpy(tunnelEntry.genCfg.macDa.arEther, prvTgfMirroredPacketTunnelL2Part.daMac,    sizeof(TGF_MAC_ADDR));
    cpssOsMemCpy(tunnelEntry.genCfg.data, prvTgfTunnelStartArr, sizeof(prvTgfTunnelStartArr));
    tunnelEntry.genCfg.etherType = TGF_ETHERTYPE_0800_IPV4_TAG_CNS;

    rc = prvTgfTunnelStartEntrySet(PRV_TGF_TUNNEL_START_IPV4_INDEX_CNS, CPSS_TUNNEL_GENERIC_E, &tunnelEntry);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Failure in prvTgfTunnelStartEntrySet ");
}

/**
* @internal prvTgfMirrorAndTunnelStartTestTtiConfigSet function
* @endinternal
*
* @brief  Rx Port TTI Config
*/
static void prvTgfMirrorAndTunnelStartTestTtiConfigSet()
{
    GT_STATUS rc;
    ...
    pattern.udbArray.udb[0] = 0x44;
    mask.udbArray.udb[0]    = 0xff;

    pattern.udbArray.udb[1] = 0x55;
    mask.udbArray.udb[1]    = 0xff;

    pattern.udbArray.udb[2] = 0x22;
    mask.udbArray.udb[2]    = 0xff;

    pattern.udbArray.udb[3] = 0x22;
    mask.udbArray.udb[3]    = 0xff;

    pattern.udbArray.udb[4] = 0x08;
    ...
    rc = prvTgfTtiUserDefinedByteSet(prvTgfDevNum, PRV_TGF_TTI_KEY_UDB_IPV4_OTHER_E, 4,
            PRV_TGF_TTI_OFFSET_METADATA_E, 24);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfTtiUserDefinedByteSet failed");
    ...
    ruleAction.egressInterface.devPort.portNum  = prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS];
    ruleAction.tunnelStart                       = GT_TRUE;
    ruleAction.tunnelStartPtr                    = PRV_TGF_TUNNEL_START_IPV4_INDEX_CNS;
    ruleAction.ingressPipeBypass                 = GT_TRUE;
    ruleAction.tsPassengerPacketType             = PRV_TGF_TUNNEL_PASSENGER_OTHER_E;

    rc = prvTgfTtiRule2Set(PRV_TGF_TTI_INDEX_CNS, PRV_TGF_TTI_KEY_UDB_IPV4_OTHER_E,
                          &pattern, &mask, &ruleAction);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfTtiRuleSet");
}

/**
* @internal prvTgfMirrorAndTunnelStartTestConfigurationSet function
* @endinternal
*
* @brief   Set test configuration:
*/
GT_VOID prvTgfMirrorAndTunnelStartTestConfigurationSet
(
    GT_U32 testRun
)
{
    /* Bridge config */
    prvTgfMirrorAndTunnelStartTestBridgeConfigSet();

    /* Mirror port config */
    prvTgfMirrorAndTunnelStartTestMirrorPortConfigSet(testRun);

    /* TTI config */
    prvTgfMirrorAndTunnelStartTestTtiConfigSet();
}

/**
* @internal prvTgfMirrorAndTunnelStartTestPacketValidate function
* @endinternal
*
* @brief Validate mirror packet
*/
static GT_VOID prvTgfMirrorAndTunnelStartTestPacketValidate
(
    GT_U32 testRun
)
{
    ...
    TGF_NET_DSA_STC                     rxParam;
    GT_BOOL                             getFirst = GT_TRUE;
    GT_U32                              pktCount = 0, ii = 0, pktParse = 0;

    for(pktCount = 0; pktCount < 1; pktCount++)
    {
        ...
        if(testRun == PRV_TGF_RX_MIRROR_AND_NO_TS || testRun == PRV_TGF_TX_MIRROR_AND_NO_TS)
        {
            /* MAC DA Verification */
            for (ii = 0; ii < 6; ii++)
            {
                UTF_VERIFY_EQUAL1_STRING_MAC(prvTgfPacketL2PartUC1.daMac[ii],
                        packetBuf[pktParse],
            ...
            for (ii = 0; ii < 5; ii++)
            {
                UTF_VERIFY_EQUAL1_STRING_MAC(prvTgfPacketL2PartUC1.saMac[ii],
                        packetBuf[pktParse],
        ...
        else
        {
            ...
            for (ii = 0; ii < 5; ii++)
            {
                UTF_VERIFY_EQUAL1_STRING_MAC(0x00,
                        packetBuf[pktParse],
                        "Packet SA is not matching at index[%d]", ii);
                pktParse++;
            }
...
/**
* @internal prvTgfMirrorAndTunnelStartTestTrafficGenerate function
* @endinternal
*
* @brief   Set test configuration:
*/
GT_VOID prvTgfMirrorAndTunnelStartTestTrafficGenerate
(
    GT_U32 testRun
)
{
    ...
    egressPortInterface.devPort.portNum   = (testRun == PRV_TGF_RX_MIRROR_AND_NO_TS) ? prvTgfPortsArray[PRV_TGF_RX_PORT2_IDX_CNS] :
    ...
    if(testRun == PRV_TGF_TX_MIRROR_AND_NO_TS)
    {
        rc = cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableGet(prvTgfDevNum, PRV_TGF_MIRROR_TX_INDEX_CNS, &prvTgfRestoreCfg.enable);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableGet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
        rc = cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableSet(prvTgfDevNum, PRV_TGF_MIRROR_TX_INDEX_CNS, GT_TRUE);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableSet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
    }

    /* setup transmit params */
    rc = prvTgfSetTxSetupEth(prvTgfDevNum, &prvTgfIPv4PacketInfoEnahancedMirrorTest, 1, 0, NULL);
    ...
    /*verify the Mirror packet*/
    prvTgfMirrorAndTunnelStartTestPacketValidate(testRun);
}

/**
* @internal prvTgfMirrorAndTunnelStartTestConfigurationRestore function
* @endinternal
*
* @brief   Restore test configuration
*/
GT_VOID prvTgfMirrorAndTunnelStartTestConfigurationRestore
(
    GT_U32 testRun
)
{
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgDefVlanEntryInvalidate: %d", PRV_TGF_VLANID_ANOTHER_CNS);
    ...
    rc = prvTgfTtiPortLookupEnableSet(
        prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], PRV_TGF_TTI_KEY_UDB_IPV4_OTHER_E, prvTgfRestoreCfg.ttiLookup);
    UTF_VERIFY_EQUAL1_STRING_MAC(
    ...
    if(testRun == PRV_TGF_TX_MIRROR_AND_NO_TS)
    {
        rc = cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableSet(prvTgfDevNum, PRV_TGF_MIRROR_TX_INDEX_CNS, GT_FALSE);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "cpssDxChMirrorTreatEgressMirroredAsIngressMirroredEnableSet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
    }

    if(testRun == PRV_TGF_RX_MIRROR_AND_NO_TS)
    {
        rc = prvTgfMirrorAnalyzerInterfaceSet(PRV_TGF_MIRROR_INDEX_CNS, &prvTgfRestoreCfg.rxAnalyzerInterface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceSet: %d", PRV_TGF_MIRROR_INDEX_CNS);
        rc = prvTgfMirrorRxPortSet(prvTgfPortsArray[PRV_TGF_TX_PORT_IDX_CNS], prvTgfRestoreCfg.rxMirrorEnable, prvTgfRestoreCfg.rxMirrorIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfMirrorRxPortSet");
    }
    else
    {
        rc = prvTgfMirrorAnalyzerInterfaceSet(PRV_TGF_MIRROR_TX_INDEX_CNS, &prvTgfRestoreCfg.txAnalyzerInterface);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfMirrorAnalyzerInterfaceSet: %d", PRV_TGF_MIRROR_TX_INDEX_CNS);
        rc = prvTgfMirrorTxPortSet(prvTgfPortsArray[PRV_TGF_LB_PORT_IDX_CNS], prvTgfRestoreCfg.txMirrorEnable, prvTgfRestoreCfg.txMirrorIndex);
...
/**
* @internal prvTgfFalconEgressVlanMirroringConfig function
* @endinternal
*
* @brief   Basic test to validate egress VLAN mirroring in FALCON with
*          egress VLAN attribute table entry write config, send traffic and
*          whether the analyzer port recieve packet.
*/
GT_VOID prvTgfFalconEgressVlanMirroringConfig
(
    GT_VOID
)
{
    ...
    /* AUTODOC: VLAN entry info */
    vlanInfo.mirrToTxAnalyzerEn = GT_TRUE;
    vlanInfo.mirrToTxAnalyzerIndex = analyzerIndex;
    ...
    /* AUTODOC: create VLAN 5 with tagged ports [0,1,2,3,4] */
    ...
    /* AUTODOC : analyzer port will be prvTgfPortsArray[4] , remove it from the tested vlan */
    rc = prvTgfBrgVlanMemberRemove(prvTgfDevNum,vlanId,prvTgfPortsArray[EXTRA_ANALYZER_PORT_INDEX_CNS]);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfBrgVlanMemberRemove: %d",
                                 vlanId);

    analyzerInterfaceInfo.interface.type = CPSS_INTERFACE_PORT_E;
    analyzerInterfaceInfo.interface.devPort.hwDevNum = prvTgfDevNum;
    analyzerInterfaceInfo.interface.devPort.portNum = prvTgfPortsArray[EXTRA_ANALYZER_PORT_INDEX_CNS];
    ...
    checkGotPackets(PRV_TGF_RCV_PORT_IDX_CNS, prvTgfBurstCount);
    checkGotPackets(EXTRA_ANALYZER_PORT_INDEX_CNS /* Analyzer port array index */, prvTgfBurstCount );

    /* AUTODOC: Restore VLAN, FDB entry and mirror config */
    prvTgfEgressVlanMirroringTest_configRestore();
}
