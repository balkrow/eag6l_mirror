    ...
    *expectedCounter = 1;
    ...
    switch (event)
    {
        ...
        case CPSS_PP_PORT_AN_PARALLEL_DETECT_E:
            ...
            break;
        case CPSS_PP_BM_AGED_PACKET_E:
            /* per tile event */
            switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
            {
                case CPSS_PP_FAMILY_DXCH_FALCON_E:  *expectedCounter = 3*32; break;
                case CPSS_PP_FAMILY_DXCH_AC5X_E:    *expectedCounter = 8; break;
                case CPSS_PP_FAMILY_DXCH_AC5P_E:    *expectedCounter = 48; break;
                case CPSS_PP_FAMILY_DXCH_HARRIER_E: *expectedCounter = 48; break;
                default: break;
            }

            break;
        case CPSS_PP_CNC_WRAPAROUND_BLOCK_E:
            *expectedCounter = PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.cncBlocks;
            break;
        case CPSS_PP_PORT_LANE_SQ_EVENT_E:
            *expectedCounter = prvCpssDxChHwInitNumOfSerdesGet(devNum);
            break;

        case CPSS_SRVCPU_SDK_WATCHDOG_E:
        case CPSS_SRVCPU_SDK_DOORBELL_E:
            /* interrupts per SCPU instance */
            *expectedCounter = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.mg.sip6MgNumOfUnits;
            switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
            {
                case CPSS_PP_FAMILY_DXCH_AC5X_E:
                case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
                    /* sip6MgNumOfUnits holds only units used for SDMA/AU but more exist for SCPU */
                    *expectedCounter = 3;
                    break;
                default: break;
            }
    ...
    if(isPerMac)
    {
        ...
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
        {
            numMacPorts = 80 * PRV_CPSS_PP_MAC(devNum)->multiPipe.numOfTiles;
        }
#ifndef ASIC_SIMULATION
        else
                ...
                if (PRV_CPSS_PP_MAC(devNum)->devFamily != CPSS_PP_FAMILY_DXCH_FALCON_E)
                {
                    ...
                    switch (PRV_CPSS_PP_MAC(devNum)->devFamily)
                    {
                        case CPSS_PP_FAMILY_DXCH_AAS_E:
                            numOfTsu = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.multiDataPath.maxDp;
                            numOfChannels = 64;
                            break;
                        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
                            numOfTsu = 1;
                    ...
                    *expectedCounter += (numOfTsu * numOfChannels * numOfQueues);
                }
            }
        }
        else
        ...
        switch (event)
        {
            ...
            case CPSS_PP_PORT_RX_FIFO_OVERRUN_E:
                ...
                {
                    if (event == CPSS_PP_PORT_TX_FIFO_OVERRUN_E ||
                        event == CPSS_PP_PORT_RX_FIFO_OVERRUN_E)
                        *expectedCounter *= 2;
                }
                break;
            default:
...
GT_VOID prvTgfGenEvent(GT_VOID)
{
            ...
            eventItr++)
    {
        ...
        if ((eventItr == (GT_U32)CPSS_PP_TQ_PORT_MICRO_BURST_E)||
            ...
            (eventItr == (GT_U32)CPSS_PP_BM_AGED_PACKET_E) ||
            (eventItr == (GT_U32)CPSS_PP_PORT_LANE_SQ_EVENT_E))
        {
             /* Simulation of Micro burst,headroom, PB aging, SERDES
                and critical interrupts is not supported */
            continue;
        }
#endif
        ...
        if ((event == CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E) || (event == CPSS_PP_PORT_LANE_SQ_EVENT_E))
        {
            if (portMng == GT_TRUE)
            {
                PRV_UTF_LOG1_MAC("prvTgfGenEvent - %s event restricted in port manager\n",uniEvName[eventItr]);
                continue;
            }
            ...
            {
                PRV_UTF_LOG0_MAC("prvTgfGenEvent - CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E event not supported in ASIC simulation");
                continue;
            }
#endif
        }
        ...
        for (evExtDataItr = 0; evExtDataItr < evExtDataSize; evExtDataItr++)
        {
            ...
            else
            {
                ...
                if((CPSS_PP_PORT_LINK_STATUS_CHANGED_E == event) &&
                   (portMng == GT_TRUE))
                {
                   /* port manager may consume CPU time on Link Change event.
                      provide additional time to make test stable. */
                   cpssOsTimerWkAfter(700);
                }
            }
...
)
{
    ...
    if (CPSS_PP_PORT_LINK_STATUS_CHANGED_E == uniEv /*non-PM mode (and in WM PM for link UP) */||
        CPSS_PP_PORT_LANE_SQ_EVENT_E == uniEv /* PM mode (in WM PM for link down) */) {
        if(CPSS_PP_PORT_LINK_STATUS_CHANGED_E == uniEv)
        {
            PRV_UTF_LOG2_MAC("SIGNAL STATE CHANGE for dev=%d port=%d\n", devNum, evExtData);
            physicalPortNum = evExtData;
        }
        else
        {
            physicalPortNum = 0;
            (void)cpssDxChPortPhysicalPortMapReverseMappingGet(devNum,
                CPSS_DXCH_PORT_MAPPING_TYPE_SERDES_E,
                evExtData,
                &physicalPortNum);
            PRV_UTF_LOG3_MAC("LANE_SQ_EVENT for dev=%d (evExtData=%d) port=%d\n",
                devNum, evExtData,physicalPortNum);
        }
        cpssOsSigSemWait(prvTgPortEventReceivedAckSem, 50); /* wait but not a long time */
        prvTgPortEventReceptorLastEvExtData = physicalPortNum;
...
)
{
    ...
    while (1)
    {
        st = cpssOsSigSemWait(prvTgPortEventReceivedSem, 10000); /* wait 10 sec for WM tests */
    ...
    for (i= 0; i< portCnt; i++)
    {
        ...
        for(forceLinkValue = 1 ; /* no criteria */ ; /* no modify*/ )
        {
            ...
            else
            {
                ...
                else
                {
                    if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
                    {
                        /* "force link up/down WA" does not generates link change interrupt on HW.
                           Use MAC/PCS loopback to get interrupt. */
                        st = cpssDxChPortInternalLoopbackEnableSet(devNum, currentPort, forceLinkValue ? GT_TRUE : GT_FALSE);
                        if (st != GT_OK)
                        {
                            PRV_UTF_LOG1_MAC("FAIL : cpssDxChPortInternalLoopbackEnableSet - port[%d]\n",
                                             currentPort);
                            goto unset_ev_cbk;
                        }
                    }
                ...
                if (forceLinkValue &&  !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
                {
...
GT_VOID prvTgfEventPortLinkStatusUnbindBind(GT_VOID)
{
    ...
    {
        evHndlrCauseAllArr[0] = CPSS_PP_PORT_LANE_SQ_EVENT_E;
        /* also unbind CPSS_PP_PORT_LANE_SQ_EVENT_E event */
        rc = cpssEventUnBind(evHndlrCauseAllArr, 1);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, rc);
    }
    ...
    if(GT_TRUE == prvUtfIsPortManagerUsed())
    {
        /* also bind CPSS_PP_PORT_LANE_SQ_EVENT_E event to the handler */
        rc = prvCpssDrvEvReqQUserHndlSet(CPSS_PP_PORT_LANE_SQ_EVENT_E, hnd);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, rc);
    }
...
)
{
    ...
    if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E) &&
        (portMapArrayCnt > 200))
    {
        /* reduce duration for test. 200 first ports checks all 4 tiles. */
        portMapArrayCnt = 200;
    }
