...
static void tcDpRemappingOnStackPort(IN GT_U32 testNumber)
{
    ...
    PRV_UTF_LOG1_MAC("=========== tcDpRemappingOnStackPort(%d) ==========\n", testNumber);
...
/* AUTODOC: 1. Configure vlan5 with ports 0,18,36,54
            ...
            3. Set a physical port 54 to be a cascade port for egress direction.
               ...
               Extended DSA tag qos mode set by API cpssDxChCosDscpToProfileMapSet modifying
               field "Port QoS Mode" of "Physical Port Attributes" table for all devices.
               For AAS devices run additional test case that sets the same mode from TTI Action instead.
            ...
            5. Enable physical egress port 54 for tc and dp remapping (set as stack port).
...
{   /* skip multi device boards for cascading related tests */
    ...
    {
        tcDpRemappingOnStackPort(3);/* test number 3 : tcdp remapping via TTI action for cascade port*/
    }
}
