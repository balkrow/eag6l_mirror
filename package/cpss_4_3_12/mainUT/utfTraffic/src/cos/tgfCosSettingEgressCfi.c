...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        cpssAction.redirect.redirectCmd = CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_OUT_IF_E;
        cpssAction.redirect.data.outIf.outInterface.type = CPSS_INTERFACE_PORT_E;
        cpssAction.redirect.data.outIf.outInterface.devPort.hwDevNum  = hwDevNum;
        cpssAction.redirect.data.outIf.outInterface.devPort.portNum =
            prvTgfPortsArray[PRV_TGF_EGR_PORT_IDX_CNS];
        cpssAction.redirect.data.outIf.tunnelStart = GT_TRUE;
        cpssAction.redirect.data.outIf.tunnelType =
            CPSS_DXCH_PCL_ACTION_REDIRECT_TUNNEL_TYPE_ETHERNET_E;
        cpssAction.redirect.data.outIf.tunnelPtr =
            prvTgfRouterArpTunnelStartLineIndex;
        cpssAction.bypassBridge = GT_TRUE;/* (sip5) must be set explicitly */
        ...
        devRuleIndex = prvWrAppDxChTcamPclConvertedIndexGet_fromUT(
            prvTgfDevNum, PRV_TGF_PCL_RULE_IDX_CNS, CPSS_DXCH_TCAM_RULE_SIZE_30_B_E);
