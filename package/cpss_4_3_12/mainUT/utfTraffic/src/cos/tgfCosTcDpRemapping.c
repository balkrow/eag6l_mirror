...
/**
* @internal prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfig function
* @endinternal
*
* @brief   Function configures TTI rule for Extened Qos Mapping Mode.
*/
static GT_VOID prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfig
(
    IN  GT_U32                  portIndex
)
{
    ...
    UTF_VERIFY_EQUAL1_STRING_MAC(
        GT_OK, rc, "prvTgfTtiPacketTypeKeySizeSet: %d", prvTgfDevNum);

    /* AUTODOC: enable TTI lookup for port 0, key TTI_KEY_ETH */
    rc = prvTgfTtiPortLookupEnableSet(
        prvTgfPortsArray[portIndex], PRV_TGF_TTI_KEY_UDB_ETHERNET_OTHER_E, GT_TRUE);
    ...
    ttiAction.qosMappingMode                = PRV_TGF_TTI_TC_DP_MODE_E;
    ttiAction.bridgeBypass                  = GT_TRUE;

    /* use empty rule matching all packets */

    ttiIndex = portIndex;

    rc = prvTgfTtiRuleSet(ttiIndex, PRV_TGF_TTI_RULE_ETH_E,
                          &ttiPattern, &ttiMask, &ttiAction);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfTtiRuleSet: %d", prvTgfDevNum);
}

/**
* @internal prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfigRestore function
* @endinternal
*
* @brief   Restore configuration of TTI rule for Extened Qos Mapping Mode
*/
static GT_VOID prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfigRestore
(
    IN  GT_U32                  portIndex
)
{
    ...
    rc = prvTgfTtiPortLookupEnableSet(
        prvTgfPortsArray[portIndex], PRV_TGF_TTI_KEY_UDB_ETHERNET_OTHER_E, GT_FALSE);
    UTF_VERIFY_EQUAL2_STRING_MAC(
        GT_OK, rc, "prvTgfTtiPortLookupEnableSet: %d %d", prvTgfDevNum, prvTgfPortsArray[portIndex]);

    rc = prvTgfTtiMacModeSet(PRV_TGF_TTI_KEY_ETH_E, prvTgfRestoreCfg.macMode);
    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc, "prvTgfTtiMacModeGet: %d", prvTgfDevNum);

    /* AUTODOC: invalidate port TTI rules */
    rc = prvTgfTtiRuleValidStatusSet(portIndex, GT_FALSE);
    UTF_VERIFY_EQUAL2_STRING_MAC(
        GT_OK, rc, "prvTgfTtiRuleValidStatusSet: %d, %d", prvTgfDevNum, portIndex);
...
)
{
    ...
    if (testNumber == 3)
    {
        ...
        /* AUTODOC: set extended DSA qos mode for packet from TTI Action */
        prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfig(
...
)
{
    ...
    if (testNumber == 1)
    {
        tcDpRemapping.isStack = CPSS_DXCH_PORT_PROFILE_NETWORK_E;
    }
    if ((testNumber == 2) || (testNumber == 3))
    {
        tcDpRemapping.isStack = CPSS_DXCH_PORT_PROFILE_CSCD_E;
    }
    tcDpRemapping.tc      = 3;
...
)
{
    ...
    if (testNumber == 3)
    {
        prvTgfCosTcDpRemappingOnStackExtenedQosMappingModeTtiRuleConfigRestore(
            PRV_TGF_SEND_PORT_IDX_CNS);
    }
