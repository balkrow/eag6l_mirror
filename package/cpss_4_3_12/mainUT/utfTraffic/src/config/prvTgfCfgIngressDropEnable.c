...
static PRV_TGF_BRG_DROP_CNTR_MODE_ENT   dropCntrMode;
...
)
{
    ...
    if(packetCmd == CPSS_PACKET_CMD_DROP_SOFT_E)
    {
        return;
    }
...
)
{
    ...
    if(packetCmd == CPSS_PACKET_CMD_DROP_SOFT_E)
    {
        /* AUTODOC save drop counter default mode */
        rc = prvTgfBrgCntDropCntrModeGet(&dropCntrMode);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Error of prvTgfBrgCntDropCntrModeGet");

        /* AUTODOC: set drop counter mode - drop with ALL */
        ...
        /* AUTODOC: PCL rule and action configuration */
        prvTgfCfgIngressDropEnableTestPclRulesAndActionsSet(CPSS_PACKET_CMD_DROP_SOFT_E);

        return;
    }

    prvTgfCfgIngressDropEnableTestPclRulesAndActionsSet(CPSS_PACKET_CMD_DROP_HARD_E);
...
)
{
    ...
    rc = prvTgfBrgCntDropCntrModeSet(dropCntrMode);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "Error of prvTgfBrgCntDropCntrModeSet");
}
/**
...
)
{
    ...
    if(packetCmd == CPSS_PACKET_CMD_DROP_SOFT_E)
    {
        /* Check SOFT dropped packet by TTI/PCL gets counted in bridge, get the bridge ingress drop counter */
        rc = prvTgfBrgCntDropCntrGet(&dropCount);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc,
                                "Error of prvTgfBrgCntDropCntrGet");
        UTF_VERIFY_EQUAL0_STRING_MAC(1, dropCount, "Expect drop count in bridge ");
        return;
    }
...
)
{
    ...
    prvTgfCfgIngressDropEnableTestConfigurationSet(GT_FALSE, GT_FALSE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestGenerateTraffic(GT_FALSE, GT_FALSE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestConfigurationRestore();

    /* Check for drop in EQ enable = GT_FALSE, enable egress PCL lookup */
    prvTgfCfgIngressDropEnableTestConfigurationSet(GT_FALSE, GT_TRUE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestGenerateTraffic(GT_FALSE, GT_TRUE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestConfigurationRestore();

    /* Check for drop in EQ enable = GT_TRUE */
    prvTgfCfgIngressDropEnableTestConfigurationSet(GT_TRUE, GT_FALSE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestGenerateTraffic(GT_TRUE, GT_FALSE, CPSS_PACKET_CMD_DROP_HARD_E);
    prvTgfCfgIngressDropEnableTestConfigurationRestore();

    /* Check bridge drop counter for SOFT drop packet from TTI/PCL , when dropCounterMode = "count ALL" */
    prvTgfCfgIngressDropEnableTestConfigurationSet(GT_TRUE, GT_FALSE, CPSS_PACKET_CMD_DROP_SOFT_E);
    prvTgfCfgIngressDropEnableTestGenerateTraffic(GT_TRUE, GT_FALSE, CPSS_PACKET_CMD_DROP_SOFT_E);
    prvTgfCfgIngressDropEnableTestConfigurationRestore();
}
