...
)
{
    ...
    ipfixEntry.lastCpuOrDropCode = CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E;
    /* set dummy entry in the ipfix */
    rc = prvTgfIpfixEntrySet(prvTgfDevNum,PRV_TGF_POLICER_STAGE_INGRESS_0_E,
        0/*entryIndex*/,&ipfixEntry);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfIpfixEntrySet: %d, %d",
    ...
    ingr0_second[0] = 0x5a2e7c4f;
    ingr0_second[1] = 0x5a2e7c4f;
    ...
    uploadSet.timer.nanoSecondTimer = 0x171f6528;
    ...
    UTF_VERIFY_EQUAL0_STRING_MAC(uploadSet.uploadMode, uploadGet.uploadMode,
                                 "Upload mode different than expected");

    UTF_VERIFY_EQUAL0_STRING_MAC(uploadSet.timer.nanoSecondTimer, uploadGet.timer.nanoSecondTimer,
                                 "Upload nanoSecond different than expected");

    UTF_VERIFY_EQUAL0_STRING_MAC(ingr0_second[0], uploadGet.timer.secondTimer.l[0],
                                 "Upload second.l[0] different than expected");

    UTF_VERIFY_EQUAL0_STRING_MAC(ingr0_second[1], uploadGet.timer.secondTimer.l[1],
                                 "Upload second.l[1] different than expected");
        ...
        timerGet.secondTimer.l[0] > (ingr0_second[0] + timeout_sec + 1))
    {
        UTF_VERIFY_EQUAL0_STRING_MAC((ingr0_second[0] + timeout_sec), timerGet.secondTimer.l[0],
                                     "ipfix second.l[0] different than expected");
    }
        ...
        timerGet.secondTimer.l[1] > (ingr0_second[1] + sec0_overflow + 1))
    {
        UTF_VERIFY_EQUAL0_STRING_MAC((ingr0_second[1] + sec0_overflow), timerGet.secondTimer.l[1],
                                     "ipfix second.l[1] different than expected");
    }

    /* read entry from the ipfix , so it will give it new timestamp due to the read */
    /* and then read the entry again to get this timestamp , and compare it to the 'Absolute time'*/
    rc = prvTgfIpfixEntryGet(prvTgfDevNum,PRV_TGF_POLICER_STAGE_INGRESS_0_E,
        0/*entryIndex*/,GT_TRUE/*reset*/,&ipfixEntry);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc, "prvTgfIpfixEntryGet: %d, %d",
                                 prvTgfDevNum, PRV_TGF_POLICER_STAGE_INGRESS_0_E);
    UTF_VERIFY_EQUAL0_STRING_MAC(0, ipfixEntry.timeStamp,
                                 "ipfix entry ipfixEntry.timestamp different than expected");
    ...
    if((ipfixEntry.timeStamp >> 8)/*8 bits seconds*/ -
        (timerGet.secondTimer.l[0] & 0xFF)/*8 bits seconds*/ > 1)/*should not be*/
    {
        UTF_VERIFY_EQUAL0_STRING_MAC((ipfixEntry.timeStamp >> 8), (timerGet.secondTimer.l[0] & 0xFF),
            "ipfix entry ipfixEntry.timestamp different than expected (diff more than 1)");
    }
