...
/**
********************************************************************************
* @file prvTgfIpv6HeaderFailCheck.c
*
...
static GT_BOOL ipHeaderServiceEnabled;
/******************************* Test packet **********************************/
...
)
{
    ...
    /* AUTODOC: Save the Ip header check state */
...
/**
* @internal prvTgfIpv6HeaderFailCheckGenerate function
* @endinternal
*
...
)
{
   ...
   /* AUTODOC: Case with IP header check enabled and correct header
    ...
    /* Exception counter get - no Ip Header fail */
    ...
    /* AUTODOC: Case with Ip Header check enabled and incorrect header */

    /* AUTODOC: NO Need to enable Unicast IPv6 Routing on sender port
                NO Need yo enable IPv6 Unicast Routing on sender Vlan
                IP Header Check should be done also on  non routed packets */

    /* AUTODOC: Disable Unicast IPv6 Routing on sender port */
    rc = prvTgfIpPortRoutingEnable(PRV_TGF_SEND_PORT_IDX_CNS,
    ...
    /* AUTODOC: Disable Unicast IPv6 Routing on sender vlan */
    rc = prvTgfIpVlanRoutingEnable(PRV_TGF_SEND_VLANID_CNS,
        CPSS_IP_UNICAST_E, CPSS_IP_PROTOCOL_IPV6_E, GT_FALSE);
    UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, rc,
        "prvTgfIpVlanRoutingEnable: vlanid=%d, enable=%d",
    ...
    /* AUTODOC: disable UC RPF fail check */
    ...
    UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, rc,
     ...
     /* AUTODOC: Enable the Ip header check state */
     prvTgfIpBridgeServiceEnable(prvTgfDevNum,
        PRV_TGF_IP_HEADER_CHECK_BRG_SERVICE_E,
        PRV_TGF_IP_BRG_SERVICE_IPV6_ENABLE_DISABLE_E,
        GT_TRUE);
   ...
   /* Transmit packet */
    ...
    /* AUTODOC: Case with Ip header check disabled and version!=6
       since we will not have Ip header fail we expect the packet to be bridged */

    /* AUTODOC: Disable the Ip header check state */
    prvTgfIpBridgeServiceEnable(prvTgfDevNum,
        PRV_TGF_IP_HEADER_CHECK_BRG_SERVICE_E,
        PRV_TGF_IP_BRG_SERVICE_IPV6_ENABLE_DISABLE_E,
        GT_FALSE);
    ...
    /* AUTODOC: Case with Ip header check disabled and correct version */
    /* Set back to version correct and see packet passes */
...
)
{
    ...
    /* AUTODOC: Restore the Ip header check state */
