...
)
{
    ...
    {
        lastVrId = PRV_TGF_LAST_VR_ID_IRONMAN_CNS;
    }
    else
    {
        lastVrId = PRV_TGF_LAST_VR_ID_CNS;
    }

    if ( vlanCount > PRV_TGF_VLANS_COUNT_CNS ||
         initialVrId + vlanCount - 1 > lastVrId)
    {
        return GT_BAD_PARAM;
    };
...
)
{
    ...
    if (initialVrId + burstCount - 1 > lastVrId)
    {
        return GT_BAD_PARAM;
    };
...
)
{
    ...
    for (initialVrId  = PRV_TGF_FIRST_VR_ID_CNS;
         initialVrId <= lastVrId;
         initialVrId += burstCount)
    {

        /* check if a burstCount is too big (it's possible on last iteration) */
        if (burstCount > lastVrId - initialVrId  + 1)
        {

            burstCount = lastVrId - initialVrId + 1;
        };
...
)
{
    ...
    for (vrId = PRV_TGF_FIRST_VR_ID_CNS; vrId <= lastVrId; vrId++)
    {
