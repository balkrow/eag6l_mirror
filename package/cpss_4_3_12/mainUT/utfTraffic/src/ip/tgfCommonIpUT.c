...
#include <ip/prvTgfIpv6UcRpfFailCheck.h>
#include <ip/prvTgfIpv6HeaderFailCheck.h>
...
GT_VOID prvTgfIpv4UcNatL2(GT_VOID);
...
UTF_TEST_CASE_MAC(tgfIpv4UcNatL2)
{
    /* Supported only on SIP6_30 devices */
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test IPv6 unicast routing RPF Fail mismatch check
1. Setup routing between two VLANs
2. Setup ARP and FDB entries
3. Check following cases when packet passes:
3.1 UC RPF Fail check enabled, NextHopVlanId correct
3.2 UC RPF Fail check disabled, NextHopVlanId correct
3.3 UC RPF Fail check disabled, NextHopVlanId incorrect
4. Check if packet dropped in cases when:
4.1. UC RPF Fail check enabled, NextHopVlanId incorrect
*/
UTF_TEST_CASE_MAC(tgfIpv6UcRpfFailCheck)
{
    prvTgfIpv6UcRpfFailCheckConfig();
    prvTgfIpv6UcRpfFailCheckGenerate();
    prvTgfIpv6UcRpfFailCheckRestore();
}

/*----------------------------------------------------------------------------*/
/* AUTODOC: Test IPv6 unicast routing Ip Header check
1. Setup routing between two VLANs
2. Setup ARP and FDB entries
3. Check following cases when packet passes:
3.1 Ip Header Fail check enabled, version correct
3.2 Ip Header check disabled, version correct
3.3 Ip Header check disabled, version incorrect
4. Check if packet dropped in cases when:
4.1. Ip Header check enabled, version incorrect
*/
UTF_TEST_CASE_MAC(tgfIpv6HeaderFailCheck)
{
    prvTgfIpv6HeaderFailCheckConfig();
    prvTgfIpv6HeaderFailCheckGenerate();
    prvTgfIpv6HeaderFailCheckRestore();
}


/*----------------------------------------------------------------------------*/
/* AUTODOC: Test Arp RPF Fail mismatch check
1. Setup routing between two VLANs
2. Setup ARP and FDB entries
3. Check following cases when packet passes:
3.1 UC RPF Fail check enabled, NextHopVlanId correct
...
4.1. UC RPF Fail check enabled, NextHopVlanId incorrect
*/
UTF_TEST_CASE_MAC(tgfIpArpRpfFailCheck)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E);

    prvTgfIpArpRpfFailCheckConfig();
    prvTgfIpArpRpfFailCheckGenerate();
    prvTgfIpArpRpfFailCheckRestore();
}
/*----------------------------------------------------------------------------*/
...
/* ##### TestCase 8: PBR = 2, PBR-LPM Priority = PBR, PBR Leaf Priority = FDB, Expect: FDB Route (ECMP) ###### */
    ...
    /* Restore configuration */
    prvTgfIpv4UcPbrLpmRoutingPriorityConfigRestore(prvUtfVrfId, GT_FALSE /*fdbEcmpTest*/);

}

/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(tgfIpv4UcPbrLpmRoutingPriorityFdbEcmp)
{
    ...
    5. Set Generic FDB Route configuration
    6. Set FDB Route configuration
    7. Set pbr leaf and lpm leaf configuration.
    8. Generate traffic and compare expected result.
    9. Restore configuration
    */
...
/* ##### TestCase: PBR = 2, PBR-LPM Priority = PBR, PBR Leaf Priority = FDB, Expect: FDB Route (ECMP) ###### */
    /* Set LPM leaf priority*/
    prvTgfIpv4UcPbrLpmRoutingPriorityLpmLeafEntrySet(prvUtfVrfId, CPSS_DXCH_LPM_LEAF_ENTRY_PRIORITY_FDB_E,
                                                     GT_TRUE /*applyPbr*/,
                                                     GT_FALSE /*deleteExisted*/, GT_TRUE /*isEcmpTest*/,
    ...
    /* Restore configuration */
    prvTgfIpv4UcPbrLpmRoutingPriorityConfigRestore(prvUtfVrfId, GT_TRUE /*fdbEcmpTest*/);
}
...
UTF_TEST_CASE_MAC(prvTgfIpLpmHaCheck)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum,  UTF_XCAT3_E|UTF_AC5_E|UTF_BOBCAT2_E | UTF_CAELUM_E|
    ...
    if((PRV_CPSS_SIP_6_20_CHECK_MAC(prvTgfDevNum))||
       (PRV_CPSS_SIP_6_15_CHECK_MAC(prvTgfDevNum)))
    {
        totalNumIpUc=10198;
    }
    else
    ...
    prvTgfIpLpmHaValidityCheckAndTrafficSend(0,prvTgfPortsArray[3],GT_TRUE);
    prvTgfIpLpmHaSystemRecoveryStateSet();
    prvTgfIpLpmHaDelBasicConfig(totalNumIpUc);
    /* change state to catchup */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E);
    prvTgfIpLpmHaAddConfigAfterDelete();
    prvTgfIpLpmHaAddBasicConfig(totalNumIpUc,0,prvTgfPortsArray[3],GT_FALSE,GT_FALSE);
    /* change state to complition */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E);
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test IP LPM HA after fail due to wrong replay:
    1.configure VLAN, FDB entries;
    configure ipv4/v6 UC/MC Routing entries;
    send IPv4/IPv6 UC/MC traffic and verify expected packet Ip counters;

    2.Set HA flag
    Delete all entries, VR, LPM - deleted from SW only
    Configure again the VR, do not add any LPM prefixes.
    call prvCpssDxChLpmRamSyncSwHwForHa();
    Run validity.
    send IPv4/IPv6 UC/MC traffic and verify NO traffic;

    3.Set HA flag
    Delete all entries, VR, LPM - deleted from SW only
    Configure again the VR, add only 10 LPM prefixes.
    call prvCpssDxChLpmRamSyncSwHwForHa();
    Run validity.
    send IPv4/IPv6 UC/MC traffic and verify NO traffic;
    ...
    5.Set HA flag
    Delete all entries, VR, LPM - deleted from SW only
    Configure again the VR, all prefixes like in step 1 plus one exta entry.
    call prvCpssDxChLpmRamSyncSwHwForHa();
    Run validity.
    send IPv4/IPv6 UC/MC traffic and verify expected packet Ip counters;

*/
UTF_TEST_CASE_MAC(prvTgfIpLpmHaCheckFail)
{
    GT_U32 totalNumIpUc;
    GT_U32 totalNumIpUc_more;
    GT_U32 totalNumIpUc_less;
    GT_U32 startTime;
    GT_U32 endTime;
    GT_U32 notAppFamilyBmp;
    ...
    /* this test is supported only for sip6 and above */
    if(GT_FALSE == PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
    {
        prvUtfSkipTestsSet();
        return;
    }
    ...
    /* we do not want to get to full capacity with defrag */
    if((PRV_CPSS_SIP_6_20_CHECK_MAC(prvTgfDevNum))||
       (PRV_CPSS_SIP_6_15_CHECK_MAC(prvTgfDevNum)))
    {
        totalNumIpUc=400;
    ...
    prvTgfIpLpmHaAddBasicConfig(totalNumIpUc,0,prvTgfPortsArray[3],GT_TRUE,GT_FALSE);
    prvTgfIpLpmHaValidityCheckAndTrafficSend(0,prvTgfPortsArray[3],GT_TRUE);
    prvTgfIpLpmHaSystemRecoveryStateSet(); /* set HA process init state */
    prvTgfIpLpmHaDelBasicConfig(totalNumIpUc);/* delete prefixes from shadow but not from the HA */
    /* change state to catchup */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E);
    prvTgfIpLpmHaAddConfigAfterDelete();/* create LPM DB, Add device list and Add VR */
    /* do not replay any prefixes */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E);
    prvTgfIpLpmHaSyncSwHw(); /* we will get a fail in the sync but new logic will overcome
                                the fail and update the HW according to existing SW*/
    /* expect no traffic - there are no prefixes */
    prvTgfIpLpmHaValidityCheckAndTrafficSend(0,prvTgfPortsArray[3],GT_FALSE);

    prvTgfIpLpmHaSystemRecoveryStateSet();
    prvTgfIpLpmHaDelBasicConfig(0);/* no prefixes were added */
    prvTgfIpLpmHaAddConfigAfterDelete();/* create LPM DB, Add device list and Add VR */
    /* replay only part of the prefixes */
    totalNumIpUc_less = 10;
    /* change state to catchup*/
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E);
    prvTgfIpLpmHaAddBasicConfig(totalNumIpUc_less,0,prvTgfPortsArray[3],GT_TRUE,GT_FALSE);
    /* change state to complition */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E);
    prvTgfIpLpmHaSyncSwHw(); /* we will get a fail in the sync but new logic will overcome
                                the fail and update the HW according to existing SW*/
    /* still expect no traffic since the needded prefix was not added */
    prvTgfIpLpmHaValidityCheckAndTrafficSend(0,prvTgfPortsArray[3],GT_FALSE);

    prvTgfIpLpmHaSystemRecoveryStateSet();
    prvTgfIpLpmHaDelBasicConfig(totalNumIpUc_less);
    /* change state to catchup*/
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E);
    prvTgfIpLpmHaAddConfigAfterDelete();/* create LPM DB, Add device list and Add VR */
    /* replay the prefixes and one more */
    totalNumIpUc_more = totalNumIpUc + 1;
    prvTgfIpLpmHaAddBasicConfig(totalNumIpUc_more,0,prvTgfPortsArray[3],GT_TRUE,GT_FALSE);
     /* change state to complition */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E);
    prvTgfIpLpmHaSyncSwHw(); /* we will get a fail in the sync but new logic will overcome
                                the fail and update the HW according to existing SW*/
    /* expect traffic since the needded prefix was added */
    prvTgfIpLpmHaValidityCheckAndTrafficSend(0,prvTgfPortsArray[3],GT_TRUE);

    prvTgfIpLpmHaSystemRecoveryStateSet();
    prvTgfIpLpmHaDelBasicConfig(totalNumIpUc_more);/* delete prefixes from shadow but not from the HA */
    /* change state to catchup*/
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E);
    prvTgfIpLpmHaAddConfigAfterDelete(); /* replay the prefixes as in the beggining of the test */
    prvTgfIpLpmHaAddBasicConfig(totalNumIpUc,0,prvTgfPortsArray[3],GT_FALSE,GT_FALSE);
    /* change state to complition */
    prvTgfIpLpmHaSystemRecoveryStateUpdate(CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E);
    prvTgfIpLpmHaSyncSwHw();
...
UTF_TEST_CASE_MAC(prvTgfIpLpmHaBulkCheck)
{
    ...
    prvTgfIpLpmHaAddConfigAfterDelete();
    startTime2 = cpssOsTime();
    prvTgfIpLpmHaBulkAddBasicConfig(totalNumIpUc,0,prvTgfPortsArray[3],GT_FALSE,GT_TRUE);
    endTime2 = cpssOsTime();
...
UTF_TEST_CASE_MAC(tgfIpv4UcRoutingNonExistedVrf)
{
    GT_U32              prvTgfVrfId = 0;

    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E|
                                        UTF_BOBCAT2_E|UTF_XCAT3_E|UTF_BOBCAT3_E|UTF_CAELUM_E|UTF_AC3X_E|
                                        UTF_ALDRIN_E|UTF_ALDRIN2_E);

    UTF_SIP6_GM_NOT_READY_SKIP_MAC("Test PASS HW ,but skipped since GM fails: CPSS-14391");

    prvTgfIpvxUcRoutingNonExistedVrfConfigSet(prvTgfVrfId);

    prvTgfIpvxUcRoutingNonExistedVrfPclConfigSet();

    prvTgfIpvxUcRoutingNonExistedVrfIpConfigSet(prvTgfVrfId);

    prvTgfIpvxUcRoutingNonExistedVrfTrafficGenerate(GT_TRUE /* isIpv4 */);

    prvTgfIpvxUcRoutingNonExistedVrfConfigRestore(prvTgfVrfId);
}

UTF_TEST_CASE_MAC(tgfIpv6UcRoutingNonExistedVrf)
{
    GT_U32              prvTgfVrfId = 0;
    ...
    PRV_TGF_SKIP_FAILED_TEST_MAC(UTF_IRONMAN_L_E | UTF_AAS_E, "CPSS-14497");
    ...
    prvTgfIpvxUcRoutingNonExistedVrfTrafficGenerate(GT_FALSE /* isIpv4 */);
...
UTF_TEST_CASE_MAC(prvTgfIpvx4kVrfsAddDelete)
{
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E|
                                        ...
                                        UTF_ALDRIN_E|UTF_ALDRIN2_E||UTF_AC5X_E | UTF_HARRIER_E|UTF_IRONMAN_L_E | UTF_AAS_E);
...
UTF_TEST_CASE_MAC(prvTgfIpTtlHopLimitPhaVerify)
{
    ...
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_CAELUM_E |
        UTF_ALDRIN_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_IRONMAN_L_E | UTF_AAS_E);
...
UTF_TEST_CASE_MAC(tgfIpv4UcRoutingAddManyByOctetVsByBulk)
{
    ...
    /* The test allocate huge memory for the bulk, which causes Out Of CPU Mem
       So, In shared Lib we'll skip it*/
    SKIP_TEST_MAC
#endif
...
/* AUTODOC: Test Ipv4 regular and bulk entries.
   add non full match and full match prefixes
   use regular add and bulk add
   run validity
*/
UTF_TEST_CASE_MAC(prvTgfIpv4AddRegularAndBulkEntries)
{
/*
    1. Add prefixes
        1.1.1.0/24 using bulk
        1.1.1.1/32 using regular add
        2.1.1.1/32 using regular add
        2.1.1.0/24 using bulk
    2. run validity
    3. Delete prefixes added
    2. run validity
*/
    /* Set Base configuration */
    prvTgfIpv4UcRouteConfigSet();

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* Restore configuration */
    prvTgfIpv4UcRouteConfigRestore();

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
}

/* AUTODOC: Test Ipv4 Add and Updtae Regular/ECMP bulk entries.
   add non full match and full match prefixes
   use regular and ECMP entries with bulk operation
   run validity
*/
UTF_TEST_CASE_MAC(prvTgfIpv4AddAndUpdateRegularAndEcmpBulkEntries)
{
/*
    1. Add ipv4 prefixes
        using bulk with nextHop=3
        overide using bulk with nextHop=4
        overide using bulk with ECMP index 5
    2. run validity after each operation in stage 1
    3. Delete prefixes added
    4. run validity

    Do it all again with different prefixes
    set isBe = GT_TRUE
*/
    GT_U32      notAppFamilyBmp;
    ...
    /* Set Base configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(0,GT_FALSE,GT_TRUE,GT_FALSE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
    /* update NextHop retular value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(1,GT_FALSE,GT_TRUE,GT_FALSE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* update NextHop to be ECMP value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(2,GT_FALSE,GT_TRUE,GT_FALSE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* Restore configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigRestore(GT_FALSE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

     /* Set Base configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(0,GT_FALSE,GT_TRUE,GT_TRUE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
    /* update NextHop retular value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(1,GT_FALSE,GT_TRUE,GT_TRUE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* update NextHop to be ECMP value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(2,GT_FALSE,GT_TRUE,GT_TRUE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
...
/* AUTODOC: Test Ipv6 Add and Updtae Regular/ECMP bulk entries.
   add non full match and full match prefixes
   use regular and ECMP entries with bulk operation
   run validity
*/
UTF_TEST_CASE_MAC(prvTgfIpv6AddAndUpdateRegularAndEcmpBulkEntries)
{
/*
    1. Add ipv6 prefixes
        using bulk with nextHop=6
        overide using bulk with nextHop=5
        overide using bulk with ECMP index 1
    2. run validity after each operation in stage 1
    3. Delete prefixes added
    4. run validity
...
*/
    ...
    /* Set Base configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(0,GT_TRUE,GT_TRUE,GT_FALSE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
    /* update NextHop retular value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(1,GT_TRUE,GT_TRUE,GT_FALSE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* update NextHop to be ECMP value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(2,GT_TRUE,GT_TRUE,GT_FALSE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* Restore configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigRestore(GT_TRUE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

       /* Set Base configuration */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(0,GT_TRUE,GT_TRUE,GT_TRUE);

    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
    /* update NextHop retular value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(1,GT_TRUE,GT_TRUE,GT_TRUE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();

    /* update NextHop to be ECMP value */
    prvTgfIpv4Ipv6UcAddRegularEcmpBulkEntriesConfigSet(2,GT_TRUE,GT_TRUE,GT_TRUE);
    /* validity check */
    prvTgfIpv4Ipv6ValidityCheck();
...
/* AUTODOC: Test Ipv4 & IPv6 unicast routing - random.
   adding and deleting prefixes untill the LPM is full.
   Add random prefixes
   Fill the LPM with ipv4/ipv6 UC prefixes
   Delete all ipv4/ipv6 prefixes
   Do it in a loop acording to parameters
*/
UTF_TEST_CASE_MAC(prvTgfIpv4Ipv6UcRoutingMultipleAddAndFlush)
{
/*
    1. Add random prefixes
    2. Fill the LPM with ipv4/ipv6 UC prefixes
    3. Delete all ipv4/ipv6 prefixes
    4. Do it in a loop acording to parameters

*/
    GT_STATUS   rc=GT_OK;
    GT_BOOL     defragEnable;
    GT_U32      addedNumIpv4=0,maxAddedNumIpv4=0;
    GT_U32      addedNumIpv6=0,maxAddedNumIpv6=0;
    GT_U32      numberOfLoops=0,baselineNumber=0;
    GT_U32      idx;

    /* Supported only falcon */
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E|
                                        UTF_BOBCAT2_E|UTF_BOBCAT3_E|UTF_CAELUM_E|UTF_AC3X_E|UTF_ALDRIN_E|UTF_ALDRIN2_E);
    ...
    /* sip7 configuration device can not fit in 20 small banks configuration - TBD find a way to test this senario */
    if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) == GT_FALSE)
    {
        rc = prvWrAppDbEntryAdd("sharedTableMode", 1);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvWrAppDbEntryAdd(sharedTableMode)");
        ...
        /* get number of loops to run accordig to baseline number */
        baselineNumber = prvUtfSeedFromStreamNameGet();
        /* set number of loops to run */
        numberOfLoops = baselineNumber%30 + 1;

        /* first time we run the test to fill the LPM and to find the max prefix number added with defragEnable=GT_TRUE */
        defragEnable=GT_TRUE;
        rc = prvTgfIpLpmIpv4Ipv6UcPrefixAddMaxPrefix(defragEnable,&maxAddedNumIpv4,&maxAddedNumIpv6);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmIpv4Ipv6UcPrefixAddMaxPrefix");

        /* we run the test in a loop according to baseline */
        for (idx = 0; idx < numberOfLoops; idx++)
        {
            if (idx%2==0)
            {
                /* now we call the test to add 70% of ipv4 and 60% of ipv6 */
                addedNumIpv4 = maxAddedNumIpv4 * 0.7;
                addedNumIpv6 = maxAddedNumIpv6 * 0.6;
            }
            else
            {
                /* now we call the test to add 60% of ipv4 and 70% of ipv6 */
                addedNumIpv4 = maxAddedNumIpv4 * 0.6;
                addedNumIpv6 = maxAddedNumIpv6 * 0.7;
            }

            /* Flush all prefix first */
            rc = prvTgfIpLpmFlushUcPrefixes((GT_BOOL)(idx%2));
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes fail ");
            rc = prvTgfIpLpmFlushUcPrefixes((GT_BOOL)((idx+1)%2));
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes fail ");

            rc = prvTgfIpLpmAddUcPrefixes((GT_BOOL)(idx%2), defragEnable, (idx%2 == 0)?addedNumIpv4:addedNumIpv6);
            if(rc != GT_OK)
            {
                /* GT_OUT_OF_PP_MEM is a valid fail */
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_PP_MEM, rc, "prvTgfIpLpmAddUcPrefixes fail");
            }
            else
            {
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmAddUcPrefixes");
            }

            rc = prvTgfIpLpmAddUcPrefixes((GT_BOOL)((idx+1)%2), defragEnable, ((idx+1)%2 == 0)?addedNumIpv4:addedNumIpv6);
            if (rc != GT_OK)
            {
                /* GT_OUT_OF_PP_MEM is a valid fail */
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_PP_MEM, rc, "prvTgfIpLpmAddUcPrefixes fail");
            ...
            cpssOsPrintf("Test done %d\n", idx);
        }

        /* clean LPM */
        rc = prvTgfIpLpmFlushUcPrefixes(GT_TRUE);/* flush ipv6 */
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes ipv6 fail");

        rc = prvTgfIpLpmFlushUcPrefixes(GT_FALSE);/* flush ipv4 */
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes ipv4 fail");

        /* we run the test to fill the LPM and to find the max prefix number added with defragEnable=GT_FALSE */
        defragEnable=GT_FALSE;
        rc = prvTgfIpLpmIpv4Ipv6UcPrefixAddMaxPrefix(defragEnable,&maxAddedNumIpv4,&maxAddedNumIpv6);
        if (rc != GT_OK)
        {
            /* GT_OUT_OF_PP_MEM is a valid fail */
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_PP_MEM, rc, "prvTgfIpLpmIpv4Ipv6UcPrefixAddMaxPrefix fail");
        }
        else
        {
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmIpv4Ipv6UcPrefixAddMaxPrefix");
        ...
        for (idx = 0; idx < numberOfLoops; idx++)
        {
            ...
            /* Flush all prefix first */
            rc = prvTgfIpLpmFlushUcPrefixes((GT_BOOL)(idx%2));
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prv_FLCN_LpmUcPrefixFlush_CPSS");

            rc = prvTgfIpLpmFlushUcPrefixes((GT_BOOL)((idx+1)%2));
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes");
    ...
    {
        cpssOsPrintf("SIP7 configuration device can not fit in 20 small banks configuration - TBD find a way to test this senario\n");
    }
}
/* AUTODOC: Test Ipv4 & IPv6 unicast routing - random.
   adding and deleting prefixes using bulk untill the LPM is full.
   Add random prefixes
   Fill the LPM with ipv4/ipv6 UC prefixes using bulk
   Delete all ipv4/ipv6 prefixes using bulk
   Do it in a loop acording to parameters
*/
UTF_TEST_CASE_MAC(prvTgfIpv4Ipv6UcRoutingMultipleBulkAndFlush)
{
/*
    1. Add random prefixes using bulk
    2. Fill the LPM with ipv4/ipv6 UC prefixes using bulk
    3. Delete all ipv4/ipv6 prefixes using bulk
    4. Do it in a loop acording to parameters
...
*/
    ...
    GT_U32      numberOfLoops=0,baselineNumber=0;
    GT_U32      idx=0;
    GT_U32      ulRnd = 0, x = 0, y = 5;
    GT_U32      ulDelBitmap = 0;
    ...
    /* sip7 configuration device can not fit in 28 big banks and 2 small banks configuration */
    if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) == GT_FALSE)
    {
        rc = prvWrAppDbEntryAdd("sharedTableMode", 0);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvWrAppDbEntryAdd(sharedTableMode)");
        ...
        baselineNumber = prvUtfSeedFromStreamNameGet();
        /* set number of loops to run */
        numberOfLoops = baselineNumber%4 + 1;
        ...
        while (numberOfLoops)
        {
            ...
            prvTgfIpLpmAddIpv6UcEntriesbyPrefixStreamIncrBulk(0x1F,addedNumIpv6);
            prvTgfIpLpmAddIpv4UcEntriesbyPrefixStreamIncrBulk(0x1F,addedNumIpv4);
            ...
            switch (ulRnd)
            {
                ...
                case 4 :
                    ...
                    break;
                default :
             		cpssOsPrintf("rand not expected\n");
                    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, GT_FAIL, "rand not expected");
            }
            if (ulDelBitmap >= 0x4)
            {
                prvTgfIpLpmAddIpv4UcEntriesbyPrefixStreamIncrBulkDel(ulDelBitmap,addedNumIpv4);
                prvTgfIpv4Ipv6UcMcValidityCheck();

                prvTgfIpLpmAddIpv6UcEntriesbyPrefixStreamIncrBulkDel(ulDelBitmap,addedNumIpv6);
                prvTgfIpv4Ipv6UcMcValidityCheck();
            }
            else
            {
    ...
    {
        cpssOsPrintf("SIP7 configuration device can not fit in 28 big banks and 2 small banks configuration - TBD find a way to test this senario\n");
    }
}

/* AUTODOC: Test IPv6 multicast routing - random.
   adding and deleting prefixes using untill the LPM is full.
   Add random prefixes
   Fill the LPM with ipv6 MC prefixes
   Delete all ipv6 prefixes
   Do it in a loop acording to parameters
*/
UTF_TEST_CASE_MAC(prvTgfIpv6McRoutingMultipleAddAndFlush)
{
/*
    1. Add random prefixes
    2. Fill the LPM with ipv6 MC prefixes
...
*/
    GT_STATUS   rc=GT_OK;
    GT_BOOL     defragEnable;
    GT_U32      maxAddedNumIpv6=0,maxAddedNumIpv4=0;
    GT_U32      numberOfLoops=0,baselineNumber=0;
    GT_U32      idx;
    ...
    /* sip7 configuration device can not fit in 20 small banks configuration */
    if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) == GT_FALSE)
    {
        rc = prvWrAppDbEntryAdd("sharedTableMode", 1);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvWrAppDbEntryAdd(sharedTableMode)");
        ...
        /* first time we run the test to fill the LPM and to find the max prefix number added with defragEnable=GT_TRUE */
        defragEnable=GT_TRUE;
        rc = prvTgfIpLpmIpv4Ipv6McPrefixAddMaxPrefix(defragEnable,&maxAddedNumIpv4,&maxAddedNumIpv6);
        if (rc!=GT_OK)
        {
            cpssOsPrintf("prvTgfIpLpmIpv4Ipv6McPrefixAddMaxPrefix return %d\n", rc);
        }
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmIpv4Ipv6McPrefixAddMaxPrefix");

        /* we run the test in a loop according to baseline */
        for (idx = 0; idx < numberOfLoops; idx++)
        {
            /* Flush all ipv6 MC prefix first */
            rc = prvTgfIpLpmFlushMcPrefixes(GT_TRUE);
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushMcPrefixes fail ");

            if (rc!=GT_OK)
            {
    			cpssOsPrintf("Test fail on prvTgfIpLpmFlushMcPrefixes idx= %d,rc=%d\n", idx,rc);
                break;
            }
            rc = prvTgfIpLpmAddMcPrefixes(GT_TRUE, defragEnable, maxAddedNumIpv6);
            if(rc!=GT_OK) {
                /* GT_OUT_OF_PP_MEM is a valid fail */
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_PP_MEM, rc, "prvTgfIpLpmAddMcPrefixes fail");
                if (rc!=GT_OUT_OF_PP_MEM)
                {
    				cpssOsPrintf("Test fail on prvTgfIpLpmAddMcPrefixes idx= %d,rc=%d\n", idx,rc);
                    break;
                }
            }
            else
            {
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmAddMcPrefixes");
            }
        ...
        /* clean LPM */
        rc = prvTgfIpLpmFlushMcPrefixes(GT_TRUE);/* flush ipv6 */
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushMcPrefixes ipv6 fail");
...
/* AUTODOC: Test IPv4 multicast routing - random.
   adding and deleting prefixes using untill the LPM is full.
   Add random prefixes
   Fill the LPM with ipv4 MC prefixes
   Delete all ipv4 prefixes
   Do it in a loop acording to parameters
*/
UTF_TEST_CASE_MAC(prvTgfIpv4McRoutingMultipleAddAndFlush)
{
/*
    1. Add random prefixes
    2. Fill the LPM with ipv4 MC prefixes
...
*/
    ...
    if(PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum) == GT_FALSE)
    {
        ...
        for (idx = 0; idx < numberOfLoops; idx++)
        {
            /* Flush all ipv4 MC prefix first */
            rc = prvTgfIpLpmFlushMcPrefixes(GT_FALSE);
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushUcPrefixes fail ");
            ...
            rc = prvTgfIpLpmAddMcPrefixes(GT_FALSE, defragEnable, maxAddedNumIpv4);
            if(rc!=GT_OK) {
                /* GT_OUT_OF_PP_MEM is a valid fail */
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_PP_MEM, rc, "prvTgfIpLpmAddMcPrefixes fail");
                if (rc!=GT_OUT_OF_PP_MEM)
        ...
        /* clean LPM */
        rc = prvTgfIpLpmFlushMcPrefixes(GT_FALSE);/* flush ipv4 */
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpLpmFlushMcPrefixes ipv4 fail");
...
/*
    ...
    3. Restore configuration
*/
    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum,  (~UTF_CPSS_PP_ALL_SIP6_10_CNS) | UTF_IRONMAN_L_E | UTF_AAS_E );
...
/*----------------------------------------------------------------------------*/
/* AUTODOC: Test FORWARD DSA Route Clear PHA Thread :
    configure VLAN, FDB entries;
    configure UC Routing entries;
    add IPv4 UC prefix to LPM DB with exact packet's DIP match;
    add IPv6 UC prefix to LPM DB with exact packet's DIP match;
    send IPv4 UC traffic and verify expected packet DSA tag context and PHA trigger;
    Verify:
    Packet#1 : EPCL rule no match, no PHA trigger so FORWARD DSA Route field is left set
    Packet#2 : EPCL rule match, PHA triggered so FORWARD DSA Route field gets cleared


*/
UTF_TEST_CASE_MAC(prvTgfForwardDsaRouteClearPhaVerify)
{

    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, UTF_XCAT3_E | UTF_AC5_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_CAELUM_E |
        UTF_ALDRIN_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);

    /* Test uses PHA feature but GM does not support PHA, therefore skip test */
    GM_NOT_SUPPORT_THIS_TEST_MAC

    prvTgfForwardDsaRouteClearPhaConfig();

    prvTgfForwardDsaRouteClearPhaTrafficGenerate();

    prvTgfForwardDsaRouteClearPhaRestore();
}
...
UTF_SUIT_PBR_BEGIN_TESTS_MAC(tgfIp)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv4UcNatL2)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv6UcRpfFailCheck)
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv6HeaderFailCheck)
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpArpRpfFailCheck)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv4UcPbrLpmRoutingPriorityFdbEcmp)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv4UcRoutingNonExistedVrf)
    UTF_SUIT_DECLARE_TEST_MAC(tgfIpv6UcRoutingNonExistedVrf)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpLpmHaCheckFail)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv4AddRegularAndBulkEntries)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv4AddAndUpdateRegularAndEcmpBulkEntries)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv6AddAndUpdateRegularAndEcmpBulkEntries)

    /* test to check random scenarios */

    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv4Ipv6UcRoutingMultipleAddAndFlush)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv4Ipv6UcRoutingMultipleBulkAndFlush)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv6McRoutingMultipleAddAndFlush)
    UTF_SUIT_DECLARE_TEST_MAC(prvTgfIpv4McRoutingMultipleAddAndFlush)
	UTF_SUIT_DECLARE_TEST_MAC(prvTgfForwardDsaRouteClearPhaVerify)
