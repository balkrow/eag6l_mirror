...
/* L2NAT configs orig */
static PRV_TGF_IP_NAT_L2_PORT_CONFIG_STC   l2NatPortCfgOrig;
static PRV_TGF_IP_NAT_L2_GLOBAL_CONFIG_STC l2NatGlobalCfgOrig;
...
/*********************Packet definitions for L2NAT*****************************/

#define PRV_TGF_VLANID_1_CNS 2
...
/* packet's IPv4 */
static TGF_PACKET_IPV4_STC prvTgfPacketIpv4Part1 =
{
    4,                                  /* version                */
    5,                                  /* headerLen              */
    0,                                  /* typeOfService          */
...
static TGF_PACKET_PART_STC prvTgfPacketPartArray1[] =
{
    ...
    {TGF_PACKET_PART_IPV4_E,      &prvTgfPacketIpv4Part1},    /* packet's IPv4 */
    {TGF_PACKET_PART_PAYLOAD_E,   &prvTgfPacketPayloadPart1}  /* PAYLOAD part  */
};
...
)
{
    ...
    /* SIP and DIP prefix length configuration is
     * valid only for Ironman device */
    if (PRV_CPSS_SIP_6_30_CHECK_MAC(prvTgfDevNum))
    {
        /* For L3NAT the SIP and DIP prefix lengths should be 32 */
        natEntry.nat44Entry.sipPrefixLen = 32;
        natEntry.nat44Entry.dipPrefixLen = 32;
    }
...
/**
* @internal prvTgfL2NatBridgeConfig function
* @endinternal
*
* @brief   Configures the VLAN ID on ingress port and adds a
*          FDB entry with the packet DMAC, VLAN ID and egress port
*/
static GT_VOID prvTgfL2NatBridgeConfig
(
    GT_VOID
)
{
    ...
    /* create VLAN */
    rc = prvTgfBrgDefVlanEntryWrite(PRV_TGF_VLANID_1_CNS);
    ...
    /* Add FDB entry with Packet's DMAC, VLAN and egress port */
    rc = prvTgfBrgDefFdbMacEntryOnPortSet(prvTgfPacketL2Part.daMac,
                                          PRV_TGF_VLANID_1_CNS,
                                          prvTgfDevNum,
                                          prvTgfPortsArray[PRV_TGF_NEXTHOPE_PORT_IDX_CNS]/*Egress*/,
                                          GT_TRUE);
...
/**
* @internal prvTgfL2NatBridgeTrafficGenerate function
* @endinternal
*
* @brief   Generates traffic with packet definitions and sends it
*          to ingress port
*/
static GT_VOID prvTgfL2NatBridgeTrafficGenerate
(
    GT_VOID
)
{
    ...
    portInterface.type             = CPSS_INTERFACE_PORT_E;
    portInterface.devPort.hwDevNum = prvTgfDevNum;
    portInterface.devPort.portNum  = prvTgfPortsArray[PRV_TGF_NEXTHOPE_PORT_IDX_CNS]/*Egress*/;
    ...
    /* number of parts in packet */
    partsCount = sizeof(prvTgfPacketPartArray1) / sizeof(prvTgfPacketPartArray1[0]);

    /* calculate packet size */
    rc = prvTgfPacketSizeGet(prvTgfPacketPartArray1, partsCount, &packetSize);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfPacketSizeGet");
    ...
    PRV_UTF_LOG1_MAC("\n========== Sending packet on port=%d==========\n\n",
                     prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS]);

    /* Send 1 packets on Ingress port(1) */
    rc = prvTgfStartTransmitingEth(prvTgfDevNum, prvTgfPortsArray[PRV_TGF_SEND_PORT_IDX_CNS]);/* Ingress */
...
/**
* @internal prvTgfL2NatEntrySet function
* @endinternal
*
* @brief   Configures the NAT entry in NAT44 table
*/
static GT_VOID prvTgfL2NatEntrySet
(
    GT_VOID
)
{
    ...
    /* Translate Dest IP to 34.110.170.187 */
    natEntry.nat44Entry.modifyDip = GT_TRUE;
    natEntry.nat44Entry.newDip.arIP[0] = 0x22;
    natEntry.nat44Entry.newDip.arIP[1] = 0x6E;
    natEntry.nat44Entry.newDip.arIP[2] = 0xAA;
    natEntry.nat44Entry.newDip.arIP[3] = 0xBB;

    /* Translate Src IP to 170.186.202.218 */
    natEntry.nat44Entry.modifySip = GT_TRUE;
    natEntry.nat44Entry.newSip.arIP[0] = 0xAA;
    natEntry.nat44Entry.newSip.arIP[1] = 0xBA;
    natEntry.nat44Entry.newSip.arIP[2] = 0xCA;
...
/**
* @internal prvTgfL2NatConfigSet function
* @endinternal
*
* @brief   Configures L2NAT parameters in EPORT table
*          and global config registers
*/
static GT_VOID prvTgfL2NatConfigSet
(
    GT_VOID
)
{
    GT_STATUS   rc;
    PRV_TGF_IP_NAT_L2_PORT_CONFIG_STC l2NatPortCfg;
    PRV_TGF_IP_NAT_L2_GLOBAL_CONFIG_STC l2NatGlobalCfg;

    l2NatPortCfg.l2NatEnable      = GT_TRUE;
    l2NatPortCfg.natPtr           = PRV_TGF_NAT_ENTRY_IDX_CNS;
    l2NatGlobalCfg.l2NatSrcIdSize = 0;
    l2NatGlobalCfg.l2NatSrcIdLsb  = 0;

    /* Save original L2NAT params */
    rc = prvTgfIpNatL2ConfigSet(prvTgfDevNum,
                                prvTgfPortsArray[PRV_TGF_NEXTHOPE_PORT_IDX_CNS]/* Egress port */,
                                PRV_TGF_IP_NAT_TYPE_NAT44_E,
                                &l2NatPortCfg, &l2NatGlobalCfg);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpNatL2ConfigSet");
...
/**
* @internal prvTgfL2NatBridgeCapturedByField function
* @endinternal
*
* @brief   This code is another way to do traffic check,
*          like vfd array, but shorter. In some cases it could be more convenient.
...
* @param[in] portNum                  - port number
*/
static GT_VOID prvTgfL2NatBridgeCapturedByField
(
    IN GT_U8                devNum,
    IN GT_U32               portNum
)
{
    ...
    {
        /* check packet len */
        if (packetActualLength < prvTgfPacketIpv4Part1.totalLen)
        {
            UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, GT_FAIL, "packet len is wrong");
        }
        else  /* check packet content */
        {
            /* check SIP */
            if (0 != cpssOsMemCmp(&natEntry.nat44Entry.newSip.arIP[0], &(packetBuff[26]), 4))
            {
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, GT_FAIL, "packet Src IP is not as expected");
            }
            /* check DIP */
            if (0 != cpssOsMemCmp(&natEntry.nat44Entry.newDip.arIP[0], &(packetBuff[30]), 4))
            {
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, GT_FAIL, "packet Dest IP is not as expected");
            }
        }
    }
}
...
/**
* @internal prvTgfL2NatBridgeConfigRestore function
* @endinternal
*
* @brief   Restores the original NAT entry and original L2NAT params
*/
static GT_VOID prvTgfL2NatBridgeConfigRestore
(
    GT_VOID
)
{
    ...
    /* Restore L2NAT configurations */
    rc = prvTgfIpNatL2ConfigSet(prvTgfDevNum,
                                prvTgfPortsArray[PRV_TGF_NEXTHOPE_PORT_IDX_CNS]/* Egress port */,
                                PRV_TGF_IP_NAT_TYPE_NAT44_E,
                                &l2NatPortCfgOrig, &l2NatGlobalCfgOrig);
    UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, rc, "prvTgfIpNatL2ConfigSet");
...
/**
* @internal prvTgfIpv4UcNatL2 function
* @endinternal
*
* @brief   Set test configuration
*/
GT_VOID prvTgfIpv4UcNatL2
(
    GT_VOID
)
{
    /* Configure VLAN and FDB Entry */
    prvTgfL2NatBridgeConfig();

    /* Configure NAT Entry */
    prvTgfL2NatEntrySet();

    /* Configure L2NAT params */
    prvTgfL2NatConfigSet();

    /* Generate traffic */
    prvTgfL2NatBridgeTrafficGenerate();

    /* Verify traffic on port 3 with expected fields */
    prvTgfL2NatBridgeCapturedByField(prvTgfDevNum,
                prvTgfPortsArray[PRV_TGF_NEXTHOPE_PORT_IDX_CNS]/* Egress port */);

    /* Restore L2NAT configs and NAT entries */
    prvTgfL2NatBridgeConfigRestore();
}
