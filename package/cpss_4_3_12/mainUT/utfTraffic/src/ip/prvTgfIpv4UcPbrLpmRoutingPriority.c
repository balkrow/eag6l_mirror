...
)
{
    ...
    /* Set counter set to 2 for FDB ECMP test */
    if (nhVlanId == PRV_TGF_NEXTHOP_VLANID3_CNS)
    {
        regularEntryPtr->countSet = CPSS_IP_CNT_SET2_E;
    }
...
)
{
    ...
    for (portIter = 0; portIter < PRV_TGF_PORT_COUNT_CNS; portIter++) {
        ...
        if ((isEcmpTest) && (portIter == CPSS_IP_CNT_SET2_E))
        {
            UTF_VERIFY_EQUAL0_STRING_MAC(ipCounters.inUcPkts, 1, "IP counters check failed");
        }
    }
    PRV_UTF_LOG0_MAC("\n");
...
*
* @param[in] prvUtfVrfId              - virtual router index
* @param[in] fdbEcmpTest              - GT_TRUE if the test is FDB ECMP test
*
*/
