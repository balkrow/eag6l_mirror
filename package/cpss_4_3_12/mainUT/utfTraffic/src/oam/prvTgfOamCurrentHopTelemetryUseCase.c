...
)
{
    ...
    rc = prvTgfTtiRule2Get(prvTgfDevNum, prvTgfTtiRuleIndex, PRV_TGF_TTI_RULE_IPV4_E,
                           &ttiPattern, &ttiMask, &ttiAction);
...
)
{
    ...
    pclId = PRV_TGF_PCL_DEFAULT_ID_MAC(
    ...
    devRuleIndex = prvWrAppDxChTcamPclConvertedIndexGet_fromUT(
        prvTgfDevNum, ruleIndex, CPSS_DXCH_TCAM_RULE_SIZE_30_B_E);
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        cpssAction.mirror.cpuCode                        = CPSS_NET_IP_HDR_ERROR_E;
        cpssAction.mirror.mirrorToTxAnalyzerPortEn       = GT_TRUE;
        cpssAction.mirror.egressMirrorToAnalyzerIndex    = analyzerIndex;
        cpssAction.mirror.egressMirrorToAnalyzerMode     = CPSS_DXCH_MIRROR_EGRESS_NOT_DROPPED_E;
        ...
        UTF_VERIFY_EQUAL2_STRING_MAC(
            ...
            prvTgfDevNum, prvTgfPortsArray[PRV_TGF_TRANS_PORT_IDX_CNS]);
    ...
    else
    {
        ...
        pattern.ruleEgrStdIpL2Qos.common.pclId       = (GT_U16)pclId;

        rc = prvTgfPclRuleSet(ruleFormat, ruleIndex, &mask, &pattern, &action);
        UTF_VERIFY_EQUAL3_STRING_MAC(
            GT_OK, rc, "prvTgfPclRuleSet: %d, %d, %d", prvTgfDevNum, ruleFormat, ruleIndex);
    }
}
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        cpssAction.flowId                     = PRV_TGF_EPCL_FLOW_ID_CNS;
        cpssAction.policer.policerEnable      = CPSS_DXCH_PCL_POLICER_ENABLE_COUNTER_ONLY_E;
        cpssAction.egressPolicy               = CPSS_PCL_DIRECTION_EGRESS_E;
        cpssAction.epclPhaInfo.phaThreadType  = CPSS_DXCH_PCL_ACTION_EGRESS_PHA_THREAD_INT_IOAM_MIRROR_E;
        cpssAction.epclPhaInfo.phaThreadUnion.epclIntIoamMirror.erspanIII_P = GT_TRUE;
