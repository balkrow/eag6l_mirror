...
)
{
    ...
    {
        /* Validate keep alive aging status - As Exception occured */
        prvTgfOamConfigurationGet(&oamConfig);
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.keepaliveAgingStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);

        /* clear the event status & and disable LOC detect */
        oamConfig.oamEntry.keepaliveAgingStatus = PRV_TGF_OAM_EXCEPTION_STATUS_PASS_E;
        oamConfig.oamEntry.locDetectionDisable  = GT_TRUE;
        prvTgfOamConfigurationSet(&oamConfig);

        /* Wait 5 seconds */
        cpssOsTimerWkAfter(secondsDelay * 1000);

        /* AUTODOC: Check no exception */
        prvTgfDefOamGeneralTrafficCheck(
                PRV_TGF_SEND_PORT_IDX_CNS,
                PRV_TGF_OAM_EXCEPTION_TYPE_LAST_E,
        ...
        oamConfig.oamEntry.keepaliveAgingStatus = PRV_TGF_OAM_EXCEPTION_STATUS_PASS_E;
        oamConfig.oamEntry.locDetectionDisable  = GT_FALSE;
        prvTgfOamConfigurationSet(&oamConfig);
        ...
        /* recheck the event status bit - event should have occured */
        prvTgfOamConfigurationGet(&oamConfig);
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.keepaliveAgingStatus,
                "prvTgfTrunkHashGlobalModeGet: %d", prvTgfDevNum);

        /* AUTODOC: Validate exception suppress for keepAlive aging */
        /* Enable 'aging exception suppress' */
        oamConfig.oamEntry.keepaliveAgingStormSuppressEnable = GT_TRUE;
        oamConfig.oamEntry.ageState = PRV_TGF_OAM_KEEPALIVE_AGING_THRESHOLD_STC;
        prvTgfOamConfigurationSet(&oamConfig);
        ...
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
                "prvTgfOamExceptionCounterGet: %d", prvTgfDevNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(0, counterValue,
                "unexpected keep alive exception counter value for : %d", prvTgfDevNum);

        /* recheck the event status bit - event should not have occured (Previous state is FAIL)*/
        prvTgfOamConfigurationGet(&oamConfig);
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.keepaliveAgingStatus,
                "prvTgfTrunkHashGlobalModeGet: %d", prvTgfDevNum);

        /***** Validate Keep alive recovery command *****/
        /* Set exception recovery packet command */
        exceptionConfig.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
        exceptionConfig.cpuCode = CPSS_NET_FIRST_USER_DEFINED_E + PRV_TGF_OAM_EXCEPTION_TYPE_KEEPALIVE_AGING_E;
        ...
        oamConfig.oamEntry.keepaliveAgingStormSuppressEnable = GT_FALSE;
        prvTgfOamConfigurationSet(&oamConfig);
...
)
{
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(prvTgfDevNum))
    {
        ...
        /* Validate keep Tx period status - As Exception occured */
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.txPeriodStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);

        /***** Validate exception suppress command *****/
        /* Set exception suppress packet command */
        exceptionConfig.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
        exceptionConfig.cpuCode = CPSS_NET_FIRST_USER_DEFINED_E + PRV_TGF_OAM_EXCEPTION_TYPE_TX_PERIOD_E;
        rc = cpssDxChOamExceptionSuppressConfigSet(
                ...
                &exceptionConfig);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, rc,
                "cpssDxChOamExceptionsuppressConfigSet: %d", prvTgfDevNum);

        /* AUTODOC: Suppress Exception and try to generate the Exception */
        oamConfig.oamEntry.txPeriodStormSuppressEnable  = GT_TRUE;
        prvTgfOamConfigurationSet(&oamConfig);
        ...
        /* Validate keep Tx period status - As no Exception occured (previous state) */
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.txPeriodStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);

        /* AUTODOC: Enable period field checking */
        oamConfig.oamEntry.txPeriodStormSuppressEnable  = GT_FALSE;
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(prvTgfDevNum))
    {
        /* AUTODOC: Get common OAM keepalive configurations */
        prvTgfOamConfigurationGet(&oamConfig);

        /* Validate keep Tx period status - As no exception occured */
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_PASS_E,
                oamConfig.oamEntry.txPeriodStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);
    }
...
)
{
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(prvTgfDevNum))
    {
        /* Validate invalid hash status - As Exception occured */
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.hashVerificationStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);

        /***** Validate invalid keep alive suppress command *****/
        /* Set exception suppress packet command */
        exceptionConfig.command = CPSS_PACKET_CMD_TRAP_TO_CPU_E;
        exceptionConfig.cpuCode = CPSS_NET_FIRST_USER_DEFINED_E + PRV_TGF_OAM_EXCEPTION_TYPE_KEEPALIVE_INVALID_HASH_E;
        rc = cpssDxChOamExceptionSuppressConfigSet(
                prvTgfDevNum,
                CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
        ...
        /* Enable 'invalid hash exception suppress' */
        oamConfig.oamEntry.invalidHashKeepaliveStormSuppressEnable = GT_TRUE;
        prvTgfOamConfigurationSet(&oamConfig);
        ...
        /* Validate invalid hash status - As no exception occured (Previous fail status) */
        UTF_VERIFY_EQUAL1_STRING_MAC(PRV_TGF_OAM_EXCEPTION_STATUS_FAIL_E,
                oamConfig.oamEntry.hashVerificationStatus,
                "prvTgfOamConfigurationGet: %d", prvTgfDevNum);

        /* Restore */
        /* AUTODOC: Reset lock hash value and clear the event status */
        oamConfig.oamEntry.invalidHashKeepaliveStormSuppressEnable  = GT_FALSE;
        oamConfig.oamEntry.hashVerificationStatus = PRV_TGF_OAM_EXCEPTION_STATUS_PASS_E;
    }
    ...
    if(PRV_CPSS_SIP_5_15_CHECK_MAC(prvTgfDevNum))
    {
        ...
        /******** Validate packet recovery command *********/
        /* Set exception recovery packet command */
        exceptionConfig.command = CPSS_PACKET_CMD_FORWARD_E;
        exceptionConfig.cpuCode = CPSS_NET_FIRST_USER_DEFINED_E + PRV_TGF_OAM_EXCEPTION_TYPE_KEEPALIVE_INVALID_HASH_E;
        rc = cpssDxChOamExceptionRecoveryConfigSet(
...
{
    GT_STATUS st;
    static TGF_TCAM_CLIENTS_PCL_RULE_SUB_FIELD_STC epclSubFieldArr[] =
    {
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        st = prvTgfSip7TcamClientsOneLookupDevInit(
            prvTgfDevNum, TGF_TCAM_CLIENTS_PCL_LOOKUP_EPCL_E,
            CPSS_DXCH_TCAM_RULE_SIZE_30_B_E, epclSubFieldArr);
        UTF_VERIFY_EQUAL0_STRING_MAC(
            GT_OK, st, "prvTgfSip7TcamClientsOneLookupDevInit");

        st = prvTgfPclDefPortInitSip7(
            ...
            PRV_TGF_EOAM_TEST_PCLID, 1/*emSubProfileId*/);
        UTF_VERIFY_EQUAL0_STRING_MAC(
            GT_OK, st, "prvTgfPclDefPortInitSip7");
    }
    else
    {
    ...
    if(PRV_CPSS_SIP_6_CHECK_MAC(prvTgfDevNum))
    {
        ...
        st =  prvTgfTrunkHashGlobalModeSet(PRV_TGF_TRUNK_LBH_PACKETS_INFO_E);
...
)
{
    ...
    if (PRV_CPSS_SIP_7_CHECK_MAC(prvTgfDevNum))
    {
        ...
        cpssAction.flowId = PRV_TGF_EPCL_ACTION_FLOW_ID_CNS; /* EPCL Action flow ID */

        commonKeyHeaderMask = 0;
        commonKeyHeaderPattern = 0;

        devRuleIndex = prvWrAppDxChTcamPclConvertedIndexGet_fromUT(
            prvTgfDevNum, ruleIdx, CPSS_DXCH_TCAM_RULE_SIZE_30_B_E);
        st = tgfTcamClientsPclRuleSet(
            ...
            subFieldConditionArr, &cpssAction);
