...
static GT_VOID prvTgfSgtUserIpclConfig(GT_VOID)
{
    ...
    /* Tag1 - up1 Update */
    action.qos.ingressUp1Cmd       = PRV_TGF_PCL_ACTION_INGRESS_UP1_CMD_ALL_E;
...
)
{
    ...
    if(PRV_CPSS_SIP_6_30_CHECK_MAC(prvTgfDevNum))
    {
        /* Use Copy UDB member instead of updateTag1FromUdb starting from IronMan
           to copy content of UDB 48,49 */
        action.vlan.copyUdb           = GT_TRUE;
        ...
        rc = cpssDxChPclEpgConfigSet(
            prvTgfDevNum, CPSS_PCL_DIRECTION_INGRESS_E,
            CPSS_PCL_LOOKUP_NUMBER_0_E, &epgConfig);
