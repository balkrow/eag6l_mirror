...
*/
UTF_TEST_CASE_MAC(cpssDxChBrgSecurBreachPortGroupGlobalDropCntrSet)
{
/*
    ITERATE_DEVICES_PORT_GROUPS (DxChx)
    1.1.1 Call function with dropCnt [0 / 0x1000 / 0xFFFF ].
    Expected: GT_OK.
    1.1.2. Call cpssDxChBrgSecurBreachPortGroupGlobalDropCntrGet with non-NULL dropCntPtr.
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            */
            ...
            st = cpssDxChBrgSecurBreachPortGroupGlobalDropCntrSet(dev, portGroupsBmp, dropCnt);
            ...
            */
            ...
            st = cpssDxChBrgSecurBreachPortGroupGlobalDropCntrGet(dev, portGroupsBmp, &dropCntRet);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                            "cpssDxChBrgSecurBreachPortGroupGlobalDropCntrGet: %d", dev);

            if (GT_OK == st)
            {
                /* Verifying values */
                UTF_VERIFY_EQUAL1_STRING_MAC(dropCnt, dropCntRet,
                 "cpssDxChBrgSecurBreachPortGroupGlobalDropCntrGet: get another dropCnt than was set: dev = %d", dev);
            }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChBrgSecurityBreach)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgSecurBreachPortGroupGlobalDropCntrSet)
