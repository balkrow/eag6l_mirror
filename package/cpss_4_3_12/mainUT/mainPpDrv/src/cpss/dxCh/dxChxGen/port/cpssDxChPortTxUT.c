...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        {
            params.poolLimitEnable = GT_TRUE;
            params.queueExp = 1;
        }
        else
        {
            params.poolLimitEnable = GT_FALSE;
            params.queueExp = 0;
        }
...
*/
    ...
    GT_U32 poolRestoreBuff[2]={0,0};
    GT_U32 poolRestoreDesc[2]={0,0};
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for(i=0;i<2;i++)
        {
          st = cpssDxChPortTxSharedPoolLimitsGet(dev, i, poolRestoreBuff+i, poolRestoreDesc+i);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
        /*
        ...
        */
        ...
        st = cpssDxChPortTxSharedPoolLimitsSet(dev, poolNum, maxBufNum, maxDescNum,GT_FALSE);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        */
        ...
        for(i=0;i<2;i++)
        {
          st = cpssDxChPortTxSharedPoolLimitsSet(dev, i, 0, 0,GT_FALSE);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
        for(i=0;i<2;i++)
        {
          st = cpssDxChPortTxSharedPoolLimitsSet(dev, i, poolRestoreBuff[i], poolRestoreDesc[i],GT_FALSE);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
    }
...
*/
    ...
    */
    UTF_TEST_CASE_MAC(cpssDxChPortTxQShapingCreditAccumulationEnableSet)
    {
    /*
        ITERATE_DEVICES_PHY_CPU_PORTS (DxChx)
        1.1.1. Call with enable [GT_FALSE and GT_TRUE].
    ...
    */
        ...
        GT_U8       dev;
        GT_PHYSICAL_PORT_NUM  port   = PORT_TX_VALID_PHY_PORT_CNS;
        GT_BOOL     creditAccumulationEnable = GT_FALSE;
        GT_BOOL     creditAccumulationEnableGet = GT_FALSE;
        GT_U8       i;
        ...
        while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
        {
            ...
            while (GT_OK == prvUtfNextTxqPortGet(&port, GT_TRUE))
            {
                ...
                */
                /* Call function with enable = GT_FALSE] */
                creditAccumulationEnable = GT_FALSE;

                for(i=0;i<8;i++)
                {
                    st = cpssDxChPortTxQShapingCreditAccumulationEnableSet(dev, port, i,creditAccumulationEnable);
                    UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, creditAccumulationEnable);

                    /*
                        1.1.2. Call cpssDxChPortTxShaperAvbModeEnableGet
                        Expected: GT_OK and the same value.
                    */
                    st = cpssDxChPortTxQShapingCreditAccumulationEnableGet(dev, port, i,&creditAccumulationEnableGet);
                    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

                    /* Verifying values */
                    UTF_VERIFY_EQUAL2_STRING_MAC(creditAccumulationEnable, creditAccumulationEnableGet,
                           "get another enable than was set: %d, %d", dev, port);

                }
                /* Call function with enable = GT_TRUE] */
                creditAccumulationEnable = GT_TRUE;
            ...
            while (GT_OK == prvUtfNextTxqPortGet(&port, GT_FALSE))
            {
                /* 1.2.1. Call function for each non-active port */
                /* enable == GT_TRUE    */
                st = cpssDxChPortTxQShapingCreditAccumulationEnableSet(dev, port, 0,creditAccumulationEnable);
                UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
            }
            ...
            /* 1.5. Invalid queue number */
            st = cpssDxChPortTxQShapingCreditAccumulationEnableSet(dev, port, 257,creditAccumulationEnable);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
        }
...
(
   ...
   */
   UTF_TEST_CASE_MAC(cpssDxChPortTxQShapingCreditAccumulationEnableGet)
   {
   /*
       ITERATE_DEVICES_PHY_CPU_PORTS (DxChx)
       1.1.1. Call with enable [GT_FALSE and GT_TRUE].
       ...
       while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
       {
           ...
           while (GT_OK == prvUtfNextTxqPortGet(&port, GT_TRUE))
           {
               ...
               */
               ...
               for(i=0;i<8;i++)
               {
                   st = cpssDxChPortTxQShapingCreditAccumulationEnableGet(dev, port, i,&creditAccumulationEnable);
                   UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, creditAccumulationEnable);
           ...
           while (GT_OK == prvUtfNextTxqPortGet(&port, GT_FALSE))
           {
               /* 1.2.1. Call function for each non-active port */
               /* enable == GT_TRUE    */
               st = cpssDxChPortTxQShapingCreditAccumulationEnableGet(dev, port, 0,&creditAccumulationEnable);
               UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
           }
           ...
           /* 1.5. Invalid queue number */
           st = cpssDxChPortTxQShapingCreditAccumulationEnableGet(dev, port, 257,&creditAccumulationEnable);
           UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);

          /*
                   1.1.2. Call function with creditAccumulationEnable[NULL].
                   Expected: GT_BAD_PTR.
                */
           st = cpssDxChPortTxQShapingCreditAccumulationEnableGet(dev, port, 0,NULL);
           UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, port);
       }
...
*/
    ...
    UTF_TEST_CASE_MAC(cpssDxChPortTx4TcTailDropWredProfileSet)
    {
    ...
    GT_U8                             dev;
    CPSS_PORT_TX_DROP_PROFILE_SET_ENT profile          = 0;
    CPSS_PORT_TX_Q_TAIL_DROP_WRED_TC_DP_PROF_PARAMS_STC params;
    CPSS_PORT_TX_Q_TAIL_DROP_WRED_TC_DP_PROF_PARAMS_STC paramsGet;
    CPSS_PORT_TX_Q_TAIL_DROP_WRED_TC_DP_PROF_PARAMS_STC paramsSave;
    GT_U32 tc =0;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        st = cpssDxChPortTx4TcTailDropWredProfileGet(dev, profile,tc, &paramsSave);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChPortTx4TcTailDropWredProfileGet: %d", dev);

        params.dp0WredAttributes.guaranteedLimit = 1<<10;
        params.dp1WredAttributes.guaranteedLimit = params.dp0WredAttributes.guaranteedLimit<<2;
        params.dp2WredAttributes.guaranteedLimit = params.dp1WredAttributes.guaranteedLimit<<2;

        params.dp0WredAttributes.probability = CPSS_PORT_TX_TAIL_DROP_PROBABILITY_100_E;
        params.dp1WredAttributes.probability = CPSS_PORT_TX_TAIL_DROP_PROBABILITY_89_E;
        params.dp2WredAttributes.probability = CPSS_PORT_TX_TAIL_DROP_PROBABILITY_25_E;

        params.dp0WredAttributes.wredSize = 1<<5;
        params.dp1WredAttributes.wredSize = params.dp0WredAttributes.wredSize<<2;
        params.dp2WredAttributes.wredSize = params.dp1WredAttributes.wredSize<<2;

        st = cpssDxChPortTx4TcTailDropWredProfileSet(dev, profile, tc, &params);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, profile);


        st = cpssDxChPortTx4TcTailDropWredProfileGet(dev, profile,tc, &paramsGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChPortTx4TcTailDropWredProfileGet: %d", dev);

        if(0!=osMemCmp(&params,&paramsGet, sizeof(CPSS_PORT_TX_Q_TAIL_DROP_WRED_TC_DP_PROF_PARAMS_STC)))
        {
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, GT_FAIL, "osMemCmp failed : %d", dev);
        }

        /*max size*/
        params.dp0WredAttributes.guaranteedLimit = BIT20;

        st = cpssDxChPortTx4TcTailDropWredProfileSet(dev, profile, tc, &params);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, dev, profile);

        params.dp0WredAttributes.guaranteedLimit = 20;
        params.dp1WredAttributes.wredSize =  BIT20;

        st = cpssDxChPortTx4TcTailDropWredProfileSet(dev, profile, tc, &params);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, dev, profile);

        params.dp1WredAttributes.wredSize =  20;
        tc = CPSS_16_TC_RANGE_CNS;
        ...
        /*clean up*/
        st = cpssDxChPortTx4TcTailDropWredProfileSet(dev, profile,tc,  &paramsSave);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, profile);
...
UTF_TEST_CASE_MAC(cpssDxChPortTxQueueOptimizedThroughputEnableSet)
{
/*
    1.1. Call with non-null enablePtr.
    Expected: GT_OK.
...
*/
    ...
    GT_PHYSICAL_PORT_NUM port;
    GT_U32      queueOffset;
    GT_BOOL     enableGet,enableSave;
    CPSS_DXCH_DETAILED_PORT_MAP_STC * portShadowPtr;
    ...
    GT_U32 notApplicableDevices =
        ...
        UTF_CAELUM_E | UTF_ALDRIN_E|UTF_ALDRIN2_E | UTF_AC3X_E | UTF_BOBCAT3_E );
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
       ...
       UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
        ...
        while (GT_OK == prvUtfNextPhyPortGet(&port, GT_TRUE))
        {
            st = prvCpssDxChPortPhysicalPortMapShadowDBGet(dev, port, /*OUT*/ &portShadowPtr);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

            if (portShadowPtr->valid == GT_FALSE||
                CPSS_DXCH_PORT_MAPPING_TYPE_ETHERNET_MAC_E!=portShadowPtr->portMap.mappingType)
            {
               continue;
            }

            for(queueOffset = 0; queueOffset<CPSS_TC_RANGE_CNS;queueOffset++)
            {
                st = cpssDxChPortTxQueueOptimizedThroughputEnableGet(dev, port, queueOffset,&enableSave);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                st = cpssDxChPortTxQueueOptimizedThroughputEnableSet(dev, port, queueOffset,GT_TRUE);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                st = cpssDxChPortTxQueueOptimizedThroughputEnableGet(dev, port, queueOffset,&enableGet);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_TRUE, enableGet, dev, port, queueOffset);

                st = cpssDxChPortTxQueueOptimizedThroughputEnableSet(dev, port, queueOffset,GT_FALSE);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                st = cpssDxChPortTxQueueOptimizedThroughputEnableGet(dev, port, queueOffset,&enableGet);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_FALSE, enableGet, dev, port, queueOffset);

                st = cpssDxChPortTxQueueOptimizedThroughputEnableSet(dev, port, queueOffset,enableSave);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);
            }
        }
        ...
        {
            /* 1.2.1. Call function for each non-active port */
            st = cpssDxChPortTxQueueOptimizedThroughputEnableSet(dev, port, 0,GT_TRUE);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
        }
    }
...
UTF_TEST_CASE_MAC(cpssDxChPortTxBindPortQueueToDpSet)
{

    GT_STATUS   st    = GT_OK;
    GT_U8       dev;
    GT_PHYSICAL_PORT_NUM port;
    GT_U32      queueOffset,maxQOffset;
    CPSS_DXCH_DETAILED_PORT_MAP_STC * portShadowPtr;
    GT_U32      profileSave;
    GT_U32      profileSet []= {0,128,512,1023};
    GT_U32      profileGet,i;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
       ...
       UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
        ...
        while (GT_OK == prvUtfNextPhyPortGet(&port, GT_TRUE))
        {
            ...
            st = prvCpssSip6TxqUtilsMaxQueueOffsetGet(dev, port, &maxQOffset);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

            for(queueOffset = 0; queueOffset<=maxQOffset;queueOffset++)
            {
                st = cpssDxChPortTxBindPortQueueToDpGet(dev, port, queueOffset,&profileSave);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);
                for(i=0;i<sizeof(profileSet)/sizeof(profileSet[0]);i++)
                {
                    st = cpssDxChPortTxBindPortQueueToDpSet(dev, port, queueOffset,profileSet[i]);
                    UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                    st = cpssDxChPortTxBindPortQueueToDpGet(dev, port, queueOffset,&profileGet);
                    UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);

                    UTF_VERIFY_EQUAL3_PARAM_MAC(profileSet[i], profileGet, dev, port, queueOffset);
                }
                st = cpssDxChPortTxBindPortQueueToDpSet(dev, port, queueOffset,profileSave);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, port, queueOffset);
            }

            /* 1.2. Check queue out of range*/
            st = cpssDxChPortTxBindPortQueueToDpSet(dev, port, queueOffset,profileSet[0]);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_PARAM, st, dev, port, queueOffset);

            /* 1.3. Check profile out of range*/
            st = cpssDxChPortTxBindPortQueueToDpSet(dev, port, 0,2023);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_PARAM, st, dev, port, queueOffset);
        }
        ...
        while (GT_OK == prvUtfNextPhyPortGet(&port, GT_FALSE))
        {
            ...
            st = cpssDxChPortTxBindPortQueueToDpSet(dev, port, 0,0);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
        }
    }
...
UTF_TEST_CASE_MAC(cpssDxChPortTxBindPortQueueToDpGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
       ...
       UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
        ...
        while (GT_OK == prvUtfNextPhyPortGet(&port, GT_TRUE))
        {
            ...
            /* 1.3. Check NULL  profile */
            st = cpssDxChPortTxBindPortQueueToDpGet(dev, port, 0,NULL);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_PTR, st, dev, port, queueOffset);
        }
        ...
        {
            /* 1.4.1. Call function for each non-active port */
            st = cpssDxChPortTxBindPortQueueToDpGet(dev, port, 0,&profileGet);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
        }
    }
...
UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxQShapingCreditAccumulationEnableSet)
UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxQShapingCreditAccumulationEnableGet)

UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTx4TcTailDropWredProfileSet)
UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxQueueOptimizedThroughputEnableSet)
UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxBindPortQueueToDpSet)
UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxBindPortQueueToDpGet)
