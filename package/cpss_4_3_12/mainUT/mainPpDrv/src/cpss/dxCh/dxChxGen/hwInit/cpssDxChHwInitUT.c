...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_CH2_E | UTF_CH3_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
...
static void prv_cpssDxChHwAccessInfoStoreTest(void)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        {
            /* dont know why gcc12 complain on : PRV_CPSS_DXCH_DEV_REGS_MAC(dev)->bridgeRegs.qosProfileRegs.macQosTable
               and not complain about PRV_CPSS_PP_MAC(dev)->devFamily that is above this check.
            */
            break;
        }
...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, (UTF_AC5_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E));
...
typedef enum
{
    PRV_INIT_SYS_TIME_CPU_UNKNOWN_E,
    PRV_INIT_SYS_TIME_CPU_WIN_SIM_E,
    PRV_INIT_SYS_TIME_CPU_LINUX_SIM_E,
    PRV_INIT_SYS_TIME_CPU_ARMARCH64_E,
    PRV_INIT_SYS_TIME_CPU_ARMV7_E
} PRV_INIT_SYS_TIME_CPU_ENT;

#define FAMILY(_f) CPSS_PP_FAMILY_DXCH_##_f##_E
#define CPU(_c) PRV_INIT_SYS_TIME_CPU_##_c##_E
#define LE 0
#define BE 1
#define EXT 0
#define INT 1
...
typedef struct
{
    ...
    PRV_INIT_SYS_TIME_CPU_ENT cpuArch;
    GT_U32                    bigEnd;
    GT_U32                    internalCpu;
    GT_U32                    sharedMemory;
    GT_U32                    boardIdx;
    ...
    GT_U32                    portMgr;
    GT_U32                    numOfTiles;
    GT_U32                    patternMilliSec;
} PRV_INIT_SYSTEM_TIME_CHECK_STC;

static const PRV_INIT_SYSTEM_TIME_CHECK_STC initSystemTimesArr[] =
{
    {FAMILY(AC5),     CPU(WIN_SIM),   LE,  EXT,   0, 19, 10, PORT_LE,  /*tiles*/0,   1400},  /* debug WIN WM */
    {FAMILY(AC5),     CPU(WIN_SIM),   LE,  EXT,   0, 19, 10, PORT_MG,  /*tiles*/0,   1746},  /* debug WIN WM */
    {FAMILY(AC5X),    CPU(WIN_SIM),   LE,  EXT,   0, 38,  1, PORT_LE,  /*tiles*/0,   6236},  /* debug WIN WM */
    {FAMILY(AC5),     CPU(LINUX_SIM), LE,  EXT,   0, 19, 10, PORT_LE,  /*tiles*/0,    279},  /* debug LINUX WM */
    {FAMILY(AC5),     CPU(ARMARCH64), LE,  EXT,   0, 19,  2, PORT_MG,  /*tiles*/0,   1183},  /* ac5-external_ARMv8_4_4 */
    {FAMILY(AC5),     CPU(ARMARCH64), LE,  INT,   0, 19, 10, PORT_LE,  /*tiles*/0,   3464},  /* ac5-rd_ARMv8_4_14 */
    {FAMILY(AC5P),    CPU(ARMARCH64), LE,  EXT,   0, 37,  1, PORT_MG,  /*tiles*/0,  11152},  /* ac5p_ARMv8_4_14 */
    {FAMILY(AC5P),    CPU(ARMARCH64), LE,  EXT, SHM, 37,  1, PORT_MG,  /*tiles*/0,  11809},  /* ac5p_shared_ARMv8_4_14 */
    {FAMILY(AC5X),    CPU(ARMARCH64), LE,  EXT,   0, 38,  1, PORT_MG,  /*tiles*/0,   7778},  /* ac5x-external_ARMv8_4_4 */
    {FAMILY(AC5X),    CPU(ARMARCH64), LE,  EXT, SHM, 38,  1, PORT_MG,  /*tiles*/0,   7986},  /* ac5x-external_shared_ARMv8_4_4 */
    {FAMILY(AC5X),    CPU(ARMARCH64), BE,  EXT,   0, 38,  3, PORT_MG,  /*tiles*/0,  55871},  /* ac5x-rd_ARMv8be_4_14 */
    {FAMILY(AC5X),    CPU(ARMARCH64), LE,  INT,   0, 38,  1, PORT_MG,  /*tiles*/0,   6140},  /* ac5x-internal_ARMv8_4_14 */
    {FAMILY(HARRIER), CPU(ARMARCH64), LE,  EXT,   0, 40,  1, PORT_MG,  /*tiles*/0,   9172},  /* harrier_ARMv8_4_14 */
    {FAMILY(IRONMAN), CPU(ARMARCH64), LE,  INT,   0, 41,  1, PORT_MG,  /*tiles*/0,   5698},  /* im-l_ARMv8_4_14 */
    {FAMILY(IRONMAN), CPU(ARMARCH64), LE,  INT,   0, 41,  3, PORT_MG,  /*tiles*/0,   6314},  /* im-m_ARMv8_4_14 */
    {FAMILY(FALCON),  CPU(ARMARCH64), LE,  EXT,   0, 35,  1, PORT_MG,  /*tiles*/4,  28661},  /* falcon_ARMv8_4_4 */
    {FAMILY(FALCON),  CPU(ARMARCH64), LE,  EXT,   0, 35,  2, PORT_LE,  /*tiles*/4,  41656},  /* falcon_ARMv8_4_4 */
    {FAMILY(FALCON),  CPU(ARMARCH64), BE,  EXT,   0, 35,  7, PORT_MG,  /*tiles*/2,  18902}   /* falcon-4t_ARMv8be_4_4 */
};
static const GT_U32 initSystemTimesArrSize =
    sizeof(initSystemTimesArr) / sizeof(initSystemTimesArr[0]);
...
UTF_TEST_CASE_MAC(cpssInitSystemTimeCheck)
{
    GT_STATUS rc;
    GT_U32 checkedTimeMilliSec = prvAppDemoFirstInitTimes.initSystemTimeTotal;
    GT_U32 idx;
    GT_U32 patternMilliSec;
    GT_U32 lowMilliSec;
    GT_U32 highMilliSec;
    PRV_INIT_SYS_TIME_CPU_ENT cpuArch = PRV_INIT_SYS_TIME_CPU_UNKNOWN_E;
    GT_U32 internalCpu;
    GT_U32 numOfTiles = 0;
    GT_U32 boardIdx;
    GT_U32 boardRevId;
    GT_U32 multiProcessApp;
    GT_U32 sharedMemory = 0;
    GT_BOOL pmEnabled;
    GT_U32 portMgr;
    union
    {
    ...
    if (checkedTimeMilliSec == 0) /* Time not saved */
    {
        /* SKIP macro can be ignored, this case - no */
        prvUtfSkipTestsSet();
        return;
    }
    cpssOsPrintf(
        "Durations: total %d, Phase1 %d Phase2 %d PpLogicalInit %d cpssLibInit %d\n",
        prvAppDemoFirstInitTimes.initSystemTimeTotal,
    ...
    bigEndTst.arr[2] = 3;
    bigEndTst.arr[3] = 4;
    bigEnd = (bigEndTst.u32 == 0x01020304) ? 1 : 0;

    internalCpu = (CPSS_HW_INFO_BUS_TYPE_MBUS_E ==
        PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(prvTgfDevNum)->hwInfo[0].busType)
            ? 1 : 0;
    ...
    #endif
    cpssInitSystemGet(
        &boardIdx, &boardRevId, &multiProcessApp);

    pmEnabled = GT_FALSE;
    rc = cpssDxChPortManagerEnableGet(prvTgfDevNum, &pmEnabled);
    if (rc != GT_OK)
    {
        /* device does not support Port Manager */
        pmEnabled = GT_FALSE;
    }
    portMgr = BOOL2BIT_MAC(pmEnabled);

    cpssOsPrintf(
        "bigEnd %d internalCpu %d sharedMemory %d, portMgr %d\n",
        bigEnd, internalCpu, sharedMemory, portMgr);
    cpssOsPrintf(
        "board: boardIdx %d boardRevId %d time: checkedTimeMilliSec %d\n",
        boardIdx, boardRevId, checkedTimeMilliSec);

    #ifdef ASIC_SIMULATION
        /* this skip can be canceled by prvUtfDebugSkipIgnoreSet call */
        SKIP_TEST_MAC;
    #endif

    #ifdef ASIC_SIMULATION
        #if defined(_WIN32) || defined(_WIN64)
            cpssOsPrintf("Windows simulation\n");
            cpuArch = PRV_INIT_SYS_TIME_CPU_WIN_SIM_E;
        #endif
        #if defined(__GNUC__)
            cpssOsPrintf("Linux simulation\n");
            cpuArch = PRV_INIT_SYS_TIME_CPU_LINUX_SIM_E;
        #endif
    #else
        #if (__ARM_ARCH == 8) && defined(LINUX)
            cpssOsPrintf("ARMARCH64\n");
            cpuArch = PRV_INIT_SYS_TIME_CPU_ARMARCH64_E;
        #endif
        #if (__ARM_ARCH == 7) && defined(LINUX)
            cpssOsPrintf("ARMV7\n");
            cpuArch = PRV_INIT_SYS_TIME_CPU_ARMV7_E;
        #endif

    #endif

    if (cpuArch == PRV_INIT_SYS_TIME_CPU_UNKNOWN_E)
    {
        /* SKIP macro can be ignored, this case - no */
        prvUtfSkipTestsSet();
        return;
    ...
    for (idx = 0; (idx < initSystemTimesArrSize); idx++)
    {
        if (PRV_CPSS_PP_MAC(prvTgfDevNum)->devFamily
            != initSystemTimesArr[idx].family) continue;
        if (cpuArch      != initSystemTimesArr[idx].cpuArch) continue;
        if (bigEnd       != initSystemTimesArr[idx].bigEnd) continue;
        if (internalCpu  != initSystemTimesArr[idx].internalCpu) continue;
        if (sharedMemory != initSystemTimesArr[idx].sharedMemory) continue;
        if (boardIdx     != initSystemTimesArr[idx].boardIdx) continue;
        if (boardRevId   != initSystemTimesArr[idx].boardRevId) continue;
        if (portMgr      != initSystemTimesArr[idx].portMgr) continue;
        if (numOfTiles   != initSystemTimesArr[idx].numOfTiles) continue;
        /*found*/
        break;
    }
    if (idx >= initSystemTimesArrSize)
    {
        /* SKIP macro can be ignored, this case - no */
        prvUtfSkipTestsSet();
        return;
    }
    patternMilliSec = initSystemTimesArr[idx].patternMilliSec;
    lowMilliSec = (patternMilliSec * 80) / 100;
    highMilliSec = (patternMilliSec * 120) / 100;
    cpssOsPrintf(
        "cpssInitSystemTimeCheck check bounds : lowMilliSec %d highMilliSec %d\n",
        lowMilliSec, highMilliSec);
    if ((checkedTimeMilliSec < lowMilliSec) || (checkedTimeMilliSec > highMilliSec))
    {
        UTF_VERIFY_EQUAL1_STRING_MAC(
            0, 1, "value %d out of bounds", checkedTimeMilliSec);
    }
}
#endif /*#if !defined(CPSS_APP_PLATFORM_REFERENCE) || defined(MIXED_MODE)*/
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChHwInit)
    ...
    #if !defined(CPSS_APP_PLATFORM_REFERENCE) || defined(MIXED_MODE)
        UTF_SUIT_DECLARE_TEST_MAC(cpssInitSystemTimeCheck)
    #endif
...
)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_IN_BMP_MAC(dev, portGroupsBmp, portGroupId)
        {
            ...
            if(PRV_CPSS_DXCH_PP_MAC(dev)->errata.
                info_PRV_CPSS_DXCH_XCAT_RESTRICTED_ADDRESS_FILTERING_WA_E.unitRangesPtr[portGroupId] == NULL)
            {
                /* The WA is not initialized */
                continue;
            }
...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_CH2_E | UTF_CH3_E | UTF_LION_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_XCAT_E| UTF_XCAT2_E | UTF_AC3X_E | UTF_BOBCAT3_E | UTF_ALDRIN2_E | UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
