        ...
        case PRV_UTF_DXCH_DATA_INTEGRITY_OP_ERROR_INJECTION_E:
            switch (memType)
            {
                ...
                case CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_CORESIGHT_CSTB_E:
                    /* CNM and embedded CPU RAMs in AC5/X and IM */
                    doSkip = PRV_CPSS_DXCH_AC5X_CHECK_MAC(dev) || PRV_CPSS_DXCH_AC5_CHECK_MAC(dev) ||
                             PRV_CPSS_DXCH_IRONMAN_CHECK_MAC(dev);
                    break;
                ...
                case CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_EPI_USX_PCS_U_F91M_E:
                    /* RAM is constantly used on AC5X RD board where USX ports configured and connected to PHYs.
                       Therefore Get return always FALSE status of error injection. */
                    doSkip = PRV_CPSS_DXCH_AC5X_CHECK_MAC(dev);
                    break;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        for (memType = 0; memType < devLastMem; memType++)
        {
            ...
            for (locationIter=0; locationsArr[locationIter] != NULL; locationIter++)
            {
                    ...
                    locationPtr->type == CPSS_DXCH_LOCATION_LOGICAL_INDEX_INFO_TYPE)
                {
                    switch (locationPtr->info.logicalEntryInfo.logicaTableInfo[0].logicalTableType)
                    {
                        case PRV_FALCON_LOGICAL_TABLES_AMBIGUOUS_PROTECTION_CASES_MAC:
                            ...
                            break;
                        case CPSS_DXCH_LOGICAL_TABLE_STREAM_E:
                        case CPSS_DXCH_LOGICAL_TABLE_STREAM_GATE_E:
                            if (PRV_CPSS_SIP_6_30_CHECK_MAC(dev))
                            {
                                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_VALUE, st, dev,
                                                            memType, locationIter);
                ...
                if (((PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E) ||
                      ...
                      (PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E))
                                        &&
        ...
        */
        ...
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            if(PRV_CPSS_SIP_6_30_CHECK_MAC(dev))
            {
                memType = CPSS_DXCH_DIAG_DATA_INTEGRITY_MEM_TYPE_EGF_EFT_LOCAL_MC_FIFO_0_E;
            }
            else
            {
...
*/
    ...
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(
        ...
        UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        for (memType = 0; memType < devLastMem; memType++)
        {
            ...
            for (locationIter=0; locationsArr[locationIter] != NULL; locationIter++)
            {
                ...
                /* IronMan mentioned below VLAN table has only Parity protected RAMs.
                   Other SIP_6 devices has mixture of Parity and ECC RAMs. */
                if (PRV_CPSS_SIP_6_CHECK_MAC(dev) &&
                    locationPtr->type == CPSS_DXCH_LOCATION_LOGICAL_INDEX_INFO_TYPE)
                {
                    switch (locationPtr->info.logicalEntryInfo.logicaTableInfo[0].logicalTableType)
...
*/
    ...
    locationsArr[3] = (CPSS_DXCH_LOCATION_SPECIFIC_INFO_STC*)NULL; /* last entry should be NULL */
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_XCAT3_E | UTF_AAS_E);
...
)
{
    ...
    /* first entry need more time for systems without System Reset because
       previous diag tests may create problem for TCAM Parity daemon */
    timeout = (ruleIdx > 1) ? WAIT_TIME : (2*WAIT_TIME);
