...
extern GT_STATUS prvCpssFalconTxqTailDropPoolMappingValidityCheck
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  tileNum
);

extern GT_STATUS  prvCpssSip6TxqUtilsPbSizeGet
(
    IN GT_U8 devNum,
...
UTF_TEST_CASE_MAC(cpssDxChPortTxQPoolModeSet)
{
/*
    1.1. Call with non-null enablePtr.
    Expected: GT_OK.
...
*/
    ...
    GT_STATUS   st    = GT_OK;
    GT_U8       dev;
    CPSS_PORT_TX_POOL_MODE_ENT     modes[] =
        {
            CPSS_PORT_TX_POOL_MODE_FOUR_LOSSLESS_E,
            CPSS_PORT_TX_POOL_MODE_ONE_LOSSY_TWO_LOSLESS_E,
            CPSS_PORT_TX_POOL_MODE_TWO_LOSSY_E
        };
    GT_U32 i,j,tmp =0;

    CPSS_PORT_TX_POOL_MODE_ENT modeGet;
    GT_U32  maxBufNum[2];/*pool 0 and pool 1 TD limit*/
    GT_U32  maxBufNumSet,maxBufNumGet,maxBufNumGetExp,pbSize;
    GT_BOOL                       enable[8];
    CPSS_DXCH_PFC_THRESHOLD_STC   thresholdCfg[9];
    CPSS_DXCH_PFC_HYSTERESIS_CONF_STC hysteresisCfg[8];
    ...
    GT_U32 notApplicableDevices =
        (UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_CH2_E | UTF_CH3_E  |
        UTF_XCAT_E | UTF_LION_E | UTF_XCAT2_E | UTF_LION2_E | UTF_BOBCAT2_E | UTF_XCAT3_E | UTF_AC5_E |
        UTF_CAELUM_E | UTF_ALDRIN_E|UTF_ALDRIN2_E | UTF_AC3X_E | UTF_BOBCAT3_E |UTF_AAS_E);
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
      for(j=0;j<2;j++)
      {
          st = cpssDxChPortTxSharedPoolLimitsGet(dev,j,maxBufNum+j,&tmp);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
      }

      maxBufNumSet = maxBufNum[0]>>1;

      /*set new value.use autocompute*/
      st = cpssDxChPortTxSharedPoolLimitsSet(dev,0,maxBufNumSet,tmp,GT_TRUE);
      UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

      st = cpssDxChPortTxSharedPoolLimitsGet(dev,1,&maxBufNumGetExp,&tmp);
      UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

      st =prvCpssSip6TxqUtilsPbSizeGet(dev,&pbSize);
      UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

      for(j=0;j<8;j++)
      {
          st = cpssDxChPortPfcGlobalTcThresholdGet(dev,j,enable+j,thresholdCfg+j,hysteresisCfg+j);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

          thresholdCfg[8].guaranteedThreshold =100;

          st = cpssDxChPortPfcGlobalTcThresholdSet(dev,j,enable[j],thresholdCfg+8,hysteresisCfg+j);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
      }
      ...
      {
      /*
               1.1. Call with new mode
               Expected: GT_OK.
            */
            st = cpssDxChPortTxQPoolModeSet(dev,modes[i]);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            if(modes[i]==CPSS_PORT_TX_POOL_MODE_FOUR_LOSSLESS_E)
            {
                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,3,3);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,4,2);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);

                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,5,1);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);

                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,6,0);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);
            }
            else if(modes[i]==CPSS_PORT_TX_POOL_MODE_ONE_LOSSY_TWO_LOSLESS_E)
            {
                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,3,3);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);
                ...
                /*pool 1 is reserved*/
                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,5,1);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, i);
            ...
            {
                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,3,1);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);

                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,4,1);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, i);
            ...
            st = cpssDxChPortTxQPoolModeGet(dev,&modeGet);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            UTF_VERIFY_EQUAL1_PARAM_MAC(modes[i], modeGet, dev);

            /*run validity*/
            st =prvCpssFalconTxqTailDropPoolMappingValidityCheck(dev,0);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            if(modeGet!=CPSS_PORT_TX_POOL_MODE_TWO_LOSSY_E)
            {
                st = cpssDxChPortTxSharedPoolLimitsSet(dev,1,maxBufNum[1],0,GT_FALSE);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_STATE, st, dev);

                st = cpssDxChPortTxSharedPoolLimitsGet(dev,1,&maxBufNumGet,&tmp);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

                 if(GT_TRUE==PRV_CPSS_DXCH_ERRATA_GET_MAC(
                    dev,PRV_CPSS_DXCH_FALCON_TAIL_DROP_ON_VIRTUAL_BUFFERS_E)
                    )
                {

                    /*should be 2*PB-Pool0 since we used autocompute*/
                    UTF_VERIFY_EQUAL1_PARAM_MAC(maxBufNumGet, 2*pbSize-maxBufNumSet, dev);
                 }
                 else
                 {
                    /*Autocompute disabled so it's should be max value*/
                    UTF_VERIFY_EQUAL1_PARAM_MAC(maxBufNumGet, 0xFFFFF/*max value*/, dev);
                 }

                for(j=0;j<8;j++)
                {
                    st = cpssDxChPortPfcGlobalTcThresholdGet(dev,j,enable+j,thresholdCfg+8,hysteresisCfg+j);
                    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
                    UTF_VERIFY_EQUAL1_PARAM_MAC(0,thresholdCfg[8].guaranteedThreshold,j);
                }
            }
            else
            {
                /*check that Pool1 is returned to original value*/
                 st = cpssDxChPortTxSharedPoolLimitsGet(dev,1,&maxBufNumGet,&tmp);
                 UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
                 UTF_VERIFY_EQUAL1_PARAM_MAC(maxBufNumGetExp,maxBufNumGet,dev);

                  /*check that global TC  is returned to original value*/
                 for(j=0;j<8;j++)
                 {
                     st = cpssDxChPortPfcGlobalTcThresholdGet(dev,j,enable+j,thresholdCfg+8,hysteresisCfg+j);
                     UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

                     UTF_VERIFY_EQUAL1_PARAM_MAC(100, thresholdCfg[8].guaranteedThreshold, j);
                 }
            }
            ...
            for(j=0;j<8;j++)
            {
                st = cpssDxChPortTxTcMapToSharedPoolSet(dev,j,0);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
            }
       }

       /*restore*/
       st = cpssDxChPortTxSharedPoolLimitsSet(dev,1,0,tmp,GT_FALSE);
       UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

       for(j=0;j<2;j++)
       {
           st = cpssDxChPortTxSharedPoolLimitsSet(dev,j,maxBufNum[j],tmp,GT_FALSE);
           UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
       }

       for(j=0;j<8;j++)
      {
          st = cpssDxChPortPfcGlobalTcThresholdSet(dev,j,enable[j],thresholdCfg+j,hysteresisCfg+j);
          UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
      }
    }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChPortTxQPoolModeSet(dev,modes[0]);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChPortTxQPoolModeGet)
{
/*
    1.1. Call with non-null enablePtr.
    Expected: GT_OK.
...
*/
    ...
    GT_STATUS   st    = GT_OK;
    GT_U8       dev;
    CPSS_PORT_TX_POOL_MODE_ENT mode;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
           ...
           Expected: GT_OK.
        */
        st = cpssDxChPortTxQPoolModeGet(dev, &mode);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        /*
           ...
           Expected: GT_BAD_PTR.
        */
        st = cpssDxChPortTxQPoolModeGet(dev, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PTR, st, "%d, enablePtr = NULL", dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChPortPfc)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxQPoolModeSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortTxQPoolModeGet)
