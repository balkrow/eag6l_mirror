...
UTF_TEST_CASE_MAC(cpssDxChIpfixManagerDataGet)
{
    GT_STATUS   st = GT_OK;
    GT_U8       dev;
    GT_PORT_GROUPS_BMP      portGroupsBmp = 1;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            st = cpssDxChIpfixManagerDataGet(dev, portGroupsBmp, flowId);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                                        "cpssDxChIpfixManagerDataGet failed, dev : %d", dev);
...
UTF_TEST_CASE_MAC(cpssDxChIpfixManagerDataGetAll)
{
    GT_STATUS   st = GT_OK;
    GT_U8       dev;
    CPSS_DXCH_IPFIX_MANAGER_ATTRIBUTES_STC  attributes;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChIpfixManagerDataGetAll(dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                                    "cpssDxChIpfixManagerDataGetAll, dev: %d", dev);
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChIpfixManager)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpfixManagerDataGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpfixManagerDataGetAll)
