...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(dev) &&
            !PRV_CPSS_PP_MAC(dev)->isGmDevice)
        {
            /* use used defined register from CNM.RFU unit */
            regAddr = 0x00000018 + prvCpssDxChHwUnitBaseAddrGet(dev,PRV_CPSS_DXCH_UNIT_CNM_RFU_E,NULL);
        }
        else
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            /* register '0' not exists in MG unit */
...
*/
       ...
       CPSS_CHANNEL_PEX_MBUS_E == PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(dev)->mngInterfaceType)
    {
        ...
        if (PRV_CPSS_SIP_7_CHECK_MAC(dev) &&
            !PRV_CPSS_PP_MAC(dev)->isGmDevice)
        {
            /* no register in the device hold the 0x11AB so let it read CNM.RFU unit
               and it will fail to get the 0x11AB */
            offset = 0x00000018 + prvCpssDxChHwUnitBaseAddrGet(dev,PRV_CPSS_DXCH_UNIT_CNM_RFU_E,NULL);
        }
        else
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            /* register '0x50' in MG unit */
...
*/
    ...
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_CH2_E | UTF_CH3_E | UTF_XCAT_E | UTF_XCAT3_E |
                                     ...
                                     UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_FALCON_E | UTF_IRONMAN_L_E );
...
*/
    ...
    CPSS_PP_FAMILY_TYPE_ENT         devFamily;
    GT_BOOL                         compBistSupported;

    /* TCAM BIST is not implemented in GM*/
    GM_NOT_SUPPORT_THIS_TEST_MAC;

    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_LION2_E | UTF_AAS_E);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        switch (devFamily)
        {
            ...
            case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
                compBistSupported = GT_TRUE;
                break;
            default:
                compBistSupported = GT_FALSE;
                break;
        }
        /*
            1.1. Call with memBistType [CPSS_DIAG_PP_MEM_BIST_PCL_TCAM_E /
        ...
        */
        ...
        /* call with memBistType = CPSS_DIAG_PP_MEM_BIST_PCL_TCAM_E */
        memBistType = CPSS_DIAG_PP_MEM_BIST_PCL_TCAM_E;
        bistTestType = CPSS_DIAG_PP_MEM_BIST_PURE_MEMORY_TEST_E;
        timeOut = 1000;
        ...
        if (compBistSupported)
        {
            bistTestType = CPSS_DIAG_PP_MEM_BIST_COMPARE_TEST_E;
            clearMemoryAfterTest = GT_FALSE;
            ...
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, memBistType);
    #else
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, memBistType,testsResultBmpPtr);
            if (PRV_CPSS_SIP_5_CHECK_MAC(dev))
            {
                clearMemoryAfterTest = GT_TRUE;
...
UTF_TEST_CASE_MAC(cpssDxChDiagDeviceVoltageGet)
{
    ...
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, notAppFamilyBmp | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E | UTF_FALCON_E);
    GM_NOT_SUPPORT_THIS_TEST_MAC;
...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_AC5X_E | UTF_HARRIER_E | UTF_AAS_E);
...
UTF_TEST_CASE_MAC(cpssDxChDiagPortGroupTablePrint)
{
    ...
    */
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CPSS_PP_ALL_SIP7_CNS);/*sip7 not ready yet for this test that called from LUA : deadlock_check_without_traffic */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            */
            ...
            {
                /* exclude tables not relevant to device */
                st = prvCpssDxChTableNumEntriesGet(dev, tableType, &numEntries);
                if ((st != GT_OK) || (numEntries == 0))
                {
                   continue;
                }
                /* get accurate table size - important for Policer memories */
                st = prvCpssDxChDiagDataIntegrityTableHwMaxIndexGet(
                    dev, tableType, &numEntries, &maxNumLines);
                if ((st != GT_OK) || (numEntries == 0))
                {
                   continue;
                }
