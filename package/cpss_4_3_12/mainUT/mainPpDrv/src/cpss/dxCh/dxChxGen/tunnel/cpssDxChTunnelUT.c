...
UTF_TEST_CASE_MAC(cpssDxChTunnelStartEntrySet)
{
    ...
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_AAS_E);
         ...
         (PRV_CPSS_SIP_5_20_CHECK_MAC(dev)==GT_TRUE))
    {
        ...
        */
        ...
        if (UTF_CPSS_PP_HW_INFO_E_ARCH_SUPPORTED_MAC(dev)) {
            ...
            config.ipv6Cfg.destIp.arIP[3] = 250;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: NON GT_OK
        */
        maxVlanId = UTF_CPSS_PP_MAX_VLAN_INDEX_CNS(dev);
        if (maxVlanId>BIT_16)
        {
            /* vlanId is configured as GT_U16, can not check more the 16 bits */
            UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PARAM, GT_OK);
        }
        else
        {
            /* for max value of 64K we can not do this check */
            if (maxVlanId!=BIT_16)
            {
                vlanId = (GT_U16)maxVlanId;
                st = cpssDxChTunnelStartEgessVlanTableServiceIdGet(dev, vlanId,&vlanServiceId);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        else
        {
            ...
            {
                vlanId = (GT_U16)maxVlanId;
                st = cpssDxChTunnelStartEgessVlanTableServiceIdSet(dev, vlanId, vlanServiceId);
                UTF_VERIFY_NOT_EQUAL1_PARAM_MAC(GT_OK, st, dev);
           }
        }
...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, (UTF_FALCON_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E));
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTunnelStartPortGroupEntrySet1)
{
    GT_STATUS               st = GT_OK;
    GT_U8                   dev;
    GT_U32                              lineIndex     = 0;
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, (UTF_CPSS_PP_E_ARCH_CNS & (~UTF_CPSS_PP_ALL_SIP7_CNS)));

    /* 1. Go over all active devices. */
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        if (PRV_CPSS_DXCH_PP_MAC(dev)->vportInfo.useVPortMode == GT_FALSE)
        {
            SKIP_TEST_MAC;
        }
        /*  1.1.1. */
        ...
        config.mplsVportCfg.tag0QosMarkMode  = CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E;
        config.mplsVportCfg.tag1QosMarkMode  = CPSS_DXCH_TUNNEL_START_VLAN_QOS_MARK_TUNNEL_BASED_E;
        config.mplsVportCfg.up0              = 1;
        config.mplsVportCfg.dei0             = GT_TRUE;
        config.mplsVportCfg.up1              = 3;
        config.mplsVportCfg.dei1             = GT_FALSE;
        config.mplsVportCfg.threadId         = 7;
        config.mplsVportCfg.analyzerIndex    = 15;
        config.mplsVportCfg.egressMirrorMode = CPSS_DXCH_MIRROR_EGRESS_TAIL_DROP_E;
        config.mplsVportCfg.rawLabels        = GT_TRUE;
        config.mplsVportCfg.mplsEthertypeSelect = 4;
        config.mplsVportCfg.cncPtr              = 1;
        config.mplsVportCfg.flowOrGcfProfileId  = 1;
        config.mplsVportCfg.flowOrGcfBits       = 1;
        config.mplsVportCfg.mplsLabelStackCfg.numOfLabels                = 15;
        config.mplsVportCfg.mplsLabelStackCfg.lastQosUniformLabel        = 1;
        config.mplsVportCfg.mplsLabelStackCfg.globaExplMappingTableIndex = 1;
        config.mplsVportCfg.mplsLabelStackCfg.lastTtlUniformLabel        = 1;
        config.mplsVportCfg.mplsLabelStackCfg.cwIndex                    = 1;
        config.mplsVportCfg.mplsLabelStackCfg.flowLabelInsertion         = GT_TRUE;

        for (idx = 0; idx < config.mplsVportCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode = 1;
            config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].label = 130+idx;
            config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].exp = 1;
            config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].ttl = 241+idx;
            config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = GT_FALSE;
        }

        st = cpssDxChTunnelStartPortGroupEntrySet(dev, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, lineIndex,
                                                  tunnelType, &config);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, lineIndex, tunnelType);

        st = cpssDxChTunnelStartPortGroupEntryGet(dev, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, lineIndex,
                                                  &tunnelTypeGet, &configGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,"cpssDxChTunnelStartPortGroupEntryGet: %d", dev);

        /* validation values */
        UTF_VERIFY_EQUAL1_STRING_MAC(tunnelType, tunnelTypeGet, "got another tunnelType then was set: %d", dev);

        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.tag0QosMarkMode, configGet.mplsVportCfg.tag0QosMarkMode,
                                     "got another Tag0 QosMark Mode than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.tag1QosMarkMode, configGet.mplsVportCfg.tag1QosMarkMode,
                                     "got another Tag1 QosMark Mode than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.up0, configGet.mplsVportCfg.up0,
                                     "got another UP0 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.dei0, configGet.mplsVportCfg.dei0,
                                     "got another DEI0 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.up1, configGet.mplsVportCfg.up1,
                                     "got another UP1 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.dei1, configGet.mplsVportCfg.dei1,
                                     "got another DEI1 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.threadId, configGet.mplsVportCfg.threadId,
                                     "got another Thread ID than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.analyzerIndex, configGet.mplsVportCfg.analyzerIndex, "got another AnalyzerIndex than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.egressMirrorMode, configGet.mplsVportCfg.egressMirrorMode, "got another EgressMirrorMode than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.rawLabels, configGet.mplsVportCfg.rawLabels, "got another RawLabels than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.mplsEthertypeSelect, configGet.mplsVportCfg.mplsEthertypeSelect, "got another MplsEthertypeSelect than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.cncPtr, configGet.mplsVportCfg.cncPtr, "got another cncPtr than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.flowOrGcfProfileId, configGet.mplsVportCfg.flowOrGcfProfileId, "got another flowOrGcfProfileId than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.flowOrGcfBits, configGet.mplsVportCfg.flowOrGcfBits, "got another flowOrGcfBits than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.numOfLabels, configGet.mplsVportCfg.mplsLabelStackCfg.numOfLabels, "got another numOfLabels than was set: %d", dev);
        for (idx = 0; idx < configGet.mplsVportCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         configGet.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         "got another ExpMappingMode than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         configGet.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         "got another Label value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         configGet.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         "got another Exp value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         configGet.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         "got another TTL value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         configGet.mplsVportCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         "got another EL/ELI Insertion flag than was set for Label%u: %d", idx, dev);
        }
    }
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTunnelStartHftPortGroupEntrySet)
{
    GT_STATUS               st = GT_OK;
    GT_U8                   dev;
    GT_U32                              lineIndex     = 0;
    CPSS_DXCH_HFT_ENTRY_TYPE_ENT        hftEntryType;
    CPSS_DXCH_HFT_ENTRY_TYPE_ENT        hftEntryTypeGet;
    CPSS_DXCH_HFT_CONFIG_UNT            config;
    CPSS_DXCH_HFT_CONFIG_UNT            configGet;
    GT_U32                              idx;
    CPSS_DXCH_HFT_TABLE_ENT             hftTableId;

    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, ((UTF_CPSS_PP_E_ARCH_CNS | UTF_AC5_E | UTF_XCAT3_E) & (~UTF_CPSS_PP_ALL_SIP7_CNS)));
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        hftTableId = CPSS_DXCH_HFT_TABLE_0_E;
        /* Call with HFT type MPLS */
        hftEntryType = CPSS_DXCH_HFT_MPLS_E;
        lineIndex  = 10;

        config.mplsLabelCfg.hftCommonCfg.subType = 0;
        config.mplsLabelCfg.hftCommonCfg.analyzerIndex = 15;
        config.mplsLabelCfg.hftCommonCfg.egressMirrorMode = CPSS_DXCH_MIRROR_EGRESS_TAIL_DROP_E;
        config.mplsLabelCfg.hftCommonCfg.subThreadId      = 7;
        config.mplsLabelCfg.hftCommonCfg.cncPtr           = 1;
        config.mplsLabelCfg.hftCommonCfg.gcfProfileId     = 1;
        config.mplsLabelCfg.hftCommonCfg.gcfBits0To4      = 1;
        config.mplsLabelCfg.mplsLabelStackCfg.numOfLabels                = 15;
        config.mplsLabelCfg.mplsLabelStackCfg.lastQosUniformLabel        = 1;
        config.mplsLabelCfg.mplsLabelStackCfg.globaExplMappingTableIndex = 1;
        config.mplsLabelCfg.mplsLabelStackCfg.lastTtlUniformLabel        = 1;
        config.mplsLabelCfg.mplsLabelStackCfg.cwIndex                    = 1;
        config.mplsLabelCfg.mplsLabelStackCfg.flowLabelInsertion         = GT_TRUE;

        for (idx = 0; idx < config.mplsLabelCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode = 1;
            config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].label = 130+idx;
            config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].exp = 1;
            config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].ttl = 241+idx;
            config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = GT_FALSE;
        }

        st = cpssDxChTunnelStartHftPortGroupEntrySet(dev, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, lineIndex,
                                                     hftTableId, hftEntryType, &config);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, lineIndex, hftEntryType);

        st = cpssDxChTunnelStartHftPortGroupEntryGet(dev, CPSS_PORT_GROUP_UNAWARE_MODE_CNS, lineIndex,
                                                     hftTableId, &hftEntryTypeGet, &configGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,"cpssDxChTunnelStartHftPortGroupEntryGet: %d", dev);

        /* validation values */
        UTF_VERIFY_EQUAL1_STRING_MAC(hftEntryType, hftEntryTypeGet, "got another hftEntryType then was set: %d", dev);

        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.subType, configGet.mplsLabelCfg.hftCommonCfg.subType,
                                     "got another Sub Type than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.analyzerIndex, configGet.mplsLabelCfg.hftCommonCfg.analyzerIndex,
                                     "got another Analyzer Index than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.egressMirrorMode, configGet.mplsLabelCfg.hftCommonCfg.egressMirrorMode,
                                     "got another EgressMirrorMode than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.subThreadId, configGet.mplsLabelCfg.hftCommonCfg.subThreadId,
                                     "got another Sub Thread ID than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.cncPtr, configGet.mplsLabelCfg.hftCommonCfg.cncPtr,
                                     "got another cncPtr than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.gcfProfileId, configGet.mplsLabelCfg.hftCommonCfg.gcfProfileId,
                                     "got another gcfProfileId than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.hftCommonCfg.gcfBits0To4, configGet.mplsLabelCfg.hftCommonCfg.gcfBits0To4,
                                     "got another gcfBits0To4 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.numOfLabels, configGet.mplsLabelCfg.mplsLabelStackCfg.numOfLabels, "got another numOfLabels than was set: %d", dev);
        for (idx = 0; idx < configGet.mplsLabelCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         configGet.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         "got another ExpMappingMode than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         configGet.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         "got another Label value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         configGet.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         "got another Exp value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         configGet.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         "got another TTL value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         configGet.mplsLabelCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         "got another EL/ELI Insertion flag than was set for Label%u: %d", idx, dev);
        }
        /* Call with HFT type MPLS */
        hftEntryType = CPSS_DXCH_HFT_MPLS_EXT_E;
        lineIndex  = 11;

        config.mplsLabelExtCfg.hftCommonCfg.subType = 0;
        config.mplsLabelExtCfg.hftCommonCfg.analyzerIndex = 15;
        config.mplsLabelExtCfg.hftCommonCfg.egressMirrorMode = CPSS_DXCH_MIRROR_EGRESS_TAIL_DROP_E;
        config.mplsLabelExtCfg.hftCommonCfg.subThreadId      = 7;
        config.mplsLabelExtCfg.hftCommonCfg.cncPtr           = 1;
        config.mplsLabelExtCfg.hftCommonCfg.gcfProfileId     = 1;
        config.mplsLabelExtCfg.hftCommonCfg.gcfBits0To4      = 1;
        config.mplsLabelExtCfg.gcfBits5To47[0]               = 7;
        config.mplsLabelExtCfg.gcfBits5To47[1]               = 1;
        config.mplsLabelExtCfg.flowId                        = 1;
        config.mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels                = 15;
        config.mplsLabelExtCfg.mplsLabelStackCfg.lastQosUniformLabel        = 1;
        config.mplsLabelExtCfg.mplsLabelStackCfg.globaExplMappingTableIndex = 1;
        config.mplsLabelExtCfg.mplsLabelStackCfg.lastTtlUniformLabel        = 1;
        config.mplsLabelExtCfg.mplsLabelStackCfg.cwIndex                    = 1;
        config.mplsLabelExtCfg.mplsLabelStackCfg.flowLabelInsertion         = GT_TRUE;

        for (idx = 0; idx < config.mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode = 1;
            config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].label = 130+idx;
            config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].exp = 1;
            config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].ttl = 241+idx;
            config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion = GT_FALSE;
        }
        ...
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.subType,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.subType,
                                     "got another Sub Type than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.analyzerIndex,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.analyzerIndex,
                                     "got another Analyzer Index than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.egressMirrorMode,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.egressMirrorMode,
                                     "got another EgressMirrorMode than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.subThreadId,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.subThreadId,
                                     "got another Sub Thread ID than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.cncPtr,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.cncPtr,
                                     "got another cncPtr than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.gcfProfileId,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.gcfProfileId,
                                     "got another gcfProfileId than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.hftCommonCfg.gcfBits0To4,
                                     configGet.mplsLabelExtCfg.hftCommonCfg.gcfBits0To4,
                                     "got another gcfBits0To4 than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.gcfBits5To47[0],
                                     configGet.mplsLabelExtCfg.gcfBits5To47[0],
                                     "got another gcfBits5To47[0] than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.gcfBits5To47[1],
                                     configGet.mplsLabelExtCfg.gcfBits5To47[1],
                                     "got another gcfBits5To47[1] than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.flowId,
                                     configGet.mplsLabelExtCfg.flowId,
                                     "got another Flow ID than was set: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels,
                                     configGet.mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels,
                                     "got another numOfLabels than was set: %d", dev);
        for (idx = 0; idx < configGet.mplsLabelExtCfg.mplsLabelStackCfg.numOfLabels; idx++)
        {
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         configGet.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].expMappingMode,
                                         "got another ExpMappingMode than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         configGet.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].label,
                                         "got another Label value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         configGet.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].exp,
                                         "got another Exp value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         configGet.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].ttl,
                                         "got another TTL value than was set for Label%u: %d", idx, dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(config.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         configGet.mplsLabelExtCfg.mplsLabelStackCfg.mplsLabels[idx].isElOrEliInsertion,
                                         "got another EL/ELI Insertion flag than was set for Label%u: %d", idx, dev);
        }
    }
    /* 2. For not-active devices and devices from non-applicable family */
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChTunnel)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTunnelStartPortGroupEntrySet1)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTunnelStartHftPortGroupEntrySet)
    ...
    switch(tunnelType)
        ...
        case CPSS_TUNNEL_GENERIC_IPV4_E:
               ...
               (CPSS_TUNNEL_GENERIC_IPV4_E == tunnelType))
            {
                ...
                /* sip7 do not support retainCRC field */
