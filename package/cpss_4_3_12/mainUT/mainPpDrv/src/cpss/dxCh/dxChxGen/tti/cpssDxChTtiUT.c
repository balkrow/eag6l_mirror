...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        {
            entryIndex = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? 256 : 128;
        }
        else
        {
            entryIndex = 8;
...
*/
    ...
    CPSS_DXCH_TTI_KEY_TYPE_ENT  keyType = CPSS_DXCH_TTI_KEY_IPV4_E;
    CPSS_DXCH_TTI_KEY_TCAM_SEGMENT_MODE_ENT segmentMode = CPSS_DXCH_TTI_KEY_TCAM_SEGMENT_MODE_ONE_TCAM_E;
...
UTF_TEST_CASE_MAC(cpssDxChTtiIpToMeSet)
{
    ...
    /* prepare iterator for go over all active devices */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&devNum, (UTF_IRONMAN_L_E | UTF_AAS_E));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        interfaceInfo.includeSrcInterface   = CPSS_DXCH_TTI_IP_TO_ME_DONT_USE_SRC_INTERFACE_FIELDS_E;
        ...
        */
        ...
        /*
            ...
            Expected: NOT GT_OK.
        */
        entryIndex = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tti.TTI_ip2me_numEntries+1;
        ...
        */
        ...
        /*           out of range interfaceInfo.srcDevice (not relevant) and                                            */
        /*           other valid params.                                                                                */
        interfaceInfo.includeSrcInterface   = CPSS_DXCH_TTI_IP_TO_ME_USE_SRC_INTERFACE_FIELDS_EXCLUDE_SRC_DEVICE_E;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
                ...
                direction ++)
        {
            for (ii = 0; ii < sizeof(lookupNumArr)/sizeof(lookupNumArr[0]); ii++)
            {
                ...
                for(genericActionMode = CPSS_DXCH_TTI_GENERIC_ACTION_MODE_DST_EPG_E ;
...
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeSrcPortProfileIdSet)
{

    GT_STATUS                           st = GT_OK;
    GT_U8                               devNum;
    GT_PORT_NUM                         portNum;
    GT_U32                              srcPortProfileId;
    GT_U32                              srcPortProfileIdGet;

    /* this feature is on sip7 devices */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        {
            srcPortProfileId = 0;
            st = cpssDxChTtiGenericRangeSrcPortProfileIdSet(devNum, portNum, srcPortProfileId);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, srcPortProfileId);

            st = cpssDxChTtiGenericRangeSrcPortProfileIdGet(devNum, portNum, &srcPortProfileIdGet);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,
                "srcPortProfileIdGet: %d, %d", devNum, portNum);
            UTF_VERIFY_EQUAL2_STRING_MAC(srcPortProfileId, srcPortProfileIdGet,
                "got another srcPortProfileId then was set: %d, %d", devNum, portNum);

            srcPortProfileId = BIT_10-1;
            st = cpssDxChTtiGenericRangeSrcPortProfileIdSet(devNum, portNum, srcPortProfileId);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, srcPortProfileId);
            ...
            srcPortProfileId = BIT_10;
            st = cpssDxChTtiGenericRangeSrcPortProfileIdSet(devNum, portNum, srcPortProfileId);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum, portNum, srcPortProfileId);
...
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeSrcPortProfileIdGet)
{
    GT_STATUS                           st = GT_OK;
    GT_U8                               devNum;
    GT_PORT_NUM                         portNum;
    GT_U32                              srcPortProfileIdPtr;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        while (GT_OK == prvUtfNextDefaultEPortGet(&portNum, GT_TRUE))
        {
            st = cpssDxChTtiGenericRangeSrcPortProfileIdGet(devNum, portNum, &srcPortProfileIdPtr);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, srcPortProfileIdPtr);

            st = cpssDxChTtiGenericRangeSrcPortProfileIdGet(devNum, portNum, NULL);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, srcPortProfileIdPtr = NULL",devNum, portNum);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeBytePairCfgSet)
{

    GT_STATUS                                       st = GT_OK;
    GT_U8                                           devNum;

    GT_U32                                          bytePairIndex = 0;
    CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC     bytePairCfg;
    CPSS_DXCH_TTI_RANGE_CHECK_BYTE_PAIR_CFG_STC     bytePairCfgGet;

    cpssOsBzero((GT_VOID*) &bytePairCfg,                  sizeof(bytePairCfg));
    cpssOsBzero((GT_VOID*) &bytePairCfgGet,               sizeof(bytePairCfgGet));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        bytePairCfg.anchorType = CPSS_DXCH_TTI_OFFSET_L2_E;
        bytePairCfg.byteOffset = 12;
        bytePairCfg.enable = GT_TRUE;
        bytePairCfg.mask[0] = 0xFF;
        bytePairCfg.mask[1] = 0x11;

        st = cpssDxChTtiGenericRangeBytePairCfgSet(devNum, bytePairIndex, &bytePairCfg);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, bytePairIndex);

        /*
            1.2. Call cpssDxChTtiGenericRangeBytePairCfgGet.
            Expected: GT_OK and the same bytePairCfg.
        */
        st = cpssDxChTtiGenericRangeBytePairCfgGet(devNum, bytePairIndex, &bytePairCfgGet);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericRangeBytePairCfgGet: %d, %d", devNum, bytePairIndex);

        UTF_VERIFY_EQUAL2_STRING_MAC(bytePairCfg.anchorType, bytePairCfgGet.anchorType,
            "get another bytePairCfg.anchorType than was set: %d, %d", devNum, bytePairIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(bytePairCfg.byteOffset, bytePairCfgGet.byteOffset,
            "get another bytePairCfg.byteOffset than was set: %d, %d", devNum, bytePairIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(bytePairCfg.enable, bytePairCfgGet.enable,
            "get another bytePairCfg.enable than was set: %d, %d", devNum, bytePairIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(bytePairCfg.mask[0], bytePairCfgGet.mask[0],
            "get another bytePairCfg.mask[0] than was set: %d, %d", devNum, bytePairIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(bytePairCfg.mask[1], bytePairCfgGet.mask[1],
            "get another bytePairCfg.mask[1] than was set: %d, %d", devNum, bytePairIndex);
        /*
            1.3. Call with valid values.
            Expected: GT_OK.
        */
        ...
        bytePairCfg.anchorType = CPSS_DXCH_TTI_OFFSET_L3_MINUS_2_E;
        bytePairCfg.byteOffset = 255;
        bytePairCfg.enable = GT_TRUE;
        bytePairCfg.mask[0] = 0xFF;
        bytePairCfg.mask[1] = 0xFF;

        st = cpssDxChTtiGenericRangeBytePairCfgSet(devNum, bytePairIndex, &bytePairCfg);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, bytePairIndex);

        /*
            1.4. Call cpssDxChTtiGenericRangeBytePairCfgGet.
            Expected: GT_OK and the same bytePairCfg.
        */
        st = cpssDxChTtiGenericRangeBytePairCfgGet(devNum, bytePairIndex, &bytePairCfgGet);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericRangeBytePairCfgGet: %d, %d", devNum, bytePairIndex);
        ...
        /*
            1.5. Call with out of range bytePairIndex and other valid params.
            Expected: NOT GT_OK.
        */
        bytePairIndex = 2;

        st = cpssDxChTtiGenericRangeBytePairCfgSet(devNum, bytePairIndex, &bytePairCfg);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, devNum, bytePairIndex);

        /* Restore valid params. */
        bytePairIndex = 0;

        /*
            1.6. Call with wrong bytePairCfg.anchorType enum values and
                           other valid params.
            Expected: GT_BAD_PARAM.
        */
        UTF_ENUMS_CHECK_MAC(cpssDxChTtiGenericRangeBytePairCfgSet
                            (devNum, bytePairIndex, &bytePairCfg),
                             bytePairCfg.anchorType);


        /*
            1.7. Call with NULL bytePairCfgPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericRangeBytePairCfgSet(devNum, bytePairIndex, NULL);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, valuePtr = NULL",devNum, bytePairIndex);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeBytePairCfgGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            1.1. Call with bytePairIndex [0 / 1] and other valid params.
            Expected: GT_OK.
        */
        /* iterate with bytePairIndex = 0 */
        bytePairIndex = 0;

        st = cpssDxChTtiGenericRangeBytePairCfgGet(devNum, bytePairIndex, &bytePairCfg);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, bytePairIndex);

        /* iterate with bytePairIndex = 1 */
        bytePairIndex = 1;
        ...
        /*
            1.2. Call with bad param bytePairIndex and other valid params.
            Expected: NOT GT_OK.
        */
        bytePairIndex = 2;
        ...
        /*
            1.3. Call with NULL bytePairCfgPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericRangeBytePairCfgGet(devNum, bytePairIndex, NULL);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, valuePtr = NULL",devNum, bytePairIndex);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeParamsSet)
{

    GT_STATUS                                       st = GT_OK;
    GT_U8                                           devNum;

    GT_U32                                          rangeClassifictionIndex = 0;
    CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC rangeClassifictionParams;
    CPSS_DXCH_GENERIC_RANGE_CLASSIFICATION_PARAMS_STC rangeClassifictionParamsGet;

    cpssOsBzero((GT_VOID*) &rangeClassifictionParams,                  sizeof(rangeClassifictionParams));
    cpssOsBzero((GT_VOID*) &rangeClassifictionParamsGet,               sizeof(rangeClassifictionParamsGet));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            ...
            Expected: GT_OK.
        */
        rangeClassifictionIndex = 1;

        rangeClassifictionParams.lookupIndex = 0;
        rangeClassifictionParams.matchId = 3;
        rangeClassifictionParams.maxValue = 0x222;
        rangeClassifictionParams.minValue = 0x111;
        rangeClassifictionParams.profileId = 5;
        rangeClassifictionParams.profileMode = CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_PORT_PROFILE_E;
        rangeClassifictionParams.valid = GT_TRUE;

        st = cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, &rangeClassifictionParams);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, rangeClassifictionIndex);

        /*
            1.2. Call cpssDxChTtiGenericRangeParamsGet.
            Expected: GT_OK and the same rangeClassifictionParams.
        */
        st = cpssDxChTtiGenericRangeParamsGet(devNum, rangeClassifictionIndex, &rangeClassifictionParamsGet);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericRangeParamsGet: %d, %d", devNum, rangeClassifictionIndex);

        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.lookupIndex, rangeClassifictionParamsGet.lookupIndex,
            "get another rangeClassifictionParams.lookupIndex than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.matchId, rangeClassifictionParamsGet.matchId,
            "get another rangeClassifictionParams.matchId than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.maxValue, rangeClassifictionParamsGet.maxValue,
            "get another rangeClassifictionParams.maxValue than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.minValue, rangeClassifictionParamsGet.minValue,
            "get another rangeClassifictionParams.minValue than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.profileId, rangeClassifictionParamsGet.profileId,
            "get another rangeClassifictionParams.profileId than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.profileMode, rangeClassifictionParamsGet.profileMode,
            "get another rangeClassifictionParams.profileMode than was set: %d, %d", devNum, rangeClassifictionIndex);
        UTF_VERIFY_EQUAL2_STRING_MAC(rangeClassifictionParams.valid, rangeClassifictionParamsGet.valid,
            "get another rangeClassifictionParams.valid than was set: %d, %d", devNum, rangeClassifictionIndex);

        /*
            1.3. Call with valid values.
            Expected: GT_OK.
        */
        rangeClassifictionIndex = 127;

        rangeClassifictionParams.lookupIndex = 1;
        rangeClassifictionParams.matchId = 255;
        rangeClassifictionParams.maxValue = 0xFFFF;
        rangeClassifictionParams.minValue = 0;
        rangeClassifictionParams.profileId = 1023;
        rangeClassifictionParams.profileMode = CPSS_DXCH_GENERIC_RANGE_PROFILE_MODE_NOT_USED_E;
        rangeClassifictionParams.valid = GT_TRUE;
        ...
        /*
            1.4. Call cpssDxChTtiGenericRangeParamsGet.
            Expected: GT_OK and the same rangeClassifictionParams.
        */
        st = cpssDxChTtiGenericRangeParamsGet(devNum, rangeClassifictionIndex, &rangeClassifictionParamsGet);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericRangeParamsGet: %d, %d", devNum, rangeClassifictionIndex);
        ...
        /*
            1.5. Call with out of range rangeClassifictionIndex and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionIndex = 128;

        st = cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, &rangeClassifictionParams);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, devNum, rangeClassifictionIndex);

        /* Restore valid params. */
        rangeClassifictionIndex = 1;

        /*
            1.6. Call with bad param ClassifictionParams.lookupIndex and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionParams.lookupIndex = 2;
        ...
        /*
            1.7. Call with out of range ClassifictionParams.matchId and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionParams.matchId = 256;

        st = cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, &rangeClassifictionParams);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum, rangeClassifictionIndex);

        /* Restore correct values. */
        rangeClassifictionParams.matchId = 255;

        /*
            1.8. Call with out of range ClassifictionParams.maxValue and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionParams.maxValue = 0x1FFFF;
        ...
        /*
            1.9. Call with out of range ClassifictionParams.minValue and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionParams.minValue = 0x1FFFF;

        st = cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, &rangeClassifictionParams);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum, rangeClassifictionIndex);

        /* Restore correct values. */
        rangeClassifictionParams.minValue = 0xFFFF;

        /*
            1.10. Call with bad param ClassifictionParams.profileId and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionParams.profileId = 1024;
        ...
        /*
            1.11. Call with wrong range ClassifictionParams.profileMode enum values and
                           other valid params.
            Expected: GT_BAD_PARAM.
        */
        UTF_ENUMS_CHECK_MAC(cpssDxChTtiGenericRangeParamsSet
                            (devNum, rangeClassifictionIndex, &rangeClassifictionParams),
                             rangeClassifictionParams.profileMode);

        /*
            1.12. Call with NULL rangeClassifictionParamsPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericRangeParamsSet(devNum, rangeClassifictionIndex, NULL);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, valuePtr = NULL",devNum, rangeClassifictionIndex);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericRangeParamsGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            1.1. Call with rangeClassifictionIndex [1 / 127] and other valid params.
            Expected: GT_OK.
        */
        /* iterate with rangeClassifictionIndex = 1 */
        rangeClassifictionIndex = 1;

        st = cpssDxChTtiGenericRangeParamsGet(devNum, rangeClassifictionIndex, &rangeClassifictionParams);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, rangeClassifictionIndex);

        /* iterate with bytePairIndex = 127 */
        rangeClassifictionIndex = 127;
        ...
        /*
            1.2. Call with bad param rangeClassifictionIndex and other valid params.
            Expected: NOT GT_OK.
        */
        rangeClassifictionIndex = 0;
        ...
        /*
            1.3. Call with NULL rangeClassifictionParamsPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericRangeParamsGet(devNum, rangeClassifictionIndex, NULL);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, valuePtr = NULL",devNum, rangeClassifictionIndex);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationLsbLocationSet)
{

    GT_STATUS                                       st = GT_OK;
    GT_U8                                           devNum;

    GT_U32                                          lsbLocation = 0;
    GT_U32                                          lsbLocationGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        st = cpssDxChTtiGenericClassificationLsbLocationSet(devNum, lsbLocation);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.2. Call cpssDxChTtiGenericClassificationLsbLocationGet.
            Expected: GT_OK and the same lsbLocation.
        */
        st = cpssDxChTtiGenericClassificationLsbLocationGet(devNum, &lsbLocationGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericClassificationLsbLocationGet: %d", devNum);

        UTF_VERIFY_EQUAL1_STRING_MAC(lsbLocation, lsbLocationGet,
            "get another lsbLocation than was set: %d, %d", devNum);
        ...
        */
        ...
        /*
            1.4. Call cpssDxChTtiGenericClassificationLsbLocationGet.
            Expected: GT_OK and the same lsbLocation.
        */
        st = cpssDxChTtiGenericClassificationLsbLocationGet(devNum, &lsbLocationGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericClassificationLsbLocationGet: %d", devNum);
        ...
        /*
            1.5. Call with out of range lsbLocation and other valid params.
            Expected: NOT GT_OK.
        */
        lsbLocation = 128;
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationLsbLocationGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChTtiGenericClassificationLsbLocationGet(devNum, &lsbLocation);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.2. Call with NULL lsbLocationPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericClassificationLsbLocationGet(devNum, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PTR, st, "%d, valuePtr = NULL",devNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationDefaultValueSet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        {
            defaultValue = 0;
            st = cpssDxChTtiGenericClassificationDefaultValueSet(devNum, portNum, defaultValue);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, defaultValue);

            st = cpssDxChTtiGenericClassificationDefaultValueGet(devNum, portNum, &defaultValueGet);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,
                "cpssDxChTtiGenericClassificationDefaultValueGet: %d, %d", devNum, portNum);
            UTF_VERIFY_EQUAL2_STRING_MAC(defaultValue, defaultValueGet,
                "got another defaultValue then was set: %d, %d", devNum, portNum);

            defaultValue = BIT_10-1;
            st = cpssDxChTtiGenericClassificationDefaultValueSet(devNum, portNum, defaultValue);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, defaultValue);
            ...
            defaultValue = BIT_10;
            st = cpssDxChTtiGenericClassificationDefaultValueSet(devNum, portNum, defaultValue);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum, portNum, defaultValue);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationDefaultValueGet)
{
    GT_STATUS                           st = GT_OK;
    GT_U8                               devNum;
    GT_PORT_NUM                         portNum;
    GT_U32                              defaultValuePtr;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        while (GT_OK == prvUtfNextDefaultEPortGet(&portNum, GT_TRUE))
        {
            st = cpssDxChTtiGenericClassificationDefaultValueGet(devNum, portNum, &defaultValuePtr);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, portNum, defaultValuePtr);

            st = cpssDxChTtiGenericClassificationDefaultValueGet(devNum, portNum, NULL);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, defaultValuePtr = NULL",devNum, portNum);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationParamsSet)
{
    ...
    CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC genClassifictionParams;
    CPSS_DXCH_GENERIC_CLASSIFICATION_PARAMS_STC genClassifictionParamsGet;

    cpssOsBzero((GT_VOID*) &genClassifictionParams,                  sizeof(genClassifictionParams));
    cpssOsBzero((GT_VOID*) &genClassifictionParamsGet,               sizeof(genClassifictionParamsGet));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            ...
            Expected: GT_OK.
        */
        genClassifictionParams.lsbLocation[0] = 0;
        genClassifictionParams.lsbLocation[1] = 1;
        genClassifictionParams.lsbLocation[2] = 15;
        genClassifictionParams.lsbLocation[3] = 16;
        genClassifictionParams.mask[0] = 0;
        genClassifictionParams.mask[1] = 0xFF;
        genClassifictionParams.mask[2] = 0xFF;
        genClassifictionParams.mask[3] = 0;
        genClassifictionParams.mask[4] = 0;
        genClassifictionParams.mask[5] = 0x11;
        genClassifictionParams.mask[6] = 0x22;
        genClassifictionParams.mask[7] = 0x33;


        st = cpssDxChTtiGenericClassificationParamsSet(devNum, &genClassifictionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.2. Call cpssDxChTtiGenericClassificationParamsGet.
            Expected: GT_OK and the same genClassifictionParams.
        */
        st = cpssDxChTtiGenericClassificationParamsGet(devNum, &genClassifictionParamsGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,"cpssDxChTtiGenericRangeParamsGet: %d", devNum);

        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.lsbLocation[0], genClassifictionParamsGet.lsbLocation[0],
            "get another genClassifictionParams.lsbLocation[0] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.lsbLocation[1], genClassifictionParamsGet.lsbLocation[1],
            "get another genClassifictionParams.lsbLocation[1] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.lsbLocation[2], genClassifictionParamsGet.lsbLocation[2],
            "get another genClassifictionParams.lsbLocation[2] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.lsbLocation[3], genClassifictionParamsGet.lsbLocation[3],
            "get another genClassifictionParams.lsbLocation[3] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[0], genClassifictionParamsGet.mask[0],
            "get another genClassifictionParams.mask[0] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[1], genClassifictionParamsGet.mask[1],
            "get another genClassifictionParams.mask[1] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[2], genClassifictionParamsGet.mask[2],
            "get another genClassifictionParams.mask[2] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[3], genClassifictionParamsGet.mask[3],
            "get another genClassifictionParams.mask[3] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[4], genClassifictionParamsGet.mask[4],
            "get another genClassifictionParams.mask[4] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[5], genClassifictionParamsGet.mask[5],
            "get another genClassifictionParams.mask[5] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[6], genClassifictionParamsGet.mask[6],
            "get another genClassifictionParams.mask[6] than was set: %d", devNum);
        UTF_VERIFY_EQUAL1_STRING_MAC(genClassifictionParams.mask[7], genClassifictionParamsGet.mask[7],
            "get another genClassifictionParams.mask[7] than was set: %d", devNum);

        for (i=0; i<4; i++)
        {
            /*
                1.3. Call with out of range genClassifictionParams.lsbLocation[0] and other valid params.
                Expected: NOT GT_OK.
            */
            genClassifictionParams.lsbLocation[i] = 33;

            st = cpssDxChTtiGenericClassificationParamsSet(devNum, &genClassifictionParams);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

            /* Restore valid params. */
            genClassifictionParams.lsbLocation[i] = 1;
        }

        /*
            1.4. Call with NULL genClassifictionParamsPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericClassificationParamsSet(devNum, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PTR, st, "%d, genClassifictionParamsPtr = NULL",devNum);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiGenericClassificationParamsGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChTtiGenericClassificationParamsGet(devNum, &genClassifictionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.2. Call with NULL genClassifictionParamsPtr and other valid params.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChTtiGenericClassificationParamsGet(devNum, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PTR, st, "%d, genClassifictionParamsPtr = NULL",devNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiEliExceptionCmdSet)
{

    GT_STATUS                 st = GT_OK;
    GT_U8                     devNum;
    CPSS_PACKET_CMD_ENT       boSExcpCmd;
    CPSS_NET_RX_CPU_CODE_ENT  boSExcpDropCode;
    CPSS_PACKET_CMD_ENT       boSExcpCmdGet;
    CPSS_NET_RX_CPU_CODE_ENT  boSExcpDropCodeGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        boSExcpCmd = CPSS_PACKET_CMD_DROP_SOFT_E;
        boSExcpDropCode = CPSS_NET_EGRESS_MIRROR_TO_CPU_E;
        /*
            1.1. Call with valid values.
            Expected: GT_OK.
        */
        st = cpssDxChTtiEliExceptionCmdSet(devNum, boSExcpCmd, boSExcpDropCode);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.2. Get the configured params and compare.
            Expected: GT_OK.
        */
        st = cpssDxChTtiEliExceptionCmdGet(devNum, &boSExcpCmdGet, &boSExcpDropCodeGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        if(boSExcpCmd != boSExcpCmdGet)
        {
            PRV_UTF_LOG2_MAC("Got another boSExcpCmd than was set, expected = %u, received = %u\n",
                             boSExcpCmd, boSExcpCmdGet);
            st = GT_FAIL;
        }
        if(boSExcpDropCode != boSExcpDropCodeGet)
        {
            PRV_UTF_LOG2_MAC("Got another boSExcpDropCode than was set, expected = %u, received = %u\n",
                             boSExcpDropCode, boSExcpDropCodeGet);
            st = GT_FAIL;
        }
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /* 1.3 Negative Scenarios*/
        boSExcpCmd = CPSS_PACKET_CMD_ROUTE_E;
        st = cpssDxChTtiEliExceptionCmdSet(devNum, boSExcpCmd, boSExcpDropCode);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        boSExcpCmd = CPSS_PACKET_CMD_DROP_SOFT_E;
        boSExcpDropCode = CPSS_NET_LOCK_PORT_MIRROR_E;
        st = cpssDxChTtiEliExceptionCmdSet(devNum, boSExcpCmd, boSExcpDropCode);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);
...
(
    IN  GT_U8                          devNum,
    IN  CPSS_PACKET_CMD_ENT            *boSExcpCmdPtr,
    IN  CPSS_NET_RX_CPU_CODE_ENT       *boSExcpDropCodePtr
);
*/
UTF_TEST_CASE_MAC(cpssDxChTtiEliExceptionCmdGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        /* 1.3 Negative Scenarios*/
        st = cpssDxChTtiEliExceptionCmdGet(devNum, NULL, NULL);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiNonForwardingLabelQosAttributesSet)
{

    GT_STATUS                 st = GT_OK;
    GT_U8                     devNum;
    CPSS_DXCH_TTI_NON_FORWARDING_LABEL_TYPE_ENT        labelType;
    CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC  qosParams;
    CPSS_DXCH_TTI_NON_FORWARDING_LABEL_QOS_PARAMS_STC  qosParamsGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        labelType                      = CPSS_DXCH_TTI_ELI_LABEL_E;
        qosParams.qosModel             = CPSS_DXCH_TTI_QOS_SHORT_PIPE_MODEL_E;
        qosParams.ttlModel             = CPSS_DXCH_TTI_TTL_PIPE_MODEL_E;
        qosParams.qosMappingTableIndex = 126;
        qosParams.trustExpFlag         = GT_TRUE;
        qosParams.trustUpFlag          = CPSS_DXCH_TTI_TRUST_L2_UP_E;
        /*
            1.1. Call with valid values for ELI Label
            Expected: GT_OK.
        */
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
        ...
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChTtiNonForwardingLabelQosAttributesGet(devNum, labelType, &qosParamsGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
        UTF_VERIFY_EQUAL1_PARAM_MAC(qosParams.qosModel, qosParamsGet.qosModel, devNum);
        UTF_VERIFY_EQUAL1_PARAM_MAC(qosParams.ttlModel, qosParamsGet.ttlModel, devNum);
        UTF_VERIFY_EQUAL1_PARAM_MAC(qosParams.qosMappingTableIndex, qosParamsGet.qosMappingTableIndex, devNum);
        UTF_VERIFY_EQUAL1_PARAM_MAC(qosParams.trustExpFlag, qosParamsGet.trustExpFlag, devNum);
        UTF_VERIFY_EQUAL1_PARAM_MAC(qosParams.trustUpFlag, qosParamsGet.trustUpFlag, devNum);

        /*
            1.3. Call with valid values for Explicit NULL Label
            Expected: GT_OK.
        */
        labelType                      = CPSS_DXCH_TTI_EXPLICIT_NULL_LABEL_E;

        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);

        /*
            1.4. Get the configured params and compare.
            Expected: GT_OK.
        */
        st = cpssDxChTtiNonForwardingLabelQosAttributesGet(devNum, labelType, &qosParamsGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
        ...
        /* 1.5 Negative Scenarios*/
        labelType = CPSS_DXCH_TTI_EXPLICIT_NULL_LABEL_E+1;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        labelType          = CPSS_DXCH_TTI_ELI_LABEL_E;
        qosParams.qosModel = CPSS_DXCH_TTI_QOS_SHORT_PIPE_MODEL_E+1;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosParams.qosModel = CPSS_DXCH_TTI_QOS_SHORT_PIPE_MODEL_E;
        qosParams.ttlModel = CPSS_DXCH_TTI_TTL_SHORT_PIPE_MODEL_E+1;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosParams.ttlModel = CPSS_DXCH_TTI_TTL_PIPE_MODEL_E;
        qosParams.qosMappingTableIndex = 128;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosParams.qosMappingTableIndex = 126;
        qosParams.trustExpFlag = 2;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosParams.trustExpFlag = GT_TRUE;
        qosParams.trustUpFlag = CPSS_DXCH_TTI_TRUST_L2_OUTER_ETH_UP_E+1;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, &qosParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosParams.trustUpFlag = CPSS_DXCH_TTI_TRUST_L2_UP_E;
        st = cpssDxChTtiNonForwardingLabelQosAttributesSet(devNum, labelType, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, devNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiNonForwardingLabelQosAttributesGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        /* 1.5 Negative Scenarios*/
        st = cpssDxChTtiNonForwardingLabelQosAttributesGet(devNum, labelType, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, devNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiQosActionResolutionTableEntrySet)
{

    GT_STATUS                 st = GT_OK;
    GT_U8                     devNum;
    CPSS_DXCH_TTI_LOOKUP_TYPE_ENT                      lookupType;
    GT_U32                                             lookupNum;
    GT_U32                                             entryIndex;
    CPSS_DXCH_TTI_QOS_ACTION_RESOLUTION_PARAMS_STC     qosActionResolutionParams;
    CPSS_DXCH_TTI_QOS_ACTION_RESOLUTION_PARAMS_STC     qosActionResolutionParamsGet;
    ...
    {
        /*
            1.1. Call with valid values for Parallel Lookup
            Expected: GT_OK.
        */
        lookupType                                                 = CPSS_DXCH_TTI_PARALLEL_LOOKUP_E;
        entryIndex                                                 = 62;
        qosActionResolutionParams.nextQosModelSelectionMode        = CPSS_DXCH_TTI_QOS_MODEL_CURRENT_E;
        qosActionResolutionParams.nextQosMappingTableSelectionMode = CPSS_DXCH_TTI_TRUST_QOS_MAPPING_TABLE_CURRENT_E;
        qosActionResolutionParams.nextQosTrustFieldsSelectionMode  = CPSS_DXCH_TTI_QOS_TRUST_FIELDS_SELECTION_CURRENT_E;
        qosActionResolutionParams.nextExp4QosSelectionMode         = CPSS_DXCH_TTI_EXP_4_QOS_SELECTION_CURRENT_E;
        qosActionResolutionParams.nextExp4EgressSelectionMode      = CPSS_DXCH_TTI_EXP_4_EGRESS_SELECTION_CURRENT_E;

        for (lookupNum = 0; lookupNum <= CPSS_DXCH_TTI_LOOKUP_NUM_3_E; lookupNum++)
        {
            st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                             entryIndex, &qosActionResolutionParams);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, lookupNum);

            /*
                1.2. Get the configured params and compare.
                Expected: GT_OK.
            */
            st = cpssDxChTtiQosActionResolutionTableEntryGet(devNum, lookupType, lookupNum,
                                                             entryIndex, &qosActionResolutionParamsGet);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, lookupNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(qosActionResolutionParams.nextQosModelSelectionMode,
                                        qosActionResolutionParamsGet.nextQosModelSelectionMode, devNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(qosActionResolutionParams.nextQosMappingTableSelectionMode,
                                        qosActionResolutionParamsGet.nextQosMappingTableSelectionMode, devNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(qosActionResolutionParams.nextQosTrustFieldsSelectionMode,
                                        qosActionResolutionParamsGet.nextQosTrustFieldsSelectionMode, devNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(qosActionResolutionParams.nextExp4QosSelectionMode,
                                        qosActionResolutionParamsGet.nextExp4QosSelectionMode, devNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(qosActionResolutionParams.nextExp4EgressSelectionMode,
                                        qosActionResolutionParamsGet.nextExp4EgressSelectionMode, devNum);
        }

        /*
            1.1. Call with valid values for Serial Lookup
            Expected: GT_OK.
        */
        lookupType = CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E;
        for (lookupNum = 0; lookupNum < CPSS_DXCH_TTI_LOOKUP_NUM_3_E; lookupNum++)
        {
            st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                             entryIndex, &qosActionResolutionParams);
        ...
        /* 1.5 Negative Scenarios*/
        lookupType = CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E+1;
        lookupNum  = CPSS_DXCH_TTI_LOOKUP_NUM_2_E;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        lookupType = CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E;
        qosActionResolutionParams.nextQosModelSelectionMode = CPSS_DXCH_TTI_QOS_MODEL_CURRENT_E+1;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosActionResolutionParams.nextQosModelSelectionMode = CPSS_DXCH_TTI_QOS_MODEL_CURRENT_E;
        qosActionResolutionParams.nextQosMappingTableSelectionMode = CPSS_DXCH_TTI_TRUST_QOS_MAPPING_TABLE_CURRENT_E+1;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosActionResolutionParams.nextQosMappingTableSelectionMode = CPSS_DXCH_TTI_TRUST_QOS_MAPPING_TABLE_CURRENT_E;
        qosActionResolutionParams.nextQosTrustFieldsSelectionMode = CPSS_DXCH_TTI_QOS_TRUST_FIELDS_SELECTION_CURRENT_E+1;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosActionResolutionParams.nextQosTrustFieldsSelectionMode = CPSS_DXCH_TTI_QOS_TRUST_FIELDS_SELECTION_CURRENT_E;
        qosActionResolutionParams.nextExp4QosSelectionMode = CPSS_DXCH_TTI_EXP_4_QOS_SELECTION_NEXT_E+1;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosActionResolutionParams.nextExp4QosSelectionMode = CPSS_DXCH_TTI_EXP_4_QOS_SELECTION_NEXT_E;
        qosActionResolutionParams.nextExp4EgressSelectionMode = CPSS_DXCH_TTI_EXP_4_EGRESS_SELECTION_NEXT_E+1;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &qosActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        qosActionResolutionParams.nextExp4EgressSelectionMode = CPSS_DXCH_TTI_EXP_4_EGRESS_SELECTION_NEXT_E;
        st = cpssDxChTtiQosActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, devNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiQosActionResolutionTableEntryGet)
{
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiTtlActionResolutionTableEntrySet)
{
    ...
    GT_U32                                             lookupNum;
    GT_U32                                             entryIndex;
    CPSS_DXCH_TTI_TTL_ACTION_RESOLUTION_PARAMS_STC     ttlActionResolutionParams;
    CPSS_DXCH_TTI_TTL_ACTION_RESOLUTION_PARAMS_STC     ttlActionResolutionParamsGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        lookupType                                                 = CPSS_DXCH_TTI_PARALLEL_LOOKUP_E;
        entryIndex                                                 = 14;
        ttlActionResolutionParams.nextTTLModelSelectionMode        = CPSS_DXCH_TTI_TTL_MODEL_CURRENT_E;
        ttlActionResolutionParams.nextTTLSelectionMode             = CPSS_DXCH_TTI_TTL_SELECT_CURRENT_E;

        for (lookupNum = 0; lookupNum <= CPSS_DXCH_TTI_LOOKUP_NUM_3_E; lookupNum++)
        {
            st = cpssDxChTtiTtlActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                             entryIndex, &ttlActionResolutionParams);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, lookupNum);
            ...
            /*
                ...
                Expected: GT_OK.
            */
            st = cpssDxChTtiTtlActionResolutionTableEntryGet(devNum, lookupType, lookupNum,
                                                             entryIndex, &ttlActionResolutionParamsGet);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, lookupNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(ttlActionResolutionParams.nextTTLModelSelectionMode,
                                        ttlActionResolutionParamsGet.nextTTLModelSelectionMode, devNum);
            UTF_VERIFY_EQUAL1_PARAM_MAC(ttlActionResolutionParams.nextTTLSelectionMode,
                                        ttlActionResolutionParamsGet.nextTTLSelectionMode, devNum);
        }
        ...
        */
        ...
        lookupType = CPSS_DXCH_TTI_SERIAL_EM_LOOKUP_E;
        ttlActionResolutionParams.nextTTLModelSelectionMode = CPSS_DXCH_TTI_TTL_MODEL_CURRENT_E+1;
        st = cpssDxChTtiTtlActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &ttlActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);

        ttlActionResolutionParams.nextTTLModelSelectionMode = CPSS_DXCH_TTI_TTL_MODEL_CURRENT_E;
        ttlActionResolutionParams.nextTTLSelectionMode      = CPSS_DXCH_TTI_TTL_SELECT_NEXT_E+1;
        st = cpssDxChTtiTtlActionResolutionTableEntrySet(devNum, lookupType, lookupNum,
                                                         entryIndex, &ttlActionResolutionParams);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, devNum);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTtiTtlActionResolutionTableEntryGet)
{
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChTti)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeSrcPortProfileIdSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeSrcPortProfileIdGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeBytePairCfgSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeBytePairCfgGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeParamsSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericRangeParamsGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationLsbLocationSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationLsbLocationGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationDefaultValueSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationDefaultValueGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationParamsSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiGenericClassificationParamsGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiEliExceptionCmdSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiEliExceptionCmdGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiNonForwardingLabelQosAttributesSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiNonForwardingLabelQosAttributesGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiQosActionResolutionTableEntrySet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiQosActionResolutionTableEntryGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiTtlActionResolutionTableEntrySet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTtiTtlActionResolutionTableEntryGet)
