...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /* 1.5. AC5X Logic Test */
        if (PRV_CPSS_SIP_6_15_CHECK_MAC(dev)  && !PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            blockNum = 0;
        ...
        */
        ...
        if (PRV_CPSS_SIP_5_20_CHECK_MAC(dev))
        {
            ...
            */
            ...
            /* restore default mode to avoid problems in Upload tests for AAS */
            format = CPSS_DXCH_CNC_COUNTER_FORMAT_MODE_0_E;
            st = cpssDxChCncCounterFormatSet(dev, blockNum, format);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, format);
            ...
            */
            ...
            if (PRV_CPSS_SIP_5_20_CHECK_MAC(dev))
            {
                ...
                */
                ...
                /* restore default mode to make CNC Upload tests working stable. */
                format = CPSS_DXCH_CNC_COUNTER_FORMAT_MODE_0_E;
                st = cpssDxChCncPortGroupCounterFormatSet(dev, portGroupsBmp, blockNum, format);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, format);
            }
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        if(GT_FALSE == PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
             /* 1.3. Call with wrong enum values profileSet. */
        ...
        {
            st = cpssDxChCncQueueStatusLimitSet(dev, PRV_CPSS_DXCH_SIP_7_PORT_QUEUE_TX_DROP_PROFILE_MAX_MAC, tcQueue, queueLimit);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
        }
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
       ...
       {
            st = cpssDxChCncQueueStatusLimitGet(dev, 1024, tcQueue, &queueLimit);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
       }
...
*/
UTF_TEST_CASE_MAC(cpssDxChCncEgressPacketTypePassDropClientIndexModeSet)
{
/*
    ITERATE_DEVICES (Ironamn)
    1.1. Call function
         indexMode[CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CMD_AND_CODE_E /
                   CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CODE_ONLY_E]
    Expected: GT_OK.
    1.2. Call cpssDxChCncEgressPacketTypePassDropClientIndexModeSet
        with not NULL indexModePtr and other params from 1.1.
    Expected: GT_OK and the same indexMode as was set.
    1.3. Call function with wrong enum values indexMode and other params from 1.1.
    Expected: GT_BAD_PARAM.
*/
    ...
    CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_ENT indexModeArr[] =
    {
        CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CMD_AND_CODE_E,
        CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_CODE_ONLY_E
    };
    GT_U32  indexModeArrSize = sizeof(indexModeArr) / sizeof(indexModeArr[0]);
    CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_ENT indexModeGet;
    CPSS_DXCH_CNC_EGRESS_PACKET_TYPE_CLIENT_INDEX_MODE_ENT indexMode;

    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(
        &dev, ~UTF_CPSS_PP_ALL_SIP6_30_CNS);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for (i = 0; (i < indexModeArrSize); i++)
        {
            st = cpssDxChCncEgressPacketTypePassDropClientIndexModeSet(
                dev, indexModeArr[i]);
            UTF_VERIFY_EQUAL2_PARAM_MAC(
                GT_OK, st, dev, indexModeArr[i]);

            st = cpssDxChCncEgressPacketTypePassDropClientIndexModeGet(
                dev, &indexModeGet);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        /*
            1.3. Call function with wrong enum values and other params from 1.1.
            Expected: GT_BAD_PARAM.
        */
        UTF_ENUMS_CHECK_MAC(cpssDxChCncEgressPacketTypePassDropClientIndexModeSet
                           (dev, indexMode),
                           indexMode);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChCncEgressPacketTypePassDropClientIndexModeGet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function cpssDxChCncEgressPacketTypePassDropClientIndexModeGet
        with not NULL indexModePtr.
    Expected: GT_OK.
    1.2. Call function cpssDxChCncEgressPacketTypePassDropClientIndexModeGet
        with NULL indexModePtr.
    Expected: GT_BAD_PTR.
*/
    GT_STATUS   st = GT_OK;
...
*/
UTF_TEST_CASE_MAC(cpssDxChCncBlockSharedClientConfigSet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function with blockNum [0 / (CNC_BLOCKS_NUM - 1)],
                            clientIndex [0/2]
                            clientsConfigPtr[clientEnable: GT_TRUE/GT_FALSE
                                             clientSelect: CPSS_DXCH_CNC_CLIENT_TTI_PARALLEL_0_E/
                                                           CPSS_DXCH_CNC_CLIENT_PORT_STAT_E
                                             clientOffset: 0/1023
                                             indexRangesBmp: 1/255]
    Expected: GT_OK.
    1.2. Call function cpssDxChCncBlockSharedClientConfigGet with not NULL clientsConfigPtr
                                                       and other params from 1.1.
    Expected: GT_OK and the same params as was set.
    1.3. Call function with out of range blockNum [CNC_BLOCKS_NUM]
                       and other params from 1.1.
    Expected: GT_BAD_PARAM.
    1.4. Call function with out of range clientIndex [3]
                       and other params from 1.1.
    Expected: GT_BAD_PARAM.
    1.5. Call function with wrong enum values clientSelect and other params from 1.1.
    Expected: GT_BAD_PARAM.
    1.6. Call function with out of range clientOffset and other params from 1.1.
    Expected: GT_OUT_OF_RANGE.
*/
    GT_STATUS                                     st              = GT_OK;
    GT_U8                                         dev;
    ...
    CPSS_DXCH_CNC_BLOCK_SHARED_CLIENTS_CONFIG_STC clientsCfgSet;
    CPSS_DXCH_CNC_BLOCK_SHARED_CLIENTS_CONFIG_STC clientsCfgGet;

    cpssOsMemSet(&clientsCfgSet,0,sizeof(clientsCfgSet));
    cpssOsMemSet(&clientsCfgGet,0,sizeof(clientsCfgGet));

    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, ~UTF_CPSS_PP_ALL_SIP6_30_CNS);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            ...
            Expected: GT_OK.
        */
        clientsCfgSet.clientEnable       = GT_TRUE;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_TTI_PARALLEL_0_E;
        clientsCfgSet.clientOffset       = 0;
        clientsCfgSet.indexRangesBmp.l[0]= 1;

        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);

        st = cpssDxChCncBlockSharedClientConfigGet(dev, blockNum, clientIndex, &clientsCfgGet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        st = cpssOsMemCmp(&clientsCfgSet,&clientsCfgGet,sizeof(clientsCfgSet)) == 0 ? GT_OK : GT_BAD_STATE;
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        clientIndex                      = 1;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_EGRESS_TRG_EPORT_E;
        clientsCfgSet.clientOffset       = 256;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        ...
        clientIndex                      = 2;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_PORT_STAT_E;
        clientsCfgSet.clientOffset       = 512;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        ...
        clientIndex                      = 1;
        clientsCfgSet.clientEnable       = GT_FALSE;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        ...
        blockNum                         = (CNC_BLOCKS_NUM(dev) - 1);
        clientsCfgSet.clientEnable       = GT_TRUE;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_TTI_PARALLEL_1_E;
        clientsCfgSet.clientOffset       = 0;
        clientsCfgSet.indexRangesBmp.l[0]= 7;
        ...
        clientIndex                      = 1;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_EGRESS_PCL_PARALLEL_0_E;
        clientsCfgSet.clientOffset       = 128;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        ...
        clientIndex                      = 2;
        clientsCfgSet.clientSelect       = CPSS_DXCH_CNC_CLIENT_QUEUE_STAT_E;
        clientsCfgSet.clientOffset       = 1023;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev, blockNum, clientIndex);
        ...
        /*
            1.2. Call function with NULL clientsCfgSet
                               and other params from 1.1.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, NULL);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_PTR, st, dev, blockNum, clientIndex);
        ...
        */
        ...
        /*
            1.3. Call function with out of range clientIndex [3]
                               and other params from 1.1.
            Expected: GT_BAD_PARAM.
        */
        blockNum    = 0;
        ...
        clientIndex = 0;
        /*
            1.4. Call function with out of range clientOffset [1024]
                               and other params from 1.1.
            Expected: GT_BAD_PARAM.
        */
        clientsCfgSet.clientOffset  = (PRV_CPSS_SIP_7_CHECK_MAC(dev) ? _8K /*13 bits*/: 1024/*10 bits*/);
        clientsCfgSet.clientEnable  = GT_TRUE;
        st = cpssDxChCncBlockSharedClientConfigSet(dev, blockNum, clientIndex, &clientsCfgSet);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, dev, blockNum);

        /*
            1.5. Call function with wrong enum values client and other params from 1.1.
            Expected: GT_BAD_PARAM.
        */
        clientsCfgSet.clientOffset  = (PRV_CPSS_SIP_7_CHECK_MAC(dev) ? _8K /*13 bits*/: 1024/*10 bits*/) - 1;
        UTF_ENUMS_CHECK_MAC(cpssDxChCncBlockSharedClientConfigSet
                           (dev, blockNum, clientIndex, &clientsCfgSet),
                           clientsCfgSet.clientSelect);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChCncBlockSharedClientConfigGet)
{
/*
    ITERATE_DEVICES (DxCh3 and above)
    1.1. Call function with blockNum [0 / (CNC_BLOCKS_NUM - 1)],
                            clientIndex [0/2]
                            and not NULL clientsConfigPtr.
    Expected: GT_OK.
    1.2. Call function with out of range blockNum [CNC_BLOCKS_NUM]
                            and other params from 1.1.
    Expected: NOT GT_OK.
    1.3. Call function with wrong enum values client
                            and other params from 1.1.
    Expected: GT_BAD_PARAM.
    1.4. Call function with clientsConfigPtr [NULL]
                            and other params from 1.1.
    Expected: GT_BAD_PTR.
*/
    GT_STATUS                                     st              = GT_OK;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        clientIndex = 3;
        st = cpssDxChCncBlockSharedClientConfigGet(dev, blockNum, clientIndex, &clientsCfgGet);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_BAD_PARAM, st, dev, blockNum, clientIndex);
        ...
        */
        ...
        /*
            1.4. Call function with clientsCfgGet [NULL]
                                    and other params from 1.1.
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChCncBlockSharedClientConfigGet(dev, blockNum, clientIndex, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PTR, st, "%d, updateEnablePtr = NULL", dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChCnc)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCncBlockSharedClientConfigSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCncBlockSharedClientConfigGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCncEgressPacketTypePassDropClientIndexModeSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCncEgressPacketTypePassDropClientIndexModeGet)
