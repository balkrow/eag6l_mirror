...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            1.1. Call with stage [CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
                   ...
                   entry{
                        ...
                        periodCheckEnable[GT_FALSE, GT_TRUE, GT_FALSE],
                        keepaliveTxPeriod[0, 5, 7],
                        protectionOnRdiEnable[GT_FALSE, GT_TRUE, GT_FALSE],
                        protectionRdiState[GT_FALSE, GT_TRUE, GT_FALSE],
                        ignoreBadKeepAliveInAgingEnable[GT_FALSE, GT_TRUE, GT_FALSE]}.
            Expected: GT_OK.
        ...
        */
        ...
        entry.protectionRdiState    = GT_FALSE;
        entry.protectionOnRdiEnable = GT_FALSE;
        entry.ignoreBadKeepAliveInAgingEnable = GT_FALSE;
        ...
        if (GT_OK == st)
        {
            ...
            {
                UTF_VERIFY_EQUAL1_STRING_MAC(entry.ignoreBadKeepAliveInAgingEnable,
                        tmpEntry.ignoreBadKeepAliveInAgingEnable,
                        "got another rdiCheckEnable: %d",
                        tmpEntry.ignoreBadKeepAliveInAgingEnable);
                UTF_VERIFY_EQUAL1_STRING_MAC(entry.protectionOnRdiEnable,
                        tmpEntry.protectionOnRdiEnable,
                        "got another rdiCheckEnable: %d",
                        tmpEntry.protectionOnRdiEnable);
                UTF_VERIFY_EQUAL1_STRING_MAC(entry.protectionRdiState,
                        tmpEntry.protectionRdiState,
                        "got another rdiCheckEnable: %d",
                        tmpEntry.protectionRdiState);
            }
        }
        ...
        entry.protectionRdiState    = GT_TRUE;
        entry.protectionOnRdiEnable = GT_TRUE;
        entry.ignoreBadKeepAliveInAgingEnable = GT_TRUE;
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /* call with exceptionType = CPSS_DXCH_OAM_EXCEPTION_TYPE_SUMMARY_E */
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            cpssOsBzero((GT_VOID*) &tmpEntry, sizeof(tmpEntry));
        ...
        for (stage = CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E; stage <= CPSS_DXCH_OAM_STAGE_TYPE_EGRESS_E; stage++)
        {
            ...
            for (command = CPSS_PACKET_CMD_FORWARD_E; command <= CPSS_PACKET_CMD_LOOPBACK_E; command++)
            {
                ...
                else
                {
                    ...
                    /* SOFT-DROP can be applied for EGRESS stage only from SIP6 devices */
                    if ((command == CPSS_PACKET_CMD_DROP_SOFT_E) && (PRV_CPSS_SIP_6_CHECK_MAC(dev)))
                    {
                        applicCmd = GT_TRUE;
                    }
                }
...
*/
UTF_TEST_CASE_MAC(cpssDxChOamExceptionRecoveryConfigGet)
{
/*
    ITERATE_DEVICES (SIP5)
    1.1. Call with stage [CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
...
*/
    GT_STATUS                                   st              = GT_OK;
    GT_U8                                       dev             = 0;
    GT_U32                                      stageIndex, exceptionIndex;
    CPSS_DXCH_OAM_STAGE_TYPE_ENT                stageArr[2] = {CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E, CPSS_DXCH_OAM_STAGE_TYPE_EGRESS_E};
    CPSS_DXCH_OAM_EXCEPTION_TYPE_ENT            exceptionTypeArr[4] = {CPSS_DXCH_OAM_EXCEPTION_TYPE_MEG_LEVEL_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_KEEPALIVE_INVALID_HASH_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_SOURCE_INTERFACE_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_TX_PERIOD_E};
    CPSS_DXCH_OAM_EXCEPTION_COMMAND_CONFIG_STC  exceptionCommand;
    CPSS_DXCH_OAM_STAGE_TYPE_ENT                stage;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        for(stageIndex=0; stageIndex<2; stageIndex++)
        {
            ...
            for(exceptionIndex=0; exceptionIndex<4; exceptionIndex++)
            {
                st = cpssDxChOamExceptionRecoveryConfigGet(dev,
                        stageArr[stageIndex],
                        exceptionTypeArr[exceptionIndex],
                        &exceptionCommand);
                if(GT_FALSE == isOamStageSupported(dev,stageArr[stageIndex]))
                {
                    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stageArr[stageIndex]);
                }
                else
                {
                    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, stageArr[stageIndex]);
                }
            }
        }
        ...
        */
        stage = CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E;
        UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionRecoveryConfigGet
                            (dev, stage, exceptionTypeArr[0], &exceptionCommand),
                            stage);

        if(GT_FALSE == isOamStageSupported(dev,stageArr[0]))
        {
        }
        else
        {
        ...
        /*
            ...
            Expected: NON GT_OK.
        */
        st = cpssDxChOamExceptionRecoveryConfigGet(dev, stageArr[0], exceptionTypeArr[0], NULL);
        if(GT_FALSE == isOamStageSupported(dev,stageArr[0]))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stageArr[0]);
        }
        else
        {
            UTF_VERIFY_NOT_EQUAL1_STRING_MAC(GT_OK, st,
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChOamExceptionRecoveryConfigGet(dev, stageArr[0], exceptionTypeArr[0], &exceptionCommand);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
(
    ...
    IN  CPSS_DXCH_OAM_STAGE_TYPE_ENT                stage,
    IN  CPSS_DXCH_OAM_EXCEPTION_TYPE_ENT            exceptionType,
    IN  CPSS_DXCH_OAM_EXCEPTION_COMMAND_CONFIG_STC *exceptionCommand
)
*/
UTF_TEST_CASE_MAC(cpssDxChOamExceptionRecoveryConfigSet)
{
/*
    ITERATE_DEVICES (SIP5)
    1.1. Call with stage [CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
                   ...
                   exceptionType[CPSS_DXCH_OAM_EXCEPTION_TYPE_MEG_LEVEL_E,
                                 ...
                                 CPSS_DXCH_OAM_EXCEPTION_TYPE_SOURCE_INTERFACE_E,
                                 CPSS_DXCH_OAM_EXCEPTION_TYPE_TX_PERIOD_E],
                   exceptionCommand{command[CPSS_PACKET_CMD_TRAP_TO_CPU_E,
                                           CPSS_PACKET_CMD_MIRROR_TO_CPU_E,
                                           CPSS_PACKET_CMD_MIRROR_TO_CPU_E ],
                                   cpuCode[(CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E + 1),
                                           CPSS_NET_UNKNOWN_UC_E,
    ...
    1.7. Call with command[CPSS_PACKET_CMD_FORWARD_E] and
        out of range cpuCode[0x5AAAAAA5](not relevant) and other valid params.
    Expected: GT_OK.
    1.8. Call with out of range exceptionCommand and other valid params.
    Expected: GT_BAD_PTR.
*/
    ...
    CPSS_DXCH_OAM_EXCEPTION_TYPE_ENT                exceptionType       = 0;
    CPSS_DXCH_OAM_EXCEPTION_COMMAND_CONFIG_STC      exceptionCommand;
    CPSS_DXCH_OAM_EXCEPTION_COMMAND_CONFIG_STC      tmpConfig;
    GT_U32                                          stageIndex, exceptionIndex;
    CPSS_DXCH_OAM_STAGE_TYPE_ENT                    stageArr[2] = {CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E, CPSS_DXCH_OAM_STAGE_TYPE_EGRESS_E};
    CPSS_DXCH_OAM_EXCEPTION_TYPE_ENT                exceptionTypeArr[4] = {CPSS_DXCH_OAM_EXCEPTION_TYPE_MEG_LEVEL_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_KEEPALIVE_INVALID_HASH_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_SOURCE_INTERFACE_E,
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_TX_PERIOD_E};
    cpssOsMemSet(&exceptionCommand, 0, sizeof(exceptionCommand));
    cpssOsMemSet(&tmpConfig, 0, sizeof(tmpConfig));
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for(stageIndex=0; stageIndex<2; stageIndex++)
        {
            ...
            for(exceptionIndex=0; exceptionIndex<4; exceptionIndex++)
            {
                /* SIP5 - EOAM supports only FORWARD & DROP */
                if(PRV_CPSS_SIP_5_CHECK_MAC(prvTgfDevNum) && stageIndex == 1)
                {
                    exceptionCommand.command = (exceptionIndex >2)?CPSS_PACKET_CMD_DROP_HARD_E:CPSS_PACKET_CMD_FORWARD_E;
                }
                else
                {
                    exceptionCommand.command = (exceptionIndex >2)?CPSS_PACKET_CMD_MIRROR_TO_CPU_E:CPSS_PACKET_CMD_TRAP_TO_CPU_E;
                }
                exceptionCommand.cpuCode = (exceptionIndex >2)?(CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E + 1):CPSS_NET_UNKNOWN_UC_E;
                st = cpssDxChOamExceptionRecoveryConfigSet(dev,
                        stageArr[stageIndex],
                        exceptionTypeArr[exceptionIndex],
                        &exceptionCommand);
                if(GT_FALSE == isOamStageSupported(dev,stageArr[stageIndex]))
                ...
                if (GT_OK == st)
                {
                    UTF_VERIFY_EQUAL3_STRING_MAC(exceptionCommand.command,
                            tmpConfig.command,
                            "got another command: %d exceptionIndex=%d, stageIndex=%d",
                            tmpConfig.command, exceptionIndex, stageIndex);
                    if(exceptionCommand.command == CPSS_PACKET_CMD_FORWARD_E)
                    {
                        UTF_VERIFY_EQUAL3_STRING_MAC(0,
                                tmpConfig.cpuCode,
                                "got another cpuCode: %d exceptionIndex=%d, stageIndex=%d",
                                tmpConfig.cpuCode, exceptionIndex, stageIndex);
                    }
                    else
                    {
                        UTF_VERIFY_EQUAL3_STRING_MAC(exceptionCommand.cpuCode,
                                tmpConfig.cpuCode,
                                "got another cpuCode: %d exceptionIndex=%d, stageIndex=%d",
                                tmpConfig.cpuCode, exceptionIndex, stageIndex);
                    }
        ...
        /* Egress invalid packet command check */
        exceptionCommand.command = CPSS_PACKET_CMD_MIRROR_TO_CPU_E;
        exceptionCommand.cpuCode = CPSS_NET_UNKNOWN_UC_E;
        st = cpssDxChOamExceptionRecoveryConfigSet(dev, stageArr[0],
                exceptionTypeArr[0], &exceptionCommand);
        if(GT_FALSE == isOamStageSupported(dev,stage))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stage);
        }
        ...
        /*
            ...
            Expected: GT_BAD_PARAM.
        */
        UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionRecoveryConfigSet
                            (dev, stage, exceptionTypeArr[0], &exceptionCommand),
                            stage);
        ...
        else
        {
            ...
            */
            UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionRecoveryConfigSet
                                (dev, stage, exceptionType, &exceptionCommand),
            ...
            */
            ...
            UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionRecoveryConfigSet
                                (dev, stage, exceptionType, &exceptionCommand),
                                exceptionCommand.cpuCode);
        }

        st = cpssDxChOamExceptionRecoveryConfigSet(dev, stageArr[0], exceptionTypeArr[0], NULL);
        if(GT_FALSE == isOamStageSupported(dev,stageArr[0]))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stage);
        }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChOamExceptionRecoveryConfigSet(dev, stageArr[0], exceptionTypeArr[0],
                                           &exceptionCommand);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChOamExceptionSuppressConfigGet)
{
/*
    ITERATE_DEVICES (SIP5)
    1.1. Call with stage [CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        for(stageIndex=0; stageIndex<2; stageIndex++)
        {
            ...
            for(exceptionIndex=0; exceptionIndex<4; exceptionIndex++)
            {
                st = cpssDxChOamExceptionSuppressConfigGet(dev,
                        stageArr[stageIndex],
                        exceptionTypeArr[exceptionIndex],
                        &exceptionCommand);
                if(GT_FALSE == isOamStageSupported(dev,stageArr[stageIndex]))
        ...
        */
        stage = CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E;
        UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionSuppressConfigGet
                            (dev, stage, exceptionTypeArr[0], &exceptionCommand),
                            stage);
        ...
        /*
            ...
            Expected: NON GT_OK.
        */
        st = cpssDxChOamExceptionSuppressConfigGet(dev, stageArr[0], exceptionTypeArr[0], NULL);
        if(GT_FALSE == isOamStageSupported(dev,stageArr[0]))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stageArr[0]);
        }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChOamExceptionSuppressConfigGet(dev, stageArr[0], exceptionTypeArr[0], &exceptionCommand);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChOamExceptionSuppressConfigSet)
{
/*
    ITERATE_DEVICES (SIP5)
    1.1. Call with stage [CPSS_DXCH_OAM_STAGE_TYPE_INGRESS_E,
...
*/
    ...
    CPSS_DXCH_OAM_EXCEPTION_TYPE_ENT                exceptionTypeArr[4] = {CPSS_DXCH_OAM_EXCEPTION_TYPE_MEG_LEVEL_E,
                                                                    ...
                                                                    CPSS_DXCH_OAM_EXCEPTION_TYPE_TX_PERIOD_E};
#if 0
    CPSS_PACKET_CMD_ENT                     commandArr[5] = {CPSS_PACKET_CMD_FORWARD_E,
                                                             CPSS_PACKET_CMD_MIRROR_TO_CPU_E,
                                                             CPSS_PACKET_CMD_TRAP_TO_CPU_E,
                                                             CPSS_PACKET_CMD_DROP_HARD_E,
                                                             CPSS_PACKET_CMD_DROP_SOFT_E};
    CPSS_NET_RX_CPU_CODE_ENT                cpuCodeArr[2] = {CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E + 1,
                                                             CPSS_NET_UNKNOWN_UC_E};
#endif
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for(stageIndex=0; stageIndex<2; stageIndex++)
        {
            ...
            for(exceptionIndex=0; exceptionIndex<4; exceptionIndex++)
            {
                ...
                exceptionCommand.cpuCode = (exceptionIndex >2)?(CPSS_NET_FIRST_UNKNOWN_HW_CPU_CODE_E + 1):CPSS_NET_UNKNOWN_UC_E;
                st = cpssDxChOamExceptionSuppressConfigSet(dev,
                        stageArr[stageIndex],
                        exceptionTypeArr[exceptionIndex],
                        &exceptionCommand);
                if(GT_FALSE == isOamStageSupported(dev,stageArr[stageIndex]))
        ...
        exceptionCommand.command = CPSS_PACKET_CMD_MIRROR_TO_CPU_E;
        exceptionCommand.cpuCode = CPSS_NET_UNKNOWN_UC_E;
        st = cpssDxChOamExceptionSuppressConfigSet(dev, stageArr[0],
                exceptionTypeArr[0], &exceptionCommand);
        if(GT_FALSE == isOamStageSupported(dev,stage))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stage);
        ...
        /*
            ...
            Expected: GT_BAD_PARAM.
        */
        UTF_ENUMS_CHECK_MAC(cpssDxChOamExceptionSuppressConfigSet
                            (dev, stage, exceptionTypeArr[0], &exceptionCommand),
                            stage);
        ...
        st = cpssDxChOamExceptionSuppressConfigSet(dev, stageArr[0], exceptionTypeArr[0], NULL);
        if(GT_FALSE == isOamStageSupported(dev,stageArr[0]))
        {
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev, stage);
        }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChOamExceptionSuppressConfigSet(dev, stageArr[0], exceptionTypeArr[0],
                                           &exceptionCommand);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChOam)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChOamExceptionSuppressConfigGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChOamExceptionSuppressConfigSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChOamExceptionRecoveryConfigGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChOamExceptionRecoveryConfigSet)
