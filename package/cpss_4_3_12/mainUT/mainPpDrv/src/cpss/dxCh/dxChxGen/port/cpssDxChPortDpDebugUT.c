...
UTF_TEST_CASE_MAC(cpssDxChPortDpDebugCpToRxCountingConfigSet)
{
    ...
    GT_BOOL     layer2CutThroughGet;

    /* prepare iterator for go over all active devices */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, UTF_CPSS_PP_ALL_SIP6_CNS);

    /* correct parameters */
    criteriaParamBmp     = 0x3FF; /* will be overridden */
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        {
            criteriaParamBmp     = 0x3FF;
        }
        if (PRV_CPSS_SIP_6_10_CHECK_MAC(dev))
        {
            criteriaParamBmp     = 0x1FF; /* bypassed layer2CutThrough */
        }
        else
        {
            /* bypassed highPriorityPacket, contextDone, descriptorValid, layer2CutThrough*/
        ...
        while(GT_OK == prvUtfNextRxDmaPortGet(&port, GT_TRUE))
        {
            st = cpssDxChPortDpDebugCpToRxCountingConfigSet(
                ...
                highPriorityPacket, contextDone, descriptorValid, layer2CutThrough);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
            ...
            st = cpssDxChPortDpDebugCpToRxCountingConfigGet(
                ...
                &highPriorityPacketGet, &contextDoneGet, &descriptorValidGet, &layer2CutThroughGet);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
            UTF_VERIFY_EQUAL0_PARAM_MAC(criteriaParamBmp, criteriaParamBmpGet);
            ...
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(dev))
            {
                UTF_VERIFY_EQUAL0_PARAM_MAC(layer2CutThrough, layer2CutThroughGet);
            }
        }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        while(GT_OK == prvUtfNextRxDmaPortGet(&port, GT_TRUE))
        {
            ...
            st = cpssDxChPortDpDebugCpToRxCountingConfigGet(
                dev, port, &criteriaParamBmp, &multicastPacket, &cutThroughPacket,
                &cutThroughTerminated, &trunkatedHeader, &dummyDescriptor,
                NULL, &contextDone, &descriptorValid, &layer2CutThrough);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, port);

            st = cpssDxChPortDpDebugCpToRxCountingConfigGet(
                dev, port, &criteriaParamBmp, &multicastPacket, &cutThroughPacket,
                &cutThroughTerminated, &trunkatedHeader, &dummyDescriptor,
                &highPriorityPacket, NULL, &descriptorValid, &layer2CutThrough);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, port);

            st = cpssDxChPortDpDebugCpToRxCountingConfigGet(
                dev, port, &criteriaParamBmp, &multicastPacket, &cutThroughPacket,
                &cutThroughTerminated, &trunkatedHeader, &dummyDescriptor,
                &highPriorityPacket, &contextDone, NULL, &layer2CutThrough);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, port);

            st = cpssDxChPortDpDebugCpToRxCountingConfigGet(
                dev, port, &criteriaParamBmp, &multicastPacket, &cutThroughPacket,
                &cutThroughTerminated, &trunkatedHeader, &dummyDescriptor,
                &highPriorityPacket, NULL, &descriptorValid, NULL);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, port);
        }
