...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /* Sip7 support up to 64K vlans so we can not check any illegal value for
           GT_U16 vlanId */
        if((PRV_CPSS_SIP_5_CHECK_MAC(dev))&&(!PRV_CPSS_SIP_7_CHECK_MAC(dev)))
        {
        ...
        */
        if((PRV_CPSS_SIP_5_CHECK_MAC(dev)))
        {
            routeEntriesArray.entry.regularEntry.ingressMirrorToAnalyzerIndex = PRV_CPSS_DXCH_IP_MIRROR_ANALYZER_MAX_INDEX_MAC(dev) + 1;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        if(PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            ...
            routeEntry.ingressMirrorToAnalyzerIndex = 62;
        }
        ...
        */
        ...
        if((PRV_CPSS_SIP_5_CHECK_MAC(dev)))
        {
            ...
            */
            routeEntry.ingressMirror = GT_TRUE;
            routeEntry.ingressMirrorToAnalyzerIndex = PRV_CPSS_DXCH_IP_MIRROR_ANALYZER_MAX_INDEX_MAC(dev) + 1;
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        routeEntry.ttlHopLimitDecEnable     = GT_TRUE;
        routeEntry.ingressMirror            = GT_TRUE;
        routeEntry.ingressMirrorToAnalyzerIndex = PRV_CPSS_SIP_7_CHECK_MAC(dev)? 62 : 0;
        routeEntry.qosProfileMarkingEnable  = GT_FALSE;
        routeEntry.qosProfileIndex          = 0;
        routeEntry.qosPrecedence            = CPSS_PACKET_ATTRIBUTE_ASSIGN_PRECEDENCE_HARD_E;
        routeEntry.modifyUp                 = CPSS_PACKET_ATTRIBUTE_MODIFY_KEEP_PREVIOUS_E;
...
*/
    ...
    cpssOsMemSet(&natEntryGet, 0, sizeof(CPSS_DXCH_IP_NAT_ENTRY_UNT));
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        else
        {
            ...
            if (PRV_CPSS_SIP_6_30_CHECK_MAC(dev))
            {
                isEqual = (natEntry.nat44Entry.sipPrefixLen == natEntryGet.nat44Entry.sipPrefixLen)? GT_TRUE : GT_FALSE;
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_TRUE, isEqual,
                        "get another sipPrefixLen than was set: %d", dev);
                isEqual = (natEntry.nat44Entry.dipPrefixLen == natEntryGet.nat44Entry.dipPrefixLen)? GT_TRUE : GT_FALSE;
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_TRUE, isEqual,
                        "get another dipPrefixLen than was set: %d", dev);
            }
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        {
           /* in sip7 nextHop table has 48K entries deep.
               For L3VPN there are 4 sub-entries inside each L3NHE Router Next Hop Entry, so total size is 192K.

               ECMP Router Indirection Table is 48K entries deep.
               One ECMP Indirection Table entry is having 4 sub-entries each pointing to a nhe_pointer.
               So the total scale is 192K.

               Router ECMP table : 2 entries per line --> max valid entry index is 32K-1
               */
            ecmpEntry.routeEntryBaseIndex = numNextHopEntries*4-1;
        }
        else
        {
