...
UTF_TEST_CASE_MAC(cpssDxChBrgL2DlbWaLttInfoSet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Call function with out of bound value for secondaryDlbBaseIndex */
        lttIndex                = 30;
        secondaryDlbBaseIndex   = ((PRV_CPSS_DXCH_PP_MAC(dev)->bridge.l2EcmpNumMembers -
                                  PRV_CPSS_DXCH_PP_MAC(dev)->bridge.l2EcmpNumMembers_usedByTrunk));
        st = cpssDxChBrgL2DlbWaLttInfoSet(dev, lttIndex, secondaryDlbBaseIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_BAD_PARAM, st, "cpssDxChBrgL2DlbWaLttInfoSet: Failed");

        /* Call function with out of bound value for lttIndex */
        lttIndex                = PRV_CPSS_DXCH_PP_HW_INFO_LAST_INDEX_IN_L2_LTT_ECMP_MAC(dev) + 1;
        secondaryDlbBaseIndex   = 1100;
        st = cpssDxChBrgL2DlbWaLttInfoSet(dev, lttIndex, secondaryDlbBaseIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OUT_OF_RANGE, st, "cpssDxChBrgL2DlbWaLttInfoSet: Failed");

        memEnd = osMemGetHeapBytesAllocated();
        memEnd -= (2 * sizeof(PRV_CPSS_DXCH_BRG_L2_DLB_WA_LTT_USAGE_STC));
        UTF_VERIFY_EQUAL0_STRING_MAC(memStart, memEnd, "Memory Leak detected, cpssDxChBrgL2DlbWaLttInfoDelete");

        lttIndex = 20;
        st = cpssDxChBrgL2DlbWaLttInfoDelete(dev, lttIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, st, "cpssDxChBrgL2DlbWaLttInfoDelete: Failed");
        lttIndex = 30;
        st = cpssDxChBrgL2DlbWaLttInfoDelete(dev, lttIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_OK, st, "cpssDxChBrgL2DlbWaLttInfoDelete: Failed");
...
*/
UTF_TEST_CASE_MAC(cpssDxChBrgL2DlbWaLttInfoDelete)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        lttIndex                = 20;
        st = cpssDxChBrgL2DlbWaLttInfoDelete(dev, lttIndex);
        UTF_VERIFY_EQUAL0_STRING_MAC(GT_NOT_FOUND, st, "cpssDxChBrgL2DlbWaLttInfoSet: Failed");
...
UTF_TEST_CASE_MAC(cpssDxChBrgL2DlbWaExecute_regionEntryVerification)
{
    ...
    for(ii = 0; ii <L2_DLB_LTT_ENTRY_CNS; ii++)
    {
        ...
        st = cpssDxChBrgL2DlbWaLttInfoDelete(dev, lttIndex[ii]);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChBrgL2DlbWaLttInfoDelete: Failed, index[%d]", ii);
    }
}
/*----------------------------------------------------------------------------*/
/*
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChBrgL2Dlb)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgL2DlbWaLttInfoDelete)
