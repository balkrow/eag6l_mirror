...
void cliWrapCpssDxChVirtualTcamHelp()
{
    ...
    cpssOsPrintf("---- clientGroup, hitNumber, ruleSize<std,ext,ultra,10b,20b,30b,40b,50b,60b,80b>,\n");
    cpssOsPrintf("---- tcamSpaceUnmovable, autoResize, guaranteedNumOfRules,\n");
    cpssOsPrintf("---- ruleAdditionMethod<index or priority>\n");
    cpssOsPrintf("cliWrapCpssDxChVirtualTcamAppAllocationSet entryIndex, startRowIndex, numOfRows, columnsBitmap\n");
    cpssOsPrintf("---- entryIndex - size of array set to (entryIndex + 1), 0xFFFFFFFF - set to 0\n");
    ...
    cpssOsPrintf("cliWrapPrvCpssSystemRecoveryStateUpdate \n");
    cpssOsPrintf("---- systemRecoveryState <prepare,init,completion,catch_up>\n");
    cpssOsPrintf("---- systemRecoveryProcess <hsu,fast_boot,ha,not_active,parallel_ha,hitless_startup>\n");
    cpssOsPrintf("---- systemRecoveryHaState <read_write,read_no_write,no_read_write,no_read_no_write>\n");
}
...
static ENUM_STR_TO_VAL_STC stringToValTabTcamRuleSize[] =
{
    ...
    {"80b",   CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_80_B_E},
    ENUM_STR_TO_VAL_END_MAC
};
...
GT_U8 prvCliWrapCpssDxChVirtualTcamMngFirstDevNumGet
(
...
GT_U32 prvCliWrapCpssDxChVirtualTcamIsSip7
(
    IN GT_U32 vTcamMngId
)
{
    ...
    isSip7 = 0;
    devNum = prvCliWrapCpssDxChVirtualTcamMngFirstDevNumGet(vTcamMngId);
    if ((devNum != 0xFF) && PRV_CPSS_SIP_7_CHECK_MAC(devNum))
    {
        isSip7 = 1;
    }
    return isSip7;
...
static GT_U32                                   allocationSpaceArrSize = 0;
static CPSS_DXCH_VIRTUAL_TCAM_SPACE_SEGMENT_STC allocationSpaceArr[16];
...
)
{
    ...
    {
        allocationSpaceArrSize = 0;
        return GT_OK;
    }
    if (entryIndex >= (sizeof(allocationSpaceArr) / sizeof(allocationSpaceArr[0])))
    {
        return GT_BAD_PARAM;
    }
    allocationSpaceArr[entryIndex].startRowIndex = startRowIndex;
    allocationSpaceArr[entryIndex].numOfRows     = numOfRows;
    allocationSpaceArr[entryIndex].columnsBitmap = columnsBitmap;
    allocationSpaceArrSize = (entryIndex + 1);
    return GT_OK;
}
...
)
{
    ...
    vTcamInfo.allocationSpaceArrSize = allocationSpaceArrSize;
    vTcamInfo.allocationSpaceArr     = allocationSpaceArr;
        ...
        case CPSS_DXCH_VIRTUAL_TCAM_RULE_TYPE_PCL_E:
            switch (vTcamInfo.ruleSize)
            {
                ...
                case CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_30_B_E: /*CPSS_DXCH_VIRTUAL_TCAM_RULE_SIZE_STD_E*/
                    if (prvCliWrapCpssDxChVirtualTcamIsSip7(vTcamMngId))
                    {
                        ruleTcamTypePtr->rule.pcl.ruleFormat =
...
)
{
    ...
    for (index = 0; (index < numOfRules); index++)
    {
        ...
        if ((expectedRuleStamp != ruleStamp)
            || (expectedActionStamp != actionStamp))
        {
            cpssOsPrintf(
...
)
{
    ...
    if (vMgrPtr->vTcamCfgPtrArr == NULL)
    {
        return GT_OK;
    }
...
static ENUM_STR_TO_VAL_STC stringToValTabSysRecoveryState[] =
{
    {"prepare",    CPSS_SYSTEM_RECOVERY_PREPARATION_STATE_E},
    {"init",       CPSS_SYSTEM_RECOVERY_INIT_STATE_E},
    {"completion", CPSS_SYSTEM_RECOVERY_COMPLETION_STATE_E},
    {"catch_up",   CPSS_SYSTEM_RECOVERY_HW_CATCH_UP_STATE_E},
    ENUM_STR_TO_VAL_END_MAC
};

static CPSS_SYSTEM_RECOVERY_STATE_ENT stringToSysRecoveryState(ENUM_STR string)
{
    return stringToEnum(stringToValTabSysRecoveryState, string);
}

static ENUM_STR_TO_VAL_STC stringToValTabSysRecoveryProcess[] =
{
    {"hsu",             CPSS_SYSTEM_RECOVERY_PROCESS_HSU_E},
    {"fast_boot",       CPSS_SYSTEM_RECOVERY_PROCESS_FAST_BOOT_E},
    {"ha",              CPSS_SYSTEM_RECOVERY_PROCESS_HA_E},
    {"not_active",      CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E},
    {"parallel_ha",     CPSS_SYSTEM_RECOVERY_PROCESS_PARALLEL_HA_E},
    {"hitless_startup", CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E},
    ENUM_STR_TO_VAL_END_MAC
};

static CPSS_SYSTEM_RECOVERY_PROCESS_ENT stringToSysRecoveryProcess(ENUM_STR string)
{
    return stringToEnum(stringToValTabSysRecoveryProcess, string);
}

static ENUM_STR_TO_VAL_STC stringToValTabSysRecoveryHaState[] =
{
    {"read_write",        CPSS_SYSTEM_RECOVERY_HA_STATE_READ_ENABLE_WRITE_ENABLE_E},
    {"read_no_write",     CPSS_SYSTEM_RECOVERY_HA_STATE_READ_ENABLE_WRITE_DISABLE_E},
    {"no_read_write",     CPSS_SYSTEM_RECOVERY_HA_STATE_READ_DISABLE_WRITE_ENABLE_E},
    {"no_read_no_write",  CPSS_SYSTEM_RECOVERY_HA_STATE_READ_DISABLE_WRITE_DISABLE_E},
    ENUM_STR_TO_VAL_END_MAC
};

static CPSS_SYSTEM_RECOVERY_HA_STATE_ENT stringToSysRecoveryHaState(ENUM_STR string)
{
    return stringToEnum(stringToValTabSysRecoveryHaState, string);
}
...
)
{
    ...
    CPSS_SYSTEM_RECOVERY_HA_STATE_ENT    systemRecoveryHaState;
    CPSS_SYSTEM_RECOVERY_INFO_STC newSystemRecoveryInfo;

    systemRecoveryState = stringToSysRecoveryState(sysRecoveryState);
    if (systemRecoveryState == INVALID_ENUM_VALUE_CNS)
    {
        /* message already printed */
        return GT_FAIL;
    }
    systemRecoveryProcess = stringToSysRecoveryProcess(sysRecoveryProcess);
    if (systemRecoveryProcess == INVALID_ENUM_VALUE_CNS)
    {
        /* message already printed */
        return GT_FAIL;
    }
    systemRecoveryHaState = stringToSysRecoveryHaState(sysRecoveryHaState);
    if (systemRecoveryHaState == INVALID_ENUM_VALUE_CNS)
    {
        /* message already printed */
        return GT_FAIL;
    }
    ...
    newSystemRecoveryInfo.systemRecoveryMode.haReadWriteState = systemRecoveryHaState;
