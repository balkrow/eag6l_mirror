...
#define HOW_MUCH_TIME(msg, rc)                                                      \
    ...
    PRV_UTF_LOG3_MAC("%s, total time takes : %d sec, %d nanosec\n",                     \
        #msg, seconds, nanoSec)
...
static GT_U32   ut_member_add_as_disabled = 0;

static GT_STATUS    ut_cpssDxChTrunkMemberAdd(
    IN GT_U8                    devNum,
    IN GT_TRUNK_ID              trunkId,
...
)
{
    ...
    {
        /* add the member as disabled */
        rc = cpssDxChTrunkMemberDisabledAdd(devNum, trunkId, memberPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
        /* then enable the disabled member */
        rc = cpssDxChTrunkMemberEnable(devNum, trunkId, memberPtr);
        if(rc != GT_OK)
        {
            return rc;
        }
...
UTF_TEST_CASE_MAC(start_ut_member_add_as_disabled)
{
    ut_member_add_as_disabled = 1;
}
UTF_TEST_CASE_MAC(end_ut_member_add_as_disabled)
{
    ut_member_add_as_disabled = 0;
}
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = ut_cpssDxChTrunkMemberAdd(dev, trunkId, &enabledMembersArray[5]);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTrunkMemberDisabledAdd)
{
/*
ITERATE_DEVICES (DxChx)
1.1. Call with trunk id [2], non-NULL memberPtr {port [8], device [devNum]}. Expected: GT_OK.
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChTrunkMemberDisabledAdd(dev, trunkId, &member);
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "%d, %d, memberPtr->device = %d, memberPtr->port = %d",
                                     dev, trunkId, member.hwDevice, member.port);

        /* check that the member considered disabled in the trunk DB */
        numOfMembers = CPSS_TRUNK_MAX_NUM_OF_MEMBERS_CNS;
        st = cpssDxChTrunkDbDisabledMembersGet(dev, trunkId, &numOfMembers, membersArray);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
        UTF_VERIFY_EQUAL2_PARAM_MAC(1, numOfMembers, dev, trunkId);
        UTF_VERIFY_EQUAL2_PARAM_MAC(member.hwDevice, membersArray[0].hwDevice, dev, trunkId);
        UTF_VERIFY_EQUAL2_PARAM_MAC(member.port    , membersArray[0].port    , dev, trunkId);
        st = cpssDxChTrunkDbEnabledMembersGet(dev, trunkId, &numOfMembers, membersArray);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
        UTF_VERIFY_EQUAL2_PARAM_MAC(0, numOfMembers, dev, trunkId);
        ...
        /* check that the member considered enabled in the trunk DB */
        numOfMembers = CPSS_TRUNK_MAX_NUM_OF_MEMBERS_CNS;
        st = cpssDxChTrunkDbEnabledMembersGet(dev, trunkId, &numOfMembers, membersArray);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
        UTF_VERIFY_EQUAL2_PARAM_MAC(1, numOfMembers, dev, trunkId);
        ...
        /* 1.2. Try to add the same trunk member again, after is already enabled in the trunk
            accordingly to header the result must be OK.    */
        /* Call with trunk id [2], non-NULL memberPtr [tested dev, virtual port 8].                     */
        /* Expected: GT_OK.                                                                             */
        st = cpssDxChTrunkMemberDisabledAdd(dev, trunkId, &member);
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "%d, %d, memberPtr->device = %d, memberPtr->port = %d",
        ...
        /* 1.6. Check for NULL handling. Call with trunk id [2], memberPtr [NULL].                      */
        /* Expected: GT_BAD_PTR.                                                                        */
        st = cpssDxChTrunkMemberDisabledAdd(dev, trunkId, NULL);
        UTF_VERIFY_EQUAL2_STRING_MAC(GT_BAD_PTR, st, "%d, %d, memberPtr = NULL", dev, trunkId);
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = ut_cpssDxChTrunkMemberAdd(dev, trunkId, &member);
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        else if (PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            entryIndex = 256;/*split UC,MC table (each 128 entries)*/
        }
...
UTF_TEST_CASE_MAC(cpssDxChTrunkDbIsMemberOfTrunkGetNext)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = ut_cpssDxChTrunkMemberAdd(dev, trunkId2, &member);
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberAdd: %d, %d, %d, %d",
                                     dev, trunkId, member.hwDevice, member.port);
        ...
        st = ut_cpssDxChTrunkMemberAdd(dev, trunkId3, &member);
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberAdd: %d, %d, %d, %d",
                                     dev, trunkId, member.hwDevice, member.port);
...
*/
    ...
    GT_BOOL                                 enable;
    CPSS_DXCH_TRUNK_LBH_GLOBAL_MODE_ENT     hashMode;
    CPSS_DXCH_TRUNK_LBH_GLOBAL_MODE_ENT     hashModeGet;
    CPSS_DXCH_TRUNK_LBH_GLOBAL_MODE_ENT     hashModes[] = { CPSS_DXCH_TRUNK_LBH_PACKETS_INFO_E,
                                                            CPSS_DXCH_TRUNK_LBH_INGRESS_PORT_E,
                                                            CPSS_DXCH_TRUNK_LBH_PACKETS_INFO_CRC_E};
    GT_U32  numOfModes = sizeof(hashModes) / sizeof(hashModes[0]);
   ...
   /* 1. Go over all active devices. */
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        st = cpssDxChTrunkHashGlobalModeGet(dev, &hashModeGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        for(ii = 0 ; ii < numOfModes; ii++)
        {
            ...
            */
                ...
                hashEntityType ++)
            {
                ...
                hashInputData.ipDipArray[i+0] = 0xff;
                hashInputData.ipDipArray[i+1] = 0x44;
                hashInputData.ipDipArray[i+2] = 0x55;
                hashInputData.ipDipArray[i+3] = 0x66;


                cpssOsMemSet(hashInputData.ipSipArray, 0, sizeof(hashInputData.ipSipArray));
                hashInputData.ipSipArray[i+0] = 0xff;
                hashInputData.ipSipArray[i+1] = 0x77;
                hashInputData.ipSipArray[i+2] = 0x88;
                hashInputData.ipSipArray[i+3] = 0x99;
        ...
        /* restore */
        st = cpssDxChTrunkHashGlobalModeSet(dev, hashModeGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
    }
...
*/
    ...
    PRV_CPSS_HASH_BIT_SET_CONFIG_STC clientConfigs[] =
    {
        ...
        {CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_TRUNK_DESIGNATED_TABLE_E, 18,12},
        ...
        {CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_INGRESS_OAM_E, 20,12},
        {CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_INGRESS_OAM_E, 31, 1},
        {CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E, 0, 12},
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*  1.6. Call function with sum of start bit and number of bits >12
            (EGR_OAM)
            sip7 support 16 bits that allowed from any position in the 32 bits.
            Expected: GT_OUT_OF_RANGE.
        */
        ...
        {
            st = cpssDxChTrunkHashNumBitsSet(dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E, 0,16);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E);
            st = cpssDxChTrunkHashNumBitsSet(dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E, 16,16);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E);
            st = cpssDxChTrunkHashNumBitsSet(dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E, 0,17);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, dev,
                CPSS_DXCH_TRUNK_HASH_CLIENT_TYPE_EGRESS_OAM_E);
        }
    }
...
UTF_TEST_CASE_MAC(cpssDxChTrunkHashMaskCrcParamOverrideSet)
{
    ...
    CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_ENT   lastFieldType;
    GT_BOOL    override;
    GT_BOOL    getOverride;
    ...
    lastFieldType = CPSS_DXCH_TRUNK_HASH_MASK_CRC_PARAM_OVERRIDE_IP_DIP_SIP_BY_7MPLS_LABELS_E;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
            ...
            fieldType++)
        {
            ...
            rc = ((PRV_CPSS_SIP_7_CHECK_MAC(dev) != GT_TRUE) &&
                  (fieldType == lastFieldType)) ? GT_BAD_PARAM : GT_OK;
        ...
        /* set all values , and then check that can get all those values */
        for(fieldType = (PRV_CPSS_SIP_7_CHECK_MAC(dev) == GT_TRUE) ? lastFieldType + 1 : lastFieldType;
            fieldType < 100; fieldType++)
        {
            override = (fieldType & 1) ? GT_FALSE : GT_TRUE;
...
/* set the trunk mode to 'sorted' and then run next tests:
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkMemberDisabledAdd)
...
UTF_TEST_CASE_MAC(cpssDxChTrunkDbMembersSortingEnableSet_membersManipulations)
{
    ...
    for(ii = 0 ; ii < (sizeof(enableArr) / sizeof(enableArr[0])) ; ii++)
    {
        ...
        UTF_TEST_CALL_MAC(cpssDxChTrunkMemberDisabledAdd);
    }
...
*/
    ...
    CPSS_TRUNK_DESIGNATED_TABLE_MC_PORT_MODE_ENT     mode    =
        CPSS_TRUNK_DESIGNATED_TABLE_MC_PORT_MODE_FORBID_ALL_E;
    CPSS_TRUNK_DESIGNATED_TABLE_MC_PORT_MODE_ENT     modeGet =
        CPSS_TRUNK_DESIGNATED_TABLE_MC_PORT_MODE_FORBID_ALL_E;

    GT_U32  nonSupportedDevices;

    nonSupportedDevices = UTF_LION2_E;
...
)
{
    ...
    /* check that the API is valid (or not) */
    st = ut_cpssDxChTrunkMemberAdd(dev, trunkId , &usedMember);
    UTF_VERIFY_EQUAL2_PARAM_MAC(expectedSt, st, dev, trunkId);
    ...
    /* check that the API is valid (or not) */
    st = cpssDxChTrunkMemberDisabledAdd(dev, trunkId , &usedMember);
    UTF_VERIFY_EQUAL2_PARAM_MAC(expectedSt, st, dev, trunkId);
...
UTF_TEST_CASE_MAC(cpssDxChTrunkEcmpLttTableSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        l2EcmpNumMembers = PRV_CPSS_SIP_7_CHECK_MAC(dev) ?
            PRV_CPSS_DXCH_PP_MAC(dev)->hwInfo.parametericTables.numEntriesTrunkEcmp :
            PRV_CPSS_DXCH_PP_MAC(dev)->bridge.l2EcmpNumMembers;
    ...
    )
{
    ...
    for(ii = 0 ; ii < maxMembers ; ii+= ((maxMembers/4) + (maxMembers/4)/2 - 1))
    {
        ...
        /* keep alive */
        PRV_UTF_LOG3_MAC("Going to : cpssDxChTrunkMembersSet with : [%d] members [%d] enabled , [%d] disabled\n",
        ...
        PRV_UTF_LOG1_MAC("ended iteration : took [%d] seconds \n",
    ...
    IN GT_U32   maxNumOfMembers)
{
    ...
    for(ii = numOfEnabledMembers + numOfDisabledMembers ; ii < maxMembers ; ii++,iterations++)
    {
        ...
        st = ut_cpssDxChTrunkMemberAdd(dev,trunkId,
...
UTF_TEST_CASE_MAC(cpssDxChTrunkMemberManipulation_4K_members)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        if(sortMode == GT_TRUE)
        {
            cpssOsPrintf("running in SORT MODE on device[%d] \n",dev);
            /* set sorting mode */
            st = cpssDxChTrunkDbMembersSortingEnableSet(dev,  GT_TRUE);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
        ...
        if(sortMode == GT_TRUE)
        {
            cpssOsPrintf("disabled SORT MODE on device[%d] \n",dev);
            /* restore sorting mode */
            st = cpssDxChTrunkDbMembersSortingEnableSet(dev,  GT_FALSE);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
...
UTF_TEST_CASE_MAC(cpssDxChTrunkMemberManipulation_4K_members_sort_mode)
{
    /* this test is for baseline execution only because it's long
        on HW of Falcon (4 tiles) it takes 300 seconds
    */
    PRV_TGF_SKIP_NON_BASELINE_TEST_MAC();

    sortMode = GT_TRUE;

    UTF_TEST_CALL_MAC(cpssDxChTrunkMemberManipulation_4K_members);
...
UTF_TEST_CASE_MAC(cpssDxChTrunkDbSharedPort_cascade_basic_config)
{
    ...
    GT_U32                  ii,jj,kk,port,counter,allPortsCounter,entryIndex;
    ...
    */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        for(kk = 0 ; kk < 2 ; kk++)
        {
            ...
            /* check it ! */
            for(entryIndex = PRV_CPSS_DEV_TRUNK_INFO_MAC(dev)->numOfDesignatedTrunkEntriesHw / 2;
                entryIndex < PRV_CPSS_DEV_TRUNK_INFO_MAC(dev)->numOfDesignatedTrunkEntriesHw;
                entryIndex++)
            {
                st = cpssDxChTrunkDesignatedPortsEntryGet(dev,entryIndex,&designatedPortsBmp);
                UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
...
/* debug function to set 'max' num of hash to be used per trunk */
static GT_U32 g_maxHashNumberPerTrunk  = 23;
static GT_U32 g_maxNumOfMembers        = 4;
void tgfTrunkDb_debug_maxHashNumberPerTrunk(GT_U32  hashCnt, GT_U32 maxNumOfMembers)
{
        g_maxHashNumberPerTrunk = hashCnt;
        g_maxNumOfMembers       = maxNumOfMembers;
}

/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbAddResilientCascadeMember)
{
    GT_U8                               dev;
    GT_STATUS                           st;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* start manipulation on the trunk */
        CPSS_PORTS_BMP_PORT_CLEAR_ALL_MAC(&portsBmp);
        portNum = TRUNK_TESTED_VIRT_PORT_ID_CNS(dev);
        ...
        st = cpssDxChTrunkDbCascadePolicyInfoSet(dev, trunkId, &policyInfo);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        st = prvCpssGenericTrunkDebugResilientHashDbValidate(dev, trunkId);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        prvDebugGenericTrunkDebugResilientHashDbDump(dev, trunkId);

        /* Add 8 ports to the existing Cascade Resilient Trunk
         * Validate the DB after each addition
         */
        for(portNum = 1; portNum<8; portNum++)
        {
            st = cpssDxChTrunkCascadePortAdd(dev, trunkId, portNum);
            UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, st, "cpssDxChTrunkCascadePortAdd: %d, %d, %d",
                    dev, trunkId, portNum);

            st = prvCpssGenericTrunkDebugResilientHashDbValidate(dev, trunkId);
            UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, st,
                    "prvCpssGenericTrunkDebugResilientHashDbValidate: %d, %d, %d",
                    dev, trunkId, portNum);
            prvDebugGenericTrunkDebugResilientHashDbDump(dev, trunkId);
        }
        /* Remove all 8 ports(1 by 1) from the existing Cascade Resilient Trunk
         * Validate the DB after each removal
         */
        for(portNum = 0; portNum<8; portNum++)
        {
            st = cpssDxChTrunkCascadePortRemove(dev, trunkId, portNum);
            UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberRemove: %d, %d, %d",
                    dev, trunkId,
                    portNum);
        ...
        /** Remove the Last port from the Cascade Resilient Trunk
         *  Expected is Memory should intact, Till Trunk Remove
         */
        portNum = TRUNK_TESTED_VIRT_PORT_ID_CNS(dev);
        st = cpssDxChTrunkCascadePortRemove(dev, trunkId, portNum);
        UTF_VERIFY_EQUAL3_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberRemove: %d, %d, %d",
...
/* calculate index of ECMP member in Enabled array
   return 0xFFFFFFFF if not found */
static GT_U32 utTrunkMemberIndexGet(
    IN GT_U32 numOfEnabledMembers,
    IN CPSS_TRUNK_MEMBER_STC   enabledMembersArray[],
    IN CPSS_TRUNK_MEMBER_STC *memberEntry)
{
    GT_U32 ii;
    for (ii = 0 ; ii < numOfEnabledMembers; ii++)
    {
        if ((memberEntry->port == enabledMembersArray[ii].port) &&
            (memberEntry->hwDevice == enabledMembersArray[ii].hwDevice))
        {
            return ii;
        }
    }
...
/* function checks validity of following items in HW:
   - Trunk LTT table is configured correctly
   - Trunk ECMP entries distributed evenly between enabled members of trunk. */
static GT_VOID utTrunkResilientEcmpTblValidate
(
    IN GT_U8                        devNum,
    IN GT_TRUNK_ID                  trunkId,
    IN GT_U32                       expNumOfEnabledMembers,
    IN GT_U32                       ecmpBase,
    IN GT_U32                       ecmpSize
)
{
    GT_STATUS                           st; /* return status */
    CPSS_DXCH_TRUNK_ECMP_LTT_ENTRY_STC  ecmpLttInfo; /* LTT entry */
    CPSS_DXCH_BRG_L2_ECMP_ENTRY_STC   ecmpMember;  /* ECMP entry */
    CPSS_TRUNK_MEMBER_STC             trunkMember; /* trunk ECMP entry */
    CPSS_TRUNK_MEMBER_STC   *enabledMembersArray = NULL; /* erray of enabled members */
    GT_U16                  *memberInstanceNumInEcmpArr = NULL; /* array of ECMP pathes counts per enabled member */
    GT_U32      l2EcmpNumMembers_usedByTrunk; /* value of ECMP table manager before test */
    GT_U32      numOfEnabledMembers; /* number of enabled members in trunk */
    GT_U32      memberIdx;           /* index of member */
    GT_U32      minNumOfEcmpEntries; /* minimal number of ECMP paths per member */
    GT_U32      maxNumOfEcmpEntries; /* maximal number of ECMP paths per member */
    GT_U32      summary;             /* summary of ECMP paths */
    GT_U32      ii;                  /* iterator */
    ...
    /* validate LTT entry */
    UTF_VERIFY_EQUAL0_PARAM_MAC(GT_TRUE, ecmpLttInfo.ecmpEnable);
    UTF_VERIFY_EQUAL0_PARAM_MAC(ecmpSize, ecmpLttInfo.ecmpNumOfPaths);
    UTF_VERIFY_EQUAL0_PARAM_MAC(ecmpBase, ecmpLttInfo.ecmpStartIndex);
    UTF_VERIFY_EQUAL0_PARAM_MAC(GT_FALSE, ecmpLttInfo.ecmpRandomPathEnable);
    ...
    enabledMembersArray = cpssOsMalloc(ecmpSize * sizeof(CPSS_TRUNK_MEMBER_STC));
    if (NULL == enabledMembersArray)
    {
        UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(GT_OUT_OF_CPU_MEM, GT_OK);
        goto exit_clean;
    }

    memberInstanceNumInEcmpArr = cpssOsMalloc(ecmpSize * sizeof(GT_U16));
    if (NULL == memberInstanceNumInEcmpArr)
    {
        UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(GT_OUT_OF_CPU_MEM, GT_OK);
        goto exit_clean;
    }
    cpssOsMemSet(memberInstanceNumInEcmpArr, 0, ecmpSize * sizeof(GT_U16));

    numOfEnabledMembers = ecmpSize;
    st = cpssDxChTrunkDbEnabledMembersGet(devNum,trunkId,&numOfEnabledMembers,enabledMembersArray);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
    UTF_VERIFY_EQUAL0_PARAM_MAC(expNumOfEnabledMembers, numOfEnabledMembers);

    /* read ECMP table and calculate ECMP distribution in memberInstanceNumInEcmpArr */
    for (ii = 0; ii < ecmpSize; ii++)
    {
        if(PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            st = cpssDxChTrunkEcmpTableGet(devNum, ecmpBase + ii, &trunkMember);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
        }
        else
        {
            st = cpssDxChBrgL2EcmpTableGet(devNum, ecmpBase + ii, &ecmpMember);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
            trunkMember.port     = ecmpMember.targetEport;
            trunkMember.hwDevice = ecmpMember.targetHwDevice;
        }

        memberIdx = utTrunkMemberIndexGet(numOfEnabledMembers,enabledMembersArray,&trunkMember);
        if (memberIdx >= numOfEnabledMembers)
        {
            UTF_VERIFY_EQUAL1_PARAM_MAC(memberIdx, ecmpSize, ii);
            goto exit_clean;
        }

        memberInstanceNumInEcmpArr[memberIdx]++;
    }

    /* calculate minimal and maximal values for distribution */
    minNumOfEcmpEntries = ecmpSize / numOfEnabledMembers;
    UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(0, minNumOfEcmpEntries);

    maxNumOfEcmpEntries = minNumOfEcmpEntries + 1;
    summary = 0;

    /* check ECMP distribution */
    for (ii = 0; ii < numOfEnabledMembers; ii++)
    {
        summary += memberInstanceNumInEcmpArr[ii];
        if (memberInstanceNumInEcmpArr[ii] < minNumOfEcmpEntries)
        {
            UTF_VERIFY_EQUAL1_PARAM_MAC(minNumOfEcmpEntries, memberInstanceNumInEcmpArr[ii], ii);
        }
        else if (memberInstanceNumInEcmpArr[ii] > maxNumOfEcmpEntries)
        {
            UTF_VERIFY_EQUAL1_PARAM_MAC(maxNumOfEcmpEntries, memberInstanceNumInEcmpArr[ii], ii);
        }
    }

    /* check that all paths are utilized */
    UTF_VERIFY_EQUAL0_PARAM_MAC(ecmpSize, summary);

exit_clean:

    if (enabledMembersArray) cpssOsFree(enabledMembersArray);
    if (memberInstanceNumInEcmpArr) cpssOsFree(memberInstanceNumInEcmpArr);
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbAddResilientMember)
{
    GT_U8                               dev;
    GT_STATUS                           st;
    GT_TRUNK_ID                         trunkId = 2;
    ...
    GT_PHYSICAL_PORT_NUM                portNum;
    GT_U32                              ii;
    GT_U32                              memberArrayIndex;
    GT_U32                              testTrunkEcmpBase;
    GT_U32                              testNumOfEnabledMembers;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        numOfEnabledMembers  = g_maxNumOfMembers;
        numOfDisabledMembers = 0;
        testNumOfEnabledMembers = 0;
        testTrunkEcmpBase = 0;
        ...
        memberArrayIndex = 0;
        l2EcmpStartIndex = l2EcmpNumMembers - g_maxHashNumberPerTrunk;
        for(ii=1; ii<=30; ii++)
        {
            /* set this trunk Flex Info */
            l2EcmpStartIndex -= g_maxHashNumberPerTrunk;
            st = cpssDxChTrunkFlexInfoSet(dev, ii, l2EcmpStartIndex, g_maxHashNumberPerTrunk);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            if (ii == trunkId)
            {
                testTrunkEcmpBase = l2EcmpStartIndex;
                testNumOfEnabledMembers = numOfEnabledMembers;
            }
            /* set this trunk policy as RESILIENT */
            policyInfo.policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E;
            st = cpssDxChTrunkDbPolicyInfoSet(dev, ii/*trunkId*/, &policyInfo);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            /* start manipulation on the trunk */
            st = cpssDxChTrunkMembersSet(dev, ii,
                    numOfEnabledMembers,
                    &membersArray_set_4K_tests[memberArrayIndex],/*enabled members in the start*/
                    numOfDisabledMembers,
                    &membersArray_set_4K_tests[numOfEnabledMembers]);/* disabled members at the end */
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
            memberArrayIndex += (numOfDisabledMembers + numOfEnabledMembers);

            utTrunkResilientEcmpTblValidate(dev, ii, numOfEnabledMembers, l2EcmpStartIndex, g_maxHashNumberPerTrunk);

            st = prvCpssGenericTrunkDebugResilientHashDbValidate(dev, ii);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        }
        ...
        testNumOfEnabledMembers++;
        utTrunkResilientEcmpTblValidate(dev, trunkId, testNumOfEnabledMembers, testTrunkEcmpBase, g_maxHashNumberPerTrunk);
        ...
        for(portNum = 0; portNum<8; portNum++)
        {
            ...
            st = prvCpssGenericTrunkDebugResilientHashDbValidate(dev, trunkId);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, "Member Add: dev - %d, port %d",
                    dev, portNum);
        ...
        /* Remove all 8 members(1 by 1) from the existing Resilient Trunk
         * Validate the DB after each removal
         */
        for(portNum = 0; portNum<8; portNum++)
        {
            ...
            st = prvCpssGenericTrunkDebugResilientHashDbValidate(dev, trunkId);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, "Member Remove dev %d, port %d",
                    dev, portNum);
        ...
        /* remove ports those not members - no changes expected */
        for(portNum = 0; portNum<numOfEnabledMembers; portNum++)
        {
            st = cpssDxChTrunkMemberRemove(dev, trunkId, &membersArray_set_4K_tests[portNum]);
            UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberRemove: %d, %d, %d, %d",
                    dev, trunkId, member.hwDevice, member.port);
        ...
        /* 2nd Iteration - Addition (Reset List)
         * Add 8 ports to the existing Cascade Resilient Trunk
         * Validate the DB after each addition
         */
        for(portNum = 0; portNum<8; portNum++)
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbPolicyInfoSet)
{
    GT_U8                           dev;
    GT_STATUS                       st;
    GT_TRUNK_ID                     trunkId;
    CPSS_TRUNK_POLICY_STC           policyInfo;
    CPSS_TRUNK_POLICY_STC           policyInfoGet;
    CPSS_DXCH_TRUNK_MEMBERS_MODE_ENT    orig_trunkMembersMode;
    GT_U32                              orig_maxNumberOfTrunks;
    GT_U32                              l2EcmpNumMembers;
    GT_U32                              l2EcmpStartIndex;
    ...
    cpssOsMemSet(&policyInfo, 0, sizeof(policyInfo));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        policyInfo.policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E;
        st = cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, &policyInfo);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, trunkId);
        ...
        l2EcmpStartIndex = l2EcmpNumMembers - g_maxHashNumberPerTrunk;
        st = cpssDxChTrunkFlexInfoSet(dev, trunkId, l2EcmpStartIndex, g_maxHashNumberPerTrunk);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        st = cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, &policyInfo);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
        st = cpssDxChTrunkDbPolicyInfoGet(dev, trunkId, &policyInfoGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                "cpssDxChTrunkDbPolicyInfoGet: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(policyInfo.policy, policyInfoGet.policy,
                "cpssDxChTrunkDbPolicyInfoGet: %d, Returned a different value ", dev);

        member.hwDevice                 = TRUNK_HW_DEV_NUM_MAC(dev);
        member.port                     = TRUNK_TESTED_VIRT_PORT_ID_CNS(dev);

        /* Verify Not applicable APIs return failure */
        st = cpssDxChTrunkDesignatedMemberSet(dev, trunkId, GT_TRUE, &member);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, trunkId);
        ...
        member.port = port;
        st = cpssDxChTrunkMembersSet(dev, trunkId, 1, &member, 0, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        st = cpssDxChTrunkPortMcEnableSet(dev, port,
                CPSS_TRUNK_DESIGNATED_TABLE_MC_PORT_MODE_FORBID_ALL_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, trunkId);
        ...
        /* Update policy mode */
        policyInfo.policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_REWRITE_E;
        st = cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, &policyInfo);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
        ...
        UTF_ENUMS_CHECK_MAC(cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, &policyInfo),
                                                    policyInfo.policy);

        /* Validate NULL check */
        st = cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, NULL);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, trunkId);
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbPolicyInfoGet)
{
    GT_U8                           dev;
    GT_STATUS                       st;
    GT_TRUNK_ID                     trunkId;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChTrunkDbPolicyInfoGet(dev, trunkId, NULL);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, trunkId);
        ...
        st = cpssDxChTrunkDbPolicyInfoGet(dev, trunkId, &policyInfo);
        UTF_VERIFY_NOT_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbCascadePolicyInfoSet)
{
    GT_U8                           dev;
    GT_STATUS                       st;
    GT_TRUNK_ID                     trunkId;
    ...
    CPSS_DXCH_TRUNK_MEMBERS_MODE_ENT    orig_trunkMembersMode;
    GT_U32                              orig_maxNumberOfTrunks;
    CPSS_TRUNK_WEIGHTED_MEMBER_STC      membersArray[1];
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChTrunkDbCascadePolicyInfoGet(dev, trunkId, &policyInfoGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                "cpssDxChTrunkDbCascadePolicyInfoGet: %d", dev);
        UTF_VERIFY_EQUAL1_STRING_MAC(policyInfo.policy, policyInfoGet.policy,
                "cpssDxChTrunkDbCascadePolicyInfoGet: %d, Returned a different value ", dev);

        /* Verify Not applicable APIs return failure */
        membersArray[0].member.port     = TRUNK_TESTED_VIRT_PORT_ID_CNS(dev);
        membersArray[0].member.hwDevice = TRUNK_HW_DEV_NUM_MAC(dev);
        membersArray[0].weight          = 3;
        st = cpssDxChTrunkCascadeTrunkWithWeightedPortsSet(dev, trunkId, 1/*numOfMembers*/, membersArray);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, trunkId);
        ...
        UTF_ENUMS_CHECK_MAC(cpssDxChTrunkDbCascadePolicyInfoSet(dev, trunkId, &policyInfo),
                                                    policyInfo.policy);

        /* Validate NULL check */
        st = cpssDxChTrunkDbCascadePolicyInfoSet(dev, trunkId, NULL);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, trunkId);
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbCascadePolicyInfoGet)
{
    GT_U8                           dev;
    GT_STATUS                       st;
    GT_TRUNK_ID                     trunkId;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChTrunkDbCascadePolicyInfoGet(dev, trunkId, NULL);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, dev, trunkId);
        ...
        st = cpssDxChTrunkDbCascadePolicyInfoGet(dev, trunkId, &policyInfo);
        UTF_VERIFY_NOT_EQUAL2_PARAM_MAC(GT_OK, st, dev, trunkId);
...
static CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT g_scale_test_policy = CPSS_TRUNK_LOAD_BALANCE_POLICY_RESILIENT_E;
GT_STATUS cpssDxChTrunkDbResilientScale_full_capacity_policy_set
(
    IN CPSS_TRUNK_LOAD_BALANCE_POLICY_TYPE_ENT policy
)
{
    g_scale_test_policy = policy;
    return GT_OK;
}

/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbResilientScale_full_capacity)
{
    GT_U8                               dev;
    GT_STATUS                           st;
    GT_TRUNK_ID                         trunkId = 100;
    CPSS_TRUNK_POLICY_STC               policyInfo;
    CPSS_DXCH_TRUNK_MEMBERS_MODE_ENT    orig_trunkMembersMode;
    GT_U32                              orig_maxNumberOfTrunks;
    GT_U32                              maxTrunksInDev;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        l2EcmpNumMembers = (l2EcmpNumMembers > _4K)?
            _4K:l2EcmpNumMembers;
        maxTrunksInDev   = PRV_CPSS_DXCH_PP_MAC(dev)->fineTuning.tableSize.trunksNum;
        ...
        memStart = osMemGetHeapBytesAllocated();
        st = cpssDxChTrunkInit(dev ,maxTrunksInDev, CPSS_DXCH_TRUNK_MEMBERS_MODE_FLEX_E);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        memEnd = osMemGetHeapBytesAllocated();
        totalAllocatedMemory = memEnd - memStart;
        PRV_UTF_LOG1_MAC("\n\nTrunk_Manager_FootPrint  : Created with Flex mode: Memory consumed [%d]\n",
                memEnd - memStart);

        /* set this trunk Flex Info */
        l2EcmpStartIndex = 0;
        st = cpssDxChTrunkFlexInfoSet(dev, trunkId, l2EcmpStartIndex, l2EcmpNumMembers);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        numOfEnabledMembers = (l2EcmpNumMembers < (_4K-1))?(l2EcmpNumMembers-2):(_4K-2);
        numOfDisabledMembers = 0;

        /* copy to 'set' array from the 'orig' array */
        cpssOsMemCpy(membersArray_set_4K_tests,
                     membersArray_orig_4K_tests,
                     (sizeof(CPSS_TRUNK_MEMBER_STC)*(numOfEnabledMembers+numOfDisabledMembers)));

        memStart = osMemGetHeapBytesAllocated();
        /* set this trunk policy as RESILIENT */
        policyInfo.policy = g_scale_test_policy;
        st = cpssDxChTrunkDbPolicyInfoSet(dev, trunkId, &policyInfo);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        st = cpssDxChTrunkMembersSet(dev,trunkId,
                ...
                &membersArray_set_4K_tests[numOfEnabledMembers]);/* disabled members at the end */
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        HOW_MUCH_TIME(Trunk_Manager_Performance: cpssDxChTrunkMembersSet With one Member, st);
        ...
        st = cpssDxChTrunkMembersSet(dev,trunkId,
                ...
                &membersArray_set_4K_tests[numOfEnabledMembers]);/* disabled members at the end */
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        HOW_MUCH_TIME(Trunk_Manager_Performance: cpssDxChTrunkMembersSet With All Members, st);
        ...
        UTF_VERIFY_EQUAL4_STRING_MAC(GT_OK, st, "cpssDxChTrunkMemberAdd: %d, %d, %d, %d",
                                     dev, trunkId, member.hwDevice, member.port);
        HOW_MUCH_TIME(Trunk_Manager_Performance: cpssDxChTrunkMemberAdd Adding last member, st);

        memEnd = osMemGetHeapBytesAllocated();
        totalAllocatedMemory += (memEnd - memStart);
        PRV_UTF_LOG2_MAC("Trunk_Manager_FootPrint  : Increased Memory for creating (%d) trunk member [%d]\n\n",
                numOfEnabledMembers + 1, memEnd - memStart);
        ...
        memStart   = memEnd;
        memEnd     = osMemGetHeapBytesAllocated();
        UTF_VERIFY_EQUAL1_STRING_MAC((memStart - memEnd), totalAllocatedMemory,
                "Trunk_Manager_FootPrint: Allocated memory and Freed memory did not match [dev %d]", dev);
...
/*----------------------------------------------------------------------------*/
UTF_TEST_CASE_MAC(cpssDxChTrunkDbResilient_16_members_flexMode)
{
    GT_U8                               dev;
    GT_STATUS                           st;
    GT_TRUNK_ID                         trunkId = 100;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        l2EcmpNumMembers = l2EcmpNumMembers/maxTrunksInDev;
        if (l2EcmpNumMembers == 0)
        {
            /* test is not supported by the device */
            continue;
        }
        ...
        /* set this trunk Flex Info */
        l2EcmpStartIndex = 0;
        st = cpssDxChTrunkFlexInfoSet(dev, 0xffff, l2EcmpStartIndex, l2EcmpNumMembers);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        numOfEnabledMembers = l2EcmpNumMembers-1;
        numOfDisabledMembers = 0;
...
#define  TRUNK_HIGH_LEVEL_LIST_1  \
    /* indicate that need to run in ut_member_add_as_disabled mode */ \
    UTF_SUIT_DECLARE_TEST_MAC(start_ut_member_add_as_disabled)  \
    TRUNK_HIGH_LEVEL_LIST         \
    /* indicate that need to stop run in ut_member_add_as_disabled mode (restore orig mode) */ \
    UTF_SUIT_DECLARE_TEST_MAC(end_ut_member_add_as_disabled)  \
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkMemberManipulation_4K_members_sort_mode)

/* list holds test for resilient hash scenario */
#define  TRUNK_HIGH_LEVEL_LIST_RESILIENT_HASH                               \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbAddResilientMember)            \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbAddResilientCascadeMember)     \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbPolicyInfoSet)                 \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbPolicyInfoGet)                 \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbCascadePolicyInfoSet)          \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbCascadePolicyInfoGet)          \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbResilientScale_full_capacity)  \
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTrunkDbResilient_16_members_flexMode)
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChTrunkHighLevel)
    TRUNK_HIGH_LEVEL_LIST
    TRUNK_HIGH_LEVEL_LIST_RESILIENT_HASH
