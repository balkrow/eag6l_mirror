...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CH1_E | UTF_CH1_DIAMOND_E | UTF_XCAT2_E | UTF_AAS_E);
    /* 1. Go over all active devices. */
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* 1.1. Call with dropPkts [10]. Expected: GT_OK.   */
        ...
        UTF_VERIFY_EQUAL1_STRING_MAC(dropPkts, dropPktsRet.l[0],
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        while(GT_OK == prvUtfNextPhyPortGet(&port, GT_TRUE))
        {
            ...
            */
            ...
            if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
            {
                /* value that returned may be HW default or
                   another value that previous tests like Data Integrity may assign.*/
            }
            else
            {
                UTF_VERIFY_EQUAL2_STRING_MAC(saLsbMode, saLsbModeGet,
...
UTF_TEST_CASE_MAC(cpssDxChIpPortGroupDropCntSet)
{
    ...
    if(regAddr != 0xFFFFFFFF)
    {
        /* test not suitable for eArch devices , as offsets in the units not match
           so there is no meanning to call next lines */

            prvUtfPerPortGroupCounter64BitGet(regAddr, 0, 32,
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: GT_OK.
        */
        routerMacSaIndex = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? _4K-1 : 255;
        st = cpssDxChIpRouterGlobalMacSaGet(devNum, routerMacSaIndex,
                                            &macSaAddr);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
        ...
        /*
            ...
            Expected: GT_BAD_PARAM.
        */
        routerMacSaIndex = PRV_CPSS_SIP_7_CHECK_MAC(devNum) ? _4K : 256;
        st = cpssDxChIpRouterGlobalMacSaGet(devNum, routerMacSaIndex,
                                            &macSaAddr);
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChIpFdbRouteCounterSetIndexSet
(
    IN GT_U8                        devNum,
    IN CPSS_IP_CNT_SET_ENT          countSetIndex
)
*/
UTF_TEST_CASE_MAC(cpssDxChIpFdbRouteCounterSetIndexSet)
{
/*
ITERATE_DEVICES (Sip7 and above)
1.1. Call with cntSet [CPSS_IP_CNT_SET0_E / CPSS_IP_CNT_SET1_E / CPSS_IP_CNT_SET2_E / CPSS_IP_CNT_SET3_E / CPSS_IP_CNT_NO_SET_E] and non-null counters.
Expected: GT_OK.
1.2. Call with wrong enum values cntSet  and other valid parameters from 1.1.
Expected: GT_BAD_PARAM.
*/
    GT_STATUS               st = GT_OK;
    GT_U8                   dev;
    CPSS_IP_CNT_SET_ENT     cntSet,cntSetGet;
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* 1.1. Call with cntSet [CPSS_IP_CNT_SET0_E / CPSS_IP_CNT_SET1_E / CPSS_IP_CNT_SET2_E / CPSS_IP_CNT_SET3_E / CPSS_IP_CNT_NO_SET_E] and non-null counters.
           Expected: GT_OK.     */
        for(cntSet = CPSS_IP_CNT_SET0_E ; cntSet <= CPSS_IP_CNT_NO_SET_E ; cntSet++)
        {
            st = cpssDxChIpFdbRouteCounterSetIndexSet(dev, cntSet);
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, cntSet);

            st = cpssDxChIpFdbRouteCounterSetIndexGet(dev, &cntSetGet);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

            UTF_VERIFY_EQUAL1_PARAM_MAC(cntSet, cntSetGet, dev);
        }
        /*cntSet = CPSS_IP_CNT_NO_SET_E+1*/
        st = cpssDxChIpFdbRouteCounterSetIndexSet(dev, cntSet);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, cntSet);

        /* 1.2. Call with wrong enum values cntSet  and other
           valid parameters from 1.1. Expected: GT_BAD_PARAM.   */
        UTF_ENUMS_CHECK_MAC(cpssDxChIpFdbRouteCounterSetIndexSet
                            (dev, cntSet),
                            cntSet);
    }
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChIpFdbRouteCounterSetIndexGet
(
    IN GT_U8                        devNum,
    OUT CPSS_IP_CNT_SET_ENT          *countSetIndexPtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChIpFdbRouteCounterSetIndexGet)
{
/*
ITERATE_DEVICES (Sip7 and above)
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChIpFdbRouteCounterSetIndexGet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
    }
...
UTF_TEST_CASE_MAC(cpssDxChIpEcmpUcRpfAccessLevelSet)
{
/*
    ITERATE_DEVICES
    1.1 Iterate over levels.
    1.1.1 Call cpssDxChIpEcmpUcRpfAccessLevelSet.
    Expected: GT_OK.
    1.1.2 Call cpssDxChIpEcmpUcRpfAccessLevelGet.
    Expected: GT_OK.
    1.1.3 Compare level we get with level we set.
    Expected: GT_OK.
    1.2. Call cpssDxChIpEcmpUcRpfAccessLevelSet with wrong level.
    Expected: GT_BAD_PARAM.
    1.3 Call cpssDxChIpEcmpUcRpfAccessLevelGet with NULL accessLevel pointer.
    Expected: GT_BAD_PTR.
    1.4 Call cpssDxChIpEcmpUcRpfAccessLevelSet with wrong protocol.
    Expected: GT_BAD_PARAM.
    1.5 Call cpssDxChIpEcmpUcRpfAccessLevelGet with wrong protocol.
    Expected: GT_BAD_PARAM.
*/
    GT_STATUS   st = GT_OK;
    GT_U8       dev;
    GT_U32      notAppFamilyBmp;
    GT_U32      level=0, temp_level, saved_level_ipv4, saved_level_ipv6;
    CPSS_IP_PROTOCOL_STACK_ENT       protocolStack=CPSS_IP_PROTOCOL_IPV4_E;
    ...
    {
        /* save original state */
        st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev, CPSS_IP_PROTOCOL_IPV4_E, &saved_level_ipv4);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev, CPSS_IP_PROTOCOL_IPV6_E, &saved_level_ipv6);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        for(protocolStack = CPSS_IP_PROTOCOL_IPV4_E; protocolStack <= CPSS_IP_PROTOCOL_IPV6_E; protocolStack++)
        {
            ...
            {
                /*  1.1.1  */
                st = cpssDxChIpEcmpUcRpfAccessLevelSet(dev,protocolStack, level);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, level);

                /*  1.1.2  */
                st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev, protocolStack, &temp_level);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

                UTF_VERIFY_EQUAL1_PARAM_MAC(level, temp_level, level);
            }
            /* 1.2 */
            st = cpssDxChIpEcmpUcRpfAccessLevelSet(dev, protocolStack, BIT_6);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);

            /* 1.3 */
            st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev,protocolStack, NULL);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);

            /* 1.4 */
            st = cpssDxChIpEcmpUcRpfAccessLevelSet(dev, CPSS_IP_PROTOCOL_ALL_E, 0);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);

             /* 1.5  - fist call for ipv4 the get will return GT_BAD_PARAM,
                since ipv6_accessvalue=0 and ipv4_accessvalue=BIT_6-1
                second call for ipv6 the get will return GT_OK since
                both ipv4 and ipv6 have the same access value = BIT6-1*/
            if (protocolStack == CPSS_IP_PROTOCOL_IPV4_E)
            {
                st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev, CPSS_IP_PROTOCOL_IPV4V6_E, &temp_level);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
            }
            else
            {
        ...
        /* restore original state */
        st = cpssDxChIpEcmpUcRpfAccessLevelSet(dev, CPSS_IP_PROTOCOL_IPV4_E, saved_level_ipv4);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        st = cpssDxChIpEcmpUcRpfAccessLevelSet(dev, CPSS_IP_PROTOCOL_IPV6_E, saved_level_ipv6);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
    }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        for(protocolStack = CPSS_IP_PROTOCOL_IPV4_E; protocolStack <= CPSS_IP_PROTOCOL_IPV6_E; protocolStack++)
        {
            ...
            st = cpssDxChIpEcmpUcRpfAccessLevelGet(dev,protocolStack, &level);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
        }
    }
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet
(
    IN  GT_U8    devNum,
    IN  GT_BOOL  enable
)
*/
UTF_TEST_CASE_MAC(cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet)
{
/*
ITERATE_DEVICES (SIP5 and above)
1.1. Call with enable [GT_TRUE and GT_FALSE].
Expected: GT_OK.
1.2. Call cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet.
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet(dev, enable);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, enable);

        /* 1.2.  */
        st = cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet(dev, &enableGet);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        /* Verifying values */
        UTF_VERIFY_EQUAL1_STRING_MAC(enable, enableGet,
            "cpssDxChIpUcRouteAgingModeGet: get another enable than was set: dev = %d",
                                     dev);
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet
(
    IN  GT_U8    devNum,
    OUT GT_BOOL  *enablePtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet)
{
/*
ITERATE_DEVICES (SIP5 and above)
...
*/
    ...
    {
        /* 1.1.  */
        st = cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet(dev, &enable);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        /* 1.1.  */
        st = cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChIpCtrl)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpFdbRouteCounterSetIndexSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpFdbRouteCounterSetIndexGet)

    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpEcmpUcRpfAccessLevelSet)

    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpUcBypassRouterforIgmpOrMldPacketsSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChIpUcBypassRouterforIgmpOrMldPacketsGet)
