...
int debug_pex_perfcheck()
{
    ...
    * in PPC architecture */
    ...
    prvCpssDrvHwPpPortGroupReadRegister(prvTgfDevNum, 0, regAddr, &temp);
    prvCpssDrvHwPpPortGroupReadRegister(prvTgfDevNum, 0, 0x00000040, &lastTs);
    prvCpssDrvHwPpPortGroupReadRegister(prvTgfDevNum, 0, regAddr, &temp);
    prvCpssDrvHwPpPortGroupReadRegister(prvTgfDevNum, 0, 0x00000040, &lastTs2);
...
UTF_TEST_CASE_MAC(cpssDxChTcamLookupProfileCfgSet)
{
    ...
    GT_U32                                keyMuxBanks;
    volatile /* use volatile to solve false GCC warning */ GT_U32                       index;
    GT_U32                                cfgWordSize;
    GT_U32                                numOfTcamProfiles;
    GT_U32                                numOfMuxLines;
    ...
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum)) continue;

        numOfTcamProfiles      = 64;
        numOfMuxLines          = 48;
        numOfSubKeys1_3        = 3;
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            numOfTcamProfiles      = 32;
            numOfMuxLines          = 24;
            numOfSubKeys1_3        = 1;
        }
        for (tcamProfileId = 1; (tcamProfileId < numOfTcamProfiles); tcamProfileId++)
        {
                  ...
                  subKeyPattern++)
            {
                ...
                tcamLookupProfileCfg.subKey1_3_MuxTableLineIndexes[0] = ((tcamProfileId + 10) % numOfMuxLines);
                tcamLookupProfileCfg.subKey1_3_MuxTableLineIndexes[1] = ((tcamProfileId + 20) % numOfMuxLines);
                tcamLookupProfileCfg.subKey1_3_MuxTableLineIndexes[2] = ((tcamProfileId + 30) % numOfMuxLines);
                keyMuxBanks =
                ...
                */
                ...
                if (st == GT_OK)
                {
                    ...
                    /* copy data not stored to HW before compare */
                    for (index = numOfSubKeys1_3; (index < 3); index++)
                    {
                        tcamLookupProfileCfgGet.subKey0_3_Sizes[index + 1] =
                            tcamLookupProfileCfg.subKey0_3_Sizes[index + 1];
                        tcamLookupProfileCfgGet.subKey1_3_MuxTableLineIndexes[index] =
                            tcamLookupProfileCfg.subKey1_3_MuxTableLineIndexes[index];
                        tcamLookupProfileCfgGet.subKey1_3_MuxTableLineOffsets[index] =
                            tcamLookupProfileCfg.subKey1_3_MuxTableLineOffsets[index];
                    }
...
UTF_TEST_CASE_MAC(cpssDxChTcamLookupMuxTableLineSet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        if (PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            numOfTcamMuxTableLines = 24;
        }
        for (entryIndex = 1; (entryIndex < numOfTcamMuxTableLines); entryIndex++)
        {
...
*/
UTF_TEST_CASE_MAC(cpssDxChTcamClientGroupLookupMuxTableLineSet)
{
    GT_STATUS                                    st              = GT_OK;
    GT_U8                                        devNum          = 0;
    GT_U32                                       entryIndex;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        if (PRV_CPSS_SIP_7_CHECK_MAC(devNum) == 0) continue;

        numOfTcamMuxTableLines = 48;
        tcamClientGroupMax = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId;

        for (tcamClientGroup = 0; (tcamClientGroup < tcamClientGroupMax); tcamClientGroup++)
        {
            for (entryIndex = 1; (entryIndex < numOfTcamMuxTableLines); entryIndex++)
            {
                for (index = 0; (index < 30); index++)
                ...
                st = cpssDxChTcamClientGroupLookupMuxTableLineSet(
                    devNum, tcamClientGroup, entryIndex, &tcamLookupMuxTableLineCfg);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, tcamClientGroup, entryIndex);
                st = cpssDxChTcamClientGroupLookupMuxTableLineGet(
                    devNum, tcamClientGroup, entryIndex, &tcamLookupMuxTableLineCfgGet);
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, devNum, tcamClientGroup, entryIndex);
                for (index = 0; (index < 30); index++)
                {
                    UTF_VERIFY_EQUAL4_PARAM_MAC(
                        tcamLookupMuxTableLineCfg.superKeyTwoByteUnitsOffsets[index],
                        tcamLookupMuxTableLineCfgGet.superKeyTwoByteUnitsOffsets[index],
                        devNum, tcamClientGroup, entryIndex, index);
                }
            }
        }
        tcamClientGroup = 0;
        ...
        st = cpssDxChTcamClientGroupLookupMuxTableLineSet(
            devNum, tcamClientGroup, entryIndex, NULL);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PTR, st, devNum, entryIndex);
        ...
        entryIndex = 1;
        tcamClientGroup = tcamClientGroupMax;
        st = cpssDxChTcamClientGroupLookupMuxTableLineSet(
            devNum, tcamClientGroup, entryIndex, &tcamLookupMuxTableLineCfg);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, devNum, entryIndex);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChTcamClientGroupLookupMuxTableLineGet)
{
    GT_STATUS                                    st              = GT_OK;
    GT_U8                                        devNum          = 0;
    GT_U32                                       entryIndex;
...
typedef struct{
    ...
    CPSS_DXCH_TCAM_RULE_SIZE_ENT subKeySizes[4];
    GT_U32 subKeyMuxTableLineIndexes[4];
    GT_U32 subKeyMuxTableLineOffsets[4];
    GT_U32 sharedMuxTableLineIndex;
    GT_U32 sharedMuxTableLineOffset;
} CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC;
*/
UTF_TEST_CASE_MAC(cpssDxChTcamClientGroupLookupProfileCfgSet)
{
    GT_STATUS                             st              = GT_OK;
    GT_U8                                 devNum;
    GT_U32                                tcamProfileId;
    ...
    GT_U32                                tcamClientGroupMax;
    CPSS_DXCH_TCAM_RULE_SIZE_ENT          subKeyPattern;
    CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC tcamLookupProfileCfg;
    CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC tcamLookupProfileCfgGet;
    GT_U32                                keyMuxBanks;
    GT_U32                                index;
    GT_U32                                cfgWordSize;
    GT_U32                                numOfTcamProfiles;
    ...
    cfgWordSize = sizeof(CPSS_DXCH_TCAM_CLIENT_GROUP_LOOKUP_PROFILE_CFG_STC) / 4;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        tcamClientGroupMax = PRV_CPSS_DXCH_PP_MAC(devNum)->hwInfo.tcam.sip6maxTcamGroupId - 1;
        numOfTcamProfiles      = 64;
        numOfMuxLines          = 48;
        for (tcamClientGroup = 0; (tcamClientGroup <= tcamClientGroupMax); tcamClientGroup++)
        {
            for (tcamProfileId = 1; (tcamProfileId < numOfTcamProfiles); tcamProfileId++)
            {
                for (subKeyPattern = (CPSS_DXCH_TCAM_RULE_SIZE_ENT)0;
                      ...
                      subKeyPattern++)
                {
                    tcamLookupProfileCfg.subKeySizes[0] = subKeyPattern;
                    tcamLookupProfileCfg.subKeySizes[1] = ((subKeyPattern + 10) % (CPSS_DXCH_TCAM_RULE_SIZE_80_B_E + 1));
                    tcamLookupProfileCfg.subKeySizes[2] = ((subKeyPattern + 20) % (CPSS_DXCH_TCAM_RULE_SIZE_80_B_E + 1));
                    tcamLookupProfileCfg.subKeySizes[3] = ((subKeyPattern + 30) % (CPSS_DXCH_TCAM_RULE_SIZE_80_B_E + 1));
                    tcamLookupProfileCfg.subKeyMuxTableLineIndexes[0] = ((tcamProfileId + 10) % numOfMuxLines);
                    tcamLookupProfileCfg.subKeyMuxTableLineIndexes[1] = ((tcamProfileId + 20) % numOfMuxLines);
                    tcamLookupProfileCfg.subKeyMuxTableLineIndexes[2] = ((tcamProfileId + 30) % numOfMuxLines);
                    tcamLookupProfileCfg.subKeyMuxTableLineIndexes[3] = ((tcamProfileId + 40) % numOfMuxLines);
                    keyMuxBanks =
                        (tcamLookupProfileCfg.subKeySizes[0] >= CPSS_DXCH_TCAM_RULE_SIZE_60_B_E)
                        ? 6 : ((GT_U32)tcamLookupProfileCfg.subKeySizes[0] + 1);
                    tcamLookupProfileCfg.subKeyMuxTableLineOffsets[0] = (6 - keyMuxBanks);
                    keyMuxBanks =
                        (tcamLookupProfileCfg.subKeySizes[1] >= CPSS_DXCH_TCAM_RULE_SIZE_60_B_E)
                        ? 6 : ((GT_U32)tcamLookupProfileCfg.subKeySizes[1] + 1);
                    tcamLookupProfileCfg.subKeyMuxTableLineOffsets[1] = (6 - keyMuxBanks);
                    keyMuxBanks =
                        (tcamLookupProfileCfg.subKeySizes[2] >= CPSS_DXCH_TCAM_RULE_SIZE_60_B_E)
                        ? 6 : ((GT_U32)tcamLookupProfileCfg.subKeySizes[2] + 1);
                    tcamLookupProfileCfg.subKeyMuxTableLineOffsets[2] = (6 - keyMuxBanks);
                    keyMuxBanks =
                        (tcamLookupProfileCfg.subKeySizes[3] >= CPSS_DXCH_TCAM_RULE_SIZE_60_B_E)
                        ? 6 : ((GT_U32)tcamLookupProfileCfg.subKeySizes[3] + 1);
                    tcamLookupProfileCfg.subKeyMuxTableLineOffsets[3] = (6 - keyMuxBanks);
                    tcamLookupProfileCfg.sharedMuxTableLineIndex  = ((tcamProfileId + 50) % numOfMuxLines);
                    tcamLookupProfileCfg.sharedMuxTableLineOffset = (subKeyPattern % 4);
                    ...
                    */
                    ...
                    st = cpssDxChTcamClientGroupLookupProfileCfgSet(
                        devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
                    UTF_VERIFY_EQUAL4_PARAM_MAC(
                        GT_OK, st, devNum, tcamClientGroup, tcamProfileId, subKeyPattern);
                    if (st == GT_OK)
                    {
                        st = cpssDxChTcamClientGroupLookupProfileCfgGet(
                            devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfgGet);
                        UTF_VERIFY_EQUAL4_PARAM_MAC(
                            GT_OK, st, devNum, tcamClientGroup, tcamProfileId, subKeyPattern);

                        if (tcamClientGroup != tcamClientGroupMax)
                        {
                            for (index = 0; (index < cfgWordSize); index++)
                            {
                                UTF_VERIFY_EQUAL5_PARAM_MAC(
                                    ((GT_U32*)&tcamLookupProfileCfg)[index],
                                    ((GT_U32*)&tcamLookupProfileCfgGet)[index],
                                    devNum, tcamClientGroup, tcamProfileId, subKeyPattern, index);
                            }
                        }
                        else
                        {
                            /* last client group reduced configuration */
                            UTF_VERIFY_EQUAL4_PARAM_MAC(
                                tcamLookupProfileCfg.subKeySizes[0],
                                tcamLookupProfileCfgGet.subKeySizes[0],
                                devNum, tcamClientGroup, tcamProfileId, subKeyPattern);
                            UTF_VERIFY_EQUAL4_PARAM_MAC(
                                tcamLookupProfileCfg.subKeyMuxTableLineIndexes[0],
                              tcamLookupProfileCfgGet.subKeyMuxTableLineIndexes[0],
                              devNum, tcamClientGroup, tcamProfileId, subKeyPattern);
                            UTF_VERIFY_EQUAL4_PARAM_MAC(
                                tcamLookupProfileCfg.subKeyMuxTableLineOffsets[0],
                                tcamLookupProfileCfgGet.subKeyMuxTableLineOffsets[0],
                                devNum, tcamClientGroup, tcamProfileId, subKeyPattern);
                        }
                   }
                }
        ...
        tcamClientGroup = 0;
        tcamProfileId = 1;
        tcamLookupProfileCfg.subKeySizes[0] = CPSS_DXCH_TCAM_RULE_SIZE_30_B_E;
        tcamLookupProfileCfg.subKeySizes[1] = CPSS_DXCH_TCAM_RULE_SIZE_30_B_E;
        tcamLookupProfileCfg.subKeySizes[2] = CPSS_DXCH_TCAM_RULE_SIZE_30_B_E;
        tcamLookupProfileCfg.subKeySizes[3] = CPSS_DXCH_TCAM_RULE_SIZE_30_B_E;
        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[0] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[1] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[2] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[3] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineOffsets[0] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineOffsets[1] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineOffsets[2] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineOffsets[3] = 0;
        tcamLookupProfileCfg.sharedMuxTableLineIndex  = 0;
        tcamLookupProfileCfg.sharedMuxTableLineOffset = 0;

        st = cpssDxChTcamClientGroupLookupProfileCfgSet(
            devNum, tcamClientGroup, tcamProfileId, NULL);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PTR, st);

        tcamClientGroup = tcamClientGroupMax + 1;
        st = cpssDxChTcamClientGroupLookupProfileCfgSet(
            devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PARAM, st);
        ...
        tcamProfileId = 1;
        tcamLookupProfileCfg.subKeySizes[0] = (CPSS_DXCH_TCAM_RULE_SIZE_ENT)100;
        tcamProfileId = 64;
        st = cpssDxChTcamClientGroupLookupProfileCfgSet(
            devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PARAM, st);
        ...
        tcamLookupProfileCfg.subKeySizes[0] = CPSS_DXCH_TCAM_RULE_SIZE_30_B_E;
        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[0] = 48;
        st = cpssDxChTcamClientGroupLookupProfileCfgSet(
            devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PARAM, st);

        tcamLookupProfileCfg.subKeyMuxTableLineIndexes[0] = 0;
        tcamLookupProfileCfg.subKeyMuxTableLineOffsets[0] = 6;
        st = cpssDxChTcamClientGroupLookupProfileCfgSet(
            devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
        UTF_VERIFY_EQUAL0_PARAM_MAC(GT_BAD_PARAM, st);
...
*/
UTF_TEST_CASE_MAC(cpssDxChTcamClientGroupLookupProfileCfgGet)
{
    GT_STATUS                             st              = GT_OK;
    GT_U8                                 devNum;
    GT_U32                                tcamProfileId;
            ...
            for (tcamProfileId = 1; (tcamProfileId < numOfTcamProfiles); tcamProfileId++)
            {
                ...
                */
                st = cpssDxChTcamClientGroupLookupProfileCfgGet(
                    devNum, tcamClientGroup, tcamProfileId, &tcamLookupProfileCfg);
                UTF_VERIFY_EQUAL3_PARAM_MAC(
                    GT_OK, st, devNum, tcamClientGroup, tcamProfileId);
            }
        }
...
/*#endif*/
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTcamClientGroupLookupMuxTableLineSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTcamClientGroupLookupMuxTableLineGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTcamClientGroupLookupProfileCfgSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChTcamClientGroupLookupProfileCfgGet)
