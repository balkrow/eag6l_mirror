...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* 1.1 Iterate over all drop mode values
                Expected values:
                GT_OK for SIP_5 and above
                for non SIP_5 expected GT_BAD_PARAM starting from CPSS_DXCH_BRG_DROP_CNTR_MAC_SA_MOVED_E
                */
        for(dropMode = CPSS_DXCH_BRG_DROP_CNTR_COUNT_ALL_E; dropMode <= CPSS_DXCH_BRG_DROP_CNTR_MAC_SPOOF_E; dropMode++)
        {
            if((dropMode >= CPSS_DXCH_BRG_DROP_CNTR_MAC_SA_MOVED_E) && (PRV_CPSS_SIP_5_CHECK_MAC(dev) == GT_FALSE))
            {
                expected_st = GT_BAD_PARAM;
            }

            /* Set Drop mode */
            st = cpssDxChBrgCntDropCntrModeSet(dev, dropMode);
            UTF_VERIFY_EQUAL1_STRING_MAC(expected_st, st,
                "cpssDxChBrgCntDropCntrModeSet failed: dev = %d", dev);
            /* get Drop mode */
            st = cpssDxChBrgCntDropCntrModeGet(dev, &dropModeRet);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                "cpssDxChBrgCntDropCntrModeGet failed: dev = %d", dev);
            /* Verifying values */
            if((dropMode < CPSS_DXCH_BRG_DROP_CNTR_MAC_SA_MOVED_E) || (PRV_CPSS_SIP_5_CHECK_MAC(dev) == GT_TRUE))
        ...
        /*
           1.2. Call with wrong enum values dropMode.
           Expected: GT_BAD_PARAM.
        */
        ...
        */
        ...
        else
        {
            ...
            if (maxVlanId!=BIT_16)
            {
                ...
                st = cpssDxChBrgCntBridgeIngressCntrModeSet(dev, cntrSetId,setMode, port, vlan);
                UTF_VERIFY_EQUAL5_PARAM_MAC(GT_OK, st, dev, cntrSetId,setMode, port, vlan);
            }
        }
        ...
        */
        ...
        else
        {
            ...
            if (maxVlanId!=BIT_16)
            {
                vlan = (GT_U16)maxVlanId;
                st = cpssDxChBrgCntBridgeIngressCntrModeSet(dev, cntrSetId,setMode, port, vlan);
                UTF_VERIFY_NOT_EQUAL3_STRING_MAC(GT_OK, st, "%d, %d, vlan = %d",dev, cntrSetId, vlan);
