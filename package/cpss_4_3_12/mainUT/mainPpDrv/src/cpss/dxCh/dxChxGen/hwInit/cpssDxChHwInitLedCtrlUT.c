...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        indicatedPort = CPSS_DXCH_LED_PORT27_E;
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = cpssDxChLedStreamPortClassPolarityInvertEnableSet(
            dev, portNum, classNum, GT_FALSE);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, portNum);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChLedParallelConfigSet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function with valid parameteres.
    Expected: GT_OK.
    1.2. Call cpssDxChLedParallelConfigGet
    Expected: GT_OK and the same parameters as were set.
    1.3. Call with out of range dataBitSelector.
    Expected: GT_OUT_OF_RANGE.
    1.4. Call with ledParallelConfigPtr NULL pointer.
    Expected: GT_BAD_PTR.
*/
    GT_STATUS   st       = GT_OK;
    ...
    CPSS_LED_PARALLEL_BIT_CONFIG_STC  ledParallelBitConfig;
    CPSS_LED_PARALLEL_BIT_CONFIG_STC  ledParallelBitConfigGet;
    CPSS_LED_PARALLEL_CONFIG_STC      ledParallelConfig;
    CPSS_LED_PARALLEL_CONFIG_STC      ledParallelConfigGet;

    /* prepare device iterator */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, (UTF_IRONMAN_L_E | UTF_AAS_E));

    cpssOsMemSet (&ledParallelBitConfig, 0, sizeof(ledParallelBitConfig));
    cpssOsMemSet (&ledParallelBitConfigGet, 0, sizeof(ledParallelBitConfigGet));
    cpssOsMemSet (&ledParallelConfig, 0, sizeof(ledParallelConfig));
    cpssOsMemSet (&ledParallelConfigGet, 0, sizeof(ledParallelConfigGet));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for (state = GT_FALSE; state <= GT_TRUE; state++)
        {
            ledParallelConfig.ledParallelModeEnable = state;

            for (bitSelector = 0; bitSelector < CPSS_LED_BIT_SELECTORS_MAX_NUM_CNS; bitSelector++)
            {
                ledParallelBitConfig.dataBitSelector = cpssOsRand() % IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS;
                for (polarityInvert = GT_FALSE; polarityInvert <= GT_TRUE; polarityInvert++)
                {
                    ledParallelBitConfig.polarityInvert = polarityInvert;

                    ledParallelConfig.ledParallelBitConfigArr[bitSelector] = ledParallelBitConfig;
                    /*
                        1.1. Call function with state [GT_FALSE / GT_TRUE]
                             for all valid bitSelector and polarityInvert.
                        Expected: GT_OK.
                    */
                    st = cpssDxChLedParallelConfigSet(dev, &ledParallelConfig);
                    UTF_VERIFY_EQUAL4_PARAM_MAC(GT_OK, st, dev,
                                                ledParallelConfig.ledParallelModeEnable,
                                                ledParallelBitConfig.dataBitSelector,
                                                ledParallelBitConfig.polarityInvert);
                    /*
                        1.2. Call cpssDxChLedParallelConfigGet
                        Expected: GT_OK and the same ledParallelModeEnable, dataBitSelector and polarityInvert.
                    */
                    st = cpssDxChLedParallelConfigGet(dev, &ledParallelConfigGet);
                    ledParallelBitConfigGet = ledParallelConfig.ledParallelBitConfigArr[bitSelector];
                    UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                                   "cpssDxChLedParallelConfigGet: %d", dev);
                    UTF_VERIFY_EQUAL2_STRING_MAC(ledParallelConfig.ledParallelModeEnable,
                                                 ledParallelConfigGet.ledParallelModeEnable,
                                   "get another mode than was set: %d %d",
                                                 ledParallelConfig.ledParallelModeEnable,
                                                 ledParallelConfigGet.ledParallelModeEnable);
                    UTF_VERIFY_EQUAL2_STRING_MAC(ledParallelBitConfig.dataBitSelector,
                                                 ledParallelBitConfigGet.dataBitSelector,
                                   "get another data bit selector than was set: %d %d",
                                                 ledParallelBitConfig.dataBitSelector,
                                                 ledParallelBitConfigGet.dataBitSelector);
                    UTF_VERIFY_EQUAL2_STRING_MAC(ledParallelBitConfig.polarityInvert,
                                                 ledParallelBitConfigGet.polarityInvert,
                                   "get another polarity than was set: %d %d",
                                                 ledParallelBitConfig.polarityInvert,
                                                 ledParallelBitConfigGet.polarityInvert);
                }
            }
        ...
        /*
            1.3. Call with dataBitSelector out of range value.
            Expected: GT_OUT_OF_RANGE.
        */
        for (bitSelector = 0; bitSelector < CPSS_LED_BIT_SELECTORS_MAX_NUM_CNS; bitSelector++)
        {
            ledParallelConfig.ledParallelBitConfigArr[bitSelector].dataBitSelector = IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS;
            st = cpssDxChLedParallelConfigSet(dev, &ledParallelConfig);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, ledParallelConfig.ledParallelBitConfigArr[bitSelector].dataBitSelector);
        }
        /*
            1.4. Call with ledParallelConfigPtr NULL pointer.
            Expected: GT_NULL_PTR.
        */
        st = cpssDxChLedParallelConfigSet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChLedParallelConfigGet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function with valid parameters
    Expected: GT_OK.
    1.2. Call with ledParallelConfigPtr NULL pointer.
    Expected: GT_BAD_PTR.
*/
    GT_STATUS   st       = GT_OK;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChLedParallelConfigGet(dev, &ledParallelConfig);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                       "cpssDxChLedParallelConfigGet: %d", dev);

        /*
            1.2. Call with ledParallelConfig NULL pointer.
            Expected: GT_NULL_PTR.
        */
        st = cpssDxChLedParallelConfigGet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChLedStreamReorderMapSet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function with valid parameteres.
    ...
    1.2. Call cpssDxChLedParallelConfigGet
    Expected: GT_OK and the same parameters as were set.
    1.3. Call function with zero array size and NULL pointer.
    Expected: GT_OK.
    1.4. Call cpssDxChLedParallelConfigGet
    Expected: GT_OK and the same parameters as were set.
    1.5. Call with out of range array size.
    Expected: GT_OUT_OF_RANGE.
    1.6. Call with out of range array values.
    Expected: GT_OUT_OF_RANGE.
*/
    GT_STATUS   st       = GT_OK;
    ...
    GT_BOOL     enable;
    GT_BOOL     enableGet;
    GT_U32      reorderMapArraySize = IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS;
    GT_U32      ledSerialOutputBitMapArr[IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS];
    GT_U32      ledSerialOutputBitMapArrGet[IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS];
    GT_U32      ii;

    /* prepare device iterator */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, (UTF_IRONMAN_L_E | UTF_AAS_E));

    cpssOsMemSet (&ledSerialOutputBitMapArr, 0, sizeof(ledSerialOutputBitMapArr));
    cpssOsMemSet (&ledSerialOutputBitMapArrGet, 0, sizeof(ledSerialOutputBitMapArrGet));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        for (enable = GT_FALSE; enable <= GT_TRUE; enable++)
        {
            ...
            {
                ledSerialOutputBitMapArr[ii] = cpssOsRand() % IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS;
            }
            /*
                1.1. Call function with valid parameters.
                Expected: GT_OK.
            */
            st = cpssDxChLedStreamReorderMapSet(dev, enable, reorderMapArraySize, ledSerialOutputBitMapArr);
            UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st, dev,
                                        enable,
                                        reorderMapArraySize);
            /*
                1.2. Call cpssDxChLedParallelConfigGet
                Expected: GT_OK and the same get parameters.
            */
            st = cpssDxChLedStreamReorderMapGet(dev, reorderMapArraySize, &enableGet, ledSerialOutputBitMapArrGet);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                           "cpssDxChLedStreamReorderMapGet: %d", dev);
            UTF_VERIFY_EQUAL2_STRING_MAC(enable,
                                         enableGet,
                           "get another state than was set: %d %d",
                                         enable,
                                         enableGet);
            for (ii = 0; ii < reorderMapArraySize; ii++)
            {
                UTF_VERIFY_EQUAL3_STRING_MAC(ledSerialOutputBitMapArr[ii],
                                             ledSerialOutputBitMapArrGet[ii],
                               "get another data bit map than was set: %d %d %d",
                                             ii,
                                             ledSerialOutputBitMapArr[ii],
                                             ledSerialOutputBitMapArrGet[ii]);
            }
            ...
            /*
                ...
                Expected: GT_OK.
            */
            st = cpssDxChLedStreamReorderMapSet(dev, enable, 0, NULL);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                           "cpssDxChLedStreamReorderMapGet: %d", dev);
            /*
                1.4. Call cpssDxChLedParallelConfigGet
        ...
        /*
            ...
            Expected: GT_OUT_OF_RANGE.
        */
        st = cpssDxChLedStreamReorderMapSet(dev, enable, reorderMapArraySize+1, ledSerialOutputBitMapArr);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OUT_OF_RANGE, st, dev,
                                    enable,
                                    reorderMapArraySize+1);
        ...
        */
        for (ii = 0; ii < reorderMapArraySize; ii++)
        {
            ledSerialOutputBitMapArr[ii] = IRONMAN_LED_SERIAL_DATA_STREAM_WIDTH_CNS;
        }
        st = cpssDxChLedStreamReorderMapSet(dev, enable, reorderMapArraySize, ledSerialOutputBitMapArr);
        UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OUT_OF_RANGE, st, dev,
                                    enable,
...
*/
UTF_TEST_CASE_MAC(cpssDxChLedStreamReorderMapGet)
{
/*
    ITERATE_DEVICES (Ironman)
    1.1. Call function with valid parameters
    Expected: GT_OK.
    1.2. Call with zero array size and NULL pointer.
    Expected: GT_OK.
    1.3. Call with enable NULL pointer.
    Expected: GT_BAD_PTR.
    1.4. Call with out of range array size.
    Expected: GT_OUT_OF_RANGE.
*/
    GT_STATUS   st       = GT_OK;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChLedStreamReorderMapGet(dev, reorderMapArraySize, &enable, ledSerialOutputBitMapArr);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                       "cpssDxChLedStreamReorderMapGet: %d", dev);
        ...
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChLedStreamReorderMapGet(dev, reorderMapArraySize, &enable, NULL);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                       "cpssDxChLedStreamReorderMapGet: %d", dev);
        ...
        /*
            ...
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChLedStreamReorderMapGet(dev, reorderMapArraySize, NULL, NULL);
        ...
        /*
            ...
            Expected: GT_OUT_OF_RANGE.
        */
        st = cpssDxChLedStreamReorderMapGet(dev, reorderMapArraySize+1, &enable, NULL);
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChHwInitLedCtrl)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChLedParallelConfigSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChLedParallelConfigGet)

    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChLedStreamReorderMapSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChLedStreamReorderMapGet)
