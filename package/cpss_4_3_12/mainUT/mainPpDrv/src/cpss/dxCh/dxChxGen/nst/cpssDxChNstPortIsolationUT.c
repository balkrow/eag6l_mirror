...
/*even if the device support 512 port , still the port isolation not supports more than 256 or 128
  SIP7 support all available physical ports */
#define PORT_NUM_MAX_PORT_ISOLATION_CNS(__dev) (PRV_CPSS_SIP_7_CHECK_MAC(__dev)? 0xFFFFFFFF :(PRV_CPSS_SIP_6_CHECK_MAC(__dev) ? 128 : 256))
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        if(!PRV_CPSS_SIP_7_CHECK_MAC(dev) && UTF_CPSS_PP_HW_INFO_E_ARCH_ENABLED_MAC(dev))
        {
