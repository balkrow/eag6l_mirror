...
GT_STATUS debug_cpssDxChBrgFdbBankCounterReset
(
    IN  GT_U8           devNum
);
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChBrgFdbPortConfigSet
(
    IN GT_U8                                devNum,
    IN GT_U8                                port,
    IN CPSS_DXCH_BRG_FDB_PORT_CONFIG_STC    *configPtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbPortConfigSet)
{
/*
    1.1.1. Call with valid values
           configPtr->autoLearnEnable   [GT_FALSE, GT_TRUE]
           configPtr->naMsgToCpuEnable  [GT_FALSE, GT_TRUE]
           configPtr->unkSrcAddrCmd     [CPSS_LOCK_FRWRD_E,
    ...
    1.1.2. Call values back and verify they are the same as were set
    Expected: get values are the same as were set

    1.1.3. Call API with wrong enum value cmd.
    Expected: GT_BAD_PARAM

    1.2. For all active devices go over all non available physical ports.
    Expected: GT_BAD_PARAM

    1.3. For active device check out of bound value for port number.
    Expected: GT_BAD_PARAM

    1.4. For active device check API with CPU port number
    Expected: GT_OK

    1.5 Call API with configPtr pointer as NULL
    Expected: GT_BAD_PTR.

    2. Check API for none active devices
...
*/
    ...
    GT_BOOL                stateGet = GT_FALSE;
    CPSS_PORT_LOCK_CMD_ENT cmdGet   = CPSS_LOCK_FRWRD_E;
    CPSS_DXCH_BRG_FDB_PORT_CONFIG_STC config;
    GT_U32                 i,j,k;
    GT_U32                 autoLearnEnableArr[2]  = {GT_FALSE,GT_TRUE};
    GT_U32                 naMsgToCpuEnableArr[2] = {GT_FALSE,GT_TRUE};
    CPSS_PORT_LOCK_CMD_ENT unkSrcAddrCmdArr[5] = {CPSS_LOCK_FRWRD_E,
                                                  CPSS_LOCK_SOFT_DROP_E,
                                                  CPSS_LOCK_DROP_E,
                                                  CPSS_LOCK_MIRROR_E,
                                                  CPSS_LOCK_TRAP_E};
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        while (GT_OK == prvUtfNextVirtPortGet(&port, GT_TRUE))
        {
            ...
            */
            ...
            for (i=0;i<2;i++)
            { /* loop on autoLearnEnableArr */
                config.autoLearnEnable = autoLearnEnableArr[i];
                for (j=0;j<2;j++)
                { /* loop on naMsgToCpuEnableArr */
                    config.naMsgToCpuEnable = naMsgToCpuEnableArr[j];
                    for (k=0;k<5;k++)
                    { /* loop on unkSrcAddrCmdArr */
                        config.unkSrcAddrCmd = unkSrcAddrCmdArr[k];

                        /* Call the API with valid values */
                        st = cpssDxChBrgFdbPortConfigSet(dev, port, &config);
                        /* Verify API finished OK */
                        UTF_VERIFY_EQUAL5_PARAM_MAC(GT_OK, st,
                                                    dev,
                                                    port,
                                                    config.autoLearnEnable,
                                                    config.naMsgToCpuEnable,
                                                    config.unkSrcAddrCmd);
                        ...
                        */
                        ...
                        /* Call cpssDxChBrgFdbNaToCpuPerPortGet API to read back naMsgToCpuEnable
                           parameter. Compare it to the value that was set */
                        st = cpssDxChBrgFdbNaToCpuPerPortGet(dev, port, &stateGet);
                        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,
                                   "cpssDxChBrgFdbNaToCpuPerPortGet - device:%d, port:%d", dev, port);
                        UTF_VERIFY_EQUAL2_STRING_MAC(config.naMsgToCpuEnable, stateGet,
                                   "naMsgToCpuEnable different than was set - device:%d, port:%d", dev, port);

                        /* Call cpssDxChBrgFdbPortLearnStatusGet API to read back autoLearnEnable
                           & unkSrcAddrCmd parameters. Compare it to the value that was set */
                        st = cpssDxChBrgFdbPortLearnStatusGet(dev, port, &stateGet, &cmdGet);
                        UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,
                                   "cpssDxChBrgFdbPortLearnStatusGet - device:%d, port:%d", dev, port);
                        UTF_VERIFY_EQUAL2_STRING_MAC(config.autoLearnEnable, stateGet,
                                   "autoLearnEnable different than was set - device:%d, port:%d", dev, port);
                        if (config.autoLearnEnable == GT_FALSE)
                        {
                            UTF_VERIFY_EQUAL2_STRING_MAC(config.unkSrcAddrCmd, cmdGet,
                                       "unkSrcAddrCmd different than was set - device:%d, port:%d", dev, port);

                            /*
                                1.1.3. Call API with wrong enum value cmd.
                                Expected: GT_BAD_PARAM.
                            */
                            config.unkSrcAddrCmd = 0xFFFFFFFF;
                            /* Call the API with invalid unkSrcAddrCmd  */
                            st = cpssDxChBrgFdbPortConfigSet(dev, port, &config);
        ...
        */
        ...
        /* Set cmd back to valid value */
        config.unkSrcAddrCmd = CPSS_LOCK_FRWRD_E;
        ...
        /*
           ...
           Expected: GT_BAD_PTR.
         */
        st = cpssDxChBrgFdbPortConfigSet(dev, port, NULL);
    ...
    port                    = BRG_FDB_VALID_PHY_PORT_CNS;
    config.autoLearnEnable  = GT_FALSE;
    config.naMsgToCpuEnable = GT_FALSE;
    config.unkSrcAddrCmd    = CPSS_LOCK_FRWRD_E;
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        if(PRV_CPSS_SIP_5_CHECK_MAC(dev))
        {
            ...
            else
            {
                ...
                if(PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks == 16)
                {
                    numEntries = 13;/* to check entry at index [hash[13]]*/
                    hashIndexMask = 0xf;/* 4 bits */
                }
                else
                if(PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks == 8)
                {
                    numEntries = 6;/* to check entry at index [hash[6]]*/
                    hashIndexMask = 0x7;/* 3 bits */
                }
                else /*numOfBanks == 4*/
                {
                    numEntries = 2;/* to check entry at index [hash[2]]*/
                    hashIndexMask = 0x3;/* 2 bits */
                }
            }
        ...
        if (PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            ...
            /* Maximal value */
            macEntry.userDefined = 0x3ff;

            /* Set current muxing mode */
            st = cpssDxChBrgFdbMacEntryMuxingModeSet(dev, CPSS_DXCH_FDB_MAC_ENTRY_MUXING_MODE_UDB_10_SRC_ID_1_DA_ACCESS_LEVEL_E);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChBrgFdbMacEntryMuxingModeSet: %d", dev);
        }
        ...
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            tgf_macEntryKey.entryType = PRV_TGF_FDB_ENTRY_TYPE_MAC_ADDR_E;
            tgf_macEntryKey.vid1  = 0;

            cpssOsMemCpy(&tgf_macEntryKey.key.macVlan.macAddr,
                         &macEntry.key.key.macVlan.macAddr,
                         sizeof(macEntry.key.key.macVlan.macAddr));
            tgf_macEntryKey.key.macVlan.vlanId = macEntry.key.key.macVlan.vlanId;
            ...
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                ...
                tgf_macEntryKey.entryType);

            st = utfCpssDxChBrgFdbMacEntryRead(dev, hwIndex, &validGet, &skipGet, &agedGet, &HwDevNumGet, &entryGet);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st,
                       "cpssDxChBrgFdbMacEntryRead: %d, %d", dev, hwIndex);
            ...
            /* Restore current muxing mode */
            st = cpssDxChBrgFdbMacEntryMuxingModeSet(dev, sip6FdbMacEntryMuxingMode);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "cpssDxChBrgFdbMacEntryMuxingModeSet: %d", dev);
        ...
        */
        ...
        /*   Sip7 support up to 64K vlans so we can not check any illegal value for
            GT_U16 vlanId
        */
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        if(!PRV_CPSS_SIP_7_CHECK_MAC(dev))/* the field is ignored in sip7 */
        {
        ...
        */
        macEntry.key.entryType          = CPSS_MAC_ENTRY_EXT_TYPE_IPV4_UC_E;
        macEntry.key.key.ipv4Unicast.vrfId = PRV_CPSS_SIP_7_CHECK_MAC(dev) ? BIT_15 : PRV_CPSS_MAX_NUM_VLANS_CNS;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        else if(PRV_CPSS_SIP_5_CHECK_MAC(dev))
        {
           ...
           */
            ...
            */
            ...
            if(!PRV_CPSS_SIP_6_CHECK_MAC(dev))/* the field is ignored in sip6 */
            {
                macEntry.fdbRoutingInfo.qosProfileIndex=1;
            }
            else
            ...
            */
            ...
            if(!PRV_CPSS_SIP_6_CHECK_MAC(dev))/* the field have single bit in sip6 */
            {
                macEntry.fdbRoutingInfo.mtuProfileIndex=2;
            }
            else
            ...
            */
            ...
            {
                UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st, "%d, macEntry.fdbRoutingInfo.countSet = %d should be ignored",
                                             dev, macEntry.fdbRoutingInfo.countSet);
            }
        ...
        if(PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            ...
            st = utfCpssDxChBrgFdbMacEntrySet(dev, &macEntry);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st, "%d, macEntry.fdbRoutingInfo.routingType = %d",
            ...
            st = utfCpssDxChBrgFdbMacEntrySet(dev, &macEntry);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st, "%d, macEntry.fdbRoutingInfo.multipathPointer = %d",
                                dev, macEntry.fdbRoutingInfo.multipathPointer);

            if(!PRV_CPSS_SIP_7_CHECK_MAC(dev))
            {
                macEntry.fdbRoutingInfo.multipathPointer = BIT_15;
            }
            else
            {
                macEntry.fdbRoutingInfo.multipathPointer = BIT_18;
            }
...
/* MAC MC test */
GT_VOID utfCpssDxChBrgFdbMacMcEntryWrite
(
    IN GT_U8                        dev
)
{
    ...
    /*
        1.31. Call function with index [8],
                    skip [GT_FALSE],
                    macEntryPtr {key {CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E,
                                      macVlan->vlanId[100],
                                      macVlan-> macAddr[01:1A:FF:FF:FF:FF]},
                                               dstInterface {type[CPSS_INTERFACE_PORT_E],
                                                             devPort {devNum [devNum],
                                                                      portNum [10]},
                                                             trunkId [0],
    ...
    Expected: GT_OK         - for SIP6_30 device.
              GT_BAD_PARAM  - for all other devices
    */
    index = 8;
    skip  = GT_FALSE;
    ...
    macEntry.dstInterface.type                  = CPSS_INTERFACE_PORT_E;
    macEntry.dstInterface.devPort.hwDevNum      = dev;
    macEntry.dstInterface.devPort.portNum       = 10;
    ...
    if(PRV_CPSS_SIP_6_30_CHECK_MAC(dev))
    {
        ...
        /*
            1.31. Call cpssDxChBrgFdbMacEntryRead with index [8]
                                                      and non-NULL pointers.
            Expected: GT_OK and the same parameters except VLAN as input was - check by fields (only valid).
        */
    ...
    /*
        1.32. Call function with index [8],
                    skip [GT_FALSE],
                    macEntryPtr {key {CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E,
                                      macVlan->vlanId[100],
                                      macVlan-> macAddr[01:1A:FF:FF:FF:FF]},
    ...
    */
    ...
    /*
        1.33. Call cpssDxChBrgFdbMacEntryRead with index [8]
                                                  and non-NULL pointers.
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /* MAC MC test */
        utfCpssDxChBrgFdbMacMcEntryWrite(dev);
    }
...
*/
    ...
    CPSS_MAC_SECURE_AUTO_LEARN_MODE_ENT modeGet = CPSS_MAC_SECURE_AUTO_LEARN_DISABLED_E;
...
*/
    ...
    /* prepare device iterator */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_CPSS_PP_ALL_SIP7_CNS);
...
static void prvCpssDxChTestFdbHash(IN GT_U8 dev)
{
    ...
    if (PRV_CPSS_SIP_5_10_CHECK_MAC(dev))
    {
        ...
        /* the test did not care about bank counters when added entries by index ,
           therefor we must clean after it , so it may not fail other test that will
           not be able to add FDB entries (by message) */
        (void)debug_cpssDxChBrgFdbBankCounterReset(dev);
    }
...
static GT_U32  aasFdbSizesArr[]     = {_8K,_16K,_32K,_64K,_128K,_256K,LAST_FDB_SIZE_CNS};
...
*/
    ...
    GT_U32                     hash;
    GT_STATUS                  expected;
    CPSS_MAC_HASH_FUNC_MODE_ENT     origHashMode;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        if (PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            /* Get device hash mode  */
            st = cpssDxChBrgFdbHashModeGet(dev, &origHashMode);
            if (st != GT_OK)
            {
                PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgFdbHashModeGet FAILED, rc = [%d] \n", st);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        */
        ...
        if (PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            /* Restore device hash mode  */
            st = cpssDxChBrgFdbHashModeSet(dev, origHashMode);
            if (st != GT_OK)
            {
                PRV_UTF_LOG1_MAC("[TGF]: cpssDxChBrgFdbHashModeSet FAILED, rc = [%d] \n", st);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: NOT GT_OK.
        */
        entryKey.key.ipv4Unicast.vrfId = PRV_CPSS_SIP_7_CHECK_MAC(dev) ? BIT_15 : 4096;
        st = cpssDxChBrgFdbHashCrcMultiResultsCalc(dev, &entryKey,
                                                   multiHashStartBankIndex,
                                                   numOfBank, crcMultiHashArr);
        UTF_VERIFY_NOT_EQUAL1_PARAM_MAC(GT_OK, st, dev);
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        bankIndex = PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks > 4 ? 5 : 2;

        st = cpssDxChBrgFdbBankCounterValueGet(dev, bankIndex, &value);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);

        /* call with bankIndex = 15 */
        bankIndex = PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks - 1;
        ...
        /*
            ...
            Expected: NOT GT_OK.
        */
        bankIndex = PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks;
...
)
{
    ...
    GT_U32                      maxMultiHash = PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks;/* max valid values in crcMultiHashArr[] */
    ...
    hashParams.numOfMultipleHashes = !PRV_CPSS_SIP_5_CHECK_MAC(dev) ? 0 :
        PRV_CPSS_DXCH_PP_MAC(dev)->bridge.fdbHashParams.numOfBanks;
    ...
    if(useMultiHash == GT_TRUE)
    {
        ...
        numIterations /= NUM_BANKS_MULTI_HASH_CNS;
    }
        ...
        else
        {
            ...
            for(multiHashIndex = 0 ; multiHashIndex  < maxMultiHash; multiHashIndex++)
            {
                ...
                if (UTF_CPSS_PP_HW_INFO_E_ARCH_ENABLED_MAC(dev))
                {
                    /* decrement Leaned MACs Counter */
                    st = cpssDxChBrgFdbBankCounterUpdate(
                        dev, (hashIndex / (tableLength / maxMultiHash))/*bankIndex*/, GT_FALSE /*incOrDec*/);
                    if (st != GT_OK)
                    {
                        utfGeneralStateMessageSave(7,"FAILED [%s]",STR(cpssDxChBrgFdbBankCounterUpdate));
                        return st;
...
/**
GT_STATUS cpssDxChBrgFdbSizeSet
(
    IN  GT_U8                           devNum,
    IN  CPSS_DXCH_BRG_FDB_TBL_SIZE_ENT  fdbSize
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbSizeSet)
{
    GT_STATUS                                      st       = GT_OK;
    GT_U8                                          dev;
    CPSS_DXCH_BRG_FDB_TBL_SIZE_ENT  fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_256K_E+1;
    GT_U32                          fdbSizeValue;
    GT_U32                          fdbSizeValueGet;
    GT_U32                          fdbSizeValueRestore;

    /* prepare iterator for go over all active devices */
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(&dev, UTF_NONE_FAMILY_E);

    /* 1. Go over all active devices.
     * Verify 8K and 16K size */
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* Store the initial value for restore */
        fdbSizeValueRestore = PRV_CPSS_DXCH_PP_MAC(dev)->fineTuning.tableSize.fdb;
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                "cpssDxChBrgFdbPortLearnStatusGet: %d, %d", dev);

        do{
            fdbSize--;
            switch(fdbSize)
            {
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_8K_E:     fdbSizeValue = _8K;    break;
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_16K_E:    fdbSizeValue = _16K;   break;
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_32K_E:    fdbSizeValue = _32K;   break;
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_64K_E:    fdbSizeValue = _64K;   break;
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_128K_E:   fdbSizeValue = _128K;  break;
                case CPSS_DXCH_BRG_FDB_TBL_SIZE_256K_E:   fdbSizeValue = _256K;  break;
                default:
                    /* Will never hit this scenario */
                    PRV_UTF_LOG1_MAC("cpssDxChBrgFdbSizeSet: Got Invalid size %d\n", fdbSize);
                    return;
            }

            if(fdbSizeValue > PRV_CPSS_DXCH_PP_MAC(dev)->fineTuning.tableSize.fdb)
            {
                st = cpssDxChBrgFdbSizeSet(dev, fdbSize);
                UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, fdbSize);
            }
            else
            {
                st = cpssDxChBrgFdbSizeSet(dev, fdbSize);
                UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, fdbSize);

                st = cpssDxChCfgTableNumEntriesGet(dev, CPSS_DXCH_CFG_TABLE_FDB_E, &fdbSizeValueGet);
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                        "cpssDxChBrgFdbPortLearnStatusGet: %d, %d", dev);

                UTF_VERIFY_EQUAL1_STRING_MAC(fdbSizeValue, fdbSizeValueGet,
                        "get another status than was set: %d, %d", dev);
            }
        } while(fdbSize != CPSS_DXCH_BRG_FDB_TBL_SIZE_8K_E);

        /* Restore the fdb size */
        switch(fdbSizeValueRestore)
        {
            case _4K:    fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_4K_E;    break;
            case _8K:    fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_8K_E;    break;
            case _16K:   fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_16K_E;   break;
            case _32K:   fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_32K_E;   break;
            case _64K:   fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_64K_E;   break;
            case _128K:  fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_128K_E;  break;
            case _256K:  fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_256K_E;  break;
            default:
                PRV_UTF_LOG1_MAC("cpssDxChBrgFdbSizeSet: restore Invalid size %d\n", fdbSize);
                return;
        }
        PRV_CPSS_DXCH_PP_MAC(dev)->fineTuning.tableSize.fdb = fdbSizeValueRestore;
        st = cpssDxChBrgFdbSizeSet(dev, fdbSize);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, fdbSize);
    }

    dev = 0;
    fdbSize = CPSS_DXCH_BRG_FDB_TBL_SIZE_0_E+1;
    st = cpssDxChBrgFdbSizeSet(dev, fdbSize);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, fdbSize);
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChBrgFdbMcAddrDelEnableSet
(
    IN GT_U8      devNum,
    IN GT_BOOL    enable
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbMcAddrDelEnableSet)
{
/*
    1.1.1. Call with valid values
    ...
    1.1.3 Check API for none active devices
    Expected: GT_NOT_APPLICABLE_DEVICE

    1.1.4 Call function with out of bound value for device id
    Expected: GT_BAD_PARAM
*/
    ...
    GT_BOOL                         enableGet;
    GT_U32                          i;
    GT_BOOL                         enableArr[2]    = {GT_FALSE, GT_TRUE};
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        { /* loop on enableArr */
            ...
            /* Call the API with valid values */
            st = cpssDxChBrgFdbMcAddrDelEnableSet(dev, enable);
            /* Verify API finished OK */
            UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, enable);
            ...
            /* Call cpssDxChBrgFdbMcAddrDelEnableGet API to read back enable.
               Compare it to the value that was set */
            st = cpssDxChBrgFdbMcAddrDelEnableGet(dev, &enableGet);
            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                       "cpssDxChBrgFdbMcAddrDelEnableGet - device:%d ", dev);
            UTF_VERIFY_EQUAL1_STRING_MAC(enable, enableGet,
                       "enable different than was set - device:%d", dev);
        }
    }
...
/**
GT_STATUS cpssDxChBrgFdbMcAddrDelEnableGet
(
    IN  GT_U8      devNum,
    OUT GT_BOOL    *enablePtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbMcAddrDelEnableGet)
{
    GT_STATUS    st      = GT_OK;
    GT_U8        dev;
    GT_BOOL      enableGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChBrgFdbMcAddrDelEnableGet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
...
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChBrgFdbSpConfigSet
(
    IN GT_U8                              devNum,
    IN CPSS_DXCH_BRG_FDB_SP_CONFIG_STC    *spConfigPtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbSpConfigSet)
{
/*
    1.1.1. Call with valid values
           spConfigPtr->spEntryFastAgeEnable    [GT_FALSE, GT_TRUE]
           spConfigPtr->spLearnEnableNaToCpu    [GT_FALSE, GT_TRUE]
    Expected: GT_OK

    1.1.2. Call values back and verify they are the same as were set
    Expected: get values are the same as were set

    1.1.3 Call API with spConfigPtr pointer as NULL
    Expected: GT_BAD_PTR.

    1.1.4 Check API for none active devices
    Expected: GT_NOT_APPLICABLE_DEVICE

    1.1.5 Call function with out of bound value for device id
    Expected: GT_BAD_PARAM
*/

    GT_STATUS                       st       = GT_OK;
    GT_U8                           dev;
    CPSS_DXCH_BRG_FDB_SP_CONFIG_STC spConfig;
    CPSS_DXCH_BRG_FDB_SP_CONFIG_STC spConfigGet;
    GT_U32                          i, j;
    GT_U32                          spEntryFastAgeEnableArr[2] = {GT_FALSE, GT_TRUE};
    GT_U32                          spLearnEnableNaToCpuArr[2] = {GT_FALSE, GT_TRUE};
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Clear structure before filling it */
        cpssOsMemSet(&spConfig, 0, sizeof(spConfig));

        for (i = 0; i < 2; i++)
        { /* loop on spEntryFastAgeEnableArr */
            spConfig.spEntryFastAgeEnable = spEntryFastAgeEnableArr[i];
            for (j = 0; j < 2; j++)
            { /* loop on spLearnEnableNaToCpuArr */
                spConfig.spLearnEnableNaToCpu = spLearnEnableNaToCpuArr[j];

                /* Call the API with valid values */
                st = cpssDxChBrgFdbSpConfigSet(dev, &spConfig);
                /* Verify API finished OK */
                UTF_VERIFY_EQUAL3_PARAM_MAC(GT_OK, st,
                                            dev,
                                            spConfig.spEntryFastAgeEnable,
                                            spConfig.spLearnEnableNaToCpu);
                ...
                /* Call cpssDxChBrgFdbSpConfigGet API to read back SP config.
                   Compare it to the value that was set */
                st = cpssDxChBrgFdbSpConfigGet(dev, &spConfigGet);
                UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                           "cpssDxChBrgFdbSpConfigGet - device:%d ", dev);
                UTF_VERIFY_EQUAL1_STRING_MAC(spConfig.spEntryFastAgeEnable, spConfigGet.spEntryFastAgeEnable,
                           "spEntryFastAgeEnable different than was set - device:%d", dev);
                UTF_VERIFY_EQUAL1_STRING_MAC(spConfig.spLearnEnableNaToCpu, spConfigGet.spLearnEnableNaToCpu,
                           "spLearnEnableNaToCpu different than was set - device:%d", dev);
            }
        }
        ...
        /*
           ...
           Expected: GT_BAD_PTR.
         */
        st = cpssDxChBrgFdbSpConfigSet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
    }
    ...
    spConfig.spEntryFastAgeEnable = GT_FALSE;
    spConfig.spLearnEnableNaToCpu = GT_FALSE;
...
/**
GT_STATUS cpssDxChBrgFdbSpConfigGet
(
    IN  GT_U8                              devNum,
    OUT CPSS_DXCH_BRG_FDB_SP_CONFIG_STC    *spConfigPtr
)
*/
UTF_TEST_CASE_MAC(cpssDxChBrgFdbSpConfigGet)
{
    GT_STATUS                       st      = GT_OK;
    GT_U8                           dev;
    CPSS_DXCH_BRG_FDB_SP_CONFIG_STC spConfigGet;
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChBrgFdbSpConfigGet(dev, NULL);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PTR, st, dev);
...
/*----------------------------------------------------------------------------*/
/**
* @internal dbgDxChFdbAgingTimeCheck function
* @endinternal
*
* @brief   function to measure actual FDB aging timeout
*          Function measure time between addition of new entry
*          and till aging daemon invalidate it.
*
*
* @param[in] devNum               - device number.
* @param[in] agingTime            - aging time to test in seconds
* @param[in] iterations           - number of iterations to measure aging.
* @param[in] timeoutInMilisec     - granularity of measured time in milliseconds.
* @param[in] entryIndex           - FDB entry index that is used by this function.
*
...
*/
GT_STATUS dbgDxChFdbAgingTimeCheck
(
    IN GT_U8 devNum,
    IN GT_U32 agingTime,
    IN GT_U32 iterations,
    IN GT_U32 timeoutInMilisec,
...
)
{
    ...
    GT_U32  secondsStart, secondsEnd,
            ...
            seconds, nanoSec; /* time of aging */


    st = cpssDxChBrgFdbActionModeSet(devNum, CPSS_FDB_ACTION_AGE_WITH_REMOVAL_E);
    if (st != GT_OK)
    {
        return st;
    }
    st = cpssDxChBrgFdbAAandTAToCpuSet(devNum, GT_FALSE);
    if (st != GT_OK)
    {
        return st;
    }

    st = cpssDxChBrgFdbAgingTimeoutSet(devNum,agingTime);
    if (st != GT_OK)
    {
        return st;
    }

    st = cpssDxChBrgFdbAgingTimeoutGet(devNum,&timeoutGet);
    if (st != GT_OK)
    {
        return st;
    }

    cpssOsPrintf("Check Aging actual timeout for input %d sec \n",agingTime);
    cpssOsPrintf("Use FDB entry# %d, check %d times, granularity %d milliseconds\n",entryIndex, iterations, timeoutInMilisec);

    if(timeoutGet != agingTime)
    {
        cpssOsPrintf("the timeout was set to [%d] (not [%d]) \n",timeoutGet,agingTime);
    }

    st = cpssDxChBrgFdbMacTriggerModeSet(devNum,CPSS_ACT_AUTO_E);
    if (st != GT_OK)
    {
        return st;
    }
    ...
    macEntry.key.key.macVlan.macAddr.arEther[4] = 0x44;
    macEntry.key.key.macVlan.macAddr.arEther[5] = 0x55;
    /* set the entry that needs 1 aging cycle */
    macEntry.age    = GT_FALSE;

    valid = GT_FALSE;

    /* two first iterations should be skipped to get only precise data:
       - first iteration is used to add FDB entry
       - second one may provide wrong result because aging daemon scans FDB
         constantly and first added entry may be added in the middle of scan.
    */
    iterations +=2;
    ...
    for (ii = 0; ii < iterations; ii++)
    {
        ...
        /* check Valid status of entry and stop when it became 'no valid' - aging done */
        do
        {
            cpssOsTimerWkAfter(timeoutInMilisec);
            st = cpssDxChBrgFdbMacEntryStatusGet(devNum,entryIndex, &valid, &skip);
            if (st != GT_OK)
            {
                return st;
            }
        ...
        if (ii > 1)
        {
            prvCpssTimeSecondsAndNanoDiff(secondsStart,nanoSecondsStart,secondsEnd,nanoSecondsEnd, &seconds, &nanoSec);

            timeoutGet = seconds * 1000 + nanoSec / 1000000;

            cpssOsPrintf(" Actual aging time in miliseconds %d\n", timeoutGet);
        }

        st = cpssDxChBrgFdbMacEntryWrite(devNum,entryIndex,GT_FALSE,&macEntry);
...
)
{
    ...
    /*
        this is debug function because the CPU should not interfere with the PP
    ...
    */
    ...
    for(ii = 0 ; ii < 16 ; ii++)
    {
        regAddr = PRV_CPSS_DXCH_DEV_REGS_VER1_MAC(devNum)->
                    FDB.FDBCore.FDBBankCntrs.FDBBankCntr[ii];

            /* Reset counter value */
            (void)prvCpssHwPpWriteRegister(devNum, regAddr,0);
    }
    return GT_OK;
}
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChBrgFdb)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbPortConfigSet)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbSizeSet)

    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbMcAddrDelEnableSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbMcAddrDelEnableGet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbSpConfigSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChBrgFdbSpConfigGet)
