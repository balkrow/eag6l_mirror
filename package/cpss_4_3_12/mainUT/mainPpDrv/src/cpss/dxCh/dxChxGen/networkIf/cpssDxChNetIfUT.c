...
/* GDMA TxGenerator queue chain size - Default set by application
* Used to restore after setting MAX size with prvCpssDxChNetIfGdmaSgdChainSizeSet(16bit)
*/
#define GDMA_TX_GENERATOR_CHAIN_SIZE  256
...
static GT_STATUS utCpssDxChNetIfPacketAdd
(
    IN  GT_U8                   dev,
    IN  GT_PORT_GROUPS_BMP      portGroupsBmp,
    IN  GT_U32                  txQueue,
    IN  GT_U32                  *packetIdPtr
)
{
    GT_STATUS                   st;
    CPSS_DXCH_NET_TX_PARAMS_STC packetParams;
    GT_HW_DEV_NUM               hwDevNum;
    GT_U8                       packetData[128]; /* could be any garbage */
    GT_U32                      packetDataLength = 64;
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        st = prvCpssDxChNetIfNumQueuesGet(dev, &toCpu_numOfQueues,&fromCpu_numOfQueues);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        /*
            1.1. Call with queueIdx[7], numOfBuff[100], not NULL packetBuffs, buffLen, rxParamsPtr.
            Expected: GT_OK.
        */
        ...
        /*
            ...
            Expected: NOT GT_OK.
        */
        queueIdx = (GT_U8)toCpu_numOfQueues;
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        {
            /* skip Traffic Generator Queue and CnM SRAM queue 4 to avoid traffic injection */
            if (UTF_CPSS_PP_HW_INFO_E_ARCH_ENABLED_MAC(dev) || PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(dev))
            {
                if ((queue == 3) || (queue  == 6) || (queue == 4))
                {
                    continue;
                }
            }
        ...
        /*
            ...
            Expected: NOT GT_OK.
        */
        queue = (GT_U8)fromCpu_numOfQueues;
...
/*
    ...
    1.1. Call with dsaInfoPtr{commonParams(dsaTagType[CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT /
    ...
    1.10. Call with dsaInfoPtr{commonParams(dsaTagType[CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT /
                                                      CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT],
                                           vpt [0 / 7],
                                           cfiBit [0 / 1],
                                           vid [0 / 100],
    ...
    1.24. Call with dsaInfoPtr{commonParams(dsaTagType[CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT /
                                                      CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT],
                                           vpt [0 / 7],
                                           cfiBit [0 / 1],
                                           vid [0 / 100],
    ...
    1.28. Call with dsaInfoPtr{commonParams(dsaTagType[CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT /
                                                      CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT],
                                           vpt [0 / 7],
                                           cfiBit [0 / 1],
                                           vid [0 / 100],
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            ...
            Expected: GT_OK.
        */
        /* iterate with dsaInfo.commonParams.dsaTagType = CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT */
        ...
        /* iterate with dsaInfo.commonParams.dsaTagType = CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT */
        ...
        */
        ...
        /*
            1.27 Call with dsaInfo.commonParams.dsaTagType [CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT] (rxtended rule)
                           dsaInfoPtr->dsaInfo.toAnalyzer.devPort.portNum [31] (out of range)
                           and other params from 1.24
            Expected: NOT GT_OK.
        */
...
UTF_TEST_CASE_MAC(cpssDxChNetIfSdmaPhysicalPortToQueueGet)
{
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(0, toCpu_numOfQueues);

        /* check that the 8 queues per CPU port are different from each other
           and that port <--> firstQueue are matched  */
        for(firstQueue = 0 ; firstQueue < toCpu_numOfQueues ; firstQueue += 8)
        {
...
*/
    ...
    if(GT_TRUE == PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(prvTgfDevNum))
    {
...
*/
    ...
    {
        /*
            1.1. Call with correct dev only for non SDMA case.
            It's wrong to call this function in SDMA case second time after cpssInitSystem.                                                 .
            Expected: NOT GT_OK.
        */
        if (!UTF_CPSS_PP_IS_SDMA_USED_MAC(dev))
        {
...
/*
    ...
    1.1. Call with pcktParamsPtr{
                      ...
                      dsaParam {commonParams {dsaTagType [CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT],
                                              vpt[0],
                                              cfiBit [0],
                                              vid [0],
    ...
    1.17. Call with pcktParamsPtr {
                      ...
                      sdmaInfo {recalcCrc [GT_FALSE],
                                txQueue [3],
                                invokeTxBufferQueueEvent [GT_FALSE] (not relevant)},
                      dsaParam {commonParams {dsaTagType [CPSS_DXCH_NET_DSA_2_WORD_TYPE_ENT],
                                              vpt[3],
                                              cfiBit [1],
                                              vid [100],
    ...
    Expected: GT_BAD_PTR.
    1.49. pcktParams.sdmaInfo.bypassQueueEnable = GT_TRUE
    Expected: GT_BAD_PARAM.
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        pcktParams.dsaParam.commonParams.dsaTagType     = CPSS_DXCH_NET_DSA_1_WORD_TYPE_ENT;
        if (UTF_CPSS_PP_HW_INFO_E_ARCH_ENABLED_MAC(dev))
        {
        ...
        */
        ...
        pcktParams.sdmaInfo.txQueue = (GT_U8)fromCpu_numOfQueues;
        ...
        */
        ...
        /*
            1.49. Call with pcktParams.sdmaInfo.bypassQueueEnable = GT_TRUE and other valid params same as 1.1.
            Expected: GT_BAD_PARAM.
        */
        pcktParams.sdmaInfo.bypassQueueEnable = GT_TRUE;

        st = cpssDxChNetIfSdmaSyncTxPacketSend(dev, &pcktParams, buffList,
                                              buffLenList, numOfBufs);

        UTF_VERIFY_EQUAL1_STRING_MAC(GT_BAD_PARAM, st, "%d, pcktParams.sdmaInfo.bypassQueueEnable = GT_TRUE", dev);

        /* restore previous value */
        pcktParams.sdmaInfo.bypassQueueEnable = GT_FALSE;
    }
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /*
            ...
            Expected: GT_BAD_PARAM.
        */
        queueIdx = (GT_U8)fromCpu_numOfQueues;
...
#if !defined(ASIC_SIMULATION) && !defined(CPU_BE)
/*----------------------------------------------------------------------------*/
/*
GT_STATUS cpssDxChNetIfSdmaTxPacketSendLater
(
    IN GT_U8                                    devNum,
    IN CPSS_DXCH_NET_TX_PARAMS_STC              *pcktParamsPtr,
    IN GT_U8                                    *buffList[],
...
*/
UTF_TEST_CASE_MAC(cpssDxChNetIfSdmaTxPacketSendLater)
{
    GT_STATUS                   st                      = GT_OK;
    GT_U8                       dev                     = 0;
    GT_U8                       i                       = 0;
    CPSS_DXCH_NET_TX_PARAMS_STC pcktParams;
    GT_U8                       *buffList[6];
    GT_U32                      buffLenList[6];
    GT_U32                      numOfBufs               = 6;
    GT_U32                      cookie                  = 0;
    CPSS_PP_FAMILY_TYPE_ENT     devFamily               = 0;
    CPSS_UNI_EV_CAUSE_ENT       evCause[1] = { CPSS_PP_TX_BUFFER_QUEUE_E };
    GT_UINTPTR                  evReqHndl;
    GT_U8                       packetHeader[14] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};

    buffList[0] = cpssOsCacheDmaMalloc(64);
    buffLenList[0] = 64;
    buffList[1] = cpssOsCacheDmaMalloc(128);
    buffLenList[1] = 128;
    buffList[2] = cpssOsCacheDmaMalloc(256);
    buffLenList[2] = 256;
    buffList[3] = cpssOsCacheDmaMalloc(512);
    buffLenList[3] = 512;
    buffList[4] = cpssOsCacheDmaMalloc(1024);
    buffLenList[4] = 1024;
    buffList[5] = cpssOsCacheDmaMalloc(2048);
    buffLenList[5] = 2048;

    cpssOsBzero((GT_VOID*) &pcktParams, sizeof(pcktParams));

    cpssOsMemSet((GT_VOID*) buffList[0], 0x11, buffLenList[0]);
    cpssOsMemSet((GT_VOID*) buffList[1], 0x22, buffLenList[1]);
    cpssOsMemSet((GT_VOID*) buffList[2], 0x33, buffLenList[2]);
    cpssOsMemSet((GT_VOID*) buffList[3], 0x44, buffLenList[3]);
    cpssOsMemSet((GT_VOID*) buffList[4], 0x55, buffLenList[4]);
    cpssOsMemSet((GT_VOID*) buffList[5], 0x66, buffLenList[5]);

    cpssOsMemCpy(buffList[0], packetHeader, sizeof(packetHeader));
    cpssOsMemCpy(buffList[1], packetHeader, sizeof(packetHeader));
    cpssOsMemCpy(buffList[2], packetHeader, sizeof(packetHeader));
    cpssOsMemCpy(buffList[3], packetHeader, sizeof(packetHeader));
    cpssOsMemCpy(buffList[4], packetHeader, sizeof(packetHeader));
    cpssOsMemCpy(buffList[5], packetHeader, sizeof(packetHeader));
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        pcktParams.dsaParam.commonParams.vpt            = 0x0;
        pcktParams.dsaParam.commonParams.cfiBit         = 0x0;

        if (pcktParams.dsaParam.commonParams.dsaTagType == CPSS_DXCH_NET_DSA_4_WORD_TYPE_ENT)
        {
            pcktParams.dsaParam.commonParams.vid = 0xFFE;
        }
        else
        {
            pcktParams.dsaParam.commonParams.vid = 0xFFF;
        ...
        st = extUtilHwDeviceNumberGet(dev,
        ...
        st = cpssDxChNetIfSdmaTxQueueEnable(dev, pcktParams.sdmaInfo.txQueue, GT_FALSE);

        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "%d", dev);

        for(i=0; i<numOfBufs; i++)
        {
            st = cpssDxChNetIfSdmaTxPacketSend(dev, &pcktParams, buffList+i,
                                                buffLenList+i, 1);

            UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "%d", dev);
        }

        st = cpssDxChNetIfSdmaTxQueueEnable(dev, pcktParams.sdmaInfo.txQueue, GT_TRUE);

        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st, "%d", dev);
    }

    /* Wait for CPSS_PP_TX_BUFFER_QUEUE_E to be processed */
...

    {
        ...
        for(i=0; i<numOfBufs; i++)
        {
            /* Free pool buffers used during Async send */
            st = cpssDxChNetIfTxBufferQueueGet(evReqHndl, &devPtr, &cookiePtr, &queuePtr, &statusPtr);
    ...
    cpssOsCacheDmaFree(buffList[1]);
    cpssOsCacheDmaFree(buffList[2]);
    cpssOsCacheDmaFree(buffList[3]);
    cpssOsCacheDmaFree(buffList[4]);
    cpssOsCacheDmaFree(buffList[5]);
}
#endif
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            */
            ...
            /*
                ...
                Expected: GT_BAD_PARAM.
            */
            txQueue = (PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev)) ?
                      PRV_CPSS_NUM_OF_GDMA_RINGS_CNS : NUM_OF_TX_QUEUES;
            st = cpssDxChNetIfSdmaTxGeneratorBurstStatusGet(dev, portGroupsBmp,
                                                            txQueue, &burstStatus);
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(dev, portGroupId)
        {
            ...
            */
            ...
            /* The SDMA descriptors are linked to list up to (512M-1) entries.
             * The GDMA descriptors are organized as (16k-1) array of descriptors.
             * The number of descriptors and resulting array size is configured by
             * appDemo at INIT-time and allocated. "num of descs" could be REDUCED at
             * runtime by reducing Ring-Chain-Size in the control register but
             * it CANNOT be increased => GDMA is tested for MAX=256 only.
             */
            /*
                1.1.1. Call with txQueue [6],
                                 burstEnable[GT_FALSE, GT_TRUE, GT_TRUE],
            ...
            */
            txQueue = 6;
            burstEnable = GT_TRUE;
            burstPacketsNumber = PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev) ?
                                 GDMA_TX_GENERATOR_CHAIN_SIZE/2 : BIT_28;
            st = cpssDxChNetIfSdmaTxGeneratorEnable(dev, portGroupsBmp,
                                                    txQueue, burstEnable,
                                                    burstPacketsNumber);
            if (PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev))
            {
                /* Cannot enable Burst-mode for empty queue ~ for RingSize=0 */
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
            }
            else
            {
            ...
            */
            ...
            if (PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev))
            {
                /* burst size == num packets in queue - GT_OK */
                burstPacketsNumber = 1;
                st = utCpssDxChNetIfPacketAdd(dev, portGroupsBmp, txQueue, &packetId);
                UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
            ...
            */
            ...
            /*
                1.1.3. Call with burstEnable[GT_FALSE],
                    ...
                    other parameters same as 1.1.1.
                Expected: GT_OK.
				(the burstPacketsNumber is just ignored)
            */
            burstEnable = GT_FALSE;
            ...
            */
            burstEnable = GT_TRUE;
            burstPacketsNumber = PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev) ?
                                 (GDMA_TX_GENERATOR_CHAIN_SIZE + 1) : BIT_29;
        ...
        PRV_CPSS_GEN_PP_START_LOOP_NON_ACTIVE_PORT_GROUPS_MAC(dev,portGroupId)
        {
            ...
            burstEnable = GT_TRUE;
            burstPacketsNumber = (PRV_CPSS_DXCH_GDMA_SUPPORTED_MAC(dev)) ?
                                 GDMA_TX_GENERATOR_CHAIN_SIZE : BIT_29-1;
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        st = prvCpssDxChNetIfNumQueuesGet(dev, &toCpu_numOfQueues,&fromCpu_numOfQueues);
            ...
            packetParams.sdmaInfo.txQueue = (GT_U8)fromCpu_numOfQueues;
            ...
            */
            packetParams.dsaParam.dsaType = CPSS_DXCH_NET_DSA_CMD_FORWARD_E;
            packetParams.dsaParam.dsaInfo.forward.dstInterface.type = CPSS_INTERFACE_VID_E;
            /*origSrcPhyIsTrunk,origSrcPhy relevant for 'useVidx' (vidx/vlan interface) */
...
#if !defined(ASIC_SIMULATION) && !defined(CPU_BE)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChNetIfSdmaTxPacketSendLater)
#endif
...
)
{
    ...
    */
    ...
    dsaInfoPtr->dsaInfo.forward.srcIsTagged = CPSS_DXCH_BRG_VLAN_PORT_TAG0_CMD_E;
