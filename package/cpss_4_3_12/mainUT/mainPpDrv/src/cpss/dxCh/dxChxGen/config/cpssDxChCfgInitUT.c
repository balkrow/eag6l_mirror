...
*/
    ...
    PRV_TGF_SKIP_FAILED_TEST_MAC(UTF_FALCON_E | UTF_IRONMAN_L_E | UTF_AAS_E, "Jira-9593");
...
/* code copied from : aas_initPpLogicalInit */
static GT_STATUS    sip7_fill_default_ppLogicalInit_params(
    IN GT_U8 devNum,
    OUT CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC *sharedTablesSizeInfoPtr
)
{
    CPSS_DXCH_PP_CONFIG_SHARED_TABLES_SIZE_STC sharedTablesSizes;
    GT_U32  ii;

    cpssOsMemSet(&sharedTablesSizes,0, sizeof(sharedTablesSizes));
    ...
    *sharedTablesSizeInfoPtr = sharedTablesSizes;

    /* cpssDxChCfgPpLogicalInit aligns Fine Tuning DB with real size of tables
       need to restore default values of some fine tuning parameters to avoid
       false failures in tests */
    PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.tunnelStart = _128K;
    PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.routerArp   = _512K;
    PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.sip7_hf0    =
    PRV_CPSS_DXCH_PP_MAC(devNum)->fineTuning.tableSize.sip7_hf1    = _32K;
    return GT_OK;
}
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        {
            st = sip7_fill_default_ppLogicalInit_params(dev, &ppConfigPtr.sharedTablesSizeInfo);
            UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
        }
...
UTF_TEST_CASE_MAC(cpssDxChCfgPpLogicalInit_enhanced)
{
    ...
    /* prepare device iterator */
    PRV_TGF_APPLIC_DEV_RESET_MAC(&dev, (UTF_FALCON_E | UTF_AC5P_E));
        ...
        */
        ...
        while(GT_OK == prvUtfNextPhyPortGet(&utPhysicalPort, GT_TRUE))
        {
            ...
            */
            ...
            if(PRV_CPSS_SIP_7_CHECK_MAC(devNum) && (ePort >= _8K))
            {
                /* SIP7 support 16K ePorts but API support value up to 8K */
                ePort = _8K / 2;
            }
            ...
            if(0 != cpssOsMemCmp(&global,&globalGet,sizeof(globalGet)))
            {
                ...
                if(PRV_CPSS_SIP_6_CHECK_MAC(devNum))
                {
                    /* this API is 'legacy' support with limitation on the mask,
                        ...
                        than the pattern */
                    if(global.pattern == globalGet.pattern &&
                       globalGet.mask == (global.mask >> 1))/*lost single bit*/
                    {
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        /* vlan value is defined as 13 bits
           in sip7 maxVlanValue can be up to 16 bits
           so we want to prevent setting more then 13 bits in this API */
        vlan = (vlan & 0x1FFF);
        st = cpssDxChCfgIngressDropCntrModeSet(dev, mode, portNum, vlan);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
...
*/
UTF_TEST_CASE_MAC(cpssDxChCfgAuqFuqDisable)
{
/*
    ITERATE_DEVICE (DxChx)
    1.1. Call with correct dev.
...
*/
    ...
    /* 1. Go over all active devices. */
    /* Don't check valid parameters to avoid problems in following tests */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChCfgAuqFuqDisable(dev);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
*/
UTF_TEST_CASE_MAC(cpssDxChCfgServiceCpuDisable)
{
/*
    ITERATE_DEVICE (DxChx)
    1.1. Call with correct dev.
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChCfgServiceCpuDisable(dev);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChCfgInit)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCfgAuqFuqDisable)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChCfgServiceCpuDisable)
