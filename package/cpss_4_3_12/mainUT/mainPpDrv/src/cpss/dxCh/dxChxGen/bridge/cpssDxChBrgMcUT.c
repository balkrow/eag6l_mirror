...
static GT_U16 numOfVidxGet(IN GT_U8   dev)
{
    ...
    if(PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(dev))
    {
        ...
        */
        return (GT_U16)PRV_CPSS_ACCESS_TABLE_INFO_PTR(dev)[CPSS_DXCH_TABLE_MULTICAST_E].maxNumOfEntries;
    }
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        if(PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(dev))
        {
            vidx = PRV_CPSS_ACCESS_TABLE_INFO_PTR(dev)[CPSS_DXCH_TABLE_MULTICAST_E].maxNumOfEntries;
        }
