...
UTF_TEST_CASE_MAC(cpssDxChStreamEntrySet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamEntrySet
                                         (dev, index , &sngIrfInfo, &srfMappingInfo));
        ...
        for(index = 0 ; index < NUM_SNG_ENTRIES_CNS ; index += NUM_SNG_STEP_CNS)
        {
            ...
            sngIrfInfo.individualRecoveryEnable = (GT_BOOL)((index>>2) % 2);
            sngIrfInfo.takeAnySeqNum            = (GT_BOOL)((index>>3) % 2);
            sngIrfInfo.recoverySeqNum      = GT_NA;/* read only field */
        ...
        for(index = 0 ; index < NUM_SNG_ENTRIES_CNS ; index += NUM_SNG_STEP_CNS)
        {
            ...
            UTF_VERIFY_EQUAL0_PARAM_MAC(sngIrfInfo.individualRecoveryEnable ,sngIrfInfoGet.individualRecoveryEnable);
            UTF_VERIFY_EQUAL0_PARAM_MAC(sngIrfInfo.takeAnySeqNum          ,sngIrfInfoGet.takeAnySeqNum         );
            UTF_VERIFY_NOT_EQUAL0_PARAM_MAC(sngIrfInfo.recoverySeqNum     ,sngIrfInfoGet.recoverySeqNum        );
...
UTF_TEST_CASE_MAC(cpssDxChStreamEntryGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamEntryGet
                                         (dev, index, &sngIrfInfoGet, &irfCountersGet, &srfMappingInfoGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamExceptionCommandSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamExceptionCommandSet
                                         (dev, type, command));
...
UTF_TEST_CASE_MAC(cpssDxChStreamExceptionCommandGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamExceptionCommandGet
                                         (dev, type, &commandGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamExceptionCpuCodeSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamExceptionCpuCodeSet
                                         (dev, type, cpuCode));
...
UTF_TEST_CASE_MAC(cpssDxChStreamExceptionCpuCodeGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamExceptionCpuCodeGet
                                         (dev, type, &cpuCodeGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamCountingModeGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamCountingModeGet
                                         (dev, &irfModeGet ,&srfTaglessEnableGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfDaemonInfoSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfDaemonInfoSet
                                         (dev, &info));

        if(PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            restartMax = (BIT_24-1);
            leDetectionMax = (BIT_27-1);
        }
        else
        {
            restartMax = (BIT_23-1);
            leDetectionMax = (BIT_26-1);
        }

        time_restartArr[0] = restartMax / PRV_CPSS_PP_MAC(dev)->coreClock;/* max allowed */
        time_restartArr[1] = time_restartArr[0] / 2;
        time_restartArr[2] = time_restartArr[0] / 4;
        time_restartArr[3] = GT_NA;

        time_leDetectionArr[0] = leDetectionMax / PRV_CPSS_PP_MAC(dev)->coreClock ;/* max allowed */
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfDaemonInfoGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfDaemonInfoGet
                                         (dev, &infoGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamAgingGroupSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamAgingGroupSet
                                         (dev, startIndex, agingGroupBmp));
...
UTF_TEST_CASE_MAC(cpssDxChStreamAgingGroupGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamAgingGroupGet
                                         (dev, startIndex, &agingGroupBmpGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfEntrySet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfEntrySet
                                         (dev, index, &srfConfigInfo));
        ...
        for(index = 0 ; index < NUM_SRF_ENTRIES_CNS ; index += NUM_SRF_STEP_CNS)
        {
            ...
            srfConfigInfo.leDifference                = BIT_20 - 1 - index;
        ...
        for(index = 0 ; index < NUM_SRF_ENTRIES_CNS ; index += NUM_SRF_STEP_CNS)
        {
            ...
            UTF_VERIFY_EQUAL0_PARAM_MAC(srfConfigInfo.leDifference                 ,srfConfigInfoGet.leDifference                );
        }
        ...
        srfConfigInfo.leDifference  = BIT_20;
        st = cpssDxChStreamSrfEntrySet(dev, index , &srfConfigInfo);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OUT_OF_RANGE, st, dev);
        srfConfigInfo.leDifference = 1;
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfEntryGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfEntryGet
                                         (dev, index, &srfConfigInfoGet, &srfCountersGet, &srfStatusInfoGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfLatentErrorDetectedGroupGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfLatentErrorDetectedGroupGet
                                         (dev, startIndex , &errorGroupBmpGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfHistoryBufferEntrySet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfHistoryBufferEntrySet
                                         (dev, index, &histBuff));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfHistoryBufferEntryGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfHistoryBufferEntryGet
                                         (dev, index ,&histBuffGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfZeroBitVectorGroupEntrySet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfZeroBitVectorGroupEntrySet
                                         (dev, startIndex ,vector));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfZeroBitVectorGroupEntryGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfZeroBitVectorGroupEntryGet
                                         (dev, startIndex, &vectorGet));
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfTrgPortMappedMulticastIndexSet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfTrgPortMappedMulticastIndexSet
                                         (dev, portNum, index));
        ...
        index = PRV_CPSS_SIP_7_CHECK_MAC(dev)? BIT_10 : BIT_6;
        st = cpssDxChStreamSrfTrgPortMappedMulticastIndexSet(dev, portNum, index);
...
UTF_TEST_CASE_MAC(cpssDxChStreamSrfTrgPortMappedMulticastIndexGet)
{
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* Checks TSN features for not applicable device */
        UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(dev,
                                         cpssDxChStreamSrfTrgPortMappedMulticastIndexGet
                                         (dev, portNum, &indexGet));
...
)
{
    ...
    "I_E    = individual Recovery Enable : 'I' individual , '-' (disabled) \n"
    "Any    = take Any Seq Num : 'A' (any) , '-' not any \n"
    "recovery = recovery Seq Num"
    );
...
*/
    cpssOsPrintf(
"index |SeqNum |A_E |  R |I_E |Any |recovery\n"
"      | HEX   |    |    |    |    |        \n"
"===========================================\n"
    );
    ...
    for(index = 0 ; index < NUM_SNG_ENTRIES_CNS ; index++)
    {
        ...
        cpssOsPrintf(" %s ",
            info.individualRecoveryEnable == GT_TRUE ? "I" : "-");
        cpssOsPrintf(" %s ",
            info.takeAnySeqNum == GT_TRUE ? "A" : "-");
        cpssOsPrintf(" %d ",
            info.recoverySeqNum);
