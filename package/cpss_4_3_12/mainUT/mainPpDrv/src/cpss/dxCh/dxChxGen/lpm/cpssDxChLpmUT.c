...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* for sip7 the fields are muxed so need to compare only relevant fields */
        if(PRV_CPSS_SIP_7_CHECK_MAC(dev))
        {
            if (leaf.isIpv6Mc==GT_FALSE)
            {
        ...
        leaf.isIpv6Mc = GT_TRUE;
        st = cpssDxChLpmLeafEntryWrite(dev, leafIndex, &leaf);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, leafIndex);

        cpssOsMemSet(&leafGet, 0, sizeof(CPSS_DXCH_LPM_LEAF_ENTRY_STC));
        leafGet.isIpv6Mc = GT_TRUE;/* sip7 meed to know how to read the data since the fields are muxed  */
        st = cpssDxChLpmLeafEntryRead(dev, leafIndex, &leafGet);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, leafIndex);
...
*/
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        if(PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_AC3X_E) {
            ...
            if(PRV_CPSS_PP_MAC(dev)->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E)
            {
                /* size is configured in aas_initPpLogicalInit(...) */
                UTF_VERIFY_EQUAL1_PARAM_MAC(_5K, size, dev);
            }
            else
            {
