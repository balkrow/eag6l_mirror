...
UTF_TEST_CASE_MAC(prvCpssDxChPortPacketBufferSpeedCfg)
{
    ...
    PRV_TGF_NOT_APPLIC_DEV_RESET_MAC(
        ...
        UTF_BOBCAT2_E | UTF_BOBCAT3_E | UTF_CAELUM_E | UTF_ALDRIN_E | UTF_AC3X_E | UTF_ALDRIN2_E | UTF_AC5P_E | UTF_AC5X_E | UTF_HARRIER_E | UTF_IRONMAN_L_E | UTF_AAS_E);
...
*/
UTF_TEST_CASE_MAC(cpssDxChPortPacketBufferAgingTimeoutSet)
{
/*
    ITERATE_DEVICES (DxChx)
    1.1. Call function with timeout [1000].
    Expected: GT_OK.
    1.2. Call function with timeout [5000].
    Expected: GT_OK.
    1.3. Call function with timeout [6000].
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        */
        ...
        st = cpssDxChPortPacketBufferAgingTimeoutSet(dev, timeout);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, timeout);

        st = cpssDxChPortPacketBufferAgingTimeoutGet(dev, &timeoutGet);
        UTF_VERIFY_EQUAL1_STRING_MAC(GT_OK, st,
                       "cpssDxChPortPacketBufferAgingTimeoutGet: %d", dev);
        ...
        */
        ...
        st = cpssDxChPortPacketBufferAgingTimeoutSet(dev, timeout);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OUT_OF_RANGE, st, dev, timeout);

        /*
            1.4. Call function with timeout [500].
            Expected: GT_OK.
        */
        timeout = 500;
...
*/
UTF_TEST_CASE_MAC(cpssDxChPortPacketBufferAgingTimeoutGet)
{
/*
    ITERATE_DEVICES (DxChx)
...
*/
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        /*
            ...
            Expected: GT_OK.
        */
        st = cpssDxChPortPacketBufferAgingTimeoutGet(dev, &timeout);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        ...
        /*
            ...
            Expected: GT_BAD_PTR.
        */
        st = cpssDxChPortPacketBufferAgingTimeoutGet(dev, NULL);
...
UTF_SUIT_BEGIN_TESTS_MAC(prvCpssDxChPortPacketBuffer)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortPacketBufferAgingTimeoutSet)
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPortPacketBufferAgingTimeoutGet)
