...
static void cpssDxChHwInitDeviceMatrixCheckTest(IN GT_U8 dev)
{
    ...
    switch(devInfo.genDevInfo.devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_AC5X_E:
            ...
            SKIP_TEST_MAC; /* return inside macro under condition - break after it needed */
            break;
        ...
        case CPSS_PP_FAMILY_DXCH_IRONMAN_E:
            switch (PRV_CPSS_PP_MAC(dev)->devType)
            {
                case CPSS_IRONMAN_M_DEVICES_CASES_MAC:
                    SET_MATRIX_PTR_AND_SIZE_MAC(ptrMatrixData, matrixDataEntriesNum, prvUtfDeviceMatrixDataForIronmanM);
                    break;
                default:
                    SET_MATRIX_PTR_AND_SIZE_MAC(ptrMatrixData, matrixDataEntriesNum, prvUtfDeviceMatrixDataForIronmanL);
            }
            /* Test Ironman matrix devices */
            if (prvTgfResetModeGet() == GT_TRUE)
            {
                prvCpssDxChHwBobCat2DeviceMatrixTest(ptrMatrixData, matrixDataEntriesNum);
                return;
    ...
    /* disable PM task to avoid problems after cpssDxChCfgDevRemove */
    ...
    /* disable DI shadow to speedup test */
    ppPhase1Params.numOfDataIntegrityElements = 0;
    ...
    for( ii = 0 ; ii < matrixDataEntriesNum ; ii++ )
    {
        ...
        if (PRV_CPSS_SIP_6_CHECK_MAC(dev))
        {
            ...
            st = prvCpssDxChDeviceDidResetConfig(dev);
            if (st != GT_OK)
            {
                break;
            }
        ...
        /* Set default priority table access mode value - relevant for preemptable SIP6 devices */
        if(prvCpssSip6TxqPreemptionCapableDeviceGet(dev))
        {
            st = cpssDxChCscdQosTcDpRemapTableAccessModeSet(dev, CPSS_DXCH_CSCD_QOS_2B_SRC_2B_TRG_E);
            UTF_VERIFY_EQUAL2_STRING_MAC(GT_OK, st, "cpssDxChCscdQosTcDpRemapTableAccessModeSet: entry %d, device 0x%x",
...
UTF_TEST_CASE_MAC(cpssDxChHwInitDeviceMatrixCheckTest)
{
    ...
    /* skip AC5X RD boards */
    if(((boardIdx == 38) && (boardRevId == 2)) ||
       ((boardIdx == 38) && (boardRevId == 3)))
    {
        SKIP_TEST_MAC;
    }
    ...
    {
        /* previous suits create problem for this test.
           need reset system before start it. */
