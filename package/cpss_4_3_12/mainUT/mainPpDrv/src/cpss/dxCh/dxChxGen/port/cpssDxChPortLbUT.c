...
#include <common/tgfPortGen.h>          /* prvTgfPortModeSpeedSet() */
...
UTF_TEST_CASE_MAC(cpssDxChPortLoopbackPktTypeToAnalyzerSet)
{
    ...
    */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        for (analyzerIndex = 0; analyzerIndex <= CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(dev); analyzerIndex++)
        {
            /* go over all valid loopback ports */
            st = lbPortIterResetFn(&lbPort, dev);
            UTF_VERIFY_EQUAL0_PARAM_MAC(GT_OK, st);
        ...
        /* 1.2. call with invalid analyzer index */
        st = cpssDxChPortLoopbackPktTypeToAnalyzerSet(
            dev, CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(dev) + 1, validLbPort, GT_FALSE);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, validLbPort);
...
UTF_TEST_CASE_MAC(cpssDxChPortLoopbackPktTypeToAnalyzerGet)
{
    ...
    */
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        ...
        /* 1.2. call with invalid analyzer index */
        st = cpssDxChPortLoopbackPktTypeToAnalyzerGet(
            dev, CPSS_DXCH_MIRROR_ANALYZER_MAX_INDEX_MAC(dev) + 1, &lbPort, &enable);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_BAD_PARAM, st, dev);
    }
...
/* The cpssDxChPcaLoopbackCtrl test adds/deletets regular Network ports
 * therefore (Legacy=1 == PortManager=disable) is required.
 */
extern GT_STATUS cpssDxChSamplePortManager_debugLegacyCommandsMode_Set(IN GT_BOOL legacyMode);

/*
* DESCRIPTION:
*     Check Speed(bandwidth) and link control procedures for Egress Loopback.
*     "Egress Loopback" is new SIP7 HW feature; applied on a packet stream according
*     to PCL rules.
*     Only one-per-DP loopback could be configured, has its own dedicated CHANNEL=20.
* AGENDA:
*     RES=  expected result
*     LB - loopback
*     extPort - regular external network port
*     pcaPort - internal/special port dedicated for Egress Loopback (128..131)
*     Set_LB nnG  - config Gbps and UP a port               over LB API
*     Del_LB      - config port Down, free port's resources over LB API
*     Set_NET nnG - config nn Gbps and UP a port            over Regular/Net API
*     Del_NET     - config port Down, free port's resources over Regular/Net API
*
* APPLICABLE_DEVICES - SIP7 (only devNum=0 is tested)     RES=no-check
* Set test env: extPort 0,8 with 400G defined             RES=not-a-test
* Check init 4 pcaPort ports 25G are present and Link UP  RES=OK
* Get LB speed on Net port                                RES=GT_NO_SUCH
* Set_LB pcaPort 25G on existing 25G LB port 128          RES=OK
* Set_LB pcaPort 50G on existing 25G LB port 128          RES=BAD_STATE
* Del_LB pcaPort from 2 pcaPorts - 128 and 131            RES=OK
* Set_LB pcaPort 128 50G with existing CPU 25G            RES=NO_RESOURCE
* Set_LB pcaPort 131 50G (no CPU on DP[3])                RES=OK
*
* Set_LB on extPort DP[0] with wrong non-pcaPort       RES=BAD_PARAM
* Set_LB on extPort DP[0] with pcaPort 131 DP[3]       RES=BAD_PARAM
* Del_LB  on Net extPort 0 failed, port stays 400G     RES=BAD_STATE
* Del_NET extPort 0 and 20                             RES=not-a-test
* Set_LB 400G on free extPort 0 with free LP:DP[0]     RES=OK
* Set_LB 400G on free extPort 20 but busy DP[1]        RES=BAD_STATE
* Del_NET on LB extPort 0 failed                       RES=BAD_STATE
* Del_LB  extPort 0                                    RES=OK
* Set_LB 50G on free SLOW extPort 7                    RES=OK
* Loopback port is not locked by PortManager           RES=OK
*
* Restore environment
*/
static void utActiveDev_CpssDxChPcaLoopbackCtrl
(
    IN GT_U8    dev
)
{
    GT_U32      port;   /* Resource Port */
    GT_U32      pcaPortDp0 = 128;
    GT_U32      pcaPortDp3 = 131;
    GT_U32      extPortDp0 = 0;  /* max 800G port */
    GT_U32      extPortDp3 = 70; /* max 400G port */
    GT_U32      extPortSlow = 7; /* max 50G port */
    OUT CPSS_PORT_SPEED_ENT                     speed;
    CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_ENT  linkStatus;
    GT_BOOL     cond;
    GT_STATUS   st;

    PRV_TGF_NOT_SUPPORTED_DEV_CHECK_MAC(prvTgfDevNum, ~UTF_CPSS_PP_ALL_SIP7_CNS);

    /* Set test env: extPort 0,8 with 400G defined (ignore 'st') */
    st = cpssDxChSamplePortManager_debugLegacyCommandsMode_Set(GT_TRUE);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, extPortSlow, CPSS_PORT_SPEED_NA_E);
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, extPortDp0, CPSS_PORT_SPEED_NA_E);
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, extPortDp0, CPSS_PORT_SPEED_NA_E);
    st = prvTgfPortModeSpeedSet(dev, extPortDp0, CPSS_PORT_INTERFACE_MODE_KR8_E, CPSS_PORT_SPEED_400G_E);
    st = prvTgfPortModeSpeedSet(dev, extPortDp3, CPSS_PORT_INTERFACE_MODE_KR8_E, CPSS_PORT_SPEED_400G_E);

    /* Check init 4 pcaPort ports 25G are present and Link UP  RES=OK */
    for (port = pcaPortDp0; port <= pcaPortDp3; port++)
    {
        st = cpssDxChPortSpeedGet(dev, port, &speed);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
        cond = (speed == CPSS_PORT_SPEED_25000_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);
        /* UT(cpssDxChBrgEgrFltPortLinkEnableSet) leve link DONW -> make it Up */
        cpssDxChBrgEgrFltPortLinkEnableSet(dev, port, CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E);
        st = cpssDxChBrgEgrFltPortLinkEnableGet(dev, port, &linkStatus);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
        cond = (linkStatus == CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);
    }

    /* Get LB speed on Net port                                RES=GT_NO_SUCH */
    st = prvCpssDxChPortPcaLoopbackSpeedGet(dev, extPortDp0, &speed);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NO_SUCH, st, dev);

    /* Set_LB pcaPort 25G on existing 25G LB port 128          RES=OK */
    port = pcaPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_25000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

    /* Set_LB pcaPort 50G on existing 25G LB port 128          RES=GT_BAD_STATE */
    port = pcaPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_50000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, port);

    /* Del_LB pcaPort from 2 pcaPorts - 128 and 131            RES=OK */
    port = pcaPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_NA_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
    port = pcaPortDp3;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, port, CPSS_PORT_SPEED_NA_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

    /* Set_LB pcaPort 128 50G with existing CPU 25G            RES=NO_RESOURCE */
    port = pcaPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_50000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_NO_RESOURCE, st, dev, port);
    /* Set_LB pcaPort 131 50G (no CPU on DP[3])                RES=OK */
    port = pcaPortDp3;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, port, CPSS_PORT_SPEED_50000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

    /* ---------------- NET vs Loopback interactions -------------------------- */
    /* Set_LB on extPort DP[0] with wrong non-pcaPort       RES=BAD_PARAM */
    port = extPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, port, port, CPSS_PORT_SPEED_25000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_PARAM, st, dev, port);
    ...
    st = cpssDxChPortSpeedGet(dev, port, &speed);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
    cond = (speed == CPSS_PORT_SPEED_400G_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);

    /* Del_NET extPort 0 and 20                             RES=not-a-test */
    port = extPortDp0;
    st = prvTgfPortModeSpeedSet(dev, port, CPSS_PORT_INTERFACE_MODE_NA_E, CPSS_PORT_SPEED_NA_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

    /* Set_LB 400G on free extPort 0 with free LP:DP[0]     RES=OK */
    port = extPortDp0;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_400G_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

    /* Set_LB 400G on free extPort 20 but busy DP[1]        RES=BAD_STATE */
    port = extPortDp3;
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, port, CPSS_PORT_SPEED_400G_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_BAD_STATE, st, dev, port);
    ...
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, port, CPSS_PORT_SPEED_50000_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
    st = cpssDxChBrgEgrFltPortLinkEnableGet(dev, pcaPortDp0, &linkStatus);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
    cond = (linkStatus == CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E);
    UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);

    /* Loopback port is not locked by PortManager           RES=OK */
    st = cpssDxChSamplePortManager_debugLegacyCommandsMode_Set(GT_FALSE);
    UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, dev);
        port = pcaPortDp3;              /* it has been set 50G in previous step */
        st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, port, CPSS_PORT_SPEED_NA_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);

        st = cpssDxChPortSpeedGet(dev, port, &speed);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);/*y*/
        cond = (speed == CPSS_PORT_SPEED_NA_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);/*y*/
        st = cpssDxChBrgEgrFltPortLinkEnableGet(dev, port, &linkStatus);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, dev, port);
        cond = (linkStatus != CPSS_DXCH_BRG_EGR_FLT_PORT_LINK_STATUS_FORCE_LINK_UP_E);
        UTF_VERIFY_EQUAL2_PARAM_MAC(GT_TRUE, cond, dev, port);
    ...
    /* Restore/Set default environment (ignore 'st') */
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, extPortSlow, CPSS_PORT_SPEED_NA_E);
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp0, extPortDp0, CPSS_PORT_SPEED_NA_E);
    st = cpssDxChPortEgressLoopbackSpeedSet(dev, pcaPortDp3, extPortDp3, CPSS_PORT_SPEED_NA_E);
    st = prvTgfPortModeSpeedSet(dev, extPortDp0, CPSS_PORT_INTERFACE_MODE_KR8_E, CPSS_PORT_SPEED_400G_E);
    st = prvTgfPortModeSpeedSet(dev, extPortDp3, CPSS_PORT_INTERFACE_MODE_KR8_E, CPSS_PORT_SPEED_400G_E);
    for (port = pcaPortDp0; port <= pcaPortDp3; port++)
    {
        st = cpssDxChPortEgressLoopbackSpeedSet(dev, port, port, CPSS_PORT_SPEED_NA_E);
        st = cpssDxChPortEgressLoopbackSpeedSet(dev, port, port, CPSS_PORT_SPEED_25000_E);
    }

} /* END utActiveDev_CpssDxChPcaLoopbackCtrl */

UTF_TEST_CASE_MAC(cpssDxChPcaLoopbackCtrl)
{
    GT_U8       dev;
    GT_U32      notAppFamilyBmp;
    GT_STATUS   st;

    notAppFamilyBmp = CPSS_XCAT3_E | CPSS_AC5_E | CPSS_BOBCAT2_E | CPSS_CAELUM_E | CPSS_ALDRIN_E | \
    ...
    while (GT_OK == prvUtfNextDeviceGet(&dev, GT_TRUE))
    {
        utActiveDev_CpssDxChPcaLoopbackCtrl(dev);
    }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&dev, GT_FALSE))
    {
        st = cpssDxChPortEgressLoopbackSpeedSet(dev, 0, 0, CPSS_PORT_SPEED_NA_E);
        UTF_VERIFY_EQUAL1_PARAM_MAC(GT_NOT_APPLICABLE_DEVICE, st, dev);
    }
...
UTF_SUIT_BEGIN_TESTS_MAC(cpssDxChPortLoopback)
    ...
    UTF_SUIT_DECLARE_TEST_MAC(cpssDxChPcaLoopbackCtrl)
