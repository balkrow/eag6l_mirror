...
int tm_aging_drop_profiles_ut(uint8_t devNum, int num_of_iterations)
{
    ...
    array_size = (get_tm_port_count(TM_ENV(tm_ctl_PTR)) - TM_UT_INTERNAL_USE_DP_NUM )/ TM_AGING_PROFILES;
    if (array_size < dp_aging.dp_ut_aging_ranges)
    {
        array_size = dp_aging.dp_ut_aging_ranges;
    }
    dp_aging.dp_ut_aging_indexes_arr = (uint16_t*)tm_malloc((sizeof(uint16_t) * array_size));
