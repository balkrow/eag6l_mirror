...
static __THREAD_LOCAL GT_U32   internal_skippedDevicesBmp[(PRV_CPSS_MAX_PP_DEVICES_CNS + 31) / 32] = {0};
...
#define PRV_MY_PHY_PORT_NUM_MAC(_devNum)((PRV_CPSS_SIP_7_CHECK_MAC(_devNum))?304:((PRV_CPSS_SIP_6_10_CHECK_MAC(_devNum)) ? 75 : 288))
...
/**
* @internal prvUtfPrivatePerPortGroupPerRegisterCounter64BitGet function
* @endinternal
*
* @brief   This Function checks counters in the simulation.
*
...
* @param[in] setPerGroupCounterPerReg - Pointer to set per port group counter function.
*/
static GT_VOID prvUtfPrivatePerPortGroupPerRegisterCounter64BitGet
(
    IN GT_U32                                               inRegAddr,
    IN GT_U32                                               fieldOffset,
    IN GT_U32                                               fieldLength,
    IN PRV_UTF_CPSS_GET_COUNTER_64BIT_FUN                   getCounter,
    IN PRV_UTF_CPSS_SET_COUNTER_FUN                         setCounter,
    IN PRV_UTF_CPSS_GET_PER_GROUP_COUNTER_64BIT_FUN         getPerGroupCounter,
    IN PRV_UTF_CPSS_SET_PER_GROUP_COUNTER_FUN               setPerGroupCounter,
    IN PRV_UTF_CPSS_GET_COUNTER_64BIT_PER_REG_FUN           getCounterPerReg,
    IN PRV_UTF_CPSS_SET_COUNTER_PER_REG_FUN                 setCounterPerReg,
    IN PRV_UTF_CPSS_GET_PER_GROUP_COUNTER_64BIT_PER_REG_FUN getPerGroupCounterPerReg,
    IN PRV_UTF_CPSS_SET_PER_GROUP_COUNTER_PER_REG_FUN       setPerGroupCounterPerReg
)
{
    GT_STATUS               st = GT_OK;         /* return status */
    GT_U8                   devNum;             /* device number */
    GT_U32                  writeValueArr[3];   /* write counter value */
    GT_U64                  counterValue;       /* read counter value */
    GT_U32                  i;                  /* iterator */
    GT_PORT_GROUPS_BMP      portGroupsBmp = 0;  /* port group bitmap */
    GT_U32                  portGroupId;        /* port group id */
    GT_U32                  countMaxValue;      /* counter maximal value */
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        for(i = 0; i < 3; i++)
        {
            ...
            UTF_VERIFY_EQUAL1_PARAM_MAC(GT_OK, st, devNum);
            UTF_VERIFY_EQUAL1_STRING_MAC(writeValueArr[i], counterValue.l[0],
               "get another enable than was set: %d", devNum);
        }
    ...
    while(GT_OK == prvUtfNextDeviceGet(&devNum, GT_TRUE))
    {
        ...
        PRV_CPSS_GEN_PP_START_LOOP_PORT_GROUPS_MAC(devNum, portGroupId)
        {
            ...
            for(i = 0; i < 3; i++)
            {
                ...
                UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, portGroupsBmp);
                UTF_VERIFY_EQUAL2_STRING_MAC(writeValueArr[i], counterValue.l[0],
                "get another enable than was set: %d%d", devNum, portGroupsBmp);
                ...
                UTF_VERIFY_EQUAL2_PARAM_MAC(GT_OK, st, devNum, portGroupsBmp);
                UTF_VERIFY_EQUAL2_STRING_MAC(0, counterValue.l[0],
                "get another enable than was set: %d%d", devNum, portGroupsBmp);
                UTF_VERIFY_EQUAL2_STRING_MAC(0, counterValue.l[1],
                "get another enable than was set: %d%d", devNum, portGroupsBmp);
...
/**
* @internal prvUtfPerPortGroupCounter64BitGet function
* @endinternal
*
* @brief   This Function checks counters in the simulation.
*/
GT_VOID prvUtfPerPortGroupCounter64BitGet
(
    IN GT_U32                                           regAddr,
    IN GT_U32                                           fieldOffset,
    IN GT_U32                                           fieldLength,
...
{
    prvUtfPrivatePerPortGroupPerRegisterCounter64BitGet(regAddr, fieldOffset,
                                                        fieldLength, getCounter,
                                                        setCounter,getPerGroupCounter,
                                                        setPerGroupCounter,
                                                        NULL, NULL, NULL, NULL);
...
/**
* @internal prvTgfResetAndInitSystem_noSoftReset function
* @endinternal
*
* @brief   Function for enhanced UT reset, includes cpssInitSystem.
...
* @retval GT_NOT_SUPPORTED         - on system that not support it
*/
GT_STATUS prvTgfResetAndInitSystem_noSoftReset(void)
{
    GT_STATUS rc;
...
/**
* @internal prvUtfIsIronMan_M_System function
* @endinternal
*
* @brief   Is current system IronMan M based
*
* @retval GT_TRUE                  - exists
* @retval GT_FALSE                 - not exists
*/
GT_BOOL prvUtfIsIronMan_M_System
(
    GT_VOID
)
{
...
/* Debug Skip Ignore */
/* GT_TRUE cause ignoring the SKIP statement in the tests */
/* GT_FALSE allows regular execution of the tests         */
static GT_BOOL prvUtfDebugSkipIgnore = GT_FALSE;

/**
* @internal prvUtfDebugSkipIgnoreSet function
* @endinternal
*
* @brief   Set debug skip ignore flag
*/
GT_VOID prvUtfDebugSkipIgnoreSet
(
    GT_BOOL debugSkipIgnore
)
{
    prvUtfDebugSkipIgnore = debugSkipIgnore;
}

/**
* @internal prvUtfDebugSkipIgnoreGet function
* @endinternal
*
* @brief   Get debug skip ignore flag
*/
GT_BOOL prvUtfDebugSkipIgnoreGet
(
    GT_VOID
)
{
   return prvUtfDebugSkipIgnore;
}

/**
* @internal prvUtfPmTaskEnableSet function
* @endinternal
*
* @brief   Enable or Disable PM task
*
* @param[in] enable  - GT_FALSE - disable PM task
*                      GT_TRUE - enable PM task
*/
GT_VOID prvUtfPmTaskEnableSet
(
    GT_BOOL enable
)
{
    ...
    {
        prvWrAppPortManagerTaskEnableSet(enable);
    }
#else
