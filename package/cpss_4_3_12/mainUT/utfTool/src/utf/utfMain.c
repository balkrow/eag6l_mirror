...
static GT_U32  allow_rerun = 0;
/* the cs_ver is going to call this function with '1' */
GT_STATUS   tgfAllowRerun(IN GT_U32 allowRerun)
{
    allow_rerun = allowRerun;
    return GT_OK;
}
GT_U32   tgfIsAllowRerun(void)
{
    return allow_rerun;
}
/*
...
#define UTF_MAX_TEST_PATH_LEN_CNS 512
...
static GT_CHAR *memLeakageSkippedTestArray[] =
                                        {"tgfBasicIpv42SharedVirtualRouter"
                                         ...
                                         ,"tgfTrunkSaLearning"
                                         ,"tgfTrunkCrcHash_4K_members_known_UC_EPCL_resilient_mode"
                                         ,"tgfTrunk_80_members_flood_EPCL_resilient_mode"
                                         ,"tgfIpfixManagerFirstPacketsCheck"
                                         ,"tgfIpfixManagerDataPacketsCheck"
                                         ,"tgfIpfixManagerEntriesAddDelete"
                                         ,"tgfIpfixManagerAging"
                                         ,"tgfIpfixFwNewFlowsLearn"
                                         ,"tgfIpfixFwEmAutoLearnAndFirstPackets"
                                         ,"tgfIpfixFwDataPacketsCheck"
                                         ,"tgfIpfixFwFlowsLearnFullCapacity"
                                         ,"tgfIpfixFwEntriesAddDelete"
                                         ,"tgfIpfixFwAging"
                                         ,"tgfIpfixFwElephantFlows"
                                         ,"prvTgfNetIfTxSdmaEvents"
                                         ,"prvTgfIfaIngressNode"
...
static GT_CHAR *ipLpmTestArray[NUM_IP_LPM_TESTS] =
                               {"tgfNstL3PortIsolation",
                                ...
                                "prvTgfFdbBasicIpv4UcRoutingByIndex_TtiRedirectToVrfId",
...
/* High Availablity suit name - in case of error during those tests we need reset cpss to avoid fail following tests  */
#define HIGH_AVAILABILITY_SUIT "tgfHighAvailability"
...
)
{
    ...
    /* look for this exact contact in one of the already existing nodes in utfPreSkippedRulesDbPtr
       so there will not be duplications
       and we will not need to remove the list during cpssInitSystem
    */
    {
        UTF_PRESKIPPED_RULE_STC* p;
#define CONTINUE_IF_NO_MATCH_STRINGS(a,b) \
            if ( ((a) && !(b)) || (!(a) && (b))) \
                continue;                         \
            if ((a) && cpssOsStrCmp((a),(b)))     \
               continue

        for(p = utfPreSkippedRulesDbPtr; (p) ; p = p->next)
        {
            CONTINUE_IF_NO_MATCH_STRINGS(rulePtr->suitLow  , p->suitLow );
            CONTINUE_IF_NO_MATCH_STRINGS(rulePtr->suitHigh , p->suitHigh);
            CONTINUE_IF_NO_MATCH_STRINGS(rulePtr->testLow  , p->testLow );
            CONTINUE_IF_NO_MATCH_STRINGS(rulePtr->testHigh , p->testHigh);

            /* already exists node , so not adding it */
            cpssOsFree(rulePtr);
            return GT_OK;
        }
    }
...
)
{
    ...
    if (NULL == failureMsgPtr)
    {
        switch (argc)
        {
            ...
            case 8:
#ifndef INTFMT
                PRV_UTF_LOG8_MAC("%d, %d, %d, %d, %d, %d, %d, %d\n",
                                 argvPtr[0], argvPtr[1], argvPtr[2], argvPtr[3], argvPtr[4], argvPtr[5], argvPtr[6], argvPtr[7]);
#else
                PRV_UTF_LOG8_MAC(INTFMT INTFMT INTFMT INTFMT INTFMT INTFMT INTFMT INTFMT_LAST,
                                 argvPtr[0], argvPtr[1], argvPtr[2], argvPtr[3], argvPtr[4], argvPtr[5], argvPtr[6], argvPtr[7]);
#endif
                break;
            default:
...
/* this function needed to be used by the expect script .
   only because calling directly to the 'utfTestsRun' is covered by 'galtisWrapper' (wrUtfTestsRun)
   that is subject to 'timeout' that cause the 'utfTestsRun' to not return '->' after the timeout
   so we get @@@!!!0!!!### (instead of @@@!!!0!!!###->) so the expect script fail
*/
GT_STATUS utfTestsRunNew
(
    IN const GT_CHAR *testPathPtr,
    IN GT_U32        numOfRuns,
    IN GT_BOOL       continueFlag
...
{
    return utfTestsRun(testPathPtr,numOfRuns,continueFlag);
}
...
)
{
    ...
    /*  1. do not remove the existing list of 'skip tests' as it cause to forget
        tests that we wanted to skip in the first place !
        (that called from terminal before the cpssInitSystem )

        2. to not inflate this DB by utfAddPreSkippedRule that called every time
           from the appDemoFalconMainUtForbidenTests and similar functions , the
           utfAddPreSkippedRule will not add new node for already existing forbidden test/suite
...
static void utfIsTestNeedReRun(void)
{
    GT_U32  ii;
    static GT_CHAR* rerunSuitesArr[] = {
          "tgfTrunk" /*those send many bursts of 32 packets , sometimes one packet get lost*/
         ,"tgfEvent" /*those sometimes loose interrupts */
         ,"tgfNetIf" /*thoee sometimes loose single packet */
         /* MUST BE LAST */
         ,NULL
        };
    static GT_CHAR* rerunTestsArr[] = {
         "prvTgfGenEventPhysicalPortExtExDataMapping"
        ,"prvTgfBrgFdbManagerBasicLimitDynamicUcLearn"
        ,"tgfOamKeepAliveAgeState"/* get extra keep alive exception counter */
        ,"cpssDxChBrgFdbManagerAgingScan_with_appDemo"
         /* MUST BE LAST */
         ,NULL
        };
    if (prvTgfReRunVariable != TGF_RE_RUN_STATUS_CAPABLE_E)
    ...
    for (ii = 0; rerunSuitesArr[ii] ; ii++)
    {
        if (0 == cpssOsStrCmp(utfCtxCommon.testEntriesArray[utfCtxPtr->currTestNum].suitNamePtr,
                            rerunSuitesArr[ii]))
        {
            /* turn on the flag to state the test may need re-run in case it fail */
            prvTgfReRunVariable = TGF_RE_RUN_STATUS_DO_RE_RUN_E;
            return ;
        }
    }

    for (ii = 0; rerunTestsArr[ii] ; ii++)
    {
        if (0 == cpssOsStrCmp(utfCtxCommon.testEntriesArray[utfCtxPtr->currTestNum].testNamePtr,
                            rerunTestsArr[ii]))
        {
            /* turn on the flag to state the test may need re-run in case it fail */
            prvTgfReRunVariable = TGF_RE_RUN_STATUS_DO_RE_RUN_E;
...
#ifdef ASIC_SIMULATION
static GT_STATUS prvTgfXcat3xPerformaceBoost(GT_VOID)
{
    CPSS_API_LOCK_MAC(prvTgfDevNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    /* boost performance of the ENH-UT */
    debug_set_directAccessMode (1);
    debug_set_directAccessMode1(1);
    CPSS_API_UNLOCK_MAC(prvTgfDevNum,PRV_CPSS_FUNCTIONALITY_CONFIGURATION_CNS);
    return GT_OK;
}
#endif
...
/* tolerance for memory leakage in bytes.
   All leakages > this value are errors. */
static GT_U32 utfMemoryLeakageTolerance = 0;

/**
* @internal utfMemoryHeapLeakageToleranceSet function
* @endinternal
*
* @brief   This function sets tolerance for memory leakage checks.
*
* @param[in] toleranceInBytes      -   value of acceptable memory leakage.
*
* @retval old value of acceptable memory leakage.
*
*/
GT_U32 utfMemoryHeapLeakageToleranceSet(IN GT_U32 toleranceInBytes)
{
    GT_U32 oldValue = utfMemoryLeakageTolerance;
    utfMemoryLeakageTolerance = toleranceInBytes;
    return oldValue;
}
...
)
{
    ...
    if (GT_OK != st)
    {
        if (forceToIgnoreMemoryLeakage == GT_TRUE)
        {
            ...
            cpssOsPrintf("UTF : [%s] : Ignore memory leak !\n",
                utfCtxCommon.testEntriesArray[utfCtxPtr->currTestNum].testNamePtr);
        }
        else if(utfMemoryLeakageTolerance && (mem <= utfMemoryLeakageTolerance))
        {
            cpssOsPrintf("UTF : [%s] : Ignore memory leak of [%d] bytes\n",
                utfCtxCommon.testEntriesArray[utfCtxPtr->currTestNum].testNamePtr, mem);
        }
        else
        {
            errorCountIncrement(utfCtxPtr->currTestNum,GT_TRUE);
    ...
    /* restore tolerance to default value */
    utfMemoryLeakageTolerance = 0;
...
)
{
    ...
    /* run unit test */
    if ((prvUtfDebugSkipIgnoreGet() == GT_FALSE)
        && (utfIsTestPreSkipped(
    ...
    else
    {
        ...
        CPSS_ENABLER_OS_RMON_CREATE_ENTRY_CHECKPOINT(suitTestName);
        utfCtxCommon.testEntriesArray[testNum].testFuncPtr();
        CPSS_ENABLER_OS_RMON_CREATE_EXIT_CHECKPOINT(suitTestName);
        if (g_checkIntrrTableThreshold > 0)
        {
            utfIntprTableCheck();
        }
    ...
    if ((utfCtxPtr->testEntriesArray[testNum].errorCount > 0)
        && (prvUtfSkipList != NULL)
        && (prvUtfDebugSkipIgnoreGet() == GT_FALSE))
    {
    ...
    if (GT_FALSE == prvUtfSkipTestsFlagGet())
    {
        if (utfCtxPtr->testEntriesArray[testNum].errorCount > 0)
        {
            /* check if test need re-run --> it will set flag prvTgfReRunVariable
               with value TGF_RE_RUN_STATUS_DO_RE_RUN_E */
            utfIsTestNeedReRun();
...
)
{
    GT_U32 ii;
    GT_U32 res = 1;
    for (ii = 0; memLeakageSkippedTestArray[ii] ; ii++)
    {
    ...
    for (countIdx = 0; countIdx < utfCtxCommon.usedTestNum; countIdx++)
    {
            ...
            (GT_TRUE == isSuitFound) || isSuiteRange)
        {
            ...
            if (CPSS_OS_SEM_WAIT_FOREVER_CNS == utfTestTimeOut)
            {
                ...
                prvTgfReRunVariable = TGF_RE_RUN_STATUS_DISABLED_E; /* re-run disabled by default */
                if(allow_rerun)
                {
                ...
                {
                    prvTgfReRunVariable = TGF_RE_RUN_STATUS_CAPABLE_E; /* re-run capable (linux WM that 'run all')*/
                }
                #endif
                ...
                if (utfCtxPtr->testEntriesArray[utfCtxCommon.testIndexArray[countIdx]].errorCount > 0)
                {
                    ...
                    /*reset cpss if HA tests failed when running all Enhanced UT */
                    if (testPathPtr == NULL && (0 == cpssOsStrCmp(currSuitNamePtr,HIGH_AVAILABILITY_SUIT)))
                    {
                        st = prvTgfResetAndInitSystem();
                        if (st != GT_OK)
                        {
                            PRV_UTF_LOG0_MAC("[UTF]: prvTgfResetAndInitSystem: failed \n");
                        }
