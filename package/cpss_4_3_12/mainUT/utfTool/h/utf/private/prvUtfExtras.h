...
#define ALIGN_EVID_TO_4K_TILL_MAX_DEV_MAC(evid) \
    (((UTF_CPSS_PP_MAX_VLAN_INDEX_CNS(prvTgfDevNum))<BIT_16)?\
    (ALIGN_RESOURCE_TO_DEV_MAC(evid , _4K , ((GT_U16)UTF_CPSS_PP_MAX_VLAN_INDEX_CNS(prvTgfDevNum)))):\
    (ALIGN_RESOURCE_TO_DEV_MAC(evid , _4K , 0XFFFF)))
...
/* max physical port */
    ...
    /* Max trunk ECMP index into trunk ECMP table */
    #define UTF_CPSS_PP_MAX_TRUNK_ECMP_INDEX_CNS(_devNum)                   \
             (((_devNum) < PRV_CPSS_MAX_PP_DEVICES_CNS) ?                   \
                (PRV_CPSS_SIP_7_CHECK_MAC(_devNum) ?                         \
                 PRV_CPSS_DXCH_PP_MAC(_devNum)->hwInfo.parametericTables.numEntriesTrunkEcmp : \
                 PRV_CPSS_DXCH_PP_MAC(_devNum)->bridge.l2EcmpNumMembers) :      \
             _1K)
    ...
    /* Max trunk ECMP index into trunk ECMP table */
    #define UTF_CPSS_PP_MAX_TRUNK_ECMP_INDEX_CNS(_devNum)       _1K
...
/* all sip6.30 devices */
#define UTF_CPSS_PP_ALL_SIP6_30_CNS         (UTF_IRONMAN_L_E | UTF_CPSS_PP_ALL_SIP7_CNS)
/* all sip7 devices */
#define UTF_CPSS_PP_ALL_SIP7_CNS            (UTF_AAS_E)
...
/* Macro checks TSN support in UT tests */
#define UTF_CPSS_DXCH_TSN_SUPPORTED_MAC(_devNum, _func)                                    \
    {                                                                                       \
        if (PRV_CPSS_DXCH_PP_MAC(_devNum)->fineTuning.featureInfo.tsnSupported == GT_FALSE) \
        {                                                                                   \
                st = _func;                                                                 \
                /* expects GT_NOT_APPLICABLE_DEVICE for devices not supported TSN */        \
                UTF_VERIFY_EQUAL0_STRING_MAC(GT_NOT_APPLICABLE_DEVICE, st,                  \
                              "Fail on wrong return value: %d");                            \
                continue;                                                                   \
        }                                                                                   \
    }

/* Macro checks TSN support in TGF tests */
#define PRV_TGF_TSN_SUPPORTED_MAC(_devNum)    \
    if (PRV_CPSS_DXCH_PP_MAC(_devNum)->fineTuning.featureInfo.tsnSupported == GT_FALSE) \
        SKIP_TEST_MAC
...
#define PRV_TGF_SKIP_LONG_TEST_SHLIB_WM_MAC(_skipFamilyBmp)      \
    if (GT_TRUE == prvUtfSkipLongTestsFlagGet((GT_U32)_skipFamilyBmp)) \
        SKIP_TEST_MAC
#else
#define PRV_TGF_SKIP_LONG_TEST_SHLIB_WM_MAC(_skipFamilyBmp)
...
GT_BOOL prvUtfDebugSkipIgnoreGet(GT_VOID);

/* set skip flag and update skip tests num */
#define SKIP_TEST_MAC   \
        {                                                       \
            if (GT_FALSE == prvUtfDebugSkipIgnoreGet())         \
            {                                                   \
                prvUtfSkipTestsSet();                           \
                return ;                                        \
            }                                                   \
...
typedef GT_STATUS (*PRV_UTF_CPSS_GET_COUNTER_64BIT_FUN)
(
    IN  GT_U8                   devNum,
    OUT GT_U64                  *counterValue
);
...
typedef GT_STATUS (*PRV_UTF_CPSS_GET_PER_GROUP_COUNTER_64BIT_FUN)
(
    IN  GT_U8                   devNum,
    IN  GT_PORT_GROUPS_BMP      portGroupsBmp,
    OUT GT_U64                  *counterValue
...
typedef GT_STATUS (*PRV_UTF_CPSS_GET_COUNTER_64BIT_PER_REG_FUN)
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  regAddr,
    OUT GT_U64                  *counterValue
...
typedef GT_STATUS (*PRV_UTF_CPSS_GET_PER_GROUP_COUNTER_64BIT_PER_REG_FUN)
(
    IN  GT_U8                   devNum,
    IN  GT_PORT_GROUPS_BMP      portGroupsBmp,
    IN  GT_U32                  regAddr,
