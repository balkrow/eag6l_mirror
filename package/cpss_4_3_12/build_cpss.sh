...
## GM_HAWK           :  specify using of   GM libs for GM_HAWK    simulation image build
## GM_IRONMAN        :  specify using of   GM libs for GM_IRONMAN simulation image build
## GM_SEAHAWK        :  specify using of   GM libs for GM_SEAHAWK simulation image build
##
...
if [ "$1" = "help" ]; then
   ...
   echo ""
   echo "      GM options:"
   echo "         GM_FALCON GM_HAWK GM_IRONMAN GM_SEAHAWK"
   echo ""
   ...
   echo ""
   echo "      SSE options:"
   echo "         SSE_OFF    - Sub Sequence Engine cannot be in use (default) "
   echo "         SSE_ON     - Sub Sequence Engine can be in use "
   echo ""
    ...
    case $arg in
       ...
       i386_64_BEYE)
        ...
        ARMv7_LK_3_10 | ARMv7_LK_4_4)
            TARGET="armv7"
            ;;
        ARMv7be_LK_3_10 | ARMv7be_LK_4_4 | MSYS_BE)
            TARGET="armv7be"
            ;;
        AARCH64v8)
            TARGET="armv8"
        ...
        STACK_EXT)
            stack_ext="CONFIG_INCLUDE_STACK_EXTENSION=y"
            ;;

        MACSEC_APP)
            macsec_app="CONFIG_INCLUDE_MACSEC_APP=y"
            ;;
...
echo "make $CPSS_DIR TARGET=$TARGET FAMILY=$FAMILY ASK=${ask_build} ASK_LIB_BUILD=${ask_lib} MIXED_MODE=${mixed_mode_build} $dbg $utf $gal $lua $ezb $mpd $tm $mtx $lck $log $shr $gm $sse $asim $target $silent $ext -j ${NUM_CORES:-1} $CALLER_PATH $BUILD_FOLDER $CONFIG_MAKE_ARG"
make $CPSS_DIR TARGET=$TARGET FAMILY=$FAMILY ASK=${ask_build} ASK_LIB_BUILD=${ask_lib} MIXED_MODE=${mixed_mode_build} $dbg $utf $gal $lua $ezb $mpd $tm $mtx $log $shr $gm $sse $asim $target $silent $ext $stack_ext -j ${NUM_CORES:-1} $CALLER_PATH $BUILD_FOLDER $CONFIG_MAKE_ARG
