...
#if defined CPSS_APP_PLATFORM && !defined MIXED_MODE
#define portMgr PRV_APP_REF_PORT_MGR_MODE
#else
...
)
{
    ...
    apParams.fcAsmDir       = (CPSS_DXCH_PORT_AP_FLOW_CONTROL_ENT)inFields[4];
...
config:
    ...
    else if (enable==GT_TRUE)
    {
        ...
        for (ii=0;ii<CPSS_DXCH_PORT_AP_IF_ARRAY_SIZE_CNS;ii++)
        {
            ...
            portParams.portParamsType.apPort.modesArr[ii].fecSupported = (CPSS_PORT_FEC_MODE_ENT)apParams.fecAbilityArr[ii];
            portParams.portParamsType.apPort.modesArr[ii].fecRequested = (CPSS_PORT_FEC_MODE_ENT)apParams.fecRequestedArr[ii];
        }
        ...
        if (GT_FALSE == useCpssDefault)
        {
          ...
          portParams.portParamsType.apPort.apAttrs.fcAsmDir = (CPSS_PORT_AP_FLOW_CONTROL_ENT)apParams.fcAsmDir;
