...
)
{
    ...
    if (pgEnable == GT_FALSE)
    {
        result = cpssDxChIpfixEntryGet(devNum, stage, currentIndex, (CPSS_DXCH_IPFIX_RESET_MODE_ENT)reset, &ipfixEntry);
    }
    else
    {
        result = cpssDxChIpfixPortGroupEntryGet(devNum, pgBmp, stage, currentIndex, (CPSS_DXCH_IPFIX_RESET_MODE_ENT)reset, &ipfixEntry);
    }
