...
)
{
    ...
    params.eligiblePrioFuncId = (CPSS_TM_ELIG_FUNC_QUEUE_ENT)inArgs[5];
...
)
{
    ...
    for(ii=0; ii<8; ii++)
        quantumArrPtr[ii] = (GT_U32)inArgs[2+ii];
    for(ii=0; ii<8; ii++)
        schdModeArr[ii] = (CPSS_TM_SCHD_MODE_ENT)inArgs[10+ii];

    eligPrioFuncId = (GT_U32)inArgs[18];
