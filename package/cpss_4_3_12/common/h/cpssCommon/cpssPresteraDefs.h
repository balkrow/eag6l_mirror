...
/* Max number of PPs in system
  PRV_CPSS_MAX_PP_DEVICES_CNS may be defined in  .config */
#ifndef PRV_CPSS_MAX_PP_DEVICES_CNS
#define PRV_CPSS_MAX_PP_DEVICES_CNS             128
#endif /* PRV_CPSS_MAX_PP_DEVICES_CNS */

/* Max number of physical ports in PP device */
/* see also CPSS_MAX_PORTS_NUM_CNS  */
#define PRV_CPSS_MAX_PP_PORTS_NUM_CNS           CPSS_MAX_PORTS_NUM_CNS
...
#define PRV_CPSS_DXCH_AAS_FUNCTIONS_SUPPORT_CNS     BIT_25
...
/* Set 10 bytes of Reduce Action to a 'reduce action' field (84 bits) in a table entry.

   startEntryPtr - pointer to start entry data
   fieldsInfoArr[] - array of fields info.
   fieldId - the 'Field ID'  - index in fieldsInfoArr[].
   _reduceActionBytes[] - the array of 10 bytes (reduce action)
   _expandedActionIndex - the expanded action index to set into reduced action
NOTEs:
   1. the field MUST NOT be with value with more bits then length
   Return values:
...
*/
#define REDUCED_ACTION_SIP7_SET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_reduceActionBytes,_expandedActionIndex)   \
{                                                                                                    \
    GT_U32    _wordsArr[3];                                                                          \
                                                                                                     \
    /* convert the 9 bytes of reduce action to 3 words */                                            \
    ...
    _wordsArr[1] = _reduceActionBytes[4]<<0     |                                                    \
                   ...
                   _reduceActionBytes[6]<<16    |                                                    \
                   _reduceActionBytes[7]<<24;                                                        \
    _wordsArr[2] = _reduceActionBytes[8]<<0     |                                                    \
                   _reduceActionBytes[9]<<8     |                                                    \
                   (_expandedActionIndex&0xF)<<16;                                                   \
                                                                                                     \
    U_ANY_SET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_wordsArr);                        \
}

/* Get 10 bytes of Reduce Action to a 'reduce action' field (68 bits) in a table entry.
...
*/
#define REDUCED_ACTION_SIP7_GET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_reduceActionBytes,_expandedActionIndex)   \
{                                                                                                   \
    GT_U32    _wordsArr[3];                                                                         \
                                                                                                    \
    U_ANY_GET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_wordsArr);                       \
    ...
    (_reduceActionBytes)[6] = (GT_U8)(_wordsArr[1] >>  16);                                         \
    (_reduceActionBytes)[7] = (GT_U8)(_wordsArr[1] >>  24);                                         \
    (_reduceActionBytes)[8] = (GT_U8)(_wordsArr[2] >>  0);                                          \
    (_reduceActionBytes)[9] = (GT_U8)(_wordsArr[2] >>  8);                                          \
    (_expandedActionIndex)  = (GT_U8)((_wordsArr[2] >> 16)&0xF);                                    \
}
...
/* Set 16 bytes of Key to a 'key only' field (130 bits) in a table entry.

   startEntryPtr - pointer to start entry data
   fieldsInfoArr[] - array of fields info.
   fieldId - the 'Field ID'  - index in fieldsInfoArr[].
   _keyPatternBytes[] - the array of 16 bytes (key 130 bits)
NOTEs:
   1. the field MUST NOT be with value with more bits then length
   Return values:
        GT_OK           - no error
        ...
        GT_BAD_PARAM    - on bad parameter : info.length == 0 or  valueArr[...] > maxValue
*/
#define KEY_ONLY_SIP7_SET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_keyPatternBytes)          \
{                                                                                                   \
    GT_U32    _wordsArr[4];                                                                         \
                                                                                                    \
    /* convert the 16 bytes of key to 4 words */                                                    \
    _wordsArr[0] = _keyPatternBytes[0]<<0   |                                                       \
                   _keyPatternBytes[1]<<8   |                                                       \
                   _keyPatternBytes[2]<<16  |                                                       \
                   _keyPatternBytes[3]<<24;                                                         \
    ...
    _wordsArr[2] = _keyPatternBytes[8]<<0   |                                                       \
                   ...
                   _keyPatternBytes[10]<<16 |                                                       \
                   _keyPatternBytes[11]<<24;                                                        \
    _wordsArr[3] = _keyPatternBytes[12]<<0   |                                                      \
                   _keyPatternBytes[13]<<8   |                                                      \
                   _keyPatternBytes[14]<<16  |                                                      \
                   _keyPatternBytes[15]<<24;                                                        \
                                                                                                    \
    U_ANY_SET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_wordsArr);                       \
}
...
*/
#define KEY_ONLY_SIP7_GET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_keyPatternBytes)             \
{                                                                                                \
    GT_U32    _wordsArr[4];                                                                      \
                                                                                                 \
    U_ANY_GET_FIELD_BY_ID_MAC(startEntryPtr,fieldsInfoArr,fieldId,_wordsArr);                    \
                                                                                                 \
    /* convert the 4 words to 16 bytes of reduce action plus expandedActionIndex */              \
    (_keyPatternBytes)[0]  = (GT_U8)(_wordsArr[0] >>  0);                                        \
    (_keyPatternBytes)[1]  = (GT_U8)(_wordsArr[0] >>  8);                                        \
    (_keyPatternBytes)[2]  = (GT_U8)(_wordsArr[0] >>  16);                                       \
    (_keyPatternBytes)[3]  = (GT_U8)(_wordsArr[0] >>  24);                                       \
    ...
    (_keyPatternBytes)[12] = (GT_U8)(_wordsArr[3] >>  0);                                        \
    (_keyPatternBytes)[13] = (GT_U8)(_wordsArr[3] >>  8);                                        \
    (_keyPatternBytes)[14] = (GT_U8)(_wordsArr[3] >>  16);                                       \
    (_keyPatternBytes)[15] = (GT_U8)(_wordsArr[3] >>  24);                                       \
}
...
#define PRV_CPSS_MAX_DEV_FUNCTIONALITY_RESOLUTION_CNS 6
...
/*macro for API used for TRAFFIC MANAGER functionality (configurations - no traffic) */
#define PRV_CPSS_FUNCTIONALITY_PER_DEVICE_TRAFFIC_MANAGER_CNS (4)
/*macro for API used for FlexE functionality (configurations - no traffic) */
#define PRV_CPSS_FUNCTIONALITY_FLEXE_CNS (5)
...
#define CPSS_ZERO_LEVEL_API_LOCK_MAC\
    ...
    {\
        ...
        {\
            if(NULL!=cpssOsPrintf)\
            { \
               cpssOsPrintf("[ File %s : line %d ] Mutex global shared DB is not initialized\n",__FILE__,__LINE__);\
