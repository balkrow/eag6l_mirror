...
/**
********************************************************************************
* @file prvCpssSip7DevMemManager.h
*
...
********************************************************************************
*/
#ifndef __prvCpssSip7DevMemManager
#define __prvCpssSip7DevMemManager

#include <cpss/extServices/os/gtOs/gtGenTypes.h>

#define DMM_BLOCK_FREE_SIP7                      0
#define DMM_BLOCK_ALLOCATED_SIP7                 1
#define DMM_MIN_ALLOCATE_SIZE_IN_BYTE_SIP7_CNS   32 /* only 132 bits are used but address alignment should be 32 bytes */

/* Illegal values constant to distinguish between an error
   due to CPU memory allocation or DMM block allocation */
#define DMM_MALLOC_FAIL_SIP7                     0
#define DMM_BLOCK_NOT_FOUND_SIP7                PRV_SHARED_GLOBAL_VAR_GET(commonMod.genericHwInitDir.dmmBlockNotFoundIndication)

#define SIZE_IN_WORDS_SIP7(handler)      (((GT_DMM_BLOCK_SIP7*)handler)->word1 & 0x7FFFFFF)
#define SET_SIZE_IN_WORDS_SIP7(handler, size)    \
            if((size) > 0x7FFFFFF)            \
            {                               \
            ...
            {                               \
                handler->word1 = (handler->word1 & 0xF8000000) | (size);     \
            }

#define DMM_BLOCK_STATUS_SIP7(handler)   ((((GT_DMM_BLOCK_SIP7*)handler)->word1>>27) & 0x1)
/* following two macro's instead of old one implemented to avoid warning
  "statement not reached" in DIAB WB 2.6
*/
#define SET_DMM_BLOCK_STATUS_FREE_SIP7(handler)  \
    ((GT_DMM_BLOCK_SIP7*)handler)->word1 &= ~(1<<27)

#define SET_DMM_BLOCK_STATUS_ALLOCATED_SIP7(handler) \
    ((GT_DMM_BLOCK_SIP7*)handler)->word1 |= (1<<27)

#define OFFSET_IN_WORDS_SIP7(handler)    (((GT_DMM_BLOCK_SIP7*)handler)->word2 & 0x7FFFFFF) /* 27 bits */
#define SET_OFFSET_IN_WORDS_SIP7(handler, offset)    \
            if((offset) > 0x7FFFFFF)           \
            {                               \
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);     \
            }                               \
            ...
            {                               \
                handler->word2 = (handler->word2 & 0xF8000000) | (offset);   \
            }

#define DMM_BLOCK_STAMP_SIP7(handler)    ((((GT_DMM_BLOCK_SIP7*)handler)->word4>>3) & 0xFF) /* bits 3-10 */
 ...
 */
#define SET_DMM_BLOCK_STAMP_SIP7(handler, stamp) \
    handler->word4 = (handler->word4 & 0x7) | ((stamp) << 3)

#define DMM_GET_SIZE_SIP7(handler)  (SIZE_IN_WORDS_SIP7(handler) << 2)
#define DMM_GET_OFFSET_SIP7(handler)  (OFFSET_IN_WORDS_SIP7(handler) << 2)
#define DMM_GET_PARTITION_SIP7(handler)  ((GT_UINTPTR)(((GT_DMM_BLOCK_SIP7*)handler)->nextBySizeOrPartitionPtr.partitionPtr))


#define GET_DMM_BUCKET_SW_ADDRESS_SIP7(handler)      (((GT_DMM_BLOCK_SIP7*)handler)->word3)
#define SET_DMM_BUCKET_SW_ADDRESS_SIP7(handler, address)                                \
            {                                                                      \
                ((GT_DMM_BLOCK_SIP7*)handler)->word3 = ((GT_UINTPTR)address);           \
            }
#define RESET_BUCKET_SW_ADDRESS_SIP7(handler)  \
    ((GT_DMM_BLOCK_SIP7*)handler)->word3 = 0

/* Protocol: ipv4=0, ipv6=1, FCoE=2 bits 0-1 */
#define GET_DMM_BLOCK_PROTOCOL_SIP7(handler)     (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0x3)
#define SET_DMM_BLOCK_PROTOCOL_SIP7(handler, protocol)                                   \
        if((protocol) > 0x2)                                                        \
        {                                                                           \
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);       \
        }                                                                           \
        ...
        {                                                                           \
            ((GT_DMM_BLOCK_SIP7*)handler)->word4 =                                       \
            (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0xFFFFFFFC) | (protocol);            \
        }
#define RESET_BLOCK_PROTOCOL_SIP7(handler)                                               \
    ((GT_DMM_BLOCK_SIP7*)handler)->word4 =                                               \
            (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0xFFFFFFFC)      ;                   \

/* Block was moved: yes or no - bit 2 */
#define GET_DMM_BLOCK_WAS_MOVED_SIP7(handler)     (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0x4)
#define SET_DMM_BLOCK_WAS_MOVED_SIP7(handler, moved)                                     \
        if((moved) > 0x1)                                                           \
        {                                                                           \
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, LOG_ERROR_NO_MSG);       \
        }                                                                           \
        ...
        {                                                                           \
            ((GT_DMM_BLOCK_SIP7*)handler)->word4 =                                       \
            (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0xFFFFFFFB) | ((moved)<<2);          \
        }
#define RESET_BLOCK_WAS_MOVED_SIP7(handler)                                              \
    ((GT_DMM_BLOCK_SIP7*)handler)->word4 =                                               \
            (((GT_DMM_BLOCK_SIP7*)handler)->word4 & 0xFFFFFFFB)      ;                   \

/*Note : The number must be multiple of 32*/
#define DMM_MAXIMAL_BLOCK_SIZE_ALLOWED_SIP7  2112
/*This is DMM_MAXIMAL_BLOCK_SIZE_ALLOWED / 32*/
#define DMM_EMPTINESS_ARRAY_SIZE_SIP7        ((DMM_MAXIMAL_BLOCK_SIZE_ALLOWED_SIP7/32)+1)   /*33*/

#define DMM_STAMP_FREE_SIP7                  0x59
#define DMM_STAMP_USED_SIP7                  0xC8
#define DMM_STAMP_USED_STATIC_SIP7           0xC9

struct _gtDmmPartitionSip7;
/*
 * Typedef: GT_DMM_BLOCK_SIP7
 *
 ...
 *
 */
typedef struct dmmBlockSip7
{
    union
    {
        struct _gtDmmPartitionSip7 *partitionPtr;
        struct  dmmBlockSip7 *nextBySize;
    }nextBySizeOrPartitionPtr;
    struct  dmmBlockSip7 *prevBySize;
    struct  dmmBlockSip7 *nextByAddr, *prevByAddr;
    GT_U32      word1; /* sizeInWords:27(bits 0-26), status:1(bit #27)                       */
    GT_U32      word2; /* offsetInWords:27(bits 0-26)                                        */
    GT_UINTPTR  word3; /* bucketSwAddress:32(bits 0-31)                                      */
    GT_U32      word4; /* stamp:8(bits 3-10),blockWasMoved:1(bit2),protocolValue:2(bits 0-1) */
}GT_DMM_BLOCK_SIP7;

/*
 * Typedef: _gtDmmSortedPartitionElementSip7
 *
 ...
 *
 */
typedef struct _gtDmmSortedPartitionElementSip7
{
     GT_DMM_BLOCK_SIP7                           *blockElementPtr;
     struct _gtDmmSortedPartitionElementSip7     *nextSortedElemPtr;

}GT_DMM_SORTED_PARTITION_ELEMENT_SIP7;

/*
 * Typedef: GT_DMM_PARTITION_SIP7
 *
 * Description: Device memory block handler.
 *
 * Fields:
 ...
 *
 */
typedef struct _gtDmmPartitionSip7
{
    GT_DMM_BLOCK_SIP7                       *tableOfSizePointers[DMM_MAXIMAL_BLOCK_SIZE_ALLOWED_SIP7+1];
    GT_DMM_BLOCK_SIP7                       *bigBlocksList;
    GT_U32                                  emptinessBitArray[DMM_EMPTINESS_ARRAY_SIZE_SIP7];
    ...
    GT_U32                                  partitionSize;
    GT_DMM_BLOCK_SIP7                       *pointedBlock;
    GT_DMM_BLOCK_SIP7                       *pointedFirstFreeBlock;
    GT_DMM_BLOCK_SIP7                       *pointedFirstBlock;
    GT_DMM_SORTED_PARTITION_ELEMENT_SIP7    *tableOfSortedUsedBlocksPointers[DMM_MAXIMAL_BLOCK_SIZE_ALLOWED_SIP7+1];
}GT_DMM_PARTITION_SIP7;
