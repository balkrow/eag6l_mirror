...
#define PRV_APP_REF_GLOBAL_LOG_DB PRV_NON_SHARED_GLOBAL_VAR_DIRECT_GET(nonVolatileDb.appRefLogDb.capLogDb)


#define CPSS_APP_PLATFORM_LOG_ERR_MAC(...) \
do {                                                                           \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_ERR_E)             \
 {                                                                             \
...
#define CPSS_APP_PLATFORM_LOG_INFO_MAC(...) \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_INFO_E) \
     CPSS_APP_PLAT_PRINTF("<INFO> "__VA_ARGS__)

#define CPSS_APP_PLATFORM_LOG_DBG_MAC(...) \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_DBG_E) \
     CPSS_APP_PLAT_PRINTF("\n<DBG> "__VA_ARGS__)

#define CPSS_APP_PLATFORM_LOG_ENTRY_MAC() \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_ENTRY_E) \
     CPSS_APP_PLAT_PRINTF("\n[IN] %s\n", __func__)

#define CPSS_APP_PLATFORM_LOG_EXIT_MAC(_rc) \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_EXIT_E) \
     CPSS_APP_PLAT_PRINTF("\n[OUT] %s RET=0x%x\n", __func__, _rc)

#define CPSS_APP_PLATFORM_LOG_PRINT_MAC(...) \
 if (PRV_APP_REF_GLOBAL_LOG_DB[CAP_LOG_MODULE] & CPSS_APP_PLATFORM_LOG_LEVEL_PRINT_E) \
     CPSS_APP_PLAT_PRINTF(__VA_ARGS__)
