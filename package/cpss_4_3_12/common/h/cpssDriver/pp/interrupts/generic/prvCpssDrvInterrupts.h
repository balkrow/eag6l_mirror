...
typedef struct
{
    /** @brief : information per port group
    */
    struct{
        /** @brief : An array containing all interrupt nodes to be en-queued / de-queued
         *   in the interrupt queues
        */
        PRV_CPSS_DRV_EV_REQ_NODE_STC    *intNodesPool;

        /** @brief : An array containing all interrupt SW nodes to be en-queued / de-queued
         *   in the event queues
        */
        PRV_CPSS_DRV_EV_REQ_NODE_STC    *swEvNodesPool;

        /** @brief : The interrupt information hierarchy tree root.
         *  dynamic allocated according to numOfIntBits/32
         */
        PRV_CPSS_DRV_INTERRUPT_SCAN_STC *intScanRoot;

        /** @brief : Holds the interrupt mask registers shadow.
         *  dynamic allocated according to numOfIntBits/32
         */
        GT_U32                          *intMaskShadow;

        /** @brief : Id for access to the isr connection table */
        GT_UINTPTR                      isrConnectionId;

        /** @brief : The interrupt vector number for this port group. */
        GT_U32                          intVecNum;

        /** @brief : a cookie 'per port group' ,
    ...
    /** @brief : A pointer to an ISR to be called when this device should be scanned for interrupts.*/
    CPSS_EVENT_ISR_FUNC                  isrFuncPtr;

    PRV_CPSS_DRV_INT_INIT_PHASE_ENT      *initPhase;

    /** @brief : Number of interrupt bits defined for this device.*/
    GT_U32              numOfIntBits;
    /** @brief : Number of software interrupt nodes defined for this device.*/
    GT_U32              numOfSwIntNodes;
    /** @brief : Array of pointers to the different Gpps interrupt service routines.*/
    PRV_CPSS_DRV_GPP_INT_INFO_STC        * *gppIsrFuncsArr;

    /** @brief : register address for the summary mask relevant only for PCIe */
    GT_U32                               intSumMaskRegAddr;

    /** @brief : Do we need to initialize the
    ...
    */
    ...
    /* num elements in uniEvMapTableWithExtDataArr */
    GT_U32 uniEvMapTableWithExtDataSize;

    /* typedef for uniEvMapTableArr[][2] */
    const PRV_CPSS_DRV_EVENT_INFO_TYPE *uniEvMapTableArr;

    /* num elements in uniEvMapTableArr */
    GT_U32 uniEvMapTableSize;

    /* number of port per core */
