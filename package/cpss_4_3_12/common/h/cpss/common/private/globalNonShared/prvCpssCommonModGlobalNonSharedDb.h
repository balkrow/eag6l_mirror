...
typedef struct
{
    ...
    MV_DEVICE_TEMPERATURE_GET       hwsDeviceTemperatureGetFuncGlobalPtr;
...
/**
 *@struct
 *        PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_INVOKER_SRC_GLOBAL_DB
 *
 * @brief  Structure contain global variables that are not meant
 *         to be shared between processes (when CPSS is
 *        linked as shared object) in common module ,labServices
 *        directory, src file : mvHwsSseInvoker.c
*/
typedef struct
{
    SSE_ARG_CODE                                    argCodeList[MAX_ARGS];
    MV_HWS_SSE_INVOKER_SERDES_RD_WR_CALLBACKS       rdWrSerdesRegCbFuncs;
    GT_BOOL                                         isScriptLoaded;
    GT_BOOL                                         arePrintsEnable;
}PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_INVOKER_SRC_GLOBAL_DB;

/**
 *@struct
 *        PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_MAPPER_SRC_GLOBAL_DB
 *
 * @brief  Structure contain global variables that are not meant
 *         to be shared between processes (when CPSS is
 *        linked as shared object) in common module ,labServices
 *        directory, src file : mvHwsSseMapper.c
*/
typedef struct
{
    SSE_LUT_ENTRY serdesLUT [MAX_SERDES_OP_TYPES];
}PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_MAPPER_SRC_GLOBAL_DB;
...
typedef struct
{
    ...
    PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_INVOKER_SRC_GLOBAL_DB   mvHwsSseInvokerSrc;
    PRV_CPSS_COMMON_MOD_LAB_SERVICES_DIR_NON_SHARED_SSE_MAPPER_SRC_GLOBAL_DB    mvHwsSseMapperSrc;
...
} PRV_CPSS_COMMON_MOD_CPSS_HW_INIT_DIR_NON_SHARED_GLOBAL_DB;
#endif /*CHX_FAMILY*/

/**
* @struct PRV_CPSS_COMMON_MOD_DAIG_SSE_PARSER_NON_SHARED_GLOBAL_DB
 *
 * @brief  Structure contain global variables that are meant to be shared
 * between processes (when CPSS is linked as shared object)
 *   in common module, sse directory for prvCpssDxChSseParser
*/
typedef struct
{
    SSE_SEQ_INST     seqInstruction;
    GT_BOOL          isLoopOpCodeRecognized;
    GT_BOOL          arePrintsEnable;
}PRV_CPSS_COMMON_MOD_DIAG_SSE_PARSER_NON_SHARED_GLOBAL_DB;
 ...
 * @brief  Structure contain global variables that are meant to be shared
 * between processes (when CPSS is linked as shared object)
 *   in common module ,diag directory
*/
typedef struct
{

    PRV_CPSS_COMMON_MOD_DIAG_SSE_PARSER_NON_SHARED_GLOBAL_DB    cpssDxChSseParserSrc;
}PRV_CPSS_COMMON_MOD_DIAG_NON_SHARED_GLOBAL_DB;
...
#ifdef CHX_FAMILY
    PRV_CPSS_COMMON_MOD_CPSS_HW_INIT_DIR_NON_SHARED_GLOBAL_DB        genericHwInitDir;
#endif
    PRV_CPSS_COMMON_MOD_DIAG_NON_SHARED_GLOBAL_DB                    diagDir;
