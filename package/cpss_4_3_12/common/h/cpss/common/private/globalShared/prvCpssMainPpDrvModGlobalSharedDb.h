...
/**
 *@struct
 *        PRV_CPSS_MAIN_PP_MOD_LAB_SERVICES_DIR_SHARED_AAS_DEV_INIT_SRC_GLOBAL_DB
 *
 * @brief  Structure contain global variables that are meant to be shared
 * between processes (when CPSS is linked as shared object)
 *   in mainPpDrv module ,labServices directory, src file :
...
{
    GT_BOOL hwsInitDone[HWS_MAX_DEVICE_NUM];
    GT_BOOL aasDbInitDone;
} PRV_CPSS_MAIN_PP_DRV_MOD_LAB_SERVICES_DIR_SHARED_AAS_DEV_INIT_SRC_GLOBAL_DB;
...
typedef struct
{
    ...
    GT_U32  hwsFalconNumOfRavens ;
    GT_BOOL hwsSerdesFwDownloadDone ;

} PRV_CPSS_MAIN_PP_DRV_MOD_LAB_SERVICES_DIR_SHARED_FALCON_DEV_INIT_SRC_GLOBAL_DB;
...
typedef struct
{
    ...
    PRV_CPSS_MAIN_PP_DRV_MOD_LAB_SERVICES_DIR_SHARED_FALCON_DEV_INIT_SRC_GLOBAL_DB   falconDevinitSrc;
    PRV_CPSS_MAIN_PP_DRV_MOD_LAB_SERVICES_DIR_SHARED_AAS_DEV_INIT_SRC_GLOBAL_DB      aasDevInitSrc;
...
typedef struct
{
    ...
    GT_BOOL     extendedPortDeletedFlag;
...
typedef struct
{
    ...
    GT_U32 packetBytesArray[PRV_CPSS_DXCH_LPM_NUM_OF_OCTETS_FOR_GON_IN_IPV6_MC_PROTOCOL_CNS];
    GT_BOOL packetsByteArrayStatus[PRV_CPSS_DXCH_LPM_NUM_OF_OCTETS_FOR_GON_IN_IPV6_MC_PROTOCOL_CNS];
    GT_U32 grpPrefix;
    GT_BOOL InSrcTree;
...
typedef struct
{
    GT_VOID     * mpdGlobalSharedDb;
} PRV_CPSS_MAIN_PP_DRV_MOD_PHY_DIR_SHARED_GLOBAL_DB;
...
typedef struct
{
    /* flow manager DB to. Actual type of pointer is CPSS_DXCH_FLOW_MNG_PTR */
    GT_VOID * flowMngDB[PRV_CPSS_DXCH_FLOW_MANAGER_MAX_ID_CNS + 1];
...
typedef struct
{
    ...
    PRV_CPSS_DXCH_CUCKOO_FUNC_PTR_STC  cuckooDbFuncPtr[PRV_CPSS_DXCH_CUCKOO_CLIENTS_LAST_E];
...
typedef struct
{
    ...
    GT_U32 (*debug_unitAccessEnableCheckCallbackPtr)(GT_U8 devNum, GT_U32 unitId);
...
typedef struct
{
    ...
    /** AAS-L unit base address array */
    GT_U32 prvDxChAasUnitsBaseAddr[PRV_CPSS_DXCH_UNIT_LAST_E + 1];
    ...
    /** AAS unit ID to base address DB */
    PRV_CPSS_DXCH_UNITID_UNITBASEADDR_STC prvDxChAasUnitsIdUnitBaseAddrArr[AAS_MAX_UNITS];
...
typedef struct
{
    ...
    /** 1 - Check "reset needed" stamp in HW, if found appDemo does reset and recall Phase1Init */
    /** 0 - Phase1Init called once without reset                                                */
    GT_U32  force_early_check_for_device_not_reset;
    ...
    /** debug value for temperature sensor threshold */
    GT_32 debugTsenThreshold;
...
typedef struct
{
    ...
    */
    ...
    /**@brief : !!! debug only !!! patch for sip7 till proper API allow to pass
      *     this param in : CPSS_DXCH_BRG_FDB_HASH_PARAMS_STC
      *     hold the value of maxFidValue (when != 0) , needed for sip7 that supports
      *     (64K-1) instead of 8K-1 of sip5 devices
    */
    GT_U32  debug_maxFidValue;

} PRV_CPSS_MAIN_PP_DRV_MOD_DXCH_CONFIG_DIR_SHARED_GLOBAL_DB;

/* max number of managers supported.
   must be aligned with CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS. */
#define PRV_CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS    32
...
typedef struct
{
    ...
    GT_VOID * prvCpssDxChFdbManagerDbArr[PRV_CPSS_DXCH_BRG_FDB_MANAGER_MAX_INSTANCE_NUM_CNS];
...
/* maximal amount of Virtual TCAM Managers must be equal to
   CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS */
#define PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS 31
...
{
    /** TCAM manager DB
     *  Actual type of pointer is PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_PTR
     */
    GT_VOID *vTcamMngDB[PRV_CPSS_DXCH_VIRTUAL_TCAM_MNG_MAX_ID_CNS + 1];
     ...
     * completion.
     * Actual type of pointer is CPSS_DXCH_VIRTUAL_TCAM_HA_DB_REPLAY_RULE_ENTRY_STC
     */
    GT_VOID *replayRuleDb;
    GT_VOID *replayRuleDbTail;
    /** TCAM Manager HA Replay DB
     * Used to store list of rules which were unmapped found during HA config-replay
     * Actual type of pointer is CPSS_DXCH_VIRTUAL_TCAM_HA_UNMAPPED_RULE_ENTRY_STC
     */
    GT_VOID *replayUnmappedRuleDb;
    GT_VOID *replayUnmappedRuleDbTail;
    ...
    /** memory to build list of vTCAM space-conversion rules
     *  Actual type of pointer is PRV_SEG_DRAG_HORZ_BLOCK_ROW_STATUS_CHANGE_RULE_PTR */
    GT_VOID *work_rules_arr[100];
    ...
    /* dynamic allocated array for vTCAM resize
       actual type of pointer is PRV_CPSS_DXCH_VIRTUAL_TCAM_DB_RESIZE_HW_INDEX_MAP_INFO_STC */
    GT_VOID  *prvCpssDxChVirtualTcamDbResizeHwIndexDbArr;
...
#define PRV_SHARED_GENERIC_TM_DIR_GLOBAL_VAR_GET(_var) \
            PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.genericTmDir._var)

/**
* @struct PRV_CPSS_MAIN_PP_DRV_MOD_GENERIC_TM_DIR_SHARED_GLOBAL_DB
*
* @brief  Structure contain global variables that are meant to be shared
* between processes (when CPSS is linked as shared object)
* in mainPpDrv module , generic tm directory
*/
typedef struct
{
    /** Count TM Lib init executions */
    GT_U32      tmCurrentNum;

    /** LAD parameter Settings */
    CPSS_TM_CTL_LAD_INF_PARAM_STC tmLadParamsArr[CPSS_TM_CTL_MAX_NUM_OF_LADS_CNS+1];

    /** TM HW Params for BC2 */
    PRV_CPSS_TM_HW_PARAMS_STC tmHwParamsBc2;

    /** TM HW Params for Bobk */
    PRV_CPSS_TM_HW_PARAMS_STC tmHwParamsBobk;

    /** TM Debug Log enable/disable */
    GT_BOOL tmDbgLogOn;

    /** TM to CPSS Error Codes conversion array */
    GT_STATUS tmToCpssErrCodes[101]; /* ArraySize: TM_CONF_MAX_ERROR + 1 */

    /** TM to CPSS Error Codes conversion array */
    GT_STATUS tmToCpssErrCodesNeg[140];

    /** TM BAP DRAM Params */
    GT_BOOL  bapDramAccessTest;
    GT_U32   bapDramAccessTestRetries;
    GT_U32   bapDramBaseAddress;
    GT_U32   bapDramHighAddress;

} PRV_CPSS_MAIN_PP_DRV_MOD_GENERIC_TM_DIR_SHARED_GLOBAL_DB;
#endif

typedef struct {
    GT_VOID_PTR   *flexeDbPtr;
} PRV_CPSS_MAIN_PP_DRV_MOD_GENERIC_FLEXE_DIR_SHARED_GLOBAL_DB;
...
typedef struct
{
    ...
    PRV_CPSS_MAIN_PP_DRV_MOD_PHY_DIR_SHARED_GLOBAL_DB           phyDir;
#if defined (INCLUDE_TM)
    PRV_CPSS_MAIN_PP_DRV_MOD_GENERIC_TM_DIR_SHARED_GLOBAL_DB    genericTmDir;
#endif
    PRV_CPSS_MAIN_PP_DRV_MOD_GENERIC_FLEXE_DIR_SHARED_GLOBAL_DB    genericFlexeDir;
#endif
