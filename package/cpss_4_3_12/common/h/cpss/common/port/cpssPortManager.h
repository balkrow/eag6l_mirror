...
typedef enum{
    ...
    /** @brief Notify CPSS on low level units (SerDes or PCS) status
...
typedef enum{
    ...
    /** @brief Port is in reset state, all of the low level units
     *  (SerDes or PCS) and MAC units are in reset
     */
    CPSS_PORT_MANAGER_STATE_RESET_E,                /* 0 */

    /** @brief Port is in link down state, both the MAC unit and the low level
     *  units (SerDes or PCS) are unreset but down (for example,
     *  SerDes is in unreset but signal is not yet detected from a
     *  peer)
     */
     ...
     *  low level (SerDes or PCS) are up
     */
    CPSS_PORT_MANAGER_STATE_LINK_UP_E,              /* 3 */

    /** Port MAC link is down, but low level units (SerDes or PCS)
     *  are up */
...
/**
* @union CPSS_PORT_MANAGER_LOOPBACK_MODE_STC
 *
 * @brief Enumeration of different port loopback types
*/
typedef struct{
...
} CPSS_PORT_MANAGER_LOOPBACK_MODE_STC;
...
*/
typedef struct {
    GT_BOOL                                 enableRegularTraffic;

    CPSS_PORT_MANAGER_LOOPBACK_TYPE_ENT     loopbackType;

    CPSS_PORT_MANAGER_LOOPBACK_MODE_STC     loopbackMode;
...
typedef enum{
    ...
    /* Default profile interconnect parameters:
        10G SERDES speed – Up to 7m cable, loss ≤ 20dB
        25G SERDES speed – 1m cable and above, loss > 15dB
        PAM4 – Interconnect with 15dB-25dB loss */
    CPSS_PORT_MANAGER_INTERCONNECT_PROFILE_DEFAULT_E = 0,

    /* Customed profile #1:
        APPLICABLE DEVICES: Falcon.
        10G SERDES speed – 7m cable and above, loss > 20db
        25G/27G SERDES speed – Up to 1m cable, loss ≤ 15dB
        PAM4 – Interconnect with Falcon to Falcon, loss ≤ 15dB */
    CPSS_PORT_MANAGER_INTERCONNECT_PROFILE_1_E,

    /* Customed profile #2:
    APPLICABLE DEVICES: Falcon.
    10G SERDES speed – For future use only
    25G SERDES speed – For future use only
    PAM4 – Interconnect with Falcon to Falcon, loss 15-30dB
    106G AP port mode, 27.3G SERDES speed – Initiate auto-SERDES profile mechanism */
...
typedef struct{
    ...
    /*
        Bypass preemption verification. When set to 1 the MAC will not attempt
        to verify preemption. Only set this bit if it is known that the peer port
        supports preemption. Has no effect if TX_PREEMPT_EN is set to 0
    */
    GT_BOOL                                     disableVerification;
...
typedef enum {
  ...
  /* create static port using ANP */
  CPSS_PM_PORT_ATTR_STATIC_OVER_ANP_E     =0x8000,

  /* set duplex mode for non AN(clause37) */
  CPSS_PM_PORT_ATTR_DUPLEX_MODE_E        =0x10000,

  /* set special mode when signal detection callback provided by application (for case PM over FW) */
  CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E =0x20000,
...
typedef struct {
  ...
  GT_BOOL                                       pmOverFw;
  GT_BOOL                                       staticOverANP;
  CPSS_PORT_DUPLEX_ENT                          duplexMode;     /* for non AN mode, for 10M/100M, for SGMII/QSGMII/OUSGMII */
  GT_BOOL                                       extSigDetCallbackEnable;
...
typedef enum {
  ...
  /*TX Adapt*/
  CPSS_PM_AP_PORT_ATTR_TX_POST_ADAPT_E        =0x400
 ...
 *
 * @brief Available ANP functions call over PM
 *
 */
typedef enum
{
    CPSS_PM_PORT_ANP_FUNC_PD_INIT_E,
    CPSS_PM_PORT_ANP_FUNC_PD_WAIT_FOR_PROG_E,
    CPSS_PM_PORT_ANP_FUNC_PD_RESET_E,
    CPSS_PM_PORT_ANP_FUNC_PD_AUTO_NEG_SET_E,
    CPSS_PM_PORT_ANP_FUNC_PD_AN37_E,
    CPSS_PM_PORT_ANP_FUNC_STATIC_OVER_ANP_SET_E,
    CPSS_PM_PORT_ANP_FUNC_STATIC_OVER_ANP_GET_E,
    CPSS_PM_PORT_ANP_FUNC_PCS_CFG_DONE_E,
    CPSS_PM_PORT_ANP_FUNC_SKIP_RESET_E,
    CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E,
    CPSS_PM_PORT_ANP_FUNC_TX_POST_ADAPT_SET_E,

    CPSS_PM_PORT_ANP_FUNC_LAST_E

} CPSS_PM_PORT_ANP_FUNC_ENT;
...
typedef enum
{
    CPSS_PM_PORT_ANP_TRX_TRAINING_E,
    CPSS_PM_PORT_ANP_RX_TRAINING_E,
    CPSS_PM_PORT_ANP_NO_TRAINING_E,

    CPSS_PM_PORT_ANP_TRAINING_LAST_E

} CPSS_PM_PORT_ANP_TRAINING_MODE_ENT;

/**
*  @enum CPSS_PM_PORT_ANP_PARAM_ENT
 *
 * @brief Available parameters used for ANP functions call over
 *        PM
 *
 */
typedef enum
{
    CPSS_PM_PORT_ANP_PARAM_IFMODE_E = 0x1,
    CPSS_PM_PORT_ANP_PARAM_SPEED_E  = 0x2,
    CPSS_PM_PORT_ANP_PARAM_ENABLE_E = 0x4,
    CPSS_PM_PORT_ANP_PARAM_TRAINING_MODE_E = 0x8,

    CPSS_PM_PORT_ANP_PARAM_LAST_E

} CPSS_PM_PORT_ANP_PARAM_ENT;


/**
* @struct CPSS_PM_PORT_ANP_CALL_STC
 *
 * @brief This structure contains extra parameters for ANP
 *        function call
*/
typedef struct {
  ...
  CPSS_PORT_SPEED_ENT                   speed;
  GT_BOOL                               enable;
  CPSS_PM_PORT_ANP_TRAINING_MODE_ENT    trainingMode;
    /*
       CPSS_PM_PORT_ANP_PARAM_IFMODE_E
       CPSS_PM_PORT_ANP_PARAM_SPEED_E,
       CPSS_PM_PORT_ANP_PARAM_ENABLE_E,
       CPSS_PM_PORT_ANP_PARAM_TRAINING_MODE_E
       CPSS_PM_PORT_ANP_PARAM_TX_POST_ADAPT_SET_E
    */
  GT_U32                            anpParamBitMask;

} CPSS_PM_PORT_ANP_PARAM_STC;
