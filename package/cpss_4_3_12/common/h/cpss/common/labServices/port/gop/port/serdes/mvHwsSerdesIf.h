...
* @endinternal
*
* @brief   some SDs have an issue with band-gap unit, so need to
*          use band-gap units from other SDs to compensate
*          signal drop. If activated, configuration will be
*          executed in device init for all SDs, once, and should
*          not be deleted (to avoid glitches).
*
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] serdesNum                - serdes number
...
#if defined(MV_HWS_REDUCED_BUILD) && !defined(AC5_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
