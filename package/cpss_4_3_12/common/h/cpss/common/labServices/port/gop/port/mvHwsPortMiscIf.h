...
#define EYE_MAX_PHASE_LEVEL 2048
#define EYE_MAX_VOLT_STEPS 129
#define MAX_EYE_NUM 3

/**
 * @enum MV_HWS_RX_INIT_MODE
 *
 * @brief This enum includes rx_init modes
*/
typedef enum
{
    RX_INIT_START,
...
typedef struct
{
    ...
    /* Advanced (Thresholds) */
    GT_U16 midpointLargeThresKLane;
    GT_U16 midpointSmallThresKLane;
    GT_U16 midpointLargeThresCLane;
    GT_U16 midpointSmallThresCLane;
    GT_U16 inxSumfMidpointAdatptiveEnLane;

    /* Advanced (DFE) */
    GT_U16 dfeResF0aHighThresInitLane;
    GT_U16 dfeResF0aHighThresEndLane;

    /* Advanced (CTLE) */
    GT_U16 sumfBoostTargetC0;
    GT_U16 sumfBoostTargetC1;
    GT_U16 sumfBoostTargetC2;
    GT_U16 midpointPhaseOs0;
    GT_U16 midpointPhaseOs1;
    GT_U16 midpointPhaseOs2;
...
typedef enum{
    ...
    /* add extra precoding encoding to SerDes (only for PAM4) */
    MV_HWS_PORT_SERDES_OPERATION_PRECODING_E = 0x1,

    /* check cal_status after training and update dfe_cmn in case of failure (only for Falcon) */
    MV_HWS_PORT_SERDES_OPERATION_DFE_COMMON_E = 0x2,

    /* add a calculation to level3 (indication of eye levels) based on trained level3 (only for Falcon) */
    MV_HWS_PORT_SERDES_OPERATION_CALC_LEVEL3_E = 0x4,

    /* reset DFE */
    MV_HWS_PORT_SERDES_OPERATION_RESET_RXTX_E = 0x8,

    /* Interconnect Change algorithm - tune CTLE (RES, CAP) */
    MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_IC_E = 0x10,

    /* update CM3 on training mode for portModes need only Rx training */
    MV_HWS_PORT_SERDES_OPERATION_RX_TRAIN_E = 0x20,

    /* update CM3 on optical mode */
    MV_HWS_PORT_SERDES_OPERATION_IS_FIBER_E = 0x40,

    /* check if dtl_clamping_triggered after training (indicates CDR not locked, if triggered, consider retrain) */
    MV_HWS_PORT_SERDES_OPERATION_CHECK_CLAMPING_TRIGGERED_E = 0x80,

    /* restore configuretion back from CPSS_PORT_MANAGER_EVENT_PORT_NO_DEBUG_E */
    MV_HWS_PORT_SERDES_OPERATION_DEBUG_MODE_E = 0x100,

    /* CDR algorithm - tune CTLE (RES, CAP) */
    MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_CDR_E = 0x200,

    /* configure CTLE limits via command interface */
    MV_HWS_PORT_SERDES_OPERATION_PRE_RX_TRAINING_E = 0x400,

    /* Ultra Short Reach */
    MV_HWS_PORT_SERDES_OPERATION_SET_USR_E = 0x800,

    /* set CTLE limits */
    MV_HWS_PORT_SERDES_OPERATION_SET_CTLE_LIMITS_E = 0x1000,

    /* preset check */
    MV_HWS_PORT_SERDES_OPERATION_PRESET_CHECK_E = 0x2000,

    /* check tx impedance calibration to indicate SerDes power-up issue */
    MV_HWS_PORT_SERDES_OPERATION_CHECK_TX_IMPEDANCE_CALIBRATION_E = 0x4000,

    /* check the sync e status and according to the configured ports in the group, return if valid */
    MV_HWS_PORT_SERDES_OPERATION_SYNC_E_CHECK_STATUS_E = 0x8000,
	/* set cdr serdes values (mufi and muff params) */
    MV_HWS_PORT_SERDES_OPERATION_CDR_VALUES_SET_E = 0x10000
 ...
 *
 * @brief Eye: Middle, Top or Bottom
*/
typedef enum{

    MV_HWS_PORT_SERDES_TMB_TOP_E = 0,
    MV_HWS_PORT_SERDES_TMB_MID_E = 1,
    MV_HWS_PORT_SERDES_TMB_BOT_E = 2

} MV_HWS_PORT_SERDES_EOM_TMB_ENT;

/**
* @enum MV_HWS_PORT_SERDES_EOM_MIN_SAMPLES_ENT
 *
 * @brief minimum number of bits to sample
*/
typedef enum{

    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_3_E =  1000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_4_E =  10000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_5_E =  100000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_6_E =  1000000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_7_E =  10000000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_8_E =  100000000,
    MV_HWS_PORT_SERDES_MIN_SAMPLES_ORDER_10_9_E =  1000000000

} MV_HWS_PORT_SERDES_EOM_MIN_SAMPLES_ENT;

/**
* @enum MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT
 *
 * @brief Bit Error Rate Threshold in nano (factor of 1E-9)
*/
typedef enum{

    MV_HWS_PORT_SERDES_BER_THRESHOLD_ORDER_10_3_E =  1000,
    MV_HWS_PORT_SERDES_BER_THRESHOLD_ORDER_10_4_E =  10000,
    MV_HWS_PORT_SERDES_BER_THRESHOLD_ORDER_10_5_E =  100000,
    MV_HWS_PORT_SERDES_BER_THRESHOLD_ORDER_10_6_E =  1000000

} MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT;
 ...
 *
 * @brief EOM input parametets
*/
typedef struct
{
    MV_HWS_PORT_SERDES_EOM_MIN_SAMPLES_ENT      minSamples;
    MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT    berThreshold;
    MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT    berThresholdMax;
    GT_U32                                      voltageSteps;
    GT_U32                                      phaseLevels;
}MV_HWS_COMPHY_C12GP41P2V_EOM_IN_PARAMS;
...
typedef struct
{
    ...
    GT_U32                                      voltageSteps;
    GT_U32                                      phaseLevels;
}MV_HWS_COMPHY_C28GP4_EOM_IN_PARAMS;
...
typedef struct
{
    MV_HWS_PORT_SERDES_EOM_MIN_SAMPLES_ENT      minSamples;
    MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT    berThreshold;
    GT_BOOL                                     eomStatsMode;
    MV_HWS_PORT_SERDES_EOM_BER_THRESHOLD_ENT    berThresholdMax;
    GT_U32                                      phaseStepSize;
    GT_U32                                      voltageStepSize;
}MV_HWS_COMPHY_C112G_EOM_IN_PARAMS;
...
typedef struct
{
    ...
    GT_U32                                      phaseStepSize;
    GT_U32                                      voltageStepSize;
}MV_HWS_COMPHY_C56G_EOM_IN_PARAMS;

/**
 * @union MV_HWS_SERDES_EOM_PARAMS_UNT
 *
 * @brief Contains EOM input parametets struct per serdes type
*/
typedef union {
    MV_HWS_COMPHY_C12GP41P2V_EOM_IN_PARAMS    comphyC12GP41P2VEomInParams;
    MV_HWS_COMPHY_C28GP4_EOM_IN_PARAMS        comphyC28GP4EomInParams;
    MV_HWS_COMPHY_C112G_EOM_IN_PARAMS         comphyC112GEomInParams;
    MV_HWS_COMPHY_C56G_EOM_IN_PARAMS          comphyC56GEomInParams;
}MV_HWS_SERDES_EOM_IN_PARAMS_UNT;

/**
 * @struct MV_HWS_COMPHY_C12GP41P2V_EOM_OUT_PARAMS
 *
 * @brief EOM output parametets
*/
typedef struct
{
    GT_U32         width_mUI;
    GT_U32         height_mV;
}MV_HWS_COMPHY_C12GP41P2V_EOM_OUT_PARAMS;
...
typedef struct
{
    ...
    GT_U32         height_mV;
    GT_U32         sampleCount;
}MV_HWS_COMPHY_C28GP4_EOM_OUT_PARAMS;
 ...
 * @brief EOM output parametets
 *
 * @note NRZ - matrix[0] is valid (mid eye)
 *      PAM4 - matrix[0] = top eye, matrix[1] = mid eye, matrix[2] = bot eye
*/
typedef struct
{
    GT_U32         width_mUI[MAX_EYE_NUM];
    GT_U32         height_mV[MAX_EYE_NUM];
    GT_U32         sampleCount[MAX_EYE_NUM];

    /* Estimated Eye Amplitudes Structure */
    GT_FLOAT64     Q[MAX_EYE_NUM];           /* Q factor */
    GT_FLOAT64     SNR[MAX_EYE_NUM];         /* Signal-to-noise ratio */
    GT_FLOAT64     upperMean[MAX_EYE_NUM];   /* Upper mean amplitude */
    GT_FLOAT64     lowerMean[MAX_EYE_NUM];   /* Lower mean amplitude */
    GT_FLOAT64     upperStdDev[MAX_EYE_NUM]; /* Upper standard deviation */
    GT_FLOAT64     lowerStdDev[MAX_EYE_NUM]; /* Lower standard deviation */
}MV_HWS_COMPHY_C112G_EOM_OUT_PARAMS;
...
typedef struct
{
    ...
    GT_FLOAT64     upperStdDev[MAX_EYE_NUM]; /* Upper standard deviation */
    GT_FLOAT64     lowerStdDev[MAX_EYE_NUM]; /* Lower standard deviation */
}MV_HWS_COMPHY_C56G_EOM_OUT_PARAMS;

/**
 * @union MV_HWS_SERDES_EOM_PARAMS_UNT
 *
 * @brief Contains EOM output parametets struct per serdes type
*/
typedef union {
    MV_HWS_COMPHY_C12GP41P2V_EOM_OUT_PARAMS    comphyC12GP41P2VEomOutParams;
    MV_HWS_COMPHY_C28GP4_EOM_OUT_PARAMS        comphyC28GP4EomOutParams;
    MV_HWS_COMPHY_C112G_EOM_OUT_PARAMS         comphyC112GEomOutParams;
    MV_HWS_COMPHY_C56G_EOM_OUT_PARAMS          comphyC56GEomOutParams;
}MV_HWS_SERDES_EOM_OUT_PARAMS_UNT;
...
/**
* @internal mvHwsPortSerdesClampingValuesResetAndstatusGet function
* @endinternal
*
* @brief   Reset clamping values if clamping status is false,
*          and get the clamping status
*
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] phyPortNum               - physical port number
...
* @retval 1                        - on error
*/
GT_STATUS mvHwsPortSerdesClampingValuesResetAndstatusGet
(
   IN GT_U8                   devNum,
   IN GT_U32                  portGroup,
   IN GT_U32                  phyPortNum,
...
/**
* @internal mvHwsPortSignalDetectMask function
* @endinternal
*
* @brief   Port signal detect mask configuration.
*
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] phyPortNum               - physical port number
* @param[in] portMode                 - port standard metric
* @param[in] maskEnable               - enable/disable mask for port signal detect
*
* @retval 0                        - on success
* @retval 1                        - on error
*/
GT_STATUS mvHwsPortSignalDetectMask
(
    IN GT_U8                 devNum,
    IN GT_U32                portGroup,
    IN GT_U32                phyPortNum,
