...
#define PRV_CPSS_IS_DUAL_HW_DEVICE_MAC(hwDevNum) \
    (((hwDevNum) < BIT_12) && ( \
     PRV_SHARED_GLOBAL_VAR_GET(mainPpDrvMod.hwInitDir.prvCpssHwDevNumModeDb[(hwDevNum)]) \
...
/* Ironman SAR Status0 register offset and fields */
#define PRV_CPSS_IRONMAN_SAR_STATUS_0_REG               0xF8200
#define PRV_CPSS_IRONMAN_SAR_STATUS_0_REF_CLK_SEL_BIT   19
...
/* check SIP_7 */
#define PRV_CPSS_SIP_7_CHECK_MAC(_devNum)                 \
    (PRV_CPSS_SIP_CHECK_MAC(_devNum,CPSS_GEN_SIP_7_E))
...
#define PRV_CPSS_DEV_USER_SOFT_RESET_SUPPORTED_DEVICES_MAC(_devNum)  \
    if (PRV_CPSS_SIP_6_CHECK_MAC(_devNum) == 0 &&   \
        PRV_CPSS_PP_MAC(_devNum)->devFamily != CPSS_PP_FAMILY_DXCH_AC5_E) \
    { \
...
typedef struct{
    ...
    /** @brief Shared memory manager creation indication */
    GT_BOOL cnmRamManagerEnabled;
    /** @brief manager parameters per memory type    */
    struct{
        /** @brief Size of the CnM SRAM\DRAM */
        GT_U64  cnmRamSize;
        /** @brief Base address of the CnM SRAM\DRAM */
        GT_U64  cnmRamBase;
        /** @brief Free blocks list */
        GT_VOID *cnmRamFreeList;
        /** @brief Used blocks list */
        GT_VOID *cnmRamUsedList;
    }cnmRam[4];
...
typedef struct
{
    ...
    CPSS_HW_DRIVER_STC*                    i2cMasters[CPSS_I2C_FALCON_MAX_BUS_PER_PP_CNS];
    ...
    /** indication 'per device' if L2NAT supported by the device */
    GT_BOOL                                l2NatSupported;

    GT_U32                                 sdmaPortNum;
    GT_U32                                 sdmaPorts[CPSS_MAX_SDMA_PORT_NUM_CNS];
