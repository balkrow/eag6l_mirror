...
)
{
    ...
    /*No hwInfo is passed as an argument here.
           Since there is no per device data base  yet-  we can not matched mapped resources to devices.
           This will be done later upon driver creation.

        */
    rc = cpssGlobalDbInit(cpssAslrSupport,GT_TRUE,0,0);
    if(rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    mvHwsAvagoSerdesDbFree();

    prvCpssGenericSrvCpuDestroy();
#endif

    return cpssGlobalDbDestroy(GT_TRUE/*unlink shmem*/,GT_TRUE,NULL);
}
...
#define STR_FOR_FAMILY(family) devInfoPtr->devFamily == family  ? #family :
    ...
    for(ii=0; devInfoPtr->numOfPorts > (32*ii); ii++)
    {
        DEBUG_PRINT((PRINT_FORMAT_8_8x_MAC(devInfoPtr->existingPorts.ports[ii])));
    }
