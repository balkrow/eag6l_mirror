...
GT_BOOL mvHwsApPortEnableCtrl = GT_FALSE;
#else
GT_BOOL mvHwsApPortEnableCtrl = GT_TRUE;
#endif

#if defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
GT_U32  mvPortCtrlApStopDebugFlag = 0;
#endif
...
#ifdef IRONMAN_DEV_SUPPORT
GT_U8 refClockSourceDb[IRONMAN_PORTS_NUM];
#endif
...
#if defined (ALDRIN_DEV_SUPPORT) || defined(BC3_DEV_SUPPORT) || defined(ALDRIN2_DEV_SUPPORT) || defined(PIPE_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
    ...
    case MV_HWS_IPC_PORT_OPERATION_MSG:
        ...
        switch (portOperationMsg->portOperation) {
            ...
            case MV_HWS_IPC_PORT_OPERATION_AN_DISABLE:
                mvPortCtrlLogAdd(AN_PORT_MNG_LOG_DEBUG(msgParams->phyPortNum, 0, portOperationMsg->portData[0], portOperationMsg->portData[1]));

                if ( (GT_BOOL)portOperationMsg->portData[1] ) /* allPortsInChip*/
                {
                    if ( (GT_BOOL)portOperationMsg->portData[0] )
                    {
                        mvPortCtrlApStopDebugFlag |= (AP_DEBUG_ALL_PORTS_MASK );
                    }
                    else
                    {
                        mvPortCtrlApStopDebugFlag &= ~(AP_DEBUG_ALL_PORTS_MASK );
                ...
                {
#ifdef AC5_DEV_SUPPORT
                    i = msgParams->phyPortNum;
#else
                    i = portIndex;
#endif
                    if ( (GT_BOOL)portOperationMsg->portData[0] )
                    {
                        mvPortCtrlApStopDebugFlag |= (1<<i);
                    }
                    else
                    {
                        mvPortCtrlApStopDebugFlag &= ~(1<<i );
                ...
                break;
#endif
            case MV_HWS_IPC_PORT_OPERATION_DUMP:
                {
