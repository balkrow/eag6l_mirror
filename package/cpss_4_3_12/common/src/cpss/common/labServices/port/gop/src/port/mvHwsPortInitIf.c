...
const GT_U8 sdParamsOverridePermmittedSpeedIndexesSip6_10[][2] =
{/*  SerDes Speed     Index*/
...
#ifndef RAVEN_DEV_SUPPORT
    if ((HWS_DEV_SIP_6_CHECK_MAC(devNum)) && (!HWS_DEV_SIP_6_10_CHECK_MAC(devNum)))
#endif
    {
#ifndef AC5_DEV_SUPPORT
...
#ifndef AC5_DEV_SUPPORT
        for(sdSpeedIterator = 0 ;
           (sdParamsOverridePermmittedSpeedIndexesSip6_10[sdSpeedIterator][0] != sdSpeed) &&
                (sdParamsOverridePermmittedSpeedIndexesSip6_10[sdSpeedIterator][0] != LAST_MV_HWS_SERDES_SPEED);
            sdSpeedIterator++);
        return (sdParamsOverridePermmittedSpeedIndexesSip6_10[sdSpeedIterator][1]);
#endif
    }
    else
    {
...
)
{
    ...
    GT_BOOL                         multiLaneConfig = GT_FALSE;
    ...
    serdesConfig.refClockSource = (MV_HWS_REF_CLOCK_SRC)refClockSource;
    ...
    switch (hwsDeviceSpecInfo[devNum].devType)
    {
        ...
        case Ironman:
        case Harrier:
            multiLaneConfig = GT_TRUE;
            break;
        default:
            multiLaneConfig = GT_FALSE;
            break;
    }
    if (multiLaneConfig)
    {
...
)
{
    ...
    serdesConfig.refClockSource = (MV_HWS_REF_CLOCK_SRC)PRIMARY_LINE_SRC;
    serdesConfig.encoding       = SERDES_ENCODING_NA;
    /*serdesConfig.serdesType     = (MV_HWS_SERDES_TYPE)(HWS_DEV_SERDES_TYPE(devNum));*/
#ifndef RAVEN_DEV_SUPPORT
...
#if !defined(RAVEN_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
