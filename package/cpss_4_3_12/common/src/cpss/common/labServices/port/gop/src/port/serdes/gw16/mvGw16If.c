...
GT_STATUS mvHwsGW16SerdesOverridesApply
(
    ...
    for(d2dSubLaneNum = 0; d2dSubLaneNum < 5; d2dSubLaneNum++)
    {
        ...
        if((regValue & 0x1) == 0x1)
        {
            ...
            for(byteCount = 0; byteCount < 3; byteCount++)
            {
                ...
                status->errorsCntr.l[0] += regValue;
            }
...
)
{
    ...
    for(devIndex = 0; devIndex < PRV_HWS_NUMBER_OF_RAVENS; devIndex++)
    {
