...
#define BASE_DFX_SERVER_IRONMAN_UNIT    0x84000000
    ...
    switch (HWS_DEV_SILICON_TYPE(devNum))
    {
        case Aldrin2:
            ...
            *avsVol = ((*avsVol) * 10) + 640;
            break;
        case AC5:
            /* Read the AVS Control register and configure bits [31:29] to 0x1 */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum, BASE_DFX_SERVER_AC5_UNIT + DFX_SERVER_AVS_CONTROL, &avsCtrl));
            avsCtrl = (avsCtrl & 0x1FFFFFFF);
            avsCtrl = (avsCtrl | 0x20000000);
            CHECK_STATUS(hwsServerRegSetFuncPtr(devNum, BASE_DFX_SERVER_AC5_UNIT + DFX_SERVER_AVS_CONTROL, avsCtrl));
            /* Read the AVS Voltage from AVS Status register bits [9:0] */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum,BASE_DFX_SERVER_AC5_UNIT + DFX_SERVER_AVS_STATUS, avsVol));
            *avsVol = (*avsVol & 0x3ff);
            *avsVol = ((*avsVol) * 1198) /1024;
            break;
        case AC5X:
            /* Read the AVS Control register and configure bits [31:29] to 0x1 */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum, BASE_DFX_SERVER_AC5X_UNIT + DFX_SERVER_AVS_CONTROL, &avsCtrl));
            avsCtrl = (avsCtrl & 0x1FFFFFFF);
            avsCtrl = (avsCtrl | 0x20000000);
            CHECK_STATUS(hwsServerRegSetFuncPtr(devNum, BASE_DFX_SERVER_AC5X_UNIT + DFX_SERVER_AVS_CONTROL, avsCtrl));
            /* Read the AVS Voltage from AVS Status register bits [9:0] */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum,BASE_DFX_SERVER_AC5X_UNIT + DFX_SERVER_AVS_STATUS, avsVol));
            *avsVol = (*avsVol & 0x3ff);
            *avsVol = ((*avsVol) * 1198) /1024;
            break;
        case AC5P:
            /* Read the AVS Control register and configure bits [31:29] to 0x1 */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum, BASE_DFX_SERVER_AC5P_UNIT + DFX_SERVER_AVS_CONTROL, &avsCtrl));
            avsCtrl = (avsCtrl & 0x1FFFFFFF);
            avsCtrl = (avsCtrl | 0x20000000);
            CHECK_STATUS(hwsServerRegSetFuncPtr(devNum, BASE_DFX_SERVER_AC5P_UNIT + DFX_SERVER_AVS_CONTROL, avsCtrl));
            /* Read the AVS Voltage from AVS Status register bits [9:0] */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum,BASE_DFX_SERVER_AC5P_UNIT + DFX_SERVER_AVS_STATUS, avsVol));
            *avsVol = (*avsVol & 0x3ff);
            *avsVol = ((*avsVol) * 1198) /1024;
            break;
        case Harrier:
            /* Read the AVS Control register and configure bits [31:29] to 0x1 */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum, BASE_DFX_SERVER_HARRIER_UNIT + DFX_SERVER_AVS_CONTROL, &avsCtrl));
            avsCtrl = (avsCtrl & 0x1FFFFFFF);
            avsCtrl = (avsCtrl | 0x20000000);
            CHECK_STATUS(hwsServerRegSetFuncPtr(devNum, BASE_DFX_SERVER_HARRIER_UNIT + DFX_SERVER_AVS_CONTROL, avsCtrl));
            /* Read the AVS Voltage from AVS Status register bits [10:1] */
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum,BASE_DFX_SERVER_HARRIER_UNIT + DFX_SERVER_AVS_5NM_STATUS, avsVol));
            *avsVol = (*avsVol) >> 1;
            *avsVol = (*avsVol & 0x3ff);
            ...
            if (version == 0)   /* Version A0 */
            {
                *avsVol = ((*avsVol) * 1222) /1024;
            }
            else    /* Other versions */
            {
                *avsVol = ((*avsVol) * 1236) /1024;
            }
            break;
        case Ironman:
            CHECK_STATUS(hwsServerRegGetFuncPtr(devNum,BASE_DFX_SERVER_IRONMAN_UNIT + DFX_SERVER_AVS_ENABLE_CONTROL, avsVol));
            *avsVol = (*avsVol) >> 3;
            *avsVol = (*avsVol & 0x3ff);
            *avsVol = ((*avsVol) * 1198) /1024;
            break;
...
)
{
    ...
    switch (HWS_DEV_SILICON_TYPE(devNum))
    {
        ...
        case AC5P:
            ...
            break;
        case Ironman:
            if ((vcore < 750) || (vcore > 900)) /* range is between 0.75V-0.9V */
            {
                return GT_OUT_OF_RANGE;
            }
            regAddr = DFX_SERVER_AVS_ENABLE_CONTROL + BASE_DFX_SERVER_IRONMAN_UNIT;
            avsTargetVal = (vcore * 1024) / 1198; /* converting from - VALUE = (VDD / 1.198 ) * 1024 */
            break;
        case Harrier:
            ...
            if (version == 0)   /* Version A0 */
            {
                avsTargetVal = (vcore * 1024) / 1222; /* converting from - VALUE = (VDD / 1.222 ) * 1024 */
            }
            else    /* Other versions */
            {
                avsTargetVal = (vcore * 1024) / 1236; /* converting from - VALUE = (VDD / 1.236 ) * 1024 */
            }
            break;
        default:
            return GT_NOT_APPLICABLE_DEVICE;
...
)
{
    ...
    if(HWS_DEV_SILICON_TYPE(devNum) == Falcon)
    {
        ...
        /*
            ...
            broadcast write operation to all tiles - so it is needed to
            use another function.
        */
        CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, 0, controlAddr, (2 << 29), (7 << 29)));
