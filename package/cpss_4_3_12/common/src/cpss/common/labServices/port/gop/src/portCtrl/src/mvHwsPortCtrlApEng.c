...
#if !defined(BC2_DEV_SUPPORT) && !defined(AC3_DEV_SUPPORT) && !defined(AC5_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
...
#ifdef IRONMAN_DEV_SUPPORT
extern GT_U8 refClockSourceDb[IRONMAN_PORTS_NUM];
#endif
...
/****************************************************************************************************************************************/
#endif /* #ifndef IRONMAN_DEV_SUPPORT */
/****************************************************************************************************************************************/
...
static GT_STATUS mvApPortDelete(GT_U8 portIndex)
{
    ...
    if (portSm->status != PORT_SM_DELETE_IN_PROGRESS)/*If port task is already in delete process skip those steps*/
    {
...
#if !defined(BC2_DEV_SUPPORT) && !defined(AC3_DEV_SUPPORT) && !defined(AC5_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT) && !defined(BC3_DEV_SUPPORT)
...
GT_STATUS mvApPortInit(GT_U8 portIndex)
{
    ...
    GT_BOOL sigDetect = GT_FALSE;
    MV_HWS_AUTO_TUNE_STATUS rxInitStatus;
    GT_U8 laneNum = AP_CTRL_LANE_GET(apSm->ifNum);


    if ( apSm->status == AP_PORT_SIGNAL_DETECT_RX_INIT_WAIT ) /*step18*/
    {
        ...
        if (mvHwsSerdesSignalDetectGet(apDevNum, apPortGroup, laneNum, COM_PHY_C12GP41P2V_SIP6_30, &sigDetect) != GT_OK)
        {
        ...
        else
        {
            ...
            {
                if (mvHwsSerdesRxInit(apDevNum, apPortGroup, laneNum, RX_INIT_BLOCKING, &rxInitStatus) != GT_OK )
                {
                    /*apSm->status = AP_PORT_SERDES_INIT_FAILURE;*/
                    AP_CTRL_SM_SET_STATUS(apSm->status, AP_PORT_SERDES_INIT_FAILURE);
                ...
                /*step19+20*/
                if ((mvHwsIronmanAnpPortAnTxRxClockGate(apDevNum, apSm->portNum, GT_FALSE, MV_HWS_ANP_RX_ONLY) != GT_OK) || (mvHwsIronmanAnpPortAnTxRxReset(apDevNum, apSm->portNum, GT_FALSE, MV_HWS_ANP_RX_ONLY) != GT_OK))
                {
                    /*apSm->status = AP_PORT_SERDES_INIT_FAILURE;*/
                    AP_CTRL_SM_SET_STATUS(apSm->status, AP_PORT_SERDES_INIT_FAILURE);
                ...
                /*skip AP_PORT_SM_TX_DISABLE_STATE;*/
    ...
    if(mvHwsIronmanAnpPortStart(0,apSm->portNum, AP_CTRL_ADV_ALL_GET(apSm->capability),apSm->options) != GT_OK)/*step1-13*/
    {
        /*apSm->status = AP_PORT_SERDES_INIT_FAILURE;*/
        AP_CTRL_SM_SET_STATUS(apSm->status, AP_PORT_SERDES_INIT_FAILURE);
    ...
    if (mvApSerdesPowerUp(laneNum, GT_TRUE, apSm->portNum, 0) != GT_OK) /*step14+15+RX_PLL_READY*/
    {
    ...
    /*step16,17*/
    if ((mvHwsIronmanAnpPortAnTxRxClockGate(apDevNum, apSm->portNum, GT_FALSE, MV_HWS_ANP_TX_ONLY) != GT_OK) || (mvHwsIronmanAnpPortAnTxRxReset(apDevNum, apSm->portNum, GT_FALSE, MV_HWS_ANP_TX_ONLY) != GT_OK))
    {
        /*apSm->status = AP_PORT_SERDES_INIT_FAILURE;*/
        AP_CTRL_SM_SET_STATUS(apSm->status, AP_PORT_SERDES_INIT_FAILURE);
    ...
    /*apSm->status = AP_PORT_SIGNAL_DETECT_RX_INIT_WAIT*/
    AP_CTRL_SM_SET_STATUS(apSm->status, AP_PORT_SIGNAL_DETECT_RX_INIT_WAIT);/*step18*/
    mvPortCtrlLogAdd(AP_PORT_SM_LOG(apSm->state, apSm->status, apSm->portNum, apSm->ARMSmStatus));
    mvPortCtrlThresholdSet(apIntrop->abilityDuration, &(apTimer->abilityThreshold));
...
static void mvApPortResolutionSuccess(GT_U8 portIndex, GT_U8 mode)
{
...
#if defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
    GT_U8 laneNum;
#endif
...
#elif defined(BC3_DEV_SUPPORT) || defined(ALDRIN2_DEV_SUPPORT) || defined(PIPE_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
...
#ifdef IRONMAN_DEV_SUPPORT
    laneNum = AP_CTRL_LANE_GET(apSm->ifNum);
    /* Set AN TX reset to Reset*/
    mvHwsIronmanAnpPortAnTxRxReset(0, apSm->portNum, GT_TRUE, MV_HWS_ANP_TX_ONLY);/*step23*/
    /* Set AN TX clock to gate */
    mvHwsIronmanAnpPortAnTxRxClockGate(0, apSm->portNum, GT_TRUE, MV_HWS_ANP_TX_ONLY);/*step24*/
     /* Set AN RX reset to Reset*/
    mvHwsIronmanAnpPortAnTxRxReset(0, apSm->portNum, GT_TRUE, MV_HWS_ANP_RX_ONLY);/*step25*/
    /* Set AN RX clock to gate */
    mvHwsIronmanAnpPortAnTxRxClockGate(0, apSm->portNum, GT_TRUE, MV_HWS_ANP_RX_ONLY);/*step26*/
    mvApSerdesPowerUp(laneNum, GT_FALSE, apSm->portNum, 0);/*step27*/
    /* Gate/un-gate sd_tx_clk back to COMPHY + Set AN/PCS mux to PCS*/
    mvHwsIronmanAnpPortAnPcsMuxSet(0, apSm->portNum, GT_FALSE);/*step28-30*/
#endif
    /*step31*/
    ...
    rc = mvPortCtrlApPortMsgSend(portIndex,
                                ...
                                (GT_U8)portMode,
                                (GT_U8)GT_FALSE,
#if defined(BC3_DEV_SUPPORT) || defined(ALDRIN_DEV_SUPPORT) || defined(ALDRIN2_DEV_SUPPORT) || defined(PIPE_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
                                (GT_U8)refClock,
                                (GT_U8)refClockSrc);
#else
                                (GT_U8)MHz_156,
...
static GT_STATUS mvApPortResolutionAP(GT_U8 portIndex)
{
...
#if defined(BC3_DEV_SUPPORT) || defined(ALDRIN2_DEV_SUPPORT)  || defined(ALDRIN_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
    MV_HWS_AP_SM_TIMER *apTimer = &(mvHwsPortCtrlApPortDetect.apPortSm[portIndex].timer);
#endif
    ...
    AP_ST_ARB_FSM_SET(apSm->ARMSmStatus, ST_AN_GOOD_CK);
    mvPortCtrlLogAdd(AP_PORT_SM_LOG(apSm->state, apSm->status, apSm->portNum, apSm->ARMSmStatus));
#if !defined(IRONMAN_DEV_SUPPORT) && !defined(AC5_DEV_SUPPORT)
...
#ifdef IRONMAN_DEV_SUPPORT
    if ( mvHwsIronmanAnpPortHCDGet(apDevNum, apSm->portNum, &(apSm->hcdStatus)) != GT_OK )
    {
        /*TBD*/
        found = GT_FALSE;
    }
    ...
    {
        found = AP_ST_HCD_FOUND_GET(apSm->hcdStatus);
    }
#else
    ...
    if (found)
    {
#ifndef IRONMAN_DEV_SUPPORT /*done in mvHwsIronmanAnpPortHCDGet*/
        /* AP HCD update */
        AP_ST_HCD_FOUND_SET(apSm->hcdStatus, 1);
    ...
    else /* NO Resolution */
    {
...
#endif
        /*apSm->state = AP_PORT_SM_TX_DISABLE_STATE;*/
        /* in Ironman disable state was not implemented since it is done in the ANP ip (HW) */
        AP_CTRL_SM_SET_STATE(apSm->state, AP_PORT_SM_TX_DISABLE_STATE);
#endif


        /* AP validate Ability failure case */
        /* TBD - ask Eyal which additional steps are needed - i added AN disable*/
...
#endif
#ifdef IRONMAN_DEV_SUPPORT
            mvHwsIronmanAnpPortAnSet(0/*dev*/,(GT_U32)(apSm->portNum),MV_HWS_ANP_AN_OPER_DISABLE);/*MB - i added it not in eyal doc - assume i return to init stae and start all over again so i set an disable */
#endif
            /*apSm->state = AP_PORT_SM_INIT_STATE;*/
            AP_CTRL_SM_SET_STATE(apSm->state, AP_PORT_SM_INIT_STATE);
        ...
        {
            mvHwsIronmanAnpPortAnSet(0/*dev*/,(GT_U32)(apSm->portNum),MV_HWS_ANP_AN_OPER_RESTART);/*step no HCD*/
            mvPortCtrlThresholdSet(apIntrop->abilityDuration, &(apTimer->abilityThreshold));
        }
#endif
    }
...
#else
    mvHwsIronmanAnpPortAnArbStateGet(0, apSm->portNum, &fsmState);/*step21*/
    if(fsmState == ST_AN_GOOD_CK)
    {
        AP_ST_ARB_FSM_SET(apSm->ARMSmStatus, fsmState);
        apTimer->abilityCount = 0;
    }
...
#ifdef IRONMAN_DEV_SUPPORT
    if (apSm->status == AP_PORT_INIT_SUCCESS)
#else
...
static GT_STATUS mvApPortLinkValidate(GT_U8 portIndex, GT_U16 linkCheckInterval, GT_U16 linkCheckDuration)
{
    ...
    */
#ifdef IRONMAN_DEV_SUPPORT
    mvHwsIronmanAnpPortAnArbStateGet(0, apSm->portNum, &anArbState); /*If ST_TX_DISABLE then link_fail_inhibit_timer expire - step37*/
    if((anArbState == ST_TX_DISABLE) || (*portInit & MV_PORT_CTRL_NUM(portIndex)))
#else
    time = mvPortCtrlCurrentTs() - apStats->timestampTime;
    limit = linkCheckInterval * linkCheckDuration;

    if ((time > limit) || (*portInit & MV_PORT_CTRL_NUM(portIndex)))
#endif
    {
#ifdef IRONMAN_DEV_SUPPORT
        /* reset PCS sd tx & rx - step 37.1*/
        mvHwsIronmanAnpPortPcsSdTxRxReset(0/*devNum*/, apSm->portNum, GT_TRUE);
        /* gate PCS sd tx & rx clocks - step 37.2 */
        mvHwsIronmanAnpPortPcsSdTxRxClockGate(0/*devNum*/, apSm->portNum, GT_TRUE);
#endif
        /* Link check failure */
...
GT_STATUS mvApPortLinkUp(GT_U8 portIndex)
{
    ...
    if ((apPortMode == Port_1000Base_KX) || (apPortMode == Port_10GBase_KX4) || (apPortMode == Port_2500Base_KX))
    {
...
#endif
#ifdef IRONMAN_DEV_SUPPORT
                /*overwrite link_status value in ANP to 0x1*/
                mvHwsIronmanAnpPortANPLinkStatusSet(0,apSm->portNum, GT_TRUE);/*step36*/
#endif
...
#if (defined(BC3_DEV_SUPPORT))
            if (mvPortCtrlCurrentTs() - apStats->timestampTime > 500)
            {
                apStats->timestampTime = mvPortCtrlCurrentTs();
                if (_1000Base_X != portMode)
                {
                    mvHwsPortAvagoDfeCfgSet(apDevNum,apPortGroup,apSm->portNum,portMode,mvPortCtrlLinkUpDfeMode);
                }
            }
#endif
            /* Link check timer restart */
...
#endif
#ifdef IRONMAN_DEV_SUPPORT
                /*overwrite link_status value in ANP to 0x0*/
                mvHwsIronmanAnpPortANPLinkStatusSet(0,apSm->portNum, GT_FALSE);/*step38*/
#endif
                /* Link failure */
                mvApPortLinkFailure(portIndex);
                /*Doorbell interrupt*/
...
/*if ((portIndex > 14) && (portIndex < 24)){
...
#ifdef IRONMAN_DEV_SUPPORT
            mvHwsIronmanAnpPortStopAn(0,apSm->portNum); /*TBD - dee if needed also AP_PORT_SM_DELETE_STATE. added not as part of eyal seq but as follow the use of mvHwsAnpPortStopAn */
#endif
    ...
    /* if the function "mvApPortDelete" fail we don't change the status and try to delete it
       again in function "mvApPortDeleteValidate" */
    /*In case we succussed in sending message to port task but serdes power down failed - re send delete and try again to power down the serdes*/
    else if (((apSm->state == AP_PORT_SM_DELETE_STATE) || (apSm->state ==AP_PORT_SM_DISABLE_STATE)) &&
            ((apSm->status != AP_PORT_DELETE_IN_PROGRESS) || (portSm->status == PORT_SM_DELETE_FAILURE)))
    {
        if (mvApPortDelete(portIndex) != GT_OK)
        {
            return GT_OK;
