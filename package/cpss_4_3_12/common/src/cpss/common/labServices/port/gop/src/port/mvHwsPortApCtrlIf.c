...
)
{
    ...
    if((HWS_DEV_SIP_6_10_CHECK_MAC(devNum)) && (!HWS_DEV_SIP_6_30_CHECK_MAC(devNum)))
    {
    ...
    if (rcode == GT_OK)
    {
        if ( !(apStatus->hcdResult.hcdMisc &0x1)) /*check if OPTICAL_MODE*/
        {
            ...
            case Port_1000Base_KX:
                ...
                break;
            case Port_2500Base_KX:
                apStatus->postApPortMode = _2500Base_X;
                break;
...
/*Override IPC Issue*/
    ...
    if ((HWS_DEV_SIP_6_10_CHECK_MAC(devNum)) && (!HWS_DEV_SIP_6_30_CHECK_MAC(devNum)))
    {
        CHECK_STATUS(mvHwsPortAnpSerdesRxConfigSet(devNum, portGroup, phyPortNum, serdesSpeed, serdesNumber, serdesType, rxConfigPtr));
    }
    else
#endif
    {
...
/*Override IPC Issue*/
    ...
    if ((HWS_DEV_SIP_6_10_CHECK_MAC(devNum)) && (!HWS_DEV_SIP_6_30_CHECK_MAC(devNum)))
    {
        CHECK_STATUS(mvHwsPortAnpSerdesTxConfigSet(devNum, portGroup, phyPortNum, serdesSpeed, serdesNumber, serdesType, txConfigPtr));
    }
    else
#endif
    {
...
/**
* @internal mvHwsApPortCtrlDebugModeSet function
* @endinternal
*
* @brief   Set AP machine (CM3/ANP) in debug mode (stop/start AP
*          machine)
*
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] phyPortNum               - physical port number
* @param[in] enable                   - enable/disable debug
*                                       mode
* @param[in] allPorts                 - disable all port in chip
*
* @retval 0                        - on success
* @retval 1                        - on error
*/
GT_STATUS mvHwsApPortCtrlDebugModeSet
(
    GT_U8                 devNum,
    GT_U32                portGroup,
    GT_U32                phyPortNum,
...
)
{
    ...
    if((HWS_DEV_SIP_6_10_CHECK_MAC(devNum)) && (!HWS_DEV_SIP_6_30_CHECK_MAC(devNum)))
    {
        CHECK_STATUS(mvHwsAnpPortDebugModeSet(devNum, phyPortNum, enable, allPorts));
    }
    else
#endif
    {
        CHECK_STATUS(mvHwsPortApSerdesAnDisableSetIpc(devNum, portGroup, phyPortNum, enable, allPorts));
    }
    return GT_OK;
}
