...
********************************************************************/
#ifndef MCESD_C12GP41P2V_API_SIP6_30_H
#define MCESD_C12GP41P2V_API_SIP6_30_H
...
@note Speeds that require specific programming sequences: 11.5625G, 10G, 2.578125G, 12.375G
@note For speeds that require specific programming sequences, call SetRefFreq() then call SetTxRxBitRate() before power on.
@note Only reference frequency of 25MHz is supported for 10G, 2.578125G, and 12.375G.
...
@param[out] widthLeft - left EYE width
@param[out] widthRight - right EYE width
...
/**
@brief  Converts width and height code to ps and mv
...
@param[out] heightmVUpper - upper height converted to mV
@param[out] heightmVLower - lower height converted to mV
...
@param[in]      minSamples - minimum number of bits to sample
@param[in]      berThreshold - Bit Error Rate Threshold in nano (factor of 1E-9)
@param[in,out]  eyeRawDataPtr - pointer to S_C12GP41P2V_EYE_RAW_PTR which store eye raw data
...
@note Call EOMGetEyeData() before to populate eyeRawDataPtr
...
@note See EOMGetWidthHeight() for example of berThreshold
...
/**
@brief  Retrieves ring oscillator frequency

@param[in]  devPtr - pointer to MCESD_DEV initialized by mcesdLoadDriver() call

@param[out] freq - Calculated freq
...
/**
@brief  Enable/Disable CDR Freeze state on a specific lane

@param[in]  devPtr - pointer to MCESD_DEV initialized by mcesdLoadDriver() call
@param[in]  state - MCESD_TRUE to freeze CDR on the specified lane
...
/**
@brief  Get CDR Freeze state on the specified lane

@param[in]  devPtr - pointer to MCESD_DEV initialized by mcesdLoadDriver() call

@param[out] state - MCESD_TRUE indicates CDR is frozen on the specified lane
