...
GT_STATUS mvPortCtrlOsResourcesInit(void)
{
...
#if !defined (FALCON_DEV_SUPPORT) && !defined (AC5P_DEV_SUPPORT) && !defined (HARRIER_DEV_SUPPORT)
    ...
    if (xTaskCreate(mvHwsAnDetectRoutine,
                (const char * const) "AP 0-7 Detect",
                MV_AP_STACK,
                ( void * ) &(anTaskNum[0]),
                MV_AP_PRIORITY,
                &mvHwsAnDetectHandler) != pdPASS)
    {
     ...
     /*(M9)*/
    if (xTaskCreate(mvHwsAnDetectRoutine,
                (const char * const) "AP2 8-15 Detect",
                MV_AP_STACK,
                ( void * ) &(anTaskNum[1]),
                MV_AP_PRIORITY,
    ...
    if (xTaskCreate(mvHwsPortMtcRoutine,
                (const char * const) "Port Maintanance",
