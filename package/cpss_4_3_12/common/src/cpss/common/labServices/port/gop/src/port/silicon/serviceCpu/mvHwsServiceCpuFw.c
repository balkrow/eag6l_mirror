...
#define PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_SET(_var,_val) \
   PRV_SHARED_GLOBAL_VAR_GET(commonMod.labServicesDir.mvHwsServiceCpuFwSrc._var)=_val

#define PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_GET(_var) \
   PRV_SHARED_GLOBAL_VAR_GET(commonMod.labServicesDir.mvHwsServiceCpuFwSrc._var)

#define IPC_MSG_SIZE 0x50
...
/*#ifndef ASIC_SIMULATION*/
 PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_SET(mvHwsServiceCpuEnabled[devNum], enable);
...
{
    return PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_GET(mvHwsServiceCpuEnabled)[devNum];
}
...
{
    devNum = devNum;
    PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_SET(mvHwsServiceCpuDbaEnabled, enable);
...
{
    devNum = devNum;
    return PRV_SHARED_COMMON_DIR_SERVICE_CPU_FW_SRC_GLOBAL_VAR_GET(mvHwsServiceCpuDbaEnabled);
}
...
*                       ...
*                       0xffffffff - MSYS
* @param[in] fwID     - unique fw ID
* @param[in] fwName     - firmware identifier string
*                       "bc3DbaFw", "mvHwsServiceCpuCm3Ac5Fw", ...
* @returns              GT_STATUS
...
)
{
    ...
    if (fwChannel == 0)
    {
        PRV_CPSS_GENERIC_SRV_CPU_IPC_CHANNEL_CONFIG_STC ipcCfg[] = {
            { 0,   0,  0,               4,  IPC_MSG_SIZE },
            { 1,   0,  0,               4,  IPC_MSG_SIZE },
            { 2,   4,  IPC_MSG_SIZE,    0,  0 },
            { 0xff,0,  0,               0,  0 }
        };

        /*Check that transfer IPC msg size will not exceed limitation*/
        maxMsgSize = (IPC_MSG_SIZE+3)&(~3);/*ipc channel msg size align to 4*/
        msgSize = sizeof(MV_HWS_IPC_CTRL_MSG_STRUCT);/*data struct that contain union of all define IPC msg*/
        if (msgSize > maxMsgSize)
        {
            hwsOsPrintf("msg size (%d) exceed IPC max msg size (%d)\n",msgSize ,maxMsgSize);
            return GT_FAIL;
        }
#ifndef ASIC_SIMULATION
        /* Is fw is not the default */
        if ( fwId && (GT_OK != cpssSrvCpuSetOptionalFW(devNum, scpuId, fwId, fwName)) )
            return GT_FAIL;
#endif
        rc = prvCpssGenericSrvCpuInit(devNum, scpuId, ipcCfg, prvMvHwsPreStartAP, (GT_UINTPTR)smemData, &fwChannel);
