...
)
{
    ...
    /* for tuning operation, iterate list and execute for false value only */
    if (operation == MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_CDR_E)
    {
        for (i = 0; i < curPortParams.numOfActLanes; i++)
        {
            if (dataPtr[i] == 0) /* CDR not locked */
            {
                CHECK_STATUS(mvHwsSerdesOperation(devNum, portGroup, curLanesList[i], portMode, operation, dataPtr, resultPtr));
            }
        }
        return GT_OK;
    }
...
{
    GT_U32 regVal;
    GT_U32 clockInKhz = coreClk * 1000;

    if ((portMode == SGMII) || (portMode == SGMII2_5))
    {
        regVal = 2 * clockInKhz;
    }
    else if (HWS_1000BASE_MODE_CHECK(portMode) || (portMode == _2500Base_X))
    {
        regVal = (GT_U32)(1.5 * clockInKhz);
    }
    else
    {
...
)
{
    ...
    if ((HWS_2500_MODE_CHECK(portMode)) || (portMode == SGMII) || (HWS_1000BASE_MODE_CHECK(portMode)))
    {
...
CPSS_TBD_BOOKMARK_PHOENIX
      if (HWS_DEV_SIP_6_10_CHECK_MAC(devNum) || (HWS_DEV_SILICON_TYPE(devNum) == AAS))
      {
         return GT_OK;
      }
#endif
...
#else
    CHECK_STATUS(mvHwsSerdesClampingValuesResetAndStatusGet(devNum, portGroup,(curLanesList[0] & 0xFFFF), HWS_DEV_SERDES_TYPE(devNum, (curLanesList[0] & 0xFFFF)), clampingStatusPtr));
#endif
...
)
{
   ...
   if (!(HWS_DEV_SIP_6_10_CHECK_MAC(devNum) || (AC5 == hwsDeviceSpecInfo[devNum].devType) || (HWS_DEV_SILICON_TYPE(devNum) == AAS)))
   {
        hwsDeviceSpecGetFuncPtr(&hwsDevFunc);
...
GT_STATUS mvHwsPortApGetMaxLanesPortModeGet(GT_U32 capability, MV_HWS_PORT_STANDARD *portModePtr)
{
                           ...
                           (1<<AP_CTRL_40GBase_KR2_SHIFT)))
    {
        ...
        else if (AP_CTRL_40GBase_KR2_GET(capability))
        {
            *portModePtr = _40GBase_KR2;
        }
        else
        {
...
)
{
    ...
    if(((HWS_DEV_SILICON_TYPE(devNum) != Falcon) && (HWS_DEV_SILICON_TYPE(devNum) != Raven)) || mvHwsMtipIsReducedPort(devNum,phyPortNum))
    {
        return GT_NOT_SUPPORTED;
    }

    /* Convert phyPortNum to point at the base port number in group */
    phyPortNum = (phyPortNum / 8) * 8;
...
#if ((defined (CHX_FAMILY) && (!defined(MV_HWS_REDUCED_BUILD_EXT_CM3)))) || defined (AC5P_DEV_SUPPORT) || defined(HARRIER_DEV_SUPPORT)
/*************************************************************************
...
)
{
    ...
    if((HWS_DEV_SILICON_TYPE(devNum) == AC5X) && ((phyPortNum >= 50) && (phyPortNum <= 53)))
    {
        phyPortNum = 50;
        size = 4;
    }
    ...
    {
        phyPortNum =  (phyPortNum / MV_HWS_AC5P_GOP_PORT_NUM_CNS) * MV_HWS_AC5P_GOP_PORT_NUM_CNS;
        size = 8;
    }
    else if (HWS_DEV_SILICON_TYPE(devNum) == Harrier)
    {
        if(phyPortNum < 32)
        {
            phyPortNum = (phyPortNum / MV_HWS_HARRIER_GOP_PORT_NUM_CNS) * MV_HWS_HARRIER_GOP_PORT_NUM_CNS;
            size = 8;
        }
        else
        {
...
)
{
    ...
    case Raven:
        ...
        break;
#endif
#if (!defined(MV_HWS_REDUCED_BUILD_EXT_CM3)) || defined (AC5P_DEV_SUPPORT) || defined(HARRIER_DEV_SUPPORT)
    case AC5X:
    case AC5P:
    case Harrier:
...
)
{
   ...
   for (i = 0; i < curPortParams.numOfActLanes; i++)
   {
       CHECK_STATUS(mvHwsSerdesSigDetectMask(devNum, (portGroup + ((curLanesList[i] >> 16) & 0xFF)),
                                            (curLanesList[i] & 0xFFFF), HWS_DEV_SERDES_TYPE(devNum, (curLanesList[i] & 0xFFFF)), maskEnable));
   }
