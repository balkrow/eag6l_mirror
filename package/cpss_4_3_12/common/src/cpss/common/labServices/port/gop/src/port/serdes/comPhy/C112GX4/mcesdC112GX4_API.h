...
*
*@note If preprocessor directive C112GX4_DFE_MILLIVOLTS is defined, then tapValue is returned in milli-volts.
...
@param[out] txSpeed - enum that represents the TX bitrate
@param[out] rxSpeed - enum that represents the RX bitrate

@note Gets bitrate using pin readout fields (PIN_PHY_GEN_TX_RD, PIN_PHY_GEN_RX_RD)
...
@param[out] heightuV - height converted to uV

@note Converts width to mUI by code * 1000 / maxUI
@note Converts height to uV by look using DFE tables and upper and lower height code values
@note Height is in uV to preserve precision because values in DFE tables are in mV with three significant figures
