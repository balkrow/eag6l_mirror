...
)
{
    ...
    if ((hwsDeviceSpecInfo[devNum].devType != Ironman) && (mvHwsMtipIsReducedPort(devNum, phyPortNum)))
    {
        localApCfg->apLaneNum = 16;
    }
    hwsOsMemSetFuncPtr(&apCfgIpc, 0, sizeof(MV_HWS_IPC_PORT_AP_DATA_STRUCT));

    apCfgIpc.portGroup          = (GT_U16)portGroup;
    apCfgIpc.phyPortNum         = (GT_U16)phyPortNum;
    apCfgIpc.advMode            = localApCfg->modesVector;

    AP_CTRL_FC_PAUSE_SET(apCfgIpc.options, (GT_U32)(localApCfg->fcPause  & AP_CTRL_FC_PAUSE_MASK ));
    AP_CTRL_FC_ASM_SET(apCfgIpc.options, (GT_U32)(localApCfg->fcAsmDir & AP_CTRL_FC_ASM_MASK ));
    AP_CTRL_FEC_ABIL_SET(apCfgIpc.options, (GT_U32)(localApCfg->fecSup & AP_CTRL_FEC_ABIL_MASK ));
    AP_CTRL_FEC_REQ_SET(apCfgIpc.options, (GT_U32)(localApCfg->fecReq & AP_CTRL_FEC_REQ_MASK  ));
    AP_CTRL_LB_EN_SET(apCfgIpc.options, (GT_U32)(localApCfg->nonceDis & AP_CTRL_LB_EN_MASK ));
    AP_CTRL_CTLE_BIAS_VAL_SET(apCfgIpc.options, (GT_U32)(localApCfg->ctleBiasValue & AP_CTRL_CTLE_BIAS_VAL_MASK));
    AP_CTRL_SPECIAL_SPEED_SET(apCfgIpc.options, (GT_U32)(localApCfg->specialSpeeds & AP_CTRL_20G_R1_MASK ));
    AN_CTRL_SKIP_RES_SET(apCfgIpc.options, (GT_U32)(localApCfg->skipRes & AN_CTRL_SKIP_RES_MASK ));
    AN_CTRL_PRECODING_SET(apCfgIpc.options, (GT_U32)(localApCfg->extraOperation & MV_HWS_PORT_SERDES_OPERATION_PRECODING_E));
    AN_CTRL_RX_TRAIN_SET(apCfgIpc.options, (GT_U32)((localApCfg->extraOperation & MV_HWS_PORT_SERDES_OPERATION_RX_TRAIN_E)>> 5));
    AN_CTRL_IS_OPTICAL_MODE_SET(apCfgIpc.options, (GT_U32)((localApCfg->extraOperation & MV_HWS_PORT_SERDES_OPERATION_IS_FIBER_E)>> 6));
    AN_CTRL_IS_EXT_SIGDET_CB_EN_SET(apCfgIpc.options, (GT_U32)(localApCfg->extSigDetCallbackEnable & AN_CTRL_IS_EXT_SIGDET_CB_EN_MASK));
...
)
{
    ...
    hwsOsMemCopyFuncPtr(&portSerdesTxConfig.txCfg, txConfigPtr, sizeof(MV_HWS_SERDES_TX_OVERRIDE_CONFIG_DATA_UNT));
...
)
{
    ...
    hwsOsMemCopyFuncPtr(&portSerdesRxConfig.rxCfg, rxConfigPtr, sizeof(MV_HWS_SERDES_RX_OVERRIDE_CONFIG_DATA_UNT));
...
/**
* @internal mvHwsPortOverFwExtSigDetIpc function
* @endinternal
*
* @brief   Set external signal detect current port state
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] portNum                  - physical port number
* @param[in] sigDetCurrentStatus      - signal detect current status
*
* @retval 0                        - on success
* @retval 1                        - on error
*/
GT_STATUS mvHwsPortOverFwExtSigDetIpc
(
    GT_U8       devNum,
    GT_U32      portGroup,
    GT_U32      portNum,
    GT_BOOL     sigDetCurrentStatus
)
{
    GT_U32 portData[MV_HWS_IPC_PORT_OPERATION_DATA_LEN_CNS];
    GT_STATUS rc;

    portData[0]= sigDetCurrentStatus;
    /* instead of creating a new ipc, we will use OperationIpc */
    rc = mvHwsPortOperationIpc(devNum, portGroup,
                               portNum,
                               MV_HWS_IPC_PORT_OPERATION_EXT_SIG_DET_SET,
                               portData);
    return rc;
}
