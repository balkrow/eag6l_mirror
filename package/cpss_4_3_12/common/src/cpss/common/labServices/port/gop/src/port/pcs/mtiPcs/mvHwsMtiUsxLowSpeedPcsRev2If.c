...
)
{
    ...
    switch(portMode)
    {
        case HWS_1000BASE_MODE_CASE:   /* single */
        case SGMII2_5:
        case SGMII:
        case _2500Base_X:
...
)
{
    ...
    if(POWER_DOWN == action)
    {
        ...
        {
            CHECK_STATUS(mvHwsGlobalMacToLocalIndexConvert(devNum, portMacNum, portMode, &convertIdx));

            CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, portMacNum, MTI_USX_LPCS_UNIT, LPCS_UNITS_GMODE_LPCS_ENABLE_E, &fieldData, NULL));
            if((convertIdx.ciderIndexInUnit % 8) / 4)
            {
                fieldData = fieldData & 0x0F;   /* disable QSGMII_1 */
                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, MTI_USX_LPCS_UNIT, LPCS_UNITS_GMODE_LPCS_ENABLE_E, fieldData, NULL));

                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, MTI_USX_LPCS_UNIT, LPCS_UNITS_GMODE_QSGMII_1_ENABLE_E, 0, NULL));
            }
            else
            {
                fieldData = fieldData & 0xF0;   /* disable QSGMII_0 */
                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, MTI_USX_LPCS_UNIT, LPCS_UNITS_GMODE_LPCS_ENABLE_E, fieldData, NULL));

                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, MTI_USX_LPCS_UNIT, LPCS_UNITS_GMODE_QSGMII_0_ENABLE_E, 0, NULL));
            }
...
* @retval 1                        - on error
*/
static GT_STATUS mvHwsMtiUsxLowSpeedPcsRxReset
(
    GT_U8                   devNum,
    GT_U32                  portGroup,
    GT_U32                  portMacNum,
...
/**
 * @internal mvHwsMtiUsxLowSpeedPcsRxResetGet function
 * @endinternal
 *
 ...
 * @return GT_STATUS
 */
static GT_STATUS mvHwsMtiUsxLowSpeedPcsRxResetGet
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  portGroup,
...
GT_STATUS mvHwsMtiUsxLowSpeedPcsRev2IfInit(MV_HWS_PCS_FUNC_PTRS **funcPtrArray)
{
    ...
    funcPtrArray[MTI_USX_PCS_LOW_SPEED]->pcsRxResetFunc             = mvHwsMtiUsxLowSpeedPcsRxReset;
    funcPtrArray[MTI_USX_PCS_LOW_SPEED]->pcsRxResetGetFunc          = mvHwsMtiUsxLowSpeedPcsRxResetGet;
