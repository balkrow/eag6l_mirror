...
********************************************************************************
*/
/*MB - lots of AVAGO function that Ironman not needed and can save space*/
...
#if ((!defined MV_HWS_REDUCED_BUILD) || defined(AC5P_DEV_SUPPORT) || defined(HARRIER_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT))
...
* @retval 1                        - on error
*/
#if ((!defined MV_HWS_REDUCED_BUILD) || defined(RAVEN_DEV_SUPPORT) || defined(FALCON_DEV_SUPPORT) || defined(AC5P_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT))
...
)
{
    ...
    {
        if(((phyPortNum  == MV_HWS_AAS_GOP_PORT_1TILE_CPU_0_CNS) && (hwsDeviceSpecInfo[devNum].numOfTiles == 1)) ||
           ((phyPortNum  == MV_HWS_AAS_GOP_PORT_2TILE_CPU_0_CNS || phyPortNum  == MV_HWS_AAS_GOP_PORT_2TILE_CPU_1_CNS) && (hwsDeviceSpecInfo[devNum].numOfTiles == 2)))
        {
            isCpu = GT_TRUE;
        }
        else if(phyPortNum % MV_HWS_AAS_GOP_PORT_NUM_CNS >= 16)
        {
            isCpu = GT_TRUE;
        }
    }
...
)
{
    ...
    for (i = 0; i < curPortParams.numOfActLanes; i++)
    {
        ...
        dfeCfg = ((HWS_DEV_SERDES_TYPE(devNum, MV_HWS_SERDES_NUM(curLanesList[i])) == AVAGO_16NM) ||
                  (HWS_DEV_SERDES_TYPE(devNum, MV_HWS_SERDES_NUM(curLanesList[i])) == AVAGO)) ? &portStandard : NULL;
...
)
{
    ...
    switch (portTuningMode)
    {
        ...
        case TRxTuneStart:
            for (i = 0 ; i < curPortParams.numOfActLanes ; i++)
            {
                CHECK_STATUS(mvHwsSerdesTxAutoTuneStart(devNum, portGroup, curPortParams.activeLanesList[i], HWS_DEV_SERDES_TYPE(devNum,curPortParams.activeLanesList[i]),GT_TRUE));
            }
            break;
        case TRxTuneStop:
            for (i = 0 ; i < curPortParams.numOfActLanes ; i++)
            {
                CHECK_STATUS(mvHwsSerdesTxAutoTuneStart(devNum, portGroup, curPortParams.activeLanesList[i], HWS_DEV_SERDES_TYPE(devNum,curPortParams.activeLanesList[i]),GT_FALSE));
            }
            break;
        case TRxTuneStatusNonBlocking:
            ...
            tuneRes = (MV_HWS_AUTO_TUNE_STATUS_RES*)results;
            CHECK_STATUS(mvHwsPortAutoTuneStateCheck(devNum, portGroup, phyPortNum, portMode, NULL, &tuneRes->txTune));
            break;
        default:
            HWS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, LOG_ARG_STRING_MAC("training mode not supported"));
    }
...
)
{
    ...
    GT_U32  fieldName1, fieldName2, fieldName3, fieldName4, fieldName5;
    ...
    if (HWS_DEV_SIP_6_10_CHECK_MAC(devNum))
    {
        ...
        /*RSFEC_SYMBLERR*/
        /****************/
        if ((MTI_USX_RSFEC_UNIT == unitId) &&
            ((curPortParams.serdesSpeed == _20_625G) || (curPortParams.serdesSpeed == _25_78125G)))
        {
                /* For the USX_IP it's 4 fec lanes over 1 physical lane.
                   Both AC5X and AC5P, both for 25G RS-FEC and any 20G USX with RS-FEC (O/Q/D).*/
                iterNum = 4;
                startIndex = 0;
                nrz_R8_Factor = 1;
        }
...
/**
* @internal mvHwsFcfecStatusGet function
* @endinternal
*
* @brief   Return FC-FEC status.
*
...
* @param[in] portNum                  - physical port number
*
* @param[out] fcfecStatusPtr          - pointer to struct that
*                                       will contain the FC-FEC status
*
* @retval GT_OK                    - on success
* @retval GT_FAIL                  - on error
* @retval GT_BAD_PARAM             - wrong devNum or port
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS mvHwsFcfecStatusGet
(
    IN  GT_U8                      devNum,
    IN  GT_U32                     portGroup,
    IN  GT_U32                     portNum,
    OUT MV_HWS_FCFEC_STATUS_STC    *fcfecStatusPtr
)
{
    MV_HWS_PORT_INIT_PARAMS   curPortParams;
    GT_U32                    regData;
    ...
    if(fcfecStatusPtr == NULL)
    {
        HWS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PTR, LOG_ARG_STRING_MAC("fcfecStatusPtr is null"));
    }
    ...
    if(HWS_DEV_SILICON_TYPE(devNum) == Ironman)
    {
        fieldName = IRONMAN_PCS_FCFEC_FC_FEC_STATUS_FC_LOCKED_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_10G_PCS_FCFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));

        if (regData == 0x1)
        {
            fcfecStatusPtr->fcLocked = GT_TRUE;
        }
        else
        {
            fcfecStatusPtr->fcLocked = GT_FALSE;
        }
    }
    else
    {
...
)
{
    ...
    /* CPU ports of Ironman have the same register database structure
       as regular port, so there is no need for special treatment */
    if((mvHwsMtipIsReducedPort(devNum,portNum) == GT_TRUE) && (hwsDeviceSpecInfo[devNum].devType != Ironman))
    {
        fieldName = CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_VL0_CCW_LO_VL0_CCW_LO_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_CPU_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
        fieldName = CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_COUNTER_HI_VENDOR_COUNTER_HI_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_CPU_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
        fcfecCountersPtr->blocksCorrectedError[0] = regData | (regDataHi << 16);
        fcfecCountersPtr->numReceivedBlocksCorrectedError += regData | (regDataHi << 16);

        fieldName = CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_VL0_NCCW_LO_VL0_NCCW_LO_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_CPU_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
        fieldName = CPU_PCS_RSFEC_UNITS_RSFEC_VENDOR_COUNTER_HI_VENDOR_COUNTER_HI_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_CPU_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
        fcfecCountersPtr->blocksUncorrectedError[0] = regData | (regDataHi << 16);
        fcfecCountersPtr->numReceivedBlocksUncorrectedError += regData | (regDataHi << 16);
    ...
    if (HWS_DEV_SILICON_TYPE(devNum) == Harrier || HWS_DEV_SILICON_TYPE(devNum) == AC5P)
    {
        ...
        for (laneIdx = 0; laneIdx < curPortParams.numOfActLanes; laneIdx++)
        {
            ...
            fieldName = RSFEC_UNIT_VL0_CCW_LO_P0_CORRECTED_FEC_CODEWORDS_0_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
            fieldName = RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));

            fcfecCountersPtr->blocksCorrectedError[laneIdx] = regData | (regDataHi << 16);
            fcfecCountersPtr->numReceivedBlocksCorrectedError += regData | (regDataHi << 16);

            if (curPortParams.numOfActLanes == 2)
            {
                fieldName = RSFEC_UNIT_VL1_CCW_LO_P0_CORRECTED_FEC_CODEWORDS_1_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                fieldName = RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
            ...
            fieldName = RSFEC_UNIT_VL0_NCCW_LO_P0_UNCORRECTED_FEC_CODEWORDS_0_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
            fieldName = RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));

            fcfecCountersPtr->blocksUncorrectedError[laneIdx] = regData | (regDataHi << 16);
            fcfecCountersPtr->numReceivedBlocksUncorrectedError += regData | (regDataHi << 16);

            if (curPortParams.numOfActLanes == 2)
            {
                fieldName = RSFEC_UNIT_VL1_NCCW_LO_P0_UNCORRECTED_FEC_CODEWORDS_1_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                fieldName = RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+nextPortShift), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
    ...
    else if(HWS_DEV_SILICON_TYPE(devNum) == Ironman)
    {
        fieldName = IRONMAN_PCS_FCFEC_FC_FEC_CCW_LO_COUNTER_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_10G_PCS_FCFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
        fieldName = IRONMAN_PCS_FCFEC_FC_FEC_COUNTER_HI_COUNTER_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_10G_PCS_FCFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
        fcfecCountersPtr->blocksCorrectedError[0] = regData | (regDataHi << 16);
        fcfecCountersPtr->numReceivedBlocksCorrectedError += regData | (regDataHi << 16);

        fieldName = IRONMAN_PCS_FCFEC_FC_FEC_NCCW_LO_COUNTER_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_10G_PCS_FCFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
        fieldName = IRONMAN_PCS_FCFEC_FC_FEC_COUNTER_HI_COUNTER_E;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_10G_PCS_FCFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
        fcfecCountersPtr->blocksUncorrectedError[0] = regData | (regDataHi << 16);
    ...
    else if(HWS_DEV_SILICON_TYPE(devNum) == AC5X)
    {
        if (mvHwsUsxModeCheck(devNum, portNum, curPortParams.portStandard) == GT_TRUE)
        {
            fieldName = USX_RSFEC_UNITS_BASE_R_FEC_CCW_LO_FEC_CCW_LO_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_USX_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
            fieldName = USX_RSFEC_UNITS_BASE_R_FEC_CW_HI_FEC_CW_HI_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_USX_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));

            fcfecCountersPtr->blocksCorrectedError[0] = regData | (regDataHi << 16);
            fcfecCountersPtr->numReceivedBlocksCorrectedError += regData | (regDataHi << 16);

            fieldName = USX_RSFEC_UNITS_BASE_R_FEC_NCCW_LO_FEC_NCCW_LO_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_USX_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
            fieldName = USX_RSFEC_UNITS_BASE_R_FEC_CW_HI_FEC_CW_HI_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, MTI_USX_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
            fcfecCountersPtr->blocksUncorrectedError[0] = regData | (regDataHi << 16);
        ...
        else    /* PCS100/PCS50/PCS25 unit */
        {
            for (laneIdx = 0; laneIdx < curPortParams.numOfActLanes; laneIdx++)
            {
                ...
                fieldName = PHOENIX_RSFEC_UNIT_VL0_CCW_LO_P0_CORRECTED_FEC_CODEWORDS_0_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                fieldName = PHOENIX_RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));

                fcfecCountersPtr->blocksCorrectedError[laneIdx] = regData | (regDataHi << 16);
                fcfecCountersPtr->numReceivedBlocksCorrectedError += regData | (regDataHi << 16);

                if (curPortParams.numOfActLanes == 2)/*40G-R2/50G-R2*/
                {
                    fieldName = PHOENIX_RSFEC_UNIT_VL1_CCW_LO_P0_CORRECTED_FEC_CODEWORDS_1_E;
                    CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                    fieldName = PHOENIX_RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                    CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
                ...
                fieldName = PHOENIX_RSFEC_UNIT_VL0_NCCW_LO_P0_UNCORRECTED_FEC_CODEWORDS_0_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                fieldName = PHOENIX_RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
                ...
                if (curPortParams.numOfActLanes == 2)/*40G-R2/50G-R2*/
                {
                    fieldName = PHOENIX_RSFEC_UNIT_VL1_NCCW_LO_P0_UNCORRECTED_FEC_CODEWORDS_1_E;
                    CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regData, NULL));
                    fieldName = PHOENIX_RSFEC_UNIT_COUNTER_HI_P0_UPPER_COUNTER_BITS_E;
                    CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, (portNum+laneIdx), MTI_RSFEC_UNIT, curPortParams.portStandard, fieldName, &regDataHi, NULL));
...
)
{
    ...
    MV_HWS_UNITS_ID             unitId;
    GT_U32                      fieldName1, fieldName2;
    GT_U32 timeOut = 0;
    MV_HWS_HAWK_CONVERT_STC     ciderIndex;
    MV_HWS_RSFEC_COUNTERS_STC   sample = {{{0}},{{0}},{{0}},{0},{{0}}},
                                addingResult = {{{0}},{{0}},{{0}},{0},{{0}}};
    GT_STATUS                   rc = GT_OK;
    ...
    else if (HWS_DEV_SIP_6_10_CHECK_MAC(devNum) )
    {
        ...
        if(mvHwsMtipIsReducedPort(devNum,portNum) == GT_TRUE )
        {
            ...
            {
                /* If AC5X read from CPU Statistics unit */
                unitId = MTI_CPU_RSFEC_STATISTICS_UNIT;
            }
        }
        else if (mvHwsUsxModeCheck(devNum, portNum, curPortParams.portStandard) == GT_TRUE)
        {
        ...
        /* Set counters CLEAR_ON_READ */
        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_STATN_CONFIG_CLEAR_ON_READ : PHOENIX_RSFEC_STATISTICS_STATN_CONFIG_CLEAR_ON_READ_E;
        ...
        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_STATN_CONTROL : PHOENIX_RSFEC_STATISTICS_STATN_CONTROL_E;
        ...
        do{
            fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_STATN_CONTROL_CMD_CAPTURE : PHOENIX_RSFEC_STATISTICS_STATN_CONTROL_CMD_CAPTURE_E;
        ...
        /* Read captured data */
        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_COUNTER_CAPTURE_COUNTER_LOW_COUNTER_CAPTURE_CORRECTED : PHOENIX_RSFEC_STATISTICS_COUNTER_CAPTURE_TOTAL_CODEWORDS_CORRECTED_TOTAL_CW_CORRECTED_E;
        fieldName2 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_SLOW_DATA_HI_LATCHED_COUNTER_HI_SLOW : PHOENIX_RSFEC_STATISTICS_SLOW_DATA_HI_LATCHED_DATA_HI_SLOW_E;

        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        sample.correctedFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        sample.correctedFecCodeword.l[1] = regData;

        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_COUNTER_CAPTURE_COUNTER_LOW_COUNTER_CAPTURE_CORRECT : PHOENIX_RSFEC_STATISTICS_COUNTER_CAPTURE_TOTAL_CODEWORDS_CORRECT_TOTAL_CW_CORRECT_E;

        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        sample.correctFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        sample.correctFecCodeword.l[1] = regData;

        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_COUNTER_CAPTURE_COUNTER_LOW_COUNTER_CAPTURE_UNCORRECTABLE : PHOENIX_RSFEC_STATISTICS_COUNTER_CAPTURE_TOTAL_CODEWORDS_UNCORRECTABLE_TOTAL_CW_UNCORRECTABLE_E;

        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        sample.uncorrectedFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        sample.uncorrectedFecCodeword.l[1] = regData;

        fieldName1 = HWS_DEV_IS_AC5P_BASED_DEV_MAC(devNum) ? RSFEC_STATISTICS_UNITS_RSFEC_COUNTER_CAPTURE_COUNTER_LOW_COUNTER_CAPTURE_TOTAL_CW_RECEIVED : PHOENIX_RSFEC_STATISTICS_COUNTER_CAPTURE_TOTAL_CODEWORDS_RECEIVED_TOTAL_CW_RECEIVED_E;

        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        sample.total_cw_received.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        sample.total_cw_received.l[1] = regData;

        /* Clear all counters by reading and saving the addad data for the recuierd counters */
        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_TOTAL_CODEWORDS_RECEIVED_TOTAL_CW_RECEIVED_E : RSFEC_STATISTICS_UNITS_RSFEC_TOTAL_CODEWORDS_RECIVED_TOTAL_CW_RECEIVED;
        fieldName2 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_SLOW_DATA_HI_LATCHED_DATA_HI_SLOW_E : RSFEC_STATISTICS_UNITS_RSFEC_SLOW_DATA_HI_LATCHED_COUNTER_HI_SLOW;

        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        addingResult.total_cw_received.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        addingResult.total_cw_received.l[1] = regData;

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_TOTAL_CODEWORDS_CORRECT_TOTAL_CW_CORRECT_E : RSFEC_STATISTICS_UNITS_RSFEC_TOTAL_CODEWORDS_CORRECT_TOTAL_CW_CORRECT;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        addingResult.correctFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        addingResult.correctFecCodeword.l[1] = regData;

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_TOTAL_CODEWORDS_CORRECTED_TOTAL_CW_CORRECTED_E : RSFEC_STATISTICS_UNITS_RSFEC_TOTATL_CODEWORDS_CORRECTED_TOTAL_CW_CORRECTED;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        addingResult.correctedFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        addingResult.correctedFecCodeword.l[1] = regData;

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_TOTAL_CODEWORDS_UNCORRECTABLE_TOTAL_CW_UNCORRECTABLE_E : RSFEC_STATISTICS_UNITS_RSFEC_TOTATL_CODEWORDS_UNCORRECTABLE_TOTATL_CW_UNCORRECTABLE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        addingResult.uncorrectedFecCodeword.l[0] = regData;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName2, &regData, NULL));
        addingResult.uncorrectedFecCodeword.l[1] = regData;

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_1SE_CW_CORRECTED_1SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_1SE_CW_CORRECTED_1SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_2SE_CW_CORRECTED_2SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_2SE_CW_CORRECTED_2SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_3SE_CW_CORRECTED_3SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_3SE_CW_CORRECTED_3SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_4SE_CW_CORRECTED_4SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_4SE_CW_CORRECTED_4SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_5SE_CW_CORRECTED_5SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_5SE_CW_CORRECTED_5SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_6SE_CW_CORRECTED_6SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_6SE_CW_CORRECTED_6SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_7SE_CW_CORRECTED_7SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_7SE_CW_CORRECTED_7SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_8SE_CW_CORRECTED_8SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_8SE_CW_CORRECTED_8SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_9SE_CW_CORRECTED_9SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_9SE_CW_CORRECTED_9SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_10SE_CW_CORRECTED_10SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_10SE_CW_CORRECTED_10SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_11SE_CW_CORRECTED_11SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_11SE_CW_CORRECTED_11SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_12SE_CW_CORRECTED_12SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_12SE_CW_CORRECTED_12SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_13SE_CW_CORRECTED_13SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_13SE_CW_CORRECTED_13SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_14SE_CW_CORRECTED_14SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_14SE_CW_CORRECTED_14SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_CODEWORDS_CORRECTED_15SE_CW_CORRECTED_15SE_E : RSFEC_STATISTICS_UNITS_RSFEC_CODEWORDS_CORRECTED_15SE_CW_CORRECTED_15SE;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        fieldName1 = ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) ) ? PHOENIX_RSFEC_STATISTICS_SYMBOL_ERROR_CORRECTED_LANE0_SE_CORRECTED_LANE0_E : RSFEC_STATISTICS_UNITS_RSFEC_TOTAL_SYMBOLS_ERROR_CORRECTED;
        CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

        if ( HWS_DEV_SILICON_TYPE(devNum) == AC5X && (unitId == MTI_RSFEC_STATISTICS_UNIT) )
        {
            fieldName1 = PHOENIX_RSFEC_STATISTICS_SYMBOL_ERROR_CORRECTED_LANE0_SE_CORRECTED_LANE1_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

            fieldName1 = PHOENIX_RSFEC_STATISTICS_SYMBOL_ERROR_CORRECTED_LANE0_SE_CORRECTED_LANE2_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));

            fieldName1 = PHOENIX_RSFEC_STATISTICS_SYMBOL_ERROR_CORRECTED_LANE0_SE_CORRECTED_LANE3_E;
            CHECK_STATUS(genUnitPortModeRegisterFieldGet(devNum, 0, portNum, unitId, curPortParams.portStandard, fieldName1, &regData, NULL));
        }

#ifdef DEBUG_RSFEC_ERROR
        cpssOsPrintf("result1={\n\tcorrectFecCodeword=0x%x%x,\n\tcorrectedFecCodeword=0x%x%x,\n\tuncorrectedFecCodeword=0x%x%x,\n\t total_cw_received=0x%x%x\n}\n",
                     sample.correctFecCodeword.l[1], sample.correctFecCodeword.l[0], sample.correctedFecCodeword.l[1], sample.correctedFecCodeword.l[0],
                     sample.uncorrectedFecCodeword.l[1], sample.uncorrectedFecCodeword.l[0], sample.total_cw_received.l[1], sample.total_cw_received.l[0]);
#endif
        /* recalculate the captured values with the addition of the delta data that was read during the cleanup */
        rsfecCountersPtr->correctFecCodeword = prvCpssMathAdd64(sample.correctFecCodeword, addedRsfecResult[portNum].correctFecCodeword);
        rsfecCountersPtr->correctedFecCodeword = prvCpssMathAdd64(sample.correctedFecCodeword, addedRsfecResult[portNum].correctedFecCodeword);
        rsfecCountersPtr->uncorrectedFecCodeword = prvCpssMathAdd64(sample.uncorrectedFecCodeword, addedRsfecResult[portNum].uncorrectedFecCodeword);
        rsfecCountersPtr->total_cw_received = prvCpssMathAdd64(sample.total_cw_received, addedRsfecResult[portNum].total_cw_received);

#ifdef DEBUG_RSFEC_ERROR
        cpssOsPrintf("addingResult={\n\tcorrectFecCodeword=0x%x%x,\n\tcorrectedFecCodeword=0x%x%x,\n\tuncorrectedFecCodeword=0x%x%x,\n\t total_cw_received=0x%x%x\n}\n",
                     addingResult.correctFecCodeword.l[1], addingResult.correctFecCodeword.l[0], addingResult.correctedFecCodeword.l[1], addingResult.correctedFecCodeword.l[0],
                     addingResult.uncorrectedFecCodeword.l[1], addingResult.uncorrectedFecCodeword.l[0], addingResult.total_cw_received.l[1], addingResult.total_cw_received.l[0]);
        cpssOsPrintf("addedResult={\n\tcorrectFecCodeword=0x%x%x,\n\tcorrectedFecCodeword=0x%x%x,\n\tuncorrectedFecCodeword=0x%x%x,\n\t total_cw_received=0x%x%x\n}\n",
                     addedRsfecResult[portNum].correctFecCodeword.l[1], addedRsfecResult[portNum].correctFecCodeword.l[0], addedRsfecResult[portNum].correctedFecCodeword.l[1],  addedRsfecResult[portNum].correctedFecCodeword.l[0],
                     addedRsfecResult[portNum].uncorrectedFecCodeword.l[1], addedRsfecResult[portNum].uncorrectedFecCodeword.l[0], addedRsfecResult[portNum].total_cw_received.l[1], addedRsfecResult[portNum].total_cw_received.l[0]);
        cpssOsPrintf("result2={\n\tcorrectFecCodeword=0x%x%x,\n\tcorrectedFecCodeword=0x%x%x,\n\tuncorrectedFecCodeword=0x%x%x,\n\t total_cw_received=0x%x%x\n\t[%s]}\n",
                     rsfecCountersPtr->correctFecCodeword.l[1], rsfecCountersPtr->correctFecCodeword.l[0], rsfecCountersPtr->correctedFecCodeword.l[1], rsfecCountersPtr->correctedFecCodeword.l[0],
                     rsfecCountersPtr->uncorrectedFecCodeword.l[1], rsfecCountersPtr->uncorrectedFecCodeword.l[0], rsfecCountersPtr->total_cw_received.l[1], rsfecCountersPtr->total_cw_received.l[0],
                     (0 == prvCpssMathCompare64(rsfecCountersPtr->total_cw_received,prvCpssMathAdd64(rsfecCountersPtr->correctFecCodeword,prvCpssMathAdd64(rsfecCountersPtr->correctedFecCodeword,rsfecCountersPtr->uncorrectedFecCodeword))))?"TRUE":"FALSE");
#endif

        /* calculate data delta for next update */
        addedRsfecResult[portNum].correctFecCodeword = prvCpssMathSub64(addingResult.correctFecCodeword,sample.correctFecCodeword);
        addedRsfecResult[portNum].correctedFecCodeword = prvCpssMathSub64(addingResult.correctedFecCodeword,sample.correctedFecCodeword);
        addedRsfecResult[portNum].uncorrectedFecCodeword = prvCpssMathSub64(addingResult.uncorrectedFecCodeword,sample.uncorrectedFecCodeword);
        addedRsfecResult[portNum].total_cw_received = prvCpssMathSub64(addingResult.total_cw_received,sample.total_cw_received);
    }
    else
    {
...
#if ((defined CHX_FAMILY) || defined(RAVEN_DEV_SUPPORT) || defined (FALCON_DEV_SUPPORT) || defined (AC5P_DEV_SUPPORT) || defined (HARRIER_DEV_SUPPORT) || defined (IRONMAN_DEV_SUPPORT))
...
)
{
...
#if !defined (RAVEN_DEV_SUPPORT) && !defined(FALCON_DEV_SUPPORT) && !defined(AC5P_DEV_SUPPORT)
    localPortIndex = PRV_HWS_D2D_CONVERT_PORT_TO_CHANNEL_MAC(devNum, portNum);
#else
    localPortIndex = portNum %8;
#endif
    ...
    for (i = 0; i < curPortParams.numOfActLanes; i++)
    {
        switch (fecType) {
            case RS_FEC_544_514:
                if ((!HWS_PAM4_MODE_CHECK(portMode)) && !((portMode == _200GBase_KR8)
                     || (portMode == _200GBase_CR8 ) || (portMode == _200GBase_SR_LR8 )
                     || (portMode == _106GBase_KR4) || (portMode == _107GBase_KR4)
...
/**
* @internal mvHwsAasUnitMarkerIntervalCalc function
* @endinternal
*
...
* @param[in] unit            - unit for calculations (0 - PCS, 1 - MAC)
* @param[out] markerIntervalPtr - (pointer to) marker interval value
*
* @retval 0                  - on success
* @retval 1                  - on error
*/
GT_STATUS mvHwsAasUnitMarkerIntervalCalc
(
    GT_U8                   devNum,
    MV_HWS_PORT_STANDARD    portMode,
...
)
{
    ...
    switch(portMode)
    {
        ...
        case HWS_50G_R2_MODE_CASE:
            ...
            {
                markerIntervalValue = 0x3FFF;
            }
            break;

        case HWS_100G_R1_MODE_CASE:
        case HWS_100G_R2_MODE_CASE: /*100G_R2*/
    ...
    if (unit == 0) /*PCS*/
    {
        switch(portMode)
        {
            ...
            case HWS_100G_R2_MODE_CASE: /*100G_R2*/
            case HWS_100G_R4_MODE_CASE:
                *markerIntervalPtr = markerIntervalValue;
                break;

            case HWS_200G_R2_MODE_CASE:
            case HWS_200G_R4_MODE_CASE:
                *markerIntervalPtr = (markerIntervalValue+1)/5;
                break;

            case HWS_400G_R4_MODE_CASE:
            case HWS_400G_R8_MODE_CASE:
            case HWS_800G_R8_MODE_CASE:
                *markerIntervalPtr = (markerIntervalValue+1)*2/5;
                break;

#if 0
            case HWS_800G_R8_MODE_CASE:
                *markerIntervalPtr = (markerIntervalValue+1)*4/5;
                break;
#endif
            default:
                *markerIntervalPtr = 0;
    ...
    else if(unit == 1) /*MAC*/
    {
        switch(portMode)
        {
            ...
            case HWS_400G_R8_MODE_CASE:
            case HWS_800G_R8_MODE_CASE:
                *markerIntervalPtr = (markerIntervalValue+1)/8;
                break;
...
/**
* @internal mvHwsMtipMacTxSectionFullCalc function
* @endinternal
*
* @brief  MAC TX FIFO full threshold calc function
*
*
* @param[in] devNum          - system device number
* @param[in] portNum         - port number
* @param[in] portMode        - port mode
* @param[in] attributesPtr   - (pointer to) port attributes structure
* @param[out] txFifoFullThreshold   - (pointer to) calculated value
*
* @retval 0                  - on success
* @retval 1                  - on error
*/
GT_STATUS mvHwsMtipMacTxSectionFullCalc
(
    GT_U8                   devNum,
    GT_U32                  portNum,
    MV_HWS_PORT_STANDARD    portMode,
...
)
{
    ...
    switch(portMode)
    {
        ...
        case _10G_QXGMII:
            ...
            {
                if((attributesPtr->portSpeed == MV_HWS_PORT_SPEED_10M_E) ||
                   (attributesPtr->portSpeed == MV_HWS_PORT_SPEED_100M_E))
                {
                    *txFifoFullThreshold = (attributesPtr->duplexMode == MV_HWS_PORT_HALF_DUPLEX_E) ? 13 : 2;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_1G_E)
                {
                    *txFifoFullThreshold = 0x2;
                }
                else /* MV_HWS_PORT_SPEED_2_5G_E, MV_HWS_PORT_SPEED_5G_E, MV_HWS_PORT_SPEED_10G_E */
                {
                    *txFifoFullThreshold = 0x7;
                }
            }
            else /* assuming MV_HWS_PORT_SPEED_2_5G_E, MV_HWS_PORT_SPEED_5G_E, MV_HWS_PORT_SPEED_10G_E since attributePtr is NULL */
            {
                *txFifoFullThreshold = 0x7;
            }
            break;
...
/**
* @internal mvHwsMtipMacTxSectionEmptyCalc function
* @endinternal
*
* @brief  MAC TX FIFO empty threshold calc function
*
*
* @param[in] devNum          - system device number
* @param[in] portNum         - port number
* @param[in] portMode        - port mode
* @param[in] attributesPtr   - (pointer to) port attributes structure
* @param[out] txFifoEmptyThreshold   - (pointer to) calculated value
*
* @retval 0                  - on success
* @retval 1                  - on error
*/
GT_STATUS mvHwsMtipMacTxSectionEmptyCalc
(
    GT_U8                   devNum,
    GT_U32                  portNum,
    MV_HWS_PORT_STANDARD    portMode,
...
)
{
    ...
    switch(portMode)
    {
        ...
        case _1000Base_X:
        case _1000Base_SX:
            *txFifoEmptyThreshold = 0x3;
            break;

        case _2500Base_X:
        case SGMII2_5:
        case _2_5G_SXGMII:
            *txFifoEmptyThreshold = 0x8;
            break;
        case _5GBaseR:
        case _5G_SXGMII:
            *txFifoEmptyThreshold = 0x9;
            break;
        case _10GBase_KR:
        case _10GBase_SR_LR:
        case _10G_SXGMII:
            *txFifoEmptyThreshold = 0xc;
            break;
        case _12GBaseR:
            *txFifoEmptyThreshold = 0x0;
            break;
        ...
        case _10G_QXGMII:
            if ( attributesPtr != NULL )
            {
                if ( attributesPtr->portSpeed == MV_HWS_PORT_SPEED_10M_E )
                {
                    *txFifoEmptyThreshold = (attributesPtr->duplexMode == MV_HWS_PORT_HALF_DUPLEX_E) ? 0 : 3;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_100M_E)
                {
                    *txFifoEmptyThreshold = (attributesPtr->duplexMode == MV_HWS_PORT_HALF_DUPLEX_E) ? 0 : 4;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_1G_E)
                {
                    *txFifoEmptyThreshold = 0x3;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_2_5G_E)
                {
                    *txFifoEmptyThreshold = 0x8;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_5G_E)
                {
                    *txFifoEmptyThreshold = 0x9;
                }
                else if(attributesPtr->portSpeed == MV_HWS_PORT_SPEED_10G_E)
                {
                    *txFifoEmptyThreshold = 0xc;
                }
                else
...
)
{
    ...
    else
    {
        ...
        if(((HWS_1000BASE_MODE_CHECK(portMode)) || (portMode == _2500Base_X) || (portMode == SGMII) )&& (enable == GT_TRUE))
        {
