...
* @retval 1                        - on error
*/
static GT_STATUS mvHwsMtiLowSpeedPcsRev2LoopBack
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  portGroup,
...
)
{
    ...
    */
    ...
    /* in 1000Base-X mode, the inband negotiation state machine might never complete without this reset */
    if ((portMode == _1000Base_X) && (enable == GT_TRUE))
    {
        CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, MTI_LOW_SP_PCS_UNIT, LPCS_UNITS_CONTROL_P0_RESET_E, 1, NULL));
    }
...
)
{
    ...
    MV_HWS_REG_ADDR_FIELD_STC fieldReg;
    GT_U32 baseAddr, regAddr, unitIndex, unitNum, fieldStart, fieldLength, pcsField;
    MV_HWS_HAWK_CONVERT_STC convertIdx;
    GT_STATUS rc;
...
)
{
    ...
    */
    if(hwsDeviceSpecInfo[devNum].devType == AC5X)
    {
        extField = PHOENIX_MTIP_EXT_UNITS_STATUS_P0_LPCS_RX_SYNC_E;
    }
    else
    {
        extField = HAWK_MTIP_EXT_BR_UNITS_STATUS_P0_LPCS_RX_SYNC_E;
    }
...
/**
* @internal mvHwsMtiLowSpeedPcsRev2RxReset function
* @endinternal
*
...
* @retval 1                        - on error
*/
static GT_STATUS mvHwsMtiLowSpeedPcsRev2RxReset
(
    GT_U8                   devNum,
    GT_U32                  portGroup,
    GT_U32                  portMacNum,
...
)
{
    ...
    GT_U32 regAddr, regData = 0, fieldStart;
    MV_HWS_HAWK_CONVERT_STC convertIdx;
    MV_HWS_REG_ADDR_FIELD_STC fieldReg;
    GT_STATUS rc;
    ...
    case RESET:
        ...
        break;
    case UNRESET:
        for (i = 0; i < params.numOfActLanes; i++)
        {
            regData |= (1 <<(i));
        }
        break;
    ...
    CHECK_STATUS(mvUnitExtInfoGet(devNum, MTI_EXT_UNIT, portMacNum, &baseAddr, &unitIndex, &unitNum ));

    /* gc_sd_rx_reset_  */
    rc = genUnitRegDbEntryGet(devNum, portMacNum, MTI_EXT_UNIT, portMode,
    ...
    regAddr = baseAddr + fieldReg.regOffset;
    fieldStart = fieldReg.fieldStart + convertIdx.ciderIndexInUnit;
    CHECK_STATUS(hwsRegisterSetFieldFunc(devNum, 0, regAddr, fieldStart, params.numOfActLanes, regData));

    return GT_OK;
}

/**
 * @internal mvHwsMtiLowSpeedPcsRev2RxResetGet function
 * @endinternal
 *
 ...
 * @return GT_STATUS
 */
static GT_STATUS mvHwsMtiLowSpeedPcsRev2RxResetGet
(
    GT_U8                   devNum,
    GT_U32                  portGroup,
    GT_U32                  portMacNum,
...
)
{
    MV_HWS_PORT_INIT_PARAMS params;
    GT_U32 baseAddr, unitIndex, unitNum, i;
    GT_U32 regAddr, regData = 0, data, fieldStart;
    MV_HWS_HAWK_CONVERT_STC convertIdx;
    MV_HWS_REG_ADDR_FIELD_STC fieldReg;
    GT_STATUS rc;
    ...
    regAddr = baseAddr + fieldReg.regOffset;
    fieldStart = fieldReg.fieldStart + convertIdx.ciderIndexInUnit;
    CHECK_STATUS(hwsRegisterGetFieldFunc(devNum, 0, regAddr, fieldStart, params.numOfActLanes, &data));

    *state = (data != regData) ? RESET : UNRESET;
...
GT_STATUS mvHwsMtiLowSpeedPcsRev2IfInit(MV_HWS_PCS_FUNC_PTRS **funcPtrArray)
{
    ...
    funcPtrArray[MTI_PCS_LOW_SPEED]->pcsRxResetFunc         = mvHwsMtiLowSpeedPcsRev2RxReset;
    funcPtrArray[MTI_PCS_LOW_SPEED]->pcsRxResetGetFunc      = mvHwsMtiLowSpeedPcsRev2RxResetGet;
