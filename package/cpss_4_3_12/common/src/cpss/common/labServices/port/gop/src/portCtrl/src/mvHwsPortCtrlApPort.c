...
extern GT_U32  mvPortCtrlApStopDebugFlag;
#endif


#if defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
/**
* @internal mvApPortDebugModeStatusGet function
* @endinternal
*
* @brief  check Debug Mode status (enable/disable)
*
* @param[in] port                     - port index
*
* @retval GT_BOOL                     - Debug Mose status
*                                       (enable/disable)
*/
GT_BOOL mvApPortDebugModeStatusGet(GT_U8 portIndex)
{
    GT_BOOL debugModeEnable = GT_FALSE;
    GT_U8 i;
    ...
    if(mvPortCtrlApStopDebugFlag & ((1 << i)|(AP_DEBUG_ALL_PORTS_MASK)))
    {
        debugModeEnable = GT_TRUE;
    }
                                           ...
                                           MV_HWS_IPC_CTRL_MSG_AP_STRUCT *msg)
{
    ...
    /*if status PORT_SM_DELETE_FAILURE we failed in serdes power down and this step was already done - skip it*/
    if (info->status != PORT_SM_DELETE_FAILURE )
    {
#ifndef IRONMAN_DEV_SUPPORT /*for ironman port enable done on host*/
...
#if !defined(BC2_DEV_SUPPORT) && !defined(AC5_DEV_SUPPORT) && !defined(AC5P_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
...
#if defined(BC2_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
    /* Execute HWS Training check */
    if (mvHwsPortAutoTuneSet(0, /* devNum */
                             0, /* portGroup */
                             (GT_U32)recvMsg->phyPortNum,
    ...
    else if (tuneResult == TUNE_PASS)
    {
...
#if defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
        {
            ...
            if (GT_OK != mvHwsPortOperation(0 /* devNum */, 0 /* portGroup */, recvMsg->phyPortNum, recvMsg->portMode,
                               MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_IC_E, &data, &result))
            {
                mvPortCtrlApPortFail("Error, Port-%d serdes operation Failed\n",
                                     recvMsg->phyPortNum, &msgType, recvMsg, MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_IC_E);
                    return;
            }
            /* avoid warnings */
            data = data;
...
#if defined(BC2_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)/* for AC3/AC5 doorbell interrupt not supported yet - must enable port here unconditionally. NOTE: BC2_DEV_SUPPORT common for AC3 and BC2 */
...
*
*******************************************************************************/
#if !defined (AC5_DEV_SUPPORT) && !defined (AC5P_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
...
#if defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
    if(mvApPortDebugModeStatusGet(portIndex))
    {
        return;
    }
#endif
            ...
            if (mvPortCtrlApPortPendMsg(portIndex, &recvMsg, &msgType, PORT_MSG_PROC) == GT_OK)
            {
                ...
                *portInit &= ~(MV_PORT_CTRL_NUM(portIndex));
                ...
                portFecType = (AP_ST_HCD_FEC_RES_NONE == AP_ST_HCD_FEC_RES_GET(apSm->hcdStatus))
                    ? FEC_OFF : FC_FEC;
#if defined(BC2_DEV_SUPPORT) || defined(AC5_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
                if (Alleycat3A0 == hwsDeviceSpecInfo[0].devType || AC5 == hwsDeviceSpecInfo[0].devType || Ironman == hwsDeviceSpecInfo[0].devType)
                {
                    ...
                    portInitInParam.portFecMode = portFecType;
...
#ifdef IRONMAN_DEV_SUPPORT
                    /*Ironman - step31-35 - all part of portInit*/
                    /* Un-gate PCS sd tx & rx clocks  */
                    rc = mvHwsIronmanAnpPortPcsSdTxRxClockGate(0/*devNum*/, recvMsg->phyPortNum, GT_FALSE);
                    if (rc != GT_OK)
                    {
                        mvPortCtrlApPortFail("Error, Port Mng, Port-%d Un-gate PCS sd tx & rx clocks\n",
                                             portIndex, &msgType, recvMsg, PORT_SM_MAC_PCS_CONFIG_FAILURE);
                        return;
                    }
                    /* Un-reset PCS sd tx & rx */
                    rc = mvHwsIronmanAnpPortPcsSdTxRxReset(0/*devNum*/, recvMsg->phyPortNum, GT_FALSE);
                    if (rc != GT_OK)
                    {
                        mvPortCtrlApPortFail("Error, Port Mng, Port-%d Un-reset PCS sd tx & rx\n",
                                             portIndex, &msgType, recvMsg, PORT_SM_MAC_PCS_CONFIG_FAILURE);
                        return;
                    }
#endif
                    ...
                    if ((portMode != _1000Base_X) && (portMode != _10GBase_KX4) && (portMode != _2500Base_X))
                    {
...
#ifdef IRONMAN_DEV_SUPPORT
                        /*Ironman ignore tuneRes and don't upatet it - if mvHwsPortAutoTuneSet terurn with OK, this parameter can be set as pass/ready */
