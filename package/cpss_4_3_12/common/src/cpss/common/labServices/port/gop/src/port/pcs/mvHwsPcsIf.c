...
/**
 * @internal mvHwsPcsRxResetGet function
 * @endinternal
 *
 * @brief   Return the PCS sd_rx_reset state.
 *
 * @param[in] devNum
 * @param[in] portGroup
 * @param[in] phyPortNum
...
)
{
    ...
    MV_HWS_IS_PCS_FUNC_IMPLEMENTED_CHECK_AND_RETURN(devNum, curPortParams.portPcsType, pcsRxResetGetFunc)

    CHECK_STATUS(PRV_NON_SHARED_PCS_DIR_PCS_IF_SRC_GLOBAL_VAR(hwsPcsFuncsPtr)[devNum][portPcsType]->pcsRxResetGetFunc(devNum, portGroup, phyPortNum, portMode, state));
...
)
{
    ...
    CHECK_STATUS(PRV_NON_SHARED_PCS_DIR_PCS_IF_SRC_GLOBAL_VAR(hwsPcsFuncsPtr)[devNum][pcsType]->pcsRxResetFunc(devNum, portGroup, pcsNum, portMode, action));
    return GT_OK;
}
...
/**
* @internal mvHwsPcsRemoteFaultSelectSet function
* @endinternal
*
* @brief   Configure the PCS Remote Fault Enable/Disable.
*
* @param[in] devNum                   - system device number
* @param[in] pcsNum                   - PCS number
* @param[in] pcsType                  - PCS type according to port configuration
* @param[in] portMode                 - Port Mode
* @param[in] enable                   - GT_TRUE for Remote fault enable, GT_FALSE otherwise
*
* @retval 0                        - on success
* @retval 1                        - on error
*/

GT_STATUS mvHwsPcsRemoteFaultSelectSet
(
    IN GT_U8                devNum,
    IN GT_U32               pcsNum,
    IN MV_HWS_PORT_PCS_TYPE pcsType,
...
)
{
    ...
    MV_HWS_IS_PCS_FUNC_IMPLEMENTED_CHECK_AND_RETURN(devNum, pcsType, pcsTxRemoteFaultSelectSetFunc)

    CHECK_STATUS_EXT(PRV_NON_SHARED_PCS_DIR_PCS_IF_SRC_GLOBAL_VAR(hwsPcsFuncsPtr)[devNum][pcsType]->pcsTxRemoteFaultSelectSetFunc(devNum, pcsNum, portMode, enable),
                     LOG_ARG_STRING_MAC("pcs type"), LOG_ARG_GEN_PARAM_MAC(pcsType));

    return GT_OK;
}

/** @internal mvHwsPcsPortAnDoneGet function
* @endinternal
*
* @brief   Read  AN Done bit of PCS.
*
* @param[in] devNum                   - system device number
* @param[in] portGroup                - port group (core) number
* @param[in] phyPortNum               - MAC number
* @param[in] portMode                 - port mode
*
* @param[out] anDone                  - autoneg done.
*
* @retval 0                        - on success
* @retval 1                        - on error
*/
GT_STATUS mvHwsPcsPortAnDoneGet
(
    IN GT_U8                   devNum,
    IN GT_U32                  portGroup,
    IN GT_U32                  phyPortNum,
...
)
{
    ...
    MV_HWS_IS_PCS_FUNC_IMPLEMENTED_CHECK_AND_RETURN(devNum, portPcsType, pcsAnDoneGetFunc)

    CHECK_STATUS(PRV_NON_SHARED_PCS_DIR_PCS_IF_SRC_GLOBAL_VAR(hwsPcsFuncsPtr)[devNum][portPcsType]->pcsAnDoneGetFunc(devNum, portGroup, curPortParams.portPcsNumber, portMode, anDone));
