...
MCESD_STATUS API_C28GP4X4_WriteReg
(
    IN MCESD_DEV_PTR devPtr,
    IN MCESD_U8 lane,
    IN MCESD_U32 reg,
...
)
{
    ...
    MCESD_ATTEMPT(API_C28GP4X4_HwWriteReg(devPtr, reg, value));

    return MCESD_OK;
}

MCESD_STATUS API_C28GP4X4_ReadReg
(
    IN MCESD_DEV_PTR devPtr,
    IN MCESD_U8 lane,
    IN MCESD_U32 reg,
...
)
{
    ...
    MCESD_ATTEMPT(API_C28GP4X4_HwReadReg(devPtr, reg, data));
    return MCESD_OK;
}

MCESD_STATUS API_C28GP4X4_WriteField
(
    IN MCESD_DEV_PTR devPtr,
    IN MCESD_U8 lane,
...
)
{
    ...
    /* Call high level API_C28GP4X4_ReadReg() which will switch lanes if necessary */
    MCESD_ATTEMPT(API_C28GP4X4_ReadReg(devPtr, lane, fieldPtr->reg, &regValue));
    ...
    /* Call low level API_C28GP4X4_HwWriteReg() since the lane switching was already done */
    MCESD_ATTEMPT(API_C28GP4X4_HwWriteReg(devPtr, fieldPtr->reg, combinedValue));

    return MCESD_OK;
}

MCESD_STATUS API_C28GP4X4_ReadField
(
    IN MCESD_DEV_PTR devPtr,
    IN MCESD_U8 lane,
    IN MCESD_FIELD_PTR fieldPtr,
...
MCESD_STATUS API_C28GP4X4_PollField
(
    IN MCESD_DEV_PTR devPtr,
    IN MCESD_U8 lane,
    IN MCESD_FIELD_PTR fieldPtr,
...
)
{
    ...
    {
        MCESD_U32 data;
        MCESD_ATTEMPT(API_C28GP4X4_ReadField(devPtr, lane, fieldPtr, &data));
        if (data == value)
            return MCESD_OK;
...
MCESD_STATUS API_C28GP4X4_PollPin
(
    IN MCESD_DEV_PTR devPtr,
...
)
{
    ...
    {
        MCESD_U32 data;
        MCESD_ATTEMPT(API_C28GP4X4_HwGetPinCfg(devPtr, pin, &data));
        if (data == value)
            return MCESD_OK;
...
)
{
    ...
    MCESD_ATTEMPT(API_C28GP4X4_HwReadReg(devPtr, field.reg, &regValue));

    /* Modify the register value with the desired field value */
    combinedValue = (regValue & field.retainMask) | ( value << field.loBit );

    MCESD_ATTEMPT(API_C28GP4X4_HwWriteReg(devPtr, field.reg, combinedValue));
