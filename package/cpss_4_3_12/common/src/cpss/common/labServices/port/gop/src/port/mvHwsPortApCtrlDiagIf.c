...
GT_U32 mvHwsFwAPLogOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwAPLogCountOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwAPLogPointerOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwAPLogResetOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwAPLogPointer[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];

/* Port Log Control Parameters */
GT_U32 mvHwsFwLogOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwLogCountOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwLogPointerOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwLogResetOffsetAddr[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
GT_U32 mvHwsFwLogPointer[SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS];
...
char *apPortMngSmStatus[] =
{
    ...
    "AP port enable wait",
    "AP port signal_detect/rx_init wait"
};
...
{
    if (cpuId < SRVCPU_MAX_IPC_CHANNEL*PRV_CPSS_MAX_PP_DEVICES_CNS)
    {
...
)
{
    ...
    address = (chipIndex/4) * 0x20000000;
     ...
     address +=  0x380000;
     prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,mvHwsFwAPLogCountOffsetAddr[devNum*16+chipIndex]+address,0xFFFFFFFF, &count);
     /*hwsRegisterGetFuncPtr(0, 0, mvHwsFwAPLogCountOffsetAddr[chipIndex]+address, &count, 0xFFFFFFFF);*/
     hwsOsPrintf("logCount address: 0x%x count %d\n",mvHwsFwAPLogCountOffsetAddr[devNum*16+chipIndex],count);
     prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,mvHwsFwAPLogPointerOffsetAddr[devNum*16+chipIndex]+address,0xFFFFFFFF, &count);
     /*hwsRegisterGetFuncPtr(0, 0, mvHwsFwAPLogPointerOffsetAddr[chipIndex]+address, &count, 0xFFFFFFFF);*/
     hwsOsPrintf("logOffset address: 0x%x offset %d\n",mvHwsFwAPLogPointerOffsetAddr[devNum*16+chipIndex],count);

     address += mvHwsFwAPLogOffsetAddr[devNum*16+chipIndex];
     hwsOsPrintf("portLog address: 0x%x (0x%x)\n",mvHwsFwAPLogOffsetAddr[devNum*16+chipIndex],address);

    for (i = 0; i <  MV_PORT_CTRL_LOG_SIZE; i++)
    {
        prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,address+i*8,0xFFFFFFFF, &timestamp);
        prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,address+i*8+4,0xFFFFFFFF, &info);
   ...
   /* print extra data ftom ap in falcon*/
    if(hwsDeviceSpecInfo[devNum].devType == Falcon)
    {
        ...
        {
            prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,address+ MV_PORT_CTRL_LOG_SIZE*8 +i*8,0xFFFFFFFF, &timestamp);
            prvCpssDrvHwPpPortGroupReadRegBitMask(devNum,0,address+ MV_PORT_CTRL_LOG_SIZE*8 +i*8+4,0xFFFFFFFF, &info);
...
)
{
   ...
   /* GT_UINTPTR fwChannel = PRV_SHARED_LAB_SERVICES_DIR_PORT_SRC_GLOBAL_VAR(hwsIpcChannelHandlers)[devNum];*/
    ...
    if(hwsDeviceSpecInfo[devNum].devType == Falcon)
    {
        ...
        hwsSprintf(buffer,"%08d: AN SM   AP version number %02x/%03x_%01x\n",
                           0,
                           ((logInfo>>16)&0xFF), ((logInfo>>4)&0xFFF), (logInfo&0xF));
        mvApPortCtrlDebugInfoPrint(buffer, output);
    }
