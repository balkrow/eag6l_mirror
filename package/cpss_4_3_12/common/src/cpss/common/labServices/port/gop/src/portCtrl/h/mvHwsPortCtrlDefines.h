...
/* For CM3: SHMEM_SIZE = 0x800; LOAD_ADDR = 0; SHMEM_REGS = (SHMEM_BASE + SHMEM_SIZE - 4); SHMEM_BASE = SRAM size (128KB BOBK) - SHMEM_SIZE;*/
  ...
  #ifdef CM3
    #define LOAD_ADDR       0
    #if defined (ALDRIN2_DEV_SUPPORT) || defined (FALCON_DEV_SUPPORT) || defined (AC5P_DEV_SUPPORT)
        #define SHMEM_BASE         (0x5f800)
    #elif defined (HARRIER_DEV_SUPPORT)
        #ifdef SHMEM_BASE
          #undef SHMEM_BASE
        #endif
        #define SHMEM_BASE         (0x3f800)
    #else
 ...
 #define HWS_IPC_LINK_ID (IPC_CM3_FREERTOS_LINK_ID)
  ...
  #endif
  #define SHMEM_REGS      (SHMEM_BASE + SHMEM_SIZE - 4)
 #elif defined(RAVEN_DEV_SUPPORT) || defined(IRONMAN_DEV_SUPPORT)
...
#define MV_PORT_CTRL_AP_VERSION       (0x043120)    /*version 4.3.12 043120*/
...
typedef enum
{
    ...
    AP_PORT_SIGNAL_DETECT_RX_INIT_WAIT     = 28,

    AP_PORT_MAX_STATUS                     = 29
