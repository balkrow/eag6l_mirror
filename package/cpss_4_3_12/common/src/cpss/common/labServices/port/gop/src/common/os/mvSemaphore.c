...
static GT_STATUS mvMGRead(GT_U8 devNum, GT_U32 addr, GT_U32 *dataPtr)
{
    ...
    *dataPtr = (MV_MEMIO32_READ(MG_BASE | (addr&0x000FFFFF)));
#else
...
#elif defined (HARRIER_DEV_SUPPORT) || defined (AC5P_DEV_SUPPORT)
    MV_MEMIO32_WRITE((MG_BASE | (addr&0x000FFFFF)), data);
#else
...
#else
            rc = regWrite(baseAddr + num*4, cpuId);
            if (rc!=GT_OK)
            {
                hwsOsPrintf("Error: Failed to write semaphore value. (%d)\n", rc);
            }
            tmp = regRead(baseAddr + num*4);
#endif
        }
        else
        {
...
#else
        CHECK_STATUS(regWrite(baseAddr + num*4, 0xFF));
#endif
