...
GT_U32 mvHwsComphyCodeGet (GT_U32 index, GT_U32 code[])
{
#ifndef AC5P_DEV_SUPPORT
    return code[index];
#else
    GT_U32 spi_data = 0xFFFFFFFF;
    if (NULL != hwsSFlashGetFuncPtr)
    {
        CHECK_STATUS(hwsSFlashGetFuncPtr(0, (GT_U32)&code[index], (GT_U8*)&spi_data, 4));
        return spi_data;
    }
    return code[index];
#endif
...
MCESD_STATUS mvHwsComphySetPinCallback(MCESD_DEV_PTR sDev, MCESD_U16 pin, MCESD_U32 pinValue);
MCESD_STATUS mvHwsComphyGetPinCallback(MCESD_DEV_PTR sDev, MCESD_U16 pin, MCESD_U32 *pinValue);
...
extern const MV_HWS_COMPHY_REG_FIELD_STC mvHwsComphyN5XC56GP5X4PinToRegMapSdw[];
extern const MV_HWS_SERDES_TXRX_TUNE_PARAMS harrierTxRxTuneParams[];
extern const MV_HWS_COMPHY_REG_FIELD_STC mvHwsComphyC12GP41P2VPinToRegMap[];
extern const MV_HWS_SERDES_TXRX_TUNE_PARAMS alleycat5TxRxTuneParams[];
extern const MV_HWS_SERDES_TXRX_TUNE_PARAMS ironmanTxRxTuneParams[];
extern const MV_HWS_COMPHY_REG_FIELD_STC mvHwsComphyC12GP41P2VSip6_30_PinToRegMap[];
...
)
{
    switch(devType)
    {
      ...
      case AC5P:
        ...
        break;
      case Harrier:
        return mvHwsComphyN5XC56GP5X4PinToRegMapSdw;
        break;
      case AC5:
        return mvHwsComphyC12GP41P2VPinToRegMap;
        break;
      case Ironman:
        return mvHwsComphyC12GP41P2VSip6_30_PinToRegMap;
        break;
...
#ifdef SHARED_MEMORY
        /* Update the sDev's  function pointers due to ASLR */
       (*sDevPtr)->fmcesdReadReg = mvHwsComphyRegisterReadCallback;
       (*sDevPtr)->fmcesdWriteReg = mvHwsComphyRegisterWriteCallback;
       (*sDevPtr)->fmcesdSetPinCfg = mvHwsComphySetPinCallback;
       (*sDevPtr)->fmcesdGetPinCfg = mvHwsComphyGetPinCallback;
       (*sDevPtr)->fmcesdWaitFunc = mvHwsComphyWait;
#endif
    }
...
MCESD_STATUS mvHwsComphyRegisterWriteCallback(MCESD_DEV_PTR sDev, MCESD_U32 reg, MCESD_U32 value)
{
    ...
    if (serdesInfo->serdesType == COM_PHY_C12GP41P2V || serdesInfo->serdesType == COM_PHY_C12GP41P2V_SIP6_30)
    {
...
MCESD_STATUS mvHwsComphySetPinCallback(MCESD_DEV_PTR sDev, MCESD_U16 pin, MCESD_U32 pinValue)
{
    ...
    if (serdesInfo->serdesType == COM_PHY_C12GP41P2V_SIP6_30)
    {
        rc = genUnitRegisterFieldSet(devInfo->devNum, 0, serdesInfo->serdesNum, SERDES_UNIT, pinToRegMap[pin].regOffset, pinValue, NULL);
    }
    else
#endif
    {
        ...
        if (((serdesInfo->serdesType == COM_PHY_N5XC56GP5X4)|| (serdesInfo->serdesType == COM_PHY_C112GX4)||(serdesInfo->serdesType == COM_PHY_C28GP4X4)) &&
             (pinToRegMap[pin].regOffset > 0x20000))
        {
            CHECK_STATUS(mvHwsComphySelectLane(sDev, serdesInfo, pinToRegMap[pin].regOffset));
        }
...
MCESD_STATUS mvHwsComphyGetPinCallback(MCESD_DEV_PTR sDev, MCESD_U16 pin, MCESD_U32 *pinValue)
{
    ...
    if (serdesInfo->serdesType == COM_PHY_C12GP41P2V_SIP6_30)
    {
        rc = genUnitRegisterFieldGet(devInfo->devNum, 0, serdesInfo->serdesNum, SERDES_UNIT, pinToRegMap[pin].regOffset, pinValue, NULL);
    }
    else
#endif
    {
        ...
        rc = hwsSerdesRegGetFuncPtr(devInfo->devNum, 0, EXTERNAL_REG,
                                                    serdesInfo->serdesNum,
                                                    reg, &value, mask);
       *pinValue = (MCESD_U32)(value >> pinToRegMap[pin].fieldStart);
    ...
    for(serdesIterator = 0 ; serdesIterator < HWS_DEV_SERDES_INFO(devNum).numOfSerdeses ; serdesIterator++)
    {
        ...
        if((serdesType == currSerdes->serdesType) || (serdesType == SERDES_LAST))
        {
            if(currSerdes->internalLane == 0)/* First lane of group of lanes - master lane */
            {
                ...
                IF_C12GP41P2V_SIP6_30 (currSerdes->serdesType, ipMajorRev = IP_MAJOR_C12GP41P2V_SIP6_30; ipMinorRev = IP_MINOR_C12GP41P2V_SIP6_30;);
...
)
{
    ...
    MV_HWS_PER_SERDES_INFO_PTR  serdesInfo = NULL;
...
)
{
    ...
    return mvHwsComphySetPinCallback(sDev, (MCESD_U16)pinNum, (MCESD_U32)pinVal);
}
...
)
{
    ...
    MV_HWS_SSE_INVOKER_SERDES_RD_WR_CALLBACKS   serdesRdWrCbsObj;
#endif
...
#if !defined (MV_HWS_FREE_RTOS) || defined (AC5_DEV_SUPPORT)
    ...
    IF_C12GP41P2V_SIP6_30(serdesType, CHECK_STATUS(mvHwsComphyC12GP41P2VSip6_30_IfInit(funcPtrArray)););
#endif
...
#if defined (SSE_ENABLED) && defined (CHX_FAMILY)
    serdesRdWrCbsObj.readRegFuncPtr = mvHwsComphySerdesRegisterRead;
    serdesRdWrCbsObj.writeRegFuncPtr = mvHwsComphySerdesRegisterWrite;
    rc = mvHwsSseInvokerRegistration(SSE_LUT_SERDES_E, &serdesRdWrCbsObj);
#endif
...
)
{
    ...
    {
        serdesData->tuneOverrideParams = (MV_HWS_SERDES_TXRX_TUNE_PARAMS*)hwsOsMallocFuncPtr(MV_HWS_SERDES_TXRX_OVERRIDE_TUNE_PARAMS_ARR_SIZE*sizeof(MV_HWS_SERDES_TXRX_TUNE_PARAMS));
        if (NULL == serdesData->tuneOverrideParams)
        {
            return GT_NO_RESOURCE;
        }
        for (idx  = 0; idx  < MV_HWS_SERDES_TXRX_OVERRIDE_TUNE_PARAMS_ARR_SIZE; idx ++)
        {
            serdesData->tuneOverrideParams[idx].serdesSpeed = LAST_MV_HWS_SERDES_SPEED;
        }
    }
    ...
    {
#ifndef _WIN32
        /*Set NA values for the RX; TX will be set with override values*/
        idx = 0;
        while (tuneParams[idx].serdesSpeed != LAST_MV_HWS_SERDES_SPEED)
        {
            idx++;
        }
        hwsOsMemCopyFuncPtr(&(serdesData->tuneOverrideParams[speedIdx].rxParams),
                            &(tuneParams[idx].rxParams),
                            sizeof(MV_HWS_SERDES_RX_CONFIG_DATA_UNT));
#else
        hwsOsMemSetFuncPtr(&(serdesData->tuneOverrideParams[speedIdx].rxParams),
                           0,
                           sizeof(MV_HWS_SERDES_RX_CONFIG_DATA_UNT));
#endif
    }

    serdesData->tuneOverrideParams[speedIdx].serdesSpeed = serdesSpeed;
    hwsOsMemCopyFuncPtr(&(serdesData->tuneOverrideParams[speedIdx].txParams),
                        txComphyConfigPtr,
...
)
{
    ...
    {
#ifndef _WIN32
        /*Set NA values for the TX; RX will be set with override values*/
        idx = 0;
        while (tuneParams[idx].serdesSpeed != LAST_MV_HWS_SERDES_SPEED)
        {
            idx++;
        ...
        hwsOsMemCopyFuncPtr(&(serdesData->tuneOverrideParams[speedIdx].txParams),
                            &(tuneParams[idx].txParams),
                            sizeof(MV_HWS_SERDES_TX_CONFIG_DATA_UNT));
#else
        /*Set 0 values for the TX; RX will be set with override values*/
        hwsOsMemSetFuncPtr(&(serdesData->tuneOverrideParams[speedIdx].txParams),
                           0,
                           sizeof(MV_HWS_SERDES_TX_CONFIG_DATA_UNT));
#endif
    }
