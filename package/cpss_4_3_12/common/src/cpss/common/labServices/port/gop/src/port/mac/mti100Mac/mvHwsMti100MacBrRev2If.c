...
)
{
    ...
    /* Bug in 400G_IP in Harrier where link_ok & link_ok_clean are swapped ,
       effects USX modes due to 1G WA.
       Force_link_ok_dis/en -> forces the value of link_ok_clean status bit for Harrier */

    if((HWS_DEV_SILICON_TYPE(devNum) == Harrier) &&
       ( HWS_USX_S_MODE_CHECK(portMode) || (portMode == SGMII2_5) || (portMode == SGMII)))
    {
        extField = HAWK_MTIP_EXT_BR_UNITS_STATUS_P0_LINK_OK_CLEAN_E;
    }
    else
    {
...
* @retval 1                        - on error
*/
static GT_STATUS mvHwsMtiMac100BrRev2PortEnableGet
(
    GT_U8                   devNum,
    GT_U32                  portGroup,
    GT_U32                  portMacNum,
