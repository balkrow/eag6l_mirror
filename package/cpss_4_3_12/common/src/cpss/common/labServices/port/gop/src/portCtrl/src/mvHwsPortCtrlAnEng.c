...
)
{
    ...
    if ((AP_CTRL_SPECIAL_SPEED_GET(anSm->options)&& AP_CTRL_SPECIAL_SPEED_MODE_GET(anSm->capability)) || (AN_CTRL_SKIP_RES_GET(anSm->options)))/*106G, 42G, 53G,47G */
    {
        ...
        if ((AP_CTRL_SPECIAL_SPEED_MODE_GET(anSm->capability)) && (AP_CTRL_SPECIAL_SPEED_GET(anSm->options)))
        {
            if(portModesCurr != NULL && AP_CTRL_100GBase_KR4_GET(anSm->capability))/* 106G, 107G */
            {
                if( AP_CTRL_SPECIAL_SPEED_GET(anSm->options) == 1)
                {
                    portModesCurr->serdesSpeed = _27_34375G;
                }
                else if( AP_CTRL_SPECIAL_SPEED_GET(anSm->options) == 2) /* 107G */
                {
                    portModesCurr->serdesSpeed = _27_5G;
                }
                else
                {
                    return GT_FAIL;
            ...
            else if (portModesCurr != NULL && AP_CTRL_40GBase_KR4_GET(anSm->capability))/* 42G/48G_R4 */
            {
                if( AP_CTRL_SPECIAL_SPEED_GET(anSm->options) == 1)
                {
...
)
{
    ...
    if (AN_CTRL_IS_EXT_SIGDET_CB_EN_GET(anSm->options))
    {
        if ((anSm->extendedParams & 1)== 0)
        {
            tuneRes.txTune = TUNE_NOT_READY;
        }
    }
...
)
{
   ...
   /* GT_32 oCoreStat0to15Result2 = 0, dfeStatusRead=0;*/
    ...
    GT_32                       calStatus, hf;
    GT_STATUS                   rc;
    GT_U32                      profile, cnt, calDeltaLimit = 0x5000, txStatus = 0;
    GT_32                       tj;
    ...
    if(cfgHalType == MV_HWS_IPC_PORT_OPERATION_CFG_HAL_SET_CAL_DELTA_LIMIT)
    {
        calDeltaLimit = cfgHalRegisterValue;
    }
    ...
    if ( anSm->status == AN_PORT_SERDES_TRAINING_IN_PROGRESS)
    {
        ...
        /* Execute HWS Training check */
        {
            ...
            {
                if (AN_CTRL_SKIP_RES_GET(anSm->options))
                {
                    for (serdesIndex = 0; serdesIndex < curPortParams.numOfActLanes; serdesIndex++)
                    {
                        mvHwsAvagoSerdesOperation(
                                                    anDevNum,
                                                    anPortGroup,
                                                    serdesIndex,
                                                    NON_SUP_MODE,
                                                    MV_HWS_PORT_SERDES_OPERATION_PRESET_CHECK_E,
                                                    NULL,
                                                    &txStatus);
                        if(txStatus != GT_OK)
                        {
    ...
    if (tuneResult == TUNE_NOT_COMPLITED)
    {
        if (( HWS_PAM4_MODE_CHECK(portMode)) && ( anSm->status == AN_PORT_SERDES_TRAINING_IN_PROGRESS)&&
            ...
            GT_U32 mask = 0x7FFF;
            if ((mvPortCtrlCurrentTs() - anStats->timestampTime)> 4000)
                mask = 0xFFFF;
             ...
             for ( serdesIndex = 0; serdesIndex < curPortParams.numOfActLanes; serdesIndex++ )
             {
                 ...
                 if (((calStatus & mask)) > calDeltaLimit) {
                      if (calDeltaCntr[anSm->portNum] !=0xFF && calDeltaCntr[anSm->portNum] > 0) {
                          calDeltaCntr[anSm->portNum]--;
                          mvPortCtrlLogAdd(AN_PORT_MNG_LOG_DEBUG_SERDES(curPortParams.activeLanesList[serdesIndex],LOG_DEBUG_SERDES_OPERATION, ((calStatus)&0xffff) ));
                          mvPortCtrlAnPortFail("Error, Port-%d TRX training Failed\n",
                                               portIndex, AN_PORT_SERDES_TRAINING_FAILURE);
                          return;
                      }
    ...
    else if (tuneResult == TUNE_PASS)
    {
        ...
        if (( HWS_PAM4_MODE_CHECK(portMode)) && ( anSm->status == AN_PORT_SERDES_TRAINING_IN_PROGRESS)){
            ...
            {
                mvHwsAvagoSerdesSpicoInterrupt(anDevNum, anPortGroup, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[serdesIndex]), 0x2C, 0x11B, &calStatus);
                if (((calStatus & 0xFFFF)) > calDeltaLimit) {
                     status2[anSm->portNum]++; /* count num of retries */
                     if (calDeltaCntr[anSm->portNum] !=0xFF && calDeltaCntr[anSm->portNum] > 0) {
                         calDeltaCntr[anSm->portNum]--;
                         mvPortCtrlLogAdd(AN_PORT_MNG_LOG_DEBUG_SERDES(curPortParams.activeLanesList[serdesIndex],LOG_DEBUG_SERDES_OPERATION, (calStatus&0xFFFF)));
                         mvPortCtrlAnPortFail("Error, Port-%d TRX training Failed\n",
                ...
                /* this is a bug - could be here forever*/
                if ( (calStatus & 0x8000))
                {
                    mvPortCtrlLogAdd(AN_PORT_MNG_LOG_DEBUG_SERDES(curPortParams.activeLanesList[serdesIndex],LOG_DEBUG_SERDES_OPERATION, status2[anSm->portNum]));
                    mvPortCtrlAnPortFail("Error, Port-%d TRX training Failed\n",
                                         portIndex, AN_PORT_SERDES_TRAINING_FAILURE);
                    return;
                }
            }
            status2[anSm->portNum] = 0;
            for ( serdesIndex = 0; serdesIndex < curPortParams.numOfActLanes; serdesIndex++ )
            {
...
)
{
    ...
    GT_32                           dfeCommon, gainShape1,gainShape2;
    ...
    if (anSm->status == AN_PORT_SERDES_CONFIGURATION_IN_PROGRESS)
    {
        ...
        if ( HWS_PAM4_MODE_CHECK(portMode)) {
            ...
            else
            {
                ...
                /* in the last cycle change also the gain value*/
                mvHwsAvago16nmSerdesHalGet(anDevNum, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[0]), HWS_SERDES_RXEQ_CTLE,0x4,&gainShape1);
                mvHwsAvago16nmSerdesHalGet(anDevNum, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[0]), HWS_SERDES_RXEQ_CTLE,0x5,&gainShape2);
                switch (calDeltaCntr[anSm->portNum] )
                {
                case 3:
                    /*gainShape1*/
                    gainShape2 -= 1;
                ...
                for ( serdesIndex = 0; serdesIndex < curPortParams.numOfActLanes; serdesIndex++ )
                {
                    mvPortCtrlLogAdd(AN_PORT_MNG_LOG_DEBUG(anSm->portNum,7, gainShape1, gainShape2));
                    /*gain shape 2*/
                    mvHwsAvago16nmSerdesHalSet(anDevNum, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[serdesIndex]), HWS_SERDES_RXEQ_CTLE,0x4,gainShape1);
                    mvHwsAvago16nmSerdesHalSet(anDevNum, MV_HWS_SERDES_NUM(curPortParams.activeLanesList[serdesIndex]), HWS_SERDES_RXEQ_CTLE,0x5,gainShape2);
                    /* apply CTLE values */
