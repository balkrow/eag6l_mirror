...
CPSS_ADDITION_START
    if ((MCESD_U32)(value << fieldPtr->loBit) > fieldPtr->mask)
    {
        MCESD_DBG_INFO("reg         = 0x%x\n", fieldPtr->reg       );
        MCESD_DBG_INFO("hiBit       = 0x%x\n", fieldPtr->hiBit     );
        MCESD_DBG_INFO("loBit       = 0x%x\n", fieldPtr->loBit     );
        MCESD_DBG_INFO("totalBits   = 0x%x\n", fieldPtr->totalBits );
        MCESD_DBG_INFO("mask        = 0x%x\n", fieldPtr->mask      );
        MCESD_DBG_INFO("value        = 0x%x\n", value      );
        MCESD_DBG_INFO("combinedValue        = 0x%x\n", combinedValue      );
        return MCESD_FAIL;
    }
CPSS_ADDITION_END
