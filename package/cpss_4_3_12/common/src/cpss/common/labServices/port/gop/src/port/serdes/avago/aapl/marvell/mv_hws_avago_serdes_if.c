...
)
{
    ...
    {
        PRV_AAPL_SERDES_DB[chipIndex]->sbus_rings = 2;
    }
    else
    {
        PRV_AAPL_SERDES_DB[chipIndex]->sbus_rings = 1;
    }
    /* Register User-supplied Communication Functions */
    aapl_register_sbus_fn(PRV_AAPL_SERDES_DB[chipIndex], user_supplied_sbus_function, 0, 0);

    /*reset devNumber and portGroup*/
    PRV_AAPL_SERDES_DB[chipIndex]->devNum = devNum;
    PRV_AAPL_SERDES_DB[chipIndex]->portGroup = 0;
...
{
    *devAddr = (GT_UINTPTR)(PRV_AAPL_SERDES_DB[chipIndex]);
...
)
{
    ...
    return (PRV_AAPL_SERDES_DB[chipIndex] == NULL) ? GT_NOT_INITIALIZED : GT_OK;
}
...
)
{
    ...
    aapl_connect(PRV_AAPL_SERDES_DB[chipIndex], ip_address, tcp_port);
    if(PRV_AAPL_SERDES_DB[chipIndex]->return_code < 0)
    {
        hwsOsPrintf("aapl_connect failed (return code 0x%x)\n", PRV_AAPL_SERDES_DB[chipIndex]->return_code);
...
)
{
    ...
    avago_aacs_server(PRV_AAPL_SERDES_DB[chipIndex], tcpPort);
...
)
{
    ...
    {
        if ((endDevNum > (PRV_HWS_NUMBER_OF_RAVENS-1)) || (startDevNum > endDevNum))
        {
            return GT_BAD_PARAM;
        }
    }
...
static int mvHwsAvagoSpicoLoad(unsigned char devNum, unsigned int chipIndex, unsigned int sbus_addr)
{
    ...
    if (HWS_DEV_SERDES_TYPE(devNum, 0 /* serdesNum*/) == AVAGO_16NM)
    {
        ...
        if((PRV_SHARED_SERDES_DIR_SERDES_IF_SRC_GLOBAL_VAR_GET(serdesExternalFirmware))
           && (avago_load_rom_from_file(PRV_AAPL_SERDES_DB[chipIndex],"serdes_fw//avago_16nm//serdes.fw.rom",&serdesFwSize,(int**)&serdesFwPtr) == 0))
        {
    ...
    else
    {
        ...
        if((PRV_SHARED_SERDES_DIR_SERDES_IF_SRC_GLOBAL_VAR_GET(serdesExternalFirmware))
           && (avago_load_rom_from_file(PRV_AAPL_SERDES_DB[chipIndex],"serdes_fw//avago//serdes.fw.rom",&serdesFwSize,(int**)&serdesFwPtr) == 0))
        {
    ...
    avago_spico_upload(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, bist, serdesFwSize, serdesFwPtr);
    if (PRV_AAPL_SERDES_DB[chipIndex]->return_code < 0)
    {
        hwsOsPrintf("mvHwsAvagoSpicoLoad: Loading Spico FW Load Failed on Raven_%d, (sbus_addr=%d), (return code 0x%x)\n", chipIndex, sbus_addr, PRV_AAPL_SERDES_DB[chipIndex]->return_code);
        return GT_INIT_ERROR;
    }
#ifndef MICRO_INIT
    if (memAllocated)
    {
        aapl_free(PRV_AAPL_SERDES_DB[chipIndex], (int*)serdesFwPtr, __func__);
    }
#endif
    return GT_OK;
...
static int mvHwsAvagoSbusMasterLoad(unsigned char devNum, unsigned int chipIndex, unsigned int sbus_addr)
{
    ...
    avago_spico_upload(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, bist, sbusMasterFwSize, sbusMasterFwPtr);
    if (PRV_AAPL_SERDES_DB[chipIndex]->return_code < 0)
    {
        hwsOsPrintf("Loading Sbus Master Avago FW Failed (chipIndex=%d) (return code 0x%x)\n", chipIndex, PRV_AAPL_SERDES_DB[chipIndex]->return_code);
        return GT_INIT_ERROR;
    }
#ifndef MICRO_INIT
    if (HWS_DEV_SERDES_TYPE(devNum, 0 /* serdesNum*/) == AVAGO)
    {
        ...
        avago_spico_upload_swap_image(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, swapFw28nmSize, serdesFw28nmSwapPtr);
        if (PRV_AAPL_SERDES_DB[chipIndex]->return_code < 0)
        {
    ...
    {
        AVAGO_DBG(("Loading SWAP file to SBus Master address: 0x%x  data[0]=%x\n", sbus_addr , serdesFw28nmSwapPtr[0]));
        avago_spico_upload_swap_image(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, swapFwSize, serdesFwDataSwapPtr);
        if (PRV_AAPL_SERDES_DB[chipIndex]->return_code < 0)
        {
            hwsOsPrintf("Loading Sbus Master SWAP Avago FW Failed (return code 0x%x)\n", PRV_AAPL_SERDES_DB[chipIndex]->return_code);
            return GT_INIT_ERROR;
...
int mvHwsAaplSerdesDbInit(unsigned char devNum, unsigned int chipsInDevBmp)
{
    ...
    for (chipInd = 0; chipInd < numOfChipsInDev; chipInd++)
    {
        ...
        {
            if ( NULL == PRV_AAPL_SERDES_DB[chipInd + devNum * numOfChipsInDev] )
            {
                PRV_AAPL_SERDES_DB[chipInd + devNum * numOfChipsInDev] = (Aapl_t*)cpssOsMalloc(sizeof(Aapl_t));
            }
        }
    }
...
/**
* @internal mvHwsAvagoSerdesInit2 function
* @endinternal
*
* @brief   Initialize Avago related configurations
*
...
* @retval 1                        - on error
*/
int mvHwsAvagoSerdesInit2(unsigned char devNum, unsigned int chipIndex)
{
#if !defined(MV_HWS_REDUCED_BUILD_EXT_CM3) || defined(MICRO_INIT)
    unsigned int chip=0;    /* we have only one chip per PRV_AAPL_SERDES_DB[devNum] */
...
#endif
    {
        ...
        for (ring=0 ; ring < PRV_AAPL_SERDES_DB[chipIndex]->sbus_rings ; ring++)
        {
...
int mvHwsAvagoSerdesInit(unsigned char devNum, unsigned int chipIndex)
{
    ...
    /* Validate AAPL */
#ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
    if (PRV_AAPL_SERDES_DB[chipIndex] == NULL)
    {
   ...
   if (PRV_AAPL_SERDES_DB[chipIndex] != NULL)
...
#ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
    aapl_init(PRV_AAPL_SERDES_DB[chipIndex]);
#else
    aapl_init(&(aaplSerdesDbDef[chipIndex]));
    PRV_AAPL_SERDES_DB[chipIndex] = &(aaplSerdesDbDef[chipIndex]);
#endif
    PRV_AAPL_SERDES_DB[chipIndex]->devNum    = devNum;
    PRV_AAPL_SERDES_DB[chipIndex]->portGroup = 0;
    PRV_AAPL_SERDES_DB[chipIndex]->chipIndex = chipIndex;
    ...
    user_supplied_sbus_soft_reset(PRV_AAPL_SERDES_DB[chipIndex]);
#ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
    }
#endif
    ...
    if(regAddr != 0xFFFFFFFF)
    {
        ...
        CHECK_STATUS_EXT(hwsRegisterSetFuncPtr(devNum, PRV_AAPL_SERDES_DB[chipIndex]->portGroup, regAddr, sbusClkDiv/*0x3*/, 0xFFFF), LOG_ARG_NONE_MAC );
    }
    ...
    if(regAddr1 != 0xFFFFFFFF)
    {
        if((HWS_DEV_SILICON_TYPE(devNum) == Raven) || (HWS_DEV_SILICON_TYPE(devNum) == Falcon))
        {
            ...
            CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, PRV_AAPL_SERDES_DB[chipIndex]->portGroup, regAddr1, 0x53, 0xFFFF));
        }
        else
        {
            /* Change the Sensor clk divider to 78 */
            CHECK_STATUS(hwsRegisterSetFuncPtr(devNum, PRV_AAPL_SERDES_DB[chipIndex]->portGroup, regAddr1, 0x4E, 0xFFFF));
        }
    }
    ...
    {
        PRV_AAPL_SERDES_DB[chipIndex]->communication_method = AAPL_DEFAULT_COMM_METHOD;
    ...
    {
        PRV_AAPL_SERDES_DB[chipIndex]->communication_method = AVAGO_USER_SUPPLIED_I2C;
        #ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
    ...
    {
        PRV_AAPL_SERDES_DB[chipIndex]->communication_method = AVAGO_USER_SUPPLIED_SBUS_DIRECT;
    }
    else
    {
        #ifndef MV_HWS_REDUCED_BUILD_EXT_CM3
        hwsOsPrintf("mvHwsAvagoSerdesInit: unknown communication method %x\n", PRV_AAPL_SERDES_DB[chipIndex]->communication_method);
        #endif
        return GT_INIT_ERROR;
    }
...
#if !defined(MV_HWS_REDUCED_BUILD_EXT_CM3) || defined(MICRO_INIT)
    aapl_get_ip_info(PRV_AAPL_SERDES_DB[chipIndex],1);
#endif
    ...
    if (HWS_DEV_SILICON_TYPE(devNum) == Raven || HWS_DEV_SILICON_TYPE(devNum) == Falcon)
    {
        PRV_AAPL_SERDES_DB[chipIndex]->jtag_idcode[addr_struct.chip] = 0x0000057f;
    }
    else
    {
        PRV_AAPL_SERDES_DB[chipIndex]->jtag_idcode[addr_struct.chip] = 0x0912557f;
    }
...
#if !defined(MV_HWS_REDUCED_BUILD_EXT_CM3) || defined(MICRO_INIT)
    {
        for (ring=0 ; ring < PRV_AAPL_SERDES_DB[chipIndex]->sbus_rings ; ring++)
        {
            ...
            {
                for( curr_adr = 0; curr_adr <= PRV_AAPL_SERDES_DB[chipIndex]->max_sbus_addr[chip][ring]; curr_adr++ )
                {
                    if (PRV_AAPL_SERDES_DB[chipIndex]->ip_type[chip][ring][curr_adr] == AVAGO_SERDES)
                    {
...
if (system_recovery.systemRecoveryProcess != CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
    {
        if ( (HWS_DEV_SILICON_TYPE(devNum) == BobK) || (HWS_DEV_SILICON_TYPE(devNum) == Aldrin) )
        {
            ...
            PRV_AAPL_SERDES_DB[chipIndex]->ip_rev[0][0][254] = 0xbe;
            PRV_AAPL_SERDES_DB[chipIndex]->ip_type[0][0][253] = 0x3;
            PRV_AAPL_SERDES_DB[chipIndex]->lsb_rev[0][0][253] = 0xff;
            PRV_AAPL_SERDES_DB[chipIndex]->lsb_rev[0][0][255] = 0x4;
            PRV_AAPL_SERDES_DB[chipIndex]->spico_running[0][0][253]=0x1;
        }
        else
            if ( (HWS_DEV_SILICON_TYPE(devNum) == Raven) || (HWS_DEV_SILICON_TYPE(devNum) == Falcon) )
            {
                PRV_AAPL_SERDES_DB[chipIndex]->spico_running[0][0][253]=0x1;
                PRV_AAPL_SERDES_DB[chipIndex]->ip_type[0][0][253] = 0x3;
                PRV_AAPL_SERDES_DB[chipIndex]->lsb_rev[0][0][253] = -1;
            }
    }
#endif /*MICRO_INIT*/
...
)
{
    ...
    if (result == NULL)
    {
        avago_spico_int(PRV_AAPL_SERDES_DB[chipIndex], sbusAddr, interruptCode, interruptData);
    }
    else
    {
#ifndef BOBK_DEV_SUPPORT
        *result = SIGNED16_TO_SIGNED32(avago_spico_int(PRV_AAPL_SERDES_DB[chipIndex], sbusAddr, interruptCode, interruptData));
#else
        *result = avago_spico_int(PRV_AAPL_SERDES_DB[chipIndex], sbusAddr, interruptCode, interruptData);
#endif
    }
...
)
{
    ...
    AVAGO_LOCK(devNum, chipIndex);
    *mode = 0x70 == (avago_serdes_mem_rd(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, AVAGO_ESB, 0x0f8) & 0x70);
    AVAGO_UNLOCK(devNum, chipIndex);
    CHECK_AVAGO_RET_CODE();
...
)
{
    ...
    if ((miliSeconds > lastTimeStamp + timeInterval) || (miliSeconds < lastTimeStamp))
    {
        ...
        temperatureGlobal = avago_sensor_get_temperature(PRV_AAPL_SERDES_DB[chipIndex], sensor_addr, 0, 0);
...
#else
        temperatureGlobal = avago_sbm_get_temperature(PRV_AAPL_SERDES_DB[chipIndex], sensor_addr, 0);
#endif
        AVAGO_UNLOCK(devNum, chipIndex);
#if defined(MV_HWS_FREE_RTOS)
   ...
   GT_U32 sbus_addr;
    ...
    if (powerUp == GT_FALSE)
    {
        ...
        for (i = 0; i < numOfSer; i++)
        {
            ...
            avago_sbus_wr(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, 0x7, 0x0);

            /* 0x7 , 0x2
            enable the spico which is now at init state (like after loading the spico FW). */
            avago_sbus_wr(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, 0x7, 0x2);

            avago_serdes_set_tx_rx_enable(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, GT_FALSE, GT_FALSE, GT_FALSE);
            if (PRV_AAPL_SERDES_DB[chipIndex]->return_code != 0)
            {
                PRV_AAPL_SERDES_DB[chipIndex]->return_code = 0;
            }
        }
        AVAGO_UNLOCK(devNum, chipIndex);
...
/* Moved to higher function */
    ...
    if (serdesConfigPtr->serdesType == AVAGO_16NM)
    {
        ...
        AVAGO_LOCK(devNum, chipIndex);
        errors = avago_parallel_serdes_init(PRV_AAPL_SERDES_DB[chipIndex],&addr_struct[0],config);
        if ((errors > 0) || (PRV_AAPL_SERDES_DB[chipIndex]->return_code != 0))
        {
...
#endif
    {
        ...
        for (i = 0; i < numOfSer; i++)
        {
            ...
            errors = avago_serdes_init(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, config);
            if ((errors > 0) || (PRV_AAPL_SERDES_DB[chipIndex]->return_code != 0))
            {
                AVAGO_UNLOCK(devNum, chipIndex);
                CHECK_AVAGO_RET_CODE();
    ...
    if (hwsDeviceSpecInfo[devNum].serdesType == AVAGO_16NM && !isGenericPwcl )
    {
        ...
        for (i = 0; i < numOfSer; i++)
        {
            ...
            /* disable ILB */
            avago_serdes_set_rx_input_loopback(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, FALSE);
        }
        AVAGO_UNLOCK(devNum, chipIndex);
        CHECK_AVAGO_RET_CODE();
#endif
...
)
{
    ...
    if (encoding == SERDES_ENCODING_PAM4) {
        serdesConfig.encoding = (MV_HWS_SERDES_ENCODING_TYPE)AVAGO_SERDES_PAM4;
    } else {/* SERDES_ENCODING_NRZ */
        serdesConfig.encoding = (MV_HWS_SERDES_ENCODING_TYPE)AVAGO_SERDES_NRZ;
    }
    ...
    config->tx_divider = config->rx_divider = divider;
    config->skip_crc = 1;
    config->rx_encoding = config->tx_encoding = (Avago_serdes_line_encoding_t)serdesConfig.encoding;
...
)
{
    ...
    if (errors == 0 && aapl_get_return_code(PRV_AAPL_SERDES_DB[chipIndex]) == 0)
    {
#ifndef MV_HWS_REDUCED_BUILD
        AVAGO_DBG(("The SerDes at address %s is initialized.\n", aapl_addr_to_str(sbus_addr)));
#else
...
)
{
    ...
    CHECK_STATUS(mvHwsAvagoConvertSerdesToSbusAddr(devNum, serdesNum, &sbus_addr));
     if (reset == GT_FALSE)
        val = 2;
    AVAGO_LOCK(devNum, chipIndex);
    avago_sbus_wr(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, 0x7, val);
    AVAGO_UNLOCK(devNum, chipIndex);
    return GT_OK;
}
...
)
{
    ...
    chipIndex = mvHwsAvagoGetAaplChipIndex(devNum, serdesNum);
    CHECK_STATUS(mvHwsAvagoConvertSerdesToSbusAddr(devNum, serdesNum, &sbus_addr));
    configp = avago_serdes_eye_config_construct(PRV_AAPL_SERDES_DB[chipIndex]);
    if (configp == NULL) {
        rc = GT_BAD_PTR;
        goto return_error;
    }
    ...
    datap = avago_serdes_eye_data_construct(PRV_AAPL_SERDES_DB[chipIndex]);
    if (datap == NULL) {
        avago_serdes_eye_config_destruct(PRV_AAPL_SERDES_DB[chipIndex],configp);
        rc = GT_BAD_PTR;
        goto return_error;
    }
    if (hwsDeviceSpecInfo[devNum].serdesType != AVAGO_16NM)
    {
        ...
        {
            avago_serdes_eye_data_destruct(PRV_AAPL_SERDES_DB[chipIndex],datap);
            avago_serdes_eye_config_destruct(PRV_AAPL_SERDES_DB[chipIndex],configp);
            rc = GT_FAIL;
            goto return_error;
        }
    ...
    AVAGO_LOCK(devNum, chipIndex);
    errors = avago_serdes_eye_get(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, configp, datap);
    return_code = PRV_AAPL_SERDES_DB[chipIndex]->return_code;
    AVAGO_UNLOCK(devNum, chipIndex);

    PRV_AAPL_SERDES_DB[chipIndex]->return_code = return_code;
    if ((GT_OK != errors) ||
        (return_code < 0))
    {
        avago_serdes_eye_data_destruct(PRV_AAPL_SERDES_DB[chipIndex],datap);
...
)
{
    ...
    AVAGO_LOCK(devNum, chipIndex);
    /* Tx polarity En */
    avago_serdes_set_tx_invert(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, invertTx);
    AVAGO_UNLOCK(devNum, chipIndex);
    CHECK_AVAGO_RET_CODE();

    AVAGO_LOCK(devNum, chipIndex);
    /* Rx Polarity En */
    avago_serdes_set_rx_invert(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, invertRx);
    AVAGO_UNLOCK(devNum, chipIndex);
    CHECK_AVAGO_RET_CODE();
#endif /* ASIC_SIMULATION */
    return GT_OK;
...
)
{
    ...
    *invertTx = avago_serdes_get_tx_invert(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr);
    AVAGO_UNLOCK(devNum, chipIndex);
    CHECK_AVAGO_RET_CODE();

    AVAGO_LOCK(devNum, chipIndex);
    /* Get the RX inverter polarity mode: TRUE - inverter is enabled, FALSE - data is not being inverted */
    *invertRx = avago_serdes_get_rx_invert(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr);
    AVAGO_UNLOCK(devNum, chipIndex);
    CHECK_AVAGO_RET_CODE();
    ...
    uint sbus_addr)
{
    ...
    if (avago_diag_sbus_rw_test(PRV_AAPL_SERDES_DB[chipIndex], avago_make_sbus_controller_addr(sbus_addr), 2) == TRUE)
    {
...
{
#ifndef ASIC_SIMULATION
    GT_U32 interruptCode = 0x126;
    GT_BOOL needRetrain = GT_FALSE;
    MV_HWS_SERDES_CONFIG_STC   serdesConfig;
    MV_HWS_PORT_INIT_PARAMS    curPortParams;
    MV_HWS_AUTO_TUNE_STATUS  txStatus;
    GT_32  i = 0, spicoRes = 0, sumSpicoRes = 0, calStatus, dfeCommon;
    GT_U32 interruptDataArray[INTERRUPT_ARRAY_SIZE] = {0x6400, 0x6500, 0x6600, 0x6700, 0x6800};
#endif
    ...
    switch ( operation ) {
        ...
        case MV_HWS_PORT_SERDES_OPERATION_RESET_RXTX_E:
            ...
            break;
        case MV_HWS_PORT_SERDES_OPERATION_PRESET_CHECK_E:
            /* Read all options of preset 1,2,3, init and only one can be or none */
            for(i = 0; i < INTERRUPT_ARRAY_SIZE; i++)
            {
                CHECK_STATUS(mvHwsAvagoSerdesSpicoInterrupt(
                                                            devNum,
                                                            portGroup,
                                                            serdesNum,
                                                            interruptCode,
                                                            interruptDataArray[i],
                                                            &spicoRes));
                sumSpicoRes += spicoRes;
            }
            txStatus = (sumSpicoRes > 1) ? TUNE_FAIL : TUNE_PASS;
            if(result != NULL)
            {
                *result = txStatus;
            }
...
)
{
    ...
    AVAGO_LOCK(devNum, chipIndex);
     ...
     result = avago_spico_int(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, interruptCode, interruptData);
...
GT_STATUS dbgSbusReset(GT_U32 serdesNum, GT_U32 chipIndex)
{
    ...
    CHECK_STATUS(mvHwsAvagoConvertSerdesToSbusAddr(0, serdesNum, &sbus_addr));
        ...
        avago_sbus_wr(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, 0x7, 0x0);
        avago_sbus_wr(PRV_AAPL_SERDES_DB[chipIndex], sbus_addr, 0x07, 0x2);
        AVAGO_UNLOCK(0, chipIndex);

        return GT_OK;
}

GT_STATUS mvHwsAvagoSerdesDbAllocate
(
    GT_VOID
)
{
    GT_U32 size;
    if(NULL == PRV_AAPL_SERDES_VOID_DB)
    {
        size = sizeof(Aapl_t *)*HWS_MAX_DEVICE_NUM * RAVEN_MAX_AVAGO_CHIP_NUMBER;
        PRV_AAPL_SERDES_VOID_DB = cpssOsMalloc(size);
        if(NULL == PRV_AAPL_SERDES_DB)
        {
            return GT_OUT_OF_CPU_MEM;
        }

        cpssOsMemSet(PRV_AAPL_SERDES_DB,0,size);
    }
...
GT_VOID mvHwsAvagoSerdesDbFree
(
    GT_VOID
)
{
    ...
    if(NULL != PRV_AAPL_SERDES_DB)
    {
        cpssOsFree(PRV_AAPL_SERDES_VOID_DB);
        PRV_AAPL_SERDES_VOID_DB=NULL;
    }
