...
/* MB - TBD - add support for Ironman or make sure not in use here*/
...
#if !defined(BC3_DEV_SUPPORT) && !defined(ALDRIN2_DEV_SUPPORT) && !defined(PIPE_DEV_SUPPORT) && !defined(RAVEN_DEV_SUPPORT) && !defined(FALCON_DEV_SUPPORT) && !defined(IRONMAN_DEV_SUPPORT)
                           ...
                           )
{
    ...
    if (GT_OK != hwsPortModeParamsGetToBuffer(0, 0, portMacNum, portMode, &curPortParams))
    {
        ...
        apCfg.polarityVector = polarity;
        apCfg.nonceDis = GT_TRUE;
#endif
...
#elif defined(HARRIER_DEV_SUPPORT)
        apCfg.apLaneNum = curPortParams.activeLanesList[0];
#elif defined(BOBK_DEV_SUPPORT) || defined(ALDRIN_DEV_SUPPORT)/* if bobk or ALDRIN */
...
#if defined(HARRIER_DEV_SUPPORT)
            AP_CTRL_25GBase_KR1_SET(apCfg.modesVector, 1);
            AP_CTRL_25GBase_KR1S_SET(apCfg.modesVector, 1);
#else
...
#if defined(HARRIER_DEV_SUPPORT)
            AP_CTRL_25GBase_CR1_SET(apCfg.modesVector, 1);
            AP_CTRL_25GBase_CR1S_SET(apCfg.modesVector, 1);
#endif
...
#if defined(HARRIER_DEV_SUPPORT)
        osPrintf("MacNum %d LaneNum %d portAdvMode 0x%x fecAbil %d fecReq %d polarity 0x%x\n",
                 portMacNum, apCfg.apLaneNum, apCfg.modesVector, fecAbil, fecReq, apCfg.polarityVector);
        apCfg.fecAdvanceAbil = fecAbil;
        apCfg.fecAdvanceReq = fecReq;

        /*update hws with ANP params*/
        rc = mvHwsPortAnpStart(0, 0, portMacNum, &apCfg);
#else
