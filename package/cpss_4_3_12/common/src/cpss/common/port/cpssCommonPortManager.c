...
#define PORT_MANAGER_DISABLE_HW_SIGNAL_MASK
...
#define PRV_CPSS_PORT_MANAGER_STATIC_OVER_ANP_MODES_CHECK_MAC(_portMode) ( _1000Base_X == _portMode || SGMII == _portMode || _2500Base_X == _portMode || \
                                                                          _5GBaseR == _portMode || _50GBase_KR2_C == _portMode || _50GBase_CR2_C == _portMode || \
                                                                          _50GBase_KR2 == _portMode || _50GBase_CR2 == _portMode || _53GBase_KR2 == _portMode ||  \
                                                                          _52_5GBase_KR2 == _portMode || _50GBase_SR2 == _portMode || _40GBase_KR2 == _portMode || \
                                                                          _200GBase_CR8 == _portMode || _200GBase_KR8 == _portMode || _200GBase_SR_LR8 == _portMode || \
                                                                          HWS_10G_MODE_CHECK(_portMode) || HWS_25G_MODE_CHECK(_portMode) || HWS_40G_R4_MODE_CHECK(_portMode) || \
                                                                          HWS_100G_R4_MODE_CHECK(_portMode) || HWS_PAM4_MODE_CHECK(_portMode) || HWS_USX_O_MODE_CHECK(_portMode) || \
                                                                          HWS_USX_S_MODE_CHECK(_portMode) || HWS_USX_D_MODE_CHECK(_portMode) || HWS_USX_Q_MODE_CHECK(_portMode) )

/* For port modes that do not require RX training: _1000Base_X, _1000Base_SX, SGMII, SGMII2_5, QSGMII, _5_625GBaseR, _100Base_FX, _2500Base_X and _5GBaseR for SIP_6_10 devices */
#define PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(_portMode) HWS_RX_TRAIN_SKIP_NEEDED_CHECK(_portMode)

/* From port manager perspective, "simple port mode"s are those: */
#define PRV_CPSS_PORT_MANAGER_1000BASE_MODES_CHECK_MAC(_portMode) (_1000Base_X == _portMode || _1000Base_SX == _portMode)

/* Macro for slow port modes */
#define PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(_mode) ((_mode == SGMII) || (_mode == QSGMII) || (_mode == _5G_QUSGMII)|| \
                                        (_mode == _10G_OUSGMII) || PRV_CPSS_PORT_MANAGER_1000BASE_MODES_CHECK_MAC(_mode) || (_mode == SGMII2_5) || (_mode == _2500Base_X))
...
#define PRV_CPSS_PORT_MANAGER_1000BASE_CHECK_MACRO(_mode) ((_mode == CPSS_PORT_INTERFACE_MODE_1000BASE_X_E) || (_mode == CPSS_PORT_INTERFACE_MODE_1000BASE_SXLX_E))

#define PRV_CPSS_PORT_MANAGER_LOW_IFMODE_CHECK_MACRO(_mode) (PRV_CPSS_PORT_MANAGER_1000BASE_CHECK_MACRO(_mode) || (_mode == CPSS_PORT_INTERFACE_MODE_2500BASE_X_E) || (_mode == CPSS_PORT_INTERFACE_MODE_SGMII_E))
...
/* Macro for getting unmask and return error */
#define PRV_PORT_MANAGER_UNMASK_AND_RETURN_ERROR(devNum, tmpPortManagerDbPtr, rc, rc_tmp, funcNameStr)  \
if ( !PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMode) )                                          \
{                                                                                                       \
...
CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] " funcNameStr " returnd %d", devNum, portNum, rc);

/* Macro that show devies using polling method */
/* NOTE!!: removing AC5P/X casuing those issues: CPSS-15230 & CPSS-15235 (with included solutaion in the comments) */
#define PRV_PORT_MANAGER_POLLING_DEVICES(devNum) ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E) ||  \
...
#define PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum) ((PRV_CPSS_SIP_6_CHECK_MAC(devNum)) ||  \
        (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum)))
...
#define  CPSS_PM_CDR_DELAY_START_TIME(pmStats)\
        cpssOsTimeRT(&(pmStats->cdrDelayTimeStampSec),&(pmStats->cdrDelayTimeStampNSec))

#define PRV_PORT_MANAGER_DEFAULT_PARALLEL_DETECT_EN             GT_TRUE
#define PRV_PORT_MANAGER_DEFAULT_PARALLEL_DETECT_RESTART_NUM    15
#define PRV_PORT_MANAGER_DEFAULT_PARALLEL_DETECT_ATTEMPT_NUM    3
...
#define PRV_PORT_MANAGER_CHECK_BOOL_ARR(arr, size, allTrue)\
        {                                                  \
            GT_U8 i;                                       \
            allTrue = GT_TRUE;                             \
            for (i = 0; i < size; i++)                     \
...
#define PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode) \
        ((PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum)) && (!PRV_PORT_MANAGER_POLLING_DEVICES(devNum)) && (portMode == _1000Base_X)) \
...
/**
* @internal prvCpssPortManagerApPortCreate
* @endinternal
*
* @brief  Create AP port
*
* @param[in] devNum                 - device number
* @param[in] portNum                - physical port number
* @param[in] tmpPortManagerDbPtr    - pm database ptr
* @param[in] apParams               - ap port parameters pointer
* @param[in] portMacNum             - port MAC number
* @param[in] portGroup              - group of ports identifier
*
* @retval GT_OK                    - on success
*
*/
static GT_STATUS prvCpssPortManagerApPortCreate
(
    IN GT_SW_DEV_NUM                devNum,
    IN GT_PHYSICAL_PORT_NUM         portNum,
    IN PRV_CPSS_PORT_MNG_DB_STC     *tmpPortManagerDbPtr,
    IN CPSS_PORT_AP_PARAMS_STC      *apParams,
    IN GT_U32                       portMacNum,
    GT_U32                          portGroup
);

/**
* @internal prvCpssPortManagerApEnableTraffic
* @endinternal
*
* @brief  Enable\Disable Traffic from MAC perpsective
*
...
* @param[in] portNum                - physical port number
* @param[in] tmpPortManagerDbPtr    - pm database ptr
* @param[in] enableTraffic          - enable traffic is set to TRUE. Othewise disable traffic
*
* @retval GT_OK                    - on success
*
*/
static GT_STATUS prvCpssPortManagerApEnableTraffic
(
    IN GT_SW_DEV_NUM                   devNum,
    IN GT_PHYSICAL_PORT_NUM            portNum,
    IN PRV_CPSS_PORT_MNG_DB_STC        *tmpPortManagerDbPtr,
    GT_BOOL                            enableTraffic
);

/************************************************
        Code
************************************************/
GT_STATUS prvCpssPortManagerTimeDiffGet
(
    IN  GT_U32 timeStampSec,
    IN  GT_U32 timeStampNSec,
    OUT GT_U32 *timerPtr
)
{
    GT_U32 sec,nsec;
    ...
    secDiff = sec - timeStampSec;
    nsecDiff = nsec - timeStampNSec;
...
/**
* @internal prvCpssPortManagerLowLevelCheckAndSet function
* @endinternal
*
* @brief  API for getting low level interrupts and if found,
*         setting them.
*
* @note  APPLICABLE DEVICES:   xCat3; AC5; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note  NOT APPLICABLE DEVICES: Lion2; Bobcat2.
...
* @param[in] portNum            - physical port number
* @param[in] operation          - mask operation (mask or unmask)
* @param[in] maskedPtr          - pointer with mask
* @param[in] uniEvent           - the event to check and set
*
* @retval GT_OK                - on success
* @retval GT_FAIL              - on error
* @retval GT_NOT_SUPPORTED     - on not supported device
* @retval GT_BAD_PARAM         - on bad physical port number
*/
GT_STATUS prvCpssPortManagerLowLevelCheckAndSet
(
    IN GT_U8                   devNum,
    IN GT_PHYSICAL_PORT_NUM    portNum,
    IN CPSS_EVENT_MASK_SET_ENT operation,
    IN CPSS_EVENT_MASK_SET_ENT *maskedPtr,
    IN CPSS_UNI_EV_CAUSE_ENT   uniEvent
)
{
    GT_STATUS rc;
    rc = cpssEventDeviceMaskWithEvExtDataGet(devNum, uniEvent, portNum, maskedPtr);
    if (rc == GT_NOT_FOUND)
    {
    ...
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataGet operation: %d uniEvent: %d returned %d ", devNum, portNum, operation, uniEvent, rc);
    }
    else
    {
        rc = cpssEventDeviceMaskWithEvExtDataSet(devNum, uniEvent, portNum, operation);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataSet %d uniEvent: %d returned %d ", devNum, portNum, operation, uniEvent, rc);
        }
    }
...
)
{
    ...
    if(PRV_CPSS_PORT_MANAGER_CHECKING_GIGE_PORTS_ON_AC3_BASED_DEVICE(devNum,portNum))
    {
        ...
        rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,portNum,operation,&tmpOperation,CPSS_PP_PORT_SYNC_STATUS_CHANGED_E);
        if (rc != GT_NOT_FOUND && rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataSet %d SYNC_STATUS_CHANGED_E returned %d ", devNum,portNum, operation, rc);
        }
        return GT_OK;
    }
    ...
    rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,portNum,operation,&tmpOperation,CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E);
    if (rc != GT_NOT_FOUND && rc != GT_OK)
    {
    ...
    /* check if CPSS_PP_PORT_LANE_SQ_EVENT_E exists for 'serdesNum' */
    if (!PRV_PORT_MANAGER_POLLING_DEVICES(devNum) &&  (portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E))
    {
        for (i = 0; i < curPortParams.numOfActLanes; i++)
        {
            rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,curPortParams.activeLanesList[i], operation, &tmpOperation,CPSS_PP_PORT_LANE_SQ_EVENT_E);
            if (rc != GT_NOT_FOUND && rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataS%d LANE_SQ_EVENT returned %d ", devNum,portNum, operation, rc);
            }
        }

        if (operation == CPSS_EVENT_MASK_E)
        {
            CPSS_PM_SET_MASKING_BIT(tmpPortManagerDbPtr, portNum, CPSS_PM_PORT_LANE_SQ_EVENT_MASKING_E);
        }
        else
        {
            CPSS_PM_SET_UNMASKING_BIT(tmpPortManagerDbPtr, portNum, CPSS_PM_PORT_LANE_SQ_EVENT_MASKING_E);
        }
    }
    ...
    if (!PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        ...
        rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,portNum,operation,&tmpOperation,CPSS_PP_PORT_PCS_GB_LOCK_SYNC_CHANGE_E);
        if (rc != GT_NOT_FOUND && rc != GT_OK)
        {
        ...
        rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,portNum,operation,&tmpOperation,CPSS_PP_PORT_MMPCS_SIGNAL_DETECT_CHANGE_E);
        if (rc != GT_NOT_FOUND && rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataSet %d MMPCS_SIGNAL_DETECT_CHANGE returned %d ", devNum,portNum, operation, rc);
        }
...
)
{
    ...
    PRV_CPSS_DRV_EV_REQ_NODE_STC *swEvNodePool;
    CPSS_SYSTEM_RECOVERY_INFO_STC tempSystemRecovery_Info;
    GT_U32                        portMacNum;

    if(uniEv == CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)/* allowed also on AC3/5 */
    {
        swEvNodePool = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroup].swEvNodesPool;
        if(swEvNodePool == NULL)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, "swEvNodePool not initialized in device [%d] portGroup[%d]",devNum,portGroup);
        }
        ...
        rc = prvCpssDrvEvReqQInsertSwEvent(&swEvNodePool[portMacNum]);
...
)
{
    ...
    /* Force link down */
    if (!PRV_CPSS_SIP_6_CHECK_MAC(devNum)) /* can't set LF together with RF */
    {
...
/**
* @internal  prvCpssHarrierLoopbackSyncECheck function
* @endinternal
*
* @brief  API that will check on create if port can be
*         configured to RX2TX Loopback, and set the syncE status accordingly.
*        on delete it will set the syncE status for the last
*        port deleted in the bundle.
*
* @note  APPLICABLE DEVICES:   Harrier
*
* @param[in] devNum          - device number
* @param[in] portNum         - physical port number
* @param[in[ curPortParams   - current port params
* @param[in] enable          - enable in create, disable in delete
*
* @retval GT_OK              - on success
* @retval GT_FAIL            - on error
* @retval GT_NOT_SUPPORTED   - on when port can't be configured
* @retval GT_NOT_INITIALIZED - on not initialized functions
*
* @note In the supported devices, RX2TX can't be configured when
*       another port in the bundle is configured to something
*       that is not RX2TX. and non-RX2TX ports can't be
*       configured when someone in the bundle already config
*       with RX2TX. Each time when confining  the ports, in
*       Rx2Tx it's will check that there is non-Rx2Tx in the
*       bundle, otherwise it's will check that all the ports are
*       non-RX2TX. if all configured ok, it's will set the sync
*       E status to disabled, on Delete, the last ports in the
*       bundle when deleted, will set sync E to enabled
*
*/
static GT_STATUS prvCpssHarrierLoopbackSyncECheck
(
    IN  GT_SW_DEV_NUM                            devNum,
    IN  GT_PHYSICAL_PORT_NUM                     portNum,
...
)
{
    ...
    GT_U32                                  portGroup;
    GT_U32                                  errorStatus = 0;
    GT_U32                                  extraOperationDataArray[] = {0 /* create of delete */,0 /* regular or LB*/,0 /* num of active lanes */};

    if (PRV_CPSS_PP_MAC(devNum)->devFamily != CPSS_PP_FAMILY_DXCH_HARRIER_E)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "[Port %d/%2d] Calling prvCpssHarrierLoopbackSyncECheck for unsupported device, return=%d",
                                                   devNum, portNum, GT_NOT_SUPPORTED);
    }
    ...
    speedDb    = tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb;
    portGroup  = tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portGroup;
...
/* The Algorithm state machine on the serdes level
 -----------------------------------------------------------------------------------------
     [Start]
        |
        V
    <any ports cfg?> --no--> <port RX2TX?>  --yes--> <create event?> -- yes --> (set sync E = 1, return OK)
        |                        |                         |
       yes                       no                        no
        |                        |                         |
        V                        V                         V
    <create event?>  --no-->  (return OK)         (set sync E = 0, return OK)
        |                        ^
       yes                       |
        |                        no
        V                        |
<port is RX2TX & sync E = 1 -----|
        or
 port is non-RX2TX & sync E = 0>
        |
       yes
        |
        V
...
*/
    ...
    /* for delete non RX2TX port the sync E already need to enable, no point to execute to enable it again */
    if ((enable == 0 ) && !(PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg)))
    {
        return GT_OK;
    }
    ...
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED, "[Port %d/%2d ppPortExtraOperationsSetFunc is null, return %d", devNum, portNum, GT_NOT_INITIALIZED);
    }

    /* set the data array acording to the state and the LP status */
    if (enable) /* create */
    {
        extraOperationDataArray[0] = 1;
    }
    else /* delete*/
    {
        extraOperationDataArray[0] = 0;
    }

    if (PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg))
    {
        extraOperationDataArray[1] = 1;
    }
    else /* non RX2TX*/
    {
        extraOperationDataArray[1] = 0;
    }

    extraOperationDataArray[2] = curPortParams.numOfActLanes;
    ...
    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                         portGroup, ifModeDb, speedDb, MV_HWS_PORT_SERDES_OPERATION_SYNC_E_CHECK_STATUS_E, extraOperationDataArray, &errorStatus);
    if (rc != GT_OK)
    {
        if (errorStatus != 1)
        {
        ...
        {
             CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] sync E status is incorrect for the port status!", devNum, portNum);
        }
    }
...
/**
* @internal prvCpssPortManagerFecStatusUpdate function
* @endinternal
*
...
*/
GT_STATUS prvCpssPortManagerFecStatusUpdate
(
    GT_SW_DEV_NUM devNum,
    GT_PHYSICAL_PORT_NUM portNum,
    PRV_CPSS_PORT_MNG_DB_STC* tmpPortManagerDbPtr
...
{
    GT_STATUS rc = GT_OK;
    MV_HWS_RSFEC_STATUS_STC     fecStatus;
    CPSS_PORT_FEC_MODE_ENT      fecAbility = CPSS_PORT_FEC_MODE_LAST_E;

    /* not valid for sip 5.20 and below */
    if (!(PRV_CPSS_SIP_6_CHECK_MAC(devNum)))
    {
        return GT_OK;
    }
    ...
    {
        if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppFecModeGetFunc != NULL)
        {
    ...
    {
        fecAbility = tmpPortManagerDbPtr->portsApAttributesDb[portNum]->portResolution.fecStatus;
    }

    if(((fecAbility == CPSS_PORT_RS_FEC_MODE_ENABLED_E) || (fecAbility == CPSS_PORT_RS_FEC_544_514_MODE_ENABLED_E)) &&
       (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRsFecStatusGet != NULL))
    {
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRsFecStatusGet(devNum, portNum, &fecStatus);
        if(rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"     --- Error at ppPortRsFecStatusGet rc:%d---", rc);
        }
    }
    return GT_OK;
}
...
)
{
    ...
    if(PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg))
    {
        if (enable)
        {
            if (loopbackCfg.enableRegularTraffic == GT_FALSE)
            {
...
static GT_STATUS prvCpssPortManagerCallLinkUpCallback
(
    IN  GT_SW_DEV_NUM                            devNum,
    IN  GT_PHYSICAL_PORT_NUM                     portNum,
    IN  GT_BOOL                                  linkUp
)
{
    ...
    GT_U32   portGroup;
    PRV_PORT_MANAGER_GET_PM_DATABASE(devNum, tmpPortManagerDbPtr, rc);
    /* update about link change*/
    if ((( linkUp == GT_TRUE)&& (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM != CPSS_PORT_MANAGER_STATE_LINK_UP_E)) ||
        (( linkUp == GT_FALSE) && (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_UP_E)))
    {
        portGroup = PRV_CPSS_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(CAST_SW_DEVNUM(devNum), portNum);
        /* event to the application (indirect call) option */
        rc = prvCpssPortManagerPushEvent(CAST_SW_DEVNUM(devNum), portGroup,
            portNum,
            portNum,/*extraData*/
            CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E);
        if (rc != GT_OK)
        {
...
/*
* @internal prvCpssPortManagerUsxSerdesTrainStatusGet function
* @endinternal
*
* @brief  Get the Serdes RX tune status for USX port Modes
*
* @note  APPLICABLE DEVICES    : AC5P; AC5X; Harrier; Ironman.
* @note  NOT APPLICABLE DEVICES: Lion2; Bobcat2; Pipe; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon;
*
* @param[in] devNum               - device number
* @param[in] portMacNum           - physical port number
* @param[in] portMode             - port mode
* @param[out] rxTrainStart        - Get status of rx train start for a port in group.
* @param[out] bundleTrainDone     - Get serdes rx train done for the port in group.
*
* @retval GT_OK              - on success
* @retval GT_FAIL            - on error
* @retval GT_NOT_SUPPORTED   - on not supported device
* @retval GT_BAD_PARAM       - on bad physical port number
*/
static GT_STATUS prvCpssPortManagerUsxSerdesTrainStatusGet
(
    IN GT_SW_DEV_NUM         devNum,
    IN GT_PHYSICAL_PORT_NUM  portNum,
    IN MV_HWS_PORT_STANDARD  portMode,
    OUT GT_BOOL              *rxTrainStart,
    OUT GT_BOOL              *bundleTrainDone
)
{
    GT_STATUS rc = GT_OK;
    GT_U32  step = 1, portsInBundle = 1, startPort = 0;
    GT_U32  tmpPort, i;
    GT_U32  portMacNum;
    PRV_CPSS_PORT_MNG_DB_STC  *tmpPortManagerDbPtr;
    ...
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling ppCheckAndGetMacFunc failed=%d ",
                                                    devNum, portNum, rc);
    }

    *rxTrainStart    = GT_FALSE;
    *bundleTrainDone = GT_FALSE;
...
)
{
    ...
    CPSS_PORT_INTERFACE_MODE_ENT    ifMode;
    CPSS_PORT_SPEED_ENT             speed;
    GT_U32                          data, result = 0;
    CPSS_PORT_MANAGER_LOOPBACK_CONFIG_STC   loopbackCfg;
    GT_BOOL                                 anyLoopback;
    MV_HWS_PORT_STANDARD            portMode;
    GT_BOOL skipIcal = GT_FALSE, bundleTrainDone = GT_FALSE;
    ...
    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppLinkStatusChangedNotifyFunc(devNum, portNum, tmpPortManagerDbPtr->portMngSmDb[portNum]->portType, GT_FALSE);
    if (rc != GT_OK)
    {
    ...
    rc = prvCpssPortManagerLoopbackModesDbGet(devNum, portNum, tmpPortManagerDbPtr->portMngSmDb[portNum]->portType, &loopbackCfg, &anyLoopback);
    if (rc != GT_OK)
    {
    ...
    /* update about link change*/
    prvCpssPortManagerCallLinkUpCallback(devNum,portNum,GT_FALSE);
    ...
    rc = prvCpssPortManagerUsxSerdesTrainStatusGet(devNum, portNum, portMode, &skipIcal, &bundleTrainDone);
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling prvCpssPortManagerUsxSerdesTrainStatusGet failed=%d ",
                                                    devNum, portNum, rc);
    }

    if(skipIcal == GT_FALSE)
    {
        if ((PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) || PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E) && !PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))
        {
        ...
        /* reset PCS sd_rx_reset */
        if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            /* Reset PCS Rx */
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortPcsResetSetFunc(devNum,portNum,CPSS_PORT_PCS_RESET_MODE_RX_E,GT_TRUE);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling ppPortPcsResetSetFunc failed=%d ",
                                                        devNum, portNum, rc);
            }
        }
    }

/*Open this code for avoid link up down up before link training is done on both side*/
#if 1
    if ( PRV_CPSS_SIP_6_CHECK_MAC(devNum) )
    {
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_PORT_MANAGER_POLLING_DEVICES(devNum) && (!anyLoopback))
    {
        MV_HWS_PORT_INIT_PARAMS        curPortParams;      /* parameters of current port */
        GT_BOOL                        signalDetect;

        if ((PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode)))
        {
            if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
            {
                GT_U32   dataMufif[] = {4 /* mufi */,5 /* muff */};

                /* Set mufi/f params to default */
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                                 portGroup, ifMode, speed,
                                                                                                 MV_HWS_PORT_SERDES_OPERATION_CDR_VALUES_SET_E, dataMufif, &result);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] could not perform reset rxtx operation", devNum, portNum);
                }
        ...
        if (GT_OK != hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), portGroup, tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMacNum, NON_SUP_MODE, &curPortParams))
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,
                                    "[MacPort %d] hwsPortModeParamsGetToBuffer returned null ", tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMacNum);
        }

        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppSigDetGetFunc(devNum, portNum, &signalDetect);
        if(rc != GT_OK)
        {
             CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppSigDetGetFunc returned error code %d ", devNum, portNum, rc);
        }
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"SignalDetect %s lock",(signalDetect) ? "is" : "not");

        if (signalDetect)
        {
            rc = prvCpssPortManagerPushEvent(CAST_SW_DEVNUM(devNum), portGroup, portNum, curPortParams.activeLanesList[0], CPSS_PP_PORT_LANE_SQ_EVENT_E);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortManagerPushEvent failed=%d", devNum, portNum, rc);
            }
        }
    }
...
)
{
    ...
    **********************************************************/
    ...
    /* for Mac loopback and RX2TX serdes loopback when traffic enable is false, no need to lock align and gearbox */
    if(((PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg)) || (PRV_CPSS_PORT_MANAGER_MAC_LOOPBACK_CHECK(loopbackCfg)))
         && (loopbackCfg.enableRegularTraffic == GT_FALSE))
    {
    ...
    else
    {
        if (unitsBitmap & PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PORT_MANAGER_GB_LOCK_E))
        {
            ...
            if (!(PRV_CPSS_PORT_MANAGER_SPEED_CG_CHECK_MAC(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb) ||
                 (curPortParams.portPcsType == MTI_PCS_10G_LPCS) ||
                 (curPortParams.portPcsType == MTI_USX_PCS_LOW_SPEED) ||
                 (curPortParams.portPcsType == MTI_PCS_LOW_SPEED) ||
        ...
        {
            /* in single lane no need to check for align lock on non sip_6_10 devices*/
            if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) || (curPortParams.numOfActLanes > 1))
            {
...
)
{
    ...
    GT_U32                               step, portsInBundle = 1, startPort, tmpPort, i, portMacNum;
    MV_HWS_PORT_STANDARD                 portMode;
    ...
    if(CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E == systemRecoveryInfo.systemRecoveryProcess)
    {
        ...
        /*
            For USX port modes (non single) need to update rxStabilityTrainDone for
            all ports in SD_group.
        */
        portMacNum = tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMacNum;
        portMode   = tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMode;
        if ( mvHwsUsxModeCheck(devNum,portMacNum,portMode))
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"USX port mode - reset rxStabilityTrainDone for all port in SD_group");
            rc = mvHwsUsxExtSerdesGroupParamsGet(devNum,portMacNum,portMode, &startPort, &step, &portsInBundle);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "serdes group failed=%d",rc);
            ...
            if ( portsInBundle>1 )
            {
                if ( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppConvertMacToPortFunc != NULL)
                {
                    ...
                    {
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppConvertMacToPortFunc(devNum, i, &tmpPort);
                        tmpPortManagerDbPtr->portMngSmDb[tmpPort]->rxStabilityTrainDone = CPSS_PORT_MANAGER_IN_PROGRESS_E;
                    }
                }
            }
        }
...
)
{
    ...
    MV_HWS_PORT_STANDARD                portMode;
    MV_HWS_PORT_INIT_PARAMS             curPortParams;
    MV_HWS_SERDES_TX_CONFIG_DATA_UNT    txDefaultConfig;

    if ((tuneValuesPtr->type == CPSS_PORT_SERDES_COMPHY_C112G_E) || (tuneValuesPtr->type == CPSS_PORT_SERDES_COMPHY_C56G_E))
    {
        PRV_PORT_MANAGER_GET_PM_DATABASE(devNum, tmpPortManagerDbPtr, rc);
        if (portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E)
        {
        ...
        else if (portType == CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E)
        {
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCheckAndGetMacFunc(devNum, portNum, &portMacNum);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC( GT_NOT_INITIALIZED, "[Port %d/%2d] ppCheckAndGetMacFunc returned error", devNum, portNum);
            }

            rc = prvCpssCommonPortIfModeToHwsTranslate(CAST_SW_DEVNUM(devNum),
                        tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[advertisedNum].ifModeDb,
                        tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[advertisedNum].speedDb,
                        &portMode);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC( GT_NOT_INITIALIZED, "[Port %d/%2d] prvCpssCommonPortIfModeToHwsTranslate returned error", devNum, portNum);
            }
        }
        else /* portType is not valid */
        {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC( GT_NOT_INITIALIZED, "[Port %d/%2d] Invalid port type", devNum, portNum);
        }
        if (GT_OK != hwsPortModeParamsGetToBuffer(devNum, 0, portMacNum, portMode, &curPortParams))
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC( GT_NOT_INITIALIZED, "[Port %d/%2d] hwsPortModeParamsGetToBuffer returned error", devNum, portNum );
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
        }

        rc = mvHwsSerdesDbTxConfigGet(devNum, 0, serdesNum, HWS_DEV_SERDES_TYPE(devNum,serdesNum), curPortParams.serdesSpeed, SERDES_DB_DEFAULT_ONLY, &txDefaultConfig);
        if ( rc != GT_OK)
        {
            return rc;
        }
    ...
    case CPSS_PORT_SERDES_COMPHY_C112G_E:
        if (dbValidBitMap & CPSS_PM_DB_LANE_OVERRIDE_TX_E)
        {
            if (PRV_PORT_MANAGER_SERDES_PARAMS_SIGNED_CHECK(tuneValuesPtr->txTune.comphy_C112G.pre2, -9,  9,   NA_16BIT) ||
                ...
                (ABS(((tuneValuesPtr->txTune.comphy_C112G.pre2 == NA_16BIT) ? txDefaultConfig.txComphyC112G.pre2 : tuneValuesPtr->txTune.comphy_C112G.pre2)
                    + ((tuneValuesPtr->txTune.comphy_C112G.pre == NA_16BIT) ? txDefaultConfig.txComphyC112G.pre : tuneValuesPtr->txTune.comphy_C112G.pre)
                    + ((tuneValuesPtr->txTune.comphy_C112G.post == NA_16BIT) ? txDefaultConfig.txComphyC112G.post : tuneValuesPtr->txTune.comphy_C112G.post)
                    + ((tuneValuesPtr->txTune.comphy_C112G.main == NA_16BIT) ? txDefaultConfig.txComphyC112G.main : tuneValuesPtr->txTune.comphy_C112G.main)) > 63))
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "[Port %d/%2d] TX param out of range for device", devNum, portNum);
            }
        }
    ...
    case CPSS_PORT_SERDES_COMPHY_C56G_E:
        if (dbValidBitMap & CPSS_PM_DB_LANE_OVERRIDE_TX_E)
        {
            if (PRV_PORT_MANAGER_SERDES_PARAMS_SIGNED_CHECK(tuneValuesPtr->txTune.comphy_C56G.pre2, -9,  9,  NA_16BIT) ||
                ...
                (ABS(((tuneValuesPtr->txTune.comphy_C56G.pre2 == NA_16BIT) ? txDefaultConfig.txComphyC56G.pre2 : tuneValuesPtr->txTune.comphy_C56G.pre2)
                    + ((tuneValuesPtr->txTune.comphy_C56G.pre == NA_16BIT) ? txDefaultConfig.txComphyC56G.pre : tuneValuesPtr->txTune.comphy_C56G.pre)
                    + ((tuneValuesPtr->txTune.comphy_C56G.post == NA_16BIT) ? txDefaultConfig.txComphyC56G.post : tuneValuesPtr->txTune.comphy_C56G.post)
                    + ((tuneValuesPtr->txTune.comphy_C56G.main == NA_16BIT) ? txDefaultConfig.txComphyC56G.main : tuneValuesPtr->txTune.comphy_C56G.main)) > 63))
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_RANGE, "[Port %d/%2d] TX param out of range for device", devNum, portNum);
            }
        }
...
)
{
    ...
    rc = prvCpssPortManagerSerdesValuesCheck(devNum, portNum, advertisedNum, serdesIndex, portType, dbValidBitMap, tuneValuesPtr);
    if (rc != GT_OK)
    {
        return rc;
    }
...
)
{
    ...
    if (rxSet == GT_TRUE)
    {
        ...
        else /*comphy serdes seires*/
        {
            ...
            else if (tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.type == CPSS_PORT_SERDES_COMPHY_C12G_E)
            {
                ...
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.minCap ,  tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.minCap , NA_8BIT,         GT_U8);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.maxCap ,  tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.maxCap , NA_8BIT,         GT_U8);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.minRes ,  tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.minRes , NA_8BIT,         GT_U8);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.maxRes ,  tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.maxRes , NA_8BIT,         GT_U8);
            ...
            {
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dataRate, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dataRate, NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.res1Sel , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.res1Sel , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.res2Sel , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.res2Sel , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.cap1Sel , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.cap1Sel , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.cap2Sel , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.cap2Sel , NA_16BIT,        GT_U16);
                ...
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmufi , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmufi , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmuff , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmuff , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmupi , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmupi , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmupf , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmupf , NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointLargeThresKLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointLargeThresKLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointSmallThresKLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointSmallThresKLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointLargeThresCLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointLargeThresCLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointSmallThresCLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointSmallThresCLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.inxSumfMidpointAdatptiveEnLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.inxSumfMidpointAdatptiveEnLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dfeResF0aHighThresInitLane, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dfeResF0aHighThresInitLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dfeResF0aHighThresEndLane , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dfeResF0aHighThresEndLane, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCap,              tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCap,               NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCapN,             tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCapN,              NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC0,   tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC0,    NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC1,   tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC1,    NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC2,   tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC2,    NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs0,    tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs0,     NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs1,    tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs1,     NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs2,    tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs2,     NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCap1,             tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCap1,              NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.maxCap1,             tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.maxCap1,              NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minRes1,             tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minRes1,              NA_16BIT,        GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.maxRes1,             tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.maxRes1,              NA_16BIT,        GT_U16);
            }
            ...
            {
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current1Sel   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current1Sel,    NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1Sel        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1Sel,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1Extra      , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1Extra,       NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.res1Sel       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.res1Sel,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cap1Sel       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cap1Sel,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cl1Ctrl       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cl1Ctrl,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.enMidFreq     , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.enMidFreq,      NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cs1Mid        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cs1Mid,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rs1Mid        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rs1Mid,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rfCtrl        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rfCtrl,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1TiaSel     , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1TiaSel,      NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1TiaExtra   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1TiaExtra,    NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.hpfRSel1st    , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.hpfRSel1st,     NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current1TiaSel, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current1TiaSel, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl2Tune       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl2Tune,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl2Sel        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl2Sel,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rs2Sel        , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rs2Sel,         NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current2Sel   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current2Sel,    NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cap2Sel       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cap2Sel,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.hpfRsel2nd    , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.hpfRsel2nd,     NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmufi       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmufi,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmuff       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmuff,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmupi       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmupi,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmupf       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmupf,        NA_16BIT, GT_U16);
                ...
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.minCap1       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.minCap1,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.maxCap1       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.maxCap1,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.minRes1       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.minRes1,        NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.maxRes1       , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.maxRes1,        NA_16BIT, GT_U16);
            ...
            {
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cur1Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cur1Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl1Sel   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl1Sel   , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl1Extra , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl1Extra , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.res1Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.res1Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cap1Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cap1Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.enMidfreq, tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.enMidfreq, NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cs1Mid   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cs1Mid   , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rs1Mid   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rs1Mid   , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cur2Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cur2Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl2Sel   , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl2Sel   , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl2TuneG , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl2TuneG , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.res2Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.res2Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cap2Sel  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cap2Sel  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmufi  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmufi  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmuff  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmuff  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmupi  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmupi  , NA_16BIT, GT_U16);
                PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmupf  , tmpPortManagerDbPtr->portsAttributedDb.perSerdesDbPtr[globalSerdesIndex]->portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmupf  , NA_16BIT, GT_U16);
            ...
            rc = mvHwsComphySerdesManualRxDBSet(CAST_SW_DEVNUM(devNum), portGroup, portMacNum, curPortParams.serdesSpeed, globalSerdesIndex, &rxConfig );
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] mvHwsComphySerdesManualRxDBSet failed=%d", devNum, portNum, rc);
            }
#endif
        }
    ...
    else /*tx set*/
    {
        ...
        else /*comphy serdes */
        {
            ...
            rc = mvHwsComphySerdesManualTxDBSet(CAST_SW_DEVNUM(devNum), portGroup, portMacNum, curPortParams.serdesSpeed, globalSerdesIndex, &txConfig );
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] mvHwsComphySerdesManualTxDBSe failed=%d", devNum, portNum, rc);
            }
#endif
        }
    }
...
)
{
    ...
    if (rxSet == GT_TRUE)
    {
        ...
        else if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.type == CPSS_PORT_SERDES_COMPHY_C12G_E)
        {
            ...
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.minCap ,  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.minCap , NA_8BIT,         GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.maxCap ,  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.maxCap , NA_8BIT,         GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.minRes ,  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.minRes , NA_8BIT,         GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC12GP41P2V.maxRes ,  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C12G.maxRes , NA_8BIT,         GT_U8);
        ...
        else if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.type == CPSS_PORT_SERDES_COMPHY_C28G_E)
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dataRate, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dataRate, NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.res1Sel , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.res1Sel , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.res2Sel , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.res2Sel , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.cap1Sel , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.cap1Sel , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.cap2Sel , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.cap2Sel , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.squelch , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.squelch , NA_16BIT_SIGNED, GT_16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmufi , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmufi , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmuff , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmuff , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmupi , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmupi , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.selmupf , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.selmupf , NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointLargeThresKLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointLargeThresKLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointSmallThresKLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointSmallThresKLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointLargeThresCLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointLargeThresCLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointSmallThresCLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointSmallThresCLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dfeResF0aHighThresInitLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dfeResF0aHighThresInitLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.dfeResF0aHighThresEndLane , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.dfeResF0aHighThresEndLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.inxSumfMidpointAdatptiveEnLane, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.inxSumfMidpointAdatptiveEnLane, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCap,              tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCap,               NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCapN,             tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCapN,              NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC0,   tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC0,    NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC1,   tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC1,    NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.sumfBoostTargetC2,   tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.sumfBoostTargetC2,    NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs0,    tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs0,     NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs1,    tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs1,     NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.midpointPhaseOs2,    tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.midpointPhaseOs2,     NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minCap1,             tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minCap1,              NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.maxCap1,             tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.maxCap1,              NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.minRes1,             tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.minRes1,              NA_16BIT,        GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC28GP4.maxRes1,             tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C28G.maxRes1,              NA_16BIT,        GT_U16);


            rc = mvHwsApSerdesRxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_C28GP4X1, &rxConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesRxManualConfigSet failed=%d", portNum, rc);
            }
        }
        else if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.type == CPSS_PORT_SERDES_COMPHY_C112G_E)
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current1Sel   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current1Sel,    NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1Sel        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1Sel,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1Extra      , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1Extra,       NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.res1Sel       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.res1Sel,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cap1Sel       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cap1Sel,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cl1Ctrl       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cl1Ctrl,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.enMidFreq     , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.enMidFreq,      NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cs1Mid        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cs1Mid,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rs1Mid        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rs1Mid,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rfCtrl        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rfCtrl,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1TiaSel     , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1TiaSel,      NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl1TiaExtra   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl1TiaExtra,    NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.hpfRSel1st    , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.hpfRSel1st,     NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current1TiaSel, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current1TiaSel, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl2Tune       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl2Tune,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rl2Sel        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rl2Sel,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.rs2Sel        , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.rs2Sel,         NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.current2Sel   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.current2Sel,    NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.cap2Sel       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.cap2Sel,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.hpfRsel2nd    , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.hpfRsel2nd,     NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmufi       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmufi,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmuff       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmuff,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmupi       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmupi,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.selmupf       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.selmupf,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.squelch       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.squelch,        NA_8BIT,  GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.minCap1       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.minCap1,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.maxCap1       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.maxCap1,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.minRes1       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.minRes1,        NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC112G.maxRes1       , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C112G.maxRes1,        NA_16BIT, GT_U16);

            rc = mvHwsApSerdesRxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_C112GX4, &rxConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesRxManualConfigSet failed=%d", portNum, rc);
            }
        }
        else if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.type == CPSS_PORT_SERDES_COMPHY_C56G_E)
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cur1Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cur1Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl1Sel   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl1Sel   , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl1Extra , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl1Extra , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.res1Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.res1Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cap1Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cap1Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.enMidfreq, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.enMidfreq, NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cs1Mid   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cs1Mid   , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rs1Mid   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rs1Mid   , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cur2Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cur2Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl2Sel   , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl2Sel   , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.rl2TuneG , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.rl2TuneG , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.res2Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.res2Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.cap2Sel  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.cap2Sel  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmufi  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmufi  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmuff  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmuff  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmupi  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmupi  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.selmupf  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.selmupf  , NA_16BIT, GT_U16);
            PRV_PORT_MANAGER_CHECK_AND_SET(rxConfig.rxComphyC56G.squelch  , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.rxTune.comphy_C56G.squelch  , NA_16BIT_SIGNED, GT_16);

            rc = mvHwsApSerdesRxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_N5XC56GP5X4, &rxConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesRxManualConfigSet failed=%d", portNum, rc);
            }
    ...
    else /*tx set*/
    {
        ...
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC28GP4.pre , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C12G.pre,  NA_8BIT, GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC28GP4.peak, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C12G.peak, NA_8BIT, GT_U8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC28GP4.post, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C12G.post, NA_8BIT, GT_U8);

            rc = mvHwsApSerdesTxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_C28GP4X1, &txConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesTxManualConfigSet failed=%d", portNum, rc);
            }
        ...
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC112G.pre2, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C112G.pre2, NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC112G.pre , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C112G.pre,  NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC112G.main, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C112G.main, NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC112G.post, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C112G.post, NA_8BIT_SIGNED, GT_8);

            rc = mvHwsApSerdesTxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_C112GX4, &txConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesTxManualConfigSet failed=%d", portNum, rc);
            }
        ...
        {
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC56G.pre2, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C56G.pre2, NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC56G.pre , tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C56G.pre,  NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC56G.main, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C56G.main, NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC56G.post, tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C56G.post, NA_8BIT_SIGNED, GT_8);
            PRV_PORT_MANAGER_CHECK_AND_SET(txConfig.txComphyC56G.usr,  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perSerdesDb[(advertisedNum * PORT_MANAGER_MAX_LANES) + (laneSerdesIndex)].portSerdesParams.serdesCfgDb.txTune.comphy_C56G.usr,  NA_8BIT_SIGNED, GT_8);

            rc = mvHwsApSerdesTxManualConfigSet(CAST_SW_DEVNUM(devNum),portGroup,portMacNum,
                                                curPortParams.serdesSpeed, curPortParams.activeLanesList[laneSerdesIndex], COM_PHY_N5XC56GP5X4, &txConfig);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %2d] mvHwsApSerdesTxManualConfigSet failed=%d", portNum, rc);
            }
...
)
{
        ...
        if (rxSgnlOk == GT_FALSE)
        {
            ...
            else
            {
                ...
                if ((!PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode)) && (!PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum)))
                {
...
)
{
    ...
    loopbackCfgPtrOut->enableRegularTraffic = GT_FALSE;
    switch (loopbackType)
    {
    case CPSS_PORT_MANAGER_LOOPBACK_TYPE_MAC_E:
...
/* TBD - currently, loopback not supported in AP (portMode needed in this stage in order
  to perform serdes loopback) */
    ...
    portCfgDbPtr->enableRegularTraffic = loopbackCfgPtr->enableRegularTraffic;
...
)
{
    ...
    tmpPortManagerDbPtr->portMngSmDb[portNum]->duplexMode = CPSS_PORT_FULL_DUPLEX_E;
    tmpPortManagerDbPtr->portMngSmDb[portNum]->extSigDetCallbackEnable = GT_FALSE;
    ...
    if (portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E)
    {
        ...
        for (laneIndex=0; laneIndex<curPortParams.numOfActLanes; laneIndex++)
        {
            ...
            {
                mvHwsComphySerdesManualDBClear(devNum, portNum, curPortParams.activeLanesList[laneIndex]);
            }
#endif
        }
...
)
{
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        ...
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortCheckMapValidity(devNum, port, &validity);
            if (validity == GT_FALSE)
            {
                continue;
            }
...
extern GT_STATUS mvHwsExtFecTypeSet
(
    IN GT_U8                       devNum,
    IN GT_U32                      portNum,
    IN MV_HWS_PORT_STANDARD        portMode,
...
)
{
...
#ifdef CHX_FAMILY  /* function mvHwsExtFecTypeSet is in DXCH code (AC5P function) */
    /* need to set FEC parameters for WM before : ppPortApStatusGetFunc() */
        ...
        PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E))
    {
        ...
        rc = prvCpssCommonPortIfModeToHwsTranslate(CAST_SW_DEVNUM(devNum),
            tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[0].ifModeDb,
            tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[0].speedDb,
                                                   &portMode);
        ...
        */
        /* need to be called before : ppPortApStatusGetFunc() */
        CHECK_STATUS(mvHwsExtFecTypeSet(devNum, portMacNum, portMode,
            tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.fecAbilityArr[0] ==
                CPSS_PORT_RS_FEC_544_514_MODE_ENABLED_E ? RS_FEC_544_514 :
            tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.fecAbilityArr[0] ==
                CPSS_PORT_RS_FEC_MODE_ENABLED_E ? RS_FEC :
            tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.fecAbilityArr[0] ==
                CPSS_PORT_FEC_MODE_ENABLED_E ? FC_FEC :
                FEC_OFF
                ));
    }
#endif /*CHX_FAMILY*/
    ...
    {
        CPSS_PM_PORT_ANP_PARAM_STC anpParam;
        CPSS_PORT_SPEED_ENT speed;
        ...
        MV_HWS_PORT_INIT_PARAMS curPortParams;
        MV_HWS_PORT_STANDARD    portMode;
        MV_HWS_PORT_FEC_MODE    fecHws;
        GT_U32                  portGroup = PRV_CPSS_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(CAST_SW_DEVNUM(devNum), portNum);

        cpssOsMemSet(&anpParam, 0, sizeof(CPSS_PM_PORT_ANP_PARAM_STC));
        ...
        rc = prvCpssCommonPortIfModeToHwsTranslate(CAST_SW_DEVNUM(devNum),
                                                   ...
                                                   speed,
                                                   &portMode);
        PRV_CPSS_CONVERT_CPSS_FEC_MODE_VAL_TO_HWS_MAC(apStatus.fecType, fecHws);
        if(GT_OK != hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), portGroup, portMacNum, portMode, &curPortParams))
        {
            return GT_OK;
        }
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "port mode %d last speed:%d apSpeed:%d  last ifmode:%d apIfmode:%d last fec:%d apFec:%d", portMode, speed, apStatus.portMode.speed, ifMode, apStatus.portMode.ifMode, curPortParams.portFecMode, fecHws);

        /*if pizza already configured, make sure it is the same as reported (need to check speed only)*/
        if ((speed != CPSS_PORT_SPEED_NA_HCD_E) && (ifMode != CPSS_PORT_INTERFACE_MODE_NA_HCD_E)&&
            ((apStatus.portMode.speed != speed) || (curPortParams.portFecMode!= fecHws) || (apStatus.portMode.ifMode != ifMode)) &&
            (!CPSS_PORT_SPEED_SPECIAL_MODE(speed)) && (!(tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.portOperationsBitmap & MV_HWS_PORT_SERDES_OPERATION_RX_TRAIN_E)))
        {
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                anpParam.enable = GT_TRUE;
                anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_SKIP_RESET_E, &anpParam);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"ppPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_SKIP_RESET_E) fail");
                }
            }
            ...
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"call ppPortModeSpeedSetFunc(power down) rc %d",rc);

            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                anpParam.enable = GT_FALSE;
                anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_SKIP_RESET_E, &anpParam);
                if (rc != GT_OK)
                {
        ...
        if(speed == CPSS_PORT_SPEED_NA_HCD_E)
        {
            ...
            tmpPortManagerDbPtr->portMngSmDb[portNum]->fecAbility = apStatus.fecType;
             ...
             CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"call ppPortModeSpeedSetFunc(power up) ifmode %d speed %d fec %d rc %d",
                                                        apStatus.portMode.ifMode,apStatus.portMode.speed,apStatus.fecType,rc);
            ...
            if (PRV_CPSS_SIP_6_CHECK_MAC(devNum))
            {
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortPtpParamsSetFunc(devNum,portNum);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortPtpParamsSetFunc failed=%d\n",devNum, portNum,rc);
                }
            }
        }
        else  if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            anpParam.ifMode = apStatus.portMode.ifMode;
            anpParam.speed = apStatus.portMode.speed;

            anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_IFMODE_E;
            anpParam.anpParamBitMask |= CPSS_PM_PORT_ANP_PARAM_SPEED_E;
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PCS_CFG_DONE_E, &anpParam);

            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_PCS_CFG_DONE_E) returned rc %d",devNum, portNum, rc );
            }
        }
...
)
{
    ...
    if (linkUp && (tmpPortManagerDbPtr->portMngSmDb[portNum]->autoNegotiation.inbandEnable && !tmpPortManagerDbPtr->portMngSmDb[portNum]->autoNegotiation.byPassEnable) && PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && (tmpPortManagerDbPtr->portMngSmDb[portNum]->isRemotePort == GT_FALSE))
    {
        portInitInParam.autoNegEnabled = GT_TRUE;
        portInitInParam.portSpeed = prvCpssCommonPortSpeedEnumToHwsSpeedConvert(tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb);

        rc = prvCpssCommonPortIfModeToHwsTranslate(CAST_SW_DEVNUM(devNum), tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->ifModeDb, tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb, &portMode);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppUsxReplicationSetFunc != NULL && (HWS_USX_PORT_MODE_FAMILY_CHECK(portMode) || (portMode == SGMII)))
        {
    ...
    {
        /* update about link change*/
        prvCpssPortManagerCallLinkUpCallback(devNum,portNum,linkUp);
        if (linkUp)
        {
    ...
    if (linkUp)
    {
        ...
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"     --- Link Up ---");
        tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM = CPSS_PORT_MANAGER_STATE_LINK_UP_E;
        prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampNSec, &tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.linkUpTime);
        tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.linkOkCnt++;

        /* clears the fec symbol err and updates the timer for SER  calculation */
        rc = prvCpssPortManagerFecStatusUpdate(devNum, portNum, tmpPortManagerDbPtr);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"     --- Error at prvCpssPortManagerFecStatusUpdate rc:%d---", rc);
        }
    }
    else
    {
...
)
{
    ...
    if (PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum))
    {
        tmpPortManagerDbPtr->portMngSmDb[portNum]->oneShotiCal = GT_TRUE;
    }
...
GT_STATUS prvCpssPortSignalDetectHwMask
(
    IN GT_U8                    devNum,
    IN GT_U32                   portNum,
    IN GT_BOOL                  mask,
...
{
#ifdef PORT_MANAGER_DISABLE_HW_SIGNAL_MASK
    ...
    GT_U32                   portMacNum;
    GT_U32                   portGroup = PRV_CPSS_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(CAST_SW_DEVNUM(devNum), portNum);
    CPSS_EVENT_MASK_SET_ENT  operation, tmpOperation;
    MV_HWS_PORT_INIT_PARAMS  curPortParams;
    GT_U32                   i;
    CPSS_PORT_MANAGER_PORT_TYPE_ENT portType;
    PRV_CPSS_PORT_MNG_DB_STC  *tmpPortManagerDbPtr;
    ...
    if(rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortManagerLoopbackModesDbGet failed", devNum,portNum, rc);
    }
    ...
    if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
    {
        if ( portNum >= 24 && portNum <= 29 )
            CHECK_STATUS(genUnitRegisterSet(devNum, portGroup, MMPCS_UNIT, portMacNum, CHANNEL_CONFIGURATION, (mask<<14), (1<<14)));
    }
    else if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        GT_BOOL enableSignalDetectMask = GT_FALSE;
        GT_U32  step, portsInBundle, startPort;

        if(portMode == NON_SUP_MODE)
        {
            enableSignalDetectMask = GT_TRUE;
        }
        else
        {
            if ( mvHwsUsxModeCheck(devNum,portMacNum,portMode))
            {
                ...
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"mvHwsUsxExtSerdesGroupParamsGet failed for mac_port %d", portMacNum);
                }
                if(portsInBundle == 1)
                {
                    enableSignalDetectMask = GT_TRUE;
        ...
        if (enableSignalDetectMask == GT_TRUE)
        {
            if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesSigDetMaskSet != NULL)
            {
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesSigDetMaskSet(devNum, portNum, mask);
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"%s signal detect",mask? "Masking":"Unmasking");
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"signal detect %s failed",mask ? "mask":"unmask");
                }
            }
            else
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortSerdesSigDetMaskSet is NULL ", devNum,portNum, rc);
            }
            ...
            if(portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E)
            {
                ...
                operation = ((mask == GT_TRUE) ? CPSS_EVENT_MASK_E : CPSS_EVENT_UNMASK_E);
                ...
                for(i = 0; i < curPortParams.numOfActLanes; i++)
                {
                    ...
                    if (rc != GT_NOT_FOUND && rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling prvCpssPortManagerLowLevelCheckAndSet %d LANE_SQ_EVENT returned %d ", devNum,portNum, operation, rc);
                    }
                }
            }
        }
...
)
{
    GT_STATUS                       rc = GT_OK;
    GT_U32                          regValue;
    GT_U32                          extFieldName1;
    GT_U32                          extFieldName2;
    GT_U32                          pcsField1;
    GT_U32                          pcsField2;
    CPSS_PORT_MANAGER_PORT_TYPE_ENT portType;
    ...
    MV_HWS_PORT_STANDARD            portMode;
    MV_HWS_PORT_INIT_PARAMS         curPortParams;
    MV_HWS_UNITS_ID                 unitIdPcs;
    MV_HWS_UNITS_ID                 unitIdExt;
    PRV_CPSS_PORT_MNG_DB_STC  *tmpPortManagerDbPtr;
    ...
    ifMode     = tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->ifModeDb;
    portType   = tmpPortManagerDbPtr->portMngSmDb[portNum]->portType;
    portSpeed  = tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb;
    portMacNum = tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[portNum]->portMacNum;

    if ((portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E ) &&
        (CPSS_PORT_USX_PORT_MODE_CHECK_MAC(ifMode) || (ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E)))
    {
        /*
            WA: FE-4245079
           1. When the link_status/lpcs_link_status change interrupt value is '1',
               read LP_Ability[15]. If the value is '1' - do nothing.
              If the value is '0' - set force_link_ok_dis to '1' and move to Step 2.

           2. If a link_status/lpcs_link_status change interrupt is received - set force_link_ok_dis to '0' and move to Step 1.
        */

        rc = prvCpssCommonPortIfModeToHwsTranslate(devNum,ifMode, portSpeed, &portMode);
        if (rc != GT_OK)
        {
            return rc;
        }
        ...
        switch (curPortParams.portPcsType)
        {
            ...
            case MTI_PCS_100:
            case MTI_PCS_50:
                unitIdPcs =  MTI_LOW_SP_PCS_UNIT;
                pcsField1 = LPCS_UNITS_CONTROL_P0_ANENABLE_E;
                pcsField2 = LPCS_UNITS_PARTNER_ABILITY_P0_LP_NP_E;

                unitIdExt = MTI_EXT_UNIT;
                /* SGMII & SGMII_2.5 based on LPCS */
                if (ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E)
                {
                    if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                    {
                        extFieldName1 = PHOENIX_MTIP_EXT_UNITS_STATUS_P0_LPCS_LINK_STATUS_E;
                    }
                    else
                    {
                        extFieldName1 = HAWK_MTIP_EXT_BR_UNITS_STATUS_P0_LPCS_LINK_STATUS_E;
                    }
                }
                else
                {
                    if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                    {
                        extFieldName1 = PHOENIX_MTIP_EXT_UNITS_STATUS_P0_LINK_STATUS_E;
                    }
                    else
                    {
                        extFieldName1 = HAWK_MTIP_EXT_BR_UNITS_STATUS_P0_LINK_STATUS_E;
                    }
                }

                if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                {
                    extFieldName2 = PHOENIX_MTIP_EXT_UNITS_CONTROL1_P0_FORCE_LINK_OK_DIS_E;
                }
                else
                {
                    extFieldName2 = HAWK_MTIP_EXT_BR_UNITS_CONTROL_P0_FORCE_LINK_OK_DIS_E;
                }
                break;

            case MTI_USX_PCS_LOW_SPEED:
            case MTI_USX_PCS:
                unitIdPcs = MTI_USX_LPCS_UNIT;
                pcsField1 = LPCS_UNITS_CONTROL_P0_ANENABLE_E;
                pcsField2 = LPCS_UNITS_PARTNER_ABILITY_P0_LP_NP_E;

                unitIdExt = MTI_USX_EXT_UNIT;
                /* SGMII, QSGMII, SGMII2_5 are on USX_LPCS */
                if (curPortParams.portPcsType == MTI_USX_PCS_LOW_SPEED)
                {
                    extFieldName1 = MTIP_USX_EXT_UNITS_STATUS_P0_LPCS_LINK_STATUS_E;
                }
                else
                {
                    extFieldName1 = MTIP_USX_EXT_UNITS_STATUS_P0_LINK_STATUS_E;
                }
                extFieldName2 = MTIP_USX_EXT_UNITS_CONTROL1_P0_FORCE_LINK_OK_DIS_E;
                break;

            case MTI_CPU_SGPCS:
                unitIdPcs = MTI_CPU_SGPCS_UNIT;
                pcsField1 = CPU_SGPCS_UNIT_PORT_CONTROL_PORT_AN_ENABLE_E;
                pcsField2 = CPU_SGPCS_UNIT_PORT_PARTNER_ABILITY_PORT_LP_NP_E;

                unitIdExt = MTI_CPU_EXT_UNIT;
                extFieldName1 = MTIP_CPU_EXT_UNITS_STATUS_LINK_STATUS_E;
                extFieldName2 = MTIP_CPU_EXT_UNITS_CONTROL_FORCE_LINK_OK_DIS_E;
                break;
            default:
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
        }

        CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, portMacNum, unitIdPcs, pcsField1, &regValue, NULL));/*ANENABLE*/
        if (regValue == 0x01)
        {
            CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, portMacNum, unitIdExt, extFieldName1, &regValue, NULL));/*LINK_STATUS or LPCS_LINK_STATUS*/
            if (regValue == 0x01)
            {
                CHECK_STATUS(genUnitRegisterFieldGet(devNum, 0, portMacNum, unitIdPcs, pcsField2, &regValue, NULL));/*PARTNER_ABILITY_PORT_LP_NP*/
                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, unitIdExt, extFieldName2, !regValue, NULL));/*FORCE_LINK_OK_DIS*/
            }
            else
            {
                CHECK_STATUS(genUnitRegisterFieldSet(devNum, 0, portMacNum, unitIdExt, extFieldName2, 1, NULL));/*FORCE_LINK_OK_DIS*/
            }
        }
    }
...
GT_STATUS prvCpssPortManagerIsInDebugState
(
    IN  GT_U8                  devNum,
    IN  GT_U32                 portNum,
...
)
{
    ...
    CPSS_NULL_PTR_CHECK_MAC(isInDebugStatePtr);
    *isInDebugStatePtr = GT_FALSE;
    ...
    if (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_DEBUG_E)
    {
        *isInDebugStatePtr = GT_TRUE;
    }
...
)
{
    ...
    cpssOsMemSet(&anpParam, 0, sizeof(CPSS_PM_PORT_ANP_PARAM_STC));
    cpssOsMemSet(&apParams, 0, sizeof(CPSS_PORT_AP_PARAMS_STC));
    ...
    switch (stage)
    {
        ...
        */
        case CPSS_PORT_MANAGER_EVENT_CREATE_AND_DISABLE_E:
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "AP - CPSS_PORT_MANAGER_EVENT_CREATE_AND_DISABLE_E");

            rc = prvCpssPortManagerApPortCreate(devNum, portNum, tmpPortManagerDbPtr, &apParams, portMacNum, portGroup);
            if (GT_OK != rc)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "Create event was failed for port %d in device number %d due %d", portNum, devNum, rc);
                return rc;
            }

            rc = prvCpssPortManagerApEnableTraffic(devNum, portNum, tmpPortManagerDbPtr, GT_FALSE);
            if (GT_OK != rc)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "Create and disable event was failed for port %d in device number %d due %d", portNum, devNum, rc);
                return rc;
            }
        ...
        */
        case CPSS_PORT_MANAGER_EVENT_ENABLE_E:
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "AP - CPSS_PORT_MANAGER_EVENT_ENABLE_E");

            return(prvCpssPortManagerApEnableTraffic(devNum, portNum, tmpPortManagerDbPtr, GT_TRUE));
        ...
        */
        case CPSS_PORT_MANAGER_EVENT_FORCE_LINK_DOWN_E:
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "AP - CPSS_PORT_MANAGER_EVENT_FORCE_LINK_DOWN_E");
            GT_ATTR_FALLTHROUGH;
        ...
        case CPSS_PORT_MANAGER_EVENT_DELETE_E:
            ...
            if((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                ...
                || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E))
            {
                prvCpssPortSignalDetectHwMask(CAST_SW_DEVNUM(devNum), portNum, GT_FALSE, NON_SUP_MODE);
                rc = cpssEventDeviceMaskWithEvExtDataSet(devNum,
                                                      CPSS_PP_PORT_PCS_ALIGN_LOCK_LOST_E,
                                                      portNum, CPSS_EVENT_MASK_E);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataSet %d ALIGN_LOCK_LOST returned %d ", devNum,portNum, CPSS_EVENT_UNMASK_E, rc);
                }
            }
                ...
                (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.status == CPSS_PORT_MANAGER_PARALLEL_DETECT_SUCCESS_E)))
            {
                ...
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_RESET_E, &anpParam);
                if (rc != GT_OK)
                {
            ...
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortApConfigSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                        GT_FALSE, &apParams, 0, GT_FALSE, GT_FALSE);
            if (rc!=GT_OK)
            ...
            if (stage == CPSS_PORT_MANAGER_EVENT_DELETE_E)
            {
                ...
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Port %d in device number %d was deleted",portNum, devNum);
            }
            else
            {
        ...
        case CPSS_PORT_MANAGER_EVENT_DISABLE_E:
            {
                ...
                if (GT_OK != rc)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "Disable event for port %d in device number %d faile due %d", portNum, devNum, rc);
                }
            ...
            if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E || PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
            {
                if(tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.enable)
                {
                       ...
                       (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.status == CPSS_PORT_MANAGER_NO_PARALLEL_DETECT_E))
                    {
                        ...
                        tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.status = CPSS_PORT_MANAGER_PARALLEL_DETECT_IN_PROGRESS_E;
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_INIT_E, &anpParam);
                        if(rc != GT_OK)
                        {
                        ...
                        {
                            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_WAIT_FOR_PROG_E, &anpParam);
                            if(rc != GT_OK)
                            {
                            ...
                            if ( PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E )
                            {
                                ...
                                anpParam.enable = GT_FALSE;
                                anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
                                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_AUTO_NEG_SET_E, &anpParam);
                                if (rc != GT_OK)
                                {
                                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"parallel detect fail to set AN37");
                                    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_RESET_E, &anpParam);
                                    if (rc != GT_OK)
                                    {
                                        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"parallel detect fail to resert back to AN");
                    ...
                    else if(tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.restartCnt > tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.restartMaxNum)
                    {
                        ...
                        {
                            if ( tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.restartCnt == (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.restartMaxNum + PRV_PORT_MANAGER_DEFAULT_PARALLEL_DETECT_ATTEMPT_NUM))
                            {
                        ...
                        else if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.status == CPSS_PORT_MANAGER_PARALLEL_DETECT_AN37_CHECK_E)
                        {
                            ...
                            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"parallel detect peer AN37 is disabled");
                            anpParam.enable = GT_FALSE;
                            anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
                            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_AUTO_NEG_SET_E, &anpParam);
                            if (rc != GT_OK)
        ...
        */
        case CPSS_PORT_MANAGER_EVENT_LOW_LEVEL_STATUS_CHANGED_E:
            if(tmpPortManagerDbPtr->portMngSmDb[portNum]->extSigDetCallbackEnable == GT_TRUE)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"External callback for signal detect: stage=%d ", stage);
                mvHwsPortOverFwExtSigDetIpc(devNum, 0, portMacNum, GT_TRUE);
            }
/* need to enable once signal detect mask is available */
#ifndef PORT_MANAGER_DISABLE_HW_SIGNAL_MASK
            if((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
                || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E))
            {
                ...
                if (rc == GT_OK)
                {
                       ...
                       (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E))
                    {
                        ...
                        {
                            GT_BOOL cdrLockArr[PORT_MANAGER_MAX_LANES];
                            GT_BOOL cdrLock;
                            ...
                            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc(devNum, portNum, cdrLockArr);
                            PRV_PORT_MANAGER_CHECK_BOOL_ARR(cdrLockArr, curPortParams.numOfActLanes, cdrLock);
                            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"CDR %s locked",(cdrLock)?"is":"not");
                            if ( cdrLock == GT_FALSE )
                            {
                                alignLock = GT_FALSE;
                            }
                    ...
                    prvCpssPortSignalDetectHwMask(CAST_SW_DEVNUM(devNum), portNum, alignLock, NON_SUP_MODE);
                }
            }
#endif
            if ( PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) )
        ...
        */
        case CPSS_PORT_MANAGER_EVENT_MAC_LEVEL_STATUS_CHANGED_E:
            ...
            if (!tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperDisableSM)
            {
                if((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
                || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E))
                {
                    linkUp ? (prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampNSec,
                                                             &tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.linkUpTime)) :
                             (tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.linkUpTime = 0);
                }
            ...
            if (linkUp == GT_TRUE)
            {
                ...
                if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
                {
                    if (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.status == CPSS_PORT_MANAGER_PARALLEL_DETECT_IN_PROGRESS_E )
                    {
                        ...
                        anpParam.enable = GT_TRUE;
                        anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_PD_AN37_E, &anpParam);
                        if(rc != GT_OK)
...
)
{
    ...
    *ignoreTrainingPtr = GT_FALSE;
        ...
        (loopbackCfgPtr->loopbackMode.serdesLoopbackMode == CPSS_PORT_SERDES_LOOPBACK_ANALOG_TX2RX_E))
    {
        ...
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"config SERDES_LOOPBACK TX2RX enableRegularTraffic %d ignore training %d portMode %d",loopbackCfgPtr->enableRegularTraffic, *ignoreTrainingPtr, *portModePtr);
    }
    else
    {
...
GT_STATUS prvCpssPortManagerInitializeSignalStabilityFilterPhase
(
    IN  GT_SW_DEV_NUM                          devNum,
    IN  GT_PHYSICAL_PORT_NUM                   portNum,
    IN  MV_HWS_PORT_INIT_PARAMS                *curPortParamsPtr,
...
)
{
    ...
    ****************************************/
    ...
    if (tmpPortManagerDbPtr->portMngSmDb[portNum]->signalDetected == GT_TRUE)
    {
        ...
        if (((curPortParamsPtr->numOfActLanes > 1) && (portStatuses.alignLock == GT_FALSE || portStatuses.gbLock == GT_FALSE)) ||
            ...
            (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && ((portStatuses.alignLock == GT_FALSE) || (portStatuses.gbLock == GT_FALSE))))
        {
            ...
            if(!PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode) && (!ignoreTraining) && (!PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum)))
            {
                rc = prvCpssPortManagerSigStableRxTraining(devNum, portNum);
                if (rc != GT_OK)
                {
    ...
    **********************************************************/
    ...
    if ((!ignoreTraining) && (!PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum)))
    {
...
)
{
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) || (curPortParamsPtr->numOfActLanes > 1))
    {
...
)
{
    /* Variables declarations */
    GT_STATUS                               rc;
    GT_U32                                  portGroup, data, result = 0, portMacNum;
    CPSS_PORT_SPEED_ENT                     speed;
    MV_HWS_PORT_STANDARD                    portMode;
    CPSS_PORT_INTERFACE_MODE_ENT            ifMode;
    GT_BOOL                                 ignoreTraining;
...
/**-------------------------------------Initialize-Set-Up---------------------------------------------------------*/
    ...
    */
    ...
    if(bundleTrainDone == GT_TRUE)
    {
        tmpPortManagerDbPtr->portMngSmDb[portNum]->rxStabilityTrainDone = CPSS_PORT_MANAGER_DONE_E;
    }

    /* for sip6 and ac3 based devices need only 1 shot iCal */
    if ((PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum)) && (tmpPortManagerDbPtr->portMngSmDb[portNum]->oneShotiCal) &&
        (!PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) || CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E == ifMode) && (!ignoreTraining))
    {
        ...
        if ( !skipIcal )
        {
            /* call serdes function before starting the training*/
            if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL && PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"start MV_HWS_PORT_SERDES_OPERATION_PRE_RX_TRAINING_E operation");
                data = curPortParamsPtr->serdesSpeed;
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                    CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_PRE_RX_TRAINING_E, &data, &result);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] could not perform reset rxtx operation", devNum, portNum);
                }
...
/**--------------------------------------------------------------------------------------------------------------*/
    ...
    if ((PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E != ifMode) &&
        (PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg)))
    {
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "config loopback type %d  enableRegularTraffic %d", loopbackCfg.loopbackType, loopbackCfg.enableRegularTraffic);
        rc = prvCpssPortManagerLoopbackModesHwSet(devNum, portNum, ifMode, speed, &loopbackCfg);
        if (rc != GT_OK)
        {
    ...
    if ((tmpPortManagerDbPtr->portMngSmDb[portNum]->rxStabilityTrainDone == CPSS_PORT_MANAGER_IN_PROGRESS_E) && (!ignoreTraining) && (!bundleTrainDone))
    {
        ...
        else
        {
            ...
            /* Check if dtl_clamping_triggered */
            if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
            {
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                    CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_CHECK_CLAMPING_TRIGGERED_E, &data, &result);
                if ( GT_OK != rc )
                {
                    return rc;
                }
            ...
            if ( (CPSS_PORT_SERDES_AUTO_TUNE_FAIL_E == rxTuneStatus) || result )
            {
                if (result)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "DTL Clamping triggered");
        ...
        tmpPortManagerDbPtr->portMngSmDb[portNum]->rxStabilityTrainDone = CPSS_PORT_MANAGER_DONE_E;
        prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampNSec, &tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.rxTrainingTime);

        /* Interconnect change Algorithm */
        if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Execute IC algorithm");
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_IC_E, &data, &result);
            if ( GT_OK != rc )
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_IC_E returned error code %d ", devNum, portNum, rc);
            }
        }

        if( (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL) &&
            (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E || PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E) )
        {
            data = curPortParamsPtr->serdesSpeed;

            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_SET_CTLE_LIMITS_E, &data, &result);
            if ( GT_OK != rc )
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling MV_HWS_PORT_SERDES_OPERATION_SET_CTLE_MARGINS_E returned error code %d ", devNum, portNum, rc);
            }
        }
        ...
        if ( HWS_PAM4_MODE_CHECK(portMode)) {
            ...
            if((PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL) && (PRV_CPSS_SIP_6_CHECK_MAC(devNum)))
            {
                ...
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                                    portGroup, ifMode, speed,
                                                                                                    MV_HWS_PORT_SERDES_OPERATION_DFE_COMMON_E, &data, &needRetrain);
                if (rc != GT_OK)
                {
        ...
        /* unreset PCS sd_rx_reset after training complete */
        if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            /* Unreset PCS Rx */
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortPcsResetSetFunc(devNum, portNum, CPSS_PORT_PCS_RESET_MODE_RX_E, GT_FALSE);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortPcsResetSetFunc failed=%d", devNum, portNum, rc);
            }
        }
...
signalStabilityFilerPhase:
   /* 1000baseX and all slow modes except OUSGMII should go through signal stability check but thats it, should need to throug training and alignment check */
    if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E != ifMode)
    {
        ...
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppSigDetGetFunc(devNum, portNum, &signlDetect);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppSigDetGetFunc(PRV_CPSS_PORT_MANAGER_SIG_DET_E): rc=%d\n",
                                                      devNum, portNum, rc);
        }
        if(signlDetect || PRV_CPSS_PORT_MANAGER_MAC_LOOPBACK_CHECK(loopbackCfg))
        {
    ...
    rc = prvCpssPortManagerInitializeSignalStabilityFilterPhase(devNum, portNum, curPortParamsPtr, tmpPortManagerDbPtr);
    if (rc != GT_OK)
    {
         return rc;
    }
...
tunePhase:
    ...
    if (CPSS_PORT_SERDES_AUTO_TUNE_PASS_E == tuneStatus)
    {
        ...
        if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            /* Auto-Negotiation sequence for slow modes except _2500Base_X
               Note: need to enable AN on 2.5G SGMII for link signalling from the PHY but no speed negotiation */
            if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && ( _2500Base_X != portMode))
            {
                CPSS_PORT_MANAGER_SGMII_AUTO_NEGOTIATION_STC autoNegotiation = tmpPortManagerDbPtr->portMngSmDb[portNum]->autoNegotiation;

                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Configure Auto-Negotiation for slow modes");
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppAutoNeg1GSgmiiFunc(devNum, portNum, ifMode, &autoNegotiation);
                if (rc != GT_OK)
                {
...
alignLockTimerPhase:
        ...
        if (align == GT_TRUE)
        {
            prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampNSec, &tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.alignLockTime);
...
skipStabilityCheck:
            ...
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                rc = prvCpssPortManagerLinkChangeWA(devNum, portNum);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling prvCpssPortManagerLinkChangeWA failed %d ", devNum, portNum, rc);
                }
            }
        ...
        else if (timeout == GT_TRUE)
        {
            ...
            if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_PORT_MANAGER_POLLING_DEVICES(devNum))
            {
                GT_BOOL rxSignalOk = GT_FALSE;

                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppSigDetGetFunc(devNum, portNum, &rxSignalOk);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppSigDetGetFunc failed=%d", devNum, portNum, rc);
                }
...
)
{
    ...
    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
    {
        /* Auto-Negotiation sequence for slow modes except for _2500Base_X
           Note: need to enable AN on 2.5G SGMII for link signalling from the PHY but no speed negotiation */
        if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && ( _2500Base_X != portMode))
        {
            CPSS_PORT_MANAGER_SGMII_AUTO_NEGOTIATION_STC autoNegotiation = tmpPortManagerDbPtr->portMngSmDb[portNum]->autoNegotiation;
...
)
{
    ...
    /* Auto-Negotiation sequence for slow modes except _2500Base_X for SIP_6 devices */
    if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum) || ( _2500Base_X != portMode)))
    {
    ...
    if (!PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))
    {
        ...
        else if (!PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum))
        {
...
)
{
    ...
    if ( onlyPmDelete == GT_FALSE )
    {
        if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
        {
            rc = prvCpssPortSignalDetectHwMask(CAST_SW_DEVNUM(devNum), portNum, GT_FALSE, portMode);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling prvCpssPortSignalDetectHwMask failed %d ", devNum, portNum, rc);
            }

            /* Add the Workaround for Link change */
    ...
    if ( onlyPmDelete == GT_FALSE )
    {
        ...
        /* reset to defualt setting for Auto-Negotiation sequence for slow modes except _2500Base_X for SIP_6 devices */
        if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum) || ( _2500Base_X != portMode)))
        {
            ...
            else if ( PRV_CPSS_PORT_MANAGER_1000BASE_CHECK_MACRO(ifMode) )
            {
                if ( PRV_CPSS_SIP_6_CHECK_MAC(devNum) )
                {
                    autoNegotiationTmp.inbandEnable                 = GT_TRUE;
            ...
            if (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum) && ((SGMII == portMode) || (QSGMII == portMode)))
            {
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortForceLinkPassSetFunc(devNum,portNum,GT_FALSE);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"ppPortForceLinkPassSetFunc returned error code %d ", rc);
                    return rc;
                }
            }
        }
        ...
        if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"on delete check and set sync E setting");
            rc = prvCpssHarrierLoopbackSyncECheck(devNum,portNum, curPortParams,GT_FALSE);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssHarrierLoopbackSyncECheck failed, return %d", devNum, portNum, rc);
            }
        }
    ...
    if ( onlyPmDelete == GT_FALSE )
    {
        ...
        if ((PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortBwTxFifoSetFunc != NULL) && !PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum))
        {
    ...
    if (tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP)
    {
        CPSS_PM_PORT_ANP_PARAM_STC anpParam;
        cpssOsMemSet(&anpParam, 0, sizeof(CPSS_PM_PORT_ANP_PARAM_STC));
        ...
        anpParam.enable = GT_FALSE;
        anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_ENABLE_E;
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_STATIC_OVER_ANP_SET_E, &anpParam);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"ppPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_STATIC_OVER_ANP_SET_E) fail");
        }
        tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP = GT_FALSE;

        cpssOsMemSet(&anpParam, 0, sizeof(CPSS_PM_PORT_ANP_PARAM_STC));

        anpParam.trainingMode = CPSS_PM_PORT_ANP_RX_TRAINING_E;
        anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_TRAINING_MODE_E;
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E, &anpParam);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"ppPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E) fail");
        }
    }
...
/**
* @internal
*           prvCpssPortManagerCdrLockHandling
* @endinternal
*
* @brief  function that handing the CDR lock status according to
*         the Port Manager state machine.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Caelum; Aldrin; Bobcat3; Aldrin2; Lion2; Bobcat2; AC3X.
*
* @param[in] devNum                - physical device number
* @param[in] portNum               - physical port number
* @param[in] tmpPortManagerDbPtr   - port manager DB
*
* @param[out] signalDetectPtr      - port signal status
* @param[out] cdrLockPtr           - port CDR lock status
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong port number or device
* @retval GT_HW_ERROR              - on hardware error
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssPortManagerCdrLockHandling
(
    IN  GT_SW_DEV_NUM                  devNum,
    IN  GT_PHYSICAL_PORT_NUM           portNum,
    IN  PRV_CPSS_PORT_MNG_DB_STC       *tmpPortManagerDbPtr,
    OUT GT_BOOL                        *signalDetectPtr,
...
)
{
    ...
    GT_BOOL                                          cdrLockArr[PORT_MANAGER_MAX_LANES];
    GT_BOOL                                          skipIcal = GT_FALSE, bundleTrainDone = GT_FALSE;
    CPSS_PORT_SERDES_AUTO_TUNE_STATUS_ENT            rxInitStatus;
    CPSS_PORT_SPEED_ENT                              speed;
    CPSS_PORT_INTERFACE_MODE_ENT                     ifMode;
    MV_HWS_PORT_STANDARD                             portMode;
    GT_BOOL                                          anyLoopback;
    ...
    /* check port signal*/
    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppSigDetGetFunc(devNum, portNum, signalDetectPtr);
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] signal detcet faild", devNum, portNum);
    }

    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Signal %s up",(*signalDetectPtr)?"is":"not");

    /* when there is no singal, no need to handle CDR status*/
    if (*signalDetectPtr == GT_FALSE)
    {
        return GT_OK;
    }

    /* get port current params */
    if (GT_OK != hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), portGroup, portMacNum, portMode, &curPortParams))
    {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_FAIL,
                                   "[MacPort %d] hwsPortModeParamsGetToBuffer returned null ", portMacNum);
    ...
    /* prevent changing align lock indication because of sq_events during mac loopback without cables */
    rc = prvCpssPortManagerLoopbackModesDbGet(devNum, portNum, portType, &loopbackCfg, &anyLoopback);
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortManagerLoopbackModesDbGet failed=%d",
    ...
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling prvCpssPortManagerUsxSerdesTrainStatusGet failed=%d ", devNum, portNum, rc);
    }

    /*RX_INIT*/
    if (( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRxInitFunc != NULL ) && (!skipIcal))
    {
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRxInitFunc(devNum, portNum, CPSS_PORT_SERDES_RX_INIT_MODE_BLOCKING_E, &rxInitStatus);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] cpssDxChPortSerdesRxInit failed=%d", devNum, portNum, rc);
        }

        if (PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(cdrPrintStatusLock)[portNum] == GT_FALSE)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"RX_INIT Done");
        }
    }

    /*read cdr */
    if (( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc != NULL ) && (!anyLoopback))
    {
        PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc(devNum, portNum, cdrLockArr);
        PRV_PORT_MANAGER_CHECK_BOOL_ARR(cdrLockArr, curPortParams.numOfActLanes, *cdrLockPtr);

        if (PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(cdrPrintStatusLock)[portNum] == GT_FALSE)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "CDR %s locked", (*cdrLockPtr) ? "is" : "not");
            if (*cdrLockPtr == GT_FALSE)
            {
                PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(cdrPrintStatusLock)[portNum] = GT_TRUE;
            }
        }

        if ( *cdrLockPtr == GT_FALSE )
        {
            /*if at least one value from cdrLockArr is false, activate serdes tuning */
            if (!PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))
            {
                if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
                {
                    ...
                    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                                                       CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_CDR_E, (GT_U32*)cdrLockArr, &result);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling ppPortExtraOperationsSetFunc returned error code %d ", devNum, portNum, rc);
                    }
        ...
        /* cdrLockPtr == GT_TRUE */
        else if (!(PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode)))
        {
            PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(cdrPrintStatusLock)[portNum] = GT_FALSE;
            /* 1000baseX and non training modes: unreset PCS sd_rx_reset after signal detected */
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum) && PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))
            {
                /* Unreset PCS Rx */
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortPcsResetSetFunc(devNum, portNum, CPSS_PORT_PCS_RESET_MODE_RX_E, GT_FALSE);
                if (rc != GT_OK)
...
GT_STATUS prvCpssPortManagerInitEventCdr
(
    IN  GT_SW_DEV_NUM                  devNum,
    IN  GT_PHYSICAL_PORT_NUM           portNum,
    IN  PRV_CPSS_PORT_MNG_DB_STC       *tmpPortManagerDbPtr
)
{
    ...
    if (tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState == CPSS_PM_CDR_INIT_CDR_CHECK_STATE_E)
    {
        if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
        {
            GT_U32   dataMufif[] = {4 /* mufi */,5 /* muff */};
        ...
        rc =  prvCpssPortManagerCdrLockHandling(devNum, portNum, tmpPortManagerDbPtr, &signalDetect, &cdrLock);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling prvCpssPortManagerCdrLockHandling failed %d ", devNum, portNum, rc);
        }

        if (cdrLock == GT_FALSE || signalDetect == GT_FALSE)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Exiting Init state - signalDetect is %d | cdrLock  is %d", signalDetect, cdrLock);

            if (signalDetect == GT_FALSE)
                prvCpssPortManagerModeToDownState(devNum, portNum);

            return GT_OK;
        }

        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"start timer and check clampig");
        CPSS_PM_CDR_DELAY_START_TIME(tmpPortManagerDbPtr->portMngSmDb[portNum]);
        tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState = CPSS_PM_CDR_INIT_CLAMPING_STATE_E;
    }
    else if (tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState == CPSS_PM_CDR_INIT_CLAMPING_STATE_E)
    {
        prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrDelayTimeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrDelayTimeStampNSec, &timeDiff);
        ...
        if (signalDetect == GT_FALSE)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"signal is down during the clamping, move to down");
            prvCpssPortManagerModeToDownState(devNum, portNum);
        }

        if (timeDiff >= 100 /* nano seconds */)
        {
            if ( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppClampingResetAndStatusGetFunc == NULL)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_INITIALIZED,"[Port %d/%2d] ppClampingResetFunc not initialized", devNum, portNum);
            }

            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppClampingResetAndStatusGetFunc(devNum, portNum, &clampingDone);
            if (clampingDone == GT_FALSE)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"clamping status is not ok,back to check cdr");

                tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState = CPSS_PM_CDR_INIT_CDR_CHECK_STATE_E;
                return GT_OK;
            }
            else
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"clamping status is ok");
                 PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(cdrPrintStatusLock)[portNum] = GT_FALSE;
                /* 1000baseX and non training modes: unreset PCS sd_rx_reset after signal detected */
                if (PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))
                {
                ...
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"change mufi and muff serdes params");
                if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
                {
                    GT_U32   dataMufif[] = {5 /* mufi */,4 /* muff */};
                ...
                prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[portNum]->timeStampNSec, &tmpPortManagerDbPtr->portMngSmDb[portNum]->statistics.linkFailToSignalTime);
                CPSS_PM_START_TIME(tmpPortManagerDbPtr->portMngSmDb[portNum]);

                tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState = CPSS_PM_CDR_INIT_DONE_STATE_E;
            }
        }
    }
    else
...
/**
* @internal
*           prvCpssPortManagerLowLevelStatusChangedSip6AndAC3Based
* @endinternal
*
* @brief  function that perfrom the low level status change for
*         sip 6 and ac3 based devices
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  Caelum; Aldrin; Bobcat3; Aldrin2; Lion2; Bobcat2; AC3X.
*
...
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssPortManagerLowLevelStatusChangedSip6AndAC3Based
(
    IN GT_SW_DEV_NUM                  devNum,
    IN GT_PHYSICAL_PORT_NUM           portNum,
    IN PRV_CPSS_PORT_MNG_DB_STC       *tmpPortManagerDbPtr,
...
)
{
    ...
    GT_U32                                  portGroup, portsInBundleCounter = 1, step = 1, portsInBundle = 1, startPort = 0, tmpPort, i;
    GT_BOOL                                 alignLock;
    CPSS_PORT_SERDES_AUTO_TUNE_STATUS_ENT   rxInitStatus;
    PRV_CPSS_PORT_MANAGER_PORT_STATUSES_STC portStatuses;
    CPSS_PORT_MANAGER_PORT_TYPE_ENT         portType;
    ...
    if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        /* check if align lock changed*/
        rc = prvCpssPortPcsAlignLockStatusGet(CAST_SW_DEVNUM(devNum), portNum,portMacNum, &alignLock);
        if (rc == GT_OK)
        {
#ifndef PORT_MANAGER_DISABLE_HW_SIGNAL_MASK
            if(((tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_UP_E) ||
               (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E)) && (!anyLoopback))
            {
                if (( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc != NULL ) &&
                     (!PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode)))
                {
            ...
            rc = prvCpssPortSignalDetectHwMask(CAST_SW_DEVNUM(devNum),portNum,alignLock,portMode);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling prvCpssPortSignalDetectHwMask failed %d ", devNum, portNum, rc);
            }
    ...
    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"low level signal status: %d",signalDetect);

    /* if NOT! sip 6 interrupt driven devices with 1000_BaseX mode */
    /* The reason for some devices and modes doing the CDR in the init it's because interrupt stroming happening due to optical module
       showing incorrcet signal and casuing port manager to push interrpts (on interrupt base devices).
       when CDR checking in the init, it's don't need to push interrupt, but using the application init pollling to check over and over the status */
    if (!(PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode)))
    {
        if ( mvHwsUsxModeCheck(devNum,portMacNum,portMode))
        {
        ...
        /*RX_INIT*/
        if (( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRxInitFunc != NULL ) &&
            (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_DOWN_E) &&
            ((PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))|| (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E)) && (!skipIcal))
        {
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortRxInitFunc(devNum, portNum, CPSS_PORT_SERDES_RX_INIT_MODE_BLOCKING_E, &rxInitStatus);
            if (rc != GT_OK)
            {
            ...
            /* return info according to RX init status: PASS, NOT COMPLITED or FAIL*/
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "serdes Rx Init executed, return status: %s",
                                                       ((rxInitStatus == CPSS_PORT_SERDES_AUTO_TUNE_PASS_E) ? "PASS" :
                                                        ((rxInitStatus == CPSS_PORT_SERDES_AUTO_TUNE_NOT_COMPLITED_E) ? "NOT COMPLITED" : "FAIL")));
        }

        /*read cdr */
        if (( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc != NULL ) &&
            (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_DOWN_E) && (!anyLoopback) &&
            ((PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))|| (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E)))
        {
            ...
            /*if at least one value from cdrLockArr is false, activate serdes tuning */
            if ( (cdrLock == GT_FALSE) && !PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode) )
            {
                if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
                {
                    GT_U32 result;
                    ...
                    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                                                       ...
                                                       portGroup, tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->ifModeDb,
                                                       tmpPortManagerDbPtr->portsAttributedDb.perPhyPortDbPtr[portNum]->speedDb,
                                                       MV_HWS_PORT_SERDES_OPERATION_RES_CAP_SEL_TUNING_CDR_E, (GT_U32*)cdrLockArr, &result);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling ppPortExtraOperationsSetFunc returned error code %d ", devNum, portNum, rc);
                    }
            ...
            if ( cdrLock == GT_FALSE )
            {
                /* for polling devices, the task will handle this */
                if (signalDetect && !PRV_PORT_MANAGER_POLLING_DEVICES(devNum))
                {
                    GT_U32 data;

                    /* This is used for clearing the regiseter and no for reading the data */
                    rc = prvCpssPortManagerEventBitMaskingReading(devNum, portNum, curPortParams.activeLanesList[0], &curPortParams, tmpPortManagerDbPtr, CPSS_PP_PORT_LANE_SQ_EVENT_E, &data);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] calling prvCpssPortManagerEventBitMaskingReading %d ", devNum, portNum, rc);
                    }
    ...
    if ( mvHwsUsxModeCheck(devNum,portMacNum,portMode))
    {
        ...
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"LOW LEVEL event was received on USX port. port in bundle: %d", portsInBundle);
        portsInBundleCounter = portsInBundle;
    }

    /* for NON_USX ports, this for loop need to run only 1 time */
    /* for USX ports this for loop will run over the ports bundle and will perfrom low level status changed */
    for (i = startPort; portsInBundleCounter > 0; i += step, portsInBundleCounter--)
    {
        /* check if usx ports, cant use mvHwsUsxModeCheck here since for AC5X it's reutrn true even for non usx ports. portsInBundle > 1 make sure it's really usx ports */
        if (portsInBundle > 1)
        {
            if ( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppConvertMacToPortFunc != NULL)
            {
            ...
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PTR, "[Port %d/%2d] ppConvertMacToPortFunc is invalid ", devNum, i);
            }
            if (tmpPortManagerDbPtr->portMngSmDb[tmpPort] == NULL ||
                tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM == CPSS_PORT_MANAGER_STATE_RESET_E ||
                tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM == CPSS_PORT_MANAGER_STATE_INIT_IN_PROGRESS_E)
            {
                continue;
            }
        }
        ...
        {
            if (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_INIT_IN_PROGRESS_E)
            {
                return GT_OK;
            }
            tmpPort = portNum;
        ...
        /* PCS align lock(0x1) + PCS GB lock(0x2) + signal detect(0x4) + sq lane (0x10)*/
        if ( tmpPortManagerDbPtr->portMngSmDb[tmpPort]->maskingBitMap & 0x17 )
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"Ignore low level event while interrupt is masked");
            continue;
        }

        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"low_level changed portAdminSM %d", tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM);

        rc = prvCpssPortManagerMaskLowLevelEvents(CAST_SW_DEVNUM(devNum), tmpPort, CPSS_EVENT_MASK_E);
        if (rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"prvCpssPortManagerMaskLowLevelEvents failed %d ", rc );
            if (portsInBundle == 1)
            {
                return rc;
            }
        ...
        portMacNum = tmpPortManagerDbPtr->portsAttributedDb.generalPortCfgPtr[tmpPort]->portMacNum;

        if(tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_DOWN_E && (signalDetect == GT_TRUE))
        {
            /* if NOT! sip 6 interrupt driven devices with 1000_BaseX mode */
            if (!(PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode)))
            {
                prvCpssPortManagerTimeDiffGet(tmpPortManagerDbPtr->portMngSmDb[tmpPort]->timeStampSec, tmpPortManagerDbPtr->portMngSmDb[tmpPort]->timeStampNSec, &tmpPortManagerDbPtr->portMngSmDb[tmpPort]->statistics.linkFailToSignalTime);
                CPSS_PM_START_TIME(tmpPortManagerDbPtr->portMngSmDb[tmpPort]);
            }

            tmpPortManagerDbPtr->portMngSmDb[tmpPort]->signalDetected = GT_FALSE;
            tmpPortManagerDbPtr->portMngSmDb[tmpPort]->oneShotiCal    = GT_TRUE;

            rc = hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), 0, portMacNum, NON_SUP_MODE, &curPortParams);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort," hwsPortModeParamsGetToBuffer returned null %d ", rc );
                if (portsInBundle == 1)
                {
                    return rc;
                }
            ...
            if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
            {
                ...
                {
                    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortLowPowerEnableFunc(CAST_SW_DEVNUM(devNum), tmpPort, laneNum, GT_FALSE);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] cpssDxChPortSerdesLowPowerModeEnable failed=%d", devNum, tmpPort, rc);
                    }
                }
            }

            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"change to init state");

            /* if NOT! sip 6 interrupt driven devices with 1000_BaseX mode */
            if (PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode))
            {
                tmpPortManagerDbPtr->portMngSmDb[tmpPort]->cdrState = CPSS_PM_CDR_INIT_CDR_CHECK_STATE_E;
            }

            *stagePtr = CPSS_PORT_MANAGER_EVENT_INIT_E;
            tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM = CPSS_PORT_MANAGER_STATE_INIT_IN_PROGRESS_E;
        }
        else if (signalDetect == GT_FALSE)
        {
            rc = prvCpssPortManagerStopAdaptive(devNum, tmpPort, tmpPortManagerDbPtr);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"prvCpssPortManagerStopAdaptive failed %d ", rc );
                if (portsInBundle == 1)
                {
                    return rc;
                }
            ...
            prvCpssPortManagerModeToDownState(devNum,tmpPort);

            if(PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum))
            {
                if ( tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portOperDisableSM != GT_TRUE)
                {
                    ...
                    {
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortEnableWaWithLinkStatusSet(devNum, tmpPort);
                        if (rc != GT_OK)
                        {
                            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort," cpssDxChPortEnableWaWithLinkStatusSet: error, rc = %d ", rc );
                            if (portsInBundle == 1)
                            {
                                return rc;
                            }
        ...
        else if((tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM == CPSS_PORT_MANAGER_STATE_LINK_UP_E) ||
                 (tmpPortManagerDbPtr->portMngSmDb[tmpPort]->portAdminSM == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E ))
        {
            if(PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
            {
                rc = prvCpssPortStatusesCheck(devNum, tmpPort, &portStatuses,
                              PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PORT_MANAGER_GB_LOCK_E) |
                              PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PORT_MANAGER_ALIGN_LOCK_E));
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling prvCpssPortStatusesCheck returned %d", devNum, tmpPort, rc);
                }

                if (portStatuses.gbLock == GT_FALSE || portStatuses.alignLock == GT_FALSE)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort, "mac level status change: gearbox %s locked & align %s locked. move to down  ",
                                                                    (portStatuses.gbLock) ? "is" : "not", (portStatuses.alignLock) ? "is" : "not");
                    prvCpssPortManagerModeToDownState(devNum,tmpPort);
                    return GT_OK;
                }
            }
            /* clearing the mask on low lvl done above, for link up /mac link down case */
            rc = prvCpssPortManagerMaskLowLevelEvents(CAST_SW_DEVNUM(devNum), tmpPort, CPSS_EVENT_UNMASK_E);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, tmpPort,"prvCpssPortManagerMaskLowLevelEvents failed %d ", rc );
                if (portsInBundle == 1)
...
)
{
         ...
         tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E))
    {
        ...
        if (!(PRV_PORT_MANAGER_POLLING_DEVICES(devNum)) && (portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E))
        {
            /* mac level interrupt can be recive if we have signal detecet, gb lock and align lock.
               in case that one of them is false, need to go back to the pcs/serdes level to check them again.
               signal is handle by the serdes and will send low level when there is no signal,
               but for gb lock and align lock, need to check them before handling mac level event.
               in case there is no lock, move the port down to handle it again

               note: simple/xpcs mode don't have gb & align lock*/
            if (!(PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode) || PRV_CPSS_PORT_MANAGER_XPCS_MODE_CHECK(ifMode)))
            {
                PRV_CPSS_PORT_MANAGER_PORT_STATUSES_STC portStatuses;
                ...
                if (portStatuses.gbLock == GT_FALSE || portStatuses.alignLock == GT_FALSE)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "mac level status change: gearbox %s locked & align %s locked. move to down  ",
                                                                (portStatuses.gbLock) ? "is" : "not", (portStatuses.alignLock) ? "is" : "not");
        ...
        if (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E)
        {
            if(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
            {
                        ...
                        && (CPSS_PORT_RS_FEC_MODE_ENABLED_E == fecAbility))
                {
                    ...
                    serdesNum = curPortParams.activeLanesList[0];
    ...
    else if (*stagePtr == CPSS_PORT_MANAGER_EVENT_LOW_LEVEL_STATUS_CHANGED_E)
    {
        if (PRV_PORT_MANAGER_SIP_6_AND_AC3_BASED_DEVICE(devNum))
        {
            rc = prvCpssPortManagerLowLevelStatusChangedSip6AndAC3Based(devNum, portNum, tmpPortManagerDbPtr, stagePtr);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortManagerLowLevelStatusChangedSip6AndAC3Based failed=%d", devNum, portNum, rc);
            }
            return GT_OK;
        }
        else
        {
            ...
            /* PCS align lock(0x1) + PCS GB lock(0x2) + signal detect(0x4) + sq lane (0x10)*/
            if ( tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & 0x17 )
            {
...
)
{
    ...
    if ((stage == CPSS_PORT_MANAGER_EVENT_CREATE_E) || (stage == CPSS_PORT_MANAGER_EVENT_CREATE_AND_DISABLE_E) ||
        (stage == CPSS_PORT_MANAGER_EVENT_UNFORCE_LINK_DOWN_E) || (stage == CPSS_PORT_MANAGER_EVENT_CREATE_AND_FORCE_LINK_DOWN_E))
    {
        rc = prvCpssCommonPortIfModeToHwsTranslate (CAST_SW_DEVNUM(devNum), ifMode, speed, &portMode);
        if (rc != GT_OK)
        {
...
)
{
    *exitFromFunc = GT_TRUE;
    ...
    {
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"enable operation of port %d for device called upon already enabled port, so just returning.", portNum, devNum);
        return GT_OK;
    }

    else if ((db->portOperDisableSM == GT_TRUE) && (CPSS_PORT_MANAGER_EVENT_DISABLE_E == stage) && (!db->isRemotePort))
    {
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"disable operation of port %d for device called upon already disabled port, so just returning.", portNum, devNum);
        return GT_OK;
    }
...
static GT_STATUS prvCpssPortApEnableCheck
(
    GT_SW_DEV_NUM                    devNum,
    GT_PHYSICAL_PORT_NUM             portNum
)
{
    ...
    GT_U32 retries, timeout = 100;
    retries = timeout;

    if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortApEnableGetFunc == NULL)
    {
        return GT_OK;
    }
    /* verify that the port is not configured to AP in the service CPU, wait till 1 second */
    do {
        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortApEnableGetFunc(CAST_SW_DEVNUM(devNum), portNum, &portApEnable);
        if(rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortApEnableGetFunc failed=%d\n",devNum, portNum,rc);
            return rc;
        }
        if (portApEnable == GT_FALSE)
        {
            break;
        }
    ...
    if (retries != timeout)
    {
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"port was in AP for %d retries",retries);
        if (retries == 0)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NO_RESOURCE, "[Port %d/%2d] port still in AP mode", devNum, portNum);
        }
    }
    return GT_OK;
}
...
)
{
    ...
    MV_HWS_PORT_INIT_PARAMS      curPortParams;
    GT_U32                       data;
    GT_U32                       result = 0, serdesOperationData;
    GT_BOOL                      rxSignalOk;
    ...
    if (tempSystemRecovery_Info.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_HA_E)
    {
        ...
        {
            /*in case of CPSS_PORT_MANAGER_EVENT_CREATE_AND_FORCE_LINK_DOWN_E api should continue without old hw setting check*/
            if (portEventStc->portEvent != CPSS_PORT_MANAGER_EVENT_CREATE_AND_FORCE_LINK_DOWN_E)
            {
                return GT_OK;
            }
        }
    ...
    if (stage ==  CPSS_PORT_MANAGER_EVENT_PORT_DEBUG_E )
    {
        ...
        /* stop AP machine (cm3/ANP) */
        if((PRV_CPSS_SIP_6_CHECK_MAC(devNum)) || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
        {
            if((tmpPortManagerDbPtr->portMngSmDb[portNum]->portType == CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E) ||
               (tmpPortManagerDbPtr->portMngSmDb[portNum]->pmOverFw))
            {
                if(mvHwsApPortCtrlDebugModeSet(devNum, 0, portMacNum, GT_TRUE, GT_FALSE) != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "[Port %d/%2d] disable AP machine failed", devNum, portNum);
                }
            }
        }
    ...
    else if (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM == CPSS_PORT_MANAGER_STATE_DEBUG_E )
    {
        ...
        else
        {
            if (stage == CPSS_PORT_MANAGER_EVENT_PORT_NO_DEBUG_E)
            {
                ...
                if ( tmpPortManagerDbPtr->portMngSmDb[portNum]->portType != CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E ) /*Currentlly this extara opertion relevant only on Harrier NON-AN.
                                                                                                                    If it wiil be relevant for AN port need to use AP db to get ifmode/speed/portGroup*/
                {
                    if ( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL )
                    {
                        serdesOperationData = 0; /* indicates NO_DEBUG state */
                        ...
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(
                            CAST_SW_DEVNUM(devNum), portNum, portGroup, ifMode, speed, MV_HWS_PORT_SERDES_OPERATION_DEBUG_MODE_E, &serdesOperationData, &result);
                        if ( GT_OK != rc )
                        {
                            return rc;
                        }
                ...
                else/*for AP port and ANP - need to delet and create the port when exit debug mode to ensure clean opertion after debug*/
                {
                    if((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)
                        || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E)
                        || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E))
                    {
                        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Exit Debug Mode - AP Port (ANP) - delete and create port");
                        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, LOG_ERROR_NO_MSG);
                    }
                }
            ...
            tmpPortManagerDbPtr->portMngSmDb[portNum]->enterDebugState = CPSS_PORT_MANAGER_STATE_RESET_E;
            /* stop debug mode and start AP machine (cm3/ANP) */
            if((PRV_CPSS_SIP_6_CHECK_MAC(devNum)) || (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E))
            {
                if((tmpPortManagerDbPtr->portMngSmDb[portNum]->portType == CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E) ||
                   (tmpPortManagerDbPtr->portMngSmDb[portNum]->pmOverFw))
                {
                    if(mvHwsApPortCtrlDebugModeSet(devNum,0, portMacNum, GT_FALSE, GT_FALSE) != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_BAD_STATE, "[Port %d/%2d] enable AP machine failed", devNum, portNum);
                    }
                }
            }
            if (stage == CPSS_PORT_MANAGER_EVENT_PORT_NO_DEBUG_E)
    ...
    if (portType == CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E)
    {
        ...
        /*Set Static port over ANP if relevant*/
        if ((stage == CPSS_PORT_MANAGER_EVENT_CREATE_E) && (tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP))
        {
            if (anyLoopback == GT_FALSE)
            {
                ...
                {
                    tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP = GT_FALSE;
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "(portNum %d)pPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_STATIC_OVER_ANP_SET_E) failed=%d", portNum, rc);
                }
                else
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Set port as static port over ANP");
                }
            }
            else
            {
                tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP = GT_FALSE;
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "(portNum %d)Static port Over ANP is not valid with loopback - configure port not over ANP", portNum, GT_NOT_SUPPORTED);
            }
        }
    }
...
/**--------------------------------------------------------EVENTS-HANDLER---------------------------------------------------------*/
    ...
    if (stage == CPSS_PORT_MANAGER_EVENT_LOW_LEVEL_STATUS_CHANGED_E || stage == CPSS_PORT_MANAGER_EVENT_MAC_LEVEL_STATUS_CHANGED_E)
    {
        if((tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP))
        {
            ...
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Stage: CPSS_PORT_MANAGER_EVENT_LOW_LEVEL_STATUS_CHANGED_E. Static port over ANP - ignor this stage");
            }
#endif
        }
        else
    ...
    if (stage == CPSS_PORT_MANAGER_EVENT_INIT_E)
    {
        if (tmpPortManagerDbPtr->portMngSmDb[portNum]->staticOverANP)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"INIT Event is ignore for static port over ANP");
        }
        else
        {
            /* if sip 6 interrupt driven devices with 1000_BaseX mode */
            if ((PRV_CPSS_PORT_MANAGER_CHECK_IF_CDR_IS_DONE_IN_INIT_STATE(devNum, portMode)) &&
                (tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState != CPSS_PM_CDR_INIT_DONE_STATE_E) &&
                (!anyLoopback))
            {
                rc =  prvCpssPortManagerInitEventCdr(devNum ,portNum ,tmpPortManagerDbPtr);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling prvCpssPortManagerInitEventCdr returned error code %d ", devNum, portNum, rc);
                }

                if (tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState== CPSS_PM_CDR_INIT_DONE_STATE_E)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"init CDR is done");
                }

                return GT_OK;
            }

            if(PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode) && !PRV_CPSS_PORT_MANAGER_1000BASE_MODES_CHECK_MAC(portMode))
            {
        ...
        (stage == CPSS_PORT_MANAGER_EVENT_UNFORCE_LINK_DOWN_E))
    {
        ...
        /* Masking SQ event for polling devices */
        if (PRV_PORT_MANAGER_POLLING_DEVICES(devNum) && (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum)))
        {
            CPSS_EVENT_MASK_SET_ENT   tmpOperation;
            for (i = 0; i < curPortParams.numOfActLanes; i++)
            {
                rc = prvCpssPortManagerLowLevelCheckAndSet(devNum,curPortParams.activeLanesList[i], CPSS_EVENT_MASK_E, &tmpOperation,CPSS_PP_PORT_LANE_SQ_EVENT_E);
                if (rc != GT_NOT_FOUND && rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] Calling cpssEventDeviceMaskWithEvExtDataS%d LANE_SQ_EVENT returned %d ", devNum,portNum, CPSS_EVENT_MASK_E, rc);
                }
            }
        }

        if ((!isRemotePort) || /* not remote port */
            (PRV_CPSS_PORT_MANAGER_CHECKING_GIGE_PORTS_ON_AC3_BASED_DEVICE(devNum,portNum)) || /* port 0-23 in AC3 BASED (GIGE ports) */
            (PRV_CPSS_SIP_5_15_CHECK_MAC(devNum) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum)) && PRV_CPSS_PORT_MANAGER_MAC_LOOPBACK_CHECK(loopbackCfg))) /* sip 5.15 and sip 5.20 with mac loopback*/
        {
            rc = prvCpssPortManagerMaskEvents(CAST_SW_DEVNUM(devNum), portNum, CPSS_EVENT_MASK_E);
            if (rc != GT_OK)
            {
        ...
        if(!isRemotePort)
        {
            ...
            if (rc != GT_OK)
            {
                PRV_PORT_MANAGER_UNMASK_AND_RETURN_ERROR(devNum,tmpPortManagerDbPtr,rc,rc_tmp,"prvCpssPortManagerEnableLinkLevel");
            }
        }
        /* loopback configuration - get from DB */
        rc = prvCpssPortManagerLoopbackModesDbGet(devNum, portNum, portType, &loopbackCfg, &anyLoopback);
       ...
       /* if user configured FEC mode, this is the place to set it */
        ...
        if (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"on create check and set sync E setting");
            rc = prvCpssHarrierLoopbackSyncECheck(devNum,portNum,curPortParams,GT_TRUE);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssHarrierLoopbackSyncECheck failed, return %d", devNum, portNum, rc);
            }
        ...
        if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_BOBCAT3_E) ||
             (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_ALDRIN2_E))
        {
            /* verify that the port is not configured to AP in the service CPU */
            rc = prvCpssPortApEnableCheck(devNum,portNum);
            if (rc!=GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortApEnableCheck failed=%d\n",devNum, portNum,rc);
            }
        }
        /* bringing up the port using portModeSpeedSet */
        tmpPortManagerDbPtr->portMngSmDb[portNum]->lbPort = anyLoopback;
        ...
        if (rc != GT_OK)
        {
            ...
            PRV_PORT_MANAGER_UNMASK_AND_RETURN_ERROR(devNum,tmpPortManagerDbPtr,rc,rc_tmp,"ppPortModeSpeedSetFunc");
        }

        /* Check tx impedance calibration */
        if ( (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL) &&
            (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5_E) && (portMode ==_22GBase_SR) )
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Check tx impedance calibration");
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                               portGroup, ifMode, speed,
                                                                                               MV_HWS_PORT_SERDES_OPERATION_CHECK_TX_IMPEDANCE_CALIBRATION_E,&serdesOperationData, &result);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling ppPortExtraOperationsSetFunc returned error code %d ", devNum, portNum, rc);
            }

            /* if Tx impedance calibration indicates SD issue, need to re-create port since SD reset alone can
               harm its configuration, so we won't be able to recover */
            if (result == 0x1)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Tx impedance calibration failed, re-create port");

                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortModeSpeedSetFunc(devNum, portModeSpeedSetBitmap, GT_FALSE, ifMode, speed,
                                                                                             tmpPortManagerDbPtr->portMngSmDb[portNum] /* fecAbility*/);
                if (rc != GT_OK)
                {
                    PRV_PORT_MANAGER_UNMASK_AND_RETURN_ERROR(devNum,tmpPortManagerDbPtr,rc,rc_tmp,"ppPortModeSpeedSetFunc");
        ...
        /* need to configure USR for Ironman-M serdes 6 */
        switch (PRV_CPSS_PP_MAC(devNum)->devType)
        {
            case CPSS_IRONMAN_M_DEVICES_CASES_MAC:
                if (curPortParams.activeLanesList[0] == 6)
                {
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"configure USR for Ironman-M serdes 6");
                    if(PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc != NULL)
                    {
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                                           portGroup, ifMode, speed,
                                                                                                           MV_HWS_PORT_SERDES_OPERATION_SET_USR_E,&serdesOperationData, &result);
                        if (rc != GT_OK)
                        {
                            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling ppPortExtraOperationsSetFunc returned error code %d ", devNum, portNum, rc);
                        }
        ...
        if ((!PRV_CPSS_SIP_6_10_CHECK_MAC(devNum)) || PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
        {
            ...
            if (PRV_CPSS_PORT_MANAGER_SLOW_MODE_CHECK(portMode) && (!PRV_CPSS_SIP_6_CHECK_MAC(devNum) || ( _2500Base_X != portMode)))
            {
                ...
                /* AC5: disable auto negotiation configurations for any type of TX_2_RX loopback */
                if (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum) && ((SGMII == portMode) || (QSGMII == portMode)))
                {
                    if((loopbackCfg.loopbackType == CPSS_PORT_MANAGER_LOOPBACK_TYPE_MAC_E) || (PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_ANALOG_TX2RX_CHECK(loopbackCfg)))
                    {
                        autoNegotiation.inbandEnable                 = GT_FALSE;
                        autoNegotiation.byPassEnable                 = GT_TRUE;
                        autoNegotiation.duplexEnable                 = GT_FALSE;
                        autoNegotiation.speedEnable                  = GT_FALSE;
                        autoNegotiation.flowCtrlEnable               = GT_FALSE;
                    }
                }
            ...
            (PRV_CPSS_SIP_6_CHECK_MAC(devNum) && (tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperationsBitmap)))
        {
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortExtraOperationsSetFunc(CAST_SW_DEVNUM(devNum), portNum,
                                                                                               portGroup, ifMode, speed,
                                                                                               tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperationsBitmap,&serdesOperationData, &result);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling ppPortExtraOperationsSetFunc returned error code %d ", devNum, portNum, rc);
            }
        ...
        if (!(PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode)) || CPSS_PORT_IF_MODE_QSGMII_USX_QUSGMII_CHECK_MAC(ifMode))
        {
            ...
            if (anyLoopback == GT_TRUE && !(PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_DIGITAL_RX2TX_CHECK(loopbackCfg)))
            {
                ...
                if (tempSystemRecovery_Info.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
                {
                    ...
                    /* Check if MAC loopback is being configured for Multi-lane USX port with enableRegularTraffic false, then return error and don't configure loopback.
                        This limitation is added as For MAC TX2RX loopback we disable SerDes Tx and in case of USX modes multiple ports share the same SerDes and
                        disabling SerDes Tx will affect other ports in the same group */
                    if ((loopbackCfg.loopbackType == CPSS_PORT_MANAGER_LOOPBACK_TYPE_MAC_E) && (HWS_USX_MULTI_LANE_MODE_CHECK(portMode)) && (loopbackCfg.enableRegularTraffic == GT_FALSE))
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "[Port %d/%2d] MAC loopback is not suported for Multi-lane USX mode with enableRegularTraffic false", devNum, portNum);
                    }
        ...
        else if (PRV_CPSS_PORT_MANAGER_SIMPLE_MODES_CHECK_MAC(portMode))/* handling loopback for low speeds */
        {
            ...
            if (anyLoopback == GT_TRUE)
            {
                    ...
                    (PRV_CPSS_PORT_MANAGER_SERDES_LOOPBACK_ANALOG_TX2RX_CHECK(loopbackCfg)))
                {
                    ...
                    /* For AC3/AC5 for SGMII in any TX_2_RX loopback mode it's needed to configure force link up.
                       QSGMII port mode treated above.
                       In order to get good traffic it's also need to disable inBand/Speed/Duplex AN*/
                    if (PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum) && (portMode == SGMII))
                    {
                        rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortForceLinkPassSetFunc(devNum,portNum,GT_TRUE);
                        if (rc != GT_OK)
                        {
        ...
        /* checking for rx_signal_ok, if it is true, then pushing signal_detect interrupt to the event queue. */
        if (!(PRV_CPSS_PORT_MANAGER_MAC_LOOPBACK_CHECK(loopbackCfg)) &&
            !(PRV_PORT_MANAGER_POLLING_DEVICES(devNum)))
        {
            ...
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"rx_signal_ok state=%d", rxSignalOk);
            /* pushing signal_detect interrupt (that we cleared) to the event queue.
            ...
            if (rxSignalOk == GT_TRUE)
            {
                ...
                {
                    uniEv = CPSS_PP_PORT_LANE_SQ_EVENT_E;
                    extraData = curPortParams.activeLanesList[0];
                }
                else
                {
                    if (MMPCS == curPortParams.portPcsType || QSGMII_PCS == curPortParams.portPcsType)
                    {
                        uniEv = CPSS_PP_PORT_MMPCS_SIGNAL_DETECT_CHANGE_E;
                    }
                    else
                    {
                        uniEv = CPSS_UNI_RSRVD_EVENT_E;/* dummy ... not to be used */
                    }
                }

                if (CPSS_UNI_RSRVD_EVENT_E != uniEv)
                {
                    /* This is used for clearing the regiseter and no for reading the data */
                    rc = prvCpssPortManagerEventBitMaskingReading(devNum, portNum,extraData, &curPortParams, tmpPortManagerDbPtr, uniEv, &data);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] calling prvCpssPortManagerEventBitMaskingReading %d ", devNum, portNum, rc);
                    }

                    rc = prvCpssPortManagerPushEvent(CAST_SW_DEVNUM(devNum), portGroup, portNum, extraData, uniEv);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] prvCpssPortManagerPushEvent failed=%d", devNum, portNum, rc);
                    }
        ...
        *****************************************/
        ...
        if(CPSS_PORT_MANAGER_EVENT_CREATE_AND_DISABLE_E == stage)
        {
           ...
           tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperDisableSM = GT_TRUE;
            ...
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum,"Disabling port returned error code %d ", rc);
                return rc;
            }
        ...
        if (PRV_CPSS_PORT_MANAGER_MAC_LOOPBACK_CHECK(loopbackCfg))
        {
            ...
            if(GT_FALSE == tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperDisableSM)
            {
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortMacEnableSetFunc(devNum, portNum, GT_TRUE);
                if (GT_OK != rc)
                {
...
)
{
    ...
    if (portStagePtr->portState == CPSS_PORT_MANAGER_STATE_LINK_UP_E)
    {
        ...
        case CPSS_PORT_MANAGER_PORT_TYPE_REGULAR_E:
            ...
            if((CPSS_PORT_USX_PORT_MODE_CHECK_MAC(portStagePtr->ifMode)) || (portStagePtr->ifMode == CPSS_PORT_INTERFACE_MODE_SGMII_E))
            {
                CPSS_PORT_SPEED_ENT speed;
                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppSpeedGetFromExtFunc(devNum, portNum, &speed);
                if (rc != GT_OK)
...
static GT_STATUS prvCpssPortManagerSwInterruptPortInit
(
    GT_U8  devNum,
    GT_U32 portNum,
    GT_U32 portMacNum
...
{
    PRV_CPSS_DRV_EV_REQ_NODE_STC *swEvNodePtr = NULL;
    GT_U32   portGroup;
    ...
    if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroup].swEvNodesPool == NULL)
    {
        PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroup].swEvNodesPool =
            cpssOsMalloc(PRV_CPSS_PP_MAC(devNum)->numOfPorts * sizeof(PRV_CPSS_DRV_EV_REQ_NODE_STC));
        if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroup].swEvNodesPool == NULL)
        {
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_OUT_OF_CPU_MEM, LOG_ERROR_NO_MSG);
        }
        PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.numOfSwIntNodes = PRV_CPSS_PP_MAC(devNum)->numOfPorts;
    }

    swEvNodePtr = &PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroup].swEvNodesPool[portMacNum];
    cpssOsMemSet(swEvNodePtr,0,sizeof(*swEvNodePtr));

    swEvNodePtr->devNum           = devNum;
    swEvNodePtr->portGroupId      = portGroup;
    swEvNodePtr->nextPtr          = NULL;
    swEvNodePtr->prevPtr          = NULL;
    swEvNodePtr->intStatus        = PRV_CPSS_DRV_EV_DRVN_INT_ACTIVE_E;
    swEvNodePtr->intRecStatus     = PRV_CPSS_DRV_EV_DRVN_INT_IDLE_E;
    swEvNodePtr->intCbFuncPtr     = NULL;
    swEvNodePtr->uniEvCause       = CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E;
    swEvNodePtr->uniEvExt         = portNum; /* we use the physical port number not the MAC number !!! to avoid converts */
    swEvNodePtr->intMaskReg       = portMacNum;/* use as debug info ... if needed */
#ifdef SHARED_MEMORY
    /* mandatory for logic in evReqQRcvedEvRemoveOne to not use call
       prvCpssDrvEvReqUniEvMaskSet as 'callback function'
       we already not enter to prvCpssDrvEvReqUniEvMaskSet , as drvEventMaskDevice
       will call to prvCpssDrvEvReqUniSwEvMaskSet instead
    */
    swEvNodePtr->fake            = GT_TRUE;
#endif /*SHARED_MEMORY*/
    return GT_OK;
}

/* the function need to be called only after 'port mapping' and before 'mask/unmask' of events */
static GT_STATUS prvCpssPortManagerSwInterruptInit
(
    GT_U8 devNum
)
{
    ...
    GT_U32   maxPorts = (PRV_CPSS_SIP_5_CHECK_MAC(devNum) && !(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PX_FAMILY_PIPE_E))?
                         PRV_CPSS_MAX_PP_PORTS_NUM_CNS :
                         PRV_CPSS_PP_MAC(devNum)->numOfPorts;
#ifdef CHX_FAMILY
    GT_BOOL   validity;
#endif /*CHX_FAMILY*/

    /* init all ports for SW event : CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */
    for(portNum = 0; portNum < maxPorts; portNum++)
    {
#ifdef CHX_FAMILY
        if(CPSS_IS_DXCH_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily) && PRV_CPSS_SIP_5_CHECK_MAC(devNum))
        {
            PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortCheckMapValidity(devNum, portNum, &validity);
            if (validity == GT_FALSE)
            {
                continue;
            }
        ...
        rc = prvCpssPortManagerSwInterruptPortInit(devNum, portNum , portMacNum);
        if (rc != GT_OK)
        {
            return rc;
        }
...
)
{
    ...
    /* bind PM callbacks - used from CPSS to call PM functions */
    ...
    /*cpssOsMemSet(&tmpPortManagerDbPtr->clbkFunc, 0, sizeof(CPSS_PORT_MANAGER_CLBK_FUNC_STC));*/
    ...
    rc = prvCpssPortManagerSwInterruptInit(devNum);
    if (rc != GT_OK)
    {
        return rc;
    }
...
/**
* @internal prvCpssPortManagerRenumber function
* @endinternal
*
* @brief   renumber internal data base with new device number
*           validation of device number is done from the calling function
*
...
* @retval GT_OK                    - on success
*/
GT_STATUS prvCpssPortManagerRenumber
(
    IN  GT_U8                                   oldDevNum,
    IN  GT_U8                                   newDevNum
)
{
    /* update port manager information if port manager is used */
    if (PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(isPortMgrEnable)[oldDevNum] == GT_TRUE)
    {
        /* clean old device info */
        PRV_SHARED_PORT_DIR_COMMON_PORT_MANAGER_SRC_GLOBAL_VAR(isPortMgrEnable)[oldDevNum] = GT_FALSE;
    ...
    *confiData   = NULL;
    ...
    if (bm)
    {
        if ( bm & CPSS_PM_ATTR_TRAIN_MODE_E )
        {
            ...
            if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum) && !PRV_CPSS_SIP_6_30_CHECK_MAC(devNum))
            {
                if ( portAttr->trainMode == CPSS_PORT_SERDES_AUTO_TUNE_MODE_TX_TRAINING_START_E || portAttr->trainMode == CPSS_PORT_SERDES_AUTO_TUNE_MODE_RX_TRAINING_E ||  portAttr->trainMode == CPSS_PORT_SERDES_AUTO_TUNE_MODE_LAST_E )
                {
                    CPSS_PM_PORT_ANP_PARAM_STC anpParam;
                    if ( portAttr->trainMode == CPSS_PORT_SERDES_AUTO_TUNE_MODE_TX_TRAINING_START_E )
                    {
                        anpParam.trainingMode = CPSS_PM_PORT_ANP_TRX_TRAINING_E;
                    }
                    else if (portAttr->trainMode == CPSS_PORT_SERDES_AUTO_TUNE_MODE_RX_TRAINING_E)
                    {
                        anpParam.trainingMode = CPSS_PM_PORT_ANP_RX_TRAINING_E;
                    }
                    else
                    {
                        anpParam.trainingMode = CPSS_PM_PORT_ANP_NO_TRAINING_E;
                    }
                    anpParam.anpParamBitMask = CPSS_PM_PORT_ANP_PARAM_TRAINING_MODE_E;
                    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E, &anpParam);
                    if (rc != GT_OK)
                    {
                        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "(portNum %d)pPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E) failed=%d", portNum, rc);
                    }
                }
                else
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "(portNum %d)pPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_TRAINING_MODE_SET_E) failed=%d", portNum, GT_NOT_SUPPORTED);
                }
            }
        }
        ...
        if (bm & CPSS_PM_PORT_ATTR_PREEMPTION_E)
        {
            ...
            cpssOsMemCpy(&(portSmDbPtr->preemptionParams),&(portAttr->preemptionParams),sizeof(CPSS_PM_MAC_PREEMPTION_PARAMS_STC));
        }
        else
        {
        ...
        else
        {
            ...
            else if ( (PRV_CPSS_PORT_MANAGER_1000BASE_CHECK_MACRO(pmDB->portsAttributedDb.perPhyPortDbPtr[portNum]->ifModeDb)) ||
...
#ifndef ASIC_SIMULATION
        if (bm & CPSS_PM_PORT_ATTR_STATIC_OVER_ANP_E)
        {
            if ((PRV_CPSS_SIP_6_10_CHECK_MAC(devNum)) && (!PRV_CPSS_SIP_6_30_CHECK_MAC(devNum)) && (PRV_CPSS_PORT_MANAGER_STATIC_OVER_ANP_MODES_CHECK_MAC(portMode)) && (!(portSmDbPtr->isRemotePort)))
            {
                rc = prvCpssPortManagerLoopbackModesDbGet(devNum, portNum, pmDB->portMngSmDb[portNum]->portType, &loopbackCfg, &anyLoopback);
                if (rc != GT_OK)
                {
                ...
                if (anyLoopback == GT_FALSE)
                {
                    portSmDbPtr->staticOverANP = portAttr->staticOverANP;
                    portSmDbPtr->userOverrideByteBitmap |= PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_STATIC_OVER_ANP_ENABLE_E);
                    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Set static port over ANP:%d", portAttr->staticOverANP);
                }
                else
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "error: STATIC_OVER_ANP not supported in loopback mode");
                }
            }
            else
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, "error: STATIC_OVER_ANP not supported (not applicable device/port_mode or remote port)");
            }
        }
#endif

        if (bm & CPSS_PM_PORT_ATTR_DUPLEX_MODE_E)
        {
            portSmDbPtr->userOverrideByteBitmap |= PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_DUPLEX_MODE_E);
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "DUPLEX Mode  %d", portAttr->duplexMode);
            portSmDbPtr->duplexMode = portAttr->duplexMode;
        }

        if (bm & CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E)
        {
            portSmDbPtr->userOverrideByteBitmap |= PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E);
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "SIG_DET_CALLBACK_ENABLED Mode  %d", portAttr->extSigDetCallbackEnable);
            portSmDbPtr->extSigDetCallbackEnable = portAttr->extSigDetCallbackEnable;
        }
...
)
{
   ...
   **/
    ...
    **/
    ...
    if (portSmDbPtr->userOverrideByteBitmap & PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_STATIC_OVER_ANP_ENABLE_E))
    {
        portAttr->staticOverANP = portSmDbPtr->staticOverANP;
        CPSS_PM_SET_VALID_ATTR(portParams, CPSS_PM_PORT_ATTR_STATIC_OVER_ANP_E);
    }

    if (portSmDbPtr->userOverrideByteBitmap & PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_DUPLEX_MODE_E))
    {
        portAttr->duplexMode = portSmDbPtr->duplexMode;
        CPSS_PM_SET_VALID_ATTR(portParams, CPSS_PM_PORT_ATTR_DUPLEX_MODE_E);
    }

    if (portSmDbPtr->userOverrideByteBitmap & PRV_CPSS_PORT_MANAGER_BIT_MAC(PRV_CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E))
    {
        portAttr->extSigDetCallbackEnable = portSmDbPtr->extSigDetCallbackEnable;
        CPSS_PM_SET_VALID_ATTR(portParams, CPSS_PM_PORT_ATTR_EXTERNAL_SIG_DET_CALLBACK_ENABLED_E);
    }
...
)
{
    ...
    {
        pmDB->portsApAttributesDb[portNum]->apAttributesStc.parallelDetect.restartMaxNum  = PRV_PORT_MANAGER_DEFAULT_PARALLEL_DETECT_RESTART_NUM;
    ...
    if (bm & CPSS_PM_AP_PORT_ATTR_TX_POST_ADAPT_E)
        pmDB->portsApAttributesDb[portNum]->apAttributesStc.apTxPostAdapt    = apAttrs->apTxPostAdapt;
    else
        pmDB->portsApAttributesDb[portNum]->apAttributesStc.apTxPostAdapt    = GT_FALSE;
...
)
{
    ...
    */
    ...
    apAttrs->apTxPostAdapt  = pmDB->portsApAttributesDb[portNum]->apAttributesStc.apTxPostAdapt;
    if (pmDB->portsApAttributesDb[portNum]->apAttributesStc.overrideAttrsBitMask & CPSS_PM_AP_PORT_ATTR_TX_POST_ADAPT_E)
        CPSS_PM_SET_VALID_AP_ATTR(portParams, CPSS_PM_AP_PORT_ATTR_TX_POST_ADAPT_E);
    /* Override Params */
    /*   CPSS_PM_AP_PORT_ATTR_LANE_OVR_PARAMS_E, */
...
)
{
    ...
    {
        *serdesType = CPSS_PORT_SERDES_COMPHY_C12G_E;
    }
    else if (PRV_CPSS_SIP_6_20_CHECK_MAC(devNum))
    {
        *serdesType = CPSS_PORT_SERDES_COMPHY_C56G_E;
    }
    else if (PRV_CPSS_SIP_6_15_CHECK_MAC(devNum))
    {
        *serdesType = CPSS_PORT_SERDES_COMPHY_C28G_E;
    }
    else if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
        *serdesType = CPSS_PORT_SERDES_COMPHY_C112G_E;
    }
...
)
{
    ...
    if (GT_OK != hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), portGroup, portMacNum, portMode, &curPortParams))
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, "[MacPort %d] hwsPortModeParamsGetToBuffer returned null ", portMacNum);
    }

    cpssOsPrintf("devNum: %d  portNum:  %d MAC portNum: %d\n", devNum, portNum, portMacNum);
    cpssOsPrintf("---port-configuration---\n");
    cpssOsPrintf("  mode: %d  speed: %d fec: %d portMode: %d\n", ifMode, speed, tmpPortManagerDbPtr->portMngSmDb[portNum]->fecAbility,portMode);
    ...
    cpssOsPrintf(" cdrState is: %d\n",  tmpPortManagerDbPtr->portMngSmDb[portNum]->cdrState);

    cpssOsPrintf("---extSigDetCallbackEnable---\n");
    cpssOsPrintf("  extSigDetCallbackEnable: %d\n",tmpPortManagerDbPtr->portMngSmDb[portNum]->extSigDetCallbackEnable);

    cpssOsPrintf("---Duplex---\n");
    cpssOsPrintf("  Duplex: %d\n", tmpPortManagerDbPtr->portMngSmDb[portNum]->duplexMode);

    cpssOsPrintf("---interconnectProfile---\n");
    cpssOsPrintf("  interconnectProfile: %d\n", tmpPortManagerDbPtr->portMngSmDb[portNum]->interconnectProfile);
        ...
        (tmpPortManagerDbPtr->portMngSmDb[portNum]->portAdminSM != CPSS_PORT_MANAGER_STATE_FORCE_LINK_DOWN_E))
    {
        ...
        if ( PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppCdrLockGetFunc != NULL )
        {
            ...
            if(rc == GT_OK)
            {
                for (i = 0; i < curPortParams.numOfActLanes; i++)
                {
                    cpssOsPrintf("cdr lock status for lane %d: %s\n", curPortParams.activeLanesList[i], (cdrLockArr[i] == GT_TRUE ? "locked" : "not locked"));
                }
            }
        }
    }
    ...
    cpssOsPrintf("  CPSS_PM_PCS_ALIGN_LOCK_LOST_MASKING_E is: %s\n",
             (tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & CPSS_PM_PCS_ALIGN_LOCK_LOST_MASKING_E ? "mask" : "unmask"));

    cpssOsPrintf("  CPSS_PM_PORT_LANE_SQ_EVENT_MASKING_E is: %s\n",
         (tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & CPSS_PM_PORT_LANE_SQ_EVENT_MASKING_E ? "mask" : "unmask"));

    if (!PRV_CPSS_SIP_6_CHECK_MAC(devNum))
    {
        cpssOsPrintf("  CPSS_PM_PCS_GB_LOCK_SYNC_CHANGE_MASKING_E is: %s\n",
                 (tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & CPSS_PM_PCS_GB_LOCK_SYNC_CHANGE_MASKING_E ? "mask" : "unmask"));

        cpssOsPrintf("  CPSS_PM_MMPCS_SIGNAL_DETECT_CHANGE_MASKING_E is: %s\n",
                 (tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & CPSS_PM_MMPCS_SIGNAL_DETECT_CHANGE_MASKING_E ? "mask" : "unmask"));
    }

    cpssOsPrintf("---mac level mask---\n");
    cpssOsPrintf("  CPSS_PM_PORT_LINK_STATUS_CHANGED_E is: %s\n",
             (tmpPortManagerDbPtr->portMngSmDb[portNum]->maskingBitMap & CPSS_PM_PORT_LINK_STATUS_CHANGED_E ? "mask" : "unmask"));
...
)
{
    ...
    GT_BOOL     linkStatus = GT_FALSE;
    GT_CHAR     trafficPrefixErrMsg[20] = "";

    CPSS_PORT_MANAGER_DEV_CHECK_MAC(devNum);

    if (NULL == tmpPortManagerDbPtr)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PTR, "No Port DB doesn't created");
    }

    /* Enable traffic can be achieved only if link is up */
    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppLinkStatusGetFunc(devNum, portNum, &linkStatus);
    if (GT_OK != rc)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "Get link status for port %d of device %d failed. Reason: %d", portNum, devNum, rc);
    }

    if (GT_TRUE == enableTraffic)
    {
        if (GT_TRUE == linkStatus)
        {
            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Link is up. Enable traffic for port %d of device %d", portNum, devNum);
        }
        else
        {
            tmpPortManagerDbPtr->portMngSmDb[portNum]->portOperDisableSM = !enableTraffic;

            CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Link is down. Traffic cannot be enabled for port %d of device %d", portNum, devNum);
            return GT_OK;
        }

        cpssOsStrNCpy(trafficPrefixErrMsg, "Enable traffic", cpssOsStrlen("Enable traffic"));
    }

    if (GT_FALSE == enableTraffic)
    {
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "Disable traffic for port %d of device %d", portNum, devNum);

        cpssOsStrNCpy(trafficPrefixErrMsg, "Disable traffic", cpssOsStrlen("Disable traffic"));
    }

    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortMacEnableSetFunc(devNum, portNum, enableTraffic);
    if (rc != GT_OK)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "%s for port %d in device %d failed. Reason:%d", trafficPrefixErrMsg, portNum, devNum, rc);
    }

    rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppLinkStatusChangedNotifyFunc(devNum, portNum, CPSS_PORT_MANAGER_PORT_TYPE_802_3AP_E, enableTraffic);
    if (rc != GT_OK)
    {
        CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "Status changed notification for port %d in device %d failed. Reason:%d", portNum, devNum, rc);
    }
...
{
    GT_U32                      i;
    GT_U32                      advertisedNum;
    GT_U32                      laneIndex;
    GT_STATUS                   rc;
    MV_HWS_PORT_STANDARD        portMode ,maxPortMode = LAST_PORT_MODE;
    MV_HWS_PORT_INIT_PARAMS     curPortParams;
    ...
    for (i = 0; i < CPSS_PORT_AP_IF_ARRAY_SIZE_CNS; i++)
    {
        ...
        if (((apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_NA_E) && (apParams->modesAdvertiseArr[i].ifMode == CPSS_PORT_INTERFACE_MODE_NA_E)) ||
            ((apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_10_E) && (apParams->modesAdvertiseArr[i].ifMode == CPSS_PORT_INTERFACE_MODE_REDUCED_10BIT_E)))
        {
            continue;
        }

        if (PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesFreeCheck != NULL)
        {
            /* checking per advertising speed, if the serdes are valid and not used
               if one of them is not valid, the create will fail for all the advertising speeds*/
            rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortSerdesFreeCheck(CAST_SW_DEVNUM(devNum), portNum,
                                                                                                             apParams->modesAdvertiseArr[i].ifMode, apParams->modesAdvertiseArr[i].speed);
            if (rc != GT_OK)
            {
                CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc,"[Port %d/%2d] Calling pphPortSerdesFreeCheck returned error code %d ",
                                                           devNum, portNum, rc);
            }
        }
        ...
        if ((apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_10000_E) || (apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_40000_E) ||
            (apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_42000_E) || (apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_47200_E) ||
            (apParams->modesAdvertiseArr[i].speed == CPSS_PORT_SPEED_12000_E))
        {
        ...
        if (maxLanes < curPortParams.numOfActLanes)
        {
            ...
            maxPortMode = portMode;
        }
    }

    if (PRV_CPSS_SIP_6_10_CHECK_MAC(devNum))
    {
         /* mask CPSS_PP_PORT_LANE_SQ_EVENT_E interrupt */
        rc = hwsPortModeParamsGetToBuffer(CAST_SW_DEVNUM(devNum), 0, portMacNum, maxPortMode, &curPortParams);
        if(rc != GT_OK)
        {
            CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] hwsPortModeParamsGetToBuffer returned rc %d",devNum, portNum, rc );
        }
        ...
        CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "mask CPSS_PP_PORT_LANE_SQ_EVENT_E",curPortParams.numOfActLanes );
    }
    ...
    for (advertisedNum = 0; advertisedNum < CPSS_PORT_AP_IF_ARRAY_SIZE_CNS; advertisedNum++)
    {
            ...
            (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[advertisedNum].ifModeDb != CPSS_PORT_INTERFACE_MODE_REDUCED_10BIT_E))
        {
            ...
            /* for AC5X, need to enable Tx adapt for 25G/10G serdes speeds if ap tx adapt enable in DB*/
            if ((PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E) &&
                (tmpPortManagerDbPtr->portsApAttributesDb[portNum]->apAttributesStc.apTxPostAdapt == GT_TRUE) &&
                (curPortParams.serdesSpeed == _10_3125G || curPortParams.serdesSpeed == _25_78125G))
            {
                anpParam.ifMode      =  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[advertisedNum].ifModeDb;
                anpParam.speed       =  tmpPortManagerDbPtr->portsApAttributesDb[portNum]->perPhyPortDb[advertisedNum].speedDb;
                anpParam.enable      =  GT_TRUE;

                rc = PRV_NON_SHARED_GLOBAL_VAR_GET(nonSharedDeviceSpecificDb[devNum].ppCommonPortFuncPtrsStc).ppPortAnpCallFunc(devNum, portNum, CPSS_PM_PORT_ANP_FUNC_TX_POST_ADAPT_SET_E, &anpParam);
                if (rc != GT_OK)
                {
                    CPSS_PORT_MANAGER_LOG_ERROR_AND_RETURN_MAC(rc, "[Port %d/%2d] ppPortAnpCallFunc (CPSS_PM_PORT_ANP_FUNC_TX_POST_ADAPT_SET_E) returned rc %d",devNum, portNum, rc );
                }
            }
    ...
    CPSS_PORT_MANAGER_LOG_PORT_INFORMATION_MAC(devNum, portNum, "AP - port created successfully ");
