...
)
{
    ...
    GT_U32                  localPortMacNum;/* local mac in the core */
    ...
    /* for Lion2,Hooper mvHwsPortCheckGearBox need to get the local mac in the port group */
    localPortMacNum = PRV_CPSS_GLOBAL_PORT_TO_LOCAL_PORT_CONVERT_MAC(devNum, portMacNum);
    ...
    {
        rc = mvHwsPortCheckGearBox(devNum, portGroup, localPortMacNum, portMode, gbLockPtr);
...
/*
* @internal prvCpssPortPcsAnDoneGet function
* @endinternal
*
* @brief   Return PCS AN done status
*
* @note   APPLICABLE DEVICES:      AC5P; AC5X;
...
*
* @param[out] anDone                  - AN Done state
*
* @retval GT_OK                    - on success.
* @retval GT_BAD_PARAM             - on bad parameters
* @retval GT_BAD_PTR               - gbLockPtr == NULL
* @retval GT_NOT_APPLICABLE_DEVICE - on not applicable device
*/
GT_STATUS prvCpssPortPcsAnDoneGet
(
    IN  GT_U8                   devNum,
    IN  GT_U32                  portMacNum,
    OUT GT_BOOL                 *anDone
)
{
    ...
    rc = mvHwsPcsPortAnDoneGet(devNum, portGroup, portMacNum, portMode, anDone);
    if(rc != GT_OK)
    {
        CPSS_LOG_INFORMATION_MAC("Hws return code is %d", rc);
    }
