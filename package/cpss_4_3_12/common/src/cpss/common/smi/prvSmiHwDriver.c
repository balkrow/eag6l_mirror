...
#ifdef SHARED_MEMORY
extern GT_VOID prvCpssDrvHwDrvReload
(
...
GT_VOID cpssHwDriverSmiReloadDrv
(
...
)
{
    ...
    do
    {
        read = drv->common.parent->read;
#ifdef SHARED_MEMORY
        localDrv.type = drv->common.parent->type;
        prvCpssDrvHwDrvReload(&localDrv);
        read = localDrv.read;
#endif
        if (read(drv->common.parent, drv->as, drv->reg, &reg, 1) != GT_OK)
        {
        ...
        reg = value;
#endif
        PRV_CPSS_SMI_MAX_NUM_ITERATIONS_CHECK_CNS(drv->common.devNum, (retryCnt++), GT_NOT_READY);
...
)
{
    ...
    prvCpssSmiXsmiMppMuxSet(drv->common.devNum, drv->interfaceNum, PRV_CPSS_SMI_BUS_TYPE_SMI_E);
    writeMask = drv->common.parent->writeMask;
#ifdef SHARED_MEMORY
    localDrv.type = drv->common.parent->type;
    prvCpssDrvHwDrvReload(&localDrv);
    ...
    for (;count;count--,regAddr++, dataPtr++)
    {
        ...
        rc = writeMask(drv->common.parent, drv->as, drv->reg, &reg, 1, 0xffffffff);
        if (rc != GT_OK)
...
{
    drv->read = (CPSS_HW_DRIVER_METHOD_READ)prvSmiHwDriverRead;
    drv->writeMask = (CPSS_HW_DRIVER_METHOD_WRITE_MASK)prvSmiHwDriverWriteMask;
    drv->destroy = (CPSS_HW_DRIVER_METHOD_DESTROY)prvSmiHwDriverDestroyDrv;
}
...
)
{
    ...
    drv->common.type = CPSS_HW_DRIVER_TYPE_SMI_E;
