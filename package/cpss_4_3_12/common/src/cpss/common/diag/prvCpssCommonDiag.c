...
#define PCI_3_WRITE_DATA_INTO_ADDR_MAC(_address , _doByteSwap , _data)   \
    ...
    static const GT_U32 gmUsed = 1;
#else
    static const GT_U32 gmUsed = 0;
#endif
...
/**
* @internal prvCpssDiagGenRegsInfoSearch function
* @endinternal
*
* @brief   Search for information about the registers in use. Wrapper for prvCpssDiagGenRegsInfoSearch.
*
* @note   APPLICABLE DEVICES:      xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3; Aldrin2; Falcon; AC5P; AC5X; Harrier; Ironman; Pipe.
* @note   NOT APPLICABLE DEVICES:  None.
*
...
*
*/
GT_STATUS prvCpssDiagGenRegsInfoSearch
(
    IN    GT_U8                              devNum,
    IN    GT_U32                             portGroupId,
    IN    PRV_CPSS_DIAG_REGS_SEARCH_TYPE_ENT searchType,
...
{
    return regsInfoSearch(
        devNum, portGroupId, searchType,
        regsListPtr, regsListSize,
        offset, unitFirstAddr, unitLastAddr,
        checkRegExistFuncPtr, regsNumPtr, regsInfoPtr);
}
...
)
{
...
#ifdef CHX_FAMILY
    {
        ...
        if(PRV_CPSS_PP_MAC(devNum) && PRV_CPSS_SIP_7_CHECK_MAC(devNum) &&
            !gmUsed)
        {
            GT_U32  rfuOffset = prvCpssDxChHwUnitBaseAddrGet(devNum, PRV_CPSS_DXCH_UNIT_CNM_RFU_E, NULL);
            /*<device_id>*/
            address = rfuOffset + 0x00000028;
            /* no register with 0x11AB , to know if SWAP needed or not */
...
)
{
    ...
    PRV_CPSS_DFX_PIPES_BMP_GET_FUNC      getDfxPipesFuncPtr;
    ...
    if(PRV_CPSS_PP_MAC(devNum)->diagInfo.dfxPipesBmpGetFunc)
    {
        ...
        getDfxPipesFuncPtr = PRV_CALLBACK_GET(PRV_CPSS_PP_MAC(devNum)->diagInfo.dfxPipesBmpGetFunc,PRV_CPSS_DFX_PIPES_BMP_GET_FUNC);

        rc = getDfxPipesFuncPtr(devNum, &allPipesBmp);
        if(GT_OK != rc)
        {
            return rc;
        }
