...
/* The following determines the driver working mode:
   if defined I2C_BUSY_WAIT_MODE - will busy poll I2C status
   if not defined, will sleep for 1ms
*/
#define I2C_BUSY_WAIT_MODE
...
)
{
    ...
    GT_U32 ctrl = 0;
#ifdef I2C_BUSY_WAIT_MODE
    GT_U32 t1_sec, t1_nano_sec;
    GT_U32 t2_sec, t2_nano_sec;
    double elapsedTime;
#endif
...
#ifdef I2C_BUSY_WAIT_MODE
        cpssOsTimeRT(&t1_sec, &t1_nano_sec);

        if (MV_TWSI_STATE_DONE != drv->controller_state)
            do {
                cpssOsTimeRT(&t2_sec, &t2_nano_sec);

                elapsedTime = (t2_sec - t1_sec) * 1000000.0;
                elapsedTime += (t2_nano_sec - t1_nano_sec)/1000.0; /* usec */

                MV_TWSI_READ_REG(MV_TWSI_REG_CTRL, ctrl);
            }
            while ((!(ctrl & MV_TWSI_CTRL_BIT_IF)) && (elapsedTime < 150.0 /* typical period is 50-60 */) );
#else
...
static GT_STATUS prvI2cHwDriverGetDrvData(GT_U8 devNum, GT_U8 bus_id, char* drvPath, GT_U32  *asPtr, GT_U32* ctrlRegAddrPtr)
{
    ...
    GT_U32  baseAddr;
    GT_BOOL err;
    GT_U32 (*baseAddrGetFunc) (IN GT_U32 devNum, IN PRV_CPSS_DXCH_UNIT_ENT unitId, OUT GT_BOOL *errorPtr);
...
)
{
    ...
    if (ppConfig->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E){
        if (bus_id >= CPSS_I2C_FALCON_MAX_BUS_PER_PP_CNS)
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    else if (ppConfig->devFamily == CPSS_PP_FAMILY_DXCH_IRONMAN_E){
        if (bus_id >= CPSS_I2C_IRONMAN_MAX_BUS_PER_PP_CNS)
            CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
    }
    else
    ...
    if (!PRV_CPSS_SIP_6_CHECK_MAC(devNum) && (frequency > 100000) )
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_BAD_PARAM, LOG_ERROR_NO_MSG);
