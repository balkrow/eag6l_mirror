...
)
{
    ...
    {
        cpssOsMemSet(&(PRV_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_MANAGERS_DB.managersStatusArray), 0, sizeof(PRV_CPSS_SYSTEM_RECOVERY_MANAGER_STATUS_STC)*CPSS_SYSTEM_RECOVERY_LAST_MANAGER_E);
    }
...
/**
* @internal prvCpssSystemRecoveryHaManagersStatusGet function
* @endinternal
*
* @brief   This function print system recovery manager status under parallel HA mode
*
* @note   APPLICABLE DEVICES:      Aldrin2;Falcon; AC5P; AC5X; Harrier; Ironman.
* @note   NOT APPLICABLE DEVICES:  xCat3; AC5; Lion2; Bobcat2; Caelum; Aldrin; AC3X; Bobcat3;
*
* @retval GT_OK                    - on success.
*/
GT_STATUS prvCpssSystemRecoveryHaManagersStatusGet
(
    GT_VOID
)
{
    ...
    cpssOsPrintf("\n CPSS_SYSTEM_RECOVERY_NO_MANAGERS_E = 0"
                 "\n CPSS_SYSTEM_RECOVERY_PORT_MANAGER_E = 1"
                 "\n CPSS_SYSTEM_RECOVERY_FDB_MANAGER_E = 2"
                 "\n CPSS_SYSTEM_RECOVERY_LPM_MANAGER_E = 3  "
                 "\n CPSS_SYSTEM_RECOVERY_TCAM_MANAGER_E = 4"
                 "\n CPSS_SYSTEM_RECOVERY_EXACT_MATCH_MANAGER_E = 5  "
                 "\n CPSS_SYSTEM_RECOVERY_TRUNK_MANAGER_E = 6");
    while (ii != CPSS_SYSTEM_RECOVERY_LAST_MANAGER_E )
    {
        cpssOsPrintf("\n manager %d  recoveryStarted %d recoveryDone %d ",
                     ii, PRV_SYSTEM_RECOVERY_MANAGER_STATUS_MAC(ii).recoveryStarted ,PRV_SYSTEM_RECOVERY_MANAGER_STATUS_MAC(ii).recoveryDone);
        ii++;
    }
    return GT_OK;
}
