...
)
{
    ...
    if((PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB.errorBmp[bmpOffset] & (1 << bitOffset)) != 0)
    {
        return GT_TRUE;
    }
    return GT_FALSE;
...
)
{
    ...
    if (PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB.errorInjectionEnable == GT_TRUE)
    {
        ...
        if (file.fd!=CPSS_OS_FILE_INVALID )
        {
            ...
            CPSS_LOG_INFORMATION_MAC("\ncheck error index [%d] ,DB error bitmap [0x%x][0x%x][0x%x] ",
                             errorInjectionIndex,
                             PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB.errorBmp[0],
                             PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB.errorBmp[1],
                             PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB.errorBmp[2]);
...
)
{
    ...
    cpssOsMemCpy(&(PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB), &errorBmp, sizeof(CPSS_SYSTEM_RECOVERY_ERROR_INJECTION_STC));
    return GT_OK;
}
...
)
{
    ...
    *errorBmpPtr = PRV_NON_SHARED_GLOBAL_VAR_SYSTEM_RECOVERY_ERROR_INJECTION_DB;
    return GT_OK;
}
