...
static PRV_CPSS_HW_DRIVER_LIST_NODE_STC** prvDrvListGetNodeChildPtrPtr(CPSS_HW_DRIVER_STC *drv)
{
    ...
    if (drv == NULL)
    {
        return (PRV_CPSS_HW_DRIVER_LIST_NODE_STC **)&PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(cpssHwDriverListPtr);
    }
    if (drv->parent == NULL)
    {
        baseNode = (PRV_CPSS_HW_DRIVER_LIST_NODE_STC **)&PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(cpssHwDriverListPtr);
    }
    else
    {
...
)
{
    ...
    if (path[0] != '/')
        return NULL;
    prvCpssHwDriverListCheckLock();
    drv = prvCpssHwDriverLookup((PRV_CPSS_HW_DRIVER_LIST_NODE_STC *)PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(cpssHwDriverListPtr), path+1);
    cpssOsMutexUnlock(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(prvCpssHwDriverListMtx));
    return drv;
}
...
GT_STATUS cpssHwDriverPrintList(void)
{
    ...
    prvCpssHwDriverListCheckLock();
    c = (PRV_CPSS_HW_DRIVER_LIST_NODE_STC *)PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(cpssHwDriverListPtr);
    while (c)
    {
        for (i=0; i < sp; i++)
...
/**
* @internal cpssHwDriverRenumberDevNum function
* @endinternal
*
* @brief   Upon devNum renumber, updates drivers with new devNum
*
* @param[in] oldDevNum                - old dev num
* @param[in] newDevNum                - new dev num
*
*/
GT_VOID cpssHwDriverRenumberDevNum(
    IN GT_U8    oldDevNum,
    IN GT_U8    newDevNum
)
{
    /* c - iterator to traverse the driver's registry
       stack - stores the registry stack layers, we are currently in
    */
    PRV_CPSS_HW_DRIVER_LIST_NODE_STC *c, *stack[16];
    /* sp - the stack depth we are currently in */
    GT_32 sp = 0, i;
    CPSS_HW_DRIVER_STC *drv;
    GT_CHAR buf[128], *pBuf;

    prvCpssHwDriverListCheckLock();
    c = (PRV_CPSS_HW_DRIVER_LIST_NODE_STC *)PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CON_GEN_INIT_SRC_VAR_GET(cpssHwDriverListPtr);
    while (c)
    {
        /* skip root (i.e. /EAGLEffff:ff.255) && skip aliases */
        if (sp && (!c->alias))
        {
            cpssOsMemSet(buf, 0, sizeof(buf));
            buf[0] = '/';
            pBuf = buf + 1;
            for (i=0; i < sp; i++) {
                cpssOsStrCpy(pBuf, stack[i]->name);
                pBuf += cpssOsStrlen(stack[i]->name);
            }
            *pBuf = '/';
            pBuf++;
            cpssOsStrCpy(pBuf, c->name);

            drv = cpssHwDriverLookup(buf);
            if (drv && (drv->devNum==oldDevNum))
                drv->devNum = newDevNum;
        }
