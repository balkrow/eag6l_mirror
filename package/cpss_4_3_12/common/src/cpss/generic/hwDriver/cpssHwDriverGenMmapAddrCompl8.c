...
)
{
    GT_U32 compl;
    int i;
    GT_UINTPTR perProcessBase = PRV_PER_PROCESS_BASE_GET(drv);
    GT_UINTPTR      reg;
    GT_UINTPTR      region;
    ...
    if (drv->nextIndex >= drv->numRegions)
        drv->nextIndex = 0;
    /**((volatile GT_U32*)(drv->compl[i].reg- drv->base+perProcessBase)) = CPSS_32BIT_LE(compl);*/
    reg = drv->compl[i].reg;
    reg -= drv->base;
    reg += perProcessBase;

     *((volatile GT_U32*)(reg)) = CPSS_32BIT_LE(compl);
   GT_SYNC; /* to avoid from write combining */
   drv->compl[i].val = compl;
   region =  drv->compl[i].region;
   region -= drv->base;
   region += perProcessBase;
...
GT_VOID cpssHwDriverGenMmapAc8DrvReload
(
    CPSS_HW_DRIVER_STC *drv
)
{
    drv->read = (CPSS_HW_DRIVER_METHOD_READ)cpssHwDriverGenMmapAc8Read;
    drv->writeMask =  (CPSS_HW_DRIVER_METHOD_WRITE_MASK)cpssHwDriverGenMmapAc8WriteMask;
    drv->destroy = (CPSS_HW_DRIVER_METHOD_DESTROY)cpssHwDriverGenMmapAc8DestroyDrv;
}
...
* @param[in] devNum   -device number that driver is serving(required for shared library mode)
...
)
{
    ...
    /*
            ...
            We could not configure it at cpssGlobalNonSharedDbPpMappingUpdate since no per device data base was available.
       */
