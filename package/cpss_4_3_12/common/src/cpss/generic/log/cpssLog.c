...
static const PRV_CPSS_ENUM_STRING_VALUE_PAIR_STC prvCpssLogEnum_map_PRV_CPSS_LOG_FUNC_RET_CODE_ENT[] =
{
    ...
    PRV_CPSS_LOG_ENUM_NAME_AND_VALUE_MAC(GT_SHAPER_GRANULARITY_OUT_OF_RANGE)

};
PRV_CPSS_LOG_STC_ENUM_MAP_ARRAY_SIZE_MAC(PRV_CPSS_LOG_FUNC_RET_CODE_ENT);

GT_VOID prvCpssDxChLogDbPoolIdBufFree
(
    IN  GT_VOID   *cookiePtr,
    IN  GT_VOID   *pBuf
)
{   cookiePtr = cookiePtr;
    (GT_VOID) cpssBmPoolBufFree(PRV_LOG_GLOBAL_NON_VOLATILE_VAR_DIRECT_GET(prvCpssLogDbPoolId),pBuf);
}
...
)
{
    ...
    if(rc == GT_NOT_FOUND)
    {
        /* the implementation not found the task as 'osTaskCreate' , but returned
           it's pthreads id or windows handle ... it is OK for us !

           it happens for the 'ISR context' that calls function like : cpssSystemRecoveryStateGet(...)
           it also happens when non-Marvell application which is not required to use 'osTaskCreate' calls any CPSS API
        */
        rc = GT_OK;
    }
    if (rc != GT_OK)
...
* @param[in] strBufferPtr             - (pointer to) log string buffer
*                                       None.
*/
...
{
    PRV_CPSS_LOG_API_HISTORY_ENTRY_STC * currentHistoryEntryPtr; /* Pointer to current history log database entry */
    ...
    switch (logPhase)
    {
        case PRV_CPSS_LOG_PHASE_ENTER_E:
            ...
            if (PRV_LOG_GLOBAL_VAR_VOLATILE_GET(currentHistoryEntryNum) < PRV_LOG_GLOBAL_NON_VOLATILE_VAR_DIRECT_GET(prvCpssLogStackSize))
            {
                ...
                cpssOsSprintf(PRV_LOG_GLOBAL_VAR_VOLATILE_GET(currentHistoryEntryPtr)->entryHistoryFileName, "%s_pid_%d_%d", PRV_LOG_GLOBAL_VAR_VOLATILE_GET(prvCpssLogHistoryName),cpssOsTaskGetPid(), PRV_LOG_GLOBAL_VAR_VOLATILE_GET(currentHistoryEntryNum));
    ...
    currentHistoryEntryPtr = PRV_LOG_GLOBAL_VAR_VOLATILE_GET(currentHistoryEntryPtr);
    if (currentHistoryEntryPtr)
    {
        if(0 != currentHistoryEntryPtr->entryHistoryFile.fd)
        {
            cpssOsFprintf(currentHistoryEntryPtr->entryHistoryFile.fd, strBufferPtr);
        }
    }
}
...
)
{
    ...
    if (rc != GT_OK)
    {
        PRV_CPSS_LOG_AND_HISTORY_MAC(PRV_CPSS_LOG_PHASE_ERROR_E, PRV_LOG_GLOBAL_NON_VOLATILE_VAR_DIRECT_GET(prvCpssLogContextEntryPtr)->prvCpssLogApiContextLib, CPSS_LOG_TYPE_ERROR_E,
        "prvCpssLogApiFunctionLogTrigger: rc = %d lib[%d],type[%d]\n",
            rc,PRV_LOG_GLOBAL_NON_VOLATILE_VAR_DIRECT_GET(prvCpssLogContextEntryPtr)->prvCpssLogApiContextLib, logType);
        return GT_FALSE;
    }
...
static void cpssLogLockUnlockAllDevices(IN GT_BOOL lock)
{
    GT_U32  devNum;

    if( NULL == cpssSharedGlobalVarsPtr ||
        GT_FALSE ==PRV_SHARED_GLOBAL_VAR_GET(sysConfigPhase1Done))
    {
        return;
    }
    ...
    if(lock == GT_TRUE)
    {
        ...
        /* zero level Lock the Logger DB (must come after device-level) */
        CPSS_ZERO_LEVEL_API_LOCK_NO_RETURN_MAC;
    }
    else
    {
        /* unlock zero level (must come before device-level) */
        CPSS_ZERO_LEVEL_API_UNLOCK_MAC;
...
#else   /*!CPSS_API_LOCK_PROTECTION*/
    #define  cpssLogLockUnlockAllDevices(lock) /* empty */
...
{
    /* lock the DB of the LOG and lock the running thread from enter the CPSS APIs
       while we will enable/disable the log */
    cpssLogLockUnlockAllDevices(GT_TRUE);

    PRV_NON_SHARED_GLOBAL_VAR_LOG_SET(enable);

    /* unlock what was previously locked */
    cpssLogLockUnlockAllDevices(GT_FALSE);
}
...
)
{
    ...
    /* lock the DB of the LOG and lock the running thread from enter the CPSS APIs
       while we will enable/disable the log for the libs and states */
    cpssLogLockUnlockAllDevices(GT_TRUE);
...
)
{
    ...
    /* delete tree */
    rc = prvCpssAvlTreeDelete(PRV_LOG_GLOBAL_NON_VOLATILE_VAR_DIRECT_GET(prvCpssLogAvlTreeId), prvCpssDxChLogDbPoolIdBufFree, NULL);
    if(rc != GT_OK )
    {
        return rc;
    }
...
)
{
    ...
    {
        /* address of the new 16 byte series */
        if((byteCount & 0x0F) == 0)
        {
...
GT_VOID * prvCpssDxChLogCallbacksBind
(
    GT_VOID
)
{
    /*due to const will be located in read only section*/
    static GT_COMP_RES (* const prvCpssLogFunctionDb[])(GT_VOID *,GT_VOID *) =
    {
       prvCpssLogContextEntryCompare,
       NULL
    };

    return (GT_VOID *)prvCpssLogFunctionDb;
}
