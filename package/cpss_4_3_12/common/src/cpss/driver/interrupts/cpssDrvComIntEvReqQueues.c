...
{
    GT_U32      i;
    PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC * eventDataArr; /* pointer to events DB */

    if (PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArrSize) < numOfEvents)
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    }

    eventDataArr = (PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC *)PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr);

    for(i = 0; i < numOfEvents; i++)
    {
        eventDataArr[i].evNodeList     = NULL;
        eventDataArr[i].userHndlPtr    = NULL;
        eventDataArr[i].nextPtr        = NULL;
        eventDataArr[i].uniEvCause     = i;
    }
...
)
{
    ...
    if(PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr) != NULL)
    {
        return GT_OK;
    }

    PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr) =
    ...
    PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_SET(uniEvQArrSize, numOfEvents);
...
)
{
    ...
    if (PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArrSize) <= uniEvCause)
    {
        return NULL;
    }

    eventDataArr = (PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC *)PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr);
    return eventDataArr[uniEvCause].userHndlPtr;
}
...
)
{
    ...
    eventDataArr[uniEvCause].nextPtr = evQNewHndlPtr->evListPtr;
    evQNewHndlPtr->evListPtr      = &eventDataArr[uniEvCause];

    /* save the user handle in queue control */
    eventDataArr[uniEvCause].userHndlPtr = evQNewHndlPtr;
...
)
{
    ...
    */
    ...
    currEvReqPtr = eventDataArr[uniEvCause].userHndlPtr->evListPtr;
    ...
    eventDataArr[uniEvCause].evNodeList     = NULL;
    eventDataArr[uniEvCause].userHndlPtr    = NULL;
    eventDataArr[uniEvCause].nextPtr        = NULL;
...
)
{
    ...
    eventDataArr = (PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC *)PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr);
    evQueue = &(eventDataArr[newNode->uniEvCause]);
...
)
{
    ...
    eventDataArr = (PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC *)PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr);
    evQueuePtr = &(eventDataArr[uniEvCause]);
...
)
{
    ...
    for (i = 0; i < PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArrSize); i++)
    {
        if (NULL == eventDataArr[i].evNodeList)
        {
            continue;
        }

        evQueuePtr = &(eventDataArr[i]);
...
)
{
    ...
    while (1)
    {
        ...
        intMaskSetFptr = evNodePtr->intMaskSetFptr;
#else
        /*The interrupt mask setting routine
          when NULL the prvCpssDrvInterruptMaskSet is used.
        */
        if(GT_TRUE == evNodePtr->fake)/* needed for CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */
        {
            /* the fake not need to use the HW mask function */
        }
        else
        if(PRV_INTERRUPT_CTRL_GET(evNodePtr->devNum).intMaskSetFptr == NULL)
        {
        ...
        {
            intMaskSetFptr   = PRV_INTERRUPT_CTRL_GET(evNodePtr->devNum).intMaskSetFptr;
        }
#endif
            if(NULL!=intMaskSetFptr)
            {
             intMaskSetFptr(evNodePtr, CPSS_EVENT_UNMASK_E);
            }
        }
...
)
{
    ...
    for (event = 0; event < intNodesPoolSize; event++)
    {
        ...
        /*The interrupt mask setting routine
          when NULL the prvCpssDrvInterruptMaskSet is used.
        */
        if(GT_TRUE == nodePoolArr[event].fake)/* needed for CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E */
        {
            /* the fake not need to use the HW mask function */
        }
        else
...
)
{
    ...
    FREE_PTR_MAC(PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(uniEvQArr));

    /* reset size of uniEvQArr array */
    PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_SET(uniEvQArrSize,0);
...
/**
* @internal prvCpssDrvEvReqQInsertSwEvent function
* @endinternal
*
* @brief   This function inserts a new event node to the tail of the event queue.
*         The active event counter is incremented.
* @param[in] swEvNodePtr             - Pointer to the fake event nodes pool
*
* @retval GT_OK   - on success
* @retval GT_FAIL - if failed
*
* @note :Software event Insert
*/
GT_STATUS prvCpssDrvEvReqQInsertSwEvent
(
    IN PRV_CPSS_DRV_EV_REQ_NODE_STC      *swEvNodePtr
)
{
    PRV_CPSS_DRV_EV_REQ_NODE_STC     *newNode;      /* The new node to be inserted.     */
    PRV_CPSS_DRV_EV_REQ_Q_CTRL_STC   *evQueue;      /* The unified event queue          */
