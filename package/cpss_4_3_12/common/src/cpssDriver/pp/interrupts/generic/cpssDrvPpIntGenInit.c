...
{
    PRV_CPSS_DRV_EV_REQ_NODE_STC *intNodesPoolReqDrvn;
    PRV_CPSS_DRV_EV_REQ_NODE_STC *swEvNodePoolReqDrvn;
    ...
    /* Update all software interrupt's related data structure, to    */
    /* include newDevNum instead of oldDevNum.              */
    for(ii = 0; ii < PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->intCtrl.numOfSwIntNodes; ii++)
    {
        portGroupId = PRV_CPSS_GLOBAL_PORT_TO_PORT_GROUP_ID_CONVERT_MAC(newDevNum, ii);
        swEvNodePoolReqDrvn = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->intCtrl.portGroupInfo[portGroupId].swEvNodesPool;
        if(swEvNodePoolReqDrvn != NULL)
        {          
            swEvNodePoolReqDrvn[ii].devNum = newDevNum;            
        }
    }
