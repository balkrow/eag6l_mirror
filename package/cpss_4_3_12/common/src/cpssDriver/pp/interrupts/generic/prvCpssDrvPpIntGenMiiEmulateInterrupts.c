...
/**
********************************************************************************
* @file prvCpssDrvPpIntGenMiiEmulateInterrupts.c
*
* @note
...
)
{
    ...
    for(i = 0; (i < PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.numOfIntBits)
...
)
{
    ...
    PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[rxQueue]).pcktIn++;

    /* validate minimal packet size */
    if (segmentLen[0] < PRV_CPSS_GEN_NETIF_MII_BUFF_MIN_SIZE_CNS)
    {
        qCtrl_PTR->packetFreeFunc_PTR(segmentList, numOfSegments, rxQueue);
        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[rxQueue]).pcktLen++;
        return GT_OK;
    }
    ...
    {
        qCtrl_PTR->packetFreeFunc_PTR(segmentList, numOfSegments, rxQueue);
        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[rxQueue]).noQbuffer++;
                       ...
                       CPSS_OS_MSGQ_WAIT_FOREVER) != GT_OK)
    {
        ...
        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[rxQueue]).msgSend++;
    ...
    PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[rxQueue]).pcktTreated++;
...
/**
* @internal miiDbgPortStat function
* @endinternal
*
*/
GT_STATUS miiDbgPortStat(void)
{
    ...
    for (i = 0; i < NUM_OF_RX_QUEUES; i++)
    {
        cpssOsPrintf("%-1d %-7X %-7X %-7X %-7X %-7X %-7X %-7X %-7X %-7X\n",
                        i, PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).pcktIn, PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).pcktTreated,
                        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).command, PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).cpuCode,
                        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).msgSend, PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).noQbuffer,
                        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).pcktLen, PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).intType,
                        PRV_SHARED_COMMON_INT_DIR_GENERIC_GLOBAL_VAR_GET(ethPortDbg[i]).device);
    }
