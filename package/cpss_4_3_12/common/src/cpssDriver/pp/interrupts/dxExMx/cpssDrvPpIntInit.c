...
)
{
    ...
    switch (PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_AAS_E:
            ...
            {
                case 1:
                    devIntPtr = &PRV_SHARED_COMMON_INT_DIR_DXEXMX_GLOBAL_VAR_GET(prvCpssDrvDevFamilyInterruptInfoArray[PRV_CPSS_DRV_FAMILY_DXCH_AAS_1_TILE_E]);
                    break;
                case 2:
                    devIntPtr = &PRV_SHARED_COMMON_INT_DIR_DXEXMX_GLOBAL_VAR_GET(prvCpssDrvDevFamilyInterruptInfoArray[PRV_CPSS_DRV_FAMILY_DXCH_AAS_2_TILES_E]);
                    break;
                default:
                    CPSS_LOG_ERROR_AND_RETURN_MAC(GT_NOT_SUPPORTED, "Bad number of tiles[%d] (supported 1/2)",
                        PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->numOfTiles);
            }
#endif /*!GM_USED*/
            break;
...
)
{
    ...
    PRV_CPSS_DRV_DEVICE_INTERRUPTS_INFO_STC *devIntPtr = NULL;
    CPSS_SYSTEM_RECOVERY_INFO_STC oldSystemRecoveryInfo,newSystemRecoveryInfo; /* holds system recovery information */
    ...
    if (intVecNum == CPSS_PP_INTERRUPT_VECTOR_USE_POLLING_CNS)
    {
...
#ifdef CHX_FAMILY
            if(PRV_CPSS_PP_MAC(devNum) && PRV_CPSS_SIP_7_CHECK_MAC(devNum))
            {
                PRV_SHARED_COMMON_INT_DIR_DXEXMX_GLOBAL_VAR_GET(intArr[devNum].intSumReg) =
                    0x00000100 + /* address of summary of tree 0 */
                    prvCpssDxChHwUnitBaseAddrGet(devNum, PRV_CPSS_DXCH_UNIT_CNM_INTERRUPTS_E, NULL);
            }
            else
            if (PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->mngInterfaceType == CPSS_CHANNEL_PEX_FALCON_Z_E)
            {
        ...
        if (oldSystemRecoveryInfo.systemRecoveryProcess != CPSS_SYSTEM_RECOVERY_PROCESS_HA_E &&
            oldSystemRecoveryInfo.systemRecoveryProcess != CPSS_SYSTEM_RECOVERY_PROCESS_HITLESS_STARTUP_E)
        {
...
#ifdef CHX_FAMILY
        initPhase = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.initPhase[i];
        if (initPhase == PRV_CPSS_DRV_INT_INIT_PHASE_MACSEC_E)
        {
            /* check access to MACSec registers */
            rc = prvCpssMacSecIntMaskRegCheck(devNum, devIntPtr->maskRegistersAddressesArray[i], &skip);
            if (rc != GT_OK)
            {
                /* Incase MACSec is disabled, these registers will not be accesible */
                skip = GT_TRUE;
            }
            if(skip)
            {
