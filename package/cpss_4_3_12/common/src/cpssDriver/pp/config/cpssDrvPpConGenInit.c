...
extern GT_VOID cpssHwDriverPexRenumber(
    IN  GT_U8               newDevNum,
    IN  CPSS_HW_DRIVER_STC * genDrvPtr
);
#endif
...
static GT_STATUS prvCpssDrvEvReqUniSwEvMaskSet
(
    IN GT_U8                devNum,
    IN GT_U32               portGroupId,
    IN GT_U32               uniEvent,   /*CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E*/
    IN GT_U32               evExtData,
...
)
{
    ...
    PRV_CPSS_DRV_EV_REQ_NODE_STC  *nodePoolArr;
    nodePoolArr = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroupId].swEvNodesPool;
    ...
    for(ii = 0 ; ii < PRV_CPSS_PP_MAC(devNum)->numOfPorts; ii++)
    {
        if(evExtData != PRV_CPSS_DRV_EV_REQ_UNI_EV_EXTRA_DATA_ANY_CNS)
        {
            if(nodePoolArr[ii].uniEvExt != evExtData)
            {
                continue;
            }
        }
        ...
        /* we not update the HW , as this is SW event */
        if(CPSS_EVENT_MASK_E == operation)
        {
            nodePoolArr[ii].intStatus    = PRV_CPSS_DRV_EV_DRVN_INT_STOPPED_E;
            nodePoolArr[ii].intRecStatus = PRV_CPSS_DRV_EV_DRVN_INT_MASKED_E;
        }
        else
        {
            nodePoolArr[ii].intStatus    = PRV_CPSS_DRV_EV_DRVN_INT_ACTIVE_E;
            nodePoolArr[ii].intRecStatus = PRV_CPSS_DRV_EV_DRVN_INT_IDLE_E;
        }
    }
...
static GT_STATUS prvCpssDrvEvReqUniSwEvMaskGet
(
    IN  GT_U8                               devNum,
    IN  GT_U32                              portGroupId,
    IN  GT_U32                              uniEvent,
...
)
{
    ...
    *maskedPtr = CPSS_EVENT_MASK_E;
    ...
    for(ii = 0 ; ii < PRV_CPSS_PP_MAC(devNum)->numOfPorts; ii++)
    {
        ...
        *maskedPtr = nodePoolArr[ii].intStatus == PRV_CPSS_DRV_EV_DRVN_INT_STOPPED_E ?
...
)
{
    ...
    {
        if(uniEvent == CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)
        {
            rc = prvCpssDrvEvReqUniSwEvMaskSet(devNum,portGroupId,(GT_U32)uniEvent, evExtData, operation);
        }
        else
        {
...
)
{
    ...
    PRV_CPSS_DRV_START_LOOP_PORT_GROUPS_MAC(devNum,portGroupId)
    {
        if(uniEvent == CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E)
        {
            rc = prvCpssDrvEvReqUniSwEvMaskGet(devNum,portGroupId,(GT_U32)uniEvent, evExtData, maskedPtr);
        }
        else
        {
...
)
{
    ...
    PRV_CPSS_DRV_START_LOOP_PORT_GROUPS_MAC(devNum,portGroupId)
    {
        ...
        for (eventItr = 0; eventItr < nodesPoolSize; eventItr++)
        {
            ...
            if(CPSS_PP_PORT_PM_LINK_STATUS_CHANGED_E == eventItr)
            {
                /* this is not HW event */
                continue;
            }
...
)
{
    ...
    cpssOsMemSet(PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST,0,sizeof(PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST));
    cpssOsMemSet(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST,0,sizeof(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST));
...
)
{
    ...
    if(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum])
    {
    ...
    PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum] = cpssOsMalloc(sizeof(PRV_CPSS_DRV_OBJ_STC));
    if(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum] == NULL)
    {
...
exit_cleanly_lbl:
    if((rc != GT_OK) && (PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum]))
    {
        /* release the allocation */
        cpssOsFree(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum]);
        PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum] = NULL;
    }
...
)
{
    ...
    if(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[newDevNum] || PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum))
    {
        CPSS_LOG_ERROR_AND_RETURN_MAC(GT_ALREADY_EXIST, LOG_ERROR_NO_MSG);
    }

    /* swap the function objects */
    PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[newDevNum] = PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum];
    PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum] = NULL;

    /* swap the info DB */
    PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[newDevNum] = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum);
...
)
{
    ...
    PRV_CPSS_DRV_START_LOOP_PORT_GROUPS_MAC(devNum,portGroupId)
    {
        ...
        FREE_PTR_MAC(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->intCtrl.portGroupInfo[portGroupId].swEvNodesPool);
    ...
    /* clear the memory */
    FREE_PTR_MAC(PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[devNum]);
    FREE_PTR_MAC(PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[devNum]);

    /* check if there are still existing devices */
    for(ii = 0 ; ii < PRV_CPSS_MAX_PP_DEVICES_CNS ; ii++)
    {
        if(PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[ii])
        {
            break;
        }
    }
...
)
{
    ...
    /* swap the function objects */
    PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[newDevNum] = PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[oldDevNum];
    PRV_SHARED_CPSS_DRIVER_PP_OBJ_CONFIG_NO_CAST[oldDevNum] = NULL;

    /* swap the info DB */
    PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[newDevNum] = PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[oldDevNum];
    PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[oldDevNum] = NULL;
    ...
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->hwInfo[0].devNum = newDevNum;
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->drivers[0]->devNum = newDevNum;
#ifndef ASIC_SIMULATION
    if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->drivers[0]->type == CPSS_HW_DRIVER_TYPE_PEX_E)
    {
        cpssHwDriverPexRenumber(newDevNum,PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(newDevNum)->drivers[0]);
    }
#endif
#endif
    /* Renumbers registry drivers (i2c, smi, xsmi) */
    cpssHwDriverRenumberDevNum(oldDevNum, newDevNum);
...
)
{
    ...
    PRV_ERRATA_STC_FIELD_GET(devNum,addrCheckFuncPtr)= addrCheckFuncPtr;
...
)
{
    ...
    PRV_ERRATA_STC_FIELD_GET(devNum,callbackFuncPtr) = callbackFuncPtr;
    PRV_ERRATA_STC_FIELD_GET(devNum,callbackDataPtr) = callbackDataPtr;
...
#define STR_FOR_FAMILY(family) drvPpInfoPtr->devFamily == family  ? #family :
    ...
    DEBUG_PRINT((" devFamily = [%s] \n",
        ...
        STR_FOR_FAMILY(CPSS_PP_FAMILY_DXCH_AAS_E          )
