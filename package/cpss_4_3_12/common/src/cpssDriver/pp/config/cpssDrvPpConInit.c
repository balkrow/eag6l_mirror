...
#else
    #define AAS_ADDR_MG0    0
    #define AAS_ADDR_SERVER 0
    #define AAS_ADDR_CNM_CnM_RFU 0
#endif /*CHX_FAMILY*/
 ...
 */
typedef struct {
    /** indication for 'early init' so can't use PRV_CPSS_PP_MAC(devNum)->isGmDevice
     * that not set yet.
     * set according to compilation flag that the device is GM (Golden Model) */
    GT_U32  earlyInit_isGmDevice;
    ...
    /** base address for CNM.RFU unit.
     *  needed in SIP7 where the MG unit not exists */
    GT_U32  rfuUnitBaseAddr;

    /** Revision Id ,as was read from the DFX server */
     GT_U32 revisionId;
...
#define RFU_DEVICE_ID_CNS              (mgDfxInfoPtr->rfuUnitBaseAddr + 0x00000028)
...
static const CPSS_PORTS_BMP_STC ironman_m_portsBmp = {
    {
        GLOBAL_PORT_BIT_MAC(16) |
        GLOBAL_PORT_BIT_MAC(17) |
        GLOBAL_PORT_BIT_MAC(18) |
        GLOBAL_PORT_BIT_MAC(19) |
        GLOBAL_PORT_BIT_MAC(20) |
        GLOBAL_PORT_BIT_MAC(21) |
        GLOBAL_PORT_BIT_MAC(22) |
        GLOBAL_PORT_BIT_MAC(23)
        ,
        GLOBAL_PORT_BIT_MAC(40) |
        GLOBAL_PORT_BIT_MAC(41) |
        GLOBAL_PORT_BIT_MAC(42) |
        GLOBAL_PORT_BIT_MAC(43) |
        GLOBAL_PORT_BIT_MAC(44) |
        GLOBAL_PORT_BIT_MAC(45) |
        GLOBAL_PORT_BIT_MAC(46) |
        GLOBAL_PORT_BIT_MAC(47) |
        GLOBAL_PORT_BIT_MAC(48) |
        GLOBAL_PORT_BIT_MAC(49) |
        GLOBAL_PORT_BIT_MAC(50) |
        GLOBAL_PORT_BIT_MAC(51) |
...
static const CPSS_PORTS_BMP_STC aas_portsBmp = {
    {
     /* start DP[0] */
     PORT_RANGE_MAC(0,16) |
     PORT_RANGE_MAC(20,12),
     PORT_RANGE_MAC(32,4) |
     PORT_RANGE_MAC(40,16) |
     PORT_RANGE_MAC(60,4),
     PORT_RANGE_MAC(64,12)
    }

};

/* Ironman - devices with integrated PHY */
static const  CPSS_PP_DEVICE_TYPE ironman_m_legalDevTypes[] =
{
...
/* AC5Y - devices with integrated PHY */
static const  CPSS_PP_DEVICE_TYPE ac5y_m_legalDevTypes[] =
{
    /* start with the PCIe identification , so the cpssSupportedTypes[] will have it */
    /* other wise 'pcie scan' by the appDemo will fail to recognize the device       */
...
/* AC5Y - devices without integrated PHY */
static const  CPSS_PP_DEVICE_TYPE ac5y_l_legalDevTypes[] =
{
    /* start with the PCIe identification , so the cpssSupportedTypes[] will have it */
    /* other wise 'pcie scan' by the appDemo will fail to recognize the device       */
...
/* AAS devices */
static const  CPSS_PP_DEVICE_TYPE aas_legalDevTypes[] =
{
    /* start with the PCIe identification , so the cpssSupportedTypes[] will have it */
    /* other wise 'pcie scan' by the appDemo will fail to recognize the device       */
...
/* AAS devices */
static const  CPSS_PP_DEVICE_TYPE aas_2_tiles_legalDevTypes[] =
{
    /* start with the PCIe identification , so the cpssSupportedTypes[] will have it */
    /* other wise 'pcie scan' by the appDemo will fail to recognize the device       */
...
#define AAS_CNS             CPSS_PP_FAMILY_DXCH_AAS_E
...
const CPSS_SUPPORTED_TYPES_STC cpssSupportedTypes[] =
{
    ...
    /* Ironman - devices with integrated PHY */
    {IRONMAN_CNS , 55  ,ironman_m_legalDevTypes,   &ironman_m_portsBmp, NULL,NULL},

    /* Ironman - devices without integrated PHY */
    {IRONMAN_CNS , 55  ,ironman_l_legalDevTypes,   NULL,NULL,NULL},

    /* AC5Y - devices with integrated PHY */
    {IRONMAN_CNS , 55  ,ac5y_m_legalDevTypes,      &ironman_m_portsBmp, NULL,NULL},

    /* AC5Y - devices without integrated PHY */
    {IRONMAN_CNS , 55  ,ac5y_l_legalDevTypes,      NULL,NULL,NULL},

    /*AAS Network-ports + SpecialPorts(CPU + LoopbackPerDP)*/
    {AAS_CNS     , NUM_OF_TILES_AND_PORTS(1,80+4+4) ,aas_legalDevTypes,         &aas_portsBmp,NULL,NULL},
    {AAS_CNS     , NUM_OF_TILES_AND_PORTS(2,160+2),aas_2_tiles_legalDevTypes, NULL,NULL,NULL},
...
)
{
    ...
    PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[devNum] = ppConfigPtr;
...
static const PRV_DEV_FAMILY_PREFIX_STC prefixDb[] = /* use representative device from each family */
{
    ...
    {PRV_FAMILY_PREFIX_GET_MAC(CPSS_98DXC720_CNS),      CPSS_PP_FAMILY_DXCH_HARRIER_E},/*Aldrin3M   */
    {PRV_FAMILY_PREFIX_GET_MAC(CPSS_98CN106xxS_CNS),    CPSS_PP_FAMILY_DXCH_HARRIER_E},/*Harrier_MCM*/
    ...
    {PRV_FAMILY_PREFIX_GET_MAC(CPSS_98DX4180_CNS),      CPSS_PP_FAMILY_DXCH_AAS_E},

    /* must be last (array termination) */
    {GT_NA,CPSS_PP_FAMILY_LAST_E}
};
/**
...
)
{
    ...
    for (ii = 0; prefixDb[ii].prefix != GT_NA; ii++)
    {
...
)
{
    switch(deviceFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_AAS_E:
            if(mgDfxInfoPtr->earlyInit_isGmDevice)
            {
                mgDfxInfoPtr->mg0UnitBaseAddr = AAS_ADDR_MG0;
            }
            else
            {
                mgDfxInfoPtr->rfuUnitBaseAddr = AAS_ADDR_CNM_CnM_RFU;
            }
            mgDfxInfoPtr->dfxUnitBaseAddr = AAS_ADDR_SERVER;
            break;
        default:
            break;
    }
...
)
{
    ...
    ppConfigPtr->hwInfo[portGroupId].devNum = devNum;
    ...
    else  /* PCI/PEX compatible */
    {
        ...
        if (deviceFamily == CPSS_PP_FAMILY_DXCH_HARRIER_E)
        {
            /* Pcie Software Conf Control */
            address = PRV_CPSS_HARRIER_PCI_E_SW_CONF_CONTROL_REG_ADDR_CNC;
            /* Assert PRAM Skip Enable, force LTSSM Enable and assert FW Load Done indication */
            rc = prvCpssDrvHwPpPortGroupWriteRegister(devNum, portGroupId, address, 0xF);
            if (rc!= GT_OK)
            {
                CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
            }

            /* Device General Control 4 */
            address = 0x000F8260;
            rc = prvCpssDrvHwPpPortGroupReadRegister(devNum, portGroupId,address, &regData);
            if(rc != GT_OK)
            {
        ...
        if(ppInInfoPtr->ppHAState == CPSS_SYS_HA_MODE_ACTIVE_E)
        {
            if(deviceFamily == CPSS_PP_FAMILY_DXCH_AAS_E && !mgDfxInfoPtr->earlyInit_isGmDevice)
            {
                /* need to read from the RFU unit (and not from the MG --> than not exists) .
                   regardless to ppInInfoPtr->devIdLocationType */
                    /* read DeviceID from MG register */
                ...
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "failed to read 'device_id' from the CNM.RFU");
                }

                CPSS_TBD_BOOKMARK_AAS
                /* no register for 'vendor id' */
                tempDevId = (readDeviceId << 16) | MARVELL_VENDOR_ID ;

                /* get the revision from the DFX */
                rc = prvCpssDrvHwPpGetRegField(devNum,
                         DFX_DEVICE_ID_AND_REV_REG_ADDR_CNS,
                         16,4,&mgDfxInfoPtr->revisionId);
                if (rc != GT_OK)
                {
                    CPSS_LOG_ERROR_AND_RETURN_MAC(rc, "failed to read 'revision' from the DFX");
                }

                goto after_readDeviceId;
            }
    ...
    if(deviceFamily == CPSS_PP_FAMILY_DXCH_AAS_E &&
       !mgDfxInfoPtr->earlyInit_isGmDevice)
    {
        /* currently no 'VENDOR_ID' in the AAS , so use the 'device Id' register
           that also can't be 0xFFFFFFFF (unless 'BUS read error') */
        ppConfigPtr->vendorIdRegAddress = RFU_DEVICE_ID_CNS;
    }
    else
    {
        /* store vendor ID register address , to read value that is NEVER 0xFFFFFFFF */
        ppConfigPtr->vendorIdRegAddress = VENDOR_ID_REG_ADDR;
    }
...
)
{
    ...
    if(ppConfigPtr->devFamily == CPSS_PP_FAMILY_DXCH_AAS_E &&
        !mgDfxInfoPtr->earlyInit_isGmDevice)
    {
        /* can't get it from the MG unit not from the RFU , so already got it
           from the DFX server */
        *revisionIdPtr = (GT_U8)mgDfxInfoPtr->revisionId;

        /* any init with the GDMA unit , need to be part of the GDMA init function
           and not here */
        return GT_OK;
    }
    ...
    if(CPSS_DXCH_ALL_SIP5_FAMILY_MAC(ppConfigPtr->devFamily) &&
        !mgDfxInfoPtr->earlyInit_isGmDevice)/* The GM not supports the DFX server registers */
    {
...
)
{
    ...
    CPSS_SYSTEM_RECOVERY_INFO_STC  oldSystemRecoveryInfo,newSystemRecoveryInfo; /*system recovery info*/
...
#ifdef GM_USED /* can't use PRV_CPSS_PP_MAC(devNum)->isGmDevice that not set yet */
    mgDfxInfo.earlyInit_isGmDevice = 1;
#else
    mgDfxInfo.earlyInit_isGmDevice = 0;
#endif
    ...
    */
    ...
    {
        rc = prvCpssHwDriverPipeSlaveSMISetDrv(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->drivers[0]);
        if (rc!= GT_OK)
            CPSS_LOG_ERROR_AND_RETURN_MAC(rc, LOG_ERROR_NO_MSG);
    }
#endif
    ...
    switch(ppOutInfoPtr->devFamily)
    {
        ...
        case CPSS_PP_FAMILY_DXCH_AAS_E:
                ...
                rc = prvCpssDrvPpIntDefDxChBobcat2Init__GM(devNum); /* in Lion2 file */
#else /*!GM_USED*/
                rc = prvCpssDrvPpIntDefDxChAasInit(devNum,ppOutInfoPtr->revision);
#endif /*!GM_USED*/
                if(rc != GT_OK)
                {
                    goto exit_cleanly_lbl;
...
exit_cleanly_lbl:
    if(rc != GT_OK)
    {
           ...
           PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum))
        {
            ...
            cpssOsFree(PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[devNum]);
            PRV_SHARED_CPSS_DRIVER_PP_CONFIG_NO_CAST[devNum] = NULL;
        }
    }
