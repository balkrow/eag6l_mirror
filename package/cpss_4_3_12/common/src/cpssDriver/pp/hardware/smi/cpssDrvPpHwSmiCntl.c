...
)
{
    ...
    retVal = cpssExtDrvHwIfSmiTaskReadReg(BASEADDR(devNum,portGroupId),
                                regAddr, &temp);
...
)
{
    ...
    retVal = cpssExtDrvHwIfSmiTaskWriteReg(BASEADDR(devNum,portGroupId),
                                 regAddr, temp);
    if (retVal != GT_OK)
    {
        return retVal;
...
* @retval GT_FAIL                  - on hardware error
*
* @note this function actually reads the register value modifies the requested field
...
)
{
    ...
    for( i = 0; i < length; i++)
    {
        debugPrint(("hwPpReadRam on device %d, from address 0x%x, data is 0x%x.\n",
...
)
{
    ...
    for( i = 0; i < length; i++)
    {
        debugPrint(("hwPpWriteRam on device %d, from address 0x%x, data is 0x%x.\n",
...
)
{
    ...
    retVal = cpssExtDrvHwIfSmiInterruptReadReg(BASEADDR(devNum,portGroupId),
                                     regAddr, &temp);
...
)
{
    ...
    temp = HW_BYTE_SWAP(devNum, portGroupId,value);
    retVal = cpssExtDrvHwIfSmiInterruptWriteReg(BASEADDR(devNum,portGroupId),
                                      regAddr, temp);
    if (retVal != GT_OK)
    {
        return retVal;
