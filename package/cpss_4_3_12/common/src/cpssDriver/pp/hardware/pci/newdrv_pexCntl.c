...
GT_VOID cpssHwDriverPexRenumber(
    IN  GT_U8               newDevNum,
...
)
{
    ...
    CPSS_HW_DRIVER_PEX_ALL_STC *drvPtr;

    drvPtr = (CPSS_HW_DRIVER_PEX_ALL_STC * )genDrvPtr;
...
#ifdef SHARED_MEMORY
    localDrv.type =drv->as[addrSpace]->type;
    /*need to reload function pointers due to ASLR*/
    prvCpssDrvHwDrvReload(&localDrv);
    readFunc = localDrv.read;
#else
    readFunc = drv->as[addrSpace]->read;
#endif

    rc = readFunc(drv->as[addrSpace], 0, regAddr, dataPtr, count);
...
#else
    writeMaskFunc = drv->as[addrSpace]->writeMask;
#endif


    rc = writeMaskFunc(drv->as[addrSpace], 0, regAddr, dataPtr, count, mask);

    if(GT_TRUE == pex_access_trace_enable && rc != GT_OK)/*save about half the number of lines during cpssInitSystem */
    {
        REG_PEX_REG_ACCESS_TRACE_PRINTF(regAddr-4,"PEX Write: ERROR : rc %d\n", rc);
    }
...
)
{
    ...
    if (hwInfo->resource.switching.start)
    {
...
#endif
        {
            d = cpssHwDriverGenMmapAc8CreateDrv(
                        ...
                        compRegionMask,CPSS_HW_DRIVER_AS_SWITCHING_E,hwInfo->devNum);
            if (d == NULL)
            {
                cpssOsFree(drv);
                return NULL;
    ...
    if (hwInfo->resource.mg1.start)
        drv->as[CPSS_HW_DRIVER_AS_MG1_E] = cpssHwDriverGenMmapAc8CreateDrv(
                        ...
                        0x3,CPSS_HW_DRIVER_AS_MG1_E,hwInfo->devNum);
    if (hwInfo->resource.mg2.start)
        drv->as[CPSS_HW_DRIVER_AS_MG2_E] = cpssHwDriverGenMmapAc8CreateDrv(
                        ...
                        0x1,CPSS_HW_DRIVER_AS_MG2_E,hwInfo->devNum);
    if (hwInfo->resource.mg3.start)
        drv->as[CPSS_HW_DRIVER_AS_MG3_E] = cpssHwDriverGenMmapAc8CreateDrv(
                        ...
                        0x1,CPSS_HW_DRIVER_AS_MG3_E,hwInfo->devNum);
