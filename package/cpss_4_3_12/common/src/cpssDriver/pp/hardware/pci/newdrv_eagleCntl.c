...
)
{
    ...
    /* check the cache of last matched (for consecutive access) */
    {
        ...
        {
#ifdef SHARED_MEMORY
            *winBase = PRV_PER_PROCESS_SWITCH_BASE_GET(drv->common.devNum)+ii*WINDOW_SIZE;
#else
    ...
    windowReplace[ii]++;
     ...
     drv->bar0->devNum = drv->common.devNum;
#else
...
)
{
    ...
    drv->common.devNum= hwInfo->devNum;
    ...
    cpssGlobalNonSharedDbPpMappingSyncPrimaryProcess(hwInfo,drv->common.devNum);
    drv->common.adressSpaceType = CPSS_HW_DRIVER_AS_CNM_E;
...
)
{
    ...
    {
        start = hwInfo->resource.cnm.start;
    }
    else
    {
        start = hwInfo->resource.switching.start + 1;
    }
