...
#define PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(_var) \
              (PRV_SHARED_GLOBAL_VAR_GET(commonMod.configDir.dDrvPpHwSrc._var))

void onEmulator_devNum_iterateRamActionSet(IN GT_U8  devNum)
{
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_devNum_iterateRamAction) = devNum;
}
...
)
{
    ...
    case CPSS_HW_DRIVER_TYPE_PEX_E:
        ...
        break;
    case CPSS_HW_DRIVER_TYPE_SRV_CPU_RAM_INDIRECT_E:
        cpssHwDriverSrvCpuRamIndirectDrvReload(drv);
        break;
    case CPSS_HW_DRIVER_TYPE_SRV_CPU_RAM_DIRECT_E:
        cpssHwDriverSrvCpuRamDirectDrvReload(drv);
        break;
#endif
    case CPSS_HW_DRIVER_TYPE_GEN_MMAP_RELOF_E:
        cpssHwDriverGenMmapRelativeAddrDrvReload(drv);
        break;
    case CPSS_HW_DRIVER_TYPE_GEN_MMAP_AC8_E:
        cpssHwDriverGenMmapAc8DrvReload(drv);
        break;
    case CPSS_HW_DRIVER_TYPE_GEN_MMAP_AC4_E:
        cpssHwDriverGenMmapAc4DrvReload(drv);
        break;
#ifdef ASIC_SIMULATION
    case CPSS_HW_DRIVER_TYPE_SIMULATION:
        cpssHwDriverSimulationReload(drv);
        break;
#endif
    case CPSS_HW_DRIVER_TYPE_SMI_E:
        cpssHwDriverSmiReloadDrv(drv);
        break;
    default:
        break;
    }
...
/**
* @internal onEmulator_checkAccessToSwitchIsOk_enableSet function
* @endinternal
*
* @brief 'on emulator' allow to check if the switch is accessible , and not return 0xFFFFFFFF on every read.
*           for that the function read 'well known' register address that it's content will not be 0xFFFFFFFF
*
* @param[in] enable          - enable/disable the check
*
* @retval GT_OK
*/
GT_STATUS onEmulator_checkAccessToSwitchIsOk_enableSet(IN GT_U32   enable)
{
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_checkAccessToSwitchIsOk_enable) = enable;
    return GT_OK;
}
/**
* @internal onEmulator_sleep_enableSet function
* @endinternal
*
* @brief 'on emulator' allow to do sleep of 'x' milisec after every read/write operation
*
* @param[in] sleepTime          - time to sleep in millisec
*
* @retval GT_OK
*/
GT_STATUS onEmulator_sleep_enableSet(IN GT_U32   sleepTime)
{
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_sleep) = sleepTime;
    return GT_OK;
}

/**
* @internal onEmulator_print_counterSet function
* @endinternal
*
* @brief 'on emulator' allow to print "." (as progress indication) every 'x' iterations
*
* @param[in] counter          - the 'x' value
*
* @retval GT_OK
*/
GT_STATUS onEmulator_print_counterSet(IN GT_U32   counter)
{
    PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter_max) = counter;
    return GT_OK;
}

/**
* @internal onEmulator_checkAccessToSwitchIsOk function
* @endinternal
*
* @brief 'on emulator' allow to check if the switch is accessible , and not return 0xFFFFFFFF on every read.
*           for that the function read 'well known' register address that it's content will not be 0xFFFFFFFF
*           other capabilities of this function:
*           1. print "." (as progress indication) every 'x' iterations
*               see function : onEmulator_print_counterSet(counter)
*           2. do sleep of 'x' milisec after every read/write operation
*               see function : onEmulator_sleep_enableSet(sleepTime)
*
* @param[in] devNum             - device number
* @param[in] drv                - pointer to the 'read/write driver'
*
* @retval None
*/
static void onEmulator_checkAccessToSwitchIsOk(
    IN GT_U8 devNum,
    IN CPSS_HW_DRIVER_STC *drv
)
{
    ...
    if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter_max))
    {
        PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter) ++;
        if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter) >= PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter_max))
        {
            cpssOsPrintf(".");
            PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_print_counter) = 0;
        }
    }

    if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_sleep))
    {
        cpssOsTimerWkAfter(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_sleep));
    }

    if(!PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_checkAccessToSwitchIsOk_enable))
    {
        return;
    }

    if(PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->vendorIdRegAddress != 0)
    {
        /*read the register that it's content can never be ZERO ---> unless ERROR on the BUS */
        regAddr = PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_PP_CONFIG(devNum)->vendorIdRegAddress;
    }
    else
    {
        return;
    }
    drv->read(drv, CPSS_HW_DRIVER_AS_SWITCHING_E, regAddr, &data, 1);

    if(data == 0xFFFFFFFF)
    {
        cpssOsPrintf("Error - read from 'well know addr' [0x%8.8x] is not OK any more got(0xFFFFFFFF) \n",regAddr);
    }
...
)
{
    ...
    rc = drv->common.parent->read(drv->common.parent, addrSpace, regAddr, dataPtr, count);
    if((*dataPtr) == 0x000badad)
    {
        cpssOsPrintf("ERROR : Reg [0x%8.8x] hold value 0x000badad \n",regAddr);
    }

    /* check that the PCIe to CnM still function */
    onEmulator_checkAccessToSwitchIsOk(drv->devNum,drv->common.parent);
...
)
{
    ...
    rc = drv->common.parent->writeMask(drv->common.parent, addrSpace, regAddr, dataPtr, count, mask);
...
)
{
    ...
    cpssGlobalNonSharedDbPpMappingSyncPrimaryProcess(hwInfo,devNum);
    ...
    if (drv != NULL)
    {
        ...
        {
            cpssHwDriverRegister(drv, s);
            cpssOsPrintf("NOTE : Running dedicated Emulator Driver (read/write registers) \n");
...
)
{
    ...
    if (PRV_ERRATA_STC_FIELD_GET(devNum,addrCheckFuncPtr) != NULL)
    {
        /* check address */
        rc = PRV_ERRATA_STC_FIELD_GET(devNum,addrCheckFuncPtr)(devNum,
                                                                 portGroupId,
                                                                 regAddr,
                                                                 1);
        if (rc != GT_OK)
    ...
    if (PRV_ERRATA_STC_FIELD_GET(devNum,callbackFuncPtr) != NULL)
    {
        rc = PRV_ERRATA_STC_FIELD_GET(devNum,callbackFuncPtr)(
            devNum, portGroupId,
            PRV_ERRATA_STC_FIELD_GET(devNum,callbackDataPtr),
...
)
{
    ...
    if(cpssDeviceRunCheck_onEmulator() && devNum == PRV_SHARED_GLOBAL_VAR_CPSS_DRIVER_HW(onEmulator_devNum_iterateRamAction))
    {
        GT_U32  ii;
...
/**
* @internal prvCpssDrvHwPpPortGroupWriteRamWithoutSkip function
* @endinternal
*
* @brief   Writes to PP's RAM.
*         in the specific port group in the device
...
*
*/
GT_STATUS prvCpssDrvHwPpPortGroupWriteRamWithoutSkip
(
    IN GT_U8    devNum,
    IN GT_U32   portGroupId,
    IN GT_U32   addr,
...
* @brief   Writes to PP's RAM.
*         in the specific port group in the device.
*         When HA flag is set, return GT_OK without write to ram
*
* @param[in] devNum                   - The PP device number to write to.
* @param[in] portGroupId              - The port group id. relevant only to 'multi-port-groups'
*                                      devices. Supports value CPSS_PORT_GROUP_UNAWARE_MODE_CNS
...
)
{
    ...
    return prvCpssDrvHwPpPortGroupWriteRamWithoutSkip(devNum, portGroupId, addr, length, data);
}
