...
)
{
    ...
    {
        CPSS_LOG_ERROR_MAC("partition->pointedFirstBlock should not be NULL since it is always allocated in prvCpssDmmCreatePartition");
        return 0;
    }
...
)
{
    ...
    if( (blockSize > partition->maxBlockSizeInWords) ||
        ...
        (alignment != 2 )*/ )
    {
        CPSS_LOG_ERROR_MAC("prvCpssDmmAllocate : wrong size calculation!!!!");
        return DMM_BLOCK_NOT_FOUND;
    }
...
)
{
    ...
    if (protocol>2)
    {
        CPSS_LOG_ERROR_MAC("protocol not valid in prvCpssDmmGetAllocatedLinesForGivenPartitionAndProtocol");
        return 0;
    }
...
)
{
    ...
    if (protocol>2)
    {
        CPSS_LOG_ERROR_MAC("protocol not valid in prvCpssDmmSortPartition");
        return 0;
    }
