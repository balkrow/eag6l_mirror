...
#ifdef SHARED_MEMORY
typedef GT_U32 AVL_TREE_CALLBACK_FUNC ;
#else
typedef prototypeCmpFunc AVL_TREE_CALLBACK_FUNC;
#endif
...
typedef struct{
     ...
     */
    AVL_TREE_CALLBACK_FUNC compareFunc;
...
*                                      by malloc function.
*
* @param[in] compareFuncId   - Id of compare function.Required for shared library mode in order to identigy function.
*
...
)
{
    ...
    treePtr->compareFunc = (AVL_TREE_CALLBACK_FUNC)PRV_CALLBACK_ID_GET(compareFunc);

    if(!treePtr->compareFunc)
    {
...
*                                      user data, or NULL if no free operation should be done
*                                      on the user's data. The function gets 2 parameters:
*                                      compareFuncId pointer and data ponter from node.
* @param[in] cookiePtr                - cookie parameter to be passed to dataFreeFunc function.
*
* @retval GT_STATUS      GT_OK     - For successful operation.
* @retval GT_NOT_INITIALIZED       - the given tree has not been initialized.
...
)
{
    ...
    compareFunc = PRV_CALLBACK_GET(treePtr->compareFunc,prototypeCmpFunc);
...
)
{
    ...
    pRetData = prvCpssAvlRecusSearch(treePtr->pHead, pData,compareFunc);
...
)
{
     ...
     */
    {
        ...
        pNode = prvCpssAvlRecusFindNext(treePtr->pHead, pUserNode->pData, compareFunc);
...
)
{
    ...
    {
        compareFunc = PRV_CALLBACK_GET(treePtr->compareFunc,prototypeCmpFunc);
        cprResult =  compareFunc(pNode->pData, pData);
        switch (cprResult)
        {
            case GT_EQUAL:
...
)
{
    ...
    {
        compareFunc = PRV_CALLBACK_GET(treePtr->compareFunc,prototypeCmpFunc);
        if (GT_SMALLER != compareFunc(dbEntry1Ptr, dbEntry2Ptr))
        {
            return GT_FALSE;
        }
