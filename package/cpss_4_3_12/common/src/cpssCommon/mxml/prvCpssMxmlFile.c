 ...
 * Contents:
 *
 *   prvCpssMxmlLoadFd()            - Load a file descriptor into an XML node tree.
 *   --prvCpssMxmlLoadFile()          - Load a file into an XML node tree.
 *   prvCpssMxmlLoadString()        - Load a string into an XML node tree.
 *   prvCpssMxmlSaveAllocString()   - Save an XML node tree to an allocated string.
 *   prvCpssMxmlSaveFd()            - Save an XML tree to a file descriptor.
 *   --prvCpssMxmlSaveFile()          - Save an XML tree to a file.
 *   prvCpssMxmlSaveString()        - Save an XML node tree to a string.
 *   prvCpssMxmlSAXLoadFd()         - Load a file descriptor into an XML node tree
 *                             using a SAX callback.
 *   --prvCpssMxmlSAXLoadFile()       - Load a file into an XML node tree
 *                             using a SAX callback.
 *   prvCpssMxmlSAXLoadString()     - Load a string into an XML node tree
 *                             using a SAX callback.
 *   prvCpssMxmlSetCustomHandlers() - Set the handling functions for custom data.
 *   prvCpssMxmlSetErrorCallback()  - Set the error message callback.
 *   prvCpssMxmlSetWrapMargin()     - Set the the wrap margin when saving XML data.
...
/*
 * Limit recursion depth that can require large amount of stack space
 */
...
{
  CPSS_OS_FILE_TYPE_STC          *fp;         /* File descriptor */
  void*             fs;
...
static GT_INLINE int   mxml_isspace(int ch)
            {
              return (ch == ' ' || ch == '\t' || ch == '\r' ||
                      ch == '\n');
            }
...
static int      mxml_write_node(mxml_node_t *node, void *p,
                    ...
                    _mxml_putc_cb_t putc_cb,
                    _mxml_global_t *global, int recursion_depth);
...
prvCpssMxmlLoadFd(mxml_node_t    *top,     /* I - Top node */
           CPSS_OS_FILE_TYPE_STC       *fp,       /* I - File descriptor to read from */
           void           *fs,
...
mxml_node_t *               /* O - First node or NULL if the file could not be read. */
prvCpssMxmlLoadFile(mxml_node_t    *top,   /* I - Top node */
...
prvCpssMxmlLoadString(mxml_node_t    *top, /* I - Top node */
    ...
    mxml_save_cb_t cb)          /* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  */
  ...
  bytes = prvCpssMxmlSaveString(node, buffer, sizeof(buffer), cb);
  ...
  */
  ...
  prvCpssMxmlSaveString(node, s, bytes + 1, cb);
...
prvCpssMxmlSaveFd(mxml_node_t    *node,            /* I - Node to write */
           CPSS_OS_FILE_TYPE_STC       *fp,               /* I - File descriptor to write to */
           void           *fs,              /* I - genFS_PTR */
           mxml_save_cb_t cb)               /* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  */
  ...
  if ((col = mxml_write_node(node, &buf, cb, 0, mxml_fd_putc, global, RECURSION_DEPTH)) < 0)
    return (-1);
...
int                 /* O - 0 on success, -1 on error. */
prvCpssMxmlSaveFile(mxml_node_t    *node,  /* I - Node to write */
...
prvCpssMxmlSaveString(mxml_node_t    *node,    /* I - Node to write */
               ...
               mxml_save_cb_t cb)   /* I - Whitespace callback or MXML_NO_CALLBACK */
{
  ...
  */
  ...
  if ((col = mxml_write_node(node, ptr, cb, 0, mxml_string_putc, global, RECURSION_DEPTH)) < 0)
    return (-1);
 ...
 * child nodes of the specified type.
 *
 * The SAX callback must call prvCpssMxmlRetain() for any nodes that need to
...
mxml_node_t *               /* O - First node or NULL if the file could not be read. */
prvCpssMxmlSAXLoadFd(mxml_node_t    *top,  /* I - Top node */
              CPSS_OS_FILE_TYPE_STC       *fp,    /* I - File descriptor to read from */
              mxml_load_cb_t cb,    /* I - Callback function or MXML_NO_CALLBACK */
...
void
prvCpssMxmlSetErrorCallback(mxml_error_cb_t cb)/* I - Error callback function */
{
  _mxml_global_t *global = _mxml_global();
                    /* Global data */
...
void
prvCpssMxmlSetWrapMargin(int column)       /* I - Column for wrapping, 0 to disable wrapping */
{
  _mxml_global_t *global = _mxml_global();
                    /* Global data */
...
mxml_fd_read(_mxml_fdbuf_t *buf)        /* I - File descriptor buffer */
{
  ...
  */
  ...
  if ((bytes = cpssOsFread(buf->buffer, 1, sizeof(buf->buffer), buf->fp)) < 0)
      return (-1);
...
mxml_fd_write(_mxml_fdbuf_t *buf)   /* I - File descriptor buffer */
{
  ...
  */
  ...
  for (ptr = buf->buffer; ptr < buf->current; ptr += bytes)
    if ((bytes =cpssOsFwrite(buf->buffer, 1, sizeof(buf->buffer), buf->fp))  < 0)
      return (-1);
                    ...
                    /* I  - Get character function */
{
  ...
  *entptr = '\0';
  ...
  else if ((ch = prvCpssMxmlEntityGetValue(entity)) < 0)
  ...
  while ((ch = (*getc_cb)(p, &encoding)) != EOF)
  {
        ...
        bufptr > buffer)
    {
      ...
      *bufptr = '\0';
      ...
      {
      case MXML_INTEGER :
        node = prvCpssMxmlNewInteger(parent, _mxml_strtol(buffer, &bufptr, 0));
        break;

      case MXML_OPAQUE :
        node = prvCpssMxmlNewOpaque(parent, buffer);
        break;

#if 0
      case MXML_REAL :
        node = prvCpssMxmlNewReal(parent, strtod(buffer, &bufptr));
        break;
#endif

      case MXML_TEXT :
        node = prvCpssMxmlNewText(parent, whitespace, buffer);
        break;
      ...
      case MXML_CUSTOM :
        if (global->custom_load_cb)
        {
          ...
          node = prvCpssMxmlNewCustom(parent, NULL, NULL);
    ...
    */
    ...
    {
      node = prvCpssMxmlNewText(parent, whitespace, "");
    ...
    *bufptr = '\0';
    ...
    if ((node = prvCpssMxmlNewElement(parent, buffer)) == NULL)
    {
     /*
      ...
      */
      ...
      if (cb && parent)
        ...
        else if (sax_cb)
        {
          ...
          if (!prvCpssMxmlRelease(node) && first == node)
            first = NULL;
        }
      }
  ...
  */
  ...
  while ((ch = (*getc_cb)(p, encoding)) != EOF)
  {
    ...
    *ptr = '\0';
    ...
    if (prvCpssMxmlElementGetAttr(node, name))
      goto error;
    ...
    */
    ...
    while ((ch = (*getc_cb)(p, encoding)) != EOF)
      ...
      */
      ...
      prvCpssMxmlElementSetAttr(node, name, value);
    }
    else
    {
      mxml_error("Missing value for attribute '%s' in element %s!",
                    ...
                    /* I - Write callback */
{
  ...
  if (*s == '\"' || *s == '\'')
  {
    ...
    */
    ...
    while (*s && *s != quote)
    {
      if ((name = prvCpssMxmlEntityGetName(*s)) != NULL)
      {
    if ((*putc_cb)('&', p) < 0)
          return (-1);
  ...
  while (node != NULL)
  {
    ...
    */
    ...
    switch (node->type)
    {
      ...
      else if ((!(node->child)) && (node->value.element.name[0] == '!' ||
               node->value.element.name[0] == '?'))
      {
           /*
