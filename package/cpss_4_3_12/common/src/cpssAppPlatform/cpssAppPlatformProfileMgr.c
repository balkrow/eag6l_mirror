...
#define PRV_APP_REF_PROFILE_MGR_VAR(_var) PRV_SHARED_GLOBAL_VAR_GET(appRefMod.cpssProfileMgrSrc._var)
...
)
{
    ...
    {
        if ( PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[devNum] )
        {
            *profileStr =  PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[devNum];
            return GT_OK;
        }
    }
    ...
    {
        if (PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr)
        {
            *profileStr = PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr;
            return GT_OK;
        }
    }
...
)
{
    ...
    for( i=0; i<CPSS_APP_PLATFORM_MAX_PP_CNS; i++)
    {
        ...
        PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[i] = NULL;
    }

    for( i=0; i<CPSS_APP_PLATFORM_MAX_PP_CNS; i++)
    {

        PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_db).ppProfileStr[i] = NULL;
    }

    PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr = NULL;
    PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_db).runtimeProfileStr = NULL;
...
)
{
    ...
    if (entryType & CPSS_APP_PLATFORM_PROFILE_DB_ENTRY_TYPE_PP_E)
    {
       PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[devNum] = NULL;
       PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_db).ppProfileStr[devNum] = NULL;
    }
...
)
{
    ...
    for (i=0; i<PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count); i++)
    {
        if (PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_list_arr)[i] == profilePtr && PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i])
        {
            if (entryType & CPSS_APP_PLATFORM_PROFILE_DB_ENTRY_TYPE_PP_E)
                PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[devNum] = PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i];
            else if (entryType & CPSS_APP_PLATFORM_PROFILE_DB_ENTRY_TYPE_RT_E)
                PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr = PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i];
            else
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
    ...
    for (i=0; i<PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_count); i++)
    {
        if (PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_list_arr)[i] == profilePtr && PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_name_arr)[i])
        {
            if (entryType & CPSS_APP_PLATFORM_PROFILE_DB_ENTRY_TYPE_PP_E)
                PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_db).ppProfileStr[devNum] = PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_name_arr)[i];
            else if (entryType & CPSS_APP_PLATFORM_PROFILE_DB_ENTRY_TYPE_RT_E)
                PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_db).runtimeProfileStr = PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_name_arr)[i];
            else
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
...
{
   return  PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count);
}
...
{
   PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count)++;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[count] = name;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_descr_arr)[count] = descr;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_list_arr)[count] = name;
}
...
{
   return  PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_count);
}
...
{
   PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_count)++;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_name_arr)[count] = name;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_descr_arr)[count] = descr;
}
...
{
    PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_list_arr)[count] = name;
}
...
)
{
    ...
    for (i=0; i < PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count); i++)
    {
        if (0 == cpssOsStrCmp(PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i], profileName))
        {
            if (PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_list_arr)[i])
            {
                *profile = PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_list_arr)[i];
                PRV_APP_REF_PROFILE_MGR_VAR(last_used_profile) =i;
                return GT_OK;
            } else {
                CPSS_LOG_ERROR_AND_RETURN_MAC(GT_FAIL, LOG_ERROR_NO_MSG);
...
)
{
    ...
    for (i=0; i<PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count); i++)
    {
        if (PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_list_arr)[i] == profilePtr && PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i])
        {
            *profileName = PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i];
            return GT_OK;
        }
    }
...
)
{
    ...
    for (i=0; i < PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_count); i++)
    {
        CPSS_APP_PLATFORM_LOG_PRINT_MAC("\n[%2d]   %40s : %s", i, PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[i],
                                                                  PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_descr_arr)[i]);
    }
...
)
{
    ...
    for (i=0; i < PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_count); i++)
    {
        CPSS_APP_PLATFORM_LOG_PRINT_MAC("\n[%2d]   %40s : %s", i, PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_name_arr)[i],
                                                                  PRV_APP_REF_PROFILE_MGR_VAR(cap_test_profile_descr_arr)[i]);
    }
...
)
{
    ...
    for (i=0; i < CPSS_APP_PLATFORM_MAX_PP_CNS; i++)
    {
        if (PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[i])
            CPSS_APP_PLATFORM_LOG_PRINT_MAC("\n[%2d]  [PP: %s ][RT: %s ]", i,
                          PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[i] ? PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).ppProfileStr[i] : "NULL",
                          PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr ? PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_db).runtimeProfileStr : "NULL"
                        );
    }
...
{
    return PRV_APP_REF_PROFILE_MGR_VAR(cap_profile_name_arr)[PRV_APP_REF_PROFILE_MGR_VAR(last_used_profile)];
}