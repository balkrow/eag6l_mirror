...
#define PRV_APP_REF_PORT_INIT_GLOBAL_VAR(_var) PRV_SHARED_GLOBAL_VAR_GET(appRefMod.cpssPortInitSrc._var)
...
{
    PRV_APP_REF_PORT_INIT_GLOBAL_VAR(appRefPortManagerTaskSleepTime) = timeInMilli;
    return GT_OK;
}
...
)
{
    ...
    ***********************************************************/
    ...
    /************************************************************
        1.1 Getting port Params
    ***********************************************************/
    rc = cpssDxChPortManagerPortParamsGet(devNum, portNum, &portParams);
    if (rc != GT_OK && rc != GT_NOT_INITIALIZED)
    {
        CPSS_APP_PLATFORM_LOG_ERR_MAC("cpssDxChPortManagerPortParamsGet returned=%d, portNum=%d\n", rc, portNum);
        return rc;
    }
    ...
    if (portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_FAILURE_E ||
       ...
       portParams.portParamsType.regPort.ifMode != CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E))
    {
    ...
    ***********************************************************/
        ...
        (CPSS_PORT_MANAGER_EVENT_PORT_AP_PARALLEL_DETECT_E != portEventStc.portEvent))
    {
        ...
        /* Signal the handler */
        if(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(isSip6) == GT_FALSE)
        {
            rc = cpssOsSigSemSignal(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(portInitSeqSignalSem));
        }
    }
...
)
{
    ...
    while(1)
    {
        if(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(isSip6) == GT_FALSE)
        {
            cpssOsSigSemWait(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(portInitSeqSignalSem), 0);
        }
        ...
        while (portsExistFlag>0)
        {
            ...
            if(GT_TRUE==PRV_APP_REF_PORT_INIT_GLOBAL_VAR(stopPmTask))
            {
                PRV_APP_REF_PORT_INIT_GLOBAL_VAR(stopPmTask) = GT_FALSE;
                        ...
                        **********************************************************************/
                        ...
                        if(PRV_CPSS_SIP_6_CHECK_MAC(devNum) || PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum))
                        {
                            ...
                            if(!loopback)
                            {
                                ...
                                if( ((portConfigOutParams.ifMode != CPSS_PORT_INTERFACE_MODE_QSGMII_E) &&
                                     (portConfigOutParams.ifMode != CPSS_PORT_INTERFACE_MODE_USX_QUSGMII_E)) &&

                                    ((portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_LINK_UP_E) ||
                                     (portConfigOutParams.portState == CPSS_PORT_MANAGER_STATE_MAC_LINK_DOWN_E)) &&

                                    !(CAP_PORT_MANAGER_ANALOG_TX2RX_LOOPBACK_CHECK(portParams)) )
                                {
                        ...
                        {
                            CPSS_APP_PLATFORM_LOG_DBG_MAC("Launching whole INIT on port %d, devNum : %d\n",portNum, devNum);
                        ...
                        if (i % 32 == 31)
                        {
                            ...
                            cpssOsTimerWkAfter(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(appRefPortManagerTaskSleepTime)/4);
        ...
        cpssOsTimerWkAfter(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(appRefPortManagerTaskSleepTime));
        } /* while (portsExistFlag>0) */
...
)
{
    ...
    ***********************************************************/
    ...
    if (portStage.portState == CPSS_PORT_MANAGER_STATE_FAILURE_E ||
        ((portStage.portUnderOperDisable == GT_TRUE) && (!PRV_CPSS_SIP_6_10_CHECK_MAC(devNum)) && (!PRV_CPSS_DXCH_IS_AC3_BASED_DEVICE_MAC(devNum)) && (portParams.portParamsType.regPort.ifMode != CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E)))
    {
...
)
{
    ...
    for(devNum = 0; devNum < CPSS_APP_PLATFORM_MAX_PP_CNS; devNum++)
    {
        ...
        {
            PRV_APP_REF_PORT_INIT_GLOBAL_VAR(isSip6) = PRV_CPSS_SIP_6_CHECK_MAC(devNum);
            break;
        }
    }
    ...
    if(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(isSip6) == GT_FALSE)
    {
        ...
        if( (0 == PRV_APP_REF_PORT_INIT_GLOBAL_VAR(portInitSeqSignalSem)))
        {
            if(cpssOsSigSemBinCreate("PRV_APP_REF_PORT_INIT_GLOBAL_VAR(portInitSeqSignalSem)", CPSS_OS_SEMB_EMPTY_E, &PRV_APP_REF_PORT_INIT_GLOBAL_VAR(portInitSeqSignalSem)) != GT_OK)
            {
    ...
    rc = cpssOsTaskCreate("portManagerTask", /* Task Name      */
                           ...
                           NULL,             /* Arguments list */
                           &PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid));       /* task ID        */
    if (rc != GT_OK)
    {
    ...
    CPSS_APP_PLATFORM_LOG_INFO_MAC("portManagerTask created with task id %d (0x%x)\n",PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid),PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid));
...
* @param[in] speed                 - speed for the port
* @param[in] fecMode               - fec mode for the port
* @param[in] laneParams            - lane TX/RX parameters
* @param[in] portEnable            - is port enabled
* @param[in] overrideTxParam       - Override default tx-parameters.
...
(
    ...
    IN  CPSS_PM_MODE_LANE_PARAM_STC  laneParams,
    IN  GT_BOOL                      portEnable,
    IN  GT_BOOL                      overrideTxParam,
    IN  GT_BOOL                      powerUp
)
{
    GT_STATUS rc = GT_OK;
    ...
    if (powerUp)
    {
        ...
        if (CPSS_IS_DXCH_FAMILY_MAC(PRV_CPSS_PP_MAC(devNum)->devFamily))
        {
            if((ifMode == CPSS_PORT_INTERFACE_MODE_USX_OUSGMII_E) || (ifMode == CPSS_PORT_INTERFACE_MODE_USX_10G_QXGMII_E) ||
                    (ifMode == CPSS_PORT_INTERFACE_MODE_QSGMII_E))
            {
                ...
                rc = cpssDxChSamplePortManagerAutoNegotiationSet(devNum,portNum,GT_TRUE,GT_FALSE,GT_FALSE,GT_FALSE,GT_TRUE,GT_TRUE,GT_FALSE);
                if (rc != GT_OK)
                {
                    CPSS_APP_PLATFORM_LOG_ERR_MAC("cpssDxChSamplePortManagerAutoNegotiationSet returned %d\n", rc);
                }
            } else {
        ...
        if(GT_TRUE == overrideTxParam)
        {
          if(speed == CPSS_PORT_SPEED_25000_E && PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_FALCON_E)
          {
              rc = cpssDxChSamplePortManagerSerdesTxParamsSet(devNum, portNum, 0, &(laneParams.txParams));
              if (rc != GT_OK)
              {
                  CPSS_APP_PLATFORM_LOG_ERR_MAC("cpssDxChSamplePortManagerSerdesTxParamsSet returned %d\n", rc);
                  return rc;
              }
          }
          /*Tx Params Set not called when all params are INVALID*/
          if( (PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5P_E
                  && (laneParams.txParams.txTune.comphy_C112G.pre  != NA_16BIT
                  ||  laneParams.txParams.txTune.comphy_C112G.pre2 != NA_16BIT
                  ||  laneParams.txParams.txTune.comphy_C112G.post != NA_16BIT
                  ||  laneParams.txParams.txTune.comphy_C112G.main != NA_16BIT))
              ||(PRV_CPSS_PP_MAC(devNum)->devFamily == CPSS_PP_FAMILY_DXCH_AC5X_E
                  && (laneParams.txParams.txTune.comphy_C28G.pre  != NA_16BIT
                  ||  laneParams.txParams.txTune.comphy_C28G.post != NA_16BIT
                  ||  laneParams.txParams.txTune.comphy_C28G.peak != NA_16BIT)))
          {
              rc = cpssDxChSamplePortManagerSerdesTxParamsSet(devNum, portNum, 0, &(laneParams.txParams));
              if (rc != GT_OK)
              {
...
)
{
    ...
    if(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid))
    {
        PRV_APP_REF_PORT_INIT_GLOBAL_VAR(stopPmTask) = GT_TRUE;

        while(iterator--)
        {
            if(GT_FALSE==PRV_APP_REF_PORT_INIT_GLOBAL_VAR(stopPmTask))
            {
                break;
            }
        ...
        rc = cpssOsTaskDelete(PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid));       /* task ID        */
        if (rc != GT_OK)
        {
        ...
        CPSS_APP_PLATFORM_LOG_INFO_MAC("portManagerTask task id %d (0x%x) is deleted. Graceful exit %d\n",PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid),
            PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid),0!=iterator);

        PRV_APP_REF_PORT_MGR_MODE = GT_FALSE;
        PRV_APP_REF_PORT_INIT_GLOBAL_VAR(stopPmTask) = GT_FALSE;
        PRV_APP_REF_PORT_INIT_GLOBAL_VAR(pmTid) = 0;
    }
