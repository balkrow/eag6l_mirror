...
GT_STATUS cpssAppPlatformSystemPpInfoGet
(
    IN  CPSS_APP_PLATFORM_PROFILE_STC  *inputProfileListPtr,
    OUT GT_U8                          *swDevNum
)
...
)
{
    ...
    CPSS_APP_PLATFORM_PROFILE_STC     *profileListPtr = inputProfileListPtr;
    CPSS_APP_PLATFORM_PROFILE_STC     *cascadeProfileListPtr = inputProfileListPtr;
    ...
    PRV_CPSS_APP_START_LOOP_PROFILE_MAC(cascadeProfileListPtr, CPSS_APP_PLATFORM_PROFILE_TYPE_PP_E)
    {
        if(cascadeProfileListPtr->profileValue.ppInfoPtr == NULL)
        {
            /* PP profile is Empty. proceed to next profile */
            continue;
        }
        ppProfilePtr = cascadeProfileListPtr->profileValue.ppInfoPtr;
        /*****************************************************/
    ...
    PRV_CPSS_APP_END_LOOP_PROFILE_MAC(cascadeProfileListPtr, CPSS_APP_PLATFORM_PROFILE_TYPE_PP_E)
...
)
{
    ...
    if (system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
    {
        ...
        if(PRV_APP_REF_RUN_TIME_CONFIG_VAR(eventHandlerInitDone) == GT_FALSE)
        {
            ...
            PRV_APP_REF_RUN_TIME_CONFIG_VAR(eventHandlerInitDone)= GT_TRUE;
        }
    }

    /* Port Manager Init */
    if(PRV_APP_REF_PORT_MGR_MODE == GT_FALSE)
    {
    ...
    if (system_recovery.systemRecoveryProcess == CPSS_SYSTEM_RECOVERY_PROCESS_NOT_ACTIVE_E)
    {
        for(devNum = 0; devNum < CPSS_APP_PLATFORM_MAX_PP_CNS; devNum++)
        {
            ...
            {
                if(NULL!=cpssAppUtilsGenEventFuncInitCb)
                {
                    rc = cpssAppUtilsGenEventFuncInitCb(devNum);
