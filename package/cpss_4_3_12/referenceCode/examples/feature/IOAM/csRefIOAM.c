...
)
{
    ...
    char       fourByteChar[9];
    char       byteChar[3];
    GT_U32     i, nextHeadProt=0, ttl=0, lngthTemp=0, traceTypeBmp=0, hopLimit, nodeID, ingressInt, egressInt, numOfRecords=0;
    GT_BOOL    isIPv4;
    char       *pktStringTmp;
    char       *ioamPtr;
...
parsing_return_label:
    ...
    if((traceTypeBmp > 0) && (traceTypeBmp <= 32) && (lngthTemp >= 2))
    {
        numOfRecords = (GT_U32)((lngthTemp - 2) / ones((GT_U32)traceTypeBmp));
    }
    else
    {
        numOfRecords = 0;
