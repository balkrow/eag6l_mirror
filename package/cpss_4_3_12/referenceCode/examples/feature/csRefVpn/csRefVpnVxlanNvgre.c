...
/*         Feature relate code                 */
/***********************************************/
#define VPN_SYS_DEAFULT_TPID_PROFILE_CNS 0
...
/***********************************************/
/*      vxLAN table related DB infomation      */
/***********************************************/
#define VPN_TS_FIRST_ENTRY_CNS    0
#define VPN_TS_NUM_OF_ENTRIES_CNS 10
#define VPN_TS_ENTRY_SIZE_CNS     2 /* Num of lines in entry. */
static  GT_U32  prvVpnCurrAvaiableTsEntry = 0xFFFFFF;


#define VPN_TTI_NUM_OF_ENTRIES_CNS 10
#define VPN_TTI_ENTRY_SIZE_CNS   3 /* Num of lines in entry. */
static  GT_U32  prvVpnFirstTtiEntry=0xFFFFFF, prvVpnCurrAvaiableTtiEntry=0xFFFFFF;


#define VPN_PCL_NUM_OF_ENTRIES_CNS 10
#define VPN_PCL_ENTRY_SIZE_CNS   3 /* Num of lines in entry. */
static  GT_U32  prvVpnFirstPclEntry=0xFFFFFF, prvVpnCurrAvaiablePclEntry=0xFFFFFF;
...
/* prvVpnLocalIpAddress - Array of local device IP address.
*                         If it is the packet's, then packet's is further processed  It is the SIP in ingress packet
*                         It is the DIP in egress packet 

static GT_U8   prvVpnLocalIpAddress[16];
static GT_BOOL prvVpnLocalIpAddressSet = GT_FALSE;
*/
...
)
{
  ...
  /************************** Undo configuration block ************************/
  if(GT_TRUE == undoConfig)
  {
      rc = cpssDxCh3PclTunnelTermForceVlanModeEnableSet(devNum, GT_TRUE);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
...
)
{
  ...
  static GT_U32  sourceIdTtiOverrideBmp;
  ...
  if(GT_TRUE == undoConfig)
  {
      ...
      /* is undo needed */
      if(sourceIdTtiOverrideBmp & (1<<VPN_MESH_ID_BIT_IN_SOURCE_ID_CNS))
      {
      ...
      rc = prvVpnIpv6NNITtiPclConfigurationSet(devNum, GT_TRUE);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      /* Set profile bitmap to include 0x8100 only for ETH1 and ETH0. */
      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_DEAFULT_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE0_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_DEAFULT_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE1_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_EMPTY_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE0_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_EMPTY_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE1_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);


      rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E,0 ,0x8100);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_SYS_DEAFULT_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE0_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanIngressTpidProfileSet(devNum, VPN_SYS_DEAFULT_TPID_PROFILE_CNS, CPSS_VLAN_ETHERTYPE1_E, 1);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E, 0, 0x8100);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E, 1, 0x8100);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
  ...
  prvVpnFirstTtiEntry = appDemoDxChTcamTtiBaseIndexGet(devNum, 0); /* get base index for TTI first hit lookup. */
  prvVpnCurrAvaiableTtiEntry = prvVpnFirstTtiEntry;

  prvVpnFirstPclEntry = appDemoDxChTcamIpclBaseIndexGet(devNum, 0); /* get base index for PCL first lookup. */
  prvVpnCurrAvaiablePclEntry = prvVpnFirstPclEntry;

  prvVpnCurrAvaiableTsEntry = VPN_TS_NUM_OF_ENTRIES_CNS;
  ...
  rc = cpssDxChTtiSourceIdBitsOverrideSet(devNum, VPN_TTI_LOOKUP_CNS,
                                          sourceIdTtiOverrideBmp | (1<<VPN_MESH_ID_BIT_IN_SOURCE_ID_CNS));
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);
  ...
  rc = prvVpnIpv6NNITtiPclConfigurationSet(devNum, GT_FALSE);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);
  ...
  rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_INGRESS_E,1 ,0x88A8);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);

  rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E,0 ,0x8100);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);

  rc = cpssDxChBrgVlanTpidEntrySet(devNum, CPSS_DXCH_ETHERTYPE_TABLE_EGRESS_E,1 ,0x88A8);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);
...
* @param[in] loalIpAddress - Array of local system IP address.
*
*
* @retval GT_OK                    - on success
* @retval GT_BAD_PARAM             - on wrong parameters
...
*
*/
static GT_STATUS prvCsRefVpnMac2MeSet
(
  IN GT_U8          devNum,
  IN GT_ETHERADDR  *mac2me/*,
  IN GT_CHAR       *loalIpAddress*/,
  IN GT_BOOL        undoConfig
)
{
    GT_STATUS rc = GT_OK;
...
{
    GT_STATUS rc;
    rc = prvCsRefVpnMac2MeSet(devNum, mac2me, GT_FALSE);
    CSREF_LOG_RETURN_NOT_OK_MAC(rc);
...
)
{
  ...
  rc = prvVpnVxlanNvgreGenericInit(devNum, GT_FALSE);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);

  if(prvVpnCurrAvaiableTtiEntry >= prvVpnFirstTtiEntry + VPN_TTI_NUM_OF_ENTRIES_CNS*VPN_TTI_ENTRY_SIZE_CNS)
  ...
  entryIndex = prvVpnCurrAvaiableTtiEntry;
  ...
  /* Advance enrty pointers. */
  prvVpnCurrAvaiableTtiEntry += VPN_TTI_ENTRY_SIZE_CNS;
...
)
{
  ...
  if( (prvVpnCurrAvaiableTsEntry  >= VPN_TS_FIRST_ENTRY_CNS + VPN_TS_NUM_OF_ENTRIES_CNS*VPN_TS_ENTRY_SIZE_CNS)||
      (prvVpnCurrAvaiableTtiEntry >= prvVpnFirstTtiEntry + VPN_TTI_NUM_OF_ENTRIES_CNS*VPN_TTI_ENTRY_SIZE_CNS)  ||
      (prvVpnCurrAvaiablePclEntry >= prvVpnFirstPclEntry + VPN_PCL_NUM_OF_ENTRIES_CNS*VPN_PCL_ENTRY_SIZE_CNS)   )
      CSREF_LOG_RETURN_NOT_OK_MAC(GT_NO_RESOURCE);
  ...
  */
  ...
  if(ipProt == CPSS_IP_PROTOCOL_IPV6_E)
  {
    ...
    entryIndex = prvVpnCurrAvaiablePclEntry;
  ...
  /* Egress Processing. */
  routerArpTunnelStartLineIndex = prvVpnCurrAvaiableTsEntry;
...
*/
  ...
  /* Advance enrty pointers. */
  prvVpnCurrAvaiableTsEntry  += VPN_TS_ENTRY_SIZE_CNS;
  prvVpnCurrAvaiableTtiEntry += VPN_TTI_ENTRY_SIZE_CNS;
  if(ipProt == CPSS_IP_PROTOCOL_IPV6_E)
    prvVpnCurrAvaiablePclEntry += VPN_PCL_ENTRY_SIZE_CNS;
...
/*  CPSS_TUNNEL_TYPE_ENT tunnelType; */
  ...
  entryIndex = prvVpnFirstTtiEntry;
  rc = cpssDxChTtiRuleSet(devNum, entryIndex, ruleType, &ttiPattern, &ttiMask, &ttiAction);
  CSREF_LOG_RETURN_NOT_OK_MAC(rc);
  ...
  /* Advance tables pointers */
  prvVpnCurrAvaiableTsEntry  += VPN_TS_ENTRY_SIZE_CNS;
  prvVpnFirstTtiEntry        += VPN_TTI_ENTRY_SIZE_CNS;
...
/**
* @internal csRefVpnConfigClean function
* @endinternal
*
...
*
* @param[in] devNum             - device number
* @param[in] firstAssignedEport - first ePort assigned the NNI or UNI interface
* @param[in] numOfEports        - NUmber of assigned ePorts.
* @param[in] physicalEportBmp   - bitmap of physical ePorts used in VPN configuration.
*
*
*
* @retval GT_OK                    - on success
...
*
*/
GT_STATUS csRefVpnConfigClean
(
  IN GT_U8              devNum,
  IN GT_PORT_NUM        firstAssignedEport,
  IN GT_U32             numOfEports,
  IN CPSS_PORTS_BMP_STC physicalEportBmp
)
{
    GT_STATUS                          rc = GT_OK;
    CPSS_DXCH_TTI_KEY_TYPE_ENT         keyType;
    ...
    {
      if(CPSS_PORTS_BMP_IS_PORT_SET_MAC(&physicalEportBmp, portNum) == 0) 
        continue;
      ...
      /* Set TPID to profile that recognize 0x8100 only as TAG1 for physical port (no need for assigned eport) */
      rc = cpssDxChBrgVlanPortIngressTpidProfileSet(devNum, portNum, CPSS_VLAN_ETHERTYPE0_E, GT_TRUE, VPN_SYS_DEAFULT_TPID_PROFILE_CNS);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);

      rc = cpssDxChBrgVlanPortIngressTpidProfileSet(devNum, portNum, CPSS_VLAN_ETHERTYPE1_E, GT_TRUE, VPN_SYS_DEAFULT_TPID_PROFILE_CNS);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
    }


   /* assigned eport undo configuration. */
    for(portNum = firstAssignedEport ; portNum < firstAssignedEport+numOfEports ; portNum++)
    {
      /* Egress processing undo */
      osMemSet(&physicalInfo,0,sizeof(physicalInfo));
      rc = cpssDxChBrgEportToPhysicalPortTargetMappingTableSet(devNum, portNum, &physicalInfo);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      /* Clean ePort configuration history.  */
      osMemSet(&egressEportInfo, 0, sizeof(egressEportInfo));
      rc = cpssDxChBrgEportToPhysicalPortEgressPortInfoSet(devNum, portNum, &egressEportInfo);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      /* Enable egress vlan filterring on eport */
      rc = cpssDxChBrgEgrFltVlanEPortFilteringEnableSet(devNum, portNum, GT_TRUE);
      if(rc != GT_NOT_APPLICABLE_DEVICE)
          CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      /* Take egress tag state from eVlan */
      rc = cpssDxChBrgVlanEgressPortTagStateModeSet(devNum, portNum, CPSS_DXCH_BRG_VLAN_EPORT_TAG_STATE_EVLAN_MODE_E);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
      
      /* Send packet with Tag1 only if received with Tag1. */
      rc = cpssDxChBrgVlanPortRemoveTag1IfRxWithoutTag1EnableSet(devNum, portNum, GT_FALSE);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
    }   


    /* Clean all TTI rules */
    for(entryIndex = prvVpnFirstTtiEntry ; entryIndex  < prvVpnCurrAvaiableTtiEntry ; entryIndex += VPN_TTI_ENTRY_SIZE_CNS)
    {
      rc = cpssDxChTtiRuleValidStatusSet(devNum, entryIndex , GT_FALSE);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
    }
    
    /* Clean all PCL rules */
    for(entryIndex = prvVpnFirstPclEntry ; entryIndex  < prvVpnCurrAvaiablePclEntry ; entryIndex += VPN_PCL_ENTRY_SIZE_CNS)
    {
      rc = cpssDxChPclRuleInvalidate(devNum, 0/*tcamIndex*/, CPSS_PCL_RULE_SIZE_30_BYTES_E, entryIndex);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
    }
    
    /* Clean all tunnel start rules */    
    for(entryIndex = VPN_TS_FIRST_ENTRY_CNS ; entryIndex  < prvVpnCurrAvaiableTtiEntry ; entryIndex += VPN_TS_ENTRY_SIZE_CNS)
    {
      osMemSet(&tsConfig, 0, sizeof(tsConfig));
      rc = cpssDxChTunnelStartEntrySet(devNum, entryIndex, CPSS_TUNNEL_GENERIC_IPV6_E, &tsConfig);
      CSREF_LOG_RETURN_NOT_OK_MAC(rc);
    }

    rc = prvCsRefVpnMac2MeSet(devNum, NULL, GT_TRUE);
    CSREF_LOG_RETURN_NOT_OK_MAC(rc);
 
    rc = prvVpnVxlanNvgreGenericInit(devNum, GT_TRUE);
    CSREF_LOG_RETURN_NOT_OK_MAC(rc);
