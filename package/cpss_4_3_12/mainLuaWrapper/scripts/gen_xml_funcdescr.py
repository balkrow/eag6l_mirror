...
reTypedefFloat = re.compile(r'(typedef) (double|float) ([a-zA-Z_][a-zA-Z_0-9]*);\r?\n?$')
...
def scan_type_info_from_gdb(file_name):
    ...
    def line_type(line):
        if line.startswith("string") or line.startswith("GT_STRING"):
            return "none"
        ...
        if reTypedefInt.match(line) != None:
            return "int"
        if reTypedefFloat.match(line) != None:
            return "float"
    ...
    for line in lines:
        ...
        if ltype in ["enum", "struct", "union" ]:
            ...
            continue
        if ltype == "float":
            m = reTypedefFloat.match(line)
            all_defs[m.group(3)] = "float"
        if ltype == "int":
        ...
        for decl in scan_c_src(source):
            ...
            for param in decl['params']:
                ...
                if ptype in typeExcludeList:
                    ...
                    break
                if pdir in [ "IN", "INOUT" ]:
                    if type_classify(ptype) not in ["bool", "int", "float", "enum", "struct", "union","string" ]:
        ...
        for t in structures_required[:]:
            ...
            if t not in all_structs and t in scanned_structs:
                ...
                for fld in c['flds']:
                    ...
                    handled = True
                    if fcls in ["bool", "int", "float", "enum", "struct", "union","string"]:
