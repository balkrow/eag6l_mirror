...
)
{
    ...
    if (rc != GT_OK)
    {
        /* maybe previous test have not cleared capture - retry */
...
/*******************************************************************************
* prvLuaTgfBuildPacketPart
*
* DESCRIPTION:
*       Build packet bytes from structure of TGF_PACKET_STC that the LUA hold
*       and return the bytes as 'string'
*
* INPUTS:
*       packetInfoPtr - (pointer to) the packet info
*
* OUTPUTS:
*       1. rc - return code
*       2. sting of the bytes for this packet part(s)
*
* RETURNS:
*       return code
*
...
)
{
    ...
    GT_U32       packetLen;/* number of bytes in packetBuff*/ /* save space of CRC*/
    TGF_AUTO_CHECKSUM_FULL_INFO_STC checksumInfo[1];

    cpssOsMemSet(&checksumInfo[0], 0, sizeof(checksumInfo));
    cmdOsMemSet(&packetInfo, 0,sizeof(packetInfo));

    PARAM_STRUCT(rc, &packetInfo, 1, TGF_PACKET_STC);
    PRV_LUA_TGF_TR_VERIFY_GT_OK(rc, packetInfo, "packetInfo : Input parameters");

    packetLen = TGF_RX_BUFFER_MAX_SIZE_CNS - 4;/* save space of CRC*/
    /* build packet from input fields */
    rc = tgfTrafficEnginePacketBuild(&packetInfo, packetBuff,&packetLen,checksumInfo,NULL);
    PRV_LUA_TGF_VERIFY_GT_OK(rc, "tgfTrafficEnginePacketBuild : failed");

    if(checksumInfo[0].singleInfoArr[TGF_AUTO_CHECKSUM_FIELD_IPV4_E].enable)
    {
        TGF_AUTO_CHECKSUM_INFO_STC *currChecksumInfoPtr;/* current checksum info pointer */

        currChecksumInfoPtr = &checksumInfo[0].singleInfoArr[TGF_AUTO_CHECKSUM_FIELD_IPV4_E];
        ...
        if(rc != GT_OK)
        {
            PRV_LUA_TGF_VERIFY_GT_OK(rc, "tgfTrafficEnginePacketCheckSum16BitsCalc : failed");
        }
    }
    ...
    return 2;/* rc , packet bytes as string */
}
