...
*           struct  TGF_PACKET_VXLAN_GPE_STC
*           struct  TGF_PACKET_NSH_STC
*           struct  TGF_PACKET_SGT_TAG_STC
*           enum    PRV_TGF_BRG_VLAN_PORT_TAG_CMD_ENT
*           struct  TGF_PACKET_SRH_STC
*
* DEPENDENCIES:
*
* COMMENTS:
...
void prv_lua_to_c_TGF_PACKET_VXLAN_GPE_STC(
    lua_State *L,
    TGF_PACKET_VXLAN_GPE_STC *val
)
{
    F_NUMBER(val, -1, flags, GT_U8);
    F_NUMBER(val, -1, reserved0, GT_U16);
    F_NUMBER(val, -1, nextProtocol, GT_U8);
    F_NUMBER(val, -1, vni, GT_U32);
    F_NUMBER(val, -1, reserved1, GT_U8);
}

void prv_c_to_lua_TGF_PACKET_VXLAN_GPE_STC(
    lua_State *L,
    TGF_PACKET_VXLAN_GPE_STC *val
)
{
    ...
    FO_NUMBER(val, t, reserved0, GT_U16);
    FO_NUMBER(val, t, nextProtocol, GT_U8);
    FO_NUMBER(val, t, vni, GT_U32);
    FO_NUMBER(val, t, reserved1, GT_U8);
    lua_settop(L, t);
}
add_mgm_type(TGF_PACKET_VXLAN_GPE_STC);

void prv_lua_to_c_TGF_PACKET_NSH_STC(
    lua_State *L,
    TGF_PACKET_NSH_STC *val
)
{
    F_NUMBER(val, -1, baseHdr, GT_U32);
    F_NUMBER(val, -1, servicePathHdr, GT_U32);
    F_NUMBER(val, -1, metaDataWord3Hdr, GT_U32);
    F_NUMBER(val, -1, metaDataWord2Hdr, GT_U32);
    F_NUMBER(val, -1, metaDataWord1Hdr, GT_U32);
    F_NUMBER(val, -1, metaDataWord0Hdr, GT_U32);
}

void prv_c_to_lua_TGF_PACKET_NSH_STC(
    lua_State *L,
    TGF_PACKET_NSH_STC *val
)
{
    ...
    lua_newtable(L);
    t = lua_gettop(L);
    FO_NUMBER(val, t, baseHdr, GT_U32);
    FO_NUMBER(val, t, servicePathHdr, GT_U32);
    FO_NUMBER(val, t, metaDataWord3Hdr, GT_U32);
    FO_NUMBER(val, t, metaDataWord2Hdr, GT_U32);
    FO_NUMBER(val, t, metaDataWord1Hdr, GT_U32);
    FO_NUMBER(val, t, metaDataWord0Hdr, GT_U32);
    lua_settop(L, t);
}
add_mgm_type(TGF_PACKET_NSH_STC);

void prv_lua_to_c_TGF_PACKET_SGT_TAG_STC(
    lua_State *L,
    TGF_PACKET_SGT_TAG_STC *val
)
{
    F_NUMBER(val, -1, etherType, TGF_ETHER_TYPE);
    ...
    F_NUMBER(val, -1, sgtValue, GT_U16);
}

void prv_c_to_lua_TGF_PACKET_SGT_TAG_STC(
    lua_State *L,
    TGF_PACKET_SGT_TAG_STC *val
)
{
    ...
    FO_NUMBER(val, t, sgtValue, GT_U16);
    lua_settop(L, t);
}
add_mgm_type(TGF_PACKET_SGT_TAG_STC);
...
void prv_lua_to_c_TGF_PACKET_SRH_STC(
    lua_State *L,
    TGF_PACKET_SRH_STC *val
)
{
    F_NUMBER(val, -1, nextHeader,   GT_U8);
    F_NUMBER(val, -1, headerLength, GT_U8);
    F_NUMBER(val, -1, routingType,  GT_U8);
    F_NUMBER(val, -1, segmentsLeft, GT_U8);
    F_NUMBER(val, -1, lastEntry,    GT_U8);
    F_NUMBER(val, -1, flags,        GT_U8);
    F_NUMBER(val, -1, tag,          GT_U16);
}

void prv_c_to_lua_TGF_PACKET_SRH_STC(
    lua_State *L,
    TGF_PACKET_SRH_STC *val
)
{
    ...
    lua_newtable(L);
    t = lua_gettop(L);
    FO_NUMBER(val, t, nextHeader,   GT_U8);
    FO_NUMBER(val, t, headerLength, GT_U8);
    FO_NUMBER(val, t, routingType,  GT_U8);
    FO_NUMBER(val, t, segmentsLeft, GT_U8);
    FO_NUMBER(val, t, lastEntry,    GT_U8);
    FO_NUMBER(val, t, flags,        GT_U8);
    FO_NUMBER(val, t, tag,          GT_U16);
    lua_settop(L, t);
}

add_mgm_enum(TGF_PACKET_SRH_STC);
