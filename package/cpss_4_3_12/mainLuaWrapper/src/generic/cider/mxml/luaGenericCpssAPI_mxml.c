...
#ifdef SHARED_MEMORY
#define LUA_GENERIC_CPSS_API_MALLOC(l)      malloc(l)
#define LUA_GENERIC_CPSS_API_FREE(l)        free(l)
#else
#define LUA_GENERIC_CPSS_API_MALLOC(l)     osMalloc(l)
#define LUA_GENERIC_CPSS_API_FREE(l)        osFree(l)
#endif
...
static int prvLuaCpssAPI_load_mxml(lua_State *L)
{
    ...
    fp = LUA_GENERIC_CPSS_API_MALLOC(sizeof(CPSS_OS_FILE_TYPE_STC));
    if(fp == NULL)
    {
        return 1;
    }
