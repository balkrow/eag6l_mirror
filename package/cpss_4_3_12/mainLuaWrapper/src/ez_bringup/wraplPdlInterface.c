...
int wr_utils_pdl_interface_run_validation(lua_State *L)
{
        ...
        pdlStatus = iDbgPdlConvertStringToEnum(IDBG_PDL_UTILS_INTERFACE_EXTENDED_TYPE, (char*) interfaceTypeStr, &value);
        if (pdlStatus != PDL_OK)
        {
                if (verbose)
                {
        ...
        interfaceType = (PDL_INTERFACE_EXTENDED_TYPE_ENT) value;
        ...
        for (; index < count; index++)
                ...
                if ((interfaceType == PDL_INTERFACE_EXTENDED_TYPE_SMI_E || interfaceType == PDL_INTERFACE_EXTENDED_TYPE_XSMI_E) && (smiXsmiAttributes.interfaceType != interfaceType))
                continue;
                ...
                for (subInterator = 0; subInterator < subInteratorEnd; subInterator++)
                                ...
                                case PDL_INTERFACE_EXTENDED_TYPE_SMI_E:
                                        ...
                                        if (smiXsmiAttributes.interfaceType == interfaceType)
                                                ...
                                                IDBG_PDL_LOGGER_IGNORE_RESULT_PDL_EXECUTE(pdlStatus, pdlSmiXsmiHwGetValue, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, index),
                                                                IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_8, smiXsmiAttributes.phyInfo.readWriteAddress.dev),
                        ...
                        if (verbose)
                        {
                                ...
                                if (interfaceType == PDL_INTERFACE_EXTENDED_TYPE_SMI_E || interfaceType == PDL_INTERFACE_EXTENDED_TYPE_XSMI_E)
                                                {
                                        IDBG_PDL_LOGGER_API_OUTPUT_LOG(" [dev %d]:", smiXsmiAttributes.phyInfo.readWriteAddress.dev);
                                }
                                else
                                        if (interfaceType == PDL_INTERFACE_EXTENDED_TYPE_GPIO_E)
                                                        {
