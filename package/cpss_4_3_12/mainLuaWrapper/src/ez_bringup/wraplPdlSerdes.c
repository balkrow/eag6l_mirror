                ...
                )
{
        ...
        const char *ifModeStr, *connectorTypeStr, *connectorTechStr;
        ...
        PDL_CONNECTOR_TECHNOLOGY_ENT connectorTech;
        ...
        for (dev = firstDev; dev < lastDev; dev++)
                ...
                while (1)
                {
                        ...
                        is_exist = FALSE;
                        for (pdlStatus2 = pdlSerdesDbFineTuneAttrGetFirst(dev, lane, &interfaceMode, &connectorType, &connectorTech);
                                        pdlStatus2 == PDL_OK;
                                        pdlStatus2 = pdlSerdesDbFineTuneAttrGetNext(dev, lane, interfaceMode, connectorType, connectorTech, &interfaceMode, &connectorType, &connectorTech))
                                                        {
                                is_exist = TRUE;
                                IDBG_PDL_LOGGER_IGNORE_RESULT_PDL_EXECUTE(pdlStatus, pdlSerdesDbFineTuneAttrGet, IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, dev),
                                                ...
                                                IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(UINT_32, connectorTech),
                                ...
                                if (pdlStatus != PDL_OK)
                                {
                                        ...
                                        if (verbose)
                                        {
                                                pdlLibConvertEnumValueToString(PDL_ENUM_ID_L1_INTERFACE_MODE_TYPE_E, interfaceMode, (char**) &ifModeStr);
                                                pdlLibConvertEnumValueToString(PDL_ENUM_ID_CONNECTOR_TYPE_TYPE_E, connectorType, (char**) &connectorTypeStr);
                                                pdlLibConvertEnumValueToString(PDL_ENUM_ID_CONNECTOR_TECHNOLOGY_TYPE_E,connectorTech, (char**) &connectorTechStr);
                                                IDBG_PDL_LOGGER_API_OUTPUT_LOG("dev %d lane %d mode %s connector %s Tech %d: Can't retrieve configuration from PDL\n", dev, lane, ifModeStr,
                                                                connectorTypeStr, connectorTechStr);
                                        }
                                        continue;
                                }
...
{
        int i;
        wr_utils_pdl_serdes_set_params_TYP params[11];
        IDBG_PDL_LOGGER_RESULT_DATA_STC result;
        if (lua_gettop(L) == 10)
                        {
        ...
        IDBG_PDL_LOGGER_API_EXECUTE(WR_PDL_CMDLINE_GET_MAC(L),
                        ...
                        IDBG_PDL_LOGGER_ADD_IN_PARAM_MAC(PTR, params[10].paramPtr));
        if (result.outDescriptionPtr)
        printf("%s", result.outDescriptionPtr);
...
int wr_utils_pdl_serdes_apply_config(lua_State *L)
{
        ...
        pdlStatus = pdlLibConvertStringToEnumValue(PDL_ENUM_ID_SPEED_TYPE_E, (char*) speedStr, &pdlSpeed);
        if (pdlStatus != PDL_OK)
        {
                if (verbose)
                {
        ...
        for (pdlStatus = pdlSerdesPortDbSerdesInfoGetFirst(dev, port, &serdesInfo);
                ...
                for (connectorType = PDL_CONNECTOR_TYPE_SFPPLUS_E; connectorType < PDL_CONNECTOR_TYPE_LAST_E; connectorType++)
                                {
                        pdlStatus2 = pdlSerdesDbFineTuneAttrGet(dev, serdesInfo.absSerdesNum, pdlIfMode, connectorType, PDL_CONNECTOR_TECHNOLOGY_UNKNOWN_E,
