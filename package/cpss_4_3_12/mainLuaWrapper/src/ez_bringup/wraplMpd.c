...
int wr_utils_mpd_get_green_power_consumption(lua_State *L)
{
        ...
        params.internal.phyPowerConsumptions.shortReach = shortReach;
        params.internal.phyPowerConsumptions.energyDetetct = energyDetetct;
        params.internal.phyPowerConsumptions.portSpeed = portSpeed;
        params.internal.phyPowerConsumptions.getMax = getMax; 
        params.internal.phyPowerConsumptions.portUp = portUp;
        mpd_result = mpdPerformPhyOperation(rel_ifIndex, (MPD_OP_CODE_ENT)PRV_MPD_OP_CODE_GET_GREEN_POW_CONSUMPTION_E, &params);
        if (mpd_result != MPD_OK_E){
            if (mpd_result == MPD_OP_FAILED_E){
                    sprintf(error, "failed");
            }
        ...
        printf("get green power consumption operation on relIfindex %d is %d\n", 
                rel_ifIndex, params.internal.phyPowerConsumptions.greenConsumption);
...
int wr_utils_mpd_get_green_readiness(lua_State *L)
{
        ...
        params.internal.phyGreen.type = readinessType;

        mpd_result = mpdPerformPhyOperation(rel_ifIndex, (MPD_OP_CODE_ENT)PRV_MPD_OP_CODE_GET_GREEN_READINESS_E, &params);
        if (mpd_result != MPD_OK_E){
            if (mpd_result == MPD_OP_FAILED_E){
                    sprintf(error, "failed");
            }
        ...
        printf("readiness: %s\n");
        if (params.internal.phyGreen.readiness == MPD_GREEN_READINESS_OPRNORMAL_E){
            printf("OPRNORMAL\n");
        }
        else if (params.internal.phyGreen.readiness == MPD_GREEN_READINESS_FIBER_E){
            printf("FIBER\n");
        }
        else if (params.internal.phyGreen.readiness == MPD_GREEN_READINESS_COMBO_FIBER_E){
            printf("COMBO FIBER\n");
        }
        else if (params.internal.phyGreen.readiness == MPD_GREEN_READINESS_NOT_SUPPORTED_E){
            printf("NOT SUPPORTED\n");
        }
        else if (params.internal.phyGreen.readiness == MPD_GREEN_READINESS_ALWAYS_ENABLED_E){
            printf("ALWAYS_ENABLED\n");
        }

        printf("Short reach speeds are %d:\n");
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_10M_CNS){
            printf("10M ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_10M_HD_CNS){
            printf("10M_HD ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_100M_CNS){
            printf("100M ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_100M_HD_CNS){
            printf("100M_HD ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_1G_CNS){
            printf("1G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_10G_CNS){
            printf("10G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_2500M_CNS){
            printf("2500M ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_5G_CNS){
            printf("5G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_12G_CNS){
            printf("12G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_16G_CNS){
            printf("16G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_13600M_CNS){
            printf("13600M ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_20G_CNS){
            printf("20G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_40G_CNS){
            printf("40G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_100G_CNS){
            printf("100G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_25G_CNS){
            printf("25G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_50G_CNS){
            printf("50G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_24G_CNS){
            printf("24G ");
        }
        if (params.internal.phyGreen.srSpeeds & MPD_SPEED_CAPABILITY_200G_CNS){
            printf("200G ");
        }
...
int wr_utils_mpd_get_dte_status(lua_State *L)
{
        ...
        if (params.phyDteStatus.detect == TRUE)
        {
...
int wr_utils_mpd_set_combo_media_type(lua_State *L)
{
        ...
        mpd_result = mpdPerformPhyOperation(rel_ifIndex, (MPD_OP_CODE_ENT)PRV_MPD_OP_CODE_SET_COMBO_MEDIA_TYPE_E, &params);
        if (mpd_result != MPD_OK_E)
                        {
                if (mpd_result == MPD_OP_FAILED_E)
                                {
...
/*****************************************************************************
 * FUNCTION NAME: wr_utils_mpd_set_ptp_init
 *
 * DESCRIPTION:
 *
 *
 *****************************************************************************/
int wr_utils_mpd_set_ptp_init(lua_State *L)
{
        UINT_32 phyNumber = 0;
        MPD_OPERATIONS_PARAMS_UNT opParams;
        MPD_RESULT_ENT mpd_result;
        const char *reasonStr;
        char error[40] = "not supported";
        UINT_32 nanoSeconds, lowSeconds, highSeconds, fractionalNanoSeconds;
        if (lua_gettop(L) == 6) {
                phyNumber = lua_tointeger(L, 1);
                reasonStr = lua_tostring(L, 2);
                nanoSeconds = lua_tointeger(L, 3);
                lowSeconds = lua_tointeger(L, 4);
                highSeconds = lua_tointeger(L, 5);
                fractionalNanoSeconds = lua_tointeger(L, 6);
        }
        else {
                lua_pushnil(L);
                return 0;
        ...
        if (strcmp(reasonStr, "MPD_PTP_LOAD_TIME_FOR_PULSE_E") == 0) {
            opParams.phyPtpInit.reason = MPD_PTP_LOAD_TIME_FOR_PULSE_E;
        } else if (strcmp(reasonStr, "MPD_PTP_LOAD_TIME_FOR_PPS_E") == 0) {
            opParams.phyPtpInit.reason = MPD_PTP_LOAD_TIME_FOR_PPS_E;
        } else {
            sprintf(error, "unknown reason type %s",reasonStr);
            printf("phyNumber %d error:%s \n",phyNumber ,error);
            lua_pushnil(L);
            return 0;
        }
        opParams.phyPtpInit.loadTime.fractionalNanoSeconds = fractionalNanoSeconds;
        opParams.phyPtpInit.loadTime.highSeconds = highSeconds;
        opParams.phyPtpInit.loadTime.lowSeconds = lowSeconds;
        opParams.phyPtpInit.loadTime.nanoSeconds = nanoSeconds;
        mpd_result = mpdPerformOperationOnPhy(phyNumber, MPD_OP_PHY_CODE_SET_PTP_INIT_E, &opParams);
        if (mpd_result != MPD_OK_E)
        {
                if (mpd_result == MPD_OP_FAILED_E)
                {
                ...
                printf("MPD_OP_PHY_CODE_SET_PTP_INIT_E operation is %s on phyNumber %d\n", error, phyNumber);
                lua_pushnumber(L, 0);
                return 1;
        }
...
/*$ END OF wr_utils_mpd_set_ptp_init */
...
int wr_utils_mpd_set_present_notification(lua_State *L)
{
        ...
        if (lua_gettop(L) == 3) {
                ...
                opModeStr = lua_tostring(L, 3);
                if (strcmp(opModeStr, "MPD_OP_MODE_FIBER_E") == 0)
                    opMode = MPD_OP_MODE_FIBER_E;
                else
                    if (strcmp(opModeStr, "MPD_OP_MODE_DIRECT_ATTACH_E") == 0)
                        opMode = MPD_OP_MODE_DIRECT_ATTACH_E;
                    else
                        if (strcmp(opModeStr, "MPD_OP_MODE_COPPER_SFP_1000BASE_X_MODE_E") == 0)
                            opMode = MPD_OP_MODE_COPPER_SFP_1000BASE_X_MODE_E;
                        else
                            if (strcmp(opModeStr, "MPD_OP_MODE_COPPER_SFP_SGMII_MODE_E") == 0)
                                opMode = MPD_OP_MODE_COPPER_SFP_SGMII_MODE_E;
                            else
                                if (strcmp(opModeStr, "MPD_OP_MODE_UNKNOWN_E") == 0)
                                    opMode = MPD_OP_MODE_UNKNOWN_E;
                                else
                                {
                                        lua_pushnil(L);
                                        return 0;
                                }
                params.phySfpPresentNotification.opMode = opMode;
        }
        else {
                lua_pushnil(L);
                return 0;
...
int wr_utils_mpd_set_loop_back(lua_State *L)
{
        ...
        mpd_result = mpdPerformPhyOperation(rel_ifIndex, (MPD_OP_CODE_ENT)PRV_MPD_OP_CODE_SET_LOOP_BACK_E, &params);
        if (mpd_result != MPD_OK_E)
                        {
                if (mpd_result == MPD_OP_FAILED_E)
                                {
...
int wr_utils_mpd_mtd_debug_reg_dump(lua_State *L)
{
        ...
        const char *dumpTypeStr;
        char error[20] = "not supported";
    
        if (lua_gettop(L) == 2) {
                rel_ifIndex = lua_tointeger(L, 1);
                dumpTypeStr = lua_tostring(L, 2);
                if (strcmp(dumpTypeStr, "MPD_DEBUG_REGISTERS_FULL_DUMP_E") == 0)
                    dumpType = MPD_DEBUG_REGISTERS_FULL_DUMP_E;
                else 
                    if (strcmp(dumpTypeStr, "MPD_DEBUG_REGISTERS_STRAP_DUMP_E") == 0)
                        dumpType = MPD_DEBUG_REGISTERS_STRAP_DUMP_E;
                    else 
                        if (strcmp(dumpTypeStr, "MPD_DEBUG_REGISTERS_LINK_FLAP_DEBUG_DUMP_E") == 0)
                            dumpType = MPD_DEBUG_REGISTERS_LINK_FLAP_DEBUG_DUMP_E;
                         else
                        {
                            lua_pushnil(L);
                            return 0;
        ...
        mpd_result = prvMpdMtdDebugRegDump(rel_ifIndex, dumpType);
        if (mpd_result != MPD_OK_E)
                        {
                if (mpd_result == MPD_OP_FAILED_E)
                                {
...
/*****************************************************************************
 * FUNCTION NAME: wr_utils_mpd_debug_set_ptp_tod_capture
 *
 * DESCRIPTION:
 *
 *
 *****************************************************************************/
int wr_utils_mpd_debug_set_ptp_tod_capture(lua_State *L)
{
        UINT_32 phyNumber;
        MPD_RESULT_ENT mpd_result;
        char error[20] = "not supported";
        ...
        mpd_result = mpdPerformOperationOnPhy(phyNumber, MPD_OP_PHY_CODE_SET_PTP_TOD_CAPTURE_E, &params);
        if (mpd_result != MPD_OK_E)
        {
            if (mpd_result == MPD_OP_FAILED_E)
            {
            ...
            printf("MPD_OP_PHY_CODE_SET_PTP_TOD_CAPTURE_E operation is %s on phy Number %d\n", error, phyNumber);
            lua_pushnumber(L, 0);
            return 1;
        }
        lua_pushnumber(L, 0);
...
/*$ END OF wr_utils_mpd_debug_set_ptp_tod_capture */

/*****************************************************************************
 * FUNCTION NAME: wr_utils_mpd_debug_get_ptp_tod_capture_value
 *
 * DESCRIPTION:
 *
 *
 *****************************************************************************/
int wr_utils_mpd_debug_get_ptp_tod_capture_value(lua_State *L)
{
        UINT_32 phyNumber;
        MPD_RESULT_ENT mpd_result;
        MPD_PTP_CAPTURED_FRC_STC *capturedFrc_PTR;
        char error[20] = "not supported";
        MPD_OPERATIONS_PARAMS_UNT params;
        ...
        mpd_result = mpdPerformOperationOnPhy(phyNumber, MPD_OP_PHY_CODE_GET_PTP_TOD_CAPTURE_VALUE_E, &params);
        if (mpd_result != MPD_OK_E)
        {
            if (mpd_result == MPD_OP_FAILED_E)
            {
            ...
            printf("MPD_OP_PHY_CODE_GET_PTP_TOD_CAPTURE_VALUE_E operation is %s on phy Number %d\n", error, phyNumber);
            lua_pushnumber(L, 0);
            return 1;
        }

        capturedFrc_PTR = &params.phyPtpCapturedFrc.todCapture0;
        if (capturedFrc_PTR->valid) {
            printf("phyNumber [%d] todCapture0: highSeconds [%d] lowSeconds [%d] nanoseconds [%d] fractionalNanoseconds [%d]\n"
                   ,phyNumber , capturedFrc_PTR->highSeconds, capturedFrc_PTR->lowSeconds, capturedFrc_PTR->nanoseconds, capturedFrc_PTR->fractionalNanoseconds);
        } else {
            printf("phyNumber [%d] todCapture0: is not valid\n",phyNumber);
        }

        capturedFrc_PTR = &params.phyPtpCapturedFrc.todCapture1;
        if (capturedFrc_PTR->valid) {
            printf("phyNumber [%d] todCapture1: highSeconds [%d] lowSeconds [%d] nanoseconds [%d] fractionalNanoseconds [%d]\n"
                   ,phyNumber , capturedFrc_PTR->highSeconds, capturedFrc_PTR->lowSeconds, capturedFrc_PTR->nanoseconds, capturedFrc_PTR->fractionalNanoseconds);
        } else {
            printf("phyNumber [%d] todCapture1: is not valid\n",phyNumber);
        }
...
/*$ END OF wr_utils_mpd_debug_get_ptp_tod_capture_value */
