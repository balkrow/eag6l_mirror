...
)
{
    ...
    lua_newtable(L);
    if((appDemoDbEntryGet_func!= NULL) && (appDemoDbEntryGet_func("firstDevNum", &value) == GT_OK))
    {
        devId = (GT_U8)value-1;
    }
...
LUA_WRAP_STUB(wr_utils_mpd_debug_set_ptp_tod_capture)
LUA_WRAP_STUB(wr_utils_mpd_debug_get_ptp_tod_capture_value)
#endif /*!INCLUDE_MPD*/
