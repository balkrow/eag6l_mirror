...
)
{
    ...
    GT_U32              numPortsForPortIsolation;
    GT_U32              limited_numPortsForPortIsolation;
    ...
    {
        numPortsForPortIsolation = PRV_CPSS_PP_MAC(devNum)->numOfVirtPorts;
    }
    else
    {
        numPortsForPortIsolation = PRV_CPSS_DXCH_MAX_PHY_PORT_NUMBER_MAC(devNum);

        if(!PRV_CPSS_SIP_7_CHECK_MAC(devNum))
        {
            /* limited to 128 ports in falcon and 256 ports in BC3 !*/
            /* logic from internal_cpssDxChNstPortIsolationTableEntrySet(...) */
            limited_numPortsForPortIsolation = PRV_CPSS_SIP_6_CHECK_MAC(devNum) ? 128 :256;

            if(numPortsForPortIsolation > limited_numPortsForPortIsolation)
            {
                numPortsForPortIsolation = limited_numPortsForPortIsolation;
            }
        }
        else
        {
            /* sip7 not limit the number of physical ports in the port isolation ports bmp */
            /* it is according to 'number of physical ports' */
        }
    }

    prvCpssDrvPortsFullMaskGet(numPortsForPortIsolation,&existingPorts);


    /* localPortsMembers &= existingPorts */
