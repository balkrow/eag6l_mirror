...
use_prv_print_struct(CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC);
...
use_prv_struct(CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC);
...
)
{
    ...
    F_BOOL(val, -1, cancelDrop);
    F_BOOL(val, -1, bypassBridge);
    F_BOOL(val, -1, bypassIngressPipe);
    F_ENUM(val, -1, egressPolicy, CPSS_PCL_DIRECTION_ENT);
    F_STRUCT(val, -1, lookupConfig, CPSS_DXCH_PCL_ACTION_LOOKUP_CONFIG_STC);
    F_STRUCT(val, -1, headerAlterationOptions, CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC);
    ...
    if (lua_istable(L, -1))
    {
        if (val->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
        {
    ...
    if (lua_istable(L, -1))
    {
        ...
        else
        {
            ...
            if (lua_istable(L, -1))
            {
                ...
                F_BOOL(&(val->vlan.ingress), -1, copyUdb);
            }
            lua_pop(L, 1);
        }
    }
    ...
    F_BOOL(  val, -1, exactMatchDisable);
    if (val->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
    {
    ...
    if (val->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
    {
        F_ENUM(  val, -1, egressCncIndexMode, CPSS_DXCH_PCL_ACTION_EGRESS_CNC_INDEX_MODE_ENT);
        F_BOOL(  val, -1, enableEgressMaxSduSizeCheck);
        F_NUMBER(val, -1, egressMaxSduSizeProfile, GT_U32);
    }
}
...
)
{
    ...
    FO_BOOL(val, t, cancelDrop);
    FO_BOOL(val, t, bypassBridge);
    FO_BOOL(val, t, bypassIngressPipe);
    FO_ENUM(val, t, egressPolicy, CPSS_PCL_DIRECTION_ENT);
    FO_STRUCT(val, t, lookupConfig, CPSS_DXCH_PCL_ACTION_LOOKUP_CONFIG_STC);
    FO_STRUCT(val, t, headerAlterationOptions, CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC);
    ...
    t1 = lua_gettop(L);
        ...
        else
        {
            ...
            t2 = lua_gettop(L);
                ...
                FO_BOOL(&(val->vlan.ingress), t2, copyUdb);
            lua_setfield(L, t1, "ingress");
        }
    ...
    FO_BOOL(  val, t, exactMatchDisable);
    if (val->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
    {
    ...
    if (val->egressPolicy == CPSS_PCL_DIRECTION_EGRESS_E)
    {
        FO_ENUM(  val, t, egressCncIndexMode, CPSS_DXCH_PCL_ACTION_EGRESS_CNC_INDEX_MODE_ENT);
        FO_BOOL(  val, t, enableEgressMaxSduSizeCheck);
        FO_NUMBER(val, t, egressMaxSduSizeProfile, GT_U32);
    }
...
)
{
    ...
    if (CPSS_PCL_DIRECTION_EGRESS_E == val->egressPolicy)
    {
    ...
    P_BOOL(  val,   fullName, exactMatchDisable);
...
)
{
    ...
    P_ENUM(val,     fullName, pcl1OverrideConfigIndex,
                                        CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT);
    P_ENUM(val,     fullName, mpclOverrideConfigIndex,
                                        CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT);
    P_ENUM(val,     fullName, epclOverrideConfigIndex,
                                        CPSS_DXCH_PCL_LOOKUP_CONFIG_INDEX_ENT);
}

/*************************************************************************
* prv_lua_c_print_CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC
*
* Description:
*       Prints CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC fields
*
* Parameters:
*       name                                      - structure variable
*                                                   name or path to
...
*
*************************************************************************/
void prv_lua_c_print_CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC
(
    const GT_CHAR_PTR                                           name,
    const GT_CHAR_PTR                                           nameExtension,
    const CPSS_DXCH_PCL_ACTION_HEADER_ALTERATION_OPTIONS_STC    *val
)
{
    P_NAME_CONVERSIONS(fullName, name, nameExtension);
    P_BOOL(val, fullName, routerTtlDecrementDisable);
    P_BOOL(val, fullName, routerMacSaAssignmentDisable);
    P_BOOL(val, fullName, routerMacDaAndIpHeaderModificationsDisable);
}
    ...
    if (lua_istable(L, -1))
    {
        switch (val->redirectCmd)
        {
            ...
            case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
                lua_getfield(L, -1, "genericActionInfo");
                if (lua_istable(L, -1))
                {
                    F_NUMBER(&(val->data.genericActionInfo), -1, genericAction, GT_U32);
                }
                lua_pop(L, 1);
                break;
        }
    ...
    t1 = lua_gettop(L);
        switch (val->redirectCmd)
        {
            ...
            case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
                lua_newtable(L);
                t2 = lua_gettop(L);
                    FO_NUMBER(&(val->data.genericActionInfo), t2, genericAction, GT_U32);
                lua_setfield(L, t1, "genericActionInfo");
                break;
        }
...
)
{
    ...
    switch (val->redirectCmd)
    {
        ...
        case CPSS_DXCH_PCL_ACTION_REDIRECT_CMD_ASSIGN_GENERIC_ACTION_E:
            P_NAME_CONVERSIONS_BEGIN(dataName, fullName, "data");
            {
                P_NUMBER(&(val->data.genericActionInfo),  dataName, genericAction, GT_U32);
            }
            P_NAME_CONVERSIONS_END(dataName, fullName, "data");
            break;
...
)
{
    ...
    P_BOOL(val,     fullName, assignSourceVPortProfileEnable);
    P_NUMBER(val,   fullName, sourceVPortProfileValue, GT_U32);
}
...
/*************************************************************************
* prv_lua_c_print_CPSS_DXCH_IP_ECMP_ENTRY_STC
*
* Description:
*       Prints CPSS_DXCH_IP_ECMP_ENTRY_STC fields
*
* Parameters:
*
* Returns:
...
*
*************************************************************************/
void prv_lua_c_print_CPSS_DXCH_IP_ECMP_ENTRY_STC
(
    const GT_CHAR_PTR                       name,
    const GT_CHAR_PTR                       nameExtension,
    const CPSS_DXCH_IP_ECMP_ENTRY_STC       *val
)
{
    P_NAME_CONVERSIONS(fullName, name, nameExtension);
    P_BOOL(val,     fullName, randomEnable);
    P_NUMBER(val,   fullName, numOfPaths, GT_U32);
    P_NUMBER(val,   fullName, routeEntryBaseIndex, GT_U32);
    P_ENUM(val,     fullName, multiPathMode, CPSS_DXCH_IP_ROUTE_ENTRY_METHOD_ENT);
}
...
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, enhancedSflow, CPSS_DXCH_PHA_THREAD_INFO_TYPE_ENHANCED_SFLOW_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, ifaEgressNodeMirror, CPSS_DXCH_PHA_THREAD_INFO_TYPE_IFA_EGRESS_NODE_MIRROR_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, ifaHeader, CPSS_DXCH_PHA_THREAD_INFO_TYPE_IFA_HEADER_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, ifaMetadata, CPSS_DXCH_PHA_THREAD_INFO_TYPE_IFA_METADATA_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, clearOutgoingMtagCmd, CPSS_DXCH_PHA_THREAD_INFO_TYPE_CLEAR_OUTGOING_MTAG_COMMAND_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, copyFlowIdToTrgPhyPort, CPSS_DXCH_PHA_THREAD_INFO_TYPE_COPY_FLOW_ID_TO_TRG_PHY_PORT_STC)
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_THREAD_INFO_UNT, mplsLsrInStackingSystem, CPSS_DXCH_PHA_THREAD_INFO_TYPE_MPLS_LSR_IN_STACKING_SYSTEM_STC)
...
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_SOURCE_PORT_ENTRY_TYPE_UNT, ifaSourcePort, CPSS_DXCH_PHA_PORT_ENTRY_TYPE_IFA_PORT_STC)
...
UNION_MEMBER_STRUCT(CPSS_DXCH_PHA_TARGET_PORT_ENTRY_TYPE_UNT, ifaTargetPort, CPSS_DXCH_PHA_PORT_ENTRY_TYPE_IFA_PORT_STC)
...
)
{
    ...
    F_ARRAY_START(val, -1, ser);
    {
        int idx;
        for (idx = 0; idx < 8; idx++) {
            F_ARRAY_FLOAT(val, ser, idx, GT_FLOAT64);
        }
    }
    F_ARRAY_END(val, -1, ser);
    F_FLOAT(val, -1, totalSer, GT_FLOAT64);
    F_FLOAT(val, -1, serTimeInterval, GT_FLOAT64);
}
...
)
{
    ...
    FO_ARRAY_START(val, t, ser);
    {
        int idx;
        for (idx = 0; idx < 8; idx++) {
            FO_ARRAY_FLOAT(val, ser, idx, GT_FLOAT64);
        }
    }
    FO_ARRAY_END(val, t, ser);
    FO_FLOAT(val, t, totalSer, GT_FLOAT64);
    FO_FLOAT(val, t, serTimeInterval, GT_FLOAT64);
    lua_settop(L, t);
}
