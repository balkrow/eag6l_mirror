...
*       Function Relevant mode : All modes
*
*       Getting of CPSS_MAC_ENTRY_EXT_STC comparison mask from lua stack.
*
* APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; AC5; Lion; xCat2; Lion2.
*
...
(
    IN  lua_State                                       *L,
    IN  GT_32                                           L_index,
    OUT WRAPL_CPSS_MAC_ADDRESS_TABLE_COUNT_STATISTIC    *statisticPtr,
    OUT GT_CHAR_PTR                                     *error_message
)
{
    GT_STATUS                                           status = GT_OK;
...
*       Function Relevant mode : All modes
*
*       CPSS wrapper of pushing WRAPL_CPSS_MAC_ADDRESS_TABLE_COUNT_STATISTIC to
*       lua stack.
*
* APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; AC5; Lion; xCat2; Lion2.
...
(
    IN GT_STATUS                                        status,
    IN lua_State                                        *L,
    IN WRAPL_CPSS_MAC_ADDRESS_TABLE_COUNT_STATISTIC     *statistic
)
{
    if (GT_OK == status)
    {
...
*
* RETURNS:
*       GT_OK and true are pushed to lua stack if no errors occurs and checking
*       was successful
*       GT_OK, true and warning message are pushed to lua stack if no errors
*       occurs and checking was not successful
*       error status and error message if error occurs
*
* COMMENTS:
*
*******************************************************************************/
...
)
{
    ...
    *error_message = NULL;
    ...
    /* Bug: Unclear behavior of cpssDxChBrgMcIpv6BytesSelectGet...
    if ((GT_OK == status) &&
...
)
{
    ...
    *error_message = NULL;
    ...
    /* Bug: Unclear behavior of cpssDxChBrgMcIpv6BytesSelectSet...
    ...
    *entryWasFound                              = GT_FALSE;
           ...
           (GT_TRUE == nonLastIndex))
    {
        ...
        if (GT_OK == status)
        {
            ...
            {
                *entryWasFound =
                    prv_lua_compare_mask(searchedMacEntryPtr,
                                         patternMacEntryPtr,
                                         maskMacEntryPtr,
                                         sizeof(*searchedMacEntryPtr))?
                    GT_TRUE:GT_FALSE;
            }
        }
...
)
{
    ...
    GT_UNUSED_PARAM(searchOnlyValid);
    ...
    *errorMessagePtr = NULL;
    ...
    status = cpssDxChBrgFdbHashModeGet(
        devNum, &hashMode);
    if (status != GT_OK)
    {
        status        = GT_BAD_PARAM;
    ...
    {
        status = cpssDxChBrgFdbHashCrcMultiResultsCalc(
            devNum, &(macEntry.key),
...
/* for shared VLAN learning mode no need to compare VIDs of entries*/
    ...
    for(j=0;j<hashLenth;j++)
    {
           status =  prvDxChBrgFdbMacEntryRead(devNum, indexArr[j],
                                               &searchedValid,
                                               &searchedSkip,
                                               &searchedAged,
                                               &searchedDevNum,
...
(
    ...
    IN    GT_BOOL                                       aged,
    IN    CPSS_MAC_ENTRY_EXT_STC                        *macEntryPtr,
    INOUT WRAPL_CPSS_MAC_ADDRESS_TABLE_COUNT_STATISTIC  *statisticPtr
)
{
    GT_STATUS                   status          = GT_OK;

    GT_UNUSED_PARAM(aged);
    ...
    if ((GT_FALSE == skip) && (GT_TRUE == macEntryPtr->isStatic) && (GT_TRUE == valid))
    {
        statisticPtr->static_entries_count++;
    }

    if ((GT_FALSE == skip) && (GT_FALSE == macEntryPtr->isStatic) && (GT_TRUE == valid))
    {
...
*
* OUTPUTS:
*       number of returned to lua elements, status and FDB entry index to
...
*
*       Getting of next fdb entry and its index with getting data from lua or
*       its creating, if not exists. Result is pushed to stack.
*
* APPLICABLE DEVICES:
*        DxCh1; DxCh1_Diamond; DxCh2; DxCh3; xCat; xCat3; AC5; Lion; xCat2; Lion2.
*
...
)
{
    ...
    do
    {
        ...
        rc = cpssDxChBrgFdbTrunkAgingModeSet(
            devNum, CPSS_DXCH_BRG_FDB_AGE_TRUNK_MODE_REGULAR_E);
        if (GT_OK != rc)
        {
                break;
        }
...
)
{
    ...
    CPSS_MAC_ENTRY_EXT_KEY_STC   calc_macEntryKey;
    ...
    rc = cpssDxChBrgFdb16BitFidHashEnableGet(devNum,&Fdb16BitFidHashEnable);
    if (rc == GT_NOT_APPLICABLE_DEVICE)/* support non sip5 */
    {
        Fdb16BitFidHashEnable = GT_FALSE;
    }
    else if(rc != GT_OK)
    {
        return rc;
    ...
    calc_macEntryKey = *macEntryKeyPtr;
    if(Fdb16BitFidHashEnable == GT_FALSE &&
       macEntryKeyPtr->entryType == CPSS_MAC_ENTRY_EXT_TYPE_MAC_ADDR_E &&
       macEntryKeyPtr->key.macVlan.vlanId >= 0x1000)
    {
        /* Not using more than 12 bits of VID ... so need to modify before calling cpssDxChBrgFdbHashCalc(...) */
        calc_macEntryKey.key.macVlan.vlanId &= 0xFFF;
    }
...
/*******************************************************************************
* wrlCpssDxChBrgFdbHashCrcMultiResultsCalc
*
* DESCRIPTION:
*       Function Relevant mode : All modes
*
*       Calculates the CRC multiple hash results.
*
*
* APPLICABLE DEVICES:
...
*
*******************************************************************************/
int wrlCpssDxChBrgFdbHashCrcMultiResultsCalc
(
    IN lua_State *L
)
{
    GT_U32                      paramIndex              = 1;
    GT_U8                       devNum                  = (GT_U8) lua_tonumber(L, paramIndex++); /* devId */
    GT_U32                      multiHashStartBankIndex = (GT_U32) lua_tonumber(L, paramIndex++); /* devId */
    GT_U32                      numOfBanks              = (GT_U32) lua_tonumber(L, paramIndex++);
    ...
    if (0 != lua_istable(L, paramIndex))
    {
        lua_pushvalue(L, paramIndex);
        prv_lua_to_c_CPSS_MAC_ENTRY_EXT_KEY_STC(L, &entryKey);
        lua_pop(L, 1);
    }

    status = cpssDxChBrgFdbHashCrcMultiResultsCalc(devNum,
                                                   &entryKey,
    ...
    P_CALLING_API(
        ...
        PC_NUMBER(devNum,                       devNum,                     GT_U8,
        PC_STRUCT(&entryKey,                    entryKey,                   CPSS_MAC_ENTRY_EXT_KEY_STC,
        PC_NUMBER(multiHashStartBankIndex,      multiHashStartBankIndex,    GT_U32,
        PC_NUMBER(numOfBanks,                   numOfBanks,                 GT_U32,
        PC_LAST_PARAM)))),
        PC_STATUS);

    returned_param_count += prvWrlCpssStatusToLuaPush(status, L, error_message);
    ...
    if (GT_OK == status)
    {
        ...
        t = lua_gettop(L);
        lua_newtable(L);
        for (index = 0; index < numOfBanks; index++)
        {
            lua_pushnumber(L, crcMultiHashArr[index]);
            lua_rawseti(L, -2, index);
        }
        lua_setfield(L, t, "crcMultiHashArr");
        lua_settop(L, t);
        returned_param_count += 1;
    }
