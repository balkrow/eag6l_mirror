...
)
{
    if ((GT_OK == status) && (GT_TRUE == condition))
    {
        ...
        for (port = 0; port < PRV_CPSS_PP_MAC(devNum)->numOfPorts; port++)
        {
            if (CPSS_PORTS_BMP_IS_PORT_SET_MAC(portsMembersPtr, port))
            {
                ...
                lua_rawseti(L, t1, (int)lua_objlen(L, t1) + 1);
            }
        }
        ...
        if ((GT_OK == status) && (GT_TRUE == isTaggingCommandSupported))
        {
            ...
            {
                prv_c_to_lua_enum(
                    L, "CPSS_DXCH_BRG_VLAN_PORT_TAG_CMD_ENT",
    ...
    #define F_ENUM(field, fieldtype) \
        prv_c_to_lua_enum(L, # fieldtype, vlanInfoPtr->field); \
        lua_setfield(L, t, #field);
...
)
{
    ...
    if (lua_type(L, -1) == LUA_TTABLE)
    {
        for (i = 1; i <= lua_objlen(L, -1); i++)
        {
            ...
            {
                GT_U32 portNum = lua_tointeger(L, -1);
...
#define F_ENUM(field, fieldtype) \
    lua_getfield(L, 3, #field); \
    vlanInfoPtr.field = (fieldtype)prv_lua_to_c_enum(L, # fieldtype); \
    lua_pop(L, 1);
...
)
{
    ...
    if (status == GT_OK)
    {
        while (vlanId <= maxVlanId)
        {
            st = cpssDxChBrgVlanEntryRead(devNum, vlanId++, &portsMembers,
                                          &portsTagging, &vlanInfo, &isValid,
                                          &portsTaggingCmd);
            P_CALLING_API(
                cpssDxChBrgVlanEntryRead,
...
)
{
    ...
    while ((GT_OK == status) && (vlanId <= maxVlanId))
    {
        ...
        /* since vlanId is GT_U16 and maxVlanId can be 64K
