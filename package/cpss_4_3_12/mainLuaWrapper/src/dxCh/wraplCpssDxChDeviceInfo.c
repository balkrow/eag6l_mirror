    ...
    switch (interfacePtr->type)
    {
        ...
        case CPSS_INTERFACE_VIDX_E:
            status  = prvCpssDxChDoesMulticastGroupIndexExist(
                          devNum, (GT_U16)interfacePtr->vidx, isValidInterfacePtr,
                          errorMessagePtrPtr);
            break;
...
)
{
    ...
    {
        rc = cpssDxChNetIfGdmaCpuPortsInfoGet(devNum,&allCpuPortsInfo);
        /* 1'st param */
        lua_pushinteger(L, rc);
        /* 2'nd param */
        lua_pushinteger(L, allCpuPortsInfo.toCpuInfo.numOfValidPorts);
        /* 3'rd param */
        lua_pushinteger(L, allCpuPortsInfo.fromCpuInfo.numOfValidPorts);
        return 3;
