...
enum mvebu_efuse_errors {
	...
	MVEBU_EFUSE_IM_SET_LDO_VPP_VAL};
...
struct SecurityIDFuse_info {
	...
	mv_u32 writeAddr;
	mv_u32 writeBitMask;
	/*Each fuse belongs to a range that can be locked from further write access*/
	mv_u8 idFuseRange;
};
...
int mvebu_efuse_write(char *fusename, char *value_arr);
