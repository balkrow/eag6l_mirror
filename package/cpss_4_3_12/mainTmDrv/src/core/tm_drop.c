...
static const float scaling_probability[TM_MAX_PROB_12H+1] = { 100.0, 50.0, 25.0, 12.5 };
...
static int prv_tm_create_wred_curve_proc_at_index(tm_handle hndl,
                         enum tm_level level,
                         uint8_t cos,
                         float * probability_points_percents,
                         float max_probability,
                         uint8_t *curve_index)
{
    ...
    int rc;
    int curve_ind = (int)TM_INVAL;
    struct rmctl *rmctl = NULL;

    TM_CTL(ctl, hndl);

    rmctl = (struct rmctl *)ctl->rm;
    CHECK_RM_HANDLE(rmctl);
    ...
    {
    case Q_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_Q_CURVE]), *curve_index);
        break;
    case A_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_A_CURVE]), *curve_index);
        break;
    case B_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_B_CURVE]), *curve_index);
        break;
    case C_LEVEL:
        if (cos >= TM_WRED_COS) {
            rc = -EDOM;
        ...
        rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_C_CURVE_COS_0+cos]), *curve_index);
        break;
    case P_LEVEL:
        if(cos == (uint8_t)TM_INVAL)
        {
            rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_P_CURVE]), *curve_index);
        }
        else
        {
            if (cos >= TM_WRED_COS) {
            ...
            rc = reserve_resource(&(rmctl->rm_profiles[RM_WRED_P_CURVE_COS_0+cos]), *curve_index);
        }
        break;
    default:
        break;
...
int tm_create_wred_curve_at_index(tm_handle hndl,
                         enum tm_level level,
                         uint8_t cos,
                         uint8_t *prob,
                         uint8_t *curve_index_ptr)
{
    ...
    return prv_tm_create_wred_curve_proc_at_index(hndl,level, cos, probability_points_ps, max_curve_probability, curve_index_ptr);
}

int tm_create_wred_traditional_curve_at_index(tm_handle hndl,
                                     enum tm_level level,
                                     uint8_t cos,
                                     float max_curve_probability,
                                     uint8_t *curve_index_ptr)
...
int tm_create_drop_profile_at_index(tm_handle hndl,
                           enum tm_level level,
                           uint8_t cos,
                           struct tm_drop_profile_params *profile_params,
                           uint16_t *prof_index)
{
    ...
    {
    case Q_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_Q_DROP_PRF]), (uint32_t)*prof_index);
        break;
    case A_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_A_DROP_PRF]), (uint32_t)*prof_index);
        break;
    case B_LEVEL:
        rc = reserve_resource(&(rmctl->rm_profiles[RM_B_DROP_PRF]), (uint32_t)*prof_index);
        break;
    case C_LEVEL:
        if (cos >=RM_COS) return -EFAULT;
        rc = reserve_resource(&(rmctl->rm_profiles[RM_C_DROP_PRF_COS_0+cos]), (uint32_t)*prof_index);
        break;
    case P_LEVEL:
        if(cos == (uint8_t)TM_INVAL) /* Global Port */
        {
            rc = reserve_resource(&(rmctl->rm_profiles[RM_P_DROP_PRF]), (uint32_t)*prof_index);
        }
        else
        {
            if (cos >=RM_COS) return -EFAULT;
            rc = reserve_resource(&(rmctl->rm_profiles[RM_P_DROP_PRF_COS_0+cos]), (uint32_t)*prof_index);
        }
        break;
    default:
        break;
                                          ...
                                          uint8_t * resolution)
{
    ...
    rc = get_hw_drop_resolution(hndl, &resolution_mask);
    if (rc)
        rc = TM_HW_COLOR_NUM_CONFIG_FAIL;

    if (resolution_mask & (1 << lvl))
    {
        *resolution = 4;
        if (ctl->dp_unit.local[lvl].resolution == *resolution)
            *num = TM_3_COLORS;
        else
        /* default for 6 bit resolution */
            *num = TM_2_COLORS;
    }
    else
    {
        *resolution = 6;
        if (ctl->dp_unit.local[lvl].resolution == *resolution)
        {
            if (ctl->dp_unit.local[lvl].color_num == 2) *num = TM_2_COLORS;
            else                                        *num = TM_1_COLORS;
        }
        else /* default for 4 bit resolution */
            *num = TM_3_COLORS;
    }
    return rc;
}
