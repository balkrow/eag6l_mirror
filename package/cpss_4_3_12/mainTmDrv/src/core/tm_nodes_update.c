...
/*        assert(ctl->tm_b_lvl_drop_profiles[node->wred_profile_ref].use_counter >= 0); */
    ...
    if (fl_changed)
    {
        if (set_hw_node_elig_prio_function(hndl, B_LEVEL, index) ||
            set_hw_b_node_params(hndl, index))
        {
...
#if 0 /* Following code will never run since (params->wred_profile_ref[i] != params->wred_profile_ref[i]) is always false */
