...
*/
    rc = get_hw_queue_params(hndl, queue_index, &queue_hw_data);
    if (rc) goto out;
    params->shaping_profile_ref = node->shaping_profile_ref;
    params->quantum = queue_hw_data.dwrr_quantum;
    params->wred_profile_ref = queue_hw_data.wred_profile_ref;
                                 ...
                                 struct tm_a_node_params *params)
{
    ...
    rc =    get_hw_a_node_params(hndl, node_index, &a_node_hw_data);
    if (rc) goto out;
    ...
    params->wred_profile_ref = a_node_hw_data.wred_profile_ref;
                                 ...
                                 struct tm_b_node_params *params)
{
    ...
    rc =    get_hw_b_node_params(hndl, node_index, &b_node_hw_data);
    if (rc) goto out;
    ...
    params->wred_profile_ref = b_node_hw_data.wred_profile_ref;
                                 ...
                                 struct tm_c_node_params *params)
{
    ...
    rc = get_hw_c_node_params(hndl, node_index,&c_node_hw_data);
    if (rc) goto out;
...
/*   params->wred_cos = c_node_hw_data.wred_cos; */
    ...
    {
        params->wred_profile_ref[i] = c_node_hw_data.wred_profile_ref[i];
        if (c_node_hw_data.wred_profile_ref[i]) params->wred_cos += (1<< i);
    }
...

{
    ...
    rc = get_hw_port_params(hndl, port_index, &port_hw_data);
    if (rc) goto out;
...
int tm_traverse_port_tree_hw(tm_handle hndl, unsigned int port_index, TM_PORT_TRAVERSE_TREE_FUN dump_fun, void * vpEnv,  void* vpUserData)
{
    ...
    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.PortRangeMap , port_index, TM_Sched_PortRangeMap)
    if (rc) return -1;
    TM_REGISTER_GET(TM_Sched_PortRangeMap , Lo , portChildLo , (int) )
    TM_REGISTER_GET(TM_Sched_PortRangeMap , Hi , portChildHi, (int) )

    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.PortEligPrioFuncPtr , port_index , TM_Sched_PortEligPrioFuncPtr)
    if (rc) return -1;
    ...
    if (port_ptr->mapping.nodeStatus == TM_NODE_USED)
    {
        ...
        {
            TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ClvltoPortAndBlvlRangeMap , ic , TM_Sched_ClvltoPortAndBlvlRangeMap)
            if (rc) return -1;
            ...
            TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ClvlEligPrioFuncPtr , ic , TM_Sched_ClvlEligPrioFuncPtr)
            if (rc) return -1;
            ...
            if (cnodeEf != TM_NODE_DISABLED_FUN)
            {
                ...
                {
                    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.BLvltoClvlAndAlvlRangeMap , ib , TM_Sched_BLvltoClvlAndAlvlRangeMap)
                    if (rc) return -1;
                    ...
                    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.BlvlEligPrioFuncPtr , ib , TM_Sched_BlvlEligPrioFuncPtr)
                    if (rc) return -1;
                    ...
                    if (bnodeEf != TM_NODE_DISABLED_FUN)
                    {
                        ...
                        {
                            TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.ALvltoBlvlAndQueueRangeMap , ia , TM_Sched_ALvltoBlvlAndQueueRangeMap)
                            if (rc) return -1;
                            ...
                            TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.AlvlEligPrioFuncPtr , ia , TM_Sched_AlvlEligPrioFuncPtr)
                            if (rc) return -1;
                            ...
                            if (anodeEf != TM_NODE_DISABLED_FUN)
                            {
                                ...
                                {
                                    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.QueueAMap , iq , TM_Sched_QueueAMap)
                                    if (rc) return -1;
                                    TM_REGISTER_GET(TM_Sched_QueueAMap , Alvl , queueParent,(int))

                                    TM_READ_TABLE_REGISTER(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(TM).Sched.QueueEligPrioFuncPtr , iq , TM_Sched_QueueEligPrioFuncPtr)
                                    if (rc) return -1;
