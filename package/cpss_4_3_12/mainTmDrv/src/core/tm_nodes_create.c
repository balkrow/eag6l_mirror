...
#include <systemRecovery/tm_nodes_index_fetch.h>
...
int tm_create_asym_port(tm_handle hndl, uint8_t port_index,
                        struct tm_port_params *params)
{
    return  tm_create_asym_port_ex(hndl, port_index,  params, -1 /* no preallocated range */);
}
                                            ...
                                            uint32_t first_c_node_in_range, uint32_t c_nodes_range)
{
    ...
    return tm_create_asym_port_ex(hndl, port_index, params, first_c_node_in_range);
}
                   ...
                   uint32_t num_of_queues)
{
    ...
    rc = tm_create_asym_port(ctl, port_index, params);
    if (rc)
        goto out;
                              ...
                              uint32_t * queue_index_ptr)
{
    ...
    int rc = 0;
    int i;
    tm_node_status_t old_node_status = TM_NODE_FREE;
...
int tm_create_a_node_to_b_node_at_index_ex(tm_handle hndl, uint32_t parent_b_node_index,
                               struct tm_a_node_params *a_params, uint32_t first_queue_in_range,
                               uint32_t *a_node_index_ptr)
{
    struct tm_port  *parent_port = NULL;
    ...
    /**************************************************************************/
    /* looking for reserved or previously deleted nodes in the range */
    if ((*a_node_index_ptr >= (uint32_t)parent_b_node->mapping.childLo) &&
        (*a_node_index_ptr <= (uint32_t)parent_b_node->mapping.childHi) &&
        (ctl->tm_a_node_array[*a_node_index_ptr].mapping.nodeStatus > TM_NODE_USED))
    {
        i = *a_node_index_ptr;
        found = 1;
    }
                               ...
                               uint32_t *a_node_index_ptr)
{
    ...
    if (SYSTEM_STATE_HA_CATCH_UP_STATE_MAC)
    {
        ret = tm_create_a_node_to_b_node_index_fetch(hndl, parent_b_node_index, a_params, a_node_index_ptr, &first_queue_index);
        if (ret != 0)
        {
            return ret;
        }

        ret = tm_create_a_node_to_b_node_at_index_ex(hndl, parent_b_node_index, a_params, first_queue_index, a_node_index_ptr);
    }
    else
    {
        ret = tm_create_a_node_to_b_node_ex(hndl,parent_b_node_index, a_params, NO_PREDEFINED_RANGE ,a_node_index_ptr);
    }
...
/**
 */
int tm_create_b_node_to_c_node_at_index_ex(tm_handle hndl, uint32_t parent_c_node_index,
                               struct tm_b_node_params *b_params, uint32_t first_child_in_range,
                               uint32_t *b_node_index_ptr)
{
    struct tm_port *parent_port = NULL;
    ...
    /*******************************************************************************/
    /* looking for reserved or previously deleted nodes in the range */
    if ((*b_node_index_ptr >= (uint32_t)parent_c_node->mapping.childLo) &&
        (*b_node_index_ptr <= (uint32_t)parent_c_node->mapping.childHi) &&
        (ctl->tm_b_node_array[*b_node_index_ptr].mapping.nodeStatus > TM_NODE_USED))
    {
        i = *b_node_index_ptr;
        found = 1;
    }
                               ...
                               uint32_t *b_node_index_ptr)
{
    ...
    if (SYSTEM_STATE_HA_CATCH_UP_STATE_MAC)
    {
        ret = tm_create_b_node_to_c_node_index_fetch(hndl, parent_c_node_index, b_params, b_node_index_ptr, &first_a_node_index);
        if (ret != 0)
        {
            return ret;
        }

        ret = tm_create_b_node_to_c_node_at_index_ex(hndl, parent_c_node_index, b_params, first_a_node_index, b_node_index_ptr);
    }
    else
    {
        ret = tm_create_b_node_to_c_node_ex(hndl, parent_c_node_index, b_params, NO_PREDEFINED_RANGE, b_node_index_ptr);
    }
...
/**
 */
int tm_create_c_node_to_port_at_index_ex(tm_handle hndl, uint8_t parent_port_index,
                             struct tm_c_node_params *c_params, uint32_t first_child_in_range,
                             uint32_t *c_node_index_ptr)
{
    struct tm_port  *parent_port = NULL;
    ...
    /*******************************************************************************/
    /* looking for reserved or previously deleted nodes in the range */
    if ((*c_node_index_ptr >= (uint32_t)parent_port->mapping.childLo) &&
        (*c_node_index_ptr <= (uint32_t)parent_port->mapping.childHi) &&
        (ctl->tm_c_node_array[*c_node_index_ptr].mapping.nodeStatus > TM_NODE_USED))
    {
        i = *c_node_index_ptr;
    }
    else
                             ...
                             uint32_t *c_node_index_ptr)
{
    ...
    if (SYSTEM_STATE_HA_CATCH_UP_STATE_MAC)
    {
        ret = tm_create_c_node_to_port_index_fetch(hndl, parent_port_index, c_params, c_node_index_ptr, &first_b_node_index);
        if (ret != 0)
        {
            return ret;
        }

        ret = tm_create_c_node_to_port_at_index_ex(hndl, parent_port_index, c_params, first_b_node_index, c_node_index_ptr);
    }
    else
    {
        ret = tm_create_c_node_to_port_ex(hndl, parent_port_index, c_params, NO_PREDEFINED_RANGE,c_node_index_ptr);
    }
