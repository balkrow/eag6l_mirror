...
int delete_c_node_proc(tm_handle hndl, uint32_t index)
{
    ...
    set_sw_c_node_default(c_node,1);

    /* Download  parameters to H/W */
    if (set_hw_node_mapping(hndl, C_LEVEL, index))              return TM_HW_C_NODE_CONFIG_FAIL;
...
int delete_b_node_proc(tm_handle hndl, uint32_t index)
{
    ...
    set_sw_b_node_default(b_node,1);

    /* Download  parameters to H/W */
    if (set_hw_node_mapping(hndl, B_LEVEL, index))              return TM_HW_B_NODE_CONFIG_FAIL;
...
int delete_a_node_proc(tm_handle hndl, uint32_t index)
{
    ...
    set_sw_a_node_default(a_node,1);

    /* Download  parameters to H/W */
    if (set_hw_node_mapping(hndl, A_LEVEL, index))              return TM_HW_A_NODE_CONFIG_FAIL;
...
static int tm_delete_queue(tm_handle hndl, uint32_t index)
{
    ...
    set_sw_queue_default(queue,1);

    /* Download  parameters to H/W */
    /* Uninstall Queue */
    if (set_hw_uninstall_queue(hndl, index))                    return TM_HW_QUEUE_CONFIG_FAIL;

    if (set_hw_node_mapping(hndl, Q_LEVEL, index))              return TM_HW_QUEUE_CONFIG_FAIL;
