...
#define TM_WRITE_REGISTER(address, register_name)   \
    if(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_log_write_requests))\
    {\
        PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy) = (void*)TM_REGISTER_VAR_ADDR(register_name);\
        tm_printf(" function %s -  writing to register %s(0x%08X %08X) value 0x%016llX\n",__FUNCTION__,#address,address.l[1],address.l[0],*((uint64_t*)PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy)));\
    }\
    ...
    tm_convert_to_u64(TM_REGISTER_VAR_ADDR(register_name),&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_write_dummy));\
    rc = tm_register_write(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address),  &PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_write_dummy));\
...
#define TM_READ_REGISTER(address, register_name)    \
    if(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_log_read_requests))    tm_printf(" function %s -  reading from register %s(0x%08X %08X) ... ",__FUNCTION__, #address, address.l[1], address.l[0]);\
    rc = tm_register_read(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address),&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_read_dummy)  );\
    tm_convert_from_u64(&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_read_dummy),TM_REGISTER_VAR_ADDR(register_name));\
    if(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_log_read_requests))\
    {\
        if (rc==0)\
        {\
            PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy) = (void*)TM_REGISTER_VAR_ADDR(register_name);\
            tm_printf(" result 0x%016llX\n",*((uint64_t*)PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy)));\
        }\
...
#define TM_WRITE_TABLE_REGISTER(address, index , register_name) \
    ...
    {\
        PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy) = (void*)TM_REGISTER_VAR_ADDR(register_name);\
        tm_printf(" function %s -  writing to table register %s(base address 0x%08X %08X) , shift %ld)  value 0x%016llX\n",__FUNCTION__,#address,address.l[1],address.l[0],index, *((uint64_t*)PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_pAliasingDummy)));\
    }\
    ...
    tm_convert_to_u64(TM_REGISTER_VAR_ADDR(register_name),&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_write_dummy));\
    rc = tm_table_entry_write(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address), index, &PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_write_dummy));\
...
#define TM_READ_TABLE_REGISTER(address, index , register_name)  \
    if(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_log_read_requests))    tm_printf(" function %s -  reading from  table register %s(base address 0x%08X %08X , shift %d) ...",__FUNCTION__,#address,address.l[1],address.l[0],index);\
    rc = tm_table_entry_read(TM_ENV(ctl), REGISTER_ADDRESS_ARG(address), index,&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_read_dummy));\
    tm_convert_from_u64(&PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(convert_tm_read_dummy),TM_REGISTER_VAR_ADDR(register_name));\
    if(PRV_SHARED_TM_PLATFORM_DIR_GLOBAL_VAR_GET(tm_log_read_requests))\
    {\
        if (rc==0)\
