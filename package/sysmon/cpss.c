...
#if defined(_WIN32)
extern GT_U32 win32GetPid
(
    GT_VOID
)
{
    /* placeholder used in CPSS logs */
    return 0;
}
#endif
...
static CPSS_OS_FILE_TYPE_STC prvOsLogLocalFile =
    {(CPSS_OS_FILE)CPSS_OS_FILE_INVALID, (CPSS_OS_FILE)NULL, NULL};
...
GT_VOID appDemoDxChCnmResetPreparation_callbackFunc(GT_VOID)
{
    ...
    START_LOOP_ALL_DEVICES(devIdx, devNum)
    {
        /* Prevent redundant CNM reset as call-back may be called multiple times */
...
)
{
    ...
    */
...
#endif
        if (prvAppDemoFirstInitTimes.initSystemTimeTotal == 0)
        {
    ...
    if((appDemoOnDistributedSimAsicSide == 0) && (appDemoPrePhase1Init == GT_FALSE))
    {
        ...
        {
            CPSS_ENABLER_OS_RMON_CREATE_ENTRY_CHECKPOINT("boardCfgFuncs.boardAfterPhase2Config");
            rc = boardCfgFuncs.boardAfterPhase2Config((GT_U8)boardRevId);
            CPSS_ENABLER_OS_RMON_CREATE_EXIT_CHECKPOINT("boardCfgFuncs.boardAfterPhase2Config");
...
)
{
    ...
    else if (prvOsLogMode == APP_DEMO_CPSS_LOG_MODE_LOCAL_FILE_E
             && prvOsLogLocalFile.fd != (CPSS_OS_FILE)CPSS_OS_FILE_INVALID) /* write to a local file */
    {
        if (osFprintf(prvOsLogLocalFile.fd, "%s", buffer) < 0)
        {
...
)
{
    ...
    if (prvOsLogMode == APP_DEMO_CPSS_LOG_MODE_MEMFS_FILE_E && prvOsLogMemFsFile >= 0)
    {
        ...
        prvOsLogMemFsFile = -1;
             ...
             && prvOsLogLocalFile.fd != (CPSS_OS_FILE)CPSS_OS_FILE_INVALID)
    {
        osFclose(&prvOsLogLocalFile);
        prvOsLogLocalFile.fd = (CPSS_OS_FILE)CPSS_OS_FILE_INVALID;
    }
    ...
    if (mode == APP_DEMO_CPSS_LOG_MODE_MEMFS_FILE_E || mode == APP_DEMO_CPSS_LOG_MODE_LOCAL_FILE_E)
    {
        ...
        {
            prvOsLogLocalFile.fd = osFopen(name, "w", &prvOsLogLocalFile);
            if (prvOsLogLocalFile.fd == (CPSS_OS_FILE)CPSS_OS_FILE_INVALID)
            {
...
typedef struct{
    ...
    /* Indicates task type,
     *  GT_TRUE  - Task is specific to a device
     *  GT_FALSE - Task is Global
     */
    GT_BOOL     isTaskPerDevice;

    /* Indicates the device number the task belongs to
     *   NOTE - Valid only in case of isTaskPerDevice = GT_TRUE
     */
    GT_SW_DEV_NUM devNum;
...
recheck_tid_lbl:
    ...
    for(ii = 0 ; ii < APP_DEMO_OS_TASK_MAX_NUM ; ii++)
    {
        ...
        appDemoOsTaskInfoArr[ii].gracefulExitInfo.cbFunc    = cbFunc;
        appDemoOsTaskInfoArr[ii].gracefulExitInfo.cookiePtr = cookiePtr;
        appDemoOsTaskInfoArr[ii].isTaskPerDevice            = isTaskPerDevice;
...
)
{
    ...
    for(ii = 0 ; ii < APP_DEMO_OS_TASK_MAX_NUM ; ii++)
    {
        ...
        /* Notify Tasks specific to device */
...
)
{
    ...
    for(ii = 0 ; ii < APP_DEMO_OS_TASK_MAX_NUM ; ii++)
    {
        ...
        /* Terminate Only device specific Tasks */
        if((appDemoOsTaskInfoArr[ii].isTaskPerDevice == GT_TRUE) &&
           (appDemoOsTaskInfoArr[ii].devNum == devNum))
        {
...
)
{
    ...
    if(found == GT_TRUE)
    {
        ...
        if(rc == GT_OK &&
           appDemoOsTaskInfoArr[ii].gracefulExitInfo.cbFunc &&
           appDemoOsTaskInfoArr[ii].taskSupportsGracefulExit)
        {
            /* let the CB of the task do it's specific cleaning (AFTER the task was killed
               so it can release semaphore/memory/others that the task was using ) */
            appDemoOsTaskInfoArr[ii].gracefulExitInfo.cbFunc(
                appDemoOsTaskInfoArr[ii].gracefulExitInfo.cookiePtr);
        }
        ...
        appDemoOsTaskInfoArr[ii].taskGracefullyReadyFor_osTaskDelete = 0;
        appDemoOsTaskInfoArr[ii].taskSupportsGracefulExit = 0;
        appDemoOsTaskInfoArr[ii].gracefulExitInfo.cbFunc = NULL;
        appDemoOsTaskInfoArr[ii].gracefulExitInfo.cookiePtr = NULL;
        return rc;
    }
...
)
{
    ...
    osFuncBindPtr->osMemBindInfo.osMemCacheDmaMallocByDeviceFunc =    osCacheDmaMallocByDevice;
#endif
...
#endif
#if defined(_WIN32)
    osFuncBindPtr->osTaskBindInfo.osTaskGetPid     = win32GetPid;
#endif
...
)
{
    ...
    if (eag6lCpssInitialized == GT_TRUE)
    {
        return GT_OK;
    }
...
)
{
    ...
    GT_U8                           devIdx,_dummy;          /* Loop index.                                  */
...
GT_STATUS appDemoDbPpDump(void)
{
    ...
    char * busTypeArr[CPSS_HW_INFO_BUS_TYPE_MAX_NUM+1]={"NONE","PCI","PCI_e","SMI", "MBUS", "NA"};
    char * busTypePtr;
...
* @brief Set register address bounds to be traced.
*
* @param[in] maxNumOfRanges - maximal number of address ranges.
...
*
* @param[in] rangeAddr     -  The base of address range.
...
)
{
    ...
    if(isHwResetOnInit == GT_TRUE)
    {
        osPrintSync("osCacheDmaFreeAll : destroy All DMA allocations \n");
        osCacheDmaFreeAll();
    }
...
*
*/
GT_STATUS cpssAppDemoResetSystem(
...
/**
* @internal cpssAppDemoDevSupportSystemResetSet function
* @endinternal
*
...
* @retval GT_OK                    - on success else if failed
*/
GT_STATUS cpssAppDemoDevSupportSystemResetSet(
...
)
{
    ...
    GT_U32  numOfCoresOnPex = 0;/* the Lion2/Hooper recognized on the PEX as
                               the number of the cores */
    GT_U32  isCaelum = 0;/* when 1 caelum exists */
    ...
    switch(firstDevId)
    {
        ...
        case CPSS_AC5X_ALL_DEVICES_CASES_MAC:
            primaryFamily = CPSS_PP_FAMILY_DXCH_AC5X_E;
            break;
        case CPSS_BOBCAT2_ALL_DEVICES_CASES_MAC:
            primaryFamily = CPSS_PP_FAMILY_DXCH_BOBCAT2_E;
            break;
    ...
    switch(secondaryDevId)
    {
        case CPSS_BOBK_CAELUM_DEVICES_CASES_MAC:
            ...
            break;
        case CPSS_AC5X_ALL_DEVICES_CASES_MAC:
            secondaryFamily = CPSS_PP_FAMILY_DXCH_AC5X_E;
            break;
        case CPSS_BOBCAT2_ALL_DEVICES_CASES_MAC:
            secondaryFamily = CPSS_PP_FAMILY_DXCH_BOBCAT2_E;
            break;
    ...
    if((numOfPp == 2) && isCaelum &&
       ((primaryFamily   == CPSS_PP_FAMILY_DXCH_AC5X_E) ||
        (secondaryFamily == CPSS_PP_FAMILY_DXCH_AC5X_E)))
    {
        cpssOsPrintf("Recognized 'complex' board of AC5X and Caelum devices \n");
        *boardIdxPtr   = 26;
        *boardRevIdPtr =  1;
    }
    else
...
start_devId_classification_lbl:
    switch(devId)
    {
        case CPSS_PIPE_ALL_DEVICES_CASES_MAC:
            boardIdx = 33;
        ...
        default:
            if(useDebugDeviceId == GT_TRUE &&
                deviceIdDebug[0] != 0 &&
                devId != deviceIdDebug[0])
            {
                devId = deviceIdDebug[0];
                goto start_devId_classification_lbl;
            }
...
)
{
    ...
    START_LOOP_ALL_DEVICES(devIndex,devNum)
    {
        ...
        /*unmask interrupts*/
        hwInfo = cpssDrvHwPpHwInfoStcPtrGet(devNum,CPSS_PORT_GROUP_UNAWARE_MODE_CNS);
        if (hwInfo != NULL)
        {
            rc = cpssExtDrvIntEnable(hwInfo->intMask.switching);
            if (GT_OK != rc)
            {
                return rc;
            }
...
* @param[in] unitRegsPtr              - (pointer to)unit registers description
...
)
{
    ...
    GT_U32    unitBaseAddr;
    GT_BOOL   errorValue;
    const APP_DEMO_REG_PRINT_DATA_STC *regDataPtr;
    const APP_DEMO_REG_PRINT_FIELD_STC *fieldsArrPtr;
    const GT_U32 regsPerLine = 16;
    ...
    cpssOsPrintf("Unit: %s instance %d \n", unitRegsPtr->name, unitInstance);
    unitBaseAddr = prvCpssDxChHwUnitBaseAddrGet(
        devNum, unitRegsPtr->prvUnitIdArrPtr[unitInstance], &errorValue);
    if (errorValue != GT_FALSE)
    {
        cpssOsPrintf("Invalid Unit Id \n");
        return GT_BAD_PARAM;
    }

    for (regDataPtr = unitRegsPtr->registersArrPtr;
         (regDataPtr->name != NULL); regDataPtr++)
    {
        if (regDataPtr->regsAmount <= 1)
        {
            /* single register */
            cpssOsPrintf(
                "REG(0x%04X): %s; ", regDataPtr->addrBaseOffset, regDataPtr->name);
            regAddr = unitBaseAddr + regDataPtr->addrBaseOffset;
            rc = prvCpssHwPpReadRegister(
                devNum, regAddr, &(regDataArr[0]));
            if (rc != GT_OK)
            {
                cpssOsPrintf("register read error address 0x%08X \n", regAddr);
                return rc;
            }
            if (regDataPtr->fieldsArrPtr)
            {
                for (fieldsArrPtr = regDataPtr->fieldsArrPtr;
                     (fieldsArrPtr->length != 0); fieldsArrPtr++)
                {
                    mask = (1 << fieldsArrPtr->length) - 1;
                    val = (regDataArr[0] >> fieldsArrPtr->startBit) & mask;
                    cpssOsPrintf(
                        (fieldsArrPtr->hexPrint ? " %s 0x%X" : " %s %d"),
                        fieldsArrPtr->name, val);
                }
            }
            else
            {
                cpssOsPrintf("data 0x%08X", regDataArr[0]);
            }
            cpssOsPrintf("\n");
        }
        else
        {
            /* set of registers with the same fields */
            for (regIdxLow = 0; (regIdxLow < regDataPtr->regsAmount); regIdxLow += regsPerLine)
            {
                regIdxHigh = regIdxLow + regsPerLine - 1;
                if (regIdxHigh >= regDataPtr->regsAmount)
                {
                    regIdxHigh = regDataPtr->regsAmount - 1;
                }
                cpssOsPrintf(
                    "REG(0x%04X)[%d - %d]: %s \n", regDataPtr->addrBaseOffset,
                    regIdxLow, regIdxHigh, regDataPtr->name);
                regAddr = unitBaseAddr + regDataPtr->addrBaseOffset
                    + (regIdxLow * regDataPtr->addrStep);
                regsInLine = (1 + regIdxHigh - regIdxLow);
                for (ii = 0; (ii < regsInLine); ii++)
                {
                    rc = prvCpssHwPpReadRegister(
                        devNum, (regAddr + (ii * regDataPtr->addrStep)), &(regDataArr[ii]));
                    if (rc != GT_OK)
                    {
                        cpssOsPrintf(
                            "register read error address 0x%08X \n",
                            (regAddr + (ii * regDataPtr->addrStep)));
                        return rc;
                    }
                }
                if (regDataPtr->fieldsArrPtr)
                {
                    for (fieldsArrPtr = regDataPtr->fieldsArrPtr;
                         (fieldsArrPtr->length != 0); fieldsArrPtr++)
                    {
                        cpssOsPrintf("%s", fieldsArrPtr->name);
                        mask = (1 << fieldsArrPtr->length) - 1;
                        for (ii = 0; (ii < regsInLine); ii++)
                        {
                            val = (regDataArr[ii] >> fieldsArrPtr->startBit) & mask;
                            cpssOsPrintf(
                                (fieldsArrPtr->hexPrint ? " 0x%X" : " %d"), val);
                        }
                        cpssOsPrintf("\n");
                    }
                }
                else
                {
                    ...
                    for (ii = 0; (ii < regsInLine); ii++)
                    {
                        cpssOsPrintf(" 0x%08X", regDataArr[ii]);
                    }
                    cpssOsPrintf("\n");
                }
            }
