...
#define WM_OK       0
#define WM_ERROR    1
#define WM_FULL                  (0x14) /* Item is full (Queue or table etc...)  */
...
/* struct : WM_SPECIFIC_DEVICE_INFO_STC
...
*/
typedef struct {
    /** @brief : the pci Device Id , the 16 bits in address 0x0 of the PCI config space
     *  used if the value != 0xFFFF
     *  allow forcing specific <pci device id> 0x9061 instead of 0x9041 that
     *  comes from the 'register file'.
    */
    GT_U16      pciDeviceId;
    /** @brief : the revision , the 4 bits in address 0x8 of the PCI config space
     *  used if the value not more than 0xF
    */
    GT_U32      revision;

}WM_SPECIFIC_DEVICE_INFO_STC;
