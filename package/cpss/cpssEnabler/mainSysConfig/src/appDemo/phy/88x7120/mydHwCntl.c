...
extern MYD_32 usleep(MYD_U32 usec);
/******************************************************************************
 ...
 * @return MYD_STATUS
 */
MYD_STATUS mydHwCtrlRWMutexCreate()
{
    char buf[64]={0};
 ...
 * @return MYD_STATUS
 */
MYD_STATUS mydHwCtrlRWMutexDestroy()
{
...
MYD_STATUS wrMydCtrlXsmiRead(
    ...
    MYD_U16* value)
{
    GT_APPDEMO_XPHY_INFO_STC*       phyInfo = (GT_APPDEMO_XPHY_INFO_STC*)pDev->hostContext;
    GT_STATUS                       rc;

    /* Before switch xsmi chanel in cpld, we need make sure the last transaction is done */
    cpssOsMutexLock(xsmi_extn_mtx);

    gtAppDemoPhyXSmiExtention(phyInfo,NULL);
    rc = cpssXsmiRegisterRead(phyInfo->hostDevNum,phyInfo->xsmiInterface,mdioPort,reg,mmd,(GT_U16*)value);

    cpssOsMutexUnlock(xsmi_extn_mtx);
    ...
    MYD_U16 value)
{
    ...
    /* Before switch xsmi chanel in cpld, we need make sure the last transaction is done */
    cpssOsMutexLock(xsmi_extn_mtx);
    rc = gtAppDemoPhyXSmiExtention(phyInfo,NULL);

    rc = cpssXsmiRegisterWrite(phyInfo->hostDevNum,phyInfo->xsmiInterface,mdioPort,reg,mmd,(GT_U16)value);
