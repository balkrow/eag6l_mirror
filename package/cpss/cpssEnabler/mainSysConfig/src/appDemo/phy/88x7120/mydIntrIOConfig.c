...
/*******************************************************************
 MYD_STATUS mydGetIntrSrcStatus
     ...
     See the mydAPI.h for structure and bits definition.
*******************************************************************/
MYD_STATUS mydGetIntrSrcStatus
(
    IN MYD_DEV_PTR pDev,
...
{
    MYD_U16 mdioPort, mdioPortIndex;
    MYD_U16 indexStart = MYD_LINE_SIDE;
    MYD_U16 indexEndEx = MYD_HOST_SIDE+1;
    MYD_U16 host_or_line;
...
MYD_STATUS mydSetGPIOInterruptConfig
    Set the Level 3 GPIO Interrupt Configurations (Enable/Disable and Type)
*******************************************************************************/
MYD_STATUS mydSetGPIOInterruptConfig
(
    IN MYD_DEV_PTR pDev,
...
)
{
    ...
    {
        MYD_DBG_ERROR("mydSetGPIOInterruptConfig: The given Pin is not configured to the MYD_PIN_MODE_GPIO.\n");
        return MYD_FAIL;
    }
    ...
    {
        MYD_DBG_ERROR("mydSetGPIOInterruptConfig: The GPIO interrupt only works when the Pin is set to MYD_GPIO_INPUT_MODE.\n");
        return MYD_FAIL;
    }

    ATTEMPT(mydGPIOPinGetPortAndReg(pDev, gpioPinId, &GPIOPort, &GPIOCtrlReg));

    ATTEMPT(mydHwXmdioRead(pDev, GPIOPort, MYD_PORT_REG, GPIOCtrlReg, &regVal));
    ATTEMPT(mydHwSetRegFieldToWord(regVal, gpioIntrEnable, 12, 1, &regVal));
    ATTEMPT(mydHwSetRegFieldToWord(regVal, gpioIntrType, 8, 3, &regVal));
    ATTEMPT(mydHwXmdioWrite(pDev, GPIOPort, MYD_PORT_REG, GPIOCtrlReg, regVal));
...
MYD_STATUS mydGetGPIOInterruptConfig
    Get the Level 3 GPIO Interrupt Configurations (Enable/Disable and Type)
*******************************************************************************/
MYD_STATUS mydGetGPIOInterruptConfig
(
    IN MYD_DEV_PTR pDev,
...
)
{
    ...
    if (pinMode != MYD_PIN_MODE_GPIO)
    {
        MYD_DBG_ERROR("mydGetGPIOInterruptConfig: The given Pin is not configured to the MYD_PIN_MODE_GPIO.\n");
        return MYD_FAIL;
    }
    ...
    if (gpioDirection != MYD_GPIO_INPUT_MODE)
    {
        MYD_DBG_ERROR("mydGetGPIOInterruptConfig: The GPIO interrupt only works when the Pin is set to MYD_GPIO_INPUT_MODE.\n");
        return MYD_FAIL;
    }
    ...
    ATTEMPT(mydHwXmdioRead(pDev, GPIOPort, MYD_PORT_REG, GPIOCtrlReg, &regVal));
    ATTEMPT(mydHwGetRegFieldFromWord(regVal, 12, 1, gpioIntrEnable));
    ATTEMPT(mydHwGetRegFieldFromWord(regVal, 8, 3, gpioIntrType));
...
/*******************************************************************************
 MYD_STATUS mydConfigRclkMux
    ...
    Option includes source selection, lane selection, divide ratio and divider configs
*******************************************************************************/
MYD_STATUS mydConfigRclkMux
(
    IN MYD_DEV_PTR pDev,
...
)
{
    ...
    if (pDev->deviceId == MYD_DEV_X7120)
    {
        ...
        for (portIndex = 0; portIndex < pDev->portCount; ++portIndex)
        {
            ...
            {
                ATTEMPT(mydRClkMuxInputCheck(portIndex, interfaceIndex, 1, rClkMux.x7120.mux1[portIndex][interfaceIndex]));
                ATTEMPT(mydRClkMuxInputCheck(portIndex, interfaceIndex, 2, rClkMux.x7120.mux2[portIndex][interfaceIndex]));
                ...
                for (muxOptionIdx = 1; muxOptionIdx <= 2; ++muxOptionIdx)
                {
                    if (muxOptionIdx == 1) /* Option 1 */
                    {
                        ...
                        rClkMuxOptionPtr = &(rClkMux.x7120.mux1[portIndex][interfaceIndex]);
                    }
                    else /* Option 2 */
                    {
                        rcvClkCtrlReg = (interfaceIndex==MYD_HOST_MODE_IDX) ? MYD_HOST_RCVCLK_CTRL2 : MYD_LINE_RCVCLK_CTRL2;
                        rClkMuxOptionPtr = &(rClkMux.x7120.mux2[portIndex][interfaceIndex]);
                    }
    ...
    else if (pDev->deviceId == MYD_DEV_X6141)
    {
        ...
        for (interfaceIndex = MYD_HOST_MODE_IDX; interfaceIndex <= MYD_LINE_MODE_IDX; ++interfaceIndex)
        {
            ATTEMPT(mydRClkMuxInputCheck(0, interfaceIndex, 1, rClkMux.x6141.mux1[interfaceIndex]));
            ATTEMPT(mydRClkMuxInputCheck(0, interfaceIndex, 2, rClkMux.x6141.mux2[interfaceIndex]));
            ...
            for (muxOptionIdx = 1; muxOptionIdx <= 2; ++muxOptionIdx)
            {
                ...
                {
                    rcvClkCtrlReg = (interfaceIndex==MYD_HOST_MODE_IDX) ? MYD_HOST_RCVCLK_CTRL1 : MYD_LINE_RCVCLK_CTRL1;
                    rClkMuxOptionPtr = &(rClkMux.x6141.mux1[interfaceIndex]);
                }
                else /* Option 2 */
                {
                    rcvClkCtrlReg = (interfaceIndex==MYD_HOST_MODE_IDX) ? MYD_HOST_RCVCLK_CTRL2 : MYD_LINE_RCVCLK_CTRL2;
                    rClkMuxOptionPtr = &(rClkMux.x6141.mux2[interfaceIndex]);
                }
