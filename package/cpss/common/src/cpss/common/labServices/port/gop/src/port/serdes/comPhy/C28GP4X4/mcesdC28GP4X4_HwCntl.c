...
)
{
    ...
    {
        MCESD_DBG_INFO("fmcesdWriteReg 0x%X failed to write to dev=C28GP4X4, reg=0x%X\n", value, reg);
        return MCESD_FAIL;
    }
...
)
{
    ...
    {
        MCESD_DBG_INFO("fmcesdReadReg failed from dev=C28GP4X4, reg=0x%X\n", reg);
        return MCESD_FAIL;
    }
...
)
{
    ...
    {
        MCESD_DBG_INFO("fmcesdSetPinCfg 0x%X failed to configure dev=C28GP4X4, pin=0x%X\n", pinValue, pin);
        return MCESD_FAIL;
    }
...
)
{
    ...
    {
        MCESD_DBG_INFO("fmcesdGetPinCfg failed from dev=C28GP4X4, pin=0x%X\n", pin);
        return MCESD_FAIL;
    }
...
)
{
    ...
    {
        MCESD_DBG_INFO("fmcesdWaitFunc failed from dev=C28GP4X4\n");
        return MCESD_FAIL;
    }
