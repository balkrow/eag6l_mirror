...
void prvCpssLogParamFunc_CPSS_PP_PHASE2_UPLOAD_QUEUE_TYPE_ENT_PTR_PTR(
    IN CPSS_LOG_LIB_ENT            contextLib,
    IN CPSS_LOG_TYPE_ENT           logType,
    IN GT_CHAR_PTR                 namePtr,
    IN va_list                   * argsPtr,
...
{
    PRV_CPSS_LOG_SET_PARAM_VAL_MAC(CPSS_PP_PHASE2_UPLOAD_QUEUE_TYPE_ENT**, paramVal);
    if (paramVal == NULL || *paramVal == NULL)
    {
        PRV_CPSS_LOG_AND_HISTORY_PARAM_MAC(contextLib, logType, "%s = NULL\n", namePtr);
        return;
    }
    PRV_CPSS_LOG_ENUM_MAC(namePtr, **paramVal, CPSS_PP_PHASE2_UPLOAD_QUEUE_TYPE_ENT);
}
