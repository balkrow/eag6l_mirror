...
#include <LinkCryptPtp/mtdMsecTypes.h>
#include <LinkCryptPtp/mtdMsecInternal.h>
#include <LinkCryptPtp/mtdPtp1step.h>
#include <LinkCryptPtp/mtdPtpApi.h>
#include <mpdTypes.h>


MTD_STATIC MTD_UINT mtd_pr_init_data[] =
{
...
MTD_STATIC MTD_UINT mtd_ur_init_data[] =
{
...
/* Read lut entry */
MTD_STATUS mtd_read_lut_entry (MTD_U16 port, MTD_64 *lut_key, MTD_64 *lut_valid, MTD_32 *lut_key_top, MTD_32 *lut_valid_top, MTD_64 *lut_action, short lut_index, int path )
{
    MTD_STATUS   retVal = MTD_OK;
...
MTD_STATUS mtd_ptp_1step_display_contents(MTD_U16 port, MTD_U16 addr, int size)
{
    MTD_STATUS   retVal = MTD_OK;
    MPD_UNUSED_PARAM(port);
    MPD_UNUSED_PARAM(addr);
    MPD_UNUSED_PARAM(size);
#ifdef MTD_DEBUG
    int index;
...
MTD_U64 mtd_ptp_lut_act_adjust ( MTD_PTP_1STEP_LUT_ACT name, MTD_U64 value )
{
	...
	/* KW */
    if (name < 0){
    	MTD_DBG_ERROR("mtd_ptp_lut_act_adjust: name out of bounds %d, \n", name);
    	return dataLong;
    }
	/* End KW */
...
MTD_STATIC MTD_STATUS getPTPTsQueue(MtdPtpTsQueueStruct *tsQueue, MTD_U16 *buf)
{
    MTD_STATUS   retVal = MTD_OK;
    int i;
    ...
    tsQueue->transpSpec  = (*(buf + 7)>>12)&0xf;
    tsQueue->msgId  = (*(buf + 7)>>8)&0xf;
    
    tsQueue->ts.extTs.nanoseconds = *(buf + 1);
    tsQueue->ts.extTs.nanoseconds <<= 16;
    tsQueue->ts.extTs.nanoseconds |= ((unsigned int)*(buf + 0)&0xFFFF);
    
    tsQueue->ts.extTs.topseconds = *(buf + 3);
    tsQueue->ts.extTs.seconds = *(buf + 2);
    
    tsQueue->ts.extTs.seconds <<= 16;
    tsQueue->ts.extTs.seconds |= ((unsigned int)*(buf + 5)&0xFFFF);
    for (i=0; i<8;i++)
    {
    ...
    short lut_index, int path )
{
    ...
    MTD_64 tmp_lut_key;
    MTD_64 tmp_lut_vld;
    MTD_32 tmp_lut_key_top;
    MTD_32 tmp_lut_vld_top;
    MTD_64 tmp_lut_act; 
...
#ifdef MPD_DISABLED
    if (regname < 0)/*reg_addr is unsigned ... can't be less than 0 */
    {
