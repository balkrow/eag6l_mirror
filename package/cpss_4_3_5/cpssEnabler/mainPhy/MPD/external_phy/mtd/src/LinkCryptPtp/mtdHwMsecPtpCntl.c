...
#include <LinkCryptPtp/mtdMsecPtpApiDefs.h>
#include <LinkCryptPtp/mtdMsecInternal.h>
#include <LinkCryptPtp/mtdPtp1step.h>
...
MTD_STATIC MTD_INT indirect = 1; /* select indirect or direct access registers, this is not the same as MTD_MACSEC_INDIRECT_ACCESS! */
...
)
{
    ...
    if (devPtr->devInfo & MTD_MACSEC_INDIRECT_ACCESS)
    {
        /* check if already busy with another command */
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,MTD_MACSEC_ADDR_STATUS,&readResult));
        ...
        /* write 'write' command */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_CNTL,MTD_MACSEC_WR_CMD));

        /* write data */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_LOW_WORD,(MTD_U16)(value)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_HIGH_WORD,(MTD_U16)(value >> 16)));

        /* write address to start 'write' process */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_ADDR_STATUS,reg));
    ...
    {
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg,(MTD_U16)(value & 0xFFFF)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg+1,(MTD_U16)(value>>16)));
    }

    return MTD_OK;
}

MTD_STATIC MTD_STATUS mtdHwMsecReadTry(MTD_DEV_PTR devPtr, MTD_U16 port, MTD_U16 dev, MTD_U16 reg,MTD_U32 *data);
...
MTD_STATIC MTD_STATUS mtdHwMsecReadTry(MTD_DEV_PTR devPtr, MTD_U16 port, MTD_U16 dev, MTD_U16 reg,MTD_U32 *data)
{
    ...
    if (devPtr->devInfo & MTD_MACSEC_INDIRECT_ACCESS)
    {
        ...
        /* write 'read' command */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_CNTL,MTD_MACSEC_RD_CMD));
        ...
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,MTD_MACSEC_LOW_WORD,&readResult));
        *data = (MTD_U32)readResult; /* assign low word */
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,MTD_MACSEC_HIGH_WORD,&readResult));
    ...
    else
    {
        ...
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,dev,reg,&dummy));

        /* now make sure the address read is correct */
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,dev,0x97FD,&dummy));
        ...
        *data = 0;
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,dev,0x97FE,&dummy));
        *data = dummy;
        /* now read the high word */
        MTD_ATTEMPT(mtdHwXmdioRead(devPtr,port,dev,0x97FF,&dummy));
...
)
{
    ...
    if (useIndirectAddressing == MTD_TRUE)
    {
        ...
        /* write 'write' command */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_CNTL,MTD_MACSEC_DBLWR_CMD));
        ...
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_LOW_WORD,(MTD_U16)(value >> 32))); /* write low word of reg + 2 */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_HIGH_WORD,(MTD_U16)(value >> 48)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_LOW_WORD,(MTD_U16)(value)));       /* write low word of reg + 0 */
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,MTD_MACSEC_HIGH_WORD,(MTD_U16)(value >> 16)));
    ...
    {
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg,(MTD_U16)(value & 0xFFFF)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg+1,(MTD_U16)((value & 0xFFFF0000ULL)>>16)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg+2,(MTD_U16)((value & 0xFFFF00000000ULL)>>32)));
        MTD_ATTEMPT(mtdHwXmdioWrite(devPtr,port,dev,reg+3,(MTD_U16)((value & 0xFFFF000000000000ULL)>>48)));
    }
...
)
{
    ...
    MTD_ATTEMPT(mtdHwMsecRead(devPtr,port,dev,regAddr,&dummy));

    MTD_ATTEMPT(mtdHwGetRegFieldFromLongWord(dummy,fieldOffset,fieldLength,data));
...
)
{
    ...
    MTD_ATTEMPT(mtdHwMsecSetRegFieldToLongWord(dummy,data,fieldOffset,fieldLength,&dummy2));

    MTD_ATTEMPT(mtdHwMsecWrite(devPtr,port,dev,regAddr,dummy2));
...
)
{
    ...
    if(dev->devInfo & MTD_MACSEC_INDIRECT_ACCESS)
    {
...
MTD_STATIC MTD_STATUS _mtdMsecReadPortRegTry (MTD_DEV *dev,MTD_U16 port,MTD_U16 addr,MTD_U32   *data);
