...
)
{
    ...
    *result = MYD_STATUS_MODE_GENERAL_ERROR;
    ...
    if (mdioPortIndex >= MYD_MAX_PORTS) {
        MYD_DBG_ERROR("mydSetModeSelection: mdioPortIndex out of bounds %d\n",mdioPortIndex);
        *result = MYD_STATUS_MODE_GENERAL_ERROR;
        return MYD_FAIL;
    }
	/* End KW */
    if (maxLaneUsed == MYD_8_LANE_MODE)
    {
    	...
    	/* KW */
        if (mdioPortIndex+1 >= MYD_MAX_PORTS) {
            MYD_DBG_ERROR("mydSetModeSelection: mdioPortIndex+1 out of bounds %d\n",mdioPortIndex);
            *result = MYD_STATUS_MODE_GENERAL_ERROR;
            return MYD_FAIL;
        }
    	/* End KW */
...
)
{
    ...
    MYD_U16 modeType,mdioIndex;

    MYD_CHECK_DEV(pDev, mdioPort, laneOffset);
    mdioIndex = MYD_GET_PORT_IDX(pDev, mdioPort);
    if (mdioIndex >= MYD_MAX_PORTS) {
    	PRV_MPD_HANDLE_FAILURE_MAC(mdioPort, MPD_ERROR_SEVERITY_FATAL_E, "mdioIndex out of range");
        return MPD_OP_FAILED_E;
    }
    hostOpMode = pDev->hostConfig[mdioIndex][laneOffset].opMode;
    lineOpMode = pDev->lineConfig[mdioIndex][laneOffset].opMode;
...
)
{
    MYD_U16 portIndex;
    MYD_U16 laneOffset;
    MYD_U16 timerOption = 0;

    switch (extendANegTimerOption)
    {
        case MYD_ANEG_TIMER_0:
            timerOption = 0x0;
            break;

        case MYD_ANEG_TIMER_1:
            timerOption = 0x15;
            break;

        case MYD_ANEG_TIMER_2:
            timerOption = 0x2A;
            break;

        case MYD_ANEG_TIMER_3:
            timerOption = 0x3F;
            break;

        default:
            MYD_DBG_ERROR("mydExtendDeviceAutoNegTimer: Invalid extendANegTimerOption: %u\n", extendANegTimerOption);
            return MYD_FAIL;
            break;
    }
    ...
    for (portIndex=0; portIndex<pDev->portCount; portIndex++)
    {
        for (laneOffset=0; laneOffset<MYD_NUM_LANES; laneOffset++)
        {
            ATTEMPT(mydHwSetPhyRegField(pDev, pDev->mdioPort+portIndex, MYD_AUTONEG,
                                        0x8001+(0x0200*laneOffset), 9, 6, timerOption));
            ATTEMPT(mydHwSetPhyRegField(pDev, pDev->mdioPort+portIndex, MYD_AUTONEG,
                                        0x9001+(0x0200*laneOffset), 9, 6, timerOption));
        }
    }
...
/*******************************************************************
 ...
 Access NIMB register to set the min/max to trigger the iCal calibration
*******************************************************************/
MYD_STATUS mydNIMBSetEyeCoeff
(
    IN MYD_DEV_PTR pDev,
...
)
{
    ...
    ATTEMPT(mydGetChipFWRevision(pDev, mdioPort, &major, &minor));
    
    /* Query the status and return MYD_OK for unsupported firmware */
    if (major == 0)
    {
        MYD_DBG_ERROR("mydNIMBSetEyeCoeff: unsupported firmware version %u.%u\n", major, minor);
        return MYD_FAIL;
    }

    switch (eyeCoeffOptions)
    {
        case MYD_NIMB_EYE_DELTA_DEFAULT:
        {
            /* Trigger iCal if min eye value setting */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F6, 0xA));
            /* Trigger iCal if max eye deviation to 6-eye average */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F7, 0xF));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F8, 0xF));
            break;
        }

        case MYD_NIMB_EYE_DELTA_UPDATE:
        {
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F6, minEyeVal));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F7, maxEyeDeltaLine));
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F8, maxEyeDeltaHost));
            break;
        }

        default:
        {
            MYD_DBG_ERROR("mydNIMBSetEyeCoeff: invalid eyeCoeffOptions: %u\n", eyeCoeffOptions);
            return MYD_FAIL;
            break;
        }
    }
...
/*******************************************************************
 ...
 Access NIMB register to set the operation to start/stop the iCal 
...
(
    ...
    IN MYD_U16 mdioPort,
    IN MYD_U16 laneOffset, /* unused. reserve for future */
    IN MYD_U16 opOption    /* refer to MYD_NIMB_OP_xxx options */
)
{
    MYD_U16 major, minor;
    ...
    if (major == 0)
    {
        MYD_DBG_ERROR("mydNIMBSetOp: unsupported firmware version %u.%u\n", major, minor);
        return MYD_FAIL;
    }

    switch(opOption)
    {
        case MYD_NIMB_OP_ENABLED:
            /* start operations */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0x0)); /* toggle */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0x1));
            break;

        case MYD_NIMB_OP_DISABLED:
            /* stop operations */
            ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0, 2, 0x0));
            break;

        case MYD_NIMB_OP_TEMP_RAMP:
            /* start temperature ramp and all other operations */
            ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0x0)); /* toggle */
            ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, MYD_PMA_DEV, 0xC8F4, 0, 2, 0x3));
            break;

        default:
            MYD_DBG_ERROR("mydNIMBSetOp: invalid option: %u\n", opOption);
            return MYD_FAIL;
            break;
    }
    MYD_UNUSED_PARAM(laneOffset);
...
/*******************************************************************
 ...
 Access NIMB register for operation status
*******************************************************************/
MYD_STATUS mydNIMBGetOpStatus
(
    IN MYD_DEV_PTR pDev,
    IN MYD_U16 mdioPort,
    IN MYD_U16 laneOffset, /* unused. reserve for future */
    OUT MYD_U16 *opStatus /* 1 = running, 0 = stop */
)
{
    MYD_U16 major, minor;
    ...
    *opStatus = 0;
    ...
    ATTEMPT(mydHwXmdioRead(pDev, mdioPort, MYD_PMA_DEV, 0xC8F5, opStatus));
...
/*******************************************************************
 ...
 Access NIMB register to set the ANEG initiator lane
    15:12 Reset lane (debugging)
...
)
{
    ...
    {   
        MYD_DBG_ERROR("mydNIMBSetANegLaneSwap: incorrect host/side selection: %u\n",
                       host_or_line);                                      
        return MYD_FAIL;
    }                                                                          
    ...
    if (major == 0)
    {
        MYD_DBG_ERROR("mydNIMBSetANegLaneSwap: unsupported firmware version %u.%u\n", major, minor);
        return MYD_FAIL;
    }

    ATTEMPT(mydHwSetPhyRegField(pDev, mdioPort, host_or_line, 0xF08F, 0, 12, autoNegLaneSwapVal));
...
/*******************************************************************
 ...
 Access NIMB register to set the temperature ramp options
*******************************************************************/
MYD_STATUS mydNIMBSetTempRamp
(
    IN MYD_DEV_PTR pDev,
...
{
    MYD_U16 major, minor;
    MYD_U16 threshold1, threshold2, threshold3, threshold4;
    ...
    if (major == 0)
    {
        MYD_DBG_ERROR("mydNIMBSetTempRamp: unsupported firmware version %u.%u\n", major, minor);
        return MYD_FAIL;
    }
    ...
    else
    {
        ...
        threshold4 = tempThreshold4;
    }

    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_LINE_SIDE, 0xF091, threshold1));
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_HOST_SIDE, 0xF091, threshold1));

    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_LINE_SIDE, 0xF092, threshold2));
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_HOST_SIDE, 0xF092, threshold2));

    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_LINE_SIDE, 0xF093, threshold3));
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_HOST_SIDE, 0xF093, threshold3));

    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_LINE_SIDE, 0xF094, threshold4));
    ATTEMPT(mydHwXmdioWrite(pDev, mdioPort, MYD_HOST_SIDE, 0xF094, threshold4));
