		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (!discover_retries || packet_num < discover_retries) {
					...
					/* broadcast */
#if 1 /*[#422] mv switch port down when dhcp discover, balkrow, 2023-02-01*/
					if(!strcmp(client_data.interface, "mplane1"))
						system("ifconfig cfiu3 down");
					else
						system("ifconfig cfiu1 down");

						send_discover(requested_ip);

					if(!strcmp(client_data.interface, "mplane1"))
						system("ifconfig cfiu3 up");
					else
						system("ifconfig cfiu1 up");
#endif
