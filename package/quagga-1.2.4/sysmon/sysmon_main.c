...
struct zebra_privs_t sysmon_privs =
{
...
char config_default[] = SYSCONFDIR SYSMON_DEFAULT_CONFIG;
...
/* SYSMON VTY bind address. */
char *vty_addr = NULL;

/* SYSMON VTY connection port. */
int vty_port = SYSMON_VTY_PORT;
...
Daemon which manages sysmon version 1 and 2.\n\n\
...
main (int argc, char **argv)
{
  ...
  zlog_default = openzlog (progname, ZLOG_SYSMON,
  ...
  while (1) 
	...
	case 'C':
	  ...
	  break;
	case 'u':
	  sysmon_privs.user = optarg;
	  break;
	case 'g':
	  sysmon_privs.group = optarg;
	  break;
	case 'v':
